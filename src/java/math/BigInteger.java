 ...
 * {@code >>>}) is omitted, as this operation makes little sense in combination
 * with the "infinite word size" abstraction provided by this class.
 *
 * <p>
 * Semantics of bitwise logical operations exactly mimic those of Java's bitwise
 * integer operators. The binary operators ({@code and}, {@code or}, {@code xor}
 * ) implicitly perform sign extension on the shorter of the two operands prior
 ...
 * of their operand. If necessary, the operand is sign- extended so that it
 * contains the designated bit. None of the single-bit operations can produce a
 * BigInteger with a different sign from the BigInteger being operated on, as
 * they affect only a single bit, and the "infinite word size" abstraction
 ...
 * {@code (i + j)} is shorthand for "a BigInteger whose value is that of the
 * BigInteger {@code i} plus that of the BigInteger {@code j}." The pseudo-code
 * expression {@code (i == j)} is shorthand for "{@code true} if and only if the
 * BigInteger {@code i} represents the same value as the BigInteger {@code j}."
 ...
 *
 * BigInteger must support values in the range -2
 * <sup>{@code Integer.MAX_VALUE}</sup> (exclusive) to +2
 * <sup>{@code Integer.MAX_VALUE}</sup> (exclusive) and may support values
 * outside of that range.
 *
 * The range of probable prime values is limited and may be less than the full
 * supported positive range of {@code BigInteger}. The range must be at least 1
 * to 2<sup>500000000</sup>.
 *
 * @implNote BigInteger constructors and operations throw
 *           {@code ArithmeticException} when the result is out of the supported
 *           range of -2<sup>{@code Integer.MAX_VALUE}</sup> (exclusive) to +2
 *           <sup>{@code Integer.MAX_VALUE}</sup> (exclusive).
 *
 * @see BigDecimal
 * @author Josh Bloch
...
public class BigInteger extends Number implements Comparable<BigInteger> {
     ...
     * {@code mag[0]}) must be non-zero. This is necessary to ensure that there
     * is exactly one representation for each BigInteger value. Note that this
     * implies that the BigInteger zero has a zero-length mag array.
     */
    final int[] mag;
     ...
     * @serial
     * @see #bitCount
     * @deprecated Deprecated since logical value is offset from stored value
     *             and correction factor is applied in accessor method.
     */
    @Deprecated
     ...
     * BigInteger that contains a nonzero int, or -2 (either value is
     * acceptable). The least significant int has int-number 0, the next int in
     * order of increasing significance has int-number 1, and so forth.
     * 
     * @deprecated Deprecated since logical value is offset from stored value
     *             and correction factor is applied in accessor method.
     */
    ...
    /**
     * The threshold value for using 3-way Toom-Cook multiplication. If the
     * number of ints in each mag array is greater than the Karatsuba threshold,
     * and the number of ints in at least one of the mag arrays is greater than
     * this threshold, then Toom-Cook multiplication will be used.
     */
    private static final int TOOM_COOK_THRESHOLD = 240;

    /**
     * The threshold value for using Karatsuba squaring. If the number of ints
     * in the number are larger than this value, Karatsuba squaring will be
     * used. This value is found experimentally to work well.
     */
    private static final int KARATSUBA_SQUARE_THRESHOLD = 128;

    /**
     * The threshold value for using Toom-Cook squaring. If the number of ints
     * in the number are larger than this value, Toom-Cook squaring will be
     * used. This value is found experimentally to work well.
     */
     ...
     * ints in the divisor are larger than this value, Burnikel-Ziegler division
     * may be used. This value is found experimentally to work well.
     */
    static final int BURNIKEL_ZIEGLER_THRESHOLD = 80;

    /**
     * The offset value for using Burnikel-Ziegler division. If the number of
     * ints in the divisor exceeds the Burnikel-Ziegler threshold, and the
     ...
     * the number of ints in the number are larger than this value, the
     * Schoenhage algorithm will be used. In practice, it appears that the
     * Schoenhage routine is faster for any threshold down to 2, and is
     * relatively flat for thresholds between 2-25, so this choice may be varied
     * within this range for very small effect.
     */
    private static final int SCHOENHAGE_BASE_CONVERSION_THRESHOLD = 20;

    /**
     * The threshold value for using squaring code to perform multiplication of
     * a {@code BigInteger} instance by itself. If the number of ints in the
     * number are larger than this value, {@code multiply(this)} will return
     * {@code square()}.
     */
    private static final int MULTIPLY_SQUARE_THRESHOLD = 20;

    /**
     * The threshold for using an intrinsic version of implMontgomeryXXX to
     * perform Montgomery multiplication. If the number of ints in the number is
     * more than this value we do not use the intrinsic.
     */
     ...
     * a BigInteger value of 0, whether signum is -1, 0 or 1.
     *
     * @param signum
     ...
     *                               legal values (-1, 0,
     *                               and 1), or {@code signum} is 0 and
     *                               {@code magnitude} contains
     *                               one or more non-zero bytes.
     */
     ...
     * optional minus or plus sign followed by a sequence of one or more digits
     * in the specified radix. The character-to-digit mapping is provided by
     * {@code
     ...
     *                               of a BigInteger in
     *                               the specified radix, or {@code radix} is
     *                               outside the range
     *                               from {@link Character#MIN_RADIX} to
    ...
    public BigInteger(String val, int radix) {
        ...
        while (cursor < len && Character.digit(val.charAt(cursor),
                radix) == 0) {
            cursor++;
        }
    ...
    /*
     * Constructs a new BigInteger using a char array with radix=10. Sign is
     * precalculated outside and not allowed in the val.
     */
     ...
     * the range 0 to (2<sup>{@code numBits}</sup> - 1), inclusive. The
     * uniformity of the distribution assumes that a fair source of random bits
     * is provided in {@code rnd}. Note that this constructor always constructs
     ...
     *                  to
     *                  tolerate. The probability that the new BigInteger
     *                  represents a
     *                  prime number will exceed (1 -
     *                  1/2<sup>{@code certainty}</sup>
     ...
     *                             {@code bitLength < 2} or {@code bitLength} is
     *                             too large.
     * @see #bitLength()
     */
    public BigInteger(int bitLength, int certainty, Random rnd) {
        ...
        prime = (bitLength < SMALL_PRIME_THRESHOLD ? smallPrime(bitLength,
                certainty, rnd) : largePrime(bitLength, certainty, rnd));
        signum = 1;
        mag = prime.mag;
    }
     ...
     */
    private static BigInteger smallPrime(int bitLength, int certainty,
            Random rnd) {
        ...
        while (true) {
            ...
            if (bitLength > 6) {
                ...
                if ((r % 3 == 0) || (r % 5 == 0) || (r % 7 == 0) || (r
                        % 11 == 0) || (r % 13 == 0) || (r % 17 == 0) || (r
                                % 19 == 0) || (r % 23 == 0) || (r % 29 == 0)
    ...
    private static final BigInteger SMALL_PRIME_PRODUCT = valueOf(3L * 5 * 7
     ...
     */
    private static BigInteger largePrime(int bitLength, int certainty,
            Random rnd) {
        BigInteger p;
    ...
    /**
     * Returns the first integer greater than this {@code BigInteger} that is
     * probably prime. The probability that the number returned by this method
     * is composite does not exceed 2<sup>-100</sup>. This method will never
     * skip over a prime when searching: if it returns {@code p}, there is no
     * prime {@code q} such that {@code this < q < p}.
     *
     * @return the first integer greater than this {@code BigInteger} that is
     *         probably prime.
     * @throws ArithmeticException
     *                             {@code this < 0} or {@code this} is too
     *                             large.
     * @since 1.5
     */
    ...
    /**
     * Returns {@code true} if this BigInteger is probably prime, {@code false}
     ...
     *                  a measure of the uncertainty that the caller is willing
     *                  to
     *                  tolerate: if the call returns {@code true} the
     *                  probability
     *                  that this BigInteger is prime exceeds
     *                  {@code (1 - 1/2<sup>certainty</sup>)}. The execution
     *                  time of
     *                  this method is proportional to the value of this
     *                  parameter.
     * @return {@code true} if this BigInteger is probably prime, {@code false}
     *         if it's definitely composite.
     */
    ...
    /**
     * This internal constructor differs from its public cousin with the
    ...
    private void checkRange() {
        if (mag.length > MAX_MAG_LENGTH || mag.length == MAX_MAG_LENGTH
                && mag[0] < 0) {
            reportOverflow();
        }
    }
     ...
     * recalculate powers of radix^(2^n) more than once. This speeds Schoenhage
    ...
    static {
         ...
         * with just the very first value. Additional values will be created on
         * demand.
         */
    ...
    /**
     * Package private methods used by BigDecimal code to add a BigInteger with
     * a long. Assumes val is not equal to INFLATED.
     */
    BigInteger add(long val) {
        ...
        if (cmp == 0)
            return ZERO;
        int[] resultMag = (cmp > 0 ? subtract(mag, Math.abs(val))
    ...
    /**
     * Adds the contents of the int array x and long value val. This method
     * allocates a new int array to hold the answer and returns a reference to
     * that array. Assumes x.length &gt; 0 and val is non-negative
     */
    private static int[] add(int[] x, long val) {
        ...
        if (highWord == 0) {
            ...
            if (xIndex == 1) {
                ...
                result[xIndex] = (int) sum;
                sum = (x[--xIndex] & LONG_MASK) + (highWord & LONG_MASK)
                        + (sum >>> 32);
                result[xIndex] = (int) sum;
            }
        }
    ...
    private static int[] subtract(long val, int[] little) {
        ...
        if (highWord == 0) {
            ...
            if (little.length == 1) {
                long difference = ((int) val & LONG_MASK) - (little[0]
                        & LONG_MASK);
                ...
                long difference = ((int) val & LONG_MASK) - (little[1]
                        & LONG_MASK);
                result[1] = (int) difference;
                difference = (highWord & LONG_MASK) - (little[0] & LONG_MASK)
                        + (difference >> 32);
     ...
     * This method allocates the space necessary to hold the answer. assumes val
     * &gt;= 0
     */
    private static int[] subtract(int[] big, long val) {
        ...
        if (highWord == 0) {
            ...
            result[bigIndex] = (int) difference;
            difference = (big[--bigIndex] & LONG_MASK) - (highWord & LONG_MASK)
                    + (difference >> 32);
            result[bigIndex] = (int) difference;
        }
     ...
     * Returns a BigInteger whose value is {@code (this * val)}.
     *
     * @implNote An implementation may offer better algorithmic performance when
     *           {@code val == this}.
     *
     * @param val
    ...
    private static BigInteger multiplyByInt(int[] x, int y, int sign) {
        if (Integer.bitCount(y) == 1) {
            return new BigInteger(shiftLeft(x, Integer.numberOfTrailingZeros(
                    y)), sign);
        }
        int xlen = x.length;
    ...
    BigInteger multiply(long v) {
        ...
        if (dh != 0L) {
            ...
            for (int i = xlen - 1; i >= 0; i--) {
                long product = (value[i] & LONG_MASK) * dh + (rmag[rstart]
                        & LONG_MASK) + carry;
                rmag[rstart--] = (int) product;
                carry = product >>> 32;
            }
     ...
     * for large numbers than what is commonly called the "grade-school"
     ...
     * O(n^2). In contrast, the Karatsuba algorithm has complexity of
     * O(n^(log2(3))), or O(n^1.585). It achieves this increased performance by
     * doing 3 multiplies instead of 4 when evaluating the product. As it has
     * some overhead, should be used when both numbers are larger than a certain
    ...
    private static BigInteger multiplyKaratsuba(BigInteger x, BigInteger y) {
        ...
        BigInteger result = p1.shiftLeft(32 * half).add(p3.subtract(p1)
                .subtract(p2)).shiftLeft(32 * half).add(p2);
     ...
     * efficient for large numbers than what is commonly called the
     * "grade-school" algorithm used in multiplyToLen. If the numbers to be
     * multiplied have length n, the "grade-school" algorithm has an asymptotic
     * complexity of O(n^2). In contrast, 3-way Toom-Cook has a complexity of
     * about O(n^1.465). It achieves this increased asymptotic performance by
     * breaking each number into three parts and by doing 5 multiplies instead
     * of 9 when evaluating the product. Due to overhead (additions, shifts, and
     * one division) in the Toom-Cook algorithm, it should only be used when
     * both numbers are larger than a certain threshold (found experimentally).
     * This threshold is generally larger than that for Karatsuba
     * multiplication, so this algorithm is generally only used when numbers
     * become significantly larger.
     *
     * The algorithm used is the "optimal" 3-way Toom-Cook algorithm outlined by
     * Marco Bodrato.
     *
     * See: http://bodrato.it/toom-cook/ http://bodrato.it/papers/#WAIFI2007
     *
     * "Towards Optimal Toom-Cook Multiplication for Univariate and Multivariate
     * Polynomials in Characteristic 2 and 0." by Marco BODRATO; In C.Carlet and
     * B.Sunar, Eds., "WAIFI'07 proceedings", p. 116-133, LNCS #4547. Springer,
    ...
    private static BigInteger multiplyToomCook3(BigInteger a, BigInteger b) {
        ...
        db1 = db1.add(b1);
        v1 = da1.multiply(db1);
        v2 = da1.add(a2).shiftLeft(1).subtract(a0).multiply(db1.add(b2)
                .shiftLeft(1).subtract(b0));
        vinf = a2.multiply(b2);
        ...
        BigInteger result = vinf.shiftLeft(ss).add(t2).shiftLeft(ss).add(t1)
     ...
     * @param slice
     *                  The index of which slice is requested, which must be a
     *                  number
     *                  from 0 to size-1. Slice 0 is the highest-order bits, and
     *                  slice
     *                  size-1 are the lowest-order bits. Slice 0 may be of
     *                  different
     *                  size than the other slices.
     * @param fullsize
     *                  The size of the larger integer array, used to align
     *                  slices to
     *                  the appropriate position when multiplying
     *                  different-sized
     *                  numbers.
     */
    ...
    /**
     * Does an exact division (that is, the remainder is known to be zero) of
     * the specified number by 3. This is used in Toom-Cook multiplication. This
     * is an efficient algorithm that runs in linear time. If the argument is
     * not exactly divisible by 3, results are undefined. Note that this is
     * expected to be called with positive arguments only.
     */
     ...
     */
    private static void implSquareToLenChecks(int[] x, int len, int[] z,
            int zlen) throws RuntimeException {
        if (len < 1) {
     ...
     */
    private static final int[] implSquareToLen(int[] x, int len, int[] z,
            int zlen) {
        /*
    ...
    private BigInteger squareKaratsuba() {
        ...
        return xhs.shiftLeft(half * 32).add(xl.add(xh).square().subtract(xhs
                .add(xls))).shiftLeft(half * 32).add(xls);
    }
    ...
    private BigInteger squareToomCook3() {
        ...
        return vinf.shiftLeft(ss).add(t2).shiftLeft(ss).add(t1).shiftLeft(ss)
    ...
    public BigInteger divide(BigInteger val) {
        if (val.mag.length < BURNIKEL_ZIEGLER_THRESHOLD || mag.length
    ...
    /**
     * Returns a BigInteger whose value is {@code (this / val)} using an O(n^2)
     ...
     * @throws ArithmeticException
     *                             if {@code val} is zero.
     * @see MutableBigInteger#divideKnuth(MutableBigInteger, MutableBigInteger,
     *      boolean)
     */
    private BigInteger divideKnuth(BigInteger val) {
        MutableBigInteger q = new MutableBigInteger(),
                a = new MutableBigInteger(this.mag), b = new MutableBigInteger(
                        val.mag);
     ...
     *         the initial element, and the remainder {@code (this % val)} is
     *         the final element.
     * @throws ArithmeticException
     *                             if {@code val} is zero.
     */
    ...
    private BigInteger[] divideAndRemainderBurnikelZiegler(BigInteger val) {
        ...
        MutableBigInteger r = new MutableBigInteger(this)
                .divideAndRemainderBurnikelZiegler(new MutableBigInteger(val),
                        q);
        BigInteger qBigInt = q.isZero() ? ZERO
                : q.toBigInteger(signum * val.signum);
        ...
        if (partToSquare.mag.length == 1 && scaleFactor <= 62) {
            ...
            if (powersOfTwo > 0) {
                if (bitsToShift + scaleFactor <= 62) { // Fits in long?
                    ...
                    return valueOf(result * newSign).shiftLeft(
                            (int) bitsToShift);
                }
            } else {
     ...
     * @throws ArithmeticException
     *                             {@code m} &le; 0 or the exponent is negative
     *                             and this
     *                             BigInteger is not <i>relatively prime</i> to
     *                             {@code m}.
     * @see #modInverse
     */
    public BigInteger modPow(BigInteger exponent, BigInteger m) {
        ...
        BigInteger base = (this.signum < 0 || this.compareTo(m) >= 0 ? this.mod(
                m) : this);
        BigInteger result;
        if (m.testBit(0)) { // odd modulus
            ...
            if (m.mag.length < MAX_MAG_LENGTH / 2) {
                result = a1.multiply(m2).multiply(y1).add(a2.multiply(m1)
                        .multiply(y2)).mod(m);
            } else {
                MutableBigInteger t1 = new MutableBigInteger();
                new MutableBigInteger(a1.multiply(m2)).multiply(
                        new MutableBigInteger(y1), t1);
                MutableBigInteger t2 = new MutableBigInteger();
                new MutableBigInteger(a2.multiply(m1)).multiply(
                        new MutableBigInteger(y2), t2);
                t1.add(t2);
                MutableBigInteger q = new MutableBigInteger();
    ...
    private static int[] montgomeryMultiply(int[] a, int[] b, int[] n, int len,
        ...
        if (len > MONTGOMERY_INTRINSIC_THRESHOLD) {
            ...
            return implMontgomeryMultiply(a, b, n, len, inv, materialize(
                    product, len));
        }
    }
    ...
    private static int[] montgomerySquare(int[] a, int[] n, int len, long inv,
        ...
        if (len > MONTGOMERY_INTRINSIC_THRESHOLD) {
            ...
            return implMontgomerySquare(a, n, len, inv, materialize(product,
                    len));
        }
    }

    // Range-check everything.
    private static void implMontgomeryMultiplyChecks(int[] a, int[] b, int[] n,
            int len, int[] product) throws RuntimeException {
        if (len % 2 != 0) {
            throw new IllegalArgumentException(
                    "input array length must be even: " + len);
        }
        ...
        if (len > a.length || len > b.length || len > n.length
    ...
    // intrinsics.
    private static int[] implMontgomeryMultiply(int[] a, int[] b, int[] n,
            int len, long inv, int[] product) {
        product = multiplyToLen(a, len, b, len, product);
        return montReduce(product, n, len, (int) inv);
    }
    ...
    private BigInteger oddModPow(BigInteger y, BigInteger z) {
         ...
         * != 0, we have to decide what pattern to multiply by, and when to do
         * it. We decide, remember to do it in future after a suitable number of
        ...
        long n0 = (mod[modLen - 1] & LONG_MASK) + ((mod[modLen - 2]
                & LONG_MASK) << 32);
        ...
        MutableBigInteger q = new MutableBigInteger(),
                a2 = new MutableBigInteger(a), b2 = new MutableBigInteger(mod);
        ...
        for (int i = 1; i < tblmask; i++) {
            table[i] = montgomeryMultiply(t, table[i - 1], mod, modLen, inv,
                    null);
        }
     ...
     * Parameters validation.
     */
    private static void implMulAddCheck(int[] out, int[] in, int offset,
            int len, int k) {
        if (len > in.length) {
            throw new IllegalArgumentException("input length is out of bound: "
                    + len + " > " + in.length);
        }
        if (offset < 0) {
            throw new IllegalArgumentException("input offset is invalid: "
                    + offset);
        }
        if (offset > (out.length - 1)) {
            throw new IllegalArgumentException("input offset is out of bound: "
                    + offset + " > " + (out.length - 1));
        }
        if (len > (out.length - offset)) {
            throw new IllegalArgumentException("input len is out of bound: "
     ...
     * Java Runtime may use intrinsic for this method.
     */
    private static int implMulAdd(int[] out, int[] in, int offset, int len,
            int k) {
     ...
     * {@code BigInteger >= 0} and {@code p > 0}.
     */
    private BigInteger mod2(int p) {
        ...
        System.arraycopy(this.mag, (this.mag.length - numInts), mag, 0,
                numInts);
    ...
    /**
     * Returns a BigInteger whose value is {@code (this}<sup>-1</sup>
     * {@code mod m)}.
     *
     * @param m
     *          the modulus.
     ...
     * @throws ArithmeticException
     *                             {@code  m} &le; 0, or this BigInteger has no
     *                             multiplicative
     *                             inverse mod m (that is, this BigInteger is
     *                             not <i>relatively
     *                             prime</i> to m).
     */
    ...
    /**
     * Returns a magnitude array whose value is {@code (mag << n)}. The shift
     * distance, {@code n}, is considered unnsigned. (Computes
     * <tt>this * 2<sup>n</sup></tt>.)
     *
     * @param mag
     *            magnitude, the most-significant int ({@code mag[0]}) must be
     *            non-zero.
     * @param n
     ...
     * case this method performs a left shift. (Computes
    ...
    /**
     * Returns a BigInteger whose value is {@code (this >> n)}. The shift
     * distance, {@code n}, is considered unsigned. (Computes
     * <tt>floor(this * 2<sup>-n</sup>)</tt>.)
     *
     * @param n
     *          unsigned shift distance, in bits.
    ...
    private BigInteger shiftRightImpl(int n) {
        ...
        if (signum < 0) {
            ...
            for (int i = magLen - 1, j = magLen - nInts; i >= j
                    && !onesLost; i--)
                onesLost = (mag[i] != 0);
            if (!onesLost && nBits != 0)
    ...
    public BigInteger and(BigInteger val) {
        ...
        for (int i = 0; i < result.length; i++)
            result[i] = (getInt(result.length - i - 1) & val.getInt(
                    result.length - i - 1));
    ...
    /**
     * Returns a BigInteger whose value is {@code (~this)}. (This method returns
     * a negative value if and only if this BigInteger is non-negative.)
     *
     * @return {@code ~this}
     */
    ...
    /**
     * Returns a BigInteger whose value is {@code (this & ~val)}. This method,
     * which is equivalent to {@code and(val.not())}, is provided as a
     ...
     * the designated bit set. (Computes {@code (this | (1<<n))}.)
     *
     * @param n
     *          index of bit to set.
    ...
    /**
     * Returns a BigInteger whose value is equivalent to this BigInteger with
     * the designated bit cleared. (Computes {@code (this & ~(1<<n))}.)
     *
     * @param n
    ...
    /**
     * Returns a BigInteger whose value is equivalent to this BigInteger with
     * the designated bit flipped. (Computes {@code (this ^ (1<<n))}.)
     *
     * @param n
    ...
    /**
     * Returns {@code true} if this BigInteger is probably prime, {@code false}
     * if it's definitely composite. If {@code certainty} is &le; 0,
     ...
     *                  tolerate: if the call returns {@code true} the
     *                  probability
     *                  that this BigInteger is prime exceeds (1 - 1/2<sup>
     *                  {@code certainty}</sup>). The execution time of this
     *                  method is
     ...
     * {@code
     * (x.compareTo(y)} &lt;<i>op</i>&gt; {@code 0)}, where &lt;<i>op</i>&gt; is
     ...
     * case for {@code Integer.toString}). The digit-to-character mapping
     * provided by {@code Character.forDigit} is used, and a minus sign is
     ...
     * <p/>
     * See Knuth, Donald, _The Art of Computer Programming_, Vol. 2, Answers to
    ...
    private static void toString(BigInteger u, StringBuilder sb, int radix,
        ...
        if (u.mag.length <= SCHOENHAGE_BASE_CONVERSION_THRESHOLD) {
            ...
            if ((s.length() < digits) && (sb.length() > 0)) {
                for (int i = s.length(); i < digits; i++) { // May be a faster
                                                            // way to
                    sb.append('0'); // do this?
                }
            }
        ...
        n = (int) Math.round(Math.log(b * LOG_TWO / logCache[radix]) / LOG_TWO
                - 1.0);
    ...
    /**
     * Returns the value radix^(2^exponent) from the cache. If this value
     * doesn't already exist in the cache, it is added.
     * <p/>
     ...
     * digit-to-character mapping provided by {@code Character.forDigit} is
    ...
    public byte[] toByteArray() {
        ...
        for (int i = byteLen
                - 1, bytesCopied = 4, nextInt = 0, intIndex = 0; i >= 0; i--) {
            if (bytesCopied == 4) {
    ...
    /**
     * Converts this BigInteger to an {@code int}. This conversion is analogous
     * to a <i>narrowing primitive conversion</i> from {@code long} to
     * {@code int} as defined in section 5.1.3 of <cite>The Java&trade; Language
    ...
    /**
     * Converts this BigInteger to a {@code long}. This conversion is analogous
     * to a <i>narrowing primitive conversion</i> from {@code long} to
     * {@code int} as defined in section 5.1.3 of <cite>The Java&trade; Language
    ...
    /**
     * Converts this BigInteger to a {@code float}. This conversion is similar
     ...
     * Language Specification</cite>: if this BigInteger has too great a
    ...
    public float floatValue() {
        ...
        /*
         * We need the top SIGNIFICAND_WIDTH bits, including the "implicit" one
         * bit. To make rounding easier, we pick out the top SIGNIFICAND_WIDTH +
         * 1 bits, so we have one to help us round up or down. twiceSignifFloor
         * will contain the top SIGNIFICAND_WIDTH + 1 bits, and signifFloor the
         * top SIGNIFICAND_WIDTH.
         ...
         */
        boolean increment = (twiceSignifFloor & 1) != 0 && ((signifFloor
                & 1) != 0 || abs().getLowestSetBit() < shift);
    ...
    /**
     * Converts this BigInteger to a {@code double}. This conversion is similar
     * to the <i>narrowing primitive conversion</i> from {@code double} to
     * {@code float} as defined in section 5.1.3 of <cite>The Java&trade;
     * Language Specification</cite>: if this BigInteger has too great a
    ...
    public double doubleValue() {
        ...
        twiceSignifFloor = ((highBits & LONG_MASK) << 32) | (lowBits
                & LONG_MASK);
        ...
        long bits = (long) ((exponent
                + DoubleConsts.EXP_BIAS)) << (DoubleConsts.SIGNIFICAND_WIDTH
                        - 1);
        bits += signifRounded;
        /*
     ...
     * indexed by radix. The first is the number of digits of the given radix
     * that can fit in a Java long without "going negative", i.e., the highest
     * integer n such that radix**n < 2**63. The second is the "long radix" that
     * tears each number into "long digits", each of which consists of the
     * number of digits in the corresponding element in digitsPerLong
     * (longRadix[i] = i**digitPerLong[i]). Both arrays have nonsense values in
     * their 0 and 1 elements, as radixes 0 and 1 are not used.
     */
    private static int digitsPerLong[] = { 0, 0, 62, 39, 31, 27, 24, 22, 20, 19,
            18, 18, 17, 17, 16, 16, 15, 15, 15, 14, 14, 14, 14, 13, 13, 13, 13,
            13, 13, 12, 12, 12, 12, 12, 12, 12, 12 };

    private static BigInteger longRadix[] = { null, null, valueOf(
            0x4000000000000000L), valueOf(0x383d9170b85ff80bL), valueOf(
                    0x4000000000000000L), valueOf(0x6765c793fa10079dL), valueOf(
                            0x41c21cb8e1000000L), valueOf(0x3642798750226111L),
            valueOf(0x1000000000000000L), valueOf(0x12bf307ae81ffd59L), valueOf(
                    0xde0b6b3a7640000L), valueOf(0x4d28cb56c33fa539L), valueOf(
                            0x1eca170c00000000L), valueOf(0x780c7372621bd74dL),
            valueOf(0x1e39a5057d810000L), valueOf(0x5b27ac993df97701L), valueOf(
                    0x1000000000000000L), valueOf(0x27b95e997e21d9f1L), valueOf(
                            0x5da0e1e53c5c8000L), valueOf(0xb16a458ef403f19L),
            valueOf(0x16bcc41e90000000L), valueOf(0x2d04b7fdd9c0ef49L), valueOf(
                    0x5658597bcaa24000L), valueOf(0x6feb266931a75b7L), valueOf(
                            0xc29e98000000000L), valueOf(0x14adf4b7320334b9L),
            valueOf(0x226ed36478bfa000L), valueOf(0x383d9170b85ff80bL), valueOf(
                    0x5a3c23e39c000000L), valueOf(0x4e900abb53e6b71L), valueOf(
                            0x7600ec618141000L), valueOf(0xaee5720ee830681L),
            valueOf(0x1000000000000000L), valueOf(0x172588ad4f5f0981L), valueOf(
                    0x211e44f7d02c1000L), valueOf(0x2ee56725f06e5c71L), valueOf(
                            0x41c21cb8e1000000L) };
    ...
    private static int digitsPerInt[] = { 0, 0, 30, 19, 15, 13, 11, 11, 10, 9,
            9, 8, 8, 8, 8, 7, 7, 7, 7, 7, 7, 7, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6,
            6, 6, 6, 5 };
     ...
     *
     * @serialField signum
     *              int signum of this BigInteger.
     * @serialField magnitude
     *              int[] magnitude array of this BigInteger.
     * @serialField bitCount
     *              int number of bits in this BigInteger
     * @serialField bitLength
     *              int the number of bits in the minimal two's-complement
     *              representation of this BigInteger
     * @serialField lowestSetBit
     *              int lowest set bit in the twos complement representation
     */
     ...
     * array is discarded. Note: The current convention is to initialize the
     * cache fields, bitCount, bitLength and lowestSetBit, to 0 rather than some
     * other marker value. Therefore, no explicit action to set these fields
     * needs to be taken in readObject because those fields already have a 0
     * value be default since defaultReadObject is not being used.
     */
    private void readObject(java.io.ObjectInputStream s)
            throws java.io.IOException, ClassNotFoundException {
        /*
         * In order to maintain compatibility with previous serialized forms,
         * the magnitude of a BigInteger is serialized as an array of bytes. The
         * magnitude field is used as a temporary store for the byte array that
         * is deserialized. The cached computation fields should be transient
         * but are serialized for compatibility reasons.
         */
        ...
        if (mag.length >= MAX_MAG_LENGTH) {
            try {
                ...
                throw new java.io.StreamCorruptedException(
                        "BigInteger: Out of the supported range");
            }
        }
    }
    ...
    private static class UnsafeHolder {
        ...
        static {
            try {
                ...
                signumOffset = unsafe.objectFieldOffset(BigInteger.class
                        .getDeclaredField("signum"));
                magOffset = unsafe.objectFieldOffset(BigInteger.class
                        .getDeclaredField("mag"));
            } catch (Exception ex) {
    ...
    private byte[] magSerializedForm() {
        ...
        for (int i = byteLen - 1, bytesCopied = 4, intIndex = len
                - 1, nextInt = 0; i >= 0; i--) {
            if (bytesCopied == 4) {
    ...
    /**
     * Converts this {@code BigInteger} to a {@code long}, checking for lost
     ...
     * @throws ArithmeticException
     *                             if the value of {@code this} will not exactly
     *                             fit in a
     *                             {@code long}.
    ...
    /**
     * Converts this {@code BigInteger} to an {@code int}, checking for lost
     * information. If the value of this {@code BigInteger} is out of the range
     * of the {@code int} type, then an {@code ArithmeticException} is thrown.
     *
    ...
    /**
     * Converts this {@code BigInteger} to a {@code short}, checking for lost
     * information. If the value of this {@code BigInteger} is out of the range
     * of the {@code short} type, then an {@code ArithmeticException} is thrown.
     *
    ...
    /**
     * Converts this {@code BigInteger} to a {@code byte}, checking for lost
     * information. If the value of this {@code BigInteger} is out of the range
     * of the {@code byte} type, then an {@code ArithmeticException} is thrown.
     *
