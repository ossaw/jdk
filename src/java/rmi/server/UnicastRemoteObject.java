 ...
 * <li>Calling the {@link #exportObject(Remote) exportObject(Remote)} method.
 * <strong>Deprecated.</strong>
 * <li>Calling the {@link #exportObject(Remote, int) exportObject(Remote, port)}
 * method.
 * <li>Calling the
 ...
 * </ol>
 * <p>
 * The fourth technique, {@link #exportObject(Remote)}, always uses statically
 * generated stubs and is deprecated.
 * <p>
 * The other five techniques all use the following approach: if the
 * {@code java.rmi.server.ignoreStubClasses} property is {@code true} (case
 * insensitive) or if a static stub cannot be found, stubs are generated
 * dynamically using {@link java.lang.reflect.Proxy Proxy} objects. Otherwise,
 * static stubs are used.
 * <p>
 * The default value of the {@code java.rmi.server.ignoreStubClasses} property
 * is {@code false}.
 * <p>
 * Statically generated stubs are typically pregenerated from the remote
 * object's class using the {@code rmic} tool. A static stub is loaded and an
 * instance of that stub class is constructed as described below.
 * <ul>
 * <li>A "root class" is determined as follows: if the remote object's class
 * directly implements an interface that extends {@link Remote}, then the remote
 * object's class is the root class; otherwise, the root class is the most
 * derived superclass of the remote object's class that directly implements an
 * interface that extends {@code Remote}.
 * <li>The name of the stub class to load is determined by concatenating the
 * binary name of the root class with the suffix {@code _Stub}.
 * <li>The stub class is loaded by name using the class loader of the root
 * class. The stub class must extend {@link RemoteStub} and must have a public
 * constructor that has one parameter of type {@link RemoteRef}.
 ...
 * {@link java.lang.reflect.Proxy Proxy} with the following characteristics:
 * <ul>
 * <li>The proxy's class is defined by the class loader of the remote object's
 * class.
 * <li>The proxy implements all the remote interfaces implemented by the remote
 * object's class.
 * <li>The proxy's invocation handler is a {@link RemoteObjectInvocationHandler}
 * instance constructed with a {@link RemoteRef}.
 * <li>If the proxy could not be created, a {@link StubNotFoundException} will
 * be thrown.
 * </ul>
 *
 * @implNote Depending upon which constructor or static method is used for
 *           exporting an object, {@link RMISocketFactory} may be used for
 *           creating sockets. By default, server sockets created by
 *           {@link RMISocketFactory} listen on all network interfaces. See the
 *           {@link RMISocketFactory} class and the section <a href=
 *           "{@docRoot}/../platform/rmi/spec/rmi-server29.html">RMI Socket
 *           Factories</a> in the <a href=
 *           "{@docRoot}/../platform/rmi/spec/rmiTOC.html">Java RMI
 *           Specification</a>.
 * @author Ann Wollrath
 * @author Peter Jones
 * @since JDK1.1
