 ...
 * remote object registry on a particular host (including the local host), or to
 * create a remote object registry that accepts calls on a specific port.
 * <p>
 * Note that a <code>getRegistry</code> call does not actually make a connection
 * to the remote host. It simply creates a local reference to the remote
 * registry and will succeed even if no registry is running on the remote host.
 * Therefore, a subsequent method invocation to a remote registry returned as a
 * result of this method may fail.
 *
 * @author Ann Wollrath
 * @author Peter Jones
 * @since JDK1.1
...
public final class LocateRegistry {
     ...
     * <code>port</code>. Communication with this remote registry will use the
     * supplied <code>RMIClientSocketFactory</code> <code>csf</code> to create
     * <code>Socket</code> connections to the registry on the remote
     ...
     *        port on which the registry accepts requests
     * @param csf
     *        client-side <code>Socket</code> factory used to make
     *        connections to the registry. If <code>csf</code> is null,
     *        then
     *        the default client-side <code>Socket</code> factory will be
     *        used in the registry stub.
    ...
    public static Registry getRegistry(String host, int port, RMIClientSocketFactory csf)
         ...
         **/
        LiveRef liveRef = new LiveRef(new ObjID(ObjID.REGISTRY_ID), new TCPEndpoint(host, port, csf, null),
                false);
    ...
    /**
     * Creates and exports a <code>Registry</code> instance on the local host
     * that uses custom socket factories for communication with that instance.
     * The registry that is created listens for incoming requests on the given
     * <code>port</code> using a <code>ServerSocket</code> created from the
     * supplied <code>RMIServerSocketFactory</code>.
     * <p>
     * The <code>Registry</code> instance is exported as if the static
     ...
     * @param ssf
     *        server-side <code>ServerSocket</code> factory used to accept
     *        connections to the registry
     * @return the registry
     * @exception RemoteException
     *            if the registry could not be exported
