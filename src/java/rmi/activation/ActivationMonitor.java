 ...
 * <code>ActivationGroup</code> and is obtained when a group is reported active
 * via a call to <code>ActivationSystem.activeGroup</code> (this is done
...
public interface ActivationMonitor extends Remote {
    ...
    /**
     * An activation group calls its monitor's <code>inactiveObject</code>
     * method when an object in its group becomes inactive (deactivates). An
     * activation group discovers that an object (that it participated in
     ...
     * longer valid. The monitor considers the reference associated with
     * <code>id</code> as a stale reference. Since the reference is considered
     * stale, a subsequent <code>activate</code> call for the same activation
     ...
     * informs its monitor if an object in its group becomes active by other
     * means than being activated directly (i.e., the object is registered and
     * "activated" itself).
     *
     * @param id
    ...
    /**
     * Informs that the group is now inactive. The group will be recreated upon
     * a subsequent request to activate an object within the group. A group
     * becomes inactive when all objects in the group report that they are
     * inactive.
     *
     * @param id
     *                    the group's id
