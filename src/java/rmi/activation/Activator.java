...
/**
 * The <code>Activator</code> facilitates remote object activation. A "faulting"
 * remote reference calls the activator's <code>activate</code> method to obtain
 * a "live" reference to a "activatable" remote object. Upon receiving a request
 * for activation, the activator looks up the activation descriptor for the
 * activation identifier, <code>id</code>, determines the group in which the
 ...
 * <code>newInstance</code> method). The activator initiates the execution of
 * activation groups as necessary. For example, if an activation group for a
...
public interface Activator extends Remote {
	 ...
	 * <code>id</code>. If the activator knows the object to be active already,
	 * and <code>force</code> is false , the stub with a "live" reference is
	 * returned immediately to the caller; otherwise, if the activator does not
	 * know that corresponding the remote object is active, the activator uses
	 * the activation descriptor information (previously registered) to
	 * determine the group (VM) in which the object should be activated. If an
	 * <code>ActivationInstantiator</code> corresponding to the object's group
	 * descriptor already exists, the activator invokes the activation group's
	 ...
	 * receives the activation group's call back (via the
	 * <code>ActivationSystem</code>'s <code>activeGroup</code> method)
	 * specifying the activation group's reference, the activator can then
	 * invoke that activation instantiator's <code>newInstance</code> method to
	 * forward each pending activation request to the activation group and
	 ...
	 * object. If the activator kept a strong reference to the remote object,
	 * the activator would then prevent the object from being garbage collected
	 * under the normal distributed garbage collection mechanism.
	 * <p>
	 *
	 * @param id
