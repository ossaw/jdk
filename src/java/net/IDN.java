 ...
 * entire range of Unicode, while traditional domain names are restricted to
 ...
 * two operations: ToASCII and ToUnicode. These 2 operations employ
 * <a href="http://www.ietf.org/rfc/rfc3491.txt">Nameprep</a> algorithm, which
 ...
 * flags:
 * <ul>
 * <li>If the ALLOW_UNASSIGNED flag is used, the domain name string to be
 * converted can contain code points that are unassigned in Unicode 3.2, which
 * is the Unicode version on which IDN conversion is based. If the flag is not
 * used, the presence of such unassigned code points is treated as an error.
 * <li>If the USE_STD3_ASCII_RULES flag is used, ASCII strings are checked
 * against <a href="http://www.ietf.org/rfc/rfc1122.txt">RFC 1122</a> and
 * <a href="http://www.ietf.org/rfc/rfc1123.txt">RFC 1123</a>. It is an error if
 ...
 * <i>homographed</i> - maliciously misspelled by substitution of non-Latin
 * letters. <a href="http://www.unicode.org/reports/tr36/">Unicode Technical
 * Report #36</a> discusses security issues of IDN support as well as possible
...
public final class IDN {
	 ...
	 * labels in a domain name are always separated by dots. The following
	 * characters are recognized as dots: &#0092;u002E (full stop), &#0092;u3002
	 * (ideographic full stop), &#0092;uFF0E (fullwidth full stop), and
	 * &#0092;uFF61 (halfwidth ideographic full stop). if dots are used as label
	 * separators, this method also changes all of them to &#0092;u002E (full
	...
	private static String toASCIIInternal(String label, int flag) {
		// step 1
		// Check if the string contains code points outside the ASCII range
		// 0..0x7c.
	...
	private static String toUnicodeInternal(String label, int flag) {
		...
		if (!isASCII) {
			// step 2
			// perform the nameprep operation; flag ALLOW_UNASSIGNED is used
			// here
			try {
				UCharacterIterator iter = UCharacterIterator.getInstance(label);
				dest = namePrep.prepare(iter, flag);
			} catch (Exception e) {
				// toUnicode never fails; if any step fails, return the input
				// string
				return label;
			}
		} else {
	...
	// dots might be: \u002E (full stop), \u3002 (ideographic full stop), \uFF0E
	// (fullwidth full stop),
