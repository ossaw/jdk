 ...
 * "localhost" (for the local machine). The wildcard "*" may be included once in
 * a DNS name host specification. If it is included, it must be in the leftmost
 ...
 * </pre>
 * <p>
 * The <A HREF="Inet6Address.html#lform">full uncompressed form</A> of an IPv6
 * literal address is also valid.
 * <p>
 * The port or portrange is optional. A port specification of the form "N-",
 * where <i>N</i> is a port number, signifies all ports numbered <i>N</i> and
 * above, while a specification of the form "-N" indicates all ports numbered
 * <i>N</i> and below. The special port value {@code 0} refers to the entire
 * <i>ephemeral</i> port range. This is a fixed range of ports a system may use
 * to allocate dynamic ports from. The actual range may be system dependent.
 * <p>
 ...
 * </pre>
 * 
 * The "listen" action is only meaningful when used with "localhost" and means
 * the ability to bind to a specified port. The "resolve" action is implied when
 * any of the other actions are present. The action "resolve" refers to host/ip
 ...
 * share confidential data among parties who may not otherwise have access to
 * the data.
 *
...
public final class SocketPermission extends Permission implements java.io.Serializable {
    ...
    /**
     * Creates a new SocketPermission object with the specified actions. The
     * host is expressed as a DNS name, or as a numerical IP address.
     * Optionally, a port or a portrange may be supplied (separated from the DNS
     * name or IP address by a colon).
     * <p>
     * To specify the local machine, use "localhost" as the <i>host</i>. Also
     * note: An empty <i>host</i> String ("") is equivalent to "localhost".
     * <p>
     * The <i>actions</i> parameter contains a comma-separated list of the
     * actions granted for the specified host (and port(s)). Possible actions
     * are "connect", "listen", "accept", "resolve", or any combination of
     * those. "resolve" is automatically added when any of the other three are
     * specified.
     * <p>
    ...
    private static String getHost(String host) {
        if (host.equals("")) {
            ...
            /*
             * IPv6 literal address used in this context should follow the
             * format specified in RFC 2732; if not, we try to solve the
             * unambiguous case
             */
            int ind;
            if (host.charAt(0) != '[') {
                if ((ind = host.indexOf(':')) != host.lastIndexOf(':')) {
                    /*
                     * More than one ":", meaning IPv6 address is not in RFC
                     * 2732 format; We will rectify user errors for all
                     * unambiguious cases
                     */
    ...
    /**
     * Returns true if the permission has specified zero as its value (or lower
     * bound) signifying the ephemeral range
     */
    ...
    /**
     * Initialize the SocketPermission object. We don't do any DNS lookups as
     * this point, instead we hold off until the implies method is called.
     */
    ...
    private static int getMask(String action) {
        ...
        while (i != -1) {
            ...
            if (i >= 6 && (a[i - 6] == 'c' || a[i - 6] == 'C') && (a[i - 5] == 'o' || a[i - 5] == 'O') && (a[i
                    - 4] == 'n' || a[i - 4] == 'N') && (a[i - 3] == 'n' || a[i - 3] == 'N') && (a[i
                            - 2] == 'e' || a[i - 2] == 'E') && (a[i - 1] == 'c' || a[i - 1] == 'C')
                    && (a[i] == 't' || a[i] == 'T')) {
                matchlen = 7;
                mask |= CONNECT;

            } else if (i >= 6 && (a[i - 6] == 'r' || a[i - 6] == 'R') && (a[i - 5] == 'e' || a[i - 5] == 'E')
                    && (a[i - 4] == 's' || a[i - 4] == 'S') && (a[i - 3] == 'o' || a[i - 3] == 'O') && (a[i
                            - 2] == 'l' || a[i - 2] == 'L') && (a[i - 1] == 'v' || a[i - 1] == 'V')
                    && (a[i] == 'e' || a[i] == 'E')) {
                matchlen = 7;
                mask |= RESOLVE;

            } else if (i >= 5 && (a[i - 5] == 'l' || a[i - 5] == 'L') && (a[i - 4] == 'i' || a[i - 4] == 'I')
                    && (a[i - 3] == 's' || a[i - 3] == 'S') && (a[i - 2] == 't' || a[i - 2] == 'T') && (a[i
                            - 1] == 'e' || a[i - 1] == 'E') && (a[i] == 'n' || a[i] == 'N')) {
                matchlen = 6;
                mask |= LISTEN;

            } else if (i >= 5 && (a[i - 5] == 'a' || a[i - 5] == 'A') && (a[i - 4] == 'c' || a[i - 4] == 'C')
                    && (a[i - 3] == 'c' || a[i - 3] == 'C') && (a[i - 2] == 'e' || a[i - 2] == 'E') && (a[i
                            - 1] == 'p' || a[i - 1] == 'P') && (a[i] == 't' || a[i] == 'T')) {
                matchlen = 6;
                mask |= ACCEPT;
    ...
    /**
     * Checks if this socket permission object "implies" the specified
     * permission.
     * <P>
     ...
     * <li><i>p</i>'s port range is included in this port range. Note: port
     * range is ignored when p only contains the action, 'resolve'.
     * </ul>
     * Then {@code implies} checks each of the following, in order, and for each
     * returns true if the stated condition is true:
     * <ul>
     * <li>If this object was initialized with a single IP address and one of
     * <i>p</i>'s IP addresses is equal to this object's IP address.
     * <li>If this object is a wildcard domain (such as *.sun.com), and <i>p</i>
     * 's canonical name (the name without any preceding *) ends with this
     * object's canonical host name. For example, *.sun.com implies
     * *.eng.sun.com.
     * <li>If this object was not initialized with a single IP address, and one
     * of this object's IP addresses equals one of <i>p</i>'s IP addresses.
    ...
    /**
     * Checks if the incoming Permission's action are a proper subset of the
     * this object's actions.
     * <P>
     * Check, in the following order:
     * <ul>
     * <li>Checks that "p" is an instanceof a SocketPermission
     * <li>Checks that "p"'s actions are a proper subset of the current object's
     * actions.
     * <li>Checks that "p"'s port range is included in this port range
     * <li>If this object was initialized with an IP address, checks that one of
     * "p"'s IP addresses is equal to this object's IP address.
     * <li>If either object is a wildcard domain (i.e., "*.sun.com"), attempt to
     * match based on the wildcard.
     * <li>If this object was not initialized with an IP address, attempt to
     * find a match based on the IP addresses in both objects.
     ...
     * @return true if <i>obj</i> is a SocketPermission, and has the same
     *         hostname, port range, and actions as this SocketPermission
     *         object. However, port range will be ignored in the comparison if
    ...
    public int hashCode() {
        /*
         * If this SocketPermission was initialized with an IP address or a
         * wildcard, use getName().hashCode(), otherwise use the hashCode() of
         * the host name returned from java.net.InetAddress.getHostName method.
         */
     ...
     * specified mask. Always returns present actions in the following order:
     ...
     * returns present actions in the following order: connect, listen, accept,
     * resolve.
     *
     * @return the canonical string representation of the actions.
     */
     ...
     * objects.
     * <p>
     * SocketPermission objects must be stored in a manner that allows them to
     * be inserted into the collection in any order, but that also enables the
     * PermissionCollection {@code implies} method to be implemented in an
    ...
    /**
     * WriteObject is called to save the state of the SocketPermission to a
    ...
    /**
     * readObject is called to restore the state of the SocketPermission from a
     * stream.
     */
    ...
    /**
     * Check the system/security property for the ephemeral port range for this
     * system. The suffix is either "high" or "low"
     */
    ...
    private static boolean inRange(int policyLow, int policyHigh, int targetLow, int targetHigh) {
        ...
        return (targetLow <= policyHigh && targetHigh <= policyHigh) || (targetLow >= ephemeralLow
                && targetHigh <= ephemeralHigh);
    }
    /*
     * public String toString() { StringBuffer s = new
     ...
     * System.out.println("-----\n"); System.out.println("this.implies(that) = "
     ...
     * SocketPermission("www-sun.com","connect")); System.out.println(
     * "nps.implies(that) = " + nps.implies(that_));
     * System.out.println("-----\n"); }
     */
}

/**
 * if (init'd with IP, key is IP as string) if wildcard, its the wild card else
 * its the cname?
 *
...
final class SocketPermissionCollection extends PermissionCollection implements Serializable {
    ...
    /**
     * Adds a permission to the SocketPermissions. The key for the hash is the
     ...
     */
    /*
     * Writes the contents of the perms field out as a Vector for serialization
