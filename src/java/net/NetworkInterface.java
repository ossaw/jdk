...
public final class NetworkInterface {
	 ...
	 * Setting such an interface on a MulticastSocket will cause the kernel to
	 * choose one interface for sending multicast packets.
	 *
	 */
	NetworkInterface() {}
	 ...
	 * for each InetAddress. Only InetAddresses where the {@code checkConnect}
	 * doesn't throw a SecurityException will be returned in the Enumeration.
	 * However, if the caller has the {@link NetPermission}
	 * ("getNetworkInformation") permission, then all InetAddresses are
	 * returned.
	 * 
	...
	public Enumeration<InetAddress> getInetAddresses() {
		...
		class checkedAddresses implements Enumeration<InetAddress> {
			...
			checkedAddresses() {
				...
				if (sec != null) {
					try {
						sec.checkPermission(new NetPermission(
	...
	/**
	 * Get a List of all or a subset of the {@code InterfaceAddresses} of this
	 * network interface.
	 * <p>
	 * If there is a security manager, its {@code checkConnect} method is called
	 * with the InetAddress for each InterfaceAddress. Only InterfaceAddresses
	 * where the {@code checkConnect} doesn't throw a SecurityException will be
	...
	public java.util.List<InterfaceAddress> getInterfaceAddresses() {
		java.util.List<InterfaceAddress> lst = new java.util.ArrayList<InterfaceAddress>(
				1);
		SecurityManager sec = System.getSecurityManager();
		for (int j = 0; j < bindings.length; j++) {
			try {
				if (sec != null) {
					sec.checkConnect(bindings[j].getAddress().getHostAddress(),
							-1);
				}
				lst.add(bindings[j]);
			} catch (SecurityException e) {
	 ...
	 * subinterface, or {@code null} if it is a physical (non virtual) interface
	 * or has no parent.
	 *
	...
	/**
	 * Returns the index of this network interface. The index is an integer
	 * greater or equal to zero, or {@code -1} for unknown. This is a system
	 * specific value and interfaces with the same name can have different
	 ...
	 *
	 * @return A {@code NetworkInterface} with the specified name, or
	 *         {@code null} if there is no network interface with the specified
	 *         name.
	 *
	 ...
	 * <p>
	 * If the specified IP address is bound to multiple network interfaces it is
	 * not defined which network interface is returned.
	 *
	 * @param addr
	 ...
	 * contains at least one element, possibly representing a loopback interface
	 * that only supports communication between entities on this machine.
	 *
	...
	/**
	 * Returns whether a network interface is a point to point interface. A
	 * typical point to point interface would be a PPP connection through a
	 * modem.
	 *
	 * @return {@code true} if the interface is a point to point interface.
	 * @exception SocketException
	 *                            if an I/O error occurs.
	 * @since 1.6
	 */
	 ...
	 * and if it can be accessed given the current privileges. If a security
	 * manager is set, then the caller must have the permission
	 * {@link NetPermission}("getNetworkInformation").
	 *
	 * @return a byte array containing the address, or {@code null} if the
	 *         address doesn't exist, is not accessible or a security manager is
	 *         set and the caller does not have the permission
	...
	public byte[] getHardwareAddress() throws SocketException {
		...
		for (InetAddress addr : addrs) {
			if (addr instanceof Inet4Address) {
				return getMacAddr0(((Inet4Address) addr).getAddress(), name,
						index);
			}
		}
	...
	private native static boolean isUp0(String name, int ind)
			throws SocketException;

	private native static boolean isLoopback0(String name, int ind)
			throws SocketException;

	private native static boolean supportsMulticast0(String name, int ind)
			throws SocketException;

	private native static boolean isP2P0(String name, int ind)
			throws SocketException;

	private native static byte[] getMacAddr0(byte[] inAddr, String name,
			int ind) throws SocketException;

	private native static int getMTU0(String name, int ind)
			throws SocketException;

	/**
	 * Compares this object against the specified object. The result is
	 * {@code true} if and only if the argument is not {@code null} and it
	 * represents the same NetworkInterface as this object.
	 * <p>
