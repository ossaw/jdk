 ...
 * machine to another might be routed differently, and might arrive in any
...
public final class DatagramPacket {
	...
	/**
	 * Constructs a {@code DatagramPacket} for receiving packets of length
	 * {@code length}, specifying an offset into the buffer.
	 * <p>
	...
	/**
	 * Constructs a datagram packet for sending packets of length {@code length}
	 * with offset {@code ioffset}to the specified port number on the specified
	 * host. The {@code length} argument must be less than or equal to
	 * {@code buf.length}.
	 *
	 * @param buf
	 *                the packet data.
	...
	/**
	 * Constructs a datagram packet for sending packets of length {@code length}
	 * to the specified port number on the specified host. The {@code length}
	...
	/**
	 * Returns the data buffer. The data received or the data to be sent starts
	 * from the {@code offset} in the buffer, and runs for {@code length} long.
	 *
	...
	/**
	 * Set the data buffer for this packet. This sets the data, length and
	 ...
	 *
	 * @param length
	 *               the length of the data and/or the length of the buffer used
	 *               to
	 *               receive data
	 *
	...
	public synchronized void setData(byte[] buf, int offset, int length) {
		...
		if (length < 0 || offset < 0 || (length + offset) < 0 || ((length
				+ offset) > buf.length)) {
	 ...
	 * DatagramPacket set to 0, and the length set to the length of {@code buf}.
	 *
	 * @param buf
	 *            the buffer to set for this packet.
	 *
	...
	/**
	 * Set the length for this packet. The length of the packet is the number of
	 * bytes from the packet's data buffer that will be sent, or the number of
	 * bytes of the packet's data buffer that will be used for receiving data.
	 * The length must be lesser or equal to the offset plus the length of the
	 * packet's buffer.
	 *
	 * @param length
	 ...
	 *                                     the packet's data buffer length.
	 *
	 * @see #getLength
	 * @see #setData
	 *
	...
	public synchronized void setLength(int length) {
		if ((length + offset) > buf.length || length < 0 || (length
				+ offset) < 0) {
