 ...
 * port number. Class D IP addresses are in the range <CODE>224.0.0.0</CODE> to
 * <CODE>239.255.255.255</CODE>, inclusive. The address 224.0.0.0 is reserved
 * and should not be used.
 * <P>
 * One would join a multicast group by first creating a MulticastSocket with the
 * desired port, then invoking the <CODE>joinGroup(InetAddress groupAddr)</CODE>
 * method:
 * 
 * <PRE>
 ...
 * recipients to that host and port receive the message (within the time-to-live
 * range of the packet, see below). The socket needn't be a member of the
 * multicast group to send messages to it.
 * <P>
 ...
 * and port. A socket relinquishes membership in a group by the
 * leaveGroup(InetAddress addr) method. <B> Multiple MulticastSocket's</B> may
 * subscribe to a multicast group and port concurrently, and they will all
 * receive group datagrams.
 * <P>
...
public class MulticastSocket extends DatagramSocket {
	...
	/**
	 * Used on some platforms to record if an outgoing interface has been set
	 * for this socket.
	 */
	 ...
	 * <p>
	 * If there is a security manager, its {@code checkListen} method is first
	 * called with the SocketAddress port as its argument to ensure the
	 * operation is allowed. This could result in a SecurityException.
	 * <p>
	 * When the socket is created the
	 * {@link DatagramSocket#setReuseAddress(boolean)} method is called to
	...
	/**
	 * The lock on the socket's interface - used by setInterface and
	 * getInterface
	 */
	 ...
	 * {@code MulticastSocket} in order to control the scope of the multicasts.
	 *
	 * <p>
	 * The ttl is an <b>unsigned</b> 8-bit quantity, and so <B>must</B> be in
	 * the range {@code 0 <= ttl <= 0xFF }.
	 *
	 * @param ttl
	 *            the time-to-live
	 * @exception IOException
	 ...
	 * 255} or an {@code IllegalArgumentException} will be thrown. Multicast
	 * packets sent with a TTL of {@code 0} are not transmitted on the network
	...
	/**
	 * Get the default time-to-live for multicast packets sent out on the
	 * socket.
	 *
	 * @exception IOException
	 ...
	 * {@code checkMulticast} method with the {@code mcastaddr} argument as its
	 * argument.
	 *
	 * @param mcastaddr
	...
	public void joinGroup(InetAddress mcastaddr) throws IOException {
		...
		/**
		 * required for some platforms where it's not possible to join a group
		 * without setting the interface first.
		 */
	...
	/**
	 * Set the multicast network interface used by methods whose behavior would
	 * be affected by the value of the network interface. Useful for multihomed
	 * hosts.
	 * 
	 * @param inf
	 *            the InetAddress
	 ...
	 *
	 * @return An {@code InetAddress} representing the address of the network
	...
	public InetAddress getInterface() throws SocketException {
		...
		synchronized (infLock) {
			...
			/**
			 * Different InetAddress from what we set with setInterface so
			 * enumerate the current interface to see if the address set by
			 * setInterface is bound to this interface.
			 */
			try {
				...
				/**
				 * No match so reset infAddress to indicate that the interface
				 * has changed via means
				 */
				infAddress = null;
				return ia;
			} catch (Exception e) {
	...
	/**
	 * Specify the network interface for outgoing multicast datagrams sent on
	 * this socket.
	 *
	 * @param netIf
	 *            the interface
	 ...
	 * other than the default for the socket. This method need only be used in
	 * instances where a particular TTL is desired; otherwise it is preferable
	 * to set a TTL once on the socket, and use that default TTL for all
	 * packets. This method does <B>not </B> alter the default TTL for the
	 * socket. Its behavior may be affected by {@code setInterface}.
	 *
	 * <p>
	 * If there is a security manager, this method first performs some security
	 * checks. First, if {@code p.getAddress().isMulticastAddress()} is true,
	 * this method calls the security manager's {@code checkMulticast} method
	 * with {@code p.getAddress()} and {@code ttl} as its arguments. If the
	 * evaluation of that expression is false, this method instead calls the
	 * security manager's {@code checkConnect} method with arguments
	 ...
	 *
	 * @param p
	 *            is the packet to be sent. The packet should contain the
	 *            destination multicast ip address and the data to be sent. One
	 *            does not need to be the member of the group to send packets to
	 *            a destination multicast address.
	 * @param ttl
	 ...
	 *             mcastSocket.setTimeToLive(newttl); mcastSocket.send(p);
