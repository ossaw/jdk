 ...
 * by: <B>SocketImpl</B> and <B>DatagramSocketImpl</B>. Subclasses of these
 * should override the methods of this interface in order to support their own
 * options.
 * <P>
 ...
 * DatagramSocketImpl, <B>you won't use these directly.</B> There are type-safe
...
public interface SocketOptions {
	...
	/**
	 * Enable/disable the option specified by <I>optID</I>. If the option is to
	 * be enabled, and it takes an option-specific "value", this is passed in
	 * <I>value</I>. The actual type of value is option-specific, and it is an
	 * error to pass something that isn't of the expected type: <BR>
	 * 
	 * <PRE>
	 * SocketImpl s;
	 * ...
	 ...
	 * 
	 * <BR>
	 * Any option can be disabled using this method with a Boolean(false): <BR>
	 * 
	 * <PRE>
	 ...
	 * parameter, setting its value to anything other than <I>Boolean(false)</I>
	 * implicitly enables it. <BR>
	 ...
	 * java.lang.Boolean(true) if enabled, java.lang.Boolean(false) if disabled,
	 * e.g.: <BR>
	 * 
	 * <PRE>
	 * SocketImpl s;
	 ...
	 * </PRE>
	 * <P>
	 * For options that take a particular type as a parameter, getOption(int)
	 * will return the parameter's value, else it will return
	 ...
	 * network is not buffered pending acknowledgement of previously written
	 * data.
	 * <P>
	 ...
	 * locally bound address cannot be changed). The default local address of a
	 * socket is INADDR_ANY, meaning any local address on a multi-homed host. A
	 * multi-homed host can use this option to accept connections to only one of
	 * its addresses (in the case of a ServerSocket or DatagramSocket), or to
	 * specify its return address to the peer (for a Socket or DatagramSocket).
	 ...
	 * ability of the process to send broadcast messages. It is supported for
	 * only datagram sockets and only on networks that support the concept of a
	 * broadcast message (e.g. Ethernet, token ring, etc.), and it is set by
	...
	/**
	 * Set which outgoing interface on which to send multicast packets. Useful
	 * on hosts with multiple network interfaces, where applications want to use
	 * other than the system default. Takes/returns an InetAddress.
	 * <P>
	 ...
	 * return from a <B>close()</B> of a TCP Socket. Enabling this option with a
	 * non-zero Integer <I>timeout</I> means that a <B>close()</B> will block
	 * pending the transmission and acknowledgement of all data written to the
	 * peer, at which point the socket is closed <I>gracefully</I>. Upon
	 * reaching the linger timeout, the socket is closed <I>forcefully</I>, with
	 * a TCP RST. Enabling the option with a timeout of zero does a forceful
	 * close immediately. If the specified timeout value exceeds 65,535 it will
	 ...
	 * outgoing network I/O. When used in set, this is a suggestion to the
	 * kernel from the application about the size of buffers to use for the data
	...
	/**
	 * Set a hint the size of the underlying buffers used by the platform for
	 * incoming network I/O. When used in set, this is a suggestion to the
	 * kernel from the application about the size of buffers to use for the data
	 ...
	 * everything is OK). TCP will send another probe following another 2 hours
	 ...
	 * socket will be received through the socket input stream. When the option
	 * is disabled (which is the default) urgent data is silently discarded.
	 *
