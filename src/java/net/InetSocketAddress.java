 ...
 * be <I>unresolved</I> but can still be used on some circumstances like
 * connecting through a proxy.
 * <p>
 * It provides an immutable object used by sockets for binding, connecting, or
 * as returned values.
 * <p>
 * The <i>wildcard</i> is a special local IP address. It usually means "any" and
 * can only be used for {@code bind} operations.
 *
 * @see java.net.Socket
...
public class InetSocketAddress extends SocketAddress {
	 ...
	 * the port number a specified value.
	 * <p>
	 * A valid port value is between 0 and 65535. A port number of {@code zero}
	 * will let the system pick up an ephemeral port in a {@code bind}
	 * operation.
	 * <p>
	 * 
	 * @param port
	...
	public InetSocketAddress(InetAddress addr, int port) {
		holder = new InetSocketAddressHolder(null,
	 ...
	 * that attempt fails, the address will be flagged as <I>unresolved</I>.
	 * <p>
	 * If there is a security manager, its {@code checkConnect} method is called
	 * with the host name as its argument to check the permission to resolve it.
	 ...
	 *             if a security manager is present and permission to resolve
	 ...
	 * address will be flagged as <I>unresolved</I>.
	 * <p>
	 * A valid port value is between 0 and 65535. A port number of {@code zero}
	 * will let the system pick up an ephemeral port in a {@code bind}
	 * operation.
	...
	static {
		try {
			...
			FIELDS_OFFSET = unsafe
					.objectFieldOffset(InetSocketAddress.class.getDeclaredField("holder"));
			UNSAFE = unsafe;
	...
	/**
	 * Gets the {@code hostname}. Note: This method may trigger a name service
	 * reverse lookup if the address was created with a literal IP address.
	 *
	 ...
	 * have a hostname (it was created using a literal). This has the benefit of
	 * <b>not</b> attempting a reverse lookup.
	 *
	 ...
	 *
	 * @return {@code true} if the hostname couldn't be resolved into an
	 *         {@code InetAddress}.
	 */
	 ...
	 * <p>
	 * Two instances of {@code InetSocketAddress} represent the same address if
