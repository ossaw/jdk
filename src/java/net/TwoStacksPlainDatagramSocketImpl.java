...
class TwoStacksPlainDatagramSocketImpl extends AbstractPlainDatagramSocketImpl {
	...
	/*
	 * Needed for ipv6 on windows because we need to know if the socket was
	 * bound to ::0 or 0.0.0.0, when a caller asks for it. In this case, both
	 * sockets are used, but we don't know whether the caller requested ::0 or
	 * 0.0.0.0 and need to remember it here.
	 */
	...
	/*
	 * saved between successive calls to receive, if data is detected on both
	 * sockets at same time. To ensure that one socket is not starved, they
	 * rotate using this field
	 */
