...
public class Socket implements java.io.Closeable {
     ...
     * with the proxy host address and port number as its arguments. This could
     * result in a SecurityException.
     * <P>
     * Examples:
     * <UL>
     * <LI>{@code Socket s = new Socket(Proxy.NO_PROXY);} will create a plain
     * socket ignoring any other proxy configuration.</LI>
     * <LI>
     ...
     *
     * @param proxy
     *        a {@link java.net.Proxy Proxy} object specifying what kind
     *        of
     *        proxying should be used.
     * @throws IllegalArgumentException
     *         if the proxy is of an invalid type or
     *         {@code null}.
     ...
     * 
     * @param impl
     *        an instance of a <B>SocketImpl</B> the subclass wishes to use
     *        on the Socket.
     ...
     * the named host.
     * <p>
     * If the specified host is {@code null} it is the equivalent of specifying
     * the address as {@link java.net.InetAddress#getByName
     * InetAddress.getByName}{@code (null)}. In other words, it is equivalent to
     * specifying an address of the loopback interface.
     * </p>
     * <p>
     * If the application has specified a server socket factory, that factory's
     * {@code createSocketImpl} method is called to create the actual socket
     * implementation. Otherwise a "plain" socket is created.
     * <p>
     * If there is a security manager, its {@code checkConnect} method is called
     * with the host address and {@code port} as its arguments. This could
     * result in a SecurityException.
     *
     * @param host
     ...
     *            if the IP address of the host could
     *            not be determined.
     ...
     * </p>
     * <p>
     * A local port number of {@code zero} will let the system pick up a free
     * port in the {@code bind} operation.
     * </p>
     * <p>
     * If there is a security manager, its {@code checkConnect} method is called
     * with the host address and {@code port} as its arguments. This could
     ...
     *
     * @param host
     *        the name of the remote host, or {@code null} for the
     *        loopback
     *        address.
     * @param port
     *        the remote port
     ...
     *        {@code null} for
     *        the {@code anyLocal} address.
     * @param localPort
     *        the local port the socket is bound to, or {@code zero}
     *        for a
    ...
    public Socket(String host, int port, InetAddress localAddr, int localPort) throws IOException {
        this(host != null ? new InetSocketAddress(host, port)
                : new InetSocketAddress(InetAddress.getByName(null), port), new InetSocketAddress(localAddr,
                        localPort), true);
    }
     ...
     * specifying the address as the AnyLocal address (see
     * {@link java.net.InetAddress#isAnyLocalAddress
     * InetAddress.isAnyLocalAddress}{@code ()}).
     * <p>
     * A local port number of {@code zero} will let the system pick up a free
     * port in the {@code bind} operation.
     * </p>
    ...
    public Socket(InetAddress address, int port, InetAddress localAddr, int localPort) throws IOException {
        this(address != null ? new InetSocketAddress(address, port) : null, new InetSocketAddress(localAddr,
                localPort), true);
    }
     ...
     * </p>
     * <p>
     * If the stream argument is {@code true}, this creates a stream socket. If
     * the stream argument is {@code false}, it creates a datagram socket.
     * <p>
     * If the application has specified a server socket factory, that factory's
     * {@code createSocketImpl} method is called to create the actual socket
     * implementation. Otherwise a "plain" socket is created.
     ...
     *        the port number.
     * @param stream
     *        a {@code boolean} indicating whether this is a stream
     *        socket
     ...
     * <p>
     * If there is a security manager, its {@code checkConnect} method is called
     * with {@code host.getHostAddress()} and {@code port} as its arguments.
     ...
     *         bound.
     * @throws IllegalArgumentException
     *         if bindpoint is a SocketAddress subclass
     *         not supported by
     *         this socket
     * @throws SecurityException
     *         if a security manager exists and its
     *         {@code checkListen}
     *         method doesn't allow the bind to the
     *         local port.
     * @since 1.4
     * @see #isBound
     */
     ...
     * called with the local address and {@code -1} as its arguments to see if
     * the operation is allowed. If the operation is not allowed, the
     * {@link InetAddress#getLoopbackAddress loopback} address is returned.
     *
     * @return the local address to which the socket is bound, the loopback
     *         address if denied by the security manager, or the wildcard
     ...
     * method will continue to return the local port number after the socket is
     * closed.
     *
     ...
     * {@code InetSocketAddress } is {@link #close closed}, then this method
     * will continue to return an {@code InetSocketAddress} after the socket is
     * closed. In that case the returned {@code InetSocketAddress}'s address is
     * the {@link InetAddress#isAnyLocalAddress wildcard} address and its port
     * is the local port that it was bound to.
     * <p>
     * If there is a security manager set, its {@code checkConnect} method is
     * called with the local address and {@code -1} as its arguments to see if
     * the operation is allowed. If the operation is not allowed, a
     * {@code SocketAddress} representing the
     ...
     *         socket, or a {@code SocketAddress} representing the loopback
     *         address if denied by the security manager, or {@code null} if the
     *         socket is not bound yet.
     * @see #getLocalAddress()
     * @see #getLocalPort()
     ...
     *
     * @return the socket channel associated with this socket, or {@code null}
     ...
     *            stream, the
     *            socket is closed, the socket is not connected, or
     *            the
    ...
    /**
     * Returns setting for {@link SocketOptions#SO_LINGER SO_LINGER}. -1 returns
     * implies that the option is disabled.
     * The setting only affects socket close.
     *
    ...
    /**
     * Enable/disable {@link SocketOptions#SO_OOBINLINE SO_OOBINLINE} (receipt
     * of TCP urgent data)
     ...
     *
     * @param on
     *        {@code true} to enable {@link SocketOptions#SO_OOBINLINE
     *        SO_OOBINLINE}, {@code false} to disable.
     * @exception SocketException
     *            if there is an error in the underlying
     *            protocol, such as a
     *            TCP error.
     ...
     * timeout, a read() call on the InputStream associated with this Socket
     * will block for only this amount of time. If the timeout expires, a
     * <B>java.net.SocketTimeoutException</B> is raised, though the Socket is
     * still valid. The option <B>must</B> be enabled prior to entering the
     * blocking operation to have effect. The timeout must be {@code > 0}. A
    ...
    /**
     * Returns setting for {@link SocketOptions#SO_TIMEOUT SO_TIMEOUT}. 0
     * returns implies that the option is disabled (i.e., timeout of infinity).
     *
     ...
     * {@link SocketOptions#SO_SNDBUF SO_SNDBUF} option is used by the
     * platform's networking code as a hint for the size to set the underlying
     * network I/O buffers.
     * <p>
     * Because {@link SocketOptions#SO_SNDBUF SO_SNDBUF} is a hint, applications
     * that want to verify what size the buffers were set to should call
    ...
    /**
     * Get value of the {@link SocketOptions#SO_SNDBUF SO_SNDBUF} option for
     * this {@code Socket}, that is the buffer size used by the platform for
     * output on this {@code Socket}.
     * 
     * @return the value of the {@link SocketOptions#SO_SNDBUF SO_SNDBUF} option
     ...
     * specified value for this {@code Socket}. The
     * {@link SocketOptions#SO_RCVBUF SO_RCVBUF} option is used by the
     * platform's networking code as a hint for the size to set the underlying
     * network I/O buffers.
     * <p>
     ...
     * <p>
     * Because {@link SocketOptions#SO_RCVBUF SO_RCVBUF} is a hint, applications
     * that want to verify what size the buffers were set to should call
     * {@link #getReceiveBufferSize()}.
     * <p>
     * The value of {@link SocketOptions#SO_RCVBUF SO_RCVBUF} is also used to
     * set the TCP receive window that is advertized to the remote peer.
     ...
     * connecting the socket to its remote peer.</li>
     * </ol>
     * 
     * @param size
     ...
     * for this {@code Socket}, that is the buffer size used by the platform for
     * input on this {@code Socket}.
     *
    ...
    /**
     * Enable/disable the {@link SocketOptions#SO_REUSEADDR SO_REUSEADDR} socket
     * option.
     * <p>
     ...
     * known as the {@code TIME_WAIT} state or {@code 2MSL} wait state). For
     * applications using a well known socket address or port it may not be
     * possible to bind a socket to the required {@code SocketAddress} if there
     * is a connection in the timeout state involving the socket address or
     * port.
     * <p>
     * Enabling {@link SocketOptions#SO_REUSEADDR SO_REUSEADDR} prior to binding
     * the socket using {@link #bind(SocketAddress)} allows the socket to be
     * bound even though a previous connection is in a timeout state.
     * <p>
     * When a {@code Socket} is created the initial setting of
     * {@link SocketOptions#SO_REUSEADDR SO_REUSEADDR} is disabled.
     * <p>
     * The behaviour when {@link SocketOptions#SO_REUSEADDR SO_REUSEADDR} is
     * enabled or disabled after a socket is bound (See {@link #isBound()}) is
     * not defined.
     *
     * @param on
    ...
    public String toString() {
        try {
            if (isConnected())
                return "Socket[addr=" + getImpl().getInetAddress() + ",port=" + getImpl().getPort()
    ...
    /**
     * Sets the client socket implementation factory for the application. The
     * factory can be specified only once.
     * <p>
     * When an application creates a new client socket, the socket
     * implementation factory's {@code createSocketImpl} method is called to
     * create the actual socket implementation.
     * <p>
     ...
     * then it could invoke this method with the values {@code (1, 0, 0)}. If
