 ...
 * </pre>
 * <ul>
 * <li>CookieHandler is at the core of cookie management. User can call
 * CookieHandler.setDefault to set a concrete CookieHanlder implementation to be
 * used.</li>
 * <li>CookiePolicy.shouldAccept will be called by CookieManager.put to see
 * whether or not one cookie should be accepted and put into cookie store. User
 * can use any of three pre-defined CookiePolicy, namely ACCEPT_ALL, ACCEPT_NONE
 * and ACCEPT_ORIGINAL_SERVER, or user can define his own CookiePolicy
 * implementation and tell CookieManager to use it.</li>
 * <li>CookieStore is the place where any accepted HTTP cookie is stored in. If
 * not specified when created, a CookieManager instance will use an internal
 * in-memory implementation. Or user can implements one and tell CookieManager
 * to use it.</li>
 * <li>Currently, only CookieStore.add(URI, HttpCookie) and CookieStore.get(URI)
 * are used by CookieManager. Others are for completeness and might be needed by
 * a more sophisticated CookieStore implementation, e.g. a NetscapeCookieSotre.
 * </li>
 * </ul>
 * </blockquote>
 *
