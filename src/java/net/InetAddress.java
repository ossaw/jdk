 ...
 *
 * <P>
 * There is a <a href="doc-files/net-properties.html#Ipv4IPv6">couple of System
 * Properties</a> affecting how IPv4 and IPv6 addresses are used.
 * </P>
 *
...
public class InetAddress implements java.io.Serializable {
	...
	static class InetAddressHolder {
		 ...
		 * identification (see RFC 2818 and RFC 6125). If an address was created
		 * with a raw IP address, a reverse name lookup may introduce endpoint
		 * identification security issue via DNS forging.
		 *
	...
	/**
	 * Constructor for the Socket.accept() method. This creates an empty
	 * InetAddress, which is filled in by the accept() method. This InetAddress,
	 * however, is not put in the address cache, since it is not created by
	 * name.
	 */
	InetAddress() {
	 ...
	 * the try should take. If the operation times out before getting an answer,
	 ...
	 * host.
	 * <p>
	 * The {@code network interface} and {@code ttl} parameters let the caller
	 * specify which network interface the test will go through and the maximum
	 * number of hops the packets should go through. A negative value for the
	 * {@code ttl} will result in an IllegalArgumentException being thrown.
	 * <p>
	 * The timeout value, in milliseconds, indicates the maximum amount of time
	 * the try should take. If the operation times out before getting an answer,
	 * the host is deemed unreachable. A negative value will result in an
	 ...
	 *
	 * @param netif
	 *                the NetworkInterface through which the test will be done,
	 *                or
	 ...
	 *                the time, in milliseconds, before the call aborts
	 * @throws IllegalArgumentException
	 *                                  if either {@code timeout} or {@code ttl}
	 *                                  are negative.
	 * @return a {@code boolean}indicating if the address is reachable.
	 * @throws IOException
	 *                     if a network error occurs
	 ...
	 * called with the hostname and {@code -1} as its arguments to see if the
	 * operation is allowed. If the operation is not allowed, it will return the
	...
	/**
	 * Returns the hostname for this address. If the host is equal to null, then
	 * this address refers to any of the local machine's available network
	 * addresses. this is package private so SocketPermission can make calls
	 * into here without a security check.
	 *
	 * <p>
	 * If there is a security manager, this method first calls its
	 * {@code checkConnect} method with the hostname and {@code -1} as its
	 * arguments to see if the calling code is allowed to know the hostname for
	 * this IP address, i.e., to connect to the host. If the operation is not
	...
	private static String getHostFromNameService(InetAddress addr,
		...
		for (NameService nameService : nameServices) {
			try {
				...
				/*
				 * check to see if calling code is allowed to know the hostname
				 * for this IP address, ie, connect to the host
				 */
				if (check) {
				...
				/*
				 * now get all the IP addresses for this hostname, and make sure
				 * one of them matches the original IP address. We do this to
				 * try and prevent spoofing.
				 */
	 ...
	 * {@code true} if and only if the argument is not {@code null} and it
	 ...
	 * the form: hostname / literal IP address.
	 *
	...
	/**
	 * A cache that manages entries based on a policy specified at creation
	 * time.
	 */
	static final class Cache {
		...
		/**
		 * Add an entry to the cache. If there's already an entry then for this
		 * host then the entry will be replaced.
		 */
		...
		/**
		 * Query the cache for the specific host. If found then return its
		 * CacheEntry, or null if not found.
		 */
		public CacheEntry get(String host) {
			...
			if (entry != null && policy != InetAddressCachePolicy.FOREVER) {
				if (entry.expiration >= 0 && entry.expiration < System
						.currentTimeMillis()) {
					cache.remove(host);
					entry = null;
				}
	...
	/*
	 * Initialize cache and insert anyLocalAddress into the unknown array with
	 * no expiry.
	 */
	 ...
	 */
	private static void cacheAddresses(String hostname, InetAddress[] addresses,
			boolean success) {
	...
	/*
	 * Lookup hostname in cache (positive & negative cache). If found return
	 * addresses, null if not found.
	 */
				...
				nameService = java.security.AccessController.doPrivileged(
						new java.security.PrivilegedExceptionAction<NameService>() {
							public NameService run() {
								Iterator<NameServiceDescriptor> itr = ServiceLoader
										.load(NameServiceDescriptor.class)
										.iterator();
								while (itr.hasNext()) {
									NameServiceDescriptor nsd = itr.next();
									if (providerName.equalsIgnoreCase(nsd
											.getType() + "," + nsd
													.getProviderName())) {
										try {
	...
	static {
		...
		provider = AccessController.doPrivileged(new GetPropertyAction(
				propPrefix + n));
		while (provider != null) {
	 ...
	 * "{@code java.sun.com}", or a textual representation of its IP address.
	 * <p>
	 ...
	 * <p>
	 * The host name can either be a machine name, such as
	 * "{@code java.sun.com}", or a textual representation of its IP address. If
	 * a literal IP address is supplied, only the validity of the address format
	 * is checked.
	 *
	 * <p>
	 * For {@code host} specified in literal IPv6 address, either the form
	 ...
	 *
	 * <p>
	 * If the host is {@code null} then an {@code InetAddress} representing an
	 ...
	 * @exception UnknownHostException
	 *                                 if no IP address for the {@code host}
	 *                                 could be found, or
	 ...
	 *
	 * <p>
	 * For {@code host} specified in <i>literal IPv6 address</i>, either the
	 ...
	 *
	 * <p>
	 * If there is a security manager and {@code host} is not null and
	 * {@code host.length() } is not equal to zero, the security manager's
	 * {@code checkConnect} method is called with the hostname and {@code -1} as
	...
	private static InetAddress[] getAllByName(String host, InetAddress reqAddr)
		...
		if (Character.digit(host.charAt(0), 16) != -1 || (host.charAt(
				0) == ':')) {
			byte[] addr = null;
			int numericZone = -1;
			String ifname = null;
	...
	/**
	 * check if the literal address string has %nn appended returns -1 if not,
	 * or the numeric value otherwise.
	 *
	 * %nn may also be a string that represents the displayName of a currently
	...
	private static InetAddress[] getAllByName0(String host, InetAddress reqAddr,
		...
		/*
		 * make sure the connection to the host is allowed, before we give out a
		 * hostname
		 */
		if (check) {
	...
	private static InetAddress[] getAddressesFromNameService(String host,
			InetAddress reqAddr) throws UnknownHostException {
			...
			try {
				...
				for (NameService nameService : nameServices) {
					try {
						 ...
						 * constructor. if you do you will still be allocating
						 * space when the lookup fails.
						 */
						...
						if (host.equalsIgnoreCase("localhost")) {
							InetAddress[] local = new InetAddress[] { impl
									.loopbackAddress() };
							addresses = local;
							success = true;
							break;
				...
				// More to do?
				if (reqAddr != null && addresses.length > 1 && !addresses[0]
						.equals(reqAddr)) {
					// Find it?
					int i = 1;
	...
	/**
	 * Returns an {@code InetAddress} object given the raw IP address . The
	 * argument is in network byte order: the highest order byte of the address
	 * is in {@code getAddress()[0]}.
	 *
	 * <p>
	 ...
	 * with the local host name and {@code -1} as its arguments to see if the
	 * operation is allowed. If the operation is not allowed, an InetAddress
	 * representing the loopback address is returned.
	 *
	 * @return the address of the local host.
	 *
	 * @exception UnknownHostException
	 *                                 if the local host name could not be
	 *                                 resolved into an
	 *                                 address.
	 *
	 * @see SecurityManager#checkConnect
		...
		try {
			...
			synchronized (cacheLock) {
				...
				if (ret == null) {
					...
					try {
						localAddrs = InetAddress.getAddressesFromNameService(
								local, null);
					} catch (UnknownHostException uhe) {
						// Rethrow with a more informative error message.
						UnknownHostException uhe2 = new UnknownHostException(
								local + ": " + uhe.getMessage());
						uhe2.initCause(uhe);
						throw uhe2;
					}
	...
	static InetAddressImpl loadImpl(String implName) {
		 ...
		 * implementation object we instantiate, to which we delegate the real
		 * work (like native methods). This property can vary across
		 * implementations of the java. classes. The default is an empty String
		 * "".
		 */
		String prefix = AccessController.doPrivileged(new GetPropertyAction(
				"impl.prefix", ""));
		try {
	...
	private void readObjectNoData(ObjectInputStream s) throws IOException,
	...
	static {
		try {
			...
			FIELDS_OFFSET = unsafe.objectFieldOffset(InetAddress.class
	...
	private static final ObjectStreamField[] serialPersistentFields = {
			...
			new ObjectStreamField("address", int.class), new ObjectStreamField(
					"family", int.class), };
