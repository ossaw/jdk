...
class HostPortrange {
    ...
    HostPortrange(String scheme, String str) {
        ...
        if (str.charAt(0) == '[') {
            ...
            formatter.format("%02x%02x:%02x%02x:%02x%02x:%02x" + "%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x",
    ...
    /**
     * Convert to lower case, and check that all chars are ascii alphanumeric,
     * '-' or '.' only.
     */
    ...
    /**
     * returns true if the hostname part started with * hostname returns the
     * remaining part of the host component eg "*.foo.com" -> ".foo.com" or "*"
     * -> ""
     *
     * @return
     */
