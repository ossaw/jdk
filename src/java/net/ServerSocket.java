 ...
 * {@code SocketImpl} class. An application can change the socket factory that
...
public class ServerSocket implements java.io.Closeable {
	 ...
	 * {@code 0} means that the port number is automatically allocated,
	 ...
	 * to connect) is set to {@code 50}. If a connection indication arrives when
	 ...
	 * with the {@code port} argument as its argument to ensure the operation is
	 ...
	 * with the specified backlog. A port number of {@code 0} means that the
	 ...
	 * <p>
	 * The maximum queue length for incoming connection indications (a request
	 * to connect) is set to the {@code backlog} parameter. If a connection
	 ...
	 * allowed. This could result in a SecurityException.
	 *
	 * The {@code backlog} argument is the requested maximum number of pending
	 ...
	 * be greater than {@code 0}. If it is less than or equal to {@code 0}, then
	 ...
	 * requests to one of its addresses. If <i>bindAddr</i> is null, it will
	 * default accepting connections on any/all local addresses. The port must
	 * be between 0 and 65535, inclusive. A port number of {@code 0} means that
	 * the port number is automatically allocated, typically from an ephemeral
	 * port range. This port number can then be retrieved by calling
	 * {@link #getLocalPort getLocalPort}.
	 *
	 * <P>
	 * If there is a security manager, this method calls its {@code checkListen}
	 * method with the {@code port} argument as its argument to ensure the
	...
	/**
	 *
	 * Binds the {@code ServerSocket} to a specific address (IP address and port
	 * number).
	 * <p>
	 ...
	 * method will continue to return the local address after the socket is
	 * closed.
	 * <p>
	 ...
	 *
	 * @return the address to which this socket is bound, or the loopback
	 ...
	 * <p>
	 * If the socket was bound prior to being {@link #close closed}, then this
	 * method will continue to return the port number after the socket is
	 * closed.
	 *
	 ...
	 * <p>
	 * If the socket was bound prior to being {@link #close closed}, then this
	 * method will continue to return the address of the endpoint after the
	 * socket is closed.
	 * <p>
	 * If there is a security manager set, its {@code checkConnect} method is
	 * called with the local address and {@code -1} as its arguments to see if
	 ...
	 *
	 * <p>
	 * A new Socket {@code s} is created and, if there is a security manager,
	 * the security manager's {@code checkAccept} method is called with
	 * {@code s.getInetAddress().getHostAddress()} and {@code s.getPort()} as
	 ...
	 * their own subclass of socket. So a FooServerSocket will typically hand
	 * this method an <i>empty</i> FooSocket. On return from implAccept the
	 ...
	 * timeout, a call to accept() for this ServerSocket will block for only
	 * this amount of time. If the timeout expires, a
	 * <B>java.net.SocketTimeoutException</B> is raised, though the ServerSocket
	 * is still valid. The option <B>must</B> be enabled prior to entering the
	...
	/**
	 * Retrieve setting for {@link SocketOptions#SO_TIMEOUT SO_TIMEOUT}. 0
	 ...
	 * <p>
	 * When a {@code ServerSocket} is created the initial setting of
	 ...
	 * If there is a security manager set, its {@code checkConnect} method is
	 * called with the local address and {@code -1} as its arguments to see if
	 * the operation is allowed. If the operation is not allowed, an
	 * {@code InetAddress} representing the
	...
	/**
	 * Sets the server socket implementation factory for the application. The
	...
	/**
	 * Sets a default proposed value for the {@link SocketOptions#SO_RCVBUF
	 * SO_RCVBUF} option for sockets accepted from this {@code ServerSocket}.
	 * The value actually set in the accepted socket must be determined by
	 * calling {@link Socket#getReceiveBufferSize()} after the socket is
	 * returned by {@link #accept()}.
	 * <p>
	 ...
	 * for this {@code ServerSocket}, that is the proposed buffer size that will
	 * be used for Sockets accepted from this {@code ServerSocket}.
	 *
	 * <p>
