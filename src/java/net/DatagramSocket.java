 ...
 * In some implementations, broadcast packets may also be received when a
 ...
 * } Both cases will create a DatagramSocket able to receive broadcasts on UDP
 * port 8888.
 *
...
public class DatagramSocket implements java.io.Closeable {
    ...
    /**
     * Set when a socket is ST_CONNECTED until we are certain that any packets
     * which might have been received prior to calling connect() but not read by
     * the application have been read. During this time we check the source
     * address of all packets received to be sure they are from the connected
     * destination. Other packets are read but silently dropped.
     */
    private boolean explicitFilter = false;
    private int bytesLeftToFilter;
    /*
     * Connection state: ST_NOT_CONNECTED = socket not connected ST_CONNECTED =
     * socket connected ST_CONNECTED_NO_IMPL = socket connected but not at impl
     * level
     */
     ...
     * number). Binds socket if not already bound.
     * <p>
     * 
     * @param address
     *                The remote address.
    ...
    private synchronized void connectInternal(InetAddress address, int port)
        ...
        if (oldImpl || (impl instanceof AbstractPlainDatagramSocketImpl
                && ((AbstractPlainDatagramSocketImpl) impl)
     ...
     * local host machine. The socket will be bound to the
     * {@link InetAddress#isAnyLocalAddress wildcard} address, an IP address
     ...
     *
     * @param impl
     *             an instance of a <B>DatagramSocketImpl</B> the subclass
     *             wishes
     *             to use on the DatagramSocket.
     * @since 1.4
     */
     ...
     * <p>
     * If there is a security manager, its {@code checkListen} method is first
     * called with the port from the socket address as its argument to ensure
     * the operation is allowed. This could result in a SecurityException.
     *
     * @param bindaddr
     *                 local socket address to bind, or {@code null} for an
     *                 unbound
     *                 socket.
     *
     * @exception SocketException
     ...
     * <p>
     * If there is a security manager, its {@code checkListen} method is first
     * called with the {@code port} argument as its argument to ensure the
     ...
     * local port must be between 0 and 65535 inclusive. If the IP address is
     * 0.0.0.0, the socket will be bound to the
     * {@link InetAddress#isAnyLocalAddress wildcard} address, an IP address
     * chosen by the kernel.
     *
     * <p>
    ...
    private void checkOldImpl() {
        if (impl == null)
            return;
        // DatagramSocketImpl.peekdata() is a protected method, therefore we
        // need to use
    ...
    void createImpl() throws SocketException {
        if (impl == null) {
            if (factory != null) {
                ...
                boolean isMulticast = (this instanceof MulticastSocket) ? true
                        : false;
                impl = DefaultDatagramSocketImplFactory
                        .createDatagramSocketImpl(isMulticast);
    ...
    /**
     * Get the {@code DatagramSocketImpl} attached to this socket, creating it
     * if necessary.
     *
     * @return the {@code DatagramSocketImpl} attached to that DatagramSocket
     * @throws SocketException
     *                         if creation fails.
     * @since 1.4
     */
     ...
     * @throws SocketException
     *                                  if any error happens during the bind, or
     *                                  if the socket is
     *                                  already bound.
     ...
     *                                  if addr is a SocketAddress subclass not
     *                                  supported by this
     *                                  socket.
     * @since 1.4
     */
     ...
     * is a {@link InetAddress#isMulticastAddress multicast address}, the
     * security manager's
     * {@link java.lang.SecurityManager#checkMulticast(InetAddress)
     * checkMulticast} method is invoked with the given {@code address}.
     * Otherwise, the security manager's
     * {@link java.lang.SecurityManager#checkConnect(String,int) checkConnect}
     * and {@link java.lang.SecurityManager#checkAccept checkAccept} methods are
     * invoked, with the given {@code address} and {@code port}, to verify that
     * datagrams are permitted to be sent and received respectively.
     *
     * <p>
     * When a socket is connected, {@link #receive receive} and {@link #send
     * send} <b>will not perform any security checks</b> on incoming and
     * outgoing packets, other than matching the packet's and the socket's
     * address and port. On a send operation, if the packet's address is set and
     * the packet's address and the socket's address do not match, an
     * {@code IllegalArgumentException} will be thrown. A socket connected to a
     ...
     *                                  and it does not
     *                                  permit access to the given remote
     *                                  address
     *
     * @see #disconnect
     */
     ...
     * connect(InetAddress,int)} with the the given socket addresses IP address
     * and port number.
     *
     * @param addr
     *             The remote address.
    ...
    /**
     * Disconnects the socket. If the socket is closed or not connected, then
     ...
     * <p>
     * If the socket was bound prior to being {@link #close closed}, then this
     * method will continue to return {@code true} after the socket is closed.
     *
     ...
     * <p>
     * If the socket was connected prior to being {@link #close closed}, then
     * this method will continue to return {@code true} after the socket is
     * closed.
     *
     ...
     * <p>
     * If the socket was connected prior to being {@link #close closed}, then
     * this method will continue to return the connected address after the
     * socket is closed.
     *
     ...
     * {@code -1} if the socket is not connected.
     * <p>
     * If the socket was connected prior to being {@link #close closed}, then
     * this method will continue to return the connected port number after the
     * socket is closed.
     *
    ...
    /**
     * Sends a datagram packet from this socket. The {@code DatagramPacket}
     ...
     * First, if {@code p.getAddress().isMulticastAddress()} is true, this
     * method calls the security manager's {@code checkMulticast} method with
     * {@code p.getAddress()} as its argument. If the evaluation of that
     * expression is false, this method instead calls the security manager's
     * {@code checkConnect} method with arguments
     * {@code p.getAddress().getHostAddress()} and {@code p.getPort()}. Each
     * call to a security manager method could result in a SecurityException if
     * the operation is not allowed.
     *
     * @param p
     *          the {@code DatagramPacket} to be sent.
     *
     ...
     *                                     may be thrown if the socket is
     *                                     connected to a currently
     *                                     unreachable destination. Note, there
    ...
    public void send(DatagramPacket p) throws IOException {
        ...
        synchronized (p) {
            ...
            if (connectState == ST_NOT_CONNECTED) {
                // check the address is ok wiht the security manager on every
                // send.
                SecurityManager security = System.getSecurityManager();
                ...
                if (security != null) {
                    if (p.getAddress().isMulticastAddress()) {
                        ...
                        security.checkConnect(p.getAddress().getHostAddress(), p
                                .getPort());
                    }
                }
            } else {
     ...
     * the {@code DatagramPacket}'s buffer is filled with the data received. The
     * datagram packet also contains the sender's IP address, and the port
     * number on the sender's machine.
     * <p>
     * This method blocks until a datagram is received. The {@code length} field
     * of the datagram packet object contains the length of the received
     ...
     *
     * @param p
     *          the {@code DatagramPacket} into which to place the incoming
     *          data.
     * @exception IOException
     *                                     if an I/O error occurs.
    ...
    public synchronized void receive(DatagramPacket p) throws IOException {
        synchronized (p) {
            if (!isBound())
                bind(new InetSocketAddress(0));
            if (connectState == ST_NOT_CONNECTED) {
                // check the address is ok with the security manager before
                // every recv.
                SecurityManager security = System.getSecurityManager();
                if (security != null) {
                    while (true) {
                        ...
                        try {
                            ...
                            // it in a tmp buffer.
                            DatagramPacket tmp = new DatagramPacket(new byte[1],
                                    1);
                            getImpl().receive(tmp);
     ...
     *
     * <p>
     * If there is a security manager, its {@code checkConnect} method is first
     * called with the host address and {@code -1} as its arguments to see if
     * the operation is allowed.
     *
     * @see SecurityManager#checkConnect
     * @return the local address to which the socket is bound, {@code null} if
     *         the socket is closed, or an {@code InetAddress} representing
     *         {@link InetAddress#isAnyLocalAddress wildcard} address if either
     *         the socket is not bound, or the security manager
     *         {@code checkConnect} method does not allow the operation
     * @since 1.1
     */
     ...
     *         {@code -1} if the socket is closed, or {@code 0} if it is not
     *         bound yet.
     */
     ...
     * {@code > 0}. A timeout of zero is interpreted as an infinite timeout.
     *
     * @param timeout
     ...
     * {@code DatagramSocket}. The SO_SNDBUF option is used by the network
     ...
     * {@code DatagramPacket} that is larger than the setting of SO_SNDBUF then
     * it is implementation specific if the packet is sent or discarded.
     *
     * @param size
    ...
    /**
     * Get value of the SO_SNDBUF option for this {@code DatagramSocket}, that
     ...
     * {@code DatagramSocket}. The SO_RCVBUF option is used by the the network
     * implementation as a hint to size the underlying network I/O buffers. The
    ...
    /**
     * Get value of the SO_RCVBUF option for this {@code DatagramSocket}, that
     ...
     * {@code SO_REUSEADDR} socket option allows multiple sockets to be bound to
     * the same socket address if the {@code SO_REUSEADDR} socket option is
     ...
     * <p>
     * When a {@code DatagramSocket} is created the initial setting of
     * {@code SO_REUSEADDR} is disabled.
     * <p>
     * The behaviour when {@code SO_REUSEADDR} is enabled or disabled after a
     ...
     *                            {@code SO_RESUEADDR} socket option, or the
     *                            socket is
     *                            closed.
     * @since 1.4
     * @see #getReuseAddress()
    ...
    public synchronized void setReuseAddress(boolean on)
        ...
        if (isClosed())
            throw new SocketException("Socket is closed");
        // Integer instead of Boolean for compatibility with older
        // DatagramSocketImpl
        if (oldImpl)
            getImpl().setOption(SocketOptions.SO_REUSEADDR, new Integer(on ? -1
                    : 0));
        else
            getImpl().setOption(SocketOptions.SO_REUSEADDR, Boolean.valueOf(
                    on));
    }
     ...
     *
     * @return a {@code boolean} indicating whether or not SO_REUSEADDR is
     *         enabled.
     * @exception SocketException
     *                            if there is an error in the underlying
     *                            protocol, such as
     ...
     * 
     * @return a {@code boolean} indicating whether or not SO_BROADCAST is
     *         enabled.
     * @exception SocketException
     *                            if there is an error in the underlying
     *                            protocol, such as
     ...
     * Notes:
     * <p>
     * For Internet Protocol v4 the value consists of an {@code integer}, the
     * least significant 8 bits of which represent the value of the TOS octet in
     * IP packets sent by the socket. RFC 1349 defines the TOS values as
     * follows:
     *
     * <UL>
     ...
     * <p>
     * for Internet Protocol v6 {@code tc} is the value that would be placed
     * into the sin6_flowinfo field of the IP header.
     *
     * @param tc
     ...
     *         {@code null} if this socket was not created for a channel
     *
     * @since 1.4
     * @spec JSR-51
     */
     ...
     * <p>
     * Passing {@code null} to the method is a no-op unless the factory was
     * already set.
     *
     * <p>
