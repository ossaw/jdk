 ...
 * {@link #getPasswordAuthentication()} in a sub-class. This method will
 * typically use the various getXXX() accessor methods to get information about
 * the entity requesting authentication. It must then acquire a username and
 * password either by interacting with the user or through some other
 * non-interactive means. The credentials are then returned as a
 * {@link PasswordAuthentication} return value.
 * <p>
 * An instance of this concrete sub-class is then registered with the system by
 * calling {@link #setDefault(Authenticator)}. When authentication is required,
 * the system will invoke one of the requestPasswordAuthentication() methods
 * which in turn will call the getPasswordAuthentication() method of the
 * registered object.
 * <p>
...
public abstract class Authenticator {
	...
	/**
	 * Sets the authenticator that will be used by the networking code when a
	 * proxy or an HTTP server asks for authentication.
	 * <p>
	 * First, if there is a security manager, its {@code checkPermission} method
	 * is called with a {@code NetPermission("setDefaultAuthenticator")}
	 * permission. This may result in a java.lang.SecurityException.
	 *
	 * @param a
	 *          The authenticator to be set. If a is {@code null} then any
	 *          previously set authenticator is removed.
	 *
	 * @throws SecurityException
	 *                           if a security manager exists and its
	 *                           {@code checkPermission}
	 *                           method doesn't allow setting the default
	 *                           authenticator.
	 *
	...
	/**
	 * Ask the authenticator that has been registered with the system for a
	 * password.
	 * <p>
	 * First, if there is a security manager, its {@code checkPermission} method
	 * is called with a {@code NetPermission("requestPasswordAuthentication")}
	 * permission. This may result in a java.lang.SecurityException.
	 *
	 * @param addr
	 *                 The InetAddress of the site requesting authorization, or
	 *                 null
	 *                 if not known.
	 * @param port
	 ...
	 *                           if a security manager exists and its
	 *                           {@code checkPermission}
	 *                           method doesn't allow the password
	...
	/**
	 * Ask the authenticator that has been registered with the system for a
	 * password. This is the preferred method for requesting a password because
	 * the hostname can be provided in cases where the InetAddress is not
	 * available.
	 * <p>
	 * First, if there is a security manager, its {@code checkPermission} method
	 * is called with a {@code NetPermission("requestPasswordAuthentication")}
	 ...
	 * @param addr
	 *                 The InetAddress of the site requesting authentication, or
	 *                 null
	 *                 if not known.
	 * @param port
	 *                 the port for the requested connection.
	...
	/**
	 * Gets the {@code hostname} of the site or proxy requesting authentication,
	...
	/**
	 * Gets the {@code InetAddress} of the site requesting authorization, or
	 * {@code null} if not available.
	 *
	 * @return the InetAddress of the site requesting authorization, or null if
	 *         it's not available.
	 */
	 ...
	 * 
	 * @return an {@code int} indicating the port for the requested connection.
	 */
	...
	/**
	 * Give the protocol that's requesting the connection. Often this will be
	 * based on a URL, but in a future JDK it could be, for example, "SOCKS" for
	 * a password-protected SOCKS5 firewall.
	 *
	 * @return the protocol, optionally followed by "/version", where version is
	 *         a version number.
	 *
	 ...
	 *
	 * @return the prompt string given by the requestor (realm for http
	 *         requests)
	 */
	...
	/**
	 * Gets the scheme of the requestor (the HTTP scheme for an HTTP firewall,
	 * for example).
	 *
