 ...
 * >
 * <tr>
 * <th>{@code openConnection()}</th>
 * <th>{@code connect()}</th>
 * </tr>
 * <tr>
 * <td>Manipulate parameters that affect the connection to the remote resource.
 * </td>
 ...
 * </ul>
 * <p>
 * Default values for the {@code AllowUserInteraction} and {@code UseCaches}
 * parameters can be set using the methods
 * {@code setDefaultAllowUserInteraction} and {@code setDefaultUseCaches}.
 * <p>
 * Each of the above {@code set} methods has a corresponding {@code get} method
 * to retrieve the value of the parameter or general request property. The
 * specific parameters and general request properties that are applicable are
 * protocol specific.
 * <p>
 ...
 * </ul>
 * <p>
 * provide convenient access to these fields. The {@code getContentType} method
 * is used by the {@code getContent} method to determine the type of the remote
 ...
 * there are only two interesting methods: {@code getInputStream} and
 * {@code getContent}, which are mirrored in the {@code URL} class by
 * convenience methods.
 * <p>
 * More information on the request properties and header fields of an
 * {@code http} connection can be found at: <blockquote>
 * 
 * <pre>
...
public abstract class URLConnection {
	 ...
	 * the {@code URLConnection} constructor.
	 *
	...
	/**
	 * This variable is set by the {@code setDoInput} method. Its value is
	 * returned by the {@code getDoInput} method.
	 * <p>
	 * A URL connection can be used for input and/or output. Setting the
	 * {@code doInput} flag to {@code true} indicates that the application
	 * intends to read data from the URL connection.
	 * <p>
	...
	/**
	 * This variable is set by the {@code setDoOutput} method. Its value is
	 * returned by the {@code getDoOutput} method.
	 * <p>
	 * A URL connection can be used for input and/or output. Setting the
	 * {@code doOutput} flag to {@code true} indicates that the application
	 * intends to write data to the URL connection.
	 * <p>
	...
	/**
	 * If {@code true}, this {@code URL} is being examined in a context in which
	 * it makes sense to allow user interactions such as popping up an
	 * authentication dialog. If {@code false}, then no user interaction is
	 * allowed.
	 * <p>
	 * The value of this field can be set by the {@code setAllowUserInteraction}
	 * method. Its value is returned by the {@code getAllowUserInteraction}
	 * method. Its default value is the value of the argument in the last
	 * invocation of the {@code setDefaultAllowUserInteraction} method.
	 *
	...
	/**
	 * If {@code true}, the protocol is allowed to use caching whenever it can.
	 * If {@code false}, the protocol must always try to get a fresh copy of the
	 * object.
	 * <p>
	 * This field is set by the {@code setUseCaches} method. Its value is
	 * returned by the {@code getUseCaches} method.
	 * <p>
	 ...
	 * <p>
	 * A nonzero value gives a time as the number of milliseconds since January
	 * 1, 1970, GMT. The object is fetched only if it has been modified more
	 * recently than that time.
	 * <p>
	 * This variable is set by the {@code setIfModifiedSince} method. Its value
	 * is returned by the {@code getIfModifiedSince} method.
	 * <p>
	 * The default value of this field is {@code 0}, indicating that the
	 * fetching must always occur.
	 *
	...
	/**
	 * If {@code false}, this connection object has not created a communications
	 * link to the specified URL. If {@code true}, the communications link has
	 * been established.
	 */
	 ...
	 * load the user-specific table, defined by
	 * &quot;content.types.user.table&quot; property. If that fails, it tries to
	 * load the default built-in table.
	 *
	 ...
	 * manager's {@code checkSetFactory} method to ensure the operation is
	 ...
	 *
	 * @return an {@code int} that indicates the connect timeout value in
	 *         milliseconds
	 * @see #setConnectTimeout(int)
	 * @see #connect()
	 * @since 1.5
	 ...
	 *
	 * @param timeout
	 *            an {@code int} that specifies the timeout value to be used in
	 *            milliseconds
	 * @throws IllegalArgumentException
	 *             if the timeout parameter is negative
	 *
	 ...
	 *
	 * @return the content encoding of the resource that the URL references, or
	 *         {@code null} if not known.
	 * @see java.net.URLConnection#getHeaderField(java.lang.String)
	 */
	 ...
	 *         0 if not known. The value is the number of milliseconds since
	 *         January 1, 1970 GMT.
	 * @see java.net.URLConnection#getHeaderField(java.lang.String)
	 */
	...
	/**
	 * Returns the value of the {@code last-modified} header field. The result
	 * is the number of milliseconds since January 1, 1970 GMT.
	 *
	 * @return the date the resource referenced by this {@code URLConnection}
	 ...
	 * (e.g., {@code http-ng}) have pre-parsed headers. Classes for that
	...
	/**
	 * Returns the key for the {@code n}<sup>th</sup> header field. It returns
	...
	/**
	 * Returns the value for the {@code n}<sup>th</sup> header field. It returns
	 * {@code null} if there are fewer than {@code n+1}fields.
	 * <p>
	 * This method can be used in conjunction with the
	 * {@link #getHeaderFieldKey(int) getHeaderFieldKey} method to iterate
	 ...
	 * the {@code getContentType} method. If this is the first time that the
	 * application has seen that specific content type, a content handler for
	 * that content type is created:
	 * <ol>
	 * <li>If the application has set up a content handler factory instance
	 * using the {@code setContentHandlerFactory} method, the
	 * {@code createContentHandler} method of that instance is called with the
	 * content type as an argument; the result is a content handler for that
	 * content type.
	 * <li>If no content handler factory has yet been set up, or if the
	 * factory's {@code createContentHandler} method returns {@code null}, then
	 * the application loads the class named: <blockquote>
	 * 
	 * <pre>
	 *         sun.net.www.content.&lt;<i>contentType</i>&gt;
	 * </pre>
	 * 
	 * </blockquote> where &lt;<i>contentType</i>&gt; is formed by taking the
	 * content-type string, replacing all slash characters with a {@code period}
	 * ('.'), and all other non-alphanumeric characters with the underscore
	 * character '{@code _}'. The alphanumeric characters are specifically the
	 * 26 uppercase ASCII letters '{@code A}' through '{@code Z}', the 26
	 * lowercase ASCII letters '{@code a}' through '{@code z}', and the 10 ASCII
	 * digits '{@code 0}' through '{@code 9}'. If the specified class does not
	 * exist, or is not a subclass of {@code ContentHandler}, then an
	 ...
	 * returns {@code java.security.AllPermission}. Subclasses should override
	 * this method and return the permission that best represents the permission
	 ...
	 * after connecting. For example, an HTTP sever, say foo.com, may redirect
	 * the connection to a different host, say bar.com. Before connecting the
	 ...
	 * recipient to learn about a particular URL. In the first case, the
	 * permission should be obtained <em>after</em> the object has been
	 * obtained. For example, in an HTTP connection, this will represent the
	 * permission to connect to the host from which the data was ultimately
	 * fetched. In the second case, the permission should be obtained and tested
	 ...
	 *
	 * @exception IOException
	 *                if the computation of the permission requires network or
	 *                file I/O and an exception occurs while computing it.
	 */
	 ...
	 * flag to true if you intend to use the URL connection for input, false if
	 ...
	 * flag to true if you intend to use the URL connection for output, false if
	...
	/**
	 * Set the value of the {@code allowUserInteraction} field of this
	 * {@code URLConnection}.
	 *
	...
	/**
	 * Returns the value of the {@code allowUserInteraction} field for this
	 * object.
	 *
	 * @return the value of the {@code allowUserInteraction} field for this
	 *         object.
	...
	/**
	 * Sets the default value of the {@code allowUserInteraction} field for all
	 ...
	 * button in a browser). If the UseCaches flag on a connection is true, the
	 * connection is allowed to use whatever caches it can. If false, caches are
	 * to be ignored. The default value comes from DefaultUseCaches, which
	 * defaults to true.
	 *
	 * @param usecaches
	 *            a {@code boolean} indicating whether or not to allow caching
	 * @throws IllegalStateException
	 *             if already connected
	 * @see #getUseCaches()
	 */
	...
	/**
	 * Returns the value of this {@code URLConnection}'s {@code useCaches}
	 * field.
	 *
	 * @return the value of this {@code URLConnection}'s {@code useCaches}
	 *         field.
	...
	/**
	 * Sets the value of the {@code ifModifiedSince} field of this
	 * {@code URLConnection} to the specified value.
	 *
	...
	/**
	 * Returns the default value of a {@code URLConnection}'s {@code useCaches}
	 * flag.
	 * <p>
	 * Ths default is "sticky", being a part of the static state of all
	 * URLConnections. This flag applies to the next, and all following
	 * URLConnections that are created.
	 *
	 * @return the default value of a {@code URLConnection}'s {@code useCaches}
	 *         flag.
	...
	/**
	 * Sets the default value of the {@code useCaches} field to the specified
	 * value.
	 *
	 ...
	 * {@code URLConnection} is created, it is initialized with these
	 * properties.
	 *
	 * @param key
	 *            the keyword by which the request is known (e.g.,
	...
	/**
	 * Sets the {@code ContentHandlerFactory} of an application. It can be
	 * called at most once by an application.
	 * <p>
	 * The {@code ContentHandlerFactory} instance is used to construct a content
	 * handler from a content type
	 * <p>
	 * If there is a security manager, this method first calls the security
	 * manager's {@code checkSetFactory} method to ensure the operation is
	 * allowed. This could result in a SecurityException.
	...
	/*
	 * Media types are in the format: type/subtype*(; parameter). For looking up
	 * the content handler, we should ignore those parameters.
	 */
	...
	/**
	 * Looks for a content handler in a user-defineable set of places. By
	 * default it looks in sun.net.www.content, but users can define a
	 * vertical-bar delimited set of class prefixes to search through in
	 * addition by defining the java.content.handler.pkgs property. The class
	...
	/**
	 * Utility function to map a MIME content type into an equivalent pair of
	 * class name components. For example: "text/html" would be returned as
	 * "text.html"
	 */
	 ...
	 * content handlers. Tries to get the java.content.handler.pkgs property to
	 * use as a set of package prefixes to search. Whether or not that property
	 * has been defined, the sun.net.www.content is always the last one on the
	...
	/**
	 * Tries to determine the content type of an object, based on the specified
	 * "file" component of a URL. This is a convenience method that can be used
	 ...
	 * the beginning of the input stream. This method can be used by subclasses
	 * that override the {@code getContentType} method.
	 * <p>
	 * Ideally, this routine would not be needed. But many {@code http} servers
	...
	static public String guessContentTypeFromStream(InputStream is) throws IOException {
		...
		if (c1 == 0x00 && c2 == 0x00 && c3 == 0xfe && c4 == 0xff) {
			if (c5 == 0 && c6 == 0 && c7 == 0 && c8 == '<' && c9 == 0 && c10 == 0 && c11 == 0
					&& c12 == '?' && c13 == 0 && c14 == 0 && c15 == 0 && c16 == 'x') {
				return "application/xml";
			}
		}

		if (c1 == 0xff && c2 == 0xfe && c3 == 0x00 && c4 == 0x00) {
			if (c5 == '<' && c6 == 0 && c7 == 0 && c8 == 0 && c9 == '?' && c10 == 0 && c11 == 0
					&& c12 == 0 && c13 == 'x' && c14 == 0 && c15 == 0 && c16 == 0) {
				return "application/xml";
			}
		}
		...
		if (c1 == 0xD0 && c2 == 0xCF && c3 == 0x11 && c4 == 0xE0 && c5 == 0xA1 && c6 == 0xB1
				&& c7 == 0x1A && c8 == 0xE1) {

			/*
			 * Above is signature of Microsoft Structured Storage. Below this,
			 * could have tests for various SS entities. For now, just test for
			 * FlashPix.
			 */
			if (checkfpx(is)) {
		...
		if (c1 == 'R' && c2 == 'I' && c3 == 'F' && c4 == 'F') {
			/*
			 * I don't know if this is official but evidence suggests that .wav
	...
	/**
	 * Check for FlashPix image data in InputStream is. Return true if the
	 * stream has FlashPix data, false otherwise. Before calling this method,
	 * the stream should have already been checked to be sure it contains
	...
	static private boolean checkfpx(InputStream is) throws IOException {
		 ...
		 * Lacking that, need to dig via offsets to get to the FlashPix ClassID.
		 * Details:
		 *
		 ...
		 *
		 * where: clsidOffset = 0x50. rootEntryOffset = headerSize +
		 * sectorSize*sectDirStart + 128*rootEntryDirectory
		 *
		 * where: headerSize = 0x200 (always) sectorSize = 2 raised to power of
		 * uSectorShift, which is found in the header at offset 0x1E.
		 * sectDirStart = found in the header at offset 0x30. rootEntryDirectory
		 * = in general, should search for directory labelled as root. We will
		 * assume value of 0 (i.e., rootEntry is in first directory)
		 */
		...
		/*
		 * How far can we skip? Is there any performance problem here? This skip
		 * can be fairly long, at least 0x4c650 in at least one case. Have to
		 * assume that the skip will fit in an int. Leave room to read whole
		 * root dir
		 */
		...
		/*
		 * should be at beginning of ClassID, which is as follows (in Intel byte
		 * order): 00 67 61 56 54 C1 CE 11 85 53 00 AA 00 A1 F9 5B
		 *
		 * This is stored from Windows as long,short,short,char[8] so for byte
		 * order changes, the order only changes for the first 8 bytes in the
		 * ClassID.
		 *
		 * Test against this, ignoring second byte (Intel) since this could
		 * change depending on part of Fpx file we have.
		 */
		...
		// intel byte order
		if (byteOrder == 0xFE && c[0] == 0x00 && c[2] == 0x61 && c[3] == 0x56 && c[4] == 0x54
				&& c[5] == 0xC1 && c[6] == 0xCE && c[7] == 0x11 && c[8] == 0x85 && c[9] == 0x53
				&& c[10] == 0x00 && c[11] == 0xAA && c[12] == 0x00 && c[13] == 0xA1 && c[14] == 0xF9
				&& c[15] == 0x5B) {
			is.reset();
			return true;
		}
		...
		else if (c[3] == 0x00 && c[1] == 0x61 && c[0] == 0x56 && c[5] == 0x54 && c[4] == 0xC1
				&& c[7] == 0xCE && c[6] == 0x11 && c[8] == 0x85 && c[9] == 0x53 && c[10] == 0x00
				&& c[11] == 0xAA && c[12] == 0x00 && c[13] == 0xA1 && c[14] == 0xF9
				&& c[15] == 0x5B) {
			is.reset();
			return true;
		}
