...
/**
 * SOCKS (V4 & V5) TCP socket implementation (RFC 1928). This is a subclass of
 * PlainSocketImpl. Note this class should <b>NOT</b> be public.
 */
...
class SocksSocketImpl extends PlainSocketImpl implements SocksConsts {
	...
	private synchronized void privilegedConnect(final String host, final int port,
	...
	private int readSocksReply(InputStream in, byte[] data, long deadlineMillis)
		...
		for (int attempts = 0; received < len && attempts < 3; attempts++) {
			int count;
			try {
				count = ((SocketInputStream) in).read(data, received, len - received,
	 ...
	 */
	private boolean authenticate(byte method, InputStream in, BufferedOutputStream out)
			throws IOException {
	...
	private boolean authenticate(byte method, InputStream in, BufferedOutputStream out,
		 ...
		 * provided Authenticator, if any - the user.name & no password
			...
			PasswordAuthentication pw = java.security.AccessController
					.doPrivileged(new java.security.PrivilegedAction<PasswordAuthentication>() {
						public PasswordAuthentication run() {
							return Authenticator.requestPasswordAuthentication(server, addr,
									serverPort, "SOCKS5", "SOCKS authentication", null);
						}
					});
			if (pw != null) {
		...
		/**
		 * GSSAPI authentication mechanism. Unfortunately the RFC seems out of
		 * sync with the Reference implementation. I'll leave this in for future
		 * completion.
		 */
		// if (method == GSSAPI) {
		// try {
		...
		// /* RFC 1961 states that if Context initialisation fails the
		// connection
		// MUST be closed */
		// e.printStackTrace();
		// in.close();
	 ...
	 * connect to the SOCKS proxy and negotiate the access. If the proxy grants
	 * the connections, then the connect is successful and all further traffic
	 ...
	 *             if endpoint is null or a SocketAddress subclass not supported
	 *             by this socket
	 */
	@Override
	protected void connect(SocketAddress endpoint, int timeout) throws IOException {
		...
		if (server == null) {
			...
			// IPv6 litteral?
			if (epoint.getAddress() instanceof Inet6Address && (!host.startsWith("["))
					&& (host.indexOf(":") >= 0)) {
				host = "[" + host + "]";
			}
			try {
	 ...
	 * means "accept incoming connection from", so the SocketAddress is the the
	...
	protected synchronized void socksBind(InetSocketAddress saddr) throws IOException {
		...
		if (server == null) {
			...
			try {
				uri = new URI(
						"serversocket://" + ParseUtil.encodePath(host) + ":" + saddr.getPort());
			} catch (URISyntaxException e) {
				// This shouldn't happen
				assert false : e;
				uri = null;
	...
	protected void acceptFrom(SocketImpl s, InetSocketAddress saddr) throws IOException {
		...
		/**
		 * This is where we have to do some fancy stuff. The datastream from the
		 * socket "accepted" by the proxy will come through the cmdSocket. So we
		 * have to swap the socketImpls
		 */
