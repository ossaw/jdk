 ...
 * <tr>
 * <td>{@code 1080:0:0:0:8:800:200C:417A}
 * <td>
 * </tr>
 * </table>
 * </blockquote>
 ...
 * available to compress the zeros. The use of "::" indicates multiple groups of
 ...
 * <table cellpadding=0 cellspacing=0 summary="layout">
 * <tr>
 * <td>{@code ::FFFF:129.144.52.38}
 * <td>
 * </tr>
 * <tr>
 * <td>{@code ::129.144.52.38}
 * <td>
 * </tr>
 * </table>
 * </blockquote>
 ...
 * <tr>
 * <th valign=top><i>IPv4-mapped address</i></th>
 ...
 * representation. However, it will be converted into an IPv4 address.</td>
 * </tr>
 * </table>
 * </blockquote>
 *
 ...
 * <ol>
 * <li><i>As a numeric identifier.</i> This must be a positive integer that
 * identifies the particular interface and scope as understood by the system.
 * Usually, the numeric values can be determined through administration tools on
 * the system. Each interface may have multiple values, one for each scope. If
 * the scope is unspecified, then the default value used is zero.</li>
...
public final class Inet6Address extends InetAddress {
	...
	private class Inet6AddressHolder {
		...
		/**
		 * scope_id. The scope specified when the object is created. If the
		 * object is created with an interface name, then the scope_id is not
		 * determined until the time it is needed.
		 */
		int scope_id; // 0
		...
		/**
		 * scoped interface. scope_id is derived from this as the scope_id of
		 * the first address whose scope is the same as this address for the
		 * named interface.
		 */
	...
	/*
	 * checking of value for scope_id should be done by caller scope_id must be
	 * >= 0, or -1 to indicate not being set
	 */
	 ...
	 * {@link InetAddress#getByAddress(String,byte[])} except that the IPv6
	 * scope_id is set to the value corresponding to the given interface for the
	 * address type specified in {@code addr}. The call will fail with an
	 * UnknownHostException if the given interface does not have a numeric
	 * scope_id assigned for the given address type (eg. link-local or
	 * site-local). See <a href="Inet6Address.html#scoped">here</a> for a
	 ...
	 *             if IP address is of illegal length, or if the interface does
	 *             not have a numeric scope_id assigned for the given address
	 *             type.
	 *
	 * @since 1.5
	 */
	 ...
	 * Create an Inet6Address in the exact manner of
	 * {@link InetAddress#getByAddress(String,byte[])} except that the IPv6
	 * scope_id is set to the given numeric value. The scope_id is not checked
	 * to determine if it corresponds to any interface on the system. See
	...
	/*
	 * check the two Ipv6 addresses and return false if they are both non global
	 * address types, but not the same. (ie. one is sitelocal and the other
	...
	/**
	 * @serialField ipaddress
	 *                  byte[]
	 * @serialField scope_id
	 *                  int
	 * @serialField scope_id_set
	 *                  boolean
	 * @serialField scope_ifname_set
	 *                  boolean
	 * @serialField ifname
	 *                  String
	 */
	...
	static {
		try {
			...
			FIELDS_OFFSET = unsafe
					.objectFieldOffset(Inet6Address.class.getDeclaredField("holder6"));
			UNSAFE = unsafe;
	...
	private void readObject(ObjectInputStream s) throws IOException, ClassNotFoundException {
		...
		Inet6AddressHolder h = new Inet6AddressHolder(ipaddress, scope_id, scope_id_set,
				scope_ifname, scope_ifname_set);
	...
	static String numericToTextFormat(byte[] src) {
		...
		for (int i = 0; i < (INADDRSZ / INT16SZ); i++) {
			sb.append(Integer
					.toHexString(((src[i << 1] << 8) & 0xff00) | (src[(i << 1) + 1] & 0xff)));
