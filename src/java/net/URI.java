 ...
 * Resource Identifiers (URI): Generic Syntax</i></a>, amended by
 ...
 * supports scope_ids. The syntax and usage of scope_ids is described
 * <a href="Inet6Address.html#scoped">here</a>. This class provides constructors
 * for creating URI instances from their components or by parsing their string
 ...
 * <tr>
 * <td>{@code mailto:java-net@java.sun.com}
 * <td>
 * </tr>
 * <tr>
 * <td>{@code news:comp.lang.java}
 * <td>
 * </tr>
 * <tr>
 * <td>{@code urn:isbn:096139210x}</td>
 * </tr>
 * </table>
 * </blockquote>
 *
 ...
 * component. An opaque URI has a scheme, a scheme-specific part, and possibly a
 * fragment, but has no other components. A hierarchical URI always has a path
 * (though it may be empty) and a scheme-specific-part (which at least contains
 ...
 * <td>The US-ASCII alphabetic characters, {@code 'A'}&nbsp;through&nbsp;
 * {@code 'Z'} and {@code 'a'}&nbsp;through&nbsp;{@code 'z'}</td>
 * </tr>
 * <tr>
 * <th valign=top><i>digit</i></th>
 * <td>The US-ASCII decimal digit characters, {@code '0'}&nbsp;through&nbsp;
 * {@code '9'}</td>
 * </tr>
 * <tr>
 ...
 * <td>The characters in the string {@code ",;:$&+="}</td>
 * </tr>
 * <tr>
 ...
 * <td>Escaped octets, that is, triplets consisting of the percent character (
 * {@code '%'}) followed by two hexadecimal digits ({@code '0'}-{@code '9'},
 * {@code 'A'}-{@code 'F'}, and {@code 'a'}-{@code 'f'})</td>
 * </tr>
 * <tr>
 * <th valign=top><i>other</i></th>
 * <td>The Unicode characters that are not in the US-ASCII character set, are
 * not control characters (according to the
 * {@link java.lang.Character#isISOControl(char) Character.isISOControl}
 * method), and are not space characters (according to the
 * {@link java.lang.Character#isSpaceChar(char) Character.isSpaceChar}
 * method)&nbsp;&nbsp;<i>(<b>Deviation from RFC 2396</b>, which is limited to
 * US-ASCII)</i></td>
 * </tr>
 * </table>
 * </blockquote>
 ...
 * user-info, path, query, and fragment components differ slightly in terms of
 ...
 * sequence of escaped octets that represent that character in the UTF-8
 * character set. The Euro currency symbol ({@code '\u005Cu20AC'}), for example,
 * is encoded as {@code "%E2%82%AC"}. <i>(<b>Deviation from RFC&nbsp;2396</b>,
 ...
 * it. The space character, for example, is quoted by replacing it with
 * {@code "%20"}. UTF-8 contains US-ASCII, hence for US-ASCII characters this
 ...
 * replacing it with the sequence of characters that it represents in the UTF-8
 * character set. UTF-8 contains US-ASCII, hence decoding has the effect of
 * de-quoting any quoted US-ASCII characters as well as that of decoding any
 * encoded non-US-ASCII characters. If a
 * <a href="../nio/charset/CharsetDecoder.html#ce">decoding error</a> occurs
 ...
 * any illegal characters in its argument to be quoted and preserves any escaped
 ...
 * <p>
 * The
 * {@linkplain #URI(java.lang.String,java.lang.String,java.lang.String,int,java.lang.String,java.lang.String,java.lang.String)
 * multi-argument constructors} quote illegal characters as required by the
 * components in which they appear. The percent character ({@code '%'}) is
 * always quoted by these constructors. Any <i>other</i> characters are
 * preserved.
 * </p>
 * </li>
 *
 ...
 * getRawFragment}, {@link #getRawAuthority() getRawAuthority}, and
 * {@link #getRawSchemeSpecificPart() getRawSchemeSpecificPart} methods return
 * the values of their corresponding components in raw form, without
 * interpreting any escaped octets. The strings returned by these methods may
 * contain both escaped octets and <i>other</i> characters, and will not contain
 ...
 * {@link #getAuthority() getAuthority}, and {@link #getSchemeSpecificPart()
 ...
 * both <i>other</i> characters and illegal characters, and will not contain any
 * escaped octets.
 * </p>
 * </li>
 *
 ...
 * &nbsp;), and that does not encode characters except those that must be
 * quoted, the following identities also hold:
 * 
 * <pre>
 ...
 * string is parsed according to the generic syntax without regard to the
 ...
 * the resource that it describes. A URL must be absolute, that is, it must
 * always specify a scheme. A URL string is parsed according to its scheme. A
 * stream handler is always established for a URL, and in fact it is impossible
 * to create a URL instance for a scheme for which no handler is available.
 * Equality and hashing depend upon both the scheme and the Internet address of
 * the host, if any; comparison is not defined. In other words, a URL is a
 * structured string that supports the syntactic operation of resolution as well
 ...
 *      <a href="http://www.ietf.org/rfc/rfc2732.txt"><i>RFC&nbsp;2732: Format
 *      for Literal IPv6 Addresses in URLs</i></a>, <br>
 *      <a href="URISyntaxException.html">URISyntaxException</a>
 */
...
public final class URI implements Comparable<URI>, Serializable {
	 ...
	 * non-empty path, a query component, or a fragment component. This allows
	 * the parsing of URIs such as {@code "file:///foo/bar"}, which seems to be
	 * the intent of RFC&nbsp;2396 although the grammar does not permit it. If
	 ...
	 * <a href="http://www.ietf.org/rfc/rfc2732.txt">RFC&nbsp;2732</a>: Each
	 * element of a dotted-quad address must contain no more than three decimal
	 * digits. Each element is further constrained to have a value no greater
	 * than 255.
	 * </p>
	 * </li>
	 *
	 ...
	 * permitted to start with an <i>alphanum</i> character. This seems to be
	 * the intent of
	 * <a href="http://www.ietf.org/rfc/rfc2396.txt">RFC&nbsp;2396</a>
	 ...
	 * be enclosed in square brackets ({@code '['} and {@code ']'}) as specified
	 * by <a href="http://www.ietf.org/rfc/rfc2732.txt">RFC&nbsp;2732</a>. The
	 * IPv6 address itself must parse according to
	 * <a href="http://www.ietf.org/rfc/rfc2373.txt">RFC&nbsp;2373</a>. IPv6
	 * addresses are further constrained to describe no more than sixteen bytes
	 * of address information, a constraint implicit in RFC&nbsp;2373 but not
	 ...
	 * user-information, path, query, and fragment components, as well as in the
	 * authority component if the authority is registry-based. This allows URIs
	 ...
	 * commercial-at character ({@code '@'}). Any character not in the
	 ...
	 * address but is not enclosed in square brackets ({@code '['} and
	 ...
	 * appended, followed by the query. Any character that is not a
	 ...
	 * appended, followed by the fragment. Any character that is not a legal URI
	 * character is quoted.
	 * </p>
	 * </li>
	 *
	 ...
	 * {@link #parseServerAuthority()} method upon the result; this may cause a
	 * {@link URISyntaxException} to be thrown.
	 * </p>
	 *
	 * @param scheme
	 *            Scheme name
	 ...
	 * followed by the authority. If the authority contains a literal IPv6
	 * address then the address must be enclosed in square brackets ({@code '['}
	 * and {@code ']'}). Any character not in the <i>unreserved</i>,
	 * <i>punct</i>, <i>escaped</i>, or <i>other</i> categories, and not equal
	 * to the commercial-at character ({@code '@'}), is
	 ...
	 * that is not a <a href="#legal-chars">legal URI character</a> is
	 * <a href="#quote">quoted</a>.
	 * </p>
	 * </li>
	 *
	 ...
	 * <p>
	 * Finally, if a fragment is given then a hash character ({@code '#'}) is
	 * appended to the string, followed by the fragment. Any character that is
	 ...
	 * cause a {@link URISyntaxException} to be thrown.
	 * </p>
	 *
	 * @param scheme
	 *            Scheme name
	 ...
	 * always distinguish a malformed server-based authority from a legitimate
	 * registry-based authority. It must therefore treat some instances of the
	 ...
	 * both of these segments are removed. This step is repeated until it is no
	 * longer applicable.
	 * </p>
	 * </li>
	 *
	 ...
	 * character ({@code ':'}), then a {@code "."} segment is prepended. This
	 * prevents a relative URI with a path such as {@code "a:b/c/d"} from later
	 * being re-parsed as an opaque URI with a scheme of {@code "a"} and a
	 * scheme-specific part of {@code "b/c/d"}. <b><i>(Deviation from
	 * RFC&nbsp;2396)</i></b>
	 * </p>
	 * </li>
	 *
	 ...
	 * their removal. A normalized path will begin with a {@code "."} segment if
	 * one was inserted by step 3 above. Otherwise, a normalized path will not
	 * contain any {@code "."} or {@code ".."} segments.
	 * </p>
	 *
	 ...
	 * URI. This is done by concatenating all but the last segment of this URI's
	 * path, if any, with the given URI's path and then normalizing the result
	 ...
	 * {@code (URI.}{@link #create(String) create}{@code (str))}.
	 * </p>
	 *
	 * @param str
	 *            The string to be parsed into a URI
	 ...
	 * authority components of the two URIs are not identical, or if the path of
	 * this URI is not a prefix of the path of the given URI, then the given URI
	 * is returned.
	 * </p>
	 * </li>
	 *
	 ...
	 *
	 * @return The scheme component of this URI, or {@code null} if the scheme
	 *         is undefined
	 */
	 ...
	 * </p>
	 *
	 * @return The raw scheme-specific part of this URI (never {@code null})
	 */
	 ...
	 * </p>
	 *
	 * @return The decoded scheme-specific part of this URI (never {@code null})
	 */
	 ...
	 * constrained to have valid user-information, host, and port components.
	 * </p>
	 *
	 * @return The raw authority component of this URI, or {@code null} if the
	 ...
	 * </p>
	 *
	 * @return The decoded authority component of this URI, or {@code null} if
	 *         the authority is undefined
	 */
	 ...
	 * </p>
	 *
	 * @return The raw user-information component of this URI, or {@code null}
	 ...
	 * of escaped octets are <a href="#decode">decoded</a>.
	 * </p>
	 *
	 * @return The decoded user-information component of this URI, or
	 *         {@code null} if the user information is undefined
	 */
	 ...
	 * characters ({@code '.'}), optionally followed by a period character. Each
	 * label consists of <i>alphanum</i> characters as well as hyphen characters
	 ...
	 * {@code +.}<i>digit</i>{@code +.}<i>digit</i>{@code +}, where no
	 * <i>digit</i> sequence is longer than three characters and no sequence has
	 ...
	 * and consisting of hexadecimal digits, colon characters ({@code ':'}), and
	 * possibly an embedded IPv4 address. The full syntax of IPv6 addresses is
	 * specified in
	 * <a href="http://www.ietf.org/rfc/rfc2373.txt"><i>RFC&nbsp;2373: IPv6
	 ...
	 * method does not perform any decoding.
	 *
	 * @return The host component of this URI, or {@code null} if the host is
	 *         undefined
	 */
	 ...
	 * </p>
	 *
	 * @return The port component of this URI, or {@code -1} if the port is
	 *         undefined
	 */
	public int getPort() {
		return port;
	 ...
	 * </p>
	 *
	 * @return The path component of this URI, or {@code null} if the path is
	 *         undefined
	 */
	 ...
	 * </p>
	 *
	 * @return The decoded path component of this URI, or {@code null} if the
	 *         path is undefined
	 */
	 ...
	 * </p>
	 *
	 * @return The raw query component of this URI, or {@code null} if the query
	 *         is undefined
	 */
	 ...
	 * </p>
	 *
	 * @return The decoded query component of this URI, or {@code null} if the
	 *         query is undefined
	 */
	 ...
	 * </p>
	 *
	 * @return The raw fragment component of this URI, or {@code null} if the
	 ...
	 * </p>
	 *
	 * @return The decoded fragment component of this URI, or {@code null} if
	 ...
	 * registry-based, or both be server-based. If their authorities are defined
	 * and are registry-based, then they must be equal. If their authorities are
	 ...
	 * less than the second. Unless otherwise noted, string components are
	 * ordered according to their natural, case-sensitive ordering as defined by
	 ...
	 * according to their user-information components; if these components are
	 * identical then the URIs are ordered according to the ordering of their
	 * hosts, without regard to case; if the hosts are identical then the URIs
	 ...
	 * components are ordered according to the ordering of their paths; if their
	 * paths are identical then they are ordered according to the ordering of
	 * their queries; if the queries are identical then they are ordered
	 ...
	 * computed from the originally-given components, as appropriate, is
	 * returned. Otherwise this URI was created by normalization, resolution, or
	 ...
	 * invocation of the {@link #toString() toString} method. Otherwise this
	 * method works as if by invoking that method and then
	 * <a href="#encode">encoding</a> the result.
	 * </p>
	 *
	 ...
	 * then the {@link java.io.ObjectOutputStream#defaultWriteObject()} method
	 * of the given object-output stream is invoked.
	 * </p>
	 *
	 * @param os
	...
	// are replaced with '\uFFFD'.
	// Exception: any "%" found between "[]" is left alone. It is an IPv6
	// literal
	// with a scope_id
	//
