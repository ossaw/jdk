...
public abstract class URLStreamHandler {
	 ...
	 *
	 * <p>
	 * If for the handler's protocol (such as HTTP or JAR), there exists a
	 ...
	 * For example, for HTTP an HttpURLConnection will be returned, and for JAR
	...
	/**
	 * Same as openConnection(URL), except that the connection will be made
	 * through the specified proxy; Protocol handlers that do not support
	 * proxying will ignore the proxy parameter and make a normal connection.
	 *
	 ...
	 *          the URL that this connects to.
	 * @param p
	 *          the proxy through which the connection will be made. If direct
	...
	/**
	 * Parses the string representation of a {@code URL} into a {@code URL}
	 * object.
	 * <p>
	 ...
	 * representation as if it were an {@code http} specification. Most URL
	 * protocol families have a similar parsing. A stream protocol handler for a
	 * protocol that has a different syntax must override this routine.
	 *
	 * @param u
	 *              the {@code URL} to receive the result of parsing the spec.
	 * @param spec
	 *              the {@code String} representing the URL that must be parsed.
	 * @param start
	 *              the character index at which to begin parsing. This is just
	 *              past the '{@code :}' (if there is one) that specifies the
	 *              determination of the protocol name.
	 * @param limit
	 *              the character position to stop parsing at. This is the end
	 *              of
	...
	protected void parseURL(URL u, String spec, int start, int limit) {
		...
		boolean isUNCName = (start <= limit - 4) && (spec.charAt(start) == '/')
				&& (spec.charAt(start + 1) == '/') && (spec.charAt(start
						+ 2) == '/') && (spec.charAt(start + 3) == '/');
					...
					if ((ind = host.indexOf(']')) > 2) {
						...
						if (nhost.length() > ind + 1) {
							if (nhost.charAt(ind + 1) == ':') {
								...
								if (nhost.length() > (ind + 1)) {
									port = Integer.parseInt(nhost.substring(ind
											+ 1));
								}
							} else {
		...
		if (start < limit) {
			if (spec.charAt(start) == '/') {
				...
				if (ind == -1 && authority != null)
					seperator = "/";
				path = path.substring(0, ind + 1) + seperator + spec.substring(
						start, limit);
	 ...
	 * other protocols that have different requirements for equals(). This
	 * method requires that none of its arguments is null. This is guaranteed by
	 * the fact that it is only called by java.net.URL class.
	 * 
	 * @param u1
	 *           a URL object
	 * @param u2
	 *           a URL object
	 * @return {@code true} if the two urls are considered equal, ie. they refer
	 *         to the same fragment in the same file.
	 * @since 1.3
	 */
	 ...
	 * the same protocol, host, port, and path. This method requires that none
	 * of its arguments is null. This is guaranteed by the fact that it is only
	 * called indirectly by java.net.URL class.
	 * 
	 * @param u1
	 *           a URL object
	 * @param u2
	...
	protected boolean sameFile(URL u1, URL u2) {
		...
		if (!((u1.getProtocol() == u2.getProtocol()) || (u1
				.getProtocol() != null && u1.getProtocol().equalsIgnoreCase(u2
						.getProtocol()))))
			return false;
	 ...
	 * @return {@code true} if and only if they are equal, {@code false}
	 *         otherwise.
	 * @since 1.3
	 */
	 ...
	 * classes derived from URLStreamHandler are able to use this method to set
	 * the values of the URL fields.
	 *
	 * @param u
	 *                  the URL to modify.
	 * @param protocol
	...
	protected void setURL(URL u, String protocol, String host, int port,
		...
		u.set(u.getProtocol(), host, port, authority, userInfo, path, query,
				ref);
	}
	...
	protected void setURL(URL u, String protocol, String host, int port,
		...
		/*
		 * Only old URL handlers call this, so assume that the host field might
		 * contain "user:passwd@host". Fix as necessary.
		 */
