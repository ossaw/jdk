...
public class URLClassLoader extends SecureClassLoader implements Closeable {
     ...
     * delegation parent {@code ClassLoader}. The URLs will be searched in the
     * order specified for classes and resources after first searching in the
     * parent class loader. Any URL that ends with a '/' is assumed to refer to
     * a directory. Otherwise, the URL is assumed to refer to a JAR file which
     ...
     * JarFiles or FileInputStreams). We don't care about Http resources since
     * they don't need to be closed.
     * If the resource is coming from a jar file we keep a (weak) reference to
     * the JarFile object which can be closed if URLClassLoader.close() called.
     ...
     * loader is closed, then any resources opened by this method will be
     * closed.
     * <p>
     ...
     * classes or resources that are defined by this loader. Classes and
     * resources defined by any of this loader's parents in the delegation
     * hierarchy are still accessible. Also, any classes or resources that are
     * already loaded, are still accessible.
     * <p>
     * In the case of jar: and file: URLs, it also closes any files that were
     * opened by it. If another thread is loading a class when the {@code close}
     * method is invoked, then the result of that load is undefined.
     * <p>
     * The method makes a best effort attempt to close all opened files, by
     * catching {@link IOException}s internally. Unchecked exceptions and errors
     * are not caught. Calling close on an already closed loader has no effect.
     * <p>
     * 
     * @exception IOException
     *            if closing any file opened by this class
     *            loader resulted
     *            in an IOException. Any such exceptions are
     *            caught
     *            internally. If only one is caught, then it
     *            is re-thrown.
     *            If more than one exception is caught, then
     *            the second and
     *            following exceptions are added as suppressed
     *            exceptions of
     *            the first one caught, which is then
     *            re-thrown.
     * @exception SecurityException
    ...
    /*
     * Retrieve the package using the specified package name. If non-null,
     * verify the package using the specified code source and manifest.
     */
     ...
     *            existing package either
     *            in this class loader or one of its
     *            ancestors
     ...
     * @return a {@code URL} for the resource, or {@code null} if the resource
     *         could not be found, or if the loader is closed.
     */
     ...
     * @return an {@code Enumeration} of {@code URL}s If the loader is closed,
     ...
     * permission to connect to and accept connections from that authority may
     * be granted. If the protocol is "file" and the path specifies a file, then
     * permission to read that file is granted. If protocol is "file" and the
     * path is a directory, permission is granted to read all files and
     ...
     *        the codesource
     * @exception NullPointerException
     *            if {@code codesource} is {@code null}.
     ...
     * {@code loadClass} method of the URLClassLoader returned by this method
     * will invoke the {@code SecurityManager.checkPackageAccess} method before
     * loading the class.
     *
     * @param urls
     ...
     * {@code loadClass} method of the URLClassLoader returned by this method
     * will invoke the {@code SecurityManager.checkPackageAccess} before loading
     * the class.
     *
     * @param urls
     *        the URLs to search for classes and resources
