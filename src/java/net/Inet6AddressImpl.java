...
class Inet6AddressImpl implements InetAddressImpl {
	...
	public boolean isReachable(InetAddress addr, int timeout, NetworkInterface netif, int ttl)
		...
		if (netif != null) {
			/*
			 * Let's make sure we bind to an address of the proper family. Which
			 * means same family as addr because at this point it could be
			 * either an IPv6 address or an IPv4 address (case of a dual stack
			 * system).
			 */
	...
	public synchronized InetAddress loopbackAddress() {
		if (loopbackAddress == null) {
			if (InetAddress.preferIPv6Address) {
				byte[] loopback = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
