...
/**
 * Represents implementations of URLConnection caches. An instance of such a
 * class can be registered with the system by doing
 * ResponseCache.setDefault(ResponseCache), and the system will call this object
 * in order to:
 *
 * <ul>
 * <li>store resource data which has been retrieved from an external source into
 * the cache</li>
 * <li>try to fetch a requested resource that may have been stored in the cache
 * </li>
 * </ul>
 *
 ...
 * retrieved from the cache, then the protocol handlers will fetch the resource
 ...
 * <a href="http://www.ietf.org/rfc/rfc2616.txt"><i>RFC&nbsp;2616: Hypertext
...
public abstract class ResponseCache {
    ...
    /**
     * Retrieve the cached response based on the requesting uri, request method
     * and request headers. Typically this method is called by the protocol
     * handler before it sends out the request to get the network resource. If a
    ...
    /**
     * The protocol handler calls this method after a resource has been
     * retrieved, and the ResponseCache must decide whether or not to store the
     * resource in its cache. If the resource is to be cached, then put() must
     * return a CacheRequest object which contains an OutputStream that the
     * protocol handler will use to write the resource into the cache. If the
