 ...
 * long} in an object. An object of type {@code Long} contains a single field
 * whose type is {@code long}.
 *
 * <p>
 * In addition, this class provides several methods for converting a
 * {@code long} to a {@code String} and a {@code String} to a {@code
 * long}, as well as other constants and methods useful when dealing with a
 * {@code long}.
 *
 * <p>
...
public final class Long extends Number implements Comparable<Long> {
	/**
	 * A constant holding the minimum value a {@code long} can have, -2
	 * <sup>63</sup>.
	 */
	@Native
	...
	/**
	 * A constant holding the maximum value a {@code long} can have, 2
	 * <sup>63</sup>-1.
	 */
	@Native
	...
	public static final Class<Long> TYPE = (Class<Long>) Class
			.getPrimitiveClass("long");
	 ...
	 * ASCII minus sign {@code '-'} ({@code '\u005Cu002d'}). If the first
	 ...
	 * zero character {@code '0'} ({@code '\u005Cu0030'}); otherwise, the first
	...
	public static String toUnsignedString(long i, int radix) {
		if (i >= 0)
			...
			switch (radix) {
				...
				case 10:
					/*
					 * We can get the effect of an unsigned division by 10 on a
					 * long
					 * value by first shifting right, yielding a positive value,
					 * and
					 * then dividing by 5. This allows the last digit and
					 * preceding
					 * digits to be isolated more quickly than by an initial
	...
	private static BigInteger toUnsignedBigInteger(long i) {
		if (i >= 0L)
			...
			return (BigInteger.valueOf(Integer.toUnsignedLong(upper)))
					.shiftLeft(32).add(BigInteger.valueOf(Integer
							.toUnsignedLong(lower)));
		}
	}
	 ...
	 * @param shift
	 *               the log2 of the base to format in (4 for hex, 3 for octal,
	 *               1
	 *               for binary)
	 * @param buf
	 ...
	 */
	static int formatUnsignedLong(long val, int shift, char[] buf, int offset,
			int len) {
		int charPos = len;
		int radix = 1 << shift;
		int mask = radix - 1;
	 ...
	 * arguments to the {@link #toString(long, int)} method.
	 *
	 * @param i
	 ...
	 * value.
	 *
	 * The argument is converted to unsigned decimal representation and returned
	 * as a string exactly as if the argument and radix 10 were given as
	 * arguments to the {@link #toUnsignedString(long, int)} method.
	 *
	 * @param i
	 *          an integer to be converted to an unsigned string.
	 ...
	 * ASCII plus sign {@code '+'} ({@code '\u005Cu002B'}) to indicate a
	 * positive value. The resulting {@code long} value is returned.
	 *
	 * <p>
	 ...
	 *
	 * <li>The {@code radix} is either smaller than
	 ...
	 *
	 * @param s
	 *              the {@code String} containing the {@code long}
	 *              representation
	 *              to be parsed.
	 * @param radix
	 ...
	 * the radix {@code 10} were given as arguments to the
	 ...
	 *
	 * @param s
	 *          a {@code String} containing the {@code long} representation to
	 *          be parsed
	 ...
	 * the {@link #parseUnsignedLong(java.lang.String, int)} method.
	 *
	 * @param s
	 ...
	 * the arguments were given to the {@link #parseLong(java.lang.String, int)}
	 * method. The result is a {@code Long} object that represents the
	 * {@code long} value specified by the string.
	 *
	 * <p>
	 ...
	 * decimal {@code long}, exactly as if the argument were given to the
	 * {@link #parseLong(java.lang.String)} method. The result is a {@code Long}
	...
	/**
	 * Returns a {@code Long} instance representing the specified {@code long}
	 * value. If a new {@code Long} instance is not required, this method should
	 * generally be used in preference to the constructor {@link #Long(long)},
	 * as this method is likely to yield significantly better space and time
	 * performance by caching frequently requested values.
	 *
	 * Note that unlike the {@linkplain Integer#valueOf(int) corresponding
	 * method} in the {@code Integer} class, this method is <em>not</em>
	 * required to cache values within a particular range.
	 *
	 * @param l
	 *          a long value.
	 ...
	 * Long.parseLong} method with the indicated radix (10, 16, or 8). This
	...
	/**
	 * Determines the {@code long} value of the system property with the
	 * specified name.
	 *
	 * <p>
	 ...
	 * value of this property is then interpreted as a {@code
	 * long} value using the grammar supported by {@link Long#decode decode} and
	 * a {@code Long} object representing this value is returned.
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * The second argument is the default value. A {@code Long} object that
	 ...
	 * </blockquote>
	 *
	 * to avoid the unnecessary allocation of a {@code Long} object when the
	 ...
	 *         numerically less than the argument {@code Long}; and a value
	 *         greater than {@code 0} if this {@code Long} is numerically
	...
	public static long divideUnsigned(long dividend, long divisor) {
		...
		if (dividend > 0) // Both inputs non-negative
			...
			/*
			 * For simple code, leveraging BigInteger. Longer and faster code
			 * written directly in terms of operations on longs is possible; see
			 * "Hacker's Delight" for divide and remainder algorithms.
			 */
			return toUnsignedBigInteger(dividend).divide(toUnsignedBigInteger(
					divisor)).longValue();
		}
	}
	...
	public static long remainderUnsigned(long dividend, long divisor) {
		if (dividend > 0 && divisor > 0) { // signed comparisons
			...
			if (compareUnsigned(dividend, divisor) < 0) // Avoid explicit check
														// for 0 divisor
				return dividend;
			else
	...
	public static long reverseBytes(long i) {
		i = (i & 0x00ff00ff00ff00ffL) << 8 | (i >>> 8) & 0x00ff00ff00ff00ffL;
		return (i << 48) | ((i & 0xffff0000L) << 16) | ((i >>> 16)
				& 0xffff0000L) | (i >>> 48);
	}
