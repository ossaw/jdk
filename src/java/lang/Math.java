 ...
 * implementations of the equivalent functions of class {@code Math} are not
 ...
 * required.
 * <p>
 * By default many of the {@code Math} methods simply call the equivalent method
 * in {@code StrictMath} for their implementation. Code generators are
 * encouraged to use platform-specific native libraries or microprocessor
 * instructions, where available, to provide higher-performance implementations
 * of {@code Math} methods. Such higher-performance implementations still must
 * conform to the specification for {@code Math}.
 * <p>
 ...
 * floating-point {@code Math} methods is measured in terms of <i>ulps</i>,
 * units in the last place. For a given floating-point format, an
 * {@linkplain #ulp(double) ulp} of a specific real number value is the distance
 * between the two floating-point values bracketing that numerical value. When
 * discussing the accuracy of a method as a whole rather than at a specific
 * argument, the number of ulps cited is for the worst-case error at any
 * argument. If a method always has an error less than 0.5 ulps, the method
 * always returns the floating-point number nearest the exact result; such a
 * method is <i>correctly rounded</i>. A correctly rounded method is generally
 * the best a floating-point approximation can be; however, it is impractical
 * for many floating-point methods to be correctly rounded. Instead, for the
 * {@code Math} class, a larger error bound of 1 or 2 ulps is allowed for
 * certain methods. Informally, with a 1 ulp error bound, when the exact result
 * is a representable number, the exact result should be returned as the
 * computed result; otherwise, either of the two floating-point values which
 * bracket the exact result may be returned. For exact results large in
 * magnitude, one of the endpoints of the bracket may be infinite. Besides
 * accuracy at individual arguments, maintaining proper relations between the
 * method at different arguments is also important. Therefore, most methods with
 * more than 0.5 ulp errors are required to be <i>semi-monotonic</i>: whenever
 * the mathematical function is non-decreasing, so is the floating-point
 * approximation, likewise, whenever the mathematical function is
 * non-increasing, so is the floating-point approximation. Not all
 * approximations that have 1 ulp accuracy will automatically meet the
 * monotonicity requirements.
 * <p>
 * The platform uses signed two's complement integer arithmetic with int and
 * long primitive types. The developer should choose the primitive type to
 * ensure that arithmetic operations consistently produce correct results, which
 * in some cases means the operations will not overflow the range of values of
 * the computation. The best practice is to choose the primitive type and
 * algorithm to avoid overflow. In cases where the size is {@code int} or
 * {@code long} and overflow errors need to be detected, the methods
 * {@code addExact}, {@code subtractExact}, {@code multiplyExact}, and
 * {@code toIntExact} throw an {@code ArithmeticException} when the results
 * overflow. For other arithmetic operations such as divide, absolute value,
 * increment, decrement, and negation overflow occurs only with a specific
 * minimum or maximum value and should be checked against the minimum or maximum
 * as appropriate.
 *
 * @author unascribed
...
public final class Math {
     ...
     * coordinates ({@code x},&nbsp;{@code y}) to polar coordinates (r,&nbsp;
     * <i>theta</i>). This method computes the phase <i>theta</i> by computing
     * an arc tangent of {@code y/x} in the range of -<i>pi</i> to <i>pi</i>.
     * Special cases:
     * <ul>
    ...
    public static int round(float a) {
        ...
        if ((shift & -32) == 0) { // shift >= 0 && shift < 32
            ...
            // a is either
            // - a finite number with abs(a) <
            // exp(2,FloatConsts.SIGNIFICAND_WIDTH-32) < 1/2
            // - a finite number with ulp(a) >= 1 and hence a is a mathematical
            // integer
            // - an infinity or NaN
            return (int) a;
        }
    ...
    public static long round(double a) {
        ...
        if ((shift & -64) == 0) { // shift >= 0 && shift < 64
            ...
            // a is either
            // - a finite number with abs(a) <
            // exp(2,DoubleConsts.SIGNIFICAND_WIDTH-64) < 1/2
            // - a finite number with ulp(a) >= 1 and hence a is a mathematical
            // integer
            // - an infinity or NaN
            return (long) a;
    ...
    /**
     * Returns the value of the {@code long} argument; throwing an exception if
     ...
     * quotient and the {@code /} operator returns the integer closest to zero.
     * <br>
     ...
     * <ul>
     * <li>{@code floorMod(+4, -3) == -2}; &nbsp; and {@code (+4 % -3) == +1}
     * </li>
     * <li>{@code floorMod(-4, +3) == +2}; &nbsp; and {@code (-4 % +3) == -1}
     * </li>
     * <li>{@code floorMod(-4, -3) == -1}; &nbsp; and {@code (-4 % -3) == -1 }
     * </li>
     * </ul>
     * </li>
     * </ul>
        ...
        switch (exp) {
            ...
            default:
                ...
                if (exp >= DoubleConsts.MIN_EXPONENT) {
                    ...
                    // number of positions
                    return Double.longBitsToDouble(1L << (exp - (DoubleConsts.MIN_EXPONENT
    ...
    /**
     * Returns the size of an ulp of the argument. An ulp, unit in the last
     * place, of a {@code float} value is the positive distance between this
     * floating-point value and the {@code
        ...
        switch (exp) {
            ...
            default:
                ...
                if (exp >= FloatConsts.MIN_EXPONENT) {
                    ...
                    // number of positions
                    return Float.intBitsToFloat(1 << (exp - (FloatConsts.MIN_EXPONENT
     ...
     * sine of <i>x</i> is defined to be (<i>e<sup>x</sup>&nbsp;-&nbsp;e
     * <sup>-x</sup></i>)/2 where <i>e</i> is {@linkplain Math#E Euler's number}
     * .
     * <p>
     * Special cases:
     * <ul>
     ...
     * cosine of <i>x</i> is defined to be (<i>e<sup>x</sup>&nbsp;+&nbsp;e
     * <sup>-x</sup></i>)/2 where <i>e</i> is {@linkplain Math#E Euler's number}
     * .
     * <p>
     * Special cases:
     ...
     * tangent of <i>x</i> is defined to be (<i>e<sup>x</sup>&nbsp;-&nbsp;e
     * <sup>-x</sup></i>)/(<i>e<sup>x</sup>&nbsp;+&nbsp;e<sup>-x</sup></i>), in
     * other words, {@linkplain Math#sinh sinh(<i>x</i>)}/{@linkplain Math#cosh
    ...
    public static double copySign(double magnitude, double sign) {
        return Double.longBitsToDouble((Double.doubleToRawLongBits(sign) & (DoubleConsts.SIGN_BIT_MASK))
                | (Double.doubleToRawLongBits(magnitude) & (DoubleConsts.EXP_BIT_MASK
    ...
    public static float copySign(float magnitude, float sign) {
        return Float.intBitsToFloat((Float.floatToRawIntBits(sign) & (FloatConsts.SIGN_BIT_MASK)) | (Float
                .floatToRawIntBits(magnitude) & (FloatConsts.EXP_BIT_MASK | FloatConsts.SIGNIF_BIT_MASK)));
    }
    ...
    public static int getExponent(float f) {
         ...
         * value
         */
        return ((Float.floatToRawIntBits(f) & FloatConsts.EXP_BIT_MASK) >> (FloatConsts.SIGNIFICAND_WIDTH
    ...
    public static double nextAfter(double start, double direction) {
        ...
        if (Double.isNaN(start) || Double.isNaN(direction)) {
             ...
             * treated as signed- magnitude integers . Since Java's integers are
             * two's complement, incrementing" the two's complement
             * representation of a logically negative floating-point value
             * *decrements* the signed-magnitude representation. Therefore, when
             * the integer representation of a floating-point values is less
             * than zero, the adjustment to the representation is in the
             * opposite direction than would be expected at first .
             */
     ...
     * of negative infinity. This method is semantically equivalent to
     * {@code nextAfter(f,
     * Float.NEGATIVE_INFINITY)}; however, a {@code nextDown} implementation may
     * run faster than its equivalent {@code nextAfter} call.
     * <p>
     * Special Cases:
     * <ul>
    ...
    public static double scalb(double d, int scaleFactor) {
        /*
         * This method does not need to be declared strictfp to compute the same
         * correct result on all platforms. When scaling up, it does not matter
         * what order the multiply-store operations are done; the result will be
         ...
         * next multiply will round it away to zero. This is done by first
         * multiplying by 2 ^ (scaleFactor % n) and then multiplying several
         * times by by 2^n as needed where n is the exponent of number that is a
         * covenient power of two. In this way, at most one real rounding error
         * occurs. If the double value set is being used exclusively, the
    ...
    public static float scalb(float f, int scaleFactor) {
        ...
        /*
         * Since + MAX_SCALE for float fits well within the double exponent
         * range and + float -> double conversion is exact the multiplication
         * below will be exact. Therefore, the rounding that occurs when the
         * double product is cast to float will be the correctly rounded float
         * result. Since all operations other than the final multiply will be
         * exact, it is not necessary to declare this method strictfp.
         */
