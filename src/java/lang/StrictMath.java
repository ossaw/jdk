...
/**
 * The class {@code StrictMath} contains methods for performing basic numeric
 ...
 * well-known network library {@code netlib} as the package "Freely
 * Distributable Math Library,"
 * <a href="ftp://ftp.netlib.org/fdlibm.tar">{@code fdlibm}</a>. These
 * algorithms, which are written in the C programming language, are then to be
 * understood as executed with all floating-point operations following the rules
 * of Java floating-point arithmetic.
 * <p>
 * The Java math library is defined with respect to {@code fdlibm} version 5.3.
 * Where {@code fdlibm} provides more than one definition for a function (such
 * as {@code acos}), use the "IEEE 754 core function" version (residing in a
 * file whose name begins with the letter {@code e}). The methods which require
 * {@code fdlibm} semantics are {@code sin}, {@code cos}, {@code tan},
 * {@code asin}, {@code acos}, {@code atan}, {@code exp}, {@code log},
 * {@code log10}, {@code cbrt}, {@code atan2}, {@code pow}, {@code sinh},
 * {@code cosh}, {@code tanh}, {@code hypot}, {@code expm1}, and {@code log1p}.
 * <p>
...
public final class StrictMath {
     ...
     * the cube root of that value's magnitude. Special cases:
     * <ul>
     * <li>If the argument is NaN, then the result is NaN.
     ...
     * @param increment
     *        value to add when the argument is non-integral
     */
    private static double floorOrCeil(double a, double negativeBoundary, double positiveBoundary,
            double sign) {
        int exponent = Math.getExponent(a);

        if (exponent < 0) {
            /*
             * Absolute value of argument is less than 1. floorOrceil(-0.0) =>
             * -0.0 floorOrceil(+0.0) => +0.0
             */
     ...
     * the result is the integer value that is even. Special cases:
     * <ul>
     * <li>If the argument value is already equal to a mathematical integer,
     * then the result is the same as the argument.
     * <li>If the argument is NaN or an infinity or positive zero or negative
    ...
    public static double rint(double a) {
        /*
         * If the absolute value of a is not less than 2^52, it is either a
         * finite integer (the double format does not have enough significand
         * bits for a number that large to have any fractional portion), an
         * infinity, or a NaN. In any of these cases, rint of the argument is
         * the argument.
         * Otherwise, the sum (twoToThe52 + a ) will properly round away any
         * fractional portion of a since ulp(twoToThe52) == 1.0; subtracting out
         * twoToThe52 from this sum will then be exact and leave the rounded
         * integer portion of a.
         * This method does *not* need to be declared strictfp to get fully
         * reproducible results. Whether or not a method is declared strictfp
         * can only make a difference in the returned result if some operation
         * would overflow or underflow with strictfp semantics. The operation
         * (twoToThe52 + a ) cannot overflow since large values of a are
         * screened out; the add cannot underflow since twoToThe52 is too large.
         * The subtraction ((twoToThe52 + a ) - twoToThe52) will be exact as
         * discussed above and thus cannot overflow or meaningfully underflow.
         * Finally, the last multiply in the return statement is by plus or
         * minus 1.0, which is exact too.
         */
     ...
     * <p>
     * See {@link Math#floorDiv(int, int) Math.floorDiv} for examples and a
     * comparison to the integer division {@code /} operator.
     *
     * @param x
     *        the dividend
     * @param y
     ...
     * </ul>
     * <p>
     * See {@link Math#floorMod(int, int) Math.floorMod} for examples and a
     * comparison to the {@code %} operator.
     *
     * @param x
     *        the dividend
     * @param y
     ...
     * floating-point argument. For this method, a NaN {@code sign} argument is
     * always treated as if it were positive.
     *
     * @param magnitude
