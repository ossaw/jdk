...
public class MethodHandleProxies {
     ...
     * <em>Implementation Note:</em> Therefore, each instance must implement a
     * unique single-method interface. Implementations may not bundle together
     * multiple single-method interfaces onto single implementation classes in
     * the style of {@link java.awt.AWTEventMulticaster}.
     * <p>
    ...
    public static <T> T asInterfaceInstance(final Class<T> intfc, final MethodHandle target) {
        ...
        if (proxyLoader == null) {
            ClassLoader cl = Thread.currentThread().getContextClassLoader(); // avoid
                                                                             // use
                                                                             // of
                                                                             // BCP
        ...
        final Object proxy;
        if (System.getSecurityManager() != null) {
            // sun.invoke.WrapperInstance is a restricted interface not
            // accessible
            // by any non-null class loader.
            final ClassLoader loader = proxyLoader;
            proxy = AccessController.doPrivileged(new PrivilegedAction<Object>() {
                public Object run() {
                    return Proxy.newProxyInstance(loader, new Class<?>[] { intfc, WrapperInstance.class },
                            ih);
                }
            });
        } else {
     ...
     * requirement may be tested via {@link #isWrapperInstance
     * isWrapperInstance}.
     * 
     * @param x
     *        any reference
