...
/**
 * Shared information for a group of method types, which differ only by
 * reference types, and therefore share a common erasure and wrapping.
 * <p>
 * For an empirical discussion of the structure of method types, see <a href=
 * "http://groups.google.com/group/jvm-languages/browse_thread/thread/ac9308ae74da9b7e/">
 * the thread "Avoiding Boxing" on jvm-languages</a>. There are approximately
 * 2000 distinct erased method types in the JDK. There are a little over 10
 * times that number of unerased types. No more than half of these are likely to
 * be loaded at once.
 * 
 * @author John Rose
 */
final class MethodTypeForm {
    ...
    final MethodType basicType; // the canonical erasure, with primitives
                                // simplified
    ...
    static final int MH_BASIC_INV = 0, // cached instance of MH.invokeBasic
    ...
    /**
     * Return the type corresponding uniquely (1-1) to this MT-form. It might
     * have any primitive returns or arguments, but will have no references
     * except Object.
     */
    ...
    /**
     * Return the basic type derived from the erased type of this MT-form. A
     * basic type is erased (all references Object) and also has all primitive
     * types (except int, long, float, double, void) normalized to int. Such
     * basic types correspond to low-level JVM calling sequences.
     */
    ...
    /**
     * Build an MTF for a given type, which must have all references erased to
     * Object. This MTF will stand for that type and all un-erased variations.
     * Eagerly compute some basic properties of the type, common to all
     * variations.
     */
    ...
    private static char unpack(long packed, int word) { // word==0 => return a,
                                                        // ==3 => return d
        assert (word <= 3);
    ...
    public int argSlotToParameter(int argSlot) {
        ...
        // Valid arguments slots contain incremented entries, so as to be
        // non-zero.
    ...
    /**
     * Codes for {@link #canonicalize(java.lang.Class, int)}. ERASE means change
     * every reference to {@code Object}. WRAP means convert primitives
     * (including {@code void} to their corresponding wrapper types. UNWRAP
     * means the reverse of WRAP. INTS means convert all non-void primitive
     * types to int or long, according to size. LONGS means convert all non-void
     * primitives to long, regardless of size. RAW_RETURN means convert a type
    ...
    /**
     * Canonicalize the given return or param type. Return null if the type is
    ...
    /**
     * Canonicalize each param type in the given array. Return null if all types
