...
class LambdaFormEditor {
	...
	private static final class Transform extends SoftReference<LambdaForm> {
		...
		private static long packedBytes(int b0, int b1, int b2, int b3) {
			...
			return ((b0 << 0 * PACKED_BYTE_SIZE) | (b1 << 1 * PACKED_BYTE_SIZE)
					| (b2 << 2 * PACKED_BYTE_SIZE) | (b3 << 3 * PACKED_BYTE_SIZE));
		}
		...
		public boolean equals(Transform that) {
			return this.packedBytes == that.packedBytes
					&& Arrays.equals(this.fullBytes, that.fullBytes);
		}
	...
	LambdaForm bindArgumentForm(int pos) {
		...
		if (pos != 0) {
			...
			// class.
			buf.replaceFunctions(oldData.getterFunctions(), newData.getterFunctions(),
					oldBaseAddress);
	...
	LambdaForm spreadArgumentsForm(int pos, Class<?> arrayType, int arrayLength) {
		...
		Name checkSpread = new Name(MethodHandleImpl.Lazy.NF_checkSpreadArgument, spreadParam,
				arrayLength);
	...
	LambdaForm collectArgumentArrayForm(int pos, MethodHandle arrayCollector) {
		...
		assert (collectorType.parameterList()
				.equals(Collections.nCopies(collectorArity, elementType)));
	...
	private LambdaForm makeArgumentCombinationForm(int pos, MethodType combinerType,
	...
	LambdaForm filterReturnForm(BasicType newType, boolean constantZero) {
		Transform.Kind kind = (constantZero ? Transform.Kind.FILTER_RETURN_TO_ZERO
