...
abstract class DelegatingMethodHandle extends MethodHandle {
	...
	private static LambdaForm chooseDelegatingForm(MethodHandle target) {
		...
		return makeReinvokerForm(target, MethodTypeForm.LF_DELEGATE, DelegatingMethodHandle.class,
				NF_getTarget);
	}

	static LambdaForm makeReinvokerForm(MethodHandle target, int whichCache, Object constraint,
		...
		return makeReinvokerForm(target, whichCache, constraint, debugString, true, getTargetFn,
				null);
	}

	/** Create a LF which simply reinvokes a target of the given basic type. */
	static LambdaForm makeReinvokerForm(MethodHandle target, int whichCache, Object constraint,
			String debugString, boolean forceInline, NamedFunction getTargetFn,
	...
	static {
		try {
			NF_getTarget = new NamedFunction(
					DelegatingMethodHandle.class.getDeclaredMethod("getTarget"));
