...
/**
 * Working storage for an LF that is being transformed. Similarly to a
 * StringBuffer, the editing can take place in multiple steps.
 */
final class LambdaFormBuffer {
    ...
    void setNames(Name[] names2) {
        names = originalNames = names2; // keep a record of where everything was
                                        // to start with
    ...
    /**
     * We have just overwritten the name at pos1 with the name at pos2. This
     * means that there are two copies of the name, which we will have to fix
     * later.
     */
    ...
    /**
     * Create a private, writable copy of names. Preserve the original copy, for
     * reference.
     */
    void startEdit() {
        assert (verifyArity());
    ...
    LambdaForm endEdit() {
        assert (verifyFirstChange());
        // Assuming names have been changed pairwise from originalNames[i] to
        // names[i],
    ...
    /**
     * Replace any Name whose function is in oldFns with a copy whose function
    ...
    /**
     * Replace a parameter by another parameter or expression already in the
     * form.
     */
    LambdaFormBuffer replaceParameterByCopy(int pos, int valuePos) {
        assert (pos != valuePos);
        replaceName(pos, names[valuePos]);
        noteDuplicate(pos, valuePos); // temporarily, will occur twice in the
                                      // names array
        return this;
    }
