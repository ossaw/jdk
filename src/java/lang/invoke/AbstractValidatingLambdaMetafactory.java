...
/* package */ abstract class AbstractValidatingLambdaMetafactory {
     ...
     * with their values, given this program: interface II<T> { Object foo(T x);
     * } interface JJ<R extends Number> extends II<R> { } class CC { String
     * impl(int i) { return "impl:"+i; }} class X { public static void
     * main(String[] args) { JJ<Integer> iii = (new CC())::impl;
     * System.out.printf(">>> %s\n", iii.foo(44)); }}
     */
    ...
    final MethodHandle implMethod; // Raw method handle for the implementation
                                   // method
    final MethodHandleInfo implInfo; // Info about the implementation method
                                     // handle "MethodHandleInfo[5
                                     // CC.impl(int)String]"
    final int implKind; // Invocation kind for implementation "5"=invokevirtual
    final boolean implIsInstanceMethod; // Is the implementation an instance
                                        // method "true"
    final Class<?> implDefiningClass; // Type defining the implementation "class
                                      // CC"
    final MethodType implMethodType; // Type of the implementation method
                                     // "(int)String"
    final MethodType instantiatedMethodType; // Instantiated erased functional
                                             // interface method type
                                             // "(Integer)Object"
    final boolean isSerializable; // Should the returned instance be
                                  // serializable
    final Class<?>[] markerInterfaces; // Additional marker interfaces to be
                                       // implemented
    final MethodType[] additionalBridges; // Signatures of additional methods to
                                          // bridge
    ...
    AbstractValidatingLambdaMetafactory(MethodHandles.Lookup caller, MethodType invokedType,
        ...
        if ((caller.lookupModes() & MethodHandles.Lookup.PRIVATE) == 0) {
            throw new LambdaConversionException(String.format("Invalid caller: %s", caller.lookupClass()
                    .getName()));
        }
        ...
        for (Class<?> c : markerInterfaces) {
            if (!c.isInterface()) {
                throw new LambdaConversionException(String.format("Marker interface %s is not an interface", c
                        .getName()));
            }
        }
    }
    ...
    void validateMetafactoryArgs() throws LambdaConversionException {
        switch (implKind) {
            ...
            case MethodHandleInfo.REF_invokeSpecial:
                break;
            default:
                throw new LambdaConversionException(String.format("Unsupported MethodHandle kind: %s",
                        implInfo));
        }
        ...
        if (implIsInstanceMethod) {
            ...
            if (implReceiverClass != implDefiningClass && !implReceiverClass.isAssignableFrom(
                    receiverClass)) {
                throw new LambdaConversionException(String.format(
        ...
        if (!isAdaptableToAsReturn(actualReturnType, expectedType)) {
            throw new LambdaConversionException(String.format(
                    "Type mismatch for lambda return: %s is not convertible to %s", actualReturnType,
                    expectedType));
        }
        if (!isAdaptableToAsReturnStrict(expectedType, samReturnType)) {
            throw new LambdaConversionException(String.format(
                    "Type mismatch for lambda expected return: %s is not convertible to %s", expectedType,
                    samReturnType));
        }
        for (MethodType bridgeMT : additionalBridges) {
    ...
    private boolean isAdaptableTo(Class<?> fromType, Class<?> toType, boolean strict) {
        ...
        if (fromType.isPrimitive()) {
            ...
            if (toType.isPrimitive()) {
                ...
                // both are reference types: fromType should be a superclass of
                // toType.
    ...
    /**
     * Check type adaptability for return types -- special handling of void
     * type) and parameterized fromType
     * 
     * @return True if 'fromType' can be converted to 'toType'
     */
    private boolean isAdaptableToAsReturn(Class<?> fromType, Class<?> toType) {
        return toType.equals(void.class) || !fromType.equals(void.class) && isAdaptableTo(fromType, toType,
                false);
    }
    ...
    /***********
     * Logging support -- for debugging only, uncomment as needed static final
     * Executor logPool = Executors.newSingleThreadExecutor(); protected static
     ...
     *           MethodHandleProxyLambdaMetafactory.logPool.execute(new
     *           Runnable() {
     * @Override public void run() { System.out.println(s);
