...
/**
 * A {@code MemberName} is a compact symbolic datum which fully characterizes a
 * method or field reference. A member name refers to a field, method,
 * constructor, or member type. Every member name has a simple name (a string)
 * and a type (either a Class or MethodType). A member name may also have a
 * non-null declaring class, or it may be simply a naked name/type pair. A
 * member name may also have non-zero modifier flags. Finally, a member name may
 * be either resolved or unresolved. If it is resolved, the existence of the
 * named
 * <p>
 * Whether resolved or not, a member name provides no access rights or
 * invocation capability to its possessor. It is merely a compact representation
 * of all symbolic information necessary to link to and properly use the named
 * member.
 * <p>
 * When resolved, a member name's internal implementation may include references
 * to JVM metadata. This representation is stateless and only decriptive. It
 * provides no private information and no capability to use the member.
 * <p>
 * By contrast, a {@linkplain java.lang.reflect.Method} contains fuller
 * information about the internals of a method (except its bytecodes) and also
 * allows invocation. A MemberName is much lighter than a Method, since it
 * contains about 7 fields to the 16 of Method (plus its sub-arrays), and those
 * seven fields omit much of the information in Method.
 * 
 * @author jrose
 */
/* non-public */ final class MemberName implements Member, Cloneable {
    ...
    /**
     * Return the declaring class of this member. In the case of a bare name and
     * type, the declaring class will be null.
     */
    ...
    /**
     * Return the simple name of this member. For a type, it is the same as
     * {@link Class#getSimpleName}. For a method or field, it is the simple name
     * of the member. For a constructor, it is always {@code "<init>"}.
     */
    ...
    /**
     * Return the declared type of this member, which must be a method or
     * constructor.
     */
    public MethodType getMethodType() {
        ...
        {
            // Get a snapshot of type which doesn't get changed by racing
            // threads.
    ...
    /**
     * Return the actual type under which this method or constructor must be
     * invoked. For non-static methods or constructors, this is the type with a
     * leading parameter, a reference to declaring class. For static methods, it
    ...
    /**
     * Utility method to produce either the method type or field type of this
     * member.
     */
    ...
    /**
     * Utility method to produce the signature of this member, used within the
     * class file format to describe its type.
     */
        ...
        if (isField()) {
            ...
            if (clazz.isInterface())
                assert (refKind == REF_invokeInterface || refKind == REF_invokeStatic
                        || refKind == REF_invokeSpecial || refKind == REF_invokeVirtual
    ...
    /**
     * Utility method to query if this member is a method handle invocation
     * (invoke or invokeExact).
     */
    ...
    /**
     * Utility method to query whether this member or its defining class is
     * final.
     */
    ...
    // let the rest (native, volatile, transient, etc.) be tested via
    // Modifier.isFoo
    ...
    /**
     * Utility method to query the modifier flags of this member; returns false
    ...
    /**
     * Utility method to query whether this member is a method, constructor, or
     * field.
     */
    ...
    /**
     * Utility method to query whether this member is neither public, private,
     * nor protected.
     */
    ...
    /**
     * Utility method to query whether this member is accessible from a given
     * lookup class.
     */
    ...
    /**
     * Calls down to the VM to fill in the fields. This method is synchronized
     * to avoid racing calls.
     */
    ...
    /**
     * Create a name for the given reflected method. The resulting name will be
     * in a resolved state.
     */
     ...
     * kind. In that case it must already be REF_invokeSpecial.
     */
     ...
     * REF_invokeVirtual; also switch either to REF_invokeInterface if
     * clazz.isInterface. The end result is to get a fully virtualized version
     * of the MN. (Note that resolving in the JVM will sometimes devirtualize,
     * changing REF_invokeVirtual of a final to REF_invokeSpecial, and
     * REF_invokeInterface in some corner cases to either of the previous two;
    ...
    /**
     * Create a name for the given reflected field. The resulting name will be
     * in a resolved state.
     */
    ...
    /**
     * Create a name for the given class. The resulting name will be in a
     * resolved state.
     */
    ...
    /**
     * Create a name for a signature-polymorphic invoker. This is a placeholder
     * for a signature-polymorphic instance (of MH.invokeExact, etc.) that the
     * JVM does not reify. See comments on
     * {@link MethodHandleNatives#linkMethod}.
     */
    ...
    /**
     * Get the definition of this member name. This may be in a super-class of
    ...
    /**
     * Decide if two member names have exactly the same symbolic content. Does
     * not take into account any actual class members, so even if two member
     * names resolve to the same actual member, they may be distinct references.
     */
    public boolean equals(MemberName that) {
        ...
        if (that == null)
            return false;
        return this.clazz == that.clazz && this.getReferenceKind() == that.getReferenceKind() && Objects
                .equals(this.name, that.name) && Objects.equals(this.getType(), that.getType());
    }

    // Construction from symbolic parts, for queries:
    /**
     * Create a field or type name from the given components: Declaring class,
     * name, type, reference kind. The declaring class may be supplied as null
    ...
    /**
     * Create a method or constructor name from the given components: Declaring
     * class, name, type, reference kind. It will be a constructor if and only
     * if the name is {@code "<init>"}. The declaring class may be supplied as
     * null if this is to be a bare name and type. The last argument is
    ...
    /**
     * Query whether this member name is resolved. A resolved member name is one
     * for which the JVM has found a method, constructor, field, or type binding
     * corresponding exactly to the name. (Document?)
     */
    ...
    /**
     * Produce a string form of this member name. For types, it is simply the
     * type's own string (as reported by {@code toString}). For fields, it is
     * {@code "DeclaringClass.name/type"}. For methods and constructors, it is
     * {@code "DeclaringClass.name(ptype...)rtype"}. If the declaring class is
     * null, the prefix {@code "DeclaringClass."} is omitted. If the member is
     * unresolved, a prefix {@code "*."} is prepended.
     */
    ...
    /**
     * A factory type for resolving member names with the help of the VM. TBD:
    ...
    /* non-public */ static class Factory {
        ...
        List<MemberName> getMembers(Class<?> defc, String matchName, Object matchType, int matchFlags,
            ...
            int bufCount = 0;
            for (;;) {
                bufCount = MethodHandleNatives.getMembers(defc, matchName, matchSig, matchFlags, lookupClass,
                        totalCount, buf);
        ...
        /**
         * Produce a resolved version of the given member. Super types are
         * searched (for inherited members) if {@code searchSupers} is true.
         * Access checking is performed on behalf of the given
         * {@code lookupClass}. If lookup fails or access is not permitted, null
        ...
        private MemberName resolve(byte refKind, MemberName ref, Class<?> lookupClass) {
            ...
            try {
                ...
                // * LC: lookupClass
                // * REFC: symbolic reference class (MN.clazz before
                // resolution);
                // * DEFC: resolved method holder (MN.clazz after resolution);
                // * PTYPES: parameter types (MN.type)
                //
                // What we care about when resolving a MemberName is consistency
                // between DEFC and PTYPES.
                // We do type alias (TA) checks on DEFC to ensure that. DEFC is
                // not known until the JVM
                // finishes the resolution, so do TA checks right after
                // MHN.resolve() is over.
                //
                // All parameters passed by a caller are checked against MH type
                // (PTYPES) on every invocation,
                // so it is safe to call a MH from any context.
                //
                // REFC view on PTYPES doesn't matter, since it is used only as
                // a starting point for resolution and doesn't
         ...
         * searched (for inherited members) if {@code searchSupers} is true.
         * Access checking is performed on behalf of the given
         * {@code lookupClass}. If lookup fails or access is not permitted, a
         * {@linkplain ReflectiveOperationException} is thrown. Otherwise a
         * fresh copy of the given member is returned, with modifier bits filled
         * in.
         */
        public <NoSuchMemberException extends ReflectiveOperationException> MemberName resolveOrFail(
                byte refKind, MemberName m, Class<?> lookupClass, Class<NoSuchMemberException> nsmClass)
         ...
         * searched (for inherited members) if {@code searchSupers} is true.
         * Access checking is performed on behalf of the given
         * {@code lookupClass}. If lookup fails or access is not permitted,
        ...
        /**
         * Return a list of all methods defined by the given class. Super types
         * are searched (for inherited members) if {@code searchSupers} is true.
         * Access checking is performed on behalf of the given
        ...
        /**
         * Return a list of matching methods defined by the given class. Super
         * types are searched (for inherited members) if {@code searchSupers} is
         * true. Returned methods will match the name (if not null) and the type
        ...
        /**
         * Return a list of all constructors defined by the given class. Access
        ...
        /**
         * Return a list of all fields defined by the given class. Super types
         * are searched (for inherited members) if {@code searchSupers} is true.
         * Access checking is performed on behalf of the given
         * {@code lookupClass}. Inaccessible members are not added to the last.
         */
         ...
         * Return a list of all fields defined by the given class. Super types
         * are searched (for inherited members) if {@code searchSupers} is true.
         * Returned fields will match the name (if not null) and the type (if
        ...
        /**
         * Return a list of all nested types defined by the given class. Super
         * types are searched (for inherited members) if {@code searchSupers} is
    ...
    // System.out.println(Factory.INSTANCE.getMethods(MemberName.class, true,
    // null));
    // }
}
