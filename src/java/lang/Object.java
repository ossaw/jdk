...
public class Object {
     ...
     * any object {@code x}, the expression: <blockquote>
     * 
     * <pre>
     * x.clone() != x
     * </pre>
     ...
     * the interface {@code Cloneable}, then a
     ...
     * relinquishes the lock on this object. The awakened threads will compete
     * in the usual manner with any other threads that might be actively
     * competing to synchronize on this object; for example, the awakened
     * threads enjoy no reliable privilege or disadvantage in being the next
     * thread to lock this object.
     * <p>
     * This method should only be called by a thread that is the owner of this
     * object's monitor. See the {@code notify} method for a description of the
     ...
     * <p>
     * This method causes the current thread (call it <var>T</var>) to place
     * itself in the wait set for this object and then to relinquish any and all
     * synchronization claims on this object. Thread <var>T</var> becomes
     * disabled for thread scheduling purposes and lies dormant until one of
     * four things happens:
     * <ul>
     * <li>Some other thread invokes the {@code notify} method for this object
     * and thread <var>T</var> happens to be arbitrarily chosen as the thread to
     * be awakened.
     * <li>Some other thread invokes the {@code notifyAll} method for this
     * object.
     * <li>Some other thread {@linkplain Thread#interrupt() interrupts} thread
     * <var>T</var>.
     * <li>The specified amount of real time has elapsed, more or less. If
     * {@code timeout} is zero, however, then real time is not taken into
     * consideration and the thread simply waits until notified.
     * </ul>
     * The thread <var>T</var> is then removed from the wait set for this object
     ...
     * on the object are restored to the status quo ante - that is, to the
     * situation as of the time that the {@code wait} method was invoked. Thread
     * <var>T</var> then returns from the invocation of the {@code wait} method.
     * Thus, on return from the {@code wait} method, the synchronization state
     * of the object and of thread {@code T} is exactly as it was when the
     * {@code wait} method was invoked.
     * <p>
     * A thread can also wake up without being notified, interrupted, or timing
     * out, a so-called <i>spurious wakeup</i>. While this will rarely occur in
     ...
     * "Concurrent Programming in Java (Second Edition)" (Addison-Wesley, 2000),
     * or Item 50 in Joshua Bloch's "Effective Java Programming Language Guide"
     * (Addison-Wesley, 2001).
     * <p>
     * If the current thread is {@linkplain java.lang.Thread#interrupt()
     * interrupted} by any thread before or while it is waiting, then an
     * {@code InterruptedException} is thrown. This exception is not thrown
     * until the lock status of this object has been restored as described
     * above.
     * <p>
     * Note that the {@code wait} method, as it places the current thread into
     * the wait set for this object, unlocks only this object; any other objects
     * on which the current thread may be synchronized remain locked while the
     * thread waits.
     * <p>
     * This method should only be called by a thread that is the owner of this
     * object's monitor. See the {@code notify} method for a description of the
     ...
     * {@link java.lang.Object#notify()} method or the
     * {@link java.lang.Object#notifyAll()} method for this object, or some
     ...
     * before giving up. The amount of real time, measured in nanoseconds, is
     * given by: <blockquote>
     * 
     * <pre>
     ...
     * {@link #wait(long)} of one argument. In particular, {@code wait(0, 0)}
     * means the same thing as {@code wait(0)}.
     * <p>
     * The current thread must own this object's monitor. The thread releases
     * ownership of this monitor and waits until either of the following two
     * conditions has occurred:
     * <ul>
     * <li>Another thread notifies threads waiting on this object's monitor to
     ...
     * Causes the current thread to wait until another thread invokes the
     * {@link java.lang.Object#notify()} method or the
     * {@link java.lang.Object#notifyAll()} method for this object. In other
     ...
     * overrides the {@code finalize} method to dispose of system resources or
     * to perform other cleanup.
     * <p>
     * The general contract of {@code finalize} is that it is invoked if and
     * when the Java&trade; virtual machine has determined that there is no
     * longer any means by which this object can be accessed by any thread that
     * has not yet died, except as a result of an action taken by the
     * finalization of some other object or class which is ready to be
     * finalized. The {@code finalize} method may take any action, including
     * making this object available again to other threads; the usual purpose of
     * {@code finalize}, however, is to perform cleanup actions before the
     * object is irrevocably discarded. For example, the finalize method for an
     * object that represents an input/output connection might perform explicit
     ...
     * action; it simply returns normally. Subclasses of {@code Object} may
     * override this definition.
     * <p>
     * The Java programming language does not guarantee which thread will invoke
     * the {@code finalize} method for any given object. It is guaranteed,
     ...
     * <p>
     * Any exception thrown by the {@code finalize} method causes the
