 ...
 * applications that can instantiate their own class loaders could then load
 * their own rogue classes into the system. These newly loaded classes could be
 ...
 * class)</td>
 * <td>This would grant an attacker permission to get the class loader for a
 ...
 * <td>The context class loader is used by system code and extensions when they
 * need to lookup resources that might not exist in the system class loader.
 * Granting setContextClassLoader permission would allow code to change which
 ...
 * <td>The context class loader is used by system code and extensions when they
 * need to lookup resources that might not exist in the system class loader.
 * Granting enableContextClassLoaderOverride permission would allow a subclass
 ...
 * <td>The security manager is a class that allows applications to implement a
 * security policy. Granting the setSecurityManager permission would allow code
 * to change which security manager is used by installing a different, possibly
 ...
 * <td>This would allow code to read the value, or determine the existence, of a
 ...
 * stream handler factory used by URL</td>
 * <td>This allows code to set the actual implementation for the socket, server
 ...
 * <td>This allows changing the value of the standard system streams. An
 * attacker may change System.in to monitor and steal user input, or may set
 ...
 * <tt>stop</tt>, <tt>suspend</tt>, <tt>resume</tt>, <tt>setDaemon</tt>,
 * <tt>setPriority</tt>, <tt>setName</tt> and
 * <tt>setUncaughtExceptionHandler</tt> methods</td>
 ...
 * <code>setDaemon</code>, <code>setMaxPriority</code>, <code>stop</code>, and
 ...
 * <td>This allows code to obtain policy information for a particular code
 * source. While obtaining policy information does not compromise the security
 ...
 * <td>This allows code to obtain file system information such as disk usage or
 * disk space available to the caller. This is potentially dangerous because it
 ...
 * <td>This would allow code to read the particular file associated with the
 ...
 * <td>This allows code to write to a particular file associated with the
 ...
 * SecurityManager <code>checkPackageAccess</code> method</td>
 * <td>This gives code access to classes in packages to which it normally does
 ...
 * <code>defineClass</code> method when that class loader calls the
 * SecurityManager <code>checkPackageDefinition</code> method.</td>
 * <td>This grants code permission to define a class in a particular package.
 * This is dangerous because malicious code with this permission may define
 ...
 * <td>This grants code permission to query a class for its public, protected,
 * default (package) access, and private fields and/or methods. Although the
 * code would have access to the private and protected field and method names,
 * it would not have access to the private/protected field data and would not be
 * able to invoke any private methods. Nevertheless, malicious code may use this
 * information to better aim an attack. Additionally, it may invoke any public
 * methods and/or access public fields in the class. This could be dangerous if
 * the code would normally not be able to invoke those methods and/or access the
 * fields because it can't cast the object to the class/interface with those
 * methods and fields.</td>
 * </tr>
 * <tr>
 * <td>queuePrintJob</td>
 ...
 * <td>This allows retrieval of the stack trace information of another thread.
 ...
 * due to an uncaught exception</td>
 ...
 * allows retrieval or update operations within the Preferences persistent
 * backing store.)</td>
 * <td>This permission allows the user to read from or write to the preferences
 * backing store if the user running the code has sufficient OS privileges to
 * read/write to that backing store. The actual backing store may reside within
 ...
 * prompting behavior.</td>
 * <td>For more information, refer to Java Plug-In's guides,
 * <a href= "../../../technotes/guides/plugin/developer_guide/security.html">
 * Applet Security Basics</a> and
 * <a href= "../../../technotes/guides/plugin/developer_guide/rsa_how.html#use">
