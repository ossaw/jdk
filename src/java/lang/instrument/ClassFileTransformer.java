 ...
 * class files. The transformation occurs before the class is defined by the
 * JVM.
 * <P>
...
public interface ClassFileTransformer {
	 ...
	 * <code>canRetransform</code> as true</li>
	 ...
	 * {@link java.lang.instrument.Instrumentation#addTransformer(ClassFileTransformer,boolean)
	 * addTransformer}, the transformer will be called for every new class
	 * definition and every class redefinition. Retransformation capable
	 * transformers will also be called on every class retransformation. The
	 * request for a new class definition is made with
	 * {@link java.lang.ClassLoader#defineClass ClassLoader.defineClass} or its
	 * native equivalents. The request for a class redefinition is made with
	 * {@link java.lang.instrument.Instrumentation#redefineClasses
	 * Instrumentation.redefineClasses} or its native equivalents. The request
	 * for a class retransformation is made with
	 * {@link java.lang.instrument.Instrumentation#retransformClasses
	 * Instrumentation.retransformClasses} or its native equivalents. The
	 * transformer is called during the processing of the request, before the
	 * class file bytes have been verified or applied. When there are multiple
	 * transformers, transformations are composed by chaining the
	 ...
	 * In all other cases, this method is called. Within each of these
	 ...
	 * transformer is:
	 * <ul>
	 * <li>for new class definition, the bytes passed to
	 * <code>ClassLoader.defineClass</code></li>
	 * <li>for class redefinition,
	 * <code>definitions.getDefinitionClassFile()</code> where
	 * <code>definitions</code> is the parameter to
	 * {@link java.lang.instrument.Instrumentation#redefineClasses
	 * Instrumentation.redefineClasses}</li>
	 * <li>for class retransformation, the bytes passed to the new class
	 * definition or, if redefined, the last redefinition, with all
	 * transformations made by retransformation incapable transformers reapplied
	 * automatically and unaltered; for details see
	 * {@link java.lang.instrument.Instrumentation#retransformClasses
	 ...
	 * retransform will still be attempted. Thus, throwing an exception has the
	 * same effect as returning <code>null</code>. To prevent unexpected
	 * behavior when unchecked exceptions are generated in transformer code, a
	 * transformer can catch <code>Throwable</code>. If the transformer believes
	 * the <code>classFileBuffer</code> does not represent a validly formatted
	 * class file, it should throw an <code>IllegalClassFormatException</code>;
