 ...
 * code. Instrumentation is the addition of byte-codes to methods for the
 * purpose of gathering data to be utilized by tools. Since the changes are
 ...
 * an <code>Instrumentation</code> instance is passed to the
 * <code>premain</code> method of the agent class.
 * </p>
 * </li>
 * <li>
 * <p>
 * When a JVM provides a mechanism to start agents sometime after the JVM is
 * launched. In that case an <code>Instrumentation</code> instance is passed to
 * the <code>agentmain</code> method of the agent code.
 * </p>
 * </li>
 * </ol>
 * <p>
 * These mechanisms are described in the {@linkplain java.lang.instrument
 * package specification}.
 * <p>
 * Once an agent acquires an <code>Instrumentation</code> instance, the agent
 * may call methods on the instance at any time.
 *
 * @since 1.5
 */
public interface Instrumentation {
	/**
	 * Registers the supplied transformer. All future class definitions will be
	 * seen by the transformer, except definitions of classes upon which any
	 * registered transformer is dependent. The transformer is called when
	 * classes are loaded, when they are {@linkplain #redefineClasses redefined}
	 * . and if <code>canRetransform</code> is true, when they are
	 * {@linkplain #retransformClasses retransformed}. See
	 * {@link java.lang.instrument.ClassFileTransformer#transform
	 * ClassFileTransformer.transform} for the order of transform calls. If a
	 * transformer throws an exception during execution, the JVM will still call
	 * the other registered transformers in order. The same transformer may be
	 * added more than once, but it is strongly discouraged -- avoid this by
	 * creating a new instance of transformer class.
	 * <P>
	...
	/**
	 * Unregisters the supplied transformer. Future class definitions will not
	 * be shown to the transformer. Removes the most-recently-added matching
	 * instance of the transformer. Due to the multi-threaded nature of class
	 * loading, it is possible for a transformer to receive calls after it has
	 * been removed. Transformers should be written defensively to expect this
	 * situation.
	 *
	 * @param transformer
	 ...
	 * retransformation of classes. The ability to retransform an already loaded
	 * class is an optional capability of a JVM. Retransformation will only be
	 * supported if the <code>Can-Retransform-Classes</code> manifest attribute
	 * is set to <code>true</code> in the agent JAR file (as described in the
	 * {@linkplain java.lang.instrument package specification}) and the JVM
	 * supports this capability. During a single instantiation of a single JVM,
	 * multiple calls to this method will always return the same answer.
	 * 
	 * @return true if the current JVM configuration supports retransformation
	 ...
	 * {@linkplain #redefineClasses redefined}, the initial class file bytes can
	 * be transformed with the {@link java.lang.instrument.ClassFileTransformer
	 * ClassFileTransformer}. This function reruns the transformation process
	 * (whether or not a transformation has previously occurred). This
	 * retransformation follows these steps:
	 * <ul>
	 * <li>starting from the initial class file bytes</li>
	 ...
	 * during the last class load or redefine are reused as the output of the
	 * transformation; note that this is equivalent to reapplying the previous
	 * transformation, unaltered; except that
	 * {@link java.lang.instrument.ClassFileTransformer#transform transform} is
	 * not called</li>
	 * <li>for each transformer that was added with <code>canRetransform</code>
	 * true, the {@link java.lang.instrument.ClassFileTransformer#transform
	 * transform} method is called in these transformers</li>
	 * <li>the transformed class file bytes are installed as the new definition
	 * of the class</li>
	 * </ul>
	 * <P>
	 *
	 ...
	 * applied), however they might not exactly match them. The constant pool
	 * might not have the same layout or contents. The constant pool may have
	 * more or fewer entries. Constant pool entries may be in a different order;
	 * however, constant pool indices in the bytecodes of methods will
	...
	/**
	 * Returns whether or not the current JVM configuration supports
	 * redefinition of classes. The ability to redefine an already loaded class
	 * is an optional capability of a JVM. Redefinition will only be supported
	 * if the <code>Can-Redefine-Classes</code> manifest attribute is set to
	 * <code>true</code> in the agent JAR file (as described in the
	 * {@linkplain java.lang.instrument package specification}) and the JVM
	 * supports this capability. During a single instantiation of a single JVM,
	 * multiple calls to this method will always return the same answer.
	 * 
	 * @return true if the current JVM configuration supports redefinition of
	 ...
	 * recompiling from source for fix-and-continue debugging. Where the
	 ...
	 * {@linkplain #retransformClasses retransformation} or
	 * {@linkplain #redefineClasses redefinition}. If a class is modifiable then
	 * this method returns <code>true</code>. If a class is not modifiable then
	 * this method returns <code>false</code>.
	 * <P>
	 * For a class to be retransformed, {@link #isRetransformClassesSupported}
	 * must also be true. But the value of
	 * <code>isRetransformClassesSupported()</code> does not influence the value
	 * returned by this function. For a class to be redefined,
	 * {@link #isRedefineClassesSupported} must also be true. But the value of
	 * <code>isRedefineClassesSupported()</code> does not influence the value
	 * returned by this function.
	 * <P>
	 * Primitive classes (for example, <code>java.lang.Integer.TYPE</code>) and
	...
	/**
	 * Returns an array of all classes for which <code>loader</code> is an
	 * initiating loader. If the supplied loader is <code>null</code>, classes
	 ...
	 * class loader for the purpose of instrumentation. Failure to observe this
	 * warning could result in unexpected behavior that is difficult to
	 * diagnose. For example, suppose there is a loader L, and L's parent for
	 * delegation is the bootstrap class loader. Furthermore, a method in class
	 * C, a class defined by L, makes reference to a non-public accessor class
	 * C$1. If the JAR file contains a class C$1 then the delegation to the
	 * bootstrap class loader will cause C$1 to be defined by the bootstrap
	 * class loader. In this example an <code>IllegalAccessError</code> will be
	 ...
	 * virtual machine has previously unsuccessfully attempted to resolve always
	 * fails with the same error that was thrown as a result of the initial
	 * resolution attempt. Consequently, if the JAR file contains an entry that
	 * corresponds to a class for which the Java virtual machine has
	 ...
	 * loader for the purpose of instrumentation. Failure to observe this
	 ...
	 * <code>appendToClassPathForInstrumentation</code> which takes a single
	 * parameter of type <code>java.lang.String</code>. The method is not
	 * required to have <code>public</code> access. The name of the JAR file is
	 ...
	 * {@linkplain #setNativeMethodPrefix(ClassFileTransformer,String) setting a
	 * native method prefix}. The ability to set a native method prefix is an
	 * optional capability of a JVM. Setting a native method prefix will only be
	 * supported if the <code>Can-Set-Native-Method-Prefix</code> manifest
	 * attribute is set to <code>true</code> in the agent JAR file (as described
	 * in the {@linkplain java.lang.instrument package specification}) and the
	 * JVM supports this capability. During a single instantiation of a single
	 * JVM, multiple calls to this method will always return the same answer.
	 * 
	 * @return true if the current JVM configuration supports setting a native
	 *         method prefix, false if not.
	 ...
	 * </pre>
	 * <p>
	 * We could transform the class file (with the ClassFileTransformer during
	 * the initial definition of the class) so that this becomes:
	 * 
	 * <pre>
	 *   boolean foo(int x) {
	 ...
	 * </pre>
	 * <p>
	 * Where <code>foo</code> becomes a wrapper for the actual native method
	 * with the appended prefix "wrapped_". Note that "wrapped_" would be a poor
	 * choice of prefix since it might conceivably form the name of an existing
	 * method thus something like "$$$MyAgentWrapped$$$_" would be better but
	 * would make these examples less readable.
	 * <p>
	 * The wrapper will allow data to be collected on the native method call,
	 * but now the problem becomes linking up the wrapped method with the native
	 * implementation. That is, the method <code>wrapped_foo</code> needs to be
	 * resolved to the native implementation of <code>foo</code>, which might
	 * be:
	 * 
	 * <pre>
	 ...
	 * to occur. Specifically, when the standard resolution fails, the
	 * resolution is retried taking the prefix into consideration. There are two
	 * ways that resolution occurs, explicit resolution with the JNI function
	 * <code>RegisterNatives</code> and the normal automatic resolution. For
	 * <code>RegisterNatives</code>, the JVM will attempt this association:
	 * 
	 * <pre>
	 * {@code
	 ...
	 * <p>
	 * Since each <code>ClassFileTransformer</code> can do its own
	 * transformation of the bytecodes, more than one layer of wrappers may be
	 * applied. Thus each transformer needs its own prefix. Since
	 * transformations are applied in order, the prefixes, if applied, will be
	 * applied in the same order (see
	 * {@link #addTransformer(ClassFileTransformer,boolean) addTransformer}).
	 * Thus if three transformers applied wrappers, <code>foo</code> might
	 * become <code>$trans3_$trans2_$trans1_foo</code>. But if, say, the second
	 * transformer did not apply a wrapper to <code>foo</code> it would be just
	 * <code>$trans3_$trans1_foo</code>. To be able to efficiently determine the
	 * sequence of prefixes, an intermediate prefix is only applied if its
	 * non-native wrapper exists. Thus, in the last example, even though
	 * <code>$trans1_foo</code> is not a native method, the
	 * <code>$trans1_</code> prefix is applied since <code>$trans1_foo</code>
	 * exists.
	 *
	 * @param transformer
