...
public class Runtime {
	 ...
	 * any, are started in some unspecified order and allowed to run
	 ...
	 * machine will promptly shut down and exit. When the virtual machine is
	 * terminated due to user logoff or system shutdown the underlying operating
	 ...
	 * machine is terminated externally, for example with the <tt>SIGKILL</tt>
	 * signal on Unix or the <tt>TerminateProcess</tt> call on Microsoft
	 * Windows. The virtual machine may also abort if a native method goes awry
	 ...
	 *             If the specified hook has already been registered, or if it
	 *             can be determined that the hook is already running or has
	 *             already been run
	 *
	 ...
	 *
	 * @throws NullPointerException
	 *             If <code>command</code> is <code>null</code>, or one of the
	 ...
	 * <code>new {@link StringTokenizer}(command)</code> with no further
	 * modification of the character categories. The tokens produced by the
	 * tokenizer are then placed in the new string array <code>cmdarray</code>,
	 * in the same order.
	 *
	 * @param command
	 *            a specified system command.
	 ...
	 *
	 * @throws NullPointerException
	 *             If <code>cmdarray</code> is <code>null</code>, or one of the
	 *             elements of <code>cmdarray</code> is <code>null</code>
	 *
	 * @throws IndexOutOfBoundsException
	 *             If <code>cmdarray</code> is an empty array (has length
	 *             <code>0</code>)
	 *
	 * @see ProcessBuilder
	 */
	 ...
	 * @throws NullPointerException
	 *             If <code>cmdarray</code> is <code>null</code>, or one of the
	 *             elements of <code>cmdarray</code> is <code>null</code>, or
	 *             one of the elements of <code>envp</code> is <code>null</code>
	 *
	 * @throws IndexOutOfBoundsException
	 *             If <code>cmdarray</code> is an empty array (has length
	 *             <code>0</code>)
	 ...
	 * the <code>gc</code> method may result in increasing the value returned by
	 ...
	 * control returns from the method call, the virtual machine has made its
	 * best effort to recycle all discarded objects.
	 * <p>
	 * The name <code>gc</code> stands for "garbage collector". The virtual
	 ...
	 * explicitly.
	 * <p>
	 * The method {@link System#gc()} is the conventional and convenient means
	 ...
	 * not yet been run. When control returns from the method call, the virtual
	 * machine has made a best effort to complete all outstanding finalizations.
	 * <p>
	 * The virtual machine performs the finalization process automatically as
	 * needed, in a separate thread, if the <code>runFinalization</code> method
	 * is not invoked explicitly.
	 * <p>
	 * The method {@link System#runFinalization()} is the conventional and
	...
	/**
	 * Enables/Disables tracing of instructions. If the <code>boolean</code>
	 * argument is <code>true</code>, this method suggests that the Java virtual
	 * machine emit debugging information for each instruction in the virtual
	 * machine as it is executed. The format of this information, and the file
	 * or other output stream to which it is emitted, depends on the host
	 * environment. The virtual machine may ignore this request if it does not
	 * support this feature. The destination of the trace output is system
	 * dependent.
	 * <p>
	 * If the <code>boolean</code> argument is <code>false</code>, this method
	 * causes the virtual machine to stop performing the detailed instruction
	...
	/**
	 * Enables/Disables tracing of method calls. If the <code>boolean</code>
	 * argument is <code>true</code>, this method suggests that the Java virtual
	 * machine emit debugging information for each method in the virtual machine
	 * as it is called. The format of this information, and the file or other
	 * output stream to which it is emitted, depends on the host environment.
	 * The virtual machine may ignore this request if it does not support this
	 * feature.
	 * <p>
	 * Calling this method with argument false suggests that the virtual machine
	 ...
	 * VM, then the JNI_OnLoad_L function exported by the library is invoked
	 ...
	 * <p>
	 * First, if there is a security manager, its <code>checkLink</code> method
	 * is called with the <code>filename</code> as its argument. This may result
	 ...
	 * <p>
	 * The method {@link System#load(String)} is the conventional and convenient
	 ...
	 *                if either the filename is not an absolute path name, the
	 *                native library is not statically linked with the VM, or
	 *                the library cannot be mapped to a native library image by
	 *                the host system.
	 ...
	 * The <code>libname</code> argument must not contain any platform specific
	 * prefix, file extension or path. If a native library called
	 ...
	 * <p>
	 * First, if there is a security manager, its <code>checkLink</code> method
	 * is called with the <code>libname</code> as its argument. This may result
	 * in a security exception.
	 * <p>
	 * The method {@link System#loadLibrary(String)} is the conventional and
	 * convenient means of invoking this method. If native methods are to be
	 * used in the implementation of a class, a standard strategy is to put the
	 * native code in a library file (call it <code>LibFile</code>) and then to
	 * put a static initializer: <blockquote>
	 * 
	 * <pre>
	 * static {
	 ...
	 * </pre>
	 * 
	 * </blockquote> within the class declaration. When the class is loaded and
	 ...
	 *                specified dynamic library
	 * @exception UnsatisfiedLinkError
	 *                if either the libname argument contains a file path, the
	 *                native library is not statically linked with the VM, or
	 *                the library cannot be mapped to a native library image by
	 *                the host system.
	 * @exception NullPointerException
	 ...
	 * <code>InputStream</code> and returns an <code>InputStream</code>
	 * equivalent to the argument in all respects except that it is localized:
	 * as characters in the local character set are read from the stream, they
	 * are automatically converted from the local character set to Unicode.
	 * <p>
	 ...
	 * <code>OutputStream</code> and returns an <code>OutputStream</code>
	 * equivalent to the argument in all respects except that it is localized:
	 * as Unicode characters are written to the stream, they are automatically
