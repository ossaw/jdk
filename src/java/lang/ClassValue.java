 ...
 * class encountered at a message send call site, it can use a
 * {@code ClassValue} to cache information needed to perform the message send
 * quickly, for each class encountered.
 * 
...
public abstract class ClassValue<T> {
     ...
     * value with the {@link #get get} method.
     * <p>
     ...
     * it is obtained by an invocation of the {@link #computeValue computeValue}
     * method.
     * <p>
     * The actual installation of the value on the class is performed
     * atomically. At that point, if several racing threads have computed
     * values, one is chosen, and returned to all the racing threads.
     * <p>
     * The {@code type} parameter is typically a class, but it may be any type,
     * such as an interface, a primitive type (like {@code int.class}), or
     * {@code void.class}.
     * <p>
     * In the absence of {@code remove} calls, a class value has a simple state
     * diagram: uninitialized and initialized. When {@code remove} calls are
     * made, the rules for value observation are more complex. See the
    ...
    public T get(Class<?> type) {
        ...
        // racing e : current value <=> stale value from current cache or from
        // stale cache
        // invariant: e is null or an Entry with readable Entry.version and
        // Entry.value
        if (match(e))
            // invariant: No false positive matches. False negatives are OK if
            // rare.
        ...
        // 4. the GC has somehow managed to delete e.version and clear the
        // reference
        return getFromBackup(cache, type);
    }

    /**
     * Removes the associated value for the given class. If this value is
     * subsequently {@linkplain #get read} for the same class, its value will be
     * reinitialized by invoking its {@link #computeValue computeValue} method.
     * This may result in an additional invocation of the {@code computeValue}
     * method for the given class.
     * <p>
     * In order to explain the interaction between {@code get} and
     * {@code remove} calls, we must model the state transitions of a class
     * value to take into account the alternation between uninitialized and
     * initialized states. To do this, number these states sequentially from
     * zero, and note that uninitialized (or removed) states are numbered with
     * even numbers, while initialized (or re-initialized) states have odd
     * numbers.
     * <p>
     ...
     * <p>
     * When a thread {@code T} queries a class value in state {@code 2N}, the
     * thread first attempts to initialize the class value to state {@code 2N+1}
     * by invoking {@code computeValue} and installing the resulting value.
     * <p>
     * When {@code T} attempts to install the newly computed value, if the state
     * is still at {@code 2N}, the class value will be initialized with the
     * computed value, advancing it to state {@code 2N+1}.
     * <p>
     * Otherwise, whether the new state is even or odd, {@code T} will discard
     * the newly computed value and retry the {@code get} operation.
     * <p>
     * Discarding and retrying is an important proviso, since otherwise
     * {@code T} could potentially install a disastrously stale value. For
     * example:
     * <ul>
     * <li>{@code T} calls {@code CV.get(C)} and sees state {@code 2N}
     * <li>{@code T} quickly computes a time-dependent value {@code V0} and gets
     * ready to install it
     * <li>{@code T} is hit by an unlucky paging or scheduling event, and goes
     * to sleep for a long time
     * <li>...meanwhile, {@code T2} also calls {@code CV.get(C)} and sees state
     * {@code 2N}
     * <li>{@code T2} quickly computes a similar time-dependent value {@code V1}
     * and installs it on {@code CV.get(C)}
     * <li>{@code T2} (or a third thread) then calls {@code CV.remove(C)},
     * undoing {@code T2}'s work
     * <li>the previous actions of {@code T2} are repeated several times
     * <li>also, the relevant computed values change over time: {@code V1},
     * {@code V2}, ...
     * <li>...meanwhile, {@code T} wakes up and attempts to install {@code V0};
     * <em>this must fail</em>
     * </ul>
     * We can assume in the above scenario that {@code CV.computeValue} uses
     * locks to properly observe the time-dependent states as it computes
     * {@code V1}, etc. This does not remove the threat of a stale value, since
     * there is a window of time between the return of {@code computeValue} in
     * {@code T} and the installation of the the new value. No user
    ...
    private static Entry<?>[] getCacheCarefully(Class<?> type) {
        // racing type.classValueMap{.cacheArray} : null => new Entry[X] <=> new
        // Entry[Y]
        ...
        return cache;
        // invariant: returned value is safe to dereference and check for an
        // Entry
    }

    /**
     * Initial, one-element, empty cache used by all Class instances. Must never
     * be filled.
     */
    private static final Entry<?>[] EMPTY_CACHE = { null };

    /**
     * Slow tail of ClassValue.get to retry at nearby locations in the cache, or
     * take a slow lock and check the hash table. Called only if the first probe
     * was empty or a collision. This is a separate method, so compilers can
    ...
    private T getFromHashMap(Class<?> type) {
        // The fail-safe recovery is to fall back to the underlying
        // classValueMap.
        ClassValueMap map = getMap(type);
        for (;;) {
            ...
            if (e != null)
                return e.value();
            // else try again, in case a racing thread called remove (so e ==
            // null)
        }
    }
    ...
    boolean match(Entry<?> e) {
        // racing e.version : null (blank) => unique Version token => null
        // (GC-ed version)
        // non-racing this.version : v1 => v2 => ... (updates are read
        // faithfully from volatile)
        return (e != null && e.get() == this.version);
        // invariant: No false positives on version match. Null is OK for false
        // negative.
        // invariant: If version matches, then e.value is readable (final set in
        // Entry.<init>)
    }
     ...
     * object {@code ClassValue.this} is incorrect since subclasses may override
     * {@code ClassValue.equals}, which could confuse keys in the ClassValueMap.
     */
    final Identity identity = new Identity();

    /**
     * Current version for retrieving this class value from the cache. Any
     * number of computeValue calls can be cached in association with one
     * version. But the version changes when a remove (on any type) is executed.
     * A version change invalidates all cache entries for the affected
     * ClassValue, by marking them as stale. Stale cache entries do not force
     * another call to computeValue, but they do require a synchronized visit to
     * a backing map.
     * <p>
     * All user-visible state changes on the ClassValue take place under a lock
     * inside the synchronized methods of ClassValueMap. Readers (of
     * ClassValue.get) are notified of such state changes when this.version is
     * bumped to a new token. This variable must be volatile so that an
     ...
     * (under a lock) to this.value == V2. This will typically be harmless, but
     * if T1 and T2 interact causally via some other channel, such that T1's
     * further actions are constrained (in the JMM) to happen after the V2
     * event, then T1's observation of V1 will be an error.
     * <p>
     * The practical effect of making this.version be volatile is that it cannot
     * be hoisted out of a loop (by an optimizing JIT) or otherwise cached. Some
     * machines may also require a barrier instruction to execute before
     * this.version.
     */
     ...
     * <li>else live
     * </ul>
     * Promises are never put into the cache; they only live in the backing map
     * while a computeValue call is in flight. Once an entry goes stale, it can
     * be reset at any time into the dead state.
     */
    static class Entry<T> extends WeakReference<Version<T>> {
        ...
        Entry(Version<T> version) {
            super(version);
            this.value = this; // for a promise, value is not of type T, but
                               // Entry!
        }
    ...
    private static ClassValueMap getMap(Class<?> type) {
        ...
        // if a null is observed, a map is created (lazily, synchronously,
        // uniquely)
    ...
    /**
     * A backing map for all ClassValues, relative a single given type. Gives a
    ...
    static class ClassValueMap extends WeakHashMap<ClassValue.Identity, Entry<?>> {
        ...
        /**
         * Number of entries initially allocated to each type when first used
         * with any ClassValue. It would be pointless to make this much smaller
         * than the Class and ClassValueMap objects themselves. Must be a power
         * of 2.
         */
        private static final int INITIAL_ENTRIES = 32;

        /**
         * Build a backing map for ClassValues, relative the given type. Also,
         * create an empty cache array and install it on the class.
         */
        ...
        /**
         * Initiate a query. Store a promise (placeholder) if there is no value
         * yet.
         */
        synchronized <T> Entry<T> startEntry(ClassValue<T> classValue) {
            ...
            if (e == null) {
                e = v.promise();
                // The presence of a promise means that a value is pending for
                // v.
                ...
                if (e.version() != v) {
                    // There is a stale but valid entry here; make it fresh
                    // again.
                    // Once an entry is in the hash table, we don't care what
                    // its version is.
        ...
        synchronized <T> Entry<T> finishEntry(ClassValue<T> classValue, Entry<T> e) {
            ...
            if (e == e0) {
                // We can get here during exception processing, unwinding from
                // computeValue.
                assert (e.isPromise());
        ...
        synchronized void removeEntry(ClassValue<?> classValue) {
            ...
            if (e == null) {
                // Uninitialized, and no pending calls to computeValue. No
                // change.
            } else if (e.isPromise()) {
        ...
        static Entry<?> loadFromCache(Entry<?>[] cache, int i) {
            ...
            // invariant: returned value is null or well-constructed (ready to
            // match)
        }
        ...
        static <T> Entry<T> probeBackupLocations(Entry<?>[] cache, ClassValue<T> classValue) {
            ...
            for (int i = home + 1; i < home + PROBE_LIMIT; i++) {
                ...
                if (classValue.match(e)) {
                    // relocate colliding entry e2 (from cache[home]) to first
                    // empty slot
                    cache[home] = e;
                    if (pos2 >= 0) {
        ...
        /// --------
        /// Below this line all functions are private, and assume synchronized
        /// access.
        /// --------
        ...
        /**
         * Clearing a cache slot risks disconnecting following entries from the
         * head of a non-null run, which would allow them to be found via
         * reprobes. Find an entry after cache[begin] to plug into the hole, or
         * return null if none is needed.
         */
        ...
        /**
         * Add the given entry to the cache, in its home location, unless it is
         * out of date.
         */
        ...
        /**
         * Note an entry that is about to be overwritten. If it is not live,
         * quietly replace it by null. If it is an actual null, increment
         * cacheLoad, because the caller is going to store something in its
         * place.
         */
