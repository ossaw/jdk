...
/**
 * The {@code Character} class wraps a value of the primitive type {@code char}
 * in an object. An object of type {@code Character} contains a single field
 * whose type is {@code char}.
 * <p>
 * In addition, this class provides several methods for determining a
 * character's category (lowercase letter, digit, etc.) and for converting
 * characters from uppercase to lowercase and vice versa.
 * <p>
 * Character information is based on the Unicode Standard, version 6.2.0.
 * <p>
 * The methods and data of class {@code Character} are defined by the
 * information in the <i>UnicodeData</i> file that is part of the Unicode
 * Character Database maintained by the Unicode Consortium. This file specifies
 * various properties including name and general category for every defined
 * Unicode code point or character range.
 * <p>
 ...
 *
 * <p>
 * The {@code char} data type (and therefore the value that a {@code Character}
 * object encapsulates) are based on the original Unicode specification, which
 * defined characters as fixed-width 16-bit entities. The Unicode Standard has
 * since been changed to allow for characters whose representation requires more
 * than 16 bits. The range of legal <em>code point</em>s is now U+0000 to
 * U+10FFFF, known as <em>Unicode scalar value</em>. (Refer to the
 * <a href="http://www.unicode.org/reports/tr27/#notation"><i> definition</i>
 * </a> of the U+<i>n</i> notation in the Unicode Standard.)
 *
 * <p>
 * <a name="BMP">The set of characters from U+0000 to U+FFFF</a> is sometimes
 * referred to as the <em>Basic Multilingual Plane (BMP)</em>.
 * <a name="supplementary">Characters</a> whose code points are greater than
 * U+FFFF are called <em>supplementary character</em>s. The Java platform uses
 * the UTF-16 representation in {@code char} arrays and in the {@code String}
 * and {@code StringBuffer} classes. In this representation, supplementary
 * characters are represented as a pair of {@code char} values, the first from
 * the <em>high-surrogates</em> range, (&#92;uD800-&#92;uDBFF), the second from
 * the <em>low-surrogates</em> range (&#92;uDC00-&#92;uDFFF).
 *
 * <p>
 * A {@code char} value, therefore, represents Basic Multilingual Plane (BMP)
 * code points, including the surrogate code points, or code units of the UTF-16
 * encoding. An {@code int} value represents all Unicode code points, including
 * supplementary code points. The lower (least significant) 21 bits of
 * {@code int} are used to represent Unicode code points and the upper (most
 * significant) 11 bits must be zero. Unless otherwise specified, the behavior
 * with respect to supplementary characters and surrogate {@code char} values is
 * as follows:
 *
 * <ul>
 * <li>The methods that only accept a {@code char} value cannot support
 * supplementary characters. They treat {@code char} values from the surrogate
 ...
 * represent a letter.
 *
 * <li>The methods that accept an {@code int} value support all Unicode
 * characters, including supplementary characters. For example,
 * {@code Character.isLetter(0x2F81A)} returns {@code true} because the code
 * point value represents a letter (a CJK ideograph).
 * </ul>
 *
 * <p>
 * In the Java SE API documentation, <em>Unicode code point</em> is used for
 * character values in the range between U+0000 and U+10FFFF, and
 * <em>Unicode code unit</em> is used for 16-bit {@code char} values that are
 * code units of the <em>UTF-16</em> encoding. For more information on Unicode
 * terminology, refer to the <a href="http://www.unicode.org/glossary/">Unicode
 * Glossary</a>.
 *
 * @author Lee Boynton
 * @author Guy Steele
...
public final class Character implements java.io.Serializable, Comparable<Character> {
	 ...
	 * radix argument in radix-conversion methods such as the {@code digit}
	 * method, the {@code forDigit} method, and the {@code toString} method of
	 * class {@code Integer}.
	 *
	 ...
	 * constant value of this field is the largest value permitted for the radix
	 * argument in radix-conversion methods such as the {@code digit} method,
	 * the {@code forDigit} method, and the {@code toString} method of class
	 * {@code Integer}.
	 *
	 * @see Character#digit(char, int)
	 * @see Character#forDigit(int, int)
	...
	/**
	 * Undefined bidirectional character type. Undefined {@code char} values
	...
	/**
	 * The maximum value of a Unicode surrogate code unit in the UTF-16
	 * encoding, constant {@code '\u005CuDFFF'}.
	 *
	 * @since 1.5
	 */
	...
	/**
	 * The minimum value of a
	 * <a href="http://www.unicode.org/glossary/#code_point"> Unicode code
	 ...
	 * Unicode specification. Character blocks generally define characters used
	 * for a specific script or purpose. A character is contained by at most one
	 * Unicode block.
	 *
	 * @since 1.2
	 */
	public static final class UnicodeBlock extends Subset {
		...
		/**
		 * Creates a UnicodeBlock with the given identifier name and alias
		 * names.
		 */
		...
		public static final UnicodeBlock MATHEMATICAL_OPERATORS = new UnicodeBlock(
				"MATHEMATICAL_OPERATORS", "MATHEMATICAL OPERATORS", "MATHEMATICALOPERATORS");
		...
		public static final UnicodeBlock ENCLOSED_ALPHANUMERICS = new UnicodeBlock(
				"ENCLOSED_ALPHANUMERICS", "ENCLOSED ALPHANUMERICS", "ENCLOSEDALPHANUMERICS");
		...
		public static final UnicodeBlock MISCELLANEOUS_SYMBOLS = new UnicodeBlock(
				"MISCELLANEOUS_SYMBOLS", "MISCELLANEOUS SYMBOLS", "MISCELLANEOUSSYMBOLS");
		...
		public static final UnicodeBlock CJK_UNIFIED_IDEOGRAPHS = new UnicodeBlock(
				"CJK_UNIFIED_IDEOGRAPHS", "CJK UNIFIED IDEOGRAPHS", "CJKUNIFIEDIDEOGRAPHS");
		 ...
		 *             {@link #LOW_SURROGATES}. These new constants match the
		 *             block definitions of the Unicode Standard. The
		 *             {@link #of(char)} and {@link #of(int)} methods return the
		 *             new constants, not SURROGATES_AREA.
		 */
		@Deprecated
		...
		public static final UnicodeBlock CYRILLIC_SUPPLEMENTARY = new UnicodeBlock(
				"CYRILLIC_SUPPLEMENTARY", "CYRILLIC SUPPLEMENTARY", "CYRILLICSUPPLEMENTARY",
		...
		public static final UnicodeBlock SUPPLEMENTAL_ARROWS_A = new UnicodeBlock(
				"SUPPLEMENTAL_ARROWS_A", "SUPPLEMENTAL ARROWS-A", "SUPPLEMENTALARROWS-A");
		...
		public static final UnicodeBlock SUPPLEMENTAL_ARROWS_B = new UnicodeBlock(
				"SUPPLEMENTAL_ARROWS_B", "SUPPLEMENTAL ARROWS-B", "SUPPLEMENTALARROWS-B");
		 ...
		 * block represents codepoint values in the high surrogate range: U+D800
		 * through U+DB7F
		 *
		 * @since 1.5
		 */
		 ...
		 * block. This block represents codepoint values in the private use high
		 ...
		 * represents codepoint values in the low surrogate range: U+DC00
		 * through U+DFFF
		 *
		 * @since 1.5
		 */
		 ...
		 * @since 1.7
		 */
		public static final UnicodeBlock TAI_THAM = new UnicodeBlock("TAI_THAM", "TAI THAM",
				"TAITHAM");
		 ...
		 * @since 1.7
		 */
		public static final UnicodeBlock OL_CHIKI = new UnicodeBlock("OL_CHIKI", "OL CHIKI",
				"OLCHIKI");
		...
		public static final UnicodeBlock MODIFIER_TONE_LETTERS = new UnicodeBlock(
				"MODIFIER_TONE_LETTERS", "MODIFIER TONE LETTERS", "MODIFIERTONELETTERS");
		 ...
		 * @since 1.7
		 */
		public static final UnicodeBlock KAYAH_LI = new UnicodeBlock("KAYAH_LI", "KAYAH LI",
				"KAYAHLI");
		...
		public static final UnicodeBlock HANGUL_JAMO_EXTENDED_A = new UnicodeBlock(
				"HANGUL_JAMO_EXTENDED_A", "HANGUL JAMO EXTENDED-A", "HANGULJAMOEXTENDED-A");
		 ...
		 * @since 1.7
		 */
		public static final UnicodeBlock TAI_VIET = new UnicodeBlock("TAI_VIET", "TAI VIET",
				"TAIVIET");
		...
		public static final UnicodeBlock HANGUL_JAMO_EXTENDED_B = new UnicodeBlock(
				"HANGUL_JAMO_EXTENDED_B", "HANGUL JAMO EXTENDED-B", "HANGULJAMOEXTENDED-B");
		...
		public static final UnicodeBlock ANCIENT_GREEK_NUMBERS = new UnicodeBlock(
				"ANCIENT_GREEK_NUMBERS", "ANCIENT GREEK NUMBERS", "ANCIENTGREEKNUMBERS");
		...
		public static final UnicodeBlock INSCRIPTIONAL_PARTHIAN = new UnicodeBlock(
				"INSCRIPTIONAL_PARTHIAN", "INSCRIPTIONAL PARTHIAN", "INSCRIPTIONALPARTHIAN");
		...
		public static final UnicodeBlock INSCRIPTIONAL_PAHLAVI = new UnicodeBlock(
				"INSCRIPTIONAL_PAHLAVI", "INSCRIPTIONAL PAHLAVI", "INSCRIPTIONALPAHLAVI");
		...
		public static final UnicodeBlock COUNTING_ROD_NUMERALS = new UnicodeBlock(
				"COUNTING_ROD_NUMERALS", "COUNTING ROD NUMERALS", "COUNTINGRODNUMERALS");
		 ...
		 * @since 1.8
		 */
		public static final UnicodeBlock SUNDANESE_SUPPLEMENT = new UnicodeBlock(
				"SUNDANESE_SUPPLEMENT", "SUNDANESE SUPPLEMENT", "SUNDANESESUPPLEMENT");
		 ...
		 * @since 1.8
		 */
		public static final UnicodeBlock MEETEI_MAYEK_EXTENSIONS = new UnicodeBlock(
				"MEETEI_MAYEK_EXTENSIONS", "MEETEI MAYEK EXTENSIONS", "MEETEIMAYEKEXTENSIONS");
		 ...
		 * @since 1.8
		 */
		public static final UnicodeBlock MEROITIC_HIEROGLYPHS = new UnicodeBlock(
				"MEROITIC_HIEROGLYPHS", "MEROITIC HIEROGLYPHS", "MEROITICHIEROGLYPHS");
		...
		private static final int blockStarts[] = { 0x0000, // 0000..007F; Basic
				...
				0x18B0, // 18B0..18FF; Unified Canadian Aboriginal Syllabics
						// Extended
				...
				0x2B820, // unassigned
				0x2F800, // 2F800..2FA1F; CJK Compatibility Ideographs
							// Supplement
				0x2FA20, // unassigned
		...
		private static final UnicodeBlock[] blocks = { BASIC_LATIN, LATIN_1_SUPPLEMENT,
				LATIN_EXTENDED_A, LATIN_EXTENDED_B, IPA_EXTENSIONS, SPACING_MODIFIER_LETTERS,
				COMBINING_DIACRITICAL_MARKS, GREEK, CYRILLIC, CYRILLIC_SUPPLEMENTARY, ARMENIAN,
				HEBREW, ARABIC, SYRIAC, ARABIC_SUPPLEMENT, THAANA, NKO, SAMARITAN, MANDAIC, null,
				ARABIC_EXTENDED_A, DEVANAGARI, BENGALI, GURMUKHI, GUJARATI, ORIYA, TAMIL, TELUGU,
				KANNADA, MALAYALAM, SINHALA, THAI, LAO, TIBETAN, MYANMAR, GEORGIAN, HANGUL_JAMO,
				ETHIOPIC, ETHIOPIC_SUPPLEMENT, CHEROKEE, UNIFIED_CANADIAN_ABORIGINAL_SYLLABICS,
				OGHAM, RUNIC, TAGALOG, HANUNOO, BUHID, TAGBANWA, KHMER, MONGOLIAN,
				UNIFIED_CANADIAN_ABORIGINAL_SYLLABICS_EXTENDED, LIMBU, TAI_LE, NEW_TAI_LUE,
				KHMER_SYMBOLS, BUGINESE, TAI_THAM, null, BALINESE, SUNDANESE, BATAK, LEPCHA,
				OL_CHIKI, null, SUNDANESE_SUPPLEMENT, VEDIC_EXTENSIONS, PHONETIC_EXTENSIONS,
				PHONETIC_EXTENSIONS_SUPPLEMENT, COMBINING_DIACRITICAL_MARKS_SUPPLEMENT,
				LATIN_EXTENDED_ADDITIONAL, GREEK_EXTENDED, GENERAL_PUNCTUATION,
				SUPERSCRIPTS_AND_SUBSCRIPTS, CURRENCY_SYMBOLS, COMBINING_MARKS_FOR_SYMBOLS,
				...
				ENCLOSED_ALPHANUMERICS, BOX_DRAWING, BLOCK_ELEMENTS, GEOMETRIC_SHAPES,
				MISCELLANEOUS_SYMBOLS, DINGBATS, MISCELLANEOUS_MATHEMATICAL_SYMBOLS_A,
				...
				GEORGIAN_SUPPLEMENT, TIFINAGH, ETHIOPIC_EXTENDED, CYRILLIC_EXTENDED_A,
				SUPPLEMENTAL_PUNCTUATION, CJK_RADICALS_SUPPLEMENT, KANGXI_RADICALS, null,
				IDEOGRAPHIC_DESCRIPTION_CHARACTERS, CJK_SYMBOLS_AND_PUNCTUATION, HIRAGANA, KATAKANA,
				BOPOMOFO, HANGUL_COMPATIBILITY_JAMO, KANBUN, BOPOMOFO_EXTENDED, CJK_STROKES,
				...
				YI_SYLLABLES, YI_RADICALS, LISU, VAI, CYRILLIC_EXTENDED_B, BAMUM,
				MODIFIER_TONE_LETTERS, LATIN_EXTENDED_D, SYLOTI_NAGRI, COMMON_INDIC_NUMBER_FORMS,
				PHAGS_PA, SAURASHTRA, DEVANAGARI_EXTENDED, KAYAH_LI, REJANG, HANGUL_JAMO_EXTENDED_A,
				JAVANESE, null, CHAM, MYANMAR_EXTENDED_A, TAI_VIET, MEETEI_MAYEK_EXTENSIONS,
				...
				ARABIC_PRESENTATION_FORMS_A, VARIATION_SELECTORS, VERTICAL_FORMS,
				COMBINING_HALF_MARKS, CJK_COMPATIBILITY_FORMS, SMALL_FORM_VARIANTS,
				...
				ANCIENT_SYMBOLS, PHAISTOS_DISC, null, LYCIAN, CARIAN, null, OLD_ITALIC, GOTHIC,
				null, UGARITIC, OLD_PERSIAN, null, DESERET, SHAVIAN, OSMANYA, null,
				CYPRIOT_SYLLABARY, IMPERIAL_ARAMAIC, null, PHOENICIAN, LYDIAN, null,
				MEROITIC_HIEROGLYPHS, MEROITIC_CURSIVE, KHAROSHTHI, OLD_SOUTH_ARABIAN, null,
				AVESTAN, INSCRIPTIONAL_PARTHIAN, INSCRIPTIONAL_PAHLAVI, null, OLD_TURKIC, null,
				RUMI_NUMERAL_SYMBOLS, null, BRAHMI, KAITHI, SORA_SOMPENG, CHAKMA, null, SHARADA,
				null, TAKRI, null, CUNEIFORM, CUNEIFORM_NUMBERS_AND_PUNCTUATION, null,
				EGYPTIAN_HIEROGLYPHS, null, BAMUM_SUPPLEMENT, null, MIAO, null, KANA_SUPPLEMENT,
				null, BYZANTINE_MUSICAL_SYMBOLS, MUSICAL_SYMBOLS, ANCIENT_GREEK_MUSICAL_NOTATION,
				null, TAI_XUAN_JING_SYMBOLS, COUNTING_ROD_NUMERALS, null,
				MATHEMATICAL_ALPHANUMERIC_SYMBOLS, null, ARABIC_MATHEMATICAL_ALPHABETIC_SYMBOLS,
				null, MAHJONG_TILES, DOMINO_TILES, PLAYING_CARDS, ENCLOSED_ALPHANUMERIC_SUPPLEMENT,
				ENCLOSED_IDEOGRAPHIC_SUPPLEMENT, MISCELLANEOUS_SYMBOLS_AND_PICTOGRAPHS, EMOTICONS,
				null, TRANSPORT_AND_MAP_SYMBOLS, ALCHEMICAL_SYMBOLS, null,
				CJK_UNIFIED_IDEOGRAPHS_EXTENSION_B, null, CJK_UNIFIED_IDEOGRAPHS_EXTENSION_C,
				CJK_UNIFIED_IDEOGRAPHS_EXTENSION_D, null, CJK_COMPATIBILITY_IDEOGRAPHS_SUPPLEMENT,
		 ...
		 * given character, or {@code null} if the character is not a member of
		 * a defined block.
		 *
		 * <p>
		 * <b>Note:</b> This method cannot handle
		 * <a href="Character.html#supplementary"> supplementary characters</a>.
		 * To support all Unicode characters, including supplementary
		 * characters, use the {@link #of(int)} method.
		 *
		 * @param c
		 *            The character in question
		 * @return The {@code UnicodeBlock} instance representing the Unicode
		 *         block of which this character is a member, or {@code null} if
		 *         the character is not a member of any Unicode block
		 */
		...
		/**
		 * Returns the object representing the Unicode block containing the
		 * given character (Unicode code point), or {@code null} if the
		 ...
		 *                if the specified {@code codePoint} is an invalid
		 *                Unicode code point.
		...
		/**
		 * Returns the UnicodeBlock with the given name. Block names are
		 * determined by The Unicode Standard. The file
		 * Blocks-&lt;version&gt;.txt defines blocks for a particular version of
		 * the standard. The {@link Character} class specifies the version of
		 * the standard that it supports.
		 * <p>
		 * This method accepts block names in the following forms:
		 * <ol>
		 * <li>Canonical block names as defined by the Unicode Standard. For
		 ...
		 * <li>Canonical block names with all spaces removed. For example,
		 * "BasicLatin" is a valid block name for the "Basic Latin" block.
		 * <li>The text representation of each constant UnicodeBlock identifier.
		 * For example, this method will return the {@link #BASIC_LATIN} block
		 * if provided with the "BASIC_LATIN" name. This form replaces all
		 * spaces and hyphens in the canonical name with underscores.
		 * </ol>
		 * Finally, character case is ignored for all of the valid block name
		 * forms. For example, "BASIC_LATIN" and "basic_latin" are both valid
		 * block names. The en_US locale's case mapping rules are used to
		 * provide case-insensitive string comparisons for block name
		 * validation.
		 * <p>
		 ...
		 *
		 * @param blockName
		 *            A {@code UnicodeBlock} name.
		 * @return The {@code UnicodeBlock} instance identified by
		 *         {@code blockName}
		 * @throws IllegalArgumentException
		 *             if {@code blockName} is an invalid name
		 * @throws NullPointerException
		 *             if {@code blockName} is null
		 * @since 1.5
		 */
	...
	/**
	 * A family of character subsets representing the character scripts defined
	 * in the <a href="http://www.unicode.org/reports/tr24/"> <i>Unicode
	 * Standard Annex #24: Script Names</i></a>. Every Unicode character is
	 * assigned to a single Unicode script, either a specific script, such as
	 * {@link Character.UnicodeScript#LATIN Latin}, or one of the following
	 * three special values, {@link Character.UnicodeScript#INHERITED Inherited}
	...
	public static enum UnicodeScript {
		...
		private static final int[] scriptStarts = { 0x0000, // 0000..0040;
															// COMMON
		...
		private static final UnicodeScript[] scripts = { COMMON, LATIN, COMMON, LATIN, COMMON,
				LATIN, COMMON, LATIN, COMMON, LATIN, COMMON, LATIN, COMMON, LATIN, COMMON, LATIN,
				COMMON, BOPOMOFO, COMMON, INHERITED, GREEK, COMMON, GREEK, COMMON, GREEK, COMMON,
				GREEK, COMMON, GREEK, COPTIC, GREEK, CYRILLIC, INHERITED, CYRILLIC, ARMENIAN,
				COMMON, ARMENIAN, HEBREW, ARABIC, COMMON, ARABIC, COMMON, ARABIC, COMMON, ARABIC,
				COMMON, ARABIC, INHERITED, ARABIC, COMMON, ARABIC, INHERITED, ARABIC, COMMON,
				ARABIC, SYRIAC, ARABIC, THAANA, NKO, SAMARITAN, MANDAIC, ARABIC, DEVANAGARI,
				INHERITED, DEVANAGARI, COMMON, DEVANAGARI, BENGALI, GURMUKHI, GUJARATI, ORIYA,
				TAMIL, TELUGU, KANNADA, MALAYALAM, SINHALA, THAI, COMMON, THAI, LAO, TIBETAN,
				COMMON, TIBETAN, MYANMAR, GEORGIAN, COMMON, GEORGIAN, HANGUL, ETHIOPIC, CHEROKEE,
				CANADIAN_ABORIGINAL, OGHAM, RUNIC, COMMON, RUNIC, TAGALOG, HANUNOO, COMMON, BUHID,
				TAGBANWA, KHMER, MONGOLIAN, COMMON, MONGOLIAN, COMMON, MONGOLIAN,
				CANADIAN_ABORIGINAL, LIMBU, TAI_LE, NEW_TAI_LUE, KHMER, BUGINESE, TAI_THAM,
				BALINESE, SUNDANESE, BATAK, LEPCHA, OL_CHIKI, SUNDANESE, INHERITED, COMMON,
				INHERITED, COMMON, INHERITED, COMMON, INHERITED, COMMON, INHERITED, COMMON, LATIN,
				...
				TIFINAGH, ETHIOPIC, CYRILLIC, COMMON, HAN, COMMON, HAN, COMMON, HAN, COMMON, HAN,
				INHERITED, HANGUL, COMMON, HAN, COMMON, HIRAGANA, INHERITED, COMMON, HIRAGANA,
				COMMON, KATAKANA, COMMON, KATAKANA, BOPOMOFO, HANGUL, COMMON, BOPOMOFO, COMMON,
				KATAKANA, HANGUL, COMMON, HANGUL, COMMON, KATAKANA, COMMON, HAN, COMMON, HAN, YI,
				LISU, VAI, CYRILLIC, BAMUM, COMMON, LATIN, COMMON, LATIN, SYLOTI_NAGRI, COMMON,
				PHAGS_PA, SAURASHTRA, DEVANAGARI, KAYAH_LI, REJANG, HANGUL, JAVANESE, CHAM, MYANMAR,
				TAI_VIET, MEETEI_MAYEK, ETHIOPIC, MEETEI_MAYEK, HANGUL, UNKNOWN, HAN, LATIN,
				ARMENIAN, HEBREW, ARABIC, COMMON, ARABIC, COMMON, INHERITED, COMMON, INHERITED,
				COMMON, ARABIC, COMMON, LATIN, COMMON, LATIN, COMMON, KATAKANA, COMMON, KATAKANA,
				COMMON, HANGUL, COMMON, LINEAR_B, COMMON, GREEK, COMMON, INHERITED, LYCIAN, CARIAN,
				OLD_ITALIC, GOTHIC, UGARITIC, OLD_PERSIAN, DESERET, SHAVIAN, OSMANYA, CYPRIOT,
				IMPERIAL_ARAMAIC, PHOENICIAN, LYDIAN, MEROITIC_HIEROGLYPHS, MEROITIC_CURSIVE,
				KHAROSHTHI, OLD_SOUTH_ARABIAN, AVESTAN, INSCRIPTIONAL_PARTHIAN,
				INSCRIPTIONAL_PAHLAVI, OLD_TURKIC, ARABIC, BRAHMI, KAITHI, SORA_SOMPENG, CHAKMA,
				SHARADA, TAKRI, CUNEIFORM, EGYPTIAN_HIEROGLYPHS, BAMUM, MIAO, KATAKANA, HIRAGANA,
				COMMON, INHERITED, COMMON, INHERITED, COMMON, INHERITED, COMMON, INHERITED, COMMON,
				GREEK, COMMON, ARABIC, COMMON, HIRAGANA, COMMON, HAN, COMMON, INHERITED, UNKNOWN };
		 ...
		 * @param codePoint
		 *            the character (Unicode code point) in question.
		 * @return The {@code UnicodeScript} constant representing the Unicode
		 *         script of which this character is assigned to.
		 *
		 * @exception IllegalArgumentException
		 *                if the specified {@code codePoint} is an invalid
		 *                Unicode code point.
		...
		/**
		 * Returns the UnicodeScript constant with the given Unicode script name
		 * or the script name alias. Script names and their aliases are
		 * determined by The Unicode Standard. The files
		 * Scripts&lt;version&gt;.txt and
		 * PropertyValueAliases&lt;version&gt;.txt define script names and the
		 * script name aliases for a particular version of the standard. The
		 * {@link Character} class specifies the version of the standard that it
		 * supports.
		 * <p>
		 * Character case is ignored for all of the valid script names. The
		 * en_US locale's case mapping rules are used to provide
		 * case-insensitive string comparisons for script name validation.
		 * <p>
		 *
		 * @param scriptName
		 *            A {@code UnicodeScript} name.
		 * @return The {@code UnicodeScript} constant identified by
		 *         {@code scriptName}
		 * @throws IllegalArgumentException
		 *             if {@code scriptName} is an invalid name
		 * @throws NullPointerException
		 *             if {@code scriptName} is null
		 */
	 ...
	 * <tt>char</tt> value. If a new <tt>Character</tt> instance is not
	 * required, this method should generally be used in preference to the
	 * constructor {@link #Character(char)}, as this method is likely to yield
	...
	/**
	 * Returns a hash code for this {@code Character}; equal to the result of
	 * invoking {@code charValue()}.
	 *
	 ...
	 *
	 * @param value
	 *            The {@code char} for which to return a hash code.
	 ...
	 * {@code Character} object that represents the same {@code char} value as
	 * this object.
	 *
	 * @param obj
	...
	/**
	 * Returns a {@code String} object representing this {@code Character}'s
	 * value. The result is a string of length 1 whose sole component is the
	 * primitive {@code char} value represented by this {@code Character}
	 * object.
	 *
	 ...
	 * The result is a string of length 1 consisting solely of the specified
	 * {@code char}.
	 *
	 * @param c
	 ...
	 * <a href="http://www.unicode.org/glossary/#code_point"> Unicode code point
	 * value</a>.
	 *
	 * @param codePoint
	 *            the Unicode code point to be tested
	 * @return {@code true} if the specified code point value is between
	 *         {@link #MIN_CODE_POINT} and {@link #MAX_CODE_POINT} inclusive;
	 ...
	 * <a href="#BMP">Basic Multilingual Plane (BMP)</a>. Such code points can
	 * be represented using a single {@code char}.
	 *
	 * @param codePoint
	 ...
	 *            the character (Unicode code point) to be tested
	 * @return {@code true} if the specified code point is between
	 *         {@link #MIN_SUPPLEMENTARY_CODE_POINT} and {@link #MAX_CODE_POINT}
	...
	/**
	 * Determines if the given {@code char} value is a Unicode <i>surrogate code
	 * unit</i>.
	 *
	 * <p>
	 * Such values do not represent characters by themselves, but are used in
	 ...
	 *            the {@code char} value to be tested.
	 * @return {@code true} if the {@code char} value is between
	 *         {@link #MIN_SURROGATE} and {@link #MAX_SURROGATE} inclusive;
	 *         {@code false} otherwise.
	 * @since 1.7
	 */
	 ...
	 * <a href="http://www.unicode.org/glossary/#surrogate_pair"> Unicode
	 * surrogate pair</a>.
	 * 
	 * <p>
	 ...
	 * @return {@code true} if the specified high and low-surrogate code values
	 *         represent a valid surrogate pair; {@code false} otherwise.
	 * @since 1.5
	 */
	...
	/**
	 * Returns the code point at the given index of the {@code CharSequence}. If
	 * the {@code char} value at the given index in the {@code CharSequence} is
	 * in the high-surrogate range, the following index is less than the length
	 * of the {@code CharSequence}, and the {@code char} value at the following
	 * index is in the low-surrogate range, then the supplementary code point
	 * corresponding to this surrogate pair is returned. Otherwise, the
	 * {@code char} value at the given index is returned.
	 *
	 * @param seq
	 *            a sequence of {@code char} values (Unicode code units)
	 * @param index
	 *            the index to the {@code char} values (Unicode code units) in
	 *            {@code seq} to be converted
	 * @return the Unicode code point at the given index
	 * @exception NullPointerException
	 *                if {@code seq} is null.
	 * @exception IndexOutOfBoundsException
	 *                if the value {@code index} is negative or not less than
	...
	/**
	 * Returns the code point at the given index of the {@code char} array. If
	 * the {@code char} value at the given index in the {@code char} array is in
	 * the high-surrogate range, the following index is less than the length of
	 * the {@code char} array, and the {@code char} value at the following index
	 * is in the low-surrogate range, then the supplementary code point
	 * corresponding to this surrogate pair is returned. Otherwise, the
	 * {@code char} value at the given index is returned.
	 *
	 * @param a
	 ...
	 * {@code CharSequence}. If the {@code char} value at {@code (index - 1)} in
	 * the {@code CharSequence} is in the low-surrogate range,
	 * {@code (index - 2)} is not negative, and the {@code char} value at
	 * {@code (index - 2)} in the {@code CharSequence} is in the high-surrogate
	 * range, then the supplementary code point corresponding to this surrogate
	 * pair is returned. Otherwise, the {@code char} value at
	 * {@code (index - 1)} is returned.
	 *
	 * @param seq
	 *            the {@code CharSequence} instance
	 * @param index
	 ...
	 *                if {@code seq} is null.
	 * @exception IndexOutOfBoundsException
	 *                if the {@code index} argument is less than 1 or greater
	...
	/**
	 * Returns the code point preceding the given index of the {@code char}
	 * array. If the {@code char} value at {@code (index - 1)} in the
	 * {@code char} array is in the low-surrogate range, {@code (index - 2)} is
	 * not negative, and the {@code char} value at {@code (index - 2)} in the
	 * {@code char} array is in the high-surrogate range, then the supplementary
	 * code point corresponding to this surrogate pair is returned. Otherwise,
	 * the {@code char} value at {@code (index - 1)} is returned.
	 *
	 * @param a
	 *            the {@code char} array
	 * @param index
	...
	/**
	 * Returns the code point preceding the given index of the {@code char}
	 * array, where only array elements with {@code index} greater than or equal
	 * to {@code start} can be used. If the {@code char} value at
	 * {@code (index - 1)} in the {@code char} array is in the low-surrogate
	 * range, {@code (index - 2)} is not less than {@code start}, and the
	 * {@code char} value at {@code (index - 2)} in the {@code char} array is in
	 * the high-surrogate range, then the supplementary code point corresponding
	 * to this surrogate pair is returned. Otherwise, the {@code char} value at
	 * {@code (index - 1)} is returned.
	 *
	 * @param a
	 *            the {@code char} array
	 ...
	 *            the index following the code point that should be returned
	 * @param start
	 *            the index of the first array element in the {@code char} array
	 * @return the Unicode code point value before the given index.
	 * @exception NullPointerException
	 *                if {@code a} is null.
	 * @exception IndexOutOfBoundsException
	 *                if the {@code index} argument is not greater than the
	 *                {@code start} argument or is greater than the length of
	 *                the {@code char} array, or if the {@code start} argument
	 ...
	 * <a href="http://www.unicode.org/glossary/#high_surrogate_code_unit"> high
	 * surrogate code unit</a>) of the
	 * <a href="http://www.unicode.org/glossary/#surrogate_pair"> surrogate
	 ...
	 * <a href="Character.html#supplementary">supplementary character</a>, an
	 ...
	 * <a href="http://www.unicode.org/glossary/#low_surrogate_code_unit"> low
	 * surrogate code unit</a>) of the
	 * <a href="http://www.unicode.org/glossary/#surrogate_pair"> surrogate
	 * pair</a> representing the specified supplementary character (Unicode code
	 * point) in the UTF-16 encoding. If the specified character is not a
	 ...
	 * Plane or Plane 0) value, the same value is stored in
	 * {@code dst[dstIndex]}, and 1 is returned. If the specified code point is
	 * a supplementary character, its surrogate values are stored in
	 * {@code dst[dstIndex]} (high-surrogate) and {@code dst[dstIndex+1]}
	 * (low-surrogate), and 2 is returned.
	 *
	 * @param codePoint
	 *            the character (Unicode code point) to be converted.
	 * @param dst
	 *            an array of {@code char} in which the {@code codePoint}'s
	 *            UTF-16 value is stored.
	 * @param dstIndex
	 *            the start index into the {@code dst} array where the converted
	 *            value is stored.
	 * @return 1 if the code point is a BMP code point, 2 if the code point is a
	 *         supplementary code point.
	 * @exception IllegalArgumentException
	 *                if the specified {@code codePoint} is not a valid Unicode
	 *                code point.
	 * @exception NullPointerException
	 *                if the specified {@code dst} is null.
	 * @exception IndexOutOfBoundsException
	 *                if {@code dstIndex} is negative or not less than
	 *                {@code dst.length}, or if {@code dst} at {@code dstIndex}
	 *                doesn't have enough array element(s) to store the
	 *                resulting {@code char} value(s). (If {@code dstIndex} is
	 *                equal to {@code dst.length-1} and the specified
	...
	/**
	 * Converts the specified character (Unicode code point) to its UTF-16
	 * representation stored in a {@code char} array. If the specified code
	 * point is a BMP (Basic Multilingual Plane or Plane 0) value, the resulting
	 * {@code char} array has the same value as {@code codePoint}. If the
	 * specified code point is a supplementary code point, the resulting
	 * {@code char} array has the corresponding surrogate pair.
	 *
	 * @param codePoint
	 *            a Unicode code point
	 * @return a {@code char} array having {@code codePoint}'s UTF-16
	 *         representation.
	 * @exception IllegalArgumentException
	 *                if the specified {@code codePoint} is not a valid Unicode
	 *                code point.
	 ...
	 * {@code beginIndex} and extends to the {@code char} at index
	 * {@code endIndex - 1}. Thus the length (in {@code char}s) of the text
	 * range is {@code endIndex-beginIndex}. Unpaired surrogates within the text
	 ...
	 * @param endIndex
	 *            the index after the last {@code char} of the text range.
	 ...
	 * {@code char} array argument. The {@code offset} argument is the index of
	 * the first {@code char} of the subarray and the {@code count} argument
	 * specifies the length of the subarray in {@code char}s. Unpaired
	 * surrogates within the subarray count as one code point each.
	 *
	 * @param a
	 *            the {@code char} array
	 * @param offset
	 *            the index of the first {@code char} in the given {@code char}
	 *            array
	 * @param count
	 ...
	 *                if {@code a} is null.
	 * @exception IndexOutOfBoundsException
	 *                if {@code offset} or {@code count} is negative, or if
	 *                {@code offset +
	 ...
	 * given {@code index} by {@code codePointOffset} code points. Unpaired
	 * surrogates within the text range given by {@code index} and
	 ...
	 *                if {@code seq} is null.
	 * @exception IndexOutOfBoundsException
	 *                if {@code index} is negative or larger then the length of
	 *                the char sequence, or if {@code codePointOffset} is
	 *                positive and the subsequence starting with {@code index}
	 *                has fewer than {@code codePointOffset} code points, or if
	 ...
	 * {@code Character.getType(ch)}, is {@code LOWERCASE_LETTER}, or it has
	 ...
	 * <p>
	 * <b>Note:</b> This method cannot handle
	 * <a href="#supplementary"> supplementary characters</a>. To support all
	 * Unicode characters, including supplementary characters, use the
	 * {@link #isLowerCase(int)} method.
	 *
	 * @param ch
	 *            the character to be tested.
	 * @return {@code true} if the character is lowercase; {@code false}
	 *         otherwise.
	 ...
	 * <p>
	 * A character is lowercase if its general category type, provided by
	 * {@link Character#getType getType(codePoint)}, is {@code LOWERCASE_LETTER}
	 ...
	 * {@code Character.getType(ch)}, is {@code UPPERCASE_LETTER}. or it has
	 ...
	 * @param ch
	 *            the character to be tested.
	 * @return {@code true} if the character is uppercase; {@code false}
	 *         otherwise.
	 * @see Character#isLowerCase(char)
	 * @see Character#isTitleCase(char)
	 ...
	 * {@code UPPERCASE_LETTER}, or it has contributory property Other_Uppercase
	 ...
	 * provided by {@code Character.getType(ch)}, is {@code TITLECASE_LETTER}.
	 * <p>
	 * Some characters look like pairs of Latin letters. For example, there is
	 * an uppercase letter that looks like "LJ" and has a corresponding
	 * lowercase letter that looks like "lj". A third form, which looks like
	 * "Lj", is the appropriate form to use when rendering a word in lowercase
	 ...
	 * <a href="#supplementary"> supplementary characters</a>. To support all
	 * Unicode characters, including supplementary characters, use the
	 * {@link #isTitleCase(int)} method.
	 *
	 * @param ch
	 *            the character to be tested.
	 * @return {@code true} if the character is titlecase; {@code false}
	 *         otherwise.
	 * @see Character#isLowerCase(char)
	 ...
	 * <p>
	 * A character is a titlecase character if its general category type,
	 * provided by {@link Character#getType(int) getType(codePoint)}, is
	 ...
	 * <ul>
	 * <li>{@code '\u005Cu0030'} through {@code '\u005Cu0039'}, ISO-LATIN-1
	 * digits ({@code '0'} through {@code '9'})
	 * <li>{@code '\u005Cu0660'} through {@code '\u005Cu0669'}, Arabic-Indic
	 * digits
	 * <li>{@code '\u005Cu06F0'} through {@code '\u005Cu06F9'}, Extended
	 * Arabic-Indic digits
	 * <li>{@code '\u005Cu0966'} through {@code '\u005Cu096F'}, Devanagari
	 * digits
	 ...
	 * @param ch
	 *            the character to be tested.
	 * @return {@code true} if the character is a digit; {@code false}
	 *         otherwise.
	 * @see Character#digit(char, int)
	 * @see Character#forDigit(int, int)
	 * @see Character#getType(char)
	 ...
	 * <a href="#supplementary"> supplementary characters</a>. To support all
	 * Unicode characters, including supplementary characters, use the
	 * {@link #isDefined(int)} method.
	 *
	 * @param ch
	 *            the character to be tested
	 * @return {@code true} if the character has a defined meaning in Unicode;
	 *         {@code false} otherwise.
	 ...
	 * provided by {@code Character.getType(ch)}, is any of the following:
	 * <ul>
	 ...
	 * @param ch
	 *            the character to be tested.
	 * @return {@code true} if the character is a letter; {@code false}
	 *         otherwise.
	 * @see Character#isDigit(char)
	 ...
	 * {@code Character.isLetter(char ch)} or {@code Character.isDigit(char ch)}
	 * returns {@code true} for the character.
	 *
	 * <p>
	 * <b>Note:</b> This method cannot handle
	 * <a href="#supplementary"> supplementary characters</a>. To support all
	 ...
	 * @param ch
	 *            the character to be tested.
	 * @return {@code true} if the character is a letter or digit; {@code false}
	 *         otherwise.
	 * @see Character#isDigit(char)
	 ...
	 * <p>
	 * A character is considered to be a letter or digit if either
	 * {@link #isLetter(int) isLetter(codePoint)} or {@link #isDigit(int)
	 * isDigit(codePoint)} returns {@code true} for the character.
	 *
	 * @param codePoint
	 *            the character (Unicode code point) to be tested.
	 * @return {@code true} if the character is a letter or digit; {@code false}
	...
	public static boolean isLetterOrDigit(int codePoint) {
		return ((((1 << Character.UPPERCASE_LETTER) | (1 << Character.LOWERCASE_LETTER)
				| (1 << Character.TITLECASE_LETTER) | (1 << Character.MODIFIER_LETTER)
				| (1 << Character.OTHER_LETTER) | (1 << Character.DECIMAL_DIGIT_NUMBER)) >> getType(
						codePoint))
				& 1) != 0;
	}
	...
	public static boolean isAlphabetic(int codePoint) {
		return (((((1 << Character.UPPERCASE_LETTER) | (1 << Character.LOWERCASE_LETTER)
				| (1 << Character.TITLECASE_LETTER) | (1 << Character.MODIFIER_LETTER)
				| (1 << Character.OTHER_LETTER) | (1 << Character.LETTER_NUMBER)) >> getType(
						codePoint))
				& 1) != 0) || CharacterData.of(codePoint).isOtherAlphabetic(codePoint);
	}

	/**
	 * Determines if the specified character (Unicode code point) is a CJKV
	 * (Chinese, Japanese, Korean and Vietnamese) ideograph, as defined by the
	 * Unicode Standard.
	 *
	 * @param codePoint
	 *            the character (Unicode code point) to be tested.
	 ...
	 * <a href="#supplementary"> supplementary characters</a>. To support all
	 * Unicode characters, including supplementary characters, use the
	 * {@link #isJavaIdentifierStart(int)} method.
	 *
	 * @param ch
	 *            the character to be tested.
	 * @return {@code true} if the character may start a Java identifier;
	...
	/**
	 * Determines if the character (Unicode code point) is permissible as the
	 * first character in a Java identifier.
	 * <p>
	 * A character may start a Java identifier if and only if one of the
	 * following conditions is true:
	 * <ul>
	 * <li>{@link #isLetter(int) isLetter(codePoint)} returns {@code true}
	 * <li>{@link #getType(int) getType(codePoint)} returns
	 * {@code LETTER_NUMBER}
	 * <li>the referenced character is a currency symbol (such as {@code '$'})
	 * <li>the referenced character is a connecting punctuation character (such
	 * as {@code '_'}).
	 * </ul>
	 *
	 * @param codePoint
	 ...
	 * <a href="#supplementary"> supplementary characters</a>. To support all
	 * Unicode characters, including supplementary characters, use the
	 * {@link #isJavaIdentifierPart(int)} method.
	 *
	 * @param ch
	 *            the character to be tested.
	 * @return {@code true} if the character may be part of a Java identifier;
	 ...
	 * <li>it is a combining mark
	 * <li>it is a non-spacing mark
	 * <li>{@link #isIdentifierIgnorable(int) isIdentifierIgnorable(codePoint)}
	 * returns {@code true} for the character
	 * </ul>
	 *
	 * @param codePoint
	 ...
	 * <p>
	 * A character may start a Unicode identifier if and only if one of the
	 * following conditions is true:
	 * <ul>
	 * <li>{@link #isLetter(char) isLetter(ch)} returns {@code true}
	 * <li>{@link #getType(char) getType(ch)} returns {@code LETTER_NUMBER}.
	 ...
	 * <a href="#supplementary"> supplementary characters</a>. To support all
	 * Unicode characters, including supplementary characters, use the
	 * {@link #isUnicodeIdentifierStart(int)} method.
	 *
	 * @param ch
	 *            the character to be tested.
	 * @return {@code true} if the character may start a Unicode identifier;
	 *         {@code false} otherwise.
	 * @see Character#isJavaIdentifierStart(char)
	 * @see Character#isLetter(char)
	 * @see Character#isUnicodeIdentifierPart(char)
	 ...
	 * <li>it is a combining mark
	 * <li>it is a non-spacing mark
	 * <li>{@code isIdentifierIgnorable} returns {@code true} for this
	 * character.
	 * </ul>
	 *
	 * <p>
	 ...
	 * <a href="#supplementary"> supplementary characters</a>. To support all
	 * Unicode characters, including supplementary characters, use the
	 * {@link #isUnicodeIdentifierPart(int)} method.
	 *
	 * @param ch
	 *            the character to be tested.
	 * @return {@code true} if the character may be part of a Unicode
	 ...
	 * </ul>
	 *
	 * <li>all characters that have the {@code FORMAT} general category value
	 * </ul>
	 *
	 * <p>
	 * <b>Note:</b> This method cannot handle
	 * <a href="#supplementary"> supplementary characters</a>. To support all
	 * Unicode characters, including supplementary characters, use the
	 * {@link #isIdentifierIgnorable(int)} method.
	 *
	 * @param ch
	 *            the character to be tested.
	 * @return {@code true} if the character is an ignorable control character
	 *         that may be part of a Java or Unicode identifier; {@code false}
	 *         otherwise.
	 * @see Character#isJavaIdentifierPart(char)
	 * @see Character#isUnicodeIdentifierPart(char)
	 * @since 1.1
	 ...
	 * <p>
	 * Note that {@code Character.isLowerCase(Character.toLowerCase(ch))} does
	 * not always return {@code true} for some ranges of characters,
	 * particularly those that are symbols or ideographs.
	 *
	 * <p>
	 * In general, {@link String#toLowerCase()} should be used to map characters
	 * to lowercase. {@code String} case mapping methods have several benefits
	 * over {@code Character} case mapping methods. {@code String} case mapping
	 * methods can perform locale-sensitive mappings, context-sensitive
	 * mappings, and 1:M character mappings, whereas the {@code Character} case
	 ...
	 *
	 * <p>
	 * Note that {@code Character.isLowerCase(Character.toLowerCase(codePoint))}
	 * does not always return {@code true} for some ranges of characters,
	 * particularly those that are symbols or ideographs.
	 *
	 * <p>
	 * In general, {@link String#toLowerCase()} should be used to map characters
	 ...
	 * @param codePoint
	 *            the character (Unicode code point) to be converted.
	 * @return the lowercase equivalent of the character (Unicode code point),
	 *         if any; otherwise, the character itself.
	 * @see Character#isLowerCase(int)
	 * @see String#toLowerCase()
	 *
	 * @since 1.5
	 ...
	 * information from the UnicodeData file.
	 * <p>
	 * Note that {@code Character.isUpperCase(Character.toUpperCase(ch))} does
	 * not always return {@code true} for some ranges of characters,
	 * particularly those that are symbols or ideographs.
	 *
	 * <p>
	 * In general, {@link String#toUpperCase()} should be used to map characters
	 * to uppercase. {@code String} case mapping methods have several benefits
	 * over {@code Character} case mapping methods. {@code String} case mapping
	 * methods can perform locale-sensitive mappings, context-sensitive
	 * mappings, and 1:M character mappings, whereas the {@code Character} case
	 * mapping methods cannot.
	 ...
	 *
	 * <p>
	 * Note that {@code Character.isUpperCase(Character.toUpperCase(codePoint))}
	 * does not always return {@code true} for some ranges of characters,
	 * particularly those that are symbols or ideographs.
	 *
	 * <p>
	 ...
	 * titlecase mapping. If the {@code char} argument is already a titlecase
	 * {@code char}, the same {@code char} value will be returned.
	 * <p>
	 * Note that {@code Character.isTitleCase(Character.toTitleCase(ch))} does
	 * not always return {@code true} for some ranges of characters.
	 *
	 * <p>
	 * <b>Note:</b> This method cannot handle
	 * <a href="#supplementary"> supplementary characters</a>. To support all
	 * Unicode characters, including supplementary characters, use the
	 * {@link #toTitleCase(int)} method.
	 *
	 * @param ch
	 *            the character to be converted.
	 * @return the titlecase equivalent of the character, if any; otherwise, the
	 *         character itself.
	 * @see Character#isTitleCase(char)
	 * @see Character#toLowerCase(char)
	 * @see Character#toUpperCase(char)
	 ...
	 * case mapping information from the UnicodeData file. If a character has no
	 * explicit titlecase mapping and is not itself a titlecase char according
	 * to UnicodeData, then the uppercase mapping is returned as an equivalent
	 * titlecase mapping. If the character argument is already a titlecase
	 * character, the same character value will be returned.
	 *
	 * <p>
	 * Note that {@code Character.isTitleCase(Character.toTitleCase(codePoint))}
	 * does not always return {@code true} for some ranges of characters.
	 *
	 * @param codePoint
	 *            the character (Unicode code point) to be converted.
	...
	/**
	 * Returns the numeric value of the character {@code ch} in the specified
	 * radix.
	 * <p>
	 * If the radix is not in the range {@code MIN_RADIX} &le; {@code radix}
	 * &le; {@code MAX_RADIX} or if the value of {@code ch} is not a valid digit
	 * in the specified radix, {@code -1} is returned. A character is a valid
	 * digit if at least one of the following is true:
	 * <ul>
	 * <li>The method {@code isDigit} is {@code true} of the character and the
	 * Unicode decimal digit value of the character (or its single-character
	 * decomposition) is less than the specified radix. In this case the decimal
	 * digit value is returned.
	 * <li>The character is one of the uppercase Latin letters {@code 'A'}
	 * through {@code 'Z'} and its code is less than {@code radix + 'A' - 10}.
	 * In this case, {@code ch - 'A' + 10} is returned.
	 * <li>The character is one of the lowercase Latin letters {@code 'a'}
	 * through {@code 'z'} and its code is less than {@code radix + 'a' - 10}.
	 * In this case, {@code ch - 'a' + 10} is returned.
	 * <li>The character is one of the fullwidth uppercase Latin letters A (
	 * {@code '\u005CuFF21'}) through Z ({@code '\u005CuFF3A'}) and its code is
	 * less than {@code radix + '\u005CuFF21' - 10}. In this case,
	 * {@code ch - '\u005CuFF21' + 10} is returned.
	 * <li>The character is one of the fullwidth lowercase Latin letters a (
	 * {@code '\u005CuFF41'}) through z ({@code '\u005CuFF5A'}) and its code is
	 * less than {@code radix + '\u005CuFF41' - 10}. In this case,
	 * {@code ch - '\u005CuFF41' + 10} is returned.
	 * </ul>
	 *
	 * <p>
	 * <b>Note:</b> This method cannot handle
	 ...
	 * <p>
	 * If the radix is not in the range {@code MIN_RADIX} &le; {@code radix}
	 * &le; {@code MAX_RADIX} or if the character is not a valid digit in the
	 * specified radix, {@code -1} is returned. A character is a valid digit if
	 * at least one of the following is true:
	 * <ul>
	 * <li>The method {@link #isDigit(int) isDigit(codePoint)} is {@code true}
	 * of the character and the Unicode decimal digit value of the character (or
	 * its single-character decomposition) is less than the specified radix. In
	 ...
	 * {@code '\u005CuFF21'}) through Z ({@code '\u005CuFF3A'}) and its code is
	 * less than {@code radix + '\u005CuFF21' - 10}. In this case,
	 * {@code codePoint - '\u005CuFF21' + 10} is returned.
	 * <li>The character is one of the fullwidth lowercase Latin letters a (
	 * {@code '\u005CuFF41'}) through z ({@code '\u005CuFF5A'}) and its code is
	 * less than {@code radix + '\u005CuFF41'- 10}. In this case,
	 * {@code codePoint - '\u005CuFF41' + 10} is returned.
	 * </ul>
	 *
	 * @param codePoint
	 *            the character (Unicode code point) to be converted.
	...
	/**
	 * Returns the {@code int} value that the specified Unicode character
	 * represents. For example, the character {@code '\u005Cu216C'} (the roman
	 * numeral fifty) will return an int with a value of 50.
	 * <p>
	 * The letters A-Z in their uppercase ({@code '\u005Cu0041'} through
	 * {@code '\u005Cu005A'}), lowercase ({@code '\u005Cu0061'} through
	 * {@code '\u005Cu007A'}), and full width variant ({@code '\u005CuFF21'}
	 * through {@code '\u005CuFF3A'} and {@code '\u005CuFF41'} through
	 * {@code '\u005CuFF5A'}) forms have numeric values from 10 through 35. This
	 * is independent of the Unicode specification, which does not assign
	 * numeric values to these {@code char} values.
	 * <p>
	 * If the character does not have a numeric value, then -1 is returned. If
	 * the character has a numeric value that cannot be represented as a
	 ...
	 * <a href="#supplementary"> supplementary characters</a>. To support all
	 * Unicode characters, including supplementary characters, use the
	 * {@link #getNumericValue(int)} method.
	 *
	 * @param ch
	 *            the character to be converted.
	...
	/**
	 * Returns the {@code int} value that the specified character (Unicode code
	 * point) represents. For example, the character {@code '\u005Cu216C'} (the
	 * Roman numeral fifty) will return an {@code int} with a value of 50.
	 * <p>
	 * The letters A-Z in their uppercase ({@code '\u005Cu0041'} through
	 * {@code '\u005Cu005A'}), lowercase ({@code '\u005Cu0061'} through
	 * {@code '\u005Cu007A'}), and full width variant ({@code '\u005CuFF21'}
	 ...
	 * method returns {@code true} for the following five characters only:
	 * <table summary="truechars">
	 * <tr>
	 * <td>{@code '\t'}</td>
	 * <td>{@code U+0009}</td>
	 * <td>{@code HORIZONTAL TABULATION}</td>
	 * </tr>
	 * <tr>
	 * <td>{@code '\n'}</td>
	 * <td>{@code U+000A}</td>
	 ...
	 * <td>{@code '\f'}</td>
	 * <td>{@code U+000C}</td>
	 * <td>{@code FORM FEED}</td>
	 * </tr>
	 * <tr>
	 * <td>{@code '\r'}</td>
	 * <td>{@code U+000D}</td>
	 * <td>{@code CARRIAGE RETURN}</td>
	 * </tr>
	 * <tr>
	 * <td>{@code ' '}</td>
	 * <td>{@code U+0020}</td>
	 ...
	 * @param ch
	 *            the character to be tested.
	 * @return {@code true} if the character is ISO-LATIN-1 white space;
	 *         {@code false} otherwise.
	...
	/**
	 * Determines if the specified character is a Unicode space character. A
	 * character is considered to be a space character if and only if it is
	 * specified to be a space character by the Unicode Standard. This method
	 * returns true if the character's general category type is any of the
	 * following:
	 * <ul>
	 ...
	 * <a href="#supplementary"> supplementary characters</a>. To support all
	 * Unicode characters, including supplementary characters, use the
	 * {@link #isSpaceChar(int)} method.
	 *
	 * @param ch
	 *            the character to be tested.
	 * @return {@code true} if the character is a space character; {@code false}
	 *         otherwise.
	 * @see Character#isWhitespace(char)
	 * @since 1.1
	 */
	...
	/**
	 * Determines if the specified character (Unicode code point) is a Unicode
	 * space character. A character is considered to be a space character if and
	 * only if it is specified to be a space character by the Unicode Standard.
	 * This method returns true if the character's general category type is any
	 * of the following:
	 *
	 * <ul>
	 ...
	 * character is a Java whitespace character if and only if it satisfies one
	 * of the following criteria:
	 * <ul>
	 * <li>It is a Unicode space character ({@code SPACE_SEPARATOR},
	 * {@code LINE_SEPARATOR}, or {@code PARAGRAPH_SEPARATOR}) but is not also a
	 ...
	 * @param ch
	 *            the character to be tested.
	 * @return {@code true} if the character is a Java whitespace character;
	 *         {@code false} otherwise.
	 * @see Character#isSpaceChar(char)
	 * @since 1.1
	 */
	 ...
	 * according to Java. A character is a Java whitespace character if and only
	 * if it satisfies one of the following criteria:
	 * <ul>
	 * <li>It is a Unicode space character ({@link #SPACE_SEPARATOR},
	 * {@link #LINE_SEPARATOR}, or {@link #PARAGRAPH_SEPARATOR}) but is not also
	 * a non-breaking space ({@code '\u005Cu00A0'}, {@code '\u005Cu2007'},
	 * {@code '\u005Cu202F'}).
	 * <li>It is {@code '\u005Ct'}, U+0009 HORIZONTAL TABULATION.
	 * <li>It is {@code '\u005Cn'}, U+000A LINE FEED.
	 * <li>It is {@code '\u005Cu000B'}, U+000B VERTICAL TABULATION.
	 ...
	 * the range {@code '\u005Cu0000'} through {@code '\u005Cu001F'} or in the
	 * range {@code '\u005Cu007F'} through {@code '\u005Cu009F'}.
	 *
	 * <p>
	 * <b>Note:</b> This method cannot handle
	 * <a href="#supplementary"> supplementary characters</a>. To support all
	 * Unicode characters, including supplementary characters, use the
	 * {@link #isISOControl(int)} method.
	 *
	 * @param ch
	 *            the character to be tested.
	 ...
	 * character if its code is in the range {@code '\u005Cu0000'} through
	 * {@code '\u005Cu001F'} or in the range {@code '\u005Cu007F'} through
	 * {@code '\u005Cu009F'}.
	 *
	 * @param codePoint
	 *            the character (Unicode code point) to be tested.
	 ...
	 * @param ch
	 *            the character to be tested.
	 * @return a value of type {@code int} representing the character's general
	 *         category.
	 ...
	 * specified radix. If the value of {@code radix} is not a valid radix, or
	 * the value of {@code digit} is not a valid digit in the specified radix,
	 * the null character ({@code '\u005Cu0000'}) is returned.
	 * <p>
	 * The {@code radix} argument is valid if it is greater than or equal to
	 * {@code MIN_RADIX} and less than or equal to {@code MAX_RADIX}. The
	 * {@code digit} argument is valid if {@code 0 <= digit < radix}.
	 * <p>
	 * If the digit is less than 10, then {@code '0' + digit} is returned.
	 * Otherwise, the value {@code 'a' + digit - 10} is returned.
	 *
	 * @param digit
	 *            the number to convert to a character.
	 * @param radix
	 *            the radix.
	 * @return the {@code char} representation of the specified digit in the
	 *         specified radix.
	 ...
	 * Character directionality is used to calculate the visual ordering of
	 * text. The directionality value of undefined {@code char} values is
	 ...
	 * <a href="#supplementary"> supplementary characters</a>. To support all
	 * Unicode characters, including supplementary characters, use the
	 * {@link #getDirectionality(int)} method.
	 *
	 * @param ch
	 *            {@code char} for which the directionality property is
	 *            requested.
	 ...
	 * visual ordering of text. The directionality value of undefined character
	 * is {@link #DIRECTIONALITY_UNDEFINED}.
	 *
	 * @param codePoint
	 *            the character (Unicode code point) for which the
	 ...
	 * {@code '\u005Cu0028'} LEFT PARENTHESIS is semantically defined to be an
	 * <i>opening parenthesis</i>. This will appear as a "(" in text that is
	 ...
	 * <a href="#supplementary"> supplementary characters</a>. To support all
	 * Unicode characters, including supplementary characters, use the
	 * {@link #isMirrored(int)} method.
	 *
	 * @param ch
	 *            {@code char} for which the mirrored property is requested
	 * @return {@code true} if the char is mirrored, {@code false} if the
	 *         {@code char} is not mirrored or is not defined.
	 * @since 1.4
	 */
	...
	/**
	 * Determines whether the specified character (Unicode code point) is
	 * mirrored according to the Unicode specification. Mirrored characters
	 * should have their glyphs horizontally mirrored when displayed in text
	 * that is right-to-left. For example, {@code '\u005Cu0028'} LEFT
	 * PARENTHESIS is semantically defined to be an <i>opening parenthesis</i>.
	 * This will appear as a "(" in text that is left-to-right but as a ")" in
	 * text that is right-to-left.
	 *
	 * @param codePoint
	 *            the character (Unicode code point) to be tested.
	 * @return {@code true} if the character is mirrored, {@code false} if the
	 *         character is not mirrored or is not defined.
	 * @since 1.5
	 */
	 ...
	 * 
	 * @return the value {@code 0} if the argument {@code Character} is equal to
	 *         this {@code Character}; a value less than {@code 0} if this
	 *         {@code Character} is numerically less than the {@code Character}
	 *         argument; and a value greater than {@code 0} if this
	 *         {@code Character} is numerically greater than the
	 *         {@code Character} argument (unsigned comparison). Note that this
	...
	/**
	 * Compares two {@code char} values numerically. The value returned is
	 ...
	 * @param codePoint
	 *            the character (Unicode code point) to be converted.
	 * @return either the uppercase equivalent of the character, if any, or an
	 *         error flag ({@code Character.ERROR}) that indicates that a 1:M
	 *         {@code char} mapping exists.
	 * @see Character#isLowerCase(char)
	 * @see Character#isUpperCase(char)
	 * @see Character#toLowerCase(char)
	 * @see Character#toTitleCase(char)
	...
	/**
	 * Converts the character (Unicode code point) argument to uppercase using
	 * case mapping information from the SpecialCasing file in the Unicode
	 * specification. If a character has no explicit uppercase mapping, then the
	 * {@code char} itself is returned in the {@code char[]}.
	 *
	 * @param codePoint
	 *            the character (Unicode code point) to be converted.
	 ...
	 *
	 * @param ch
	 *            The {@code char} of which to reverse the byte order.
	 * @return the value obtained by reversing (or, equivalently, swapping) the
	 *         bytes in the specified <tt>char</tt> value.
	 * @since 1.5
	 */
	...
	/**
	 * Returns the Unicode name of the specified character {@code codePoint}, or
	 * null if the code point is {@link #UNASSIGNED unassigned}.
	 * <p>
	 * Note: if the specified character is not assigned a name by the
	 * <i>UnicodeData</i> file (part of the Unicode Character Database
	 * maintained by the Unicode Consortium), the returned name is the same as
	 ...
	 *
	 * @return the Unicode name of the specified character, or null if the code
	 *         point is unassigned.
	 *
	 * @exception IllegalArgumentException
	 *                if the specified {@code codePoint} is not a valid Unicode
