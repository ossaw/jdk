...
public abstract class Reference<T> {
     ...
     * either Pending or Inactive, depending upon whether or not the instance
     ...
     * objects through the discovered field. The discovered field is also used
     * for linking Reference objects in the pending list.
     */
    ...
    /*
     * When active: NULL pending: this Enqueued: next reference in queue (or
     * this if last) Inactive: this
     */
     ...
     * (or this if last) pending: next element in the pending list (or null if
     * last) otherwise: NULL
     */
    transient private Reference<T> discovered; /* used by VM */

    /*
     * Object used to synchronize with the garbage collector. The collector must
     * acquire this lock at the beginning of each collection cycle. It is
    ...
    /*
     * List of References waiting to be enqueued. The collector adds References
     * to this list, while the Reference-handler thread removes them. This list
     * is protected by the above lock object. The list uses the discovered field
     * to link its elements.
     */
    ...
    private static class ReferenceHandler extends Thread {
        ...
        static {
            ...
            // so that we don't get into trouble later in the run loop if
            // there's
     ...
     * {@link Reference}s at the moment and the program can do some other useful
        ...
        try {
            synchronized (lock) {
                if (pending != null) {
                    ...
                    // so do this before un-linking 'r' from the 'pending'
                    // chain...
     ...
     * the program or by the garbage collector. If this reference object was not
     * registered with a queue when it was created, then this method will always
     * return <code>false</code>.
     *
     * @return <code>true</code> if and only if this reference object has been
     *         enqueued
     */
     ...
     *         enqueued; <code>false</code> if it was already enqueued or if it
