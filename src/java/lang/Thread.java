...
public class Thread implements Runnable {
     ...
     * debugging or testing purposes, where it may help to reproduce bugs due to
     * race conditions. It may also be useful when designing concurrency control
     ...
     *                                  if the value of {@code millis} is
     *                                  negative
     *
     ...
     * @throws IllegalArgumentException
     *                                  if the value of {@code millis} is
     *                                  negative, or the value of
     *                                  {@code nanos} is not in the range
     *                                  {@code 0-999999}
     *
     * @throws InterruptedException
     *                                  if any thread has interrupted the
     ...
     *                            if {@code true}, inherit initial values for
     *                            inheritable
     *                            thread-locals from the constructing thread
     */
    private void init(ThreadGroup g, Runnable target, String name,
     ...
     * name. Automatically generated names are of the form {@code "Thread-"+}
     * <i>n</i>, where <i>n</i> is an integer.
     */
    public Thread() {
     ...
     *               thread is started. If {@code null}, this classes
     *               {@code run}
     *               method does nothing.
     */
    ...
    /**
     * Creates a new Thread that inherits the given AccessControlContext. This
     ...
     *                           if the current thread cannot create a thread in
     *                           the specified
     *                           thread group
     */
    ...
    /**
     * Allocates a new {@code Thread} object so that it has {@code target} as
     * its run object, has the specified {@code name} as its name, and belongs
     ...
     * {@code RuntimePermission("enableContextClassLoaderOverride")} permission
     * when invoked directly or indirectly by the constructor of a subclass
     * which overrides the {@code getContextClassLoader} or
     ...
     * {@linkplain #setPriority setPriority} may be used to change the priority
     * to a new value.
     *
     * <p>
     * The newly created thread is initially marked as being a daemon thread if
     * and only if the thread creating it is currently marked as a daemon
     * thread. The method {@linkplain #setDaemon setDaemon} may be used to
     ...
     *                           if the current thread cannot create a thread in
     *                           the specified
     *                           thread group or cannot override the context
     *                           class loader
     *                           methods.
     */
     ...
     * Allocates a new {@code Thread} object so that it has {@code target} as
     * its run object, has the specified {@code name} as its name, and belongs
     * to the thread group referred to by {@code group}, and has the specified
     * <i>stack size</i>.
     *
     * <p>
     * This constructor is identical to
     * {@link #Thread(ThreadGroup,Runnable,String)} with the exception of the
     * fact that it allows the thread stack size to be specified. The stack size
     ...
     * throwing a {@link StackOverflowError}. Similarly, specifying a lower
     * value may allow a greater number of threads to exist concurrently without
     * throwing an {@link OutOfMemoryError} (or other internal error). The
     * details of the relationship between the value of the <tt>stackSize</tt>
     * parameter and the maximum recursion depth and concurrency level are
     ...
     * the virtual machine may instead use some platform-specific minimum value;
     * if the specified value is unreasonably high, the virtual machine may
     * instead use some platform-specific maximum. Likewise, the virtual machine
     * is free to round the specified value up or down as it sees fit (or to
     ...
     * stack size necessary to perform a given computation will likely vary from
     * one JRE implementation to another. In light of this variation, careful
     * tuning of the stack size parameter may be required, and the tuning may
     * need to be repeated for each JRE implementation on which an application
     * is to run.</i>
     *
     * <p>
     ...
     *
     * @param group
     *                  the thread group. If {@code null} and there is a
     *                  security
     *                  manager, the group is determined by
     *                  {@linkplain SecurityManager#getThreadGroup
     *                  SecurityManager.getThreadGroup()}. If there is not a
     ...
     * method is called with <code>this</code> as its argument. This may result
     * in a <code>SecurityException</code> being raised (in the current thread).
     * <p>
     * If this thread is different from the current thread (that is, the current
     * thread is trying to stop a thread other than itself), the security
     * manager's <code>checkPermission</code> method (with a
     * <code>RuntimePermission("stopThread")</code> argument) is called in
     * addition. Again, this may result in throwing a
     * <code>SecurityException</code> (in the current thread).
     * <p>
     * The thread represented by this thread is forced to stop whatever it is
     * doing abnormally and to throw a newly created <code>ThreadDeath</code>
     * object as an exception.
     * <p>
     * It is permitted to stop a thread that has not yet been started. If the
     * thread is eventually started, it immediately terminates.
     * <p>
     ...
     * throwing of <code>ThreadDeath</code> causes <code>finally</code> clauses
     * of <code>try</code> statements to be executed before the thread
     * officially dies). If a <code>catch</code> clause catches a
     * <code>ThreadDeath</code> object, it is important to rethrow the object so
     ...
     * uncaught exception is an instance of <code>ThreadDeath</code>.
     *
     ...
     *             were in an inconsistent state, the damaged objects become
     *             visible to other threads, potentially resulting in arbitrary
     *             behavior. Many uses of <code>stop</code> should be replaced
     *             by code that simply modifies some variable to indicate that
     *             the target thread should stop running. The target thread
     *             should check this variable regularly, and return from its run
     *             method in an orderly fashion if the variable indicates that
     *             it is to stop running. If the target thread waits for long
     *             periods (on a condition variable, for example), the
     *             <code>interrupt</code> method should be used to interrupt the
     *             wait. For more information, see <a href=
     ...
     *             furthermore could be used to generate exceptions that the
     *             target thread was not prepared to handle. For more
     ...
     * {@link java.nio.channels.InterruptibleChannel InterruptibleChannel} then
     * the channel will be closed, the thread's interrupt status will be set,
     * and the thread will receive a
     * {@link java.nio.channels.ClosedByInterruptException}.
     *
     * <p>
     * If this thread is blocked in a {@link java.nio.channels.Selector} then
     * the thread's interrupt status will be set and it will return immediately
     * from the selection operation, possibly with a non-zero value, just as if
     * the selector's {@link java.nio.channels.Selector#wakeup wakeup} method
     * were invoked.
     *
     * <p>
     ...
     *             locked. However, the method was never implemented. If if were
     *             to be implemented, it would be deadlock-prone in much the
     *             manner of {@link #suspend}. If the target thread held a lock
     *             protecting a critical system resource when it was destroyed,
     *             no thread could ever access this resource again. If another
     *             thread ever attempted to lock this resource, deadlock would
     *             result. Such deadlocks typically manifest themselves as
     *             "frozen" processes. For more information, see <a href=
     *             "{@docRoot}/../technotes/guides/concurrency/threadPrimitiveDeprecation.html"
     *             > Why are Thread.stop, Thread.suspend and Thread.resume
     *             Deprecated?</a>.
     ...
     *             monitor protecting a critical system resource when it is
     *             suspended, no thread can access this resource until the
     *             target thread is resumed. If the thread that would resume the
     *             target thread attempts to lock this monitor prior to calling
     *             <code>resume</code>, deadlock results. Such deadlocks
     ...
     *             which has been deprecated because it is deadlock-prone. For
     ...
     * and might be affected by the presence of certain system threads. This
     * method is intended primarily for debugging and monitoring purposes.
     *
     * @return an estimate of the number of active threads in the current
     *         thread's thread group and in any other thread group that has the
     *         current thread's thread group as an ancestor
     */
     ...
     * thread's thread group and its subgroups. This method simply invokes the
     * {@link java.lang.ThreadGroup#enumerate(Thread[])} method of the current
     * thread's thread group.
     *
     * <p>
     * An application might use the {@linkplain #activeCount activeCount} method
     * to get an estimate of how big the array should be, however <i>if the
     * array is too short to hold all the threads, the extra threads are
     * silently ignored.</i> If it is critical to obtain every active thread in
     * the current thread's thread group and its subgroups, the invoker should
     * verify that the returned int value is strictly less than the length of
     * {@code tarray}.
     *
     * <p>
     * Due to the inherent race condition in this method, it is recommended that
     * the method only be used for debugging and monitoring purposes.
     *
     * @param tarray
     ...
     *
     * @throws SecurityException
     *                           if {@link java.lang.ThreadGroup#checkAccess}
     *                           determines that
     *                           the current thread cannot access its thread
     *                           group
     */
     ...
     *             which is deprecated. Further, the results of this call were
     *             never well-defined.
     */
    @Deprecated
    ...
    /**
     * Waits at most {@code millis} milliseconds for this thread to die. A
     ...
     * method is invoked. It is recommended that applications not use
     * {@code wait}, {@code notify}, or {@code notifyAll} on {@code Thread}
     * instances.
     *
     * @param millis
     ...
     *
     * @throws SecurityException
     *                                     if {@link #checkAccess} determines
    ...
    public String toString() {
        ...
        if (group != null) {
            return "Thread[" + getName() + "," + getPriority() + "," + group
                    .getName() + "]";
        } else {
     ...
     * thread when loading classes and resources. If not
     * {@linkplain #setContextClassLoader set}, the default is the ClassLoader
     ...
     *
     * @throws SecurityException
     *                           if the current thread cannot get the context
     *                           ClassLoader
     *
     * @since 1.2
     */
     ...
     * provide the appropriate class loader, through
     * {@code getContextClassLoader}, to code running in the thread when loading
     ...
     *
     * @param cl
     *           the context ClassLoader for this Thread, or null indicating
     *           the system class loader (or, failing that, the bootstrap class
     *           loader)
     *
     * @throws SecurityException
     *                           if the current thread cannot set the context
     *                           ClassLoader
     *
     * @since 1.2
     */
     ...
     * has not started, has started but has not yet been scheduled to run by the
     * system, or has terminated. If the returned array is of non-zero length
     * then the first element of the array represents the top of the stack,
     * which is the most recent method invocation in the sequence. The last
     * element of the array represents the bottom of the stack, which is the
     * least recent method invocation in the sequence.
     *
     * <p>
     * If there is a security manager, and this thread is not the current
     * thread, then the security manager's <tt>checkPermission</tt> method is
     * called with a <tt>RuntimePermission("getStackTrace")</tt> permission to
     * see if it's ok to get the stack trace.
     *
     * <p>
     ...
     *                           method doesn't allow getting the stack trace of
     *                           thread.
     * @see SecurityManager#checkPermission
     ...
     * threads and each map value is an array of <tt>StackTraceElement</tt> that
     * represents the stack dump of the corresponding <tt>Thread</tt>. The
     * returned stack traces are in the format specified for the
     * {@link #getStackTrace getStackTrace} method.
     *
     * <p>
     * The threads may be executing while this method is called. The stack trace
     * of each thread only represents a snapshot and each stack trace may be
     * obtained at different time. A zero-length array will be returned in the
     * map value if the virtual machine has no stack trace information about a
     * thread.
     *
     * <p>
     ...
     * <tt>RuntimePermission("modifyThreadGroup")</tt> permission to see if it
     * is ok to get the stack trace of all threads.
     *
     ...
     * <tt>long</tt> number generated when this thread was created. The thread
     * ID is unique and remains unchanged during its lifetime. When a thread is
     ...
     * to a specified waiting time is in this state.</li>
     ...
     * are virtual machine states which do not reflect any operating system
     * thread states.
     *
     * @since 1.5
     * @see #getState
    ...
    public enum State {
         ...
         * thread in the blocked state is waiting for a monitor lock to enter a
         * synchronized block/method or reenter a synchronized block/method
         ...
         *
         * For example, a thread that has called <tt>Object.wait()</tt> on an
         * object is waiting for another thread to call <tt>Object.notify()</tt>
         * or <tt>Object.notifyAll()</tt> on that object. A thread that has
         * called <tt>Thread.join()</tt> is waiting for a specified thread to
         * terminate.
         */
        WAITING,
     ...
     * @throws SecurityException
     *                           if the current thread is not allowed to modify
     *                           this thread.
     * @see #setDefaultUncaughtExceptionHandler
     * @see ThreadGroup#uncaughtException
     * @since 1.5
    ...
    static class WeakClassKey extends WeakReference<Class<?>> {
        /**
         * saved value of the referent's identity hash code, to maintain a
         * consistent hash code after the referent has been cleared
         */
        private final int hash;

        /**
         * Create a new WeakClassKey to the given object, registered with a
         * queue.
         */
        ...
        /**
         * Returns true if the given object is this identical WeakClassKey
         * instance, or, if this object's referent has not been cleared, if the
         * given object is another WeakClassKey instance with the identical
         * non-null referent as this one.
         */
        @Override
        public boolean equals(Object obj) {
            ...
            if (obj instanceof WeakClassKey) {
                Object referent = get();
                return (referent != null) && (referent == ((WeakClassKey) obj)
                        .get());
            } else {
                return false;
            }
