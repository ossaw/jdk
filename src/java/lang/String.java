 ...
 * Character Representations</a> in the {@code Character} class for more
 * information). Index values refer to {@code char} code units, so a
 * supplementary character uses two positions in a {@code String}.
 * <p>
...
public final class String implements java.io.Serializable, Comparable<String>, CharSequence {
	 ...
	 * point of the subarray and the {@code count} argument specifies the length
	 * of the subarray. The contents of the subarray are converted to
	 ...
	 * {@code String}, and the {@code char} value at the following index is in
	 ...
	 * {@code (index -
	 * 2)} is in the high-surrogate range, then the supplementary code point
	 * value of the surrogate pair is returned. If the {@code char} value at
	 * {@code index -
	 * 1} is an unpaired low-surrogate or a high-surrogate, the surrogate value
	 * is returned.
	 *
	 * @param index
	 ...
	 * this {@code String}. The text range begins at the specified
	...
	/**
	 * Returns the index within this {@code String} that is offset from the
	 ...
	 *                {@code codePointOffset} is negative and the substring
	 ...
	 * character to be copied is at index {@code srcEnd-1} (thus the total
	 * number of characters to be copied is {@code srcEnd-srcBegin}). The
	 * characters are copied into the subarray of {@code dst} starting at index
	 * {@code dstBegin} and ending at index: <blockquote>
	 * 
	 * <pre>
	 ...
	 * {@code true} if and only if this {@code String} represents the same
	 ...
	 * represented by this {@code String} object is compared lexicographically
	 * to the character sequence represented by the argument string. The result
	 * is a negative integer if this {@code String} object lexicographically
	 * precedes the argument string. The result is a positive integer if this
	 * {@code String} object lexicographically follows the argument string. The
	 * result is zero if the strings are equal; {@code compareTo} returns
	 ...
	 * {@code compareTo} returns the difference of the two character values at
	 ...
	 * case, {@code compareTo} returns the difference of the lengths of the
	 ...
	 * the argument other. The result is true if these substrings represent
	 * identical character sequences. The substring of this {@code String}
	 * object to be compared begins at index {@code toffset} and has length
	 * {@code len}. The substring of other to be compared begins at index
	 * {@code ooffset} and has length {@code len}. The result is {@code false}
	 ...
	 * that: {@code this.charAt(toffset + }<i>k</i>
	 * {@code ) != other.charAt(ooffset + } <i>k</i>{@code )}
	 * </ul>
	 *
	 * @param toffset
	 ...
	 * 
	 * <pre>
	 * Character.toLowerCase(this.charAt(toffset + k)) != Character
	 * 		.toLowerCase(other.charAt(ooffset + k))
	 * </pre>
	 * 
	 * </blockquote> and: <blockquote>
	 * 
	 * <pre>
	 * Character.toUpperCase(this.charAt(toffset + k)) != Character
	 * 		.toUpperCase(other.charAt(ooffset + k))
	 * </pre>
	 * 
	 * </blockquote>
	 * </ul>
	 ...
	 * For values of {@code ch} in the range from 0 to 0xFFFF (inclusive), this
	 ...
	 * {@code fromIndex}, then the index of the first such occurrence is
	 * returned. For values of {@code ch} in the range from 0 to 0xFFFF
	 ...
	 * specified character. For values of {@code ch} in the range from 0 to
	 * 0xFFFF (inclusive), the index (in Unicode code units) returned is the
	 ...
	 * 
	 * </blockquote> is true. In either case, if no such character occurs in
	 * this string, then {@code -1} is returned. The {@code String} is searched
	 ...
	 * For values of {@code ch} in the range from 0 to 0xFFFF (inclusive), the
	 * index returned is the largest value <i>k</i> such that: <blockquote>
	 * 
	 * <pre>
	 * (this.charAt(<i>k</i>) == ch) {@code &&} (<i>k</i> &lt;= fromIndex)
	 * </pre>
	 ...
	 *            the value of {@code fromIndex}. If it is greater than or equal
	 *            to the length of this string, it has the same effect as if it
	 ...
	 * source is the character array being searched, and the target is the
	 ...
	 */
	static int indexOf(char[] source, int sourceOffset, int sourceCount, String target,
			int fromIndex) {
	 ...
	 *            the index to begin searching from.
	 */
	static int lastIndexOf(char[] source, int sourceOffset, int sourceCount, String target,
			int fromIndex) {
	 ...
	 * @exception IndexOutOfBoundsException
	 *                if {@code beginIndex} is negative or larger than the
	 ...
	 *
	 * @throws IndexOutOfBoundsException
	 *             if {@code beginIndex} or {@code endIndex} is negative, if
	...
	/**
	 * Converts all of the characters in this {@code String} to lower case using
	 * the rules of the given {@code Locale}. Case mapping is based on the
	 * Unicode Standard version specified by the {@link java.lang.Character
	 * Character} class. Since case mappings are not always 1:1 char mappings,
	 * the resulting {@code String} may be a different length than the original
	 * {@code String}.
	 * <p>
	 ...
	 * "doc-files/capchi.gif" alt="capchi"> <img src="doc-files/captheta.gif"
	 * alt="captheta"><img src="doc-files/capupsil.gif" alt="capupsil"> <img src
	 * ="doc-files/capsigma.gif" alt="capsigma"></td>
	 * <td><img src="doc-files/iota.gif" alt="iota"><img src="doc-files/chi.gif"
	 * alt="chi"> <img src="doc-files/theta.gif" alt="theta"><img src=
	 * "doc-files/upsilon.gif" alt="upsilon"> <img src="doc-files/sigma1.gif"
	 * alt="sigma"></td>
	 * <td>lowercased all chars in String</td>
	 * </tr>
	 * </table>
	...
	public String toLowerCase(Locale locale) {
		...
		int resultOffset = 0; /*
								 * result may grow, so i+resultOffset is the
								 * write location in result
								 */
		...
		for (int i = firstUpper; i < len; i += srcCount) {
			...
			if ((lowerChar == Character.ERROR)
				...
				if (lowerChar == Character.ERROR) {
					...
					resultOffset += Character.toChars(lowerChar, result, i + resultOffset)
							- srcCount;
					continue;
				} else {
	 ...
	 * keys, and HTML tags. For instance, {@code "TITLE".toLowerCase()} in a
	 * Turkish locale returns {@code "t\u005Cu0131tle"}, where '\u005Cu0131' is
	 * the LATIN SMALL LETTER DOTLESS I character. To obtain correct results for
	...
	/**
	 * Converts all of the characters in this {@code String} to upper case using
	 * the rules of the given {@code Locale}. Case mapping is based on the
	 * Unicode Standard version specified by the {@link java.lang.Character
	 * Character} class. Since case mappings are not always 1:1 char mappings,
	 * the resulting {@code String} may be a different length than the original
	...
	public String toUpperCase(Locale locale) {
		...
		for (int i = firstLower; i < len; i += srcCount) {
			...
			if ((upperChar == Character.ERROR)
				...
				if (upperChar == Character.ERROR) {
					if (localeDependent) {
						upperCharArray = ConditionalSpecialCasing.toUpperCaseCharArray(this, i,
								locale);
					} else {
						upperCharArray = Character.toUpperCaseCharArray(srcChar);
					}
				} else if (srcCount == 2) {
					resultOffset += Character.toChars(upperChar, result, i + resultOffset)
							- srcCount;
					continue;
				} else {
	 ...
	 * results if used for strings that are intended to be interpreted locale
	 * independently. Examples are programming language identifiers, protocol
	 * keys, and HTML tags. For instance, {@code "title".toUpperCase()} in a
	 * Turkish locale returns {@code "T\u005Cu0130TLE"}, where '\u005Cu0130' is
	 * the LATIN CAPITAL LETTER I WITH DOT ABOVE character. To obtain correct
	 * results for locale insensitive strings, use
	 ...
	 * <p>
	 * If this {@code String} object represents an empty character sequence, or
	 * the first and last characters of character sequence represented by this
	 * {@code String} object both have codes greater than {@code '\u005Cu0020'}
	 * (the space character), then a reference to this {@code String} object is
	 * returned.
	 * <p>
	 ...
	 * {@code '\u005Cu0020'}. A {@code String} object is returned, representing
	 * the substring of this string that begins with the character at index
	 * <i>k</i> and ends with the character at index <i>m</i>-that is, the
	 * result of {@code this.substring(k, m + 1)}.
	 * <p>
	 ...
	 *         trailing white space removed, or this string if it has no leading
	 ...
	 * subarray. The {@code count} argument specifies the length of the
	 ...
	 * @exception IndexOutOfBoundsException
	 *                if {@code offset} is negative, or {@code count} is
	 *                negative, or {@code offset+count} is larger than
	 *                {@code data.length}.
	 */
	 ...
	 * returned. Otherwise, this {@code String} object is added to the pool and
	 ...
	 * String literals are defined in section 3.10.5 of the <cite>The
