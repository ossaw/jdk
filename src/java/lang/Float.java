...
/**
 * The {@code Float} class wraps a value of primitive type {@code float} in an
 * object. An object of type {@code Float} contains a single field whose type is
 * {@code float}.
 *
 * <p>
 * In addition, this class provides several methods for converting a
 * {@code float} to a {@code String} and a {@code String} to a {@code float}, as
 * well as other constants and methods useful when dealing with a {@code float}.
 *
 * @author Lee Boynton
...
public final class Float extends Number implements Comparable<Float> {
    /**
     * A constant holding the positive infinity of type {@code float}. It is
     * equal to the value returned by {@code Float.intBitsToFloat(0x7f800000)}.
     */
    public static final float POSITIVE_INFINITY = 1.0f / 0.0f;

    /**
     * A constant holding the negative infinity of type {@code float}. It is
     * equal to the value returned by {@code Float.intBitsToFloat(0xff800000)}.
     */
    public static final float NEGATIVE_INFINITY = -1.0f / 0.0f;

    /**
     * A constant holding a Not-a-Number (NaN) value of type {@code float}. It
     ...
     * {@code float}, (2-2<sup>-23</sup>)&middot;2<sup>127</sup>. It is equal to
     * the hexadecimal floating-point literal {@code 0x1.fffffeP+127f} and also
     * equal to {@code Float.intBitsToFloat(0x7f7fffff)}.
     */
    public static final float MAX_VALUE = 0x1.fffffeP+127f; // 3.4028235e+38f

    /**
     * A constant holding the smallest positive normal value of type
     * {@code float}, 2<sup>-126</sup>. It is equal to the hexadecimal
     * floating-point literal {@code 0x1.0p-126f} and also equal to
     ...
     * {@code float}, 2<sup>-149</sup>. It is equal to the hexadecimal
     * floating-point literal {@code 0x0.000002P-126f} and also equal to
     * {@code Float.intBitsToFloat(0x1)}.
     */
    public static final float MIN_VALUE = 0x0.000002P-126f; // 1.4e-45f

    /**
     * Maximum exponent a finite {@code float} variable may have. It is equal to
    ...
    /**
     * Minimum exponent a normalized {@code float} variable may have. It is
     * equal to the value returned by {@code
    ...
    public static final Class<Float> TYPE = (Class<Float>) Class
            .getPrimitiveClass("float");

    /**
     * Returns a string representation of the {@code float} argument. All
     ...
     * first character of the result is '{@code -}' ({@code '\u005Cu002D'}); if
     * the sign is positive, no sign character appears in the result. As for the
     * magnitude <i>m</i>:
     * <ul>
     ...
     * {@code '\u005Cu002E'}), followed by one or more decimal digits
     * representing the fractional part of <i>m</i>.
     * <li>If <i>m</i> is less than 10<sup>-3</sup> or greater than or equal to
     * 10<sup>7</sup>, then it is represented in so-called
     * "computerized scientific notation." Let <i>n</i> be the unique integer
     * such that 10<sup><i>n</i> </sup>&le; <i>m</i> {@literal <} 10
     * <sup><i>n</i>+1</sup>; then let <i>a</i> be the mathematically exact
     * quotient of <i>m</i> and 10<sup><i>n</i></sup> so that 1 &le; <i>a</i>
     * {@literal <} 10. The magnitude is then represented as the integer part of
     * <i>a</i>, as a single decimal digit, followed by '{@code .}' (
     * {@code '\u005Cu002E'}), followed by decimal digits representing the
     ...
     * <i>a</i>? There must be at least one digit to represent the fractional
     * part, and beyond that as many, but only as many, more digits as are
     * needed to uniquely distinguish the argument value from adjacent values of
     * type {@code float}. That is, suppose that <i>x</i> is the exact
     * mathematical value represented by the decimal representation produced by
     * this method for a finite nonzero argument <i>f</i>. Then <i>f</i> must be
     * the {@code float} value nearest to <i>x</i>; or, if two {@code float}
     * values are equally close to <i>x</i>, then <i>f</i> must be one of them
     * and the least significant bit of the significand of <i>f</i> must be
     * {@code 0}.
     *
     * <p>
    ...
    /**
     * Returns a hexadecimal string representation of the {@code float}
     ...
     * {@code "0x0.0p0"}.
     *
     * <li>If <i>m</i> is a {@code float} value with a normalized
     ...
     * value.
     *
     * <li>If <i>m</i> is a {@code float} value with a subnormal representation,
     * the significand is represented by the characters {@code "0x0."} followed
     * by a hexadecimal representation of the rest of the significand as a
     * fraction. Trailing zeros in the hexadecimal representation are removed.
     * Next, the exponent is represented by {@code "p-126"}. Note that there
     ...
     * <tr>
     * <td>{@code Float.MIN_VALUE}</td>
     * <td>{@code 0x0.000002p-126}</td>
     * </table>
     * 
     * @param f
     *          the {@code float} to be converted.
     * @return a hex string representation of the argument.
     * @since 1.5
     * @author Joseph D. Darcy
     */
    public static String toHexString(float f) {
        if (Math.abs(f) < FloatConsts.MIN_NORMAL && f != 0.0f) {// float
                                                                // subnormal
    ...
    /**
     * Returns a {@code Float} object holding the {@code float} value
     ...
     * type {@code float} by the usual round-to-nearest rule of IEEE 754
     ...
     * ulp(MAX_VALUE)}/2), rounding to {@code float} will result in an infinity
     ...
     * string is converted directly to the target floating-point type. In
     * general, the two-step sequence of conversions, string to {@code double}
     * followed by {@code double} to {@code float}, is <em>not</em> equivalent
     * to converting a string directly to {@code float}. For example, if first
     * converted to an intermediate {@code double} and then to {@code float},
     * the string<br>
     * {@code "1.00000017881393421514957253748434595763683319091796875001d"}<br>
     * results in the {@code float} value {@code 1.0000002f}; if the string is
     * converted directly to {@code float}, <code>1.000000<b>1</b>f</code>
     * results.
     *
     * <p>
     * To avoid calling this method on an invalid string and having a
     * {@code NumberFormatException} be thrown, the documentation for
     * {@link Double#valueOf Double.valueOf} lists a regular expression which
     * can be used to screen the input.
     *
     * @param s
     *          the string to be parsed.
     * @return a {@code Float} object holding the value represented by the
    ...
    /**
     * Returns a {@code Float} instance representing the specified {@code float}
     * value. If a new {@code Float} instance is not required, this method
     * should generally be used in preference to the constructor
     * {@link #Float(float)}, as this method is likely to yield significantly
    ...
    /**
     * Returns a new {@code float} initialized to the value represented by the
    ...
    public boolean equals(Object obj) {
        return (obj instanceof Float) && (floatToIntBits(
