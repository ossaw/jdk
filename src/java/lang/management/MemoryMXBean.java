 ...
 * {@link javax.management.Notification notifications} if any one of the memory
 * pools supports a <a href="MemoryPoolMXBean.html#UsageThreshold">usage
 * threshold</a> or a
 ...
 * <ul>
 * <li>{@link MemoryNotificationInfo#MEMORY_THRESHOLD_EXCEEDED usage threshold
 * exceeded notification} - for notifying that the memory usage of a memory pool
 * is increased and has reached or exceeded its
 ...
 * collection usage threshold exceeded notification} - for notifying that the
 * memory usage of a memory pool is greater than or equal to its
 * <a href="MemoryPoolMXBean.html#CollectionThreshold"> collection usage
 ...
 * {@link CompositeData CompositeData} that represents a
 * {@link MemoryNotificationInfo} object containing information about the memory
...
public interface MemoryMXBean extends PlatformManagedObject {
	/**
	 * Returns the approximate number of objects for which finalization is
	 * pending.
	 *
	 ...
	 * allocation. The heap consists of one or more memory pools. The
	 * <tt>used</tt> and <tt>committed</tt> size of the returned memory usage is
	 * the sum of those values of all heap memory pools whereas the
	 * <tt>init</tt> and <tt>max</tt> size of the returned memory usage
	 * represents the setting of the heap memory which may not be the sum of
	 * those of all heap memory pools.
	 * <p>
	 ...
	 * pools. The <tt>used</tt> and <tt>committed</tt> size of the returned
	 * memory usage is the sum of those values of all non-heap memory pools
	 ...
	 * attributes as specified in {@link MemoryUsage#from MemoryUsage}.
	 *
	 * @return a {@link MemoryUsage} object representing the non-heap memory
	 *         usage.
	 */
	 ...
	 *
	 * @return <tt>true</tt> if verbose output for the memory system is enabled;
	 *         <tt>false</tt> otherwise.
	 */
	public boolean isVerbose();

	/**
	 * Enables or disables verbose output for the memory system. The verbose
	 * output information and the output stream to which the verbose information
	...
	/**
	 * Runs the garbage collector. The call <code>gc()</code> is effectively
