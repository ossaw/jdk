 ...
 * calling the {@link ManagementFactory#getThreadMXBean} method or from the
 ...
 * <blockquote> {@link ManagementFactory#THREAD_MXBEAN_NAME
 * <tt>java.lang:type=Threading</tt>} </blockquote>
 *
 * It can be obtained by calling the {@link PlatformManagedObject#getObjectName}
 * method.
 *
 * <h3>Thread ID</h3> Thread ID is a positive long value returned by calling the
 * {@link java.lang.Thread#getId} method for a thread. The thread ID is unique
 * during its lifetime. When a thread is terminated, this thread ID may be
 * reused.
 *
 * <p>
 ...
 *
 * <h3>Thread CPU time</h3> A Java virtual machine implementation may support
 * measuring the CPU time for the current thread, for any thread, or for no
 * threads.
 *
 * <p>
 * The {@link #isThreadCpuTimeSupported} method can be used to determine if a
 * Java virtual machine supports measuring of the CPU time for any thread. The
 * {@link #isCurrentThreadCpuTimeSupported} method can be used to determine if a
 * Java virtual machine supports measuring of the CPU time for the current
 * thread. A Java virtual machine implementation that supports CPU time
 * measurement for any thread will also support that for the current thread.
 *
 * <p>
 ...
 * {@link #isThreadCpuTimeEnabled} and {@link #setThreadCpuTimeEnabled} methods
 * can be used to test if CPU time measurement is enabled and to enable/disable
 * this support respectively. Enabling thread CPU measurement could be expensive
 * in some Java virtual machine implementations.
 *
 * <h3>Thread Contention Monitoring</h3> Some Java virtual machines may support
 * thread contention monitoring. When thread contention monitoring is enabled,
 * the accumulated elapsed time that the thread has blocked for synchronization
 * or waited for notification will be collected and returned in the
 ...
 *
 * <h3>Synchronization Information and Deadlock Detection</h3> Some Java virtual
 * machines may support monitoring of {@linkplain #isObjectMonitorUsageSupported
 * object monitor usage} and {@linkplain #isSynchronizerUsageSupported ownable
 * synchronizer usage}. The {@link #getThreadInfo(long[], boolean, boolean)} and
 * {@link #dumpAllThreads} methods can be used to obtain the thread stack trace
 * and synchronization information including which {@linkplain LockInfo
 * <i>lock</i>} a thread is blocked to acquire or waiting on and which locks the
 * thread currently owns.
 * <p>
 * The <tt>ThreadMXBean</tt> interface provides the
 * {@link #findMonitorDeadlockedThreads} and {@link #findDeadlockedThreads}
...
public interface ThreadMXBean extends PlatformManagedObject {
	 ...
	 * <blockquote> {@link #getThreadInfo(long, int) getThreadInfo(id,
	 * 0);} </blockquote>
	 *
	 * <p>
	 ...
	 * </blockquote>
	 *
	 * @return the total CPU time for the current thread if CPU time measurement
	...
	/**
	 * Returns the total CPU time for a thread of the specified ID in
	 * nanoseconds. The returned value is of nanoseconds precision but not
	 ...
	 *
	 * <p>
	 * If the thread of the specified ID is not alive or does not exist, this
	 * method returns <tt>-1</tt>. If CPU time measurement is disabled, this
	 * method returns <tt>-1</tt>. A thread is alive if it has been started and
	 * has not yet died.
	 * <p>
	 * If CPU time measurement is enabled after the thread has started, the Java
	 * virtual machine implementation may choose any time up to and including
	 * the time that the capability is enabled as the point where CPU time
	 * measurement starts.
	 *
	 * @param id
	...
	/**
	 * Returns the CPU time that a thread of the specified ID has executed in
	 * user mode in nanoseconds. The returned value is of nanoseconds precision
	 * but not necessarily nanoseconds accuracy.
	 *
	 * <p>
	 * If the thread of the specified ID is not alive or does not exist, this
	 * method returns <tt>-1</tt>. If CPU time measurement is disabled, this
	 ...
	 * measurement for any thread. A Java virtual machine implementation that
	 * supports CPU time measurement for any thread will also support CPU time
	 * measurement for the current thread.
	 *
	 * @return <tt>true</tt> if the Java virtual machine supports CPU time
	 *         measurement for any thread; <tt>false</tt> otherwise.
	 */
	public boolean isThreadCpuTimeSupported();

	/**
	 * Tests if the Java virtual machine supports CPU time measurement for the
	 * current thread. This method returns <tt>true</tt> if
	 * {@link #isThreadCpuTimeSupported} returns <tt>true</tt>.
	 *
	 * @return <tt>true</tt> if the Java virtual machine supports CPU time
	 *         measurement for current thread; <tt>false</tt> otherwise.
	 */
	 ...
	 * monitors. That is, threads that are blocked waiting to enter a
	 * synchronization block or waiting to reenter a synchronization block after
	 * an {@link Object#wait Object.wait} call, where each thread owns one
	 ...
	 * simplest case, thread A is blocked waiting for a monitor owned by thread
	 * B, and thread B is blocked waiting for a monitor owned by thread A.
	 * <p>
	 * This method is designed for troubleshooting use, but not for
	 * synchronization control. It might be an expensive operation.
	 * <p>
	 * This method finds deadlocks involving only object monitors. To find
	 * deadlocks involving both object monitors and
	 * <a href="LockInfo.html#OwnableSynchronizer">ownable synchronizers</a>,
	 * the {@link #findDeadlockedThreads findDeadlockedThreads} method should be
	 * used.
	 *
	...
	/**
	 * Finds cycles of threads that are in deadlock waiting to acquire object
	 * monitors or <a href="LockInfo.html#OwnableSynchronizer">ownable
	 * synchronizers</a>.
	 *
	 * Threads are <em>deadlocked</em> in a cycle waiting for a lock of these
	 * two types if each thread owns one lock while trying to acquire another
	 * lock already held by another thread in the cycle.
	 * <p>
	 * This method is designed for troubleshooting use, but not for
	 * synchronization control. It might be an expensive operation.
	 *
	 * @return an array of IDs of the threads that are deadlocked waiting for
	 *         object monitors or ownable synchronizers, if any; <tt>null</tt>
	 *         otherwise.
	 *
	 * @throws java.lang.SecurityException
	 *         if a security manager exists and the caller does not have
