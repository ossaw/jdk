...
/**
 * The {@code ManagementFactory} class is a factory class for getting managed
 * beans for the Java platform. This class consists of static methods each of
 * which returns one or more <i>platform MXBeans</i> representing the management
 * interface of a component of the Java virtual machine.
 *
 * <h3><a name="MXBean">Platform MXBeans</a></h3>
 * <p>
 * A platform MXBean is a <i>managed bean</i> that conforms to the
 * <a href="../../../javax/management/package-summary.html">JMX</a>
 * Instrumentation Specification and only uses a set of basic data types. A JMX
 * management application and the {@linkplain #getPlatformMBeanServer platform
 * MBeanServer} can interoperate without requiring classes for MXBean specific
 * data types. The data types being transmitted between the JMX connector server
 * and the connector client are {@linkplain javax.management.openmbean.OpenType
 * open types} and this allows interoperation across versions. See
 * <a href="../../../javax/management/MXBean.html#MXBean-spec"> the
 * specification of MXBeans</a> for details.
 *
 * <a name="MXBeanNames"></a>
 * <p>
 * Each platform MXBean is a {@link PlatformManagedObject} and it has a unique
 * {@link javax.management.ObjectName ObjectName} for registration in the
 * platform {@code MBeanServer} as returned by by the
 * {@link PlatformManagedObject#getObjectName getObjectName} method.
 *
 * <p>
 * An application can access a platform MXBean in the following ways:
 * <h4>1. Direct access to an MXBean interface</h4> <blockquote>
 * <ul>
 * <li>Get an MXBean instance by calling the {@link #getPlatformMXBean(Class)
 * getPlatformMXBean} or {@link #getPlatformMXBeans(Class) getPlatformMXBeans}
 * method and access the MXBean locally in the running virtual machine.</li>
 * <li>Construct an MXBean proxy instance that forwards the method calls to a
 * given {@link MBeanServer MBeanServer} by calling the
 * {@link #getPlatformMXBean(MBeanServerConnection, Class)} or
 * {@link #getPlatformMXBeans(MBeanServerConnection, Class)} method. The
 * {@link #newPlatformMXBeanProxy newPlatformMXBeanProxy} method can also be
 * used to construct an MXBean proxy instance of a given {@code ObjectName}. A
 * proxy is typically constructed to remotely access an MXBean of another
 * running virtual machine.</li>
 * </ul>
 * <h4>2. Indirect access to an MXBean interface via MBeanServer</h4>
 * <ul>
 * <li>Go through the platform {@code MBeanServer} to access MXBeans locally or
 * a specific <tt>MBeanServerConnection</tt> to access MXBeans remotely. The
 * attributes and operations of an MXBean use only <em>JMX open types</em> which
 * include basic data types, {@link javax.management.openmbean.CompositeData
 * CompositeData}, and {@link javax.management.openmbean.TabularData
 * TabularData} defined in {@link javax.management.openmbean.OpenType OpenType}.
 * The mapping is specified in the {@linkplain javax.management.MXBean MXBean}
 ...
 * method returns all management interfaces supported in the Java virtual
 * machine including the standard management interfaces listed in the tables
 * below as well as the management interfaces extended by the JDK
 * implementation.
 * <p>
 ...
 * <td>{@link #CLASS_LOADING_MXBEAN_NAME java.lang:type=ClassLoading}</td>
 * </tr>
 * <tr>
 * <td>{@link MemoryMXBean}</td>
 * <td>{@link #MEMORY_MXBEAN_NAME java.lang:type=Memory}</td>
 * </tr>
 * <tr>
 * <td>{@link ThreadMXBean}</td>
 * <td>{@link #THREAD_MXBEAN_NAME java.lang:type=Threading}</td>
 * </tr>
 * <tr>
 * <td>{@link RuntimeMXBean}</td>
 * <td>{@link #RUNTIME_MXBEAN_NAME java.lang:type=Runtime}</td>
 * </tr>
 * <tr>
 * <td>{@link OperatingSystemMXBean}</td>
 * <td>{@link #OPERATING_SYSTEM_MXBEAN_NAME java.lang:type=OperatingSystem}</td>
 * </tr>
 * <tr>
 ...
 * <td>{@link #COMPILATION_MXBEAN_NAME java.lang:type=Compilation}</td>
 * </tr>
 * </table>
 * </blockquote>
 *
 ...
 * management interfaces. <blockquote> <table border summary=
 * "The list of Management Interfaces and their single instances">
 * <tr>
 * <th>Management Interface</th>
 * <th>ObjectName</th>
 ...
 * <td>{@link #MEMORY_MANAGER_MXBEAN_DOMAIN_TYPE java.lang:type=MemoryManager}
 * <tt>,name=</tt><i>manager's name</i></td>
 * </tr>
 * <tr>
 * <td>{@link MemoryPoolMXBean}</td>
 * <td>{@link #MEMORY_POOL_MXBEAN_DOMAIN_TYPE java.lang:type=MemoryPool}
 * <tt>,name=</tt><i>pool's name</i></td>
 * </tr>
 * <tr>
...
public class ManagementFactory {
    ...
    /**
     * The domain name and the type key property in the <tt>ObjectName</tt> for
     * a {@link GarbageCollectorMXBean}. The unique <tt>ObjectName</tt> for a
     * <tt>GarbageCollectorMXBean</tt> can be formed by appending this string
     * with "<tt>,name=</tt><i>collector's name</i>".
     */
    public final static String GARBAGE_COLLECTOR_MXBEAN_DOMAIN_TYPE = "java.lang:type=GarbageCollector";

    /**
     * The domain name and the type key property in the <tt>ObjectName</tt> for
     * a {@link MemoryManagerMXBean}. The unique <tt>ObjectName</tt> for a
     * <tt>MemoryManagerMXBean</tt> can be formed by appending this string with
    ...
    /**
     * The domain name and the type key property in the <tt>ObjectName</tt> for
     * a {@link MemoryPoolMXBean}. The unique <tt>ObjectName</tt> for a
     * <tt>MemoryPoolMXBean</tt> can be formed by appending this string with
    ...
    /**
     * Returns the platform {@link javax.management.MBeanServer MBeanServer}. On
     ...
     * {@link MemoryPoolMXBean pools} and {@link MemoryManagerMXBean managers},
     * will automatically be registered and deregistered into the platform
     * {@code MBeanServer}.
     * <p>
     * If the system property {@code javax.management.builder.initial} is set,
     * the platform {@code MBeanServer} creation will be done by the specified
     * {@link javax.management.MBeanServerBuilder}.
     * <p>
     * It is recommended that this platform MBeanServer also be used to register
     * other application managed beans besides the platform MXBeans. This will
     * allow all MBeans to be published through the same {@code MBeanServer} and
     * hence allow for easier network publishing and discovery. Name conflicts
     * with the platform MXBeans should be avoided.
     *
     * @return the platform {@code MBeanServer}; the platform MXBeans are
     *         registered into the platform {@code MBeanServer} at the first
    ...
    public static synchronized MBeanServer getPlatformMBeanServer() {
        ...
        if (platformMBeanServer == null) {
            ...
            HashMap<ObjectName, DynamicMBean> dynmbeans = ManagementFactoryHelper
                    .getPlatformDynamicMBeans();
     ...
     * <a href="#MXBeanNames">MXBean name</a> that forwards its method calls
     * through the given <tt>MBeanServerConnection</tt>.
     *
     * <p>
     * This method is equivalent to:
     * <blockquote> {@link java.lang.reflect.Proxy#newProxyInstance
     * Proxy.newProxyInstance}<tt>(mxbeanInterface.getClassLoader(),
     *        new Class[] { mxbeanInterface }, handler)</tt> </blockquote>
     *
     ...
     * are dispatched. This <tt>handler</tt> converts an input parameter from an
     * MXBean data type to its mapped open type before forwarding to the
     * <tt>MBeanServer</tt> and converts a return value from an MXBean method
     * call through the <tt>MBeanServer</tt> from an open type to the
     * corresponding return type declared in the MXBean interface.
     *
     * <p>
     * If the MXBean is a notification emitter (i.e., it implements
     * {@link javax.management.NotificationEmitter NotificationEmitter}), both
     * the <tt>mxbeanInterface</tt> and <tt>NotificationEmitter</tt> will be
     * implemented by this proxy.
     *
     * <p>
     * <b>Notes:</b>
     * <ol>
     * <li>Using an MXBean proxy is a convenience remote access to a platform
     * MXBean of a running virtual machine. All method calls to the MXBean proxy
     * are forwarded to an <tt>MBeanServerConnection</tt> where
     * {@link java.io.IOException IOException} may be thrown when the
     * communication problem occurs with the connector server. An application
     * remotely accesses the platform MXBeans using proxy should prepare to
     * catch <tt>IOException</tt> as if accessing with the
     * <tt>MBeanServerConnector</tt> interface.</li>
     *
     * <li>When a client application is designed to remotely access MXBeans for
     * a running virtual machine whose version is different than the version on
     * which the application is running, it should prepare to catch
     * {@link java.io.InvalidObjectException InvalidObjectException} which is
     * thrown when an MXBean proxy receives a name of an enum constant which is
     * missing in the enum class loaded in the client application.</li>
     *
     ...
     * newProxyInstance} method cannot be used to create a proxy for a platform
     * MXBean. The proxy object created by <tt>MBeanServerInvocationHandler</tt>
     * does not handle the properties of the platform MXBeans described in the
     * <a href="#MXBean">class specification</a>.</li>
     * </ol>
     *
     * @param connection
     ...
     *         calls through the given <tt>MBeanServerConnection</tt>, or
     ...
     * {@code mxbeanInterface} which is specified to have one single instance in
     * the Java virtual machine. This method may return {@code null} if the
     * management interface is not implemented in the Java virtual machine (for
     * example, a Java virtual machine with no compilation system does not
     * implement {@link CompilationMXBean}); otherwise, this method is
     ...
     * {@code mxbeanInterface} in the Java virtual machine. The returned list
    ...
    /**
     * Returns the platform MXBean proxy for {@code mxbeanInterface} which is
     * specified to have one single instance in a Java virtual machine and the
     * proxy will forward the method calls through the given
     * {@code MBeanServerConnection}. This method may return {@code null} if the
     * management interface is not implemented in the Java virtual machine being
     * monitored (for example, a Java virtual machine with no compilation system
     * does not implement {@link CompilationMXBean}); otherwise, this method is
     * equivalent to calling:
     * 
     * <pre>
    ...
    /**
     * Returns the list of the platform MXBean proxies for forwarding the method
     * calls of the {@code mxbeanInterface} through the given
     * {@code MBeanServerConnection}. The returned list may contain zero, one,
     * or more instances. The number of instances in the returned list is
     * defined in the specification of the given management interface. The order
     * is undefined and there is no guarantee that the list returned is in the
     ...
     * {@link PlatformManagedObject}, representing all management interfaces for
     * monitoring and managing the Java platform.
     *
     * @return the set of {@code Class} objects, subinterface of
     *         {@link PlatformManagedObject} representing the management
