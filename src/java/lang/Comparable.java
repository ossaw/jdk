 ...
 * ordering</i>, and the class's <tt>compareTo</tt> method is referred to as its
 ...
 * a {@linkplain Comparator comparator}.
 * <p>
 *
 * The natural ordering for a class <tt>C</tt> is said to be <i>consistent with
 * equals</i> if and only if <tt>e1.compareTo(e2) == 0</tt> has the same boolean
 * value as <tt>e1.equals(e2)</tt> for every <tt>e1</tt> and <tt>e2</tt> of
 * class <tt>C</tt>. Note that <tt>null</tt> is not an instance of any class,
 * and <tt>e.compareTo(null)</tt> should throw a <tt>NullPointerException</tt>
 * even though <tt>e.equals(null)</tt> returns <tt>false</tt>.
 * <p>
 *
 ...
 * for set (or map), which is defined in terms of the <tt>equals</tt> method.
 * <p>
 *
 * For example, if one adds two keys <tt>a</tt> and <tt>b</tt> such that
 * {@code (!a.equals(b) && a.compareTo(b) == 0)} to a sorted set that does not
 * use an explicit comparator, the second <tt>add</tt> operation returns false
 * (and the size of the sorted set does not increase) because <tt>a</tt> and
 * <tt>b</tt> are equivalent from the sorted set's perspective.
 * <p>
 *
 * Virtually all Java core classes that implement <tt>Comparable</tt> have
 * natural orderings that are consistent with equals. One exception is
 * <tt>java.math.BigDecimal</tt>, whose natural ordering equates
 * <tt>BigDecimal</tt> objects with equal values and different precisions (such
 * as 4.0 and 4.00).
 * <p>
 *
 ...
 * quotient for the natural ordering is the equivalence relation defined by the
