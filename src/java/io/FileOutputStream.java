...
public class FileOutputStream extends OutputStream {
	...
	/**
	 * Creates a file output stream to write to the file with the specified
	 * name. If the second argument is <code>true</code>, then bytes will be
	 * written to the end of the file rather than the beginning. A new
	 ...
	 * the returned channel will be equal to the number of bytes written to the
	 * file so far unless this stream is in append mode, in which case it will
	 * be equal to the size of the file. Writing bytes to this stream will
	 * increment the channel's position accordingly. Changing the channel's
	 * position, either explicitly or by writing, will change this stream's file
	 * position.
	 *
	...
	protected void finalize() throws IOException {
		if (fd != null) {
			if (fd == FileDescriptor.out || fd == FileDescriptor.err) {
				 ...
				 * that finalizer is only called when safe to do so. All
				 * references using the fd have become unreachable. We can call
				 * close()
				 */
				close();
			}
