...
public class FileInputStream extends InputStream {
     ...
     * thrown.
     * <p>
     * This constructor does not throw an exception if <code>fdObj</code> is
     * {@link java.io.FileDescriptor#valid() invalid}. However, if the methods
     * are invoked on the resulting stream to attempt I/O on the stream, an
    ...
    public FileInputStream(FileDescriptor fdObj) {
        ...
        /*
         * FileDescriptor is being shared by streams. Register this stream with
         * FileDescriptor tracker.
         */
        fd.attach(this);
    }
     ...
     * position, an <code>IOException</code> is thrown. The actual number of
     * bytes skipped is returned. If it skips forwards, it returns a positive
     * value. If it skips backwards, it returns a negative value.
     *
     * <p>
     * This method may skip more bytes than what are remaining in the backing
     * file. This produces no exception and the number of bytes skipped may
     * include some number of bytes that were beyond the EOF of the backing
     ...
     *
     * <p>
     * The initial {@link java.nio.channels.FileChannel#position() position} of
     * the returned channel will be equal to the number of bytes read from the
     * file so far. Reading bytes from this stream will increment the channel's
    ...
    protected void finalize() throws IOException {
        if ((fd != null) && (fd != FileDescriptor.in)) {
            /*
             * if fd is shared, the references in FileDescriptor will ensure
             * that finalizer is only called when safe to do so. All references
             * using the fd have become unreachable. We can call close()
             */
            close();
        }
    }
