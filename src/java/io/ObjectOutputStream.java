 ...
 * from the corresponding ObjectInputstream with the same types and in the same
 * order as they were written.
 * <p>
 * Primitive data types can also be written to the stream using the appropriate
 * methods from DataOutput. Strings can also be written using the writeUTF
 * method.
 * <p>
 ...
 * individual fields to the ObjectOutputStream using the writeObject method or
 ...
 * readObject methods that throw the NotSerializableException. The exception
 * will be caught by the ObjectOutputStream and abort the serialization process.
 * <p>
 ...
 * appearing subsequently in the serialization stream. The process by which enum
 * constants are serialized cannot be customized; any class-specific writeObject
 * and writeReplace methods defined by enum types are ignored during
 * serialization. Similarly, any serialPersistentFields or serialVersionUID
 * field declarations are also ignored--all enum types have a fixed
 * serialVersionUID of 0L.
 * <p>
 * Primitive data, excluding serializable fields and externalizable data, is
 * written to the ObjectOutputStream in block-data records. A block data record
 * is composed of a header and data. The block data header consists of a marker
 * and the number of bytes to follow the header. Consecutive primitive data
 * writes are merged into one block-data record. The blocking factor used for a
 * block-data record will be 1024 bytes. Each block-data record will be filled
 * up to 1024 bytes, or be written whenever there is a termination of block-data
 * mode. Calls to the ObjectOutputStream methods writeObject, defaultWriteObject
 * and writeFields initially terminate any existing block-data record.
 *
 * @author Mike Warres
 * @author Roger Riggs
 ...
 * @see <a href="../../../platform/serialization/spec/output.html">Object
...
public class ObjectOutputStream extends OutputStream implements ObjectOutput, ObjectStreamConstants {
     ...
     * currently being serialized and descriptor for current class. Null when
     ...
     * <p>
     * Every effort will be made to avoid introducing additional backwards
     ...
     * and the readObject methods. Objects referenced by this object are written
     * transitively so that a complete equivalent graph of objects can be
     * reconstructed by an ObjectInputStream.
     * <p>
     * Exceptions are thrown for problems with the OutputStream and for classes
     * that should not be serialized. All exceptions are fatal to the
     ...
     * <ul>
     * <li>An object written via writeUnshared is always serialized in the same
     * manner as a newly appearing object (an object that has not been written
     * to the stream yet), regardless of whether or not the object has been
     * written previously.
     ...
     * the stream. By default this method does nothing. The corresponding method
     ...
     * in the stream. The default implementation of this method in
     * <code>ObjectOutputStream</code> does nothing.
     * <p>
     * The corresponding method in <code>ObjectInputStream</code> is
     ...
     * method checks that the stream requesting to do replacement can be
     * trusted. The first occurrence of each object written into the
     * serialization stream is passed to replaceObject. Subsequent references to
     * the object are replaced by the object returned by the original call to
     * replaceObject. To ensure that the private state of objects is not
     * unintentionally exposed, only trusted streams may use replaceObject.
     * <p>
     ...
     * the substituted object is compatible with every field where the reference
     ...
     * object. This method should return the object to be substituted or the
     * original object.
     * <p>
     * Null can be returned as the object to be substituted, but may cause
     * NullReferenceException in classes that contain references to the original
     * object since they may be expecting an object instead of null.
     *
     * @param obj
     ...
     * serialized.
     * <p>
     * If <code>enable</code> is true, and there is a security manager
     ...
     * <a href="DataInput.html#modified-utf-8">modified UTF-8</a> format. Note
     * that there is a significant difference between writing a String into the
    ...
    public static abstract class PutField {
        ...
        /**
         * Write the data and fields to the specified ObjectOutput stream, which
         * must be the same stream that produced this <code>PutField</code>
         * object.
         *
         * @param out
         ...
         *             calling the
         *             {@link java.io.ObjectOutputStream#writeFields()} method.
         */
        @Deprecated
    ...
    private void writeOrdinaryObject(Object obj, ObjectStreamClass desc, boolean unshared)
            throws IOException {
        if (extendedDebugInfo) {
            debugInfoStack.push((depth == 1 ? "root " : "") + "object (class \"" + obj.getClass().getName()
                    + "\", " + obj.toString() + ")");
        }
        try {
            desc.checkSerialize();
    ...
    private void defaultWriteFields(Object obj, ObjectStreamClass desc) throws IOException {
        ...
        for (int i = 0; i < objVals.length; i++) {
            if (extendedDebugInfo) {
                debugInfoStack.push("field (class \"" + desc.getName() + "\", name: \"" + fields[numPrimFields
                        + i].getName() + "\", type: \"" + fields[numPrimFields + i].getType() + "\")");
            }
            try {
    ...
    private class PutFieldImpl extends PutField {
        ...
        /**
         * Creates PutFieldImpl object for writing fields defined in given class
         * descriptor.
         */
        ...
        public void write(ObjectOutput out) throws IOException {
            /*
             * Applications should *not* use this method to write PutField data,
             * as it will lead to stream corruption if the PutField object
             * writes any primitive data (since block data mode is not unset/set
             * properly, as is done in OOS.writeFields()). This broken
            ...
            for (int i = 0; i < objVals.length; i++) {
                if (extendedDebugInfo) {
                    debugInfoStack.push("field (class \"" + desc.getName() + "\", name: \""
                            + fields[numPrimFields + i].getName() + "\", type: \"" + fields[numPrimFields + i]
                                    .getType() + "\")");
                }
                try {
                    writeObject0(objVals[i], fields[numPrimFields + i].isUnshared());
         ...
         * and any other non-null type matches assignable types only. Throws
    ...
    private static class BlockDataOutputStream extends OutputStream implements DataOutput {
         ...
         * old mode, no action is taken. If the new mode differs from the old
         * mode, any buffered data is flushed before switching to the new mode.
         */
         ...
         * true, copies the values to an intermediate buffer before writing them
         ...
         * prefixed with a 2-byte header; all others start with a 5-byte header.
         */
         ...
         * methods repeatedly, these methods are optimized for writing groups of
    ...
    private static class ReplaceTable {
        ...
        /**
         * Looks up and returns replacement for given object. If no replacement
         * is found, returns the lookup object itself.
         */
    ...
    /**
     * Stack to keep debug information about the state of the serialization
     * process, for embedding in exception messages.
     */
