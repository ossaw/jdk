 ...
 * serializable class closest to java.lang.object and finishing with the
 ...
 *
 * <p>
 * Implementing the Serializable interface allows object serialization to save
 * and restore the entire state of the object and it allows classes to evolve
 * between the time the stream is written and the time it is read. It
 ...
 * the methods of ObjectOutput and ObjectInput. It is the responsibility of the
 * objects to handle any versioning that occurs.
 *
 * <p>
 ...
 * method <code>Enum.valueOf(Class, String)</code> with the enum constant's base
 * type and the received constant name as arguments. Like other serializable or
 * externalizable objects, enum constants can function as the targets of back
 * references appearing subsequently in the serialization stream. The process by
 * which enum constants are deserialized cannot be customized: any
 * class-specific readObject, readObjectNoData, and readResolve methods defined
 * by enum types are ignored during deserialization. Similarly, any
 ...
 * @see <a href="../../../platform/serialization/spec/input.html"> Object
 *      Serialization Specification, Section 3, Object Input Classes</a>
 * @since JDK1.1
 */
public class ObjectInputStream extends InputStream implements ObjectInput,
	...
	static {
		...
		sun.misc.SharedSecrets.setJavaOISAccess(new JavaOISAccess() {
			public void setObjectInputFilter(ObjectInputStream stream,
	...
	private static class Logging {
		...
		/*
		 * Logger for ObjectInputFilter results. Setup the filter logger if it
		 * is set to INFO or WARNING. (Assuming it will not change).
		 */
		private static final PlatformLogger traceLogger;
		private static final PlatformLogger infoLogger;
		static {
			PlatformLogger filterLog = PlatformLogger.getLogger(
					"java.io.serialization");
			infoLogger = (filterLog != null && filterLog.isLoggable(
					PlatformLogger.Level.INFO)) ? filterLog : null;
			traceLogger = (filterLog != null && filterLog.isLoggable(
					PlatformLogger.Level.FINER)) ? filterLog : null;
		}
	}
	 ...
	 *                                  if untrusted subclass illegally
	 *                                  overrides security-sensitive
	 *                                  methods
	 ...
	 *                                instead of objects.
	 * @throws IOException
	 *                                if I/O errors occurred while reading from
	 *                                the underlying
	 *                                stream
	 ...
	 * <ul>
	 * <li>If readUnshared is called to deserialize a back-reference (the stream
	 * representation of an object which has been written previously to the
	 * stream), an ObjectStreamException will be thrown.
	 *
	 * <li>If readUnshared returns successfully, then any subsequent attempts to
	 ...
	 * invocation of readObject or readUnshared on the ObjectInputStream, even
	 * if the underlying data stream has been manipulated.
	 *
	 * <p>
	 ...
	 * @param prio
	 *             controls the order of callbacks;zero is a good default. Use
	 *             higher numbers to be called back earlier, lower numbers for
	 *             later callbacks. Within a priority, callbacks are processed
	 *             in
	 *             no particular order.
	 * @throws NotActiveException
	 *                                The stream is not currently reading
	 *                                objects so it is invalid
	 ...
	 * description. Subclasses may implement this method to allow classes to be
	 ...
	 * </pre>
	 * 
	 * where <code>loader</code> is determined as follows: if there is a method
	 * on the current thread's stack whose declaring class was defined by a
	 * user-defined class loader (and was not a generated to implement
	 * reflective invocations), then <code>loader</code> is class loader
	 * corresponding to the closest such method to the currently executing
	 * frame; otherwise, <code>loader</code> is <code>null</code>. If this call
	 * results in a <code>ClassNotFoundException</code> and the name of the
	 * passed <code>ObjectStreamClass</code> instance is the Java language
	 * keyword for a primitive type or void, then the <code>Class</code> object
	 * representing that primitive type or void will be returned (e.g., an
	 * <code>ObjectStreamClass</code> with the name <code>"int"</code> will be
	 * resolved to <code>Integer.TYPE</code>). Otherwise, the
	 * <code>ClassNotFoundException</code> will be thrown to the caller of this
	 * method.
	 *
	 * @param desc
	 ...
	 *
	 * <p>
	 * This method is called exactly once for each unique proxy class descriptor
	 * in the stream.
	 *
	 * <p>
	 * The corresponding method in <code>ObjectOutputStream</code> is
	 ...
	 * class loader up the execution stack, or <code>null</code> if no non-
	 * <code>null</code> class loaders are on the stack (the same class loader
	 * choice used by the <code>resolveClass</code> method). Unless any of the
	 * resolved interfaces are non-public, this same value of
	 * <code>loader</code> is also the class loader passed to
	 * <code>Proxy.getProxyClass</code>; if non-public interfaces are present,
	 * their class loader is passed instead (if more than one non-public
	 * interface class loader is encountered, an <code>IllegalAccessError</code>
	 * is thrown). If <code>Proxy.getProxyClass</code> throws an
	 ...
	 *
	 * <p>
	 * When a subclass is replacing objects it must insure that the substituted
	 ...
	 * All subsequent references to the object will be redirected to the new
	 * object.
	 *
	 * @param obj
	 ...
	 *
	 * <p>
	 * If <i>enable</i> is true, and there is a security manager installed, this
	 * method first calls the security manager's <code>checkPermission</code>
	 * method with the <code>SerializablePermission("enableSubstitution")</code>
	 * permission to ensure it's ok to enable the stream to allow objects read
	 ...
	 *                           <code>checkPermission</code> method denies
	 *                           enabling the
	 *                           stream to allow objects read from the stream to
	 *                           be replaced.
	 * @see SecurityManager#checkPermission
	 * @see java.io.SerializablePermission
	 */
	 ...
	 *
	 * @throws IOException
	 *                                  if there are I/O errors while reading
	 *                                  from the underlying
	 *                                  <code>InputStream</code>
	 * @throws StreamCorruptedException
	 *                                  if control information in the stream is
	...
	protected void readStreamHeader() throws IOException,
		...
		if (s0 != STREAM_MAGIC || s1 != STREAM_VERSION) {
			throw new StreamCorruptedException(String.format(
					"invalid stream header: %04X%04X", s0, s1));
		}
	}
	 ...
	 * available. Consider using java.io.DataInputStream.readFully to read
	...
	/**
	 * Reads a String in <a href="DataInput.html#modified-utf-8">modified
	 * UTF-8</a> format.
	 *
	 * @return the String.
	 * @throws IOException
	...
	/**
	 * Returns the serialization filter for this stream. The serialization
	 * filter is the most recent filter set in
	 * {@link #setInternalObjectInputFilter setInternalObjectInputFilter} or the
	 * initial process-wide filter from
	...
	/**
	 * Set the serialization filter for the stream. The filter's
	 * {@link ObjectInputFilter#checkInput checkInput} method is called for each
	 * class and reference in the stream. The filter can check any or all of the
	 * class, the array length, the number of references, the depth of the
	 * graph, and the size of the input stream.
	 * <p>
	 * If the filter returns {@link ObjectInputFilter.Status#REJECTED
	 * Status.REJECTED}, {@code null} or throws a {@link RuntimeException}, the
	 * active {@code readObject} or {@code readUnshared} throws
	 * {@link InvalidClassException}, otherwise deserialization continues
	 * uninterrupted.
	 * <p>
	 * The serialization filter is initialized to the value of
	 * {@link ObjectInputFilter.Config#getSerialFilter()
	 * ObjectInputFilter.Config.getSerialFilter} when the
	 * {@code  ObjectInputStream} is constructed and can be set to a custom
	 * filter only once.
	 *
	 * @implSpec The filter, when not {@code null}, is invoked during
	 *           {@link #readObject readObject} and {@link #readUnshared
	 *           readUnshared} for each object (regular or class) in the stream
	 *           including the following:
	 *           <ul>
	 *           <li>each object reference previously deserialized from the
	 *           stream (class is {@code null}, arrayLength is -1),
	 *           <li>each regular class (class is not {@code null}, arrayLength
	 *           is -1),
	 *           <li>each interface of a dynamic proxy and the dynamic proxy
	 *           class itself (class is not {@code null}, arrayLength is -1),
	 *           <li>each array is filtered using the array type and length of
	 *           the array (class is the array type, arrayLength is the
	 *           requested length),
	 *           <li>each object replaced by its class' {@code readResolve}
	 *           method is filtered using the replacement object's class, if not
	 *           {@code null}, and if it is an array, the arrayLength, otherwise
	 *           -1,
	 *           <li>and each object replaced by {@link #resolveObject
	 *           resolveObject} is filtered using the replacement object's
	 *           class, if not {@code null}, and if it is an array, the
	 *           arrayLength, otherwise -1.
	 *           </ul>
	 *
	 *           When the {@link ObjectInputFilter#checkInput checkInput} method
	 *           is invoked it is given access to the current class, the array
	 *           length, the current number of references already read from the
	 *           stream, the depth of nested calls to {@link #readObject
	 *           readObject} or {@link #readUnshared readUnshared}, and the
	 *           implementation dependent number of bytes consumed from the
	 *           input stream.
	 *           <p>
	 ...
	 *           normally or exceptionally. The depth starts at {@code 1} and
	 *           increases for each nested object and decrements when each
	 *           nested call returns. The count of references in the stream
	 *           starts at {@code 1} and is increased before reading an object.
	 *
	 * @param filter
	 ...
	 *                               if the
	 *                               {@linkplain #getInternalObjectInputFilter()
	 *                               current
	 *                               filter} is not {@code null} and is not the
	 *                               process-wide
	 *                               filter
	 */
	private final void setInternalObjectInputFilter(ObjectInputFilter filter) {
		...
		if (serialFilter != null && serialFilter != ObjectInputFilter.Config
				.getSerialFilter()) {
	...
	/**
	 * Invoke the serialization filter if non-null. If the filter rejects or an
	 * exception is thrown, throws InvalidClassException.
	 *
	 * @param clazz
	 *                    the class; may be null
	 * @param arrayLength
	 *                    the array length requested; use {@code -1} if not
	 *                    creating an
	 *                    array
	 * @throws InvalidClassException
	 *                               if it rejected by the filter or a
	 *                               {@link RuntimeException} is
	 *                               thrown
	 */
	private void filterCheck(Class<?> clazz, int arrayLength)
		...
		if (serialFilter != null) {
			...
			try {
				status = serialFilter.checkInput(new FilterValues(clazz,
						arrayLength, totalObjectRefs, depth, bin
								.getBytesRead()));
			...
			if (status == null || status == ObjectInputFilter.Status.REJECTED) {
				...
				if (Logging.infoLogger != null) {
					Logging.infoLogger.info(
							...
							bin.getBytesRead(), Objects.toString(ex, "n/a"));
				}
	...
	public static abstract class GetField {
		 ...
		 * @throws IOException
		 *                                  if there are I/O errors while
		 *                                  reading from the underlying
		 *                                  <code>InputStream</code>
		 * @throws IllegalArgumentException
		 *                                  if <code>name</code> does not
		 *                                  correspond to a
		 *                                  serializable field
		 */
		 ...
		 *             the name of the field
		 * @param val
		 *             the default value to use if <code>name</code> does not
		 *             have a value
		 ...
		 *                                  <code>InputStream</code>
		 * @throws IllegalArgumentException
		 *                                  if type of <code>name</code> is not
		 *                                  serializable or if
	...
	private static boolean auditSubclass(final Class<?> subcl) {
		Boolean result = AccessController.doPrivileged(
				new PrivilegedAction<Boolean>() {
					public Boolean run() {
						for (Class<?> cl = subcl; cl != ObjectInputStream.class; cl = cl
								.getSuperclass()) {
							try {
								cl.getDeclaredMethod("readUnshared",
										(Class[]) null);
								return Boolean.FALSE;
							} catch (NoSuchMethodException ex) {
							}
							try {
								cl.getDeclaredMethod("readFields",
										(Class[]) null);
								return Boolean.FALSE;
							} catch (NoSuchMethodException ex) {
							}
			...
			switch (tc) {
				...
				case TC_BLOCKDATALONG:
					if (oldMode) {
						...
						throw new OptionalDataException(bin
	 ...
	 * replacement object, or echoes provided object if no replacement occurred.
	 * Expects that passHandle is set to given object's handle prior to calling
	 * this method.
	 */
	private Object checkResolve(Object obj) throws IOException {
		...
		if (rep != obj) {
			// The type of the original object has been filtered but
			// resolveObject
	...
	private Object readHandle(boolean unshared) throws IOException {
		...
		if (passHandle < 0 || passHandle >= handles.size()) {
			throw new StreamCorruptedException(String.format(
					"invalid handle value: %08X", passHandle + baseWireHandle));
		}
		if (unshared) {
		...
		filterCheck(null, -1); // just a check for number of references, depth,
								// no class
		return obj;
	}
	...
	private boolean isCustomSubclass() {
		...
		return getClass().getClassLoader() != ObjectInputStream.class
				.getClassLoader();
	}
	...
	/**
	 * Reads in and returns new string. Sets passHandle to new string's assigned
	 * handle.
	 */
	...
	private Object readOrdinaryObject(boolean unshared) throws IOException {
		...
		if (obj != null && handles.lookupException(passHandle) == null && desc
	...
	private void readExternalData(Externalizable obj, ObjectStreamClass desc)
		...
		try {
			...
			if (obj != null) {
				try {
					 ...
					 * CNFException to passHandle at this point; this mark call
					 * is included to address cases where the readExternal
		 ...
		 * consumed. Since there's no "correct" action to take in this case, we
		 * mimic the behavior of past serialization implementations and blindly
		 * hope that the stream is in sync; if it isn't and additional
	...
	private void readSerialData(Object obj, ObjectStreamClass desc)
		...
		for (int i = 0; i < slots.length; i++) {
			...
			if (slots[i].hasData) {
				if (obj == null || handles.lookupException(
						passHandle) != null) {
	 ...
	 * throws a StreamCorruptedException. This method is called when a TC_RESET
	 * typecode is encountered.
	 */
	 ...
	 * null if only code from the null class loader is on the stack. This method
	 * is also called via reflection by the following RMI-IIOP class:
	 *
	...
	private class GetFieldImpl extends GetField {
		...
		/**
		 * Creates GetFieldImpl object for reading fields defined in given class
		 * descriptor.
		 */
		...
		void readFields() throws IOException {
			...
			int numPrimFields = fields.length - objVals.length;
			for (int i = 0; i < objVals.length; i++) {
				objVals[i] = readObject0(fields[numPrimFields + i]
						.isUnshared());
		 ...
		 * null matches all types, Object.class matches all non-primitive types,
		 * and any other non-null type matches assignable types only. If no
		 * matching field is found in the (incoming) class descriptor but a
		 * matching field is present in the associated local class descriptor,
		 * returns -1. Throws IllegalArgumentException if neither incoming nor
		 * local class descriptor contains a match.
		 */
	...
	static class FilterValues implements ObjectInputFilter.FilterInfo {
		...
		public FilterValues(Class<?> clazz, long arrayLength,
				long totalObjectRefs, long depth, long streamBytes) {
			this.clazz = clazz;
	 ...
	 */
	private class BlockDataInputStream extends InputStream implements
			DataInput {
			...
			try {
				for (;;) {
					...
					switch (tc) {
						 ...
						 * level
						 * than other typecodes, since primitive data reads may
						 * span
		...
		private void refill() throws IOException {
			try {
				do {
					pos = 0;
					if (unread > 0) {
						int n = in.read(buf, 0, Math.min(unread,
								MAX_BLOCK_SIZE));
						if (n >= 0) {
							end = n;
							unread -= n;
		...
		public int available() throws IOException {
			if (blkmode) {
				...
				int unreadAvail = (unread > 0) ? Math.min(in.available(),
						unread) : 0;
		...
		private int readUTFChar(StringBuilder sbuf, long utflen)
			...
			switch (b1 >> 4) {
				...
				case 14: // 3 byte format: 1110xxxx 10xxxxxx 10xxxxxx
					...
					sbuf.append((char) (((b1 & 0x0F) << 12) | ((b2 & 0x3F) << 6)
							| ((b3 & 0x3F) << 0)));
					return 3;
	...
	private static void setValidator(ObjectInputStream ois,
			ObjectStreamClassValidator validator) {
		ois.validator = validator;
	}

	static {
		SharedSecrets.setJavaObjectInputStreamAccess(
				ObjectInputStream::setValidator);
	}
}
