 ...
 * file pointer can be read by the {@code getFilePointer} method and set by the
 * {@code seek} method.
 * <p>
 ...
 * an {@code IOException} may be thrown if the stream has been closed.
 *
 * @author unascribed
 * @since JDK1.0
 */
...
public class RandomAccessFile implements DataOutput, DataInput, Closeable {
     ...
     * with the {@code name} argument as its argument to see if read access to
     * the file is allowed. If the mode allows writing, the security manager's
     * {@code checkWrite} method is also called with the {@code name} argument
     ...
     *
     * <p>
     * The <a name="mode"><tt>mode</tt></a> argument specifies the access mode
     ...
     * <p>
     * If there is a security manager, its {@code checkRead} method is called
     * with the pathname of the {@code file} argument as its argument to see if
     * read access to the file is allowed. If the mode allows writing, the
     * security manager's {@code checkWrite} method is also called with the path
     ...
     *
     * <p>
     * The {@link java.nio.channels.FileChannel#position() position} of the
     * returned channel will always be equal to this object's file-pointer
     * offset as returned by the {@link #getFilePointer getFilePointer} method.
     * Changing this object's file-pointer offset, whether explicitly or by
     * reading or writing bytes, will change the position of the channel, and
     * vice versa. Changing the file's length via this object will change the
     * length seen via the file channel, and vice versa.
     *
     ...
     * read-write mode if the O_RDWR bit in {@code mode} is true, else the file
     * is opened as read-only. If the {@code name} refers to a directory, an
     ...
     * {@code InputStream}, this method behaves in exactly the same way as the
     * {@link InputStream#read()} method of {@code InputStream}.
     *
    ...
    /**
     * Reads up to {@code len} bytes of data from this file into an array of
     * bytes. This method blocks until at least one byte of input is available.
     * <p>
     * Although {@code RandomAccessFile} is not a subclass of
     * {@code InputStream}, this method behaves in exactly the same way as the
     * {@link InputStream#read(byte[], int, int)} method of {@code InputStream}.
     *
     * @param b
     *            the buffer into which the data is read.
     * @param off
    ...
    /**
     * Reads up to {@code b.length} bytes of data from this file into an array
    ...
    /**
     * Attempts to skip over {@code n} bytes of input discarding the skipped
     * bytes.
     * <p>
     *
     * This method may skip over some smaller number of bytes, possibly zero.
     * This may result from any of a number of conditions; reaching end of file
     * before {@code n} bytes have been skipped is only one possibility. This
     * method never throws an {@code EOFException}. The actual number of bytes
     ...
     *
     * <p>
     * If the present length of the file as returned by the {@code length}
     * method is greater than the {@code newLength} argument then the file will
     * be truncated. In this case, if the file offset as returned by the
     * {@code getFilePointer} method is greater than {@code newLength} then
     * after this method returns the offset will be equal to {@code newLength}.
     *
     * <p>
     * If the present length of the file as returned by the {@code length}
     * method is smaller than the {@code newLength} argument then the file will
     * be extended. In this case, the contents of the extended portion of the
    ...
    /**
     * Reads a {@code boolean} from this file. This method reads a single byte
     * from the file, starting at the current file pointer. A value of {@code 0}
     * represents {@code false}. Any other value represents {@code true}. This
     * method blocks until the byte is read, the end of the stream is detected,
     ...
     * is {@code b}, where <code>0&nbsp;&lt;=&nbsp;b&nbsp;&lt;=&nbsp;255</code>,
     * then the result is: <blockquote>
     * 
     * <pre>
     * (byte) (b)
     * </pre>
     ...
     * read, in order, are {@code b1} and {@code b2}, where each of the two
     * values is between {@code 0} and {@code 255}, inclusive, then the result
     * is equal to: <blockquote>
     * 
     * <pre>
     * (short) ((b1 &lt;&lt; 8) | b2)
     * </pre>
     ...
     * read, in order, are {@code b1} and {@code b2}, where
     ...
     * in order, are {@code b1}, {@code b2}, {@code b3}, and {@code b4}, where
     ...
     * bytes from the file, starting at the current file pointer. If the bytes
     * read, in order, are {@code b1}, {@code b2}, {@code b3}, {@code b4},
     * {@code b5}, {@code b6}, {@code b7}, and {@code b8,} where: <blockquote>
     * 
     * <pre>
    ...
    /**
     * Reads a {@code float} from this file. This method reads an {@code int}
     * value, starting at the current file pointer, as if by the {@code readInt}
     * method and then converts that {@code int} to a {@code float} using the
     * {@code intBitsToFloat} method in class {@code Float}.
     * <p>
     * This method blocks until the four bytes are read, the end of the stream
     * is detected, or an exception is thrown.
     *
    ...
    /**
     * Reads a {@code double} from this file. This method reads a {@code long}
     * value, starting at the current file pointer, as if by the
     * {@code readLong} method and then converts that {@code long} to a
     * {@code double} using the {@code longBitsToDouble} method in class
     * {@code Double}.
     * <p>
     * This method blocks until the eight bytes are read, the end of the stream
     * is detected, or an exception is thrown.
     ...
     * if by {@code readUnsignedShort}. This value gives the number of following
     ...
     * {@code true} is written out as the value {@code (byte)1}; the value
     * {@code false} is written out as the value {@code (byte)0}. The write
    ...
    /**
     * Writes a {@code byte} to the file as a one-byte value. The write starts
    ...
    /**
     * Writes a {@code short} to the file as two bytes, high byte first. The
    ...
    /**
     * Writes an {@code int} to the file as four bytes, high byte first. The
     * write starts at the current position of the file pointer.
     *
     * @param v
    ...
    /**
     * Writes a {@code long} to the file as eight bytes, high byte first. The
     * write starts at the current position of the file pointer.
     *
     * @param v
     ...
     * {@code floatToIntBits} method in class {@code Float}, and then writes
     * that {@code int} value to the file as a four-byte quantity, high byte
     ...
     * {@code doubleToLongBits} method in class {@code Double}, and then writes
     * that {@code long} value to the file as an eight-byte quantity, high byte
     * first. The write starts at the current position of the file pointer.
     *
     * @param v
     ...
     * <a href="DataInput.html#modified-utf-8">modified UTF-8</a> encoding in a
