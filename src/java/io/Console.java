 ...
 * virtual machine was launched. If the virtual machine is started
 * automatically, for example by a background job scheduler, then it will
 ...
 * {@link java.lang.System#console()} method. If no console device is available
 * then an invocation of that method will return <tt>null</tt>.
 * <p>
 * Read and write operations are synchronized to guarantee the atomic completion
 * of critical operations; therefore invoking methods {@link #readLine()},
 * {@link #readPassword()}, {@link #format format()}, {@link #printf printf()}
 * as well as the read, format and write operations on the objects returned by
 * {@link #reader()} and {@link #writer()} may block in multithreaded scenarios.
 * <p>
 * Invoking <tt>close()</tt> on the objects returned by the {@link #reader()}
 * and the {@link #writer()} will not close the underlying stream of those
 * objects.
 * <p>
 * The console-read methods return <tt>null</tt> when the end of the console
 * input stream is reached, for example by typing control-D on Unix or control-Z
 * on Windows. Subsequent read operations will succeed if additional characters
 * are later entered on the console's input device.
 * <p>
 * Unless otherwise specified, passing a <tt>null</tt> argument to any method in
 * this class will cause a {@link NullPointerException} to be thrown.
 * <p>
 * <b>Security note:</b> If an application needs to read a password or other
 * secure data, it should use {@link #readPassword()} or
...
public final class Console implements Flushable {
	...
	/**
	 * Retrieves the unique {@link java.io.Reader Reader} object associated with
	 * this console.
	 * <p>
	 ...
	 * destination buffer has space for more characters. The {@code Reader}'s
	 * {@code read} methods may block if a line bound has not been entered or
	 * reached on the console's input device. A line bound is considered to be
	 * any one of a line feed (<tt>'\n'</tt>), a carriage return (<tt>'\r'</tt>
	 * ), a carriage return followed immediately by a linefeed, or an end of
	 * stream.
	 *
	...
	private Console() {
		...
		reader = new LineReader(StreamDecoder
				.forInputStreamReader(new FileInputStream(FileDescriptor.in), readLock, cs));
		rcb = new char[1024];
	}
}
