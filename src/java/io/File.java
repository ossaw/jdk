 ...
 * partitions. The object, if any, will reside on the partition
 * <a name="partName">named</a> by some ancestor of the absolute form of this
 * pathname.
 * <p>
 ...
 * <p>
 * The
 * <a href="../../java/nio/file/package-summary.html">{@code java.nio.file}</a>
 * package defines interfaces and classes for the Java virtual machine to access
 * files, file attributes, and file systems. This API may be used to overcome
 * many of the limitations of the {@code java.io.File} class. The {@link #toPath
 * toPath} method may be used to obtain a {@link Path} that uses the abstract
 * path represented by a {@code File} object to locate a file. The resulting
 * {@code Path} may be used with the {@link java.nio.file.Files} class to
 * provide more efficient and extensive access to additional file operations,
 * file attributes, and I/O exceptions to help diagnose errors when an operation
 * on a file fails.
 *
 * @author unascribed
 * @since JDK1.0
...
public class File implements Serializable, Comparable<File> {
     ...
     * file path is very limited, and it only covers Nul character check.
    ...
    /**
     * Internal constructor for already-normalized pathname strings. The
     * parameter order is used to disambiguate this method from the public(File,
     * String) constructor.
     */
     ...
     * defined by the FileSystem.getDefaultParent method. On Unix this default
     * is "/", while on Microsoft Windows it is "\\". This is required for
     ...
     *             abstract pathname into a URL by first converting it into a
     *             URI, via the {@link #toURI() toURI} method, and then
     *             converting the URI into a URL via the
     ...
     * components of the UNC (including the server name component) are encoded
     * in the {@code URI} path. The authority component is undefined, meaning
     * that it is represented as {@code null}. The {@link Path} class defines
     * the {@link Path#toUri toUri} method to encode the server name in the
     * authority component of the resulting {@code URI}. The {@link #toPath
     * toPath} method may be used to obtain a {@code Path} representing this
     * abstract pathname.
     *
     ...
     * marked as unreadable. Consequently this method may return {@code true}
     ...
     * virtual machine with special privileges that allow it to modify files
     * that are marked read-only. Consequently this method may return
     ...
     * <p>
     * Where it is required to distinguish an I/O exception from the case that
     * the file is not a normal file, or where several attributes of the same
     * file are required at the same time, then the
     * {@link java.nio.file.Files#readAttributes(Path,Class,LinkOption[])
     * Files.readAttributes} method may be used.
     *
     * @return <code>true</code> if and only if the file denoted by this
     ...
     * required at the same time, or where the time of last access or the
     * creation time are required, then the
     * {@link java.nio.file.Files#readAttributes(Path,Class,LinkOption[])
     * Files.readAttributes} method may be used.
     *
     ...
     * returns {@code null}. Otherwise an array of strings is returned, one for
     ...
     * directories, and may be more responsive when working with remote
     * directories.
     *
     ...
     * <p>
     * If this abstract pathname does not denote a directory, then this method
     * returns {@code null}. Otherwise an array of {@code File} objects is
     ...
     * {@link #File(File, String) File(File,&nbsp;String)} constructor.
     * Therefore if this pathname is absolute then each resulting pathname is
     * absolute; if this pathname is relative then each resulting pathname will
     ...
     *         will be empty if the directory is empty. Returns {@code null} if
     ...
     * array must satisfy the filter. If the given {@code filter} is
     * {@code null} then all pathnames are accepted. Otherwise, a pathname
     * satisfies the filter if and only if the value {@code true} results when
     * the {@link FilenameFilter#accept
     * FilenameFilter.accept(File,&nbsp;String)} method of the filter is invoked
     * on this abstract pathname and the name of a file or directory in the
     ...
     * <p>
     * Note that the {@link java.nio.file.Files} class defines the
     * {@link java.nio.file.Files#move move} method to move or rename a file in
     ...
     * directory will not change until it is either deleted or marked to allow
     * write access. On some platforms it may be possible to start the Java
     * virtual machine with special privileges that allow it to modify files
     * that are marked read-only. Whether or not a read-only file or directory
     ...
     * abstract pathname. On some platforms it may be possible to start the Java
     * virtual machine with special privileges that allow it to modify files
     * that disallow write operations.
     * <p>
     ...
     * abstract pathname. On some platforms it may be possible to start the Java
     * virtual machine with special privileges that allow it to execute files
     * that are not marked executable. Consequently this method may return
     ...
     * {@code root} directory from which all other files in that file system can
     * be reached. Windows platforms, for example, have a root directory for
     * each active drive; UNIX platforms have a single root directory, namely
     * {@code "/"}. The set of available filesystem roots is affected by various
     * system-level operations such as the insertion or ejection of removable
     ...
     * canonical pathname of any file physically present on the local machine
     * will begin with one of the roots returned by this method.
     * <p>
     ...
     * returned by this method. Thus, for example, {@code File} objects denoting
     * the root directories of the mapped network drives of a Windows platform
     * will be returned by this method, while {@code File} objects containing
     * UNC pathnames will not be returned by this method.
     * <p>
     ...
     *         if the abstract pathname does not name a partition. This value
     *         will be less than or equal to the total file system size returned
    ...
    private static class TempDirectory {
        ...
        private static final File tmpdir = new File(AccessController.doPrivileged(new GetPropertyAction(
                "java.io.tmpdir")));
     ...
     * equivalent to evaluating the expression: <blockquote>
     * 
     * <pre>
     ...
     * returns a {@code Path} that may be used to access the current user
     * directory.
     *
