 ...
 * directory. A pathname consisting of the special token
 ...
 * <a href="../nio/file/package-summary.html#links">symbolic link</a> to be read
 * by invoking the {@link java.nio.file.Files#readSymbolicLink readSymbolicLink
 * } method.
 * </DL>
 * <P>
...
public final class FilePermission extends Permission implements Serializable {
	...
	/**
	 * initialize a FilePermission object. Common to all constructors. Also
		...
		cpath = AccessController.doPrivileged(new PrivilegedAction<String>() {
			public String run() {
				try {
					String path = cpath;
					if (cpath.endsWith("*")) {
						// call getCanonicalPath with a path with wildcard
						// character
	...
	/**
	 * Creates a new FilePermission object using an action mask. More efficient
	 * than the FilePermission(String, String) constructor. Can be used from
	 * within code that needs to create a FilePermission object to pass into the
	 ...
	 * example, "/tmp/*" implies "/tmp/foo", since "/tmp/*" encompasses all
	 * files in the "/tmp" directory, including the one named "foo".
	 * </ul>
	 *
	 * @param p
	...
	public boolean equals(Object obj) {
		...
		return (this.mask == that.mask) && this.cpath.equals(that.cpath)
				&& (this.directory == that.directory) && (this.recursive == that.recursive);
	}
		...
		while (i != -1) {
			...
			if (i >= 3 && (a[i - 3] == 'r' || a[i - 3] == 'R')
				...
				mask |= DELETE;
					...
					&& (a[i - 6] == 'e' || a[i - 6] == 'E') && (a[i - 5] == 'a' || a[i - 5] == 'A')
					&& (a[i - 4] == 'd' || a[i - 4] == 'D') && (a[i - 3] == 'l' || a[i - 3] == 'L')
					&& (a[i - 2] == 'i' || a[i - 2] == 'I') && (a[i - 1] == 'n' || a[i - 1] == 'N')
					&& (a[i] == 'k' || a[i] == 'K')) {
				matchlen = 8;
				mask |= READLINK;
	 ...
	 * write, execute, delete, readlink. For example, if this FilePermission
	...
	/**
	 * WriteObject is called to save the state of the FilePermission to a
 ...
 * implies method. For example, if you have two FilePermissions:
 * <OL>
 * <LI>"/tmp/-", "read"
 * <LI>"/tmp/scratch/foo", "write"
 * </OL>
 * And you are calling the implies function with the FilePermission:
 * "/tmp/scratch/foo", "read,write", then the implies function must take into
...
final class FilePermissionCollection extends PermissionCollection implements Serializable {
	...
	/**
	 * Adds a permission to the FilePermissionCollection. The key for the hash
	 * is permission.path.
	 *
	 * @param permission
	 ...
	 *                - if this FilePermissionCollection object has been marked
	 *                readonly
	 */
