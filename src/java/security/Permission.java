 ...
 * {@code java.io.FilePermission} object, the permission name is the pathname of
 * a file (or directory), and the actions list (such as "read, write") specifies
 * which actions are granted for the specified file (or for files in the
...
public abstract class Permission implements Guard, java.io.Serializable {
	 ...
	 * {@code SecurityManager.checkPermission} method is called, passing this
	 * permission object as the permission to check. Returns silently if access
	 ...
	 * <P>
	 * Do not use the {@code equals} method for making access control decisions;
	 ...
	 * <P>
	 * The required {@code hashCode} behavior for Permission Objects is the
	 * following:
	 * <ul>
	 ...
	 * must consistently return the same integer. This integer need not remain
	 ...
	 * Permission objects must produce the same integer result.
	 * </ul>
	 *
	...
	/**
	 * Returns an empty PermissionCollection for a given Permission object, or
	 * null if one is not defined. Subclasses of class Permission should
	 * override this if they need to store their permissions in a particular
	 * PermissionCollection object in order to provide the correct semantics
	 * when the {@code PermissionCollection.implies} method is called. If null
	 * is returned, then the caller of this method is free to store permissions
	 * of this type in any PermissionCollection they choose (one that uses a
	 * Hashtable, one that uses a Vector, etc).
	 *
	 ...
	 * format: '("ClassName" "name" "actions")', or '("ClassName" "name")' if
	 * actions list is null or empty.
	 *
