 ...
 * below). Currently the SecurityPermission object is used to guard access to
 * the Policy, Security, Provider, Signer, and Identity objects.
 * <P>
 ...
 * {@code DomainCombiner}. Extreme care must be taken when granting this
 * permission. Malicious code could create a DomainCombiner that augments the
 ...
 * <td>This allows someone to query the policy via the {@code getPermissions}
 * call, which discloses which permissions would be granted to a given
 * CodeSource. While revealing the policy does not compromise the security of
 * the system, it does provide malicious code with additional information which
 ...
 * object)</td>
 * <td>Granting this permission is extremely dangerous, as malicious code may
 ...
 * <td>Depending on the particular key for which access has been granted, the
 * code may have access to the list of security providers, as well as the
 * location of the system-wide and user security policies. while revealing this
 * information does not compromise the security of the system, it does provide
 ...
 * <td>This could include setting a security provider or defining the location
 * of the system-wide security policy. Malicious code that has permission to set
 * a new security provider may set a rogue provider that steals confidential
 * information such as cryptographic private keys. In addition, malicious code
 * with permission to set the location of the system-wide security policy may
 ...
 * <td>This would allow somebody to introduce a possibly malicious provider
 ...
 * (which manages the installed providers) currently does not check the
 * integrity or authenticity of a provider before attaching it. The
 ...
 * <td>This may change the behavior or disable execution of other parts of the
 * program. If a provider subsequently requested by the program has been
 * removed, execution may fail. Also, if the removed provider is not explicitly
 * requested by the rest of the program, but it would normally be the provider
 * chosen when a cryptography service is requested (due to its previous order in
 ...
 * <td>This disables the lookup of services implemented by the provider. This
 * may thus change the behavior or disable execution of other parts of the
 ...
 * <td>This disables the lookup of services implemented by the provider. They
 * are no longer accessible due to removal of the properties specifying their
 * names and locations. This may change the behavior or disable execution of
 ...
 * <p>
 * This would allow somebody to introduce a possibly malicious provider (e.g.,
 * one that discloses the private keys passed to it) as the highest-priority
 * provider. This would be possible because the Security object (which manages
 ...
 * introduce a different public key (e.g., its own) that is not trusted by the
 ...
 * <td>This allows attackers to set the general description for an identity.
 ...
 * <td>This allows attackers to set a certificate for an identity's public key.
 * This is dangerous because it affects the trust relationship across the
 ...
 * <td>This allows attackers to remove a certificate for an identity's public
 * key. This is dangerous because it affects the trust relationship across the
 ...
 * used, and whether or not it is considered "trusted" in that scope</td>
 * <td>The scope that is printed out may be a filename, in which case it may
 * convey local system information. For example, here's a sample printout of an
 * identity named "carol", who is marked not trusted in the user's identity
 * database:<br>
 ...
 * <td>It is very dangerous to allow access to a private key; private keys are
 ...
 * communication between the target and its peers. The target's peers might wrap
 * an encryption session key under the target's "new" public key, which would
 * allow the attacker (who possesses the corresponding private key) to unwrap
...
public final class SecurityPermission extends BasicPermission {
     ...
     * symbolic name of the SecurityPermission. An asterisk may appear at the
     * end of the name, following a ".", or by itself, to signify a wildcard
     * match.
     *
     * @param name
    ...
    /**
     * Creates a new SecurityPermission object with the specified name. The name
     * is the symbolic name of the SecurityPermission, and the actions String is
