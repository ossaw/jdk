 ...
 * caller should retain a handle onto the digest object, and clone it for each
...
public class DigestInputStream extends FilterInputStream {
	...
	/**
	 * Creates a digest input stream, using the specified input stream and
	 * message digest.
	 *
	 * @param stream
	 *            the input stream.
	...
	/**
	 * Reads a byte, and updates the message digest (if the digest function is
	 * on). That is, this method reads a byte from the input stream, blocking
	 * until the byte is actually read. If the digest function is on (see
	 * {@link #on(boolean) on}), this method will then call {@code update} on
	...
	/**
	 * Reads into a byte array, and updates the message digest (if the digest
	 * function is on). That is, this method reads up to {@code len} bytes from
	 * the input stream into the array {@code b}, starting at offset {@code off}
	 * . This method blocks until the data is actually read. If the digest
	 * function is on (see {@link #on(boolean) on}), this method will then call
	 * {@code update} on the message digest associated with this stream, passing
	 * it the data.
	 *
	 * @param b
	 ...
	 * call to one of the {@code read} methods results in an update on the
	...
	/**
	 * Prints a string representation of this digest input stream and its
