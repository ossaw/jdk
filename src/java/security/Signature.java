 ...
 * be specified as {@code SHA1withDSA}. In the case of RSA, there are multiple
...
public abstract class Signature extends SignatureSpi {
	 ...
	 * to the particular algorithm. The mapping is done in
	...
	/**
	 * Possible {@link #state} value, signifying that this signature object has
	...
	/**
	 * Possible {@link #state} value, signifying that this signature object has
	 * been initialized for verification.
	 */
	...
	private final static List<ServiceId> rsaIds = Arrays.asList(new ServiceId[] {
	 ...
	 *                if no Provider supports a Signature implementation for the
	 *                specified algorithm.
	 *
	 * @see Provider
	 */
	 ...
	 *
	 * @param algorithm
	 *            the name of the algorithm requested. See the Signature section
	 *            in the <a href=
	 *            "{@docRoot}/../technotes/guides/security/StandardNames.html#Signature"
	 *            > Java Cryptography Architecture Standard Algorithm Name
	 *            Documentation</a> for information about standard algorithm
	 ...
	 *
	 * @exception NoSuchAlgorithmException
	 *                if a SignatureSpi implementation for the specified
	...
	public static Signature getInstance(String algorithm, String provider)
		...
		Instance instance = GetInstance.getInstance("Signature", SignatureSpi.class, algorithm,
				provider);
		return getInstance(instance, algorithm);
	}
	...
	private static Signature getInstanceRSA(Provider p) throws NoSuchAlgorithmException {
		...
		try {
			...
			throw new NoSuchAlgorithmException(
					"no such algorithm: " + RSA_SIGNATURE + " for provider " + p.getName(), e);
		}
	}
	...
	public final void initVerify(PublicKey publicKey) throws InvalidKeyException {
		...
		if (!skipDebug && pdebug != null) {
			pdebug.println("Signature." + algorithm + " verification algorithm from: "
	 ...
	 * corresponding private key are not supposed to be used for digital
	 * signatures, an {@code InvalidKeyException} is thrown.
	 *
	 * @param certificate
	 *            the certificate of the identity whose signature is going to be
	 *            verified.
	 *
	 * @exception InvalidKeyException
	 *                if the public key in the certificate is not encoded
	 *                properly or does not include required parameter
	 *                information or cannot be used for digital signature
	 *                purposes.
	 * @since 1.3
	 */
	...
	public final void initSign(PrivateKey privateKey) throws InvalidKeyException {
		...
		if (!skipDebug && pdebug != null) {
			pdebug.println("Signature." + algorithm + " signing algorithm from: "
					+ this.provider.getName());
		}
	}
	 ...
	 * {@code initSign(PrivateKey)}. That is, the object is reset and available
	 * to generate another signature from the same signer, if desired, via new
	 ...
	 *                this signature algorithm is unable to process the input
	 *                data provided.
	 */
	 ...
	 *
	 * @exception SignatureException
	 *                if this signature object is not initialized properly, if
	 *                this signature algorithm is unable to process the input
	 *                data provided, or if {@code len} is less than the actual
	 *                signature length.
	 *
	 * @since 1.2
	 */
	 ...
	 * {@code initVerify(PublicKey)}. That is, the object is reset and available
	 ...
	 *                input data provided, etc.
	 * @exception IllegalArgumentException
	 *                if the {@code signature} byte array is null, or the
	 *                {@code offset} or {@code length} is less than 0, or the
	 *                sum of the {@code offset} and {@code length} is greater
	 *                than the length of the {@code signature} byte array.
	 * @since 1.4
	 */
	 ...
	 * Processes the {@code data.remaining()} bytes starting at at
	 * {@code data.position()}. Upon return, the buffer's position will be equal
	...
	/**
	 * Sets the specified algorithm parameter to the specified value. This
	 * method supplies a general-purpose mechanism through which it is possible
	 * to set the various parameters of this object. A parameter may be any
	 * settable parameter for the algorithm, such as a parameter size, or a
	 * source of random bits for signature generation (if appropriate), or an
	 ...
	 *                if {@code param} is an invalid parameter for this
	 ...
	 * a general-purpose mechanism through which it is possible to get the
	 * various parameters of this object. A parameter may be any settable
	 * parameter for the algorithm, such as a parameter size, or a source of
	 * random bits for signature generation (if appropriate), or an indication
	 * of whether or not to perform a specific but optional computation. A
	 ...
	 *
	 * @exception InvalidParameterException
	 *                if {@code param} is an invalid parameter for this engine,
	 *                or another exception occurs while trying to get this
	 *                parameter.
	 *
	...
	/*
	 * The following class allows providers to extend from SignatureSpi rather
	 * than from Signature. It represents a Signature with an encapsulated,
	 * provider-supplied SPI object (of type SignatureSpi). If the provider
	 * implementation is an instance of SignatureSpi, the getInstance() methods
	 * above return an instance of this class, with the SPI object encapsulated.
	 *
	 * Note: All SPI methods from the original Signature class have been moved
	 * up the hierarchy into a new class (SignatureSpi), which has been
	 * interposed in the hierarchy between the API (Signature) and its original
	 * parent (Object).
	 */
	...
	private static class Delegate extends Signature {
		 ...
		 *
		 * @exception CloneNotSupportedException
		 *                if this is called on a delegate that does not support
		 *                {@code Cloneable}.
		 */
		public Object clone() throws CloneNotSupportedException {
			chooseFirstProvider();
		...
		private void chooseProvider(int type, Key key, SecureRandom random)
			...
			synchronized (lock) {
				...
				throw new InvalidKeyException("No installed provider supports this key: " + k,
						lastException);
			}
		}
