 ...
 * such permissions can be instantiated during Policy initialization. For
 * example, it is always possible to instantiate a java.io.FilePermission, since
 * the FilePermission class is found on the CLASSPATH.
 * <p>
 * Other permission classes may not yet exist during Policy initialization. For
 * example, a referenced permission class may be in a JAR file that will later
 * be loaded. For each such class, an UnresolvedPermission is instantiated.
 * Thus, an UnresolvedPermission is essentially a "placeholder" containing
 ...
 * <p>
 * To instantiate the new class, UnresolvedPermission assumes the class provides
 * a zero, one, and/or two-argument constructor. The zero-argument constructor
 * would be used to instantiate a permission without a name and without actions.
 * A one-arg constructor is assumed to take a {@code String} name as input, and
 * a two-arg constructor is assumed to take a {@code String} name and
 * {@code String} actions as input. UnresolvedPermission may invoke a
 * constructor with a {@code null} name and/or actions. If an appropriate
 * permission constructor is not available, the UnresolvedPermission is ignored
 * and the relevant permission will not be granted to executing code.
 * <p>
 * The newly created permission object replaces the UnresolvedPermission, which
 * is removed.
 * <p>
 * Note that the {@code getName} method for an {@code UnresolvedPermission}
 * returns the {@code type} (class name) for the underlying permission that has
 * not been resolved.
 *
...
public final class UnresolvedPermission extends Permission implements java.io.Serializable {
    ...
    private static final sun.security.util.Debug debug = sun.security.util.Debug.getInstance("policy,access",
    ...
    /**
     * Creates a new UnresolvedPermission containing the permission information
     * needed later to actually create a Permission of the specified class, when
        ...
        if (certs != null) {
            ...
            if (this.certs == null) {
                ...
                while (i < certs.length) {
                    count++;
                    while (((i + 1) < certs.length) && ((X509Certificate) certs[i]).getIssuerDN().equals(
    ...
    /**
     * Checks two UnresolvedPermission objects for equality. Checks that
     * <i>obj</i> is an UnresolvedPermission, and has the same type (class)
     ...
     * signer certificates. Supporting certificate chains are not taken into
     * consideration by this method.
     *
     * @param obj
     *        the object we are testing for equality with this object.
     * @return true if obj is an UnresolvedPermission, and has the same type
     *         (class) name, permission name, actions, and certificates as this
     *         object.
     */
     ...
     * UnresolvedPermission. That is, the actions for the permission that will
     * be created when this UnresolvedPermission is resolved may be non-null,
     * but an UnresolvedPermission itself is never considered to have any
     * actions.
     *
     ...
     * resolved.
     *
     * @return the type (class name) of the underlying permission that has not
     *         been resolved
     * @since 1.5
     */
    ...
    /**
     * Get the target name of the underlying permission that has not been
     * resolved.
     *
     * @return the target name of the underlying permission that has not been
     *         resolved, or {@code null}, if there is no target name
     * @since 1.5
     */
     ...
     *
     * @return the actions for the underlying permission that has not been
     *         resolved, or {@code null} if there are no actions
     * @since 1.5
     */
    ...
    /**
     * Get the signer certificates (without any supporting chain) for the
     * underlying permission that has not been resolved.
     *
     * @return the signer certificates for the underlying permission that has
     *         not been resolved, or null, if there are no signer certificates.
    ...
    /**
     * Returns a string describing this UnresolvedPermission. The convention is
     * to specify the class name, the permission name, and the actions, in the
     ...
     *
     * @serialData An initial {@code String} denoting the {@code type} is
     *             followed by a {@code String} denoting the {@code name} is
     *             followed by a {@code String} denoting the {@code actions} is
     *             followed by an {@code int} indicating the number of
     *             certificates to follow (a value of "zero" denotes that there
     *             are no certificates associated with this object). Each
     *             certificate is written out starting with a {@code String}
     *             denoting the certificate type, followed by an {@code int}
     *             specifying the length of the certificate encoding, followed
