 ...
 * {@link PKIXParameters#addCertPathChecker addCertPathChecker} methods of the
 * {@code PKIXParameters} and {@code PKIXBuilderParameters} class. Each of the
 ...
 * maintain any internal state that may be necessary to check successive
 * certificates. The {@link #init init} method is used to initialize the
 * internal state of the checker so that the certificates of a new certification
 * path may be checked. A stateful implementation <b>must</b> override the
 * {@link #clone clone} method if necessary in order to allow a PKIX
 ...
 * target to most-trusted CA) or in the reverse direction (from most-trusted CA
 * to target). A {@code PKIXCertPathChecker} implementation <b>must</b> support
 * reverse checking (the ability to perform its checks when it is presented with
 * certificates in the reverse direction) and <b>may</b> support forward
 * checking (the ability to perform its checks when it is presented with
 * certificates in the forward direction). The
...
public abstract class PKIXCertPathChecker implements CertPathChecker, Cloneable {
	 ...
	 * passed to the {@link #check check} method (forward or reverse). A
	 * {@code PKIXCertPathChecker} <b>must</b> support reverse checking and
	 ...
	 *            target to most-trusted CA (forward); if {@code false}, from
	 *            most-trusted CA to target (reverse).
	 * @throws CertPathValidatorException
	 *             if this {@code PKIXCertPathChecker} is unable to check
	 *             certificates in the specified order; it should never be
	 *             thrown if the forward flag is false since reverse checking
	 *             must be supported
	 */
	@Override
	 ...
	 * the ability of the {@code PKIXCertPathChecker} to perform its checks when
	 * certificates are presented to the {@code check} method in the forward
	 * direction (from target to most-trusted CA).
	 *
	...
	/**
	 * Returns an immutable {@code Set} of X.509 certificate extensions that
	 * this {@code PKIXCertPathChecker} supports (i.e. recognizes, is able to
	 * process), or {@code null} if no extensions are supported.
	 * <p>
	 * Each element of the set is a {@code String} representing the Object
	 * Identifier (OID) of the X.509 extension that is supported. The OID is
	 * represented by a set of nonnegative integers separated by periods.
	 * <p>
	 * All X.509 certificate extensions that a {@code PKIXCertPathChecker} might
	 * possibly be able to process should be included in the set.
	 *
	 * @return an immutable {@code Set} of X.509 extension OIDs (in
	 *         {@code String} format) supported by this
	 *         {@code PKIXCertPathChecker}, or {@code null} if no extensions are
	 *         supported
	 */
	 ...
	 *            a {@code Collection} of OID strings representing the current
	...
	/**
	 * Returns a clone of this object. Calls the {@code Object.clone()} method.
	 * All subclasses which maintain state must support and override this
	 * method, if necessary.
	 *
