...
/**
 * <p>
 * Abstract class for X.509 certificates. This provides a standard way to access
 * all the attributes of an X.509 certificate.
 * <p>
 * In June of 1996, the basic X.509 v3 format was completed by ISO/IEC and ANSI
 * X9, which is described below in ASN.1:
 * 
 * <pre>
 ...
 * functionality in Internet security systems. Common applications include
 * Privacy Enhanced Mail (PEM), Transport Layer Security (SSL), code signing for
 * trusted software distribution, and Secure Electronic Transactions (SET).
 * <p>
 * These certificates are managed and vouched for by <em>Certificate
 * Authorities</em> (CAs). CAs are services which create certificates by placing
 * data in the X.509 standard format and then digitally signing that data. CAs
 * act as trusted third parties, making introductions between principals who
 * have no direct knowledge of each other. CA certificates are either signed by
...
public abstract class X509Certificate extends Certificate implements
     ...
     * In other words, this determines whether the certificate would be valid at
     * the given date/time.
     *
     * @param date
    ...
    /**
     * <strong>Denigrated</strong>, replaced by
     ...
     * as an {@code X500Principal}.
     * <p>
     * It is recommended that subclasses override this method.
     *
     * @return an {@code X500Principal} representing the issuer distinguished
     *         name
     * @since 1.4
     */
     ...
     * certificate as an {@code X500Principal}. If the subject value is empty,
     * then the {@code getName()} method of the returned {@code X500Principal}
     * object returns an empty string ("").
     * <p>
     * It is recommended that subclasses override this method.
     *
     * @return an {@code X500Principal} representing the subject distinguished
     *         name
     * @since 1.4
     */
    ...
    /**
     * Gets the DER-encoded certificate information, the {@code tbsCertificate}
     * from this certificate. This can be used to verify the signature
     * independently.
     *
     ...
     * {@code BasicConstraints} extension, (OID = 2.5.29.19).
     * <p>
     * The basic constraints extension identifies whether the subject of the
     * certificate is a Certificate Authority (CA) and how deep a certification
     * path may exist through that CA. The {@code pathLenConstraint} field (see
     * below) is meaningful only if {@code cA} is set to TRUE. In this case, it
     * gives the maximum number of CA certificates that may follow this
     * certificate in a certification path. A value of zero indicates that only
     * an end-entity certificate may follow in the path.
     * <p>
     * The ASN.1 definition for this is:
     * 
     * <pre>
     ...
     *         extension is present in the certificate and the subject of the
     *         certificate is a CA, otherwise -1. If the subject of the
     *         certificate is a CA and {@code pathLenConstraint} does not
     *         appear, {@code Integer.MAX_VALUE} is returned to indicate that
     *         there is no limit to the allowed length of the certification
     *         path.
     */
     ...
     * </pre>
     * <p>
     * If this certificate does not contain a {@code SubjectAltName} extension,
     * {@code null} is returned. Otherwise, a {@code Collection} is returned
     * with an entry representing each {@code GeneralName} included in the
     * extension. Each entry is a {@code List} whose first entry is an
     ...
     * names are returned as {@code String}s, using the well-established string
     * formats for those types (subject to the restrictions included in RFC
     * 3280). IPv4 address names are returned using dotted quad notation. IPv6
     * address names are returned in the form "a1:a2:...:a8", where a1-a8 are
     * hexadecimal values representing the eight 16-bit pieces of the address.
     * OID names are returned as {@code String}s represented as a series of
     * nonnegative integers separated by periods. And directory names
     * (distinguished names) are returned in
     * <a href="http://www.ietf.org/rfc/rfc2253.txt"> RFC 2253</a> string
     * format. No standard string format is defined for otherNames, X.400 names,
     * EDI party names, or any other type of names. They are returned as byte
     * arrays containing the ASN.1 DER encoded form of the name.
     * <p>
     * Note that the {@code Collection} returned may contain more than one name
     * of the same type. Also, note that the returned {@code Collection} is
     * immutable and any entries containing byte arrays are cloned to protect
     ...
     * </pre>
     * 
     * The ASN.1 definition of {@code GeneralNames} is defined in
     * {@link #getSubjectAlternativeNames getSubjectAlternativeNames}.
     * <p>
     * If this certificate does not contain an {@code IssuerAltName} extension,
     * {@code null} is returned. Otherwise, a {@code Collection} is returned
     * with an entry representing each {@code GeneralName} included in the
     * extension. Each entry is a {@code List} whose first entry is an
     * {@code Integer} (the name type, 0-8) and whose second entry is a
     * {@code String} or a byte array (the name, in string or ASN.1 DER encoded
     * form, respectively). For more details about the formats used for each
