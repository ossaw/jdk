 ...
 *
 * <p>
 * A {@code PKIXRevocationChecker} checks the revocation status of certificates
 * with the Online Certificate Status Protocol (OCSP) or Certificate Revocation
 * Lists (CRLs). OCSP is described in RFC 2560 and is a network protocol for
 * determining the status of a certificate. A CRL is a time-stamped list
 * identifying revoked certificates, and RFC 5280 describes an algorithm for
 * determining the revocation status of certificates using CRLs.
 *
 * <p>
 * Each {@code PKIXRevocationChecker} must be able to check the revocation
 * status of certificates with OCSP and CRLs. By default, OCSP is the preferred
 * mechanism for checking revocation status, with CRLs as the fallback
 * mechanism. However, this preference can be switched to CRLs with the
 * {@link Option#PREFER_CRLS PREFER_CRLS} option. In addition, the fallback
 ...
 * {@link CertPathValidator#getRevocationChecker getRevocationChecker} method of
 * a PKIX {@code CertPathValidator}. Additional parameters and options specific
 * to revocation can be set (by calling the {@link #setOcspResponder
 * setOcspResponder} method for instance). The {@code PKIXRevocationChecker} is
 * added to a {@code PKIXParameters} object using the
 * {@link PKIXParameters#addCertPathChecker addCertPathChecker} or
 * {@link PKIXParameters#setCertPathCheckers setCertPathCheckers} method, and
 * then the {@code PKIXParameters} is passed along with the {@code CertPath} to
 * be validated to the {@link CertPathValidator#validate validate} method of a
 * PKIX {@code CertPathValidator}. When supplying a revocation checker in this
 * manner, it will be used to check revocation irrespective of the setting of
 * the {@link PKIXParameters#isRevocationEnabled RevocationEnabled} flag.
 ...
 * {@code PKIXParameters}, it clones the {@code PKIXRevocationChecker}; thus any
 * subsequent modifications to the {@code PKIXRevocationChecker} have no effect.
 *
 * <p>
 ...
 *      OCSP</i></a>, <br>
 *      <a href="http://www.ietf.org/rfc/rfc5280.txt"><i>RFC&nbsp;5280: Internet
