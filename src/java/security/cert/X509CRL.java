 ...
 * verifying a remote user's digital signature), that system not only checks the
 * certificate signature and validity but also acquires a suitably- recent CRL
 * and checks that the certificate serial number is not on that CRL. The meaning
 * of "suitably-recent" may vary with local policy, but it usually means the
 * most recently-issued CRL. A CA issues a new CRL on a regular periodic basis
 * (e.g., hourly, daily, or weekly). Entries are added to CRLs as revocations
 * occur, and an entry may be removed when the certificate expiration date is
 * reached.
 * <p>
...
public abstract class X509CRL extends CRL implements X509Extension {
	...
	/**
	 * Compares this CRL for equality with the given object. If the
	 * {@code other} object is an {@code instanceof} {@code X509CRL}, then its
	...
	/**
	 * Gets the {@code version} (version number) value from the CRL. The ASN.1
	...
	/**
	 * Gets the {@code thisUpdate} date from the CRL. The ASN.1 definition for
	 * this is:
	 * 
	 * <pre>
	 * thisUpdate   ChoiceOfTime
	 ...
	 *
	 * @return the {@code nextUpdate} date from the CRL, or null if not present.
	 */
	 ...
	 *
	 * @param serialNumber
	 *            the serial number of the certificate for which a CRL entry is
	 *            to be looked up
	 ...
	 * The default implementation will only return entries for certificates
	 * issued by the CRL issuer. Subclasses that wish to support indirect CRLs
	...
	/**
	 * Gets all the entries from this CRL. This returns a Set of X509CRLEntry
	 * objects.
	 *
	...
	/**
	 * Gets the DER-encoded CRL information, the {@code tbsCertList} from this
	 * CRL. This can be used to verify the signature independently.
	 *
	...
	/**
	 * Gets the signature algorithm name for the CRL signature algorithm. An
	 * example is the string "SHA256withRSA". The ASN.1 definition for this is:
	 * 
	 * <pre>
	 * signatureAlgorithm   AlgorithmIdentifier
	 *
	 ...
	 * are null; the parameters are usually supplied with the public key. If
	 * access to individual parameter values is needed then use
