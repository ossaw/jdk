 ...
 * {@code CertificateRevokedException} contains additional information about the
 * revoked certificate, such as the date on which the certificate was revoked
 * and the reason it was revoked.
 *
 * @author Sean Mullan
 * @since 1.7
...
public class CertificateRevokedException extends CertificateException {
	 ...
	 *         authority that signed the certificate's revocation status
	 *         information
	 */
	...
	/**
	 * Constructs a {@code CertificateRevokedException} with the specified
	 * revocation date, reason code, authority name, and map of extensions.
	 *
	 * @param revocationDate
	 *            the date on which the certificate was revoked. The date is
	 *            copied to protect against subsequent modification.
	 * @param reason
	 *            the revocation reason
	 * @param extensions
	 *            a map of X.509 Extensions. Each key is an OID String that maps
	 *            to the corresponding Extension. The map is copied to prevent
	 *            subsequent modification.
	 * @param authority
	 *            the {@code X500Principal} that represents the name of the
	 *            authority that signed the certificate's revocation status
	 *            information
	 * @throws NullPointerException
	 *             if {@code revocationDate}, {@code reason}, {@code authority},
	 ...
	 * extension of this {@code CertificateRevokedException}. The invalidity
	 * date is the date on which it is known or suspected that the private key
	 * was compromised or that the certificate otherwise became invalid. This
	 * implementation calls {@code getExtensions()} and checks the returned map
	 * for an entry for the Invalidity Date extension OID ("2.5.29.24"). If
	 * found, it returns the invalidity date in the extension; otherwise null. A
	 * new Date object is returned each time the method is invoked to protect
	...
	/**
	 * Returns a map of X.509 extensions containing additional information about
	 * the revoked certificate, such as the Invalidity Date Extension. Each key
	 * is an OID String that maps to the corresponding Extension.
	 *
	 * @return an unmodifiable map of X.509 extensions, or an empty map if there
	 *         are no extensions
	 */
	...
	public String getMessage() {
		return "Certificate has been revoked, reason: " + reason + ", revocation date: "
				+ revocationDate + ", authority: " + authority + ", extension OIDs: "
				+ extensions.keySet();
	}
	 ...
	 *
	 * @serialData the size of the extensions map (int), followed by all of the
	 *             extensions in the map, in no particular order. For each
	 *             extension, the following data is emitted: the OID String
	 *             (Object), the criticality flag (boolean), the length of the
	 *             encoded extension value byte array (int), and the encoded
	...
	private void readObject(ObjectInputStream ois) throws IOException, ClassNotFoundException {
		...
		for (int i = 0; i < size; i++) {
			...
			Extension ext = sun.security.x509.Extension.newExtension(new ObjectIdentifier(oid),
					critical, extVal);
