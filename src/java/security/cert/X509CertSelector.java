...
/**
 * A {@code CertSelector} that selects {@code X509Certificates} that match all
 * specified criteria. This class is particularly useful when selecting
 * certificates from a {@code CertStore} to build a PKIX-compliant certification
 * path.
 * <p>
 * When first constructed, an {@code X509CertSelector} has no criteria enabled
 * and each of the {@code get} methods return a default value ({@code null}, or
 * {@code -1} for the {@link #getBasicConstraints getBasicConstraints} method).
 * Therefore, the {@link #match match} method would return {@code true} for any
 * {@code X509Certificate}. Typically, several criteria are enabled (by calling
 * {@link #setIssuer setIssuer} or {@link #setKeyUsage setKeyUsage}, for
 * instance) and then the {@code X509CertSelector} is passed to
 * {@link CertStore#getCertificates CertStore.getCertificates} or some similar
 * method.
 * <p>
 * Several criteria can be enabled (by calling {@link #setIssuer setIssuer} and
 * {@link #setSerialNumber setSerialNumber}, for example) such that the
 * {@code match} method usually uniquely matches a single
 * {@code X509Certificate}. We say usually, since it is possible for two issuing
 * CAs to have the same distinguished name and each issue a certificate with the
 * same serial number. Other unique combinations include the issuer, subject,
...
public class X509CertSelector implements CertSelector {
	...
	/**
	 * Creates an {@code X509CertSelector}. Initially, no criteria are set so
	 * any {@code X509Certificate} will match.
	 */
	 ...
	 * {@code X509Certificate} must be equal to the {@code X509Certificate}
	 * passed to the {@code match} method. If {@code null}, then this check is
	 * not applied.
	 *
	 * <p>
	 ...
	 *
	 * @param cert
	 *             the {@code X509Certificate} to match (or {@code null})
	 * @see #getCertificate
	 */
	 ...
	 * the certificate serial number in the {@code X509Certificate}. If
	 * {@code null}, any certificate serial number will do.
	 *
	 * @param serial
	 *               the certificate serial number to match (or {@code null})
	 * @see #getSerialNumber
	 */
	 ...
	 * the issuer distinguished name in the {@code X509Certificate}. If
	 * {@code null}, any issuer distinguished name will do.
	 *
	 * @param issuer
	 *               a distinguished name as X500Principal (or {@code null})
	 * @since 1.5
	 */
	 ...
	 * <a href="http://www.ietf.org/rfc/rfc2253.txt">RFC 2253</a> String form of
	 ...
	 *
	 * @param issuerDN
	 *                 a distinguished name in RFC 2253 format (or {@code null})
	 * @throws IOException
	 ...
	 * Sets the issuer criterion. The specified distinguished name must match
	 * the issuer distinguished name in the {@code X509Certificate}. If
	 * {@code null} is specified, the issuer criterion is disabled and any
	 * issuer distinguished name will do.
	 * <p>
	 * If {@code issuerDN} is not {@code null}, it should contain a single DER
	 ...
	 * the subject distinguished name in the {@code X509Certificate}. If
	 * {@code null}, any subject distinguished name will do.
	 *
	 * @param subject
	 *                a distinguished name as X500Principal (or {@code null})
	 * @since 1.5
	 ...
	 *
	 * @param subjectDN
	 *                  a distinguished name in RFC 2253 format (or
	 *                  {@code null})
	 * @throws IOException
	 *                     if a parsing error occurs (incorrect form for DN)
	 */
	 ...
	 * {@code null}, any subject distinguished name will do.
	 * <p>
	 * If {@code subjectDN} is not {@code null}, it should contain a single DER
	 * encoded distinguished name, as defined in X.501. For the ASN.1 notation
	 * for this structure, see {@link #setIssuer(byte [] issuerDN)
	...
	/**
	 * Sets the subjectKeyIdentifier criterion. The {@code X509Certificate} must
	 * contain a SubjectKeyIdentifier extension for which the contents of the
	 * extension matches the specified criterion value. If the criterion value
	 * is {@code null}, no subjectKeyIdentifier check will be done.
	 * <p>
	 * If {@code subjectKeyID} is not {@code null}, it should contain a single
	 * DER encoded value corresponding to the contents of the extension value
	 * (not including the object identifier, criticality setting, and
	 * encapsulating OCTET STRING) for a SubjectKeyIdentifier extension. The
	 * ASN.1 notation for this structure follows.
	 *
	 * <pre>
	 * {@code
	 ...
	 * {@code X509CertSelector}. Instead, the values are compared using a
	 ...
	 *
	 * @param subjectKeyID
	 *                     the subject key identifier (or {@code null})
	...
	/**
	 * Sets the authorityKeyIdentifier criterion. The {@code X509Certificate}
	 * must contain an AuthorityKeyIdentifier extension for which the contents
	 * of the extension value matches the specified criterion value. If the
	 * criterion value is {@code null}, no authorityKeyIdentifier check will be
	 * done.
	 * <p>
	 * If {@code authorityKeyID} is not {@code null}, it should contain a single
	 * DER encoded value corresponding to the contents of the extension value
	 * (not including the object identifier, criticality setting, and
	 * encapsulating OCTET STRING) for an AuthorityKeyIdentifier extension. The
	 * ASN.1 notation for this structure follows.
	 *
	 * <pre>
	 * {@code
	 ...
	 * </pre>
	 * <p>
	 * Authority key identifiers are not parsed by the {@code X509CertSelector}.
	 * Instead, the values are compared using a byte-by-byte comparison.
	 * <p>
	 * When the {@code keyIdentifier} field of {@code AuthorityKeyIdentifier} is
	 * populated, the value is usually taken from the
	 * {@code SubjectKeyIdentifier} extension in the issuer's certificate. Note,
	 * however, that the result of
	 ...
	 * input to {@code setAuthorityKeyIdentifier}. This is because the
	 ...
	 * {@code SubjectKeyIdentifier} extension, it will be necessary to extract
	 * the value of the embedded {@code KeyIdentifier} OCTET STRING, then DER
	 * encode this OCTET STRING inside a SEQUENCE. For more details on
	 ...
	 *                       the authority key identifier (or {@code null})
	 ...
	 * the certificate validity period for the {@code X509Certificate}. If
	 * {@code null}, no certificateValid check will be done.
	 * <p>
	 * Note that the {@code Date} supplied here is cloned to protect against
	 * subsequent modifications.
	 *
	 * @param certValid
	 *                  the {@code Date} to check (or {@code null})
	 ...
	 * the private key validity period for the {@code X509Certificate}. If
	 * {@code null}, no privateKeyValid check will be done.
	 * <p>
	 * Note that the {@code Date} supplied here is cloned to protect against
	 * subsequent modifications.
	 *
	...
	/**
	 * Sets the subjectPublicKeyAlgID criterion. The {@code X509Certificate}
	 * must contain a subject public key with the specified algorithm. If
	 * {@code null}, no subjectPublicKeyAlgID check will be done.
	 *
	 * @param oid
	 *            The object identifier (OID) of the algorithm to check for (or
	 *            {@code null}). An OID is represented by a set of nonnegative
	 *            integers separated by periods.
	 * @throws IOException
	 *                     if the OID is invalid, such as the first component
	 *                     being not
	 *                     0, 1 or 2 or the second component being greater than
	 *                     39.
	 *
	...
	/**
	 * Sets the subjectPublicKey criterion. The {@code X509Certificate} must
	 * contain the specified subject public key. If {@code null}, no
	 * subjectPublicKey check will be done.
	 *
	 * @param key
	 *            the subject public key to check for (or {@code null})
	 ...
	 * <p>
	 * If {@code key} is not {@code null}, it should contain a single DER
	...
	public void setSubjectPublicKey(byte[] key) throws IOException {
		if (key == null) {
			...
			subjectPublicKey = X509Key.parse(new DerValue(
					subjectPublicKeyBytes));
		}
	}

	/**
	 * Sets the keyUsage criterion. The {@code X509Certificate} must allow the
	 * specified keyUsage values. If {@code null}, no keyUsage check will be
	 * done. Note that an {@code X509Certificate} that has no keyUsage extension
	 ...
	 *                 returned by {@link X509Certificate#getKeyUsage()
	 *                 X509Certificate.getKeyUsage()}. Or {@code null}.
	 * @see #getKeyUsage
	 */
	...
	/**
	 * Sets the extendedKeyUsage criterion. The {@code X509Certificate} must
	 * allow the specified key purposes in its extended key usage extension. If
	 * {@code keyPurposeSet} is empty or {@code null}, no extendedKeyUsage check
	 * will be done. Note that an {@code X509Certificate} that has no
	 * extendedKeyUsage extension implicitly allows all key purposes.
	 * <p>
	 * Note that the {@code Set} is cloned to protect against subsequent
	 * modifications.
	 *
	 * @param keyPurposeSet
	 *                      a {@code Set} of key purpose OIDs in string format
	 *                      (or
	 *                      {@code null}). Each OID is represented by a set of
	 *                      nonnegative
	 *                      integers separated by periods.
	 * @throws IOException
	 *                     if the OID is invalid, such as the first component
	...
	public void setExtendedKeyUsage(Set<String> keyPurposeSet)
		...
		if ((keyPurposeSet == null) || keyPurposeSet.isEmpty()) {
			...
			this.keyPurposeSet = Collections.unmodifiableSet(
					new HashSet<String>(keyPurposeSet));
	 ...
	 * enabled, the {@code X509Certificate} must contain all of the specified
	 * subject alternative names. If disabled, the {@code X509Certificate} must
	 * contain at least one of the specified subject alternative names.
	 *
	 * <p>
	 * The matchAllNames flag is {@code true} by default.
	 *
	 * @param matchAllNames
	 *                      if {@code true}, the flag is enabled; if
	 *                      {@code false}, the
	 *                      flag is disabled.
	...
	/**
	 * Sets the subjectAlternativeNames criterion. The {@code X509Certificate}
	 * must contain all or at least one of the specified
	 * subjectAlternativeNames, depending on the value of the matchAllNames flag
	 * (see {@link #setMatchAllSubjectAltNames setMatchAllSubjectAltNames}).
	 * <p>
	 ...
	 * <p>
	 * The {@code names} parameter (if not {@code null}) is a {@code Collection}
	 * with one entry for each name to be included in the subject alternative
	 * name criterion. Each entry is a {@code List} whose first entry is an
	 * {@code Integer} (the name type, 0-8) and whose second entry is a
	 * {@code String} or a byte array (the name, in string or ASN.1 DER encoded
	 * form, respectively). There can be multiple names of the same type. If
	 * {@code null} is supplied as the value for this argument, no
	 * subjectAlternativeNames check will be performed.
	 * <p>
	 * Each subject alternative name in the {@code Collection} may be specified
	 * either as a {@code String} or as an ASN.1 encoded byte array. For more
	 * details about the formats used, see
	 * {@link #addSubjectAlternativeName(int type, String name)
	 * addSubjectAlternativeName(int type, String name)} and
	 * {@link #addSubjectAlternativeName(int type, byte [] name)
	 * addSubjectAlternativeName(int type, byte [] name)}.
	 * <p>
	 * <strong>Note:</strong> for distinguished names, specify the byte array
	 * form instead of the String form. See the note in
	 * {@link #addSubjectAlternativeName(int, String)} for more information.
	 * <p>
	 * Note that the {@code names} parameter can contain duplicate names (same
	 * name and name type), but they may be removed from the {@code Collection}
	 * of names returned by the {@link #getSubjectAlternativeNames
	 * getSubjectAlternativeNames} method.
	 * <p>
	 * Note that a deep copy is performed on the {@code Collection} to protect
	 * against subsequent modifications.
	 *
	 * @param names
	 *              a {@code Collection} of names (or {@code null})
	 * @throws IOException
	 ...
	 * {@code X509Certificate} must contain all or at least one of the specified
	 * subjectAlternativeNames, depending on the value of the matchAllNames flag
	 * (see {@link #setMatchAllSubjectAltNames setMatchAllSubjectAltNames}).
	 * <p>
	 ...
	 * the restrictions included in RFC 3280). IPv4 address names are supplied
	 * using dotted quad notation. OID address names are represented as a series
	 * of nonnegative integers separated by periods. And directory names
	 ...
	 * the {@link #addSubjectAlternativeName(int type, byte [] name)
	 * addSubjectAlternativeName(int type, byte [] name)} method.
	 * <p>
	 * <strong>Note:</strong> for distinguished names, use
	 * {@linkplain #addSubjectAlternativeName(int, byte[])} instead. This method
	 * should not be relied on as it can fail to match some certificates because
	 * of a loss of encoding information in the RFC 2253 String form of some
	 * distinguished names.
	 *
	 * @param type
	 *             the name type (0-8, as specified in RFC 3280, section
	 *             4.2.1.7)
	 * @param name
	 *             the name in string form (not {@code null})
	 * @throws IOException
	 *                     if a parsing error occurs
	 */
	 ...
	 * in RFC 3280 and X.509. The encoded byte array should only contain the
	 ...
	 * subjectAlternativeNames criterion. The {@code X509Certificate} must
	 * contain the specified subjectAlternativeName.
	 *
	 * @param type
	 *             the name type (0-8, as specified in RFC 3280, section
	 *             4.2.1.7)
	 ...
	 * returning a {@code Collection} of {@code GeneralNameInterface}s. Throw an
	 * IllegalArgumentException or a ClassCastException if the argument is
	 * malformed.
	 *
	 * @param names
	 *              a Collection with one entry per name. Each entry is a
	 *              {@code List} whose first entry is an Integer (the name type,
	 *              0-8) and whose second entry is a String or a byte array (the
	 *              name, in string or ASN.1 DER encoded form, respectively).
	 *              There can be multiple names of the same type. Null is not an
	 *              acceptable value.
	 * @return a Set of {@code GeneralNameInterface}s
	 * @throws IOException
	 *                     if a parsing error occurs
	 */
	private static Set<GeneralNameInterface> parseNames(
			Collection<List<?>> names) throws IOException {
	 ...
	 * setSubjectAlternativeNames (or X509CRLSelector.setIssuerNames). Throw an
	 * {@code IllegalArgumentException} or a {@code ClassCastException} if one
	 * of the objects is malformed.
	 *
	 * @param object1
	 ...
	 * Object that may be a byte array holding the ASN.1 DER encoded name or a
	 * String form of the name. Except for X.509 Distinguished Names, the String
	 * form of the name must not be the result from calling toString on an
	 * existing GeneralNameInterface implementing class. The output of toString
	 * is not compatible with the String constructors for names other than
	 * Distinguished Names.
	 *
	 * @param type
	 *             name type (0-8)
	...
	static GeneralNameInterface makeGeneralNameInterface(int type, Object name)
		...
		if (name instanceof String) {
			...
			switch (type) {
				...
				default:
					throw new IOException(
							"unable to parse String names of type " + type);
			}
			if (debug != null) {
				debug.println("X509CertSelector.makeGeneralNameInterface() "
			...
			if (debug != null) {
				debug.println(
						"X509CertSelector.makeGeneralNameInterface() result: "
								+ result.toString());
			}
		} else {
			if (debug != null) {
	...
	/**
	 * Sets the name constraints criterion. The {@code X509Certificate} must
	 * have subject and subject alternative names that meet the specified name
	 * constraints.
	 * <p>
	 * The name constraints are specified as a byte array. This byte array
	 * should contain the DER encoded form of the name constraints, as they
	 * would appear in the NameConstraints structure defined in RFC 3280 and
	 ...
	 *              not
	 *              the OID or criticality flag. Can be {@code null}, in which
	 *              case no name constraints check will be performed.
	 * @throws IOException
	 *                     if a parsing error occurs
	 ...
	 * equal to zero, {@code X509Certificates} must include a basicConstraints
	 * extension with a pathLen of at least this value. If the value is -2, only
	 * end-entity certificates are accepted. If the value is -1, no check is
	 * done.
	 * <p>
	...
	public void setBasicConstraints(int minMaxPathLen) {
		if (minMaxPathLen < -2) {
			throw new IllegalArgumentException(
					"basic constraints less than -2");
		}
		basicConstraints = minMaxPathLen;
	}

	/**
	 * Sets the policy constraint. The {@code X509Certificate} must include at
	 * least one of the specified policies in its certificate policies
	 * extension. If {@code certPolicySet} is empty, then the
	 * {@code X509Certificate} must include at least some specified policy in
	 * its certificate policies extension. If {@code certPolicySet} is
	 ...
	 *
	 * @param certPolicySet
	 *                      a {@code Set} of certificate policy OIDs in string
	 *                      format (or
	 *                      {@code null}). Each OID is represented by a set of
	 *                      nonnegative
	 *                      integers separated by periods.
	 * @throws IOException
	 *                     if a parsing error occurs on the OID such as the
	 *                     first
	...
	/**
	 * Sets the pathToNames criterion. The {@code X509Certificate} must not
	 * include name constraints that would prohibit building a path to the
	 * specified names.
	 * <p>
	 * This method allows the caller to specify, with a single method call, the
	 * complete set of names which the {@code X509Certificates}'s name
	 ...
	 * <p>
	 * The {@code names} parameter (if not {@code null}) is a {@code Collection}
	 * with one entry for each name to be included in the pathToNames criterion.
	 * Each entry is a {@code List} whose first entry is an {@code Integer} (the
	 * name type, 0-8) and whose second entry is a {@code String} or a byte
	 * array (the name, in string or ASN.1 DER encoded form, respectively).
	 * There can be multiple names of the same type. If {@code null} is supplied
	 * as the value for this argument, no pathToNames check will be performed.
	 * <p>
	 * Each name in the {@code Collection} may be specified either as a
	 * {@code String} or as an ASN.1 encoded byte array. For more details about
	 * the formats used, see {@link #addPathToName(int type, String name)
	 ...
	 * Note that the {@code names} parameter can contain duplicate names (same
	 * name and name type), but they may be removed from the {@code Collection}
	 * of names returned by the {@link #getPathToNames getPathToNames} method.
	 * <p>
	 * Note that a deep copy is performed on the {@code Collection} to protect
	 * against subsequent modifications.
	 *
	 * @param names
	 *              a {@code Collection} with one entry per name (or
	 *              {@code null})
	 * @throws IOException
	 *                     if a parsing error occurs
	 * @see #getPathToNames
	 ...
	 * must not include name constraints that would prohibit building a path to
	 * the specified name.
	 * <p>
	 * This method allows the caller to add a name to the set of names which the
	 * {@code X509Certificates}'s name constraints must permit. The specified
	 * name is added to any previous value for the pathToNames criterion. If the
	 * name is a duplicate, it may be ignored.
	 * <p>
	 ...
	 * restrictions included in RFC 3280). IPv4 address names are supplied using
	 ...
	 * format is defined for otherNames, X.400 names, EDI party names, IPv6
	 * address names, or any other type of names. They should be specified using
	 * the {@link #addPathToName(int type, byte [] name) addPathToName(int type,
	 * byte [] name)} method.
	 * <p>
	 * <strong>Note:</strong> for distinguished names, use
	 * {@linkplain #addPathToName(int, byte[])} instead. This method should not
	 * be relied on as it can fail to match some certificates because of a loss
	 * of encoding information in the RFC 2253 String form of some distinguished
	 * names.
	 *
	 * @param type
	 *             the name type (0-8, as specified in RFC 3280, section
	 ...
	 * The name is provided as a byte array. This byte array should contain the
	 * DER encoded name, as it would appear in the GeneralName structure defined
	 * in RFC 3280 and X.509. The ASN.1 definition of this structure appears in
	...
	/**
	 * A private method that adds a name (String or byte array) to the
	 * pathToNames criterion. The {@code X509Certificate} must contain the
	 * specified pathToName.
	 *
	 * @param type
	 *             the name type (0-8, as specified in RFC 3280, section
	 *             4.2.1.7)
	 ...
	 * {@code X509Certificate} must be equal to the {@code X509Certificate}
	 * passed to the {@code match} method. If {@code null}, this check is not
	 * applied.
	 *
	 ...
	 * match the certificate serial number in the {@code X509Certificate}. If
	 * {@code null}, any certificate serial number will do.
	 *
	 ...
	 * {@code X509Certificate}. If {@code null}, the issuer criterion is
	 * disabled and any issuer distinguished name will do.
	 *
	 * @return the required issuer distinguished name as X500Principal (or
	 *         {@code null})
	 * @since 1.5
	 */
	 ...
	 * names.
	 * <p>
	 * Returns the issuer criterion as a {@code String}. This distinguished name
	 * must match the issuer distinguished name in the {@code X509Certificate}.
	 * If {@code null}, the issuer criterion is disabled and any issuer
	 * distinguished name will do.
	 * <p>
	 * If the value returned is not {@code null}, it is a distinguished name, in
	 * RFC 2253 format.
	 *
	 ...
	 * distinguished name will do.
	 * <p>
	 * If the value returned is not {@code null}, it is a byte array containing
	 ...
	 * {@code X509Certificate}. If {@code null}, the subject criterion is
	 * disabled and any subject distinguished name will do.
	 *
	 * @return the required subject distinguished name as X500Principal (or
	 *         {@code null})
	 * @since 1.5
	 */
	 ...
	 * names.
	 * <p>
	 * Returns the subject criterion as a {@code String}. This distinguished
	 ...
	 * must match the subject distinguished name in the {@code X509Certificate}.
	 * If {@code null}, the subject criterion is disabled and any subject
	 * distinguished name will do.
	 * <p>
	 * If the value returned is not {@code null}, it is a byte array containing
	 * a single DER encoded distinguished name, as defined in X.501. The ASN.1
	...
	/**
	 * Returns the subjectKeyIdentifier criterion. The {@code X509Certificate}
	 * must contain a SubjectKeyIdentifier extension with the specified value.
	 * If {@code null}, no subjectKeyIdentifier check will be done.
	 * <p>
	 * Note that the byte array returned is cloned to protect against subsequent
	 * modifications.
	 *
	...
	/**
	 * Returns the authorityKeyIdentifier criterion. The {@code X509Certificate}
	 * must contain a AuthorityKeyIdentifier extension with the specified value.
	 * If {@code null}, no authorityKeyIdentifier check will be done.
	 * <p>
	 * Note that the byte array returned is cloned to protect against subsequent
	 * modifications.
	 *
	 ...
	 * within the certificate validity period for the {@code X509Certificate}.
	 * If {@code null}, no certificateValid check will be done.
	 * <p>
	 ...
	 * within the private key validity period for the {@code X509Certificate}.
	 * If {@code null}, no privateKeyValid check will be done.
	 * <p>
	 * Note that the {@code Date} returned is cloned to protect against
	 * subsequent modifications.
	 *
	...
	/**
	 * Returns the subjectPublicKeyAlgID criterion. The {@code X509Certificate}
	 * must contain a subject public key with the specified algorithm. If
	 * {@code null}, no subjectPublicKeyAlgID check will be done.
	 *
	...
	/**
	 * Returns the subjectPublicKey criterion. The {@code X509Certificate} must
	 * contain the specified subject public key. If {@code null}, no
	 * subjectPublicKey check will be done.
	 *
	...
	/**
	 * Returns the keyUsage criterion. The {@code X509Certificate} must allow
	 * the specified keyUsage values. If null, no keyUsage check will be done.
	 * <p>
	 ...
	 *         by {@link X509Certificate#getKeyUsage()
	 *         X509Certificate.getKeyUsage()}. Or {@code null}.
	 * @see #setKeyUsage
	 */
	...
	/**
	 * Returns the extendedKeyUsage criterion. The {@code X509Certificate} must
	 * allow the specified key purposes in its extended key usage extension. If
	 * the {@code keyPurposeSet} returned is empty or {@code null}, no
	 * extendedKeyUsage check will be done. Note that an {@code X509Certificate}
	 * that has no extendedKeyUsage extension implicitly allows all key
	 * purposes.
	 *
	 * @return an immutable {@code Set} of key purpose OIDs in string format (or
	 *         {@code null})
	...
	/**
	 * Indicates if the {@code X509Certificate} must contain all or at least one
	 * of the subjectAlternativeNames specified in the
	 * {@link #setSubjectAlternativeNames setSubjectAlternativeNames} or
	 * {@link #addSubjectAlternativeName addSubjectAlternativeName} methods. If
	 * {@code true}, the {@code X509Certificate} must contain all of the
	 * specified subject alternative names. If {@code false}, the
	 * {@code X509Certificate} must contain at least one of the specified
	 * subject alternative names.
	 *
	 * @return {@code true} if the flag is enabled; {@code false} if the flag is
	 *         disabled. The flag is {@code true} by default.
	 ...
	 * {@code X509Certificate} must contain all or at least one of the specified
	 * subjectAlternativeNames, depending on the value of the matchAllNames flag
	 * (see {@link #getMatchAllSubjectAltNames getMatchAllSubjectAltNames}). If
	 * the value returned is {@code null}, no subjectAlternativeNames check will
	 * be performed.
	 * <p>
	 * If the value returned is not {@code null}, it is a {@code Collection}
	 * with one entry for each name to be included in the subject alternative
	 * name criterion. Each entry is a {@code List} whose first entry is an
	 * {@code Integer} (the name type, 0-8) and whose second entry is a
	 * {@code String} or a byte array (the name, in string or ASN.1 DER encoded
	 * form, respectively). There can be multiple names of the same type. Note
	 * that the {@code Collection} returned may contain duplicate names (same
	 * name and name type).
	 * <p>
	 * Each subject alternative name in the {@code Collection} may be specified
	 * either as a {@code String} or as an ASN.1 encoded byte array. For more
	...
	/**
	 * Clone an object of the form passed to setSubjectAlternativeNames and
	 * setPathToNames. Throw a {@code RuntimeException} if the argument is
	 * malformed.
	 * <p>
	 * This method wraps cloneAndCheckNames, changing any {@code IOException}
	 * into a {@code RuntimeException}. This method should be used when the
	 * object being cloned has already been checked, so there should never be
	 * any exceptions.
	 *
	 * @param names
	 *              a {@code Collection} with one entry per name. Each entry is
	 *              a
	 *              {@code List} whose first entry is an Integer (the name type,
	 *              0-8) and whose second entry is a String or a byte array (the
	 *              name, in string or ASN.1 DER encoded form, respectively).
	 ...
	 * setSubjectAlternativeNames and setPathToNames. Throw an
	 * {@code IOException} if the argument is malformed.
	 *
	 * @param names
	 *              a {@code Collection} with one entry per name. Each entry is
	 *              a
	 ...
	 *              0-8) and whose second entry is a String or a byte array (the
	 *              name, in string or ASN.1 DER encoded form, respectively).
	 *              There can be multiple names of the same type. {@code null}
	 *              is
	 ...
	 *                     if a parsing error occurs
	 */
	private static Set<List<?>> cloneAndCheckNames(Collection<List<?>> names)
			throws IOException {
	...
	/**
	 * Returns the name constraints criterion. The {@code X509Certificate} must
	 * have subject and subject alternative names that meet the specified name
	 * constraints.
	 * <p>
	 * The name constraints are returned as a byte array. This byte array
	 * contains the DER encoded form of the name constraints, as they would
	 * appear in the NameConstraints structure defined in RFC 3280 and X.509.
	 * The ASN.1 notation for this structure is supplied in the documentation
	 * for {@link #setNameConstraints(byte [] bytes) setNameConstraints(byte []
	 * bytes)}.
	 * <p>
	 * Note that the byte array returned is cloned to protect against subsequent
	 * modifications.
	 ...
	 * equal to zero, the {@code X509Certificates} must include a
	...
	/**
	 * Returns the policy criterion. The {@code X509Certificate} must include at
	 * least one of the specified policies in its certificate policies
	 * extension. If the {@code Set} returned is empty, then the
	 * {@code X509Certificate} must include at least some specified policy in
	 * its certificate policies extension. If the {@code Set} returned is
	 * {@code null}, no policy check will be performed.
	 *
	 * @return an immutable {@code Set} of certificate policy OIDs in string
	 *         format (or {@code null})
	 * @see #setPolicy
	 */
	...
	/**
	 * Returns a copy of the pathToNames criterion. The {@code X509Certificate}
	 * must not include name constraints that would prohibit building a path to
	 * the specified names. If the value returned is {@code null}, no
	 ...
	 * {@code Collection} returned may contain duplicate names (same name and
	 * name type).
	 * <p>
	 * Each name in the {@code Collection} may be specified either as a
	 * {@code String} or as an ASN.1 encoded byte array. For more details about
	...
	public String toString() {
		...
		sb.append("  matchAllSubjectAltNames flag: " + String.valueOf(
		...
		if (subjectKeyID != null) {
			...
			sb.append("  Subject Key Identifier: " + enc.encodeBuffer(
					subjectKeyID) + "\n");
		}
		if (authorityKeyID != null) {
			HexDumpEncoder enc = new HexDumpEncoder();
			sb.append("  Authority Key Identifier: " + enc.encodeBuffer(
					authorityKeyID) + "\n");
		}
		if (certificateValid != null) {
		...
		if (subjectPublicKeyAlgID != null) {
			sb.append("  Subject Public Key AlgID: " + subjectPublicKeyAlgID
					.toString() + "\n");
		}
	 ...
	 *
	 * @param cert
	 *              a {@code X509Certificate}
	 * @param extId
	 *              an {@code integer} which specifies the extension index.
	 *              Currently, the supported extensions are as follows: index 0
	 *              -
	 *              PrivateKeyUsageExtension index 1 -
	 *              SubjectAlternativeNameExtension index 2 -
	 *              NameConstraintsExtension index 3 -
	 *              CertificatePoliciesExtension index 4 -
	 *              ExtendedKeyUsageExtension
	 * @return an {@code Extension} object whose real type is as specified by
	 *         the extension oid.
	 * @throws IOException
	 *                     if cannot construct the {@code Extension} object with
	 *                     the
	 *                     extension encoding retrieved from the passed in
	...
	public boolean match(Certificate cert) {
		...
		if (debug != null) {
			debug.println("X509CertSelector.match(SN: " + (xcert
		...
		boolean result = matchBasicConstraints(xcert) && matchKeyUsage(xcert)
				&& matchExtendedKeyUsage(xcert) && matchSubjectKeyID(xcert)
				&& matchAuthorityKeyID(xcert) && matchPrivateKeyValid(xcert)
				&& matchSubjectPublicKeyAlgID(xcert) && matchPolicy(xcert)
				&& matchSubjectAlternativeNames(xcert) && matchPathToNames(
						xcert) && matchNameConstraints(xcert);
	...
	private boolean matchSubjectKeyID(X509Certificate xcert) {
		...
		try {
			...
			if (certSubjectKeyID == null || !Arrays.equals(subjectKeyID,
					certSubjectKeyID)) {
				if (debug != null) {
					debug.println("X509CertSelector.match: "
	...
	private boolean matchAuthorityKeyID(X509Certificate xcert) {
		...
		try {
			...
			if (certAuthKeyID == null || !Arrays.equals(authorityKeyID,
					certAuthKeyID)) {
				if (debug != null) {
					debug.println("X509CertSelector.match: "
	...
	private boolean matchPrivateKeyValid(X509Certificate xcert) {
		...
		try {
			ext = (PrivateKeyUsageExtension) getExtensionObject(xcert,
			...
			if (debug != null) {
				...
				debug.println("X509CertSelector.match: private key usage not "
						+ "within validity date; ext.NOT_After: " + time
			...
			if (debug != null) {
				String time = "n/a";
				try {
					Date notBefore = ext.get(
							PrivateKeyUsageExtension.NOT_BEFORE);
					time = notBefore.toString();
				} catch (CertificateException ex) {
					// not able to retrieve notBefore value
				}
				debug.println("X509CertSelector.match: private key usage not "
						+ "within validity date; ext.NOT_BEFORE: " + time
						+ "; X509CertSelector: " + this.toString());
				e2.printStackTrace();
			}
			return false;
			...
			if (debug != null) {
				debug.println("X509CertSelector.match: IOException in "
						+ "private key usage check; X509CertSelector: " + this
								.toString());
				e4.printStackTrace();
			}
			return false;
	...
	private boolean matchKeyUsage(X509Certificate xcert) {
		...
		if (certKeyUsage != null) {
			for (int keyBit = 0; keyBit < keyUsage.length; keyBit++) {
				if (keyUsage[keyBit] && ((keyBit >= certKeyUsage.length)
						|| !certKeyUsage[keyBit])) {
					if (debug != null) {
						debug.println("X509CertSelector.match: "
	...
	private boolean matchExtendedKeyUsage(X509Certificate xcert) {
		...
		try {
			ExtendedKeyUsageExtension ext = (ExtendedKeyUsageExtension) getExtensionObject(
					xcert, EXTENDED_KEY_USAGE_ID);
			if (ext != null) {
				Vector<ObjectIdentifier> certKeyPurposeVector = ext.get(
						ExtendedKeyUsageExtension.USAGES);
				if (!certKeyPurposeVector.contains(ANY_EXTENDED_KEY_USAGE)
						&& !certKeyPurposeVector.containsAll(
								keyPurposeOIDSet)) {
					if (debug != null) {
	...
	private boolean matchSubjectAlternativeNames(X509Certificate xcert) {
		if ((subjectAlternativeNames == null) || subjectAlternativeNames
				.isEmpty()) {
			return true;
		}
		try {
			SubjectAlternativeNameExtension sanExt = (SubjectAlternativeNameExtension) getExtensionObject(
					xcert, SUBJECT_ALT_NAME_ID);
			if (sanExt == null) {
				if (debug != null) {
					debug.println("X509CertSelector.match: "
			...
			GeneralNames certNames = sanExt.get(
					SubjectAlternativeNameExtension.SUBJECT_NAME);
			Iterator<GeneralNameInterface> i = subjectAlternativeGeneralNames
					.iterator();
			while (i.hasNext()) {
				GeneralNameInterface matchName = i.next();
				boolean found = false;
				for (Iterator<GeneralName> t = certNames.iterator(); t.hasNext()
						&& !found;) {
				...
				if (!found && (matchAllSubjectAltNames || !i.hasNext())) {
					if (debug != null) {
						debug.println(
								"X509CertSelector.match: subject alternative "
	...
	private boolean matchPolicy(X509Certificate xcert) {
		...
		try {
			CertificatePoliciesExtension ext = (CertificatePoliciesExtension) getExtensionObject(
					xcert, CERT_POLICIES_ID);
			if (ext == null) {
				if (debug != null) {
					debug.println("X509CertSelector.match: "
			...
			List<PolicyInformation> policies = ext.get(
					CertificatePoliciesExtension.POLICIES);
			/*
			 * Convert the Vector of PolicyInformation to a Vector of
			 * CertificatePolicyIds for easier comparison.
			 */
			List<CertificatePolicyId> policyIDs = new ArrayList<CertificatePolicyId>(
					policies.size());
			...
			if (policy != null) {
				...
				/*
				 * if the user passes in an empty policy Set, then we just want
				 * to make sure that the candidate certificate has some policy
				 * OID in its CertPoliciesExtension
				 */
	...
	private boolean matchPathToNames(X509Certificate xcert) {
		...
		try {
			NameConstraintsExtension ext = (NameConstraintsExtension) getExtensionObject(
					xcert, NAME_CONSTRAINTS_ID);
			if (ext == null) {
				return true;
			}
			if ((debug != null) && Debug.isOn("certpath")) {
				debug.println("X509CertSelector.match pathToNames:\n");
				Iterator<GeneralNameInterface> i = pathToGeneralNames
						.iterator();
				while (i.hasNext()) {
			...
			GeneralSubtrees permitted = ext.get(
					NameConstraintsExtension.PERMITTED_SUBTREES);
			GeneralSubtrees excluded = ext.get(
					NameConstraintsExtension.EXCLUDED_SUBTREES);
			if (excluded != null) {
	...
	private boolean matchExcluded(GeneralSubtrees excluded) {
		/*
		 * Enumerate through excluded and compare each entry to all pathToNames.
		 * If any pathToName is within any of the subtrees listed in excluded,
		 * return false.
		 */
				...
				if (excludedName.getType() == pathToName.getType()) {
					switch (pathToName.constrains(excludedName)) {
						...
						case GeneralNameInterface.NAME_MATCH:
							if (debug != null) {
								debug.println(
										"X509CertSelector.match: name constraints "
												+ "inhibit path to specified name");
								debug.println(
										"X509CertSelector.match: excluded name: "
												+ pathToName);
							}
							return false;
						default:
	...
	private boolean matchPermitted(GeneralSubtrees permitted) {
		/*
		 * Enumerate through pathToNames, checking that each pathToName is in at
		 * least one of the subtrees listed in permitted. If not, return false.
		 * However, if no subtrees of a given type are listed, all names of that
		 * type are permitted.
		 */
		Iterator<GeneralNameInterface> i = pathToGeneralNames.iterator();
		while (i.hasNext()) {
	...
	@SuppressWarnings("unchecked") // Safe casts assuming clone() works
									// correctly
	...
	public Object clone() {
		try {
			...
			if (subjectAlternativeNames != null) {
				copy.subjectAlternativeNames = cloneSet(
						subjectAlternativeNames);
				copy.subjectAlternativeGeneralNames = cloneSet(
