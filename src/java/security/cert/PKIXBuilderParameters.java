 ...
 * <p>
 * A PKIX {@code CertPathBuilder} uses these parameters to
 * {@link CertPathBuilder#build build} a {@code CertPath} which has been
 * validated according to the PKIX certification path validation algorithm.
 *
 * <p>
 * To instantiate a {@code PKIXBuilderParameters} object, an application must
 * specify one or more <i>most-trusted CAs</i> as defined by the PKIX
 * certification path validation algorithm. The most-trusted CA can be specified
 * using one of two constructors. An application can call
 * {@link #PKIXBuilderParameters(Set, CertSelector) PKIXBuilderParameters(Set,
 * CertSelector)}, specifying a {@code Set} of {@code TrustAnchor} objects, each
 * of which identifies a most-trusted CA. Alternatively, an application can call
 * {@link #PKIXBuilderParameters(KeyStore, CertSelector)
 * PKIXBuilderParameters(KeyStore, CertSelector)}, specifying a {@code KeyStore}
 ...
 * certificate that the {@code CertPathBuilder} will attempt to build a path to.
 * The constraints are specified as a {@code CertSelector} object. These
 * constraints should provide the {@code CertPathBuilder} with enough search
 * criteria to find the target certificate. Minimal criteria for an
 * {@code X509Certificate} usually include the subject name and/or one or more
 * subject alternative names. If enough criteria is not specified, the
 * {@code CertPathBuilder} may throw a {@code CertPathBuilderException}.
 * <p>
...
public class PKIXBuilderParameters extends PKIXParameters {
    ...
    /**
     * Creates an instance of {@code PKIXBuilderParameters} with the specified
    ...
    /**
     * Creates an instance of {@code PKIXBuilderParameters} that populates the
     * set of most-trusted CAs from the trusted certificate entries contained in
     * the specified {@code KeyStore}. Only keystore entries that contain
     * trusted {@code X509Certificate}s are considered; all other certificate
     * types are ignored.
     *
     * @param keystore
     ...
     * {@code CertPathBuilder} instance must not build paths longer than the
     * length specified.
     *
     * <p>
     ...
     * {@code pathLenConstraint} field of the extension overrides the maximum
     * path length parameter whenever the result is a certification path of
     * smaller length.
     *
     * @param maxPathLength
