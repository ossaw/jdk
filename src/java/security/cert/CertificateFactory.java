...
/**
 * This class defines the functionality of a certificate factory, which is used
 * to generate certificate, certification path ({@code CertPath}) and
 * certificate revocation list (CRL) objects from their encodings.
 * <p>
 * For encodings consisting of multiple certificates, use
 * {@code generateCertificates} when you want to parse a collection of possibly
 * unrelated certificates. Otherwise, use {@code generateCertPath} when you want
 * to generate a {@code CertPath} (a certificate chain) and subsequently
 ...
 * of {@code java.security.cert.X509CRL}.
 * <p>
 * The following example reads a file with Base64 encoded certificates, which
 * are each bounded at the beginning by -----BEGIN CERTIFICATE-----, and bounded
 * at the end by -----END CERTIFICATE-----. We convert the
 * {@code FileInputStream} (which does not support {@code mark} and
 * {@code reset}) to a {@code BufferedInputStream} (which supports those
 * methods), so that each call to {@code generateCertificate} consumes only one
...
public class CertificateFactory {
    ...
    /**
     * Returns a certificate factory object that implements the specified
     * certificate type.
     * <p>
     * This method traverses the list of registered security Providers, starting
     * with the most preferred Provider. A new CertificateFactory object
     * encapsulating the CertificateFactorySpi implementation from the first
    ...
    public static final CertificateFactory getInstance(String type) throws CertificateException {
        try {
            Instance instance = GetInstance.getInstance("CertificateFactory", CertificateFactorySpi.class,
                    type);
     ...
     * factory.
     *
     * @return the name of the certificate type associated with this certificate
     *         factory.
     */
     ...
     * the next available byte after the inherent end-of-certificate marker. If
     * the data in the input stream does not contain an inherent
     * end-of-certificate marker (other than EOF) and there is trailing data
     * after the certificate is parsed, a {@code CertificateException} is
     * thrown.
     *
     * @param inStream
     ...
     * default encoding. The name of the default encoding is the first element
     * of the {@code Iterator} returned by the {@link #getCertPathEncodings
    ...
    /**
     * Generates a {@code CertPath} object and initializes it with a
     * {@code List} of {@code Certificate}s.
     * <p>
     ...
     * collection view can be typecast to the corresponding certificate class.
     ...
     * {@code inStream} may contain a sequence of DER-encoded certificates in
     * the formats described for
     * {@link #generateCertificate(java.io.InputStream) generateCertificate}. In
     * addition, {@code inStream} may contain a PKCS#7 certificate chain. This
     * is a PKCS#7 <i>SignedData</i> object, with the only significant field
     * being <i>certificates</i>. In particular, the signature and the contents
     * are ignored. This format allows multiple certificates to be downloaded at
     * once. If no certificates are present, an empty collection is returned.
     * <p>
     * Note that if the given input stream does not support
     * {@link java.io.InputStream#mark(int) mark} and
     * {@link java.io.InputStream#reset() reset}, this method will consume the
     ...
     * {@link java.io.InputStream#mark(int) mark} and
     * {@link java.io.InputStream#reset() reset}, this method will consume the
     * entire input stream. Otherwise, each call to this method consumes one CRL
     * and the read position of the input stream is positioned to the next
     * available byte after the inherent end-of-CRL marker. If the data in the
     * input stream does not contain an inherent end-of-CRL marker (other than
     * EOF) and there is trailing data after the CRL is parsed, a
     ...
     * contain a sequence of DER-encoded CRLs. In addition, {@code inStream} may
     * contain a PKCS#7 CRL set. This is a PKCS#7 <i>SignedData</i> object, with
     * the only significant field being <i>crls</i>. In particular, the
