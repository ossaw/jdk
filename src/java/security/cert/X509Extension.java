 ...
 * associating additional attributes with users or public keys, for managing the
 ...
 * extension it does not recognize. A non-critical extension may be ignored if
 ...
 * </pre>
 * 
 * Since not all extensions are known, the {@code getExtensionValue} method
 * returns the DER-encoded OCTET STRING of the extension value (i.e., the
 * {@code extnValue}). This can then be handled by a <em>Class</em> that
...
public interface X509Extension {
	 ...
	 *
	 * @return {@code true} if a critical extension is found that is not
	 ...
	 * </pre>
	 * 
	 * @return a Set (or an empty Set if none are marked critical) of the
	 *         extension OID strings for extensions that are marked critical. If
	 *         there are no extensions present at all, then this method returns
	 *         null.
	 */
	 ...
	 * </pre>
	 *
	 * @return a Set (or an empty Set if none are marked non-critical) of the
	 *         extension OID strings for extensions that are marked
	 *         non-critical. If there are no extensions present at all, then
