 ...
 * PermissionCollections. For example, if any {@code java.io.FilePermission}
 * objects are added to an instance of this class, they are all stored in a
 * single PermissionCollection. It is the PermissionCollection returned by a
 * call to the {@code newPermissionCollection} method in the FilePermission
 * class. Similarly, any {@code java.lang.RuntimePermission} objects are stored
 * in the PermissionCollection returned by a call to the
 * {@code newPermissionCollection} method in the RuntimePermission class. Thus,
 * this class represents a collection of PermissionCollections.
 * <p>
 * When the {@code add} method is called to add a Permission, the Permission is
 * stored in the appropriate PermissionCollection. If no such collection exists
 * yet, the Permission object's class is determined and the
 * {@code newPermissionCollection} method is called on that class to create the
 * PermissionCollection and add it to the Permissions object. If
 * {@code newPermissionCollection} returns null, then a default
 * PermissionCollection that uses a hashtable will be created and used. Each
 * hashtable entry stores a Permission object as both the key and the value.
 * <p>
 * Enumerations returned via the {@code elements} method are not
 * <em>fail-fast</em>. Modifications to a collection should not be performed
...
public final class Permissions extends PermissionCollection implements Serializable {
     ...
     * FilePermission, it is added to the FilePermissionCollection stored in
     ...
     * the specified permission's class implies the permissions expressed in the
     ...
     * directory, and another FilePermission that specifies "write" access for
     * all files in the "/tmp/scratch/foo" directory. Then if the
     * {@code implies} method is called with a permission specifying both "read"
     * and "write" access to files in the "/tmp/scratch/foo" directory,
     ...
     * whose type is the same as that of <i>p</i>. For example, if <i>p</i> is a
     * FilePermission, the FilePermissionCollection stored in this Permissions
     ...
     * exist. To do so, it first calls the {@code newPermissionCollection}
     ...
     * when the {@code PermissionCollection.implies} method is called. If the
     * call returns a PermissionCollection, that collection is stored in this
     * Permissions object. If the call returns null and createEmpty is true,
     * then this method instantiates and stores a default PermissionCollection
    ...
    private PermissionCollection getUnresolvedPermissions(Permission p) {
        ...
        UnresolvedPermissionCollection uc = (UnresolvedPermissionCollection) permsMap.get(
     ...
     */
    private static final ObjectStreamField[] serialPersistentFields = { new ObjectStreamField("perms",
            Hashtable.class), new ObjectStreamField("allPermission", PermissionCollection.class), };
...
final class PermissionsHash extends PermissionCollection implements Serializable {
    ...
    private void readObject(ObjectInputStream in) throws IOException, ClassNotFoundException {
        ...
        // the perms key, so this cast is safe, unless the data is corrupt.
        @SuppressWarnings("unchecked")
        Hashtable<Permission, Permission> perms = (Hashtable<Permission, Permission>) gfields.get("perms",
                null);
