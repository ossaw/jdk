 ...
 * <p>
 * More specifically, it encapsulates a context and has a single method,
 * {@code checkPermission}, that is equivalent to the {@code checkPermission}
 * method in the AccessController class, with one difference: The
 * AccessControlContext {@code checkPermission} method makes access decisions
 ...
 * security check that should be made within a given context actually needs to
 * be done from within a <i>different</i> context (for example, from within a
 * worker thread).
 * <p>
 * An AccessControlContext is created by calling the
 * {@code AccessController.getContext} method. The {@code getContext} method
 * takes a "snapshot" of the current calling context, and places it in an
 ...
 * {@code checkPermission} method on the previously-saved AccessControlContext
...
public final class AccessControlContext {
     ...
     * {@code AccessControlContext} and {@code DomainCombiner}. This constructor
     * associates the provided {@code DomainCombiner} with the provided
     ...
     * currently in effect, and the context in this object. The request is
     * allowed only if every ProtectionDomain in the context implies the
     * permission. Otherwise the request is denied.
     * <p>
     * This method quietly returns if the access request is permitted, or throws
    ...
    public void checkPermission(Permission perm) throws AccessControlException {
            ...
            if (!dumpDebug) {
                ...
                for (int i = 0; context != null && i < context.length; i++) {
                    if (context[i].getCodeSource() != null && context[i].getCodeSource().getLocation() != null
                            && Debug.isOn("codebase=" + context[i].getCodeSource().getLocation()
                                    .toString())) {
                        dumpDebug = true;
                        break;
                    }
         ...
         * first domain was a Privileged system domain. This is to make the
         * common case for system code very fast
         */
    ...
    private void checkPermission2(Permission perm) {
        ...
        /*
         * Ignore the limited permissions and parent fields of a wrapper context
         * since they were already carried down into the unwrapped context.
         */
        if (isWrapped) {
            return;
        }
    ...
    /**
     * Take the stack-based context (this) and combine it with the privileged or
     * inherited context, if need be. Any limited privilege scope is flagged
     * regardless of whether the assigned context comes from an immediately
     * enclosing limited doPrivileged(). The limited privilege scope can
     * indirectly flow from the inherited parent thread or an assigned context
     * previously captured by getContext().
     */
    ...
    /**
     * Checks two AccessControlContext objects for equality. Checks that
     * <i>obj</i> is an AccessControlContext and has the same set of
     * ProtectionDomains as this context.
     * <P>
     * 
     * @param obj
     *        the object we are testing for equality with this object.
     * @return true if <i>obj</i> is an AccessControlContext, and has the same
     *         set of ProtectionDomains as this context, false otherwise.
     */
    ...
    /*
     * Compare for equality based on state that is free of limited privilege
     * complications.
     */
    ...
    /*
     * Compare for equality based on state that is captured during a call to
     * AccessController.getContext() when a limited privilege scope is in
     * effect.
     */
    private boolean equalLimitedContext(AccessControlContext that) {
        ...
        /*
         * Wrapped instances should never escape outside the implementation this
         * class and AccessController so this will probably never happen but it
        ...
        /*
         * The protection domains and combiner of a privilegedContext are not
         * relevant because they have already been included in the context of
         * this instance by optimize() so we only care about any limited
    ...
    /*
     * Follow the privilegedContext link making our best effort to skip through
     * any wrapper contexts.
     */
    ...
    /**
     * Returns the hash code value for this context. The hash code is computed
     * by exclusive or-ing the hash code of all the protection domains in the
     * context together.
     *
