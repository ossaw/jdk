 ...
 * keys. Key pair generators are constructed using the {@code getInstance}
 ...
 * randomness. The keysize is interpreted differently for different algorithms
 * (e.g., in the case of the <i>DSA</i> algorithm, the keysize corresponds to
 * the length of the modulus). There is an
 * {@link #initialize(int, java.security.SecureRandom) initialize} method in
 * this KeyPairGenerator class that takes these two universally shared types of
 * arguments. There is also one that takes just a {@code keysize} argument, and
 * uses the {@code SecureRandom} implementation of the highest-priority
 * installed provider as the source of randomness. (If none of the installed
 * providers supply an implementation of {@code SecureRandom}, a system-provided
 * source of randomness is used.)
 *
 * <p>
 * Since no other parameters are specified when you call the above
 * algorithm-independent {@code initialize} methods, it is up to the provider
 * what to do about the algorithm-specific parameters (if any) to be associated
 * with each of the keys.
 *
 * <p>
 * If the algorithm is the <i>DSA</i> algorithm, and the keysize (modulus size)
 * is 512, 768, or 1024, then the <i>Sun</i> provider uses a set of precomputed
 * values for the {@code p}, {@code q}, and {@code g} parameters. If the modulus
 * size is not one of the above values, the <i>Sun</i> provider creates a new
 * set of parameters. Other providers might have precomputed parameter sets for
 * more than just the three modulus sizes mentioned above. Still others might
 * not have a list of precomputed parameters at all and instead always create
 * new parameter sets.
 *
 * <li><b>Algorithm-Specific Initialization</b>
 * <p>
 * For situations where a set of algorithm-specific parameters already exists
 * (e.g., so-called <i>community parameters</i> in DSA), there are two
 * {@link #initialize(java.security.spec.AlgorithmParameterSpec) initialize}
 * methods that have an {@code AlgorithmParameterSpec} argument. One also has a
 * {@code SecureRandom} argument, while the the other uses the
 * {@code SecureRandom} implementation of the highest-priority installed
 * provider as the source of randomness. (If none of the installed providers
 * supply an implementation of {@code SecureRandom}, a system-provided source of
 * randomness is used.)
 * </ul>
 *
 * <p>
 * In case the client does not explicitly initialize the KeyPairGenerator (via a
 * call to an {@code initialize} method), each provider must supply (and
 * document) a default initialization. For example, the <i>Sun</i> provider uses
 * a default modulus size (keysize) of 1024 bits.
 *
 * <p>
 * Note that this class is abstract and extends from {@code KeyPairGeneratorSpi}
 * for historical reasons. Application developers should only take notice of the
 * methods defined in this {@code KeyPairGenerator} class; all the methods in
 ...
 * > KeyPairGenerator section</a> of the Java Cryptography Architecture Standard
...
public abstract class KeyPairGenerator extends KeyPairGeneratorSpi {
	...
	private static KeyPairGenerator getInstance(Instance instance, String algorithm) {
		...
		if (!skipDebug && pdebug != null) {
			pdebug.println(
					"KeyPairGenerator." + algorithm + " algorithm from: " + kpg.provider.getName());
		}
	 ...
	 * with the most preferred Provider. A new KeyPairGenerator object
	 * encapsulating the KeyPairGeneratorSpi implementation from the first
	 ...
	 *                if no Provider supports a KeyPairGeneratorSpi
	 ...
	 *
	 * @exception NoSuchAlgorithmException
	 *                if a KeyPairGeneratorSpi implementation for the specified
	...
	public static KeyPairGenerator getInstance(String algorithm, String provider)
		...
		Instance instance = GetInstance.getInstance("KeyPairGenerator", KeyPairGeneratorSpi.class,
				algorithm, provider);
		return getInstance(instance, algorithm);
	}
	 ...
	 * parameter set and the {@code SecureRandom} implementation of the
	 ...
	 * {@code SecureRandom}, a system-provided source of randomness is used.)
	 *
	 * @param keysize
	 ...
	 * the {@code SecureRandom} implementation of the highest-priority installed
	 * provider as the source of randomness. (If none of the installed providers
	 * supply an implementation of {@code SecureRandom}, a system-provided
	 * source of randomness is used.).
	 *
	 * <p>
	 * This concrete method has been added to this previously-defined abstract
	 * class. This method calls the KeyPairGeneratorSpi
	 * {@link KeyPairGeneratorSpi#initialize( java.security.spec.AlgorithmParameterSpec, java.security.SecureRandom)
	 * initialize} method, passing it {@code params} and a source of randomness
	 * (obtained from the highest-priority installed provider or system-provided
	 * if none of the installed providers supply one). That {@code initialize}
	 * method always throws an UnsupportedOperationException if it is not
	...
	/**
	 * Initializes the key pair generator with the given parameter set and
	 ...
	 * class. This method calls the KeyPairGeneratorSpi
	 * {@link KeyPairGeneratorSpi#initialize( java.security.spec.AlgorithmParameterSpec, java.security.SecureRandom)
	 * initialize} method, passing it {@code params} and {@code random}. That
	 * {@code initialize} method always throws an UnsupportedOperationException
	 ...
	 * rather than from KeyPairGenerator. It represents a KeyPairGenerator with
	 * an encapsulated, provider-supplied SPI object (of type
	 * KeyPairGeneratorSpi). If the provider implementation is an instance of
	 * KeyPairGeneratorSpi, the getInstance() methods above return an instance
	 * of this class, with the SPI object encapsulated.
	 *
	 ...
	 * been interposed in the hierarchy between the API (KeyPairGenerator) and
	 * its original parent (Object).
	 */
	...
	private static final class Delegate extends KeyPairGenerator {
		...
		Delegate(Instance instance, Iterator<Service> serviceIterator, String algorithm) {
			...
			if (!skipDebug && pdebug != null) {
				pdebug.println(
						"KeyPairGenerator." + algorithm + " algorithm from: " + provider.getName());
			}
		}

		/**
		 * Update the active spi of this class and return the next
		 * implementation for failover. If no more implemenations are available,
		 * this method returns null. However, the active spi of this class is
		 * never set to null.
		 */
