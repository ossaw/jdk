 ...
 * <p>
 * <b>Note:</b> Granting AllPermission should be done with extreme care, as it
 * implies all other permissions. Thus, it grants code the ability to run with
 * security disabled. Extreme caution should be taken before granting such a
 * permission to code. This permission should be used only during testing, or in
 * extremely rare cases where an application or applet is completely trusted and
 ...
 * AllPermission objects must be stored in a manner that allows them to be
 * inserted in any order, but enable the implies function to evaluate the
 * implies method in an efficient (and consistent) manner.
 *
 * @see java.security.Permission
 * @see java.security.Permissions
 * @author Roland Schemers
