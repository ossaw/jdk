...
/**
 * <p>
 * This class extends the concept of a codebase to encapsulate not only the
 * location (URL) but also the certificate chains that were used to verify
...
public class CodeSource implements java.io.Serializable {
    ...
    /**
     * Constructs a CodeSource and associates it with the specified location and
     * set of certificates.
     *
     * @param url
     *        the location (URL).
     ...
     * CodeSource objects are considered equal if their locations are of
     * identical value and if their signer certificate chains are of identical
     * value. It is not required that the certificate chains be in the same
     * order.
     *
     * @param obj
    ...
    public final URL getLocation() {
        /*
         * since URL is practically immutable, returning itself is not a
         * security problem
         */
     ...
     * {@link #CodeSource(URL url, CodeSigner[] signers)} constructor then its
     * certificate chains are extracted and used to create an array of
     * Certificate objects. Each signer certificate is followed by its
     * supporting certificate chain (which may be empty). Each signer
     * certificate and its supporting certificate chain is ordered bottom-to-top
     ...
     * constructor then its certificate chains are extracted and used to create
     * an array of CodeSigner objects. Note that only X.509 certificates are
     ...
     * CodeSource.
     * <p>
     * More specifically, this method makes the following checks. If any fail,
     * it returns false. If they all succeed, it returns true.
     * <ul>
     * <li><i>codesource</i> must not be null.
     * <li>If this object's certificates are not null, then all of this object's
     * certificates must be present in <i>codesource</i>'s certificates.
     * <li>If this object's location (getLocation()) is not null, then the
     * following checks are made against this object's location and
     * <i>codesource</i>'s:
     * <ul>
     * <li><i>codesource</i>'s location must not be null.
     * <li>If this object's location equals <i>codesource</i>'s location, then
     * return true.
     * <li>This object's protocol (getLocation().getProtocol()) must be equal to
     * <i>codesource</i>'s protocol, ignoring case.
     * <li>If this object's host (getLocation().getHost()) is not null, then the
     ...
     * <li>If this object's port (getLocation().getPort()) is not equal to -1
     * (that is, if a port is specified), it must equal <i>codesource</i>'s port
     * or default port (codesource.getLocation().getDefaultPort()).
     * <li>If this object's file (getLocation().getFile()) doesn't equal
     * <i>codesource</i>'s file, then the following checks are made: If this
     * object's file ends with "/-", then <i>codesource</i>'s file must start
     * with this object's file (exclusive the trailing "-"). If this object's
     * file ends with a "/*", then <i>codesource</i>'s file must start with this
     * object's file and must not have any further "/" separators. If this
     * object's file doesn't end with a "/", then <i>codesource</i>'s file must
     * match this object's file with a '/' appended.
     * <li>If this object's reference (getLocation().getRef()) is not null, it
     * must equal <i>codesource</i>'s reference.
     * </ul>
     * </ul>
     * <p>
     ...
     * "http://java.sun.com/classes/foo.jar" and null certificates:
     * 
     * <pre>
     *     http:
     ...
     * </pre>
     *
     * Note that if this CodeSource has a null location and a null certificate
    ...
    private boolean matchLocation(CodeSource that) {
        ...
        if (location.getFile().endsWith("/-")) {
            ...
            if ((!that.location.getFile().equals(location.getFile())) && (!that.location.getFile().equals(
                    location.getFile() + "/"))) {
                return false;
            }
        }
        ...
        if (thisHost != null) {
            if (("".equals(thisHost) || "localhost".equals(thisHost)) && ("".equals(thatHost) || "localhost"
                    .equals(thatHost))) {
                // ok
    ...
    /**
     * Returns a string describing this CodeSource, telling its URL and
     * certificates.
     *
     ...
     *
     * @serialData An initial {@code URL} is followed by an {@code int}
     *             indicating the number of certificates to follow (a value of
     *             "zero" denotes that there are no certificates associated with
     *             this object). Each certificate is written out starting with a
     *             {@code String} denoting the certificate type, followed by an
     *             {@code int} specifying the length of the certificate
     *             encoding, followed by the certificate encoding itself which
     *             is written out as an array of bytes. Finally, if any code
    ...
    private CodeSigner[] convertCertArrayToSignerArray(java.security.cert.Certificate[] certs) {
        ...
        try {
            ...
            while (i < certs.length) {
                ...
                while (j < certs.length && certs[j] instanceof X509Certificate && ((X509Certificate) certs[j])
