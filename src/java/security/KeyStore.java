 ...
 * certificates.
 * <p>
 * A {@code KeyStore} manages different types of entries. Each type of entry
 * implements the {@code KeyStore.Entry} interface. Three basic
 * {@code KeyStore.Entry} implementations are provided:
 * <ul>
 * <li><b>KeyStore.PrivateKeyEntry</b>
 * <p>
 * This type of entry holds a cryptographic {@code PrivateKey}, which is
 * optionally stored in a protected format to prevent unauthorized access. It is
 * also accompanied by a certificate chain for the corresponding public key.
 * <p>
 ...
 * <p>
 * This type of entry holds a cryptographic {@code SecretKey}, which is
 * optionally stored in a protected format to prevent unauthorized access.
 * <li><b>KeyStore.TrustedCertificateEntry</b>
 * <p>
 * This type of entry contains a single public key {@code Certificate} belonging
 * to another party. It is called a <i>trusted certificate</i> because the
 * keystore owner trusts that the public key in the certificate indeed belongs
 * to the identity identified by the <i>subject</i> (owner) of the certificate.
 * <p>
 * This type of entry can be used to authenticate other parties.
 * </ul>
 * <p>
 * Each entry in a keystore is identified by an "alias" string. In the case of
 * private keys and their associated certificate chains, these strings
 ...
 * differ in case.
 * <p>
 * Whether keystores are persistent, and the mechanisms used by the keystore if
 * it is persistent, are not specified here. This allows use of a variety of
 * techniques for protecting sensitive (e.g., private or secret) keys. Smart
 * cards or other integrated cryptographic engines (SafeKeyper) are one option,
 * and simpler mechanisms such as files may also be used (in a variety of
 * formats).
 * <p>
 ...
 * </pre>
 *
 * To create an empty keystore using the above {@code load} method, pass
 * {@code null} as the {@code InputStream} argument.
 * <p>
 * Once the keystore has been loaded, it is possible to read existing entries
 ...
 * following standard {@code KeyStore} type:
 * <ul>
 * <li>{@code PKCS12}</li>
 * </ul>
 * This type is described in the <a href=
 * "{@docRoot}/../technotes/guides/security/StandardNames.html#KeyStore"
 * > KeyStore section</a> of the Java Cryptography Architecture Standard
 * Algorithm Name Documentation. Consult the release documentation for your
 * implementation to see if any other types are supported.
 *
 * @author Jan Luehe
...
public class KeyStore {
     ...
     * default keystore type. In the Security properties file, the default
     * keystore type is given as: <pre> keystore.type=jks </pre>
     */
     ...
     * {@link #store(KeyStore.LoadStoreParameter) store} parameters.
     *
     * @since 1.5
     */
     ...
     * <p>
     * The information stored in a {@code ProtectionParameter} object protects
     * the contents of a keystore. For example, protection parameters may be
     * used to check the integrity of keystore data, or to protect the
     * confidentiality of sensitive keystore data (such as a {@code PrivateKey}
     * ).
     *
     * @since 1.5
     */
    ...
    public static class PasswordProtection implements ProtectionParameter, javax.security.auth.Destroyable {
        ...
        /**
         * Gets the name of the protection algorithm. If none was set then the
         * keystore provider will use its default protection algorithm. The name
         * of the default protection algorithm for a given keystore type is set
         * using the {@code 'keystore.<type>.keyProtectionAlgorithm'} security
         ...
         *
         * @return the algorithm parameter specification, or {@code  null}, if
         *         none was set
         * @since 1.8
         */
    ...
    public static interface Entry {
        ...
        /**
         * An attribute associated with a keystore entry. It comprises a name
        ...
        public interface Attribute {
            ...
            /**
             * Returns the attribute's value. Multi-valued attributes encode
             * their values as a single string.
             *
    ...
    /**
     * A {@code KeyStore} entry that holds a {@code PrivateKey} and
    ...
    public static final class PrivateKeyEntry implements Entry {
         ...
         * corresponding certificate chain.
         * <p>
         * The specified {@code chain} is cloned before it is stored in the new
         ...
         * <p>
         * The specified {@code chain} and {@code attributes} are cloned before
        ...
        public PrivateKeyEntry(PrivateKey privateKey, Certificate[] chain, Set<Attribute> attributes) {
            ...
            for (int i = 1; i < clonedChain.length; i++) {
                if (!certType.equals(clonedChain[i].getType())) {
                    throw new IllegalArgumentException("chain does not contain certificates "
                            + "of the same type");
                }
            }
         ...
         *
         * @return an array of {@code Certificate}s corresponding to the
         *         certificate chain for the public key. If the certificates are
         *         of type X.509, the runtime type of the returned array is
        ...
        /**
         * Gets the end entity {@code Certificate} from the certificate chain in
         * this entry.
         *
         * @return the end entity {@code Certificate} (at index 0) from the
         *         certificate chain in this entry. If the certificate is of
         *         type X.509, the runtime type of the returned certificate is
         *         {@code X509Certificate}.
         */
    ...
    public static final class SecretKeyEntry implements Entry {
         ...
         * <p>
         * The specified {@code attributes} is cloned before it is stored in the
    ...
    /**
     * A {@code KeyStore} entry that holds a trusted {@code Certificate}.
     *
     * @since 1.5
     */
    public static final class TrustedCertificateEntry implements Entry {
        ...
        /**
         * Constructs a {@code TrustedCertificateEntry} with a trusted
         * {@code Certificate}.
         *
         * @param trustedCert
        ...
        /**
         * Constructs a {@code TrustedCertificateEntry} with a trusted
         * {@code Certificate} and associated entry attributes.
         * <p>
         * The specified {@code attributes} is cloned before it is stored in the
    ...
    protected KeyStore(KeyStoreSpi keyStoreSpi, Provider provider, String type) {
        ...
        if (!skipDebug && pdebug != null) {
            pdebug.println("KeyStore." + type.toUpperCase() + " type from: " + this.provider.getName());
        }
    }
     ...
     * with the most preferred Provider. A new KeyStore object encapsulating the
     * KeyStoreSpi implementation from the first Provider that supports the
     ...
     * Returns a keystore object of the specified type.
     * <p>
     * A new KeyStore object encapsulating the KeyStoreSpi implementation from
     ...
     * (acronym for {@literal "Java keystore"}) if no such property exists.
     * <p>
     * The default keystore type can be used by applications that do not want to
     * use a hard-coded keystore type when calling one of the
     * {@code getInstance} methods, and want to provide a default keystore type
     * in case a user does not specify its own.
     * <p>
     ...
     *         {@literal "jks"} if no such property exists.
    ...
    public final void setKeyEntry(String alias, Key key, char[] password, Certificate[] chain)
        ...
        if ((key instanceof PrivateKey) && (chain == null || chain.length == 0)) {
            throw new IllegalArgumentException("Private key must be " + "accompanied by certificate "
                    + "chain");
        }
     ...
     * {@code key} must be encoded as an {@code EncryptedPrivateKeyInfo} as
     * defined in the PKCS #8 standard.
     * <p>
     * If the given alias already exists, the keystore information associated
     * with it is overridden by the given key (and possibly certificate chain).
     *
     ...
     * call to {@code setKeyEntry}, or created by a call to {@code setEntry}
    ...
    /**
     * Returns true if the entry identified by the given alias was created by a
     * call to {@code setCertificateEntry}, or created by a call to
     ...
     *            could not be stored
     */
    public final void store(OutputStream stream, char[] password) throws KeyStoreException, IOException,
     ...
     * <p>
     * A password may be given to unlock the keystore (e.g. the keystore resides
     * on a hardware token device), or to check the integrity of the keystore
     * data. If a password is not given for integrity checking, then integrity
     * checking is not performed.
     * <p>
     * In order to create an empty keystore, or if the keystore cannot be
     * initialized from a stream, pass {@code null} as the {@code stream}
     * argument.
     * <p>
     * Note that if this keystore has already been loaded, it is reinitialized
     ...
     *            loaded
     */
    public final void load(InputStream stream, char[] password) throws IOException, NoSuchAlgorithmException,
     ...
     * @since 1.5
     */
    public final void load(LoadStoreParameter param) throws IOException, NoSuchAlgorithmException,
            CertificateException {
    ...
    /**
     * Gets a keystore {@code Entry} for the specified alias with the specified
     ...
     * @since 1.5
     */
    public final Entry getEntry(String alias, ProtectionParameter protParam) throws NoSuchAlgorithmException,
    ...
    /**
     * Saves a keystore {@code Entry} under the specified alias. The protection
     * parameter is used to protect the {@code Entry}.
     * <p>
    ...
    /**
     * Determines if the keystore {@code Entry} for the specified {@code alias}
     ...
     * instantiate and initialize a KeyStore object. That process is triggered
     * when the {@linkplain #getKeyStore} method is called.
     * <p>
     * This makes it possible to decouple configuration from KeyStore object
     * creation and e.g. delay a password prompt until it is needed.
     *
     * @see KeyStore
    ...
    public static abstract class Builder {
         ...
         * {@link KeyStore.Entry Entry} with the given alias. The
         * {@code getKeyStore} method must be invoked before this method may be
         * called.
         *
        ...
        /**
         * Returns a new Builder that encapsulates the given KeyStore. The
         * {@linkplain #getKeyStore} method of the returned object will return
         * {@code keyStore}, the {@linkplain #getProtectionParameter
         * getProtectionParameter()} method will return
         * {@code protectionParameters}.
         * <p>
         * This is useful if an existing KeyStore object needs to be used with
         * Builder-based APIs.
         *
         ...
         * builder will create a KeyStore of type {@code type} and call its
         * {@link KeyStore#load load()} method. The {@code inputStream} argument
         * is constructed from {@code file}. If {@code protection} is a
         * {@code PasswordProtection}, the password is obtained by calling the
         * {@code getPassword} method. Otherwise, if {@code protection} is a
         * {@code CallbackHandlerProtection}, the password is obtained by
         * invoking the CallbackHandler.
         * <p>
         * Subsequent calls to {@link #getKeyStore} return the same object as
         * the initial call. If the initial call to failed with a
         * KeyStoreException, subsequent calls also throw a KeyStoreException.
         * <p>
         * The KeyStore is instantiated from {@code provider} if non-null.
         * Otherwise, all installed providers are searched.
         * <p>
         ...
         * {@code load} method.
         * <p>
         * <em>Note</em> that the {@link #getKeyStore} method is executed within
         * the {@link AccessControlContext} of the code invoking this method.
         *
         * @return a new Builder object
         * @param type
        ...
        public static Builder newInstance(String type, Provider provider, File file,
            ...
            if ((protection instanceof PasswordProtection == false)
                ...
                throw new IllegalArgumentException("Protection must be PasswordProtection or "
                        + "CallbackHandlerProtection");
            }
            if (file.isFile() == false) {
                throw new IllegalArgumentException("File does not exist or it does not refer "
                        ...
                        try {
                            ...
                            if (protection instanceof PasswordProtection) {
                                ...
                                CallbackHandler handler = ((CallbackHandlerProtection) protection)
                                        .getCallbackHandler();
                                PasswordCallback callback = new PasswordCallback("Password for keystore "
                                        + file.getName(), false);
         ...
         * Returns a new Builder object.
         * <p>
         * Each call to the {@link #getKeyStore} method on the returned builder
         * will return a new KeyStore object of type {@code type}. Its
         * {@link KeyStore#load(KeyStore.LoadStoreParameter) load()} method is
         * invoked using a {@code LoadStoreParameter} that encapsulates
         * {@code protection}.
         * <p>
         * The KeyStore is instantiated from {@code provider} if non-null.
         * Otherwise, all installed providers are searched.
