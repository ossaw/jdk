 ...
 * <LI>check to see if a particular permission is implied in the collection,
 ...
 * of Permission object should first be called. The default behavior (from the
 ...
 * the {@code PermissionCollection.implies} method is called. If a non-null
 * value is returned, that PermissionCollection must be used. If null is
 ...
 * collection, which stores only Permission objects for a given Permission type.
 * A PermissionCollection may also be heterogeneous. For example, Permissions is
 * a PermissionCollection subclass that represents a collection of
 * PermissionCollections. That is, its members are each a homogeneous
 ...
 * PermissionCollection should call the {@code newPermissionCollection} method
 * on the permission's class to see if it requires a special
 * PermissionCollection. If {@code newPermissionCollection} returns null, the
 * PermissionCollection is free to store the permission in any type of
 * PermissionCollection it desires (one using a Hashtable, one using a Vector,
 ...
 * synchronized properly. Furthermore, Enumerations returned via the
 * {@code elements} method are not <em>fail-fast</em>. Modifications to a
 * collection should not be performed while enumerating over that collection.
 *
 * @see Permission
 * @see Permissions
 *
...
public abstract class PermissionCollection implements java.io.Serializable {
     ...
     * </pre>
     *
     * {@code super.toString} is a call to the {@code toString} method of this
     * object's superclass, which is Object. The result is this
