...
/**
 * Hash table based mapping, which uses weak references to store keys and
 * reference-equality in place of object-equality to compare them. An entry will
 * automatically be removed when its key is no longer in ordinary use. Both null
 * values and the null key are supported. This class does not require additional
 * synchronization. A thread-safety is provided by a fragile combination of
 * synchronized blocks and volatile fields. Be very careful during editing!
 *
...
abstract class WeakIdentityMap<T> {
	...
	private static final int MAXIMUM_CAPACITY = 1 << 30; // it MUST be a power
															// of two
	private static final Object NULL = new Object(); // special object for null
														// key

	private final ReferenceQueue<Object> queue = new ReferenceQueue<Object>();

	private volatile Entry<T>[] table = newTable(1 << 3); // table's length MUST
															// be a power of two
		...
		synchronized (NULL) {
			...
			if (++this.size >= this.threshold) {
				if (this.table.length == MAXIMUM_CAPACITY) {
					...
					// If ignoring null elements and processing ref queue caused
					// massive
					// shrinkage, then restore old table. This should be rare,
					// but avoids
