 ...
 * the abstract <code>PersistenceDelegate</code> class and is the delegate used
 * by default for classes about which no information is available. The
 * <code>DefaultPersistenceDelegate</code> provides, version resilient, public
 * API-based persistence for classes that follow the JavaBeans&trade;
 * conventions without any class specific configuration.
 * <p>
 ...
 * providing code-free persistence for JavaBeans, the
 * <code>DefaultPersistenceDelegate</code> provides a convenient means to effect
 * persistent storage for classes that have a constructor that, while not
 * nullary, simply requires some property values as arguments.
 *
...
public class DefaultPersistenceDelegate extends PersistenceDelegate {
    ...
    /**
     * Creates a default persistence delegate for a class with a constructor
     * whose arguments are the values of the property names as specified by
     * <code>constructorPropertyNames</code>. The constructor arguments are
     * created by evaluating the property names in the order they are supplied.
     * To use this class to specify a single preferred constructor for use in
     * the serialization of a particular type, we state the names of the
     * properties that make up the constructor's arguments. For example, the
     * <code>Font</code> class which does not define a nullary constructor can
     * be handled with the following persistence delegate:
     *
     * <pre>
     ...
     * the class of <code>oldInstance</code> explicitly declares an "equals"
     * method this method returns the value of
     * <code>oldInstance.equals(newInstance)</code>. Otherwise, this method uses
     * the superclass's definition which returns true if the classes of the two
     * instances are equal.
     *
     * @param oldInstance
    ...
    protected boolean mutatesTo(Object oldInstance, Object newInstance) {
        ...
        return (constructor.length == 0) || !definesEquals(oldInstance) ? super.mutatesTo(oldInstance,
                newInstance) : oldInstance.equals(newInstance);
    }

    /**
     * This default implementation of the <code>instantiate</code> method
     * returns an expression containing the predefined method name "new" which
     * denotes a call to a constructor with the arguments as specified in the
    ...
    private void doProperty(Class<?> type, PropertyDescriptor pd, Object oldInstance, Object newInstance,
                    ...
                    for (int i = 0; i < a.length; i = i + 3) {
                        try {
                            ...
                            if (f.get(null).equals(oldValue)) {
                                out.remove(oldValue);
                                out.writeExpression(new Expression(oldValue, f, "get", new Object[] {
                                        null }));
                            }
                        } catch (Exception ex) {}
                    }
    ...
    private void initBean(Class<?> type, Object oldInstance, Object newInstance, Encoder out) {
        ...
        for (EventSetDescriptor d : info.getEventSetDescriptors()) {
            ...
            // Asssume the listeners are in the same order and that there are no
            // gaps.
     ...
     * all state held in objects of this type is exposed via the matching pairs
     * of "setter" and "getter" methods in the order they are returned by the
     * Introspector. If a property descriptor defines a "transient" attribute
     * with a value equal to <code>Boolean.TRUE</code> the property is ignored
     * by this default implementation. Note that this use of the word
     * "transient" is quite independent of the field modifier that is used by
     * the <code>ObjectOutputStream</code>.
     * <p>
     * For each non-transient property, an expression is created in which the
     * nullary "getter" method is applied to the <code>oldInstance</code>. The
     * value of this expression is the value of the property in the instance
     * that is being serialized. If the value of this expression in the cloned
     * environment <code>mutatesTo</code> the target value, the new value is
     * initialized to make it equivalent to the old value. In this case, because
     * the property value has not changed there is no need to call the
     * corresponding "setter" method and no statement is emitted. If not
     * however, the expression for this value is replaced with another
     * expression (normally a constructor) and the corresponding "setter" method
     * is called to install the new property value in the object. This scheme
     * removes default information from the output produced by streams using
     * this delegate.
     * <p>
     * In passing these statements to the output stream, where they will be
     * executed, side effects are made to the <code>newInstance</code>. In most
     * cases this allows the problem of properties whose values depend on each
     * other to actually help the serialization process by making the number of
     * statements that need to be written to the output smaller. In general, the
     * problem of handling interdependent properties is reduced to that of
     * finding an order for the properties in a class such that no property
