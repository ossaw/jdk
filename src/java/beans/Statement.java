...
/**
 * A <code>Statement</code> object represents a primitive statement in which a
 * single method is applied to a target and a set of arguments - as in
 * <code>"a.setFoo(b)"</code>. Note that where this example uses names to denote
 * the target and its argument, a statement object does not require a name space
 * and is constructed with the values themselves. The statement object
 * associates the named method with its environment as a simple set of values:
...
public class Statement {
	...
	/**
	 * Creates a new {@link Statement} object for the specified target object to
	 * invoke the method specified by the name and by the array of arguments.
	 * <p>
	 * The {@code target} and the {@code methodName} values should not be
	 * {@code null}. Otherwise an attempt to execute this {@code Expression}
	 * will result in a {@code NullPointerException}. If the {@code arguments}
	 * value is {@code null}, an empty array is used as the value of the
	...
	/**
	 * Returns the target object of this statement. If this method returns
	 * {@code null}, the {@link #execute} method throws a
	...
	/**
	 * Returns the name of the method to invoke. If this method returns
	 * {@code null}, the {@link #execute} method throws a
	 * {@code NullPointerException}.
	 *
	...
	/**
	 * Returns the arguments for the method to invoke. The number of arguments
	 * and their types must match the method being called. {@code null} can be
	 * used as a synonym of an empty array.
	 *
	 ...
	 * of the target and arguments are used in place of the compile-time type
	 * information and, like the {@link java.lang.reflect.Method} class itself,
	 * conversion between primitive values and their associated wrapper classes
	 * is handled internally.
	 * <p>
	 * The following method types are handled as special cases:
	 * <ul>
	 * <li>Static methods may be called by using a class object as the target.
	 * <li>The reserved method name "new" may be used to call a class's
	 * constructor as if all classes defined static "new" methods. Constructor
	 * invocations are typically considered {@code Expression}s rather than
	 * {@code Statement}s as they return a value.
	 * <li>The method names "get" and "set" defined in the
	 * {@link java.util.List} interface may also be applied to array instances,
	 * mapping to the static methods of the same name in the {@code Array}
	 * class.
	 * </ul>
	 *
	...
	private Object invokeInternal() throws Exception {
		...
		if (target instanceof Class) {
			/*
			 * For class methods, simluate the effect of a meta class by taking
			 * the union of the static methods of the actual class, with the
			 * instance methods of "Class.class" and the overloaded
			 * "newInstance" methods defined by the constructors. This way
			 * "System.class", for example, will perform both the static method
			 * getProperties() and the instance method getSuperclass() defined
			 * in "Class.class".
			 */
			 ...
			 * involving arrays much shorter and simplifies the archiving
			 * infrastrcure. The Array.set() method introduces an unusual idea -
			 * that of a static method changing the state of an instance.
			 * Normally statements with side effects on objects are instance
