 ...
 * <p>
 * PropertyEditor supports a variety of different kinds of ways of displaying
 * and updating property values. Most PropertyEditors will only need to support
 * a subset of the different options available in this API.
 * <P>
 * Simple PropertyEditors may only support the getAsText and setAsText methods
 * and need not support (say) paintValue or getCustomEditor. More complex types
 * may be unable to support getAsText and setAsText but will instead support
 * paintValue and getCustomEditor.
 * <p>
 ...
 * non-null String[] from getTags() and return a non-null value from getAsText
 * or (3) simply return a non-null String from getAsText().
 * <p>
 * Every property editor must support a call on setValue when the argument
 * object is of the type for which this is the corresponding propertyEditor. In
 * addition, each property editor must either support a custom editor, or
 * support setAsText.
 * <p>
...
public interface PropertyEditor {
	...
	/**
	 * Adds a listener for the value change. When the property editor changes
	 * its value it should fire a {@link PropertyChangeEvent} on all registered
	 * {@link PropertyChangeListener}s, specifying the {@code null} value for
