...
/**
 * The <code>XMLEncoder</code> class is a complementary alternative to the
 * <code>ObjectOutputStream</code> and can used to generate a textual
 * representation of a <em>JavaBean</em> in the same way that the
 * <code>ObjectOutputStream</code> can be used to create binary representation
 * of <code>Serializable</code> objects. For example, the following fragment can
 * be used to create a textual representation the supplied <em>JavaBean</em> and
 * all its properties:
 * 
 * <pre>
 ...
 * </pre>
 * 
 * Despite the similarity of their APIs, the <code>XMLEncoder</code> class is
 * exclusively designed for the purpose of archiving graphs of <em>JavaBean</em>
 * s as textual representations of their public properties. Like Java source
 * files, documents written this way have a natural immunity to changes in the
 * implementations of the classes involved. The <code>ObjectOutputStream</code>
 ...
 * <em>JavaBean</em>s in which they are represented as XML documents complying
 * with version 1.0 of the XML specification and the UTF-8 character encoding of
 * the Unicode/ISO 10646 character set. The XML documents produced by the
 * <code>XMLEncoder</code> class are:
 * <ul>
 * <li><em>Portable and version resilient</em>: they have no dependencies on the
 * private implementation of any class and so, like Java source files, they may
 * be exchanged between environments which may have different versions of some
 * of the classes and between VMs from different vendors.
 * <li><em>Structurally compact</em>: The <code>XMLEncoder</code> class uses a
 * <em>redundancy elimination</em> algorithm internally so that the default
 * values of a Bean's properties are not written to the stream.
 * <li><em>Fault tolerant</em>: Non-structural errors in the file, caused either
 * by damage to the file or by API changes made to classes in an archive remain
 * localized so that a reader can report the error and continue to load the
 * parts of the document which were not affected by the error.
 * </ul>
 * <p>
 * Below is an example of an XML archive containing some user interface
 * components from the <em>swing</em> toolkit:
 * 
 * <pre>
 ...
 * <ul>
 * <li>Each element represents a method call.
 * <li>The "object" tag denotes an <em>expression</em> whose value is to be used
 * as the argument to the enclosing element.
 * <li>The "void" tag denotes a <em>statement</em> which will be executed, but
 * whose result will not be used as an argument to the enclosing method.
 * <li>Elements which contain elements use those elements as arguments, unless
 * they have the tag: "void".
 * <li>The name of the method is denoted by the "method" attribute.
 * <li>XML's standard "id" and "idref" attributes are used to make references to
 * previous expressions - so as to deal with circularities in the object graph.
 * <li>The "class" attribute is used to specify the target of a static method or
 * constructor explicitly; its value being the fully qualified name of the
 * class.
 * <li>Elements with the "void" tag are executed using the outer context as the
 * target if no target is defined by a "class" attribute.
 * <li>Java's String class is treated specially and is written
 * &lt;string&gt;Hello, world&lt;/string&gt; where the characters of the string
 * are converted to bytes using the UTF-8 character encoding.
 * </ul>
 * <p>
 ...
 * <p>
 * <ul>
 * <li>The default method name is "new".
 * <li>A reference to a java class is written in the form
 * &lt;class&gt;javax.swing.JButton&lt;/class&gt;.
 * <li>Instances of the wrapper classes for Java's primitive types are written
 * using the name of the primitive type as the tag. For example, an instance of
 * the <code>Integer</code> class could be written: &lt;int&gt;123&lt;/int&gt;.
 * Note that the <code>XMLEncoder</code> class uses Java's reflection package in
 * which the conversion between Java's primitive types and their associated
 * "wrapper classes" is handled internally. The API for the
 * <code>XMLEncoder</code> class itself deals only with <code>Object</code>s.
 * <li>In an element representing a nullary method whose name starts with "get",
 * the "method" attribute is replaced with a "property" attribute whose value is
 * given by removing the "get" prefix and decapitalizing the result.
 * <li>In an element representing a monadic method whose name starts with "set",
 * the "method" attribute is replaced with a "property" attribute whose value is
 * given by removing the "set" prefix and decapitalizing the result.
 * <li>In an element representing a method named "get" taking one integer
 * argument, the "method" attribute is replaced with an "index" attribute whose
 * value the value of the first argument.
 * <li>In an element representing a method named "set" taking two arguments, the
 * first of which is an integer, the "method" attribute is replaced with an
 * "index" attribute whose value the value of the first argument.
 * <li>A reference to an array is written using the "array" tag. The "class" and
 * "length" attributes specify the sub-type of the array and its length
 * respectively.
 * </ul>
 * <p>
 * For more information you might also want to check out
 * <a href="http://java.sun.com/products/jfc/tsc/articles/persistence4">Using
 * XMLEncoder</a>, an article in <em>The Swing Connection.</em>
 * 
 * @see XMLDecoder
...
public class XMLEncoder extends Encoder implements AutoCloseable {
    ...
    private class ValueData {
        public int refs = 0;
        public boolean marked = false; // Marked -> refs > 0 unless ref was a
                                       // target.
    ...
    /**
     * Creates a new XML encoder to write out <em>JavaBeans</em> to the stream
     * <code>out</code> using an XML encoding.
     *
     * @param out
    ...
    /**
     * Creates a new XML encoder to write out <em>JavaBeans</em> to the stream
     * <code>out</code> using the given <code>charset</code> starting from the
    ...
    /**
     * Records the Statement so that the Encoder will produce the actual output
    ...
    public void writeStatement(Statement oldStm) {
        ...
        try {
            ...
            /*
             * Note we must do the mark first as we may require the results of
             * previous values in this context for this statement. Test case is:
             * os.setOwner(this); os.writeObject(this);
             */
            mark(oldStm);
            ...
            if (target instanceof Field) {
                ...
                if ((method == null) || (args == null)) {} else if (method.equals("get")
                        && (args.length == 1)) {
                    target = args[0];
            ...
            getExceptionListener().exceptionThrown(new Exception("XMLEncoder: discarding statement " + oldStm,
                    e));
        }
    ...
    /**
     * Records the Expression so that the Encoder will produce the actual output
     * when the stream is flushed.
     * <P>
     * This method should only be invoked within the context of initializing a
     * persistence delegate or setting up an encoder to read from a resource
     * bundle.
     * <P>
     * For more information about using resource bundles with the XMLEncoder,
    ...
    /**
     * This method writes out the preamble associated with the XML encoding if
     * it has not been written already and then writes out all of the values
     * that been written to the stream since the last time <code>flush</code>
     * was called. After flushing, all internal references to the values that
     * were written to this stream are cleared.
     */
    public void flush() {
        if (!preambleWritten) { // Don't do this in constructor - it throws ...
            ...
            writeln("<java version=" + quote(System.getProperty("java.version")) + " class=" + quote(
                    XMLDecoder.class.getName()) + ">");
    ...
    /**
     * This method calls <code>flush</code>, writes the closing postamble and
     * then closes the output stream associated with this stream.
     */
    public void close() {
        flush();
        writeln("</java>");
     ...
     * Unicode code point, and pairs of Unicode <em>surrogate characters</em>
     * can be combined to encode Unicode code point in documents containing only
    ...
    private void outputValue(Object value, Object outer, boolean isArgument) {
        ...
        if (d.exp != null) {
            ...
            if (isArgument && target instanceof Field && methodName.equals("get")) {
                ...
                writeln("<object class=" + quote(f.getDeclaringClass().getName()) + " field=" + quote(f
                        .getName()) + "/>");
                return;
            }
    ...
    private void outputStatement(Statement exp, Object outer, boolean isArgument) {
        ...
        if (target == outer) {} else if (target == Array.class && methodName.equals("newInstance")) {
            tag = "array";
        ...
        } else if ((!expression && methodName.startsWith("set") && args.length == 1) || (expression
                && methodName.startsWith("get") && args.length == 0)) {
            if (3 < methodName.length()) {
                attributes = attributes + " property=" + quote(Introspector.decapitalize(methodName.substring(
                        3)));
            }
