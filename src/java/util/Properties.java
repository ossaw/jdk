...
/**
 * The {@code Properties} class represents a persistent set of properties. The
 * {@code Properties} can be saved to a stream or loaded from a stream. Each key
 ...
 * <p>
 * Because {@code Properties} inherits from {@code Hashtable}, the {@code put}
 * and {@code putAll} methods can be applied to a {@code Properties} object.
 * Their use is strongly discouraged as they allow the caller to insert entries
 * whose keys or values are not {@code Strings}. The {@code setProperty} method
 * should be used instead. If the {@code store} or {@code save} method is called
 * on a "compromised" {@code Properties} object that contains a non-
 * {@code String} key or value, the call will fail. Similarly, the call to the
 * {@code propertyNames} or {@code list} method will fail if it is called on a
 * "compromised" {@code Properties} object that contains a non-{@code String}
 * key.
 * <p>
 * The {@link #load(java.io.Reader) load(Reader)} <tt>/</tt>
 * {@link #store(java.io.Writer, java.lang.String) store(Writer, String)}
 * methods load and store properties from and to a character based stream in a
 * simple line-oriented format specified below.
 ...
 * be written using Unicode escapes as defined in section 3.3 of <cite>The
 * Java&trade; Language Specification</cite>; only a single 'u' character is
 ...
 * used, however a specific encoding may be specified if required.
 * Implementations are required to support UTF-8 and UTF-16 and may support
 * other encodings. An XML properties document has the following DOCTYPE
 * declaration:
 *
 * <pre>
 ...
 *
 * @see <a href="../../../technotes/tools/solaris/native2ascii.html">
 *      native2ascii tool for Solaris</a>
 * @see <a href="../../../technotes/tools/windows/native2ascii.html">
...
public class Properties extends Hashtable<Object, Object> {
     ...
     * the end of the stream. A natural line may be either a blank line, a
     * comment line, or hold all or some of a key-element pair. A logical line
     * holds all the data of a key-element pair, which may be spread out across
     * several adjacent natural lines by escaping the line terminator sequence
     * with a backslash character {@code \}. Note that a comment line cannot be
     * extended in this manner; every natural line that is a comment must have
     * its own comment indicator, as described below. Lines are read from input
     * until the end of the stream is reached.
     * <p>
     * A natural line that contains only white space characters is considered
     * blank and is ignored. A comment line has an ASCII {@code '#'} or
     * {@code '!'} as its first non-white space character; comment lines are
     * also ignored and do not encode key-element information. In addition to
     * line terminators, this format considers the characters space ({@code ' '}
     * , {@code '\u005Cu0020'}), tab ({@code '\t'}, {@code '\u005Cu0009'}), and
     * form feed ({@code '\f'}, {@code '\u005Cu000C'}) to be white space.
     * <p>
     ...
     * input is processed from left to right, a non-zero even number of 2
     * <i>n</i> contiguous backslashes before a line terminator (or elsewhere)
     ...
     * unescaped {@code '='}, {@code ':'}, or white space character other than a
     * line terminator. All of these key termination characters may be included
     * in the key by escaping them with a preceding backslash character; for
     * example,
     * <p>
     * {@code \:\=}
     * <p>
     * would be the two-character key {@code ":="}. Line terminator characters
     * can be included using {@code \r} and {@code \n} escape sequences. Any
     * white space after the key is skipped; if the first non-white space
     * character after the key is {@code '='} or {@code ':'}, then it is ignored
     * and any white space characters after it are also skipped. All remaining
     * characters on the line become part of the associated element string; if
     * there are no remaining characters, the element is the empty string
     * {@code ""}. Once the raw character sequences constituting the key and
     * element are identified, escape processing is performed as described
     * above.
     * <p>
     ...
     * </pre>
     * 
     * specifies that the key is {@code "cheeses"} and the associated element is
     ...
     * string literals (see sections 3.3 and 3.10.6 of <cite>The Java&trade;
     ...
     * <li>The character sequence {@code \b} does <i>not</i> represent a
     * backspace character.
     * <li>The method does not treat a backslash character, {@code \}, before a
     * non-valid escape character as an error; the backslash is silently
     * dropped. For example, in a Java string the sequence {@code "\z"} would
     ...
     * {@code "\b"} as equivalent to the single character {@code 'b'}.
     ...
     * \u0009 and \u000c) from the beginning of a "natural line". Method returns
        ...
        while (current < len) {
            ...
            if (c > '\u00ff' || c == '\n' || c == '\r') {
                ...
                if (c > '\u00ff') {
                    ...
                    if (current == len - 1 || (comments.charAt(current + 1) != '#' && comments.charAt(current
                            + 1) != '!'))
                        bw.write("#");
                }
                last = current + 1;
     ...
     *             occurs while saving the property list. The preferred way to
     *             save a properties list is via the
     ...
     * <p>
     * Properties from the defaults table of this {@code Properties} table (if
     * any) are <i>not</i> written out by this method.
     * <p>
     * If the comments argument is not null, then an ASCII {@code #} character,
     * the comments string, and a line separator are first written to the output
     * stream. Thus, the {@code comments} can serve as an identifying comment.
     * Any one of a line feed ('\n'), a carriage return ('\r'), or a carriage
     * return followed immediately by a line feed in comments is replaced by a
     * line separator generated by the {@code Writer} and if the next character
     * in comments is not character {@code #} or character {@code !} then an
     * ASCII {@code #} is written out after that line separator.
     * <p>
     * Next, a comment line is always written, consisting of an ASCII {@code #}
     * character, the current date and time (as if produced by the
     * {@code toString} method of {@code Date} for the current time), and a line
     * separator as generated by the {@code Writer}.
     * <p>
     * Then every entry in this {@code Properties} table is written out, one per
     * line. For each entry the key string is written, then an ASCII {@code =},
     * then the associated element string. For the key, all space characters are
     * written with a preceding {@code \} character. For the element, leading
     * space characters, but not embedded or trailing space characters, are
     * written with a preceding {@code \} character. The key and element
     * characters {@code #}, {@code !}, {@code =}, and {@code :} are written
    ...
    /**
     * Writes this property list (key and element pairs) in this
     * {@code Properties} table to the output stream in a format suitable for
     * loading into a {@code Properties} table using the
    ...
    /**
     * Emits an XML document representing all of the properties contained in
     * this table, using the specified encoding.
     * <p>
     ...
     * </pre>
     * <p>
     * If the specified comment is {@code null} then no comment will be stored
     * in the document.
     * <p>
     * An implementation is required to support writing of XML documents that
     * use the "{@code UTF-8}" or "{@code UTF-16}" encoding. An implementation
     * may support additional encodings.
     * <p>
     * The specified stream remains open after this method returns.
     *
     * @param os
     ...
     * method then the system-wide provider is located as follows:
     * </p>
     * <ol>
     * <li>If the system property {@code sun.util.spi.XmlPropertiesProvider} is
     * defined then it is taken to be the full-qualified name of a concrete
     ...
     * locate a provider with the system class loader as the initiating loader
     * and {@code sun.util.spi.XmlPropertiesProvider} as the service type. If
     * this process fails then an unspecified error is thrown. If there is more
     * than one service provider installed then it is not specified as to which
     * provider will be used.</li>
     * <li>If the provider is not found by the above means then a system default
     * provider will be instantiated and used.</li>
     * </ol>
     */
    private static class XmlSupport {
        ...
        private static XmlPropertiesProvider loadProviderAsService(ClassLoader cl) {
            Iterator<XmlPropertiesProvider> iterator = ServiceLoader.load(XmlPropertiesProvider.class, cl)
                    .iterator();
