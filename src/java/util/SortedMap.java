 ...
 * The map is ordered according to the {@linkplain Comparable natural ordering}
 * of its keys, or by a {@link Comparator} typically provided at sorted map
 * creation time. This order is reflected when iterating over the sorted map's
 * collection views (returned by the {@code entrySet}, {@code keySet} and
 ...
 * keys must be <em>mutually comparable</em>: {@code k1.compareTo(k2)} (or
 ...
 * map is to correctly implement the {@code Map} interface. (See the
 * {@code Comparable} interface or {@code Comparator} interface for a precise
 * definition of <em>consistent with equals</em>.) This is so because the
 * {@code Map} interface is defined in terms of the {@code equals} operation,
 * but a sorted map performs all key comparisons using its {@code compareTo} (or
 ...
 * </li>
 * <li>A constructor with a single argument of type {@code Map}, which creates a
 * new map with the same key-value mappings as its argument, sorted according to
 * the keys' natural ordering.</li>
 ...
 * subrange from {@code lowEndpoint} to {@code successor(highEndpoint)}. For
 * example, suppose that {@code m} is a map whose keys are strings. The
 * following idiom obtains a view containing all of the key-value mappings in
 * {@code m} whose keys are between {@code low} and {@code high}, inclusive:
 * 
 * <pre>
 * SortedMap&lt;String, V&gt; sub = m.subMap(low, high + "\0");
 * </pre>
 *
 * A similar technique can be used to generate an <em>open range</em> (which
 * contains neither endpoint). The following idiom obtains a view containing all
 * of the key-value mappings in {@code m} whose keys are between {@code low} and
 * {@code high}, exclusive:
 * 
 * <pre>
...
public interface SortedMap<K, V> extends Map<K, V> {
	 ...
	 * {@code null} if this map uses the {@linkplain Comparable natural
	 * ordering} of its keys.
	 *
	 * @return the comparator used to order the keys in this map, or
	 *         {@code null} if this map uses the natural ordering of its keys
	 */
	 ...
	 *
	 * @return a set view of the keys contained in this map, sorted in ascending
	 *         order
	 */
	Set<K> keySet();
