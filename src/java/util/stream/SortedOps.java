...
final class SortedOps {
	...
	private static final class OfRef<T> extends ReferencePipeline.StatefulOp<T, T> {
		...
		public <P_IN> Node<T> opEvaluateParallel(PipelineHelper<T> helper,
			...
			if (StreamOpFlag.SORTED.isKnown(helper.getStreamAndOpFlags()) && isNaturalSort) {
				...
				// parallel sort
				T[] flattenedData = helper.evaluate(spliterator, true, generator)
						.asArray(generator);
	 ...
	 * <p>
	 * Sorting sinks first accept all elements, buffering then into an array or
	 * a re-sizable data structure, if the size of the pipeline is known or
	 * unknown respectively. At the end of the sink protocol those elements are
	 * sorted and then pushed downstream. This class records if
	 * {@link #cancellationRequested} is called. If so it can be inferred that
	 * the source pushing source elements into the pipeline knows that the
	 * pipeline is short-circuiting. In such cases sub-classes pushing elements
	 * downstream will preserve the short-circuiting protocol by calling
	 * {@code downstream.cancellationRequested()} and checking the result is
	 * {@code false} before an element is pushed.
	 * <p>
