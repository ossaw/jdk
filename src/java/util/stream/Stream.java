 ...
 * </pre>
 *
 * In this example, {@code widgets} is a {@code Collection<Widget>}. We create a
 * stream of {@code Widget} objects via {@link Collection#stream
 * Collection.stream()}, filter it to produce a stream containing only the red
 ...
 * <em>stream pipeline</em>. A stream pipeline consists of a source (which might
 * be an array, a collection, a generator function, an I/O channel, etc), zero
 * or more <em>intermediate operations</em> (which transform a stream into
 * another stream, such as {@link Stream#filter(Predicate)}), and a
 * <em>terminal operation</em> (which produces a result or side-effect, such as
 ...
 * {@code mapToInt} in the example above. To preserve correct behavior, these
 * <em>behavioral parameters</em>:
 * <ul>
 * <li>must be
 * <a href="package-summary.html#NonInterference">non-interfering</a> (they do
 * not modify the stream source); and</li>
 * <li>in most cases must be
 * <a href="package-summary.html#Statelessness">stateless</a> (their result
 ...
 * by {@link Files#lines(Path, Charset)}) will require closing. Most streams are
 ...
 * is a property of the stream. Streams are created with an initial choice of
 * sequential or parallel execution. (For example, {@link Collection#stream()
 * Collection.stream()} creates a sequential stream, and
 * {@link Collection#parallelStream() Collection.parallelStream()} creates a
 * parallel one.) This choice of execution mode may be modified by the
...
public interface Stream<T> extends BaseStream<T, Stream<T>> {
	 ...
	 *          following produces a stream containing all the line items in all
	 *          the orders:
	 * 
	 *          <pre>
	 * {@code
	 ...
	 *            </a>,
	 *            <a href="package-summary.html#Statelessness">stateless</a>
	 *            function to apply to each element which produces a stream of
	 *            new values
	 * @return the new stream
	 */
	 ...
	 * operation</a>.
	 *
	 * @apiNote Preserving stability for {@code distinct()} in parallel
	 *          pipelines is relatively expensive (requires that the operation
	 *          act as a full barrier, with substantial buffering overhead), and
	 *          stability is often not needed. Using an unordered stream source
	 *          (such as {@link #generate(Supplier)}) or removing the ordering
	 *          constraint with {@link #unordered()} may result in significantly
	 *          more efficient execution for {@code distinct()} in parallel
	 *          pipelines, if the semantics of your situation permit. If
	 *          consistency with encounter order is required, and you are
	 *          experiencing poor performance or memory utilization with
	 ...
	 *          to see the elements as they flow past a certain point in a
	 *          pipeline:
	 * 
	 *          <pre>
	 * {@code
	 ...
	 *
	 * @apiNote While {@code limit()} is generally a cheap operation on
	 *          sequential stream pipelines, it can be quite expensive on
	 *          ordered parallel pipelines, especially for large values of
	 *          {@code maxSize}, since {@code limit(n)} is constrained to return
	 *          not just any <em>n</em> elements, but the <em>first n</em>
	 *          elements in the encounter order. Using an unordered stream
	 *          source (such as {@link #generate(Supplier)}) or removing the
	 *          ordering constraint with {@link #unordered()} may result in
	 *          significant speedups of {@code limit()} in parallel pipelines,
	 *          if the semantics of your situation permit. If consistency with
	 *          encounter order is required, and you are experiencing poor
	 ...
	 * operation</a>.
	 *
	 * @apiNote While {@code skip()} is generally a cheap operation on
	 *          sequential stream pipelines, it can be quite expensive on
	 *          ordered parallel pipelines, especially for large values of
	 *          {@code n}, since {@code skip(n)} is constrained to skip not just
	 *          any <em>n</em> elements, but the <em>first n</em> elements in
	 *          the encounter order. Using an unordered stream source (such as
	 *          {@link #generate(Supplier)}) or removing the ordering constraint
	 *          with {@link #unordered()} may result in significant speedups of
	 *          {@code skip()} in parallel pipelines, if the semantics of your
	 *          situation permit. If consistency with encounter order is
	 ...
	 * <a href="../concurrent/package-summary.html#MemoryVisibility"><i>happens-
	 * before</i></a> performing the action for subsequent elements, but for any
	 * given element, the action may be performed in whatever thread the library
	 * chooses.
	 *
	 * @param action
	 *            a <a href="package-summary.html#NonInterference"> non-
	 ...
	 * .
	 *
	 * @apiNote The generator function takes an integer, which is the size of
	 *          the desired array, and produces an array of the desired size.
	 *          This can be concisely expressed with an array constructor
	 *          reference:
	 * 
	 *          <pre>
	 * {@code
	 ...
	 *             if the runtime type of the array returned from the array
	 *             generator is not a supertype of the runtime type of every
	 ...
	 *          by an explicit combination of {@code map} and {@code reduce}
	 *          operations. The {@code accumulator} function acts as a fused
	 ...
	 * reduction is one in which the reduced value is a mutable result
	 ...
	 *          {@code collect()}. For example, the following will accumulate
	 ...
	 * mutable data structures. Therefore, even when executed in parallel with
	 * non-thread-safe data structures (such as {@code ArrayList}), no
	 ...
	 * terminal operation</a>.
	 *
	 * @apiNote This method evaluates the <em>universal quantification</em> of
	 ...
	 *
	 * @apiNote This method evaluates the <em>universal quantification</em> of
	 *          the negated predicate over the elements of the stream (for all x
	...
	/**
	 * Returns an infinite sequential ordered {@code Stream} produced by
	 ...
	 * invoked.
	 *
	 * @implNote Use caution when constructing streams from repeated
	 ...
	 * phase, after which elements may not be added. The built phase begins when
	 * the {@link #build()} method is called, which creates an ordered
	...
	public interface Builder<T> extends Consumer<T> {
		 ...
		 * {@code IllegalStateException} is thrown if there are further attempts
