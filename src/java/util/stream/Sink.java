 ...
 *
 * <p>
 * A sink may be in one of two states: an initial state and an active state. It
 * starts out in the initial state; the {@code begin()} method transitions it to
 * the active state, and the {@code end()} method transitions it back into the
 * initial state, where it can be re-used. Data-accepting methods (such as
 * {@code accept()} are only valid in the active state.
 *
 * @apiNote A stream pipeline consists of a source, zero or more intermediate
 *          stages (such as filtering or mapping), and a terminal stage, such as
 *          reduction or for-each. For concreteness, consider the pipeline:
 *
 *          <pre>
 * {@code
 ...
 *          filtering stage consumes strings and emits a subset of those
 *          strings; the mapping stage consumes strings and emits ints; the
 *          reduction stage consumes those ints and computes the maximal value.
 *
 *          <p>
 *          A {@code Sink} instance is used to represent each stage of this
 *          pipeline, whether the stage accepts objects, ints, longs, or
 *          doubles. Sink has entry points for {@code accept(Object)},
 *          {@code accept(int)}, etc, so that we do not need a specialized
 *          interface for each primitive specialization. (It might be called a
 *          "kitchen sink" for this omnivorous tendency.) The entry point to the
 *          pipeline is the {@code Sink} for the filtering stage, which sends
 *          some elements "downstream" -- into the {@code Sink} for the mapping
 *          stage, which in turn sends integral values downstream into the
 *          {@code Sink} for the reduction stage. The {@code Sink}
 *          implementations associated with a given stage is expected to know
 *          the data type for the next stage, and call the correct
 *          {@code accept} method on its downstream {@code Sink}. Similarly,
 *          each stage must implement the correct {@code accept} method
 *          corresponding to the data type it accepts.
 *
 *          <p>
 *          The specialized subtypes such as {@link Sink.OfInt} override
 *          {@code accept(Object)} to call the appropriate primitive
 *          specialization of {@code accept}, implement the appropriate
 *          primitive specialization of {@code Consumer}, and re-abstract the
 *          appropriate primitive specialization of {@code accept}.
 *
 *          <p>
 *          The chaining subtypes such as {@link ChainedInt} not only implement
 *          {@code Sink.OfInt}, but also maintain a {@code downstream} field
 *          which represents the downstream {@code Sink}, and implement the
 *          methods {@code begin()}, {@code end()}, and
 *          {@code cancellationRequested()} to delegate to the downstream
 *          {@code Sink}. Most implementations of intermediate operations will
 *          use these chaining wrappers. For example, the mapping stage in the
 ...
 *
 *          <p>
 *          Here, we implement {@code Sink.ChainedReference<U>}, meaning that we
 *          expect to receive elements of type {@code U} as input, and pass the
 *          downstream sink to the constructor. Because the next stage expects
 *          to receive integers, we must call the {@code accept(int)} method
 *          when emitting values to the downstream. The {@code accept()} method
 *          applies the mapping function from {@code U} to {@code int} and
