...
/**
 * A sequence of primitive long-valued elements supporting sequential and
 * parallel aggregate operations. This is the {@code long} primitive
 * specialization of {@link Stream}.
 *
 * <p>
 * The following example illustrates an aggregate operation using {@link Stream}
 * and {@link LongStream}, computing the sum of the weights of the red widgets:
 *
 * <pre>
 * {
 * 	&#64;code
 * 	long sum = widgets.stream().filter(w -> w.getColor() == RED).mapToLong(
 * 			w -> w.getWeight()).sum();
 * }
 * </pre>
 *
...
public interface LongStream extends BaseStream<Long, LongStream> {
	 ...
	 *          source (such as {@link #generate(LongSupplier)}) or removing the
	 ...
	 *          {@link #generate(LongSupplier)}) or removing the ordering
	 ...
	 * <a href="package-summary.html#Associativity">associative</a> accumulation
	 * function, and returns an {@code OptionalLong} describing the reduced
	 ...
	 * case of a <a href="package-summary.html#Reduction">reduction</a> and is
	 * equivalent to:
	 * 
	 * <pre>
	 * {@code
	...
	/**
	 * Returns an infinite sequential ordered {@code LongStream} produced by
	...
	public static LongStream generate(LongSupplier s) {
		...
		return StreamSupport.longStream(
				new StreamSpliterators.InfiniteSupplyingSpliterator.OfLong(
						Long.MAX_VALUE, s), false);
	}

	/**
	 * Returns a sequential ordered {@code LongStream} from
	...
	public static LongStream range(long startInclusive,
		...
		if (startInclusive >= endExclusive) {
			...
			long m = startInclusive + Long.divideUnsigned(endExclusive
					- startInclusive, 2) + 1;
			return concat(range(startInclusive, m), range(m, endExclusive));
		} else {
			return StreamSupport.longStream(new Streams.RangeLongSpliterator(
	...
	public static LongStream rangeClosed(long startInclusive,
		...
		if (startInclusive > endInclusive) {
			...
			long m = startInclusive + Long.divideUnsigned(endInclusive
					- startInclusive, 2) + 1;
			return concat(range(startInclusive, m), rangeClosed(m,
					endInclusive));
		} else {
			return StreamSupport.longStream(new Streams.RangeLongSpliterator(
	...
	public static LongStream concat(LongStream a, LongStream b) {
		...
		Spliterator.OfLong split = new Streams.ConcatSpliterator.OfLong(a
				.spliterator(), b.spliterator());
		LongStream stream = StreamSupport.longStream(split, a.isParallel() || b
				.isParallel());
