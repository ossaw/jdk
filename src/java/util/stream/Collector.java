...
/**
 * A <a href="package-summary.html#Reduction">mutable reduction operation</a>
 * that accumulates input elements into a mutable result container, optionally
 ...
 * final transform on the result. They are:
 * <ul>
 * <li>creation of a new result container ({@link #supplier()})</li>
 * <li>incorporating a new data element into a result container (
 * {@link #accumulator()})</li>
 ...
 * {@code t2}, the results {@code r1} and {@code r2} in the computation below
 * must be equivalent:
 * 
 * <pre>
 * {
 ...
 * <ul>
 * <li>The first argument passed to the accumulator function, both arguments
 * passed to the combiner function, and the argument passed to the finisher
 * function must be the result of a previous invocation of the result supplier,
 * accumulator, or combiner functions.</li>
 * <li>The implementation should not do anything with the result of any of the
 * result supplier, accumulator, or combiner functions other than to pass them
 * again to the accumulator, combiner, or finisher functions, or return them to
 * the caller of the reduction operation.</li>
 * <li>If a result is passed to the combiner or finisher function, and the same
 * object is not returned from that function, it is never used again.</li>
 * <li>Once a result is passed to the combiner or finisher function, it is never
 * passed to the accumulator function again.</li>
 * <li>For non-concurrent collectors, any result returned from the result
 * supplier, accumulator, or combiner functions must be serially
 * thread-confined. This enables collection to occur in parallel without the
 * {@code Collector} needing to implement any additional synchronization. The
 * reduction implementation must manage that the input is properly partitioned,
 * that partitions are processed in isolation, and combining happens only after
 * accumulation is complete.</li>
 * <li>For concurrent collectors, an implementation is free to (but not required
 * to) implement reduction concurrently. A concurrent reduction is one where the
 ...
 * isolated during accumulation. A concurrent reduction should only be applied
 ...
 * {@link #of(Supplier, BiConsumer, BinaryOperator, Characteristics...)} can be
 * used to construct collectors. For example, you could create a collector that
 * accumulates widgets into a {@code TreeSet} with:
 *
 * <pre>
 * {
 *     &#64;code
 ...
 *
 * @apiNote Performing a reduction operation with a {@code Collector} should
 *          produce a result equivalent to:
 * 
 *          <pre>
 * {@code
 ...
 *          combine the partial results to achieve a parallel reduction.
 ...
 *          produce a new collector. For example, given the following collector
 *          that computes the sum of the salaries of a stream of employees:
 *
 *          <pre>
 * {@code
 ...
 *          </pre>
 *
 *          If we wanted to create a collector to tabulate the sum of salaries
 *          by department, we could reuse the "sum of salaries" logic using
...
public interface Collector<T, A, R> {
    ...
    /**
     * Returns a {@code Set} of {@code Collector.Characteristics} indicating the
    ...
    enum Characteristics {
        ...
        /**
         * Indicates that the finisher function is the identity function and can
         * be elided. If set, it must be the case that an unchecked cast from A
         * to R will succeed.
         */
        IDENTITY_FINISH
    }
