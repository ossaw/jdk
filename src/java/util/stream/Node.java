 ...
 *
 * <p>
 * A {@code Node} contains a fixed number of elements, which can be accessed via
 * the {@link #count}, {@link #spliterator}, {@link #forEach}, {@link #asArray},
 * or {@link #copyInto} methods. A {@code Node} may have zero or more child
 * {@code Node}s; if it has no children (accessed via {@link #getChildCount} and
 * {@link #getChild(int)}, it is considered <em>flat
 ...
 * @apiNote
 *          <p>
 *          A {@code Node} typically does not store the elements directly, but
 *          instead mediates access to one or more existing (effectively
 *          immutable) data structures such as a {@code Collection}, array, or a
 *          set of other {@code Node}s. Commonly {@code Node}s are formed into a
 *          tree whose shape corresponds to the computation tree that produced
 *          the elements that are contained in the leaf nodes. The use of
 *          {@code Node} within the stream framework is largely to avoid copying
 *          data unnecessarily during parallel operations.
 *
 * @param <T>
...
interface Node<T> {
	 ...
	 *
	 * @param consumer
	 *                 a {@code Consumer} that is to be invoked with each
	 *                 element in
	 *                 this {@code Node}
	 */
	 ...
	 *
	 * @param from
	 *                  The (inclusive) starting offset of elements to include,
	 *                  must
	 *                  be in range 0..count().
	 * @param to
	 *                  The (exclusive) end offset of elements to include, must
	 *                  be in
	 *                  range 0..count().
	 * @param generator
	...
	default Node<T> truncate(long from, long to, IntFunction<T[]> generator) {
		...
		for (int i = 0; (i < size) && spliterator.tryAdvance(
				nodeBuilder); i++) {
		}
		nodeBuilder.end();
	 ...
	 *
	 * <p>
	 * Depending on the underlying implementation, this may return a reference
	 * to an internal array rather than a copy. Since the returned array may be
	 * shared, the returned array should not be modified. The {@code generator}
	 * function may be consulted to create the array if a new array needs to be
	 * created.
	 *
	 * @param generator
	 *                  a factory function which takes an integer parameter and
	 *                  returns a new, empty array of that size and of the
	 *                  appropriate
	 *                  array type
	...
	public interface OfPrimitive<T, T_CONS, T_ARR, T_SPLITR extends Spliterator.OfPrimitive<T, T_CONS, T_SPLITR>, T_NODE extends OfPrimitive<T, T_CONS, T_ARR, T_SPLITR, T_NODE>>
		 ...
		 *
		 * @param action
		 *               a consumer that is to be invoked with each element in
		 *               this
		 *               {@code Node.OfPrimitive}
		 */
		 ...
		 *           {@link #count()} and then invokes
		 *           {@link #copyInto(T[], int)} with that array at an offset of
		 *           0.
		 */
		@Override
		default T[] asArray(IntFunction<T[]> generator) {
			if (java.util.stream.Tripwire.ENABLED)
				java.util.stream.Tripwire.trip(getClass(),
						"{0} calling Node.OfPrimitive.asArray");
	 ...
	 */
	interface OfInt extends
			OfPrimitive<Integer, IntConsumer, int[], Spliterator.OfInt, OfInt> {
		 ...
		 *                 a {@code Consumer} that is to be invoked with each
		 *                 element
		 *                 in this {@code Node}. If this is an
		 *                 {@code IntConsumer},
		 *                 it is cast to {@code IntConsumer} so the elements may
		 *                 be
		 ...
		 *           {@link #asPrimitiveArray()} to obtain an int[] array then
		 *           and copies the elements from that int[] array into the
		 *           boxed Integer[] array. This is not efficient and it is
		 *           recommended to invoke {@link #copyInto(Object, int)}.
		 */
		@Override
		default void copyInto(Integer[] boxed, int offset) {
			if (Tripwire.ENABLED)
				Tripwire.trip(getClass(),
						"{0} calling Node.OfInt.copyInto(Integer[], int)");
		...
		@Override
		default Node.OfInt truncate(long from, long to,
			...
			nodeBuilder.begin(size);
			for (int i = 0; i < from && spliterator.tryAdvance(
					(IntConsumer) e -> {
					}); i++) {
			}
			for (int i = 0; (i < size) && spliterator.tryAdvance(
					(IntConsumer) nodeBuilder); i++) {
			}
			nodeBuilder.end();
			return nodeBuilder.build();
		}
	 ...
	 */
	interface OfLong extends
			OfPrimitive<Long, LongConsumer, long[], Spliterator.OfLong, OfLong> {
		 ...
		 *                 in this {@code Node}. If this is an
		 *                 {@code LongConsumer},
		 *                 it is cast to {@code LongConsumer} so the elements
		 *                 may be
		 *                 processed without boxing.
		 */
		@Override
		 ...
		 *
		 * @implSpec the default implementation invokes
		 *           {@link #asPrimitiveArray()} to obtain a long[] array then
		 *           and copies the elements from that long[] array into the
		 *           boxed Long[] array. This is not efficient and it is
		 *           recommended to invoke {@link #copyInto(Object, int)}.
		 */
		@Override
		default void copyInto(Long[] boxed, int offset) {
			if (Tripwire.ENABLED)
				Tripwire.trip(getClass(),
						"{0} calling Node.OfInt.copyInto(Long[], int)");
		...
		@Override
		default Node.OfLong truncate(long from, long to,
	...
	interface OfDouble extends
		 ...
		 *                 in this {@code Node}. If this is an
		 *                 {@code DoubleConsumer}
		 *                 , it is cast to {@code DoubleConsumer} so the
		 *                 elements may
		 *                 be processed without boxing.
		 */
		@Override
		 ...
		 *
		 * @implSpec the default implementation invokes
		 *           {@link #asPrimitiveArray()} to obtain a double[] array then
		 *           and copies the elements from that double[] array into the
		 *           boxed Double[] array. This is not efficient and it is
		 *           recommended to invoke {@link #copyInto(Object, int)}.
		 */
		@Override
		...
		@Override
		default Node.OfDouble truncate(long from, long to,
