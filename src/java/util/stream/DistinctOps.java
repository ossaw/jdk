		...
		return new ReferencePipeline.StatefulOp<T, T>(upstream, StreamShape.REFERENCE,
			...
			<P_IN> Node<T> opEvaluateParallel(PipelineHelper<T> helper,
				...
				if (StreamOpFlag.DISTINCT.isKnown(helper.getStreamAndOpFlags())) {
					...
					// If null has been seen then copy the key set into a
					// HashSet that supports null values
					// and add null
					Set<T> keys = map.keySet();
					if (seenNull.get()) {
						// TODO Implement a more efficient set-union view,
						// rather than copying
			...
			<P_IN> Spliterator<T> opEvaluateParallelLazy(PipelineHelper<T> helper,
				...
				if (StreamOpFlag.DISTINCT.isKnown(helper.getStreamAndOpFlags())) {
					...
					// Lazy
					return new StreamSpliterators.DistinctSpliterator<>(
