 ...
 * parallel aggregate operations. This is the {@code double} primitive
 * specialization of {@link Stream}.
 * <p>
 * The following example illustrates an aggregate operation using {@link Stream}
 * and {@link DoubleStream}, computing the sum of the weights of the red
 * widgets:
 *
 * <pre>
 * {
...
public interface DoubleStream extends BaseStream<Double, DoubleStream> {
     ...
     *          source (such as {@link #generate(DoubleSupplier)}) or removing
     *          the ordering constraint with {@link #unordered()} may result in
     ...
     *          {@link #generate(DoubleSupplier)}) or removing the ordering
     *          constraint with {@link #unordered()} may result in significant
     *          speedups of {@code skip()} in parallel pipelines, if the
     *          semantics of your situation permit. If consistency with
     *          encounter order is required, and you are experiencing poor
     *          performance or memory utilization with {@code skip()} in
     *          parallel pipelines, switching to sequential execution with
     *          {@link #sequential()} may improve performance.
     * @param n
     ...
     * .
     * <p>
     * For parallel stream pipelines, this operation does <em>not</em> guarantee
     * to respect the encounter order of the stream, as doing so would sacrifice
     ...
     * {@code accumulator.apply(identity, x)} is equal to {@code x}. The
     ...
     * <a href="package-summary.html#Reduction">reduction</a>. If floating-point
     * summation were exact, this method would be equivalent to:
     *
     * <pre>
     * {@code
     ...
     * </pre>
     *
     * However, since floating-point summation is not exact, the above code is
     * not necessarily equivalent to the summation computation done by this
     * method.
     * <p>
     * If any stream element is a NaN or the sum is at any point a NaN then the
     * sum will be NaN.
     ...
     * .
     *
     * @apiNote Elements sorted by increasing absolute magnitude tend to yield
     ...
     * technique to reduce the error bound in the {@link #sum numerical sum}
    ...
    /**
     * Returns a {@code Stream} consisting of the elements of this stream, boxed
     * to {@code Double}.
     * <p>
     * This is an <a href="package-summary.html#StreamOps">intermediate
     * operation</a>.
    ...
    /**
     * Returns an infinite sequential ordered {@code DoubleStream} produced by
     ...
     * <p>
     * The first element (position {@code 0}) in the {@code DoubleStream} will
    ...
    public static DoubleStream iterate(final double seed, final DoubleUnaryOperator f) {
        ...
        return StreamSupport.doubleStream(Spliterators.spliteratorUnknownSize(iterator, Spliterator.ORDERED
    ...
    public static DoubleStream generate(DoubleSupplier s) {
        ...
        return StreamSupport.doubleStream(new StreamSpliterators.InfiniteSupplyingSpliterator.OfDouble(
     ...
     * the {@link #build()} method is called, which creates an ordered stream
     * whose elements are the elements that were added to the stream builder, in
