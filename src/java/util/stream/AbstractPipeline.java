...
abstract class AbstractPipeline<E_IN, E_OUT, S extends BaseStream<E_OUT, S>>
	 ...
	 *             if this pipeline stage is not the source stage.
	 */
	@SuppressWarnings("unchecked")
	...
	final <P_IN> long exactOutputSizeIfKnown(Spliterator<P_IN> spliterator) {
		return StreamOpFlag.SIZED.isKnown(getStreamAndOpFlags()) ? spliterator.getExactSizeIfKnown()
				: -1;
	}
	...
	@Override
	@SuppressWarnings("unchecked")
	final <P_IN> Node<E_OUT> evaluate(Spliterator<P_IN> spliterator, boolean flatten,
			IntFunction<E_OUT[]> generator) {
		if (isParallel()) {
			// @@@ Optimize if op of this pipeline stage is a stateful op
			return evaluateToNode(this, spliterator, flatten, generator);
		} else {
			Node.Builder<E_OUT> nb = makeNodeBuilder(exactOutputSizeIfKnown(spliterator),
					generator);
	...
	abstract <P_IN> Node<E_OUT> evaluateToNode(PipelineHelper<E_OUT> helper,
			Spliterator<P_IN> spliterator, boolean flattenTree, IntFunction<E_OUT[]> generator);
	 ...
	 *            if {@literal >=0}, then a node builder will be created that
	 *            has a fixed capacity of at most sizeIfKnown elements. If
	 *            {@literal < 0}, then the node builder has an unfixed capacity.
	 *            A fixed capacity node builder will throw exceptions if an
	 *            element is added after builder has reached capacity, or is
	 *            built before the builder has reached capacity.
	 *
	 * @param generator
	 *            the array generator to be used to create instances of a T[]
	 *            array. For implementations supporting primitive nodes, this
