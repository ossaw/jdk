...
/**
 * Helper class for executing <a href="package-summary.html#StreamOps"> stream
 * pipelines</a>, capturing all of the information about a stream pipeline
 * (output shape, intermediate operations, stream flags, parallelism, etc) in
 * one place.
 *
 * <p>
 ...
 * , methods, which can use the {@code PipelineHelper} to access information
 * about the pipeline such as head shape, stream flags, and size, and use the
 * helper methods such as {@link #wrapAndCopyInto(Sink, Spliterator)},
...
abstract class PipelineHelper<P_OUT> {
     ...
     * described pipeline. This will incorporate stream flags from the stream
     * source, all the intermediate operations and the terminal operation.
     *
     ...
     * return {@code -1}.
     *
     * @apiNote The exact output size is known if the {@code Spliterator} has
     *          the {@code SIZED} characteristic, and the operation flags
     *          {@link StreamOpFlag#SIZED} is known on the combined stream and
     *          operation flags.
     *
     * @param spliterator
     ...
     * {@link Sink#cancellationRequested()} is checked after each element,
     * stopping if cancellation is requested.
     *
     * @implSpec This method conforms to the {@code Sink} protocol of calling
     *           {@code Sink.begin} before pushing elements, via
     *           {@code Sink.accept}, and calling {@code Sink.end} after all
     ...
     *
     * @implNote If the pipeline has no intermediate operations and the source
     *           is backed by a {@code Node} then that {@code Node} will be
     *           returned (or flattened and then returned). This reduces copying
     *           for a pipeline consisting of a stateful operation followed by a
     *           terminal operation that returns an array, such as:
     * 
     *           <pre>
     * {@code
