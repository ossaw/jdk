...
class JarVerifier {
	...
	/*
	 * a table mapping names to code signers, for jar entries that have had
	 * their actual hashes verified
	 */
	private Hashtable<String, CodeSigner[]> verifiedSigners;

	/*
	 * a table mapping names to code signers, for jar entries that have passed
	 * the .SF/.DSA/.EC -> MANIFEST check
	 */
	...
	/**
	 * "queue" of pending PKCS7 blocks that we couldn't parse until we parsed
	 * the .SF file
	 */
	...
	/*
	 * The output stream to use when keeping track of files we are interested in
	 */
	...
	/**
	 * This method scans to see which entry we're parsing and keeps various
	 * state information depending on what type of file is being parsed.
	 */
	public void beginEntry(JarEntry je, ManifestEntryVerifier mev)
		...
		/*
		 * Assumptions: 1. The manifest should be the first entry in the
		 * META-INF directory. 2. The .SF/.DSA/.EC files follow the manifest,
		 * before any normal entries 3. Any of the following will throw a
		 * SecurityException: a. digest mismatch between a manifest section and
		 * the SF section. b. digest mismatch between the actual jar entry and
		 * the manifest
		 */

		if (parsingMeta) {
			String uname = name.toUpperCase(Locale.ENGLISH);
			if ((uname.startsWith("META-INF/") || uname.startsWith(
					"/META-INF/"))) {
				...
				if (uname.equals(JarFile.MANIFEST_NAME) || uname.equals(
						JarIndex.INDEX_NAME)) {
					return;
				}
		...
		if (!name.equals(JarFile.MANIFEST_NAME)) {
			if (sigFileSigners.get(name) != null || verifiedSigners.get(
					name) != null) {
		...
		if (!parsingBlockOrSF) {
			...
			try {
				...
				if (uname.endsWith(".SF")) {
					...
					Iterator<SignatureFileVerifier> it = pendingBlocks
							.iterator();
					while (it.hasNext()) {
	...
	/**
	 * Return an array of java.security.cert.Certificate objects for the given
	 * file in the jar.
	 * 
	 * @deprecated
	 */
	...
	public java.security.cert.Certificate[] getCerts(JarFile jar,
			JarEntry entry) {
		return mapSignersToCertArray(getCodeSigners(jar, entry));
	}

	/**
	 * return an array of CodeSigner objects for the given file in the jar. this
	 * array is not cloned.
	 *
	 */
	...
	public CodeSigner[] getCodeSigners(JarFile jar, JarEntry entry) {
		...
		if (eagerValidation && sigFileSigners.get(name) != null) {
			/*
			 * Force a read of the entry data to generate the verification hash.
			 */
			try {
	...
	/**
	 * returns true if there no files to verify. should only be called after all
	 * the META-INF entries have been processed.
	 */
	...
	/**
	 * called to let us know we have processed all the META-INF entries, and if
	 * we re-read one of them, don't re-process it. Also gets rid of any data
	 * structures we needed when parsing META-INF entries.
	 */
	void doneWithMeta() {
		parsingMeta = false;
		...
		if (sigFileSigners.containsKey(JarFile.MANIFEST_NAME)) {
			CodeSigner[] codeSigners = sigFileSigners.remove(
	...
	static class VerifierStream extends java.io.InputStream {
		...
		VerifierStream(Manifest man, JarEntry je, InputStream is,
	...
	/*
	 * Create a unique mapping from codeSigner cache entries to CodeSource. In
	 * theory, multiple URLs origins could map to a single locally cached and
	 * shared JAR file although in practice there will be a single URL in use.
	 */
	private synchronized CodeSource mapSignersToCodeSource(URL url,
			CodeSigner[] signers) {
	...
	private CodeSource[] mapSignersToCodeSources(URL url,
			List<CodeSigner[]> signers, boolean unsigned) {
	...
	private CodeSigner[] findMatchingSigners(CodeSource cs) {
		...
		/*
		 * In practice signers should always be optimized above but this handles
		 * a CodeSource of any type, just in case.
		 */
		CodeSource[] sources = mapSignersToCodeSources(cs.getLocation(),
				getJarCodeSigners(), true);
		...
		if (j != -1) {
			CodeSigner[] match;
			match = ((VerifierCodeSource) sourceList.get(j))
					.getPrivateSigners();
			if (match == null) {
				match = emptySigner;
			}
	...
	/*
	 * Instances of this class hold uncopied references to internal signing data
	 * that can be compared by object reference identity.
	 */
	private static class VerifierCodeSource extends CodeSource {
		...
		/*
		 * All VerifierCodeSource instances are constructed based on singleton
		 * signerCache or signerCacheCert entries for each unique signer. No
		 * CodeSigner<->Certificate[] conversion is required. We use these
		 * assumptions to optimize equality comparisons.
		 */
	...
	private synchronized Map<String, CodeSigner[]> signerMap() {
		if (signerMap == null) {
			/*
			 * Snapshot signer state so it doesn't change on us. We care only
			 * about the asserted signatures. Verification of signature validity
			 * happens via the JarEntry apis.
			 */
			signerMap = new HashMap<>(verifiedSigners.size() + sigFileSigners
					.size());
	...
	public synchronized Enumeration<String> entryNames(JarFile jar,
		...
		final Iterator<Map.Entry<String, CodeSigner[]>> itor = map.entrySet()
				.iterator();
		boolean matchUnsigned = false;

		/*
		 * Grab a single copy of the CodeSigner arrays. Check to see if we can
		 * optimize CodeSigner equality test.
		 */
		...
		final Enumeration<String> enum2 = (matchUnsigned) ? unsignedEntryNames(
				jar) : emptyEnumeration;
	...
	/*
	 * Like entries() but screens out internal JAR mechanism entries and
	 * includes signed entries with no ZIP data.
	 */
	...
	private Enumeration<String> unsignedEntryNames(JarFile jar) {
		...
		return new Enumeration<String>() {
			...
			/*
			 * Grab entries from ZIP directory but screen out metadata.
			 */
			public boolean hasMoreElements() {
				if (name != null) {
					return true;
	...
	static CodeSource getUnsignedCS(URL url) {
		return new VerifierCodeSource(null, url,
