 ...
 * {@link JarEntry#getCodeSigners()} to further determine if the signature can
 * be trusted.
 *
...
public class JarFile extends ZipFile {
    ...
    // indicates if Class-Path attribute present (only valid if
    ...
    /*
     * Ensures that the JarVerifier has been created if one is necessary (i.e.,
     * the jar appears to be signed.) This is done as a quick check to avoid
     * processing of the manifest for unsigned jars.
     */
        ...
        if (verify) {
            ...
            if (names != null) {
                for (int i = 0; i < names.length; i++) {
                    ...
                    if (name.endsWith(".DSA") || name.endsWith(".RSA") || name.endsWith(".EC") || name
                            .endsWith(".SF")) {
    ...
    /*
     * Initializes the verifier object by reading all the manifest entries and
     * passing them to the verifier.
     */
    ...
    /*
     * Reads all the bytes for a given entry. Used to process the META-INF
     * files.
     */
    ...
    public synchronized InputStream getInputStream(ZipEntry ze) throws IOException {
        ...
        return new JarVerifier.VerifierStream(getManifestFromReference(), ze instanceof JarFileEntry
                ? (JarEntry) ze : getJarEntry(ze.getName()), super.getInputStream(ze), jv);
    }
    ...
    /**
     * Returns {@code true} iff this JAR file has a manifest with the Class-Path
     * attribute
     */
    ...
    /**
     * Returns true if the pattern {@code src} is found in {@code b}. The
     * {@code lastOcc} and {@code optoSft} arrays are the precomputed bad
     * character and good suffix shifts.
     */
    ...
    Enumeration<String> entryNames(CodeSource[] cs) {
        ...
        /*
         * JAR file has no signed content. Is there a non-signing code source?
         */
    ...
    /**
     * Returns an enumeration of the zip file entries excluding internal JAR
     * mechanism entries and including signed entries missing from the ZIP
     * directory.
     */
