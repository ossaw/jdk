 ...
 * specifier of {@link java.util.Formatter}. This interface allows basic control
 * for formatting arbitrary objects.
 ...
 * (precision != -1 && precision < 10); String out = (usesymbol ? symbol :
 * name);
 * // apply precision if (precision == -1 || out.length() < precision) { //
 * write it all sb.append(out); } else { sb.append(out.substring(0, precision -
 * 1)).append('*'); }
 ...
 * this interface will cause a {@link NullPointerException} to be thrown.
 *
 * @since 1.5
 */
public interface Formattable {
     ...
     *
     * @param formatter
     *        The {@link Formatter formatter}. Implementing classes
     *        may call
     *        {@link Formatter#out() formatter.out()} or
     *        {@link Formatter#locale() formatter.locale()} to obtain
     *        the
     *        {@link Appendable} or {@link Locale} used by this
     *        <tt>formatter</tt> respectively.
     * @param flags
     *        The flags modify the output format. The value is
     *        interpreted
     *        as a bitmask. Any combination of the following flags may
     *        be
     *        set: {@link FormattableFlags#LEFT_JUSTIFY},
     *        {@link FormattableFlags#UPPERCASE}, and
     *        {@link FormattableFlags#ALTERNATE}. If no flags are set,
     *        the
     ...
     *        <tt>'&nbsp;&nbsp;'</tt> until the total number of
     *        characters
     *        equals width. The padding is at the beginning by
     *        default. If
     *        the {@link FormattableFlags#LEFT_JUSTIFY} flag is set
     *        then the
     *        padding will be at the end. If <tt>width</tt> is
     *        <tt>-1</tt>
     ...
     *        output
     *        will be truncated to <tt>precision</tt> characters even
     *        if the
     *        <tt>width</tt> is greater than the <tt>precision</tt>.
     *        If
     *        <tt>precision</tt> is <tt>-1</tt> then there is no
     *        explicit
     *        limit on the number of characters.
     * @throws IllegalFormatException
     *         If any of the parameters are invalid. For
     *         specification of
