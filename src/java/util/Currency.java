 ...
 * codes. Visit the
 * <a href="http://www.iso.org/iso/home/standards/currency_codes.htm"> ISO web
 * site</a> for more information.
 * <p>
 * The class is designed so that there's never more than one
 * <code>Currency</code> instance for any given currency. Therefore, there's no
 * public constructor. You obtain a <code>Currency</code> instance using the
 * <code>getInstance</code> methods.
 * <p>
 * Users can supersede the Java runtime currency data by means of the system
 * property {@code java.util.currency.data}. If this system property is defined
 * then its value is the location of a properties file, the contents of which
 * are key/value pairs of the ISO 3166 country codes and the ISO 4217 currency
 * data respectively. The value part consists of three ISO 4217 values of a
 * currency, i.e., an alphabetic code, a numeric code, and a minor unit. Those
 * three ISO 4217 values are separated by commas. The lines which start with
 * '#'s are considered comment lines. An optional UTC timestamp may be specified
 * per currency entry if users need to specify a cutover date indicating when
 * the new data comes into effect. The timestamp is appended to the end of the
 * currency properties and uses a comma as a separator. If a UTC datestamp is
 * present and valid, the JRE will only use the new currency properties if the
 * current UTC date is later than the date specified at class loading time. The
 * format of the timestamp must be of ISO 8601 format :
 ...
 * </code>
 * <p>
 * will supersede the currency data for Japan if {@code Currency} class is
 * loaded after 1st January 2014 00:00:00 GMT.
 * <p>
 * Where syntactically malformed entries are encountered, the entry is ignored
 * and the remainder of entries in file are processed. For instances where
 * duplicate country code entries exist, the behavior of the Currency
 * information for that {@code Currency} is undefined and the remainder of
 * entries in file are processed.
 *
 * @since 1.4
 */
public final class Currency implements Serializable {
	...
	// - bits 0-4: final char for currency code for simple country, or ID of
	// special case
	// - special case IDs:
	...
	// - otherCurrenciesDFD: decimal format digits for currencies in
	// otherCurrencies, same order
	...
	// delta from entry index component in main table to index into special case
	// tables
	private static final int SPECIAL_CASE_COUNTRY_INDEX_DELTA = 1;
	// mask for distinguishing simple and special case countries
	private static final int COUNTRY_TYPE_MASK = SIMPLE_CASE_COUNTRY_MASK
	...
	static {
		AccessController.doPrivileged(new PrivilegedAction<Void>() {
			...
			public Void run() {
				...
				try {
					String dataFile = homeDir + File.separator + "lib"
					...
					if (propFile.exists()) {
						...
						Pattern propertiesPattern = Pattern.compile(
								"([A-Z]{3})\\s*,\\s*(\\d{3})\\s*,\\s*"
										...
										+ "\\d{2}:\\d{2})?");
						for (String key : keys) {
							replaceCurrencyData(propertiesPattern, key
									.toUpperCase(Locale.ROOT), props
											.getProperty(key).toUpperCase(
													Locale.ROOT));
						}
					}
				} catch (IOException e) {
					info("currency.properties is ignored because of an IOException",
							e);
				}
				return null;
			}
	 ...
	 *                                     if <code>currencyCode</code> is null
	 * @exception IllegalArgumentException
	 *                                     if <code>currencyCode</code> is not a
	 *                                     supported ISO 4217
	 *                                     code.
	 */
	...
	private static Currency getInstance(String currencyCode,
			int defaultFractionDigits, int numericCode) {
		...
		if (defaultFractionDigits == Integer.MIN_VALUE) {
			...
			// A currency code must have 3 characters and exist in the main
			// table
			...
			if ((tableEntry & COUNTRY_TYPE_MASK) == SIMPLE_CASE_COUNTRY_MASK
					&& tableEntry != INVALID_COUNTRY_ENTRY && currencyCode
							.charAt(2) - 'A' == (tableEntry
									& SIMPLE_CASE_COUNTRY_FINAL_CHAR_MASK)) {
				defaultFractionDigits = (tableEntry
						& SIMPLE_CASE_COUNTRY_DEFAULT_DIGITS_MASK) >> SIMPLE_CASE_COUNTRY_DEFAULT_DIGITS_SHIFT;
				numericCode = (tableEntry
						& NUMERIC_CODE_MASK) >> NUMERIC_CODE_SHIFT;
			} else {
				// Check for '-' separately so we don't get false hits in the
				// table.
		...
		Currency currencyVal = new Currency(currencyCode, defaultFractionDigits,
				numericCode);
	 ...
	 * locale. The language and variant components of the locale are ignored.
	 * The result may vary over time, as countries change their currencies. For
	 * example, for the original member countries of the European Monetary
	 * Union, the method returns the old national currencies until December 31,
	 * 2001, and the Euro from January 1, 2002, local time of the respective
	 * countries.
	 * <p>
	 * The method returns <code>null</code> for territories that don't have a
	 * currency, such as Antarctica.
	 *
	 * @param locale
	 *               the locale for whose country a <code>Currency</code>
	 *               instance
	 *               is needed
	 * @return the <code>Currency</code> instance for the country of the given
	 *         locale, or {@code null}
	 * @exception NullPointerException
	 *                                     if <code>locale</code> or its country
	 *                                     code is {@code null}
	 * @exception IllegalArgumentException
	 *                                     if the country of the given
	 *                                     {@code locale} is not a
	...
	public static Currency getInstance(Locale locale) {
		...
		if ((tableEntry & COUNTRY_TYPE_MASK) == SIMPLE_CASE_COUNTRY_MASK
			...
			char finalChar = (char) ((tableEntry
					& SIMPLE_CASE_COUNTRY_FINAL_CHAR_MASK) + 'A');
			int defaultFractionDigits = (tableEntry
					& SIMPLE_CASE_COUNTRY_DEFAULT_DIGITS_MASK) >> SIMPLE_CASE_COUNTRY_DEFAULT_DIGITS_SHIFT;
			int numericCode = (tableEntry
					& NUMERIC_CODE_MASK) >> NUMERIC_CODE_SHIFT;
			...
			if (tableEntry == COUNTRY_WITHOUT_CURRENCY_ENTRY) {
				...
				int index = (tableEntry & SPECIAL_CASE_COUNTRY_INDEX_MASK)
						- SPECIAL_CASE_COUNTRY_INDEX_DELTA;
				if (scCutOverTimes[index] == Long.MAX_VALUE || System
						.currentTimeMillis() < scCutOverTimes[index]) {
					return getInstance(scOldCurrencies[index],
							scOldCurrenciesDFD[index],
							scOldCurrenciesNumericCode[index]);
				} else {
					return getInstance(scNewCurrencies[index],
							scNewCurrenciesDFD[index],
	 ...
	 * that represent obsolete ISO 4217 codes. The set can be modified without
			...
			if (available == null) {
				...
				for (char c1 = 'A'; c1 <= 'Z'; c1++) {
					for (char c2 = 'A'; c2 <= 'Z'; c2++) {
						...
						if ((tableEntry
								& COUNTRY_TYPE_MASK) == SIMPLE_CASE_COUNTRY_MASK
								&& tableEntry != INVALID_COUNTRY_ENTRY) {
							char finalChar = (char) ((tableEntry
									& SIMPLE_CASE_COUNTRY_FINAL_CHAR_MASK)
									+ 'A');
							...
							sb.append(c2);
							sb.append(finalChar);
							available.add(getInstance(sb.toString(),
	 ...
	 * {@link Locale.Category#DISPLAY DISPLAY} locale. For example, for the US
	 * Dollar, the symbol is "$" if the default locale is the US, while for
	...
	public String getSymbol(Locale locale) {
		...
		String symbol = pool.getLocalizedObject(CurrencyNameGetter.INSTANCE,
	 ...
	 * for the Japanese Yen it's 0. In the case of pseudo-currencies, such as
	 ...
	 * default {@link Locale.Category#DISPLAY DISPLAY} locale. If there is no
	 * suitable display name found for the default locale, the ISO 4217 currency
	 * code is returned.
	 * <p>
	 * This is equivalent to calling {@link #getDisplayName(Locale)
	...
	public String getDisplayName(Locale locale) {
		LocaleServiceProviderPool pool = LocaleServiceProviderPool.getPool(
				CurrencyNameProvider.class);
		String result = pool.getLocalizedObject(CurrencyNameGetter.INSTANCE,
	...
	/**
	 * Gets the main table entry for the country whose country code consists of
	 * char1 and char2.
	 */
	...
	/**
	 * Sets the main table entry for the country whose country code consists of
	 * char1 and char2.
	 */
	 ...
	 * implementation.
	 */
	private static class CurrencyNameGetter implements
			LocaleServiceProviderPool.LocalizedObjectGetter<CurrencyNameProvider, String> {
	 ...
	 *
	 * @param pattern
	 *                regex pattern for the properties
	 * @param ctry
	 *                country code
	 * @param curdata
	 *                currency data. This is a comma separated string that
	 *                consists
	 *                of "three-letter alphabet code", "three-digit numeric
	 *                code",
	 *                and "one-digit (0-9) default fraction digit". For example,
	 *                "JPZ,392,0". An optional UTC date can be appended to the
	 *                string (comma separated) to allow a currency change take
	 *                effect after date specified. For example,
	 *                "JP=JPZ,999,0,2014-01-01T00:00:00" has no effect unless
	 *                UTC
	 *                time is past 1st January 2014 00:00:00 GMT.
	 */
	private static void replaceCurrencyData(Pattern pattern, String ctry,
			String curdata) {
		...
		if (!m.find() || (m.group(4) == null && countOccurrences(curdata,
				',') >= 3)) {
			// format is not recognized. ignore the data
			// if group(4) date string is null and we've 4 values, bad date
			// value
			info("currency.properties entry for " + ctry
					+ " ignored because the value format is not recognized.",
					null);
			return;
		}
		...
		try {
			if (m.group(4) != null && !isPastCutoverDate(m.group(4))) {
				info("currency.properties entry for " + ctry
						+ " ignored since cutover date has not passed :"
						+ curdata, null);
				return;
			}
		} catch (ParseException ex) {
			info("currency.properties entry for " + ctry
					+ " ignored since exception encountered :" + ex
							.getMessage(), null);
			return;
		}
		...
		if (fraction > SIMPLE_CASE_COUNTRY_MAX_DEFAULT_DIGITS) {
			info("currency.properties entry for " + ctry
					...
					+ SIMPLE_CASE_COUNTRY_MAX_DEFAULT_DIGITS + ":" + curdata,
					null);
			return;
		}
		...
		if (index == scOldCurrencies.length) {
			...
			// special case
			entry |= SPECIAL_CASE_COUNTRY_MASK | (index
					+ SPECIAL_CASE_COUNTRY_INDEX_DELTA);
		}
