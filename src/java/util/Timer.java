...
public class Timer {
	 ...
	 * for if the timer will be used to schedule repeating
	 * "maintenance activities", which must be performed as long as the
	 * application is running, but should not prolong the lifetime of the
	 * application.
	 *
	 * @param isDaemon
	 ...
	 *                  time in milliseconds between successive task executions.
	 * @throws IllegalArgumentException
	 *                                  if {@code firstTime.getTime() < 0}, or
	 *                                  {@code period <= 0}
	 * @throws IllegalStateException
	 *                                  if task was already scheduled or
	 *                                  cancelled, timer was
	 *                                  cancelled, or timer thread terminated.
	 * @throws NullPointerException
	 *                                  if {@code task} or {@code firstTime} is
	 *                                  null
	 */
...
class TaskQueue {
	...
	private void fixDown(int k) {
		...
		while ((j = k << 1) <= size && j > 0) {
			if (j < size && queue[j].nextExecutionTime > queue[j
					+ 1].nextExecutionTime)
