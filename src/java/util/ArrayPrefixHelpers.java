...
class ArrayPrefixHelpers {
	...
	/*
	 * Parallel prefix (aka cumulate, scan) task classes are based loosely on
	 * Guy Blelloch's original algorithm
	 * (http://www.cs.cmu.edu/~scandal/alg/scan.html): Keep dividing by two to
	 * threshold segment size, and then: Pass 1: Create tree of partial sums for
	 * each segment Pass 2: For each segment, cumulate with offset of left
	 * sibling
	 *
	 * This version improves performance within FJ framework mainly by allowing
	 * the second pass of ready left-hand sides to proceed even if some
	 * right-hand side first passes are still executing. It also combines first
	 * and second pass for leftmost segment, and skips the first pass for
	 * rightmost segment (whose result is not needed for second pass). It
	 * similarly manages to avoid requiring that users supply an identity basis
	 * for accumulations by tracking those segments/subtasks for which the first
	 * existing element is used as base.
	 *
	 * Managing this relies on ORing some bits in the pendingCount for
	 * phases/states: CUMULATE, SUMMED, and FINISHED. CUMULATE is the main phase
	 * bit. When false, segments compute only their sum. When true, they
	 * cumulate array elements. CUMULATE is set at root at beginning of second
	 * pass and then propagated down. But it may also be set earlier for
	 * subtrees with lo==0 (the left spine of tree). SUMMED is a one bit join
	 * count. For leafs, it is set when summed. For internal nodes, it becomes
	 * true when one child is summed. When the second child finishes summing, we
	 * then moves up tree to trigger the cumulate phase. FINISHED is also a one
	 * bit join count. For leafs, it is set when cumulated. For internal nodes,
	 * it becomes true when one child is cumulated. When the second child
	 * finishes cumulating, it then moves up tree, completing at the root.
	 *
	 * To better exploit locality and reduce overhead, the compute method loops
	 * starting with the current task, moving if possible to one of its subtasks
	 * rather than forking.
	 *
	 * As usual for this sort of utility, there are 4 versions, that are simple
	 * copy/paste/adapt variants of each other. (The double and int versions
	 * differ from long version soley by replacing "long" (with case-matching)).
	 */
	...
	static final class CumulateTask<T> extends CountedCompleter<Void> {
		...
		public CumulateTask(CumulateTask<T> parent, BinaryOperator<T> function, T[] array, int lo,
				int hi) {
			super(parent);
			...
			int p;
			this.threshold = (p = (hi - lo)
					/ (ForkJoinPool.getCommonPoolParallelism() << 3)) <= MIN_PARTITION
							? MIN_PARTITION : p;
		}

		/** Subtask constructor */
		CumulateTask(CumulateTask<T> parent, BinaryOperator<T> function, T[] array, int origin,
	...
	static final class LongCumulateTask extends CountedCompleter<Void> {
		...
		/** Root task constructor */
		public LongCumulateTask(LongCumulateTask parent, LongBinaryOperator function, long[] array,
				int lo, int hi) {
			super(parent);
			this.function = function;
			this.array = array;
		...
		/** Subtask constructor */
		LongCumulateTask(LongCumulateTask parent, LongBinaryOperator function, long[] array,
	...
	static final class DoubleCumulateTask extends CountedCompleter<Void> {
		...
		/** Root task constructor */
		public DoubleCumulateTask(DoubleCumulateTask parent, DoubleBinaryOperator function,
	...
	static final class IntCumulateTask extends CountedCompleter<Void> {
		...
		/** Root task constructor */
		public IntCumulateTask(IntCumulateTask parent, IntBinaryOperator function, int[] array,
				int lo, int hi) {
			super(parent);
			this.function = function;
			this.array = array;
