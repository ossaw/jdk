...
/**
 * <p>
 * Hash table and linked list implementation of the <tt>Set</tt> interface, with
 ...
 * the order in which elements were inserted into the set (
 * <i>insertion-order</i>). Note that insertion order is <i>not</i> affected if
 * an element is <i>re-inserted</i> into the set. (An element <tt>e</tt> is
 ...
 * cost associated with {@link TreeSet}. It can be used to produce a copy of a
 * set that has the same order as the original, regardless of the original set's
 * implementation:
 * 
 * <pre>
 *     void foo(Set s) {
 ...
 * Performance is likely to be just slightly below that of <tt>HashSet</tt>, due
 * to the added expense of maintaining the linked list, with one exception:
 * Iteration over a <tt>LinkedHashSet</tt> requires time proportional to the
 ...
 * <tt>HashSet</tt>. Note, however, that the penalty for choosing an excessively
