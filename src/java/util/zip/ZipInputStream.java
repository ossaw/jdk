...
public class ZipInputStream extends InflaterInputStream implements ZipConstants {
        ...
        switch (entry.method) {
            ...
            case STORED:
                ...
                if (remaining == 0 && entry.crc != crc.getValue()) {
                    throw new ZipException("invalid entry CRC (expected 0x" + Long.toHexString(entry.crc)
    ...
    private void readEnd(ZipEntry e) throws IOException {
        ...
        if (e.size != inf.getBytesWritten()) {
            throw new ZipException("invalid entry size (expected " + e.size + " but got " + inf
                    .getBytesWritten() + " bytes)");
        }
        if (e.csize != inf.getBytesRead()) {
            throw new ZipException("invalid entry compressed size (expected " + e.csize + " but got " + inf
                    .getBytesRead() + " bytes)");
        }
