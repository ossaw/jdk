 ...
 * ZLIB compression library. The ZLIB compression library was initially
...
public class Deflater {
    ...
    /**
     * Creates a new compressor using the specified compression level. If
     * 'nowrap' is true then the ZLIB header and checksum fields will not be
     * used in order to support the compression format used in both GZIP and
     * PKZIP.
     * 
     * @param level
     ...
     * the history buffer can be predetermined. When the data is later
     ...
     * strategy (and may be flushed); the new strategy will take effect only
     ...
     * {@code deflate} will compress the input available so far with the old
     * level (and may be flushed); the new level will take effect only after
     * that invocation.
     *
     * @param level
     ...
     * 
     * @return true if the input data buffer is empty and setInput() should be
     *         called in order to provide more input
     */
     ...
     * reached.
     * 
     * @return true if the end of the compressed data output stream has been
     *         reached
     */
     ...
     * {@code deflater.deflate(b, off, len)} yields the same result as the
     ...
     * the same result as the invocation of
     ...
     * the {@link #needsInput()} returns {@code true} after this invocation if
     ...
     * this method should be invoked again with the same {@code flush} parameter
    ...
    /**
     * Resets deflater so that a new set of input data can be processed. Keeps
     * current compression level and strategy settings.
     */
    public void reset() {
        synchronized (zsRef) {
            ensureOpen();
     ...
     * also be called automatically by the finalize() method. Once this method
     * is called, the behavior of the Deflater object is undefined.
     */
    public void end() {
        synchronized (zsRef) {
