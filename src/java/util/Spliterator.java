 ...
 * operations. Operations using a Spliterator that cannot split, or does so in a
 ...
 * {@link #IMMUTABLE}, {@link #CONCURRENT}, and {@link #SUBSIZED}. These may be
 * employed by Spliterator clients to control, specialize or simplify
 * computation. For example, a Spliterator for a {@link Collection} would report
 ...
 * the spliterator <em>binds</em> to the element source; and detection of
 * structural interference of the element source detected after binding.</a> A
 * <em>late-binding</em> Spliterator binds to the source of elements at the
 * point of first traversal, first split, or first query for estimated size,
 * rather than at the time the Spliterator is created. A Spliterator that is not
 * <em>late-binding</em> binds to the source of elements at the point of
 * construction or first invocation of any method. Modifications made to the
 * source prior to binding are reflected when the Spliterator is traversed.
 * After binding a Spliterator should, on a best-effort basis, throw
 * {@link ConcurrentModificationException} if structural interference is
 * detected. Spliterators that do this are called <em>fail-fast</em>. The bulk
 * traversal method ({@link #forEachRemaining forEachRemaining()}) of a
 ...
 * {@link #SIZED}, this value corresponds exactly to the number of elements that
 * would be encountered in a successful traversal. However, even when not
 ...
 * {@link Spliterator#tryAdvance(java.util.function.Consumer)} and
 * {@link Spliterator#forEachRemaining(java.util.function.Consumer)} box
 * primitive values to instances of their corresponding wrapper class. Such
 * boxing may undermine any performance advantages gained by using the primitive
 * specializations. To avoid boxing, the corresponding primitive-based methods
 * should be used. For example,
 * {@link Spliterator.OfInt#tryAdvance(java.util.function.IntConsumer)} and
 * {@link Spliterator.OfInt#forEachRemaining(java.util.function.IntConsumer)}
 * should be used in preference to
 * {@link Spliterator.OfInt#tryAdvance(java.util.function.Consumer)} and
 * {@link Spliterator.OfInt#forEachRemaining(java.util.function.Consumer)}.
 * Traversal of primitive values using boxing-based methods {@link #tryAdvance
 * tryAdvance()} and {@link #forEachRemaining(java.util.function.Consumer)
 * forEachRemaining()} does not affect the order in which the values,
 * transformed to boxed values, are encountered.
 *
 * @apiNote
 *          <p>
 *          Spliterators, like {@code Iterator}s, are for traversing the
 *          elements of a source. The {@code Spliterator} API was designed to
 *          support efficient parallel traversal in addition to sequential
 *          traversal, by supporting decomposition as well as single-element
 *          iteration. In addition, the protocol for accessing elements via a
 ...
 *          <ul>
 *          <li>The source cannot be structurally interfered with. <br>
 *          For example, an instance of
 *          {@link java.util.concurrent.CopyOnWriteArrayList} is an immutable
 *          source. A Spliterator created from the source reports a
 *          characteristic of {@code IMMUTABLE}.</li>
 *          <li>The source manages concurrent modifications. <br>
 *          For example, a key set of a
 *          {@link java.util.concurrent.ConcurrentHashMap} is a concurrent
 *          source. A Spliterator created from the source reports a
 *          characteristic of {@code CONCURRENT}.</li>
 *          <li>The mutable source provides a late-binding and fail-fast
 *          Spliterator. <br>
 *          Late binding narrows the window during which interference can affect
 *          the calculation; fail-fast detects, on a best-effort basis, that
 *          structural interference has occurred after traversal has commenced
 *          and throws {@link ConcurrentModificationException}. For example,
 *          {@link ArrayList}, and many other non-concurrent {@code Collection}
 *          classes in the JDK, provide a late-binding, fail-fast spliterator.
 *          </li>
 *          <li>The mutable source provides a non-late-binding but fail-fast
 *          Spliterator. <br>
 *          The source increases the likelihood of throwing
 *          {@code ConcurrentModificationException} since the window of
 *          potential interference is larger.</li>
 *          <li>The mutable source provides a late-binding and non-fail-fast
 *          Spliterator. <br>
 *          The source risks arbitrary, non-deterministic behavior after
 *          traversal has commenced since interference is not detected.</li>
 *          <li>The mutable source provides a non-late-binding and non-fail-fast
 *          Spliterator. <br>
 *          The source increases the risk of arbitrary, non-deterministic
 ...
 *          held in even locations, and unrelated tag data are held in odd
 ...
 *          parallel computation, here is one way to implement an associated
 *          parallel forEach, that illustrates the primary usage idiom of
 *          splitting off subtasks until the estimated amount of work is small
 *          enough to perform sequentially. Here we assume that the order of
 *          processing across subtasks doesn't matter; different (forked) tasks
 *          may further split and process elements concurrently in undetermined
 *          order. This example uses a
 *          {@link java.util.concurrent.CountedCompleter}; similar usages apply
 *          to other parallel task constructions.
 *
 *          <pre>
 * {@code
 ...
 *          </pre>
 *
 * @implNote If the boolean system property
 *           {@code org.openjdk.java.util.stream.tripwire} is set to
...
public interface Spliterator<T> {
	 ...
	 * {@link #ORDERED} the action is performed on the next element in encounter
	 ...
	 * current thread, until all elements have been processed or the action
	 * throws an exception. If this Spliterator is {@link #ORDERED}, actions are
	 * performed in encounter order. Exceptions thrown by the action are relayed
	 * to the caller.
	 *
	 ...
	 * non-null return:
	 * <ul>
	 * <li>the value reported for {@code estimateSize()} before splitting, must,
	 * after splitting, be greater than or equal to {@code estimateSize()} for
	 * this and the returned Spliterator; and</li>
	 ...
	 * constraints, and efficiency considerations.
	 *
	 * @apiNote An ideal {@code trySplit} method efficiently (without traversal)
	 *          divides its elements exactly in half, allowing balanced parallel
	 *          computation. Many departures from this ideal remain highly
	 *          effective; for example, only approximately splitting an
	 *          approximately balanced tree, or for a tree in which leaf nodes
	 *          may contain either one or two elements, failing to further split
	 ...
	 * yet been partially traversed, this estimate must be an accurate count of
	 ...
	 *          compute. For example, a sub-spliterator of an approximately
	 *          balanced binary tree may return a value that estimates the
	 *          number of elements to be half of that of its parent; if the root
	 ...
	 * The result is represented as ORed values from {@link #ORDERED},
	 * {@link #DISTINCT}, {@link #SORTED}, {@link #SIZED}, {@link #NONNULL},
	 * {@link #IMMUTABLE}, {@link #CONCURRENT}, {@link #SUBSIZED}. Repeated
	 ...
	 * no guarantees can be made about any computation using this Spliterator.
	 *
	...
	/**
	 * Returns {@code true} if this Spliterator's {@link #characteristics}
	 ...
	 *          any {@link List}. But no order is guaranteed for hash-based
	 *          collections such as {@link HashSet}. Clients of a Spliterator
	 *          that reports {@code ORDERED} are expected to preserve ordering
	 ...
	 * elements {@code x, y}, {@code !x.equals(y)}. This applies for example, to
	 * a Spliterator based on a {@link Set}.
	 */
	 ...
	 *          implement {@link NavigableSet} or {@link SortedSet} report
	 *          {@code SORTED}.
	 */
	 ...
	 *          {@code Collection} report this characteristic. Sub-spliterators,
	 *          such as those for {@link HashSet}, that cover a sub-set of
	 *          elements and approximate their reported size do not.
	 */
	public static final int SIZED = 0x00000040;

	/**
	 * Characteristic value signifying that the source guarantees that
	 * encountered elements will not be {@code null}. (This applies, for
	 * example, to most concurrent collections, queues, and maps.)
	 */
	 ...
	 *          guaranteeing accuracy with respect to elements present at the
	 *          point of Spliterator construction, but possibly not reflecting
	 ...
	 *          approximately balanced binary tree, will report {@code SIZED}
	 *          but not {@code SUBSIZED}, since it is common to know the size of
	 *          the entire tree but not the exact sizes of subtrees.
	 */
	...
	public interface OfPrimitive<T, T_CONS, T_SPLITR extends Spliterator.OfPrimitive<T, T_CONS, T_SPLITR>>
		 ...
		 * Spliterator is {@link #ORDERED} the action is performed on the next
		 * element in encounter order. Exceptions thrown by the action are
	...
	public interface OfInt extends OfPrimitive<Integer, IntConsumer, OfInt> {
		 ...
		 *           {@code IntConsumer}, by boxing the argument of
	...
	public interface OfLong extends OfPrimitive<Long, LongConsumer, OfLong> {
		 ...
		 *           otherwise the action is adapted to an instance of
		 *           {@code LongConsumer}, by boxing the argument of
