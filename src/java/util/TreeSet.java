 ...
 * {@code Comparator} for a precise definition of <i>consistent with equals</i>
 * .) This is so because the {@code Set} interface is defined in terms of the
 * {@code equals} operation, but a {@code TreeSet} instance performs all element
 * comparisons using its {@code compareTo} (or {@code compare}) method, so two
 * elements that are deemed equal by this method are, from the standpoint of the
