 ...
 * {@code tailMap} differ from the like-named {@code
 * SortedMap} methods in accepting additional arguments describing whether lower
 ...
 * {@code SortedMap} to allow existing implementations of {@code SortedMap} to
 * be compatibly retrofitted to implement {@code NavigableMap}, but extensions
...
public interface NavigableMap<K, V> extends SortedMap<K, V> {
     ...
     * progress (except through the iterator's own {@code
     * remove} operation), the results of the iteration are undefined. The set
     ...
     * in progress (except through the iterator's own {@code
     * remove} operation), the results of the iteration are undefined. The set
     * supports element removal, which removes the corresponding mapping from
     * the map, via the {@code Iterator.remove}, {@code Set.remove},
     * {@code removeAll}, {@code retainAll}, and {@code clear} operations. It
     ...
     * are equal, the returned map is empty unless {@code fromInclusive} and
     * {@code toInclusive} are both true. The returned map is backed by this
     ...
     * equal to, if {@code inclusive} is true) {@code fromKey}. The returned map
     * is backed by this map, so changes in the returned map are reflected in
     * this map, and vice-versa. The returned map supports all optional map
     * operations that this map supports.
     *
