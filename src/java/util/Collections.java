...
public class Collections {
	...
	static class SynchronizedNavigableMap<K, V> extends SynchronizedSortedMap<K, V>
		...
		public SortedMap<K, V> subMap(K fromKey, K toKey) {
			synchronized (mutex) {
				return new SynchronizedNavigableMap<>(nm.subMap(fromKey, true, toKey, false),
						mutex);
			}
		}
	...
	static class CheckedNavigableSet<E> extends CheckedSortedSet<E>
		...
		public NavigableSet<E> subSet(E fromElement, boolean fromInclusive, E toElement,
			...
			return checkedNavigableSet(
					ns.subSet(fromElement, fromInclusive, toElement, toInclusive), type);
		}
	...
	static class CheckedList<E> extends CheckedCollection<E> implements List<E> {
		 ...
		 *
		 * @throws ClassCastException
		 *             if the class of an element returned by the operator
		 *             prevents it from being added to this collection. The
		 *             exception may be thrown after some elements of the list
		...
		static class CheckedEntrySet<K, V> implements Set<Map.Entry<K, V>> {
			...
			public Object[] toArray() {
				...
				Object[] dest = (CheckedEntry.class.isInstance(source.getClass().getComponentType())
						? source : new Object[source.length]);
	...
	static class CheckedNavigableMap<K, V> extends CheckedSortedMap<K, V>
		...
		public Entry<K, V> lowerEntry(K key) {
			...
			return (null != lower) ? new CheckedMap.CheckedEntrySet.CheckedEntry<>(lower, valueType)
					: null;
		}
		...
		public Entry<K, V> floorEntry(K key) {
			...
			return (null != floor) ? new CheckedMap.CheckedEntrySet.CheckedEntry<>(floor, valueType)
					: null;
		}
		...
		public Entry<K, V> ceilingEntry(K key) {
			...
			return (null != ceiling)
					? new CheckedMap.CheckedEntrySet.CheckedEntry<>(ceiling, valueType) : null;
		}
		...
		public Entry<K, V> higherEntry(K key) {
			...
			return (null != higher)
					? new CheckedMap.CheckedEntrySet.CheckedEntry<>(higher, valueType) : null;
		}
		...
		public Entry<K, V> firstEntry() {
			...
			return (null != first) ? new CheckedMap.CheckedEntrySet.CheckedEntry<>(first, valueType)
					: null;
		}

		public Entry<K, V> lastEntry() {
			Entry<K, V> last = nm.lastEntry();
			return (null != last) ? new CheckedMap.CheckedEntrySet.CheckedEntry<>(last, valueType)
					: null;
		}
		...
		public NavigableMap<K, V> subMap(K fromKey, boolean fromInclusive, K toKey,
				boolean toInclusive) {
			return checkedNavigableMap(nm.subMap(fromKey, fromInclusive, toKey, toInclusive),
					keyType, valueType);
		}
	...
	private static class ReverseComparator2<T> implements Comparator<T>, Serializable {
		...
		public boolean equals(Object o) {
			return (o == this) || (o instanceof ReverseComparator2
	 ...
	 * @throws NullPointerException
	 *             if one collection contains a {@code null} element and
	 *             {@code null} is not an eligible element for the other
	 *             collection. (
	 *             <a href="Collection.html#optional-restrictions">optional</a>)
	 * @throws ClassCastException
	 *             if one collection contains an element that is of a type which
