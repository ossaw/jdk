...
/**
 * A specialized {@link Map} implementation for use with enum type keys. All of
 * the keys in an enum map must come from a single enum type that is specified,
 ...
 * <p>
 * Enum maps are maintained in the <i>natural order</i> of their keys (the order
 * in which the enum constants are declared). This is reflected in the iterators
 * returned by the collections views ({@link #keySet()}, {@link #entrySet()},
 * and {@link #values()}).
 * <p>
 * Iterators returned by the collection views are <i>weakly consistent</i>: they
 * will never throw {@link ConcurrentModificationException} and they may or may
 ...
 * {@link NullPointerException}. Attempts to test for the presence of a null key
 * or to remove one will, however, function properly. Null values are permitted.
 * <P>
 * Like most collection implementations <tt>EnumMap</tt> is not synchronized. If
 * multiple threads access an enum map concurrently, and at least one of the
 * threads modifies the map, it should be synchronized externally. This is
 * typically accomplished by synchronizing on some object that naturally
 * encapsulates the enum map. If no such object exists, the map should be
 * "wrapped" using the {@link Collections#synchronizedMap} method. This is best
 ...
 * likely (though not guaranteed) to be faster than their {@link HashMap}
 * counterparts.
 * <p>
 ...
 * @since 1.5
 */
public class EnumMap<K extends Enum<K>, V> extends AbstractMap<K, V> implements java.io.Serializable,
        Cloneable {
    /**
    ...
    /**
     * Array representation of this map. The ith element is the value to which
     * universe[i] is currently mapped, or null if it isn't mapped to anything,
     ...
     * map is an <tt>EnumMap</tt> instance, this constructor behaves identically
     * to {@link #EnumMap(EnumMap)}. Otherwise, the specified map must contain
     ...
     * returned set obeys the general contract outlined in {@link Map#keySet()}.
     * The set's iterator will return the keys in their natural order (the order
     ...
     * {@link Map#values()}. The collection's iterator will return the values in
     * the order their corresponding keys appear in map, which is their natural
     ...
     * returned set obeys the general contract outlined in {@link Map#keySet()}.
     * The set's iterator will return the mappings in the order their keys
     * appear in map, which is their natural order (the order in which the enum
    ...
    private class EntryIterator extends EnumMapIterator<Map.Entry<K, V>> {
        ...
        private class Entry implements Map.Entry<K, V> {
            ...
            public boolean equals(Object o) {
                ...
                return (e.getKey() == keyUniverse[index] && (ourValue == hisValue || (ourValue != null
    ...
    /**
     * Returns all of the values comprising K. The result is uncloned, cached,
