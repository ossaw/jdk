 ...
 * <tt>Handler</tt> (defaults to <tt>Level.ALL</tt>).</li>
 ...
 * class to use (defaults to <tt>java.util.logging.XMLFormatter</tt>)</li>
 * <li>&lt;handler-name&gt;.encoding the name of the character set encoding to
 * use (defaults to the default platform encoding).</li>
 * <li>&lt;handler-name&gt;.limit specifies an approximate maximum amount to
 * write (in bytes) to any one file. If this is zero, then there is no limit.
 * (Defaults to no limit).</li>
 * <li>&lt;handler-name&gt;.count specifies how many output files to cycle
 * through (defaults to 1).</li>
 * <li>&lt;handler-name&gt;.pattern specifies a pattern for generating the
 * output file name. See below for details. (Defaults to "%h/java%u.log").</li>
 * <li>&lt;handler-name&gt;.append specifies whether the FileHandler should
 ...
 * <li>java.util.logging.FileHandler.formatter=java.util.logging.SimpleFormatter
 * </li>
 * </ul>
 * <p>
...
public class FileHandler extends StreamHandler {
	 ...
	 * output to a target stream (b) keeps track of how many bytes have been
	 * written
	 */
	 ...
	 *                                 if there are IO problems opening the
	 *                                 files.
		...
		for (;;) {
			...
			synchronized (locks) {
				...
				while (channel == null && retries++ < 1) {
					try {
						channel = FileChannel.open(lockFilePath, CREATE_NEW,
								WRITE);
						fileCreated = true;
						...
						if (Files.isRegularFile(lockFilePath,
								LinkOption.NOFOLLOW_LINKS) && isParentWritable(
										lockFilePath)) {
							try {
								channel = FileChannel.open(lockFilePath, WRITE,
										APPEND);
	...
	/**
	 * Generate a file based on a user-supplied pattern, generation number, and
	 * an integer uniqueness suffix
	 * 
	 * @param pattern
	 *                   the pattern for naming the output file
	 * @param generation
				...
				if (ch2 == 't') {
					...
					if (isSetUID()) {
						...
						throw new IOException(
								"can't use %h in set UID program");
					}
					ix++;
					word = "";
					continue;
