 ...
 * Logger. It is important to note that the Logger returned by one of the
 * {@code getLogger} factory methods may be garbage collected at any time if a
 * strong reference to the Logger is not kept.
 * <p>
 ...
 * that this logger cares about. If a Logger's level is set to <tt>null</tt>,
 * then its effective level is inherited from its parent, which may in turn
 * obtain it recursively from its parent, and so on up the tree.
 * <p>
 ...
 * inherit its effective level from its parent.
 * <p>
 ...
 * to describe the logging message. It will then call a Filter (if present) to
 * do a more detailed check on whether the record should be published. If that
 * passes it will then publish the LogRecord to its output Handlers. By default,
 * loggers also publish to their parent's Handlers, recursively up the tree.
 * <p>
 * Each Logger may have a {@code ResourceBundle} associated with it. The
 * {@code ResourceBundle} may be specified by name, using the
 * {@link #getLogger(java.lang.String, java.lang.String)} factory method, or by
 * value - using the {@link #setResourceBundle(java.util.ResourceBundle)
 * setResourceBundle} method. This bundle will be used for localizing logging
 * messages. If a Logger does not have its own {@code ResourceBundle} or
 * resource bundle name, then it will inherit the {@code ResourceBundle} or
 * resource bundle name from its parent, recursively up the tree.
 * <p>
 ...
 * logger has (or inherits) a localization {@code ResourceBundle} and if the
 * {@code ResourceBundle} has a mapping for the msg string, then the msg string
 * is replaced by the localized value. Otherwise the original msg string is
 * used. Typically, formatters use java.text.MessageFormat style formatting to
 * format parameters, so for example a format string "{0} {1}" would format two
 ...
 * unnecessary message construction. For example, if the developer wants to log
 * system health status for diagnosis, with the String-accepting version, the
 ...
 * below, the status will only be collected when the log level FINER is enabled.
 * 
 * <pre>
 * <code>
 ...
 * {@link #setResourceBundle(java.util.ResourceBundle) setResourceBundle}, and
 * then only whether a resource bundle name was specified through the
 * {@link #getLogger(java.lang.String, java.lang.String) getLogger} factory
 * method. If no {@code ResourceBundle} or no resource bundle name is found,
 * then it will use the nearest {@code ResourceBundle} or resource bundle name
 * inherited from its parent tree.<br>
 * When a {@code ResourceBundle} was inherited or specified through the
 * {@link #setResourceBundle(java.util.ResourceBundle) setResourceBundle}
 * method, then that {@code ResourceBundle} will be used. Otherwise if the
 * logger only has or inherited a resource bundle name, then that resource
 * bundle name will be mapped to a {@code ResourceBundle} object, using the
 * default Locale at the time of logging. <br id="ResourceBundleMapping">
 * When mapping resource bundle names to {@code ResourceBundle} objects, the
 * logger will first try to use the Thread's
 * {@linkplain java.lang.Thread#getContextClassLoader() context class loader} to
 * map the given resource bundle name to a {@code ResourceBundle}. If the thread
 * context class loader is {@code null}, it will try the
 ...
 * like the "logp" method, but also take an explicit resource bundle object for
 * use in localizing the log message.
 * <li>
 * <p>
 ...
 * Logging framework will make a "best effort" to determine which class and
 * method called into the logging method. However, it is important to realize
 * that this automatically inferred information may only be approximate (or may
 * even be quite wrong!). Virtual machines are allowed to do extensive
 ...
 * <p>
 * <b>Subclassing Information:</b> Note that a LogManager class may provide its
 * own implementation of named Loggers for any point in the namespace.
...
public class Logger {
	...
	private static final class LoggerBundle {
		...
		final ResourceBundle userBundle; // Bundle set through
											// setResourceBundle.
	...
	// code
	private static final LoggerBundle SYSTEM_BUNDLE = new LoggerBundle(
			SYSTEM_LOGGER_RB_NAME, null);
	...
	private static final LoggerBundle NO_RESOURCE_BUNDLE = new LoggerBundle(
			null, null);
	...
	private ArrayList<LogManager.LoggerWeakRef> kids; // WeakReferences to
														// loggers that have us
														// as parent
	...
	/**
	 * The "global" Logger object is provided as a convenience to developers who
	 * are making casual use of the Logging package. Developers who are making
	 * serious use of the logging package (for example in products) should
	 * create and use their own Logger objects, with appropriate names, so that
	 * logging can be controlled on a suitable per-Logger granularity.
	 * Developers also need to keep a strong reference to their Logger objects
	 * to prevent them from being garbage collected.
	 * <p>
	 * 
	 * @deprecated Initialization of this field is prone to deadlocks. The field
	 *             must be initialized by the Logger class initialization which
	 *             may cause deadlocks with the LogManager class initialization.
	 *             In such cases two class initialization wait for each other to
	 *             complete. The preferred way to get the global logger object
	 *             is via the call <code>Logger.getGlobal()</code>. For
	 *             compatibility with old JDK versions where the
	 *             <code>Logger.getGlobal()</code> is not available use the call
	 *             <code>Logger.getLogger(Logger.GLOBAL_LOGGER_NAME)</code> or
	 ...
	 *                           dot-separated name and
	 *                           should normally be based on the package name or
	 *                           class name of
	 ...
	 *                           localizing messages for
	 *                           this logger. May be null if none of the
	 *                           messages require
	 *                           localization.
	...
	private void setCallersClassLoaderRef(Class<?> caller) {
		ClassLoader callersClassLoader = ((caller != null) ? caller
				.getClassLoader() : null);
	...
	private ClassLoader getCallersClassLoader() {
		return (callersClassLoaderRef != null) ? callersClassLoaderRef.get()
				: null;
	}
	...
	private static class SystemLoggerHelper {
		static boolean disableCallerCheck = getBooleanProperty(
	 ...
	 * created Logger. It is important to understand that a previously created
	 * Logger with the given name may be garbage collected at any time if there
	 * is no strong reference to the Logger. In particular, this means that two
	 * back-to-back calls like {@code getLogger("MyLogger").log(...)} may use
	 * different Logger objects named "MyLogger" if there is no strong reference
	 ...
	 * parent's Handlers. It will be registered in the LogManager global
	 * namespace.
	 * <p>
	 * Note: The LogManager may only retain a weak reference to the newly
	 * created Logger. It is important to understand that a previously created
	 * Logger with the given name may be garbage collected at any time if there
	 * is no strong reference to the Logger. In particular, this means that two
	 * back-to-back calls like {@code getLogger("MyLogger", ...).log(...)} may
	 * use different Logger objects named "MyLogger" if there is no strong
	 ...
	 *                           name of ResourceBundle to be used for
	 *                           localizing messages for
	 *                           this logger. May be {@code null} if none of the
	 *                           messages
	 ...
	 *                                  if the Logger already exists and uses a
	 *                                  different resource
	 *                                  bundle name; or if
	 *                                  {@code resourceBundleName} is
	 *                                  {@code null}
	 *                                  but the named logger has a resource
	 *                                  bundle set.
	 * @throws NullPointerException
	 *                                  if the name is null.
	 */
	...
	/**
	 * Create an anonymous Logger. The newly created Logger is not registered in
	 ...
	 * its effective level and handlers from the root logger. Changing its
	 * parent via the {@link #setParent(java.util.logging.Logger) setParent}
	 * method will still require the security permission specified by that
	 * method.
	 * <p>
	 *
	...
	public static Logger getAnonymousLogger(String resourceBundleName) {
		...
		Logger result = new Logger(null, resourceBundleName, Reflection
				.getCallerClass(), manager, false);
	...
	/**
	 * Retrieve the localization resource bundle for this logger. This method
	 * will return a {@code ResourceBundle} that was either set by the
	 * {@link #setResourceBundle(java.util.ResourceBundle) setResourceBundle}
	 * method or <a href="#ResourceBundleMapping">mapped from the the resource
	 * bundle name</a> set via the
	 * {@link Logger#getLogger(java.lang.String, java.lang.String) getLogger}
	 * factory method for the current default locale. <br>
	 * Note that if the result is {@code null}, then the Logger will use a
	 * resource bundle or resource bundle name inherited from its parent.
	 *
	...
	/**
	 * Retrieve the localization resource bundle name for this logger. This is
	 * either the name specified through the
	 * {@link #getLogger(java.lang.String, java.lang.String) getLogger} factory
	 * method, or the {@linkplain ResourceBundle#getBaseBundleName() base name}
	 * of the ResourceBundle set through
	 * {@link #setResourceBundle(java.util.ResourceBundle) setResourceBundle}
	 * method. <br>
	 * Note that if the result is {@code null}, then the Logger will use a
	 * resource bundle or resource bundle name inherited from its parent.
	 ...
	 *                           if a security manager exists, this logger is
	 *                           not anonymous,
	...
	public void log(LogRecord record) {
		...
		Logger logger = this;
		while (logger != null) {
			final Handler[] loggerHandlers = isSystemLogger ? logger
					.accessCheckedHandlers() : logger.getHandlers();
	 ...
	 * <p>
	 * If the logger is currently enabled for the given message level then the
	 * message is constructed by invoking the provided supplier function and
	 ...
	 * rather than the LogRecord parameters property. Thus it is processed
	 ...
	 * message and the given {@link Throwable} are then stored in a
	 * {@link LogRecord} which is forwarded to all registered output handlers.
	 * <p>
	 * Note that the thrown argument is stored in the LogRecord thrown property,
	 * rather than the LogRecord parameters property. Thus it is processed
	 * specially by output Formatters and is not treated as a formatting
	 ...
	 *                     The string message (or a key in the message catalog)
	 * @deprecated Use
	 *             {@link #logrb(java.util.logging.Level, java.lang.String, java.lang.String, java.util.ResourceBundle, java.lang.String, java.lang.Object...)}
	 *             instead.
	 */
	@Deprecated
	...
	/**
	 * Log a message, specifying source class, method, and resource bundle, with
	 * an optional list of message parameters.
	 * <p>
	 * If the logger is currently enabled for the given message level then a
	 * corresponding LogRecord is created and forwarded to all the registered
	 * output Handler objects.
	 * <p>
	 * The {@code msg} string is localized using the given resource bundle. If
	 * the resource bundle is {@code null}, then the {@code msg} string is not
	 * localized.
	 * <p>
	 * 
	 * @param level
	 ...
	 * @param bundle
	 *                     Resource bundle to localize {@code msg}, can be
	 *                     {@code null}.
	 * @param msg
	 *                     The string message (or a key in the message catalog)
	 * @param params
	 ...
	 *                     Throwable associated with log message.
	 * @deprecated Use
	 *             {@link #logrb(java.util.logging.Level, java.lang.String, java.lang.String, java.util.ResourceBundle, java.lang.String, java.lang.Throwable)}
	 *             instead.
	 */
	@Deprecated
	public void logrb(Level level, String sourceClass, String sourceMethod,
	...
	/**
	 * Log a INFO message, which is only to be constructed if the logging level
	...
	/**
	 * Log a FINE message, which is only to be constructed if the logging level
	 * is such that the message will actually be logged.
	 * <p>
	 * If the logger is currently enabled for the FINE message level then the
	 * message is constructed by invoking the provided supplier function and
	...
	/**
	 * Log a FINER message, which is only to be constructed if the logging level
	 * is such that the message will actually be logged.
	 * <p>
	 * If the logger is currently enabled for the FINER message level then the
	 * message is constructed by invoking the provided supplier function and
	...
	private static ResourceBundle findSystemResourceBundle(
			final Locale locale) {
					...
					public ResourceBundle run() {
						try {
							return ResourceBundle.getBundle(
									SYSTEM_LOGGER_RB_NAME, locale, ClassLoader
	...
	/**
	 * Private utility method to map a resource bundle name to an actual
	 * resource bundle, using a simple one-entry cache. Returns null for a null
	 * name. May also return null if we can't find the resource bundle and there
	 * is no suitable previous cached value.
	 *
	 * @param name
	 *                               the ResourceBundle to locate
	 * @param userCallersClassLoader
	 *                               if true search using the caller's
	 *                               ClassLoader
	...
	private synchronized ResourceBundle findResourceBundle(String name,
		...
		// {@linkplain java.lang.ClassLoader#getSystemClassLoader() system
		// ClassLoader}.
	...
	private synchronized void setupResourceInfo(String name,
		...
		if (lb.resourceBundleName != null) {
			...
			throw new IllegalArgumentException(lb.resourceBundleName + " != "
					+ name);
		}
	...
	/**
	 * Sets a resource bundle on this logger. All messages will be logged using
	 * the given resource bundle for its specific
	 * {@linkplain ResourceBundle#getLocale locale}.
	 * 
	 * @param bundle
	 *               The resource bundle that this logger shall use.
	 * @throws NullPointerException
	 *                                  if the given bundle is {@code null}.
	 * @throws IllegalArgumentException
	 *                                  if the given bundle doesn't have a
	 *                                  {@linkplain ResourceBundle#getBaseBundleName
	 *                                  base name}, or
	 *                                  if this logger already has a resource
	 *                                  bundle set but the
	 *                                  given bundle has a different base name.
	 * @throws SecurityException
	 *                                  if a security manager exists, this
	 *                                  logger is not anonymous,
	 *                                  and the caller does not have
	 *                                  LoggingPermission("control").
	 * @since 1.8
	 */
	public void setResourceBundle(ResourceBundle bundle) {
		...
		if (baseName == null || baseName.isEmpty()) {
			throw new IllegalArgumentException(
					"resource bundle must have a name");
		}
	...
	private void doSetParent(Logger newParent) {
		...
		synchronized (treeLock) {
			...
			if (parent != null) {
				...
				for (Iterator<LogManager.LoggerWeakRef> iter = parent.kids
	...
	final void removeChildLogger(LogManager.LoggerWeakRef child) {
		synchronized (treeLock) {
			for (Iterator<LogManager.LoggerWeakRef> iter = kids.iterator(); iter
					.hasNext();) {
	...
	private LoggerBundle getEffectiveLoggerBundle() {
		...
		while (target != null) {
			...
			final String rbName = isSystemLogger
					// ancestor of a system logger is expected to be a system
					// logger.
					// ignore resource bundle name if it's not.
					? (target.isSystemLogger ? trb.resourceBundleName : null)
					: target.getResourceBundleName();
			if (rbName != null) {
				return LoggerBundle.get(rbName, findResourceBundle(rbName,
						true));
			}
