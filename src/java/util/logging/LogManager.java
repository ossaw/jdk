 ...
 * <ul>
 * <li>Manages a hierarchical namespace of Logger objects. All named Loggers are
 * stored in this namespace.
 * <li>Manages a set of logging control properties. These are simple key-value
 * pairs that can be used by Handlers and other logging objects to configure
 * themselves.
 * </ul>
 * <p>
 * The global LogManager object can be retrieved using
 * LogManager.getLogManager(). The LogManager object is created during class
 * initialization and cannot subsequently be changed.
 * <p>
 ...
 * can use <tt>readConfiguration(InputStream)</tt> to define properties in the
 * LogManager.
 * <p>
 * If "java.util.logging.config.class" property is <b>not</b> set, then the
 ...
 * <ul>
 * <li>A property "handlers". This defines a whitespace or comma separated list
 * of class names for handler classes to load and register as handlers on the
 * root Logger (the Logger named ""). Each class name must be for a Handler
 * class which has a default constructor. Note that these Handlers may be
 * created lazily, when they are first used.
 *
 * <li>A property "&lt;logger&gt;.handlers". This defines a whitespace or comma
 * separated list of class names for handlers classes to load and register as
 * handlers to the specified logger. Each class name must be for a Handler class
 * which has a default constructor. Note that these Handlers may be created
 * lazily, when they are first used.
 *
 * <li>A property "&lt;logger&gt;.useParentHandlers". This defines a boolean
 * value. By default every logger calls its parent in addition to handling the
 * logging message itself, this often result in messages being handled by the
 * root logger as well. When setting this property to false a Handler needs to
 * be configured for this logger otherwise no logging messages are delivered.
 *
 * <li>A property "config". This property is intended to allow arbitrary
 * configuration code to be run. The property defines a whitespace or comma
 * separated list of class names. A new instance will be created for each named
 * class. The default constructor of each class may execute arbitrary code to
 * update the logging configuration, such as setting logger levels, adding
 * handlers, adding filters, etc.
 * </ul>
 * <p>
 * Note that all classes loaded during LogManager configuration are first
 * searched on the system class path before any user class path. That includes
 * the LogManager class, any config classes, and any handler classes.
 * <p>
...
public class LogManager {
    ...
    // count to allow for cases where the same listener is registered many
    // times.
    ...
    static {
        manager = AccessController.doPrivileged(
    ...
    private class Cleaner extends Thread {
        ...
        private Cleaner() {
            /*
             * Set context class loader to null in order to avoid keeping a
             * strong reference to an application classloader.
             */
    ...
    /**
     * Lazy initialization: if this instance of manager is the global manager
     * then this method will read the initial configuration and add the root
     * logger and global logger by calling addLogger().
     *
     * Note that it is subtly different from what we do in LoggerContext. In
     * LoggerContext we're patching up the logger context tree in order to add
     * the root and global logger *to the context tree*.
     *
     * For this to work, addLogger() must have already have been called once on
     * the LogManager instance for the default logger being added.
     *
     * This is why ensureLogManagerInitialized() needs to be called before any
     * logger is added to any logger context.
     *
     */
            ...
            try {
                AccessController.doPrivileged(new PrivilegedAction<Object>() {
                    ...
                    public Object run() {
                        ...
                        // Create and retain Logger for the root of the
                        // namespace.
                        ...
                        // Do not call Logger.getGlobal() here as this might
                        // trigger
            ...
            synchronized (this) {
                if (!readPrimordialConfiguration) {
                    ...
                    try {
                        ...
                        assert false : "Exception raised while reading logging configuration: "
                                + ex;
                    }
                }
            }
    ...
    /**
     * Adds an event listener to be invoked when the logging properties are
     * re-read. Adding multiple instances of the same event Listener results in
     * multiple entries in the property event listener table.
     *
     * <p>
     * <b>WARNING:</b> This method is omitted from this class in all subset
     ...
     *             logging configuration by overridding the
     *             {@link #readConfiguration readConfiguration} method.
     */
    @Deprecated
    ...
    /**
     * Removes an event listener for property change events. If the same
     * listener instance has been added to the listener table through multiple
     * invocations of <CODE>addPropertyChangeListener</CODE>, then an equivalent
     * number of <CODE>removePropertyChangeListener</CODE> invocations are
     * required to remove all instances of that listener from the listener
     * table.
     * <P>
    ...
    // Returns the LoggerContext for the user code (i.e. application or
    // AppContext).
    ...
    private LoggerContext getUserContext() {
        ...
        if (sm != null && javaAwtAccess != null) {
            // for each applet, it has its own LoggerContext isolated from
            // others
    ...
    Logger demandSystemLogger(String name, String resourceBundleName) {
        ...
        final Logger sysLogger = getSystemContext().demandLogger(name,
                resourceBundleName);
        ...
        // LogManager will set the sysLogger's handlers via LogManager.addLogger
        // method.
        if (logger != sysLogger && sysLogger
                .accessCheckedHandlers().length == 0) {
    ...
    // doesn't exist in the user context, it'll also be added to the user
    // context.
    ...
    class LoggerContext {
        ...
        private void ensureDefaultLogger(Logger logger) {
            ...
            if (!requiresDefaultLoggers() || logger == null
                    || logger != Logger.global
                            && logger != LogManager.this.rootLogger) {
        ...
        synchronized boolean addLocalLogger(Logger logger,
                boolean addDefaultLoggersIfNeeded) {
            ...
            // implementation as a handler class may be only visible to
            // LogManager
        ...
        // If logger.getUseParentHandlers() returns 'true' and any of the
        // logger's
        // parents have levels or handlers defined, make sure they are
        // instantiated.
        private void processParentHandlers(final Logger logger,
                final String name) {
            final LogManager owner = getOwner();
            AccessController.doPrivileged(new PrivilegedAction<Void>() {
                @Override
                public Void run() {
                    if (logger != owner.rootLogger) {
                        boolean useParent = owner.getBooleanProperty(name
            ...
            for (;;) {
                ...
                if (owner.getProperty(pname + ".level") != null || owner
    ...
    final class SystemLoggerContext extends LoggerContext {
        ...
        Logger demandLogger(String name, String resourceBundleName) {
            ...
            if (result == null) {
                ...
                Logger newLogger = new Logger(name, resourceBundleName, null,
                        getOwner(), true);
                do {
     ...
     * @param logger
     *               the new logger.
     * @return true if the argument logger was registered successfully, false if
     *         a logger of that name already exists.
     * @exception NullPointerException
     ...
     * sensitive logging. It is also important to note that the Logger
     * associated with the String {@code name} may be garbage collected at any
     * time if there is no strong reference to the Logger. The caller of this
     * method must check the return value for null in order to properly handle
     ...
     * method only reports on the loggers that are currently registered. It is
     * also important to note that this method only returns the name of a
     * Logger, not a strong reference to the Logger itself. The returned String
     * does nothing to prevent the Logger from being garbage collected. In
     * particular, if the returned name is passed to
     * {@code LogManager.getLogger()}, then the caller must check the return
     * value from {@code LogManager.getLogger()} for null to properly handle the
     * case where the Logger has been garbage collected in the time since its
     * name was returned by this method.
     * <p>
     * 
     ...
     *                              if there are IO problems reading the
     *                              configuration.
     */
    public void readConfiguration() throws IOException, SecurityException {
        ...
        if (cname != null) {
            try {
                ...
                // calling readConfiguration(InputStream) with a suitable
                // stream.
                try {
                ...
                System.err.println("Logging configuration class \"" + cname
                        + "\" failed");
                System.err.println("" + ex);
     ...
     * Handlers and (except for the root logger) sets the level to null. The
     * root logger's level is set to Level.INFO.
     *
     * @exception SecurityException
     *                              if a security manager exists and if the
     *                              caller does not
    ...
    /**
     * Reinitialize the logging properties and reread the logging configuration
     * from the given stream, which should be in java.util.Properties format. A
    ...
    public void readConfiguration(InputStream ins) throws IOException,
        ...
        if (listeners != null) {
            ...
            Object ev = Beans.newPropertyChangeEvent(LogManager.class, null,
                    null, null);
    ...
    private final Permission controlPermission = new LoggingPermission(
            "control", null);
     ...
     * <p>
     * If the check fails we throw a SecurityException, otherwise we return
     * normally.
     *
     * @exception SecurityException
     *                              if a security manager exists and if the
    ...
    /**
     * String representation of the {@link javax.management.ObjectName} for the
    ...
    /**
     * Returns <tt>LoggingMXBean</tt> for managing loggers. An alternative way
     * to manage loggers is through the
     * {@link java.lang.management.PlatformLoggingMXBean} interface that can be
     * obtained by calling:
     * 
     * <pre>
    ...
    /**
     * A class that provides access to the java.beans.PropertyChangeListener and
     * java.beans.PropertyChangeEvent without creating a static dependency on
     * java.beans. This class can be removed once the addPropertyChangeListener
     * and removePropertyChangeListener methods are removed.
     */
    private static class Beans {
        private static final Class<?> propertyChangeListenerClass = getClass(
                "java.beans.PropertyChangeListener");

        private static final Class<?> propertyChangeEventClass = getClass(
                "java.beans.PropertyChangeEvent");

        private static final Method propertyChangeMethod = getMethod(
                propertyChangeListenerClass, "propertyChange",
                propertyChangeEventClass);

        private static final Constructor<?> propertyEventCtor = getConstructor(
                propertyChangeEventClass, Object.class, String.class,
        ...
        static Object newPropertyChangeEvent(Object source, String prop,
            ...
            try {
                return propertyEventCtor.newInstance(source, prop, oldValue,
                        newValue);
        ...
        /**
         * Invokes the given PropertyChangeListener's propertyChange method with
         * the given event.
         */
