 ...
 * method name and source class name, then the LogRecord class will infer them
 ...
 * <li>Because objects in the parameters array may not be serializable, during
 * serialization all objects in the parameters array are written as the
 * corresponding Strings (using Object.toString).
 * <li>The ResourceBundle is not transmitted as part of the serialized form, but
 * the resource bundle name is, and the recipient object's readObject method
 * will attempt to locate a suitable resource bundle.
 * </ul>
 *
 * @since 1.4
 */
...
public class LogRecord implements java.io.Serializable {
    ...
    /**
     * The default value of threadID will be the current thread's thread id, for
     * ease of correlation, unless it is greater than MIN_SEQUENTIAL_THREAD_ID,
     * in which case we try harder to keep our promise to keep threadIDs unique
     * by avoiding collisions due to 32-bit wraparound. Unfortunately,
     * LogRecord.getThreadID() returns int, while Thread.getId() returns long.
     */
     ...
     * which assigns unique sequence numbers to each new LogRecord in increasing
     * order.
     * 
     ...
     * <p>
     * Sequence numbers are normally assigned in the LogRecord constructor, so
    ...
    private boolean isLoggerImplFrame(String cname) {
        ...
        return (cname.equals("java.util.logging.Logger") || cname.startsWith(
                "java.util.logging.LoggingProxyImpl") || cname.startsWith("sun.util.logging."));
    }
}
