 ...
 * contexts) isolated parallel computations that may generate subtasks. Class
 * {@code SplittableRandom} supports methods for producing pseudorandom numbers
 * of type {@code int}, {@code long}, and {@code double} with similar usages as
 * for class {@link java.util.Random} but differs in the following ways:
 * <ul>
 * <li>Series of generated values pass the DieHarder suite testing independence
 * and uniformity properties of random number generators. (Most recently
 * validated with
 * <a href="http://www.phy.duke.edu/~rgb/General/dieharder.php"> version
 * 3.31.1</a>.) These tests validate only the methods for certain types and
 * ranges, but similar properties are expected to hold, at least approximately,
 * for others as well. The <em>period</em> (length of any series of generated
 * values before it repeats) is at least 2<sup>64</sup>.</li>
 * <li>Method {@link #split} constructs and returns a new SplittableRandom
 * instance that shares no mutable state with the current instance. However,
 * with very high probability, the values collectively generated by the two
 * objects have the same statistical properties as if the same quantity of
 * values were generated by a single thread using a single {@code
 * SplittableRandom} object.</li>
 * <li>Instances of SplittableRandom are <em>not</em> thread-safe. They are
 * designed to be split, not shared, across threads. For example, a
 * {@link java.util.concurrent.ForkJoinTask fork/join-style} computation using
 * random numbers might include a construction of the form {@code new
 * Subtask(aSplittableRandom.split()).fork()}.
 * <li>This class provides additional methods for generating random streams,
 * that employ the above techniques when used in {@code
 * stream.parallel()} mode.</li>
 * </ul>
 * <p>
 * Instances of {@code SplittableRandom} are not cryptographically secure.
...
public final class SplittableRandom {
     ...
     * function is based on David Stafford's
     * (http://zimbry.blogspot.com/2011/09/better-bit-mixing-improving-on.html)
     * "Mix13" variant of the "64-bit finalizer" function in Austin Appleby's
     ...
     * mixGamma) uses different (Murmurhash3's) mix constants. To avoid
     * potential weaknesses in bit-mixing transformations, we restrict gammas to
     * odd values with at least 24 0-1 or 1-0 bit transitions. Rather than
     * rejecting candidates with too few or too many bits set, method mixGamma
     * flips some bits (which has the effect of mapping at most 4 to any given
     * gamma value). This reduces the effective set of 64bit odd gamma values by
     ...
     * represent the seed rather than use an explicit SplittableRandom. To
     * bootstrap the defaultGen, we start off using a seed based on current time
    ...
    /**
     * The golden ratio scaled to 64bits, used as the initial gamma value for
     * (unsplit) SplittableRandoms.
     */
    private static final long GOLDEN_GAMMA = 0x9e3779b97f4a7c15L;

    /**
     * The least non-zero value returned by nextDouble(). This value is scaled
     * by a random value of 53 bits to produce a result.
     */
    ...
    /*
     * Internal versions of nextX methods used by streams, as well as the public
     * nextX(origin, bound) methods. These exist mainly to avoid the need for
     * multiple versions of stream spliterators across the different exported
     * forms of streams.
     */
    ...
    final long internalNextLong(long origin, long bound) {
         ...
         * over-represented values. The expected number of iterations under an
         * ideal generator varies from 1 to 2, depending on the bound. The loop
         * itself takes an unlovable form. Because the first candidate is
    ...
    /**
     * Creates a new SplittableRandom instance that is likely to generate
     * sequences of values that are statistically independent of those of any
     * other instances in the current program; and may, and typically does, vary
    ...
    /**
     * Constructs and returns a new SplittableRandom instance that shares no
     * mutable state with this instance. However, with very high probability,
     * the set of values collectively generated by the two objects has the same
     * statistical properties as if the same quantity of values were generated
     * by a single thread using a single SplittableRandom object. Either or both
     * of the two objects may be further split using the {@code split()} method,
     * and the same expected statistical properties apply to the entire set of
     * generators constructed by such recursive splitting.
     *
     ...
     * pseudorandom {@code int} values from this generator and/or one split from
     * it.
     *
     * @param streamSize
     ...
     * Returns a stream producing the given {@code streamSize} number of
     * pseudorandom {@code int} values from this generator and/or one split from
     * it; each value conforms to the given origin (inclusive) and bound
     * (exclusive).
     *
     * @param streamSize
     *        the number of values to generate
