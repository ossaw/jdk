 ...
 *
 * <p>
 * Commands submitted using the {@link Executor#execute(Runnable)} and
 * {@link ExecutorService} {@code submit} methods are scheduled with a requested
 * delay of zero. Zero and negative delays (but not periods) are also allowed in
 * {@code schedule} methods, and are treated as requests for immediate
 * execution.
 *
 * <p>
 * All {@code schedule} methods accept <em>relative</em> delays and periods as
 * arguments, not absolute times or dates. It is a simple matter to transform an
 * absolute time represented as a {@link java.util.Date} to the required form.
 * For example, to schedule at a certain future {@code date}, you can use:
 * {@code schedule(task,
 * date.getTime() - System.currentTimeMillis(),
 * TimeUnit.MILLISECONDS)}. Beware however that expiration of a relative delay
 * need not coincide with the current {@code Date} at which the task is enabled
 * due to network time synchronization protocols, clock drift, or other factors.
 *
 * <p>
 * The {@link Executors} class provides convenient factory methods for the
 * ScheduledExecutorService implementations provided in this package.
 *
 * <h3>Usage Example</h3>
 *
...
public interface ScheduledExecutorService extends ExecutorService {
	 ...
	 *                     the time to delay first execution
	 * @param delay
	 *                     the delay between the termination of one execution
	 *                     and the
	 *                     commencement of the next
	 * @param unit
	 *                     the time unit of the initialDelay and delay
	 *                     parameters
	 * @return a ScheduledFuture representing pending completion of the task,
	 *         and whose {@code get()} method will throw an exception upon
	 *         cancellation
