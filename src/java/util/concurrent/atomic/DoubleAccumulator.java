 ...
 * function is applied with the current value as its first argument, and the
 * given update as the second argument. For example, to maintain a running
 * maximum value, you could supply {@code Double::max} along with {@code
 * Double.NEGATIVE_INFINITY} as the identity. The order of accumulation within
 * or across threads is not guaranteed. Thus, this class may not be applicable
 * if numerical stability is required, especially when combining values of
 * substantially different orders of magnitude.
 * <p>
 * Class {@link DoubleAdder} provides analogs of the functionality of this class
 * for the common special case of maintaining sums. The call
 * {@code new DoubleAdder()} is equivalent to {@code new
...
public class DoubleAccumulator extends Striped64 implements Serializable {
    ...
    public void accumulate(double x) {
        ...
        int m;
        Cell a;
        if ((as = cells) != null || (r = Double.doubleToRawLongBits(function.applyAsDouble(Double
                .longBitsToDouble(b = base), x))) != b && !casBase(b, r)) {
            boolean uncontended = true;
            if (as == null || (m = as.length - 1) < 0 || (a = as[getProbe() & m]) == null
                    || !(uncontended = (r = Double.doubleToRawLongBits(function.applyAsDouble(Double
                            .longBitsToDouble(v = a.value), x))) == v || a.cas(v, r)))
    ...
    private static class SerializationProxy implements Serializable {
        ...
        /**
         * Returns a {@code DoubleAccumulator} object with initial state held by
         * this proxy.
         *
         * @return a {@code DoubleAccumulator} object with initial state held by
         *         this proxy.
         */
    ...
    /**
     * Returns a <a href=
     * "../../../../serialized-form.html#java.util.concurrent.atomic.DoubleAccumulator.SerializationProxy">
     * SerializationProxy</a> representing the state of this instance.
     *
     * @return a {@link SerializationProxy} representing the state of this
     *         instance
     */
