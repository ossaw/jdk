...
/**
 * One or more variables that together maintain an initially zero {@code double}
 ...
 * (or, equivalently {@link #doubleValue}) returns the current total combined
 * across the variables maintaining the sum. The order of accumulation within or
...
public class DoubleAdder extends Striped64 implements Serializable {
	...
	/*
	 * Note that we must use "long" for underlying representations, because
	 * there is no compareAndSet for double, due to the fact that the bitwise
	 * equals used in any CAS implementation is not the same as double-precision
	 * equals. However, we use CAS only to detect and alleviate contention, for
	 * which bitwise equals works best anyway. In principle, the long/double
	 * conversions used here should be essentially free on most platforms since
	...
	private static class SerializationProxy implements Serializable {
		...
		/**
		 * Returns a {@code DoubleAdder} object with initial state held by this
		 * proxy.
		 *
		 * @return a {@code DoubleAdder} object with initial state held by this
		 *         proxy.
		 */
	...
	/**
	 * Returns a <a href=
	 * "../../../../serialized-form.html#java.util.concurrent.atomic.DoubleAdder.SerializationProxy">
