 ...
 *
 * <p>
 * LongAdders can be used with a {@link java.util.concurrent.ConcurrentHashMap}
 * to maintain a scalable frequency map (a form of histogram or multiset). For
 * example, to add a count to a {@code ConcurrentHashMap
 * <String,LongAdder> freqs}, initializing if not already present, you can use
 * {@code
...
public class LongAdder extends Striped64 implements Serializable {
	...
	private static class SerializationProxy implements Serializable {
		...
		/**
		 * Return a {@code LongAdder} object with initial state held by this
		 * proxy.
		 *
		 * @return a {@code LongAdder} object with initial state held by this
		 *         proxy.
