 ...
 * single variables. In essence, the classes in this package extend the notion
 * of {@code volatile} values, fields, and array elements to those that also
 * provide an atomic conditional update operation of the form:
 *
 * <pre>
 ...
 * the {@code expectedValue}, reporting {@code true} on success. The classes in
 ...
 * {@link java.util.concurrent.atomic.AtomicReference} each provide access and
 * updates to a single variable of the corresponding type. Each class also
 * provides appropriate utility methods for that type. For example, classes
 * {@code AtomicLong} and {@code AtomicInteger} provide atomic increment
 ...
 * reflection-based utilities that provide access to the associated field types.
 * These are mainly of use in atomic data structures in which several
 * {@code volatile} fields of the same node (for example, the links of a tree
 * node) are independently subject to atomic updates. These classes enable
 ...
 * {@link java.util.concurrent.atomic.AtomicReferenceArray} classes further
 ...
 * efficient than {@code
 * compareAndSet} in the normal case, but differs in that any given invocation
 * of the {@code weakCompareAndSet} method may return {@code
 * false} <em>spuriously</em> (that is, for no apparent reason). A {@code false}
 * return means only that the operation may be retried if desired, relying on
 * the guarantee that repeated invocation when the variable holds
 * {@code expectedValue} and no other thread is also attempting to set the
 * variable will eventually succeed. (Such spurious failures may for example be
 * due to memory contention effects that are unrelated to whether the expected
 * and current values are equal.) Additionally {@code weakCompareAndSet} does
 * not provide ordering guarantees that are usually needed for synchronization
 * control. However, the method may be useful for updating counters and
 * statistics when such updates are unrelated to the other happens-before
 * orderings of a program. When a thread sees an update to an atomic variable
 * caused by a {@code weakCompareAndSet}, it does not necessarily see updates to
 ...
 * methods such as {@code equals}, {@code hashCode} and {@code compareTo}.
 * (Because atomic variables are expected to be mutated, they are poor choices
 * for hash table keys.) Additionally, classes are provided only for those types
 * that are commonly useful in intended applications. For example, there is no
