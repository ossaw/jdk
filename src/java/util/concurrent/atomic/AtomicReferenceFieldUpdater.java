 ...
 * {@code volatile} reference fields of designated classes. This class is
 * designed for use in atomic data structures in which several reference fields
 * of the same node are independently subject to atomic updates. For example, a
 * tree node might be declared as
 *
 * <pre>
 * {
 *     &#64;code
 ...
 * can guarantee atomicity only with respect to other invocations of
...
public abstract class AtomicReferenceFieldUpdater<T, V> {
    ...
    /**
     * Creates and returns an updater for objects with the given field. The
     * Class arguments are needed to check that reflective types and generic
     * types match.
     *
     * @param tclass
    ...
    public static <U, W> AtomicReferenceFieldUpdater<U, W> newUpdater(
        ...
        return new AtomicReferenceFieldUpdaterImpl<U, W>(tclass, vclass,
                fieldName, Reflection.getCallerClass());
    }
     ...
     * the given updated value if the current value {@code ==} the expected
     * value. This method is guaranteed to be atomic with respect to other calls
     * to {@code compareAndSet} and {@code set}, but not necessarily with
     ...
     * updated value. This operation is guaranteed to act as a volatile store
    ...
    /**
     * Eventually sets the field of the given object managed by this updater to
    ...
    /**
     * Gets the current value held in the field of the given object managed by
     * this updater.
     *
     * @param obj
    ...
    private static final class AtomicReferenceFieldUpdaterImpl<T, V> extends
        ...
        /*
         * Internal type checks within all update methods contain internal
         * inlined optimizations checking for the common cases where the class
         * is final (in which case a simple getClass comparison suffices) or is
         * of type Object (in which case no check is needed because all objects
         * are instances of Object). The Object case is handled simply by
         * setting vclass to null in constructor. The targetCheck and
         * updateCheck methods are invoked when these faster screenings fail.
         */

        AtomicReferenceFieldUpdaterImpl(final Class<T> tclass,
                final Class<V> vclass, final String fieldName,
                final Class<?> caller) {
            final Field field;
         ...
         */
        private static boolean isAncestor(ClassLoader first,
                ClassLoader second) {
