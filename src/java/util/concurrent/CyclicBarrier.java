 ...
 *
 * <p>
 * A {@code CyclicBarrier} supports an optional {@link Runnable} command that is
 * run once per barrier point, after the last thread in the party arrives, but
 * before any threads are released. This <em>barrier action</em> is useful for
 * updating shared-state before any of the parties continue.
 *
 * <p>
 * <b>Sample usage:</b> Here is an example of using a barrier in a parallel
 ...
 * the merger determines that a solution has been found then {@code done()} will
 * return {@code true} and each worker will terminate.
 *
 * <p>
 ...
 * it is released. To facilitate this, each invocation of {@link #await} returns
 * the arrival index of that thread at the barrier. You can then choose which
 * thread should execute the barrier action, for example:
 * 
 * <pre>
 *  {@code
 * if (barrier.await() == 0) {
 ...
 *
 * <p>
 * The {@code CyclicBarrier} uses an all-or-none breakage model for failed
 ...
 * {@link BrokenBarrierException} (or {@link InterruptedException} if they too
...
public class CyclicBarrier {
	 ...
	 * generation changes whenever the barrier is tripped, or is reset. There
	 * can be many generations associated with threads using the barrier - due
	 * to the non-deterministic way the lock may be allocated to waiting threads
	 * - but only one of these can be active at a time (the one to which
	 * {@code count} applies) and all the rest are either broken or tripped.
	 * There need not be an active generation if there has been a break but no
	 * subsequent reset.
	 */
	 ...
	 * barrier {@linkplain #isBroken is broken} when {@code await} is invoked,
	 ...
	 *         {@code getParties() - 1} indicates the first to arrive and zero
	 *         indicates the last to arrive
	 * @throws InterruptedException
	 *                                if the current thread was interrupted
	 *                                while waiting
	 * @throws BrokenBarrierException
	 *                                if <em>another</em> thread was interrupted
	 *                                or timed out while
	 *                                the current thread was waiting, or the
	 *                                barrier was reset, or
	 *                                the barrier was broken when {@code await}
	 *                                was called, or the
	 *                                barrier action (if present) failed due to
	 *                                an exception
	 */
	 ...
	 *                                if the specified timeout elapses. In this
	 *                                case the barrier
	 *                                will be broken.
	 * @throws BrokenBarrierException
	 *                                if <em>another</em> thread was interrupted
	 ...
	 * {@link BrokenBarrierException}. Note that resets <em>after</em> a
	 * breakage has occurred for other reasons can be complicated to carry out;
	 * threads need to re-synchronize in some other way, and choose one to
	 * perform the reset. It may be preferable to instead create a new barrier
	 * for subsequent use.
	 */
	public void reset() {
		final ReentrantLock lock = this.lock;
