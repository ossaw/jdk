 ...
 * to invocations of the {@link #countDown} method, after which all waiting
 * threads are released and any subsequent invocations of {@link #await await}
 ...
 * {@link #await await} wait at the gate until it is opened by a thread invoking
 ...
 * <li>The second is a completion signal that allows the driver to wait until
 ...
 * latch, and queue all the Runnables to an Executor. When all sub-parts are
 * complete, the coordinating thread will be able to pass through await. (When
 * threads must repeatedly count down in this way, instead use a
 * {@link CyclicBarrier}.)
 *
 * <pre>
 *  {@code
 ...
 * actions following a successful return from a corresponding {@code await()} in
 * another thread.
 *
 * @since 1.5
 * @author Doug Lea
