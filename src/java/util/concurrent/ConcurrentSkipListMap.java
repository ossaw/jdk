 ...
 * <a href="http://en.wikipedia.org/wiki/Skip_list" target="_top">SkipLists</a>
 ...
 * <em>not</em> support the {@code Entry.setValue} method. (Note however that it
 * is possible to change mappings in the associated map using {@code put},
 * {@code putIfAbsent}, or {@code replace}, depending on exactly which effect
 * you need.)
 *
 * <p>
 ...
 * modified during traversal. Additionally, the bulk operations {@code putAll},
 * {@code equals}, {@code toArray}, {@code containsValue}, and {@code clear} are
 * <em>not</em> guaranteed to be performed atomically. For example, an iterator
 * operating concurrently with a {@code putAll} operation might view only some
 ...
 * {@code null} keys or values because some null return values cannot be
...
public class ConcurrentSkipListMap<K, V> extends AbstractMap<K, V> implements
     ...
     * require a separate traversal pass occurring after the base-level action,
     ...
     * p=0.5 (see method doPut) mean that about one-quarter of the nodes have
     * indices. Of those that do, half have one level, a quarter have two, and
     * so on (see Pugh's Skip List Cookbook, sec 3.4). The expected total space
     ...
     * To produce random values without interference across threads, we use
     * within-JDK thread local random support (via the "secondary seed", to
     * avoid interference with user-level ThreadLocalRandom.)
     * A previous version of this class wrapped non-comparable keys with their
     * comparators to emulate Comparables when using comparators vs Comparables.
     * However, JVMs now appear to better handle infusing
     * comparator-vs-comparable choice into search loops. Static method
     * cpr(comparator, x, y) is used for all comparisons, which works well as
     * long as the comparator argument is set up outside of loops (thus
     * sometimes passed as an argument to internal methods) to avoid field
     * re-reads.
    ...
    static final class Node<K, V> {
        ...
        static {
            try {
                ...
                Class<?> k = Node.class;
                valueOffset = UNSAFE.objectFieldOffset(k.getDeclaredField(
                        "value"));
    ...
    static class Index<K, V> {
        ...
        static {
            try {
                UNSAFE = sun.misc.Unsafe.getUnsafe();
                Class<?> k = Index.class;
                rightOffset = UNSAFE.objectFieldOffset(k.getDeclaredField(
                        "right"));
            } catch (Exception e) {
                throw new Error(e);
            }
    ...
    /**
     * Gets value for key. Almost the same as findNode, but returns the found
     * value (to avoid retries during re-reads)
     *
     * @param key
     *            the key
     ...
     *                              if the keys in {@code m} are not
     *                              {@link Comparable}, or are
     *                              not mutually comparable
     * @throws NullPointerException
     *                              if the specified map or any of its keys or
     *                              values are null
     */
     ...
     * @throws NullPointerException
     *                              if the specified sorted map or any of its
     *                              keys or values are
     *                              null
     */
     ...
     *             mapping represented by the map, followed by {@code null}. The
     ...
     * contains more than {@code Integer.MAX_VALUE} elements, it returns
    ...
    public V computeIfAbsent(K key,
        ...
        if (key == null || mappingFunction == null)
            throw new NullPointerException();
        V v, p, r;
        if ((v = doGet(key)) == null && (r = mappingFunction.apply(
                key)) != null)
     ...
     * is <em>NOT</em> guaranteed to be applied once atomically.
     *
     * @param key
     *                          key with which the specified value is to be
     *                          associated
     ...
     * @return the new value associated with the specified key, or null if none
     * @throws NullPointerException
     *                              if the specified key or value is null or the
     *                              remappingFunction is null
     * @since 1.8
     */
     ...
     *
     * <p>
     * The set's iterator returns the keys in ascending order. The set's
     ...
     * map's comparator (see {@link #comparator()}) is {@code null}. Otherwise,
     * the spliterator's comparator is the same as or imposes the same total
     * ordering as the map's comparator.
     *
     * <p>
    ...
    public ConcurrentNavigableMap<K, V> descendingMap() {
        ...
        return (dm != null) ? dm
                : (descendingMap = new SubMap<K, V>(this, null, false, null,
                        false, true));
    }
    ...
    static final class KeySet<E> extends AbstractSet<E> implements
        ...
        public Iterator<E> iterator() {
            if (m instanceof ConcurrentSkipListMap)
                ...
                return ((ConcurrentSkipListMap.SubMap<E, Object>) m)
                        .keyIterator();
        }
        ...
        public NavigableSet<E> subSet(E fromElement, boolean fromInclusive,
            ...
            return new KeySet<E>(m.subMap(fromElement, fromInclusive, toElement,
                    toInclusive));
        }
    ...
    static final class EntrySet<K1, V1> extends AbstractSet<Map.Entry<K1, V1>> {
        ...
        public Spliterator<Map.Entry<K1, V1>> spliterator() {
            if (m instanceof ConcurrentSkipListMap)
                ...
                return (Spliterator<Map.Entry<K1, V1>>) ((SubMap<K1, V1>) m)
                        .entryIterator();
        }
    }
     ...
     * Instances of this class are constructed only using the {@code subMap},
     * {@code headMap}, and {@code tailMap} methods of their underlying maps.
     *
     * @serial include
     */
    static final class SubMap<K, V> extends AbstractMap<K, V> implements
         ...
         */
        SubMap(ConcurrentSkipListMap<K, V> map, K fromKey,
                boolean fromInclusive, K toKey, boolean toInclusive,
                boolean isDescending) {
            Comparator<? super K> cmp = map.comparator;
            if (fromKey != null && toKey != null && cpr(cmp, fromKey,
                    toKey) > 0)
        ...
        boolean tooLow(Object key, Comparator<? super K> cmp) {
            int c;
            return (lo != null && ((c = cpr(cmp, key, lo)) < 0 || (c == 0
                    && !loInclusive)));
        }

        boolean tooHigh(Object key, Comparator<? super K> cmp) {
            int c;
            return (hi != null && ((c = cpr(cmp, key, hi)) > 0 || (c == 0
                    && !hiInclusive)));
        }
        ...
        public int size() {
            Comparator<? super K> cmp = m.comparator;
            long count = 0;
            for (ConcurrentSkipListMap.Node<K, V> n = loNode(cmp); isBeforeEnd(
                    n, cmp); n = n.next) {
                if (n.getValidValue() != null)
                    ++count;
            }
         ...
         */
        SubMap<K, V> newSubMap(K fromKey, boolean fromInclusive, K toKey,
                boolean toInclusive) {
            Comparator<? super K> cmp = m.comparator;
        ...
        public Set<Map.Entry<K, V>> entrySet() {
            ...
            return (es != null) ? es
                    : (entrySetView = new EntrySet<K, V>(this));
        }
        ...
        /**
         * Variant of main Iter class to traverse through submaps. Also serves
         * as back-up Spliterator for views
         */
     ...
     * advancing either across or down decreases by about 25%. To make this
     * observation useful, we need to know initial size, which we don't. But we
     * can just use Integer.MAX_VALUE so that we don't prematurely zero out
     * while splitting.
     */
    ...
    final KeySpliterator<K, V> keySpliterator() {
        ...
        for (;;) { // ensure h corresponds to origin p
            ...
            if ((p = b.next) == null || p.value != null)
                return new KeySpliterator<K, V>(cmp, h, p, null, (p == null) ? 0
                        : Integer.MAX_VALUE);
    ...
    static final class EntrySpliterator<K, V> extends CSLMSpliterator<K, V>
        ...
        public final Comparator<Map.Entry<K, V>> getComparator() {
            ...
            if (comparator != null) {
                ...
                return (Comparator<Map.Entry<K, V>> & Serializable) (e1,
                        e2) -> {
                    @SuppressWarnings("unchecked")
