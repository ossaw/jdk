...
/**
 * A scalable concurrent {@link NavigableSet} implementation based on a
 * {@link ConcurrentSkipListMap}. The elements of the set are kept sorted
 * according to their {@linkplain Comparable natural ordering}, or by a
 * {@link Comparator} provided at set creation time, depending on which
 * constructor is used.
 *
 * <p>
 * This implementation provides expected average <i>log(n)</i> time cost for the
 * {@code contains}, {@code add}, and {@code remove} operations and their
 ...
 * these sets, determining the current number of elements requires a traversal
 ...
 * of the {@link Set} and {@link Iterator} interfaces. Like most other
 * concurrent collection implementations, this class does not permit the use of
 * {@code null} elements, because {@code null} arguments and return values
 ...
 * @since 1.6
 */
public class ConcurrentSkipListSet<E> extends AbstractSet<E> implements
        NavigableSet<E>, Cloneable, java.io.Serializable {

    private static final long serialVersionUID = -2479143111061671589L;

    /**
     * The underlying map. Uses Boolean.TRUE as value for each element. This
     * field is declared final for the sake of thread safety, which entails some
     * ugliness in clone().
     */
    ...
    /**
     * Constructs a new set containing the same elements and using the same
     * ordering as the specified sorted set.
     *
     * @param s
     *          sorted set whose elements will comprise the new set
     * @throws NullPointerException
     *                              if the specified sorted set or any of its
     *                              elements are null
     */
     ...
     * sets, determining the current number of elements requires traversing them
     ...
     * contains no element {@code e2} such that {@code e.equals(e2)}. If this
     ...
     *         element
     * @throws ClassCastException
     *                              if {@code e} cannot be compared with the
     *                              elements currently
     *                              in this set
     * @throws NullPointerException
     *                              if the specified element is null
     ...
     * formally, removes an element {@code e} such that {@code o.equals(e)}, if
     ...
     * @throws ClassCastException
     *                              if {@code o} cannot be compared with the
     *                              elements currently
     *                              in this set
     * @throws NullPointerException
     *                              if the specified element is null
     ...
     * {@code (comparator())}. The expression
     ...
     * set's comparator (see {@link #comparator()}) is {@code null}. Otherwise,
     * the spliterator's comparator is the same as or imposes the same total
     * ordering as the set's comparator.
     *
    ...
    public Spliterator<E> spliterator() {
        if (m instanceof ConcurrentSkipListMap)
            ...
            return (Spliterator<E>) ((ConcurrentSkipListMap.SubMap<E, ?>) m)
                    .keyIterator();
    }
