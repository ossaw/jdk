...
/**
 * A version of {@link AbstractQueuedSynchronizer} in which synchronization
 * state is maintained as a {@code long}. This class has exactly the same
 * structure, properties, and methods as {@code AbstractQueuedSynchronizer} with
 * the exception that all state-related parameters and results are defined as
 * {@code long} rather than {@code int}. This class may be useful when creating
 * synchronizers such as multilevel locks and barriers that require 64 bits of
 * state.
 * <p>
 ...
 * @author Doug Lea
 */
public abstract class AbstractQueuedLongSynchronizer extends AbstractOwnableSynchronizer implements
        java.io.Serializable {

    private static final long serialVersionUID = 7373984972572414692L;

    /*
     * To keep sources in sync, the remainder of this source file is exactly
     * cloned from AbstractQueuedSynchronizer, replacing class name and changing
     * ints related with sync state to longs. Please keep it that way.
     */
    ...
    private void cancelAcquire(Node node) {
        ...
        if (node == tail && compareAndSetTail(node, pred)) {
            ...
            int ws;
            if (pred != head && ((ws = pred.waitStatus) == Node.SIGNAL || (ws <= 0 && compareAndSetWaitStatus(
     ...
     * <a href="AbstractQueuedSynchronizer.html#barging">barging</a>. Such a
    ...
    public class ConditionObject implements Condition, java.io.Serializable {
         ...
         * {@link AbstractQueuedLongSynchronizer#hasWaiters(ConditionObject)}.
         *
         * @return {@code true} if there are any waiting threads
         * @throws IllegalMonitorStateException
         *         if {@link #isHeldExclusively}
         ...
         * {@link AbstractQueuedLongSynchronizer#getWaitQueueLength(ConditionObject)}
         * .
         *
         * @return the estimated number of waiting threads
         * @throws IllegalMonitorStateException
         ...
         * {@link AbstractQueuedLongSynchronizer#getWaitingThreads(ConditionObject)}
         * .
         *
         * @return the collection of threads
         * @throws IllegalMonitorStateException
     ...
     * subclass AtomicLong, which would be efficient and useful otherwise. So,
     * as the lesser of evils, we natively implement using hotspot intrinsics
     * API. And while we are at it, we do the same for other CASable fields
    ...
    static {
        try {
            stateOffset = unsafe.objectFieldOffset(AbstractQueuedLongSynchronizer.class.getDeclaredField(
                    "state"));
            headOffset = unsafe.objectFieldOffset(AbstractQueuedLongSynchronizer.class.getDeclaredField(
                    "head"));
            tailOffset = unsafe.objectFieldOffset(AbstractQueuedLongSynchronizer.class.getDeclaredField(
                    "tail"));
