 ...
 * {@code park} will return immediately if the permit is available, consuming it
 ...
 * deprecated methods {@code Thread.suspend} and {@code Thread.resume} to be
 * unusable for such purposes: Races between one thread invoking {@code park}
 * and another thread trying to {@code unpark} it will preserve liveness, due to
 * the permit. Additionally, {@code park} will return if the caller's thread was
 * interrupted, and timeout versions are supported. The {@code park} method may
 * also return at any other time, for "no reason", so in general must be invoked
 * within a loop that rechecks conditions upon return. In this sense
 ...
 * monitoring and diagnostic tools to identify the reasons that threads are
 * blocked. (Such tools may access blockers using method
 ...
 * concurrency control applications. The {@code park} method is designed for use
 * only in constructions of the form:
 *
 * <pre>
 *  {@code
 ...
 * </pre>
 *
 * where neither {@code canProceed} nor any other actions prior to the call to
...
public class LockSupport {
	 ...
	 * available. If the thread was blocked on {@code park} then it will
	 * unblock. Otherwise, its next call to {@code park} is guaranteed not to
	 * block. This operation is not guaranteed to have any effect at all if the
	 ...
	 *            the synchronization object responsible for this thread parking
	 * @param deadline
	 *            the absolute time, in milliseconds from the Epoch, to wait
	 *            until
	 * @since 1.6
	 */
	...
	/**
	 * Returns the pseudo-randomly initialized or updated secondary seed. Copied
	 * from ThreadLocalRandom due to package access restrictions.
	 */
