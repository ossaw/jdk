...
public class ReentrantReadWriteLock implements ReadWriteLock,
	...
	abstract static class Sync extends AbstractQueuedSynchronizer {
		 ...
		 * Initialized only in constructor and readObject. Removed whenever a
		 * thread's read hold count drops to 0.
		 */
		private transient ThreadLocalHoldCounter readHolds;

		/**
		 * The hold count of the last thread to successfully acquire readLock.
		 * This saves ThreadLocal lookup in the common case where the next
		 * thread to release is the last one to acquire. This is non-volatile
		 * since it is just used as a heuristic, and would be great for threads
		 * to cache.
		 *
		 * <p>
		 * Can outlive the Thread for which it is caching the read hold count,
		 * but avoids garbage retention by not retaining a reference to the
		 * Thread.
		 *
		 * <p>
		 * Accessed via a benign data race; relies on the memory model's final
		 ...
		 *
		 * <p>
		 * More precisely, firstReader is the unique thread that last changed
		 * the shared count from 0 to 1, and has not released the read lock
		 * since then; null if there is no such thread.
		 *
		 * <p>
		 ...
		 *
		 * <p>
		 * Accessed via a benign data race; relies on the memory model's
		 * out-of-thin-air guarantees for references.
		 *
		 * <p>
		 * This allows tracking of read holds for uncontended read locks to be
		 * very cheap.
		 */
		...
		protected final int tryAcquireShared(int unused) {
			...
			if (exclusiveCount(c) != 0 && getExclusiveOwnerThread() != current)
				return -1;
			int r = sharedCount(c);
			if (!readerShouldBlock() && r < MAX_COUNT && compareAndSetState(c, c
					+ SHARED_UNIT)) {
				if (r == 0) {
					firstReader = current;
	...
	static final class NonfairSync extends Sync {
		...
		final boolean readerShouldBlock() {
			/*
			 * As a heuristic to avoid indefinite writer starvation, block if
			 * the thread that momentarily appears to be head of queue, if one
			 * exists, is a waiting writer. This is only a probabilistic effect
			 * since a new reader will not block if there is a waiting writer
			 * behind other enabled readers that have not yet drained from the
			 * queue.
			 */
	...
	public static class WriteLock implements Lock, java.io.Serializable {
		 ...
		 * been set to use a fair ordering policy, a call to {@code tryLock()}
		 * <em>will</em> immediately acquire the lock if it is available,
		 * whether or not other threads are currently waiting for the write
		 ...
		 * Acquires the write lock if neither the read nor write lock are held
		 * by another thread and returns immediately with the value {@code true}
		 * , setting the write lock hold count to one. If this lock has been set
		 * to use a fair ordering policy then an available lock
