...
/**
 * Utility classes commonly useful in concurrent programming. This package
 * includes a few small standardized extensible frameworks, as well as some
 ...
 * class is being used, tasks may execute in a newly created thread, an existing
 ...
 * include the concrete class {@link java.util.concurrent.FutureTask} providing
 ...
 * {@link java.util.concurrent.ForkJoinTask} and its subclasses. These classes
 ...
 * versions of put and take: {@link java.util.concurrent.LinkedBlockingQueue},
 ...
 * time-outs in addition to indefinite waits. In all cases that time-outs are
 * used, the time-out specifies the minimum time that the method should wait
 * before indicating that it timed-out. Implementations make a &quot;best
 * effort&quot; to detect time-outs as soon as possible after they occur.
 * However, an indefinite amount of time may elapse between a time-out being
 * detected and a thread actually executing again after that time-out. All
 ...
 * {@link java.util.concurrent.CopyOnWriteArraySet}. When many threads are
 * expected to access a given collection, a {@code ConcurrentHashMap} is
 * normally preferable to a synchronized {@code HashMap}, and a
 * {@code ConcurrentSkipListMap} is normally preferable to a synchronized
 ...
 * example {@code java.util.Hashtable} and
 * {@code Collections.synchronizedMap(new HashMap())} are synchronized. But
 * {@link java.util.concurrent.ConcurrentHashMap} is "concurrent". A concurrent
 * collection is thread-safe, but not governed by a single exclusion lock. In
 * the particular case of ConcurrentHashMap, it safely permits any number of
 * concurrent reads as well as a tunable number of concurrent writes.
 * "Synchronized" classes can be useful when you need to prevent all access to a
 * collection via a single lock, at the expense of poorer scalability. In other
 * cases in which multiple threads are expected to access a common collection,
 ...
 * {@linkplain java.util.Iterator Iterators} and
 * {@linkplain java.util.Spliterator Spliterators} provide
 ...
 * <i>happens-before</i> relation on memory operations such as reads and writes
 * of shared variables. The results of a write by one thread are guaranteed to
 * be visible to a read by another thread only if the write operation
 * <i>happens-before</i> the read operation. The {@code synchronized} and
 ...
 * successful "acquiring" method such as {@code Lock.lock},
