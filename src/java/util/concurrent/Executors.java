...
public class Executors {
     ...
     * equivalent {@code newScheduledThreadPool(1)} the returned executor is
     * guaranteed not to be reconfigurable to use additional threads.
     * 
     ...
     * tasks.) Tasks are guaranteed to execute sequentially, and no more than
     * one task will be active at any given time. Unlike the otherwise
     * equivalent {@code newScheduledThreadPool(1, threadFactory)} the returned
     ...
     * the thread invoking this {@code defaultThreadFactory} method. Each new
     * thread is created as a non-daemon thread with priority set to the smaller
     * of {@code Thread.NORM_PRIORITY} and the maximum priority permitted in the
     * thread group. New threads have names accessible via
     * {@link Thread#getName} of <em>pool-N-thread-M</em>, where <em>N</em> is
     * the sequence number of this factory, and <em>M</em> is the sequence
     ...
     * same settings as {@link Executors#defaultThreadFactory}, additionally
     * setting the AccessControlContext and contextClassLoader of new threads to
     * be the same as the thread invoking this {@code privilegedThreadFactory}
     * method. A new {@code privilegedThreadFactory} can be created within an
     * {@link AccessController#doPrivileged AccessController.doPrivileged}
     * action setting the current thread's access control context to create
     ...
     * not have the same {@link java.lang.ThreadLocal} or
     ...
     * {@link ThreadPoolExecutor#beforeExecute(Thread, Runnable)}. Also, if it
     * is necessary to initialize worker threads to have the same
     ...
     * method should normally be invoked within an
     * {@link AccessController#doPrivileged AccessController.doPrivileged}
     * action to create callables that will, if possible, execute under the
     * selected permission settings holding within that action; or if not
    ...
    /**
     * Returns a {@link Callable} object that will, when called, execute the
     * given {@code callable} under the current access control context, with the
     * current context class loader as the context class loader. This method
     * should normally be invoked within an {@link AccessController#doPrivileged
     * AccessController.doPrivileged} action to create callables that will, if
     * possible, execute under the selected permission settings holding within
     * that action; or if not possible, throw an associated
     ...
     */
    static class DelegatedScheduledExecutorService extends DelegatedExecutorService implements
