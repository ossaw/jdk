 ...
 * {@code ConcurrentLinkedQueue} is an appropriate choice when many threads will
 * share access to a common collection. Like most other concurrent collection
 ...
 * <a href="http://www.cs.rochester.edu/u/michael/PODC96.html"> Simple, Fast,
 * and Practical Non-Blocking and Blocking Concurrent Queue Algorithms</a> by
 * Maged M. Michael and Michael L. Scott.
 * <p>
 * Iterators are <i>weakly consistent</i>, returning elements reflecting the
 * state of the queue at some point at or since the creation of the iterator.
 ...
 * the {@link Queue} and {@link Iterator} interfaces.
 * <p>
 * Memory consistency effects: As with other concurrent collections, actions in
 * a thread prior to placing an object into a {@code ConcurrentLinkedQueue}
...
public class ConcurrentLinkedQueue<E> extends AbstractQueue<E> implements Queue<E>, java.io.Serializable {
     ...
     * it can always be reached in O(N) time from head as well. - The elements
     * contained in the queue are the non-null items in Nodes that are reachable
     * from head. CASing the item reference of a Node to null atomically removes
     * it from the queue. Reachability of all elements from head must remain
     ...
     * tail are only updated using CAS, so they never regress, although again
    ...
    /**
     * A node from which the first live (non-deleted) node (if any) can be
     * reached in O(1) time. Invariants: - all live nodes are reachable from
     * head via succ() - head != null - (tmp = head).next != tmp || tmp != head
     * Non-invariants: - head.item may or may not be null. - it is permitted for
     * tail to lag behind head, that is, for tail to not be reachable from head!
     */
    private transient volatile Node<E> head;

    /**
     * A node from which the last node on list (that is, the unique node with
     * node.next == null) can be reached in O(1) time. Invariants: - the last
     * node is always reachable from tail via succ() - tail != null
     * Non-invariants: - tail.item may or may not be null. - it is permitted for
     * tail to lag behind head, that is, for tail to not be reachable from head!
    ...
    /**
     * Creates a {@code ConcurrentLinkedQueue} initially containing the elements
    ...
    /**
     * Tries to CAS head to p. If successful, repoint old head to itself as
     * sentinel for succ(), below.
     */
    ...
    /**
     * Returns the first live (non-deleted) node on list, or null if none. This
     * is yet another variant of poll/peek; here returning the first node, not
     * element. We could make peek() a wrapper around first(), but that would
     * cost an extra volatile read of item, and the need to add a retry loop to
     * deal with the possibility of losing a race to a concurrent poll().
     */
    Node<E> first() {
        restartFromHead:
        for (;;) {
     ...
     * traversal. Additionally, if elements are added or removed during
     * execution of this method, the returned result may be inaccurate. Thus,
     ...
     * collection's iterator. Attempts to {@code addAll} of a queue to itself
