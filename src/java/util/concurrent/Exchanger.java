 ...
 * pairs. Each thread presents some object on entry to the {@link #exchange
 ...
 * {@code exchange()} in each thread
 * <a href="package-summary.html#MemoryVisibility"><i>happen-before</i></a>
 * those subsequent to a return from the corresponding {@code exchange()} in the
 * other thread.
 *
 * @since 1.5
...
public class Exchanger<V> {
     ...
     * implementation instead uses a form of elimination arena, that spreads out
     * this contention by arranging that some threads typically use different
     * slots, while still ensuring that eventually, any two parties will be able
     * to exchange items. That is, we cannot completely partition across
     * threads, but instead give threads arena indices that will on average grow
     * under contention and shrink under lack of contention. We approach this by
     * defining the Nodes that we need anyway as ThreadLocals, and include in
     * them per-thread index and related bookkeeping state. (We can safely reuse
     ...
     * single-slot slotExchange method. On contention, not only must the slots
     * be in different locations, but the locations must not encounter memory
     * contention due to being on the same cache line (or more generally, the
     * same coherence unit). Because, as of this writing, there is no way to
     * determine cacheline size, we define a value that is enough for common
     * platforms. Additionally, extra care elsewhere is taken to avoid other
     * false/unintended sharing and to enhance locality, including adding
     ...
     * that reliably indicate contention are when two attempted releases collide
     * -- one of two attempted offers can legitimately fail to CAS without
     * indicating contention by more than one other thread. (Note: it is
     * possible but not worthwhile to more precisely detect contention by
     * reading slot values after CAS failures.) When a thread has collided at
     * each slot within the current arena bound, it tries to expand the arena
     * size by one. We track collisions within bounds by using a version
     ...
     * implement by piggybacking on the use of spin->yield->block that is
     * essential for reasonable waiting performance anyway -- in a busy
     * exchanger, offers are usually almost immediately released, in which case
     * context switching on multiprocessors is extremely slow/wasteful. Arena
     * waits just omit the blocking part, and instead cancel. The spin count is
     * empirically chosen to be a value that avoids blocking 99% of the time
     * under maximum sustained exchange rates on a range of test machines. Spins
     * and yields entail some limited randomness (using a cheap xorshift) to
     * avoid regular patterns that can induce unproductive grow/shrink cycles.
     * (Using a pseudorandom also helps regularize spin cycle duration by making
     * branches unpredictable.) Also, during an offer, a waiter can "know" that
     * it will be released when its slot has changed, but cannot yet proceed
     * until match is set. In the mean time it cannot cancel the offer, so
     * instead spins/yields. Note: It is possible to avoid this secondary check
     * by changing the linearization point to be a CAS of the match field (as
     * done in one case in the Scott & Scherer DISC paper), which also increases
     ...
     * 0.) It is possible (and has been tried) to use randomized,
     * prime-value-stepped, or double-hash style traversal instead of simple
     * cyclic traversal to reduce bunching. But empirically, whatever benefits
     * these may have don't overcome their added overhead: We are managing
     ...
     * Exchanger field "slot" rather than arena array elements. However, it
     * still needs minimal collision detection to trigger arena construction.
     ...
     * spin->yield->block/cancel code), and heavily dependent on intrinsics
     * (Unsafe) to use inlined embedded CAS and related memory access operations
     * (that tend not to be as readily inlined by dynamic compilers when they
     * are hidden behind other methods that would more nicely name and
     * encapsulate the intended effects). This includes the use of putOrderedX
     * to clear fields of the per-thread Nodes between uses. Note that field
     * Node.item is not declared as volatile even though it is read by releasing
     * threads, because they only do so after CAS operations that must precede
     * access, and all uses by the owning thread are otherwise acceptably
     * ordered by other operations. (Because the actual points of atomicity are
     * slot CASes, it would also be legal for the write to Node.match in a
     * release to be weaker than a full volatile write. However, this is not
     * done because it could allow further postponement of the write, delaying
     * progress.)
     */

    /**
     * The byte distance (as a shift value) between any two used slots in the
     * arena. 1 << ASHIFT should be at least cacheline size.
     */
    private static final int ASHIFT = 7;

    /**
     * The maximum supported arena index. The maximum allocatable arena size is
     * MMASK + 1. Must be a power of two minus one, less than (1<<(31-ASHIFT)).
     * The cap of 255 (0xff) more than suffices for the expected scaling limits
     * of the main algorithms.
     */
    private static final int MMASK = 0xff;

    /**
     * Unit for sequence/version bits of bound field. Each successful change to
     * the bound also adds SEQ.
     */
    ...
    /**
     * The maximum slot index of the arena: The number of slots that can in
     * principle hold all threads without contention, or at most the maximum
     * indexable value.
     */
    static final int FULL = (NCPU >= (MMASK << 1)) ? MMASK : NCPU >>> 1;

    /**
     * The bound for spins while waiting for a match. The actual number of
     * iterations will on average be about twice this value due to
     * randomization. Note: Spinning is disabled when NCPU==1.
     */
    private static final int SPINS = 1 << 10;

    /**
     * Value representing null arguments/returns from public methods. Needed
     * because the API originally didn't disallow null arguments, which it
     * should have.
     */
    private static final Object NULL_ITEM = new Object();

    /**
     * Sentinel value returned by internal exchange methods upon timeout, to
     * avoid need for separate timed versions of these methods.
     */
    ...
    /**
     * Elimination array; null until enabled (within slotExchange). Element
     * accesses use emulation of volatile gets and CAS.
     */
    ...
    /**
     * The index of the largest valid arena position, OR'ed with SEQ number in
     * high bits, incremented on each update. The initial update from 0 to SEQ
     * is used to ensure that the arena array is constructed only once.
     */
    ...
    private final Object slotExchange(Object item, boolean timed, long ns) {
        ...
        if (t.isInterrupted()) // preserve interrupt status so caller can
                               // recheck
            return null;
     ...
     * <li>is {@linkplain Thread#interrupt interrupted} while waiting for the
     * exchange,
     * </ul>
