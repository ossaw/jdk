 ...
 * that no synchronization lock is held when {@link #acquire} is called as that
 * would prevent an item from being returned to the pool. The semaphore
 ...
 * commonly known as a <em>binary
 * semaphore</em>, because it only has two states: one permit available, or zero
 * permits available. When used in this way, the binary semaphore has the
 * property (unlike many {@link java.util.concurrent.locks.Lock}
 * implementations), that the &quot;lock&quot; can be released by a thread other
 * than the owner (as semaphores have no notion of ownership). This can be
 ...
 * parameter. When set false, this class makes no guarantees about the order in
 * which threads acquire permits. In particular, <em>barging</em> is permitted,
 * that is, a thread invoking {@link #acquire} can be allocated a permit ahead
 * of a thread that has been waiting - logically the new thread places itself at
 * the head of the queue of waiting threads. When fairness is set true, the
 * semaphore guarantees that threads invoking any of the {@link #acquire()
 * acquire} methods are selected to obtain permits in the order in which their
 * invocation of those methods was processed (first-in-first-out; FIFO). Note
 * that FIFO ordering necessarily applies to specific internal points of
 ...
 * and similarly upon return from the method. Also note that the untimed
 * {@link #tryAcquire() tryAcquire} methods do not honor the fairness setting,
 ...
 *
 * <p>
 * This class also provides convenience methods to {@link #acquire(int) acquire}
 * and {@link #release(int) release} multiple permits at a time. Beware of the
 * increased risk of indefinite postponement when these methods are used without
 * fairness set true.
 *
 * <p>
 * Memory consistency effects: Actions in a thread prior to calling a "release"
 * method such as {@code release()}
 * <a href="package-summary.html#MemoryVisibility"><i>happen-before</i></a>
 * actions following a successful "acquire" method such as {@code acquire()} in
 * another thread.
 *
 * @since 1.5
 * @author Doug Lea
...
public class Semaphore implements java.io.Serializable {
	...
	/**
	 * Creates a {@code Semaphore} with the given number of permits and the
	 ...
	 * invokes the {@link #release} method for this semaphore and the current
	 * thread is next to be assigned a permit.
	 *
	 * <p>
	 * If the current thread is {@linkplain Thread#interrupt interrupted} while
	 * waiting for a permit then it will continue to wait, but the time at which
	 * the thread is assigned a permit may change compared to the time it would
	 * have received the permit had no interruption occurred. When the thread
	 ...
	 * value {@code true}, reducing the number of available permits by one.
	 *
	 * <p>
	 * If no permit is available then this method will return immediately with
	 * the value {@code false}.
	 *
	 * <p>
	 * Even when this semaphore has been set to use a fair ordering policy, a
	 * call to {@code tryAcquire()} <em>will</em> immediately acquire a permit
	 * if one is available, whether or not other threads are currently waiting.
	 ...
	 * setting, then use {@link #tryAcquire(long, TimeUnit) tryAcquire(0,
	 ...
	 * <ul>
	 * <li>has its interrupted status set on entry to this method; or
	 * <li>is {@linkplain Thread#interrupt interrupted} while waiting to acquire
	 * a permit,
	 * </ul>
	 * then {@link InterruptedException} is thrown and the current thread's
	 * interrupted status is cleared.
	 ...
	 *
	 * <p>
	 * Releases a permit, increasing the number of available permits by one. If
	 * any threads are trying to acquire a permit, then one is selected and
	 ...
	 * </ul>
	 * then {@link InterruptedException} is thrown and the current thread's
	 * interrupted status is cleared. Any permits that were to be assigned to
	 * this thread are instead assigned to other threads trying to acquire
	 * permits, as if permits had been made available by a call to
	 * {@link #release()}.
	 *
	 * @param permits
	 ...
	 * semaphore, the current thread is next to be assigned permits and the
	 * number of available permits satisfies this request.
	 *
	 * <p>
	 * If the current thread is {@linkplain Thread#interrupt interrupted} while
	 * waiting for permits then it will continue to wait and its position in the
	 * queue is not affected. When the thread does return from this method its
	 ...
	 * <p>
	 * Acquires the given number of permits, if they are available, and returns
	 * immediately, with the value {@code true}, reducing the number of
	 ...
	 * <p>
	 * Even when this semaphore has been set to use a fair ordering policy, a
	 * call to {@code tryAcquire} <em>will</em> immediately acquire a permit if
	 * one is available, whether or not other threads are currently waiting.
	 * This &quot;barging&quot; behavior can be useful in certain circumstances,
	 * even though it breaks fairness. If you want to honor the fairness
	 * setting, then use {@link #tryAcquire(int, long, TimeUnit)
	 ...
	 * If the specified waiting time elapses then the value {@code false} is
	 * returned. If the time is less than or equal to zero, the method will not
	 * wait at all. Any permits that were to be assigned to this thread, are
	 * instead assigned to other threads trying to acquire permits, as if the
	 ...
	 * permits by that amount. If any threads are trying to acquire permits,
	 * then one is selected and given the permits that were just released. If
	 * the number of available permits satisfies that thread's request then that
	 * thread is (re)enabled for thread scheduling purposes; otherwise the
	 * thread will wait until sufficient permits are available. If there are
	 * still permits available after this thread's request has been satisfied,
	 * then those permits are assigned in turn to other threads trying to
	 * acquire permits.
	 *
	 * <p>
	 * There is no requirement that a thread that releases a permit must have
	 * acquired that permit by calling {@link Semaphore#acquire acquire}.
	 * Correct usage of a semaphore is established by programming convention in
	 * the application.
	 *
	 * @param permits
	...
	/**
	 * Shrinks the number of available permits by the indicated reduction. This
	 * method can be useful in subclasses that use semaphores to track resources
	 * that become unavailable. This method differs from {@code acquire} in that
	 ...
	 * cancellations may occur at any time, a {@code true} return does not
	 * guarantee that any other thread will ever acquire. This method is
	...
	/**
	 * Returns a string identifying this semaphore, as well as its state. The
	 * state, in brackets, includes the String {@code "Permits ="} followed by
