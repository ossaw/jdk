 ...
 * values). In other words, in an <tt>IdentityHashMap</tt>, two keys <tt>k1</tt>
 * and <tt>k2</tt> are considered equal if and only if <tt>(k1==k2)</tt>. (In
 ...
 * Increasing the number of buckets ("rehashing") may be fairly expensive, so it
 * pays to create identity hash maps with a sufficiently large expected maximum
 * size. On the other hand, iteration over collection views requires time
 ...
 * holding keys and values. (This has better locality for large tables than does
...
public class IdentityHashMap<K, V> extends AbstractMap<K, V> implements Map<K, V>, java.io.Serializable,
     ...
     * <tt>IdentityHashMap</tt> instance and the other is a normal map.</b>
     *
    ...
    private class EntryIterator extends IdentityHashMapIterator<Map.Entry<K, V>> {
        ...
        private class Entry implements Map.Entry<K, V> {
            ...
            public boolean equals(Object o) {
                ...
                return (e.getKey() == unmaskNull(traversalTable[index]) && e
                        .getValue() == traversalTable[index + 1]);
            }
     ...
     * its backing map, the set returned by this method defines element equality
     ...
     * references as the returned set. The symmetry and transitivity
     ...
     * of their hashcodes. This is mandated by the change in the semantics of
     * the <tt>equals</tt> method, in order to enforce the general contract of
     * the <tt>Object.hashCode</tt> method among sets returned by this method.
     *
     ...
     * <tt>Collection's</tt> general contract. Like its backing map, the
     * collection returned by this method defines element equality as
     * reference-equality rather than object-equality. This affects the behavior
     * of its <tt>contains</tt>, <tt>remove</tt> and <tt>containsAll</tt>
     * methods.</b>
     */
    ...
    /**
     * Returns a {@link Set} view of the mappings contained in this map. Each
     * element in the returned set is a reference-equality-based
     * <tt>Map.Entry</tt>. The set is backed by the map, so changes to the map
    ...
    /**
     * Similar form as array-based Spliterators, but skips blank elements, and
     * guestimates size as decreasing by half per split.
     */
