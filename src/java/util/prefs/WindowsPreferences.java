...
class WindowsPreferences extends AbstractPreferences {
    ...
    /**
     * Maximum byte-encoded path length for Windows native functions, ending
     * <tt>null</tt> character not included.
     */
     ...
     * System root node.
     */
    static final Preferences systemRoot = new WindowsPreferences(SYSTEM_ROOT_NATIVE_HANDLE,
            WINDOWS_ROOT_PATH);
     ...
     */
    private static native int[] WindowsRegOpenKey(int hKey, byte[] subKey, int securityMask);
    ...
    private static int[] WindowsRegOpenKey1(int hKey, byte[] subKey, int securityMask) {
        ...
        if (result[ERROR_CODE] == ERROR_SUCCESS) {
            ...
            logger().warning("Trying to recreate Windows registry node " + byteArrayToString(subKey)
                    + " at root 0x" + Integer.toHexString(hKey) + ".");
            // Try recreation
     ...
     */
    private static native int WindowsRegSetValueEx(int hKey, byte[] valueName, byte[] value);
     ...
     */
    private static native byte[] WindowsRegEnumValue(int hKey, int valueIndex, int maxValueNameLength);
     ...
     * created. Logs a warning message, if Windows Registry is unavailable.
     */
    private WindowsPreferences(WindowsPreferences parent, String name) {
        ...
        if (result[ERROR_CODE] != ERROR_SUCCESS) {
            logger().warning("Could not create windows registry node " + byteArrayToString(
                    windowsAbsolutePath()) + " at root 0x" + Integer.toHexString(rootNativeHandle())
                    + ". Windows RegCreateKeyEx(...) returned error code " + result[ERROR_CODE] + ".");
            isBackingStoreAvailable = false;
            return;
        }
    ...
    /**
     * Constructs a root node creating the underlying Windows registry node and
     * all of its parents, if they have not yet been created. Logs a warning
     * message, if Windows Registry is unavailable.
     * 
     * @param rootNativeHandle
     *        Native handle to one of Windows top level keys.
     * @param rootDirectory
     *        Path to root directory, as a byte-encoded string.
     */
    private WindowsPreferences(int rootNativeHandle, byte[] rootDirectory) {
        ...
        if (result[ERROR_CODE] != ERROR_SUCCESS) {
            logger().warning("Could not open/create prefs root node " + byteArrayToString(
                    windowsAbsolutePath()) + " at root 0x" + Integer.toHexString(rootNativeHandle())
                    + ". Windows RegCreateKeyEx(...) returned error code " + result[ERROR_CODE] + ".");
            isBackingStoreAvailable = false;
            return;
    ...
    /**
     * Returns Windows absolute path of the current node as a byte array. Java
     * "/" separator is transformed into Windows "\".
     * 
    ...
    /**
     * Opens current node's underlying Windows registry key using a given
     * security mask.
     * 
     * @param securityMask
     ...
     * security mask.
     * 
     * @param windowsAbsolutePath
     *        Windows absolute path of the key as a
     *        byte-encoded string.
     * @param mask1
     *        Preferred Windows security mask.
     * @param mask2
    ...
    private int openKey(byte[] windowsAbsolutePath, int mask1, int mask2) {
        /*
         * Check if key's path is short enough be opened at once otherwise use a
         * path-splitting procedure
         */
        if (windowsAbsolutePath.length <= MAX_WINDOWS_PATH_LENGTH + 1) {
            ...
            if (result[ERROR_CODE] != ERROR_SUCCESS) {
                logger().warning("Could not open windows registry node " + byteArrayToString(
                        windowsAbsolutePath()) + " at root 0x" + Integer.toHexString(rootNativeHandle())
                        + ". Windows RegOpenKey(...) returned error code " + result[ERROR_CODE] + ".");
                result[NATIVE_HANDLE] = NULL_NATIVE_HANDLE;
                if (result[ERROR_CODE] == ERROR_ACCESS_DENIED) {
                    throw new SecurityException("Could not open windows registry node " + byteArrayToString(
                            windowsAbsolutePath()) + " at root 0x" + Integer.toHexString(rootNativeHandle())
                            + ": Access denied");
                }
            }
    ...
    /**
     * Opens Windows registry key at a given relative path with respect to a
     * given Windows registry key.
     * 
     * @param windowsAbsolutePath
     *        Windows relative path of the key as a
     *        byte-encoded string.
     * @param nativeHandle
    ...
    private int openKey(int nativeHandle, byte[] windowsRelativePath, int mask1, int mask2) {
        ...
        if (windowsRelativePath.length <= MAX_WINDOWS_PATH_LENGTH + 1) {
            ...
            if (result[ERROR_CODE] != ERROR_SUCCESS) {
                logger().warning("Could not open windows registry node " + byteArrayToString(
                        windowsAbsolutePath()) + " at root 0x" + Integer.toHexString(nativeHandle)
                        + ". Windows RegOpenKey(...) returned error code " + result[ERROR_CODE] + ".");
                result[NATIVE_HANDLE] = NULL_NATIVE_HANDLE;
            }
            return result[NATIVE_HANDLE];
            ...
            System.arraycopy(windowsRelativePath, separatorPosition + 1, nextRelativePath, 0,
                    nextRelativePath.length);
    ...
    /**
     * Closes Windows registry key. Logs a warning if Windows registry is
     * unavailable.
     * 
     * @param key's
     *        Windows registry handle.
     * @see #openKey(int)
     * @see #openKey(byte[],int)
     * @see #openKey(int, byte[],int)
     */
    private void closeKey(int nativeHandle) {
        ...
        if (result != ERROR_SUCCESS) {
            logger().warning("Could not close windows registry node " + byteArrayToString(
                    windowsAbsolutePath()) + " at root 0x" + Integer.toHexString(rootNativeHandle())
    ...
    /**
     * Implements <tt>AbstractPreferences</tt> <tt>putSpi()</tt> method. Puts
     * name-value pair into the underlying Windows registry node. Logs a
     * warning, if Windows registry is unavailable.
     * 
     * @see #getSpi(String)
     */
    protected void putSpi(String javaName, String value) {
        ...
        int result = WindowsRegSetValueEx1(nativeHandle, toWindowsName(javaName), toWindowsValueString(
                value));
        if (result != ERROR_SUCCESS) {
            logger().warning("Could not assign value to key " + byteArrayToString(toWindowsName(javaName))
                    + " at Windows registry node " + byteArrayToString(windowsAbsolutePath()) + " at root 0x"
    ...
    /**
     * Implements <tt>AbstractPreferences</tt> <tt>getSpi()</tt> method. Gets a
     * string value from the underlying Windows registry node. Logs a warning,
     * if Windows registry is unavailable.
     * 
     ...
     * node, if this value still exists. Logs a warning, if Windows registry is
     * unavailable or key has already been deleted.
     */
    protected void removeSpi(String key) {
        ...
        if (result != ERROR_SUCCESS && result != ERROR_FILE_NOT_FOUND) {
            logger().warning("Could not delete windows registry value " + byteArrayToString(
                    windowsAbsolutePath()) + "\\" + toWindowsName(key) + " at root 0x" + Integer.toHexString(
                            rootNativeHandle()) + ". Windows RegDeleteValue(...) returned error code "
                    + result + ".");
            isBackingStoreAvailable = false;
        }
        closeKey(nativeHandle);
    ...
    /**
     * Implements <tt>AbstractPreferences</tt> <tt>keysSpi()</tt> method. Gets
     * value names from the underlying Windows registry node. Throws a
     * BackingStoreException and logs a warning, if Windows registry is
     * unavailable.
     */
    protected String[] keysSpi() throws BackingStoreException {
        ...
        int nativeHandle = openKey(KEY_QUERY_VALUE);
        if (nativeHandle == NULL_NATIVE_HANDLE) {
            throw new BackingStoreException("Could not open windows registry node " + byteArrayToString(
                    windowsAbsolutePath()) + " at root 0x" + Integer.toHexString(rootNativeHandle()) + ".");
        }
        int[] result = WindowsRegQueryInfoKey1(nativeHandle);
        if (result[ERROR_CODE] != ERROR_SUCCESS) {
            String info = "Could not query windows registry node " + byteArrayToString(windowsAbsolutePath())
                    + " at root 0x" + Integer.toHexString(rootNativeHandle())
                    + ". Windows RegQueryInfoKeyEx(...) returned error code " + result[ERROR_CODE] + ".";
            logger().warning(info);
        ...
        for (int i = 0; i < valuesNumber; i++) {
            byte[] windowsName = WindowsRegEnumValue1(nativeHandle, i, maxValueNameLength + 1);
            if (windowsName == null) {
                String info = "Could not enumerate value #" + i + "  of windows node " + byteArrayToString(
                        windowsAbsolutePath()) + " at root 0x" + Integer.toHexString(rootNativeHandle())
                        + ".";
                logger().warning(info);
                throw new BackingStoreException(info);
    ...
    /**
     * Implements <tt>AbstractPreferences</tt> <tt>childrenNamesSpi()</tt>
     * method. Calls Windows registry to retrive children of this node. Throws a
     * BackingStoreException and logs a warning message, if Windows registry is
     * not available.
     */
    protected String[] childrenNamesSpi() throws BackingStoreException {
        ...
        for (int i = 0; i < subKeysNumber; i++) {
            byte[] windowsName = WindowsRegEnumKeyEx1(nativeHandle, i, maxKeyLength + 1);
            if (windowsName == null) {
                String info = "Could not enumerate key #" + i + "  of windows node " + byteArrayToString(
                        windowsAbsolutePath()) + " at root 0x" + Integer.toHexString(rootNativeHandle())
                        + ". ";
                logger().warning(info);
                throw new BackingStoreException(info);
    ...
    /**
     * Implements <tt>Preferences</tt> <tt>flush()</tt> method. Flushes Windows
     * registry changes to disk. Throws a BackingStoreException and logs a
     * warning message if Windows registry is not available.
     */
    public void flush() throws BackingStoreException {
        ...
        if (result != ERROR_SUCCESS) {
            String info = "Could not flush windows registry node " + byteArrayToString(windowsAbsolutePath())
                    + " at root 0x" + Integer.toHexString(rootNativeHandle())
    ...
    /**
     * Implements <tt>Preferences</tt> <tt>sync()</tt> method. Flushes Windows
     * registry changes to disk. Equivalent to flush().
     * 
     * @see flush()
     */
     ...
     * Constructs a child node with a given name and creates its underlying
     * Windows registry node, if it does not exist. Logs a warning message, if
     ...
     * Deletes underlying Windows registry node. Throws a BackingStoreException
     * and logs a warning, if Windows registry is not available.
     */
    public void removeNodeSpi() throws BackingStoreException {
        int parentNativeHandle = ((WindowsPreferences) parent()).openKey(DELETE);
        if (parentNativeHandle == NULL_NATIVE_HANDLE) {
            throw new BackingStoreException("Could not open parent windows registry node of "
                    + byteArrayToString(windowsAbsolutePath()) + " at root 0x" + Integer.toHexString(
                            rootNativeHandle()) + ".");
        }
        int result = WindowsRegDeleteKey(parentNativeHandle, toWindowsName(name()));
        if (result != ERROR_SUCCESS) {
            String info = "Could not delete windows registry node " + byteArrayToString(windowsAbsolutePath())
                    + " at root 0x" + Integer.toHexString(rootNativeHandle())
                    + ". Windows RegDeleteKeyEx(...) returned error code " + result + ".";
            logger().warning(info);
            throw new BackingStoreException(info);
        }
        closeKey(parentNativeHandle);
     ...
     * {@link #toWindowsName(String) toWindowsName()} for a detailed description
     * of encoding conventions.
     * 
    ...
    private static String toJavaName(byte[] windowsNameArray) {
        ...
        for (int i = 0; i < windowsName.length(); i++) {
            if ((ch = windowsName.charAt(i)) == '/') {
                char next = ' ';
                if ((windowsName.length() > i + 1) && ((next = windowsName.charAt(i + 1)) >= 'A')
                        && (next <= 'Z')) {
                    ch = next;
                    i++;
     ...
     * string, using altBase64 encoding. See {@link #toWindowsName(String)
     * toWindowsName()} for a detailed description of encoding conventions.
     */
    ...
    /**
     * Converts value's or node's name to its Windows representation as a
     * byte-encoded string. Two encodings, simple and altBase64 are used.
     * <p>
     * <i>Simple</i> encoding is used, if java string does not contain any
     * characters less, than 0x0020, or greater, than 0x007f. Simple encoding
     * adds "/" character to capital letters, i.e. "A" is encoded as "/A".
     * Character '\' is encoded as '//', '/' is encoded as '\'. The constructed
     * string is converted to byte array by truncating the highest byte and
     * adding the terminating <tt>null</tt> character.
     * <p>
     * <i>altBase64</i> encoding is used, if java string does contain at least
     * one character less, than 0x0020, or greater, than 0x007f. This encoding
     * is marked by setting first two bytes of the Windows string to '/!'. The
     * java name is then encoded using byteArrayToAltBase64() method from Base64
     * class.
     */
    ...
    /**
     * Converts value's or node's name to its Windows representation as a
     * byte-encoded string, using altBase64 encoding. See
     * {@link #toWindowsName(String) toWindowsName()} for a detailed description
     * of encoding conventions.
     */
    ...
    /**
     * Converts value string from its Windows representation to java string. See
    ...
    private static String toJavaValueString(byte[] windowsNameArray) {
        ...
        for (int i = 0; i < windowsName.length(); i++) {
            if ((ch = windowsName.charAt(i)) == '/') {
                ...
                } else if ((windowsName.length() > i + 1) && ((windowsName.charAt(i + 1)) >= 'A')
                        && (next <= 'Z')) {
                    ch = next;
                    i++;
                } else if ((windowsName.length() > i + 1) && (next == '/')) {
    ...
    /**
     * Converts value string to it Windows representation. as a byte-encoded
     * string. Encoding algorithm adds "/" character to capital letters, i.e.
     * "A" is encoded as "/A". Character '\' is encoded as '//', '/' is encoded
     * as '\'. Then encoding scheme similar to jdk's native2ascii converter is
     * used to convert java string to a byte array of ASCII characters.
     */
    ...
    private int rootNativeHandle() {
        return (isUserNode() ? USER_ROOT_NATIVE_HANDLE : SYSTEM_ROOT_NATIVE_HANDLE);
    }
