...
class XmlSupport {
    ...
    private static final String PREFS_DTD = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>"
            ...
            "<!ELEMENT preferences (root) >" + "<!ATTLIST preferences"
            + " EXTERNAL_XML_VERSION CDATA \"0.0\"  >" +

            "<!ELEMENT root (map, node*) >" + "<!ATTLIST root"
            + "          type (system|user) #REQUIRED >" +

            "<!ELEMENT node (map, node*) >" + "<!ATTLIST node"
            + "          name CDATA #REQUIRED >" +

            "<!ELEMENT map (entry*) >" + "<!ATTLIST map"
            + "  MAP_XML_VERSION CDATA \"0.0\"  >" + "<!ELEMENT entry EMPTY >"
     ...
     * subnodes, to the specified output stream. Preferences are exported as an
     * XML document conforming to the definition in the Preferences spec.
     *
     * @throws IOException
     ...
     *                               removed with the
     *                               {@link Preferences#removeNode()} method.
     */
    static void export(OutputStream os, final Preferences p, boolean subTree)
        ...
        Element xmlRoot = (Element) preferences.appendChild(doc.createElement(
                "root"));
        ...
        for (Preferences kid = p, dad = kid
                .parent(); dad != null; kid = dad, dad = kid.parent()) {
            ancestors.add(kid);
        }
        Element e = xmlRoot;
     ...
     *                               preferences or children out
     *                               of the specified preferences node.
     */
    private static void putPreferencesInXml(Element elt, Document doc,
        ...
        synchronized (((AbstractPreferences) prefs).lock) {
            ...
            for (int i = 0; i < keys.length; i++) {
                Element entry = (Element) map.appendChild(doc.createElement(
                        "entry"));
        ...
        if (subTree) {
            for (int i = 0; i < kidNames.length; i++) {
                Element xmlKid = (Element) elt.appendChild(doc.createElement(
                        "node"));
     ...
     *                                           type.
     */
    static void importPreferences(InputStream is) throws IOException,
        ...
        try {
            ...
            Element xmlRoot = (Element) doc.getDocumentElement().getChildNodes()
                    .item(0);
     ...
     * requisite DTD URI.
     */
    private static Document loadPrefsDoc(InputStream in) throws SAXException,
            IOException {
    ...
    private static final void writeDoc(Document doc, OutputStream out)
        ...
        try {
            ...
            t.transform(new DOMSource(doc), new StreamResult(new BufferedWriter(
    ...
    private static void ImportPrefs(Preferences prefsNode, Element map) {
        ...
        for (int i = 0, numEntries = entries.getLength(); i < numEntries; i++) {
            ...
            prefsNode.put(entry.getAttribute("key"), entry.getAttribute(
                    "value"));
        }
    }
     ...
     *
     * @throws IOException
     *                     if writing to the specified output stream results in
     *                     an
     *                     <tt>IOException</tt>.
     */
    static void exportMap(OutputStream os, Map<String, String> map)
        ...
        for (Iterator<Map.Entry<String, String>> i = map.entrySet()
                .iterator(); i.hasNext();) {
            Map.Entry<String, String> e = i.next();
            Element xe = (Element) xmlMap.appendChild(doc.createElement(
                    "entry"));
    ...
    static void importMap(InputStream is, Map<String, String> m)
        ...
        try {
            ...
            for (int i = 0, numEntries = entries
                    .getLength(); i < numEntries; i++) {
                Element entry = (Element) entries.item(i);
