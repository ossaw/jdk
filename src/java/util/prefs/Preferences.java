 ...
 * and one for system preferences. Each user has a separate user preference
 * tree, and all users in a given system share the same system preference tree.
 * The precise description of "user" and "system" will vary from implementation
 * to implementation. Typical information stored in the user preference tree
 * might include font choice, color choice, or preferred window location and
 * size for a particular application. Typical information stored in the system
 * preference tree might include installation configuration data for an
 * application.
 * <p>
 * Nodes in a preference tree are named in a similar fashion to directories in a
 * hierarchical file system. Every node in a preference tree has a <i>node
 * name</i> (which is not necessarily unique), a unique <i>absolute path
 * name</i>, and a path name <i>relative</i> to each ancestor including itself.
 * <p>
 * The root node has a node name of the empty string (""). Every other node has
 * an arbitrary node name, specified at the time it is created. The only
 * restrictions on this name are that it cannot be the empty string, and it
 * cannot contain the slash character ('/').
 * <p>
 * The root node has an absolute path name of <tt>"/"</tt>. Children of the root
 * node have absolute path names of <tt>"/" + </tt><i>&lt;node name&gt;</i>. All
 * other nodes have absolute path names of <i>&lt;parent's absolute path
 * name&gt;</i><tt> + "/" + </tt><i>&lt;node name&gt;</i>. Note that all
 ...
 * <li>No path name with the exception of the root's absolute path name ends in
 * the slash character.
 ...
 * to the backing store. Normal termination of the Java Virtual Machine will
 * <i>not</i> result in the loss of pending updates -- an explicit
 * <tt>flush</tt> invocation is <i>not</i> required upon termination to ensure
 * that pending updates are made persistent.
 * <p>
 * All of the methods that read preferences from a <tt>Preferences</tt> object
 * require the invoker to provide a default value. The default value is returned
 * if no value has been previously set <i>or if the backing store is
 * unavailable</i>. The intent is to allow applications to operate, albeit with
 * slightly degraded functionality, even if the backing store becomes
 ...
 * applications should have no need to invoke any of these methods, which can be
 * identified by the fact that they are declared to throw
 * {@link BackingStoreException}.
 * <p>
 * The methods in this class may be invoked concurrently by multiple threads in
 * a single JVM without the need for external synchronization, and the results
 * will be equivalent to some serial execution. If this class is used
 * concurrently <i>by multiple JVMs</i> that store their preference data in the
 * same backing store, the data store will not be corrupted, but no other
 ...
 * "exported" to an XML document, and XML documents representing preferences to
 * be "imported" back into the system. This facility may be used to back up all
 * or part of a preference tree, and subsequently restore from the backup.
 * <p>
 ...
 * <i>not</i> accessed when exporting or importing preferences; it merely serves
 ...
 * </pre>
 *
 * Every <tt>Preferences</tt> implementation must have an associated
 * {@link PreferencesFactory} implementation. Every Java(TM) SE implementation
 * must provide some means of specifying which <tt>PreferencesFactory</tt>
 * implementation is used to generate the root preferences nodes. This allows
 * the administrator to replace the default preferences implementation with an
 * alternative implementation.
 * <p>
 * Implementation note: In Sun's JRE, the <tt>PreferencesFactory</tt>
 * implementation is located as follows:
 * <ol>
 * <li>
 * <p>
 * If the system property <tt>java.util.prefs.PreferencesFactory</tt> is
 * defined, then it is taken to be the fully-qualified name of a class
 * implementing the <tt>PreferencesFactory</tt> interface. The class is loaded
 * and instantiated; if this process fails then an unspecified error is thrown.
 * </p>
 * </li>
 * <li>
 * <p>
 * If a <tt>PreferencesFactory</tt> implementation class file has been installed
 * in a jar file that is visible to the
 * {@link java.lang.ClassLoader#getSystemClassLoader system class loader}, and
 * that jar file contains a provider-configuration file named
 * <tt>java.util.prefs.PreferencesFactory</tt> in the resource directory
 * <tt>META-INF/services</tt>, then the first class name specified in that file
 * is taken. If more than one such jar file is provided, the first one found
 * will be used. The class is loaded and instantiated; if this process fails
 ...
 * file is provided, then the system-wide default <tt>PreferencesFactory</tt>
...
public abstract class Preferences {
    ...
    private static PreferencesFactory factory() {
        ...
        if (factoryName != null) {
            ...
            try {
                return (PreferencesFactory) Class.forName(factoryName, false, ClassLoader
                        .getSystemClassLoader()).newInstance();
            } catch (Exception ex) {
                try {
                    ...
                    return (PreferencesFactory) Class.forName(factoryName, false, Thread.currentThread()
    ...
    private static PreferencesFactory factory1() {
        ...
        Iterator<PreferencesFactory> itr = ServiceLoader.load(PreferencesFactory.class, ClassLoader
        ...
        try {
            return (PreferencesFactory) Class.forName(platformFactory, false, Preferences.class
                    .getClassLoader()).newInstance();
        } catch (Exception e) {
            throw new InternalError("Can't instantiate platform default Preferences factory "
                    + platformFactory, e);
        }
    }
     ...
     * is associated (by convention) with the specified class's package. The
     * convention is as follows: the absolute path name of the node is the fully
     * qualified package name, preceded by a slash (<tt>'/'</tt>), and with each
     * period (<tt>'.'</tt>) replaced by a slash. For example the absolute path
     * name of the node associated with the class <tt>com.acme.widget.Foo</tt>
     * is <tt>/com/acme/widget</tt>.
     * <p>
     * This convention does not apply to the unnamed package, whose associated
     * preference node is <tt>&lt;unnamed&gt;</tt>. This node is not intended
     * for long term use, but for convenience in the early development of
     * programs that do not yet belong to a package, and for "throwaway"
     * programs. <i>Valuable data should not be stored at this node as it is
     * shared by all programs that use it.</i>
     * <p>
     ...
     * preferences node and decreases the likelihood of a run-time failure. (If
     * the class name is misspelled, it will typically result in a compile-time
     * error.)
     * <p>
     * Invoking this method will result in the creation of the returned node and
     * its ancestors if they do not already exist. If the returned node did not
     * exist prior to this call, this node and any ancestors that were created
     * by this call are not guaranteed to become permanent until the
     * <tt>flush</tt> method is called on the returned node (or one of its
     ...
     *         denies
     *         <tt>RuntimePermission("preferences")</tt>.
     ...
     *
     * @param c
     *        the class for whose package a system preference node is
     *        desired.
     ...
     *         if the package has node preferences node
     *         associated with it.
     */
    ...
    /**
     * This permission object represents the permission required to get access
     * to the user or system root (which in turn allows for all other
     * operations).
     */
     ...
     * @throws IllegalArgumentException
     *         if <tt>key.length()</tt> exceeds
     *         <tt>MAX_KEY_LENGTH</tt> or
     *         if <tt>value.length</tt> exceeds
     ...
     *         to a failure in the
     *         backing store, or inability to communicate
     *         with it.
     * @throws IllegalStateException
     *         if this node (or an ancestor) has been
     *         removed with the
     ...
     * specified key in this preference node. The associated string is the one
     * that would be returned if the float value were passed to
     ...
     *         contains multiple
     *         consecutive slash characters, or ends
     ...
     *         if this node (or an ancestor) has been
     *         removed with the
     *         {@link #removeNode()} method and
     *         <tt>pathName</tt> is not the
     *         empty string (<tt>""</tt>).
     */
     ...
     * , {@link #isUserNode()}, {@link #flush()} or {@link #node(String)
     * nodeExists("")} on the corresponding <tt>Preferences</tt> instance will
     * fail with an <tt>IllegalStateException</tt>. (The methods defined on
     * {@link Object} can still be invoked on a node after it has been removed;
     * they will not throw <tt>IllegalStateException</tt>.)
     * <p>
     ...
     *         completed due to a failure in the
     *         backing store, or inability to
     *         communicate with it.
     * @throws IllegalStateException
     *         if this node (or an ancestor) has
     *         already been removed with
     *         the {@link #removeNode()} method.
     * @throws UnsupportedOperationException
     *         if this method is invoked on the
     *         root node.
     * @see #flush()
     */
    ...
    /**
     * Returns <tt>true</tt> if this preference node is in the user preference
    ...
    /**
     * Returns a string representation of this preferences node, as if computed
     * by the expression:<tt>(this.isUserNode() ? "User" :
     ...
     * (Preference change events are <i>not</i> generated by the
     * {@link #removeNode()} method, which generates a <i>node change event</i>.
     ...
     * @throws IllegalArgumentException
     *         if <tt>pcl</tt> was not a registered
     *         preference change
    ...
    /**
     * Registers the specified listener to receive <i>node change events</i> for
     * this node. A node change event is generated when a child node is added to
     * or removed from this node. (A single {@link #removeNode()} invocation
     * results in multiple <i>node change events</i>, one for every node in the
     * subtree rooted at the removed node.)
     * <p>
     * Events are only guaranteed for changes made within the same JVM as the
     * registered listener, though some implementations may generate events for
     * changes made outside this JVM. Events may be generated before the changes
     ...
     *
     * @param ncl
     *        The <tt>NodeChangeListener</tt> to add.
     * @throws NullPointerException
     *         if <tt>ncl</tt> is null.
     * @throws IllegalStateException
     *         if this node (or an ancestor) has been
     *         removed with the
     *         {@link #removeNode()} method.
     ...
     *
     * @param ncl
     *        The <tt>NodeChangeListener</tt> to remove.
     * @throws IllegalArgumentException
     *         if <tt>ncl</tt> was not a registered
     *         <tt>NodeChangeListener</tt> on this
     *         node.
     * @throws IllegalStateException
     *         if this node (or an ancestor) has been
     *         removed with the
     ...
     * originally came from a different user's preference tree). If any of the
     * preferences described by the document inhabit preference nodes that do
     * not exist, the nodes will be created.
     * <p>
     ...
     * This method is an exception to the general rule that the results of
     * concurrently executing multiple methods in this class yields results
     * equivalent to some serial execution. The method behaves as if implemented
     * on top of the other public methods in this class, notably
     * {@link #node(String)} and {@link #put(String, String)}.
     *
     * @param is
     ...
     *         with the mandated document
     *         type.
     * @throws SecurityException
     *         If a security manager is
     *         present and it denies
     *         <tt>RuntimePermission("preferences")</tt>.
     * @see RuntimePermission
     */
