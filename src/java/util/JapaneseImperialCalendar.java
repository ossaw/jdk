...
class JapaneseImperialCalendar extends Calendar {
    ...
    // (singleton).
    private static final LocalGregorianCalendar jcal = (LocalGregorianCalendar) CalendarSystem.forName(
            "japanese");
    ...
    public void roll(int field, int amount) {
        ...
        switch (field) {
                ...
                if (!isTransitionYear(jdate.getNormalizedYear())) {
                    ...
                    if (year == getMaximum(YEAR)) {
                        ...
                        if (n == max) {
                            ...
                            if (jd.getDayOfMonth() == d.getDayOfMonth() && jd.getTimeOfDay() > d
                                    .getTimeOfDay()) {
                                jd.setMonth(n + 1);
                    ...
                    set(MONTH, n);
                    if (n == min) {
                        if (!(transition.getMonth() == BaseCalendar.JANUARY && transition
                                .getDayOfMonth() == 1)) {
            ...
            case DAY_OF_MONTH: {
                ...
                int value = getRolledValue((int) (cachedFixedDate - month1), amount, 0, actualMonthLength()
                        - 1);
                LocalGregorianCalendar.Date d = getCalendarDate(month1 + value);
                assert getEraIndex(d) == internalGetEra() && d.getYear() == internalGet(YEAR) && d.getMonth()
                        - 1 == internalGet(MONTH);
                set(DAY_OF_MONTH, d.getDayOfMonth());
                return;
            }
    ...
    public String getDisplayName(int field, int style, Locale locale) {
        if (!checkDisplayNameParams(field, style, SHORT, NARROW_FORMAT, locale, ERA_MASK | YEAR_MASK
                | MONTH_MASK | DAY_OF_WEEK_MASK | AM_PM_MASK)) {
            return null;
        }
        ...
        String name = CalendarDataUtility.retrieveFieldValueName(getCalendarType(), field, fieldValue, style,
                locale);
    ...
    public Map<String, Integer> getDisplayNames(int field, int style, Locale locale) {
        if (!checkDisplayNameParams(field, style, ALL_STYLES, NARROW_FORMAT, locale, ERA_MASK | YEAR_MASK
                | MONTH_MASK | DAY_OF_WEEK_MASK | AM_PM_MASK)) {
            return null;
        }
    ...
    private int computeFields(int fieldMask, int tzMask) {
        ...
        if ((fieldMask & (DAY_OF_YEAR_MASK | WEEK_OF_YEAR_MASK | WEEK_OF_MONTH_MASK
            ...
            long fixedDateMonth1 = transitionYear ? getFixedDateMonth1(jdate, fixedDate)
                    : fixedDate - dayOfMonth + 1;
        ...
        if (!isLenient()) {
            for (int field = 0; field < FIELD_COUNT; field++) {
                ...
                if (originalFields[field] != internalGet(field)) {
                    ...
                    throw new IllegalArgumentException(getFieldName(field) + "=" + wrongValue + ", expected "
        ...
        if (isFieldSet(fieldMask, MONTH)) {
            ...
            if (isFieldSet(fieldMask, DAY_OF_MONTH)) {
                ...
                if (isFieldSet(fieldMask, WEEK_OF_MONTH)) {
                    ...
                    if (dowim >= 0) {
                        fixedDate = LocalGregorianCalendar.getDayOfWeekDateOnOrBefore(fixedDate + (7 * dowim)
                                - 1, dayOfWeek);
                    } else {
                        ...
                        // date.
                        fixedDate = LocalGregorianCalendar.getDayOfWeekDateOnOrBefore(fixedDate + lastDate
                                - 1, dayOfWeek);
                    }
                }
            }
    ...
    private static int getTransitionEraIndex(LocalGregorianCalendar.Date date) {
        ...
        if (transitionDate.getYear() == date.getNormalizedYear() && transitionDate.getMonth() == date
                .getMonth()) {
            return eraIndex;
        }
        if (eraIndex < eras.length - 1) {
