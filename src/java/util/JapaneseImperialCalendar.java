...
/**
 * <code>JapaneseImperialCalendar</code> implements a Japanese calendar system
 * in which the imperial era-based year numbering is supported from the Meiji
 * era. The following are the eras supported by this calendar system.
 * 
 * <pre>
 * <tt>
 ...
 *
 * <p>
 * <code>ERA</code> value 0 specifies the years before Meiji and the Gregorian
 * year values are used. Unlike {@link GregorianCalendar}, the Julian to
 * Gregorian transition is not supported because it doesn't make any sense to
 * the Japanese calendar systems used before Meiji. To represent the years
 * before Gregorian year 1, 0 and negative values are used. The Japanese
 * Imperial rescripts and government decrees don't specify how to deal with time
 * differences for applying the era transitions. This calendar implementation
...
class JapaneseImperialCalendar extends Calendar {
     ...
     * This implementation uses sun.util.calendar.LocalGregorianCalendar to
     * perform most of the calendar calculations. LocalGregorianCalendar is
     * configurable and reads <JRE_HOME>/lib/calendars.properties at the
     * start-up.
     */
    ...
    // Reference to the sun.util.calendar.LocalGregorianCalendar instance
    // (singleton).
    private static final LocalGregorianCalendar jcal = (LocalGregorianCalendar) CalendarSystem
            .forName("japanese");
    ...
    private static final Era BEFORE_MEIJI_ERA = new Era("BeforeMeiji", "BM",
     ...
     * ---------- ------- ------- ------- ------- ERA 0 0 1 1 YEAR -292275055 1
     * ? ? MONTH 0 0 11 11 WEEK_OF_YEAR 1 1 52* 53 WEEK_OF_MONTH 0 0 4* 6
    ...
    static {
        ...
        sinceFixedDates[index] = gcal.getFixedDate(BEFORE_MEIJI_ERA
                .getSinceDate());
        ...
        for (int i = 1; i < eras.length; i++) {
            ...
            if (!(lgd.getMonth() == BaseCalendar.JANUARY && lgd
                    .getDayOfMonth() == 1)) {
                y--;
            }
            year = Math.min(y, year);
        }
        LEAST_MAX_VALUES[YEAR] = year; // Max year could be smaller than this
                                       // value.
        LEAST_MAX_VALUES[DAY_OF_YEAR] = dayOfYear;
    }

    /**
     * jdate always has a sun.util.calendar.LocalGregorianCalendar.Date instance
     * to avoid overhead of creating it for each calculation.
     */
    private transient LocalGregorianCalendar.Date jdate;

    /**
     * Temporary int[2] to get time zone offsets. zoneOffsets[0] gets the GMT
     * offset value and zoneOffsets[1] gets the daylight saving value.
     */
    ...
    /**
     * Constructs a <code>JapaneseImperialCalendar</code> based on the current
     * time in the given time zone with the given locale.
     *
     * @param zone
     *                the given time zone.
     * @param aLocale
     ...
     * argument is a <code>JapaneseImperialCalendar</code> object that
     * represents the same time value (millisecond offset from the
    ...
    /**
     * Generates the hash code for this <code>JapaneseImperialCalendar</code>
     * object.
     */
    public int hashCode() {
    ...
    public void add(int field, int amount) {
        ...
        if (field == YEAR) {
            LocalGregorianCalendar.Date d = (LocalGregorianCalendar.Date) jdate
                    .clone();
            d.addYear(amount);
            pinDayOfMonth(d);
    ...
    public void roll(int field, int amount) {
        ...
        switch (field) {
            ...
            // E.g., <jan31>.roll(MONTH, 1) -> <feb28> or <feb29>.
            {
                if (!isTransitionYear(jdate.getNormalizedYear())) {
                    ...
                    if (year == getMaximum(YEAR)) {
                        ...
                        CalendarDate d = jcal.getCalendarDate(Long.MAX_VALUE,
                                getZone());
                        max = d.getMonth() - 1;
                        int n = getRolledValue(internalGet(field), amount, min,
                                max);
                        if (n == max) {
                            ...
                            if (jd.getDayOfMonth() == d.getDayOfMonth() && jd
                                    .getTimeOfDay() > d.getTimeOfDay()) {
                                jd.setMonth(n + 1);
                        ...
                        CalendarDate jd = jcal.getCalendarDate(time, getZone());
                        CalendarDate d = jcal.getCalendarDate(Long.MIN_VALUE,
                                getZone());
                        min = d.getMonth() - 1;
                        int n = getRolledValue(internalGet(field), amount, min,
                                max);
                    ...
                    if (n == min) {
                        if (!(transition.getMonth() == BaseCalendar.JANUARY
                            ...
                            if (jdate.getDayOfMonth() < transition
                                    .getDayOfMonth()) {
            ...
            case WEEK_OF_YEAR: {
                ...
                if (!(d.getEra() == jdate.getEra() && d.getYear() == jdate
                        .getYear())) {
                    min++;
                }
            ...
            case WEEK_OF_MONTH: {
                boolean isTransitionYear = isTransitionYear(jdate
                        .getNormalizedYear());
                ...
                long monthDay1st = LocalGregorianCalendar
                        .getDayOfWeekDateOnOrBefore(month1 + 6,
                                getFirstDayOfWeek());
            ...
            case DAY_OF_MONTH: {
                ...
                // TODO: Need to change the spec to be usable DAY_OF_MONTH
                // rolling...
                ...
                int value = getRolledValue((int) (cachedFixedDate - month1),
                        amount, 0, actualMonthLength() - 1);
                LocalGregorianCalendar.Date d = getCalendarDate(month1 + value);
                assert getEraIndex(d) == internalGetEra() && d
                        .getYear() == internalGet(YEAR) && d.getMonth()
                                - 1 == internalGet(MONTH);
                set(DAY_OF_MONTH, d.getDayOfMonth());
                return;
            }
            ...
            case DAY_OF_WEEK: {
                ...
                if (!isTransitionYear(normalizedYear) && !isTransitionYear(
                        normalizedYear - 1)) {
                ...
                long fd = cachedFixedDate;
                long dowFirst = LocalGregorianCalendar
                        .getDayOfWeekDateOnOrBefore(fd, getFirstDayOfWeek());
                fd += amount;
                if (fd < dowFirst) {
                    fd += 7;
    ...
    public String getDisplayName(int field, int style, Locale locale) {
        if (!checkDisplayNameParams(field, style, SHORT, NARROW_FORMAT, locale,
                ERA_MASK | YEAR_MASK | MONTH_MASK | DAY_OF_WEEK_MASK
                        | AM_PM_MASK)) {
            return null;
        }
        ...
        if (field == YEAR && (getBaseStyle(style) != LONG || fieldValue != 1
                || get(ERA) == 0)) {
            return null;
        }

        String name = CalendarDataUtility.retrieveFieldValueName(
                getCalendarType(), field, fieldValue, style, locale);
    ...
    public Map<String, Integer> getDisplayNames(int field, int style,
        ...
        if (!checkDisplayNameParams(field, style, ALL_STYLES, NARROW_FORMAT,
                locale, ERA_MASK | YEAR_MASK | MONTH_MASK | DAY_OF_WEEK_MASK
                        | AM_PM_MASK)) {
            return null;
        }
        Map<String, Integer> names;
        names = CalendarDataUtility.retrieveFieldValueNames(getCalendarType(),
                field, style, locale);
     ...
     * smallest value returned by the {@link Calendar#get(int) get} method for
     * any possible time value, taking into consideration the current values of
     * the {@link Calendar#getFirstDayOfWeek() getFirstDayOfWeek},
    ...
    public int getMaximum(int field) {
        switch (field) {
            case YEAR: {
                ...
                LocalGregorianCalendar.Date d = jcal.getCalendarDate(
                        Long.MAX_VALUE, getZone());
     ...
     *         <code>JapaneseImperialCalendar</code>
     * @see #getMinimum(int)
     * @see #getMaximum(int)
     * @see #getGreatestMinimum(int)
     * @see #getLeastMaximum(int)
    ...
    public int getActualMinimum(int field) {
        ...
        LocalGregorianCalendar.Date jd = jcal.getCalendarDate(jc
                .getTimeInMillis(), getZone());
        ...
        switch (field) {
            ...
            case WEEK_OF_YEAR: {
                ...
                if ((day1 < jan1) || (day1 == jan1 && jd.getTimeOfDay() < d
                        .getTimeOfDay())) {
                    value++;
                }
            }
     ...
     * {@link Calendar#getFirstDayOfWeek() getFirstDayOfWeek},
     * {@link Calendar#getMinimalDaysInFirstWeek() getMinimalDaysInFirstWeek},
     * and {@link Calendar#getTimeZone() getTimeZone} methods. For example, if
     * the date of this instance is Heisei 16February 1, the actual maximum
     * value of the <code>DAY_OF_MONTH</code> field is 29 because Heisei 16 is a
     * leap year, and if the date of this instance is Heisei 17 February 1, it's
     * 28.
     *
     * @param field
     *              the calendar field
        ...
        switch (field) {
                ...
                if (isTransitionYear(date.getNormalizedYear())) {
                    ...
                    if (fd < transition) {
                        LocalGregorianCalendar.Date ldate = (LocalGregorianCalendar.Date) date
                                .clone();
                        jcal.getCalendarDateFromFixedDate(ldate, transition
                                - 1);
                    ...
                    LocalGregorianCalendar.Date d = jcal.getCalendarDate(
                            Long.MAX_VALUE, getZone());
                    if (date.getEra() == d.getEra() && date.getYear() == d
                            .getYear()) {
                        value = d.getMonth() - 1;
                    }
                }
            ...
            case DAY_OF_YEAR: {
                if (isTransitionYear(date.getNormalizedYear())) {
                    ...
                    d.setDate(date.getNormalizedYear(), BaseCalendar.JANUARY,
                            1);
                    if (fd < transition) {
                    ...
                    if (date.getEra() == d.getEra() && date.getYear() == d
                        ...
                        CalendarDate d1 = jcal.getCalendarDate(Long.MIN_VALUE,
                                getZone());
            ...
            case WEEK_OF_YEAR: {
                if (!isTransitionYear(date.getNormalizedYear())) {
                    LocalGregorianCalendar.Date jd = jcal.getCalendarDate(
                            Long.MAX_VALUE, getZone());
                    if (date.getEra() == jd.getEra() && date.getYear() == jd
                            .getYear()) {
                        long fd = jcal.getFixedDate(jd);
                        long jan1 = getFixedDateJan1(jd, fd);
                        value = getWeekNumber(jan1, fd);
                    } else if (date.getEra() == null && date
                            .getYear() == getMinimum(YEAR)) {
                        CalendarDate d = jcal.getCalendarDate(Long.MIN_VALUE,
                                getZone());
                        // shift 400 years to avoid underflow
                        d.addYear(+400);
                        ...
                        CalendarDate d = gcal.newCalendarDate(
                                TimeZone.NO_TIMEZONE);
                        d.setDate(date.getNormalizedYear(),
                                BaseCalendar.JANUARY, 1);
            ...
            case YEAR: {
                CalendarDate jd = jcal.getCalendarDate(jc.getTimeInMillis(),
                        getZone());
                CalendarDate d;
                int eraIndex = getEraIndex(date);
                if (eraIndex == eras.length - 1) {
                    ...
                    d = jcal.getCalendarDate(eras[eraIndex + 1].getSince(
                            getZone()) - 1, getZone());
                    value = d.getYear();
    ...
    /**
     * Returns the millisecond offset from the beginning of the year. In the
     * year for Long.MIN_VALUE, it's a pseudo value beyond the limit. The given
     * CalendarDate object must have been normalized before calling this method.
     */
    ...
    /**
     * The fixed date corresponding to jdate. If the value is Long.MIN_VALUE,
     * the fixed date value is unknown.
     */
    ...
    private int computeFields(int fieldMask, int tzMask) {
        ...
        if ((fieldMask & (DAY_OF_YEAR_MASK | WEEK_OF_YEAR_MASK
            ...
            boolean transitionYear = isTransitionYear(jdate
                    .getNormalizedYear());
            int dayOfYear;
            long fixedDateJan1;
            if (transitionYear) {
                ...
                CalendarDate dx = jcal.getCalendarDate(Long.MIN_VALUE,
                        getZone());
            ...
            long fixedDateMonth1 = transitionYear ? getFixedDateMonth1(jdate,
                    fixedDate) : fixedDate - dayOfMonth + 1;
            ...
            if (weekOfYear == 0) {
                ...
                if (!(transitionYear || isTransitionYear(d
                        .getNormalizedYear()))) {
                    ...
                    if (jdate.getYear() == 1) {
                        ...
                        if (era > HEISEI) {
                            ...
                            if (normalizedYear == pd.getYear()) {
                                d.setMonth(pd.getMonth()).setDayOfMonth(pd
                                        .getDayOfMonth());
                            }
                        } else {
    ...
    private int getWeekNumber(long fixedDay1, long fixedDate) {
        ...
        long fixedDay1st = LocalGregorianCalendar.getDayOfWeekDateOnOrBefore(
                fixedDay1 + 6, getFirstDayOfWeek());
    ...
    private long getFixedDate(int era, int year, int fieldMask) {
        ...
        LocalGregorianCalendar.Date date = jcal.newCalendarDate(
                TimeZone.NO_TIMEZONE);
                ...
                if (isFieldSet(fieldMask, WEEK_OF_MONTH)) {
                    ...
                    if (dowim >= 0) {
                        fixedDate = LocalGregorianCalendar
                                .getDayOfWeekDateOnOrBefore(fixedDate + (7
                                        * dowim) - 1, dayOfWeek);
                    } else {
                        ...
                        // date.
                        fixedDate = LocalGregorianCalendar
                                .getDayOfWeekDateOnOrBefore(fixedDate + lastDate
                                        - 1, dayOfWeek);
                    }
                }
            }
     ...
     *                  calculated.
     *                  The date has to be in the cut-over year.
     * @param fixedDate
     *                  the fixed date representation of the date
     */
    private long getFixedDateJan1(LocalGregorianCalendar.Date date,
            long fixedDate) {
        Era era = date.getEra();
     ...
     *                  calculated.
     *                  The date must be in the era transition year.
     * @param fixedDate
     *                  the fixed date representation of the date
     */
    ...
    /**
     * Returns a LocalGregorianCalendar.Date produced from the specified fixed
     * date.
     *
     * @param fd
     *           the fixed date
    ...
    private static LocalGregorianCalendar.Date getCalendarDate(long fd) {
        LocalGregorianCalendar.Date d = jcal.newCalendarDate(
                TimeZone.NO_TIMEZONE);
        jcal.getCalendarDateFromFixedDate(d, fd);
        return d;
    }
    ...
    /**
     * Returns the length of the specified month in the specified Gregorian
    ...
    private int monthLength(int month, int gregorianYear) {
        return CalendarUtils.isGregorianLeapYear(gregorianYear)
                ? GregorianCalendar.LEAP_MONTH_LENGTH[month]
    ...
    private int monthLength(int month) {
        ...
        return jdate.isLeapYear() ? GregorianCalendar.LEAP_MONTH_LENGTH[month]
                : GregorianCalendar.MONTH_LENGTH[month];
    }
    ...
    /**
     * Returns the index to the new era if the given date is in a transition
     * month. For example, if the give date is Heisei 1 (1989) January 20, then
     * the era index for Heisei is returned. Likewise, if the given date is
     * Showa 64 (1989) January 3, then the era index for Heisei is returned. If
     * the given date is not in any transition month, then -1 is returned.
     */
    ...
    private void pinDayOfMonth(LocalGregorianCalendar.Date date) {
        ...
        if (year != getMinimum(YEAR)) {
            ...
            LocalGregorianCalendar.Date d = jcal.getCalendarDate(Long.MIN_VALUE,
                    getZone());
            LocalGregorianCalendar.Date realDate = jcal.getCalendarDate(time,
                    getZone());
            ...
            if (realDate.getDayOfMonth() == d.getDayOfMonth() && tod < d
                    .getTimeOfDay()) {
     ...
     * ERA is the current era, but a zero (unset) ERA means before Meiji.
     */
