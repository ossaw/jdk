 ...
 * {@code 'C'}, {@code 'X'}, {@code 'E'}, {@code 'G'}, {@code 'A'}, and
 * {@code 'T'}) are the same as those for the corresponding lower-case
 * conversion characters except that the result is converted to upper case
 * according to the rules of the prevailing {@link java.util.Locale Locale}. The
 ...
 * <td valign="top">floating point
 * <td>The result is formatted as a hexadecimal floating-point number with a
 ...
 * completely identical to those defined by GNU {@code date} and POSIX
 ...
 * numeric time zone offset from GMT, e.g. {@code -0800}. This value will be
 * adjusted as necessary for Daylight Saving Time. For {@code long},
 ...
 * be adjusted as necessary for Daylight Saving Time. For {@code long},
 * {@link Long}, and {@link Date} the time zone used is the
 * {@linkplain TimeZone#getDefault() default time zone} for this instance of the
 ...
 * lower-case conversion characters except that the result is converted to upper
 * case according to the rules of the prevailing {@link java.util.Locale Locale}
 * . The result is equivalent to the following invocation of
 * {@link String#toUpperCase()}
 *
 * <pre>
 * out.toUpperCase()
 ...
 * <td>Left justifies the output. Spaces (<tt>'&#92;u0020'</tt>) will be added
 * at the end of the converted value as required to fill the minimum width of
 ...
 * width then the output will be padded by <tt>'&nbsp;&nbsp;'</tt> (
 * <tt>'&#92;u0020'</tt>) until the total number of characters equals the width.
 ...
 * <td>Formats the argument as a Unicode character as described in
 ...
 * {@linkplain java.text.DecimalFormatSymbols#getGroupingSeparator grouping
 * separator} is inserted by scanning the integer part of the string from least
 ...
 * the type as returned by the static {@code SIZE} field in the
 ...
 * <td>The upper-case variant of {@code 'x'}. The entire string representing the
 ...
 * <td>Requires the output to be padded with leading
 * {@linkplain java.text.DecimalFormatSymbols#getZeroDigit zeros} to the minimum
 ...
 * <td>Requires the output to include the locale-specific
 ...
 * radix indicator, and parentheses. If the length of the converted value is
 * less than the width then the output will be padded by spaces (
 ...
 * &lt;= <i>a</i> &lt; 10. The magnitude is then represented as the integer part
 * of <i>a</i>, as a single decimal digit, followed by the decimal separator
 * followed by decimal digits representing the fractional part of <i>a</i>,
 * followed by the exponent symbol {@code 'e'} (<tt>'&#92;u0065'</tt>), followed
 ...
 * the default value is {@code 6}. If the precision is less than the number of
 * digits which would appear after the decimal point in the string returned by
 * {@link Float#toString(float)} or {@link Double#toString(double)}
 * respectively, then the value will be rounded using the
 * {@linkplain java.math.BigDecimal#ROUND_HALF_UP round half up algorithm}.
 ...
 * the hexadecimal representation of the rest of the significand as a fraction.
 * The exponent is represented by {@code 'p'} (<tt>'&#92;u0070'</tt>) followed
 * by a decimal string of the unbiased exponent as if produced by invoking
 ...
 * followed by the hexadecimal representation of the rest of the significand as
 * a fraction, and the exponent represented by {@code 'p-1022'}. If the
 * precision is in the interval [1,&nbsp;12], the subnormal value is normalized
 * such that it begins with the characters {@code '0x1.'}, rounded to the number
 ...
 * <td>The upper-case variant of {@code 'a'}. The entire string representing the
 * number will be converted to upper case including the {@code 'x'} (
 ...
 * written to the output. This includes any signs, digits, grouping separators,
 * decimal separators, exponential symbol, radix indicator, parentheses, and
 * strings representing infinity and NaN as applicable. If the length of the
 ...
 * <i>a</i> is equal to the precision. If the precision is not specified then
 * the default value is {@code 6}. If the precision is less than the number of
 * digits to the right of the decimal point then the value will be rounded using
 ...
 * output will be padded by spaces (<tt>'&#92;u0020'</tt>) until the total
...
public final class Formatter implements Closeable, Flushable {
	...
	private class FormatSpecifier implements FormatString {
		...
		private void checkDateTime() {
			...
			checkBadFlags(Flags.ALTERNATE, Flags.PLUS, Flags.LEADING_SPACE, Flags.ZERO_PAD,
		...
		// value > 0
		private void print(StringBuilder sb, BigDecimal value, Locale l, Flags f, char c,
			...
			if (c == Conversion.SCIENTIFIC) {
				...
				if ((value.equals(BigDecimal.ZERO)) || ((value.compareTo(tenToTheNegFour) != -1)
		...
		private Appendable print(StringBuilder sb, TemporalAccessor t, char c, Locale l)
				throws IOException {
			if (sb == null)
				sb = new StringBuilder();
			try {
				...
				case DateTime.ZONE: { // 'Z' (symbol)
					...
					if (!(zid instanceof ZoneOffset)
						...
						sb.append(TimeZone.getTimeZone(zid.getId()).getDisplayName(
								zid.getRules().isDaylightSavings(instant), TimeZone.SHORT,
								(l == null) ? Locale.US : l));
						break;
					}
					sb.append(zid.getId());
	...
	private static class Conversion {
		...
		static boolean isValid(char c) {
			return (isGeneral(c) || isInteger(c) || isFloat(c) || isText(c) || c == 't'
					|| isCharacter(c));
		}
