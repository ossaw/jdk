 ...
 *
 * <p>
 * <a name="modify_default_behavior">For the {@code getBundle} factory</a>
 * methods that take no {@link Control} instance, their
 * <a href="#default_behavior"> default behavior</a> of resource bundle loading
 * can be modified with <em>installed</em> {@link ResourceBundleControlProvider}
 * implementations. Any installed providers are detected at the
 * {@code ResourceBundle} class loading time. If any of the providers provides a
 * {@link Control} for the given base name, that {@link Control} will be used
 * instead of the default {@link Control}. If there is more than one service
 * provider installed for supporting the same base name, the first one returned
 * from {@link ServiceLoader} will be used.
 *
...
public abstract class ResourceBundle {
	 ...
	 * class loader) to either a resource bundle or NONEXISTENT_BUNDLE wrapped
	 * by a BundleReference.
	 *
	 * The cache is a ConcurrentMap, allowing the cache to be searched
	 * concurrently by multiple threads. This will also allow the cache keys to
	 * be reclaimed along with the ClassLoaders they reference.
	 *
	...
	/**
	 * Returns the base name of this bundle, if known, or {@code null} if
	 * unknown.
	 *
	 * If not null, then this is the value of the {@code baseName} parameter
	 * that was passed to the {@code ResourceBundle.getBundle(...)} method when
	 * the resource bundle was loaded.
	 *
	 * @return The base name of the resource bundle, as provided to and expected
	 *         by the {@code ResourceBundle.getBundle(...)} methods.
	 *
	 * @see #getBundle(java.lang.String, java.util.Locale,
	...
	/**
	 * The back link to the cache key. null if this bundle isn't in the cache
	 * (yet) or has expired.
	 */
	 ...
	 *                                     if the object found for the given key
	 *                                     is not a string
		...
		if (obj == null) {
			...
			if (obj == null) {
				throw new MissingResourceException(
						"Can't find resource for bundle " + this.getClass()
								.getName() + ", key " + key, this.getClass()
										.getName(), key);
			}
		}
		return obj;
	...
	/*
	 * Automatic determination of the ClassLoader to be used to load resources
	...
	private static class RBClassLoader extends ClassLoader {
		...
		private static final ClassLoader loader = ClassLoader
	 ...
	 * locale, and the class loader to determine if the resource is a match to
	 * the requested one. The loader may be null, but the base name and the
	 * locale must have a non-null value.
	 */
		...
		public boolean equals(Object other) {
			...
			try {
				...
				return (otherEntry.loaderRef != null)
						...
						// treat it as unequal
						&& (loader != null) && (loader == otherEntry.loaderRef
								.get());
	...
	private static class LoaderReference extends WeakReference<ClassLoader>
		...
		LoaderReference(ClassLoader referent, ReferenceQueue<Object> q,
				CacheKey key) {
			super(referent, q);
			cacheKey = key;
		}
	...
	public static final ResourceBundle getBundle(String baseName) {
		return getBundleImpl(baseName, Locale.getDefault(), getLoader(Reflection
				.getCallerClass()), getDefaultControl(baseName));
	}

	/**
	 * Returns a resource bundle using the specified base name, the default
	 * locale and the specified control. Calling this method is equivalent to
	 * calling
	 * 
	 * <pre>
	 ...
	 * privileges of <code>ResourceBundle</code>. See
	 * {@link #getBundle(String, Locale, ClassLoader, Control) getBundle} for
	 * the complete description of the resource bundle loading process with a
	...
	public static final ResourceBundle getBundle(String baseName,
			Locale locale) {
		return getBundleImpl(baseName, locale, getLoader(Reflection
				.getCallerClass()), getDefaultControl(baseName));
	}

	/**
	 * Returns a resource bundle using the specified base name, target locale
	 * and control, and the caller's class loader. Calling this method is
	 ...
	 *                                     of the <code>locales</code> can be
	 *                                     found.
	 * @exception IllegalArgumentException
	 *                                     if the given <code>control</code>
	 *                                     doesn't perform properly
	...
	public static final ResourceBundle getBundle(String baseName,
		...
		return getBundleImpl(baseName, targetLocale, getLoader(Reflection
				.getCallerClass()), control);
	}
	 ...
	 *
	 * <p>
	 * <a name="default_behavior">The following describes the default
	 * behavior</a>.
	 *
	 * <p>
	 ...
	 * Locale.getDefault}) to generate a sequence of <a name="candidates">
	 * <em>candidate bundle names</em></a>. If the specified locale's language,
	 * script, country, and variant are all empty strings, then the base name is
	 * the only candidate bundle name. Otherwise, a list of candidate locales is
	 * generated from the attribute values of the specified locale (language,
	 * script, country and variant) and appended to the base name. Typically,
	 ...
	 * omitted, along with the underscore. For example, if country is an empty
	 * string, the second and the fifth candidate bundle names above would be
	 * omitted. Also, if script is an empty string, the candidate names
	 ...
	 *
	 * <p>
	 * <code>getBundle</code> then iterates over the candidate bundle names to
	 * find the first one for which it can <em>instantiate</em> an actual
	 ...
	 *
	 * <ul>
	 * <li>First, it attempts to load a class using the generated class name. If
	 * such a class can be found and loaded using the specified class loader, is
	 * assignment compatible with ResourceBundle, is accessible from
	 ...
	 * {@link PropertyResourceBundle} instance from its contents. If successful,
	 * this instance becomes the <em>result resource bundle</em>.
	 * </ul>
	 *
	 * <p>
	 * This continues until a result resource bundle is instantiated or the list
	 * of candidate bundle names is exhausted. If no matching resource bundle is
	 * found, the default control's {@link Control#getFallbackLocale
	 ...
	 * <em>parent chain</em> is instantiated</a>. If the result bundle already
	 * has a parent (perhaps because it was returned from a cache) the chain is
	 * complete.
	 *
	 * <p>
	 * Otherwise, <code>getBundle</code> examines the remainder of the candidate
	 * locale list that was used during the pass that generated the result
	 * resource bundle. (As before, candidate bundle names where the final
	 * component is an empty string are omitted.) When it comes to the end of
	 * the candidate list, it tries the plain bundle name. With each of the
	 * candidate bundle names it attempts to instantiate a resource bundle
	 * (first looking for a class and then a properties file, as described
	 * above).
	 *
	 * <p>
	 * Whenever it succeeds, it calls the previously instantiated resource
	 * bundle's {@link #setParent(java.util.ResourceBundle) setParent} method
	 * with the new resource bundle. This continues until the list of names is
	 * exhausted or the current bundle already has a non-null parent.
	 *
	 * <p>
	 * Once the parent chain is complete, the bundle is returned.
	 *
	 * <p>
	 * <b>Note:</b> <code>getBundle</code> caches instantiated resource bundles
	 * and might return the same resource bundle instance multiple times.
	 *
	 * <p>
	 * <b>Note:</b>The <code>baseName</code> argument should be a fully
	 ...
	 *
	 * <p>
	 * Calling <code>getBundle</code> with the locale arguments below will
	 * instantiate resource bundles as follows:
	 *
	 * <table summary="getBundle() locale to resource bundle mapping">
	 * <tr>
	 * <td>Locale("fr", "CH")</td>
	 * <td>MyResources_fr_CH.class, parent MyResources_fr.properties, parent
	 * MyResources.class</td>
	 * </tr>
	 * <tr>
	 * <td>Locale("fr", "FR")</td>
	 ...
	 * by the MyResources_fr_CH.class. Likewise, MyResources.properties is also
	 * hidden by MyResources.class.
	 *
	 * @param baseName
	 *                 the base name of the resource bundle, a fully qualified
	 *                 class
	...
	public static ResourceBundle getBundle(String baseName, Locale locale,
		...
		return getBundleImpl(baseName, locale, loader, getDefaultControl(
				baseName));
	}

	/**
	 * Returns a resource bundle using the specified base name, target locale,
	 * class loader and control. Unlike the
	 * {@linkplain #getBundle(String, Locale, ClassLoader)
	 * <code>getBundle</code> factory methods with no <code>control</code>
	 * argument}, the given <code>control</code> specifies how to locate and
	 * instantiate resource bundles. Conceptually, the bundle loading process
	 * with the given <code>control</code> is performed in the following steps.
	 *
	 * <ol>
	 ...
	 * <code>loader</code>. If the requested resource bundle instance is found
	 * in the cache and the time-to-live periods of the instance and all of its
	 * parent instances have not expired, the instance is returned to the
	 * caller. Otherwise, this factory method proceeds with the loading process
	 * below.</li>
	 *
	 * <li>The {@link ResourceBundle.Control#getFormats(String)
	 * control.getFormats} method is called to get resource bundle formats to
	 * produce bundle or resource names. The strings <code>"java.class"</code>
	 * and <code>"java.properties"</code> designate class-based and
	 * {@linkplain PropertyResourceBundle property}-based resource bundles,
	 * respectively. Other strings starting with <code>"java."</code> are
	 * reserved for future extensions and must not be used for
	 ...
	 * control.getCandidateLocales} method is called with the target locale to
	 * get a list of <em>candidate <code>Locale</code>s</em> for which resource
	 * bundles are searched.</li>
	 *
	 * <li>
	 * The
	 ...
	 * <code>ResourceBundle</code> for the base bundle name, a candidate locale,
	 * and a format. (Refer to the note on the cache lookup below.) This step is
	 * iterated over all combinations of the candidate locales and formats until
	 * the <code>newBundle</code> method returns a <code>ResourceBundle</code>
	 * instance or the iteration has used up all the combinations. For example,
	 * if the candidate locales are <code>Locale("de", "DE")</code>,
	 * <code>Locale("de")</code> and <code>Locale("")</code> and the formats are
	 * <code>"java.class"</code> and <code>"java.properties"</code>, then the
	 * following is the sequence of locale-format combinations to be used to
	 * call <code>control.newBundle</code>.
	 *
	 * <table style=
	 * "width: 50%; text-align: left; margin-left: 40px;" border="0" cellpadding
	 * ="2" cellspacing="2" summary="locale-format combinations for newBundle">
	 * <tbody>
	 * <tr>
	 * <td style=
	 * "vertical-align: top; text-align: left; font-weight: bold; width: 50%;">
	 * <code>Locale</code><br>
	 * </td>
	 * <td style=
	 * "vertical-align: top; text-align: left; font-weight: bold; width: 50%;">
	 ...
	 * <tr>
	 * <td style="vertical-align: top; width: 50%;">
	 * <code>Locale("de", "DE")</code></td>
	 * <td style="vertical-align: top; width: 50%;"><code>java.properties</code>
	 * <br>
	 * </td>
	 * </tr>
	 * <tr>
	 * <td style="vertical-align: top; width: 50%;"><code>Locale("de")</code>
	 * </td>
	 ...
	 * </li>
	 *
	 * <li>If the previous step has found no resource bundle, proceed to Step 6.
	 * If a bundle has been found that is a base bundle (a bundle for
	 ...
	 * locales other than Locale(""), put the bundle on hold and proceed to Step
	 * 6. If a bundle has been found that is not a base bundle, proceed to Step
	 * 7.</li>
	 *
	 ...
	 * (alternative to the current target locale) to try further finding a
	 * resource bundle. If the method returns a non-null locale, it becomes the
	 * next target locale and the loading process starts over from Step 3.
	 * Otherwise, if a base bundle was found and put on hold in a previous Step
	 * 5, it is returned to the caller now. Otherwise, a
	 ...
	 * <a href="./ResourceBundle.html#parent_chain">parent chain</a> is
	 * instantiated based on the list of candidate locales from which it was
	 * found. Finally, the bundle is returned to the caller.</li>
	 * </ol>
	 *
	 * <p>
	 * During the resource bundle loading process above, this factory method
	 * looks up the cache before calling the
	 * {@link Control#newBundle(String, Locale, String, ClassLoader, boolean)
	 * control.newBundle} method. If the time-to-live period of the resource
	 * bundle found in the cache has expired, the factory method calls the
	 * {@link ResourceBundle.Control#needsReload(String, Locale, String, ClassLoader, ResourceBundle, long)
	 * control.needsReload} method to determine whether the resource bundle
	 * needs to be reloaded. If reloading is required, the factory method calls
	 ...
	 * method puts a dummy resource bundle in the cache as a mark of nonexistent
	 * resource bundles in order to avoid lookup overhead for subsequent
	 * requests. Such dummy resource bundles are under the same expiration
	 ...
	 * <code>foo/bar/Messages.properties</code>, which is put on hold because
	 * it's the base bundle. <code>getBundle</code> calls
	 * {@link Control#getFallbackLocale(String, Locale)
	 * control.getFallbackLocale("foo.bar.Messages", Locale.ITALY)} which
	 * returns <code>Locale.FRENCH</code>. Next, <code>getBundle</code> tries
	 ...
	 *
	 * <p>
	 * <code>getBundle</code> finds <code>foo/bar/Messages_fr.properties</code>
	 * and creates a <code>ResourceBundle</code> instance. Then,
	 * <code>getBundle</code> sets up its parent chain from the list of the
	 * candidate locales. Only <code>foo/bar/Messages.properties</code> is found
	 * in the list and <code>getBundle</code> creates a
	 * <code>ResourceBundle</code> instance that becomes the parent of the
	 * instance for <code>foo/bar/Messages_fr.properties</code>.
	 *
	 * @param baseName
	 *                     the base name of the resource bundle, a fully
	 *                     qualified class
	...
	private static ResourceBundle getBundleImpl(String baseName, Locale locale,
		...
		for (Locale targetLocale = locale; targetLocale != null; targetLocale = control
			...
			if (!isKnownControl && !checkList(candidateLocales)) {
				throw new IllegalArgumentException(
						"Invalid Control: getCandidateLocales");
			}
		...
		if (bundle == null) {
			if (baseBundle == null) {
				throwMissingResourceException(baseName, locale, cacheKey
						.getCause());
			}
			bundle = baseBundle;
		}
	...
	/**
	 * Checks if the given <code>List</code> is not null, not empty, not having
	 * null in its elements.
	 */
	...
	private static ResourceBundle findBundle(CacheKey cacheKey,
			List<Locale> candidateLocales, List<String> formats, int index,
			Control control, ResourceBundle baseBundle) {
		...
		if (bundle != NONEXISTENT_BUNDLE) {
			...
			try {
				...
				// Put NONEXISTENT_BUNDLE in the cache as a mark that there's no
				// bundle
	...
	private static ResourceBundle loadBundle(CacheKey cacheKey,
			List<String> formats, Control control, boolean reload) {
		...
		for (int i = 0; i < size; i++) {
			...
			try {
				bundle = control.newBundle(cacheKey.getName(), targetLocale,
						format, cacheKey.getLoader(), reload);
	...
	private static void throwMissingResourceException(String baseName,
		...
		throw new MissingResourceException("Can't find bundle for base name "
				+ baseName + ", locale " + locale, baseName + "_" + locale, // className
				"", // key
				cause);
	}

	/**
	 * Finds a bundle in the cache. Any expired bundles are marked as `expired'
	 * and removed from the cache upon return.
	 *
	 * @param cacheKey
	 ...
	 *         true upon return if the bundle in the cache has expired.
	 */
					...
					synchronized (bundle) {
						...
						if (!bundle.expired && expirationTime >= 0
								&& expirationTime <= System
										.currentTimeMillis()) {
							try {
								bundle.expired = control.needsReload(key
										.getName(), key.getLocale(), key
												.getFormat(), key.getLoader(),
										bundle, key.loadTime);
							} catch (Exception e) {
								cacheKey.setCause(e);
							}
	 ...
	 * @return the ResourceBundle for the cacheKey; if someone has put the
	 *         bundle before this call, the one found in the cache is returned.
	 */
	private static ResourceBundle putBundleInCache(CacheKey cacheKey,
			ResourceBundle bundle, Control control) {
		setExpirationTime(cacheKey, control);
		if (cacheKey.expirationTime != Control.TTL_DONT_CACHE) {
			CacheKey key = (CacheKey) cacheKey.clone();
			BundleReference bundleRef = new BundleReference(bundle,
					referenceQueue, key);
			bundle.cacheKey = key;
	...
	private static void setExpirationTime(CacheKey cacheKey, Control control) {
		long ttl = control.getTimeToLive(cacheKey.getName(), cacheKey
				.getLocale());
		if (ttl >= 0) {
	...
	/**
	 * Removes all resource bundles from the cache that have been loaded using
	 ...
	 * @return <code>true</code> if the given <code>key</code> is contained in
	 ...
	 * <code>null</code>. Once the <code>Set</code> has been created, the value
	 * is kept in this <code>ResourceBundle</code> in order to avoid producing
	 * the same <code>Set</code> in subsequent calls. Subclasses can override
	 * this method for faster handling.
	 *
	 ...
	 * ResourceBundle.getBundle} factory methods during the bundle loading
	 * process. In other words, a <code>ResourceBundle.Control</code>
	 * collaborates with the factory methods for loading resource bundles. The
	 * default implementation of the callback methods provides the information
	 * necessary for the factory methods to perform the
	 * <a href="./ResourceBundle.html#default_behavior">default behavior</a>.
	 *
	 * <p>
	 * In addition to the callback methods, the
	 * {@link #toBundleName(String, Locale) toBundleName} and
	 * {@link #toResourceName(String, String) toResourceName} methods are
	 * defined primarily for convenience in implementing the callback methods.
	 * However, the <code>toBundleName</code> method could be overridden to
	 * provide different conventions in the organization and packaging of
	 * localized resources. The <code>toResourceName</code> method is
	 * <code>final</code> to avoid use of wrong resource and class name
	 * separators.
	 *
	 * <p>
	 ...
	 * bundle. Otherwise, the <code>ResourceBundle.getBundle</code> methods may
	 * return unintended bundles. For example, if only <code>"java.class"</code>
	 * is returned by the <code>getFormats</code> method for the first call to
	 * <code>ResourceBundle.getBundle</code> and only
	 * <code>"java.properties"</code> for the second call, then the second call
	 * will return the class-based one that has been cached during the first
	 * call.
	 *
	 * <p>
	 * A <code>ResourceBundle.Control</code> instance must be thread-safe if
	 * it's simultaneously used by multiple threads.
	 * <code>ResourceBundle.getBundle</code> does not synchronize to call the
	 * <code>ResourceBundle.Control</code> methods. The default implementations
	 * of the methods are thread-safe.
	 *
	 * <p>
	 ...
	 * loading process. The following are examples of changing the default
	 ...
	 *
	 * <p>
	 * The following code lets <code>ResourceBundle.getBundle</code> look up
	 * only properties-based resources.
	 *
	 * <pre>
	 * import java.util.*;
	 ...
	 * <a href="./ResourceBundle.html#default_behavior_example">example</a> in
	...
	public static class Control {
		 ...
		 * <code>"java.class"</code> and <code>"java.properties"</code>, in this
		 * order. This <code>List</code> is
		 * {@linkplain Collections#unmodifiableList(List) unmodifiable}.
		 *
		 * @see #getFormats(String)
		 */
		public static final List<String> FORMAT_DEFAULT = Collections
				.unmodifiableList(Arrays.asList("java.class",
						"java.properties"));

		/**
		 * The class-only format <code>List</code> containing
		 * <code>"java.class"</code>. This <code>List</code> is
		 * {@linkplain Collections#unmodifiableList(List) unmodifiable}.
		 *
		 * @see #getFormats(String)
		 */
		...
		/**
		 * The time-to-live constant for disabling the expiration control for
		 * loaded resource bundle instances in the cache.
		 *
		 * @see #getTimeToLive(String, Locale)
		 */
		 ...
		 * <code>formats</code>. The <code>formats</code> must be equal to one
		 * of {@link Control#FORMAT_PROPERTIES}, {@link Control#FORMAT_CLASS} or
		 * {@link Control#FORMAT_DEFAULT}. <code>ResourceBundle.Control</code>
		 * instances returned by this method are singletons and thread-safe.
		 *
		 * <p>
		 * Specifying {@link Control#FORMAT_DEFAULT} is equivalent to
		 * instantiating the <code>ResourceBundle.Control</code> class, except
		 * that this method returns a singleton.
		 *
		 * @param formats
		 ...
		 * Returns a <code>ResourceBundle.Control</code> in which the
		 * {@link #getFormats(String) getFormats} method returns the specified
		 * <code>formats</code> and the
		 * {@link Control#getFallbackLocale(String, Locale) getFallbackLocale}
		 * method returns <code>null</code>. The <code>formats</code> must be
		 * equal to one of {@link Control#FORMAT_PROPERTIES},
		 * {@link Control#FORMAT_CLASS} or {@link Control#FORMAT_DEFAULT}.
		 * <code>ResourceBundle.Control</code> instances returned by this method
		 * are singletons and thread-safe.
		 *
		 * @param formats
		 *                the formats to be returned by the
		 *                <code>ResourceBundle.Control.getFormats</code> method
		 ...
		 * order specified by the list. The list returned by this method must
		 * have at least one <code>String</code>. The predefined formats are
		 * <code>"java.class"</code> for class-based resource bundles and
		 * <code>"java.properties"</code> for {@linkplain PropertyResourceBundle
		 * properties-based} ones. Strings starting with <code>"java."</code>
		 * are reserved for future extensions and must not be used by
		 ...
		 * be mutated after it has been returned by <code>getFormats</code>.
		 *
		 * <p>
		 * The default implementation returns {@link #FORMAT_DEFAULT} so that
		 * the <code>ResourceBundle.getBundle</code> factory method looks up
		 * first class-based resource bundles, then properties-based ones.
		 *
		 * @param baseName
		 *                 the base name of the resource bundle, a fully
		 *                 qualified
		 ...
		 * method is called by the <code>ResourceBundle.getBundle</code> factory
		 * method each time the factory method tries finding a resource bundle
		 * for a target <code>Locale</code>.
		 *
		 * <p>
		 ...
		 * corresponding resource bundles for the candidate locales exist and
		 * their parents are not defined by loaded resource bundles themselves.
		 * The last element of the list must be a {@linkplain Locale#ROOT root
		 * locale} if it is desired to have the base bundle as the terminal of
		 * the parent chain.
		 *
		 * <p>
		 * If the given locale is equal to <code>Locale.ROOT</code> (the root
		 * locale), a <code>List</code> containing only the root
		 * <code>Locale</code> must be returned. In this case, the
		 * <code>ResourceBundle.getBundle</code> factory method loads only the
		 ...
		 * full variant value in the original list. For example, if the the
		 * variant consists of two subtags <em>V1</em> and <em>V2</em>:
		 *
		 * <ul>
		 ...
		 * </li>
		 *
		 * <li>Special cases for Chinese. When an input <code>Locale</code> has
		 * the language "zh" (Chinese) and an empty script value, either "Hans"
		 * (Simplified) or "Hant" (Traditional) might be supplied, depending on
		 * the country. When the country is "CN" (China) or "SG" (Singapore),
		 * "Hans" is supplied. When the country is "HK" (Hong Kong SAR China),
		 * "MO" (Macau SAR China), or "TW" (Taiwan), "Hant" is supplied. For all
		 * other countries or when the country is empty, no script is supplied.
		 ...
		 * </ul>
		 *
		 * If the locale is exactly <code>Locale("no", "NO", "NY")</code>, it is
		 * first converted to <code>Locale("nn", "NO")</code> and then the above
		 ...
		 * implementations may modify before returning it to the caller.
		 * However, a subclass must not modify it after it has been returned by
		 * <code>getCandidateLocales</code>.
		 *
		 * <p>
		 * For example, if the given <code>baseName</code> is "Messages" and the
		 * given <code>locale</code> is
		...
		private static class CandidateListCache extends
				LocaleObjectCache<BaseLocale, List<Locale>> {
			protected List<Locale> createObject(BaseLocale base) {
				...
				if (language.equals("nb") || isNorwegianBokmal) {
					List<Locale> tmpList = getDefaultList("nb", script, region,
							variant);
					// Insert a locale replacing "nb" with "no" for every list
					// entry
					...
					for (Locale l : tmpList) {
						...
						bokmalList.add(Locale.getInstance("no", l.getScript(), l
								.getCountry(), l.getVariant(), null));
					}
					return bokmalList;
				} else if (language.equals("nn") || isNorwegianNynorsk) {
					// Insert no_NO_NY, no_NO, no after nn
					List<Locale> nynorskList = getDefaultList("nn", script,
							region, variant);
				...
				// Special handling for Chinese
					...
					if (script.length() == 0 && region.length() > 0) {
						// Supply script for users who want to use
						// zh_Hans/zh_Hant
						...
						// Supply region(country) for users who still package
						// Chinese
			...
			private static List<Locale> getDefaultList(String language,
					String script, String region, String variant) {
				...
				if (variants != null) {
					for (String v : variants) {
						list.add(Locale.getInstance(language, script, region, v,
								null));
					}
				}
				if (region.length() > 0) {
					list.add(Locale.getInstance(language, script, region, "",
							null));
				}
				if (script.length() > 0) {
					list.add(Locale.getInstance(language, script, "", "",
							null));
					...
					if (variants != null) {
						for (String v : variants) {
							list.add(Locale.getInstance(language, "", region, v,
									null));
						}
					}
					if (region.length() > 0) {
						list.add(Locale.getInstance(language, "", region, "",
								null));
					}
				}
		 ...
		 * <code>ResourceBundle.getBundle</code> factory method. This method is
		 * called from the factory method every time when no resulting resource
		 * bundle has been found for <code>baseName</code> and
		 ...
		 * {@linkplain Locale#getDefault() default <code>Locale</code>} if the
		 * given <code>locale</code> isn't the default one. Otherwise,
		 * <code>null</code> is returned.
		 *
		 * @param baseName
		 *                 the base name of the resource bundle, a fully
		 *                 qualified
		 *                 class name for which
		 *                 <code>ResourceBundle.getBundle</code>
		 *                 has been unable to find any resource bundles (except
		 *                 for
		 *                 the base bundle)
		 * @param locale
		 *                 the <code>Locale</code> for which
		 *                 <code>ResourceBundle.getBundle</code> has been unable
		 *                 to
		 *                 find any resource bundles (except for the base
		 *                 bundle)
		 ...
		 * format and locale, using the given class loader if necessary. This
		 * method returns <code>null</code> if there is no resource bundle
		 * available for the given parameters. If a resource bundle can't be
		 * instantiated due to an unexpected error, the error must be reported
		 * by throwing an <code>Error</code> or <code>Exception</code> rather
		 * than simply returning <code>null</code>.
		 *
		 * <p>
		 * If the <code>reload</code> flag is <code>true</code>, it indicates
		 * that this method is being called because the previously loaded
		 * resource bundle has expired.
		 *
		 * <p>
		 * The default implementation instantiates a <code>ResourceBundle</code>
		 * as follows.
		 *
		 * <ul>
		 *
		 * <li>The bundle name is obtained by calling
		 * {@link #toBundleName(String, Locale) toBundleName(baseName, locale)}.
		 * </li>
		 *
		 ...
		 * {@link Class#newInstance()}. Note that the <code>reload</code> flag
		 * is ignored for loading class-based resource bundles in this default
		 * implementation.</li>
		 *
		 * <li>If <code>format</code> is <code>"java.properties"</code>,
		 * {@link #toResourceName(String, String) toResourceName(bundlename,
		 * "properties")} is called to get the resource name. If
		 * <code>reload</code> is <code>true</code>,
		 * {@link ClassLoader#getResource(String) load.getResource} is called to
		 * get a {@link URL} for creating a {@link URLConnection}. This
		 * <code>URLConnection</code> is used to
		 * {@linkplain URLConnection#setUseCaches(boolean) disable the caches}
		 * of the underlying resource loading layers, and to
		 * {@linkplain URLConnection#getInputStream() get an
		 * <code>InputStream</code>}. Otherwise,
		 ...
		 * @param loader
		 *                 the <code>ClassLoader</code> to use to load the
		 *                 bundle
		 * @param reload
		 ...
		 *                 <code>false</code> otherwise
		 * @return the resource bundle instance, or <code>null</code> if none
		 *         could be found.
		 * @exception NullPointerException
		 ...
		 *                                        if the instantiation of a
		 *                                        class fails for some other
		 *                                        reason.
		 ...
		 *                                        present and creation of new
		 *                                        instances is denied. See
		 *                                        {@link Class#newInstance()}
		 *                                        for details.
		 * @exception IOException
		 *                                        if an error occurred when
		 *                                        reading resources using any
		 *                                        I/O operations
		 */
		...
		/**
		 * Returns the time-to-live (TTL) value for resource bundles that are
		 * loaded under this <code>ResourceBundle.Control</code>. Positive
		 * time-to-live values specify the number of milliseconds a bundle can
		 * remain in the cache without being validated against the source data
		 * from which it was constructed. The value 0 indicates that a bundle
		 * must be validated each time it is retrieved from the cache.
		 * {@link #TTL_DONT_CACHE} specifies that loaded resource bundles are
		 * not put in the cache. {@link #TTL_NO_EXPIRATION_CONTROL} specifies
		 * that loaded resource bundles are put in the cache with no expiration
		 * control.
		 *
		 * <p>
		 * The expiration affects only the bundle loading process by the
		 * <code>ResourceBundle.getBundle</code> factory method. That is, if the
		 * factory method finds a resource bundle in the cache that has expired,
		 * the factory method calls the
		 * {@link #needsReload(String, Locale, String, ClassLoader, ResourceBundle, long)
		 * needsReload} method to determine whether the resource bundle needs to
		 * be reloaded. If <code>needsReload</code> returns <code>true</code>,
		 * the cached resource bundle instance is removed from the cache.
		 * Otherwise, the instance stays in the cache, updated with the new TTL
		 * value returned by this method.
		 *
		 * <p>
		 * All cached resource bundles are subject to removal from the cache due
		 * to memory constraints of the runtime environment. Returning a large
		 * positive value doesn't mean to lock loaded resource bundles in the
		 * cache.
		 *
		 * <p>
		 ...
		 *         {@link #TTL_NO_EXPIRATION_CONTROL} to disable the expiration
		...
		/**
		 * Determines if the expired <code>bundle</code> in the cache needs to
		 * be reloaded based on the loading time given by <code>loadTime</code>
		 * or some other criteria. The method returns <code>true</code> if
		 * reloading is required; <code>false</code> otherwise.
		 * <code>loadTime</code> is a millisecond offset since the
		 * <a href="Calendar.html#Epoch"> <code>Calendar</code> Epoch</a>.
		 *
		 * The calling <code>ResourceBundle.getBundle</code> factory method
		 * calls this method on the <code>ResourceBundle.Control</code> instance
		 * used for its current invocation, not on the instance used in the
		 * invocation that originally loaded the resource bundle.
		 *
		 * <p>
		 * The default implementation compares <code>loadTime</code> and the
		 * last modified time of the source data of the resource bundle. If it's
		 * determined that the source data has been modified since
		 * <code>loadTime</code>, <code>true</code> is returned. Otherwise,
		 * <code>false</code> is returned. This implementation assumes that the
		 * given <code>format</code> is the same string as its file suffix if
		 * it's not one of the default formats, <code>"java.class"</code> or
		 ...
		 *                 bundle
		 * @param bundle
		 *                 the resource bundle instance that has been expired in
		 *                 the
		 *                 cache
		 * @param loadTime
		...
		public boolean needsReload(String baseName, Locale locale,
			...
			if (format.equals("java.class") || format.equals(
					"java.properties")) {
				format = format.substring(5);
			}
			boolean result = false;
			try {
				String resourceName = toResourceName0(toBundleName(baseName,
						locale), format);
				if (resourceName == null) {
					return result;
				}
		...
		/**
		 * Converts the given <code>baseName</code> and <code>locale</code> to
		 * the bundle name. This method is called from the default
		 ...
		 * </pre>
		 * 
		 * where <code>language</code>, <code>script</code>,
		 * <code>country</code>, and <code>variant</code> are the language,
		 * script, country, and variant values of <code>locale</code>,
		 * respectively. Final component values that are empty Strings are
		 * omitted along with the preceding '_'. When the script is empty, the
		 * script value is omitted along with the preceding '_'. If all of the
		 ...
		 * locale is <code>Locale("en")</code>, then <code>"baseName_en"</code>
		 * is returned.
		 *
		 * <p>
		 * Overriding this method allows applications to use different
		 * conventions in the organization and packaging of localized resources.
		 *
		 * @param baseName
		 *                 the base name of the resource bundle, a fully
		 *                 qualified
		...
		public String toBundleName(String baseName, Locale locale) {
			...
			if (script != "") {
				if (variant != "") {
					sb.append(language).append('_').append(script).append('_')
							.append(country).append('_').append(variant);
				} else if (country != "") {
					sb.append(language).append('_').append(script).append('_')
							.append(country);
				} else {
		...
		/**
		 * Converts the given <code>bundleName</code> to the form required by
		 * the {@link ClassLoader#getResource ClassLoader.getResource} method by
		 * replacing all occurrences of <code>'.'</code> in
		 * <code>bundleName</code> with <code>'/'</code> and appending a
		 * <code>'.'</code> and the given file <code>suffix</code>. For example,
		 * if <code>bundleName</code> is
	...
	private static class SingleFormatControl extends Control {
		private static final Control PROPERTIES_ONLY = new SingleFormatControl(
				FORMAT_PROPERTIES);

		private static final Control CLASS_ONLY = new SingleFormatControl(
				FORMAT_CLASS);
	...
	private static final class NoFallbackControl extends SingleFormatControl {
		private static final Control NO_FALLBACK = new NoFallbackControl(
				FORMAT_DEFAULT);

		private static final Control PROPERTIES_ONLY_NO_FALLBACK = new NoFallbackControl(
				FORMAT_PROPERTIES);

		private static final Control CLASS_ONLY_NO_FALLBACK = new NoFallbackControl(
				FORMAT_CLASS);
