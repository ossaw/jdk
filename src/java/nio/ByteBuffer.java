 ...
 * <li>
 * <p>
 * Absolute and relative {@link #get() <i>get</i>} and {@link #put(byte)
 * <i>put</i>} methods that read and write single bytes;
 * </p>
 * </li>
 *
 * <li>
 * <p>
 * Relative {@link #get(byte[]) <i>bulk get</i>} methods that transfer
 ...
 * <li>
 * <p>
 * Relative {@link #put(byte[]) <i>bulk put</i>} methods that transfer
 ...
 * <li>
 * <p>
 * Absolute and relative {@link #getChar() <i>get</i>} and {@link #putChar(char)
 * <i>put</i>} methods that read and write values of other primitive types,
 * translating them to and from sequences of bytes in a particular byte order;
 * </p>
 * </li>
 *
 * <li>
 * <p>
 * Methods for creating <i><a href="#views">view buffers</a></i>, which allow a
 * byte buffer to be viewed as a buffer containing values of some other
 * primitive type; and
 * </p>
 * </li>
 *
 ...
 * {@link #slice slicing} a byte buffer.
 * </p>
 * </li>
 *
 * </ul>
 *
 * <p>
 * Byte buffers can be created either by {@link #allocate <i>allocation</i>},
 * which allocates space for the buffer's
 *
 *
 * 
 * content, or by {@link #wrap(byte[]) <i>wrapping</i>} an existing byte array
 * into a buffer.
 *
 *
 *
 ...
 *
 * <p>
 * A byte buffer is either <i>direct</i> or <i>non-direct</i>. Given a direct
 ...
 * {@link #allocateDirect(int) allocateDirect} factory method of this class. The
 ...
 * {@link java.nio.channels.FileChannel#map mapping} a region of a file directly
 * into memory. An implementation of the Java platform may optionally support
 * the creation of direct byte buffers from native code via JNI. If an instance
 * of one of these kinds of buffers refers to an inaccessible region of memory
 * then an attempt to access that region will not change the buffer's content
 * and will cause an unspecified exception to be thrown either at the time of
 * the access or at some later time.
 *
 * <p>
 * Whether a byte buffer is direct or non-direct may be determined by invoking
 * its {@link #isDirect isDirect} method. This method is provided so that
 ...
 * primitive types, except <tt>boolean</tt>. Primitive values are translated to
 * (or from) sequences of bytes according to the buffer's current byte order,
 * which may be retrieved and modified via the {@link #order order} methods.
 * Specific byte orders are represented by instances of the {@link ByteOrder}
 ...
 * , <tt>int</tt>, <tt>long</tt>, and <tt>double</tt>. The index parameters of
 * the absolute <i>get</i> and <i>put</i> methods are in terms of bytes rather
 * than of the type being read or written.
 *
 * <a name="views"></a>
 *
 * <p>
 * For access to homogeneous binary data, that is, sequences of values of the
 * same type, this class defines methods that can create <i>views</i> of a given
 * byte buffer. A <i>view buffer</i> is simply another buffer whose content is
 ...
 * and mark values are independent. The {@link #asFloatBuffer() asFloatBuffer}
 * method, for example, creates an instance of the {@link FloatBuffer} class
 ...
 * <li>
 * <p>
 * A view buffer provides relative bulk <i>get</i> and <i>put</i> methods that
 ...
 * <li>
 * <p>
 * A view buffer is potentially much more efficient because it will be direct
...
public abstract class ByteBuffer extends Buffer implements
	 ...
	 * to zero. Whether or not it has a {@link #hasArray backing array} is
	 * unspecified.
	 *
	 * @param capacity
	 ...
	 * &nbsp;<tt>remaining()</tt>, then no bytes are transferred and a
	 * {@link BufferUnderflowException} is thrown.
	 *
	 * <p>
	 * Otherwise, this method copies <tt>length</tt> bytes from this buffer into
	 ...
	 * <tt>src.remaining()</tt> bytes from the given buffer into this buffer,
	 ...
	 *
	 * @param src
	 *            The source buffer from which bytes are to be read; must not be
	 *            this buffer
	 *
	 * @return This buffer
	 *
	 ...
	 *
	 * <p>
	 * Otherwise, this method copies <tt>length</tt> bytes from the given array
	 ...
	 *
	 * @param length
	 *               The number of bytes to be read from the given array; must
	 *               be
	...
	/**
	 * Returns the byte array that backs this buffer&nbsp;&nbsp;<i>(optional
	 * operation)</i>.
	 *
	 * <p>
	...
	public final ByteBuffer order(ByteOrder bo) {
		...
		nativeByteOrder = (bigEndian == (Bits
	...
	/**
	 * Relative <i>put</i> method for writing a char value&nbsp;&nbsp;
	...
	/**
	 * Relative <i>put</i> method for writing a long value&nbsp;&nbsp;
	 * <i>(optional operation)</i>.
	 *
	 * <p>
