 ...
 * contiguous sequences of chars from a char array,&#32;a&#32;string, or some
 ...
 * char buffer created via the <tt>wrap</tt> methods of this class will be
 * non-direct. A char buffer created as a view of a byte buffer will be direct
 * if, and only if, the byte buffer itself is direct. Whether or not a char
...
public abstract class CharBuffer extends Buffer
	 ...
	 * its position will be <tt>start</tt>, its limit will be <tt>end</tt>, and
	 ...
	 *            than <tt>csq.length()</tt>. The new buffer's limit will be set
	 *            to this value.
	 *
	...
	/**
	 * Relative <i>get</i> method. Reads the char at this buffer's current
	 ...
	 * </pre>
	 *
	 * except that it first checks that there are sufficient chars in this
	 ...
	 *
	 * @param offset
	 *            The offset within the array of the first char to be written;
	 *            must be non-negative and no larger than <tt>dst.length</tt>
	 *
	 * @param length
	 *            The maximum number of chars to be written to the given array;
	 ...
	 *
	 * @throws BufferUnderflowException
	 *             If there are fewer than <tt>length</tt> chars remaining in
	 *             this buffer
	 *
	 * @throws IndexOutOfBoundsException
	 *             If the preconditions on the <tt>offset</tt> and
	 ...
	 *
	 * @param offset
	 *            The offset within the array of the first char to be read; must
	 *            be non-negative and no larger than <tt>array.length</tt>
	 *
	 * @param length
	 *            The number of chars to be read from the given array; must be
	 ...
	 *
	 * @param end
	 *            The offset within the string of the last char to be read, plus
	 ...
	 * char at index <i>p</i>&nbsp;+&nbsp;1 is copied to index one, and so forth
	 * until the char at index <tt>limit()</tt>&nbsp;-&nbsp;1 is copied to index
	 ...
	 * order} of the underlying hardware. The byte order of a char buffer
