 ...
 * <li>
 * <p>
 * The {@link #getPath getPath} method converts a system dependent
 * <em>path string</em>, returning a {@link Path} object that may be used to
 * locate and access a file.
 * </p>
 * </li>
 * <li>
 * <p>
 * The {@link #getPathMatcher getPathMatcher} method is used to create a
 * {@link PathMatcher} that performs match operations on paths.
 * </p>
 * </li>
 * <li>
 * <p>
 * The {@link #getFileStores getFileStores} method returns an iterator over the
 * underlying {@link FileStore file-stores}.
 * </p>
 * </li>
 * <li>
 * <p>
 * The {@link #getUserPrincipalLookupService getUserPrincipalLookupService}
 ...
 * <li>
 * <p>
 * The {@link #newWatchService newWatchService} method creates a
 ...
 * when the {@code FileSystem} is created and can be tested by invoking its
 ...
 * but whether a file system is <i>asynchronously closeable</i> is provider
 * specific and therefore unspecified. In other words, if a thread is accessing
 * an object in a file system, and another thread invokes the {@code close}
 * method then it may require to block until the first operation is complete.
 * Closing a file system causes all open channels, watch services, and other
 * {@link Closeable closeable} objects associated with the file system to be
 * closed.
 *
 * @since 1.7
 */
...
public abstract class FileSystem implements Closeable {
     ...
     * directory-streams}, {@link WatchService watch-service}, and other
     * closeable objects associated with this file system. The
     ...
     * the iterator. In the case of the default provider, the
     ...
     * <tt>("getFileStoreAttributes")</tt>. If denied, then no file stores are
     * returned by the iterator. In addition, the security manager's
     * {@link SecurityManager#checkRead(String)} method is invoked to check read
     * access to the file store's <em>top-most</em> directory. If denied, the
     ...
     * path string, to a {@code Path}. If {@code more} does not specify any
     ...
     * {@link InvalidPathException} thrown, if the path string contains
     ...
     * those allowed by any file store, and where an implementation supports a
     ...
     * exception is created with an {@link InvalidPathException#getIndex index}
     * value indicating the first position in the {@code path} parameter that
     ...
     * <tr>
     * <td>{@code *.java}</td>
     * <td>Matches a path that represents a file name ending in {@code .java}
     * </td>
     * </tr>
     * <tr>
     * <td>{@code *.*}</td>
     ...
     * <li>
     * <p>
     * The {@code *} character matches zero or more {@link Character characters}
     * of a {@link Path#getName(int) name} component without crossing directory
     * boundaries.
     * </p>
     * </li>
     * <li>
     ...
     * {@code \\} matches a single backslash and "\{" matches a left brace for
     * example.
     * </p>
     * </li>
     * <li>
     ...
     * single character of a name component out of a set of characters. For
     ...
     * (inclusive). These forms can be mixed so [abce-g] matches {@code "a"},
     * {@code "b"}, {@code "c"}, {@code "e"}, {@code "f"} or {@code "g"}. If the
     * character after the {@code [} is a {@code !} then it is used for negation
     * so {@code
     *   [!a-c]} matches any character except {@code "a"}, {@code "b"}, or
     * {@code
     *   "c"}.
     * <p>
     ...
     * matches if any subpattern in the group matches. The {@code ","} character
     ...
     * pattern matches file name {@code ".login"}. The {@link Files#isHidden}
     ...
     * manner. This includes characters representing any
