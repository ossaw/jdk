 ...
 * use file systems that have a file <em>owner</em>, <em>group-owner</em>, and
 * related <em>access
 ...
 * permission defined by the {@link PosixFilePermission} class. These nine
 * permission bits determine the <em>read</em>, <em>write</em>, and
 * <em>execute</em> access for the file owner, group, and others (others meaning
 ...
 * by this attribute view are as defined by {@link BasicFileAttributeView} and
 * {@link FileOwnerAttributeView}, and in addition, the following attributes are
 ...
 * defined by {@link BasicFileAttributeView}. It may also be used to update the
 ...
 * initial permissions when creating a file or directory. The initial
 * permissions are provided to the {@link Files#createFile createFile} or
 * {@link Files#createDirectory createDirectory} methods as a
 * {@link FileAttribute} with {@link FileAttribute#name name}
 * {@code "posix:permissions"} and a {@link FileAttribute#value value} that is
 * the set of permissions. The following example uses the
 * {@link PosixFilePermissions#asFileAttribute asFileAttribute} method to
 * construct a {@code FileAttribute} when creating a file:
 *
 * <pre>
 *     Path path = ...
 ...
 * example, a process has a <em>umask</em> that impacts the permission bits of
 * newly created files. Where an implementation supports the setting of the
 * access permissions, and the underlying file system supports access
 * permissions, then it is required that the value of the actual access
 * permissions will be equal or less than the value of the attribute provided to
 * the {@link Files#createFile createFile} or {@link Files#createDirectory
 * createDirectory} methods. In other words, the file may be more secure than
 * requested.
 *
 * @since 1.7
 */
