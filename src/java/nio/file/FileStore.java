 ...
 * partition, volume, concrete file system or other implementation specific
 * means of file storage. The {@code FileStore} for where a file is stored is
...
public abstract class FileStore {
     ...
     * indicate, for example, the format used or if the file store is local or
     * remote.
     *
     ...
     * that it is possible to use most or any of these bytes. The number of
     * unallocated bytes is most likely to be accurate immediately after the
     * space attributes are obtained. It is likely to be made inaccurate by any
     * external I/O operations including those made on the system outside of
     ...
     * case of the default provider, this method cannot guarantee to give the
     * correct result when the file store is not a local storage device. The
     ...
     * will always return {@code true}. In the case of the default provider,
     * this method cannot guarantee to give the correct result when the file
     * store is not a local storage device. The reasons for this are
