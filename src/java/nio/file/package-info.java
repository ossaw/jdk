 ...
 * <em>target</em> of the link. Exceptions to this are when a symbolic link is
 * deleted or renamed/moved in which case the link is deleted or removed rather
 * than the target of the link. This package includes support for symbolic links
 * where implementations provide these semantics. File systems may support other
 ...
 * method to construct a {@link java.nio.file.Path} by converting the abstract
 * path represented by the {@code java.io.File} object. The resulting
 * {@code Path} can be used to operate on the same file as the {@code File}
 * object. The {@code Path} specification provides further information on the
 ...
 * guaranteed to be consistent with other views provided by other instances in
 * the same Java virtual machine. The view may or may not, however, be
 * consistent with the view of the file system as seen by other concurrently
 * running programs due to caching performed by the underlying operating system
 * and delays induced by network-filesystem protocols. This is true regardless
 * of the language in which these other programs are written, and whether they
 ...
 * {@link java.nio.file.StandardOpenOption#DSYNC DSYNC} options are used when
 * opening a file to require that updates to the file are written synchronously
 * to the underlying storage device. In the case of the default provider, and
 * the file resides on a local storage device, and the
 * {@link java.nio.channels.SeekableByteChannel seekable} channel is connected
 * to a file that was opened with one of these options, then an invocation of
 * the {@link java.nio.channels.WritableByteChannel#write(java.nio.ByteBuffer)
 * write} method is only guaranteed to return when all changes made to the file
 ...
 * method of any class or interface in this package will cause a
 * {@link java.lang.NullPointerException NullPointerException} to be thrown.
 * Additionally, invoking a method with a collection containing a {@code null}
 ...
 * associated with a {@link java.nio.file.FileSystem} that has been
 * {@link java.nio.file.FileSystem#close closed}. Additionally, any methods that
 * attempt write access to a file system will throw
 * {@link java.nio.file.ReadOnlyFileSystemException} when invoked on an object
 ...
 * cases, these methods define specific I/O exceptions for common cases. These
 * exceptions, noted as <i>optional specific exceptions</i>, are thrown by the
 * implementation where it can detect the specific error. Where the specific
 * error cannot be detected then the more general {@code
