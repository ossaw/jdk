 ...
 * is identified by the URI scheme "file". It creates the {@link FileSystem}
 * that provides access to the file systems accessible to the Java virtual
 * machine. The {@link FileSystems} class defines how file system providers are
 * located and loaded. The default provider is typically a system-default
 * provider but may be overridden if the system property {@code
 ...
 * file system is identified by a {@code URI} where the URI's scheme matches the
 * provider's {@link #getScheme scheme}. The default file system, for example,
 * is identified by the URI {@code "file:///"}. A memory-based file system, for
 * example, may be identified by a URI such as {@code "memory:///?name=logfs"}.
 ...
 * a provider is the factory for a single file system then it is provider
 * dependent if the file system is created when the provider is initialized, or
 * later when the {@code newFileSystem} method is invoked. In the case of the
 * default provider, the {@code FileSystem} is created when the provider is
 * initialized.
 * <p>
...
public abstract class FileSystemProvider {
     ...
     * the default provider but care needs to be taken to avoid circular loading
     * of other installed providers. If circular loading of installed providers
    ...
    private static List<FileSystemProvider> loadInstalledProviders() {
        ...
        ServiceLoader<FileSystemProvider> sl = ServiceLoader.load(FileSystemProvider.class, ClassLoader
        ...
        if (installedProviders == null) {
            ...
            synchronized (lock) {
                if (installedProviders == null) {
                    ...
                    List<FileSystemProvider> list = AccessController.doPrivileged(
     ...
     * <p>
     * The {@code uri} parameter is an absolute, hierarchical URI, with a scheme
     * equal (without regard to case) to the scheme supported by this provider.
     * The exact form of the URI is highly provider dependent. The {@code env}
     * parameter is a map of provider specific properties to configure the file
     * system.
     * <p>
     * This method throws {@link FileSystemAlreadyExistsException} if the file
     * system already exists because it was previously created by an invocation
     * of this method. Once a file system is
     * {@link java.nio.file.FileSystem#close closed} it is provider-dependent if
     * the provider allows a new file system to be created with the same URI as
     * a file system it previously created.
     *
     * @param uri
     *        URI reference
     * @param env
     ...
     * <p>
     * This method returns a reference to a {@code FileSystem} that was created
     * by invoking the {@link #newFileSystem(URI,Map) newFileSystem(URI,Map)}
     * method. File systems created the {@link #newFileSystem(Path,Map)
     * newFileSystem(Path,Map)} method are not returned by this method. The file
     * system is identified by its {@code URI}. Its exact form is highly
     * provider dependent. In the case of the default provider the URI's path
     * component is {@code "/"} and the authority, query and fragment components
     * are undefined (Undefined components are represented by {@code null}).
     * <p>
     * Once a file system created by this provider is
     * {@link java.nio.file.FileSystem#close closed} it is provider-dependent if
     * this method returns a reference to the closed file system or throws
     * {@link FileSystemNotFoundException}. If the provider allows a new file
     * system to be created with the same URI as a file system it previously
     * created then this method throws the exception if invoked after the file
     * system is closed (and before a new instance is created by the
     * {@link #newFileSystem newFileSystem} method).
     * <p>
     * If a security manager is installed then a provider implementation may
     * require to check a permission before returning a reference to an existing
     * file system. In the case of the {@link FileSystems#getDefault default}
     ...
     * <p>
     * The exact form of the URI is file system provider dependent. In the case
     * of the default provider, the URI scheme is {@code "file"} and the given
     * URI has a non-empty path component, and undefined query, and fragment
     * components. The resulting {@code Path} is associated with the default
     * {@link FileSystems#getDefault default} {@code FileSystem}.
     * <p>
     * If a security manager is installed then a provider implementation may
     * require to check a permission. In the case of the
     * {@link FileSystems#getDefault default} file system, no permission check
     * is required.
     *
     * @param uri
     *        The URI to convert
     ...
     * file system.
     * <p>
     * This method is intended for specialized providers of pseudo file systems
     * where the contents of one or more files is treated as a file system. The
     * {@code env} parameter is a map of provider specific properties to
     * configure the file system.
     * <p>
     * If this provider does not support the creation of such file systems or if
     * the provider does not recognize the file type of the given file then it
     * throws {@code UnsupportedOperationException}. The default implementation
     ...
     * <p>
     * The default implementation of this method opens a channel to the file as
     * if by invoking the {@link #newByteChannel} method and constructs a stream
     * that reads bytes from the channel. This method should be overridden where
     * appropriate.
     *
     * @param path
     ...
     * The default implementation of this method opens a channel to the file as
     * if by invoking the {@link #newByteChannel} method and constructs a stream
     * that writes bytes to the channel. This method should be overridden where
     * appropriate.
     *
     * @param path
     ...
     * AsynchronousFileChannel.open} method. A provider that does not support
     * all the features required to construct an asynchronous file channel
     * throws {@code UnsupportedOperationException}. The default provider is
     * required to support the creation of asynchronous file channels. When not
     * overridden, the default implementation of this method throws
     ...
     * <p>
     * The default implementation of this method simply invokes {@link #delete}
     * ignoring the {@code NoSuchFileException} when the file does not exist. It
     ...
     * specified by the {@link Files#copy(Path,Path,CopyOption[])} method except
     * that both the source and target paths must be associated with this
     * provider.
     *
     * @param source
     ...
     * located. This method works in exactly the manner specified by the
     ...
     * <td>Checks that the file exists and that the Java virtual machine has
     * permission to {@link Runtime#exec execute} the file. The semantics may
     * differ when checking access to a directory. For example, on UNIX systems,
     * checking for {@code EXECUTE} access checks that the Java virtual machine
     * has permission to search the directory in order to access file or
     * subdirectories.</td>
     * </tr>
     * </table>
     * <p>
     * If the {@code modes} parameter is of length zero, then the existence of
     * the file is checked.
     * <p>
     * This method follows symbolic links if the file referenced by this object
     * is a symbolic link. Depending on the implementation, this method may
     * require to read file permissions, access control lists, or other file
     * attributes in order to check the effective access to the file. To
