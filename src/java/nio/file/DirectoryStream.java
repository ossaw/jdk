 ...
 *
 * <p>
 * <b> While {@code DirectoryStream} extends {@code Iterable}, it is not a
 ...
 *
 * <p>
 * An important property of the directory stream's {@code Iterator} is that its
 * {@link Iterator#hasNext() hasNext} method is guaranteed to read-ahead by at
 * least one element. If {@code hasNext} method returns {@code true}, and is
 ...
 *
 * <p>
 * A {@code DirectoryStream} is opened upon creation and is closed by invoking
 * the {@code close} method. Closing a directory stream releases any resources
 * associated with the stream. Failure to close the stream may result in a
 * resource leak. The try-with-resources statement provides a useful construct
 ...
 * Due to read-ahead, the {@code Iterator} may return one or more elements after
 * the directory stream has been closed. Once these buffered elements have been
 * read, then subsequent calls to the {@code hasNext} method returns
 * {@code false}, and subsequent calls to the {@code next} method will throw
 * {@code NoSuchElementException}.
 *
 * <p>
 * A directory stream is not required to be <i>asynchronously closeable</i>. If
 * a thread is blocked on the directory stream's iterator reading from the
 ...
 * the {@code Iterator}'s {@code hasNext} or {@code next} methods to throw
 * {@link DirectoryIteratorException} with the {@link IOException} as the cause.
 * As stated above, the {@code hasNext} method is guaranteed to read-ahead by at
 * least one element. This means that if {@code hasNext} method returns
 * {@code true}, and is followed by a call to the {@code next} method, then it
 * is guaranteed that the {@code next} method will not fail with a
 ...
 *
 * <p>
 * The iterator is <i>weakly consistent</i>. It is thread safe but does not
 * freeze the directory while iterating, so it may (or may not) reflect updates
 * to the directory that occur after the {@code DirectoryStream} is created.
 *
 * <p>
 * <b>Usage Examples:</b> Suppose we want a list of the source files in a
 * directory. This example uses both the for-each and try-with-resources
 * constructs.
 * 
 * <pre>
