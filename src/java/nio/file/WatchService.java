 ...
 * represent the registration. When an event for an object is detected the key
 * is <em>signalled</em>, and if not currently signalled, it is queued to the
 * watch service so that it can be retrieved by consumers that invoke the
 * {@link #poll() poll} or {@link #take() take} methods to retrieve keys and
 ...
 * time then care should be taken to ensure that the key's {@code reset} method
 * is only invoked after its events have been processed. The {@link #close
 ...
 * that it may accumulate. Where an implementation <em>knowingly</em> discards
 * events then it arranges for the key's {@link WatchKey#pollEvents pollEvents}
 ...
 * {@link StandardWatchEventKinds#ENTRY_MODIFY ENTRY_MODIFY} event in some
 ...
 * particular, it is not required that changes to files carried out on remote
 * systems be detected.
 *
 * @since 1.7
