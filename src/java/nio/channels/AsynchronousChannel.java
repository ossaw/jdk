 ...
 * operation to throw {@link java.util.concurrent.CancellationException}.
 * Whether the underlying I/O operation can be cancelled is highly
 * implementation specific and therefore not specified. Where cancellation
 * leaves the channel, or the entity to which it is connected, in an
 * inconsistent state, then the channel is put into an implementation specific
 * <em>error state</em> that prevents further attempts to initiate I/O
 * operations that are <i>similar</i> to the operation that was cancelled. For
 ...
 * channel into an error state; further attempts to initiate a {@code read}
 * operation cause an unspecified runtime exception to be thrown. Similarly, if
