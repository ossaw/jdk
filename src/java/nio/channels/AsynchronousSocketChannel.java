 ...
 * {@code AsynchronousSocketChannel} is created by invoking one of the
 * {@link #open open} methods defined by this class. A newly-created channel is
 * open but not yet connected. A connected {@code AsynchronousSocketChannel} is
 * created when a connection is made to the socket of an
 ...
 * {@link #setOption(SocketOption,Object) setOption} method. Asynchronous socket
 * channels support the following options: <blockquote> <table border summary=
 * "Socket options">
 * <tr>
 * <th>Option Name</th>
 * <th>Description</th>
 ...
 * {@link InterruptedByTimeoutException}. A timeout may leave the channel, or
 * the underlying connection, in an inconsistent state. Where the implementation
 * cannot guarantee that bytes have not been read from the channel then it puts
 * the channel into an implementation specific <em>error state</em>. A
 * subsequent attempt to initiate a {@code read} operation causes an unspecified
 * runtime exception to be thrown. Similarly if a {@code write} operation times
 * out and the implementation cannot guarantee bytes have not been written to
 * the channel then further attempts to {@code write} to the channel cause an
...
public abstract class AsynchronousSocketChannel implements
		AsynchronousByteChannel, NetworkChannel {
	 ...
	 *                                         If a connection operation is
	 *                                         already in progress on this
	 *                                         channel
	 ...
	 * @param remote
	 *                   The remote address to which this channel is to be
	 *                   connected
	 * @param attachment
	 ...
	 * {@code
	 * Future} representing the pending result. The {@code Future}'s
	 ...
	 *                                       If a read operation is already in
	 ...
	 */
	public abstract <A> void read(ByteBuffer dst, long timeout, TimeUnit unit,
	 ...
	 * @param offset
	 *                   The offset within the buffer array of the first buffer
	 *                   into
	 *                   which bytes are to be transferred; must be non-negative
	 *                   and no
	 ...
	 *                                       If the channel group has terminated
	 */
	public abstract <A> void read(ByteBuffer[] dsts, int offset, int length,
	 ...
	 *                                       If a write operation is already in
	 *                                       progress on this channel
	 * @throws NotYetConnectedException
	 *                                       If this channel is not yet
	 *                                       connected
	 ...
	 * <i>r</i>. Up to the first <tt>srcs[offset].remaining()</tt> bytes of this
	 * sequence are written from buffer <tt>srcs[offset]</tt>, up to the next
	 ...
	 *                                       If the channel group has terminated
	 */
	public abstract <A> void write(ByteBuffer[] srcs, int offset, int length,
			long timeout, TimeUnit unit, A attachment,
			CompletionHandler<Long, ? super A> handler);
