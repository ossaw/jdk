...
public interface AsynchronousByteChannel extends AsynchronousChannel {
     ...
     * byte sequence will be transferred into the buffer so that the first byte
     * in the sequence is at index <i>p</i> and the last byte is at index
     ...
     * {@link #read(ByteBuffer,Object,CompletionHandler)
     * read(ByteBuffer,Object,CompletionHandler)} method except that instead of
     * specifying a completion handler, this method returns a {@code Future}
     ...
     * the write is performed; the index of the last byte written will be
     * <i>p</i>&nbsp;<tt>+</tt>&nbsp;<i>n</i>&nbsp;<tt>-</tt>&nbsp;<tt>1</tt>.
     ...
     * exactly the same manner as the
     * {@link #write(ByteBuffer,Object,CompletionHandler)
     * write(ByteBuffer,Object,CompletionHandler)} method except that instead of
     * specifying a completion handler, this method returns a {@code Future}
     * representing the pending result. The {@code Future}'s {@link Future#get()
