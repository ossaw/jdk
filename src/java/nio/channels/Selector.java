 ...
 * {@link java.nio.channels.spi.SelectorProvider selector provider} to create a
 * new selector. A selector may also be created by invoking the
 ...
 * registrations of this selector. This set is returned by the {@link #keys()
 * keys} method.
 * </p>
 * </li>
 *
 ...
 * key's interest set during a prior selection operation. This set is returned
 * by the {@link #selectedKeys() selectedKeys} method. The selected-key set is
 ...
 * removed directly from the selected-key set by invoking the set's
 * {@link java.util.Set#remove(java.lang.Object) remove} method or by invoking
 * the {@link java.util.Iterator#remove() remove} method of an
 * {@link java.util.Iterator iterator} obtained from the set. Keys are never
 * removed from the selected-key set in any other way; they are not, in
 ...
 * a member, and its channel is deregistered. This step leaves the cancelled-key
 * set empty.
 * </p>
 * </li>
 *
 ...
 * of each remaining channel to perform any of the operations identified by its
 * key's interest set as of the moment that the selection operation began. For a
 ...
 * to that set and its ready-operation set is modified to identify exactly those
 * operations for which the channel is now reported to be ready. Any readiness
 ...
 * ready-operation set is modified to identify any new operations for which the
 * channel is reported to be ready. Any readiness information previously
 * recorded in the ready set is preserved; in other words, the ready set
 * returned by the underlying system is bitwise-disjoined into the key's current
 * ready set.
 * </p>
 * </li>
 *
 ...
 * interrupt} method, in which case its interrupt status will be set and the
 ...
 * The iterators returned by these sets' {@link java.util.Set#iterator()
 * iterator} methods are <i>fail-fast:</i> If the set is modified after the
 * iterator is created, in any way except by invoking the iterator's own
 * {@link java.util.Iterator#remove() remove} method, then a
...
public abstract class Selector implements Closeable {
	 ...
	 * This method performs a blocking <a href="#selop">selection operation</a>.
	 * It returns only after at least one channel is selected, this selector's
	 * {@link #wakeup wakeup} method is invoked, or the current thread is
