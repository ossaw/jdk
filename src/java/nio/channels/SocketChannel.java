 ...
 * <p>
 * A socket channel is created by invoking one of the {@link #open open} methods
 * of this class. It is not possible to create a channel for an arbitrary,
 * pre-existing socket. A newly-created socket channel is open but not yet
 * connected. An attempt to invoke an I/O operation upon an unconnected channel
 * will cause a {@link NotYetConnectedException} to be thrown. A socket channel
 * can be connected by invoking its {@link #connect connect} method; once
 * connected, a socket channel remains connected until it is closed. Whether or
 * not a socket channel is connected may be determined by invoking its
 * {@link #isConnected isConnected} method.
 * <p>
 * Socket channels support <i>non-blocking connection:</i>&nbsp;A socket channel
 * may be created and the process of establishing the link to the remote socket
 * may be initiated via the {@link #connect connect} method for later completion
 * by the {@link #finishConnect finishConnect} method. Whether or not a
 * connection operation is in progress may be determined by invoking the
 * {@link #isConnectionPending isConnectionPending} method.
 * <p>
 * Socket channels support <i>asynchronous shutdown,</i> which is similar to the
 * asynchronous close operation specified in the {@link Channel} class. If the
 * input side of a socket is shut down by one thread while another thread is
 * blocked in a read operation on the socket's channel, then the read operation
 * in the blocked thread will complete without reading any bytes and will return
 * <tt>-1</tt>. If the output side of a socket is shut down by one thread while
 * another thread is blocked in a write operation on the socket's channel, then
 * the blocked thread will receive an {@link AsynchronousCloseException}.
 * <p>
 * Socket options are configured using the
 * {@link #setOption(SocketOption,Object) setOption} method. Socket channels
 * support the following options: <blockquote> <table border summary=
 * "Socket options">
 * <tr>
 * <th>Option Name</th>
 * <th>Description</th>
 ...
 * concurrent reading and writing, though at most one thread may be reading and
 * at most one thread may be writing at any given time. The {@link #connect
 * connect} and {@link #finishConnect finishConnect} methods are mutually
 * synchronized against each other, and an attempt to initiate a read or write
 * operation while an invocation of one of these methods is in progress will
 * block until that invocation is complete.
 * </p>
 *
...
public abstract class SocketChannel extends AbstractSelectableChannel implements ByteChannel,
     ...
     * {@link java.nio.channels.spi.SelectorProvider#openSocketChannel
     * openSocketChannel} method of the system-wide default
     ...
     *
     * @param remote
     *        The remote address to which the new channel is to be
     *        connected
     * @return A new, and connected, socket channel
     * @throws AsynchronousCloseException
     *         If another thread closes this
     *         channel while the connect
     ...
     *         If the given remote address is
     *         not fully resolved
     ...
     * returns <tt>(</tt>{@link SelectionKey#OP_CONNECT} <tt>|</tt>&nbsp;
     * {@link SelectionKey#OP_READ} <tt>|</tt>&nbsp;
     ...
     *
     * @return <tt>true</tt> if, and only if, this channel's network socket is
     ...
     * method returns <tt>true</tt>. Otherwise this method returns
     ...
     * endpoint.
     * <p>
     * This method may be invoked at any time. If a read or write operation upon
     * this channel is invoked while an invocation of this method is in progress
     * then that operation will first block until this invocation is complete.
     * If a connection attempt is initiated but fails, that is, if an invocation
     * of this method throws a checked exception, then the channel will be
     * closed.
     * </p>
     *
     * @param remote
     ...
     * failed, the socket channel will become connectable and this method may be
     * invoked to complete the connection sequence. If the connection operation
     * failed then invoking this method will cause an appropriate
     * {@link java.io.IOException} to be thrown.
     * <p>
     * If this channel is already connected then this method will not block and
     * will immediately return <tt>true</tt>. If this channel is in non-blocking
     * mode then this method will return <tt>false</tt> if the connection
     * process is not yet complete. If this channel is in blocking mode then
     * this method will block until the connection either completes or fails,
     * and will always either return <tt>true</tt> or throw a checked exception
     ...
     *         If this channel is not connected and
     ...
     * address then the return value from this method is of type
