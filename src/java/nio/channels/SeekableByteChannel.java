 ...
 * The current position can be {@link #position() <i>queried</i>} and
...
public interface SeekableByteChannel extends ByteChannel {
     ...
     * position is updated with the number of bytes actually read. Otherwise
     ...
     * {@link java.nio.file.StandardOpenOption#APPEND APPEND} option, in which
     * case the position is first advanced to the end. The entity to which the
     * channel is connected is grown, if necessary, to accommodate the written
     * bytes, and then the position is updated with the number of bytes actually
     ...
     *         number of bytes from the beginning of the entity to the current
     *         position
     *
     ...
     * entity, typically a file, that is opened with the
     * {@link java.nio.file.StandardOpenOption#APPEND APPEND} option. When
     * opened for append, the position is first advanced to the end before
     * writing.
     *
     * @param newPosition
     ...
     * greater than or equal to the current size then the entity is not
     * modified. In either case, if the current position is greater than the
