 ...
 * Channels are bound to a group by specifying the group when constructing the
 * channel. The associated thread pool is <em>owned</em> by the group;
 * termination of the group results in the shutdown of the associated thread
 * pool.
 *
 * <p>
 * In addition to groups created explicitly, the Java virtual machine maintains
 * a system-wide <em>default group</em> that is constructed automatically.
 * Asynchronous channels that do not specify a group at construction time are
 * bound to the default group. The default group has an associated thread pool
 * that creates new threads as needed. The default group may be configured by
 * means of system properties defined in the table below. Where the
 * {@link java.util.concurrent.ThreadFactory ThreadFactory} for the default
 * group is not configured then the pooled threads of the default group are
 ...
 * <td>The value of this property is taken to be the fully-qualified name of a
 * concrete {@link java.util.concurrent.ThreadFactory ThreadFactory} class. The
 * class is loaded using the system class loader and instantiated. The factory's
 * {@link java.util.concurrent.ThreadFactory#newThread newThread} method is
 * invoked to create each thread for the default group's thread pool. If the
 * process to load and instantiate the value of the property fails then an
 * unspecified error is thrown during the construction of the default group.
 * </td>
 * </tr>
 * <tr>
 * <td>{@code java.nio.channels.DefaultThreadPool.initialSize}</td>
 * <td>The value of the {@code initialSize} parameter for the default group (see
 * {@link #withCachedThreadPool withCachedThreadPool}). The value of the
 * property is taken to be the {@code String} representation of an
 * {@code Integer} that is the initial size parameter. If the value cannot be
 * parsed as an {@code Integer} it causes an unspecified error to be thrown
 ...
 * invoked directly by the initiating thread. To avoid stack overflow, an
 ...
 *
 * <p>
 * The {@link #shutdown() shutdown} method is used to initiate an <em>orderly
 ...
 * the group <em>terminates</em> when all asynchronous channels that are bound
 * to the group are closed, all actively executing completion handlers have run
 * to completion, and resources used by the group are released. No attempt is
 * made to stop or interrupt threads that are executing completion handlers. The
...
public abstract class AsynchronousChannelGroup {
     ...
     * {@link AsynchronousChannelProvider#openAsynchronousChannelGroup(int,ThreadFactory)
     * openAsynchronousChannelGroup(int,ThreadFactory)} method of the
     ...
     *
     * <p>
     * The {@code executor} parameter is an {@code ExecutorService} that creates
     * new threads as needed to execute tasks that are submitted to handle I/O
     * events and dispatch completion results for operations initiated on
     * asynchronous channels in the group. It may reuse previously constructed
     * threads when they are available.
     *
     * <p>
     * The {@code initialSize} parameter may be used by the implementation as a
     * <em>hint</em> as to the initial number of tasks it may submit. For
     ...
     * <p>
     * The group is created by invoking the
     * {@link AsynchronousChannelProvider#openAsynchronousChannelGroup(ExecutorService,int)
     * openAsynchronousChannelGroup(ExecutorService,int)} method of the
     * system-wide default {@link AsynchronousChannelProvider} object.
     *
     * @param executor
     ...
     * support <em>direct handoff</em> or <em>unbounded queuing</em> of
     * submitted tasks, and the thread that invokes the
     * {@link ExecutorService#execute execute} method should never invoke the
     * task directly. An implementation may mandate additional constraints.
     *
     * <p>
     * The executor is intended to be used exclusively by the resulting
     * asynchronous channel group. Termination of the group results in the
     ...
     * {@link AsynchronousChannelProvider#openAsynchronousChannelGroup(ExecutorService,int)
     * openAsynchronousChannelGroup(ExecutorService,int)} method of the
     * system-wide default {@link AsynchronousChannelProvider} object with an
     * {@code
     ...
     *
     * <p>
     * Where this method returns {@code true}, then the associated thread pool
     * has also {@link ExecutorService#isTerminated terminated}.
     *
     ...
     * asynchronous channels in the group are closed, all actively executing
     ...
     * 
     * <p>
     * This method blocks until the group has terminated, or the timeout occurs,
