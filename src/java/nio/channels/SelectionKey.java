 ...
 * it is instead added to the selector's
 ...
 * subclass of {@link SelectableChannel} defines an
 * {@link SelectableChannel#validOps() validOps()} method which returns a set
...
public abstract class SelectionKey {
	 ...
	 * the selector detects that the corresponding channel is ready for reading,
	 * has reached end-of-stream, has been remotely shut down for further
	 * reading, or has an error pending, then it will add <tt>OP_READ</tt> to
	 * the key's ready-operation set and add the key to its
	 * selected-key&nbsp;set.
	 * </p>
	 */
	 ...
	 * the start of a <a href="Selector.html#selop">selection operation</a>. If
	 * the selector detects that the corresponding channel is ready for writing,
	 * has been remotely shut down for further writing, or has an error pending,
	 * then it will add <tt>OP_WRITE</tt> to the key's ready set and add the key
	 ...
	 * If the selector detects that the corresponding socket channel is ready to
	 * complete its connection sequence, or has an error pending, then it will
	 * add <tt>OP_CONNECT</tt> to the key's ready set and add the key to its
	 * selected-key&nbsp;set.
	 * </p>
	 */
	 ...
	 * ready to accept another connection, or has an error pending, then it will
	 * add <tt>OP_ACCEPT</tt> to the key's ready set and add the key to its
	 * selected-key&nbsp;set.
	 * </p>
	 */
	...
	private static final AtomicReferenceFieldUpdater<SelectionKey, Object> attachmentUpdater = AtomicReferenceFieldUpdater
			.newUpdater(SelectionKey.class, Object.class, "attachment");
