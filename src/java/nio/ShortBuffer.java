 ...
 * contiguous sequences of shorts from a short array or some other short buffer
 ...
 * short buffer created via the <tt>wrap</tt> methods of this class will be
 * non-direct. A short buffer created as a view of a byte buffer will be direct
 * if, and only if, the byte buffer itself is direct. Whether or not a short
...
public abstract class ShortBuffer extends Buffer implements
	...
	/**
	 * Relative <i>get</i> method. Reads the short at this buffer's current
	 ...
	 * </pre>
	 *
	 * except that it first checks that there are sufficient shorts in this
	 ...
	 * short at index <i>p</i>&nbsp;+&nbsp;1 is copied to index one, and so
	 * forth until the short at index <tt>limit()</tt>&nbsp;-&nbsp;1 is copied
	 ...
	 * order} of the underlying hardware. The byte order of a short buffer
