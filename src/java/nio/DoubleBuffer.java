 ...
 * <li>
 * <p>
 * Absolute and relative {@link #get() <i>get</i>} and {@link #put(double)
 * <i>put</i>} methods that read and write single doubles;
 * </p>
 * </li>
 *
 * <li>
 * <p>
 * Relative {@link #get(double[]) <i>bulk get</i>} methods that transfer
 * contiguous sequences of doubles from this buffer into an array; and
 * </p>
 * </li>
 *
 * <li>
 * <p>
 * Relative {@link #put(double[]) <i>bulk put</i>} methods that transfer
 * contiguous sequences of doubles from a double array or some other double
 ...
 * {@link #slice slicing} a double buffer.
 * </p>
 * </li>
 *
 * </ul>
 *
 * <p>
 * Double buffers can be created either by {@link #allocate <i>allocation</i>},
 ...
 * 
 * 
 * content, by {@link #wrap(double[]) <i>wrapping</i>} an existing double array
 ...
 * double buffer created via the <tt>wrap</tt> methods of this class will be
 * non-direct. A double buffer created as a view of a byte buffer will be direct
 * if, and only if, the byte buffer itself is direct. Whether or not a double
...
public abstract class DoubleBuffer extends Buffer implements Comparable<DoubleBuffer> {
	 ...
	 * be the number of doubles remaining in this buffer, and its mark will be
	 ...
	 * array. If there are fewer doubles remaining in the buffer than are
	 * required to satisfy the request, that is, if <tt>length</tt>&nbsp;
	 * <tt>&gt;</tt>&nbsp;<tt>remaining()</tt>, then no doubles are transferred
	 * and a {@link BufferUnderflowException} is thrown.
	 *
	 * <p>
	 * Otherwise, this method copies <tt>length</tt> doubles from this buffer
	 ...
	 * </pre>
	 *
	 * except that it first checks that there are sufficient doubles in this
	 ...
	 *
	 * @param offset
	 *            The offset within the array of the first double to be written;
	 ...
	 *
	 * @throws BufferUnderflowException
	 *             If there are fewer than <tt>length</tt> doubles remaining in
	 *             this buffer
	 *
	 * @throws IndexOutOfBoundsException
	 *             If the preconditions on the <tt>offset</tt> and
	 ...
	 *
	 * <p>
	 * This method transfers the doubles remaining in the given source buffer
	 * into this buffer. If there are more doubles remaining in the source
	 ...
	 * <tt>src.remaining()</tt> doubles from the given buffer into this buffer,
	 ...
	 *
	 * @param src
	 *            The source buffer from which doubles are to be read; must not
	 *            be this buffer
	 *
	 * @return This buffer
	 *
	 ...
	 *             remaining doubles in the source buffer
	 *
	 * @throws IllegalArgumentException
	 ...
	 * array. If there are more doubles to be copied from the array than remain
	 * in this buffer, that is, if <tt>length</tt>&nbsp;<tt>&gt;</tt>&nbsp;
	 * <tt>remaining()</tt>, then no doubles are transferred and a
	 * {@link BufferOverflowException} is thrown.
	 *
	 * <p>
	 * Otherwise, this method copies <tt>length</tt> doubles from the given
	 ...
	 *
	 * @param length
	 *            The number of doubles to be read from the given array; must be
	...
	/**
	 * Returns the double array that backs this buffer&nbsp;&nbsp;<i>(optional
	 * operation)</i>.
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * The doubles between the buffer's current position and its limit, if any,
	 * are copied to the beginning of the buffer. That is, the double at index
	 * <i>p</i>&nbsp;=&nbsp;<tt>position()</tt> is copied to index zero, the
	 * double at index <i>p</i>&nbsp;+&nbsp;1 is copied to index one, and so
	 * forth until the double at index <tt>limit()</tt>&nbsp;-&nbsp;1 is copied
	 ...
	 *
	 * <p>
	 * The buffer's position is set to the number of doubles copied, rather than
	 ...
	 * 
	 * This method considers two double elements {@code a} and {@code b} to be
	 * equal if {@code (a == b) || (Double.isNaN(a) && Double.isNaN(b))}. The
	 * values {@code -0.0} and {@code +0.0} are considered to be equal, unlike
	 ...
	 * {@link Double#compare(double,double)}, except that {@code -0.0} and
	 * {@code 0.0} are considered to be equal. {@code Double.NaN} is considered
	 * by this method to be equal to itself and greater than all other
	 * {@code double} values (including {@code Double.POSITIVE_INFINITY}).
	 *
	 * 
	 * 
	 * 
	...
	private static int compare(double x, double y) {
		...
		return ((x < y) ? -1
				: (x > y) ? +1 : (x == y) ? 0 : Double.isNaN(x) ? (Double.isNaN(y) ? 0 : +1) : -1);
	 ...
	 * existing <tt>double</tt> array is the {@link ByteOrder#nativeOrder native
	 * order} of the underlying hardware. The byte order of a double buffer
