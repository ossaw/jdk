 ...
 * </p>
 * <p>
 * A buffer's <i>limit</i> is the index of the first element that should not be
 * read or written. A buffer's limit is never negative and is never greater than
 * its capacity.
 * </p>
 * <p>
 * A buffer's <i>position</i> is the index of the next element to be read or
 ...
 * <blockquote>
 * <p>
 * <i>Relative</i> operations read or write one or more elements starting at the
 * current position and then increment the position by the number of elements
 * transferred. If the requested transfer exceeds the limit then a relative
 * <i>get</i> operation throws a {@link BufferUnderflowException} and a relative
 * <i>put</i> operation throws a {@link BufferOverflowException}; in either
 * case, no data is transferred.
 * </p>
 * <p>
 * <i>Absolute</i> operations take an explicit element index and do not affect
 * the position. Absolute <i>get</i> and <i>put</i> operations throw an
 * {@link IndexOutOfBoundsException} if the index argument exceeds the limit.
 * </p>
 * </blockquote>
 * <p>
 ...
 * <p>
 * A buffer's <i>mark</i> is the index to which its position will be reset when
 * the {@link #reset reset} method is invoked. The mark is not always defined,
 * but when it is defined it is never negative and is never greater than the
 * position. If the mark is defined then it is discarded when the position or
 * the limit is adjusted to a value smaller than the mark. If the mark is not
 * defined then invoking the {@link #reset reset} method causes an
 * {@link InvalidMarkException} to be thrown.
 * <h2>Invariants</h2>
 * <p>
 * The following invariant holds for the mark, position, limit, and capacity
 * values:
 * <blockquote> <tt>0</tt> <tt>&lt;=</tt> <i>mark</i> <tt>&lt;=</tt>
 * <i>position</i> <tt>&lt;=</tt> <i>limit</i> <tt>&lt;=</tt>
 * <i>capacity</i> </blockquote>
 * <p>
 ...
 * constructed. Each element of a newly-allocated buffer is initialized to zero.
 ...
 * <li>
 * <p>
 * {@link #clear} makes a buffer ready for a new sequence of channel-read or
 * relative <i>put</i> operations: It sets the limit to the capacity and the
 * position to zero.
 * </p>
 * </li>
 * <li>
 * <p>
 * {@link #flip} makes a buffer ready for a new sequence of channel-write or
 ...
 * <li>
 * <p>
 * {@link #rewind} makes a buffer ready for re-reading the data that it already
 ...
 * methods of each buffer class are specified as <i>optional operations</i> that
 * will throw a {@link ReadOnlyBufferException} when invoked upon a read-only
 * buffer. A read-only buffer does not allow its content to be changed, but its
 * mark, position, and limit values are mutable. Whether or not a buffer is
 * read-only may be determined by invoking its {@link #isReadOnly isReadOnly}
 * method.
 * <h2>Thread safety</h2>
 * <p>
...
public abstract class Buffer {
    ...
    /**
     * Checks the current position against the limit, throwing a
     * {@link BufferOverflowException} if it is not smaller than the limit, and
     * then increments the position.
     *
     * @return The current position value, before it is incremented
     */
