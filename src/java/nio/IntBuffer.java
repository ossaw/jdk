 ...
 * <li>
 * <p>
 * Absolute and relative {@link #get() <i>get</i>} and {@link #put(int)
 * <i>put</i>} methods that read and write single ints;
 * </p>
 * </li>
 *
 * <li>
 * <p>
 * Relative {@link #get(int[]) <i>bulk get</i>} methods that transfer contiguous
 * sequences of ints from this buffer into an array; and
 * </p>
 * </li>
 *
 * <li>
 * <p>
 * Relative {@link #put(int[]) <i>bulk put</i>} methods that transfer contiguous
 * sequences of ints from an int array or some other int buffer into this
 * buffer;&#32;and
 * </p>
 * </li>
 *
 ...
 * {@link #slice slicing} an int buffer.
 * </p>
 * </li>
 *
 * </ul>
 *
 * <p>
 * Int buffers can be created either by {@link #allocate <i>allocation</i>},
 ...
 * 
 * 
 * content, by {@link #wrap(int[]) <i>wrapping</i>} an existing int array into a
 * buffer, or by creating a <a href="ByteBuffer.html#views"><i>view</i></a> of
 ...
 * int buffer created via the <tt>wrap</tt> methods of this class will be
...
public abstract class IntBuffer extends Buffer implements Comparable<IntBuffer> {
	 ...
	 * be the number of ints remaining in this buffer, and its mark will be
	...
	/**
	 * Relative <i>get</i> method. Reads the int at this buffer's current
	 ...
	 *
	 * <p>
	 * This method transfers ints from this buffer into the given destination
	 * array. If there are fewer ints remaining in the buffer than are required
	 * to satisfy the request, that is, if <tt>length</tt>&nbsp;<tt>&gt;</tt>
	 * &nbsp;<tt>remaining()</tt>, then no ints are transferred and a
	 * {@link BufferUnderflowException} is thrown.
	 *
	 * <p>
	 * Otherwise, this method copies <tt>length</tt> ints from this buffer into
	 ...
	 * </pre>
	 *
	 * except that it first checks that there are sufficient ints in this buffer
	 ...
	 * this buffer. If there are more ints remaining in the source buffer than
	 ...
	 * <tt>src.remaining()</tt> ints from the given buffer into this buffer,
	 ...
	 *
	 * @param src
	 *            The source buffer from which ints are to be read; must not be
	 *            this buffer
	 *
	 * @return This buffer
	 *
	 ...
	 *
	 * <p>
	 * Otherwise, this method copies <tt>length</tt> ints from the given array
	 ...
	 *
	 * <p>
	 * This method transfers the entire content of the given source int array
	...
	/**
	 * Returns the int array that backs this buffer&nbsp;&nbsp;<i>(optional
	 * operation)</i>.
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * The ints between the buffer's current position and its limit, if any, are
	 * copied to the beginning of the buffer. That is, the int at index <i>p</i>
	 * &nbsp;=&nbsp;<tt>position()</tt> is copied to index zero, the int at
	 * index <i>p</i>&nbsp;+&nbsp;1 is copied to index one, and so forth until
	 * the int at index <tt>limit()</tt>&nbsp;-&nbsp;1 is copied to index
	 ...
	 *
	 * <p>
	 * The buffer's position is set to the number of ints copied, rather than to
	 * zero, so that an invocation of this method can be followed immediately by
	 * an invocation of another relative <i>put</i> method.
	 * </p>
	 *
	 *
	 * 
	 ...
	 *
	 * <p>
	 * The hash code of a int buffer depends only upon its remaining elements;
	 ...
	 *
	 * <p>
	 * Two int buffers are compared by comparing their sequences of remaining
	 ...
	 *
	 * <p>
	 * The byte order of an int buffer created by allocation or by wrapping an
	 * existing <tt>int</tt> array is the {@link ByteOrder#nativeOrder native
	 * order} of the underlying hardware. The byte order of an int buffer
