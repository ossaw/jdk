 ...
 * <li>
 * <p>
 * Absolute and relative {@link #get() <i>get</i>} and {@link #put(float)
 * <i>put</i>} methods that read and write single floats;
 * </p>
 * </li>
 * <li>
 * <p>
 * Relative {@link #get(float[]) <i>bulk get</i>} methods that transfer
 * contiguous sequences of floats from this buffer into an array; and
 * </p>
 * </li>
 * <li>
 * <p>
 * Relative {@link #put(float[]) <i>bulk put</i>} methods that transfer
 * contiguous sequences of floats from a float array or some other float buffer
 ...
 * {@link #slice slicing} a float buffer.
 * </p>
 * </li>
 * </ul>
 * <p>
 * Float buffers can be created either by {@link #allocate <i>allocation</i>},
 * which allocates space for the buffer's
 * content, by {@link #wrap(float[]) <i>wrapping</i>} an existing float array
 ...
 * float buffer created via the <tt>wrap</tt> methods of this class will be
...
public abstract class FloatBuffer extends Buffer implements Comparable<FloatBuffer> {
     ...
     * be the number of floats remaining in this buffer, and its mark will be
     ...
     * array. If there are fewer floats remaining in the buffer than are
     * required to satisfy the request, that is, if <tt>length</tt>&nbsp;
     * <tt>&gt;</tt>&nbsp;<tt>remaining()</tt>, then no floats are transferred
     * and a {@link BufferUnderflowException} is thrown.
     * <p>
     * Otherwise, this method copies <tt>length</tt> floats from this buffer
     ...
     * </pre>
     *
     * except that it first checks that there are sufficient floats in this
     ...
     * into this buffer. If there are more floats remaining in the source buffer
     ...
     * <tt>src.remaining()</tt> floats from the given buffer into this buffer,
     ...
     *
     * @param src
     *        The source buffer from which floats are to be read; must not
     *        be this buffer
     * @return This buffer
     * @throws BufferOverflowException
     ...
     * array. If there are more floats to be copied from the array than remain
     ...
     * <p>
     * Otherwise, this method copies <tt>length</tt> floats from the given array
     ...
     * Relative bulk <i>put</i> method&nbsp;&nbsp;<i>(optional operation)</i>.
     * <p>
     * This method transfers the entire content of the given source float array
    ...
    /**
     * Returns the float array that backs this buffer&nbsp;&nbsp;<i>(optional
     * operation)</i>.
     * <p>
     ...
     * float at index <i>p</i>&nbsp;+&nbsp;1 is copied to index one, and so
     * forth until the float at index <tt>limit()</tt>&nbsp;-&nbsp;1 is copied
     ...
     * <p>
     * The hash code of a float buffer depends only upon its remaining elements;
     ...
     * This method considers two float elements {@code a} and {@code b} to be
     * equal if {@code (a == b) || (Float.isNaN(a) && Float.isNaN(b))}. The
     ...
     * {@link Float#compare(float,float)}, except that {@code -0.0} and
     * {@code 0.0} are considered to be equal. {@code Float.NaN} is considered
     * by this method to be equal to itself and greater than all other
     * {@code float} values (including {@code Float.POSITIVE_INFINITY}).
     * <p>
     ...
     * <p>
     * The byte order of a float buffer created by allocation or by wrapping an
     * existing <tt>float</tt> array is the {@link ByteOrder#nativeOrder native
     * order} of the underlying hardware. The byte order of a float buffer
