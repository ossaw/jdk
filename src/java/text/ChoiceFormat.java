...
/**
 * A <code>ChoiceFormat</code> allows you to attach a format to a range of
 * numbers. It is generally used in a <code>MessageFormat</code> for handling
 * plurals. The choice is specified with an ascending list of doubles, where
 * each item specifies a half-open interval up to the next item: <blockquote>
 * 
 * <pre>
 * X matches j if and only if limit[j] &le; X &lt; limit[j+1]
 * </pre>
 * 
 * </blockquote> If there is no match, then either the first or last index is
 * used, depending on whether the number (X) is too low or too high. If the
 * limit array is not in ascending order, the results of formatting will be
 * incorrect. ChoiceFormat also accepts <code>&#92;u221E</code> as equivalent to
 * infinity(INF).
 * <p>
 * <strong>Note:</strong> <code>ChoiceFormat</code> differs from the other
 * <code>Format</code> classes in that you create a <code>ChoiceFormat</code>
 * object with a constructor (not with a <code>getInstance</code> style factory
 * method). The factory methods aren't necessary because
 * <code>ChoiceFormat</code> doesn't require any complex setup for a given
 * locale. In fact, <code>ChoiceFormat</code> doesn't implement any locale
 * specific behavior.
 * <p>
 * When creating a <code>ChoiceFormat</code>, you must specify an array of
 * formats and an array of limits. The length of these arrays must be the same.
 * For example,
 * <ul>
 * <li><em>limits</em> = {1,2,3,4,5,6,7}<br>
 * <em>formats</em> = {"Sun","Mon","Tue","Wed","Thur","Fri","Sat"}
 * <li><em>limits</em> = {0, 1, ChoiceFormat.nextDouble(1)}<br>
 * <em>formats</em> = {"no files", "one file", "many files"}<br>
 * (<code>nextDouble</code> can be used to get the next higher double, to make
...
public class ChoiceFormat extends NumberFormat {
    ...
    public String toPattern() {
        ...
        for (int i = 0; i < choiceLimits.length; ++i) {
            ...
            // Append choiceFormats[i], using quotes if there are special
            // characters.
            // Single quotes themselves must be escaped in either case.
            String text = choiceFormats[i];
            boolean needQuote = text.indexOf('<') >= 0 || text.indexOf('#') >= 0 || text.indexOf(
                    '\u2264') >= 0 || text.indexOf('|') >= 0;
            if (needQuote)
                result.append('\'');
            if (text.indexOf('\'') < 0)
     ...
     * <code>format(double, StringBuffer, FieldPosition)</code> thus the range
    ...
    /**
     * Finds the least double greater than {@code d}. If {@code NaN}, returns
     * same value.
     * <p>
    ...
    /**
     * Finds the greatest double less than {@code d}. If {@code NaN}, returns
     * same value.
     *
     * @param d
     *        the reference value
    ...
    public boolean equals(Object obj) {
        ...
        return (Arrays.equals(choiceLimits, other.choiceLimits) && Arrays.equals(choiceFormats,
                other.choiceFormats));
    }
     ...
     * <code>choiceFormats[i]</code> if the number being formatted is greater
     * than or equal to <code>choiceLimits[i]</code> and less than
     * <code>choiceLimits[i+1]</code>.
     * 
     * @serial
     */
     ...
     * (bits < 0) { significand |= (SIGN | EXPONENT); } long exponent = bits &
     * EXPONENT; if (positive) { significand += 1; // FIXME fix overflow &
     * underflow } else { significand -= 1; // FIXME fix overflow & underflow }
     * bits = exponent | (significand & ~EXPONENT); return
     ...
     * {@code positive} is {@code false}). If {@code NaN}, returns same value.
     * Does not affect floating-point flags, provided these member functions do
     * not: Double.longBitsToDouble(long) Double.doubleToLongBits(double)
     * Double.isNaN(double)
     *
     * @param d
     *        the reference value
