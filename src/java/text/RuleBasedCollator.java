 ...
 * <ol>
 * <li>If a special collation rule controlled by a &lt;modifier&gt; is specified
 * it applies to the whole collator object.
 ...
 * put them in single quotes (e.g. ampersand =&gt; '&amp;'). Note that unquoted
 * white space characters are ignored; e.g. <code>b c</code> is treated as
 * <code>bc</code>.
 * <LI><strong>Modifier</strong>: There are currently two modifiers that turn on
 * special collation rules.
 * <UL>
 * <LI>'@' : Turns on backwards sorting of accents (secondary differences), as
 * in French.
 * <LI>'!' : Turns on Thai/Lao vowel-consonant swapping. If this rule is in
 ...
 * <p>
 * '&amp;' : Indicates that the next rule follows the position to where the
 * reset text-argument would be sorted.
 * </UL>
 *
 * <p>
 * This sounds more complicated than it is in practice. For example, the
 * following are equivalent ways of expressing the same thing: <blockquote>
 * 
 * <pre>
 * a &lt; b &lt; c
 * a &lt; b &amp; b &lt; c
 ...
 * </pre>
 * 
 * </blockquote> Notice that the order is important, as the subsequent item goes
 * immediately after the text-argument. The following are not equivalent:
 * <blockquote>
 * 
 * <pre>
 * a &lt; b &amp; a &lt; c
 ...
 * </pre>
 * 
 * </blockquote> Either the text-argument must already be present in the
 * sequence, or some initial substring of the text-argument must be present.
 * (e.g. "a &lt; b &amp; ae &lt; e" is valid since "a
 * " is present in the sequence before "ae" is reset). In this latter case, "ae"
 * is not entered and treated as a single character; instead, "e" is sorted as
 * if it were expanded to two characters: "a" followed by an "e". This
 * difference appears in natural languages: in traditional Spanish "ch" is
 * treated as though it contracts to a single character (expressed as
 * "c &lt; ch &lt; d"), while in traditional German a-umlaut is treated as
 * though it expanded to two characters (expressed as
 * "a,A &lt; b,B ... &amp;ae;&#92;u00e3&amp;AE;&#92;u00c3"). [&#92;u00e3 and
 * &#92;u00c3 are, of course, the escape sequences for a-umlaut.]
 * <p>
 ...
 * be in canonical order, you should set the collator to CANONICAL_DECOMPOSITION
 * or FULL_DECOMPOSITION to enable sorting of combining sequences. Second, if
 * the strings contain characters with compatibility decompositions (such as
 * full-width and half-width forms), you must use FULL_DECOMPOSITION, since the
 * rule tables only include canonical mappings.
 *
 * <p>
 ...
 * <UL>
 * <LI>A text-argument contains unquoted punctuation symbols (e.g.
 * "a &lt; b-c &lt; d").
 * <LI>A relation or reset character not followed by a text-argument (e.g.
 * "a &lt; ,b").
 * <LI>A reset where the text-argument (or an initial substring of the
 * text-argument) is not already in the sequence. (e.g.
 * "a &lt; b &amp; e &lt; f")
 * </UL>
 * If you produce one of these errors, a <code>RuleBasedCollator</code> throws a
 ...
 * V &lt; w, W &lt; x, X &lt; y, Y &lt; z, Z &lt; &#92;u00E6, &#92;u00C6 &lt;
 * &#92;u00F8, &#92;u00D8 &lt; &#92;u00E5 = a&#92;u030A, &#92;u00C5 =
 * A&#92;u030A; aa, AA"
 *
 * <p>
 * To create a <code>RuleBasedCollator</code> object with specialized rules
 * tailored to your needs, you construct the <code>RuleBasedCollator</code> with
 * the rules contained in a <code>String</code> object. For example:
 * <blockquote>
 * 
 * <pre>
 ...
 *
 * <p>
 * A new collation rules string can be created by concatenating rules strings.
 * For example, the rules returned by {@link #getRules()} could be concatenated
 * to combine multiple <code>RuleBasedCollator</code>s.
 *
 * <p>
...
public class RuleBasedCollator extends Collator {
	...
	// thread safety with this scheme. The CollationElementIterator is
	// responsible
	// for generating collation elements from strings and returning one element
	// at
	...
	// tables specifying the collation order of characters for a particular
	// locale
	// or use. It also contains the base logic that CollationElementIterator
	// uses to map from characters to collation elements. A single
	// RBCollationTables
	 ...
	 *                           A format exception will be thrown if the build
	 *                           process of
	 *                           the rules fails. For example, build rule "a
	 *                           &lt; ? &lt; d"
	 *                           will cause the constructor to throw the
	 *                           ParseException
	 *                           because the '?' is not quoted.
	 */
	 ...
	 *               the collation rules to build the collation table from.
	 * @param decomp
	 *               the decomposition strength used to build the collation
	 *               table
	 ...
	 *                           the rules fails. For example, build rule "a < ?
	 *                           < d" will
	 *                           cause the constructor to throw the
	 *                           ParseException because
	 *                           the '?' is not quoted.
	 */
	 ...
	 * collation rules. Returns information about whether a string is less than,
	 * greater than or equal to another string in a language. This can be
	...
	public synchronized int compare(String source, String target) {
		...
		// indicate whether the source string is less or greater than the
		// target.
		//
		...
		while (true) {
			...
			if (sOrder == tOrder) {
				if (tables.isFrenchSec() && pSOrder != 0) {
					if (!checkSecTer) {
						// in french, a secondary difference more to the right
						// is stronger,
						// so accents have to be checked with each base element
						checkSecTer = initialCheckSecTer;
						// but tertiary differences are less important than the
						// first
						// secondary difference, so checking tertiary remains
						// disabled
						checkTertiary = false;
					}
				}
			...
			if (pSOrder != pTOrder) {
				if (sOrder == 0) {
					...
					// Skip to the next source element, but don't fetch another
					// target element.
					gett = false;
					continue;
				}
				...
				// The source and target elements aren't ignorable, but it's
				// still possible
				// for the primary component of one of the elements to be
				// ignorable....

				if (pSOrder == 0) // primary order in source is ignorable
				{
					// The source's primary is ignorable, but the target's
					// isn't. We treat ignorables
					...
					// Neither of the orders is ignorable, and we already know
					// that the primary
					// orders are different because of the (pSOrder != pTOrder)
					// test above.
				...
				// primary order is the same, but complete order is different.
				// So there
				// are no base elements at this point, only ignorables (Since
				// the strings are
				// normalized)

				if (checkSecTer) {
					// a secondary or tertiary difference may still matter
					short secSOrder = CollationElementIterator.secondaryOrder(
							sOrder);
					short secTOrder = CollationElementIterator.secondaryOrder(
							tOrder);
					if (secSOrder != secTOrder) {
						// there is a secondary difference
						result = (secSOrder < secTOrder) ? Collator.LESS
								: Collator.GREATER;
						// (strength is SECONDARY)
						checkSecTer = false;
						// (even in french, only the first secondary difference
						// within
						// a base character matters)
					} else {
						if (checkTertiary) {
							// a tertiary difference may still matter
							short terSOrder = CollationElementIterator
									.tertiaryOrder(sOrder);
							short terTOrder = CollationElementIterator
									.tertiaryOrder(tOrder);
							if (terSOrder != terTOrder) {
								// there is a tertiary difference
								result = (terSOrder < terTOrder) ? Collator.LESS
										: Collator.GREATER;
		...
		if (sOrder != CollationElementIterator.NULLORDER) {
			...
			// since this point is only reached when sOrder or tOrder is
			// NULLORDER.)
			// The source string has more elements, but the target string
			// hasn't.
			do {
				if (CollationElementIterator.primaryOrder(sOrder) != 0) {
					// We found an additional non-ignorable base character in
					// the source string.
					// This is a primary difference, so the source is greater
					return Collator.GREATER; // (strength is PRIMARY)
				} else if (CollationElementIterator.secondaryOrder(
						sOrder) != 0) {
					// Additional secondary elements mean the source string is
					// greater
					if (checkSecTer) {
						result = Collator.GREATER; // (strength is SECONDARY)
						checkSecTer = false;
			...
			} while ((sOrder = sourceCursor
					.next()) != CollationElementIterator.NULLORDER);
		} else if (tOrder != CollationElementIterator.NULLORDER) {
			// The target string has more elements, but the source string
			// hasn't.
			do {
				if (CollationElementIterator.primaryOrder(tOrder) != 0)
					...
					// Additional secondary elements in the target mean the
					// source string is less
					if (checkSecTer) {
						result = Collator.LESS; // (strength is SECONDARY)
						checkSecTer = false;
					}
				}
			} while ((tOrder = targetCursor
					.next()) != CollationElementIterator.NULLORDER);
		}
	 ...
	 * with CollationKey.compareTo. This overrides
	 * java.text.Collator.getCollationKey. It can be overriden in a subclass.
	 */
	public synchronized CollationKey getCollationKey(String source) {
		//
		// The basic algorithm here is to find all of the collation elements for
		// each
		// character in the source string, convert them to a char
		// representation,
		// and put them into the collation key. But it's trickier than that.
		// Each collation element in a string has three components: primary (A
		// vs B),
		// secondary (A vs A-acute), and tertiary (A' vs a); and a primary
		// difference
		// at the end of a string takes precedence over a secondary or tertiary
		// difference earlier in the string.
		//
		// To account for this, we put all of the primary orders at the
		// beginning of the
		// string, followed by the secondary and tertiary orders, separated by
		// nulls.
		//
		// Here's a hypothetical example, with the collation element represented
		// as
		...
		//
		// To make things even trickier, secondary differences (accent marks)
		// are compared
		// starting at the *end* of the string in languages with French
		// secondary ordering.
		// But when comparing the accent marks on a single base character, they
		// are compared
		// from the beginning. To handle this, we reverse all of the accents
		// that belong
		// to each base character, then we reverse the entire string of
		// secondary orderings
		// at the end. Taking the same example above, a French collator might
		// return
		// this instead:
		//
		...
		while ((order = sourceCursor
				.next()) != CollationElementIterator.NULLORDER) {
			secOrder = CollationElementIterator.secondaryOrder(order);
			terOrder = CollationElementIterator.tertiaryOrder(order);
			if (!CollationElementIterator.isIgnorable(order)) {
				primResult.append((char) (CollationElementIterator.primaryOrder(
						order) + COLLATIONKEYOFFSET));

				if (compareSec) {
					//
					// accumulate all of the ignorable/secondary characters
					// attached
					// to a given base character
					//
					if (tables.isFrenchSec() && preSecIgnore < secResult
							.length()) {
						//
						// We're doing reversed secondary ordering and we've hit
						// a base
						// (non-ignorable) character. Reverse any secondary
						// orderings
						// that applied to the last base character. (see block
						// comment above.)
						//
						RBCollationTables.reverse(secResult, preSecIgnore,
								secResult.length());
					}
					// Remember where we are in the secondary orderings - this
					// is how far
				...
				if (compareSec && secOrder != 0)
					secResult.append((char) (secOrder + tables.getMaxSecOrder()
							+ COLLATIONKEYOFFSET));
				if (compareTer && terOrder != 0)
					terResult.append((char) (terOrder + tables.getMaxTerOrder()
							+ COLLATIONKEYOFFSET));
			}
		}
		if (tables.isFrenchSec()) {
			if (preSecIgnore < secResult.length()) {
				// If we've accumulated any secondary characters after the last
				// base character,
				// reverse them.
				RBCollationTables.reverse(secResult, preSecIgnore, secResult
						.length());
			}
			// And now reverse the entire secResult to get French secondary
			// ordering.
		...
		if (getStrength() == IDENTICAL) {
			...
			int mode = getDecomposition();
			if (mode == CANONICAL_DECOMPOSITION) {
				primResult.append(Normalizer.normalize(source,
						Normalizer.Form.NFD));
			} else if (mode == FULL_DECOMPOSITION) {
				primResult.append(Normalizer.normalize(source,
	 ...
	 * @return true if the current table-based collation object is the same as
	 *         the table-based collation object obj; false otherwise.
	 */
