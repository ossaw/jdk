...
/**
 * The <code>CollationElementIterator</code> class is used as an iterator to
 * walk through each character of an international string. Use the iterator to
 * return the ordering priority of the positioned character. The ordering
 ...
 * </pre>
 * 
 * </blockquote> The key of a character is an integer composed of primary
 * order(short), secondary order(byte), and tertiary order(byte). Java strictly
 * defines the size and signedness of its primitive data types. Therefore, the
 * static functions <code>primaryOrder</code>, <code>secondaryOrder</code>, and
 * <code>tertiaryOrder</code> return <code>int</code>, <code>short</code>, and
 * <code>short</code> respectively to ensure the correctness of the key value.
 *
 * <p>
 ...
 *
 * <p>
 * <code>CollationElementIterator.next</code> returns the collation order of the
 * next character. A collation order consists of primary order, secondary order
 * and tertiary order. The data type of the collation order is
 * <strong>int</strong>. The first 16 bits of a collation order is its primary
 * order; the next 8 bits is the secondary order and the last 8 bits is the
 * tertiary order.
 *
 * <p>
 * <b>Note:</b> <code>CollationElementIterator</code> is a part of
 * <code>RuleBasedCollator</code> implementation. It is only usable with
 * <code>RuleBasedCollator</code> instances.
 *
 * @see Collator
...
public final class CollationElementIterator {
	...
	CollationElementIterator(String sourceText, RuleBasedCollator owner) {
		...
		if (sourceText.length() != 0) {
			NormalizerBase.Mode mode = CollatorUtilities.toNormalizerMode(owner
					.getDecomposition());
	 ...
	 * will return the first collation element in the string.
	 */
	public void reset() {
		if (text != null) {
			text.reset();
	...
	public int next() {
		...
		NormalizerBase.Mode ownerMode = CollatorUtilities.toNormalizerMode(owner
				.getDecomposition());
		...
		if (ordering.isSEAsianSwapping()) {
			...
			if (isThaiPreVowel(ch)) {
				...
				if (isThaiBaseConsonant(consonant)) {
					buffer = makeReorderedBuffer(consonant, value, buffer,
							true);
					value = buffer[0];
					expIndex = 1;
	 ...
	 * collation element corresponding to the specified position in the text. If
	 * that position is in the middle of a contracting character sequence, the
	 * result of the next call to next() is the collation element for that
	 * sequence. This means that getOffset() is not guaranteed to return the
	 * same value as was passed to a preceding call to setOffset().
	 *
	 * @param newOffset
	 *                  The new character offset into the original text.
	 * @since 1.2
	 */
	@SuppressWarnings("deprecation") // getBeginIndex, getEndIndex and setIndex
										// are deprecated
	public void setOffset(int newOffset) {
		if (text != null) {
			if (newOffset < text.getBeginIndex() || newOffset >= text
					.getEndIndex()) {
				text.setIndexOnly(newOffset);
			} else {
				int c = text.setIndex(newOffset);

				// if the desired character isn't used in a contracting
				// character
				// sequence, bypass all the backing-up logic-- we're sitting on
				// the right character already
				if (ordering.usedInContractSeq(c)) {
					// walk backwards through the string until we see a
					// character
	...
	/**
	 * Returns the character offset in the original text corresponding to the
	 * next collation element. (That is, getOffset() returns the position in the
	 * text corresponding to the collation element that will be returned by the
	 * next call to next().) This value will always be the index of the FIRST
	 * character corresponding to the collation element (a contracting character
	 * sequence is when two or more characters all correspond to the same
	 * collation element). This means if you do setOffset(x) followed
	 * immediately by getOffset(), getOffset() won't necessarily return x.
	 *
	...
	/**
	 * Determine if a character is a Lao vowel (which sorts after its base
	 * consonant).
	 */
	...
	/**
	 * This method produces a buffer which contains the collation elements for
	 * the two characters, with colFirst's values preceding another character's.
	 * Presumably, the other character precedes colFirst in logical order
	 * (otherwise you wouldn't need this method would you?). The assumption is
	 * that the other char's value(s) have already been computed. If this char
	 * has a single element it is passed to this method as lastValue, and
	 * lastExpansion is null. If it has an expansion it is passed in
	 * lastExpansion, and colLastValue is ignored.
	 */
	private int[] makeReorderedBuffer(int colFirst, int lastValue,
			int[] lastExpansion, boolean forward) {

		int[] result;

		int firstValue = ordering.getUnicodeOrder(colFirst);
		if (firstValue >= RuleBasedCollator.CONTRACTCHARINDEX) {
			firstValue = forward ? nextContractChar(colFirst)
					: prevContractChar(colFirst);
		}
		...
		if (firstExpansion == null && lastExpansion == null) {
			...
			int firstLength = firstExpansion == null ? 1
			...
			if (lastExpansion == null) {
				...
				System.arraycopy(lastExpansion, 0, result, firstLength,
						lastLength);
			}
		}
	 ...
	 * @return the next contracting character's ordering. Returns NULLORDER if
	...
	private int nextContractChar(int ch) {
		...
		// find out the length of the longest contracting character sequence in
		// the list.
		// There's logic in the builder code to make sure the longest sequence
		// is always
		// the last.
		pair = list.lastElement();
		int maxLength = pair.entryName.length();

		// (the Normalizer is cloned here so that the seeking we do in the next
		// loop
		// won't affect our real position in the text)
		NormalizerBase tempText = (NormalizerBase) text.clone();

		// extract the next maxLength characters in the string (we have to do
		// this using the
		// Normalizer to ensure that our offsets correspond to those the rest of
		// the
		...
		// now that we have that fragment, iterate through this list looking for
		// the
		// longest sequence that matches the characters in the actual text.
		// (maxLength
		// is used here to keep track of the length of the longest sequence)
		// Upon exit from this loop, maxLength will contain the length of the
		// matching
		// sequence and order will contain the collation-element value
		// corresponding
		// to this sequence
		maxLength = 1;
		for (int i = list.size() - 1; i > 0; i--) {
			...
			if (fragment.startsWith(pair.entryName) && pair.entryName
					.length() > maxLength) {
		...
		// seek our current iteration position to the end of the matching
		// sequence
		// and return the appropriate collation-element value (if there was no
		// matching
		// sequence, we're already seeked to the right position and order
		// already contains
	...
	private int prevContractChar(int ch) {
		...
		// switched things so that the next() and previous() calls on the
		// Normalizer
		// are switched and so that we skip entry pairs with the fwd flag turned
		// on
		// rather than off. Notice that we still use append() and startsWith()
		// when
		// working on the fragment. This is because the entry pairs that are
		// used
