...
/**
 * A <code>CollationKey</code> represents a <code>String</code> under the rules
 * of a specific <code>Collator</code> object. Comparing two
 ...
 * that can be compared bitwise. This allows fast comparisons once the keys are
 * generated. The cost of generating keys is recouped in faster comparisons when
 * <code>String</code>s need to be compared many times. On the other hand, the
 * result of a comparison is often determined by the first couple of characters
 * of each <code>String</code>. <code>Collator.compare</code> examines only as
 * many characters as it needs which allows it to be faster when doing single
 * comparisons.
 * <p>
 * The following example shows how <code>CollationKey</code>s might be used to
 * sort a list of <code>String</code>s. <blockquote>
 * 
 * <pre>
 * {
 *     &#64;code
