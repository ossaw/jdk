...
/**
 * {@code DateFormat} is an abstract class for date/time formatting subclasses
 * which formats and parses dates or time in a language-independent manner. The
 * date/time formatting subclass, such as {@link SimpleDateFormat}, allows for
 * formatting (i.e., date &rarr; text), parsing (text &rarr; date), and
 * normalization. The date is represented as a <code>Date</code> object or as
 * the milliseconds since January 1, 1970, 00:00:00 GMT.
 *
 * <p>
 * {@code DateFormat} provides many class methods for obtaining default
 * date/time formatters based on the default or a given locale and a number of
 * formatting styles. The formatting styles include {@link #FULL}, {@link #LONG}
 * , {@link #MEDIUM}, and {@link #SHORT}. More detail and examples of using
 * these styles are provided in the method descriptions.
 *
 * <p>
 ...
 * {@link #getDateInstance(int, Locale) getDateInstance()}. <blockquote>
 * 
 * <pre>
 * {
 *     &#64;code
 ...
 * There are other static factory methods available. Use {@code getTimeInstance}
 * to get the time format for that country. Use {@code getDateTimeInstance} to
 * get a date and time format. You can pass in different options to these
 * factory methods to control the length of the result; from {@link #SHORT} to
 * {@link #MEDIUM} to {@link #LONG} to {@link #FULL}. The exact result depends
 * on the locale, but generally:
 * <ul>
 * <li>{@link #SHORT} is completely numeric, such as {@code 12.13.52} or
 * {@code 3:30pm}
 * <li>{@link #MEDIUM} is longer, such as {@code Jan 12, 1952}
 * <li>{@link #LONG} is longer, such as {@code January 12, 1952} or
 * {@code 3:30:32pm}
 ...
 * control), you can try casting the {@code DateFormat} you get from the factory
 * methods to a {@link SimpleDateFormat}. This will work for the majority of
 * countries; just remember to put it in a {@code try} block in case you
 * encounter an unusual one.
 *
 * <p>
 * You can also use forms of the parse and format methods with
 * {@link ParsePosition} and {@link FieldPosition} to allow you to
 * <ul>
...
public abstract class DateFormat extends Format {
     ...
     *
     * <p>
     * Subclasses should initialize this field to a {@link Calendar} appropriate
     * for the {@link Locale} associated with this <code>DateFormat</code>.
     * 
     * @serial
     */
     ...
     *                      returned
     *                      string. On input: an alignment field, if desired. On
     *                      output:
     *                      the offsets of the alignment field. For example,
     *                      given a time
     *                      text "1996.07.10 AD at 15:08:56 PDT", if the given
     *                      fieldPosition is DateFormat.YEAR_FIELD, the begin
     *                      index and
     *                      end index of fieldPosition will be set to 0 and 4,
     *                      respectively. Notice that if the same time field
     *                      appears more
     *                      than once in a pattern, the fieldPosition will be
     *                      set for the
     *                      first occurrence of that time field. For instance,
     *                      formatting
     *                      a Date to the time string "1 PM PDT (Pacific
     *                      Daylight Time)"
     *                      using the pattern "h a z (zzzz)" and the alignment
     *                      field
     *                      DateFormat.TIMEZONE_FIELD, the begin index and end
     *                      index of
     *                      fieldPosition will be set to 5 and 8, respectively,
     *                      for the
    ...
    public final StringBuffer format(Object obj, StringBuffer toAppendTo,
        ...
        if (obj instanceof Date)
            ...
            return format(new Date(((Number) obj).longValue()), toAppendTo,
                    fieldPosition);
        else
     ...
     * @param toAppendTo
     *                      the string buffer for the returning date/time
     *                      string.
     * @param fieldPosition
     *                      keeps track of the position of the field within the
     *                      returned
    ...
    public final String format(Date date) {
        return format(date, new StringBuffer(), DontCareFieldPosition.INSTANCE)
                .toString();
    }
     ...
     * example, a time text {@code "07/10/96 4:5 PM, PDT"} will be parsed into a
     * {@code Date} that is equivalent to {@code Date(837039900000L)}.
     *
     * <p>
     * By default, parsing is lenient: If the input is not in the form used by
     * this object's format method but can still be parsed as a date, then the
     * parse succeeds. Clients may insist on strict adherence to the format by
     * calling {@link #setLenient(boolean) setLenient(false)}.
     *
     * <p>
     * This parsing operation uses the {@link #calendar} to produce a
     * {@code Date}. As a result, the {@code calendar}'s date-time fields and
     * the {@code TimeZone} value may have been overwritten, depending on
     * subclass implementations. Any {@code
     ...
     * <p>
     * This is equivalent to calling {@link #getTimeInstance(int, Locale)
     ...
     * 
     * @param style
     *                the given formatting style. For example, SHORT for "h:mm
     *                a" in
     *                the US locale.
     * @param aLocale
     *                the given locale.
     ...
     * {@link java.util.Locale.Category#FORMAT FORMAT} locale.
     * <p>
     * This is equivalent to calling {@link #getDateInstance(int, Locale)
     ...
     * 
     * @param style
     *                the given formatting style. For example, SHORT for
     *                "M/d/yy" in
     *                the US locale.
     * @param aLocale
     *                the given locale.
     ...
     * {@link #getDateTimeInstance(int, int, Locale)
     ...
     * styles for the default {@link java.util.Locale.Category#FORMAT FORMAT}
     * locale.
     * <p>
     * This is equivalent to calling
     * {@link #getDateTimeInstance(int, int, Locale)
     ...
     * @see java.util.Locale.Category#FORMAT
     * @param dateStyle
     *                  the given date formatting style. For example, SHORT for
     *                  "M/d/yy" in the US locale.
     * @param timeStyle
     *                  the given time formatting style. For example, SHORT for
     ...
     * installed {@link java.text.spi.DateFormatProvider DateFormatProvider}
     * implementations. It must contain at least a <code>Locale</code> instance
     * equal to {@link java.util.Locale#US Locale.US}.
     *
     * @return An array of locales for which localized <code>DateFormat</code>
     ...
     *
     * <p>
     * Any {@link java.util.TimeZone TimeZone} and {@linkplain #isLenient()
     * leniency} values that have previously been set are overwritten by
     * {@code newCalendar}'s values.
     *
     * @param newCalendar
     ...
     *
     * <p>
     * The {@code TimeZone} set by this method may be overwritten as a result of
     * a call to the parse method.
     *
     * @param zone
     ...
     * {@link #setCalendar(java.util.Calendar) setCalendar()}.
     *
     * @param lenient
    ...
    /**
     * Tell whether date/time parsing is to be lenient. This method is
     * equivalent to the following call. <blockquote>
     * 
     * <pre>
     * {@code
     ...
     * </blockquote>
     *
     * @return {@code true} if the {@link #calendar} is lenient; {@code false}
     *         otherwise.
    ...
    public boolean equals(Object obj) {
        ...
        return (// calendar.equivalentTo(other.calendar) // THIS API DOESN'T
                // EXIST YET!
        calendar.getFirstDayOfWeek() == other.calendar.getFirstDayOfWeek()
                && calendar.getMinimalDaysInFirstWeek() == other.calendar
                        .getMinimalDaysInFirstWeek() && calendar
                                .isLenient() == other.calendar.isLenient()
     ...
     * 
     * @param timeStyle
     *                  a value from 0 to 3 indicating the time format, ignored
     *                  if
     *                  flags is 2
     * @param dateStyle
     *                  a value from 0 to 3 indicating the time format, ignored
     ...
     *                  flags is 1
     * @param flags
     *                  either 1 for a time format, 2 for a date format, or 3
     *                  for a
     *                  date/time format
     * @param loc
    ...
    private static DateFormat get(int timeStyle, int dateStyle, int flags,
        ...
        if (dateFormat == null) {
            dateFormat = get(LocaleProviderAdapter.forJRE(), timeStyle,
                    dateStyle, loc);
        }
        return dateFormat;
    }

    private static DateFormat get(LocaleProviderAdapter adapter, int timeStyle,
            int dateStyle, Locale loc) {
        ...
        if (timeStyle == -1) {
            ...
            if (dateStyle == -1) {
                ...
                dateFormat = provider.getDateTimeInstance(dateStyle, timeStyle,
                        loc);
            }
        }
        return dateFormat;
     ...
     * <code>DateFormat.formatToCharacterIterator</code> and as field
    ...
    public static class Field extends Format.Field {
        ...
        /**
         * Returns the <code>Field</code> constant that corresponds to the
         * <code>Calendar</code> constant <code>calendarField</code>. If there
         * is no direct mapping between the <code>Calendar</code> constant and a
         * <code>Field</code>, null is returned.
         *
         * @throws IllegalArgumentException
         *                                  if <code>calendarField</code> is not
         *                                  the value of a
        ...
        public static Field ofCalendarField(int calendarField) {
            if (calendarField < 0
                    || calendarField >= calendarToFieldMapping.length) {
         ...
         *                      <code>Field</code>
         *                      corresponds to; any value, even one outside the
         *                      range of
         *                      legal <code>Calendar</code> values may be used,
         *                      but
         *                      <code>-1</code> should be used for values that
         *                      don't
         ...
         * attribute. For example, if this represents the hours field of a
         * <code>Calendar</code>, this would return <code>Calendar.HOUR</code>.
         * If there is no corresponding <code>Calendar</code> constant, this
         * will return -1.
         *
        ...
        /**
         * Constant identifying the time of day indicator (e.g. "a.m." or
         * "p.m.") field.
         */
        public final static Field AM_PM = new Field("am pm", Calendar.AM_PM);

        /**
         * Constant identifying the hour field, where the legal values are 1 to
         * 12.
         */
        public final static Field HOUR1 = new Field("hour 1", -1);

        /**
         * Constant identifying the hour field, where the legal values are 0 to
         * 11.
         */
