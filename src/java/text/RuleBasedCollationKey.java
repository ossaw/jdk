...
final class RuleBasedCollationKey extends CollationKey {
	 ...
	 * <strong>Note:</strong> RuleBasedCollationKeys created by different
	...
	/**
	 * Compare this RuleBasedCollationKey and the target for equality. The
	 * collation rules of the Collator object which created these keys are
	 * applied. <strong>Note:</strong> RuleBasedCollationKeys created by
	...
	/**
	 * Creates a hash code for this RuleBasedCollationKey. The hash value is
	 * calculated on the key itself, not the String from which the key was
	 * created. Thus if x and y are RuleBasedCollationKeys, then x.hashCode(x)
	 * == y.hashCode() if x.equals(y) is true. This allows language-sensitive
	 * comparison in a hash table. See the CollatinKey class description for an
	 * example.
	 * 
	...
	/**
	 * Converts the RuleBasedCollationKey to a sequence of bits. If two
	 * RuleBasedCollationKeys could be legitimately compared, then one could
	 * compare the byte arrays for each of those keys to obtain the same result.
	 * Byte arrays are organized most significant byte first.
	 */
