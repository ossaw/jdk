...
final class RBCollationTables {
	// ===========================================================================================
	// The following diagram shows the data structure of the RBCollationTables
	// object.
	// Suppose we have the rule, where 'o-umlaut' is the unicode char 0x00F6.
	// "a, A < b, B < c, C, ch, cH, Ch, CH < d, D ... < o, O; 'o-umlaut'/E,
	// 'O-umlaut'/E ...".
	...
	final class BuildAPI {
		/**
		 * Private constructor. Prevents anyone else besides RBTableBuilder from
		 * gaining direct access to the internals of this class.
		 */
		private BuildAPI() {
		}

		/**
		 * This function is used by RBTableBuilder to fill in all the members of
		 * this object. (Effectively, the builder class functions as a "friend"
		 * of this class, but to avoid changing too much of the logic, it
		 * carries around "shadow" copies of all these variables until the end
		 * of the build process and then copies them en masse into the actual
		 * tables object once all the construction logic is complete. This
		 ...
		 *            mapping)
		 * @param cTbl
		 *            The collator's contracting-character table (the value for
		 *            contractTable)
		 * @param eTbl
		...
		void fillInTables(boolean f2ary, boolean swap, UCompactIntArray map,
				Vector<Vector<EntryPair>> cTbl, Vector<int[]> eTbl, IntHashtable cFlgs, short mso,
				short mto) {
			frenchSec = f2ary;
