...
/**
 * <code>SimpleDateFormat</code> is a concrete class for formatting and parsing
 * dates in a locale-sensitive manner. It allows for formatting (date &rarr;
 * text), parsing (text &rarr; date), and normalization.
 *
 * <p>
 ...
 * to create a date-time formatter with either <code>getTimeInstance</code>,
 * <code>getDateInstance</code>, or <code>getDateTimeInstance</code> in
 * <code>DateFormat</code>. Each of these class methods can return a date/time
 * formatter initialized with a default format pattern. You may modify the
 * format pattern using the <code>applyPattern</code> methods as desired. For
 * more information on using these methods, see {@link DateFormat}.
 *
 ...
 * <code>'</code>) to avoid interpretation. <code>"''"</code> represents a
 * single quote. All other characters are not interpreted; they're simply copied
 * into the output string during formatting or matched against the input string
 * during parsing.
 * <p>
 ...
 * "Chart shows pattern letters, date/time component, presentation, and
 * examples."
 * >
 ...
 * <ul>
 * <li><strong><a name="text">Text:</a></strong> For formatting, if the number
 * of pattern letters is 4 or more, the full form is used; otherwise a short or
 * abbreviated form is used if available. For parsing, both forms are accepted,
 * independent of the number of pattern letters.<br>
 * <br>
 * </li>
 * <li><strong><a name="number">Number:</a></strong> For formatting, the number
 * of pattern letters is the minimum number of digits, and shorter numbers are
 * zero-padded to this amount. For parsing, the number of pattern letters is
 * ignored unless it's needed to separate two adjacent fields.<br>
 * <br>
 * </li>
 ...
 * <li>For parsing, if the number of pattern letters is more than 2, the year is
 * interpreted literally, regardless of the number of digits. So using the
 * pattern "MM/dd/yyyy", "01/11/12" parses to Jan 11, 12 A.D.
 * <li>For parsing with the abbreviated year pattern ("y" or "yy"),
 * <code>SimpleDateFormat</code> must interpret the abbreviated year relative to
 * some century. It does this by adjusting dates to be within 80 years before
 * and 20 years after the time the <code>SimpleDateFormat</code> instance is
 * created. For example, using a pattern of "MM/dd/yy" and a
 * <code>SimpleDateFormat</code> instance created on Jan 1, 1997, the string
 * "01/11/12" would be interpreted as Jan 11, 2012 while the string "05/04/64"
 * would be interpreted as May 4, 1964. During parsing, only strings consisting
 * of exactly two digits, as defined by {@link Character#isDigit(char)}, will be
 * parsed into the default century. Any other numeric string, such as a one
 * digit string, a three or more digit string, or a two digit string that isn't
 * all digits (for example, "-1"), is interpreted literally. So "01/02/3" or
 * "01/02/003" are parsed, using the same pattern, as Jan 2, 3 AD. Likewise,
 * "01/02/-3" is parsed as Jan 2, 4 BC.
 * </ul>
 * Otherwise, calendar system specific forms are applied. For both formatting
 * and parsing, if the number of pattern letters is 4 or more, a calendar
 * specific {@linkplain Calendar#LONG long form} is used. Otherwise, a calendar
 ...
 * calendar} doesn't support any
 * <a href="../util/GregorianCalendar.html#week_year"> week years</a>, the
 * calendar year ({@code 'y'}) is used instead. The support of week years can be
 * tested with a call to {@link DateFormat#getCalendar() getCalendar()}.
 * {@link java.util.Calendar#isWeekDateSupported() isWeekDateSupported()}.<br>
 * <br>
 * </li>
 * <li><strong><a name="month">Month:</a></strong> If the number of pattern
 * letters is 3 or more, the month is interpreted as <a href="#text">text</a>;
 * otherwise, it is interpreted as a <a href="#number">number</a>.<br>
 * <ul>
 ...
 * explicitly with constructor
 * {@link #SimpleDateFormat(String, DateFormatSymbols)} or method
 * {@link #setDateFormatSymbols(DateFormatSymbols)}, the month names given by
 * the {@code DateFormatSymbols} are used.</li>
 * <li>Letter <em>L</em> produces the standalone form of month names.</li>
 * </ul>
 * <br>
 * </li>
 * <li><strong><a name="timezone">General time zone:</a></strong> Time zones are
 * interpreted as <a href="#text">text</a> if they have names. For time zones
 ...
 * </pre>
 * 
 * <i>Hours</i> must be between 0 and 23, and <i>Minutes</i> must be between 00
 * and 59. The format is locale independent and digits must be taken from the
 ...
 * <li><strong><a name="iso8601timezone">ISO 8601 Time zone:</a></strong> The
 * number of pattern letters designates the format for both formatting and
 * parsing as follows:
 * 
 * <pre>
 *     <i>ISO8601TimeZone:</i>
 ...
 *
 * <p>
 * For formatting, if the offset value from GMT is 0, {@code "Z"} is produced.
 * If the number of pattern letters is 1, any fraction of an hour is ignored.
 * For example, if the pattern is {@code "X"} and the time zone is
 ...
 * {@link IllegalArgumentException} is thrown when constructing a {@code
...
public class SimpleDateFormat extends DateFormat {
     ...
     * <code>defaultCenturyStart</code> on stream.
    ...
    /**
     * True when a negative sign follows a number. (True as default in Arabic.)
     */
     ...
     * <code>defaultCenturyStart</code>, which may be any date. May not be null.
     * 
     * @serial
     * @since JDK1.1.4
     */
     ...
     */
    private static final ConcurrentMap<Locale, NumberFormat> cachedNumberFormatData = new ConcurrentHashMap<>(
            3);

    /**
     * The Locale used to instantiate this <code>SimpleDateFormat</code>. The
     * value may be null if this object has been created by an older
     * <code>SimpleDateFormat</code> and deserialized.
     *
     * @serial
     * @since 1.6
     */
    ...
    /**
     * Indicates whether this <code>SimpleDateFormat</code> should use the
     * DateFormatSymbols. If true, the format and parse methods use the
     * DateFormatSymbols values. If false, the format and parse methods call
     * Calendar.getDisplayName or Calendar.getDisplayNames.
     */
     ...
     * {@link java.util.Locale.Category#FORMAT FORMAT} locale. <b>Note:</b> This
     * constructor may not support all locales. For full coverage, use the
     * factory methods in the {@link DateFormat} class.
     */
    public SimpleDateFormat() {
        this("", Locale.getDefault(Locale.Category.FORMAT));
        applyPatternImpl(LocaleProviderAdapter.getResourceBundleBased()
                .getLocaleResources(locale).getDateTimePattern(SHORT, SHORT,
                        calendar));
    }
     ...
     * factory methods in the {@link DateFormat} class.
     * <p>
     * This is equivalent to calling {@link #SimpleDateFormat(String, Locale)
    ...
    /**
     * Constructs a <code>SimpleDateFormat</code> using the given pattern and
     * the default date format symbols for the given locale. <b>Note:</b> This
     * constructor may not support all locales. For full coverage, use the
     * factory methods in the {@link DateFormat} class.
     *
     * @param pattern
     ...
     * @exception NullPointerException
     *                                     if the given pattern or locale is
     *                                     null
     * @exception IllegalArgumentException
     *                                     if the given pattern is invalid
     */
     ...
     * @exception NullPointerException
     *                                     if the given pattern or formatSymbols
     *                                     is null
     * @exception IllegalArgumentException
     *                                     if the given pattern is invalid
     */
    ...
    private void initializeCalendar(Locale loc) {
        if (calendar == null) {
            assert loc != null;
            // The format object must be constructed using the symbols for this
            // zone.
            // However, the calendar should use the current default TimeZone.
            // If this is not contained in the locale zone strings, then the
            // zone
     ...
     * compiled pattern is: <blockquote> CompiledPattern: EntryList EntryList:
     * Entry EntryList Entry Entry: TagField TagField data TagField: Tag Length
     * TaggedData Tag: pattern_char_index TAG_QUOTE_CHARS Length: short_length
     * long_length TaggedData: TAG_QUOTE_ASCII_CHAR ascii_char
     *
     * </blockquote>
     *
     * where `short_length' is an 8-bit unsigned integer between 0 and 254.
     * `long_length' is a sequence of an 8-bit integer 255 and a 32-bit signed
     * integer value which is split into upper and lower 16-bit fields in two
     * char's. `pattern_char_index' is an 8-bit integer between 0 and 18.
     * `ascii_char' is an 7-bit ASCII character value. `data' depends on its Tag
     * value.
     * <p>
     * If Length is short_length, Tag and short_length are packed in a single
     * char, as illustrated below. <blockquote> char[0] = (Tag << 8) |
     * short_length; </blockquote>
     *
     * If Length is long_length, Tag and 255 are packed in the first char and a
     * 32-bit integer, as illustrated below. <blockquote> char[0] = (Tag << 8) |
     * 255; char[1] = (char) (long_length >>> 16); char[2] = (char) (long_length
     * & 0xffff); </blockquote>
     * <p>
     * If Tag is a pattern_char_index, its Length is the number of pattern
     * characters. For example, if the given pattern is "yyyy", Tag is 1 and
     * Length is 4, followed by no data.
     * <p>
     * If Tag is TAG_QUOTE_CHARS, its Length is the number of char's following
     * the TagField. For example, if the given pattern is "'o''clock'", Length
     * is 7 followed by a char sequence of
     * <code>o&nbs;'&nbs;c&nbs;l&nbs;o&nbs;c&nbs;k</code>.
     * <p>
     * TAG_QUOTE_ASCII_CHAR is a special tag and has an ASCII character in place
     * of Length. For example, if the given pattern is "'o'", the TaggedData
     * entry is <code>((TAG_QUOTE_ASCII_CHAR&nbs;<<&nbs;8)&nbs;|&nbs;'o')</code>
     * .
     *
     * @exception NullPointerException
     *                                     if the given pattern is null
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (c == '\'') {
                ...
                if (!inQuote) {
                    ...
                    if (len == 1) {
                        ...
                        if (ch < 128) {
                            compiledCode.append(
                                    (char) (TAG_QUOTE_ASCII_CHAR << 8 | ch));
                        } else {
                            compiledCode.append((char) (TAG_QUOTE_CHARS << 8
                                    | 1));
            ...
            if (!(c >= 'a' && c <= 'z' || c >= 'A' && c <= 'Z')) {
                ...
                if (c < 128) {
                    ...
                    compiledCode.append((char) (TAG_QUOTE_CHARS << 8 | (j
                            - i)));
                    for (; i < j; i++) {
    ...
    private static void encode(int tag, int length, StringBuilder buffer) {
        if (tag == PATTERN_ISO_ZONE && length >= 4) {
            throw new IllegalArgumentException(
                    "invalid ISO 8601 format: length=" + length);
        }
        if (length < 255) {
    ...
    /*
     * Initialize the fields we use to disambiguate ambiguous years. Separate so
     * we can call it from readObject().
     */
     ...
     *
     * @param date
     *                   the date-time value to be formatted into a date-time
     *                   string.
     * @param toAppendTo
     ...
     *                                     object, or if the
     *                                     Format's pattern string is invalid.
     * @param obj
    ...
    private static final int[] PATTERN_INDEX_TO_CALENDAR_FIELD = { Calendar.ERA,
            Calendar.YEAR, Calendar.MONTH, Calendar.DATE, Calendar.HOUR_OF_DAY,
            Calendar.HOUR_OF_DAY, Calendar.MINUTE, Calendar.SECOND,
            Calendar.MILLISECOND, Calendar.DAY_OF_WEEK, Calendar.DAY_OF_YEAR,
            Calendar.DAY_OF_WEEK_IN_MONTH, Calendar.WEEK_OF_YEAR,
            Calendar.WEEK_OF_MONTH, Calendar.AM_PM, Calendar.HOUR,
            Calendar.HOUR, Calendar.ZONE_OFFSET, Calendar.ZONE_OFFSET,
            CalendarBuilder.WEEK_YEAR, // Pseudo
            // Calendar
            // field
    ...
    private static final int[] PATTERN_INDEX_TO_DATE_FORMAT_FIELD = {
            ...
            DateFormat.HOUR0_FIELD, DateFormat.TIMEZONE_FIELD,
            DateFormat.TIMEZONE_FIELD, DateFormat.YEAR_FIELD,
            DateFormat.DAY_OF_WEEK_FIELD, DateFormat.TIMEZONE_FIELD,
    ...
    private static final Field[] PATTERN_INDEX_TO_DATE_FORMAT_FIELD_ID = {
            ...
            Field.HOUR_OF_DAY1, Field.HOUR_OF_DAY0, Field.MINUTE, Field.SECOND,
            Field.MILLISECOND, Field.DAY_OF_WEEK, Field.DAY_OF_YEAR,
            Field.DAY_OF_WEEK_IN_MONTH, Field.WEEK_OF_YEAR, Field.WEEK_OF_MONTH,
            Field.AM_PM, Field.HOUR1, Field.HOUR0, Field.TIME_ZONE,
            Field.TIME_ZONE, Field.YEAR, Field.DAY_OF_WEEK, Field.TIME_ZONE,
            Field.MONTH };
    ...
    private void subFormat(int patternCharIndex, int count,
        ...
        if (field == CalendarBuilder.WEEK_YEAR) {
            ...
            value = CalendarBuilder.toISODayOfWeek(calendar.get(
        ...
        switch (patternCharIndex) {
            ...
            case PATTERN_YEAR: // 'y'
                if (calendar instanceof GregorianCalendar) {
                    ...
                    if (current == null) {
                        zeroPaddingNumber(value, style == Calendar.LONG ? 1
            ...
            case PATTERN_MONTH: // 'M' (context seinsive)
                if (useDateFormatSymbols) {
                    ...
                    if (count < 3) {
                        ...
                        current = calendar.getDisplayName(field, style | 0x8000,
                                locale);
                        if (current == null) {
                            current = calendar.getDisplayName(field, style,
                                    locale);
                        }
                    }
                }
            ...
            case PATTERN_HOUR_OF_DAY1: // 'k' 1-based. eg, 23:59 + 1 hour =>> 24:59
                if (current == null) {
                    if (value == 0) {
                        zeroPaddingNumber(calendar.getMaximum(
                                Calendar.HOUR_OF_DAY) + 1, count, maxIntCount,
                                buffer);
                    } else {
                        zeroPaddingNumber(value, count, maxIntCount, buffer);
                    }
            ...
            case PATTERN_HOUR1: // 'h' 1-based. eg, 11PM + 1 hour =>> 12 AM
                if (current == null) {
                    if (value == 0) {
                        zeroPaddingNumber(calendar.getLeastMaximum(
                                Calendar.HOUR) + 1, count, maxIntCount, buffer);
                    } else {
                        zeroPaddingNumber(value, count, maxIntCount, buffer);
                    }
                }
            ...
            case PATTERN_ZONE_NAME: // 'z'
                if (current == null) {
                    if (formatData.locale == null
                            || formatData.isZoneStringsSet) {
                        int zoneIndex = formatData.getZoneIndex(calendar
                                .getTimeZone().getID());
                        if (zoneIndex == -1) {
                            ...
                            int index = (calendar.get(Calendar.DST_OFFSET) == 0)
                                    ? 1
                                    : 3;
                            if (count < 4) {
                                // Use the short name
                            ...
                            String[][] zoneStrings = formatData
                                    .getZoneStringsWrapper();
                            buffer.append(zoneStrings[zoneIndex][index]);
                        }
                    } else {
                        TimeZone tz = calendar.getTimeZone();
                        boolean daylight = (calendar.get(
                                Calendar.DST_OFFSET) != 0);
                        int tzstyle = (count < 4 ? TimeZone.SHORT
                                : TimeZone.LONG);
            ...
            case PATTERN_ZONE_VALUE: // 'Z' ("-/+hhmm" form)
                value = (calendar.get(Calendar.ZONE_OFFSET) + calendar.get(
                        Calendar.DST_OFFSET)) / 60000;
            ...
            default:
                ...
                // case PATTERN_HOUR_OF_DAY0: // 'H' 0-based. eg, 23:59 + 1 hour =>>
                // 00:59
                ...
                // case PATTERN_ISO_DAY_OF_WEEK: // 'u' pseudo field, Monday = 1,
                // ..., Sunday = 7
                if (current == null) {
                    zeroPaddingNumber(value, count, maxIntCount, buffer);
                }
    ...
    private void zeroPaddingNumber(int value, int minDigits, int maxDigits,
        ...
        numberFormat.format((long) value, buffer,
     ...
     * parsed date is returned. The updated <code>pos</code> can be used to
     ...
     *
     * <p>
     * This parsing operation uses the {@link DateFormat#calendar calendar} to
     * produce a {@code Date}. All of the {@code
     * calendar}'s date-time fields are {@linkplain Calendar#clear() cleared}
     * before parsing, and the {@code calendar}'s default values of the
     * date-time fields are used for any missing date-time information. For
     * example, the year value of the parsed {@code Date} is 1970 with
     * {@link GregorianCalendar} if no year value is given from the parsing
     * operation. The {@code
     * TimeZone} value may be overwritten, depending on the given pattern and
     * the time zone value in {@code text}. Any {@code
     * TimeZone} value that has previously been set by a call to
     * {@link #setTimeZone(java.util.TimeZone) setTimeZone} may need to be
     * restored for further operations.
     *
     * @param text
    ...
    public Date parse(String text, ParsePosition pos) {
        ...
        for (int i = 0; i < compiledPattern.length;) {
            ...
            switch (tag) {
                case TAG_QUOTE_ASCII_CHAR:
                    if (start >= textLength || text.charAt(
                            start) != (char) count) {
                        pos.index = oldStart;
        ...
        try {
            ...
            if (ambiguousYear[0]) {
                if (parsedDate.before(defaultCenturyStart)) {
                    parsedDate = calb.addYear(100).establish(calendar)
                            .getTime();
                }
            }
        }
    ...
    private int matchString(String text, int start, int field, String[] data,
        ...
        for (; i < count; ++i) {
            ...
            if (length > bestMatchLength && text.regionMatches(true, start,
                    data[i], 0, length)) {
                bestMatch = i;
    ...
    /**
     * Performs the same thing as matchString(String, int, int, String[]). This
     * method takes a Map<String, Integer> instead of String[].
     */
        ...
        if (data != null) {
            ...
            if (data instanceof SortedMap) {
                for (String name : data.keySet()) {
                    if (text.regionMatches(true, start, name, 0, name
                            .length())) {
    ...
    private int matchZoneString(String text, int start, String[] zoneNames) {
        for (int i = 1; i <= 4; ++i) {
            ...
            if (text.regionMatches(true, start, zoneName, 0, zoneName
                    .length())) {
                return i;
            }
        }
    ...
    private boolean matchDSTString(String text, int start, int zoneIndex,
            int standardIndex, String[][] zoneStrings) {
    ...
    private int subParseZoneString(String text, int start,
            CalendarBuilder calb) {
        boolean useSameName = false; // true if standard and daylight time use
        ...
        if (zoneIndex != -1) {
            ...
            if ((nameIndex = matchZoneString(text, start, zoneNames)) > 0) {
                if (nameIndex <= 2) {
                    // Check if the standard name (abbr) and the daylight name
                    // are the same.
                    useSameName = zoneNames[nameIndex].equalsIgnoreCase(
                            zoneNames[nameIndex + 2]);
                }
        ...
        if (tz != null) { // Matched any ?
            ...
            if (!(useSameName || (nameIndex >= 3 && dstAmount == 0))) {
                calb.clear(Calendar.ZONE_OFFSET).set(Calendar.DST_OFFSET,
                        dstAmount);
            }
    ...
    /**
     * Parses numeric forms of time zone offset, such as "hh:mm", and sets calb
     * to the parsed value.
     *
     * @param text
     *              the text to be parsed
     * @param start
     *              the character position to start parsing
     * @param sign
     *              1: positive; -1: negative
     * @param count
     *              0: 'Z' or "GMT+hh:mm" parsing; 1 - 3: the number of 'X's
     * @param colon
     *              true - colon required between hh and mm; false - no colon
     *              required
     * @param calb
     *              a CalendarBuilder in which the parsed value is stored
     * @return updated parsed position, or its negative value to indicate a
     *         parsing error
     */
     ...
     * @param start
     *                         where to start parsing.
     * @param patternCharIndex
     *                         the index of the pattern character.
     * @param count
     *                         the count of a pattern character.
     * @param obeyCount
     *                         if true, then the next field directly abuts this
     *                         one, and we
     *                         should use the count to know when to stop
     *                         parsing.
     * @param ambiguousYear
     *                         return parameter; upon return, if
     *                         ambiguousYear[0] is true,
     *                         then a two-digit year was parsed and may need to
     *                         be
     *                         readjusted.
     * @param origPos
     *                         origPos.errorIndex is used to return an error
     *                         index at which a
     *                         parse error occurred, if matching failure occurs.
     ...
     */
    private int subParse(String text, int start, int patternCharIndex,
            int count, boolean obeyCount, boolean[] ambiguousYear,
        ...
        pos.index = start;
        if (patternCharIndex == PATTERN_WEEK_YEAR && !calendar
                .isWeekDateSupported()) {
            // use calendar year 'y' instead
            patternCharIndex = PATTERN_YEAR;
        }
        ...
        parsing: {
            ...
            // a number value. We handle further, more generic cases below. We
            // need
            ...
            if (patternCharIndex == PATTERN_HOUR_OF_DAY1
                ...
                // It would be good to unify this with the obeyCount logic
                // below,
                ...
                if (obeyCount) {
                    ...
                    number = numberFormat.parse(text.substring(0, start
                            + count), pos);
                } else {
                ...
                if (number == null) {
                    ...
                    if (useFollowingMinusSignAsDelimiter && (value < 0)
                            && (((pos.index < text.length()) && (text.charAt(
                                    pos.index) != minusSign))
                                    || ((pos.index == text.length()) && (text
                                            .charAt(pos.index
                                                    - 1) == minusSign)))) {
                        value = -value;
                        pos.index--;
            ...
            switch (patternCharIndex) {
                case PATTERN_ERA: // 'G'
                    if (useDateFormatSymbols) {
                        if ((index = matchString(text, start, Calendar.ERA,
                                formatData.getEras(), calb)) > 0) {
                            return index;
                        }
                    } else {
                        Map<String, Integer> map = getDisplayNamesMap(field,
                                locale);
                        if ((index = matchString(text, start, field, map,
                                calb)) > 0) {
                            return index;
                        }
                    }
                ...
                case PATTERN_YEAR: // 'y'
                    if (!(calendar instanceof GregorianCalendar)) {
                        ...
                        int style = (count >= 4) ? Calendar.LONG
                                : Calendar.SHORT;
                    ...
                    // If there are 3 or more YEAR pattern characters, this
                    // indicates
                    // that the year value is to be treated literally, without any
                    // two-digit year adjustments (e.g., from "01" to 2001).
                    // Otherwise
                    // we made adjustments to place the 2-digit year in the proper
                    // century, for parsed strings from "00" to "99". Any other
                    // string
                    ...
                    if (count <= 2 && (pos.index - actualStart) == 2
                            ...
                            && Character.isDigit(text.charAt(actualStart
                                    + 1))) {
                        // Assume for example that the defaultCenturyStart is
                        // 6/18/1903.
                        // This means that two-digit years will be forced into the
                        // range
                        // 6/18/1903 to 6/17/2003. As a result, years 00, 01, and 02
                        // correspond to 2000, 2001, and 2002. Years 04, 05, etc.
                        // correspond
                        // to 1904, 1905, etc. If the year is 03, then it is 2003 if
                        // the
                        // other fields specify a date before 6/18, or 1903 if they
                        // specify a
                        // date afterwards. As a result, 03 is an ambiguous year.
                        // All other
                        // two-digit years are unambiguous.
                        int ambiguousTwoDigitYear = defaultCenturyStartYear
                                % 100;
                ...
                case PATTERN_HOUR_OF_DAY1: // 'k' 1-based. eg, 23:59 + 1 hour =>>
                                           // 24:59
                    if (!isLenient()) {
                    ...
                    // [We computed 'value' above.]
                    if (value == calendar.getMaximum(Calendar.HOUR_OF_DAY)
                            + 1) {
                        value = 0;
                    }
                ...
                case PATTERN_DAY_OF_WEEK: // 'E'
                {
                    if (useDateFormatSymbols) {
                        ...
                        int newStart;
                        if ((newStart = matchString(text, start,
                                Calendar.DAY_OF_WEEK, formatData.getWeekdays(),
                                calb)) > 0) {
                            return newStart;
                        }
                        // DDDD failed, now try DDD
                        if ((index = matchString(text, start,
                                Calendar.DAY_OF_WEEK, formatData
                                        .getShortWeekdays(), calb)) > 0) {
                            return index;
                        }
                    } else {
                ...
                case PATTERN_ZONE_VALUE: // 'Z'
                {
                    ...
                    try {
                        ...
                        if (sign == 0) {
                            ...
                            if ((c == 'G' || c == 'g') && (text.length()
                                    - start) >= GMT.length() && text
                                            .regionMatches(true, start, GMT, 0,
                                                    GMT.length())) {
                                ...
                                if (sign == 0) { /* "GMT" without offset */
                                    calb.set(Calendar.ZONE_OFFSET, 0).set(
                                            Calendar.DST_OFFSET, 0);
                                    return pos.index;
                                }
                            ...
                            int i = subParseNumericZone(text, ++pos.index, sign,
                                    0, false, calb);
                            if (i > 0) {
                                return i;
                            }
                ...
                default:
                    ...
                    // case PATTERN_WEEK_OF_YEAR: // 'w'
                    // case PATTERN_WEEK_OF_MONTH: // 'W'
                    // case PATTERN_HOUR0: // 'K' 0-based. eg, 11PM + 1 hour =>> 0
                    // AM
                    ...
                    if (number != null) {
                        ...
                        if (useFollowingMinusSignAsDelimiter && (value < 0)
                                && (((pos.index < text.length()) && (text
                                        .charAt(pos.index) != minusSign))
                                        || ((pos.index == text.length())
                                                && (text.charAt(pos.index
                                                        - 1) == minusSign)))) {
                            value = -value;
                            pos.index--;
                        }
    ...
    public String toLocalizedPattern() {
        return translatePattern(pattern, DateFormatSymbols.patternChars,
    ...
    public void applyLocalizedPattern(String pattern) {
        String p = translatePattern(pattern, formatData.getLocalPatternChars(),
    ...
    /**
     * Creates a copy of this <code>SimpleDateFormat</code>. This also clones
     * the format's date format symbols.
     *
    ...
    /**
     * Returns the hash code value for this <code>SimpleDateFormat</code>
     * object.
     *
     * @return the hash code value for this <code>SimpleDateFormat</code>
     *         object.
     */
    @Override
    public int hashCode() {
    ...
    public boolean equals(Object obj) {
        ...
        return (pattern.equals(that.pattern) && formatData.equals(
                that.formatData));
    }

    private static final int[] REST_OF_STYLES = { Calendar.SHORT_STANDALONE,
            Calendar.LONG_FORMAT, Calendar.LONG_STANDALONE, };
    ...
    private Map<String, Integer> getDisplayNamesMap(int field, Locale locale) {
        ...
        for (int style : REST_OF_STYLES) {
            Map<String, Integer> m = calendar.getDisplayNames(field, style,
                    locale);
            if (m != null) {
                map.putAll(m);
            }
    ...
    /**
     * After reading an object from the input stream, the format pattern in the
     * object is verified.
     * <p>
     * 
    ...
    /**
     * Analyze the negative subpattern of DecimalFormat and set/update values as
     * necessary.
     */
    private void checkNegativeNumberExpression() {
        if ((numberFormat instanceof DecimalFormat) && !numberFormat.equals(
            ...
            if (!numberPattern.equals(originalNumberPattern)) {
                ...
                if (separatorIndex > -1) {
                    ...
                    if ((minusIndex > numberPattern.lastIndexOf('0'))
                        ...
                        minusSign = ((DecimalFormat) numberFormat)
