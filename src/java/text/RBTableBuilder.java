...
/**
 * This class contains all the code to parse a RuleBasedCollator pattern and
 * build a RBCollationTables object from it. A particular instance of tis class
 * exists only during the actual build process-- once an RBCollationTables
 * object has been built, the RBTableBuilder object goes away. This object
 * carries all of the state which is only needed during the build process, plus
 * a "shadow" copy of all of the state that will go into the tables object
 * itself. This object communicates with RBCollationTables through a separate
 * class, RBCollationTables.BuildAPI, this is an inner class of
 * RBCollationTables and provides a separate private API for communication with
 * RBTableBuilder. This class isn't just an inner class of RBCollationTables
 * itself because of its large size. For source-code readability, it seemed
 * better for the builder to have its own source file.
 */
final class RBTableBuilder {
	...
	/**
	 * Create a table-based collation object with the given rules. This is the
	 * main function that actually builds the tables and stores them back in the
	 * RBCollationTables object. It is called ONLY by the RBCollationTables
	 * constructor.
	 * 
	...
	public void build(String pattern, int decmp) throws ParseException {
		...
		// Normalize the build rules. Find occurances of all decomposed
		// characters
		// and normalize the rules before feeding into the builder. By
		// "normalize",
		// we mean that all precomposed Unicode characters must be converted
		// into
		// a base character and one or more combining characters (such as
		// accents).
		// When there are multiple combining characters attached to a base
		// character,
		// the combining characters must be in their canonical order
		//
		// sherman/Note:
		// (1)decmp will be NO_DECOMPOSITION only in ko locale to prevent
		// decompose
		// hangual syllables to jamos, so we can actually just call decompose
		// with
			...
			if (entry != null) {
				...
				if (groupChars.length() > 1) {
					switch (groupChars.charAt(groupChars.length() - 1)) {
						case '@':
							frenchSec = true;
							groupChars = groupChars.substring(0, groupChars
									.length() - 1);
							break;
						case '!':
				...
				if (expChars.length() != 0) {
					...
					if (Character.isHighSurrogate(ch) && groupChars
							.length() == 2) {
						addOrder(Character.toCodePoint(ch, groupChars.charAt(
								1)), order);
					} else {
		...
		mapping.compact();
		/*
		 * System.out.println("mappingSize=" + mapping.getKSize()); for (int j =
		 * 0; j < 0xffff; j++) { int value = mapping.elementAt(j); if (value !=
		 * RBCollationTables.UNMAPPED) System.out.println("index=" +
		 * Integer.toString(j, 16) + ", value=" + Integer.toString(value, 16));
		 * }
		 */
		tables.fillInTables(frenchSec, seAsianSwapping, mapping, contractTable,
				expandTable, contractFlags, maxSecOrder, maxTerOrder);
	}
	...
	private void addComposedChars() throws ParseException {
		...
		while ((c = iter.next()) != ComposedCharIter.DONE) {
			if (getCharOrder(c) == RBCollationTables.UNMAPPED) {
				//
				// We don't already have an ordering for this pre-composed
				// character.
				//
				...
				//
				// TODO: What we should really be doing here is trying to find
				// the
				// longest initial substring of the decomposition that is
				// present
				// in the tables as a contracting character sequence, and find
				// its
				...
				// only thing need to do is to check if this decomposed
				// character
				...
				if (contractOrder != RBCollationTables.UNMAPPED) {
					...
					//
					// We don't have a contracting ordering for the entire
					// string
					// that results from the decomposition, but if we have
					// orders
					...
					for (int i = 0; i < s.length(); i++) {
						if (getCharOrder(s.charAt(
								i)) == RBCollationTables.UNMAPPED) {
							allThere = false;
							break;
						}
					}
	 ...
	 *
	 * When the expanding character tables are built by addExpandOrder, it
	 * doesn't know what the final ordering of each character in the expansion
	 * will be. Instead, it just puts the raw character code into the table,
	 * adding CHARINDEX as a flag. Now that we've finished building the mapping
	 * table, we can go back and look up that character to see what its real
	 * collation order is and stick that into the expansion table. That lets us
	 * avoid doing a two-stage lookup later.
	 */
		...
		if (expandTable != null) {
			for (int i = 0; i < expandTable.size(); i++) {
				...
				for (int j = 0; j < valueList.length; j++) {
					...
					if (order < RBCollationTables.EXPANDCHARINDEX
							&& order > CHARINDEX) {
						...
						if (realValue == RBCollationTables.UNMAPPED) {
							// The real value is still unmapped, maybe it's
							// ignorable
	...
	private final int increment(int aStrength, int lastValue) {
		switch (aStrength) {
			case Collator.PRIMARY:
				// increment priamry order and mask off secondary and tertiary
				// difference
	...
	private final void addOrder(int ch, int anOrder) {
		...
		if (order >= RBCollationTables.CONTRACTCHARINDEX) {
			// There's already an entry for this character that points to a
			// contracting
			// character table. Instead of adding the character directly to the
			// mapping
	...
	private final void addContractOrder(String groupChars, int anOrder,
		...
		/*
		 * char ch0 = groupChars.charAt(0); int ch =
		 * Character.isHighSurrogate(ch0)? Character.toCodePoint(ch0,
		 * groupChars.charAt(1)):ch0;
		 */
		// See if the initial character of the string already has a contract
		// table.
		int entry = mapping.elementAt(ch);
		Vector<EntryPair> entryTable = getContractValuesImpl(entry
				- RBCollationTables.CONTRACTCHARINDEX);

		if (entryTable == null) {
			// We need to create a new table of contract entries for this base
			// char
			int tableIndex = RBCollationTables.CONTRACTCHARINDEX + contractTable
					.size();
			...
			entryTable.addElement(new EntryPair(groupChars.substring(0,
					Character.charCount(ch)), entry));
		...
		if (index != RBCollationTables.UNMAPPED) {
			...
			// NOTE: This little bit of logic is here to speed
			// CollationElementIterator
			// .nextContractChar(). This code ensures that the longest sequence
			// in
			// this list is always the _last_ one in the list. This keeps
			// nextContractChar() from having to search the entire list for the
			// longest
			// sequence.
	...
	/**
	 * If the given string has been specified as a contracting string in this
	 * collation table, return its ordering. Otherwise return UNMAPPED.
	 */
	private int getContractOrder(String groupChars) {
		...
		if (contractTable != null) {
			...
			if (entryTable != null) {
				int index = RBCollationTables.getEntry(entryTable, groupChars,
						true);
				if (index != RBCollationTables.UNMAPPED) {
					EntryPair pair = entryTable.elementAt(index);
					result = pair.value;
	...
	private final int getCharOrder(int ch) {
		...
		if (order >= RBCollationTables.CONTRACTCHARINDEX) {
			Vector<EntryPair> groupList = getContractValuesImpl(order
					- RBCollationTables.CONTRACTCHARINDEX);
	...
	private Vector<EntryPair> getContractValues(int ch) {
		...
		return getContractValuesImpl(index
				- RBCollationTables.CONTRACTCHARINDEX);
	}
	 ...
	 */
	private final void addExpandOrder(String contractChars, String expandChars,
	...
	/**
	 * Create a new entry in the expansion table that contains the orderings for
	 * the given characers. If anOrder is valid, it is added to the beginning of
	 * the expanded list of orders.
	 */
	private int addExpansion(int anOrder, String expandChars) {
		...
		for (int i = 0; i < expandChars.length(); i++) {
			...
			int ch;
			if (Character.isHighSurrogate(ch0)) {
				if (++i == expandChars.length() || !Character.isLowSurrogate(
						ch1 = expandChars.charAt(i))) {
		...
		if (j < valueList.length) {
			// we had at least one supplementary character, the size of
			// valueList
	...
	private void addContractFlags(String chars) {
		...
		for (int i = 0; i < len; i++) {
			c0 = chars.charAt(i);
			c = Character.isHighSurrogate(c0) ? Character.toCodePoint(c0, chars
					.charAt(++i)) : c0;
