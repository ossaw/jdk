...
/**
 * Utility class for normalizing and merging patterns for collation. Patterns
 * are strings of the form <entry>*, where <entry> has the form: <pattern> :=
 * <entry>* <entry> := <separator><chars>{"/"<extension>} <separator> := "=",
 * ",", ";", "<", "&" <chars>, and <extension> are both arbitrary strings.
 * unquoted whitespaces are ignored. 'xxx' can be used to quote characters One
 * difference from Collator is that & is used to reset to a current point. Or,
 * in other words, it introduces a new sequence which is to be added to the old.
 * That is: "a < b < c < d" is the same as "a < b & b < c & c < d" OR
 * "a < b < d & b < c" XXX: make '' be a single quote.
 * 
 * @see PatternEntry
...
final class MergeCollation {
    ...
    // privates
    // ============================================================
    ArrayList<PatternEntry> patterns = new ArrayList<>(); // a list of
                                                          // PatternEntries
    ...
    /*
     * If the strength is RESET, then just change the lastEntry to be the
     * current. (If the current is not in patterns, signal an error). If not,
     * then remove the current entry, and add it after lastEntry (which is
     * usually at the end).
     */
    private final void fixEntry(PatternEntry newEntry) throws ParseException {
        // check to see whether the new entry has the same characters as the
        // previous
        // entry did (this can happen when a pattern declaring a difference
        // between two
        // strings that are canonically equivalent is normalized). If so, and
        // the strength
        // is anything other than IDENTICAL or RESET, throw an exception (you
        // can't
        // declare a string to be unequal to itself). --rtg 5/24/99
        if (lastEntry != null && newEntry.chars.equals(lastEntry.chars) && newEntry.extension.equals(
                lastEntry.extension)) {
