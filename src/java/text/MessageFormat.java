 ...
 * <code>"'{0}'"</code> represents string <code>"{0}"</code>, not a
 * <i>FormatElement</i>. A single quote itself must be represented by doubled
 * single quotes {@code ''} throughout a <i>String</i>. For example, pattern
 * string <code>"'{''}'"</code> is interpreted as a sequence of <code>'{</code>
 * (start of quoting and a left curly brace), <code>''</code> (a single quote),
 ...
 * <code>"{1,number,<u>$'#',##</u>}"</code> (<i>SubformatPattern</i> with
 ...
 * digits {@code '0'} through {@code '9'}, and represents an index into the
 * {@code arguments} array passed to the {@code format} methods or the result
 * array returned by the {@code parse} methods.
 * <p>
 * The <i>FormatType</i> and <i>FormatStyle</i> values are used to create a
 * {@code Format} instance for the format element. The following table shows how
 ...
 * DecimalFormat}{@code (subformatPattern,}
 * {@link DecimalFormatSymbols#getInstance(Locale)
 * DecimalFormatSymbols.getInstance}{@code (getLocale()))}
 * <tr>
 * <td headers="ft" rowspan=6><code>date</code>
 * <td headers="fs"><i>(none)</i>
 * <td headers="sc">{@link DateFormat#getDateInstance(int,Locale)
 * DateFormat.getDateInstance}{@code (}{@link DateFormat#DEFAULT}
 * {@code , getLocale())}
 * <tr>
 * <td headers="fs"><code>short</code>
 * <td headers="sc">{@link DateFormat#getDateInstance(int,Locale)
 * DateFormat.getDateInstance}{@code (}{@link DateFormat#SHORT}
 * {@code , getLocale())}
 * <tr>
 ...
 * <td headers="sc">{@link DateFormat#getDateInstance(int,Locale)
 * DateFormat.getDateInstance}{@code (}{@link DateFormat#LONG}
 * {@code , getLocale())}
 * <tr>
 * <td headers="fs"><code>full</code>
 * <td headers="sc">{@link DateFormat#getDateInstance(int,Locale)
 * DateFormat.getDateInstance}{@code (}{@link DateFormat#FULL}
 * {@code , getLocale())}
 * <tr>
 * <td headers="fs"><i>SubformatPattern</i>
 * <td headers="sc">{@code new}
 * {@link SimpleDateFormat#SimpleDateFormat(String,Locale) SimpleDateFormat}
 * {@code (subformatPattern, getLocale())}
 * <tr>
 * <td headers="ft" rowspan=6><code>time</code>
 * <td headers="fs"><i>(none)</i>
 * <td headers="sc">{@link DateFormat#getTimeInstance(int,Locale)
 * DateFormat.getTimeInstance}{@code (}{@link DateFormat#DEFAULT}
 * {@code , getLocale())}
 * <tr>
 * <td headers="fs"><code>short</code>
 * <td headers="sc">{@link DateFormat#getTimeInstance(int,Locale)
 * DateFormat.getTimeInstance}{@code (}{@link DateFormat#SHORT}
 * {@code , getLocale())}
 * <tr>
 * <td headers="fs"><code>medium</code>
 * <td headers="sc">{@link DateFormat#getTimeInstance(int,Locale)
 ...
 * <td headers="fs"><code>long</code>
 * <td headers="sc">{@link DateFormat#getTimeInstance(int,Locale)
 * DateFormat.getTimeInstance}{@code (}{@link DateFormat#LONG}
 * {@code , getLocale())}
 * <tr>
 * <td headers="fs"><code>full</code>
 * <td headers="sc">{@link DateFormat#getTimeInstance(int,Locale)
 * DateFormat.getTimeInstance}{@code (}{@link DateFormat#FULL}
 * {@code , getLocale())}
 * <tr>
 * <td headers="fs"><i>SubformatPattern</i>
 * <td headers="sc">{@code new}
 ...
 * </pre>
 * 
 * </blockquote> The output with different values for <code>fileCount</code>:
 * <blockquote>
 * 
 * <pre>
 ...
 * special; occurrences of '{' are used to indicate subformats, and cause
 ...
 * will be the final result of the parsing. For example, <blockquote>
 * 
 * <pre>
...
public class MessageFormat extends Format {
	 ...
	 * {@link java.util.Locale.Category#FORMAT FORMAT} locale and the specified
	...
	/**
	 * Sets the formats to use for the values passed into <code>format</code>
	 * methods or returned from <code>parse</code> methods. The indices of
	 * elements in <code>newFormats</code> correspond to the argument indices
	 * used in the previously set pattern string. The order of formats in
	 * <code>newFormats</code> thus corresponds to the order of elements in the
	 ...
	 * pattern string. The order of formats in <code>newFormats</code>
	 ...
	 * then only the first <code>newFormats.length</code> formats are replaced.
	 * <p>
	 * Since the order of format elements in a pattern string often changes
	 * during localization, it is generally better to use the
	 * {@link #setFormatsByArgumentIndex setFormatsByArgumentIndex} method,
	 * which assumes an order of formats corresponding to the order of elements
	 * in the <code>arguments</code> array passed to the <code>format</code>
	 ...
	 * Since the order of format elements in a pattern string often changes
	 * during localization, it is generally better to use the
	 * {@link #setFormatByArgumentIndex setFormatByArgumentIndex} method, which
	 ...
	 *                if {@code formatElementIndex} is equal to or larger than
	...
	/**
	 * Gets the formats used for the values passed into <code>format</code>
	 * methods or returned from <code>parse</code> methods. The indices of
	 ...
	 * <p>
	 * Since the order of format elements in a pattern string often changes
	 * during localization, it's generally better to use the
	 * {@link #getFormatsByArgumentIndex getFormatsByArgumentIndex} method,
	 * which assumes an order of formats corresponding to the order of elements
	 * in the <code>arguments</code> array passed to the <code>format</code>
	 * methods or the result array returned by the <code>parse</code> methods.
	 *
	 ...
	 * <code>arguments</code> element at the format element's argument index as
	 ...
	 * <td>
	 * <code>DateFormat.getDateTimeInstance(DateFormat.SHORT, DateFormat.SHORT, getLocale()).format(argument)</code>
	 * <tr>
	 * <td><code>null</code>
	 ...
	 * <code>Field.ARGUMENT</code>, the location of the first formatted string
	 * will be returned.
	 *
	 * @param arguments
	 ...
	 *         text appended
	 * @exception IllegalArgumentException
	 *                if an argument in the <code>arguments</code> array is not
	 ...
	 * Formats an array of objects and appends the <code>MessageFormat</code>'s
	 * pattern, with format elements replaced by the formatted objects, to the
	 * provided <code>StringBuffer</code>. This is equivalent to <blockquote>
	 ...
	 * <code>AttributedCharacterIterator</code>. You can use the returned
	 * <code>AttributedCharacterIterator</code> to build the resulting String,
	 * as well as to determine information about the resulting String.
	 * <p>
	 * The text of the returned <code>AttributedCharacterIterator</code> is the
	 ...
	 * the <code>arguments</code> array. The keys of these attributes are of
	 * type <code>MessageFormat.Field</code>, their values are
	 * <code>Integer</code> objects indicating the index in the
	 * <code>arguments</code> array of the argument from which the text was
	 * generated.
	 * <p>
	 * The attributes/value from the underlying <code>Format</code> instances
	 * that <code>MessageFormat</code> uses will also be placed in the resulting
	 * <code>AttributedCharacterIterator</code>. This allows you to not only
	 ...
	 * in the string the parsing failed. The returned error index is the
	 ...
	 * parsed object array is returned. The updated <code>pos</code> can be used
	 * to indicate the starting point for the next call to this method. If an
	 * error occurs, then the index of <code>pos</code> is not changed, the
	 * error index of <code>pos</code> is set to the index of the character
	...
	public static class Field extends Format.Field {
		 ...
		 * an argument passed into <code>formatToCharacterIterator</code>. The
		 * value associated with the key will be an <code>Integer</code>
	...
	/**
	 * One less than the number of entries in <code>offsets</code>. Can also be
	 * thought of as the index of the highest-numbered element in
	 * <code>offsets</code> that is being used. All of these arrays should have
	 * the same number of elements being used as <code>offsets</code> does, and
	 ...
	 * as necessary. If <code>characterIterators</code> is null and
	 * <code>fp</code> is non-null and identifies
	 * <code>Field.MESSAGE_ARGUMENT</code>, the location of the first replaced
		...
		for (int i = 0; i <= maxOffset; ++i) {
			...
			if (false) { // if (argRecursion == 3){
				...
				if (obj == null) {
					...
					subFormatter = DateFormat.getDateTimeInstance(DateFormat.SHORT,
							DateFormat.SHORT, locale);// fix
				...
				if (characterIterators != null) {
					...
					if (last != result.length()) {
						characterIterators
								.add(createAttributedCharacterIterator(result.substring(last)));
						last = result.length();
					}
					...
					if (arg != null && arg.length() > 0) {
						result.append(arg);
						characterIterators.add(createAttributedCharacterIterator(arg,
								Field.ARGUMENT, Integer.valueOf(argumentNumber)));
						last = result.length();
					}
				} else {
	...
	/**
	 * Convenience method to append all the characters in <code>iterator</code>
	 * to the StringBuffer <code>result</code>.
	 */
	...
	private static final String[] NUMBER_MODIFIER_KEYWORDS = { "", "currency", "percent",
			"integer" };
	...
	private static final String[] DATE_TIME_MODIFIER_KEYWORDS = { "", "short", "medium", "long",
			"full" };
	...
	private void readObject(ObjectInputStream in) throws IOException, ClassNotFoundException {
		...
		boolean isValid = maxOffset >= -1 && formats.length > maxOffset
				&& offsets.length > maxOffset && argumentNumbers.length > maxOffset;
		if (isValid) {
