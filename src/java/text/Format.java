 ...
 * locale-sensitive objects into <code>String</code>s (the <code>format</code>
 * method) and for parsing <code>String</code>s back into objects (the
 * <code>parseObject</code> method).
 *
 * <p>
 * Generally, a format's <code>parseObject</code> method must be able to parse
 * any string formatted by its <code>format</code> method. However, there may be
 * exceptional cases where this is not possible. For example, a
 * <code>format</code> method might create two adjacent integer numbers with no
 * separator in between, and in this case the <code>parseObject</code> could not
 ...
 * <code>Format</code>-- <code>DateFormat</code>, <code>MessageFormat</code>,
 * and <code>NumberFormat</code>--for formatting dates, messages, and numbers,
 * respectively.
 * <p>
 * Concrete subclasses must implement three methods:
 * <ol>
 * <li>
 * <code>format(Object obj, StringBuffer toAppendTo, FieldPosition pos)</code>
 ...
 * are used, for example, by <code>MessageFormat</code>. Subclasses often also
 * provide additional <code>format</code> methods for specific input types as
 * well as <code>parse</code> methods for specific result types. Any
 * <code>parse</code> method that does not take a <code>ParsePosition</code>
 * argument should throw <code>ParseException</code> when no text in the
 * required format is at the beginning of the input text.
 *
 * <p>
 * Most subclasses will also implement the following factory methods:
 * <ol>
 * <li><code>getInstance</code> for getting a useful format object appropriate
 * for the current locale
 * <li><code>getInstance(Locale)</code> for getting a useful format object
 ...
 * <code>getPercentInstance</code> and <code>getCurrencyInstance</code> methods
 * for getting specialized number formatters.
 *
 * <p>
 * Subclasses of <code>Format</code> that allow programmers to create objects
 * for locales (with <code>getInstance(Locale)</code> for example) must also
 * implement the following class method: <blockquote>
 * 
 * <pre>
 ...
 * named <code><em>item</em>_FIELD</code> where <code><em>item</em></code>
 * identifies the field. For examples of these constants, see
 * <code>ERA_FIELD</code> and its friends in {@link DateFormat}.
 *
...
public abstract class Format implements Serializable, Cloneable {
	 ...
	 * <blockquote> {@link #format(Object, StringBuffer, FieldPosition) format}
	 * <code>(obj,
	 ...
	 * @param pos
	 *            A <code>FieldPosition</code> identifying a field in the
	 *            formatted text
	 ...
	 * <code>AttributedCharacterIterator</code>, but typically the attribute key
	 ...
	 */
	AttributedCharacterIterator createAttributedCharacterIterator(
			AttributedCharacterIterator iterator, AttributedCharacterIterator.Attribute key,
			Object value) {
	 ...
	 * <code>Format.formatToCharacterIterator</code> and as field identifiers in
	 * <code>FieldPosition</code>.
	 *
	 * @since 1.4
	 */
	 ...
	 * such as in a <code>FieldPosition</code> or for an
	 * <code>AttributedCharacterIterator</code>.
	 * <p>
	 * Delegates should NOT assume that the <code>Format</code> will notify the
