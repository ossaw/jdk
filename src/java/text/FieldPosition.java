 ...
 * <code>_FIELD</code>. The constants are defined in the various subclasses of
 * <code>Format</code>.
 * <li>By a <code>Format.Field</code> constant, see <code>ERA_FIELD</code> and
 * its friends in <code>DateFormat</code> for an example.
 * </ul>
 * <p>
...
public class FieldPosition {
    ...
    /**
     * Input: Desired field to determine start and end offsets for. The meaning
     * depends on the subclass of Format.
     */
    int field = 0;

    /**
     * Output: End offset of field in text. If the field does not occur in the
     * text, 0 is returned.
     */
    int endIndex = 0;

    /**
     * Output: Start offset of field in text. If the field does not occur in the
     * text, 0 is returned.
     */
    int beginIndex = 0;
    ...
    /**
     * Creates a FieldPosition object for the given field. Fields are identified
     * by constants, whose names typically end with _FIELD, in the various
    ...
    /**
     * Creates a <code>FieldPosition</code> object for the given field. The
     * field is identified by an attribute constant from one of the
     * <code>Field</code> subclasses as well as an integer field ID defined by
     * the <code>Format</code> subclasses. <code>Format</code> subclasses that
     * are aware of <code>Field</code> should give precedence to
     * <code>attribute</code> and ignore <code>fieldID</code> if
     * <code>attribute</code> is not null. However, older <code>Format</code>
     * subclasses may not be aware of <code>Field</code> and rely on
     * <code>fieldID</code>. If the field has no corresponding integer constant,
     ...
     * <code>Field</code> subclasses. May return null if the field is specified
     ...
     * with the FieldPosition. When the delegate is notified of the same field
    ...
    /**
     * Return true if the receiver wants a <code>Format.Field</code> value and
     * <code>attribute</code> is equal to it, or true if the receiver represents
     * an inteter constant and <code>field</code> equals it.
     */
    ...
    /**
     * An implementation of FieldDelegate that will adjust the begin/end of the
     * FieldPosition if the arguments match the field of the FieldPosition.
     */
    private class Delegate implements Format.FieldDelegate {
        /**
         * Indicates whether the field has been encountered before. If this is
         * true, and <code>formatted</code> is invoked, the begin/end are not
         * updated.
         */
