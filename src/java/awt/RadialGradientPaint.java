...
/**
 * The {@code RadialGradientPaint} class provides a way to fill a shape with a
 * circular radial color gradient pattern. The user may specify 2 or more
 * gradient colors, and this paint will provide an interpolation between each
 * color.
 * <p>
 * The user must specify the circle controlling the gradient pattern, which is
 * described by a center point and a radius. The user can also specify a
 * separate focus point within that circle, which controls the location of the
 * first color of the gradient. By default the focus is set to be the center of
 * the circle.
 * <p>
 * This paint will map the first color of the gradient to the focus point, and
 * the last color to the perimeter of the circle, interpolating smoothly for any
 * in-between colors specified by the user. Any line drawn from the focus point
 * to the circumference will thus span all the gradient colors.
 * <p>
 * Specifying a focus point outside of the radius of the circle will cause the
 * rings of the gradient pattern to be centered on the point just inside the
 * edge of the circle in the direction of the focus point. The rendering will
 * internally use this modified location as if it were the specified focus
 * point.
 * <p>
 ...
 * takes when it is filling the space outside the circle's radius by setting
 * {@code CycleMethod} to either {@code REFLECTION} or {@code REPEAT}. The
 * gradient color proportions are equal for any particular line drawn from the
 * focus point. The following figure shows that the distance AB is equal to the
 * distance BC, and the distance AD is equal to the distance DE.
 * <center> <img src = "doc-files/RadialGradientPaint-3.png" alt="image showing
 * the distance AB=BC, and AD=DE"> </center> If the gradient and graphics
 * rendering transforms are uniformly scaled and the user sets the focus so that
 * it coincides with the center of the circle, the gradient color proportions
 * are equal for any line drawn from the center. The following figure shows the
 * distances AB, BC, AD, and DE. They are all equal.
 * <center> <img src = "doc-files/RadialGradientPaint-4.png" alt="image showing
 * the distance of AB, BC, AD, and DE are all equal"> </center> Note that some
 * minor variations in distances may occur due to sampling at the granularity of
 * a pixel. If no cycle method is specified, {@code NO_CYCLE} will be chosen by
 ...
 *
 * <p>
 * The following code demonstrates typical usage of {@code RadialGradientPaint},
 ...
 * <center> <img src = "doc-files/RadialGradientPaint-1.png" alt="image showing
 * the output of the sameple code"> </center>
 *
 * <p>
 * It is also possible to specify a non-centered focus point, as in the
 * following code:
 * 
 * <pre>
 * Point2D center = new Point2D.Float(50, 50);
 ...
 * float[] dist = { 0.0f, 0.2f, 1.0f };
 * Color[] colors = { Color.RED, Color.WHITE, Color.BLUE };
 * RadialGradientPaint p = new RadialGradientPaint(center, radius, focus, dist,
 *         colors, CycleMethod.NO_CYCLE);
 * </pre>
 *
 * <p>
 * This image demonstrates the previous example code, with non-centered focus
 * for each of the three cycle methods:
 * <center> <img src = "doc-files/RadialGradientPaint-2.png" alt="image showing
 * the output of the sample code"> </center>
 *
 * @see java.awt.Paint
...
public final class RadialGradientPaint extends MultipleGradientPaint {
    ...
    /**
     * Constructs a {@code RadialGradientPaint} with a default {@code NO_CYCLE}
     * repeating method and {@code SRGB} color space, using the center as the
     * focus point.
     *
     * @param cx
     *                  the X coordinate in user space of the center point of
     *                  the
     *                  circle defining the gradient. The last color of the
     *                  gradient
     *                  is mapped to the perimeter of this circle.
     * @param cy
     *                  the Y coordinate in user space of the center point of
     *                  the
     *                  circle defining the gradient. The last color of the
     *                  gradient
     *                  is mapped to the perimeter of this circle.
     ...
     *                  is
     *                  used at the focus point, the last color around the
     *                  perimeter
     *                  of the circle.
     *
     ...
     *
     * @throws NullPointerException
     *                                  if {@code center} point is null, or
     *                                  {@code fractions} array
     *                                  is null, or {@code colors} array is null
     * @throws IllegalArgumentException
     *                                  if {@code radius} is non-positive, or
     *                                  {@code fractions.length != colors.length},
     *                                  or {@code colors}
    ...
    /**
     * Constructs a {@code RadialGradientPaint} with a default {@code SRGB}
     ...
     *                    colors along the gradient
     * @param colors
     *                    array of colors to use in the gradient. The first
     *                    color is
     *                    used at the focus point, the last color around the
     *                    perimeter
     *                    of the circle.
     ...
     *
     * @param center
     *                    the center point, in user space, of the circle
     *                    defining the
     *                    gradient
     * @param radius
     *                    the radius of the circle defining the extents of the
     ...
     *                                  if {@code center} point is null, or
     *                                  {@code fractions} array
     *                                  is null, or {@code colors} array is
     *                                  null, or
     *                                  {@code cycleMethod} is null
     * @throws IllegalArgumentException
     *                                  if {@code radius} is non-positive, or
     *                                  {@code fractions.length != colors.length},
     *                                  or {@code colors}
     ...
     *                    gradient
     * @param fx
     *                    the X coordinate of the point in user space to which
     *                    the first
     *                    color is mapped
     * @param fy
     *                    the Y coordinate of the point in user space to which
     *                    the first
     *                    color is mapped
     * @param fractions
     *                    numbers ranging from 0.0 to 1.0 specifying the
     ...
     *
     * @throws NullPointerException
     *                                  if one of the points is null, or
     *                                  {@code fractions} array is
    ...
    public RadialGradientPaint(Point2D center, float radius, Point2D focus,
            float[] fractions, Color[] colors, CycleMethod cycleMethod) {
        this(center, radius, focus, fractions, colors, cycleMethod,
     ...
     *                          the center point in user space of the circle
     *                          defining the
     *                          gradient. The last color of the gradient is
     *                          mapped to the
     *                          perimeter of this circle.
     * @param radius
     *                          the radius of the circle defining the extents of
     *                          the color
     *                          gradient
     * @param focus
     *                          the point in user space to which the first color
     *                          is mapped
     * @param fractions
     *                          numbers ranging from 0.0 to 1.0 specifying the
     *                          distribution of
     *                          colors along the gradient
     * @param colors
     *                          array of colors to use in the gradient. The
     *                          first color is
     *                          used at the focus point, the last color around
     *                          the perimeter
     *                          of the circle.
     * @param cycleMethod
     *                          either {@code NO_CYCLE}, {@code REFLECT}, or
     ...
     *                                  null, or {@code colors} array is null,
     *                                  or {@code cycleMethod}
     *                                  is null, or {@code colorSpace} is null,
     *                                  or
     *                                  {@code gradientTransform} is null
     * @throws IllegalArgumentException
     *                                  if {@code radius} is non-positive, or
     *                                  {@code fractions.length != colors.length},
     *                                  or {@code colors}
     ...
     *                                  order
     */
    @ConstructorProperties({ "centerPoint", "radius", "focusPoint", "fractions",
            "colors", "cycleMethod", "colorSpace", "transform" })
    public RadialGradientPaint(Point2D center, float radius, Point2D focus,
            float[] fractions, Color[] colors, CycleMethod cycleMethod,
    ...
    /**
     * Constructs a {@code RadialGradientPaint} with a default {@code SRGB}
     * color space. The gradient circle of the {@code RadialGradientPaint} is
     * defined by the given bounding box.
     * <p>
     * This constructor is a more convenient way to express the following
     * (equivalent) code:<br>
     *
     * <pre>
     ...
     * </pre>
     *
     * @param gradientBounds
     *                       the bounding box, in user space, of the circle
     *                       defining the
     *                       outermost extent of the gradient
     * @param fractions
     *                       numbers ranging from 0.0 to 1.0 specifying the
     *                       distribution of
     *                       colors along the gradient
     ...
     *
     * @throws NullPointerException
     *                                  if {@code gradientBounds} is null, or
     *                                  {@code fractions} array
     *                                  is null, or {@code colors} array is
     *                                  null, or
     *                                  {@code cycleMethod} is null
     * @throws IllegalArgumentException
     *                                  if {@code gradientBounds} is empty, or
     *                                  {@code fractions.length != colors.length},
     *                                  or {@code colors}
     *                                  is less than 2 in size, or a
     *                                  {@code fractions} value is less
    ...
    public RadialGradientPaint(Rectangle2D gradientBounds, float[] fractions,
        ...
        this(new Point2D.Double(gradientBounds.getCenterX(), gradientBounds
                .getCenterY()), 1.0f, new Point2D.Double(gradientBounds
                        .getCenterX(), gradientBounds.getCenterY()), fractions,
                colors, cycleMethod, ColorSpaceType.SRGB,
    ...
    /**
     * Creates and returns a {@link PaintContext} used to generate a circular
     * radial color gradient pattern. See the description of the
     * {@link Paint#createContext createContext} method for information on null
     * parameter handling.
     *
     * @param cm
    ...
    public PaintContext createContext(ColorModel cm, Rectangle deviceBounds,
        ...
        return new RadialGradientPaintContext(this, cm, deviceBounds,
                userBounds, transform, hints, (float) center.getX(),
                (float) center.getY(), radius, (float) focus.getX(),
    ...
    /**
     * Returns a copy of the focus point of the radial gradient. Note that if
     * the focus point specified when the radial gradient was constructed lies
     * outside of the radius of the circle, this method will still return the
     * original focus point even though the rendering may center the rings of
     * color on a different point that lies inside the radius.
     *
