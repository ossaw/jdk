...
/**
 * The <code>Printable</code> interface is implemented by the <code>print</code>
 * methods of the current page painter, which is called by the printing system
 * to render a page. When building a {@link Pageable}, pairs of
 * {@link PageFormat} instances and instances that implement this interface are
 * used to describe each page. The instance implementing <code>Printable</code>
 * is called to print the page's graphics.
 * <p>
 * A <code>Printable(..)</code> may be set on a <code>PrinterJob</code>. When
 * the client subsequently initiates printing by calling
 * <code>PrinterJob.print(..)</code> control
 * <p>
 * is handed to the printing system until all pages have been printed. It does
 * this by calling <code>Printable.print(..)</code> until all pages in the
 * document have been printed. In using the <code>Printable</code> interface the
 * printing commits to image the contents of a page whenever requested by the
 * printing system.
 * <p>
 * The parameters to <code>Printable.print(..)</code> include a
 * <code>PageFormat</code> which describes the printable area of the page,
 * needed for calculating the contents that will fit the page, and the page
 * index, which specifies the zero-based print stream index of the requested
 * page.
 * <p>
 ...
 * PageFormat. The size, orientation and imageable area of the page is therefore
 * already determined and rendering must be within this imageable area. This is
...
public interface Printable {
    ...
    /**
     * Returned from {@link #print(Graphics, PageFormat, int)} to signify that
     * the requested page was rendered.
     */
    int PAGE_EXISTS = 0;

    /**
     * Returned from <code>print</code> to signify that the
     * <code>pageIndex</code> is too large and that the requested page does not
     * exist.
     */
    int NO_SUCH_PAGE = 1;
