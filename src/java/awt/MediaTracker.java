...
public class MediaTracker implements java.io.Serializable {
    ...
    /**
     * A given <code>Component</code> that will be tracked by a media tracker
     * where the image will eventually be drawn.
     *
     * @serial
    ...
    Component target;
    /**
     * The head of the list of <code>Images</code> that is being tracked by the
    ...
    public synchronized void addImage(Image image, int id, int w, int h) {
        ...
        if (rvImage != null) {
            addImageImpl(rvImage, id, w == -1 ? -1 : 2 * w, h == -1 ? -1
                    : 2 * h);
        }
    }

    private void addImageImpl(Image image, int id, int w, int h) {
        head = MediaEntry.insert(head, new ImageMediaEntry(this, image, id, w,
                h));
    }
    ...
    /**
     * Checks to see if all images being tracked by this media tracker have
     * finished loading.
     * <p>
     * This method does not start loading the images if they are not already
     * loading.
     * <p>
     * If there is an error while loading or scaling an image, then that image
     * is considered to have finished loading. Use the <code>isErrorAny</code>
     * or <code>isErrorID</code> methods to check for errors.
     * 
     * @return <code>true</code> if all images have finished loading, have been
     *         aborted, or have encountered an error; <code>false</code>
     *         otherwise
     ...
     * finished loading.
     * <p>
     * If the value of the <code>load</code> flag is <code>true</code>, then
     * this method starts loading any images that are not yet being loaded.
     * <p>
     * If there is an error while loading or scaling an image, that image is
     * considered to have finished loading. Use the <code>isErrorAny</code> and
     * <code>isErrorID</code> methods to check for errors.
     * 
     * @param load
     *             if <code>true</code>, start loading any images that are not
     *             yet being loaded
     * @return <code>true</code> if all images have finished loading, have been
     *         aborted, or have encountered an error; <code>false</code>
     *         otherwise
     ...
     * 
     * @return <code>true</code> if any of the images tracked by this media
     *         tracker had an error during loading; <code>false</code> otherwise
     * @see java.awt.MediaTracker#isErrorID
     ...
     * 
     * @return an array of media objects tracked by this media tracker that have
     *         encountered an error, or <code>null</code> if there are none with
     *         errors
     * @see java.awt.MediaTracker#isErrorAny
    ...
    /**
     * Starts loading all images tracked by this media tracker. This method
    ...
    /**
     * Starts loading all images tracked by this media tracker. This method
     * waits until all the images being tracked have finished loading, or until
     * the length of time specified in milliseconds by the <code>ms</code>
     * argument has passed.
     * <p>
     * If there is an error while loading or scaling an image, then that image
     * is considered to have finished loading. Use the <code>isErrorAny</code>
     ...
     * @return <code>true</code> if all images were successfully loaded;
    ...
    /**
     * Calculates and returns the bitwise inclusive <b>OR</b> of the status of
     * all media that are tracked by this media tracker.
     * <p>
     * Possible flags defined by the <code>MediaTracker</code> class are
     * <code>LOADING</code>, <code>ABORTED</code>, <code>ERRORED</code>, and
     * <code>COMPLETE</code>. An image that hasn't started loading has zero as
     * its status.
     * <p>
     * If the value of <code>load</code> is <code>true</code>, then this method
     * starts loading any images that are not yet being loaded.
     *
     * @param load
     *             if <code>true</code>, start loading any images that are not
     *             yet being loaded
     * @return the bitwise inclusive <b>OR</b> of the status of all of the media
     *         being tracked
    ...
    private synchronized boolean checkID(int id, boolean load, boolean verify) {
        ...
        boolean done = true;
        while (cur != null) {
            if (cur.getID() == id && (cur.getStatus(load, verify)
                    & DONE) == 0) {
                done = false;
            }
            cur = cur.next;
    ...
    /**
     * Checks the error status of all of the images tracked by this media
     * tracker with the specified identifier.
     * 
     * @param id
     *           the identifier of the images to check
     * @return <code>true</code> if any of the images with the specified
     *         identifier had an error during loading; <code>false</code>
     *         otherwise
     * @see java.awt.MediaTracker#isErrorAny
     * @see java.awt.MediaTracker#getErrorsID
     */
    public synchronized boolean isErrorID(int id) {
        MediaEntry cur = head;
        while (cur != null) {
            if (cur.getID() == id && (cur.getStatus(false, true)
                    & ERRORED) != 0) {
                return true;
            }
            cur = cur.next;
    ...
    /**
     * Returns a list of media with the specified ID that have encountered an
     * error.
     * 
     * @param id
     *           the identifier of the images to check
     * @return an array of media objects tracked by this media tracker with the
     *         specified identifier that have encountered an error, or
    ...
    public synchronized Object[] getErrorsID(int id) {
        ...
        numerrors = 0;
        while (cur != null) {
            if (cur.getID() == id && (cur.getStatus(false, false)
                    & ERRORED) != 0) {
                errors[numerrors++] = cur.getMedia();
            }
            cur = cur.next;
     ...
     * specified identifier. This method waits until all the images with the
     * specified identifier have finished loading.
     * <p>
     * If there is an error while loading or scaling an image, then that image
     * is considered to have finished loading. Use the <code>isErrorAny</code>
     * and <code>isErrorID</code> methods to check for errors.
     * 
     * @param id
     *           the identifier of the images to check
     ...
     * Starts loading all images tracked by this media tracker with the
     * specified identifier. This method waits until all the images with the
     * specified identifier have finished loading, or until the length of time
     * specified in milliseconds by the <code>ms</code> argument has passed.
     * <p>
     * If there is an error while loading or scaling an image, then that image
     * is considered to have finished loading. Use the <code>statusID</code>,
     * <code>isErrorID</code>, and <code>isErrorAny</code> methods to check for
     * errors.
     * 
     * @param id
     *           the identifier of the images to check
     * @param ms
     *           the length of time, in milliseconds, to wait for the loading
     *           to complete
     * @see java.awt.MediaTracker#waitForAll
     * @see java.awt.MediaTracker#waitForID(int)
     * @see java.awt.MediaTracker#statusID
    ...
    /**
     * Calculates and returns the bitwise inclusive <b>OR</b> of the status of
     * all media with the specified identifier that are tracked by this media
     * tracker.
     * <p>
     * Possible flags defined by the <code>MediaTracker</code> class are
     * <code>LOADING</code>, <code>ABORTED</code>, <code>ERRORED</code>, and
     ...
     *             yet being loaded
     * @return the bitwise inclusive <b>OR</b> of the status of all of the media
     *         with the specified identifier that are being tracked
    ...
    /**
     * Removes the specified image from this media tracker. All instances of the
     * specified image are removed, regardless of scale or ID.
     * 
     * @param image
    ...
    /**
     * Removes the specified image from the specified tracking ID of this media
     * tracker. All instances of <code>Image</code> being tracked under the
     * specified ID are removed regardless of scale.
     * 
     * @param image
     *              the image to be removed
     * @param id
    ...
    /**
     * Removes the specified image with the specified width, height, and ID from
     * this media tracker. Only the specified instance (with any duplicates) is
     * removed.
     * 
     * @param image
     *               the image to be removed
    ...
    public synchronized void removeImage(Image image, int id, int width,
        ...
        Image rvImage = getResolutionVariant(image);
        if (rvImage != null) {
            removeImageImpl(rvImage, id, width == -1 ? -1 : 2 * width,
                    height == -1 ? -1 : 2 * height);
        }
        notifyAll(); // Notify in case remaining images are "done".
    }
