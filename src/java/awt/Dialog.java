...
public class Dialog extends Window {
	 ...
	 *            <code>MODELESS</code>; if <code>true</code>, the modality type
	 ...
	 *            {@link java.awt.Dialog Dialog}, {@link java.awt.Frame Frame},
	 *            any of their descendents or <code>null</code>
	 *
	 * @exception java.lang.IllegalArgumentException
	 *                if the <code>owner</code> is not an instance of
	 *                {@link java.awt.Dialog Dialog} or {@link java.awt.Frame
	 *                Frame}
	 * @exception java.lang.IllegalArgumentException
	 *                if the <code>owner</code>'s
	 *                <code>GraphicsConfiguration</code> is not from a screen
	 ...
	 *                if the calling thread does not have permission to create
	 *                modal dialogs with the given <code>modalityType</code>
	 *
	 * @see java.awt.Dialog.ModalityType
	 * @see java.awt.Dialog#setModal
	 * @see java.awt.Dialog#setModalityType
				...
				if (conditionalShow(predictedFocusOwner, time)) {
					...
					if (modalityType == ModalityType.TOOLKIT_MODAL) {
						...
						while (it.hasNext()) {
							...
							EventQueue eventQueue = (EventQueue) appContext
					...
					try {
						final EventQueue eventQueue = AccessController
								.doPrivileged(new PrivilegedAction<EventQueue>() {
	 ...
	 *
	 * @param undecorated
	 *            {@code true} if no dialog decorations are to be enabled;
	 *            {@code false} if dialog decorations are to be enabled
	 *
	 * @throws IllegalComponentStateException
	 *             if the dialog is displayable
	 * @throws IllegalComponentStateException
	 *             if {@code undecorated} is {@code false}, and this dialog does
	 *             not have the default shape
	 * @throws IllegalComponentStateException
	 *             if {@code undecorated} is {@code false}, and this dialog
	 *             opacity is less than {@code 1.0f}
	 * @throws IllegalComponentStateException
	 *             if {@code undecorated} is {@code false}, and the alpha value
	 *             of this dialog background color is less than {@code 1.0f}
	 *
	 * @see #isUndecorated
	 * @see Component#isDisplayable
	 * @see Window#getShape
	...
	void modalShow() {
		...
		for (Dialog d : modalDialogs) {
			if (d.shouldBlock(this)) {
				...
				if ((w == this) || !shouldBlock(d)
						|| (modalityType.compareTo(d.getModalityType()) < 0)) {
					blockers.add(d);
				}
			}
		}
		...
		for (Window w : unblockedWindows) {
			if (shouldBlock(w) && !blockersHierarchies.contains(w)) {
				if ((w instanceof Dialog) && ((Dialog) w).isModal_NoClientCode()) {
					Dialog wd = (Dialog) w;
					if (wd.shouldBlock(this)
							&& (modalDialogs.indexOf(wd) > modalDialogs.indexOf(this))) {
						continue;
					}
				}
				toBlock.add(w);
