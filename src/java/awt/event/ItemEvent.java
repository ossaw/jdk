 ...
 * List) when an item is selected or deselected by the user. The event is passed
 * to every <code>ItemListener</code> object which registered to receive such
 * events using the component's <code>addItemListener</code> method.
 * <P>
 * The object that implements the <code>ItemListener</code> interface gets this
 * <code>ItemEvent</code> when the event occurs. The listener is spared the
 * details of processing individual mouse movements and mouse clicks, and can
 * instead process a "meaningful" (semantic) event like "item selected" or
 * "item deselected".
 * <p>
 * An unspecified behavior will be caused if the {@code id} parameter of any
 * particular {@code ItemEvent} instance is not in the range from
 * {@code ITEM_FIRST} to {@code ITEM_LAST}.
 * <p>
 * The {@code stateChange} of any {@code ItemEvent} instance takes one of the
 * following values:
 * <ul>
