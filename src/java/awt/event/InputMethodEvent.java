...
/**
 * Input method events contain information about text that is being composed
 * using an input method. Whenever the text changes, the input method sends an
 * event. If the text component that's currently using the input method is an
 * active client, the event is dispatched to that component. Otherwise, it is
 * dispatched to a separate composition window.
 *
 * <p>
 ...
 * together replace any uncommitted composed text sent in previous events, or
 * the currently selected committed text. Committed text should be integrated
 * into the text component's persistent data, it will not be sent again.
 * Composed text may be sent repeatedly, with changes to reflect the user's
 * editing operations. Committed text always precedes composed text.
 *
...
public class InputMethodEvent extends AWTEvent {
	 ...
	 * This event is generated by input methods while processing input if only
	 * the caret changed.
	 */
	@Native
	 ...
	 *                                the event type
	 * @param when
	 *                                a long integer that specifies the time the
	 *                                event occurred
	 * @param text
	 *                                the combined committed and composed text,
	 *                                committed text
	 *                                first; must be <code>null</code> when the
	 *                                event type is
	 *                                <code>CARET_POSITION_CHANGED</code>; may
	 *                                be <code>null</code>
	 *                                for <code>INPUT_METHOD_TEXT_CHANGED</code>
	 *                                if there's no
	 *                                committed or composed text
	 * @param committedCharacterCount
	 *                                the number of committed characters in the
	 *                                text
	 * @param caret
	 *                                the caret (a.k.a. insertion point);
	 *                                <code>null</code> if
	 *                                there's no caret within current composed
	 *                                text
	 * @param visiblePosition
	 *                                the position that's most important to be
	 *                                visible;
	 *                                <code>null</code> if there's no
	 *                                recommendation for a visible
	 *                                position within current composed text
	 * @throws IllegalArgumentException
	 *                                  if <code>id</code> is not in the range
	 *                                  <code>INPUT_METHOD_FIRST</code>..
	 *                                  <code>INPUT_METHOD_LAST</code>; or if id
	 *                                  is
	 *                                  <code>CARET_POSITION_CHANGED</code> and
	 *                                  <code>text</code> is
	 *                                  not <code>null</code>; or if
	 *                                  <code>committedCharacterCount</code> is
	 *                                  not in the range
	 *                                  <code>0</code>..
	...
	public InputMethodEvent(Component source, int id, long when,
		...
		if (committedCharacterCount < 0
				|| committedCharacterCount > textLength) {
			throw new IllegalArgumentException(
					"committedCharacterCount outside of valid range");
		}
	...
	public InputMethodEvent(Component source, int id,
			AttributedCharacterIterator text, int committedCharacterCount,
			TextHitInfo caret, TextHitInfo visiblePosition) {
		this(source, id, getMostRecentEventTimeForSource(source), text,
	 ...
	 *                        <code>null</code> if there's no recommendation for
	 *                        a visible
	 *                        position within current composed text
	 * @throws IllegalArgumentException
	 *                                  if <code>id</code> is not in the range
	...
	public InputMethodEvent(Component source, int id, TextHitInfo caret,
		...
		this(source, id, getMostRecentEventTimeForSource(source), null, 0,
				caret, visiblePosition);
	}

	/**
	 * Gets the combined committed and composed text. Characters from index 0 to
	 * index <code>getCommittedCharacterCount() - 1</code> are committed text,
	 * the remaining characters are composed text.
	 *
	 * @return the text. Always null for CARET_POSITION_CHANGED; may be null for
	 ...
	 * is, the composed text within getText() if this is an
	 * <code>INPUT_METHOD_TEXT_CHANGED</code> event, the composed text within
	 * getText() of the preceding <code>INPUT_METHOD_TEXT_CHANGED</code> event
	 * otherwise.
	 *
	 * @return the caret (a.k.a. insertion point). Null if there's no caret
	 *         within current composed text.
	 */
	 ...
	 * <p>
	 * The offset of the visible position is relative to the current composed
	 * text; that is, the composed text within getText() if this is an
	 * <code>INPUT_METHOD_TEXT_CHANGED</code> event, the composed text within
	 * getText() of the preceding <code>INPUT_METHOD_TEXT_CHANGED</code> event
	 * otherwise.
	 *
	 * @return the position that's most important to be visible. Null if there's
	 *         no recommendation for a visible position within current composed
	 *         text.
	 */
	 ...
	 * for event-logging and for debugging. It contains the event ID in text
	 * form, the characters of the committed and composed text separated by "+",
	 * the number of committed characters, the caret, and the visible position.
	 *
	...
	public String paramString() {
		...
		if (visiblePosition == null) {
			...
			visiblePositionString = "visible position: " + visiblePosition
					.toString();
		}

		return typeStr + ", " + textString + ", " + countString + ", "
				+ caretString + ", " + visiblePositionString;
	}
	...
	private void readObject(ObjectInputStream s) throws ClassNotFoundException,
		...
		s.defaultReadObject();
		if (when == 0) {
			// Can't use getMostRecentEventTimeForSource because source is
			// always null during deserialization
			when = EventQueue.getMostRecentEventTime();
		}
	}

	/**
	 * Get the most recent event time in the {@code EventQueue} which the
	 * {@code source} belongs to.
	 *
	 * @param source
	...
	private static long getMostRecentEventTimeForSource(Object source) {
		...
		EventQueue eventQueue = SunToolkit.getSystemEventQueueImplPP(
				appContext);
		return AWTAccessor.getEventQueueAccessor().getMostRecentEventTime(
				eventQueue);
	}
}
