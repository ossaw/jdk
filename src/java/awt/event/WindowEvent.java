...
public class WindowEvent extends ComponentEvent {
	...
	/**
	 * The window opened event. This event is delivered only the first time a
	 ...
	 * attempts to close the window from the window's system menu. If the
	 * program does not explicitly hide or dispose the window while processing
	...
	/**
	 * The window iconified event. This event is delivered when the window has
	 * been changed from a normal to a minimized state. For many platforms, a
	 * minimized window is displayed as the icon specified in the window's
	 * iconImage property.
	 * 
	...
	/**
	 * The window deiconified event type. This event is delivered when the
	 * window has been changed from a minimized to a normal state.
	 */
	@Native
	 ...
	 * Window becomes the focused Window, which means that the Window, or one of
	...
	/**
	 * The window-state-changed event type. This event is delivered when a
	 * Window's state is changed by virtue of it being iconified, maximized etc.
	 * 
	 * @since 1.4
	 */
	@Native
	 ...
	 *
	 * @param source
	 *            The <code>Window</code> object that originated the event
	 * @param id
	 *            An integer indicating the type of event. For information on
	 *            allowable values, see the class description for
	 *            {@link WindowEvent}
	 * @param opposite
	 *            The other window involved in the focus or activation change,
	 *            or <code>null</code>
	 * @param oldState
	 *            Previous state of the window for window state change event.
	 *            See {@code #getOldState()} for allowable values
	 * @param newState
	 *            New state of the window for window state change event. See
	 *            {@code #getNewState()} for allowable values
	...
	/**
	 * Constructs a <code>WindowEvent</code> object with the specified opposite
	 * <code>Window</code>. The opposite <code>Window</code> is the other
	 * <code>Window</code> involved in this focus or activation change. For a
	 * <code>WINDOW_ACTIVATED</code> or <code>WINDOW_GAINED_FOCUS</code> event,
	 * this is the <code>Window</code> that lost activation or focus. For a
	 * <code>WINDOW_DEACTIVATED</code> or <code>WINDOW_LOST_FOCUS</code> event,
	 * this is the <code>Window</code> that gained activation or focus. If this
	 * focus change occurs with a native application, with a Java application in
	 * a different VM, or with no other <code>Window</code>, then the opposite
	 * Window is <code>null</code>.
	 * <p>
	 * This method throws an <code>IllegalArgumentException</code> if
	 * <code>source</code> is <code>null</code>.
	 *
	 ...
	 *            An integer indicating the type of event. For information on
	 *            allowable values, see the class description for
	 *            {@link WindowEvent}. It is expected that this constructor will
	 *            not be used for other then {@code WINDOW_ACTIVATED},
	 *            {@code WINDOW_DEACTIVATED}, {@code WINDOW_GAINED_FOCUS}, or
	 *            {@code WINDOW_LOST_FOCUS}. {@code WindowEvent} types, because
	 *            the opposite <code>Window</code> of other event types will
	 *            always be {@code null}.
	 * @param opposite
	 *            The other <code>Window</code> involved in the focus or
	 *            activation change, or <code>null</code>
	 * @throws IllegalArgumentException
	 *             if <code>source</code> is null
	 * @see #getWindow()
	 * @see #getID()
	...
	/**
	 * Constructs a <code>WindowEvent</code> object with the specified previous
	 * and new window states.
	 * <p>
	 * This method throws an <code>IllegalArgumentException</code> if
	 * <code>source</code> is <code>null</code>.
	 *
	 ...
	 *            allowable values, see the class description for
	 *            {@link WindowEvent}. It is expected that this constructor will
	 *            not be used for other then {@code WINDOW_STATE_CHANGED}
	 *            {@code WindowEvent} types, because the previous and new window
	 *            states are meaningless for other event types.
	 * @param oldState
	 *            An integer representing the previous window state. See
	 *            {@code #getOldState()} for allowable values
	 * @param newState
	 *            An integer representing the new window state. See
	 *            {@code #getNewState()} for allowable values
	 * @throws IllegalArgumentException
	 *             if <code>source</code> is null
	 * @see #getWindow()
	...
	/**
	 * Returns the other Window involved in this focus or activation change. For
	 * a WINDOW_ACTIVATED or WINDOW_GAINED_FOCUS event, this is the Window that
	 * lost activation or focus. For a WINDOW_DEACTIVATED or WINDOW_LOST_FOCUS
	 * event, this is the Window that gained activation or focus. For any other
	 * type of WindowEvent, or if the focus or activation change occurs with a
	 * native application, with a Java application in a different VM or context,
	 * or with no other Window, null is returned.
	 *
	...
	/**
	 * For <code>WINDOW_STATE_CHANGED</code> events returns the previous state
	 * of the window. The state is represented as a bitwise mask.
	 * <ul>
	...
	/**
	 * For <code>WINDOW_STATE_CHANGED</code> events returns the new state of the
	 * window. The state is represented as a bitwise mask.
	 * <ul>
	 * <li><code>NORMAL</code> <br>
	 * Indicates that no state bits are set.
	 * <li><code>ICONIFIED</code>
	...
	public String paramString() {
		...
		typeStr += ",opposite=" + getOppositeWindow() + ",oldState=" + oldState + ",newState="
				+ newState;
