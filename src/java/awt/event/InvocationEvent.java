 ...
 * </code> when dispatched by the AWT event dispatcher thread. This class can be
 * used as a reference implementation of <code>ActiveEvent</code> rather than
 * declaring a new class and defining <code>dispatch()</code>.
 * <p>
 *
 * Instances of this class are placed on the <code>EventQueue</code> by calls to
 * <code>invokeLater</code> and <code>invokeAndWait</code>. Client code can use
 * this fact to write replacement functions for <code>invokeLater
 * </code> and <code>invokeAndWait</code> without writing special-case code in
 * any <code>AWTEventListener</code> objects.
 * <p>
 * An unspecified behavior will be caused if the {@code id} parameter of any
 * particular {@code InvocationEvent} instance is not in the range from
 * {@code INVOCATION_FIRST} to {@code INVOCATION_LAST}.
 *
 * @author Fred Ecks
...
public class InvocationEvent extends AWTEvent implements ActiveEvent {
	 ...
	 * exception or after the event was disposed.
	 *
	 * @see #isDispatched
	 */
	...
	/**
	 * Indicates whether the <code>run()</code> method of the
	 * <code>runnable</code> was executed or not.
	 *
	 * @see #isDispatched
	 * @since 1.7
	 */
	 ...
	 * exception instance variable. If false, Throwables are propagated up to
	 * the EventDispatchThread's dispatch loop.
	 */
	 ...
	 * <tt>InvocationEvent(source, runnable)</tt> behaves in exactly the same
	 * way as the invocation of
	 ...
	 *
	 * @param source
	 *            The <code>Object</code> that originated the event
	 * @param runnable
	 *            The <code>Runnable</code> whose <code>run</code> method will
	 *            be executed
	 ...
	 * <p>
	 * An invocation of the form <tt>InvocationEvent(source,
	 * runnable, notifier, catchThrowables)</tt> behaves in exactly the same way
	 ...
	 *            be executed
	 * @param notifier
	 *            The {@code Object} whose <code>notifyAll</code> method will be
	 *            called after <code>Runnable.run</code> has returned or thrown
	 *            an exception or after the event was disposed
	 * @param catchThrowables
	 *            Specifies whether <code>dispatch</code> should catch Throwable
	 *            when executing the <code>Runnable</code>'s <code>run</code>
	 *            method, or should instead propagate those Throwables to the
	 ...
	 * Constructs an <code>InvocationEvent</code> with the specified source
	 * which will execute the runnable's <code>run</code> method when
	 * dispatched. If listener is non-<code>null</code>,
	 * <code>listener.run()</code> will be called immediately after
	 * <code>run</code> has returned, thrown an exception or the event was
	 * disposed.
	 * <p>
	 * This method throws an <code>IllegalArgumentException</code> if
	 * <code>source</code> is <code>null</code>.
	 ...
	 *            be executed
	 * @param listener
	 *            The <code>Runnable</code>Runnable whose <code>run()</code>
	 *            method will be called after the {@code InvocationEvent} was
	...
	/**
	 * Constructs an <code>InvocationEvent</code> with the specified source and
	 * ID which will execute the runnable's <code>run</code> method when
	 * dispatched. If notifier is non-<code>null</code>, <code>notifyAll</code>
	 * will be called on it immediately after <code>run</code> has returned or
	 * thrown an exception.
	 * <p>
	 * This method throws an <code>IllegalArgumentException</code> if
	 * <code>source</code> is <code>null</code>.
	 ...
	 *            be executed
	 * @param notifier
	 *            The <code>Object</code> whose <code>notifyAll</code> method
	 *            will be called after <code>Runnable.run</code> has returned or
	 *            thrown an exception or after the event was disposed
	 * @param catchThrowables
	 *            Specifies whether <code>dispatch</code> should catch Throwable
	 *            when executing the <code>Runnable</code>'s <code>run</code>
	 *            method, or should instead propagate those Throwables to the
	...
	private InvocationEvent(Object source, int id, Runnable runnable, Object notifier,
	 ...
	 *         Exception was thrown or if this InvocationEvent does not catch
	 *         exceptions
	 */
	 ...
	 *         Throwable was thrown or if this InvocationEvent does not catch
	 *         Throwables
	 * @since 1.5
	 */
	 ...
	 * thrown while dispatching, {@code false} otherwise. The method should be
	 * called by a waiting thread that calls the {@code notifier.wait()} method.
	 * Since spurious wakeups are possible (as explained in
	 * {@link Object#wait()}), this method should be used in a waiting loop to
	 * ensure that the event got dispatched:
	 * 
	 * <pre>
	 ...
	 * {@code isDispatched()} method returns {@code false}, and the
	 * {@code while} loop executes once more, thus, causing the awakened thread
	 * to revert to the waiting mode.
	 * <p>
	 ...
	 * that the waiting thread will not enter the {@code notifier.wait()}
	 * method. Otherwise, there is no guarantee that the waiting thread will
	 * ever be woken from the wait.
	 *
	 ...
	 * 
	 * @param dispatched
	 *            true if the event was dispatched false if the event was
	 *            disposed
	 */
	...
	public String paramString() {
		...
		return typeStr + ",runnable=" + runnable + ",notifier=" + notifier + ",catchExceptions="
				+ catchExceptions + ",when=" + when;
	}
}
