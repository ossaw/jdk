...
public class MouseWheelEvent extends MouseEvent {
     ...
     * source component, type, modifiers, coordinates, scroll type, scroll
     * amount, and wheel rotation.
     * <p>
     * Absolute coordinates xAbs and yAbs are set to source's location on screen
     * plus relative coordinates x and y. xAbs and yAbs are set to zero if the
     * source is not showing.
     * <p>
     ...
     *        response to
     *        this event; valid values are
     *        <code>WHEEL_UNIT_SCROLL</code>
     *        and <code>WHEEL_BLOCK_SCROLL</code>
     * @param scrollAmount
     *        for scrollType <code>WHEEL_UNIT_SCROLL</code>, the
     *        number of
     *        units to be scrolled
     * @param wheelRotation
     ...
     * @see MouseEvent#MouseEvent(java.awt.Component, int, long, int, int, int,
     *      int, boolean)
     * @see MouseEvent#MouseEvent(java.awt.Component, int, long, int, int, int,
     ...
     * passed to the constructor, the MouseWheelEvent instance is still created
     ...
     * @since 1.6
     */
    public MouseWheelEvent(Component source, int id, long when, int modifiers, int x, int y, int xAbs,
            int yAbs, int clickCount, boolean popupTrigger, int scrollType, int scrollAmount,
            int wheelRotation) {
     ...
     * scroll type, scroll amount, and wheel rotation.
     * <p>
     * Note that passing in an invalid <code>id</code> parameter results in
     * unspecified behavior. This method throws an
     * <code>IllegalArgumentException</code> if <code>source</code> equals
     * <code>null</code>.
     * <p>
     * Even if inconsistent values for relative and absolute coordinates are
     * passed to the constructor, a <code>MouseWheelEvent</code> instance is
     * still created and no exception is thrown.
     *
     * @param source
     *        the <code>Component</code> that originated
     *        the event
     * @param id
     *        the integer value that identifies the event
     * @param when
     *        a long value that gives the time when the
     *        event occurred
     * @param modifiers
     *        the modifier keys down during event (shift,
     *        ctrl, alt, meta)
     * @param x
     *        the horizontal <code>x</code> coordinate for
     *        the mouse
     *        location
     * @param y
     *        the vertical <code>y</code> coordinate for
     *        the mouse location
     * @param xAbs
     *        the absolute horizontal <code>x</code>
     *        coordinate for the
     *        mouse location
     * @param yAbs
     *        the absolute vertical <code>y</code>
     *        coordinate for the mouse
     *        location
     * @param clickCount
     *        the number of mouse clicks associated with
     *        the event
     * @param popupTrigger
     *        a boolean value, <code>true</code> if this
     *        event is a trigger
     *        for a popup-menu
     * @param scrollType
     ...
     * @param scrollAmount
     *        for scrollType
     *        <code>WHEEL_UNIT_SCROLL</code>, the number of
     *        units to be scrolled
     * @param wheelRotation
     *        the integer number of "clicks" by which the
     *        mouse wheel was
     *        rotated
     * @param preciseWheelRotation
     *        the double number of "clicks" by which the
     *        mouse wheel was
     *        rotated
     * @throws IllegalArgumentException
     *         if <code>source</code> is null
     ...
     *         MouseWheelEvent.WHEEL_BLOCK_SCROLL, depending on the
     *         configuration of the native platform.
    ...
    /**
     * Returns the number of units that should be scrolled per click of mouse
     * wheel rotation. Only valid if <code>getScrollType</code> returns
    ...
    /**
     * Returns the number of "clicks" the mouse wheel was rotated, as an
     * integer. A partial rotation may occur if the mouse supports a
     * high-resolution wheel. In this case, the method returns zero until a full
     * "click" has been accumulated.
     *
     ...
     * A partial rotation may occur if the mouse supports a high-resolution
     * wheel. In this case, the return value will include a fractional "click".
     *
     ...
     * common-case MouseWheelListener - to scroll a ScrollPane or JScrollPane by
     * an amount which conforms to the platform settings. (Note, however, that
     * <code>ScrollPane</code> and <code>JScrollPane</code> already have this
     ...
     * <P>
     * Direction of scroll, amount of wheel movement, and platform settings for
     * wheel scrolling are all accounted for. This method does not and cannot
     * take into account value of the Adjustable/Scrollable unit increment, as
     * this will vary among scrolling components.
     * <P>
     ...
     *         of mouse wheel rotation, and on the wheel scrolling settings of
     *         the native platform
     * @see #getScrollType
     * @see #getScrollAmount
     * @see MouseWheelListener
    ...
    public String paramString() {
        ...
        return super.paramString() + ",scrollType=" + scrollTypeStr + ",scrollAmount=" + getScrollAmount()
                + ",wheelRotation=" + getWheelRotation() + ",preciseWheelRotation="
