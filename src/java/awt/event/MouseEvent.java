 ...
 * corresponding mouse events are dispatched to the first ancestor that has
 * enabled the mouse event type.
 * <p>
 ...
 * </PRE>
 * 
 * If <b>button 2</b> is released first, the <code>MOUSE_RELEASED</code>/
 * <code>MOUSE_CLICKED</code> pair for <code>BUTTON2_MASK</code> arrives first,
 ...
 * <p>
 * {@code MOUSE_DRAGGED} events are delivered to the {@code Component} in which
 * the mouse button was pressed until the mouse button is released (regardless
 * of whether the mouse position is within the bounds of the {@code Component}).
...
public class MouseEvent extends InputEvent {
    ...
    /**
     * The mouse event's x coordinate. The x value is relative to the component
    ...
    /**
     * The mouse event's y coordinate. The y value is relative to the component
     * that fired the event.
     *
     * @serial
     * @see #getY()
    ...
    /**
     * The mouse event's x absolute coordinate. In a virtual device multi-screen
     * environment in which the desktop area could span multiple physical screen
     * devices, this coordinate is relative to the virtual coordinate system.
    ...
    /**
     * The mouse event's y absolute coordinate. In a virtual device multi-screen
     * environment in which the desktop area could span multiple physical screen
     * devices, this coordinate is relative to the virtual coordinate system.
     * Otherwise, this coordinate is relative to the coordinate system
     * associated with the Component's GraphicsConfiguration.
     ...
     * <code>MOUSE_RELEASED</code>. For the above, the <code>clickCount</code>
     ...
     * MouseInfo.getNumberOfButtons()} method. This range already includes
     * constants {@code NOBUTTON}, {@code BUTTON1}, {@code BUTTON2}, and
     * {@code BUTTON3} if these buttons are present. So it is allowed to use
     * these constants too. For example, for a mouse with two buttons this field
     ...
     * {@link Toolkit#areExtraMouseButtonsEnabled()} disabled by Java then the
     * field may not contain the value larger than {@code BUTTON3}.
     * 
     * @serial
     * @see #getButton()
     ...
     * certain gestures. If <code>popupTrigger</code> = <code>false</code>, no
     * popup menu should appear. If it is <code>true</code> then a popup menu
     * should appear.
     *
     * @serial
    ...
    /**
     * A number of buttons available on the mouse at the {@code Toolkit}
     * machinery startup.
     */
     ...
     * multi-screen environment in which the desktop area could span multiple
     * physical screen devices, these coordinates are relative to the virtual
     * coordinate system. Otherwise, these coordinates are relative to the
     * coordinate system associated with the Component's GraphicsConfiguration.
     *
     * @return a <code>Point</code> object containing the absolute x and y
     *         coordinates.
    ...
    /**
     * Returns the absolute horizontal x position of the event. In a virtual
     * device multi-screen environment in which the desktop area could span
     * multiple physical screen devices, this coordinate is relative to the
     * virtual coordinate system. Otherwise, this coordinate is relative to the
     * coordinate system associated with the Component's GraphicsConfiguration.
     *
    ...
    /**
     * Returns the absolute vertical y position of the event. In a virtual
     * device multi-screen environment in which the desktop area could span
     * multiple physical screen devices, this coordinate is relative to the
     * virtual coordinate system. Otherwise, this coordinate is relative to the
     * coordinate system associated with the Component's GraphicsConfiguration.
     ...
     * component, type, time, modifiers, coordinates, click count, popupTrigger
     * flag, and button number.
     * <p>
     * Creating an invalid event (such as by using more than one of the old
     * _MASKs, or modifier/button values which don't match) results in
     * unspecified behavior. An invocation of the form
     ...
     * x, y, xAbs, yAbs, clickCount, popupTrigger, button)</tt> where xAbs and
     * yAbs defines as source's location on screen plus relative coordinates x
     * and y. xAbs and yAbs are set to zero if the source is not showing. This
     * method throws an <code>IllegalArgumentException</code> if
    ...
    /**
     * Constructs a <code>MouseEvent</code> object with the specified source
     * component, type, modifiers, coordinates, click count, and popupTrigger
     * flag. An invocation of the form
     ...
     * where xAbs and yAbs defines as source's location on screen plus relative
     * coordinates x and y. xAbs and yAbs are set to zero if the source is not
     * showing. This method throws an <code>IllegalArgumentException</code> if
     * <code>source</code> is <code>null</code>.
     *
     * @param source
     *        The <code>Component</code> that originated the event
    ...
    /*
     * if the button is an extra button and it is released or clicked then in
     * Xsystem its state is not modified. Exclude this button number from
     * ExtModifiers mask.
     */
    ...
    /**
     * Constructs a <code>MouseEvent</code> object with the specified source
     * component, type, time, modifiers, coordinates, absolute coordinates,
     * click count, popupTrigger flag, and button number.
     * <p>
     * Creating an invalid event (such as by using more than one of the old
     * _MASKs, or modifier/button values which don't match) results in
     * unspecified behavior. Even if inconsistent values for relative and
     * absolute coordinates are passed to the constructor, the mouse event
     * instance is still created and no exception is thrown. This method throws
     * an <code>IllegalArgumentException</code> if <code>source</code> is
     * <code>null</code>.
     *
     * @param source
     *        The <code>Component</code> that originated the event
    ...
    public MouseEvent(Component source, int id, long when, int modifiers, int x, int y, int xAbs, int yAbs,
        ...
        if (button > BUTTON3) {
            ...
            // correctly.
            // Being pressed they report the state=0 both on the press and on
            // the release.
            // For 1-3 buttons the state value equals zero on press and non-zero
            // on release.
            // Other modifiers like Shift, ALT etc seem report well with extra
            // buttons.
            // The problem reveals as follows: one button is pressed and then
            // another button is pressed and released.
            // So, the getModifiersEx() would not be zero due to a first button
            // and we will skip this modifier.
    ...
    /**
     * Returns which, if any, of the mouse buttons has changed state. The
     * returned value is ranged from 0 to the
     * {@link java.awt.MouseInfo#getNumberOfButtons()
     * MouseInfo.getNumberOfButtons()} value. The returned value includes at
     * least the following constants:
     * <ul>
     * <li>{@code NOBUTTON}
     * <li>{@code BUTTON1}
     * <li>{@code BUTTON2}
     * <li>{@code BUTTON3}
     * </ul>
     * It is allowed to use those constants to compare with the returned button
     * number in the application. For example,
     * 
     * <pre>
     * if (anEvent.getButton() == MouseEvent.BUTTON1) {
     * </pre>
     * 
     * In particular, for a mouse with one, two, or three buttons this method
     ...
     * <li>3 ({@code BUTTON3})
     * </ul>
     * Button numbers greater then {@code BUTTON3} have no constant identifier.
     * So if a mouse with five buttons is installed, this method may return the
     * following values:
     * <ul>
     * <li>0 ({@code NOBUTTON})
     * <li>1 ({@code BUTTON1})
     ...
     * {@link Toolkit#areExtraMouseButtonsEnabled() disabled} by Java then the
     * AWT event subsystem does not produce mouse events for the extended mouse
     ...
     *
     * @return one of the values from 0 to
     *         {@link java.awt.MouseInfo#getNumberOfButtons()
     ...
     *         {@code BUTTON2}, {@code BUTTON3}; <br>
     *         {@code NOBUTTON}, {@code BUTTON1}, {@code BUTTON2} or
     *         {@code BUTTON3} if support for the extended mouse buttons is
     *         {@link Toolkit#areExtraMouseButtonsEnabled() disabled} by Java
     * @since 1.4
     * @see Toolkit#areExtraMouseButtonsEnabled()
     * @see java.awt.MouseInfo#getNumberOfButtons()
     * @see #MouseEvent(Component, int, long, int, int, int, int, int, int,
     *      boolean, int)
     * @see InputEvent#getMaskForButton(int)
     */
     ...
     * mouse buttons that were down during the event, such as "Shift", or
     * "Ctrl+Shift". These strings can be localized by changing the
     * <code>awt.properties</code> file.
     * <p>
     * Note that the <code>InputEvent.ALT_MASK</code> and
     * <code>InputEvent.BUTTON2_MASK</code> have equal values, so the "Alt"
     * string is returned for both modifiers. Likewise, the
     * <code>InputEvent.META_MASK</code> and
     * <code>InputEvent.BUTTON3_MASK</code> have equal values, so the "Meta"
     * string is returned for both modifiers.
     * <p>
    ...
    public static String getMouseModifiersText(int modifiers) {
        ...
        // an integer as a parameter then we may not restrict this with the
        // number
        // of buttons installed on the mouse.
        // It's a temporary solution. We need to somehow hold the number of
        // buttons somewhere else.
        for (int i = 1; i <= cachedNumberOfButtons; i++) {
            mask = InputEvent.getMaskForButton(i);
            if ((modifiers & mask) != 0 && buf.indexOf(Toolkit.getProperty("AWT.button" + i, "Button"
                    + i)) == -1) // 1,2,3
                                                                                                                           // buttons
                                                                                                                           // may
                                                                                                                           // already
