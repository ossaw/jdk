...
final class RadialGradientPaintContext extends MultipleGradientPaintContext {
    ...
    RadialGradientPaintContext(RadialGradientPaint paint, ColorModel cm, Rectangle deviceBounds,
            Rectangle2D userBounds, AffineTransform t, RenderingHints hints, float cx, float cy, float r,
            float fx, float fy, float[] fractions, Color[] colors, CycleMethod cycleMethod,
    ...
    private void simpleNonCyclicFillRaster(int pixels[], int off, int adjust, int x, int y, int w, int h) {
         ...
         * 2*X*a00 + a00^2 + Y^2 + 2*Y*a10 + a10^2 = (X^2+Y^2) + 2*(X*a00+Y*a10)
         * + (a00^2+a10^2) = gRel + 2*(X*a00+Y*a10) + (a00^2+a10^2) = gRel +
         * 2*DP + SD (where DP = dot product between X,Y and a00,a10 and SD =
         * dot product square of the delta vector) For the step after that we
         * get: gRel'' = (X+2*a00)^2 + (Y+2*a10)^2 = X^2 + 4*X*a00 + 4*a00^2 +
         * Y^2 + 4*Y*a10 + 4*a10^2 = (X^2+Y^2) + 4*(X*a00+Y*a10) +
         * 4*(a00^2+a10^2) = gRel + 4*DP + 4*SD = gRel' + 2*DP + 3*SD The
         * increment changed by: (gRel'' - gRel') - (gRel' - gRel) = (2*DP +
         * 3*SD) - (2*DP + SD) = 2*SD Note that this value depends only on the
         * (inverse of the) transformation matrix and so is a constant for the
         * loop. To make this all relative to the unit circle, we need to divide
         * all values as follows: [XY] /= radius gRel /= radiusSq DP /= radiusSq
         * SD /= radiusSq
         */
        ...
        for (int j = 0; j < h; j++) {
             ...
             * to calculate sqrt(gRel) for these values since sqrt(N>=1) ==
             * (M>=1). Note that gRel follows a parabola which can only be < 1
             * for a small region around the center on each scanline. In
             * particular: gDeltaDelta is always positive gDelta is <0 until it
             * crosses the midpoint, then >0 To the left and right of that
             * region, it will always be >=1 out to infinity, so we can process
             * the line in 3 regions: out to the left - quick fill until gRel <
             * 1, updating gRel in the heart - slow fraction=sqrt fill while
             * gRel < 1 out to the right - quick fill rest of scanline, ignore
             * gRel
             */
            int i = 0;
     ...
     */
    private void cyclicCircularGradientFillRaster(int pixels[], int off, int adjust, int x, int y, int w,
            int h) {
