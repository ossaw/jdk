...
/**
 * The <code>Choice</code> class presents a pop-up menu of choices. The current
 ...
 * <hr>
 * <p>
 * After this choice menu has been added to a panel, it appears as follows in
 * its normal state:
 * <p>
 * <img src="doc-files/Choice-1.gif" alt=
 * "The following text describes the graphic" style=
 * "float:center; margin: 7px 10px;">
 * <p>
 * In the picture, <code>"Green"</code> is the current choice. Pushing the mouse
 * button down on the object causes a menu to appear with the current choice
 * highlighted.
 * <p>
 * Some native platforms do not support arbitrary resizing of
 * <code>Choice</code> components and the behavior of
 * <code>setSize()/getSize()</code> is bound by such limitations. Native GUI
 * <code>Choice</code> components' size are often bound by such attributes as
 * font size and length of items contained within the <code>Choice</code>.
 * <p>
 * 
 * @author Sami Shaio
...
public class Choice extends Component implements ItemSelectable, Accessible {
    /**
     * The items for the <code>Choice</code>. This can be a <code>null</code>
     * value.
     * 
     * @serial
     * @see #add(String)
    ...
    /**
     * The index of the current choice for this <code>Choice</code> or -1 if
     * nothing is selected.
     * 
     * @serial
     * @see #getSelectedItem()
     ...
     * item, until a different selection is made by the user by calling one of
    ...
    /**
     * Creates the <code>Choice</code>'s peer. This peer allows us to change the
     * look of the <code>Choice</code> without changing its functionality.
     * 
    ...
    /**
     * Obsolete as of Java 2 platform v1.1. Please use the <code>add</code>
     * method instead.
     * <p>
     * Adds an item to this <code>Choice</code> menu.
     * 
    ...
    /**
     * Inserts an item to this <code>Choice</code>, but does not invalidate the
     * <code>Choice</code>. Client methods must provide their own
    ...
    /**
     * Inserts the item into this choice at the specified position. Existing
     * items at an index greater than or equal to <code>index</code> are shifted
     * up by one to accommodate the new item. If <code>index</code> is greater
     * than or equal to the number of items in this choice, <code>item</code> is
     * added to the end of this choice.
     * <p>
     * If the item is the first one being added to the choice, then the item
     * becomes selected. Otherwise, if the selected item was one of the items
     * shifted, the first item in the choice becomes the selected item. If the
    ...
    /**
     * Removes the first occurrence of <code>item</code> from the
     * <code>Choice</code> menu. If the item being removed is the currently
     * selected item, then the first item in the choice becomes the selected
     * item. Otherwise, the currently selected item remains selected (and the
    ...
    /**
     * Removes an item from the choice menu at the specified position. If the
     * item being removed is the currently selected item, then the first item in
     * the choice becomes the selected item. Otherwise, the currently selected
    ...
    /**
     * Removes an item from the <code>Choice</code> at the specified position,
     * but does not invalidate the <code>Choice</code>. Client methods must
     ...
     * 
     * @return a string representation of the currently selected item in this
     *         choice menu
    ...
    /**
     * Returns an array (length 1) containing the currently selected item. If
     * this choice has no items, returns <code>null</code>.
     * 
     * @see ItemSelectable
     */
    ...
    /**
     * Sets the selected item in this <code>Choice</code> menu to be the item at
    ...
    /**
     * Sets the selected item in this <code>Choice</code> menu to be the item
     * whose name is equal to the specified string. If more than one item
     * matches (is equal to) the specified string, the one with the smallest
     * index is selected.
     * <p>
     * Note that this method should be primarily used to initially select an
     * item in this component. Programmatically calling this method will
     ...
     * <code>Choice</code> menu. Item events are sent in response to user input,
     * but not in response to calls to <code>select</code>. If l is
     ...
     * events from this <code>Choice</code> menu. If l is <code>null</code>, no
     ...
     *
     * @return all of this choice's <code>ItemListener</code>s or an empty array
     ...
     * <code><em>Foo</em>Listener</code>s upon this <code>Choice</code>.
     ...
     * you can query a <code>Choice</code> <code>c</code> for its item listeners
     ...
     * method. Otherwise, it calls its superclass's <code>processEvent</code>
     * method.
     * <p>
    ...
    /**
     * Processes item events occurring on this <code>Choice</code> menu by
     ...
     * serializable <code>ItemListeners</code> as optional data. The
     * non-serializable <code>ItemListeners</code> are detected and no attempt
     ...
     * <code>null</code> adds a listener to receive item events fired by the
     ...
     * <code>Choice</code>. For <code>Choice</code> components, the
    ...
    /**
     * This class implements accessibility support for the <code>Choice</code>
     * class. It provides an implementation of the Java Accessibility API
     * appropriate to choice user-interface elements.
     * 
     * @since 1.3
     */
