...
/**
 * This class represents a check box that can be included in a menu. Selecting
 * the check box in the menu changes its state from "on" to "off" or from "off"
 * to "on."
 * <p>
 ...
 * <p>
 * The item labeled <code>Check</code> shows a check box menu item in its "off"
 * state.
 * <p>
 * When a check box menu item is selected, AWT sends an item event to the item.
 * Since the event is an instance of <code>ItemEvent</code>, the
 ...
 * <code>ItemListener</code> objects that have registered an interest in item
...
public class CheckboxMenuItem extends MenuItem implements ItemSelectable, Accessible {
	...
	/**
	 * Create a check box menu item with an empty label. The item's state is
	...
	/**
	 * Create a check box menu item with the specified label. The item's state
	...
	/**
	 * Creates the peer of the checkbox item. This peer allows us to change the
	 * look of the checkbox item without changing its functionality. Most
	 ...
	 * "off."
	 *
	 * @return the state of this check box menu item, where <code>true</code>
	 *         indicates "on" and <code>false</code> indicates "off"
	 * @see #setState
	 */
	 ...
	 * box menu item will <i>not</i> trigger an <code>ItemEvent</code>. The only
	...
	/**
	 * Returns the an array (length 1) containing the checkbox menu item label
	 ...
	 * box menu item. Item events are sent in response to user actions, but not
	 ...
	 * events from this check box menu item. If l is null, no exception is
	 ...
	 * you can query a <code>CheckboxMenuItem</code> <code>c</code> for its item
	...
	/**
	 * Processes events on this check box menu item. If the event is an instance
	 * of <code>ItemEvent</code>, this method invokes the
	 * <code>processItemEvent</code> method. If the event is not an item event,
	 ...
	 * <p>
	 * This method is not called unless item events are enabled for this menu
	 * item. Item events are enabled when one of the following occurs:
	 * <ul>
	...
	void doMenuEvent(long when, int modifiers) {
		setState(!state);
		Toolkit.getEventQueue().postEvent(new ItemEvent(this, ItemEvent.ITEM_STATE_CHANGED,
				getLabel(), state ? ItemEvent.SELECTED : ItemEvent.DESELECTED));
	}

	/**
	 * Returns a string representing the state of this
	 * <code>CheckBoxMenuItem</code>. This method is intended to be used only
	 ...
	 * <code>Checkbox</code> menu item. Unrecognized keys or values will be
	 * ignored.
	 *
	...
	/**
	 * Gets the AccessibleContext associated with this CheckboxMenuItem. For
	 * checkbox menu items, the AccessibleContext takes the form of an
	 * AccessibleAWTCheckboxMenuItem. A new AccessibleAWTCheckboxMenuItem is
	 ...
	 * <code>CheckboxMenuItem</code> class. It provides an implementation of the
	 * Java Accessibility API appropriate to checkbox menu item user-interface
	 * elements.
	 * 
	 * @since 1.3
	 */
