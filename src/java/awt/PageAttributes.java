 ...
 * orientation, logical origin, print quality, and resolution of every page
 * which uses the instance. Attribute names are compliant with the Internet
 * Printing Protocol (IPP) 1.1 where possible. Attribute values are partially
 ...
 * constructor. For example, to set the color state to monochrome, use the
 * following code:
 * 
 * <pre>
...
public final class PageAttributes implements Cloneable {
	...
	public static final class MediaType extends AttributeValue {
		...
		private static final String NAMES[] = { "iso-4a0", "iso-2a0", "iso-a0", "iso-a1", "iso-a2",
				"iso-a3", "iso-a4", "iso-a5", "iso-a6", "iso-a7", "iso-a8", "iso-a9", "iso-a10",
				"iso-b0", "iso-b1", "iso-b2", "iso-b3", "iso-b4", "iso-b5", "iso-b6", "iso-b7",
				"iso-b8", "iso-b9", "iso-b10", "jis-b0", "jis-b1", "jis-b2", "jis-b3", "jis-b4",
				"jis-b5", "jis-b6", "jis-b7", "jis-b8", "jis-b9", "jis-b10", "iso-c0", "iso-c1",
				"iso-c2", "iso-c3", "iso-c4", "iso-c5", "iso-c6", "iso-c7", "iso-c8", "iso-c9",
				"iso-c10", "iso-designated-long", "executive", "folio", "invoice", "ledger",
				"na-letter", "na-legal", "quarto", "a", "b", "c", "d", "e", "na-10x15-envelope",
				"na-10x14-envelope", "na-10x13-envelope", "na-9x12-envelope", "na-9x11-envelope",
	...
	/**
	 * A type-safe enumeration of possible orientations. These orientations are
	 * in partial compliance with IPP 1.1.
	 * 
	 * @since 1.3
	 */
	 ...
	 *            an integer array of 3 elements. The first element must be
	 *            greater than 0. The second element must be must be greater
	 *            than 0. The third element must be either <code>3</code> or
	 *            <code>4</code>.
	...
	/**
	 * Specifies whether pages using these attributes will be rendered in color
	 * or monochrome. Not specifying this attribute is equivalent to specifying
	...
	/**
	 * Returns the paper size for pages using these attributes. This attribute
	 ...
	 * printer. If an exact match cannot be found, an implementation will choose
	 * the closest possible match. Not specifying this attribute is equivalent
	 * to specifying the default size for the default locale. The default size
	 * for locales in the United States and Canada is MediaType.NA_LETTER. The
	...
	/**
	 * Sets the paper size for pages using these attributes to the default size
	 * for the default locale. The default size for locales in the United States
	 * and Canada is MediaType.NA_LETTER. The default size for all other locales
	 * is MediaType.ISO_A4.
	 */
	 ...
	 * IllegalArgumentException. Not specifying the property is equivalent to
	 * calling setOrientationRequested(OrientationRequestedType.PORTRAIT).
	 *
	 * @param orientationRequested
	 *            <code>3</code> or <code>4</code>
	 * @throws IllegalArgumentException
	 *             if orientationRequested is not <code>3</code> or
	 *             <code>4</code>
	 */
	...
	/**
	 * Returns whether drawing at (0, 0) to pages using these attributes draws
	 * at the upper-left corner of the physical page, or at the upper-left
	 * corner of the printable area. (Note that these locations could be
	 * equivalent.) This attribute cannot be modified by, and is not subject to
	 * any limitations of, the implementation or the target printer.
	 *
	...
	/**
	 * Specifies whether drawing at (0, 0) to pages using these attributes draws
	 * at the upper-left corner of the physical page, or at the upper-left
	 * corner of the printable area. (Note that these locations could be
	 * equivalent.) Not specifying the property is equivalent to specifying
	 * OriginType.PHYSICAL.
	 *
	 * @param origin
	 *            OriginType.PHYSICAL or OriginType.PRINTABLE
	...
	/**
	 * Specifies the print quality for pages using these attributes. Specifying
	 * <code>3</code> denotes draft. Specifying <code>4</code> denotes normal.
	 * Specifying <code>5</code> denotes high. Specifying any other value will
	 * generate an IllegalArgumentException. Not specifying the property is
	...
	/**
	 * Returns the print resolution for pages using these attributes. Index 0 of
	 * the array specifies the cross feed direction resolution (typically the
	 * horizontal resolution). Index 1 of the array specifies the feed direction
	 ...
	 * centimeter. <code>3</code> denotes dots per inch. <code>4</code> denotes
	 * dots per centimeter.
	 *
	 * @return an integer array of 3 elements. The first element must be greater
	 *         than 0. The second element must be must be greater than 0. The
	 *         third element must be either <code>3</code> or <code>4</code>.
	 */
	 ...
	 * implementation and the target printer. Index 0 of the array specifies the
	 * cross feed direction resolution (typically the horizontal resolution).
	 * Index 1 of the array specifies the feed direction resolution (typically
	 * the vertical resolution). Index 2 of the array specifies whether the
	 * resolutions are in dots per inch or dots per centimeter. <code>3</code>
	 * denotes dots per inch. <code>4</code> denotes dots per centimeter. Note
	 * that the 1.1 printing implementation (Toolkit.getPrintJob) requires that
	 * the feed and cross feed resolutions be the same. Not specifying the
	 ...
	 * resolutions. The actual resolutions will be determined by the limitations
	 * of the implementation and the target printer. Not specifying the property
	 ...
	 * refer to the same unique enumeration object. This means that an aliased
	 * media is equal to its underlying unique media. Printer resolutions are
	 * equal if and only if the feed resolution, cross feed resolution, and
	 * units are equal.
	 *
	 * @param obj
	...
	public boolean equals(Object obj) {
		...
		return (color == rhs.color && media == rhs.media
				&& orientationRequested == rhs.orientationRequested && origin == rhs.origin
	...
	public int hashCode() {
		return (color.hashCode() << 31 ^ media.hashCode() << 24
				^ orientationRequested.hashCode() << 23 ^ origin.hashCode() << 22
				^ printQuality.hashCode() << 20 ^ printerResolution[2] >> 2 << 19
	...
	public String toString() {
		...
		return "color=" + getColor() + ",media=" + getMedia() + ",orientation-requested="
				+ getOrientationRequested() + ",origin=" + getOrigin() + ",print-quality="
				+ getPrintQuality() + ",printer-resolution=[" + printerResolution[0] + ","
