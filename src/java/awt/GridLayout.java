 ...
 * non-zero values, either by a constructor or by the <tt>setRows</tt> and
 * <tt>setColumns</tt> methods, the number of columns specified is ignored.
 * Instead, the number of columns is determined from the specified number of
 * rows and the total number of components in the layout. So, for example, if
 * three rows and two columns have been specified and nine components are added
 * to the layout, they will be displayed as three rows of three columns.
 * Specifying the number of columns affects the layout only when the number of
 * rows is set to zero.
 *
...
public class GridLayout implements LayoutManager, java.io.Serializable {
	...
	public void layoutContainer(Container parent) {
		synchronized (parent.getTreeLock()) {
			...
			int extraWidthAvailable = (widthWOInsets - (widthOnComponent * ncols + totalGapsWidth))
					/ 2;

			int totalGapsHeight = (nrows - 1) * vgap;
			int heightWOInsets = parent.height - (insets.top + insets.bottom);
			int heightOnComponent = (heightWOInsets - totalGapsHeight) / nrows;
			int extraHeightAvailable = (heightWOInsets
					- (heightOnComponent * nrows + totalGapsHeight)) / 2;
			if (ltr) {
