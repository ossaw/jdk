 ...
 * listener objects.
 * <p>
 * Extend this class to create a <code>DropTargetEvent</code> listener and
 * override the methods for the events of interest. (If you implement the
 * <code>DropTargetListener</code> interface, you have to define all of the
 * methods in it. This abstract class defines a null implementation for every
 * method except <code>drop(DropTargetDropEvent)</code>, so you only have to
 * define methods for events you care about.) You must provide an implementation
 * for at least <code>drop(DropTargetDropEvent)</code>. This method cannot have
 * a null implementation because its specification requires that you either
 * accept or reject the drop, and, if accepted, indicate whether the drop was
 * successful.
 * <p>
 * Create a listener object using the extended class and then register it with a
 * <code>DropTarget</code>. When the drag enters, moves over, or exits the
 * operable part of the drop site for that <code>DropTarget</code>, when the
