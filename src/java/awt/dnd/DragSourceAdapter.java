 ...
 * listener objects.
 * <p>
 * Extend this class to create a <code>DragSourceEvent</code> listener and
 * override the methods for the events of interest. (If you implement the
 * <code>DragSourceListener</code> interface, you have to define all of the
 ...
 * <code>DragSource</code>. When the drag enters, moves over, or exits a drop
 * site, when the drop action changes, and when the drag ends, the relevant
 * method in the listener object is invoked, and the
...
public abstract class DragSourceAdapter implements DragSourceListener,
     ...
     * This method is invoked when all the following conditions are true:
     * <UL>
     * <LI>The cursor's hotspot has moved, but still intersects the operable
     * part of the drop site associated with the previous dragEnter()
     * invocation.
     ...
     * <code>DragSourceDropEvent</code> can be used to determine the termination
     * state. The getDropAction() method returns the operation that the drop
     * site selected to apply to the Drop operation. Once this method is
     * complete, the current <code>DragSourceContext</code> and associated
