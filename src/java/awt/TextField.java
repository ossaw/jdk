...
/**
 * A <code>TextField</code> object is a text component that allows for the
 * editing of a single line of text.
 * <p>
 * For example, the following image depicts a frame with four text fields of
 * varying widths. Two of these text fields display the predefined text
 * <code>"Hello"</code>.
 * <p>
 * <img src="doc-files/TextField-1.gif" alt=
 * "The preceding text describes this image." style=
 ...
 * sent to the text field. A <code>KeyEvent</code> may be one of three types:
 * keyPressed, keyReleased, or keyTyped. The properties of a key event indicate
 * which of these types it is, as well as additional information about the
 * event, such as what modifiers are applied to the key event and the time at
 * which the event occurred.
 * <p>
 * The key event is passed to every <code>KeyListener</code> or
 * <code>KeyAdapter</code> object which registered to receive such events using
 * the component's <code>addKeyListener</code> method. (<code>KeyAdapter</code>
 * objects implement the <code>KeyListener</code> interface.)
 * <p>
 * It is also possible to fire an <code>ActionEvent</code>. If action events are
 * enabled for the text field, they may be fired by pressing the
 * <code>Return</code> key.
 * <p>
 * The <code>TextField</code> class's <code>processEvent</code> method examines
 * the action event and passes it along to <code>processActionEvent</code>. The
 * latter method redirects the event to any <code>ActionListener</code> objects
 * that have registered to receive action events generated by this text field.
 *
 * @author Sami Shaio
...
public class TextField extends TextComponent {
    ...
    /**
     * The echo character, which is used when the user wishes to disguise the
     * characters typed into the text field. The disguises are removed if
     * echoChar = <code>0</code>.
     *
     * @serial
     * @see #getEchoChar()
    ...
    /**
     * Creates the TextField's peer. The peer allows us to modify the appearance
     * of the TextField without changing its functionality.
     */
     ...
     * password. If <code>echoChar</code> = <code>0</code>, user input is echoed
     * to the screen unchanged.
     * <p>
     * A Java platform implementation may support only a limited, non-empty set
     * of echo characters. This function returns the echo character originally
     * requested via setEchoChar(). The echo character actually used by the
     * TextField implementation might be different.
     * 
     ...
     * An echo character is useful for text fields where user input should not
     * be echoed to the screen, as in the case of a text field for entering a
     * password. Setting <code>echoChar</code> = <code>0</code> allows user
     * input to be echoed to the screen again.
     * <p>
     * A Java platform implementation may support only a limited, non-empty set
     * of echo characters. Attempts to set an unsupported echo character will
     * cause the default echo character to be used instead. Subsequent calls to
     * getEchoChar() will return the echo character originally requested. This
     * might or might not be identical to the echo character actually used by
     ...
     * password.
     * 
     * @return <code>true</code> if this text field has a character set for
     *         echoing; <code>false</code> otherwise.
     * @see java.awt.TextField#setEchoChar
     * @see java.awt.TextField#getEchoChar
     */
    ...
    /**
     * Gets the minimum dimensions for a text field with the specified number of
     * columns.
     * 
     * @param columns
     *        the number of columns in this text field.
     ...
     * text field. If l is null, no exception is thrown and no action is
     * performed.
     * <p>
     ...
     * action events from this text field. If l is null, no exception is thrown
     ...
     * textfield.
     *
     * @return all of this textfield's <code>ActionListener</code>s or an empty
     ...
     * <code><em>Foo</em>Listener</code>s upon this <code>TextField</code>.
     ...
     * you can query a <code>TextField</code> <code>t</code> for its action
     ...
     * <code>ActionEvent</code>, it invokes the <code>processActionEvent</code>
     * method. Otherwise, it invokes <code>processEvent</code> on the
     * superclass.
     * <p>
    ...
    /**
     * Processes action events occurring on this text field by dispatching them
     ...
     * serializable ActionListener(s) as optional data. The non-serializable
     * ActionListener(s) are detected and no attempt is made to serialize them.
     *
     * @serialData Null terminated sequence of zero or more pairs. A pair
     *             consists of a String and Object. The String indicates the
     *             type of object and is one of the following : ActionListenerK
     ...
     * receive action events fired by the TextField. Unrecognized keys or values
     * will be ignored.
     *
     * @exception HeadlessException
     *            if
    ...
    /**
     * Gets the AccessibleContext associated with this TextField. For text
     * fields, the AccessibleContext takes the form of an
     * AccessibleAWTTextField. A new AccessibleAWTTextField instance is created
     * if necessary.
     *
     * @return an AccessibleAWTTextField that serves as the AccessibleContext of
     *         this TextField
     * @since 1.3
     */
     ...
     * <code>TextField</code> class. It provides an implementation of the Java
