...
/**
 * The <code>AlphaComposite</code> class implements basic alpha compositing
 * rules for combining source and destination colors to achieve blending and
 * transparency effects with graphics and images. The specific rules implemented
 * by this class are the basic set of 12 rules described in T. Porter and T.
 ...
 * by their corresponding alpha components. Since the <code>ColorModel</code>
 * and <code>Raster</code> classes allow the storage of pixel data in either
 * premultiplied or non-premultiplied form, all input data must be normalized
 ...
 * 12 desirable visual effects. The equations for determining
 * <em>F<sub>s</sub></em> and <em>F<sub>d</sub></em> are given in the
 * descriptions of the 12 static fields that specify visual effects. For
 * example, the description for <a href="#SRC_OVER"><code>SRC_OVER</code></a>
 * specifies that <em>F<sub>s</sub></em> = 1 and <em>F<sub>d</sub></em> = (1-
 * <em>A<sub>s</sub></em>). Once a set of equations for determining the blending
 * factors is known they can then be applied to each pixel to produce a result
 ...
 * SRC_IN rule were first applied to the source pixel against a pixel with the
 * indicated alpha by multiplying both the raw source alpha and the raw source
 * colors by the alpha in the <code>AlphaComposite</code>. This leads to the
 * following equation for producing the alpha used in the Porter and Duff
 * blending equation:
 *
 * <pre>
 *      <em>A<sub>s</sub></em> = <em>A<sub>sr</sub></em> * <em>A<sub>ac</sub></em>
 * </pre>
 *
 * All of the raw source color components need to be multiplied by the alpha in
 * the <code>AlphaComposite</code> instance. Additionally, if the source was not
 * in premultiplied form then the color components also need to be multiplied by
 ...
 *      <em>C<sub>s</sub></em> = <em>C<sub>sr</sub></em> * <em>A<sub>ac</sub></em>           (if source is premultiplied)
 * </pre>
 *
 ...
 *      <em>C<sub>d</sub></em> = <em>C<sub>dr</sub></em>         (if destination is premultiplied)
 * </pre>
 *
 ...
 *      <em>C<sub>df</sub></em> = <em>C<sub>r</sub></em> / <em>A<sub>r</sub></em>            (if dest is not premultiplied)
 * </pre>
 *
 ...
 * object created by the <code>AlphaComposite</code> class have premultiplied
 ...
 * class produces. It is recommended that such destinations should treat their
 ...
 * destination for a sequence of a few to a dozen compositing operations. An
 * image format with fewer than 8 bits of storage per component is of limited
 * use for just one or two compositing operations before the rounding errors
 * dominate the results. An image format that does not separately store color
 * components is not a good candidate for any type of translucent blending. For
 * example, <code>BufferedImage.TYPE_BYTE_INDEXED</code> should not be used as a
 ...
 * uses 8 bits per component for storage. The stored values for a nearly
 * transparent darkened red might be:
 *
 * <pre>
 ...
 * would indicate that the results were (in integer format):
 *
 * <pre>
 ...
 * try to store this result back into a destination that is not premultiplied,
 * dividing out the alpha will give us very few choices for the
 * non-premultiplied red value. In this case an implementation that performs the
 * math in integer space without shortcuts is likely to end up with the final
 * pixel values of:
 *
 * <pre>
 ...
 * little, if any, roundoff error. Or, an implementation using integer math
 ...
 * equations. To account for such differences, it is probably best to expect
 * only that the premultiplied form of the results to match between
 * implementations and image formats. In this case both answers, expressed in
 * premultiplied form would equate to:
 *
 * <pre>
 *    (A, R, G, B) = (0x01, 0x01, 0x00, 0x00)
 * </pre>
 ...
 * simplification of removing the divide by alpha in the case of the SRC rule is
 * technically not valid if the denominator (alpha) is 0. But, since the results
 * should only be expected to be accurate when viewed in premultiplied form, a
 * resulting alpha of 0 essentially renders the resulting color components
...
public final class AlphaComposite implements Composite {
	 ...
	 *             if <code>rule</code> is not one of the following:
	 *             {@link #CLEAR}, {@link #SRC}, {@link #DST}, {@link #SRC_OVER}
	 *             , {@link #DST_OVER}, {@link #SRC_IN}, {@link #DST_IN},
	 *             {@link #SRC_OUT}, {@link #DST_OUT}, {@link #SRC_ATOP},
	 ...
	 *            source. <code>alpha</code> must be a floating point number in
	 *            the inclusive range [0.0,&nbsp;1.0].
	 * @throws IllegalArgumentException
	 *             if <code>alpha</code> is less than 0.0 or greater than 1.0,
	 *             or if <code>rule</code> is not one of the following:
	 *             {@link #CLEAR}, {@link #SRC}, {@link #DST}, {@link #SRC_OVER}
	 *             , {@link #DST_OVER}, {@link #SRC_IN}, {@link #DST_IN},
	 *             {@link #SRC_OUT}, {@link #DST_OUT}, {@link #SRC_ATOP},
	 *             {@link #DST_ATOP}, or {@link #XOR}
