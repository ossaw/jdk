 ...
 * code, see
 * <a href="http://www.oracle.com/technetwork/java/painting-140037.html">
...
public abstract class Component implements ImageObserver, MenuContainer,
    ...
    private static final PlatformLogger mixingLog = PlatformLogger.getLogger(
            "java.awt.mixing.Component");

    /**
     * The peer of the component. The peer implements the component's behavior.
     * The peer is set when the <code>Component</code> is added to a container
     * that also is a peer.
     * 
     * @see #addNotify
     * @see #removeNotify
    ...
    /**
     * A reference to a <code>GraphicsConfiguration</code> object used to
     * describe the characteristics of a graphics destination. This value can be
     * <code>null</code>.
     *
     * @since 1.3
     * @serial
    ...
    /**
     * A reference to a <code>BufferStrategy</code> object used to manipulate
     * the buffers on this component.
     *
     * @since 1.4
    ...
    public enum BaselineResizeBehavior {
         ...
         * int nextBaseline = getBaseline(preferredSize.width, preferredSize.height
         *         + 1);
     ...
     * the user-specified shape of the component. The 'null' value means the
     * component has normal shape (or has no shape at all) and
     * applyCompoundShape() will skip the following shape identical to normal.
     */
    private transient Region compoundShape = null;

    /*
     * Represents the shape of this lightweight component to be cut out from
     * heavyweight components should they intersect. Possible values: 1. null -
     * consider the shape rectangular 2. EMPTY_REGION - nothing gets cut out
     * (children still get cut out) 3. non-empty - this shape gets cut out.
     */
    ...
    static {
        AWTAccessor.setComponentAccessor(new AWTAccessor.ComponentAccessor() {
            public void setBackgroundEraseDisabled(Component comp,
                    boolean disabled) {
            ...
            public boolean requestFocus(Component comp,
     ...
     * synchronization monitor) for AWT component-tree and layout operations.
     * 
    ...
    /**
     * Assuming that mouse location is stored in PointerInfo passed to this
     * method, it finds a Component that is in the same Window as this Component
     * and is located under the mouse pointer. If no such Component exists, null
     * is returned. NOTE: this method should be called under the protection of
    ...
    Component findUnderMouseInWindow(PointerInfo pi) {
        ...
        if (!Toolkit.getDefaultToolkit().getMouseInfoPeer().isWindowUnderMouse(
                win)) {
            return null;
        }
        ...
        Component inTheSameWindow = win.findComponentAt(relativeToWindow.x,
                relativeToWindow.y, INCLUDE_DISABLED);
        return inTheSameWindow;
    }

    /**
     * Returns the position of the mouse pointer in this <code>Component</code>
     * 's coordinate space if the <code>Component</code> is directly under the
     * mouse pointer, otherwise returns <code>null</code>. If the
     * <code>Component</code> is not showing on the screen, this method returns
     * <code>null</code> even if the mouse pointer is above the area where the
     * <code>Component</code> would be displayed. If the <code>Component</code>
     * is partially or fully obscured by other <code>Component</code>s or native
     * windows, this method returns a non-null value only if the mouse pointer
     * is located above the unobscured part of the <code>Component</code>.
     * <p>
            ...
            synchronized (getTreeLock()) {
                ...
                if (peer != null) {
                    ...
                    createHierarchyEvents(HierarchyEvent.HIERARCHY_CHANGED,
                            this, parent, HierarchyEvent.SHOWING_CHANGED,
                ...
                if (componentListener != null || (eventMask
                        & AWTEvent.COMPONENT_EVENT_MASK) != 0 || Toolkit
                                .enabledOnToolkit(
     ...
     *                                           if the <code>Component</code>
     *                                           does not have its own locale
     *                                           and has not yet been added to a
     *                                           containment hierarchy such
     *                                           that the locale can be
     *                                           determined from the containing
     *                                           parent
     * @since JDK1.1
     */
     ...
     *                                        if the component is not showing on
     *                                        the screen
     * @see #setLocation
     * @see #getLocation
     */
            ...
            try {
                ...
                if (peer != null) {
                    ...
                    if (!(peer instanceof LightweightPeer)) {
                        ...
                        resized = (oldWidth != this.width)
                                || (oldHeight != this.height);
                        moved = (oldX != this.x) || (oldY != this.y);
                        // fix for 5025858: do not send ComponentEvents for
                        // toplevel
                        // windows here as it is done from peer or native code
                        // when
    ...
    private void notifyNewBounds(boolean resized, boolean moved) {
        if (componentListener != null || (eventMask
                & AWTEvent.COMPONENT_EVENT_MASK) != 0 || Toolkit
                        .enabledOnToolkit(AWTEvent.COMPONENT_EVENT_MASK)) {
            if (resized) {
                ComponentEvent e = new ComponentEvent(this,
            ...
            if (this instanceof Container && ((Container) this)
                    .countComponents() > 0) {
                boolean enabledOnToolkit = Toolkit.enabledOnToolkit(
     ...
     * <code>null</code> value otherwise returns false.
     *
    ...
    public FontMetrics getFontMetrics(Font font) {
        ...
        if (fm instanceof SunFontManager && ((SunFontManager) fm)
    ...
    public void paintAll(Graphics g) {
        if (isShowing()) {
            GraphicsCallback.PeerPaintCallback.getInstance().runOneComponent(
                    this, new Rectangle(0, 0, width, height), g, g.getClip(),
    ...
    public void repaint(long tm, int x, int y, int width, int height) {
        if (this.peer instanceof LightweightPeer) {
            ...
            if (isVisible() && (this.peer != null) && (width > 0)
                    && (height > 0)) {
    ...
    public void printAll(Graphics g) {
        if (isShowing()) {
            GraphicsCallback.PeerPrintCallback.getInstance().runOneComponent(
                    this, new Rectangle(0, 0, width, height), g, g.getClip(),
                    GraphicsCallback.LIGHTWEIGHTS
                            | GraphicsCallback.HEAVYWEIGHTS);
        }
    ...
    public VolatileImage createVolatileImage(int width, int height) {
        ...
        if (peer instanceof LightweightPeer) {
            ...
            return (peer != null) ? peer.createVolatileImage(width, height)
                    : null;
        }
    }
     ...
     *                         if an image with the specified capabilities
     *                         cannot be
     *                         created
    ...
    public boolean prepareImage(Image image, int width, int height,
        ...
        ComponentPeer peer = this.peer;
        if (peer instanceof LightweightPeer) {
            return (parent != null) ? parent.prepareImage(image, width, height,
                    observer)
                    : getToolkit().prepareImage(image, width, height, observer);
        } else {
            return (peer != null) ? peer.prepareImage(image, width, height,
                    observer)
                    : getToolkit().prepareImage(image, width, height, observer);
        }
    }
     ...
     * 
     * @param image
     *                 the <code>Image</code> object whose status is being
     *                 checked
     * @param observer
     *                 the <code>ImageObserver</code> object to be notified as
     *                 the
     ...
     *                 checked
     * @param width
     *                 the width of the scaled version whose status is to be
     *                 checked
     * @param height
    ...
    public int checkImage(Image image, int width, int height,
        ...
        ComponentPeer peer = this.peer;
        if (peer instanceof LightweightPeer) {
            return (parent != null) ? parent.checkImage(image, width, height,
                    observer)
                    : getToolkit().checkImage(image, width, height, observer);
        } else {
            return (peer != null) ? peer.checkImage(image, width, height,
                    observer)
                    : getToolkit().checkImage(image, width, height, observer);
        }
    }
     ...
     * supplied. It will always create a <code>BufferStrategy</code> with that
     * number of buffers. A page-flipping strategy is attempted first, then a
     * blitting strategy using accelerated buffers. Finally, an unaccelerated
     * blitting strategy is used.
     * <p>
     ...
     * buffer capabilities).
     * <p>
     * Each time this method is called, <code>dispose</code> will be invoked on
     * the existing <code>BufferStrategy</code>.
     * 
     * @param numBuffers
    ...
    void createBufferStrategy(int numBuffers, BufferCapabilities caps)
        ...
        if (numBuffers == 1) {
            ...
            SunGraphicsEnvironment sge = (SunGraphicsEnvironment) GraphicsEnvironment
            ...
            if (caps.isPageFlipping()) {
                bufferStrategy = new FlipSubRegionBufferStrategy(numBuffers,
                        caps);
            } else {
                bufferStrategy = new BltSubRegionBufferStrategy(numBuffers,
                        caps);
            }
        }
    }
    ...
    /**
     * This is a proxy capabilities class used when a FlipBufferStrategy is
     * created instead of the requested Blit strategy.
     *
    ...
    private class ProxyCapabilities extends ExtendedBufferCapabilities {
        ...
        private ProxyCapabilities(BufferCapabilities orig) {
            super(orig.getFrontBufferCapabilities(), orig
                    .getBackBufferCapabilities(), orig
                            .getFlipContents() == BufferCapabilities.FlipContents.BACKGROUND
     ...
     *         BufferStrategy. If there is no BufferStrategy or no back buffer,
    ...
    /**
     * Inner class for flipping buffers on a component. That component must be a
    ...
    protected class FlipBufferStrategy extends BufferStrategy {
        ...
        /**
         * Whether or not the drawing buffer has been recently restored from a
         * lost state.
         */
        protected boolean validatedContents; // = false
        /**
         * Size of the back buffers. (Note: these fields were added in 6.0 but
         * kept package-private to avoid exposing them in the spec. None of
         * these fields/methods really should have been marked protected when
         * they were introduced in 1.4, but now we just have to live with that
         * decision.)
         */
        int width;
        int height;
        ...
        /**
         * Creates a new flipping buffer strategy for this component. The
         * component must be a <code>Canvas</code> or <code>Window</code>.
         * 
         * @see Canvas
         * @see Window
         * @param numBuffers
         ...
         * @exception AWTException
         *                                     if the capabilities supplied
         *                                     could not be supported or
         *                                     met
         * @exception ClassCastException
         *                                     if the component is not a canvas
         *                                     or window.
         * @exception IllegalStateException
         *                                     if the component has no peer
         * @exception IllegalArgumentException
         *                                     if {@code numBuffers} is less
         *                                     than two, or if
         *                                     {@code BufferCapabilities.isPageFlipping}
         *                                     is not
         *                                     {@code true}.
         ...
         * @param caps
         *                   the capabilities of the buffers.
         *                   <code>BufferCapabilities.isPageFlipping</code> must
         *                   be
         *                   <code>true</code>.
         * @exception AWTException
         *                                     if the capabilities supplied
         ...
         *                                     or if
         *                                     <code>BufferCapabilities.isPageFlipping</code>
         *                                     is not
         *                                     <code>true</code>.
        ...
        /**
         * Updates internal buffers (both volatile and non-volatile) by
         * requesting the back-buffer from the peer.
         */
         ...
         * @exception IllegalStateException
         *                                  if the buffers have not yet been
         *                                  created
         */
         ...
         * 
         * @param flipAction
         *                   an integer value describing the flipping action for
         *                   the
         *                   contents of the back buffer. This should be one of
         *                   the
         *                   values of the
         *                   <code>BufferCapabilities.FlipContents</code>
         *                   property.
         * @exception IllegalStateException
         *                                  if the buffers have not yet been
         *                                  created
        ...
        protected void flip(BufferCapabilities.FlipContents flipAction) {
            if (peer != null) {
                ...
                if (backBuffer != null) {
                    peer.flip(0, 0, backBuffer.getWidth(null), backBuffer
                            .getHeight(null), flipAction);
                }
            } else {
                throw new IllegalStateException(
                        "Component must have a valid peer");
        ...
        /**
         * @return the graphics on the drawing buffer. This method may not be
         *         synchronized for performance reasons; use of this method by
         *         multiple threads should be handled at the application level.
         *         Disposal of the graphics object must be handled by the
         *         application.
         */
        ...
        /**
         * Makes specified region of the the next available buffer visible by
         * either blitting or flipping.
         */
    ...
    protected class BltBufferStrategy extends BufferStrategy {
        ...
        /**
         * Insets for the hosting Component. The size of the back buffer is
         ...
         * 
         * @param numBuffers
         *                   number of buffers to create, including the front
         *                   buffer
         * @param caps
         *                   the capabilities of the buffers
         */
        ...
        public void dispose() {
            if (backBuffers != null) {
                for (int counter = backBuffers.length
                        - 1; counter >= 0; counter--) {
        ...
        public Graphics getDrawGraphics() {
            ...
            g.constrain(-insets.left, -insets.top, backBuffer.getWidth(null)
                    + insets.left, backBuffer.getHeight(null) + insets.top);
            return g;
        }

        /**
         * @return direct access to the back buffer, as an image. If there is no
         *         back buffer, returns null.
         */
        Image getBackBuffer() {
            if (backBuffers != null) {
         ...
         */
        public void show() {
            showSubRegion(insets.left, insets.top, width - insets.right, height
                    - insets.bottom);
        }

        /**
         * Package-private method to present a specific rectangular area of this
         * buffer. This class currently shows only the entire buffer, by calling
         * showSubRegion() with the full dimensions of the buffer. Subclasses
         * (e.g., BltSubRegionBufferStrategy and FlipSubRegionBufferStrategy)
         * may have region-specific show methods that call this method with
         * actual sub regions of the buffer.
         */
        void showSubRegion(int x1, int y1, int x2, int y2) {
            if (backBuffers == null) {
                return;
            ...
            try {
                ...
                for (int i = 0; i < backBuffers.length; i++) {
                    g.drawImage(backBuffers[i], x1, y1, x2, y2, x1, y1, x2, y2,
                            null);
                    g.dispose();
                    g = null;
        ...
        void revalidate(boolean checkSize) {
            ...
            if (checkSize) {
                ...
                if (getWidth() != width || getHeight() != height || !insets
                        .equals(this.insets)) {
                    // component has been resized; recreate the backbuffers
    ...
    /**
     * Private class to perform sub-region blitting. Swing will use this
     * subclass via the SubRegionShowable interface in order to copy only the
     * area changed during a repaint. See
     ...
     * @param y
     *          the <i>y</i> coordinate
     * @return the component or subcomponent that contains the (<i>x</i>,&nbsp;
     *         <i>y</i>) location; <code>null</code> if the location is outside
     *         this component
    ...
    void dispatchEventImpl(AWTEvent e) {
        ...
        if (compContext != null && !compContext.equals(AppContext
                .getAppContext())) {
            if (eventLog.isLoggable(PlatformLogger.Level.FINE)) {
                eventLog.fine("Event " + e
        ...
        if (!(e instanceof KeyEvent)) {
            // Timestamp of a key event is set later in
            // DKFM.preDispatchKeyEvent(KeyEvent).
        ...
        if ((e instanceof FocusEvent) && focusLog.isLoggable(
        ...
        if (areInputMethodsEnabled()) {
            ...
            if (id == FocusEvent.FOCUS_GAINED) {
                InputContext inputContext = getInputContext();
                if (inputContext != null
                        && inputContext instanceof sun.awt.im.InputContext) {
            ...
            case WindowEvent.WINDOW_CLOSING:
                if (toolkit instanceof WindowClosingListener) {
                    windowClosingException = ((WindowClosingListener) toolkit)
                            .windowClosingNotify((WindowEvent) e);
        ...
        if (!(e instanceof KeyEvent)) {
            ...
            if (e instanceof FocusEvent && (tpeer == null
                    || tpeer instanceof LightweightPeer)) {
        ...
        synchronized (getTreeLock()) {
            ...
            if (anc != null && anc.eventEnabled(e)) {
                ...
                newMWE = new MouseWheelEvent(anc, // new source
                        ...
                        e.getXOnScreen(), e.getYOnScreen(), e.getClickCount(), e
                                .isPopupTrigger(), e.getScrollType(), e
                                        .getScrollAmount(), e
                                                .getWheelRotation(), e
     ...
     * component.
     *
     * @return all <code>ComponentListener</code>s of this component or an empty
    ...
    public void addHierarchyListener(HierarchyListener l) {
        ...
        synchronized (this) {
            notifyAncestors = (hierarchyListener == null && (eventMask
    ...
    public void removeHierarchyListener(HierarchyListener l) {
        ...
        synchronized (this) {
            notifyAncestors = (hierarchyListener != null && (eventMask
                    & AWTEvent.HIERARCHY_EVENT_MASK) == 0);
            hierarchyListener = AWTEventMulticaster.remove(hierarchyListener,
                    l);
            notifyAncestors = (notifyAncestors && hierarchyListener == null);
        }
        if (notifyAncestors) {
    ...
    public void addHierarchyBoundsListener(HierarchyBoundsListener l) {
        ...
        boolean notifyAncestors;
        synchronized (this) {
            notifyAncestors = (hierarchyBoundsListener == null && (eventMask
                    & AWTEvent.HIERARCHY_BOUNDS_EVENT_MASK) == 0);
            hierarchyBoundsListener = AWTEventMulticaster.add(
    ...
    public void removeHierarchyBoundsListener(HierarchyBoundsListener l) {
        ...
        synchronized (this) {
            notifyAncestors = (hierarchyBoundsListener != null && (eventMask
                    & AWTEvent.HIERARCHY_BOUNDS_EVENT_MASK) == 0);
            hierarchyBoundsListener = AWTEventMulticaster.remove(
                    hierarchyBoundsListener, l);
            notifyAncestors = (notifyAncestors
                    && hierarchyBoundsListener == null);
        }
    ...
    int numListening(long mask) {
        ...
        if ((mask == AWTEvent.HIERARCHY_EVENT_MASK && (hierarchyListener != null
                ...
                || (mask == AWTEvent.HIERARCHY_BOUNDS_EVENT_MASK
                        && (hierarchyBoundsListener != null || (eventMask
                                & AWTEvent.HIERARCHY_BOUNDS_EVENT_MASK) != 0))) {
            return 1;
        } else {
            return 0;
    ...
    int createHierarchyEvents(int id, Component changed,
        ...
        switch (id) {
            case HierarchyEvent.HIERARCHY_CHANGED:
                if (hierarchyListener != null || (eventMask
                        & AWTEvent.HIERARCHY_EVENT_MASK) != 0
                        || enabledOnToolkit) {
            ...
            case HierarchyEvent.ANCESTOR_RESIZED:
                ...
                if (hierarchyBoundsListener != null || (eventMask
                        & AWTEvent.HIERARCHY_BOUNDS_EVENT_MASK) != 0
                        || enabledOnToolkit) {
                    HierarchyEvent e = new HierarchyEvent(this, id, changed,
                            changedParent);
     ...
     * <p>
     * Refer to <a href=
     * "{@docRoot}/java/awt/doc-files/AWTThreadIssues.html#ListenersThreads" >
    ...
    public synchronized void removeInputMethodListener(InputMethodListener l) {
        ...
        inputMethodListener = AWTEventMulticaster.remove(inputMethodListener,
                l);
    }
    ...
    protected final void enableEvents(long eventsToEnable) {
        ...
        synchronized (this) {
            if ((eventsToEnable & AWTEvent.HIERARCHY_EVENT_MASK) != 0
                    && hierarchyListener == null && (eventMask
                            & AWTEvent.HIERARCHY_EVENT_MASK) == 0) {
     ...
     */
    private static final Class[] coalesceEventsParams = { AWTEvent.class,
            AWTEvent.class };
    ...
    public void addNotify() {
        synchronized (getTreeLock()) {
            ...
            if (peer == null || peer instanceof LightweightPeer) {
                ...
                if (parent != null) {
                    long mask = 0;
                    if ((mouseListener != null) || ((eventMask
                            & AWTEvent.MOUSE_EVENT_MASK) != 0)) {
                        mask |= AWTEvent.MOUSE_EVENT_MASK;
                    }
                    if ((mouseMotionListener != null) || ((eventMask
                            & AWTEvent.MOUSE_MOTION_EVENT_MASK) != 0)) {
                        mask |= AWTEvent.MOUSE_MOTION_EVENT_MASK;
                    }
                    if ((mouseWheelListener != null) || ((eventMask
                            & AWTEvent.MOUSE_WHEEL_EVENT_MASK) != 0)) {
                        mask |= AWTEvent.MOUSE_WHEEL_EVENT_MASK;
                    }
                    if (focusListener != null || (eventMask
                            & AWTEvent.FOCUS_EVENT_MASK) != 0) {
                        mask |= AWTEvent.FOCUS_EVENT_MASK;
                    }
                    if (keyListener != null || (eventMask
            ...
            if (hierarchyListener != null || (eventMask
                    & AWTEvent.HIERARCHY_EVENT_MASK) != 0 || Toolkit
                            .enabledOnToolkit(AWTEvent.HIERARCHY_EVENT_MASK)) {
                HierarchyEvent e = new HierarchyEvent(this,
                        HierarchyEvent.HIERARCHY_CHANGED, this, parent,
    ...
    public void removeNotify() {
        ...
        synchronized (getTreeLock()) {
            if (isFocusOwner() && KeyboardFocusManager
    ...
    final void setFocusTraversalKeys_NoIDCheck(int id,
        ...
        synchronized (this) {
            ...
            if (keystrokes != null) {
                for (AWTKeyStroke keystroke : keystrokes) {
                    ...
                    for (int i = 0; i < focusTraversalKeys.length; i++) {
                        ...
                        if (getFocusTraversalKeys_NoIDCheck(i).contains(
                                keystroke)) {
                            throw new IllegalArgumentException(
                                    "focus traversal keys must be unique for a Component");
                        }
                    }
                }
            }
            ...
            focusTraversalKeys[id] = (keystrokes != null) ? Collections
                    .unmodifiableSet(new HashSet<AWTKeyStroke>(keystrokes))
                    : null;
        }
     ...
     *                                  whether focus traversal keys are enabled
     *                                  for this Component
     * @see #getFocusTraversalKeysEnabled
     * @see #setFocusTraversalKeys
     * @see #getFocusTraversalKeys
     ...
     * this method can be guaranteed only for lightweight <code>Component</code>
     * s. This method is not intended for general use, but exists instead as a
     * hook for lightweight component libraries, such as Swing.
     *
     * <p>
     * Note: Not all focus transfers result from invoking this method. As such,
     * a component may receive focus without this or any of the other
     ...
     *
     * @param temporary
     *                  true if the focus change is temporary, such as when the
     *                  window
     ...
     * value. However, because specifying an arbitrary temporary state may not
     * be implementable on all native windowing systems, correct behavior for
     * this method can be guaranteed only for lightweight components. This
     * method is not intended for general use, but exists instead as a hook for
     * lightweight component libraries, such as Swing.
     *
     * <p>
     * Note: Not all focus transfers result from invoking this method. As such,
     * a component may receive focus without this or any of the other
    ...
    final boolean requestFocusHelper(boolean temporary,
        ...
        // 1) Check if the event being dispatched is a system-generated mouse
        // event.
        AWTEvent currentEvent = EventQueue.getCurrentEvent();
        if (currentEvent instanceof MouseEvent && SunToolkit.isSystemGenerated(
                currentEvent)) {
            // 2) Sanity check: if the mouse event component source belongs to
            // the same containing window.
            Component source = ((MouseEvent) currentEvent).getComponent();
            if (source == null || source
                    .getContainingWindow() == getContainingWindow()) {
                focusLog.finest(
                        "requesting focus by mouse event \"in window\"");

                // If both the conditions are fulfilled the focus request should
                // be strictly
                // bounded by the toplevel window. It's assumed that the mouse
                // event activates
                // the window (if it wasn't active) and this makes it possible
                // for a focus
                // request with a strong in-window requirement to change focus
                // in the bounds
                // of the toplevel. If, by any means, due to asynchronous nature
                // of the event
                // dispatching mechanism, the window happens to be natively
                // inactive by the time
                // this focus request is eventually handled, it should not
                // re-activate the
                // toplevel. Otherwise the result may not meet user
                // expectations. See 6981400.
        ...
        if (EventQueue.isDispatchThread()) {
            ...
            // A focus request made from outside EDT should not be associated
            // with any event
    ...
    private boolean isRequestFocusAccepted(boolean temporary,
        ...
        Component focusOwner = KeyboardFocusManager.getMostRecentFocusOwner(
                window);
        if (focusOwner == null) {
            // sometimes most recent focus owner may be null, but focus owner is
            // not
        ...
        boolean ret = Component.requestFocusController.acceptRequestFocus(
                focusOwner, this, temporary, focusedWindowChangeAllowed, cause);
        if (focusLog.isLoggable(PlatformLogger.Level.FINEST)) {
    ...
    // Swing access this method through reflection to implement InputVerifier's
    // functionality.
    ...
    boolean transferFocus(boolean clearOnFailure) {
        ...
        if (toFocus != null && !toFocus.isFocusOwner() && toFocus != this) {
            res = toFocus.requestFocusInWindow(
    ...
    public void firePropertyChange(String propertyName, byte oldValue,
        ...
        firePropertyChange(propertyName, Byte.valueOf(oldValue), Byte.valueOf(
                newValue));
    }
    ...
    public void firePropertyChange(String propertyName, char oldValue,
        ...
        firePropertyChange(propertyName, new Character(oldValue), new Character(
                newValue));
    }
    ...
    public void firePropertyChange(String propertyName, short oldValue,
        ...
        firePropertyChange(propertyName, Short.valueOf(oldValue), Short.valueOf(
                newValue));
    }
    ...
    public void firePropertyChange(String propertyName, long oldValue,
        ...
        firePropertyChange(propertyName, Long.valueOf(oldValue), Long.valueOf(
                newValue));
    }
    ...
    public void firePropertyChange(String propertyName, float oldValue,
        ...
        firePropertyChange(propertyName, Float.valueOf(oldValue), Float.valueOf(
                newValue));
    }
    ...
    public void firePropertyChange(String propertyName, double oldValue,
        ...
        firePropertyChange(propertyName, Double.valueOf(oldValue), Double
                .valueOf(newValue));
    }
    ...
    /**
     * This hack is for Swing serialization. It will invoke the Swing package
     * private method <code>compWriteObjectNotify</code>.
     */
    private void doSwingSerialization() {
        ...
        for (Class<?> klass = Component.this
                .getClass(); klass != null; klass = klass.getSuperclass()) {
            if (klass.getPackage() == swingPackage && klass
                    .getClassLoader() == null) {
                ...
                for (int counter = methods.length
                        - 1; counter >= 0; counter--) {
    ...
    /**
     * Writes default serializable fields to stream. Writes a variety of
     * serializable listeners as optional data. The non-serializable listeners
     ...
     *             type of object and is one of the following (as of 1.4):
     *             <code>componentListenerK</code> indicating an
     *             <code>ComponentListener</code> object;
     *             <code>focusListenerK</code> indicating an
     *             <code>FocusListener</code> object; <code>keyListenerK</code>
     *             indicating an <code>KeyListener</code> object;
     ...
     * @serialData an optional <code>ComponentOrientation</code> (after
     *             <code>inputMethodListener</code>, as of 1.2)
     *
     ...
     * <code>null</code> adds a listener to receive a variety of events fired by
     * the component. Unrecognized keys or values will be ignored.
     *
     * @param s
        ...
        try {
            while (null != (keyOrNull = s.readObject())) {
                ...
                if (hierarchyListenerK == key) {
                    ...
                    addHierarchyBoundsListener((HierarchyBoundsListener) (s
                            .readObject()));
                } else {
    ...
    final void relocateComponent() {
        synchronized (getTreeLock()) {
            ...
            int nativeX = x;
            int nativeY = y;
            for (Component cont = getContainer(); cont != null && cont
     ...
     * Component will contain all of the methods in interface Accessible, though
     * it won't actually implement the interface - that will be up to the
     * individual objects which extend Component.
     */
     ...
     * <code>Component</code>. The method implemented by this base class returns
     * null. Classes that extend <code>Component</code> should implement this
     * method to return the <code>AccessibleContext</code> associated with the
     * subclass.
     *
     *
     * @return the <code>AccessibleContext</code> of this <code>Component</code>
     * @since 1.3
     */
    ...
    protected abstract class AccessibleAWTComponent extends AccessibleContext
         ...
         * add/remove ComponentListener and FocusListener to track target
         * Component's state.
         */
        ...
        /**
         * Fire PropertyChange listener, if one is registered, when
         * shown/hidden..
         * 
         * @since 1.3
         */
        ...
        public void addPropertyChangeListener(PropertyChangeListener listener) {
            ...
            if (propertyListenersCount++ == 0) {
                Component.this.addComponentListener(
        ...
        public void removePropertyChangeListener(
                PropertyChangeListener listener) {
            if (--propertyListenersCount == 0) {
                Component.this.removeComponentListener(
                        accessibleAWTComponentHandler);
         ...
         * return <code>java.awt.Component.getName()</code>, as that generally
         * isn't a localized name, and doesn't have meaning for the user. If the
         * object is fundamentally a text object (e.g. a menu item), the
         * accessible name should be the text of the object (e.g. "save"). If
         * the object has a tooltip, the tooltip text may also be an appropriate
         * String to return.
         *
         ...
         * object - e.g. a "Save" icon on a toolbar that had "save" as the
         * tooltip text shouldn't return the tooltip text as the description,
         * but something like "Saves the current text document" instead).
         *
         ...
         *
         * @return an instance of <code>AccessibleRole</code> describing the
         *         role of the object
         ...
         *
         * @return an instance of <code>AccessibleStateSet</code> containing the
        ...
        /**
         * Gets the <code>Accessible</code> parent of this object. If the parent
         * of this object implements <code>Accessible</code>, this method should
         * simply return <code>getParent</code>.
         *
         * @return the <code>Accessible</code> parent of this object -- can be
         *         <code>null</code> if this object does not have an
         ...
         *
         * @return the index of this object in its parent; or -1 if this object
        ...
        /**
         * Gets the <code>AccessibleComponent</code> associated with this object
         * if one exists. Otherwise return <code>null</code>.
         *
         ...
         * implementation and/or the native system do not support changing the
         * mouse cursor shape.
         * 
         * @param cursor
         ...
         * @param f
         *          the <code>Font</code>
         * @return the <code>FontMetrics</code>, if supported, the object;
         *         otherwise, <code>null</code>
         * @see #getFont
         */
         ...
         * screen because one of the objects that this object is contained by is
         ...
         * example, it happens to be underneath a menu that was pulled down).
         *
         ...
         *
         * @param p
         *          the <code>Point</code> relative to the coordinate system
         *          of the object
         ...
         *
         * @return location of object on screen -- can be <code>null</code> if
        ...
        /**
         * Gets the location of the object relative to the parent in the form of
         ...
         *         object's bounds in the coordinate space of the screen;
         *         <code>null</code> if this object or its parent are not on the
         *         screen
         */
        public Point getLocation() {
         ...
         * bounds specify this object's width, height, and location relative to
         * its parent.
         *
         * @return a rectangle indicating this component's bounds;
         ...
         * <code>Rectangle</code> object. The bounds specify this object's
         * width, height, and location relative to its parent.
         *
         * @param r
         ...
         * <code>Dimension</code> object contains this objects's height, and the
         * width field of the <code>Dimension</code> object contains this
         * object's width.
         *
         * @return a <code>Dimension</code> object that indicates the size of
         *         this component; <code>null</code> if this object is not on
         *         the screen
         */
        public Dimension getSize() {
    ...
    /**
     * Gets the index of this object in its accessible parent. If this object
     * does not have an accessible parent, returns -1.
     *
    ...
    AccessibleStateSet getAccessibleStateSet() {
        synchronized (getTreeLock()) {
            ...
            if (this instanceof Accessible) {
                AccessibleContext ac = ((Accessible) this)
     ...
     * @param className
     *                  The name of the class. Must be fully-qualified class
     *                  name.
            ...
            if (peer != null) {
                ...
                if (shape.equals(getNormalShape())) {
                    ...
                    if (mixingLog.isLoggable(PlatformLogger.Level.FINER)) {
                        mixingLog.fine("this = " + this + "; compAbsolute="
                                + compAbsolute + "; shape=" + shape);
                    }
    ...
    /**
     * Returns the shape previously set with applyCompoundShape(). If the
     * component is LW or no shape was applied yet, the method returns the
     * normal shape.
     */
    private Region getAppliedShape() {
        checkTreeLock();
        // XXX: if we allow LW components to have a shape, this must be changed
        return (this.compoundShape == null || isLightweight())
                ? getNormalShape()
                : this.compoundShape;
    }
    ...
    Point getLocationOnWindow() {
        ...
        for (Container parent = getContainer(); parent != null
                && !(parent instanceof Window); parent = parent
                        .getContainer()) {
    ...
    final Region getNormalShape() {
        checkTreeLock();
        // XXX: we may take into account a user-specified shape for this
        // component
        Point compAbsolute = getLocationOnWindow();
        return Region.getInstanceXYWH(compAbsolute.x, compAbsolute.y,
                getWidth(), getHeight());
    }
    ...
    final ComponentPeer getHWPeerAboveMe() {
        ...
        while (cont != null) {
            for (int i = indexAbove; i > -1; i--) {
                ...
                if (comp != null && comp.isDisplayable() && !comp
                        .isLightweight()) {
                    return comp.getPeer();
                }
            }
    ...
    private Region calculateCurrentShape() {
        ...
        if (getContainer() != null) {
            ...
            while (cont != null) {
                for (int index = comp
                        .getSiblingIndexAbove(); index != -1; --index) {
                    /*
                     * It is assumed that:
                     * getComponent(getContainer().getComponentZOrder(comp)) ==
                     * comp
    ...
    void applyCurrentShape() {
        ...
        if (!areBoundsValid()) {
            ...
            return; // Because applyCompoundShape() ignores such components
                    // anyway
        }
        if (mixingLog.isLoggable(PlatformLogger.Level.FINE)) {
    ...
    private final void applyCurrentShapeBelowMe() {
        ...
        if (parent != null && parent.isShowing()) {
            ...
            // Second, if my container is non-opaque, reapply shapes of siblings
            // of my container
            Container parent2 = parent.getContainer();
            while (!parent.isOpaque() && parent2 != null) {
                parent2.recursiveApplyCurrentShape(parent
    ...
    final void subtractAndApplyShapeBelowMe() {
        ...
        if (parent != null && isShowing()) {
            ...
            // Second, if my container is non-opaque, cut siblings of my
            // container
            Container parent2 = parent.getContainer();
            while (!parent.isOpaque() && parent2 != null) {
                parent2.recursiveSubtractAndApplyShape(opaqueShape, parent
                        .getSiblingIndexBelow());
    ...
    void mixOnHiding(boolean isLightweight) {
        // We cannot be sure that the peer exists at this point, so we need the
        // argument
        // to find out whether the hiding component is (well, actually was) a LW
        // or a HW.
        synchronized (getTreeLock()) {
            if (mixingLog.isLoggable(PlatformLogger.Level.FINE)) {
    ...
    void mixOnZOrderChanging(int oldZorder, int newZorder) {
        synchronized (getTreeLock()) {
            ...
            if (mixingLog.isLoggable(PlatformLogger.Level.FINE)) {
                mixingLog.fine("this = " + this + "; oldZorder=" + oldZorder
                        + "; newZorder=" + newZorder + "; parent=" + parent);
            }
            if (!isMixingNeeded()) {
                return;
            }
    ...
    final boolean isMixingNeeded() {
        ...
        Window window = getContainingWindow();
        if (window != null) {
            if (!window.hasHeavyweightDescendants() || !window
                    .hasLightweightDescendants() || window.isDisposing()) {
                if (mixingLog.isLoggable(PlatformLogger.Level.FINE)) {
                    mixingLog.fine("containing window = " + window
                            + "; has h/w descendants = " + window
                                    .hasHeavyweightDescendants()
                            + "; has l/w descendants = " + window
