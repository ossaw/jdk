 ...
 * "https://docs.oracle.com/javase/tutorial/uiswing/misc/focus.html#transferTiming">
 * Timing Focus Transfers</a>, a section in
...
public abstract class Toolkit {
    ...
    /**
     * Creates this toolkit's implementation of the <code>Desktop</code> using
    ...
    /**
     * Creates this toolkit's implementation of <code>TextField</code> using the
     * specified peer interface.
     * 
     * @param target
    ...
    /**
     * Creates this toolkit's implementation of <code>Label</code> using the
     * specified peer interface.
     * 
     * @param target
    ...
    /**
     * Creates this toolkit's implementation of <code>List</code> using the
     * specified peer interface.
     * 
     * @param target
    ...
    /**
     * Creates this toolkit's implementation of <code>Checkbox</code> using the
     * specified peer interface.
     * 
     * @param target
    ...
    /**
     * Creates this toolkit's implementation of <code>Scrollbar</code> using the
     * specified peer interface.
     * 
     * @param target
    ...
    /**
     * Creates this toolkit's implementation of <code>TextArea</code> using the
     * specified peer interface.
     * 
     * @param target
    ...
    /**
     * Creates this toolkit's implementation of <code>Choice</code> using the
     * specified peer interface.
     * 
     * @param target
    ...
    /**
     * Creates this toolkit's implementation of <code>Frame</code> using the
     * specified peer interface.
     * 
     * @param target
    ...
    /**
     * Creates this toolkit's implementation of <code>Canvas</code> using the
     * specified peer interface.
     * 
     * @param target
    ...
    /**
     * Creates this toolkit's implementation of <code>Panel</code> using the
     * specified peer interface.
     * 
     * @param target
    ...
    /**
     * Creates this toolkit's implementation of <code>Window</code> using the
     * specified peer interface.
     * 
     * @param target
    ...
    /**
     * Creates this toolkit's implementation of <code>Dialog</code> using the
     * specified peer interface.
     * 
     * @param target
    ...
    /**
     * Creates this toolkit's implementation of <code>MenuBar</code> using the
     * specified peer interface.
     * 
     * @param target
    ...
    /**
     * Creates this toolkit's implementation of <code>Menu</code> using the
     * specified peer interface.
     * 
     * @param target
    ...
    /**
     * Creates this toolkit's implementation of <code>PopupMenu</code> using the
     * specified peer interface.
     * 
     * @param target
    ...
    /**
     * Creates this toolkit's implementation of <code>MenuItem</code> using the
     * specified peer interface.
     * 
     * @param target
    ...
    /**
     * Creates this toolkit's implementation of <code>CheckboxMenuItem</code>
     ...
     * 
     * @return this toolkit's implementation of helper for
     *         <code>MouseInfo</code>
    ...
    /**
     * Creates this toolkit's implementation of <code>Font</code> using the
     * specified peer interface.
     * 
     * @param name
     *        the font to be implemented
     * @param style
     *        the style of the font, such as <code>PLAIN</code>,
     ...
     * resizing, or statically, after resizing is complete. Use
     * {@code isDynamicLayoutActive()} to detect if this feature enabled in this
     * program and is supported by this operating system and/or window manager.
     * Note that this feature is supported not on all platforms, and conversely,
     * that this feature cannot be turned off on some platforms. On these
     * platforms where dynamic layout during resizing is not supported (or is
     * always supported), setting this property has no effect. Note that this
     * feature can be set or unset as a property of the operating system or
     * window manager on some platforms. On such platforms, the dynamic resize
     * property must be set at the operating system or window manager level
     * before this method can take effect. This method does not change support
     * or settings of the underlying operating system or window manager. The
     * OS/WM support can be queried using
     * getDesktopProperty("awt.dynamicLayoutSupported") method.
     *
     * @param dynamic
     *        If true, Containers should re-layout their components as
     *        the
     *        Container is being resized. If false, the layout will be
     *        validated after resizing is completed.
     * @exception HeadlessException
     *            if GraphicsEnvironment.isHeadless() returns
     *            true
     ...
     * resizing, or statically, after resizing is complete. Note: this method
     * returns the value that was set programmatically; it does not reflect
     * support at the level of the operating system or window manager for
     * dynamic layout on resizing, or the current operating system or window
     * manager settings. The OS/WM support can be queried using
     * getDesktopProperty("awt.dynamicLayoutSupported").
     *
     * @return true if validation of Containers is done dynamically, false if
     *         validation is done after resizing is finished.
     * @exception HeadlessException
     *            if GraphicsEnvironment.isHeadless() returns
     *            true
     ...
     * supported by the underlying operating system and/or window manager). If
     * dynamic layout is currently inactive then Containers re-layout their
     ...
     * 
     * @param gc
     *        a <code>GraphicsConfiguration</code>
     ...
     * <p>
     * <code>ColorModel</code> is an abstract class that encapsulates the
     * ability to translate between the pixel values of an image and its red,
     ...
     * @deprecated see
     *             {@link java.awt.GraphicsEnvironment#getAvailableFontFamilyNames()}
    ...
    /**
     * Used internally by the assistive technologies functions; set at init time
     * and used at load time
     */
    private static String atNames;

    /**
     * Initializes properties related to assistive technologies. These
     * properties are used both in the loadAssistiveProperties() function below,
     * as well as other classes in the jdk that depend on the properties (such
     * as the use of the screen_magnifier_present property in Java2D hardware
     * acceleration initialization). The initialization of the properties must
     * be done before the platform- specific Toolkit class is instantiated so
     * that all necessary properties are set up properly before any classes
    ...
    private static void initAssistiveTechnologies() {
        ...
        atNames = java.security.AccessController.doPrivileged(new java.security.PrivilegedAction<String>() {
            public String run() {
                ...
                // Try loading the per-user accessibility properties
                // file.
                try {
                    ...
                    // Per-user accessibility properties file does not
                    // exist
                }
                ...
                if (properties.size() == 0) {
                    try {
                        File propsFile = new File(System.getProperty("java.home") + sep + "lib" + sep
                                + "accessibility.properties");
                        FileInputStream in = new FileInputStream(propsFile);
                        ...
                        // System-wide accessibility properties file
                        // does
                        // not exist;
                    }
                }

                // Get whether a screen magnifier is present. First
                // check
                // the system property and then check the properties
                // file.
                ...
                // Get the names of any assistive technolgies to load.
                // First
                // check the system property and then check the
                // properties
                // file.
     ...
     * 'assistive_technologies' specified in the Sun reference implementation by
     * a line in the 'accessibility.properties' file. The form is
     * "assistive_technologies=..." where the "..." is a comma-separated list of
     * assistive technology classes to load. Each class is loaded in the order
     * given and a single instance of each is created using
     * Class.forName(class).newInstance(). All errors are handled via an
     * AWTError exception.
     * <p>
     * The assumption is made that assistive technology classes are supplied as
     * part of INSTALLED (as opposed to: BUNDLED) extensions or specified on the
     * class path (and therefore can be loaded using the class loader returned
     * by a call to <code>ClassLoader.getSystemClassLoader</code>, whose
        ...
        if (atNames != null) {
            ...
            while (parser.hasMoreTokens()) {
                ...
                try {
                    ...
                    throw new AWTError("Error trying to install Assistive" + " Technology: " + atName + " "
                            + e);
                }
            }
        }
     ...
     * <p>
     * If a system property named <code>"java.awt.headless"</code> is set to
     * <code>true</code> then the headless implementation of
     ...
     * <code>"awt.toolkit"</code>, that property is treated as the name of a
     * class that is a subclass of <code>Toolkit</code>; otherwise the default
     * platform-specific implementation of <code>Toolkit</code> is used.
     * <p>
     ...
     * assistive technology classes to load. Each class is loaded in the order
     * given and a single instance of each is created using
     * Class.forName(class).newInstance(). This is done just after the AWT
     * toolkit is created. All errors are handled via an AWTError exception.
     * 
     * @return the default toolkit.
     * @exception AWTError
     *            if a toolkit could not be found, or if one could not
     *            be
    ...
    /**
     * Returns an image which gets pixel data from the specified file, whose
     * format can be either GIF, JPEG or PNG. The underlying toolkit attempts to
     * resolve multiple requests with the same filename to the same returned
     * Image.
     * <p>
     * Since the mechanism required to facilitate this sharing of
     * <code>Image</code> objects may continue to hold onto images that are no
     * longer in use for an indefinite period of time, developers are encouraged
     * to implement their own caching of images by using the
     * {@link #createImage(java.lang.String) createImage} variant wherever
     * available. If the image data contained in the specified file changes, the
     * <code>Image</code> object returned from this method may still contain
     * stale information which was loaded from the file after a prior call.
     * Previously loaded image data can be manually discarded by calling the
     * {@link Image#flush flush} method on the returned <code>Image</code>.
     * <p>
     * This method first checks if there is a security manager installed. If so,
     * the method calls the security manager's <code>checkRead</code> method
     * with the file specified to ensure that the access to the image is
     * allowed.
     * 
     * @param filename
     *        the name of a file containing pixel data in a recognized
     *        file
     *        format.
    ...
    /**
     * Returns an image which gets pixel data from the specified URL. The pixel
     * data referenced by the specified URL must be in one of the following
     * formats: GIF, JPEG or PNG. The underlying toolkit attempts to resolve
     * multiple requests with the same URL to the same returned Image.
     * <p>
     * Since the mechanism required to facilitate this sharing of
     * <code>Image</code> objects may continue to hold onto images that are no
     * longer in use for an indefinite period of time, developers are encouraged
     * to implement their own caching of images by using the
     * {@link #createImage(java.net.URL) createImage} variant wherever
     * available. If the image data stored at the specified URL changes, the
     * <code>Image</code> object returned from this method may still contain
     * stale information which was fetched from the URL after a prior call.
     * Previously loaded image data can be manually discarded by calling the
     * {@link Image#flush flush} method on the returned <code>Image</code>.
     * <p>
     * This method first checks if there is a security manager installed. If so,
     * the method calls the security manager's <code>checkPermission</code>
     * method with the url.openConnection().getPermission() permission to ensure
     * that the access to the image is allowed. For compatibility with pre-1.2
     * security managers, if the access is denied with
     * <code>FilePermission</code> or <code>SocketPermission</code>, the method
     * throws the <code>SecurityException</code> if the corresponding 1.1-style
     * SecurityManager.checkXXX method also denies permission.
     * 
     * @param url
     ...
     * This method first checks if there is a security manager installed. If so,
     * the method calls the security manager's <code>checkRead</code> method
     * with the specified file to ensure that the image creation is allowed.
     * 
     * @param filename
     *        the name of a file containing pixel data in a recognized
     *        file
     ...
     * the method calls the security manager's <code>checkPermission</code>
     * method with the url.openConnection().getPermission() permission to ensure
     * that the image creation is allowed. For compatibility with pre-1.2
     * security managers, if the access is denied with
     * <code>FilePermission</code> or <code>SocketPermission</code>, the method
     * throws <code>SecurityException</code> if the corresponding 1.1-style
     * SecurityManager.checkXXX method also denies permission.
     * 
     * @param url
     *        the URL to use in fetching the pixel data.
     ...
     *        <code>-1</code>.
     * @param observer
     *        the <code>ImageObserver</code> object to be notified as
     *        the
     ...
     * @param width
     *        the width of the scaled version whose status is being
     *        checked,
     *        or <code>-1</code>.
     * @param height
     ...
     * array.
     * <p>
     * The data must be in some image format, such as GIF or JPEG, that is
     * supported by this toolkit.
     * 
     * @param imagedata
     *        an array of bytes, representing image data in a
     *        supported
     *        image format.
     * @return an image.
     * @since JDK1.1
    ...
    /**
     * Gets a <code>PrintJob</code> object which is the result of initiating a
     * print operation on the toolkit's platform.
     * <p>
     * Each actual implementation of this method should first check if there is
     * a security manager installed. If there is, the method should call the
     * security manager's <code>checkPrintJobAccess</code> method to ensure
     * initiation of a print operation is allowed. If the default implementation
     * of <code>checkPrintJobAccess</code> is used (that is, that method is not
     * overriden), then this results in a call to the security manager's
     * <code>checkPermission</code> method with a <code>
     ...
     *        Properties are not standardized and are not consistent
     *        across
     *        implementations. Because of this, PrintJobs which require
     *        job
     *        and page control should use the version of this function
     *        which
     *        takes JobAttributes and PageAttributes objects. This
     *        object
     *        may be updated to reflect the user's job choices on exit.
     *        May
     *        be null.
     * @return a <code>PrintJob</code> object, or <code>null</code> if the user
     ...
     *        the parent of the print dialog. May not be null.
     * @param jobtitle
     *        the title of the PrintJob. A null title is
     *        equivalent to "".
     * @param jobAttributes
     *        a set of job attributes which will control the
     *        PrintJob. The
     *        attributes will be updated to reflect the user's
     *        choices as
     *        outlined in the JobAttributes documentation. May be
     *        null.
     * @param pageAttributes
     *        a set of page attributes which will control the
     *        PrintJob. The
     *        attributes will be applied to every page in the
     *        job. The
     *        attributes will be updated to reflect the user's
     *        choices as
     *        outlined in the PageAttributes documentation. May
     *        be null.
     * @return a <code>PrintJob</code> object, or <code>null</code> if the user
     *         cancelled the print job.
     * @throws NullPointerException
     ...
     *         cross feed and feed
     *         resolutions. Also if this thread has
     *         access to the file
     *         system and jobAttributes specifies print
     *         to file, and the
     *         specified destination file exists but is
     *         a directory rather
     *         than a regular file, does not exist but
     *         cannot be created, or
     *         cannot be opened for any other reason.
     *         However in the case of
     *         print to file, if a dialog is also
     *         requested to be displayed
     *         then the user will be given an
     *         opportunity to select a file
     *         and proceed with printing. The dialog
     *         will ensure that the
     *         selected output file is valid before
     *         returning from this
     *         method.
     * @throws SecurityException
     *         if this thread is not allowed to
     *         initiate a print job
     *         request, or if jobAttributes specifies
     *         print to file, and
     *         this thread is not allowed to access the
     *         file system
     * @see java.awt.PrintJob
     * @see java.awt.GraphicsEnvironment#isHeadless
     * @see java.lang.RuntimePermission
     ...
     * clipboard facilities provided by the native platform. This clipboard
     * enables data transfer between Java programs and native applications which
     * use native clipboard facilities.
     * <p>
     ...
     * equivalent flavor, a Reader is returned. <b>Note:</b> The behavior of the
     * system Clipboard's <code>getTransferData()</code> method for <code>
     ...
     * the user selects text, using either the mouse or the keyboard. Typically,
     * this is implemented by installing a <code>FocusListener</code> on all
     * <code>Component</code>s which support text selection, and, between
     * <code>FOCUS_GAINED</code> and <code>FOCUS_LOST</code> events delivered to
     * that <code>Component</code>, updating the system selection
     * <code>Clipboard</code> when the selection changes inside the
     * <code>Component</code>. Properly updating the system selection ensures
     * that a Java application will interact correctly with native applications
     * and other Java applications running simultaneously on the system. Note
     * that <code>java.awt.TextComponent</code> and
     ...
     * shortcuts.
     * <p>
     * Menu shortcuts, which are embodied in the <code>MenuShortcut</code>
     * class, are handled by the <code>MenuBar</code> class.
     * <p>
     * By default, this method returns <code>Event.CTRL_MASK</code>. Toolkit
     * implementations should override this method if the <b>Control</b> key
     * isn't the correct key for accelerators.
     * 
     * @return the modifier mask on the <code>Event</code> class that is used
     ...
     *            if <code>keyCode</code> is not one of the
     *            valid key codes
     * @exception java.lang.UnsupportedOperationException
     *            if the host system doesn't allow getting the
     *            state of this
     *            key programmatically, or if the keyboard
     *            doesn't have this
     *            key
     * @exception HeadlessException
     *            if GraphicsEnvironment.isHeadless() returns
    ...
    /**
     * Sets the state of the given locking key on the keyboard. Valid key codes
     * are {@link java.awt.event.KeyEvent#VK_CAPS_LOCK VK_CAPS_LOCK},
     * {@link java.awt.event.KeyEvent#VK_NUM_LOCK VK_NUM_LOCK},
     * {@link java.awt.event.KeyEvent#VK_SCROLL_LOCK VK_SCROLL_LOCK}, and
     * {@link java.awt.event.KeyEvent#VK_KANA_LOCK VK_KANA_LOCK}.
     * <p>
     * Depending on the platform, setting the state of a locking key may involve
     * event processing and therefore may not be immediately observable through
     * getLockingKeyState.
     *
     * @exception java.lang.IllegalArgumentException
     *            if <code>keyCode</code> is not one of the
     *            valid key codes
     * @exception java.lang.UnsupportedOperationException
     *            if the host system doesn't allow setting the
     *            state of this
     *            key programmatically, or if the keyboard
     *            doesn't have this
     *            key
    ...
    /**
     * Give native peers the ability to query the native container given a
     * native component (eg the direct parent may be lightweight).
     */
    ...
    /**
     * Creates a new custom cursor object. If the image to display is invalid,
     * the cursor will be hidden (made completely transparent), and the hotspot
     * will be set to (0, 0).
     * <p>
     ...
     * @param hotSpot
     *        the X and Y of the large cursor's hot spot; the hotSpot
     *        values
     *        must be less than the Dimension returned by
     *        <code>getBestCursorSize</code>
     * @param name
     *        a localized description of the cursor, for Java
     *        Accessibility
     *        use
     ...
     * resize the image to a supported size. Since converting low-resolution
     * images is difficult, no guarantees are made as to the quality of a cursor
     * image which isn't a supported size. It is therefore recommended that this
     * method be called and an appropriate image used so no image conversion is
     * made.
     *
     * @param preferredWidth
     *        the preferred cursor width the component would
     *        like to use.
     * @param preferredHeight
     *        the preferred cursor height the component would
     *        like to use.
     * @return the closest matching supported cursor size, or a dimension of 0,0
     *         if the Toolkit implementation doesn't support custom cursors.
     * @exception HeadlessException
     *            if GraphicsEnvironment.isHeadless() returns
     *            true
     * @see java.awt.GraphicsEnvironment#isHeadless
     ...
     * supported maximum, the Toolkit implementation will attempt to flatten the
     * palette to the maximum. Since converting low-resolution images is
     * difficult, no guarantees are made as to the quality of a cursor image
     * which has more colors than the system supports. It is therefore
     * recommended that this method be called and an appropriate image used so
     * no image conversion is made.
     *
    ...
    /**
     * Returns whether Toolkit supports this state for <code>Frame</code>s. This
     * method tells whether the <em>UI
     * concept</em> of, say, maximization or iconification is supported. It will
     * always return false for "compound" states like
     * <code>Frame.ICONIFIED|Frame.MAXIMIZED_VERT</code>. In other words, the
     * rule of thumb is that only queries with a single frame state constant as
     * an argument are meaningful.
     * <p>
     * Note that supporting a given concept is a platform- dependent feature.
     * Due to native limitations the Toolkit object may report a particular
     * state as supported, however at the same time the Toolkit object will be
     * unable to apply the state to a given frame. This circumstance has two
     * following consequences:
     * <ul>
     * <li>Only the return value of {@code false} for the present method
     * actually indicates that the given state is not supported. If the method
     * returns {@code true} the given state may still be unsupported and/or
     * unavailable for a particular frame.
     * <li>The developer should consider examining the value of the
     * {@link java.awt.event.WindowEvent#getNewState} method of the
     * {@code WindowEvent} received through the
     * {@link java.awt.event.WindowStateListener}, rather than assuming that the
     * state given to the {@code setExtendedState()} method will be definitely
     * applied. For more information see the documentation for the
     * {@link Frame#setExtendedState} method.
     * </ul>
     *
     * @param state
     *        one of named frame state constants.
     * @return <code>true</code> is this frame state is supported by this
     *         Toolkit implementation, <code>false</code> otherwise.
     * @exception HeadlessException
     *            if
     ...
     * sun.awt.resources.awt.properties. The ResourceBundle is stored here, so
     ...
     * loads native libraries. A number of classes in the AWT package have a
     * native method, initIDs(), which initializes the JNI field and method ids
     * used in the native portion of their implementation.
     ...
     * Motif, Microsoft Windows, or Tiny. The problem is that this means that
     * the native libraries must be loaded by the java.* classes, which do not
     * necessarily know the names of the libraries to load. A better way of
     ...
     * java.awt.image.ColorModel.loadLibraries(). Unfortunately, classes can be
     * loaded in java.awt.image that depend on libawt and there is no way to
     * call Toolkit.loadLibraries() directly. -hung
     */
    ...
    static {
        ...
        java.security.AccessController.doPrivileged(new java.security.PrivilegedAction<Void>() {
            public Void run() {
                try {
                    resources = ResourceBundle.getBundle("sun.awt.resources.awt", CoreResourceBundleControl
    ...
    /**
     * Get the application's or applet's EventQueue instance. Depending on the
     * Toolkit implementation, different EventQueues may be returned for
     * different applets. Applets should therefore not assume that the
     * EventQueue instance returned by this method will be shared by other
     * applets or the system.
     * <p>
     * If there is a security manager then its
     * {@link SecurityManager#checkPermission checkPermission} method is called
    ...
    /**
     * Gets the application's or applet's <code>EventQueue</code> instance,
     * without checking access. For security reasons, this can only be called
     * from a <code>Toolkit</code> subclass.
     * 
     * @return the <code>EventQueue</code> object
     */
     ...
     *
     * @param abstractRecognizerClass
     *        The abstract class of the required
     *        recognizer
     * @param ds
     *        The DragSource
     * @param c
     *        The Component target for the
     ...
     * A desktop property is a uniquely named value for a resource that is
     * Toolkit global in nature. Usually it also is an abstract representation
     * for an underlying platform dependent desktop setting. For more
     * information on desktop properties supported by the AWT see
     ...
     * property. When a {@link java.beans.PropertyChangeListenerProxy} object is
     * added, its property name is ignored, and the wrapped listener is added.
     * If {@code name} is {@code null} or {@code pcl} is {@code null}, no
    ...
    /**
     * Removes the specified property change listener for the named desktop
     * property. When a {@link java.beans.PropertyChangeListenerProxy} object is
     * removed, its property name is ignored, and the wrapped listener is
     * removed. If {@code name} is {@code null} or {@code pcl} is {@code null},
     ...
     * toolkit. The returned array contains
     * {@link java.beans.PropertyChangeListenerProxy} objects that associate
     * listeners with the names of desktop properties.
     *
     * @return all of this toolkit's {@link PropertyChangeListener} objects
     *         wrapped in {@code java.beans.PropertyChangeListenerProxy} objects
    ...
    /**
     * Returns an array of all property change listeners associated with the
     * specified name of a desktop property.
     *
     * @param propertyName
     *        the named property
     * @return all of the {@code PropertyChangeListener} objects associated with
     *         the specified name of a desktop property or an empty array if no
    ...
    /**
     * Returns whether the always-on-top mode is supported by this toolkit. To
     * detect whether the always-on-top mode is supported for a particular
     * Window, use {@link Window#isAlwaysOnTopSupported}.
     * 
     * @return <code>true</code>, if current toolkit supports the always-on-top
     *         mode, otherwise returns <code>false</code>
     ...
     *
     * @param modalityType
     *        modality type to be checked for support by this
     *        toolkit
     ...
     * toolkit. If an unsupported modal exclusion type property is set on a
     * window, then <code>Dialog.ModalExclusionType.NO_EXCLUDE</code> is used
     * instead.
     *
     * @param modalExclusionType
     *        modal exclusion type to be checked for support
     *        by this toolkit
     * @return <code>true</code>, if current toolkit supports given modal
     *         exclusion type, <code>false</code> otherwise
    ...
    /*
     * Extracts a "pure" AWTEventListener from a AWTEventListenerProxy, if the
     * listener is proxied.
     */
    ...
    /**
     * Adds an AWTEventListener to receive all AWTEvents dispatched system-wide
     * that conform to the given <code>eventMask</code>.
     * <p>
     * First, if there is a security manager, its <code>checkPermission</code>
     * method is called with an
     * <code>AWTPermission("listenToAllAWTEvents")</code> permission. This may
     * result in a SecurityException.
     * <p>
     * <code>eventMask</code> is a bitmask of event types to receive. It is
     * constructed by bitwise OR-ing together the event masks defined in
     * <code>AWTEvent</code>.
     * <p>
     * Note: event listener use is not recommended for normal application use,
     * but are intended solely to support special purpose facilities including
     * support for accessibility, event record/playback, and diagnostic tracing.
    ...
    public void removeAWTEventListener(AWTEventListener listener) {
        ...
        synchronized (this) {
            ...
            eventListener = ToolkitEventMulticaster.remove(eventListener, (selectiveListener == null) ? localL
                    : selectiveListener);
        }
    }
    ...
    /**
     * Returns an array of all the <code>AWTEventListener</code>s registered on
     * this toolkit. If there is a security manager, its {@code checkPermission}
     * method is called with an {@code AWTPermission("listenToAllAWTEvents")}
     * permission. This may result in a SecurityException. Listeners can be
     * returned within <code>AWTEventListenerProxy</code> objects, which also
     * contain the event mask for the given listener. Note that listener objects
     * added multiple times appear only once in the returned array.
     *
     * @return all of the <code>AWTEventListener</code>s or an empty array if no
    ...
    /**
     * Returns an array of all the <code>AWTEventListener</code>s registered on
     * this toolkit which listen to all of the event types specified in the
     * {@code eventMask} argument. If there is a security manager, its
     * {@code checkPermission} method is called with an
     * {@code AWTPermission("listenToAllAWTEvents")} permission. This may result
     * in a SecurityException. Listeners can be returned within
     * <code>AWTEventListenerProxy</code> objects, which also contain the event
     * mask for the given listener. Note that listener objects added multiple
     * times appear only once in the returned array.
     *
     * @param eventMask
     *        the bitmask of event types to listen for
     * @return all of the <code>AWTEventListener</code>s registered on this
     *         toolkit for the specified event types, or an empty array if no
    ...
    /*
     * This method notifies any AWTEventListeners that an event is about to be
     * dispatched.
    ...
    private class SelectiveAWTEventListener implements AWTEventListener {
        ...
        public void eventDispatched(AWTEvent event) {
            ...
            if (((eventBit = eventMask & AWTEvent.COMPONENT_EVENT_MASK) != 0
                    ...
                    && event.id <= ComponentEvent.COMPONENT_LAST) || ((eventBit = eventMask
                            & AWTEvent.CONTAINER_EVENT_MASK) != 0
                            && event.id >= ContainerEvent.CONTAINER_FIRST
                            && event.id <= ContainerEvent.CONTAINER_LAST) || ((eventBit = eventMask
                                    & AWTEvent.FOCUS_EVENT_MASK) != 0 && event.id >= FocusEvent.FOCUS_FIRST
                                    && event.id <= FocusEvent.FOCUS_LAST) || ((eventBit = eventMask
                                            & AWTEvent.KEY_EVENT_MASK) != 0 && event.id >= KeyEvent.KEY_FIRST
                                            && event.id <= KeyEvent.KEY_LAST) || ((eventBit = eventMask
                    ...
                    || ((eventBit = eventMask & AWTEvent.MOUSE_EVENT_MASK) != 0
                            && event.id != MouseEvent.MOUSE_MOVED && event.id != MouseEvent.MOUSE_DRAGGED
                            && event.id != MouseEvent.MOUSE_WHEEL && event.id >= MouseEvent.MOUSE_FIRST
                            && event.id <= MouseEvent.MOUSE_LAST) || ((eventBit = eventMask
                                    & AWTEvent.WINDOW_EVENT_MASK) != 0
                                    && (event.id >= WindowEvent.WINDOW_FIRST
                                            && event.id <= WindowEvent.WINDOW_LAST)) || ((eventBit = eventMask
                                                    & AWTEvent.ACTION_EVENT_MASK) != 0
                    ...
                    || ((eventBit = eventMask & AWTEvent.ADJUSTMENT_EVENT_MASK) != 0
                            ...
                            && event.id <= AdjustmentEvent.ADJUSTMENT_LAST) || ((eventBit = eventMask
                                    & AWTEvent.ITEM_EVENT_MASK) != 0 && event.id >= ItemEvent.ITEM_FIRST
                                    && event.id <= ItemEvent.ITEM_LAST) || ((eventBit = eventMask
                                            & AWTEvent.TEXT_EVENT_MASK) != 0
                                            && event.id >= TextEvent.TEXT_FIRST
                                            && event.id <= TextEvent.TEXT_LAST) || ((eventBit = eventMask
                    ...
                    || ((eventBit = eventMask & AWTEvent.PAINT_EVENT_MASK) != 0
                            && event.id >= PaintEvent.PAINT_FIRST && event.id <= PaintEvent.PAINT_LAST)
                    || ((eventBit = eventMask & AWTEvent.INVOCATION_EVENT_MASK) != 0
                            && event.id >= InvocationEvent.INVOCATION_FIRST
                            && event.id <= InvocationEvent.INVOCATION_LAST) || ((eventBit = eventMask
                                    & AWTEvent.HIERARCHY_EVENT_MASK) != 0
                                    && event.id == HierarchyEvent.HIERARCHY_CHANGED) || ((eventBit = eventMask
                    ...
                    || ((eventBit = eventMask & AWTEvent.WINDOW_STATE_EVENT_MASK) != 0
                            && event.id == WindowEvent.WINDOW_STATE_CHANGED) || ((eventBit = eventMask
                                    & AWTEvent.WINDOW_FOCUS_EVENT_MASK) != 0
    ...
    /**
     * Returns a map of visual attributes for the abstract level description of
     * the given input method highlight, or null if no mapping is found. The
     * style field of the input method highlight is ignored. The map returned is
     * unmodifiable.
     * 
     * @param highlight
    ...
    private static class DesktopPropertyChangeSupport extends PropertyChangeSupport {
        ...
        public synchronized void addPropertyChangeListener(String propertyName,
            ...
            PropertyChangeSupport pcs = (PropertyChangeSupport) AppContext.getAppContext().get(
        ...
        /*
         * we do expect that all other fireXXX() methods of
         * java.beans.PropertyChangeSupport use this method. If this will be
         * changed we will need to change this class.
         */
        @Override
    ...
    /**
     * Reports whether events from extra mouse buttons are allowed to be
     * processed and posted into {@code EventQueue}. <br>
     * To change the returned value it is necessary to set the
     * {@code sun.awt.enableExtraMouseButtons} property before the
     * {@code Toolkit} class initialization. This setting could be done on the
     ...
     * </pre>
     * 
     * Alternatively, the property could be set in the application by using the
     * following code:
     * 
     * <pre>
     ...
     * </pre>
     * 
     * before the {@code Toolkit} class initialization. If not set by the time
     * of the {@code Toolkit} class initialization, this property will be
     * initialized with {@code true}. Changing this value after the
     * {@code Toolkit} class initialization will have no effect.
     * <p>
     * 
     * @exception HeadlessException
     *            if GraphicsEnvironment.isHeadless() returns
     *            true
     * @return {@code true} if events from extra mouse buttons are allowed to be
     *         processed and posted; {@code false} otherwise
