...
public class EventQueue {
     ...
     * That is, the EventQueue object is actually implemented as NUM_PRIORITIES
     * queues and all Events on a particular internal Queue have identical
     * priority. Events are pulled off the EventQueue starting with the Queue of
     * highest priority. We progress in decreasing order across all Queues.
     */
    private Queue[] queues = new Queue[NUM_PRIORITIES];

    /*
     * The next EventQueue on the stack, or null if this EventQueue is on the
     * top of the stack. If nextQueue is non-null, requests to post an event are
     * forwarded to nextQueue.
     */
    private EventQueue nextQueue;

    /*
     * The previous EventQueue on the stack, or null if this is the "base"
     * EventQueue.
     */
     ...
     * all the EventQueues from the AppContext. Synchronization on any
     * particular event queue(s) is not enough: we should lock the whole stack.
     */
    private final Lock pushPopLock;
    private final Condition pushPopCond;

    /*
     * Dummy runnable to wake up EDT from getNextEvent() after push/pop is
     * performed
     */
    ...
    private final String name = "AWT-EventQueue-" + threadInitNumber
            .getAndIncrement();
    ...
    static {
        AWTAccessor.setEventQueueAccessor(new AWTAccessor.EventQueueAccessor() {
            ...
            public void removeSourceEvents(EventQueue eventQueue, Object source,
    ...
    public EventQueue() {
        ...
        /*
         * NOTE: if you ever have to start the associated event dispatch thread
         * at this point, be aware of the following problem: If this EventQueue
         * instance is created in SunToolkit.createNewAppContext() the started
         * dispatch thread may call AppContext.getAppContext() before
         * createNewAppContext() completes thus causing mess in thread group to
         * appcontext mapping.
         */

        appContext = AppContext.getAppContext();
        pushPopLock = (Lock) appContext.get(AppContext.EVENT_QUEUE_LOCK_KEY);
        pushPopCond = (Condition) appContext.get(
                AppContext.EVENT_QUEUE_COND_KEY);
    }

    /**
     * Posts a 1.1-style event to the <code>EventQueue</code>. If there is an
     * existing event on the queue with the same ID and event source, the source
     * <code>Component</code>'s <code>coalesceEvents</code> method will be
     * called.
     *
     * @param theEvent
     *                 an instance of <code>java.awt.AWTEvent</code>, or a
     *                 subclass
     *                 of it
     * @throws NullPointerException
     *                              if <code>theEvent</code> is
     *                              <code>null</code>
     */
    ...
    /**
     * Posts the event to the internal Queue of specified priority, coalescing
     * as appropriate.
     *
     * @param theEvent
     *                 an instance of <code>java.awt.AWTEvent</code>, or a
    ...
    /*
     * Should avoid of calling this method by any means as it's working time is
     * dependant on EQ length. In the wors case this method alone can slow down
     * the entire application 10 times by stalling the Event processing. Only
     * here by backward compatibility reasons.
     */
    private boolean coalesceOtherEvent(AWTEvent e, int priority) {
        ...
        for (EventQueueItem entry = queues[priority].head; entry != null; entry = entry.next) {
            ...
            if (entry.event.getSource() == source && entry.event
                    .getID() == id) {
    ...
    private static int eventToCacheIndex(AWTEvent e) {
        switch (e.getID()) {
            ...
            case MouseEvent.MOUSE_DRAGGED:
                // Return -1 for SunDropTargetEvent since they are usually
                // synchronous
                // and we don't want to skip them by coalescing with MouseEvent or
                // other drag events
    ...
    /**
     * Removes an event from the <code>EventQueue</code> and returns it. This
     * method will block until an event has been posted by another thread.
     * 
    ...
    public AWTEvent getNextEvent() throws InterruptedException {
        do {
             ...
             * synchronized block to avoid deadlock when event queues are nested
             * with push()/pop().
             */
            SunToolkit.flushPendingEvents(appContext);
            pushPopLock.lock();
    ...
    /**
     * Returns the first event on the <code>EventQueue</code> without removing
     * it.
     * 
     ...
     * @return the first event of the specified id or <code>null</code> if there
     *         is no such event
     */
     ...
     * 
     * @param event
     *              an instance of <code>java.awt.AWTEvent</code>, or a subclass
     *              of it
     * @throws NullPointerException
    ...
    protected void dispatchEvent(final AWTEvent event) {
        ...
        final PrivilegedAction<Void> action = new PrivilegedAction<Void>() {
            public Void run() {
                ...
                // dispatch thread (e.g. performing
                // DefaultKeyboardFocusManager.sendMessage),
        ...
        if (srcAcc == null) {
            ...
            javaSecurityAccess.doIntersectionPrivilege(
                    new PrivilegedAction<Void>() {
                        public Void run() {
                            javaSecurityAccess.doIntersectionPrivilege(action,
                                    eventAcc);
                            return null;
                        }
                    }, stack, srcAcc);
    ...
    private static AccessControlContext getAccessControlContextFrom(
            Object src) {
        return src instanceof Component ? ((Component) src)
                .getAccessControlContext()
                : src instanceof MenuComponent ? ((MenuComponent) src)
                        .getAccessControlContext()
                        : src instanceof TrayIcon ? ((TrayIcon) src)
     ...
     * dispatched, its timestamp will be returned. If no events have yet been
     * dispatched, the EventQueue's initialization time will be returned
     * instead.In the current version of the JDK, only <code>InputEvent</code>s,
     * <code>ActionEvent</code>s, and <code>InvocationEvent</code>s have
     * timestamps; however, future versions of the JDK may add timestamps to
     * additional event types. Note that this method should only be invoked from
     * an application's {@link #isDispatchThread event dispatching thread}. If
     * this method is invoked from another thread, the current system time (as
     * reported by <code>System.currentTimeMillis()</code>) will be returned
     * instead.
     *
    ...
    private AWTEvent getCurrentEventImpl() {
        pushPopLock.lock();
        try {
            return (Thread.currentThread() == dispatchThread) ? currentEvent
                    .get() : null;
        } finally {
            pushPopLock.unlock();
        }
     ...
     * pending events are transferred to the new <code>EventQueue</code> for
     * processing by it.
     *
     * @param newEventQueue
     *                      an <code>EventQueue</code> (or subclass thereof)
     *                      instance to
     *                      be use
     * @see java.awt.EventQueue#pop
     * @throws NullPointerException
     *                              if <code>newEventQueue</code> is
     *                              <code>null</code>
     * @since 1.2
     */
    public void push(EventQueue newEventQueue) {
        ...
        try {
            ...
            if ((topQueue.dispatchThread != null) && (topQueue.dispatchThread
                    .getEventQueue() == this)) {
            ...
            while (topQueue.peekEvent() != null) {
                try {
                    // Use getNextEventPrivate() as it doesn't call
                    // flushPendingEvents()
                    newEventQueue.postEventPrivate(topQueue
                            .getNextEventPrivate());
            ...
            if (topQueue.dispatchThread != null) {
                ...
                // go newEventQueue
                topQueue.postEventPrivate(new InvocationEvent(topQueue,
                        dummyRunnable));
            }
    ...
    /**
     * Stops dispatching events using this <code>EventQueue</code>. Any pending
     * events are transferred to the previous <code>EventQueue</code> for
     * processing.
     * <p>
     ...
     * Use the {@link SecondaryLoop#enter} and {@link SecondaryLoop#exit}
     * methods to start and stop the event loop and dispatch the events from
     * this queue.
     *
     * @return secondaryLoop A new secondary loop object, which can be used to
     *         launch a new nested event loop and dispatch events from this
     *         queue
     *
    ...
    SecondaryLoop createSecondaryLoop(Conditional cond, EventFilter filter,
        ...
        try {
            ...
            return new WaitDispatchSupport(dispatchThread, cond, filter,
                    interval);
        } finally {
            pushPopLock.unlock();
        }
    ...
    /**
     * Returns true if the calling thread is {@link Toolkit#getSystemEventQueue
     * the current AWT EventQueue}'s dispatch thread. Use this method to ensure
     * that a particular task is being executed (or not being) there.
     * <p>
     * Note: use the {@link #invokeLater} or {@link #invokeAndWait} methods to
     * execute a task in {@link Toolkit#getSystemEventQueue the current AWT
     * EventQueue}'s dispatch thread.
     * <p>
     *
     * @return true if running in {@link Toolkit#getSystemEventQueue the current
    ...
    final void detachDispatchThread(EventDispatchThread edt) {
        ...
        SunToolkit.flushPendingEvents(appContext);
        /*
         * This synchronized block is to secure that the event dispatch thread
         * won't die in the middle of posting a new event to the associated
         * event queue. It is important because we notify that the event
         * dispatch thread is busy after posting a new event to its queue, so
         * the EventQueue.dispatchThread reference must be valid at that point.
         */
        pushPopLock.lock();
        try {
     ...
     * <code>EventQueue</code>.
     * @return the event dispatch thread associated with this event queue or
     * <code>null</code> if this event queue doesn't have a working thread
     * associated with it
    ...
    /*
     * Removes any pending events for the specified source object. If
     * removeAllEvents parameter is <code>true</code> then all events for the
     * specified source object are removed, if it is <code>false</code> then
     * <code>SequencedEvent</code>, <code>SentEvent</code>,
     ...
     * events are removed.
     * This method is normally called by the source's <code>removeNotify</code>
     * method.
     */
    ...
    private void setCurrentEventAndMostRecentTimeImpl(AWTEvent e) {
        ...
        try {
            ...
            // a type public, or to place it in sun.awt. Both of these
            // approaches
     ...
     * in the {@link #isDispatchThread dispatch thread} of
     * {@link Toolkit#getSystemEventQueue the system EventQueue}. This will
     * happen after all pending events are processed.
     *
     * @param runnable
     *                 the <code>Runnable</code> whose <code>run</code> method
     *                 should
     *                 be executed asynchronously in the
     *                 {@link #isDispatchThread
     *                 event dispatch thread} of
     *                 {@link Toolkit#getSystemEventQueue
    ...
    public static void invokeLater(Runnable runnable) {
        Toolkit.getEventQueue().postEvent(new InvocationEvent(Toolkit
                .getDefaultToolkit(), runnable));
    }
     ...
     * in the {@link #isDispatchThread dispatch thread} of
     * {@link Toolkit#getSystemEventQueue the system EventQueue}. This will
     * happen after all pending events are processed. The call blocks until this
     * has happened. This method will throw an Error if called from the
     * {@link #isDispatchThread event dispatcher thread}.
     *
     * @param runnable
     *                 the <code>Runnable</code> whose <code>run</code> method
     *                 should
     *                 be executed synchronously in the {@link #isDispatchThread
     *                 event dispatch thread} of
     *                 {@link Toolkit#getSystemEventQueue
     *                 the system EventQueue}
     * @exception InterruptedException
     *                                      if any thread has interrupted this
     *                                      thread
     * @exception InvocationTargetException
     *                                      if an throwable is thrown when
     *                                      running
     *                                      <code>runnable</code>
     * @see #invokeLater
     * @see Toolkit#getSystemEventQueue
     ...
     * First it proceeds to the EventQueue on the top of the stack, then
     * notifies the associated dispatch thread if it exists or starts a new one
     * otherwise.
     */
 ...
 * queue. An EventQueue object is composed of multiple internal Queues, one for
 * each priority supported by the EventQueue. All Events on a particular
