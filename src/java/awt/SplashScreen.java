 ...
 * Virtual Machine (JVM) starts. The splash screen is displayed as an
 ...
 * is supported both for GIF and PNG. The window is positioned at the center of
 * the screen. The position on multi-monitor systems is not specified. It is
 ...
 * should not have a leading slash. <BR>
 ...
 * screen image, get the splash screen native window position/size, and paint in
 * the splash screen. It cannot be used to create the splash screen. You should
 * use the options provided by the Java implementation for that.
 * <p>
 * This class cannot be instantiated. Only a single instance of this class can
 * exist, and it may be obtained by using the {@link #getSplashScreen()} static
 * method. In case the splash screen has not been created at application startup
 * via the command line or manifest file option, the
 * <code>getSplashScreen</code> method returns <code>null</code>.
 *
 * @author Oleg Semenov
 * @since 1.6
 */
public final class SplashScreen {
	...
	/**
	 * Returns the {@code SplashScreen} object used for Java startup splash
	 * screen control on systems that support display.
	 *
	 * @throws UnsupportedOperationException
	 *             if the splash screen feature is not supported by the current
	 *             toolkit
	 * @throws HeadlessException
	 *             if {@code GraphicsEnvironment.isHeadless()} returns true
	 ...
	 * specified URL; GIF, JPEG and PNG image formats are supported. The method
	 * returns after the image has finished loading and the window has been
	 * updated. The splash screen window is resized according to the size of the
	 * image and is centered on the screen.
	 *
	 * @param imageURL
	 *            the non-<code>null</code> URL for the new splash screen image
	 ...
	 * This may be useful if, for example, you want to replace the splash screen
	 * with your window at the same location.
	 * <p>
	 * You cannot control the size or position of the splash screen. The splash
	 * screen size is adjusted automatically when the image changes.
	 * <p>
	...
	/**
	 * Returns the size of the splash screen window as a {@link Dimension}. This
	 * may be useful if, for example, you want to draw on the splash screen
	 * overlay surface.
	 * <p>
	 * You cannot control the size or position of the splash screen. The splash
	 * screen size is adjusted automatically when the image changes.
	...
	/**
	 * Creates a graphics context (as a {@link Graphics2D} object) for the
	 * splash screen overlay image, which allows you to draw over the splash
	 * screen. Note that you do not draw on the main image but on the image that
	 * is displayed over the main image using alpha blending. Also note that
	 * drawing on the overlay image does not necessarily update the contents of
	 * splash screen window. You should call {@code update()} on the
	 * <code>SplashScreen</code> when you want the splash screen to be updated
	 * immediately.
	 * <p>
	 ...
	 *
	 * @throws IllegalStateException
	 *             if the overlay image does not exist; for example, if
	 *             {@code createGraphics} has never been called, or if the
	...
	public void update() throws IllegalStateException {
		...
		if (!(image.getSampleModel() instanceof SinglePixelPackedSampleModel)) {
			throw new AssertionError("Overlay image has invalid sample model == "
					+ image.getSampleModel().getClass().getName());
		}
	 ...
	 * hidden using {@link #close()}, it is also hidden automatically when the
	 * first AWT/Swing window is made visible.
	 * <p>
	...
	/**
	 * The instance reference for the singleton. (<code>null</code> if no
	 * instance exists yet.)
	 *
	 * @see #getSplashScreen
	 * @see #close
