...
/**
 * The <code>List</code> component presents the user with a scrolling list of
 * text items. The list can be set up so that the user can choose either one
 ...
 * <hr>
 * <p>
 * where <code>cnt</code> is a container, produces the following scrolling list:
 * <p>
 * <img src="doc-files/List-1.gif" alt=
 ...
 * already selected deselects it. In the preceding example, only one item from
 * the scrolling list can be selected at a time, since the second argument when
 * creating the new scrolling list is <code>false</code>. If the List does not
 * allow multiple selections, selecting an item causes any other selected item
 * to be deselected.
 * <p>
 * Note that the list in the example shown was created with four visible rows.
 * Once the list has been created, the number of visible rows cannot be changed.
 * A default <code>List</code> is created with four rows, so that
 * <code>lst = new List()</code> is equivalent to
 * <code>list = new List(4, false)</code>.
 * <p>
 * Beginning with Java&nbsp;1.1, the Abstract Window Toolkit sends the
 * <code>List</code> object all mouse, keyboard, and focus events that occur
 * over it. (The old AWT event model is being maintained only for backwards
 * compatibility, and its use is discouraged.)
 * <p>
 * When an item is selected or deselected by the user, AWT sends an instance of
 * <code>ItemEvent</code> to the list. When the user double-clicks on an item in
 * a scrolling list, AWT sends an instance of <code>ActionEvent</code> to the
 * list following the item event. AWT also generates an action event when the
 * user presses the return key while an item in the list is selected.
 * <p>
 * If an application wants to perform some action based on an item in this list
 * being selected or activated by the user, it should implement
 * <code>ItemListener</code> or <code>ActionListener</code> as appropriate and
 * register the new listener to receive events from this list.
 * <p>
 * For multiple-selection scrolling lists, it is considered a better user
 * interface to use an external gesture (such as clicking on a button) to
 * trigger the action.
 * 
 * @author Sami Shaio
...
public class List extends Component implements ItemSelectable, Accessible {
	 ...
	 * <code>List</code> Component. It is specified only once, and that is when
	 * the list component is actually created. It will never change.
	 *
	 * @serial
	 * @see #getRows()
	 */
	...
	/**
	 * <code>multipleMode</code> is a variable that will be set to
	 * <code>true</code> if a list component is to be set to multiple selection
	 * mode, that is where the user can select more than one item in a list at
	 * one time. <code>multipleMode</code> will be set to false if the list
	 * component is set to single selection, that is where the user can only
	 * select one item on the list at any one time.
	 *
	 * @serial
	 * @see #isMultipleMode()
	...
	/**
	 * <code>selected</code> is an array that will contain the indices of items
	...
	/**
	 * This variable contains the value that will be used when trying to make a
	 * particular list item visible.
	 *
	 * @serial
	 * @see #makeVisible(int)
	 */
	...
	/**
	 * Creates a new scrolling list. By default, there are four visible lines
	 * and multiple selections are not allowed. Note that this is a convenience
	 * method for <code>List(0, false)</code>. Also note that the number of
	 ...
	 * visible lines. By default, multiple selections are not allowed. Note that
	 * this is a convenience method for <code>List(rows, false)</code>. Also
	 * note that the number of visible rows in the list cannot be changed after
	 * it has been created.
	 * 
	 * @param rows
	...
	/**
	 * The default number of visible rows is 4. A list with zero rows is
	 * unusable and unsightly.
	 */
	final static int DEFAULT_VISIBLE_ROWS = 4;

	/**
	 * Creates a new scrolling list initialized to display the specified number
	 * of rows. Note that if zero rows are specified, then the list will be
	 * created with a default of four rows. Also note that the number of visible
	 * rows in the list cannot be changed after it has been created. If the
	 * value of <code>multipleMode</code> is <code>true</code>, then the user
	 * can select multiple items from the list. If it is <code>false</code>,
	...
	/**
	 * Construct a name for this component. Called by <code>getName</code> when
	 ...
	 * indicated by the index. The index is zero-based. If the value of the
	 * index is less than zero, or if the value of the index is greater than or
	 * equal to the number of items in the list, then the item is added to the
	 * end of the list.
	 * 
	 * @param item
	...
	/**
	 * Replaces the item at the specified index in the scrolling list with the
	 * new string.
	 * 
	 * @param newValue
	...
	/**
	 * Removes the first occurrence of an item from the list. If the specified
	 * item is selected, and is the only selected item in the list, the list is
	...
	/**
	 * Removes the item at the specified position from this scrolling list. If
	 * the item with the specified position is selected, and is the only
	 ...
	 * @since JDK1.1
	 * @exception ArrayIndexOutOfBoundsException
	 *                if the <code>position</code> is less than 0 or greater
	 *                than <code>getItemCount()-1</code>
	 */
	...
	/**
	 * @deprecated replaced by <code>remove(String)</code> and
	 ...
	 *
	 * @return the index of the selected item; if no item is selected, or if
	 *         multiple items are selected, <code>-1</code> is returned.
	 * @see #select
	 * @see #deselect
	 * @see #isIndexSelected
	 */
	 ...
	 *
	 * @return an array of the selected indexes on this scrolling list; if no
	 *         item is selected, a zero-length array is returned.
	 * @see #select
	 * @see #deselect
	 * @see #isIndexSelected
	 */
	 ...
	 *
	 * @return the selected item on the list; if no item is selected, or if
	 *         multiple items are selected, <code>null</code> is returned.
	 * @see #select
	 * @see #deselect
	 * @see #isIndexSelected
	 */
	 ...
	 *
	 * @return an array of the selected items on this scrolling list; if no item
	 *         is selected, a zero-length array is returned.
	 * @see #select
	 * @see #deselect
	 * @see #isIndexSelected
	 */
	 ...
	 * 
	 * @return an array of <code>Object</code>s representing the selected items
	 *         on this scrolling list; if no item is selected, a zero-length
	 *         array is returned.
	 ...
	 * <i>not</i> trigger an <code>ItemEvent</code>. The only way to trigger an
	...
	/**
	 * Gets the number of visible lines in this list. Note that once the
	 * <code>List</code> has been created, this number will never change.
	 * 
	 ...
	 * selections. When the selection mode is changed from multiple-selection to
	 * single-selection, the selected items change as follows: If a selected
	 * item has the location cursor, only that item will remain selected. If no
	...
	/**
	 * @deprecated As of JDK version 1.1, replaced by
	 *             <code>setMultipleMode(boolean)</code>.
	 */
	@Deprecated
	 ...
	 * list. Action events occur when a user double-clicks on a list item or
	 * types Enter when the list has the keyboard focus.
	 * <p>
	 ...
	 * double-clicks on a list item. If listener <code>l</code> is
	 ...
	 * you can query a <code>List</code> <code>l</code> for its item listeners
	 ...
	 * <code>ItemEvent</code>, it invokes the <code>processItemEvent</code>
	 * method. Else, if the event is an instance of <code>ActionEvent</code>, it
	 * invokes <code>processActionEvent</code>. If the event is not an item
	 * event or an action event, it invokes <code>processEvent</code> on the
	 * superclass.
	 * <p>
	 ...
	 * registered <code>ItemListener</code> objects.
	 * <p>
	 * This method is not called unless item events are enabled for this
	 * component. Item events are enabled when one of the following occurs:
	 * <ul>
	 * <li>An <code>ItemListener</code> object is registered via
	 ...
	 * to any registered <code>ActionListener</code> objects.
	 * <p>
	 * This method is not called unless action events are enabled for this
	 * component. Action events are enabled when one of the following occurs:
	 * <ul>
	 * <li>An <code>ActionListener</code> object is registered via
	...
	/**
	 * Returns the parameter string representing the state of this scrolling
	 * list. This string is useful for debugging.
	 * 
	 ...
	 *             method is expected to be retained only as a package private
	 *             method.
	 */
	@Deprecated
	...
	/*
	 * Serialization support. Since the value of the selected field isn't
	 * necessarily up to date, we sync it up with the peer before serializing.
	 */

	/**
	 * The <code>List</code> component's Serialized Data Version.
	 *
	 * @serial
	 */
	 ...
	 * serializable <code>ItemListeners</code> and <code>ActionListeners</code>
	 * as optional data. The non-serializable listeners are detected and no
	 ...
	 * <code>null</code> adds a listener to receive both item events and action
	 * events (as specified by the key stored in the stream) fired by the
	 ...
	 * <code>List</code>. For lists, the <code>AccessibleContext</code> takes
	 * the form of an <code>AccessibleAWTList</code>. A new
	 * <code>AccessibleAWTList</code> instance is created, if necessary.
	 *
	...
	/**
	 * This class implements accessibility support for the <code>List</code>
	 * class. It provides an implementation of the Java Accessibility API
	 * appropriate to list user-interface elements.
	 * 
	 * @since 1.3
	 */
	protected class AccessibleAWTList extends AccessibleAWTComponent
		...
		/**
		 * Returns the Accessible child contained at the local coordinate Point,
		 * if one exists.
		 *
		...
		/**
		 * This class implements accessibility support for List children. It
		 * provides an implementation of the Java Accessibility API appropriate
		 * to list children user-interface elements.
		 * 
		 * @since 1.3
		 */
		protected class AccessibleAWTListChild extends AccessibleAWTComponent
			...
			//
			/**
			 * Gets the AccessibleContext for this object. In the implementation
			 * of the Java Accessibility API for this class, return this object,
			 * which acts as its own AccessibleContext.
			 *
			 * @return this object
			 */
			public AccessibleContext getAccessibleContext() {
			 ...
			 * object is composed of a set of unique AccessibleState's. A change
			 ...
			 * implementation and/or the native system do not support changing
			...
			/**
			 * Determine if the object is enabled. Objects that are enabled will
			 * also have the AccessibleState.ENABLED state set in their
			 * AccessibleStateSet.
			 *
			 ...
			 * object intends to be visible; however, it may not be showing on
			 * the screen because one of the objects that this object is
			 * contained by is currently not visible. To determine if an object
			 ...
			 * object ancestors. Note: this will return true even if the object
			 * is obscured by another (for example, it to object is underneath a
			 ...
			 *
			 * @return location of object on screen; null if this object is not
			 *         on the screen
			 * @see #getBounds
			 * @see #getLocation
			 */
			 ...
			 *         the objects's bounds in the coordinate space of the
			 *         screen; null if this object or its parent are not on the
			 *         screen
			 * @see #getBounds
			 ...
			 * can accept focus will also have the
