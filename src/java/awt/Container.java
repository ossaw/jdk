...
public class Container extends Component {
	...
	/**
	 * Event router for lightweight components. If this container is native,
	 * this dispatcher takes care of forwarding and retargeting the events to
	 * lightweight components contained (if any).
	 */
	 ...
	 * policies, then the current KeyboardFocusManager's default policy is used.
	 * If the value is non-null, this policy will be inherited by all
	 * focus-cycle-root children that have no keyboard-traversal policy of their
	 * own (as will, recursively, their focus-cycle-root children).
	 * <p>
	...
	/**
	 * A constant which toggles one of the controllable behaviors of
	 * <code>getMouseEventTarget</code>. It is used to specify whether the
	 * method can return the Container on which it is originally called in case
	 * if none of its children are the current mouse event targets.
	 *
	 ...
	 * A constant which toggles one of the controllable behaviors of
	 * <code>getMouseEventTarget</code>. It is used to specify whether the
	 * method should search only lightweight components.
	 *
	 * @see #getMouseEventTarget(int, int, boolean)
	 */
	...
	/*
	 * Number of HW or LW components in this container (including all descendant
	 * containers).
	 */
	private transient int numOfHWComponents = 0;
	private transient int numOfLWComponents = 0;

	private static final PlatformLogger mixingLog = PlatformLogger.getLogger(
			"java.awt.mixing.Container");

	/**
	 * @serialField ncomponents
	 *              int The number of components in this container. This
	 *              value can be null.
	 * @serialField component
	 *              Component[] The components in this container.
	 * @serialField layoutMgr
	 *              LayoutManager Layout manager for this container.
	 * @serialField dispatcher
	 *              LightweightDispatcher Event router for lightweight
	 *              components. If this container is native, this dispatcher
	 *              takes care of forwarding and retargeting the events to
	 *              lightweight components contained (if any).
	 * @serialField maxSize
	 *              Dimension Maximum size of this Container.
	 * @serialField focusCycleRoot
	 *              boolean Indicates whether this Component is the root of
	 *              a focus traversal cycle. Once focus enters a traversal
	 *              cycle, typically it cannot leave it via focus traversal
	 *              unless one of the up- or down-cycle keys is pressed.
	 *              Normal traversal is limited to this Container, and all
	 *              of this Container's descendants that are not descendants
	 *              of inferior focus cycle roots.
	 * @serialField containerSerializedDataVersion
	 *              int Container Serial Data Version.
	 * @serialField focusTraversalPolicyProvider
	 *              boolean Stores the value of focusTraversalPolicyProvider
	 *              property.
	 */
	private static final ObjectStreamField[] serialPersistentFields = {
			...
			new ObjectStreamField("containerSerializedDataVersion",
					Integer.TYPE), new ObjectStreamField(
							"focusTraversalPolicyProvider", Boolean.TYPE), };
	...
	/**
	 * Constructs a new Container. Containers can be extended directly, but are
	 * lightweight in this case and must be contained by a parent somewhere
	 * higher up in the component tree that is native. (such as Frame for
	 * example).
	 */
	public Container() {}
	...
	/**
	 * Checks that the component comp can be added to this container Checks :
	 * index in bounds of container's size, comp is not one of this container's
	 * parents, and comp is not a window. Comp and container must be on the same
	 * GraphicsDevice. if comp is container, all sub-components must be on same
	 * GraphicsDevice.
	 *
	 * @since 1.5
	 */
	private void checkAdding(Component comp, int index) {
		...
		if (thisTopLevel != compTopLevel) {
			throw new IllegalArgumentException(
					"component and container should be in the same top-level window");
		}
		if (thisGC != null) {
	...
	/**
	 * Removes component comp from this container without making unneccessary
	 * changes and generating unneccessary events. This function intended to
	 * perform optimized remove, for example, if newParent and current parent
	 * are the same it just changes index without calling removeNotify. Note:
	 * Should be called while holding treeLock Returns whether removeNotify was
	 * invoked
	 * 
	 * @since: 1.5
	 */
	private boolean removeDelicately(Component comp, Container newParent,
			int newIndex) {
		checkTreeLock();
		...
		if (newParent != this) {
			...
			adjustListeningChildren(AWTEvent.HIERARCHY_EVENT_MASK, -comp
					.numListening(AWTEvent.HIERARCHY_EVENT_MASK));
			adjustListeningChildren(AWTEvent.HIERARCHY_BOUNDS_EVENT_MASK, -comp
					.numListening(AWTEvent.HIERARCHY_BOUNDS_EVENT_MASK));
			...
			// add it by the newIndex without newIndex decrement if even we
		...
		if (comp.parent == null) { // was actually removed
			if (containerListener != null || (eventMask
					& AWTEvent.CONTAINER_EVENT_MASK) != 0 || Toolkit
							.enabledOnToolkit(AWTEvent.CONTAINER_EVENT_MASK)) {
			...
			comp.createHierarchyEvents(HierarchyEvent.HIERARCHY_CHANGED, comp,
					this, HierarchyEvent.PARENT_CHANGED, Toolkit
							.enabledOnToolkit(AWTEvent.HIERARCHY_EVENT_MASK));
	 ...
	 * Verifies that container is enable and showing, and if it is focus cycle
	 * root its FTP allows component to be focus owner
	 * 
	 * @since 1.5
	 */
	boolean canContainFocusOwner(Component focusOwnerCandidate) {
		...
		if (isFocusCycleRoot()) {
			...
			if (policy instanceof DefaultFocusTraversalPolicy) {
				if (!((DefaultFocusTraversalPolicy) policy).accept(
						focusOwnerCandidate)) {
					return false;
				}
			}
	...
	/**
	 * Checks whether or not this container has heavyweight children. Note:
	 * Should be called while holding tree lock
	 * 
	 * @return true if there is at least one heavyweight children in a
	...
	/**
	 * Checks whether or not this container has lightweight children. Note:
	 * Should be called while holding tree lock
	 * 
	 * @return true if there is at least one lightweight children in a
	 *         container, false otherwise
	 * @since 1.7
	 */
	...
	/**
	 * Returns closest heavyweight component to this container. If this
	 * container is heavyweight returns this.
	 * 
	 * @since 1.5
	 */
	...
	/**
	 * Detects whether or not remove from current parent and adding to new
	 * parent requires call of removeNotify on the component. Since removeNotify
	 * destroys native window this might (not) be required. For example, if new
	 * container and old containers are the same we don't need to destroy native
	 * window.
	 * 
	 * @since: 1.5
	 */
	private static boolean isRemoveNotifyNeeded(Component comp,
			Container oldContainer, Container newContainer) {
		if (oldContainer == null) { // Component didn't have parent - no
									// removeNotify
			return false;
		}
		...
		// If component is lightweight non-Container or lightweight Container
		// with all but heavyweight
		// children there is no need to call remove notify
		if (comp.isLightweight()) {
			boolean isContainer = comp instanceof Container;

			if (!isContainer || (isContainer && !((Container) comp)
					.hasHeavyweightDescendants())) {
				return false;
			}
		}

		// If this point is reached, then the comp is either a HW or a LW
		// container with HW descendants.
		...
		if (newNativeContainer != oldNativeContainer) {
			// Native containers change - check whether or not current platform
			// supports
			// changing of widget hierarchy on native level without recreation.
			// The current implementation forbids reparenting of LW containers
			// with HW descendants
			// into another native container w/o destroying the peers. Actually
			// such an operation
			// is quite rare. If we ever need to save the peers, we'll have to
			// slightly change the
			// addDelicately() method in order to handle such LW containers
			// recursively, reparenting
	 ...
	 * container. The z-order determines the order that components are painted;
	 ...
	 * higher z-order.
	 * <p>
	 * If the component is a child of some other container, it is removed from
	 * that container before being added to this container. The important
	 * difference between this method and
	 * <code>java.awt.Container.add(Component, int)</code> is that this method
	 * doesn't call <code>removeNotify</code> on the component while removing it
	 * from its previous container unless necessary and when allowed by the
	 * underlying native windowing system. This way, if the component has the
	 * keyboard focus, it maintains the focus when moved to the new position.
	 * <p>
	 * This property is guaranteed to apply only to lightweight non-
	 * <code>Container</code> components.
	 * <p>
	 * This method changes layout-related information, and therefore,
	 * invalidates the component hierarchy.
	 * <p>
	 * <b>Note</b>: Not all platforms support changing the z-order of
	 * heavyweight components from one container into another without the call
	 * to <code>removeNotify</code>. There is no way to detect whether a
	 * platform supports this, so developers shouldn't make any assumptions.
	 *
	 * @param comp
	 *              the component to be moved
	 * @param index
	 *              the position in the container's list to insert the
	 *              component,
	 *              where <code>getComponentCount()</code> appends to the end
	 * @exception NullPointerException
	 *                                     if <code>comp</code> is
	 *                                     <code>null</code>
	 * @exception IllegalArgumentException
	 *                                     if <code>comp</code> is one of the
	 *                                     container's parents
	 * @exception IllegalArgumentException
	 *                                     if <code>index</code> is not in the
	 *                                     range
	 *                                     <code>[0, getComponentCount()]</code>
	 *                                     for moving between
	 *                                     containers, or not in the range
	 *                                     <code>[0, getComponentCount()-1]</code>
	 *                                     for moving inside
	 *                                     a container
	 * @exception IllegalArgumentException
	 *                                     if adding a container to itself
	 * @exception IllegalArgumentException
	 *                                     if adding a <code>Window</code> to a
	 *                                     container
	...
	public void setComponentZOrder(Component comp, int index) {
		synchronized (getTreeLock()) {
			...
			boolean peerRecreated = (curParent != null) ? curParent
					.removeDelicately(comp, this, index) : false;
	...
	/**
	 * Traverses the tree of components and reparents children heavyweight
	 * component to new heavyweight parent.
	 * 
	 * @since 1.5
	 */
	private void reparentTraverse(ContainerPeer parentPeer, Container child) {
		...
		for (int i = 0; i < child.getComponentCount(); i++) {
			...
			if (comp.isLightweight()) {
				...
				// If it is container it might contain heavyweight children we
				// need to reparent
				if (comp instanceof Container) {
	...
	/**
	 * Reparents child component peer to this container peer. Container must be
	 * heavyweight.
	 * 
	 * @since 1.5
	 */
	private void reparentChild(Component comp) {
		...
		if (comp.isLightweight()) {
			// If component is lightweight container we need to reparent all its
			// explicit heavyweight children
			if (comp instanceof Container) {
				// Traverse component's tree till depth-first until encountering
	...
	/**
	 * Adds component to this container. Tries to minimize side effects of this
	 * adding - doesn't call remove notify if it is not required.
	 * 
	 * @since 1.5
	 */
	private void addDelicately(Component comp, Container curParent, int index) {
		...
		invalidateIfValid();
		if (peer != null) {
			if (comp.peer == null) { // Remove notify was called or it didn't
										// have peer - create new one
				comp.addNotify();
			} else { // Both container and child have peers, it means child peer
						// should be reparented.
						// In both cases we need to reparent native widgets.
				Container newNativeContainer = getHeavyweightContainer();
				Container oldNativeContainer = curParent
						.getHeavyweightContainer();
				if (oldNativeContainer != newNativeContainer) {
					// Native container changed - need to reparent native
					// widgets
				...
				if (!comp.isLightweight() && isLightweight()) {
					// If component is heavyweight and one of the containers is
					// lightweight
		...
		if (curParent != this) {
			...
			if (containerListener != null || (eventMask
					& AWTEvent.CONTAINER_EVENT_MASK) != 0 || Toolkit
							.enabledOnToolkit(AWTEvent.CONTAINER_EVENT_MASK)) {
				ContainerEvent e = new ContainerEvent(this,
						ContainerEvent.COMPONENT_ADDED, comp);
				dispatchEvent(e);
			}
			comp.createHierarchyEvents(HierarchyEvent.HIERARCHY_CHANGED, comp,
					this, HierarchyEvent.PARENT_CHANGED, Toolkit
			...
			// If component is focus owner or parent container of focus owner
			// check that after reparenting
			// focus owner moved out if new container prohibit this kind of
			// focus owner.
			if (comp.isFocusOwner() && !comp.canBeFocusOwnerRecursively()) {
				...
				if (focusOwner != null && isParentOf(focusOwner) && !focusOwner
			...
			comp.createHierarchyEvents(HierarchyEvent.HIERARCHY_CHANGED, comp,
					this, HierarchyEvent.HIERARCHY_CHANGED, Toolkit
							.enabledOnToolkit(AWTEvent.HIERARCHY_EVENT_MASK));
		}
	 ...
	 * higher a component is in the z-order hierarchy, the lower its index. The
	 * component with the lowest z-order index is painted last, above all other
	 * child components.
	 *
	 * @param comp
	 *             the component being queried
	 * @return the z-order index of the component; otherwise returns -1 if the
	 *         component is <code>null</code> or doesn't belong to the container
	...
	public int getComponentZOrder(Component comp) {
		...
		synchronized (getTreeLock()) {
			// Quick check - container should be immediate parent of the
			// component
			if (comp.parent != this) {
				return -1;
			}
	 ...
	 * constraints object. This is a convenience method for {@link #addImpl}.
	 * <p>
	 * This method changes layout-related information, and therefore,
	 * invalidates the component hierarchy. If the container has already been
	 * displayed, the hierarchy must be validated thereafter in order to display
	 ...
	 *                    insert the
	 *                    component; <code>-1</code> means insert at the end
	 *                    component
	 * @exception NullPointerException
	 *                                     if {@code comp} is {@code null}
	 * @exception IllegalArgumentException
	 ...
	 * This method also notifies the layout manager to add the component to this
	 * container's layout using the specified constraints object via the
	 * <code>addLayoutComponent</code> method.
	 * <p>
	 * The constraints are defined by the particular layout manager being used.
	 * For example, the <code>BorderLayout</code> class defines five
	 ...
	 * <code>GridBagConstraints</code> object. Failure to pass the correct type
	 * of constraints object results in an <code>IllegalArgumentException</code>
	 * .
	 * <p>
	 ...
	 * {@link LayoutManager#addLayoutComponent(String,Component)} is invoked on
	 * it.
	 * <p>
	 ...
	 * container.
	 * <p>
	 * This is the method to override if a program needs to track every add
	 * request to a container as all other add methods defer to this one. An
	 * overriding method should usually include a call to the superclass's
	 * version of the method:
	 *
	 * <blockquote>
	 * <code>super.addImpl(comp, constraints, index)</code> </blockquote>
	 * <p>
	 * This method changes layout-related information, and therefore,
	 * invalidates the component hierarchy. If the container has already been
	 * displayed, the hierarchy must be validated thereafter in order to display
	 ...
	 *                    end
	 * @exception IllegalArgumentException
	 *                                     if {@code index} is invalid; if
	 *                                     {@code comp} is a child of
	 *                                     this container, the valid range is
	 *                                     {@code [-1, getComponentCount()-1]};
	 *                                     if component is not a
	 *                                     child of this container, the valid
	 *                                     range is
	 *                                     {@code [-1, getComponentCount()]}
	 *
	 * @exception IllegalArgumentException
	 *                                     if {@code comp} is an ancestor of
	 *                                     this container
	 * @exception IllegalArgumentException
	...
	protected void addImpl(Component comp, Object constraints, int index) {
		synchronized (getTreeLock()) {
			/*
			 * Check for correct arguments: index in bounds, comp cannot be one
			 * of this container's parents, and comp cannot be a window. comp
			 * and container must be on the same GraphicsDevice. if comp is
			 * container, all sub-components must be on same GraphicsDevice.
			 */
			...
			if (layoutMgr != null) {
				if (layoutMgr instanceof LayoutManager2) {
					((LayoutManager2) layoutMgr).addLayoutComponent(comp,
							constraints);
	...
	/**
	 * Checks that all Components that this Container contains are on the same
	 * GraphicsDevice as this Container. If not, throws an
	 ...
	 * container. This method also notifies the layout manager to remove the
	 ...
	 *                                        if {@code index} is not in range
	 *                                        {@code [0, getComponentCount()-1]}
	 * @see #add
	 * @see #invalidate
	 * @see #validate
		...
		synchronized (getTreeLock()) {
			...
			while (!component.isEmpty()) {
				...
				comp.createHierarchyEvents(HierarchyEvent.HIERARCHY_CHANGED,
						comp, this, HierarchyEvent.PARENT_CHANGED, Toolkit
								.enabledOnToolkit(
	...
	final int createHierarchyEvents(int id, Component changed,
		...
		return listeners + super.createHierarchyEvents(id, changed,
				changedParent, changeFlags, enabledOnToolkit);
	}
	 ...
	 * to preserve backward compatibility this new optimized behavior is enabled
	 * only when the {@code java.awt.smartInvalidate} system property value is
	 * set to {@code true}.
	 * <p>
	 ...
	 * calling the {@code validate()} method on the top-level container (such as
	 * a {@code Frame} object) should be used to restore the validity of the
	 * component hierarchy.
	 * <p>
	...
	/**
	 * Invalidates the parent of the container unless the container is a
	 * validate root.
	 */
	@Override
	 ...
	 * of the {@code LayoutManager2} interface, then the
	 * {@link LayoutManager2#invalidateLayout(Container)} method is invoked on
	 ...
	 * <p>
	 * Validating a container means laying out its subcomponents. Layout-related
	 * changes, such as setting the bounds of a component, or adding a component
	 * to the container, invalidate the container automatically. Note that the
	 * ancestors of the container may be invalidated also (see
	 * {@link Component#invalidate} for details.) Therefore, to restore the
	 * validity of the hierarchy, the {@code
	 * validate()} method should be invoked on the top-most invalid container of
	 * the hierarchy.
	 * <p>
	...
	/**
	 * Indicates whether valid containers should also traverse their children
	 * and call the validateTree() method on them.
	 *
	 ...
	 * subtrees marked as needing it (those marked as invalid). Synchronization
	 * should be provided by the method that calls this one:
	 * <code>validate</code>.
	 *
	 * @see #doLayout
	 * @see #validate
	...
	/**
	 * Recursively descends the container tree and invalidates all contained
	 * components.
	 */
	void invalidateTree() {
		synchronized (getTreeLock()) {
	...
	public void setFont(Font f) {
		...
		if (newfont != oldfont && (oldfont == null || !oldfont.equals(
				newfont))) {
			invalidateTree();
		}
	}
	 ...
	 * then {@link LayoutManager#preferredLayoutSize(Container)} is used to
	 ...
	 * {@code preferredLayoutSize} on the {@code LayoutManager} every time this
	 * method is invoked, rather the {@code LayoutManager} will only be queried
	 * after the {@code Container} becomes invalid.
	 *
	 * @return an instance of <code>Dimension</code> that represents the
	...
	public Dimension preferredSize() {
		...
		if (dim == null || !(isPreferredSizeSet() || isValid())) {
			synchronized (getTreeLock()) {
				prefSize = (layoutMgr != null) ? layoutMgr.preferredLayoutSize(
						this) : super.preferredSize();
				dim = prefSize;
			}
		}
		if (dim != null) {
	...
	/**
	 * Returns the minimum size of this container. If the minimum size has not
	 * been set explicitly by {@link Component#setMinimumSize(Dimension)} and
	 * this {@code Container} has a {@code non-null} {@link LayoutManager}, then
	 * {@link LayoutManager#minimumLayoutSize(Container)} is used to calculate
	 * the minimum size.
	 *
	 * <p>
	 * Note: some implementations may cache the value returned from the
	 * {@code LayoutManager}. Implementations that cache need not invoke
	 * {@code minimumLayoutSize} on the {@code LayoutManager} every time this
	 * method is invoked, rather the {@code LayoutManager} will only be queried
	 * after the {@code Container} becomes invalid.
	 *
	...
	public Dimension minimumSize() {
		...
		if (dim == null || !(isMinimumSizeSet() || isValid())) {
			synchronized (getTreeLock()) {
				minSize = (layoutMgr != null) ? layoutMgr.minimumLayoutSize(
						this) : super.minimumSize();
				dim = minSize;
			}
		}
		if (dim != null) {
	...
	/**
	 * Returns the maximum size of this container. If the maximum size has not
	 * been set explicitly by {@link Component#setMaximumSize(Dimension)} and
	 * the {@link LayoutManager} installed on this {@code Container} is an
	 * instance of {@link LayoutManager2}, then
	 * {@link LayoutManager2#maximumLayoutSize(Container)} is used to calculate
	 * the maximum size.
	 *
	 * <p>
	 * Note: some implementations may cache the value returned from the
	 * {@code LayoutManager2}. Implementations that cache need not invoke
	 * {@code maximumLayoutSize} on the {@code LayoutManager2} every time this
	 * method is invoked, rather the {@code LayoutManager2} will only be queried
	 * after the {@code Container} becomes invalid.
	 *
	...
	public void paint(Graphics g) {
		if (isShowing()) {
			...
			GraphicsCallback.PaintCallback.getInstance().runComponents(
					getComponentsSync(), g, GraphicsCallback.LIGHTWEIGHTS);
		}
	}
	...
	public void print(Graphics g) {
		if (isShowing()) {
			...
			GraphicsCallback.PrintCallback.getInstance().runComponents(
					getComponentsSync(), g, GraphicsCallback.LIGHTWEIGHTS);
		}
	}
	...
	public void paintComponents(Graphics g) {
		if (isShowing()) {
			GraphicsCallback.PaintAllCallback.getInstance().runComponents(
					getComponentsSync(), g, GraphicsCallback.TWO_PASSES);
		}
	}

	/**
	 * Simulates the peer callbacks into java.awt for printing of lightweight
	 * Containers.
	 * 
	 * @param g
	...
	public void printComponents(Graphics g) {
		if (isShowing()) {
			GraphicsCallback.PrintAllCallback.getInstance().runComponents(
					getComponentsSync(), g, GraphicsCallback.TWO_PASSES);
		}
	}
	 ...
	 * <code><em>Foo</em>Listener</code>s upon this <code>Container</code>.
	 ...
	 * you can query a <code>Container</code> <code>c</code> for its container
	 * listeners with the following code:
	 *
	 * <pre>
	 * ContainerListener[] cls = (ContainerListener[]) (c.getListeners(
	 ...
	 *         Listener</code>s on this container, or an empty array if no such
	 ...
	 * <code>processContainerEvent</code> method, else it invokes its
	...
	/**
	 * Processes container events occurring on this container by dispatching
	 * them to any registered ContainerListener objects. NOTE: This method will
	 * not be called unless container events are enabled for this component;
	 ...
	 * COMPONENT_RESIZED and COMPONENT_MOVED events. We have to do this here
	 * instead of in processComponentEvent because ComponentEvents may not be
		...
		synchronized (getTreeLock()) {
			switch (e.getID()) {
				case ComponentEvent.COMPONENT_RESIZED:
					createChildHierarchyEvents(HierarchyEvent.ANCESTOR_RESIZED,
							0, Toolkit.enabledOnToolkit(
									AWTEvent.HIERARCHY_BOUNDS_EVENT_MASK));
					break;
				case ComponentEvent.COMPONENT_MOVED:
					createChildHierarchyEvents(HierarchyEvent.ANCESTOR_MOVED, 0,
							Toolkit.enabledOnToolkit(
									AWTEvent.HIERARCHY_BOUNDS_EVENT_MASK));
					break;
				default:
					break;
	...
	/*
	 * Dispatches an event to this component, without trying to forward it to
	 * any subcomponents
	 * @param e the event
	 */
	...
	/**
	 * Fetchs the top-most (deepest) lightweight component that is interested in
	 * receiving mouse events.
	 */
	Component getMouseEventTarget(int x, int y, boolean includeSelf) {
		return getMouseEventTarget(x, y, includeSelf,
				MouseEventTargetFilter.FILTER, !SEARCH_HEAVYWEIGHTS);
	}
	...
	Component getDropTargetEventTarget(int x, int y, boolean includeSelf) {
		return getMouseEventTarget(x, y, includeSelf,
				DropTargetEventTargetFilter.FILTER, SEARCH_HEAVYWEIGHTS);
	}

	/**
	 * A private version of getMouseEventTarget which has two additional
	 * controllable behaviors. This method searches for the top-most descendant
	 * of this container that contains the given coordinates and is accepted by
	 * the given filter. The search will be constrained to lightweight
	 * descendants if the last argument is <code>false</code>.
	 *
	 * @param filter
	 *                           EventTargetFilter instance to determine whether
	 *                           the given
	 *                           component is a valid target for this event.
	 * @param searchHeavyweights
	 *                           if <code>false</code>, the method will bypass
	 *                           heavyweight
	 *                           components during the search.
	 */
	private Component getMouseEventTarget(int x, int y, boolean includeSelf,
			EventTargetFilter filter, boolean searchHeavyweights) {
		Component comp = null;
		if (searchHeavyweights) {
			comp = getMouseEventTargetImpl(x, y, includeSelf, filter,
					SEARCH_HEAVYWEIGHTS, searchHeavyweights);
		}
	 ...
	 * controllable behaviors. This method searches for the top-most descendant
	 * of this container that contains the given coordinates and is accepted by
	 * the given filter. The search will be constrained to descendants of only
	 * lightweight children or only heavyweight children of this container
	 * depending on searchHeavyweightChildren. The search will be constrained to
	 * only lightweight descendants of the searched children of this container
	 * if searchHeavyweightDescendants is <code>false</code>.
	 *
	 * @param filter
	 *                                     EventTargetFilter instance to
	 *                                     determine whether the selected
	 *                                     component is a valid target for this
	 *                                     event.
	 * @param searchHeavyweightChildren
	 *                                     if <code>true</code>, the method will
	 *                                     bypass immediate
	 *                                     lightweight children during the
	 *                                     search. If <code>false</code>,
	 *                                     the methods will bypass immediate
	 *                                     heavyweight children during
	 *                                     the search.
	 * @param searchHeavyweightDescendants
	 *                                     if <code>false</code>, the method
	 *                                     will bypass heavyweight
	 *                                     descendants which are not immediate
	 *                                     children during the
	 *                                     search. If <code>true</code>, the
	 *                                     method will traverse both
	...
	private Component getMouseEventTargetImpl(int x, int y, boolean includeSelf,
			EventTargetFilter filter, boolean searchHeavyweightChildren,
			boolean searchHeavyweightDescendants) {
		synchronized (getTreeLock()) {

			for (int i = 0; i < component.size(); i++) {
				Component comp = component.get(i);
				if (comp != null && comp.visible && ((!searchHeavyweightChildren
					...
					if (comp instanceof Container) {
						...
						Component deeper = child.getMouseEventTarget(x
								- child.x, y - child.y, includeSelf, filter,
	 ...
	 * windowed parent to enable some kind of events on their behalf. This is
	 * needed for events that are normally only dispatched to windows to be
	 * accepted so that they can be forwarded downward to the lightweight
	 * component that has enabled them.
	 */
	 ...
	 * component is returned in the case where there is overlap in the
	 * components. This is determined by finding the component closest to the
	 * index 0 that claims to contain the given point via Component.contains(),
	 * except that Components which have native peers take precedence over those
	 ...
	 * @param p
	 *          the point.
	 * @return returns the component that contains the point, or
	 *         <code>null</code> if the component does not contain the point.
	 * @see Component#contains
	 * @since JDK1.1
	 */
	...
	/**
	 * Returns the position of the mouse pointer in this <code>Container</code>
	 * 's coordinate space if the <code>Container</code> is under the mouse
	 * pointer, otherwise returns <code>null</code>. This method is similar to
	 * {@link Component#getMousePosition()} with the exception that it can take
	 * the <code>Container</code>'s children into account. If
	 * <code>allowChildren</code> is <code>false</code>, this method will return
	 * a non-null value only if the mouse pointer is above the
	 * <code>Container</code> directly, not above the part obscured by children.
	 * If <code>allowChildren</code> is <code>true</code>, this method returns a
	 * non-null value if the mouse pointer is above <code>Container</code> or
	 ...
	 * Setting 'ignoreEnabled' to 'false' bypasses disabled Components during
	 * the search. This behavior is used by the lightweight cursor support in
	 * sun.awt.GlobalCursorManager.
	 *
	 * The addition of this feature is temporary, pending the adoption of new,
	 * public API which exports this feature.
	 */
	 ...
	 * native screen resource. Making a container undisplayable will cause all
	 * of its children to be made undisplayable. This method is called by the
	 * toolkit internally and should not be called directly by programs.
	 * 
	 * @see Component#isDisplayable
	 * @see #addNotify
	 */
	public void removeNotify() {
		synchronized (getTreeLock()) {
			...
			// If some of the children had focus before disposal then it still
			// has.
			// Auto-transfer focus to the next (or previous) component if
			// auto-transfer
			// is enabled.
	 ...
	 * The native container of the LW component has this field set to tell it
	 * that it should block Mouse events for all LW children except for the
	 * modal component.
	 * In the case of nested Modal components, we store the previous modal
	 * component in the new modal components value of modalComp;
	 */
	...
	private void startLWModal() {
		...
		Component predictedFocusOwner = (Component.isInstanceOf(this,
	...
	private void stopLWModal() {
		synchronized (getTreeLock()) {
			if (modalAppContext != null) {
				...
				// initially shown
				SunToolkit.postEvent(modalAppContext, new PeerEvent(this,
	...
	/**
	 * Prints a listing of this container to the specified output stream. The
	 * listing starts at the specified indentation.
	 * <p>
	 * The immediate children of the container are printed with an indentation
	 * of <code>indent+1</code>. The children of those children are printed at
	 * <code>indent+2</code> and so on.
	 *
	 * @param out
	 *               a print stream
	 * @param indent
	 ...
	 * </tr>
	 * <tr>
	 * <td>KeyboardFocusManager.DOWN_CYCLE_TRAVERSAL_KEYS
	 * <td>
	 ...
	 * for this Container. (See <code>setFocusTraversalKeys</code> for a full
	 * description of each key.)
	 * <p>
	 * If a Set of traversal keys has not been explicitly defined for this
	 * Container, then this Container's parent's Set is returned. If no Set has
	 * been explicitly defined for any of this Container's ancestors, then the
	 ...
	 * Container's focus traversal cycle. Each focus traversal cycle has only a
	 * single focus cycle root and each Container which is not a focus cycle
	 * root belongs to only a single focus traversal cycle. Containers which are
	 * focus cycle roots belong to two cycles: one rooted at the Container
	 ...
	 * Note: this function acquires treeLock Note: this function traverses
	 * children tree only in one Window.
	 * 
	 * @param comp
	 *             a component in test, must not be null
	 */
	...
	void clearMostRecentFocusOwnerOnHide() {
		...
		synchronized (getTreeLock()) {
			...
			if (window != null) {
				Component comp = KeyboardFocusManager.getMostRecentFocusOwner(
						window);
	 ...
	 * this Container. If this method returns <code>false</code>, this Container
	 * will inherit its focus traversal policy from an ancestor.
	 *
	 ...
	 * <a href="doc-files/FocusSpec.html#FocusTraversalPolicyProviders">focus
	 * traversal policy provider</a>.
	 *
	...
	/**
	 * Sets whether this container will be used to provide focus traversal
	 * policy. Container with this property as <code>true</code> will be used to
	 * acquire focus traversal policy instead of closest focus cycle root
	 * ancestor.
	 * 
	 * @param provider
	 *                 indicates whether this container will be used to provide
	 *                 focus
	 *                 traversal policy
	 * @see #setFocusTraversalPolicy
	 * @see #getFocusTraversalPolicy
	...
	public final void setFocusTraversalPolicyProvider(boolean provider) {
		...
		firePropertyChange("focusTraversalPolicyProvider", oldProvider,
				provider);
	}

	/**
	 * Returns whether this container provides focus traversal policy. If this
	 * property is set to <code>true</code> then when keyboard focus manager
	 * searches container hierarchy for focus traversal policy and encounters
	 * this container before any other container with this property as true or
	 * focus cycle roots then its focus traversal policy will be used instead of
	 * focus cycle root's policy.
	 * 
	 * @see #setFocusTraversalPolicy
	 * @see #getFocusTraversalPolicy
	 * @see #setFocusCycleRoot
	...
	public void transferFocusDownCycle() {
		if (isFocusCycleRoot()) {
			...
			Component toFocus = getFocusTraversalPolicy().getDefaultComponent(
					this);
			if (toFocus != null) {
	...
	/**
	 * Sets the <code>ComponentOrientation</code> property of this container and
	 ...
	 *
	 * @param o
	 *          the new component orientation of this container and the
	 ...
	 * <li>this Container's focus traversal policy ("focusTraversalPolicy")</li>
	 * <li>this Container's focus-cycle-root state ("focusCycleRoot")</li>
	 * </ul>
	 * Note that if this Container is inheriting a bound property, then no event
	 ...
	 * state("focusTraversalPolicyProvider")</li>
	 * <li>this Container's focus-traversal-policy-provider
	 * state("focusTraversalPolicyProvider")</li>
	 * </ul>
	 * Note that if this Container is inheriting a bound property, then no event
	 ...
	 * <li>Writes a list of serializable ContainerListener(s) as optional data.
	 * The non-serializable ContainerListner(s) are detected and no attempt is
	 * made to serialize them.</li>
	 * <li>Write this Container's FocusTraversalPolicy if and only if it is
	 * Serializable; otherwise, <code>null</code> is written.</li>
	 * </ul>
	 *
	 * @param s
	 *          the <code>ObjectOutputStream</code> to write
	 * @serialData <code>null</code> terminated sequence of 0 or more pairs; the
	 *             pair consists of a <code>String</code> and
	 ...
	 *             <code>ContainerListener</code> object; the
	 *             <code>Container</code>'s <code>FocusTraversalPolicy</code>,
	 *             or <code>null</code>
	 *
	 ...
	 * <li>Reads a list of serializable ContainerListener(s) as optional data.
	 * If the list is null, no Listeners are installed.</li>
	 * <li>Reads this Container's FocusTraversalPolicy, which may be null, as
	 * optional data.</li>
	 * </ul>
	 *
	 * @param s
	...
	private void readObject(ObjectInputStream s) throws ClassNotFoundException,
		...
		Component[] tmpComponent = (Component[]) f.get("component",
				EMPTY_ARRAY);
		...
		containerSerializedDataVersion = f.get("containerSerializedDataVersion",
				1);
		focusTraversalPolicyProvider = f.get("focusTraversalPolicyProvider",
				false);
	 ...
	 * accessibility. This class is not meant to be used directly by application
	 * developers, but is instead meant only to be subclassed by container
	 * developers.
	 * <p>
	...
	protected class AccessibleAWTContainer extends AccessibleAWTComponent {
		 ...
		 * the children of this object implement <code>Accessible</code>, then
		 * this method should return the number of children of this object.
		 *
		...
		/**
		 * Returns the <code>Accessible</code> child, if one exists, contained
		...
		/**
		 * Number of PropertyChangeListener objects registered. It's used to
		 * add/remove ContainerListener to track target Container's state.
		 */
		...
		/**
		 * Fire <code>PropertyChange</code> listener, if one is registered, when
		...
		public void removePropertyChangeListener(
				PropertyChangeListener listener) {
			if (--propertyListenersCount == 0) {
				Container.this.removeContainerListener(
	 ...
	 * coordinate <code>Point</code>, if one exists. Otherwise returns
	 * <code>null</code>.
	 *
	 * @param p
	 *          the point defining the top-left corner of the
	 *          <code>Accessible</code>, given in the coordinate space of the
	 *          object's parent
	 * @return the <code>Accessible</code> at the specified location, if it
				...
				if (!this.contains(p.x, p.y)) {
					...
					for (int i = 0; i < ncomponents; i++) {
						...
						if ((comp != null) && comp.isShowing()) {
							...
							if (comp.contains(p.x - location.x, p.y
									- location.y)) {
								ret = comp;
							}
						}
	 ...
	 * children of this object implement <code>Accessible</code>, then this
	...
	final void increaseComponentCount(Component c) {
		synchronized (getTreeLock()) {
			...
			for (Container cont = this; cont != null; cont = cont
					.getContainer()) {
	...
	final void recursiveSubtractAndApplyShape(Region shape) {
		recursiveSubtractAndApplyShape(shape, getTopmostComponentIndex(),
				getBottommostComponentIndex());
	}

	final void recursiveSubtractAndApplyShape(Region shape, int fromZorder) {
		recursiveSubtractAndApplyShape(shape, fromZorder,
				getBottommostComponentIndex());
	}
	...
	final void recursiveSubtractAndApplyShape(Region shape, int fromZorder,
		...
		if (mixingLog.isLoggable(PlatformLogger.Level.FINE)) {
			mixingLog.fine("this = " + this + "; shape=" + shape + "; fromZ="
					+ fromZorder + "; toZ=" + toZorder);
		}
		if (fromZorder == -1) {
			return;
		}
		...
		for (int index = fromZorder; index <= toZorder; index++) {
			...
			if (!comp.isLightweight()) {
				comp.subtractAndApplyShape(shape);
					...
					.hasHeavyweightDescendants() && comp.isShowing()) {
	...
	final void recursiveApplyCurrentShape() {
		recursiveApplyCurrentShape(getTopmostComponentIndex(),
				getBottommostComponentIndex());
	}
	...
	final void recursiveApplyCurrentShape(int fromZorder, int toZorder) {
		checkTreeLock();
		if (mixingLog.isLoggable(PlatformLogger.Level.FINE)) {
			mixingLog.fine("this = " + this + "; fromZ=" + fromZorder + "; toZ="
					+ toZorder);
		}
		if (fromZorder == -1) {
			return;
	...
	private void recursiveRelocateHeavyweightChildren(Point origin) {
		for (int index = 0; index < getComponentCount(); index++) {
			...
			if (comp.isLightweight()) {
				...
				ComponentPeer peer = comp.getPeer();
				if (peer != null) {
					peer.setBounds(origin.x + comp.getX(), origin.y + comp
							.getY(), comp.getWidth(), comp.getHeight(),
	...
	final boolean isRecursivelyVisibleUpToHeavyweightContainer() {
		...
		for (Container cont = this; cont != null && cont
				.isLightweight(); cont = cont.getContainer()) {
			if (!cont.isVisible()) {
				return false;
			}
		}
	...
	void mixOnReshaping() {
		synchronized (getTreeLock()) {
			...
			if (isLightweight() && hasHeavyweightDescendants()) {
				...
				for (Container cont = getContainer(); cont != null && cont
						.isLightweight(); cont = cont.getContainer()) {
	...
	void mixOnZOrderChanging(int oldZorder, int newZorder) {
		synchronized (getTreeLock()) {
			if (mixingLog.isLoggable(PlatformLogger.Level.FINE)) {
				mixingLog.fine("this = " + this + "; oldZ=" + oldZorder
						+ "; newZ=" + newZorder);
			}
...
class LightweightDispatcher implements java.io.Serializable, AWTEventListener {
	...
	/*
	 * Clean up any resources allocated when dispatcher was created; should be
	 * called from Container.removeNotify
	 */
	void dispose() {
	...
	boolean dispatchEvent(AWTEvent e) {
		...
		/*
		 * Fix for BugTraq Id 4389284. Dispatch SunDropTargetEvents regardless
		 * of eventMask value. Do not update cursor on dispatching
		 * SunDropTargetEvents.
		 */
	...
	private boolean isMouseGrab(MouseEvent e) {
		...
		if (e.getID() == MouseEvent.MOUSE_PRESSED || e
				.getID() == MouseEvent.MOUSE_RELEASED) {
		...
		if (met != null) {
			switch (id) {
				...
				case MouseEvent.MOUSE_CLICKED:
					// 4508327: MOUSE_CLICKED should never be dispatched to a
					// Component
					// other than that which received the MOUSE_PRESSED event. If
					// the
					// mouse is now over a different Component, don't dispatch the
					// event.
					// The previous fix for a similar problem was associated with
					// bug
					// 4155217.
					if (mouseOver == met) {
				...
				case MouseEvent.MOUSE_WHEEL:
					...
					// retarget the event to a parent that DOES have the events
					// enabled.
					if (eventLog.isLoggable(PlatformLogger.Level.FINEST)
							&& (mouseOver != null)) {
						eventLog.finest("retargeting mouse wheel to "
								+ mouseOver.getName() + ", " + mouseOver
										.getClass());
					}
					retargetMouseEvent(mouseOver, id, e);
					break;
	...
	private boolean processDropTargetEvent(SunDropTargetEvent e) {
		...
		/*
		 * Fix for BugTraq ID 4395290. It is possible that SunDropTargetEvent's
		 * Point is outside of the native container bounds. In this case we
	...
	private void trackDropTargetEnterExit(Component targetOver, MouseEvent e) {
		...
		Component tle = retargetMouseEnterExit(targetOver, e,
				targetLastEnteredDT.get(), isMouseDTInNativeContainer);
	...
	private void trackMouseEnterExit(Component targetOver, MouseEvent e) {
		...
		if (id != MouseEvent.MOUSE_EXITED && id != MouseEvent.MOUSE_DRAGGED
				&& id != LWD_MOUSE_DRAGGED_OVER && !isMouseInNativeContainer) {
		...
		Component tle = retargetMouseEnterExit(targetOver, e, targetLastEntered
				.get(), isMouseInNativeContainer);
		targetLastEntered = new WeakReference<>(tle);
	}

	private Component retargetMouseEnterExit(Component targetOver, MouseEvent e,
			Component lastEntered, boolean inNativeContainer) {
		int id = e.getID();
	...
	/*
	 * Listens to global mouse drag events so even drags originating from other
	 * heavyweight containers will generate enter/exit events in this container
	 */
	...
	private void stopListeningForOtherDrags() {
		...
		java.security.AccessController.doPrivileged(
				new java.security.PrivilegedAction<Object>() {
					public Object run() {
						nativeContainer.getToolkit().removeAWTEventListener(
								LightweightDispatcher.this);
						return null;
					}
				});
	...
	/*
	 * (Implementation of AWTEventListener) Listen for drag events posted in
	 * other hw components so we can track enter/exit regardless of where a drag
	 * originated
	 */
	public void eventDispatched(AWTEvent e) {
		boolean isForeignDrag = (e instanceof MouseEvent)
				&& !(e instanceof SunDropTargetEvent)
				&& (e.id == MouseEvent.MOUSE_DRAGGED) && (e
						.getSource() != nativeContainer);
		...
		synchronized (nativeContainer.getTreeLock()) {
			...
			//
			me = new MouseEvent(nativeContainer, LWD_MOUSE_DRAGGED_OVER,
					srcEvent.getWhen(), srcEvent.getModifiersEx() | srcEvent
							.getModifiers(), srcEvent.getX(), srcEvent.getY(),
					srcEvent.getXOnScreen(), srcEvent.getYOnScreen(), srcEvent
							.getClickCount(), srcEvent.isPopupTrigger(),
					srcEvent.getButton());
			...
			if (AppContext.getAppContext() != nativeContainer.appContext) {
				...
				me.translatePoint(ptSrcOrigin.x - ptDstOrigin.x, ptSrcOrigin.y
						- ptDstOrigin.y);
			}
		}
		...
		Component targetOver = nativeContainer.getMouseEventTarget(me.getX(), me
				.getY(), Container.INCLUDE_SELF);
	...
	void retargetMouseEvent(Component target, int id, MouseEvent e) {
		...
		for (component = target; component != null
				&& component != nativeContainer; component = component
						.getParent()) {
			x -= component.x;
			y -= component.y;
		}
		...
		if (component != null) {
			if (e instanceof SunDropTargetEvent) {
				retargeted = new SunDropTargetEvent(target, id, x, y,
						((SunDropTargetEvent) e).getDispatcher());
			} else if (id == MouseEvent.MOUSE_WHEEL) {
				retargeted = new MouseWheelEvent(target, id, e.getWhen(), e
						.getModifiersEx() | e.getModifiers(), x, y, e
				...
				retargeted = new MouseEvent(target, id, e.getWhen(), e
						.getModifiersEx() | e.getModifiers(), x, y, e
								.getXOnScreen(), e.getYOnScreen(), e
			...
			if (target == nativeContainer) {
				...
				if (nativeContainer.modalComp != null) {
					if (((Container) nativeContainer.modalComp).isAncestorOf(
							target)) {
	 ...
	 * components need a windowed container to host window-related events. This
	 * separate mask indicates events that have been requested by contained
	 * lightweight components without effecting the mask of the windowed
	 * component itself.
	 */
