...
public class Container extends Component {
	...
	private static final PlatformLogger eventLog = PlatformLogger
	...
	private static final PlatformLogger mixingLog = PlatformLogger
	 ...
	 *                if {@code index} is invalid (see {@link #addImpl} for
	 *                details)
	 ...
	 * @param index
	 *            the position in the container's list to insert the component,
	 *            where <code>getComponentCount()</code> appends to the end
	 * @exception NullPointerException
	 *                if <code>comp</code> is <code>null</code>
	 * @exception IllegalArgumentException
	 *                if <code>comp</code> is one of the container's parents
	 * @exception IllegalArgumentException
	 *                if <code>index</code> is not in the range
	 ...
	 * @exception IllegalArgumentException
	 *                if adding a <code>Window</code> to a container
		...
		if (curParent != this) {
			...
			if (comp.isFocusOwner() && !comp.canBeFocusOwnerRecursively()) {
				...
				Component focusOwner = KeyboardFocusManager.getCurrentKeyboardFocusManager()
						.getFocusOwner();
				if (focusOwner != null && isParentOf(focusOwner)
						&& !focusOwner.canBeFocusOwnerRecursively()) {
	 ...
	 * @exception IllegalArgumentException
	 *                if {@code index} is invalid; if {@code comp} is a child of
	 *                this container, the valid range is
	 *                {@code [-1, getComponentCount()-1]}; if component is not a
	 *                child of this container, the valid range is
	 *                {@code [-1, getComponentCount()]}
	 *
	 * @exception IllegalArgumentException
	 *                if {@code comp} is an ancestor of this container
	 * @exception IllegalArgumentException
	...
	public void removeAll() {
		synchronized (getTreeLock()) {
			...
			while (!component.isEmpty()) {
				...
				if (containerListener != null || (eventMask & AWTEvent.CONTAINER_EVENT_MASK) != 0
						|| Toolkit.enabledOnToolkit(AWTEvent.CONTAINER_EVENT_MASK)) {
					ContainerEvent e = new ContainerEvent(this, ContainerEvent.COMPONENT_REMOVED,
							comp);
					dispatchEvent(e);
				}
	...
	final int createHierarchyEvents(int id, Component changed, Container changedParent,
			long changeFlags, boolean enabledOnToolkit) {
		checkTreeLock();
	...
	final void createChildHierarchyEvents(int id, long changeFlags, boolean enabledOnToolkit) {
		...
		for (int count = listeners, i = 0; count > 0; i++) {
			count -= component.get(i).createHierarchyEvents(id, this, parent, changeFlags,
					enabledOnToolkit);
		}
	}
	...
	static {
		...
		isJavaAwtSmartInvalidate = AccessController
	...
	public Dimension preferredSize() {
		...
		if (dim == null || !(isPreferredSizeSet() || isValid())) {
			synchronized (getTreeLock()) {
				prefSize = (layoutMgr != null) ? layoutMgr.preferredLayoutSize(this)
						: super.preferredSize();
				dim = prefSize;
			}
		}
	...
	public Dimension minimumSize() {
		...
		if (dim == null || !(isMinimumSizeSet() || isValid())) {
			synchronized (getTreeLock()) {
				minSize = (layoutMgr != null) ? layoutMgr.minimumLayoutSize(this)
						: super.minimumSize();
				dim = minSize;
			}
		}
	...
	Component getMouseEventTarget(int x, int y, boolean includeSelf) {
		return getMouseEventTarget(x, y, includeSelf, MouseEventTargetFilter.FILTER,
				!SEARCH_HEAVYWEIGHTS);
	}
	 ...
	 *
	 * @param filter
	 *            EventTargetFilter instance to determine whether the given
	 *            component is a valid target for this event.
	 * @param searchHeavyweights
	 *            if <code>false</code>, the method will bypass heavyweight
	 ...
	 *
	 * @param filter
	 *            EventTargetFilter instance to determine whether the selected
	 *            component is a valid target for this event.
	 * @param searchHeavyweightChildren
	 *            if <code>true</code>, the method will bypass immediate
	 *            lightweight children during the search. If <code>false</code>,
	 *            the methods will bypass immediate heavyweight children during
	 *            the search.
	 * @param searchHeavyweightDescendants
	 *            if <code>false</code>, the method will bypass heavyweight
	 *            descendants which are not immediate children during the
	 *            search. If <code>true</code>, the method will traverse both
	 *            lightweight and heavyweight descendants during the search.
	 */
				...
				if (comp != null && comp.visible
					...
					if (comp instanceof Container) {
						...
						Component deeper = child.getMouseEventTarget(x - child.x, y - child.y,
								includeSelf, filter, searchHeavyweightDescendants);
						if (deeper != null) {
							return deeper;
						}
					} else {
	...
	static class MouseEventTargetFilter implements EventTargetFilter {
		...
		public boolean accept(final Component comp) {
			return (comp.eventMask & AWTEvent.MOUSE_MOTION_EVENT_MASK) != 0
					...
					|| comp.mouseListener != null || comp.mouseMotionListener != null
	...
	public Point getMousePosition(boolean allowChildren) throws HeadlessException {
		...
		PointerInfo pi = java.security.AccessController
				.doPrivileged(new java.security.PrivilegedAction<PointerInfo>() {
	...
	private void startLWModal() {
		...
		if (predictedFocusOwner != null) {
			KeyboardFocusManager.getCurrentKeyboardFocusManager().enqueueKeyEvents(time,
					predictedFocusOwner);
		}
		...
		if (EventQueue.isDispatchThread()) {
			...
			synchronized (getTreeLock()) {
				Toolkit.getEventQueue().postEvent(
						new PeerEvent(this, pumpEventsForHierarchy, PeerEvent.PRIORITY_EVENT));
		...
		if (predictedFocusOwner != null) {
			KeyboardFocusManager.getCurrentKeyboardFocusManager().dequeueKeyEvents(time,
					predictedFocusOwner);
		}
	}
		...
		synchronized (getTreeLock()) {
			if (modalAppContext != null) {
				...
				SunToolkit.postEvent(modalAppContext,
						new PeerEvent(this, new WakingRunnable(), PeerEvent.PRIORITY_EVENT));
			}
	 ...
	 *            indicates whether this Container is the root of a focus
	 *            traversal cycle
	 * @see #isFocusCycleRoot()
	 * @see #setFocusTraversalPolicy
	 * @see #getFocusTraversalPolicy
	 ...
	 * 
	 * @param provider
	 *            indicates whether this container will be used to provide focus
	 *            traversal policy
	 * @see #setFocusTraversalPolicy
	 * @see #getFocusTraversalPolicy
	...
	final void recursiveSubtractAndApplyShape(Region shape, int fromZorder, int toZorder) {
		...
		if (mixingLog.isLoggable(PlatformLogger.Level.FINE)) {
			mixingLog.fine("this = " + this + "; shape=" + shape + "; fromZ=" + fromZorder
					+ "; toZ=" + toZorder);
		}
		if (fromZorder == -1) {
			return;
	...
	private void recursiveRelocateHeavyweightChildren(Point origin) {
		for (int index = 0; index < getComponentCount(); index++) {
			...
			if (comp.isLightweight()) {
				...
				ComponentPeer peer = comp.getPeer();
				if (peer != null) {
					peer.setBounds(origin.x + comp.getX(), origin.y + comp.getY(), comp.getWidth(),
							comp.getHeight(), ComponentPeer.SET_LOCATION);
				}
			}
		}
	}
	...
	final boolean isRecursivelyVisibleUpToHeavyweightContainer() {
		...
		for (Container cont = this; cont != null
				&& cont.isLightweight(); cont = cont.getContainer()) {
			if (!cont.isVisible()) {
				return false;
			}
		}
	...
	void mixOnReshaping() {
		synchronized (getTreeLock()) {
			...
			if (isLightweight() && hasHeavyweightDescendants()) {
				...
				for (Container cont = getContainer(); cont != null
						&& cont.isLightweight(); cont = cont.getContainer()) {
...
class LightweightDispatcher implements java.io.Serializable, AWTEventListener {
		...
		if (met != null) {
			...
			case MouseEvent.MOUSE_WHEEL:
				...
				if (eventLog.isLoggable(PlatformLogger.Level.FINEST) && (mouseOver != null)) {
					eventLog.finest("retargeting mouse wheel to " + mouseOver.getName() + ", "
							+ mouseOver.getClass());
				}
				retargetMouseEvent(mouseOver, id, e);
				break;
	...
	public void eventDispatched(AWTEvent e) {
		...
		synchronized (nativeContainer.getTreeLock()) {
			...
			me = new MouseEvent(nativeContainer, LWD_MOUSE_DRAGGED_OVER, srcEvent.getWhen(),
					srcEvent.getModifiersEx() | srcEvent.getModifiers(), srcEvent.getX(),
					srcEvent.getY(), srcEvent.getXOnScreen(), srcEvent.getYOnScreen(),
					srcEvent.getClickCount(), srcEvent.isPopupTrigger(), srcEvent.getButton());
			...
			if (AppContext.getAppContext() != nativeContainer.appContext) {
				...
				Runnable r = new Runnable() {
					public void run() {
						...
						Component targetOver = nativeContainer.getMouseEventTarget(
								mouseEvent.getX(), mouseEvent.getY(), Container.INCLUDE_SELF);
	...
	void retargetMouseEvent(Component target, int id, MouseEvent e) {
		...
		for (component = target; component != null
				&& component != nativeContainer; component = component.getParent()) {
			x -= component.x;
			y -= component.y;
		}
		...
		if (component != null) {
			if (e instanceof SunDropTargetEvent) {
				...
				retargeted = new MouseWheelEvent(target, id, e.getWhen(),
						e.getModifiersEx() | e.getModifiers(), x, y, e.getXOnScreen(),
				...
				retargeted = new MouseEvent(target, id, e.getWhen(),
						e.getModifiersEx() | e.getModifiers(), x, y, e.getXOnScreen(),
