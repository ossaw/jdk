...
/**
 * A {@code Window} object is a top-level window with no borders and no menubar.
 * The default layout for a window is {@code BorderLayout}.
 * <p>
 * A window must have either a frame, dialog, or another window defined as its
 * owner when it's constructed.
 * <p>
 * In a multi-screen environment, you can create a {@code Window} on a different
 * screen device by constructing the {@code Window} with
 ...
 * figure.
 * <p>
 * <img src="doc-files/MultiScreen.gif" alt=
 ...
 * <p>
 * In such an environment, when calling {@code setLocation}, you must pass a
 * virtual coordinate to this method. Similarly, calling
 * {@code getLocationOnScreen} on a {@code Window} returns virtual device
 * coordinates. Call the {@code getBounds} method of a
 * {@code GraphicsConfiguration} to find its origin in the virtual coordinate
 * system.
 * <p>
 * The following code sets the location of a {@code Window} at (10, 10) relative
 * to the origin of the physical screen of the corresponding
 * {@code GraphicsConfiguration}. If the bounds of the
 * {@code GraphicsConfiguration} is not taken into account, the {@code Window}
 * location would be set at (10, 10) relative to the virtual-coordinate system
 * and would appear on the primary physical screen, which might be different
 * from the physical screen of the specified {@code GraphicsConfiguration}.
 *
 * <pre>
 ...
 * window management system. Calls to {@code setLocation}, {@code setSize}, and
 * {@code setBounds} are requests (not directives) which are forwarded to the
 * window management system. Every effort will be made to honor such requests.
 * However, in some cases the window management system may ignore such requests,
 * or modify the requested geometry in order to place and size the
 * {@code Window} in a way that more closely matches the desktop settings.
 * <p>
 * Due to the asynchronous nature of native event handling, the results returned
 * by {@code getBounds}, {@code getLocation}, {@code getLocationOnScreen}, and
 * {@code getSize} might not reflect the actual geometry of the Window on screen
 * until the last request has been processed. During the processing of
 * subsequent requests these values might change accordingly while the window
...
public class Window extends Container implements Accessible {
	...
	/**
	 * Contains all the windows that have a peer object associated, i. e.
	 * between addNotify() and removeNotify() calls. The list of all Window
	 * instances can be obtained from AppContext object.
	 *
	 * @since 1.6
	 */
	...
	/**
	 * Contains the modal dialog that blocks this window, or null if the window
	 * is unblocked.
	 *
	 * @since 1.6
	 */
	...
	/**
	 * Indicates whether this window should receive focus on subsequently being
	 * shown (with a call to {@code setVisible(true)}), or being moved to the
	 * front (with a call to {@code toFront()}).
	 *
	 * @serial
	...
	/**
	 * These fields are initialized in the native peer code or via AWTAccessor's
	 * WindowAccessor.
	 */
	...
	/**
	 * These fields represent the desired location for the security warning if
	 * this window is untrusted. See com.sun.awt.SecurityWarning for more
	 * details.
	 */
	...
	/**
	 * Initialize JNI field and method IDs for fields that may be accessed from
	 * C.
	 */
	 ...
	 * {@code AWTPermission("showWindowWithoutWarningBanner")} to determine
	 ...
	 *                if {@code gc} is not from a screen device
	 * @exception HeadlessException
	 *                when {@code GraphicsEnvironment.isHeadless()} returns
	 *                {@code true}
	 *
	 ...
	 * {@code AWTPermission("showWindowWithoutWarningBanner")}. If that check
	 ...
	 * {@code Frame} as its owner. The window will not be focusable unless its
	 ...
	 *            window has no owner
	 * @exception IllegalArgumentException
	 *                if the {@code owner}'s {@code GraphicsConfiguration} is
	...
	/**
	 * Constructs a new, initially invisible window with the specified
	 * {@code Window} as its owner. This window will not be focusable unless its
	 ...
	 * {@code Window} and a {@code GraphicsConfiguration} of a screen device.
	 * The Window will not be focusable unless its nearest owning {@code Frame}
	 * or {@code Dialog} is showing on the screen.
	 * <p>
	 * If there is a security manager set, it is invoked to check
	 * {@code AWTPermission("showWindowWithoutWarningBanner")}. If that check
	 * fails with a {@code SecurityException} then a warning banner is created.
	 *
	 * @param owner
	 *            the window to act as owner or {@code null} if this window has
	 *            no owner
	 * @param gc
	 *            the {@code GraphicsConfiguration} of the target screen device;
	 *            if {@code gc} is {@code null}, the system default
	...
	/**
	 * Returns the sequence of images to be displayed as the icon for this
	 * window.
	 * <p>
	 * This method returns a copy of the internally stored list, so all
	 * operations on the returned object will not affect the window's behavior.
	 *
	 * @return the copy of icon images' list for this window, or empty list if
	 *         this window doesn't have icon images.
	 * @see #setIconImages
	 ...
	 * Subsequent calls to {@code getIconImages} will always return a copy of
	 * the {@code icons} list.
	 * <p>
	 * Depending on the platform capabilities one or several images of different
	 * dimensions will be used as the window's icon.
	 * <p>
	 * The {@code icons} list is scanned for the images of most appropriate
	 * dimensions from the beginning. If the list contains several images of the
	 * same size, the first will be used.
	 * <p>
	 * Ownerless windows with no icon specified use platfrom-default icon. The
	 * icon of an owned window may be inherited from the owner unless explicitly
	 * overridden. Setting the icon to {@code null} or empty list restores the
	 * default behavior.
	 * <p>
	 * Note : Native windowing systems may use different images of differing
	 * dimensions to represent a window, depending on the context (e.g. window
	 * decoration, window list, taskbar, etc.). They could also use just a
	 ...
	 * screen resource. This method is called internally by the toolkit and
	 ...
	 * its subcomponents. The resulting width and height of the window are
	 * automatically enlarged if either of dimensions is less than the minimum
	 * size as specified by the previous call to the {@code setMinimumSize}
	 * method.
	 * <p>
	 * If the window and/or its owner are not displayable yet, both of them are
	 * made displayable before calculating the preferred size. The Window is
	...
	/**
	 * Sets the minimum size of this window to a constant value. Subsequent
	 * calls to {@code getMinimumSize} will always return this value. If current
	 * window's size is less than {@code minimumSize} the size of the window is
	 * automatically enlarged to honor the minimum size.
	 * <p>
	 * If the {@code setSize} or {@code setBounds} methods are called afterwards
	 * with a width or height less than that was specified by the
	 * {@code setMinimumSize} method the window is automatically enlarged to
	 * meet the {@code minimumSize} value. The {@code minimumSize} value also
	 * affects the behaviour of the {@code pack} method.
	 * <p>
	 ...
	 * below the {@code minimumSize} value. This behaviour is
	 * platform-dependent.
	 *
	 * @param minimumSize
	...
	final void closeSplashScreen() {
		...
		if (beforeFirstWindowShown.getAndSet(false)) {
			// We don't use SplashScreen.getSplashScreen() to avoid
			// instantiating
	 ...
	 * {@code b}.
	 * <p>
	 * If the method shows the window then the window is also made focused under
	 * the following conditions:
	 * <ul>
	 * <li>The {@code Window} meets the requirements outlined in the
	 * {@link #isFocusableWindow} method.
	 * <li>The {@code Window}'s {@code autoRequestFocus} property is of the
	 * {@code true} value.
	 ...
	 * {@code autoRequestFocus} property). The property is not taken into
	 * account if the window is a modal dialog, which blocks the currently
	 * focused window.
	 * <p>
	 * Developers must never assume that the window is the focused or active
	 ...
	 * prior to being made visible. If the Window is already visible, this will
	...
	/**
	 * Hide this Window, its subcomponents, and all of its owned children. The
	 * Window and its subcomponents can be made visible again with a call to
	 * {@code show}.
	 * 
	 * @see #show
	 * @see #dispose
	...
	/*
	 * Fix for 4872170. If dispose() is called on parent then its children have
	 * to be disposed as well as reported in javadoc. So we need to implement
	 * this functionality even if a child overrides dispose() in a wrong way
	 * without calling super.dispose().
	 */
	void disposeImpl() {
		dispose();
		if (getPeer() != null) {
	...
	/*
	 * Should only be called while holding the tree lock. It's overridden here
	 * because parent == owner in Window, and we shouldn't adjust counter on
	 * owner
	 */
	 ...
	 * Window until this Window receives a WINDOW_GAINED_FOCUS or
	 * WINDOW_ACTIVATED event. On platforms where the top-most window is the
	 * focused window, this method will <b>probably</b> focus this Window (if it
	 * is not already focused) under the following conditions:
	 * <ul>
	 * <li>The window meets the requirements outlined in the
	 * {@link #isFocusableWindow} method.
	 * <li>The window's property {@code autoRequestFocus} is of the {@code true}
	 * value.
	...
	/**
	 * Gets the warning string that is displayed with this window. If this
	 * window is insecure, the warning string is displayed somewhere in the
	 * visible area of the window. A window is insecure if there is a security
	 * manager and the security manager denies
	 * {@code AWTPermission("showWindowWithoutWarningBanner")}.
	 * <p>
	 * If the window is secure, then {@code getWarningString} returns
	 * {@code null}. If the window is insecure, this method checks for the
	 * system property {@code awt.appletWarning} and returns the string value of
	 * that property.
	 * 
		...
		if (sm != null) {
			try {
				...
				warningString = AccessController.doPrivileged(
						new GetPropertyAction("awt.appletWarning", "Java Applet Window"));
			}
		}
	}

	/**
	 * Gets the {@code Locale} object that is associated with this window, if
	 * the locale has been set. If no locale has been set, then the default
	 * locale is returned.
	 * 
	...
	/**
	 * Gets the input context for this window. A window always has an input
	 * context, which is shared by subcomponents unless they create and set
	 * their own.
	 * 
	 ...
	 * 
	 * @param cursor
	 *            One of the constants defined by the {@code Cursor} class. If
	...
	/*
	 * Returns a list of all displayable Windows, i. e. all the Windows which
	 * peer is not null.
	 *
	 * @see #addNotify
	 * 
	 ...
	 * created by this application. If called from an applet, the array includes
	 * only the {@code Window}s accessible by that applet.
	 * <p>
	 * <b>Warning:</b> this method may return system created windows, such as a
	 * print dialog. Applications should not assume the existence of these
	 * dialogs, nor should an application assume anything about these dialogs
	 * such as component positions, {@code LayoutManager}s or serialization.
	 *
	 * @see Frame#getFrames
	...
	/**
	 * Returns an array of all {@code Window}s created by this application that
	 * have no owner. They include {@code Frame}s and ownerless {@code Dialog}s
	 * and {@code Window}s. If called from an applet, the array includes only
	 * the {@code Window}s accessible by that applet.
	 * <p>
	 * <b>Warning:</b> this method may return system created windows, such as a
	 * print dialog. Applications should not assume the existence of these
	 * dialogs, nor should an application assume anything about these dialogs
	 ...
	 * {@link java.awt.Dialog.ModalExclusionType Dialog.ModalExclusionType} for
	 ...
	 *
	 * @param exclusionType
	 *            the modal exclusion type for this window; a {@code null} value
	 *            is equivalent to {@link Dialog.ModalExclusionType#NO_EXCLUDE
	 *            NO_EXCLUDE}
	 * @throws SecurityException
	 *             if the calling thread does not have permission to set the
	 *             modal exclusion property to the window with the given
	 *             {@code exclusionType}
	 * @see java.awt.Dialog.ModalExclusionType
	...
	public void setModalExclusionType(Dialog.ModalExclusionType exclusionType) {
		...
		/*
		 * if (isModalBlocked()) { modalBlocker.unblockWindow(this); }
		 * Dialog.checkShouldBeBlocked(this); updateChildrenBlocking();
		 */
	}
	...
	/**
	 * Adds the specified window state listener to receive window events from
	 * this window. If {@code l} is {@code null}, no exception is thrown and no
	 * action is performed.
	 * <p>
	 * Refer to <a href="doc-files/AWTThreadIssues.html#ListenersThreads" >AWT
	 * Threading Issues</a> for details on AWT's threading model.
	...
	/**
	 * Adds the specified window focus listener to receive window events from
	 * this window. If l is null, no exception is thrown and no action is
	 * performed.
	 * <p>
	 * Refer to <a href="doc-files/AWTThreadIssues.html#ListenersThreads" >AWT
	 * Threading Issues</a> for details on AWT's threading model.
	 ...
	 * window events from this window. If l is null, no exception is thrown and
	 ...
	 * window events from this window. If {@code l} is {@code null}, no
	 ...
	 *                if {@code listenerType} doesn't specify a class or
	 *                interface that implements {@code java.util.EventListener}
	...
	/**
	 * Processes events on this window. If the event is an {@code WindowEvent},
	 * it invokes the {@code processWindowEvent} method, else it invokes its
	 * superclass's {@code processEvent}.
	 * <p>
	 * Note that if the event parameter is {@code null} the behavior is
	...
	/**
	 * Processes window events occurring on this window by dispatching them to
	 * any registered WindowListener objects. NOTE: This method will not be
	 * called unless window events are enabled for this component; this happens
	 * when one of the following occurs:
	 * <ul>
	 * <li>A WindowListener object is registered via {@code addWindowListener}
	...
	/**
	 * Processes window focus event occurring on this window by dispatching them
	 * to any registered WindowFocusListener objects. NOTE: this method will not
	 * be called unless window focus events are enabled for this window. This
	...
	/**
	 * Processes window state event occurring on this window by dispatching them
	 * to any registered {@code WindowStateListener} objects. NOTE: this method
	 * will not be called unless window state events are enabled for this
	 * window. This happens when one of the following occurs:
	 * <ul>
	...
	/**
	 * Implements a debugging hook -- checks to see if the user has typed
	 * <i>control-shift-F1</i>. If so, the list of child windows is dumped to
	 * {@code System.out}.
	 * 
	 * @param e
	 *            the keyboard event
	...
	/**
	 * Sets whether this window should always be above other windows. If there
	 * are multiple always-on-top windows, their relative order is unspecified
	 * and platform dependent.
	 * <p>
	 * If some other window is already always-on-top then the relative order
	 * between these windows is unspecified (depends on platform). No window can
	 * be brought to be over the always-on-top window except maybe another
	 ...
	 * window, the window is immediately brought forward, "sticking" it in the
	 ...
	 * platform-dependent change in the z-order of top-level windows, but other
	 ...
	 * use {@link Toolkit#isAlwaysOnTopSupported()} and
	 * {@link Window#isAlwaysOnTopSupported()}. If always-on-top mode isn't
	 * supported for this window or this window's toolkit does not support
	 ...
	 *
	 * @param alwaysOnTop
	 *            true if the window should always be above other windows
	 * @throws SecurityException
	 *             if the calling thread does not have permission to set the
	 ...
	 * platforms may not support always-on-top windows, some may support only
	 * some kinds of top-level windows; for example, a platform may not support
	...
	public Component getFocusOwner() {
		return (isFocused()) ? KeyboardFocusManager.getCurrentKeyboardFocusManager().getFocusOwner()
				: null;
	}
	 ...
	 * method returns the same Component as {@code getFocusOwner()}. If this
	 * Window is not focused, then the child Component that most recently
	...
	public Component getMostRecentFocusOwner() {
		if (isFocused()) {
			...
			if (mostRecent != null) {
				...
				return (isFocusableWindow()) ? getFocusTraversalPolicy().getInitialComponent(this)
						: null;
			}
		}
	}
	 ...
	 * Window. Otherwise, the active Window is the first Frame or Dialog that is
	 ...
	 * <p>
	 * If the traversal key has not been explicitly set for this Window, then
	 * this Window's parent's traversal key is returned. If the traversal key
	 * has not been explicitly set for any of this Window's ancestors, then the
	...
	/**
	 * Always returns {@code true} because all Windows must be roots of a focus
	 * traversal cycle.
	 *
	 * @return {@code true}
	 ...
	 * must be set to {@code true}. For a Window which is not a Frame or Dialog
	 * to be focusable, its focusable Window state must be set to {@code true},
	 * its nearest owning Frame or Dialog must be showing on the screen, and it
	 * must contain at least one Component in its focus traversal cycle. If any
	 * of these conditions is not met, then neither this Window nor any of its
	 * subcomponents can become the focus owner.
	 *
	...
	/**
	 * Returns whether this Window can become the focused Window if it meets the
	 * other requirements outlined in {@code isFocusableWindow}. If this method
	 * returns {@code false}, then {@code isFocusableWindow} will return
	 * {@code false} as well. If this method returns {@code true}, then
	 * {@code isFocusableWindow} may return {@code true} or {@code false}
	 * depending upon the other requirements which must be met in order for a
	 ...
	 * other requirements outlined in {@code isFocusableWindow}. If this
	 * Window's focusable Window state is set to {@code false}, then
	 * {@code isFocusableWindow} will return {@code false}. If this Window's
	 * focusable Window state is set to {@code true}, then
	 * {@code isFocusableWindow} may return {@code true} or {@code false}
	 * depending upon the other requirements which must be met in order for a
	 * Window to be focusable.
	 * <p>
	 * Setting a Window's focusability state to {@code false} is the standard
	 ...
	 * consistent behavior across platforms, set the {@code Window}'s focusable
	...
	public void setFocusableWindowState(boolean focusableWindowState) {
		...
		if (oldFocusableWindowState && !focusableWindowState && isFocused()) {
			for (Window owner = getOwner(); owner != null; owner = owner.getOwner()) {
				...
				if (toFocus != null
						&& toFocus.requestFocus(false, CausedFocusEvent.Cause.ACTIVATION)) {
					return;
				}
			}
	...
	/**
	 * Sets whether this window should receive focus on subsequently being shown
	 * (with a call to {@link #setVisible setVisible(true)}), or being moved to
	 * the front (with a call to {@link #toFront}).
	 * <p>
	 ...
	 * {@link #setVisible setVisible(true)} is called on already visible
	 ...
	 *            whether this window should be focused on subsequently being
	...
	/**
	 * Returns whether this window should receive focus on subsequently being
	 * shown (with a call to {@link #setVisible setVisible(true)}), or being
	 * moved to the front (with a call to {@link #toFront}).
	 * <p>
	 ...
	 * <li>this Window's focus traversal policy ("focusTraversalPolicy")</li>
	 * <li>this Window's focusable Window state ("focusableWindowState")</li>
	 * <li>this Window's always-on-top state("alwaysOnTop")</li>
	 * </ul>
	 * Note that if this Window is inheriting a bound property, then no event
	 ...
	 * serializable {@code WindowListener}s and {@code WindowFocusListener}s as
	 * optional data. Writes a list of child windows as optional data. Writes a
	 ...
	 *             following: {@code windowListenerK} indicating a
	...
	/**
	 * Reads the {@code ObjectInputStream} and an optional list of listeners to
	 * receive various events fired by the component; also reads a list of
	 * (possibly {@code null}) child windows. Unrecognized keys or values will
	 * be ignored.
	 *
	 * @param s
	 ...
	 *
	 * @return an AccessibleAWTWindow that serves as the AccessibleContext of
	 *         this Window
	 * @since 1.3
	 */
	...
	/**
	 * This class implements accessibility support for the {@code Window} class.
	 * It provides an implementation of the Java Accessibility API appropriate
	 * to window user-interface elements.
	 * 
	 * @since 1.3
	 */
	 ...
	 * supplied. It will always create a {@code BufferStrategy} with that number
	 * of buffers. A page-flipping strategy is attempted first, then a blitting
	 * strategy using accelerated buffers. Finally, an unaccelerated blitting
	 * strategy is used.
	 * <p>
	 ...
	 *                met; this may happen, for example, if there is not enough
	 *                accelerated memory currently available, or if page
	 *                flipping is specified but not possible.
	 * @exception IllegalArgumentException
	 *                if numBuffers is less than 1, or if caps is {@code null}
	 * @see #getBufferStrategy
	 * @since 1.4
	 */
	...
	/**
	 * Returns the {@code BufferStrategy} used by this component. This method
	 * will return null if a {@code BufferStrategy} has not yet been created or
	 * has been disposed.
	 *
	...
	Component setTemporaryLostComponent(Component component) {
		...
		// Check that "component" is an acceptable focus owner and don't store
		// it otherwise
		// - or later we will have problems with opposite while handling
		// WINDOW_GAINED_FOCUS
	...
	/**
	 * Checks whether this window can contain focus owner. Verifies that it is
	 * focusable and as container it can container focus owner.
	 * 
	 * @since 1.5
	 */
	 ...
	 * {@code getLocation}) the next time the Window is made visible. This
	 * behavior resembles a native window shown without programmatically setting
	 * its location. Most windowing systems cascade windows if their locations
	 * are not explicitly set. The actual location is determined once the window
	 * is shown on the screen.
	 * <p>
	 * This behavior can also be enabled by setting the System Property
	 * "java.awt.Window.locationByPlatform" to "true", though calls to this
	 * method take precedence.
	 * <p>
	 * Calls to {@code setVisible}, {@code setLocation} and {@code setBounds}
	 * after calling {@code setLocationByPlatform} clear this property of the
	 * Window.
	 * <p>
	 ...
	 * </pre>
	 * 
	 * The window will be shown at platform's default location and {@code flag}
	 * will be {@code false}.
	 * <p>
	 ...
	 *            {@code true} if this Window should appear at the default
	 *            location, {@code false} if at the current location
	 * @throws IllegalComponentStateException
	 *             if the window is showing on screen and locationByPlatform is
	 *             {@code true}.
	 * @see #setLocation
	 * @see #isShowing
	 * @see #setVisible
	 ...
	 * for the native windowing system the next time this Window is made
	 * visible. This method always returns {@code false} if the Window is
	 ...
	 * {@inheritDoc}
	 * <p>
	 * The {@code width} or {@code height} values are automatically enlarged if
	 * either is less than the minimum size as specified by previous call to
	 * {@code setMinimumSize}.
	 * <p>
	 * The method changes the geometry-related data. Therefore, the native
	 ...
	 * {@inheritDoc}
	 * <p>
	 * The {@code r.width} or {@code r.height} values will be automatically
	 * enlarged if either is less than the minimum size as specified by previous
	 * call to {@code setMinimumSize}.
	 * <p>
	 * The method changes the geometry-related data. Therefore, the native
	 ...
	 * 
	 * @return {@code true} if the component and all of its ancestors until a
	 *         toplevel window are visible, {@code false} otherwise
	 */
	boolean isRecursivelyVisible() {
		// 5079694 fix: for a toplevel to be displayed, its parent doesn't have
		// to be visible.
	 ...
	 * <li>The window must be undecorated (see {@link Frame#setUndecorated} and
	 * {@link Dialog#setUndecorated})
	 * <li>The window must not be in full-screen mode (see
	 * {@link GraphicsDevice#setFullScreenWindow(Window)})
	 * </ul>
	 * <p>
	 * If the requested opacity value is less than {@code 1.0f}, and any of the
	 * above conditions are not met, the window opacity will not change, and the
	 * {@code IllegalComponentStateException} will be thrown.
	 * <p>
	 * The translucency levels of individual pixels may also be effected by the
	 * alpha component of their color (see {@link Window#setBackground(Color)})
	 ...
	 *
	 * @throws IllegalArgumentException
	 *             if the opacity is out of the range [0..1]
	 * @throws IllegalComponentStateException
	 *             if the window is decorated and the opacity is less than
	 *             {@code 1.0f}
	 * @throws IllegalComponentStateException
	 *             if the window is in full screen mode, and the opacity is less
	 *             than {@code 1.0f}
	 ...
	 * belong to the given {@link Shape} remain visible and clickable. If the
	 * shape argument is {@code null}, this method restores the default shape,
	 * making the window rectangular on most platforms.
	 * <p>
	 * The following conditions must be met to set a non-null shape:
	 * <ul>
	 * <li>The {@link GraphicsDevice.WindowTranslucency#PERPIXEL_TRANSPARENT
	 * PERPIXEL_TRANSPARENT} translucency must be supported by the underlying
	 * system
	 * <li>The window must be undecorated (see {@link Frame#setUndecorated} and
	 * {@link Dialog#setUndecorated})
	 * <li>The window must not be in full-screen mode (see
	 ...
	 * conditions are not met, the shape of this window will not change, and
	 * either the {@code UnsupportedOperationException} or {@code
	 * IllegalComponentStateException} will be thrown.
	 * <p>
	 * The translucency levels of individual pixels may also be effected by the
	 * alpha component of their color (see {@link Window#setBackground(Color)})
	 * and the opacity value (see {@link #setOpacity(float)}). See
	 ...
	 * PERPIXEL_TRANSLUCENT} translucency, the alpha component of the given
	 * background color may effect the mode of operation for this window: it
	 * indicates whether this window must be opaque (alpha equals {@code 1.0f})
	 * or per-pixel translucent (alpha is less than {@code 1.0f}). If the given
	 * background color is {@code null}, the window is considered completely
	 * opaque.
	 * <p>
	 ...
	 * IllegalComponentStateException} will be thrown.
	 * <p>
	 * When the window is per-pixel translucent, the drawing sub-system respects
	 * the alpha value of each individual pixel. If a pixel gets painted with
	 * the alpha color component equal to zero, it becomes visually transparent.
	 * If the alpha of the pixel is equal to 1.0f, the pixel is fully opaque.
	 * Interim values of the alpha color component make the pixel
	 * semi-transparent. In this mode, the background of the window gets painted
	 * with the alpha value of the given background color. If the alpha value of
	 * the argument of this method is equal to {@code 0}, the background is not
	 * painted at all.
	 * <p>
	 ...
	 *
	 * @throws IllegalComponentStateException
	 *             if the alpha value of the given background color is less than
	 *             {@code 1.0f} and the window is decorated
	 * @throws IllegalComponentStateException
	 *             if the alpha value of the given background color is less than
	 *             {@code 1.0f} and the window is in full-screen mode
	 * @throws UnsupportedOperationException
	 *             if the alpha value of the given background color is less than
	 *             {@code 1.0f} and
	 *             {@link GraphicsDevice.WindowTranslucency#PERPIXEL_TRANSLUCENT
	 ...
	 * <p>
	 * The method returns {@code false} if the background color of the window is
	 * not {@code null} and the alpha component of the color is less than
		...
		AWTAccessor.setWindowAccessor(new AWTAccessor.WindowAccessor() {
			...
			public void setOpaque(Window window, boolean opaque) {
				...
				window.setBackground(
						new Color(bg.getRed(), bg.getGreen(), bg.getBlue(), opaque ? 255 : 0));
			}
			...
			public Point2D calculateSecurityWarningPosition(Window window, double x, double y,
					double w, double h) {
