 ...
 * <td>After retrieving the AWT event queue, malicious code may peek at and even
 ...
 * First, such an icon may look like the icon of some known application (such as
 * a firewall or anti-virus) and order a user to do something unsafe (with help
 ...
 * <td>The java.awt.Robot object allows code to generate native-level mouse and
 * keyboard events as well as read the screen. It could allow malicious code to
 ...
 * program is in direct control of rendering. Depending on the implementation,
 * the security warning may not be shown for the windows used to enter the
 * full-screen exclusive mode (assuming that the {@code
 ...
 * events dispatched in the system, allowing it to read all user input (such as
 * passwords). Each AWT event listener is called from within the context of that
 * event queue's EventDispatchThread, so if the accessEventQueue permission is
 * also enabled, malicious code could modify the contents of AWT event queues
 ...
 * <td>Interfaces such as the java.awt.Composite interface or the java.awt.Robot
 ...
 * <td>When <code>SecurityManager</code> is installed, the invoking thread must
 ...
 * {@link Window#setAlwaysOnTop}</td>
 * <td>The malicious window might make itself look and behave like a real full
 ...
 * window was created by an applet</td>
 ...
 * security-sensitive decisions based on whether or not the window belongs to an
 * applet (entering a username and password into a dialog box, for example),
 ...
 * applets in the browser. When launching applications from Java Web Start, its
 ...
 * <td>Constantly watching the mouse pointer, an applet can make guesses about
 * what the user is doing, i.e. moving the mouse to the lower left corner of the
 * screen most likely means that the user is about to launch an application. If
 * a virtual keypad is used so that keyboard is emulated using the mouse, an
 * applet may guess what is being typed.</td>
 * </tr>
 * </table>
 *
