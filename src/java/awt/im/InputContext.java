...
/**
 * Provides methods to control text input facilities such as input methods and
 * keyboard layouts. Two methods handle both input methods and keyboard layouts:
 * selectInputMethod lets a client component select an input method or keyboard
 * layout by locale, getLocale lets a client component obtain the locale of the
 * current input method or keyboard layout. The other methods more specifically
 ...
 * hierarchy. However, this means that only one text input operation is possible
 ...
 * package, and installed into a Java SE Runtime Environment as extensions.
 ...
 * implementation or the underlying platform does not support native input
 * methods. In this case, input contexts can still be created and used; their
 * behavior is specified with the individual methods below.
 *
...
public class InputContext {
    ...
    /**
     * Constructs an InputContext. This method is protected so clients cannot
     * instantiate InputContext directly. Input contexts are obtained by calling
     * {@link #getInstance}.
     */
    ...
    /**
     * Attempts to select an input method or keyboard layout that supports the
     ...
     * a native input method or keyboard layout by locale. For host operating
     ...
     * operating systems that don't provide such API, <code>getLocale</code>
     * assumes that the current locale of all native input methods or keyboard
     * layouts provided by the host operating system is the system's default
     * locale.
     *
    ...
    /**
     * Sets the subsets of the Unicode character set that input methods of this
     * input context should be allowed to input. Null may be passed in to
     * indicate that all characters are allowed. The initial value is null. The
     * setting applies to the current input method as well as input methods
     * selected after this call is made. However, applications cannot rely on
     * this call having the desired effect, since this setting cannot be passed
     * on to all host input methods - applications still need to apply their own
     * character validation. If no input methods are available, then this method
     * has no effect.
     *
     * @param subsets
    ...
    /**
     * Enables or disables the current input method for composition, depending
     * on the value of the parameter <code>enable</code>.
     * <p>
     ...
     * An input method that is enabled for composition interprets incoming
     * events for both composition and control purposes, while a disabled input
     * method does not interpret events for composition.
     *
    ...
    /**
     * Asks the current input method to reconvert text from the current client
     * component. The input method obtains the text to be reconverted from the
     * client component using the {@link InputMethodRequests#getSelectedText
     * InputMethodRequests.getSelectedText} method. The other
     * <code>InputMethodRequests</code> methods must be prepared to deal with
     * further information requests by the input method. The composed and/or
     * committed text will be sent to the client component as a sequence of
     * <code>InputMethodEvent</code>s. If the input method cannot reconvert the
     * given text, the text is returned as committed text in an
    ...
    /**
     * Dispatches an event to the active input method. Called by AWT. If no
     ...
     * component's {@link java.awt.Component#removeNotify()
     * Component.removeNotify} method. Potentially pending input from input
     * methods for this component is discarded. If no input methods are
    ...
    /**
     * Releases the resources used by this input context. Called by AWT for the
     * default input context of each Window. If no input methods are available,
     ...
     * control object provides methods that control the behavior of the input
     * method or obtain information from the input method. The type of the
     * object is an input method specific class. Clients have to compare the
     * result against known input method control object classes and cast to the
     * appropriate class to invoke the methods provided.
     * <p>
     * If no input methods are available or the current input method does not
     * provide an input method control object, then null is returned.
     *
