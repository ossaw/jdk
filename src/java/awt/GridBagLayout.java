...
/**
 * The <code>GridBagLayout</code> class is a flexible layout manager that aligns
 * components vertically, horizontally or along their baseline without requiring
 * that the components be of the same size. Each <code>GridBagLayout</code>
 * object maintains a dynamic, rectangular grid of cells, with each component
 * occupying one or more cells, called its <em>display area</em>.
 * <p>
 * Each component managed by a <code>GridBagLayout</code> is associated with an
 * instance of {@link GridBagConstraints}. The constraints object specifies
 * where a component's display area should be located on the grid and how the
 * component should be positioned within its display area. In addition to its
 * constraints object, the <code>GridBagLayout</code> also considers each
 ...
 * right corner of the container with x increasing to the left and y increasing
 * downward.
 * <p>
 * To use a grid bag layout effectively, you must customize one or more of the
 * <code>GridBagConstraints</code> objects that are associated with its
 * components. You customize a <code>GridBagConstraints</code> object by setting
 ...
 * <code>gridx&nbsp;=&nbsp;0</code>, <code>gridy&nbsp;=&nbsp;0</code>. For
 * horizontal left-to-right layout, a component's leading corner is its upper
 * left. For horizontal right-to-left layout, a component's leading corner is
 * its upper right. Use <code>GridBagConstraints.RELATIVE</code> (the default
 * value) to specify that the component be placed immediately following (along
 * the x axis for <code>gridx</code> or the y axis for <code>gridy</code>) the
 * component that was added to the container just before this component was
 * added.
 * <dt>{@link GridBagConstraints#gridwidth},
 * {@link GridBagConstraints#gridheight}
 * <dd>Specifies the number of cells in a row (for <code>gridwidth</code>) or
 * column (for <code>gridheight</code>) in the component's display area. The
 * default value is 1. Use <code>GridBagConstraints.REMAINDER</code> to specify
 * that the component's display area will be from <code>gridx</code> to the last
 * cell in the row (for <code>gridwidth</code>) or from <code>gridy</code> to
 * the last cell in the column (for <code>gridheight</code>).
 *
 * Use <code>GridBagConstraints.RELATIVE</code> to specify that the component's
 * display area will be from <code>gridx</code> to the next to the last cell in
 * its row (for <code>gridwidth</code> or from <code>gridy</code> to the next to
 * the last cell in its column (for <code>gridheight</code>).
 *
 * <dt>{@link GridBagConstraints#fill}
 * <dd>Used when the component's display area is larger than the component's
 * requested size to determine whether (and how) to resize the component.
 * Possible values are <code>GridBagConstraints.NONE</code> (the default),
 * <code>GridBagConstraints.HORIZONTAL</code> (make the component wide enough to
 * fill its display area horizontally, but don't change its height),
 * <code>GridBagConstraints.VERTICAL</code> (make the component tall enough to
 * fill its display area vertically, but don't change its width), and
 * <code>GridBagConstraints.BOTH</code> (make the component fill its display
 * area entirely).
 * <dt>{@link GridBagConstraints#ipadx}, {@link GridBagConstraints#ipady}
 * <dd>Specifies the component's internal padding within the layout, how much to
 * add to the minimum size of the component. The width of the component will be
 * at least its minimum width plus <code>ipadx</code> pixels. Similarly, the
 * height of the component will be at least the minimum height plus
 * <code>ipady</code> pixels.
 * <dt>{@link GridBagConstraints#insets}
 * <dd>Specifies the component's external padding, the minimum amount of space
 ...
 * There are three kinds of possible values: absolute, orientation-relative, and
 * baseline-relative Orientation relative values are interpreted relative to the
 * container's <code>ComponentOrientation</code> property while absolute values
 * are not. Baseline relative values are calculated relative to the baseline.
 * Valid values are:
 *
 * <center>
 * <table BORDER=0 WIDTH=800 SUMMARY=
 ...
 * <dd>Used to determine how to distribute space, which is important for
 * specifying resizing behavior. Unless you specify a weight for at least one
 * component in a row (<code>weightx</code>) and column (<code>weighty</code>),
 * all the components clump together in the center of their container. This is
 * because when the weight is zero (the default), the <code>GridBagLayout</code>
 * object puts any extra space between its grid of cells and the edges of the
 * container.
 * </dl>
 * <p>
 * Each row may have a baseline; the baseline is determined by the components in
 * that row that have a valid baseline and are aligned along the baseline (the
 * component's anchor value is one of {@code
 * BASELINE}, {@code BASELINE_LEADING} or {@code BASELINE_TRAILING}). If none of
 * the components in the row has a valid baseline, the row does not have a
 * baseline.
 * <p>
 * If a component spans rows it is aligned either to the baseline of the start
 * row (if the baseline-resize behavior is {@code
 * CONSTANT_ASCENT}) or the end row (if the baseline-resize behavior is
 * {@code CONSTANT_DESCENT}). The row that the component is aligned to is called
 * the <em>prevailing row</em>.
 * <p>
 * The following figure shows a baseline layout and includes a component that
 * spans rows: <center>
 * <table summary="Baseline Layout">
 * <tr ALIGN=CENTER>
 * <td><img src="doc-files/GridBagLayout-baseline.png" alt=
 * "The following text describes this graphic (Figure 1)." style="float:center">
 * </td>
 * </table>
 * </center> This layout consists of three components:
 * <ul>
 * <li>A panel that starts in row 0 and ends in row 1. The panel has a
 * baseline-resize behavior of <code>CONSTANT_DESCENT</code> and has an anchor
 * of <code>BASELINE</code>. As the baseline-resize behavior is
 * <code>CONSTANT_DESCENT</code> the prevailing row for the panel is row 1.
 * <li>Two buttons, each with a baseline-resize behavior of
 * <code>CENTER_OFFSET</code> and an anchor of <code>BASELINE</code>.
 * </ul>
 * Because the second button and the panel share the same prevailing row, they
 * are both aligned along their baseline.
 * <p>
 ...
 * prevailing row changes. The baseline is anchored to the bottom of the display
 * area if any components with the same prevailing row have a baseline-resize
 * behavior of <code>CONSTANT_DESCENT</code>, otherwise the baseline is anchored
 * to the top of the display area. The following rules dictate the resize
 * behavior:
 * <ul>
 * <li>Resizable components positioned above the baseline can only grow as tall
 * as the baseline. For example, if the baseline is at 100 and anchored at the
 * top, a resizable component positioned above the baseline can never grow more
 * than 100 units.
 * <li>Similarly, resizable components positioned below the baseline can only
 * grow as high as the difference between the display height and the baseline.
 * <li>Resizable components positioned on the baseline with a baseline-resize
 * behavior of <code>OTHER</code> are only resized if the baseline at the
 * resized size fits within the display area. If the baseline is such that it
 * does not fit within the display area the component is not resized.
 * <li>Components positioned on the baseline that do not have a baseline-resize
 * behavior of <code>OTHER</code> can only grow as tall as
 * {@code display height - baseline + baseline of component}.
 * </ul>
 * If you position a component along the baseline, but the component does not
 * have a valid baseline, it will be vertically centered in its space. Similarly
 * if you have positioned a component relative to the baseline and none of the
 * components in the row have a valid baseline the component is vertically
 * centered.
 * <p>
 * The following figures show ten components (all buttons) managed by a grid bag
 * layout. Figure 2 shows the layout for a horizontal, left-to-right container
 * and Figure 3 shows the layout for a horizontal, right-to-left container.
 *
 * <center>
 * <table WIDTH=600 summary="layout">
 * <tr ALIGN=CENTER>
 * <td><img src="doc-files/GridBagLayout-1.gif" alt=
 * "The preceding text describes this graphic (Figure 1)." style=
 * "float:center; margin: 7px 10px;"></td>
 * <td><img src="doc-files/GridBagLayout-2.gif" alt=
 * "The preceding text describes this graphic (Figure 2)." style=
 * "float:center; margin: 7px 10px;"></td>
 * <tr ALIGN=CENTER>
 ...
 * </center>
 * <p>
 * Each of the ten components has the <code>fill</code> field of its associated
 * <code>GridBagConstraints</code> object set to
 * <code>GridBagConstraints.BOTH</code>. In addition, the components have the
 * following non-default constraints:
 *
 * <ul>
...
public class GridBagLayout implements LayoutManager2, java.io.Serializable {
	 ...
	 * compatibility. Previously, this was the maximum number of grid positions
	 * (both horizontal and vertical) that could be laid out by the grid bag
	 * layout. Current implementation doesn't impose any limits on the size of a
	 * grid.
	 */
	...
	/**
	 * This hashtable maintains the association between a component and its
	 * gridbag constraints. The Keys in <code>comptable</code> are the
	 * components and the values are the instances of
	...
	/**
	 * This field holds the layout information for the gridbag. The information
	 * in this field is based on the most recent validation of the gridbag. If
	 * <code>layoutInfo</code> is <code>null</code> this indicates that there
	 * are no components in the gridbag or if there are components, they have
	...
	/**
	 * This field holds the overrides to the column minimum width. If this field
	 * is non-<code>null</code> the values are applied to the gridbag after all
	 * of the minimum columns widths have been calculated. If columnWidths has
	 * more elements than the number of columns, columns are added to the
	 * gridbag to match the number of elements in columnWidth.
	 *
	 * @serial
	 ...
	 * is non-<code>null</code> the values are applied to the gridbag after all
	 * of the minimum row heights have been calculated. If
	 * <code>rowHeights</code> has more elements than the number of rows, rows
	 * are added to the gridbag to match the number of elements in
	 * <code>rowHeights</code>.
	 *
	 * @serial
	 * @see #getLayoutDimensions()
	 */
	 ...
	 * non-<code>null</code> the values are applied to the gridbag after all of
	 * the columns weights have been calculated. If
	 * <code>columnWeights[i]</code> &gt; weight for column i, then column i is
	 * assigned the weight in <code>columnWeights[i]</code>. If
	 * <code>columnWeights</code> has more elements than the number of columns,
	 * the excess elements are ignored - they do not cause more columns to be
	 * created.
	 *
	 * @serial
	 */
	 ...
	 * <code>null</code> the values are applied to the gridbag after all of the
	 * rows weights have been calculated. If <code>rowWeights[i]</code> &gt;
	 * weight for row i, then row i is assigned the weight in
	 * <code>rowWeights[i]</code>. If <code>rowWeights</code> has more elements
	 * than the number of rows, the excess elements are ignored - they do not
	 * cause more rows to be created.
	 *
	 * @serial
	 */
	 ...
	 * @return the constraint for the specified component in this grid bag
	 *         layout; a copy of the actual constraint object is returned
	 */
	...
	/**
	 * Retrieves the constraints for the specified component. The return value
	 * is not a copy, but is the actual <code>GridBagConstraints</code> object
	 * used by the layout mechanism.
	 * <p>
	 * If <code>comp</code> is not in the <code>GridBagLayout</code>, a set of
	 * default <code>GridBagConstraints</code> are returned. A <code>comp</code>
	 * value of <code>null</code> is invalid and returns <code>null</code>.
	 *
	 * @param comp
	 *            the component to be queried
	 ...
	 * coordinates of the top-left corner of the layout area regardless of the
	 * <code>ComponentOrientation</code> value of the container. This is
	 * distinct from the grid origin given by the cell coordinates (0,0). Most
	 * applications do not call this method directly.
	 * 
	 * @return the graphics origin of the cell in the top-left corner of the
	 *         layout grid
	 * @see java.awt.ComponentOrientation
	 * @since JDK1.1
	 */
	 ...
	 * 
	 * @return an array of two arrays, containing the widths of the layout
	 *         columns and the heights of the layout rows
	 * @since JDK1.1
	 */
	...
	/**
	 * Determines the weights of the layout grid's columns and rows. Weights are
	 * used to calculate how much a given column or row stretches beyond its
	 * preferred size, if the layout has extra room to fill.
	 * <p>
	 * Most applications do not call this method directly.
	 * 
	 * @return an array of two arrays, representing the horizontal weights of
	 *         the layout columns and the vertical weights of the layout rows
	 * @since JDK1.1
	 */
	 ...
	 * <code>(x,&nbsp;y)</code>. Each cell is identified by its column index
	 * (ranging from 0 to the number of columns minus 1) and its row index
	 * (ranging from 0 to the number of rows minus 1).
	 * <p>
	 * If the <code>(x,&nbsp;y)</code> point lies outside the grid, the
	 * following rules are used. The column index is returned as zero if
	 * <code>x</code> lies to the left of the layout for a left-to-right
	 * container or to the right of the layout for a right-to-left container.
	 * The column index is returned as the number of columns if <code>x</code>
	 * lies to the right of the layout in a left-to-right container or to the
	 * left in a right-to-left container. The row index is returned as zero if
	 * <code>y</code> lies above the layout, and as the number of rows if
	 * <code>y</code> lies below the layout. The orientation of a container is
	 * determined by its <code>ComponentOrientation</code> property.
	 * 
	 * @param x
	 *            the <i>x</i> coordinate of a point
	 * @param y
	 *            the <i>y</i> coordinate of a point
	 * @return an ordered pair of indexes that indicate which cell in the layout
	 *         grid contains the point (<i>x</i>,&nbsp;<i>y</i>).
	 * @see java.awt.ComponentOrientation
	 * @since JDK1.1
	 */
	 ...
	 *                if <code>constraints</code> is not a
	...
	/**
	 * Lays out the specified container using this grid bag layout. This method
	 * reshapes components in the specified container in order to satisfy the
	 * constraints of this <code>GridBagLayout</code> object.
	 * <p>
	 * Most applications do not call this method directly.
	 * 
	 * @param parent
	 ...
	 *
	 * System.out.println("Col\tWidth\tWeight"); for (x=0; x<s.width; x++) {
	 * System.out.println(x + "\t" + s.minWidth[x] + "\t" + s.weightX[x]); }
	 * System.out.println("Row\tHeight\tWeight"); for (x=0; x<s.height; x++) {
	 * System.out.println(x + "\t" + s.minHeight[x] + "\t" + s.weightY[x]); } }
	 */
	 ...
	 * System.out.println( "wt " + constraints.weightx + " " +
	 * constraints.weighty + ", " +
	 *
	 * "box " + constraints.gridx + " " + constraints.gridy + " " +
	 * constraints.gridwidth + " " + constraints.gridheight + ", " +
	 *
	 * "min " + constraints.minWidth + " " + constraints.minHeight + ", " +
	 *
	 * "pad " + constraints.insets.bottom + " " + constraints.insets.left + " "
	 * + constraints.insets.right + " " + constraints.insets.top + " " +
	 * constraints.ipadx + " " + constraints.ipady); }
	 */

	/**
	 * Fills in an instance of <code>GridBagLayoutInfo</code> for the current
	 * set of managed children. This requires three passes through the set of
	 * children:
	 *
	 * <ol>
	 ...
	 * </ol>
	 *
	 * This also caches the minsizes for all the children when they are first
	 * encountered (so subsequent loops don't need to ask again).
	 * <p>
	 * This method should only be used internally by <code>GridBagLayout</code>.
	 *
	 * @param parent
	 ...
	 * we may use preCalculated sizes in whole class because of upper estimation
	 * of maximumArrayXIndex and maximumArrayYIndex.
	 */
	...
	private long[] preInitMaximumArraySizes(Container parent) {
		...
		for (int compId = 0; compId < components.length; compId++) {
			...
			// -1==RELATIVE, means that column|row equals to previously added
			// component,
			// since each next Component with gridx|gridy == RELATIVE starts
			// from
			// previous position, so we should start from previous component
			// which
			// already used in maximumArray[X|Y]Index calculation. We could just
			// increase
			// maximum by 1 to handle situation when component with gridx=-1 was
			// added.
			if (curX < 0) {
			...
			// gridwidth|gridheight may be equal to RELATIVE (-1) or REMAINDER
			// (0)
			// in any case using 1 instead of 0 or -1 should be sufficient to
			// for
		...
		/*
		 * fix for 4623196. now return long array instead of Point
		 */
	 ...
	 * new code should call {@link #getLayoutInfo(java.awt.Container, int)
	 * getLayoutInfo} instead. This method is the same as
	 * <code>getLayoutInfo</code>; refer to <code>getLayoutInfo</code> for
	 * details on parameters and return value.
	 */
	protected GridBagLayoutInfo GetLayoutInfo(Container parent, int sizeflag) {
		synchronized (parent.getTreeLock()) {
			...
			Dimension d;
			Component components[] = parent.getComponents();
			// Code below will address index curX+curWidth in the case of
			// yMaxArray, weightY
			...
			/*
			 * fix for 4623196. If user try to create a very big grid we can get
			 * NegativeArraySizeException because of integer value overflow
			 * (EMPIRICMULTIPLIER*gridSize might be more then
			 * Integer.MAX_VALUE). We need to detect this situation and try to
			 * create a grid with Integer.MAX_VALUE size instead.
			 */
			maximumArrayXIndex = (EMPIRICMULTIPLIER * arraySizes[0] > Integer.MAX_VALUE)
					? Integer.MAX_VALUE : EMPIRICMULTIPLIER * (int) arraySizes[0];
			maximumArrayYIndex = (EMPIRICMULTIPLIER * arraySizes[1] > Integer.MAX_VALUE)
					? Integer.MAX_VALUE : EMPIRICMULTIPLIER * (int) arraySizes[1];
			...
			for (compindex = 0; compindex < components.length; compindex++) {
				...
				/*
				 * Adjust the grid width and height fix for 5005945:
				 * unneccessary loops removed
				 */
				px = curX + curWidth;
				if (layoutWidth < px) {
					layoutWidth = px;
			...
			for (compindex = 0; compindex < components.length; compindex++) {
				...
				if (hasBaseline) {
						...
						if (constraints.ascent >= 0) {
							if (curHeight == 1) {
								...
								if (constraints.baselineResizeBehavior == Component.BaselineResizeBehavior.CONSTANT_DESCENT) {
									maxDescent[curY + curHeight - 1] = Math.max(
											maxDescent[curY + curHeight - 1], constraints.descent);
								} else {
									maxAscent[curY] = Math.max(maxAscent[curY], constraints.ascent);
								}
							}
							if (constraints.baselineResizeBehavior == Component.BaselineResizeBehavior.CONSTANT_DESCENT) {
								baselineType[curY + curHeight
										- 1] |= (1 << constraints.baselineResizeBehavior.ordinal());
							} else {
								baselineType[curY] |= (1 << constraints.baselineResizeBehavior
										.ordinal());
							}
						}
						break;
					...
					case GridBagConstraints.ABOVE_BASELINE_TRAILING:
						...
						pixels_diff = constraints.minHeight + constraints.insets.top
								+ constraints.ipady;
					...
					case GridBagConstraints.BELOW_BASELINE_TRAILING:
						...
						pixels_diff = constraints.minHeight + constraints.insets.bottom
								+ constraints.ipady;
			...
			if (rowWeights != null)
				System.arraycopy(rowWeights, 0, r.weightY, 0,
						Math.min(r.weightY.length, rowWeights.length));
			...
			for (i = 1; i != Integer.MAX_VALUE; i = nextSize, nextSize = Integer.MAX_VALUE) {
				for (compindex = 0; compindex < components.length; compindex++) {
					...
					if (constraints.tempWidth == i) {
						px = constraints.tempX
								+ constraints.tempWidth; /* right column */
					...
					if (constraints.tempHeight == i) {
						py = constraints.tempY
								+ constraints.tempHeight; /* bottom row */
						...
						if (hasBaseline) {
							...
							case GridBagConstraints.ABOVE_BASELINE_LEADING:
							case GridBagConstraints.ABOVE_BASELINE_TRAILING:
								pixels_diff = constraints.insets.top + constraints.minHeight
										+ constraints.ipady + maxDescent[constraints.tempY];
								break;
							case GridBagConstraints.BELOW_BASELINE:
							case GridBagConstraints.BELOW_BASELINE_LEADING:
							case GridBagConstraints.BELOW_BASELINE_TRAILING:
	...
	/**
	 * Calculate the baseline for the specified component. If {@code c} is
	 * positioned along it's baseline, the baseline is obtained and the
	 * {@code constraints} ascent, descent and baseline resize behavior are set
	 * from the component; and true is returned. Otherwise false is returned.
	 */
	...
	/**
	 * Adjusts the x, y, width, and height fields to the correct values
	 * depending on the constraint geometry and pads. This method should only be
	 * used internally by <code>GridBagLayout</code>.
	 *
	 * @param constraints
	 *            the constraints to be applied
	 * @param r
	 ...
	 * {@link #adjustForGravity(java.awt.GridBagConstraints, java.awt.Rectangle)
	 * adjustForGravity} instead. This method is the same as
	 * <code>adjustForGravity</code>; refer to <code>adjustForGravity</code> for
	 ...
	 *
	 * @param cellY
	 *            the location of the row, does not include insets
	 * @param cellHeight
	 *            the height of the row, does not take into account insets
	 * @param r
	 *            available bounds for the component, is padded by insets and
	 *            ipady
	 */
		...
		if (cons.ascent >= 0) {
			if (cons.baselineResizeBehavior == Component.BaselineResizeBehavior.CONSTANT_DESCENT) {
				...
				int maxY = cellY + cellHeight
						- layoutInfo.maxDescent[cons.tempY + cons.tempHeight - 1] + cons.descent
						- cons.insets.bottom;
				...
				if (cons.baselineResizeBehavior == Component.BaselineResizeBehavior.OTHER) {
					...
					if (ascent >= 0 && ascent <= baseline) {
						...
						if (baseline + (r.height - ascent - cons.insets.top) <= cellHeight
								- cons.insets.bottom) {
							// It fits, we're good.
							fits = true;
							...
							int ascent2 = componentAdjusting.getBaseline(r.width,
									cellHeight - cons.insets.bottom - baseline + ascent);
							if (ascent2 >= 0) {
				...
				if (cons.isVerticallyResizable()) {
					...
					case CONSTANT_ASCENT:
						r.height = Math.max(cons.minHeight,
								cellY + cellHeight - r.y - cons.insets.bottom);
						break;
					case CENTER_OFFSET: {
						...
						delta += delta;
						if (delta > 0 && (cons.minHeight + cons.centerPadding + delta) / 2
								+ cons.centerOffset != baseline) {
							// Off by 1
							delta--;
						}
						r.height = cons.minHeight + delta;
						r.y = cellY + baseline - (r.height + cons.centerPadding) / 2
								- cons.centerOffset;
					}
						break;
					case OTHER:
	...
	/**
	 * Positions the specified component above the baseline. That is the bottom
	 * edge of the component will be aligned along the baseline. If the row does
	 * not have a baseline, this centers the component.
	 */
	private void alignAboveBaseline(GridBagConstraints cons, Rectangle r, int cellY,
			int cellHeight) {
	 ...
	 */
	private void alignBelowBaseline(GridBagConstraints cons, Rectangle r, int cellY,
			int cellHeight) {
		if (layoutInfo.hasBaseline(cons.tempY)) {
			if (layoutInfo.hasConstantDescent(cons.tempY)) {
				// Prefer descent
	...
	private void centerVertically(GridBagConstraints cons, Rectangle r, int cellHeight) {
		if (!cons.isVerticallyResizable()) {
			r.y += Math.max(0, (cellHeight - cons.insets.top - cons.insets.bottom - cons.minHeight
					- cons.ipady) / 2);
		}
	}
	 ...
	 * <code>getLayoutInfo</code>. This method should only be used internally by
	 ...
	 * This method is obsolete and supplied for backwards compatibility only;
	 * new code should call
	 * {@link #getMinSize(java.awt.Container, GridBagLayoutInfo) getMinSize}
	 * instead. This method is the same as <code>getMinSize</code>; refer to
	 * <code>getMinSize</code> for details on parameters and return value.
	 */
	protected Dimension GetMinSize(Container parent, GridBagLayoutInfo info) {
	...
	/**
	 * Lays out the grid. This method should only be used internally by
	 * <code>GridBagLayout</code>.
	 *
	 * @param parent
	 *            the layout container
	...
	/**
	 * This method is obsolete and supplied for backwards compatibility only;
	 * new code should call {@link #arrangeGrid(Container) arrangeGrid} instead.
	 * This method is the same as <code>arrangeGrid</code>; refer to
	 * <code>arrangeGrid</code> for details on the parameter.
	 */
	protected void ArrangeGrid(Container parent) {
		Component comp;
		int compindex;
		 ...
		 * DEBUG
		 *
		 * DumpLayoutInfo(info); for (compindex = 0 ; compindex <
		 * components.length ; compindex++) { comp = components[compindex]; if
		 * (!comp.isVisible()) continue; constraints = lookupConstraints(comp);
		 * DumpConstraints(constraints); } System.out.println("minSize " +
		 * r.width + " " + r.height);
		 */
		 ...
		 * DEBUG
		 *
		 * System.out.println("Re-adjusted:"); DumpLayoutInfo(info);
		 */
		...
		for (compindex = 0; compindex < components.length; compindex++) {
			...
			if ((r.width <= 0) || (r.height <= 0)) {
				...
				if (comp.x != r.x || comp.y != r.y || comp.width != r.width
