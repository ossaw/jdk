...
public interface Shape {
	 ...
	 * > definition of insideness</a> can lead to situations where points on the
	 * defining outline of the {@code shape} may not be considered contained in
	 * the returned {@code bounds} object, but only in cases where those points
	 * are also not considered contained in the original {@code shape}.
	 * </p>
	 * <p>
	 * If a {@code point} is inside the {@code shape} according to the
	 * {@link #contains(double x, double y) contains(point)} method, then it
	 * must be inside the returned {@code Rectangle} bounds object according to
	 * the {@link #contains(double x, double y) contains(point)} method of the
	 ...
	 * </p>
	 * <p>
	 * If a {@code point} is not inside the {@code shape}, then it might still
	 * be contained in the {@code bounds} object:
	 * </p>
	 * <p>
	 ...
	 * <p>
	 * If a {@code point} is inside the {@code shape} according to the
	 * {@link #contains(Point2D p) contains(point)} method, then it must be
	 * inside the returned {@code Rectangle2D} bounds object according to the
	 * {@link #contains(Point2D p) contains(point)} method of the {@code bounds}
	 * . Specifically:
	 * </p>
	 * <p>
	 ...
	 * <code>Shape</code>, as described by the <a href=
	 * "{@docRoot}/java/awt/Shape.html#def_insideness"> definition of
	 * insideness</a>.
	 * 
	 * @param x
	 ...
	 * {@code true} even though the rectangular area does not intersect the
	 ...
	 * rectangular area to be considered contained within the <code>Shape</code>
	 * .
	 * <p>
	 ...
	 * </ul>
	 * This means that for some {@code Shapes} this method might return
	 * {@code false} even though the {@code Shape} contains the rectangular
	 * area. The {@link java.awt.geom.Area Area} class performs more accurate
	 * geometric computations than most {@code Shape} objects and therefore can
	 ...
	 * allows a {@code Shape} implementation to conservatively return
	 * {@code false} when:
	 * <ul>
	 * <li>the <code>intersect</code> method returns <code>true</code> and
	 * <li>the calculations to determine whether or not the <code>Shape</code>
	 ...
	 * </ul>
	 * This means that for some {@code Shapes} this method might return
	 * {@code false} even though the {@code Shape} contains the
	 * {@code Rectangle2D}. The {@link java.awt.geom.Area Area} class performs
	 * more accurate geometric computations than most {@code Shape} objects and
