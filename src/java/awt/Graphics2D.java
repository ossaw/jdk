 ...
 * <li>The current <code>Paint</code> is queried for a <code>PaintContext</code>
 ...
 * any inconsistent results between aliased and antialiased rendering. If
 ...
 * <code>drawArc</code>, <code>drawPolyline</code>, and <code>drawPolygon</code>
 * , {@link #draw(Shape) draw} can now be called with the desired
 * <code>Shape</code>. For example, when drawing a rectangle:
 * 
 * <pre>
...
public abstract class Graphics2D extends Graphics {
     ...
     * @param raised
     *               a boolean value that determines whether the rectangle
     *               appears
     ...
     * 
     * @param img
     *              the specified image to be rendered. This method does nothing
     ...
     *         completely rendered, or if it's null; <code>false</code> if the
     ...
     * accordance with the specification of the {@link TextAttribute} class.
     * <p>
     * The baseline of the first character is at position (<i>x</i>,&nbsp;
     * <i>y</i>) in User Space. For characters in script systems such as Hebrew
     * and Arabic, the glyphs can be rendered from right to left, in which case
     * the coordinate supplied is the location of the leftmost character on the
     * baseline.
     * 
     * @param iterator
     ...
     * @param onStroke
     *                 flag used to choose between testing the stroked or the
     *                 filled
     *                 shape. If the flag is <code>true</code>, the
     *                 <code>Stroke</code> outline is tested. If the flag is
     *                 <code>false</code>, the filled <code>Shape</code> is
     *                 tested.
     ...
     *                           if a custom <code>Composite</code> object is
     *                           being used to
     *                           render to the screen and a security manager is
     *                           set and its
     *                           <code>checkPermission</code> method does not
     *                           allow the
     *                           operation.
     * @param comp
     ...
     * 
     * @param color
     *              the background color that is used in subsequent calls to
     *              <code>clearRect</code>
     * @see #getBackground
     ...
     * the clip has been cleared using {@link Graphics#setClip(Shape) setClip}
     * with a <code>null</code> argument, the specified <code>Shape</code>
