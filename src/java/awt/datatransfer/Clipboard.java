...
public class Clipboard {
	...
	/**
	 * A set of <code>DataFlavor</code>s that is available on this local
	 * clipboard. It is used for tracking changes of <code>DataFlavor</code>s
	 * available on this clipboard.
	 *
	 * @since 1.5
	 */
	 ...
	 * <code>owner</code>, that owner is notified that it no longer holds
	 * ownership of the clipboard contents via an invocation of
	 * <code>ClipboardOwner.lostOwnership()</code> on that owner. An
	 * implementation of <code>setContents()</code> is free not to invoke
	 * <code>lostOwnership()</code> directly from this method. For example,
	 * <code>lostOwnership()</code> may be invoked later on a different thread.
	 * The same applies to <code>FlavorListener</code>s registered on this
	 * clipboard.
	 * <p>
	 * The method throws <code>IllegalStateException</code> if the clipboard is
	 ...
	 * <code>null</code>. The parameter Object requestor is not currently used.
	 * The method throws <code>IllegalStateException</code> if the clipboard is
	 * currently unavailable. For example, on some platforms, the system
	 * clipboard is unavailable while it is accessed by another application.
	 *
	 ...
	 * the specified <code>DataFlavor</code>. The class of the object returned
	 ...
	 * <code>FlavorEvent</code>s from this clipboard. If <code>listener</code>
	 ...
	 * receives <code>FlavorEvent</code>s from this <code>Clipboard</code>. This
	 * method performs no function, nor does it throw an exception, if the
	 * listener specified by the argument was not previously added to this
	 * <code>Clipboard</code>. If <code>listener</code> is <code>null</code>, no
	...
	/**
	 * Checks change of the <code>DataFlavor</code>s and, if necessary, notifies
	 * all listeners that have registered interest for notification on
	 * <code>FlavorEvent</code>s.
	 *
	 * @since 1.5
	 */
	private void fireFlavorsChanged() {
		...
		FlavorListener[] flavorListenerArray = (FlavorListener[]) flavorListeners
	...
	/**
	 * Returns a set of <code>DataFlavor</code>s currently available on this
	 * clipboard.
	 *
	 * @return a set of <code>DataFlavor</code>s currently available on this
	 *         clipboard
	 *
	 * @since 1.5
	 */
