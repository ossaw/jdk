 ...
 * determining the next and previous Components to focus when traversing forward
 * or backward in a UI. Each FocusTraversalPolicy must also provide algorithms
 * for determining the first, last, and default Components in a traversal cycle.
 * First and last Components are used when normal forward and backward
 * traversal, respectively, wraps. The default Component is the first to receive
 * focus when traversing down into a new focus traversal cycle. A
 * FocusTraversalPolicy can optionally provide an algorithm for determining a
...
public abstract class FocusTraversalPolicy {
	 ...
	 * visible for the first time. Once the Window has been made visible by a
	 * call to <code>show()</code> or <code>setVisible(true)</code>, the initial
