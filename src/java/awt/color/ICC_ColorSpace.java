...
/**
 *
 * The ICC_ColorSpace class is an implementation of the abstract ColorSpace
 * class. This representation of device independent and device dependent color
 * spaces is based on the International Color Consortium Specification
 * ICC.1:2001-12, File Format for Color Profiles (see
 * <A href="http://www.color.org">http://www.color.org</A>).
 * <p>
 * Typically, a Color or ColorModel would be associated with an ICC Profile
 * which is either an input, display, or output profile (see the ICC
 ...
 * image, or device (see ICC_Profile). Attempting to create an ICC_ColorSpace
 ...
 * directions. Should an ICC_ColorSpace object be used in a way requiring a
 * conversion from PCS to the profile's native space and there is inadequate
 * data to correctly perform the conversion, the ICC_ColorSpace object will
 * produce output in the specified type of color space (e.g. TYPE_RGB,
 * TYPE_CMYK, etc.), but the specific color values of the output data will be
 * undefined.
 * <p>
 ...
 * spaces via ColorSpace.getInstance().
 * 
 * @see ColorSpace
 * @see ICC_Profile
 */
...
public class ICC_ColorSpace extends ColorSpace {
	 ...
	 *                if profile is inappropriate for representing a ColorSpace.
	 */
	 ...
	 * <code>toCIEXYZ</code> method of this color space to first convert from
	 * the input color space to the CS_CIEXYZ color space, and then use the
	 * <code>fromCIEXYZ</code> method of the CS_sRGB color space to convert from
	 * CS_CIEXYZ to the output color space. See {@link #toCIEXYZ(float[])
	 * toCIEXYZ} and {@link #fromCIEXYZ(float[]) fromCIEXYZ} for further
	 * information.
	 * <p>
	 * 
	 * @param colorvalue
	 *            a float array with length of at least the number of components
	 *            in this ColorSpace.
	 * @return a float array of length 3.
	 * @throws ArrayIndexOutOfBoundsException
	 *             if array length is not at least the number of components in
	 *             this ColorSpace.
	 */
	public float[] toRGB(float[] colorvalue) {
		...
		if (this2srgb == null) {
			...
			transformList[0] = mdl.createTransform(thisProfile, ColorTransform.Any,
					ColorTransform.In);
	 ...
	 * the best perceptual match between input and output colors. In order to do
	 * colorimetric conversion of color values, you should use the
	 * <code>toCIEXYZ</code> method of the CS_sRGB color space to first convert
	 * from the input color space to the CS_CIEXYZ color space, and then use the
	 * <code>fromCIEXYZ</code> method of this color space to convert from
	 * CS_CIEXYZ to the output color space. See {@link #toCIEXYZ(float[])
	 * toCIEXYZ} and {@link #fromCIEXYZ(float[]) fromCIEXYZ} for further
	 * information.
	 * <p>
	...
	public float[] fromRGB(float[] rgbvalue) {
		...
		if (srgb2this == null) {
			...
			ICC_ColorSpace srgbCS = (ICC_ColorSpace) ColorSpace.getInstance(CS_sRGB);
			PCMM mdl = CMSManager.getModule();
			transformList[0] = mdl.createTransform(srgbCS.getProfile(), ColorTransform.Any,
					ColorTransform.In);
			transformList[1] = mdl.createTransform(thisProfile, ColorTransform.Any,
					ColorTransform.Out);
	 ...
	 * conversion process in which colors are transformed from an input color
	 * space to CS_CIEXYZ and then to an output color space. This representation
	 * is not the same as the XYZ values that would be measured from the given
	 * color value by a colorimeter. A further transformation is necessary to
	 * compute the XYZ values that would be measured using current CIE
	 * recommended practices. The paragraphs below explain this in more detail.
	 * <p>
	 ...
	 * space used in the methods toCIEXYZ and fromCIEXYZ is the CIEXYZ PCS
	 * defined by the ICC Specification. This is also the color space
		...
		if (this2xyz == null) {
			...
			try {
				transformList[0] = mdl.createTransform(thisProfile,
						ICC_Profile.icRelativeColorimetric, ColorTransform.In);
			} catch (CMMException e) {
				transformList[0] = mdl.createTransform(thisProfile, ColorTransform.Any,
						ColorTransform.In);
			}
			transformList[1] = mdl.createTransform(xyzCS.getProfile(), ColorTransform.Any,
					ColorTransform.Out);
	 ...
	 * the CS_CIEXYZ color space. This representation is useful in a two-step
	 * color conversion process in which colors are transformed from an input
	 * color space to CS_CIEXYZ and then to an output color space. The color
	 * values returned by this method are not those that would produce the XYZ
	 * value passed to the method when measured by a colorimeter. If you have
	 * XYZ values corresponding to measurements made using current CIE
	...
	public float[] fromCIEXYZ(float[] colorvalue) {
		...
		if (xyz2this == null) {
			...
			ICC_ColorSpace xyzCS = (ICC_ColorSpace) ColorSpace.getInstance(CS_CIEXYZ);
			PCMM mdl = CMSManager.getModule();
			transformList[0] = mdl.createTransform(xyzCS.getProfile(), ColorTransform.Any,
					ColorTransform.In);
			try {
				transformList[1] = mdl.createTransform(thisProfile,
						ICC_Profile.icRelativeColorimetric, ColorTransform.Out);
			} catch (CMMException e) {
				transformList[1] = CMSManager.getModule().createTransform(thisProfile,
	 ...
	 * component. For TYPE_XYZ spaces, this method returns minimum values of 0.0
	 * for all components. For TYPE_Lab spaces, this method returns 0.0 for L
	 * and -128.0 for a and b components. This is consistent with the encoding
	 * of the XYZ and Lab Profile Connection Spaces in the ICC specification.
	 * For all other types, this method returns 0.0 for all components. When
	 * using an ICC_ColorSpace with a profile that requires different minimum
	 ...
	 *             if component is less than 0 or greater than numComponents -
	 *             1.
	 * @since 1.4
	 */
	 ...
	 * component. For TYPE_XYZ spaces, this method returns maximum values of 1.0
	 * + (32767.0 / 32768.0) for all components. For TYPE_Lab spaces, this
	 * method returns 100.0 for L and 127.0 for a and b components. This is
	 * consistent with the encoding of the XYZ and Lab Profile Connection Spaces
	 * in the ICC specification. For all other types, this method returns 1.0
	 * for all components. When using an ICC_ColorSpace with a profile that
	 * requires different maximum component values, it is necessary to subclass
