 ...
 * have one-to-one correspondence. For example, the character '&aacute;' LATIN
 * SMALL LETTER A WITH ACUTE, can be represented by two glyphs: one for 'a' and
 * one for '&acute;'. On the other hand, the two-character string "fi" can be
 * represented by a single glyph, an "fi" ligature. In complex writing systems,
 * such as Arabic or the South and South-East Asian writing systems, the
 * relationship between characters and glyphs can be more complicated and
 * involve context-dependent selection of glyphs as well as glyph reordering.
 *
 ...
 * Internationalization FAQ</a> document.
 *
...
public class Font implements java.io.Serializable {
    ...
    /*
     * If the origin of a Font is a created font then this attribute must be set
     * on all derived fonts too.
     */
    private transient boolean createdFont = false;

    /*
     * This is true if the font transform is not identity. It is used to avoid
     * unnecessary instantiation of an AffineTransform.
     */
    private transient boolean nonIdentityTx;

    /*
     * A cached value used when a transform is required for internal use. This
     * must not be exposed to callers since AffineTransform is mutable.
     */
    ...
    /**
     * Return the AttributeValues object associated with this font. Most of the
     * time, the internal object is null. If required, it will be created from
     * the 'standard' state on the font. Only non-default values will be set in
     * the AttributeValues object.
     *
     * <p>
     * Since the AttributeValues object is mutable, and it is cached in the
     * font, care must be taken to ensure that it is not mutated.
     */
    ...
    private Font2D getFont2D() {
        ...
        if (fm.usingPerAppContextComposites() && font2DHandle != null
            ...
            font2DHandle = fm.findFont2D(name, style,
                    FontManager.LOGICAL_FALLBACK).handle;
        }
        /*
         * Do not cache the de-referenced font2D. It must be explicitly
         * de-referenced to pick up a valid font in the event that the original
         * one is marked invalid
         */
     ...
     *              names for logical fonts are: Dialog, DialogInput,
     *              Monospaced,
     ...
     * @param style
     *              the style constant for the {@code Font} The style argument
     *              is
     *              an integer bitmask that may be {@code PLAIN}, or a bitwise
     *              union of {@code BOLD} and/or {@code ITALIC} (for example,
     *              {@code ITALIC} or {@code BOLD|ITALIC}). If the style
     *              argument
     *              does not conform to one of the expected integer bitmasks
     *              then
     *              the style is set to {@code PLAIN}.
     * @param size
    ...
    private Font(String name, int style, float sizePts, boolean created,
         ...
         * parent. One exception is that if the derived font is requested to be
         * in a different style, then also check if its a CompositeFont and if
         * so build a new CompositeFont from components of that style.
         * CompositeFonts can only be marked as "created" if they are used to
         * add fall backs to a physical font. And non-composites are always from
         * "Font.createFont()" and shouldn't get this treatment.
         */
        if (created) {
            if (handle.font2D instanceof CompositeFont && handle.font2D
                    .getStyle() != style) {
                FontManager fm = FontManagerFactory.getInstance();
    ...
    private Font(File fontFile, int fontFormat, boolean isCopy,
        ...
        /*
         * Font2D instances created by this method track their font file so that
         * when the Font2D is GC'd it can also remove the file.
         */
        FontManager fm = FontManagerFactory.getInstance();
    ...
    /*
     * This constructor is used when one font is derived from another. Fonts
     * created from a stream will use the same font2D instance as the parent.
     * They can be distinguished because the "created" argument will be "true".
     * Since there is no way to recreate these fonts they need to have the
     * handle to the underlying font2D passed in. "created" is also true when a
     * special composite is referenced by the handle for essentially the same
     * reasons. But when deriving a font in these cases two particular
     * attributes need special attention: family/face and style. The
     * "composites" in these cases need to be recreated with optimal fonts for
     * the new values of family and style. For fonts created with createFont()
     * these are treated differently. JDK can often synthesise a different style
     * (bold from plain for example). For fonts created with "createFont" this
     * is a reasonable solution but its also possible (although rare) to derive
     * a font with a different family attribute. In this case JDK needs to break
     * the tie with the original Font2D and find a new Font. The oldName and
     * oldStyle are supplied so they can be compared with what the Font2D and
     * the values. To speed things along : oldName == null will be interpreted
     * as the name is unchanged. oldStyle = -1 will be interpreted as the style
     * is unchanged. In these cases there is no need to interrogate "values".
     */
        ...
        if (created) {
            ...
            if (handle.font2D instanceof CompositeFont) {
                if (newStyle != -1 || newName != null) {
                    FontManager fm = FontManagerFactory.getInstance();
                    this.font2DHandle = fm.getNewComposite(newName, newStyle,
                            handle);
                }
    ...
    /**
     * Creates a new <code>Font</code> with the specified attributes. Only keys
     * defined in {@link java.awt.font.TextAttribute TextAttribute} are
     * recognized. In addition the FONT attribute is not recognized by this
     * constructor (see {@link #getAvailableAttributes}). Only attributes that
     * have values of valid types will affect the new <code>Font</code>.
     * <p>
     * If <code>attributes</code> is <code>null</code>, a new <code>Font</code>
     * is initialized with default values.
     * 
     * @see java.awt.font.TextAttribute
     * @param attributes
     *                   the attributes to assign to the new <code>Font</code>,
     *                   or
     *                   <code>null</code>
     */
     ...
     * 
     * @param font
     *             from which to create this <code>Font</code>.
     ...
     */
    private static final int PRIMARY_MASK = AttributeValues.getMask(EFAMILY,
            EWEIGHT, EWIDTH, EPOSTURE, ESIZE, ETRANSFORM, ESUPERSCRIPT,
            ETRACKING);
     ...
     */
    private static final int LAYOUT_MASK = AttributeValues.getMask(
            ECHAR_REPLACEMENT, EFOREGROUND, EBACKGROUND, EUNDERLINE,
            ESTRIKETHROUGH, ERUN_DIRECTION, EBIDI_EMBEDDING, EJUSTIFICATION,
            EINPUT_METHOD_HIGHLIGHT, EINPUT_METHOD_UNDERLINE, ESWAP_COLORS,
    ...
    /**
     * Returns a <code>Font</code> appropriate to the attributes. If
     * <code>attributes</code>contains a <code>FONT</code> attribute with a
     * valid <code>Font</code> as its value, it will be merged with any
     * remaining attributes. See {@link java.awt.font.TextAttribute#FONT} for
     * more information.
     *
     * @param attributes
     *                   the attributes to assign to the new <code>Font</code>
     * @return a new <code>Font</code> created with the specified attributes
     * @throws NullPointerException
    ...
    public static Font getFont(Map<? extends Attribute, ?> attributes) {
        ...
        if (attributes instanceof AttributeMap && ((AttributeMap) attributes)
                .getValues() != null) {
            ...
            if (values.isNonDefault(EFONT)) {
                ...
                return new Font(values, font.name, font.style, font.createdFont,
                        font.font2DHandle);
            }
    ...
    /**
     * Used with the byte count tracker for fonts created from streams. If a
     * thread can create temp files anyway, no point in counting font bytes.
     */
    ...
    /**
     * Returns a new <code>Font</code> using the specified font type and input
     * data. The new <code>Font</code> is created with a point size of 1 and
     * style {@link #PLAIN PLAIN}. This base font can then be used with the
     * <code>deriveFont</code> methods in this class to derive new
     * <code>Font</code> objects with varying sizes, styles, transforms and font
     * features. This method does not close the {@link InputStream}.
     * <p>
     * To make the <code>Font</code> available to Font constructors the returned
     * <code>Font</code> must be registered in the
     * <code>GraphicsEnviroment</code> by calling
     * {@link GraphicsEnvironment#registerFont(Font) registerFont(Font)}.
     * 
     * @param fontFormat
     *                   the type of the <code>Font</code>, which is
     *                   {@link #TRUETYPE_FONT TRUETYPE_FONT} if a TrueType
     *                   resource is
     *                   specified. or {@link #TYPE1_FONT TYPE1_FONT} if a Type
     *                   1
     *                   resource is specified.
     * @param fontStream
     *                   an <code>InputStream</code> object representing the
     *                   input data
     *                   for the font.
     * @return a new <code>Font</code> created with the specified font type.
     * @throws IllegalArgumentException
     *                                  if <code>fontFormat</code> is not
     *                                  <code>TRUETYPE_FONT</code>
     *                                  or<code>TYPE1_FONT</code>.
     * @throws FontFormatException
     *                                  if the <code>fontStream</code> data does
     *                                  not contain the
     *                                  required font tables for the specified
     *                                  format.
     * @throws IOException
     *                                  if the <code>fontStream</code> cannot be
     *                                  completely read.
    ...
    private static Font createFont0(int fontFormat, InputStream fontStream,
            CreatedFontTracker tracker) throws java.awt.FontFormatException,
            java.io.IOException {
        ...
        try {
            final File tFile = AccessController.doPrivileged(
                    new PrivilegedExceptionAction<File>() {
                        public File run() throws IOException {
                            return Files.createTempFile("+~JF", ".tmp")
                                    .toFile();
                        }
                    });
            if (tracker != null) {
            ...
            int totalSize = 0;
            try {
                final OutputStream outStream = AccessController.doPrivileged(
                ...
                try {
                    ...
                    for (;;) {
                        ...
                        if (tracker != null) {
                            if (totalSize
                                    + bytesRead > CreatedFontTracker.MAX_FILE_SIZE) {
                                throw new IOException("File too big.");
                            }
                            if (totalSize + tracker
                                    .getNumBytes() > CreatedFontTracker.MAX_TOTAL_BYTES) {
                 ...
                 * be removed. To support long-lived AppContexts, we need to
                 * then decrement the byte count by the size of the file. If the
                 * data isn't a valid font, the implementation will delete the
                 * tmp file and decrement the byte count in the tracker object
                 * before returning from the constructor, so we can set
                 * 'copiedFontData' to true here without waiting for the results
                 * of that constructor.
                 */
                copiedFontData = true;
    ...
    /**
     * Returns a new <code>Font</code> using the specified font type and the
     * specified font file. The new <code>Font</code> is created with a point
     * size of 1 and style {@link #PLAIN PLAIN}. This base font can then be used
     * with the <code>deriveFont</code> methods in this class to derive new
     * <code>Font</code> objects with varying sizes, styles, transforms and font
     * features.
     * 
     * @param fontFormat
     ...
     *                   {@link #TRUETYPE_FONT TRUETYPE_FONT} if a TrueType
     *                   resource is
     *                   specified or {@link #TYPE1_FONT TYPE1_FONT} if a Type 1
     *                   resource is specified. So long as the returned font, or
     *                   its
     *                   derived fonts are referenced the implementation may
     *                   continue
     *                   to access <code>fontFile</code> to retrieve font data.
     *                   Thus
     *                   the results are undefined if the file is changed, or
     *                   becomes
     *                   inaccessible.
     *                   <p>
     *                   To make the <code>Font</code> available to Font
     *                   constructors
     *                   the returned <code>Font</code> must be registered in
     *                   the
     *                   <code>GraphicsEnviroment</code> by calling
     *                   {@link GraphicsEnvironment#registerFont(Font)
     *                   registerFont(Font)}.
     * @param fontFile
     *                   a <code>File</code> object representing the input data
     *                   for the
     *                   font.
     * @return a new <code>Font</code> created with the specified font type.
     * @throws IllegalArgumentException
     ...
     *                                  or<code>TYPE1_FONT</code>.
     * @throws NullPointerException
     *                                  if <code>fontFile</code> is null.
     * @throws IOException
     *                                  if the <code>fontFile</code> cannot be
     *                                  read.
     * @throws FontFormatException
     *                                  if <code>fontFile</code> does not
     *                                  contain the required font
    ...
    public static Font createFont(int fontFormat, File fontFile)
        ...
        if (sm != null) {
            FilePermission filePermission = new FilePermission(fontFile
                    .getPath(), "read");
     ...
     * This transform is not necessarily the one used to construct the font. If
     * the font has algorithmic superscripting or width adjustment, this will be
     * incorporated into the returned <code>AffineTransform</code>.
     * <p>
     * Typically, fonts will not be transformed. Clients generally should call
     * {@link #isTransformed} first, and only call this method if
     * <code>isTransformed</code> returns true.
     *
     * @return an {@link AffineTransform} object representing the transform
     *         attribute of this <code>Font</code> object.
     */
    public AffineTransform getTransform() {
        /*
         * The most common case is the identity transform. Most callers should
         * call isTransformed() first, to decide if they need to get the
         * transform, but some may not. Here we check to see if we have a
         * nonidentity transform, and only do the work to fetch and/or compute
         * it if so, otherwise we return a new identity transform.
         * Note that the transform is _not_ necessarily the same as the
         * transform passed in as an Attribute in a Map, as the transform
         * returned will also reflect the effects of WIDTH and SUPERSCRIPT
         * attributes. Clients who want the actual transform need to call
        ...
        if (nonIdentityTx) {
            ...
            if (values.getSuperscript() != 0) {
                ...
                at.preConcatenate(AffineTransform.getTranslateInstance(0,
                        trans));
                at.scale(scale, scale);

                // note on placement and italics
                // We preconcatenate the transform because we don't want to
                // translate along
                // the italic angle, but purely perpendicular to the baseline.
                // While this
                // looks ok for superscripts, it can lead subscripts to stack on
                // each other
                // and bring the following text too close. The way we deal with
                // potential
                // collisions that can occur in the case of italics is by
                // adjusting the
                // horizontal spacing of the adjacent glyphvectors. Examine the
                // italic
                // angle of both vectors, if one is non-zero, compute the
                // minimum ascent
                // and descent, and then the x position at each for each vector
                // along its
                // italic angle starting from its (offset) baseline. Compute the
                // difference
                // between the x positions and use the maximum difference to
                // adjust the
    ...
    private static final float[] ssinfo = { 0.0f, 0.375f, 0.625f, 0.7916667f,
            0.9027778f, 0.9768519f, 1.0262346f, 1.0591564f, };

    /**
     * Returns the family name of this <code>Font</code>.
     *
     * <p>
     * The family name of a font is font specific. Two fonts such as Helvetica
     * Italic and Helvetica Bold have the same family name, <i>Helvetica</i>,
     * whereas their font face names are <i>Helvetica Bold</i> and <i>Helvetica
     * Italic</i>. The list of available family names may be obtained by using
     * the {@link GraphicsEnvironment#getAvailableFontFamilyNames()} method.
     *
     * <p>
     * Use <code>getName</code> to get the logical name of the font. Use
    ...
    /**
     * Returns the family name of this <code>Font</code>, localized for the
     * specified locale.
     *
     * <p>
     * The family name of a font is font specific. Two fonts such as Helvetica
     ...
     * @return a <code>String</code> representing the family name of the font,
     ...
     * Helvetica Bold could be returned as a font face name. Use
     * <code>getFamily</code> to get the family name of the font. Use
     * <code>getName</code> to get the logical name of the font.
     * 
     * @return a <code>String</code> representing the font face name of this
     *         <code>Font</code>.
     * @see #getFamily
     * @see #getName
     * @since 1.2
    ...
    /**
     * Returns the font face name of the <code>Font</code>, localized for the
     * specified locale. For example, Helvetica Fett could be returned as the
     * font face name. Use <code>getFamily</code> to get the family name of the
     * font.
     * 
     * @param l
     *          a locale for which to get the font face name
     * @return a <code>String</code> representing the font face name, localized
    ...
    /**
     * Returns the point size of this <code>Font</code> in <code>float</code>
     * value.
     * 
     * @return the point size of this <code>Font</code> as a <code>float</code>
     *         value.
     * @see #getSize
     * @since 1.2
     */
     ...
     * 
     * @return <code>true</code> if this <code>Font</code> has a PLAIN style;
    ...
    /**
     * Indicates whether or not this <code>Font</code> object has a transform
     * that affects its size in addition to the Size attribute.
     * 
     * @return <code>true</code> if this <code>Font</code> object has a
     *         non-identity AffineTransform attribute. <code>false</code>
     *         otherwise.
    ...
    /**
     * Return true if this Font contains attributes that require extra layout
     * processing.
     * 
     ...
     * interpreted as a <code>Font</code> object according to the specification
     * of <code>Font.decode(String)</code> If the specified property is not
     * found, or the executing code does not have permission to read the
     * property, null is returned instead.
     *
     * @param nm
     *           the property name
     * @return a <code>Font</code> object that the property name describes, or
    ...
    /**
     * Returns the <code>Font</code> that the <code>str</code> argument
     * describes. To ensure that this method returns the desired Font, format
     * the <code>str</code> parameter in one of these ways
     *
     * <ul>
     ...
     * <li><em>fontname</em>
     * </ul>
     * in which <i>style</i> is one of the four case-insensitive strings:
     ...
     * Arial, bold, with a point size of 18, you would call this method with:
     * "Arial-BOLD-18". This is equivalent to calling the Font constructor :
     * <code>new Font("Arial", Font.BOLD, 18);</code> and the values are
     * interpreted as specified by that constructor.
     * <p>
     * A valid trailing decimal field is always interpreted as the pointsize.
     * Therefore a fontname containing a trailing decimal value should not be
     * used in the fontname only form.
     * <p>
     * If a style name field is not one of the valid style strings, it is
     * interpreted as part of the font name, and the default style is used.
     * <p>
     * Only one of ' ' or '-' may be used to separate fields in the input. The
     * identified separator is the one closest to the end of the string which
     * separates a valid pointsize, or a valid style name from the rest of the
     * string. Null (empty) pointsize and style fields are treated as valid
     * fields with the default value for that field.
     * <p>
     * Some font names may include the separator characters ' ' or '-'. If
     * <code>str</code> is not formed with 3 components, e.g. such that
     * <code>style</code> or <code>pointsize</code> fields are not present in
     * <code>str</code>, and <code>fontname</code> also contains a character
     * determined to be the separator character then these characters where they
     * appear as intended to be part of <code>fontname</code> may instead be
     * interpreted as separators so the font name may not be properly
     * recognised.
     *
     * <p>
     * The default size is 12 and the default style is PLAIN. If
     * <code>str</code> does not specify a valid size, the returned
     * <code>Font</code> has a size of 12. If <code>str</code> does not specify
     * a valid style, the returned Font has a style of PLAIN. If you do not
     * specify a valid font name in the <code>str</code> argument, this method
     * will return a font with the family name "Dialog". To determine what font
     * family names are available on your system, use the
     * {@link GraphicsEnvironment#getAvailableFontFamilyNames()} method. If
     * <code>str</code> is <code>null</code>, a new <code>Font</code> is
     * returned with the family name "Dialog", a size of 12 and a PLAIN style.
     * 
     * @param str
     *            the name of the font, or <code>null</code>
    ...
    public static Font decode(String str) {
        ...
        if (sizeIndex > 0 && sizeIndex + 1 < strlen) {
            try {
                fontSize = Integer.valueOf(str.substring(sizeIndex + 1))
                        .intValue();
                if (fontSize <= 0) {
                    fontSize = 12;
                }
    ...
    /**
     * Gets the specified <code>Font</code> from the system properties list. As
     * in the <code>getProperty</code> method of <code>System</code>, the first
     * argument is treated as the name of a system property to be obtained. The
     * <code>String</code> value of this property is then interpreted as a
     * <code>Font</code> object.
     * <p>
     * The property value should be one of the forms accepted by
     * <code>Font.decode(String)</code> If the specified property is not found,
     * or the executing code does not have permission to read the property, the
     * <code>font</code> argument is returned instead.
     * 
     * @param nm
     *             the case-insensitive property name
     * @param font
     *             a default <code>Font</code> to return if property
    ...
    public int hashCode() {
        if (hash == 0) {
            ...
            /*
             * It is possible many fonts differ only in transform. So include
             * the transform in the hash calculation. nonIdentityTx is set
             * whenever there is a transform in 'values'. The tests for null are
             * required because it can also be set for other reasons.
             */
            if (nonIdentityTx && values != null && values
                    .getTransform() != null) {
     ...
     * @return <code>true</code> if the objects are the same or if the argument
     *         is a <code>Font</code> object describing the same font as this
        ...
        if (obj != null) {
            try {
                ...
                if (size == font.size && style == font.style
                        ...
                        && pointSize == font.pointSize && name.equals(
                                font.name)) {

                    /*
                     * 'values' is usually initialized lazily, except when the
                     * font is constructed from a Map, or derived using a Map or
                     * other values. So if only one font has the field
                     * initialized we need to initialize it in the other
    ...
    public String toString() {
        ...
        return getClass().getName() + "[family=" + getFamily() + ",name=" + name
                + ",style=" + strStyle + ",size=" + size + "]";
    } // toString()

    /**
     * Serialization support. A <code>readObject</code> method is neccessary
     * because the constructor creates the font's peer, and we can't serialize
     * the peer. Similarly the computed font "family" may be different at
     * <code>readObject</code> time than at <code>writeObject</code> time. An
     * integer version is written so that future versions of this class will be
    ...
    /**
     * Reads the <code>ObjectInputStream</code>. Unrecognized keys or values
     * will be ignored.
     *
     * @param s
    ...
    private void readObject(java.io.ObjectInputStream s)
        ...
        if (fRequestedAttributes != null) {
            ...
            AttributeValues extras = AttributeValues.fromSerializableHashtable(
    ...
    /**
     * Returns the number of glyphs in this <code>Font</code>. Glyph codes for
     * this <code>Font</code> range from 0 to <code>getNumGlyphs()</code> - 1.
     * 
    ...
    /**
     * Returns the glyphCode which is used when this <code>Font</code> does not
     * have a glyph for a specified unicode code point.
     * 
     ...
     * use a different baseline. The character argument determines the writing
     * system to use. Clients should not assume all characters use the same
     * baseline.
     *
     * @param c
     ...
     * <code>Font</code>. These attributes can be used to derive other fonts.
     * 
     * @return an array containing the keys of all the attributes supported by
     *         this <code>Font</code>.
     * @since 1.2
     */
    public Attribute[] getAvailableAttributes() {
        ...
        Attribute attributes[] = { TextAttribute.FAMILY, TextAttribute.WEIGHT,
                TextAttribute.WIDTH, TextAttribute.POSTURE, TextAttribute.SIZE,
                TextAttribute.TRANSFORM, TextAttribute.SUPERSCRIPT,
                TextAttribute.CHAR_REPLACEMENT, TextAttribute.FOREGROUND,
                TextAttribute.BACKGROUND, TextAttribute.UNDERLINE,
                TextAttribute.STRIKETHROUGH, TextAttribute.RUN_DIRECTION,
                TextAttribute.BIDI_EMBEDDING, TextAttribute.JUSTIFICATION,
                TextAttribute.INPUT_METHOD_HIGHLIGHT,
                TextAttribute.INPUT_METHOD_UNDERLINE, TextAttribute.SWAP_COLORS,
                TextAttribute.NUMERIC_SHAPING, TextAttribute.KERNING,
     ...
     * 
     * @param style
     *              the style for the new <code>Font</code>
     * @param size
     ...
     *              the style for the new <code>Font</code>
     * @param trans
     *              the <code>AffineTransform</code> associated with the new
     *              <code>Font</code>
     * @return a new <code>Font</code> object.
     * @throws IllegalArgumentException
     *                                  if <code>trans</code> is
     *                                  <code>null</code>
     * @since 1.2
     */
    ...
    /**
     * Creates a new <code>Font</code> object by replicating the current
     * <code>Font</code> object and applying a new set of font attributes to it.
     *
     * @param attributes
     ...
     * <a href="../../java/lang/Character.html#supplementary"> supplementary
     * characters</a>. To support all Unicode characters, including
     * supplementary characters, use the {@link #canDisplay(int)} method or
     * <code>canDisplayUpTo</code> methods.
     *
     * @param c
     ...
     *
     * @param codePoint
     *                  the character (Unicode code point) for which a glyph is
     *                  needed.
    ...
    public boolean canDisplay(int codePoint) {
        if (!Character.isValidCodePoint(codePoint)) {
            throw new IllegalArgumentException("invalid code point: " + Integer
     ...
     * characters in the specified <code>text</code> starting at
     * <code>start</code> and ending at <code>limit</code>. This method is a
     * convenience overload.
     * 
     * @param text
     *              the specified array of <code>char</code> values
     * @param start
     *              the specified starting offset (in <code>char</code>s) into
     *              the
     *              specified array of <code>char</code> values
     * @param limit
     *              the specified ending offset (in <code>char</code>s) into the
     *              specified array of <code>char</code> values
     * @return an offset into <code>text</code> that points to the first
     *         character in <code>text</code> that this <code>Font</code> cannot
     *         display; or <code>-1</code> if this <code>Font</code> can display
     *         all characters in <code>text</code>.
     * @since 1.2
     */
     ...
     * specified by the <code>iter</code> starting at <code>start</code> and
     * ending at <code>limit</code>.
     *
     * @param iter
     *              a {@link CharacterIterator} object
     * @param start
     ...
     *              <code>CharacterIterator</code>.
     * @return an offset into <code>iter</code> that points to the first
     *         character in <code>iter</code> that this <code>Font</code> cannot
     *         display; or <code>-1</code> if this <code>Font</code> can display
     *         all characters in <code>iter</code>.
     * @since 1.2
     */
    ...
    /**
     * Returns the italic angle of this <code>Font</code>. The italic angle is
     * the inverse slope of the caret which best matches the posture of this
     * <code>Font</code>.
     * 
    ...
    /*
     * The FRC hints don't affect the value of the italic angle but we need to
     * pass them in to look up a strike. If we can pass in ones already being
     * used it can prevent an extra strike from being allocated. Note that since
     * italic angle is a property of the font, the font transform is needed not
     * the device transform. Finally, this is private but the only caller of
     * this in the JDK - and the only likely caller - is in this same class.
     */
    ...
    /**
     * Checks whether or not this <code>Font</code> has uniform line metrics. A
     * logical <code>Font</code> might be a composite font, which means that it
     * is composed of different physical fonts to cover different code ranges.
     * Each of these fonts might have different <code>LineMetrics</code>. If the
     * logical <code>Font</code> is a single font then the metrics would be
     * uniform.
     * 
     * @return <code>true</code> if this <code>Font</code> has uniform line
    ...
    private FontLineMetrics defaultLineMetrics(FontRenderContext frc) {
        ...
        if (flmref == null || (flm = flmref.get()) == null || !flm.frc.equals(
                frc)) {

            /*
             * The device transform in the frc is not used in obtaining line
             * metrics, although it probably should be: REMIND find why not? The
             * font transform is used but its applied in getFontMetrics, so just
             * pass identity here
             */
            float[] metrics = new float[8];
            getFont2D().getFontMetrics(this, identityTx, frc
                    .getAntiAliasingHint(), frc.getFractionalMetricsHint(),
                    metrics);
            ...
            int baselineIndex = 0; // need real index, assumes roman for
                                   // everything
                                   // need real baselines eventually
            float[] baselineOffsets = { 0, (descent / 2f - ascent) / 2f,
                    -ascent };
            ...
            if (isTransformed()) {
                AffineTransform ctx = values.getCharTransform(); // extract
                                                                 // rotation
                if (ctx != null) {
            ...
            CoreMetrics cm = new CoreMetrics(ascent, descent, leading, height,
                    baselineIndex, baselineOffsets, strikethroughOffset,
                    strikethroughThickness, underlineOffset, underlineThickness,
     ...
     * @param frc
     *            the specified <code>FontRenderContext</code>
    ...
    /**
     * Returns a <code>LineMetrics</code> object created with the specified
     * arguments.
     * 
     * @param str
     *                   the specified <code>String</code>
     * @param beginIndex
     *                   the initial offset of <code>str</code>
     * @param limit
     *                   the end offset of <code>str</code>
     * @param frc
     *                   the specified <code>FontRenderContext</code>
     * @return a <code>LineMetrics</code> object created with the specified
     *         arguments.
     ...
     * @param beginIndex
     *                   the initial offset of <code>chars</code>
     * @param limit
     *                   the end offset of <code>chars</code>
     * @param frc
     *                   the specified <code>FontRenderContext</code>
     * @return a <code>LineMetrics</code> object created with the specified
     *         arguments.
     ...
     * 
     * @param ci
     *                   the specified <code>CharacterIterator</code>
     * @param beginIndex
     *                   the initial offset in <code>ci</code>
     * @param limit
     *                   the end offset of <code>ci</code>
     * @param frc
     *                   the specified <code>FontRenderContext</code>
     * @return a <code>LineMetrics</code> object created with the specified
     *         arguments.
    ...
    /**
     * Returns the logical bounds of the specified <code>String</code> in the
     * specified <code>FontRenderContext</code>. The logical bounds contains the
     * origin, ascent, advance, and height, which includes the leading. The
     * logical bounds does not always enclose all the text. For example, in some
     * languages and in some fonts, accent marks can be positioned above the
     * ascent or below the descent. To obtain a visual bounding box, which
     * encloses all the text, use the {@link TextLayout#getBounds() getBounds}
     * method of <code>TextLayout</code>.
     * <p>
     ...
     * @param frc
     *            the specified <code>FontRenderContext</code>
     * @return a {@link Rectangle2D} that is the bounding box of the specified
     ...
     *                                   if <code>beginIndex</code> is less than
     *                                   zero, or
     *                                   <code>limit</code> is greater than the
     *                                   length of
     *                                   <code>str</code>, or
     ...
     *                                   length of
     *                                   <code>chars</code>, or
     *                                   <code>beginIndex</code> is greater
    ...
    public Rectangle2D getStringBounds(char[] chars, int beginIndex, int limit,
        ...
        if (beginIndex > limit) {
            throw new IndexOutOfBoundsException("range length: " + (limit
                    - beginIndex));
        }
        ...
        boolean simple = values == null || (values.getKerning() == 0 && values
                .getLigatures() == 0 && values.getBaselineTransform() == null);
        if (simple) {
        ...
        if (simple) {
            ...
            return new Rectangle2D.Float(0, -tl.getAscent(), tl.getAdvance(), tl
                    .getAscent() + tl.getDescent() + tl.getLeading());
        }
    }

    /**
     * Returns the logical bounds of the characters indexed in the specified
     * {@link CharacterIterator} in the specified <code>FontRenderContext</code>
     * . The logical bounds contains the origin, ascent, advance, and height,
     * which includes the leading. The logical bounds does not always enclose
     * all the text. For example, in some languages and in some fonts, accent
     * marks can be positioned above the ascent or below the descent. To obtain
     * a visual bounding box, which encloses all the text, use the
     * {@link TextLayout#getBounds() getBounds} method of
     ...
     *                   the initial offset in <code>ci</code>
     * @param limit
     *                   the end offset in <code>ci</code>
     * @param frc
     *                   the specified <code>FontRenderContext</code>
     * @return a <code>Rectangle2D</code> that is the bounding box of the
     ...
     *                                   if <code>beginIndex</code> is less than
     *                                   the start index of
     *                                   <code>ci</code>, or <code>limit</code>
     *                                   is greater than the
     *                                   end index of <code>ci</code>, or
     *                                   <code>beginIndex</code> is
     *                                   greater than <code>limit</code>
     */
    ...
    /**
     * Returns the bounds for the character with the maximum bounds as defined
     ...
     * @param frc
     *            the specified <code>FontRenderContext</code>
     * @return a <code>Rectangle2D</code> that is the bounding box for the
     *         character with the maximum bounds.
     */
    public Rectangle2D getMaxCharBounds(FontRenderContext frc) {
        ...
        return new Rectangle2D.Float(0, -metrics[0], metrics[3], metrics[0]
                + metrics[1] + metrics[2]);
    }

    /**
     * Creates a {@link java.awt.font.GlyphVector GlyphVector} by mapping
     * characters to glyphs one-to-one based on the Unicode cmap in this
     * <code>Font</code>. This method does no other processing besides the
     * mapping of glyphs to characters. This means that this method is not
     * useful for some scripts, such as Arabic, Hebrew, Thai, and Indic, that
     ...
     * @param str
     *            the specified <code>String</code>
     * @return a new <code>GlyphVector</code> created with the specified
     ...
     * @return a new <code>GlyphVector</code> created with the specified array
     *         of characters and the specified <code>FontRenderContext</code>.
     */
    ...
    /**
     * Creates a {@link java.awt.font.GlyphVector GlyphVector} by mapping the
     * specified characters to glyphs one-to-one based on the Unicode cmap in
     * this <code>Font</code>. This method does no other processing besides the
     * mapping of glyphs to characters. This means that this method is not
     * useful for some scripts, such as Arabic, Hebrew, Thai, and Indic, that
     * require reordering, shaping, or ligature substitution.
     * 
     ...
     * @return a new <code>GlyphVector</code> created with the specified integer
     *         array and the specified <code>FontRenderContext</code>.
     */
    public GlyphVector createGlyphVector(FontRenderContext frc,
            int[] glyphCodes) {
    ...
    /**
     * Returns a new <code>GlyphVector</code> object, performing full layout of
     * the text if possible. Full layout is required for complex text, such as
     * Arabic or Hindi. Support for different scripts depends on the font and
     * implementation.
     * <p>
     * Layout requires bidi analysis, as performed by <code>Bidi</code>, and
     * should only be performed on text that has a uniform direction. The
     * direction is indicated in the flags parameter,by using
     * LAYOUT_RIGHT_TO_LEFT to indicate a right-to-left (Arabic and Hebrew) run
     * direction, or LAYOUT_LEFT_TO_RIGHT to indicate a left-to-right (English)
     * run direction.
     * <p>
     * In addition, some operations, such as Arabic shaping, require context, so
     * that the characters at the start and limit can have the proper shapes.
     * Sometimes the data in the buffer outside the provided range does not have
     * valid data. The values LAYOUT_NO_START_CONTEXT and
     * LAYOUT_NO_LIMIT_CONTEXT can be added to the flags parameter to indicate
     * that the text before start, or after limit, respectively, should not be
     ...
     *         start and limit, with glyphs chosen and positioned so as to best
     *         represent the text
     * @throws ArrayIndexOutOfBoundsException
     *                                        if start or limit is out of bounds
     * @see java.text.Bidi
    ...
    public GlyphVector layoutGlyphVector(FontRenderContext frc, char[] text,
        ...
        StandardGlyphVector gv = gl.layout(this, frc, text, start, limit
                - start, flags, null);
        GlyphLayout.done(gl);
        return gv;
    }
    ...
    private static void applyTransform(AffineTransform trans,
