 ...
 * dimensions of the border area may be obtained using the
 * <code>getInsets</code> method, however, since these dimensions are
 * platform-dependent, a valid insets value cannot be obtained until the frame
 ...
 * border effectively obscures a portion of the frame, constraining the area
 ...
 * <p>
 * A frame may have its native decorations (i.e. <code>Frame</code> and
 * <code>Titlebar</code>) turned off with <code>setUndecorated</code>. This can
 * only be done while the frame is not {@link Component#isDisplayable()
 * displayable}.
 * <p>
 * In a multi-screen environment, you can create a <code>Frame</code> on a
 * different screen device by constructing the <code>Frame</code> with
 * {@link #Frame(GraphicsConfiguration)} or {@link #Frame(String title,
 * GraphicsConfiguration)}. The <code>GraphicsConfiguration</code> object is one
 * of the <code>GraphicsConfiguration</code> objects of the target screen
 * device.
 * <p>
 ...
 * relative to the virtual-coordinate system. The origin of the
 ...
 * shows (-80,-100) coords." style="float:center; margin: 7px 10px;">
 * <p>
 ...
 * <code>getLocationOnScreen</code> on a <code>Frame</code> returns virtual
 ...
 * coordinate system.
 * <p>
 * The following code sets the location of the <code>Frame</code> at (10, 10)
 ...
 * <li><code>WINDOW_CLOSING</code>: <br>
...
public class Frame extends Window implements MenuContainer {
    ...
    /**
     * Constructs a new instance of <code>Frame</code> that is initially
     * invisible. The title of the <code>Frame</code> is empty.
     * 
    ...
    /**
     * Constructs a new, initially invisible {@code Frame} with the specified
    ...
    /**
     * Constructs a new, initially invisible <code>Frame</code> object with the
     * specified title.
     * 
     * @param title
    ...
    /**
     * Constructs a new, initially invisible <code>Frame</code> object with the
     * specified title and a <code>GraphicsConfiguration</code>.
     * 
     * @param title
     *              the title to be displayed in the frame's border. A
     *              <code>null</code> value is treated as an empty string, "".
     ...
     * resource. Making a frame displayable will cause any of its children to be
     * made displayable. This method is called internally by the toolkit and
     ...
     * border.
     * 
     * @return the title of this frame, or an empty string ("") if this frame
     ...
     * {@link Window#getIconImages Window.getIconImages()} instead.
     * <p>
     * If a list of several images was specified as a Window's icon, this method
     * will return the first item of the list.
     *
     * @return the icon image for this frame, or <code>null</code> if this frame
     *         doesn't have an icon image.
     ...
     * 
     * @return the menu bar for this frame, or <code>null</code> if this frame
     *         doesn't have a menu bar.
     * @see #setMenuBar(MenuBar)
     */
     ...
     * 
     * @param mb
     *           the menu bar being set. If this parameter is <code>null</code>
    ...
    /**
     * Indicates whether this frame is resizable by the user. By default, all
     * frames are initially resizable.
     * 
     ...
     * <p>
     * In older versions of JDK a frame state could only be NORMAL or ICONIFIED.
     * Since JDK 1.4 set of supported frame states is expanded and frame state
     * is represented as a bitwise mask.
     * <p>
     * For compatibility with applications developed earlier this method still
     * accepts {@code Frame.NORMAL} and {@code Frame.ICONIFIED} only. The iconic
     * state of the frame is only changed, other aspects of frame state are not
     * affected by this method. If the state passed to this method is neither
     * {@code
     * Frame.NORMAL} nor {@code Frame.ICONIFIED} the method performs no actions
     * at all.
     * <p>
     * Note that if the state is not supported on a given platform, neither the
     * state nor the return value of the {@link #getState} method will be
     * changed. The application may determine whether a specific state is
     * supported via the {@link java.awt.Toolkit#isFrameStateSupported} method.
     * <p>
     * <b>If the frame is currently visible on the screen</b> (the
     * {@link #isShowing} method returns {@code true}), the developer should
     * examine the return value of the
     * {@link java.awt.event.WindowEvent#getNewState} method of the
     * {@code WindowEvent} received through the
     * {@link java.awt.event.WindowStateListener} to determine that the state
     * has actually been changed.
     * <p>
     * <b>If the frame is not visible on the screen</b>, the events may or may
     * not be generated. In this case the developer may assume that the state
     * changes immediately after this method returns. Later, when the {@code
     * setVisible(true)} method is invoked, the frame will attempt to apply this
     * state. Receiving any
     * {@link java.awt.event.WindowEvent#WINDOW_STATE_CHANGED} events is not
     * guaranteed in this case also.
     *
     * @param state
     *              either <code>Frame.NORMAL</code> or
     ...
     * <p>
     * Note that if the state is not supported on a given platform, neither the
     * state nor the return value of the {@link #getExtendedState} method will
     * be changed. The application may determine whether a specific state is
     * supported via the {@link java.awt.Toolkit#isFrameStateSupported} method.
     * <p>
     * <b>If the frame is currently visible on the screen</b> (the
     * {@link #isShowing} method returns {@code true}), the developer should
    ...
    private boolean isFrameStateSupported(int state) {
        if (!getToolkit().isFrameStateSupported(state)) {
            ...
            if (((state & ICONIFIED) != 0) && !getToolkit()
                    .isFrameStateSupported(ICONIFIED)) {
                return false;
            } else {
                state &= ~ICONIFIED;
            }
     ...
     * <code>Frame.NORMAL</code> and <code>Frame.ICONIFIED</code> but it only
     * reports the iconic state of the frame, other aspects of frame state are
     ...
     * <p>
     * When a frame is in maximized state the system supplies some defaults
     * bounds. This method allows some or all of those system supplied values to
     * be overridden.
     * <p>
     * If <code>bounds</code> is <code>null</code>, accept bounds supplied by
     * the system. If non-<code>null</code> you can override some of the system
     * supplied values while accepting others by setting those fields you want
     * to accept from system to <code>Integer.MAX_VALUE</code>.
     * <p>
     ...
     * <code>Integer.MAX_VALUE</code> to indicate that system supplied values
     * for this field must be used.
     *
     ...
     *
     * @param undecorated
     *                    {@code true} if no frame decorations are to be
     *                    enabled;
     *                    {@code false} if frame decorations are to be enabled
     *
     * @throws IllegalComponentStateException
     *                                        if the frame is displayable
     * @throws IllegalComponentStateException
     *                                        if {@code undecorated} is
     *                                        {@code false}, and this frame does
     *                                        not have the default shape
     * @throws IllegalComponentStateException
     *                                        if {@code undecorated} is
     *                                        {@code false}, and this frame
     *                                        opacity is less than {@code 1.0f}
     * @throws IllegalComponentStateException
     *                                        if {@code undecorated} is
     *                                        {@code false}, and the alpha value
     *                                        of this frame background color is
     *                                        less than {@code 1.0f}
     *
     * @see #isUndecorated
     * @see Component#isDisplayable
     ...
     * initially decorated.
     * 
     * @return <code>true</code> if frame is undecorated; <code>false</code>
     *         otherwise.
     ...
     * 
     * @param m
     *          the menu component to remove. If <code>m</code> is
     ...
     * screen resource. Making a Frame undisplayable will cause any of its
     * children to be made undisplayable. This method is called by the toolkit
     ...
     *             <code>Component.setCursor(Cursor)</code>.
     */
    @Deprecated
    ...
    /**
     * Returns an array of all {@code Frame}s created by this application. If
     * called from an applet, the array includes only the {@code Frame}s
     * accessible by that applet.
     * <p>
     * <b>Warning:</b> this method may return system created frames, such as a
     * shared, hidden frame which is used by Swing. Applications should not
     * assume the existence of these frames, nor should an application assume
     * anything about these frames such as component positions,
     * <code>LayoutManager</code>s or serialization.
     * <p>
     * <b>Note</b>: To obtain a list of all ownerless windows, including
     * ownerless {@code Dialog}s (introduced in release 1.6), use
     * {@link Window#getOwnerlessWindows Window.getOwnerlessWindows}.
     *
    ...
    /*
     * Serialization support. If there's a MenuBar we restore its (transient)
     * parent field here. Likewise for top level windows that are "owned" by
     * this frame.
     */
    ...
    /**
     * Writes default serializable fields to stream. Writes an optional
     * serializable icon <code>Image</code>, which is available as of 1.4.
     *
     * @param s
    ...
    /**
     * Reads the <code>ObjectInputStream</code>. Tries to read an icon
     * <code>Image</code>, which is optional data available as of 1.4. If an
     * icon <code>Image</code> is not available, but anything other than an EOF
     * is detected, an <code>OptionalDataException</code> will be thrown.
     * Unrecognized keys or values will be ignored.
     *
     * @param s
     *          the <code>ObjectInputStream</code> to read
     * @exception                   java.io.OptionalDataException
     *                              if an icon <code>Image</code> is not
     *                              available, but
     *                              anything other than an EOF is detected
     * @exception HeadlessException
     *                              if
     *                              <code>GraphicsEnvironment.isHeadless</code>
     *                              returns
     ...
     *
     * @return an AccessibleAWTFrame that serves as the AccessibleContext of
     *         this Frame
     * @since 1.3
     */
    ...
    /**
     * This class implements accessibility support for the <code>Frame</code>
     * class. It provides an implementation of the Java Accessibility API
     * appropriate to frame user-interface elements.
     * 
     * @since 1.3
     */
