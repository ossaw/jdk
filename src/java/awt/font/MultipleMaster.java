...
/**
 * The <code>MultipleMaster</code> interface represents Type 1 Multiple Master
 * fonts. A particular {@link Font} object can implement this interface.
 */
public interface MultipleMaster {

	/**
	 * Returns the number of multiple master design controls. Design axes
	 * include things like width, weight and optical scaling.
	 * 
	 ...
	 * for each axis. For example, design limits for weight could be from 0.1 to
	 * 1.0. The values are returned in the same order returned by
	 ...
	 * default value for weight could be 1.6. The values are returned in the
	 * same order returned by <code>getDesignAxisNames</code>.
	 * 
	 ...
	 * values contained in the specified array. The size of the array must
	 * correspond to the value returned from <code>getNumDesignAxes</code> and
	 * the values of the array elements must fall within limits specified by
	 * <code>getDesignAxesLimits</code>. In case of an error, <code>null</code>
	 * is returned.
	 * 
	 * @param axes
	 ...
	 * 
	 * @param glyphWidths
	 *            an array of floats representing the desired width of each
	 *            glyph in font space
	 * @param avgStemWidth
