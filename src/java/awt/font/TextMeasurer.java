...
/**
 * The <code>TextMeasurer</code> class provides the primitive operations needed
 * for line break: measuring up to a given advance, determining the advance of a
 * range of characters, and generating a <code>TextLayout</code> for a range of
 * characters. It also provides methods for incremental editing of paragraphs.
 * <p>
 ...
 * {@link AttributedCharacterIterator#getBeginIndex() getBeginIndex} method of
 * <code>AttributedCharacterIterator</code> defines the absolute index of the
 * first character. The value returned by the
 * {@link AttributedCharacterIterator#getEndIndex() getEndIndex} method of
 * <code>AttributedCharacterIterator</code> defines the index past the last
 * character. These values define the range of indexes to use in calls to the
 * <code>TextMeasurer</code>. For example, calls to get the advance of a range
 * of text or the line break of a range of text must use indexes between the
 * beginning and end index values. Calls to
 * {@link #insertChar(java.text.AttributedCharacterIterator, int) insertChar}
 * and {@link #deleteChar(java.text.AttributedCharacterIterator, int)
 * deleteChar} reset the <code>TextMeasurer</code> to use the beginning index
 * and end index of the <code>AttributedCharacterIterator</code> passed in those
 * calls.
 * <p>
 * Most clients will use the more convenient <code>LineBreakMeasurer</code>,
 * which implements the standard line break policy (placing as many words as
...
public final class TextMeasurer implements Cloneable {
    ...
    /*
     * static { String s = System.getProperty("estLines"); if (s != null) { try
     * { Float f = new Float(s); EST_LINES = f.floatValue(); }
    ...
    /**
     * Constructs a <code>TextMeasurer</code> from the source text. The source
     * text should be a single entire paragraph.
     * 
     * @param text
     *             the source paragraph. Cannot be null.
     * @param frc
     *             the information about a graphics device which is needed to
     *             measure the text correctly. Cannot be null.
     */
    ...
    private void initAll(AttributedCharacterIterator text) {
        ...
        // set paragraph attributes
        {
            ...
            if (haveFont) {
                ...
                LineMetrics lm = defaultFont.getLineMetrics(fChars, charsStart,
                        charsStart + 1, fFrc);
            ...
            fBaselineOffsets = TextLine.getNormalizedOffsets(fBaselineOffsets,
                    fBaseline);
        }
    ...
    private void generateComponents(int startingAt, int endingAt) {
        ...
        try {
            fComponents = TextLine.getComponents(fParagraph, fChars, startingAt,
                    endingAt, charsLtoV, fLevels, factory);
        } catch (IllegalArgumentException e) {
            System.out.println("startingAt=" + startingAt + "; endingAt="
                    + endingAt);
    ...
    /**
     * According to the Unicode Bidirectional Behavior specification (Unicode
     * Standard 2.0, section 3.11), whitespace at the ends of lines which would
     * naturally flow against the base direction must be made to flow with the
     * line direction, and moved to the end of the line. This method returns the
     * start of the sequence of trailing whitespace characters to move to the
     * end of a line taken from the given range.
     */
    private int trailingCdWhitespaceStart(int startPos, int limitPos) {
        ...
        if (fLevels != null) {
            ...
            for (int cdWsStart = limitPos; --cdWsStart >= startPos;) {
                if ((fLevels[cdWsStart] % 2) == baseLevel || Character
                        .getDirectionality(
                                fChars[cdWsStart]) != Character.DIRECTIONALITY_WHITESPACE) {
                    return ++cdWsStart;
                }
            }
        }
    ...
    private TextLineComponent[] makeComponentsOnRange(int startPos,
        ...
        while (linePos < limitPos) {
            ...
            components[newCompIndex++] = fComponents[tlcIndex].getSubset(start
                    - tlcStart, limit - tlcStart, subsetFlag);
            linePos += (limit - start);
            if (linePos == breakPt) {
                breakPt = limitPos;
                subsetFlag = fIsDirectionLTR ? TextLineComponent.LEFT_TO_RIGHT
    ...
    private TextLine makeTextLineOnRange(int startPos, int limitPos) {
        ...
        TextLineComponent[] components = makeComponentsOnRange(startPos,
                limitPos);

        return new TextLine(fFrc, components, fBaselineOffsets, fChars,
                startPos, limitPos, charsLtoV, charLevels, fIsDirectionLTR);
    ...
    private void makeLayoutWindow(int localStart) {
        ...
        // If we've already gone past the layout window, format to end of
        // paragraph
        if (layoutCount > 0 && !haveLayoutWindow) {
            float avgLineLength = Math.max(layoutCharCount / layoutCount, 1);
            compLimit = Math.min(localStart + (int) (avgLineLength * EST_LINES),
                    fChars.length);
        }
     ...
     * beginning at <code>start</code> and possible measuring up to
     * <code>maxAdvance</code> in graphical width.
     *
     * @param start
     *                   the character index at which to start measuring.
     *                   <code>start</code> is an absolute index, not relative
     *                   to the
     *                   start of the paragraph
     * @param maxAdvance
     *                   the graphical width in which the line must fit
     * @return the index after the last character that will fit on a line
     *         beginning at <code>start</code>, which is not longer than
     *         <code>maxAdvance</code> in graphical width
     * @throws IllegalArgumentException
     *                                  if <code>start</code> is less than the
     *                                  beginning of the
     *                                  paragraph.
     */
    public int getLineBreakIndex(int start, float maxAdvance) {
        ...
        if (!haveLayoutWindow || localStart < fComponentStart
    ...
    /**
     * Returns the graphical width of a line beginning at <code>start</code> and
     * including characters up to <code>limit</code>. <code>start</code> and
     * <code>limit</code> are absolute indices, not relative to the start of the
     * paragraph.
     *
     * @param start
     *              the character index at which to start measuring
     ...
     * @return the graphical width of a line beginning at <code>start</code> and
     *         including characters up to <code>limit</code>
     * @throws IndexOutOfBoundsException
     *                                   if <code>limit</code> is less than
     *                                   <code>start</code>
     * @throws IllegalArgumentException
     *                                   if <code>start</code> or
     *                                   <code>limit</code> is not between
     ...
     * @param limit
     *              the index after the last character. Must be greater than
     *              <code>start</code>
    ...
    public TextLayout getLayout(int start, int limit) {
        ...
        return new TextLayout(textLine, fBaseline, fBaselineOffsets,
                fJustifyRatio);
    }
    ...
    /**
     * Updates the <code>TextMeasurer</code> after a single character has been
     * inserted into the paragraph currently represented by this
     * <code>TextMeasurer</code>. After this call, this
     * <code>TextMeasurer</code> is equivalent to a new
     * <code>TextMeasurer</code> created from the text; however, it will usually
     * be more efficient to update an existing <code>TextMeasurer</code> than to
     * create a new one from scratch.
     *
     * @param newParagraph
     *                     the text of the paragraph after performing the
     *                     insertion.
     *                     Cannot be null.
     * @param insertPos
     *                     the position in the text where the character was
     *                     inserted.
     *                     Must not be less than the start of
    ...
    public void insertChar(AttributedCharacterIterator newParagraph,
        ...
        System.arraycopy(fChars, newCharIndex, newChars, newCharIndex + 1, end
                - insertPos - 1);
        fChars = newChars;

        if (fBidi != null || Bidi.requiresBidi(newChars, newCharIndex,
                newCharIndex + 1) || newParagraph.getAttribute(
                        TextAttribute.BIDI_EMBEDDING) != null) {
        ...
        fParagraph = StyledParagraph.insertChar(newParagraph, fChars, insertPos,
                fParagraph);
        invalidateComponents();
    }
    ...
    /**
     * Updates the <code>TextMeasurer</code> after a single character has been
     * deleted from the paragraph currently represented by this
     * <code>TextMeasurer</code>. After this call, this
     * <code>TextMeasurer</code> is equivalent to a new
     * <code>TextMeasurer</code> created from the text; however, it will usually
     * be more efficient to update an existing <code>TextMeasurer</code> than to
     ...
     *                     the position in the text where the character was
     *                     removed. Must
     *                     not be less than the start of
     *                     <code>newParagraph</code>, and
     *                     must not be greater than the end of
     *                     <code>newParagraph</code>.
     * @throws IndexOutOfBoundsException
    ...
    public void deleteChar(AttributedCharacterIterator newParagraph,
        ...
        System.arraycopy(fChars, changedIndex + 1, newChars, changedIndex, end
                - deletePos);
        fChars = newChars;
        ...
        fParagraph = StyledParagraph.deleteChar(newParagraph, fChars, deletePos,
                fParagraph);
        invalidateComponents();
    }
