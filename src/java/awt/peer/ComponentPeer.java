...
public interface ComponentPeer {
    ...
    /**
     * Operation for {@link #setBounds(int, int, int, int, int)}, indicating a
     * change in the component location only.
     *
    ...
    /**
     * Operation for {@link #setBounds(int, int, int, int, int)}, indicating a
     * change in the component size only.
     *
     * @see #setBounds(int, int, int, int, int)
     */
    ...
    /**
     * Operation for {@link #setBounds(int, int, int, int, int)}, indicating a
     * change in the component size and location.
     *
     * @see #setBounds(int, int, int, int, int)
     */
    ...
    /**
     * Operation for {@link #setBounds(int, int, int, int, int)}, indicating a
     * change in the component client size. This is used for setting the
     * 'inside' size of windows, without the border insets.
     *
     * @see #setBounds(int, int, int, int, int)
     */
    ...
    /**
     * Resets the setBounds() operation to DEFAULT_OPERATION. This is not passed
     * into {@link #setBounds(int, int, int, int, int)}.
     ...
     *
     * @return {@code true} when the component has been obscured, {@code false}
     *         otherwise
    ...
    /**
     * Returns {@code true} when the peer can determine if a component has been
     * obscured, {@code false} false otherwise.
     *
     * @return {@code true} when the peer can determine if a component has been
     *         obscured, {@code false} false otherwise
     * @see #isObscured()
     * @see javax.swing.JViewport#needsRepaintAfterBlit
     */
     ...
     * specified relative to the component's parent. The {@code op} parameter
     * specifies which properties change. If it is {@link #SET_LOCATION}, then
     * only the location changes (and the size parameters can be ignored). If
     * {@code op} is {@link #SET_SIZE}, then only the size changes (and the
     * location can be ignored). If {@code op} is {@link #SET_BOUNDS}, then both
     * change. There is a special value {@link #SET_CLIENT_SIZE}, which is used
     * only for window-like components to set the size of the client (i.e. the
    ...
    /**
     * Disposes all resources held by the component peer. This is called when
     * the component has been disconnected from the component hierarchy and is
     ...
     */
    boolean requestFocus(Component lightweightChild, boolean temporary, boolean focusedWindowChangeAllowed,
    ...
    /**
     * Creates an empty volatile image with the specified width and height. This
     * is generally used as an accelerated backbuffer for drawing onto the
     * component (e.g. by Swing).
     *
     * @param width
     *        the width of the image
     ...
     * start loading the image (if not already loaded) and create an appropriate
    ...
    /**
     * Determines the status of the construction of the screen representaion of
     * the specified image.
     *
     * @param img
     *        the image to check
    ...
    /**
     * Determines if the component handles wheel scrolling itself. Otherwise it
     * is delegated to the component's parent.
     *
    ...
    /**
     * Create {@code numBuffers} flipping buffers with the specified buffer
     * capabilities.
     *
     * @param numBuffers
    ...
    /**
     * Reparents this peer to the new parent referenced by {@code newContainer}
     ...
     * sub-elements. For instance, a lightweight Checkbox needs to layout the
     * box, as well as the text label.
     *
