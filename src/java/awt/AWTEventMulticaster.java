 ...
 * add} and {@code remove} methods is the field maintaining the listeners. In
 * addition you must assign the result of the {@code add} and {@code remove}
 ...
 * remove} methods do not alter {@code AWTEventMulticaster} in anyway. If
 * necessary, a new {@code AWTEventMulticaster} is created. In this way it is
 * safe to add and remove listeners during the process of an event dispatching.
 * However, event listeners added during the process of an event dispatch
 * operation are not notified of the event currently being dispatched.
 * <p>
 * All of the {@code add} methods allow {@code null} arguments. If the first
 * argument is {@code null}, the second argument is returned. If the first
 * argument is not {@code null} and the second argument is {@code null}, the
 * first argument is returned. If both arguments are {@code non-null}, a new
 * {@code AWTEventMulticaster} is created using the two arguments and returned.
 * <p>
 * For the {@code remove} methods that take two arguments, the following is
 * returned:
 * <ul>
 * <li>{@code null}, if the first argument is {@code null}, or the arguments are
 * equal, by way of {@code ==}.
 * <li>the first argument, if the first argument is not an instance of
 * {@code AWTEventMulticaster}.
 * <li>result of invoking {@code remove(EventListener)} on the first argument,
 * supplying the second argument to the {@code remove(EventListener)} method.
 * </ul>
 * <p>
 * Swing makes use of {@link javax.swing.event.EventListenerList
 * EventListenerList} for similar logic. Refer to it for details.
 *
...
public class AWTEventMulticaster implements ComponentListener, ContainerListener, FocusListener, KeyListener,
        MouseListener, MouseMotionListener, WindowListener, WindowFocusListener, WindowStateListener,
        ActionListener, ItemListener, AdjustmentListener, TextListener, InputMethodListener,
        HierarchyListener, HierarchyBoundsListener, MouseWheelListener {

    protected final EventListener a, b;

    /**
     * Creates an event multicaster instance which chains listener-a with
     * listener-b. Input parameters <code>a</code> and <code>b</code> should not
     * be <code>null</code>, though implementations may vary in choosing whether
     ...
     * <p>
     * The returned multicaster contains all the listeners in this multicaster
     * with the exception of all occurrences of {@code oldl}. If the resulting
     * multicaster contains only one regular listener the regular listener may
     * be returned. If the resulting multicaster is empty, then {@code null} may
     * be returned instead.
     * <p>
    ...
    /**
     * Handles the componentAdded container event by invoking the componentAdded
     * methods on listener-a and listener-b.
     * 
     * @param e
     *        the component event
    ...
    /**
     * Handles the focusGained event by invoking the focusGained methods on
    ...
    /**
     * Handles the focusLost event by invoking the focusLost methods on
     * listener-a and listener-b.
     * 
     * @param e
     *        the focus event
    ...
    /**
     * Handles the keyTyped event by invoking the keyTyped methods on listener-a
     * and listener-b.
     * 
     * @param e
     *        the key event
    ...
    /**
     * Handles the keyPressed event by invoking the keyPressed methods on
     * listener-a and listener-b.
     * 
     * @param e
     *        the key event
    ...
    /**
     * Handles the keyReleased event by invoking the keyReleased methods on
     * listener-a and listener-b.
     * 
     * @param e
     *        the key event
    ...
    /**
     * Handles the mouseClicked event by invoking the mouseClicked methods on
     * listener-a and listener-b.
     * 
     * @param e
     *        the mouse event
    ...
    /**
     * Handles the mousePressed event by invoking the mousePressed methods on
     * listener-a and listener-b.
     * 
     * @param e
     *        the mouse event
    ...
    /**
     * Handles the mouseReleased event by invoking the mouseReleased methods on
     * listener-a and listener-b.
     * 
     * @param e
     *        the mouse event
    ...
    /**
     * Handles the mouseEntered event by invoking the mouseEntered methods on
     * listener-a and listener-b.
     * 
     * @param e
     *        the mouse event
    ...
    /**
     * Handles the mouseExited event by invoking the mouseExited methods on
     * listener-a and listener-b.
     * 
     * @param e
     *        the mouse event
    ...
    /**
     * Handles the mouseDragged event by invoking the mouseDragged methods on
     * listener-a and listener-b.
     * 
     * @param e
     *        the mouse event
    ...
    /**
     * Handles the mouseMoved event by invoking the mouseMoved methods on
     * listener-a and listener-b.
     * 
     * @param e
     *        the mouse event
    ...
    /**
     * Handles the windowDeiconfied event by invoking the windowDeiconified
     * methods on listener-a and listener-b.
     * 
     * @param e
     *        the window event
    ...
    /**
     * Handles the windowStateChanged event by invoking the windowStateChanged
     * methods on listener-a and listener-b.
     * 
     * @param e
     *        the window event
    ...
    /**
     * Handles the windowLostFocus event by invoking the windowLostFocus methods
     * on listener-a and listener-b.
     * 
     * @param e
     *        the window event
    ...
    /**
     * Handles the actionPerformed event by invoking the actionPerformed methods
     * on listener-a and listener-b.
     * 
     * @param e
     *        the action event
    ...
    /**
     * Handles the itemStateChanged event by invoking the itemStateChanged
     * methods on listener-a and listener-b.
     * 
     * @param e
     *        the item event
    ...
    /**
     * Handles the hierarchyChanged event by invoking the hierarchyChanged
     * methods on listener-a and listener-b.
     * 
     * @param e
     *        the item event
    ...
    /**
     * Handles the ancestorMoved event by invoking the ancestorMoved methods on
     * listener-a and listener-b.
     * 
     * @param e
     *        the item event
    ...
    /**
     * Handles the ancestorResized event by invoking the ancestorResized methods
     * on listener-a and listener-b.
     * 
     * @param e
     *        the item event
    ...
    /**
     * Handles the mouseWheelMoved event by invoking the mouseWheelMoved methods
     * on listener-a and listener-b.
     * 
     * @param e
     *        the mouse event
    ...
    /**
     * Adds component-listener-a with component-listener-b and returns the
    ...
    /**
     * Adds container-listener-a with container-listener-b and returns the
     * resulting multicast listener.
     * 
     * @param a
    ...
    /**
     * Adds focus-listener-a with focus-listener-b and returns the resulting
     * multicast listener.
     * 
     * @param a
     *        focus-listener-a
    ...
    /**
     * Adds key-listener-a with key-listener-b and returns the resulting
     * multicast listener.
     * 
     * @param a
     *        key-listener-a
    ...
    /**
     * Adds mouse-listener-a with mouse-listener-b and returns the resulting
     * multicast listener.
     * 
     * @param a
     *        mouse-listener-a
    ...
    /**
     * Adds mouse-motion-listener-a with mouse-motion-listener-b and returns the
     * resulting multicast listener.
     * 
     * @param a
    ...
    /**
     * Adds window-listener-a with window-listener-b and returns the resulting
     * multicast listener.
     * 
     * @param a
     *        window-listener-a
    ...
    /**
     * Adds window-state-listener-a with window-state-listener-b and returns the
     * resulting multicast listener.
     * 
     * @param a
    ...
    /**
     * Adds window-focus-listener-a with window-focus-listener-b and returns the
     * resulting multicast listener.
     * 
     * @param a
    ...
    /**
     * Adds action-listener-a with action-listener-b and returns the resulting
     * multicast listener.
     * 
     * @param a
     *        action-listener-a
    ...
    /**
     * Adds item-listener-a with item-listener-b and returns the resulting
     * multicast listener.
     * 
     * @param a
     *        item-listener-a
    ...
    /**
     * Adds adjustment-listener-a with adjustment-listener-b and returns the
     * resulting multicast listener.
     * 
     * @param a
    ...
    /**
     * Adds input-method-listener-a with input-method-listener-b and returns the
     * resulting multicast listener.
     * 
     * @param a
    ...
    /**
     * Adds hierarchy-listener-a with hierarchy-listener-b and returns the
     * resulting multicast listener.
     * 
     * @param a
    ...
    /**
     * Adds mouse-wheel-listener-a with mouse-wheel-listener-b and returns the
     * resulting multicast listener.
     * 
     * @param a
    ...
    /**
     * Removes the old component-listener from component-listener-l and returns
    ...
    /**
     * Removes the old container-listener from container-listener-l and returns
     * the resulting multicast listener.
     * 
     * @param l
    ...
    /**
     * Removes the old focus-listener from focus-listener-l and returns the
     * resulting multicast listener.
     * 
     * @param l
     *        focus-listener-l
    ...
    /**
     * Removes the old key-listener from key-listener-l and returns the
     * resulting multicast listener.
     * 
     * @param l
     *        key-listener-l
    ...
    /**
     * Removes the old mouse-listener from mouse-listener-l and returns the
     * resulting multicast listener.
     * 
     * @param l
     *        mouse-listener-l
    ...
    /**
     * Removes the old mouse-motion-listener from mouse-motion-listener-l and
     * returns the resulting multicast listener.
     * 
     * @param l
    ...
    /**
     * Removes the old window-listener from window-listener-l and returns the
     * resulting multicast listener.
     * 
     * @param l
     *        window-listener-l
    ...
    /**
     * Removes the old window-state-listener from window-state-listener-l and
     * returns the resulting multicast listener.
     * 
     * @param l
    ...
    /**
     * Removes the old window-focus-listener from window-focus-listener-l and
     * returns the resulting multicast listener.
     * 
     * @param l
    ...
    /**
     * Removes the old action-listener from action-listener-l and returns the
     * resulting multicast listener.
     * 
     * @param l
     *        action-listener-l
    ...
    /**
     * Removes the old item-listener from item-listener-l and returns the
     * resulting multicast listener.
     * 
     * @param l
     *        item-listener-l
    ...
    /**
     * Removes the old hierarchy-listener from hierarchy-listener-l and returns
     * the resulting multicast listener.
     * 
     * @param l
    ...
    /**
     * Removes the old mouse-wheel-listener from mouse-wheel-listener-l and
     * returns the resulting multicast listener.
     * 
     * @param l
     ...
     * from l. l is usually an AWTEventMulticaster. Bug 4513402 revealed that if
     * l differed in type from the element type of a, an ArrayStoreException
    ...
    /**
     * Returns an array of all the objects chained as <code><em>Foo</em>
     * Listener</code>s by the specified <code>java.util.EventListener</code>.
     * <code><em>Foo</em>Listener</code>s are chained by the
     * <code>AWTEventMulticaster</code> using the <code>add<em>Foo</em>
     * Listener</code> method. If a <code>null</code> listener is specified,
     * this method returns an empty array. If the specified listener is not an
     * instance of <code>AWTEventMulticaster</code>, this method returns an
     * array which contains only the specified listener. If no such listeners
