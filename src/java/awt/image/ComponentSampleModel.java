 ...
 * bands are numbered from 0 to N-1. This class can represent image data for
 * which each sample is an unsigned integral number which can be stored in 8,
 * 16, or 32 bits (using <code>DataBuffer.TYPE_BYTE</code>,
 * <code>DataBuffer.TYPE_USHORT</code>, or <code>DataBuffer.TYPE_INT</code>,
 * respectively), data for which each sample is a signed integral number which
 * can be stored in 16 bits (using <code>DataBuffer.TYPE_SHORT</code>), or data
 * for which each sample is a signed float or double quantity (using
 * <code>DataBuffer.TYPE_FLOAT</code> or <code>DataBuffer.TYPE_DOUBLE</code>,
 * respectively). All samples of a given ComponentSampleModel are stored with
 * the same precision. All strides and offsets must be non-negative. This class
 * supports {@link DataBuffer#TYPE_BYTE TYPE_BYTE},
 * {@link DataBuffer#TYPE_USHORT TYPE_USHORT}, {@link DataBuffer#TYPE_SHORT
 * TYPE_SHORT}, {@link DataBuffer#TYPE_INT TYPE_INT},
 * {@link DataBuffer#TYPE_FLOAT TYPE_FLOAT}, {@link DataBuffer#TYPE_DOUBLE
 * TYPE_DOUBLE},
 * 
...
public class ComponentSampleModel extends SampleModel {
	...
	/**
	 * The number of bands in this <code>ComponentSampleModel</code>.
	 */
	protected int numBands = 1;

	/**
	 * The number of banks in this <code>ComponentSampleModel</code>.
	 */
	protected int numBanks = 1;

	/**
	 * Line stride (in data array elements) of the region of image data
	 * described by this ComponentSampleModel.
	 */
	protected int scanlineStride;

	/**
	 * Pixel stride (in data array elements) of the region of image data
	 * described by this ComponentSampleModel.
	 */
	 ...
	 *                       described
	 * @param pixelStride
	 *                       the pixel stride of the region of image data
	 *                       described
	 * @param scanlineStride
	 *                       the line stride of the region of image data
	 *                       described
	 * @param bandOffsets
	 ...
	 *                                  0
	 * @throws IllegalArgumentException
	 *                                  if <code>scanlineStride</code> is less
	 *                                  than 0
	 * @throws IllegalArgumentException
	 *                                  if <code>numBands</code> is less than 1
	 * @throws IllegalArgumentException
	 ...
	 * @throws IllegalArgumentException
	 *                                  if the length of
	 *                                  <code>bankIndices</code> does not equal
	 *                                  the
	 *                                  length of <code>bankOffsets</code>
	 * @throws IllegalArgumentException
	 *                                  if any of the bank indices of
	 *                                  <code>bandIndices</code> is
	 *                                  less than 0
	 * @throws IllegalArgumentException
	 *                                  if <code>dataType</code> is not one of
	 *                                  the supported data
	...
	private int getBufferSize() {
		...
		if (scanlineStride < 0 || scanlineStride > (Integer.MAX_VALUE
				/ height)) {
	...
	/**
	 * Creates a new <code>ComponentSampleModel</code> with the specified width
	 * and height. The new <code>SampleModel</code> will have the same number of
	 * bands, storage data type, interleaving scheme, and pixel stride as this
	...
	public SampleModel createCompatibleSampleModel(int w, int h) {
		...
		for (int i = 0; i < bands; i++)
			bandOff[i] += base;
		return new ComponentSampleModel(dataType, w, h, pStride, lStride,
				bankIndices, bandOff);
	}

	/**
	 * Creates a new ComponentSampleModel with a subset of the bands of this
	 * ComponentSampleModel. The new ComponentSampleModel can be used with any
	 * DataBuffer that the existing ComponentSampleModel can be used with. The
	 * new ComponentSampleModel/DataBuffer combination will represent an image
	 * with a subset of the bands of the original
	 * ComponentSampleModel/DataBuffer combination.
	 * 
	 * @param bands
	 *              a subset of bands from this
	 *              <code>ComponentSampleModel</code>
	 * @return a <code>ComponentSampleModel</code> created with a subset of
	 *         bands from this <code>ComponentSampleModel</code>.
	 */
	public SampleModel createSubsetSampleModel(int bands[]) {
		if (bands.length > bankIndices.length)
			throw new RasterFormatException("There are only "
					+ bankIndices.length + " bands");
		...
		return new ComponentSampleModel(this.dataType, width, height,
				this.pixelStride, this.scanlineStride, newBankIndices,
				newBandOffsets);
	}

	/**
	 * Creates a <code>DataBuffer</code> that corresponds to this
	 * <code>ComponentSampleModel</code>. The <code>DataBuffer</code> object's
	 * data type, number of banks, and size are be consistent with this
	 * <code>ComponentSampleModel</code>.
	 * 
	 * @return a <code>DataBuffer</code> whose data type, number of banks and
	 *         size are consistent with this <code>ComponentSampleModel</code>.
	 */
	...
	/**
	 * Gets the offset for the first band of pixel (x,y). A sample of the first
	 * band can be retrieved from a <code>DataBuffer</code> <code>data</code>
	...
	/**
	 * Gets the offset for band b of pixel (x,y). A sample of band
	 * <code>b</code> can be retrieved from a <code>DataBuffer</code>
	 ...
	 * 
	 * @return an array containing the number of bits per sample for all bands,
	 *         where each element in the array represents a band.
	 */
	 ...
	 * {@link #setDataElements(int, int, Object, DataBuffer) } methods. For a
	 * <code>ComponentSampleModel</code>, this is identical to the number of
	 * bands.
	 * 
	 * @return the number of data elements needed to transfer a pixel with the
	 ...
	 * <code>TransferType</code>. For a <code>ComponentSampleModel</code>, this
	 * is the same as the data type, and samples are returned one per array
	 * element. Generally, <code>obj</code> should be passed in as
	 * <code>null</code>, so that the <code>Object</code> is created
	 * automatically and is the right primitive data type.
	 * <p>
	 ...
	 * transfer is usually more efficient than using <code>getPixel</code> and
	 * <code>setPixel</code>.
	 * 
	 * <pre>
	 ...
	 * </pre>
	 *
	 * Using <code>getDataElements</code> and <code>setDataElements</code> to
	 * transfer between two <code>DataBuffer/SampleModel</code> pairs is
	 * legitimate if the <code>SampleModel</code> objects have the same number
	 * of bands, corresponding bands have the same number of bits per sample,
	 * and the <code>TransferType</code>s are the same.
	 * <p>
	 * If <code>obj</code> is not <code>null</code>, it should be a primitive
	 * array of type <code>TransferType</code>. Otherwise, a
	 ...
	 * @param obj
	 *             if non-<code>null</code>, a primitive array in which to
	 *             return
	 *             the pixel data
	 * @param data
	...
	public Object getDataElements(int x, int y, Object obj, DataBuffer data) {
		...
		switch (type) {
			...
			case DataBuffer.TYPE_FLOAT:
				...
				for (int i = 0; i < numDataElems; i++) {
					fdata[i] = data.getElemFloat(bankIndices[i], pixelOffset
							+ bandOffsets[i]);
				}
			...
			case DataBuffer.TYPE_DOUBLE:
				...
				for (int i = 0; i < numDataElems; i++) {
					ddata[i] = data.getElemDouble(bankIndices[i], pixelOffset
							+ bandOffsets[i]);
				}
	...
	/**
	 * Returns all samples for the specified pixel in an int array, one sample
	 * per array element. An <code>ArrayIndexOutOfBoundsException</code> might
	...
	/**
	 * Returns all samples for the specified rectangle of pixels in an int
	 * array, one sample per array element. An
	 * <code>ArrayIndexOutOfBoundsException</code> might be thrown if the
	...
	public int[] getPixels(int x, int y, int w, int h, int iArray[],
		...
		if (x < 0 || x >= width || w > width || x1 < 0 || x1 > width || y < 0
				|| y >= height || y > height || y1 < 0 || y1 > height) {
			throw new ArrayIndexOutOfBoundsException(
					"Coordinate out of bounds!");
		}
		int pixels[];
		...
		for (int i = 0; i < h; i++) {
			...
			for (int j = 0; j < w; j++) {
				for (int k = 0; k < numBands; k++) {
					pixels[srcOffset++] = data.getElem(bankIndices[k],
							pixelOffset + bandOffsets[k]);
				}
	...
	/**
	 * Returns as int the sample in a specified band for the pixel located at
	 * (x,y). An <code>ArrayIndexOutOfBoundsException</code> might be thrown if
	...
	public int getSample(int x, int y, int b, DataBuffer data) {
		...
		int sample = data.getElem(bankIndices[b], y * scanlineStride + x
				* pixelStride + bandOffsets[b]);
		return sample;
	}

	/**
	 * Returns the sample in a specified band for the pixel located at (x,y) as
	 * a float. An <code>ArrayIndexOutOfBoundsException</code> might be thrown
	 ...
	 * @param data
	 *             The DataBuffer containing the image data
	 * @return a float value representing the sample in the specified band for
	 *         the specified pixel.
	 */
	public float getSampleFloat(int x, int y, int b, DataBuffer data) {
		...
		float sample = data.getElemFloat(bankIndices[b], y * scanlineStride + x
				* pixelStride + bandOffsets[b]);
		return sample;
	}
	 ...
	 * double. An <code>ArrayIndexOutOfBoundsException</code> might be thrown if
	 * the coordinates are not in bounds.
	 * 
	 * @param x
	 *             The X coordinate of the pixel location
	 ...
	 * @param data
	 *             The DataBuffer containing the image data
	 * @return a double value representing the sample in the specified band for
	 *         the specified pixel.
	 */
	public double getSampleDouble(int x, int y, int b, DataBuffer data) {
		...
		double sample = data.getElemDouble(bankIndices[b], y * scanlineStride
				+ x * pixelStride + bandOffsets[b]);
		return sample;
	}

	/**
	 * Returns the samples in a specified band for the specified rectangle of
	 * pixels in an int array, one sample per data array element. An
	 * <code>ArrayIndexOutOfBoundsException</code> might be thrown if the
	 * coordinates are not in bounds.
	 * 
	 * @param x
	 ...
	 *               the band to return
	 * @param iArray
	 *               if non-<code>null</code>, returns the samples in this array
	 * @param data
	 *               the <code>DataBuffer</code> containing the image data
	...
	/**
	 * Sets the data for a single pixel in the specified <code>DataBuffer</code>
	 * from a primitive array of type <code>TransferType</code>. For a
	 * <code>ComponentSampleModel</code>, this is the same as the data type, and
	 ...
	 * and the <code>TransferType</code>s are the same.
	 * <p>
	 * A <code>ClassCastException</code> is thrown if <code>obj</code> is not a
	 * primitive array of type <code>TransferType</code>. An
	 * <code>ArrayIndexOutOfBoundsException</code> might be thrown if the
	 * coordinates are not in bounds, or if <code>obj</code> is not large enough
	...
	public void setDataElements(int x, int y, Object obj, DataBuffer data) {
		...
		switch (type) {
			...
			case DataBuffer.TYPE_FLOAT:
				...
				for (int i = 0; i < numDataElems; i++) {
					data.setElemFloat(bankIndices[i], pixelOffset
							+ bandOffsets[i], farray[i]);
				}
				break;
			...
			case DataBuffer.TYPE_DOUBLE:
				...
				for (int i = 0; i < numDataElems; i++) {
					data.setElemDouble(bankIndices[i], pixelOffset
							+ bandOffsets[i], darray[i]);
				}
				break;
	...
	/**
	 * Sets a pixel in the <code>DataBuffer</code> using an int array of samples
	 * for input. An <code>ArrayIndexOutOfBoundsException</code> might be thrown
	 * if the coordinates are not in bounds.
	 * 
	 * @param x
	 *               The X coordinate of the pixel location
	 ...
	 * one sample per array element. An
	 * <code>ArrayIndexOutOfBoundsException</code> might be thrown if the
	 * coordinates are not in bounds.
	 * 
	 * @param x
	 ...
	 * <code>DataBuffer</code> using an int for input. An
	 * <code>ArrayIndexOutOfBoundsException</code> might be thrown if the
	 * coordinates are not in bounds.
	 * 
	 * @param x
	...
	/**
	 * Sets a sample in the specified band for the pixel located at (x,y) in the
	 * <code>DataBuffer</code> using a float for input. An
	 * <code>ArrayIndexOutOfBoundsException</code> might be thrown if the
	 * coordinates are not in bounds.
	 * 
	 * @param x
	...
	public void setSample(int x, int y, int b, float s, DataBuffer data) {
		...
		data.setElemFloat(bankIndices[b], y * scanlineStride + x * pixelStride
				+ bandOffsets[b], s);
	}

	/**
	 * Sets a sample in the specified band for the pixel located at (x,y) in the
	 * <code>DataBuffer</code> using a double for input. An
	 * <code>ArrayIndexOutOfBoundsException</code> might be thrown if the
	 * coordinates are not in bounds.
	 * 
	 * @param x
	...
	public void setSample(int x, int y, int b, double s, DataBuffer data) {
		...
		data.setElemDouble(bankIndices[b], y * scanlineStride + x * pixelStride
				+ bandOffsets[b], s);
	}

	/**
	 * Sets the samples in the specified band for the specified rectangle of
	 * pixels from an int array containing one sample per data array element. An
	 * <code>ArrayIndexOutOfBoundsException</code> might be thrown if the
	 * coordinates are not in bounds.
	 * 
	 * @param x
	...
	public boolean equals(Object o) {
		...
		return this.width == that.width && this.height == that.height
				...
				&& this.dataType == that.dataType && Arrays.equals(
						this.bandOffsets, that.bandOffsets) && Arrays.equals(
								this.bankIndices, that.bankIndices)
				&& this.numBands == that.numBands
