...
public class BufferedImage extends java.awt.Image implements WritableRenderedImage, Transparency {
	 ...
	 * types. The <code>ColorSpace</code> for the image is the default sRGB
	 * space.
	 * 
	 * @param width
	...
	public BufferedImage(int width, int height, int imageType) {
		...
		case TYPE_INT_ARGB_PRE: {
			colorModel = new DirectColorModel(ColorSpace.getInstance(ColorSpace.CS_sRGB), 32,
					0x00ff0000, // Red
					0x0000ff00, // Green
					0x000000ff, // Blue
					0xff000000, // Alpha
		...
		case TYPE_3BYTE_BGR: {
			...
			raster = Raster.createInterleavedRaster(DataBuffer.TYPE_BYTE, width, height, width * 3,
					3, bOffs, null);
		}
			break;
		...
		case TYPE_4BYTE_ABGR: {
			...
			colorModel = new ComponentColorModel(cs, nBits, true, false, Transparency.TRANSLUCENT,
					DataBuffer.TYPE_BYTE);
			raster = Raster.createInterleavedRaster(DataBuffer.TYPE_BYTE, width, height, width * 4,
					4, bOffs, null);
		}
			break;
		...
		case TYPE_USHORT_565_RGB: {
			colorModel = new DirectColorModel(16, DCM_565_RED_MASK, DCM_565_GRN_MASK,
					DCM_565_BLU_MASK);
			raster = colorModel.createCompatibleWritableRaster(width, height);
		}
			break;
		...
		case TYPE_USHORT_555_RGB: {
			colorModel = new DirectColorModel(15, DCM_555_RED_MASK, DCM_555_GRN_MASK,
					DCM_555_BLU_MASK);
			raster = colorModel.createCompatibleWritableRaster(width, height);
		}
			break;
	 ...
	 * per pixel. If the color model has 1 or 2 entries, the image will have 1
	 * bit per pixel. If it has 3 or 4 entries, the image with have 2 bits per
	 * pixel. If it has between 5 and 16 entries, the image will have 4 bits per
	 ...
	 *             or if the imageType is TYPE_BYTE_BINARY and the color map has
	...
	public BufferedImage(int width, int height, int imageType, IndexColorModel cm) {
		if (cm.hasAlpha() && cm.isAlphaPremultiplied()) {
			throw new IllegalArgumentException(
					"This image types do not have " + "premultiplied alpha.");
		}
		...
		default:
			throw new IllegalArgumentException(
					"Invalid image type (" + imageType + ").  Image type must"
	 ...
	 * <code>ColorModel</code> and <code>Raster</code>. If the number and types
	 * of bands in the <code>SampleModel</code> of the <code>Raster</code> do
	 * not match the number and types required by the <code>ColorModel</code> to
	 ...
	 * <code>Raster</code> data by alpha to match the
	 * <code>alphaPremultiplied</code> state in the <code>ColorModel</code>.
	 * Properties for this <code>BufferedImage</code> can be established by
	 * passing in a {@link Hashtable} of <code>String</code>/<code>Object</code>
	 * pairs.
	 * 
	 * @param cm
	 ...
	 *                <code>SampleModel</code> of the <code>Raster</code> do not
	 *                match the number and types required by the
	 *                <code>ColorModel</code> to represent its color and alpha
	 *                components.
	 * @exception IllegalArgumentException
	 *                if <code>raster</code> is incompatible with
	 *                <code>cm</code>
	 * @see ColorModel
	 * @see Raster
	 * @see WritableRaster
	...
	public BufferedImage(ColorModel cm, WritableRaster raster, boolean isRasterPremultiplied,
		...
		if (!cm.isCompatibleRaster(raster)) {
			throw new IllegalArgumentException(
					"Raster " + raster + " is incompatible with ColorModel " + cm);
		}

		if ((raster.minX != 0) || (raster.minY != 0)) {
			throw new IllegalArgumentException("Raster " + raster
					+ " has minX or minY not equal to zero: " + raster.minX + " " + raster.minY);
		}
		...
		if ((raster instanceof IntegerComponentRaster) && (numBands == 3 || numBands == 4)) {
		...
		else if ((cm instanceof IndexColorModel) && (numBands == 1) && isStandard
		...
		else if ((raster instanceof ShortComponentRaster) && (cm instanceof DirectColorModel)
				&& isStandard && (numBands == 3) && !cm.hasAlpha()) {
			DirectColorModel dcm = (DirectColorModel) cm;
			if (dcm.getRedMask() == DCM_565_RED_MASK) {
				...
				&& isStandard && (raster.getSampleModel() instanceof PixelInterleavedSampleModel)
			...
			if (ccm.getNumComponents() != numBands) {
				throw new RasterFormatException(
						"Number of components in " + "ColorModel (" + ccm.getNumComponents()
	...
	/**
	 * Returns the image type. If it is not one of the known types, TYPE_CUSTOM
	 * is returned.
	 * 
	 ...
	 * <code>BufferedImage</code> objects with <code>ColorModel</code> objects
	 * that support a separate spatial alpha channel, such as
	 * <code>ComponentColorModel</code> and <code>DirectColorModel</code>.
	 * Returns <code>null</code> if there is no alpha channel associated with
	 * the <code>ColorModel</code> in this image. This method assumes that for
	 * all <code>ColorModel</code> objects other than
	 * <code>IndexColorModel</code>, if the <code>ColorModel</code> supports
	 * alpha, there is a separate alpha channel which is stored as the last band
	 * of image data. If the image uses an <code>IndexColorModel</code> that has
	 * alpha in the lookup table, this method returns <code>null</code> since
	 * there is no spatially discrete alpha channel. This method creates a new
	 * <code>WritableRaster</code>, but shares the data array.
	 * 
	 * @return a <code>WritableRaster</code> or <code>null</code> if this
	 *         <code>BufferedImage</code> has no alpha channel associated with
	 *         its <code>ColorModel</code>.
	 */
	 ...
	 * and default sRGB colorspace. Color conversion takes place if this default
	...
	/**
	 * Sets a pixel in this <code>BufferedImage</code> to the specified RGB
	 ...
	 * convenient, since it is declared to return a <code>Graphics2D</code>.
	 * 
	 ...
	 * returned <code>BufferedImage</code> shares the same data array as the
	 * original image.
	 * 
	 * @param x
	 ...
	 * about its immediate sources. It returns an empty <code>Vector</code> if
	 * the <code>BufferedImage</code> has no immediate sources.
	 * 
	...
	/**
	 * Returns the y offset of the tile grid relative to the origin, For
	 * example, the y coordinate of the location of tile (0,&nbsp;0). This is
	 * always zero.
	 * 
	 ...
	 * array, not pixel locations. The <code>Raster</code> that is returned is
	 ...
	 *                if both <code>tileX</code> and <code>tileY</code> are not
	 *                equal to 0
	 */
	...
	/**
	 * Returns the image as one large tile. The <code>Raster</code> returned is
	 * a copy of the image data is not updated if the image is changed.
	 * 
	 ...
	 * <code>BufferedImage</code>. The <code>Raster</code> returned is a copy of
	 ...
	 * <code>WritableRaster</code>. The region to be computed is determined from
	 * the bounds of the specified <code>WritableRaster</code>. The specified
	 * <code>WritableRaster</code> must have a <code>SampleModel</code> that is
	 * compatible with this image. If <code>outRaster</code> is
	...
	/**
	 * Removes a tile observer. If the observer was not registered, nothing
	 * happens. If the observer was registered for multiple notifications, it is
	 ...
	 * checked out for writing. Returns <code>null</code> if none are checked
	 * out.
	 * 
	 * @return a <code>Point</code> array that indicates the tiles that are
	 *         checked out for writing, or <code>null</code> if no tiles are
	...
	/**
	 * Checks out a tile for writing. All registered <code>TileObservers</code>
	 * are notified when a tile goes from having no writers to having one
	 * writer.
	 * 
	 * @param tileX
	 *            the x index of the tile
	...
	/**
	 * Relinquishes permission to write to a tile. If the caller continues to
	 * write to the tile, the results are undefined. Calls to this method should
	 * only appear in matching pairs with calls to
	 * {@link #getWritableTile(int, int) getWritableTile(int, int)}. Any other
	 * leads to undefined results. All registered <code>TileObservers</code> are
