...
public abstract class PackedColorModel extends ColorModel {
    ...
    /**
     * Constructs a <code>PackedColorModel</code> from a color mask array, which
     * specifies which bits in an <code>int</code> pixel representation contain
     * each of the color samples, and an alpha mask. Color components are in the
     * specified <code>ColorSpace</code>. The length of
     * <code>colorMaskArray</code> should be the number of components in the
     * <code>ColorSpace</code>. All of the bits in each mask must be contiguous
     * and fit in the specified number of least significant bits of an
     * <code>int</code> pixel representation. If the <code>alphaMask</code> is
     * 0, there is no alpha. If there is alpha, the <code>boolean</code>
     * <code>isAlphaPremultiplied</code> specifies how to interpret color and
     * alpha samples in pixel values. If the <code>boolean</code> is
     * <code>true</code>, color samples are assumed to have been multiplied by
     * the alpha sample. The transparency, <code>trans</code>, specifies what
     * alpha values can be represented by this color model. The transfer type is
    ...
    public PackedColorModel(ColorSpace space, int bits, int[] colorMaskArray, int alphaMask,
        ...
        super(bits, PackedColorModel.createBitsArray(colorMaskArray, alphaMask), space, (alphaMask == 0
                ? false : true), isAlphaPremultiplied, trans, transferType);
        if (bits < 1 || bits > 32) {
    ...
    /**
     * Constructs a <code>PackedColorModel</code> from the specified masks which
     * indicate which bits in an <code>int</code> pixel representation contain
     * the alpha, red, green and blue color samples. Color components are in the
     * specified <code>ColorSpace</code>, which must be of type
     * ColorSpace.TYPE_RGB. All of the bits in each mask must be contiguous and
     * fit in the specified number of least significant bits of an
     * <code>int</code> pixel representation. If <code>amask</code> is 0, there
     * is no alpha. If there is alpha, the <code>boolean</code>
     * <code>isAlphaPremultiplied</code> specifies how to interpret color and
     * alpha samples in pixel values. If the <code>boolean</code> is
     * <code>true</code>, color samples are assumed to have been multiplied by
     * the alpha sample. The transparency, <code>trans</code>, specifies what
    ...
    public PackedColorModel(ColorSpace space, int bits, int rmask, int gmask, int bmask, int amask,
            boolean isAlphaPremultiplied, int trans, int transferType) {
        super(bits, PackedColorModel.createBitsArray(rmask, gmask, bmask, amask), space, (amask == 0 ? false
                : true), isAlphaPremultiplied, trans, transferType);
    ...
    /**
     * Returns the mask indicating which bits in a pixel contain the specified
     * color/alpha sample. For color samples, <code>index</code> corresponds to
     * the placement of color sample names in the color space. Thus, an
     * <code>index</code> equal to 0 for a CMYK ColorSpace would correspond to
     * Cyan and an <code>index</code> equal to 1 would correspond to Magenta. If
     * there is alpha, the alpha <code>index</code> would be:
     * 
     * <pre>
    ...
    /**
     * Returns a mask array indicating which bits in a pixel contain the color
     * and alpha samples.
     * 
     * @return the mask array , which indicates which bits of the
     *         <code>int</code> pixel representation contain the color or alpha
     *         samples.
     */
    ...
    /*
     * A utility function to compute the mask offset and scalefactor, store
     * these and the mask in instance arrays, and verify that the mask fits in
     * the specified pixel size.
     */
    private void DecomposeMask(int mask, int idx, String componentName) {
        ...
        if (off + count > pixel_bits) {
            throw new IllegalArgumentException(componentName + " mask " + Integer.toHexString(maskArray[idx])
    ...
    /**
     * Returns a {@link WritableRaster} representing the alpha channel of an
     * image, extracted from the input <code>WritableRaster</code>. This method
     * assumes that <code>WritableRaster</code> objects associated with this
     ...
     * spatial alpha channel associated with this <code>ColorModel</code>. This
     * method creates a new <code>WritableRaster</code>, but shares the data
     * array.
     * 
     * @param raster
     ...
     *         instance of <code>PackedColorModel</code> and equals this
    ...
    private final static int[] createBitsArray(int[] colorMaskArray, int alphaMask) {
        ...
        for (int i = 0; i < numColors; i++) {
            ...
            if (arr[i] < 0) {
                throw new IllegalArgumentException("Noncontiguous color mask (" + Integer.toHexString(
                        colorMaskArray[i]) + "at index " + i);
            }
        }
        if (alphaMask != 0) {
            arr[numColors] = countBits(alphaMask);
            if (arr[numColors] < 0) {
                throw new IllegalArgumentException("Noncontiguous alpha mask (" + Integer.toHexString(
                        alphaMask));
            }
        }
        return arr;
