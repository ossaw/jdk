 ...
 * color and alpha information as separate samples and that store each sample in
 * a separate data element. This class can be used with an arbitrary
 * <CODE>ColorSpace</CODE>. The number of color samples in the pixel values must
 * be same as the number of color components in the <CODE>ColorSpace</CODE>.
 ...
 * samples to components. Depending on the transfer type used to create an
 * instance of <code>ComponentColorModel</code>, the pixel sample values
 * represented by that instance may be signed or unsigned and may be of integral
 * type or float or double (see below for details). The translation from sample
 * values to normalized color/alpha components must follow certain rules. For
 * float and double samples, the translation is an identity, i.e. normalized
 * component values are equal to the corresponding sample values. For integral
 * samples, the translation should be only a simple scale and offset, where the
 * scale and offset constants may be different for each component. The result of
 * applying the scale and offset constants is a set of color/alpha component
 * values, which are guaranteed to fall within a certain range. Typically, the
 * range for a color component will be the range defined by the
 * <code>getMinValue</code> and <code>getMaxValue</code> methods of the
 * <code>ColorSpace</code> class. The range for an alpha component should be 0.0
 * to 1.0.
 * <p>
 * Instances of <code>ComponentColorModel</code> created with transfer types
 * <CODE>DataBuffer.TYPE_BYTE</CODE>, <CODE>DataBuffer.TYPE_USHORT</CODE>, and
 * <CODE>DataBuffer.TYPE_INT</CODE> have pixel sample values which are treated
 * as unsigned integral values. The number of bits in a color or alpha sample of
 * a pixel value might not be the same as the number of bits for the
 * corresponding color or alpha sample passed to the
 ...
 * bits of a sample value hold the component value, where n is the number of
 * significant bits for the component passed to the constructor. It also assumes
 * that any higher-order bits in a sample value are zero. Thus, sample values
 * range from 0 to 2<sup>n</sup> - 1. This class maps these sample values to
 * normalized color component values such that 0 maps to the value obtained from
 * the <code>ColorSpace's</code> <code>getMinValue</code> method for each
 * component and 2<sup>n</sup> - 1 maps to the value obtained from
 * <code>getMaxValue</code>. To create a <code>ComponentColorModel</code> with a
 * different color sample mapping requires subclassing this class and overriding
 * the <code>getNormalizedComponents(Object, float[], int)</code> method. The
 * mapping for an alpha sample always maps 0 to 0.0 and 2<sup>n</sup> - 1 to
 * 1.0.
 * <p>
 * For instances with unsigned sample values, the unnormalized color/alpha
 * component representation is only supported if two conditions hold. First,
 * sample value value 0 must map to normalized component value 0.0 and sample
 * value 2<sup>n</sup> - 1 to 1.0. Second the min/max range of all color
 * components of the <code>ColorSpace</code> must be 0.0 to 1.0. In this case,
 * the component representation is the n least significant bits of the
 * corresponding sample. Thus each component is an unsigned integral value
 * between 0 and 2<sup>n</sup> - 1, where n is the number of significant bits
 * for a particular component. If these conditions are not met, any method
 * taking an unnormalized component argument will throw an
 * <code>IllegalArgumentException</code>.
 * <p>
 * Instances of <code>ComponentColorModel</code> created with transfer types
 * <CODE>DataBuffer.TYPE_SHORT</CODE>, <CODE>DataBuffer.TYPE_FLOAT</CODE>, and
 * <CODE>DataBuffer.TYPE_DOUBLE</CODE> have pixel sample values which are
 * treated as signed short, float, or double values. Such instances do not
 * support the unnormalized color/alpha component representation, so any methods
 * taking such a representation as an argument will throw an
 * <code>IllegalArgumentException</code> when called on one of these instances.
 * The normalized component values of instances of this class have a range which
 * depends on the transfer type as follows: for float samples, the full range of
 * the float data type; for double samples, the full range of the float data
 * type (resulting from casting double to float); for short samples, from
 * approximately -maxVal to +maxVal, where maxVal is the per component maximum
 * value for the <code>ColorSpace</code> (-32767 maps to -maxVal, 0 maps to 0.0,
 * and 32767 maps to +maxVal). A subclass may override the scaling for short
 * sample values to normalized component values by overriding the
 * <code>getNormalizedComponents(Object, float[], int)</code> method. For float
 * and double samples, the normalized component values are taken to be equal to
 * the corresponding sample values, and subclasses should not attempt to add any
 * non-identity scaling for these transfer types.
 * <p>
 * Instances of <code>ComponentColorModel</code> created with transfer types
 * <CODE>DataBuffer.TYPE_SHORT</CODE>, <CODE>DataBuffer.TYPE_FLOAT</CODE>, and
 * <CODE>DataBuffer.TYPE_DOUBLE</CODE> use all the bits of all sample values.
 * Thus all color/alpha components have 16 bits when using
 * <CODE>DataBuffer.TYPE_SHORT</CODE>, 32 bits when using
 * <CODE>DataBuffer.TYPE_FLOAT</CODE>, and 64 bits when using
 * <CODE>DataBuffer.TYPE_DOUBLE</CODE>. When the
 * <code>ComponentColorModel(ColorSpace, int[], boolean, boolean, int, int)</code>
 * form of constructor is used with one of these transfer types, the bits array
 * argument is ignored.
 * <p>
 * It is possible to have color/alpha sample values which cannot be reasonably
 * interpreted as component values for rendering. This can happen when
 * <code>ComponentColorModel</code> is subclassed to override the mapping of
 * unsigned sample values to normalized color component values or when signed
 * sample values outside a certain range are used. (As an example, specifying an
 * alpha component as a signed short value outside the range 0 to 32767,
 * normalized range 0.0 to 1.0, can lead to unexpected results.) It is the
 * responsibility of applications to appropriately scale pixel data before
 * rendering such that color components fall within the normalized range of the
 * <code>ColorSpace</code> (obtained using the <code>getMinValue</code> and
 * <code>getMaxValue</code> methods of the <code>ColorSpace</code> class) and
 * the alpha component is between 0.0 and 1.0. If color or alpha component
 * values fall outside these ranges, rendering results are indeterminate.
 * <p>
 * Methods that use a single int pixel representation throw an
 * <CODE>IllegalArgumentException</CODE>, unless the number of components for
 * the <CODE>ComponentColorModel</CODE> is one and the component value is
 * unsigned -- in other words, a single color component using a transfer type of
 * <CODE>DataBuffer.TYPE_BYTE</CODE>, <CODE>DataBuffer.TYPE_USHORT</CODE>, or
 * <CODE>DataBuffer.TYPE_INT</CODE> and no alpha.
 * <p>
 * A <CODE>ComponentColorModel</CODE> can be used in conjunction with a
 * <CODE>ComponentSampleModel</CODE>, a <CODE>BandedSampleModel</CODE>, or a
 * <CODE>PixelInterleavedSampleModel</CODE> to construct a
...
public class ComponentColorModel extends ColorModel {
     ...
     * <code>float</code>, and <code>double</code> transfer types; it is
     * <code>false</code> for <code>byte</code>, <code>ushort</code>, and
     * <code>int</code> transfer types.
     */
     ...
     * <CODE>DataBuffer.TYPE_FLOAT</CODE>, and
     * <CODE>DataBuffer.TYPE_DOUBLE</CODE>. If not null, the <CODE>bits</CODE>
     * array specifies the number of significant bits per color and alpha
     * component and its length should be at least the number of components in
     * the <CODE>ColorSpace</CODE> if there is no alpha information in the pixel
     * values, or one more than this number if there is alpha information. When
     * the <CODE>transferType</CODE> is <CODE>DataBuffer.TYPE_SHORT</CODE>,
     * <CODE>DataBuffer.TYPE_FLOAT</CODE>, or
     * <CODE>DataBuffer.TYPE_DOUBLE</CODE> the <CODE>bits</CODE> array argument
     * is ignored. <CODE>hasAlpha</CODE> indicates whether alpha information is
     * present. If <CODE>hasAlpha</CODE> is true, then the boolean
     * <CODE>isAlphaPremultiplied</CODE> specifies how to interpret color and
     * alpha samples in pixel values. If the boolean is true, color samples are
     ...
     * by this color model. The acceptable <code>transparency</code> values are
     * <CODE>OPAQUE</CODE>, <CODE>BITMASK</CODE> or <CODE>TRANSLUCENT</CODE>.
     * The <CODE>transferType</CODE> is the type of primitive array used to
    ...
    public ComponentColorModel(ColorSpace colorSpace, int[] bits, boolean hasAlpha,
        ...
        super(bitsHelper(transferType, colorSpace, hasAlpha), bitsArrayHelper(bits, transferType, colorSpace,
                hasAlpha), colorSpace, hasAlpha, isAlphaPremultiplied, transparency, transferType);
        switch (transferType) {
     ...
     * <CODE>DataBuffer.TYPE_INT</CODE>, <CODE>DataBuffer.TYPE_SHORT</CODE>,
     * <CODE>DataBuffer.TYPE_FLOAT</CODE>, and
     * <CODE>DataBuffer.TYPE_DOUBLE</CODE>. The number of significant bits per
     * color and alpha component will be 8, 16, 32, 16, 32, or 64, respectively.
     * The number of color components will be the number of components in the
     * <CODE>ColorSpace</CODE>. There will be an alpha component if
     * <CODE>hasAlpha</CODE> is <CODE>true</CODE>. If <CODE>hasAlpha</CODE> is
     * true, then the boolean <CODE>isAlphaPremultiplied</CODE> specifies how to
     * interpret color and alpha samples in pixel values. If the boolean is
     * true, color samples are assumed to have been multiplied by the alpha
     * sample. The <CODE>transparency</CODE> specifies what alpha values can be
     * represented by this color model. The acceptable <code>transparency</code>
     * values are <CODE>OPAQUE</CODE>, <CODE>BITMASK</CODE> or
     * <CODE>TRANSLUCENT</CODE>. The <CODE>transferType</CODE> is the type of
     ...
     * @since 1.4
     */
    public ComponentColorModel(ColorSpace colorSpace, boolean hasAlpha, boolean isAlphaPremultiplied,
     ...
     * necessary. The pixel value is specified as an int. The returned value
     * will be a non pre-multiplied value. If the alpha is premultiplied, this
     * method divides it out before returning the value (if the alpha value is
     ...
     * model format. A color conversion is done if necessary. The returned value
     * will be in a non pre-multiplied format. If the alpha is premultiplied,
     ...
     * value (if the alpha value is 0, the red value will be 0). Since
     * <code>ComponentColorModel</code> can be subclassed, subclasses inherit
     ...
     * conversion is done if necessary. The <CODE>pixel</CODE> value is
     * specified by an array of data elements of type <CODE>transferType</CODE>
     * passed in as an object reference. The returned value is a non
     * pre-multiplied value. If the alpha is premultiplied, this method divides
     * it out before returning the value (if the alpha value is 0, the green
     * value will be 0). Since <code>ComponentColorModel</code> can be
     * subclassed, subclasses inherit the implementation of this method and if
     * they don't override it then they throw an exception if they use an
     ...
     * passed in as an object reference. The returned value is a non
     * pre-multiplied value. If the alpha is premultiplied, this method divides
     * it out before returning the value (if the alpha value is 0, the blue
     * value will be 0). Since <code>ComponentColorModel</code> can be
     * subclassed, subclasses inherit the implementation of this method and if
     * they don't override it then they throw an exception if they use an
     * unsupported <code>transferType</code>.
     ...
     * <CODE>transferType</CODE> passed in as an object reference. Since
     * <code>ComponentColorModel</code> can be subclassed, subclasses inherit
     * the implementation of this method and if they don't override it then they
     * throw an exception if they use an unsupported <code>transferType</code>.
     *
     ...
     * this method divides it out of the color components (if the alpha value is
     * 0, the color values will be 0). Since <code>ComponentColorModel</code>
     * can be subclassed, subclasses inherit the implementation of this method
     * and if they don't override it then they throw an exception if they use an
     * unsupported <code>transferType</code>.
     *
     * @param inData
     *        The pixel from which you want to get the color/alpha
    ...
    public int getRGB(Object inData) {
        ...
        float[] rgb = colorSpace.toRGB(norm);
        return (getAlpha(inData) << 24) | (((int) (rgb[0] * 255.0f + 0.5f)) << 16) | (((int) (rgb[1] * 255.0f
                + 0.5f)) << 8) | (((int) (rgb[2] * 255.0f + 0.5f)) << 0);
    }
     ...
     * allocated. Since <code>ComponentColorModel</code> can be subclassed,
     * subclasses inherit the implementation of this method and if they don't
    ...
    public Object getDataElements(int rgb, Object pixel) {
        ...
        if (is_sRGB_stdScale || is_LinearRGB_stdScale) {
            ...
            if (supportsAlpha) {
                ...
                if (nBits[numColorComponents] == 8) {
                    ...
                    intpixel[numColorComponents] = (int) (alp * factor * ((1 << nBits[numColorComponents])
                            - 1) + 0.5f);
                }
                if (isAlphaPremultiplied) {
                    factor *= alp;
        ...
        throw new IllegalArgumentException("This method has not been " + "implemented for transferType "
                + transferType);
    }

    /**
     * Returns an array of unnormalized color/alpha components given a pixel in
     * this <CODE>ColorModel</CODE>. An IllegalArgumentException is thrown if
     * the component value for this <CODE>ColorModel</CODE> is not conveniently
     * representable in the unnormalized form. Color/alpha components are stored
     ...
     * data elements of type <CODE>transferType</CODE> passed in as an object
     * reference. An IllegalArgumentException is thrown if the component values
     * for this <CODE>ColorModel</CODE> are not conveniently representable in
     * the unnormalized form. Color/alpha components are stored in the
     * <CODE>components</CODE> array starting at <CODE>offset</CODE> (even if
     * the array is allocated by this method). Since
     * <code>ComponentColorModel</code> can be subclassed, subclasses inherit
     * the implementation of this method and if they don't override it then this
     * method might throw an exception if they use an unsupported
     * <code>transferType</code>.
     *
     * @param pixel
        ...
        switch (transferType) {
            case DataBuffer.TYPE_BYTE:
                ...
                if (needAlpha) {
                    ...
                    for (int c = 0, nc = normOffset; c < numColorComponents; c++, nc++) {
                        bpixel[c] = (byte) ((stdNormComponents[nc] * alpha) * ((float) ((1 << nBits[c]) - 1))
                                + 0.5f);
                    }
                    bpixel[numColorComponents] = (byte) (alpha * ((float) ((1 << nBits[numColorComponents])
                            - 1)) + 0.5f);
                } else {
            ...
            case DataBuffer.TYPE_USHORT:
                ...
                if (needAlpha) {
                    float alpha = stdNormComponents[numColorComponents + normOffset];
                    for (int c = 0, nc = normOffset; c < numColorComponents; c++, nc++) {
                        uspixel[c] = (short) ((stdNormComponents[nc] * alpha) * ((float) ((1 << nBits[c])
                                - 1)) + 0.5f);
                    }
                    uspixel[numColorComponents] = (short) (alpha * ((float) ((1 << nBits[numColorComponents])
                            - 1)) + 0.5f);
                } else {
                    for (int c = 0, nc = normOffset; c < numComponents; c++, nc++) {
            ...
            case DataBuffer.TYPE_INT:
                ...
                if (needAlpha) {
                    float alpha = stdNormComponents[numColorComponents + normOffset];
                    for (int c = 0, nc = normOffset; c < numColorComponents; c++, nc++) {
                        ipixel[c] = (int) ((stdNormComponents[nc] * alpha) * ((float) ((1 << nBits[c]) - 1))
                                + 0.5f);
                    }
                    ipixel[numColorComponents] = (int) (alpha * ((float) ((1 << nBits[numColorComponents])
                            - 1)) + 0.5f);
                } else {
                    for (int c = 0, nc = normOffset; c < numComponents; c++, nc++) {
     ...
     * <code>normOffset</code>).
     * <p>
     * This method must be overridden by a subclass if that subclass is designed
     * to translate pixel sample values to color component values in a
     * non-default way. The default translations implemented by this class is
     * described in the class comments. Any subclass implementing a non-default
     * translation must follow the constraints on allowable translations defined
     * there.
     * 
     * @param pixel
     *        the specified pixel
     ...
     * method also returns an instance of this <CODE>ColorModel</CODE> with the
     * <CODE>isAlphaPremultiplied</CODE> flag set appropriately. Since
    ...
    public ColorModel coerceData(WritableRaster raster, boolean isAlphaPremultiplied) {
        ...
        if (!signed) {
            ...
            return new ComponentColorModel(colorSpace, supportsAlpha, isAlphaPremultiplied, transparency,
                    transferType);
        }
     ...
     * extracted from the input <CODE>Raster</CODE>. This method assumes that
     * <CODE>Raster</CODE> objects associated with this <CODE>ColorModel</CODE>
     * store the alpha band, if present, as the last band of image data. Returns
     * null if there is no separate spatial alpha channel associated with this
     * <CODE>ColorModel</CODE>. This method creates a new <CODE>Raster</CODE>,
     ...
     *
     * @param obj
     *        The object to compare with this color model.
