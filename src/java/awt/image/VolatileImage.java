...
public abstract class VolatileImage extends Image implements Transparency {
    ...
    /**
     * Validated image has been restored and is now ready to use. Note that
     * restoration causes contents of the image to be lost.
     */
    public static final int IMAGE_RESTORED = 1;

    /**
     * Validated image is incompatible with supplied
     * <code>GraphicsConfiguration</code> object and should be re-created as
     * appropriate. Usage of the image as-is after receiving this return code
     * from <code>validate</code> is undefined.
     */
    public static final int IMAGE_INCOMPATIBLE = 2;

    /**
     * Returns a static snapshot image of this object. The
     * <code>BufferedImage</code> returned is only current with the
     * <code>VolatileImage</code> at the time of the request and will not be
     * updated with any future changes to the <code>VolatileImage</code>.
     * 
     ...
     * VolatileImage object is optimized for rendering operations and blitting
     * to the screen or other VolatileImage objects, as opposed to reading back
     * the pixels of the image. Therefore, operations such as
     * <code>getSource</code> may not perform as fast as operations that do not
     * rely on reading the pixels. Note also that the pixel values read from the
     * image are current with those in the image only at the time that they are
     * retrieved. This method takes a snapshot of the image at the time the
     * request is made and the ImageProducer object returned works with that
     * static snapshot image, not the original VolatileImage. Calling
     * getSource() is equivalent to calling getSnapshot().getSource().
     * 
     * @return an {@link ImageProducer} that can be used to produce the pixels
     *         for a <code>BufferedImage</code> representation of this Image.
     * @see ImageProducer
     * @see #getSnapshot()
     */
    ...
    /**
     * Attempts to restore the drawing surface of the image if the surface had
     * been lost since the last <code>validate</code> call. Also validates this
     * image against the given GraphicsConfiguration parameter to see whether
     * operations from this image to the GraphicsConfiguration are compatible.
     * An example of an incompatible combination might be a situation where a
     * VolatileImage object was created on one graphics device and then was used
     * to render to a different graphics device. Since VolatileImage objects
     * tend to be very device-specific, this operation might not work as
     * intended, so the return code from this validate call would note that
     * incompatibility. A null or incorrect value for gc may cause incorrect
     * values to be returned from <code>validate</code> and may cause later
     ...
     *         affected and the image may need to be re-rendered.<BR>
     *         <code>IMAGE_INCOMPATIBLE</code> if the image is incompatible with
     *         the <code>GraphicsConfiguration</code> object passed into the
     *         <code>validate</code> method. Incompatibility implies that the
     *         image may need to be recreated with a new <code>Component</code>
     *         or <code>GraphicsConfiguration</code> in order to get an image
     *         that can be used successfully with this
     *         <code>GraphicsConfiguration</code>. An incompatible image is not
     *         checked for whether restoration was necessary, so the state of
     *         the image is unchanged after a return value of
     *         <code>IMAGE_INCOMPATIBLE</code> and this return value implies
     *         nothing about whether the image needs to be restored.
     ...
     * application at the end of any series of rendering operations to or from
     * the image to see whether the image needs to be validated and the
     * rendering redone.
     * 
     ...
     * specific capabilities of this VolatileImage. This would allow programmers
     * to find out more runtime information on the specific VolatileImage object
     * that they have created. For example, the user might create a
     * VolatileImage but the system may have no video memory left for creating
     * an image of that size, so although the object is a VolatileImage, it is
     * not as accelerated as other VolatileImage objects on this platform might
     * be. The user might want that information to find other solutions to their
     * problem.
     * 
     * @return an <code>ImageCapabilities</code> object that contains the
     *         capabilities of this <code>VolatileImage</code>.
     * @since 1.4
     */
