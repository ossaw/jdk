...
public class PixelGrabber implements ImageConsumer {
	 ...
	 *                 rectangle of
	 *                 pixels to retrieve from the image, relative to the
	 *                 default
	 ...
	 * @param off
	 *                 the offset into the array of where to store the first
	 *                 pixel
	 * @param scansize
	 ...
	 * 
	 * @param ip
	 *                 the <code>ImageProducer</code> that produces the image
	 *                 from
	 *                 which to retrieve pixels
	 * @param x
	 *                 the x coordinate of the upper left corner of the
	 *                 rectangle of
	 *                 pixels to retrieve from the image, relative to the
	...
	/**
	 * Create a PixelGrabber object to grab the (x, y, w, h) rectangular section
	 * of pixels from the specified image. The pixels are accumulated in the
	 * original ColorModel if the same ColorModel is used for every call to
	 * setPixels, otherwise the pixels are accumulated in the default RGB
	 * ColorModel. If the forceRGB parameter is true, then the pixels will be
	 * accumulated in the default RGB ColorModel anyway. A buffer is allocated
	 * by the PixelGrabber to hold the pixels in either case. If {@code (w < 0)}
	 * or {@code (h < 0)}, then they will default to the remaining width and
	 ...
	 *                 the height of the rectangle of pixels to retrieve
	 * @param forceRGB
	 *                 true if the pixels should always be converted to the
	 *                 default
	 *                 RGB ColorModel
	 */
	 ...
	 * until the specified timeout has elapsed. This method behaves in the
	 * following ways, depending on the value of <code>ms</code>:
	 * <ul>
	 * <li>If {@code ms == 0}, waits until all pixels are delivered
	 * <li>If {@code ms > 0}, waits until all pixels are delivered as timeout
	 * expires.
	 * <li>If {@code ms < 0}, returns <code>true</code> if all pixels are
	 * grabbed, <code>false</code> otherwise and does not wait.
	 * </ul>
	 * 
	 * @param ms
	...
	/**
	 * Get the width of the pixel buffer (after adjusting for image width). If
	 * no width was specified for the rectangle of pixels to grab then then this
	 * information will only be available after the image has delivered the
	 * dimensions.
	 * 
	 * @return the final width used for the pixel buffer or -1 if the width is
	 *         not yet known
	 * @see #getStatus
	 */
	...
	/**
	 * Get the height of the pixel buffer (after adjusting for image height). If
	 * no width was specified for the rectangle of pixels to grab then then this
	 * information will only be available after the image has delivered the
	 * dimensions.
	 * 
	 * @return the final height used for the pixel buffer or -1 if the height is
	 *         not yet known
	 * @see #getStatus
	 */
	...
	/**
	 * Get the pixel buffer. If the PixelGrabber was not constructed with an
	 * explicit pixel buffer to hold the pixels then this method will return
	 * null until the size and format of the image data is known. Since the
	 * PixelGrabber may fall back on accumulating the data in the default RGB
	 * ColorModel at any time if the source image uses more than one ColorModel
	 * to deliver the data, the array object returned by this method may change
	 * over time until the image grab is complete.
	 * 
	...
	public synchronized Object getPixels() {
		return (bytePixels == null) ? ((Object) intPixels)
				: ((Object) bytePixels);
	}

	/**
	 * Get the ColorModel for the pixels stored in the array. If the
	 * PixelGrabber was constructed with an explicit pixel buffer then this
	 * method will always return the default RGB ColorModel, otherwise it may
	 * return null until the ColorModel used by the ImageProducer is known.
	 * Since the PixelGrabber may fall back on accumulating the data in the
	 * default RGB ColorModel at any time if the source image uses more than one
	 * ColorModel to deliver the data, the ColorModel object returned by this
	 * method may change over time until the image grab is complete and may not
	 * reflect any of the ColorModel objects that was used by the ImageProducer
	 ...
	 * <p>
	 * Note: This method is intended to be called by the ImageProducer of the
	 * Image whose pixels are being grabbed. Developers using this class to
	 * retrieve pixels from an image should avoid calling this method directly
	 * since that operation could result in problems with retrieving the
	 * requested pixels.
	 * 
	 * @param width
	...
	public void setDimensions(int width, int height) {
		...
		} else if (intPixels == null && imageModel == ColorModel
				.getRGBdefault()) {
	...
	/**
	 * Returns the status of the pixels. The ImageObserver flags representing
	 * the available pixel information are returned. This method and
	 * {@link #getStatus() getStatus} have the same implementation, but
	 * <code>getStatus</code> is the preferred method because it conforms to the
	 * convention of naming information-retrieval methods with the form
	 * "getXXX".
	 * 
	 * @return the bitwise OR of all relevant ImageObserver flags
	 * @see ImageObserver
