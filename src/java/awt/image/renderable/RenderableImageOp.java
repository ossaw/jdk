...
public class RenderableImageOp implements RenderableImage {
	...
	/**
	 * Constructs a RenderedImageOp given a ContextualRenderedImageFactory
	 * object, and a ParameterBlock containing RenderableImage sources and other
	 * parameters. Any RenderedImage sources referenced by the ParameterBlock
	 * will be ignored.
	 *
	 * @param CRIF
	...
	/**
	 * Returns a vector of RenderableImages that are the sources of image data
	 * for this RenderableImage. Note that this method may return an empty
	 * vector, to indicate that the image has no sources, or null, to indicate
	 ...
	 * createRendering() or createScaledRendering()) with the same arguments may
	 * produce different results. This method may be used to determine whether
	 * an existing rendering may be cached and reused. The CRIF's isDynamic
	 * method will be called.
	 * 
	 * @return <code>true</code> if successive renderings with the same
	 *         arguments might produce different results; <code>false</code>
	 *         otherwise.
	 */
	...
	/**
	 * Gets the width in user coordinate space. By convention, the usual width
	 * of a RenderableImage is equal to the image's aspect ratio (width divided
	 * by height).
	 *
	...
	/**
	 * Gets the height in user coordinate space. By convention, the usual height
	 * of a RenderedImage is equal to 1.0F.
	 *
	...
	/**
	 * Change the current ParameterBlock of the operation, allowing editing of
	 * image rendering chains. The effects of such a change will be visible when
	 * a new rendering is created from this RenderableImageOp or any dependent
	 * RenderableImageOp.
	 *
	 * @param paramBlock
	...
	/**
	 * Creates a RenderedImage instance of this image with width w, and height h
	 ...
	 * image. In addition any RenderedImages that are obtained via the
	 * getSources() method on the created RenderedImage may have such a
	 * property.
	 *
	 * @param w
	...
	/**
	 * Gets a RenderedImage instance of this image with a default width and
	 * height in pixels. The RenderContext is built automatically with an
	 * appropriate usr2dev transform and an area of interest of the full image.
	 * All the rendering hints come from hints passed in. Implementors of this
	 * interface must be sure that there is a defined default width and height.
	 *
	 * @return a RenderedImage containing the rendered data.
	 */
	 ...
	 * using the backwards-mapped RenderContexts obtained in step 1, resulting
	 * in a rendering of each source;
	 * <li>ContextualRenderedImageFactory.create() is called with a new
	 ...
	 * were used to create the image. In addition any RenderedImages that are
	 * obtained via the getSources() method on the created RenderedImage may
