...
/**
 * ContextualRenderedImageFactory provides an interface for the functionality
 * that may differ between instances of RenderableImageOp. Thus different
 * operations on RenderableImages may be performed by a single class such as
 * RenderedImageOp through the use of multiple instances of
 * ContextualRenderedImageFactory. The name ContextualRenderedImageFactory is
...
public interface ContextualRenderedImageFactory extends RenderedImageFactory {
    ...
    /**
     * Maps the operation's output RenderContext into a RenderContext for each
     * of the operation's sources. This is useful for operations that can be
     * expressed in whole or in part simply as alterations in the RenderContext,
     * such as an affine mapping, or operations that wish to obtain lower
     * quality renderings of their sources in order to save processing effort or
     * transmission bandwith. Some operations, such as blur, can also use this
     ...
     * containing the operation's sources and parameters. The output is a
     * RenderedImage that takes the RenderContext into account to determine its
     * dimensions and placement on the image plane. This method houses the
     * "intelligence" that allows a rendering-independent operation to adapt to
     ...
     * given set of sources, in rendering-independent space. The bounds are
     * returned as a Rectangle2D, that is, an axis-aligned rectangle with
     ...
     * create(RenderContext, ParameterBlock)) with the same arguments may
