...
public interface RenderedImage {
	 ...
	 * image data for this RenderedImage. This method returns null if the
	 * RenderedImage object has no information about its immediate sources. It
	 * returns an empty Vector if the RenderedImage object has no immediate
	 * sources.
	 * 
	 ...
	 * properties and whether it is immutable is determined by the implementing
	 * class. This method returns java.awt.Image.UndefinedProperty if the
	 ...
	 * from this image will have this as their ColorModel. This can return null.
	 * 
	 ...
	 * coordinate of the upper-left pixel of tile (0, 0). (Note that tile (0, 0)
	...
	/**
	 * Returns the Y offset of the tile grid relative to the origin, i.e., the Y
	 * coordinate of the upper-left pixel of tile (0, 0). (Note that tile (0, 0)
	 * may not actually exist.)
	 * 
	...
	/**
	 * Returns tile (tileX, tileY). Note that tileX and tileY are indices into
	 * the tile array, not pixel locations. The Raster that is returned is live
	...
	/**
	 * Returns the image as one large tile (for tile based images this will
	 * require fetching the whole image and copying the image data over). The
	 * Raster returned is a copy of the image data and will not be updated if
	...
	/**
	 * Computes and returns an arbitrary region of the RenderedImage. The Raster
	 * returned is a copy of the image data and will not be updated if the image
	 * is changed.
	 * 
	 * @param rect
	...
	/**
	 * Computes an arbitrary rectangular region of the RenderedImage and copies
	 * it into a caller-supplied WritableRaster. The region to be computed is
	 * determined from the bounds of the supplied WritableRaster. The supplied
	 * WritableRaster must have a SampleModel that is compatible with this
