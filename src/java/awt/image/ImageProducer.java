...
public interface ImageProducer {
     ...
     * <code>ImageProducer</code> for access to the image data during a later
     * reconstruction of the <code>Image</code>. The <code>ImageProducer</code>
     * may, at its discretion, start delivering the image data to the consumer
     * using the <code>ImageConsumer</code> interface immediately, or when the
     * next available image reconstruction is triggered by a call to the
     ...
     * registered. The <code>ImageProducer</code> should stop sending data to
    ...
    /**
     * Registers the specified <code>ImageConsumer</code> object as a consumer
     * and starts an immediate reconstruction of the image data which will then
     * be delivered to this consumer and any other consumer which might have
    ...
    /**
     * Requests, on behalf of the <code>ImageConsumer</code>, that the
     * <code>ImageProducer</code> attempt to resend the image data one more time
     * in TOPDOWNLEFTRIGHT order so that higher quality conversion algorithms
     * which depend on receiving pixels in order can be used to produce a better
     * output version of the image. The <code>ImageProducer</code> is free to
     * ignore this call if it cannot resend the data in that order. If the data
     * can be resent, the <code>ImageProducer</code> should respond by executing
     * the following minimum set of <code>ImageConsumer</code> method calls:
     * 
     * <pre>
     * {@code
