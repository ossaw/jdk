 ...
 * bank of the DataBuffer. Accessor methods are provided so that image data can
 * be manipulated directly. Pixel stride is the number of data array elements
 * between two samples for the same band on the same scanline. Scanline stride
 * is the number of data array elements between a given sample and the
 * corresponding sample in the same column of the next scanline. Band offsets
 * denote the number of data array elements from the first data array element of
 * the bank of the DataBuffer holding each band to the first sample of the band.
 * The bands are numbered from 0 to N-1. Bank indices denote the correspondence
 * between a bank of the data buffer and a band of image data. This class
 ...
 * {@link DataBuffer#TYPE_FLOAT TYPE_FLOAT} and {@link DataBuffer#TYPE_DOUBLE
...
public class PixelInterleavedSampleModel extends ComponentSampleModel {
    ...
    public PixelInterleavedSampleModel(int dataType, int w, int h, int pixelStride, int scanlineStride,
        ...
        if (maxBandOff > scanlineStride) {
            throw new IllegalArgumentException("Offsets between bands must be" + " less than the scanline "
                    + " stride");
        }
        ...
        if (pixelStride < maxBandOff) {
            throw new IllegalArgumentException("Pixel stride must be greater"
                    + " than or equal to the offsets" + " between bands");
        }
    }

    /**
     * Creates a new PixelInterleavedSampleModel with the specified width and
     * height. The new PixelInterleavedSampleModel will have the same number of
     * bands, storage data type, and pixel stride as this
     * PixelInterleavedSampleModel. The band offsets may be compressed such that
    ...
    /**
     * Creates a new PixelInterleavedSampleModel with a subset of the bands of
     * this PixelInterleavedSampleModel. The new PixelInterleavedSampleModel can
     * be used with any DataBuffer that the existing PixelInterleavedSampleModel
     * can be used with. The new PixelInterleavedSampleModel/DataBuffer
     * combination will represent an image with a subset of the bands of the
