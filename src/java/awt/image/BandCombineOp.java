 ...
 * <p>
 * The width of the matrix must be equal to the number of bands in the source
 * <CODE>Raster</CODE>, optionally plus one. If there is one more column in the
 * matrix than the number of bands, there is an implied 1 at the end of the
 * vector of band samples representing a pixel. The height of the matrix must be
 * equal to the number of bands in the destination.
 * <p>
...
public class BandCombineOp implements RasterOp {
	...
	/**
	 * Constructs a <CODE>BandCombineOp</CODE> with the specified matrix. The
	 * width of the matrix must be equal to the number of bands in the source
	 * <CODE>Raster</CODE>, optionally plus one. If there is one more column in
	 * the matrix than the number of bands, there is an implied 1 at the end of
	 * the vector of band samples representing a pixel. The height of the matrix
	 * must be equal to the number of bands in the destination.
	 * <p>
	 * The first subscript is the row index and the second is the column index.
	 * This operation uses none of the currently defined rendering hints; the
	...
	public BandCombineOp(float[][] matrix, RenderingHints hints) {
		...
		for (int i = 0; i < nrows; i++) {
			/*
			 * Arrays.copyOf is forgiving of the source array being too short,
			 * but it is also faster than other cloning methods, so we provide
			 * our own protection for short matrix rows.
			 */
	 ...
	 * the number of bands in the source or destination is incompatible with the
	 * matrix. See the class comments for more details.
	 * <p>
	 * If the destination is null, it will be created with a number of bands
	 * equalling the number of rows in the matrix. No exception is thrown if the
	...
	/**
	 * Returns the bounding box of the transformed destination. Since this is
	 * not a geometric operation, the bounding box is the same for the source
	 * and destination. An <CODE>IllegalArgumentException</CODE> may be thrown
	 * if the number of bands in the source is incompatible with the matrix. See
	 ...
	 * and number of bands. An <CODE>IllegalArgumentException</CODE> may be
	 * thrown if the number of bands in the source is incompatible with the
	 * matrix. See the class comments for more details.
	 *
	 * @param src
	 *            The <CODE>Raster</CODE> to be filtered.
	 ...
	 * in the source <CODE>Raster</CODE>. If <CODE>dstPt</CODE> is specified, it
	 * is used to hold the return value. Since this is not a geometric
	 * operation, the point returned is the same as the specified
	 * <CODE>srcPt</CODE>.
	 *
	 * @param srcPt
