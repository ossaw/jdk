...
/**
 * This class represents pixel data packed such that the N samples which make up
 * a single pixel are stored in a single data array element, and each data data
 ...
 * TYPE_USHORT}, {@link DataBuffer#TYPE_INT TYPE_INT} data types. All data array
 * elements reside in the first bank of a DataBuffer. Accessor methods are
 * provided so that the image data can be manipulated directly. Scanline stride
 * is the number of data array elements between a given sample and the
 * corresponding sample in the same column of the next scanline. Bit masks are
 * the masks required to extract the samples representing the bands of the
 * pixel. Bit offsets are the offsets in bits into the data array element of the
 * samples representing the bands of the pixel.
 * <p>
 * The following code illustrates extracting the bits of the sample representing
 * band <code>b</code> for pixel <code>x,y</code> from DataBuffer
 * <code>data</code>:
 * 
 * <pre>
 * {
...
public class SinglePixelPackedSampleModel extends SampleModel {
     ...
     * Each sample is stored in a data array element in the position of its
     * corresponding bit mask. Each bit mask must be contiguous and masks must
     * not overlap. Bit masks exceeding data type capacity are truncated.
     * 
     * @param dataType
    ...
    /**
     * Constructs a SinglePixelPackedSampleModel with bitMasks.length bands and
     * a scanline stride equal to scanlineStride data array elements. Each
     * sample is stored in a data array element in the position of its
     * corresponding bit mask. Each bit mask must be contiguous and masks must
     * not overlap. Bit masks exceeding data type capacity are truncated.
     * 
     * @param dataType
     ...
     *                                  not greater than 0
     * @throws IllegalArgumentException
     *                                  if any mask in <code>bitMask</code> is
     *                                  not contiguous
     * @throws IllegalArgumentException
     *                                  if <code>dataType</code> is not either
     *                                  <code>DataBuffer.TYPE_BYTE</code>,
    ...
    /**
     * Returns the size of the buffer (in data array elements) needed for a data
     * buffer that matches this SinglePixelPackedSampleModel.
     */
    ...
    /**
     * Creates a new SinglePixelPackedSampleModel with the specified width and
     * height. The new SinglePixelPackedSampleModel will have the same storage
     ...
     * @return a <code>SinglePixelPackedSampleModel</code> with the specified
     *         width and height.
     * @throws IllegalArgumentException
     *                                  if <code>w</code> or <code>h</code> is
     *                                  not greater than 0
    ...
    public SampleModel createCompatibleSampleModel(int w, int h) {
        SampleModel sampleModel = new SinglePixelPackedSampleModel(dataType, w,
                h, bitMasks);
        return sampleModel;
    }
     ...
     * SinglePixelPackedSampleModel. The DataBuffer's data type and size will be
     * consistent with this SinglePixelPackedSampleModel. The DataBuffer will
     * have a single bank.
     */
     ...
     * element containing pixel <code>x,y</code> can be retrieved from a
     * DataBuffer <code>data</code> with a SinglePixelPackedSampleModel
     * <code>sppsm</code> as:
     * 
     * <pre>
    ...
    /**
     * Returns the bit offsets into the data array element representing a pixel
     * for all bands.
     * 
     ...
     * SinglePixelPackedSampleModel/DataBuffer combination will represent an
    ...
    public SampleModel createSubsetSampleModel(int bands[]) {
        if (bands.length > numBands)
            throw new RasterFormatException("There are only " + numBands
                    + " bands");
     ...
     * TransferType. For a SinglePixelPackedSampleModel, the array will have one
     * element, and the type will be the same as the storage data type.
     * Generally, obj should be passed in as null, so that the Object will be
     * created automatically and will be of the right primitive data type.
     * <p>
     * The following code illustrates transferring data for one pixel from
     * DataBuffer <code>db1</code>, whose storage layout is described by
     * SinglePixelPackedSampleModel <code>sppsm1</code>, to DataBuffer
     * <code>db2</code>, whose storage layout is described by
     * SinglePixelPackedSampleModel <code>sppsm2</code>. The transfer will
     ...
     * array, one sample per array element. ArrayIndexOutOfBoundsException may
     ...
     * pixels in an int array, one sample per array element.
     * ArrayIndexOutOfBoundsException may be thrown if the coordinates are not
     * in bounds.
     * 
     * @param x
     ...
     * primitive array of type TransferType. For a SinglePixelPackedSampleModel,
     * only the first element of the array will hold valid data, and the type of
     * the array must be the same as the storage data type of the
     * SinglePixelPackedSampleModel.
     * <p>
     * The following code illustrates transferring data for one pixel from
     * DataBuffer <code>db1</code>, whose storage layout is described by
    ...
    public void setDataElements(int x, int y, Object obj, DataBuffer data) {
        ...
        switch (type) {
            ...
            case DataBuffer.TYPE_USHORT:
                ...
                data.setElem(y * scanlineStride + x, ((int) sarray[0])
                        & 0xffff);
                break;
     ...
     * pixels from an int array containing one sample per array element.
     * ArrayIndexOutOfBoundsException may be thrown if the coordinates are not
     * in bounds.
     * 
     * @param x
    ...
    public boolean equals(Object o) {
        ...
        return this.width == that.width && this.height == that.height
                ...
                && this.dataType == that.dataType && Arrays.equals(
                        this.bitMasks, that.bitMasks) && Arrays.equals(
                                this.bitOffsets, that.bitOffsets) && Arrays
                                        .equals(this.bitSizes, that.bitSizes)
