 ...
 * how to set up the animation source and signal changes in the data (adapted
 * from the MemoryAnimationSourceDemo by Garth Dickie):
 * 
 * <pre>
 * {@code
 *
 ...
 * @see ImageProducer
 * @author Jim Graham
 * @author Animation capabilities inspired by the MemoryAnimationSource class
 *         written by Garth Dickie
 */
public class MemoryImageSource implements ImageProducer {
    ...
    /**
     * Removes an ImageConsumer from the list of consumers interested in data
     * for this image.
     * 
     * @param ic
     *        the specified <code>ImageConsumer</code>
    ...
    /**
     * Adds an ImageConsumer to the list of consumers interested in data for
     * this image and immediately starts delivery of the image data through the
    ...
    /**
     * Changes this memory image into a multi-frame animation or a single-frame
     * static image depending on the animated parameter.
     * <p>
     * This method should be called immediately after the MemoryImageSource is
     * constructed and before an image is created with it to ensure that all
     * ImageConsumers will receive the correct multi-frame data. If an
     * ImageConsumer is added to this ImageProducer before this flag is set then
     * that ImageConsumer will see only a snapshot of the pixel data that was
     ...
     * sending the complete buffer of pixels whenever there is a change. This
     * flag is ignored if the animation flag is not turned on through the
     * setAnimated() method.
     * <p>
     * This method should be called immediately after the MemoryImageSource is
     * constructed and before an image is created with it to ensure that all
    ...
    /**
     * Sends a whole new buffer of pixels to any ImageConsumers that are
     * currently interested in the data for this image and notify them that an
     * animation frame is complete. This method only has effect if the animation
     * flag has been turned on through the setAnimated() method.
     * 
    ...
    /**
     * Sends a rectangular region of the buffer of pixels to any ImageConsumers
     * that are currently interested in the data for this image and notify them
     * that an animation frame is complete. This method only has effect if the
     * animation flag has been turned on through the setAnimated() method. If
     * the full buffer update flag was turned on with the setFullBufferUpdates()
     * method then the rectangle parameters will be ignored and the entire
    ...
    /**
     * Sends a rectangular region of the buffer of pixels to any ImageConsumers
     * that are currently interested in the data for this image. If the
     * framenotify parameter is true then the consumers are also notified that
     * an animation frame is complete. This method only has effect if the
     * animation flag has been turned on through the setAnimated() method. If
     * the full buffer update flag was turned on with the setFullBufferUpdates()
     * method then the rectangle parameters will be ignored and the entire
     * buffer will always be sent.
    ...
    /**
     * Changes to a new byte array to hold the pixels for this image. If the
    ...
    private void initConsumer(ImageConsumer ic) {
        ...
        if (isConsumer(ic)) {
            ic.setHints(animating ? (fullbuffers ? (ImageConsumer.TOPDOWNLEFTRIGHT
