...
public abstract class Image {
    ...
    /**
     * convenience object; we can use this single static object for all images
     ...
     * different default priorities and applications are free to set the
     * priority for specific images via the
     ...
     * method returns <code>-1</code> and the specified
     ...
     * <code>null</code>, and the <code>ImageObserver</code> object is notified
     * later.
     * <p>
     ...
     * screen as well as any system resources that are being used to store data
     * or pixels for the image if they can be recreated. The image is reset to a
     * state similar to when it was first created so that if it is again
     * rendered, the image data will have to be recreated or fetched again from
     * its source.
     * <p>
     * Examples of how this method affects specific types of Image object:
     * <ul>
     * <li>BufferedImage objects leave the primary Raster which stores their
     * pixels untouched, but flush any information cached about those pixels
     * such as copies uploaded to the display hardware for accelerated blits.
     * <li>Image objects created by the Component methods which take a width and
     * height leave their primary buffer of pixels untouched, but have all
     * cached information released much like is done for BufferedImage objects.
     * <li>VolatileImage objects release all of their pixel resources including
     * their primary copy which is typically stored on the display hardware
     * where resources are scarce. These objects can later be restored using
     * their {@link java.awt.image.VolatileImage#validate validate} method.
     * <li>Image objects created by the Toolkit and Component classes which are
     * loaded from files, URLs or produced by an {@link ImageProducer} are
     * unloaded and all local resources are released. These objects can later be
     * reloaded from their original source as needed when they are rendered,
     * just as when they were first created.
     * </ul>
     */
    public void flush() {
     ...
     * capabilities of this Image on the specified GraphicsConfiguration. This
     * allows programmers to find out more runtime information on the specific
     * Image object that they have created. For example, the user might create a
     * BufferedImage but the system may have no video memory left for creating
     * an image of that size on the given GraphicsConfiguration, so although the
     * object may be acceleratable in general, it does not have that capability
    ...
    /**
     * Sets a hint for this image about how important acceleration is. This
     * priority hint is used to compare to the priorities of other Image objects
     * when determining how to use scarce acceleration resources such as video
     * memory. When and if it is possible to accelerate this Image, if there are
     * not enough resources available to provide that acceleration but enough
     * can be freed up by de-accelerating some other image of lower priority,
     * then that other Image may be de-accelerated in deference to this one.
     * Images that have the same priority take up resources on a first-come,
     * first-served basis.
     * 
     * @param priority
