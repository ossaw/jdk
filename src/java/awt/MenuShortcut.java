...
/**
 * The <code>MenuShortcut</code>class represents a keyboard accelerator for a
 * MenuItem.
 * <p>
 * Menu shortcuts are created using virtual keycodes, not characters. For
 * example, a menu shortcut for Ctrl-a (assuming that Control is the accelerator
 * key) would be created with code like the following:
 * <p>
 ...
 * <code>java.awt.event.KeyEvent.getExtendedKeyCodeForChar</code> call. For
 * example, a menu shortcut for "Ctrl+cyrillic ef" is created by
 * <p>
 * <code>MenuShortcut ms = new MenuShortcut(KeyEvent.getExtendedKeyCodeForChar('\u0444'), false);</code>
 * <p>
 * Note that shortcuts created with a keycode or an extended keycode defined as
 * a constant in <code>KeyEvent</code> work regardless of the current keyboard
 * layout. However, a shortcut made of an extended keycode not listed in
 * <code>KeyEvent</code> only work if the current keyboard layout produces a
 * corresponding letter.
 * <p>
 * The accelerator key is platform-dependent and may be obtained via
 * {@link Toolkit#getMenuShortcutKeyMask}.
 *
 * @author Thomas Ball
 * @since JDK1.1
 */
public class MenuShortcut implements java.io.Serializable {
    /**
     * The virtual keycode for the menu shortcut. This is the keycode with which
     * the menu shortcut will be created. Note that it is a virtual keycode, not
     * a character, e.g. KeyEvent.VK_A, not 'a'. Note: in 1.1.x you must use
     * setActionCommand() on a menu item in order for its shortcut to work,
     * otherwise it will fire a null action command.
     *
     * @serial
     * @see #getKey()
    ...
    /**
     * Indicates whether the shft key was pressed. If true, the shift key was
     * pressed. If false, the shift key was not pressed
     *
     * @serial
     * @see #usesShiftModifier()
     * @since JDK1.1
    ...
    /**
     * Returns whether this MenuShortcut is the same as another: equality is
     * defined to mean that both MenuShortcuts use the same key and both either
