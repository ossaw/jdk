...
/**
 * The <code>TexturePaint</code> class provides a way to fill a {@link Shape}
 * with a texture that is specified as a {@link BufferedImage}. The size of the
 * <code>BufferedImage</code> object should be small because the
 * <code>BufferedImage</code> data is copied by the <code>TexturePaint</code>
 * object. At construction time, the texture is anchored to the upper left
 * corner of a {@link Rectangle2D} that is specified in user space. Texture is
 * computed for locations in the device space by conceptually replicating the
 * specified <code>Rectangle2D</code> infinitely in all directions in user space
 * and mapping the <code>BufferedImage</code> to each replicated
...
public class TexturePaint implements Paint {
	...
	/**
	 * Returns a copy of the anchor rectangle which positions and sizes the
	 * textured image.
	 * 
	 * @return the <code>Rectangle2D</code> used to anchor and size this
	...
	/**
	 * Creates and returns a {@link PaintContext} used to generate a tiled image
	 * pattern. See the {@link Paint#createContext specification} of the method
	 * in the {@link Paint} interface for information on null parameter
	 * handling.
	 *
	 * @param cm
	 ...
	 * 
	 * @return the transparency mode for this <code>TexturePaint</code> as an
	 *         integer value.
	 * @see Transparency
	 */
