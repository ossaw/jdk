...
class SequencedEvent extends AWTEvent implements ActiveEvent {
    ...
    /**
     * Constructs a new SequencedEvent which will dispatch the specified nested
     * event.
     *
     * @param nested
     ...
     * dispatched or disposed. If this method is invoked before all previous
    ...
    /**
     * Sequenced events are dispatched in order, so we cannot dispatch until we
     * are the first sequenced event in the queue (i.e. it's our turn). But
     * while we wait for our turn to dispatch, the event could have been
     * disposed for a number of reasons.
     */
    ...
    /*
     * Disposes all events from disposed AppContext return first valid event
     */
     ...
     * on the list. The locks should be acquired in the same order.
     */
    final void dispose() {
        synchronized (SequencedEvent.class) {
            if (disposed) {
