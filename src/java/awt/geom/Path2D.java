...
public abstract class Path2D implements Shape, Cloneable {
     ...
     *        the winding rule
     * @param initialTypes
     *        the size to make the initial array to store the path
     *        segment
     *        types
     * @since 1.6
     */
    ...
    public static class Float extends Path2D implements Serializable {
         ...
         * @since 1.6
         */
        public final synchronized void curveTo(double x1, double y1, double x2, double y2, double x3,
                double y3) {
            needRoom(true, 6);
                ...
                switch (pointTypes[i]) {
                    case PathIterator.SEG_MOVETO:
                        if (curx != movx || cury != movy) {
                            crossings = Curve.rectCrossingsForLine(crossings, rxmin, rymin, rxmax, rymax,
                                    curx, cury, movx, movy);
                        }
                    ...
                    case PathIterator.SEG_LINETO:
                        ...
                        break;
                    case PathIterator.SEG_QUADTO:
                        crossings = Curve.rectCrossingsForQuad(crossings, rxmin, rymin, rxmax, rymax, curx,
                                cury, coords[ci++], coords[ci++], endx = coords[ci++], endy = coords[ci++],
                                0);
                        curx = endx;
                        cury = endy;
                        break;
                    case PathIterator.SEG_CUBICTO:
                        crossings = Curve.rectCrossingsForCubic(crossings, rxmin, rymin, rxmax, rymax, curx,
                                cury, coords[ci++], coords[ci++], coords[ci++], coords[ci++],
         ...
         * {@code ObjectOutputStream} followed by an explicit serialization of
         * the path segments stored in this path.
         *
         ...
         *             <li>followed by {@code NP} (or unlimited if
         *             {@code NP < 0}) sets of values consisting of a single
         *             byte indicating a path segment type followed by one or
         *             more pairs of float or double values representing the
         ...
         * {@code ObjectInputStream} followed by an explicit serialization of
         * the path segments stored in this path.
         * <p>
