 ...
 * <code>ScrollPaneAdjustable</code> objects (one for each dimension) which
 * implement the <code>Adjustable</code> interface. The API provides methods to
 * access those objects such that the attributes on the Adjustable object (such
 * as unitIncrement, value, etc.) can be manipulated.
 * <P>
 ...
 * This policy is useful if the program needs to create and manage its own
 * adjustable controls.
 * <P>
 ...
 * This can be disabled using <code>setWheelScrollingEnabled</code>. Wheel
 * scrolling can be customized by setting the block and unit increment of the
 * horizontal and vertical Adjustables. For information on how mouse wheel
 * events are dispatched, see the class description for {@link MouseWheelEvent}.
 * <P>
 * Insets are used to define any space used by scrollbars and any borders
 * created by the scroll pane. getInsets() can be used to get the current value
 * for the insets. If the value of scrollbarsAlwaysVisible is false, then the
 * value of the insets will change dynamically depending on whether the
 * scrollbars are currently visible or not.
 *
 * @author Tom Ball
 * @author Amy Fowler
 * @author Tim Prinzing
...
public class ScrollPane extends Container implements Accessible {
    ...
    /**
     * There are 3 ways in which a scroll bar can be displayed. This integer
     * will represent one of these 3 displays - (SCROLLBARS_ALWAYS,
     * SCROLLBARS_AS_NEEDED, SCROLLBARS_NEVER)
     *
     * @serial
    ...
    /**
     * An adjustable vertical scrollbar. It is important to note that you must
     * <em>NOT</em> call 3 <code>Adjustable</code> methods, namely:
    ...
    /**
     * An adjustable horizontal scrollbar. It is important to note that you must
     * <em>NOT</em> call 3 <code>Adjustable</code> methods, namely:
     * <code>setMinimum()</code>, <code>setMaximum()</code>,
     * <code>setVisibleAmount()</code>.
     *
    ...
    /**
     * Adds the specified component to this scroll pane container. If the scroll
     * pane has an existing child component, that component is removed and the
     * new one is added.
     * 
     * @param comp
    ...
    /**
     * Returns the height that would be occupied by a horizontal scrollbar,
     * which is independent of whether it is currently displayed by the scroll
     * pane or not.
     * 
     ...
     * is independent of whether it is currently displayed by the scroll pane or
     * not.
     * 
    ...
    /**
     * Returns the <code>ScrollPaneAdjustable</code> object which represents the
     * state of the vertical scrollbar. The declared return type of this method
     * is <code>Adjustable</code> to maintain backward compatibility.
     * 
    ...
    /**
     * Returns the <code>ScrollPaneAdjustable</code> object which represents the
     * state of the horizontal scrollbar. The declared return type of this
     * method is <code>Adjustable</code> to maintain backward compatibility.
     * 
     * @see java.awt.ScrollPaneAdjustable
     */
     ...
     * this method is only valid if the scroll pane contains a child. Specifying
     * a position outside of the legal scrolling bounds of the child will scroll
     * to the closest legal position. Legal bounds are defined to be the
     * rectangle: x = 0, y = 0, width = (child width - view port width), height
     * = (child height - view port height). This is a convenience method which
     ...
     * specified position is within legal scrolling bounds of the child.
     * Specifying a position outside of the legal scrolling bounds of the child
     * will scroll to the closest legal position. Legal bounds are defined to be
     * the rectangle: x = 0, y = 0, width = (child width - view port width),
     * height = (child height - view port height). This is a convenience method
     * which interfaces with the Adjustable objects which represent the state of
     * the scrollbars.
     * 
     * @param p
    ...
    /**
     * Lays out this container by resizing its child to its preferred size. If
     * the new preferred size of the child causes the current scroll position to
     * be invalid, the scroll position is set to the closest valid position.
     *
    ...
    /**
     * Determine the size to allocate the child component. If the viewport area
     * is bigger than the preferred size of the child then the child is
     * allocated enough to fill the viewport, otherwise the child is given it's
     * preferred size.
     */
    ...
    public String paramString() {
        ...
        Insets i = getInsets();
        return super.paramString() + ",ScrollPosition=(" + p.x + "," + p.y + ")" + ",Insets=(" + i.top + ","
                + i.left + "," + i.bottom + "," + i.right + ")" + ",ScrollbarDisplayPolicy=" + sdpStr
    ...
    /**
     * Gets the AccessibleContext associated with this ScrollPane. For scroll
     * panes, the AccessibleContext takes the form of an
     * AccessibleAWTScrollPane. A new AccessibleAWTScrollPane instance is
     * created if necessary.
     *
     * @return an AccessibleAWTScrollPane that serves as the AccessibleContext
     *         of this ScrollPane
     * @since 1.3
     */
     ...
     * <code>ScrollPane</code> class. It provides an implementation of the Java
     * Accessibility API appropriate to scroll pane user-interface elements.
     * 
     * @since 1.3
     */
...
/*
 * In JDK 1.1.1, the pkg private class java.awt.PeerFixer was moved to become an
 * inner class of ScrollPane, which broke serialization for ScrollPane objects
 * using JDK 1.1. Instead of moving it back out here, which would break all JDK
 * 1.1.x releases, we keep PeerFixer in both places. Because of the scoping
 * rules, the PeerFixer that is used in ScrollPane will be the one that is the
 * inner class. This pkg private PeerFixer class below will only be used if the
 * Java 2 platform is used to deserialize ScrollPane objects that were
 * serialized using JDK1.1
 */
class PeerFixer implements AdjustmentListener, java.io.Serializable {
    /*
     * serialVersionUID
