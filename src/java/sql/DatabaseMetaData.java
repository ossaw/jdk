 ...
 * driver based on JDBC&trade; technology ("JDBC driver") that is used with it.
...
public interface DatabaseMetaData extends Wrapper {
	 ...
	 * by the method <code>getProcedures</code>.
	 *
	...
	/**
	 * Retrieves whether <code>NULL</code> values are sorted high. Sorted high
	 * means that <code>NULL</code> values sort higher than any other value in a
	 * domain. In an ascending order, if this method returns <code>true</code>,
	 * <code>NULL</code> values will appear at the end. By contrast, the method
	 * <code>nullsAreSortedAtEnd</code> indicates whether <code>NULL</code>
	...
	/**
	 * Retrieves whether <code>NULL</code> values are sorted low. Sorted low
	 * means that <code>NULL</code> values sort lower than any other value in a
	 * domain. In an ascending order, if this method returns <code>true</code>,
	 * <code>NULL</code> values will appear at the beginning. By contrast, the
	 * method <code>nullsAreSortedAtStart</code> indicates whether
	 * <code>NULL</code> values are sorted at the beginning regardless of sort
	 * order.
	 *
	 * @return <code>true</code> if so; <code>false</code> otherwise
	 * @exception SQLException
	...
	/**
	 * Retrieves whether this database supports <code>ALTER TABLE</code> with
	 * add column.
	 *
	 * @return <code>true</code> if so; <code>false</code> otherwise
	 * @exception SQLException
	 ...
	 * <code>CONVERT</code> for the conversion of one JDBC type to another. The
	 * JDBC types are the generic SQL data types defined in
	 ...
	 * <i>fromType</i> and <i>toType</i>. The JDBC types are the generic SQL
	 ...
	 * the <code>SELECT</code> statement in an <code>ORDER BY</code> clause.
	 *
	 * @return <code>true</code> if so; <code>false</code> otherwise
	 * @exception SQLException
	 *                if a database access error occurs
	 ...
	 * the <code>SELECT</code> statement in a <code>GROUP BY</code> clause
	 * provided that all of the columns in the <code>SELECT</code> statement are
	 * included in the <code>GROUP BY</code> clause.
	 *
	 * @return <code>true</code> if so; <code>false</code> otherwise
	 * @exception SQLException
	 *                if a database access error occurs
	 ...
	 * the method <code>commit</code> is a noop, and the isolation level is
	 ...
	 * designated "identifier" column of a typed table (may be <code>null</code>
	 * )
	 ...
	 *            types returned from {@link #getTableTypes},to include;
	 ...
	 * <LI><B>DECIMAL_DIGITS</B> short {@code =>} scale - Null is returned for
	...
	/**
	 * Indicates that the scope of the best row identifier is the remainder of
	 ...
	 * imported
	 * <LI><B>FKTABLE_CAT</B> String {@code =>} foreign key table catalog (may
	 * be <code>null</code>)
	 * <LI><B>FKTABLE_SCHEM</B> String {@code =>} foreign key table schema (may
	 * be <code>null</code>)
	 * <LI><B>FKTABLE_NAME</B> String {@code =>} foreign key table name
	 * <LI><B>FKCOLUMN_NAME</B> String {@code =>} foreign key column name
	 * <LI><B>KEY_SEQ</B> short {@code =>} sequence number within a foreign key(
	 ...
	 * <LI><B>UPDATE_RULE</B> short {@code =>} What happens to a foreign key
	 ...
	 * compatibility)
	 * </UL>
	 * <LI><B>DELETE_RULE</B> short {@code =>} What happens to the foreign key
	 ...
	 * <LI><B>PK_NAME</B> String {@code =>} primary key name (may be
	 * <code>null</code>)
	 * <LI><B>DEFERRABILITY</B> short {@code =>} can the evaluation of foreign
	 ...
	 * key is updated, the foreign key (imported key) is changed to agree with
	 * it. For the column <code>DELETE_RULE</code>, it indicates that when the
	 * primary key is deleted, rows that imported that key are deleted.
	 * <P>
	 * A possible value for the columns <code>UPDATE_RULE</code> and
	 * <code>DELETE_RULE</code> in the <code>ResultSet</code> objects returned
	 * by the methods <code>getImportedKeys</code>, <code>getExportedKeys</code>
	 * , and <code>getCrossReference</code>.
	 */
	int importedKeyCascade = 0;

	/**
	 * For the column <code>UPDATE_RULE</code>, indicates that a primary key may
	 * not be updated if it has been imported by another table as a foreign key.
	 * For the column <code>DELETE_RULE</code>, indicates that a primary key may
	 * not be deleted if it has been imported by another table as a foreign key.
	 * <P>
	 * A possible value for the columns <code>UPDATE_RULE</code> and
	 * <code>DELETE_RULE</code> in the <code>ResultSet</code> objects returned
	 * by the methods <code>getImportedKeys</code>, <code>getExportedKeys</code>
	 ...
	 * indicates that when the primary key is updated or deleted, the foreign
	 * key (imported key) is changed to <code>NULL</code>.
	 * <P>
	 * A possible value for the columns <code>UPDATE_RULE</code> and
	 * <code>DELETE_RULE</code> in the <code>ResultSet</code> objects returned
	 * by the methods <code>getImportedKeys</code>, <code>getExportedKeys</code>
	...
	/**
	 * For the columns <code>UPDATE_RULE</code> and <code>DELETE_RULE</code>,
	 * indicates that if the primary key has been imported, it cannot be updated
	 * or deleted.
	 * <P>
	 * A possible value for the columns <code>UPDATE_RULE</code> and
	 * <code>DELETE_RULE</code> in the <code>ResultSet</code> objects returned
	 ...
	 * <OL>
	 * <LI><B>PKTABLE_CAT</B> String {@code =>} primary key table catalog (may
	 * be <code>null</code>)
	 * <LI><B>PKTABLE_SCHEM</B> String {@code =>} primary key table schema (may
	 * be <code>null</code>)
	 * <LI><B>PKTABLE_NAME</B> String {@code =>} primary key table name
	 * <LI><B>PKCOLUMN_NAME</B> String {@code =>} primary key column name
	 * <LI><B>FKTABLE_CAT</B> String {@code =>} foreign key table catalog (may
	 * be <code>null</code>) being exported (may be <code>null</code>)
	 * <LI><B>FKTABLE_SCHEM</B> String {@code =>} foreign key table schema (may
	 * be <code>null</code>) being exported (may be <code>null</code>)
	 ...
	 * value of 1 represents the first column of the foreign key, a value of 2
	 * would represent the second column within the foreign key).
	 ...
	 * <code>null</code>)
	 * <LI><B>PKTABLE_SCHEM</B> String {@code =>} parent key table schema (may
	 * be <code>null</code>)
	 ...
	 * imported
	 * <LI>importedKeyCascade - change imported key to agree with parent key
	 * update
	 * <LI>importedKeySetNull - change imported key to <code>NULL</code> if its
	 ...
	 * </UL>
	 * <LI><B>DELETE_RULE</B> short {@code =>} What happens to the foreign key
	 * when parent key is deleted.
	 * <UL>
	 * <LI>importedKeyNoAction - do not allow delete of parent key if it has
	 * been imported
	 * <LI>importedKeyCascade - delete rows that import a deleted key
	 * <LI>importedKeySetNull - change imported key to <code>NULL</code> if its
	 * primary key has been deleted
	 * <LI>importedKeyRestrict - same as importedKeyNoAction (for ODBC 2.x
	 * compatibility)
	 * <LI>importedKeySetDefault - change imported key to default if its parent
	 * key has been deleted
	 * </UL>
	 * <LI><B>FK_NAME</B> String {@code =>} foreign key name (may be
	 * <code>null</code>)
	 ...
	 *            a schema name; must match the schema name as it is stored in
	 *            the database; "" retrieves those without a schema;
	 *            <code>null</code> means drop schema name from the selection
	 *            criteria
	 * @param parentTable
	 ...
	 * <LI><B>LITERAL_PREFIX</B> String {@code =>} prefix used to quote a
	 * literal (may be <code>null</code>)
	 * <LI><B>LITERAL_SUFFIX</B> String {@code =>} suffix used to quote a
	 * literal (may be <code>null</code>)
	 ...
	 * <LI><B>LOCAL_TYPE_NAME</B> String {@code =>} localized version of type
	 ...
	 * precision. For character data, this is the length in characters. For
	 ...
	 * ROWID datatype, this is the length in bytes. Null is returned for data
	...
	/**
	 * Indicates that a <code>NULL</code> value is NOT allowed for this data
	 * type.
	 * <P>
	...
	/**
	 * Indicates that it is not known whether a <code>NULL</code> value is
	...
	/**
	 * Indicates that <code>WHERE</code> search clauses are not supported for
	 * this type.
	 * <P>
	 ...
	 * search clauses that use <code>LIKE</code> predicates.
	 * <P>
	 * A possible value for column <code>SEARCHABLE</code> in the
	 * <code>ResultSet</code> object returned by the method
	 * <code>getTypeInfo</code>.
	...
	/**
	 * Indicates that the data type can be only be used in <code>WHERE</code>
	 * search clauses that do not use <code>LIKE</code> predicates.
	 * <P>
	 * A possible value for column <code>SEARCHABLE</code> in the
	 * <code>ResultSet</code> object returned by the method
	 * <code>getTypeInfo</code>.
	 ...
	 * the method <code>ResultSet.rowDeleted</code>. If the method
	 ...
	 * DISTINCT type or the type that implements the user-generated reference
	 * type of the SELF_REFERENCING_COLUMN of a structured type as defined in
	 ...
	 *            DISTINCT) to include; <code>null</code> returns all types
	 ...
	 *         <p>
	 *         If <code>true</code> is returned, the JDBC driver must support
	 *         the returning of auto-generated keys for at least SQL INSERT
	 *         statements
	 *         <p>
	 * @exception SQLException
	 *                if a database access error occurs
	 ...
	 * <code>TYPE_CAT</code>, <code>TYPE_SCHEM</code>, <code>TYPE_NAME</code>
	 * and <code>ORDINAL_POSITION</code>. This description does not contain
	 * inherited attributes.
	 * <P>
	 * The <code>ResultSet</code> object that is returned has the following
	 * columns:
	 * <OL>
	 * <LI><B>TYPE_CAT</B> String {@code =>} type catalog (may be
	 * <code>null</code>)
	 ...
	 * name. For a UDT, the type name is fully qualified. For a REF, the type
	 * name is fully qualified and represents the target type of the reference
	 * type.
	 * <LI><B>ATTR_SIZE</B> int {@code =>} column size. For char or date types
	 ...
	 * <LI><B>CHAR_OCTET_LENGTH</B> int {@code =>} for char types the maximum
	 * number of bytes in the column
	 * <LI><B>ORDINAL_POSITION</B> int {@code =>} index of the attribute in the
	 * UDT (starting at 1)
	 * <LI><B>IS_NULLABLE</B> String {@code =>} ISO rules are used to determine
	 ...
	 * <LI><B>SCOPE_SCHEMA</B> String {@code =>} schema of table that is the
	 * scope of a reference attribute (<code>null</code> if DATA_TYPE isn't REF)
	 * <LI><B>SCOPE_TABLE</B> String {@code =>} table name that is the scope of
	 * a reference attribute (<code>null</code> if the DATA_TYPE isn't REF)
	 * <LI><B>SOURCE_DATA_TYPE</B> short {@code =>} source type of a distinct
	 * type or user-generated Ref type,SQL type from java.sql.Types (
	 * <code>null</code> if DATA_TYPE isn't DISTINCT or user-generated REF)
	 * </OL>
	 ...
	 * <code>SQLException.getSQLState</code> is an X/Open (now know as Open
	 * Group) SQL CLI SQLSTATE value.
	 * <P>
	 * 
	 * @since 1.4
	 */
	 ...
	 * Developers should use the constant <code>sqlStateSQL</code> instead.
	 *
	 * @since 1.4
	 */
	 ...
	 * <code>ROWID</code> type, and if so the lifetime for which a
	 * <code>RowId</code> object remains valid.
	 * <p>
	 ...
	 * <code>true</code> indicates that all open ResultSets are closed, even
	 * ones that are holdable. When a <code>SQLException</code> occurs while
	 * autocommit is <code>true</code>, it is vendor specific whether the JDBC
	 * driver responds with a commit operation, a rollback operation, or by
	 * doing neither a commit nor a rollback. A potential result of this
	 * difference is in whether or not holdable ResultSets are closed.
	 *
	 * @return <code>true</code> if so; <code>false</code> otherwise
	 * @exception SQLException
	 *                if a database access error occurs
	 ...
	 * This will typically contain information as to where this property is
	 * stored in the database.
	 * </ol>
	 * <p>
	 ...
	 * <LI><B>ORDINAL_POSITION</B> int {@code =>} the ordinal position, starting
	 * from 1, for the input and output parameters. A value of 0 is returned if
	 * this row describes the function's return value. For result set columns,
	 * it is the ordinal position of the column in the result set starting from
	 * 1.
	 * <LI><B>IS_NULLABLE</B> String {@code =>} ISO rules are used to determine
	 ...
	 *            used to narrow the search
	 * @param functionNamePattern
	 *            a procedure name pattern; must match the function name as it
	 *            is stored in the database
	 * @param columnNamePattern
	 ...
	 * column. The value returned will correspond to the enum name returned by
