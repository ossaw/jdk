 ...
 * identify this as an SQL <code>TIMESTAMP</code> value. It adds the ability to
 * hold the SQL <code>TIMESTAMP</code> fractional seconds value, by allowing the
 ...
 * <ul>
 * <li><code>19 </code>, which is the number of characters in yyyy-mm-dd
 * hh:mm:ss
 * <li><code> 20 + s </code>, which is the number of characters in the
 * yyyy-mm-dd hh:mm:ss.[fff...] and <code>s</code> represents the scale of the
 * given Timestamp, its fractional seconds precision.
 * </ul>
 *
 * <P>
 * <B>Note:</B> This type is a composite of a <code>java.util.Date</code> and a
 * separate nanoseconds value. Only integral seconds are stored in the
 * <code>java.util.Date</code> component. The fractional seconds - the nanos -
 * are separate. The <code>Timestamp.equals(Object)</code> method never returns
 * <code>true</code> when passed an object that isn't an instance of
 * <code>java.sql.Timestamp</code>, because the nanos component of a date is
 * unknown. As a result, the <code>Timestamp.equals(Object)</code> method is not
 * symmetric with respect to the <code>java.util.Date.equals(Object)</code>
 * method. Also, the <code>hashCode</code> method uses the underlying
 * <code>java.util.Date</code> implementation and therefore does not include
 * nanos in its computation.
 * <P>
 * Due to the differences between the <code>Timestamp</code> class and the
 * <code>java.util.Date</code> class mentioned above, it is recommended that
 * code not view <code>Timestamp</code> values generically as an instance of
 * <code>java.util.Date</code>. The inheritance relationship between
 * <code>Timestamp</code> and <code>java.util.Date</code> really denotes
 * implementation inheritance, and not type inheritance.
 */
public class Timestamp extends java.util.Date {

	/**
	 * Constructs a <code>Timestamp</code> object initialized with the given
	 * values.
	 *
	 * @param year
	 *               the year minus 1900
	 ...
	 *             <code>Timestamp(long millis)</code>
	 ...
	 * fractional seconds are stored in the <code>nanos</code> field of the
	 ...
	 * is <tt>time</tt> milliseconds after January 1, 1970 00:00:00 GMT.
	 *
	 * @param time
	 ...
	 * @param s
	 *          timestamp in format
	 *          <code>yyyy-[m]m-[d]d hh:mm:ss[.f...]</code>. The fractional
	 *          seconds may be omitted. The leading zero for <code>mm</code>
	...
	public static Timestamp valueOf(String s) {
		...
		if ((firstDash > 0) && (secondDash > 0) && (secondDash < date_s.length()
				- 1)) {
		...
		if ((firstColon > 0) & (secondColon > 0) & (secondColon < time_s
				.length() - 1)) {
		...
		return new Timestamp(year - 1900, month - 1, day, hour, minute, second,
				a_nanos);
	}
	...
	public static Timestamp valueOf(LocalDateTime dateTime) {
		return new Timestamp(dateTime.getYear() - 1900, dateTime.getMonthValue()
				- 1, dateTime.getDayOfMonth(), dateTime.getHour(), dateTime
						.getMinute(), dateTime.getSecond(), dateTime.getNano());
	}
	...
	public LocalDateTime toLocalDateTime() {
		return LocalDateTime.of(getYear() + 1900, getMonth() + 1, getDate(),
	...
	public static Timestamp from(Instant instant) {
		try {
			Timestamp stamp = new Timestamp(instant.getEpochSecond()
					* MILLIS_PER_SECOND);
	...
	public Instant toInstant() {
		return Instant.ofEpochSecond(super.getTime() / MILLIS_PER_SECOND,
				nanos);
	}
}
