...
public interface CallableStatement extends PreparedStatement {
     ...
     * to be of JDBC type <code>sqlType</code>. All OUT parameters must be
     ...
     * object returned has exactly the same value the SQL <code>CHAR</code>
     ...
     * as a <code>java.math.BigDecimal</code> object with <i>scale</i> digits to
     ...
     * <code>NULL</code>, the driver returns a Java <code>null</code>.
     * <p>
     ...
     * parameter should also be given, while a <code>REF</code> parameter
     * requires that the fully-qualified type name of the referenced type be
     * given. A JDBC driver that does not need the type code and type name
     * information may ignore it. To be portable, however, applications should
     * always provide these values for user-defined and <code>REF</code>
     * parameters.
     *
     ...
     * getter method whose Java type corresponds to the parameter's registered
     * SQL type.
     *
     * @param parameterIndex
     ...
     * type <code>sqlType</code>. All OUT parameters must be registered before a
     ...
     * for user-named and REF parameters.
     *
     * Although it is intended for user-named and REF parameters, this method
     * may be used to register a parameter of any JDBC type. If the parameter
     * does not have a user-named or REF type, the typeName parameter is
     * ignored.
     *
     * <P>
     * <B>Note:</B> When reading the value of an out parameter, you must use the
     ...
     * parameter as a {@link java.sql.Ref} object in the Java programming
     * language.
     *
     * @param parameterName
     *                      the name of the parameter
    ...
    /**
     * Retrieves the value of the designated JDBC <code>ROWID</code> parameter
     ...
     * <code>String</code> in the Java programming language.
     * <p>
     * For the fixed-length type JDBC <code>NCHAR</code>, the
     * <code>String</code> object returned has exactly the same value the SQL
     * <code>NCHAR</code> value had in the database, including any padding added
     * by the database.
     *
     * @param parameterIndex
     *                       index of the first parameter is 1, the second is 2,
     ...
     * <code>java.io.Reader</code> object in the Java programming language. It
     ...
     * the type, a <code>SQLException</code> is thrown.
     * <p>
     ...
     * the JDBC type {@code sqlType}. All OUT parameters must be registered
     * before a stored procedure is executed.
     * <p>
     * The JDBC type specified by {@code sqlType} for an OUT parameter
     * determines the Java type that must be used in the {@code get} method to
     * read the value of that parameter.
     * <p>
     * If the JDBC type expected to be returned to this output parameter is
     * specific to this particular database, {@code sqlType} may be
     * {@code JDBCType.OTHER} or a {@code SQLType} that is supported by the JDBC
     * driver. The method {@link #getObject} retrieves the value.
     * <P>
     * The default implementation will throw
     * {@code SQLFeatureNotSupportedException}
     *
    ...
    /**
     * Registers the parameter in ordinal position {@code parameterIndex} to be
     * of JDBC type {@code sqlType}. All OUT parameters must be registered
     * before a stored procedure is executed.
     * <p>
     * The JDBC type specified by {@code sqlType} for an OUT parameter
     * determines the Java type that must be used in the {@code get} method to
     ...
     * {@code STRUCT}, {@code DISTINCT}, {@code JAVA_OBJECT}, and named array
     * types.
     * <p>
     * All OUT parameters must be registered before a stored procedure is
     * executed.
     * <p>
     * For a user-defined parameter, the fully-qualified SQL type name of the
     * parameter should also be given, while a {@code REF} parameter requires
     ...
     * provide these values for user-defined and {@code REF} parameters.
     *
     * Although it is intended for user-defined and {@code REF} parameters, this
     * method may be used to register a parameter of any JDBC type. If the
     * parameter does not have a user-defined or {@code REF} type, the
     * <i>typeName</i> parameter is ignored.
     *
     * <P>
     * <B>Note:</B> When reading the value of an out parameter, you must use the
    ...
    /**
     * Registers the OUT parameter named <code>parameterName</code> to the JDBC
     * type {@code sqlType}. All OUT parameters must be registered before a
     * stored procedure is executed.
     * <p>
     * The JDBC type specified by {@code sqlType} for an OUT parameter
     * determines the Java type that must be used in the {@code get} method to
     ...
     * <p>
     * If the JDBC type expected to be returned to this output parameter is
     * specific to this particular database, {@code sqlType} should be
     * {@code JDBCType.OTHER} or a {@code SQLType} that is supported by the JDBC
     * driver.. The method {@link #getObject} retrieves the value.
     * <P>
     * The default implementation will throw
    ...
    /**
     * Registers the designated output parameter. This version of the method
     * {@code  registerOutParameter} should be used for a user-named or REF
     * output parameter. Examples of user-named types include: STRUCT, DISTINCT,
     * JAVA_OBJECT, and named array types.
     * <p>
     * All OUT parameters must be registered before a stored procedure is
