...
/**
 * The representation (mapping) in the Java&trade; programming language of an
 * SQL <code>BLOB</code> value. An SQL <code>BLOB</code> is a built-in type that
 * stores a Binary Large Object as a column value in a row of a database table.
 * By default drivers implement <code>Blob</code> using an SQL
 * <code>locator(BLOB)</code>, which means that a <code>Blob</code> object
 ...
 * <p>
 * All methods on the <code>Blob</code> interface must be fully implemented if
...
public interface Blob {
	 ...
	 * @exception SQLException
	 *                if there is an error accessing the <code>BLOB</code> or if
	 ...
	 * <code>pos</code>, and returns the number of bytes written. The array of
	 * bytes will overwrite the existing bytes in the <code>Blob</code> object
	 * starting at the position <code>pos</code>. If the end of the
	 * <code>Blob</code> value is reached while writing the array of bytes, then
	 * the length of the <code>Blob</code> value will be increased to
	 ...
	 * from the given byte array are written. The array of bytes will overwrite
	 * the existing bytes in the <code>Blob</code> object starting at the
	 * position <code>pos</code>. If the end of the <code>Blob</code> value is
	 * reached while writing the array of bytes, then the length of the
	 * <code>Blob</code> value will be increased to accommodate the extra bytes.
	 * <p>
	 * <b>Note:</b> If the value specified for <code>pos</code> is greater then
	 * the length+1 of the <code>BLOB</code> value then the behavior is
	 * undefined. Some JDBC drivers may throw a <code>SQLException</code> while
	 ...
	 * position <code>pos</code>. The bytes written to the stream will overwrite
	 * the existing bytes in the <code>Blob</code> object starting at the
	 * position <code>pos</code>. If the end of the <code>Blob</code> value is
	 * reached while writing to the stream, then the length of the
	 * <code>Blob</code> value will be increased to accommodate the extra bytes.
	 * <p>
	 * <b>Note:</b> If the value specified for <code>pos</code> is greater then
	 * the length+1 of the <code>BLOB</code> value then the behavior is
