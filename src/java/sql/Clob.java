...
/**
 * The mapping in the Java&trade; programming language for the SQL
 * <code>CLOB</code> type. An SQL <code>CLOB</code> is a built-in type that
 * stores a Character Large Object as a column value in a row of a database
 ...
 * contains a logical pointer to the SQL <code>CLOB</code> data rather than the
 * data itself. A <code>Clob</code> object is valid for the duration of the
 * transaction in which it was created.
 * <P>
 * The <code>Clob</code> interface provides methods for getting the length of an
 * SQL <code>CLOB</code> (Character Large Object) value, for materializing a
 ...
 * <p>
 * All methods on the <code>Clob</code> interface must be fully implemented if
...
public interface Clob {
     ...
     * <code>pos</code>. The string will overwrite the existing characters in
     * the <code>Clob</code> object starting at the position <code>pos</code>.
     * If the end of the <code>Clob</code> value is reached while writing the
     * given string, then the length of the <code>Clob</code> value will be
     * increased to accommodate the extra characters.
     * <p>
     * <b>Note:</b> If the value specified for <code>pos</code> is greater then
     * the length+1 of the <code>CLOB</code> value then the behavior is
     ...
     * <code>Clob</code> represents. The string will overwrite the existing
     * characters in the <code>Clob</code> object starting at the position
     ...
     * will overwrite the existing characters in the <code>Clob</code> object
     * starting at the position <code>pos</code>. If the end of the
     * <code>Clob</code> value is reached while writing characters to the
     * stream, then the length of the <code>Clob</code> value will be increased
     ...
     * represents, at position <code>pos</code>. Characters written to the
     * stream will overwrite the existing characters in the <code>Clob</code>
     * object starting at the position <code>pos</code>. If the end of the
     * <code>Clob</code> value is reached while writing characters to the
     * stream, then the length of the <code>Clob</code> value will be increased
     * to accommodate the extra characters.
     * <p>
