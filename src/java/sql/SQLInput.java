...
/**
 * An input stream that contains a stream of values representing an instance of
 * an SQL structured type or an SQL distinct type. This interface, used only for
 * custom mapping, is used by the driver behind the scenes, and a programmer
 * never directly invokes <code>SQLInput</code> methods. The <i>reader</i>
 * methods (<code>readLong</code>, <code>readBytes</code>, and so on) provide a
 * way for an implementation of the <code>SQLData</code> interface to read the
 * values in an <code>SQLInput</code> object. And as described in
 * <code>SQLData</code>, calls to reader methods must be made in the order that
 * their corresponding attributes appear in the SQL definition of the type. The
 * method <code>wasNull</code> is used to determine whether the last value read
 * was SQL <code>NULL</code>.
 * <P>
 ...
 * method <code>SQLData.getSQLType</code> to determine the SQL type of the
 * user-defined type (UDT) being custom mapped. The driver creates an instance
 * of <code>SQLInput</code>, populating it with the attributes of the UDT. The
 * driver then passes the input stream to the method
 * <code>SQLData.readSQL</code>, which in turn calls the <code>SQLInput</code>
 * reader methods in its implementation for reading the attributes from the
 * input stream.
 * 
 * @since 1.2
 */
...
public interface SQLInput {
    ...
    /**
     * Reads the next attribute in the stream and returns it as a
     * <code>java.math.BigDecimal</code> object in the Java programming
     * language.
     *
     * @return the attribute; if the value is SQL <code>NULL</code>, returns
     *         <code>null</code>
    ...
    /**
     * Reads the next attribute in the stream and returns it as an array of
    ...
    /**
     * Reads the next attribute in the stream and returns it as a stream of
     * Unicode characters.
     *
     * @return the attribute; if the value is SQL <code>NULL</code>, returns
     *         <code>null</code>
     ...
     * <code>Object</code> in the Java programming language. The actual type of
     * the object returned is determined by the default type mapping, and any
     ...
     * <P>
     * When the datum at the head of the stream is an SQL <code>NULL</code>, the
     * method returns <code>null</code>. If the datum is an SQL structured or
     * distinct type, it determines the SQL type of the datum at the head of the
     * stream. If the stream's type map has an entry for that SQL type, the
     * driver constructs an object of the appropriate class and calls the method
     * <code>SQLData.readSQL</code> on that object, which reads additional data
     * from the stream, using the protocol described for that method.
     *
     * @return the datum at the head of the stream as an <code>Object</code> in
     *         the Java programming language;<code>null</code> if the datum is
     *         SQL <code>NULL</code>
     * @exception SQLException
     *            if a database access error
     *            occurs
     ...
     *         value at the head of the stream; <code>null</code> if the value
     *         read is SQL <code>NULL</code>
     * @exception SQLException
     *            if a database access error
     *            occurs
     * @exception SQLFeatureNotSupportedException
     ...
     *
     * @return a <code>Blob</code> object representing data of the SQL
     *         <code>BLOB</code> value at the head of the stream;
     ...
     *
     * @return a <code>Clob</code> object representing data of the SQL
