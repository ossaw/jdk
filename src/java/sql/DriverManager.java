...
/**
 * <P>
 * The basic service for managing a set of JDBC drivers.<br>
 * <B>NOTE:</B> The {@link javax.sql.DataSource} interface, new in the JDBC 2.0
 * API, provides another way to connect to a data source. The use of a
 * <code>DataSource</code> object is the preferred means of connecting to a data
 * source.
 *
 * <P>
 * As part of its initialization, the <code>DriverManager</code> class will
 * attempt to load the driver classes referenced in the "jdbc.drivers" system
 ...
 * <code>getDrivers</code> have been enhanced to support the Java Standard
 * Edition
 * <a href="../../../technotes/guides/jar/jar.html#Service%20Provider">Service
 * Provider</a> mechanism. JDBC 4.0 Drivers must include the file
 * <code>META-INF/services/java.sql.Driver</code>. This file contains the name
 * of the JDBC drivers implementation of <code>java.sql.Driver</code>. For
 * example, to load the <code>my.sql.Driver</code> class, the
 * <code>META-INF/services/java.sql.Driver</code> file would contain the entry:
 * 
 * <pre>
 ...
 * <code>Class.forName()</code>. Existing programs which currently load JDBC
 * drivers using <code>Class.forName()</code> will continue to work without
 * modification.
 *
 * <P>
 ...
 * be done with the tool PolicyTool, which can be used to grant <code>permission
...
public class DriverManager {
     ...
     * @since 1.8
     */
    final static SQLPermission DEREGISTER_DRIVER_PERMISSION = new SQLPermission(
            "deregisterDriver");
     ...
     *
     * The <code>getLogWriter</code> and <code>setLogWriter</code> methods
     * should be used instead of the <code>get/setlogStream</code> methods,
    ...
    /**
     * Sets the logging/tracing <code>PrintWriter</code> object that is used by
     * the <code>DriverManager</code> and all drivers.
     * <P>
     * There is a minor versioning problem created by the introduction of the
     * method <code>setLogWriter</code>. The method <code>setLogWriter</code>
     * cannot create a <code>PrintStream</code> object that will be returned by
     * <code>getLogStream</code>---the Java platform does not provide a backward
     * conversion. As a result, a new application that uses
     * <code>setLogWriter</code> and also uses a JDBC 1.0 driver that uses
     * <code>getLogStream</code> will likely not see debugging information
     * written by that driver.
     * <P>
     * Starting with the Java 2 SDK, Standard Edition, version 1.3 release, this
     * method checks to see that there is an <code>SQLPermission</code> object
     * before setting the logging stream. If a <code>SecurityManager</code>
     * exists and its <code>checkPermission</code> method denies setting the log
     * writer, this method throws a <code>java.lang.SecurityException</code>.
     *
     * @param out
     *            the new logging/tracing <code>PrintStream</code> object;
     ...
     * also specified in the {@code Properties} object, it is
     ...
     *
     * @param url
     *             a database url of the form <code> jdbc:<em>subprotocol</em>:
     *            <em>subname</em></code>
     * @param info
            ...
            if (isDriverAllowed(aDriver.driver, callerClass)) {
                try {
                    if (aDriver.driver.acceptsURL(url)) {
                        // Success!
                        println("getDriver returning " + aDriver.driver
                                .getClass().getName());
     ...
     * registered drivers.
     * <p>
     * If a {@code null} value is specified for the driver to be removed, then
     * no action is taken.
     * <p>
     ...
     * then no action is taken. If the driver was found, it will be removed from
     * the list of registered drivers.
     * <p>
     * If a {@code DriverAction} instance was specified when the JDBC driver was
     * registered, its deregister method will be called prior to the driver
     * being removed from the list of registered drivers.
     *
     * @param driver
     ...
     *                           method denies permission to deregister a
     *                           driver.
     *
     * @see SecurityManager#checkPermission
     */
    ...
    public static synchronized void deregisterDriver(Driver driver)
        ...
        if (registeredDrivers.contains(aDriver)) {
            if (isDriverAllowed(driver, Reflection.getCallerClass())) {
                DriverInfo di = registeredDrivers.get(registeredDrivers.indexOf(
                        aDriver));
                ...
                // If the caller does not have permission to load the driver
                // then
     ...
     *
     * @param seconds
     *                the login time limit in seconds; zero means there is no
     *                limit
     * @see #getLoginTimeout
     */
     ...
     * checks to see that there is an <code>SQLPermission</code> object before
     * setting the logging stream. If a <code>SecurityManager</code> exists and
     * its <code>checkPermission</code> method denies setting the log writer,
     * this method throws a <code>java.lang.SecurityException</code>.
     *
     * @param out
    ...
    public static void println(String message) {
        synchronized (logSync) {
            if (logWriter != null) {
                ...
                // automatic flushing is never enabled, so we must do it
                // ourselves
                logWriter.flush();
            }
        }
    ...
    private static void loadInitialDrivers() {
        ...
        AccessController.doPrivileged(new PrivilegedAction<Void>() {
            public Void run() {
                ...
                /*
                 * Load these drivers, so that they can be instantiated. It may
                 * be the case that the driver class may not be there i.e. there
                 * may be a packaged driver with the service class as
                 * implementation of java.sql.Driver but the actual class may be
                 * missing. In that case a java.util.ServiceConfigurationError
                 * will be thrown at runtime by the VM trying to locate and load
                 * the service.
                 * Adding a try catch block to catch those runtime errors if
                 * driver not available in classpath but it's packaged as
                 * service and that service is there in classpath.
                 */
                try {
    ...
    private static Connection getConnection(String url,
        ...
        /*
         * When callerCl is null, we should check the application's (which is
         * invoking this class indirectly) classloader, so that the JDBC driver
         * class outside rt.jar can be loaded from here.
         */
        ClassLoader callerCL = caller != null ? caller.getClassLoader() : null;
        ...
        // Walk through the loaded registeredDrivers attempting to make a
        // connection.
        ...
        for (DriverInfo aDriver : registeredDrivers) {
            ...
            if (isDriverAllowed(aDriver.driver, callerCL)) {
                try {
                    println("    trying " + aDriver.driver.getClass()
                            .getName());
                    Connection con = aDriver.driver.connect(url, info);
                    if (con != null) {
                        // Success!
                        println("getConnection returning " + aDriver.driver
                                .getClass().getName());
                        return (con);
                    }
