...
/**
 * The permission for which the <code>SecurityManager</code> will check when
 * code that is running an application with a <code>SecurityManager</code>
 * enabled, calls the {@code DriverManager.deregisterDriver} method,
 * <code>DriverManager.setLogWriter</code> method,
 * <code>DriverManager.setLogStream</code> (deprecated) method,
 * {@code SyncFactory.setJNDIContext} method, {@code SyncFactory.setLogger}
 * method, {@code Connection.setNetworktimeout} method, or the
 * <code>Connection.abort</code> method. If there is no
 * <code>SQLPermission</code> object, these methods throw a
 * <code>java.lang.SecurityException</code> as a runtime exception.
 * <P>
 * A <code>SQLPermission</code> object contains a name (also referred to as a
 * "target name") but no actions list; there is either a named permission or
 * there is not. The target name is the name of the permission (see below). The
 * naming convention follows the hierarchical property naming convention. In
 * addition, an asterisk may appear at the end of the name, following a ".", or
 * by itself, to signify a wildcard match. For example:
 * <code>loadLibrary.*</code> and <code>*</code> signify a wildcard match, while
 * <code>*loadLibrary</code> and <code>a*b</code> do not.
 * <P>
 ...
 * <td>This is a dangerous permission to grant. The contents of the log may
 ...
 * <td>Permits an application to terminate a physical connection to a database.
 * </td>
 * </tr>
 * <tr>
 ...
 * <code>Connection</code> or objects created from the <code>Connection</code>
 * will wait for the database to reply to any one request.</td>
 * <tr>
 * <td>deregisterDriver</td>
 * <td>Allows the invocation of the {@code DriverManager} method
 * {@code deregisterDriver}</td>
