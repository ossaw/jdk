 ...
 * <code>org.omg.PortableServer.Servant</code> class. It serves as the base
 * class for all POA servant implementations and provides a number of methods
 * that may be invoked by the application programmer, as well as methods which
 * are invoked by the POA itself and may be overridden by the user to control
 * aspects of servant behavior. Based on IDL to Java spec. (CORBA V2.3.1)
 * ptc/00-01-08.pdf.
 */
...
abstract public class Servant {
	 ...
	 * 
	 * @return <code>_delegate</code> the ORB vendor-specific implementation of
	 *         <code>PortableServer::Servant</code>.
	 */
	...
	/**
	 * Supports the Java ORB portability interfaces by providing a method for
	 * classes that support ORB portability through delegation to set their
	 * delegate.
	 * 
	 * @param delegate
	...
	/**
	 * Allows the servant to obtain the object reference for the target CORBA
	 * object it is incarnating for that request.
	 * 
	 * @return <code>this_object</code> <code>Object</code> reference associated
	 *         with the request.
	 */
	 ...
	 * <code>Servant</code> (convenience method).
	 * 
	 * @return <code>orb</code> the instance of the ORB currently associated
	 *         with the <code>Servant</code>.
	 */
	 ...
	 * 
	 * @return <code>object_id</code> the <code>Object</code> ID associated with
	 *         this servant.
	 */
	...
	/**
	 * Checks to see if the specified <code>repository_id</code> is present on
	 * the list returned by <code>_all_interfaces()</code> or is the
	...
	/**
	 * Checks for the existence of an <code>Object</code>. The
	 ...
	 * information that may be useful to a program. <code>Servant</code>
	 * provides a default implementation of <code>_get_interface()</code> that
	 * can be overridden by derived servants if the default behavior is not
	 * adequate.
	 * 
	 * @return <code>get_interface</code> type information that corresponds to
	 *         this servant.
	 */
	/*
	 * public org.omg.CORBA.Object _get_interface() { return
	 * _get_delegate().get_interface(this); }
	 */
	 ...
	 * <code>CORBA::Object</code> implemented by the <code>Servant</code>. The
	 * invoker of <code>_get_interface_def</code> must narrow the result to an
	 * <code>InterfaceDef</code> in order to use it.
	 * <P>
	 * This default implementation of <code>_get_interface_def()</code> can be
	 * overridden by derived servants if the default behavior is not adequate.
	 * As defined in the CORBA 2.3.1 specification, section 11.3.1, the default
	 * behavior of <code>_get_interface_def()</code> is to use the most derived
	 * interface of a static servant or the most derived interface retrieved
	 * from a dynamic servant to obtain the <code>InterfaceDef</code>. This
	 * behavior must be supported by the <code>Delegate</code> that implements
	 * the <code>Servant</code>.
	 * 
	 * @return <code>get_interface_def</code> an <code>InterfaceDef</code>
	 *         object as a <code>CORBA::Object</code> that defines the runtime
	 *         type of the <code>CORBA::Object</code> implemented by the
	 *         <code>Servant</code>.
	 */
