...
/**
 * DynStruct objects support the manipulation of IDL struct and exception
 * values. Members of the exceptions are handled in the same way as members of a
 * struct.
 */
public class _DynStructStub extends org.omg.CORBA.portable.ObjectImpl
	 ...
	 * each member in the struct associated with a DynStruct object. The
	 * sequence contains members in the same order as the declaration order of
	 * members as indicated by the DynStruct's TypeCode. The current position is
	 * not affected. The member names in the returned sequence will be empty
	 * strings if the DynStruct's TypeCode does not contain member names.
	 */
	...
	/**
	 * Initializes the struct data value associated with a DynStruct object from
	 * a sequence of NameValuePairs. The operation sets the current position to
	 * zero if the passed sequences has non-zero length. Otherwise, if an empty
	 * sequence is passed, the current position is set to -1.
	 * <P>
	 * Members must appear in the NameValuePairs in the order in which they
	 * appear in the IDL specification of the struct as indicated by the
	 * DynStruct's TypeCode or they must be empty strings. The operation makes
	 * no attempt to assign member values based on member names.
	 *
	 * @exception TypeMismatch
	 *                if the member names supplied in the passed sequence do not
	 *                match the corresponding member name in the DynStruct's
	 ...
	 * of each member in the struct associated with a DynStruct object. The
	 * sequence contains members in the same order as the declaration order of
	 * members as indicated by the DynStruct's TypeCode. The current position is
	 * not affected. The member names in the returned sequence will be empty
	 * strings if the DynStruct's TypeCode does not contain member names.
	...
	/**
	 * Initializes the struct data value associated with a DynStruct object from
	 * a sequence of NameDynAnyPairs. The operation sets the current position to
	 * zero if the passed sequences has non-zero length. Otherwise, if an empty
	 * sequence is passed, the current position is set to -1.
	 * <P>
	...
	private static String[] __ids = { "IDL:omg.org/DynamicAny/DynStruct:1.0",
