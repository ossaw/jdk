 ...
 * DynValue representing a non-null value type, the DynValue's components
 * comprise the public and private members of the value type, including those
 * inherited from concrete base value types, in the order of definition. A
 * DynValue representing a null value type has no components and a current
 * position of -1.
 * <P>
 ...
 */
public class _DynValueStub extends org.omg.CORBA.portable.ObjectImpl
	 ...
	 * may return an empty string since the TypeCode of the value being
	...
	public String current_member_name() throws org.omg.DynamicAny.DynAnyPackage.TypeMismatch,
		...
		org.omg.CORBA.portable.ServantObject $so = _servant_preinvoke("current_member_name",
				_opsClass);
	...
	public org.omg.CORBA.TCKind current_member_kind()
			...
			org.omg.DynamicAny.DynAnyPackage.InvalidValue {
		org.omg.CORBA.portable.ServantObject $so = _servant_preinvoke("current_member_kind",
				_opsClass);
		DynValueOperations $self = (DynValueOperations) $so.servant;
	...
	/**
	 * Returns a sequence of NameValuePairs describing the name and the value of
	 * each member in the value type. The sequence contains members in the same
	 * order as the declaration order of members as indicated by the DynValue's
	 * TypeCode. The current position is not affected. The member names in the
	 * returned sequence will be empty strings if the DynValue's TypeCode does
	 ...
	 * The operation sets the current position to zero if the passed sequences
	 * has non-zero length. Otherwise, if an empty sequence is passed, the
	 * current position is set to -1. A null value type can be initialized to a
	 * non-null value type using this method.
	 * <P>
	 * Members must appear in the NameValuePairs in the order in which they
	 * appear in the IDL specification of the value type as indicated by the
	 * DynValue's TypeCode or they must be empty strings. The operation makes no
	 * attempt to assign member values based on member names.
	 *
	 * @exception TypeMismatch
	 *                if the member names supplied in the passed sequence do not
	 *                match the corresponding member name in the DynValue's
	...
	/**
	 * Returns a sequence of NameDynAnyPairs describing the name and the value
	 * of each member in the value type. The sequence contains members in the
	 * same order as the declaration order of members as indicated by the
	 * DynValue's TypeCode. The current position is not affected. The member
	 * names in the returned sequence will be empty strings if the DynValue's
	...
	public org.omg.DynamicAny.NameDynAnyPair[] get_members_as_dyn_any()
			throws org.omg.DynamicAny.DynAnyPackage.InvalidValue {
		org.omg.CORBA.portable.ServantObject $so = _servant_preinvoke("get_members_as_dyn_any",
				_opsClass);
		DynValueOperations $self = (DynValueOperations) $so.servant;
	 ...
	 * non-null value type using this method.
	 * <P>
	 * Members must appear in the NameDynAnyPairs in the order in which they
	 * appear in the IDL specification of the value type as indicated by the
	 * DynValue's TypeCode or they must be empty strings. The operation makes no
	 * attempt to assign member values based on member names.
	 *
	...
	public void set_members_as_dyn_any(org.omg.DynamicAny.NameDynAnyPair[] value)
			throws org.omg.DynamicAny.DynAnyPackage.TypeMismatch,
			org.omg.DynamicAny.DynAnyPackage.InvalidValue {
		org.omg.CORBA.portable.ServantObject $so = _servant_preinvoke("set_members_as_dyn_any",
				_opsClass);
		DynValueOperations $self = (DynValueOperations) $so.servant;
	...
	/**
	 * Replaces a null value type with a newly constructed value. Its components
	 * are initialized to default values as in
	 * DynAnyFactory.create_dyn_any_from_type_code. If the DynValueCommon
