 ...
 */
public class _DynSequenceStub extends org.omg.CORBA.portable.ObjectImpl
	...
	/**
	 * Sets the length of the sequence. Increasing the length of a sequence adds
	 * new elements at the tail without affecting the values of already existing
	 * elements. Newly added elements are default-initialized. Increasing the
	 * length of a sequence sets the current position to the first newly-added
	 * element if the previous current position was -1. Otherwise, if the
	 * previous current position was not -1, the current position is not
	 * affected. Decreasing the length of a sequence removes elements from the
	 * tail without affecting the value of those elements that remain. The new
	 * current position after decreasing the length of a sequence is determined
	 * as follows:
	 * <UL>
	 * <LI>If the length of the sequence is set to zero, the current position is
	 * set to -1.
	 * <LI>If the current position is -1 before decreasing the length, it
	 * remains at -1.
	 * <LI>If the current position indicates a valid element and that element is
	 * not removed when the length is decreased, the current position remains
	 * unaffected.
	 * <LI>If the current position indicates a valid element and that element is
	 * removed, the current position is set to -1.
	 * </UL>
	 *
	 * @exception InvalidValue
	 *                if this is a bounded sequence and len is larger than the
	 *                bound
	 */
	...
	/**
	 * Sets the elements of a sequence. The length of the DynSequence is set to
	 * the length of value. The current position is set to zero if value has
	 * non-zero length and to -1 if value is a zero-length sequence.
	 *
	 * @exception TypeMismatch
	 *                if value contains one or more elements whose TypeCode is
	 *                not equivalent to the element TypeCode of the DynSequence
	 * @exception InvalidValue
	 *                if the length of value exceeds the bound of a bounded
	 *                sequence
	 */
	...
	public org.omg.DynamicAny.DynAny[] get_elements_as_dyn_any() {
		org.omg.CORBA.portable.ServantObject $so = _servant_preinvoke("get_elements_as_dyn_any",
				_opsClass);
		DynSequenceOperations $self = (DynSequenceOperations) $so.servant;
	...
	/**
	 * Sets the elements of a sequence using DynAnys. The length of the
	 * DynSequence is set to the length of value. The current position is set to
	 * zero if value has non-zero length and to -1 if value is a zero-length
	 * sequence.
	 *
	 * @exception TypeMismatch
	 *                if value contains one or more elements whose TypeCode is
	...
	public void set_elements_as_dyn_any(org.omg.DynamicAny.DynAny[] value)
			throws org.omg.DynamicAny.DynAnyPackage.TypeMismatch,
			org.omg.DynamicAny.DynAnyPackage.InvalidValue {
		org.omg.CORBA.portable.ServantObject $so = _servant_preinvoke("set_elements_as_dyn_any",
				_opsClass);
		DynSequenceOperations $self = (DynSequenceOperations) $so.servant;
	...
	private static String[] __ids = { "IDL:omg.org/DynamicAny/DynSequence:1.0",
