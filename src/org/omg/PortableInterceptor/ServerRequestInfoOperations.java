 ...
 * each attribute or operation. If it is not valid, attempting to access it will
 ...
 * <tr>
 * <th>&nbsp;</th>
 * <th id="rec_req_ser_con" valign="bottom">receive_request_<br>
 * service_contexts</th>
 ...
 * argument, whether in, inout, or out. But only the in and inout arguments will
 * be available.</li>
 ...
 * <li>If the servant locator caused a location forward, or thrown an exception,
 * this attribute/operation may not be available in this interception point.
 * <code>NO_RESOURCES</code> with a standard minor code of 1 will be thrown if
 * it is not available.</li>
 * <li>The operation is not available in this interception point because the
 * necessary information requires access to the target object's servant, which
 * may no longer be available to the ORB. For example, if the object's adapter
 * is a POA that uses a <code>ServantLocator</code>, then the ORB invokes the
 * interception point after it calls <code>ServantLocator.postinvoke()</code>
 * </li>.
 * </ol>
 *
...
public interface ServerRequestInfoOperations
	...
	/**
	 * Returns the opaque <code>object_id</code> describing the target of the
	...
	/**
	 * Returns the server ID that was specified on ORB::init using the
	 * -ORBServerId argument.
	 */
	String server_id();
	...
	/**
	 * Returns the sequence of strings that identifies the object adapter
	 ...
	 * type. The returned <code>CORBA.Policy</code> object shall only be a
	 * policy whose type was registered via <code>register_policy_factory</code>
	 * .
	 * 
	 * @param type
	 ...
	 *                thrown, with a standard minor code of 2, if a policy for
	 ...
	 *            The <code>IOP.ServiceContext</code> to add to the reply.
	 * @param replace
	 *            Indicates the behavior of this operation when a service
	 *            context already exists with the given ID. If false, then
	 *            <code>BAD_INV_ORDER</code> with a standard minor code of 15 is
	 *            thrown. If true, then the existing service context is replaced
	 *            by the new one.
	 * @exception BAD_INV_ORDER
	 *                thrown, with a standard minor code of 15, if replace is
	 *                false and a service context already exists with the given
	 *                ID.
	 */
