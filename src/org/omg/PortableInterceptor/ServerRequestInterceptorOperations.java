...
public interface ServerRequestInterceptorOperations
	 ...
	 * interception point may or may not execute in the same thread as the
	 * target invocation.
	 * <p>
	 * This interception point may throw a system exception. If it does, no
	 * other Interceptors' <code>receive_request_service_contexts</code>
	 * operations are called. Those Interceptors on the Flow Stack are popped
	 * and their <code>send_exception</code> interception points are called.
	 * <p>
	 ...
	 * invocation.
	 * <p>
	 * In the DSI model, since the parameters are first available when the user
	 * code calls <code>arguments</code>, <code>receive_request</code> is called
	 * from within <code>arguments</code>. It is possible that
	 * <code>arguments</code> is not called in the DSI model. The target may
	 * call <code>set_exception</code> before calling <code>arguments</code>.
	 * The ORB shall guarantee that <code>receive_request</code> is called once,
	 * either through <code>arguments</code> or through
	 * <code>set_exception</code>. If it is called through
	 * <code>set_exception</code>, requesting the arguments will result in
	 * <code>NO_RESOURCES</code> being thrown with a standard minor code of 1.
	 * <p>
	 * This interception point may throw a system exception. If it does, no
	 * other Interceptors' <code>receive_request</code> operations are called.
	 * Those Interceptors on the Flow Stack are popped and their
	 * <code>send_exception</code> interception points are called.
	 * <p>
	 * This interception point may also throw a <code>ForwardRequest</code>
	 * exception. If an Interceptor throws this exception, no other
	 * Interceptors' <code>receive_request</code> operations are called. Those
	 ...
	 * the reply is returned to the client. This interception point shall
	 * execute in the same thread as the target invocation.
	 * <p>
	 * This interception point may throw a system exception. If it does, no
	 * other Interceptors' <code>send_reply</code> operations are called. The
	 ...
	 * reply service context before the exception is thrown to the client. When
	 * an exception occurs, this interception point is called. This interception
	 ...
	 * Flow Stack receive on their calls to <code>send_exception</code>. The
	 ...
	 * <code>LOCATION_FORWARD</code> status was received). This interception
	 * point shall execute in the same thread as the target invocation.
	 * <p>
	 * This interception point may throw a system exception. If it does, no
	 * other Interceptors' <code>send_other</code> operations are called. The
	 * remaining Interceptors in the Flow Stack shall have their
	 * <code>send_exception</code> interception points called.
	 * <p>
	 * This interception point may also throw a <code>ForwardRequest</code>
	 * exception. If an Interceptor throws this exception, successive
	 * Interceptors' <code>send_other</code> operations are called with the new
	 * information provided by the <code>ForwardRequest</code> exception.
	 * <p>
	 * Compliant Interceptors shall properly follow
	 * <code>completion_status</code> semantics if they throw a system exception
	 * from this interception point. The <code>completion_status</code> shall be
