 ...
 * of the <code>org.omg.CORBA.Object</code> methods. All method implementations
 * are forwarded to a <code>Delegate</code> object stored in the
 * <code>ObjectImpl</code> instance. <code>ObjectImpl</code> allows for portable
 * stubs because the <code>Delegate</code> can be implemented by a different
 * vendor-specific ORB.
 */
...
abstract public class ObjectImpl implements org.omg.CORBA.Object {
	...
	/**
	 * The field that stores the <code>Delegate</code> instance for this
	 * <code>ObjectImpl</code> object. This <code>Delegate</code> instance can
	 * be implemented by a vendor-specific ORB. Stub classes, which are derived
	 * from this <code>ObjectImpl</code> class, can be portable because they
	 * delegate all of the methods called on them to this <code>Delegate</code>
	 * object.
	 */
	 ...
	 * object to which this <code>ObjectImpl</code> object delegates all methods
	 * invoked on it.
	 *
	 ...
	 *
	 * @param delegate
	 *            the <code>Delegate</code> instance to which all method calls
	 *            on this <code>ObjectImpl</code> object will be delegated; may
	...
	/**
	 * Retrieves a string array containing the repository identifiers supported
	 * by this <code>ObjectImpl</code> object. For example, for a stub, this
	 * method returns information about all the interfaces supported by the
	 * stub.
	 *
	 ...
	 *
	 * @return an <code>orb.omg.CORBA.Object</code> object that is a duplicate
	 *         of this object
	 */
	...
	/**
	 * Releases the resources associated with this <code>ObjectImpl</code>
	 * object.
	 */
	...
	/**
	 * Checks whether the object identified by the given repository identifier
	 * is an <code>ObjectImpl</code> object.
	 *
	 * @param repository_id
	 *            a <code>String</code> object with the repository identifier to
	 *            check
	...
	/**
	 * Checks whether the the given <code>ObjectImpl</code> object is equivalent
	...
	/**
	 * Checks whether the server object for this <code>ObjectImpl</code> object
	 * has been destroyed.
	 *
	...
	/**
	 * Creates a <code>Request</code> object containing the given method that
	 ...
	 *            invoke
	 * @param arg_list
	 *            the arguments for the method; an <code>NVList</code> in which
	 *            each argument is a <code>NamedValue</code> object
	 * @param result
	 *            a <code>NamedValue</code> object to be used for returning the
	...
	/**
	 * Creates a <code>Request</code> object that contains the given context,
	 * method, argument list, container for the result, exceptions, and list of
	 * property names to be used in resolving the context strings. This
	 * <code>Request</code> object is for use in the Dynamic Invocation
	 * Interface.
	 *
	 * @param ctx
	 *            the <code>Context</code> object that contains the context
	 *            strings that must be resolved before they are sent along with
	 *            the request
	 * @param operation
	 *            the method that the new <code>Request</code> object will
	 *            invoke
	 ...
	 *            contexts in <i>ctx</i>; the strings in <i>contexts</i> are
	 *            used as arguments to the method
	 *            <code>Context.get_values</code>, which returns the value
	 ...
	 */
	public Request _create_request(Context ctx, String operation, NVList arg_list,
			NamedValue result, ExceptionList exceptions, ContextList contexts) {
		return _get_delegate().create_request(this, ctx, operation, arg_list, result, exceptions,
				contexts);
	}
	...
	/**
	 * Returns a reference to the ORB associated with this object and its
	 * delegate. This is the <code>ORB</code> object that created the delegate.
	 *
	...
	/**
	 * Retrieves the <code>Policy</code> object for this <code>ObjectImpl</code>
	...
	/**
	 * Retrieves a list of the domain managers for this <code>ObjectImpl</code>
	 * object.
	 *
	 * @return an array containing the <code>DomainManager</code> objects for
	 *         this instance of <code>ObjectImpl</code>
	 */
	...
	/**
	 * Sets this <code>ObjectImpl</code> object's override type for the given
	 * policies to the given instance of <code>SetOverrideType</code>.
	 *
	 * @param policies
	 *            an array of <code>Policy</code> objects with the policies that
	 *            will replace the current policies or be added to the current
	 *            policies
	 * @param set_add
	 *            either <code>SetOverrideType.SET_OVERRIDE</code>, indicating
	 *            that the given policies will replace any existing ones, or
	 *            <code>SetOverrideType.ADD_OVERRIDE</code>, indicating that the
	...
	/**
	 * Checks whether this <code>ObjectImpl</code> object is implemented by a
	 * local servant. If so, local invocation API's may be used.
	 *
	...
	/**
	 * Returns a Java reference to the local servant that should be used for
	 * sending a request for the method specified. If this
	 * <code>ObjectImpl</code> object is a local stub, it will invoke the
	 * <code>_servant_preinvoke</code> method before sending a request in order
	 * to obtain the <code>ServantObject</code> instance to use.
	 * <P>
	 * If a <code>ServantObject</code> object is returned, its
	 * <code>servant</code> field has been set to an object of the expected type
	 * (Note: the object may or may not be the actual servant instance). The
	 * local stub may cast the servant field to the expected type, and then
	 * invoke the operation directly. The <code>ServantRequest</code> object is
	 * valid for only one invocation and cannot be used for more than one
	 * invocation.
	 *
	 * @param operation
	 *            a <code>String</code> containing the name of the method to be
	 *            invoked. This name should correspond to the method name as it
	 *            would be encoded in a GIOP request.
	 *
	 * @param expectedType
	 *            a <code>Class</code> object representing the expected type of
	 *            the servant that is returned. This expected type is the
	 *            <code>Class</code> object associated with the operations class
	 *            for the stub's interface. For example, a stub for an interface
	 *            <code>Foo</code> would pass the <code>Class</code> object for
	 ...
	 *         due to a ForwardRequest from a POA ServantManager
	...
	/**
	 * Is called by the local stub after it has invoked an operation on the
	 * local servant that was previously retrieved from a call to the method
	 * <code>_servant_preinvoke</code>. The <code>_servant_postinvoke</code>
	 * method must be called if the <code>_servant_preinvoke</code> method
	 * returned a non-null value, even if an exception was thrown by the method
	 * invoked by the servant. For this reason, the call to the method
	 * <code>_servant_postinvoke</code> should be placed in a Java
	 * <code>finally</code> clause.
	 *
	 * @param servant
	 *            the instance of the <code>ServantObject</code> returned by the
	...
	/*
	 * The following methods were added by orbos/98-04-03: Java to IDL Mapping.
	 * These are used by RMI over IIOP.
	 */

	/**
	 * Returns an <code>OutputStream</code> object to use for marshalling the
	 * arguments of the given method. This method is called by a stub, which
	 * must indicate if a response is expected, that is, whether or not the call
	 * is oneway.
	 *
	 * @param operation
	 *            a String giving the name of the method.
	 * @param responseExpected
	 *            a boolean -- <code>true</code> if the request is not one way,
	...
	/**
	 * Invokes an operation and returns an <code>InputStream</code> object for
	 * reading the response. The stub provides the <code>OutputStream</code>
	 * object that was previously returned by a call to the
	 * <code>_request</code> method. The method specified as an argument to
	 * <code>_request</code> when it was called previously is the method that
	 * this method invokes.
	 * <P>
	 * If an exception occurs, the <code>_invoke</code> method may throw an
	 * <code>ApplicationException</code> object that contains an InputStream
	 ...
	 *             if the invocation leads to a remarshalling error
	 * @see #_request
	 */
	 ...
	 * completed after a call to the method <code>_invoke</code>. Calling this
	...
	/**
	 * Compares this <code>ObjectImpl</code> object with the given one for
	 * equality.
	 *
	 * @param obj
