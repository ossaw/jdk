 ...
 * meaning that the argument is being passed from the client to the server and
 * then back from the server to the client. <code>ARG_INOUT.value</code> is one
 * of the possible values used to indicate the direction in which a parameter is
 * being passed during a dynamic invocation using the Dynamic Invocation
 * Interface (DII).
 * <P>
