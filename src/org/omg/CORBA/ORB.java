 ...
 * <code>ORB</code> class also provides "pluggable ORB implementation" APIs that
 * allow another vendor's ORB implementation to be used.
 * <P>
 ...
 * <code>Dynamic Invocation
 * Interface</code> (DII) and the <code>Dynamic Skeleton Interface</code> (DSI).
 * These interfaces may be used by a developer directly, but most commonly they
 * are used by the ORB internally and are not seen by the general programmer.)
 * <OL>
 * <li>initializes the ORB implementation by supplying values for predefined
 * properties and environmental parameters
 * <li>obtains initial object references to services such as the NameService
 * using the method <code>resolve_initial_references</code>
 ...
 * <P>
 * An application or applet gains access to the CORBA environment by
 * initializing itself into an <code>ORB</code> using one of three
 * <code>init</code> methods. Two of the three methods use the properties
 * (associations of a name with a value) shown in the table below.<BR>
 ...
 * <TD>class name of an ORB implementation</TD>
 * </TR>
 * <TR>
 * <TD>org.omg.CORBA.ORBSingletonClass</TD>
 * <TD>class name of the ORB returned by <code>init()</code></TD>
 * </TR>
 * </TABLE>
 * <P>
 * These properties allow a different vendor's <code>ORB</code> implementation
 * to be "plugged in."
 * <P>
 ...
 * <code>init</code> is given no parameters, the default Singleton ORB is
 * returned. When the method <code>init</code> is given parameters but no ORB
 * class is specified, the Java&nbsp;IDL ORB implementation is returned.
 * <P>
 * The following code fragment creates an <code>ORB</code> object initialized
 * with the default ORB Singleton. This ORB has a restricted implementation to
 * prevent malicious applets from doing anything beyond creating typecodes. It
 * is called a singleton because there is only one instance for an entire
 * virtual machine.
 * 
 * <PRE>
 * ORB orb = ORB.init();
 * </PRE>
 * <P>
 * The following code fragment creates an <code>ORB</code> object for an
 * application. The parameter <code>args</code> represents the arguments
 * supplied to the application's <code>main</code> method. Since the property
 * specifies the ORB class to be "SomeORBImplementation", the new ORB will be
 * initialized with that ORB implementation. If p had been null, and the
 * arguments had not specified an ORB class, the new ORB would have been
 ...
 * </PRE>
 * <P>
 * The following code fragment creates an <code>ORB</code> object for the applet
 * supplied as the first parameter. If the given applet does not specify an ORB
 * class, the new ORB will be initialized with the default Java&nbsp;IDL
 * implementation.
 * 
 * <PRE>
...
abstract public class ORB {
	...
	/**
	 * Returns the <code>ORB</code> singleton object. This method always returns
	 * the same ORB instance, which is an instance of the class described by the
	 * <code>org.omg.CORBA.ORBSingletonClass</code> system property.
	 * <P>
	 * This no-argument version of the method <code>init</code> is used
	 * primarily as a factory for <code>TypeCode</code> objects, which are used
	 * by <code>Helper</code> classes to implement the method <code>type</code>.
	 ...
	 * <P>
	 * This method is not intended to be used by applets, and in the event that
	 * it is called in an applet environment, the ORB it returns is restricted
	 * so that it can be used only as a factory for <code>TypeCode</code>
	 * objects. Any <code>TypeCode</code> objects it produces can be safely
	 * shared among untrusted applets.
	 * <P>
	 * If an ORB is created using this method from an applet, a system exception
	 * will be thrown if methods other than those for creating
	 * <code>TypeCode</code> objects are invoked.
	 *
	...
	/**
	 * Creates a new <code>ORB</code> instance for a standalone application.
	 * This method may be called from applications only and returns a new fully
	 * functional <code>ORB</code> object each time it is called.
	 * 
	 * @param args
	 *            command-line arguments for the application's <code>main</code>
	...
	/**
	 * Creates a new <code>ORB</code> instance for an applet. This method may be
	 * called from applets only and returns a new fully-functional
	 ...
	 * and properties. This method, used in applications only, is implemented by
	 * subclass ORB implementations and called by the appropriate
	 ...
	 * parameters. This method, used in applets only, is implemented by subclass
	 * ORB implementations and called by the appropriate <code>init</code>
	 ...
	 * the server implementation class) to the ORB. The servant class must
	 * extend the <code>ImplBase</code> class corresponding to the interface
	 * that is supported by the server. The servant must thus be a CORBA object
	 * reference, and inherit from <code>org.omg.CORBA.Object</code>. Servants
	 * created by the user can start receiving remote invocations after the
	 * method <code>connect</code> has been called. A servant may also be
	 * automatically and implicitly connected to the ORB if it is passed as an
	 * IDL parameter in an IDL method invocation on a non-local object, that is,
	 * if the servant object has to be marshalled and sent outside of the
	 * process address space.
	 * <P>
	 * Calling the method <code>connect</code> has no effect when the servant
	 * object is already connected to the ORB.
	 * <P>
	...
	/**
	 * Destroys the ORB so that its resources can be reclaimed. Any operation
	 * invoked on a destroyed ORB reference will throw the
	 * <code>OBJECT_NOT_EXIST</code> exception. Once an ORB has been destroyed,
	 * another call to <code>init</code> with the same ORBid will return a
	 ...
	 * If an application calls <code>destroy</code> in a thread that is
	 * currently servicing an invocation, the <code>BAD_INV_ORDER</code> system
	 * exception will be thrown with the OMG minor code 3, since blocking would
	 * result in a deadlock.
	 * <p>
	 * For maximum portability and to avoid resource leaks, an application
	 * should always call <code>shutdown</code> and <code>destroy</code> on all
	 * ORB instances before exiting.
	 *
	...
	/**
	 * Disconnects the given servant object from the ORB. After this method
	 * returns, the ORB will reject incoming remote requests for the
	 * disconnected servant and will send the exception
	 * <code>org.omg.CORBA.OBJECT_NOT_EXIST</code> back to the remote client.
	 * Thus the object appears to be destroyed from the point of view of remote
	 * clients. Note, however, that local requests issued using the servant
	 * directly do not pass through the ORB; hence, they will continue to be
	 * processed by the servant.
	 * <P>
	 * Calling the method <code>disconnect</code> has no effect if the servant
	 * is not connected to the ORB.
	 * <P>
	 * Deprecated by the OMG in favor of the Portable Object Adapter APIs.
	 *
	 * @param obj
	...
	/**
	 * Returns a list of the initially available CORBA object references, such
	 * as "NameService" and "InterfaceRepository".
	 *
	 * @return an array of <code>String</code> objects that represent the object
	 *         references for CORBA services that are initially available with
	 *         this ORB
	 */
	abstract public String[] list_initial_services();

	/**
	 * Resolves a specific object reference from the set of available initial
	 * service names.
	 *
	 * @param object_name
	 ...
	 * format of this string is predefined by IIOP, allowing strings generated
	 * by a different ORB to be converted back into an object reference.
	 * <P>
	 * The resulting <code>String</code> object may be stored or communicated in
	...
	/**
	 * Allocates an <code>NVList</code> with (probably) enough space for the
	 * specified number of <code>NamedValue</code> objects. Note that the
	 * specified size is only a hint to help with storage allocation and does
	 * not imply the maximum size of the list.
	 *
	 * @param count
	 *            suggested number of <code>NamedValue</code> objects for which
	 *            to allocate space
	...
	/**
	 * Creates an <code>NVList</code> initialized with argument descriptions for
	 * the operation described in the given <code>OperationDef</code> object.
	 * This <code>OperationDef</code> object is obtained from an Interface
	 * Repository. The arguments in the returned <code>NVList</code> object are
	 * in the same order as in the original IDL operation definition, which
	 * makes it possible for the list to be used in dynamic invocation requests.
	 *
	 * @param oper
	 *            the <code>OperationDef</code> object to use to create the list
	...
	/**
	 * Creates a <code>NamedValue</code> object using the given name, value, and
	 * argument mode flags.
	 * <P>
	 * A <code>NamedValue</code> object serves as (1) a parameter or return
	 * value or (2) a context property. It may be used by itself or as an
	 ...
	 * any responses. Note that oneway invocations are not guaranteed to reach
	 * the server.
	 *
	 * @param req
	...
	/**
	 * Finds out if any of the deferred (asynchronous) invocations have a
	 * response yet.
	 * 
	...
	/**
	 * Gets the next <code>Request</code> instance for which a response has been
	 * received.
	 *
	 * @return the next <code>Request</code> object ready with a response
	 * @exception WrongTransaction
	 *                if the method <code>get_next_response</code> is called
	 *                from a transaction scope different from the one from which
	 *                the original request was sent. See the OMG Transaction
	 *                Service specification for details.
	 */
	abstract public Request get_next_response() throws WrongTransaction;

	/**
	 * Retrieves the <code>TypeCode</code> object that represents the given
	 * primitive IDL type.
	 *
	 * @param tcKind
	 *            the <code>TCKind</code> instance corresponding to the desired
	 *            primitive type
	 ...
	 * <code>struct</code>. The <code>TypeCode</code> object is initialized with
	...
	/**
	 * Creates a <code>TypeCode</code> object representing an IDL
	 * <code>union</code>. The <code>TypeCode</code> object is initialized with
	 ...
	 *            the type of the <code>union</code> discriminator
	 * @param members
	 *            an array describing the members of the <code>union</code>
	 * @return a newly-created <code>TypeCode</code> object describing an IDL
	 *         <code>union</code>
	 */
	...
	/**
	 * Creates a <code>TypeCode</code> object representing an IDL
	 * <code>enum</code>. The <code>TypeCode</code> object is initialized with
	 * the given id, name, and members.
	 *
	 * @param id
	...
	/**
	 * Creates a <code>TypeCode</code> object representing an IDL
	 * <code>alias</code> (<code>typedef</code>). The <code>TypeCode</code>
	...
	/**
	 * Creates a <code>TypeCode</code> object representing an IDL
	 * <code>exception</code>. The <code>TypeCode</code> object is initialized
	 ...
	 * @param members
	 *            an array describing the members of the <code>exception</code>
	 * @return a newly-created <code>TypeCode</code> object describing an IDL
	 *         <code>exception</code>
	 */
	...
	/**
	 * Creates a <code>TypeCode</code> object representing an IDL
	 * <code>interface</code>. The <code>TypeCode</code> object is initialized
	 ...
	 * <code>string</code>. The <code>TypeCode</code> object is initialized with
	 * the given bound, which represents the maximum length of the string. Zero
	...
	/**
	 * Creates a <code>TypeCode</code> object representing a bounded IDL
	 * <code>wstring</code> (wide string). The <code>TypeCode</code> object is
	 * initialized with the given bound, which represents the maximum length of
	 * the wide string. Zero indicates that the string described by this type
	 * code is unbounded.
	 *
	 * @param bound
	...
	/**
	 * Creates a <code>TypeCode</code> object representing an IDL
	 * <code>sequence</code>. The <code>TypeCode</code> object is initialized
	 * with the given bound and element type.
	 *
	 * @param bound
	 *            the bound for the <code>sequence</code>, 0 if unbounded
	 * @param element_type
	 ...
	 * <code>sequence</code>.
	 * <P>
	 * For the IDL <code>struct</code> Node in following code fragment, the
	 * offset parameter for creating its sequence would be 1:
	 * 
	 * <PRE>
	 *    Struct Node {
	 *        long value;
	 ...
	 * @param offset
	 *            the index to the enclosing <code>TypeCode</code> object that
	...
	/**
	 * Creates a <code>TypeCode</code> object representing an IDL
	 * <code>array</code>. The <code>TypeCode</code> object is initialized with
	 * the given length and element type.
	 *
	 * @param length
	 *            the length of the <code>array</code>
	 * @param element_type
	 *            a <code>TypeCode</code> object describing the type of element
	 ...
	 *
	 * @param digits
	 *            specifies the total number of decimal digits in the number and
	...
	/**
	 * Create a <code>TypeCode</code> object for an IDL value type. The
	 * concrete_base parameter is the TypeCode for the immediate concrete
	 * valuetype base of the valuetype for which the TypeCode is being created.
	 ...
	 * @param type_modifier
	 *            one of the value type modifier constants: VM_NONE, VM_CUSTOM,
	 *            VM_ABSTRACT or VM_TRUNCATABLE
	 * @param concrete_base
	 *            a <code>TypeCode</code> object describing the concrete
	 *            valuetype base
	 * @param members
	...
	/**
	 * Create a recursive <code>TypeCode</code> object which serves as a
	 * placeholder for a concrete TypeCode during the process of creating
	 * TypeCodes which contain recursion. The id parameter specifies the
	 * repository id of the type for which the recursive TypeCode is serving as
	 * a placeholder. Once the recursive TypeCode has been properly embedded in
	 * the enclosing TypeCode which corresponds to the specified repository id,
	 * it will function as a normal TypeCode. Invoking operations on the
	 * recursive TypeCode before it has been embedded in the enclosing TypeCode
	 * will result in a <code>BAD_TYPECODE</code> exception.
	 * <P>
	 ...
	 * </PRE>
	 * <P>
	 * To create a TypeCode for struct Node, you would invoke the TypeCode
	 * creation operations as shown below:
	 * 
	 * <PRE>
	 ...
	 * way marshaling problems, when transmitting the struct in an Any, are
	 * avoided.
	 * <P>
	 * 
	 * @param id
	...
	/**
	 * Creates an IDL <code>Any</code> object initialized to contain a
	 * <code>Typecode</code> object whose <code>kind</code> field is set to
	...
	/**
	 * Retrieves a <code>Current</code> object. The <code>Current</code>
	 * interface is used to manage thread-specific information for use by
	...
	/**
	 * Instructs the ORB to shut down, which causes all object adapters to shut
	 * down, in preparation for destruction.<br>
	 * If the <code>wait_for_completion</code> parameter is true, this operation
	 * blocks until all ORB processing (including processing of currently
	 * executing requests, object deactivation, and other object adapter
	 * operations) has completed. If an application does this in a thread that
	 * is currently servicing an invocation, the <code>BAD_INV_ORDER</code>
	 * system exception will be thrown with the OMG minor code 3, since blocking
	 * would result in a deadlock.<br>
	 ...
	 * been completed. Once an ORB has shutdown, only object reference
	 * management operations may be invoked on the ORB or any object reference
	 * obtained from it. An application may also invoke the <code>destroy</code>
	 * operation on the ORB itself. Invoking any other operation will throw the
	 * <code>BAD_INV_ORDER</code> system exception with the OMG minor code 4.
	 * <p>
	 * The <code>ORB.run</code> method will return after <code>shutdown</code>
	 * has been called.
	 *
	 * @param wait_for_completion
	 *            <code>true</code> if the call should block until the shutdown
	 *            is complete; <code>false</code> if it should return
	 *            immediately
	 * @throws org.omg.CORBA.BAD_INV_ORDER
	 *             if the current thread is servicing an invocation
	 */
	...
	/**
	 * Returns <code>true</code> if the ORB needs the main thread to perform
	 * some work, and <code>false</code> if the ORB does not need the main
	 * thread.
	 *
	...
	/**
	 * Performs an implementation-dependent unit of work if called by the main
	 * thread. Otherwise it does nothing. The methods <code>work_pending</code>
	 ...
	 * requested is passed in as the in parameter <tt>service_type</tt>, the
	 * values defined by constants in the CORBA module. If service information
	 * is available for that type, that is returned in the out parameter
	 * <tt>service_info</tt>, and the operation returns the value <tt>true</tt>.
	 * If no information for the requested services type is available, the
	 * operation returns <tt>false</tt> (i.e., the service is not supported by
	 * this ORB).
	 * <P>
	 * 
	 * @param service_type
	 *            a <code>short</code> indicating the service type for which
	 *            information is being requested
	 * @param service_info
	 *            a <code>ServiceInformationHolder</code> object that will hold
	 *            the <code>ServiceInformation</code> object produced by this
	 *            method
	...
	/**
	 * Creates a new <code>DynAny</code> object from the given <code>Any</code>
	 * object.
	 * <P>
	 * 
	 * @param value
	 ...
	 *             <a href="../DynamicAny/DynAnyFactory.html">DynAnyFactory</a>
	 *             API instead
	 */
	@Deprecated
	 ...
	 *             if the given <code>TypeCode</code> object is not consistent
	 *             with the operation.
	 * @see <a href="package-summary.html#unimpl"><code>CORBA</code> package
	 *      comments for unimplemented features</a>
	 * @deprecated Use the new
	 ...
	 * type with specified initial state. If <tt>create_policy</tt> fails to
	 * instantiate a new Policy object due to its inability to interpret the
	 * requested type and content of the policy, it raises the
	 ...
	 * @throws <tt>org.omg.CORBA.PolicyError</tt>
	 *             when the requested policy is not supported or a requested
	 *             initial state for the policy is not supported.
	 */
