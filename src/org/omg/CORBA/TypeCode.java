 ...
 * <UL>
 * <LI>in the Dynamic Invocation Interface -- to indicate the types of the
 * actual arguments or the type of the return value. <BR>
 * <code>NamedValue</code> objects are used to represent arguments and return
 * values. One of their components is an <code>Any</code> object, which in turn
 * has as one of its components a <code>TypeCode</code> object.
 * <LI>by an Interface Repository to represent the type specifications that are
 * part of many OMG IDL declarations
 * </UL>
 * <P>
 * The representation of a <code>TypeCode</code> object is opaque, but
 * abstractly, a <code>TypeCode</code> object consists of:
 * <UL>
 * <LI>a <code>kind</code> field, which is set to an instance of the class
 * <code>TCKind</code>
 * <LI>zero or more additional fields appropriate for the particular kind. For
 * example, the <code>TypeCode</code> object describing the OMG IDL type
 * <code>1ong</code> has kind <code>TCKind.tk_long</code> and no additional
 * fields. The <code>TypeCode</code> describing OMG IDL type
 * <code>sequence&lt;boolean, 10&gt;</code> has a <code>kind</code> field with
 * the value <code>TCKind.tk_sequence</code> and also fields with the values
 * <code>boolean</code> and <code>10</code> for the type of sequence elements
 ...
 * IDL type. This method creates a <code>TypeCode</code> object for type X and
 * assigns it to the <code>Any</code> object's <code>type</code> field.
 * <LI>from invocations of methods in the ORB class
 * <P>
 * For example, the following creates a <code>TypeCode</code> object for a
 * <code>string</code> with a maximum of 30 characters:
 * 
 * <PRE>
 * org.omg.CORBA.TypeCode tcString = orb.create_string_tc(30);
 * </PRE>
 * <P>
 * The following creates a <code>TypeCode</code> object for an
 * <code>array</code> of five <code>string</code>s:
 * 
 * <PRE>
 * org.omg.CORBA.TypeCode tcArray = orb.create_array_tc(5, TCKind.tk_string);
 * </PRE>
 * <P>
 * The following creates a <code>TypeCode</code> object for an interface named
 * "Account":
 * 
 * <PRE>
 ...
 * <code>Holder</code> classes for user-defined IDL types. These
 * <code>Holder</code> classes are generated by the <code>idltojava</code>
 * compiler.
 * <LI>from a CORBA Interface Repository
 * </OL>
 * <P>
 * Most of the methods in the class <code>TypeCode</code> are accessors, and the
 * information contained in a <code>TypeCode</code> object is specific to a
 * particular type. Therefore, methods must be invoked only on the kind of type
 * codes to which they apply. If an accessor method tries to access information
 * from an inappropriate kind of type code, it will throw the exception
 * <code>TypeCodePackage.BadKind</code>. For example, if the method
 * <code>discriminator_type</code> is called on anything other than a
 * <code>union</code>, it will throw <code>BadKind</code> because only
 * <code>union</code>s have a discriminator. The following list shows which
 * methods apply to which kinds of type codes:
 * <P>
 ...
 * <code>union</code>, <code>enum</code>, <code>alias</code>,
 * <code>exception</code>, <code>value</code>, <code>value_box</code>,
 * <code>native</code>, and <code>abstract_interface</code>:
 * <UL>
 * <LI><code>id</code>
 * <LI><code>name</code>
 * </UL>
 * <P>
 * These methods may be invoked on <code>struct</code>, <code>union</code>,
 * <code>enum</code>, and <code>exception</code>:
 * <UL>
 * <LI><code>member_count</code>
 * <LI><code>member_name</code>
 * </UL>
 * <P>
 * These methods may be invoked on <code>struct</code>, <code>union</code>, and
 * <code>exception</code>:
 * <UL>
 ...
 * </UL>
 * <P>
 * These methods may be invoked on <code>string</code>, <code>sequence</code>,
 * and <code>array</code>:
 * <UL>
 * <LI><code>length</code>
 * </UL>
 * <P>
 * These methods may be invoked on <code>alias</code>, <code>sequence</code>,
 * <code>array</code>, and <code>value_box</code>:
 * <UL>
 * <LI><code>content_type</code>
 * </UL>
 * <P>
 * Unlike other CORBA pseudo-objects, <code>TypeCode</code> objects can be
 * passed as general IDL parameters.
 * <p>
