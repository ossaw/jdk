...
/**
 * This class encapsulates the concept of caller-provided channel binding
 * information. Channel bindings are used to strengthen the quality with which
 * peer entity authentication is provided during context establishment. They
 ...
 * acceptSecContext} has been performed. Unless the
 ...
 * the context establishment token emitted by <code>initSecContext</code> method
 * of the <code>GSSContext</code> interface. The same bindings are set by the
 * context acceptor for its <code>GSSContext</code> object and during processing
 * of the <code>acceptSecContext</code> method a MIC is calculated in the same
 * way. The calculated MIC is compared with that found in the token, and if the
 * MICs differ, accept will throw a <code>GSSException</code> with the major
 * code set to {@link GSSException#BAD_BINDINGS BAD_BINDINGS}, and the context
 * will not be established. Some mechanisms may include the actual channel
 ...
 * address of the host system. Portable applications should therefore ensure
 * that they either provide correct information for the address fields, or omit
 * setting of the addressing information.
 *
...
public class ChannelBinding {
    ...
    /**
     * Create a ChannelBinding object with user supplied address information and
     * data. <code>null</code> values can be used for any fields which the
     ...
     *
     * @return the initiator's address. <code>null</code> is returned if the
     ...
     *
     * @return the acceptor's address. null is returned if the address has not
     *         been set.
     */
     ...
     *
     * @return the application data being used as part of the ChannelBinding.
