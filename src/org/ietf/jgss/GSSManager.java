...
/**
 * This class serves as a factory for other important GSS-API classes and also
 * provides information about the mechanisms that are supported. It can create
 * instances of classes implementing the following three GSS-API interfaces:
 * {@link GSSName GSSName}, {@link GSSCredential GSSCredential}, and
 ...
 * <code>GSSManager</code>. The default <code>GSSManager</code> instance will
 * support the Kerberos v5 GSS-API mechanism in addition to any others. This
 ...
 * application to set provider preferences on such an implementation. These
 * methods also allow the implementation to throw a well-defined exception in
 * case provider based configuration is not supported. Applications that expect
 ...
 * <p>
 * <ol>
 * <li>The application does not care about what provider is used (the default
 * case).
 * <li>The application wants a particular provider to be used preferentially,
 * either for a particular mechanism or all the time, irrespective of mechanism.
 * <li>The application wants to use the locally configured providers as far as
 * possible but if support is missing for one or more mechanisms then it wants
 ...
 * across all objects. For instance, if a GSSCredential contains elements from a
 * provider <i>p</i> for a mechanism <i>m</i>, it should generally be passed in
 * to a GSSContext that will use provider <i>p</i> for the mechanism <i>m</i>. A
 * simple rule of thumb that will maximize portability is that objects created
 * from different GSSManager's should not be mixed, and if possible, a different
 * GSSManager instance should be created if the application wants to invoke the
...
public abstract class GSSManager {
	 ...
	 * "1.2.840.113554.1.2.2" in its list. This Oid identifies the Kerberos v5
	 * GSS-API mechanism that is defined in RFC 1964.
	 *
	 * @return an array of Oid objects corresponding to the mechanisms that are
	 *         available. A <code>null</code> value is returned when no
	 *         mechanism are available (an example of this would be when
	 *         mechanism are dynamically configured, and currently no mechanisms
	 *         are installed).
	 */
	 ...
	 * {@link GSSName#NT_EXPORT_NAME GSSName.NT_EXPORT_NAME}, and the Kerberos
	 * v5 specific Oid "1.2.840.113554.1.2.2.1". The namespace for the Oid
	 * "1.2.840.113554.1.2.2.1" is defined in RFC 1964.
	 *
	 ...
	 * GSSName object. In general, the <code>GSSName</code> object created will
	 * contain multiple representations of the name, one for each mechanism that
	 * is supported; two examples that are exceptions to this are when the
	 * namespace type parameter indicates NT_EXPORT_NAME or when the GSS-API
	 * implementation is not multi-mechanism. It is not recommended to use this
	 * method with a NT_EXPORT_NAME type because representing a previously
	 * exported name consisting of arbitrary bytes as a String might cause
	 * problems with character encoding schemes. In such cases it is recommended
	 * that the bytes be passed in directly to the overloaded form of this
	 ...
	 *            mechanism specific default printable syntax should be assumed
	 *            by each mechanism that examines nameStr. It is not advisable
	 ...
	 * <code>GSSName</code> object created will contain multiple representations
	 * of the name, one for each mechanism that is supported; two examples that
	 ...
	 * The bytes that are passed in are interpreted by each underlying mechanism
	 * according to some encoding scheme of its choice for the given nametype.
	 *
	 * @param name
	 *            the byte array containing the name to create
	 * @param nameType
	 *            the Oid specifying the namespace of the name supplied in the
	 *            byte array. <code>null</code> can be used to specify that a
	 *            mechanism specific default syntax should be assumed by each
	...
	/**
	 * Factory method to convert a string name from the specified namespace to a
	 * GSSName object and canonicalize it at the same time for a mechanism. In
	 * other words, this method is a utility that does the equivalent of two
	 * steps: the {@link #createName(String, Oid) createName} and then also the
	 ...
	 * time for a mechanism. In other words, this method is a utility that does
	 * the equivalent of two steps: the {@link #createName(byte[], Oid)
	 * createName} and then also {@link GSSName#canonicalize(Oid)
	 ...
	 * mechanism providers will be documented on a per-mechanism basis. A failed
	 ...
	 *            principal.
	 * @param lifetime
	 *            The number of seconds that credentials should remain valid.
	 ...
	 * method attempts to acquire credentials for each of the mechanisms
	 * specified in the array called mechs. To determine the list of mechanisms
	 ...
	 *            lifetime.
	 * @param mechs
	 *            an array of Oid's indicating the mechanisms over which the
	 *            credential is to be acquired. Use <code>(Oid[]) null</code>
	 *            for requesting a system specific default set of mechanisms.
	 * @param usage
	 *            The intended usage for this credential object. The value of
	 *            this parameter must be one of:
	 *            {@link GSSCredential#INITIATE_AND_ACCEPT
	 ...
	 * @param mech
	 *            the Oid of the desired mechanism. Use <code>null</code> to
	 *            request the default mechanism.
	 * @param myCred
	 *            the credentials of the initiator. Use <code>null</code> to act
	 ...
	 *            GSSContext.INDEFINITE_LIFETIME} to request that the context
	 *            have the maximum permitted lifetime. Use
	 *            {@link GSSContext#DEFAULT_LIFETIME
	 *            GSSContext.DEFAULT_LIFETIME} to request a default lifetime for
	 *            the context.
	 ...
	 *
	 * @param myCred
	 *            the credentials for the acceptor. Use <code>null</code> to act
	 ...
	 * indicate if the context is transferable. Calling this method in an
	 ...
	 * would like a particular provider to be used ahead of all others when
	 * support is desired for the given mechanism. When a value of null is used
	 * instead of an <code>Oid</code> for the mechanism, the GSSManager must use
	 * the indicated provider ahead of all others no matter what the mechanism
	 ...
	 * </pre>
	 * 
	 * Now if it also desired that the mechanism of Oid m1 always be obtained
	 * from the provider B before the previously set A was checked, it would
	 * call:
	 * <p>
	 * 
	 * <pre>
	 ...
	 * </pre>
	 * 
	 * The GSSManager would then first check with B if m1 was needed. In case B
	 * did not provide support for m1, the GSSManager would continue on to check
	 ...
	 * </pre>
	 * 
	 * then the previous setting with the pair (B, m1) is subsumed by this and
	...
	/**
	 * This method is used to indicate to the GSSManager that the application
	 * would like a particular provider to be used if no other provider can be
	 * found that supports the given mechanism. When a value of null is used
	 ...
	 * </pre>
	 * 
	 * Now, if it also desired that for all mechanisms the provider B be checked
