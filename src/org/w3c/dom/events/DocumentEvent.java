...
public interface DocumentEvent {
	 ...
	 *
	 * @param eventType
	 *            The <code>eventType</code> parameter specifies the type of
	 ...
	 *            <code>dispatchEvent</code> method the appropriate event init
	 *            method must be called after creation in order to initialize
	 *            the <code>Event</code>'s values. As an example, a user wishing
	 *            to synthesize some kind of <code>UIEvent</code> would call
	 *            <code>createEvent</code> with the parameter "UIEvents". The
	 *            <code>initUIEvent</code> method could then be called on the
	 ...
	 *                support the type of <code>Event</code> interface requested
	 */
