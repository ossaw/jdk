 ...
 * <code>Nodes</code> in an implementation which supports the DOM Event Model.
 * Therefore, this interface can be obtained by using binding-specific casting
 * methods on an instance of the <code>Node</code> interface. The interface
 * allows registration and removal of <code>EventListeners</code> on an
...
public interface EventTarget {
	 ...
	 * <code>EventTarget</code> while it is processing an event, it will not be
	 * triggered by the current actions but may be triggered during a later
	 * stage of event flow, such as the bubbling phase. <br>
	 * If multiple identical <code>EventListener</code>s are registered on the
	 * same <code>EventTarget</code> with the same parameters the duplicate
	 * instances are discarded. They do not cause the <code>EventListener</code>
	 ...
	 *            implemented by the user which contains the methods to be
	 *            called when the event occurs.
	 * @param useCapture
	 *            If true, <code>useCapture</code> indicates that the user
	 *            wishes to initiate capture. After initiating capture, all
	 *            events of the specified type will be dispatched to the
	 *            registered <code>EventListener</code> before being dispatched
	 *            to any <code>EventTargets</code> beneath them in the tree.
	 ...
	 * <code>EventTarget</code> while it is processing an event, it will not be
	 * triggered by the current actions. <code>EventListener</code>s can never
	 * be invoked after being removed. <br>
	 * Calling <code>removeEventListener</code> with arguments which do not
	 ...
	 * @param useCapture
	 *            Specifies whether the <code>EventListener</code> being removed
	 *            was registered as a capturing listener or not. If a listener
	 *            was registered twice, one with capture and one without, each
	 *            must be removed separately. Removal of a capturing listener
	 *            does not affect a non-capturing version of the same listener,
	 *            and vice versa.
	 */
	 ...
	 * The target of the event is the <code> EventTarget</code> on which
	 ...
	 *                initializing the event before <code>dispatchEvent</code>
	 *                was called. Specification of the <code>Event</code>'s type
	 *                as <code>null</code> or an empty string will also trigger
	 *                this exception.
	 */
