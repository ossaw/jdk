...
public interface Event {
	 ...
	 * the event was created. Due to the fact that some systems may not provide
	 * this information the value of <code>timeStamp</code> may be not available
	 ...
	 * any <code>EventListener</code> the event will cease propagating through
	 * the tree. The event will complete dispatch to all listeners on the
	 * current <code>EventTarget</code> before event flow stops. This method may
	 * be used during any stage of event flow.
	 */
	public void stopPropagation();

	/**
	 * If an event is cancelable, the <code>preventDefault</code> method is used
	 ...
	 * interface. This method may only be called before the <code>Event</code>
	 * has been dispatched via the <code>dispatchEvent</code> method, though it
	 * may be called multiple times during that phase if necessary. If called
	 * multiple times the final invocation takes precedence. If called from a
	 * subclass of <code>Event</code> interface only the values specified in the
	 * <code>initEvent</code> method are modified, all other attributes are left
	 * unchanged.
	 * 
	 * @param eventTypeArg
	 *            Specifies the event type. This type may be any event type
	 *            currently defined in this specification or a new event type..
	 *            The string must be an XML name. Any new event type must not
	 *            begin with any upper, lower, or mixed case version of the
	 *            string "DOM". This prefix is reserved for future DOM event
	 *            sets. It is also strongly recommended that third parties
	 *            adding their own events use their own prefix to avoid
	 *            confusion and lessen the probability of conflicts with other
	 *            new events.
	 * @param canBubbleArg
