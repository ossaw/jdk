...
/**
 * Provides methods to obtain the position of a DocTree within a javadoc
 * comment. A position is defined as a simple character offset from the start of
 * a CompilationUnit where the first character is at offset 0.
 *
 * @since 1.8
 */
@jdk.Exported
public interface DocSourcePositions extends SourcePositions {
    ...
    /**
     * Gets the starting position of the tree within the comment within the
     * file. If tree is not found within file, or if the starting position is
     * not available, return {@link javax.tools.Diagnostic#NOPOS}. The given
     * tree should be under the given comment tree, and the given documentation
     * comment tree should be returned from a
     * {@link DocTrees#getDocCommentTree(com.sun.source.util.TreePath) } for a
     ...
     * If tree is not found within file, or if the ending position is not
     * available, return {@link javax.tools.Diagnostic#NOPOS}. The given tree
     * should be under the given comment tree, and the given documentation
     * comment tree should be returned from a
     * {@link DocTrees#getDocCommentTree(com.sun.source.util.TreePath) } for a
