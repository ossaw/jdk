 ...
 * <li>If the node being visited has one child, the result will be the result of
 * calling {@code scan} on that child. The child may be a simple node or itself
 * a list of nodes.
 * <li>If the node being visited has more than one child, the result will be
 * determined by calling {@code scan} each child in turn, and then combining the
 * result of each scan after the first with the cumulative result so far, as
 * determined by the {@link #reduce} method. Each child may be either a simple
 * node of a list of nodes. The default behavior of the {@code reduce} method is
...
public class TreeScanner<R, P> implements TreeVisitor<R, P> {
	...
	/**
	 * Reduces two results into a combined result. The default implementation is
	 * to return the first parameter. The general contract of the method is that
