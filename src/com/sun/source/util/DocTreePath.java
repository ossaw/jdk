...
public class DocTreePath implements Iterable<DocTree> {
	...
	/**
	 * Gets a documentation tree path for a tree node within a subtree
	 * identified by a DocTreePath object.
	 * 
	 * @return null if the node is not found
	 */
	...
	public DocTreePath(DocTreePath p, DocTree t) {
		if (t.getKind() == DocTree.Kind.DOC_COMMENT) {
			throw new IllegalArgumentException(
					"Use DocTreePath(TreePath, DocCommentTree) to construct DocTreePath for a DocCommentTree.");
		} else {
			treePath = p.treePath;
