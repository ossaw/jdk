...
public class DefaultSocketFactory implements ORBSocketFactory {
	...
	public SocketInfo getEndPointInfo(ORB orb, IOR ior, SocketInfo socketInfo) {
		...
		return new EndPointInfoImpl(ORBSocketFactory.IIOP_CLEAR_TEXT, primary
				.getPort(), primary.getHost().toLowerCase());
	}
	...
	public Socket createSocket(SocketInfo socketInfo) throws IOException,
		...
		if (orb.getORBData().acceptorSocketType().equals(
				ORBConstants.SOCKETCHANNEL)) {
			InetSocketAddress address = new InetSocketAddress(socketInfo
					.getHost(), socketInfo.getPort());
