...
/**
 * POAPolicyMediator defines an interface to which the POA delegates all policy
 * specific operations. This permits code paths for different policies to be
 * optimized by creating the correct code at POA creation time. Also note that
 * as much as possible, this interface does not do any concurrency control,
 * except as noted. The POA is responsible for concurrency control.
 */
public interface POAPolicyMediator {
     ...
     * this POAPolicyMediator's POA. This is initialized according to the
     * policies and the POA used to construct this POAPolicyMediator in the
     ...
     * POAPolicyMediator's POA. This is initialized according to the policies
     * and the POA used to construct this POAPolicyMediator in the
     * POAPolicyMediatorFactory.
     */
    int getServerId();
    ...
    /**
     * Etherealize all servants associated with this POAPolicyMediator. Does
     * nothing if the retention policy is non-retain.
     */
    void etherealizeAll();
    ...
    /**
     * Return the servant manager. Will throw WrongPolicy if the request
     * processing policy is not USE_SERVANT_MANAGER.
     */
    ServantManager getServantManager() throws WrongPolicy;

    /**
     * Set the servant manager. Will throw WrongPolicy if the request processing
     * policy is not USE_SERVANT_MANAGER.
     */
    void setServantManager(ServantManager servantManager) throws WrongPolicy;

    /**
     * Return the default servant. Will throw WrongPolicy if the request
     * processing policy is not USE_DEFAULT_SERVANT.
     */
    Servant getDefaultServant() throws NoServant, WrongPolicy;

    /**
     * Set the default servant. Will throw WrongPolicy if the request processing
     * policy is not USE_DEFAULT_SERVANT.
     */
    ...
    /**
     * Allocate a new, unique system ID. Requires the ID assignment policy to be
     * SYSTEM.
     */
