...
/**
 * POAManagerImpl is the implementation of the POAManager interface. Its public
...
public class POAManagerImpl extends org.omg.CORBA.LocalObject implements POAManager {
	...
	private boolean explicitStateChange; // initially false, set true as soon as
											...
											// discard_request, or deactivate is
											// called.
		...
		try {
			if (debug) {
				ORBUtility.dprint(this,
						"Calling countedWait on POAManager " + this + " nWaiters=" + nWaiters);
			}
			...
			if (debug) {
				ORBUtility.dprint(this,
						"Exiting countedWait on POAManager " + this + " nWaiters=" + nWaiters);
			}
		}
	}
	...
	private void notifyWaiters() {
		if (debug) {
			ORBUtility.dprint(this,
					"Calling notifyWaiters on POAManager " + this + " nWaiters=" + nWaiters);
		}
	...
	public synchronized void activate()
		...
		try {
			...
			// state was HOLDING, as well as notify any threads that were
			// waiting
	...
	public synchronized void discard_requests(boolean wait_for_completion)
		...
		try {
			...
			// Notify any invocations that were waiting because the previous
			// state was HOLDING. Those invocations will henceforth be rejected
			// with
	...
	/**
	 * <code>deactivate</code> <b>Spec: pages 3-14 thru 3-18</b> Note: INACTIVE
	...
	public void deactivate(boolean etherealize_objects, boolean wait_for_completion)
		...
		try {
			synchronized (this) {
				...
				// Notify any invocations that were waiting because the previous
				// state was HOLDING. Those invocations will then be rejected
				// with
				// an OBJ_ADAPTER exception. Also notify any threads that were
				// waiting
				// inside hold_requests() or discard_requests().
				notifyWaiters();
			}
	...
	private class POAManagerDeactivator implements Runnable {
		...
		public void run() {
			try {
				synchronized (pmi) {
					if (debug) {
						ORBUtility.dprint(this, "Calling run with etherealize_objects="
				...
				if (etherealize_objects) {
					...
					synchronized (pmi) {
						if (debug) {
							ORBUtility.dprint(this,
									"run: Preparing to etherealize with pmi=" + pmi);
						}
	...
	/**
	 * Added according to the spec CORBA V2.3; this returns the state of the
	 * POAManager
	 */
