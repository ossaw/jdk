...
public class POAPolicyMediatorImpl_R_USM extends POAPolicyMediatorBase_R {
	...
	/*
	 * This handles a rather subtle bug (4939892). The problem is that enter
	 * will wait on the entry if it is being etherealized. When the deferred
	 * state transition completes, the entry is no longer in the AOM, and so we
	 * need to get a new entry, otherwise activator.incarnate will be called
	 * twice, once for the old entry, and again when a new entry is created.
	 * This fix also required extending the FSM StateEngine to allow actions to
	 * throw exceptions, and adding a new state in the AOMEntry FSM to detect
	 * this condition.
	 */
	...
	protected java.lang.Object internalGetServant(byte[] id, String operation)
		...
		if (poa.getDebug()) {
			ORBUtility.dprint(this, "Calling POAPolicyMediatorImpl_R_USM.internalGetServant "
		...
		try {
			...
			try {
				...
				// while servant instanceof NullServant means incarnate returned
				// a
			...
			if (poa.getDebug()) {
				ORBUtility.dprint(this, "Exiting POAPolicyMediatorImpl_R_USM.internalGetServant "
						+ "for poa " + poa);
			}
		}
	}
	...
	public void etherealizeAll() {
		if (activator != null) {
			...
			ActiveObjectMap.Key[] keys = (ActiveObjectMap.Key[]) keySet
					.toArray(new ActiveObjectMap.Key[keySet.size()]);
	...
	class Etherealizer extends Thread {
		...
		public Etherealizer(POAPolicyMediatorImpl_R_USM mediator, ActiveObjectMap.Key key,
				AOMEntry entry, Servant servant, boolean debug) {
