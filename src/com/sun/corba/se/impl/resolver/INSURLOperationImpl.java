...
/**
 * This class provides an Operation that converts from CORBA INS URL strings
 * into CORBA object references. It will eventually become extensible, but for
 * now it simply encapsulates the existing implementation. Once the full
 * extensibility is in place, we want this operation to convert string to
 * INSURL, which has mainly a public resolver method that returns an object
 * reference.
 *
 * @author Hemanth
 * @author Ken
...
public class INSURLOperationImpl implements Operation {
	...
	public INSURLOperationImpl(ORB orb, Resolver bootstrapResolver) {
		this.orb = orb;
		wrapper = ORBUtilSystemException.get(orb, CORBALogDomains.ORB_RESOLVER);
		omgWrapper = OMGSystemException.get(orb, CORBALogDomains.ORB_RESOLVER);
	...
	/**
	 * This static method takes a Stringified IOR and converts it into IOR
	 * object. It is the caller's responsibility to only pass strings that start
	 * with "IOR:".
	 */
	...
	private org.omg.CORBA.Object getIORUsingCorbaloc(INSURL corbalocObject) {
		...
		if (pTemplate != null) {
			iortemp.add(pTemplate); // Add profile for GIOP version used by this
									// ORB
			profileMap.remove(giopVersion); // Now remove this value from the
											// map
		}
	...
	/**
	 * This is required for corbaname: resolution. Currently we are not caching
	 * RootNamingContext as the reference to rootNamingContext may not be
	 * Persistent in all the implementations. _REVISIT_ to clear the
	 * rootNamingContext in case of COMM_FAILURE.
	 *
	...
	private NamingContextExt getDefaultRootNamingContext() {
		synchronized (rootContextCacheLock) {
			if (rootNamingContextExt == null) {
				try {
					rootNamingContextExt = NamingContextExtHelper
	...
	/**
	 * A utility method to clear the RootNamingContext, if there is an exception
	 * in resolving CosNaming:Name from the RootNamingContext,
	 */
