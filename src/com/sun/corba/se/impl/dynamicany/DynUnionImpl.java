...
public class DynUnionImpl extends DynAnyConstructedImpl implements DynUnion {
	...
	// index either points to the discriminator or the named member is it
	// exists.
	// The currently active member, which is of the same type as the
	// discriminator.
	...
	protected boolean initializeComponentsFromAny() {
		try {
			...
			Any discriminatorAny = DynAnyUtil.extractAnyFromStream(
					discriminatorType(), input, orb);
			discriminator = DynAnyUtil.createMostDerivedDynAny(discriminatorAny,
					orb, false);
			currentMemberIndex = currentUnionMemberIndex(discriminatorAny);
			Any memberAny = DynAnyUtil.extractAnyFromStream(memberType(
					currentMemberIndex), input, orb);
			currentMember = DynAnyUtil.createMostDerivedDynAny(memberAny, orb,
					false);
	...
	// The discriminator value is set to a value consistent with the first named
	// member
	// of the union. That member is activated and (recursively) initialized to
	// its default value.
	protected boolean initializeComponentsFromTypeCode() {
		// System.out.println(this + " initializeComponentsFromTypeCode");
		try {
			// We can be sure that memberCount() > 0 according to the IDL
			// language spec
			discriminator = DynAnyUtil.createMostDerivedDynAny(memberLabel(0),
					orb, false);
			index = 0;
			currentMemberIndex = 0;
			currentMember = DynAnyUtil.createMostDerivedDynAny(memberType(0),
					orb);
			components = new DynAny[] { discriminator, currentMember };
		} catch (InconsistentTypeCode ictc) { // impossible
		}
	...
	// to the TypeCode of the unions discriminator, the operation raises
	// TypeMismatch.
	//
	// Setting the discriminator to a value that is consistent with the
	// currently
	// active union member does not affect the currently active member.
	// Setting the discriminator to a value that is inconsistent with the
	// currently
	// active member deactivates the member and activates the member that is
	// consistent
	...
	// Otherwise the current position is set to 1 (has_no_active_member returns
	// false and
	...
	public void set_discriminator(org.omg.DynamicAny.DynAny newDiscriminator)
		...
		int newCurrentMemberIndex = currentUnionMemberIndex(
				newDiscriminatorAny);
		if (newCurrentMemberIndex == NO_INDEX) {
			clearData();
			index = 0;
		} else {
			// _REVISIT_ Could possibly optimize here if we don't need to
			// initialize components
			checkInitComponents();
			if (currentMemberIndex == NO_INDEX
					|| newCurrentMemberIndex != currentMemberIndex) {
				clearData();
				index = 1;
				currentMemberIndex = newCurrentMemberIndex;
				try {
					currentMember = DynAnyUtil.createMostDerivedDynAny(
							memberType(currentMemberIndex), orb);
	...
	public void set_to_default_member()
		...
		try {
			...
			currentMember = DynAnyUtil.createMostDerivedDynAny(memberType(
					defaultIndex), orb);
			components = new DynAny[] { discriminator, currentMember };
	...
	// It sets the current position to zero and causes component_count to return
	// 1.
	// Calling set_to_no_active_member on a union that has an explicit default
	// case
	...
	public void set_to_no_active_member()
		...
		// _REVISIT_ How does one check for "entire range of discriminator
		// values"?
		if (defaultIndex() != -1) {
			throw new TypeMismatch();
		}
	...
	// (that is, the unions value consists solely of its discriminator because
	// the
	...
	// Calling this operation on a union that uses the entire range of
	// discriminator
	...
	public boolean has_no_active_member() {
		...
		checkInitComponents();
		return (checkInitComponents() ? (currentMemberIndex == NO_INDEX)
				: false);
	}
	...
	// If the unions TypeCode does not contain a member name for the currently
	// active member,
	// the operation returns an empty string.
	// Calling member_name on a union without an active member raises
	// InvalidValue.
