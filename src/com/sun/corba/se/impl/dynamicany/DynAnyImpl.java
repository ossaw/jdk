...
abstract class DynAnyImpl extends org.omg.CORBA.LocalObject implements DynAny {
	...
	// A DynAny is undestroyable if it is a node in a DynAny hierarchy other
	// than the root.
	...
	protected Any getAny(DynAny dynAny) {
		if (dynAny instanceof DynAnyImpl)
			...
			// one where component DynAnys are initialized but not the component
			// Anys.
			return dynAny.to_any();
	}

	protected void writeAny(OutputStream out) {
		// System.out.println(this + " writeAny of type " +
		// type().kind().value());
		any.write_value(out);
	}
