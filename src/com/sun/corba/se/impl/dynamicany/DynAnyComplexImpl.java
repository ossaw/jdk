...
abstract class DynAnyComplexImpl extends DynAnyConstructedImpl {
	...
	protected DynAnyComplexImpl(ORB orb, Any any, boolean copyValue) {
		...
		// This is an optimization in case the user is only interested in
		// storing Anys.
	}
	...
	protected DynAnyComplexImpl(ORB orb, TypeCode typeCode) {
		...
		// The members (if any) are (recursively) initialized to their default
		// values.
		index = 0;
	}
	...
	// Copies all the internal representations which is faster than
	// reconstructing them.
	/*
	 * public org.omg.DynamicAny.DynAny copy() { if (status == STATUS_DESTROYED)
	 * { throw new OBJECT_NOT_EXIST(); } DynAnyComplexImpl returnValue = null;
	 * if ((representations & REPRESENTATION_ANY) != 0) { // The flag "true"
	 * indicates copying the Any value returnValue =
	 * (DynAnyComplexImpl)DynAnyUtil.createMostDerivedDynAny(any, orb, true); }
	 * if ((representations & REPRESENTATION_COMPONENTS) != 0) { } return
	 * returnValue; }
	 */
	...
	// Initializes components, names, nameValuePairs and nameDynAnyPairs
	// representation
	...
	protected boolean initializeComponentsFromTypeCode() {
		...
		for (int i = 0; i < length; i++) {
			...
			try {
				...
				// ", type " + memberType.kind().value());
				/*
				 * if (memberDynAny instanceof DynAnyConstructedImpl) { if ( !
				 * ((DynAnyConstructedImpl)memberDynAny).isRecursive()) { //
				 * This is the recursive part
				 * ((DynAnyConstructedImpl)memberDynAny).
				 * initializeComponentsFromTypeCode(); } } // Other
				 * implementations have their own way of dealing with
