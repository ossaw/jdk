...
public class DynAnyUtil {
	static boolean isConsistentType(TypeCode typeCode) {
		...
		return (kind != TCKind._tk_Principal && kind != TCKind._tk_native
	 ...
	 * Moved this functionality into AnyImpl because it is needed for
	 * Any.equal() return ((AnyImpl)any).extractAny(memberType, orb); }
	 */
	...
	/*
	 * static Any setTypeOfAny(TypeCode typeCode, Any value) { if (value !=
	 * null) { value.read_value(value.create_input_stream(), typeCode); } return
	 * value; }
	 */
	...
	/*
	 * static Any copy(Any inAny, ORB orb) { Any outAny = null; if (inAny !=
	 * null && orb != null) { outAny = orb.create_any();
	 * outAny.read_value(inAny.create_input_stream(), inAny.type()); //
	 * isInitialized is set to true } return outAny; }
	 */
