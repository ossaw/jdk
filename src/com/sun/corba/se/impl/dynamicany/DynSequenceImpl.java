...
public class DynSequenceImpl extends DynAnyCollectionImpl implements
    ...
    protected boolean initializeComponentsFromAny() {
        ...
        for (int i = 0; i < length; i++) {
            // _REVISIT_ Could use read_xxx_array() methods on InputStream for
            // efficiency
    ...
    protected boolean initializeAnyFromComponents() {
        ...
        // Writing the length first is the only difference to supers
        // implementation
    ...
    //
    // Increasing the length of a sequence sets the current position to the
    // first
    ...
    //
    // Increasing the length of a bounded sequence to a value larger than the
    // bound
    // raises InvalidValue.
    //
    ...
    // The new current position after decreasing the length of a sequence is
    // determined
    // as follows:
    // ?f the length of the sequence is set to zero, the current position is set
    // to -1.
    // ?f the current position is -1 before decreasing the length, it remains at
    // -1.
    // ?f the current position indicates a valid element and that element is not
    // removed
    ...
    public void set_length(int len)
        ...
        if (len > oldLength) {
            ...
            // Increasing the length of a sequence sets the current position to
            // the first
            // newly-added element if the previous current position was -1.
            if (index == NO_INDEX)
                index = oldLength;
            ...
            // It is probably right not to destroy the released component
            // DynAnys.
            ...
            // ?f the length of the sequence is set to zero, the current
            // position is set to -1.
            // ?f the current position is -1 before decreasing the length, it
            // remains at -1.
            // ?f the current position indicates a valid element and that
            // element is not removed
            // when the length is decreased, the current position remains
            // unaffected.
            // ?f the current position indicates a valid element and that
            // element is removed,
    ...
    // to the element TypeCode of the DynSequence, the operation raises
    // TypeMismatch.
    /*
     * public void set_elements(org.omg.CORBA.Any[] value) throws
