...
public class DynFixedImpl extends DynAnyBasicImpl implements DynFixed {
    ...
    // The integer and fraction parts both must be sequences of decimal (base
    // 10) digits.
    // Either the integer part or the fraction part, but not both, may be
    // missing.
    //
    // If val contains a value whose scale exceeds that of the DynFixed or is
    // not initialized,
    // the operation raises InvalidValue.
    // The return value is true if val can be represented as the DynFixed
    // without loss of precision.
    // If val has more fractional digits than can be represented in the
    // DynFixed,
    // fractional digits are truncated and the return value is false.
    // If val does not contain a valid fixed-point literal or contains
    ...
    public boolean set_value(String val) throws org.omg.DynamicAny.DynAnyPackage.TypeMismatch,
        ...
        // If val contains a value whose scale exceeds that of the DynFixed or
        // is not initialized,
        // the operation raises InvalidValue.
        // Reinterpreted to mean raise InvalidValue only if the integer part
        // exceeds precision,
        // which is handled above (integerPart.length() > digits)
        /*
         * if (currentScale > scale) { throw new InvalidValue("Scale exceeds " +
         * scale); }
         */
