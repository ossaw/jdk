...
public final class ServerRequestInfoImpl extends RequestInfoImpl implements ServerRequestInfo {
    ...
    protected static final int MID_ADD_REPLY_SERVICE_CONTEXT = MID_RI_LAST + 8;
    ...
    /**
     * Creates a new ServerRequestInfo implementation. The constructor is
     * package scope since no other package need create an instance of this
     * class.
     */
     ...
     * overwritten. If the ID does not define an allocated slot, InvalidSlot is
     * raised.
     */
    ...
    public int request_id() {
        ...
        /*
         * NOTE: The request id in server interceptors is NOT the same as the
         * GIOP request id. The ORB may be servicing several connections, each
         * with possibly overlapping sets of request ids. Therefore we create a
         * request id specific to interceptors.
         */
    ...
    private void enqueue(AddReplyServiceContextCommand addReply) {
        ...
        for (int i = 0; i < size; i++) {
            AddReplyServiceContextCommand cmd = (AddReplyServiceContextCommand) addReplyServiceContextQueue
                    .get(i);

            if (cmd.service_context.context_id == addReply.service_context.context_id) {
                found = true;
                if (addReply.replace) {
    ...
    /**
     * Overridden from RequestInfoImpl. This version calls the super and then,
     * if we are changing to ending points, executes all enqueued
     * AddReplyServiceContextCommands.
     */
     ...
     * ending point call depending on the reply status.
     */
    ...
    /**
     * Sets the forwardRequestRaisedInEnding flag to true, indicating that a
     * server ending point has raised location forward at some point.
     */
    ...
    /**
     * Returns true if ForwardRequest was raised by a server ending point or
     * false otherwise.
     */
