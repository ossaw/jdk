...
public abstract class RequestInfoImpl extends LocalObject implements RequestInfo, RequestInfoExt {
	 ...
	 * request/reply sequence is concluded this ID may be reused. (this is NOT
	 * necessarily the same as the GIOP request_id).
	 */
	...
	/**
	 * This method returns the list of contexts for the DII operation. It raises
	 * NO_RESOURCES exception if the operation is not invoked by using DII
	 * mechanism.
	 */
	 ...
	 * It raises NO_RESOURCES exception if the operation is not invoked by using
	 * DII mechanism.
	 */
	abstract public String[] operation_context();

	/**
	 * This method returns the result from the invoked DII operation. It raises
	 * NO_RESOURCES exception if the operation is not invoked by using DII
	 * mechanism.
	 */
	 ...
	 * implementations.
	 *
	 * Indicates whether a response is expected. On the client, a reply is not
	 * returned when response_expected is false, so receive_reply cannot be
	 * called. receive_other is called unless an exception occurs, in which case
	 * receive_exception is called. On the client, within send_poll, this
	 * attribute is true.
	 */
	 ...
	 * sync_scope is undefined. It defines how far the request shall progress
	 * before control is returned to the client. This attribute may have one of
	 * the follwing values:
	 * <ul>
	...
	/**
	 * Describes the state of the result of the operation invocation. Its value
	...
	/**
	 * Implementation for get_request_service_context() differs for client and
	 ...
	 *
	 * This operation returns a copy of the service context with the given ID
	 * that is associated with the reply. IF the request's service context does
	 ...
	 *
	 *         Note: we store the connection as an internal type but expose it
	 *         here as an external type.
	 */
	...
	/**
	 * Inserts the UserException inside the given ApplicationException into the
	 * given Any. Throws an UNKNOWN with minor code
	...
	private void insertUserException(UserException userException, Any result) throws UNKNOWN {
		try {
			...
			if (userException != null) {
				...
				String helperClassName = className + "Helper";
				Class<?> helperClass = SharedSecrets.getJavaCorbaAccess()
	...
	/**
	 * Utility to wrap the given Exception in an Any object and return it. If
	 * the exception is a UserException which cannot be inserted into an any,
	 * then this returns an Any containing the system exception UNKNOWN.
	 */
	...
	/**
	 * Utility method to look up a service context with the given id and convert
	 ...
	 * object. If replace is true, any service context with the given id is
	 * replaced.
	 * <p>
	 * Raises BAD_INV_ORDER if replace is false and a service context with the
	...
	protected void addServiceContext(HashMap cachedServiceContexts, ServiceContexts serviceContexts,
		...
		EncapsOutputStream outputStream = sun.corba.OutputStreamFactory
		...
		coreServiceContext = new UnknownServiceContext(inputStream.read_long(),
	...
	/**
	 * Sets the number of interceptors whose starting interception points were
	 * successfully invoked on this client call. As specified in orbos/99-12-02,
	 * section 5.2.1., not all interceptors will be invoked if a ForwardRequest
	 * exception or a system exception is raised. This keeps track of how many
	 * were successfully executed so we know not to execute the corresponding
	 * ending interception points for the interceptors whose starting
	 * interception points were not completed. This simulates the
	 * "Flow Stack Visual Model" presented in section 5.1.3.
	 */
	...
	/**
	 * Returns the number of interceptors whose starting interception points
	 * were actually invoked on this client request. See setFlowStackIndex for
	 * more details.
	 */
	...
	/**
	 * Sets which ending interception point should be called for each
	 * interceptor in the virtual flow stack.
	 */
	...
	/**
	 * Retrieves the current ending point call type (see setEndingPointCall for
	 * more details).
	 */
	...
	/**
	 * Sets which intermediate interception point should be called for each
	 * interceptor in the virtual flow stack.
	 */
	...
	/**
	 * Sets which starting interception point should be called for each
	 * interceptor in the virtual flow stack.
	 */
	...
	/**
	 * Retrieves the current starting point call type (see setStartingPointCall
	 * for more details).
	 */
	...
	/**
	 * Returns true if all interceptors' starting and ending points have already
	 * executed to completion, or false if not yet.
	 */
	...
	/**
	 * Sets whether all interceotrs' starting and ending points have already
	...
	/**
	 * Gets the current reply_status without doing an access check (available
	 * only to package and subclasses)
	 */
	...
	/**
	 * Stores the given ForwardRequest object for later analysis. This version
	 * supplements setForwardRequest( IOR );
	 */
	...
	/**
	 * Stores the given IOR for later forward request analysis. This version
	 * supplements setForwardRequest( ForwardRequest );
	 */
	...
	/**
	 * Sets the execution point that we are currently executing (starting
	 * points, intermediate points, or ending points). This allows us to enforce
	 * the validity table.
	 */
	...
	/**
	 * Check whether the caller is allowed to access this method at this
	 * particular time. This is overridden in subclasses to implement the
	 * validity table specified in ptc/00-04-05, table 21-1 and 21-2. The
	 * currentExecutionPoint attribute is checked, and if access is forbidden at
	...
	/**
	 * The server side does an explicit set rather than taking the current
	 * PICurrent table as is done in the general RequestInfoImpl constructor.
	 */
