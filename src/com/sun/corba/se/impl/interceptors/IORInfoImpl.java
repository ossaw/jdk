...
public final class IORInfoImpl extends LocalObject implements IORInfo, IORInfoExt {
    ...
    /**
     * An ORB service implementation may determine what server side policy of a
     * particular type is in effect for an IOR being constructed by calling the
     * get_effective_policy operation. When the IOR being constructed is for an
     * object implemented using a POA, all Policy objects passed to the
     * PortableServer::POA::create_POA call that created that POA are accessible
     ...
     * set which will be included when constructing IORs. The components in this
     * set will be included in all profiles.
     * <p>
     ...
     * implementation of establish_components to add a tagged component to the
     * set which will be included when constructing IORs. The components in this
     * set will be included in the specified profile.
     * <p>
     * Any number of components may exist with the same component ID.
     * <p>
     * If the given profile ID does not define a known profile or it is
     * impossible to add components to thgat profile, BAD_PARAM is raised with a
    ...
    public int getServerPort(String type) throws UnknownType {
        ...
        int port = orb.getLegacyServerSocketManager().legacyGetTransientOrPersistentServerPort(type);
        if (port == -1) {
    ...
    private void checkState(int expectedState1, int expectedState2) {
        if ((expectedState1 != state) && (expectedState2 != state))
            throw wrapper.badState2(new Integer(expectedState1), new Integer(expectedState2), new Integer(
                    state));
    }
