...
public final class ClientRequestInfoImpl extends RequestInfoImpl implements ClientRequestInfo {
	...
	/**
	 * Creates a new ClientRequestInfo implementation. The constructor is
	 ...
	 * effective_target will contain the forwarded IOR while target will remain
	 * unchanged.
	 */
	...
	/**
	 * The profile that will be used to send the request. If a location forward
	 * has occurred for this operation's object and that object's profile change
	 * accordingly, then this profile will be that located profile.
	 */
	...
	public int request_id() {
		...
		/*
		 * NOTE: The requestId in client interceptors is the same as the GIOP
		 * request id. This works because both interceptors and request ids are
		 * scoped by the ORB on the client side.
		 */
	...
	protected void setLocatedIOR(IOR ior) {
		...
		CorbaContactInfoListIterator iterator = (CorbaContactInfoListIterator) ((CorbaInvocationInfo) orb
	...
	/**
	 * The PICurrent stack should only be popped if it was pushed. This is
	 * generally the case. But exceptions which occur after the stub's entry to
	 * _request but before the push end up in _releaseReply which will try to
	 * pop unless told not to.
	 */
