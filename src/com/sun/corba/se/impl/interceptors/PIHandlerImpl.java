...
public class PIHandlerImpl implements PIHandler {
    ...
    public PIHandlerImpl(ORB orb, String[] args) {
        ...
        orb.getLocalResolver().register(ORBConstants.PI_CURRENT_NAME, ClosureFactory.makeConstant(current));
        orb.getLocalResolver().register(ORBConstants.CODEC_FACTORY_NAME, ClosureFactory.makeConstant(
                codecFactory));
    }
    ...
    /**
     * ptc/00-08-06 p 205: "When an application calls ORB::destroy, the ORB 1)
     * waits for all requests in progress to complete 2) calls the
     * Interceptor::destroy operation for each interceptor 3) completes
     * destruction of the ORB"
     * This must be called at the end of ORB.destroy. Note that this is not part
     * of the PIHandler interface, since ORBImpl implements the ORB interface.
     */
    ...
    public void invokeClientPIStartingPoint() throws RemarshalException {
        ...
        if ((replyStatus == SYSTEM_EXCEPTION.value) || (replyStatus == LOCATION_FORWARD.value)) {
            ...
            Exception exception = invokeClientPIEndingPoint(convertPIReplyStatusToReplyMessage(replyStatus),
                    info.getException());
            if (exception == null) {
    ...
    public void invokeServerPIEndingPoint(ReplyMessage replyMessage) {
        ...
        if (!info.getAlreadyExecuted()) {
            ...
            if (!info.isDynamic() && (piReplyStatus == USER_EXCEPTION.value)) {
                info.setException(omgWrapper.unknownUserException(CompletionStatus.COMPLETED_MAYBE));
            }
    ...
    /**
     * Handles exceptions for the starting and intermediate points for server
     * request interceptors. This is common code that has been factored out into
     * this utility method.
     * <p>
    ...
    private void serverPIHandleExceptions(ServerRequestInfoImpl info) {
        ...
        } else if ((endingPointCall == ServerRequestInfoImpl.CALL_SEND_OTHER) && (info
                .getForwardRequestException() != null)) {
     ...
     * performed as quickly since it is only executed in exception conditions.
     */
    ...
    /**
     * Convenience method to get the ClientRequestInfoImpl object off the top of
     * the ThreadLocal stack. Throws an INTERNAL exception if the Info stack is
     * empty.
     */
    ...
    /**
     * Convenience method to get the ServerRequestInfoImpl object off the top of
    ...
    /**
     * Convenience method to determine whether Client PI is enabled for requests
     * on this thread.
     */
    ...
    /**
     * Creates the ORBInitInfo object to be passed to ORB intializers' pre_init
     * and post_init methods
     */
    ...
    /**
     * Called by ORBInitInfo when an interceptor needs to be registered. The
     * type is one of:
     * <ul>
     ...
     * class. This method finds the Policy Factory for the given Policy Type and
     * instantiates the Policy object from the Factory. It will throw
     * PolicyError exception, If the PolicyFactory for the given type is not
     * registered. _REVISIT_, Once Policy Framework work is completed,
     * Reorganize this method to com.sun.corba.se.spi.orb.ORB.
     */
    public org.omg.CORBA.Policy create_policy(int type, org.omg.CORBA.Any val)
        ...
        if (policyFactoryTable == null) {
            throw new org.omg.CORBA.PolicyError("There is no PolicyFactory Registered for type " + type,
                    BAD_POLICY.value);
        }
        PolicyFactory factory = (PolicyFactory) policyFactoryTable.get(new Integer(type));
        if (factory == null) {
            throw new org.omg.CORBA.PolicyError(" Could Not Find PolicyFactory for the Type " + type,
                    BAD_POLICY.value);
        }
    ...
    /**
     * This method registers the Policy Factory in the policyFactoryTable, which
     * is a HashMap. This method is made package private, because it is used
     * internally by the Interceptors.
     */
