...
public final class OldJIDLObjectKeyTemplate extends OldObjectKeyTemplateBase {
     ...
     * keys. JDK 1.3.1_01 included the byte with the value 1. Anything below 1
    ...
    public OldJIDLObjectKeyTemplate(ORB orb, int magic, int scid, InputStream is, OctetSeqHolder osh) {
        ...
        /**
         * Beginning with JDK 1.3.1_01, a byte was placed at the end of the
         * object key with a value indicating the patch version. JDK 1.3.1_01
         * had the value 1. If other patches are necessary which involve ORB
         * versioning changes, they should increment the patch version.
         * Note that if we see a value greater than 1 in this code, we will
         * treat it as if we're talking to the most recent ORB version.
         * WARNING: This code is sensitive to changes in CDRInputStream
         * getPosition. It assumes that the CDRInputStream is an encapsulation
         * whose position can be compared to the object key array length.
         */
        if (magic == ObjectKeyFactoryImpl.JAVAMAGIC_NEW && osh.value.length > ((CDRInputStream) is)
                .getPosition()) {
