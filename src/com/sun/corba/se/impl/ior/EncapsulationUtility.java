...
public class EncapsulationUtility {
    ...
    /**
     * Helper method that is used to extract data from an output stream and
    ...
    /**
     * Helper method to read the octet array from is, deencapsulate it, and
     * return as another InputStream. This must be called inside the constructor
     * of a derived class to obtain the correct stream for unmarshalling data.
     */
    static public InputStream getEncapsulationStream(InputStream is) {
        byte[] data = readOctets(is);
        EncapsInputStream result = EncapsInputStreamFactory.newEncapsInputStream(is.orb(), data, data.length);
        result.consumeEndian();
        return result;
    }

    /**
     * Helper method that reads an octet array from an input stream. Defined as
     * static here so that it can be used in another class.
     */
    ...
    static public void writeEncapsulation(WriteContents obj, OutputStream os) {
        EncapsOutputStream out = sun.corba.OutputStreamFactory.newEncapsOutputStream((ORB) os.orb());
