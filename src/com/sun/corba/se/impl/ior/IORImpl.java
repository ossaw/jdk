...
/**
 * An IOR is represented as a list of profiles. Only objects that extend
 * TaggedProfile should be added to an IOR. However, enforcing this restriction
 * requires overriding all of the addXXX methods inherited from List, so no
...
public class IORImpl extends IdentifiableContainerBase implements IOR {
	...
	/*
	 * This variable is set directly from the constructors that take an
	 * IORTemplate or IORTemplateList as arguments; otherwise it is derived from
	 * the list of TaggedProfile instances on the first call to getIORTemplates.
	 * Note that we assume that an IOR with mutiple TaggedProfile instances has
	 * the same ObjectId in each TaggedProfile, as otherwise the IOR could never
	 * be created through an ObjectReferenceFactory.
	 */
	...
	private void initializeIORTemplateList() {
		...
		ObjectId oid = null; // used to check that all profiles have the same
								// oid.
		while (iter.hasNext()) {
			...
			// Check that all oids for all profiles are the same: if they are
			// not,
			// throw exception.
			if (oid == null)
	...
	/**
	 * Return the IORTemplateList for this IOR. Will throw exception if it is
	 * not possible to generate an IOR from the IORTemplateList that is equal to
	 * this IOR, which can only happen if not every TaggedProfile in the IOR has
	 * the same ObjectId.
	 */
