...
public abstract class MessageBase implements Message {
    ...
    public static MessageBase readGIOPHeader(ORB orb, CorbaConnection connection) {
        ...
        try {
            buf = connection.read(GIOPMessageHeaderLength, 0, GIOPMessageHeaderLength, readTimeouts
        ...
        if ((buf.get(4) == 0x0D) && (buf.get(5) <= Message.JAVA_ENC_VERSION) && (buf.get(
                5) > Message.CDR_ENC_VERSION) && orb.getORBData().isJavaSerializationEnabled()) {
        ...
        if ((buf.get(4) > orbVersion.getMajor()) || ((buf.get(4) == orbVersion.getMajor()) && (buf.get(
                5) > orbVersion.getMinor()))) {
        ...
        if ((buf.get(4) == 0x01) && (buf.get(5) == 0x00)) { // 1.0
            ...
            msg10.message_size = readSize(buf.get(8), buf.get(9), buf.get(10), buf.get(11), msg10
                    .isLittleEndian()) + GIOPMessageHeaderLength;
        } else { // 1.1 & 1.2
            ...
            msg11.message_size = readSize(buf.get(8), buf.get(9), buf.get(10), buf.get(11), msg11
                    .isLittleEndian()) + GIOPMessageHeaderLength;
        }
    ...
    public static Message readGIOPBody(ORB orb, CorbaConnection connection, Message msg) {
        ...
        try {
            buf = connection.read(buf, GIOPMessageHeaderLength, msgSizeMinusHeader, readTimeouts
    ...
    private static RequestMessage createRequest(ORB orb, GIOPVersion gv, byte encodingVersion, int request_id,
        ...
        if (gv.equals(GIOPVersion.V1_0)) { // 1.0
            return new RequestMessage_1_0(orb, service_contexts, request_id, response_expected, object_key,
                    operation, requesting_principal);
        } else if (gv.equals(GIOPVersion.V1_1)) { // 1.1
            return new RequestMessage_1_1(orb, service_contexts, request_id, response_expected, new byte[] {
                    0x00, 0x00, 0x00 }, object_key, operation, requesting_principal);
            ...
            RequestMessage msg = new RequestMessage_1_2(orb, request_id, response_flags, new byte[] { 0x00,
                    0x00, 0x00 }, target, operation, service_contexts);
    ...
    public static RequestMessage createRequest(ORB orb, GIOPVersion gv, byte encodingVersion, int request_id,
            boolean response_expected, IOR ior, short addrDisp, String operation,
        ...
        if (addrDisp == KeyAddr.value) {
            ...
            requestMessage = new RequestMessage_1_2(orb, request_id, response_flags, new byte[] { 0x00, 0x00,
                    0x00 }, target, operation, service_contexts);
    ...
    public static LocateReplyMessage createLocateReply(ORB orb, GIOPVersion gv, byte encodingVersion,
