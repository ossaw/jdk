...
public class CorbaServerRequestDispatcherImpl implements CorbaServerRequestDispatcher {
	...
	/**
	 * XXX/REVISIT: We do not want to look for a servant in the
	 ...
	 * general the object may be destroyed between a locate and a request. Note
	 * that this only checks that the appropriate ObjectAdapter is available,
	 * not that the servant actually exists. Need to signal one of OBJECT_HERE,
	 * OBJECT_FORWARD, OBJECT_NOT_EXIST.
	 * 
	 ...
	 *                is thrown if we know the object does not exist here, and
	...
	public IOR locate(ObjectKey okey) {
		try {
			...
			// Called only for its side-effect of throwing appropriate
			// exceptions
	...
	public void dispatch(MessageMediator messageMediator) {
		...
		try {
			...
			try {
				...
				java.lang.Object servant = getServantWithPI(request, objectAdapter, objectId,
						oktemp, operation);
				...
				// The problem is that OADestroyed is only thrown by oa.enter,
				// in
				...
				SystemException sysex = wrapper
						.unknownExceptionInDispatch(CompletionStatus.COMPLETED_MAYBE, ex);
				request.getProtocolHandler().createSystemExceptionResponse(request, sysex,
						contexts);
			} catch (Throwable ex) {
				if (orb.subcontractDebugFlag) {
	...
	protected java.lang.Object getServantWithPI(CorbaMessageMediator request,
		...
		try {
			...
			// This must be set just after the enter so that exceptions thrown
			// by
			// enter do not cause
			// the exception reply to pop the thread stack and do an extra
			// oa.exit.
			if (request != null)
	...
	/**
	 * Always throws OBJECT_NOT_EXIST if operation is not a special method. If
	 * operation is _non_existent or _not_existent, this will just return
	 * without performing any action, so that _non_existent can return false.
	 * Always throws OBJECT_NOT_EXIST for any other special method. Update for
	 * issue 4385.
	 */
	...
	protected void consumeServiceContexts(CorbaMessageMediator request) {
		try {
			...
			// in this service context. Can we rely on just the
			...
			if (orb.subcontractDebugFlag) {
				dprint(".consumeServiceContexts: " + opAndId(request) + ": GIOP version: "
						+ giopVersion);
			...
			// are talking to the legacy ORB and set the ORB Version
			// Accordingly.
	...
	protected CorbaMessageMediator dispatchToServant(java.lang.Object servant,
			CorbaMessageMediator req, byte[] objectId, ObjectAdapter objectAdapter) {
		try {
			if (orb.subcontractDebugFlag) {
	...
	protected boolean processCodeSetContext(CorbaMessageMediator request,
		...
		try {
			...
			if (sc != null) {
				...
				// If it's GIOP 1.0, it shouldn't have this context at all. Our
				// legacy
				// ORBs sent it and we need to know if it's here to make ORB
				// versioning
				...
				// The connection's codeSetContext is null until we've received
				// a
