 ...
 * BootstrapServer. It implements Server so that it can be registered as a
 * subcontract. It is passed a BootstrapServiceProperties object which contains
 * the supported ids and their values for the bootstrap service. This Properties
 * object is only read from, never written to, and is shared among all threads.
 * <p>
 * The BootstrapServerRequestDispatcher responds primarily to GIOP requests, but
...
public class BootstrapServerRequestDispatcher implements CorbaServerRequestDispatcher {
	...
	/**
	 * Locates the object mentioned in the locate requests, and returns object
	 * here iff the object is the initial object key. A SystemException thrown
	 * if the object key is not the initial object key.
	 */
