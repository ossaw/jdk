...
/**
 * ClientDelegate is the RMI client-side subcontract or representation It
...
public class CorbaClientRequestDispatcherImpl implements ClientRequestDispatcher {
	...
	public OutputObject beginRequest(Object self, String opName, boolean isOneWay,
		...
		try {
			...
			// 7046238 - Synchronization on a single monitor for contactInfo
			// parameters
			// with identical hashCode(), so we lock on same monitor for equal
			// parameters
			// (which can refer to equal (in terms of equals()) but not the same
			// objects)
			...
			synchronized (lock) {
				if (contactInfo.isConnectionBased()) {
					...
					if (connection != null) {
						...
						// connections
						// are marked in use by registerWaiter() call and since
						// this
			...
			CorbaMessageMediator messageMediator = (CorbaMessageMediator) contactInfo
					.createMessageMediator(orb, contactInfo, connection, opName, isOneWay);
			if (orb.subcontractDebugFlag) {
			...
			try {
				...
				if (getContactInfoListIterator(orb).hasNext()) {
					...
					// starting another.
					orb.getPIHandler().makeCompletedClientRequest(ReplyMessage.LOCATION_FORWARD,
							null);
					unregisterWaiter(orb);
	...
	public InputObject marshalingComplete(java.lang.Object self, OutputObject outputObject)
		...
		try {
			...
			InputObject inputObject = marshalingComplete1(orb, messageMediator);
	...
	public InputObject marshalingComplete1(ORB orb, CorbaMessageMediator messageMediator)
		...
		try {
			...
			if (orb.subcontractDebugFlag) {
				dprint(".marshalingComplete: " + opAndId(messageMediator) + ": exception: "
						+ e.toString());
			}
			...
			Exception newException = orb.getPIHandler()
					.invokeClientPIEndingPoint(ReplyMessage.SYSTEM_EXCEPTION, e);
	...
	protected InputObject processResponse(ORB orb, CorbaMessageMediator messageMediator,
		...
		if (messageMediator.isOneWay()) {
			...
			exception = orb.getPIHandler().invokeClientPIEndingPoint(ReplyMessage.NO_EXCEPTION,
					exception);
		...
		if (messageMediator.isSystemExceptionReply()) {
			...
			if (doRemarshal) {
				...
				exception = orb.getPIHandler()
						.invokeClientPIEndingPoint(ReplyMessage.SYSTEM_EXCEPTION, se);
			...
			if (contexts != null) {
				UEInfoServiceContext usc = (UEInfoServiceContext) contexts
						.get(UEInfoServiceContext.SERVICE_CONTEXT_ID);
				...
				if (usc != null) {
					...
					// Invoke Portable Interceptors with receive_exception:
					exception = orb.getPIHandler()
							.invokeClientPIEndingPoint(ReplyMessage.SYSTEM_EXCEPTION, ue);
			...
			if (messageMediator.isDIIRequest()) {
				...
				newException = orb.getPIHandler()
						.invokeClientPIEndingPoint(ReplyMessage.USER_EXCEPTION, exception);
				...
				newException = orb.getPIHandler()
						.invokeClientPIEndingPoint(ReplyMessage.USER_EXCEPTION, appException);
			}
			...
			// Invoke Portable Interceptors with receive_other:
			Exception newException = orb.getPIHandler()
					.invokeClientPIEndingPoint(ReplyMessage.LOCATION_FORWARD, null);
			...
			// Invoke Portable Interceptors with receive_other:
			Exception newException = orb.getPIHandler()
					.invokeClientPIEndingPoint(ReplyMessage.NEEDS_ADDRESSING_MODE, null);
	...
	protected void continueOrThrowSystemOrRemarshal(CorbaMessageMediator messageMediator,
			Exception exception) throws SystemException, RemarshalException {
	...
	protected void getExceptionDetailMessage(CorbaMessageMediator messageMediator,
		...
		EncapsInputStream in = EncapsInputStreamFactory
				.newEncapsInputStream((ORB) messageMediator.getBroker(), data, data.length);
		in.consumeEndian();
	...
	protected void performCodeSetNegotiation(CorbaMessageMediator messageMediator) {
		...
		if (conn != null && conn.getCodeSetContext() == null
			...
			synchronized (conn) {
				...
				CodeSetComponentInfo.CodeSetContext result = CodeSetConversion.impl().negotiate(
						conn.getBroker().getORBData().getCodeSetComponentInfo(), serverCodeSets);
