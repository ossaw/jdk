...
public class ValueUtility {
    ...
    public static String getSignature(ValueMember member) throws ClassNotFoundException {
        ...
        if (member.type.kind().value() == TCKind._tk_value_box || member.type.kind()
                .value() == TCKind._tk_value || member.type.kind().value() == TCKind._tk_objref) {
    ...
    public static FullValueDescription translate(ORB orb, ObjectStreamClass osc, ValueHandler vh) {
        ...
        for (int interfaceIndex = 0; interfaceIndex < interfaces.length; interfaceIndex++) {
            result.supported_interfaces[interfaceIndex] = vhandler.createForAnyType(
                    interfaces[interfaceIndex]);

            if ((!(java.rmi.Remote.class.isAssignableFrom(interfaces[interfaceIndex]))) || (!Modifier
                    .isPublic(interfaces[interfaceIndex].getModifiers())))
                abstractCount++;
        }
        ...
        for (int interfaceIndex = 0; interfaceIndex < interfaces.length; interfaceIndex++) {
            if ((!(java.rmi.Remote.class.isAssignableFrom(interfaces[interfaceIndex]))) || (!Modifier
                    .isPublic(interfaces[interfaceIndex].getModifiers())))
                result.abstract_base_values[interfaceIndex] = vhandler.createForAnyType(
                        interfaces[interfaceIndex]);
    ...
    private static TypeCode createTypeCodeForClassInternal(ORB orb, java.lang.Class c, ValueHandler vh,
            IdentityKeyValueStack createdIDs) {
    ...
    private static TypeCode createTypeCodeInternal(ORB orb, java.lang.Class c, ValueHandler vh, String id,
            IdentityKeyValueStack createdIDs) {
        if (c.isArray()) {
        ...
        short modifier = (osc.isCustomMarshaled() ? org.omg.CORBA.VM_CUSTOM.value
