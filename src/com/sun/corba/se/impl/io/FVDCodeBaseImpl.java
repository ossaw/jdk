...
/**
 * This class acts as the remote interface to receivers wishing to retrieve the
 * information of a remote Class.
 */
    ...
    public FullValueDescription meta(String x) {
        try {
            ...
            if (result == null) {
                ...
                try {
                    result = ValueUtility.translate(_orb(), ObjectStreamClass.lookup(vhandler
                            .getAnyClassFromType(x)), vhandler);
                } catch (Throwable t) {
                    if (orb == null)
                        orb = ORB.init(); // d11638
                    result = ValueUtility.translate(orb, ObjectStreamClass.lookup(vhandler
                            .getAnyClassFromType(x)), vhandler);
                }
