...
public class ObjectStreamClass implements java.io.Serializable {
	...
	private static final Bridge bridge = AccessController
			.doPrivileged(new PrivilegedAction<Bridge>() {
				public Bridge run() {
					return Bridge.get();
				}
			});
		...
		if (suid != localClassDesc.suid) {
			...
			if (!arraySUID && !addedSerialOrExtern) {
				...
				throw new InvalidClassException(cl.getName(),
						"Local class not compatible:" + " stream classdesc serialVersionUID=" + suid
			...
			for (int k = j; k < destfield.length; k++) {
				if (srcfield[i].getName().equals(destfield[k].getName())) {
					...
					if (srcfield[i].isPrimitive() && !srcfield[i].typeEquals(destfield[k])) {
						// XXX I18N, logging needed
						throw new InvalidClassException(cl.getName(),
								"The type of field " + srcfield[i].getName() + " of class " + name
										+ " is incompatible.");
					}
	...
	static boolean compareClassNames(String streamName, String localName, char pkgSeparator) {
		...
		return streamName.regionMatches(false, streamNameIndex, localName, localNameIndex,
	...
	private static long _computeSerialVersionUID(Class<?> cl) {
		...
		try {
			...
			classaccess &= (Modifier.PUBLIC | Modifier.FINAL | Modifier.INTERFACE
					| Modifier.ABSTRACT);
		...
		if (hasStaticInitializerMethod == null) {
			...
			try {
				...
				hasStaticInitializerMethod = classWithThisMethod
						.getDeclaredMethod("hasStaticInitializer", new Class<?>[] { Class.class });
			} catch (NoSuchMethodException ex) {
			}
