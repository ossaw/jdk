...
/**
 * A ObjectStreamClass describes a class that can be serialized to a stream or a
 * class that was serialized to a stream. It contains the name and the
 * serialVersionUID of the class. <br>
 * The ObjectStreamClass for a specific class loaded in this Java VM can be
 * found using the lookup method.
 *
 * @author Roger Riggs
 * @since JDK1.1
 */
public class ObjectStreamClass implements java.io.Serializable {
    ...
    /**
     * Find the descriptor for a class that can be serialized. Null is returned
     * if the specified class does not implement java.io.Serializable or
    ...
    /*
     * Find the class descriptor for the specified class. Package access only so
     * it can be called from ObjectIn/OutStream.
     */
    static ObjectStreamClass lookupInternal(Class<?> cl) {
        /*
         * Synchronize on the hashtable so no two threads will do this at the
         * same time.
         */
        ...
        synchronized (descriptorFor) {
            ...
            if (desc == null) {
                ...
                /*
                 * If the class is only Serializable, lookup the descriptor for
                 * the superclass.
                 */
                ...
                /*
                 * Check if its' externalizable. If it's Externalizable, clear
                 * the serializable flag. Only one or the other may be set in
                 * the protocol.
                 */
                boolean externalizable = false;
                if (serializable) {
                    externalizable = ((superdesc != null) && superdesc
                            .isExternalizable()) || Externalizable.class
                                    .isAssignableFrom(cl);
                    if (externalizable) {
                        serializable = false;
                    }
                ...
                /*
                 * Create a new version descriptor, it put itself in the known
                 * table.
                 */
                desc = new ObjectStreamClass(cl, superdesc, serializable,
                        externalizable);
            }
    ...
    /**
     * Return the serialVersionUID for this class. The serialVersionUID defines
     * a set of classes all with the same name that have evolved from a common
     * root class and agree to be serialized and deserialized using a common
     * format.
     */
    ...
    /**
     * Return the serialVersionUID string for this class. The serialVersionUID
     ...
     */
    public static final long getActualSerialVersionUID(
     ...
     * 
     * @return an array containing an element for each persistent field of this
     *         class. Returns an array of length zero if there are no fields.
     * @since JDK1.2
     */
        ...
        try {
            for (int i = 0; i < fields.length; i++) {
                if (fields[i].getName().equals(field.name)) {
                    if (fields[i].getSignature().equals(ValueUtility
                            .getSignature(field)))
                        return true;
                }
            }
    ...
    /*
     * Create a new ObjectStreamClass from a loaded class. Don't call this
     * directly, call lookup instead.
     */
    private ObjectStreamClass(java.lang.Class<?> cl,
            ObjectStreamClass superdesc, boolean serial, boolean extern) {
        ...
        /*
         * Enter this class in the table of known descriptors. Otherwise, when
         * the fields are read it may recurse trying to find the descriptor for
         * itself.
         */
    ...
    private static final class PersistentFieldsValue extends
            ClassValue<ObjectStreamField[]> {
        ...
        protected ObjectStreamField[] computeValue(Class<?> type) {
            try {
                ...
                if (Modifier.isPrivate(mods) && Modifier.isStatic(mods)
                    ...
                    java.io.ObjectStreamField[] fields = (java.io.ObjectStreamField[]) pf
                            .get(type);
    ...
    private void init() {
        synchronized (lock) {
            ...
            if (!serializable || externalizable || forProxyClass || name.equals(
                    "java.lang.String")) {
                fields = NO_FIELDS;
                        ...
                        if (fields == null) {
                            ...
                            for (int i = 0; i < actualfields.length; i++) {
                                ...
                                if (!Modifier.isStatic(modifiers) && !Modifier
                                    ...
                                    tempFields[numFields++] = new ObjectStreamField(
                                            fld);
                                }
                            }
                            ...
                            // actual
                            // reflected Field. If there is one, make sure it's
                            // the correct
                            // type and cache it in the ObjectStreamClass for
                            // that field.
                            for (int j = fields.length - 1; j >= 0; j--) {
                                try {
                                    Field reflField = cl.getDeclaredField(
                                            fields[j].getName());
                                    if (fields[j].getType() == reflField
                                            .getType()) {
            ...
            /*
             * Get the serialVersionUID from the class. It uses the access
             * override mechanism so make sure the field objects is only used
             * here.
                ...
                AccessController.doPrivileged(new PrivilegedAction() {
                    public Object run() {
                        if (forProxyClass) {
                            ...
                            try {
                                final Field f = cl.getDeclaredField(
                                        "serialVersionUID");
                                int mods = f.getModifiers();
                                // SerialBug 5: static final SUID should be read
                                if (Modifier.isStatic(mods) && Modifier.isFinal(
                                        mods)) {
                                    f.setAccessible(true);
                                    suid = f.getLong(cl);
                                    // SerialBug 2: should be computed after
                                    // writeObject
                        ...
                        writeReplaceObjectMethod = ObjectStreamClass
                                .getInheritableMethod(cl, "writeReplace",
                                        noTypesList, Object.class);

                        readResolveObjectMethod = ObjectStreamClass
                                .getInheritableMethod(cl, "readResolve",
                                        noTypesList, Object.class);
                        ...
                        if (serializable && !forProxyClass) {
                            /*
                             * Look for the writeObject method Set the
                             * accessible flag on it here. ObjectOutputStream
                             * will call it as necessary.
                             */
                            writeObjectMethod = getPrivateMethod(cl,
                                    "writeObject", new Class<?>[] {
    ...
    private static Method getPrivateMethod(Class<?> cl, String name,
        ...
        try {
            ...
            return ((meth.getReturnType() == returnType) && ((mods
                    & Modifier.STATIC) == 0) && ((mods
                            & Modifier.PRIVATE) != 0)) ? meth : null;
     ...
     * fully-qualified name of the class whose writeObject method is being
    ...
    /*
     * Create an empty ObjectStreamClass for a class about to be read. This is
     * separate from read so ObjectInputStream can assign the wire handle early,
     * before any nested ObjectStreamClass might be read.
     */
    ...
    /*
     * Set the class this version descriptor matches. The base class name and
     * serializable hash must match. Fill in the reflected Fields that will be
     * used for reading.
     */
    final void setClass(Class<?> cl) throws InvalidClassException {
        ...
        if (suid != localClassDesc.suid) {
            ...
            /*
             * Allow adding Serializable or Externalizable to a later release of
             * the class.
             */
            boolean addedSerialOrExtern = isNonSerializable() || localClassDesc
                    .isNonSerializable();

            /*
             * Disregard the serialVersionUID of an array when name and cl.Name
             * differ. If resolveClass() returns an array with a different
             * package name, the serialVersionUIDs will not match since the
             * fully qualified array class is used in the computation of the
             * array's serialVersionUID. There is no way to set a permanent
             * serialVersionUID for an array type.
             */
        ...
        if (!compareClassNames(name, cl.getName(), '.'))
            ...
            throw new InvalidClassException(cl.getName(),
                    "Incompatible local class name. "
                            + "Expected class name compatible with " + name);
         ...
         * field in this descriptor should be stored. Each field in this
         * ObjectStreamClass (the source) is located (by name) in the
         * ObjectStreamClass of the class(the destination). In the usual
         * (non-versioned case) the field is in both descriptors and the types
         * match, so the reflected Field is copied. If the type does not match,
         * a InvalidClass exception is thrown. If the field is not present in
         * the class, the reflected Field remains null so the field will be read
         * but discarded. If extra fields are present in the class they are
         * ignored. Their values will be set to the default value by the object
         * allocator. Both the src and dest field list are sorted by type and
         * name.
         */
        ...
        nextsrc: for (int i = 0; i < srcfield.length; i++) {
            ...
            for (int k = j; k < destfield.length; k++) {
                if (srcfield[i].getName().equals(destfield[k].getName())) {
                    /* found match */
                    if (srcfield[i].isPrimitive() && !srcfield[i].typeEquals(
                            destfield[k])) {
                        // XXX I18N, logging needed
                        throw new InvalidClassException(cl.getName(),
                                "The type of field " + srcfield[i].getName()
                                        + " of class " + name
                                        + " is incompatible.");
                    }
        ...
        /*
         * get the cache of these methods from the local class implementation.
         */
     ...
     */
    static boolean compareClassNames(String streamName, String localName,
            char pkgSeparator) {
        /* compare the class names, stripping off package names. */
        ...
        return streamName.regionMatches(false, streamNameIndex, localName,
                localNameIndex, streamName.length() - streamNameIndex);
    }

    /*
     * Compare the types of two class descriptors. They match if they have the
     * same class name and suid
     */
    ...
    /**
     * Returns when or not this class should be custom marshaled (use chunking).
     * This should happen if it is Externalizable OR if it or any of its
     * superclasses has a writeObject method,
     */
    final boolean isCustomMarshaled() {
        return (hasWriteObject() || isExternalizable()) || (superclass != null
                && superclass.isCustomMarshaled());
    }

    /*
     * Return true if all instances of 'this' Externalizable class are written
     * in block-data mode from the stream that 'this' was read from. <p>
     * In JDK 1.1, all Externalizable instances are not written in block-data
     * mode. In JDK 1.2, all Externalizable instances, by default, are written
     * in block-data mode and the Externalizable instance is terminated with tag
     * TC_ENDBLOCKDATA. Change enabled the ability to skip Externalizable
     * instances.
     * IMPLEMENTATION NOTE: This should have been a mode maintained per stream;
     * however, for compatibility reasons, it was only possible to record this
     * change per class. All Externalizable classes within a given stream should
     * either have this mode enabled or disabled. This is enforced by not
     * allowing the PROTOCOL_VERSION of a stream to he changed after any objects
     * have been written.
    ...
    Object newInstance() throws InstantiationException,
            InvocationTargetException, UnsupportedOperationException {
        if (cons != null) {
            try {
    ...
    private static Constructor getSerializableConstructor(Class<?> cl) {
        ...
        try {
            ...
            if ((mods & Modifier.PRIVATE) != 0 || ((mods & (Modifier.PUBLIC
     ...
     * number of object references to read when reading from the input stream.
     */
    ...
    /*
     * JDK 1.5 has introduced some new modifier bits (such as SYNTHETIC) that
     * can affect the SVUID computation (see bug 4897937). These bits must be
     * ignored, as otherwise interoperability with ORBs in earlier JDK versions
     * can be compromised. I am adding these masks for this purpose as discussed
     * in the CCC for this bug (see http://ccc.sfbay/4897937).
     */
    ...
    /*
     * Compute a hash for the specified class. Incrementally add items to the
     * hash accumulating in the digest stream. Fold the hash into a long. Use
    ...
    private static long _computeSerialVersionUID(Class<?> cl) {
        ...
        try {
             ...
             * the interface had some methods. The ABSTRACT bit reflects that
             * the number of methods > 0. This is required so correct hashes can
             * be computed for existing class files. Previously this hack was
             * previously present in the VM.
             */
            ...
            /*
             * Get the list of interfaces supported, Accumulate their names
             * their names in Lexical order and add them to the hash
             */
            if (!cl.isArray()) {
                /*
                 * In 1.2fcs, getInterfaces() was modified to return
                 * {java.lang.Cloneable, java.io.Serializable} when called on
                 * array classes. These values would upset the computation of
                 * the hash, so we explicitly omit them from its computation.
                 */
            ...
            for (int i = 0; i < field.length; i++) {
                ...
                /*
                 * Include in the hash all fields except those that are private
            ...
            if (hasStaticInitializer(cl)) {
                ...
                data.writeInt(Modifier.STATIC); // TBD: what modifiers does it
                                                // have
            ...
            /*
             * Get the list of constructors including name and signature Sort
             * lexically, add all except the private constructors to the hash
            ...
            /*
             * Compute the hash value for this class. Use only the first 64 bits
             * of the hash.
             */
            data.flush();
    ...
    /*
     * findDescriptorFor a Class. This looks in the cache for a mapping from
     * Class -> ObjectStreamClass mappings. The hashCode of the Class is used
     * for the lookup since the Class is the key. The entries are extended from
     * java.lang.ref.SoftReference so the gc will be able to free them if
     * needed.
     */
    ...
    /*
     * The total number of bytes of primitive fields. The total number of object
     * fields.
     */
    int primBytes;
    int objFields;
    ...
    /**
     * Flag indicating whether or not this instance has successfully completed
     * initialization. This is to try to fix bug 4373844. Working to move to
    ...
    /*
     * In JDK 1.1, external data was not written in block mode. As of JDK 1.2,
     * external data is written in block data mode. This flag enables JDK 1.2 to
     * be able to read JDK 1.1 written external data.
     * @since JDK 1.2
     */
    ...
    /**
     * Beginning in Java to IDL ptc/02-01-12, RMI-IIOP has a stream format
     * version 2 which puts a fake valuetype around a Serializable's optional
     * custom data. This valuetype has a special repository ID made from the
     * Serializable's information which we are pre-computing and storing here.
     */
        ...
        if (hasStaticInitializerMethod == null) {
            ...
            try {
                ...
                hasStaticInitializerMethod = classWithThisMethod
                        .getDeclaredMethod("hasStaticInitializer",
    ...
    /*
     * Entries held in the Cache of known ObjectStreamClass objects. Entries are
     * chained together with the same hash value (modulo array size).
     */
    private static class ObjectStreamClassEntry // extends
     ...
     */
    private final static Comparator compareObjStrFieldsByName = new CompareObjStrFieldsByName();
    ...
    /*
     * It is expensive to recompute a method or constructor signature many
     * times, so compute it only once using this data structure.
     */
    private static class MethodSignature implements Comparator {
        ...
        /*
         * Given an array of Method or Constructor members, return a sorted
         * array of the non-private members.
         */
        /*
        ...
        /*
         * Assumes that o1 and o2 are either both methods or both constructors.
         */
        public int compare(Object o1, Object o2) {
     ...
     * is defined by or accessible (via inheritance) by the given class, or null
     * if no match found. Access checks are disabled on the returned method (if
     * any).
     *
