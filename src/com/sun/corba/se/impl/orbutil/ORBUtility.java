...
/**
 * Handy class full of static functions that don't belong in util.Utility for
 * pure ORB reasons.
 */
public final class ORBUtility {
	...
	private static boolean isSystemExceptionTypeCode(TypeCode type, ORB orb) {
		...
		try {
			return (type.kind().value() == TCKind._tk_except && type.member_count() == 3
	...
	/**
	 * Returns true if it was accurately determined that the remote ORB is a
	 * foreign (non-JavaSoft) ORB. Note: If passed the ORBSingleton, this will
	 * return false.
	 */
	...
	/**
	 * Marshal an integer to a byte array. The bytes are in BIGENDIAN order.
	 * i.e. array[offset] is the most-significant-byte and array[offset+3] is
	...
	public static SystemException readSystemException(InputStream strm) {
		try {
			...
			SystemException ex = (SystemException) SharedSecrets.getJavaCorbaAccess()
	...
	/**
	 * Get the class name corresponding to a particular repository Id. This is
	 * used by the system to unmarshal (instantiate) the appropriate exception
	...
	/**
	 * Get the repository id corresponding to a particular class. This is used
	 * by the system to write the appropriate repository id for a system
	 * exception.
	 * 
	 * @param name
	...
	/**
	 * Parse a version string such as "1.1.6" or "jdk1.2fcs" into a version
	 * array of integers {1, 1, 6} or {1, 2}. A string of "n." or "n..m" is
	 * equivalent to "n.0" or "n.0.m" respectively.
	 */
	...
	/**
	 * Compare two version arrays. Return 1, 0 or -1 if v1 is greater than,
	...
	public static int compareVersion(int[] v1, int[] v2) {
		...
		for (int i = 0; i < v1.length; ++i) {
			if (i >= v2.length || v1[i] > v2[i]) // v1 is longer or greater than
													// v2
				return 1;
			if (v1[i] < v2[i])
				return -1;
	...
	/**
	 * Compare two version strings. Return 1, 0 or -1 if v1 is greater than,
	 * equal to, or less than v2.
	 */
	...
	// Return a compressed representation of the thread name. This is
	// particularly
	...
	public static String getThreadName(Thread thr) {
		...
		// This depends on the formatting in SelectReaderThread and
		// CorbaConnectionImpl.
		...
		return compressClassName(ste.getClassName()) + "." + ste.getMethodName()
				+ (ste.isNativeMethod() ? "(Native Method)"
						: (ste.getFileName() != null && ste.getLineNumber() >= 0
								...
								: (ste.getFileName() != null ? "(" + ste.getFileName() + ")"
										: "(Unknown Source)")));
	}
	...
	public static synchronized void dprint(String className, String msg) {
		System.out.println(compressClassName(className) + "("
				+ getThreadName(Thread.currentThread()) + "): " + msg);
	}
	...
	/**
	 * This method obtains an IOR from a CORBA object reference. It will return
	 * null if obj is a local object, a null object, or an object implemented by
	 * a different ORB. It will throw BAD_OPERATION if obj is an unconnected
	 * RMI-IIOP object.
	 * 
	 * @return IOR the IOR that represents this objref. This will never be null.
	 * @exception BAD_OPERATION
	 *                (from oi._get_delegate) if obj is a normal objref, but
	 *                does not have a delegate set.
	 * @exception BAD_PARAM
	 *                if obj is a local object, or else was created by a foreign
	 *                ORB.
	 */
	public static IOR getIOR(org.omg.CORBA.Object obj) {
		...
		if (StubAdapter.isStub(obj)) {
			...
			// obj is implemented by a foreign ORB, because the Delegate is not
			// a
			// ClientDelegate.
	...
	/**
	 * Obtains an IOR for the object reference obj, first connecting it to the
	 * ORB if necessary.
	 * 
	 * @return IOR the IOR that represents this objref. This will never be null.
	 * @exception BAD_OPERATION
	 *                if the object could not be connected, if a connection
	 *                attempt was needed.
	 * @exception BAD_PARAM
	 *                if obj is a local object, or else was created by a foreign
	 *                ORB.
