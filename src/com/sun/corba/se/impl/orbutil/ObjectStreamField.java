...
class ObjectStreamField implements Comparable {
	...
	/**
	 * Compare this with another ObjectStreamField. return -1 if this is
	 * smaller, 0 if equal, 1 if greater types that are primitives are "smaller"
	 * than objects. if equal, the names are compared.
	 */
	...
	/**
	 * Compare the types of two class descriptors. The match if they have the
	 * same primitive types. or if they are both objects and the object types
	 * match.
	 */
	...
	/*
	 * Returns the Field ID NOT USED, since this class is used only in
	 * ObjectStreamClass_1_3_1, which is used only in RepositoryId_1_3_1. public
	 * long getFieldID( Class cl ) { if (fieldID == -1) { if (typeString !=
	 * null) fieldID = getFieldIDNative( cl, getName(), typeString ); else
	 * fieldID = getFieldIDNative( cl, getName(), getSignature() ); } return
	 * fieldID; }
	 */
	...
	// private static native long getFieldIDNative(Class c, String fieldName,
	// String fieldSig);
}
