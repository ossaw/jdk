 ...
 * interface, but does not implement the method to retrieve the next binding in
 * the NamingContext for which it was created. This is left to a subclass, which
 * is why this class is abstract; BindingIteratorImpl provides an implementation
 * of the interface operations on top of two subclass methods, allowing multiple
 * implementations of iterators that differ in storage and access to the
 * contents of a NamingContext implementation.
 * <p>
 * The operation next_one() is implemented by the subclass, whereas next_n() is
 * implemented on top of the next_one() implementation. Destroy must also be
 * implemented by the subclass.
 * <p>
 * A subclass must implement NextOne() and Destroy(); these methods are invoked
 * from synchronized methods and need therefore not be synchronized themselves.
 */
public abstract class BindingIteratorImpl extends BindingIteratorPOA {
    ...
    /**
     * lists next n bindings. It returns true or false, indicating whether there
     * were more bindings. This method has the package private scope, It will be
    ...
    /**
     * Abstract method for returning the next binding in the NamingContext for
