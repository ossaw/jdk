...
/**
 * Class TransientNamingContext implements the methods defined by
 * NamingContextDataStore, and extends the NamingContextImpl class to provide a
 * servant implementation of CosNaming::NamingContext. The
 * TransientNamingContext uses a hash table to store the mappings between
 * bindings and object references and the hash table is not persistent; thereby
 * the name "transient". This class should not be used directly; instead, the
 * class TransientNameService should be instantiated.
 * <p>
 * The keys in the hash table are InternalBindingKey objects, containing a
 * single NameComponent and implementing the proper functions, i.e., equals()
 * and hashCode() in an efficient manner. The values in the hash table are
 * InternalBindingValues and store a org.omg.CosNaming::Binding and the object
 * reference associated with the binding. For iteration,
 * TransientBindingIterator objects are created, which are passed a cloned copy
 * of the hashtable. Since elements are inserted and deleted and never modified,
 * this provides stable iterators at the cost of cloning the hash table.
 * <p>
 * To create and destroy object references, the TransientNamingContext uses the
 * orb.connect() and orb.disconnect() methods.
 *
...
public class TransientNamingContext extends NamingContextImpl implements NamingContextDataStore {
    ...
    // These can be combined, and then we simply use 3 NamingSystemException
    // wrappers,
     ...
     *            initialize.
     */
    public TransientNamingContext(com.sun.corba.se.spi.orb.ORB orb, org.omg.CORBA.Object initial, POA nsPOA)
    ...
    /**
     * Binds the object to the name component as the specified binding type. It
     * creates a InternalBindingKey object and a InternalBindingValue object and
     * inserts them in the hash table.
     * 
     * @param n
     *        A single org.omg.CosNaming::NameComponent under which the
     *        object will be bound.
     * @param obj
     *        An object reference to be bound under the supplied name.
     * @param bt
     *        The type of the binding (i.e., as object or as context).
     * @exception org.omg.CORBA.SystemException
     *            One of a fixed set of CORBA system exceptions.
     */
    public final void Bind(NameComponent n, org.omg.CORBA.Object obj, BindingType bt)
        ...
        if (oldValue != null) {
            updateLogger.warning(LogKeywords.NAMING_BIND + "Name " + getName(n) + " Was Already Bound");
            throw wrapper.transNcBindAlreadyBound();
        }
        if (updateLogger.isLoggable(Level.FINE)) {
            updateLogger.fine(LogKeywords.NAMING_BIND_SUCCESS + "Name Component: " + n.id + "." + n.kind);
        }
    }

    /**
     * Resolves the supplied name to an object reference and returns the type of
     * the resolved binding. It creates a InternalBindingKey and uses the key
     * for looking up in the hash table. If nothing is found an exception is
     * thrown, otherwise the object reference is returned and the binding type
     * set.
     * 
     * @param n
     ...
     *            One of a fixed set of CORBA system exceptions.
     */
    public final org.omg.CORBA.Object Resolve(NameComponent n, BindingTypeHolder bth)
            throws org.omg.CORBA.SystemException {
        ...
        if (readLogger.isLoggable(Level.FINE)) {
            readLogger.fine(LogKeywords.NAMING_RESOLVE_SUCCESS + "Namecomponent :" + getName(n));
        }
     ...
     * InternalBindingKey and uses it to remove the value associated with the
     * key. If nothing is found an exception is thrown, otherwise the element is
     * removed from the hash table.
     * 
     * @param n
     *        a NameComponent which is the name to unbind
     * @return the object reference bound to the name, or null if not found.
     * @exception org.omg.CORBA.SystemException
     *            One of a fixed set of CORBA system exceptions.
     */
    public final org.omg.CORBA.Object Unbind(NameComponent n) throws org.omg.CORBA.SystemException {
        ...
        if (value == null) {
            if (updateLogger.isLoggable(Level.FINE)) {
                updateLogger.fine(LogKeywords.NAMING_UNBIND_FAILURE + " There was no binding with the name "
                        + getName(n) + " to Unbind ");
            }
            return null;
        } else {
            if (updateLogger.isLoggable(Level.FINE)) {
                updateLogger.fine(LogKeywords.NAMING_UNBIND_SUCCESS + " NameComponent:  " + getName(n));
            }
            return value.theObjectRef;
        }
    }
     ...
     * TransientBindingIterator object and passes it a clone of the hash table
    ...
    public final void List(int how_many, BindingListHolder bl, BindingIteratorHolder bi)
        ...
        try {
            ...
            TransientBindingIterator bindingIterator = new TransientBindingIterator(this.orb,
    ...
    public final org.omg.CosNaming.NamingContext NewContext() throws org.omg.CORBA.SystemException {
        try {
            ...
            TransientNamingContext transContext = new TransientNamingContext(
            ...
            lifecycleLogger.log(Level.WARNING, LogKeywords.LIFECYCLE_CREATE_FAILURE, e);
            throw e;
        } catch (Exception e) {
            lifecycleLogger.log(Level.WARNING, LogKeywords.LIFECYCLE_CREATE_FAILURE, e);
    ...
    public final void Destroy() throws org.omg.CORBA.SystemException {
        ...
        try {
            ...
            lifecycleLogger.log(Level.WARNING, LogKeywords.LIFECYCLE_DESTROY_FAILURE, e);
            throw e;
        } catch (Exception e) {
            lifecycleLogger.log(Level.WARNING, LogKeywords.LIFECYCLE_DESTROY_FAILURE, e);
    ...
    /**
     * Return whether this NamingContext contains any bindings. It forwards this
     * request to the hash table.
     * 
