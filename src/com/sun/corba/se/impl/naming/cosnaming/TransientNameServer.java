...
/**
 * Class TransientNameServer is a standalone application which implements a
 * transient name service. It uses the TransientNameService class for the name
 * service implementation, and the BootstrapServer for implementing
 * bootstrapping, i.e., to get the initial NamingContext.
 * <p>
 ...
 * only a "NameService" entry together with the stringified object reference for
 * the initial NamingContext. The BootstrapServer's listening port is set by
 * first checking the supplied arguments to the name server (-ORBInitialPort),
 * and if not set, defaults to the standard port number. The BootstrapServer is
 * created supplying the Properties object, using no external File object for
 * storage, and the derived initial port number.
 * 
...
public class TransientNameServer {
	...
	private static org.omg.CORBA.Object initializeRootNamingContext(ORB orb) {
		...
		try {
			com.sun.corba.se.spi.orb.ORB coreORB = (com.sun.corba.se.spi.orb.ORB) orb;
	...
	/**
	 * Main startup routine. It instantiates a TransientNameService object and a
	...
	public static void main(String args[]) {
		...
		try {
			...
			((com.sun.corba.se.org.omg.CORBA.ORB) corb).register_initial_reference("NamingService",
					ns);
