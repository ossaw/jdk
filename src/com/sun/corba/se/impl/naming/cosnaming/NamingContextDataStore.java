 ...
 * "data store" associated with a NamingContext implementation. It allows for
 * different implementations of naming contexts that support the same API but
 * differ in storage mechanism.
 */
public interface NamingContextDataStore {
	/**
	 * Method which implements binding a name to an object as the specified
	 * binding type.
	 * 
	 * @param n
	 ...
	 * of the binding and the bound object reference. If the id and kind of the
	 * NameComponent are both empty, the initial naming context (i.e., the local
