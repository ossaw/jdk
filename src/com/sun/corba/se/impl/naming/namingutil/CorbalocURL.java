...
public class CorbalocURL extends INSURLBase {
	...
	/**
	 * A Utility method to throw BAD_PARAM exception to signal malformed INS
	 * URL.
	 */
	...
	/**
	 * If there is 'iiop:' token in the URL, this method will parses and
	 * validates that host and port information.
	 */
	...
	private IIOPEndpointInfo handleColon(String iiopInfo) {
		...
		if (tokenCount == 2) {
			...
			try {
				iiopEndpointInfo.setVersion(Integer.parseInt(version.substring(0, dot)),
	...
	/**
	 * Returns an IPV6 Port that is after [<ipv6>]:. There is no validation done
	 * here, if it is an incorrect port then the request through this URL
	 * results in a COMM_FAILURE, otherwise malformed list will result in
	 * BAD_PARAM exception thrown in checkcorbalocGrammer.
	 */
	 ...
	 * this URL results in a COMM_FAILURE, otherwise malformed list will result
	 * in BAD_PARAM exception thrown in checkcorbalocGrammer.
	 */
