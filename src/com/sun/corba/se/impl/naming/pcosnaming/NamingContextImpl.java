 ...
 * <p>
 * The operations bind(), rebind(), bind_context() and rebind_context() are all
 * really implemented by doBind(). resolve() is really implemented by
 * doResolve(), unbind() by doUnbind(). list(), new_context() and destroy() uses
 * the NamingContextDataStore interface directly. All the doX() methods are
 * public static. They synchronize on the NamingContextDataStore object.
 * <p>
...
public class NamingContextImpl extends NamingContextExtPOA
	...
	// All the INS (Interoperable Naming Service) methods are defined in this
	// class
	 ...
	 * @param objKey
	 *            as String
	 * @param TheNameService
	 *            as NameService
	 * @param TheServantManagerImpl
	 *            as ServantManagerImpl
	 * @exception java.lang.Exception
	 *                a Java exception.
	 */

	public NamingContextImpl(ORB orb, String objKey, NameService theNameService,
			ServantManagerImpl theServantManagerImpl) throws Exception {
		super();

		this.orb = orb;
		readWrapper = NamingSystemException.get(orb, CORBALogDomains.NAMING_READ);
		updateWrapper = NamingSystemException.get(orb, CORBALogDomains.NAMING_UPDATE);

		debug = true; // orb.namingDebugFlag ;
		this.objKey = objKey;
		theNameServiceHandle = theNameService;
		theServantManagerImplHandle = theServantManagerImpl;
		insImpl = new com.sun.corba.se.impl.naming.cosnaming.InterOperableNamingImpl();
	}
	...
	/**
	 * Bind an object under a name in this NamingContext. If the name contains
	 * multiple (n) components, n-1 will be resolved in this NamingContext and
	 * the object bound in resulting NamingContext. An exception is thrown if a
	 * binding with the supplied name already exists. If the object to be bound
	 * is a NamingContext it will not participate in a recursive resolve.
	 * 
	 * @param n
	 *            a sequence of NameComponents which is the name under which the
	 *            object will be bound.
	 * @param obj
	 *            the object reference to be bound.
	 * @exception org.omg.CosNaming.NamingContextPackage.NotFound
	 *                A name with multiple components was supplied, but the
	 *                first component could not be resolved.
	 * @exception org.omg.CosNaming.NamingContextPackage.CannotProceed
	 *                Could not proceed in resolving the n-1 components of the
	 *                supplied name.
	 * @exception org.omg.CosNaming.NamingContextPackage.InvalidName
	 *                The supplied name is invalid (i.e., has length less than
	 *                1).
	 * @exception org.omg.CosNaming.NamingContextPackage.AlreadyBound
	 *                The supplied name is already bound.
	 ...
	 * NamingContext and the object bound in resulting NamingContext. An
	 * exception is thrown if a binding with the supplied name already exists.
	 ...
	 *            object will be bound.
	 * @param obj
	 *            the NamingContect object reference to be bound.
	 * @exception org.omg.CosNaming.NamingContextPackage.NotFound
	 *                A name with multiple components was supplied, but the
	 *                first component could not be resolved.
	 * @exception org.omg.CosNaming.NamingContextPackage.CannotProceed
	 ...
	 *                1).
	 * @exception org.omg.CosNaming.NamingContextPackage.AlreadyBound
	 *                An object is already bound under the supplied name.
	 * @exception org.omg.CORBA.SystemException
	 *                One of a fixed set of CORBA system exceptions.
	 * @see doBind
	 */
	 ...
	 * Bind an object under a name in this NamingContext. If the name contains
	 * multiple (n) components, n-1 will be resolved in this NamingContext and
	 * the object bound in resulting NamingContext. If a binding under the
	 * supplied name already exists it will be unbound first. If the object to
	 * be bound is a NamingContext it will not participate in a recursive
	 * resolve.
	 * 
	 * @param n
	 *            a sequence of NameComponents which is the name under which the
	 ...
	 * bound to the name. If the name contains multiple (n) components, the
	 * first component will be resolved in this NamingContext and the remaining
	 * components resolved in the resulting NamingContext, provided that the
	 * NamingContext bound to the first component of the name was bound with
	 * bind_context().
	 * 
	 * @param n
	 *            a sequence of NameComponents which is the name to be resolved.
	...
	/**
	 * Remove a binding from this NamingContext. If the name contains multiple
	 * (n) components, the first n-1 components will be resolved from this
	 * NamingContext and the final component unbound in the resulting
	 * NamingContext.
	 * 
	 * @param n
	 *            a sequence of NameComponents which is the name to be unbound.
	 * @exception org.omg.CosNaming.NamingContextPackage.NotFound
	 *                A name with multiple components was supplied, but the
	 *                first component could not be resolved.
	 * @exception org.omg.CosNaming.NamingContextPackage.CannotProceed
	 ...
	 * @see doUnbind
	 */
	public void unbind(NameComponent[] n) throws org.omg.CosNaming.NamingContextPackage.NotFound,
			org.omg.CosNaming.NamingContextPackage.CannotProceed,
			org.omg.CosNaming.NamingContextPackage.InvalidName {
		if (debug)
	...
	/**
	 * List the contents of this NamingContest. A sequence of bindings is
	 * returned (a BindingList) containing up to the number of requested
	...
	public void list(int how_many, BindingListHolder bl, BindingIteratorHolder bi) {
		...
		if (debug && bl.value != null)
			dprint("list(" + how_many + ") -> bindings[" + bl.value.length + "] + iterator: "
					+ bi.value);
	}
	...
	/**
	 * Create a new NamingContext, bind it in this Naming Context and return its
	 * object reference. This is equivalent to using new_context() followed by
	 * bind_context() with the supplied name and the object reference for the
	 * newly created NamingContext.
	 * 
	 * @param n
	 *            a sequence of NameComponents which is the name to be unbound.
	 * @return an object reference for a new NamingContext object implemented by
	 *         this Name Server, bound to the supplied name.
	 * @exception org.omg.CosNaming.NamingContextPackage.AlreadyBound
	 *                An object is already bound under the supplied name.
	 * @exception org.omg.CosNaming.NamingContextPackage.NotFound
	 *                A name with multiple components was supplied, but the
	...
	/**
	 * Destroy this NamingContext object. If this NamingContext contains no
	 * bindings, the NamingContext is deleted.
	 * 
	 * @exception org.omg.CosNaming.NamingContextPackage.NotEmpty
	 *                This NamingContext is not empty (i.e., contains bindings).
	 * @exception org.omg.CORBA.SystemException
	 *                One of a fixed set of CORBA system exceptions.
	 */
	...
	/**
	 * Implements all four flavors of binding. It uses Resolve() to check if a
	 * binding already exists (for bind and bind_context), and unbind() to
	 * ensure that a binding does not already exist. If the length of the name
	 * is 1, then Bind() is called with the name and the object to bind.
	 * Otherwise, the first component of the name is resolved in this
	 * NamingContext and the appropriate form of bind passed to the resulting
	 * NamingContext. This method is static for maximal reuse - even for
	 * extended naming context implementations where the recursive semantics
	 * still apply.
	 * 
	 * @param impl
	 *            an implementation of NamingContextDataStore
	 * @param n
	 *            a sequence of NameComponents which is the name under which the
	 *            object will be bound.
	 * @param obj
	 *            the object reference to be bound.
	 * @param rebind
	 *            Replace an existing binding or not.
	 * @param bt
	 ...
	 *                first component could not be resolved.
	 * @exception org.omg.CosNaming.NamingContextPackage.CannotProceed
	 *                Could not proceed in resolving the first component of the
	 *                supplied name.
	 * @exception org.omg.CosNaming.NamingContextPackage.InvalidName
	 *                The supplied name is invalid (i.e., has length less than
	 *                1).
	 ...
	 * @see rebind_context
	 */
	private void doBind(NamingContextDataStore impl, NameComponent[] n, org.omg.CORBA.Object obj,
			boolean rebind, org.omg.CosNaming.BindingType bt)
			throws org.omg.CosNaming.NamingContextPackage.NotFound,
			org.omg.CosNaming.NamingContextPackage.CannotProceed,
			org.omg.CosNaming.NamingContextPackage.InvalidName,
			org.omg.CosNaming.NamingContextPackage.AlreadyBound {
	...
	/**
	 * Implements resolving names in this NamingContext. The first component of
	 * the supplied name is resolved in this NamingContext by calling Resolve().
	 * If there are no more components in the name, the resulting object
	 * reference is returned. Otherwise, the resulting object reference must
	 * have been bound as a context and be narrowable to a NamingContext. If
	 * this is the case, the remaining components of the name is resolved in the
	 * resulting NamingContext. This method is static for maximal reuse - even
	 * for extended naming context implementations where the recursive semantics
	 * still apply.
	 * 
	 * @param impl
	 *            an implementation of NamingContextDataStore
	...
	/**
	 * Implements unbinding bound names in this NamingContext. If the name
	 * contains only one component, the name is unbound in this NamingContext
	 * using Unbind(). Otherwise, the first component of the name is resolved in
	 * this NamingContext and unbind passed to the resulting NamingContext. This
	 * method is static for maximal reuse - even for extended naming context
	 ...
	 * CosNaming::NamingContext. It will throw appropriate exceptions if not
	 * found or not narrowable.
	 * 
	 * @param impl
	 *            an implementation of NamingContextDataStore
	 * @param n
	 *            a NameComponents which is the name to be found.
	 * @exception org.omg.CosNaming.NamingContextPackage.NotFound
	 *                The first component could not be resolved.
	 * @exception org.omg.CosNaming.NamingContextPackage.CannotProceed
	 *                Could not proceed in resolving the first component of the
	 *                supplied name.
	 * @exception org.omg.CORBA.SystemException
	...
	protected static NamingContext resolveFirstAsContext(NamingContextDataStore impl,
			NameComponent[] n) throws org.omg.CosNaming.NamingContextPackage.NotFound {
		...
		if (bth.value != BindingType.ncontext) {
			...
			throw new org.omg.CosNaming.NamingContextPackage.NotFound(NotFoundReason.not_context,
					n);
		}
	...
	private static void dprint(String msg) {
		NamingUtils.dprint("NamingContextImpl(" + Thread.currentThread().getName() + " at "
				+ System.currentTimeMillis() + " ems): " + msg);
	}

	/**
	 * Implements all flavors of binding( bind and bindcontext) This method will
	 * be called from the superclass's doBind( ) method which takes care of all
	 * the conditions before calling this method. i.e., It checks whether the
	 * Name is already Bounded, Then in the case of rebind it calls Unbind
	 * first. This method does one level binding only, To have n-level binding
	 ...
	 *            Type of binding (as object or as context).
	 * @exception org.omg.CosNaming.NamingContextPackage.NotFound
	 *                raised if the NameComoponent list is invalid
	 * @exception org.omg.CosNaming.NamingContextPackage.CannotProceed
	 *                Could not proceed in resolving the Name from the given
	 *                NameComponent
	 * @exception org.omg.CosNaming.NamingContextPackage.AlreadyBound
	 *                An object is already bound under the supplied name.
	 * @exception org.omg.CORBA.SystemException
	...
	public void Bind(NameComponent n, org.omg.CORBA.Object obj, BindingType bt) {
		...
		try {
			if (bt.value() == BindingType._nobject) {
				// If the BindingType is an ObjectRef then Stringify this ref
				// and
				...
				// If the BindingType is a NamingContext then get it's object
				// key
				// from the NameService and store it in the Internal Binding
				// Value instance
			...
			InternalBindingValue oldValue = (InternalBindingValue) this.theHashtable.put(key,
					value);

			if (oldValue != null) {
				// There was an entry with this name in the Hashtable and hence
				// throw CTX_ALREADY_BOUND
				// exception
				throw updateWrapper.namingCtxRebindAlreadyBound();
			} else {
				try {
					// Everything went smooth so update the NamingContext file
					// with the
	 ...
	 * @return the object reference bound under the supplied name.
	 * @exception org.omg.CosNaming.NamingContextPackage.NotFound
	 *                Neither a NamingContext or a Corba Object reference not
	 *                found under this Name
	 * @exception org.omg.CosNaming.NamingContextPackage.CannotProceed
	 *                Could not proceed in resolving the the supplied name.
	 * @exception org.omg.CosNaming.NamingContextPackage.InvalidName
	 *                The supplied name is invalid (i.e., has length less than
	 *                1).
	 * @exception org.omg.CORBA.SystemException
	...
	public Object Resolve(NameComponent n, BindingTypeHolder bth) throws SystemException {
		if ((n.id.length() == 0) && (n.kind.length() == 0)) {
			...
			return theNameServiceHandle.getObjectReferenceFromKey(this.objKey);
		}
		...
		try {
			...
			// Which means it's a name context. So get the NamingContext
			// reference
			...
			if (value.strObjectRef.startsWith("NC")) {
				...
				// Else, It is a Object Reference. Check whether Object
				// Reference
				...
				if (theObjectFromStringifiedReference == null) {
					try {
						theObjectFromStringifiedReference = orb
								.string_to_object(value.strObjectRef);
						value.setObjectRef(theObjectFromStringifiedReference);
					} catch (Exception e) {
						throw readWrapper.resolveConversionFailure(CompletionStatus.COMPLETED_MAYBE,
								e);
					}
				}
			}
		} catch (Exception e) {
			throw readWrapper.resolveFailure(CompletionStatus.COMPLETED_MAYBE, e);
		}
	...
	public Object Unbind(NameComponent n) throws SystemException {
		try {
			...
			if (value.strObjectRef.startsWith("NC")) {
				...
				Object theObjectFromStringfiedReference = theNameServiceHandle
						.getObjectReferenceFromKey(value.strObjectRef);
				return theObjectFromStringfiedReference;
			} else {
				Object theObjectFromStringifiedReference = value.getObjectRef();

				if (theObjectFromStringifiedReference == null) {
					theObjectFromStringifiedReference = orb.string_to_object(value.strObjectRef);
				}
	 ...
	 * PersistentBindingIterator object and passes it a clone of the hash table
	 * and an orb object. It then uses the newly created object to return the
	...
	public void List(int how_many, BindingListHolder bl, BindingIteratorHolder bi)
		...
		try {
			PersistentBindingIterator bindingIterator = new PersistentBindingIterator(this.orb,
			...
			org.omg.CosNaming.BindingIterator bindingRef = org.omg.CosNaming.BindingIteratorHelper
					.narrow(obj);
	...
	public void Destroy() throws SystemException {
		...
		/*
		 * // XXX This needs to be replaced by cleaning up the // file that
		 * backs up the naming context. No explicit // action is necessary at
		 * the POA level, since this is // created with the non-retain policy.
		 * /* try { orb.disconnect(
		 * theNameServiceHandle.getObjectReferenceFromKey( this.objKey ) ); }
		 * catch( org.omg.CORBA.SystemException e ) { throw e; } catch(
		 * Exception e ) { throw updateWrapper.transNcDestroyGotEx( e ) ; }
		 */
	}
	...
	/**
	 * This operation converts a Stringified Name into an equivalent array of
	 * Name Components.
	 * 
	 * @param sn
	...
	public org.omg.CosNaming.NameComponent[] to_name(String sn)
		...
		org.omg.CosNaming.NameComponent[] theNameComponents = getINSImpl()
				.convertToNameComponent(sn);
	 ...
	 * 
	 * @param addr
	 *            internet based address of the host machine where Name Service
	 *            is running
	 *            <p>
	 * @param sn
	 *            Stringified Name of the object
	 ...
	 *                Indicates the internet based address of the host machine
	 *                is incorrect
	 *                <p>
	 *
	 */
	...
	public void printSize() {
		...
		for (; e.hasMoreElements();) {
			InternalBindingValue thevalue = (InternalBindingValue) this.theHashtable
					.get(e.nextElement());
			if (thevalue != null) {
