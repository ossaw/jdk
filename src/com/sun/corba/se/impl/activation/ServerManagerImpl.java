...
public class ServerManagerImpl extends _ServerManagerImplBase implements BadServerIdHandler {
	...
	public void activate(int serverId)
		...
		try {
			...
			if (debug)
				System.out.println("ServerManagerImpl: locateServer called with " + " serverId="
						+ serverId + " endpointType=" + IIOP_CLEAR_TEXT.value + " block=false");
	...
	public void registerEndpoints(int serverId, String orbId, EndPointInfo[] endpointList)
		...
		synchronized (serverTable) {
			...
			if (entry == null) {
				if (debug)
					System.out.println("ServerManagerImpl: registerEndpoint for server Id "
							+ serverId + " called, but no such server is registered.");

				throw wrapper.serverNotExpectedToRegister();
			} else {
				if (debug)
					System.out.println("ServerManagerImpl: registerEndpoints for server Id "
		...
		synchronized (serverTable) {
			...
			if (debug)
				if (entry == null) {
					...
					System.out.println("ServerManagerImpl: getEntry: " + " active server found "
							+ entry + ".");
				}
			...
			if (entry == null) {
				...
				entry = new ServerTableEntry(wrapper, serverId, serverDef, initialPort, dbDirName,
						false, debug);
	...
	private ServerLocation locateServer(ServerTableEntry entry, String endpointType, boolean block)
		...
		if (block) {
			...
			String host = orb.getLegacyServerSocketManager()
			...
			for (int i = 0; i < listLength; i++) {
				...
				if (debug)
					System.out.println(
							"ServerManagerImpl: locateServer: " + "server located at location "
									+ location.hostname + " ORBid  " + serverORBAndPortList[i].orbId
	...
	private ServerLocationPerORB locateServerForORB(ServerTableEntry entry, String orbId,
			boolean block) throws InvalidORBid, ServerNotRegistered, ServerHeldDown {
		...
		if (block) {
			try {
				...
				if (debug)
					System.out.println(
							"ServerManagerImpl: locateServerForORB: " + "server held down");
			...
			for (int i = 0; i < listLength; i++) {
				...
				if (debug)
					System.out.println("ServerManagerImpl: locateServer: "
							+ "server located at location " + location.hostname + " endpointType  "
	...
	public ServerLocation locateServer(int serverId, String endpointType)
		...
		if (debug)
			System.out.println("ServerManagerImpl: locateServer called with " + " serverId="
					+ serverId + " endpointType=" + endpointType + " block=true");
	...
	public ServerLocationPerORB locateServerForORB(int serverId, String orbId)
		...
		if (debug)
			System.out.println("ServerManagerImpl: locateServerForORB called with " + " serverId="
					+ serverId + " orbId=" + orbId + " block=true");
	...
	public void handle(ObjectKey okey) {
		...
		try {
			...
			if (debug)
				System.out.println("ServerManagerImpl: handle called for server id" + serverId
						+ "  orbid  " + orbId);
			...
			IIOPProfileTemplate iptemp = IIOPFactories.makeIIOPProfileTemplate(orb,
					GIOPVersion.V1_2, addr);
