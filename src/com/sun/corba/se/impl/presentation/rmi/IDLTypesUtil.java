...
/**
 * Utility class for testing RMI/IDL Types as defined in Section 1.2 of The Java
 * Language to IDL Mapping. Note that these are static checks only. Runtime
 * checks, such as those described in Section 1.2.3, #3, are not covered.
 */
public final class IDLTypesUtil {
	...
	/*
	 * rmic -iiop does not correctly implement the clause in 1.3.4.3 about
	 * is<NAME>/get<NAME> conflicts. The spec says that is<NAME> is the property
	 * and get<NAME> is left alone, but rmic does the opposite. We will follow
	 * rmic in this, but it's easy to change.
	 */
	public static final boolean FOLLOW_RMIC = true;

	/**
	 * Validate a class to ensure it conforms to the rules for a Java RMI/IIOP
	 * interface.
	 *
	...
	public boolean isArray(Class c) {
		...
		if (c.isArray()) {
			...
			arrayType = (isPrimitive(componentType) || isRemoteInterface(componentType)
	...
	private boolean hasCorrespondingReadProperty(Method writeProperty, Class c,
		...
		try {
			...
			foundReadProperty = (isPropertyAccessorMethod(readPropertyMethod, c)
					&& (readPropertyMethod.getReturnType() == writePropertyParameters[0]));
		} catch (Exception e) {
	...
	public String getAttributeNameForProperty(String propertyName) {
		...
		if ((prefix != null) && (prefix.length() < propertyName.length())) {
			...
			if ((remainder.length() >= 2) && Character.isUpperCase(remainder.charAt(0))
	...
	/**
	 * Return IDL Type name for primitive types as defined in Section 1.3.3 of
	 * Java2IDL spec or null if not a primitive type.
	 */
	...
	/**
	 * Return IDL Type name for special case type mappings as defined in Table
	 * 1-1 of Java2IDL spec or null if given class is not a special type.
	 */
	...
	private void validateExceptions(Method method) throws IDLTypeException {
		...
		// (isCheckedException(E) => (isValue(E) ||
		// RemoteException.isAssignableFrom( E ) )
		for (int eIndex = 0; eIndex < exceptions.length; eIndex++) {
			Class exception = exceptions[eIndex];
	 ...
	 * restrictions for properties as defined in Section 1.3.4.3 of Java2IDL
	 * spec. This means that for all exceptions E declared on the method, E
	 * isChecked => RemoteException.isAssignableFrom( E ).
	 */
			...
			for (Iterator iter = currentMethodNames.iterator(); iter.hasNext();) {
				...
				if (allMethodNames.contains(methodName)) {
					String msg = "Class " + c + " inherits method " + methodName
							+ " from multiple direct interfaces.";
					throw new IDLTypeException(msg);
				} else {
	...
	private void validateConstants(final Class c) throws IDLTypeException {
		...
		try {
			fields = (Field[]) java.security.AccessController
		...
		for (int i = 0; i < fields.length; i++) {
			...
			if ((fieldType != java.lang.String.class) && !isPrimitive(fieldType)) {
				String msg = "Constant field '" + next.getName() + "' in class '"
						+ next.getDeclaringClass().getName() + "' has invalid type' "
						+ next.getType() + "'. Constants"
