...
public class InvocationHandlerFactoryImpl implements InvocationHandlerFactory {
    ...
    private class CustomCompositeInvocationHandlerImpl extends CompositeInvocationHandlerImpl implements
            LinkedInvocationHandler, Serializable {
        ...
        /**
         * Return the stub, which will actually be written to the stream. It
         * will be custom marshalled, with the actual writing done in
         * StubIORImpl. There is a corresponding readResolve method on
         * DynamicStubImpl which will re-create the full invocation handler on
         * read, and return the invocation handler on the readResolve method.
         */
    ...
    InvocationHandler getInvocationHandler(DynamicStub stub) {
        ...
        // methods.
        final InvocationHandler stubMethodHandler = new StubInvocationHandlerImpl(pm, classData, stub);
        ...
        // If the method passed to invoke is not from DynamicStub or its
        // superclasses,
