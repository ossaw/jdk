...
/**
 * StateFactory that turns java.rmi.Remote objects to org.omg.CORBA.Object. This
 * version works either with standard RMI-IIOP or Dynamic RMI-IIOP. Based on the
 * original com.sun.jndi.cosnaming.RemoteToCorba and
...
public class JNDIStateFactoryImpl implements StateFactory {
	 ...
	 * object, or if Remote object uses JRMP, return null. If the RMI-IIOP
	...
	public Object getStateToBind(Object orig, Name name, Context ctx,
		...
		ORB orb = getORB(ctx);
		if (orb == null)
			// Wrong kind of context, so just give up and let another
			// StateFactory
		...
		try {
			...
			// Wrong sort of object: just return null to allow another
			// StateFactory
			// to handle this. This can happen easily because this StateFactory
			// is specified for the application, not the service context
			// provider.
			return null;
		}
