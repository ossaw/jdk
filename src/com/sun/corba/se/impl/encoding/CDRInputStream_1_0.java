...
public class CDRInputStream_1_0 extends CDRInputStreamBase implements RestorableInputStream {
	 ...
	 * implementation uses it. If stubFactory is null, the ior's IIOPProfile
	 * must support getServant.
	 */
	...
	/**
	 * Examines the valuetag to see how many (if any) repository IDs are present
	 * on the wire. If no repository ID information is on the wire but the
	 * expectedType or expectedTypeRepId is known, it will return one of those
	...
	private java.lang.Object readBoxedIDLEntity(Class clazz, String codebase) {
		...
		try {
			...
			cls = Utility.loadClassForClass(clazz.getName() + "Helper", codebase, clazzLoader,
					clazz, clazzLoader);
			...
			try {
				readMethod = (Method) AccessController
	...
	private java.lang.Object readIDLValue(int indirection, String repId, Class clazz,
			String codebase) {
		ValueFactory factory;

		// Always try to find a ValueFactory first, as required by the spec.
		// There are some complications here in the IDL 3.0 mapping (see
		// 1.13.8),
		...
		// in the case of class implementing either StreamableValue or
		// CustomValue,
		// but abstract valuetypes only implement ValueBase, and really require
		// the use of the repId to find a factory (including the
		// DefaultFactory).
		try {
		...
		if (isChunked) {
			...
			if (orb == null || ORBVersionFactory.getFOREIGN().equals(orb.getORBVersion())
				...
				if (anEndTag < chunkedValueNestingLevel)
					throw wrapper.unexpectedEnclosingValuetype(CompletionStatus.COMPLETED_MAYBE,
							new Integer(anEndTag), new Integer(chunkedValueNestingLevel));
	...
	private void start_block() {
		...
		if (blockLength > 0 && blockLength < maxBlockLength) {
			blockLength += get_offset(); // _REVISIT_ unsafe, should use a Java
											// long

			// inBlock = true;
		} else {

			// System.out.println("start_block snooped a " +
			// Integer.toHexString(blockLength));
	...
	private String read_repositoryIds() {
		...
		if (numRepIds == 0xffffffff) {
			...
			// the
			// indirection table in case there are later indirections back to
			// them
	...
	/**
	 * Attempts to find the class described by the given repository ID string
	 * and expected type. The first attempt is to find the class locally,
