...
/**
 * Defines the contract between the BufferManager and CDR stream on the writing
 * side. The CDR stream calls back to the BufferManagerWrite when it needs more
 * room in the output buffer to continue. The BufferManager can then grow the
...
public abstract class BufferManagerWrite {
	 ...
	 * this.connection.send(fragment) If streaming: backpatch fragment size
	 * field in bbwi.buf. Set no more fragments bit. this.connection.send(bbwi).
	 */
