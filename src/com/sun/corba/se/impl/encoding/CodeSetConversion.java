...
public class CodeSetConversion {
	...
	/**
	 * Implementation of CTBConverter which uses a nio.Charset.CharsetEncoder to
	 * do the real work. Handles translation of exceptions to the appropriate
	 * CORBA versions.
	 */
	private class JavaCTBConverter extends CTBConverter {
		...
		private OMGSystemException omgWrapper = OMGSystemException
				.get(CORBALogDomains.RPC_ENCODING);
		...
		private void convertCharArray() {
			try {
				...
				// Possible optimization of directly converting into the CDR
				// buffer.
	...
	/**
	 * Special UTF16 converter which can either always write a BOM or use a
	 * specified byte order without one.
	 */
	 ...
	 * for the real work. Handles translation of exceptions to the appropriate
	 * CORBA versions.
	 */
	private class JavaBTCConverter extends BTCConverter {
		...
		/**
		 * Utility method to find a CharsetDecoder in the cache or create a new
		 * one if necessary. Throws an INTERNAL if the code set is unknown.
		 */
	...
	private class UTF16BTCConverter extends JavaBTCConverter {
		...
		/**
		 * The current solution for dealing with UTF-16 in CORBA is that if our
		 * sun.io converter requires byte order markers, and then we see a CORBA
		 * wstring/wchar without them, we switch to the sun.io converter that
