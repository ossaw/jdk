...
/**
 * Creates read/write buffer managers to handle over/under flow in
 * CDR*putStream.
 */
...
public class BufferManagerFactory {
	...
	// The next two methods allow creation of BufferManagers based on GIOP
	// version.
	...
	public static BufferManagerRead newBufferManagerRead(GIOPVersion version, byte encodingVersion,
			ORB orb) {
	...
	public static BufferManagerRead newBufferManagerRead(int strategy, byte encodingVersion,
			ORB orb) {
	...
	public static BufferManagerWrite newBufferManagerWrite(int strategy, byte encodingVersion,
			ORB orb) {
		if (encodingVersion != Message.CDR_ENC_VERSION) {
			if (strategy != BufferManagerFactory.GROW) {
				ORBUtilSystemException wrapper = ORBUtilSystemException.get((ORB) orb,
	...
	public static BufferManagerWrite newBufferManagerWrite(GIOPVersion version,
		...
		return BufferManagerFactory.newBufferManagerWrite(
				orb.getORBData().getGIOPBuffMgrStrategy(version), encodingVersion, orb);
	}
