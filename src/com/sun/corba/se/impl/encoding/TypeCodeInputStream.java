...
public class TypeCodeInputStream extends EncapsInputStream implements TypeCodeReader {
	...
	public TypeCodeInputStream(org.omg.CORBA.ORB orb, ByteBuffer byteBuffer, int size,
	...
	public void addTypeCodeAtPosition(TypeCodeImpl tc, int position) {
		...
		// if (TypeCodeImpl.debug) System.out.println(this + " adding tc " + tc
	...
	public TypeCodeImpl getTypeCodeAtPosition(int position) {
		...
		// System.out.println("Getting tc " + (TypeCode)typeMap.get(new
		// Integer(position)) +
		// " at position " + position);
		// }
	...
	public int getTopLevelPosition() {
		if (enclosure != null && enclosure instanceof TypeCodeReader) {
			...
			// This is why the size of the enclosed stream needs to be
			// substracted.
			int topPos = ((TypeCodeReader) enclosure).getTopLevelPosition();
			// Substract getBufferLength from the parents pos because it read
			// this stream
			// from its own when creating it
			int pos = topPos - getBufferLength() + getPosition();
			// if (TypeCodeImpl.debug) {
			// System.out.println("TypeCodeInputStream.getTopLevelPosition using
			// getTopLevelPosition " + topPos +
		...
		// if (TypeCodeImpl.debug) {
		// System.out.println("TypeCodeInputStream.getTopLevelPosition returning
		// getPosition() = " +
	...
	public static TypeCodeInputStream readEncapsulation(InputStream is, org.omg.CORBA.ORB _orb) {
		...
		if (is instanceof CDRInputStream) {
			encap = EncapsInputStreamFactory.newTypeCodeInputStream((ORB) _orb, encapBuffer,
					encapBuffer.length, ((CDRInputStream) is).isLittleEndian(),
		...
		// if (TypeCodeImpl.debug) {
		// System.out.println("Created TypeCodeInputStream " + encap + " with
		// parent " + is);
		// encap.printBuffer();
		// }
		return encap;
