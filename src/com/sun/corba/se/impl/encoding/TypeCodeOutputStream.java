...
public final class TypeCodeOutputStream extends EncapsOutputStream {
	...
	/*
	 * public boolean isEncapsulatedIn(TypeCodeOutputStream outerEnclosure) { if
	 * (outerEnclosure == this) return true; if (enclosure == null) return
	 * false; if (enclosure instanceof TypeCodeOutputStream) return
	 * ((TypeCodeOutputStream)enclosure).isEncapsulatedIn(outerEnclosure); //
	 * Last chance! Recursion ends with first non TypeCodeOutputStream. return
	 * (enclosure == outerEnclosure); }
	 */
	...
	public int getTopLevelPosition() {
		if (enclosure != null && enclosure instanceof TypeCodeOutputStream) {
			...
			if (isEncapsulation)
				pos += 4;
			// if (TypeCodeImpl.debug) {
			// System.out.println("TypeCodeOutputStream.getTopLevelPosition
			// using getTopLevelPosition " +
	...
	public void addIDAtPosition(String id, int position) {
		...
		// if (TypeCodeImpl.debug) System.out.println(this + " adding id " + id
	...
	public int getPositionForID(String id) {
		...
		// if (TypeCodeImpl.debug) System.out.println("Getting position " +
		// ((Integer)typeMap.get(id)).intValue() +
		// " for id " + id);
	...
	public void writeRawBuffer(org.omg.CORBA.portable.OutputStream s,
		...
		// So we have to take the first four bytes given in firstLong and write
		// them
		...
		// System.out.println("Parent position before writing kind = " +
		// ((CDROutputStream)s).getIndex());
		// }
		// }
		s.write_long(firstLong);
		// if (TypeCodeImpl.debug) {
		// if (s instanceof CDROutputStream) {
		// System.out.println("Parent position after writing kind = " +
		// ((CDROutputStream)s).getIndex());
		// }
		// }
		...
		// if (TypeCodeImpl.debug) {
		// if (s instanceof CDROutputStream) {
		// System.out.println("Parent position after writing all " + getIndex()
		// + " bytes = " + ((CDROutputStream)s).getIndex());
		// }
		// }
	}
	...
	public TypeCodeOutputStream createEncapsulation(org.omg.CORBA.ORB _orb) {
		...
		// TypeCodeOutputStream " + encap + " with parent " + this);
		return encap;
	}
	...
	public int getRealIndex(int index) {
		...
		// if (TypeCodeImpl.debug)
		// System.out.println("TypeCodeOutputStream.getRealIndex using
		// getTopLevelPosition " +
	...
	/*
	 * protected void printBuffer() { super.printBuffer(); }
	 */
