...
public class CDROutputStream_1_0 extends CDROutputStreamBase {
	...
	public void end_block() {
		...
		// System.out.println(" post end_block: " + get_offset() + " " +
		// bbwi.position());
	}
	...
	private void writeEndTag(boolean chunked) {
		...
		if (chunked) {
			...
			// Remember the last index and position. These are only used when
			// chunking.
	...
	private void writeIDLEntity(IDLEntity object) {
		...
		try {
			...
			final Class helperClass = Utility.loadClassForClass(clazz.getName() + "Helper",
			...
			try {
				writeMethod = (Method) AccessController
	...
	public void write_fixed(String string, int signum) {
		...
		// First calculate the length of the string without optional sign and
		// dot
		int numDigits = 0;
		...
		for (int i = 0; i < stringLength; i++) {
			...
			// then the representation begins with the first (most significant)
			// digit.
	...
	public void writeIndirection(int tag, int posIndirectedTo) {
		...
		// write indirection
		// Use parent.getRealIndex() so that it can be overridden by
		// TypeCodeOutputStreams
		/*
		 * System.out.println("CDROutputStream_1_0 writing indirection pos " +
		 * posIndirectedTo + " - real index " +
		 * parent.getRealIndex(get_offset()) + " = " + (posIndirectedTo -
		 * parent.getRealIndex(get_offset())));
		 */
	...
	public void start_value(String rep_id) {
		...
		if (debug) {
			dprint("start_value w/ rep id " + rep_id + " called at pos " + get_offset()
