...
public class ORBImpl extends com.sun.corba.se.spi.orb.ORB {
	...
	// XXX Should we move invocation tracking to the first level server
	// dispatcher?
	...
	// All access to resolver, localResolver, and urlOperation must be protected
	// using
	...
	// to bug 6980681 and 6238477, which was caused by a deadlock while
	// resolving a
	// corbaname: URL that contained a reference to the same ORB as the
	// ORB making the call to string_to_object. This caused a deadlock between
	// the
	...
	/**
	 * Create a new ORB. Should be followed by the appropriate set_parameters()
	 * call.
	 */
	public ORBImpl() {
	...
	private void preInit(String[] params, Properties props) {
		...
		// Note: transientServerId will wrap in about 2^32 / 86400000 = 49.7
		// days.
		...
		// to see if the IOR port matches ORB server port in
	...
	/**
	 * Get a Current pseudo-object. The Current interface is used to manage
	 * thread-specific information for use by the transactions, security and
	 * other services. This method is deprecated, and replaced by
	 * ORB.resolve_initial_references("NameOfCurrentObject");
	 *
	...
	public synchronized org.omg.CORBA.Current get_current() {
		...
		/*
		 * _REVISIT_ The implementation of get_current is not clear. How would
		 * ORB know whether the caller wants a Current for transactions or
		 * security ?? Or is it assumed that there is just one implementation
		 * for both ? If Current is thread-specific, then it should not be
		 * instantiated; so where does the ORB get a Current ?
		 * 
	...
	public synchronized String object_to_string(org.omg.CORBA.Object obj) {
		...
		try {
			...
			// Throw MARSHAL instead if this is a LOCAL_OBJECT_NOT_ALLOWED
			// error.
	 ...
	 *            the bound for the sequence.
	 * @param offset
	 *            the index to the enclosing TypeCode that is being referenced.
	 * @return the requested TypeCode.
	 */
	...
	public synchronized org.omg.CORBA.TypeCode create_abstract_interface_tc(String id,
			String name) {
		checkShutdownState();
	...
	public synchronized org.omg.CORBA.TypeCode create_value_tc(String id, String name,
			short type_modifier, TypeCode concrete_base, ValueMember[] members) {
		checkShutdownState();
		return new TypeCodeImpl(this, TCKind._tk_value, id, name, type_modifier, concrete_base,
				members);
	}
	...
	/**
	 * Get a list of the initially available CORBA services. This does not work
	 * unless an ORBInitialHost is specified during initialization (or unless
	 * there is an ORB running on the AppletHost) since the localhostname is
	 * inaccessible to applets. If a service properties URL was specified, then
	 * it is used, otherwise the bootstrapping protocol is used.
	 * 
	...
	/**
	 * Resolve the stringified reference of one of the initially available CORBA
	 * services.
	 * 
	 * @param identifier
	 *            The stringified object reference of the desired service.
	 * @return An object reference for the desired service.
	 * @exception InvalidName
	 *                The supplied identifier is not associated with a known
	 *                service.
	...
	/**
	 * If this operation is called with an id, <code>"Y"</code>, and an object,
	 * <code>YY</code>, then a subsequent call to
	 * <code>ORB.resolve_initial_references( "Y" )</code> will return object
	 * <code>YY</code>.
	 *
	 * @param id
	...
	/****************************************************************************
	 * The following methods (introduced in POA / CORBA2.1) deal with shutdown /
	 * single threading.
	 ****************************************************************************/
	...
	// Cause all ObjectAdapaterFactories to clean up all of their internal
	// state, which
	// may include activated objects that have associated state and callbacks
	// that must
	// complete in order to shutdown. This will cause new request to be
	// rejected.
	...
	/**
	 * formal/99-10-07 p 159: "If destroy is called on an ORB that has not been
	 * shut down, it will start the shutdown process and block until the ORB has
	 * shut down before it destroys the ORB."
	 */
	public void destroy() {
	...
	/**
	 * Finds and returns a value factory for the given repository ID. The value
	 * factory returned was previously registered by a call to
	...
	/**
	 * The bad server id handler is used by the Locator to send back the
	 * location of a persistant server to the client.
	 */
	...
	/**
	 * This is the implementation of the public API used to connect a
	 * servant-skeleton to the ORB.
	 */
	...
	// LocalClientRequestDispatcher as well. Bottom line: this code needs to
	// move.
	 ...
	 * Connection.java, then it is a security hole, since any unauthorized
	 * client has access to the host information. With this change it is used
	 * internally so the security problem is resolved. Also in Connection.java,
	 * the getLocalHost() implementation has changed to always call the
	 * InetAddress.getLocalHost().getHostAddress() The above mentioned method
	 * has been removed from the connection class
	 */
	...
	/**
	 * This method always returns false because the ORB never needs the main
	 * thread to do work.
	 */
	...
	/**
	 * Set the resolver used in this ORB. This resolver will be used for
	 * list_initial_services and resolve_initial_references.
	 */
	...
	/**
	 * Get the resolver used in this ORB. This resolver will be used for
	 * list_initial_services and resolve_initial_references.
	 */
	...
	/**
	 * Set the operation used in string_to_object calls. The Operation must
	 * expect a String and return an org.omg.CORBA.Object.
	 */
	...
	/**
	 * Get the operation used in string_to_object calls. The Operation must
	 * expect a String and return an org.omg.CORBA.Object.
	 */
