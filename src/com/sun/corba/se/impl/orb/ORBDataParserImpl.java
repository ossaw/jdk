...
public class ORBDataParserImpl extends ParserImplTableBase implements ORBData {
    ...
    /**
     * @return the GIOP Target Addressing preference of the ORB. This ORB by
     *         default supports all addressing dispositions unless specified
     *         otherwise via a java system property
    ...
    /**
     * Set the persistent-server-id of this server. This id is the same across
     * multiple activations of this server. The id can be set to any integer
     * value other than 0. This id must be set before any persistent objects can
     * be created.
     */
     ...
     * com.sun.corba.se.impl.iiop.ORB.getTransientServerId() which returns a
     * transient id that is guaranteed to be different across multiple
     * activations of this server. The user/environment is required to supply
     * the persistent-server-id every time this server is started, in the
     * ORBServerId parameter, System properties, or other means. The user is
     * also required to ensure that no two persistent servers on the same host
