 ...
 * The toStub method takes a server implementation and returns a stub that can
 * be used to access that server object. The connect method makes a Remote
 * object ready for remote communication. The unexportObject method is used to
 * deregister a server object, allowing it to become available for garbage
 * collection. The narrow method takes an object reference or abstract interface
 * type and attempts to narrow it to conform to the given interface. If the
 * operation is successful the result will be an object of the specified type,
 * otherwise an exception will be thrown.
 */
public class PortableRemoteObject implements javax.rmi.CORBA.PortableRemoteObjectDelegate {
	...
	public java.lang.Object narrow(java.lang.Object narrowFrom, java.lang.Class narrowTo)
		...
		try {
			...
			if (narrowTo.isInterface() && narrowTo != java.io.Serializable.class
				...
				if (narrowObj._is_a(id)) {
					...
					throw new ClassCastException(
							"Object is not of remote type " + narrowTo.getName());
				}
			} else {
				throw new ClassCastException(
						"Class " + narrowTo.getName() + " is not a valid remote interface");
			}
	...
	public void connect(Remote target, Remote source) throws RemoteException {
		...
		try {
			if (StubAdapter.isStub(source)) {
				...
				if (tie == null) {
					/*
					 * loadTie always succeeds for dynamic RMI-IIOP // No, can
					 * we get a tie for it? If not, // assume that source is a
					 * JRMP object... if (Utility.loadTie(source) != null) { //
					 * Yes, so it is an iiop object which // has not been
					 * exported... throw new RemoteException(
		...
		if (StubAdapter.isStub(target)) {
			...
			if (targetTie != null) {
				...
				/*
				 * loadTie always succeeds for dynamic RMI-IIOP if
				 * (Utility.loadTie(target) != null) { throw new
				 * RemoteException("'target' servant not exported"); }
				 */
			}
		}
