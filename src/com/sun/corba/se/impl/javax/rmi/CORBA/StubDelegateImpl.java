...
public class StubDelegateImpl implements javax.rmi.CORBA.StubDelegate {
	...
	/**
	 * Compares two stubs for equality. Returns <code>true</code> when used to
	 * compare stubs that represent the same remote object, and
	...
	/**
	 * Returns a string representation of this stub. Returns the same string for
	 * all stubs that represent the same remote object.
	 * 
	...
	/**
	 * Connects this stub to an ORB. Required after the stub is deserialized but
	 * not after it is demarshalled by an ORB stream. If an unconnected stub is
	 * passed to an ORB stream for marshalling, it is implicitly connected to
	 * that ORB. Application code should not call this method directly, but
	 * should call the portable wrapper method
	 ...
	 * 
	 * @serialData The length of the IOR type ID (int), followed by the IOR type
	 *             ID (byte array encoded using ISO8859-1), followed by the
	 *             number of IOR profiles (int), followed by the IOR profiles.
	 *             Each IOR profile is written as a profile tag (int), followed
	 *             by the length of the profile data (int), followed by the
	 *             profile data (byte array).
	 */
	public void writeObject(javax.rmi.CORBA.Stub self, java.io.ObjectOutputStream stream)
			throws IOException {
		init(self);
		ior.doWrite(stream);
	}
