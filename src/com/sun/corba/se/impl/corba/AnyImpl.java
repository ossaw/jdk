...
public class AnyImpl extends Any {
        ...
        public org.omg.CORBA.portable.InputStream create_input_stream() {
            ...
            AnyInputStream aIS = AccessController.doPrivileged(new PrivilegedAction<AnyInputStream>() {
                ...
                public AnyInputStream run() {
                    return new AnyInputStream((com.sun.corba.se.impl.encoding.EncapsInputStream) is);
                }
            });
            return aIS;
        }
    ...
    /*
     * This boolean array tells us if a given typecode must be streamed. Objects
     * that are immutable don't have to be streamed.
     */
    static boolean isStreamed[] = { false, // null
            false, // void
            false, // short
            false, // long
            false, // ushort
    ...
    /**
     * A constructor that sets the Any to contain a null. It also marks the
     * value as being invalid so that extractions throw an exception until an
     * insertion has been performed.
     */
    ...
    public void type(TypeCode tc) {
        ...
        value = 0;
        object = null;
        // null is the only legal value this Any can have after resetting the
        // type code
    ...
    // Needed for equal() in order to achieve linear performance for complex
    // types.
    // Uses up (recursively) copies of the InputStream in both Anys that got
    // created in equal().
    ...
    public void insert_long(int l) {
        ...
        // A long value is applicable to enums as well, so don't erase the enum
        // type code
    ...
    /**
     * A variant of the insertion operation that takes a typecode argument as
     * well.
     */
    ...
    /**
     * Note that the Serializable really should be an IDLEntity of some kind. It
     * shouldn't just be an RMI-IIOP type. Currently, we accept and will produce
     * RMI repIds with the latest calculations if given a non-IDLEntity
     * Serializable.
     */
    ...
    public void insert_fixed(java.math.BigDecimal value) {
        typeCode = TypeCodeImpl.convertToNative(orb, orb.create_fixed_tc(TypeCodeImpl.digits(value),
                TypeCodeImpl.scale(value)));
        object = value;
        isInitialized = true;
    }
    ...
    public void insert_fixed(java.math.BigDecimal value, org.omg.CORBA.TypeCode type) {
        try {
            if (TypeCodeImpl.digits(value) > type.fixed_digits() || TypeCodeImpl.scale(value) > type
                    .fixed_scale()) {
     ...
     * The ORB passed in should have the desired ORBVersion. It is used to
     * generate the type codes.
     */
    public TypeCode createTypeCodeForClass(java.lang.Class c, ORB tcORB) {
        ...
        classTC = (TypeCodeImpl) ValueUtility.createTypeCodeForClass(tcORB, c, ORBUtility
                .createValueHandler());
    ...
    /**
     * It looks like this was copied from io.ValueUtility at some point.
     * It's used by createTypeCodeForClass. The tcORB passed in should have the
     * desired ORB version, and is used to create the type codes.
     */
    private TypeCode getPrimitiveTypeCodeForClass(Class c, ORB tcORB) {
        ...
        if (c == Integer.TYPE) {
            ...
            if (ORBVersionFactory.getFOREIGN().compareTo(tcORB.getORBVersion()) == 0 || ORBVersionFactory
                    .getNEWER().compareTo(tcORB.getORBVersion()) <= 0)
    ...
    // Extracts a member value according to the given TypeCode from the given
    // complex Any
    ...
    // There is no other way for DynAnys to find out whether the Any is
    // initialized.
