...
public final class TypeCodeImpl extends TypeCode {
    ...
    static final String[] kindNames = { "null", "void", "short", "long", "ushort", "ulong", "float", "double",
            "boolean", "char", "octet", "any", "typecode", "principal", "objref", "struct", "union", "enum",
            "string", "sequence", "array", "alias", "exception", "longlong", "ulonglong", "longdouble",
    ...
    public TypeCodeImpl(ORB orb, int creationKind, String id, String name, TypeCode discriminator_type,
            UnionMember[] members)
    // for unions
    {
        this(orb);
                ...
                case COMPLEX:
                    ...
                    switch (_kind) {
                        ...
                        case TCKind._tk_value: {
                            ...
                            // check for member types.
                            for (int i = 0; i < _memberCount; i++)
                                if (_memberAccess[i] != tc.member_visibility(i) || !_memberTypes[i].equal(tc
                                        .member_type(i)))
                                    return false;
                            ...
                            TypeCode tccb = tc.concrete_base_type();
                            if ((_concrete_base == null && tccb != null) || (_concrete_base != null
                                    && tccb == null) || !_concrete_base.equal(tccb)) {
                                return false;
                            }
                            // ignore id and names since those are optional.
                            return true;
     ...
     * equivalence for values stored in an IDL any.
     */
    public boolean equivalent(TypeCode tc) {
        ...
        // If the result of the kind operation on either TypeCode is tk_alias,
        // recursively
        // replace the TypeCode with the result of calling content_type, until
        // the kind
        ...
        try {
            ...
            // Return true if the results of id for both TypeCodes are non-empty
            // strings
            // and both strings are equal.
            // If both ids are non-empty but are not equal, then equivalent
            // returns FALSE.
        ...
        try {
            if (myKind == TCKind._tk_struct || myKind == TCKind._tk_union || myKind == TCKind._tk_enum
            ...
            if (myKind == TCKind._tk_fixed) {
                if (myRealType.fixed_digits() != otherRealType.fixed_digits() || myRealType
                        .fixed_scale() != otherRealType.fixed_scale())
                    return false;
            }
            if (myKind == TCKind._tk_union) {
            ...
            if (myKind == TCKind._tk_struct || myKind == TCKind._tk_union || myKind == TCKind._tk_except
    ...
    public TypeCode get_compact_typecode() {
        // _REVISIT_ It isn't clear whether this method should operate on this
        // or a copy.
        // For now just return this unmodified because the name and member_name
        // fields
    ...
    boolean read_value_kind(TypeCodeReader tcis) {
        ...
        if (_kind == tk_indirect) {
            ...
            // recursive ,
            // TypeCodes i.e., a 0xffffffff indirection marker followed by a
            // long offset
    ...
    public void write_value(OutputStream os) {
        ...
        if (os instanceof TypeCodeOutputStream) {
            ...
            if (outBuffer == null) {
                ...
                // System.out.println("Using cached outBuffer: length = " +
                // outBuffer.length +
                // ", id = " + _id);
            }
    ...
    public void write_value(TypeCodeOutputStream tcos) {
        ...
        if (_kind == tk_indirect) {
            // if (debug) System.out.println("Writing indirection " + _name +
            // "to " + _id);
            // The encoding used for indirection is the same as that used for
            // recursive ,
            // TypeCodes i.e., a 0xffffffff indirection marker followed by a
        ...
        // if (debug) System.out.println("Writing " + _name + " with id " +
        // _id);
        // We have to remember the stream and position for EVERY type code
        // in case some recursive or indirect type code references it.
        //
    ...
    /**
     * This is not a copy of the TypeCodeImpl objects, but instead it copies the
     * value this type code is representing. See AnyImpl read_value and
     * write_value for usage. The state of this TypeCodeImpl instance isn't
     * changed, only used by the Any to do the correct copy.
     */
    ...
    int currentUnionMemberIndex(Any discriminatorValue) throws BadKind {
        ...
        } catch (BadKind bad) {} catch (org.omg.CORBA.TypeCodePackage.Bounds bounds) {}
        return -1;
    }
    ...
    private void printStream(PrintStream s, int level) {
        ...
        switch (_kind) {
            ...
            case TCKind._tk_alias:
                s.print("alias " + _name + " = " + (_contentType != null ? _contentType._name
                        : "<unresolved>"));
                break;
