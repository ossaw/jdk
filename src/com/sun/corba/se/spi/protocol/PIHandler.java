...
/**
 * This interface defines the PI interface that is used to interface the rest of
 * the ORB to the PI implementation.
 */
public interface PIHandler extends Closeable {
    /**
     * Complete the initialization of the PIHandler. This will execute the
     * methods on the ORBInitializers, if any are defined. This must be done
     * here so that the ORB can obtain the PIHandler BEFORE the ORBInitializers
     * run, since they will need access to the PIHandler through the ORB.
     */
     ...
     * for calls on this thread. Keeps track of the number of times this is
     * called and increments the disabledCount.
     */
    void disableInterceptorsThisThread();

    /**
     * Called for pseudo-ops to re-enable portable interceptor hooks for calls
     * on this thread. Decrements the disabledCount. If disabledCount is 0,
     * interceptors are re-enabled.
     */
     ...
     *         exception set using setClientPIException. There are four possible
     *         return types: null (no exception), SystemException,
     *         UserException, or RemarshalException.
     */
    Exception invokeClientPIEndingPoint(int replyStatus, Exception exception);

    /**
     * Called when a retry is needed after initiateClientPIRequest but before
     * invokeClientPIRequest. In this case, we need to properly balance
     * initiateClientPIRequest/cleanupClientPIRequest calls, but WITHOUT
     ...
     *         UserException, or RemarshalException.
     */
    Exception makeCompletedClientRequest(int replyStatus, Exception exception);

    /**
     * Invoked when a request is about to be created. Must be called before any
     * of the setClientPI* methods so that a new info object can be prepared for
     ...
     * ending points are called so that the info object on the stack can be
     * deinitialized and popped from the stack at the appropriate time.
     */
    void cleanupClientPIRequest();

    /**
     * Notifies PI of additional information for client-side interceptors. PI
     * will use this information as a source of information for the
     ...
     * server-side interceptors. PI will use this information as a source of
    ...
    /**
     * Notifies PI of additional information for server-side interceptors. PI
     * will use this information as a source of information for the
