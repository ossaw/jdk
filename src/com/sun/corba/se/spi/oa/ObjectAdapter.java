 ...
 * introduced by ORT. This means that all object adapters must:
 * <UL>
 * <LI>Have an ORB</LI>
 * <LI>Have a name</LI>
 ...
 * <LI>Support adding components to their profiles expressed in the adapter
 * template</LI>
 * </UL>
 * Other requirements:
 * <UL>
 ...
 * <LI>AdapterStateChanged must be invoked (from somewhere) whenever an adapter
 * state changes that is not due to an adapter manager state change.</LI>
 * </UL>
 * <P>
 ...
 * <LI>Manage the state of the adapter, if the adapter desires to implement such
 * mechanisms</LI>
 * </UL>
 * Such mechanisms are all object adapter specific, and so we do not attempt to
 * create general APIs for these functions here. The object adapter itself must
 * provide these APIs directly to the user, and they do not affect the rest of
 * the ORB. This interface basically makes it possible to plug any object
 * adapter into the ORB and have the OA work propertly with portable
 * interceptors, and also have requests dispatched properly to the object
 * adapter.
 * <P>
 * The basic function of an ObjectAdapter is to map object IDs to servants and
 * to support the dispatch operation of the subcontract, which dispatches
 * requests to servants. This is the purpose of the getInvocationServant method.
 * In addition, ObjectAdapters must be able to change state gracefully in the
 * presence of executing methods. This requires the use of the enter/exit
 * methods. Finally, ObjectAdapters often require access to information about
 * requests. This is accomodated through the OAInvocationInfo class and the
 * thread local stack maintained by push/pop/peekInvocationInfo on the ORB.
 * <P>
 * To be useful, this dispatch cycle must be extremely efficient. There are
 * several scenarios that matter:
 * <ol>
 * <li>A remote invocation, where the dispatch is handled in the server
 * subcontract.</li>
 * <li>A local invocation, where the dispatch is handled in the client
 * subcontract.</li>
 * <li>A cached local invocation, where the servant is cached when the IOR is
 * established for the client subcontract, and the dispatch is handled in the
 * client subcontract to the cached subcontract.
 * <li>
 * </ol>
 * <p>
 * Each of these 3 cases is handled a bit differently. On each request, assume
 * as known ObjectId and ObjectAdapterId, which can be obtained from the object
 * key. The ObjectAdaptorFactory is available in the subcontract registry, where
 * it is registered under the subcontract ID. The Subcontract ID is also
 * available in the object key.
 * <ol>
 ...
 * <li>pop info</li>
 * <ol>
 * </li> REVISIT: Is this the required order for exit/pop? Cna they be nested
 * instead? Note that getInvocationServant and returnServant may throw
 * exceptions. In such cases, returnServant, exit, and pop must be called in the
 * correct order.
 ...
 * <li>pop info</li>
 * <ol>
 * </li> This is the same as the remote case, except that
 * setExecuteReturnServantInResponseConstructor is not needed (or possible,
 * since there is no server request).
 * <li>The fast local pattern: When delegate is constructed, first extract
 * ObjectKey from IOR in delegate, then get ObjectId, ObjectAdapterId, and
 * ObjectAdapterFactory (oaf). Then:
 * <ol>
 * <li>oa = oaf.find( oaid )</li>
 * <li>info = oa.makeInvocationInfo( oid ) (note: no operation!)</li>
 * <li>push info (needed for the correct functioning of getInvocationServant)
 * </li>
 * <li>oa.getInvocationServant( info )</li>
 * <li>pop info
 * </ol>
 * The info instance (which includes the Servant) is cached in the client
 * subcontract.
 * <p>
