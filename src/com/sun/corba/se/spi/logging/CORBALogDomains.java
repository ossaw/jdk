...
/**
 * Defines constants for all of the logging domains used in the ORB. Note that
 * this is the suffix to the log domain. The full domain is given by
 * <code>javax.enterprise.resource.corba.{ORBId}.{Log domain}</code> where
 * {ORBId} is the ORB ID of the ORB instance doing the logging.
 * <P>
 ...
 * <li>core: service context code in rpc.protocol (will eventually move to its
 * own package)
...
public abstract class CORBALogDomains {
	...
	/**
	 * Log domain for code directly implementing the CORBA API and the
	 * typecode/any machinery.
	 */
	 ...
	 * This includes client and server delegates, client and server request
	 * dispatchers, service contexts, portable interceptors, and the GIOP
	 * protocol (but not CDR representation of data).
	 */
	public static final String RPC_PROTOCOL = "rpc.protocol";

	/**
	 * Log domain for low-level transport details, which are independent of
	 * encoding and presentation details. This includes selectors, acceptors,
	 * connections, connection management, and any other transport management
	 * functions.
	 */
	...
	/**
	 * Log domain for name service bind, rebind, destroy, and other state change
	 * operations.
	 */
	...
	/**
	 * Log domain for the ORBD locator function, which forwards client requests
	 * to their current server incarnation.
	 */
	public static final String ORBD_LOCATOR = "orbd.locator";

	/**
	 * Log domain for the ORBD activator function, which starts server instances
	 * on demand.
	 */
	...
	/**
	 * Log domain for the servertool utilitiy used to update the implementation
	 * repository.
	 */
	...
	/**
	 * Log domain for ORB client side name resolution and supporting functions
	 * such as INS.
	 */
	...
	/**
	 * Log domain for creation, destruction, and state change of Object Adapters
	 * and related classes (e.g. POAManager).
	 */
