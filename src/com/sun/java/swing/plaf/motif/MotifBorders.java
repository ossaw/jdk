...
public class MotifBorders {
		...
		public void paintBorder(Component c, Graphics g, int x, int y, int width, int height) {
			if (c instanceof AbstractButton) {
				...
				if (model.isArmed() && model.isPressed() || model.isSelected()) {
					drawBezel(g, x, y, width, height, (model.isPressed() || model.isSelected()),
							b.isFocusPainted() && b.hasFocus(), shadow, highlight, darkShadow,
							focus);
				} else {
					drawBezel(g, x, y, width, height, false, b.isFocusPainted() && b.hasFocus(),
							shadow, highlight, darkShadow, focus);
				}
			} else {
				drawBezel(g, x, y, width, height, false, false, shadow, highlight, darkShadow,
						focus);
			}
		}
	...
	public static class MenuBarBorder extends ButtonBorder {
		...
		public void paintBorder(Component c, Graphics g, int x, int y, int width, int height) {
			...
			if (menuBar.isBorderPainted() == true) {
				...
				drawBezel(g, x, y, size.width, size.height, false, false, shadow, highlight,
						darkShadow, focus);
			}
		}
	...
	public static class FrameBorder extends AbstractBorder implements UIResource {
		 ...
		 */
		protected boolean drawTopBorder(Component c, Graphics g, int x, int y, int width,
				int height) {
		 ...
		 */
		protected boolean drawLeftBorder(Component c, Graphics g, int x, int y, int width,
				int height) {
			Rectangle borderRect = new Rectangle(0, 0, getBorderInsets(c).left, height);
		 ...
		 */
		protected boolean drawRightBorder(Component c, Graphics g, int x, int y, int width,
				int height) {
			Rectangle borderRect = new Rectangle(width - getBorderInsets(c).right, 0,
		 ...
		 */
		protected boolean drawBottomBorder(Component c, Graphics g, int x, int y, int width,
				int height) {
			Rectangle borderRect;
			int marginHeight, startY;

			borderRect = new Rectangle(0, height - getBorderInsets(c).bottom, width,
	...
	public static void drawBezel(Graphics g, int x, int y, int w, int h, boolean isPressed,
			boolean hasFocus, Color shadow, Color highlight, Color darkShadow, Color focus) {
	...
	public static class MotifPopupMenuBorder extends AbstractBorder implements UIResource {
		 ...
		 *
		 */
		public MotifPopupMenuBorder(Font titleFont, Color bgColor, Color fgColor, Color shadow,
				Color highlight) {
			this.font = titleFont;
		...
		public void paintBorder(Component c, Graphics g, int x, int y, int width, int height) {
			...
			MotifGraphicsUtils.drawGroove(g, x, textLoc.y + TEXT_SPACING, width, GROOVE_HEIGHT,
