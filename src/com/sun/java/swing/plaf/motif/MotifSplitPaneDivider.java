...
public class MotifSplitPaneDivider extends BasicSplitPaneDivider {
	 ...
	 * JSplitPane so that this ins't needed
	 */
	...
	public void paint(Graphics g) {
		...
		if (getBasicSplitPaneUI().getOrientation() == JSplitPane.HORIZONTAL_SPLIT) {
			...
			// draw thumb
			g.setColor((splitPane.hasFocus()) ? focusedColor : getBackground());
	...
	public void setBasicSplitPaneUI(BasicSplitPaneUI newUI) {
		...
		if (newUI != null) {
			...
			if (splitPane != null) {
				if (mouseHandler == null)
					mouseHandler = new MotifMouseHandler();
	...
	/**
	 * Returns true if the point at <code>x</code>, <code>y</code> is inside the
	 * thumb.
	 */
	...
	/**
	 * MouseHandler is subclassed to only pass off to super if the mouse is in
	 * the thumb. Motif only allows dragging when the thumb is clicked in.
	 */
	private class MotifMouseHandler extends MouseHandler {
		public void mousePressed(MouseEvent e) {
			// Constrain the mouse pressed to the thumb.
			if (e.getSource() == MotifSplitPaneDivider.this && getDragger() == null
					&& getSplitPane().isEnabled() && isInThumb(e.getX(), e.getY())) {
