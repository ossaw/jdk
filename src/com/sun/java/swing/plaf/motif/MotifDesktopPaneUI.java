...
public class MotifDesktopPaneUI extends javax.swing.plaf.basic.BasicDesktopPaneUI {
	...
	////////////////////////////////////////////////////////////////////////////////////
	private class MotifDesktopManager extends DefaultDesktopManager
		...
		public void endDraggingFrame(JComponent f) {
			if (usingDragPane) {
				...
				if (f instanceof JInternalFrame) {
					setBoundsForFrame(f, dragPane.getX(), dragPane.getY(), dragPane.getWidth(),
							dragPane.getHeight());
				} else if (f instanceof JInternalFrame.JDesktopIcon) {
					adjustBoundsForIcon((JInternalFrame.JDesktopIcon) f, dragPane.getX(),
							dragPane.getY());
				}
			}
		}
		...
		/**
		 * Change positions of icons in the desktop pane so that they do not
		 * overlap
		 */
		...
		protected void adjustBoundsForIcon(JInternalFrame.JDesktopIcon icon, int x, int y) {
			 ...
			 * algorithm would place the icon in a non-displayed cell. Never to
			 * be ssen again.
			 */
			y = y >= maxy ? (maxy - 1) : y;
