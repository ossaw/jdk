...
public class GTKColorType extends ColorType {
	...
	private static int hlsToRGB(float h, float l, float s) {
		...
		return (((int) (r * 255)) << 16) | (((int) (g * 255.0)) << 8) | ((int) (b * 255));
	}
	...
	/**
	 * Creates and returns a new color derived from the passed in color. The
	 * transformation is done in the HLS color space using the specified
	 * arguments to scale.
	 *
	 * @param color
	 *            Color to alter
	 * @param hFactory
	 *            Amount to scale the hue
	 * @param lFactor
	 *            Amount to scale the lightness
	 * @param sFactory
	 *            Amount to sacle saturation
