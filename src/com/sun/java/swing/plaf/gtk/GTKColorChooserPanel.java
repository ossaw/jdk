...
/**
 * A color chooser panel mimicking that of GTK's: a color wheel showing hue and
 * a triangle that varies saturation and brightness.
 *
 * @author Scott Violet
 */
class GTKColorChooserPanel extends AbstractColorChooserPanel implements ChangeListener {
	...
	protected void buildChooser() {
		...
		configureSpinner(saturationSpinner, "GTKColorChooserPanel.saturationSpinner");
		...
		add(new JSeparator(SwingConstants.HORIZONTAL),
				new GridBagConstraints(1, 3, 4, 1, 1, 0, GridBagConstraints.LINE_START,
						GridBagConstraints.HORIZONTAL, new Insets(14, 0, 0, 0), 0, 0));

		add(this, "GTKColorChooserPanel.colorName", colorNameTF, 0, 4);

		add(triangle, new GridBagConstraints(0, 0, 1, 5, 0, 0, GridBagConstraints.LINE_START,
				GridBagConstraints.NONE, new Insets(14, 20, 2, 9), 0, 0));

		Box hBox = Box.createHorizontalBox();
		hBox.add(lastLabel);
		hBox.add(label);
		add(hBox, new GridBagConstraints(0, 5, 1, 1, 0, 0, GridBagConstraints.CENTER,
				GridBagConstraints.NONE, new Insets(0, 0, 0, 0), 0, 0));

		add(new JSeparator(SwingConstants.HORIZONTAL),
				new GridBagConstraints(0, 6, 5, 1, 1, 0, GridBagConstraints.LINE_START,
	...
	private void add(Container parent, String key, JComponent widget, int x, int y) {
		JLabel label = new JLabel(UIManager.getString(key + "Text", getLocale()));
		...
		if (mnemonic != null) {
			...
			String mnemonicIndex = (String) UIManager.get(key + "MnemonicIndex", getLocale());
		...
		GridBagConstraints con = new GridBagConstraints(x + 1, y, 1, 1, 0, 0,
				GridBagConstraints.FIRST_LINE_END, GridBagConstraints.NONE, new Insets(4, 0, 0, 4),
				0, 0);
		if (y == 0) {
			con.insets.top = 14;
		}
	 ...
	 * @param color
	 *            new Color
	 * @param updateSpinners
	 *            whether or not to update the spinners.
	 * @param updateHSB
	 *            if true, the hsb fields are updated based on the new color
	 * @param updateModel
	 *            if true, the model is set.
	 */
	private void setColor(Color color, boolean updateSpinners, boolean updateHSB,
			boolean updateModel) {
		if (color == null) {
			color = Color.BLACK;
		}
	...
	/**
	 * Flag indicating the angle, or hue, has changed and the triangle needs to
	 * be recreated.
	 */
	...
	private class ColorTriangle extends JPanel {
		...
		protected void processEvent(AWTEvent e) {
			if (e.getID() == MouseEvent.MOUSE_PRESSED
				...
				// currently has focus.
				if (!isSet(FLAGS_FOCUSED_TRIANGLE) && !isSet(FLAGS_FOCUSED_WHEEL)) {
		...
		public void paintComponent(Graphics g) {
			...
			g.drawImage(image, getWheelXOrigin() - size, getWheelYOrigin() - size, null);

			// Draw the focus indicator for the wheel
			if (hasFocus() && isSet(FLAGS_FOCUSED_WHEEL)) {
				g.setColor(Color.BLACK);
				g.drawOval(getWheelXOrigin() - size, getWheelYOrigin() - size, 2 * size, 2 * size);
				g.drawOval(getWheelXOrigin() - size + width, getWheelYOrigin() - size + width,
						2 * (size - width), 2 * (size - width));
			}
			...
			g.setColor(Color.BLACK);
			g.drawOval(circleX, circleY, getIndicatorSize() - 1, getIndicatorSize() - 1);
			g.setColor(Color.WHITE);
			g.drawOval(circleX + 1, circleY + 1, getIndicatorSize() - 3, getIndicatorSize() - 3);
		}
		...
		private Image getImage(int size) {
			...
			if (triangleImage == null || triangleImage.getWidth(null) != a) {
				triangleImage = new BufferedImage(a, a, BufferedImage.TYPE_INT_ARGB);
			}
			...
			g.setColor(new Color(0, 0, 0, 0));
			g.fillOval(getWheelWidth(), getWheelWidth(), 2 * (size - getWheelWidth()),
					2 * (size - getWheelWidth()));

			double rotate = Math.toRadians(-30.0) + angle;
			g.translate(size, size);
			((Graphics2D) g).rotate(rotate);
			g.drawImage(triangleImage, -a / 2, getWheelWidth() - size, null);
			((Graphics2D) g).rotate(-rotate);
		...
		private void paintTriangle(Graphics g, int size, Color color) {
			float[] colors = Color.RGBtoHSB(color.getRed(), color.getGreen(), color.getBlue(),
					null);
			float hue = colors[0];
		 ...
		 *
		 * @param size
		 *            Integer giving size of color wheel.
			...
			for (int y = -size; y < size; y++) {
				...
				for (int x = -size; x < size; x++) {
					...
					if (rad < size && rad > minSize) {
						int rgb = colorWheelLocationToRGB(x, y, rad) | 0xFF000000;
		 ...
		 * <code>y</code> give the location to adjust to and are relative to the
		 * origin of the wheel/triangle.
		 *
		 * @param x
		 *            X coordinate on the triangle to adjust to
		 * @param y
		 *            Y coordinate on the triangle to adjust to
		 * @param checkLoc
		 *            if true the location is checked to make sure it is
		 *            contained in the triangle, if false the location is
		...
		boolean adjustSB(int x, int y, boolean checkLoc) {
			...
			int maxY = (int) ((triangleSize - x1 - innerR / 2.0) * Math.tan(Math.toRadians(30.0)));
			if (y1 <= -maxY) {
				if (checkLoc) {
					return false;
				}
			...
			if (resetXY) {
				...
				setSaturationAndBrightness(saturation, value, x + getWheelXOrigin(),
						getWheelYOrigin() - y);
			}
		...
		private void setSaturationAndBrightness(float s, float b) {
			...
			x = x - innerR;
			double x1 = Math.cos(Math.toRadians(-60.0) - angle) * x
					- Math.sin(Math.toRadians(-60.0) - angle) * y;
		 ...
		 *
		 * @param x
		 *            X location to adjust to, relative to the origin of the
		 *            wheel
		 * @param y
		 *            Y location to adjust to, relative to the origin of the
		 *            wheel
		 * @param check
		 *            if true the location is checked to make sure it is
		 *            contained in the wheel, if false the location is
		...
		/**
		 * Returns the RGB color to use for the specified location. The passed
		 * in point must be on the color wheel and be relative to the origin of
		 * the color wheel.
		 *
		 * @param x
		 *            X location to get color for
		 * @param y
		 *            Y location to get color for
		 * @param rad
		 *            Radius from center of color wheel
	...
	private static class ColorAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (triangle.isWheelFocused()) {
				...
				triangle.adjustSB(triangle.getColorX() + xDelta, triangle.getColorY() + yDelta,
						true);
			}
		}
	}
