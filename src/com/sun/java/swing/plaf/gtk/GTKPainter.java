...
class GTKPainter extends SynthPainter {
	private static final PositionType[] POSITIONS = { PositionType.BOTTOM, PositionType.RIGHT,
			PositionType.TOP, PositionType.LEFT };

	private static final ShadowType SHADOWS[] = { ShadowType.NONE, ShadowType.IN, ShadowType.OUT,
	...
	public void paintFormattedTextFieldBackground(SynthContext context, Graphics g, int x, int y,
			int w, int h) {
	...
	public void paintToolBarBackground(SynthContext context, Graphics g, int x, int y, int w,
		...
		synchronized (UNIXToolkit.GTK_LOCK) {
			if (!ENGINE.paintCachedImage(g, x, y, w, h, id, state, orientation)) {
				...
				ENGINE.paintBox(g, context, id, gtkState, ShadowType.OUT, "handlebox_bin", x, y, w,
						h);
	...
	public void paintToolBarContentBackground(SynthContext context, Graphics g, int x, int y, int w,
			int h) {
		Region id = context.getRegion();
		int orientation = ((JToolBar) context.getComponent()).getOrientation();
		synchronized (UNIXToolkit.GTK_LOCK) {
			if (!ENGINE.paintCachedImage(g, x, y, w, h, id, orientation)) {
				...
				ENGINE.paintBox(g, context, id, SynthConstants.ENABLED, ShadowType.OUT, "toolbar",
						x, y, w, h);
				ENGINE.finishPainting();
			}
		}
	...
	// PASSWORD_FIELD
	//
	public void paintPasswordFieldBackground(SynthContext context, Graphics g, int x, int y, int w,
			int h) {
		paintTextBackground(context, g, x, y, w, h);
	}
	...
	public void paintRadioButtonBackground(SynthContext context, Graphics g, int x, int y, int w,
			int h) {
		Region id = context.getRegion();
		...
		if (gtkState == SynthConstants.MOUSE_OVER) {
			synchronized (UNIXToolkit.GTK_LOCK) {
				if (!ENGINE.paintCachedImage(g, x, y, w, h, id)) {
					...
					ENGINE.paintFlatBox(g, context, id, SynthConstants.MOUSE_OVER,
							ShadowType.ETCHED_OUT, "checkbutton", x, y, w, h, ColorType.BACKGROUND);
					ENGINE.finishPainting();
				}
			}
		}
	...
	public void paintRadioButtonMenuItemBackground(SynthContext context, Graphics g, int x, int y,
			int w, int h) {
		Region id = context.getRegion();
		int gtkState = GTKLookAndFeel.synthStateToGTKState(id, context.getComponentState());
		if (gtkState == SynthConstants.MOUSE_OVER) {
	...
	public void paintLabelBackground(SynthContext context, Graphics g, int x, int y, int w, int h) {
		...
		if (name == "TableHeader.renderer" || name == "GTKFileChooser.directoryListLabel"
				|| name == "GTKFileChooser.fileListLabel") {

			paintButtonBackgroundImpl(context, g, Region.BUTTON, "button", x, y, w, h, true, false,
					false, false);
		}
		/*
		 * If the label is a ListCellRenderer and it's in a container
		 * (CellRendererPane) which is in a JComboBox then we paint the label as
		 * a TextField like a gtk_entry for a combobox.
		 */
		else if (c instanceof ListCellRenderer && container != null
	...
	// DESKTOP_PANE
	//
	public void paintDesktopPaneBackground(SynthContext context, Graphics g, int x, int y, int w,
			int h) {
	...
	public void paintButtonBackground(SynthContext context, Graphics g, int x, int y, int w,
		...
		if (name != null && name.startsWith("InternalFrameTitlePane.")) {
			...
			paintButtonBackgroundImpl(context, g, Region.BUTTON, "button", x, y, w, h, paintBG,
					paintFocus, defaultCapable, toolButton);
		}
	}

	private void paintButtonBackgroundImpl(SynthContext context, Graphics g, Region id,
			String detail, int x, int y, int w, int h, boolean paintBackground, boolean paintFocus,
			boolean defaultCapable, boolean toolButton) {
		int state = context.getComponentState();
		synchronized (UNIXToolkit.GTK_LOCK) {
			if (ENGINE.paintCachedImage(g, x, y, w, h, id, state, detail, paintBackground,
					paintFocus, defaultCapable, toolButton)) {
				return;
			}
			ENGINE.startPainting(g, x, y, w, h, id, state, detail, paintBackground, paintFocus,
					defaultCapable, toolButton);

			// Paint the default indicator
			GTKStyle style = (GTKStyle) context.getStyle();
			if (defaultCapable && !toolButton) {
				Insets defaultInsets = style.getClassSpecificInsetsValue(context, "default-border",
						GTKStyle.BUTTON_DEFAULT_BORDER_INSETS);

				if (paintBackground && (state & SynthConstants.DEFAULT) != 0) {
					ENGINE.paintBox(g, context, id, SynthConstants.ENABLED, ShadowType.IN,
							"buttondefault", x, y, w, h);
				}
			...
			boolean interiorFocus = style.getClassSpecificBoolValue(context, "interior-focus",
					true);
			...
			if (toolButton) {
				...
				paintBg = (gtkState != SynthConstants.ENABLED)
	...
	// ARROW_BUTTON
	//
	public void paintArrowButtonForeground(SynthContext context, Graphics g, int x, int y, int w,
			int h, int direction) {
		Region id = context.getRegion();
		...
		int gtkState = GTKLookAndFeel.synthStateToGTKState(id, context.getComponentState());
		ShadowType shadowType = (gtkState == SynthConstants.PRESSED ? ShadowType.IN
				: ShadowType.OUT);
		synchronized (UNIXToolkit.GTK_LOCK) {
	...
	public void paintArrowButtonBackground(SynthContext context, Graphics g, int x, int y, int w,
			int h) {
		Region id = context.getRegion();
		AbstractButton button = (AbstractButton) context.getComponent();
		...
		synchronized (UNIXToolkit.GTK_LOCK) {
			...
			if (detail.startsWith("spin")) {
				 ...
				 */
				int spinState = button.getParent().isEnabled() ? SynthConstants.ENABLED
						: SynthConstants.DISABLED;
				int mody = (detail == "spinbutton_up") ? y : y - h;
				int modh = h * 2;
				ENGINE.paintBox(g, context, id, spinState, ShadowType.IN, "spinbutton", x, mody, w,
						modh);
			}
	...
	public void paintPopupMenuBackground(SynthContext context, Graphics g, int x, int y, int w,
		...
		synchronized (UNIXToolkit.GTK_LOCK) {
			...
			ENGINE.paintBackground(g, context, id, gtkState,
					style.getGTKColor(context, gtkState, GTKColorType.BACKGROUND), x + xThickness,
					y + yThickness, w - xThickness - xThickness, h - yThickness - yThickness);
			ENGINE.finishPainting();
		}
	}

	public void paintProgressBarBackground(SynthContext context, Graphics g, int x, int y, int w,
			int h) {
		Region id = context.getRegion();
		synchronized (UNIXToolkit.GTK_LOCK) {
			if (!ENGINE.paintCachedImage(g, x, y, w, h, id)) {
				ENGINE.startPainting(g, x, y, w, h, id);
				ENGINE.paintBox(g, context, id, SynthConstants.ENABLED, ShadowType.IN, "trough", x,
						y, w, h);
				ENGINE.finishPainting();
			}
		}
	...
	public void paintProgressBarForeground(SynthContext context, Graphics g, int x, int y, int w,
		...
		synchronized (UNIXToolkit.GTK_LOCK) {
			...
			ENGINE.paintBox(g, context, id, SynthConstants.MOUSE_OVER, ShadowType.OUT, "bar", x, y,
					w, h);
	...
	public void paintViewportBorder(SynthContext context, Graphics g, int x, int y, int w, int h) {
		...
		synchronized (UNIXToolkit.GTK_LOCK) {
			if (!ENGINE.paintCachedImage(g, x, y, w, h, id)) {
				ENGINE.startPainting(g, x, y, w, h, id);
				ENGINE.paintShadow(g, context, id, SynthConstants.ENABLED, ShadowType.IN,
						"scrolled_window", x, y, w, h);
				ENGINE.finishPainting();
			}
		}
	}
	...
	public void paintSeparatorBackground(SynthContext context, Graphics g, int x, int y, int w,
		...
		/*
		 * Note: In theory, the style's x/y thickness values would determine the
		 * width of the separator content. In practice, however, some engines
		 * will render a line that is wider than the corresponding thickness
		 * value. For example, ubuntulooks reports x/y thickness values of 1 for
		 * separators, but always renders a 2-pixel wide line. As a result of
		 * all this, we need to be careful not to restrict the w/h values below
		 * too much, so that the full thickness of the rendered line will be
		 * captured by our image caching code.
		 */
		String detail;
		if (c instanceof JToolBar.Separator) {
			/*
			 * GTK renders toolbar separators differently in that an artificial
			 * padding is added to each end of the separator. The value of 0.2f
			 * below is derived from the source code of gtktoolbar.c in the
			 * current version of GTK+ (2.8.20 at the time of this writing).
			 * Specifically, the relevant values are: SPACE_LINE_DIVISION 10.0
			 * SPACE_LINE_START 2.0 SPACE_LINE_END 8.0 These are used to
			 * determine the distance from the top (or left) edge of the toolbar
			 * to the other edge. So for example, the starting/top point of a
			 * vertical separator is 2/10 of the height of a horizontal toolbar
			 * away from the top edge, which is how we arrive at 0.2f below.
			 * Likewise, the ending/bottom point is 8/10 of the height away from
			 * the top edge, or in other words, it is 2/10 away from the bottom
			 * edge, which is again how we arrive at the 0.2f value below.
			 *
			 * The separator is also centered horizontally or vertically,
			 * depending on its orientation. This was determined empirically and
			 * by examining the code referenced above.
			 */
			detail = "toolbar";
			float pct = 0.2f;
	...
	public void paintSliderTrackBackground(SynthContext context, Graphics g, int x, int y, int w,
			int h) {
		Region id = context.getRegion();
		int state = context.getComponentState();
		...
		if (focused) {
			...
			focusSize = style.getClassSpecificIntValue(context, "focus-line-width", 1)
					+ style.getClassSpecificIntValue(context, "focus-padding", 1);
			x -= focusSize;
			y -= focusSize;
			w += focusSize * 2;
			h += focusSize * 2;
		...
		synchronized (UNIXToolkit.GTK_LOCK) {
			...
			ENGINE.paintBox(g, context, id, gtkState, ShadowType.IN, "trough", x + focusSize,
					y + focusSize, w - 2 * focusSize, h - 2 * focusSize);
			if (focused) {
	...
	public void paintSliderThumbBackground(SynthContext context, Graphics g, int x, int y, int w,
			int h, int dir) {
		Region id = context.getRegion();
		int gtkState = GTKLookAndFeel.synthStateToGTKState(id, context.getComponentState());
		synchronized (UNIXToolkit.GTK_LOCK) {
			if (!ENGINE.paintCachedImage(g, x, y, w, h, id, gtkState, dir)) {
				Orientation orientation = (dir == JSlider.HORIZONTAL ? Orientation.HORIZONTAL
						: Orientation.VERTICAL);
				String detail = (dir == JSlider.HORIZONTAL ? "hscale" : "vscale");
				ENGINE.startPainting(g, x, y, w, h, id, gtkState, dir);
				ENGINE.paintSlider(g, context, id, gtkState, ShadowType.OUT, detail, x, y, w, h,
						orientation);
				ENGINE.finishPainting();
			}
		}
	...
	public void paintSplitPaneDividerBackground(SynthContext context, Graphics g, int x, int y,
		...
		Orientation orientation = (splitPane.getOrientation() == JSplitPane.HORIZONTAL_SPLIT
		...
		synchronized (UNIXToolkit.GTK_LOCK) {
			if (!ENGINE.paintCachedImage(g, x, y, w, h, id, gtkState, orientation)) {
				...
				ENGINE.paintHandle(g, context, id, gtkState, ShadowType.OUT, "paned", x, y, w, h,
						orientation);
				ENGINE.finishPainting();
			}
		}
	...
	public void paintTabbedPaneContentBackground(SynthContext context, Graphics g, int x, int y,
		...
		PositionType placement = GTKLookAndFeel
				.SwingOrientationConstantToGTK(pane.getTabPlacement());
		...
		int gtkState = GTKLookAndFeel.synthStateToGTKState(id, context.getComponentState());
		synchronized (UNIXToolkit.GTK_LOCK) {
			if (!ENGINE.paintCachedImage(g, x, y, w, h, id, gtkState, placement, gapStart,
					gapSize)) {
				ENGINE.startPainting(g, x, y, w, h, id, gtkState, placement, gapStart, gapSize);
				ENGINE.paintBoxGap(g, context, id, gtkState, ShadowType.OUT, "notebook", x, y, w, h,
						placement, gapStart, gapSize);
				ENGINE.finishPainting();
			}
		}
	}
	...
	public void paintTabbedPaneTabBackground(SynthContext context, Graphics g, int x, int y, int w,
		...
		Region id = context.getRegion();
		int state = context.getComponentState();
		int gtkState = ((state & SynthConstants.SELECTED) != 0 ? SynthConstants.ENABLED
		...
		synchronized (UNIXToolkit.GTK_LOCK) {
			if (!ENGINE.paintCachedImage(g, x, y, w, h, id, gtkState, placement, tabIndex)) {
				...
				ENGINE.paintExtension(g, context, id, gtkState, ShadowType.OUT, "tab", x, y, w, h,
						side, tabIndex);
				ENGINE.finishPainting();
			}
		}
	...
	private void paintTextBackground(SynthContext context, Graphics g, int x, int y, int w, int h) {
		...
		synchronized (UNIXToolkit.GTK_LOCK) {
			...
			ENGINE.paintFlatBox(g, context, id, gtkState, ShadowType.NONE, "entry_bg",
					x + xThickness, y + yThickness, w - (2 * xThickness), h - (2 * yThickness),
	...
	private void paintTreeCellEditorBackground(SynthContext context, Graphics g, int x, int y,
			int w, int h) {
		Region id = context.getRegion();
		int gtkState = GTKLookAndFeel.synthStateToGTKState(id, context.getComponentState());
		synchronized (UNIXToolkit.GTK_LOCK) {
			if (!ENGINE.paintCachedImage(g, x, y, w, h, id, gtkState)) {
				ENGINE.startPainting(g, x, y, w, h, id, gtkState);
				ENGINE.paintFlatBox(g, context, id, gtkState, ShadowType.NONE, "entry_bg", x, y, w,
						h, ColorType.TEXT_BACKGROUND);
				ENGINE.finishPainting();
			}
		}
	}
	...
	// TOGGLE_BUTTON
	//
	public void paintToggleButtonBackground(SynthContext context, Graphics g, int x, int y, int w,
			int h) {
		Region id = context.getRegion();
		...
		paintButtonBackgroundImpl(context, g, id, "button", x, y, w, h, paintBG, paintFocus, false,
				toolButton);
	}
	...
	public void paintScrollBarBackground(SynthContext context, Graphics g, int x, int y, int w,
		...
		synchronized (UNIXToolkit.GTK_LOCK) {
			...
			ENGINE.paintBox(g, context, id, SynthConstants.PRESSED, ShadowType.IN, "trough",
	...
	// SCROLL_BAR_THUMB
	//
	public void paintScrollBarThumbBackground(SynthContext context, Graphics g, int x, int y, int w,
			int h, int dir) {
		Region id = context.getRegion();
		int gtkState = GTKLookAndFeel.synthStateToGTKState(id, context.getComponentState());
		...
		boolean rtl = sb.getOrientation() == JScrollBar.HORIZONTAL
		...
		synchronized (UNIXToolkit.GTK_LOCK) {
			if (!ENGINE.paintCachedImage(g, x, y, w, h, id, gtkState, dir, value, visible, rtl)) {
				...
				Orientation orientation = (dir == JScrollBar.HORIZONTAL ? Orientation.HORIZONTAL
						: Orientation.VERTICAL);
				ENGINE.setRangeValue(context, id, value, min, max, visible);
				ENGINE.paintSlider(g, context, id, gtkState, ShadowType.OUT, "slider", x, y, w, h,
						orientation);
				ENGINE.finishPainting();
			}
		}
	...
	public void paintToolTipBackground(SynthContext context, Graphics g, int x, int y, int w,
		...
		synchronized (UNIXToolkit.GTK_LOCK) {
			if (!ENGINE.paintCachedImage(g, x, y, w, h, id)) {
				ENGINE.startPainting(g, x, y, w, h, id);
				ENGINE.paintFlatBox(g, context, id, SynthConstants.ENABLED, ShadowType.OUT,
						"tooltip", x, y, w, h, ColorType.BACKGROUND);
				ENGINE.finishPainting();
			}
		}
	}
	...
	public void paintTreeCellBackground(SynthContext context, Graphics g, int x, int y, int w,
		...
		synchronized (UNIXToolkit.GTK_LOCK) {
			if (!ENGINE.paintCachedImage(g, x, y, w, h, id, state)) {
				...
				ENGINE.paintFlatBox(g, context, id, gtkState, ShadowType.NONE, "cell_odd", x, y, w,
						h, ColorType.TEXT_BACKGROUND);
				ENGINE.finishPainting();
			}
		}
	...
	void paintFocus(SynthContext context, Graphics g, Region id, int state, String detail, int x,
			int y, int w, int h) {
		int gtkState = GTKLookAndFeel.synthStateToGTKState(id, state);
		synchronized (UNIXToolkit.GTK_LOCK) {
	...
	void paintMetacityElement(SynthContext context, Graphics g, int gtkState, String detail, int x,
			int y, int w, int h, ShadowType shadow, ArrowType direction) {
		synchronized (UNIXToolkit.GTK_LOCK) {
			if (!ENGINE.paintCachedImage(g, x, y, w, h, gtkState, detail, shadow, direction)) {
				...
				if (detail == "metacity-arrow") {
					ENGINE.paintArrow(g, context, Region.INTERNAL_FRAME_TITLE_PANE, gtkState,
							shadow, direction, "", x, y, w, h);

				} else if (detail == "metacity-box") {
					ENGINE.paintBox(g, context, Region.INTERNAL_FRAME_TITLE_PANE, gtkState, shadow,
							"", x, y, w, h);

				} else if (detail == "metacity-vline") {
					ENGINE.paintVline(g, context, Region.INTERNAL_FRAME_TITLE_PANE, gtkState, "", x,
							y, w, h);
				}
				ENGINE.finishPainting();
			}
	...
	void paintIcon(SynthContext context, Graphics g, Method paintMethod, int x, int y, int w,
			int h) {
		int state = context.getComponentState();
		synchronized (UNIXToolkit.GTK_LOCK) {
	...
	public void paintTreeExpandedIcon(SynthContext context, Graphics g, int state, int x, int y,
			int w, int h) {
	...
	public void paintTreeCollapsedIcon(SynthContext context, Graphics g, int state, int x, int y,
			int w, int h) {
		ENGINE.paintExpander(g, context, Region.TREE,
				GTKLookAndFeel.synthStateToGTKState(context.getRegion(), state),
	...
	public void paintCheckBoxIcon(SynthContext context, Graphics g, int state, int x, int y, int w,
			int h) {
		GTKStyle style = (GTKStyle) context.getStyle();
		int size = style.getClassSpecificIntValue(context, "indicator-size",
				GTKIconFactory.DEFAULT_ICON_SIZE);
		int offset = style.getClassSpecificIntValue(context, "indicator-spacing",
				GTKIconFactory.DEFAULT_ICON_SPACING);

		ENGINE.paintCheck(g, context, Region.CHECK_BOX, "checkbutton", x + offset, y + offset, size,
				size);
	}

	public void paintRadioButtonIcon(SynthContext context, Graphics g, int state, int x, int y,
			int w, int h) {
		GTKStyle style = (GTKStyle) context.getStyle();
		int size = style.getClassSpecificIntValue(context, "indicator-size",
				GTKIconFactory.DEFAULT_ICON_SIZE);
		...
		ENGINE.paintOption(g, context, Region.RADIO_BUTTON, "radiobutton", x + offset, y + offset,
				size, size);
	}

	public void paintMenuArrowIcon(SynthContext context, Graphics g, int state, int x, int y, int w,
			int h, ArrowType dir) {
		...
		ENGINE.paintArrow(g, context, Region.MENU_ITEM, gtkState, shadow, dir, "menuitem", x + 3,
				y + 3, 7, 7);
	}

	public void paintCheckBoxMenuItemCheckIcon(SynthContext context, Graphics g, int state, int x,
			int y, int w, int h) {
		...
		ENGINE.paintCheck(g, context, Region.CHECK_BOX_MENU_ITEM, "check",
				...
				y + GTKIconFactory.CHECK_ICON_EXTRA_INSET, size, size);
	}

	public void paintRadioButtonMenuItemCheckIcon(SynthContext context, Graphics g, int state,
	...
	public void paintToolBarHandleIcon(SynthContext context, Graphics g, int state, int x, int y,
			int w, int h, Orientation orientation) {
		int gtkState = GTKLookAndFeel.synthStateToGTKState(context.getRegion(), state);
		...
		ENGINE.paintHandle(g, context, Region.TOOL_BAR, gtkState, ShadowType.OUT, "handlebox", x, y,
				w, h, orientation);
	}

	public void paintAscendingSortIcon(SynthContext context, Graphics g, int state, int x, int y,
			int w, int h) {
		ENGINE.paintArrow(g, context, Region.TABLE, SynthConstants.ENABLED, ShadowType.IN,
				ArrowType.UP, "arrow", x, y, w, h);
	}

	public void paintDescendingSortIcon(SynthContext context, Graphics g, int state, int x, int y,
			int w, int h) {
		ENGINE.paintArrow(g, context, Region.TABLE, SynthConstants.ENABLED, ShadowType.IN,
				ArrowType.DOWN, "arrow", x, y, w, h);
	}
	 ...
	 */
	private void fillArea(SynthContext context, Graphics g, int x, int y, int w, int h,
			ColorType colorType) {
	...
	static class TitledBorder extends AbstractBorder implements UIResource {
		...
		public void paintBorder(Component c, Graphics g, int x, int y, int w, int h) {
			...
			synchronized (UNIXToolkit.GTK_LOCK) {
				if (!ENGINE.paintCachedImage(g, x, y, w, h, id)) {
					ENGINE.startPainting(g, x, y, w, h, id);
					ENGINE.paintShadow(g, context, id, gtkState, ShadowType.ETCHED_IN, "frame", x,
							y, w, h);
					ENGINE.finishPainting();
				}
			}
