...
class GTKFileChooserUI extends SynthFileChooserUI {
	...
	// The accessoryPanel is a container to place the JFileChooser accessory
	// component
	...
	public String getFileName() {
		...
		String typedInName = fileNameTextField != null ? fileNameTextField.getText() : null;
		...
		JList list = mode == JFileChooser.DIRECTORIES_ONLY ? directoryList : fileList;
	...
	protected void doFileSelectionModeChanged(PropertyChangeEvent e) {
		directoryList.clearSelection();
		rightPanel.setVisible(
				((Integer) e.getNewValue()).intValue() != JFileChooser.DIRECTORIES_ONLY);
	...
	public void installComponents(JFileChooser fc) {
		...
		JPanel comboBoxPanel = new JPanel(new FlowLayout(FlowLayout.CENTER, 0, 0) {
		...
		JLabel directoryListLabel = (JLabel) headerRenderer.getTableCellRendererComponent(null,
				foldersLabelText, false, false, 0, 0);
		directoryListLabel.setName("GTKFileChooser.directoryListLabel");
		leftPanel.add(directoryListLabel,
				new GridBagConstraints(0, 0, 1, 1, 1, 0, GridBagConstraints.WEST,
						GridBagConstraints.HORIZONTAL, new Insets(0, 0, 0, 0), 0, 0));
		leftPanel.add(createDirectoryList(), new GridBagConstraints(0, 1, 1, 1, 1, 1,
		...
		JLabel fileListLabel = (JLabel) headerRenderer.getTableCellRendererComponent(null,
				filesLabelText, false, false, 0, 0);
		fileListLabel.setName("GTKFileChooser.fileListLabel");
		rightPanel.add(fileListLabel,
				new GridBagConstraints(0, 0, 1, 1, 1, 0, GridBagConstraints.WEST,
						GridBagConstraints.HORIZONTAL, new Insets(0, 0, 0, 0), 0, 0));
		rightPanel.add(createFilesList(), new GridBagConstraints(0, 1, 1, 1, 1, 1,
				GridBagConstraints.EAST, GridBagConstraints.BOTH, new Insets(0, 0, 0, 0), 0, 0));
		...
		JPanel pathFieldPanel = new JPanel(new FlowLayout(FlowLayout.LEADING, 0, 0));
		...
		Set<AWTKeyStroke> forwardTraversalKeys = fileNameTextField
		...
		fileNameTextField.setFocusTraversalKeys(KeyboardFocusManager.FORWARD_TRAVERSAL_KEYS,
				forwardTraversalKeys);

		fileNameTextField.setName("GTKFileChooser.fileNameTextField");
		fileNameTextField.getActionMap().put("fileNameCompletionAction",
				getFileNameCompletionAction());
		fileNameTextField.getInputMap().put(KeyStroke.getKeyStroke(KeyEvent.VK_TAB, 0),
				"fileNameCompletionAction");
	...
	protected void installDefaults(JFileChooser fc) {
		...
		showDirectoryIcons = Boolean.TRUE
				.equals(fc.getClientProperty("GTKFileChooser.showDirectoryIcons"));
		showFileIcons = Boolean.TRUE.equals(fc.getClientProperty("GTKFileChooser.showFileIcons"));
	}
	...
	protected void installStrings(JFileChooser fc) {
		...
		newFolderNoDirectoryErrorTitleText = UIManager
				.getString("FileChooser.newFolderNoDirectoryErrorTitleText", l);
		newFolderNoDirectoryErrorText = UIManager
				.getString("FileChooser.newFolderNoDirectoryErrorText", l);
		...
		newFolderButtonToolTipText = UIManager.getString("FileChooser.newFolderButtonToolTipText",
				l);
		deleteFileButtonToolTipText = UIManager.getString("FileChooser.deleteFileButtonToolTipText",
				l);
		renameFileButtonToolTipText = UIManager.getString("FileChooser.renameFileButtonToolTipText",
				l);
	...
	protected JScrollPane createDirectoryList() {
		...
		directoryList.putClientProperty(AccessibleContext.ACCESSIBLE_NAME_PROPERTY,
				foldersLabelText);
		align(directoryList);
	...
	protected class GTKDirectoryListModel extends AbstractListModel implements ListDataListener {
		...
		public Object getElementAt(int index) {
			return index > 0 ? getModel().getDirectories().elementAt(index - 1) : curDir;
		}
		...
		private void directoryChanged() {
			curDir = getFileChooser().getFileSystemView()
					.createFileObject(getFileChooser().getCurrentDirectory(), ".");
		}
	}
	...
	protected class DirectoryComboBoxModel extends AbstractListModel implements ComboBoxModel {
		...
		private void addItem(File directory) {
			...
			try {
				canonical = fsv
						.createFileObject(ShellFolder.getNormalizedFile(directory).getPath());
			} catch (IOException e) {
	...
	private class NewFolderAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			JFileChooser fc = getFileChooser();
			File currentDirectory = fc.getCurrentDirectory();
			String dirName = JOptionPane.showInputDialog(fc, newFolderDialogText,
					newFolderButtonText, JOptionPane.PLAIN_MESSAGE);
			...
			if (dirName != null) {
				...
				if (newDir == null || !newDir.mkdir()) {
					JOptionPane.showMessageDialog(fc,
							newFolderErrorText + newFolderErrorSeparator + " \"" + dirName + "\"",
	...
	private class RenameFileAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			JFileChooser fc = getFileChooser();
			File currentDirectory = fc.getCurrentDirectory();
			String newFileName = (String) JOptionPane.showInputDialog(fc,
					new MessageFormat(renameFileDialogText).format(new Object[] { getFileName() }),
					renameFileButtonText, JOptionPane.PLAIN_MESSAGE, null, null, getFileName());

			if (newFileName != null) {
				File oldFile = fc.getFileSystemView().createFileObject(currentDirectory,
						getFileName());
				File newFile = fc.getFileSystemView().createFileObject(currentDirectory,
						newFileName);
