...
class GTKStyle extends SynthStyle implements GTKConstants {
	...
	private static native int nativeGetColorForState(int widgetType, int state, int typeID);

	private static native Object nativeGetClassValue(int widgetType, String key);
	...
	static final Font DEFAULT_FONT = new FontUIResource("sansserif", Font.PLAIN, 10);
	...
	/**
	 * Backing style properties that are used if the style does not defined the
	 * property.
	 */
	private static final Map<String, GTKStockIcon> ICONS_MAP;

	/**
	 * The font used for this particular style, as determined at construction
	 * time.
	 */
	...
	public void installDefaults(SynthContext context) {
		...
		if (!context.getRegion().isSubregion()) {
			context.getComponent().putClientProperty(SwingUtilities2.AA_TEXT_PROPERTY_KEY,
	...
	/**
	 * Returns a <code>SynthPainter</code> that will route the appropriate calls
	 * to a <code>GTKEngine</code>.
	 *
	 * @param state
	 *            SynthContext identifying requestor
	 * @return SynthPainter
	 */
	@Override
	...
	protected Color getColorForState(SynthContext context, ColorType type) {
		...
		if (type == ColorType.TEXT_FOREGROUND && (id == Region.BUTTON || id == Region.CHECK_BOX
				|| id == Region.CHECK_BOX_MENU_ITEM || id == Region.MENU || id == Region.MENU_ITEM
				|| id == Region.RADIO_BUTTON || id == Region.RADIO_BUTTON_MENU_ITEM
				|| id == Region.TABBED_PANE_TAB || id == Region.TOGGLE_BUTTON
				|| id == Region.TOOL_TIP || id == Region.MENU_ITEM_ACCELERATOR
				|| id == Region.TABBED_PANE_TAB)) {
			type = ColorType.FOREGROUND;
		} else if (id == Region.TABLE || id == Region.LIST || id == Region.TREE
				|| id == Region.TREE_CELL) {
	...
	/**
	 * Returns color specific to the current style. This method is invoked when
	 * other variants don't fit.
	 */
	...
	public Insets getInsets(SynthContext state, Insets insets) {
		...
		if (id == Region.ARROW_BUTTON || id == Region.BUTTON || id == Region.TOGGLE_BUTTON) {
		...
		} else if (id == Region.MENU || id == Region.MENU_ITEM || id == Region.CHECK_BOX_MENU_ITEM
		...
		} else if (id == Region.PROGRESS_BAR || id == Region.SLIDER || id == Region.TABBED_PANE
				|| id == Region.TABBED_PANE_CONTENT || id == Region.TOOL_BAR
				|| id == Region.TOOL_BAR_DRAG_WINDOW || id == Region.TOOL_TIP) {
			...
			return getTextFieldInsets(state, insets);
		} else if (id == Region.SEPARATOR || id == Region.POPUP_MENU_SEPARATOR
	...
	private Insets getButtonInsets(SynthContext context, Insets insets) {
		...
		if ((component instanceof JButton) && !(component.getParent() instanceof JToolBar)
			...
			Insets defaultInsets = getClassSpecificInsetsValue(context, "default-border",
	 ...
	 * WidgetType. This method is useful in those cases where we need to fetch a
	 * value for a Region that is not associated with the component currently in
	 * use (e.g. we need to figure out the insets for a SCROLL_BAR, but certain
	 * values can only be extracted from a SCROLL_PANE region).
	 *
	 * @param wt
	 *            WidgetType for which to fetch the value
	 * @param key
	 *            Key identifying class specific value
	...
	/**
	 * Convenience method to get a class specific integer value for a particular
	 * WidgetType.
	 *
	 * @param wt
	 *            WidgetType for which to fetch the value
	 ...
	 *            Key identifying class specific value
	 * @param defaultValue
	 *            Returned if there is no value for the specified type
	 * @return Value, or defaultValue if <code>key</code> is not defined
	 */
	private static int getClassSpecificIntValue(WidgetType wt, String key, int defaultValue) {
	 ...
	 * A Style should NOT assume the opacity will remain this value, the
	...
	public boolean isOpaque(SynthContext context) {
		...
		if (region == Region.COMBO_BOX || region == Region.DESKTOP_PANE
				|| region == Region.DESKTOP_ICON || region == Region.EDITOR_PANE
				|| region == Region.FORMATTED_TEXT_FIELD || region == Region.INTERNAL_FRAME
				|| region == Region.LIST || region == Region.MENU_BAR || region == Region.PANEL
				|| region == Region.PASSWORD_FIELD || region == Region.POPUP_MENU
				|| region == Region.PROGRESS_BAR || region == Region.ROOT_PANE
				|| region == Region.SCROLL_PANE || region == Region.SPINNER
				|| region == Region.SPLIT_PANE_DIVIDER || region == Region.TABLE
				|| region == Region.TEXT_AREA || region == Region.TEXT_FIELD
				|| region == Region.TEXT_PANE || region == Region.TOOL_BAR_DRAG_WINDOW
				|| region == Region.TOOL_TIP || region == Region.TREE
	...
	public Object get(SynthContext context, Object key) {
		...
		if (key == "ScrollPane.viewportBorderInsets") {
			...
			return horiz ? new DimensionUIResource(stepperSize, sliderWidth)
			...
			if (name != null && name.startsWith("Spinner")) {
				...
				int arrowSize = (pangoFontName != null) ? PangoFonts.getFontSize(pangoFontName)
						: 10;
	 ...
	 * The code is duplicate so that it can get at the package private classes
	 * in gtk.
	 */
