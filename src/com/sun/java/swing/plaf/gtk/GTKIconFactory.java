...
class GTKIconFactory {
	...
	private static Map<String, DelegatingIcon> iconsPool = Collections
			.synchronizedMap(new HashMap<String, DelegatingIcon>());

	private static DelegatingIcon getIcon(String methodName) {
		DelegatingIcon result = iconsPool.get(methodName);
		if (result == null) {
			if (methodName == TREE_COLLAPSED_ICON || methodName == TREE_EXPANDED_ICON) {
	...
	private static class DelegatingIcon extends SynthIcon implements UIResource {
		private static final Class[] PARAM_TYPES = new Class[] { SynthContext.class, Graphics.class,
		...
		public void paintIcon(SynthContext context, Graphics g, int x, int y, int w, int h) {
			if (context != null) {
				GTKPainter.INSTANCE.paintIcon(context, g, getMethod(), x, y, w, h);
			}
		}
		...
		int getIconDimension(SynthContext context) {
			...
			iconDimension = style.getClassSpecificIntValue(context, "indicator-size",
			...
			if (region == Region.CHECK_BOX || region == Region.RADIO_BUTTON) {
				iconDimension += 2 * style.getClassSpecificIntValue(context, "indicator-spacing",
	...
	private static class SynthExpanderIcon extends DelegatingIcon {
		...
		int getIconDimension(SynthContext context) {
			updateSizeIfNecessary(context);
			return (iconDimension == -1) ? DEFAULT_ICON_SIZE : iconDimension;
		}

		private void updateSizeIfNecessary(SynthContext context) {
			if (iconDimension == -1 && context != null) {
				iconDimension = context.getStyle().getInt(context, "Tree.expanderSize", 10);
			}
		}
	}
	...
	private static class ToolBarHandleIcon extends DelegatingIcon {
		private static final Class[] PARAM_TYPES = new Class[] { SynthContext.class, Graphics.class,
				int.class, int.class, int.class, int.class, int.class, Orientation.class, };
		...
		public void paintIcon(SynthContext context, Graphics g, int x, int y, int w, int h) {
			if (context != null) {
				...
				Orientation orientation = (toolbar.getOrientation() == JToolBar.HORIZONTAL
						? Orientation.HORIZONTAL : Orientation.VERTICAL);

				if (style == null) {
					style = SynthLookAndFeel.getStyleFactory().getStyle(context.getComponent(),
							GTKRegion.HANDLE_BOX);
				}
				context = new SynthContext(toolbar, GTKRegion.HANDLE_BOX, style,
		...
		public int getIconWidth(SynthContext context) {
			...
			if (((JToolBar) context.getComponent()).getOrientation() == JToolBar.HORIZONTAL) {
				return 10;
			} else {
	...
	private static class MenuArrowIcon extends DelegatingIcon {
		private static final Class[] PARAM_TYPES = new Class[] { SynthContext.class, Graphics.class,
				int.class, int.class, int.class, int.class, int.class, ArrowType.class, };
		...
		public void paintIcon(SynthContext context, Graphics g, int x, int y, int w, int h) {
			if (context != null) {
				...
				GTKPainter.INSTANCE.paintIcon(context, g, getMethod(), x, y, w, h, arrowDir);
			}
		}
	}
}
