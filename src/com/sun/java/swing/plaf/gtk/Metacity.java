...
class Metacity implements SynthConstants {
	...
	private static final String[] themeNames = { getUserTheme(), "blueprint", "Bluecurve", "Crux",
			"SwingFallbackTheme" };
	...
	protected Metacity(String themeName)
		...
		for (int i = 0; i < n; i++) {
			...
			if (name != null) {
				...
				gm.put("has_title", Boolean.valueOf(getBooleanAttr(node, "has_title", true)));
	...
	void paintButtonBackground(SynthContext context, Graphics g, int x, int y, int w, int h) {
		...
		if (buttonName == "InternalFrameTitlePane.menuButton") {
			...
			function = "minimize";
			int nButtons = ((jif.isIconifiable() ? 1 : 0) + (jif.isMaximizable() ? 1 : 0)
					+ (jif.isClosable() ? 1 : 0));
		...
		Node frame = getNode(frame_style_set, "frame", new String[] { "focus",
				(active ? "yes" : "no"), "state", (jif.isMaximum() ? "maximized" : "normal") });

		if (function != null && frame != null) {
			Node frame_style = getNode("frame_style",
					new String[] { "name", getStringAttr(frame, "style") });
			if (frame_style != null) {
				...
				if ((right_corner && getBoolean("rounded_top_right", false))
					...
					if (right_corner) {
						g.setClip(getRoundedClipShape(0, 0, w, h, 12, 12,
								RoundRectClipShape.TOP_RIGHT));
					} else {
						g.setClip(getRoundedClipShape(0, 0, w, h, 11, 11,
	...
	protected void drawButton(Node frame_style, String function, String state, Graphics g, int w,
			int h, JInternalFrame jif) {
	...
	void paintFrameBorder(SynthContext context, Graphics g, int x0, int y0, int width, int height) {
		...
		variables.put("mini_icon_width", (frameIcon != null) ? frameIcon.getIconWidth() : 0);
		variables.put("mini_icon_height", (frameIcon != null) ? frameIcon.getIconHeight() : 0);
		...
		// These don't seem to apply here, but the Galaxy theme uses them. Not
		// sure why.
			...
			if (frame != null) {
				...
				if (frame_style != null) {
					...
					if (roundTopLeft || roundTopRight || roundBottomLeft || roundBottomRight) {
						...
						g.setClip(
								getRoundedClipShape(0, 0, width, height, 12, 12,
										(roundTopLeft ? RoundRectClipShape.TOP_LEFT : 0)
												| (roundTopRight ? RoundRectClipShape.TOP_RIGHT : 0)
												| (roundBottomLeft ? RoundRectClipShape.BOTTOM_LEFT
														: 0)
												| (roundBottomRight
														? RoundRectClipShape.BOTTOM_RIGHT : 0)));
					}
					...
					if (!minimized) {
						drawPiece(frame_style, g, "entire_background", 0, 0, width, height, jif);
					}
					drawPiece(frame_style, g, "titlebar", 0, 0, width, titleHeight, jif);
					drawPiece(frame_style, g, "titlebar_middle", leftTitlebarEdge, topTitlebarEdge,
							width - leftTitlebarEdge - rightTitlebarEdge,
							titleHeight - topTitlebarEdge - bottomTitlebarEdge, jif);
					drawPiece(frame_style, g, "left_titlebar_edge", 0, 0, leftTitlebarEdge,
							titleHeight, jif);
					drawPiece(frame_style, g, "right_titlebar_edge", width - rightTitlebarEdge, 0,
							rightTitlebarEdge, titleHeight, jif);
					drawPiece(frame_style, g, "top_titlebar_edge", 0, 0, width, topTitlebarEdge,
							jif);
					drawPiece(frame_style, g, "bottom_titlebar_edge", 0,
							titleHeight - bottomTitlebarEdge, width, bottomTitlebarEdge, jif);
					drawPiece(frame_style, g, "title", titleRect.x, titleRect.y, titleRect.width,
							titleRect.height, jif);
					if (!minimized) {
						drawPiece(frame_style, g, "left_edge", 0, titleHeight, insets.left,
								height - titleHeight, jif);
						drawPiece(frame_style, g, "right_edge", width - insets.right, titleHeight,
								insets.right, height - titleHeight, jif);
						drawPiece(frame_style, g, "bottom_edge", 0, height - insets.bottom, width,
								insets.bottom, jif);
						drawPiece(frame_style, g, "overlay", 0, 0, width, height, jif);
					}
					g.setClip(oldClip);
				}
			}
	...
	private static class Privileged implements PrivilegedAction<Object> {
		...
		public Object run() {
			if (type == GET_THEME_DIR) {
				...
				String[] dirs = new String[] { userHome + sep + ".themes",
						System.getProperty("swing.metacitythemedir"), "/usr/X11R6/share/themes",
						"/usr/X11R6/share/gnome/themes", "/usr/local/share/themes",
						"/usr/local/share/gnome/themes", "/usr/share/themes",
				...
				try {
					...
					// Note: this is a small file (< 1024 bytes) so it's not
					// worth
	...
	protected void tileImage(Graphics g, Image image, int x0, int y0, int w, int h,
			float[] alphas) {
	...
	private class ColorizeImageFilter extends RGBImageFilter {
		...
		public int filterRGB(int x, int y, int rgb) {
			...
			return ((rgb & 0xff000000) + (((int) (r * 255)) << 16) + (((int) (g * 255)) << 8)
					+ (int) (b * 255));
		}
	}
	...
	protected class TitlePaneLayout implements LayoutManager {
		...
		public Dimension minimumLayoutSize(Container c) {
			...
			// Calculate width.
			int width = getInt("left_titlebar_edge") + buttonDim.width
					+ getInt("right_titlebar_edge");
		...
		public void layoutContainer(Container c) {
			...
			JComponent menuButton = findChild(titlePane, "InternalFrameTitlePane.menuButton");
			JComponent minimizeButton = findChild(titlePane,
					"InternalFrameTitlePane.iconifyButton");
			JComponent maximizeButton = findChild(titlePane,
	...
	protected Rectangle calculateTitleArea(JInternalFrame jif) {
		...
		if (titlePane.getParent().getComponentOrientation().isLeftToRight()) {
			...
			r.width = titlePane.getWidth() - r.x - getInt("right_titlebar_edge") - buttonDim.width;
			if (button_border != null) {
	...
	protected int calculateTitleTextWidth(Graphics g, JInternalFrame jif) {
		...
		if (title != null) {
			...
			return Math.min(
					SwingUtilities2.stringWidth(jif, SwingUtilities2.getFontMetrics(jif, g), title),
					r.width);
		}
		return 0;
	}
	...
	protected void drawGTKArrow(Node node, Graphics g) {
		...
		GTKPainter.INSTANCE.paintMetacityElement(context, g, state, "metacity-arrow", x, y, w, h,
				shadowType, direction);
	}
	...
	protected void drawGTKBox(Node node, Graphics g) {
		...
		GTKPainter.INSTANCE.paintMetacityElement(context, g, state, "metacity-box", x, y, w, h,
				shadowType, null);
	}
	...
	protected void drawGTKVLine(Node node, Graphics g) {
		...
		GTKPainter.INSTANCE.paintMetacityElement(context, g, state, "metacity-vline", x, y1, 1,
				y2 - y1, null, null);
	}
		...
		if (g instanceof Graphics2D) {
			...
			for (int i = 0; i < n; i++) {
				g2.setPaint(new GradientPaint(x + (horizontal ? (i * w / n) : 0),
						y + (vertical ? (i * h / n) : 0), colors[i],
						x + (horizontal ? ((i + 1) * w / n) : 0),
						y + (vertical ? ((i + 1) * h / n) : 0), colors[i + 1]));
				g2.fillRect(x + (horizontal ? (i * w / n) : 0), y + (vertical ? (i * h / n) : 0),
						(horizontal ? (w / n) : w), (vertical ? (h / n) : h));
			}
			g2.setComposite(oldComp);
		}
	}
	...
	protected void drawPiece(Node frame_style, Graphics g, String position, int x, int y, int width,
			int height, JInternalFrame jif) {
	...
	private void updateFrameGeometry(SynthContext context) {
		...
		if (frame_style_set != null) {
			Node frame = getNode(frame_style_set, "frame",
					new String[] { "focus", (jif.isSelected() ? "yes" : "no"), "state",
							(jif.isMaximum() ? "maximized" : "normal") });
	...
	protected Node getNode(Node parent, String name, String[] attrs) {
		...
		if (node == null) {
			...
			if (inheritFrom != null) {
				Node inheritFromNode = getNode(parent.getParentNode(), parent.getNodeName(),
	...
	protected String getStringAttr(Node node, String name) {
		...
		if (attrs != null) {
			...
			if (value == null) {
				...
				if (inheritFrom != null) {
					Node inheritFromNode = getNode(node.getParentNode(), node.getNodeName(),
							new String[] { "name", inheritFrom });
					if (inheritFromNode != null) {
	...
	class ArithmeticExpressionEvaluator {
		...
		public float expression() {
			...
			while (!done && tokenizer.hasMoreTokens()) {
				...
				if ("+".equals(next) || "-".equals(next) || "`max`".equals(next)
						|| "`min`".equals(next)) {
					tokenizer.nextToken();
	...
	static class PeekableStringTokenizer extends StringTokenizer {
		...
		public String nextToken() {
			if (token != null) {
				...
				while ((token.equals(" ") || token.equals("\t")) && hasMoreTokens()) {
