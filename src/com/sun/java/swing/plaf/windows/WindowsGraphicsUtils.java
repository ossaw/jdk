...
public class WindowsGraphicsUtils {
    ...
    public static void paintText(Graphics g, AbstractButton b,
        ...
        if (xp != null && !(b instanceof JMenuItem)) {
            paintXPText(b, g, textRect.x + textShiftOffset, textRect.y + fm
                    .getAscent() + textShiftOffset, text, mnemIndex);
        } else {
            paintClassicText(b, g, textRect.x + textShiftOffset, textRect.y + fm
                    .getAscent() + textShiftOffset, text, mnemIndex);
        }
    }
    ...
    static void paintClassicText(AbstractButton b, Graphics g, int x, int y,
        ...
        if (model.isEnabled()) {
            ...
            if (!(b instanceof JMenuItem && model.isArmed())
                    && !(b instanceof JMenu && (model.isSelected() || model
                            .isRollover()))) {
                /*
            ...
            SwingUtilities2.drawStringUnderlineCharAt(b, g, text, mnemIndex, x,
                    y);
    ...
    static void paintXPText(AbstractButton b, Part part, State state,
        ...
        if (textColor instanceof UIResource) {
            textColor = xp.getColor(b, part, state, Prop.TEXTCOLOR, b
                    .getForeground());
            ...
            TypeEnum shadowType = xp.getTypeEnum(b, part, state,
                    Prop.TEXTSHADOWTYPE);
    ...
    static void repaintMnemonicsInContainer(Container cont) {
        ...
        for (int i = 0; i < cont.getComponentCount(); i++) {
            ...
            if (c instanceof AbstractButton && ((AbstractButton) c)
                    .getMnemonic() != '\0') {
                c.repaint();
                continue;
            } else if (c instanceof JLabel && ((JLabel) c)
                    .getDisplayedMnemonic() != '\0') {
                c.repaint();
                continue;
            }
