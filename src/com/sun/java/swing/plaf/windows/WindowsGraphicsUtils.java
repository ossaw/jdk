...
public class WindowsGraphicsUtils {
	...
	public static void paintText(Graphics g, AbstractButton b, Rectangle textRect, String text,
		...
		if (xp != null && !(b instanceof JMenuItem)) {
			paintXPText(b, g, textRect.x + textShiftOffset,
					textRect.y + fm.getAscent() + textShiftOffset, text, mnemIndex);
		} else {
	...
	static void paintClassicText(AbstractButton b, Graphics g, int x, int y, String text,
			int mnemIndex) {
	...
	static void paintXPText(AbstractButton b, Graphics g, int x, int y, String text,
			int mnemIndex) {
	...
	static void paintXPText(AbstractButton b, Part part, State state, Graphics g, int x, int y,
		...
		if (textColor instanceof UIResource) {
			...
			if (part == Part.TP_BUTTON && state == State.DISABLED) {
				Color enabledColor = xp.getColor(b, part, State.NORMAL, Prop.TEXTCOLOR,
						b.getForeground());
				if (textColor.equals(enabledColor)) {
					textColor = xp.getColor(b, Part.BP_PUSHBUTTON, state, Prop.TEXTCOLOR,
							textColor);
				}
			}
			...
			if (shadowType == TypeEnum.TST_SINGLE || shadowType == TypeEnum.TST_CONTINUOUS) {
				...
				if (offset != null) {
					...
					SwingUtilities2.drawStringUnderlineCharAt(b, g, text, mnemIndex, x + offset.x,
							y + offset.y);
				}
			}
		}
