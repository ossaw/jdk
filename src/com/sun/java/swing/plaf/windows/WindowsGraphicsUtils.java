...
public class WindowsGraphicsUtils {
     ...
     * All the overriden paintText methods of the ButtonUI delegates will call
     * this static method.
     * <p>
     * 
     * @param g
    ...
    public static void paintText(Graphics g, AbstractButton b, Rectangle textRect, String text,
        ...
        if (xp != null && !(b instanceof JMenuItem)) {
            paintXPText(b, g, textRect.x + textShiftOffset, textRect.y + fm.getAscent() + textShiftOffset,
                    text, mnemIndex);
        } else {
            paintClassicText(b, g, textRect.x + textShiftOffset, textRect.y + fm.getAscent()
                    + textShiftOffset, text, mnemIndex);
        }
    }
    ...
    static void paintClassicText(AbstractButton b, Graphics g, int x, int y, String text, int mnemIndex) {
        ...
        if (model.isEnabled()) {
            ...
            if (!(b instanceof JMenuItem && model.isArmed()) && !(b instanceof JMenu && (model.isSelected()
                    || model.isRollover()))) {
                /*
                 * We shall not set foreground color for selected menu or armed
                 * menuitem. Foreground must be set in appropriate Windows*
                 * class because these colors passes from BasicMenuItemUI as
                 * protected fields and we can't reach them from this class
                 */
    ...
    static void paintXPText(AbstractButton b, Part part, State state, Graphics g, int x, int y, String text,
            int mnemIndex) {
        XPStyle xp = XPStyle.getXP();
        if (xp == null) {
            return;
        ...
        if (textColor instanceof UIResource) {
            ...
            if (part == Part.TP_BUTTON && state == State.DISABLED) {
                Color enabledColor = xp.getColor(b, part, State.NORMAL, Prop.TEXTCOLOR, b.getForeground());
                if (textColor.equals(enabledColor)) {
                    textColor = xp.getColor(b, Part.BP_PUSHBUTTON, state, Prop.TEXTCOLOR, textColor);
                }
            }
            // only draw shadow if developer hasn't changed the foreground color
            // and if the current style has text shadows.
            TypeEnum shadowType = xp.getTypeEnum(b, part, state, Prop.TEXTSHADOWTYPE);
            if (shadowType == TypeEnum.TST_SINGLE || shadowType == TypeEnum.TST_CONTINUOUS) {
                Color shadowColor = xp.getColor(b, part, state, Prop.TEXTSHADOWCOLOR, Color.black);
                Point offset = xp.getPoint(b, part, state, Prop.TEXTSHADOWOFFSET);
                if (offset != null) {
                    g.setColor(shadowColor);
                    SwingUtilities2.drawStringUnderlineCharAt(b, g, text, mnemIndex, x + offset.x, y
                            + offset.y);
                }
            }
        }
    ...
    /*
     * Repaints all the components with the mnemonics in container. Recursively
     * searches for all the subcomponents.
     */
