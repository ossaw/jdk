...
public class WindowsInternalFrameTitlePane extends BasicInternalFrameTitlePane {
	...
	public void paintComponent(Graphics g) {
		...
		if (title != null) {
			...
			if (xp != null) {
				...
				if (isSelected) {
					shadowType = xp.getString(this, Part.WP_CAPTION, State.ACTIVE,
							Prop.TEXTSHADOWTYPE);
				}
				if ("single".equalsIgnoreCase(shadowType)) {
					...
					if (shadowOffset != null && shadowColor != null) {
						...
						SwingUtilities2.drawString(frame, g, title, titleX + shadowOffset.x,
	...
	public Dimension getMinimumSize() {
		...
		if (xp != null) {
			...
			// the captionbarheight is 25 but native caption is 30 (maximized
			// 26)
			if (frame.isMaximum()) {
				d.height -= 1;
			} else {
	...
	public class WindowsTitlePaneLayout extends BasicInternalFrameTitlePane.TitlePaneLayout {
		...
		public void layoutContainer(Container c) {
			...
			if (frame.isClosable()) {
				x = layoutButton(closeButton, Part.WP_CLOSEBUTTON, x, y, buttonWidth, buttonHeight,
						2, !leftToRight);
			}

			if (frame.isMaximizable()) {
				x = layoutButton(maxButton, Part.WP_MAXBUTTON, x, y, buttonWidth, buttonHeight,
						(xp != null) ? 2 : 0, !leftToRight);
			}

			if (frame.isIconifiable()) {
				layoutButton(iconButton, Part.WP_MINBUTTON, x, y, buttonWidth, buttonHeight, 0,
						!leftToRight);
			}
		}
	...
	/**
	 * A versatile Icon implementation which can take an array of Icon instances
	 * (typically <code>ImageIcon</code>s) and choose one that gives the best
	 * quality for a given Graphics2D scale factor when painting.
	 * <p>
	 * The class is public so it can be instantiated by
	 * UIDefaults.ProxyLazyValue.
	 * <p>
	 * Note: We assume here that icons are square.
	 */
	public static class ScalableIconUIResource implements Icon, UIResource {
		// We can use an arbitrary size here because we scale to it in
		// paintIcon()
		 ...
		 *         <p>
		 *         The constructor is public so it can be called by
		 *         UIDefaults.ProxyLazyValue.
		 */
