...
public class WindowsProgressBarUI extends BasicProgressBarUI {
    ...
    protected void installDefaults() {
        ...
        if (XPStyle.getXP() != null) {
            ...
            indeterminateInsets = UIManager.getInsets(
    ...
    public int getBaseline(JComponent c, int width, int height) {
        ...
        if (XPStyle.getXP() != null && progressBar.isStringPainted()
            ...
            baseline = y + (height + metrics.getAscent() - metrics.getLeading()
    ...
    protected Dimension getPreferredInnerHorizontal() {
        ...
        if (xp != null) {
            ...
            return new Dimension((int) super.getPreferredInnerHorizontal()
                    .getWidth(), skin.getHeight());
        }
    ...
    protected Dimension getPreferredInnerVertical() {
        ...
        if (xp != null) {
            ...
            return new Dimension(skin.getWidth(),
    ...
    protected void paintDeterminate(Graphics g, JComponent c) {
        ...
        if (xp != null) {
            ...
            if (progressBar.isStringPainted()) {
                // Do not paint the standard stripes from the skin, because they
                // obscure
                // the text
                ...
                g2.setStroke(new BasicStroke((float) (vertical ? barRectWidth
                        : barRectHeight), BasicStroke.CAP_BUTT,
                        BasicStroke.JOIN_BEVEL));
                if (!vertical) {
                    if (isLeftToRight) {
                        g2.drawLine(2, barRectHeight / 2 + 1, amountFull - 2,
                                barRectHeight / 2 + 1);
                    } else {
                        g2.drawLine(2 + barRectWidth, barRectHeight / 2 + 1, 2
                                + barRectWidth - (amountFull - 2), barRectHeight
                                        / 2 + 1);
                    }
                    paintString(g, 0, 0, barRectWidth, barRectHeight,
                            amountFull, null);
                } else {
                    g2.drawLine(barRectWidth / 2 + 1, barRectHeight + 1,
                            barRectWidth / 2 + 1, barRectHeight + 1 - amountFull
                                    + 2);
                    paintString(g, 2, 2, barRectWidth, barRectHeight,
                            amountFull, null);
                }

            } else {
                Skin skin = xp.getSkin(progressBar, vertical ? Part.PP_CHUNKVERT
                        : Part.PP_CHUNK);
                int thickness;
                if (vertical) {
                ...
                if (spaceSize > 0 && (nChunks * (chunkSize + spaceSize)
                        + chunkSize) < (amountFull - 4)) {
                    nChunks++;
                }

                for (int i = 0; i < nChunks; i++) {
                    if (vertical) {
                        skin.paintSkin(g, 3, barRectHeight - i * (chunkSize
                                + spaceSize) - chunkSize - 2, thickness,
                                chunkSize, null);
                    } else {
                        if (isLeftToRight) {
                            skin.paintSkin(g, 4 + i * (chunkSize + spaceSize),
                                    2, chunkSize, thickness, null);
                        } else {
                            skin.paintSkin(g, barRectWidth - (2 + (i + 1)
                                    * (chunkSize + spaceSize)), 2, chunkSize,
                                    thickness, null);
                        }
                    }
                }
    ...
    protected void paintIndeterminate(Graphics g, JComponent c) {
        ...
        if (xp != null) {
            ...
            if (boxRect != null) {
                ...
                if (progressBar.isStringPainted()) {
                    if (!vertical) {
                        paintString(g, -1, -1, barRectWidth, barRectHeight, 0,
                                null);
                    } else {
                        paintString(g, 1, 1, barRectWidth, barRectHeight, 0,
                                null);
    ...
    private Rectangle getFullChunkBounds(Rectangle box) {
        boolean vertical = (progressBar
                .getOrientation() == JProgressBar.VERTICAL);
        XPStyle xp = XPStyle.getXP();
        int gap = (xp != null) ? xp.getInt(progressBar, Part.PP_PROGRESS, null,
                Prop.PROGRESSSPACESIZE, 0) : 0;

        if (!vertical) {
            int chunksize = box.width + gap;
            return new Rectangle(box.x - chunksize * 2, box.y, chunksize * 3,
                    box.height);
        } else {
            int chunksize = box.height + gap;
            return new Rectangle(box.x, box.y - chunksize * 2, box.width,
                    chunksize * 3);
        }
    }

    private void paintIndeterminateFrame(Rectangle box, Graphics2D g,
            boolean vertical, int bgwidth, int bgheight) {
        XPStyle xp = XPStyle.getXP();
        if (xp == null) {
            return;
        }
        ...
        Rectangle progbarExtents = new Rectangle(ins.left, ins.top, bgwidth
                - ins.left - ins.right, bgheight - ins.top - ins.bottom);

        // only paint where the chunks overlap with the progress bar drawing
        // area
