...
public class WindowsProgressBarUI extends BasicProgressBarUI {
	...
	public int getBaseline(JComponent c, int width, int height) {
		...
		if (XPStyle.getXP() != null && progressBar.isStringPainted()
			...
			baseline = y
					+ (height + metrics.getAscent() - metrics.getLeading() - metrics.getDescent())
							/ 2;
		}
		return baseline;
	}
	...
	protected Dimension getPreferredInnerHorizontal() {
		...
		if (xp != null) {
			...
			return new Dimension((int) super.getPreferredInnerHorizontal().getWidth(),
					skin.getHeight());
		}
			...
			if (progressBar.isStringPainted()) {
				...
				if (!vertical) {
					if (isLeftToRight) {
						...
						g2.drawLine(2 + barRectWidth, barRectHeight / 2 + 1,
								2 + barRectWidth - (amountFull - 2), barRectHeight / 2 + 1);
					}
					paintString(g, 0, 0, barRectWidth, barRectHeight, amountFull, null);
				} else {
					g2.drawLine(barRectWidth / 2 + 1, barRectHeight + 1, barRectWidth / 2 + 1,
							barRectHeight + 1 - amountFull + 2);
				...
				if (spaceSize > 0
						&& (nChunks * (chunkSize + spaceSize) + chunkSize) < (amountFull - 4)) {
					nChunks++;
				}
				...
				for (int i = 0; i < nChunks; i++) {
					if (vertical) {
						skin.paintSkin(g, 3,
								barRectHeight - i * (chunkSize + spaceSize) - chunkSize - 2,
								thickness, chunkSize, null);
					} else {
						if (isLeftToRight) {
							skin.paintSkin(g, 4 + i * (chunkSize + spaceSize), 2, chunkSize,
									thickness, null);
						} else {
							skin.paintSkin(g,
	...
	protected Rectangle getBox(Rectangle r) {
		...
		if (xp != null) {
			...
			int gap = xp.getInt(progressBar, Part.PP_PROGRESS, null, Prop.PROGRESSSPACESIZE, 0);
	...
	protected void paintIndeterminate(Graphics g, JComponent c) {
		...
		if (xp != null) {
			...
			if (boxRect != null) {
				...
				paintIndeterminateFrame(boxRect, (Graphics2D) g, vertical, barRectWidth,
						barRectHeight);
				if (progressBar.isStringPainted()) {
					if (!vertical) {
	...
	private Rectangle getFullChunkBounds(Rectangle box) {
		...
		XPStyle xp = XPStyle.getXP();
		int gap = (xp != null)
				? xp.getInt(progressBar, Part.PP_PROGRESS, null, Prop.PROGRESSSPACESIZE, 0) : 0;
	...
	private void paintIndeterminateFrame(Rectangle box, Graphics2D g, boolean vertical, int bgwidth,
			int bgheight) {
		XPStyle xp = XPStyle.getXP();
		if (xp == null) {
			return;
		...
		Insets ins = indeterminateInsets;
		Rectangle progbarExtents = new Rectangle(ins.left, ins.top, bgwidth - ins.left - ins.right,
	...
	private void paintXPBackground(Graphics g, boolean vertical, int barRectWidth,
			int barRectHeight) {
		XPStyle xp = XPStyle.getXP();
		if (xp == null) {
			return;
