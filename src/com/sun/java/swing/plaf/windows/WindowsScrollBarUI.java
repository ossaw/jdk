...
public class WindowsScrollBarUI extends BasicScrollBarUI {
	...
	protected Dimension getMinimumThumbSize() {
		...
		return JScrollBar.HORIZONTAL == scrollbar.getOrientation() ? horizontalThumbSize
				: verticalThumbSize;
	}
	...
	protected JButton createDecreaseButton(int orientation) {
		return new WindowsArrowButton(orientation, UIManager.getColor("ScrollBar.thumb"),
	...
	protected void paintThumb(Graphics g, JComponent c, Rectangle thumbBounds) {
		...
		if (xp != null) {
			...
			if (gripperInsets == null
					|| (v && (thumbBounds.height - gripperInsets.top - gripperInsets.bottom >= skin
							.getHeight()))
					|| (!v && (thumbBounds.width - gripperInsets.left - gripperInsets.right >= skin
							.getWidth()))) {
				skin.paintSkin(g, thumbBounds.x + (thumbBounds.width - skin.getWidth()) / 2,
	...
	protected void setThumbRollover(boolean active) {
		...
		// we need to repaint the entire scrollbar because state change for
		// thumb
	...
	/**
	 * This should be pulled out into its own class if more classes need to use
	 * it.
	 * <p>
	 * Grid is used to draw the track for windows scrollbars. Grids are cached
	 * in a HashMap, with the key being the rgb components of the
	 * foreground/background colors. Further the Grid is held through a WeakRef
	 * so that it can be freed when no longer needed. As the Grid is rather
	 * expensive to draw, it is drawn in a BufferedImage.
	 */
	private static class Grid {
		...
		public Grid(Color fg, Color bg) {
			...
			image = new BufferedImage(BUFFER_SIZE, BUFFER_SIZE, BufferedImage.TYPE_BYTE_INDEXED,
					icm);
