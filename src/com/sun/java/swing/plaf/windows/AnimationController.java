...
/**
 * A class to help mimic Vista theme animations. The only kind of animation it
 * handles for now is 'transition' animation (this seems to be the only
 * animation which Vista theme can do). This is when one picture fadein over
 ...
 * animation and paints {@code Skin} in new {@code State} over the {@code Skin}
 * in last {@code State} using {@code AlphaComposite.SrcOver.derive(alpha)}
 * where {code alpha} depends on the state of animation
 * </ul>
 *
...
class AnimationController implements ActionListener, PropertyChangeListener {
		...
		if (oldState != newState) {
			...
			if (oldState != null) {
				...
				if (newState == State.DEFAULTED) {
					...
					duration = (xp != null)
							? xp.getThemeTransitionDuration(c, part, normalizeState(oldState),
									normalizeState(newState), Prop.TRANSITIONDURATIONS)
							: 1000;
				}
	...
	static void paintSkin(JComponent component, Skin skin, Graphics g, int dx, int dy, int dw,
			int dh, State state) {
