...
/**
 * The extended GSSContext interface for supporting additional functionalities
 * not defined by {@code org.ietf.jgss.GSSContext}, such as querying
...
public interface ExtendedGSSContext extends GSSContext {
     ...
     * <br>
     * <br>
     * For each supported attribute type, the type for the output are defined
     * below.
     * <ol>
     * <li>{@code KRB5_GET_TKT_FLAGS}: the returned object is a boolean array
     * for the service ticket flags, which is long enough to contain all true
     * bits. This means if the user wants to get the <em>n</em>'th bit but the
     * length of the returned array is less than <em>n</em>, it is regarded as
     * false.
     * <li>{@code KRB5_GET_SESSION_KEY}: the returned object is an instance of
     * {@link java.security.Key}, which has the following properties:
     * <ul>
     * <li>Algorithm: enctype as a string, where enctype is defined in RFC 3961,
     * section 8.
     * <li>Format: "RAW"
     * <li>Encoded form: the raw key bytes, not in any ASN.1 encoding
     * </ul>
     * <li>{@code KRB5_GET_AUTHZ_DATA}: the returned object is an array of
     ...
     * requested, the underlying context would use the delegation policy defined
     * by the environment as a hint to determine whether credentials delegation
     * should be performed. This request can only be made on the context
     ...
     * delegation.
     * <p>
     * When both this flag and the {@link GSSContext#requestCredDeleg(boolean)
     * credentials delegation flag} are true, delegation will be always tried.
     * However, if the delegation policy does not permit delegation, the value
     * of {@link #getDelegPolicyState} will be false, even if delegation is
     * performed successfully.
     * <p>
     * In any case, if the delegation is not successful, the value returned by
     * {@link GSSContext#getCredDelegState()} is false, and the value returned
     * by {@link #getDelegPolicyState()} is also false.
     * <p>
     * Not all mechanisms support delegation policy. Therefore, the application
     * should check to see if the request was honored with the
