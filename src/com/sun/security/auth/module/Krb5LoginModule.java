...
public class Krb5LoginModule implements LoginModule {
	...
	public void initialize(Subject subject, CallbackHandler callbackHandler,
		...
		if (debug) {
			System.out.print("Debug is  " + debug + " storeKey " + storeKey + " useTicketCache "
					+ useTicketCache + " useKeyTab " + useKeyTab + " doNotPrompt " + doNotPrompt
					+ " ticketCache is " + ticketCacheName + " isInitiator " + isInitiator
					+ " KeyTab is " + keyTabName + " refreshKrb5Config is " + refreshKrb5Config
					+ " principal is " + princName + " tryFirstPass is " + tryFirstPass
					+ " useFirstPass is " + useFirstPass + " storePass is " + storePass
	...
	public boolean login() throws LoginException {
		...
		if (tryFirstPass) {
			try {
				...
				cleanState();
				if (debug) {
					System.out.println("\t\t[Krb5LoginModule] " + "tryFirstPass failed with:"
							+ le.getMessage());
				}
			}
				...
				if (useKeyTab) {
					...
					if (isInitiator) {
						if (Krb5Util.keysFromJavaxKeyTab(ktab, principal).length == 0) {
							ktab = null;
							if (debug) {
								System.out.println("Key for the principal " + principal
										+ " not available in "
	...
	private void promptForName(boolean getPasswdFromSharedState) throws LoginException {
		...
		if (doNotPrompt) {
			...
			try {
				...
				throw new LoginException(uce.getMessage() + " not available to garner "
	...
	private void promptForPass(boolean getPasswdFromSharedState) throws LoginException {
		...
		if (doNotPrompt) {
			...
			try {
				...
				if (debug) {
					System.out.println(
							"\t\t[Krb5LoginModule] " + "user entered username: " + krb5PrincName);
					System.out.println();
				}
			} catch (java.io.IOException ioe) {
				throw new LoginException(ioe.getMessage());
	...
	private void validateConfiguration() throws LoginException {
		if (doNotPrompt && !useTicketCache && !useKeyTab && !tryFirstPass && !useFirstPass)
			throw new LoginException("Configuration Error" + " - either doNotPrompt should be "
					...
					+ " useKeyTab, tryFirstPass and useFirstPass" + " should be true");
		...
		if (keyTabName != null & !useKeyTab)
			throw new LoginException("Configuration Error - useKeyTab should be set to true "
		...
		if (succeeded == false) {
			...
			if (storeKey && encKeys != null) {
				...
				for (int i = 0; i < encKeys.length; i++) {
					...
					kerbKeys[i] = new KerberosKey(kerbClientPrinc, encKeys[i].getBytes(),
							encKeys[i].getEType(), (temp == null ? 0 : temp.intValue()));
				}
