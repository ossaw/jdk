...
/**
 * <p>
 * This <code>LoginModule</code> imports a user's Unix <code>Principal</code>
 * information (<code>UnixPrincipal</code>,
...
public class UnixLoginModule implements LoginModule {
		...
		if (ss == null) {
			...
			if (ss.getGroups() != null && ss.getGroups().length > 0) {
				...
				for (int i = 0; i < unixGroups.length; i++) {
					UnixNumericGroupPrincipal ngp = new UnixNumericGroupPrincipal(unixGroups[i],
							false);
	 ...
	 * associates the Unix Principals with the <code>Subject</code> currently
	...
	public boolean commit() throws LoginException {
		if (succeeded == false) {
			if (debug) {
				System.out.println(
						"\t\t[UnixLoginModule]: " + "did not add any Principals to Subject "
