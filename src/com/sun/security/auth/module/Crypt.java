...
class Crypt {
	...
	private static final byte[] FP = { 40, 8, 48, 16, 56, 24, 64, 32, 39, 7, 47,
	...
	private static final byte[] PC1_C = { 57, 49, 41, 33, 25, 17, 9, 1, 58, 50,
			42, 34, 26, 18, 10, 2, 59, 51, 43, 35, 27, 19, 11, 3, 60, 52, 44,
			36, };

	private static final byte[] PC1_D = { 63, 55, 47, 39, 31, 23, 15, 7, 62, 54,
			46, 38, 30, 22, 14, 6, 61, 53, 45, 37, 29, 21, 13, 5, 28, 20, 12,
			4, };

	private static final byte[] shifts = { 1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2,
			2, 2, 2, 1, };

	private static final byte[] PC2_C = { 14, 17, 11, 24, 1, 5, 3, 28, 15, 6,
			21, 10, 23, 19, 12, 4, 26, 8, 16, 7, 27, 20, 13, 2, };

	private static final byte[] PC2_D = { 41, 52, 31, 37, 47, 55, 30, 40, 51,
	...
	private static final byte[] e2 = { 32, 1, 2, 3, 4, 5, 4, 5, 6, 7, 8, 9, 8,
	...
	private static final byte[][] S = { { 14, 4, 13, 1, 2, 15, 11, 8, 3, 10, 6,
			...
			{ 15, 1, 8, 14, 6, 11, 3, 4, 9, 7, 2, 13, 12, 0, 5, 10, 3, 13, 4, 7,
					15, 2, 8, 14, 12, 0, 1, 10, 6, 9, 11, 5, 0, 14, 7, 11, 10,
					4, 13, 1, 5, 8, 12, 6, 9, 3, 2, 15, 13, 8, 10, 1, 3, 15, 4,
					2, 11, 6, 7, 12, 0, 5, 14, 9 },

			{ 10, 0, 9, 14, 6, 3, 15, 5, 1, 13, 12, 7, 11, 4, 2, 8, 13, 7, 0, 9,
					3, 4, 6, 10, 2, 8, 5, 14, 12, 11, 15, 1, 13, 6, 4, 9, 8, 15,
					3, 0, 11, 1, 2, 12, 5, 10, 14, 7, 1, 10, 13, 0, 6, 9, 8, 7,
					4, 15, 14, 3, 11, 5, 2, 12 },
	...
	private void encrypt(byte[] block, int fake) {
		...
		for (i = 0; i < 16; i++) {
			...
			for (j = 0; j < 8; j++) {
				t = 6 * j;
				k = S[j][(preS[t + 0] << 5) + (preS[t + 1] << 3) + (preS[t
						+ 2] << 2) + (preS[t + 3] << 1) + (preS[t + 4] << 0)
						+ (preS[t + 5] << 4)];
				t = 4 * j;
				f[t + 0] = (byte) ((k >> 3) & 01);
				f[t + 1] = (byte) ((k >> 2) & 01);
	...
	public static void main(String arg[]) {
		...
		Crypt c = new Crypt();
		try {
			byte result[] = c.crypt(arg[0].getBytes("ISO-8859-1"), arg[1]
					.getBytes("ISO-8859-1"));
