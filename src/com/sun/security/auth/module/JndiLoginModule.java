 ...
 *
 * <p>
 * This <code>LoginModule</code> interoperates with any conformant JNDI service
 * provider. To direct this <code>LoginModule</code> to use a specific JNDI
 * service provider, two options must be specified in the login
 ...
 * <code>LoginModule</code> can access the relevant user and group information.
 * Because this <code>LoginModule</code> only performs one-level searches to
 * find the relevant user information, the <code>URL</code> must point to a
 * directory one level above where the user and group information is stored in
 * the directory service. For example, to instruct this <code>LoginModule</code>
 ...
 * </pre>
 *
 * <b>NISServerHostName</b> specifies the server host name of the NIS server
 * (for example, <i>nis.sun.com</i>, and <b>NISDomain</b> specifies the domain
 * for that NIS server (for example, <i>jaas.sun.com</i>. To contact an LDAP
 * server, the following URLs must be specified:
 * 
 * <pre>
 ...
 * </pre>
 *
 * <b>LDAPServerHostName</b> specifies the server host name of the LDAP server,
 * which may include a port number (for example, <i>ldap.sun.com:389</i>), and
 * <b>LDAPName</b> specifies the entry name in the LDAP directory (for example,
 * <i>ou=People,o=Sun,c=US</i> and <i>ou=Groups,o=Sun,c=US</i> for user and
 * group information, respectively).
 *
 * <p>
 * The format in which the user's information must be stored in the directory
 * service is specified in RFC 2307. Specifically, this <code>LoginModule</code>
 * will search for the user's entry in the directory service using the user's
 * <i>uid</i> attribute, where <i>uid=<b>username</b></i>. If the search
 * succeeds, this <code>LoginModule</code> will then obtain the user's encrypted
 * password from the retrieved entry using the <i>userPassword</i> attribute.
 * This <code>LoginModule</code> assumes that the password is stored as a byte
 * array, which when converted to a <code>String</code>, has the following
 * format:
 * 
 * <pre>
 ...
 * </pre>
 *
 * The LDAP directory server must be configured to permit read access to the
 * userPassword attribute. If the user entered a valid username and password,
 * this <code>LoginModule</code> associates a <code>UnixPrincipal</code>,
 * <code>UnixNumericUserPrincipal</code>, and the relevant
 * UnixNumericGroupPrincipals with the <code>Subject</code>.
 *
 * <p>
...
public class JndiLoginModule implements LoginModule {
    ...
    public void initialize(Subject subject, CallbackHandler callbackHandler,
        ...
        strongDebug = "true".equalsIgnoreCase((String) options.get(
                "strongDebug"));
    ...
    /**
     * <p>
     * Prompt for username and password. Verify the password against the
     * relevant name service.
     *
     * <p>
     *
     ...
     * then this method associates a <code>UnixPrincipal</code> with the
    ...
    private void attemptAuthentication(boolean getPasswdFromSharedState)
        ...
        try {
             ...
             * (SearchControls.ONELEVEL_SCOPE, 0, 5000, new String[] { USER_PWD
             * }, false, false);
             */

            SearchControls controls = new SearchControls();
            NamingEnumeration<SearchResult> ne = ctx.search("", "(uid="
                    + username + ")", controls);
            if (ne.hasMore()) {
                ...
                // check the password
                if (verifyPassword(encryptedPassword, new String(
                        password)) == true) {
                ...
                // authentication succeeded
                if (storePass && !sharedState.containsKey(NAME) && !sharedState
                        .containsKey(PWD)) {
                ...
                if (debug && uidNumber != null) {
                    System.out.println("\t\t[JndiLoginModule] " + "user: '"
                            + username + "' has UID: " + uidNumber);
                }
                ...
                while (ne.hasMore()) {
                    ...
                    if (!gidNumber.equals(suppGid)) {
                        UnixNumericGroupPrincipal suppPrincipal = new UnixNumericGroupPrincipal(
                                suppGid, false);
                        supplementaryGroups.add(suppPrincipal);
                        if (debug && suppGid != null) {
                            System.out.println("\t\t[JndiLoginModule] "
                                    + "user: '" + username
                                    + "' has Supplementary Group: " + suppGid);
                        }
                    }
                }
    ...
    private void getUsernamePassword(boolean getPasswdFromSharedState)
        ...
        callbacks[0] = new NameCallback(protocol + " " + rb.getString(
                "username."));
        callbacks[1] = new PasswordCallback(protocol + " " + rb.getString(
                "password."), false);
