 ...
 * optional. If they are not present, then any signer (including unsigned code)
 * will match, and any codeBase will match. Note that the <code>principal</code>
 * name/value pair is not optional. This <code>Policy</code> implementation only
 * permits Principal-based grant entries. Note that the <i>principalClass</i>
 * may be set to the wildcard value, *, which allows it to match any
 * <code>Principal</code> class. In addition, the <i>principalName</i> may also
 ...
 * <code>PrivateCredentialPermission</code>, and the
 * "Principal Class/Principal Name" for that
 * <code>PrivateCredentialPermission</code> is "self", then the entry grants the
 * specified <code>Subject</code> permission to access its own private
 * Credential. For example, the following grants the <code>Subject</code> "Duke"
 ...
 * </pre>
 *
 * The following grants the <code>Subject</code> "Duke" access to all of its own
 * private Credentials:
 *
 * <pre>
 *   grant principal foo.com.Principal "Duke" {
 ...
 * </pre>
 *
 * The following grants all Subjects permission to access their own private
 * Credentials:
 *
 * <pre>
 *   grant principal * * {
