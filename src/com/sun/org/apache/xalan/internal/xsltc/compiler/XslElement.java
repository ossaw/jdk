...
final class XslElement extends Instruction {
    ...
    public void parseContents(Parser parser) {
        ...
        if (name == EMPTYSTRING) {
            ErrorMsg msg = new ErrorMsg(ErrorMsg.ILLEGAL_ELEM_NAME_ERR, name,
                    this);
     ...
     * time. In this case, there is no need to inspect the element name at
     * runtime to determine if a prefix exists, needs to be generated, etc.
     */
    public void translateLiteral(ClassGenerator classGen,
     ...
     * (iii) generates a new prefix and create a new qname when necessary (iv)
     * calls startElement() on the handler (v) looks up a uri in the XML when
     * the prefix is not known at compile time (vi) calls namespace() on the
     * handler (vii) evaluates the contents (viii) calls endElement().
     */
        ...
        if (!_ignore) {
            ...
            il.append(new INVOKESTATIC(cpg.addMethodref(BASIS_LIBRARY_CLASS,
                    "startXslElement", "(" + STRING_SIG + STRING_SIG
                            + TRANSLET_OUTPUT_SIG + DOM_INTF_SIG + "I)"
                            + STRING_SIG)));
