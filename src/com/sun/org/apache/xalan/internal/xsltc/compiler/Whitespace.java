...
final class Whitespace extends TopLevelElement {
    ...
    private static WhitespaceRule findContradictingRule(Vector rules, WhitespaceRule rule) {
        for (int i = 0; i < rules.size(); i++) {
            ...
            /*
             * See if there is a contradicting rule with higher priority. If the
             * rules has the same action then this rule is redundant, if they
     ...
     * il) { final ConstantPoolGen cpg = classGen.getConstantPool(); final int
     * prt = cpg.addMethodref("java/lang/System/out", "println",
     * "(Ljava/lang/String;)V"); il.append(DUP); il.append(new
     * INVOKESTATIC(prt)); }
     */
    ...
    private static void compilePredicate(Vector rules, int defaultAction, ClassGenerator classGen) {
        ...
        final MethodGenerator stripSpace = new MethodGenerator(ACC_PUBLIC | ACC_FINAL,
                ...
                new com.sun.org.apache.bcel.internal.generic.Type[] { Util.getJCRefType(DOM_INTF_SIG),
                        ...
                        com.sun.org.apache.bcel.internal.generic.Type.INT }, new String[] { "dom", "node",
     ...
     * whitespace text-nodes: - USE_PREDICATE (run the method generated by this
     * method) - STRIP_SPACE (always strip whitespace text-nodes) -
