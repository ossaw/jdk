...
/**
 * SAXImpl is the core model for SAX input source. SAXImpl objects are usually
 * created from an XSLTCDTMManager.
 * <p>
 * DOMSource inputs are handled using DOM2SAX + SAXImpl. SAXImpl has a few
 * specific fields (e.g. _node2Ids, _document) to keep DOM-related information.
 * They are used when the processing behavior between DOM and SAX has to be
 * different. Examples of these include id function and unparsed entity.
 * <p>
...
public final class SAXImpl extends SAX2DTM2 implements DOMEnhancedForDTM, DOMBuilder {
    ...
    /**
     * Lookup a namespace URI from a prefix starting at node. This method is
    ...
    /**
     * Create an org.w3c.dom.Node from a node in an iterator The iterator most
    ...
    public NodeList makeNodeList(int index) {
        ...
        if (nodeID < 0) {
            ...
            return (_nodeLists[nodeID] != null) ? _nodeLists[nodeID]
                    : (_nodeLists[nodeID] = new DTMAxisIterNodeList(this, new SingletonIterator(index)));
        } else {
    ...
    /**
     * Create an org.w3c.dom.NodeList from a node iterator The iterator most be
    ...
    /**
     * Returns singleton iterator containg the document root Works for them main
     * document (mark == 0). It cannot be made to point to any other node
     * through setStartNode().
     */
     ...
     */
    /*
     * private int[] setupMapping(String[] names, String[] uris, int[] types,
     * int nNames) { // Padding with number of names, because they // may need
     * to be added, i.e for RTFs. See copy03 final int[] result = new
     * int[m_expandedNameTable.getSize()]; for (int i = 0; i < nNames; i++) {
     * //int type = getGeneralizedType(namesArray[i]); int type =
     * m_expandedNameTable.getExpandedTypeID(uris[i], names[i], types[i],
     * false); result[type] = type; } return result; }
     */
    ...
    /**
     * Get mapping from DOM element/attribute types to external types. This
     * method is used when the document is not fully built.
     */
     ...
     */
    public SAXImpl(XSLTCDTMManager mgr, Source source, int dtmIdentity, DTMWSFilter whiteSpaceFilter,
            XMLStringFactory xstringfactory, boolean doIndexing, boolean buildIdIndex) {
        this(mgr, source, dtmIdentity, whiteSpaceFilter, xstringfactory, doIndexing, DEFAULT_BLOCKSIZE,
                buildIdIndex, false);
    }
     ...
     */
    public SAXImpl(XSLTCDTMManager mgr, Source source, int dtmIdentity, DTMWSFilter whiteSpaceFilter,
            XMLStringFactory xstringfactory, boolean doIndexing, int blocksize, boolean buildIdIndex,
            boolean newNameTable) {
        super(mgr, source, dtmIdentity, whiteSpaceFilter, xstringfactory, doIndexing, blocksize, false,
    ...
    /**
     * Call this when an xml:space attribute is encountered to define the
     * whitespace strip/preserve settings.
     */
    ...
    /**
     * Call this from endElement() to revert strip/preserve setting to whatever
     * it was before the corresponding startElement().
     */
    ...
    /**
     * Returns an iterator with all children of a specific type for a given node
     * (element)
     */
    ...
    /**
     * Similar to getAxisIterator, but this one returns an iterator containing
     ...
     * returns an iterator with nodes that belong in a certain namespace, such
     * as with <xsl:apply-templates select="blob/foo:*"/> The 'axis' specifies
     * the axis for the base iterator from which the nodes are taken, while 'ns'
    ...
    /**
     * Iterator that handles node tests that test for a namespace, but have a
     ...
     * The functionality chould be achieved by putting a filter on top of a
    ...
    public final class NamespaceChildrenIterator extends InternalAxisIteratorBase {
        ...
        public int next() {
            if (_currentNode != DTM.NULL) {
                for (int node = (NOTPROCESSED == _currentNode) ? _firstch(makeNodeIdentity(_startNode))
    ...
    public final class NamespaceAttributeIterator extends InternalAxisIteratorBase {
        ...
        public int next() {
            ...
            for (nextNode = getNextAttribute(node); nextNode != END; nextNode = getNextAttribute(nextNode)) {
    ...
    /**
     * Returns an iterator with all descendants of a node that are of a given
     * type.
     */
    ...
    /**
     * Returns an instance of the DOMBuilder inner class This class will consume
     * the input document through a SAX2 interface and populate the tree.
     */
    ...
    /**
     * Return a SerializationHandler for output handling. This method is used by
        ...
        if (rtfType == DOM.SIMPLE_RTF) {
            ...
            if (addToManager) {
                ...
                AdaptiveResultTreeImpl rtf = new AdaptiveResultTreeImpl(_dtmManager,
                        dtmPos << DTMManager.IDENT_DTM_NODE_BITS, m_wsfilter, initSize, m_buildIdIndex);
                _dtmManager.addDTM(rtf, dtmPos, 0);
                return rtf;
