 ...
 * {@link com.sun.org.apache.xml.internal.dtm.DTMWSFilter DTMWSFilter} interface
 * to the XSLTC DOM {@link com.sun.org.apache.xalan.internal.xsltc.StripFilter
 * StripFilter} interface.
 */
public class DOMWSFilter implements DTMWSFilter {
	...
	/**
	 * Test whether whitespace-only text nodes are visible in the logical view
	 * of <code>DTM</code>. Normally, this function will be called by the
	 * implementation of <code>DTM</code>; it is not normally called directly
	 * from user code.
	 *
	 * @param node
	...
	public short getShouldStripSpace(int node, DTM dtm) {
		if (m_filter != null && dtm instanceof DOM) {
			...
			if (dtm instanceof DOMEnhancedForDTM) {
				...
				// %OPT% The mapping array does not have information about all
				// the
				// exptypes. However it does contain enough information about
				// all names
				// in the translet's namesArray. If the expType does not fall
				// into the
				// range of the mapping array, it means that the expType is not
				// for one
