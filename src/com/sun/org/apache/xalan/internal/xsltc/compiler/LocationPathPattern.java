...
public abstract class LocationPathPattern extends Pattern {
	 ...
	 * template. This method is called for templates that are in the same mode
	 * and that match on the same core pattern. The rules used are: o) first
	 * check precedence - highest precedence wins o) then check priority -
	 * highest priority wins o) then check the position - the template that
	 * occured last wins
	 */
