...
class FunctionCall extends Expression {
    ...
    public Type typeCheckExternal(SymbolTable stable) throws TypeCheckError {
        ...
        // check if we are calling an instance method
            ...
            if (!_isStatic) {
                ...
                if (_namespace_format == NAMESPACE_FORMAT_CLASS && firstArgType instanceof ObjectType
                        && _clazz != null && _clazz.isAssignableFrom(((ObjectType) firstArgType)
                                .getJavaClass()))
                    hasThisArgument = true;
        ...
        if (_chosenMethod != null && _thisArgument == null && !Modifier.isStatic(_chosenMethod
                .getModifiers())) {
    ...
    public void translate(ClassGenerator classGen, MethodGenerator methodGen) {
        ...
        final boolean isExtensionFunctionEnabled = classGen.getParser().getXSLTC().getFeature(
        ...
        // run-time error message for unsupported external functions
            ...
            for (int i = 0; i < n; i++) {
                ...
                paramTemp[i] = methodGen.addLocalVariable("function_call_tmp" + i, expType.toJCType(), null,
                        null);
        ...
        if (_className != null && _className.length() > 0) {
            ...
            try {
                if (_clazz == null) {
                    ...
                    final boolean isExtensionFunctionEnabled = getXSLTC().getFeature(
                            FeatureManager.Feature.ORACLE_ENABLE_EXTENSION_FUNCTION);

                    // Check if FSP and SM - only then proceed with loading
                    if (namespace != null && isSecureProcessing && isExtensionFunctionEnabled && (namespace
                            .startsWith(JAVA_EXT_XALAN) || namespace.startsWith(JAVA_EXT_XSLTC) || namespace
                                    .startsWith(JAVA_EXT_XALAN_OLD) || namespace.startsWith(
                ...
                for (int i = 0; i < methods.length; i++) {
                    ...
                    if (Modifier.isPublic(mods) && methods[i].getName().equals(methodName) && methods[i]
    ...
    private void translateUnallowedExtension(ConstantPoolGen cpg, InstructionList il) {
        int index = cpg.addMethodref(BASIS_LIBRARY_CLASS, "unallowed_extension_functionF",
                "(Ljava/lang/String;)V");
        il.append(new PUSH(cpg, _fname.toString()));
        il.append(new INVOKESTATIC(index));
    }
