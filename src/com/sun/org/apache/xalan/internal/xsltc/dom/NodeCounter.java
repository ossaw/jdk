...
public abstract class NodeCounter {
	 ...
	 * instead of throwing a NumberFormatException, it returns 0. This method is
	 * used instead of Integer.parseInt() since it does not incur the overhead
	...
	private final void setTokens(final String format) {
		if ((_format != null) && (format.equals(_format))) {// has already been
															// set
			return;
		}
		_format = format;
	...
	/**
	 * Returns true if <tt>node</tt> matches the count pattern. By default a
	 * node matches the count patterns if it is of the same type as the starting
	 * node.
	 */
	...
	/**
	 * Returns true if <tt>node</tt> matches the from pattern. By default, no
	 * node matches the from pattern.
	 */
