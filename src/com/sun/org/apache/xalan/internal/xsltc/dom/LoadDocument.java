...
public final class LoadDocument {
	 ...
	 * com/sun/org/apache/xalan/internal/xsltc/compiler/DocumentCall.java) and
	 * returns an iterator containing the requested nodes. Builds a
	 ...
	 * document(Obj) call
	 */
	public static DTMAxisIterator documentF(Object arg, String xslURI, AbstractTranslet translet,
	...
	private static DTMAxisIterator document(String uri, String base, AbstractTranslet translet,
			DOM dom) throws Exception {
		return document(uri, base, translet, dom, false);
	}

	private static DTMAxisIterator document(String uri, String base, AbstractTranslet translet,
			DOM dom, boolean cacheDOM) throws Exception {
		try {
			...
			if (cache != null) {
				...
				String accessError = SecuritySupport.checkAccess(uri,
						translet.getAllowedProtocols(), XalanConstants.ACCESS_EXTERNAL_ALL);
				if (accessError != null) {
				...
				DOMEnhancedForDTM enhancedDOM = (DOMEnhancedForDTM) dtmManager.getDTM(
						new StreamSource(uri), false, null, true, false, translet.hasIdCall(),
						cacheDOM);
				newdom = enhancedDOM;
	...
	private static DTMAxisIterator document(DTMAxisIterator arg1, String baseURI,
	...
	/**
	 * Create a DTMAxisIterator for the newdom. This is currently only used to
