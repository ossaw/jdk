...
public final class BasisLibrary {
    ...
    /**
     * Re-use a single instance of StringBuffer (per thread) in the basis
     * library. StringBuilder is better, however, DecimalFormat only accept
     * StringBuffer
     */
    ...
    public static double roundF(double d) {
        return (d < -0.5 || d > 0.0) ? Math.floor(d + 0.5)
    ...
    /**
     * External functions that cannot be resolved are replaced with a call to
     * this method. This method will generate a runtime errors. A good
     * stylesheet checks whether the function exists using conditional
     * constructs, and never really tries to call it if it doesn't exist. But
     * simple stylesheets may result in a call to this method. The compiler
     * should generate a warning if it encounters a call to an unresolved
     * external function.
     */
     ...
     */
    public static boolean compare(DTMAxisIterator left, final double rnumber, final int op, DOM dom) {
        int node;
        // left.reset();
     ...
     * the decimal if null. Uses a specialized formatter object for very large
     * and very small numbers that ignores locales, thus using always using "."
    ...
    /**
     * Utility function: used to convert references to node-sets. If the obj is
     * an instanceof Node then create a singleton iterator.
     */
     ...
     * nodeList2Iterator. In reality, though, this causes a ClassCastException
     * in getDTMHandleFromNode because SAXImpl is not an instance of DOM2DTM. So
     * we use the more lengthy implementation below until this issue has been
     * addressed.
     *
     ...
     * iterator.
     */
    public static DTMAxisIterator nodeList2Iterator(org.w3c.dom.NodeList nodeList, Translet translet,
            DOM dom) {
        ...
        if (doc != null) {
            ...
            DOMAdapter domAdapter = new DOMAdapter(idom, translet.getNamesArray(), translet.getUrisArray(),
    ...
    public static void copy(Object obj, SerializationHandler handler, int node, DOM dom) {
        try {
            if (obj instanceof DTMAxisIterator) {
                ...
                // ((DOM)obj).copy(((com.sun.org.apache.xml.internal.dtm.ref.DTMDefaultBase)((DOMAdapter)obj).getDOMImpl()).getDocument(),
                // handler);
                DOM newDom = (DOM) obj;
     ...
     */
    public static String startXslElement(String qname, String namespace, SerializationHandler handler,
            DOM dom, int node) {
        try {
            ...
            if (index > 0) {
                ...
                if (namespace == null || namespace.length() == 0) {
                    try {
                        ...
                        handler.flushPending(); // need to flush or else can't
                                                // get namespacemappings
    ...
    /**
     * These functions are used in the execution of xsl:element to generate and
     * reset namespace prefix index local to current transformation process
     */
     ...
     * class Hence a parameter of the form "{http://foo.bar}xyz" will be
     * replaced with the corresponding values by the BasisLibrary's utility
     * method mapQNametoJavaName and thus get mapped to legal java variable
     * names
     */
    ...
    /**
     * Utility method to calculate string-length as a number of code points, to
     * avoid possible errors with string that contains complementary characters
     */
