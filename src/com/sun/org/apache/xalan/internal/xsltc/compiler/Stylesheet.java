...
public final class Stylesheet extends SyntaxTreeNode {
	...
	/**
	 * A reference to the SourceLoader set by the user (a URIResolver if the
	 * JAXP API is being used).
	 */
	...
	/**
	 * Flag indicating if this is a simplified stylesheets. A template matching
	 * on "/" must be added in this case.
	 */
	...
	public int getMinimumDescendantPrecedence() {
		if (_minimumDescendantPrecedence == -1) {
			...
			final int inclImpCount = (_includedStylesheets != null) ? _includedStylesheets.size()
					: 0;
	...
	/**
	 * Parse the version and uri fields of the stylesheet and add an entry to
	 * the symbol table mapping the name <tt>__stylesheet_</tt> to an instance
	 * of this class.
	 */
	public void parseContents(Parser parser) {
		 ...
		 * ErrorMsg.REQUIRED_ATTR_ERR,"version"); } // Verify that the version
		 * is 1.0 and nothing else else if (!_version.equals("1.0")) {
	...
	private void addStaticField(ClassGenerator classGen, String type, String name) {
		final FieldGen fgen = new FieldGen(ACC_PROTECTED | ACC_STATIC, Util.getJCRefType(type),
				name, classGen.getConstantPool());
		classGen.addField(fgen.getField());
	...
	public void translate() {
		...
		final ClassGenerator classGen = new ClassGenerator(_className, TRANSLET_CLASS,
				Constants.EMPTYSTRING, ACC_PUBLIC | ACC_SUPER, null, this);
	...
	/**
	 * Compile the namesArray, urisArray and typesArray into the static
	 * initializer. They are read-only from the translet. All translet instances
	 * can share a single copy of this informtion.
	 */
	private void compileStaticInitializer(ClassGenerator classGen) {
		...
		final MethodGenerator staticConst = new MethodGenerator(ACC_PUBLIC | ACC_STATIC,
				com.sun.org.apache.bcel.internal.generic.Type.VOID, null, null, "<clinit>",
				_className, il, cpg);
	...
	private void compileConstructor(ClassGenerator classGen, Output output) {
		...
		final MethodGenerator constructor = new MethodGenerator(ACC_PUBLIC,
				com.sun.org.apache.bcel.internal.generic.Type.VOID, null, null, "<init>",
				_className, il, cpg);
	 ...
	 * of dependency is allowed. Note also that the buildKeys() method is still
	 * generated as it is used by the LoadDocument class, but it no longer
	...
	private String compileTopLevel(ClassGenerator classGen) {
		...
		final com.sun.org.apache.bcel.internal.generic.Type[] argTypes = {
				Util.getJCRefType(DOM_INTF_SIG), Util.getJCRefType(NODE_ITERATOR_SIG),
	...
	/**
	 * This method returns a vector with variables/params and keys in the order
	 * in which they are to be compiled for initialization. The order is
	 * determined by analyzing the dependencies between them. The XSLT 1.0 spec
	 * does not allow a key to depend on a variable. However, for compatibility
	 * with Xalan interpretive, that type of dependency is allowed and,
	...
	private Vector resolveDependencies(Vector input) {
		 ...
		 * System.out.println("e = " + e + " depends on:"); Vector dep =
		 * e.getDependencies(); for (int j = 0; j < (dep != null ? dep.size() :
		 * 0); j++) { System.out.println("\t" + dep.elementAt(j)); } }
		 * System.out.println("=================================");
		 */
		 ...
		 * 0; i < result.size(); i++) { final TopLevelElement e =
		 * (TopLevelElement) result.elementAt(i); System.out.println("e = " +
		 * e); }
		 */
	 ...
	 * the input document are created in topLevel(), not in this method.
	 ...
	 * initialize global variables and global parameters. The current node is
	 * set to be the document's root node.
	 */
	private void compileTransform(ClassGenerator classGen) {
		...
		final int applyTemplates = cpg.addMethodref(getClassName(), "applyTemplates",
				applyTemplatesSig);
		...
		il.append(new INVOKEVIRTUAL(cpg.addMethodref(TRANSLET_CLASS, "makeDOMAdapter",
		...
		final int index = cpg.addMethodref(TRANSLET_CLASS, "transferOutputSettings",
				"(" + OUTPUT_HANDLER_SIG + ")V");
		il.append(new INVOKEVIRTUAL(index));

		/*
		 * Compile buildKeys() method. Note that this method is not invoked here
		 * as keys for the input document are now created in topLevel().
