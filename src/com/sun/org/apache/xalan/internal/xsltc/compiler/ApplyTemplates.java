...
final class ApplyTemplates extends Instruction {
	...
	/**
	 * Translate call-template. A parameter frame is pushed only if some
	 * template in the stylesheet uses parameters.
	 */
	public void translate(ClassGenerator classGen, MethodGenerator methodGen) {
		...
		if ((_type != null) && (_type instanceof ResultTreeType)) {
			...
			if (sortObjects.size() > 0) {
				...
				int setStartNode = cpg.addInterfaceMethodref(NODE_ITERATOR, SET_START_NODE,
						"(I)" + NODE_ITERATOR_SIG);
		...
		if (stylesheet.hasLocalParams() || hasContents()) {
			il.append(classGen.loadTranslet());
			final int popFrame = cpg.addMethodref(TRANSLET_CLASS, POP_PARAM_FRAME,
					POP_PARAM_FRAME_SIG);
