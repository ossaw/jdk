...
public class SAX2StAXEventWriter extends SAX2StAXBaseWriter {
	...
	private void writeStartDocument() throws SAXException {
		try {
			if (docLocator == null)
				...
				try {
					writer.add(
							eventFactory.createStartDocument(((Locator2) docLocator).getEncoding(),
	...
	public void comment(char[] ch, int start, int length) throws SAXException {
		if (needToCallStartDocument) {
			...
			// that we could get
			// the locator, but we can't output a comment before the start
			// document, so we're just
			// going to have to do without the locator if it hasn't been set
			// yet.
			writeStartDocument();
		}
	...
	public void processingInstruction(String target, String data) throws SAXException {
		...
		if (needToCallStartDocument) {
			// Drat. We were trying to postpone this until the first element so
			// that we could get
			// the locator, but we can't output a PI before the start document,
			// so we're just
			// going to have to do without the locator if it hasn't been set
			// yet.
			writeStartDocument();
		...
		for (int i = 0, s = attributes.getLength(); i < s; i++) {
			...
			if ("xmlns".equals(attrQName) || "xmlns".equals(attrPrefix)) {
				...
				if (attrPrefix.length() > 0) {
					...
					attribute = eventFactory.createAttribute(attrPrefix, attrURI, attrLocal,
							attrValue);
