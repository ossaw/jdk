...
public final class TemplatesImpl implements Templates, Serializable {
    ...
    /**
     * Name of the superclass of all translets. This is needed to determine
     * which, among all classes comprising a translet, is the main one.
     */
    ...
    /**
     * Cache the DTM for the stylesheet in a thread local variable, which is
     * used by the document('') function. Use ThreadLocal because a DTM cannot
     * be shared between multiple threads. Declaring it transient to fix bug
     * 22438
     */
    ...
    private static final ObjectStreamField[] serialPersistentFields = new ObjectStreamField[] {
            new ObjectStreamField("_name", String.class), new ObjectStreamField("_bytecodes", byte[][].class),
            new ObjectStreamField("_class", Class[].class), new ObjectStreamField("_transletIndex",
                    int.class), new ObjectStreamField("_outputProperties", Properties.class),
     ...
     * implemented Serializable if yes then we need to deserialize the
     ...
     * Note: This method is private for security reasons. See CR 6537898. When
     * merging with Apache, we must ensure that the privateness of this method
     * is maintained (that is why it wasn't removed).
     */
    ...
    private void defineTransletClasses() throws TransformerConfigurationException {
        ...
        TransletClassLoader loader = (TransletClassLoader) AccessController.doPrivileged(
                new PrivilegedAction() {
                    public Object run() {
                        return new TransletClassLoader(ObjectFactory.findClassLoader(), _tfactory
    ...
    /**
     * Implements JAXP's Templates.getOutputProperties(). We need to instanciate
     * a translet to get the output settings, so we might as well just
     * instanciate a Transformer and use its implementation of this method.
     */
