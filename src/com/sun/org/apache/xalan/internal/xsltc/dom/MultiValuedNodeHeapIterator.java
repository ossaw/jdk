...
/**
 * <p>
 * <code>MultiValuedNodeHeapIterator</code> takes a set of multi-valued heap
 * nodes and produces a merged NodeSet in document order with duplicates
 * removed.
 * </p>
 * <p>
 * Each multi-valued heap node (which might be a
 * {@link org.apache.xml.dtm.DTMAxisIterator}, but that's not necessary)
 * generates DTM node handles in document order. The class maintains the
 * multi-valued heap nodes in a heap, not surprisingly, sorted by the next DTM
 * node handle available form the heap node.
 * </p>
 * <p>
 * After a DTM node is pulled from the heap node that's at the top of the heap,
 * the heap node is advanced to the next DTM node handle it makes available, and
 * the heap nature of the heap is restored to ensure the next DTM node handle
 * pulled is next in document order overall.
 *
...
public abstract class MultiValuedNodeHeapIterator extends DTMAxisIteratorBase {
    ...
    public abstract class HeapNode implements Cloneable {
        ...
        /**
         * Creates a deep copy of this {@link HeapNode}. The clone is not reset
         * from the current position of the original.
         *
