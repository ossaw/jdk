...
/**
 * <p>
 * This pseudo-instruction marks the beginning of a region of byte code that can
 * be copied into a new method, termed an "outlineable" chunk. The size of the
 * Java stack must be the same at the start of the region as it is at the end of
 * the region, any value on the stack at the start of the region must not be
 * consumed by an instruction in the region of code, the region must not contain
 * a return instruction, no branch instruction in the region is permitted to
 * have a target that is outside the region, and no branch instruction outside
 * the region is permitted to have a target that is inside the region.
 * </p>
 * <p>
