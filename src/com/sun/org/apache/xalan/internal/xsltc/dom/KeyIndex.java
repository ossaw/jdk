...
public class KeyIndex extends DTMAxisIteratorBase {
	...
	/**
	 * The XSLTC DOM object if this KeyIndex is being used to implement the id()
	 * function.
	 */
	private DOM _dom;
	...
	/**
	 * Adds a node to the node list for a given value. Nodes will always be
	 ...
	 * prior to returning the node iterator. The lookup code for key() and id()
	 * differ in the way the lookup value can be whitespace separated list of
	 * tokens for the id() function, but a single string for the key() function.
	 * 
	 * @deprecated
	 */
	public void lookupId(Object value) {
		...
		while (values.hasMoreElements()) {
			...
			if (nodes == null && _enhancedDOM != null && _enhancedDOM
					.hasDOMSource()) {
	...
	public int next() {
		...
		return (_position < _nodes.cardinality()) ? _dom.getNodeHandle(_nodes
				.at(_position++)) : DTMAxisIterator.END;
	}
	 ...
	 * results from that reference to the <code>id</code> function. This is used
	 * in the implementation of <code>id</code> patterns.
	 *
	 * @param node
	 *              The context node
	 * @param value
	 *              The argument to the <code>id</code> function
	...
	public int containsID(int node, Object value) {
		...
		int rootHandle = _dom.getAxisIterator(Axis.ROOT).setStartNode(node)
				.next();
	...
	/**
	 * <p>
	 * Given a context node and the second argument to the XSLT <code>key</code>
	 * function, checks whether the context node is in the set of nodes that
	 * results from that reference to the <code>key</code> function. This is
	 * used in the implementation of key patterns.
	 * </p>
	 * <p>
	 ...
	 *              The context node
	 * @param value
	 *              The second argument to the <code>key</code> function
	...
	public int containsKey(int node, Object value) {
		...
		// Get the mapping table for the document containing the context node
		Map<String, IntegerArray> index = _rootToIndexMap.get(new Integer(
				rootHandle));
	 ...
	 * result from a reference to the XSLT <code>key</code> function or XPath
	 ...
	 *                  references
	 * @param isKeyCall
	 *                  A <code>boolean</code> indicating whether the iterator
	 *                  is
	 *                  being created for a reference <code>key</code> or
	 *                  <code>id</code>
	 */
	 ...
	 *
	 * @param keyValue
	 *                  A string representing the key values or id references
	 * @param isKeyCall
	 *                  A <code>boolean</code> indicating whether the iterator
	 *                  is
	 *                  being created for a reference <code>key</code> or
	 ...
	 *
	 * @param keyValue
	 *                  An iterator representing the key values or id references
	 * @param isKeyCall
	 *                  A <code>boolean</code> indicating whether the iterator
	 *                  is
	 *                  being created for a reference <code>key</code> or
	...
	/**
	 * An iterator representing the result of a reference to either the XSLT
	 * <code>key</code> function or the XPath <code>id</code> function.
	 */
	public class KeyIndexIterator extends MultiValuedNodeHeapIterator {

		/**
		 * <p>
		 * A reference to the <code>key</code> function that only has one key
		 * value or to the <code>id</code> function that has only one string
		 * argument can be optimized to ignore the multi-valued heap. This field
		 * will be <code>null</code> otherwise.
		 */
		 ...
		 * argument to the <code>key</code> function or a node set argument to
		 ...
		 * argument to the <code>id</code> function.
		 * </p>
		 *
		 * <p>
		...
		/**
		 * Indicates whether this object represents the result of a reference to
		 * the <code>key</code> function (<code>true</code>) or the
		 ...
		 */
		protected class KeyIndexHeapNode extends
			...
			/**
			 * Create a heap node representing DTM nodes retrieved for one key
			 * value in a reference to the <code>key</code> function or string
			 * argument to the <code>id</code> function.
			 */
			 ...
			 *         this heap node is before the current node of the argument
			 *         heap node in document order.
			 */
			 ...
			 * </p>
			 * <p>
			 * This has no real effect on this kind of heap node. Instead, the
			 * {@link KeyIndexIterator#setStartNode(int)} method should create
			 * new instances of this class to represent the effect of changing
			 * the context.
			 * </p>
			 */
		 ...
		 *
		 * @param keyValue
		 *                      the argument to <code>key</code> or
		 *                      <code>id</code> cast
		 *                      to a <code>String</code>
		 * @param isKeyIterator
		 *                      indicates whether the constructed iterator
		 *                      represents a
		 *                      reference to <code>key</code> or
		 *                      <code>id</code>.
		 */
		...
		/**
		 * Retrieve nodes for a particular key value or a particular id argument
		 * value.
		 *
		 * @param root
		 *                 The root node of the document containing the context
		 *                 node
		 * @param keyValue
		 *                 The key value of id string argument value
		...
		/**
		 * Set context node for the iterator. This will cause the iterator to
		 * reset itself, reevaluate arguments to the function, look up nodes in
		 * the input and reinitialize its internal heap.
		 *
		 * @param node
		 *             the context node
		 ...
		 * function with the context specified by {@link #setStartNode(int)} and
		 * set up this iterator to iterate over the DTM nodes that are to be
		 * returned.
		 */
		protected void init() {
			super.init();
			...
			int rootHandle = _dom.getAxisIterator(Axis.ROOT).setStartNode(
					_startNode).next();
			...
			if (_keyValueIterator == null) {
				...
				for (int keyValueNode = keyValues
						.next(); keyValueNode != DTMAxisIterator.END; keyValueNode = keyValues
								.next()) {
