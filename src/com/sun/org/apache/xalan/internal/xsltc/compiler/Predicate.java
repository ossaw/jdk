...
final class Predicate extends Expression implements Closure {
	...
	/**
	 * Returns a boolean value indicating if the nth position optimization is
	 ...
	 * number convert it to boolean by adding a comparison with position(). Note
	...
	private void compileFilter(ClassGenerator classGen, MethodGenerator methodGen) {
		...
		filterGen = new FilterGenerator(_className, "java.lang.Object", toString(),
				ACC_PUBLIC | ACC_SUPER, new String[] { CURRENT_NODE_LIST_FILTER },
		...
		for (int i = 0; i < length; i++) {
			...
			filterGen.addField(new Field(ACC_PUBLIC, cpg.addUtf8(var.getEscapedName()),
		...
		testGen = new TestGenerator(ACC_PUBLIC | ACC_FINAL,
				...
				new com.sun.org.apache.bcel.internal.generic.Type[] {
						...
						com.sun.org.apache.bcel.internal.generic.Type.INT,
						com.sun.org.apache.bcel.internal.generic.Type.INT,
						Util.getJCRefType(TRANSLET_SIG), Util.getJCRefType(NODE_ITERATOR_SIG) },
	...
	/**
	 * Returns true if the predicate is a test for the existance of an element
	 * or attribute. All we have to do is to get the first node from the step,
	 * check if it is there, and then return true/false.
	 */
	 ...
	 * returned if the expression is not of the right form. Optimization if off
	 * if null is returned.
	 */
	public Step getStep() {
	...
	/**
	 * Returns the value in an expression of the form 'step = value'. A value
	 * may be either a literal string or a variable whose type is string.
	...
	/**
	 * Translate a predicate expression. This translation pushes two references
	...
	public void translateFilter(ClassGenerator classGen, MethodGenerator methodGen) {
		...
		for (int i = 0; i < length; i++) {
			...
			if (variableClosure != null) {
				il.append(ALOAD_0);
				il.append(new GETFIELD(cpg.addFieldref(variableClosure.getInnerClassName(),
	 ...
	 * this translation pushes two references on the stack: a reference to a
	 * newly created filter object and a reference to the predicate's closure.
	 * See class <code>Step</code> for further details.
	 */
