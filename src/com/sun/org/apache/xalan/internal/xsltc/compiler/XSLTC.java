...
public final class XSLTC {
	...
	private Map<String, Integer> _attributes; // Map of all registered
												// attributes
	...
	private Map<String, Integer> _namespaces; // Map of all registered
												// namespaces
	...
	/**
	 * Set to true if template inlining is requested. Template inlining used to
	 * be the default, but we have found that Hotspots does a better job with
	 * shorter methods, so the default is *not* to inline now.
	 */
	...
	/*
	 * Class loader reference that will be used for external extension functions
	 * loading
	 */
	...
	private void setExternalExtensionFunctions(String name, Class clazz) {
		if (_isSecureProcessing && clazz != null
	...
	/*
	 * Function loads an external extension functions. The filtering of function
	 * types (external,internal) takes place in FunctionCall class
	 *
	 */
	...
	/**
	 * Set a flag indicating if templates are to be inlined or not. The default
	 * is to do inlining, but this causes problems when the stylesheets have a
	 * large number of templates (e.g. branch targets exceeding 64K or a length
	 * of a method exceeding 64K).
	 */
	...
	public int registerNamespacePrefix(QName name) {
		...
		if (code == null) {
			...
			final String uri = name.getNamespace();
			if ((uri != null) && (!uri.equals(""))) {
				// namespace::ext2:ped2 will be made empty in
	...
	/**
	 * Returns a unique name for every helper class needed to execute a
	 * translet.
	 */
	...
	/**
	 * Retrieve a string representation of the character data to be stored in
	 * the translet as a <code>char[]</code>. There may be more than one such
	 * array required.
	 * 
	 * @param index
