...
public class ExsltDynamic extends ExsltBase {
	 ...
	 * dynamically using an XPath expression passed as a string as the second
	 * argument.
	 * <p>
	 * The expressions are evaluated relative to the nodes passed as the first
	 * argument. In other words, the value for each node is calculated by
	 * evaluating the XPath expression with all context information being the
	 * same as that for the call to the dyn:max function itself, except for the
	 * following:
	 * <p>
	 * <ul>
	 * <li>the context node is the node whose value is being calculated.</li>
	 * <li>the context position is the position of the node within the node set
	 * passed as the first argument to the dyn:max function, arranged in
	 * document order.</li>
	 * <li>the context size is the number of nodes passed as the first argument
	 * to the dyn:max function.</li>
	 * </ul>
	 * <p>
	 * The dyn:max function returns the maximum of these values, calculated in
	 * exactly the same way as for math:max.
	 * <p>
	 * If the expression string passed as the second argument is an invalid
	 * XPath expression (including an empty string), this function returns NaN.
	 * <p>
	 * This function must take a second argument. To calculate the maximum of a
	 * set of nodes based on their string values, you should use the math:max
	 * function.
	 *
	 * @param myContext
	...
	public static double max(ExpressionContext myContext, NodeList nl, String expr)
		...
		for (int i = 0; i < contextNodes.getLength(); i++) {
			...
			try {
				...
				result = dynamicXPath.execute(xctxt, contextNode, xctxt.getNamespaceContext())
						.num();
	 ...
	 * argument. In other words, the value for each node is calculated by
	 * evaluating the XPath expression with all context information being the
	 * same as that for the call to the dyn:min function itself, except for the
	 * following:
	 * <p>
	 * <ul>
	 * <li>the context node is the node whose value is being calculated.</li>
	 * <li>the context position is the position of the node within the node set
	 * passed as the first argument to the dyn:min function, arranged in
	 * document order.</li>
	 * <li>the context size is the number of nodes passed as the first argument
	 * to the dyn:min function.</li>
	 * </ul>
	 * <p>
	 * The dyn:min function returns the minimum of these values, calculated in
	 * exactly the same way as for math:min.
	 * <p>
	 * If the expression string passed as the second argument is an invalid
	 * XPath expression (including an empty string), this function returns NaN.
	 * <p>
	 * This function must take a second argument. To calculate the minimum of a
	 * set of nodes based on their string values, you should use the math:min
	 * function.
	 *
	 * @param myContext
	 *            The ExpressionContext passed by the extension processor
	 ...
	 * argument. In other words, the value for each node is calculated by
	 * evaluating the XPath expression with all context information being the
	 * same as that for the call to the dyn:sum function itself, except for the
	 * following:
	 * <p>
	 * <ul>
	 * <li>the context node is the node whose value is being calculated.</li>
	 * <li>the context position is the position of the node within the node set
	 * passed as the first argument to the dyn:sum function, arranged in
	 * document order.</li>
	 * <li>the context size is the number of nodes passed as the first argument
	 * to the dyn:sum function.</li>
	 * </ul>
	 * <p>
	 * The dyn:sum function returns the sumimum of these values, calculated in
	 * exactly the same way as for sum.
	 * <p>
	 * If the expression string passed as the second argument is an invalid
	 * XPath expression (including an empty string), this function returns NaN.
	 * <p>
	 * This function must take a second argument. To calculate the sumimum of a
	 * set of nodes based on their string values, you should use the sum
	 * function.
	 *
	 * @param myContext
	 *            The ExpressionContext passed by the extension processor
	 ...
	 * argument. In other words, the value for each node is calculated by
	 * evaluating the XPath expression with all context information being the
	 * same as that for the call to the dyn:map function itself, except for the
	 * following:
	 * <p>
	 * <ul>
	 * <li>The context node is the node whose value is being calculated.</li>
	 * <li>the context position is the position of the node within the node set
	 * passed as the first argument to the dyn:map function, arranged in
	 * document order.</li>
	 * <li>the context size is the number of nodes passed as the first argument
	 * to the dyn:map function.</li>
	 * </ul>
	 * <p>
	 * If the expression string passed as the second argument is an invalid
	 ...
	 * returns the union of the node sets returned by evaluating the expression
	 * for each of the nodes in the first argument. Note that this may mean that
	 * the node set resulting from the call to the dyn:map function contains a
	 * different number of nodes from the number in the node set passed as the
	 * first argument to the function.
	 * <p>
	 * If the XPath expression evaluates as a number, the dyn:map function
	 * returns a node set containing one exsl:number element (namespace
	 * http://exslt.org/common) for each node in the node set passed as the
	 * first argument to the dyn:map function, in document order. The string
	 * value of each exsl:number element is the same as the result of converting
	 * the number resulting from evaluating the expression to a string as with
	 * the number function, with the exception that Infinity results in an
	 * exsl:number holding the highest number the implementation can store, and
	 ...
	 * returns a node set containing one exsl:boolean element (namespace
	 * http://exslt.org/common) for each node in the node set passed as the
	 * first argument to the dyn:map function, in document order. The string
	 * value of each exsl:boolean element is 'true' if the expression evaluates
	 * as true for the node, and '' if the expression evaluates as false.
	 * <p>
	 * Otherwise, the dyn:map function returns a node set containing one
	 * exsl:string element (namespace http://exslt.org/common) for each node in
	 * the node set passed as the first argument to the dyn:map function, in
	 * document order. The string value of each exsl:string element is the same
	 * as the result of converting the result of evaluating the expression for
	 ...
	 * resulting from that and so on until no more nodes are found. For example:
	 * 
	 * <pre>
	 *  dyn:closure(., '*')
	 * </pre>
	 ...
	 * node set acting as the context of the expression. The first time the
	 * expression is evaluated, the context node set is the first argument
	 * passed to the dyn:closure function. In other words, the node set for each
	 * node is calculated by evaluating the XPath expression with all context
	 * information being the same as that for the call to the dyn:closure
	 * function itself, except for the following:
	 * <p>
	 * <ul>
	 * <li>the context node is the node whose value is being calculated.</li>
	 * <li>the context position is the position of the node within the node set
	 * passed as the first argument to the dyn:closure function, arranged in
	 * document order.</li>
	 * <li>the context size is the number of nodes passed as the first argument
	 * to the dyn:closure function.</li>
	 ...
	 * resulting from evaluting the expression for each of the nodes in the
	 * source node set for that iteration. This result is then used as the
	 * source node set for the next iteration, and so on. The result of the
	 * function as a whole is the union of the node sets generated by each
	 * iteration.
	 * <p>
	 * If the expression string passed as the second argument is an invalid
	 * XPath expression (including an empty string) or an expression that does
