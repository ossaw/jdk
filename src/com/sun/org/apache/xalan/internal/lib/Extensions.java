...
/**
 * This class contains many of the Xalan-supplied extensions. It is accessed by
...
public class Extensions {
     ...
     * node-set function also found in xt and saxon. If the argument is a Result
     * Tree Fragment, then <code>nodeset</code> returns a node-set consisting of
     * a single root node as described in section 11.1 of the XSLT 1.0
     * Recommendation. If the argument is a node-set, <code>nodeset</code>
     * returns a node-set. If the argument is a string, number, or boolean, then
     * <code>nodeset</code> returns a node-set consisting of a single root node
     * with a single text node child that is the result of calling the XPath
     * string() function on the passed parameter. If the argument is anything
     * else, then a node-set is returned consisting of a single root node with a
     * single text node child that is the result of calling the java
     * <code>toString()</code> method on the passed argument. Most of the actual
     * work here is done in <code>MethodResolver</code> and
    ...
    /**
     * Returns the result of evaluating the argument as a string containing an
     * XPath expression. Used where the XPath expression is not known until
    ...
    /**
     * Returns a NodeSet containing one text node for each token in the first
     * argument. Delimiters are specified in the second argument. Tokens are
     * determined by a call to <code>StringTokenizer</code>. If the first
     * argument is an empty string or contains only delimiters, the result will
     * be an empty NodeSet.
    ...
    /**
     * Returns a NodeSet containing one text node for each token in the first
     * argument. Delimiters are whitespace. That is, the delimiters that are
     * used are tab (&#x09), linefeed (&#x0A), return (&#x0D), and space
     * (&#x20). Tokens are determined by a call to <code>StringTokenizer</code>.
     * If the first argument is an empty string or contains only delimiters, the
     * result will be an empty NodeSet.
     * Contributed to XalanJ1 by
     * <a href="mailto:benoit.cerrina@writeme.com">Benoit Cerrina</a>.
     *
     * @param toTokenize
    ...
    /**
     * Return a Node of basic debugging information from the EnvironmentCheck
     * utility about the Java environment.
     * <p>
     * Simply calls the
     * {@link com.sun.org.apache.xalan.internal.xslt.EnvironmentCheck} utility
     * to grab info about the Java environment and CLASSPATH, etc., and then
     * returns the resulting Node. Stylesheets can then maniuplate this data or
     * simply xsl:copy-of the Node. Note that we first attempt to load the more
     * advanced org.apache.env.Which utility by reflection; only if that fails
     * to we still use the internal version. Which is available from
     * <a href="http://xml.apache.org/commons/">http://xml.apache.org/commons/
     * </a>.
     * </p>
     * <p>
     * We throw a WrappedRuntimeException in the unlikely case that reading
     * information from the environment throws us an exception. (Is this really
