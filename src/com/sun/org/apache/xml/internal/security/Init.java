	...
	public static synchronized void init() {
		...
		InputStream is = AccessController.doPrivileged(new PrivilegedAction<InputStream>() {
			public InputStream run() {
				String cfile = System
						.getProperty("com.sun.org.apache.xml.internal.security.resource.config");
				if (cfile == null) {
					return null;
				}
		...
		try {
			...
			for (Node el = config.getFirstChild(); el != null; el = el.getNextSibling()) {
				...
				if (tag.equals("CanonicalizationMethods")) {
					Element[] list = XMLUtils.selectNodes(el.getFirstChild(), CONF_NS,
				...
				if (tag.equals("TransformAlgorithms")) {
					Element[] tranElem = XMLUtils.selectNodes(el.getFirstChild(), CONF_NS,
							"TransformAlgorithm");
				...
				if ("JCEAlgorithmMappings".equals(tag)) {
					...
					if (algorithmsNode != null) {
						Element[] algorithms = XMLUtils.selectNodes(algorithmsNode.getFirstChild(),
								CONF_NS, "Algorithm");
				...
				if (tag.equals("SignatureAlgorithms")) {
					...
					for (int i = 0; i < sigElems.length; i++) {
						...
						try {
							...
							if (log.isLoggable(java.util.logging.Level.FINE)) {
								log.log(java.util.logging.Level.FINE, "SignatureAlgorithm.register("
										+ uri + ", " + javaClass + ")");
							}
						} catch (ClassNotFoundException e) {
							Object exArgs[] = { uri, javaClass };
				...
				if (tag.equals("ResourceResolvers")) {
					...
					for (int i = 0; i < resolverElem.length; i++) {
						...
						if ((description != null) && (description.length() > 0)) {
							...
							if (log.isLoggable(java.util.logging.Level.FINE)) {
								log.log(java.util.logging.Level.FINE, "Register Resolver: "
										+ javaClass + ": For unknown purposes");
							}
						}
						try {
							ResourceResolver.register(javaClass);
						} catch (Throwable e) {
							log.log(java.util.logging.Level.WARNING, "Cannot register:" + javaClass
									+ " perhaps some needed jars are not installed", e);
						}
					}
				}
				...
				if (tag.equals("PrefixMappings")) {
					...
					Element[] nl = XMLUtils.selectNodes(el.getFirstChild(), CONF_NS,
							"PrefixMapping");
					...
					for (int i = 0; i < nl.length; i++) {
						...
						if (log.isLoggable(java.util.logging.Level.FINE)) {
							log.log(java.util.logging.Level.FINE,
									"Now I try to bind " + prefix + " to " + namespace);
						}
