...
/**
 * Holder of the
 * {@link com.sun.org.apache.xml.internal.security.transforms.Transform} steps
 * to be performed on the data. The input to the first Transform is the result
 * of dereferencing the <code>URI</code> attribute of the <code>Reference</code>
 * element. The output from the last Transform is the input for the
...
public class Transforms extends SignatureElementProxy {
	...
	public static final String TRANSFORM_C14N11_OMIT_COMMENTS = Canonicalizer.ALGO_ID_C14N11_OMIT_COMMENTS;

	/** Canonicalization - Recommended Canonical XML 1.1 with Comments */
	public static final String TRANSFORM_C14N11_WITH_COMMENTS = Canonicalizer.ALGO_ID_C14N11_WITH_COMMENTS;
	...
	public static final String TRANSFORM_ENVELOPED_SIGNATURE = Constants.SignatureSpecNS
			+ "enveloped-signature";
	...
	private static java.util.logging.Logger log = java.util.logging.Logger
			.getLogger(Transforms.class.getName());
	 ...
	 *
	 * @param element
	 *            is <code>Transforms</code> element
	 * @param BaseURI
	 ...
	 */
	public Transforms(Element element, String BaseURI) throws DOMException, XMLSignatureException,
			InvalidTransformException, TransformationException, XMLSecurityException {
	 ...
	 *
	 * @param transformURI
	 *            the URI form of transform that indicates which transformation
	 *            is applied to data
	 * @throws TransformationException
	 */
	...
	private void addTransform(Transform transform) {
		if (log.isLoggable(java.util.logging.Level.FINE)) {
			log.log(java.util.logging.Level.FINE,
					"Transforms.addTransform(" + transform.getURI() + ")");
		}
	 ...
	 *            the input for the <code>Transform</code>s
	...
	public XMLSignatureInput performTransforms(XMLSignatureInput xmlSignatureInput, OutputStream os)
		...
		try {
			...
			for (int i = 0; i < last; i++) {
				...
				if (log.isLoggable(java.util.logging.Level.FINE)) {
					log.log(java.util.logging.Level.FINE,
							"Perform the (" + i + ")th " + uri + " transform");
				}
	...
	public int getLength() {
		if (transforms == null) {
			transforms = XMLUtils.selectDsNodes(this.constructionElement.getFirstChild(),
					"Transform");
		}
	...
	/**
	 * Return the <it>i</it><sup>th</sup> <code>{@link Transform}</code>. Valid
	 * <code>i</code> values are 0 to <code>{@link #getLength}-1</code>.
	 *
	 * @param i
	 *            index of {@link Transform} to return
