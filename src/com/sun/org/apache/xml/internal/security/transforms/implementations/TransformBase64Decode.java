 ...
 * </A>. The base64 Transform element has no content. The input is decoded by
 ...
 * to an octet stream by performing operations logically equivalent to 1)
 * applying an XPath transform with expression self::text(), then 2) taking the
 * string-value of the node-set. Thus, if an XML element is identified by a
 * barename XPointer in the Reference URI, and its content consists solely of
 * base64 encoded character data, then this transform automatically strips away
 * the start and end tags of the identified element and any of its descendant
 * elements as well as any descendant comments and processing instructions. The
 * output of this transform is an octet stream.
 * </p>
 *
...
public class TransformBase64Decode extends TransformSpi {
    ...
    public static final String implementedTransformURI = Transforms.TRANSFORM_BASE64_DECODE;
