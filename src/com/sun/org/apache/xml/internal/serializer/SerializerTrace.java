...
public interface SerializerTrace {
	...
	/**
	 * Event type generated when characters might be written to an output
	 * stream, but these characters never are. They will ultimately be written
	 * out via EVENTTYPE_OUTPUT_CHARACTERS. This type is used as attributes are
	 * collected. Whenever the attributes change this event type is fired. At
	 * the very end however, when the attributes do not change anymore and are
	 * going to be ouput to the document the real characters will be written out
	 * using the EVENTTYPE_OUTPUT_CHARACTERS.
	 */
