...
public class NamespaceMappings {
	...
	/**
	 * Each entry (prefix) in this hashmap points to a Stack of URIs This maps a
	 * prefix (String) to a Stack of prefix mappings. All mappings in that
	 * retrieved stack have the same prefix, though possibly different URI's or
	 * depths. Such a stack must have mappings at deeper depths push later on
	 * such a stack. Mappings pushed earlier on the stack will have smaller
	 * values for MappingRecord.m_declarationDepth.
	 */
	private HashMap m_namespaces = new HashMap();

	/**
	 * The top of this stack contains the MapRecord of the last declared a
	 * namespace. Used to know how many prefix mappings to pop when leaving the
	 * current element depth. For every prefix mapping the current element depth
	...
	/**
	 * Given a namespace uri, and the namespaces mappings for the current
	...
	/**
	 * Pop, or undeclare all namespace definitions that are currently declared
	 ...
	 * @param saxHandler
	 *            The ContentHandler to notify of any endPrefixMapping() calls.
	...
	void popNamespaces(int elemDepth, ContentHandler saxHandler) {
		while (true) {
			...
			if (depth < elemDepth)
				return;
			/*
			 * the depth of the declared mapping is elemDepth or deeper so get
			 * rid of it
			 */
