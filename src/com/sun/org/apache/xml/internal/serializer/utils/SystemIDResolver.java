...
public final class SystemIDResolver {
	 ...
	 * property "user.dir" if it is available; if not (i.e. in an Applet perhaps
	 * which throws SecurityException) then we just return the relative path.
	 * The space and backslash characters are also replaced to generate a good
	 * absolute URI.
	 * </p>
	 *
	 * @param localPath
	...
	public static boolean isAbsoluteURI(String systemId) {
		/**
		 * http://www.ietf.org/rfc/rfc2396.txt Authors should be aware that a
		 * path segment which contains a colon character cannot be used as the
		 * first segment of a relative URI path (e.g., "this:that"), because it
		 * would be mistaken for a scheme name.
		 **/
		/**
		 * %REVIEW% Can we assume here that systemId is a valid URI? It looks
		 * like we cannot ( See discussion of this common problem in Bugzilla
		 * Bug 22777 ).
		 **/
	...
	/**
	 * Replace spaces with "%20" and backslashes with forward slashes in the
	...
	public static String getAbsoluteURI(String systemId) {
		...
		if (isAbsoluteURI(systemId)) {
			...
			if (systemId.startsWith("file:")) {
				...
				// Resolve the absolute path if the systemId starts with
				// "file:///"
				// or "file:/". Don't do anything if it only starts with
				// "file://".
				if (str != null && str.startsWith("/")) {
					if (str.startsWith("///") || !str.startsWith("//")) {
						// A Windows path containing a drive letter can be
						// relative.
						// A Unix path starting with "file:/" is always
						// absolute.
