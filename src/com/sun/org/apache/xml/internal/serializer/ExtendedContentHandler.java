...
/**
 * This interface describes extensions to the SAX ContentHandler interface. It
 * is intended to be used by a serializer. The methods on this interface will
...
abstract interface ExtendedContentHandler extends org.xml.sax.ContentHandler {
	 ...
	 * @param XSLAttribute
	 *            true if the added attribute is coming from an xsl:attribute
	 *            element
	 * @throws SAXException
	 */
	 ...
	 * just like the standard SAX method
	 * 
	 * <pre>
	 ...
	 *            startPrefixMapping(prefix,uri) call and the mapping applies to
	 *            the element to come. If false the mapping applies to the
	 *            current element.
	 ...
	 * @param isElement
	 *            true if it is an element name, false if it is an atttribute
	 *            name
	...
	/**
	 * Add a unique attribute to the current element. The attribute is
	 * guaranteed to be unique here. The serializer can write it out immediately
	 * without saving it in a table first. The integer flag contains information
	 * about the attribute, which helps the serializer to decide whether a
