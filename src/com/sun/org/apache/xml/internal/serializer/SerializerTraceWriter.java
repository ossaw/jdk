...
final class SerializerTraceWriter extends Writer implements WriterChain {
	...
	/**
	 * The real writer to immediately write to. This reference may be null, in
	 * which case nothing is written out, but only the trace events are fired
	 * for output.
	 */
	...
	/**
	 * The size of the internal buffer, just to keep too many events from being
	 * sent to the tracer
	 */
	...
	/**
	 * Constructor. If the writer passed in is null, then this
	 * SerializerTraceWriter will only signal trace events of what would have
	 * been written to that writer. If the writer passed in is not null then the
	 * trace events will mirror what is going to that writer. In this way tools,
	 ...
	 * These characters are never written to the real writer (m_writer) because
	 * that has already happened with every method call. This method simple
	 * informs the listener of what has already happened.
	 * 
	 * @throws IOException
	 */
	...
	public void write(final int c) throws IOException {
		 ...
		 * buffer can hold a few more characters than buf_length
		 */
		if (count >= buf_length)
			flushBuffer();
