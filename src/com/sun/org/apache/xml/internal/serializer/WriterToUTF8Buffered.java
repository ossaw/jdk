 ...
 */
/*
 * $Id: WriterToUTF8Buffered.java,v 1.2.4.1 2005/09/15 08:15:31 suresh_emailid
 * Exp $
 */
...
final class WriterToUTF8Buffered extends Writer implements WriterChain {
    ...
    /**
     * number of bytes that the byte buffer can hold. This is a fixed constant
     * is used rather than m_outputBytes.lenght for performance.
     */
    private static final int BYTES_MAX = 16 * 1024;
    /**
     * number of characters that the character buffer can hold. This is 1/3 of
     * the number of bytes because UTF-8 encoding can expand one unicode
     * character by up to 3 bytes.
     */
    ...
    /**
     * The internal buffer where data is stored. (sc & sb remove final to
     * compile in JDK 1.1.8)
     */
     ...
     * range <tt>0</tt> through <tt>m_outputBytes.length</tt>; elements
     * <tt>m_outputBytes[0]</tt> through <tt>m_outputBytes[count-1]</tt> contain
     * valid byte data.
     */
    private int count;
    ...
    /**
     * Create an buffered UTF-8 writer to write data to the specified underlying
    ...
    public void write(final int c) throws IOException {
        ...
        /*
         * If we are close to the end of the buffer then flush it. Remember the
         * buffer can hold a few more bytes than BYTES_MAX
         */
        if (count >= BYTES_MAX)
            flushBuffer();
    ...
    public void write(final char chars[], final int start, final int length)
        ...
        if (lengthx3 >= BYTES_MAX - count) {
            // The requested length is greater than the unused part of the
            // buffer
            flushBuffer();

            if (lengthx3 > BYTES_MAX) {
                /*
                 * The requested length exceeds the size of the buffer. Cut the
                 * buffer up into chunks, each of which will not cause an
                 * overflow to the output buffer m_outputBytes, and make
                 * multiple recursive calls. Be careful about integer overflows
                 * in multiplication.
                 */
                ...
                for (int chunk = 1; chunk <= chunks; chunk++) {
                    ...
                    if (c >= 0xD800 && c <= 0xDBFF) {
                        ...
                        if (end_chunk < start + length) {
                            ...
                            /*
                             * This is the last char of the last chunk, and it
                             * is the high char of a high/low pair with no low
                             * char provided. TODO: error message needed. The
                             * char array incorrectly ends in a high char of a
                             * high/low surrogate pair, but there is no
                             * corresponding low as the high is the last char
                             */
                            end_chunk--;
                        }
                    }
        ...
        {
            /*
             * This block could be omitted and the code would produce the same
             * result. But this block exists to give the JIT a better chance of
             * optimizing a tight and common loop which occurs when writing out
             * ASCII characters.
             */
            char c;
        ...
        for (; i < n; i++) {
             ...
             * Characters for UTF-8: [1111 0uuu] [10uu zzzz] [10yy yyyy] [10xx
             * xxxx]* Unicode: [1101 10ww] [wwzz zzyy] (high surrogate) [1101
             * 11yy] [yyxx xxxx] (low surrogate) * uuuuu = wwww + 1
             */
    ...
    public void write(final String s) throws IOException {
        ...
        if (lengthx3 >= BYTES_MAX - count) {
            ...
            if (lengthx3 > BYTES_MAX) {
                /*
                 * The requested length exceeds the size of the buffer, so break
                 * it up in chunks that don't exceed the buffer size.
                 */
                final int start = 0;
                int split = length / CHARS_MAX;
                final int chunks;
                ...
                for (int chunk = 1; chunk <= chunks; chunk++) {
                    ...
                    if (c >= 0xD800 && c <= 0xDBFF) {
                        ...
                        if (chunk == chunks) {
                            /*
                             * TODO: error message needed. The String
                             * incorrectly ends in a high char of a high/low
                             * surrogate pair, but there is no corresponding low
                             * as the high is the last char Recover by ignoring
                             * this last char.
                             */
                        }
                    }
