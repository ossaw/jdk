...
public final class ToUnknownStream extends SerializerBase {
    ...
    /**
     * A collection of namespace Prefix (only for first element) _namespaceURI
     * has the matching URIs for these prefix'
     */
    ...
    public ContentHandler asContentHandler() throws IOException {
        /*
         * don't return the real handler ( m_handler ) because that would expose
         * the real handler to the outside. Keep m_handler private so it can be
    ...
    public boolean startPrefixMapping(String prefix, String uri,
        ...
        if (m_firstTagNotEmitted) {
            if (m_firstElementName != null && shouldFlush) {
                /*
                 * we've already seen a startElement, and this is a prefix
                 * mapping for the up coming element, so flush the old element
    ...
    public void startElement(String namespaceURI, String localName,
            String elementName, Attributes atts) throws SAXException {
        ...
        if (m_firstTagNotEmitted) {
            ...
            if (m_firstElementName != null) {
                /*
                 * this is not the first element, but a later one. But we have
                 * the old element pending, so flush it out, then send this one
                 * on its way.
                 */
                flush();
                m_handler.startElement(namespaceURI, localName, elementName,
                        atts);
            } else {
                /*
                 * this is the very first element that we have seen, so save it
                 * for flushing later. We may yet get to know its URI due to
                 * added attributes.
                 */
                ...
                /*
                 * we don't want to call our own addAttributes, which merely
                 * delegates to the wrapped handler, but we want to add these
                 * attributes to m_attributes. So me must call super.
                 * addAttributes() In this case m_attributes is only used for
                 * the first element, after that this class totally delegates to
                 * the wrapped handler which is either XML or HTML.
                 */
                if (atts != null)
    ...
    private void initStreamOutput() throws SAXException {
        ...
        // Try to rule out if this is an not to be an HTML document based on
        // prefix
        ...
        if (firstElementIsHTML) {
            ...
            Properties htmlProperties = OutputPropertiesFactory
    ...
    private void emitFirstTag() throws SAXException {
        if (m_firstElementName != null) {
            ...
            // Output first tag
            m_handler.startElement(m_firstElementURI, null, m_firstElementName,
                    m_attributes);
            ...
            if (m_namespacePrefix != null) {
                ...
                for (int i = 0; i < n; i++) {
                    final String prefix = (String) m_namespacePrefix.elementAt(
                            i);
    ...
    private boolean isFirstElemHTML() {
        ...
        isHTML = getLocalNameUnknown(m_firstElementName).equalsIgnoreCase(
                "html");

        // Try to rule out if this is not to be an HTML document based on URI
        if (isHTML && m_firstElementURI != null && !EMPTYSTRING.equals(
                m_firstElementURI)) {
        ...
        if (isHTML && m_namespacePrefix != null) {
            /*
             * the first element has a name of "html", but lets check the
             * prefix. If the prefix points to a namespace with a URL that is
             * not "" then the doecument doesn't start with an <html> tag, and
             * isn't html
             */
            ...
            for (int i = 0; i < max; i++) {
                ...
                if (m_firstElementPrefix != null && m_firstElementPrefix.equals(
                        prefix) && !EMPTYSTRING.equals(uri)) {
    ...
    public void setTransformer(Transformer t) {
        ...
        if ((t instanceof SerializerTrace) && (((SerializerTrace) t)
                .hasTraceListeners())) {
    ...
    protected void firePseudoElement(String elementName) {
        ...
        if (m_tracer != null) {
            ...
            m_tracer.fireGenerateEvent(
                    SerializerTrace.EVENTTYPE_OUTPUT_PSEUDO_CHARACTERS, ch, 0,
                    ch.length);
        }
    }
}
