...
abstract public class ToStream extends SerializerBase {
	 ...
	 * will worry about whether single chars or surrogate pairs of high/low
	 * chars form characters in the output encoding.
	 */
	EncodingInfo m_encodingInfo = new EncodingInfo(null, null);

	/**
	 * Method reference to the sun.io.CharToByteConverter#canConvert method for
	 * this encoding. Invalid if m_charToByteConverter is null.
	 */
	...
	/**
	 * The system line separator for writing out line breaks. The default value
	...
	/**
	 * The length of the line seperator, since the write is done one character
	 * at a time.
	 */
	 ...
	 * The model will be normalized so that all whitespace is removed,and will
	 ...
	 * Must be called before calling any of the serialize methods. This method
	 * can be called multiple times and the xsl:output properties passed in the
	...
	private synchronized void init(Writer writer, Properties format, boolean defaultProperties,
		...
		setIndentAmount(OutputPropertyUtils
				.getIntProperty(OutputPropertiesFactory.S_KEY_INDENT_AMOUNT, format));
	 ...
	 *            true if the properties are the default properties
	 *
	 * @throws UnsupportedEncodingException
	 *             The encoding specified in the output format is not supported
	 */
	protected synchronized void init(OutputStream output, Properties format,
		...
		if (encoding.equalsIgnoreCase("UTF-8")) {
		...
		} else if (encoding.equals("WINDOWS-1250") || encoding.equals("US-ASCII")
			...
			try {
				...
				System.out.println("Warning: encoding \"" + encoding + "\" not supported"
						+ ", using " + Encodings.DEFAULT_MIME_ENCODING);
	...
	/**
	 * Set if the operating systems end-of-line line separator should be used
	 * when serializing. If set false NL character (decimal 10) is left alone,
	 ...
	 * <p>
	 * The encoding specified in the output properties is used, or if no
	...
	protected void indent(int depth) throws IOException {
		 ...
		 * default value is 0, so this extra test will run faster in that
		 * situation.
		 */
		if (m_indentAmount > 0)
	...
	/**
	 * Once a surrogate has been detected, write out the pair of characters if
	 * it is in the encoding, or if there is no encoding, otherwise write out an
	...
	protected int writeUTF16Surrogate(char c, char ch[], int i, int end) throws IOException {
		...
		if (!Encodings.isLowUTF16Surrogate(low)) {
			throw new IOException(
					Utils.messages.createMessage(MsgKey.ER_INVALID_UTF16_SURROGATE, new Object[] {
		...
		if (m_encodingInfo.isInEncoding(c, low)) {
			...
			if (encoding != null) {
				/*
				 * The output encoding is known, so somthing is wrong.
				 */
				...
				/*
				 * The output encoding is not known, so just write it out as-is.
				 */
				writer.write(ch, i, 2);
			}
		}
	 ...
	 *            length of chars.
	 * @param fromTextNode
	 *            true if the characters being processed are from a text node,
	...
	void writeNormalizedChars(char ch[], int start, int length, boolean isCData,
		...
		for (int i = start; i < end; i++) {
			...
			if (CharInfo.S_LINEFEED == c && useSystemLineSeparator) {
				...
				// m_cdataTagOpen = true;
				// }
			} else if (isCData && ((i < (end - 2)) && (']' == c) && (']' == ch[i + 1])
					&& ('>' == ch[i + 2]))) {
	...
	protected void cdata(char ch[], int start, final int length) throws org.xml.sax.SAXException {
		...
		try {
			 ...
			 * and if we are not already in the middle of a CDATA section
			 */
			 ...
			 * delay, so that we can merge CDATA sections on output. need to
			 * write closing delimiter later
			 */
			if (writeCDataBrackets) {
				/*
				 * if the CDATA section ends with ] don't leave it open as there
				 * is a chance that an adjacent CDATA sections starts with ]>.
	...
	public void characters(final char chars[], final int start, final int length)
		...
		try {
			...
			/*
			 * If there is some non-whitespace, mark that we may need to
			 * preserve this. This is only important if we have indentation on.
			 */
			if (i < end)
				m_ispreserve = true;
	 ...
	 * Control characters. This method is added to support Control Characters
	 * for XML 1.1 If a given character is TAB (0x09), LF (0x0A) or CR (0x0D),
	 * this method return false. Since they are whitespace characters, no
	 ...
	 * (0x2028) These are new end of line charcters added in XML 1.1. These
	 * characters must be written as Numeric Character References (NCR) in XML
	 * 1.1 output document.
	 *
	 * @param ch
	 * @return boolean
	...
	/**
	 * Process a dirty character and any preeceding clean characters that were
	 * not yet processed.
	 * 
	 * @param chars
	 ...
	 */
	private int processDirty(char[] chars, int end, int i, char ch, int lastDirty,
		...
		if (CharInfo.S_LINEFEED == ch && fromTextNode) {
			...
			startClean = accumDefaultEscape(m_writer, (char) ch, i, chars, end, fromTextNode,
					false);
			i = startClean - 1;
		}
	 ...
	 * @param fromTextNode
	 *            true if the characters being processed are from a text node,
	 *            false if the characters being processed are from an attribute
	 *            value.
	 * @param escLF
	 *            true if the linefeed should be escaped.
	 *
	...
	protected int accumDefaultEscape(Writer writer, char ch, int i, char[] chars, int len,
		...
		if (i == pos) {
			if (Encodings.isHighUTF16Surrogate(ch)) {
				...
				if (i + 1 >= len) {
					throw new IOException(
							Utils.messages.createMessage(MsgKey.ER_INVALID_UTF16_SURROGATE,
				...
				writer.write(Integer.toString(codePoint));
				writer.write(';');
				pos += 2; // count the two characters that went into writing out
							// this entity
			} else {
				/*
				 * This if check is added to support control characters in XML
				 * 1.1. If a character is a Control Character within C0 and C1
				 * range, it is desirable to write it out as Numeric Character
	...
	public void startElement(String namespaceURI, String localName, String name, Attributes atts)
		...
		try {
			...
			/*
			 * before we over-write the current elementLocalName etc. lets close
			 * out the old one (if we still need to)
			 */
			if (m_elemContext.m_startTagOpen) {
				closeStartTag();
				m_elemContext.m_startTagOpen = false;
		...
		// process the attributes now, because after this SAX call they might be
		// gone
		if (atts != null)
			addAttributes(atts);
	...
	public void processAttributes(java.io.Writer writer, int nAttrs)
		 ...
		 * "cheap" list for Stream serializer output accumulated over a series
		 * of calls to attribute(name,value)
		 */
		String encoding = getEncoding();
	...
	public void writeAttrString(Writer writer, String string, String encoding) throws IOException {
		...
		for (int i = 0; i < len;) {
			...
			if (escapingNotNeeded(ch) && (!m_charInfo.isSpecialAttrChar(ch))) {
				writer.write(ch);
				i++;
			} else { // I guess the parser doesn't normalize cr/lf in
						// attributes. -sb
		...
		try {
			...
			if (m_elemContext.m_startTagOpen) {
				...
				if (m_spaceBeforeClose)
					...
					writer.write("/>");
				/*
				 * don't need to pop cdataSectionState because this element
				 * ended so quickly that we didn't get to push the state.
				 */
	 ...
	 * that is soon to follow. Need to close any open start tag to make sure
	 * than any name space attributes due to this event are associated wih the
	 ...
	 *            The Namespace URI the prefix is mapped to.
	 * @param shouldFlush
	 *            true if any open tags need to be closed first, this will
	 *            impact which element the mapping applies to (open parent, or
	...
	public boolean startPrefixMapping(String prefix, String uri, boolean shouldFlush)
		 ...
		 * greater than the current depth because these mappings will apply to
		 * the next depth. This is in consideration that startElement() will
		 * soon be called
		 */
		...
		if (pushed) {
			/*
			 * Brian M.: don't know if we really needto do this. The callers of
			 * this object should have injected both startPrefixMapping and the
			 * attributes. We are just covering our butt here.
			 */
			String name;
			if (EMPTYSTRING.equals(prefix)) {
				...
				{ // that maps ns1 prefix to "" URI
					...
					/*
					 * for something like xmlns:abc="w3.pretend.org" the uri is
	...
	public void comment(char ch[], int start, int length) throws org.xml.sax.SAXException {
		 ...
		 *
		 * Simply mark that at this point if we do decide to indent that we
		 * should add a newline on the end of the current line before the
		 * indentation at the start of the next line.
		 */
		m_startNewLine = true;
	...
	public void startEntity(String name) throws org.xml.sax.SAXException {
		...
		if (!m_expandDTDEntities && !m_inExternalDTD) {
			/*
			 * Only leave the entity as-is if we've been told not to expand them
	...
	/**
	 * Tell if, based on space preservation constraints and the doIndent
	 * property, if an indent should occur.
	 *
	 ...
	 *            OutputProperties. Eventually this method should go away and a
	...
	private void setCdataSectionElements(String key, Properties props) {
		...
		if (null != s) {
			...
			// of a tokenizer so I can track whitespace inside of curly
			// brackets,
	 ...
	 * "official way to set URI and localName pairs. This method should be used
	 ...
	 * @throws SAXException
	 */
	protected String ensureAttributesNamespaceIsDeclared(String ns, String localName,
		...
		if (ns != null && ns.length() > 0) {
			...
			if (index > 0) {
				...
				if (uri != null && uri.equals(ns)) {
					// the prefix in the raw name is already maps to the given
					// namespace uri
					...
					this.addAttribute("http://www.w3.org/2000/xmlns/", prefixFromRawName,
							"xmlns:" + prefixFromRawName, "CDATA", ns, false);
				...
				if (prefix == null) {
					...
					this.addAttribute("http://www.w3.org/2000/xmlns/", prefix, "xmlns:" + prefix,
							"CDATA", ns, false);
				}
	...
	public void setContentHandler(ContentHandler ch) {
		// this method is really only useful in the ToSAXHandler classes but it
		// is
	 ...
	 * @param xslAttribute
	 *            true if this attribute is coming from an xsl:attribute
	 *            element.
	...
	public boolean addAttributeAlways(String uri, String localName, String rawName, String type,
		...
		// Don't use 'localName' as it gives incorrect value, rely only on
		// 'rawName'
		/*
		 * else { index = m_attributes.getIndex(uri, localName); }
		 */
		if (index >= 0) {
			...
			/*
			 * We've seen the attribute before. We may have a null uri or
			 * localName, but all we really want to re-set is the value anyway.
			 */
			...
			if (xslAttribute) {
				/*
				 * This attribute is from an xsl:attribute element so we take
				 * some care in adding it, e.g. <elem1 foo:attr1="1"
				 * xmlns:foo="uri1"> <xsl:attribute
				 * name="foo:attr2">2</xsl:attribute> </elem1>
				 *
				 * We are adding attr1 and attr2 both as attributes of elem1,
				 * and this code is adding attr2 (the xsl:attribute ). We could
				 * have a collision with the prefix like in the example above.
				 */
				...
				if (colonIndex > 0) {
					 ...
					 * for it (foo) already mapped at the current depth?
					 */
					if (existing_mapping != null
							&& existing_mapping.m_declarationDepth == m_elemContext.m_currentElemDepth
							&& !existing_mapping.m_uri.equals(uri)) {
						/*
						 * There is an existing mapping of this prefix, it
						 * differs from the one we need, and unfortunately it is
						 * at the current depth so we can not over-ride it.
						 */

						/*
						 * Are we lucky enough that an existing other prefix
						 * maps to this URI ?
						 */
						prefix = m_prefixMap.lookupPrefix(uri);
						if (prefix == null) {
							/*
							 * Unfortunately there is no existing prefix that
							 * happens to map to ours, so to avoid a prefix
							 * collision we must generated a new prefix to use.
							 * This is OK because the prefix URI mapping defined
							 * in the xsl:attribute is short in scope, just the
							 * xsl:attribute element itself, and at this point
							 * in serialization the body of the xsl:attribute,
							 * if any, is just a String. Right? . . . I sure
							 * hope so - Brian M.
							 */
							prefix = m_prefixMap.generateNextPrefix();
						}
				...
				try {
					/*
					 * This is our last chance to make sure the namespace for
					 * this attribute is declared, especially if we just
					 * generated an alternate prefix to avoid a collision (the
					 * new prefix/rawName will go out of scope soon and be lost
					 * ... last chance here.
					 */
					String prefixUsed = ensureAttributesNamespaceIsDeclared(uri, localName,
							rawName);
	...
	protected void firePseudoAttributes() {
		if (m_tracer != null) {
			try {
				...
				m_tracer.fireGenerateEvent(SerializerTrace.EVENTTYPE_OUTPUT_PSEUDO_CHARACTERS, ch,
						0, ch.length);
			} catch (IOException ioe) {
				// ignore ?
	 ...
	 * method writeAttrString() into a string buffer. In this manner trace
	 * events, and the real writing of attributes will use the same code.
	 */
	...
	/**
	 * Try's to reset the super class and reset this class for re-use, so that
	 * you don't need to create a new serializer (mostly for performance
	 * reasons).
	 *
	...
	private void resetToStream() {
		...
		/*
		 * The stream is being reset. It is one of ToXMLStream, ToHTMLStream ...
		 * and this type can't be changed so neither should m_charInfo which is
		 * associated with the type of Stream. Just leave m_charInfo as-is for
		 * the next re-use.
		 *
		 */
	...
	public void setEncoding(String encoding) {
		...
		if (old == null || !old.equals(encoding)) {
			...
			if (encoding != null && m_encodingInfo.name == null) {
				...
				String msg = Utils.messages.createMessage(MsgKey.ER_ENCODING_NOT_SUPPORTED,
						new Object[] { encoding });
				try {
	...
	static final class BoolStack {
		...
		/**
		 * Default constructor. Note that the default block size is very small,
		 * for small lists.
		 */
		public BoolStack() {
			this(32);
		...
		/**
		 * Looks at the object at the top of this stack without removing it from
		 * the stack. If the stack is empty, it returns false.
		 *
		 * @return the object at the top of this stack.
		 */
		...
		/**
		 * Looks at the object at the top of this stack without removing it from
		 * the stack. If the stack is empty, it returns true.
		 *
		 * @return the object at the top of this stack.
		 */
