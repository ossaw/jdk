 ...
 *
 * This class is used explicitly in code generated by XSLTC, so it is "public",
 * but it should be viewed as internal or package private, this is not an API.
 *
 * @xsl.usage internal
 */
public final class ToXMLStream extends ToStream {
    ...
    private static CharInfo m_xmlcharInfo =
            ...
            CharInfo.getCharInfoInternal(CharInfo.XML_ENTITIES_RESOURCE,
                    Method.XML);
    ...
    public void startDocumentInternal() throws org.xml.sax.SAXException {
        ...
        if (m_needToCallStartDocument) {
            ...
            /*
             * The call to getXMLVersion() might emit an error message and we
             * should emit this message regardless of if we are writing out an
             * XML header or not.
             */
    ...
    /**
     * Starts a whitespace preserving section. All characters printed within a
     * preserving section are printed without indentation and without
     * consolidating multiple spaces. This is equivalent to the
     * <tt>xml:space=&quot;preserve&quot;</tt> attribute. Only XML and HTML
    ...
    public void processingInstruction(String target, String data)
        ...
        if (target.equals(Result.PI_DISABLE_OUTPUT_ESCAPING)) {
            ...
            try {
                ...
                /**
                 * Before Xalan 1497, a newline char was printed out if not
                 * inside of an element. The whitespace is not significant is
                ...
                /*
                 * Don't write out any indentation whitespace now, because there
                 * may be non-whitespace text after this.
                 * Simply mark that at this point if we do decide to indent that
                 * we should add a newline on the end of the current line before
     ...
     *                     false if
     *                     declared within the elements opening tag.
     * @throws SAXException
     */
    public void addAttribute(String uri, String localName, String rawName,
            ...
            throws SAXException {
        if (m_elemContext.m_startTagOpen) {
            boolean was_added = addAttributeAlways(uri, localName, rawName,
                    type, value, xslAttribute);

            /*
             * We don't run this block of code if: 1. The attribute value was
             * only replaced (was_added is false). 2. The attribute is from an
             * xsl:attribute element (that is handled in the
             * addAttributeAlways() call just above. 3. The name starts with
             * "xmlns", i.e. it is a namespace declaration.
             */
            if (was_added && !xslAttribute && !rawName.startsWith("xmlns")) {
                String prefixUsed = ensureAttributesNamespaceIsDeclared(uri,
                        localName, rawName);
                if (prefixUsed != null && rawName != null && !rawName
                        .startsWith(prefixUsed)) {
            ...
            String msg = Utils.messages.createMessage(
                    MsgKey.ER_ILLEGAL_ATTRIBUTE_POSITION, new Object[] {
                            localName });
    ...
    /**
     * This method is used to notify the serializer of a namespace mapping (or
     * node) that applies to the current element whose startElement() call has
     * already been seen. The official SAX startPrefixMapping(prefix,uri) is to
     * define a mapping for a child element that is soon to be seen with a
     * startElement() call. The official SAX call does not apply to the current
     * element, hence the reason for this method.
     */
    ...
    /**
     * From XSLTC Declare a prefix to point to a namespace URI. Inform SAX
     * handler if this is a new prefix mapping.
     */
    protected boolean pushNamespace(String prefix, String uri) {
        try {
            if (m_prefixMap.pushNamespace(prefix, uri,
    ...
    /**
     * This method checks for the XML version of output document. If XML version
     * of output document is not specified, then output document is of version
     * XML 1.0. If XML version of output doucment is specified, but it is not
     * either XML 1.0 or XML 1.1, a warning message is generated, the XML
     * Version of output document is set to XML 1.0 and processing continues.
     * 
    ...
    private String getXMLVersion() {
        ...
        if (xmlVersion == null || xmlVersion.equals(XMLVERSION10)) {
            ...
            String msg = Utils.messages.createMessage(
                    MsgKey.ER_XML_VERSION_NOT_SUPPORTED, new Object[] {
                            xmlVersion });
            try {
                // Prepare to issue the warning message
                Transformer tran = super.getTransformer();
