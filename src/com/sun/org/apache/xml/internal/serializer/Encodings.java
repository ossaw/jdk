...
public final class Encodings extends Object {
	...
	/**
	 * A fast and cheap way to uppercase a String that is only made of printable
	 * ASCII characters.
	 * <p>
	 * This is not a public API.
	 * 
	 ...
	 * result tree. XSLT processors are required to respect values of UTF-8 and
	 * UTF-16. For other values, if the XSLT processor does not support the
	 * specified encoding it may signal an error; if it does not signal an error
	 * it should use UTF-8 or UTF-16 instead. The XSLT processor must not use an
	 * encoding whose name does not match the EncName production of the XML
	 * Recommendation [XML]. If no encoding attribute is specified, then the
	 * XSLT processor should use either UTF-8 or UTF-16."
	 *
	 * @param encoding
	 *            Reference to java-style encoding string, which may be null, in
		...
		if (null == encoding) {
			try {
				...
				if (null != encoding) {
					...
					/*
					 * See if the mime type is equal to UTF8. If you don't do
					 * that, then convertJava2MimeEncoding will convert 8859_1
					 * to "ISO-8859-1", which is not what we want, I think, and
	...
	private final static class EncodingInfos {
		...
		private Properties loadProperties() throws MalformedURLException, IOException {
			...
			try (InputStream is = openEncodingsFileStream()) {
				if (is != null) {
					...
					// critical,
					// and the output will be in any case _correct_ though maybe
					// not
		...
		// is tried in turn, until a charset can be loaded from one of the
		// names,
		...
		// As it happens, sometimes only one element of the set mime
		// names/javaName
		...
		// names is known but the javaName is unknown, by associating the
		// charset
		// loaded from one of the mime names with the unrecognized javaName.
		//
		// When none of the mime names or javaName are known - there's not much
		// we can
		// do... It can mean that this encoding is not supported for this
		// OS. If such a charset is ever use it will result in having all
		// characters
		// escaped.
		//
	...
	/**
	 * Return the unicode code point represented by the char. A bit of a dummy
	 * method, since all it does is return the char, but as an int value.
	 * <p>
	 * This is not a public API.
	 * 
	 * @param ch
