...
/**
 * This class is not a public API. It is only public because it is used in other
 * packages. This class converts SAX or SAX-like calls to a serialized document
 * for xsl:output method of "text".
 * 
 * @xsl.usage internal
 */
public final class ToTextStream extends ToStream {
	...
	public void characters(char ch[], int start, int length)
		...
		try {
			if (inTemporaryOutputState()) {
				/*
				 * leave characters un-processed as we are creating temporary
				 * output, the output generated by this serializer will be input
				 * to a final serializer later on and it will do the processing
				 * in final output state (not temporary output state).
				 * A "temporary" ToTextStream serializer is used to evaluate
				 * attribute value templates (for example), and the result of
				 * evaluating such a thing is fed into a final serializer later
				 * on.
				 */
	 ...
	 *                   The number of characters to read from the array.
	 * @param useLineSep
	 *                   true if the operating systems end-of-line separator
	 *                   should be
	 ...
	 * @throws             org.xml.sax.SAXException
	 */
	void writeNormalizedChars(final char ch[], final int start,
			final int length, final boolean useLineSep) throws IOException,
			...
			if (S_LINEFEED == c && useLineSep) {
				...
				if (codePoint != 0) {
					...
					final String msg = Utils.messages.createMessage(
							MsgKey.ER_ILLEGAL_CHARACTER, new Object[] {
									integralValue, encoding });
				...
				if (encoding != null) {
					...
					/*
					 * The output encoding is not known, so just write it out
					 * as-is.
					 */
					writer.write(c);
				}
	...
	/**
	 * Called when a Comment is to be constructed. Note that Xalan will normally
	 * invoke the other version of this method. %REVIEW% In fact, is this one
	 * ever needed, or was it a mistake?
	 *
	 * @param data
	 *             The comment data.
	 * @throws org.xml.sax.SAXException
