...
/**
 * This class acts as a base class for the XML "serializers" and the stream
 * serializers. It contains a number of common fields and methods.
 *
 * @xsl.usage internal
 */
public abstract class SerializerBase implements SerializationHandler, SerializerConstants {
     ...
     * Used to merge adjacent CDATA sections
     */
    protected boolean m_cdataTagOpen = false;

    /**
     * All the attributes of the current element, collected from
     * startPrefixMapping() calls, or addAddtribute() calls, or from the SAX
     * attributes in a startElement() call.
     */
    ...
    /**
     * Namespace support, that keeps track of currently defined prefix/uri
     * mappings. As processed elements come and go, so do the associated
     * mappings for that element.
     */
    protected NamespaceMappings m_prefixMap;

    /**
     * Handle for firing generate events. This interface may be implemented by
     * the referenced transformer object.
     */
     ...
     * frame is pushed at a startElement() and popped at an endElement(). This
     * frame contains information about the element, such as its namespace URI.
     */
    protected ElemContext m_elemContext = new ElemContext();

    /**
     * A utility buffer for converting Strings passed to character() methods to
     * character arrays. Reusing this buffer means not creating a new character
     * array everytime and it runs faster.
     */
    protected char[] m_charsBuff = new char[60];

    /**
     * A utility buffer for converting Strings passed to attribute methods to
     * character arrays. Reusing this buffer means not creating a new character
     * array everytime and it runs faster.
     */
    ...
    /**
     * If at runtime, when the qname of the attribute is known, another prefix
     * is specified for the attribute, then we can patch or hack the name with
     * this method. For a qname of the form "ns?:otherprefix:name", this
     * function patches the qname by simply ignoring "otherprefix". TODO: This
     * method is a HACK! We do not have access to the XML file, it sometimes
     ...
     * An element is currently open if a startElement() notification has occured
     * but the start of the element has not yet been written to the output. In
     * the stream case this means that we have not yet been forced to close the
     * elements opening tag by another notification, such as a character
     * notification.
     *
     * @param uri
    ...
    /**
     * Adds the given xsl:attribute to the set of collected attributes, but only
     ...
     * attributes are always added, regardless of whether on not an element is
     * currently open.
     * 
     * @param atts
    ...
    public void addAttributes(Attributes atts) throws SAXException {
        ...
        for (int i = 0; i < nAtts; i++) {
            ...
            addAttributeAlways(uri, atts.getLocalName(i), atts.getQName(i), atts.getType(i), atts.getValue(i),
                    false);
    ...
    /**
     * Push a boolean state based on if the name of the current element is found
     * in the list of qnames. A state is only pushed if there were some
     * cdata-section-names were specified.
     * <p>
     * Hidden parameters are the vector of qualified elements specified in
     * cdata-section-names attribute, and the m_cdataSectionStates stack onto
     * which whether the current element is in the list is pushed (true or
    ...
    protected boolean isCdataSection() {
        ...
        if (null != m_cdataSectionElements) {
            ...
            for (int i = 0; i < nElems; i += 2) {
                ...
                if (loc.equals(m_elemContext.m_elementLocalName) && subPartMatch(m_elemContext.m_elementURI,
                        uri)) {
                    b = true;
    ...
    public void setTransformer(Transformer t) {
        ...
        if ((m_transformer instanceof SerializerTrace) && (((SerializerTrace) m_transformer)
                .hasTraceListeners())) {
     ...
     * EVENTTYPE_OUTPUT_PSEUDO_CHARACTERS EVENTTYPE_OUTPUT_CHARACTERS which
    ...
    /**
     * Receive notification of the beginning of a document. This method is never
     * a self generated call, but only called externally.
     * <p>
     ...
     * whether from an external caller, or internally called in the serializer.
     * For historical reasons the serializer is flexible to startDocument() not
     * always being called. Even if no external call is made into
     * startDocument() this method will always be called as a self generated
     * internal startDocument, it handles what needs to be done at a
     * startDocument() call.
    ...
    protected void setDocumentInfo() {
        ...
        try {
            ...
            /*
             * String strEncoding = ((Locator2)m_locator).getEncoding(); if
             * (strEncoding != null) setEncoding(strEncoding);
             */
    ...
    final boolean inTemporaryOutputState() {
        /*
         * This is a hack. We should really be letting the serializer know that
         * it is in temporary output state with an explicit call, but from a
         * pragmatic point of view (for now anyways) having no output encoding
         * at all, not even the default UTF-8 indicates that the serializer is
         * being used for temporary RTF.
         */
    ...
    /**
     * This method adds an attribute the the current element, but should not be
     * used for an xsl:attribute child.
     * 
