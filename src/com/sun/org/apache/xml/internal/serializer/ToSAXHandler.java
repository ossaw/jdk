...
public abstract class ToSAXHandler extends SerializerBase {
    ...
    /**
     * A startPrefixMapping() call on a ToSAXHandler will pass that call on to
     * the wrapped ContentHandler, but should we also mirror these calls with
     * matching attributes, if so this field is true. For example if this field
     * is true then a call such as startPrefixMapping("prefix1","uri1") will
     * also cause the additional internally generated attribute
     * xmlns:prefix1="uri1" to be effectively added to the attributes passed to
     * the wrapped ContentHandler.
     */
    private boolean m_shouldGenerateNSAttribute = true;

    /**
     * If this is true, then the content handler wrapped by this serializer
     * implements the TransformState interface which will give the content
     * handler access to the state of the transform.
     */
    ...
    public void setContentHandler(ContentHandler _saxHandler) {
        ...
        if (m_lexHandler == null && _saxHandler instanceof LexicalHandler) {
            // we are not overwriting an existing LexicalHandler, and
            // _saxHandler
    ...
    /**
     * Set whether or not namespace declarations (e.g. xmlns:foo) should appear
     ...
     * generated additional attributes of elements that declare the namespace,
     * for example the attribute xmlns:prefix1="uri1"
     */
