...
public final class ToHTMLStream extends ToStream {
	...
	/**
	 * True if the current element is a block element. (seems like this needs to
	 * be a stack. -sb).
	 */
	...
	static void initTagReference(Trie m_elementFlags) {
		...
		m_elementFlags.put("BR", new ElemDesc(
				0 | ElemDesc.SPECIAL | ElemDesc.ASPECIAL | ElemDesc.EMPTY | ElemDesc.BLOCK));
		...
		m_elementFlags.put("MAP",
				new ElemDesc(0 | ElemDesc.SPECIAL | ElemDesc.ASPECIAL | ElemDesc.BLOCK));
		m_elementFlags.put("AREA", new ElemDesc(0 | ElemDesc.EMPTY | ElemDesc.BLOCK));
		m_elementFlags.put("LINK",
				new ElemDesc(0 | ElemDesc.HEADMISC | ElemDesc.EMPTY | ElemDesc.BLOCK));
		m_elementFlags.put("IMG", new ElemDesc(0 | ElemDesc.SPECIAL | ElemDesc.ASPECIAL
				| ElemDesc.EMPTY | ElemDesc.WHITESPACESENSITIVE));
		m_elementFlags.put("OBJECT", new ElemDesc(0 | ElemDesc.SPECIAL | ElemDesc.ASPECIAL
				| ElemDesc.HEADMISC | ElemDesc.WHITESPACESENSITIVE));
		...
		m_elementFlags.put("STYLE",
				new ElemDesc(0 | ElemDesc.HEADMISC | ElemDesc.RAW | ElemDesc.BLOCK));
		m_elementFlags.put("SCRIPT", new ElemDesc(
				0 | ElemDesc.SPECIAL | ElemDesc.ASPECIAL | ElemDesc.HEADMISC | ElemDesc.RAW));
		...
		elemDesc.setAttr("NOWRAP", ElemDesc.ATTREMPTY); // Internet-Explorer
														// extension
	 ...
	 * be method="html" otherwise we shouldn't even have a ToHTMLStream object
	 * here!)
	 *
	 * @param format
	...
	public void setOutputFormat(Properties format) {
		...
		m_specialEscapeURLs = OutputPropertyUtils
				.getBooleanProperty(OutputPropertiesFactory.S_USE_URL_ESCAPING, format);

		m_omitMetaTag = OutputPropertyUtils
				.getBooleanProperty(OutputPropertiesFactory.S_OMIT_META_TAG, format);
	...
	public static final ElemDesc getElemDesc(String name) {
		/*
		 * this method used to return m_dummy when name was null but now it
		 * doesn't check and and requires non-null name.
		 */
	...
	/**
	 * A Trie that is just a copy of the "static" one. We need this one to be
	 * able to use the faster, but not thread-safe method Trie.get2(name)
	 */
	private Trie m_htmlInfo = new Trie(m_elementFlags);

	/**
	 * Calls to this method could be replaced with calls to getElemDesc(name),
	 * but this one should be faster.
	 */
	...
	public void startElement(String namespaceURI, String localName, String name, Attributes atts)
		...
		try {
			...
			if ((elemFlags & ElemDesc.EMPTY) != 0) {
				...
				/*
				 * XSLTC sometimes calls namespaceAfterStartElement() so we need
	...
	public final void endElement(final String namespaceURI, final String localName,
			final String name) throws org.xml.sax.SAXException {
		...
		try {
			...
			if (elemEmpty) {
				...
				// This block of code can be removed if the corresponding block
				// of code
	 ...
	 * @throws org.xml.sax.SAXException
	 */
	protected void processAttribute(java.io.Writer writer, String name, String value,
			ElemDesc elemDesc) throws IOException {
		writer.write(' ');

		if (((value.length() == 0) || value.equalsIgnoreCase(name)) && elemDesc != null
	...
	public void writeAttrURI(final java.io.Writer writer, String string, boolean doURLEscaping)
		...
		for (int i = 0; i < end; i++) {
			...
			if ((ch < 32) || (ch > 126)) {
				...
				if (doURLEscaping) {
					...
					if (ch <= 0x7F) {
						...
						// Clear low 6 bits before rotate, put high 4 bits in
						// low byte,
					...
					} else if (Encodings.isHighUTF16Surrogate(ch)) // high
																	// surrogate
					{
						// I'm sure this can be done in 3 instructions, but I
						// choose
						// to try and do it exactly like it is done in the book,
						// at least
						// until we are sure this is totally clean. I don't
						// think performance
						// is a big issue with this particular function, though
						// I could be
						// wrong. Also, the stuff below clearly does more
						// masking than
						// it needs to do.
						...
						// "Note that the value of wwww from the high surrogate
						// bit pattern
						// is incremented to make the uuuuu bit pattern in the
						// scalar value
						...
						// put the middle 4 bits into the bottom of yyyyyy (byte
						// 3)
			...
			// identifier, we believe that it's OK to double escape quotes.
				...
				// Dmitri Ilyin: to check if '%' number number is invalid. It
				// must be checked if %xx is a sign, that would be encoded
				// The encoded signes are in Hex form. So %xx my be in form %3C
				// that is "<" sign. I will try to change here a little.
				...
				// HTML 4.01 reads, "Authors should use "&amp;" (ASCII decimal
				// 38)
				// instead of "&" to avoid confusion with the beginning of a
				// character
	...
	public void writeAttrString(final java.io.Writer writer, String string, String encoding)
		...
		for (int i = 0; i < end; i++) {
			...
			// System.out.println("m_attrCharsMap[ch]:
			// "+(int)m_attrCharsMap[ch]);
	...
	/**
	 * This method is used when a prefix/uri namespace mapping is indicated
	 * after the element was started with a startElement() and before and
	 * endElement(). startPrefixMapping(prefix,uri) would be used before the
	 * startElement() call.
	 * 
	 * @param uri
	 ...
	 * The caller has guaranted that this attribute is unique, which means that
	...
	static class Trie {
		 ...
		 * java.util.Hashtable The key must be a 7-bit ASCII string The value
		 * may be any Java Object One can get an object stored in a trie from
		 * its key, but the search is either case sensitive or case insensitive
		 ...
		 * 
		 * @param lowerCaseOnly
		 *            true if the search keys are to be loser case only, not
		 *            case insensitive.
		 */
		...
		/**
		 * Construct the trie from another Trie. Both the existing Trie and this
		 * new one share the same table for lookup, and it is assumed that the
		...
		public Trie(Trie existingTrie) {
			...
			// get a buffer just big enough to hold the longest key in the
			// table.
		...
		/**
		 * Get an object that matches the key. This method is faster than get(),
		...
		public Object get2(final String key) {
			...
			default: {
				/*
				 * Copy string into array. This is not thread-safe because it
				 * modifies the contents of m_charBuffer. If multiple threads
				 * were to use this Trie they all would be using this same array
				 * (not good). So this method is not thread-safe, but it is
				 * faster because converting to a char[] and looping over
				 * elements of the array is faster than a String's charAt(i).
				 */
