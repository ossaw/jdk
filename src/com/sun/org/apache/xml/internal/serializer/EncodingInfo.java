 ...
 * </pre>
 * 
 * which can be called if the character is not the high one in a surrogate pair
 * and:
 * 
 * <pre>
 ...
 * <p>
 * An EncodingInfo object is a node in a binary search tree. Such a node will
 * answer if a character is in the encoding, and do so for a given range of
 ...
 * <p>
 * This Class is not a public API, and should only be used internally within the
 * serializer.
 *
 * @xsl.usage internal
 */
public final class EncodingInfo extends Object {
    ...
    /**
     * A helper object that we can ask if a single char, or a surrogate UTF-16
     * pair of chars that form a single character, is in this encoding.
     */
    private InEncoding m_encoding;

    /**
     * This is not a public API. It returns true if the char in question is in
     * the encoding.
     * 
     * @param ch
    ...
    /**
     * This is not a public API. It returns true if the character formed by the
    ...
    /**
     * Create an EncodingInfo object based on the ISO name and Java name. If
     * both parameters are null any character will be considered to be in the
     * encoding. This is useful for when the serializer is in temporary output
    ...
    /**
     * A simple interface to isolate the implementation. We could also use some
     * new JRE 1.4 methods in another implementation provided we use reflection
     * with them.
     * <p>
    ...
    private interface InEncoding {
        ...
        /**
         * Returns true if the high/low surrogate pair forms a character that is
         * in the encoding.
         */
        public boolean isInEncoding(char high, char low);
    }
    ...
    private class EncodingImpl implements InEncoding {
        ...
        /**
         * m_first through m_last is the range of unicode values that this
         * object will return an answer on. It may delegate to a similar object
         * with a different range
         */
        final private int m_first;

        /**
         * m_explFirst through m_explLast is the range of unicode value that
         * this object handles explicitly and does not delegate to a similar
         * object.
         */
        ...
        /**
         * The object, of the same type as this one, that handles unicode values
         * in a range before the range explictly handled by this object, and to
         * which this object may delegate.
         */
        private InEncoding m_before;
        /**
         * The object, of the same type as this one, that handles unicode values
         * in a range after the range explictly handled by this object, and to
         * which this object may delegate.
         */
        private InEncoding m_after;

        /**
         * The number of unicode values explicitly handled by a single
         * EncodingInfo object. This value is tuneable, but is set to 128
         * because that covers the entire low range of ASCII type chars within a
         * single object.
         */
        private static final int RANGE = 128;

        /**
         * A flag to record if we already know the answer for the given unicode
         * value.
         */
        final private boolean m_alreadyKnown[] = new boolean[RANGE];
        /**
         * A table holding the answer on whether the given unicode value is in
         * the encoding.
         */
        ...
        private EncodingImpl(String encoding, int first, int last, int codePoint) {
            ...
            if (javaName != null) {
                ...
                if (0 <= m_explFirst && m_explFirst <= 127) {
                    ...
                    if ("UTF8".equals(javaName) || "UTF-16".equals(javaName) || "ASCII".equals(javaName)
                            || "US-ASCII".equals(javaName) || "Unicode".equals(javaName) || "UNICODE".equals(
                                    javaName) || javaName.startsWith("ISO8859")) {
                 ...
                 * writing to the final output tree. It is when writing to the
    ...
    /**
     * This is heart of the code that determines if a given character is in the
     * given encoding. This method is probably expensive, and the answer should
     * be cached.
     * <p>
     * This method is not a public API, and should only be used internally
     * within the serializer.
     * 
     * @param ch
    ...
    /**
     * This is heart of the code that determines if a given high/low surrogate
     * pair forms a character that is in the given encoding. This method is
     * probably expensive, and the answer should be cached.
     * <p>
     * This method is not a public API, and should only be used internally
     * within the serializer.
     * 
     ...
     * The method is not foolproof, because s.getBytes(encoding) has specified
     * behavior only if the characters are in the specified encoding. However
    ...
    private static boolean inEncoding(char ch, byte[] data) {
        ...
        if (data == null || data.length == 0) {
             ...
             */
                ...
                // We don't know for sure, but it looks like it is in the
                // encoding
                isInEncoding = true;
            }
        }
