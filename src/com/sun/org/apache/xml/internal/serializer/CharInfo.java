...
/**
 * This class provides services that tell if a character should have special
 * treatement, such as entity reference substitution or normalization of a
...
final class CharInfo {
    ...
    /**
     * The name of the HTML entities file. If specified, the file will be
     * resource loaded with the default class loader.
     */
    public static final String HTML_ENTITIES_RESOURCE = "com.sun.org.apache.xml.internal.serializer.HTMLEntities";

    /**
     * The name of the XML entities file. If specified, the file will be
     * resource loaded with the default class loader.
     */
     ...
     * other than quot (34), amp (38), lt (60) and gt (62) are defined in the
     * range 0 to 127.
     * 
     * @xsl.usage internal
     */
    ...
    /**
     * Array of values is faster access than a set of bits to quickly check
    ...
    /**
     * Array of values is faster access than a set of bits to quickly check
     * ASCII characters in text nodes.
     */
    ...
    /**
     * An array of bits to record if the character is in the set. Although
     * information in this array is complete, the isSpecialAttrASCII array is
     * used first because access to its values is common and faster.
     */
    private int array_of_bits[] = createEmptySetOfIntegers(65535);

    // 5 for 32 bit words, 6 for 64 bit words ...
    /*
     * This constant is used to shift an integer to quickly calculate which
     * element its bit is stored in. 5 for 32 bit words (int) , 6 for 64 bit
     * words (long)
     */
     ...
     * in the set. It is the index of the first element in the array
     * array_of_bits[] that is not used.
     */
    private int firstWordNotUsed;

    /**
     * Constructor that reads in a resource file that describes the mapping of
     * characters to entity references. This constructor is private, just to
     * force the use of the getCharInfo(entitiesResource) factory
     * Resource files must be encoded in UTF-8 and can either be properties
     * files with a .properties extension assumed. Alternatively, they can have
     * the following form, with no particular extension assumed:
     *
     * <pre>
    ...
    private CharInfo(String entitiesResource, String method, boolean internal) {
        ...
        if (entities != null) {
            ...
            try {
                ...
                if (is == null) {
                    throw new RuntimeException(Utils.messages.createMessage(MsgKey.ER_RESOURCE_COULD_NOT_FIND,
                ...
                throw new RuntimeException(Utils.messages.createMessage(MsgKey.ER_RESOURCE_COULD_NOT_LOAD,
                        new Object[] { entitiesResource, e.toString(), entitiesResource, e.toString() }));
            } finally {
                if (is != null) {
                    try {
                        is.close();
        ...
        /*
         * initialize the array isCleanTextASCII[] with a cache of values for
         * use by ToStream.character(char[], int , int) and the array
         * isSpecialTextASCII[] with the opposite values (all in the name of
         * performance!)
         */
        for (int ch = 0; ch < ASCII_MAX; ch++)
            if ((((0x20 <= ch || (0x0A == ch || 0x0D == ch || 0x09 == ch))) && (!get(ch))) || ('"' == ch)) {
        ...
        /*
         * Now that we've used get(ch) just above to initialize the two arrays
         * we will change by adding a tab to the set of special chars for XML
         * (but not HTML!). We do this because a tab is always a special
         * character in an XML attribute, but only a special character in XML
         * text if it has an entity defined for it. This is the reason for this
         * delay.
         */
    ...
    /**
     * Map a character to a String. For example given the character '>' this
     * method would return the fully decorated entity name "&lt;". Strings for
     * entity references are loaded from a properties file, but additional
     * mappings defined through calls to defineChar2String() are possible. Such
     * entity reference mappings could be over-ridden.
     * This is reusing a stored key object, in an effort to avoid heap activity.
     * Unfortunately, that introduces a threading risk. Simplest fix for now is
     * to make it a synchronized method, or to give up the reuse; I see very
     * little performance difference between them. Long-term solution would be
     * to replace the hashtable with a sparse array keyed directly from the
    ...
    /**
     * Tell if the character argument that is from an attribute value should
    ...
    /**
     * Tell if the character argument that is from a text node should have
     * special treatment.
     *
     * @param value
    ...
    /**
     * This method is used to determine if an ASCII character in a text node
    ...
    /**
     * Read an internal resource file that describes the mapping of characters
     ...
     * 1) attempt to load it as a ResourceBundle 2) try using the class loader
     * to find the specified file 3) try opening it as an URI
     * In case of 2 and 3, the resource file must be encoded in UTF-8 and have
    ...
    /**
     * For a given integer in the set it returns the single bit value used
     * within a given word that represents whether the integer is in the set or
     * not.
     */
     ...
     * This implementation uses an array of integers with 32 bits per integer.
    ...
    /**
     * If the character is a printable ASCII character then mark it as not clean
     * and needing replacement with a String on output.
     * 
     * @param ch
     */
    ...
    /**
     * If the character is a printable ASCII character then mark it as and not
     * needing replacement with a String on output.
     * 
     * @param ch
     */
     ...
     * You can set the char, then use it as a key.
