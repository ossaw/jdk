...
/**
 * This class is a stack frame that consists of information about the element
 * currently being processed by a serializer. Consider this example:
 * 
 * <pre>
 *   <A>
 *     <B1>
 ...
 * </pre>
 *
 * A stack frame will be pushed for "A" at depth 1, then another one for "B1" at
...
final class ElemContext {
	 ...
	 * If it is true the text children of the element should be output in CDATA
	 * section blocks.
	 */
	boolean m_isCdataSection;

	/**
	 * True if the current element has output escaping disabled. This is true
	 ...
	 * the frames via m_next and m_prev never changes either. Just the contents
	 * of the frames change as they are re-used. Only the reference to the
	 * current stack frame, which is held by the serializer is changed via a
	 * quick pop() or push().
	 */
	...
	final ElemContext pop() {
		/*
		 * a very simple pop. No clean up is done of the deeper stack frame. All
		 * deeper stack frames are still attached but dormant, just waiting to
		 * be re-used.
		 */
		return this.m_prev;
	}
	...
	/**
	 * This method pushes an element "stack frame" but with no initialization of
	 * values in that frame. This method is used for optimization purposes, like
	 * when pushing a stack frame for an HTML "IMG" tag which has no children
	 * and the stack frame will almost immediately be popped.
	 */
	final ElemContext push() {
		ElemContext frame = this.m_next;
		if (frame == null) {
			/*
			 * We have never been at this depth yet, and there is no stack frame
			 * to re-use, so we now make a new one.
			 */
			frame = new ElemContext(this);
			this.m_next = frame;
		}
		/*
		 * We shouldn't need to set this true because we should just be pushing
		 * a dummy stack frame that will be instantly popped. Yet we need to be
		 * ready in case this element does have unexpected children.
		 */
