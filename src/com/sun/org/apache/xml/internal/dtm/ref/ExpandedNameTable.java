 ...
 *
 * %OPT% The performance of the getExpandedTypeID() method is very important to
 * DTM building. To get the best performance out of this class, we implement a
 * simple hash algorithm directly into this class, instead of using the
 * inefficient java.util.Hashtable. The code for the get and put operations are
 * combined in getExpandedTypeID() method to share the same hash calculation
...
public class ExpandedNameTable {
	...
	/**
	 * The default load factor of the Hashtable. This is used to calcualte the
	 * threshold.
	 */
	private static float m_loadFactor = 0.75f;

	/**
	 * The initial capacity of the hash table. Use a bigger number to avoid the
	 * cost of expanding the table.
	 */
	private static int m_initialCapacity = 203;

	/**
	 * The capacity of the hash table, i.e. the size of the internal HashEntry
	 * array.
	 */
	...
	/**
	 * The internal array to store the hash entries. Each array member is a slot
	 * for a hash bucket.
	 */
	 ...
	 * type, return an ID. If the expanded-name does not exist in the internal
	 * tables, the entry will be created, and the ID will be returned. Any
	 * additional nodes that are created that have this expanded name will use
	 * this ID.
	 *
	 * @param namespace
	 *            The namespace
	...
	/**
	 * Increases the capacity of and internally reorganizes the hashtable, in
	 * order to accommodate and access its entries more efficiently. This method
	 * is called when the number of keys in the hashtable exceeds this
	...
	/**
	 * Inner class which represents a hash table entry. The field next points to
	 * the next entry which is hashed into the same bucket in the case of
	 * "hash collision".
	 */
