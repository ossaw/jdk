...
/**
 * The <code>DTMDefaultBase</code> class serves as a helper base for DTMs. It
 * sets up structures for navigation and type, while leaving data management and
 * construction to the derived classes.
 */
public abstract class DTMDefaultBase implements DTM {
	...
	/**
	 * These hold indexes to elements based on namespace and local name. The
	 * base lookup is the the namespace. The second lookup is the local name,
	 * and the last array contains the the first free element at the start, and
	 * the list of element handles following.
	 */
	...
	/**
	 * Add a node to the element indexes. The node will not be added unless it's
	 * an element.
	 *
	...
	/**
	 * Find the first matching element from the index at or after the given
	 * node.
	 *
	 * @param nsIndex
	...
	public String dumpNode(int nodeHandle) {
		...
		return "[" + nodeHandle + ": " + typestring + "(0x"
	...
	/**
	 * Given a node handle, return a node identity. If extended addressing has
	 * been used (multiple DTM IDs), we need to map the high bits of the
	...
	public int getTypedFirstChild(int nodeHandle, int nodeType) {
		...
		if (nodeType < DTM.NTYPES) {
			for (firstChild = _firstch(
					makeNodeIdentity(nodeHandle)); firstChild != DTM.NULL; firstChild = _nextsib(
							firstChild)) {
	...
	/**
	 * Given a node handle, advance to its next sibling. If not yet resolved,
	 * waits for more nodes to be added to the document and tries again.
	 * 
	 * @param nodeHandle
	 *            int Handle of the node.
	...
	/**
	 * Given a node handle, advance to the next attribute. If an attr, we
	 * advance to the next attr on the same node. If not an attribute, we return
	 * NULL.
	 *
	 * @param nodeHandle
	 *            int Handle of the node.
	 ...
	 *
	 * @param identity
	 *            int identity of the attribute node. This <strong>must</strong>
	 ...
	 *
	 * @param baseHandle
	 *            handle to original node from where the first namespace was
	 *            relative to (needed to return nodes in document order).
	 * @param nodeHandle
	 *            A namespace handle for which we will find the next node.
	 * @param inScope
	 ...
	 * construction. PLEASE NOTE that most people should use
	 * getOwnerDocument(nodeHandle) instead; this version of the operation is
	...
	/**
	 * Given a node handle, find the owning document node. Unlike the DOM, this
	...
	public int getExpandedTypeID(int nodeHandle) {
		// %REVIEW% This _should_ only be null if someone asked the wrong DTM
		// about the node...
	 ...
	 *
	 * @param namespace
	 *            The namespace URI, which may be null, may be an empty string
	 *            (which will be the same as null), or may be a namespace URI.
	 * @param localName
	...
	public int getNodeIdent(int nodeHandle) {
		/*
		 * if (nodeHandle != DTM.NULL) return nodeHandle & m_mask; else return
		 * DTM.NULL;
		 */
	...
	public int getNodeHandle(int nodeId) {
		/*
		 * if (nodeId != DTM.NULL) return nodeId | m_dtmIdent; else return
		 * DTM.NULL;
		 */
