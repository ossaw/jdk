 ...
 * to respond more like the TreeWalker's "current node" semantics. However,
...
public interface DTMIterator {
	...
	/**
	 * Reject the node. Same behavior as FILTER_SKIP. (In the DOM these differ
	 * when applied to a TreeWalker but have the same result when applied to a
	 * NodeIterator).
	 */
	...
	/**
	 * The root node of the <code>DTMIterator</code>, as specified when it was
	 * created. Note the root node is not the root node of the document tree,
	 * but the context node from where the iteration begins and ends.
	 *
	...
	/**
	 * Reset the root node of the <code>DTMIterator</code>, overriding the value
	 * specified when it was created. Note the root node is not the root node of
	 ...
	 * iterator. The available set of constants is defined above. Nodes not
	 * accepted by <code>whatToShow</code> will be skipped, but their children
	 ...
	 * the first call to <code>nextNode()</code> returns that root or (if it is
	 * rejected by the filters) the first node within its subtree which is not
	 * filtered out.
	 * 
	 ...
	 * 
	 * @return The previous node handle in the set being iterated over, or
	 *         <code>DTM.NULL</code> if there are no more members in that set.
	 */
	...
	/**
	 * Get the current node in the iterator. Note that this differs from the
	 * DOM's NodeIterator, where the current position lies between two nodes (as
	 * part of the maintain-relative-position semantic).
	 *
	...
	/**
	 * Get the current position within the cached list, which is one less than
	 * the next nextNode() call will retrieve. i.e. if you call getCurrentPos()
	 * and the return is 0, the next fetch will take place at index 1.
	 *
	 ...
	 * 0 to <code>length-1</code> inclusive. Note that this requires running the
	 * iterator to completion, and presumably filling the cache.
	 *
	 ...
	 * document order.
	 *
	 * @return true if all the nodes in the iteration well be returned in
	 *         document order.
	 */
