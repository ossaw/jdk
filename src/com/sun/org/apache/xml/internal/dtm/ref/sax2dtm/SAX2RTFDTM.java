 ...
 * deen separated for performance reasons. (In fact, one could even argue about
...
public class SAX2RTFDTM extends SAX2DTM {
    ...
    /**
     * Tail-pruning mark: Number of namespace declaration sets in use %REVIEW% I
     * don't think number of NS sets is ever different from number of NS
     * elements. We can probabably reduce these to a single stack and save some
     * storage.
     */
    IntStack mark_nsdeclset_size = new IntStack();
    /**
     * Tail-pruning mark: Number of naespace declaration elements in use
     * %REVIEW% I don't think number of NS sets is ever different from number of
     * NS elements. We can probabably reduce these to a single stack and save
     * some storage.
     */
    ...
    /**
     * Given a DTM, find the owning document node. In the case of SAX2RTFDTM,
     ...
     * DTMs used for RTFs Must Not take advantage of that feature, but this
     * seems excessively fragile. Another, much less attractive, would be to
     * just let them leak... Nah.
     *
     * @return true if and only if the pop completely emptied the RTF. That
     *         response is used when determining how to unspool
