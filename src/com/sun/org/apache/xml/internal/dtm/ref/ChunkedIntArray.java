 ...
 * driving up code size. If you want to inline it, feel free. The Java text
...
final class ChunkedIntArray {
	...
	int appendSlot(int w0, int w1, int w2, int w3) {
		/*
		 * try { int newoffset = (lastUsed+1)*slotsize; fastArray[newoffset] =
		 * w0; fastArray[newoffset+1] = w1; fastArray[newoffset+2] = w2;
		 * fastArray[newoffset+3] = w3; return ++lastUsed; }
	...
	/**
	 * Retrieve an integer from the CIA by record number and column within the
	 * record, both 0-based (though position 0 is reserved for special
	 * purposes).
	 * 
	 * @param position
	 *                 int Record number
	...
	/**
	 * Overwrite the integer found at a specific record and column. Used to
	 * back-patch existing records, most often changing their "next sibling"
	...
	/**
	 * Overwrite an entire (4-integer) record at the specified index. Mostly
	...
	/**
	 * Retrieve the contents of a record into a user-supplied buffer array. Used
	 * to reduce addressing overhead when code will access several columns of
	 * the record.
	 * 
	 * @param position
	 *                 int Record number
	...
	void readSlot(int position, int[] buffer) {
		/*
		 * try { System.arraycopy(fastArray, position*slotsize, buffer, 0,
		 * slotsize); } catch(ArrayIndexOutOfBoundsException aioobe)
		 */
		{
