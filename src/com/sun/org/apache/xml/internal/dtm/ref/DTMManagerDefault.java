 ...
 * %REVIEW% There is currently a reentrancy issue, since the finalizer for
 * XRTreeFrag (which runs in the GC thread) wants to call DTMManager.release(),
 * and may do so at the same time that the main transformation thread is
 * accessing the manager. Our current solution is to make most of the manager's
 * methods <code>synchronized</code>. Early tests suggest that doing so is not
 * causing a significant performance hit in Xalan. However, it should be noted
 * that there is a possible alternative solution: rewrite release() so it merely
 * posts a request for release onto a threadsafe queue, and explicitly process
 * that queue on an infrequent basis during main-thread activity (eg, when
 * getDTM() is invoked). The downside of that solution would be a greater delay
...
public class DTMManagerDefault extends DTMManager {
     ...
     * One DTM may have several prefix numbers, if extended node indexing is in
     * use; in that case, m_dtm_offsets[] will used to control which prefix maps
     * to which section of the DTM.
    ...
    /**
     * Map from DTM identifier numbers to offsets. For small DTMs with a single
     * identifier, this will always be 0. In overflow addressing, where
     ...
     * This array grows as necessary; see addDTM(). Growth is uncommon... but
     * access needs to be blindingly fast since it's used in node addressing.
     * (And at the moment, that includes accessing it from DTMDefaultBase, which
     * is why this is not Protected or Private.)
     */
    int m_dtm_offsets[] = new int[256];

    /**
     * The cache for XMLReader objects to be used if the user did not supply an
     * XMLReader for a SAXSource or supplied a StreamSource.
     */
    protected XMLReaderManager m_readerManager = null;

    /**
     * The default implementation of ContentHandler, DTDHandler and
     * ErrorHandler.
     */
     ...
     * "base DTM ID", with offset 0. The other version of addDTM should be used
     * if you want to add "extended" DTM IDs with nonzero offsets.
     *
     * @param dtm
     ...
     *        obtained by adding this offset to the node-number field of
     *        the
     *        public DTM Handle. For the first DTM ID accessing each DTM,
     *        this is 0; for overflow addressing it will be a multiple of
    ...
    synchronized public void addDTM(DTM dtm, int id, int offset) {
        if (id >= IDENT_MAX_DTMS) {
            ...
            throw new DTMException(XMLMessages.createXMLMessage(XMLErrorResources.ER_NO_DTMIDS_AVAIL, null)); // "No
                                                                                                              // more
                                                                                                              // DTM
                                                                                                              // IDs
                                                                                                              // are
        ...
        if (oldlen <= id) {
            ...
            //
            // %REVIEW% Should throw a more diagnostic error if we go over the
            // max...
     ...
     * source. If the unique flag is true, a new instance will always be
     ...
     * true, and incremental and doIndexing are both false, we return an
     * instance of SAX2RTFDTM, which see.
     * (I think more parameters will need to be added for error handling, and
     * entity resolution, and more explicit control of the RTF situation).
     *
     * @param source
     ...
     */
    synchronized public DTM getDTM(Source source, boolean unique, DTMWSFilter whiteSpaceFilter,
        ...
        if ((null != source) && source instanceof DOMSource) {
            DOM2DTM dtm = new DOM2DTM(this, (DOMSource) source, documentID, whiteSpaceFilter, xstringFactory,
                    doIndexing);
            ...
            if (isSAXSource || isStreamSource) {
                ...
                try {
                    ...
                    if (null == source) {
                        ...
                        if (null != urlOfSource) {
                            try {
                                ...
                                // %REVIEW% Is there a better way to send a
                                // warning?
                    ...
                    if (source == null && unique && !incremental && !doIndexing) {
                        // Special case to support RTF construction into shared
                        // DTM.
                        ...
                        dtm = new SAX2RTFDTM(this, source, documentID, whiteSpaceFilter, xstringFactory,
                                doIndexing);
                    }
                    /**************************************************************
                     * // EXPERIMENTAL 3/22/02 else if(JKESS_XNI_EXPERIMENT &&
                     * m_incremental) { dtm = new XNI2DTM(this, source,
                     * documentID, whiteSpaceFilter, xstringFactory,
                     * doIndexing); }
                     **************************************************************/
                    // Create the basic SAX2DTM.
                    else {
                        dtm = new SAX2DTM(this, source, documentID, whiteSpaceFilter, xstringFactory,
                                doIndexing);
                    }

                    // Go ahead and add the DTM to the lookup table. This needs
                    // to be
                    // done before any parsing occurs. Note offset 0, since
                    // we've just
                    // created a new DTM.
                    addDTM(dtm, dtmPos, 0);

                    boolean haveXercesParser = (null != reader) && (reader.getClass().getName().equals(
                    ...
                    // If the reader is null, but they still requested an
                    // incremental
                    // build, then we still want to set up the
                    // IncrementalSAXSource stuff.
                    ...
                    /* || ((null == reader) && incremental) */) {
                        ...
                        if (haveXercesParser) {
                            ...
                            try {
                                coParser = (IncrementalSAXSource) Class.forName(
                        ...
                        if (coParser == null) {
                            // Create a IncrementalSAXSource to run on the
                            // secondary thread.
                            if (null == reader) {
                        ...
                        /**************************************************************
                         * // EXPERIMENTAL 3/22/02 if (JKESS_XNI_EXPERIMENT &&
                         * m_incremental && dtm instanceof XNI2DTM && coParser
                         * instanceof IncrementalSAXSource_Xerces) {
                         * com.sun.org.apache.xerces.internal.xni.parser.
                         * XMLPullParserConfiguration xpc=
                         * ((IncrementalSAXSource_Xerces)coParser)
                         * .getXNIParserConfiguration(); if (xpc!=null) { //
                         * Bypass SAX; listen to the XNI stream
                         * ((XNI2DTM)dtm).setIncrementalXNISource(xpc); } else {
                         * // Listen to the SAX stream (will fail,
                         * diagnostically...)
                         * dtm.setIncrementalSAXSource(coParser); } } else
                         ***************************************************************/

                        // Have the DTM set itself up as IncrementalSAXSource's
                        // listener.
                        ...
                        try {
                            // Launch parsing coroutine. Launches a second
                            // thread,
                    ...
                    if (reader != null && !(m_incremental && incremental)) {
                        ...
                        // Reset the LexicalHandler to null after creating the
                        // DTM.
                        try {
     ...
     * be non-optimal, and there is no guarantee that the node will be found in
     * any particular DTM.
     *
     * @param node
    ...
    synchronized public int getDTMHandleFromNode(org.w3c.dom.Node node) {
        if (null == node)
            throw new IllegalArgumentException(XMLMessages.createXMLMessage(
                    XMLErrorResources.ER_NODE_NON_NULL, null)); // "node
                                                                                                                                // must
                                                                                                                                // be
                                                                                                                                // non-null
                                                                                                                                // for
        ...
        if (node instanceof com.sun.org.apache.xml.internal.dtm.ref.DTMNodeProxy)
            ...
            // Since the real root of our tree may be a DocumentFragment, we
            // need to
            // use getParent to find the root, instead of getOwnerDocument.
            // Otherwise
            // DOM2DTM#getHandleOfNode will be very unhappy.
            Node root = node;
            Node p = (root.getNodeType() == Node.ATTRIBUTE_NODE) ? ((org.w3c.dom.Attr) root).getOwnerElement()
                    : root.getParentNode();
            for (; p != null; p = p.getParentNode()) {
                root = p;
            }

            DOM2DTM dtm = (DOM2DTM) getDTM(new javax.xml.transform.dom.DOMSource(root), false, null, true,
                    true);

            int handle;

            if (node instanceof com.sun.org.apache.xml.internal.dtm.ref.dom2dtm.DOM2DTMdefaultNamespaceDeclarationNode) {
                // Can't return the same node since it's unique to a specific
                // DTM,
            ...
            if (DTM.NULL == handle)
                throw new RuntimeException(XMLMessages.createXMLMessage(
                        XMLErrorResources.ER_COULD_NOT_RESOLVE_NODE, null)); // "Could
                                                                                                                                     // not
                                                                                                                                     // resolve
                                                                                                                                     // the
                                                                                                                                     // node
     ...
     * from this URI. It may return null if any SAX2-conformant XML parser can
     * be used, or if getInputSource() will also return null. The parser must be
     * free for use (i.e., not currently in use for another parse(). After use
     * of the parser is completed, the releaseXMLReader(XMLReader) must be
     * called.
     *
     * @param inputSource
    ...
    synchronized public XMLReader getXMLReader(Source inputSource) {
        ...
        try {
            XMLReader reader = (inputSource instanceof SAXSource) ? ((SAXSource) inputSource).getXMLReader()
                    : null;

            // If user did not supply a reader, ask for one from the reader
            // manager
            if (null == reader) {
     ...
     * specified on the SAXSource object by the application code. Such a reader
     * should still be passed to releaseXMLReader, but the reader manager will
     * only re-use XMLReaders that it created.
     *
     * @param reader
    ...
    /**
     * Given a DTM, find the ID number in the DTM tables which addresses the
     * start of the document. If overflow addressing is in use, other DTM IDs
     * may also be assigned to this DTM.
     *
     * @param dtm
     *        The DTM which (hopefully) contains this node.
     * @return The DTM ID (as the high bits of a NodeHandle, not as our internal
     *         index), or -1 if the DTM doesn't belong to this manager.
     */
    ...
    /**
     * Release the DTMManager's reference(s) to a DTM, making it unmanaged. This
     * is typically done as part of returning the DTM to the heap after we're
     * done with it.
     *
     * @param dtm
     *        the DTM to be released.
     * @param shouldHardDelete
     *        If false, this call is a suggestion rather than
     *        an order, and
     *        we may not actually release the DTM. This is
     *        intended to
     *        support intelligent caching of documents... which
     *        is not
     *        implemented in this version of the DTM manager.
    ...
    synchronized public boolean release(DTM dtm, boolean shouldHardDelete) {
        ...
        if (dtm instanceof DTMDefaultBase) {
            com.sun.org.apache.xml.internal.utils.SuballocatedIntVector ids = ((DTMDefaultBase) dtm)
                    .getDTMIDs();
     ...
     * NEEDSDOC @param whatToShow NEEDSDOC @param filter NEEDSDOC @param
    ...
    synchronized public DTMIterator createDTMIterator(int whatToShow, DTMFilter filter,
        ...
        /**
         * @todo: implement this com.sun.org.apache.xml.internal.dtm.DTMManager
         *        abstract method
         */
        return null;
    }
