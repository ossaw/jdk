 ...
 * <p>
 * It does _not_ attempt to address the "node identity" question; no effort is
 * made to prevent the creation of multiple proxies referring to a single DTM
 * node. Users can create a mechanism for managing this, or relinquish the use
 * of "==" and use the .sameNodeAs() mechanism, which is under consideration for
 * future versions of the DOM.
 * <p>
...
public class DTMNodeProxy implements Node, Document, Text, Element, Attr, ProcessingInstruction,
	...
	/**
	 * Ask whether we support a given DOM feature. In fact, we do not _fully_
	 * support any DOM feature -- we're a read-only subset -- so arguably we
	...
	/**
	 * This is a bit of a problem in DTM, since a DTM may be a Document Fragment
	 ...
	 * <a href='http://www.w3.org/TR/2001/WD-DOM-Level-3-Core-20010605'>Document
	 ...
	 * <code>DOMException</code>. In case of error, the behavior is undefined.
	...
	static class DTMNodeProxyImplementation implements DOMImplementation {
		 ...
		 * simply treat the whole DOM as read-only, failing on the methods we
		 * can't support. I'm not sure which would be more useful to the caller.
		 */
		@Override
		 ...
		 *         <code>Node</code> interface, it must delegate to the primary
		 *         core <code>Node</code> and not return results inconsistent
	 ...
	 * APIs of the specified feature and version. The specialized object may
	 * also be obtained by using binding-specific casting methods but is not
	...
	public boolean isEqualNode(Node arg) {
		...
		/*
		 * if (getBaseURI() == null) { if (((NodeImpl) arg).getBaseURI() !=
		 * null) { return false; } } else if (!getBaseURI().equals(((NodeImpl)
		 * arg).getBaseURI())) { return false; }
		 */
	...
	/**
	 * DOM Level 3 Look up the namespace URI associated to the given prefix,
	 * starting from this node. Use lookupNamespaceURI(null) to lookup the
	 * default namespace
	 *
	 * @param namespaceURI
	...
	/**
	 * DOM Level 3 This method checks if the specified <code>namespaceURI</code>
	...
	public boolean isDefaultNamespace(String namespaceURI) {
		 ...
		 * } if (this.hasAttributes()) { ElementImpl elem = (ElementImpl)this;
		 * NodeImpl attr =
		 * (NodeImpl)elem.getAttributeNodeNS("http://www.w3.org/2000/xmlns/",
	...
	/**
	 * DOM Level 3 The location of the document or <code>null</code> if
	 * undefined. <br>
	...
	public Text replaceWholeText(String content) throws DOMException {
		 ...
		 * } Text currentNode = null; if (isReadOnly()){ Text newNode =
		 * this.ownerDocument().createTextNode(content); if (parent !=null) { //
		 * check if node in the tree parent.insertBefore(newNode, this);
		 * parent.removeChild(this); currentNode = newNode; } else { return
		 * newNode; } } else { this.setData(content); currentNode = this; } Node
	...
	/**
	 * DOM Level 3 Returns all text of <code>Text</code> nodes
	 * logically-adjacent text nodes to this node, concatenated in document
	 * order.
	 * 
	 * @since DOM Level 3
	 */
	...
	public String getWholeText() {
		 ...
		 * { return data; } StringBuffer buffer = new StringBuffer(); if (data
	...
	/**
	 * DOM Level 3 Returns whether this text node contains whitespace in element
