 ...
 *
 * Note that it doesn't necessarily represent a full Document tree. You can wrap
 * a DOM2DTM around a specific node and its subtree and the right things should
 * happen. (I don't _think_ we currently support DocumentFrgment nodes as roots,
 * though that might be worth considering.)
 *
 * Note too that we do not currently attempt to track document mutation. If you
 * alter the DOM after wrapping DOM2DTM around it, all bets are off.
 */
public class DOM2DTM extends DTMDefaultBaseIterators {
	...
	/**
	 * The current position in the DOM tree. Last node examined for possible
	 * copying to DTM.
	 */
	...
	/**
	 * The top of the subtree. %REVIEW%: 'may not be the same as m_context if
	 * "//foo" pattern.'
	 */
	...
	/**
	 * true if ALL the nodes in the m_root subtree have been processed; false if
	 * our incremental build has not yet finished scanning the DOM tree.
	 */
	transient private boolean m_nodesAreProcessed;

	/**
	 * The node objects. The instance part of the handle indexes directly into
	 * this vector. Each DTM node may actually be composed of several DOM nodes
	 * (for example, if logically-adjacent Text/CDATASection nodes in the DOM
	 * have been coalesced into a single DTM Text node); this table points only
	 * to the first in that sequence.
	 */
	 ...
	 * @param domSource
	 *                         the DOM source that this DTM will wrap.
	 * @param dtmIdentity
	 ...
	 *
	 * @param node
	 *                        The node that is to be added to the DTM.
	 * @param parentIndex
	 *                        The current parent index.
	 * @param previousSibling
	 *                        The previous sibling index.
	 * @param forceNodeType
	 *                        If not DTM.NULL, overrides the DOM node type. Used
	 *                        to force
	 *                        nodes to Text rather than CDATASection when their
	 *                        coalesced
	 *                        value includes ordinary Text nodes (current DTM
	 *                        behavior).
	 *
	 * @return The index identity of the node that was added.
	 */
	protected int addNode(Node node, int parentIndex, int previousSibling,
			int forceNodeType) {
		int nodeIndex = m_nodes.size();

		// Have we overflowed a DTM Identity's addressing range?
		if (m_dtmIdent
				.size() == (nodeIndex >>> DTMManager.IDENT_DTM_NODE_BITS)) {
			try {
				if (m_mgr == null)
				...
				error(XMLMessages.createXMLMessage(
						XMLErrorResources.ER_NO_DTMIDS_AVAIL, null));// "No
																										// more
																										// DTM
																										// IDs
																										// are
		...
		String localName = (type == Node.PROCESSING_INSTRUCTION_NODE) ? node
				.getNodeName() : node.getLocalName();
		...
		if (node.getLocalName() == null && (type == Node.ELEMENT_NODE
				|| type == Node.ATTRIBUTE_NODE)) {
			// warning("DOM 'level 1' node "+node.getNodeName()+" won't be
			// mapped properly in DOM2DTM.");
		}

		int expandedNameID = (null != localName) ? exnt.getExpandedTypeID(nsURI,
				localName, type) : exnt.getExpandedTypeID(type);
	 ...
	 * Each call to this method adds a new node to the table, unless the end is
	 * reached, in which case it returns null.
	 *
	...
	protected boolean nextNode() {
		...
		do {
			...
			if (pos.hasChildNodes()) {
				...
				if (ENTITY_REFERENCE_NODE != pos.getNodeType()) {
					...
					if (null != m_wsfilter) {
						short wsv = m_wsfilter.getShouldStripSpace(
								makeNodeHandle(m_last_parent), this);
			...
			// If that fails, look up and right (but not past root!)
				...
				while (m_last_parent != NULL) {
					...
					if (next != null && DOCUMENT_TYPE_NODE == next
							.getNodeType())
						next = next.getNextSibling();
					...
					if (pos == null) {
						// %TBD% Should never arise, but I want to be sure of
						// that...
						if (JJK_DEBUG) {
							System.out.println(
					...
					if (pos != null && ENTITY_REFERENCE_NODE == pos
							.getNodeType()) {
						// Nothing needs doing
						if (JJK_DEBUG)
						...
						// Fix and pop DTM
						if (m_last_kid == NULL)
							m_firstch.setElementAt(NULL, m_last_parent); // Popping
																			// from
																			// an
																			// element
						else
							m_nextsib.setElementAt(NULL, m_last_kid); // Popping
																		// from
																		// anything
																		// else
						m_last_parent = m_parent.elementAt(
								m_last_kid = m_last_parent);
					}
				}
		...
		if (next == null) {
			...
			if (JJK_DEBUG) {
				...
				for (int i = 0; i < m_nodes.size(); ++i)
					System.out.println(i + ":\t" + m_firstch.elementAt(i) + "\t"
							+ m_nextsib.elementAt(i));
			}
		...
		//
		// DTM may skip whitespace. This is handled by the suppressNode flag,
		// which
		// when true will keep the DTM node from being created.
		//
		// DTM only directly records the first DOM node of any
		...
		// advance next over this whole block. Should be simpler than
		// special-casing
		// the above loop for "Was the logically-preceeding sibling a text
		// node".
		//
		// Finally, a DTM node should be considered a CDATASection only if all
		// the
		...
		if (TEXT_NODE == nexttype || CDATA_SECTION_NODE == nexttype) {
			...
			while (n != null) {
				...
				// suppression
				suppressNode &= XMLCharacterRecognizer.isWhiteSpace(n
						.getNodeValue());
			...
			if (ELEMENT_NODE == nexttype) {
				...
				if (attrsize > 0) {
					for (int i = 0; i < attrsize; ++i) {
						...
						// addNode() will take care of switching it from
						// Attr to Namespace if necessary.
						attrIndex = addNode(attrs.item(i), nextindex, attrIndex,
								NULL);
						m_firstch.setElementAt(DTM.NULL, attrIndex);
						...
						if (!m_processedFirstElement && "xmlns:xml".equals(attrs
								.item(i).getNodeName()))
				...
				if (!m_processedFirstElement) {
					...
					attrIndex = addNode(
							new DOM2DTMdefaultNamespaceDeclarationNode(
									...
									makeNodeHandle(((attrIndex == NULL)
											? nextindex
											: attrIndex) + 1)), nextindex,
							attrIndex, NULL);
					m_firstch.setElementAt(DTM.NULL, attrIndex);
					m_processedFirstElement = true;
				}
	 ...
	 *
	 * <p>
	 * %OPT% An XPath-like search (walk up DOM to root, tracking path; walk down
	 * DTM reconstructing path) might be considerably faster on later nodes in
	 * large documents. That might also imply improving this call to handle
	 * nodes which would be in this DTM but have not yet been built, which might
	 * or might not be a Good Thing.
	 * </p>
	 *
	 * %REVIEW% This relies on being able to test node-identity via
	 * object-identity. DTM2DOM proxying is a great example of a case where that
	 * doesn't work. DOM Level 3 will provide the isSameNode() method to fix
	 * that, but until then this is going to be flaky.
	 *
	 * @param node
	...
	public int getHandleOfNode(Node node) {
		if (null != node) {
			...
			if ((m_root == node) || (m_root.getNodeType() == DOCUMENT_NODE
					&& m_root == node.getOwnerDocument()) || (m_root
							.getNodeType() != DOCUMENT_NODE && m_root
				...
				// available!
				for (Node cursor = node; cursor != null; cursor = (cursor
						.getNodeType() != ATTRIBUTE_NODE) ? cursor
								.getParentNode()
								: ((org.w3c.dom.Attr) cursor)
										.getOwnerElement()) {
					if (cursor == m_root)
	...
	public int getAttributeNode(int nodeHandle, String namespaceURI,
		...
		if (DTM.ELEMENT_NODE == type) {
			...
			while (DTM.NULL != (identity = getNextNodeIdentity(identity))) {
				...
				// (If we REALLY insist on it, this code should become a
				// subroutine
				...
				if (type == DTM.ATTRIBUTE_NODE || type == DTM.NAMESPACE_NODE) {
					...
					if (nodeuri.equals(namespaceURI) && name.equals(
							nodelocalname))
	 ...
	 * user-supplied FastStringBuffer object. Note that attributes are not
	 * considered part of the content of an element.
	 * <p>
	 * There are open questions regarding whitespace stripping. Currently we
	 * make no special effort in that regard, since the standard DOM doesn't yet
	 * provide DTD-based information to distinguish
	 * whitespace-in-element-context from genuine #PCDATA. Note that we should
	 * probably also consider xml:space if/when we address this. DOM Level 3 may
	 * solve the problem for us.
	 * <p>
	 * %REVIEW% Actually, since this method operates on the DOM side of the
	 * fence rather than the DTM side, it SHOULDN'T do any special handling. The
	 * DOM does what the DOM does; if you want DTM-level abstractions, use
	 * DTM-level methods.
	 *
	 * @param node
	...
	protected static void getNodeData(Node node, FastStringBuffer buf) {
		...
		switch (node.getNodeType()) {
			...
			case Node.ATTRIBUTE_NODE: // Never a child but might be our starting
										// node
				buf.append(node.getNodeValue());
				break;
	 ...
	 * logically followed by another Text or CDATASection node. This may involve
	 * traversing into Entity References.
	 *
	 * %REVIEW% DOM Level 3 is expected to add functionality which may allow us
	 * to retire this.
	 */
	private Node logicalNextDOMTextNode(Node n) {
		...
		if (p == null) {
			...
			for (n = n.getParentNode(); n != null && ENTITY_REFERENCE_NODE == n
					.getNodeType(); n = n.getParentNode()) {
				p = n.getNextSibling();
				if (p != null)
					break;
			}
		...
		if (null != doc) {
			...
			if (null != elem) {
				...
				if (DTM.NULL == elemHandle) {
					...
					while (DTM.NULL != (identity = getNextNodeIdentity(
							identity))) {
		...
		if (null != doc) {
			...
			if (null != doctype) {
				...
				if (null != notationName) // then it's unparsed
				{
					...
					// identifier to generate a URI for the entity instead of
					// the URI
					// specified in the system identifier. If the XSLT processor
					// does
					// not use the public identifier to generate the URI, it
					// must use
					// the system identifier; if the system identifier is a
					// relative
					// URI, it must be resolved into an absolute URI using the
					// URI of
					// the resource containing the entity declaration as the
					// base
					// URI [RFC2396]."
					...
					if (null == url) {
						...
						// This should be resolved to an absolute URL, but
						// that's hard
						// to do from here.
					}
				}
	 ...
	 *         IncrmentalSAXSource if we're bound to one and should receive the
	 ...
	 * definition of whitespace. Refer to
	 * <A href="http://www.w3.org/TR/1998/REC-xml-19980210#NT-S"> the definition
	 ...
	 * does, and that whitespace stripping and so on belong at the DTM level. If
	 * you want a stripped DOM view, wrap DTM2DOM around DOM2DTM.
	 *
	 * @param node
	 *             Node whose subtree is to be walked, gathering the contents of
	 *             all Text or CDATASection nodes.
	...
	protected static void dispatchNodeData(Node node,
			org.xml.sax.ContentHandler ch, int depth)
			throws org.xml.sax.SAXException {
