 ...
 * requests from an XML content handler similar to that of an XML DOM or SAX
 * parser to store information from the xml document in an array based dtm table
 * structure. This informtion is used later for document navigation, query, and
 * SAX event dispatch functions. The DTM can also be used directly as a document
 * composition model for an application. The requests received are:
 * <ul>
...
public class DTMDocumentImpl implements DTM, org.xml.sax.ContentHandler,
	...
	int m_docElement = NULL; // nodeHandle to the root of the actual dtm doc
								// content
	 ...
	 * has all the known problems with multithreading on multiprocessors and we
	...
	// nodes array: integer array blocks to hold the first level reference of
	// the nodes,
	...
	// text/comment table: string buffer to hold the text string values of the
	// document,
	...
	/**
	 * Wrapper for ChunkedIntArray.append, to automatically update the previous
	 * sibling's "next" reference (if necessary) and periodically wake a reader
	...
	private final int appendNode(int w0, int w1, int w2, int w3) {
		...
		previousSiblingWasParent = false; // Set the default; endElement
											// overrides
	 ...
	 * this really be Public? Changing it while DTM is in use would be a
	 * disaster.
	 *
	 * @param poolRef
	...
	/**
	 * Set a reference pointer to the namespace URI symbol table. %REVIEW%
	 * Should this really be Public? Changing it while DTM is in use would be a
	 * disaster.
	 *
	 * @param poolRef
	 *                DTMStringPool reference to an instance of table.
	...
	/**
	 * Set a reference pointer to the prefix name symbol table. %REVIEW% Should
	 * this really be Public? Changing it while DTM is in use would be a
	 * disaster.
	 *
	 * @param poolRef
	...
	/**
	 * getContentHandler returns "our SAX builder" -- the thing that someone
	 * else should send SAX events to in order to extend this DTM model.
	 *
	 * @return null if this model doesn't respond to SAX events, "this" if the
	 *         DTM object has a built-in SAX ContentHandler, the
	 ...
	 *         if the DTM object has a built-in SAX ContentHandler, the
	 *         IncrementalSAXSource if we're bound to one and should receive the
	 *         SAX stream via it for incremental build purposes...
	 */
	...
	// m_docElement = NULL; // reset nodeHandle to the root of the actual dtm
	// doc content
	// initDocument(0);
	// }
	...
	// request
	// * received. %TBD% It can be rechained later via the optional DTM writable
	// interface.
	// *
	...
	// {
	// // do document root node creation here on the first element, create nodes
	// for
	// // this element and its attributes, store the element, namespace, and
	// attritute
	// // name indexes to the nodes array, keep track of the current node and
	// parent
	// // element used
	...
	// // note - there should be no prefix separator in the name because it is
	// not associated
	// // with a name space
	...
	// //w0 = ATTRIBUTE_NODE |
	// //
	// (com.sun.org.apache.xalan.internal.templates.Constants.S_XMLNAMESPACEURI
	// << 16);
	// } else {
	// //w0 = ATTRIBUTE_NODE |
	// }
	...
	// throws DTMException
	// {
	// m_char.append(ch, start, length); // store the chunk to the text/comment
	// string table

	// // create a Text Node
	// // %TBD% may be possible to combine with appendNode()to replace the next
	// chunk of code
	// int w0 = TEXT_NODE;
	// // W1: Parent
	// int w1 = currentParent;
	...
	// throws DTMException
	// {
	// m_char.append(ch, start, length); // store the comment string to the
	...
	// with respect
	// // to the text/comment string table: charStringStart should point to the
	// starting
	// // offset of the string in the table and charStringLength the
	// acccumulated length when
	// // appendAccumulatedText starts, and reset to the end of the table and 0
	// at the end
	 ...
	 * resolved, waits for more nodes to be added to the document and tries
	 * again.
	 *
	 * @param nodeHandle
	 *                   int Handle of the node.
	...
	/**
	 * Given a node handle, advance to its last child. If not yet resolved,
	...
	public int getNextSibling(int nodeHandle) {
		...
		if ((type == ELEMENT_NODE) || (type == ATTRIBUTE_NODE)
			...
			// ###shs should cycle/wait if nextSib is 0? Working on threading
			// next
		}
	...
	/**
	 * Given a node handle, find its preceeding sibling. WARNING: DTM is
	 * asymmetric; this operation is resolved by search, and is relatively
	 * expensive.
	 *
	 * @param nodeHandle
	 *                   the id of the node.
	 ...
	 * across multiple allocation units. This call tells us how many separate
	 * accesses will be required to retrieve the entire content. PLEASE NOTE
	 * that this may not be the same as the number of SAX characters() events
	 * that caused the text node to be built in the first place, since m_chars
	 ...
	 * m_chars object may not store characters in a single contiguous array --
	 * for example,the current FastStringBuffer may split a single node's text
	 * across multiple allocation units. This call retrieves a single contiguous
	 * portion of the text -- as much as m-chars was able to store in a single
	 * allocation unit. PLEASE NOTE that this may not be the same granularityas
	 * the SAX characters() events that caused the text node to be built in the
	 * first place, since m_chars buffering may be on different boundaries than
	 ...
	 * that it a usable URI or that our caller can map it to one. TODO: Resolve
	 * Public Identifiers... or consider changing function name.
	 * <p>
	 ...
	 * Level 1 didn't store the document's base URI.) TODO: Consider resolving
	 * Relative URIs.
	 * <p>
	...
	/**
	 * Append an Attribute child at the current insertion point. Assumes that
	 * the symbols (namespace URI, local name, and prefix) have already been
	 * added to the pools, and that the content has already been appended to
	 * m_char. Note that the attribute's content has been flattened into a
	 * single string; DTM does _NOT_ attempt to model the details of entity
	...
	/**
	 * This is a shortcut to the iterators that implement the supported XPath
	 * axes (only namespace::) is not supported. Returns a bare-bones iterator
	 * that must be initialized with a start node (using
	...
	/**
	 * Starting a new document. Perform any resets/initialization not already
	 * handled.
	 */
	...
	/**
	 * All appends to this document have finished; do whatever final cleanup is
	 * needed.
	 */
