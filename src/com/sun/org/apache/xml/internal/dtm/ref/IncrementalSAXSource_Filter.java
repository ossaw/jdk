...
final class IncrementalSAXSource_Filter implements IncrementalSAXSource, ContentHandler, DTDHandler,
	...
	private ContentHandler clientContentHandler = null; // %REVIEW% support
														// multiple?
	private LexicalHandler clientLexicalHandler = null; // %REVIEW% support
														// multiple?
	private DTDHandler clientDTDHandler = null; // %REVIEW% support multiple?
	private ErrorHandler clientErrorHandler = null; // %REVIEW% support
													// multiple?
	...
	public void init(CoroutineManager co, int controllerCoroutineID, int sourceCoroutineID) {
		...
		if (fControllerCoroutineID == -1 || fSourceCoroutineID == -1)
			throw new RuntimeException(XMLMessages
					.createXMLMessage(XMLErrorResources.ER_COJOINROUTINESET_FAILED, null)); // "co_joinCoroutineSet()
																							// failed");
	 ...
	 *         you're using the do...() methods, applications should only need
	 *         to talk to the CoroutineManager once, to obtain the application's
	 *         Coroutine ID.
	 */
	 ...
	 * require calling super in order to run that logic... which seems
	 * inelegant. Hence this routine for the convenience of subclasses: every
	 * [frequency] invocations, issue a co_yield.
	 * </p>
	 *
	 * @param moreExepected
	 *            Should always be true unless this is being called at the end
	 ...
	 * before delivering any events. Note that the very first thing the
	 * controller tells us may be "I don't need events after all"!
	 */
	 ...
	 * coming from another source, we can't do that because its caller may not
	 * be prepared for this "normal abnormal exit", and instead we put ourselves
	 * in a "spin" mode where events are discarded.
	 */
	 ...
	 * convenience routine, but has the advantage that -- since we invoked
	 * parse() -- we can halt parsing quickly via a StopException rather than
	...
	public void startParse(InputSource source) throws SAXException {
		if (fNoMoreEvents)
			throw new SAXException(XMLMessages
					.createXMLMessage(XMLErrorResources.ER_INCRSAXSRCFILTER_NOT_RESTARTABLE, null)); // "IncrmentalSAXSource_Filter
																										// not
																										// currently
																										// restartable.");
		if (fXMLReader == null)
			throw new SAXException(XMLMessages
					.createXMLMessage(XMLErrorResources.ER_XMLRDR_NOT_BEFORE_STARTPARSE, null)); // "XMLReader
																									// not
																									// before
																									// startParse
																									// request");
	...
	public void run() {
		...
		try {
			...
			if (inner instanceof StopException) {
				...
				if (DEBUG) {
					System.out
							.println("Active IncrementalSAXSource_Filter UNEXPECTED SAX exception: "
									+ inner);
	...
	/**
	 * deliverMoreNodes() is a simple API which tells the coroutine parser that
	 * we need more nodes. This is intended to be called from one of our partner
	 * routines, and serves to encapsulate the details of how incremental
	 * parsing has been achieved.
	 *
	 * @param parsemore
	 *            If true, tells the incremental filter to generate another
	...
	public Object deliverMoreNodes(boolean parsemore) {
		...
		// are those previously established for this
	...
	// ================================================================
	/**
	 * Simple unit test. Attempt coroutine parsing of document indicated by
	 * first argument (as a URI), report progress.
	 */
	/*
	 ...
	 * com.sun.org.apache.xml.internal.serialize.XMLSerializer trace; trace=new
	 * com.sun.org.apache.xml.internal.serialize.XMLSerializer(System.out,null);
	 * filter.setContentHandler(trace); filter.setLexicalHandler(trace);
	 * 
	 * try { InputSource source = new InputSource(args[arg]); Object
	 * result=null; boolean more=true;
	 * 
	 ...
	 * instanceof Exception) { System.out.println("\nFilter threw exception:");
