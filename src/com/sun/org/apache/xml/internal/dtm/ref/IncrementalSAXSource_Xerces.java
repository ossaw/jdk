...
public class IncrementalSAXSource_Xerces implements IncrementalSAXSource {
	...
	/**
	 * Create a IncrementalSAXSource_Xerces, and create a SAXParser to go with
	 * it. Xerces2 incremental parsing is only supported if this constructor is
	 * used, due to limitations in the Xerces2 API (as of Beta 3). If you don't
	 * like that restriction, tell the Xerces folks that there should be a
	 * simpler way to request incremental SAX parsing.
	 */
	public IncrementalSAXSource_Xerces() throws NoSuchMethodException {
		try {
			...
			// _control_ for incremental parsing must be exercised via the
			// config.
			//
			...
			// Preload all the needed the configuration methods... I want to
			// know they're
	...
	/**
	 * Create a IncrementalSAXSource_Xerces wrapped around an existing
	 * SAXParser. Currently this works only for recent releases of Xerces-1.
	 * Xerces-2 incremental is currently possible only if we are allowed to
	 * create the parser instance, due to limitations in the API exposed by
	 * Xerces-2 Beta 3; see the no-args constructor for that code.
	 *
	 * @exception if
	 *                the SAXParser class doesn't support the Xerces incremental
	...
	// ================================================================
	/**
	 * startParse() is a simple API which tells the IncrementalSAXSource to
	...
	public void startParse(InputSource source) throws SAXException {
		if (fIncrementalParser == null)
			throw new SAXException(XMLMessages
					.createXMLMessage(XMLErrorResources.ER_STARTPARSE_NEEDS_SAXPARSER, null)); // "startParse
																								// needs
																								// a
																								// non-null
																								// SAXParser.");
		if (fParseInProgress)
			throw new SAXException(XMLMessages
					.createXMLMessage(XMLErrorResources.ER_STARTPARSE_WHILE_PARSING, null)); // "startParse
																								// may
																								// not
																								// be
																								// called
		...
		if (!ok)
			throw new SAXException(
					XMLMessages.createXMLMessage(XMLErrorResources.ER_COULD_NOT_INIT_PARSER, null)); // "could
																										// not
																										// initialize
																										// parser
																										// with");
