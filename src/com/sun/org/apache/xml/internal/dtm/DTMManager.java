...
public abstract class DTMManager {
	...
	/**
	 * Factory for creating XMLString objects. %TBD% Make this set by the
	 * caller.
	 */
	 ...
	 * creates a new factory instance using the default <code>DTMManager</code>
	 ...
	 * <p>
	 * Note that DTMIterators may not be an exact match to DOM NodeIterators.
	 * They are initialized and used in much the same way as a NodeIterator, but
	...
	/**
	 * Set a flag indicating whether an incremental transform is desired This
	 * flag should have the same value as the FEATURE_INCREMENTAL feature which
	 * is set by the TransformerFactory.setAttribut() method before a DTMManager
	 * is created
	 * 
	 * @param incremental
	...
	/**
	 * Get a flag indicating whether the transformation phase should keep track
	...
	/**
	 * Set a flag indicating whether the transformation phase should keep track
	 * of line and column numbers for the input source document. This flag
	 * should have the same value as the FEATURE_SOURCE_LOCATION feature which
	 * is set by the TransformerFactory.setAttribut() method before a DTMManager
	 * is created
	 * 
	...
	/**
	 * This value, set at compile time, controls how many bits of the DTM node
	 ...
	 * previous "golden" files will probably complain.)
	 */
	public static final int IDENT_DTM_NODE_BITS = 16;

	/**
	 * When this bitmask is ANDed with a DTM node handle number, the result is
	 * the low bits of the node's index number within that DTM. To obtain the
	 * high bits, add the DTM ID portion's offset as assigned in the DTM
	 * Manager.
	 */
	...
	/**
	 * When this bitmask is ANDed with a DTM node handle number, the result is
	 * the DTM's document identity number.
	 */
