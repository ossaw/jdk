 ...
 * these when maintaining this code.
 */
public abstract class DTMDefaultBaseTraversers extends DTMDefaultBase {
	...
	public DTMAxisTraverser getAxisTraverser(final int axis) {
		...
		default:
			throw new DTMException(
					XMLMessages.createXMLMessage(XMLErrorResources.ER_UNKNOWN_AXIS_TYPE,
		...
		if (null == traverser)
			throw new DTMException(
					XMLMessages.createXMLMessage(XMLErrorResources.ER_AXIS_TRAVERSER_NOT_SUPPORTED,
	...
	private class AncestorTraverser extends DTMAxisTraverser {
		...
		/**
		 * Traverse to the next node after the current node that is matched by
	...
	private class ChildTraverser extends DTMAxisTraverser {
		 ...
		 * calling this function, one should first call {@link #isIndexed(int)
		 * isIndexed} to make sure that the index can contain nodes that match
		 * the given expanded type ID.
		 *
		 * @param axisRoot
		 *            The root identity of the axis.
		 * @param nextPotential
		 *            The node found must match or occur after this node.
		 * @param expandedTypeID
		...
		protected int getNextIndexed(int axisRoot, int nextPotential, int expandedTypeID) {
			...
			for (;;) {
				...
				if (NOTPROCESSED != nextID) {
					...
					// Otherwise, it could be a descendant below the subtree
					// root
					// children, or it could be after the subtree root. So we
					// have
					// to climb up until the parent is less than the subtree
					// root, in
					// which case we return NULL, or until it is equal to the
					// subtree
		...
		public int first(int context, int expandedTypeID) {
			if (true) {
				...
				for (int current = _firstch(
						makeNodeIdentity(context)); DTM.NULL != current; current = _nextsib(
								current)) {
					if (m_exptype.elementAt(current) == expandedTypeID)
						return makeNodeHandle(current);
				}
		...
		public int next(int context, int current, int expandedTypeID) {
			...
			for (current = _nextsib(
					makeNodeIdentity(current)); DTM.NULL != current; current = _nextsib(current)) {
				if (m_exptype.elementAt(current) == expandedTypeID)
					return makeNodeHandle(current);
			}
	...
	/**
	 * Super class for derived classes that want a convenient way to access the
	 * indexing mechanism.
	 */
	private abstract class IndexedDTMAxisTraverser extends DTMAxisTraverser {
		 ...
		 * be implemented by derived classes, and must be robust enough to
		 ...
		 * an arriving node should terminate. This method must be implemented be
		 * a derived class.
		 *
		 * @param axisRoot
		 *            The root identity of the axis.
	...
	private class DescendantTraverser extends IndexedDTMAxisTraverser {
		/**
		 * Get the first potential identity that can be returned. This should be
		 ...
		 *            The root context of the subtree in question.
		 * @param identity
	...
	private class AllFromNodeTraverser extends DescendantOrSelfTraverser {
		...
		public int next(int context, int current) {
			...
			for (current = makeNodeIdentity(current) + 1;; current++) {
				...
				// and thus can be used to ensure that incremental construction
				// of
				// the DTM has gotten this far. Using it just for that
				// side-effect
				// is quite a kluge...
	...
	private class NamespaceDeclsTraverser extends DTMAxisTraverser {
		...
		public int next(int context, int current) {
			...
			return (context == current) ? getFirstNamespaceNode(context, false)
	...
	private class NamespaceTraverser extends DTMAxisTraverser {
		...
		public int next(int context, int current) {
			...
			return (context == current) ? getFirstNamespaceNode(context, true)
	...
	private class PrecedingTraverser extends DTMAxisTraverser {
		 ...
		 * This is an expensive operation, made worse by the stateless
		...
		protected boolean isAncestor(int contextIdent, int currentIdent) {
			...
			for (contextIdent = m_parent
					.elementAt(contextIdent); DTM.NULL != contextIdent; contextIdent = m_parent
	...
	/**
	 * Implements traversal of the Ancestor and the Preceding axis, in reverse
	 * document order.
	 */
	...
	private class RootTraverser extends AllFromRootTraverser {
		/**
		 * Return the root if it matches the expanded type ID, else return null
		 * (nothing found)
		 *
		 * @param context
		 *            The context node of this traversal.
