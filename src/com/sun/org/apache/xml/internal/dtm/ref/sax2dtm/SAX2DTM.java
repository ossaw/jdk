 ...
 */
public class SAX2DTM extends DTMDefaultBaseIterators implements EntityResolver, DTDHandler,
		ContentHandler, ErrorHandler, DeclHandler, LexicalHandler {
	...
	public void dispatchCharactersEvents(int nodeHandle, ContentHandler ch, boolean normalize)
		...
		if (isTextType(type)) {
			...
			if (DTM.NULL != firstChild) {
				...
				if (normalize)
					FastStringBuffer.sendNormalizedSAXcharacters(str.toCharArray(), 0, str.length(),
							ch);
				else
	 ...
	 *            true if the node can have a first child, false if it is
	 *            atomic.
	 *
	 * @return The index identity of the node that was added.
	 */
	protected int addNode(int type, int expandedTypeID, int parentIndex, int previousSibling,
	...
	public int getAttributeNode(int nodeHandle, String namespaceURI, String name) {
		...
		for (int attrH = getFirstAttribute(nodeHandle); DTM.NULL != attrH; attrH = getNextAttribute(
				attrH)) {
			String attrNS = getNamespaceURI(attrH);
			String attrName = getLocalName(attrH);
			boolean nsMatch = namespaceURI == attrNS
					|| (namespaceURI != null && namespaceURI.equals(attrNS));
		...
		if (m_textPendingStart >= 0) // -1 indicates no-text-in-progress
		{
			...
			if (doStrip) {
				...
				if (length > 0) {
					...
					m_previous = addNode(m_coalescedTextType, exName, m_parents.peek(), m_previous,
							dataIndex, false);
	...
	public void startDocument() throws SAXException {
		...
		int doc = addNode(DTM.DOCUMENT_NODE,
				m_expandedNameTable.getExpandedTypeID(DTM.DOCUMENT_NODE), DTM.NULL, DTM.NULL, 0,
				true);
	...
	public void startElement(String uri, String localName, String qName, Attributes attributes)
		...
		if (DEBUG) {
			...
			if (DEBUG_ATTRS & attributes != null) {
				...
				if (n == 0)
					...
					for (int i = 0; i < n; i++)
						System.out.println("\t attr: uri: " + attributes.getURI(i) + ", localname: "
								+ attributes.getLocalName(i) + ", qname: " + attributes.getQName(i)
								+ ", type: " + attributes.getType(i) + ", value: "
		...
		int elemNode = addNode(DTM.ELEMENT_NODE, exName, m_parents.peek(), m_previous, prefixIndex,
				true);
	...
	public void processingInstruction(String target, String data) throws SAXException {
		...
		m_previous = addNode(DTM.PROCESSING_INSTRUCTION_NODE, exName, m_parents.peek(), m_previous,
				dataIndex, false);
	}
	...
	public void comment(char ch[], int start, int length) throws SAXException {
		...
		m_previous = addNode(DTM.COMMENT_NODE, exName, m_parents.peek(), m_previous, dataIndex,
				false);
	}
	...
	public SourceLocator getSourceLocatorFor(int node) {
		if (m_useSourceLocationProperty) {
			...
			return new NodeLocator(null, m_sourceSystemId.elementAt(node),
					m_sourceLine.elementAt(node), m_sourceColumn.elementAt(node));
