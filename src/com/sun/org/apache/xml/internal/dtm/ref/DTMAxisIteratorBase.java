 ...
 */
/*
 * $Id: DTMAxisIteratorBase.java,v 1.2.4.1 2005/09/15 08:14:59 suresh_emailid
 * Exp $
 */
...
public abstract class DTMAxisIteratorBase implements DTMAxisIterator {
	 ...
	 * XPath. Note that this is _not_ the node's handle within the DTM!
	 */
	protected int _position = 0;

	/**
	 * The position of the marked node within the iteration; a saved itaration
	 * state that we may want to come back to. Note that only one mark is
	 * maintained; there is no stack.
	 */
	protected int _markedNode;

	/**
	 * The handle to the start, or root, of the iteration. Set this to END to
	 * construct an empty iterator.
	 */
	protected int _startNode = DTMAxisIterator.END;

	/**
	 * True if the start node should be considered part of the iteration. False
	 * will cause it to be skipped.
	 */
	protected boolean _includeSelf = false;

	/**
	 * True if this iteration can be restarted. False otherwise (eg, if we are
	 * iterating over a stream that can not be re-scanned, or if the iterator
	 * was produced by cloning another iterator.)
	 */
	...
	/**
	 * @return A DTMAxisIterator which has been reset to the start node, which
	 *         may or may not be the same as this iterator.
	 */
	...
	/**
	 * Returns the position of the last node within the iteration, as defined by
