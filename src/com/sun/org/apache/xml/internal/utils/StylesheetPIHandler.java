 ...
 */
/*
 * $Id: StylesheetPIHandler.java,v 1.2.4.1 2005/09/15 08:15:57 suresh_emailid
 * Exp $
 */
...
public class StylesheetPIHandler extends DefaultHandler {
	...
	/**
	 * Get the object that will be used to resolve URIs in href in
	...
	/**
	 * Construct a StylesheetPIHandler instance that will search for
	 * xml-stylesheet PIs based on the given criteria.
	 *
	 * @param baseID
	 ...
	 *
	 * @return Source object that references the last stylesheet reference that
		...
		if (target.equals("xml-stylesheet")) {
			...
			while (tokenizer.hasMoreTokens()) {
				if (!lookedAhead)
					...
					lookedAhead = false;
				if (tokenizer.hasMoreTokens() && (token.equals(" ") || token
						.equals("\t") || token.equals("=")))
					continue;
				...
				if (name.equals("type")) {
					...
					try {
						// Add code to use a URIResolver. Patch from Dmitri
						// Ilyin.
					...
					while (tokenizer.hasMoreTokens() && (token.equals(" ")
							|| token.equals("\t") || token.equals("=")))
						token = tokenizer.nextToken();
					alternate = token.substring(1, token.length() - 1).equals(
							"yes");
				}

			}

			if ((null != type) && (type.equals("text/xsl") || type.equals(
					"text/xml") || type.equals("application/xml+xslt"))
					&& (null != href)) {
				if (null != m_media) {
					if (null != media) {
	...
	/**
	 * The spec notes that
	 * "The xml-stylesheet processing instruction is allowed only in the prolog
	 * of an XML document."
	 ...
	 * @param localName
	 *                     The local name (without prefix), or empty string if
	 *                     not
	 ...
	 *                            since there can be no valid xml-stylesheet
	 *                            processing
	...
	/**
	 * Added additional getter and setter methods for the Base Id to fix
	 * bugzilla bug 24187
	 *
	 */
