...
/**
 * Bare-bones, unsafe, fast string buffer. No thread-safety, no parameter range
 * checking, exposed fields. Note that in typical applications, thread-safety of
 * a StringBuffer is a somewhat dubious concept in any case.
 * <p>
 ...
 * <p>
 * FastStringBuffer operates as a "chunked buffer". Doing so reduces the need to
 * recopy existing information when an append exceeds the space available; we
 * just allocate another chunk and flow across to it. (The array of chunks may
 * need to grow, admittedly, but that's a much smaller object.) Some excess
...
public class FastStringBuffer {
    ...
    // %BUG% %REVIEW% *****PROBLEM SUSPECTED: If data from an FSB is being
    // copied
    // back into the same FSB (variable set from previous variable, for example)
    // and blocksize changes in mid-copy... there's risk of severe malfunction
    // in
    // the read process, due to how the resizing code re-jiggers storage. Arggh.
    // If we want to retain the variable-size-block feature, we need to
    // reconsider
    // that issue. For now, I have forced us into fixed-size mode.
    static final boolean DEBUG_FORCE_FIXED_CHUNKSIZE = true;

    /**
     * Manifest constant: Suppress leading whitespace. This should be used when
    ...
    /**
     * Manifest constant: Suppress trailing whitespace. This should be used when
     * normalize-to-SAX is called for the last chunk of a multi-chunk output; it
     * may have to be or'ed with SUPPRESS_LEADING_WS.
     */
    public static final int SUPPRESS_TRAILING_WS = 0x02;

    /**
     * Manifest constant: Suppress both leading and trailing whitespace. This
    ...
    /**
     * Field m_rechunkBits affects our chunk-growth strategy, by saying how many
     * chunks should be allocated at one size before we encapsulate them into
     * the first chunk of the next size up. For example, if m_rechunkBits is set
     * to 3, then after 8 chunks at a given size we will rebundle them as the
     * first element of a FastStringBuffer using a chunk size 8 times larger
    ...
    /**
     * Field m_innerFSB, when non-null, is a FastStringBuffer whose total length
     * equals m_chunkSize, and which replaces m_array[0]. This allows building a
     * hierarchy of FastStringBuffers, where early appends use a smaller
     * chunkSize (for less wasted memory overhead) but later ones use a larger
     * chunkSize (for less heap activity overhead).
     */
    ...
    public FastStringBuffer(int initChunkBits, int maxChunkBits, int rebundleBits) {
        ...
        // %REVIEW%
        // Should this force to larger value, or smaller? Smaller less
        // efficient, but if
        // someone requested variable mode it's because they care about storage
        // space.
        // On the other hand, given the other changes I'm making, odds are that
        // we should
        // adopt the larger size. Dither, dither, dither... This is just stopgap
        // workaround
     ...
     * NEEDSDOC @param initChunkBits NEEDSDOC @param maxChunkBits
     */
    ...
    /**
     * Construct a FastStringBuffer, using default maxChunkBits and rebundleBits
     * values.
     * <p>
    ...
    public final void setLength(int l) {
        ...
        if (m_lastChunk == 0 && m_innerFSB != null) {
            ...
            // There's an edge case if l is an exact multiple of m_chunkBits,
            // which risks leaving
            // us pointing at the start of a chunk which has not yet been
            // allocated. Rather than
            // pay the cost of dealing with that in the append loops (more
            // scattered and more
            // inner-loop), we correct it here by moving to the safe side of
            // that
            // line -- as we would have left the indexes had we appended up to
            // that point.
     ...
     * truncation may require restoring one of the innerFSBs
     * NEEDSDOC @param l NEEDSDOC @param rootFSB
     */
    ...
    public final void append(FastStringBuffer value) {
        ...
        while (strlen > 0) {
            ...
            System.arraycopy(value.m_array[sourcechunk], sourcecolumn, m_array[m_lastChunk], m_firstFree,
                    runlength);

            if (runlength != available)
                System.arraycopy(value.m_array[sourcechunk + 1], 0, m_array[m_lastChunk], m_firstFree
                        + runlength, available - runlength);
    ...
    /**
     * @return true if the specified range of characters are all whitespace, as
     *         defined by XMLCharacterRecognizer.
     *         <p>
     ...
     * events. Note that the buffer reference passed to the ContentHandler may
     * be invalidated if the FastStringBuffer is edited; it's the user's
     ...
     *         whitespace at the start of the following chunk (if any) should be
     *         converted to a single space.
     *         <dt>SUPPRESS_LEADING_WS</dt>
     *         <dd>if this output ended in retained whitespace, and thus
     *         whitespace at the start of the following chunk (if any) should be
    ...
    public int sendNormalizedSAXcharacters(org.xml.sax.ContentHandler ch, int start, int length)
        ...
        for (int i = startChunk; i < stopChunk; ++i) {
            if (i == 0 && m_innerFSB != null)
                stateForNextChunk = m_innerFSB.sendNormalizedSAXcharacters(ch, startColumn, m_chunkSize
                        - startColumn);
            else
                stateForNextChunk = sendNormalizedSAXcharacters(m_array[i], startColumn, m_chunkSize
                        - startColumn, ch, stateForNextChunk);
        ...
        if (stopChunk == 0 && m_innerFSB != null)
            ...
            stateForNextChunk = // %REVIEW% Is this update really needed?
                    sendNormalizedSAXcharacters(m_array[stopChunk], startColumn, stopColumn - startColumn, ch,
     ...
     * This version is aware of the fact that it may be called several times in
     * succession if the data is made up of multiple "chunks", and thus must
     * actively manage the handling of leading and trailing whitespace.
    ...
    static int sendNormalizedSAXcharacters(char ch[], int start, int length,
            org.xml.sax.ContentHandler handler, int edgeTreatmentFlags) throws org.xml.sax.SAXException {
     ...
     * <p>
     * Note that, unlike sendSAXcharacters, this has to be done as a single call
     ...
     * its chunk size reset appropriately. IT SHOULD NEVER BE CALLED EXCEPT WHEN
