 ...
 * and notations) have names and all of them can have either public or system
 * identifiers or both. (In XML, only a notation can have a public identifier
 * without a system identifier, but the methods implemented in this class obey
 ...
 * (which is, in turn, relative to the location of the current working directory
 * at startup, as returned by the <tt>user.dir</tt> system property).</li>
 * <li><b>CATALOG</b> processes other catalog files. An included catalog occurs
 * logically at the end of the including catalog.</li>
 * <li><b>DELEGATE_PUBLIC</b> specifies alternate catalogs for some public
 * identifiers. The delegated catalogs are not loaded until they are needed, but
 * they are cached once loaded.</li>
 * <li><b>DELEGATE_SYSTEM</b> specifies alternate catalogs for some system
 * identifiers. The delegated catalogs are not loaded until they are needed, but
 * they are cached once loaded.</li>
 * <li><b>DELEGATE_URI</b> specifies alternate catalogs for some URIs. The
 * delegated catalogs are not loaded until they are needed, but they are cached
 * once loaded.</li>
 * <li><b>REWRITE_SYSTEM</b> specifies alternate prefix for a system identifier.
 * </li>
 * <li><b>REWRITE_URI</b> specifies alternate prefix for a URI.</li>
 * <li><b>SYSTEM_SUFFIX</b> maps any system identifier that ends with a
 * particular suffix to another system identifier.</li>
 * <li><b>URI_SUFFIX</b> maps any URI that ends with a particular suffix to
 * another URI.</li>
 * <li><b>DOCTYPE</b> associates the names of root elements with URIs. (In other
 * words, an XML processor might infer the doctype of an XML document that does
 * not include a doctype declaration by looking for the DOCTYPE entry in the
 * catalog which matches the name of the root element of the document.)</li>
 * <li><b>DOCUMENT</b> provides a default document.</li>
 * <li><b>DTDDECL</b> recognized and silently ignored. Not relevant for XML.
 * </li>
 * <li><b>ENTITY</b> associates entity names with URIs.</li>
 * <li><b>LINKTYPE</b> recognized and silently ignored. Not relevant for XML.
 * </li>
 * <li><b>NOTATION</b> associates notation names with URIs.</li>
 * <li><b>OVERRIDE</b> changes the override behavior. Initial behavior is set by
 * the system property <tt>xml.catalog.override</tt>. The default initial
 * behavior is 'YES', that is, entries in the catalog override system
 * identifiers specified in the document.</li>
 * <li><b>PUBLIC</b> maps a public identifier to a system identifier.</li>
 ...
 * "http://example.com/a/b/c" as the base URI, the relative URI "foo" is
 * resolved to the absolute URI "http://example.com/a/b/foo". You must provide
 * the trailing slash if you do not want the final component of the path to be
 ...
 * <dd>
 * <p>
 * Allow quoted components in <tt>xml.catalog.files</tt> so that URLs containing
 * colons can be used on Unix. The string passed to <tt>xml.catalog.files</tt>
 * can now have the form:
 * </p>
 * 
 * <pre>
 ...
 * </p>
 * <p>
 * If an unquoted path contains an embedded double or single quote character, no
 * special processig is performed on that character. No path can contain
 * separater characters, double, and single quotes simultaneously.
 * </p>
 * <p>
 * Fix bug in calculation of BASE entries: if a catalog contains multiple BASE
 * entries, each is relative to the preceding base, not the default base URI of
 * the catalog.
 * </p>
 * </dd>
 * <dt>1.0.1</dt>
 * <dd>
 * <p>
 * Fixed a bug in the calculation of the list of subordinate catalogs. This bug
 * caused an infinite loop where parsing would alternately process two catalogs
 * indefinitely.
 * </p>
 * </dd>
 * </dl>
...
public class Catalog {
	 ...
	 * changed by BASE entries in the catalog.
	 */
	protected URL base;
	 ...
	 * a CATALOG entry, the included catalog logically occurs <i>at the end</i>
	 * of the including catalog, and after any preceding CATALOG entries. In
	 ...
	 * of these catalogs has to be loaded, the resulting Catalog object is
	 * placed in the vector, effectively caching it for the next query.
	 * </p>
	 */
	 ...
	 * vector. This assures that matching PUBLIC keywords are encountered before
	 * DELEGATE* entries.
	 * </p>
	 */
	 ...
	 * type is unknown, each reader is attempted in turn until one succeeds.
	 * </p>
	 */
	 ...
	 * are created. Readers added subsequently will not be available. For this
	 * reason, it is best to add all of the readers before the first call to
	 * parse a catalog.
	 * </p>
	 *
	 * @param mimeType
	...
	protected void copyReaders(Catalog newCatalog) {
		...
		for (int count = 0; count < mapArr.size(); count++) {
			...
			Integer pos = readerMap.get(mimeType);
			newCatalog.addReader(mimeType, (CatalogReader) readerArr.get(pos));
		}
	}
	 ...
	 *             One of the system catalogs is identified with a filename that
	 *             is not a valid URL.
	 * @throws IOException
	 ...
	 * survive transmogrification through the URI processing that the
	 ...
	 * maintaining other information about the current catalog.
	 * </p>
	 *
	 * @param entry
	 ...
	 * <li>
	 * <p>
	 * Delegated catalogs are re-parsed each time they are needed (because a
	 * variable list of them may be needed in each case, depending on the length
	 * of the matching partial public identifier).
	 * </p>
	 * <p>
	 * But they are parsed by this method, so as long as they don't change or
	 * disappear while the program is running, they shouldn't generate errors
	 * later if they don't generate errors now.
	 * </p>
	 * <li>
	 * <p>
	 * If you add new catalogs with <code>parseCatalog</code>, they won't be
	 * loaded until they are needed or until you call
	 ...
	 *
	 * @throws MalformedURLException
	 *             The filename (URL) for a subordinate or delegated catalog is
	 *             not a valid URL.
	 * @throws IOException
	 *             Error reading some subordinate or delegated catalog file.
	 */
	 ...
	 *            required.
	 * @param publicId
	 *            The nominal public identifier for the doctype (as provided in
	 *            the source document).
	 * @param systemId
	 *            The nominal system identifier for the doctype (as provided in
	 *            the source document).
	 *
	...
	public String resolveDoctype(String entityName, String publicId, String systemId)
		...
		catalogManager.debug.message(3,
				"resolveDoctype(" + entityName + "," + publicId + "," + systemId + ")");
	 ...
	 *            required.
	 * @param publicId
	 *            The nominal public identifier for the entity (as provided in
	 *            the source document).
	 * @param systemId
	 *            The nominal system identifier for the entity (as provided in
	 *            the source document).
	 *
	...
	public String resolveEntity(String entityName, String publicId, String systemId)
		...
		catalogManager.debug.message(3,
				"resolveEntity(" + entityName + "," + publicId + "," + systemId + ")");
	 ...
	 *
	 * @param notationName
	 *            The name of the notation for which a doctype is required.
	 * @param publicId
	 *            The nominal public identifier for the notation (as provided in
	 *            the source document).
	 * @param systemId
	 *            The nominal system identifier for the notation (as provided in
	 *            the source document).
	 *
	...
	public String resolveNotation(String notationName, String publicId, String systemId)
		...
		catalogManager.debug.message(3,
				"resolveNotation(" + notationName + "," + publicId + "," + systemId + ")");
	 ...
	 * <li>If the system identifier is not null, the PUBLIC entries that were
	 * encountered when OVERRIDE YES was in effect are interrogated and the
	 * first matching entry is returned.</li>
	 * <li>If the system identifier is null, then all of the PUBLIC entries are
	 * interrogated and the first matching entry is returned. This may not be
	 * the same as the preceding case, if some PUBLIC entries are encountered
	 * when OVERRIDE NO is in effect. In XML, the only place where a public
	 * identifier may occur without a system identifier is in a notation
	 * declaration.</li>
	 * <li>Finally, if the public identifier matches one of the partial public
	 * identifiers specified in a DELEGATE* entry in the Catalog, the delegated
	 * catalog is interrogated. The first time that the delegated catalog is
	 * required, it will be retrieved and parsed. It is subsequently cached.
	 * </li>
	 * </ul>
	 *
	 * @param entityType
	 ...
	 *         found.
	 */
	protected synchronized String resolveLocalPublic(int entityType, String entityName,
