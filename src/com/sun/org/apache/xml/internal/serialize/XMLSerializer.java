 ...
 * For usage instructions see {@link Serializer}.
 * <p>
 ...
 * SAX events and using the serializer as a document handler. DOM serializing is
 * done by calling {@link #serialize(Document)} or by using DOM Level 3
 * {@link org.w3c.dom.ls.DOMSerializer} and serializing with
 * {@link org.w3c.dom.ls.DOMSerializer#write},
...
public class XMLSerializer extends BaseMarkupSerializer {
    ...
    /**
     * Controls whether namespace fixup should be performed during the
     * serialization. NOTE: if this field is set to true the following fields
     * need to be initialized: fNSBinder, fLocalNSBinder, fSymbolTable,
     * XMLSymbols.EMPTY_STRING, fXmlSymbol, fXmlnsSymbol
     */
    protected boolean fNamespaces = false;

    /**
     * Controls whether namespace prefixes will be printed out during
     * serialization
     */
     ...
     * specified output format. If <tt>format</tt> is null, will use a default
     * output format.
     *
     * @param writer
     *               The writer to use
     ...
     * using the specified output format. If <tt>format</tt> is null, will use a
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        try {
            ...
            if (rawName == null || rawName.length() == 0) {
                if (localName == null) {
                    String msg = DOMMessageFormatter.formatMessage(
                            DOMMessageFormatter.SERIALIZER_DOMAIN, "NoName",
                            null);
            ...
            if (_prefixes != null) {
                for (Map.Entry<String, String> entry : _prefixes.entrySet()) {
                    _printer.printSpace();
                    value = entry.getKey(); // The prefixes map uses the URI
                                            // value as key.
    ...
    /**
     * Called to serialize the document's DOCTYPE by the root element. The
     * document type declaration must name the root element, but the root
     * element is only known when that element is serialized, and not at the
     * start of the document.
     * <p>
     * This method will check if it has not been called before (
     * {@link #_started}), will serialize the document type declaration, and
     * will serialize all pre-root comments and PIs that were accumulated in the
     * document (see {@link #serializePreRoot}). Pre-root will be serialized
     * even if this is not the first root element of the document.
     */
    ...
    /**
     * Called to serialize a DOM element. Equivalent to calling
     * {@link #startElement}, {@link #endElement} and serializing everything
    ...
    protected void serializeElement(Element elem) throws IOException {
        ...
        if (DEBUG) {
            System.out.println("==>startElement: " + elem.getNodeName() + " ns="
        ...
        if (!fNamespaces) { // no namespace fixup should be performed
                    ...
                    if (value.equals(NamespaceContext.XMLNS_URI)) {
                        if (fDOMErrorHandler != null) {
                            ...
                            modifyDOMError(msg, DOMError.SEVERITY_ERROR, null,
                                    attr);
                            boolean continueProcess = fDOMErrorHandler
            ...
            // output element name
            // ----------------------
            // REVISIT: this could be removed if we always convert empty string
            // to null
            // for the namespaces.
            if ((uri != null && prefix != null) && uri.length() == 0 && prefix
                    .length() != 0) {
            ...
            // Assume "foo", "ns1" are declared on the parent. We should not
            // miss
            // redeclaration for both "ns1" and default namespace. To solve this
            // we add a local binder that stores declaration only for current
            // element.
            // This way we avoid outputing duplicate declarations for the same
            // element
            ...
            // We create another element body bound to the "http://xsl"
            // namespace
            // as well as namespace attribute rebounding xsl to another
            // namespace.
                ...
                if (elem.getLocalName() == null) {
                    ...
                    if (fDOMErrorHandler != null) {
                        String msg = DOMMessageFormatter.formatMessage(
                                ...
                                "NullLocalElementName", new Object[] { elem
                                        .getNodeName() });
                        modifyDOMError(msg, DOMError.SEVERITY_ERROR, null,
                                elem);
            ...
            for (i = 0; i < length; i++) {
                ...
                // Fix attribute that was declared with a prefix and
                // namespace=""
                if (uri != null && uri.length() == 0) {
                    uri = null;
                ...
                if (DEBUG) {
                    System.out.println("==>process attribute: " + attr
                            .getNodeName());
                }
                ...
                if (uri != null) { // attribute has namespace !=null
                        ...
                        if (prefix == XMLSymbols.PREFIX_XMLNS) { // xmlns:prefix
                            ...
                            if (value.length() != 0) {
                                if (localUri == null) {
                                    ...
                                    // If the DOM Level 3 namespace-prefixes
                                    ...
                                    // If the xmlns:xx is output here first,
                                    // later we should not
                                    // redeclare "xx" prefix. Instead we would
                                    // pick up different prefix
                                    // for the attribute.
                            ...
                            uri = fNSBinder.getURI(XMLSymbols.EMPTY_STRING);
                            localUri = fLocalNSBinder.getURI(
                                    XMLSymbols.EMPTY_STRING);
                            value = fSymbolTable.addSymbol(value);
                            if (localUri == null) {
                                // declaration was not printed while fixing
                    ...
                    if (prefix == XMLSymbols.EMPTY_STRING
                        ...
                        if (declaredPrefix != null
                            ...
                            if (DEBUG) {
                                System.out.println(
                                        "==> cound not find prefix for the attribute: "
                                                + prefix);
                            }
                    ...
                    printAttribute(name, (value == null)
                            ? XMLSymbols.EMPTY_STRING
                            : value, attr.getSpecified(), attr);
    ...
    /**
     * Retrieve and remove the namespaces declarations from the list of
     * attributes.
     *
     */
    ...
    protected void checkUnboundNamespacePrefixedNode(Node node)
        ...
        if (fNamespaces) {
            ...
            if (DEBUG) {
                System.out.println("==>serializeNode(" + node.getNodeName()
                        + ") [Entity Reference - Namespaces on]");
                System.out.println("==>Declared Prefix Count: " + fNSBinder
                        .getDeclaredPrefixCount());
                System.out.println("==>Node Name: " + node.getNodeName());
                System.out.println("==>First Child Node Name: " + node
                        .getFirstChild().getNodeName());
                System.out.println("==>First Child Node Prefix: " + node
                        .getFirstChild().getPrefix());
                System.out.println("==>First Child Node NamespaceURI: " + node
                        .getFirstChild().getNamespaceURI());
            }
            ...
            for (child = node.getFirstChild(); child != null; child = next) {
                ...
                if (DEBUG) {
                    System.out.println("==>serializeNode(" + child.getNodeName()
                            + ") [Child Node]");
                    System.out.println("==>serializeNode(" + child.getPrefix()
                            + ") [Child Node Prefix]");
                }
                ...
                if (fNSBinder.getURI(prefix) == null && prefix != null) {
                    fatalError("The replacement text of the entity node '"
                            + node.getNodeName()
                            + "' contains an element node '" + child
                                    .getNodeName()
                            + "' with an undeclared prefix '" + prefix + "'.");
                }
                ...
                if (child.getNodeType() == Node.ELEMENT_NODE) {
                    ...
                    for (int i = 0; i < attrs.getLength(); i++) {
                        ...
                        attrPrefix = (attrPrefix == null || attrPrefix
                                .length() == 0) ? XMLSymbols.EMPTY_STRING
