...
/**
 * Base class for a serializer supporting both DOM and SAX pretty serializing of
 * XML/HTML/XHTML documents. Derives classes perform the method-specific
 * serializing, this class provides the common serializing mechanisms.
 * <p>
 * The serializer must be initialized with the proper writer and output format
 * before it can be used by calling {@link #setOutputCharStream} or
 * {@link #setOutputByteStream} for the writer and {@link #setOutputFormat} for
 * the output format.
 * <p>
 * The serializer can be reused any number of times, but cannot be used
 * concurrently by two threads.
 * <p>
 * If an output stream is used, the encoding is taken from the output format
 * (defaults to <tt>UTF-8</tt>). If a writer is used, make sure the writer uses
 * the same encoding (if applies) as specified in the output format.
 * <p>
 * The serializer supports both DOM and SAX. DOM serializing is done by calling
 * {@link #serialize(Document)} and SAX serializing is done by firing SAX events
 * and using the serializer as a document handler. This also applies to derived
 * class.
 * <p>
 ...
 * <p>
 * For elements that are not specified as whitespace preserving, the serializer
 * will potentially break long text lines at space boundaries, indent lines, and
 * serialize elements on separate lines. Line terminators will be regarded as
 * spaces, and spaces at beginning of line will be stripped.
 * <p>
 * When indenting, the serializer is capable of detecting seemingly element
 * content, and serializing these elements indented on separate lines. An
 * element is serialized indented when it is the first or last child of an
 * element, or immediate following or preceding another element.
 *
 *
 ...
 * @see LSSerializer
 */
public abstract class BaseMarkupSerializer implements ContentHandler,
		DocumentHandler, LexicalHandler, DTDHandler, DeclHandler, DOMSerializer,
		Serializer {
	...
	/**
	 * Holds array of all element states that have been entered. The array is
	 * automatically resized. When leaving an element, it's state is not removed
	 * but reused when later returning to the same nesting level.
	 */
	private ElementState[] _elementStates;

	/**
	 * The index of the next state to place in the array, or one plus the index
	 * of the current state. When zero, we are in no state.
	 */
	private int _elementStateCount;

	/**
	 * Vector holding comments and PIs that come before the root element (even
	 * after it), see {@link #serializePreRoot}.
	 */
	private Vector _preRoot;

	/**
	 * If the document has been started (header serialized), this flag is set to
	 * true so it's not started twice.
	 */
	protected boolean _started;

	/**
	 * True if the serializer has been prepared. This flag is set to false when
	 * the serializer is reset prior to using it, and to true after it has been
	 * prepared for usage.
	 */
	 ...
	 * Accumulated here prior to starting an element and placing this list in
	 * the element state.
	 */
	...
	/**
	 * The output format associated with this serializer. This will never be a
	 * null reference. If no format was passed to the constructor, the default
	 * one for this document type will be used. The format object is never
	 * changed by the serializer.
	 */
	...
	/**
	 * Protected constructor can only be used by derived class. Must initialize
	 * the serializer before serializing any document, by calling
	...
	public void setOutputByteStream(OutputStream output) {
		if (output == null) {
			String msg = DOMMessageFormatter.formatMessage(
					DOMMessageFormatter.SERIALIZER_DOMAIN, "ArgumentIsNull",
					new Object[] { "output" });
	...
	public boolean reset() {
		if (_elementStateCount > 1) {
			String msg = DOMMessageFormatter.formatMessage(
					DOMMessageFormatter.SERIALIZER_DOMAIN, "ResetInMiddle",
					null);
	...
	protected void prepare() throws IOException {
		...
		if (_writer == null && _output == null) {
			String msg = DOMMessageFormatter.formatMessage(
					DOMMessageFormatter.SERIALIZER_DOMAIN, "NoWriterSupplied",
					null);
	...
	/**
	 * Serializes the DOM element using the previously specified writer and
	 * output format. Throws an exception only if an I/O exception occured while
	 * serializing.
	 *
	 * @param elem
	...
	/**
	 * Serializes a node using the previously specified writer and output
	 * format. Throws an exception only if an I/O exception occured while
	 * serializing.
	 *
	 * @param node
	 *             Node to serialize
	...
	/**
	 * Serializes the DOM document fragmnt using the previously specified writer
	 * and output format. Throws an exception only if an I/O exception occured
	 * while serializing.
	 *
	 * @param elem
	 *             The element to serialize
	...
	/**
	 * Serializes the DOM document using the previously specified writer and
	 * output format. Throws an exception only if an I/O exception occured while
	 * serializing.
	 *
	 * @param doc
		...
		try {
			...
			if (state.inCData || state.doCData) {
				...
				for (int index = start; index < end; ++index) {
					ch = chars[index];
					if (ch == ']' && index + 2 < end && chars[index + 1] == ']'
							&& chars[index + 2] == '>') {
						_printer.printText("]]]]><![CDATA[>");
						index += 2;
						continue;
					...
					if (!XMLChar.isValid(ch)) {
						...
						if ((ch >= ' ' && _encodingInfo.isPrintable((char) ch)
								&& ch != 0xF7) || ch == '\n' || ch == '\r'
								|| ch == '\t') {
	...
	/**
	 * Called at the end of the document to wrap it up. Will flush the output
	 * stream and throw an exception if any I/O error occured while serializing.
	 *
	 * @throws SAXException
	...
	protected void serializeNode(Node node) throws IOException {
		...
		switch (node.getNodeType()) {
			...
			case Node.ENTITY_REFERENCE_NODE: {
				...
				if (((features & DOMSerializerImpl.ENTITIES) != 0) || (node
						.getFirstChild() == null)) {
					if (fDOMFilter != null && (fDOMFilter.getWhatToShow()
			...
			case Node.PROCESSING_INSTRUCTION_NODE: {
				...
				processingInstructionIO(node.getNodeName(), node
						.getNodeValue());
				break;
			}
			...
			case Node.DOCUMENT_NODE: {
				...
				if (docType != null) {
					...
					// DOM Level 1 -- does implementation have methods?
						...
						try {
							java.lang.reflect.Method getPublicId = docTypeClass
									.getMethod("getPublicId", (Class[]) null);
							if (getPublicId.getReturnType().equals(
									String.class)) {
								docTypePublicId = (String) getPublicId.invoke(
										docType, (Object[]) null);
							}
						} catch (Exception e) {
							// ignore
						...
						try {
							java.lang.reflect.Method getSystemId = docTypeClass
									.getMethod("getSystemId", (Class[]) null);
							if (getSystemId.getReturnType().equals(
									String.class)) {
								docTypeSystemId = (String) getSystemId.invoke(
										docType, (Object[]) null);
							}
						} catch (Exception e) {
							// ignore
	...
	/**
	 * Must be called by a method about to print any type of content. If the
	 * element was just opened, the opening tag is closed and will be matched to
	 * a closing tag. Returns the current element state with <tt>empty</tt> and
	 * <tt>afterElement</tt> set to false.
	 *
	...
	/**
	 * Called to print the text contents in the prevailing element format. Since
	 * this method is capable of printing text as CDATA, it is used for that
	 * purpose as well. White space handling is determined by the current
	 * element state. In addition, the output format can dictate whether the
	 * text is printed as CDATA or unescaped.
	 *
	 * @param text
	 *                  The text to print
	 * @param unescaped
	...
	/**
	 * Returns the suitable entity reference for this character value, or null
	 * if no such entity exists. Calling this method with <tt>'&amp;'</tt> will
	 * return <tt>"&amp;amp;"</tt>.
	 *
	 * @param ch
	 *           Character value
	...
	/**
	 * Comments and PIs cannot be serialized before the root element, because
	 * the root element serializes the document type, which generally comes
	 * first. Instead such PIs and comments are accumulated inside a vector and
	 * serialized by calling this method. Will be called when the root element
	 * is serialized and when the document finished serializing.
	 *
	 * @throws IOException
	 *                     An I/O exception occured while serializing
	 */
	...
	protected void printCDATAText(String text) throws IOException {
		...
		for (int index = 0; index < length; ++index) {
			...
			if (ch == ']' && index + 2 < length && text.charAt(index + 1) == ']'
	...
	/**
	 * Called to print additional text with whitespace handling. If spaces are
	 * preserved, the text is printed as if by calling
	 * {@link #printText(String,boolean,boolean)} with a call to
	 * {@link Printer#breakLine} for each new line. If spaces are not preserved,
	 * the text is broken at space boundaries if longer than the line width;
	 * Multiple spaces are printed as such, but spaces at beginning of line are
	 * removed.
	 *
	 * @param text
	 *                      The text to print
	 * @param preserveSpace
	 *                      Space preserving flag
	 * @param unescaped
	 *                      Print unescaped
	 */
	...
	/**
	 * Print a document type public or system identifier URL. Encapsulates the
	 * URL in double quotes, escapes non-printing characters and print it
	 * equivalent to {@link #printText}.
	 *
	 * @param url
	...
	protected void printDoctypeURL(String url) throws IOException {
		...
		for (i = 0; i < url.length(); ++i) {
			if (url.charAt(i) == '"' || url.charAt(i) < 0x20 || url.charAt(
					i) > 0x7F) {
				_printer.printText('%');
	 ...
	 * Non printable characters are escaped using character references. Where
	 * the format specifies a deault entity reference, that reference is used
	 * (e.g. <tt>&amp;lt;</tt>).
	 *
	 * @param source
	...
	/**
	 * Enter a new element state for the specified element. Tag name and space
	 * preserving is specified, element state is initially empty.
	 *
	 * @return Current element state, or null
	 */
	protected ElementState enterElementState(String namespaceURI,
			String localName, String rawName, boolean preserveSpace) {
		ElementState state;
	...
	/**
	 * Leave the current element state and return to the state of the parent
	 * element. If this was the root element, return to the state of the
	 * document.
	 *
	...
	/**
	 * Returns true if in the state of the document. Returns true before
	 * entering any element and after leaving the root element.
	 *
	...
	/**
	 * Returns the namespace prefix for the specified URI. If the URI has been
	 * mapped to a prefix, returns the prefix, otherwise returns null.
	 *
	 * @param namespaceURI
	 *                     The namespace URI
		...
		if (_elementStateCount == 0)
			...
			for (int i = _elementStateCount; i > 0; --i) {
				if (_elementStates[i].prefixes != null) {
					prefix = (String) _elementStates[i].prefixes.get(
							namespaceURI);
					if (prefix != null)
						return prefix;
				}
