...
public class DOMSerializerImpl implements LSSerializer, DOMConfiguration {
	...
	public Object getParameter(String name) throws DOMException {
		...
		if (name.equalsIgnoreCase(Constants.DOM_NORMALIZE_CHARACTERS)) {
			...
			if ((features & ENTITIES) == 0 && (features & CDATA) == 0
					...
					&& (features & WELLFORMED) != 0 && (features & COMMENTS) != 0) {
				return Boolean.TRUE;
			}
	 ...
	 *                DOMSTRING_SIZE_ERR: The resulting string is too long to
	 *                fit in a <code>DOMString</code>.
	 * @exception LSException
	 *                SERIALIZE_ERR: Unable to serialize the node. DOM
	 *                applications should attach a <code>DOMErrorHandler</code>
	 *                using the parameter &quot;<i>error-handler</i>&quot; to
		...
		try {
			...
			if (wnode.getNodeType() == Node.DOCUMENT_NODE) {
				...
				String msg = DOMMessageFormatter.formatMessage(
						DOMMessageFormatter.SERIALIZER_DOMAIN, "unable-to-serialize-node", null);
			...
			throw (LSException) new LSException(LSException.SERIALIZE_ERR, e.toString())
					.initCause(e);
			...
			// mrglavas
			String msg = DOMMessageFormatter.formatMessage(DOMMessageFormatter.DOM_DOMAIN,
					"STRING_TOO_LONG", new Object[] { ioe.getMessage() });
			throw (DOMException) new DOMException(DOMException.DOMSTRING_SIZE_ERR, msg)
					.initCause(ioe);
		}
	...
	public boolean write(Node node, LSOutput destination) throws LSException {
		...
		if ((encoding = destination.getEncoding()) == null) {
			try {
				Method getEncoding = fDocument.getClass().getMethod("getInputEncoding",
						new Class[] {});
				if (getEncoding != null) {
			...
			if (encoding == null) {
				try {
					Method getEncoding = fDocument.getClass().getMethod("getXmlEncoding",
							new Class[] {});
					if (getEncoding != null) {
						encoding = (String) getEncoding.invoke(fDocument, (Object[]) null);
					}
		...
		try {
			...
			if (writer == null) {
				if (outputStream == null) {
					if (uri == null) {
						String msg = DOMMessageFormatter.formatMessage(
								DOMMessageFormatter.SERIALIZER_DOMAIN, "no-output-specified", null);
						if (ser.fDOMErrorHandler != null) {
							DOMErrorImpl error = new DOMErrorImpl();
	...
	private void prepareForSerialization(XMLSerializer ser, Node node) {
		...
		if ((features & WELLFORMED) != 0) {
			...
			Document document = (node.getNodeType() == Node.DOCUMENT_NODE) ? (Document) node
					: node.getOwnerDocument();
			try {
				versionChanged = document.getClass().getMethod("isXMLVersionChanged()",
						new Class[] {});
	...
	private void verify(Node node, boolean verifyNames, boolean xml11Version) {
		...
		case Node.ELEMENT_NODE: {
			if (verifyNames) {
				if ((features & NAMESPACES) != 0) {
					wellformed = CoreDocumentImpl.isValidQName(node.getPrefix(),
							node.getLocalName(), xml11Version);
				} else {
			...
			if (attributes != null) {
				for (int i = 0; i < attributes.getLength(); ++i) {
					...
					DOMNormalizer.isAttrValueWF(fErrorHandler, fError, fLocator, attributes, attr,
							attr.getValue(), xml11Version);
					if (verifyNames) {
		...
		case Node.CDATA_SECTION_NODE: {
			// verify content
			DOMNormalizer.isXMLCharWF(fErrorHandler, fError, fLocator, node.getNodeValue(),
					xml11Version);
		...
		case Node.PROCESSING_INSTRUCTION_NODE: {
			...
			DOMNormalizer.isXMLCharWF(fErrorHandler, fError, fLocator, pinode.getData(),
					xml11Version);
			break;
		}
		}
