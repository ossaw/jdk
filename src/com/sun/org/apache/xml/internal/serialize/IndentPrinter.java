...
public class IndentPrinter extends Printer {
    ...
    /**
     * Holds the currently accumulating text that follows {@link #_line}. When
     * the end of the part is identified by a call to {@link #printSpace} or
     * {@link #breakLine}, this part is added to the accumulated line.
     */
    private StringBuffer _text;

    /**
     * Counts how many white spaces come between the accumulated line and the
     * current accumulated text. Multiple spaces at the end of the a line will
     * not be printed.
     */
    private int _spaces;
    ...
    /**
     * Called by any of the DTD handlers to enter DTD mode. Once entered, all
     * output will be accumulated in a string that can be printed as part of the
     * document's DTD. This method may be called any number of time but will
     * only have affect the first time it's called. To exist DTD state and get
     * the accumulated DTD, call {@link #leaveDTD}.
     */
    ...
    /**
     * Called by the root element to leave DTD mode and if any DTD parts were
     * printer, will return a string with their textual content.
     */
    ...
    /**
     * Called to print additional text. Each time this method is called it
     * accumulates more text. When a space is printed ({@link #printSpace}) all
     * the accumulated text becomes one part and is added to the accumulate
    ...
    /**
     * Called to print a single space between text parts that may be broken into
     * separate lines. Must not be called to print a space when preserving
     * spaces. The text accumulated so far with {@link #printText} will be added
     * to the accumulated line, and a space separator will be counted. If the
     * line accumulated so far is long enough, it will be printed.
     */
    ...
    /**
     * Called to print a line consisting of the text accumulated so far. This is
     * equivalent to calling {@link #printSpace} but forcing the line to print
     * and starting a new line ({@link #printSpace} will only start a new line
    ...
    /**
     * Flushes the line accumulated so far to the writer and get ready to
     * accumulate the next line. This method is called by {@link #printText} and
     * {@link #printSpace} when the accumulated line plus accumulated text are
     * two long to fit on a given line. At the end of this method _line is empty
     * and _spaces is zero.
     */
    ...
    /**
     * Flush the output stream. Must be called when done printing the document,
     * otherwise some text might be buffered.
     */
    public void flush() {
