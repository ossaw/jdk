...
/**
 * <p>
 * This grammar pool is a memory sensitive cache. The grammars stored in the
 * pool are softly reachable and may be cleared by the garbage collector in
 * response to memory demand. Equality of <code>XMLSchemaDescription</code>s is
 * determined using both the target namespace for the schema and schema
 * location.
 * </p>
 *
...
final class SoftReferenceGrammarPool implements XMLGrammarPool {
	 ...
	 * This method is called after the validation finishes. The application may
	 * then choose to cache some of the returned grammars.</p> <p>In this
	...
	public void putGrammar(Grammar grammar) {
		if (!fPoolIsLocked) {
			synchronized (fGrammars) {
				...
				for (Entry entry = fGrammars[index]; entry != null; entry = entry.next) {
					if (entry.hash == hash && equals(entry.desc, desc)) {
						if (entry.grammar.get() != grammar) {
							entry.grammar = new SoftGrammarReference(entry, grammar,
									fReferenceQueue);
						}
						return;
					}
				...
				// create a new entry
				Entry entry = new Entry(hash, index, desc, grammar, fGrammars[index],
						fReferenceQueue);
	 ...
	 * Currently, the root element name is used as the key for DTD grammars and
	...
	public Grammar getGrammar(XMLGrammarDescription desc) {
		synchronized (fGrammars) {
			...
			for (Entry entry = fGrammars[index]; entry != null; entry = entry.next) {
				...
				/**
				 * If the soft reference has been cleared, remove this entry
				 * from the pool.
				 */
				if (tempGrammar == null) {
					removeEntry(entry);
	...
	/**
	 * Removes the grammar associated to the specified grammar description from
	 * the grammar pool and returns the removed grammar. Currently, the root
	 * element name is used as the key for DTD grammars and the target namespace
	 ...
	 * specified grammar description. Currently, the root element name is used
	 * as the key for DTD grammars and the target namespace is used as the key
	 * for Schema grammars.
	 *
	 * @param desc
	 *            The Grammar Description.
	...
	/*
	 * <p> Sets this grammar pool to a "locked" state--i.e., no new grammars
	 * will be added until it is "unlocked".
	 */
	...
	/*
	 * <p> Sets this grammar pool to an "unlocked" state--i.e., new grammars
	 * will be added when putGrammar or cacheGrammars are called.
	 */
	...
	/*
	 * <p>This method clears the pool-i.e., removes references to all the
	 * grammars in it.</p>
	 */
	public void clear() {
	 ...
	 * compare the root element names for DTD grammars and the target namespaces
	...
	/**
	 * This class is a grammar pool entry. Each entry acts as a node in a doubly
	 * linked list.
	 */
	...
	/**
	 * This class stores a soft reference to a grammar object. It keeps a
	 * reference to its associated entry, so that it can be easily removed from
	 * the pool.
	 */
