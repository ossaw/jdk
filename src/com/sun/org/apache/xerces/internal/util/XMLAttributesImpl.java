...
public class XMLAttributesImpl implements XMLAttributes, XMLBufferListener {
     ...
     * current state of this object. A chain is stale if its state is not the
     * same as the number of times the attribute table view has been used.
     */
    ...
    /**
     * Sets whether namespace processing is being performed. This state is
     * needed to return the correct value from the getLocalName method.
     *
     * @param namespaces
    ...
    /**
     * Adds an attribute. The attribute's non-normalized value of the attribute
     * will have the same value as the attribute value until set using the
     * <code>setNonNormalizedValue</code> method. Also, the added attribute will
     * be marked as specified in the XML instance document unless set otherwise
     * using the <code>setSpecified</code> method.
     * <p>
     * <strong>Note:</strong> If an attribute of the same name already exists,
    ...
    public int addAttribute(QName name, String type, String value, XMLString valueCache) {
        ...
        if (fLength < SIZE_LIMIT) {
            index = name.uri != null && !name.uri.equals("") ? getIndexFast(name.uri, name.localpart)
        ...
        } else if (name.uri == null || name.uri.length() == 0 || (index = getIndexFast(name.uri,
                name.localpart)) == -1) {

            /**
             * If attributes were removed from the list after the table becomes
             * in use this isn't reflected in the table view. It's assumed that
             * once a user starts removing attributes they're not likely to add
             * more. We only make the view consistent if the user of this class
             * adds attributes, removes them, and then adds more.
             */
            if (!fIsTableViewConsistent || fLength == SIZE_LIMIT || (fLength > SIZE_LIMIT
                    && fLength > fTableViewBuckets)) {
    ...
    /**
     * Sets the fields in the given QName structure with the values of the
     ...
     * If no non-normalized value is set, this method will return the same value
     ...
     *
     * @param uri
     *        The Namespace URI, or null if the name has no Namespace
     *        URI.
     * @param localName
    ...
    public int getIndex(String uri, String localPart) {
        for (int i = 0; i < fLength; i++) {
            ...
            if (attribute.name.localpart != null && attribute.name.localpart.equals(localPart)
                    && ((uri == attribute.name.uri) || (uri != null && attribute.name.uri != null
    ...
    /**
     * Look up the index of an attribute by local name only, ignoring its
     * namespace.
     *
     * @param localName
     *        The attribute's local name.
     ...
     * Look up the index of an attribute by XML 1.0 qualified name.
     * <p>
     * <strong>Note:</strong> This method uses reference comparison, and thus
     * should only be used internally. We cannot use this method in any code
     * exposed to users as they may not pass in unique strings.
     *
     * @param qName
     *        The qualified (prefixed) name.
     * @return The index of the attribute, or -1 if it does not appear in the
     ...
     * using the <code>setSpecified</code> method.
     * <p>
     * This method differs from <code>addAttribute</code> in that it does not
     * check if an attribute of the same name already exists in the list before
     * adding it. In order to improve performance of namespace processing, this
     * method allows uniqueness checks to be deferred until all the namespace
     * information is available after the entire attribute specification has
     * been read.
     * <p>
     * <strong>Caution:</strong> If this method is called it should not be mixed
     * with calls to <code>addAttribute</code> unless it has been determined
     * that all the attribute names are unique.
     *
     * @param name
     *        the attribute name
     * @param type
    ...
    /**
     * Checks for duplicate expanded names (local part and namespace name pairs)
     * in the attribute specification. If a duplicate is found its name is
     * returned.
     * <p>
     * This should be called once all the in-scope namespaces for the element
     * enclosing these attributes is known, and after all the attributes have
     * gone through namespace binding.
     *
    ...
    /**
     * Returns the position in the table view where the given attribute name
     * would be hashed.
     *
     * @param qname
     *        the attribute name
    ...
    /**
     * Prepares the table view of the attributes list for use, and populates it
     * with the attributes which have been previously read.
     */
    ...
    /**
     * Look up an attribute's value by Namespace name and Local name. If
     * Namespace is null, ignore namespace comparison. If Namespace is "", treat
     * the name as having no Namespace URI.
     * <p>
     * See {@link #getValue(int) getValue(int)} for a description of the
     * possible values.
     * </p>
    ...
    /**
     * This method will be invoked by XMLEntityReader before ScannedEntities
     * buffer is reloaded.
     */
    public void refresh() {
        if (fLength > 0) {
    ...
    /**
     * Randomly selects a new hash function and reorganizes the table view in
     * order to more evenly distribute its entries. This method is called
    ...
    static class Attribute {
        ...
        /**
         * Augmentations information for this attribute. XMLAttributes has no
         * knowledge if any augmentations were attached to Augmentations.
         */
