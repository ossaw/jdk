...
/**
 * Some useful utility methods. This class was modified in Xerces2 with a view
 * to abstracting as much as possible away from the representation of the
 * underlying parsed structure (i.e., the DOM). This was done so that, if Xerces
 * ever adopts an in-memory representation more efficient than the DOM (such as
 * a DTM), we should easily be able to convert our schema parsing to utilize it.
 *
...
public class DOMUtil {
	...
	/**
	 * Finds and returns the last child element node. Overload previous method
	 * for non-Xerces node impl.
	 */
	...
	public static void setHidden(Node node) {
		if (node instanceof com.sun.org.apache.xerces.internal.impl.xs.opti.NodeImpl)
			((com.sun.org.apache.xerces.internal.impl.xs.opti.NodeImpl) node).setReadOnly(true,
					false);
	...
	public static void setVisible(Node node) {
		if (node instanceof com.sun.org.apache.xerces.internal.impl.xs.opti.NodeImpl)
			((com.sun.org.apache.xerces.internal.impl.xs.opti.NodeImpl) node).setReadOnly(false,
					false);
		else if (node instanceof com.sun.org.apache.xerces.internal.dom.NodeImpl)
	...
	/**
	 * Finds and returns the next sibling node with the given name and attribute
	...
	/**
	 * returns local name of this element if not null, otherwise returns the
	 * name of the node
	 */
	...
	/**
	 * Creates a DOMException. On J2SE 1.4 and above the cause for the exception
	 * will be set.
	 */
	...
	/**
	 * Creates an LSException. On J2SE 1.4 and above the cause for the exception
	 * will be set.
	 */
	...
	static class ThrowableMethods {
		...
		// Attempt to get methods for java.lang.Throwable on class
		// initialization.
		static {
			try {
				fgThrowableInitCauseMethod = Throwable.class.getMethod("initCause",
						new Class[] { Throwable.class });
				fgThrowableMethodsAvailable = true;
			}
			// ClassNotFoundException, NoSuchMethodException or
			// SecurityException
			// Whatever the case, we cannot use
