...
public class URI implements Serializable {
     ...
     * necessary. If the specification does not follow the "generic URI" syntax,
     * the specification is parsed into a scheme and scheme-specific part
     * (stored as the path) only. Construct a relative URI if boolean is
     * assigned to "true" and p_uriSpec is not valid absolute URI, instead of
    ...
    /**
     * Construct a new URI from a base URI and a URI specification string. The
     * URI specification string may be a relative URI. Construct a relative URI
     * if boolean is assigned to "true" and p_uriSpec is not valid absolute URI
    ...
    private void initialize(URI p_base, String p_uriSpec, boolean allowNonAbsoluteURI)
        ...
        // matching rule.
        // Try matching net_path first, and if that fails we don't have
        // authority so
        // then attempt to match abs_path.
        //
        // net_path = "//" authority [ abs_path ]
        // abs_path = "/" path_segments
        if (((index + 1) < uriSpecLen) && (uriSpec.charAt(index) == '/' && uriSpec.charAt(index
                + 1) == '/')) {
            index += 2;
            int startPos = index;
        ...
        if (host.length() > 0) {
            ...
            if (hasPort) {
                ...
                if (portStr.length() > 0) {
                     ...
                     * (!isDigit(portStr.charAt(i))) { throw new
                     * MalformedURIException( portStr +
    ...
    private boolean isValidServerBasedAuthority(String host, int port, String userinfo) {
        ...
        if (userinfo != null) {
            ...
            while (index < end) {
                ...
                if (testChar == '%') {
                    if (index + 2 >= end || !isHex(userinfo.charAt(index + 1)) || !isHex(userinfo.charAt(index
                            + 2))) {
                        return false;
                    }
                    index += 2;
    ...
    private boolean isValidRegistryBasedAuthority(String authority) {
        ...
        while (index < end) {
            ...
            if (testChar == '%') {
                if (index + 2 >= end || !isHex(authority.charAt(index + 1)) || !isHex(authority.charAt(index
                        + 2))) {
                    return false;
                }
                index += 2;
        ...
        if (start < end) {
            ...
            if (getScheme() == null || p_uriSpec.charAt(start) == '/') {
                ...
                while (index < end) {
                    ...
                    // check for valid escape sequence
                    if (testChar == '%') {
                        if (index + 2 >= end || !isHex(p_uriSpec.charAt(index + 1)) || !isHex(p_uriSpec
                                .charAt(index + 2))) {
                ...
                while (index < end) {
                    ...
                    // contains '[' and ']'. - mrglavas
                        ...
                        throw new MalformedURIException("Opaque part contains invalid character: "
                                + testChar);
                    }
                    ++index;
                }
    ...
    public void setRegBasedAuthority(String authority) throws MalformedURIException {
        ...
        // ";" | ":" | "@" | "&" | "=" | "+" )
        else if (authority.length() < 1 || !isValidRegistryBasedAuthority(authority) || authority.indexOf(
                '/') != -1) {
    ...
    public static boolean isWellFormedIPv4Address(String address) {
        ...
        for (int i = 0; i < addrLength; i++) {
            testChar = address.charAt(i);
            if (testChar == '.') {
                if ((i > 0 && !isDigit(address.charAt(i - 1))) || (i + 1 < addrLength && !isDigit(address
                        .charAt(i + 1)))) {
                    return false;
                }
                numDigits = 0;
            ...
            // Check that this segment is not greater than 255.
                ...
                if (!(first < '2' || (first == '2' && (second < '5' || (second == '5'
                        && testChar <= '5'))))) {
                    return false;
                }
            }
    ...
    public static boolean isWellFormedIPv6Reference(String address) {
        ...
        return (index == end) || (index != -1 && isWellFormedIPv4Address(address.substring(
