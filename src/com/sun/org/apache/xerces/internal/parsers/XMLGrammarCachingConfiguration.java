...
/**
 * <p>
 * This configuration provides a generic way of using Xerces's grammar caching
 * facilities. It extends the XIncludeAwareParserConfiguration and thus may
 * validate documents according to XML schemas or DTD's. It also allows the user
 * to preparse a grammar, and to lock the grammar pool implementation such that
 * no more grammars will be added.
 * </p>
 * <p>
 * Using the com.sun.org.apache.xerces.internal.xni.parser property, an
 * application may instantiate a Xerces SAX or DOM parser with this
 * configuration. When invoked in this manner, the default behaviour will be
 * elicited; to use this configuration's specific facilities, the user will need
...
public class XMLGrammarCachingConfiguration extends XIncludeAwareParserConfiguration {
	...
	protected static final SynchronizedSymbolTable fStaticSymbolTable = new SynchronizedSymbolTable(
			BIG_PRIME);
	...
	/*
	 * lock the XMLGrammarPoolImpl object so that it does not accept any more
	 * grammars from the validators.
	 */
	...
	/*
	 * clear the XMLGrammarPoolImpl object so that it does not contain any more
	 * grammars.
	 */
	...
	/*
	 * unlock the XMLGrammarPoolImpl object so that it accepts more grammars
	 * from the validators.
	 */
	...
	/**
	 * Parse a grammar from a location identified by an URI. This method also
	...
	/**
	 * Parse a grammar from a location identified by an XMLInputSource. This
	...
	/*
	 * This method parses an XML Schema document. It requires a GrammarBucket
	 * parameter so that DOMASBuilder can extract the info it needs. Therefore,
