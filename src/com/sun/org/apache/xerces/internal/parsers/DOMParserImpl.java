...
/**
 * This is Xerces DOM Builder class. It uses the abstract DOM parser with a
 * document scanner, a dtd scanner, and a validator, as well as a grammar pool.
 *
...
public class DOMParserImpl extends AbstractDOMParser implements LSParser, DOMConfiguration {
    ...
    protected static final String VALIDATION_FEATURE = Constants.SAX_FEATURE_PREFIX
            + Constants.VALIDATION_FEATURE;

    /** XML Schema validation */
    protected static final String XMLSCHEMA = Constants.XERCES_FEATURE_PREFIX
            + Constants.SCHEMA_VALIDATION_FEATURE;

    /** XML Schema full checking */
    protected static final String XMLSCHEMA_FULL_CHECKING = Constants.XERCES_FEATURE_PREFIX
    ...
    protected static final String PSVI_AUGMENT = Constants.XERCES_FEATURE_PREFIX
    ...
    public DOMParserImpl(XMLParserConfiguration config, String schemaType) {
        ...
        if (schemaType != null) {
            if (schemaType.equals(Constants.NS_DTD)) {
                ...
                // Exception.
                fConfiguration.setProperty(Constants.JAXP_PROPERTY_PREFIX + Constants.SCHEMA_LANGUAGE,
                        Constants.NS_DTD);
    ...
    public DOMParserImpl(XMLParserConfiguration config) {
        ...
        final String[] domRecognizedFeatures = { Constants.DOM_CANONICAL_FORM, Constants.DOM_CDATA_SECTIONS,
                Constants.DOM_CHARSET_OVERRIDES_XML_ENCODING, Constants.DOM_INFOSET,
                Constants.DOM_NAMESPACE_DECLARATIONS, Constants.DOM_SPLIT_CDATA,
                Constants.DOM_SUPPORTED_MEDIATYPES_ONLY, Constants.DOM_CERTIFIED, Constants.DOM_WELLFORMED,
    ...
    /**
     * Constructs a DOM Builder using the specified symbol table and grammar
     * pool.
     */
     ...
     * filter at the completion of the construction of each <code>Element</code>
     ...
     * or to terminate the parse early. If the document is being validated when
     * it's loaded the validation happens before the filter is called.
     */
    ...
    public void setParameter(String name, Object value) throws DOMException {
        ...
        if (value instanceof Boolean) {
            ...
            try {
                ...
                } else if (name.equalsIgnoreCase(Constants.DOM_SUPPORTED_MEDIATYPES_ONLY) || name
                        .equalsIgnoreCase(Constants.DOM_NORMALIZE_CHARACTERS) || name.equalsIgnoreCase(
            ...
            if (name.equalsIgnoreCase(Constants.DOM_ERROR_HANDLER)) {
                ...
                if (value instanceof LSResourceResolver || value == null) {
                    try {
                        fConfiguration.setProperty(ENTITY_RESOLVER, new DOMEntityResolverWrapper(
                    ...
                    try {
                        if (value == null) {
                            ...
                            if (t.hasMoreTokens()) {
                                ...
                                fConfiguration.setProperty(Constants.JAXP_PROPERTY_PREFIX
                                        + Constants.SCHEMA_SOURCE, fSchemaLocations.toArray());
                            } else {
                                fConfiguration.setProperty(Constants.JAXP_PROPERTY_PREFIX
                ...
                // If this is a boolean parameter a type mismatch should be
                // thrown.
                try {
                    if (name.equals(NAMESPACE_GROWTH)) {
                        normalizedName = NAMESPACE_GROWTH;
    ...
    public Object getParameter(String name) throws DOMException {
        if (name.equalsIgnoreCase(Constants.DOM_COMMENTS)) {
            ...
            boolean infoset = fConfiguration.getFeature(NAMESPACES) && fConfiguration.getFeature(
                    Constants.DOM_NAMESPACE_DECLARATIONS) && fConfiguration.getFeature(
                            INCLUDE_COMMENTS_FEATURE) && fConfiguration.getFeature(
                                    INCLUDE_IGNORABLE_WHITESPACE) && !fConfiguration.getFeature(
                                            DYNAMIC_VALIDATION) && !fConfiguration.getFeature(
                                                    CREATE_ENTITY_REF_NODES) && !fConfiguration.getFeature(
                                                            NORMALIZE_DATA) && !fConfiguration.getFeature(
        ...
        } else if (name.equalsIgnoreCase(Constants.DOM_CHECK_CHAR_NORMALIZATION) || name.equalsIgnoreCase(
                Constants.DOM_NORMALIZE_CHARACTERS)) {
            return Boolean.FALSE;
        } else if (name.equalsIgnoreCase(Constants.DOM_NAMESPACE_DECLARATIONS) || name.equalsIgnoreCase(
                Constants.DOM_WELLFORMED) || name.equalsIgnoreCase(
                        Constants.DOM_IGNORE_UNKNOWN_CHARACTER_DENORMALIZATIONS) || name.equalsIgnoreCase(
            ...
            try {
                XMLEntityResolver entityResolver = (XMLEntityResolver) fConfiguration.getProperty(
                        ENTITY_RESOLVER);
    ...
    public boolean canSetParameter(String name, Object value) {
        ...
        if (value instanceof Boolean) {
            boolean state = ((Boolean) value).booleanValue();
            if (name.equalsIgnoreCase(Constants.DOM_SUPPORTED_MEDIATYPES_ONLY) || name.equalsIgnoreCase(
                    Constants.DOM_NORMALIZE_CHARACTERS) || name.equalsIgnoreCase(
                            Constants.DOM_CHECK_CHAR_NORMALIZATION) || name.equalsIgnoreCase(
                                    Constants.DOM_CANONICAL_FORM)) {
            ...
            } else if (name.equalsIgnoreCase(Constants.DOM_CDATA_SECTIONS) || name.equalsIgnoreCase(
                    Constants.DOM_CHARSET_OVERRIDES_XML_ENCODING) || name.equalsIgnoreCase(
                                            ...
                                            Constants.DOM_DISALLOW_DOCTYPE) || name.equalsIgnoreCase(
                                                    Constants.DOM_ENTITIES) || name.equalsIgnoreCase(
                                                            Constants.DOM_INFOSET) || name.equalsIgnoreCase(
                                                                    Constants.DOM_NAMESPACES) || name
                                                                            .equalsIgnoreCase(
                                                                                    Constants.DOM_NAMESPACE_DECLARATIONS)
                    || name.equalsIgnoreCase(Constants.DOM_VALIDATE) || name.equalsIgnoreCase(
            ...
            if (name.equalsIgnoreCase(Constants.DOM_ERROR_HANDLER)) {
                ...
                if ((value instanceof String && (value.equals(Constants.NS_XMLSCHEMA) || value.equals(
    ...
    /**
     * Parse an XML document from a location identified by an URI reference. If
     * the URI contains a fragment identifier (see section 4.1 in ), the
     * behavior is not defined by this specification.
     */
    public Document parseURI(String uri) throws LSException {

        // If DOMParser insstance is already busy parsing another document when
        // this
     ...
     * the position epcified with the <code>contextNode</code> and
    ...
    XMLInputSource dom2xmlInputSource(LSInput is) {
        ...
        else if ((is.getSystemId() != null && is.getSystemId().length() > 0) || (is.getPublicId() != null
                && is.getPublicId().length() > 0)) {
    ...
    private static DOMException newFeatureNotFoundError(String name) {
        String msg = DOMMessageFormatter.formatMessage(DOMMessageFormatter.DOM_DOMAIN, "FEATURE_NOT_FOUND",
                new Object[] { name });
        return new DOMException(DOMException.NOT_FOUND_ERR, msg);
    }

    private static DOMException newTypeMismatchError(String name) {
        String msg = DOMMessageFormatter.formatMessage(DOMMessageFormatter.DOM_DOMAIN, "TYPE_MISMATCH_ERR",
                new Object[] { name });
