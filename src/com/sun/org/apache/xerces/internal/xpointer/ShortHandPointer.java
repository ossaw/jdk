...
/**
 * <p>
 * Implements the XPointerPart interface and handles processing of ShortHand
...
class ShortHandPointer implements XPointerPart {
	...
	/**
	 * The XPointerProcessor takes care of this. Simply set the ShortHand
	 * Pointer here.
	 *
	 ...
	 * Section 3.2 of the XPointer Framework Recommendation. Note that in the
	 * current implementation only supports DTD determined ID's.
	 *
	 * @see com.sun.org.apache.xerces.internal.xpointer.XPointerPart#
	 ...
	 * @throws XNIException
	 */
	private boolean hasMatchingIdentifier(QName element, XMLAttributes attributes,
		...
		if (attributes != null) {
			for (int i = 0; i < attributes.getLength(); i++) {
				...
				// 1. If an element information item has an attribute
				// information item
				// among its [attributes] that is a schema-determined ID, then
				// it is
				...
				// item among
				// its [children] that is a schema-determined ID, then it is
				// identified by
				// the value of that element information item's [schema
				...
				// 3. If an element information item has an attribute
				// information item among
				// its [attributes] that is a DTD-determined ID, then it is
				// identified by the
				// value of that attribute information item's [normalized value]
				// property.
				// An attribute information item is a DTD-determined ID if and
				// only if it has
	...
	public String getSchemaDeterminedID(XMLAttributes attributes, int index) throws XNIException {
		...
		if (attrPSVI != null) {
			...
			// 1. It has a [member type definition] or [type definition]
			// property
			// whose value in turn has [name] equal to ID and [target namespace]
			// equal to http://www.w3.org/2001/XMLSchema;

			// 2. It has a [base type definition] whose value has that [name]
			// and [target namespace];

			// 3. It has a [base type definition] whose value has a [base type
			// definition]
			// whose value has that [name] and [target namespace], and so on
			// following
