...
public class XSGrammarBucket {
	...
	/**
	 * Put a schema grammar into the registry This method is for internal use
	 * only: it assumes that a grammar with the same target namespace is not
	...
	public boolean putGrammar(SchemaGrammar grammar, boolean deep) {
		...
		if (sg != null) {
			// if the one we have is different from the one passed, it's an
			// error
			return sg == grammar;
		}
