...
public class XSElementDecl implements XSElementDeclaration {
		...
		if (fDescription == null) {
			if (fTargetNamespace != null) {
				StringBuffer buffer = new StringBuffer(
						fTargetNamespace.length() + ((fName != null) ? fName.length() : 4) + 3);
				buffer.append('"');
	 ...
	 * declarations within named model groups: their scope will be determined
	 * when they are used in the construction of complex type definitions.
	 */
	...
	/**
	 * Locally scoped declarations are available for use only within the complex
	 * type definition identified by the <code>scope</code> property.
	 */
	 ...
	 * the namespace qualified attribute with [local name] nil from namespace
	 * http://www.w3.org/2001/XMLSchema-instance and value true (see xsi:nil
	 * (2.6.2)) even if it has no text or element content despite a {content
	 ...
	 * 
	 * @param exclusion
	 *            Extension, restriction or none. Represents final set for the
	 *            element.
	...
	/**
	 * Specifies if this declaration can be nominated as the {substitution group
	 * affiliation} of other element declarations having the same {type
	 * definition} or types derived therefrom.
	 *
	 ...
	 * 
	 * @param disallowed
	 *            Substitution, extension, restriction or none. Represents a
	 ...
	 *
	 * @return A bit flag representing {substitution, extension, restriction} or
	 *         NONE.
	 */
