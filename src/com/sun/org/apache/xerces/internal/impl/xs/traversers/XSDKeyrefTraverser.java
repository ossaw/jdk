...
class XSDKeyrefTraverser extends XSDAbstractIDConstraintTraverser {
    ...
    void traverse(Element krElem, XSElementDecl element,
        ...
        Object[] attrValues = fAttrChecker.checkAttributes(krElem, false,
                schemaDoc);
        ...
        if (krName == null) {
            reportSchemaError("s4s-att-must-appear", new Object[] {
                    SchemaSymbols.ELT_KEYREF, SchemaSymbols.ATT_NAME }, krElem);
        ...
        IdentityConstraint ret = (IdentityConstraint) fSchemaHandler
                .getGlobalDecl(schemaDoc, XSDHandler.IDENTITYCONSTRAINT_TYPE,
                        kName, krElem);
        ...
        if (ret != null) {
            if (ret.getCategory() == IdentityConstraint.IC_KEY || ret
                    .getCategory() == IdentityConstraint.IC_UNIQUE) {
                key = (UniqueOrKey) ret;
            } else {
                reportSchemaError("src-resolve", new Object[] { kName.rawname,
                        "identity constraint key/unique" }, krElem);
            }
        }
        ...
        KeyRef keyRef = new KeyRef(schemaDoc.fTargetNamespace, krName,
                element.fName, key);
        ...
        if (traverseIdentityConstraint(keyRef, krElem, schemaDoc, attrValues)) {
            // Schema Component Constraint: Identity-constraint Definition
            // Properties Correct
            // 2 If the {identity-constraint category} is keyref, the
            // cardinality of the {fields} must equal that of the {fields} of
            // the {referenced key}.
            if (key.getFieldCount() != keyRef.getFieldCount()) {
                reportSchemaError("c-props-correct.2", new Object[] { krName,
                        key.getIdentityConstraintName() }, krElem);
            } else {
                // add key reference to element decl
                // and stuff this in the grammar
                if (grammar.getIDConstraintDecl(keyRef
                ...
                final IdentityConstraint idc = grammar.getIDConstraintDecl(
                        keyRef.getIdentityConstraintName(), loc);
                if (idc == null) {
