...
/**
 * The XML representation for a wildcard declaration schema component is an
...
public class XSWildcardDecl implements XSWildcard {
	...
	// * When we really support not(NS1,NS2,...,NSN), we need to revisit these
	// items.
	...
	public boolean allowNamespace(String namespace) {
		// For a value which is either a namespace name or absent to be valid
		// with respect to a wildcard constraint (the value of a {namespace
		// constraint}) one of the following must be true:
		...
		// / 2 The constraint is a pair of not and a set, and the value is not
		// in such set.
	...
	public boolean isSubsetOf(XSWildcardDecl superWildcard) {
		...
		// For a namespace constraint (call it sub) to be an intensional subset
		// of another
	...
	public XSWildcardDecl performUnionWith(XSWildcardDecl wildcard, short processContents) {
		...
		// -5.1 If the set includes the negated namespace name, then any must be
		// the value.
		// -5.2 If the set does not include the negated namespace name, then
		// whichever of O1 or O2
		// is a pair of not and a namespace name must be the value.
		// +5.1 If the negated value is a namespace name, then The appropriate
		// case
		// among the following must be true:
		// +5.1.1 If the set includes both the namespace name and absent, then
		// any
		// must be the value.
		...
		// +5.1.4 If the set does not include either the namespace name or
		// absent,
		// then whichever of O1 or O2 is a pair of not and a namespace name must
		// be
		// the value.
		// +5.2 If the negated value is absent, then The appropriate case among
		// the
		// following must be true:
		// +5.2.1 If the set includes absent, then any must be the value.
		// +5.2.2 If the set does not include absent, then whichever of O1 or O2
		// is
	...
	public XSWildcardDecl performIntersectionWith(XSWildcardDecl wildcard, short processContents) {
		...
		// +3 If either O1 or O2 is a pair of not and a namespace name and the
		// other
		// is a set of (namespace names or absent), then that set, minus the
		// negated
		// namespace name if it was in the set, then minus absent if it was in
		// the
	...
	/**
	 * Namespace constraint. For <code>constraintType</code> LIST_NSCONSTRAINT,
	 * the list contains allowed namespaces. For <code>constraintType</code>
	 * NOT_NSCONSTRAINT, the list contains disallowed namespaces.
	 */
	public StringList getNsConstraintList() {
		return new StringListImpl(fNamespaceList,
				fNamespaceList == null ? 0 : fNamespaceList.length);
	}
