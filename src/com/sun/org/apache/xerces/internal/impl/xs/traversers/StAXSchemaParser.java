...
/**
 * <p>
 * StAXSchemaParser reads StAX events, converts them into XNI events and passes
 * them directly to the SchemaDOMParser.
 * </p>
 *
 * @xerces.internal
 *
...
final class StAXSchemaParser {
	...
	private final JAXPNamespaceContextWrapper fNamespaceContext = new JAXPNamespaceContextWrapper(
			fSymbolTable);
		...
		if (input.hasNext()) {
			...
			loop: while (input.hasNext()) {
				...
				case XMLStreamConstants.START_ELEMENT:
					...
					fillQName(fElementQName, input.getNamespaceURI(), input.getLocalName(),
							input.getPrefix());
				...
				case XMLStreamConstants.END_ELEMENT:
					...
					break;
				case XMLStreamConstants.CHARACTERS:
					fTempString.setValues(input.getTextCharacters(), input.getTextStart(),
							input.getTextLength());
