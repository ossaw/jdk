...
public class XSDDescription extends XMLResourceIdentifierImpl implements
	...
	/**
	 * Indicate that the current schema document is <include>d by another schema
	 * document.
	 */
	...
	/**
	 * Indicate that the current schema document is <import>ed by another schema
	 * document.
	 */
	 ...
	 * xsi:schemaLocation/noNamespaceSchemaLocation attribute(s) in the instance
	 * document. This value is only used if we don't defer the loading of schema
	 * documents.
	 */
	public final static short CONTEXT_INSTANCE = 4;
	/**
	 * Indicate that the parse of the current schema document is triggered by
	 * the occurrence of an element whose namespace is the target namespace of
	 * this schema document. This value is only used if we do defer the loading
	 * of schema documents until a component from that namespace is referenced
	 * from the instance.
	 */
	public final static short CONTEXT_ELEMENT = 5;
	/**
	 * Indicate that the parse of the current schema document is triggered by
	 * the occurrence of an attribute whose namespace is the target namespace of
	 * this schema document. This value is only used if we do defer the loading
	 * of schema documents until a component from that namespace is referenced
	 * from the instance.
	 */
	...
	/**
	 * Indicate that the parse of the current schema document is triggered by
	 * the occurrence of an "xsi:type" attribute, whose value (a QName) has the
	 * target namespace of this schema document as its namespace. This value is
	 * only used if we do defer the loading of schema documents until a
	 * component from that namespace is referenced from the instance.
	 */
	...
	/**
	 * Get the context. The returned value is one of the pre-defined CONTEXT_xxx
	 * constants.
	 *
	 ...
	 *
	 * @return an array of all location hints associated to the expected target
	 *         namespace
	 */
	 ...
	 * this call returns the name of such triggering component: the name of the
	 * element/attribute, or the value of the xsi:type.
	 *
	...
	/**
	 * Compares this grammar with the given grammar. Currently, we compare the
	 * target namespaces.
	 *
	 * @param descObj
