...
/**
 *
 * @author Joe Wang: This is a rewrite of the original class. The focus is on
 *         removing caching, and make the filtered stream reader more compatible
 *         with those in other implementations. Note however, that this version
 *         will not solve all the issues related to the undefined condition in
 *         the spec. The priority is to pass the TCK. Issues arising due to the
 *         requirement, that is, (1) should it initiate at BEGIN_DOCUMENT or an
 *         accepted event; (2) should hasNext() advance the underlining stream
 *         in order to find an acceptable event, would have to wait until 1.1 of
 *         StAX in which the filtered stream reader would be defined more
 *         clearly.
 */
...
public class XMLStreamFilterImpl implements javax.xml.stream.XMLStreamReader {
	...
	/**
	 * the very issue around a long discussion. but since we must pass the TCK,
	 * we have to allow hasNext() to advance the underlining stream in order to
	 * find the next acceptable event
	 */
	...
	public XMLStreamFilterImpl(XMLStreamReader reader, StreamFilter filter) {
		...
		// but it's neccessary in order to pass the TCK and yet avoid skipping
		// element
		try {
	...
	private int findNextEvent() throws XMLStreamException {
		...
		// although it seems that IllegalStateException should be thrown when
		// next() is called
		// on a stream that has no more items, we have to assume END_DOCUMENT is
		// always accepted
