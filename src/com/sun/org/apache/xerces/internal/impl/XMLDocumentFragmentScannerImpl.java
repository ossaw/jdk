 ...
 * @version $Id: XMLDocumentFragmentScannerImpl.java,v 1.19 2010-11-02 19:54:55
 *          joehw Exp $
 *
 */
public class XMLDocumentFragmentScannerImpl extends XMLScanner implements
        XMLDocumentScanner, XMLComponent, XMLEntityHandler, XMLBufferListener {
    ...
    protected static final String STANDARD_URI_CONFORMANT = Constants.XERCES_FEATURE_PREFIX
    ...
    /**
     * access external dtd: file protocol For DOM/SAX, the secure feature is set
     * to true by default
     */
    ...
    private static final String[] RECOGNIZED_FEATURES = { NAMESPACES,
            VALIDATION, NOTIFY_BUILTIN_REFS, NOTIFY_CHAR_REFS,
            Constants.STAX_REPORT_CDATA_EVENT };

    /** Feature defaults. */
    private static final Boolean[] FEATURE_DEFAULTS = { Boolean.TRUE, null,
            Boolean.FALSE, Boolean.FALSE, Boolean.TRUE };

    /** Recognized properties. */
    private static final String[] RECOGNIZED_PROPERTIES = { SYMBOL_TABLE,
            ERROR_REPORTER, ENTITY_MANAGER, XML_SECURITY_PROPERTY_MANAGER };
    ...
    /**
     * Document system identifier. REVISIT: So what's this used for? - NG
     * protected String fDocumentSystemId;
     ******/

    protected String fPITarget;

    // xxx do we need to create an extra XMLString object... look for using
    // fTempString for collecting all the data values
    ...
    /**
     * comma-delimited list of protocols that are allowed for the purpose of
     * accessing external dtd or entity references
     */
     ...
     * This class implements Iterator interface so we can directly give
     * Attributes in the form of iterator.
     */
    ...
    public void setInputSource(XMLInputSource inputSource) throws IOException {
        ...
        fEntityManager.startEntity(false, "$fragment$", inputSource, false,
                true);
        // fDocumentSystemId =
    ...
    public boolean scanDocument(boolean complete) throws IOException,
        ...
        fEntityManager.setEntityHandler(this);
        // System.out.println(" get Document Handler in NSDocumentHandler " +
        // fDocumentHandler );
        ...
        do {
            switch (event) {
                case XMLStreamConstants.START_DOCUMENT:
                    // fDocumentHandler.startDocument(fEntityManager.getEntityScanner(),fEntityManager.getEntityScanner().getVersion(),fNamespaceContext,null);//
                    // not able to get
                    break;
                ...
                case XMLStreamConstants.CHARACTERS:
                    fEntityScanner.checkNodeCount(
                            fEntityScanner.fCurrentEntity);
                ...
                case XMLStreamConstants.SPACE:
                    ...
                    // fDocumentHandler.ignorableWhitespace(getCharacterData(),
                    // null);
                    break;
                ...
                case XMLStreamConstants.PROCESSING_INSTRUCTION:
                    fEntityScanner.checkNodeCount(
                            fEntityScanner.fCurrentEntity);
                    fDocumentHandler.processingInstruction(getPITarget(),
                            getPIData(), null);
                    break;
                ...
                case XMLStreamConstants.DTD:
                    ...
                    // 1. Stax doesn't define DTD states as it does for XML
                    // Document.
                    // therefore we don't need to take care of anything here. So
                    // Just break;
                    break;
                case XMLStreamConstants.CDATA:
                    ...
                    // xxx: check if CDATA values comes from getCharacterData()
                    // function
                    fDocumentHandler.characters(getCharacterData(), null);
                    fDocumentHandler.endCDATA(null);
                    // System.out.println(" in CDATA of the
    ...
    public void reset(XMLComponentManager componentManager)
        ...
        // xerces features
        fReportCdataEvent = componentManager.getFeature(
                Constants.STAX_REPORT_CDATA_EVENT, true);
        fSecurityManager = (XMLSecurityManager) componentManager.getProperty(
                Constants.SECURITY_MANAGER, null);
        ...
        XMLSecurityPropertyManager spm = (XMLSecurityPropertyManager) componentManager
                .getProperty(XML_SECURITY_PROPERTY_MANAGER, null);
        fAccessExternalDTD = spm.getValue(
    ...
    public void reset(PropertyManager propertyManager) {
        ...
        // other settings
        // fDocumentSystemId = null;
        fNamespaces = ((Boolean) propertyManager.getProperty(
                XMLInputFactory.IS_NAMESPACE_AWARE)).booleanValue();
        fNotifyBuiltInRefs = false;

        // fElementStack2.clear();
        // fReplaceEntityReferences = true;
        // fSupportExternalEntities = true;
        Boolean bo = (Boolean) propertyManager.getProperty(
                XMLInputFactory.IS_REPLACING_ENTITY_REFERENCES);
        fReplaceEntityReferences = bo.booleanValue();
        bo = (Boolean) propertyManager.getProperty(
        ...
        Boolean coalesce = (Boolean) propertyManager.getProperty(
                XMLInputFactory.IS_COALESCING);
        if (coalesce != null)
            fIsCoalesce = coalesce.booleanValue();
        fReportCdataEvent = fIsCoalesce ? false : (fReportCdataEvent && true);
        // if fIsCoalesce is set to true, set the value of
        // fReplaceEntityReferences to true,
        // if fIsCoalesce is set to false, take the value of
        // fReplaceEntityReferences as set by application
        fReplaceEntityReferences = fIsCoalesce ? true
        ...
        // JAXP 1.5 features and properties
        XMLSecurityPropertyManager spm = (XMLSecurityPropertyManager) propertyManager
                .getProperty(XML_SECURITY_PROPERTY_MANAGER);
        fAccessExternalDTD = spm.getValue(
                XMLSecurityPropertyManager.Property.ACCESS_EXTERNAL_DTD);

        fSecurityManager = (XMLSecurityManager) propertyManager.getProperty(
    ...
    void resetCommon() {
        ...
        if (fSecurityManager != null) {
            fElementAttributeLimit = fSecurityManager.getLimit(
                    XMLSecurityManager.Limit.ELEMENT_ATTRIBUTE_LIMIT);
            fXMLNameLimit = fSecurityManager.getLimit(
                    XMLSecurityManager.Limit.MAX_NAME_LIMIT);
        } else {
            fElementAttributeLimit = 0;
            fXMLNameLimit = XMLSecurityManager.Limit.MAX_NAME_LIMIT
                    .defaultValue();
        }
    ...
    public void setFeature(String featureId, boolean state)
        ...
        if (featureId.startsWith(Constants.XERCES_FEATURE_PREFIX)) {
            String feature = featureId.substring(Constants.XERCES_FEATURE_PREFIX
                    .length());
    ...
    public void setProperty(String propertyId, Object value)
        ...
        // Xerces properties
        if (propertyId.startsWith(Constants.XERCES_PROPERTY_PREFIX)) {
            String property = propertyId.substring(
                    Constants.XERCES_PROPERTY_PREFIX.length());
    ...
    public void setDocumentHandler(XMLDocumentHandler documentHandler) {
        ...
        // System.out.println(" In Set DOCUMENT HANDLER" + fDocumentHandler + "
        // scanner =" + this);
     ...
     * pseudo-name of "[dtd]" parameter entity names start with '%'; and general
     ...
     *                   entity
     *                   encoding is not auto-detected (e.g. internal entities
     *                   or a
    ...
    public void startEntity(String name, XMLResourceIdentifier identifier,
        ...
        if (fEntityDepth == fEntityStack.length) {
            ...
            System.arraycopy(fEntityStack, 0, entityarray, 0,
                    fEntityStack.length);
     ...
     *                      Thrown by handler to signal an error.
     */
    public void endEntity(String name, Augmentations augs) throws IOException,
            XNIException {

        /**
         * // flush possible pending output buffer - see scanContent if
         * (fInScanContent && fStringBuffer.length != 0 && fDocumentHandler !=
         * null) { fDocumentHandler.characters(fStringBuffer, null);
     ...
     *                         True if a text declaration is to be scanned
     *                         instead of an XML
     *                         declaration.
     */
    protected void scanXMLDeclOrTextDecl(boolean scanningTextDecl)
        ...
        /// xxx see where its used.. this is not used anywhere. it may be useful
        /// for entity to store this information
        ...
        // set encoding on reader, only if encoding was not specified by the
        // application explicitly
        if (encoding != null && !fEntityScanner.getCurrentEntity()
     ...
     *               The PI target
     * @param data
     *               The XMLStringBuffer to fill in with the data
     */
    ...
    // xxx value returned by this function may not remain valid if another event
    // is scanned.
    ...
    void addElement(String rawname) {
        if (fElementPointer < ELEMENT_ARRAY_LENGTH) {
            ...
            // store pointer information only when element depth is less
            // MAX_DEPTH_LIMIT
            if (fElementStack.fDepth < MAX_DEPTH_LIMIT) {
                short column = storePointerForADepth(fElementPointer);
                if (column > 0) {
                    short pointer = getElementPointer(
                            (short) fElementStack.fDepth, (short) (column - 1));
                    // identity comparison shouldn't take much time and we can
                    // rely on this
                    // since its guaranteed to have same object id for same
                    // string.
    ...
    short storePointerForADepth(short elementPointer) {
        ...
        // Stores element pointer locations at particular depth , only 4 pointer
        // locations
        ...
        for (short i = 0; i < MAX_POINTER_AT_A_DEPTH; i++) {
            ...
            if (canStore(depth, i)) {
                ...
                if (DEBUG_SKIP_ALGORITHM) {
                    ...
                    sb.append(" fElementPointer = " + fElementPointer);
                    sb.append(" fElementStack.fDepth = "
    ...
    boolean canStore(short depth, short column) {
        ...
        // calle should make sure that it doesn't call for value outside allowed
        // co-ordinates
    ...
    boolean skipFromTheBuffer(String rawname) throws IOException {
        if (fEntityScanner.skipString(rawname)) {
            ...
            // If the start element was completely skipped we should encounter
            // either ' '(space),
    ...
    boolean skipQElement(String rawname) throws IOException {
        ...
        // if this character is still valid element name -- this means string
        // can't match
        if (XMLChar.isName(c)) {
            return false;
        } else {
    ...
    protected boolean skipElement() throws IOException {
        ...
        if (fLastPointerLocation != 0) {
            // Look at the next element stored in the array list.. we might just
            // get a match.
            ...
            if (rawname != null && skipFromTheBuffer(rawname)) {
                ...
                // reset it back to zero... we haven't got the correct subset
                // yet.
                fLastPointerLocation = 0;

            }
        }
        // xxx: we can put some logic here as from what column it should start
        // looking
        // for now we always start at 0
        // fallback to tolerant algorithm, it would look for differnt element
        // stored at different
    ...
    boolean skipElement(short column) throws IOException {
        ...
        for (short i = column; i < MAX_POINTER_AT_A_DEPTH; i++) {
            ...
            if (fElementArray[pointer] != null && skipFromTheBuffer(
                    fElementArray[pointer])) {
                if (DEBUG_SKIP_ALGORITHM) {
                    System.out.println();
                    System.out.println("Element " + fElementRawname
                            + " was SKIPPED at depth = " + fElementStack.fDepth
                            + " column = " + column);
                    System.out.println();
                }
                fLastPointerLocation = pointer;
     ...
     * this method. NB: Content in fAttributes is valid only till the state of
     * the parser is XMLEvent.START_ELEMENT
     *
    ...
    protected boolean scanStartElement() throws IOException, XNIException {
        ...
        if (fSkip && !fAdd) {
            ...
            if (fSkip) {
                ...
                // if skipping fails reposition the stack or fallback to normal
                // way of processing
        ...
        // when the elements are being added , we need to check if we are set
        // for skipping the elements
        if (fAdd) {
            // this sets the value of fAdd variable
            fElementStack.matchElement(fElementQName);
        }

        // xxx: We dont need another pointer, fCurrentElement, we can use
        // fElementQName
        ...
        if (fEmptyElement) {
            ...
            // We should not be popping out the context here in endELement
            // becaause the namespace context is still
            ...
            if (fDocumentHandler != null) {
                // complete element and attributes are traversed in this
                // function so we can send a callback
                // here.
                // <strong>we shouldn't be sending callback in
                // scanDocument()</strong>
    ...
    /**
     * Looks for the close of start tag, i.e. if it finds '>' or '/>' Characters
     * are consumed.
     */
    ...
    /**
     * protected void scanAttribute(AttributeIteratorImpl attributes) throws
     * IOException, XNIException { if (DEBUG_START_END_ELEMENT)
     * System.out.println(">>> scanAttribute()");
     *
     *
     * // name if (fNamespaces) { fEntityScanner.scanQName(fAttributeQName); }
     * else { String name = fEntityScanner.scanName();
     * fAttributeQName.setValues(null, name, name, null); }
     *
     * // equals fEntityScanner.skipSpaces(); if (!fEntityScanner.skipChar('='))
     * { reportFatalError("EqRequiredInAttribute", new
     * Object[]{fAttributeQName.rawname}); } fEntityScanner.skipSpaces();
     *
     *
     * // content int oldLen = attributes.getLength();
     */
    /**
     ...
     *
     * // WFC: Unique Att Spec if (oldLen == attributes.getLength()) {
     * reportFatalError("AttributeNotUnique", new
     ...
     * is no boolean isVC = fHasExternalDTD && !fStandalone;
     * scanAttributeValue(fTempString, fTempString2, fAttributeQName.rawname,
     * attributes, oldLen, isVC);
     ...
     * AttributeImpl attribute = new
     * AttributeImpl(fAttributeQName.prefix,fAttributeQName.localpart,
     * fAttributeQName.uri,fTempString.toString(),fTempString2.toString(),
     * XMLSymbols.fCDATASymbol,true); fAttributes.addAttribute(attribute); if
     * (DEBUG_START_END_ELEMENT) System.out.println("<<< scanAttribute()"); } //
    ...
    protected void scanAttribute(XMLAttributes attributes) throws IOException,
            XNIException {
        if (DEBUG_START_END_ELEMENT)
            System.out.println(this.getClass().toString()
                    + ">>> scanAttribute()");
        ...
        // this function doesn't use 'attIndex'. We are adding the attribute
        // later
        // after we have figured out that current attribute is not namespace
        // declaration
        ...
        scanAttributeValue(tmpStr, fTempString2, fAttributeQName.rawname,
                attributes, attIndex, isVC, fCurrentElement.rawname, false);

        // content
        int oldLen = attributes.getLength();
        // if the attribute name already exists.. new value is replaced with old
        // value
        attIndex = attributes.addAttribute(fAttributeQName,
                XMLSymbols.fCDATASymbol, null);

        // WFC: Unique Att Spec
        // attributes count will be same if the current attribute name already
        // exists for this element name.
        ...
        /// xxx: nonNormalizedValue is not being set as it is not required by
        /// SAX & DOM
        // attributes.setNonNormalizedValue(oldLen, fTempString2.toString());
    ...
    // It makes things easy if this functions takes XMLStringBuffer as
    // parameter..
    // this function appends the data to the buffer.
    protected int scanContent(XMLStringBuffer content) throws IOException,
            XNIException {
        ...
        if (c == '\r') {
            ...
            // xxx: We know the next chracter.. we should just skip it and add
            // ']' directlry
            fEntityScanner.scanChar(null);
     ...
     * <p>
     * <strong>Note:</strong> This method uses the fTempString and fStringBuffer
     * variables.
     *
     * @param complete
     *                 True if the CDATA section is to be scanned completely.
     *
    ...
    protected boolean scanCDATASection(XMLStringBuffer contentBuffer,
        ...
        while (true) {
            ...
            if (!fEntityScanner.scanData("]]>", contentBuffer)) {
                 ...
                 * int brackets = 2; while (fEntityScanner.skipChar(']')) {
                 * brackets++; }
                 *
                 * //When we find more than 2 square brackets if
                 * (fDocumentHandler != null && brackets > 2) { //we dont need
                 * to clear the buffer.. //contentBuffer.clear(); for (int i =
                 * 2; i < brackets; i++) { contentBuffer.append(']'); }
                 * fDocumentHandler.characters(contentBuffer, null); }
                 *
                 * if (fEntityScanner.skipChar('>')) { break; } if
                 * (fDocumentHandler != null) { //we dont need to clear the
                 * buffer now.. //contentBuffer.clear();
    ...
    protected int scanEndElement() throws IOException, XNIException {
        ...
        if (DEBUG)
            System.out.println("endElementName = " + endElementName.toString());
        ...
        // We should not be popping out the context here in endELement becaause
        // the namespace context is still
        // valid when parser is at the endElement state.
    ...
    protected void scanCharReference() throws IOException, XNIException {
        ...
        if (ch != -1) {
            ...
            if (fDocumentHandler != null) {
                ...
                // xxx: How do we deal with this - how to return
                // charReferenceValues
                // now this is being commented because this is taken care in
                // scanDocument()
     ...
     *
     * @return returns true if the new entity is started. If it was built-in
     *         entity 'false' is returned.
     * @throws IOException
    ...
    protected void scanEntityReference(XMLStringBuffer content)
        ...
        // 1. if the entity is external and support to external entities is not
        // required
        ...
        if ((isEE && !fSupportExternalEntities) || (!isEE
                && !fReplaceEntityReferences) || foundBuiltInRefs) {
            fScannerState = SCANNER_STATE_REFERENCE;
            return;
        }
        // start general entity
        if (!fEntityStore.isDeclaredEntity(name)) {
            // SUPPORT_DTD=false && ReplaceEntityReferences should throw
            // exception
            ...
            if (fHasExternalDTD && !fStandalone) {
                if (fValidation)
                    fErrorReporter.reportError(fEntityScanner,
        ...
        // if that was the case it its taken care in
        // XMLEntityManager.startEntity()
        // we immediately call the endEntity. Application gets to know if there
        // was
        // any entity that was not declared.
        fEntityManager.startEntity(true, name, false);
        // set the scaner state to content.. parser will automatically revive
    ...
    void checkDepth(String elementName) {
        fLimitAnalyzer.addValue(Limit.MAX_ELEMENT_DEPTH_LIMIT, elementName,
                fElementStack.fDepth);
        if (fSecurityManager.isOverLimit(Limit.MAX_ELEMENT_DEPTH_LIMIT,
                fLimitAnalyzer)) {
     ...
     * @param entity
     *                        built-in name
     * @param XMLStringBuffer
     *                        append the character to buffer
     *
     *                        we really dont need to call this function -- this
     *                        function is
     *                        only required when we integrate with rest of
     *                        Xerces2. SO
     *                        maintaining the current behavior and still calling
     *                        this
     *                        function to hanlde built-in entity reference.
     *
     */
     ...
     *                         a list of allowed protocols separated by comma
     * @return the name of the protocol if rejected, null otherwise
     */
    String checkAccess(String systemId, String allowedProtocols)
            throws IOException {
        String baseSystemId = fEntityScanner.getBaseSystemId();
        String expandedSystemId = XMLEntityManager.expandSystemId(systemId,
                baseSystemId, fStrictURI);
        return SecuritySupport.checkAccess(expandedSystemId, allowedProtocols,
    ...
    protected class ElementStack2 {
        ...
        public void resize() {
            /**
             * int length = fElements.length; Element [] temp = new
             * Element[length * 2]; System.arraycopy(fElements, 0, temp, 0,
             * length); fElements = temp;
             */
            // resize QNames
        ...
        /**
         * Check if the element scanned during the start element matches the
         * stored element.
         *
        ...
        public boolean matchElement(QName element) {
            ...
            if (fLastDepth > fDepth && fDepth <= 2) {
                if (DEBUG_SKIP_ALGORITHM) {
                    System.out.println("Checking if the elements match "
                            + element.rawname + " , " + fQName[fDepth].rawname);
                }
                if (element.rawname == fQName[fDepth].rawname) {
                    ...
                    fMark = fDepth - 1;
                    // we found the match and from next element skipping will
                    // start, add 1
                    fPosition = fMark + 1;
                    match = true;
                    // Once we get match decrease the count -- this was
                    // increased by nextElement()
                    --fCount;
                    if (DEBUG_SKIP_ALGORITHM) {
                        System.out.println(
                                "fAdd FALSE -- NOW ELEMENT SHOULD NOT BE ADDED");
                    ...
                    fAdd = true;
                    if (DEBUG_SKIP_ALGORITHM)
                        System.out.println("fAdd is " + fAdd);
                }
            }
        ...
        /**
         * This function doesn't increase depth. The function in this function
         * is broken down into two functions for efficiency.
         * <@see>matchElement</see>. This function just returns the pointer to
         * the object and its values are set.
         *
         * @return QName reference to the next element in the list
         */
        public QName nextElement() {

            // if number of elements becomes equal to the length of array --
            // stop the skipping
            if (fCount == fQName.length) {
                fShouldSkip = false;
                fAdd = false;
                if (DEBUG_SKIP_ALGORITHM)
                    System.out.println("SKIPPING STOPPED, fShouldSkip = "
                            + fShouldSkip);
                // xxx: this is not correct, we are returning the last element
                // this wont make any difference since flag has been set to
                // 'false'
     ...
     * checking, and it re-uses objects instead of throwing popped items away.
     *
     * @author Andy Clark, IBM
     */
    protected class ElementStack {
         ...
         * stack. Also, the QName object returned is <em>not</em> orphaned to
         * the caller. It should be considered read-only.
         *
         * @param element
        ...
        public QName getNext() {
            ...
            if (DEBUG_SKIP_ALGORITHM) {
                System.out.println("Element at fPosition = " + fPosition
                        + " is " + fElements[fPosition].rawname);
            }
        ...
        /**
         * This function should be called only when element was skipped
         * sucessfully. 1. Increase the depth - because element was sucessfully
         * skipped. 2. Store the position of the element token in array
         * "last opened tag" at depth. 3. increase the position counter so as to
        ...
        public boolean matchElement(QName element) {
            ...
            // if last depth is greater than current depth
            // if(DEBUG_SKIP_ALGORITHM){
            // System.out.println("Check if the element " + element.rawname + "
            // matches");
            // System.out.println("fLastDepth = " + fLastDepth);
            // System.out.println("fDepth = " + fDepth);
            // }
            ...
            if (fLastDepth > fDepth && fDepth <= 3) {
                if (DEBUG_SKIP_ALGORITHM) {
                    System.out.println(
                            "----------ENTERED THE LOOP WHERE WE CHECK FOR MATCHING OF ELMENT-----");
                    System.out.println("Depth = " + fDepth
                            + " Checking if INCOMING element " + element.rawname
                            + " match STORED ELEMENT " + fElements[fDepth
                                    - 1].rawname);
                }
                if (element.rawname == fElements[fDepth - 1].rawname) {
                    ...
                    if (DEBUG_SKIP_ALGORITHM) {
                        System.out.println(
                                "NOW ELEMENT SHOULD NOT BE ADDED, fAdd is set to false");
                        System.out.println("fMark = " + fMark);
                        System.out.println("fPosition = " + fPosition);
                        System.out.println("fDepth = " + fDepth);
                        System.out.println("fCount = " + fCount);
            ...
            if (match) {
                ...
                if (DEBUG_SKIP_ALGORITHM) {
                    System.out.println("At depth = " + fDepth
                            + "array position is = " + (fCount - 1));
                }
                // sicne fInt[fDepth] contains pointer to the element array
                // which are 0 based.
            ...
            if (fCount == fElements.length) {
                fSkip = false;
                fAdd = false;
                // reposition the stack -- it seems to be too complex document
                // and there is no symmerty in structure
                reposition();
                if (DEBUG_SKIP_ALGORITHM) {
                    System.out.println(
                            "ALL THE ELMENTS IN ARRAY HAVE BEEN FILLED");
            ...
            if (DEBUG_SKIP_ALGORITHM) {
                if (match) {
                    System.out.println("Storing fPosition = " + fInt[fDepth]
                            + " at fDepth = " + fDepth);
                } else {
                    System.out.println("Storing fCount = " + fInt[fDepth]
                            + " at fDepth = " + fDepth);
                }
            }
            // store the last depth
         ...
         *
         * @return Returns the actual QName object. Callee should use this
         *         object to store the details of next element encountered.
         */
        public QName nextElement() {
            if (fSkip) {
                fDepth++;
        ...
        /**
         * Pops an element off of the stack by setting the values of the
         * specified QName.
         * <p>
         * <strong>Note:</strong> The object returned is <em>not</em> orphaned
         * to the caller. Therefore, the caller should consider the object to be
         * read-only.
         */
        public QName popElement() {
            // return the same object that was pushed -- this would avoid
            // setting the values for every end element.
            // STRONG: this object is read only -- this object reference
            // shouldn't be stored.
            if (fSkip || fAdd) {
                if (DEBUG_SKIP_ALGORITHM) {
                    System.out.println("POPPING Element, at position "
                            + fInt[fDepth] + " element at that count is = "
                ...
                if (DEBUG_SKIP_ALGORITHM) {
                    System.out.println("Retrieveing element at depth = "
                            + fDepth + " is " + fElements[fDepth].rawname);
                }
                return fElements[--fDepth];
            }
        ...
        /**
         * Reposition the stack. fInt [] contains all the opened tags at
         * particular depth. Transfer all the opened tags starting from depth
         * '2' to the current depth and reposition them as per the depth.
         */
        public void reposition() {
            ...
            if (DEBUG_SKIP_ALGORITHM) {
                for (int i = 0; i < fDepth; i++) {
                    System.out.println("fElements[" + i + "]" + " = "
                            + fElements[i].rawname);
                }
            }
        }
         ...
         * we dont need to set the value for every end element encouterd. For
         * Well formedness checks we can have the same QName object that was
         * pushed. the values will be set only if application need to know about
         * the endElement -- neeraj.bajaj@sun.com
         */
    ...
    /**
     * Drives the parser to the next state/event on the input. Parser is
     * guaranteed to stop at the next state/event.
     *
     * Internally XML document is divided into several states. Each state
     * represents a sections of XML document. When this functions returns
     * normally, it has read the section of XML document and returns the state
     * corresponding to section of document which has been read. For
     * optimizations, a particular driver can read ahead of the section of
     * document (state returned) just read and can maintain a different internal
     * state.
     *
     *
     * @author Neeraj Bajaj, Sun Microsystems
    ...
    protected interface Driver {
         ...
         * Internally XML document is divided into several states. Each state
         * represents a sections of XML document. When this functions returns
         * normally, it has read the section of XML document and returns the
         * state corresponding to section of document which has been read. For
         * optimizations, a particular driver can read ahead of the section of
         * document (state returned) just read and can maintain a different
         * internal state.
         *
    ...
    /**
     * Driver to handle content scanning. This driver is capable of reading the
     * fragment of XML document. When it has finished reading fragment of XML
     * documents, it can pass the job of reading to another driver.
     *
     * This class has been modified as per the new design which is more suited
     * to efficiently build pull parser. Lot of performance improvements have
     * been done and the code has been added to support stax
    ...
    protected class FragmentContentDriver implements Driver {
        ...
        /**
         *
         * SCANNER_STATE_CONTENT and SCANNER_STATE_START_OF_MARKUP are two super
         * states of the parser. At any point of time when in doubt over the
         * current state of the parser, the state should be set to
         * SCANNER_STATE_CONTENT. Parser will automatically revive itself and
         * will set state of the parser to one of its sub state. sub states are
         * defined in the parser on the basis of different XML component like
         * SCANNER_STATE_ENTITY_REFERENCE , SCANNER_STATE_START_ELEMENT,
         * SCANNER_STATE_CDATA etc.. These sub states help the parser to have
         * fine control over the parsing. These are the different milepost,
         * parser stops at each sub state (milepost). Based on this state it is
        ...
        /**
         * Drives the parser to the next state/event on the input. Parser is
         * guaranteed to stop at the next state/event. Internally XML document
         * is divided into several states. Each state represents a sections of
         * XML document. When this functions returns normally, it has read the
         * section of XML document and returns the state corresponding to
         * section of document which has been read. For optimizations, a
         * particular driver can read ahead of the section of document (state
         * returned) just read and can maintain a different internal state.
         *
        ...
        public int next() throws IOException, XNIException {
            while (true) {
                try {
                    ...
                    // decide the actual sub state of the scanner.For more
                    // information refer to the javadoc of
                    // decideSubState.
                    ...
                    if (fIsCoalesce) {
                        ...
                        if (fLastSectionWasCharacterData) {
                            ...
                            // if we dont encounter any CDATA or ENITY REFERENCE
                            // and current state is also not
                          ...
                          // xxx: there might be another entity reference or
                          // CDATA after this
                          // <foo>blah blah &amp;&lt;<![CDATA[[aa]]>blah
                          // blah</foo>
                        else if ((fLastSectionWasCData
                                || fLastSectionWasEntityReference)) {
                            // and current state is not
                            // SCANNER_STATE_CHARACTER_DATA
                    ...
                    switch (fScannerState) {
                        ...
                        case SCANNER_STATE_START_ELEMENT_TAG: {
                            ...
                            // xxx this function returns true when element is
                            // empty.. can be linked to end element event.
                            // returns true if the element is empty
                            fEmptyElement = scanStartElement();
                            // if the element is empty the next event is "end
                            // element"
                            if (fEmptyElement) {
                                setScannerState(SCANNER_STATE_END_ELEMENT_TAG);
                            } else {
                        ...
                        case SCANNER_STATE_CHARACTER_DATA: {
                            if (DEBUG_COALESCE) {
                                System.out.println("fLastSectionWasCData = "
                                        + fLastSectionWasCData);
                                System.out.println("fIsCoalesce = "
                                        + fIsCoalesce);
                            }
                            // if last section was either entity reference or cdata
                            // or character data we should be using buffer
                            fUsebuffer = fLastSectionWasEntityReference
                                    || fLastSectionWasCData
                                    || fLastSectionWasCharacterData;

                            // When coalesce is set to true and last state was
                            // REFERENCE or CDATA or CHARACTER_DATA, buffer should
                            // not be cleared.
                            ...
                            // set the fTempString length to 0 before passing it on
                            // to scanContent
                            // scanContent sets the correct co-ordinates as per the
                            // content read
                            fTempString.length = 0;
                            int c = fEntityScanner.scanContent(fTempString);
                            if (DEBUG) {
                                System.out.println("fTempString = "
                                        + fTempString);
                            }
                            if (fEntityScanner.skipChar('<', null)) {
                                ...
                                if (fEntityScanner.skipChar('/',
                                ...
                                } else if (XMLChar.isNameStart(fEntityScanner
                                        .peekChar())) {
                                    fMarkupDepth++;
                                    fLastSectionWasCharacterData = false;
                                    setScannerState(
                                    ...
                                    // there can be cdata ahead if coalesce is true
                                ...
                                // in case last section was either entity reference
                                // or cdata or character data -- we should be using
                                // buffer
                                if (fUsebuffer) {
                                    fContentBuffer.append(fTempString);
                                    fTempString.length = 0;
                                ...
                                if (dtdGrammarUtil != null && dtdGrammarUtil
                                ...
                                fUsebuffer = true;
                                if (DEBUG) {
                                    System.out.println("fContentBuffer = "
                                            + fContentBuffer);
                                    System.out.println("fTempString = "
                                            + fTempString);
                                }
                            ...
                            if (c == '\r') {
                                ...
                                // happens when there is the character reference
                                // &#13;
                                // xxx: We know the next chracter.. we should just
                                // skip it and add ']' directlry
                                fEntityScanner.scanChar(null);
                                fUsebuffer = true;
                                ...
                                // skip it and add ']' directlry
                                fUsebuffer = true;
                                fContentBuffer.append((char) fEntityScanner
                                        .scanChar(null));
                                // remember where we are in case we get an endEntity
                                // before we
                                // could flush the buffer out - this happens when
                                // we're parsing an
                                // entity which ends with a ]
                                fInScanContent = true;

                                // We work on a single character basis to handle
                                // cases such as:
                                // ']]]>' which we might otherwise miss.
                                //
                                if (fEntityScanner.skipChar(']', null)) {
                                    ...
                                    if (fEntityScanner.skipChar('>', null)) {
                                        reportFatalError("CDEndInContent",
                                                null);
                                    }
                                }
                                c = -1;
                            ...
                            do {
                                ...
                                // we need not to grow the buffer only when
                                // isCoalesce() is not true;
                                ...
                                if (c == '<') {
                                    ...
                                    setScannerState(
                                            SCANNER_STATE_START_OF_MARKUP);
                                    break;
                                } // xxx what should be the behavior if entity
                                  // reference is present in the content ?
                                else if (c == '&') {
                                    fEntityScanner.scanChar(NameType.REFERENCE);
                                    setScannerState(SCANNER_STATE_REFERENCE);
                                    break;
                                } /// xxx since this part is also characters, it
                                  /// should be merged...
                                ...
                                // we should not be iterating again if fIsCoalesce
                                // is not set to true
                            ...
                            if (DEBUG)
                                System.out.println(
                                        "USING THE BUFFER, STRING START="
                                                ...
                                                + "=END");
                            // if fIsCoalesce is true there might be more data so
                            // call fDriver.next()
                            if (fIsCoalesce) {
                                fLastSectionWasCharacterData = true;
                                continue;
                        ...
                        case SCANNER_STATE_END_ELEMENT_TAG: {
                            if (fEmptyElement) {
                                ...
                                fEmptyElement = false;
                                setScannerState(SCANNER_STATE_CONTENT);
                                // check the case when there is comment after single
                                // element document
                                // <foo/> and some comment after this
                                return (fMarkupDepth == 0
                                ...
                                if (elementDepthIsZeroHook()) {
                                    // if element depth is zero , it indicates the
                                    // end of the document
                                    // the state shouldn't be set, because it is set
                                    // by elementDepthIsZeroHook() function
                        ...
                        case SCANNER_STATE_PI: { // SCANNER_STATE_PI: {
                            ...
                            fContentBuffer.clear();
                            // xxx: which buffer should be passed. Ideally we
                            // shouldn't have
                        ...
                        case SCANNER_STATE_CDATA: { // SCANNER_STATE_CDATA: {
                            ...
                            // we should not clear the buffer only when the last
                            // state was either SCANNER_STATE_REFERENCE or
                            // SCANNER_STATE_CHARACTER_DATA or
                            // SCANNER_STATE_REFERENCE
                            if (fIsCoalesce && (fLastSectionWasEntityReference
                                    || fLastSectionWasCData
                                    || fLastSectionWasCharacterData)) {
                            ...
                            // and just call fDispatche.next(). Since we have set
                            // the scanner state to
                            // SCANNER_STATE_CONTENT (super state) parser will
                            // automatically recover and
                            // behave appropriately. When isCoalesce is set to true
                            // we dont need to reportCDATA event
                            // 2. Check if application has set for reporting CDATA
                            // event
                            // 3. if the application has neither set the fIsCoalesce
                            // to true nor fReportCdataEvent
                        ...
                        case SCANNER_STATE_REFERENCE: {
                            ...
                            // we should not clear the buffer only when the last
                            // state was either CDATA or
                            // SCANNER_STATE_CHARACTER_DATA or
                            // SCANNER_STATE_REFERENCE
                            if (fIsCoalesce && (fLastSectionWasEntityReference
                                    || fLastSectionWasCData
                                    || fLastSectionWasCharacterData)) {
                                // fLastSectionWasEntityReference or
                                // fLastSectionWasCData are only
                            ...
                            if (fEntityScanner.skipChar('#',
                                ...
                                // if there was a text declaration, call next() it
                                // will be taken care.
                                ...
                                if (fScannerState == SCANNER_STATE_REFERENCE) {
                                    setScannerState(SCANNER_STATE_CONTENT);
                                    if (fReplaceEntityReferences && fEntityStore
                                            .isDeclaredEntity(
                                                    fCurrentEntityName)) {
                            ...
                            // Wether it was character reference, entity reference
                            // or built-in entity
                        ...
                        case SCANNER_STATE_TEXT_DECL: {
                            ...
                            if (fEntityScanner.skipString("<?xml")) {
                                ...
                                if (isValidNameChar(fEntityScanner
                                        .peekChar())) {
                                    fStringBuffer.clear();
                                    fStringBuffer.append("xml");

                                    if (fNamespaces) {
                                        while (isValidNCName(fEntityScanner
                                                .peekChar())) {
                                        ...
                                        while (isValidNameChar(fEntityScanner
                                                .peekChar())) {
                                            fStringBuffer.append(
                                                    (char) fEntityScanner
                                                            .scanChar(null));
                            ...
                            // now that we've straightened out the readers, we can
                            // read in chunks:
                            fEntityManager.fCurrentEntity.mayReadChunks = true;
                            setScannerState(SCANNER_STATE_CONTENT);
                            // xxx: we don't return any state, so how do we get to
                            // know about TEXT declarations.
                            // it seems we have to careful when to allow function
                            // issue a callback
                        ...
                        case SCANNER_STATE_ROOT_ELEMENT: {
                            if (scanRootElementHook()) {
                                fEmptyElement = true;
                                // rest would be taken care by fTrailingMiscDriver
                                // set by scanRootElementHook
                                return XMLEvent.START_ELEMENT;
                            }
                            setScannerState(SCANNER_STATE_CONTENT);
                            return XMLEvent.START_ELEMENT;
                        ...
                        default:
                            throw new XNIException("Scanner State "
                                    + fScannerState + " not Recognized ");
        ...
        /**
         * Scan for DOCTYPE hook. This method is a hook for subclasses to add
         * code to handle scanning for a the "DOCTYPE" string after the string
         * "<!" has been scanned.
         *
         * @return True if the "DOCTYPE" was scanned; false if "DOCTYPE" was not
         *         scanned.
         */
        ...
        /**
         * Element depth iz zero. This methos is a hook for subclasses to add
         * code to handle when the element depth hits zero. When scanning a
         * document fragment, an element depth of zero is normal. However, when
         * scanning a full XML document, the scanner must handle the trailing
         * miscellanous section of the document after the end of the document's
         * root element.
         *
         ...
         *         continue as normal.
         */
        protected boolean elementDepthIsZeroHook() throws IOException,
                XNIException {
            return false;
        } // elementDepthIsZeroHook():boolean

        /**
         * Scan for root element hook. This method is a hook for subclasses to
         * add code that handles scanning for the root element. When scanning a
         * document fragment, there is no "root" element. However, when scanning
         * a full XML document, the scanner must handle the root element
         * specially.
         *
         * @return True if the caller should stop and return true which allows
         *         the scanner to switch to a new scanning driver. A return
        ...
        /**
         * End of file hook. This method is a hook for subclasses to add code
         * that handles the end of file. The end of file in a document fragment
         * is OK if the markup depth is zero. However, when scanning a full XML
         * document, an end of file is always premature.
         */
        protected void endOfFileHook(EOFException e) throws IOException,
                XNIException {
    ...
    /**
     * this function gets an XMLString (which is used to store the attribute
     * value) from the special pool maintained for attributes.
     * fAttributeCacheUsedCount tracks the number of attributes that has been
     * consumed from the pool. if all the attributes has been consumed, it adds
     * a new XMLString inthe pool and returns the same XMLString.
     *
    ...
    /**
     * receives callbacks from {@link XMLEntityReader } when buffer is being
     * changed.
     * 
