 ...
 * @version $Id: XMLNSDocumentScannerImpl.java,v 1.11 2010-11-01 04:39:41 joehw
 *          Exp $
 */
public class XMLNSDocumentScannerImpl extends XMLDocumentScannerImpl {

	/**
	 * If is true, the dtd validator is no longer in the pipeline and the
	 * scanner should bind namespaces
	 */
	protected boolean fBindNamespaces;

	/**
	 * If validating parser, make sure we report an error in the scanner if DTD
	 * grammar is missing.
	 */
	protected boolean fPerformValidation;

	/**
	 * Default value of this feature is false, when in Stax mode this should be
	 * true
	 */
	...
	public void reset(PropertyManager propertyManager) {
		...
		fNotAddNSDeclAsAttribute = !((Boolean) propertyManager
				.getProperty(Constants.ADD_NAMESPACE_DECL_AS_ATTRIBUTE)).booleanValue();
	}
	...
	public int next() throws IOException, XNIException {
		// since namespace context should still be valid when the parser is at
		// the end element state therefore
		// we pop the context only when next() has been called after the end
		// element state was encountered. - nb.
	...
	/**
	 * The scanner is responsible for removing DTD validator from the pipeline
	 * if it is not needed.
	 *
	 * @param previous
	...
	protected boolean scanStartElement() throws IOException, XNIException {
		...
		if (!fSkip || fAdd) {
			...
			// StAX uses XMLNSDocumentScannerImpl so this distinction needs to
			// be maintained
			if (fNamespaces) {
		...
		if (fBindNamespaces) {
				...
				if (aprefix != XMLSymbols.EMPTY_STRING) {
					...
					if (uri == null) {
						fErrorReporter.reportError(XMLMessageFormatter.XMLNS_DOMAIN,
								"AttributePrefixUnbound", new Object[] { fElementQName.rawname,
			...
			if (length > 1) {
				...
				if (name != null) {
					if (name.uri != null) {
						fErrorReporter
								.reportError(XMLMessageFormatter.XMLNS_DOMAIN,
	...
	protected void scanAttribute(XMLAttributesImpl attributes) throws IOException, XNIException {
		...
		/**
		 * Determine whether this is a namespace declaration that will be
		 * subject to the name limit check in the scanAttributeValue operation.
		 * Namespace declaration format: xmlns="..." or xmlns:prefix="..." Note
		...
		scanAttributeValue(tmpStr, fTempString2, fAttributeQName.rawname, attributes, attrIndex,
		...
		if (fBindNamespaces) {
			if (isNSDecl) {
				...
				if (uri == XMLSymbols.EMPTY_STRING && localpart != XMLSymbols.PREFIX_XMLNS) {
					fErrorReporter.reportError(XMLMessageFormatter.XMLNS_DOMAIN,
							"EmptyPrefixedAttName", new Object[] { fAttributeQName },
							XMLErrorReporter.SEVERITY_FATAL_ERROR);
				}
				...
				boolean declared = fNamespaceContext.declarePrefix(prefix,
				...
				// xerces internals (XSAttributeChecker) has dependency on
				// namespace declaration returned
				// as part of XMLAttributes.
				// addition of namespace declaration to the attribute list is
				// controlled by fNotAddNSDeclAsAttribute
				// feature. This is required in Stax where namespace
				// declarations are not considered as attribute
	...
	protected final class NSContentDriver extends ContentDriver {
		 ...
		 * add code that handles scanning for the root element. This method will
		 * also attempt to remove DTD validator from the pipeline, if there is
		...
		/**
		 * Re-configures pipeline by removing the DTD validator if no DTD
		 * grammar exists. If no validator exists in the pipeline or there is no
		 * DTD grammar, namespace binding is performed by the scanner in the
		 * enclosing class.
		 */
