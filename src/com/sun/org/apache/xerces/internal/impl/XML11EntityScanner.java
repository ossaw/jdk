...
public class XML11EntityScanner extends XMLEntityScanner {
    ...
    protected int scanChar(NameType nt) throws IOException {
        ...
        if (c == '\n' || ((c == '\r' || c == 0x85 || c == 0x2028) && (external = fCurrentEntity
                .isExternal()))) {
    ...
    /**
     * Returns a string matching the NCName production appearing immediately on
     * the input as a symbol, or null if no NCName string is present.
     * <p>
    ...
    protected String scanNCName() throws IOException {
        ...
        if (XML11Char.isXML11NCNameStart(ch)) {
            ...
            char ch2 = fCurrentEntity.ch[fCurrentEntity.position];
            if (!XMLChar.isLowSurrogate(ch2) || !XML11Char.isXML11NCNameStart(XMLChar.supplemental(ch,
                    ch2))) {
                --fCurrentEntity.position;
                return null;
            }
    ...
    protected boolean scanQName(QName qname, XMLScanner.NameType nt) throws IOException {
        ...
        if (length > 0) {
            ...
            if (index != -1) {
                ...
                if (!XML11Char.isXML11NCNameStart(fCurrentEntity.ch[startLocal]) && (!XML11Char
                        .isXML11NameHighSurrogate(fCurrentEntity.ch[startLocal])
    ...
    protected int scanLiteral(int quote, XMLString content, boolean isNSURI) throws IOException {
        ...
        if (external) {
            while (fCurrentEntity.position < fCurrentEntity.count) {
                c = fCurrentEntity.ch[fCurrentEntity.position++];
                if (c == quote || c == '%' || !XML11Char.isXML11Content(c) || c == 0x85 || c == 0x2028) {
                    fCurrentEntity.position--;
                    break;
                }
            }
            ...
            while (fCurrentEntity.position < fCurrentEntity.count) {
                ...
                // In internal entities control characters are allowed to appear
                // unescaped.
                if ((c == quote && !fCurrentEntity.literal) || c == '%' || !XML11Char
                        .isXML11InternalEntityContent(c)) {
                    fCurrentEntity.position--;
                    break;
                }
            }
        ...
        if (fCurrentEntity.isExternal()) {
            if (XML11Char.isXML11Space(c)) {
                do {
                    ...
                    // If this is a general entity, spaces within a start
    ...
    protected boolean skipString(String s) throws IOException {
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (i < length - 1 && fCurrentEntity.position == fCurrentEntity.count) {
                invokeListeners(0);
                System.arraycopy(fCurrentEntity.ch, fCurrentEntity.count - i - 1, fCurrentEntity.ch, 0, i
                        + 1);
