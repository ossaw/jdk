 ...
 * @author Eric Ye, IBM
 *
 * @version $Id: XML11DocumentScannerImpl.java,v 1.5 2010/08/04 20:59:09 joehw
 *          Exp $
 */
public class XML11DocumentScannerImpl extends XMLDocumentScannerImpl {
	 ...
	 *                           belongs.
	 * @param isNSURI
	 *                           The flag indicating whether the content is a
	 *                           namespace URI
	 *
	...
	protected boolean scanAttributeValue(XMLString value,
		...
		int fromIndex = 0;
		if (c == quote && (fromIndex = isUnchangedByNormalization(
				value)) == -1) {
			/**
			 * Both the non-normalized and normalized attribute values are
			 * equal.
			 **/
		...
		if (c != quote) {
			...
			do {
				...
				if (DEBUG_ATTR_NORMALIZATION) {
					System.out.println("** value2: \"" + fStringBuffer
							.toString() + "\"");
				}
				if (c == '&') {
					...
					if (fEntityScanner.skipChar('#', NameType.REFERENCE)) {
						...
						if (resolveCharacter(entityName, fStringBuffer)) {
							...
							if (fEntityManager.isExternalEntity(entityName)) {
								...
								if (!fEntityManager.isDeclaredEntity(
										entityName)) {
					...
					if (DEBUG_ATTR_NORMALIZATION) {
						System.out.println("** valueF: \"" + fStringBuffer
								.toString() + "\"");
					}
				}
				...
				// this check was originally necessary. - NG
					...
					if (scanSurrogates(fStringBuffer3)) {
						...
						if (DEBUG_ATTR_NORMALIZATION) {
							System.out.println("** valueI: \"" + fStringBuffer
									.toString() + "\"");
						}
					}
	...
	protected boolean scanPubidLiteral(XMLString literal) throws IOException,
		...
		while (true) {
			...
			// REVISIT: none of these except \n and 0x20 should make it past the
			// entity scanner
			if (c == ' ' || c == '\n' || c == '\r' || c == 0x85
					|| c == 0x2028) {
				if (!skipSpace) {
	 ...
	 *
	 * @return -1 if the value would be unchanged by normalization, otherwise
	 *         the index of the first whitespace character which would be
	 *         transformed.
	 */
