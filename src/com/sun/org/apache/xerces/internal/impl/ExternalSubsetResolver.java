...
/**
 * <p>
 * This interface extends <code>XMLEntityResolver</code> providing a method to
 * resolve external subsets for documents which do not explicitly provide one.
 * The application can register an object that implements this interface with
 * the parser configuration. If registered, it will be queried to locate an
 * external subset when none is provided, even for documents that do not contain
 * DOCTYPE declarations. If the registered external subset resolver does not
 * provide an external subset for a given document, it should return
 * <code>null</code>.
 * </p>
 *
 * @xerces.internal
