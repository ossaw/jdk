...
/**
 * DFAContentModel is the implementation of XSCMValidator that does all of the
 * non-trivial element content validation. This class does the conversion from
 * the regular expression to the DFA that it then uses in its validation
 * algorithm.
 *
 * @xerces.internal
 *
...
public class XSDFACM implements XSCMValidator {
	...
	/**
	 * This is the map of unique input symbol elements to indices into each
	 * state's per-input symbol transition table entry. This is part of the
	 * built DFA information that must be kept around to do the actual
	 * validation. Note tat since either XSElementDecl or XSParticleDecl object
	 * can live here, we've got to use an Object.
	 */
	private Object fElemMap[] = null;

	/**
	 * This is a map of whether the element map contains information related to
	 * ANY models.
	 */
	...
	/**
	 * This is an array of booleans, one per state (there are fTransTableSize
	 * states in the DFA) that indicates whether that state is a final state.
	 */
	private boolean fFinalStateFlags[] = null;

	/**
	 * The list of follow positions for each NFA position (i.e. for each
	 * non-epsilon leaf node.) This is only used during the building of the DFA,
	 * and is let go afterwards.
	 */
	private CMStateSet fFollowList[] = null;

	/**
	 * This is the head node of our intermediate representation. It is only
	 * non-null during the building of the DFA (just so that it does not have to
	 * be passed all around.) Once the DFA is built, this is no longer required
	 * so its nulled out.
	 */
	private CMNode fHeadNode = null;

	/**
	 * The count of leaf nodes. This is an important number that set some limits
	 * on the sizes of data structures in the DFA process.
	 */
	private int fLeafCount = 0;

	/**
	 * An array of non-epsilon leaf nodes, which is used during the DFA build
	 * operation, then dropped.
	 */
	...
	/**
	 * This is the transition table that is the main by product of all of the
	 * effort here. It is an array of arrays of ints. The first dimension is the
	 * number of states we end up with in the DFA. The second dimensions is the
	 * number of unique elements in the content model (fElemMapSize). Each entry
	 * in the second dimension indicates the new state given that input for the
	 * first dimension's start state.
	 * <p>
	 * The fElemMap array handles mapping from element indexes to positions in
	 * the second dimension of the transition table.
	 */
	private int fTransTable[][] = null;
	/**
	 * Array containing occurence information for looping states which use
	 * counters to check minOccurs/maxOccurs.
	 */
	...
	/**
	 * Array of counters for all the for elements (or wildcards) of the form
	 * a{n,m} where n > 1 and m <= unbounded. Used to count the a's to later
	 * check against n and m. Counter set to -1 if element (or wildcard) not
	 * optimized by constant space algorithm.
	 */
	private int fElemMapCounter[];

	/**
	 * Array of lower bounds for all the for elements (or wildcards) of the form
	 * a{n,m} where n > 1 and m <= unbounded. This array stores the n's for
	 * those elements (or wildcards) for which the constant space algorithm
	 * applies (or -1 otherwise).
	 */
	private int fElemMapCounterLowerBound[];

	/**
	 * Array of upper bounds for all the for elements (or wildcards) of the form
	 * a{n,m} where n > 1 and m <= unbounded. This array stores the n's for
	 * those elements (or wildcards) for which the constant space algorithm
	 * applies, or -1 if algorithm does not apply or m = unbounded.
	 */
	...
	public Object oneTransition(QName curElem, int[] state,
		...
		for (; elemIndex < fElemMapSize; elemIndex++) {
			...
			if (type == XSParticleDecl.PARTICLE_ELEMENT) {
				matchingDecl = subGroupHandler.getMatchingElemDecl(curElem,
						(XSElementDecl) fElemMap[elemIndex]);
			...
			if (o != null) {
				if (curState == nextState) {
					if (++state[2] > o.maxOccurs
						...
						//
						// <xs:sequence>
						// <xs:element name="foo" type="xs:string" minOccurs="3"
						// maxOccurs="3"/>
	...
	Object findMatchingDecl(QName curElem, int[] state, SubstitutionGroupHandler subGroupHandler,
		...
		// if we still can't find a match, set the state to FIRST_ERROR and
		// return null
		if (elemIndex == fElemMapSize) {
			state[1] = state[0];
			state[0] = XSCMValidator.FIRST_ERROR;
		...
		if (fFinalStateFlags[curState]) {
			if (fCountingStates != null) {
				...
				if (o != null && state[2] < o.minOccurs) {
					// not enough loops on the current state to be considered
					// final.
					return false;
				}
			}
	...
	private void buildDFA(CMNode syntaxTree) {
		 ...
		 * MODIFIED (Jan, 2001)
		 *
		 * Use following rules. nullable(x+) := nullable(x), first(x+) :=
		 * first(x), last(x+) := last(x) nullable(x?) := true, first(x?) :=
		...
		for (int outIndex = 0; outIndex < fLeafCount; outIndex++) {
			...
			if (inIndex == fElemMapSize) {
				...
				if (leaf instanceof XSCMRepeatingLeaf) {
					...
					elemOccurenceMap[fElemMapSize] = new Occurence((XSCMRepeatingLeaf) leaf,
							fElemMapSize);
				}
		...
		/***
		 * Optimization(Jan, 2001); We sort fLeafList according to elemIndex
		 * which is *uniquely* associated to each leaf. We are *assuming* that
		 * each element appears in at least one leaf.
		 **/
		...
		/*
		 * Optimization(Jan, 2001); This is faster for a large content model
		 * such as, "(t001+|t002+|.... |t500+)".
		 */
		...
		while (unmarkedState < curState) {
			...
			for (int elemIndex = 0; elemIndex < fElemMapSize; elemIndex++) {
				...
				while (leafIndex != -1) {
					// If this leaf index (DFA position) is in the current
					// set...
	...
	private void dumpTree(CMNode nodeCur, int level) {
		...
		case XSParticleDecl.PARTICLE_ELEMENT: {
			System.out.print("Leaf: (pos=" + ((XSCMLeaf) nodeCur).getPosition() + "), "
					+ "(elemIndex=" + ((XSCMLeaf) nodeCur).getLeaf() + ") ");
				...
				for (int k = j + 1; k < fElemMapSize; k++) {
					if (fTransTable[i][j] != -1 && fTransTable[i][k] != -1) {
						if (conflictTable[j][k] == 0) {
							if (XSConstraints.overlapUPA(fElemMap[j], fElemMap[k],
									subGroupHandler)) {
								if (fCountingStates != null) {
		...
		for (int i = 0; i < fElemMapSize; i++) {
			for (int j = 0; j < fElemMapSize; j++) {
				if (conflictTable[i][j] == 1) {
					// errors.newError("cos-nonambig", new
					// Object[]{fElemMap[i].toString(),
					// fElemMap[j].toString()});
					// REVISIT: do we want to report all errors? or just one?
					throw new XMLSchemaException("cos-nonambig",
	 ...
	 * works if the state is in error, in which case it returns what should have
	 * been seen.
	 *
	 * @param state
	 *            the current state
	 ...
	 * Called by a validator if validation of countent model succeeds after
	 * subsuming a{n,m} to a* (or a+) to check the n and m bounds. Returns
	 * <code>null</code> if validation of bounds is successful. Returns a list
	 * of strings with error info if not. Even entries in list returned are
	 * error codes (used to look up properties) and odd entries are parameters
	 * to be passed when formatting error message. Each parameter is associated
	 * with the error code that preceeds it in the list.
	 */
