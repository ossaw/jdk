 ...
 * contained. Any simple type (atomic, list or union) will implement this
 * interface. It inherits from <code>XSTypeDecl</code>.
 *
 * @xerces.internal
...
public interface XSSimpleType extends XSSimpleTypeDefinition {
    ...
    /**
     * constants defined for the values of 'whitespace' facet. see
     * <a href='http://www.w3.org/TR/xmlschema-2/#dt-whiteSpace'> XML Schema
     * Part 2: Datatypes </a>
     */
    ...
    /**
     * Constant defined for the primitive built-in simple tpyes. see
    ...
    /**
     * return an ID representing the built-in primitive base type. REVISIT: This
     * method is (currently) for internal use only. the constants returned from
     * this method are not finalized yet. the names and values might change in
     * the further.
     *
     ...
     * that if content is a StringBuffer, for performance reasons, it's possible
     ...
     * simple types. REVISIT: Andy believes that a compare() method is
     * necessary. I don't see the necessity for schema (the only place where we
     * need to compare two values is to check min/maxIn/Exclusive facets, but we
     * only need a private method for this case.) But Andy thinks XPATH
    ...
    /**
     * Check whether this type is or is derived from ID. REVISIT: this method
     * makes ID special, which is not a good design. but since ID is not a
     * primitive, there doesn't seem to be a clean way of doing it except to
     * define special method like this.
     *
