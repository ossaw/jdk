...
/**
 * This class scans the version of the document to determine which scanner to
...
public class XMLVersionDetector {
	...
	private final char[] fExpectedVersionString = { '<', '?', 'x', 'm', 'l', ' ', 'v', 'e', 'r',
			's', 'i', 'o', 'n', '=', ' ', ' ', ' ', ' ', ' ' };
	...
	public void startDocumentParsing(XMLEntityHandler scanner, short version) {
		...
		// Make sure the locator used by the error reporter is the current
		// entity scanner.
		fErrorReporter.setDocumentLocator(fEntityManager.getEntityScanner());

		// Note: above we reset fEntityScanner in the entity manager, thus in
		// startEntity
		// in each scanner fEntityScanner field must be reset to reflect the
		// change.
		//
	...
	/**
	 * This methods scans the XML declaration to find out the version (and
	 * provisional encoding) of the document. The scanning is doing using XML
	 * 1.1 scanner.
	 * 
	 * @param inputSource
