...
/**
 * The DTD validator. The validator implements a document filter: receiving
 * document events from the scanner; validating the content and structure;
 * augmenting the InfoSet, if applicable; and notifying the parser of the
 * information resulting from the validation process.
 * <p>
 * Formerly, this component also handled DTD events and grammar construction. To
 * facilitate the development of a meaningful DTD grammar caching/preparsing
 * framework, this functionality has been moved into the XMLDTDLoader class.
 * Therefore, this class no longer implements the DTDFilter or
 ...
 */
public class XMLDTDValidator implements XMLComponent, XMLDocumentFilter,
		XMLDTDValidatorFilter, RevalidationHandler {
	...
	protected static final String BALANCE_SYNTAX_TREES = Constants.XERCES_FEATURE_PREFIX
	...
	private static final String[] RECOGNIZED_FEATURES = { NAMESPACES,
			VALIDATION, DYNAMIC_VALIDATION, BALANCE_SYNTAX_TREES };
	...
	private static final String[] RECOGNIZED_PROPERTIES = { SYMBOL_TABLE,
			ERROR_REPORTER, GRAMMAR_POOL, DATATYPE_VALIDATOR_FACTORY,
			VALIDATION_MANAGER };
	...
	/**
	 * Dynamic validation. This state of this feature is only useful when the
	 * validation feature is set to <code>true</code>.
	 */
	...
	/**
	 * Element children. This data structure is a growing stack that holds the
	 * children of elements from the root to the current element depth. This
	 * structure never gets "deeper" than the deepest element. Space is re-used
	 * once each element is closed.
	 * <p>
	 * <strong>Note:</strong> This is much more efficient use of memory than
	 * creating new arrays for each element depth.
	 * <p>
	 * <strong>Note:</strong> The use of this data structure is for validation
	 * "on the way out". If the validation model changes to "on the way in",
	 * then this data structure is not needed.
	 */
	...
	/**
	 * Element children offset stack. This stack refers to offsets into the
	 * <code>fElementChildren</code> array.
	 * 
	...
	public void reset(XMLComponentManager componentManager)
		...
		fDTDValidation = !(componentManager.getFeature(
		...
		fBalanceSyntaxTrees = componentManager.getFeature(BALANCE_SYNTAX_TREES,
				false);
		fWarnDuplicateAttdef = componentManager.getFeature(
				WARN_ON_DUPLICATE_ATTDEF, false);
		...
		fDatatypeValidatorFactory = (DTDDVFactory) componentManager.getProperty(
				Constants.XERCES_PROPERTY_PREFIX
						+ Constants.DATATYPE_VALIDATOR_FACTORY_PROPERTY);
		init();
	...
	public void startDocument(XMLLocator locator, String encoding,
		...
		if (fGrammarPool != null) {
			Grammar[] grammars = fGrammarPool.retrieveInitialGrammarSet(
	...
	public void xmlDecl(String version, String encoding, String standalone,
		...
		fGrammarBucket.setStandalone(standalone != null && standalone.equals(
				"yes"));
	...
	public void doctypeDecl(String rootElement, String publicId,
		...
		String eid = null;
		try {
			eid = XMLEntityManager.expandSystemId(systemId, fDocLocation
					.getExpandedSystemId(), false);
		} catch (java.io.IOException e) {
		}
		XMLDTDDescription grammarDesc = new XMLDTDDescription(publicId,
				systemId, fDocLocation.getExpandedSystemId(), eid, rootElement);
		...
		if (fDTDGrammar == null) {
			...
			// was provided.
			// Since so many different DTDs have roots in common, using only a
			// root name as the
			// key may cause an unexpected grammar to be retrieved from the
			// grammar pool. This scenario
			// would occur when an ExternalSubsetResolver has been queried and
			// the
			...
			// DOCTYPE. -- mrglavas
			if (fGrammarPool != null && (systemId != null
					|| publicId != null)) {
				fDTDGrammar = (DTDGrammar) fGrammarPool.retrieveGrammar(
						grammarDesc);
			}
		}
		if (fDTDGrammar == null) {
	...
	public void characters(XMLString text, Augmentations augs)
		...
		if (fPerformValidation) {
			if (fInElementContent) {
				if (fGrammarBucket.getStandalone() && fDTDGrammar
				...
				// For E15.2
				if (augs != null && augs.getItem(
						Constants.CHAR_REF_PROBABLE_WS) == Boolean.TRUE) {
					fErrorReporter.reportError(XMLMessageFormatter.XML_DOMAIN,
							"MSG_CONTENT_INVALID_SPECIFIED", new Object[] {
									fCurrentElement.rawname, fDTDGrammar
	 ...
	 * this method will be called immediately following the startParameterEntity
	 * call.
	 * <p>
	...
	public final boolean validate() {
		...
		return (fSchemaType != Constants.NS_XMLSCHEMA) && (!fDynamicValidation
				&& fValidation || fDynamicValidation && fSeenDoctypeDecl)
				&& (fDTDValidation || fSeenDoctypeDecl);
	}

	// REVISIT:we can convert into functions.. adding default attribute values..
	// and one validating.

	/** Add default attributes and validate. */
	protected void addDTDDefaultAttrsAndValidate(QName elementName,
			int elementIndex, XMLAttributes attributes) throws XNIException {
			...
			if (!specified) {
				if (required) {
					...
					if (fPerformValidation && fGrammarBucket.getStandalone()) {
						if (fDTDGrammar.getAttributeDeclIsExternal(
								attlistIndex)) {
		...
		for (int i = 0; i < attrCount; i++) {
			...
			if (fPerformValidation) {
				if (fGrammarBucket.getStandalone()) {
					...
					if (nonNormalizedValue != null) {
						String entityName = getExternalEntityRefInAttrValue(
								nonNormalizedValue);
						if (entityName != null) {
							fErrorReporter.reportError(
									XMLMessageFormatter.XML_DOMAIN,
			...
			if (!declared) {
				if (fPerformValidation) {
					...
					fErrorReporter.reportError(XMLMessageFormatter.XML_DOMAIN,
							"MSG_ATTRIBUTE_NOT_DECLARED", args,
							XMLErrorReporter.SEVERITY_ERROR);
				}
				continue;
			}
			...
			if (attributes.isSpecified(i) && type != XMLSymbols.fCDATASymbol) {
				...
				if (fPerformValidation && fGrammarBucket.getStandalone()
						&& changedByNormalization && fDTDGrammar
			...
			if (fTempAttDecl.simpleType.defaultType == XMLSimpleType.DEFAULT_TYPE_FIXED) {
				...
				if (!attrValue.equals(defaultValue)) {
					Object[] args = { elementName.localpart, attrRawName,
							attrValue, defaultValue };
					fErrorReporter.reportError(XMLMessageFormatter.XML_DOMAIN,
							"MSG_FIXED_ATTVALUE_INVALID", args,
							XMLErrorReporter.SEVERITY_ERROR);
				}
			}
	...
	protected String getExternalEntityRefInAttrValue(
			String nonNormalizedValue) {
		int valLength = nonNormalizedValue.length();
		int ampIndex = nonNormalizedValue.indexOf('&');
		while (ampIndex != -1) {
			if (ampIndex + 1 < valLength && nonNormalizedValue.charAt(ampIndex
					+ 1) != '#') {
				int semicolonIndex = nonNormalizedValue.indexOf(';', ampIndex
						+ 1);
				String entityName = nonNormalizedValue.substring(ampIndex + 1,
						semicolonIndex);
				...
				if (entIndex > -1) {
					...
					if (fEntityDecl.inExternal
							|| (entityName = getExternalEntityRefInAttrValue(
									fEntityDecl.value)) != null) {
						return entityName;
					}
				}
			}
	...
	protected void validateDTDattribute(QName element, String attValue,
		...
		switch (attributeDecl.simpleType.type) {
			...
			case XMLSimpleType.TYPE_ENUMERATION: {
				...
				if (enumVals == null) {
					...
					for (int i = 0; i < enumVals.length; i++) {
						if (attValue == enumVals[i] || attValue.equals(
								enumVals[i])) {
							found = true;
							break;
						}
				...
				if (!found) {
					...
					fErrorReporter.reportError(XMLMessageFormatter.XML_DOMAIN,
							"MSG_ATTRIBUTE_VALUE_NOT_IN_LIST", new Object[] {
									attributeDecl.name.rawname, attValue,
									enumValueString },
							XMLErrorReporter.SEVERITY_ERROR);
				}
				break;
			...
			case XMLSimpleType.TYPE_IDREF: {
				boolean isAlistAttribute = attributeDecl.simpleType.list;// Caveat -
																			// Save
																			// this
																			// information
																			// because
				...
				try {
					...
					if (isAlistAttribute) {
						fErrorReporter.reportError(
								XMLMessageFormatter.XML_DOMAIN, "IDREFSInvalid",
								new Object[] { attValue },
								XMLErrorReporter.SEVERITY_ERROR);
					} else {
						fErrorReporter.reportError(
								XMLMessageFormatter.XML_DOMAIN, ex.getKey(), ex
										.getArgs(),
								XMLErrorReporter.SEVERITY_ERROR);
					}
			...
			case XMLSimpleType.TYPE_NMTOKEN: {
				...
				try {
					...
					if (isAlistAttribute) {
						fErrorReporter.reportError(
								XMLMessageFormatter.XML_DOMAIN,
								"NMTOKENSInvalid", new Object[] { attValue },
								XMLErrorReporter.SEVERITY_ERROR);
					} else {
						fErrorReporter.reportError(
								XMLMessageFormatter.XML_DOMAIN,
								"NMTOKENInvalid", new Object[] { attValue },
								XMLErrorReporter.SEVERITY_ERROR);
					}
				}
				break;
	...
	protected boolean invalidStandaloneAttDef(QName element, QName attribute) {
		...
		boolean state = true;
		/*
		 * if (fStandaloneReader == -1) { return false; } // we are normalizing
		 * a default att value... this ok? if (element.rawname == -1) { return
	 ...
	 * @param index
	 *                   The index of the attribute to normalize
	 */
	private boolean normalizeAttrValue(XMLAttributes attributes, int index) {
			...
			if (attValue[i] == ' ') {
				...
				if (spaceStart && !leadingSpace) {
					...
					if (leadingSpace || !spaceStart) {
						eaten++;
						/***
						 * BUG #3512 *** int entityCount =
						 * attributes.getEntityCount(index); for (int j = 0; j <
						 * entityCount; j++) { int offset =
						 * attributes.getEntityOffset(index, j); int length =
						 * attributes.getEntityLength(index, j); if (offset <=
						 * i-eaten+1) { if (offset+length >= i-eaten+1) { if
						 * (length > 0) length--; } } else { if (offset > 0)
						 * offset--; } attributes.setEntityOffset(index, j,
						 * offset); attributes.setEntityLength(index, j,
						 * length); } /
						 ***/
					}
				}
		...
		if (count > 0 && fBuffer.charAt(count - 1) == ' ') {
			 ...
			 * for (int j=0; j < entityCount; j++) { int offset =
			 * attributes.getEntityOffset(index, j); int length =
			 * attributes.getEntityLength(index, j); if (offset < count-1) { if
			 * (offset+length == count) { length--; } } else { offset--; }
	...
	private final void rootElementSpecified(QName rootElement)
			throws XNIException {
		if (fPerformValidation) {
			...
			if (root1 == null || !root1.equals(root2)) {
				fErrorReporter.reportError(XMLMessageFormatter.XML_DOMAIN,
						"RootElementTypeMustMatchDoctypedecl", new Object[] {
								root1, root2 },
						XMLErrorReporter.SEVERITY_ERROR);
			}
		}
	 ...
	 * <p>
	 * This is the method of primary concern to the validator. This method is
	 * called upon the scanner reaching the end tag of an element. At that time,
	 * the element's children must be structurally validated, so it calls this
	 * method. The index of the element being checked (in the decl pool), is
	 * provided as well as an array of element name indexes of the children. The
	 * validator must confirm that this element can have these children in this
	 * order.
	 * <p>
	 * This can also be called to do 'what if' testing of content models just to
	 * see if they would be valid.
	 * <p>
	 * Note that the element index is an index into the element decl pool,
	 * whereas the children indexes are name indexes, i.e. into the string pool.
	 * <p>
	 ...
	 *
	 * @param elementIndex
	 *                     The index within the <code>ElementDeclPool</code> of
	 *                     this
	 *                     element.
	 * @param childCount
	 *                     The number of entries in the <code>children</code>
	 *                     array.
	 * @param children
	 ...
	 *                      Thrown on error.
	 */
	private int checkContent(int elementIndex, QName[] children,
			int childOffset, int childCount) throws XNIException {
		...
		if (contentType == XMLElementDecl.TYPE_EMPTY) {
			 ...
			 * XMLMessages.VC_ELEMENT_VALID, elementType); /
			 ****/
			 ...
			 * ImplementationMessages.VAL_CST, 0, null,
	...
	protected void init() {
		...
		if (fValidation || fDynamicValidation) {
			try {
				...
				fValID = fDatatypeValidatorFactory.getBuiltInDV(
						XMLSymbols.fIDSymbol);
				...
				fValENTITY = fDatatypeValidatorFactory.getBuiltInDV(
						XMLSymbols.fENTITYSymbol);
				fValENTITIES = fDatatypeValidatorFactory.getBuiltInDV(
				...
				fValNOTATION = fDatatypeValidatorFactory.getBuiltInDV(
						XMLSymbols.fNOTATIONSymbol);
	...
	private void ensureStackCapacity(int newElementDepth) {
		if (newElementDepth == fElementQNamePartsStack.length) {
			...
			System.arraycopy(this.fElementQNamePartsStack, 0, newStackOfQueue,
					0, newElementDepth);
	...
	protected boolean handleStartElement(QName element,
		...
		if (fDTDGrammar == null) {
			...
			if (fPerformValidation) {
				fErrorReporter.reportError(XMLMessageFormatter.XML_DOMAIN,
						"MSG_GRAMMAR_NOT_FOUND", new Object[] {
								element.rawname },
						XMLErrorReporter.SEVERITY_ERROR);
			}
			// modify pipeline
			...
			if (fCurrentContentSpecType == -1 && fPerformValidation) {
				fErrorReporter.reportError(XMLMessageFormatter.XML_DOMAIN,
						"MSG_ELEMENT_NOT_DECLARED", new Object[] {
								element.rawname },
						XMLErrorReporter.SEVERITY_ERROR);
			}
	...
	protected void startNamespaceScope(QName element, XMLAttributes attributes,
			Augmentations augs) {}
	...
	protected void handleEndElement(QName element, Augmentations augs,
		...
		if (fPerformValidation) {
			...
			if (elementIndex != -1 && fCurrentContentSpecType != -1) {
				...
				int result = checkContent(elementIndex, children,
						childrenOffset, childrenLength);
				...
				if (result != -1) {
					...
					if (fTempElementDecl.type == XMLElementDecl.TYPE_EMPTY) {
						fErrorReporter.reportError(
								XMLMessageFormatter.XML_DOMAIN,
								"MSG_CONTENT_INVALID", new Object[] {
						...
						fErrorReporter.reportError(
								XMLMessageFormatter.XML_DOMAIN, messageKey,
								new Object[] { element.rawname, fDTDGrammar
										.getContentSpecAsString(elementIndex) },
								XMLErrorReporter.SEVERITY_ERROR);
					}
				}
			}
			fElementChildrenLength = fElementChildrenOffsetStack[fElementDepth
					+ 1] + 1;
		}
		...
		if (fElementDepth < 0) {
			...
			if (fPerformValidation) {
				...
				if (value != null) {
					fErrorReporter.reportError(XMLMessageFormatter.XML_DOMAIN,
							"MSG_ELEMENT_WITH_ID_REQUIRED", new Object[] {
									value }, XMLErrorReporter.SEVERITY_ERROR);
				}
			}
			return;
		}
