...
/**
 * This class is a common element of all parser configurations and is used to
 * report errors that occur. This component can be queried by parser components
 * from the component manager using the following property ID:
 * 
 * <pre>
 *   http://apache.org/xml/properties/internal/error-reporter
 * </pre>
 * <p>
 * Errors are separated into domains that categorize a class of errors. In a
 * parser configuration, the parser would register a
 * <code>MessageFormatter</code> for each domain that is capable of localizing
 * error messages and formatting them based on information about the error. Any
 * parser component can invent new error domains and register additional message
 * formatters to localize messages in those domains.
 * <p>
 ...
 * </ul>
 * <p>
 * This component can use the following features and properties but they are not
 * required:
 * <ul>
...
public class XMLErrorReporter implements XMLComponent {
	...
	/**
	 * Severity: warning. Warnings represent informational messages only that
	 * should not be considered serious enough to stop parsing or indicate an
	 * error in the document's validity.
	 */
	public static final short SEVERITY_WARNING = 0;

	/**
	 * Severity: error. Common causes of errors are document structure and/or
	 * content that that does not conform to the grammar rules specified for the
	 * document. These are typically validation errors.
	 */
	public static final short SEVERITY_ERROR = 1;

	/**
	 * Severity: fatal error. Fatal errors are errors in the syntax of the XML
	 * document or invalid byte sequences for a given encoding. The XML 1.0
	 * Specification mandates that errors of this type are not recoverable.
	 * <p>
	...
	/**
	 * Default error handler. This error handler is only used in the absence of
	 * a registered error handler so that errors are not "swallowed" silently.
	 * This is one of the most common "problems" reported by users of the
	 * parser.
	 */
	 ...
	 * <p>
	 * <strong>Note:</strong> Registering a message formatter for a domain when
	 * there is already a formatter registered will cause the previous formatter
	 * to be lost. This method replaces any previously registered message
	 * formatter for the specified domain.
	 *
	 * @param domain
	...
	/**
	 * Returns the message formatter associated with the specified domain, or
	...
	/**
	 * Removes the message formatter for the specified domain and returns the
	...
	/**
	 * Reports an error. The error message passed to the error handler is
	 * formatted for the locale by the message formatter installed for the
