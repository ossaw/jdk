 ...
 *
 * @version $Id: SubstitutionGroupHandler.java,v 1.6 2010-11-01 04:39:55 joehw
 *          Exp $
 */
public class SubstitutionGroupHandler {
	...
	protected boolean substitutionGroupOK(XSElementDecl element,
		...
		// value of a {disallowed substitutions}) one of the following must be
		// true:
		...
		// {substitution group affiliation}'s {substitution group affiliation}
		// is C, or . . .
		...
		// D's {type definition} from C's {type definition} does not intersect
		// with the union of the blocking constraint, C's {prohibited
		// substitutions} (if C is complex, otherwise the empty set) and the
		// {prohibited substitutions} (respectively the empty set) of any
		// intermediate {type definition}s in the derivation of D's {type
	...
	private boolean typeDerivationOK(XSTypeDefinition derived,
		...
		if (type != base) {
			// If the base is a union, check if "derived" is allowed through any
			// of the member types.
	...
	public boolean inSubstitutionGroup(XSElementDecl element,
		...
		// 2 PSG is closed with respect to {substitution group affiliation},
		// that is, if any element declaration in the {element declarations} has
		// a {substitution group affiliation} in PSG, then it is also in PSG
		// itself.
	...
	/**
	 * get all elements that can substitute the given element, according to the
	 * spec, we shouldn't consider the {block} constraints.
	 *
