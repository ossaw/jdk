...
/**
 * The XML representation for a complexType schema component is a
...
public class XSComplexTypeDecl implements XSComplexTypeDefinition, TypeInfo {
	...
	public void setValues(String name, String targetNamespace, XSTypeDefinition baseType,
			short derivedBy, short schemaFinal, short block, short contentType, boolean isAbstract,
	...
	void appendTypeInfo(StringBuilder str) {
		...
		str.append("Complex type name='").append(fTargetNamespace).append(',').append(getTypeName())
				.append("', ");
		if (fBaseType != null) {
		...
		if ((((derivationMethod & DERIVATION_LIST) != 0)
			...
			if (!(fName.equals(SchemaSymbols.ATTVAL_ANYTYPE)
				...
				if (fBaseType != null && fBaseType instanceof XSSimpleTypeDecl) {
					...
					return ((XSComplexTypeDecl) fBaseType).isDOMDerivedFrom(ancestorNS,
	...
	private boolean isDerivedByRestriction(String ancestorNS, String ancestorName,
		...
		while (type != null && type != oldType) {
			...
			if (type instanceof XSSimpleTypeDecl) {
				...
				return ((XSSimpleTypeDecl) type).isDOMDerivedFrom(ancestorNS, ancestorName,
						derivationMethod);
			} else {
	...
	private boolean isDerivedByExtension(String ancestorNS, String ancestorName,
		...
		while (type != null && type != oldType) {
			...
			if (type instanceof XSSimpleTypeDecl) {
				...
				if ((derivationMethod & DERIVATION_EXTENSION) != 0) {
					return extension & ((XSSimpleTypeDecl) type).isDOMDerivedFrom(ancestorNS,
							ancestorName, (derivationMethod & DERIVATION_RESTRICTION));
				} else {
					return extension & ((XSSimpleTypeDecl) type).isDOMDerivedFrom(ancestorNS,
							ancestorName, derivationMethod);
				}
	...
	/**
	 * A boolean that specifies if the type definition is anonymous. Convenience
	 * attribute. This is a field is not part of XML Schema component model.
	 */
	...
	/**
	 * {base type definition} Either a simple type definition or a complex type
	 * definition.
	 */
	 ...
	 * restriction}. For simple type definition it is a subset of {extension,
	 ...
	 * restriction}.
	 *
	 * @return A bit flag that represents: {extension, restriction) or none for
	 *         complexTypes; {extension, list, restriction, union} or none for
	 *         simpleTypes;
	 */
	 ...
	 * be used as the {type definition} for the validation of element
	 * information items.
	 */
	...
	/**
	 * A simple type definition corresponding to simple content model, otherwise
	 * <code>null</code>
	 */
	 ...
	 * 
	 * @see
	 * org.apache.xerces.xs.XSComplexTypeDefinition#getAttributeUse(java.lang.
