...
public class XSAnnotationImpl implements XSAnnotation {
     ...
     * declarations for <code>annotation</code> element are added as attributes
     * nodes of the serialized <code>annotation</code>, otherwise the
     * corresponding events for all in-scope namespace declaration are sent via
     ...
     * of the component, if it's globally declared; or null otherwise.
     */
    ...
    private synchronized void writeToDOM(Node target, short type) {
        ...
        if (futureOwner instanceof CoreDocumentImpl) {
            ...
            // adoptNode will return null when the DOM implementations are not
            // compatible.
            if (newElem == null) {
