...
/**
 * The element declaration schema component traverser. <element abstract =
 * boolean : false block = (#all | List of (extension | restriction |
 * substitution)) default = string final = (#all | List of (extension |
 * restriction)) fixed = string form = (qualified | unqualified) id = ID
 * maxOccurs = (nonNegativeInteger | unbounded) : 1 minOccurs =
 * nonNegativeInteger : 1 name = NCName nillable = boolean : false ref = QName
 * substitutionGroup = QName type = QName {any attributes with non-schema
 * namespace . . .}> Content: (annotation?, ((simpleType | complexType)?,
...
class XSDElementTraverser extends XSDAbstractTraverser {
     ...
     * To handle the recursive cases efficiently, we delay the traversal and
     * return an empty particle node. We'll fill in this particle node later
     * after we've done with all the global declarations. This method causes a
     * number of data structures in the schema handler to be filled in.
     *
     * @param elmDecl
     * @param schemaDoc
     * @param grammar
     * @return the particle
     */
    XSParticleDecl traverseLocal(Element elmDecl, XSDocumentInfo schemaDoc, SchemaGrammar grammar,
            int allContextFlags, XSObject parent) {
    ...
    /**
     * Traverse a locally declared element (or an element reference).
     * This is the real traversal method. It's called after we've done with all
     * the global declarations.
     *
     * @param index
     */
    protected void traverseLocal(XSParticleDecl particle, Element elmDecl, XSDocumentInfo schemaDoc,
            SchemaGrammar grammar, int allContextFlags, XSObject parent, String[] localNSDecls) {
        ...
        if (elmDecl.getAttributeNode(SchemaSymbols.ATT_REF) != null) {
            if (refAtt != null) {
                element = (XSElementDecl) fSchemaHandler.getGlobalDecl(schemaDoc, XSDHandler.ELEMENT_TYPE,
                        refAtt, elmDecl);
                ...
                // 2 If the item's parent is not <schema>, then all of the
                // following must be true:
                // 2.1 One of ref or name must be present, but not both.
                // 2.2 If ref is present, then all of <complexType>,
                // <simpleType>, <key>, <keyref>, <unique>, nillable, default,
                // fixed, form, block and type must be absent, i.e. only
                // minOccurs, maxOccurs, id are allowed in addition to ref,
                // along with <annotation>.
                if (child != null) {
                    reportSchemaError("src-element.2.2", new Object[] { refAtt.rawname, DOMUtil.getLocalName(
                            child) }, child);
                }
            } else {
                element = null;
        ...
        checkOccurrences(particle, SchemaSymbols.ELT_ELEMENT, (Element) elmDecl.getParentNode(),
                allContextFlags, defaultVals.longValue());
     ...
     * @return the element declaration
     */
    XSElementDecl traverseNamedElement(Element elmDecl, Object[] attrValues, XSDocumentInfo schemaDoc,
            SchemaGrammar grammar, boolean isGlobal, XSObject parent) {
        ...
        if (blockAtt == null) {
            ...
            if (element.fBlock != XSConstants.DERIVATION_ALL) {
                element.fBlock &= (XSConstants.DERIVATION_EXTENSION | XSConstants.DERIVATION_RESTRICTION
            ...
            if ((element.fBlock != XSConstants.DERIVATION_ALL) && ((element.fBlock
                    | XSConstants.DERIVATION_EXTENSION_RESTRICTION_SUBSTITION) != XSConstants.DERIVATION_EXTENSION_RESTRICTION_SUBSTITION)) {
                reportSchemaError("s4s-att-invalid-value", new Object[] { element.fName, "block",
                        "must be (#all | List of (extension | restriction | substitution))" }, elmDecl);
            }
        }
        ...
        if (subGroupAtt != null) {
            element.fSubGroup = (XSElementDecl) fSchemaHandler.getGlobalDecl(schemaDoc,
                    XSDHandler.ELEMENT_TYPE, subGroupAtt, elmDecl);
        }
        ...
        if (elementType == null && typeAtt != null) {
            elementType = (XSTypeDefinition) fSchemaHandler.getGlobalDecl(schemaDoc, XSDHandler.TYPEDECL_TYPE,
                    typeAtt, elmDecl);
            if (elementType == null) {
        ...
        if (child != null) {
            String childName = DOMUtil.getLocalName(child);
            while (child != null && (childName.equals(SchemaSymbols.ELT_KEY) || childName.equals(
                    SchemaSymbols.ELT_KEYREF) || childName.equals(SchemaSymbols.ELT_UNIQUE))) {

                if (childName.equals(SchemaSymbols.ELT_KEY) || childName.equals(SchemaSymbols.ELT_UNIQUE)) {
                    // need to set <key>/<unique> to hidden before traversing
                    // it,
                    ...
                    if (DOMUtil.getAttrValue(child, SchemaSymbols.ATT_NAME).length() != 0) {
                        fSchemaHandler.checkForDuplicateNames((schemaDoc.fTargetNamespace == null) ? ","
                                + DOMUtil.getAttrValue(child, SchemaSymbols.ATT_NAME)
                                : schemaDoc.fTargetNamespace + "," + DOMUtil.getAttrValue(child,
                                        SchemaSymbols.ATT_NAME), fSchemaHandler.ATTRIBUTE_TYPE, fSchemaHandler
                                                .getIDRegistry(), fSchemaHandler.getIDRegistry_sub(), child,
                                schemaDoc);
                    }
        ...
        if (nameAtt == null) {
            if (isGlobal)
                reportSchemaError("s4s-att-must-appear", new Object[] { SchemaSymbols.ELT_ELEMENT,
                        SchemaSymbols.ATT_NAME }, elmDecl);
            else
        ...
        if (child != null) {
            reportSchemaError("s4s-elt-must-match.1", new Object[] { nameAtt,
                    "(annotation?, (simpleType | complexType)?, (unique | key | keyref)*))", DOMUtil
                            .getLocalName(child) }, child);
        }
        ...
        // <key>, <keyref>, <unique>, nillable, default, fixed, form, block and
        // type must be absent, i.e. only minOccurs, maxOccurs, id are allowed
        // in addition to ref, along with <annotation>.
        // Attributes are checked in XSAttributeChecker, elements are checked in
        // "traverse" method

        // 3 type and either <simpleType> or <complexType> are mutually
        // exclusive.
        ...
        if (element.fDefault != null) {
            ...
            if (XSConstraints.ElementDefaultValidImmediate(element.fType, element.fDefault.normalizedValue,
                    fValidationState, element.fDefault) == null) {
                reportSchemaError("e-props-correct.2", new Object[] { nameAtt,
                        element.fDefault.normalizedValue }, elmDecl);
        ...
        // the value of the {substitution group exclusions} of the {substitution
        // group affiliation}, as defined in Type Derivation OK (Complex)
        // (3.4.6) (if the {type definition} is complex) or as defined in Type
        // Derivation OK (Simple) (3.14.6) (if the {type definition} is simple).
        if (element.fSubGroup != null) {
            if (!XSConstraints.checkTypeDerivationOk(element.fType, element.fSubGroup.fType,
                    element.fSubGroup.fFinal)) {
                reportSchemaError("e-props-correct.4", new Object[] { nameAtt, subGroupAtt.prefix + ":"
                        + subGroupAtt.localpart }, elmDecl);
        ...
        if (element.fDefault != null) {
            if ((elementType.getTypeCategory() == XSTypeDefinition.SIMPLE_TYPE && ((XSSimpleType) elementType)
                    .isIDType()) || (elementType.getTypeCategory() == XSTypeDefinition.COMPLEX_TYPE
        ...
        if (isGlobal) {
            ...
            // if we are tolerating duplicates, and we found a duplicate
            // declaration
