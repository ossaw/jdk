...
class ParserForXMLSchema extends RegexParser {
     ...
     * multi-c-escape | category-c-escape | block-c-escape | &lt;any XML
     * char&gt; cc-normal-c ::= &lt;any character except [, ], \&gt;
    ...
    protected RangeToken parseCharacterClass(boolean useNrange)
        ...
        while ((type = this.read()) != T_EOF) { // Don't use 'cotinue' for this
                                                // loop.
            ...
            if (!end) { // if not shorthands...
                if (type == T_CHAR) {
                    ...
                    if (c == '-' && this.chardata != ']' && !firstloop)
                        throw this.ex("parser.cc.8", this.offset - 2); // if
                                                                       // regex
                                                                       // =
                                                                       // '[-]'
                                                                       // then
                ...
                if (this.read() != T_CHAR || this.chardata != '-' || c == '-'
                    ...
                    // c '-' ']' -> '-' is a single-range.
                    if (type == T_CHAR && this.chardata == ']') { // if - is at
                                                                  // the last
                                                                  // position
                                                                  // of the
                                                                  // group
