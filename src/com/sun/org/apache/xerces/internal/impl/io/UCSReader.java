...
public class UCSReader extends Reader {
    ...
    /**
     * Default byte buffer size (8192, larger than that of ASCIIReader since
     * it's reasonable to surmise that the average UCS-4-encoded file should be
     * 4 times as large as the average ASCII-encoded file).
     */
    ...
    /**
     * Constructs an ASCII reader from the specified input stream using the
     * default buffer size. The Endian-ness and whether this is UCS-2 or UCS-4
     ...
     * size. The Endian-ness and whether this is UCS-2 or UCS-4 needs also to be
     * known in advance.
     *
     * @param inputStream
     *        The input stream.
     ...
     *         <tt>0x00-0x7f</tt>), or -1 if the end of the stream has been
     *         reached
    ...
    public int read() throws IOException {
        ...
        if (fEncoding >= 4) {
            ...
            if (b3 == 0xff)
                return -1;
            System.err.println("b0 is " + (b0 & 0xff) + " b1 " + (b1 & 0xff) + " b2 " + (b2 & 0xff) + " b3 "
                    + (b3 & 0xff));
            if (fEncoding == UCS4BE)
    ...
    public int read(char ch[], int offset, int length) throws IOException {
        ...
        if (count == -1)
            return -1;
        // try and make count be a multiple of the number of bytes we're looking
        // for
