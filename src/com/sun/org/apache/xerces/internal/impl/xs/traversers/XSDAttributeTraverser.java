...
class XSDAttributeTraverser extends XSDAbstractTraverser {
	...
	protected XSAttributeUseImpl traverseLocal(Element attrDecl, XSDocumentInfo schemaDoc,
		...
		// optional.
		if (consType == XSConstants.VC_DEFAULT && useAtt != null
		...
		if (defaultAtt != null && attrUse != null) {
			...
			if (attrUse.fAttrDecl.getConstraintType() == XSConstants.VC_FIXED
				...
				if (attrUse.fConstraintType != XSConstants.VC_FIXED
						|| !attrUse.fAttrDecl.getValInfo().actualValue
	...
	protected XSAttributeDecl traverseGlobal(Element attrDecl, XSDocumentInfo schemaDoc,
		...
		XSAttributeDecl attribute = traverseNamedAttr(attrDecl, attrValues, schemaDoc, grammar,
				true, null);
		fAttrChecker.returnAttrArray(attrValues, schemaDoc);
		return attribute;
	...
	XSAttributeDecl traverseNamedAttr(Element attrDecl, Object[] attrValues,
		...
		if (attrType == null && typeAtt != null) {
			...
			if (type != null && type.getTypeCategory() == XSTypeDefinition.SIMPLE_TYPE) {
				...
				reportSchemaError("src-resolve",
						new Object[] { typeAtt.rawname, "simpleType definition" }, attrDecl);
				if (type == null) {
		...
		if (child != null) {
			reportSchemaError("s4s-elt-must-match.1", new Object[] { nameAtt,
					"(annotation?, (simpleType?))", DOMUtil.getLocalName(child) }, child);
		}
		...
		if (attDefault != null) {
			...
			try {
				...
				reportSchemaError(ide.getKey(), ide.getArgs(), attrDecl);
				reportSchemaError("a-props-correct.2",
						new Object[] { nameAtt, attDefault.normalizedValue }, attrDecl);
				// Recover by removing the default value
				attDefault = null;
				constraintType = XSConstants.VC_NONE;
				attribute.setValues(nameAtt, tnsAtt, attrType, constraintType, scope, attDefault,
						enclCT, annotations);
			}
		}
	...
	void checkDefaultValid(XSAttributeDecl attribute) throws InvalidDatatypeValueException {
		...
		((XSSimpleType) attribute.getTypeDefinition()).validate(
				attribute.getValInfo().normalizedValue, fValidationState, attribute.getValInfo());
		// validate the canonical lexical rep
		((XSSimpleType) attribute.getTypeDefinition()).validate(
				attribute.getValInfo().stringValue(), fValidationState, attribute.getValInfo());
	}
