...
public final class XMLLimitAnalyzer {
	...
	/**
	 * Add the value to the current max count for the specified property To find
	...
	public void debugPrint(XMLSecurityManager securityManager) {
		...
		System.out.println(formatter.format("%30s %15s %15s %15s %30s", "Property", "Limit",
				"Total size", "Size", "Entity Name"));

		for (Limit limit : Limit.values()) {
			formatter = new Formatter();
			System.out.println(formatter.format("%30s %15d %15d %15d %30s", limit.name(),
					securityManager.getLimit(limit), totalValue[limit.ordinal()],
					values[limit.ordinal()], names[limit.ordinal()]));
		}
	}
}
