...
/**
 * This class is used for reading resources requested in &lt;include&gt;
 ...
 * files as text, there would need to be some refactoring done.
 *
...
public class XIncludeTextReader {
	...
	/**
	 * Construct the XIncludeReader using the XMLInputSource and
	 * XIncludeHandler.
	 *
	 * @param source
	...
	/**
	 * Sets the XMLErrorReporter used for reporting errors while reading the
	 * text include.
	 *
	 * @param errorReporter
	 *            the XMLErrorReporter to be used for reporting errors.
	 */
	...
	protected Reader getReader(XMLInputSource source) throws IOException {
		if (source.getCharacterStream() != null) {
			...
			if (source.getByteStream() != null) {
				...
				String expandedSystemId = XMLEntityManager.expandSystemId(source.getSystemId(),
						source.getBaseSystemId(), false);
				...
				// content type will be string like "text/xml; charset=UTF-8" or
				// "text/xml"
				...
				if (index != -1) {
					...
					// this should be something like "charset=UTF-8", but we
					// want to
				...
				/**
				 * The encoding of such a resource is determined by: 1 external
				 * encoding information, if available, otherwise -- the most
				 * common type of external information is the "charset"
				 * parameter of a MIME package 2 if the media type of the
				 * resource is text/xml, application/xml, or matches the
				 * conventions text/*+xml or application/*+xml as described in
				 * XML Media Types [IETF RFC 3023], the encoding is recognized
				 * as specified in XML 1.0, otherwise 3 the value of the
			...
			// If the specified encoding wasn't a recognized IANA encoding throw
			// an IOException.
			// The XIncludeHandler will report this as a ResourceError and then
			// will
			...
			if (javaEncoding == null) {
				...
				throw new IOException(aFormatter.formatMessage(aLocale, "EncodingDeclInvalid",
						new Object[] { encoding }));
	...
	/**
	 * XMLEntityManager cares about endian-ness, since it creates its own
	 * optimized readers. Since we're just using generic Java readers for now,
	 * we're not caring about endian-ness. If this changes, even more code needs
	 * to be copied from XMLEntity manager. -- PJM
	 */
	...
	/**
	 * Read the input stream as text, and pass the text on to the
	 * XIncludeHandler using calls to characters(). This will read all of the
	 * text it can from the resource.
	 *
	 * @throws IOException
	 */
			...
			for (int i = 0; i < readSize; ++i) {
				...
				if (!isValid(ch)) {
					if (XMLChar.isHighSurrogate(ch)) {
						...
						fErrorReporter.reportError(XMLMessageFormatter.XML_DOMAIN,
								"InvalidCharInContent", new Object[] { Integer.toString(ch, 16) },
								XMLErrorReporter.SEVERITY_FATAL_ERROR);
					}
				}
			}
	...
	/**
	 * Closes the stream. Call this after parse(), or when there is no longer
	 * any need for this object.
	 *
	 * @throws IOException
	 */
	...
	/**
	 * Returns true if the specified character is a valid XML character as per
	...
	/**
	 * Sets the buffer size property for the reader which decides the chunk
