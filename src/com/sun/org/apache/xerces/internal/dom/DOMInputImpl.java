...
/**
 * This Class <code>DOMInputImpl</code> represents a single input source for an
 * XML entity.
 * <p>
 * This Class allows an application to encapsulate information about an input
 ...
 * the parser: as the argument to the <code>parse</code> method, or as the
 * return value of the <code>DOMResourceResolver.resolveEntity</code> method.
 * <p>
 * The <code>DOMParser</code> will use the <code>LSInput</code> object to
 * determine how to read XML input. If there is a character stream available,
 * the parser will read that stream directly; if not, the parser will use a byte
 * stream, if available; if neither a character stream nor a byte stream is
 * available, the parser will attempt to open a URI connection to the resource
 * identified by the system identifier.
 * <p>
 * An <code>LSInput</code> object belongs to the application: the parser shall
 * never modify it in any way (it may modify a copy if necessary). Eventhough
 * all attributes in this interface are writable the DOM implementation is
 * expected to never mutate a LSInput.
 * <p>
 * See also the
 * <a href='http://www.w3.org/TR/2001/WD-DOM-Level-3-ASLS-20011025'>Document
 * Object Model (DOM) Level 3 Abstract Schemas and Load and Save
 * Specification</a>.
 *
 * @xerces.internal
...
public class DOMInputImpl implements LSInput {
     ...
     * leaving resolution of the entity and opening of the input stream up to
     * the caller.
     *
     * @param publicId
    ...
    public DOMInputImpl(String publicId, String systemId, String baseSystemId, InputStream byteStream,
            String encoding) {
     ...
     * should set the encoding property. Setting the encoding in this way will
     * override any encoding specified in the XML declaration itself.
     */
    ...
    /**
     * An attribute of a language-binding dependent type that represents a
     * stream of 16-bit units. Application must encode the stream using UTF-16
     * (defined in and Amendment 1 of ). <br>
     ...
     * If string data is available in the input source, the parser will ignore
     * the character stream and the byte stream and will not attempt to open a
     ...
     * Encoding in Entities"). <br>
     * This attribute has no effect when the application provides a character
     * stream. For other sources of input, an encoding specified by means of
     * this attribute will override any encoding specified in the XML claration
     * or the Text Declaration, or an encoding obtained from a higher level
     ...
     * system identifier is optional if there is a byte stream or a character
     * stream, but it is still useful to provide one, since the application can
     * use it to resolve relative URIs and can include it in error messages and
     * warnings (the parser will attempt to fetch the ressource identifier by
     * the URI reference only if there is no byte stream or character stream
     * specified). <br>
     * If the application knows the character encoding of the object pointed to
     * by the system identifier, it can register the encoding by setting the
     * encoding attribute. <br>
