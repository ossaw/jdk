...
/**
 * This class represents an output destination for data. This interface allows
 ...
 * specifiedencoding), a base URI, and/or a character stream. The exact
 * definitions of a byte stream and a character stream are binding dependent.
 ...
 * method DOMImplementationLS.createLSOutput() to create objects that implement
 * this interface. The DOMSerializer will use the LSOutput object to determine
 * where to serialize the output to. The DOMSerializer will look at the
 * different outputs specified in the LSOutput in the following order to know
 * which one to output to, the first one that data can be output to will be
 * used: 1.LSOutput.characterStream 2.LSOutput.byteStream 3.LSOutput.systemId
...
public class DOMOutputImpl implements LSOutput {
     ...
     * object pointed to by the system identifier, it can set the encoding using
     * the encoding attribute. If the system ID is a relative URI reference (see
     * section 5 in [IETF RFC 2396]), the behavior is implementation dependent.
     */
     ...
     * an encoding obtained from a higher level protocol, such as HTTP [IETF RFC
     * 2616].
     */
