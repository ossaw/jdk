...
/**
 * Notations are how the Document Type Description (DTD) records hints about the
 * format of an XML "unparsed entity" -- in other words, non-XML data bound to
 * this document type, which some applications may wish to consult when
 * manipulating the document. A Notation represents a name-value pair, with its
 * nodeName being set to the declared name of the notation.
 * <P>
 * Notations are also used to formally declare the "targets" of Processing
 * Instructions.
 * <P>
 * Note that the Notation's data is non-DOM information; the DOM only records
