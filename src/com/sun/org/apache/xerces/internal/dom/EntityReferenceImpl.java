...
/**
 * EntityReference models the XML &entityname; syntax, when used for entities
 * defined by the DOM. Entities hardcoded into XML, such as character entities,
 * should instead have been translated into text by the code which generated the
 * DOM tree.
 * <P>
 ...
 * synchronized with its parent. This leads me back to the
 * "cached image of centrally defined data" solution, much as I dislike it.
 * <P>
...
public class EntityReferenceImpl extends ParentNode implements EntityReference {
    ...
    /**
     * NON-DOM: compute string representation of the entity reference. This
     * method is used to retrieve a string value for an attribute node that has
     * child nodes.
     * 
     * @return String representing a value of this entity ref. or null if any
     *         node other than EntityReference, Text is encountered during
     *         computation
     */
     ...
     * doesn't support editing the Entity though, since this only called once
     * for all.
     */
    ...
    /**
     * Enable the synchronize method which may do cloning. This method is
     * enabled when the parser is done with an EntityReference. /*** // revisit:
     * enable editing of Entity public void enableSynchronize(boolean
     * enableSynchronize) { fEnableSynchronize= enableSynchronize; } /
     ***/

    /**
     * EntityReference's children are a reflection of those defined in the named
     * Entity. This method updates them if the Entity is changed.
     * <P>
     * It is unclear what the least-cost resynch mechanism is. If we expect the
     * kids to be shallow, and/or expect changes to the Entity contents to be
     * rare, wiping them all out and recloning is simplest.
     * <P>
     * If we expect them to be deep, it might be better to first decide which
     * kids (if any) persist, and keep the ones (if any) that are unchanged
     * rather than doing all the work of cloning them again. But that latter
     * gets into having to convolve the two child lists, insert new information
     * in the right order (and possibly reorder the existing kids), and a few
     * other complexities that I really don't want to deal with in this
     * implementation.
     * <P>
    ...
    // clone the Entity.
    /***
     * // revisit: enable editing of Entity private void synchronize() { if
     * (!fEnableSynchronize) { return; } DocumentType doctype; NamedNodeMap
