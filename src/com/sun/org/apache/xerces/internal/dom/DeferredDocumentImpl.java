...
public class DeferredDocumentImpl extends DocumentImpl implements DeferredNode {
    ...
    public int createDeferredEntity(String entityName, String publicId, String systemId, String notationName,
            String baseURI) {
        // create node
     ...
     * @return int
     */
    public int setDeferredAttribute(int elementNodeIndex, String attrName, String attrURI, String attrValue,
            boolean specified, boolean id, Object type) {
    ...
    /**
     * Returns the prev sibling of the given node. This is post-normalization of
     * Text Nodes.
     */
    ...
    /**
     * Returns the <i>real</i> prev sibling of the given node, directly from the
     * data structures. Used by TextImpl#getNodeValue() to normalize values.
     */
    ...
    /**
     * Returns the index of the element definition in the table with the
     * specified name index, or -1 if no such definition exists.
     */
    ...
    /**
     * Returns the extra info of the given node. Used by AttrImpl to store
     * specified value (1 == true).
     */
    ...
    public void putIdentifier(String name, int elementNodeIndex) {
        ...
        if (DEBUG_IDS) {
            System.out.println("putIdentifier(" + name + ", " + elementNodeIndex + ')' + " // "
                    + getChunkValue(fNodeName, elementNodeIndex >> CHUNK_SHIFT, elementNodeIndex
                            & CHUNK_MASK));
        }
     ...
     * the children at once solves a lot of work to keep the two structures in
     * sync. The problem gets worse when editing the tree -- this makes it a lot
     * easier. This is not directly used in this class but this method is here
     * so that it can be shared by all deferred subclasses of AttrImpl.
     */
     ...
     * sync. The problem gets worse when editing the tree -- this makes it a lot
     * easier. This is not directly used in this class but this method is here
     * so that it can be shared by all deferred subclasses of ParentNode.
     */
    ...
    /**
     * Performs a binary search for a target value in an array of values. The
     * array of values must be in ascending sorted order before calling this
    ...
    /**
     * Clears the specified value in the given data at the chunk and index. Note
     * that this method will clear the given chunk if the reference count
     * becomes zero.
     *
     * @return Returns the old value.
     */
    ...
    /**
     * This version of putIdentifier is needed to avoid fluffing all of the
     * paths to ID attributes when a node object is created that contains an ID
     * attribute.
     */
