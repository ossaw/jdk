...
public abstract class CharacterDataImpl extends ChildNode {
    ...
    /**
     * This function added so that we can distinguish whether setNodeValue has
     * been called from some other DOM functions. or by the client.
     * <p>
     * This is important, because we do one type of Range fix-up, from the
     * high-level functions in CharacterData, and another type if the client
     * simply calls setNodeValue(value).
     */
     ...
     * (via notification to the document)
     */
    ...
    /**
     * Report number of characters currently stored in this node's data. It may
     * be 0, meaning that the value is an empty string.
     */
    public int getLength() {
        if (needsSyncData()) {
            synchronizeData();
     ...
     * node. Note that this, and insert(), are the paths by which a DOM could
     * wind up accumulating more data than the language's strings can easily
    ...
    /**
     * Remove a range of characters from the node's value. Throws a DOMException
     * if the offset is beyond the end of the string. However, a deletion
     * _count_ that exceeds the available data is accepted as a delete-to-end
     * request.
     *
     ...
     * control which mutation events are spawned. This version of the deleteData
    ...
    void internalDeleteData(int offset, int count, boolean replace) throws DOMException {
        ...
        try {
            String value = data.substring(0, offset) + (tailLength > 0 ? data.substring(offset + count, offset
                    + count + tailLength) : "");
            ...
            String msg = DOMMessageFormatter.formatMessage(DOMMessageFormatter.DOM_DOMAIN, "INDEX_SIZE_ERR",
                    null);
            throw new DOMException(DOMException.INDEX_SIZE_ERR, msg);
        }
    ...
    /**
     * Insert additional characters into the data stored in this node, at the
     * offset specified.
     *
     * @throws DOMException(INDEX_SIZE_ERR)
     *         if offset is negative
    ...
    /**
     * NON-DOM INTERNAL: Within DOM actions, we sometimes need to be able to
     * control which mutation events are spawned. This version of the insertData
     * operation allows us to do so. It is not intended for use by application
     * programs.
     */
    ...
    /**
     * Replace a series of characters at the specified (zero-based) offset with
     * a new string, NOT necessarily of the same length. Convenience method,
     * equivalent to a delete followed by an insert. Throws a DOMException if
    ...
    /**
     * Substring is more than a convenience function. In some implementations of
     * the DOM, where the stored data may exceed the length that can be returned
     * in a single string, the only way to read it all is to extract it in
