...
public class DOMNormalizer implements XMLDocumentHandler {
	...
	protected void normalizeDocument(CoreDocumentImpl document, DOMConfigurationImpl config) {
		...
		if ((fConfiguration.features & DOMConfigurationImpl.VALIDATE) != 0) {
			String schemaLang = (String) fConfiguration
					.getProperty(DOMConfigurationImpl.JAXP_SCHEMA_LANGUAGE);
		...
		try {
			...
			if (fValidationHandler != null) {
				...
				CoreDOMImplementationImpl.singleton
						.releaseValidator(XMLGrammarDescription.XML_SCHEMA, fValidationHandler);
	...
	protected Node normalizeNode(Node node) {
		...
		case Node.DOCUMENT_TYPE_NODE: {
			...
			fDTDValidator.startDocument(
					new SimpleLocator(fDocument.fDocumentURI, fDocument.fDocumentURI, -1, -1),
					fDocument.encoding, fNamespaceContext, null);
			fDTDValidator.doctypeDecl(docType.getName(), docType.getPublicId(),
		...
		case Node.ELEMENT_NODE: {
			...
			if (fDocument.errorChecking) {
				if (((fConfiguration.features & DOMConfigurationImpl.WELLFORMED) != 0)
					...
					if (fNamespaceValidation) {
						wellformed = CoreDocumentImpl.isValidQName(node.getPrefix(),
								node.getLocalName(), fDocument.isXML11Version());
					} else {
					...
					if (!wellformed) {
						String msg = DOMMessageFormatter.formatMessage(
								DOMMessageFormatter.DOM_DOMAIN, "wf-invalid-character-in-node-name",
			...
			AttributeMap attributes = (elem.hasAttributes()) ? (AttributeMap) elem.getAttributes()
					: null;
			...
			if (DEBUG_ND) {
				// normalized subtree
				System.out
						.println("***The children of {" + node.getNodeName() + "} are normalized");
				for (kid = elem.getFirstChild(); kid != null; kid = next) {
					next = kid.getNextSibling();
		...
		case Node.CDATA_SECTION_NODE: {
			...
			if ((fConfiguration.features & DOMConfigurationImpl.SPLITCDATA) != 0) {
				...
				Node parent = node.getParentNode();
				if (fDocument.errorChecking) {
					isXMLCharWF(fErrorHandler, fError, fLocator, node.getNodeValue(),
							fDocument.isXML11Version());
				}
	...
	private XMLGrammarPool createGrammarPool(DocumentTypeImpl docType) {
		...
		preParser.setProperty(Constants.XERCES_PROPERTY_PREFIX + Constants.XMLGRAMMAR_POOL_PROPERTY,
				pool);
		...
		try {
			DTDGrammar g = (DTDGrammar) preParser.preparseGrammar(XMLGrammarDescription.XML_DTD,
					is);
	...
	protected final void namespaceFixUp(ElementImpl element, AttributeMap attributes) {
		if (DEBUG) {
			System.out.println("[ns-fixup] element:" + element.getNodeName() + " uri: "
		...
		if ((fConfiguration.features & DOMConfigurationImpl.NSDECL) == 0) {
			...
			if (element.getLocalName() == null) {
				...
				if (fNamespaceValidation) {
					...
					reportDOMError(fErrorHandler, fError, fLocator, msg,
							DOMError.SEVERITY_FATAL_ERROR, "NullLocalElementName");
				} else {
					String msg = DOMMessageFormatter.formatMessage(DOMMessageFormatter.DOM_DOMAIN,
							"NullLocalElementName", new Object[] { element.getNodeName() });
					reportDOMError(fErrorHandler, fError, fLocator, msg, DOMError.SEVERITY_ERROR,
			...
			for (int i = 0; i < fAttributeList.size(); i++) {
				...
				if (uri != null) { // attribute has namespace !=null
					...
					if (fDocument.errorChecking
						...
						if (fDocument.isXMLVersionChanged()) {
							boolean wellformed = CoreDocumentImpl.isXMLName(attr.getNodeName(),
									fDocument.isXML11Version());
							if (!wellformed) {
								String msg = DOMMessageFormatter.formatMessage(
										DOMMessageFormatter.DOM_DOMAIN,
					...
					if (attr.getLocalName() == null) {
						...
						if (fNamespaceValidation) {
							String msg = DOMMessageFormatter.formatMessage(
									DOMMessageFormatter.DOM_DOMAIN, "NullLocalAttrName",
							...
							String msg = DOMMessageFormatter.formatMessage(
									...
									new Object[] { attr.getNodeName() });
							reportDOMError(fErrorHandler, fError, fLocator, msg,
									DOMError.SEVERITY_ERROR, "NullLocalAttrName");
						}
					} else {
	...
	protected final void updateQName(Node node, QName qname) {
		...
		// table.
		qname.prefix = (prefix != null && prefix.length() != 0) ? fSymbolTable.addSymbol(prefix)
				: null;
	...
	protected final class XMLAttributesProxy implements XMLAttributes {
		...
		public int addAttribute(QName qname, String attrType, String attrValue) {
			...
			if (index < 0) {
				...
				// be added back
				AttrImpl attr = (AttrImpl) ((CoreDocumentImpl) fElement.getOwnerDocument())
						.createAttributeNS(qname.uri, qname.rawname, qname.localpart);
	 ...
	 *            be reported during the parsing of this document. However, it
	 *            is <em>strongly</em> recommended that a locator be supplied
	 *            that can at least report the system identifier of the
	 *            document.
	 * @param encoding
