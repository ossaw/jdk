...
public abstract class ParentNode extends ChildNode {
     ...
     * example -- will _not_ clone those children unless a "deep clone" has been
     * requested. A shallow clone of an Attr node will yield an empty Attr of
     * the same name.
     * <p>
    ...
    /**
     * NON-DOM INTERNAL: Within DOM actions,we sometimes need to be able to
     * control which mutation events are spawned. This version of the
    ...
    Node internalInsertBefore(Node newChild, Node refChild, boolean replace)
        ...
        if (errorChecking) {
            ...
            if (ownerDocument.ancestorChecking) {
                ...
                for (NodeImpl a = this; treeSafe && a != null; a = a
                        .parentNode()) {
                    treeSafe = newChild != a;
                }
                if (!treeSafe) {
    ...
    public String getTextContent() throws DOMException {
        ...
        if (child != null) {
            ...
            if (next == null) {
                return hasTextContent(child) ? ((NodeImpl) child)
                        .getTextContent() : "";
            }
            if (fBufferStr == null) {
    ...
    final boolean hasTextContent(Node child) {
        return child.getNodeType() != Node.COMMENT_NODE && child
                .getNodeType() != Node.PROCESSING_INSTRUCTION_NODE && (child
                        .getNodeType() != Node.TEXT_NODE || ((TextImpl) child)
     ...
     * of bounds. Use to implement NodeList.item().
     * 
     * @param index
     *              int
     */
     ...
     * optimizes getChildNodes() by implementing NodeList itself. However if a
     * subclass Element implements methods with the same name as the NodeList
     ...
     * have it call this method. The resulting NodeList instance maybe shared
     * and cached in a transient field, but the cached value must be cleared if
     * the node is cloned.
     */
     ...
     * inserted child causes this node to be unnormalized, then this node is
     * flagged accordingly. The conditions for changing the normalized state
     * are:
     * <ul>
