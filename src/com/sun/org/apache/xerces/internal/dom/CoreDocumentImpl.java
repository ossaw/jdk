 ...
 * <p>
 * <b>Note:</b> When any node in the document is serialized, the entire document
...
public class CoreDocumentImpl extends ParentNode implements Document {
    ...
    /**
     * Number of alterations made to this document since its creation. Serves as
     * a "dirty bit" so that live objects such as NodeList can recognize when an
     * alteration has been made and discard its cached state information.
     * <p>
     * Any method that alters the tree structure MUST cause or be accompanied by
     * a call to changed(), to inform it that any outstanding NodeLists may have
     * to be updated.
     * <p>
     * (Required because NodeList is simultaneously "live" and integer- indexed
     * -- a bad decision in the DOM's design.)
     * <p>
     ...
     * rather than a count. This would have the advantage that "harmless"
     * changes (those producing equal() trees) would not force NodeList to
     * resynchronize. Disadvantage is that it's slightly more prone to
     * "false negatives", though that's the difference between "wildly unlikely
     * " and "absurdly unlikely". IF we start maintaining digests, we should
     * consider taking advantage of them.
     * Note: This used to be done a node basis, so that we knew what subtree
     * changed. But since only DeepNodeList really use this today, the gain
     * appears to be really small compared to the cost of having an int on every
     * (parent) node plus having to walk up the tree all the way to the root to
     * mark the branch as changed everytime a node is changed. So we now have a
     * single counter global to the document. It means that some objects may
     * flush their cache more often than necessary, but this makes nodes smaller
     * and only the document needs to be marked as changed.
     */
    ...
    private static final ObjectStreamField[] serialPersistentFields = new ObjectStreamField[] {
            new ObjectStreamField("docType", DocumentTypeImpl.class), new ObjectStreamField("docElement",
                    ElementImpl.class), new ObjectStreamField("fFreeNLCache", NodeListCache.class),
            new ObjectStreamField("encoding", String.class), new ObjectStreamField("actualEncoding",
                    String.class), new ObjectStreamField("version", String.class), new ObjectStreamField(
                            "standalone", boolean.class), new ObjectStreamField("fDocumentURI", String.class),
            new ObjectStreamField("userData", Hashtable.class), new ObjectStreamField("identifiers",
                    Hashtable.class), new ObjectStreamField("changes", int.class), new ObjectStreamField(
                            "allowGrammarAccess", boolean.class), new ObjectStreamField("errorChecking",
                                    boolean.class), new ObjectStreamField("ancestorChecking", boolean.class),
            new ObjectStreamField("xmlVersionChanged", boolean.class), new ObjectStreamField("documentNumber",
                    int.class), new ObjectStreamField("nodeCounter", int.class), new ObjectStreamField(
                            "nodeTable", Hashtable.class), new ObjectStreamField("xml11Version",
                                    boolean.class), };
    ...
    /**
     * NON-DOM: Actually creating a Document is outside the DOM's spec, since it
     * has to operate in terms of a particular implementation.
     */
    ...
    public CoreDocumentImpl(boolean grammarAccess) {
        ...
        String systemProp = SecuritySupport.getSystemProperty(Constants.SUN_DOM_PROPERTY_PREFIX
    ...
    /**
     * For DOM2 support. The createDocument factory method is in
     * DOMImplementation.
     */
    ...
    /**
     * Since a Document may contain at most one top-level Element child, and at
     * most one DocumentType declaraction, we need to subclass our add-children
     * methods to implement this constraint. Since appendChild() is implemented
     * as insertBefore(,null), altering the latter fixes both.
     * <p>
    ...
    public Node insertBefore(Node newChild, Node refChild) throws DOMException {
        ...
        if (errorChecking) {
            if ((type == Node.ELEMENT_NODE && docElement != null) || (type == Node.DOCUMENT_TYPE_NODE
                    && docType != null)) {
                String msg = DOMMessageFormatter.formatMessage(DOMMessageFormatter.DOM_DOMAIN,
    ...
    /**
     * Since we cache the docElement (and, currently, docType), replaceChild has
     * to update the cache
     * REVISIT: According to the spec it is not allowed to alter neither the
     * document element nor the document type in any way
     */
    public Node replaceChild(Node newChild, Node oldChild) throws DOMException {
        ...
        if (errorChecking && ((docType != null && oldChild.getNodeType() != Node.DOCUMENT_TYPE_NODE
                && newChild.getNodeType() == Node.DOCUMENT_TYPE_NODE) || (docElement != null && oldChild
    ...
    /**
     * Factory method; creates a CDATASection having this Document as its
     * OwnerDoc.
     *
     * @param data
    ...
    /**
     * Factory method; creates a ProcessingInstruction having this Document as
     * its OwnerDoc.
     *
     * @param target
     ...
     * legal to have more than one Element at the top level of the document, we
     * pick the one with the tagName "HTML". For XML there should be only one
     * top-level
    ...
    /**
     * Sets whether the DOM implementation performs error checking upon
     * operations. Turning off error checking only affects the following DOM
     * checks:
     * <ul>
     ...
     * </ul>
     * <p>
     * Turning off error checking does <em>not</em> turn off the following
     * checks:
     * <ul>
     * <li>Read only checks
    ...
    /**
     * DOM Internal (Was a DOM L3 Core WD public interface method
     * setActualEncoding )
    ...
    /**
     * @deprecated This method is internal and only exists for compatibility
     *             with older applications. New applications should never call
     *             this method.
     */
    ...
    /**
     * DOM Level 3 CR - Experimental. version - An attribute specifying, as part
     * of the XML declaration, the version number of this document.
     */
    public void setXmlVersion(String value) {
        if (value.equals("1.0") || value.equals("1.1")) {
            ...
            // NOT_SUPPORTED_ERR: Raised if the vesion is set to a value that is
            // not supported by
            // this document
    ...
    //
    /**
     * DOM Level 3 WD - Experimental. Indicates whether the method load should
     * be synchronous or asynchronous. When the async attribute is set to
     * <code>true</code> the load method returns control to the caller before
     * the document has completed loading. The default value of this property is
     * <code>false</code>. <br>
     ...
     * Should the DOM spec define the default value of this property? What if
     * implementing both async and sync IO is impractical in some systems?
     * 2001-09-14. default is <code>false</code> but we need to check with
     * Mozilla and IE.
     */
    ...
    /**
     * DOM Level 3 WD - Experimental. If the document is currently being loaded
     * as a result of the method <code>load</code> being invoked the loading and
    ...
    /**
     * DOM Level 3 WD - Experimental.
     * Replaces the content of the document with the result of parsing the given
     * URI. Invoking this method will either block the caller or return to the
     * caller immediately depending on the value of the async attribute. Once
     * the document is fully loaded a "load" event (as defined in [
     * <a href='http://www.w3.org/TR/2003/WD-DOM-Level-3-Events-20030331'>DOM
     * Level 3 Events</a>] , except that the <code>Event.targetNode</code> will
     * be the document, not an element) will be dispatched on the document. If
     * an error occurs, an implementation dependent "error" event will be
     ...
     * load is initiated. <br>
     * When invoking this method the parameters used in the
     * <code>DOMParser</code> interface are assumed to have their default values
     * with the exception that the parameters <code>"entities"</code> ,
     ...
     * <code>DOMParser.parseWithContext</code> with an input stream referencing
     * the URI that was passed to this call, the document as the context node,
     * and the action <code>ACTION_REPLACE_CHILDREN</code>.
     * 
     * @param uri
     *        The URI reference for the XML file to be loaded. If this is a
     *        relative URI, the base URI used by the implementation is
     *        implementation dependent.
     * @return If async is set to <code>true</code> <code>load</code> returns
     *         <code>true</code> if the document load was successfully
     *         initiated. If an error occurred when initiating the document
     *         load, <code>load</code> returns <code>false</code>.If async is
     *         set to <code>false</code> <code>load</code> returns
     *         <code>true</code> if the document was successfully loaded and
     *         parsed. If an error occurred when either loading or parsing the
    ...
    /**
     * DOM Level 3 WD - Experimental. Replace the content of the document with
     * the result of parsing the input string, this method is always
     * synchronous.
     * 
     * @param source
    ...
    /**
     * DOM Level 3 WD - Experimental. Save the document or the given node and
     * all its descendants to a string (i.e. serialize the document or node).
     * <br>
     * The parameters used in the <code>LSSerializer</code> interface are
     * assumed to have their default values when invoking this method. <br>
     * The result of a call to this method is the same the result of a call to
     * <code>LSSerializer.writeToString</code> with the document as the node to
     * write.
     * 
     * @param node
     ...
     * the document's identifiers. The Map has Elements as the keys and their
     * identifiers as the values. When an element is being imported, a check is
     * done for an associated identifier. If one exists, the identifier is
     * registered with the new, imported element. If reversedIdentifiers is
     * null, the parameter is not applied.
     */
    private Node importNode(Node source, boolean deep, boolean cloningDoc,
            Map<Node, String> reversedIdentifiers) throws DOMException {
        Node newnode = null;
        ...
        if (source == null) {
            ...
            if (thisImpl != otherImpl) {
                ...
                if (thisImpl instanceof com.sun.org.apache.xerces.internal.dom.DOMImplementationImpl
                    ...
                    // traverse the DOM and expand deffered nodes and then allow
                    // adoption
                    ...
                    // Adopting from a DOM into a DefferedDOM, this should be
                    // okay
                } else {
    ...
    /**
     * Introduced in DOM Level 2 Returns the Element whose ID is given by
     * elementId. If no such element exists, returns null. Behavior is not
    ...
    /**
     * Registers an identifier name with a specified element node. If the
     * identifier is already registered, the new element node replaces the
     * previous node. If the specified element node is null, removeIdentifier()
     * is called.
     *
     * @see #getIdentifier
    ...
    /**
     * Returns a previously registered element with the specified identifier
     * name, or null if no element is registered.
     *
     * @see #putIdentifier
     * @see #removeIdentifier
     */
    ...
    /**
     * Removes a previously registered element with the specified identifier
     * name.
     *
     * @see #putIdentifier
     * @see #getIdentifier
    ...
    /**
     * NON-DOM: a factory method used by the Xerces DOM parser to create an
     * element.
     *
     * @param namespaceURI
     *        The namespace URI of the element to create.
     ...
     * the given namespaceURI is null or an empty string and the qualifiedName
     * has a prefix that is "xml", the created element is bound to the
    ...
    /**
     * Checks if the given qualified name is legal with respect to the version
    ...
    public static final boolean isValidQName(String prefix, String local, boolean xml11Version) {
        ...
        if (!xml11Version) {
            ...
            validNCName = (prefix == null || XML11Char.isXML11ValidNCName(prefix)) && XML11Char
                    .isXML11ValidNCName(local);
        }
    ...
    /**
     * Uses the kidOK lookup table to check whether the proposed tree structure
     * is legal.
     */
    ...
    /**
     * Puts the given NodeListCache in the free list. Note: The owner node can
     * keep using it until we reuse it
     */
     ...
     * which would affect all nodes and not just the ones that have a user data.
     */
    ...
    // -SG
    /*
     * public void finalize() { if (userData == null) { return; } Enumeration
     * nodes = userData.keys(); while (nodes.hasMoreElements()) { Object node =
     * nodes.nextElement(); Hashtable t = (Hashtable) userData.get(node); if (t
     * != null && !t.isEmpty()) { Enumeration keys = t.keys(); while
     * (keys.hasMoreElements()) { String key = (String) keys.nextElement();
     * UserDataRecord r = (UserDataRecord) t.get(key); if (r.fHandler != null) {
     * r.fHandler.handle(UserDataHandler.NODE_DELETED, key, r.fData, null,
     * null); } } } } }
     */
        ...
        if (errorChecking) {
            if (namespace == null) {
                ...
                String msg = DOMMessageFormatter.formatMessage(DOMMessageFormatter.DOM_DOMAIN,
                        "NAMESPACE_ERR", null);
                throw new DOMException(DOMException.NAMESPACE_ERR, msg);
            } else if (prefix.equals("xmlns") && !namespace.equals(NamespaceContext.XMLNS_URI) || (!prefix
                    .equals("xmlns") && namespace.equals(NamespaceContext.XMLNS_URI))) {
                String msg = DOMMessageFormatter.formatMessage(DOMMessageFormatter.DOM_DOMAIN,
                        "NAMESPACE_ERR", null);
                throw new DOMException(DOMException.NAMESPACE_ERR, msg);
            }
    ...
    /**
     * We could have more xml versions in future , but for now we could do with
     * this to handle XML 1.0 and 1.1
     */
    ...
    // we should be checking the (elements, attribute, entity etc.) names only
    // when
    ...
    /**
     * NON-DOM: kept for backward compatibility Store user data related to a
     * given node This is a place where we could use weak references! Indeed,
     * the node here won't be GC'ed as long as some user data is attached to it,
     * since the userData table will have a reference to the node.
     */
    ...
    /**
     * NON-DOM: kept for backward compatibility Retreive user data related to a
     * given node
     */
    ...
    private void writeObject(ObjectOutputStream out) throws IOException {
        ...
        // userData is the original name. It has been changed to nodeUserData,
        // refer to the corrsponding @serialField
        pf.put("userData", nud);
    ...
    private void readObject(ObjectInputStream in) throws IOException, ClassNotFoundException {
        ...
        if (ids != null)
            identifiers = new HashMap<>(ids);
        if (nt != null)
            nodeTable = new HashMap<>(nt);
    }
