...
public class NamedNodeMapImpl implements NamedNodeMap, Serializable {
    ...
    /**
     * Report how many nodes are currently stored in this NamedNodeMap. Caveat:
     * This is a count rather than an index, so the highest-numbered node at any
     * time can be accessed via item(getLength()-1).
     */
    ...
    /**
     * Cloning a NamedNodeMap is a DEEP OPERATION; it always clones all the
     * nodes contained in the map.
     */
     ...
     * NamedNodeMaps readonly too. I expect that in fact the shallow version of
    ...
    /**
     * NON-DOM set the ownerDocument of this node, and the attributes it
     * contains
     */
    ...
    /**
     * Subroutine: Locate the named item, or the point at which said item should
     * be added.
     *
     * @param name
     *        Name of a node to look up.
    ...
    protected int findNamePoint(String namespaceURI, String name) {
        ...
        for (int i = 0; i < size; ++i) {
            ...
            if (namespaceURI == null) {
                if (aNamespaceURI == null && (name.equals(aLocalName) || (aLocalName == null && name.equals(a
                        .getNodeName()))))
                    return i;
            } else {
