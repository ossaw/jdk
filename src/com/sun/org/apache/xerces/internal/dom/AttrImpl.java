...
public class AttrImpl extends NodeImpl implements Attr, TypeInfo {
	...
	/**
	 * Implicit in the rerouting of getNodeValue to getValue is the need to
	 * redefine setNodeValue, for symmetry's sake. Note that since we're
	 * explicitly providing a value, Specified should be set true.... even if
	 * that value equals the default.
	 */
	 ...
	 * as "remove all children", which from outside should appear similar to
	...
	/**
	 * The "string value" of an Attribute is its text representation, which in
	 * turn is a concatenation of the string values of its children.
	 */
	...
	/**
	 * The "specified" flag is true if and only if this attribute's value was
	 * explicitly specified in the original document. Note that the
	 * implementation, not the user, is in charge of this property. If the user
	 * asserts an Attribute value (even if it ends up having the same value as
	 * the default), it is considered a specified attribute. If you really want
	 * to revert to the default, delete the attribute from the Element, and the
	...
	/**
	 * Returns the element node that this attribute is associated with, or null
	...
	/**
	 * DOM Level 3 WD- Experimental. Override inherited behavior from ParentNode
	 * to support deep equal. isEqualNode is always deep on Attr nodes.
	 */
