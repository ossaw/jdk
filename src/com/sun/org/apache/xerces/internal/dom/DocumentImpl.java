 ...
 * interface. This interface is comprised of factory methods needed to create
 * NodeIterators and TreeWalkers. The process of creating NodeIterator objects
 * also adds these references to this document. After finishing with an iterator
 * it is important to remove the object using the remove methods in this
 * implementation. This allows the release of the references from the iterator
...
public class DocumentImpl extends CoreDocumentImpl implements DocumentTraversal,
    ...
    /**
     * NON-DOM extension: Create and return a NodeIterator. The NodeIterator is
     * added to a list of NodeIterators so that it can be removed to free up the
    ...
    /**
     * Create and return a NodeIterator. The NodeIterator is added to a list of
     * NodeIterators so that it can be removed to free up the DOM Nodes it
     * references.
     *
     * @param root
     *                                 The root of the iterator.
    ...
    /**
     * Not a client function. Called by Range.detach(), so a Range can remove
     * itself from the list of Ranges.
     */
    ...
    /**
     * A method to be called when a text node has been split, so that live
     ...
     * <p>
     * Register an event listener with this Node. A listener may be
     * independently registered as both Capturing and Bubbling, but may only be
     ...
     * Introduced in DOM Level 2.
     * <p>
     * Deregister an event listener previously registered with this Node. A
     * listener must be independently removed from the Capturing and Bubbling
     * roles. Redundant removals (of listeners not currently registered for this
     * role) are ignored.
     * 
     * @param node
     ...
     * <ol>
     * <li>Event is dispatched to a particular target node, which invokes this
     * code. Note that the event's stopPropagation flag is cleared when dispatch
     * begins; thereafter, if it has been set before processing of a node
     * commences, we instead immediately advance to the DEFAULT phase.
     * <li>The node's ancestors are established as destinations for events. For
     * capture and bubble purposes, node ancestry is determined at the time
     * dispatch starts. If an event handler alters the document tree, that does
     * not change which nodes will be informed of the event.
     * <li>CAPTURING_PHASE: Ancestors are scanned, root to target, for Capturing
     * listeners. If found, they are invoked (see below).
     * <li>AT_TARGET: Event is dispatched to NON-CAPTURING listeners on the
     ...
     * specific nodes. If this DOM does, and if the event's preventDefault flag
     * has not been set, we now return to the target node and process its
     * default handler for this event, if any.
     * </ol>
     * <p>
     * Note that registration of handlers during processing of an event does not
     * take effect during this phase of this event; they will not be called
     ...
     * processed synchronously, before processing resumes on the event which
     * triggered them. Please be aware that this may result in events arriving
     * at listeners "out of order" relative to the actual sequence of requests.
     * <p>
     * Note that our implementation resets the event's stop/prevent flags when
     * dispatch begins. I believe the DOM's intent is that event objects be
    ...
    /**
     * NON-DOM INTERNAL: DOMNodeInsertedIntoDocument and ...RemovedFrom... are
     * dispatched to an entire subtree. This is the distribution code therefor.
     * They DO NOT bubble, thanks be, but may be captured.
     * <p>
     * Similar to code in dispatchingEventToSubtree however this method is only
     * used on the target node and does not start a dispatching chain on the
     * sibling of the target node as this is not part of the subtree ***** At
     * the moment I'm being sloppy and using the normal capture dispatcher on
     * every node. This could be optimized hugely by writing a capture engine
     * that tracks our position in the tree to update the capture chain without
    ...
    /**
     * NON-DOM INTERNAL: Return object for getEnclosingAttr. Carries (two
     * values, the Attr node affected (if any) and its previous string value.
    ...
    /**
     * NON-DOM INTERNAL: Convenience wrapper for calling dispatchAggregateEvents
     ...
     * DOMAttrModified and DOMSubtreeModified. Both of these should be issued
     * only once for each user-requested mutation operation, even if that
     * involves multiple changes to the DOM. For example, if a DOM operation
     * makes multiple changes to a single Attr before returning, it would be
     * nice to generate only one DOMAttrModified, and multiple changes over
     * larger scope but within a recognizable single subtree might want to
     * generate only one DOMSubtreeModified, sent to their lowest common
     * ancestor.
     * <p>
     * To manage this, use the "internal" versions of insert and remove with
     * MUTATION_LOCAL, then make an explicit call to this routine at the higher
    ...
    /**
     * NON-DOM INTERNAL: Pre-mutation context check, in preparation for later
     * generating DOMAttrModified events. Determines whether this node is within
     * an Attr
     * 
     * @param node
    ...
    /**
     * @serialData Serialized fields. Convert Maps to Hashtables and Lists to
     *             Vectors for backward compatibility.
     */
