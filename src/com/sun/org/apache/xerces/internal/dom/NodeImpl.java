...
public abstract class NodeImpl implements Node, NodeList, EventTarget, Cloneable, Serializable {
     ...
     * attributes that have the same <code>ownerElement</code>, and two nodes
     * that are the same.
     */
    ...
    /**
     * Returns a duplicate of a given node. You can consider this a generic
     * "copy constructor" for nodes. The newly returned object should be
     * completely independent of the source object's subtree, so changes in one
     * after the clone has been made will not affect the other.
     * <P>
     ...
     *      been requested. A shallow clone of an Attr node will yield an empty
     *      Attr of the same name.
     *      <p>
     *      NOTE: Clones will always be read/write, even if the node being
     *      cloned is read-only, to permit applications using only the DOM API
     *      to obtain editable copies of locked portions of the tree.
     */
    ...
    /**
     * Find the Document that this Node belongs to (the document in whose
     * context the Node was created). The Node may or may not currently be part
     * of that Document's actual contents.
     */
     ...
     * active in the DOM tree (perhaps because it has just been created or
     * removed). Note that Document, DocumentFragment, and Attribute will never
     * have parents.
     */
     ...
     * none. At this writing, Element is the only type of node which will ever
     * have attributes.
     *
     * @see ElementImpl
     */
    ...
    /**
     * Obtain a NodeList enumerating all children of this node. If there are
     * none, an (initially) empty NodeList is returned.
     * <p>
     * NodeLists are "live"; as children are added/removed the NodeList will
     * immediately reflect those changes. Also, the NodeList refers to the
     * actual nodes, so changes to those nodes made via the DOM tree will be
     * reflected in the NodeList and vice versa.
     * <p>
    ...
    /**
     * Remove a child from this Node. The removed child's subtree remains intact
     ...
     * newChild will first be removed from its previous parent, if any.
     ...
     * children. It is up to implementors or Node to override normalize() to
     * take action.
     */
    ...
    public void setPrefix(String prefix) throws DOMException {
        throw new DOMException(DOMException.NAMESPACE_ERR, DOMMessageFormatter.formatMessage(
     ...
     * Document interface, and for nodes of any type other than ELEMENT_NODE and
     * ATTRIBUTE_NODE this is the same as the nodeName attribute.
     * 
     * @since WD-DOM-Level-2-19990923
     * @see AttrNSImpl
     * @see ElementNSImpl
     ...
     * element if any, and the value of the <code>documentURI</code> attribute
     * from the <code>Document</code> interface otherwise. <br>
     ...
     * properties [base URI] defined in . When the node is a
     * <code>Notation</code>, an <code>Entity</code>, or an
     * <code>EntityReference</code>, this attribute represents the properties
     * [declaration base URI] in the . How will this be affected by resolution
     * of relative namespace URIs issue?It's not.Should this only be on
     ...
     * Null? An empty string? I think it should be the parent's.No.Should this
     * be read-only and computed or and actual read-write attribute?Read-only
     * and computed (F2F 19 Jun 2000 and teleconference 30 May 2001).If the base
     * HTML element is not yet attached to a document, does the insert change
     * the Document.baseURI? Yes. (F2F 26 Sep 2001)
     * 
     * @since DOM Level 3
     */
     ...
     * and according to the document order. This order can be extended by module
    ...
    public short compareTreePosition(Node other) {
        ...
        // If either node is of type ENTITY or NOTATION, compare as disconnected
        if (thisType == Node.ENTITY_NODE || thisType == Node.NOTATION_NODE || otherType == Node.ENTITY_NODE
        ...
        if (thisDepth > otherDepth) {
            ...
            // Check if the node we have reached is in fact "otherNode". This
            // can
        ...
        for (thisNodeP = thisNode.getParentNode(), otherNodeP = otherNode
                .getParentNode(); thisNodeP != otherNodeP;) {
            thisNode = thisNodeP;
     ...
     * getting, no serialization is performed, the returned string does not
     * contain any markup. No whitespace normalization is performed, the
     * returned string does not contain the element content whitespaces .
     ...
     * attribute is set to. On getting, no serialization is performed, the
     ...
     * whitespaces . Similarly, on setting, no parsing is performed either, the
     ...
     * if through a proxy, the references may be used completely interchangably,
     * such that all attributes have the same values and calling the same DOM
    ...
    /**
     * DOM Level 3 - Experimental: Look up the namespace URI associated to the
    ...
    public String lookupNamespaceURI(String specifiedPrefix) {
        ...
        switch (type) {
            ...
            case Node.DOCUMENT_NODE: {
                return ((NodeImpl) ((Document) this).getDocumentElement()).lookupNamespaceURI(
                        specifiedPrefix);
            }
            case Node.ENTITY_NODE:
            case Node.NOTATION_NODE:
    ...
    String lookupNamespacePrefix(String namespaceURI, ElementImpl el) {
        ...
        if (this.hasAttributes()) {
            ...
            for (int i = 0; i < length; i++) {
                ...
                if (namespace != null && namespace.equals("http://www.w3.org/2000/xmlns/")) {
                    // DOM Level 2 nodes
                    if (((attr.getNodeName().equals("xmlns")) || (attrPrefix != null && attrPrefix.equals(
     ...
     * <code>baseURI</code>. This is: they are both <code>null</code>, or they
     * have the same length and are character for character identical. The
     * <code>attributes</code> <code>NamedNodeMaps</code> are equal. This is:
     * they are both <code>null</code>, or they have the same length and for
     * each node that exists in one map there is a node that exists in the other
     * map and is equal, although not necessarily at the same index.The
     * <code>childNodes</code> <code>NodeLists</code> are equal. This is: they
     * are both <code>null</code>, or they have the same length and contain
     * equal nodes at the same index. This is true for <code>Attr</code> nodes
     * as for any other type of node. Note that normalization can affect
     ...
     * conditions must also be satisfied: The following string attributes are
     * equal: <code>publicId</code>, <code>systemId</code>,
     * <code>internalSubset</code>.The <code>entities</code>
     * <code>NamedNodeMaps</code> are equal.The <code>notations</code>
     * <code>NamedNodeMaps</code> are equal. <br>
     * On the other hand, the following do not affect equality: the
     * <code>ownerDocument</code> attribute, the <code>specified</code>
     * attribute for <code>Attr</code> nodes, the
     * <code>isWhitespaceInElementContent</code> attribute for <code>Text</code>
     ...
     * <P>
     * Most DOM users should not touch this method. Its anticpated use is during
     * construction of EntityRefernces, where it will be used to lock the
     * contents replicated from Entity so they can't be casually altered. It
     * _could_ be published as a DOM extension, if desired.
     * <P>
     * Note: since we never have any children deep is meaningless here,
     * ParentNode overrides this behavior.
     * 
     ...
     * has been extended with the ability to attach an object to each node. (If
     * you need multiple objects, you can attach a collection such as a List or
     * Map, then attach your application information to that.)
     * <p>
     * <b>Important Note:</b> You are responsible for removing references to
     * your data on nodes that are no longer used. Failure to do so will prevent
     * the nodes, your data is attached to, to be garbage collected until the
     * whole document is.
     *
     * @param data
    ...
    /**
     * For non-child nodes, the node which "points" to this node. For example,
     * the owning element for an attribute
     */
