...
public interface XSElementDeclaration extends XSTerm {
	 ...
	 * <code>scope</code>), otherwise <code>null</code> if no such definition
	 * exists.
	 */
	 ...
	 * <code>STRING_DT, SHORT_DT</code>. If the type definition of this value is
	 * a list type definition, this method returns <code>LIST_DT</code>. If the
	 * type definition of this value is a list type definition whose item type
	 * is a union type definition, this method returns
	 * <code>LISTOFUNION_DT</code>. To query the actual constraint value of the
	 * list or list of union type definitions use <code>itemValueTypes</code>.
	 * If the <code>actualNormalizedValue</code> is <code>null</code>, this
	 ...
	 * <code>actualValueType</code> is <code>LIST_DT</code>, the returned array
	 * consists of one type kind which represents the itemType. If the actual
	 * constraint value represents a list type definition whose item type is a
	 * union type definition, i.e. <code>LISTOFUNION_DT</code>, for each actual
	 ...
	 * namespace qualified attribute with local name <code>nil</code> from
	 * namespace <code>http://www.w3.org/2001/XMLSchema-instance</code> and
	 * value <code>true</code> (xsi:nil) even if it has no text or element
	 * content despite a <code>content type</code> which would otherwise require
	 * content.
	 */
	...
	/**
	 * Convenience method that checks if <code>disallowed</code> is a disallowed
	...
	/**
	 * An annotation if it exists, otherwise <code>null</code>. If not null then
	 * the first [annotation] from the sequence of annotations.
	 */
