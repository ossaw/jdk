...
public class RETest {
	...
	public static boolean test(String[] args) throws Exception {
		...
		if (args.length == 2) {
			...
			System.out
					.println("By Default will run automated tests from file 'docs/RETest.txt' ...");
			System.out.println();
	...
	private void testPrecompiledRE() {
		...
		char[] re1Instructions = { 0x007c, 0x0000, 0x001a, 0x007c, 0x0000, 0x000d, 0x0041, 0x0001,
				0x0004, 0x0061, 0x007c, 0x0000, 0x0003, 0x0047, 0x0000, 0xfff6, 0x007c, 0x0000,
				0x0003, 0x004e, 0x0000, 0x0003, 0x0041, 0x0001, 0x0004, 0x0062, 0x0045, 0x0000,
				0x0000, };
	...
	private void testSubst() {
		...
		r = new RE("http://[\\.\\w\\-\\?/~_@&=%]+");
		actual = r.subst("visit us: http://www.apache.org!", "1234<a href=\"$0\">$0</a>",
	...
	private RETestCase getNextTestCase(BufferedReader br) throws IOException {
		...
		return new RETestCase(this, tag, expr, matchAgainst, badPattern, shouldMatch,
				expectedParens);
	}
}
...
final class RETestCase {
	...
	public RETestCase(RETest test, String tag, String pattern, String toMatch, boolean badPattern,
	...
	private void testMatch() {
		...
		try {
			...
			if (checkResult(result) && (!shouldMatch || checkParens())) {
				...
				if (!tryMatchUsingCI(new CharacterArrayCharacterIterator(toMatch.toCharArray(), 0,
						toMatch.length())))
					return;

				log.append("   Match using StreamCharacterIterator\n");
				if (!tryMatchUsingCI(
						new StreamCharacterIterator(new StringBufferInputStream(toMatch))))
					return;
	...
	private boolean checkResult(boolean result) {
		...
		if (result == shouldMatch) {
			success((shouldMatch ? "Matched" : "Did not match") + " \"" + toMatch
					+ "\", as expected:");
			return true;
		} else {
			if (shouldMatch) {
