 ...
 * Regular expressions are pattern descriptions which enable sophisticated
 * matching of strings. In addition to being able to match a string against a
 ...
 * efficiency. In fact, if you want to pre-compile one or more regular
 * expressions, the 'recompile' class can be invoked from the command line to
 * produce compiled output like this:
 *
 * <pre>
 ...
 * pre-compiled expression re1 and thus avoid the overhead of compiling the
 * expression at runtime. If you require more dynamic regular expressions, you
 * can construct a single RECompiler object and re-use it to compile each
 * expression. Similarly, you can change the program run by a given matcher
 * object at any time. However, RE and RECompiler are not threadsafe (for
 * efficiency reasons, and because requiring thread safety in this class is
 ...
 * syntax) (I18N/locale issue)</li>
 * <li>com.weusours.util.re does not support nested POSIX character classes
 * (definitely should, but not completely trivial)</li>
				...
				while ((lCurrentPosition = substitution.indexOf("$",
					...
					if ((lCurrentPosition == 0 || substitution.charAt(
						...
						if (c >= '0' && c <= '9') {
							...
							// Note: if a parenthesized expression of the
							// requested
							// index is not available "null" is added to the
							// string
