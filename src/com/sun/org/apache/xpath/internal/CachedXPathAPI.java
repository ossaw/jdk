 ...
 * created for each evaluation. A faster way is to precompile the XPaths using
 * the low-level API, and then just use the XPaths over and over.
 * This is an alternative for the old XPathAPI class, which provided static
 * methods for the purpose but had the drawback of instantiating a new
 * XPathContext (and thus building a new DTMManager, and new DTMs) each time it
 * was called. XPathAPIObject instead retains its context as long as the object
 * persists, reusing the DTMs. This does have a downside: if you've changed your
 * source document, you should obtain a new XPathAPIObject to continue searching
 * it, since trying to use the old DTMs will probably yield bad results or
 * malfunction outright... and the cached DTMs may consume memory until this
 * object and its context are returned to the heap. Essentially, it's the
 * caller's responsibility to decide when to discard the cache.
 *
 * @see <a href="http://www.w3.org/TR/xpath">XPath Specification</a>
 */
public class CachedXPathAPI {
    /**
     * XPathContext, and thus the document model system (DTMs), persists through
     * multiple calls to this object. This is set in the constructor.
     */
     ...
     * its own {@link com.sun.org.apache.xml.internal.dtm.DTMManager}. Good
     * choice for simple uses.
     * </p>
     * <p>
     * Note that any particular instance of {@link CachedXPathAPI} must not be
     * operated upon by multiple threads without synchronization; we do not
     * currently support multithreaded access to a single
     ...
     * {@link com.sun.org.apache.xml.internal.dtm.DTM}) and previously
     * established location state.
     * </p>
     * <p>
     * Note that the original {@link CachedXPathAPI} and the new one should not
     * be operated upon concurrently; we do not support multithreaded access to
     * a single {@link com.sun.org.apache.xml.internal.dtm.DTM} at this time.
     * Similarly, any particular instance of {@link CachedXPathAPI} must not be
     * operated upon by multiple threads without synchronization.
     * </p>
     * <p>
     ...
     * %REVIEW% I'm somewhat concerned about the loss of encapsulation this
     * causes, but the xml-security folks say they need it.
     */
    ...
    public XObject eval(Node contextNode, String str, Node namespaceNode) throws TransformerException {
        ...
        PrefixResolverDefault prefixResolver = new PrefixResolverDefault((namespaceNode
                .getNodeType() == Node.DOCUMENT_NODE) ? ((Document) namespaceNode).getDocumentElement()
                        : namespaceNode);
