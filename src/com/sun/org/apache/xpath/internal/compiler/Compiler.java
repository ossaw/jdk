...
public class Compiler extends OpMap {
    ...
    public Expression compile(int opPos) throws TransformerException {
        ...
        switch (op) {
            ...
            case OpCodes.OP_QUO:
                ...
                break;
            default:
                error(XPATHErrorResources.ER_UNKNOWN_OPCODE, new Object[] { Integer.toString(getOp(opPos)) }); // "ERROR!
                                                                                                               // Unknown
                                                                                                               // op
                                                                                                               // code:
                                                                                                               // "+m_opMap[opPos]);
    ...
    protected StepPattern stepPattern(int opPos, int stepCount, StepPattern ancestorPattern)
        ...
        switch (stepType) {
            ...
            case OpCodes.MATCH_ATTRIBUTE:
                ...
                argLen = getArgLengthOfStep(opPos);
                opPos = getFirstChildPosOfStep(opPos);
                pattern = new StepPattern(DTMFilter.SHOW_ATTRIBUTE, getStepNS(startOpPos), getStepLocalName(
                        startOpPos), Axis.PARENT, Axis.ATTRIBUTE);
                break;
            case OpCodes.MATCH_ANY_ANCESTOR:
                if (DEBUG)
                ...
                if (0x00000500 == what)
                    addMagicSelf = false;
                pattern = new StepPattern(getWhatToShow(startOpPos), getStepNS(startOpPos), getStepLocalName(
                        startOpPos), Axis.ANCESTOR, Axis.CHILD);
                break;
            case OpCodes.MATCH_IMMEDIATE_ANCESTOR:
                if (DEBUG)
                ...
                opPos = getFirstChildPosOfStep(opPos);
                pattern = new StepPattern(getWhatToShow(startOpPos), getStepNS(startOpPos), getStepLocalName(
                        startOpPos), Axis.PARENT, Axis.CHILD);
                break;
            default:
        ...
        if (null == ancestorPattern) {
            ...
            // which is really how we're treating match patterns, it works out
            // to
             ...
             * Axis.CHILD); // We need to keep the new nodetest from affecting
             * the score... XNumber score = pattern.getStaticScore();
    ...
    Expression compileFunction(int opPos) throws TransformerException {
        ...
        if (-1 != funcID) {
            ...
            /**
             * It is a trick for function-available. Since the function table is
             * an instance field, insert this table at compilation time for
             * later usage
             */
            ...
            try {
                ...
                m_errorHandler.fatalError(new TransformerException(XSLMessages.createXPATHMessage(
            ...
            error(XPATHErrorResources.ER_FUNCTION_TOKEN_NOT_FOUND, null); // "function
                                                                          // token
                                                                          // not
                                                                          // found.");
    ...
    public void warn(String msg, Object[] args) throws TransformerException {
        ...
        if (null != m_errorHandler) {
            ...
            System.out.println(fmsg + "; file " + m_locator.getSystemId() + "; line " + m_locator
                    .getLineNumber() + "; column " + m_locator.getColumnNumber());
        }
    }
