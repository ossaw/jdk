...
public class OpMap {
    ...
    /**
     * TokenStack is the queue of used tokens. The current token is the token at
     ...
     * operations codes and indexes into the m_tokenQueue. I use an array
     ...
     * objects created.
     *
     * @return An IntVector that is the opcode list that describes the XPath
     *         operations.
     */
     ...
     * as the opcode+length bytes, so it is always 2 or greater.
     */
    ...
    /**
     * Given an operation position, return the end position, i.e. the beginning
    ...
    public int getNextStepPos(int opPos) {
        ...
        if ((stepType >= OpCodes.AXES_START_TYPES) && (stepType <= OpCodes.AXES_END_TYPES)) {
            ...
            throw new RuntimeException(XSLMessages.createXPATHMessage(XPATHErrorResources.ER_UNKNOWN_STEP,
    ...
    /**
     * Given an FROM_stepType position, return the position of the first
     * predicate, if there is one, or else this will point to the end of the
     * FROM_stepType. Example: int posOfPredicate = xpath.getNextOpPos(stepPos);
     * boolean hasPredicates = OpCodes.OP_PREDICATE ==
    ...
    public int getFirstPredicateOpPos(int opPos) throws javax.xml.transform.TransformerException {
        ...
        if ((stepType >= OpCodes.AXES_START_TYPES) && (stepType <= OpCodes.AXES_END_TYPES)) {
            ...
            error(com.sun.org.apache.xpath.internal.res.XPATHErrorResources.ER_UNKNOWN_OPCODE, new Object[] {
                    String.valueOf(stepType) }); // "ERROR!
                                                                                                                                                   // Unknown op
                                                                                                                                                   // code:
                                                                                                                                                   // "+m_opMap[opPos]);
            return -1;
    ...
    public void error(String msg, Object[] args) throws javax.xml.transform.TransformerException {
        ...
        java.lang.String fmsg = com.sun.org.apache.xalan.internal.res.XSLMessages.createXPATHMessage(msg,
                args);
    ...
    public int getStepTestType(int opPosOfStep) {
        return m_opMap.elementAt(opPosOfStep + 3); // skip past op, len, len
                                                   // without predicates
    }
