...
public class XPathContext extends DTMManager // implements ExpressionContext
{
	...
	/**
	 * Stack of cached "reusable" DTMs for Result Tree Fragments. This is a
	 * kluge to handle the problem of starting an RTF before the old one is
	 * complete.
	 *
	 ...
	 * else. (I think more parameters will need to be added for error handling,
	 * and entity resolution).
	 *
	 * @param source
	 *                    the specification of the source object, which may be
	 *                    null, in
	 *                    which case it is assumed that node construction will
	 *                    take by
	 *                    some other means.
	 * @param unique
	 ...
	 *                    null.
	 * @param incremental
	 *                    true if the construction should try and be
	 *                    incremental.
	 * @param doIndexing
	...
	public DTM getDTM(javax.xml.transform.Source source, boolean unique,
			DTMWSFilter wsfilter, boolean incremental, boolean doIndexing) {
	...
	public DTMIterator createDTMIterator(int whatToShow, DTMFilter filter,
		...
		return m_dtmManager.createDTMIterator(whatToShow, filter,
	...
	public DTMIterator createDTMIterator(int node) {
		...
		DTMIterator iter = new com.sun.org.apache.xpath.internal.axes.OneStepIteratorForward(
				Axis.SELF);
	...
	public XPathContext(Object owner) {
		m_owner = owner;
		try {
			m_ownerGetErrorListener = m_owner.getClass().getMethod(
					"getErrorListener", new Class[] {});
	...
	public void reset() {
		...
		if (m_rtfdtm_stack != null)
			for (java.util.Enumeration e = m_rtfdtm_stack.elements(); e
					.hasMoreElements();)
	...
	/**
	 * The owner context of this XPathContext. In the case of XSLT, this will be
	 ...
	 * XSLT, the Transformer object. This is needed so that XSLT functions can
	 * get the Transformer.
	 * 
	...
	public final ErrorListener getErrorListener() {
		...
		try {
			if (null != m_ownerGetErrorListener)
				retval = (ErrorListener) m_ownerGetErrorListener.invoke(m_owner,
						new Object[] {});
		} catch (Exception e) {
		}
	...
	public void setErrorListener(ErrorListener listener)
		...
		if (listener == null)
			throw new IllegalArgumentException(XSLMessages.createXPATHMessage(
					XPATHErrorResources.ER_NULL_ERROR_HANDLER, null)); // "Null
																																			// error
																																			// handler");
	...
	/**
	 * The TrAX URI Resolver for resolving URIs from the document(...) function
	 * to source tree nodes.
	 */
	 ...
	 *
	 * @param resolver
	 *                 the URIResolver to be associated with this execution
	 *                 context,
	 *                 may be null to clear an already set resolver.
	 */
	 ...
	 *
	 * @return the
	 *         <a href="http://www.w3.org/TR/xslt#dt-current-node-list">current
	 *         node list</a>, also referred to here as a <term>context node
	 *         list</term>.
	 */
	 ...
	 *
	 * @param nl
	 *           the <a href="http://www.w3.org/TR/xslt#dt-current-node-list">
	 *           current node list</a>, also referred to here as a
	 *           <term>context node list</term>.
	 * @xsl.usage internal
	 */
	...
	/**
	 * The amount to use for stacks that record information during the recursive
	 * execution.
	 */
	 ...
	 * Note that there are no bounds check and resize for this stack, so if it
	 * is blown, it's all over.
	 */
	 ...
	 * @param cn
	 *           the
	 *           <a href="http://www.w3.org/TR/xslt#dt-current-node">current
	 *           node</a>.
	 * @param en
	...
	public SubContextList getSubContextList() {
		return m_axesIteratorStack.isEmpty() ? null
				: (SubContextList) m_axesIteratorStack.peek();
	}
	...
	public final DTMIterator getContextNodes() {
		...
		try {
			...
			if (null != cnl)
				...
				return null; // for now... this might ought to be an empty
								// iterator.
	...
	public class XPathExpressionContext implements ExpressionContext {
		 ...
		 * processing requirements cannot be met another way. Consider
		 * requesting an enhancement to the ExpressionContext interface to avoid
		 ...
		 * Return the DTMManager object. Though XPathContext context extends the
		 * DTMManager, it really is a proxy for the real DTMManager. If a caller
		 * needs to make a lot of calls to the DTMManager, it is faster if it
		 * gets the real one from this function.
		 */
		public DTMManager getDTMManager() {
			return m_dtmManager;
		}
	...
	public DTM getGlobalRTFDTM() {
		...
		// If it doesn't exist, or if the one already existing is in the middle
		// of
		// being constructed, we need to obtain a new DTM to write into. I'm not
		// sure
		// the latter will ever arise, but I'd rather be just a bit paranoid..
		if (m_global_rtfdtm == null || m_global_rtfdtm.isTreeIncomplete()) {
			m_global_rtfdtm = (SAX2RTFDTM) m_dtmManager.getDTM(null, true, null,
					false, false);
		}
	...
	public DTM getRTFDTM() {
		...
		if (m_rtfdtm_stack == null) {
			...
			rtfdtm = (SAX2RTFDTM) m_dtmManager.getDTM(null, true, null, false,
					false);
			...
			// It might already be under construction -- the classic example
			// would be
			// an xsl:variable which uses xsl:call-template as part of its
			// value. To
			// handle this recursion, we have to start a new RTF DTM, pushing
			// the old
			// one onto a stack so we can return to it. This is not as uncommon
			// a case
			// as we might wish, unfortunately, as some folks insist on coding
			// XSLT
			// as if it were a procedural language...
			if (rtfdtm.isTreeIncomplete()) {
				if (++m_which_rtfdtm < m_rtfdtm_stack.size())
					rtfdtm = (SAX2RTFDTM) m_rtfdtm_stack.elementAt(
							m_which_rtfdtm);
				else {
					rtfdtm = (SAX2RTFDTM) m_dtmManager.getDTM(null, true, null,
							false, false);
					m_rtfdtm_stack.addElement(rtfdtm);
				}
			}
	 ...
	 * this point. (If it doesn't exist we don't push, since we might still be
	 * able to get away with not creating it. That requires that excessive pops
	 * be harmless.)
	 */
	 ...
	 * last mark was set.
	 *
	 * If there is no RTF DTM, there's nothing to pop so this becomes a no-op.
	 * If pushes were issued before this was called, we count on the fact that
	 ...
	 * previous RTF DTM, if one of the weird produce-an-RTF-to-build-an-RTF
	 * cases arose. Basically: If pop says this DTM is now empty, then return to
	 * the previous if one exists, in whatever state we left it in. UGLY, but
	...
	public void popRTFContext() {
		...
		if (m_which_rtfdtm == previous) {
			...
			{
				boolean isEmpty = ((SAX2RTFDTM) (m_rtfdtm_stack.elementAt(
						previous))).popRewindMark();
			}
		} else
			while (m_which_rtfdtm != previous) {
				// Empty each DTM before popping, so it's ready for reuse
				// _DON'T_ pop the previous, since it's still open (which is why
				// we
				// stacked up more of these) and did not receive a mark.
				boolean isEmpty = ((SAX2RTFDTM) (m_rtfdtm_stack.elementAt(
						m_which_rtfdtm))).popRewindMark();
				--m_which_rtfdtm;
			}
	}
	...
	/**
	 * Cleans DTMXRTreeFrag objects by removing references to DTM and
