 ...
 * However, in order for it to act as a NodeVector or NodeList, it's required
 * that setShouldCacheNodes(true) be called before the first nextNode() is
 * called, in order that nodes can be added as they are fetched. Derived classes
 * that implement iterators must override runTo(int index), in order that they
 * may run the iteration to the given index.
 * </p>
 *
 * <p>
 ...
 * there be specific subclasses of it which do so? The advantage of doing it all
 * here is that all NodeSets will respond to the same calls; the disadvantage is
 * that some of them may return less-than-enlightening results when you do so.
 * </p>
 * 
 * @xsl.usage advanced
 */
public class NodeSet implements NodeList, NodeIterator, Cloneable,
     ...
     * iteration.
     *
     * @return a new NodeSet of the same type, having the same state... except
     ...
     *         DOM's NodeFilter class. The value will be
     ...
     * reduce (and restructure) the NodeIterator's view of the document. In our
     * case, we will be using hardcoded filters built into our iterators... but
     * getFilter() is part of the DOM's NodeIterator interface, so we have to
     * support it.
     *
     ...
     * that is called will return the first node in the set.
     *
    ...
    /**
     * If an index is requested, NodeSet will call this method to run the
    ...
    /**
     * <p>
     * Copy NodeList members into this nodelist, adding in document order. Only
     * genuine node references will be copied; nulls appearing in the source
     * NodeSet will not be added to this one.
     * </p>
     *
     * <p>
     * In case you're wondering why this function is needed: NodeSet implements
     * both NodeIterator and NodeList. If this method isn't provided, Java can't
     * decide which of those to use when addNodes() is invoked. Providing the
     * more-explicit match avoids that ambiguity.)
     * </p>
     *
     * @param ns
    ...
    /**
     * Pop a node from the tail of the vector and return the top of the stack
     * after the pop.
     *
    ...
    /**
     * Pop a pair of nodes from the tail of the stack. Special purpose method
     * for TransformerImpl, pushElemTemplateElement. Performance critical.
     */
    ...
    /**
     * Set the tail of the stack to the given node. Special purpose method for
     * TransformerImpl, pushElemTemplateElement. Performance critical.
     *
     * @param n
    ...
    /**
     * Set the given node one position from the tail. Special purpose method for
     * TransformerImpl, pushElemTemplateElement. Performance critical.
     *
     * @param n
     *          Node to set
    ...
    /**
     * Return the node at the tail of the vector without popping Special purpose
     * method for TransformerImpl, pushElemTemplateElement. Performance
     * critical.
     *
     ...
     * purpose method for TransformerImpl, pushElemTemplateElement. Performance
     * critical.
     *
