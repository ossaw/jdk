...
/**
 * <p>
 * The NodeSetDTM class can act as either a NodeVector, NodeList, or
 * NodeIterator. However, in order for it to act as a NodeVector or NodeList,
 * it's required that setShouldCacheNodes(true) be called before the first
 * nextNode() is called, in order that nodes can be added as they are fetched.
 * Derived classes that implement iterators must override runTo(int index), in
 * order that they may run the iteration to the given index.
 * </p>
 * <p>
 * Note that we directly implement the DOM's NodeIterator interface. We do not
 * emulate all the behavior of the standard NodeIterator. In particular, we do
 * not guarantee to present a "live view" of the document ... but in XSLT, the
 * source document should never be mutated, so this should never be an issue.
 * </p>
 * <p>
 * Thought: Should NodeSetDTM really implement NodeList and NodeIterator, or
 * should there be specific subclasses of it which do so? The advantage of doing
 * it all here is that all NodeSetDTMs will respond to the same calls; the
...
public class NodeSetDTM extends NodeVector implements /*
    ...
    /**
     * Create a NodeSetDTM, and copy the members of the given NodeSetDTM into
     * it.
     *
     * @param nodelist
    ...
    /**
     * Create a NodeSetDTM, and copy the members of the given DTMIterator into
     * it.
     *
     * @param ni
    ...
    public NodeSetDTM(NodeList nodeList, XPathContext xctxt) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            int handle = xctxt.getDTMHandleFromNode(node);
            // Do not reorder or strip duplicate nodes from the given DOM
            // nodelist
     ...
     * provide: a node (the context node... same value as "root" defined below)
    ...
    /**
     * Clone this NodeSetDTM. At this time, we only expect this to be used with
     * LocPathIterators; it may not work with other kinds of NodeSetDTMs.
     *
     ...
     * hardcoded to show all nodes except EntityReference nodes, which have no
     * equivalent in the XPath data model.
     *
     * @return integer used as a bit-array, containing flags defined in the
     *         DOM's DTMFilter class. The value will be
    ...
    /**
     * The filter object used to screen nodes. Filters are applied to further
     * reduce (and restructure) the DTMIterator's view of the document. In our
     * case, we will be using hardcoded filters built into our iterators... but
     * getFilter() is part of the DOM's DTMIterator interface, so we have to
     * support it.
     *
     ...
     *         returning a placeholder object just to indicate that this is not
     *         going to return all nodes selected by whatToShow.
     */
     ...
     * does not expose the entity reference node itself, use the whatToShow
     ...
     *         contents of EntityRefrence nodes may be returned (though
     *         whatToShow says that the EntityReferences themselves are not
     *         shown.)
     */
     ...
     * may be passed without a DTMManager, this allows the caller to easily get
    ...
    /**
     * Get an instance of the DTMManager. Since a node iterator may be passed
     * without a DTMManager, this allows the caller to easily get the DTMManager
     ...
     * iterator in the set. After a DTMIterator is created, the first call to
    ...
    /**
     * Tells if this NodeSetDTM is "fresh", in other words, if the first
     * nextNode() that is called will return the first node in the set.
     *
     * @return true if nextNode() would return the first node in the set, false
     *         if it would return a later one.
     */
    ...
    /**
     * If an index is requested, NodeSetDTM will call this method to run the
     * iterator to the index. By default this sets m_next to the index. If the
     * index argument is -1, this signals that the iterator should be run to the
     * end.
     *
     * @param index
     ...
     * finding all the matching nodes, which may defeat attempts to defer that
     * work.
     *
    ...
    // * ambiguity.)</p>
    // *
    // * @param ns NodeSetDTM whose members should be merged into this
    // NodeSetDTM.
    // * @throws RuntimeException thrown if this NodeSetDTM is not of
    // * a mutable type.
    // */
    ...
    /**
     * Copy NodeList members into this nodelist, adding in document order. Null
    ...
    /**
     * Copy NodeList members into this nodelist, adding in document order. If a
     ...
     * index is shifted upward to have an index one greater than the value it
     * had previously.
     *
     * @param value
    ...
    /**
     * Get the current position, which is one less than the next nextNode() call
     * will retrieve. i.e. if you call getCurrentPos() and the return is 0, the
     * next fetch will take place at index 1.
     *
    ...
    /**
     * If setShouldCacheNodes(true) is called, then nodes will be cached. They
     * are not cached by default. This switch must be set before the first call
    ...
    /**
     * Tells if this iterator can have nodes added to it or set via the
     * <code>setItem(int node, int index)</code> method.
     *
    ...
    /**
     * Returns true if all the nodes in the iteration well be returned in
     * document order.
     *
     ...
     *
     * @return Axis.CHILD, etc., or -1 if the axis is not known or is of
     *         multiple types.
     */
    public int getAxis() {
        return -1;
