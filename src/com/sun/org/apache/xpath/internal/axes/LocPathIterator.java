 ...
 * can easily be done, except in the case where the LocPathIterator is "owned"
...
public abstract class LocPathIterator extends PredicatedNodeTest implements
	...
	/**
	 * Create a LocPathIterator object, including creation of step walkers from
	...
	/**
	 * Execute this iterator, meaning create a clone that can store state, and
	 * initialize it for fast execution from the current runtime state. When
	 * this is called, no actual query from the current context node is
	 * performed.
	 *
	 * @param xctxt
	 ...
	 * make sure we have found everything. This should be reviewed.
	 * </p>
	 *
	...
	/**
	 * The filter used to screen nodes. Not used at this time, this is here only
	 ...
	 *
	 * @return The "root" of this iterator, which, in XPath terms, is the node
	 *         context for this iterator.
	 */
	public int getRoot() {
		return m_context;
	}
	 ...
	 * false.
	 *
	 * @return Always true, since entity reference nodes are not visible in the
	 *         XPath model.
	 */
	...
	/**
	 * Bottleneck the return of a next node, to make returns easier from
	 * nextNode().
	 *
	 * @param nextNode
	...
	/**
	 * The pool for cloned iterators. Iterators need to be cloned because the
	 * hold running state, and thus the original iterator expression from the
	 * stylesheet pool can not be used.
	 */
	transient protected IteratorPool m_clones = new IteratorPool(this);

	/**
	 * The dtm of the context node. Careful about using this... it may not be
	...
	/**
	 * Value determined at compile time, indicates that this is an iterator at
	 * the top level of the expression, rather than inside a predicate.
	 * 
	 * @serial
	 */
	...
	/**
	 * The context node for this iterator, which doesn't change through the
	...
	/**
	 * The node context from where the expression is being executed from (i.e.
	 * for current() support). Different from m_context in that this is the
	 * context for the entire expression, rather than the context for the
	 * subexpression.
	 */
	...
	/**
	 * Fast access to the current prefix resolver. It isn't really clear that
	 * this is needed.
	 * 
	 * @serial
	 */
