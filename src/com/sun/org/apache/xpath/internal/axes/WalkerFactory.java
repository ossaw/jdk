...
/**
 * This class is both a factory for XPath location path expressions, which are
 * built from the opcode map output, and an analysis engine for the location
 * path expressions in order to provide optimization hints.
 */
public class WalkerFactory {

	/**
	 * This method is for building an array of possible levels where the target
	...
	/**
	 * Special purpose function to see if we can optimize the pattern for a
	 * DescendantIterator.
	 *
	 * @param compiler
	 *                      non-null reference to compiler object that has
	 ...
	 * better one, please do tell. This really has to do with inverting
	 * attribute axis.
	 * 
	 * @param axis
	 *             One of Axis.XXX.
	 ...
	 * generalized match pattern. What this means is that the LocationPath is
	 * read backwards, as a test on a given node, to see if it matches the
	 ...
	 * "self::node()/following-sibling::foo/child::daz[position()=2]". Taking
	 * this as a match pattern for a probable node, it works out to
	 ...
	 * the location path have to be executed by the following step, because they
	...
	static StepPattern loadSteps(MatchPatternIterator mpi, Compiler compiler,
		...
		for (StepPattern pat = step; null != pat; pat = pat
			...
			// If we don't do the magic below, then "@*/ancestor-or-self::*"
			// gets
			...
			// treatment
			// in the rewrite. The following inverted match will still not
			// select
			// anything.
			// select: "@*/child::*"
			...
			if (whatToShow == DTMFilter.SHOW_ATTRIBUTE
				...
				if (isDownwardAxisOfMany(axis)) {
					...
					// This is needed to inverse a following pattern, because of
					// the
					// wacky Xalan rules for following from an attribute. See
					// axes108.
					// By these rules, following from an attribute is not
					// strictly
					// inverseable.
	...
	private static AxesWalker createDefaultWalker(Compiler compiler, int opPos,
		...
		/*
		 * System.out.println("0: "+compiler.getOp(opPos)); System.out.println(
		 * "1: "+compiler.getOp(opPos+1)); System.out.println("2: "
		 * +compiler.getOp(opPos+2)); System.out.println("3: "
		 * +compiler.getOp(opPos+3)); System.out.println("4: "
		 * +compiler.getOp(opPos+4)); System.out.println("5: "
		 * +compiler.getOp(opPos+5));
		 */
		boolean simpleInit = false;
		...
		if (simpleInit) {
			 ...
			 * NodeTest.debugWhatToShow(whatToShow); System.out.println(
			 * "or stuff: "+(whatToShow & (DTMFilter.SHOW_ATTRIBUTE |
	...
	private static boolean isNaturalDocOrder(Compiler compiler,
		...
		// Namespaces can present some problems, so just punt if we're looking
		// for
		// these.
		if (isSet(analysis, BIT_NAMESPACE))
			return false;

		// The following, preceding, following-sibling, and preceding sibling
		// can
	...
	/**
	 * First 8 bits are the number of top-level location steps. Hopefully there
	 * will never be more that 255 location steps!!!
	 */
	...
	/**
	 * Bit is on if any of the walkers can go backwards in document order from
	 * the context node.
	 */
	...
	/**
	 * Bit is on if any of the walkers contain an node() test. This is really
	 * only useful if the count is 1.
	 */
