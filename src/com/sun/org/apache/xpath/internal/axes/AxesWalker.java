...
public class AxesWalker extends PredicatedNodeTest implements Cloneable,
	...
	/**
	 * Do a deep clone of this walker, including next and previous walkers. If
	 * the this AxesWalker is on the clone list, don't clone but return the
	 ...
	 * filter. currentNode may also be explicitly set to any node, whether or
	 * not it is within the subtree specified by the root node or would be
	 * accepted by the filter and whatToShow flags. Further traversal occurs
	 * relative to currentNode even if it is not part of the current view by
	 * applying the filters in the requested direction (not changing currentNode
	 * where no traversal is possible).
	 *
	 * @return The node at which the TreeWalker is currently positioned, only
	 *         null if setRoot has not yet been called.
	 */
	 ...
	 * document order. Warning: This can only be called after setRoot has been
	 * called!
	 *
