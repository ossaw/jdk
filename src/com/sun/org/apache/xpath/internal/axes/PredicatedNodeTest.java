...
public abstract class PredicatedNodeTest extends NodeTest implements SubContextList {
	 ...
	 * one would think, as it creates a new predicate array of the size of the
	 * count argument, and copies count predicates into the new one from the
	 * old, and then reassigns the predicates value. All this to keep from
	 * having to have a predicate count value.
	 *
	 * @param count
	 *            The number of predicates, which must be equal or less than the
	 *            existing count.
	 */
	 ...
	 *            The index of the predicate where the proximity should be taken
	 *            from.
	 *
	 * @return The node position of this walker in the sub-context node list.
	 */
	...
	boolean executePredicates(int context, XPathContext xctxt)
		...
		try {
			...
			for (int i = 0; i < nPredicates; i++) {
				...
				// System.out.println("\nBack from executing predicate
				// expression - waiting count: "+m_lpi.getWaitingCount());
				...
				if (XObject.CLASS_NUMBER == pred.getType()) {
					...
					// the
					// course of itteration, then we know there can be no more
					// true
					// occurances of this predicate, so flag that we're done
					// after
					// this.
					//
					// bugzilla 14365
					// We can't set m_foundLast = true unless we're sure that
					// -all-
	...
	/**
	 * The list of predicate expressions. Is static and does not need to be deep
	 * cloned.
	 * 
	 * @serial
	 */
	...
	/**
	 * An array of counts that correspond to the number of predicates the step
	 * contains.
	 */
