...
/**
 * This class defines a simplified type of union iterator that only tests along
 * the child axes. If the conditions are right, it is much faster than using a
 * UnionPathIterator.
 */
public class UnionChildIterator extends ChildTestIterator {
	static final long serialVersionUID = 3500298482193003495L;
	/**
	 * Even though these may hold full LocPathIterators, this array does not
	 * have to be cloned, since only the node test and predicate portion are
	 * used, and these only need static information. However, also note that
	 * index predicates can not be used!
	 */
		...
		try {
			...
			for (int i = 0; i < m_nodeTests.length; i++) {
				...
				if (score != NodeTest.SCORE_NONE) {
					// Note that we are assuming there are no positional
					// predicates!
