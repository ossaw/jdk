...
/**
 * This class extends NodeSetDTM, which implements DTMIterator, and fetches
 * nodes one at a time in document order based on a XPath
 * <a href="http://www.w3.org/TR/xpath#NT-UnionExpr">UnionExpr</a>. As each node
 * is iterated via nextNode(), the node is also stored in the NodeVector, so
 * that previousNode() can easily be done.
 * 
 * @xsl.usage advanced
 */
public class UnionPathIterator extends LocPathIterator implements Cloneable,
     ...
     * iterators from the opcode list, and call back into the Compiler to create
    ...
    /**
     * This will return an iterator capable of handling the union of paths
     * given.
     *
     * @param compiler
     *                 The Compiler which is creating this expression.
    ...
    public static LocPathIterator createUnionIterator(Compiler compiler,
        ...
        for (int i = 0; i < nPaths; i++) {
            ...
            if (lpi.getAxis() != Axis.CHILD) {
                ...
                // check for positional predicates or position function, which
                // won't work.
    ...
    /**
     * Get a cloned LocPathIterator that holds the same position as this
     * iterator.
     *
     ...
     * <a href="http://www.w3.org/TR/xpath#NT-LocationPath">location path</a>
     * contained in the union expression.
     * 
     * @serial
     */
