...
public class NodeSequence extends XObject implements DTMIterator, Cloneable, PathComponent {
    ...
    /**
     * The index of the next node to be fetched. Useful if this is a cached
     * iterator, and is being used as random access NodeList.
     */
    protected int m_next = 0;

    /**
     * A cache of a list of nodes obtained from the iterator so far. This list
     * is appended to until the iterator is exhausted and the cache is complete.
     * <p>
    ...
    /**
     * If this iterator needs to cache nodes that are fetched, they are stored
     * in the Vector in the generic object.
     */
     ...
     * that the cache keeps growing until the iterator is walked to exhaustion,
    ...
    /**
     * If the iterator needs to cache nodes as they are fetched, then this
     * method returns true.
     */
    ...
    /**
     * If this NodeSequence has a cache, and that cache is fully populated then
     * this method returns true, otherwise if there is no cache or it is not
    ...
    /**
     * If this NodeSequence has a cache, mark that it is complete. This method
     * should be called after the iterator is exhausted.
     */
    ...
    /**
     * The DTMManager to use if we're using a NodeVector only. We may well want
     * to do away with this, and store it in the NodeVector.
     */
    ...
    public int getRoot() {
        if (null != m_iter)
            ...
            // an assertion here.
            // assertion(false, "Can not get the root from a non-iterated
            // NodeSequence!");
            return DTM.NULL;
        }
    }
    ...
    public boolean isMutable() {
        return hasCache(); // though may be surprising if it also has an
                           // iterator!
    }
    ...
    public void setItem(int node, int index) {
        ...
        if (null != vec) {
            ...
            if (oldNode != node && m_cache.useCount() > 1) {
                /*
                 * If we are going to set the node at the given index to a
                 * different value, and the cache is shared (has a use count
                 * greater than 1) then make a copy of the cache and use it so
                 * we don't overwrite the value for other users of the cache.
                 */
                ...
                /*
                 * When we get to here the new cache has a use count of 1 and
                 * when setting a bunch of values on the same NodeSequence, such
                 * as when sorting, we will keep setting values in that same
                 * copy which has a use count of 1.
                 */
            }
     ...
     * process, so that it may be used from the current position. Note: Not a
     * deep clone.
     *
    ...
    /**
     * It used to be that many locations in the code simply did an assignment to
     * this.m_obj directly, rather than calling the setObject(Object) method.
     * The problem is that our super-class would be updated on what the cache
     * associated with this NodeSequence, but we wouldn't know ourselves.
     * <p>
    ...
    /**
     * Each NodeSequence object has an iterator which is "walked". As an
     * iterator is walked one obtains nodes from it. As those nodes are obtained
     * they may be cached, making the next walking of a copy or clone of the
     * iterator faster. This field (m_cache) is a reference to such a cache,
     * which is populated as the iterator is walked.
     * <p>
     * Note that multiple NodeSequence objects may hold a reference to the same
     * cache, and also (and this is important) the same iterator. The iterator
     * and its cache may be shared among many NodeSequence objects.
     * <p>
     * If one of the NodeSequence objects walks ahead of the others it fills in
     * the cache. As the others NodeSequence objects catch up they get their
     * values from the cache rather than the iterator itself, so the iterator is
     * only ever walked once and everyone benefits from the cache.
     * <p>
     * At some point the cache may be complete due to walking to the end of one
     * of the copies of the iterator, and the cache is then marked as
     * "complete". and the cache will have no more nodes added to it.
     * <p>
     * Its use-count is the number of NodeSequence objects that use it.
     */
    private final static class IteratorCache {
        /**
         * A list of nodes already obtained from the iterator. As the iterator
         * is walked the nodes obtained from it are appended to this list.
         * <p>
         * Both an iterator and its corresponding cache can be shared by
         * multiple NodeSequence objects.
         * <p>
         * For example, consider three NodeSequence objects ns1, ns2 and ns3
         * doing such sharing, and the nodes to be obtaind from the iterator
         * being the sequence { 33, 11, 44, 22, 55 }.
         * <p>
         * If ns3.nextNode() is called 3 times the the underlying iterator will
         * have walked through 33, 11, 55 and these three nodes will have been
         * put in the cache.
         * <p>
         * If ns2.nextNode() is called 2 times it will return 33 and 11 from the
         * cache, leaving the iterator alone.
         * <p>
         * If ns1.nextNode() is called 6 times it will return 33 and 11 from the
         * cache, then get 44, 22, 55 from the iterator, and appending 44, 22,
         * 55 to the cache. On the sixth call it is found that the iterator is
         * exhausted and the cache is marked complete.
         * <p>
         * Should ns2 or ns3 have nextNode() called they will know that the
         * cache is complete, and they will obtain all subsequent nodes from the
         * cache.
         * <p>
         * Note that the underlying iterator, though shared is only ever walked
         * once.
         */
        private NodeVector m_vec2;

        /**
         * true if the associated iterator is exhausted and all nodes obtained
         * from it are in the cache.
         */
        ...
        /**
         * Sets the NodeVector that holds the growing list of nodes as they are
        ...
        /**
         * Call this method with 'true' if the iterator is exhausted and the
         * cached list is complete, or no longer growing.
         */
     ...
     * iterator as a NodeSequence is walked when its nextNode() method is
     * called.
     */
