...
/**
 * A partially decoded representation of an SNMP packet. It contains the
 * information contained in any SNMP message (SNMPv1, SNMPv2 or SNMPv3).
 * <p>
...
public abstract class SnmpMsg implements SnmpDefinitions {
	 ...
	 * <P>
	 * <CODE>decodeMessage</CODE> and <CODE>encodeMessage</CODE> do not perform
	 * any check on this value. <BR>
	 * <CODE>decodeSnmpPdu</CODE> and <CODE>encodeSnmpPdu</CODE> only accept the
	 * values 0 (for SNMPv1), 1 (for SNMPv2) and 3 (for SNMPv3).
	 */
	 ...
	 * Encoding of the PDU.
	 * <P>
	 * This is usually the BER encoding of the PDU's syntax defined in RFC1157
	 * and RFC1902. However, this can be authenticated or encrypted data (but
	 * you need to implemented your own <CODE>SnmpPduFactory</CODE> class).
	 */
	...
	/**
	 * Source or destination address. <BR>
	 * For an incoming message it's the source. For an outgoing message it's the
	 * destination.
	 */
	public InetAddress address = null;

	/**
	 * Source or destination port. <BR>
	 * For an incoming message it's the source. For an outgoing message it's the
	 * destination.
	 */
	public int port = 0;
	/**
	 * Security parameters. Contain informations according to Security Model
	 * (Usm, community string based, ...).
	 */
	 ...
	 * <CODE>maxDataLength</CODE> bytes. It encodes the <CODE>pdu</CODE>. The
	 * resulting encoding is stored in the data field and the length of the
	 * encoding is stored in <CODE>dataLength</CODE>.
	 * <p>
	 * If the encoding length exceeds <CODE>maxDataLength</CODE>, the method
	 * throws an exception.
	 *
	 * @param pdu
		...
		switch (tag) {
			...
			case SnmpValue.IpAddressTag:
				try {
					...
					throw new BerException();
					// BerException("Can't build SnmpIpAddress from decoded
					// value.");
				}
				break;
			...
			case SnmpValue.TimeticksTag:
				try {
					...
					throw new BerException();
					// BerException("Can't build SnmpTimeticks from decoded
					// value.");
				}
				break;
			...
			case SnmpValue.Counter64Tag:
				...
				try {
					...
					throw new BerException();
					// BerException("Can't build SnmpCounter64 from decoded
					// value.");
				}
				break;
