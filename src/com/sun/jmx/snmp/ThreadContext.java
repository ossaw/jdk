 ...
 * stack, typically with code like this:
 * </p>
 *
 * <pre>
...
public class ThreadContext implements Cloneable {
	...
	/*
	 * The context of a thread is stored as a linked list. At the head of the
	 * list is the value returned by localContext.get(). At the tail of the list
	 * is a sentinel ThreadContext value with "previous" and "key" both null.
	 * There is a different sentinel object for each thread.
	 * 
	 * Because a null key indicates the sentinel, we reject attempts to push
	 * context entries with a null key.
	 * 
	 * The reason for using a sentinel rather than just terminating the list
	 * with a null reference is to protect against incorrect or even malicious
	 * code. If you have a reference to the sentinel value, you can erase the
	 * context stack. Only the caller of the first "push" that put something on
	 * the stack can get such a reference, so if that caller does not give this
	 * reference away, no one else can erase the stack.
	 * 
	 * If the restore method took a null reference to mean an empty stack,
	 * anyone could erase the stack, since anyone can make a null reference.
	 * 
	 * When the stack is empty, we discard the sentinel object and have
	 * localContext.get() return null. Then we recreate the sentinel object on
	 * the first subsequent push.
	 * 
	 * ThreadContext objects are immutable. As a consequence, you can give a
	 * ThreadContext object to setInitialContext that is no longer current. But
	 * the interface says this can be rejected, in case we remove immutability
	 * later.
	 */
	...
	/**
	 * <p>
	 * Check whether a value with the given key exists in the stack. This means
	 * that the <code>push</code> method was called with this key and it was not
	 * cancelled by a subsequent <code>restore</code>. This method is useful
	 * when the <code>get</code> method returns null, to distinguish between the
	 * case where the key exists in the stack but is associated with a null
	 * value, and the case where the key does not exist in the stack.
	 * </p>
	 *
	...
	/**
	 * <p>
	 * Find the ThreadContext in the stack that contains the given key, or
	...
	private static ThreadContext contextContaining(String key) throws IllegalArgumentException {
		...
		for (ThreadContext context = getContext(); context != null; context = context.previous) {
			if (key.equals(context.key))
				return context;
			/*
			 * Note that "context.key" may be null if "context" is the sentinel,
			 * so don't write "if (context.key.equals(key))"!
			 */
		}
		return null;
	}
	...
	/**
	 * <p>
	 * Push an object on the context stack with the given key. This operation
	 * can subsequently be undone by calling <code>restore</code> with the
	 * ThreadContext value returned here.
	 * </p>
	 *
	 * @param key
	...
	/**
	 * <p>
	 * Return an object that can later be supplied to <code>restore</code> to
	 * restore the context stack to its current state. The object can also be
	 * given to <code>setInitialContext</code>.
	 * </p>
	 *
	...
	/**
	 * <p>
	 * Restore the context stack to an earlier state. This typically undoes the
	 * effect of one or more <code>push</code> calls.
	 * </p>
	 *
	 * @param oldContext
	 *            the state to return. This is usually the return value of an
	 *            earlier <code>push</code> operation.
	 *
	 * @exception NullPointerException
	 *                if <code>oldContext</code> is null.
	 * @exception IllegalArgumentException
	 *                if <code>oldContext</code> does not represent a context
	 *                from this thread, or if that context was undone by an
	...
	public static void restore(ThreadContext oldContext)
		 ...
		 * NullPointerException anyway. But if someone didn't notice that during
		 * subsequent changes, they could accidentally permit restore(null) with
		 * the semantics of trashing the context stack.
		 */
		if (oldContext == null)
		...
		for (ThreadContext context = getContext(); context != oldContext; context = context.previous) {
			if (context == null) {
				throw new IllegalArgumentException(
						"Restored context is not " + "contained in current " + "context");
			}
		}

		/*
		 * Discard the sentinel if the stack is empty. This means that it is an
		 * error to call "restore" a second time with the ThreadContext value
		 * that means an empty stack. That's why we don't say that it is all
		 * right to restore the stack to the state it was already in.
		 */
	...
	/**
	 * <p>
	 * Set the initial context of the calling thread to a context obtained from
	 * another thread. After this call, the calling thread will see the same
	 ...
	 * subsequent <code>restore</code>.
	 * </p>
	 *
	 * @exception IllegalArgumentException
	 *                if the context stack was not empty before the call. An
	 *                implementation may also throw this exception if
	 *                <code>context</code> is no longer current in the thread
	 *                from which it was obtained.
	 */
	/*
	 * We rely on the fact that ThreadContext objects are immutable. This means
	 * that we don't have to check that the "context" argument is valid. It
	 * necessarily represents the head of a valid chain of ThreadContext
