...
public abstract class SnmpMibAgent implements SnmpMibAgentMBean, MBeanRegistration, Serializable {
	 ...
	 *
	 * @param nonRepeat
	 *            The number of variables, starting with the first variable in
	 ...
	 *            the last R variables. R is the number of variables following
	 *            the first <CODE>nonRepeat</CODE> variables for which multiple
	 ...
	 *
	 * @exception SnmpStatusException
	 *                An error occurred during the operation. Throwing an
	 *                exception in this method will break the atomicity of the
	 *                SET operation. Care must be taken so that the exception is
	 *                thrown in the {@link #check(SnmpMibRequest)} method
	 *                instead.
	 */
	@Override
	 ...
	 *
	 * @exception SnmpStatusException
	 *                The <CODE>set</CODE> operation cannot be performed.
	 */
	@Override
	 ...
	 *            The SNMP MIB handler.
	 * @param contextName
	 *            The MIB context name. If null is passed, will be registered in
	 *            the default context.
	 *
	 ...
	 *                The SNMP protocol adaptor does not exist in the MBean
	 *                server.
	 *
	 ...
	 * @since 1.5
	 **/
	public static SnmpMibRequest newMibRequest(SnmpPdu reqPdu, Vector<SnmpVarBind> vblist,
			int version, Object userData) {
	 ...
	 * @since 1.5
	 **/
	public static SnmpMibRequest newMibRequest(SnmpEngine engine, SnmpPdu reqPdu,
			Vector<SnmpVarBind> vblist, int version, Object userData, String principal,
			int securityLevel, int securityModel, byte[] contextName, byte[] accessContextName) {
		return new SnmpMibRequestImpl(engine, reqPdu, vblist, version, userData, principal,
				securityLevel, securityModel, contextName, accessContextName);
	}
	// ---------------------------------------------------------------------
	// PACKAGE METHODS
	// ---------------------------------------------------------------------
	 ...
	 *            The number of lexicographic successors requested for each of
	 *            the last R variables. R is the number of variables following
	 *            the first nonRepeat variables for which multiple lexicographic
	...
	void getBulkWithGetNext(SnmpMibRequest req, int nonRepeat, int maxRepeat)
		...
		if (L != 0) {
			...
			SnmpMibRequestImpl repeatedReq = new SnmpMibRequestImpl(req.getEngine(), req.getPdu(),
					repeaters, SnmpDefinitions.snmpVersionTwo, req.getUserData(),
					req.getPrincipal(), req.getSecurityLevel(), req.getSecurityModel(),
					req.getContextName(), req.getAccessContextName());
			for (int i = 2; i <= M; i++) {
				getNext(repeatedReq);
