...
public abstract class SnmpMib extends SnmpMibAgent implements Serializable {
	 ...
	 * @param defaultOid
	 *                   The OID defined in the MIB for that group (in dot
	 *                   notation).
	 *
	 ...
	 *                     for this group.
	 * @param group
	 *                     The MBean for this group, as returned by the MBean
	 *                     factory
	 ...
	 *                     registered, otherwise it is a reference to the
	 *                     MBeanServer in
	 *                     which the MIB is registered.
	 *
	 */
	protected void registerGroupNode(String groupName, String groupOid,
			ObjectName groupObjName, SnmpMibNode node, Object group,
			MBeanServer server) throws NotCompliantMBeanException,
	...
	public void get(SnmpMibRequest req) throws SnmpStatusException {
		...
		// get() method.
		for (Enumeration<SnmpRequestTree.Handler> eh = handlers
				.getHandlers(); eh.hasMoreElements();) {
			h = eh.nextElement();
			...
			for (Enumeration<SnmpMibSubRequest> rqs = handlers.getSubRequests(
					h); rqs.hasMoreElements();) {
	...
	public long[] getRootOid() {
		...
		if (rootOid == null) {
			...
			int i = 0;
			for (Enumeration<Integer> e = list.elements(); e
					.hasMoreElements();) {
	 ...
	 *
	 * @param req
	 *                   The SnmpMibRequest object holding the varbind list
	 *                   concerning
	 *                   this MIB.
	 * @param createflag
	 *                   Indicates whether the operation allow for creation of
	 *                   new
	 ...
	 */
	private SnmpRequestTree getHandlers(SnmpMibRequest req, boolean createflag,
			boolean atomic, int type) throws SnmpStatusException {
		...
		for (Enumeration<SnmpVarBind> e = req.getElements(); e
			...
			try {
				...
				root.findHandlingNode(var, var.oid.longValue(false), 0,
						handlers);
				...
				if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINEST)) {
					SNMP_ADAPTOR_LOGGER.logp(Level.FINEST, SnmpMib.class
							.getName(), "getHandlers",
							"Couldn't find a handling node for " + var.oid
									.toString());
				}
				...
				if (ver == SnmpDefinitions.snmpVersionOne) {
					...
					// SNMP index ...
					//
					final SnmpStatusException sse = new SnmpStatusException(x,
							index + 1);
					sse.initCause(x);
					throw sse;
				} else if ((type == SnmpDefinitions.pduWalkRequest)
						|| (type == SnmpDefinitions.pduSetRequestPdu)) {
					final int status = SnmpRequestTree.mapSetException(x
							.getStatus(), ver);
					...
					final SnmpStatusException sse = new SnmpStatusException(
							status, index + 1);
					sse.initCause(x);
					throw sse;
				} else if (atomic) {
				...
				final int status = SnmpRequestTree.mapGetException(x
						.getStatus(), ver);
	...
	private SnmpRequestTree getGetNextHandlers(SnmpMibRequest req)
		...
		for (Enumeration<SnmpVarBind> e = req.getElements(); e
			...
			try {
				...
				if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINEST)) {
					SNMP_ADAPTOR_LOGGER.logp(Level.FINEST, SnmpMib.class
							.getName(), "getGetNextHandlers", " Next OID of : "
									+ var.oid);
				}
				result = new SnmpOid(root.findNextHandlingNode(var, var.oid
						.longValue(false), 0, 0, handlers, checker));

				if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINEST)) {
					SNMP_ADAPTOR_LOGGER.logp(Level.FINEST, SnmpMib.class
							.getName(), "getGetNextHandlers", " is : "
									+ result);
				}
				...
				if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINEST)) {
					SNMP_ADAPTOR_LOGGER.logp(Level.FINEST, SnmpMib.class
							.getName(), "getGetNextHandlers", "Exception : " + x
									.getStatus());
				}
