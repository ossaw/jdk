 ...
 * <CODE>mibgen</CODE>, there is one which extends <CODE>SnmpMib</CODE> for
 * representing a whole MIB. <BR>
 * The class is used by the SNMP protocol adaptor as the entry point in the MIB.
 *
 * <p>
 * This generated class can be subclassed in your code in order to plug in your
...
public abstract class SnmpMib extends SnmpMibAgent implements Serializable {
	...
	/**
	 * <p>
	 * This callback should return the OID associated to the group identified by
	 * the given <code>groupName</code>.
	 * </p>
	 *
	 * <p>
	 * This method is provided as a hook to plug-in some custom specific
	 * behavior. Although doing so is discouraged you might want to subclass
	 * this method in order to store & provide more metadata information
	 * (mapping OID <-> symbolic name) within the agent, or to "change" the root
	 * of the MIB OID by prefixing the defaultOid by an application dependant
	 * OID string, for instance.
	 * </p>
	 *
	 * <p>
	 ...
	 *
	 * @param groupName
	 *            The java-ized name of the SNMP group.
	 * @param defaultOid
	 *            The OID defined in the MIB for that group (in dot notation).
	 *
	 * @return The OID of the group identified by <code>groupName</code>, in
	 *         dot-notation.
	 */
	...
	/**
	 * <p>
	 * This callback should return the ObjectName associated to the group
	 * identified by the given <code>groupName</code>.
	 * </p>
	 *
	 * <p>
	 * This method is provided as a hook to plug-in some custom specific
	 * behavior. You might want to override this method in order to provide a
	 * different object naming scheme than that proposed by default by
	 * <code>mibgen</code>.
	 * </p>
	 *
	 * <p>
	 * This method is only meaningful if the MIB is registered in the
	 * MBeanServer, otherwise, it will not be called.
	 * </p>
	 *
	 * <p>
	 ...
	 *            The java-ized name of the SNMP group.
	 * @param oid
	 *            The OID returned by getGroupOid() - in dot notation.
	 * @param defaultName
	 *            The name by default generated by <code>
	 *                    mibgen</code>
	 *
	 ...
	 * <p>
	 * This method is provided as a hook to plug-in some custom specific
	 * behavior. You might want to override this method if you want to set
	 * special links between the MBean, its metadata node, its OID or ObjectName
	 * etc..
	 * </p>
	 *
	 * <p>
	 ...
	 * <code>server</code> and <code>groupObjName</code> parameters are not
	 * null, then this method will also automatically register the group MBean
	 * with the given MBeanServer <code>server</code>.
	 * </p>
	 *
	 * @param groupName
	 *            The java-ized name of the SNMP group.
	 * @param groupOid
	 *            The OID as returned by getGroupOid() - in dot notation.
	 * @param groupObjName
	 *            The ObjectName as returned by getGroupObjectName(). This
	 *            parameter may be <code>null</code> if the MIB is not
	 *            registered in the MBeanServer.
	 * @param node
	 *            The metadata node, as returned by the metadata factory method
	 *            for this group.
	 * @param group
	 *            The MBean for this group, as returned by the MBean factory
	 *            method for this group.
	 * @param server
	 *            The MBeanServer in which the groups are to be registered. This
	 *            parameter will be <code>null</code> if the MIB is not
	 *            registered, otherwise it is a reference to the MBeanServer in
	 *            which the MIB is registered.
	 *
	 */
	protected void registerGroupNode(String groupName, String groupOid, ObjectName groupObjName,
			SnmpMibNode node, Object group, MBeanServer server) throws NotCompliantMBeanException,
			MBeanRegistrationException, InstanceAlreadyExistsException, IllegalAccessException {
	 ...
	 *
	 * <p>
	 * <b><i> This method is used internally and you should never need to call
	 * it directly.</i></b><br>
	 * It is used to establish the link between an SNMP table metadata node and
	 * its bean-like counterpart. <br>
	 * The group metadata nodes will create and register their underlying table
	 * metadata nodes in the MIB using this method. <br>
	 * The metadata nodes will be later retrieved from the MIB by the bean-like
	 * table objects using the getRegisterTableMeta() method.
	 * </p>
	 *
	 * @param name
	 *            The java-ized name of the SNMP table.
	 * @param table
	 *            The SNMP table metadata node - usually this corresponds to a
	 *            <code>mibgen</code> generated object.
	 */
	...
	public void get(SnmpMibRequest req) throws SnmpStatusException {
		...
		if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINEST)) {
			SNMP_ADAPTOR_LOGGER.logp(Level.FINEST, SnmpMib.class.getName(), "get",
					"Processing handlers for GET... ");
		}

		// For each sub-request stored in the request-tree, invoke the
		// get() method.
		for (Enumeration<SnmpRequestTree.Handler> eh = handlers.getHandlers(); eh
				.hasMoreElements();) {
			h = eh.nextElement();
			...
			for (Enumeration<SnmpMibSubRequest> rqs = handlers.getSubRequests(h); rqs
					.hasMoreElements();) {
	...
	public void set(SnmpMibRequest req) throws SnmpStatusException {
		...
		if (handlers == null)
			handlers = getHandlers(req, false, true, reqType);
		...
		if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINEST)) {
			SNMP_ADAPTOR_LOGGER.logp(Level.FINEST, SnmpMib.class.getName(), "set",
	 ...
	 * cannot be performed, the method will raise a
	...
	public void check(SnmpMibRequest req) throws SnmpStatusException {
		...
		if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINEST)) {
			SNMP_ADAPTOR_LOGGER.logp(Level.FINEST, SnmpMib.class.getName(), "check",
	...
	public void getNext(SnmpMibRequest req) throws SnmpStatusException {
		...
		if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINEST)) {
			SNMP_ADAPTOR_LOGGER.logp(Level.FINEST, SnmpMib.class.getName(), "getNext",
	...
	/**
	 * Processes a <CODE>getBulk</CODE> operation. The method implements the
	 * <CODE>getBulk</CODE> operation by calling appropriately the
	 ...
	 * <P>
	 * In order to be accurate, the method should be called once the MIB is
	 * fully initialized (that is, after a call to <CODE>init</CODE> or
	 ...
	 *
	 * @param req
	 *            The SnmpMibRequest object holding the varbind list concerning
	 *            this MIB.
	 * @param createflag
	 *            Indicates whether the operation allow for creation of new
	 *            instances (ie: it is a SET).
	 * @param atomic
	 *            Indicates whether the operation is atomic or not.
	 * @param type
	 *            Request type (from SnmpDefinitions).
	 *
	 * @return The request-tree where the original varbind list has been
	 *         dispatched to the appropriate nodes.
	 */
	private SnmpRequestTree getHandlers(SnmpMibRequest req, boolean createflag, boolean atomic,
		...
		for (Enumeration<SnmpVarBind> e = req.getElements(); e.hasMoreElements(); index++) {
			...
			try {
				...
				if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINEST)) {
					SNMP_ADAPTOR_LOGGER.logp(Level.FINEST, SnmpMib.class.getName(), "getHandlers",
							"Couldn't find a handling node for " + var.oid.toString());
				}
				...
				if (status == SnmpStatusException.noSuchInstance) {
					...
					if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINEST)) {
						SNMP_ADAPTOR_LOGGER.logp(Level.FINEST, SnmpMib.class.getName(),
								"getHandlers", "\tGET: Registering noSuchInstance");
					}
					...
					if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINEST)) {
						SNMP_ADAPTOR_LOGGER.logp(Level.FINEST, SnmpMib.class.getName(),
								"getHandlers", "\tGET: Registering noSuchObject");
					}
					...
					if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINEST)) {
						SNMP_ADAPTOR_LOGGER.logp(Level.FINEST, SnmpMib.class.getName(),
								"getHandlers", "\tGET: Registering global error: " + status);
					}
	...
	/**
	 * This method builds the temporary request-tree that will be used to
	 * perform the SNMP GET-NEXT request associated with the given vector of
	 * varbinds `list'.
	 *
	 * @param req
	 *            The SnmpMibRequest object holding the varbind list concerning
	 ...
	 *
	 * @return The request-tree where the original varbind list has been
	 *         dispatched to the appropriate nodes, and where the original OIDs
	 *         have been replaced with the correct "next" OID.
	 */
	private SnmpRequestTree getGetNextHandlers(SnmpMibRequest req) throws SnmpStatusException {
		...
		SnmpRequestTree handlers = new SnmpRequestTree(req, false,
				SnmpDefinitions.pduGetNextRequestPdu);
		...
		if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINEST)) {
			SNMP_ADAPTOR_LOGGER.logp(Level.FINEST, SnmpMib.class.getName(), "getGetNextHandlers",
					"Received MIB request : " + req);
		}
		...
		for (Enumeration<SnmpVarBind> e = req.getElements(); e.hasMoreElements(); index++) {
			...
			try {
				...
				result = new SnmpOid(root.findNextHandlingNode(var, var.oid.longValue(false), 0, 0,
						handlers, checker));
				...
				if (ver == SnmpDefinitions.snmpVersionOne) {
					if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINEST)) {
						SNMP_ADAPTOR_LOGGER.logp(Level.FINEST, SnmpMib.class.getName(),
								"getGetNextHandlers", "\tThrowing exception " + x.toString());
					}
					// The index in the exception must correspond to the
					// SNMP index ...
					//
				...
				if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINEST)) {
					SNMP_ADAPTOR_LOGGER.logp(Level.FINEST, SnmpMib.class.getName(),
							"getGetNextHandlers", "Exception : " + x.getStatus());
				}
