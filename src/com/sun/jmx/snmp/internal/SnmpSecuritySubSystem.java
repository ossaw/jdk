...
/**
 * Security sub system interface. To allow engine integration, a security sub
...
public interface SnmpSecuritySubSystem extends SnmpSubSystem {
    /**
     * Instantiates an <CODE>SnmpSecurityCache</CODE> that is dependant to the
     * model implementation. This call is routed to the dedicated model
     ...
     */
    public void releaseSecurityCache(int id, SnmpSecurityCache cache) throws SnmpUnknownSecModelException;
    ...
    public int generateRequestMsg(SnmpSecurityCache cache, int version, int msgID, int msgMaxSize,
            byte msgFlags, int msgSecurityModel, SnmpSecurityParameters params, byte[] contextEngineID,
            byte[] contextName, byte[] data, int dataLength, byte[] outputBytes) throws SnmpTooBigException,
            SnmpStatusException, SnmpSecurityException, SnmpUnknownSecModelException;

    /**
     * Called when a response is to be sent to the network. It must be
     * securized. This call is routed to the dedicated model according to the
     * model ID. <BR>
     * The specified parameters are defined in RFC 2572 (see also the
     * {@link com.sun.jmx.snmp.SnmpV3Message} class).
     * 
    ...
    /**
     * Called when a request is received from the network. It handles
     * authentication and privacy. This call is routed to the dedicated model
     * according to the model ID. <BR>
     * The specified parameters are defined in RFC 2572 (see also the
     * {@link com.sun.jmx.snmp.SnmpV3Message} class).
     * 
     * @param cache
     ...
     */
    public SnmpSecurityParameters processIncomingRequest(SnmpSecurityCache cache, int version, int msgID,
            int msgMaxSize, byte msgFlags, int msgSecurityModel, byte[] params, byte[] contextEngineID,
            byte[] contextName, byte[] data, byte[] encryptedPdu, SnmpDecryptedPdu decryptedPdu)
     ...
     */
    public SnmpSecurityParameters processIncomingResponse(SnmpSecurityCache cache, int version, int msgID,
            int msgMaxSize, byte msgFlags, int msgSecurityModel, byte[] params, byte[] contextEngineID,
            byte[] contextName, byte[] data, byte[] encryptedPdu, SnmpDecryptedPdu decryptedPdu)
            throws SnmpStatusException, SnmpSecurityException, SnmpUnknownSecModelException;
}
