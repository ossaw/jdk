...
/**
 * This engine is conformant with the RFC 2571. It is the main object within an
 * SNMP entity (agent, manager...). To an engine is associated an
 * {@link com.sun.jmx.snmp.SnmpEngineId}. The way the engineId is retrieved is
 * linked to the way the engine is instantiated. See each
 * <CODE>SnmpEngine</CODE> constructor for more details. An engine is composed
 * of a set of sub systems {@link com.sun.jmx.snmp.internal.SnmpSubSystem}. An
 * <CODE>SNMP</CODE> engine can contain a:
 * <ul>
 ...
 * </ul>
 * <P>
 * Each sub system contains a set of models. A model is an implementation of a
 * particular treatement (eg: the User based Security Model defined in RFC 2574
 * is a functional element dealing with authentication and privacy).
 * </P>
 * Engine instantiation is based on a factory. This factory, implementing
 * mandatorily {@link com.sun.jmx.snmp.SnmpEngineFactory SnmpEngineFactory} is
 * set in the method <CODE>setFactory</CODE>.
 * <p>
...
public class SnmpEngineImpl implements SnmpEngine, Serializable {
    ...
    /**
     * Flag that indicates that a report is to be sent. Value is 4, as defined
     * in RFC 2572
     */
    ...
    public synchronized int getEngineTime() {
        // We do the counter wrap in a lazt way. Each time Engine is asked for
        // his time it checks. So if nobody use the Engine, the time can wrap
        // and wrap again without incrementing nb boot. We can imagine that it
        // is irrelevant due to the amount of time needed to wrap.
    ...
    /**
     * Constructor. A Local Configuration Datastore is passed to the engine. It
     * will be used to store and retrieve data (engine Id, engine boots).
     * <P>
     * WARNING : The SnmpEngineId is computed as follow:
     * <ul>
     * <li>If an lcd file is provided containing the property "localEngineID",
     * this property value is used.</li>.
     * <li>If not, if the passed engineID is not null, this engine ID is used.
     * </li>
     * <li>If not, a time based engineID is computed.</li>
     * </ul>
     * This constructor should be called by an <CODE>SnmpEngineFactory</CODE>.
    ...
    public SnmpEngineImpl(SnmpEngineFactory fact, SnmpLcd lcd, SnmpEngineId engineid)
        ...
        if (SNMP_LOGGER.isLoggable(Level.FINER)) {
            SNMP_LOGGER.logp(Level.FINER, SnmpEngineImpl.class.getName(),
                    "SnmpEngineImpl(SnmpEngineFactory,SnmpLcd,SnmpEngineId)", "LOCAL ENGINE ID: "
                            + this.engineid);
        }
    }
    ...
    public SnmpEngineImpl(SnmpEngineFactory fact, SnmpLcd lcd, InetAddress address, int port)
        ...
        if (SNMP_LOGGER.isLoggable(Level.FINER)) {
            SNMP_LOGGER.logp(Level.FINER, SnmpEngineImpl.class.getName(),
                    "SnmpEngineImpl(SnmpEngineFactory,SnmpLcd,InetAddress,int)", "LOCAL ENGINE ID: "
                            + engineid + " / " + "LOCAL ENGINE NB BOOTS: " + boot + " / "
    ...
    public SnmpEngineImpl(SnmpEngineFactory fact, SnmpLcd lcd, int port) throws UnknownHostException {
        ...
        if (SNMP_LOGGER.isLoggable(Level.FINER)) {
            SNMP_LOGGER.logp(Level.FINER, SnmpEngineImpl.class.getName(),
                    "SnmpEngineImpl(SnmpEngineFactory,SnmpLcd,int)", "LOCAL ENGINE ID: " + engineid + " / "
                            + "LOCAL ENGINE NB BOOTS: " + boot + " / " + "LOCAL ENGINE START TIME: "
                            + getEngineTime());
        }
    }
     ...
     * <li>If not, a time based engineID is computed.</li>
     * </ul>
     * When no configuration nor java property is set for the engine ID value, a
     * unique time based engine ID will be generated. This constructor should be
    ...
    public SnmpEngineImpl(SnmpEngineFactory fact, SnmpLcd lcd) throws UnknownHostException {
        ...
        if (SNMP_LOGGER.isLoggable(Level.FINER)) {
            SNMP_LOGGER.logp(Level.FINER, SnmpEngineImpl.class.getName(),
                    "SnmpEngineImpl(SnmpEngineFactory,SnmpLcd)", "LOCAL ENGINE ID: " + engineid + " / "
                            + "LOCAL ENGINE NB BOOTS: " + boot + " / " + "LOCAL ENGINE START TIME: "
                            + getEngineTime());
        }
    }
