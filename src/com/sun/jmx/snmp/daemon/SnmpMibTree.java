...
/**
 * The class is used for building a tree representation of the different root
 * oids of the supported MIBs. Each node is associated to a specific MIB.
 */
final class SnmpMibTree {
	...
	/*
	 * public void unregister(SnmpMibAgent agent) { long[] oid=
	 * agent.getRootOid(); TreeNode node= root.retrieveMatchingBranch(oid, 0);
	 * if (node == null) return; node.removeAgent(agent); }
	 */
	...
	final class TreeNode {
		...
		TreeNode retrieveMatchingBranch(long[] oid, int cursor) {
			...
			if (node == null)
				return this;
			if (children.isEmpty()) {
				// In this case, the node does not have any children. So no
				// point to
				// continue the search ...
				return node;
			}
			if (cursor + 1 == oid.length) {
				// In this case, the oid does not have any more element. So the
				// search
				// is over.
				return node;
			}
		...
		/**
		 * Only the treeNode class can create an instance of treeNode. The
		 * creation occurs when registering a new oid.
		 */
		...
		private void registerNode(long[] oid, int cursor, SnmpMibAgent agent) {
			...
			} else if (agents.contains(agent) == false) {
				agents.addElement(agent);
			}
