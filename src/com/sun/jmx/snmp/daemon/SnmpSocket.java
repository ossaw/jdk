...
/**
 * This class creates an SNMP Datagram Socket. This class has methods helpful to
 * send SNMP inform request packets to an arbitrary port of a specified device.
 * It also runs a thread that is devoted to receiving SNMP inform response on
 * the socket. <BR>
 * A socket imposes an upper limit on size of inform response packet. Any packet
 * which exceeds this limit is truncated. By default, this limit is
 * {@link SnmpAdaptorServer#bufferSize}.
 */
...
final class SnmpSocket implements java.lang.Runnable {
			...
			if (isValid()) {
				if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINER)) {
					SNMP_ADAPTOR_LOGGER.logp(Level.FINER, SnmpSocket.class.getName(), "sendPacket",
							"Sending DatagramPacket. Length = " + dgrmpkt.getLength()
	 ...
	 * 
	 * @return <CODE>true</CODE> if the socket is initialised correctly and if
	 *         it is still active, <CODE>false</CODE> otherwise.
	 */
	...
	public synchronized void close() {
		...
		try {
			// We send an empty datagram packet to fix bug 4293791 (it's a jdk
			// 1.1 bug)
			//
			DatagramSocket sn = new java.net.DatagramSocket(0);
			byte[] ob = new byte[1];
			DatagramPacket pk = new DatagramPacket(ob, 1, java.net.InetAddress.getLocalHost(),
					_socketPort);
			sn.send(pk);
			sn.close();
		} catch (Exception e) {
	 ...
	 * which goes in an endless-loop and waits for receiving datagram packets on
	 * the socket.
	 */
	@Override
	public void run() {
		...
		while (true) {
			try {
				...
				if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINER)) {
					SNMP_ADAPTOR_LOGGER.logp(Level.FINER, SnmpSocket.class.getName(), "run",
				...
				// If the corresponding session is being destroyed, stop
				...
				if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINER)) {
					SNMP_ADAPTOR_LOGGER.logp(Level.FINER, SnmpSocket.class.getName(), "run",
							"[" + Thread.currentThread().toString() + "]:" + "Received a packet");
				}
				...
				if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINER)) {
					SNMP_ADAPTOR_LOGGER.logp(Level.FINER, SnmpSocket.class.getName(), "run",
							"[" + Thread.currentThread().toString() + "]:"
									+ "Received a packet from : " + dgrmpkt.getAddress().toString()
				...
				// If the IO exception has been generated because of closing
				// this SNMP socket,
				// (call to _socket.close while _socket is blocked for receiving
				// packet) simply terminate closing properly.
				//
				if (isClosing) {
					break;
				}
				...
				// If the exception (NullPointerException) has been generated
				// because of closing this SNMP socket,
				// (call to _socket = null while _socket is blocked for
				// receiving packet) simply terminate closing properly.
				//
				if (isClosing) {
					break;
				}
	...
	/**
	 * Finalizer of the <CODE>SnmpSocket</CODE> objects. This method is called
	 * by the garbage collector on an object when garbage collection determines
	 * that there are no more references to the object.
	 * <P>
	 * Closes the datagram socket and stops the socket thread associated to this
	 * SNMP socket.
	 */
	@Override
