...
/**
 * This class is used by the {@link com.sun.jmx.snmp.daemon.SnmpAdaptorServer
 * SNMP adaptor server} to send inform requests to an SNMP manager and receive
 * inform responses.
 * <P>
 * This class provides basic functions that enable you to fire inform requests,
 * handle retries, timeouts, and process responses from the manager. <BR>
 * The SNMP adaptor server specifies the destination of the inform request and
 * controls the size of a single inform request/response to fit into its
 * <CODE>bufferSize</CODE>. It specifies the maximum number of tries and the
 * timeout to be used for the inform requests. It also provides resources such
 * as the authentication mechanism (using its PDU factory), controlling all
 * inform requests created by it, and finally the inform response to the user.
 * <P>
 * Each inform request, when ready to be sent, is assigned a unique identifier
 * which helps in identifying the inform request with matching inform responses
 * to the protocol engine lying transparently underneath. The engine does the
 * job of retrying the inform requests when the timer expires and calls the SNMP
 * adaptor server when a timeout occurs after exhausting the maximum number of
 * tries.
 * <P>
 * The inform request object provides the method, {@link #waitForCompletion
 * waitForCompletion(long time)}, which enables a user to operate in a
 * synchronous mode with an inform request. This is done by blocking the user
 * thread for the desired time interval. The user thread gets notified whenever
 * a request reaches completion, independently of the status of the response.
 * <P>
 * If an {@link com.sun.jmx.snmp.daemon.SnmpInformHandler inform callback} is
 * provided when sending the inform request, the user operates in an
 * asynchronous mode with the inform request. The user thread is not blocked and
 * the specific inform callback implementation provided by the user is invoked
 * when the inform response is received.
 * <P>
 * <B>Note:</B> <BR>
 * From RFC 1905, the SNMP inform request is defined as a request generated and
 * transmitted by an SNMPv2 entity acting in a manager role to another SNMPv2
 * entity also acting in a manager role. The mechanisms to implement this
 * behaviour are defined in the SNMP manager API. <BR>
 * Nevertheless, this feature has derived and in some documentations, the inform
 * request appears like an SNMPv2 trap that gets responded. <BR>
 * The <CODE>SnmpInformRequest</CODE> class is used to fullfill this latter
 * case.
 * <p>
...
public class SnmpInformRequest implements SnmpDefinitions {
    ...
    /**
     * This contains a list of <CODE>SnmpVarBind</CODE> objects for making the
    ...
    /**
     * Timeout. The default amount of time is 3000 millisec.
     */
    ...
    /**
     * For SNMP Runtime internal use only. Constructor for creating new inform
     * request. This object can be created only by an SNMP adaptor object.
     * 
     * @param session
     *        <CODE>SnmpSession</CODE> object for this inform request.
     * @param adp
     *        <CODE>SnmpAdaptorServer</CODE> object for this inform
     *        request.
     * @param addr
     ...
     * @param requestCB
     *        Callback interface for the inform request.
     * @exception SnmpStatusException
     *            SNMP adaptor is not ONLINE or session is
     *            dead.
     */
    SnmpInformRequest(SnmpSession session, SnmpAdaptorServer adp, InetAddress addr, String cs, int p,
            SnmpInformHandler requestCB) throws SnmpStatusException {
     ...
     * 
     * @return <CODE>true</CODE> if the inform request was aborted,
     ...
     * 
     * @return <CODE>true</CODE> if the inform request is in progress,
     *         <CODE>false</CODE> otherwise.
     */
     ...
     * 
     * @return <CODE>true</CODE> if the inform request result is available,
     *         <CODE>false</CODE> otherwise.
     */
     ...
     * Gets the index.
     * <P>
     * NOTE: this value is equal to the <CODE>errorIndex</CODE> field minus 1.
     * 
    ...
    /**
     * Gets the maximum number of tries before declaring that the manager is not
     * responding.
     * 
    ...
    /**
     * Gets absolute time in milliseconds (based on epoch time) before which an
     * inform response is expected from a manager.
     * 
    ...
    /**
     * Gets the <CODE>SnmpVarBindList</CODE> of the inform response. It returns
     * a null value if the inform request is in progress. This ensures
     * accidental manipulation does not occur when a request is in progress. In
     * case of an error, <CODE>SnmpVarBindList</CODE> is the copy of the
     * original <CODE>SnmpVarBindList</CODE> at the time of making the inform
     * request.
     * 
     * @return The list of <CODE>SnmpVarBind</CODE> objects returned by the
     *         manager or the null value if the request is in progress.
     */
    ...
    /**
     * Used in synchronous mode only. Provides a hook that enables a synchronous
     * operation on a previously sent inform request. Only one inform request
     * can be in synchronous mode on a given thread. The blocked thread is
     * notified when the inform request state reaches completion. If the inform
     * request is not active, the method returns immediately. The user must get
     * the error status of the inform request to determine the exact status of
     * the request.
     *
     * @param time
     *        The amount of time to wait. Zero means block until complete.
     * @return <CODE>true</CODE> if the inform request has completed,
    ...
    /**
     * Cancels the active inform request and removes itself from the polling
     * list.
     */
    ...
    /**
     * Finalizer of the <CODE>SnmpInformRequest</CODE> objects. This method is
    ...
    /**
     * For SNMP Runtime internal use only. Starts an inform request in
     * asynchronous mode. The callback interface is used to notify the user upon
     * request completion.
     * 
     * @param vblst
     *        The list of <CODE>SnmpVarBind</CODE> to be used.
     * @exception SnmpStatusException
     *            This inform request is already in
     *            progress.
     */
     ...
     * active. It does nothing if the request is already active. The poll will
     * be scheduled to happen immediately.
     * 
     * @param starttime
     *        The start time for polling.
     */
    ...
    /**
     * This method creates a new request ID. The ID is submitted to the poll
    ...
    /**
     * This method determines whether the inform request is to be retried. This
     * is used if the peer did not respond to a previous request. If the request
     * exceeds the maxTries limit, a timeout is signaled.
     */
    void action() {
        if (inProgress() == false)
            return;
        while (true) {
            try {
                ...
                if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINEST)) {
                    SNMP_ADAPTOR_LOGGER.logp(Level.FINEST, SnmpInformRequest.class.getName(), "action",
    ...
    synchronized SnmpPdu constructPduPacket() {
        ...
        try {
            ...
            if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINER)) {
                SNMP_ADAPTOR_LOGGER.logp(Level.FINER, SnmpInformRequest.class.getName(), "constructPduPacket",
                        "Packet built");
            }
    ...
    boolean sendPdu() {
        try {
            ...
            SnmpMessage msg = (SnmpMessage) pduFactory.encodeSnmpPdu((SnmpPduPacket) requestPdu, adaptor
                    .getBufferSize().intValue());

            if (msg == null) {
                if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINEST)) {
                    SNMP_ADAPTOR_LOGGER.logp(Level.FINEST, SnmpInformRequest.class.getName(), "sendPdu",
            ...
            if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINER)) {
                SNMP_ADAPTOR_LOGGER.logp(Level.FINER, SnmpInformRequest.class.getName(), "sendPdu",
                        "Dump : \n" + msg.printMessage());
            }
    ...
    /**
     * Sends the prepared PDU packet to the manager and updates the data
     * structure to expect a response. It acquires a lock on the socket to
     * prevent a case where a response arrives before this thread could insert
     * the request into the wait queue.
     * 
    ...
    final void sendPduPacket(byte[] buffer, int length) throws java.io.IOException {
        ...
        if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINER)) {
            SNMP_ADAPTOR_LOGGER.logp(Level.FINER, SnmpInformRequest.class.getName(), "sendPduPacket",
                    "Send to peer. Peer/Port : " + address.getHostName() + "/" + port + ". Length = " + length
    ...
    final void processResponse() {
        ...
        if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINER)) {
            SNMP_ADAPTOR_LOGGER.logp(Level.FINER, SnmpInformRequest.class.getName(), "processResponse",
        ...
        try {
            ...
            if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINEST)) {
                SNMP_ADAPTOR_LOGGER.logp(Level.FINEST, SnmpInformRequest.class.getName(), "processResponse",
                        "Got unexpected exception", e);
            }
            reason = e.getMessage();
        }
    ...
    synchronized void parsePduPacket(SnmpPduRequestType rpdu) {
        ...
        if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINER)) {
            SNMP_ADAPTOR_LOGGER.logp(Level.FINER, SnmpInformRequest.class.getName(), "parsePduPacket",
                    "received inform response. ErrorStatus/ErrorIndex = " + errorStatus + "/" + errorIndex);
        }
    }

    /**
     * Calls the user implementation of the <CODE>SnmpInformHandler</CODE>
     * interface.
     */
    private void handleSuccess() {
        ...
        if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINER)) {
            SNMP_ADAPTOR_LOGGER.logp(Level.FINER, SnmpInformRequest.class.getName(), "handleSuccess",
        ...
        try { // catch all user exception which may happen in callback.
            ...
            if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINEST)) {
                SNMP_ADAPTOR_LOGGER.logp(Level.FINEST, SnmpInformRequest.class.getName(), "handleSuccess",
                        "Exception generated by user callback", e);
            }
        } catch (OutOfMemoryError ome) {
            if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINEST)) {
                SNMP_ADAPTOR_LOGGER.logp(Level.FINEST, SnmpInformRequest.class.getName(), "handleSuccess",
                        "OutOfMemory Error generated by user callback", ome);
            }
            Thread.yield();
        }
    }
    ...
    private void handleTimeout() {
        ...
        if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINEST)) {
            SNMP_ADAPTOR_LOGGER.logp(Level.FINEST, SnmpInformRequest.class.getName(), "handleTimeout",
                    "Snmp error/index = " + snmpErrorToString(errorStatus) + "/" + errorIndex
                            + ". Invoking timeout user defined callback...");
        }
        deleteRequest();
        notifyClient();
    ...
    private void handleError(String msg) {
        ...
        if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINEST)) {
            SNMP_ADAPTOR_LOGGER.logp(Level.FINEST, SnmpInformRequest.class.getName(), "handleError",
                    "Snmp error/index = " + snmpErrorToString(errorStatus) + "/" + errorIndex
                            + ". Invoking error user defined callback...\n" + getVarBindList());
        }
        deleteRequest();
        notifyClient();
    ...
    private void handleInternalError(String msg) {
        ...
        if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINEST)) {
            SNMP_ADAPTOR_LOGGER.logp(Level.FINEST, SnmpInformRequest.class.getName(), "handleInternalError",
                    "Snmp error/index = " + snmpErrorToString(errorStatus) + "/" + errorIndex
    ...
    /**
     * This method cancels an active inform request and removes it from the
     * polling list.
     */
    ...
    /**
     * For SNMP Runtime internal use only. Gets the active
     * <CODE>SnmpVarBindList</CODE>. The contents of it are not guaranteed to be
     * consistent when the inform request is active.
     * 
     ...
     * <CODE>SnmpVarBindList</CODE> at SnmpInformRequest creation time. You
     * cannot modify it during the life-time of the object.
     */
    ...
    final void setRequestSentTime(long sendtime) {
        ...
        if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINER)) {
            SNMP_ADAPTOR_LOGGER.logp(Level.FINER, SnmpInformRequest.class.getName(), "setRequestSentTime",
    ...
    private synchronized String tostring() {
        ...
        s.append("  Timeout/MaxTries/NumTries = " + timeout * numTries + "/" + +getMaxTries() + "/"
                + numTries);
        ...
        if (prevPollTime > 0) {
            ...
            s.append("\nNeverPolled");
        s.append(" / RemainingTime(millis) = " + timeRemainingForAction(System.currentTimeMillis()));
