 ...
 * Most connectors or adaptors extend <CODE>CommunicatorServer</CODE> and
 * inherit this behavior. Connectors or adaptors that do not fit into this model
 * do not extend <CODE>CommunicatorServer</CODE>.
 * <p>
 * A <CODE>CommunicatorServer</CODE> is an active object, it listens for client
 * requests and processes them in its own thread. When necessary, a
 ...
 * <CODE>CommunicatorServer</CODE> no longer listens to client requests and no
 * longer holds any thread or communication resources. It can be started again
 * by calling the <CODE>start</CODE> method.
 * <p>
 * A <CODE>CommunicatorServer</CODE> has a <CODE>State</CODE> attribute which
 * reflects its activity.
 * <p>
 * <TABLE>
 * <TR>
 * <TH>CommunicatorServer</TH>
 * <TH>State</TH>
 * </TR>
 * <TR>
 ...
 * <p>
 * The <CODE>STOPPING</CODE> state marks the transition from <CODE>ONLINE</CODE>
 * to <CODE>OFFLINE</CODE>. This occurs when the <CODE>CommunicatorServer</CODE>
 * is finishing or interrupting active requests.
 * <p>
...
public abstract class CommunicatorServer
	...
	/**
	 * The maximum number of clients that the CommunicatorServer can process
	 * concurrently.
	 * 
	 * @serial
	 */
	 ...
	 * If held at the same time as the lock on "this", the "this" lock must be
	 * taken first.
	 */
	 ...
	 *
	 * @param connectorType
	 *            Indicates the connector type. Possible values are: SNMP_TYPE.
	 *
	 ...
	 *            <code>Long.MAX_VALUE</code>. In that case the method will wait
	 *            until the connector starts or fails to start. If timeout is
	 *            negative or zero, returns as soon as possible without waiting.
	 * @exception CommunicationException
	 *                if the connectors fails to start.
	 * @exception InterruptedException
	 *                if the thread is interrupted or the timeout expires.
	 */
	...
	/**
	 * <p>
	 * Waits until either the State attribute of this MBean equals the specified
	 * <VAR>wantedState</VAR> parameter, or the specified <VAR>timeOut</VAR> has
	 * elapsed. The method <CODE>waitState</CODE> returns with a boolean value
	 * indicating whether the specified <VAR>wantedState</VAR> parameter equals
	 ...
	 * <VAR>wantedState</VAR> parameter (i.e. will wait indefinitely if this
	 * condition is never met).</LI>
	 * </UL>
	 *
	 * @param wantedState
	...
	public boolean waitState(int wantedState, long timeOut) {
		if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINER)) {
			SNMP_ADAPTOR_LOGGER.logp(Level.FINER, dbgTag, "waitState", wantedState
					+ "(0on,1off,2st) TO=" + timeOut + " ; current state = " + getStateString());
		}
	...
	private void waitForStart(long timeout) throws CommunicationException, InterruptedException {
		if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINER)) {
			SNMP_ADAPTOR_LOGGER.logp(Level.FINER, dbgTag, "waitForStart",
					"Timeout=" + timeout + " ; current state = " + getStateString());
		}
		...
		synchronized (stateLock) {
			...
			if (state == ONLINE) {
				...
				throw new CommunicationException(
						"Failed to start: state is " + getStringForState(state));
			}
		}
	}
	 ...
	 *         <CODE>CommunicatorServer</CODE> since its creation. This counter
	 *         is not reset by the <CODE>stop</CODE> method.
	 */
	...
	/**
	 * Sets the maximum number of clients this <CODE>CommunicatorServer</CODE>
	...
	public void run() {
		...
		try {
			...
			if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINEST)) {
				SNMP_ADAPTOR_LOGGER.logp(Level.FINEST, dbgTag, "run", "Got unexpected exception",
						x);
			}
			synchronized (stateLock) {
				startException = x;
	 ...
	 * should process the requests of the incoming client. If it throws an
	...
	/**
	 * Set the <code>MBeanServer</code> object to which incoming requests are
	 * sent. This must be either the MBean server in which this connector is
	 * registered, or an <code>MBeanServerForwarder</code> leading to that
	 * server. An <code>MBeanServerForwarder</code> <code>mbsf</code> leads to
	 * an MBean server <code>mbs</code> if <code>mbsf.getMBeanServer()</code> is
	 * either <code>mbs</code> or an <code>MBeanServerForwarder</code> leading
	 * to <code>mbs</code>.
	 *
	 * @exception IllegalArgumentException
	 *                if <code>newMBS</code> is neither the MBean server in
	 *                which this connector is registered nor an
	...
	/**
	 * This method blocks if there are too many active clients. Call to
	...
	private void waitClientTermination() {
		...
		if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINER)) {
			if (s >= 1) {
				SNMP_ADAPTOR_LOGGER.logp(Level.FINER, dbgTag, "waitClientTermination",
						"waiting for " + s + " clients to terminate");
			}
		}
	 ...
	 * @param handback
	 *            An object which will be sent back unchanged to the listener
	...
	public void addNotificationListener(NotificationListener listener, NotificationFilter filter,
		...
		if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINEST)) {
			SNMP_ADAPTOR_LOGGER.logp(Level.FINEST, dbgTag, "addNotificationListener",
					"Adding listener " + listener + " with filter " + filter + " and handback "
							+ handback);
		}
	...
	/**
	 * Removes the specified listener from this CommunicatorServer. Note that if
	 * the listener has been registered with different handback objects or
	 * notification filters, all entries corresponding to the listener will be
	 * removed.
	 *
	 * @param listener
	...
	public void removeNotificationListener(NotificationListener listener)
		...
		if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINEST)) {
			SNMP_ADAPTOR_LOGGER.logp(Level.FINEST, dbgTag, "removeNotificationListener",
	 ...
	 * notification types sent by this CommunicatorServer. There is only one
	 * type of notifications sent by the CommunicatorServer: it is
	 * <tt>{@link javax.management.AttributeChangeNotification}</tt>, sent when
	 * the <tt>State</tt> attribute of this CommunicatorServer changes.
	 */
	@Override
	...
	private void sendStateChangeNotification(int oldState, int newState) {
		...
		String message = new StringBuffer().append(dbgTag)
				.append(" The value of attribute State has changed from ").append(oldState)
				.append(" (").append(oldStateString).append(") to ").append(newState).append(" (")
		...
		if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINEST)) {
			SNMP_ADAPTOR_LOGGER.logp(Level.FINEST, dbgTag, "sendStateChangeNotification",
					"Sending AttributeChangeNotification #" + notifCount + " with message: "
							+ message);
		}
		...
		synchronized (this) {
			if (bottomMBS != null) {
				throw new IllegalArgumentException(
						"connector already " + "registered in an MBean " + "server");
			}
	 ...
	 *            registered in the <CODE>MBeanServer</CODE>. The value false
