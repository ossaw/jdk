...
class SnmpSession implements SnmpDefinitions, Runnable {
	...
	/**
	 * The SnmpSocket to be used to communicate with the manager by all inform
	 * requests created in this session.
	 */
	...
	/**
	 * This table maintains the list of inform responses. A FIFO queue is needed
	 * here.
	 */
	private transient Stack<SnmpInformRequest> informRespq = new Stack<>();
	/**
	 * The dispatcher that will service all inform responses to inform requests
	 * generated using this session object. An SnmpSession object creates one or
	 * more inform requests. Thus it services all inform requests, which are
	 * created by this session object, when an inform response arrives for an
	 * inform request generated by the session.
	 */
	private transient Thread myThread = null;
	/**
	 * Request being synchronized from session thread. This happens when a user
	 * does sync operation from a callback.
	 */
	...
	protected synchronized void initialize(SnmpAdaptorServer adp, SnmpResponseHandler snmpRespHdlr)
		...
		informSocket = new SnmpSocket(snmpRespHdlr, adp.getAddress(),
				adp.getBufferSize().intValue());
	...
	/**
	 * Indicates whether the thread for this session is active and the SNMP
	 * adaptor server ONLINE.
	 * 
	...
	/**
	 * Gets the SnmpSocket which will be used by inform requests created in this
	 * session.
	 * 
	...
	/**
	 * Gets the SnmpQManager which will be used by inform requests created in
	 * this session.
	 * 
	...
	/**
	 * Indicates whether this session is performing synchronous operation for an
	 * inform request.
	 * 
	 * @return <CODE>true</CODE> if the session is performing synchronous
	...
	/**
	 * Returns <CODE>true</CODE> if the current executing thread is this
	 * session's dispatcher. Typically used to detect whether the user is doing
	 * a sync operation from this dispatcher context. For instance, a user gives
	 * a sync command from within a request callback using its associated
	 * session.
	 * 
	 * @return <CODE>true</CODE> if current thread is this session's dispatcher,
	...
	/**
	 * Sends an inform request to the specified InetAddress destination using
	...
	/**
	 * Performs sync operations on active requests. Any number of inform
	 * requests can be done in sync mode but only one per thread. The user can
	 * do synchronous operation using the request handle only.
	 */
	...
	/**
	 * Adds the inform request object which received a response to an inform
	 * request generated by the session. This is added to a private store, which
	...
	/**
	 * Destroys any pending inform requests and then stops the session. The
	 * session will not be usable after this method returns.
	 */
	...
	/**
	 * Make sure you are killing the thread when it is active. Instead prepare
	 * for a graceful exit.
	 */
	...
	/**
	 * Finalizer of the <CODE>SnmpSession</CODE> objects. This method is called
	 ...
	 * sets all the references to the <CODE>SnmpSession</CODE> object to
	 * <CODE>null</CODE>.
	 */
	@Override
