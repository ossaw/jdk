 ...
 * <P>
 * When this SNMP protocol adaptor is started it creates a datagram socket and
 * is able to receive requests and send traps or inform requests. When it is
 * stopped, the socket is closed and neither requests and nor traps/inform
 ...
 * </UL>
 * The default object name is defined by
 * {@link com.sun.jmx.snmp.ServiceName#DOMAIN
 * com.sun.jmx.snmp.ServiceName.DOMAIN} and
 * {@link com.sun.jmx.snmp.ServiceName#SNMP_ADAPTOR_SERVER
 * com.sun.jmx.snmp.ServiceName.SNMP_ADAPTOR_SERVER}.
 * <P>
 * The SNMP protocol adaptor supports versions 1 and 2 of the SNMP protocol in a
 * stateless way: when it receives a v1 request, it replies with a v1 response,
 * when it receives a v2 request it replies with a v2 response. <BR>
 * The method {@link #snmpV1Trap snmpV1Trap} sends traps using SNMP v1 format.
 * The method {@link #snmpV2Trap snmpV2Trap} sends traps using SNMP v2 format.
 * The method {@link #snmpInformRequest snmpInformRequest} sends inform requests
 * using SNMP v2 format.
 * <P>
 * To receive data packets, the SNMP protocol adaptor uses a buffer which size
 * can be configured using the property <CODE>bufferSize</CODE> (default value
 * is 1024). Packets which do not fit into the buffer are rejected. Increasing
 * <CODE>bufferSize</CODE> allows the exchange of bigger packets. However, the
 * underlying networking system may impose a limit on the size of UDP packets.
 * Packets which size exceed this limit will be rejected, no matter what the
 * value of <CODE>bufferSize</CODE> actually is.
 * <P>
 * An SNMP protocol adaptor may serve several managers concurrently. The number
 * of concurrent managers can be limited using the property
 * <CODE>maxActiveClientCount</CODE>.
 * <p>
 * The SNMP protocol adaptor specifies a default value (10) for the
 * <CODE>maxActiveClientCount</CODE> property. When the adaptor is stopped, the
 * active requests are interrupted and an error result is sent to the managers.
 * <p>
...
public class SnmpAdaptorServer extends CommunicatorServer implements SnmpAdaptorServerMBean,
        MBeanRegistration, SnmpDefinitions, SnmpMibHandler {

    // PRIVATE VARIABLES
    // ------------------

    /**
     * Port number for sending SNMP traps. <BR>
     * The default value is 162.
     */
    private int trapPort = 162;

    /**
     * Port number for sending SNMP inform requests. <BR>
     * The default value is 162.
     */
    private int informPort = 162;

    /**
     * The <CODE>InetAddress</CODE> used when creating the datagram socket. <BR>
     * It is specified when creating the SNMP protocol adaptor. If not
     * specified, the local host machine is used.
     */
    ...
    /**
     * The enterprise OID. <BR>
     * The default value is "1.3.6.1.4.1.42".
     */
    private SnmpOid enterpriseOid = new SnmpOid("1.3.6.1.4.1.42");

    /**
     * The buffer size of the SNMP protocol adaptor. This buffer size is used
     * for both incoming request and outgoing inform requests. <BR>
    ...
    /**
     * Number of times to try an inform request before giving up. The default
     * number is 3.
     */
    private int maxTries = 3;

    /**
     * The amount of time to wait for an inform response from the manager. The
     * default amount of time is 3000 millisec.
     */
    ...
    static {
        ...
        if (s != null) {
            try {
                ...
                SNMP_ADAPTOR_LOGGER.logp(Level.FINER, SnmpAdaptorServer.class.getName(), "<static init>",
                        "Got wrong value for com.sun.jmx.snmp.threadnumber: " + s
    ...
    /**
     * Initializes this SNMP protocol adaptor using the default port (161). Use
     * the {@link com.sun.jmx.snmp.IPAcl.SnmpAcl} default implementation of the
     * <CODE>InetAddressAcl</CODE> interface.
     */
    public SnmpAdaptorServer() {
        this(true, null, com.sun.jmx.snmp.ServiceName.SNMP_ADAPTOR_PORT, null);
    }

    /**
     * Initializes this SNMP protocol adaptor using the specified port. Use the
     * {@link com.sun.jmx.snmp.IPAcl.SnmpAcl} default implementation of the
     * <CODE>InetAddressAcl</CODE> interface.
     *
     * @param port
    ...
    /**
     * Initializes this SNMP protocol adaptor using the default port (161) and
     * the specified IP address based ACL implementation.
     *
     * @param acl
    ...
    public SnmpAdaptorServer(InetAddressAcl acl) {
        this(false, acl, com.sun.jmx.snmp.ServiceName.SNMP_ADAPTOR_PORT, null);
    }

    /**
     * Initializes this SNMP protocol adaptor using the default port (161) and
     * the specified <CODE>InetAddress</CODE>. Use the
     * {@link com.sun.jmx.snmp.IPAcl.SnmpAcl} default implementation of the
     * <CODE>InetAddressAcl</CODE> interface.
     *
     * @param addr
    ...
    /**
     * Initializes this SNMP protocol adaptor using the specified port and the
     * specified <CODE>InetAddress</CODE>. Use the
     * {@link com.sun.jmx.snmp.IPAcl.SnmpAcl} default implementation of the
     * <CODE>InetAddressAcl</CODE> interface.
     *
     * @param port
    ...
    public SnmpAdaptorServer(InetAddressAcl acl, InetAddress addr) {
        this(false, acl, com.sun.jmx.snmp.ServiceName.SNMP_ADAPTOR_PORT, addr);
    }
    ...
    /**
     * Initializes this SNMP protocol adaptor using the specified port and the
     * specified <CODE>InetAddress</CODE>. This constructor allows to initialize
     * an SNMP adaptor without using the ACL mechanism (by setting the
     * <CODE>useAcl</CODE> parameter to false). <br>
     * This constructor must be used in particular with a platform that does not
    ...
    /**
     * Gets the number of managers currently being processed by this SNMP
     * protocol adaptor.
     *
     * @return The number of managers currently being processed by this SNMP
     *         protocol adaptor.
     */
    @Override
    ...
    /**
     * Returns the port used by this SNMP protocol adaptor for sending traps. By
     * default, port 162 is used.
     *
    ...
    /**
     * Returns the port used by this SNMP protocol adaptor for sending inform
     * requests. By default, port 162 is used.
     *
    ...
    /**
     * Sets the port used by this SNMP protocol adaptor for sending inform
     * requests.
     *
     * @param port
    ...
    /**
     * Returns the buffer size of this SNMP protocol adaptor. This buffer size
     * is used for both incoming request and outgoing inform requests. By
     * default, buffer size 1024 is used.
     *
    ...
    /**
     * Sets the buffer size of this SNMP protocol adaptor. This buffer size is
    ...
    /**
     * Gets the number of times to try sending an inform request before giving
     * up. By default, a maximum of 3 tries is used.
     * 
    ...
    /**
     * Changes the maximun number of times to try sending an inform request
     * before giving up.
     * 
     * @param newMaxTries
    ...
    /**
     * Gets the timeout to wait for an inform response from the manager. By
     * default, a timeout of 3 seconds is used.
     * 
     ...
     * <P>
     * When this feature is enabled, the SNMP protocol adaptor sends an
     * <CODE>authenticationFailure</CODE> trap each time an authentication
     * fails.
     * <P>
    ...
    /**
     * Returns <code>true</code> if this SNMP protocol adaptor sends a response
     * in case of authentication failure.
     * <P>
     * When this feature is enabled, the SNMP protocol adaptor sends a response
     * with <CODE>noSuchName</CODE> or <CODE>readOnly</CODE> when the
     * authentication failed. If the flag is disabled, the SNMP protocol adaptor
     * trashes the PDU silently.
     * <P>
    ...
    /**
     * Returns the enterprise OID. It is used by {@link #snmpV1Trap snmpV1Trap}
     * to fill the 'enterprise' field of the trap request.
     *
    ...
    /**
     * Returns the <CODE>snmpSilentDrops</CODE> value defined in RFC 1907
     * NMPv2-MIB .
     *
    ...
    /**
     * Returns the <CODE>snmpProxyDrops</CODE> value defined in RFC 1907
     * NMPv2-MIB .
     *
     ...
     * registered in the MBean server. If the name of the SNMP protocol adaptor
     * MBean is not specified, it is initialized with the default value:
     * {@link com.sun.jmx.snmp.ServiceName#DOMAIN
     * com.sun.jmx.snmp.ServiceName.DOMAIN}:
     * {@link com.sun.jmx.snmp.ServiceName#SNMP_ADAPTOR_SERVER
     * com.sun.jmx.snmp.ServiceName.SNMP_ADAPTOR_SERVER}. If any exception is
     * raised, the SNMP protocol adaptor MBean will not be registered in the
     * MBean server.
     *
     * @param server
    ...
    /**
     * Adds a new MIB in the SNMP MIB handler. This method is to be called to
     * set a specific agent to a specific OID. This can be useful when dealing
     * with MIB overlapping. Some OID can be implemented in more than one MIB.
     ...
     * <CODE>contextName</CODE> is useless and this method is equivalent to
    ...
    /**
     * Removes the specified MIB from the SNMP protocol adaptor. In SNMP V1 and
     * V2 the <CODE>contextName</CODE> is useless and this method is equivalent
    ...
    protected void doBind() throws CommunicationException, InterruptedException {
        ...
        try {
            ...
            if (e.getMessage().equals(InterruptSysCallMsg))
                ...
                if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINEST)) {
                    SNMP_ADAPTOR_LOGGER.logp(Level.FINEST, dbgTag, "doBind", "cannot bind on port " + port);
                }
    ...
    /**
     * Return the actual port to which the adaptor is bound. Can be different
     * from the port given at construction time if that port number was 0.
     * 
    ...
    private void createSnmpRequestHandler(SnmpAdaptorServer server, int id, DatagramSocket s,
        ...
        final SnmpRequestHandler handler = new SnmpRequestHandler(this, id, s, p, tree, m, a, factory,
                dataFactory, f, n);
    ...
    protected void doReceive() throws CommunicationException, InterruptedException {
        ...
        try {
            ...
            createSnmpRequestHandler(this, servedClientCount, socket, packet, root, mibs, ipacl, pduFactory,
     ...
     * giving up. We attempt only once...
     * 
     * @return 1
     **/
    @Override
        ...
        if ((state == ONLINE) || (state == STARTING)) {
            ...
            try {
                ...
                try {
                    ...
                    if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINER)) {
                        SNMP_ADAPTOR_LOGGER.logp(Level.FINER, dbgTag, "stop", "Sending: using port " + port);
                    }
                    sn.send(pk);
                } finally {
                    sn.close();
                ...
                if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINEST)) {
                    SNMP_ADAPTOR_LOGGER.logp(Level.FINEST, dbgTag, "stop", "Got unexpected Throwable", e);
                }
            }
        }
    }
     ...
     * The trap is sent to each destination defined in the ACL file (if
     * available). If no ACL file or no destinations are available, the trap is
    ...
    public void snmpV1Trap(int generic, int specific, SnmpVarBindList varBindList) throws IOException,
        ...
        if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINER)) {
            SNMP_ADAPTOR_LOGGER.logp(Level.FINER, dbgTag, "snmpV1Trap", "generic=" + generic + ", specific="
                    + specific);
        }
    ...
    /**
     * Sends a trap using SNMP V1 trap format. <BR>
     * The trap is sent to the specified <CODE>InetAddress</CODE> destination
     ...
     * Sends a trap using SNMP V1 trap format. <BR>
     * The trap is sent to the specified <CODE>InetAddress</CODE> destination
     * using the specified parameters (and the ACL file is not used). Note that
     * if the specified <CODE>InetAddress</CODE> destination is null, then the
    ...
    public void snmpV1Trap(InetAddress addr, SnmpIpAddress agentAddr, String cs, SnmpOid enterpOid,
            int generic, int specific, SnmpVarBindList varBindList, SnmpTimeticks time) throws IOException,
            SnmpStatusException {
    ...
    /**
     * Sends a trap using SNMP V1 trap format. <BR>
     * The trap is sent to the specified <CODE>SnmpPeer</CODE> destination. The
     * community string used is the one located in the <CODE>SnmpPeer</CODE>
    ...
    public void snmpV1Trap(SnmpPeer peer, SnmpIpAddress agentAddr, SnmpOid enterpOid, int generic,
        ...
        snmpV1Trap(peer.getDestAddr(), peer.getDestPort(), agentAddr, p.getRdCommunity(), enterpOid, generic,
                specific, varBindList, time);
    }

    private void snmpV1Trap(InetAddress addr, int port, SnmpIpAddress agentAddr, String cs, SnmpOid enterpOid,
            int generic, int specific, SnmpVarBindList varBindList, SnmpTimeticks time) throws IOException,
            SnmpStatusException {
        ...
        if (agentAddr == null) {
            ...
            try {
                final InetAddress inetAddr = (address != null) ? address : InetAddress.getLocalHost();
     ...
     * The trap is sent to the specified <CODE>SnmpPeer</CODE> destination. <BR>
     * The community string used is the one located in the <CODE>SnmpPeer</CODE>
     * parameters (<CODE>SnmpParameters.getRdCommunity() </CODE>). <BR>
     ...
     * Sends a trap using SNMP V2 trap format. <BR>
     * The trap is sent to the specified <CODE>InetAddress</CODE> destination
     * using the specified community string (and the ACL file is not used). <BR>
     * The variable list included in the outgoing trap is composed of the
     * following items:
     * <UL>
     * <LI><CODE>sysUpTime.0</CODE> with its current value</LI>
    ...
    private void snmpV2Trap(InetAddress addr, int port, String cs, SnmpOid trapOid,
            SnmpVarBindList varBindList, SnmpTimeticks time) throws IOException, SnmpStatusException {
    ...
    private void sendTrapPdu(SnmpPduPacket pdu) throws SnmpStatusException, IOException {
        ...
        try {
            ...
            if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINEST)) {
                SNMP_ADAPTOR_LOGGER.logp(Level.FINEST, dbgTag, "sendTrapPdu", "Trap pdu is too big. "
    ...
    private void sendTrapMessage(SnmpMessage msg) throws IOException, SnmpTooBigException {
        ...
        if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINER)) {
            SNMP_ADAPTOR_LOGGER.logp(Level.FINER, dbgTag, "sendTrapMessage", "sending trap to " + msg.address
                    + ":" + msg.port);
        }
        trapSocket.send(packet);
        if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINER)) {
            SNMP_ADAPTOR_LOGGER.logp(Level.FINER, dbgTag, "sendTrapMessage", "sent to " + msg.address + ":"
                    + msg.port);
        }
        snmpOutTraps++;
        snmpOutPkts++;
    ...
    synchronized void openTrapSocketIfNeeded() throws SocketException {
        if (trapSocket == null) {
            ...
            if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINER)) {
                SNMP_ADAPTOR_LOGGER.logp(Level.FINER, dbgTag, "openTrapSocketIfNeeded", "using port "
     ...
     * The inform request is sent to each destination defined in the ACL file
     * (if available). If no ACL file or no destinations are available, the
     * inform request is sent to the local host. <BR>
     * The variable list included in the outgoing inform is composed of the
     * following items:
     * <UL>
     * <LI><CODE>sysUpTime.0</CODE> with its current value</LI>
     * <LI><CODE>snmpTrapOid.0</CODE> with the value specified by
    ...
    public Vector<SnmpInformRequest> snmpInformRequest(SnmpInformHandler cb, SnmpOid trapOid,
        ...
        if (ipacl != null) {
            ...
            while (ed.hasMoreElements()) {
                ...
                while (ec.hasMoreElements()) {
                    cs = ec.nextElement();
                    informReqList.addElement(informSession.makeAsyncRequest(addr, cs, cb, fullVbl,
                            getInformPort()));
                }
            }
        }
    ...
    /**
     * Sends an inform using SNMP V2 inform request format. <BR>
     * The inform is sent to the specified <CODE>InetAddress</CODE> destination
     * using the specified community string. <BR>
     * The variable list included in the outgoing inform is composed of the
     * following items:
     * <UL>
     * <LI><CODE>sysUpTime.0</CODE> with its current value</LI>
    ...
    /**
     * Sends an inform using SNMP V2 inform request format. <BR>
     * The inform is sent to the specified <CODE>SnmpPeer</CODE> destination.
     * <BR>
     * The community string used is the one located in the <CODE>SnmpPeer</CODE>
     * parameters (<CODE>SnmpParameters.getInformCommunity() </CODE>). <BR>
     * The variable list included in the outgoing inform is composed of the
     * following items:
     * <UL>
     * <LI><CODE>sysUpTime.0</CODE> with its current value</LI>
    ...
    public SnmpInformRequest snmpInformRequest(SnmpPeer peer, SnmpInformHandler cb, SnmpOid trapOid,
        ...
        SnmpParameters p = (SnmpParameters) peer.getParams();
        return snmpInformRequest(peer.getDestAddr(), peer.getDestPort(), p.getInformCommunity(), cb, trapOid,
                varBindList);
    }
    ...
    private SnmpInformRequest snmpInformRequest(InetAddress addr, int port, String cs, SnmpInformHandler cb,
            SnmpOid trapOid, SnmpVarBindList varBindList) throws IllegalStateException, IOException,
            SnmpStatusException {
    ...
    /**
     * Gets the IP address to bind. This getter is used to initialize the
     * DatagramSocket in the SnmpSocket object created for the inform request
     * stuff.
     */
    ...
    /**
     * Finalizer of the SNMP protocol adaptor objects. This method is called by
     * the garbage collector on an object when garbage collection determines
    ...
    /**
     * Returns the time (in hundreths of second) elapsed since the SNMP protocol
     * adaptor startup.
     */
    long getSysUpTime() {
