...
class SnmpSubRequestHandler implements SnmpDefinitions, Runnable {
	 ...
	 */
	protected SnmpSubRequestHandler(SnmpEngine engine, SnmpIncomingRequest incRequest,
			SnmpMibAgent agent, SnmpPdu req) {
		this(agent, req);
	 ...
	 */
	protected SnmpSubRequestHandler(SnmpEngine engine, SnmpIncomingRequest incRequest,
			SnmpMibAgent agent, SnmpPdu req, boolean nouse) {
	...
	SnmpMibRequest createMibRequest(Vector<SnmpVarBind> vblist, int protocolVersion,
			Object userData) {
		...
		if (incRequest != null) {
			result = SnmpMibAgent.newMibRequest(engine, reqPdu, vblist, protocolVersion, userData,
					incRequest.getPrincipal(), incRequest.getSecurityLevel(),
					incRequest.getSecurityModel(), incRequest.getContextName(),
		...
		try {
			...
			try {
				...
				case pduGetRequestPdu:
					...
					if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINER)) {
						SNMP_ADAPTOR_LOGGER.logp(Level.FINER, SnmpSubRequestHandler.class.getName(),
								"run", "[" + Thread.currentThread() + "]:get operation on "
										+ agent.getMibName());
					}
				...
				case pduGetNextRequestPdu:
					if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINER)) {
						SNMP_ADAPTOR_LOGGER.logp(Level.FINER, SnmpSubRequestHandler.class.getName(),
								"run", "[" + Thread.currentThread() + "]:getNext operation on "
										+ agent.getMibName());
					}
					// #ifdef DEBUG
				...
				case pduSetRequestPdu:
					if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINER)) {
						SNMP_ADAPTOR_LOGGER.logp(Level.FINER, SnmpSubRequestHandler.class.getName(),
								"run", "[" + Thread.currentThread() + "]:set operation on "
										+ agent.getMibName());
					}
				...
				case pduWalkRequest:
					if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINER)) {
						SNMP_ADAPTOR_LOGGER.logp(Level.FINER, SnmpSubRequestHandler.class.getName(),
								"run", "[" + Thread.currentThread() + "]:check operation on "
										+ agent.getMibName());
					}
				...
				default:
					if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINEST)) {
						SNMP_ADAPTOR_LOGGER.logp(Level.FINEST,
								...
								"[" + Thread.currentThread() + "]:unknown operation (" + type
										+ ") on " + agent.getMibName());
					}
	...
	static final int mapErrorStatusToV1(int errorStatus, int reqPduType) {
		...
		} else if ((errorStatus == SnmpDefinitions.snmpRspAuthorizationError)
		...
		} else if ((errorStatus == SnmpDefinitions.snmpRspResourceUnavailable)
	...
	/**
	 * The method returns the error status of the operation. The method takes
	 * into account the protocol version.
	 */
	...
	/**
	 * Store the operation type. Remember if the type is Walk, it means that we
	 * have to invoke the check method ...
	 */
	...
	/**
	 * The varbind list specific to the current sub request. The vector must
	 * contain object of type SnmpVarBind.
	 */
