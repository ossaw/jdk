...
/**
 * Provides the callback methods that are required to be implemented by the
 * application when an inform response is received by the agent.
 * <P>
 * Each inform request can be provided with an object that implements this
 * callback interface. An application then uses the SNMP adaptor to start an
 * SNMP inform request, which marks the request as active. The methods in this
 * callback interface get invoked when any of the following happens:
 * <P>
 * <UL>
 * <LI>The agent receives the SNMP inform response.
 * <LI>The agent does not receive any response within a specified time and the
 * number of tries have exceeded the limit (timeout condition).
...
public interface SnmpInformHandler extends SnmpDefinitions {
	...
	/**
	 * This callback is invoked when a manager responds to an SNMP inform
	 * request. The callback should check the error status of the inform request
	 ...
	 * @param vblist
	 *            The <CODE>Response varBind</CODE> list for the successful
	 *            request.
	 */
	 ...
	 * specified timeout value to the SNMP inform request. The number of tries
