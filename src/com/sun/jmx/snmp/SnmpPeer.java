...
/**
 * Holds information about an SNMP agent. This information is used to
 * communicate with the agent. These are the IP address, port number, SNMP
 * parameters, and peer channel parameters (such as the maximum request packet
 * size, maximum number of variable bindings in a packet, retries, and
 * timeouts). Changing these would affect all active requests.
 * <P>
...
public class SnmpPeer implements Serializable {
	...
	/**
	 * The default SNMP packet size of an SNMP request (2 * 1024). <BR>
	 * The maximum size is initially set to Ethernet maximum transfer unit
	 * (MTU).
	 */
	...
	/**
	 * The default SNMP packet size of an SNMP response (8 * 1024). <BR>
	 * This will be the default size that the session socket uses when receiving
	 * a packet.
	 */
	...
	/**
	 * The maximum number of variable bindings that can be packed into a
	 * request. The default value is 25.
	 */
	...
	/**
	 * The amount of time to wait for a response from the peer. The default
	 * amount of time is 3000 millisec.
	 */
	private int timeout = 3000;

	/**
	 * The PDU factory. The default factory is an instance of
	 * <CODE>SnmpPduFactoryBER</CODE>.
	 */
	...
	/**
	 * Maximum packet size of the request PDU. This can be set by the user. If
	 * the request crosses this limit while encoding, the request is
	 * automatically split into multiple small request. Each of these requests
	 * will again be within this limit. The default value is (2 * 1024).
	 */
	...
	/**
	 * Sets a specific IP address to which the peer will communicate. Typically
	 * used to set an alternate IP address or a specific address which is known
	 * to respond to requests. The IP address <CODE>String</CODE> can either be
	 * a machine name, such as <CODE>ibiza</CODE>, or a <CODE>String</CODE>
	 ...
	 * Useful when you want to know which IP address is used when the address
	 * was resolved using a DNS name.
	 * 
	...
	/**
	 * Causes all subsequent requests to go to the new address obtained from the
	 * specified list of alternate addresses. If it reaches the end of list, it
	 * starts again at the first address.
	 */
	...
	/**
	 * Determines whether an SNMP <CODE>set</CODE> operation is allowed with
	 * this peer object. For now it just makes sure a parameter is configured
	 * for a write operation.
	 * 
	 * @return <CODE>true</CODE> if SNMP <CODE>set</CODE> is allowed and the
	 *         parameter is configured, <CODE>false</CODE> otherwise.
	 */
	...
	/**
	 * Gets the list of alternate <CODE>InetAddress</CODE> configured for this
	 * peer.
	 * 
	...
	/**
	 * Gets the destination port number of the peer to which SNMP requests are
	 * to be sent.
	 * 
	...
	/**
	 * Returns the <CODE>String</CODE> representation for this
	 * <CODE>SnmpPeer</CODE>.
	 * 
	 ...
	 * 
	 * @return The maximum variable bindings that can be encoded into a request
	 *         packet.
	 */
	...
	/**
	 * Configures the maximum number of variable bindings that can be sent to a
	 * peer.
	 * 
	 * @param limit
	 *            The desired limit.
	...
	/**
	 * Configures the maximum packet size that can be used when generating an
	 * SNMP request.
	 * 
	 * @param newsize
	...
	/**
	 * Finalizer of the <CODE>SnmpPeer</CODE> objects. This method is called by
	 * the garbage collector on an object when garbage collection determines
	 * that there are no more references to the object.
	 * <P>
	 * Sets all the references to this SNMP peer object to <CODE>null</CODE>.
	 */
	@Override
