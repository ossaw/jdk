...
/**
 * Represent an Access Control List (ACL) which is used to guard access to http
 * adaptor.
 * <P>
 ...
 * communities associated with a particular principal. (A principal represents
 * an entity such as a host or a group of host). Additionally, each ACL entry is
...
class AclImpl extends OwnerImpl implements Acl, Serializable {
    ...
    public boolean addEntry(Principal caller, AclEntry entry) throws NotOwnerException {
         ...
         * AclEntry ent = (AclEntry) e.nextElement(); if
         * (ent.getPrincipal().equals(entry.getPrincipal())) return false; }
         */
     ...
     * principals, represented by the Group interface.)</LI>
     * </UL>
     * 
     * @param user
