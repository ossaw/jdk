...
/**
 * This class is an internal class which is used to represent RowStatus codes as
 * defined in RFC 2579.
 *
 * It defines an additional code, <i>unspecified</i>, which is implementation
 * specific, and is used to identify unspecified actions (when for instance the
 * RowStatus variable is not present in the varbind list) or uninitialized
 * values.
 *
 * mibgen does not generate objects of this class but any variable using the
 * RowStatus textual convention can be converted into an object of this class
 * thanks to the <code>EnumRowStatus(Enumerated valueIndex)</code> constructor.
 *
 * <p>
...
public class EnumRowStatus extends Enumerated implements Serializable {
	...
	/**
	 * This value is SNMP Runtime implementation specific, and is used to
	 * identify unspecified actions (when for instance the RowStatus variable is
	 * not present in the varbind list) or uninitialized values.
	 */
	public final static int unspecified = 0;

	/**
	 * This value corresponds to the <i>active</i> RowStatus, as defined in RFC
	 * 2579 from SMIv2:
	 * <ul>
	 * <i>active</i> indicates that the conceptual row is available for use by
	 * the managed device;
	 * </ul>
	 */
	...
	/**
	 * This value corresponds to the <i>notInService</i> RowStatus, as defined
	 * in RFC 2579 from SMIv2:
	 * <ul>
	 * <i>notInService</i> indicates that the conceptual row exists in the
	 * agent, but is unavailable for use by the managed device;
	 * <i>notInService</i> has no implication regarding the internal consistency
	 * of the row, availability of resources, or consistency with the current
	...
	/**
	 * This value corresponds to the <i>notReady</i> RowStatus, as defined in
	 * RFC 2579 from SMIv2:
	 * <ul>
	 * <i>notReady</i> indicates that the conceptual row exists in the agent,
	 * but is missing information necessary in order to be available for use by
	 * the managed device (i.e., one or more required columns in the conceptual
	 * row have not been instantiated);
	 * </ul>
	 */
	public final static int notReady = 3;

	/**
	 * This value corresponds to the <i>createAndGo</i> RowStatus, as defined in
	 * RFC 2579 from SMIv2:
	 * <ul>
	 * <i>createAndGo</i> is supplied by a management station wishing to create
	 * a new instance of a conceptual row and to have its status automatically
	 * set to active, making it available for use by the managed device;
	 * </ul>
	 */
	public final static int createAndGo = 4;

	/**
	 * This value corresponds to the <i>createAndWait</i> RowStatus, as defined
	 * in RFC 2579 from SMIv2:
	 * <ul>
	 * <i>createAndWait</i> is supplied by a management station wishing to
	 * create a new instance of a conceptual row (but not make it available for
	...
	/**
	 * This value corresponds to the <i>destroy</i> RowStatus, as defined in RFC
	 * 2579 from SMIv2:
	 * <ul>
	 * <i>destroy</i> is supplied by a management station wishing to delete all
	 * of the instances associated with an existing conceptual row.
	 * </ul>
	 */
