...
final class ConvertingMethod {
	...
	/*
	 * Check that this method will be callable when we are going from open types
	 * to Java types, for example when we are going from an MXBean wrapper to
	 * the underlying resource. The parameters will be converted to Java types,
	 * so they must be "reconstructible". The return value will be converted to
	 * an Open Type, so if it is convertible at all there is no further check
	 * needed.
	 */
	...
	/*
	 * Check that this method will be callable when we are going from Java types
	 * to open types, for example when we are going from an MXBean proxy to the
	 * open types that it will be mapped to. The return type will be converted
	 * back to a Java type, so it must be "reconstructible". The parameters will
	 * be converted to open types, so if it is convertible at all there is no
	...
	Object invokeWithOpenReturn(MXBeanLookup lookup, Object obj, Object[] params)
	...
	private Object invokeWithOpenReturn(Object obj, Object[] params)
		...
		try {
			...
			final String msg = methodName() + ": cannot convert parameters " + "from open values: "
					+ e;
		...
		try {
			...
			// probably can't happen
			final String msg = methodName() + ": cannot convert return " + "value to open value: "
					+ e;
			throw new MBeanException(e, msg);
		}
	}
