...
/**
 * Base class for MBeans. There is one instance of this class for every Standard
 * MBean and every MXBean. We try to limit the amount of information per
 ...
 * derived from annotations on the interface. This MBeanInfo can never be the
 * MBeanInfo for any actual MBean, because an MBeanInfo's getClassName() is the
 ...
 * ClassLoader or any other classes loaded by its ClassLoader. Therefore the
 ...
 * concrete class's constructors. This MBeanInfo can be shared between all
 * instances of the concrete class specifying the same interface, except
 * instances that are NotificationBroadcasters. NotificationBroadcasters supply
 * the MBeanNotificationInfo[] in the MBeanInfo based on the instance method
 * NotificationBroadcaster.getNotificationInfo(), so two instances of the same
 ...
 * reduce the cost of doing this by detecting when the Standard MBean is a
 * subclass of NotificationBroadcasterSupport that does not override
...
public abstract class MBeanSupport<M> implements DynamicMBean2,
    ...
    /**
     * Return a cookie for this MBean. This cookie will be passed to MBean
     * method invocations where it can supply additional information to the
     * invocation. For example, with MXBeans it can be used to supply the
     * MXBeanLookup context for resolving inter-MXBean references.
     */
