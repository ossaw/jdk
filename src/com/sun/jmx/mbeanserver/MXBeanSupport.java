...
public class MXBeanSupport extends MBeanSupport<ConvertingMethod> {
    ...
    /**
     * <p>
     * Construct an MXBean that wraps the given resource using the given MXBean
     * interface.
     * </p>
     * 
     * @param resource
    ...
    static <T> Class<? super T> findMXBeanInterface(Class<T> resourceClass) {
        ...
        if (candidates.iterator().hasNext()) {
            ...
            final String msg = "Class " + resourceClass.getName() + " is not a JMX compliant MXBean";
            throw new IllegalArgumentException(msg);
        }
    }

    /*
     * Return all interfaces inherited by this class, directly or indirectly
     * through the parent class and interfaces.
     */
    ...
    /*
     * The sequence of events for tracking inter-MXBean references is relatively
     * complicated. We use the magical preRegister2 method which the MBeanServer
     * knows about. The steps during registration are: (1) Call user
     * preRegister, if any. If exception, abandon. (2) Call preRegister2 and
     * hence this register method. If exception, call postRegister(false) and
     * abandon. (3) Try to register the MBean. If exception, call
