 ...
 * IdentityHashMap) but where the presence of an object as a key in the map does
 * not prevent it being garbage collected (like WeakHashMap). This class does
 ...
 */
/*
 * The approach is to wrap each key in a WeakReference and use the wrapped value
 * as a key in an ordinary HashMap. The WeakReference has to be a subclass
 * IdentityWeakReference (IWR) where two IWRs are equal if they refer to the
...
class WeakIdentityHashMap<K, V> {
	...
	/**
	 * WeakReference where equals and hashCode are based on the referent. More
	 * precisely, two objects are equal if they are identical or if they both
	 * have the same non-null referent. The hashCode is the value the original
	 * referent had. Even if the referent is cleared, the hashCode remains.
	 * Thus, objects of this class can be used as keys in hash-based maps and
	 * sets.
	 */
