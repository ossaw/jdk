...
final class PerInterface<M> {
	...
	/*
	 * This method is called when invoke doesn't find the named method. Before
	 * throwing an exception, we check to see whether the jmx.invoke.getters
	 * property is set, and if so whether the method being invoked might be a
	 * getter or a setter. If so we invoke it and return the result. This is for
	 ...
	 * interface is being introspected and not thereafter. Previously, the
	 * property was consulted on every invocation. So this simpler
