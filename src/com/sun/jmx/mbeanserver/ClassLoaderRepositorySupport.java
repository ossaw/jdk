...
/**
 * This class keeps the list of Class Loaders registered in the MBean Server. It
 * provides the necessary methods to load classes using the registered Class
 * Loaders.
 *
 * @since 1.5
 */
final class ClassLoaderRepositorySupport implements
     ...
     * the correct entry when unregistering an MBean that is a ClassLoader. The
     * same object could be registered under two different names (even though
     * this is not recommended) so if we did not do this we could disturb the
     * defined semantics for the order of ClassLoaders in the repository.
     */
     ...
     * (loadClass) which are by definition linear.
     */
    private LoaderEntry[] loaders = EMPTY_LOADER_ARRAY;

    /**
     * Same behavior as add(Object o) in {@link java.util.List}. Replace the
     ...
     * is removed, regardless of whether ClassLoader values match. (In fact, the
     * ClassLoader parameter will usually be null in this case.)
     **/
    ...
    private Class<?> loadClass(final LoaderEntry list[], final String className,
        ...
        for (int i = 0; i < size; i++) {
            try {
                 ...
                 * where we invoked the method loadClass(className, null) to
                 * prevent infinite recursion. But the rule whereby the MLet
                 * only consults loaders that precede it in the CLR (via
                 * loadClassBefore) means that the recursion can't happen, and
                 * the test here caused some legitimate classloading to fail.
                 * For example, if you have dependencies C->D->E with loaders {E
                 * D C} in the CLR in that order, you would expect to be able to
                 * load C. The problem is that while resolving D, CLR delegation
                 * is disabled, so it can't find E.
                 */
