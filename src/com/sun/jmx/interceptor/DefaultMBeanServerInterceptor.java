 ...
 * contains the methods necessary for the creation, registration, and deletion
 * of MBeans as well as the access methods for registered MBeans. This is the
 * core component of the JMX infrastructure.
 * <P>
 ...
 * <CODE>ObjectName</CODE> the <CODE>ObjectName</CODE> of the
 * {@link javax.management.MBeanServerDelegate MBeanServerDelegate}. This
...
public class DefaultMBeanServerInterceptor implements MBeanServerInterceptor {
    ...
    private final transient WeakHashMap<ListenerWrapper, WeakReference<ListenerWrapper>> listenerWrappers = new WeakHashMap<ListenerWrapper, WeakReference<ListenerWrapper>>();
    ...
    /**
     * Creates a DefaultMBeanServerInterceptor with the specified repository
     * instance.
     * <p>
     * Do not forget to call <code>initialize(outer,delegate)</code> before
     * using this object.
     * 
     * @param outer
     ...
     *        instantiate MBeans
     *        and take care of class loading issues.
     * @param repository
     *        The repository to use for this MBeanServer.
     */
    public DefaultMBeanServerInterceptor(MBeanServer outer, MBeanServerDelegate delegate,
            MBeanInstantiator instantiator, Repository repository) {
        if (outer == null)
    ...
    public ObjectInstance createMBean(String className, ObjectName name, Object[] params, String[] signature)
        ...
        try {
            ...
            /*
             * Can only happen if loaderName doesn't exist, but we just passed
             * null, so we shouldn't get this exception.
             */
    ...
    private ObjectInstance createMBean(String className, ObjectName name, ObjectName loaderName,
            boolean withDefaultLoaderRepository, Object[] params, String[] signature)
            throws ReflectionException, InstanceAlreadyExistsException, MBeanRegistrationException,
            MBeanException, NotCompliantMBeanException, InstanceNotFoundException {
        ...
        if (name != null) {
            if (name.isPattern()) {
                final RuntimeException wrapped = new IllegalArgumentException("Invalid name->" + name
                        .toString());
        ...
        if (withDefaultLoaderRepository) {
            if (MBEANSERVER_LOGGER.isLoggable(Level.FINER)) {
                MBEANSERVER_LOGGER.logp(Level.FINER, DefaultMBeanServerInterceptor.class.getName(),
                        "createMBean", "ClassName = " + className + ", ObjectName = " + name);
            }
        ...
        Object moi = instantiator.instantiate(theClass, params, signature, server.getClass()
                .getClassLoader());
    ...
    public void unregisterMBean(ObjectName name) throws InstanceNotFoundException,
        ...
        /*
         * The semantics of preDeregister are tricky. If it throws an exception,
         * then the unregisterMBean fails. This allows an MBean to refuse to be
         * unregistered. If it returns successfully, then the unregisterMBean
         * can proceed. In this case the preDeregister may have cleaned up some
         * state, and will not expect to be called a second time. So if two
         * threads try to unregister the same MBean at the same time then one of
         * them must wait for the other one to either (a) call preDeregister and
         * get an exception or (b) call preDeregister successfully and
         * unregister the MBean. Suppose thread T1 is unregistering an MBean and
         * thread T2 is trying to unregister the same MBean, so waiting for T1.
         * Then a deadlock is possible if the preDeregister for T1 ends up
         * needing a lock held by T2. Given the semantics just described, there
         * does not seem to be any way to avoid this. This will not happen to
         * code where it is clear for any given MBean what thread may unregister
         * that MBean.
         * On the other hand we clearly do not want a thread that is
         * unregistering MBean A to have to wait for another thread that is
         * unregistering another MBean B (see bug 6318664). A deadlock in this
         * situation could reasonably be considered gratuitous. So holding a
         * global lock across the preDeregister call would be bad.
         * So we have a set of ObjectNames that some thread is currently
         * unregistering. When a thread wants to unregister a name, it must
         * first check if the name is in the set, and if so it must wait. When a
         * thread successfully unregisters a name it removes the name from the
         * set and notifies any waiting threads that the set has changed.
         * This implies that we must be very careful to ensure that the name is
         * removed from the set and waiters notified, no matter what code path
         * is taken.
         */
    ...
    private void exclusiveUnregisterMBean(ObjectName name) throws InstanceNotFoundException,
            MBeanRegistrationException {
    ...
    public Object getAttribute(ObjectName name, String attribute) throws MBeanException,
        ...
        if (MBEANSERVER_LOGGER.isLoggable(Level.FINER)) {
            MBEANSERVER_LOGGER.logp(Level.FINER, DefaultMBeanServerInterceptor.class.getName(),
                    "getAttribute", "Attribute = " + attribute + ", ObjectName = " + name);
        }
    ...
    public AttributeList getAttributes(ObjectName name, String[] attributes) throws InstanceNotFoundException,
        ...
        if (name == null) {
            throw new RuntimeOperationsException(new IllegalArgumentException(
                    "ObjectName name cannot be null"),
                    "Exception occurred trying to invoke the getter on the MBean");
        }
    ...
    public void setAttribute(ObjectName name, Attribute attribute) throws InstanceNotFoundException,
        ...
        if (MBEANSERVER_LOGGER.isLoggable(Level.FINER)) {
            MBEANSERVER_LOGGER.logp(Level.FINER, DefaultMBeanServerInterceptor.class.getName(),
                    "setAttribute", "ObjectName = " + name + ", Attribute = " + attribute.getName());
        }
    ...
    /**
     * Register <code>object</code> in the repository, with the given
     * <code>name</code>. This method is called by the various createMBean()
     * flavours and by registerMBean() after all MBean compliance tests have
     * been performed.
     * <p>
     * This method does not performed any kind of test compliance, and the
     * caller should make sure that the given <code>object</code> is MBean
     * compliant.
     * <p>
     ...
     * <code>name</code>.</li>
     * <li>Finally, if the <code>object</code> implements the MBeanRegistration
     * interface, it invokes postRegister() on the object.</li>
     * </ul>
     * 
     * @param object
     *        A reference to a MBean compliant object.
     * @param name
     *        The ObjectName of the <code>object</code> MBean.
     ...
     *            registration.
     **/
    private ObjectInstance registerObject(String classname, Object object, ObjectName name)
    ...
    private ObjectInstance registerDynamicMBean(String classname, DynamicMBean mbean, ObjectName name)
            throws InstanceAlreadyExistsException, MBeanRegistrationException, NotCompliantMBeanException {
        ...
        try {
            ...
            // Returns the resource context that was used.
            // The returned context does nothing for regular MBeans.
            // For ClassLoader MBeans the context makes it possible to register
            // these
    ...
    private static void throwMBeanRegistrationException(Throwable t, String where)
        ...
        if (t instanceof RuntimeException) {
            throw new RuntimeMBeanException((RuntimeException) t, "RuntimeException thrown " + where);
        } else if (t instanceof Error) {
            throw new RuntimeErrorException((Error) t, "Error thrown " + where);
        } else if (t instanceof MBeanRegistrationException) {
            throw (MBeanRegistrationException) t;
        } else if (t instanceof Exception) {
            throw new MBeanRegistrationException((Exception) t, "Exception thrown " + where);
        } else // neither Error nor Exception??
            throw new RuntimeException(t);
    }

    private static ObjectName preRegister(DynamicMBean mbean, MBeanServer mbs, ObjectName name)
    ...
    private static void postRegister(ObjectName logicalName, DynamicMBean mbean, boolean registrationDone,
        ...
        try {
            ...
            MBEANSERVER_LOGGER.fine("While registering MBean [" + logicalName + "]: "
                    + "Exception thrown by postRegister: " + "rethrowing <" + e
            ...
            MBEANSERVER_LOGGER.fine("While registering MBean [" + logicalName + "]: "
                    + "Error thrown by postRegister: " + "rethrowing <" + er
                    + ">, but keeping the MBean registered");
            throw new RuntimeErrorException(er, "Error thrown in postRegister method: " + "rethrowing <" + er
                    + ">, but keeping the MBean registered");
        }
    }
    ...
    private static void postDeregisterInvoke(ObjectName mbean, MBeanRegistration moi) {
        try {
            moi.postDeregister();
        } catch (RuntimeException e) {
            MBEANSERVER_LOGGER.fine("While unregistering MBean [" + mbean + "]: "
                    + "Exception thrown by postDeregister: " + "rethrowing <" + e
                    + ">, although the MBean is succesfully " + "unregistered");
            throw new RuntimeMBeanException(e, "RuntimeException thrown in postDeregister method: "
                    + "rethrowing <" + e + ">, although the MBean is sucessfully unregistered");
        } catch (Error er) {
            MBEANSERVER_LOGGER.fine("While unregistering MBean [" + mbean + "]: "
                    + "Error thrown by postDeregister: " + "rethrowing <" + er
                    + ">, although the MBean is succesfully " + "unregistered");
            throw new RuntimeErrorException(er, "Error thrown in postDeregister method: " + "rethrowing <"
                    + er + ">, although the MBean is sucessfully unregistered");
        }
    }
     ...
     * This is not trivial, because the MBeanServer translates the source of a
     * received notification from a reference to an MBean into the ObjectName of
     * that MBean. While that does make notification sending easier for MBean
     * writers, it comes at a considerable cost. We need to replace the source
     * of a notification, which is basically wrong if there are also listeners
     * registered directly with the MBean (without going through the MBean
     * server). We also need to wrap the listener supplied by the client of the
     * MBeanServer with a listener that performs the substitution before
     * forwarding. This is why we strongly discourage people from putting MBean
     * references in the source of their notifications. Instead they should
     * arrange to put the ObjectName there themselves.
     * However, existing code relies on the substitution, so we are stuck with
     * it.
     * Here's how we handle it. When you add a listener, we make a
     * ListenerWrapper around it. We look that up in the listenerWrappers map,
     * and if there was already a wrapper for that listener with the given
     * ObjectName, we reuse it. This map is a WeakHashMap, so a listener that is
     * no longer registered with any MBean can be garbage collected.
     * We cannot use simpler solutions such as always creating a new wrapper or
     * always registering the same listener with the MBean and using the
     * handback to find the client's original listener. The reason is that we
     * need to support the removeListener variant that removes all
     * (listener,filter,handback) triples on a broadcaster that have a given
     * listener. And we do not have any way to inspect a broadcaster's internal
     * list of triples. So the same client listener must always map to the same
     * listener registered with the broadcaster.
     * Another possible solution would be to map from ObjectName to list of
     * listener wrappers (or IdentityHashMap of listener wrappers), making this
     * list the first time a listener is added on a given MBean, and removing it
     * when the MBean is removed. This is probably more costly in memory, but
     * could be useful if some day we don't want to rely on weak references.
     */
    public void addNotificationListener(ObjectName name, NotificationListener listener,
        ...
        NotificationBroadcaster broadcaster = getNotificationBroadcaster(name, instance,
        ...
        // ------------------
        if (listener == null) {
            throw new RuntimeOperationsException(new IllegalArgumentException("Null listener"),
                    "Null listener");
        }
    ...
    public void addNotificationListener(ObjectName name, ObjectName listener, NotificationFilter filter,
        ...
        if (!(resource instanceof NotificationListener)) {
            throw new RuntimeOperationsException(new IllegalArgumentException(listener.getCanonicalName()),
        ...
        if (MBEANSERVER_LOGGER.isLoggable(Level.FINER)) {
            MBEANSERVER_LOGGER.logp(Level.FINER, DefaultMBeanServerInterceptor.class.getName(),
                    "addNotificationListener", "ObjectName = " + name + ", Listener = " + listener);
        }
    ...
    public void removeNotificationListener(ObjectName name, ObjectName listener)
        ...
        if (MBEANSERVER_LOGGER.isLoggable(Level.FINER)) {
            MBEANSERVER_LOGGER.logp(Level.FINER, DefaultMBeanServerInterceptor.class.getName(),
                    "removeNotificationListener", "ObjectName = " + name + ", Listener = " + listener);
        }
    ...
    private NotificationListener getListener(ObjectName listener) throws ListenerNotFoundException {
        ...
        if (!(resource instanceof NotificationListener)) {
            ...
            final String msg = "MBean " + listener.getCanonicalName() + " does not " + "implement "
    ...
    private void removeNotificationListener(ObjectName name, NotificationListener listener,
            NotificationFilter filter, Object handback, boolean removeAll) throws InstanceNotFoundException,
            ListenerNotFoundException {
        ...
        /*
         * We could simplify the code by assigning broadcaster after assigning
         * listenerWrapper, but that would change the error behavior when both
         * the broadcaster and the listener are erroneous.
         */

        Class<? extends NotificationBroadcaster> reqClass = removeAll ? NotificationBroadcaster.class
    ...
    private static <T extends NotificationBroadcaster> T getNotificationBroadcaster(ObjectName name,
            Object instance, Class<T> reqClass) {
        ...
        final String msg = "MBean " + name.getCanonicalName() + " does not " + "implement " + reqClass
                .getName();
        throw new RuntimeOperationsException(exc, msg);
    }
    ...
    public boolean isInstanceOf(ObjectName name, String className) throws InstanceNotFoundException {
        ...
        try {
            ...
            final String resourceClassName = (resource instanceof DynamicMBean) ? getClassName(
                    (DynamicMBean) resource) : resource.getClass().getName();
            ...
            if (MBEANSERVER_LOGGER.isLoggable(Level.FINEST)) {
                MBEANSERVER_LOGGER.logp(Level.FINEST, DefaultMBeanServerInterceptor.class.getName(),
    ...
    /**
     * <p>
     * Return the {@link java.lang.ClassLoader} that was used for loading the
     * class of the named MBean.
     * 
     * @param mbeanName
    ...
    /**
     * Sends an MBeanServerNotifications with the specified type for the MBean
    ...
    private void sendNotification(String NotifType, ObjectName name) {
        ...
        // Create notification
        // ---------------------
        MBeanServerNotification notif = new MBeanServerNotification(NotifType,
                MBeanServerDelegate.DELEGATE_NAME, 0, name);
    ...
    private Set<ObjectName> objectNamesFromFilteredNamedObjects(Set<NamedObject> list, QueryExp query) {
        ...
        if (query == null) {
            ...
            try {
                ...
                /*
                 * query.setMBeanServer is probably QueryEval.setMBeanServer so
                 * put back the old value. Since that method uses a ThreadLocal
                 * variable, this code is only needed for the unusual case where
                 * the user creates a custom QueryExp that calls a nested query
                 * on another MBeanServer.
                 */
     ...
     * Applies the specified queries to the set of NamedObjects.
     */
    private Set<ObjectInstance> objectInstancesFromFilteredNamedObjects(Set<NamedObject> list,
            QueryExp query) {
    ...
    private static String safeGetClassName(DynamicMBean mbean) {
        try {
            ...
            if (MBEANSERVER_LOGGER.isLoggable(Level.FINEST)) {
                MBEANSERVER_LOGGER.logp(Level.FINEST, DefaultMBeanServerInterceptor.class.getName(),
                        "safeGetClassName", "Exception getting MBean class name", e);
            }
            return null;
        }
    }
        ...
        if (query == null) {
            ...
            for (ObjectInstance oi : list) {
                ...
                try {
                    ...
                    /*
                     * query.setMBeanServer is probably QueryEval.setMBeanServer
                     * so put back the old value. Since that method uses a
                     * ThreadLocal variable, this code is only needed for the
                     * unusual case where the user creates a custom QueryExp
                     * that calls a nested query on another MBeanServer.
                     */
                    query.setMBeanServer(oldServer);
                }
                if (res) {
    ...
    /*
     * Get the existing wrapper for this listener, name, and mbean, if there is
     * one. Otherwise, if "create" is true, create and return one. Otherwise,
     * return null.
     * We use a WeakHashMap so that if the only reference to a user listener is
     * in listenerWrappers, it can be garbage collected. This requires a certain
     * amount of care, because only the key in a WeakHashMap is weak; the value
     * is strong. We need to recover the existing wrapper object (not just an
     * object that is equal to it), so we would like listenerWrappers to map any
     * ListenerWrapper to the canonical ListenerWrapper for that
     * (listener,name,mbean) set. But we do not want this canonical wrapper to
     * be referenced strongly. Therefore we put it inside a WeakReference and
     * that is the value in the WeakHashMap.
     */
    private NotificationListener getListenerWrapper(NotificationListener l, ObjectName name,
            DynamicMBean mbean, boolean create) {
    ...
    private static class ListenerWrapper implements NotificationListener {
        ...
        public void handleNotification(Notification notification, Object handback) {
            ...
            /*
             * Listeners are not supposed to throw exceptions. If this one does,
             * we could remove it from the MBean. It might indicate that a
             * connector has stopped working, for instance, and there is no
             * point in sending future notifications over that connection.
             * However, this seems rather drastic, so instead we propagate the
        ...
        public boolean equals(Object o) {
            ...
            /*
             * We compare all three, in case the same MBean object gets
             * unregistered and then reregistered under a different name, or the
             * same name gets assigned to two different MBean objects at
             * different times. We do the comparisons in this order to avoid the
             * slow ObjectName.equals when possible.
             */
        }
        ...
        public int hashCode() {
            ...
            /*
             * We do not include name.hashCode() in the hash because computing
             * it is slow and usually we will not have two instances of
             * ListenerWrapper with the same mbean but different ObjectNames.
             * That can happen if the MBean is unregistered from one name and
             * reregistered with another, and there is no garbage collection
             * between; or if the same object is registered under two names
             * (which is not recommended because MBeanRegistration will break).
             * But even in these unusual cases the hash code does not have to be
             * unique.
             */
        }
    ...
    private static void checkMBeanPermission(DynamicMBean mbean, String member, ObjectName objectName,
            String actions) {
        SecurityManager sm = System.getSecurityManager();
        if (sm != null) {
    ...
    /**
     * A RegistrationContext that makes it possible to perform additional post
     * registration actions (or post unregistration actions) outside of the
     * repository lock, once postRegister (or postDeregister) has been called.
    ...
    /**
     * Adds a MBean in the repository, sends
     * MBeanServerNotification.REGISTRATION_NOTIFICATION, returns
     * ResourceContext for special resources such as ClassLoaders or
     * JMXNamespaces. For regular MBean this method returns
     * ResourceContext.NONE.
     * 
     * @return a ResourceContext for special resources such as ClassLoaders or
     *         JMXNamespaces.
     */
    private ResourceContext registerWithRepository(final Object resource, final DynamicMBean object,
            final ObjectName logicalName) throws InstanceAlreadyExistsException, MBeanRegistrationException {

        // Creates a registration context, if needed.
        //
        final ResourceContext context = makeResourceContextFor(resource, logicalName);
        ...
        // ---------------------
        if (MBEANSERVER_LOGGER.isLoggable(Level.FINER)) {
            MBEANSERVER_LOGGER.logp(Level.FINER, DefaultMBeanServerInterceptor.class.getName(), "addObject",
                    "Send create notification of object " + logicalName.getCanonicalName());
        }
    ...
    /**
     * Removes a MBean in the repository, sends
     * MBeanServerNotification.UNREGISTRATION_NOTIFICATION, returns
     * ResourceContext for special resources such as ClassLoaders or
     * JMXNamespaces, or null. For regular MBean this method returns
     * ResourceContext.NONE.
     *
     * @return a ResourceContext for special resources such as ClassLoaders or
     *         JMXNamespaces.
     */
    private ResourceContext unregisterFromRepository(final Object resource, final DynamicMBean object,
            final ObjectName logicalName) throws InstanceNotFoundException {
        ...
        if (MBEANSERVER_LOGGER.isLoggable(Level.FINER)) {
            MBEANSERVER_LOGGER.logp(Level.FINER, DefaultMBeanServerInterceptor.class.getName(),
                    "unregisterMBean", "Send delete notification of object " + logicalName
                            .getCanonicalName());
        }
    ...
    /**
     * Registers a ClassLoader with the CLR. This method is called by the
     * ResourceContext from within the repository lock.
     * 
     * @param loader
     *        The ClassLoader.
     * @param logicalName
    ...
    private void addClassLoader(ClassLoader loader, final ObjectName logicalName) {
        /**
         * Called when the newly registered MBean is a ClassLoader If so, tell
         * the ClassLoaderRepository (CLR) about it. We do this even if the
         * loader is a PrivateClassLoader. In that case, the CLR remembers the
         * loader for use when it is explicitly named (e.g. as the loader in
         * createMBean) but does not add it to the list that is consulted by
         * ClassLoaderRepository.loadClass.
         */
        final ModifiableClassLoaderRepository clr = getInstantiatorCLR();
        if (clr == null) {
            final RuntimeException wrapped = new IllegalArgumentException("Dynamic addition of class loaders"
                    + " is not supported");
            throw new RuntimeOperationsException(wrapped, "Exception occurred trying to register"
    ...
    /**
     * Unregisters a ClassLoader from the CLR. This method is called by the
     * ResourceContext from within the repository lock.
     * 
     * @param loader
     *        The ClassLoader.
     ...
     *        The ClassLoader MBean ObjectName.
     */
    private void removeClassLoader(ClassLoader loader, final ObjectName logicalName) {
        /**
    ...
    /**
     * Creates a ResourceContext for a ClassLoader MBean. The resource context
     * makes it possible to add the ClassLoader to (ResourceContext.registering)
     * or resp. remove the ClassLoader from (ResourceContext.unregistered) the
     * CLR when the associated MBean is added to or resp. removed from the
     * repository.
     *
     * @param loader
     *        The ClassLoader MBean being registered or
     *        unregistered.
     * @param logicalName
     *        The name of the ClassLoader MBean.
     * @return a ResourceContext that takes in charge the addition or removal of
     *         the loader to or from the CLR.
     */
    ...
    /**
     * Creates a ResourceContext for the given resource. If the resource does
     * not need a ResourceContext, returns ResourceContext.NONE. At this time,
     * only ClassLoaders need a ResourceContext.
     *
     * @param resource
     ...
     * @return
     */
    private ResourceContext makeResourceContextFor(Object resource, ObjectName logicalName) {
        if (resource instanceof ClassLoader) {
            return createClassLoaderContext((ClassLoader) resource, logicalName);
        }
