 ...
 * "{@docRoot}/../guide/security/jaas/JAASRefGuide.html">JAAS</a> based
 * implementation of the {@link JMXAuthenticator} interface.
 * </p>
 *
 * <p>
 * Authentication is performed by passing the supplied user's credentials to one
 * or more authentication mechanisms ({@link LoginModule}) for verification. An
 * authentication mechanism acquires the user's credentials by calling
 * {@link NameCallback} and/or {@link PasswordCallback}. If authentication is
 * successful then an authenticated {@link Subject} filled in with a
 * {@link Principal} is returned. Authorization checks will then be performed
 * based on this <code>Subject</code>.
 * </p>
 *
 * <p>
 * By default, a single file-based authentication mechanism
 * {@link FileLoginModule} is configured (<code>FileLoginConfig</code>).
 * </p>
 *
 * <p>
 * To override the default configuration use the
 * <code>com.sun.management.jmxremote.login.config</code> management property
 * described in the JRE/lib/management/management.properties file. Set this
 * property to the name of a JAAS configuration entry and ensure that the entry
 * is loaded by the installed {@link Configuration}. In addition, ensure that
 * the authentication mechanisms specified in the entry acquire the user's
 * credentials by calling {@link NameCallback} and {@link PasswordCallback} and
 * that they return a {@link Subject} filled-in with a {@link Principal}, for
 * those users that are successfully authenticated.
 * </p>
 */
public final class JMXPluggableAuthenticator implements JMXAuthenticator {

	/**
	 * Creates an instance of <code>JMXPluggableAuthenticator</code> and
	 * initializes it with a {@link LoginContext}.
	 *
	 * @param env
	 *            the environment containing configuration properties for the
	...
	public JMXPluggableAuthenticator(Map<?, ?> env) {
		...
		try {
			...
			if (loginConfigName != null) {
				...
				loginContext = new LoginContext(loginConfigName,
					...
					loginContext = AccessController.doPrivileged(
							new PrivilegedExceptionAction<LoginContext>() {
								public LoginContext run()
									...
									return new LoginContext(LOGIN_CONFIG_NAME,
											null, new JMXCallbackHandler(),
	...
	/**
	 * Authenticate the <code>MBeanServerConnection</code> client with the given
	 * client credentials.
	 *
	 * @param credentials
	 *                    the user-defined credentials to be passed in to the
	 *                    server in
	 *                    order to authenticate the user before creating the
	 *                    <code>MBeanServerConnection</code>. This parameter
	 *                    must be a
	 *                    two-element <code>String[]</code> containing the
	 *                    client's
	...
	public Subject authenticate(Object credentials) {
		...
		if (!(credentials instanceof String[])) {
			...
			final String message = "Credentials should be String[] instead of "
		...
		if (aCredentials.length != 2) {
			final String message = "Credentials should have 2 elements not "
					+ aCredentials.length;
			authenticationFailure("authenticate", message);
		}
	...
	private static void authenticationFailure(String method,
			Exception exception) throws SecurityException {
		String msg;
	...
	private static void logException(String method, String message,
			Exception e) {
		if (logger.traceOn()) {
	...
	private static final ClassLogger logger = new ClassLogger(
			"javax.management.remote.misc", LOGIN_CONFIG_NAME);
	 ...
	 * credentials are already available to this class (via its enclosing
	 * class).
	 */
	...
	private static class FileLoginConfig extends Configuration {
		...
		private static final String FILE_LOGIN_MODULE = FileLoginModule.class
				.getName();
		 ...
		 *
		 * @param passwordFile
		 *                     A filepath that identifies the password file to
		 *                     use. If
		 *                     null then the default password file is used.
		 */
		public FileLoginConfig(String passwordFile) {
			...
			entries = new AppConfigurationEntry[] { new AppConfigurationEntry(
					FILE_LOGIN_MODULE,
