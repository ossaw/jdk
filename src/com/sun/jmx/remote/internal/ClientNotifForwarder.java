...
public abstract class ClientNotifForwarder {
    ...
    /*
     * An Executor that allows at most one executing and one pending Runnable.
     * It uses at most one thread -- as soon as there is no pending Runnable the
     * thread can exit. Another thread is created as soon as there is a new
     * pending Runnable. This Executor is adapted for use in a situation where
     * each Runnable usually schedules up another Runnable. On return from the
     * first one, the second one is immediately executed. So this just becomes a
     * complicated way to write a while loop, but with the advantage that you
     * can replace it with another Executor, for instance one that you are using
     * to execute a bunch of other unrelated work.
     * You might expect that a java.util.concurrent.ThreadPoolExecutor with
     * corePoolSize=0 and maximumPoolSize=1 would have the same behavior, but it
     * does not. A ThreadPoolExecutor only creates a new thread when a new task
     * is submitted and the number of existing threads is < corePoolSize. This
     * can never happen when corePoolSize=0, so new threads are never created.
     * Surprising, but there you are.
     */
    ...
    public ClientNotifForwarder(ClassLoader defaultClassLoader, Map<String, ?> env) {
         ...
         * fetchNotifications will be made. The Executor's execute method
         * reschedules another task, so you must not use an Executor that
         * executes tasks in the caller's thread.
         */
     ...
     */
    abstract protected NotificationResult fetchNotifs(long clientSequenceNumber, int maxNotifications,
            long timeout) throws IOException, ClassNotFoundException;

    abstract protected Integer addListenerForMBeanRemovedNotif() throws IOException,
            InstanceNotFoundException;

    abstract protected void removeListenerForMBeanRemovedNotif(Integer id) throws IOException,
    ...
    public synchronized void addNotificationListener(Integer listenerID, ObjectName name,
            NotificationListener listener, NotificationFilter filter, Object handback,
            Subject delegationSubject) throws IOException, InstanceNotFoundException {

        if (logger.traceOn()) {
            logger.trace("addNotificationListener", "Add the listener " + listener + " at " + name);
        }

        infoList.put(listenerID, new ClientListenerInfo(listenerID, name, listener, filter, handback,
                delegationSubject));

        init(false);
    }

    public synchronized Integer[] removeNotificationListener(ObjectName name, NotificationListener listener)
    ...
    public synchronized Integer removeNotificationListener(ObjectName name, NotificationListener listener,
            NotificationFilter filter, Object handback) throws ListenerNotFoundException, IOException {
    ...
    /*
     * Called when a connector is doing reconnection. Like
     * <code>postReconnection</code>, this method is intended to be called only
     * by a client connector: <code>RMIConnector</code> and
     * <code>ClientIntermediary</code>. Call this method will set the flag
     * beingReconnection to <code>true</code>, and the thread used to fetch
     * notifis will be stopped, a new thread can be created only after the
     * method <code>postReconnection</code> is called.
    ...
    /**
     * Called after reconnection is finished. This method is intended to be
     * called only by a client connector: <code>RMIConnector</code> and
     * <code>ClientIntermediary</code>.
     */
    public synchronized void postReconnection(ClientListenerInfo[] listenerInfos) throws IOException {
        ...
        for (int i = 0; i < len; i++) {
            if (trace) {
                logger.trace("addNotificationListeners", "Add a listener at " + listenerInfos[i]
                        .getListenerID());
            }
        ...
        if (currentFetchThread == Thread.currentThread() || state == STARTING || state == STARTED) { // doing
                                                                                                     // or
                                                                                                     // waiting
                                                                                                     // reconnection
            ...
            } else if (infoList.size() > 0) { // only new listeners added during
                                              // reconnection
    ...
    private class NotifFetcher implements Runnable {
        ...
        private void logOnce(String msg, SecurityException x) {
            ...
            if (x != null)
                logger.fine("setContextClassLoader", x);
            alreadyLogged = true;
        }
                ...
                public ClassLoader run() {
                    try {
                        ...
                        logOnce("Permission to set ContextClassLoader missing. "
                                ...
                                + "Please check your Java policy configuration: " + x, x);
                        throw x;
                    }
                }
            }, ctxt);
        ...
        private void doRun() {
            ...
            if (!shouldStop() && (nr = fetchNotifs()) != null) {
                ...
                if (missed > 0) {
                    final String msg = "May have lost up to " + missed + " notification" + (missed == 1 ? ""
                            : "s");
        ...
        void dispatchNotification(TargetedNotification tn, Integer myListenerID,
        ...
        private NotificationResult fetchNotifs() {
            try {
                NotificationResult nr = ClientNotifForwarder.this.fetchNotifs(clientSequenceNumber,
                ...
                if (!shouldStop()) {
                    logger.error("NotifFetcher-run", "Failed to fetch notification, "
         ...
         * notification that we can't deserialize (because of a missing class).
         * First we ask for 0 notifications with 0 timeout. This allows us to
         * skip sequence numbers for notifications that don't match our filters.
         * Then we ask for one notification. If that produces a
         * ClassNotFoundException, NotSerializableException or
         * UnmarshalException, we increase our sequence number and ask again.
         * Eventually we will either get a successful notification, or a return
         * with 0 notifications. In either case we can return a
         * NotificationResult. This algorithm works (albeit less well) even if
         * the server implementation doesn't optimize a request for 0
         * notifications to skip sequence numbers for notifications that don't
         * match our filters.
        ...
        private NotificationResult fetchOneNotif() {
            ...
            while (result == null && !shouldStop()) {
                ...
                try {
                    ...
                    logger.warning("NotifFetcher.fetchOneNotif", "Failed to deserialize a notification: " + e
                            .toString());
                    if (logger.traceOn()) {
                        logger.trace("NotifFetcher.fetchOneNotif", "Failed to deserialize a notification.",
                                e);
                    }
            ...
            if (notFoundCount > 0) {
                final String msg = "Dropped " + notFoundCount + " notification" + (notFoundCount == 1 ? ""
                        : "s") + " because classes were missing locally or incompatible";
                lostNotifs(msg, notFoundCount);
                // Even if result.getEarliestSequenceNumber() is now greater
                // than
                ...
                // that we saw.
                if (result != null) {
                    result = new NotificationResult(firstEarliest, result.getNextSequenceNumber(), result
     ...
     * <P>The parameter reconnected will decide whether to initilize the
     * clientSequenceNumber, initilaizing the clientSequenceNumber means to
     * ignore all notifications arrived before. If it is reconnected, we will
     * not initialize in order to get all notifications arrived during the
     * reconnection. It may cause the newly registered listeners to receive some
                ...
                if (!reconnected) {
                    try {
                        ...
                        if (state != STOPPED) { // JDK-8038940
                                                ...
                                                // thread takes over the fetching
                                                // job
                            return;
                        }
    ...
    /**
     * Import: should not remove a listener during reconnection, the
     * reconnection needs to change the listener list and that will possibly
     * make removal fail.
     */
    ...
    // state
    /**
     * This state means that a thread is being created for fetching and
     * forwarding notifications.
     */
    private static final int STARTING = 0;

    /**
     * This state tells that a thread has been started for fetching and
     * forwarding notifications.
     */
    ...
    /**
     * This state means that this object is terminated and no more thread will
     * be created for fetching notifications.
     */
    ...
    /**
     * This variable is used to tell whether a connector (RMIConnector or
     * ClientIntermediary) is doing reconnection. This variable will be set to
     * true by the method <code>preReconnection</code>, and set to false by
     * <code>postReconnection</code>. When beingReconnected == true, no thread
     * will be created for fetching notifications.
     */
