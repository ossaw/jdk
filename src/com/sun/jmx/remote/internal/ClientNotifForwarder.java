...
public abstract class ClientNotifForwarder {
	...
	abstract protected NotificationResult fetchNotifs(long clientSequenceNumber,
			int maxNotifications, long timeout) throws IOException, ClassNotFoundException;
	...
	public synchronized void addNotificationListener(Integer listenerID, ObjectName name,
		...
		infoList.put(listenerID, new ClientListenerInfo(listenerID, name, listener, filter,
				handback, delegationSubject));

		init(false);
	}

	public synchronized Integer[] removeNotificationListener(ObjectName name,
			NotificationListener listener) throws ListenerNotFoundException, IOException {
		...
		for (int i = 0; i < len; i++) {
			if (trace) {
				logger.trace("addNotificationListeners",
						"Add a listener at " + listenerInfos[i].getListenerID());
			}
	...
	private class NotifFetcher implements Runnable {
		...
		private void doRun() {
			...
			if (!shouldStop() && (nr = fetchNotifs()) != null) {
				...
				if (missed > 0) {
					final String msg = "May have lost up to " + missed + " notification"
							+ (missed == 1 ? "" : "s");
			...
			try {
				...
				if (!shouldStop()) {
					logger.error("NotifFetcher-run",
							"Failed to fetch notification, " + "stopping thread. Error is: " + ioe,
							ioe);
		...
		private NotificationResult fetchOneNotif() {
			...
			if (notFoundCount > 0) {
				...
				// that we saw.
				if (result != null) {
					result = new NotificationResult(firstEarliest, result.getNextSequenceNumber(),
