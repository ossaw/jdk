...
public class EnvHelp {
	...
	/**
	 * Name of the attribute that specifies a list of class names acceptable as
	 * parameters to the {@link RMIServer#newClient(java.lang.Object)
	 * RMIServer.newClient()} remote method call.
	 * <p>
	 ...
	 * {@linkplain JMXAuthenticator} associated with the {@linkplain RMIServer}
	 * implementation.
	 * <p>
	 * If the attribute is not set, or is null, then any class is deemed
	 * acceptable.
	 */
	...
	/**
	 * <p>
	 * Name of the attribute that specifies a default class loader object. The
	 ...
	 * <p>
	 * <ul>
	 * <li>The ClassLoader object found in <var>env</var> for
	 * <code>jmx.remote.default.class.loader</code>, if any.</li>
	 * <li>The ClassLoader pointed to by the ObjectName found in <var>env</var>
	 * for <code>jmx.remote.default.class.loader.name</code>, and registered in
	 * <var>mbs</var> if any.</li>
	 ...
	 * @param mbs
	 *            The MBeanServer for which the connector server provides remote
	 *            access.
	 *
	 ...
	 *                <ul>
	 *                <li>both <code>jmx.remote.default.class.loader</code> and
	 *                <code>jmx.remote.default.class.loader.name</code> are
	 *                specified,</li>
	 *                <li>or <code>jmx.remote.default.class.loader</code> is not
	 *                an instance of {@link ClassLoader},</li>
	 *                <li>or <code>jmx.remote.default.class.loader.name</code>
	 *                is not an instance of {@link ObjectName},</li>
	 *                <li>or <code>jmx.remote.default.class.loader.name</code>
	 *                is specified but <var>mbs</var> is null.</li>
	 * @exception InstanceNotFoundException
	 *                if <code>jmx.remote.default.class.loader.name</code> is
	 *                specified and the ClassLoader MBean is not found in
	 *                <var>mbs</var>.
	 */
	public static ClassLoader resolveServerClassLoader(Map<String, ?> env, MBeanServer mbs)
		...
		if (loader != null && name != null) {
			final String msg = "Only one of " + DEFAULT_CLASS_LOADER + " or "
					+ DEFAULT_CLASS_LOADER_NAME + " should be specified.";
		...
		if (loader != null) {
			if (loader instanceof ClassLoader) {
				...
				final String msg = "ClassLoader object is not an instance of "
						+ ClassLoader.class.getName() + " : " + loader.getClass().getName();
				throw new IllegalArgumentException(msg);
			}
		}
		...
		if (name instanceof ObjectName) {
			...
			final String msg = "ClassLoader name is not an instance of "
					+ ObjectName.class.getName() + " : " + name.getClass().getName();
			throw new IllegalArgumentException(msg);
		}
	 ...
	 *
	 * @exception IllegalArgumentException
	 *                if <code>jmx.remote.default.class.loader</code> is
	 *                specified and is not an instance of {@link ClassLoader}.
	 */
	 ...
	 *
	 * @param throwable
	 *            The {@code Throwable} on which the cause is set.
	 * @param cause
	 *            The cause to set on the supplied {@code Throwable}.
	...
	/**
	 * Returns the cause field of a {@code Throwable} object. The cause field
	 * can be got only if <var>t</var> has an {@link Throwable#getCause()}
	 * method (JDK Version >= 1.4)
	 * 
	 * @param t
	 *            {@code Throwable} on which the cause must be set.
	 * @return the cause if getCause() succeeded and the got value is not null,
	 *         otherwise return the <var>t</var>.
	 */
	public static Throwable getCause(Throwable t) {
		Throwable ret = t;

		try {
			java.lang.reflect.Method getCause = t.getClass().getMethod("getCause",
					(Class<?>[]) null);
	...
	/**
	 * <p>
	 * Name of the attribute that specifies the size of a notification buffer
	 * for a connector server. The default value is 1000.
	 */
	public static final String BUFFER_SIZE_PROPERTY = "jmx.remote.x.notification.buffer.size";

	/**
	 * Returns the size of a notification buffer for a connector server. The
	...
	public static int getNotifBufferSize(Map<String, ?> env) {
		...
		try {
			...
			logger.warning("getNotifBufferSize",
					"Can't use System property " + BUFFER_SIZE_PROPERTY + ": " + e);
		...
		try {
			if (env.containsKey(BUFFER_SIZE_PROPERTY)) {
				queueSize = (int) EnvHelp.getIntegerAttribute(env, BUFFER_SIZE_PROPERTY,
						defaultQueueSize, 0, Integer.MAX_VALUE);
			} else { // try the old one
				queueSize = (int) EnvHelp.getIntegerAttribute(env, oldP, defaultQueueSize, 0,
						Integer.MAX_VALUE);
			}
		} catch (RuntimeException e) {
	...
	/**
	 * <p>
	 * Name of the attribute that specifies the maximum number of notifications
	 * that a client will fetch from its server.. The value associated with this
	 * attribute should be an <code>Integer</code> object. The default value is
	 * 1000.
	 * </p>
	 */
	...
	/**
	 * Returns the maximum notification number which a client will fetch every
	 * time.
	 */
	...
	/**
	 * <p>
	 * Name of the attribute that specifies the timeout for a client to fetch
	 * notifications from its server. The value associated with this attribute
	 * should be a <code>Long</code> object. The default value is 60000
	 * milliseconds.
	 * </p>
	 */
	...
	/**
	 * <p>
	 * Name of the attribute that specifies an object that will check accesses
	 * to add/removeNotificationListener and also attempts to receive
	 * notifications. The value associated with this attribute should be a
	 * <code>NotificationAccessController</code> object. The default value is
	 * null.
	 * </p>
	...
	/**
	 * Get an integer-valued attribute with name <code>name</code> from
	 * <code>env</code>. If <code>env</code> is null, or does not contain an
	 * entry for <code>name</code>, return <code>defaultValue</code>. The value
	 * may be a Number, or it may be a String that is parsable as a long. It
	 * must be at least <code>minValue</code> and at most<code>maxValue</code>.
	 *
	 * @throws IllegalArgumentException
	 *             if <code>env</code> contains an entry for <code>name</code>
	 *             but it does not meet the constraints above.
	 */
	public static long getIntegerAttribute(Map<String, ?> env, String name, long defaultValue,
		...
		if (result < minValue) {
			final String msg = "Attribute " + name + " value must be at least " + minValue + ": "
					+ result;
			throw new IllegalArgumentException(msg);
		}

		if (result > maxValue) {
			final String msg = "Attribute " + name + " value must be at most " + maxValue + ": "
					+ result;
			throw new IllegalArgumentException(msg);
		}
	...
	/*
	 * Check that all attributes have a key that is a String. Could make further
	 * checks, e.g. appropriate types for attributes.
	 */
	 ...
	 * serializable, and that are not hidden by jmx.remote.x.hidden.attributes
	 * or the default list of hidden attributes.
	 */
	...
	/**
	 * Remove from the given Collection any element that is not a serializable
	 * object.
	 */
	...
	/**
	 * The value of this attribute, if present, is a string specifying what
	 * other attributes should not appear in JMXConnectorServer.getAttributes().
	 * It is a space-separated list of attribute patterns, where each pattern is
	 * either an attribute name, or an attribute prefix followed by a "*"
	 * character. The "*" has no special significance anywhere except at the end
	 * of a pattern. By default, this list is added to the list defined by
	 * {@link #DEFAULT_HIDDEN_ATTRIBUTES} (which uses the same format). If the
	 * value of this attribute begins with an "=", then the remainder of the
	 ...
	 */
	/*
	 * This list is copied directly from the spec, plus java.naming.security.*.
	 * Most of the attributes here would have been eliminated from the map
	 * anyway because they are typically not serializable. But just in case they
	 * are, we list them here to conform to the spec.
	 */
	public static final String DEFAULT_HIDDEN_ATTRIBUTES = "java.naming.security.* "
			+ "jmx.remote.authenticator " + "jmx.remote.context "
			+ "jmx.remote.default.class.loader " + "jmx.remote.message.connection.server "
			+ "jmx.remote.object.wrapping " + "jmx.remote.rmi.client.socket.factory "
			+ "jmx.remote.rmi.server.socket.factory " + "jmx.remote.sasl.callback.handler "
			+ "jmx.remote.tls.socket.factory " + "jmx.remote.x.access.file "
	...
	private static void hideAttributes(SortedMap<String, ?> map) {
		...
		/*
		 * Construct a string that is greater than any key in the map. Setting a
		 * string-to-match or a prefix-to-match to this string guarantees that
		 * we will never call next() on the corresponding iterator.
		 */
		...
		/*
		 * Read each key in sorted order and, if it matches a string or prefix,
		 * remove it.
		 */
		keys: while (keyIterator.hasNext()) {
			...
			/*
			 * Continue through string-match values until we find one that is
			 * either greater than the current key, or equal to it. In the
			 * latter case, remove the key.
			 */
			int cmp = +1;
			...
			/*
			 * Continue through the prefix values until we find one that is
			 * either greater than the current key, or a prefix of it. In the
			 * latter case, remove the key.
			 */
	...
	private static void parseHiddenAttributes(String hide, SortedSet<String> hiddenStrings,
	...
	/**
	 * <p>
	 * Name of the attribute that specifies the timeout to keep a server side
	 * connection after answering last client request. The default value is
	 * 120000 milliseconds.
	 * </p>
	 */
	...
	/**
	 * <p>
	 * Name of the attribute that specifies the period in millisecond for a
	 * client to check its connection. The default value is 60000 milliseconds.
	 * </p>
	 */
	...
	/**
	 * Computes a boolean value from a string value retrieved from a property in
	 * the given map.
	 *
	 * @param stringBoolean
	 ...
	 *         <li>{@code false} if
	 *         {@code stringBoolean.equalsIgnoreCase("false")} is {@code true}
	 *         </li>
	 *         <li>{@code true} if
	 *         {@code stringBoolean.equalsIgnoreCase("true")} is {@code true}
	 *         </li>
	 *         </ul>
	 *
	 * @throws IllegalArgumentException
	 *             if {@code ((String)env.get(prop)).equalsIgnoreCase("false")}
	 *             and {@code ((String)env.get(prop)).equalsIgnoreCase("true")}
	 *             are {@code false}.
	 */
	 ...
	 * @return
	 *         <ul>
	 *         <li>{@code defaultValue} if {@code stringBoolean} is {@code null}
	 *         </li>
	 *         <li>{@code false} if
	 *         {@code stringBoolean.equalsIgnoreCase("false")} is {@code true}
	 *         </li>
	...
	public static boolean computeBooleanFromString(String stringBoolean, boolean defaultValue) {
		if (stringBoolean == null)
			...
			throw new IllegalArgumentException(
					"Property value must be \"true\" or \"false\" instead of \"" + stringBoolean
							+ "\"");
	}

	/**
	 * Converts a map into a valid hash table, i.e. it removes all the 'null'
	 * values from the map.
	 */
	...
	/**
	 * <p>
	 * Name of the attribute that specifies whether a connector server should
	 * not prevent the VM from exiting
	 */
	public static final String JMX_SERVER_DAEMON = "jmx.remote.x.daemon";

	/**
	 * Returns true if {@value SERVER_DAEMON} is specified in the {@code env} as
	 * a key and its value is a String and it is equal to true ignoring case.
	 *
	 * @param env
	 * @return
	 */
