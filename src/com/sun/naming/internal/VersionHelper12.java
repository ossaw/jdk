...
final class VersionHelper12 extends VersionHelper {
		...
		return AccessController.doPrivileged(new PrivilegedAction<InputStream>() {
			public InputStream run() {
				try {
					...
					String pathname = javahome + java.io.File.separator + "lib"
	...
	NamingEnumeration<InputStream> getResources(final ClassLoader cl, final String name)
			throws IOException {
		Enumeration<URL> urls;
		try {
			urls = AccessController.doPrivileged(new PrivilegedExceptionAction<Enumeration<URL>>() {
				public Enumeration<URL> run() throws IOException {
					return (cl == null) ? ClassLoader.getSystemResources(name)
	...
	/**
	 * Given an enumeration of URLs, an instance of this class represents an
	 * enumeration of their InputStreams. Each operation on the URL enumeration
	 * is performed within a doPrivileged block. This is used to enumerate the
	 * resources under a foreign codebase. This class is not MT-safe.
	 */
	class InputStreamEnumeration implements NamingEnumeration<InputStream> {
		...
		/*
		 * Returns the next InputStream, or null if there are no more. An
