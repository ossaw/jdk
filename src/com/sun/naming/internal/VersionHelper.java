...
/**
 * VersionHelper was used by JNDI to accommodate differences between JDK 1.1.x
 * and the Java 2 platform. As this is no longer necessary since JNDI's
 * inclusion in the platform, this class currently serves as a set of utilities
 * for performing system-level things, such as class-loading and reading system
 * properties.
 *
 * @author Rosanna Lee
...
public abstract class VersionHelper {
    ...
    public abstract Class<?> loadClass(String className, String codebase) throws ClassNotFoundException,
            MalformedURLException;

    /*
     * Returns a JNDI property from the system properties. Returns null if the
     * property is not set, or if there is no permission to read it.
     */
    abstract String getJndiProperty(int i);

    /*
     * Reads each property in PROPS from the system properties, and returns
     * their values -- in order -- in an array. For each unset property, the
     * corresponding array element is set to null. Returns null if there is no
     * permission to call System.getProperties().
     */
    abstract String[] getJndiProperties();

    /*
     * Returns the resource of a given name associated with a particular class
     * (never null), or null if none can be found.
     */
    ...
    /*
     * Returns an enumeration (never null) of InputStreams of the resources of a
     ...
     */
    abstract NamingEnumeration<InputStream> getResources(ClassLoader cl, String name) throws IOException;
