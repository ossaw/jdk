...
public final class ResourceManager {
     ...
     * application resource files lookup to prevent recursion issues when
     * validating signed JARs.
     */
     ...
     */
    private static final String[] listProperties = { Context.OBJECT_FACTORIES, Context.URL_PKG_PREFIXES,
     ...
     * ResourceManager. A Hashtable from a provider resource file is keyed on a
     * class in the resource file's package. One from application resource files
     * is keyed on the thread's context class loader.
     */
    // WeakHashMap<Class | ClassLoader, Hashtable>
    private static final WeakHashMap<Object, Hashtable<? super String, Object>> propertiesCache = new WeakHashMap<>(
            11);
     ...
     */
    private static final WeakHashMap<ClassLoader, Map<String, List<NamedWeakReference<Object>>>> factoryCache = new WeakHashMap<>(
            11);
     ...
     * search revealed no factory. Used in getFactory().
     */
     ...
     * constructor, returns the full environment for that initial context (never
     ...
     * resource file associated with the given context. The environment may in
     * turn contain values that come from applet parameters, system properties,
     * or application resource files.
     * If <tt>concat</tt> is true and both the environment and the provider
     * resource file contain the property, the two values are concatenated (with
     * a ':' separator).
     ...
     *         resource file.
     */
    public static String getProperty(String propName, Hashtable<?, ?> env, Context ctx, boolean concat)
            throws NamingException {
     ...
     * property.
     * The property is gotten from the environment and the provider resource
     * file associated with the given context and concantenated. See
     * getProperty(). The resulting property value is a list of class names.
     * <p>
     * This method then loads each class using the current thread's context
     * class loader and keeps them in a list. Any class that cannot be loaded is
     * ignored. The resulting list is then cached in a two-level hash table,
     * keyed first by the context class loader and then by the property's value.
     * The next time threads of the same context class loader call this method,
     * they can use the cached list.
     * <p>
     ...
     * replaced by an instance of the factory object itself. Both class objects
     * and factories are wrapped in weak references so as not to prevent GC of
     * the class loader.
     * <p>
     * Note that multiple threads can be accessing the same cached list via
     * FactoryEnumeration, which locks the list during each next(). The size of
     * the list will not change, but a cached Class object might be replaced by
     ...
     * The property is gotten from the environment and the provider resource
     * file associated with the given context and concatenated. classSuffix is
     * added to the end of this list. See getProperty(). The resulting property
     * value is a list of package prefixes.
     * <p>
     * This method then constructs a list of class names by concatenating each
     * package prefix with classSuffix and attempts to load and instantiate the
     * class until one succeeds. Any class that cannot be loaded is ignored. The
     * resulting object is then cached in a two-level hash table, keyed first by
     * the context class loader and then by the property's value and
     * classSuffix. The next time threads of the same context class loader call
     * this method, they use the cached factory. If no factory can be loaded,
     * NO_FACTORY is recorded in the table so that next time it'll return
     * quickly.
     *
     * @param propName
     *        The non-null property name
    ...
    /*
     * Returns the Hashtable (never null) that results from merging all
    ...
    /*
     * Merge the properties from one hash table into another. Each property in
     * props2 that is not in props1 is added to props1. For each property in
     * both hash tables that is one of the standard JNDI properties that specify
     * colon-separated lists, the values are concatenated and stored in props1.
     */
