...
public final class FactoryEnumeration {
	...
	/**
	 * Records the input list and uses it directly to satisfy hasMore()/next()
	 * requests. An alternative would have been to use an enumeration/iterator
	 * from the list, but we want to update the list so we keep the original
	 * list. The list initially contains Class objects. As each element is used,
	 ...
	 * factory's class name so the class can be reloaded if the reference is
	 * cleared.
	 *
	 * @param factories
	 *                  A non-null list
