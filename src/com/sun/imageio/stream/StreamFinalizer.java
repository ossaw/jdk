...
/**
 * Small class to assist in properly closing an ImageInputStream instance prior
 * to garbage collection. The ImageInputStreamImpl class defines a finalize()
 * method, but in a number of its public subclasses (e.g. FileImageInputStream)
 * we override the finalize() method to be empty for performance reasons, and
 * instead rely on the Disposer mechanism for closing/disposing resources. This
 * is fine when one of these classes is instantiated directly (e.g. new
 * FileImageInputStream()) but in the unlikely case where a user defines their
 ...
 * StreamFinalizer, and the StreamFinalizer will hold a reference back to the
