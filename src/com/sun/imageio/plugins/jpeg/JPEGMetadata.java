...
public class JPEGMetadata extends IIOMetadata implements Cloneable {
    ...
    JPEGMetadata(boolean isStream, boolean isThumb, ImageInputStream iis, JPEGImageReader reader)
        ...
        if (((buffer.buf[0] & 0xff) != 0xff) || ((buffer.buf[1] & 0xff) != JPEG.SOI) || ((buffer.buf[2]
                & 0xff) != 0xff)) {
        ...
        while (!done) {
            ...
            switch (buffer.buf[buffer.bufPtr] & 0xff) {
                ...
                case JPEG.APP0:
                    ...
                    buf = buffer.buf;
                    ptr = buffer.bufPtr;
                    if ((buf[ptr + 3] == 'J') && (buf[ptr + 4] == 'F') && (buf[ptr + 5] == 'I') && (buf[ptr
                            + 6] == 'F') && (buf[ptr + 7] == 0)) {
                        if (inThumb) {
                ...
                case JPEG.APP2:
                    ...
                    if ((buffer.buf[buffer.bufPtr + 3] == 'I') && (buffer.buf[buffer.bufPtr + 4] == 'C')
                            && (buffer.buf[buffer.bufPtr + 5] == 'C') && (buffer.buf[buffer.bufPtr
                                    + 6] == '_') && (buffer.buf[buffer.bufPtr + 7] == 'P')
                            && (buffer.buf[buffer.bufPtr + 8] == 'R') && (buffer.buf[buffer.bufPtr
                                    + 9] == 'O') && (buffer.buf[buffer.bufPtr + 10] == 'F')
                            && (buffer.buf[buffer.bufPtr + 11] == 'I') && (buffer.buf[buffer.bufPtr
                                    + 12] == 'L') && (buffer.buf[buffer.bufPtr + 13] == 'E')
                ...
                case JPEG.APP14:
                    ...
                    if ((buffer.buf[buffer.bufPtr + 3] == 'A') && (buffer.buf[buffer.bufPtr + 4] == 'd')
                            && (buffer.buf[buffer.bufPtr + 5] == 'o') && (buffer.buf[buffer.bufPtr
                                    + 6] == 'b') && (buffer.buf[buffer.bufPtr + 7] == 'e')) {
                        if (isStream) {
    ...
    JPEGMetadata(ImageWriteParam param, JPEGImageWriter writer) {
        ...
        if (jparam != null) {
            ...
            markerSequence.add(new DHTMarkerSegment(jparam.getDCHuffmanTables(), jparam
                    .getACHuffmanTables()));
        } else {
            // default tables.
            markerSequence.add(new DQTMarkerSegment(JPEG.getDefaultQTables()));
            markerSequence.add(new DHTMarkerSegment(JPEG.getDefaultHuffmanTables(true), JPEG
    ...
    private void mergeJFIFsubtree(Node JPEGvariety) throws IIOInvalidTreeException {
        if (JPEGvariety.getChildNodes().getLength() != 0) {
            ...
            JFIFMarkerSegment jfifSeg = (JFIFMarkerSegment) findMarkerSegment(JFIFMarkerSegment.class, true);
            if (jfifSeg != null) {
    ...
    private void mergeStandardChromaNode(Node node, NodeList siblings) throws IIOInvalidTreeException {
        ...
        if (csName.equals("GRAY")) {
            ...
            ids[1] = (byte) 'G';
            ids[2] = (byte) 'B';
        } else if ((csName.equals("XYZ")) || (csName.equals("Lab")) || (csName.equals("Luv")) || (csName
                .equals("YxY")) || (csName.equals("HSV")) || (csName.equals("HLS")) || (csName.equals("CMY"))
                || (csName.equals("3CLR"))) {
            numChannels = 3;
        ...
        if (sof != null) {
            ...
            if (progressive) {
                ...
                if (sos != null) {
                    ...
                    markerSequence.set(markerSequence.indexOf(sos), new SOSMarkerSegment(willSubsample, ids,
                            numChannels));
                }
            }
        } else {
                ...
                if (hadAlpha != wantAlpha) {
                    if (wantAlpha) { // Adding alpha
                        ...
                        newSpecs[numChannels - 1] = sof.getComponentSpec(newID,
                                sof.componentSpecs[0].HsamplingFactor, sof.componentSpecs[0].QtableSelector);
    ...
    private void setFromNativeTree(Node root) throws IIOInvalidTreeException {
        ...
        if (!isStream) {
            if (root.getChildNodes().getLength() != 2) { // JPEGvariety and
                                                         // markerSequence
                throw new IIOInvalidTreeException("JPEGvariety and markerSequence nodes must be present",
                        root);
            }
    ...
    void setFromMarkerSequenceNode(Node markerSequenceNode) throws IIOInvalidTreeException {
        ...
        for (int i = 0; i < children.getLength(); i++) {
            ...
            if (childName.equals("dqt")) {
                ...
                throw new IIOInvalidTreeException("Invalid " + (isStream ? "stream " : "image ") + "child: "
                        + childName, node);
            }
        }
    }
        ...
        if (!isStream) {
            if (sof != null) {
                ...
                if (jfif != null) {
                    ...
                    // YCC for 3-chan.
                    if ((adobe != null) && (((numSOFBands == 1) && (adobe.transform != JPEG.ADOBE_UNKNOWN))
    ...
    void writeToStream(ImageOutputStream ios, boolean ignoreJFIF, boolean forceJFIF, List thumbnails,
            ICC_Profile iccProfile, boolean ignoreAdobe, int newAdobeTransform, JPEGImageWriter writer)
            throws IOException {
        if (forceJFIF) {
