...
public class JPEGMetadata extends IIOMetadata implements Cloneable {
     ...
     * <code>markerSequence</code> is modified. This is used by reset to restore
     * the original state.
     */
    private List resetSequence = null;

    /**
     * Set to <code>true</code> when reading a thumbnail stored as JPEG. This is
     * used to enforce the prohibition of JFIF thumbnails containing any JFIF
     * marker segments, and to ensure generation of a correct native subtree
     * during <code>getAsTree</code>.
     */
    private boolean inThumb = false;

    /**
     * Set by the chroma node construction method to signal the presence or
     * absence of an alpha channel to the transparency node construction method.
     * Used only when constructing a standard metadata tree.
     */
    ...
    /**
     * All data is a list of <code>MarkerSegment</code> objects. When accessing
     * the list, use the tag to identify the particular subclass. Any JFIF
     * marker segment must be the first element of the list if it is present,
     * and any JFXX or APP2ICC marker segments are subordinate to the JFIF
     * marker segment. This list is package visible so that the writer can
     * access it.
     * 
     * @see #MarkerSegment
     */
    ...
    /**
     * Indicates whether this object represents stream or image metadata.
     * Package-visible so the writer can see it.
     */
    ...
    JPEGMetadata(boolean isStream, boolean inThumb) {
        super(true, // Supports standard format
                ...
                JPEG.nativeImageMetadataFormatClassName, null, null); // No
                                                                      // other
                                                                      // formats
        ...
        if (isStream) {
            ...
            nativeMetadataFormatClassName = JPEG.nativeStreamMetadataFormatClassName;
        }
    }

    /*
     * Constructs a <code>JPEGMetadata</code> object by reading the contents of
     * an <code>ImageInputStream</code>. Has package-only access.
     * @param isStream A boolean indicating whether this object will be stream
     * or image metadata.
     * @param isThumb A boolean indicating whether this metadata object is for
     * an image or for a thumbnail stored as JPEG.
     * @param iis An <code>ImageInputStream</code> from which to read the
     * metadata.
     * @param reader The <code>JPEGImageReader</code> calling this constructor,
     * to which warnings should be sent.
     */
    JPEGMetadata(boolean isStream, boolean isThumb, ImageInputStream iis,
        ...
        if (((buffer.buf[0] & 0xff) != 0xff) || ((buffer.buf[1]
                & 0xff) != JPEG.SOI) || ((buffer.buf[2] & 0xff) != 0xff)) {
        ...
        while (!done) {
            ...
            switch (buffer.buf[buffer.bufPtr] & 0xff) {
                ...
                case JPEG.APP0:
                    ...
                    buf = buffer.buf;
                    ptr = buffer.bufPtr;
                    if ((buf[ptr + 3] == 'J') && (buf[ptr + 4] == 'F')
                            && (buf[ptr + 5] == 'I') && (buf[ptr + 6] == 'F')
                            && (buf[ptr + 7] == 0)) {
                        if (inThumb) {
                            ...
                            JFIFMarkerSegment dummy = new JFIFMarkerSegment(
                                    buffer);
                        } else if (isStream) {
                            throw new IIOException(
                    ...
                    } else if ((buf[ptr + 3] == 'J') && (buf[ptr + 4] == 'F')
                            && (buf[ptr + 5] == 'X') && (buf[ptr + 6] == 'X')
                            && (buf[ptr + 7] == 0)) {
                        if (isStream) {
                            throw new IIOException(
    ...
    JPEGMetadata(ImageWriteParam param, JPEGImageWriter writer) {
        ...
        if (jparam != null) {
            ...
            markerSequence.add(new DHTMarkerSegment(jparam.getDCHuffmanTables(),
                    jparam.getACHuffmanTables()));
        } else {
            // default tables.
            markerSequence.add(new DQTMarkerSegment(JPEG.getDefaultQTables()));
            markerSequence.add(new DHTMarkerSegment(JPEG
                    .getDefaultHuffmanTables(true), JPEG
                            .getDefaultHuffmanTables(false)));
        }
     ...
     */
    JPEGMetadata(ImageTypeSpecifier imageType, ImageWriteParam param,
        ...
        if (param != null) {
            ...
            if (param.canWriteProgressive()) {
                ...
                if (param
                        .getProgressiveMode() == ImageWriteParam.MODE_DEFAULT) {
                    wantProg = true;
                    wantOptimized = true;
                    wantHTables = false;
                }
            ...
            if (param instanceof JPEGImageWriteParam) {
                ...
                if (jparam.areTablesSet()) {
                    wantQTables = false; // If the param has them, metadata
                                         // shouldn't
                    wantHTables = false;
                    if ((jparam.getDCHuffmanTables().length > 2) || (jparam
                            .getACHuffmanTables().length > 2)) {
                        wantExtended = true;
                    }
                }
            ...
            if (param.canWriteCompressed()) {
                if (param
                        .getCompressionMode() == ImageWriteParam.MODE_EXPLICIT) {
        ...
        if (destType != null) {
            ...
            boolean hasExtraComponents = (cm
                    .getNumColorComponents() != numComponents);
        ...
        // sof
        markerSequence.add(new SOFMarkerSegment(wantProg, wantExtended,
                willSubsample, componentIDs, numComponents));

        // sos
        if (!wantProg) { // Default progression scans are done in the writer
            markerSequence.add(new SOSMarkerSegment(willSubsample, componentIDs,
                    numComponents));
        }
    ...
    /**
     * Returns the first MarkerSegment object in the list with the given tag, or
    ...
    /**
     * Returns the first or last MarkerSegment object in the list of the given
    ...
    MarkerSegment findMarkerSegment(Class cls, boolean first) {
        if (first) {
            ...
            ListIterator iter = markerSequence.listIterator(markerSequence
                    .size());
    ...
    /**
     * Returns the index of the first or last MarkerSegment in the list of the
     * given class, or -1 if none is found.
     */
        ...
        if (jfif != null) {
            ...
            if (jfif.resUnits != 0) {
                ...
                IIOMetadataNode horiz = new IIOMetadataNode(
                        "HorizontalPixelSize");
                horiz.setAttribute("value", Float.toString(scale
                        / jfif.Xdensity));
                dim.appendChild(horiz);

                IIOMetadataNode vert = new IIOMetadataNode("VerticalPixelSize");
                vert.setAttribute("value", Float.toString(scale
                        / jfif.Ydensity));
    ...
    public void mergeTree(String formatName, Node root)
        ...
        if (isStream && (formatName.equals(
                JPEG.nativeStreamMetadataFormatName))) {
            mergeNativeTree(root);
        } else if (!isStream && (formatName.equals(
    ...
    private void mergeNativeTree(Node root) throws IIOInvalidTreeException {
        ...
        if (root.getChildNodes().getLength() != 2) { // JPEGvariety and
                                                     // markerSequence
            throw new IIOInvalidTreeException(
                    "JPEGvariety and markerSequence nodes must be present",
                    root);
        }
    ...
    /**
     * Merge a JFIF subtree into the marker sequence, if the subtree is
     * non-empty. If a JFIF marker exists, update it from the subtree. If none
     * exists, create one from the subtree and insert it at the beginning of the
     * marker sequence.
     */
    private void mergeJFIFsubtree(Node JPEGvariety)
        ...
        if (JPEGvariety.getChildNodes().getLength() != 0) {
            ...
            JFIFMarkerSegment jfifSeg = (JFIFMarkerSegment) findMarkerSegment(
                    JFIFMarkerSegment.class, true);
            if (jfifSeg != null) {
    ...
    private void mergeSequenceSubtree(Node sequenceTree)
        ...
        for (int i = 0; i < children.getLength(); i++) {
            ...
            if (name.equals("dqt")) {
                ...
                throw new IIOInvalidTreeException("Invalid node: " + name,
                        node);
            }
        }
    }
    ...
    /**
     * Merge the given DQT node into the marker sequence. If there already exist
     * DQT marker segments in the sequence, then each table in the node replaces
     * the first table, in any DQT segment, with the same table id. If none of
     * the existing DQT segments contain a table with the same id, then the
     * table is added to the last existing DQT segment. If there are no DQT
     * segments, then a new one is created and added as follows: If there are
     * DHT segments, the new DQT segment is inserted before the first one. If
     * there are no DHT segments, the new DQT segment is inserted before an SOF
     * segment, if there is one. If there is no SOF segment, the new DQT segment
     * is inserted before the first SOS segment, if there is one. If there is no
     * SOS segment, the new DQT segment is added to the end of the sequence.
     */
    private void mergeDQTNode(Node node) throws IIOInvalidTreeException {
        ...
        if (!oldDQTs.isEmpty()) {
            ...
            for (int i = 0; i < children.getLength(); i++) {
                ...
                int childID = MarkerSegment.getAttributeValue(child, null,
                        "qtableId", 0, 3, true);
                DQTMarkerSegment dqt = null;
                int tableIndex = -1;
                for (int j = 0; j < oldDQTs.size(); j++) {
                    DQTMarkerSegment testDQT = (DQTMarkerSegment) oldDQTs.get(
                            j);
                    for (int k = 0; k < testDQT.tables.size(); k++) {
                        DQTMarkerSegment.Qtable testTable = (DQTMarkerSegment.Qtable) testDQT.tables
                                .get(k);
            ...
            int firstDHT = findMarkerSegmentPosition(DHTMarkerSegment.class,
                    true);
            int firstSOF = findMarkerSegmentPosition(SOFMarkerSegment.class,
                    true);
            int firstSOS = findMarkerSegmentPosition(SOSMarkerSegment.class,
                    true);
            if (firstDHT != -1) {
    ...
    /**
     * Merge the given DHT node into the marker sequence. If there already exist
     * DHT marker segments in the sequence, then each table in the node replaces
     * the first table, in any DHT segment, with the same table class and table
     * id. If none of the existing DHT segments contain a table with the same
     * class and id, then the table is added to the last existing DHT segment.
     * If there are no DHT segments, then a new one is created and added as
     * follows: If there are DQT segments, the new DHT segment is inserted
     * immediately following the last DQT segment. If there are no DQT segments,
     * the new DHT segment is inserted before an SOF segment, if there is one.
     * If there is no SOF segment, the new DHT segment is inserted before the
     * first SOS segment, if there is one. If there is no SOS segment, the new
     * DHT segment is added to the end of the sequence.
     */
    private void mergeDHTNode(Node node) throws IIOInvalidTreeException {
        ...
        if (!oldDHTs.isEmpty()) {
            ...
            for (int i = 0; i < children.getLength(); i++) {
                ...
                int childID = MarkerSegment.getAttributeValue(child, attrs,
                        "htableId", 0, 3, true);
                int childClass = MarkerSegment.getAttributeValue(child, attrs,
                        "class", 0, 1, true);
                DHTMarkerSegment dht = null;
                int tableIndex = -1;
                for (int j = 0; j < oldDHTs.size(); j++) {
                    DHTMarkerSegment testDHT = (DHTMarkerSegment) oldDHTs.get(
                            j);
                    for (int k = 0; k < testDHT.tables.size(); k++) {
                        DHTMarkerSegment.Htable testTable = (DHTMarkerSegment.Htable) testDHT.tables
                                .get(k);
                        if ((childID == testTable.tableID)
            ...
            int lastDQT = findMarkerSegmentPosition(DQTMarkerSegment.class,
                    false);
            int firstSOF = findMarkerSegmentPosition(SOFMarkerSegment.class,
                    true);
            int firstSOS = findMarkerSegmentPosition(SOSMarkerSegment.class,
    ...
    /**
     * Merge the given DRI node into the marker sequence. If there already
     * exists a DRI marker segment, the restart interval value is updated. If
     * there is no DRI segment, then a new one is created and added as follows:
     * If there is an SOF segment, the new DRI segment is inserted before it. If
     * there is no SOF segment, the new DRI segment is inserted before the first
     * SOS segment, if there is one. If there is no SOS segment, the new DRI
     * segment is added to the end of the sequence.
     */
    private void mergeDRINode(Node node) throws IIOInvalidTreeException {
        DRIMarkerSegment dri = (DRIMarkerSegment) findMarkerSegment(
    ...
    /**
     * Merge the given COM node into the marker sequence. A new COM marker
     * segment is created and added to the sequence using
     * insertCOMMarkerSegment.
     */
    ...
    /**
     * Insert a new COM marker segment into an appropriate place in the marker
     * sequence, as follows: If there already exist COM marker segments, the new
     * one is inserted after the last one. If there are no COM segments, the new
     * COM segment is inserted after the JFIF segment, if there is one. If there
     * is no JFIF segment, the new COM segment is inserted after the Adobe
     * marker segment, if there is one. If there is no Adobe segment, the new
     * COM segment is inserted at the beginning of the sequence.
     */
    private void insertCOMMarkerSegment(COMMarkerSegment newGuy) {
        ...
        boolean hasJFIF = (findMarkerSegment(JFIFMarkerSegment.class,
                true) != null);
    ...
    /**
     * Merge the given Adobe APP14 node into the marker sequence. If there
     * already exists an Adobe marker segment, then its attributes are updated
     * from the node. If there is no Adobe segment, then a new one is created
     * and added using insertAdobeMarkerSegment.
     */
    ...
    /**
     * Insert the given AdobeMarkerSegment into the marker sequence, as follows
     * (we assume there is no Adobe segment yet): If there is a JFIF segment,
     * then the new Adobe segment is inserted after it. If there is no JFIF
     * segment, the new Adobe segment is inserted after the last Unknown
     * segment, if there are any. If there are no Unknown segments, the new
     * Adobe segment is inserted at the beginning of the sequence.
     */
    ...
    /**
     * Merge the given Unknown node into the marker sequence. A new Unknown
     * marker segment is created and added to the sequence as follows: If there
     * already exist Unknown marker segments, the new one is inserted after the
     * last one. If there are no Unknown marker segments, the new Unknown marker
     * segment is inserted after the JFIF segment, if there is one. If there is
     * no JFIF segment, the new Unknown segment is inserted before the Adobe
     * marker segment, if there is one. If there is no Adobe segment, the new
     * Unknown segment is inserted at the beginning of the sequence.
     */
    ...
    /**
     * Merge the given SOF node into the marker sequence. If there already
     * exists an SOF marker segment in the sequence, then its values are updated
     * from the node. If there is no SOF segment, then a new one is created and
     * added as follows: If there are any SOS segments, the new SOF segment is
     * inserted before the first one. If there is no SOS segment, the new SOF
     * segment is added to the end of the sequence.
     *
     */
    ...
    /**
     * Merge the given SOS node into the marker sequence. If there already
     * exists a single SOS marker segment, then the values are updated from the
     * node. If there are more than one existing SOS marker segments, then an
     * IIOInvalidTreeException is thrown, as SOS segments cannot be merged into
     * a set of progressive scans. If there are no SOS marker segments, a new
     * one is created and added to the end of the sequence.
     */
    private void mergeSOSNode(Node node) throws IIOInvalidTreeException {
        SOSMarkerSegment firstSOS = (SOSMarkerSegment) findMarkerSegment(
                SOSMarkerSegment.class, true);
        SOSMarkerSegment lastSOS = (SOSMarkerSegment) findMarkerSegment(
                SOSMarkerSegment.class, false);
        if (firstSOS != null) {
            if (firstSOS != lastSOS) {
                throw new IIOInvalidTreeException(
                        "Can't merge SOS node into a tree with > 1 SOS node",
                        node);
            }
    ...
    private void mergeStandardChromaNode(Node node, NodeList siblings)
        ...
        if ((csType == null) || !csType.getNodeName().equals(
                "ColorSpaceType")) {
            // If there is no ColorSpaceType node, we have nothing to do
            return;
        }

        String csName = csType.getAttributes().getNamedItem("name")
                .getNodeValue();
        ...
        if (csName.equals("GRAY")) {
            ...
            ids[1] = (byte) 'G';
            ids[2] = (byte) 'B';
        } else if ((csName.equals("XYZ")) || (csName.equals("Lab")) || (csName
                .equals("Luv")) || (csName.equals("YxY")) || (csName.equals(
                        "HSV")) || (csName.equals("HLS")) || (csName.equals(
                                "CMY")) || (csName.equals("3CLR"))) {
            numChannels = 3;
        ...
        // must match, so that we can modify all the existing SOS marker
        // segments.
        ...
        // Now add a JFIF if we do want one, but only if it isn't stream
        // metadata
        ...
        int[] newTableSelectors = willSubsample ? subsampledSelectors
        ...
        if (sof != null) {
            ...
            markerSequence.set(markerSequence.indexOf(sof),
                    new SOFMarkerSegment(progressive, false, // we
                            // never
                            // need
                            // extended
                            willSubsample, ids, numChannels));
            ...
            if (progressive) {
                // if the component ids are different, update all the existing
                // scans
                ...
                if (idsDiffer) {
                    ...
                    for (Iterator iter = markerSequence.iterator(); iter
                            .hasNext();) {
                        MarkerSegment seg = (MarkerSegment) iter.next();
                        if (seg instanceof SOSMarkerSegment) {
                            ...
                            for (int i = 0; i < target.componentSpecs.length; i++) {
                                ...
                                // Find the position in the old componentSpecs
                                // array
                                ...
                                // the new component specs array in the SOF
                                // created
                                // above.
                ...
                if (sos != null) {
                    // htables - if the old htable selectors don't match the new
                    // ones,
                    // update the tables.
                    for (int i = 0; i < sos.componentSpecs.length; i++) {
                        if ((sos.componentSpecs[i].dcHuffTable != newTableSelectors[i])
                                || (sos.componentSpecs[i].acHuffTable != newTableSelectors[i])) {
                            updateHtables = true;
                        }
                    }
                    ...
                    markerSequence.set(markerSequence.indexOf(sos),
                            new SOSMarkerSegment(willSubsample, ids,
                                    numChannels));
                }
            }
        } else {
            // should be stream metadata if there isn't an SOF, but check it
            // anyway
            if (isStream) {
        ...
        if (updateQtables) {
            ...
            // If there are no tables, don't add them, as the metadata encodes
            // an
            // abbreviated stream.
            ...
            if (!tableSegments.isEmpty() && willSubsample) {
                ...
                boolean found = false;
                for (Iterator iter = tableSegments.iterator(); iter
                        .hasNext();) {
                    DQTMarkerSegment testdqt = (DQTMarkerSegment) iter.next();
                    for (Iterator tabiter = testdqt.tables.iterator(); tabiter
                            .hasNext();) {
                        DQTMarkerSegment.Qtable tab = (DQTMarkerSegment.Qtable) tabiter
                                .next();
                        if (tab.tableID == 1) {
                            found = true;
                        }
                ...
                if (!found) {
                    ...
                    for (Iterator iter = tableSegments.iterator(); iter
                            .hasNext();) {
                        DQTMarkerSegment testdqt = (DQTMarkerSegment) iter
                                .next();
                        for (Iterator tabiter = testdqt.tables
                                .iterator(); tabiter.hasNext();) {
                            DQTMarkerSegment.Qtable tab = (DQTMarkerSegment.Qtable) tabiter
                                    .next();
                            if (tab.tableID == 0) {
                                table0 = tab;
                    ...
                    DQTMarkerSegment dqt = (DQTMarkerSegment) tableSegments.get(
                            tableSegments.size() - 1);
        ...
        if (updateHtables) {
            ...
            // If we are not subsampling, we just need one, so don't do anything
            if (!tableSegments.isEmpty() && willSubsample) {
                // Is it really necessary? There should be at least 2 dc and 2
                // ac
                ...
                for (Iterator iter = tableSegments.iterator(); iter
                    ...
                    for (Iterator tabiter = testdht.tables.iterator(); tabiter
                            .hasNext();) {
                        DHTMarkerSegment.Htable tab = (DHTMarkerSegment.Htable) tabiter
                                .next();
                        if (tab.tableID == 1) {
                            found = true;
                        }
                ...
                if (!found) {
                    // Create new standard dc and ac chrominance tables and add
                    // them
                    // to the last DHT segment
                    DHTMarkerSegment lastDHT = (DHTMarkerSegment) tableSegments
                            .get(tableSegments.size() - 1);
    ...
    private boolean wantAlpha(Node transparency) {
        ...
        Node alpha = transparency.getFirstChild(); // Alpha must be first if
                                                   // present
        if (alpha.getNodeName().equals("Alpha")) {
            if (alpha.hasAttributes()) {
                String value = alpha.getAttributes().getNamedItem("value")
                        .getNodeValue();
    ...
    private void mergeStandardDimensionNode(Node node)
        ...
        if (jfif == null) {
            // Can there be one?
            // Criteria:
            // SOF must be present with 1 or 3 channels, (stream metadata fails
            // this)
            ...
            if (sof != null) {
                ...
                if ((numChannels == 1) || (numChannels == 3)) {
                    ...
                    // if Adobe present, transform = ADOBE_UNKNOWN for
                    // 1-channel,
        ...
        if (jfif != null) {
            ...
            for (int i = 0; i < children.getLength(); i++) {
                ...
                if (name.equals("PixelAspectRatio")) {
                    String valueString = attrs.getNamedItem("value")
                            .getNodeValue();
    ...
    /*
     * Return a pair of integers whose ratio (x/y) approximates the given float
     * value.
     */
            ...
            if (sof != null) {
                ...
                if (hadAlpha != wantAlpha) {
                    if (wantAlpha) { // Adding alpha
                        ...
                        // If an adobe marker is present, transform must be
                        // UNKNOWN
                        if (adobe != null) {
                            adobe.transform = JPEG.ADOBE_UNKNOWN;
                        }

                        // Add a component spec with appropriate parameters to
                        // SOF
                        ...
                        newSpecs[numChannels - 1] = sof.getComponentSpec(newID,
                        ...
                        newScanSpecs[numChannels - 1] = sos
                                .getScanComponentSpec(newID, 0);
    ...
    private void setFromNativeTree(Node root) throws IIOInvalidTreeException {
        ...
        if (!isStream) {
            ...
            if (JPEGvariety.getChildNodes().getLength() != 0) {
                markerSequence.add(new JFIFMarkerSegment(JPEGvariety
                        .getFirstChild()));
            }
        }
    ...
    void setFromMarkerSequenceNode(Node markerSequenceNode)
        ...
        for (int i = 0; i < children.getLength(); i++) {
            ...
            if (childName.equals("dqt")) {
                ...
                throw new IIOInvalidTreeException("Invalid " + (isStream
                        ? "stream "
                        : "image ") + "child: " + childName, node);
            }
        }
    }

    /**
     * Check that this metadata object is in a consistent state and return
     * <code>true</code> if it is or <code>false</code> otherwise. All the
     * constructors and modifiers should call this method at the end to
     * guarantee that the data is always consistent, as the writer relies on
     * this.
     */
    private boolean isConsistent() {
        ...
        if (!isStream) {
            if (sof != null) {
                ...
                SOSMarkerSegment sos = (SOSMarkerSegment) findMarkerSegment(
                        SOSMarkerSegment.class, true);
                if ((jfif != null) || (adobe != null) || (sof != null)
                        || (sos != null)) {
                    retval = false;
                }
            }
    ...
    /**
     * Returns the total number of bands referenced in all SOS marker segments,
     * including 0 if there are no SOS marker segments.
     */
    ...
    void writeToStream(ImageOutputStream ios, boolean ignoreJFIF,
            boolean forceJFIF, List thumbnails, ICC_Profile iccProfile,
            boolean ignoreAdobe, int newAdobeTransform, JPEGImageWriter writer)
            throws IOException {
        if (forceJFIF) {
            ...
            if (seg instanceof JFIFMarkerSegment) {
                ...
                if (ignoreAdobe == false) {
                    if (newAdobeTransform != JPEG.ADOBE_IMPOSSIBLE) {
                        AdobeMarkerSegment newAdobe = (AdobeMarkerSegment) seg
                                .clone();
