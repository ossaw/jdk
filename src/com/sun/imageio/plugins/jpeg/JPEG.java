...
/**
 * A class containing JPEG-related constants, definitions, and static methods.
 * This class and its constants must be public so that
...
public class JPEG {
    ...
    /*
     * These are kept in the inner class to avoid static initialization of the
     * CMM class until someone actually needs it. (e.g. do not init CMM on the
     ...
     * <code>ColorSpaces</code> returned by
     * <code>ColorSpace.getInstance()</code>.
     */
    static boolean isNonStandardICC(ColorSpace cs) {
        boolean retval = false;
        if ((cs instanceof ICC_ColorSpace) && (!cs.isCS_sRGB()) && (!cs.equals(ColorSpace.getInstance(
                ColorSpace.CS_CIEXYZ))) && (!cs.equals(ColorSpace.getInstance(ColorSpace.CS_GRAY))) && (!cs
                        .equals(ColorSpace.getInstance(ColorSpace.CS_LINEAR_RGB))) && (!cs.equals(ColorSpace
                                .getInstance(ColorSpace.CS_PYCC)))) {
            retval = true;
        }
        return retval;
    }
    ...
    /**
     * Returns <code>true</code> if the given imageType can be used in a JFIF
    ...
    /**
     * Given an image type, return the Adobe transform corresponding to that
     * type, or ADOBE_IMPOSSIBLE if the image type is incompatible with an Adobe
     * marker segment. If <code>input</code> is true, then the image type is
     * considered before colorspace conversion.
     */
    ...
    /**
     * Converts an ImageWriteParam (i.e. IJG) non-linear quality value to a
     * float suitable for passing to JPEGQTable.getScaledInstance().
     */
