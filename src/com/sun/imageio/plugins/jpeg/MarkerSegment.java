...
/**
 * All metadata is stored in MarkerSegments. Marker segments that we know about
 * are stored in subclasses of this basic class, which used for unrecognized
 * APPn marker segments. XXX break out UnknownMarkerSegment as a subclass and
 * make this abstract, avoiding unused data field.
 */
class MarkerSegment implements Cloneable {
	protected static final int LENGTH_SIZE = 2; // length is 2 bytes
	int tag; // See JPEG.java
	int length; /*
				 * Sometimes needed by subclasses; doesn't include itself.
				 * Meaningful only if constructed from a stream
				 */
	...
	/**
	 * Constructor for creating <code>MarkerSegment</code>s by reading from an
	...
	/**
	 * Constructor used when creating segments other than by reading them from a
	 * stream.
	 */
	...
	/**
	 * We have determined that we don't know the type, so load the data using
	...
	/**
	 * Writes the marker, tag, and length. Note that length should be verified
	 * by the caller as a correct JPEG length, i.e it includes itself.
	 */
