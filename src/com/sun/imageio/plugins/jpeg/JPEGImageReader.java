...
public class JPEGImageReader extends ImageReader {
	...
	/**
	 * The following variable contains a pointer to the IJG library structure
	 * for this reader. It is assigned in the constructor and then is passed in
	 * to every native call. It is set to 0 by dispose to avoid disposing twice.
	 */
	...
	/**
	 * List of stream positions for images, reinitialized every time a new input
	 * source is set.
	 */
	...
	/**
	 * Warning code to be passed to warningOccurred to indicate that the EOI
	 * marker is missing from the end of the stream. This usually signals that
	 * the stream is corrupted, but everything up to the last MCU should be
	 * usable.
	 */
	protected static final int WARNING_NO_EOI = 0;

	/**
	 * Warning code to be passed to warningOccurred to indicate that a JFIF
	 * segment was encountered inside a JFXX JPEG thumbnail and is being
	 * ignored.
	 */
	protected static final int WARNING_NO_JFIF_IN_THUMB = 1;

	/**
	 * Warning code to be passed to warningOccurred to indicate that embedded
	 * ICC profile is invalid and will be ignored.
	 */
	...
	private int height;
	/**
	 * Set by setImageData native code callback. A modified IJG+NIFTY colorspace
	 * code.
	 */
	...
	/**
	 * Set to true every time we seek in the stream; used to invalidate the
	 * native buffer contents in C.
	 */
	private boolean haveSeeked = false;

	/**
	 * Tables that have been read from a tables-only image at the beginning of a
	 * stream.
	 */
	...
	private static native void initReaderIDs(Class iisClass, Class qTableClass, Class huffClass);
	...
	/**
	 * Called by the native code or other classes to signal a warning. The code
	 * is used to lookup a localized message to be used when sending warnings to
	 * listeners.
	 */
	protected void warningOccurred(int code) {
		...
		try {
			...
			processWarningOccurred("com.sun.imageio.plugins.jpeg.JPEGImageReaderResources",
	...
	/**
	 * The library has it's own error facility that emits warning messages. This
	 * routine is called by the native code when it has already formatted a
	 * string for output. XXX For truly complete localization of all warning
	 * messages, the sun_jpeg_output_message routine in the native code should
	 * send only the codes and parameters to a method here in Java, which will
	 * then format and send the warnings, using localized strings. This method
	 * will have to deal with all the parameters and formats (%u with possibly
	 * large numbers, %02d, %02x, etc.) that actually occur in the JPEG library.
	 * For now, this prevents library warnings from being printed to stderr.
	 */
	...
	/**
	 * This method is called from native code in order to fill native input
	 * buffer.
	 *
	 * We block any attempt to change the reading state during this method, in
	 * order to prevent a corruption of the native decoder state.
	 *
	...
	/**
	 * This method is called from the native code in order to skip requested
	...
	/**
	 * Skip over a complete image in the stream, leaving the stream positioned
	 * such that the next byte to be read is the first byte of the next image.
	 * For JPEG, this means that we read until we encounter an EOI marker or
	 * until the end of the stream. If the stream ends before an EOI marker is
	 * encountered, an IndexOutOfBoundsException is thrown.
	 */
	...
	/**
	 * Returns <code>true</code> if there is an image beyond the current stream
	 * position. Does not disturb the stream position.
	 */
	...
	/**
	 * Push back the given number of bytes to the input stream. Called by the
	 * native code at the end of each image so that the next one can be
	 * identified from Java.
	 */
	...
	/**
	 * Read in the header information starting from the current stream position,
	 * returning <code>true</code> if the header was a tables-only image. After
	 * this call, the native IJG decompression struct will contain the image
	 * information required by most query calls below (e.g. getWidth, getHeight,
	 * etc.), if the header was not a tables-only image. If reset is
	 * <code>true</code>, the state of the IJG object is reset so that it can
	 * read a header again. This happens automatically if the header was a
	 * tables-only image.
	 */
	private native boolean readImageHeader(long structPointer, boolean clearBuffer, boolean reset)
			throws IOException;

	/*
	 * Called by the native code whenever an image header has been read. Whether
	 * we read metadata or not, we always need this information, so it is passed
	 * back independently of metadata, which may never be read.
	 */
	private void setImageData(int width, int height, int colorSpaceCode, int outColorSpaceCode,
			int numComponents, byte[] iccData) {
		this.width = width;
		this.height = height;
		...
		/*
		 * At the moment we can't rely on the ColorSpace.equals() and
		 * ICC_Profile.equals() because they do not detect the case when two
		 * profiles are created from same data.
		 *
		 * So, we have to do data comparison in order to avoid creation of
		 * different ColorSpace instances for the same embedded data.
		 */
		if (oldData == null || !java.util.Arrays.equals(oldData, newData)) {
			...
			try {
				...
				/*
				 * Embedded profile seems to be corrupted. Ignore this profile.
				 */
				iccCS = null;
				cbLock.lock();
				try {
	...
	/**
	 * Return an ImageTypeSpecifier corresponding to the given color space code,
	 * or null if the color space is unsupported.
	 */
			...
			if (iccCS != null) {
				list.add(new ImageTypeProducer() {
					protected ImageTypeSpecifier produce() {
						return ImageTypeSpecifier.createInterleaved(iccCS, JPEG.bOffsRGB, // Assume
																							// it's
																							// for
																							// RGB
	...
	/**
	 * Checks the implied color conversion between the stream and the target
	 * image, altering the IJG output color space if necessary. If a java color
	 * conversion is required, then this sets up <code>convert</code>. If bands
	 * are being rearranged at all (either source or destination bands are
	 * specified in the param), then the default color conversions are assumed
	 * to be correct. Throws an IIOException if there is no conversion
	 * available.
	 */
	private void checkColorConversion(BufferedImage image, ImageReadParam param)
			throws IIOException {
		...
		case JPEG.JCS_RGB: // IJG wants to go to RGB
			...
			} else if ((iccCS != null) && (cm.getNumComponents() == numComponents)
					&& (cs != iccCS)) {
			...
			} else if ((iccCS == null) && (!cs.isCS_sRGB())
		...
		case JPEG.JCS_YCCA: {
			...
			if ((YCC == null) || // We can't do YCC at all
					(cs != YCC) || (cm.getNumComponents() != numComponents)) {
				throw new IIOException("Incompatible color conversion");
			}
		}
			break;
	...
	/**
	 * Set the IJG output space to the given value. The library will perform the
	 * appropriate colorspace conversions.
	 */
	...
	public IIOMetadata getImageMetadata(int imageIndex) throws IOException {
		...
		try {
			...
			if ((imageMetadataIndex == imageIndex) && (imageMetadata != null)) {
	...
	private Raster readInternal(int imageIndex, ImageReadParam param, boolean wantRaster)
			throws IOException {
		...
		raster = Raster.createInterleavedRaster(buffer, destROI.width, 1, lineSize, numRasterBands,
				bandOffs, null);
		...
		if (wantRaster) {
			target = Raster.createInterleavedRaster(DataBuffer.TYPE_BYTE, destROI.width,
					destROI.height, lineSize, numRasterBands, bandOffs, null);
		} else {
			target = imRas;
		}
		...
		/*
		 * If the process is sequential, and we have restart markers, we could
		 * skip to the correct restart marker, if the library lets us. That's an
		 * optimization to investigate later.
		 */
		...
		aborted = readImage(imageIndex, structPointer, buffer.getData(), numRasterBands, srcBands,
				bandSizes, srcROI.x, srcROI.y, srcROI.width, srcROI.height, periodX, periodY,
				abbrevQTables, abbrevDCHuffmanTables, abbrevACHuffmanTables, minProgressivePass,
				maxProgressivePass, callbackUpdates);
	...
	/**
	 * This method is called back from C when the intermediate Raster is full.
	 * The parameter indicates the scanline in the target Raster to which the
	 * intermediate Raster should be copied. After the copy, we notify update
	 * listeners.
	 */
	private void acceptPixels(int y, boolean progressive) {
		...
		cbLock.lock();
		try {
			processImageUpdate(image, destROI.x, destROI.y + y, raster.getWidth(), 1, 1, 1,
					destinationBands);
			if ((y > 0) && (y % progInterval == 0)) {
				...
				if (progressive) {
					if (knownPassCount != UNKNOWN) {
						processImageProgress((pass + percentOfPass) * 100.0F / knownPassCount);
					} else if (maxProgressivePass != Integer.MAX_VALUE) {
						...
						// pass scaled to be the percentage of the total
						// remaining,
						...
						progInterval = Math.max(height / 20 * totalPasses, totalPasses);
						if (y % progInterval == 0) {
							percentToDate = previousPassPercentage + (1.0F - previousPassPercentage)
	...
	private void passStarted(int pass) {
		...
		try {
			...
			processPassStarted(image, pass, minProgressivePass, maxProgressivePass, 0, 0, 1, 1,
					destinationBands);
		} finally {
			cbLock.unlock();
		}
	...
	private native boolean readImage(int imageIndex, long structPointer, byte[] buffer,
			int numRasterBands, int[] srcBands, int[] bandSizes, int sourceXOffset,
			int sourceYOffset, int sourceWidth, int sourceHeight, int periodX, int periodY,
			JPEGQTable[] abbrevQTables, JPEGHuffmanTable[] abbrevDCHuffmanTables,
			JPEGHuffmanTable[] abbrevACHuffmanTables, int minProgressivePass,
			int maxProgressivePass, boolean wantUpdates);
	...
	public Raster readRaster(int imageIndex, ImageReadParam param) throws IOException {
		...
		try {
			cbLock.check();
			/*
			 * This could be further optimized by not resetting the dest. offset
			 * and creating a translated raster in readInternal() (see bug
			 * 4994702 for more info).
			 */

			// For Rasters, destination offset is logical, not physical, so
			// set it to 0 before calling computeRegions, so that the
			// destination
			...
			if (saveDestOffset != null) {
				target = target.createWritableTranslatedChild(saveDestOffset.x, saveDestOffset.y);
			}
		} catch (RuntimeException e) {
			resetLibraryState(structPointer);
			throw e;
	...
	public int getNumThumbnails(int imageIndex) throws IOException {
		...
		try {
			...
			JFIFMarkerSegment jfif = (JFIFMarkerSegment) imageMetadata
	...
	private synchronized void setThreadLock() {
		...
		if (theThread != null) {
			if (theThread != currThread) {
				...
				// by multiple threads.
				throw new IllegalStateException("Attempt to use instance of " + this
						+ " locked on thread " + theThread + " from thread " + currThread);
			} else {
				theLockCount++;
			}
		} else {
...
/**
 * An internal helper class that wraps producer's iterator and extracts
 * specifier instances on demand.
 */
...
/**
 * An internal helper class that provides means for deferred creation of
 * ImageTypeSpecifier instance required to describe available destination types.
 *
 * This implementation only supports standard jpeg color spaces (defined by
 * corresponding JCS color space code).
 *
 * To support other color spaces one can override produce() method to return
 * custom instance of ImageTypeSpecifier.
 */
class ImageTypeProducer {
	...
	private static final ImageTypeProducer[] defaultTypes = new ImageTypeProducer[JPEG.NUM_JCS_CODES];
	...
	protected ImageTypeSpecifier produce() {
		...
		case JPEG.JCS_RGB:
			return ImageTypeSpecifier.createInterleaved(JPEG.JCS.sRGB, JPEG.bOffsRGB,
					DataBuffer.TYPE_BYTE, false, false);
		case JPEG.JCS_RGBA:
			return ImageTypeSpecifier.createPacked(JPEG.JCS.sRGB, 0xff000000, 0x00ff0000,
					0x0000ff00, 0x000000ff, DataBuffer.TYPE_INT, false);
		case JPEG.JCS_YCC:
			if (JPEG.JCS.getYCC() != null) {
				return ImageTypeSpecifier.createInterleaved(JPEG.JCS.getYCC(), JPEG.bandOffsets[2],
						DataBuffer.TYPE_BYTE, false, false);
			} else {
				return null;
			}
		case JPEG.JCS_YCCA:
			if (JPEG.JCS.getYCC() != null) {
				return ImageTypeSpecifier.createInterleaved(JPEG.JCS.getYCC(), JPEG.bandOffsets[3],
