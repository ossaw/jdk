...
class SOSMarkerSegment extends MarkerSegment {
	...
	SOSMarkerSegment(boolean willSubsample, byte[] componentIDs, int numComponents) {
		...
		for (int i = 0; i < numComponents; i++) {
			...
			componentSpecs[i] = new ScanComponentSpec(componentIDs[i], tableSel);
		}
	}
	...
	protected Object clone() {
		...
		if (componentSpecs != null) {
			newGuy.componentSpecs = (ScanComponentSpec[]) componentSpecs.clone();
			for (int i = 0; i < componentSpecs.length; i++) {
				newGuy.componentSpecs[i] = (ScanComponentSpec) componentSpecs[i].clone();
			}
		}
		return newGuy;
	}
	...
	void updateFromNativeNode(Node node, boolean fromScratch) throws IIOInvalidTreeException {
		...
		if (children.getLength() != numComponents) {
			throw new IIOInvalidTreeException("numScanComponents must match the number of children",
					node);
		}
		componentSpecs = new ScanComponentSpec[numComponents];
		for (int i = 0; i < numComponents; i++) {
	...
	class ScanComponentSpec implements Cloneable {
		...
		ScanComponentSpec(Node node) throws IIOInvalidTreeException {
			NamedNodeMap attrs = node.getAttributes();
			componentSelector = getAttributeValue(node, attrs, "componentSelector", 0, 255, true);
			dcHuffTable = getAttributeValue(node, attrs, "dcHuffTable", 0, 3, true);
			acHuffTable = getAttributeValue(node, attrs, "acHuffTable", 0, 3, true);
		}
		...
		IIOMetadataNode getNativeNode() {
			...
			node.setAttribute("componentSelector", Integer.toString(componentSelector));
