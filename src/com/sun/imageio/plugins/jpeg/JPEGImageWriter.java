...
public class JPEGImageWriter extends ImageWriter {
	...
	private void writeOnThread(IIOMetadata streamMetadata, IIOImage image, ImageWriteParam param)
		...
		if (rasterOnly) {
			...
			if (rimage instanceof BufferedImage) {
				...
				if (srcRas.getWidth() != rimage.getWidth()
					...
					srcRas = srcRas.createChild(srcRas.getMinX(), srcRas.getMinY(),
							rimage.getWidth(), rimage.getHeight(), srcRas.getMinX(),
							srcRas.getMinY(), null);
				}
			} else {
		...
		if (metadata != null) {
			...
			if (jfif == null) {
				jfif = (JFIFMarkerSegment) metadata.findMarkerSegment(JFIFMarkerSegment.class,
						true);
			}
		}
		...
		raster = Raster.createInterleavedRaster(buffer, sourceWidth, 1, lineSize, numBandsUsed,
				bandOffs, null);
		...
		aborted = writeImage(structPointer, buffer.getData(), inCsType, outCsType, numBandsUsed,
				bandSizes, sourceWidth, destWidth, destHeight, periodX, periodY, qTables, writeDQT,
	...
	private void prepareWriteSequenceOnThread(IIOMetadata streamMetadata) throws IOException {
		...
		/*
		 * from jpeg_metadata.html: If no stream metadata is supplied to
		 * <code>ImageWriter.prepareWriteSequence</code>, then no tables-only
		 * image is written. If stream metadata containing no tables is supplied
		 * to <code>ImageWriter.prepareWriteSequence</code>, then a tables-only
			...
			if (streamMetadata instanceof JPEGMetadata) {
				...
				if (debug) {
					System.out.println("after collecting from stream metadata, "
							+ "streamQTables.length is " + streamQTables.length);
				}
	...
	private void checkJFIF(JFIFMarkerSegment jfif, ImageTypeSpecifier type, boolean input) {
		if (jfif != null) {
			if (!JPEG.isJFIFcompliant(type, input)) {
				...
				warningOccurred(input ? WARNING_IMAGE_METADATA_JFIF_MISMATCH
	...
	private void checkAdobe(AdobeMarkerSegment adobe, ImageTypeSpecifier type, boolean input) {
		if (adobe != null) {
			...
			if (adobe.transform != rightTransform) {
				warningOccurred(input ? WARNING_IMAGE_METADATA_ADOBE_MISMATCH
	...
	/**
	 * Collect all the scan info from the given metadata, and organize it into
	 * the scan info array required by the IJG libray. It is much simpler to
	 * parse out this data in Java and then just copy the data in C.
	 */
	...
	/**
	 * Finds all DQT marker segments and returns all the q tables as a single
	 * array of JPEGQTables.
	 */
	...
	/**
	 * Finds all DHT marker segments and returns all the q tables as a single
	 * array of JPEGQTables. The metadata must not be for a progressive image,
	...
	private native boolean writeImage(long structPointer, byte[] data, int inCsType, int outCsType,
			int numBands, int[] bandSizes, int srcWidth, int destWidth, int destHeight, int stepX,
			int stepY, JPEGQTable[] qtables, boolean writeDQT, JPEGHuffmanTable[] DCHuffmanTables,
			JPEGHuffmanTable[] ACHuffmanTables, boolean writeDHT, boolean optimizeHuffman,
			boolean progressive, int numScans, int[] scans, int[] componentIds,
			int[] HsamplingFactors, int[] VsamplingFactors, int[] QtableSelectors,
			boolean haveMetadata, int restartInterval);

	/**
	 * Writes the metadata out when called by the native code, which will have
	 * already written the header to the stream and established the library
	 * state. This is simpler than breaking the write call in two.
	 */
	 ...
	 * for writing. This handles ROI and band rearrangements, and expands
	 * indexed images. Subsampling is done in the native code. This is called by
	 * the native code.
	 */
	private void grabPixels(int y) {
		...
		if (isAlphaPremultiplied) {
			...
			data = sourceLine.getPixels(sourceLine.getMinX(), sourceLine.getMinY(),
					sourceLine.getWidth(), sourceLine.getHeight(), data);
			wr.setPixels(sourceLine.getMinX(), sourceLine.getMinY(), sourceLine.getWidth(),
					sourceLine.getHeight(), data);
			srcCM.coerceData(wr, false);
			sourceLine = wr.createChild(wr.getMinX(), wr.getMinY(), wr.getWidth(), wr.getHeight(),
					0, 0, srcBands);
		}
