...
public class BMPImageWriter extends ImageWriter implements BMPConstants {
    ...
    public void write(IIOMetadata streamMetadata, IIOImage image, ImageWriteParam param) throws IOException {
        ...
        if (writeRaster) {
            ...
            Rectangle rect = new Rectangle(input.getMinX(), input.getMinY(), input.getWidth(), input
                    .getHeight());
            if (sourceRegion == null)
                sourceRegion = rect;
            else
        ...
        if (!canEncodeImage(compressionType, colorModel, sampleModel)) {
            throw new IOException("Image can not be encoded with compression type " + BMPCompressionTypes
        ...
        if (isPalette == true) {
            ...
            if (compressionType == BI_BITFIELDS) {
                ...
                for (int i = 0; i < 3; i++) {
                    int mask = (a[i] & 0xFF) + ((r[i] & 0xFF) * 0x100) + ((g[i] & 0xFF) * 0x10000) + ((b[i]
                            & 0xFF) * 0x1000000);
        ...
        for (int i = 0; i < h; i++) {
            ...
            Rectangle srcRect = new Rectangle(minX * scaleX + xOffset, row * scaleY + yOffset, (w - 1)
                    * scaleX + 1, 1);
            if (!writeRaster)
    ...
    private void writePixels(int l, int scanlineBytes, int bitsPerPixel, int pixels[], int padding,
            int numBands, IndexColorModel icm) throws IOException {
        int pixel = 0;
        int k = 0;
        switch (bitsPerPixel) {
            ...
            case 1:
                ...
                for (int j = 0; j < scanlineBytes / 8; j++) {
                    bpixels[k++] = (byte) ((pixels[l++] << 7) | (pixels[l++] << 6) | (pixels[l++] << 5)
                            | (pixels[l++] << 4) | (pixels[l++] << 3) | (pixels[l++] << 2)
                            | (pixels[l++] << 1) | pixels[l++]);
                }
            ...
            case 16:
                ...
                for (int j = 0, m = 0; j < scanlineBytes; m++) {
                    ...
                    if (compressionType == BI_RGB) {
                         ...
                         */
                        spixels[m] = (short) (((0x1f & pixels[j]) << 10) | ((0x1f & pixels[j + 1]) << 5)
                                | ((0x1f & pixels[j + 2])));
                        j += 3;
                    } else {
                        for (int i = 0; i < numBands; i++, j++) {
                            spixels[m] |= (((pixels[j]) << bitPos[i]) & bitMasks[i]);
                        }
                    }
                }
            ...
            case 32:
                ...
                if (numBands == 3) {
                    ...
                    for (int j = 0, m = 0; j < scanlineBytes; m++) {
                        ipixels[m] = 0;
                        if (compressionType == BI_RGB) {
                            ipixels[m] = ((0xff & pixels[j + 2]) << 16) | ((0xff & pixels[j + 1]) << 8)
                                    | ((0xff & pixels[j]));
                            j += 3;
                        } else {
                            for (int i = 0; i < numBands; i++, j++) {
                                ipixels[m] |= (((pixels[j]) << bitPos[i]) & bitMasks[i]);
                            }
                        }
                    }
                } else {
                    ...
                    for (int j = 0; j < scanlineBytes; j++) {
                        if (icm != null) {
                            ...
                            ipixels[j] = pixels[j] << 16 | pixels[j] << 8 | pixels[j];
                        }
                    }
                }
    ...
    private void writeEmbedded(IIOImage image, ImageWriteParam bmpParam) throws IOException {
        ...
        if (writer != null) {
            ...
            param.setSourceSubsampling(bmpParam.getSourceXSubsampling(), bmpParam.getSourceYSubsampling(),
                    bmpParam.getSubsamplingXOffset(), bmpParam.getSubsamplingYOffset());
    ...
    protected boolean canEncodeImage(int compression, ImageTypeSpecifier imgType) {
        ...
        if (bpp == 16) {
            ...
            return (((compressionType == BI_RGB) && canUseRGB) || ((compressionType == BI_BITFIELDS)
                    && canUseBITFIELDS));
        }
        return true;
    }
