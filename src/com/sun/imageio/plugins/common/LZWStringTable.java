...
/**
 * General purpose LZW String Table. Extracted from GIFEncoder by Adam Doppelt
 * Comments added by Robin Luiten <code>expandCode</code> added by Robin Luiten
 * The strLen table to give quick access to the lenght of an expanded code for
 * use by the <code>expandCode</code> method added by Robin.
 **/
public class LZWStringTable {
	...
	/*
	 * each entry corresponds to a code and contains the length of data that the
	 * code expands to when decoded.
	 */
	int[] strLen;
	...
	/*
	 * @param codesize the size of code to be preallocated for the string store.
	 */
	...
	/*
	 * If expanded data doesn't fit into array only what will fit is written to
	 * buf and the return value indicates how much of the expanded code has been
	 * written to the buf. The next call to expandCode() should be with the same
	 ...
	 * of the code being expanded. This negative value also indicates the buffer
	 * is full.
	 */
	public int expandCode(byte[] buf, int offset, short code, int skipHead) {
		...
		int skipTail = codeLen - expandLen; // only > 0 if codeLen > bufSpace
											// [left overs]

		int idx = offset + expandLen; // initialise to exclusive end address of
										// buffer area
