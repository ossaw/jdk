 ...
 * as defined by RFC 1738 and its related RFCs. It is any string that conforms
 * to the syntax described therein, and may not always have corresponding
 * support in the java.net.URL class or Web browsers.
 * <p>
 * NamingManager is safe for concurrent access by multiple threads.
 * <p>
 * Except as otherwise noted, a <tt>Name</tt> or environment parameter passed to
 * any method is owned by the caller. The implementation will not modify the
...
public class NamingManager {
    ...
    /*
     * Disallow anyone from creating one of these. Made package private so that
    ...
    /**
     * The ObjectFactoryBuilder determines the policy used when trying to load
     * object factories. See getObjectInstance() and class ObjectFactory for a
     * description of the default policy. setObjectFactoryBuilder() overrides
     * this default policy by installing an ObjectFactoryBuilder. Subsequent
     * object factories will be loaded and created using the installed builder.
     * <p>
     * The builder can only be installed if the executing thread is allowed (by
     * the security manager's checkSetFactory() method) to do so. Once
     * installed, the builder cannot be replaced.
     * <p>
     * 
     * @param builder
     ...
     * using the reference's factory class name and factory codebase to load in
     * the factory's class.
     * 
     * @param ref
     ...
     * <tt>Context.OBJECT_FACTORIES</tt> property of the environment or of the
     * provider resource file associated with <tt>nameCtx</tt>.
     *
    ...
    private static Object createObjectFromFactories(Object obj, Name name, Context nameCtx,
        ...
        FactoryEnumeration factories = ResourceManager.getFactories(Context.OBJECT_FACTORIES, environment,
                nameCtx);
    ...
    /**
     * Creates an instance of an object for the specified object and
     * environment.
     * <p>
     * If an object factory builder has been installed, it is used to create a
     * factory for creating the object. Otherwise, the following rules are used
     * to create the object:
     * <ol>
     * <li>If <code>refInfo</code> is a <code>Reference</code> or
     * <code>Referenceable</code> containing a factory class name, use the named
     * factory to create the object. Return <code>refInfo</code> if the factory
     * cannot be created. Under JDK 1.1, if the factory class must be loaded
     * from a location specified in the reference, a <tt>SecurityManager</tt>
     * must have been installed or the factory creation will fail. If an
     * exception is encountered while creating the factory, it is passed up to
     * the caller.
     * <li>If <tt>refInfo</tt> is a <tt>Reference</tt> or <tt>Referenceable</tt>
     * with no factory class name, and the address or addresses are
     * <tt>StringRefAddr</tt>s with address type "URL", try the URL context
     * factory corresponding to each URL's scheme id to create the object (see
     * <tt>getURLContext()</tt>). If that fails, continue to the next step.
     * <li>Use the object factories specified in the
     * <tt>Context.OBJECT_FACTORIES</tt> property of the environment, and of the
     * provider resource file associated with <tt>nameCtx</tt>, in that order.
     * The value of this property is a colon-separated list of factory class
     * names that are tried in order, and the first one that succeeds in
     * creating an object is the one used. If none of the factories can be
     * loaded, return <code>refInfo</code>. If an exception is encountered while
     * creating the object, the exception is passed up to the caller.
     * </ol>
     * <p>
     * Service providers that implement the <tt>DirContext</tt> interface should
     * use <tt>DirectoryManager.getObjectInstance()</tt>, not this method.
     ...
     * interface) must be public and must have a public constructor that accepts
     * no arguments.
     * <p>
     * The <code>name</code> and <code>nameCtx</code> parameters may optionally
     * be used to specify the name of the object being created.
     ...
     * factory or to the object implementation. If there are several possible
     * contexts from which the object could be named -- as will often be the
     * case -- it is up to the caller to select one. A good rule of thumb is to
     * select the "deepest" context available. If <code>nameCtx</code> is null,
     * <code>name</code> is relative to the default initial context. If no name
    ...
    /*
     * Ref has no factory. For each address of type "URL", try its URL context
     * factory. Returns null if unsuccessful in creating and invoking a factory.
     */
    static Object processURLAddrs(Reference ref, Name name, Context nameCtx, Hashtable<?, ?> environment)
            throws NamingException {
    ...
    private static Object processURL(Object refInfo, Name name, Context nameCtx, Hashtable<?, ?> environment)
            throws NamingException {
        Object answer;
     ...
     * <p>
     * The resulting context is for resolving URLs of the scheme
     * <code>scheme</code>. The resulting context is not tied to a specific URL.
     * It is able to handle arbitrary URLs with the specified scheme.
     * <p>
     * The class name of the factory that creates the resulting context has the
     * naming convention <i>scheme-id</i>URLContextFactory (e.g.
     * "ftpURLContextFactory" for the "ftp" scheme-id), in the package specified
     * as follows. The <tt>Context.URL_PKG_PREFIXES</tt> environment property
     * (which may contain values taken from applet parameters, system
     * properties, or application resource files) contains a colon-separated
     * list of package prefixes. Each package prefix in the property is tried in
     * the order specified to load the factory class. The default package prefix
     * is "com.sun.jndi.url" (if none of the specified packages work, this
     * default is tried). The complete package name is constructed using the
     * package prefix, concatenated with the scheme id.
     * <p>
     * For example, if the scheme id is "ldap", and the
     * <tt>Context.URL_PKG_PREFIXES</tt> property contains
     * "com.widget:com.wiz.jndi", the naming manager would attempt to load the
     * following classes until one is successfully instantiated:
     * <ul>
     ...
     * <p>
     * If a factory is instantiated, it is invoked with the following parameters
     * to produce the resulting context.
     * <p>
     * <code>factory.getObjectInstance(null, environment);</code>
     * <p>
     * For example, invoking getObjectInstance() as shown above on a LDAP URL
     * context factory would return a context that can resolve LDAP urls (e.g.
     ...
     * <p>
     * If urlInfo is null, the result is a context for resolving URLs with the
     * scheme id 'scheme'. If urlInfo is a URL, the result is a context named by
     * the URL. Names passed to this context is assumed to be relative to this
     * context (i.e. not a URL). For example, if urlInfo is
     * "ldap://ldap.wiz.com/o=Wiz,c=us", the resulting context will be that
     * pointed to by "o=Wiz,c=us" on the server 'ldap.wiz.com'. Subsequent names
     * that can be passed to this context will be LDAP names relative to this
     * context (e.g. cn="Barbs Jensen"). If urlInfo is an array of URLs, the
     * URLs are assumed to be equivalent in terms of the context to which they
     * refer. The resulting context is like that of the single URL case. If
     * urlInfo is of any other type, that is handled by the context factory for
     * the URL scheme.
     * 
     * @param scheme
    ...
    private static Object getURLObject(String scheme, Object urlInfo, Name name, Context nameCtx,
        ...
        ObjectFactory factory = (ObjectFactory) ResourceManager.getFactory(Context.URL_PKG_PREFIXES,
                environment, nameCtx, "." + scheme + "." + scheme + "URLContextFactory", defaultPkgPrefix);
    ...
    /**
     * Use this method for accessing initctx_factory_builder while inside an
     ...
     * <p>
     * If an InitialContextFactoryBuilder has been installed, it is used to
     * create the factory for creating the initial context. Otherwise, the class
     * specified in the <tt>Context.INITIAL_CONTEXT_FACTORY</tt> environment
     * property is used. Note that an initial context factory (an object that
     * implements the InitialContextFactory interface) must be public and must
     * have a public constructor that accepts no arguments.
     *
     * @param env
     *        The possibly null environment properties used when creating
     *        the context.
     ...
     * <p>
     * The builder can only be installed if the executing thread is allowed by
     * the security manager to do so. Once installed, the builder cannot be
     * replaced.
     * 
     * @param builder
     ...
     * @see java.lang.SecurityManager#checkSetFactory
     */
    public static synchronized void setInitialContextFactoryBuilder(InitialContextFactoryBuilder builder)
            throws NamingException {
     ...
     * 
     * @return true if an initial context factory builder has been set; false
     *         otherwise.
     ...
     * <tt>getContinuationContext()</tt> stores the value of its
     * <tt>CannotProceedException</tt> parameter. This property is inherited by
     * the continuation context, and may be used by that context's service
     * provider to inspect the fields of the exception.
     * <p>
     ...
     * <p>
     * In performing an operation on a name that spans multiple namespaces, a
     * context from one naming system may need to pass the operation on to the
     * next naming system. The context implementation does this by first
     * constructing a <code>CannotProceedException</code> containing information
     * pinpointing how far it has proceeded. It then obtains a continuation
     * context from JNDI by calling <code>getContinuationContext</code>. The
     * context implementation should then resume the context operation by
     * invoking the same operation on the continuation context, using the
     * remainder of the name that has not yet been resolved.
     * <p>
     * Before making use of the <tt>cpe</tt> parameter, this method updates the
     * environment associated with that object by setting the value of the
     * property <a href="#CPE"><tt>CPE</tt></a> to <tt>cpe</tt>. This property
     * will be inherited by the continuation context, and may be used by that
     * context's service provider to inspect the fields of this exception.
     *
     * @param cpe
     ...
     * <p>
     * Service providers that implement the <tt>DirContext</tt> interface should
     * use <tt>DirectoryManager.getStateToBind()</tt>, not this method. Service
     * providers that implement only the <tt>Context</tt> interface should use
     * this method.
     * <p>
     * This method uses the specified state factories in the
     * <tt>Context.STATE_FACTORIES</tt> property from the environment
     * properties, and from the provider resource file associated with
     * <tt>nameCtx</tt>, in that order. The value of this property is a
     * colon-separated list of factory class names that are tried in order, and
     * the first one that succeeds in returning the object's state is the one
     * used. If no object's state can be retrieved in this way, return the
     * object itself. If an exception is encountered while retrieving the state,
     ...
     * <p>
     * The <code>name</code> and <code>nameCtx</code> parameters may optionally
     * be used to specify the name of the object being created. See the
     * description of "Name and Context Parameters" in
     * {@link ObjectFactory#getObjectInstance ObjectFactory.getObjectInstance()}
     * for details.
     * <p>
     * This method may return a <tt>Referenceable</tt> object. The service
     * provider obtaining this object may choose to store it directly, or to
     * extract its reference (using <tt>Referenceable.getReference()</tt>) and
     * store that instead.
     *
     * @param obj
    ...
    public static Object getStateToBind(Object obj, Name name, Context nameCtx, Hashtable<?, ?> environment)
        ...
        FactoryEnumeration factories = ResourceManager.getFactories(Context.STATE_FACTORIES, environment,
                nameCtx);
