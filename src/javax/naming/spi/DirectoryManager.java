 ...
 * implementations.
 * <p>
 * This class is an extension of <tt>NamingManager</tt>. It contains methods for
 * use by service providers for accessing object factories and state factories,
 * and for getting continuation contexts for supporting federation.
 * <p>
 * <tt>DirectoryManager</tt> is safe for concurrent access by multiple threads.
 * <p>
 * Except as otherwise noted, a <tt>Name</tt>, <tt>Attributes</tt>, or
 * environment parameter passed to any method is owned by the caller. The
 * implementation will not modify the object or keep a reference to it, although
 * it may keep a reference to a clone or copy.
 *
 * @author Rosanna Lee
...
public class DirectoryManager extends NamingManager {
     ...
     * Operates just like <tt>NamingManager.getContinuationContext()</tt>, only
     * the continuation context returned is a <tt>DirContext</tt>.
     *
     * @param cpe
    ...
    /**
     * Creates an instance of an object for the specified object, attributes,
     * and environment.
     * <p>
     * This method is the same as <tt>NamingManager.getObjectInstance</tt>
     * except for the following differences:
     * <ul>
     * <li>It accepts an <tt>Attributes</tt> parameter that contains attributes
     ...
     * <li>The object factories tried must implement either
     * <tt>ObjectFactory</tt> or <tt>DirObjectFactory</tt>. If it implements
     * <tt>DirObjectFactory</tt>, <tt>DirObjectFactory.getObjectInstance()</tt>
     * is used, otherwise, <tt>ObjectFactory.getObjectInstance()</tt> is used.
     * </ul>
     * Service providers that implement the <tt>DirContext</tt> interface should
     * use this method, not <tt>NamingManager.getObjectInstance()</tt>.
     * <p>
     *
     * @param refInfo
    ...
    public static Object getObjectInstance(Object refInfo, Name name, Context nameCtx,
        ...
        if (builder != null) {
            ...
            if (factory instanceof DirObjectFactory) {
                return ((DirObjectFactory) factory).getObjectInstance(refInfo, name, nameCtx, environment,
                        attrs);
            } else {
        ...
        if (ref != null) {
            ...
            if (f != null) {
                ...
                if (factory instanceof DirObjectFactory) {
                    return ((DirObjectFactory) factory).getObjectInstance(ref, name, nameCtx, environment,
                            attrs);
    ...
    private static Object createObjectFromFactories(Object obj, Name name, Context nameCtx,
        ...
        while (answer == null && factories.hasMore()) {
            ...
            if (factory instanceof DirObjectFactory) {
                answer = ((DirObjectFactory) factory).getObjectInstance(obj, name, nameCtx, environment,
                        attrs);
            } else {
     ...
     * <p>
     * This method is like <tt>NamingManager.getStateToBind</tt> except for the
     ...
     * containing the object to be bound, and the attributes to accompany the
     * binding. Either the object or the attributes may be null.
     * <li>The state factories tried must each implement either
     * <tt>StateFactory</tt> or <tt>DirStateFactory</tt>. If it implements
     * <tt>DirStateFactory</tt>, then <tt>DirStateFactory.getStateToBind()</tt>
     * is called; otherwise, <tt>StateFactory.getStateToBind()</tt> is called.
     * </ul>
     * Service providers that implement the <tt>DirContext</tt> interface should
     * use this method, not <tt>NamingManager.getStateToBind()</tt>.
     * <p>
     * See NamingManager.getStateToBind() for a description of how the list of
     * state factories to be tried is determined.
     * <p>
     * The object returned by this method is owned by the caller. The
