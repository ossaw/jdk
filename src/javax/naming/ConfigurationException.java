 ...
 * required to access the provider or service is malformed or missing. For
 * example, a request to use SSL as the security protocol when the service
 * provider software was not configured with the SSL component would cause such
 * an exception. Another example is if the provider requires that a URL be
 * specified as one of the environment properties but the client failed to
 * provide it.
 * <p>
