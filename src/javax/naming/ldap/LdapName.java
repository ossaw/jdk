 ...
 * <a href="http://www.ietf.org/rfc/rfc2253.txt">RFC 2253</a>. A distinguished
 * name, or DN, is composed of an ordered list of components called
 * <em>relative distinguished name</em>s, or RDNs. Details of a DN's syntax are
 * described in RFC 2253.
 * <p>
 * This class resolves a few ambiguities found in RFC 2253 as follows:
 * <ul>
 * <li>RFC 2253 leaves the term "whitespace" undefined. The ASCII space
 * character 0x20 (" ") is used in its place.
 * <li>Whitespace is allowed on either side of ',', ';', '=', and '+'. Such
 * whitespace is accepted but not generated by this code, and is ignored when
 * comparing names.
 * <li>AttributeValue strings containing '=' or non-leading '#' characters
 * (unescaped) are accepted.
 * </ul>
 * <p>
 * String names passed to <code>LdapName</code> or returned by it use the full
 * Unicode character set. They may also contain characters encoded into UTF-8
 * with each octet represented by a three-character substring such as "\\B4".
 * They may not, however, contain characters encoded into UTF-8 with each octet
 * represented by a single character in the string: the meaning would be
 * ambiguous.
 * <p>
 * <code>LdapName</code> will properly parse all valid names, but does not
 * attempt to detect all possible violations when parsing invalid names. It is
 * "generous" in accepting invalid names. The "validity" of a name is determined
 * ultimately when it is supplied to an LDAP server, which may accept or reject
 * the name based on factors such as its schema information and interoperability
 * considerations.
 * <p>
 * When names are tested for equality, attribute types, both binary and string
 * values, are case-insensitive. String values with different but equivalent
 * usage of quoting, escaping, or UTF8-hex-encoding are considered equal. The
 * order of components in multi-valued RDNs (such as "ou=Sales+cn=Bob") is not
 * significant.
 * <p>
 * The components of a LDAP name, that is, RDNs, are numbered. The indexes of a
 * LDAP name with n RDNs range from 0 to n-1. This range may be written as
 * [0,n). The right most RDN is at index 0, and the left most RDN is at index
 * n-1. For example, the distinguished name:
 * "CN=Steve Kille, O=Isode Limited, C=GB" is numbered in the following sequence
 * ranging from 0 to 2: {C=GB, O=Isode Limited, CN=Steve Kille}. An empty LDAP
 * name is represented by an empty RDN list.
 * <p>
 * Concurrent multithreaded read-only access of an instance of <tt>LdapName</tt>
...
public class LdapName implements Name {
		...
		for (int i = 0; i < rdns.size(); i++) {
			...
			if (!(obj instanceof Rdn)) {
				throw new IllegalArgumentException(
						"Entry:" + obj + "  not a valid type;list entries must be of type Rdn");
			}
	...
	/*
	 * Constructs an LDAP name given its parsed components (the elements of
	 * "rdns" in the range [beg,end)) and, optionally (if "name" is not null),
	 * the unparsed DN.
	 *
	 */
	...
	/**
	 * Determines whether this LDAP name is empty. An empty name is one with
	 * zero components.
	 * 
	 ...
	 * effect of updates to this name on this enumeration is undefined. If the
	 * name has zero components, an empty (non-null) enumeration is returned.
	 * The order of the components returned by the enumeration is same as the
	 * order in which the components are numbered as described in the class
	 * description.
	 *
	 * @return A non-null enumeration of the components of this LDAP name. Each
	 *         element of the enumeration is of class String.
	 */
	 ...
	 * 
	 * @param posn
	 *            The 0-based index of the RDN to retrieve. Must be in the range
	 *            [0,size()).
	 ...
	 * this LDAP name. Subsequent changes to this name will not affect the name
	 ...
	 * this LDAP name. Subsequent changes to this name do not affect the name
	 * that is returned and vice versa.
	 *
	 * @param posn
	 ...
	 * prefix. A name <tt>n</tt> is a prefix if it is equal to
	 * <tt>getPrefix(n.size())</tt>--in other words this LDAP name starts with
	 * 'n'. If n is null or not a RFC2253 formatted name as described in the
	...
	/**
	 * Determines whether the specified RDN sequence forms a prefix of this LDAP
	 * name. Returns true if this LdapName is at least as long as rdns, and for
	 * every position p in the range [0, rdns.size()) the component getRdn(p)
	 ...
	 * <tt>getSuffix(size()-n.size())</tt>--in other words this LDAP name ends
	 * with 'n'. If n is null or not a RFC2253 formatted name as described in
	 ...
	 * name. Returns true if this LdapName is at least as long as rdns, and for
	 * every position p in the range [size() - rdns.size(), size()) the
	 * component getRdn(p) matches rdns.get(p). Returns false otherwise. If rdns
	...
	/*
	 * Helper method for startsWith() and endsWith(). Returns true if components
	 * [beg,end) match the components of "n". If "n" is not an LdapName, each of
	 * its components is parsed as the string form of an RDN. The following must
	 * hold: end - beg == n.size().
	 */
	 ...
	 *
	 * @throws InvalidNameException
	 *             if <tt>suffix</tt> is not a valid LDAP name, or if the
	 *             addition of the components would violate the syntax rules of
	 *             this LDAP name.
	 */
	 ...
	 * within this name. Components of this LDAP name at or after the index (if
	 * any) of the first new component are shifted up (away from index 0) to
	...
	/**
	 * Adds the RDNs of a name -- in order -- at a specified position within
	 * this name. RDNs of this LDAP name at or after the index (if any) of the
	 * first new RDN are shifted up (away from index 0) to accommodate the new
	 * RDNs.
	 *
	 * @param suffixRdns
	 *            The non-null suffix <tt>Rdn</tt>s to add.
	 * @param posn
	 *            The index at which to add the suffix RDNs. Must be in the
	 *            range [0,size()].
	 *
	 * @return The updated name (not a new instance).
	 * @throws IndexOutOfBoundsException
	...
	/**
	 * Adds a single RDN at a specified position within this LDAP name. RDNs of
	 * this LDAP name at or after the index (if any) of the new RDN are shifted
	...
	/**
	 * Removes a component from this LDAP name. The component of this name at
	 * the specified position is removed. Components with indexes greater than
	...
	/**
	 * Retrieves the list of relative distinguished names. The contents of the
	 * list are unmodifiable. The indexing of RDNs in the returned list follows
	 * the numbering of RDNs as described in the class description. If the name
	 * has zero components, an empty list is returned.
	 *
	 * @return The name as a list of RDNs which are instances of the class
	 *         {@link Rdn Rdn}.
	 */
	 ...
	 * in the class description. If the name has zero components an empty string
	 * is returned.
	 *
	...
	/**
	 * Determines whether two LDAP names are equal. If obj is null or not an
	 * LDAP name, false is returned.
	 * <p>
	 * Two LDAP names are equal if each RDN in one is equal to the corresponding
	 * RDN in the other. This implies both have the same number of RDNs, and
	 * each RDN's equals() test against the corresponding RDN in the other name
	 * returns true. See {@link Rdn#equals(Object obj)} for a definition of RDN
	 * equality.
	 *
	 * @param obj
	...
	/**
	 * Compares this LdapName with the specified Object for order. Returns a
	 ...
	 * thrown.
	 * <p>
	 * Ordering of LDAP names follows the lexicographical rules for string
	 * comparison, with the extension that this applies to all the RDNs in the
	 * LDAP name. All the RDNs are lined up in their specified order and
	 * compared lexicographically. See {@link Rdn#compareTo(Object obj)
	 * Rdn.compareTo(Object obj)} for RDN comparison rules.
	 * <p>
	 * If this LDAP name is lexicographically lesser than obj, a negative number
	 * is returned. If this LDAP name is lexicographically greater than obj, a
	...
	/**
	 * Computes the hash code of this LDAP name. The hash code is the sum of the
	 * hash codes of individual RDNs of this name.
	 *
	...
	/**
	 * Serializes only the unparsed DN, for compactness and to avoid any
