 ...
 * application will be using methods specific to the class that implements this
 * interface. Such a class should have decoded the BER buffer in the response
 * and should provide methods that allow the user to access that data in the
 * response in a type-safe and friendly manner.
 * <p>
 * For example, suppose the LDAP server supported a 'get time' extended
 * operation. It would supply GetTimeRequest and GetTimeResponse classes. The
 * GetTimeResponse class might look like: <blockquote>
 * 
 * <pre>
