 ...
 * <a href="http://www.ietf.org/rfc/rfc2253.txt">RFC 2253</a>. An example of an
 ...
 * description for {@link javax.naming.ldap.LdapName <tt>LdapName</tt>}.
 * <p>
 * The Rdn class represents an RDN as attribute type/value mappings, which can
 * be viewed using {@link javax.naming.directory.Attributes Attributes}. In
 * addition, it contains convenience methods that allow easy retrieval of type
 * and value when the Rdn consist of a single type/value pair, which is how it
 * appears in a typical usage. It also contains helper methods that allow
 * escaping of the unformatted attribute value and unescaping of the value
 * formatted according to the escaping syntax defined in RFC2253. For methods
 * that take or return attribute value as an Object, the value is either a
 * String (in unescaped form) or a byte array.
 * <p>
 * <code>Rdn</code> will properly parse all valid RDNs, but does not attempt to
 * detect all possible violations when parsing invalid RDNs. It is "generous" in
 * accepting invalid RDNs. The "validity" of a name is determined ultimately
 * when it is supplied to an LDAP server, which may accept or reject the name
 * based on factors such as its schema information and interoperability
 * considerations.
 * <p>
 ...
 * {@link #unescapeValue(String) <tt>unescapeValue()</tt>} method can be used to
 * unescape the escaped comma resulting in the original value
 * <tt>"Juicy, Fruit"</tt>. The {@link #escapeValue(Object)
 * <tt>escapeValue()</tt>} method adds the escape back preceding the comma.
 * <p>
 * This class can be instantiated by a string representation of the RDN defined
 * in RFC 2253 as shown in the following code example:
 * 
 * <pre>
 * Rdn rdn = new Rdn("cn=Juicy\\, Fruit");
 * System.out.println(rdn.toString());
 ...
 * <p>
 * Concurrent multithreaded read-only access of an instance of <tt>Rdn</tt> need
 * not be synchronized.
 * <p>
...
public class Rdn implements Serializable, Comparable<Object> {
     ...
     * <a href="http://www.ietf.org/rfc/rfc2253.txt">RFC 2253</a> formatted RDN
     ...
     * formatted according to the rules defined in
    ...
    /**
     * Constructs an Rdn from the given <tt>rdn</tt>. The contents of the
     * <tt>rdn</tt> are simply copied into the newly created Rdn.
     * 
     * @param rdn
    ...
    /**
     * Constructs an Rdn from the given attribute type and value. The string
     * attribute values are not interpreted as
     * <a href="http://www.ietf.org/rfc/rfc2253.txt">RFC 2253</a> formatted RDN
     * strings. That is, the values are used literally (not parsed) and assumed
     * to be unescaped.
     *
    ...
    private boolean isEmptyValue(Object val) {
        return ((val instanceof String) && val.equals("")) || ((val instanceof byte[])
                && (((byte[]) val).length == 0));
    }
    ...
    /*
     * Adds the given attribute type and value to this Rdn. The string attribute
     * values are not interpreted as <a
    ...
    /**
     * Retrieves one of this Rdn's value. This is a convenience method for
     * obtaining the value, when the RDN contains a single type and value
     * mapping, which is the common RDN usage.
     * <p>
     * For a multi-valued RDN, this method returns value corresponding to the
     * type returned by {@link #getType() getType()} method.
     *
     ...
     * obtaining the type, when the RDN contains a single type and value
     * mapping, which is the common RDN usage.
     * <p>
     * For a multi-valued RDN, the type/value pairs have no specific order
     * defined on them. In that case, this method returns type of one of the
     * type/value pairs. The {@link #getValue() getValue()} method returns the
     ...
     * integer, zero, or a positive integer as this Rdn is less than, equal to,
     * or greater than the given Object.
     * <p>
     * If obj is null or not an instance of Rdn, ClassCastException is thrown.
     * <p>
     * The attribute type and value pairs of the RDNs are lined up against each
     * other and compared lexicographically. The order of components in
     * multi-valued Rdns (such as "ou=Sales+cn=Bob") is not significant.
     *
     * @param obj
     ...
     * escaping, or UTF8-hex-encoding are considered equal. The case of the
     * values is ignored during the comparison.
     * </ul>
     * <p>
    ...
    /**
     * Retrieves the {@link javax.naming.directory.Attributes Attributes} view
     * of the type/value mappings contained in this Rdn.
     *
     * @return The non-null attributes containing the type/value mappings of
     *         this Rdn.
     */
    ...
    private static class RdnEntry implements Comparable<RdnEntry> {
        ...
        public boolean equals(Object obj) {
            ...
            return (type.equalsIgnoreCase(that.type)) && (getValueComparable().equals(that
                    .getValueComparable()));
        }
    ...
    /**
     * Given the value of an attribute, returns a string escaped according to
     * the rules specified in <a href="http://www.ietf.org/rfc/rfc2253.txt">RFC
     * 2253</a>.
     * <p>
     * For example, if the val is "Sue, Grabbit and Runn", the escaped value
     * returned by this method is "Sue\, Grabbit and Runn".
     * <p>
     * A string value is represented as a String and binary value as a byte
     * array.
     *
     * @param val
    ...
    /*
     * Given the value of a string-valued attribute, returns a string suitable
     * for inclusion in a DN. This is accomplished by using backslash (\) to
     * escape the following characters: leading and trailing whitespace , = + <
     * > # ; " \
     */
    ...
    /*
     * Given the value of a binary attribute, returns a string suitable for
     * inclusion in a DN (such as "#CEB1DF80"). TBD: This method should actually
     * generate the ber encoding of the binary value
     */
     ...
     * in <a href="http://www.ietf.org/rfc/rfc2253.txt">RFC 2253</a>, returns
     * the unformated value. Escapes and quotes are stripped away, and
     * hex-encoded UTF-8 is converted to equivalent UTF-16 characters. Returns a
     * string value as a String, and a binary value as a byte array.
     * <p>
     * Legal and illegal values are defined in RFC 2253. This method is generous
     * in accepting the values and does not catch all illegal values. Therefore,
     * passing in an illegal value might not necessarily trigger an
            ...
            if ((chars[i] == '\\') && (i + 1 < end)) {
                if (!Character.isLetterOrDigit(chars[i + 1])) {
                    ...
                    if (utf8.length > 0) {
                        ...
                        throw new IllegalArgumentException("Not a valid attribute string value:" + val
     ...
     * representing bytes encoded as hex-pairs (such as "CEB1DF80"), returns a
    ...
    /**
     * Serializes only the unparsed RDN, for compactness and to avoid any
