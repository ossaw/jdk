 ...
 * server in batches of a specified size. The requestor controls the rate at
 * which batches are returned by the rate at which it invokes search operations.
 * <p>
...
final public class PagedResultsControl extends BasicControl {
	...
	/**
	 * The paged-results control's assigned object identifier is
	...
	/**
	 * Constructs a control to set the number of entries to be returned per page
	 * of results.
	 *
	 * @param pageSize
	...
	/**
	 * Constructs a control to set the number of entries to be returned per page
	 * of results. The cookie is provided by the server and may be obtained from
	 * the paged-results response control.
	 * <p>
	 * A sequence of paged-results can be abandoned by setting the pageSize to
