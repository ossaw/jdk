 ...
 * </pre>
 * 
 * It comprises an object identifier string and an optional ASN.1 BER encoded
 * value.
 * <p>
 * The methods in this class are used by the service provider to construct the
 * bits to send to the LDAP server. Applications typically only deal with the
 * classes that implement this interface, supplying them with any information
 * required for a particular extended operation request. It would then pass such
 * a class as an argument to the <tt>LdapContext.extendedOperation()</tt> method
 * for performing the LDAPv3 extended operation.
 * <p>
 * For example, suppose the LDAP server supported a 'get time' extended
 * operation. It would supply GetTimeRequest and GetTimeResponse classes:
 * <blockquote>
 * 
 * <pre>
...
public interface ExtendedRequest extends java.io.Serializable {
	 ...
	 *
	 * The result is the raw BER bytes including the tag and length of the
	 * request value. It does not include the request OID. This method is called
	 * by the service provider to get the bits to put into the extended
	 * operation to be sent to the LDAP server.
	 *
	 ...
	 * <p>
	 * After the service provider has sent the extended operation request to the
	 * LDAP server, it will receive a response from the server. If the operation
	 * failed, the provider will throw a NamingException. If the operation
	 * succeeded, the provider will invoke this method using the data that it
	 * got back in the response. It is the job of this method to return a class
	 * that implements the ExtendedResponse interface that is appropriate for
	 * the extended operation request.
	 * <p>
	 * For example, a Start TLS extended request class would need to know how to
	 * process a Start TLS extended response. It does this by creating a class
