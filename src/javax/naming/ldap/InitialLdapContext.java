 ...
 * "bind" operation performed by the context or contexts derived from the
 * context. These are called <em>connection request controls</em>. Use
 * <tt>getConnectControls()</tt> to get a context's connection request controls.
 * <p>
 * The request controls supplied to the initial context constructor are
 * <em>not</em> used as the context request controls for subsequent context
 * operations such as searches and lookups. Context request controls are set and
 * updated by using <tt>setRequestControls()</tt>.
 * <p>
 * As shown, there can be two different sets of request controls associated with
 * a context: connection request controls and context request controls. This is
 * required for those applications needing to send critical controls that might
 * not be applicable to both the context operation and any implicit LDAP "bind"
 * operation. A typical user program would do the following: <blockquote>
 * 
 * <pre>
 ...
 * </pre>
 * 
 * </blockquote> It specifies first the critical controls for creating the
 * initial context (<tt>critConnCtls</tt>), and then sets the context's request
 * controls (<tt>critModCtls</tt>) for the context operation. If for some reason
 * <tt>lctx</tt> needs to reconnect to the server, it will use
 * <tt>critConnCtls</tt>. See the <tt>LdapContext</tt> interface for more
 * discussion about request controls.
 * <p>
 * Service provider implementors should read the "Service Provider" section in
 * the <tt>LdapContext</tt> class description for implementation details.
 *
 * @author Rosanna Lee
...
public class InitialLdapContext extends InitialDirContext implements LdapContext {
	 ...
	 * connection request controls. Equivalent to
