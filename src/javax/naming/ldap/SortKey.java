...
/**
 * A sort key and its associated sort parameters. This class implements a sort
 * key which is used by the LDAPv3 Control for server-side sorting of search
 * results as defined in <a href="http://www.ietf.org/rfc/rfc2891.txt">RFC
 * 2891</a>.
 *
 * @since 1.5
 * @see SortControl
...
public class SortKey {
	 ...
	 *            If true then entries are arranged in ascending order.
	 *            Otherwise there are arranged in descending order.
	 * @param matchingRuleID
	 *            The possibly null ID of the matching rule to use to order the
	 *            attribute values. If not specified then the ordering matching
	 ...
	 *
	 * @return The possibly null matching rule ID. If null then the ordering
	 *         matching rule defined for the sort key attribute is used.
	 */
