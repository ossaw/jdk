...
/**
 * This class implements the LDAPv3 Extended Request for StartTLS as defined in
 * <a href="http://www.ietf.org/rfc/rfc2830.txt">Lightweight Directory Access
 * Protocol (v3): Extension for Transport Layer Security</a>
 * The object identifier for StartTLS is 1.3.6.1.4.1.1466.20037 and no extended
 * request value is defined.
 * <p>
 * <tt>StartTlsRequest</tt>/<tt>StartTlsResponse</tt> are used to establish a
 * TLS connection over the existing LDAP connection associated with the JNDI
 * context on which <tt>extendedOperation()</tt> is invoked. Typically, a JNDI
 * program uses these classes as follows. <blockquote>
 * 
 * <pre>
...
public class StartTlsRequest implements ExtendedRequest {
    ...
    /**
     * The StartTLS extended request's assigned object identifier is
    ...
    /**
     * Retrieves the StartTLS request's ASN.1 BER encoded value. Since the
     * request has no defined value, null is always returned.
     *
    ...
    /**
     * Creates an extended response object that corresponds to the LDAP StartTLS
     * extended request.
     * <p>
     * The result must be a concrete subclass of StartTlsResponse and must have
     * a public zero-argument constructor.
     * <p>
     * This method locates the implementation class by locating configuration
     * files that have the name: <blockquote><tt>
     *     META-INF/services/javax.naming.ldap.StartTlsResponse
     * </tt></blockquote> The configuration files and their corresponding
     * implementation classes must be accessible to the calling thread's context
     * class loader.
     * <p>
     ...
     * <p>
     * This method will return an instance of the first implementation class
     * that it is able to load and instantiate successfully from the list of
     * class names collected from the configuration files. This method uses the
     * calling thread's context classloader to find the configuration files and
     * to load the implementation class.
     * <p>
     * If no class can be found in this way, this method will use an
     * implementation-specific way to locate an implementation. If none is
    ...
    public ExtendedResponse createExtendedResponse(String id, byte[] berValue, int offset, int length)
        ...
        if ((id != null) && (!id.equals(OID))) {
            throw new ConfigurationException("Start TLS received the following response instead of " + OID
                    + ": " + id);
        }
