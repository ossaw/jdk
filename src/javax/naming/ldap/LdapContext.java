 ...
 *
 * For applications that do not require such controls or extended operations,
 * the more generic <tt>javax.naming.directory.DirContext</tt> should be used
 * instead.
 *
 * <h3>Usage Details About Controls</h3>
 *
 * This interface provides support for LDAP v3 controls. At a high level, this
 * support allows a user program to set request controls for LDAP operations
 * that are executed in the course of the user program's invocation of
 * <tt>Context</tt>/<tt>DirContext</tt> methods, and read response controls
 * resulting from LDAP operations. At the implementation level, there are some
 * details that developers of both the user program and service providers need
 * to understand in order to correctly use request and response controls.
 *
 ...
 * performing the corresponding LDAP operations. For connection management, it
 * uses the <em>connection request controls</em>, while for the normal LDAP
 * operations, it uses the <em>context request controls</em>.
 * <p>
 * Unless explicitly qualified, the term "request controls" refers to context
 * request controls.
 *
 * <h4>Context Request Controls</h4> There are two ways in which a context
 * instance gets its request controls:
 * <ol>
 * <li><tt>ldapContext.newInstance(<strong>reqCtls</strong>)</tt>
 * <li><tt>ldapContext.setRequestControls(<strong>reqCtls</strong>)</tt>
 * </ol>
 * where <tt>ldapContext</tt> is an instance of <tt>LdapContext</tt>. Specifying
 * <tt>null</tt> or an empty array for <tt>reqCtls</tt> means no request
 * controls. <tt>newInstance()</tt> creates a new instance of a context using
 * <tt>reqCtls</tt>, while <tt>setRequestControls()</tt> updates an existing
 * context instance's request controls to <tt>reqCtls</tt>.
 * <p>
 * Unlike environment properties, request controls of a context instance
 * <em>are not inherited</em> by context instances that are derived from it.
 * Derived context instances have <tt>null</tt> as their context request
 * controls. You must set the request controls of a derived context instance
 * explicitly using <tt>setRequestControls()</tt>.
 * <p>
 * A context instance's request controls are retrieved using the method
 * <tt>getRequestControls()</tt>.
 *
 * <h4>Connection Request Controls</h4> There are three ways in which connection
 * request controls are set:
 * <ol>
 * <li><tt>
 ...
 * </ol>
 * where <tt>refException</tt> is an instance of <tt>LdapReferralException</tt>,
 * and <tt>ldapContext</tt> is an instance of <tt>LdapContext</tt>. Specifying
 * <tt>null</tt> or an empty array for <tt>connCtls</tt> means no connection
 * request controls.
 * <p>
 * Like environment properties, connection request controls of a context
 * <em>are inherited</em> by contexts that are derived from it. Typically, you
 * initialize the connection request controls using the
 * <tt>InitialLdapContext</tt> constructor or
 * <tt>LdapReferralContext.getReferralContext()</tt>. These connection request
 * controls are inherited by contexts that share the same connection--that is,
 * contexts derived from the initial or referral contexts.
 * <p>
 * Use <tt>reconnect()</tt> to change the connection request controls of a
 * context. Invoking <tt>ldapContext.reconnect()</tt> affects only the
 * connection used by <tt>ldapContext</tt> and any new contexts instances that
 * are derived form <tt>ldapContext</tt>. Contexts that previously shared the
 * connection with <tt>ldapContext</tt> remain unchanged. That is, a context's
 * connection request controls must be explicitly changed and is not affected by
 * changes to another context's connection request controls.
 * <p>
 * A context instance's connection request controls are retrieved using the
 * method <tt>getConnectControls()</tt>.
 *
 ...
 * request controls in the environment property
 * "java.naming.ldap.control.connect" and pass this environment property on to
 * context instances that it creates.
 *
 * <h3>Response Controls</h3>
 *
 * The method <tt>LdapContext.getResponseControls()</tt> is used to retrieve the
 * response controls generated by LDAP operations executed as the result of
 * invoking a <tt>Context</tt>/<tt>DirContext</tt> operation. The result is all
 * of the responses controls generated by the underlying LDAP operations,
 * including any implicit reconnection. To get only the reconnection response
 * controls, use <tt>reconnect()</tt> followed by <tt>getResponseControls()</tt>
 * .
 *
 * <h3>Parameters</h3>
 *
 * A <tt>Control[]</tt> array passed as a parameter to any method is owned by
 * the caller. The service provider will not modify the array or keep a
 * reference to it, although it may keep references to the individual
 * <tt>Control</tt> objects in the array. A <tt>Control[]</tt> array returned by
 * any method is immutable, and may not subsequently be modified by either the
 * caller or the service provider.
 *
 * @author Rosanna Lee
