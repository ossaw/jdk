...
/**
 * The implementation class for CompoundName and CompositeName. This class is
 * package private.
 *
 * @author Rosanna Lee
...
class NameImpl {
    ...
    private final boolean isMeta(String n, int i) {
        return (isA(n, i, syntaxEscape) || isA(n, i, syntaxBeginQuote1) || isA(n, i, syntaxBeginQuote2)
                || isSeparator(n, i));
    }
    ...
    private final int extractComp(String name, int i, int len, Vector<String> comps)
        ...
        while (i < len) {
            ...
            if (start && ((one = isA(name, i, syntaxBeginQuote1)) || isA(name, i, syntaxBeginQuote2))) {
                ...
                if (isMeta(name, i + syntaxEscape.length())) {
                    ...
                    throw (new InvalidNameException(name + ": unescaped " + syntaxEscape
                            + " at end of component"));
                }
            } else if (isA(name, i, syntaxTypevalSeparator) && ((one = isA(name, i + syntaxTypevalSeparator
                    .length(), syntaxBeginQuote1)) || isA(name, i + syntaxTypevalSeparator.length(),
                            syntaxBeginQuote2))) {
    ...
    /*
     * // Determines whether this component needs any escaping. private final
     * boolean escapingNeeded(String comp) { int len = comp.length(); for (int i
     * = 0; i < len; i++) { if (i == 0) { if (isA(comp, 0, syntaxBeginQuote1) ||
     * isA(comp, 0, syntaxBeginQuote2)) { return (true); } } if
     * (isSeparator(comp, i)) { return (true); } if (isA(comp, i, syntaxEscape))
     * { i += syntaxEscape.length(); if (i >= len || isMeta(comp, i)) { return
     * (true); } } } return (false); }
     */
    private final String stringifyComp(String comp) {
        ...
        // or quote them
        if (syntaxSeparator != null && comp.indexOf(syntaxSeparator) >= 0) {
        ...
        if (syntaxSeparator2 != null && comp.indexOf(syntaxSeparator2) >= 0) {
            if (syntaxBeginQuote1 != null) {
                if (beginQuote == null) {
                    beginQuote = syntaxBeginQuote1;
                    endQuote = syntaxEndQuote1;
    ...
    /**
     * Compares obj to this NameImpl to determine ordering. Takes into account
     * syntactic properties such as elimination of blanks, case-ignore, etc, if
     * relevant.
     * Note: using syntax of this NameImpl and ignoring that of comparison
     * target.
     */
