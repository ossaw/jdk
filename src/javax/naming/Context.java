 ...
 * bindings.
 *
 * <h1>Names</h1> Each name passed as an argument to a <tt>Context</tt> method
 * is relative to that context. The empty name is used to name the context
 * itself. A name parameter may never be null.
 * <p>
 ...
 * the method descriptions below, only one version is fully documented. The
 * second version instead has a link to the first: the same documentation
 * applies to both.
 * <p>
 ...
 * instances of <tt>CompositeName</tt> are treated as composite names, while
 * <tt>Name</tt> arguments that are not instances of <tt>CompositeName</tt> are
 * treated as compound names (which might be instances of <tt>CompoundName</tt>
 * or other implementations of compound names). This allows the results of
 * <tt>NameParser.parse()</tt> to be used as arguments to the <tt>Context</tt>
 * methods. Prior to JNDI 1.2, all name arguments were treated as composite
 * names.
 * <p>
 * Furthermore, for systems that support federation, all names returned in a
 * <tt>NamingEnumeration</tt> from <tt>list()</tt> and <tt>listBindings()</tt>
 * are composite names represented as strings. See <tt>CompositeName</tt> for
 * the string syntax of names.
 * <p>
 * For systems that do not support federation, the name arguments (in either
 * <tt>Name</tt> or <tt>String</tt> forms) and the names returned in
 ...
 *
 * <h1>Exceptions</h1> All the methods in this interface can throw a
 * <tt>NamingException</tt> or any of its subclasses. See
 * <tt>NamingException</tt> and their subclasses for details on each exception.
 *
 * <h1>Concurrent Access</h1> A Context instance is not guaranteed to be
 * synchronized against concurrent access by multiple threads. Threads that need
 * to access a single Context instance concurrently should synchronize amongst
 * themselves and provide the necessary locking. Multiple threads each
 * manipulating a different Context instance need not synchronize. Note that the
 * {@link #lookup(Name) <tt>lookup</tt>} method, when passed an empty name, will
 ...
 * <tt>NamingEnumeration</tt> is not considered to have completed while the
 * enumeration is still in use, or while any referrals generated by that
 * operation are still being followed.
 *
 *
 * <h1>Parameters</h1> A <tt>Name</tt> parameter passed to any method of the
 * <tt>Context</tt> interface or one of its subinterfaces will not be modified
 * by the service provider. The service provider may keep a reference to it for
 * the duration of the operation, including any enumeration of the method's
 * results and the processing of any referrals generated. The caller should not
 * modify the object during this time. A <tt>Name</tt> returned by any such
 * method is owned by the caller. The caller may subsequently modify it; the
 ...
 * properties that define the environment in which naming and directory services
 * are accessed. For example, a context might require specification of security
 * credentials in order to access the service. Another context might require
 * that server configuration information be supplied. These are referred to as
 * the <em>environment</em> of a context. The <tt>Context</tt> interface
 * provides methods for retrieving and updating this environment.
 * <p>
 * The environment is inherited from the parent context as context methods
 * proceed from one context to the next. Changes to the environment of one
 * context do not directly affect those of other contexts.
 * <p>
 * It is implementation-dependent when environment properties are used and/or
 * verified for validity. For example, some of the security-related properties
 * are used by service providers to "log in" to the directory. This login
 * process might occur at the time the context is created, or the first time a
 * method is invoked on the context. When, and whether this occurs at all, is
 * implementation-dependent. When environment properties are added or removed
 * from the context, verifying the validity of the changes is again
 ...
 * along with <em>resource files.</em> A JNDI resource file is a file in the
 * properties file format (see {@link java.util.Properties#load
 * <tt>java.util.Properties</tt>}), containing a list of key/value pairs. The
 * key is the name of the property (e.g. "java.naming.factory.object") and the
 * value is a string in the format defined for that property. Here is an example
 ...
 * freely available. Thus JNDI resource files should be considered to be
 * "world readable", and sensitive information such as clear-text passwords
 * should not be stored there.
 * <p>
 * There are two kinds of JNDI resource files: <em>provider</em> and
 * <em>application</em>.
 *
 * <h2>Provider Resource Files</h2>
 *
 * Each service provider has an optional resource that lists properties specific
 * to that provider. The name of this resource is: <blockquote> [<em>prefix</em>
 * /]<tt>jndiprovider.properties</tt> </blockquote> where <em>prefix</em> is the
 * package name of the provider's context implementation(s), with each period
 * (".") converted to a slash ("/").
 *
 * For example, suppose a service provider defines a context implementation with
 * class name <tt>com.sun.jndi.ldap.LdapCtx</tt>. The provider resource for this
 * provider is named <tt>com/sun/jndi/ldap/jndiprovider.properties</tt>. If the
 * class is not in a package, the resource's name is simply
 * <tt>jndiprovider.properties</tt>.
 *
 * <p>
 * <a name=LISTPROPS></a> Certain methods in the JNDI class library make use of
 * the standard JNDI properties that specify lists of JNDI factories:
 * <ul>
 ...
 * classes. JNDI locates (using {@link ClassLoader#getResources
 * <tt>ClassLoader.getResources()</tt>}) all <em>application resource files</em>
 * named <tt>jndi.properties</tt> in the classpath. In addition, if the file
 * <i>java.home</i><tt>/lib/jndi.properties</tt> exists and is readable, JNDI
 * treats it as an additional application resource file. (<i>java.home</i>
 * indicates the directory named by the <tt>java.home</tt> system property.) All
 * of the properties contained in these files are placed into the environment of
 * the initial context. This environment is then inherited by other contexts.
 *
 * <p>
 ...
 * "java.naming.factory.object" property is found in three
 * <tt>jndi.properties</tt> resource files, the list of object factories is a
 * concatenation of the property values from all three files. Using this scheme,
 * each deployable component is responsible for listing the factories that it
 * exports. JNDI automatically collects and uses all of these export lists when
 ...
 * the constructor, the system properties, the applet parameters, and the
 * application resource files. See <a href=InitialContext.html#ENVIRONMENT>
 * <tt>InitialContext</tt></a> for details. This initial environment is then
 * inherited by other context instances.
 *
 * <p>
 * When the JNDI class library needs to determine the value of a property, it
 * does so by merging the values from the following two sources, in order:
 * <ol>
 * <li>The environment of the context being operated on.
 * <li>The provider resource file (<tt>jndiprovider.properties</tt>) for the
 ...
 * properties that specify a list of JNDI factories (listed
 * <a href=#LISTPROPS>above</a>), the values are concatenated into a single
 * colon-separated list. For other properties, only the first value found is
 * used.
 *
 * <p>
 * When a service provider needs to determine the value of a property, it will
 * generally take that value directly from the environment. A service provider
 * may define provider-specific properties to be placed in its own provider
 * resource file. In that case it should merge values as described in the
 * previous paragraph.
 *
 * <p>
 * In this way, each service provider developer can specify a list of factories
 * to use with that service provider. These can be modified by the application
 * resources specified by the deployer of the application or applet, which in
 * turn can be modified by the user.
 *
 * @author Rosanna Lee
...
public interface Context {
    ...
    // public static final: JLS says recommended style is to omit these
    // modifiers
     ...
     * specified in any of these sources, <tt>NoInitialContextException</tt> is
     * thrown when an initial context is required to complete an operation.
     *
     * <p>
    ...
    /**
     * Constant that holds the name of the environment property for specifying
     * the list of object factories to use. The value of the property should be
     * a colon-separated list of the fully qualified class names of factory
     * classes that will create an object given information about the object.
     * This property may be specified in the environment, an applet parameter, a
     * system property, or one or more resource files.
     *
     * <p>
    ...
    /**
     * Constant that holds the name of the environment property for specifying
     * the list of state factories to use. The value of the property should be a
     * colon-separated list of the fully qualified class names of state factory
     * classes that will be used to get an object's state given the object
     * itself. This property may be specified in the environment, an applet
     ...
     * The prefix <tt>com.sun.jndi.url</tt> is always appended to the possibly
     * empty list of package prefixes.
     *
     * <p>
    ...
    /**
     * Constant that holds the name of the environment property for specifying
     * the DNS host and domain names to use for the JNDI URL context (for
     * example, "dns://somehost/wiz.com"). This property may be specified in the
     * environment, an applet parameter, a system property, or a resource file.
     * If it is not specified in any of these sources and the program attempts
     * to use a JNDI URL containing a DNS name, a
     * <tt>ConfigurationException</tt> will be thrown.
     *
     * <p>
    ...
    /**
     * Constant that holds the name of the environment property for specifying
     * the authoritativeness of the service requested. If the value of the
     * property is the string "true", it means that the access is to the most
     * authoritative source (i.e. bypass any cache or replicas). If the value is
     * anything else, the source need not be (but may be) authoritative. If
     ...
     * This is a hint to the provider to return the results of operations in
     * batches of the specified size, so the provider can optimize its
     * performance and usage of resources. The value of the property is the
     * string representation of an integer. If unspecified, the batch size is
     ...
     * </dl>
     * If this property is not specified, the default is determined by the
     * provider.
     *
     * <p>
    ...
    /**
     * Constant that holds the name of the environment property for specifying
     * the security protocol to use. Its value is a string determined by the
     * service provider (e.g. "ssl"). If this property is unspecified, the
     ...
     * "none", "simple", "strong". If this property is unspecified, the
     * behaviour is determined by the service provider.
     *
     * <p>
    ...
    /**
     * Constant that holds the name of the environment property for specifying
     * the identity of the principal for authenticating the caller to the
     * service. The format of the principal depends on the authentication
     * scheme. If this property is unspecified, the behaviour is determined by
     ...
     * service. The value of the property depends on the authentication scheme.
     * For example, it could be a hashed password, clear-text password, key,
     * certificate, and so on. If this property is unspecified, the behaviour is
     * determined by the service provider.
     *
     * <p>
    ...
    /**
     * Constant that holds the name of the environment property for specifying
     * the preferred language to use with the service. The value of the property
     * is a colon-separated list of language tags as defined in RFC 1766. If
     * this property is unspecified, the language preference is determined by
     * the service provider.
     *
     * <p>
    ...
    /**
     * Constant that holds the name of the environment property for specifying
     * an applet for the initial context constructor to use when searching for
     * other properties. The value of this property is the
     * <tt>java.applet.Applet</tt> instance that is being executed. This
     * property may be specified in the environment parameter passed to the
     * initial context constructor. When this property is set, each property
     * that the initial context constructor looks for in the system properties
     * is first looked for in the applet's parameter list. If this property is
     * unspecified, the initial context constructor will search for properties
     * only in the environment parameter passed to it, the system properties,
