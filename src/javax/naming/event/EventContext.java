 ...
 * possible to register interest in a target that does not exist, but there
 * might be limitations in the extent to which this can be supported by the
 * service provider and underlying protocol/service.
 * <p>
 * If a service only supports registration for existing targets, an attempt to
 * register for a nonexistent target results in a <tt>NameNotFoundException</tt>
 * being thrown as early as possible, preferably at the time
 * <tt>addNamingListener()</tt> is called, or if that is not possible, the
 * listener will receive the exception through the <tt>NamingExceptionEvent</tt>
 * .
 * <p>
 * Also, for service providers that only support registration for existing
 * targets, when the target that a listener has registered for is subsequently
 * removed from the namespace, the listener is notified via a
 * <tt>NamingExceptionEvent</tt> (containing a <tt>NameNotFoundException</tt>).
 * <p>
 ...
 * named by the target the source. In other words, the target, along with the
 * scope parameter, are used to identify the object(s) that the listener is
 * interested in, but the event source is the <tt>EventContext</tt> instance
 * with which the listener has registered.
 * <p>
 ...
 * </pre>
 * 
 * </blockquote> When an object named "x/y" is subsequently deleted, the
 * corresponding <tt>NamingEvent</tt> (<tt>evt</tt>) must contain: <blockquote>
 * 
 * <pre>
 ...
 * </blockquote>
 * <p>
 * Furthermore, listener registration/deregistration is with the
 * <tt>EventContext</tt> <em>instance</em>, and not with the corresponding
 * object in the namespace. If the program intends at some point to remove a
 * listener, then it needs to keep a reference to the <tt>EventContext</tt>
 * instance on which it invoked <tt>addNamingListener()</tt> (just as it needs
 * to keep a reference to the listener in order to remove it later). It cannot
 * expect to do a <tt>lookup()</tt> and get another instance of a
 * <tt>EventContext</tt> on which to perform the deregistration.
 * <h1>Lifetime of Registration</h1> A registered listener becomes deregistered
 * when:
 * <ul>
 ...
 * <li><tt>Context.close()</tt> is invoked on the <tt>EventContext</tt> instance
 ...
 * For example, the <tt>ObjectChangeListener</tt> interface is for the
 * <tt>NamingEvent.OBJECT_CHANGED</tt> event type. To register interest in
 * multiple event types, the listener implementation should implement multiple
 * <tt>NamingListener</tt> subinterfaces and use a single invocation of
 ...
 * concurrently. See the <a href=package-summary.html#THREADING>package
 * description</a> for more information on threading issues.
 *
 * @author Rosanna Lee
...
public interface EventContext extends Context {
    /**
     * Constant for expressing interest in events concerning the object named by
     * the target.
     * <p>
    ...
    /**
     * Constant for expressing interest in events concerning objects in the
     * context named by the target, excluding the context named by the target.
     * <p>
    ...
    /**
     * Constant for expressing interest in events concerning objects in the
     * subtree of the object named by the target, including the object named by
     * the target.
     * <p>
     ...
     * <tt>ONELEVEL_SCOPE</tt>. <tt>target</tt> may name a non-context if
     * <tt>scope</tt> is either <tt>OBJECT_SCOPE</tt> or <tt>SUBTREE_SCOPE</tt>.
     * Using <tt>SUBTREE_SCOPE</tt> for a non-context might be useful, for
     * example, if the caller does not know in advance whether <tt>target</tt>
     * is a context and just wants to register interest in the (possibly
     * degenerate subtree) rooted at <tt>target</tt>.
     * <p>
     * When the listener is notified of an event, the listener may in invoked in
     * a thread other than the one in which <tt>addNamingListener()</tt> is
     * executed. Care must be taken when multiple threads are accessing the same
     ...
     * <tt>EventContext</tt>. The listener may have registered more than once
     * with this <tt>EventContext</tt>, perhaps with different target/scope
     * arguments. After this method is invoked, the listener will no longer
     * receive events with this <tt>EventContext</tt> instance as the event
     * source (except for those events already in the process of being
     * dispatched). If the listener was not, or is no longer, registered with
    ...
    /**
     * Determines whether a listener can register interest in a target that does
     * not exist.
     *
