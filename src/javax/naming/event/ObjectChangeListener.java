...
/**
 * Specifies the method that a listener of a <tt>NamingEvent</tt> with event
 * type of <tt>OBJECT_CHANGED</tt> must implement.
 * <p>
 * An <tt>OBJECT_CHANGED</tt> event type is fired when (the contents of) an
 * object has changed. This might mean that its attributes have been modified,
 * added, or removed, and/or that the object itself has been replaced. How the
 * object has changed can be determined by examining the <tt>NamingEvent</tt>'s
 ...
 * method.
 * </ol>
 * A listener that wants to be notified of namespace change events should also
 * implement the <tt>NamespaceChangeListener</tt> interface.
 *
 * @author Rosanna Lee
