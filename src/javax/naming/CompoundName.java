 ...
 * <p>
 * The components of a compound name are numbered. The indexes of a compound
 ...
 * empty compound name has no components.
 *
 * <h1>Compound Name Syntax</h1> The syntax of a compound name is specified
 * using a set of properties:
 * <dl>
 * <dt>jndi.syntax.direction
 * <dd>Direction for parsing ("right_to_left", "left_to_right", "flat"). If
 * unspecified, defaults to "flat", which means the namespace is flat with no
 * hierarchical structure.
 *
 * <dt>jndi.syntax.separator
 * <dd>Separator between atomic name components. Required unless direction is
 * "flat".
 *
 * <dt>jndi.syntax.ignorecase
 * <dd>If present, "true" means ignore the case when comparing name components.
 * If its value is not "true", or if the property is not present, case is
 * considered when comparing name components.
 *
 * <dt>jndi.syntax.escape
 * <dd>If present, specifies the escape string for overriding separator, escapes
 * and quotes.
 *
 ...
 * <dd>String delimiting end of quoted string. If present, specifies the string
 * delimiting the end of a quoted string. If not present, use syntax.beginquote
 * as end quote.
 ...
 * <dd>If present, "true" means trim any leading and trailing whitespaces in a
 * name component for comparison purposes. If its value is not "true", or if the
 * property is not present, blanks are significant.
 ...
 * <dd>If present, specifies the string that separators attribute from value
 * (e.g. "=" in "age=65")
 * </dl>
 * These properties are interpreted according to the following rules:
 * <ol>
 * <li>In a string without quotes or escapes, any instance of the separator
 * delimits two atomic names. Each atomic name is referred to as a
 * <em>component</em>.
 * <li>A separator, quote or escape is escaped if preceded immediately (on the
 * left) by the escape.
 * <li>If there are two sets of quotes, a specific begin-quote must be matched
 * by its corresponding end-quote.
 * <li>A non-escaped begin-quote which precedes a component must be matched by a
 * non-escaped end-quote at the end of the component. A component thus quoted is
 * referred to as a <em>quoted component</em>. It is parsed by removing the
 * being- and end- quotes, and by treating the intervening characters as
 * ordinary characters unless one of the rules involving quoted components
 * listed below applies.
 * <li>Quotes embedded in non-quoted components are treated as ordinary strings
 * and need not be matched.
 * <li>A separator that is escaped or appears between non-escaped quotes is
 * treated as an ordinary string and not a separator.
 * <li>An escape string within a quoted component acts as an escape only when
 * followed by the corresponding end-quote string. This can be used to embed an
 * escaped quote within a quoted component.
 * <li>An escaped escape string is not treated as an escape string.
 * <li>An escape string that does not precede a meta string (quotes or
 * separator) and is not at the end of a component is treated as an ordinary
 * string.
 * <li>A leading separator (the compound name string begins with a separator)
 * denotes a leading empty atomic component (consisting of an empty string). A
 * trailing separator (the compound name string ends with a separator) denotes a
 * trailing empty atomic component. Adjacent separators denote an empty atomic
 * component.
 * </ol>
 * <p>
 ...
 *
 * <h1>Multithreaded Access</h1> A <tt>CompoundName</tt> instance is not
 * synchronized against concurrent multithreaded access. Multiple threads trying
 * to access and modify a <tt>CompoundName</tt> should lock the object.
 *
 * @author Rosanna Lee
...
public class CompoundName implements Name {
	 ...
	 * constructors and cannot be null. It should be treated as a read-only
	 * variable by subclasses.
	 */
	protected transient NameImpl impl;
	/**
	 * Syntax properties for this compound name. This field is initialized by
	 * the constructors and cannot be null. It should be treated as a read-only
	 * variable by subclasses. Any necessary changes to mySyntax should be made
	 * within constructors and not after the compound name has been
	 * instantiated.
	 */
	protected transient Properties mySyntax;

	/**
	 * Constructs a new compound name instance using the components specified in
	 * comps and syntax. This protected method is intended to be to be used by
	 * subclasses of CompoundName when they override methods such as clone(),
	 * getPrefix(), getSuffix().
	 *
	 * @param comps
	...
	/**
	 * Generates the string representation of this compound name, using the
	 * syntax rules of the compound name. The syntax rules are described in the
	 * class description. An empty component is represented by an empty string.
	 *
	 * The string representation thus generated can be passed to the
	 * CompoundName constructor with the same syntax properties to create a new
	 * equivalent compound name.
	 *
	...
	/**
	 * Determines whether obj is syntactically equal to this compound name. If
	 * obj is null or not a CompoundName, false is returned. Two compound names
	 * are equal if each component in one is "equal" to the corresponding
	 ...
	 * jndi.syntax.ignorecase and jndi.syntax.trimblanks when comparing two
	 * components for equality. If case is ignored, two strings with the same
	 * sequence of characters but with different cases are considered equal. If
	 * blanks are being trimmed, leading and trailing blanks are ignored for the
	...
	/**
	 * Computes the hash code of this compound name. The hash code is the sum of
	 * the hash codes of the "canonicalized" forms of individual components of
	 * this compound name. Each component is "canonicalized" according to the
	 * compound name's syntax before its hash code is computed. For a
	 * case-insensitive name, for example, the uppercased form of a name has the
	 * same hash code as its lowercased equivalent.
	 *
	 ...
	 * compound name won't affect the new copy and vice versa. The clone and
	 * this compound name share the same syntax.
	 *
	 ...
	 * negative integer, zero, or a positive integer as this Name is less than,
	 ...
	 * extension that this applies to all the components in the compound name
	 * and that comparison of individual components is affected by the
	 * jndi.syntax.ignorecase and jndi.syntax.trimblanks properties, identical
	 * to how they affect equals(). If this compound name is "lexicographically"
	 * lesser than obj, a negative number is returned. If this compound name is
	...
	/**
	 * Determines whether this compound name is empty. A compound name is empty
	 * if it has zero components.
	 *
	...
	/**
	 * Retrieves the components of this compound name as an enumeration of
	 * strings. The effects of updates to this compound name on this enumeration
	 * is undefined.
	 *
	 * @return A non-null enumeration of the components of this compound name.
	 ...
	 * components in this compound name. The result and this compound name share
	 * the same syntax. Subsequent changes to this compound name does not affect
	 ...
	 * compound name 'n' is a prefix if it is equal to getPrefix(n.size())--in
	 * other words, this compound name starts with 'n'. If n is null or not a
	 * compound name, false is returned.
	 * <p>
	 * Implementation note: Currently the syntax properties of n are not used
	 ...
	 * getSuffix(size()-n.size())--in other words, this compound name ends with
	 * 'n'. If n is null or not a compound name, false is returned.
	 * <p>
	 * Implementation note: Currently the syntax properties of n are not used
	 * when doing the comparison. They might be in the future.
	 * 
	 ...
	 * compound name.
	 * <p>
	 * Implementation note: Currently the syntax properties of suffix is not
	 ...
	 * position within this compound name. Components of this compound name at
	...
	/**
	 * Adds a single component at a specified position within this compound
	 * name. Components of this compound name at or after the index of the new
	...
	/**
	 * Deletes a component from this compound name. The component of this
	 * compound name at position 'posn' is removed, and components at indices
	 ...
	 * 
	 * @serialData The syntax <tt>Properties</tt>, followed by the number of
