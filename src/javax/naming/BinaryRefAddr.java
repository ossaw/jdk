 ...
 * communication mechanism. The format and interpretation of the address type
 * and the contents of the opaque buffer are based on the agreement of three
 * parties: the client that uses the address, the object/server that can be
 * reached using the address, and the administrator or program that creates the
 * address.
 * <p>
 * An example of a binary reference address is an BER X.500 presentation
 * address. Another example of a binary reference address is a serialized form
 * of a service's object handle.
 * <p>
...
/*
 * The serialized form of a BinaryRefAddr object consists of its type name
 * String and a byte array containing its "contents".
 */

public class BinaryRefAddr extends RefAddr {
	/**
	 * Contains the bytes of the address. This field is initialized by the
	 * constructor and returned using getAddressBytes() and
	 * getAddressContents().
	 * 
	 * @serial
	 */
	...
	/**
	 * Constructs a new instance of BinaryRefAddr using its address type and a
	 ...
	 * Programs are recommended against changing this array's contents and to
	 * lock the buffer if they need to change it.
	 *
	...
	/**
	 * Determines whether obj is equal to this address. It is equal if it
	 * contains the same address type and their contents are byte-wise
	 * equivalent.
	 * 
	 * @param obj
	...
	/**
	 * Computes the hash code of this address using its address type and
	 * contents. Two BinaryRefAddrs have the same hash code if they have the
	 * same address type and the same contents. It is also possible for
	 * different BinaryRefAddrs to have the same hash code.
	 *
	...
	/**
	 * Generates the string representation of this address. The string consists
	 * of the address's type and contents with labels. The first 32 bytes of
	 * contents are displayed (in hexadecimal). If there are more than 32 bytes,
	 * "..." is used to indicate more. This string is meant to used for
	 * debugging purposes and not meant to be interpreted programmatically.
	 * 
