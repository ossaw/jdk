 ...
 * resolution of names.
 * <p>
 * <a name=ENVIRONMENT></a> When the initial context is constructed, its
 * environment is initialized with properties defined in the environment
 * parameter passed to the constructor, and in any
 * <a href=Context.html#RESOURCEFILES>application resource files</a>. In
 * addition, a small number of standard JNDI properties may be specified as
 * system properties or as applet parameters (through the use of
 * {@link Context#APPLET}). These special properties are listed in the field
 * detail sections of the <a href=Context.html#field_detail><tt>Context</tt></a>
 * and <a href=ldap/LdapContext.html#field_detail><tt>LdapContext</tt></a>
 * interface documentation.
 * <p>
 * JNDI determines each property's value by merging the values from the
 * following two sources, in order:
 * <ol>
 * <li>The first occurrence of the property from the constructor's environment
 * parameter and (for appropriate properties) the applet parameters and system
 * properties.
 * <li>The application resource files (<tt>jndi.properties</tt>).
 * </ol>
 ...
 * JNDI factories (see <a href=Context.html#LISTPROPS><tt>Context</tt></a>), all
 * of the values are concatenated into a single colon-separated list. For other
 * properties, only the first value found is used.
 * <p>
 * The initial context implementation is determined at runtime. The default
 * policy uses the environment property
 * "{@link Context#INITIAL_CONTEXT_FACTORY java.naming.factory.initial}", which
 ...
 * <em>scheme_id:rest_of_name</em>) is passed as a name parameter to any method,
 * a URL context factory for handling that scheme is located and used to resolve
 * the URL. If no such factory is found, the initial context specified by
 * <tt>"java.naming.factory.initial"</tt> is used. Similarly, when a
 * <tt>CompositeName</tt> object whose first component is a URL string is passed
 * as a name parameter to any method, a URL context factory is located and used
 * to resolve the first name component. See {@link NamingManager#getURLContext
 * <tt>NamingManager.getURLContext()</tt>} for a description of how URL context
 ...
 * multiple threads. Multiple threads each manipulating a different
...
public class InitialContext implements Context {
    ...
    /**
     * The environment associated with this InitialContext. It is initialized to
     * null and is updated by the constructor that accepts an environment or by
    ...
    /**
     * Field holding the result of calling NamingManager.getInitialContext(). It
     * is set by getDefaultInitCtx() the first time getDefaultInitCtx() is
     * called. Subsequent invocations of getDefaultInitCtx() return the value of
     * defaultInitCtx.
     * 
     ...
     * NamingManager.getInitialContext(). If true, its result is in
     * <code>defaultInitCtx</code>.
     */
    protected boolean gotDefault = false;

    /**
     * Constructs an initial context with the option of not initializing it.
     * This may be used by a constructor in a subclass when the value of the
     * environment parameter is not yet known at the time the
     * <tt>InitialContext</tt> constructor is called. The subclass's constructor
     * will call this constructor, compute the value of the environment, and
     ...
     * to it, but may save a clone. Caller should not modify mutable keys and
     * values in <tt>environment</tt> after it has been passed to the
     * constructor.
     *
     * @param environment
     ...
     * <p>
     * This method will modify <tt>environment</tt> and save a reference to it.
    ...
    /**
     * A static method to retrieve the named object. This is a shortcut method
     ...
     * </code>
     * <p>
     * If <tt>name</tt> is empty, returns a new instance of this context (which
     ...
     * Since an initial context may never be named relative to any context other
