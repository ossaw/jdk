 ...
 * <code>Name</code> parameter and one taking a <code>String</code>. These
 * overloaded versions are equivalent in that if the <code>Name</code> and
 * <code>String</code> parameters are just different representations of the same
 * name, then the overloaded versions of the same methods behave the same. In
 * the method descriptions below, only one version is documented. The second
 ...
 * object is roughly equivalent to a lookup on the name (which returns the
 * DirContext object), followed by the attribute operation invoked on the
 * DirContext object in which the caller supplies an empty name. The attributes
 * can be viewed as being stored along with the object (note that this does not
 * imply that the implementation must do so).
 * <p>
 * The second model is that attributes are associated with a name (typically an
 * atomic name) in a DirContext. In this model, an attribute operation on the
 * named object is roughly equivalent to a lookup on the name of the parent
 * DirContext of the named object, followed by the attribute operation invoked
 * on the parent in which the caller supplies the terminal atomic name. The
 * attributes can be viewed as being stored in the parent DirContext (again,
 * this does not imply that the implementation must do so). Objects that are not
 * DirContexts can have attributes, as long as their parents are DirContexts.
 * <p>
 * JNDI support both of these models. It is up to the individual service
 * providers to decide where to "store" attributes. JNDI clients are safest when
 ...
 * back might not have the same names as the attribute names you have specified.
 * This is because some directories support features that cause them to return
 * other attributes. Such features include attribute subclassing, attribute name
 * synonyms, and attribute language codes.
 * <p>
 * In attribute subclassing, attributes are defined in a class hierarchy. In
 * some directories, for example, the "name" attribute might be the superclass
 * of all name-related attributes, including "commonName" and "surName". Asking
 * for the "name" attribute might return both the "commonName" and "surName"
 * attributes.
 * <p>
 * With attribute type synonyms, a directory can assign multiple names to the
 * same attribute. For example, "cn" and "commonName" might both refer to the
 * same attribute. Asking for "cn" might return the "commonName" attribute.
 * <p>
 * Some directories support the language codes for attributes. Asking such a
 * directory for the "description" attribute, for example, might return all of
 ...
 * attributes associated with a directory object for administrative purposes. An
 * example of operational attributes is the access control list for an object.
 * <p>
 * In the <tt>getAttributes()</tt> and <tt>search()</tt> methods, you can
 * specify that all attributes associated with the requested objects be returned
 ...
 * example, when searching a single level context). The documentation of such
 * methods use the term <em>named context</em> to describe their name parameter.
 ...
 * parameter to any method will not be modified by the service provider. The
 * service provider may keep a reference to it for the duration of the
 * operation, including any enumeration of the method's results and the
 * processing of any referrals generated. The caller should not modify the
 ...
 * subclasses. See NamingException and their subclasses for details on each
 * exception.
 *
 * @author Rosanna Lee
...
public interface DirContext extends Context {
	 ...
	 * <p>
	 * If attribute does not exist, create the attribute. The resulting
	 * attribute has a union of the specified value set and the prior value set.
	 * Adding an attribute with no value will throw
	 * <code>InvalidAttributeValueException</code> if the attribute must have at
	 * least one value. For a single-valued attribute where that attribute
	 * already exists, throws <code>AttributeInUseException</code>. If
	 ...
	 * <p>
	 * If attribute already exists, replaces all existing values with new
	 * specified values. If the attribute does not exist, creates it. If no
	 * value is specified, deletes all the values of the attribute. Removal of
	 * the last value will remove the attribute if the attribute is required to
	 * have at least one value. If attempting to add more than one value to a
	 * single-valued attribute, throws
	...
	/**
	 * This constant specifies to delete the specified attribute values from the
	 * attribute.
	 * <p>
	 * The resulting attribute has the set difference of its prior value set and
	 * the specified value set. If no values are specified, deletes the entire
	 * attribute. If the attribute does not exist, or if some or all members of
	 * the specified value set do not exist, this absence may be ignored and the
	 * operation succeeds, or a NamingException may be thrown to indicate the
	 * absence. Removal of the last value will remove the attribute if the
