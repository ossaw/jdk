 ...
 * This property is determined at the time the BasicAttributes constructor is
 * called. In a case-insensitive BasicAttributes, the case of its attribute
 * identifiers is ignored when searching for an attribute, or adding attributes.
 * In a case-sensitive BasicAttributes, the case is significant.
 * <p>
 * When the BasicAttributes class needs to create an Attribute, it uses
 * BasicAttribute. There is no other dependency on BasicAttribute.
 * <p>
 * Note that updates to BasicAttributes (such as adding or removing an
 * attribute) does not affect the corresponding representation in the directory.
 ...
 * BasicAttributes instance should lock the object.
 *
 * @author Rosanna Lee
...
public class BasicAttributes implements Attributes {
	...
	/**
	 * Constructs a new instance of Attributes. The character case of attribute
	 * identifiers is significant when subsequently retrieving or adding
	 * attributes.
	 */
	...
	/**
	 * Constructs a new instance of Attributes. If <code>ignoreCase</code> is
	 * true, the character case of attribute identifiers is ignored; otherwise
	 * the case is significant.
	 * 
	 * @param ignoreCase
	 *            true means this attribute set will ignore the case of its
	...
	/**
	 * Constructs a new instance of Attributes with one attribute. The attribute
	 * specified by attrID and val are added to the newly created attribute. The
	 * character case of attribute identifiers is significant when subsequently
	...
	/**
	 * Constructs a new instance of Attributes with one attribute. The attribute
	 * specified by attrID and val are added to the newly created attribute. If
	 * <code>ignoreCase</code> is true, the character case of attribute
	 * identifiers is ignored; otherwise the case is significant.
	 * 
	 * @param attrID
	 *            non-null The id of the attribute to add. If this attribute set
	...
	/**
	 * Generates the string representation of this attribute set. The string
	 * consists of each attribute identifier and the contents of each attribute.
	 * The contents of this string is useful for debugging and is not meant to
	 ...
	 * <tt>Attributes</tt> Two <tt>Attributes</tt> are equal if they are both
	 * instances of <tt>Attributes</tt>, treat the case of attribute IDs the
	 * same way, and contain the same attributes. Each <tt>Attribute</tt> in
	 * this <tt>BasicAttributes</tt> is checked for equality using
	 * <tt>Object.equals()</tt>, which may have be overridden by implementations
	 * of <tt>Attribute</tt>). If a subclass overrides <tt>equals()</tt>, it
	 * should override <tt>hashCode()</tt> as well so that two
	 ...
	 * <p>
	 * The hash code is computed by adding the hash code of the attributes of
	 * this object. If this BasicAttributes ignores case of its attribute IDs,
	 * one is added to the hash code. If a subclass overrides
	 * <tt>hashCode()</tt>, it should override <tt>equals()</tt> as well so that
	 * two <tt>Attributes</tt> instances that are equal have the same hash code.
	 *
	 * @return an int representing the hash code of this BasicAttributes
	 *         instance.
	 * @see #equals
	 */
	public int hashCode() {
	 ...
	 * 
	 * @serialData Default field (ignoreCase flag -- a boolean), followed by the
