...
/**
 * A <code>BooleanControl</code> provides the ability to switch between two
 * possible settings that affect a line's audio. The settings are boolean values
 * (<code>true</code> and <code>false</code>). A graphical user interface might
 * represent the control by a two-state button, an on/off switch, two mutually
 * exclusive buttons, or a checkbox (among other possibilities). For example,
 * depressing a button might activate a
 * <code>{@link BooleanControl.Type#MUTE MUTE}</code> control to silence the
 * line's audio.
 * <p>
 * As with other <code>{@link Control}</code> subclasses, a method is provided
...
public abstract class BooleanControl extends Control {
	 ...
	 *            the label for the state represented by <code>true</code>, such
	 *            as "true" or "on."
	 * @param falseStateLabel
	 *            the label for the state represented by <code>false</code>,
	 ...
	 * labels for the <code>true</code> and <code>false</code> states default to
	 * "true" and "false."
	 *
	 * @param type
	...
	/**
	 * An instance of the <code>BooleanControl.Type</code> class identifies one
	 * kind of boolean control. Static instances are provided for the common
	 * types.
	 *
	 * @author Kara Kytle
	 * @since 1.3
