 ...
 * often instances of classes that implement <code>{@link SourceDataLine}</code>
 * , and the latter, <code>{@link TargetDataLine}</code>.
 * <code>{@link Port}</code> objects, too, are either source lines or target
 * lines. A mixer can accept prerecorded, loopable sound as input, by having
 * some of its source lines be instances of objects that implement the
 * <code>{@link Clip}</code> interface.
 * <p>
 * Through methods of the <code>Line</code> interface, which <code>Mixer</code>
 * extends, a mixer might provide a set of controls that are global to the
 * mixer. For example, the mixer can have a master gain control. These global
 * controls are distinct from the controls belonging to each of the mixer's
 * individual lines.
 * <p>
 * Some mixers, especially those with internal digital mixing capabilities, may
 * provide additional capabilities by implementing the <code>DataLine</code>
 * interface.
 * <p>
...
public interface Mixer extends Line {
     ...
     *         for this mixer. If no source lines are supported, an array of
     ...
     *         for this mixer. If no target lines are supported, an array of
     *         length 0 is returned.
     */
     ...
     * specified <code>Line.Info</code> object. Some lines may only be supported
    ...
    // $$fb 2002-04-12: fix for 4667258: behavior of
    // Mixer.getMaxLines(Line.Info) method doesn't match the spec
    /**
     ...
     * provided <code>Line.Info</code> object. For example, if the info object
     ...
     * otherwise an exception may be thrown. However, <code>null</code> may be
     ...
     * mixer, including the product's name, version, and vendor, along with a
     * textual description. This information may be retrieved through the
     * {@link Mixer#getMixerInfo() getMixerInfo} method of the
     * <code>Mixer</code> interface.
     *
     * @author Kara Kytle
     * @since 1.3
     */
    public static class Info {
         ...
         * <code>true</code> if they are identical.
         * 
         * @param obj
