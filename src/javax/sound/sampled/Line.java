...
/**
 * The <code>Line</code> interface represents a mono or multi-channel audio
 * feed. A line is an element of the digital audio "pipeline," such as a mixer,
 ...
 * <code>{@link Control}</code> class. The <code>Line</code> interface provides
 * two accessor methods for obtaining the line's controls:
 * <code>{@link #getControls getControls}</code> returns the entire set, and
 * <code>{@link #getControl getControl}</code> returns a single control of
 * specified type.
 * <p>
 ...
 * freed for other objects or applications. The <code>{@link #isOpen()}</code>
 ...
 * changes. The object must implement the <code>{@link LineListener}</code>
 * interface, which consists of the single method
 * <code>{@link LineListener#update update}</code>. This method will be invoked
 * when a line opens and closes (and, if it's a {@link DataLine}, when it starts
 * and stops).
 * <p>
 ...
 * the event, what type of event it was (<code>OPEN</code>, <code>CLOSE</code>,
 * <code>START</code>, or <code>STOP</code>), and how many sample frames the
 * line had processed at the time the event occurred.
 * <p>
...
public interface Line extends AutoCloseable {
     ...
     * marked as open, and an <code>OPEN</code> event is dispatched to the
     * line's listeners.
     * <p>
     ...
     * allocation. For example, a <code>DataLine</code> must be opened with a
     * particular format and buffer size. Such lines should provide a mechanism
     * for configuring these properties, such as an additional <code>open</code>
     ...
     * the buffer size is determined when data is loaded. Since this method does
     * not allow the application to specify any data to load, an
     * IllegalArgumentException is thrown. Therefore, you should instead use one
     * of the <code>open</code> methods provided in the <code>Clip</code>
     * interface to load data into the <code>Clip</code>.
     * <p>
     * For <code>DataLine</code>'s, if the <code>DataLine.Info</code> object
     * which was used to retrieve the line, specifies at least one fully
     ...
     * or capturing sound.
     * 
     * @return <code>true</code> if the line is open, otherwise
     *         <code>false</code>
     * @see #open()
     * @see #close()
     */
     ...
     * listener's <code>update()</code> method is called with a
    ...
    /**
     * A <code>Line.Info</code> object contains information about a line. The
     * only information provided by <code>Line.Info</code> itself is the Java
     * class of the line. A subclass of <code>Line.Info</code> adds other kinds
     * of information about the line. This additional information depends on
     * which <code>Line</code> subinterface is implemented by the kind of line
     * that the <code>Line.Info</code> subclass describes.
     * <p>
     * A <code>Line.Info</code> can be retrieved using various methods of the
     * <code>Line</code>, <code>Mixer</code>, and <code>AudioSystem</code>
     ...
     *      <code>Mixer.getSourceLineInfo(Line.Info)</code>
     * @see Mixer#getSourceLineInfo(Line.Info)
     *      <code>Mixer.getTargetLineInfo(Line.Info)</code>
     ...
     * @see AudioSystem#getSourceLineInfo
     *      <code>AudioSystem.getSourceLineInfo(Line.Info)</code>
     * @see AudioSystem#getTargetLineInfo
     *      <code>AudioSystem.getTargetLineInfo(Line.Info)</code>
     * @see AudioSystem#isLineSupported
     *      <code>AudioSystem.isLineSupported(Line.Info)</code>
     * @since 1.3
     */
    public static class Info {
        ...
        public String toString() {
            ...
            if (index != -1) {
                finalString = initialString.substring(0, index) + initialString.substring((index
                        + fullPackagePath.length()), initialString.length());
            } else {
