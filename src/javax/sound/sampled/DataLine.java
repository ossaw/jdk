...
/**
 * <code>DataLine</code> adds media-related functionality to its superinterface,
 * <code>{@link Line}</code>. This functionality includes transport-control
 ...
 * <code>{@link Clip}</code>, which allow an application program to write data.
 * Similarly, audio input is handled by the subinterface
 * <code>{@link TargetDataLine}</code>, which allows data to be read.
 * <p>
 * A data line has an internal buffer in which the incoming or outgoing audio
 * data is queued. The <code>{@link #drain()}</code> method blocks until this
 * internal buffer becomes empty, usually because all queued data has been
 * processed. The <code>{@link #flush()}</code> method discards any available
 * queued data from the internal buffer.
 * <p>
 * A data line produces <code>{@link LineEvent.Type#START START}</code> and
 * <code>{@link LineEvent.Type#STOP STOP}</code> events whenever it begins or
 * ceases active presentation or capture of data. These events can be generated
 * in response to specific requests, or as a result of less direct state
 * changes. For example, if <code>{@link #start()}</code> is called on an
 * inactive data line, and data is available for capture or playback, a
 * <code>START</code> event will be generated shortly, when data playback or
 * capture actually begins. Or, if the flow of data to an active data line is
 * constricted so that a gap occurs in the presentation of data, a
 * <code>STOP</code> event is generated.
 * <p>
 * Mixers often support synchronized control of multiple data lines.
 * Synchronization can be established through the Mixer interface's
 * <code>{@link Mixer#synchronize synchronize}</code> method. See the
 * description of the <code>{@link Mixer Mixer}</code> interface for a more
...
public interface DataLine extends Line {
     ...
     * used with care. If <code>drain()</code> is invoked on a stopped line that
     * has data in its queue, the method will block until the line is running
     * and the data queue becomes empty. If <code>drain()</code> is invoked by
     * one thread, and another continues to fill the data queue, the operation
     * will not complete. This method always returns when the data line is
     * closed.
     *
     * @see #flush()
     */
     ...
     * playback starts, a <code>{@link LineEvent.Type#START START}</code> event
     * is generated.
     *
     * @see #stop()
     * @see #isRunning()
     ...
     * invoking the <code>flush</code> method. When audio capture or playback
     * stops, a <code>{@link LineEvent.Type#STOP STOP}</code> event is
     * generated.
     *
     * @see #start()
     * @see #isRunning()
     ...
     * An open line begins running when the first data is presented in response
     * to an invocation of the <code>start</code> method, and continues until
     * presentation ceases in response to a call to <code>stop</code> or because
     * playback completes.
     * 
     * @return <code>true</code> if the line is running, otherwise
     *         <code>false</code>
     * @see #start()
     * @see #stop()
     */
     ...
     * <code>{@link LineEvent.Type#START START}</code> event to its listeners.
     * Similarly, when an active line becomes inactive, it sends a
     ...
     * or, if the <code>DataLine.Info</code> object, which was used to retrieve
     * this <code>DataLine</code>, specifies at least one fully qualified audio
     * format, the last one will be used as the default format. Opening the line
     * with a specific audio format (e.g.
     * {@link SourceDataLine#open(AudioFormat)}) will override the default
     * format.
     *
     ...
     * number of bytes returned from <code>available()</code> will not block;
     ...
     * <code>getLongFramePosition</code> instead.
     *
     ...
     *
     * @return the current amplitude of the signal in this line, or <code>
     *         {@link AudioSystem#NOT_SPECIFIED}</code>
     */
    public float getLevel();

    /**
     * Besides the class information inherited from its superclass,
     * <code>DataLine.Info</code> provides additional information specific to
     * data lines. This information includes:
     * <ul>
     * <li>the audio formats supported by the data line
     * <li>the minimum and maximum sizes of its internal buffer
     * </ul>
     * Because a <code>Line.Info</code> knows the class of the line its
     * describes, a <code>DataLine.Info</code> object can describe
     * <code>DataLine</code> subinterfaces such as
     * <code>{@link SourceDataLine}</code>, <code>{@link TargetDataLine}</code>,
     * and <code>{@link Clip}</code>. You can query a mixer for lines of any of
     * these types, passing an appropriate instance of
    ...
    public static class Info extends Line.Info {
         ...
         * <code>isFormatSupported(AudioFormat)</code> might return
         * <code>true</code> for certain additional formats that are missing
         * from the set returned by <code>getFormats()</code>. The reverse is
         ...
         * the <code>AudioFormat</code> instances returned by this method to
         ...
         * <code>true</code> if the specified format matches any of the
         * supported formats.
         *
         * @param format
