...
/**
 * A <code>EnumControl</code> provides control over a set of discrete possible
 ...
 * <code>{@link FloatControl}</code> objects.
 * <p>
 * Controls that provide a choice between only two settings can often be
 * implemented instead as a <code>{@link BooleanControl}</code>, and controls
 * that provide a set of values along some quantifiable dimension might be
 * implemented instead as a <code>FloatControl</code> with a coarse resolution.
 * However, a key feature of <code>EnumControl</code> is that the returned
 * values are arbitrary objects, rather than numerical or boolean values. This
 * means that each returned object can provide further information. As an
 * example, the settings of a
 * <code>{@link EnumControl.Type#REVERB REVERB}</code> control are instances of
...
public abstract class EnumControl extends Control {
	 ...
	 *
	 * @param type
	 *            the type of control represented this enumerated control object
	 * @param values
	...
	private boolean isValueSupported(Object value) {
		...
		for (int i = 0; i < values.length; i++) {
			// $$fb 2001-07-20: Fix for bug 4400392: setValue() in ReverbControl
	...
	/**
	 * An instance of the <code>EnumControl.Type</code> inner class identifies
	 * one kind of enumerated control. Static instances are provided for the
	 * common types.
	 *
	 * @see EnumControl
	 *
	...
	public static class Type extends Control.Type {
		 ...
		 * <code>{@link EnumControl#getValues}</code> on an enumerated control
		 * of type <code>REVERB</code>.)
		 */
