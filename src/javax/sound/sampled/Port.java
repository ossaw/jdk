 ...
 * access port using a <code>{@link Port.Info}</code> object.
 *
 * @author Kara Kytle
 * @since 1.3
 */
public interface Port extends Line {
	 ...
	 * and whether it is a source or a target for its mixer. By definition, a
	 * port acts as either a source or a target to its mixer, but not both.
	 ...
	 * <code>{@link Mixer#getTargetLineInfo getTargetLineInfo}</code> methods of
	 * the <code>Mixer</code> interface. Instances of the <code>Port.Info</code>
	 * class may also be constructed and used to obtain lines matching the
	 * parameters specified in the <code>Port.Info</code> object.
	 *
	 * @author Kara Kytle
	 * @since 1.3
	 */
	public static class Info extends Line.Info {
		 ...
		 * 
		 * @return <code>true</code> if the port is a source port (such as a
		 *         microphone), <code>false</code> if the port is a target port
		 *         (such as a speaker).
		 */
