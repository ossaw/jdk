 ...
 * floating-point values. Float controls are often represented in graphical user
 * interfaces by continuously adjustable objects such as sliders or rotary
 * knobs. Concrete subclasses of <code>FloatControl</code> implement controls,
 * such as gain and pan, that affect a line's audio signal in some way that an
 * application can manipulate. The <code>{@link FloatControl.Type}</code> inner
 * class provides static instances of types that are used to identify some
 * common kinds of float control.
 * <p>
 ...
 * specified period of time. <code>FloatControl</code> also includes methods
 * that return string labels for the minimum, maximum, and midpoint positions of
 * the control.
 *
...
public abstract class FloatControl extends Control {
    ...
    protected FloatControl(Type type, float minimum, float maximum, float precision, int updatePeriod,
        ...
        if (minimum > maximum) {
            throw new IllegalArgumentException("Minimum value " + minimum + " exceeds maximum value "
                    + maximum + ".");
        }
     ...
     * IllegalArgumentException is thrown. Some controls require that their line
     ...
     * 
     * @return the minimum value label, or a zero-length string if no label *
     *         has been set
     */
     ...
     * 
     * @return the maximum value label, or a zero-length string if no label *
     *         has been set
     */
    ...
    public void shift(float from, float to, int microseconds) {
        ...
        if (from > maximum) {
            throw new IllegalArgumentException("Requested value " + from + " exceeds allowable maximum value "
                    + maximum + ".");
        }
        setValue(to);
    }
    ...
    public String toString() {
        return new String(getType() + " with current value: " + getValue() + " " + units + " (range: "
                + minimum + " - " + maximum + ")");
    }
    ...
    /**
     * An instance of the <code>FloatControl.Type</code> inner class identifies
     * one kind of float control. Static instances are provided for the common
     * types.
     *
     * @author Kara Kytle
     * @since 1.3
    ...
    public static class Type extends Control.Type {
         ...
         * (boosts) the signal's volume, and a negative gain attenuates (cuts)
         * it. The gain setting defaults to a value of 0.0 dB, meaning the
         * signal's loudness is unaffected. Note that gain measures dB, not
         * amplitude. The relationship between a gain in decibels and the
         * corresponding linear amplitude multiplier is:
         * <CENTER><CODE> linearScalar = pow(10.0, gainDB/20.0) </CODE></CENTER>
         * <p>
         * The <code>FloatControl</code> class has methods to impose a maximum
         * and minimum allowable value for gain. However, because an audio
         * signal might already be at a high amplitude, the maximum setting does
         * not guarantee that the signal will be undistorted when the gain is
         ...
         * specified in the data line's <code>AudioFormat</code> object. For
         * example, if the natural rate of the media is 11025 samples per second
         * and the sample rate is set to 22050 samples per second, the media
         * will play back at twice the normal speed.
         * <p>
