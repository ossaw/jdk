 ...
 * of data in a sound stream. By examining the information stored in the audio
 ...
 * line. Sound files also have audio formats, of course. The
 * <code>{@link AudioFileFormat}</code> class encapsulates an
 * <code>AudioFormat</code> in addition to other, file-specific information.
 * Similarly, an <code>{@link AudioInputStream}</code> has an
 ...
 * ignored in the {@link #matches(AudioFormat)} method. However, methods which
 * rely on the installed service providers, like
...
public class AudioFormat {
	...
	public AudioFormat(float sampleRate, int sampleSizeInBits, int channels, boolean signed,
		...
		this((signed == true ? Encoding.PCM_SIGNED : Encoding.PCM_UNSIGNED), sampleRate,
	 ...
	 * rate of <code>AudioSystem.NOT_SPECIFIED</code> means that any sample rate
	 * is acceptable. <code>AudioSystem.NOT_SPECIFIED</code> is also returned
	 * when the sample rate is not defined for this audio format.
	 * 
	 ...
	 * AudioSystem.isConversionSupported}) or capabilities (e.g.
	 * {@link DataLine.Info#getFormats() DataLine.Info.getFormats}), a sample
	 * size of <code>AudioSystem.NOT_SPECIFIED</code> means that any sample size
	 * is acceptable. <code>AudioSystem.NOT_SPECIFIED</code> is also returned
	 * when the sample size is not defined for this audio format.
	 * 
	 ...
	 * value of <code>AudioSystem.NOT_SPECIFIED</code> means that any (positive)
	 ...
	 * size of <code>AudioSystem.NOT_SPECIFIED</code> means that any frame size
	 * is acceptable. <code>AudioSystem.NOT_SPECIFIED</code> is also returned
	 * when the frame size is not defined for this audio format.
	 * 
	 ...
	 * AudioSystem.isConversionSupported}) or capabilities (e.g.
	 * {@link DataLine.Info#getFormats() DataLine.Info.getFormats}), a frame
	 * rate of <code>AudioSystem.NOT_SPECIFIED</code> means that any frame rate
	 * is acceptable. <code>AudioSystem.NOT_SPECIFIED</code> is also returned
	 * when the frame rate is not defined for this audio format.
	 * 
	 ...
	 * formats, and make these available through the
	 * <code>{@link AudioSystem}</code> class.
	 * <p>
