 ...
 * <p>
 * The <code>SourceDataLine</code> interface provides a method for writing audio
 * data to the data line's buffer. Applications that play or mix audio should
 * write data to the source data line quickly enough to keep the buffer from
 ...
 * queued in the data line's buffer. The amount of data which can be written to
 * the buffer without blocking is the difference between the buffer size and the
 * amount of queued data. If the delivery of audio output stops due to
 * underflow, a <code>{@link LineEvent.Type#STOP STOP}</code> event is
 * generated. A <code>{@link LineEvent.Type#START START}</code> event is
 * generated when the audio output resumes.
 *
 * @author Kara Kytle
 * @see Mixer
 * @see DataLine
...
public interface SourceDataLine extends DataLine {
	 ...
	 * such a line will always result in a <code>LineUnavailableException</code>
	 * .
	 *
	 * @param format
	 ...
	 * (see <code>{@link DataLine#available available}</code>), this method
	 * blocks until the requested amount of data has been written. This applies
	 * even if the requested amount of data to write is greater than the data
	 * line's buffer size. However, if the data line is closed, stopped, or
	 * flushed before the requested amount has been written, the method no
	 * longer blocks, but returns the number of bytes written thus far.
	 * <p>
	 * The number of bytes that can be written without blocking can be
	 * ascertained using the <code>{@link DataLine#available available}</code>
	 * method of the <code>DataLine</code> interface. (While it is guaranteed
	 * that this number of bytes can be written without blocking, there is no
	 * guarantee that attempts to write additional data will block.)
	 * <p>
	 * The number of bytes to write must represent an integral number of sample
	 * frames, such that: <br>
	 * <center>
	 * <code>[ bytes written ] % [frame size in bytes ] == 0</code></center>
	 * <br>
	 ...
	 * @param len
	 *            the length, in bytes, of the valid data in the array (in other
	 *            words, the requested amount of data to write, in bytes)
	 * @param off
