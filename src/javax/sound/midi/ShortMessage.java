...
/**
 * A <code>ShortMessage</code> contains a MIDI message that has at most two data
 ...
 * meta-events. The <code>ShortMessage</code> class provides methods for getting
 * and setting the contents of the MIDI message.
 * <p>
 * A number of <code>ShortMessage</code> methods have integer parameters by
 * which you specify a MIDI status or data byte. If you know the numeric value,
 * you can express it directly. For system common and system real-time messages,
 * you can often use the corresponding fields of <code>ShortMessage</code>, such
 * as {@link #SYSTEM_RESET SYSTEM_RESET}. For channel messages, the upper four
 * bits of the status byte are specified by a command value and the lower four
 * bits are specified by a MIDI channel number. To convert incoming MIDI data
 * bytes that are in the form of Java's signed bytes, you can use the
 * <A HREF="MidiMessage.html#integersVsBytes">conversion code</A> given in the
 * <code>{@link MidiMessage}</code> class description.
 *
 * @see SysexMessage
 * @see MetaMessage
 *
...
public class ShortMessage extends MidiMessage {
	...
	/**
	 * Constructs a new <code>ShortMessage</code>. The contents of the new
	 ...
	 *             if {@code status} does not specify a valid MIDI status byte
	 *             for a message that requires no data bytes
	 * @see #setMessage(int)
	 ...
	 * @throws InvalidMidiDataException
	 *             if the status byte or all data bytes belonging to the message
	 ...
	 *            the second data byte
	 * @throws InvalidMidiDataException
	 *             if the command value, channel value or all data bytes
	 ...
	 *            the MIDI status byte
	 * @throws InvalidMidiDataException
	 *             if <code>status</code> does not specify a valid MIDI status
	 *             byte for a message that requires no data bytes.
	 * @see #setMessage(int, int, int)
	 * @see #setMessage(int, int, int, int)
	 */
	public void setMessage(int status) throws InvalidMidiDataException {
		...
		if (dataLength != 0) {
			throw new InvalidMidiDataException(
					"Status byte; " + status + " requires " + dataLength + " data bytes");
		}
	 ...
	 *            the second data byte
	 * @throws InvalidMidiDataException
	 *             if the the status byte, or all data bytes belonging to the
	 *             message, do not specify a valid MIDI message.
	 * @see #setMessage(int, int, int, int)
	 * @see #setMessage(int)
	 */
	public void setMessage(int status, int data1, int data2) throws InvalidMidiDataException {
		...
		// set the length
		length = dataLength + 1;
		// re-allocate array if ShortMessage(byte[]) constructor gave array with
		// fewer elements
	 ...
	 *            the second data byte
	 * @throws InvalidMidiDataException
	 *             if the status byte or all data bytes belonging to the
	 *             message, do not specify a valid MIDI message
	 *
	 * @see #setMessage(int, int, int)
	 * @see #setMessage(int)
	...
	public void setMessage(int command, int channel, int data1, int data2)
		...
		if (command >= 0xF0 || command < 0x80) {
			throw new InvalidMidiDataException(
					"command out of range: 0x" + Integer.toHexString(command));
		}
	 ...
	 * @throws InvalidMidiDataException
	 *             if the <code>status</code> argument does not represent the
