...
/**
 * A <code>SysexMessage</code> object represents a MIDI system exclusive
 * message.
 * <p>
 ...
 * be stored in the data array of a <code>SysexMessage</code> as follows: the
 * system exclusive message status byte (0xF0 or 0xF7), all message data bytes,
 * and finally the end-of-exclusive flag (0xF7). The length reported by the
 * <code>SysexMessage</code> object is therefore the length of the system
 * exclusive data plus two: one byte for the status byte and one for the
 * end-of-exclusive flag.
 * <p>
 * As dictated by the Standard MIDI Files specification, two status byte values
 * are legal for a <code>SysexMessage</code> read from a MIDI file:
 * <ul>
 ...
 * <code>SysexMessages</code>. In this case, the length of the system exclusive
 * data is not known in advance; the end of the system exclusive data is marked
 * by an end-of-exclusive flag (0xF7) in the MIDI wire byte stream.
 * <ul>
 * <li>0xF0: System Exclusive message (same as in MIDI wire protocol)</li>
 * <li>0xF7: End of Exclusive (EOX)</li>
 * </ul>
 * The first <code>SysexMessage</code> object containing data for a particular
 * system exclusive message should have the status value 0xF0. If this message
 * contains all the system exclusive data for the message, it should end with
 * the status byte 0xF7 (EOX). Otherwise, additional system exclusive data
 * should be sent in one or more <code>SysexMessages</code> with a status value
 * of 0xF7. The <code>SysexMessage</code> containing the last of the data for
 * the system exclusive message should end with the value 0xF7 (EOX) to mark the
 * end of the system exclusive message.
 * <p>
 * If system exclusive data from <code>SysexMessages</code> objects is being
 ...
 * <code>SysexMessage</code> contains continuing system exclusive data should
 * not be propagated via MIDI wire protocol.
 *
 * @author David Rivas
 * @author Kara Kytle
...
public class SysexMessage extends MidiMessage {
	 ...
	 * to <code>null</code> and are set explicitly by {@link #setMessage(int,
	 * byte[], int, long) setMessage}.
	 */
	// protected byte[] data = null;

	/**
	 * Constructs a new <code>SysexMessage</code>. The contents of the new
	 * message are guaranteed to specify a valid MIDI message. Subsequently, you
	 * may set the contents of the message using one of the
	 * <code>setMessage</code> methods.
	 * 
	 * @see #setMessage
	 */
	public SysexMessage() {
	 ...
	 * @param length
	 *            the length of the valid message data in the array, including
	 ...
	 * @param length
	 *            the length of the valid message data in the array; it should
	 *            be non-negative and less than or equal to {@code data.length}
	 * @throws InvalidMidiDataException
	 *             if the parameter values do not specify a valid MIDI meta
	 *             message.
	 * @see #setMessage(byte[], int)
	...
	public void setMessage(byte[] data, int length) throws InvalidMidiDataException {
		...
		if ((status != 0xF0) && (status != 0xF7)) {
			throw new InvalidMidiDataException(
					"Invalid status byte for sysex message: 0x" + Integer.toHexString(status));
		}
	 ...
	 * @throws InvalidMidiDataException
	 *             if the status byte is invalid for a sysex message
	 */
