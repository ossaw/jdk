 ...
 * "channel voice" or "channel mode" message as defined by the MIDI
 * specification. However, <code>MidiChannel</code> adds some "get" methods that
 * retrieve the value most recently set by one of the standard MIDI channel
 * messages. Similarly, methods for per-channel solo and mute have been added.
 * <p>
 * A <code>{@link Synthesizer}</code> object has a collection of
 * <code>MidiChannels</code>, usually one for each of the 16 channels prescribed
 * by the MIDI 1.0 specification. The <code>Synthesizer</code> generates sound
 * when its <code>MidiChannels</code> receive <code>noteOn</code> messages.
 * <p>
 ...
 * here. The specification is titled <code>MIDI Reference:
 * The Complete MIDI 1.0 Detailed Specification</code>, and is published by the
 * MIDI Manufacturer's Association (<a href = http://www.midi.org>
 * http://www.midi.org</a>).
 * <p>
...
public interface MidiChannel {
     ...
     * controls the note's volume and/or brightness. If <code>velocity</code> is
     ...
     * internals of the <code>Instrument</code>. If the Hold Pedal (a
     ...
     * by <code>setPolyPressure</code>. More commonly, it is a measurement of a
    ...
    /**
     * Obtains the channel's keyboard pressure. If the device does not support
     * setting channel pressure, this method always returns 0. Calling
     ...
     * <code>Instrument</code> reacts to a controller change may be specific to
     * the <code>Instrument</code>.
     * <p>
     ...
     * of a 7-bit controller is set completely by the <code>value</code>
     * argument. An additional set of controllers provide 14-bit precision by
     * using two controller numbers, one for the most significant 7 bits and
     * another for the least significant 7 bits. Controller numbers 0 through 31
     ...
     * <code>programChange</code> was successful, use <code>getProgram</code>
     * and <code>getController</code>. Since banks are changed by way of control
     * changes, you can verify the current bank with the following statement:
     * 
     * <pre>
     ...
     * by the internals of the <code>Instrument</code>. If the Hold Pedal
     * controller (see {@link #controlChange(int, int) controlChange}) is down,
     * the effect of this method is deferred until the pedal is released.
     *
     * @see #allSoundOff
    ...
    /**
     * Sets the mute state for this channel. A value of <code>true</code> means
     * the channel is to be muted, <code>false</code> means the channel can
     * sound (if other channels are not soloed).
     * <p>
    ...
    /**
     * Sets the solo state for this channel. If <code>solo</code> is
     * <code>true</code> only this channel and other soloed channels will sound.
