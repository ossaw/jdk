...
/**
 * A <code>Sequence</code> is a data structure containing musical information
 * (often an entire song or composition) that can be played back by a
 * <code>{@link Sequencer}</code> object. Specifically, the
 * <code>Sequence</code> contains timing information and one or more tracks.
 * Each <code>{@link Track track}</code> consists of a series of MIDI events
 * (such as note-ons, note-offs, program changes, and meta-events). The
 ...
 * into an input stream and invoking one of the <code>getSequence</code> methods
 * of {@link MidiSystem}. A sequence can also be built from scratch by adding
 * new <code>Tracks</code> to an empty <code>Sequence</code>, and adding
...
public class Sequence {
	 ...
	 * MIDI timing types. For tempo-based timing, <code>divisionType</code> is
	 * PPQ (pulses per quarter note) and the resolution is specified in ticks
	 * per beat. For SMTPE timing, <code>divisionType</code> specifies the
	 * number of frames per second and the resolution is specified in ticks per
	 * frame. The sequence will contain no initial tracks. Tracks may be added
	 * to or removed from the sequence using <code>{@link #createTrack}</code>
	 ...
	 * <code>divisionType</code> is PPQ (pulses per quarter note) and the
	 * resolution is specified in ticks per beat. For SMTPE timing,
	 * <code>divisionType</code> specifies the number of frames per second and
	 * the resolution is specified in ticks per frame. The sequence will be
	 * initialized with the number of tracks specified by <code>numTracks</code>
	 * . These tracks are initially empty (i.e. they contain only the meta-event
	 * End of Track). The tracks may be retrieved for editing using the
	 * <code>{@link #getTracks}</code> method. Additional tracks may be added,
	 * or existing tracks removed, using <code>{@link #createTrack}</code> and
	 ...
	 * <code>{@link #getTracks}</code>. Tracks may be removed from the sequence
	 * using <code>{@link #deleteTrack}</code>.
	 * 
	 ...
	 * may be used to load the required <code>{@link Instrument}</code> objects
