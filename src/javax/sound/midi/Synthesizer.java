 ...
 * the <code>Synthesizer</code> object. Many <code>Synthesizer</code>s support
 * <code>Receivers</code>, through which MIDI events can be delivered to the
 * <code>Synthesizer</code>. In such cases, the <code>Synthesizer</code>
 * typically responds by sending a corresponding message to the appropriate
 * <code>MidiChannel</code>, or by processing the event itself if the event
 * isn't one of the MIDI channel messages.
 * <p>
 * The <code>Synthesizer</code> interface includes methods for loading and
 * unloading instruments from soundbanks. An instrument is a specification for
 ...
 * program number (via the instrument's <code>Patch</code> object). Different
 * <code>Synthesizer</code> classes might implement different sound-synthesis
 ...
 * given synthesizer, invoke the {@link #isSoundbankSupported(Soundbank)
 * isSoundbankSupported} method of <code>Synthesizer</code>.
 * <p>
 ...
 * location specified by its <code>Patch</code> object. Loading does not
 * necessarily mean that subsequently played notes will immediately have the
 * sound of this newly loaded instrument. For the instrument to play notes, one
 * of the synthesizer's <code>MidiChannel</code> objects must receive (or have
 * received) a program-change message that causes that particular instrument's
...
public interface Synthesizer extends MidiDevice {
	 ...
	 * might be <code>null</code>, so you should check each element before using
	 * it.
	 * 
	 ...
	 * this class of <code>Synthesizer</code> does not provide voice
	 ...
	 * loaded into the patch location specified by its <code>Patch</code>
	 * object, so that if a program-change message is received (or has been
	 * received) that causes that patch to be selected, subsequent notes will be
	 * played using the sound of <code>instrument</code>. If the specified
	 * instrument is already loaded, this method does nothing and returns
	 * <code>true</code>.
	 * <p>
	 * The instrument must be part of a soundbank that this
	 * <code>Synthesizer</code> supports. (To make sure, you can use the
	 ...
	 *             if this <code>Synthesizer</code> doesn't support the
	...
	/**
	 * Remaps an instrument. Instrument <code>to</code> takes the place of
	 * instrument <code>from</code>.<br>
	 * For example, if <code>from</code> was located at bank number 2, program
	 * number 11, remapping causes that bank and program location to be occupied
	 * instead by <code>to</code>.<br>
	 * If the function succeeds, instrument <code>from</code> is unloaded.
	 * <p>
	 * To cancel the remapping reload instrument <code>from</code> by invoking
	 * one of {@link #loadInstrument}, {@link #loadInstruments} or
	 ...
	 * @throws IllegalArgumentException
	 *             if instrument <code>from</code> or instrument <code>to</code>
	 *             aren't supported by synthesizer or if instrument
	 *             <code>to</code> is not loaded
	 * @throws NullPointerException
	 *             if <code>from</code> or <code>to</code> parameters have null
	 *             value
	 * @see #loadInstrument
	 * @see #loadInstruments
	 * @see #loadAllInstruments
	 ...
	 * <code>getLoadedInstruments()</code>. Nor does the method indicate all the
	...
	/**
	 * Loads onto the <code>Synthesizer</code> all instruments contained in the
	 * specified <code>Soundbank</code>.
	 * 
	 * @param soundbank
	 *            the <code>Soundbank</code> whose are instruments are to be
	 *            loaded
	 ...
	 *         insufficient memory)
	 * @throws IllegalArgumentException
	 *             if the requested soundbank is incompatible with this
	 *             synthesizer.
	 * @see #isSoundbankSupported
	 * @see #loadInstrument
	 * @see #loadInstruments
	 ...
	 * indicates a bank and program number; the <code>Instrument</code> that has
	 * the matching <code>Patch</code> is loaded into that bank and program
	 * location.
	 * 
	 * @param soundbank
	 *            the <code>Soundbank</code> containing the instruments to load
	 * @param patchList
	 ...
	 *             if the receiver is cannot be opened, usually because the MIDI
	 *             device is in use by another application.
	 * @throws SecurityException
	 *             if the receiver cannot be opened due to security
	 *             restrictions.
	 */
