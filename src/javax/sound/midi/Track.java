 ...
 * by a <code>{@link Sequencer}</code>: sequencers play sequences, which contain
 ...
 * in the sequence containing the track. A given <code>Track</code> is
 * considered to belong to the particular <code>{@link Sequence}</code> that
 * maintains its timing. For this reason, a new (empty) track is created by
 * calling the <code>{@link Sequence#createTrack}</code> method, rather than by
 * directly invoking a <code>Track</code> constructor.
 * <p>
 * The <code>Track</code> class provides methods to edit the track by adding or
 * removing <code>MidiEvent</code> objects from it. These operations keep the
 * event list in the correct time order. Methods are also included to obtain the
...
public class Track {
        ...
        synchronized (eventsList) {
            ...
            if (!set.contains(event)) {
                ...
                if (lastEvent != eotEvent) {
                    // if there is no eot event, add our immutable instance
                    // again
                    if (lastEvent != null) {
                    ...
                    // we needn't check for a duplicate of eotEvent in
                    // "eventsList",
     ...
     * <code>Sequence</code> containing this track, and also by the tempo of the
