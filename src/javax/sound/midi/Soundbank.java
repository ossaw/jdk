...
/**
 * A <code>Soundbank</code> contains a set of <code>Instruments</code> that can
 * be loaded into a <code>Synthesizer</code>. Note that a Java Sound
 * <code>Soundbank</code> is different from a MIDI bank. MIDI permits up to
 * 16383 banks, each containing up to 128 instruments (also sometimes called
 * programs, patches, or timbres). However, a <code>Soundbank</code> can contain
 * 16383 times 128 instruments, because the instruments within a
 * <code>Soundbank</code> are indexed by both a MIDI program number and a MIDI
 * bank number (via a <code>Patch</code> object). Thus, a <code>Soundbank</code>
 ...
 * typically implement the <code>getResources()</code> method to provide access
 * to these recorded segments. This is optional, however; the method can return
 * an zero-length array if the synthesis technique doesn't use sampled sound (FM
 * synthesis and physical modeling are examples of such techniques), or if it
 * does but the implementor chooses not to make the samples accessible.
 *
