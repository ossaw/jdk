...
/**
 * A <code>SoundbankResource</code> represents any audio resource stored in a
 * <code>{@link Soundbank}</code>. Common soundbank resources include:
 * <ul>
 ...
 * instrument has a <code>Patch</code> which specifies the MIDI program and bank
 * by which it may be referenced in MIDI messages. Instrument information may be
 ...
 * <code>{@link Synthesizer}</code> to synthesize sound in response to MIDI
 * commands, or extracted for use by an application. (The terminology reflects
 * musicians' use of the word "sample" to refer collectively to a series of
 * contiguous audio samples or frames, rather than to a single, instantaneous
 * sample.) The data class for an audio sample will be an object that
 * encapsulates the audio sample data itself and information about how to
 * interpret it (the format of the audio data), such as an
 * <code>{@link javax.sound.sampled.AudioInputStream}</code>.</li>
 * <li>Embedded sequences. A sound bank may contain built-in song data stored in
 * a data object such as a <code>{@link Sequence}</code>.
 * </ul>
 * <p>
 ...
 * necessarily a one-to-one correspondence between the <code>Instruments</code>
 * and samples in a <code>Soundbank</code>. A single <code>Instrument</code> can
 * use multiple SoundbankResources (typically for notes of dissimilar pitch or
