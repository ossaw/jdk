 ...
 * synthesizing.
 * <p>
 * In Java Sound, however, a <code>Synthesizer</code> class can expose the
 * contents of its voices through its {@link Synthesizer#getVoiceStatus()
 * getVoiceStatus()} method. This behavior is recommended but optional;
 * synthesizers that don't expose their voice allocation simply return a
 * zero-length array. A <code>Synthesizer</code> that does report its voice
 * status should maintain this information at all times for all of its voices,
 * whether they are currently sounding or not. In other words, a given type of
 * <code>Synthesizer</code> always has a fixed number of voices, equal to the
 ...
 * program number, MIDI note number, and MIDI volume. All of these can change
 * during the course of a voice. While the voice is inactive, each of these
 * fields has an unspecified value, so you should check the active field first.
 *
