...
/**
 * A <code>Patch</code> object represents a location, on a MIDI synthesizer,
 * into which a single instrument is stored (loaded). Every
 * <code>Instrument</code> object has its own <code>Patch</code> object that
 * specifies the memory location into which that instrument should be loaded.
 * The location is specified abstractly by a bank index and a program number
 * (not by any scheme that directly refers to a specific address or offset in
 * RAM). This is a hierarchical indexing scheme: MIDI provides for up to 16384
 ...
 * equivalent is the {@link MidiChannel#programChange(int, int)
 * programChange(int, int)} method of <code>MidiChannel</code>.
 *
 * @see Instrument
