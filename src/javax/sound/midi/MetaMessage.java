...
/**
 * A <code>MetaMessage</code> is a <code>{@link MidiMessage}</code> that is not
 * meaningful to synthesizers, but that can be stored in a MIDI file and
 * interpreted by a sequencer program. (See the discussion in the
 * <code>MidiMessage</code> class description.) The Standard MIDI Files
 * specification defines various types of meta-events, such as sequence number,
 * lyric, cue point, and set tempo. There are also meta-events for such
 * information as lyrics, copyrights, tempo indications, time and key
 ...
 * represents a system reset message. In MIDI files, this same status value
 * denotes a <code>MetaMessage</code>. The types of meta-message are
 * distinguished from each other by the first byte that follows the status byte
 * <code>0xFF</code>. The subsequent bytes are data bytes. As with system
 * exclusive messages, there are an arbitrary number of data bytes, depending on
...
public class MetaMessage extends MidiMessage {
	...
	/**
	 * Status byte for <code>MetaMessage</code> (0xFF, or 255), which is used in
	 * MIDI files. It has the same value as SYSTEM_RESET, which is used in the
	...
	/**
	 * Constructs a new <code>MetaMessage</code>. The contents of the message
	 * are not set here; use {@link #setMessage(int, byte[], int) setMessage} to
	 * set them subsequently.
	 */
	public MetaMessage() {
	 ...
	 * @param length
	 *            an amount of bytes in the {@code data} byte array; it should
	...
	/**
	 * Sets the message parameters for a <code>MetaMessage</code>. Since only
	 * one status byte value, <code>0xFF</code>, is allowed for meta-messages,
	 * it does not need to be specified here. Calls to
	 * <code>{@link MidiMessage#getStatus getStatus}</code> return
	 * <code>0xFF</code> for all meta-messages.
	 * <p>
	 * The <code>type</code> argument should be a valid value for the byte that
	 * follows the status byte in the <code>MetaMessage</code>. The
	 * <code>data</code> argument should contain all the subsequent bytes of the
	 * <code>MetaMessage</code>. In other words, the byte that specifies the
	...
	public void setMessage(int type, byte[] data, int length) throws InvalidMidiDataException {
		...
		this.data[0] = (byte) META; // status value for MetaMessages (meta
									// events)
