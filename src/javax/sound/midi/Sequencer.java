 ...
 * <code>{@link Sequence sequence}</code> is known as a <em>sequencer</em>. A
 * MIDI sequence contains lists of time-stamped MIDI data, such as might be read
 * from a standard MIDI file. Most sequencers also provide functions for
 * creating and editing sequences.
 * <p>
 * The <code>Sequencer</code> interface includes methods for the following basic
 * MIDI sequencer operations:
 * <ul>
 ...
 * <li>synchronizing playback to an internal clock or to received MIDI messages
 * </li>
...
public interface Sequencer extends MidiDevice {
	 ...
	 *
	 * <p>
	 * This method can be called even if the <code>Sequencer</code> is closed.
	 *
	 * @param sequence
	 ...
	 * @throws InvalidMidiDataException
	 *             if invalid data is encountered in the stream, or the stream
	 *             is not supported.
	 */
	 ...
	 * This method can be called even if the <code>Sequencer</code> is closed.
	 *
	 * @return the current sequence, or <code>null</code> if no sequence is
	 *         currently set.
	 */
	 ...
	 * repetitions set with <code>setLoopCount</code>. After that, or if the
	 * loop count is 0, playback will continue to play to the end of the
	 * sequence.
	 *
	 * <p>
	 ...
	 *             if the <code>Sequencer</code> is closed.
	 *
	 ...
	 * <code>false</code>. The Sequencer starts running when either
	 * <code>{@link #start}</code> or <code>{@link #startRecording}</code> is
	 * called. <code>isRunning</code> then returns <code>true</code> until
	 * playback of the sequence completes or <code>{@link #stop}</code> is
	 * called.
	 * 
	 * @return <code>true</code> if the Sequencer is running, otherwise
	 *         <code>false</code>
	 */
	 ...
	 * <code>false</code>. The Sequencer begins recording when
	 * <code>{@link #startRecording}</code> is called, and then returns
	 * <code>true</code> until <code>{@link #stop}</code> or
	 * <code>{@link #stopRecording}</code> is called.
	 * 
	 * @return <code>true</code> if the Sequencer is recording, otherwise
	 *         <code>false</code>
	 */
	 ...
	 *            specified for the channel value, the track will receive data
	 *            from all channels.
	 ...
	 * 
	 * @param track
	 *            the track to disable for recording, or <code>null</code> to
	 ...
	 * <code>{@link #getTempoInMPQ}</code> and
	 * <code>{@link #getTempoInBPM}</code>. Those values indicate the tempo
	 * prior to scaling.
	 * <p>
	 ...
	 * the timing resolution stored in the <code>{@link Sequence}</code>.)
	 *
	 * @return current tick
	 * @see #setTickPosition
	 */
	 ...
	 * clock, or MIDI time code, depending on the value of <code>sync</code>.
	 * The <code>sync</code> argument must be one of the supported modes, as
	 * returned by <code>{@link #getMasterSyncModes}</code>.
	 *
	 * @param sync
	 ...
	 * <code>sync</code> argument must be one of the supported modes, as
	 * returned by <code>{@link #getSlaveSyncModes}</code>.
	 *
	 * @param sync
	 ...
	 * call to <code>{@link #getTrackSolo}</code>.
	 *
	 * @param track
	 *            the track number. Tracks in the current sequence are numbered
	 *            from 0 to the number of tracks in the sequence minus 1.
	 ...
	 * types. The types are specified by the <code>controllers</code> argument,
	 * which should contain an array of MIDI controller numbers. (Each number
	 * should be between 0 and 127, inclusive. See the MIDI 1.0 Specification
	 * for the numbers that correspond to various types of controllers.)
	 * <p>
	 ...
	 *
	 * @param listener
	 *            the controller event listener to add to the list of registered
	 *            listeners
	 * @param controllers
	 *            the MIDI controller numbers for which change notification is
	 *            requested
	 ...
	 * controller event. The <code>controllers</code> argument is an array of
	 * MIDI numbers corresponding to the controllers for which the listener
	 * should no longer receive change notifications. To completely remove this
	 * listener from the list of registered listeners, pass in <code>null</code>
	 * for <code>controllers</code>. The returned array contains the MIDI
	 * controller numbers for which the listener will now receive events. The
	 * array has a length of 0 if the listener will not receive change
	 * notifications for any controllers.
	 *
	 * @param listener
	 *            old listener
	 * @param controllers
	 *            the MIDI controller numbers for which change notification
	 *            should be cancelled, or <code>null</code> to cancel for all
	 *            controllers
	 * @return the numbers of all the MIDI controllers whose changes will now be
	 *         reported to the specified listener
	 *
	 ...
	 *             because it falls outside the sequence's duration or because
	 ...
	 *
	 * @param tick
	 *            the loop's ending position, in MIDI ticks (zero-based), or -1
	 *            to indicate the final tick
	 * @throws IllegalArgumentException
	 *             if the requested loop point cannot be set, usually because it
	 *             falls outside the sequence's duration or because the ending
	 *             point is before the starting point
	 *
	 * @see #setLoopStartPoint
	 * @see #setLoopCount
	 * @see #getLoopEndPoint
	 ...
	 * loop start point <code>count</code> times, after which playback will
	 ...
	 * <p>
	 * A <code>count</code> value of 0 disables looping: playback will continue
	 * at the loop end point, and it will not loop back to the loop start point.
	 ...
	 *
	 * @throws IllegalArgumentException
	 *             if <code>count</code> is negative and not equal to
	...
	/**
	 * A <code>SyncMode</code> object represents one of the ways in which a MIDI
	 ...
	 * sequencer: <code>INTERNAL_CLOCK</code>, <code>MIDI_SYNC</code>, and
	 ...
	 * messages or MIDI time code (MTC) messages, respectively. These two modes
	 * can also be used as slave modes, in which case the sequencer sends the
	 * corresponding types of MIDI messages to its receiver (whether or not the
	 * sequencer is also receiving them from a master). A fourth mode,
	 * <code>NO_SYNC</code>, is used to indicate that the sequencer should not
	 * control its receiver's timing.
	 *
	...
	public static class SyncMode {
		...
		/**
		 * Determines whether two objects are equal. Returns <code>true</code>
