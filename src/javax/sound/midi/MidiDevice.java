...
/**
 * <code>MidiDevice</code> is the base interface for all MIDI devices. Common
 * devices include synthesizers, sequencers, MIDI input ports, and MIDI output
 * ports.
 *
 * <p>
 * A <code>MidiDevice</code> can be a transmitter or a receiver of MIDI events,
 * or both. Therefore, it can provide {@link Transmitter} or {@link Receiver}
 ...
 *
 * <p>
 * A <code>MidiDevice</code> can be opened and closed explicitly as well as
 * implicitly. Explicit opening is accomplished by calling {@link #open},
 * explicit closing is done by calling {@link #close} on the
 * <code>MidiDevice</code> instance. If an application opens a
 * <code>MidiDevice</code> explicitly, it has to close it explicitly to free
 * system resources and enable the application to exit cleanly. Implicit opening
 * is done by calling {@link javax.sound.midi.MidiSystem#getReceiver
 * MidiSystem.getReceiver} and {@link javax.sound.midi.MidiSystem#getTransmitter
 * MidiSystem.getTransmitter}. The <code>MidiDevice</code> used by
 * <code>MidiSystem.getReceiver</code> and
 * <code>MidiSystem.getTransmitter</code> is implementation-dependant unless the
 * properties <code>javax.sound.midi.Receiver</code> and
 * <code>javax.sound.midi.Transmitter</code> are used (see the description of
 * properties to select default providers in {@link javax.sound.midi.MidiSystem}
 * ). A <code>MidiDevice</code> that was opened implicitly, is closed implicitly
 * by closing the <code>Receiver</code> or <code>Transmitter</code> that
 * resulted in opening it. If more than one implicitly opening
 * <code>Receiver</code> or <code>Transmitter</code> were obtained by the
 * application, the device is closed after the last <code>Receiver</code> or
 * <code>Transmitter</code> has been closed. On the other hand, calling
 * <code>getReceiver</code> or <code>getTransmitter</code> on the device
 * instance directly does not open the device implicitly. Closing these
 * <code>Transmitter</code>s and <code>Receiver</code>s does not close the
 * device implicitly. To use a device with <code>Receiver</code>s or
 * <code>Transmitter</code>s obtained this way, the device has to be opened and
 * closed explicitly.
 *
 * <p>
 * If implicit and explicit opening and closing are mixed on the same
 * <code>MidiDevice</code> instance, the following rules apply:
 *
 * <ul>
 ...
 *
 * <p>
 * A <code>MidiDevice</code> includes a <code>{@link MidiDevice.Info}</code>
 * object to provide manufacturer information and so on.
 *
 * @see Synthesizer
 * @see Sequencer
 * @see Receiver
...
public interface MidiDevice extends AutoCloseable {
	 ...
	 *
	 * <p>
	 * All <code>Receiver</code> and <code>Transmitter</code> instances open
	 * from this device are closed. This includes instances retrieved via
	 ...
	 *
	 * <p>
	 * Obtaining a <code>Receiver</code> with this method does not open the
	 * device. To be able to use the device, it has to be opened explicitly by
	 * calling {@link #open}. Also, closing the <code>Receiver</code> does not
	 ...
	 *
	 * <p>
	 * Obtaining a <code>Transmitter</code> with this method does not open the
	 * device. To be able to use the device, it has to be opened explicitly by
	 * calling {@link #open}. Also, closing the <code>Transmitter</code> does
	 * not close the device. It has to be closed explicitly by calling
	 * {@link #close}.
	 *
	...
	/**
	 * A <code>MidiDevice.Info</code> object contains assorted data about a
	 * <code>{@link MidiDevice}</code>, including its name, the company who
	...
	public static class Info {
		...
		/**
		 * Reports whether two objects are equal. Returns <code>true</code> if
