 ...
 * not only the standard MIDI messages that a synthesizer can respond to, but
 * also "meta-events" that can be used by sequencer programs. There are
 * meta-events for such information as lyrics, copyrights, tempo indications,
 * time and key signatures, markers, etc. For more information, see the Standard
 * MIDI Files 1.0 specification, which is part of the Complete MIDI 1.0 Detailed
 ...
 * the {@link #getStatus()} method of <code>MidiMessage</code> returns MIDI
 * status bytes as integers. If you are processing MIDI data that originated
 * outside Java Sound and now is encoded as signed bytes, the bytes can can be
 ...
 * as the first argument to ShortMessage's {@link ShortMessage#setMessage(int)
 * setMessage(int)} method, you can express it as 254 or 0xFE.
 *
 * @see Track
 * @see Sequence
 * @see Receiver
...
public abstract class MidiMessage implements Cloneable {
    ...
    /**
     * Constructs a new <code>MidiMessage</code>. This protected constructor is
     ...
     * in bytes is reported by the <code>{@link #getLength}</code> method.
     *
     * @return the byte array containing the complete <code>MidiMessage</code>
     *         data
     */
     ...
     * <a href="#integersVsBytes">discussion</a> in the <code>MidiMessage</code>
     * class description.
     *
