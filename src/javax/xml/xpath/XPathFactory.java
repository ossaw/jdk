 ...
 * attempt to recursively invoke a <code>newInstance</code> method, even from
 * the same thread.
 *
...
public abstract class XPathFactory {
	...
	/**
	 * <p>
	 * Protected constructor as {@link #newInstance()} or
	 * {@link #newInstance(String uri)} or
	 * {@link #newInstance(String uri, String factoryClassName, ClassLoader classLoader)}
	 ...
	 * </p>
	 * <ol>
	 * <li>If the system property {@link #DEFAULT_PROPERTY_NAME} + ":uri" is
	 * present, where uri is the parameter to this method, then its value is
	 * read as a class name. The method will try to create a new instance of
	 * this class by using the class loader, and returns it if it is
	 * successfully created.</li>
	 * <li>${java.home}/lib/jaxp.properties is read and the value associated
	 * with the key being the system property above is looked for. If present,
	 * the value is processed just like above.</li>
	 ...
	 * {@link #isObjectModelSupported(String objectModel)}. The first service
	 * provider found that supports the specified object model is returned. <br>
	 * In case of {@link java.util.ServiceConfigurationError} an
	 * {@link XPathFactoryConfigurationException} will be thrown.</li>
	 * <li>Platform default <code>XPathFactory</code> is located in a platform
	 * specific way. There must be a platform default XPathFactory for the W3C
	 * DOM, i.e. {@link #DEFAULT_OBJECT_MODEL_URI}.</li>
	 * </ol>
	 * <p>
	 ...
	 * escaped in a property file, so make sure the URIs are properly escaped in
	 * it. For example:
	 * </p>
	 * 
	 * <pre>
	 ...
	 *             If the specified object model is unavailable, or if there is
	 *             a configuration error.
	 * @throws NullPointerException
	 *             If <code>uri</code> is <code>null</code>.
	 * @throws IllegalArgumentException
	 *             If <code>uri</code> is <code>null</code> or
	...
	/**
	 * <p>
	 * Obtain a new instance of a <code>XPathFactory</code> from a factory class
	 * name. <code>XPathFactory</code> is returned if specified factory class
	 * supports the specified object model. This function is useful when there
	 ...
	 *            implementation of <code>javax.xml.xpath.XPathFactory</code>.
	 *
	 * @param classLoader
	 ...
	 *             factory class cannot be loaded, instantiated or the factory
	 *             class does not support the object model specified in the
	 ...
	 * the feature is <code>true</code>, any reference to an external function
	 * is an error. Under these conditions, the implementation must not call the
	 * {@link XPathFunctionResolver} and must throw an
	...
	/**
	 * <p>
	 * Return a new <code>XPath</code> using the underlying object model
	 * determined when the <code>XPathFactory</code> was instantiated.
	 * </p>
	 *
