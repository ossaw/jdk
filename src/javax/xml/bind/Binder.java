 ...
 * <li>{@link #unmarshal(Object)} XML infoset view to JAXB view. (Note to
 * conserve resources, it is possible to only unmarshal a subtree of the XML
 * infoset view to the JAXB view.)</li>
 * <li>application access/updates JAXB view of XML document.</li>
 * <li>{@link #updateXML(Object)} synchronizes modifications to JAXB view back
 * into the XML infoset view. Update operation preserves as much of original XML
 * infoset as possible (i.e. comments, PI, ...)</li>
 * </ul>
 *
 * <p>
 ...
 *
 * <p>
 * The template parameter, <code>XmlNode</code>, is the root interface/class for
 * the XML infoset preserving representation. A Binder implementation is
 * required to minimally support an <code>XmlNode</code> value of
 * <code>org.w3c.dom.Node.class</code>. A Binder implementation can support
...
public abstract class Binder<XmlNode> {
	 ...
	 *
	 * <p>
	 * This method is similar to {@link Unmarshaller#unmarshal(Node)} with the
	 * addition of maintaining the association between XML nodes and the
	 * produced JAXB objects, enabling future update operations,
	 * {@link #updateXML(Object, Object)} or {@link #updateJAXB(Object)}.
	 *
	 * <p>
	 * When {@link #getSchema()} is non-null, <code>xmlNode</code> and its
	 * descendants is validated during this operation.
	 *
	 * <p>
	 * This method throws {@link UnmarshalException} when the Binder's
	 * {@link JAXBContext} does not have a mapping for the XML element name or
	 * the type, specifiable via <tt>@xsi:type</tt>, of <tt>xmlNode</tt> to a
	 * JAXB mapped class. The method {@link #unmarshal(Object, Class)} enables
	 * an application to specify the JAXB mapped class that the <tt>xmlNode</tt>
	 * should be mapped to.
	 *
	 * @param xmlNode
	...
	/**
	 * Unmarshal XML root element by provided <tt>declaredType</tt> to a JAXB
	 * object tree.
	 *
	 * <p>
	 * Implements <a href="Unmarshaller.html#unmarshalByDeclaredType">Unmarshal
	 * by Declared Type</a>
	 *
	 * <p>
	 * This method is similar to {@link Unmarshaller#unmarshal(Node, Class)}
	 * with the addition of maintaining the association between XML nodes and
	 * the produced JAXB objects, enabling future update operations,
	 * {@link #updateXML(Object, Object)} or {@link #updateJAXB(Object)}.
	 *
	 * <p>
	 * When {@link #getSchema()} is non-null, <code>xmlNode</code> and its
	 ...
	 *
	 * <p>
	 * This method is similar to {@link Marshaller#marshal(Object, Node)} with
	 * the addition of maintaining the association between JAXB objects and the
	 * produced XML nodes, enabling future update operations such as
	 * {@link #updateXML(Object, Object)} or {@link #updateJAXB(Object)}.
	 *
	 * <p>
	 * When {@link #getSchema()} is non-null, the marshalled xml content is
	 ...
	 *
	 * <p>
	 * Once a JAXB object tree is associated with an XML fragment, this method
	 * enables navigation between the two trees.
	 *
	 * <p>
	 * An association between an XML element and a JAXB object is established by
	 * the bind methods and the update methods. Note that this association is
	 * partial; not all XML elements have associated JAXB objects, and not all
	 ...
	 * <p>
	 * An association between an XML element and a JAXB object is established by
	 * the unmarshal, marshal and update methods. Note that this association is
	 * partial; not all XML elements have associated JAXB objects, and not all
	 * JAXB objects have associated XML elements.
	 *
	 ...
	 *
	 * <p>
	 * For example, unknown elements/attributes in XML that were not bound to
	 * JAXB will be left untouched (whereas a marshalling operation would create
	 * a new tree that doesn't contain any of those.)
	 *
	 * <p>
	 * As a side-effect, this operation updates the association between XML
	 ...
	 *
	 * <p>
	 * This operation can be thought of as an "in-place" unmarshalling. The
	 * difference is that instead of creating a whole new JAXB tree, this
	 * operation updates an existing tree, reusing as much JAXB objects as
	 * possible.
	 *
	 * <p>
	 * As a side-effect, this operation updates the association between XML
	 ...
	 *         {@link #marshal(Object,Object)} or {@link #updateJAXB(Object)}
	 *         method invocation, but it maybe a different object, for example
	 ...
	 *
	 * <p>
	 * Calling this method with a null parameter will cause the Binder to revert
	 ...
	 * <tt>Binder</tt>. This method can only be used to set one of the standard
	 * JAXB defined unmarshal/marshal properties or a provider specific property
	 * for binder, unmarshal or marshal. Attempting to set an undefined property
	 * will result in a PropertyException being thrown. See
	 * <a href="Unmarshaller.html#supportedProps">Supported Unmarshal
	 * Properties</a> and <a href="Marshaller.html#supportedProps">Supported
	 ...
	 * <tt>Binder</tt>. This method can only be used to get one of the standard
	 * JAXB defined unmarshal/marshal properties or a provider specific property
	 * for binder, unmarshal or marshal. Attempting to get an undefined property
	 * will result in a PropertyException being thrown. See
	 * <a href="Unmarshaller.html#supportedProps">Supported Unmarshal
	 * Properties</a> and <a href="Marshaller.html#supportedProps">Supported
	 * Marshal Properties</a>.
