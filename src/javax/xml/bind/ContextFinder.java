...
class ContextFinder {
     ...
     * shouldn't be wrapped, throw the wrapped exception.
     */
    private static void handleInvocationTargetException(
            InvocationTargetException x) throws JAXBException {
    ...
    private static JAXBException handleClassCastException(Class originalType,
        ...
        return new JAXBException(Messages.format(Messages.ILLEGAL_CAST,
                // we don't care where the impl class is, we want to know where
                // JAXBContext lives in the impl
                // class' ClassLoader
                getClassClassLoader(originalType).getResource(
                        "javax/xml/bind/JAXBContext.class"), targetTypeURL));
    }

    /**
     * Create an instance of a class using the specified ClassLoader
     */
    static JAXBContext newInstance(String contextPath, String className,
        ...
        try {
            ...
            throw new JAXBException(Messages.format(Messages.PROVIDER_NOT_FOUND,
                    className), x);
            ...
            throw new JAXBException(Messages.format(
                    Messages.COULD_NOT_INSTANTIATE, className, x), x);
        }
    }

    static JAXBContext newInstance(String contextPath, Class spFactory,
            ClassLoader classLoader, Map properties) throws JAXBException {
        try {
            /*
             * javax.xml.bind.context.factory points to a class which has a
             * static method called 'createContext' that returns a
            ...
            if (context == null) {
                // try the old method that doesn't take properties. compatible
                // with 1.0.
                // it is an error for an implementation not to have both forms
            ...
            if (!(context instanceof JAXBContext)) {
                // the cast would fail, so generate an exception with a nice
                // message
                throw handleClassCastException(context.getClass(),
                        JAXBContext.class);
            }
            ...
            throw new JAXBException(Messages.format(
                    Messages.COULD_NOT_INSTANTIATE, spFactory, e), e);
        } catch (RuntimeException x) {
            // avoid wrapping RuntimeException to JAXBException,
            // because it indicates a bug in this code.
            throw x;
            ...
            // some other type of exception - just wrap it
            throw new JAXBException(Messages.format(
                    Messages.COULD_NOT_INSTANTIATE, spFactory, x), x);
        }
    }
     ...
     */
    static JAXBContext newInstance(Class[] classes, Map properties,
        ...
        if (logger.isLoggable(Level.FINE)) {
            ...
            logger.log(Level.FINE, "loaded {0} from {1}", new Object[] {
                    className, which(spi) });
        }
    ...
    static JAXBContext find(String factoryId, String contextPath,
        ...
        while (packages.hasMoreTokens()) {
            ...
            propFileName = new StringBuilder().append(packageName).append(
                    "/jaxb.properties");
            ...
            if (props != null) {
                if (props.containsKey(factoryId)) {
                    ...
                    return newInstance(contextPath, factoryClassName,
                            classLoader, properties);
                } else {
                    throw new JAXBException(Messages.format(
                            Messages.MISSING_PROPERTY, packageName, factoryId));
                }
            }
        }
        ...
        factoryClassName = AccessController.doPrivileged(new GetPropertyAction(
        ...
        if (jaxbContext != null) {
            ...
            return newInstance(contextPath, jaxbContext, classLoader,
                    properties);
        }
        ...
        try {
            final StringBuilder resource = new StringBuilder().append(
                    "META-INF/services/").append(jaxbContextFQCN);
            final InputStream resourceStream = classLoader.getResourceAsStream(
                    resource.toString());
            ...
            try {
                ...
                Logger.getLogger(ContextFinder.class.getName()).log(
        ...
        return newInstance(contextPath, PLATFORM_DEFAULT_FACTORY_CLASS,
                classLoader, properties);
    }
    ...
    static JAXBContext find(Class[] classes, Map properties)
        ...
        for (final Class c : classes) {
            // this classloader is used only to load jaxb.properties, so doing
            // this should be safe.
            ClassLoader classLoader = getClassClassLoader(c);
            Package pkg = c.getPackage();
            if (pkg == null)
                continue; // this is possible for primitives, arrays, and
                          // classes that are loaded by poorly implemented
                          // ClassLoaders
            String packageName = pkg.getName().replace('.', '/');

            // TODO: do we want to optimize away searching the same package?
            // org.Foo, org.Bar, com.Baz
            // classes from the same package might come from different class
            // loades, so it might be a bad idea
            ...
            if (props == null) {
                ...
                if (props.containsKey(JAXB_CONTEXT_FACTORY)) {
                    // trim() seems redundant, but adding to satisfy customer
                    // complaint
                    ...
                    throw new JAXBException(Messages.format(
                            Messages.MISSING_PROPERTY, packageName,
        ...
        // search for a system property second (javax.xml.bind.JAXBContext)
        logger.log(Level.FINE, "Checking system property {0}",
                JAXBContext.JAXB_CONTEXT_FACTORY);
        factoryClassName = AccessController.doPrivileged(new GetPropertyAction(
                JAXBContext.JAXB_CONTEXT_FACTORY));
        if (factoryClassName != null) {
        ...
        BufferedReader r = null;
        try {
            final String resource = new StringBuilder("META-INF/services/")
                    .append(jaxbContextFQCN).toString();
            ...
            if (resourceURL != null) {
                ...
                r = new BufferedReader(new InputStreamReader(resourceURL
                        .openStream(), "UTF-8"));
                factoryClassName = r.readLine();
                if (factoryClassName != null) {
                    factoryClassName = factoryClassName.trim();
    ...
    private static Class lookupJaxbContextUsingOsgiServiceLoader() {
        try {
            // Use reflection to avoid having any dependency on ServiceLoader
            // class
            Class target = Class.forName(
                    "com.sun.org.glassfish.hk2.osgiresourcelocator.ServiceLoader");
            Method m = target.getMethod("lookupProviderClasses", Class.class);
            Iterator iter = ((Iterable) m.invoke(null, JAXBContext.class))
                    .iterator();
            return iter.hasNext() ? (Class) iter.next() : null;
        } catch (Exception e) {
            logger.log(Level.FINE,
                    "Unable to find from OSGi: javax.xml.bind.JAXBContext");
            return null;
        }
    }
     ...
     * @param loader
     *               The ClassLoader to search. If this parameter is null, then
     *               the
    ...
    static URL which(Class clazz, ClassLoader loader) {
        ...
        String classnameAsResource = clazz.getName().replace('.', '/')
                + ".class";
    ...
    /**
     * When JAXB is in J2SE, rt.jar has to have a JAXB implementation. However,
     * rt.jar cannot have META-INF/services/javax.xml.bind.JAXBContext because
     * if it has, it will take precedence over any file that applications have
     * in their jar files.
     *
     * <p>
     * When the user bundles his own JAXB implementation, we'd like to use it,
     * and we want the platform default to be used only when there's no other
     * JAXB provider.
     *
     * <p>
    ...
    private static Class safeLoadClass(String className,
        ...
        try {
            ...
            // anyone can access the platform default factory class without
            // permission
