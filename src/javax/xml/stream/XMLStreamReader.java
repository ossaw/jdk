 ...
 * <p>
 * The XMLStreamReader is designed to iterate over XML using next() and
 * hasNext(). The data can be accessed using methods such as getEventType(),
 * getNamespaceURI(), getLocalName() and getText();
 * <p>
 * The <a href="#next()">next()</a> method causes the reader to read the next
 * parse event. The next() method returns an integer which identifies the type
 * of event just read.
 * <p>
 * The event type can be determined using
 * <a href="#getEventType()">getEventType()</a>.
 * <p>
 * Parsing events are defined as the XML Declaration, a DTD, start tag,
 * character data, white space, end tag, comment, or processing instruction. An
 * attribute or namespace event may be encountered at the root level of a
 * document as the result of a query operation.
 * <p>
 ...
 * the application. This information is provided through the property API on
 * this interface. The following two properties allow access to this
 * information: javax.xml.stream.notations and javax.xml.stream.entities. When
 * the current event is a DTD the following call will return a list of Notations
 * <code>List l = (List) getProperty("javax.xml.stream.notations");</code> The
 * following call will return a list of entity declarations:
 * <code>List l = (List) getProperty("javax.xml.stream.entities");</code> These
 * properties can only be accessed during a DTD event and are defined to return
...
public interface XMLStreamReader extends XMLStreamConstants {
     ...
     *         PROCESSING_INSTRUCTION, START_ELEMENT or
     *         END_ELEMENT
     * @throws NoSuchElementException
     *         if this is called when hasNext() returns
     *         false
     ...
     * contents. Throws an XMLStreamException if the cursor does not point to a
     * START_ELEMENT.
     * <p>
     ...
     * XMLStreamException to be thrown. The parent stream will always return the
     * same result from next() whatever is done to the sub stream.
     * 
     ...
     * @throws XMLStreamException
     *         if the underlying XML source is not
     *         well-formed
     * @throws IndexOutOfBoundsException
     *         if targetStart < 0 or > than the
     *         length of target
     * @throws IndexOutOfBoundsException
     *         if length < 0 or targetStart +
