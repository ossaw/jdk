...
/**
 * A single attachment to a <code>SOAPMessage</code> object. A
 * <code>SOAPMessage</code> object may contain zero, one, or many
 * <code>AttachmentPart</code> objects. Each <code>AttachmentPart</code> object
 * consists of two parts, application-specific content and associated MIME
 * headers. The MIME headers consists of name/value pairs that can be used to
 * identify and describe the content.
 * <p>
 * An <code>AttachmentPart</code> object must conform to certain standards.
 * <OL>
 * <LI>It must conform to <a href="http://www.ietf.org/rfc/rfc2045.txt"> MIME
 * [RFC2045] standards</a>
 ...
 * <code>AttachmentPart</code> object and MUST conform to [RFC2045]. The
 * following is an example of a Content-Type header:
 * 
 * <PRE>
 ...
 * <code>AttachmentPart</code> object, sets the header shown in the previous
 * example.
 * 
 * <PRE>
 ...
 * AttachmentPart</code> object. The content may be anything from a simple plain
 * text object to a complex XML document or image file.
 *
 * <p>
 * An <code>AttachmentPart</code> object is created with the method
 * <code>SOAPMessage.createAttachmentPart</code>. After setting its MIME
 * headers, the <code>AttachmentPart</code> object is added to the message that
 * created it with the method <code>SOAPMessage.addAttachmentPart</code>.
 *
 * <p>
 * The following code fragment, in which <code>m</code> is a
 * <code>SOAPMessage</code> object and <code>contentStringl</code> is a
 * <code>String</code>, creates an instance of <code>AttachmentPart</code>, sets
 * the <code>AttachmentPart</code> object with some content and header
 * information, and adds the <code>AttachmentPart</code> object to the
 ...
 * <code>AttachmentPart</code> instance to the same message.
 * <code>jpegData</code> is a binary byte buffer representing the jpeg file.
 * 
 * <PRE>
 ...
 * </PRE>
 * <p>
 * The <code>getContent</code> method retrieves the contents and header from an
 * <code>AttachmentPart</code> object. Depending on the
 * <code>DataContentHandler</code> objects present, the returned
 * <code>Object</code> can either be a typed Java object corresponding to the
 * MIME type or an <code>InputStream</code> object that contains the content as
 * bytes.
 * 
 * <PRE>
 ...
 * <code>AttachmentPart</code> object but does not affect its header
 * information.
 * 
 * <PRE>
 * ap1.clearContent();
...
public abstract class AttachmentPart {
	...
	/**
	 * Clears out the content of this <code>AttachmentPart</code> object. The
	 * MIME header portion is left untouched.
	 */
	 ...
	 * <code>DataContentHandler</code> object that is used to interpret the
	 * bytes and (2) the <code>Content-Type</code> given in the header.
	 * <p>
	 * For the MIME content types "text/plain", "text/html" and "text/xml", the
	 * <code>DataContentHandler</code> object does the conversions to and from
	 * the Java types corresponding to the MIME types. For other MIME types,the
	 * <code>DataContentHandler</code> object can return an
	 * <code>InputStream</code> object that contains the content data as raw
	 * bytes.
	 * <p>
	 * A SAAJ-compliant implementation must, as a minimum, return a
	 * <code>java.lang.String</code> object corresponding to any content stream
	 * with a <code>Content-Type</code> value of <code>text/plain</code>, a
	 * <code>javax.xml.transform.stream.StreamSource</code> object corresponding
	 * to a content stream with a <code>Content-Type</code> value of
	 * <code>text/xml</code>, a <code>java.awt.Image</code> object corresponding
	 * to a content stream with a <code>Content-Type</code> value of
	 * <code>image/gif</code> or <code>image/jpeg</code>. For those content
	 * types that an installed <code>DataContentHandler</code> object does not
	 * understand, the <code>DataContentHandler</code> object is required to
	 * return a <code>java.io.InputStream</code> object with the raw bytes.
	 *
	 ...
	 * of the raw attachment content is required then the
	 * {@link #getRawContentBytes} API should be used instead.
	 *
	...
	/**
	 * Gets the content of this <code>AttachmentPart</code> object as a byte[]
	 * array as if a call had been made to <code>getContent</code> and no
	 * <code>DataContentHandler</code> had been registered for the
	 * <code>content-type</code> of this <code>AttachmentPart</code>.
	 *
	 ...
	 * content of <code>AttachmentPart</code> as Base64 encoded character data,
	 * this method would base64 encode the raw bytes of the attachment and
	 * return.
	 *
	 ...
	 * header to the given type. The type of the <code>Object</code> should
	 * correspond to the value given for the <code>Content-Type</code>. This
	 * depends on the particular set of <code>DataContentHandler</code> objects
	 * in use.
	 *
	 *
	 * @param object
	 ...
	 * <P>
	 * A subsequent call to getSize() may not be an exact measure of the content
	 * size.
	 *
	 * @param content
	 ...
	 * <code>Content-Type</code> header to the value contained in
	 * <code>contentType</code>, This method would first decode the base64 input
	 * and write the resulting raw bytes to the attachment.
	 * <P>
	 * A subsequent call to getSize() may not be an exact measure of the content
	 * size.
	 *
	...
	/**
	 * Sets the given <code>DataHandler</code> object as the data handler for
	 * this <code>AttachmentPart</code> object. Typically, on an incoming
	 * message, the data handler is automatically set. When a message is being
	 * created and populated with content, the <code>setDataHandler</code>
	 ...
	 *
	 * @return a <code>String</code> giving the value of the "Content-ID" header
	 ...
	 *
	 * @return a <code>String</code> giving the value of the "Content-Location"
	 *         header or <code>null</code> if there is none
	 */
	 ...
	 *
	 * @return a <code>String</code> giving the value of the "Content-Type"
	 *         header or <code>null</code> if there is none
	 */
	...
	/**
	 * Sets the MIME header whose name is "Content-Location" with the given
	 * value.
	 *
	 *
	 ...
	 * value, adding a new header if no existing header matches. This method
	 * also removes all matching headers but the first.
	 * <p>
	 *
	...
	/**
	 * Retrieves all the headers for this <code>AttachmentPart</code> object as
	 * an iterator over the <code>MimeHeader</code> objects.
	 *
	 * @return an <code>Iterator</code> object with all of the Mime headers for
	 *         this <code>AttachmentPart</code> object
	 */
	public abstract Iterator getAllMimeHeaders();

	/**
	 * Retrieves all <code>MimeHeader</code> objects that match a name in the
	 * given array.
	 *
	 * @param names
	...
	/**
	 * Retrieves all <code>MimeHeader</code> objects whose name does not match a
