 ...
 * XML/SOAP document.
 * <P>
 * A <code>SOAPMessage</code> object consists of a SOAP part and optionally one
 * or more attachment parts. The SOAP part for a <code>SOAPMessage</code> object
 * is a <code>SOAPPart</code> object, which contains information used for
 * message routing and identification, and which can contain
 ...
 * <code>SOAPMessage.getSOAPPart()</code>. The <code>SOAPEnvelope</code> object
 * is retrieved from the <code>SOAPPart</code> object, and the
 * <code>SOAPEnvelope</code> object is used to retrieve the
 ...
 *
 * <P>
 * In addition to the mandatory <code>SOAPPart</code> object, a
 * <code>SOAPMessage</code> object may contain zero or more
 * <code>AttachmentPart</code> objects, each of which contains
 * application-specific data. The <code>SOAPMessage</code> interface provides
 * methods for creating <code>AttachmentPart</code> objects and also for adding
 * them to a <code>SOAPMessage</code> object. A party that has received a
 * <code>SOAPMessage</code> object can examine its contents by retrieving
 * individual attachment parts.
 * <P>
 * Unlike the rest of a SOAP message, an attachment is not required to be in XML
 * format and can therefore be anything from simple text to an image file.
 ...
 * application of SAAJ. For instance, a <code>MessageFactory</code> object may
 * produce <code>SOAPMessage</code> objects that conform to a particular Profile
 * such as ebXML. In this case a <code>MessageFactory</code> object might
 * produce <code>SOAPMessage</code> objects that are initialized with ebXML
 * headers.
 * <P>
 ...
 * otherwise noted in the JavaDocs for those methods the default implementations
 * simply throw an <code>UnsupportedOperationException</code> and the SAAJ
 * implementation code must override them with methods that provide the
 * specified behavior. Legacy client code does not have this restriction,
 * however, so long as there is no claim made that it conforms to some later
 * version of the specification than it was originally written for. A legacy
 * class that extends the SOAPMessage class can be compiled and/or run against
 * succeeding versions of the SAAJ API without modification. If such a class was
 * correctly implemented then it will continue to behave correctly relative to
 * the version of the specification against which it was written.
 *
 * @see MessageFactory
 * @see AttachmentPart
 */
public abstract class SOAPMessage {
	...
	/**
	 * Sets the description of this <code>SOAPMessage</code> object's content
	 * with the given description.
	 *
	 * @param description
	 *                    a <code>String</code> describing the content of this
	 *                    message
	 ...
	 * content.
	 *
	 * @return a <code>String</code> describing the content of this message or
	 *         <code>null</code> if no description has been set
	 ...
	 *
	 * @return the <code>SOAPPart</code> object for this
	...
	public SOAPBody getSOAPBody() throws SOAPException {
		throw new UnsupportedOperationException(
				"getSOAPBody must be overridden by all subclasses of SOAPMessage");
	}
	...
	public SOAPHeader getSOAPHeader() throws SOAPException {
		throw new UnsupportedOperationException(
				"getSOAPHeader must be overridden by all subclasses of SOAPMessage");
	}

	/**
	 * Removes all <code>AttachmentPart</code> objects that have been added to
	 ...
	 *
	 * @return the number of <code>AttachmentPart</code> objects that are part
	 ...
	 * entries that match the specified headers. Note that a returned attachment
	 * could have headers in addition to those specified.
	 *
	 * @param headers
	 ...
	 * via an <code>href</code> attribute as described in {@link <a href=
	 * "http://www.w3.org/TR/SOAP-attachments#SOAPReferenceToAttachements">SOAP
	 * Messages with Attachments</a>}, or via a single <code>Text</code> child
	 * node containing a URI as described in the WS-I Attachments Profile 1.0
	 * for elements of schema type {@link <a href=
	 * "http://www.ws-i.org/Profiles/AttachmentsProfile-1.0-2004-08-24.html">
	 * ref:swaRef</a>}. These two mechanisms must be supported. The support for
	 * references via <code>href</code> attribute also implies that this method
	 * should also be supported on an element that is an <i>xop:Include</i>
	 * element ( {@link <a href=
	 * "http://www.w3.org/2000/xp/Group/3/06/Attachments/XOP.html">XOP</a>}).
	 * other reference mechanisms may be supported by individual implementations
	 * of this standard. Contact your vendor for details.
	 *
	 * @param element
	 *                The <code>SOAPElement</code> containing the reference to
	 *                an
	 *                Attachment
	 * @return the referenced <code>AttachmentPart</code> or null if no such
	 *         <code>AttachmentPart</code> exists or no reference can be found
	 *         in this <code>SOAPElement</code>.
	 ...
	 * <code>SOAPMessage</code> object. An <code>AttachmentPart</code> object
	 * must be created before it can be added to a message.
	 *
	 * @param AttachmentPart
	 *                       an <code>AttachmentPart</code> object that is to
	 *                       become part
	 *                       of this <code>SOAPMessage</code> object
	...
	/**
	 * Creates an <code>AttachmentPart</code> object and populates it using the
	 * given <code>DataHandler</code> object.
	 *
	 * @param dataHandler
	 *                    the <code>javax.activation.DataHandler</code> object
	 *                    that will
	 ...
	 * <code>SOAPMessage</code> object in a transport-independent fashion.
	 *
	...
	/**
	 * Creates an <code>AttachmentPart</code> object and populates it with the
	 * specified data of the specified content type. The type of the
	 ...
	 * @param contentType
	 *                    a <code>String</code> object giving the type of
	 *                    content;
	 *                    examples are "text/xml", "text/plain", and
	 *                    "image/jpeg"
	 * @return a new <code>AttachmentPart</code> object that contains the given
	 *         data
	 * @exception IllegalArgumentException
	 ...
	 * {@link SOAPMessage#writeTo(OutputStream)} is called. However, if changes
	 * are made to a message that was received or to one that has already been
	 * sent, the method <code>saveChanges</code> needs to be called explicitly
	 * in order to save the changes. The method <code>saveChanges</code> also
	 * generates any changes that can be read back (for example, a MessageId in
	 * profiles that support a message id). All MIME headers in a message that
	 * is created for sending purposes are guaranteed to have valid values only
	 * after <code>saveChanges</code> has been called.
	 * <P>
	 ...
	 * <P>
	 *
	 * @exception <code>SOAPException</code>
	 *            if there was a problem saving changes to this message.
	 */
	public abstract void saveChanges() throws SOAPException;

	/**
	 * Indicates whether this <code>SOAPMessage</code> object needs to have the
	 * method <code>saveChanges</code> called on it.
	 *
	 * @return <code>true</code> if <code>saveChanges</code> needs to be called;
	 ...
	 * The externalization format is as defined by the SOAP 1.1 with Attachments
	 * specification.
	 * <P>
	 * If there are no attachments, just an XML stream is written out. For those
	 * messages that have attachments, <code>writeTo</code> writes a
	 ...
	 *
	 * @param out
	 *            the <code>OutputStream</code> object to which this
	 *            <code>SOAPMessage</code> object will be written
	 * @exception IOException
	 *                          if an I/O error occurs
	 * @exception SOAPException
	 *                          if there was a problem in externalizing this
	 *                          SOAP message
	 */
	public abstract void writeTo(OutputStream out) throws SOAPException,
			IOException;
