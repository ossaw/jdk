 ...
 * <code>SOAPMessage</code> object is created, it will automatically have a
 ...
 * has a MIME header of Content-Type with its value set to "text/xml". The value
 * must be "text/xml" because content in the SOAP part of a message must be in
 * XML format. Content that is not of type "text/xml" must be in an
 * <code>AttachmentPart</code> object rather than in the <code>SOAPPart</code>
 * object.
 * <P>
 ...
 * message that is received must have the MIME header Content-Type with a value
 * of "text/xml".
 * <P>
 * A client can access the <code>SOAPPart</code> object of a
 * <code>SOAPMessage</code> object by calling the method
 * <code>SOAPMessage.getSOAPPart</code>. The following line of code, in which
 * <code>message</code> is a <code>SOAPMessage</code> object, retrieves the SOAP
 * part of a message.
 * 
 * <PRE>
 ...
 * <code>SOAPHeader</code> object. The <code>SOAPPart</code> method
 * <code>getEnvelope</code> can be used to retrieve the
...
public abstract class SOAPPart implements org.w3c.dom.Document, Node {
	 ...
	 * <code>SOAPPart</code> object. Once the SOAP envelope is obtained, it can
	 * be used to get its contents.
	 *
	 ...
	 *
	 * @return a <code>String</code> giving the value of the MIME header named
	 *         "Content-Id"
	 * @see #setContentId
	 */
	...
	/**
	 * Gets all the values of the <code>MimeHeader</code> object in this
	 * <code>SOAPPart</code> object that is identified by the given
	 * <code>String</code>.
	 *
	 * @param name
	 ...
	 * value if no existing header is a match. If there is a match, this method
	 * clears all existing values for the first header that matches and sets the
	 * given value instead. If more than one header has the given name, this
	...
	/**
	 * Creates a <code>MimeHeader</code> object with the specified name and
	 * value and adds it to this <code>SOAPPart</code> object. If a
	 * <code>MimeHeader</code> with the specified name already exists, this
	 * method adds the specified value to the already existing value(s).
	 * <P>
	 * Note that RFC822 headers can contain only US-ASCII characters.
	 *
	 * @param name
	...
	/**
	 * Retrieves all the headers for this <code>SOAPPart</code> object as an
	 * iterator over the <code>MimeHeader</code> objects.
	 *
	 * @return an <code>Iterator</code> object with all of the Mime headers for
	 *         this <code>SOAPPart</code> object
	 */
	 ...
	 * from the given <code>Source</code> object. This <code>Source</code> must
