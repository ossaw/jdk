 ...
 * <P>
 * A SAAJ client can create a <code>MessageFactory</code> object using the
 * method <code>newInstance</code>, as shown in the following lines of code.
 * 
 * <PRE>
 ...
 * will produce <code>SOAPMessage</code> objects that have the following
 * elements by default:
 * <UL>
 ...
 * </UL>
 * In some cases, specialized MessageFactory objects may be obtained that
 * produce messages prepopulated with additional entries in the
 * <code>SOAPHeader</code> object and the <code>SOAPBody</code> object. The
 * content of a new <code>SOAPMessage</code> object depends on which of the two
 ...
 * <LI><code>createMessage(MimeHeaders, java.io.InputStream)</code> -- message
 * has content from the <code>InputStream</code> object and headers from the
 * <code>MimeHeaders</code> object <BR>
 * This method can be used internally by a service implementation to create a
 * message that is a response to a request.
 * </UL>
 */
public abstract class MessageFactory {
	...
	static private final String MESSAGE_FACTORY_PROPERTY = "javax.xml.soap.MessageFactory";
	 ...
	 * classname in the file META-INF/services/javax.xml.soap.MessageFactory in
	 * jars available to the runtime.
	 * <LI>Use the SAAJMetaFactory instance to locate the MessageFactory
	 ...
	 *                implementation of the <code>MessageFactory</code>.
	 * @see SAAJMetaFactory
	 */
	...
	public static MessageFactory newInstance() throws SOAPException {
		...
		try {
			...
			throw new SOAPException(
					"Unable to create message factory for SOAP: " + ex.getMessage());
		}
	...
	/**
	 * Creates a new <code>MessageFactory</code> object that is an instance of
	 * the specified implementation. May be a dynamic message factory, a SOAP
	 ...
	 * the message can be sent "as is" when a message containing only a SOAP
	 * part is sufficient. Otherwise, the <code>SOAPMessage</code> object needs
	 * to create one or more <code>AttachmentPart</code> objects and add them to
	 * itself. Any content that is not in XML format must be in an
	 ...
	 *                if the protocol of this <code>MessageFactory</code>
	 *                instance is <code>DYNAMIC_SOAP_PROTOCOL</code>
	 */
	public abstract SOAPMessage createMessage() throws SOAPException;

	/**
	 * Internalizes the contents of the given <code>InputStream</code> object
	 * into a new <code>SOAPMessage</code> object and returns the
	 * <code>SOAPMessage</code> object.
	 *
	 * @param in
	 *            the <code>InputStream</code> object that contains the data for
	 *            a message
	 * @param headers
	 ...
	 *                if there is a problem in reading data from the input
	 *                stream
	 *
	 * @exception SOAPException
	 ...
	 *                if the <code>MessageFactory</code> requires one or more
	 *                MIME headers to be present in the <code>headers</code>
