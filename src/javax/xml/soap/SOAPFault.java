 ...
 * <code>SOAPMessage</code> object or to problems that are not related to the
...
public interface SOAPFault extends SOAPBodyElement {
	 ...
	 * <code>Name</code>. This method provides a convenient way to set a fault
	 * code. For example,
	 *
	 * <PRE>
	 * SOAPEnvelope se = ...;
	 ...
	 *
	 * @param faultCodeQName
	 *            a <code>Name</code> object giving the fault code to be set. It
	 *            must be namespace qualified.
	 * @see #getFaultCodeAsName
	 *
	 * @exception SOAPException
	 *                if there was an error in adding the <i>faultcode</i>
	 ...
	 *
	 * @param faultCodeQName
	 *            a <code>QName</code> object giving the fault code to be set.
	 *            It must be namespace qualified.
	 * @see #getFaultCodeAsQName
	 *
	 * @exception SOAPException
	 *                if there was an error in adding the <code>faultcode</code>
	 *                element to the underlying XML tree.
	 *
	 ...
	 * <P>
	 * Fault codes, which given information about the fault, are defined in the
	 * SOAP 1.1 specification. This element is mandatory in SOAP 1.1. Because
	 * the fault code is required to be a QName it is preferable to use the
	 * {@link #setFaultCode(Name)} form of this method.
	 *
	 * @param faultCode
	 *            a <code>String</code> giving the fault code to be set. It must
	 *            be of the form "prefix:localName" where the prefix has been
	...
	/**
	 * Gets the fault code for this <code>SOAPFault</code> object as a
	 ...
	 *         <code>QNames</code>. This <code>Iterator</code> should not
	 *         support the optional <code>remove</code> method. The order in
	 *         which the Subcodes are returned reflects the hierarchy of
	 *         Subcodes present in the fault from top to bottom.
	 *
	 * @exception UnsupportedOperationException
	 *                if this message does not support the SOAP 1.2 concept of
	 *                Subcode.
	 *
	 * @since SAAJ 1.3
	 */
	 ...
	 * . Subsequent calls to <code>getFaultSubcodes</code> will return an empty
	 * iterator until a call to <code>appendFaultSubcode</code> is made.
	 *
	 * @exception UnsupportedOperationException
	 *                if this message does not support the SOAP 1.2 concept of
	 *                Subcode.
	 ...
	 * <P>
	 * The fault actor is the recipient in the message path who caused the fault
	 * to happen.
	 * <P>
	 ...
	 *
	 * @param faultActor
	 *            a <code>String</code> identifying the actor that caused this
	 ...
	 * string.
	 * <P>
	 * If this <code>SOAPFault</code> is part of a message that supports SOAP
	 ...
	 *            a <code>String</code> giving an explanation of the fault
	 * @param locale
	 *            a {@link java.util.Locale Locale} object indicating the native
	 ...
	 * information, the scope of the error information is restricted to faults
	 * in the <code>SOAPBodyElement</code> objects if this is a SOAP 1.1 Fault.
	 *
	 * @return a <code>Detail</code> object with application-specific error
	 *         information if present, null otherwise
	 */
	public Detail getDetail();

	/**
	 * Creates an optional <code>Detail</code> object and sets it as the
	 * <code>Detail</code> object for this <code>SOAPFault</code> object.
	 * <P>
	 * It is illegal to add a detail when the fault already contains a detail.
	 * Therefore, this method should be called only after the existing detail
	 * has been removed.
	 *
	 * @return the new <code>Detail</code> object
	 *
	 * @exception SOAPException
	 *                if this <code>SOAPFault</code> object already contains a
	 ...
	 * <code>getFaultReasonText</code> in order to obtain a localized version of
	 ...
	 *
	 * @exception SOAPException
	 *                if there was an error in retrieving the fault Reason
	 *                locales.
	 * @exception UnsupportedOperationException
	 *                if this message does not support the SOAP 1.2 concept of
	 *                Fault Reason.
	...
	/**
	 * Returns an <code>Iterator</code> over a sequence of <code>String</code>
	 * objects containing all of the Reason Text items for this
	 * <code>SOAPFault</code>.
	 *
	 * @return an <code>Iterator</code> over env:Fault/env:Reason/env:Text
	 *         items.
	 *
	 * @exception SOAPException
	 *                if there was an error in retrieving the fault Reason
	...
	/**
	 * Returns the Reason Text associated with the given <code>Locale</code>. If
	 * more than one such Reason Text exists the first matching Text is returned
	 *
	 * @param locale
	 *            -- the <code>Locale</code> for which a localized Reason Text
	 *            is desired
	 *
	 ...
	 *
	 * @exception SOAPException
	 *                if there was an error in retrieving the fault Reason text
	 ...
	 * message and an <i>xml:lang</i> derived from <code>locale</code>. If a
	 * Reason Text item with this <i>xml:lang</i> already exists its text value
	 * will be replaced with <code>text</code>. The <code>locale</code>
	 ...
	 * @param locale
	 *            -- Locale object representing the locale of the message
	 *
	 * @exception SOAPException
	 *                if there was an error in adding the Reason text or the
	 *                <code>locale</code> passed was <code>null</code>.
	 * @exception UnsupportedOperationException
	 *                if this message does not support the SOAP 1.2 concept of
	 *                Fault Reason.
	 *
	...
	/**
	 * Returns the optional Node element value for this <code>SOAPFault</code>
	 * object. The Node element is optional in SOAP 1.2.
	 *
	 * @return Content of the env:Fault/env:Node element as a String or
	 ...
	 * <code>SOAPFault</code> object. The Node element is optional in SOAP 1.2.
	 *
	 * @exception SOAPException
	 *                if there was an error in setting the Node for this
	 *                <code>SOAPFault</code> object.
	 * @exception UnsupportedOperationException
	 *                if this message does not support the SOAP 1.2 concept of
	 *                Fault Node.
	...
	/**
	 * Returns the optional Role element value for this <code>SOAPFault</code>
	 * object. The Role element is optional in SOAP 1.2.
	 *
	 * @return Content of the env:Fault/env:Role element as a String or
	 *         <code>null</code> if none
	 *
	 * @exception UnsupportedOperationException
	 *                if this message does not support the SOAP 1.2 concept of
	 ...
	 * <code>SOAPFault</code> object. The Role element is optional in SOAP 1.2.
	 *
	 * @param uri
	 *            - the URI of the Role
	 *
	 * @exception SOAPException
	 *                if there was an error in setting the Role for this
	 *                <code>SOAPFault</code> object.
	 *
	 * @exception UnsupportedOperationException
	 *                if this message does not support the SOAP 1.2 concept of
