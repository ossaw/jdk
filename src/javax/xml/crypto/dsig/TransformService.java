 ...
 * <p>
 * Each instance of <code>TransformService</code> supports a specific transform
 * or canonicalization algorithm and XML mechanism type. To create a
 * <code>TransformService</code>, call one of the static {@link #getInstance
 * getInstance} methods, passing in the algorithm URI and XML mechanism type
 * desired, for example:
 * <blockquote><code>
 * TransformService ts = TransformService.getInstance(Transform.XPATH2, "DOM");
 * </code></blockquote>
 * <p>
 * <code>TransformService</code> implementations are registered and loaded using
 * the {@link java.security.Provider} mechanism. Each
 ...
 * mechanism type that it supports. If the attribute is not specified, "DOM" is
 * assumed. For example, a service provider that supports the XPath Filter 2
 * Transform and DOM mechanism would be specified in the <code>Provider</code>
 * subclass as:
 * 
 * <pre>
 ...
 * mechanism types.
 * <p>
 * Once a <code>TransformService</code> has been created, it can be used to
 * process <code>Transform</code> or <code>CanonicalizationMethod</code>
 ...
 * created from scratch, the {@link #init(TransformParameterSpec)} method is
 * called to initialize the transform with parameters and the
 ...
 * access a single <code>TransformService</code> instance concurrently should
 * synchronize amongst themselves and provide the necessary locking. Multiple
 * threads each manipulating a different <code>TransformService</code> instance
...
public abstract class TransformService implements Transform {
     ...
     * algorithm URI (ex: {@link Transform#XPATH2}) and mechanism type (ex:
     * DOM).
     * <p>
     * This method uses the standard JCA provider lookup mechanism to locate and
     * instantiate a <code>TransformService</code> implementation of the desired
     * algorithm and <code>MechanismType</code> service attribute. It traverses
     * the list of registered security <code>Provider</code>s, starting with the
     * most preferred <code>Provider</code>. A new <code>TransformService</code>
    ...
    public static TransformService getInstance(String algorithm, String mechanismType)
        ...
        throw new NoSuchAlgorithmException(algorithm + " algorithm and " + mechanismType
                + " mechanism not available");
    }

    /**
     * Returns a <code>TransformService</code> that supports the specified
     * algorithm URI (ex: {@link Transform#XPATH2}) and mechanism type (ex: DOM)
    ...
    /**
     * Marshals the algorithm-specific parameters. If there are no parameters to
