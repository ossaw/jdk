 ...
 *
 * <p>
 * Note that the behavior of an unmarshalled <code>XMLSignature</code> is
 * undefined if the contents of the underlying DOM tree are modified by the
 * caller after the <code>XMLSignature</code> is created.
 *
 * <p>
 * Also, note that <code>DOMValidateContext</code> instances can contain
 * information and state specific to the XML signature structure it is used
 * with. The results are unpredictable if a <code>DOMValidateContext</code> is
 * used with different signature structures (for example, you should not use the
 * same <code>DOMValidateContext</code> instance to validate two different
