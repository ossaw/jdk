 ...
 * to specify the location in a DOM tree where an {@link XMLSignature} object is
 * to be marshalled when generating the signature.
 *
 * <p>
 * Note that <code>DOMSignContext</code> instances can contain information and
 * state specific to the XML signature structure it is used with. The results
 * are unpredictable if a <code>DOMSignContext</code> is used with different
 * signature structures (for example, you should not use the same
 * <code>DOMSignContext</code> instance to sign two different
 * {@link XMLSignature} objects).
 *
 * @author Sean Mullan
...
public class DOMSignContext extends DOMCryptoContext implements XMLSignContext {
	...
	/**
	 * Creates a <code>DOMSignContext</code> with the specified signing key and
	 * parent node. The signing key is stored in a
	 * {@link KeySelector#singletonKeySelector singleton KeySelector} that is
	 * returned by the {@link #getKeySelector getKeySelector} method. The
	 * marshalled <code>XMLSignature</code> will be added as the last child
	 ...
	 * {@link KeySelector#singletonKeySelector singleton KeySelector} that is
	 * returned by the {@link #getKeySelector getKeySelector} method. The
	 * marshalled <code>XMLSignature</code> will be inserted as a child element
	 * of the specified parent node and immediately before the specified next
	 * sibling node.
	 *
	 * @param signingKey
	 *            the signing key
	...
	/**
	 * Creates a <code>DOMSignContext</code> with the specified key selector and
	 * parent node. The marshalled <code>XMLSignature</code> will be added as
	 * the last child element of the specified parent node unless a next sibling
	 * node is specified by invoking the {@link #setNextSibling setNextSibling}
	 * method.
	 *
	 * @param ks
	 *            the key selector
