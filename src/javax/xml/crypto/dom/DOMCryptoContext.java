 ...
 * {@link XMLCryptoContext} interface. It also includes additional methods that
 * are specific to a DOM-based implementation for registering and retrieving
 * elements that contain attributes of type ID.
 *
 * @author Sean Mullan
...
public class DOMCryptoContext implements XMLCryptoContext {
    ...
    /**
     * This implementation uses an internal {@link HashMap} to map the URI to
    ...
    /**
     * This implementation uses an internal {@link HashMap} to map the name to
     ...
     * <code>DOMCryptoContext</code>. Attempts to modify the set via the
     * {@link Iterator#remove} method throw an
     * <code>UnsupportedOperationException</code>. The mappings are returned in
     * no particular order. Each element in the iteration is represented as a
     * {@link java.util.Map.Entry}. If the <code>DOMCryptoContext</code> is
     * modified while an iteration is in progress, the results of the iteration
     * are undefined.
     *
    ...
    /**
     * This implementation uses an internal {@link HashMap} to map the key to
     * the specified object.
     *
     * @throws IllegalArgumentException
     *                                  {@inheritDoc}
