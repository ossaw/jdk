 ...
 *
 * <p>
 * A validator object is not thread-safe and not reentrant. In other words, it
 * is the application's responsibility to make sure that one {@link Validator}
 * object is not used from more than one thread at any given time, and while the
 * <code>validate</code> method is invoked, applications may not recursively
 * call the <code>validate</code> method.
 * <p>
 *
 *
...
public abstract class Validator {
	 ...
	 *
	 * <p>
	 * <code>Validator</code> is reset to the same state as when it was created
	 * with {@link Schema#newValidator()}. <code>reset()</code> is designed to
	 * allow the reuse of existing <code>Validator</code>s thus saving resources
	 * associated with the creation of new <code>Validator</code>s.
	 * </p>
	 *
	 * <p>
	 * The reset <code>Validator</code> is not guaranteed to have the same
	 * {@link LSResourceResolver} or {@link ErrorHandler} <code>Object</code>s,
	 * e.g. {@link Object#equals(Object obj)}. It is guaranteed to have a
	 * functionally equal <code>LSResourceResolver</code> and
	 ...
	 * {@link #validate(Source source, Result result)} with <code>result</code>
	 * of <code>null</code>.
	 * </p>
	 *
	 * @param source
	...
	/**
	 * <p>
	 * Validates the specified input and send the augmented validation result to
	 ...
	 *
	 * <p>
	 * To validate one <code>Source</code> into another kind of
	 ...
	 *
	 * <p>
	 * If a document is valid, or if a document contains some errors but none of
	 * them were fatal and the <code>ErrorHandler</code> didn't throw any
	 * exception, then the method returns normally.
	 * </p>
	 *
	 * @param source
	 *            XML to be validated. Must be an XML document or XML element
	 ...
	 * <code>validate</code> method invocation.
	 *
	 * <p>
	 ...
	 * {@link SAXException} from the handler. Or for example it can print an
	 * error to the screen and try to continue the validation by returning
	 * normally from the {@link ErrorHandler}
	 *
	 * <p>
	 * If any {@link Throwable} is thrown from an {@link ErrorHandler}, the
	 * caller of the <code>validate</code> method will be thrown the same
	 * {@link Throwable} object.
	 *
	 * <p>
	 * {@link Validator} is not allowed to throw {@link SAXException} without
	 ...
	 *
	 * <p>
	 * When a new {@link Validator} object is created, initially this field is
	 * set to null.
	 *
	 * @param errorHandler
	 ...
	 *
	 * <p>
	 * {@link Validator} uses a {@link LSResourceResolver} when it needs to
	 * locate external resources while a validation, although exactly what
	 ...
	 * instances of its derived classes), then the {@link Validator} will abort
	 * the parsing and the caller of the <code>validate</code> method will
	 ...
	 * {@link Validator} to recognize a feature name but temporarily be unable
	 * to return its value. Some feature values may be available only in
	 ...
	 *
	 * <p>
	 * Feature can be used to control the way a {@link Validator} parses
	 * schemas, although {@link Validator}s are not required to recognize any
	 * specific feature names.
	 * </p>
	 *
	 * <p>
	 * The feature name is any fully-qualified URI. It is possible for a
	 * {@link Validator} to expose a feature value but to be unable to change
	 * the current value. Some feature values may be immutable or mutable only
	 ...
	 * {@link Validator} to recognize a property name but to be unable to change
	 * the current value. Some property values may be immutable or mutable only
	 * in specific contexts, such as before, during, or after a validation.
	 * </p>
	 *
	 * <p>
	 ...
	 * <li>
	 * <p>
	 * Access to external DTDs in source or Schema file is restricted to the
	 ...
	 * <p>
	 * The property name is any fully-qualified URI. It is possible for a
	 * {@link Validator} to recognize a property name but temporarily be unable
	 * to return its value. Some property values may be available only in
	 * specific contexts, such as before, during, or after a validation.
	 * </p>
	 *
	 * <p>
	 * {@link Validator}s are not required to recognize any specific property
	 * names.
	 * </p>
	 *
	 * <p>
