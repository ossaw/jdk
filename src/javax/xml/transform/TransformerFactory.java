 ...
 * named <code>"javax.xml.transform.TransformerFactory"</code>. This property
 * names a concrete subclass of the <code>TransformerFactory</code> abstract
...
public abstract class TransformerFactory {
	...
	/**
	 * <p>
	 * Obtain a new instance of a <code>TransformerFactory</code>. This static
	 * method creates a new factory instance.
	 * </p>
	 * <p>
	 * This method uses the following ordered lookup procedure to determine the
	 * <code>TransformerFactory</code> implementation class to load:
	 * </p>
	 * <ul>
	 * <li>Use the <code>javax.xml.transform.TransformerFactory</code> system
	 * property.</li>
	 ...
	 * of any property in jaxp.properties after it has been read for the first
	 * time.</li>
	 ...
	 * TransformerFactory</code> it can use the factory to configure and obtain
	 ...
	 *             Thrown in case of
	 *             {@linkplain java.util.ServiceConfigurationError service
	...
	/**
	 * <p>
	 * Obtain a new instance of a <code>TransformerFactory</code> from factory
	 * class name. This function is useful when there are multiple providers in
	 * the classpath. It gives more control to the application as it can specify
	...
	public static TransformerFactory newInstance(String factoryClassName, ClassLoader classLoader)
		...
		// exception
		return FactoryFinder.newInstance(TransformerFactory.class, factoryClassName, classLoader,
				false, false);
	}
	 ...
	 * conforms to <a href="http://www.w3.org/TR/xslt"> XSL Transformations
	 * (XSLT) Version 1.0</a>. Care must be taken not to use this
	 * <code>Transformer</code> in multiple <code>Thread</code>s running
	 * concurrently. Different <code>TransformerFactories</code> can be used
	 * concurrently by different <code>Thread</code>s.
	 * </p>
	 *
	 * @param source
	 *            <code>Source </code> of XSLT document used to create
	 *            <code>Transformer</code>. Examples of XML <code>Source</code>s
	 *            include {@link javax.xml.transform.dom.DOMSource DOMSource},
	 *            {@link javax.xml.transform.sax.SAXSource SAXSource}, and
	 *            {@link javax.xml.transform.stream.StreamSource StreamSource}.
	 *
	 * @return A <code>Transformer</code> object that may be used to perform a
	 *         transformation in a single <code>Thread</code>, never
	 *         <code>null</code>.
	 *
	 * @throws TransformerConfigurationException
	 *             Thrown if there are errors when parsing the
	 ...
	 * <code>Source</code> document via the
	 * <a href="http://www.w3.org/TR/xml-stylesheet/"> xml-stylesheet processing
	 * instruction</a> that match the given criteria. Note that it is possible
	 * to return several stylesheets, in which case they are applied as if they
	 * were a list of imports or cascades in a single stylesheet.
	 * </p>
	 *
	 * @param source
	 ...
	 *
	 * @return A <code>Source</code> <code>Object</code> suitable for passing to
	 *         the <code>TransformerFactory</code>.
	 *
	 * @throws TransformerConfigurationException
	 *             An <code>Exception</code> is thrown if an error occurings
	 *             during parsing of the <code>source</code>.
	 *
	 ...
	 * conform to implementation limits and behave in a secure fashion as
	 * defined by the implementation. Examples include resolving user defined
	 * style sheets and functions. If XML processing is limited for security
	 * reasons, it will be reported via a call to the registered
	 * {@link ErrorListener#fatalError(TransformerException exception)}. See
	 * {@link #setErrorListener(ErrorListener listener)}.</li>
	 ...
	 * <p>
	 * Feature names are fully qualified {@link java.net.URI}s. Implementations
	 * may define their own features. <code>false</code> is returned if this
	 * <code>TransformerFactory</code> or the <code>Transformer</code>s or
	 * <code>Template</code>s it creates cannot support the feature. It is
	 * possible for an <code>TransformerFactory</code> to expose a feature value
	 * but be unable to change its state.
	 ...
	 * that the implementation provides. An
	 * <code>IllegalArgumentException</code> is thrown if the underlying
	 ...
	 * {@link javax.xml.transform.TransformerConfigurationException} will be
	 * thrown by the {@link #newTransformer(Source)} method.
	 * </p>
	 * <p>
	 * Access to external reference set by the stylesheet processing
	 * instruction, Import and Include element is restricted to the protocols
	 * specified by the
	 * {@link javax.xml.XMLConstants#ACCESS_EXTERNAL_STYLESHEET} property. If
	 * access is denied during the creation of a new transformer due to the
	 * restriction of this property,
	 * {@link javax.xml.transform.TransformerConfigurationException} will be
	 * thrown by the {@link #newTransformer(Source)} method.
	 * </p>
	 ...
	 *             When implementation does not recognize the attribute.
	 */
	public abstract void setAttribute(String name, Object value);

	/**
	 * Allows the user to retrieve specific attributes on the underlying
	 * implementation. An <code>IllegalArgumentException</code> is thrown if the
	 ...
	 * transformation itself. An <code>IllegalArgumentException</code> is thrown
	 * if the <code>ErrorListener</code> listener is <code>null</code>.
	 *
	 * @param listener
	 *            The new error listener.
	 *
	 * @throws IllegalArgumentException
	 *             When <code>listener</code> is <code>null</code>
	 */
