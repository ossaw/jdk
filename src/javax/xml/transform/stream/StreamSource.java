...
public class StreamSource implements Source {
	 ...
	 * when passed this value as an argument, the Transformer supports Source
	 * input of this type.
	 */
	...
	/**
	 * <p>
	 * Zero-argument default constructor. If this constructor is used, and no
	 * Stream source is set using
	 * {@link #setInputStream(java.io.InputStream inputStream)} or
	 * {@link #setReader(java.io.Reader reader)}, then the
	 * <code>Transformer</code> will create an empty source
	 * {@link java.io.InputStream} using
	...
	/**
	 * Construct a StreamSource from a character reader. Normally, a stream
	 * should be used rather than a reader, so that the XML parser can resolve
	 * character encoding specified by the XML declaration. However, in many
	 * cases the encoding of the input stream is already resolved, as in the
	...
	/**
	 * Construct a StreamSource from a character reader. Normally, a stream
	 * should be used rather than a reader, so that the XML parser may resolve
	 * character encoding specified by the XML declaration. However, in many
	 * cases the encoding of the input stream is already resolved, as in the
	 * case of reading XML from a StringReader.
	 *
	 ...
	 *
	 * @return The byte stream that was set with setByteStream, or null if
	 *         setByteStream or the ByteStream constructor was not called.
	 */
	...
	/**
	 * Set the input to be a character reader. Normally, a stream should be used
	 * rather than a reader, so that the XML parser can resolve character
	 * encoding specified by the XML declaration. However, in many cases the
	 * encoding of the input stream is already resolved, as in the case of
	 ...
	 *
	 * @return The character stream that was set with setReader, or null if
	 *         setReader or the Reader constructor was not called.
	 */
	 ...
	 *
	 * @return The public identifier that was set with setPublicId, or null if
	 *         setPublicId was not called.
	 */
