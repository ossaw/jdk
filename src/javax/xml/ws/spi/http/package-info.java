...
/**
 * Provides HTTP SPI that is used for portable deployment of JAX-WS web services
 ...
 * application. The necessary information to create Endpoint objects may be got
 * from web service deployment descriptor files.</li>
 * <li>Container needs to create {@link javax.xml.ws.spi.http.HttpContext}
 * objects for the deployment. For example, a HttpContext could be created using
 * servlet configuration(for e.g url-pattern) for the web service in servlet
 * container case.</li>
 * <li>Then publishes all the endpoints using
 * {@link javax.xml.ws.Endpoint#publish(HttpContext)}. During publish(), JAX-WS
 * runtime registers a {@link javax.xml.ws.spi.http.HttpHandler} callback to
 * handle incoming requests or {@link javax.xml.ws.spi.http.HttpExchange}
