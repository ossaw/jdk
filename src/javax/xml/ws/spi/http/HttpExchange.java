 ...
 * <p>
 * A <code>HttpExchange</code> must be closed to free or reuse underlying
 * resources. The effect of failing to close an exchange is undefined.
 *
...
public abstract class HttpExchange {
	...
	/**
	 * Standard property: cipher suite value when the request is received over
	 * HTTPS
	 * <p>
	 * Type: String
	 */
	...
	/**
	 * Standard property: bit size of the algorithm when the request is received
	 * over HTTPS
	 * <p>
	 * Type: Integer
	 */
	 ...
	 * the chain is the one set by the client, the next is the one used to
	 ...
	 * there are multiple headers with the same name, this method returns the
	 * first header in the request. The header name is case-insensitive. This is
	 * a convienence method to get a header (instead of using the
	 ...
	 * a response header to have multiple values. This is a convenience method
	 * to add a response header(instead of using the
	...
	/**
	 * Returns the part of the request's URI from the protocol name up to the
	 * query string in the first line of the HTTP request. Container doesn't
	 * decode this string.
	 *
	...
	/**
	 * Returns the context path of all the endpoints in an application. This
	 * path is the portion of the request URI that indicates the context of the
	...
	/**
	 * Returns a {@link HttpContext} for this exchange. Container matches the
	 * request with the associated Endpoint's HttpContext
	 *
	...
	/**
	 * This must be called to end an exchange. Container takes care of closing
	 * request and response streams. This must be called so that the container
	 ...
	 * to this method will return the same stream.
	 *
	 ...
	 * {@link #setStatus}) must be called prior to calling this method. Multiple
	 * calls to this method (for the same exchange) will return the same stream.
	 *
	...
	/**
	 * Returns the unresolved address of the remote entity invoking this
	 * request.
	 *
	 ...
	 *
	 * @return decoded extra path information of web service. It is the path
	 *         that comes after the web service path but before the query string
	 *         in the request URI <tt>null</tt> if there is no extra path in the
	 *         request URI
	 */
	 ...
	 *
	 * @return undecoded query string of request URI, or <tt>null</tt> if the
	 *         request URI doesn't have one
	 */
	public abstract String getQueryString();

	/**
	 * Returns an attribute that is associated with this
	 * <code>HttpExchange</code>. JAX-WS handlers and endpoints may then access
	 * the attribute via {@link MessageContext}.
	 * <p>
	 ...
	 * {@link #REQUEST_CIPHER_SUITE}, and {@link #REQUEST_KEY_SIZE}. If there is
	 * a SSL certificate associated with the request, it must be exposed using
	...
	/**
	 * Returns the {@link Principal} that represents the authenticated user for
	 * this <code>HttpExchange</code>.
	 *
	 * @return Principal for an authenticated user, or <tt>null</tt> if not
	 *         authenticated
	 */
