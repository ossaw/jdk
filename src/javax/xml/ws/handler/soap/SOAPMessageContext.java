...
/**
 * The interface <code>SOAPMessageContext</code> provides access to the SOAP
 * message for either RPC request or response. The
 * <code>javax.xml.soap.SOAPMessage</code> specifies the standard Java API for
 * the representation of a SOAP 1.1 message with attachments.
 *
...
public interface SOAPMessageContext extends
     ...
     * Modifications to the returned <code>SOAPMessage</code> change the message
     * in-place, there is no need to subsequently call <code>setMessage</code>.
     *
     * @return Returns the <code>SOAPMessage</code>; returns <code>null</code>
     *         if no <code>SOAPMessage</code> is present in this message context
     **/
    ...
    /**
     * Gets headers that have a particular qualified name from the message in
     * the message context. Note that a SOAP message can contain multiple
     ...
     * using {@link javax.xml.ws.soap.SOAPBinding#setRoles} and
     * {@link javax.xml.ws.soap.SOAPBinding#getRoles}. <code>Handler</code>
     * instances in the handler chain use this information about the SOAP actor
     * roles to process the SOAP header blocks. Note that the SOAP actor roles
     * are invariant during the processing of SOAP message through the handler
     * chain.
     *
