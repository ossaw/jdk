...
public class JMX {
	 ...
	 * is described by the Java interface {@code MyMBean}, you can construct a
	 ...
	 * @param objectName
	 *            the name of the MBean within {@code connection} to forward to.
	 ...
	 *             <a href="package-summary.html#mgIface">compliant MBean
	 *             interface</a>
	 */
	 ...
	 * true}, then the MBean is assumed to be a {@link NotificationBroadcaster}
	 * or {@link NotificationEmitter} and the returned proxy will implement
	 * {@link NotificationEmitter} as well as {@code interfaceClass}. A call to
	 * {@link NotificationBroadcaster#addNotificationListener} on the proxy will
	 * result in a call to
	 * {@link MBeanServerConnection#addNotificationListener(ObjectName, NotificationListener, NotificationFilter, Object)}
	 * , and likewise for the other methods of {@link NotificationBroadcaster}
	 * and {@link NotificationEmitter}.
	 * </p>
	 *
	 * @param connection
	 *            the MBean server to forward to.
	 ...
	 *            by forwarding its methods via {@code connection}.
	 *
	 * @param <T>
	 *            allows the compiler to know that if the {@code
	 * interfaceClass} parameter is {@code MyMBean.class}, for example, then the
	 ...
	 * is described by the Java interface {@code MyMXBean}, you can construct a
	 ...
	 * {@code mbs.}{@link MBeanServerConnection#setAttribute setAttribute}
	 * <code>(name,
	 ...
	 * context of an MXBean. The MXBean proxy behaves the same as a Standard
	 * MBean proxy (see
	 ...
	 * {@code mbs.getAttribute("MappedAttribute")}. The MXBean mapping rules
	 * mean that the actual type of the attribute {@code
	 * MappedAttribute} will be {@link javax.management.openmbean.CompositeData
	 * CompositeData} and that is what the {@code mbs.getAttribute} call will
	 ...
	 * {@code MemoryUsage} argument into a {@code
	 * CompositeData} and call {@code mbs.invoke}. The value returned by
	 * {@code mbs.invoke} will be also be a {@code CompositeData}, and the proxy
	 ...
	 *            the name of the MBean within {@code connection} to forward to.
	 * @param interfaceClass
	 *            the MXBean interface, which will also be implemented by the
	 *            returned proxy.
	 *
	 * @param <T>
	 *            allows the compiler to know that if the {@code
	 * interfaceClass} parameter is {@code MyMXBean.class}, for example, then
	 ...
	 * additionally, if {@code notificationEmitter} is {@code
	 * true}, then the MXBean is assumed to be a {@link NotificationBroadcaster}
	 * or {@link NotificationEmitter} and the returned proxy will implement
	 * {@link NotificationEmitter} as well as {@code interfaceClass}. A call to
	 * {@link NotificationBroadcaster#addNotificationListener} on the proxy will
	 * result in a call to
	...
	/**
	 * <p>
	 * Test whether an interface is an MXBean interface. An interface is an
	 * MXBean interface if it is public, annotated {@link MXBean &#64;MXBean} or
	 * {@code @MXBean(true)} or if it does not have an {@code @MXBean}
