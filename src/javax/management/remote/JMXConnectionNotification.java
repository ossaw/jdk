 ...
 * (instances of {@link JMXConnectorServer}) and by connector clients (instances
 * of {@link JMXConnector}). For certain connectors, a session can consist of a
 * sequence of connections. Connection-opened and connection-closed
 * notifications will be sent for each one.
 * </p>
 * <p>
 ...
 * <td>A client connection has potentially lost notifications. This notification
 * only appears on the client side.</td>
 * </tr>
 * </table>
 * <p>
 * The <code>timeStamp</code> of the notification is a time value (consistent
 * with {@link System#currentTimeMillis()}) indicating when the notification was
 * constructed.
 * </p>
 *
 * @since 1.5
...
public class JMXConnectionNotification extends Notification {
    ...
    /**
     * <p>
     * Notification type string for a connection that has possibly lost
     * notifications.
     * </p>
     */
    ...
    /**
     * Constructs a new connection notification. The {@link #getSource() source}
     * of the notification depends on whether it is being sent by a connector
     * server or a connector client:
     * <ul>
     * <li>For a connector server, if it is registered in an MBean server, the
     * source is the {@link ObjectName} under which it is registered. Otherwise,
     * it is a reference to the connector server object itself, an instance of a
     * subclass of {@link JMXConnectorServer}.
     * <li>For a connector client, the source is a reference to the connector
     * client object, an instance of a class implementing {@link JMXConnector}.
     * </ul>
     *
     * @param type
    ...
    public JMXConnectionNotification(String type, Object source, String connectionId, long sequenceNumber,
         ...
         * specify that. So we make sure it is not null, in case it would throw
         * the wrong exception (e.g. IllegalArgumentException instead of
         * NullPointerException). Likewise for the sequence number.
         */
        super((String) nonNull(type), nonNull(source), Math.max(0, sequenceNumber), System
