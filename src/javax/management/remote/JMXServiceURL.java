 ...
 * characters, each of which is a letter, a digit, or one of the characters
 ...
 * the address. It can contain attributes using the semicolon syntax specified
 ...
 * address. Depending on the protocol, case can be significant in the
 * <code><em>url-path</em></code>.
 * </p>
 *
...
public class JMXServiceURL implements Serializable {
	...
	/**
	 * <p>
	 * Constructs a <code>JMXServiceURL</code> by parsing a Service URL string.
	 * </p>
	 *
	 * @param serviceURL
	 ...
	 *                if <code>serviceURL</code> does not conform to the syntax
	 *                for an Abstract Service URL or if it is not a valid name
	 *                for a JMX Remote API service. A <code>JMXServiceURL</code>
	 *                must begin with the string <code>"service:jmx:"</code>
	 *                (case-insensitive). It must not contain any characters
	...
	public JMXServiceURL(String serviceURL) throws MalformedURLException {
		...
		for (int i = 0; i < serviceURLLength; i++) {
			...
			if (c < 32 || c >= 127) {
				throw new MalformedURLException("Service URL contains " + "non-ASCII character 0x"
	...
	/**
	 * <p>
	 * Constructs a <code>JMXServiceURL</code> with the given protocol, host,
	 * and port. This constructor is equivalent to
	 ...
	 *
	 * @param host
	 *            the host part of the URL. If null, defaults to the local host
	 ...
	 *
	 * @exception MalformedURLException
	 *                if one of the parts is syntactically incorrect, or if
	 *                <code>host</code> is null and it is not possible to find
	 *                the local host name, or if <code>port</code> is negative.
	 */
	 ...
	 *
	 * @param urlPath
	 *            the URL path part of the URL. If null, defaults to the empty
	 *            string.
	 *
	 * @exception MalformedURLException
	 *                if one of the parts is syntactically incorrect, or if
	...
	public JMXServiceURL(String protocol, String host, int port, String urlPath)
		...
		if (host == null) {
			...
			/*
			 * We might have a hostname that violates DNS naming rules, for
			 * example that contains an `_'. While we could be strict and throw
			 * an exception, this is rather user-hostile. Instead we use its
			 * numerical IP address. We can only reasonably do this for the
			 * host==null case. If we're given an explicit host name that is
			 * illegal we have to reject it. (Bug 5057532.)
			 */
			try {
				...
				/*
				 * Use the numeric address, which could be either IPv4 or IPv6.
				 * validateHost will accept either.
				 */
			}
		}
	...
	private void validate(String proto, String h, int p, String url) throws MalformedURLException {
		...
		if (protoEnd == 0 || protoEnd < proto.length() || !alphaBitSet.get(proto.charAt(0))) {
			throw new MalformedURLException(
					"Missing or invalid protocol " + "name: \"" + proto + "\"");
		}
	...
	private static void validateHost(String h, int port) throws MalformedURLException {
		...
		if (isNumericIPv6Address(h)) {
			/*
			 * We assume J2SE >= 1.4 here. Otherwise you can't use the address
			 * anyway. We can't call InetAddress.getByName without checking for
			 * a numeric IPv6 address, because we mustn't try to do a DNS lookup
			 * in case the address is not actually numeric.
			 */
			try {
				InetAddress.getByName(h);
			} catch (Exception e) {
				/*
				 * We should really catch UnknownHostException here, but a bug
			...
			/*
			 * Tiny state machine to check valid host name. This checks the
			 * hostname grammar from RFC 1034 (DNS), page 11. A hostname is a
			 * dot-separated list of one or more labels, where each label
			 * consists of letters, numbers, or hyphens. A label cannot begin or
			 * end with a hyphen. Empty hostnames are not allowed. Note that
			 * numeric IPv4 addresses are a special case of this grammar.
			 * 
			 * The state is entirely captured by the last character seen, with a
			 * virtual `.' preceding the name. We represent any alphanumeric
			 * character by `a'.
			 * 
			 * We need a special hack to check, as required by the RFC 2609
			 * (SLP) grammar, that the last component of the hostname begins
			 * with a letter. Respecting the intent of the RFC, we only do this
			 * if there is more than one component. If your local hostname
			 * begins with a digit, we don't reject it.
			 */
			...
			try {
				...
				if (sawDot && !alphaBitSet.get(componentStart)) {
					/*
					 * Must be a numeric IPv4 address. In addition to the
					 * explicitly-thrown exceptions, we can get
					 * NoSuchElementException from the calls to tok.nextToken
					 * and NumberFormatException from the call to
					 * Integer.parseInt. Using exceptions for control flow this
					 * way is a bit evil but it does simplify things enormously.
					 */
	...
	/**
	 * <p>
	 * The host part of the Service URL. If the Service URL was constructed with
	 * the constructor that takes a URL string parameter, the result is the
	 * substring specifying the host in that URL. If the Service URL was
	 * constructed with a constructor that takes a separate host parameter, the
	...
	/**
	 * <p>
	 * The port of the Service URL. If no port was specified, the returned value
	 * is 0.
	 * </p>
	 *
	...
	/**
	 * <p>
	 * The URL Path part of the Service URL. This is an empty string, or a
	 * string beginning with a slash (<code>/</code>), or a string beginning
	...
	/**
	 * <p>
	 * The string representation of this Service URL. If the value returned by
	...
	public String toString() {
		/*
		 * We don't bother synchronizing the access to toString. At worst, n
		 * threads will independently compute and store the same value.
		 */
		if (toString != null)
			return toString;
	 ...
	 * returns true if and only if <code>obj</code> is an instance of
	 * <code>JMXServiceURL</code> whose {@link #getProtocol()},
	 * {@link #getHost()}, {@link #getPort()}, and {@link #getURLPath()} methods
	 * return the same values as for this object. The values for
	 * {@link #getProtocol()} and {@link #getHost()} can differ in case without
	 * affecting equality.
	 *
	 * @param obj
	...
	public boolean equals(Object obj) {
		...
		return (u.getProtocol().equalsIgnoreCase(getProtocol())
				&& u.getHost().equalsIgnoreCase(getHost()) && u.getPort() == getPort()
	...
	static {
		/*
		 * J2SE 1.4 adds lots of handy methods to BitSet that would allow us to
		 * simplify here, e.g. by not writing loops, but we want to work on J2SE
		 * 1.3 too.
		 */
