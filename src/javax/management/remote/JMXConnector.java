 ...
 * created by {@link JMXConnectorFactory#connect(JMXServiceURL, Map)
...
public interface JMXConnector extends Closeable {
	/**
	 * <p>
	 * Name of the attribute that specifies the credentials to send to the
	 * connector server during connection. The value associated with this
	 * attribute, if any, is a serializable object of an appropriate type for
	 * the server's {@link JMXAuthenticator}.
	 */
	 ...
	 * equivalent to {@link #connect(Map) connect(null)}.
	 * </p>
	 *
	 ...
	 * <code>connect</code> will attempt to establish a connection to the
	 * connector server.
	 * </p>
	 *
	 * @param env
	...
	/**
	 * <p>
	 * Returns an <code>MBeanServerConnection</code> object representing a
	 * remote MBean server. For a given <code>JMXConnector</code>, two
	 ...
	 * remote MBean server. The value returned by the MBean server method is the
	 * value returned to the client. If the MBean server method produces an
	 * <code>Exception</code>, the same <code>Exception</code> is seen by the
	 * client. If the MBean server method, or the attempt to call it, produces
	 ...
	 * operations called on the <code>MBeanServerConnection</code> must use the
	 ...
	 *
	 * @exception IOException
	 *                if a valid <code>MBeanServerConnection</code> cannot be
	 *                created, for instance because the connection to the remote
	 *                MBean server has not yet been established (with the
	 *                {@link #connect(Map) connect} method), or it has been
	 ...
	 * <p>
	 * Returns an <code>MBeanServerConnection</code> object representing a
	 * remote MBean server on which operations are performed on behalf of the
	 * supplied delegation subject. For a given <code>JMXConnector</code> and
	 * <code>Subject</code>, two successful calls to this method will usually
	 ...
	 * For each method in the returned <code>MBeanServerConnection</code>,
	 * calling the method causes the corresponding method to be called in the
	 * remote MBean server on behalf of the given delegation subject instead of
	 * the authenticated subject. The value returned by the MBean server method
	 * is the value returned to the client. If the MBean server method produces
	 * an <code>Exception</code>, the same <code>Exception</code> is seen by the
	 * client. If the MBean server method, or the attempt to call it, produces
	 * an <code>Error</code>, the <code>Error</code> is wrapped in a
	 * {@link JMXServerErrorException}, which is seen by the client.
	 * </p>
	 ...
	 *            the <code>Subject</code> on behalf of which requests will be
	 *            performed. Can be null, in which case requests will be
	 *            performed on behalf of the authenticated Subject, if any.
	 *
	 * @return an object that implements the <code>MBeanServerConnection</code>
	 ...
	 * will be made to close the connection. This attempt can succeed, in which
	 ...
	 * protocol timeout. Callers that do not want to block in a close operation
	 * should do it in a separate thread.
	 * </p>
	 *
	 * @exception IOException
	 *                if the connection cannot be closed cleanly. If this
	 *                exception is thrown, it is not known whether the server
	 ...
	 * listener will receive notifications of type
	 ...
	 * filter and handback. There is no special treatment of a duplicate entry.
	 * For example, if a listener is registered twice with no filter, then its
	 * <code>handleNotification</code> method will be called twice for each
	 * notification.
	 * </p>
	 *
	 * @param listener
	 ...
	 * listener must previously have been added. If there is more than one
	 * matching listener, all are removed.
	 * </p>
	 *
	 * @param listener
	 *            a listener to receive connection status notifications.
	 ...
	 *                if the listener is not registered with this
	 ...
	 * <p>
	 * Removes a listener from the list to be informed of changes in status. The
	 * listener must previously have been added with the same three parameters.
	 * If there is more than one matching listener, only one is removed.
	 * </p>
	 *
	 * @param l
	 *            a listener to receive connection status notifications.
	 ...
	 * @exception ListenerNotFoundException
	 *                if the listener is not registered with this
	 *                <code>JMXConnector</code>, or is not registered with the
	 ...
	 * connector server, every connection will have a unique id which does not
	 * change during the lifetime of the connection.
	 * </p>
	 *
	 * @return the unique ID of this connection. This is the same as the ID that
	 *         the connector server includes in its
	 ...
	 *
	 * @exception IOException
	 *                if the connection ID cannot be obtained, for instance
