...
/**
 * <p>
 * A class loader that only knows how to define a limited number of classes, and
 * load a limited number of other classes through delegation to another loader.
 * It is used to get around a problem with Serialization, in particular as used
 * by RMI (including RMI/IIOP). The JMX Remote API defines exactly what class
 * loader must be used to deserialize arguments on the server, and return values
 * on the client. We communicate this class loader to RMI by setting it as the
 * context class loader. RMI uses the context class loader to load classes as it
 * deserializes, which is what we want. However, before consulting the context
 * class loader, it looks up the call stack for a class with a non-null class
 * loader, and uses that if it finds one. So, in the standalone version of
 ...
 * classes required by that byte-code. We construct the byte-code somewhat
 * painstakingly, by compiling the Java code directly, converting into a string,
 * copying that string into the class that needs this loader, and using the
 ...
 * include standard J2SE classes, however. Here, a class is referenced if it is
 * the superclass or a superinterface of a defined class, or if it is the type
 * of a field, parameter, or return value. A class is not referenced if it only
 * appears in the throws clause of a method or constructor. Of course,
 * referencedClassNames should not contain any classes that the user might want
 * to deserialize, because the whole point of this loader is that it does not
 * find such classes.
 */
 ...
 * into a Java program." (interactive) (let* ((s (buffer-string)) (slen (length
 * s)) (i 0) (buf (generate-new-buffer "*string*"))) (set-buffer buf) (insert
 * "\"") (while (< i slen) (if (> (current-column) 61) (insert "\"+\n\"")) (let
 * ((c (aref s i))) (insert (cond ((> c 126) (format "\\%o" c)) ((= c ?\") "
 * \\\"") ((= c ?\\) "\\\\") ((< c 33) (let ((nextc (if (< (1+ i) slen) (aref s
 ...
 * ((value >= 32 && value < 127) && ((!lastWasOctal || (value < '0' || value >
 * '7')))) { s = Character.toString((char)value); } } if (s == null) { s = "\\"
 * + Integer.toString(value, 8); lastWasOctal = true; } else { lastWasOctal =
 * false; } if (pos > 61) { System.out.print("\""); if (i<len)
