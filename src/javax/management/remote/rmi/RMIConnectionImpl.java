...
public class RMIConnectionImpl implements RMIConnection, Unreferenced {
	 ...
	 *
	 * @param rmiServer
	 *            The RMIServerImpl object for which this connection is created.
	 *            The behavior is unspecified if this parameter is null.
	 * @param connectionId
	 *            The ID for this connection. The behavior is unspecified if
	 *            this parameter is null.
	 * @param defaultClassLoader
	 *            The default ClassLoader to be used when deserializing
	 *            marshalled objects. Can be null, to signify the bootstrap
	 *            class loader.
	 * @param subject
	 *            the authenticated subject to be used for authorization. Can be
	 *            null, to signify that no subject has been authenticated.
	 * @param env
	...
	public RMIConnectionImpl(RMIServerImpl rmiServer, String connectionId,
		...
		ClassLoaderRepository repository = AccessController
				.doPrivileged(new PrivilegedAction<ClassLoaderRepository>() {
					public ClassLoaderRepository run() {
						return mbeanServer.getClassLoaderRepository();
					}
				}, withPermissions(new MBeanPermission("*", "getClassLoaderRepository")));
		this.classLoaderWithRepository = AccessController
				.doPrivileged(new PrivilegedAction<ClassLoaderWithRepository>() {
				...
				.doPrivileged(new PrivilegedAction<ClassLoader>() {
					...
					public ClassLoader run() {
						return new CombinedClassLoader(
								Thread.currentThread().getContextClassLoader(), dcl);
					}
				});

		serverCommunicatorAdmin = new RMIServerCommunicatorAdmin(
				EnvHelp.getServerConnectionTimeout(env));
	...
	private synchronized ServerNotifForwarder getServerNotifFwd() {
		...
		if (serverNotifForwarder == null)
			serverNotifForwarder = new ServerNotifForwarder(mbeanServer, env,
					rmiServer.getNotifBuffer(), connectionId);
	...
	public ObjectInstance createMBean(String className, ObjectName name, Subject delegationSubject)
		...
		try {
			...
			if (logger.debugOn())
				logger.debug("createMBean(String,ObjectName)", "connectionId=" + connectionId
		...
		try {
			...
			if (logger.debugOn())
				logger.debug("createMBean(String,ObjectName,ObjectName)",
						"connectionId=" + connectionId + ", className=" + className + ", name="
								+ name + ", loaderName=" + loaderName);

			return (ObjectInstance) doPrivilegedOperation(CREATE_MBEAN_LOADER, params,
					delegationSubject);
		} catch (PrivilegedActionException pe) {
			Exception e = extractException(pe);
			if (e instanceof ReflectionException)
	...
	public ObjectInstance createMBean(String className, ObjectName name, MarshalledObject params,
		...
		if (debug)
			logger.debug("createMBean(String,ObjectName,Object[],String[])", "connectionId="
					+ connectionId + ", unwrapping parameters using classLoaderWithRepository.");

		values = nullIsEmpty(
				unwrap(params, classLoaderWithRepository, Object[].class, delegationSubject));

		try {
			final Object params2[] = new Object[] { className, name, values,
			...
			return (ObjectInstance) doPrivilegedOperation(CREATE_MBEAN_PARAMS, params2,
					delegationSubject);
		} catch (PrivilegedActionException pe) {
			Exception e = extractException(pe);
			if (e instanceof ReflectionException)
	...
	public ObjectInstance createMBean(String className, ObjectName name, ObjectName loaderName,
		...
		values = nullIsEmpty(unwrap(params, getClassLoader(loaderName), defaultClassLoader,
				Object[].class, delegationSubject));

		try {
			final Object params2[] = new Object[] { className, name, loaderName, values,
					nullIsEmpty(signature) };

			if (debug)
				logger.debug("createMBean(String,ObjectName,ObjectName,Object[],String[])",
						"connectionId=" + connectionId + ", className=" + className + ", name="
								+ name + ", loaderName=" + loaderName + ", signature="
								+ strings(signature));
	...
	public ObjectInstance getObjectInstance(ObjectName name, Subject delegationSubject)
		...
		try {
			...
			return (ObjectInstance) doPrivilegedOperation(GET_OBJECT_INSTANCE, params,
					delegationSubject);
		} catch (PrivilegedActionException pe) {
			Exception e = extractException(pe);
			if (e instanceof InstanceNotFoundException)
	...
	public Set<ObjectInstance> queryMBeans(ObjectName name, MarshalledObject query,
		...
		if (debug)
			logger.debug("queryMBeans",
					"connectionId=" + connectionId + " unwrapping query with defaultClassLoader.");
		...
		try {
			...
			if (debug)
				logger.debug("queryMBeans",
						"connectionId=" + connectionId + ", name=" + name + ", query=" + query);
	...
	public boolean isRegistered(ObjectName name, Subject delegationSubject) throws IOException {
		try {
			final Object params[] = new Object[] { name };
			return ((Boolean) doPrivilegedOperation(IS_REGISTERED, params, delegationSubject))
					.booleanValue();
		} catch (PrivilegedActionException pe) {
			Exception e = extractException(pe);
			if (e instanceof IOException)
	...
	public AttributeList setAttributes(ObjectName name, MarshalledObject attributes,
		...
		attrlist = unwrap(attributes, getClassLoaderFor(name), defaultClassLoader,
	...
	public Object invoke(ObjectName name, String operationName, MarshalledObject params,
		...
		values = nullIsEmpty(unwrap(params, getClassLoaderFor(name), defaultClassLoader,
				Object[].class, delegationSubject));

		try {
			final Object params2[] = new Object[] { name, operationName, values,
					nullIsEmpty(signature) };

			if (debug)
				logger.debug("invoke", "connectionId=" + connectionId + ", name=" + name
						+ ", operationName=" + operationName + ", signature=" + strings(signature));
	...
	public Integer[] addNotificationListeners(ObjectName[] names, MarshalledObject[] filters,
		...
		try {
			for (; i < names.length; i++) {
				...
				filterValues[i] = unwrap(filters[i], targetCl, defaultClassLoader,
				...
				ids[i] = (Integer) doPrivilegedOperation(ADD_NOTIFICATION_LISTENERS,
						new Object[] { names[i], filterValues[i] }, sbjs[i]);
			}
	...
	public void addNotificationListener(ObjectName name, ObjectName listener,
		...
		handbackValue = unwrap(handback, targetCl, defaultClassLoader, Object.class,
				delegationSubject);
		...
		try {
			...
			if (debug)
				logger.debug(
						...
						"connectionId=" + connectionId + ", name=" + name + ", listenerName="
								+ listener + ", filter=" + filterValue + ", handback="
								+ handbackValue);
	...
	public void removeNotificationListeners(ObjectName name, Integer[] listenerIDs,
		...
		try {
			...
			if (logger.debugOn())
				logger.debug("removeNotificationListener" + "(ObjectName,Integer[])",
						"connectionId=" + connectionId + ", name=" + name + ", listenerIDs="
								+ objects(listenerIDs));
	...
	public void removeNotificationListener(ObjectName name, ObjectName listener,
		...
		try {
			...
			doPrivilegedOperation(REMOVE_NOTIFICATION_LISTENER_OBJECTNAME, params,
					delegationSubject);
		} catch (PrivilegedActionException pe) {
			Exception e = extractException(pe);
			if (e instanceof InstanceNotFoundException)
	...
	private Object doOperation(int operation, Object[] params) throws Exception {
		...
		case INVOKE:
			return mbeanServer.invoke((ObjectName) params[0], (String) params[1],
					(Object[]) params[2], (String[]) params[3]);

		case IS_INSTANCE_OF:
			return mbeanServer.isInstanceOf((ObjectName) params[0], (String) params[1])
		...
		case REMOVE_NOTIFICATION_LISTENER:
			getServerNotifFwd().removeNotificationListener((ObjectName) params[0],
					(Integer[]) params[1]);
			return null;
	...
	private <T> T unwrap(final MarshalledObject<?> mo, final ClassLoader cl,
			...
			try {
				...
				if (reqACC != null) {
					return AccessController.doPrivileged(
							(PrivilegedExceptionAction<T>) () -> wrappedClass.cast(mo.get()),
							reqACC);
				} else {
	...
	private <T> T unwrap(final MarshalledObject<?> mo, final ClassLoader cl1, final ClassLoader cl2,
		...
		try {
			ClassLoader orderCL = AccessController
			...
			try {
				...
				for (Throwable t = e; t != null; t = t.getCause()) {
					if (t instanceof SecurityException) {
						throw t == e ? (SecurityException) t
								: new SecurityException(t.getMessage(), e);
					}
				}
			}
