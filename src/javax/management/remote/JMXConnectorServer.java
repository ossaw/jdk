 ...
 * connector server stops listening for client connections when the
...
public abstract class JMXConnectorServer extends NotificationBroadcasterSupport
	 ...
	 * server. The value associated with this attribute, if any, must be an
	 * object that implements the interface {@link JMXAuthenticator}.
	 * </p>
	 */
	...
	/**
	 * <p>
	 * Constructs a connector server that will be registered as an MBean in the
	 * MBean server it is attached to. This constructor is typically called by
	 * one of the <code>createMBean</code> methods when creating, within an
	 * MBean server, a connector server that makes it available remotely.
	 * </p>
	 */
	 ...
	 * A connector server that is created in this way can be registered in a
	 * different MBean server, or not registered in any MBean server.
	 * </p>
	 *
	 * @param mbeanServer
	 ...
	 * </p>
	 *
	 * @return the MBean server that this connector server is attached to, or
	 *         null if it is not yet attached to an MBean server.
	 */
	 ...
	 * implementation in <code>JMXConnectorServer</code> returns an array with
	 * one element, indicating that it can emit notifications of class
	 * {@link JMXConnectionNotification} with the types defined in that class. A
	 * subclass that can emit other notifications should return an array that
	 * contains this element plus descriptions of the other notifications.
	 * </p>
	 *
	 ...
	 * <code>connectionId</code> to the list returned by
	 * {@link #getConnectionIds()}, then emits a
	 * {@link JMXConnectionNotification} with type
	 * {@link JMXConnectionNotification#OPENED}.
	 * </p>
	 *
	 * @param connectionId
	 *            the ID of the new connection. This must be different from the
	 *            ID of any connection previously opened by this connector
	 *            server.
	 *
	 * @param message
	 *            the message for the emitted {@link JMXConnectionNotification}.
	 ...
	 * <code>connectionId</code> from the list returned by
	 * {@link #getConnectionIds()}, then emits a
	 * {@link JMXConnectionNotification} with type
	...
	private void sendNotification(String type, String connectionId, String message,
			Object userData) {
		Notification notif = new JMXConnectionNotification(type, getNotificationSource(),
				connectionId, nextSequenceNumber(), message, userData);
	...
	/**
	 * <p>
	 * Called by an MBean server when this connector server is registered in
	 * that MBean server. This connector server becomes attached to the MBean
	 ...
	 * necessarily the one it is being registered in.
	 * </p>
	 *
	 * @param mbs
	 ...
	 *
	 * @exception NullPointerException
	 *                if <code>mbs</code> or <code>name</code> is null.
	 */
	...
	/**
	 * <p>
	 * Called by an MBean server when this connector server is unregistered from
	 * that MBean server. If this connector server was attached to that MBean
	 * server by being registered in it, and if the connector server is still
	 * active, then unregistering it will call the {@link #stop stop} method. If
	 * the <code>stop</code> method throws an exception, the unregistration
	 * attempt will fail. It is recommended to call the <code>stop</code> method
	 * explicitly before unregistering the MBean.
	 * </p>
	 *
	 * @exception IOException
	...
	/**
	 * The name used to registered this server in an MBeanServer. It is null if
	 * the this server is not registered or has been unregistered.
	 */
