...
/**
 * A Permission to perform actions related to MBeanServers. The <em>name</em> of
 * the permission specifies the operation requested or granted by the
 * permission. For a granted permission, it can be <code>*</code> to allow all
 * of the MBeanServer operations specified below. Otherwise, for a granted or
 * requested permission, it must be one of the following:
 * <dl>
 ...
 * <dd>Find an MBeanServer with a given name, or all MBeanServers in this JVM,
 * using the method {@link MBeanServerFactory#findMBeanServer}.
 * <dt>newMBeanServer</dt>
 * <dd>Create a new MBeanServer object without keeping a reference to it, using
 * the method {@link MBeanServerFactory#newMBeanServer()} or
 * {@link MBeanServerFactory#newMBeanServer(java.lang.String)}.
 * <dt>releaseMBeanServer</dt>
 * <dd>Remove the MBeanServerFactory's reference to an MBeanServer, using the
 * method {@link MBeanServerFactory#releaseMBeanServer}.
 * </dl>
 * The <em>name</em> of the permission can also denote a list of one or more
 * comma-separated operations. Spaces are allowed at the beginning and end of
 * the <em>name</em> and before and after commas.
 * <p>
...
public class MBeanServerPermission extends BasicPermission {
	...
	private final static String[] names = { "createMBeanServer", "findMBeanServer",
	 ...
	 * demand.
	 *
	 * This isn't very scalable. If we have more than five or six permissions,
	 * we should consider doing this differently, e.g. with a Map.
	 */
	private final static String[] canonicalNames = new String[1 << N_NAMES];

	/*
	 * The target names mask. This is not private to avoid having to generate
	 * accessor methods for accesses from the collection class.
	 *
	 * This mask includes implied bits. So if it has CREATE_MASK then it
	 * necessarily has NEW_MASK too.
	 */
	transient int mask;
	 ...
	 * <code>MBeanServerPermission(name,null)</code>.
	 * </p>
	 * 
	 * @param name
	 ...
	 *                if the name is not <code>*</code> or one of the allowed
	 *                names or a comma-separated list of the allowed names.
	 */
	 ...
	 *            {@link MBeanServerPermission} class.
	 * @param actions
	 *            the associated actions. This parameter is not currently used
	 *            and must be null or the empty string.
	 * @exception NullPointerException
	 *                if the name is null.
	 * @exception IllegalArgumentException
	 *                if the name is not <code>*</code> or one of the allowed
	 *                names or a comma-separated list of the allowed names, or
	 *                if <code>actions</code> is a non-null non-empty string.
	 *
	...
	public MBeanServerPermission(String name, String actions) {
		 ...
		 * Permission.getName() is final and since we can't access "this" until
		 * after the call to the superclass constructor, there isn't any very
		 * clean way to do this. MBeanServerPermission objects aren't
		 * constructed very often, luckily.
		 */
		mask = parseMask(name);
		...
		if (actions != null && actions.length() > 0)
			throw new IllegalArgumentException(
					"MBeanServerPermission " + "actions must be null: " + actions);
	}
	...
	private static String makeCanonicalName(int mask) {
		...
		/*
		 * intern() avoids duplication when the mask has only one bit, so is
		 * equivalent to the string constants we have for the names[] array.
		 */
	}

	/*
	 * Convert the string into a bitmask, including bits that are implied by the
	 * permissions in the string.
	 */
	 ...
	 * <i>obj</i> is an MBeanServerPermission, and represents the same list of
	 * allowable actions as this object.
	 * <P>
	 * 
	 * @param obj
...
/*
 * Since every collection of MBSP can be represented by a single MBSP, that is
 * what our PermissionCollection does. We need to define a PermissionCollection
 * because the one inherited from BasicPermission doesn't know that
 * createMBeanServer implies newMBeanServer.
 *
 * Though the serial form is defined, the TCK does not check it. We do not
 * require independent implementations to duplicate it. Even though
 * PermissionCollection is Serializable, instances of this class will hardly
 * ever be serialized, and different implementations do not typically exchange
 * serialized permission collections.
 *
 ...
 * implementation from defining a PermissionCollection there with an optimized
 * "implies" method.
 */
class MBeanServerPermissionCollection extends PermissionCollection {
	/**
	 * @serial Null if no permissions in collection, otherwise a single
	 *         permission that is the union of all permissions that have been
	 *         added.
	 */
