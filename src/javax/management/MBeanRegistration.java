 ...
 * after being registered or unregistered from the MBean Server. An MBean can
 * also implement this interface in order to get a reference to the MBean Server
 * and/or its name within that MBean Server.
 * </p>
 *
 * @since 1.5
 */
public interface MBeanRegistration {
	 ...
	 * <p>
	 * If the implementation of this method throws a {@link RuntimeException} or
	 * an {@link Error}, the MBean Server will rethrow those inside a
	 * {@link RuntimeMBeanException} or {@link RuntimeErrorException},
	 * respectively. However, throwing an exception in {@code postRegister} will
	 * not change the state of the MBean: if the MBean was already registered (
	 * {@code registrationDone} is {@code true}), the MBean will remain
	 * registered.
	 * </p>
	 * <p>
	 * This might be confusing for the code calling {@code createMBean()} or
	 * {@code registerMBean()}, as such code might assume that MBean
	 * registration has failed when such an exception is raised. Therefore it is
	 * recommended that implementations of {@code postRegister} do not throw
	 ...
	 * will not change the state of the MBean: the MBean was already
	 * successfully deregistered and will remain so.
	 * </p>
	 * <p>
	 * This might be confusing for the code calling {@code unregisterMBean()},
	 * as it might assume that MBean deregistration has failed. Therefore it is
	 * recommended that implementations of {@code postDeregister} do not throw
	 * Runtime Exceptions or Errors if it can be avoided.
	 * </p>
	 */
