 ...
 * </p>
 * <p>
 * The <em>domain</em> is a string of characters not including the character
 * colon (<code>:</code>). It is recommended that the domain should not contain
 * the string "{@code //}", which is reserved for future use.
 * <p>
 * If the domain includes at least one occurrence of the wildcard characters
 * asterisk (<code>*</code>) or question mark (<code>?</code>), then the object
 * name is a pattern. The asterisk matches any sequence of zero or more
 * characters, while the question mark matches any single character.
 * </p>
 * <p>
 * If the domain is empty, it will be replaced in certain contexts by the
 * <em>default domain</em> of the MBean server in which the ObjectName is used.
 * </p>
 * <p>
 * The <em>key properties</em> are an unordered set of keys and associated
 * values.
 * </p>
 * <p>
 * Each <em>key</em> is a nonempty string of characters which may not contain
 * any of the characters comma (<code>,</code>), equals (<code>=</code>), colon,
 * asterisk, or question mark. The same key may not occur twice in a given
 * ObjectName.
 * </p>
 * <p>
 * Each <em>value</em> associated with a key is a string of characters that is
 * either unquoted or quoted.
 * </p>
 * <p>
 * An <em>unquoted value</em> is a possibly empty string of characters which may
 * not contain any of the characters comma, equals, colon, or quote.
 * </p>
 * <p>
 * If the <em>unquoted value</em> contains at least one occurrence of the
 * wildcard characters asterisk or question mark, then the object name is a
 * <em>property value pattern</em>. The asterisk matches any sequence of zero or
 * more characters, while the question mark matches any single character.
 * </p>
 * <p>
 * A <em>quoted value</em> consists of a quote (<code>"</code>), followed by a
 * possibly empty string of characters, followed by another quote. Within the
 * string of characters, the backslash (<code>\</code>) has a special meaning.
 * It must be followed by one of the following characters:
 * </p>
 * <ul>
 * <li>Another backslash. The second backslash has no special meaning and the
 * two characters represent a single backslash.</li>
 * <li>The character 'n'. The two characters represent a newline ('\n' in Java).
 * </li>
 * <li>A quote. The two characters represent a quote, and that quote is not
 * considered to terminate the quoted value. An ending closing quote must be
 * present for the quoted value to be valid.</li>
 * <li>A question mark (?) or asterisk (*). The two characters represent a
 * question mark or asterisk respectively.</li>
 * </ul>
 * <p>
 * A quote may not appear inside a quoted value except immediately after an odd
 ...
 * </p>
 * <p>
 * If the <em>quoted value</em> contains at least one occurrence of the
 * characters asterisk or question mark and they are not preceded by a
 * backslash, then they are considered as wildcard characters and the object
 * name is a <em>property value pattern</em>. The asterisk matches any sequence
 * of zero or more characters, while the question mark matches any single
 * character.
 * </p>
 * <p>
 * An ObjectName may be a <em>property list pattern</em>. In this case it may
 * have zero or more keys and associated values. It matches a nonpattern
 * ObjectName whose domain matches and that contains the same keys and
 * associated values, as well as possibly other keys and values.
 * </p>
 * <p>
 * An ObjectName is a <em>property value pattern</em> when at least one of its
 * <em>quoted</em> or <em>unquoted</em> key property values contains the
 * wildcard characters asterisk or question mark as described above. In this
 * case it has one or more keys and associated values, with at least one of the
 * values containing wildcard characters. It matches a nonpattern ObjectName
 * whose domain matches and that contains the same keys whose values match; if
 * the property value pattern is also a property list pattern then the
 * nonpattern ObjectName can contain other keys and values.
 * </p>
 * <p>
 * An ObjectName is a <em>property pattern</em> if it is either a
 * <em>property list pattern</em> or a <em>property value pattern</em> or both.
 * </p>
 * <p>
 ...
 * </p>
 * <ul>
 * <li>{@code *:type=Foo,name=Bar} to match names in any domain whose exact set
 * of keys is {@code type=Foo,name=Bar}.</li>
 * <li>{@code d:type=Foo,name=Bar,*} to match names in the domain {@code d} that
 * have the keys {@code type=Foo,name=Bar} plus zero or more other keys.</li>
 * <li>{@code *:type=Foo,name=Bar,*} to match names in any domain that has the
 * keys {@code type=Foo,name=Bar} plus zero or more other keys.</li>
 * <li>{@code d:type=F?o,name=Bar} will match e.g. {@code d:type=Foo,name=Bar}
 * and {@code d:type=Fro,name=Bar}.</li>
 * <li>{@code d:type=F*o,name=Bar} will match e.g. {@code d:type=Fo,name=Bar}
 * and {@code d:type=Frodo,name=Bar}.</li>
 * <li>{@code d:type=Foo,name="B*"} will match e.g.
 * {@code d:type=Foo,name="Bling"}. Wildcards are recognized even inside quotes,
 * and like other special characters can be escaped with {@code \}.</li>
 * </ul>
 * <p>
 ...
 * </ul>
 * <p>
 * A key property list written as a String is a comma-separated list of
 * elements. Each element is either an asterisk or a key property. A key
 * property consists of a key, an equals (<code>=</code>), and the associated
 * value.
 * </p>
 * <p>
 * At most one element of a key property list may be an asterisk. If the key
 * property list contains an asterisk element, the ObjectName is a property list
 * pattern.
 * </p>
 * <p>
 * Spaces have no special significance in a String representing an ObjectName.
 ...
 * the key <code>"&nbsp;key1&nbsp;"</code> also begins and ends with a space.
 * <p>
 * In addition to the restrictions on characters spelt out above, no part of an
 * ObjectName may contain a newline character (<code>'\n'</code>), whether the
 * domain, a key, or a value, whether quoted or unquoted. The newline character
 * can be represented in a quoted value with the sequence <code>\n</code>.
 * <p>
 ...
 * organization that specifies the MBeans, followed by a period and a string
 * whose interpretation is determined by that organization. For example, MBeans
 * specified by <code>example.com</code> would have domains such as
 * <code>com.example.MyDomain</code>. This is essentially the same convention as
 * for Java-language package names.
 * </p>
 * <p>
...
public class ObjectName implements Comparable<ObjectName>, QueryExp {
    ...
    private static final ObjectStreamField[] oldSerialPersistentFields = { new ObjectStreamField("domain",
            String.class), new ObjectStreamField("propertyList", Hashtable.class), new ObjectStreamField(
                    "propertyListString", String.class), new ObjectStreamField("canonicalName", String.class),
            new ObjectStreamField("pattern", Boolean.TYPE), new ObjectStreamField("propertyPattern",
                    Boolean.TYPE) };
    //
    ...
    /**
     * The propertyList of built object name. Initialized lazily. Table that
     * contains all the pairs (key,value) for this ObjectName.
     */
    ...
    /**
     * boolean that declares if this ObjectName contains a pattern on the key
     * property list
     */
    private transient boolean _property_list_pattern = false;

    /**
     * boolean that declares if this ObjectName contains a pattern on the value
     * of at least one key property
     */
    ...
    /**
     * Initializes this {@link ObjectName} from the given string representation.
     *
     * @param name
     *        A string representation of the {@link ObjectName}
        ...
        while (index < len) {
            ...
            if (c == '*') {
                if (_property_list_pattern)
                    throw new MalformedObjectNameException("Cannot have several '*' characters in pattern "
                            + "property list");
                else {
                    _property_list_pattern = true;
                    if ((++index < len) && (name_chars[index] != ','))
                        throw new MalformedObjectNameException("Invalid character found after '*': end of "
                                + "name or ',' expected");
                    else if (index == len) {
                        if (property_index == 0) {
            ...
            while ((in_index < len) && ((c1 = name_chars[in_index++]) != '='))
                switch (c1) {
                    ...
                    case '\n':
                        ...
                        throw new MalformedObjectNameException("Invalid character '" + ichar
                ...
                while ((++in_index < len) && ((c1 = name_chars[in_index]) != '\"')) {
                    ...
                    if (c1 == '\\') {
                        ...
                        switch (c1 = name_chars[in_index]) {
                            ...
                            default:
                                throw new MalformedObjectNameException("Invalid escape sequence '\\" + c1
                                        + "' in quoted value");
                        }
                    } else if (c1 == '\n') {
     ...
     * @param props
     *        Map containing couples <i>key</i> {@literal ->}
     *        <i>value</i>.
     * @exception MalformedObjectNameException
     *            The <code>domain</code> contains
     *            an illegal character, or
     *            one of the keys or values in
     *            <code>table</code> contains
     *            an illegal character, or one of
     *            the values in
     *            <code>table</code> does not
    ...
    private void construct(String domain, Map<String, String> props) throws MalformedObjectNameException {
        ...
        setCanonicalName(initial_chars, canonical_chars, keys, keys_map, _domain_length + 1,
                _kp_array.length);
    }
    // Category : Instance construction <==============================
    ...
    /**
     * Add passed property to the list at the given index for the passed key
     * name
     */
    private void addProperty(Property prop, int index, Map<String, Property> keys_map, String key_name)
    ...
    /**
     * Sets the canonical name of receiver from input 'specified_chars' array,
     * by filling 'canonical_chars' array with found 'nb-props' properties
     * starting at position 'prop_index'.
     */
    private void setCanonicalName(char[] specified_chars, char[] canonical_chars, String[] keys,
            Map<String, Property> keys_map, int prop_index, int nb_props) {
     ...
     * </pre>
     * <p>
     * key starts at startKey (included), and ends at endKey (excluded). If
     * (startKey == endKey), then the key is empty.
     *
     * @param s
     ...
     * </pre>
     * <p>
     * value starts at startVal (included), and ends at endVal (excluded). If
     * (startVal == endVal), then the key is empty.
     *
     * @param s
     *        The char array of the original string.
     * @param startValue
     *        index at which to begin parsing.
     * @return The first element of the int array indicates the index following
     *         the last character of the value. The second element of the int
     *         array indicates that the value is a pattern when its value equals
     *         1.
     **/
                ...
                if (last == '\\') {
                    ...
                    switch (last) {
                        ...
                        case '\"':
                            ...
                            break;
                        default:
                            throw new MalformedObjectNameException("Invalid quoted character sequence '\\"
                                    + last + "'");
                    }
                } else if (last == '\n') {
                    throw new MalformedObjectNameException("Newline in quoted value");
     ...
     *             <code>jmx.serial.form</code> differs from <code>1.0</code>):
     *             the string
     *             &quot;&lt;domain&gt;:&lt;properties&gt;&lt;wild&gt;&quot;,
     *             where:
     *             <ul>
     *             <li>&lt;domain&gt; represents the domain part of the
     *             {@link ObjectName}</li>
     *             <li>&lt;properties&gt; represents the list of properties, as
     *             returned by {@link #getKeyPropertyListString}
     *             <li>&lt;wild&gt; is empty if not
     *             <code>isPropertyPattern</code>, or is the character
     *             "<code>*</code>" if <code>isPropertyPattern</code> and
     *             &lt;properties&gt; is empty, or is "<code>,*</code>" if
     *             <code>isPropertyPattern</code> and &lt;properties&gt; is not
     *             empty.</li>
     *             </ul>
     *             The intent is that this string could be supplied to the
     *             {@link #ObjectName(String)} constructor to produce an
     *             equivalent {@link ObjectName}.</li>
     *             <li>In the old serial form (value of property
     *             <code>jmx.serial.form</code> is <code>1.0</code>):
     *             &lt;domain&gt; &lt;propertyList&gt;
     *             &lt;propertyListString&gt; &lt;canonicalName&gt;
     *             &lt;pattern&gt; &lt;propertyPattern&gt;, where:
     *             <ul>
     *             <li>&lt;domain&gt; represents the domain part of the
     *             {@link ObjectName}</li>
     *             <li>&lt;propertyList&gt; is the {@link Hashtable} that
     *             contains all the pairs (key,value) for this
     *             {@link ObjectName}</li>
     *             <li>&lt;propertyListString&gt; is the {@link String}
     *             representation of the list of properties in any order (not
     *             mandatorily a canonical representation)</li>
     *             <li>&lt;canonicalName&gt; is the {@link String} containing
     *             this {@link ObjectName}'s canonical name</li>
     *             <li>&lt;pattern&gt; is a boolean which is <code>true</code>
     *             if this {@link ObjectName} contains a pattern</li>
     *             <li>&lt;propertyPattern&gt; is a boolean which is
     *             <code>true</code> if this {@link ObjectName} contains a
     *             pattern in the list of properties</li>
     *             </ul>
     *             </li>
     *             </ul>
     */
     ...
     *             <li>&lt;wild&gt; is empty if not
     *             <code>isPropertyPattern</code>, or is the character
     *             "<code>*</code>" if this <code>isPropertyPattern</code> and
     *             &lt;properties&gt; is empty, or is "<code>,*</code>" if
     *             <code>isPropertyPattern</code> and &lt;properties&gt; is not
     *             empty.</li>
     *             </ul>
     ...
     * obtained with {@link #ObjectName(String) new ObjectName(name)} can be
     ...
     * <p>
     * Return an instance of ObjectName that can be used anywhere an object
     * obtained with {@link #ObjectName(String, String, String) new
     * ObjectName(domain, key, value)} can be used. The returned object may be
     * of a subclass of ObjectName. Calling this method twice with the same
     * parameters may return the same object or two equal but not identical
     * objects.
     * </p>
     *
     * @param domain
     ...
     *            <code>key</code>, or
     *            <code>value</code> contains an
     *            illegal character, or
     *            <code>value</code> does not
     *            follow the rules for quoting.
     * @exception NullPointerException
     *            One of the parameters is null.
     */
     ...
     * <p>
     * Return an instance of ObjectName that can be used anywhere an object
     * obtained with {@link #ObjectName(String, Hashtable) new
     * ObjectName(domain, table)} can be used. The returned object may be of a
     * subclass of ObjectName. Calling this method twice with the same
     * parameters may return the same object or two equal but not identical
     * objects.
     * </p>
     *
     ...
     *        The domain part of the object name.
     * @param table
     *        A hash table containing one or more key properties. The key
     *        of
     *        each entry in the table is the key of a key property in the
     *        object name. The associated value in the table is the
     *        associated value in the object name.
     ...
     * ObjectName. If <code>name</code> is of a subclass of ObjectName, it is
     * not guaranteed that the returned object will be of the same class.
     * </p>
     * <p>
     * The returned value may or may not be identical to <code>name</code>.
     * Calling this method twice with the same parameters may return the same
     * object or two equal but not identical objects.
     * </p>
     * <p>
     * Since ObjectName is immutable, it is not usually useful to make a copy of
     * an ObjectName. The principal use of this method is to guard against a
     * malicious caller who might pass an instance of a subclass with surprising
     * behavior to sensitive code. Such code can call this method to obtain an
     * ObjectName that is known not to have surprising behavior.
     * </p>
     *
     * @param name
     ...
     *         same semantics. If <code>name</code> respects the semantics of
     *         ObjectName, then the returned object is equal (though not
     *         necessarily identical) to <code>name</code>.
     * @exception NullPointerException
     *            The <code>name</code> is null.
     */
    ...
    public ObjectName(String domain, Hashtable<String, String> table) throws MalformedObjectNameException {
        construct(domain, table);
        /*
         * The exception for when a key or value in the table is not a String is
         * now ClassCastException rather than MalformedObjectNameException. This
     ...
     * <p>
     * An object name is a pattern if its domain contains a wildcard or if the
     * object name is a property pattern.
     *
     ...
     * <p>
     * An object name is a pattern on the key properties if it is a pattern on
     * the key property list (e.g. "d:k=v,*") or on the property values (e.g.
     * "d:k=*") or on both (e.g. "d:k=*,*").
     *
    ...
    /**
     * Checks whether the object name is a pattern on the value part of at least
     * one of the key properties.
     * <p>
     * For example, "d:k=*" and "d:k=*,*" are property value patterns whereas
     * "d:k=v,*" is not.
     *
    ...
    /**
     * Checks whether the value associated with a key in a key property is a
     * pattern.
     *
     * @param property
     *        The property whose value is to be checked.
     * @return True if the value associated with the given key property is a
     *         pattern, otherwise false.
     * @exception NullPointerException
     *            If <code>property</code> is null.
     * @exception IllegalArgumentException
     *            If <code>property</code> is not a
     *            valid key property for
     *            this ObjectName.
     * @since 1.6
     */
    ...
    /**
     * <p>
     * Returns the canonical form of the name; that is, a string representation
     * where the properties are sorted in lexical order.
     * </p>
     * <p>
     * More precisely, the canonical form of the name is a String consisting of
     * the <em>domain part</em>, a colon (<code>:</code>), the
     * <em>canonical key property list</em>, and a <em>pattern indication</em>.
     * </p>
     * <p>
     * The <em>canonical key property list</em> is the same string as described
     * for {@link #getCanonicalKeyPropertyListString()}.
     * </p>
     * <p>
     * The <em>pattern indication</em> is:
     * <ul>
     * <li>empty for an ObjectName that is not a property list pattern;
     * <li>an asterisk for an ObjectName that is a property list pattern with no
     * keys; or
     * <li>a comma and an asterisk (<code>,*</code>) for an ObjectName that is a
     * property list pattern with at least one key.
     * </ul>
     *
    ...
    /**
     * <p>
     * Returns the key properties as a Map. The returned value is a Map in which
     * each key is a key in the ObjectName's key property list and each value is
    ...
    /**
     * <p>
     * Returns the key properties as a Hashtable. The returned value is a
     * Hashtable in which each key is a key in the ObjectName's key property
     * list and each value is the associated value.
     * </p>
     * <p>
     * The returned value may be unmodifiable. If it is modifiable, changing it
     * has no effect on this ObjectName.
     * </p>
     *
     * @return The table of key properties.
     */
    ...
    /**
     * <p>
     * Returns a string representation of the list of key properties specified
     * at creation time. If this ObjectName was constructed with the constructor
     * {@link #ObjectName(String)}, the key properties in the returned String
     * will be in the same order as in the argument to the constructor.
     * </p>
     *
     * @return The key property list string. This string is independent of
     *         whether the ObjectName is a pattern.
     */
    public String getKeyPropertyListString() {
        ...
        final int total_size = _canonicalName.length() - _domain_length - 1 - (_property_list_pattern ? 2
                : 0);
    ...
    /**
     * <p>
     * Returns the serialized string of the ObjectName. properties specified at
    ...
    /**
     * <p>
     * Write a string representation of the list of key properties specified at
     * creation time in the given array, starting at the specified offset. If
     * this ObjectName was constructed with the constructor
     * {@link #ObjectName(String)}, the key properties in the returned String
     * will be in the same order as in the argument to the constructor.
     * </p>
     *
    ...
    /**
     * Returns a string representation of the list of key properties, in which
     ...
     * their key property list. Lexical order is the order implied by
     * {@link String#compareTo(String) String.compareTo(String)}.
     *
    ...
    /**
     * <p>
     * Returns a string representation of the object name. The format of this
     * string is not specified, but users can expect that two ObjectNames return
     * the same string if and only if they are equal.
     * </p>
     *
     ...
     * canonical form is the string described for {@link #getCanonicalName()}.
     *
     * @param object
     *        The object name that the current object name is to be
     *        compared
     *        with.
     * @return True if <code>object</code> is an ObjectName whose canonical form
     *         is equal to that of this ObjectName.
     */
    @Override
     ...
     * ObjectName. The returned value can be used as the value associated with a
     * key in an ObjectName. The String <code>s</code> may contain any
     * character. Appropriate quoting ensures that the returned value is legal
     * in an ObjectName.
     * </p>
     * <p>
     * The returned value consists of a quote ('"'), a sequence of characters
     * corresponding to the characters of <code>s</code>, and another quote.
     * Characters in <code>s</code> appear unchanged within the returned value
     * except:
     * </p>
     * <ul>
    ...
    /**
     * <p>
     * Returns an unquoted form of the given String. If <code>q</code> is a
     * String returned by {@link #quote quote(s)}, then
     * <code>unquote(q).equals(s)</code>. If there is no String <code>s</code>
     * for which <code>quote(s).equals(q)</code>, then unquote(q) throws an
     ...
     * @exception IllegalArgumentException
     *            if <code>q</code> could not have been
     *            returned by the
     *            {@link #quote} method, for instance
     *            if it does not begin
     *            and end with a quote (").
     * @exception NullPointerException
     *            if <code>q</code> is null.
     */
     ...
     * ObjectName. If <code>name</code> is a pattern, the result is false. If
     * this ObjectName is a pattern, the result is true if and only if
     * <code>name</code> matches the pattern. If neither this ObjectName nor
     * <code>name</code> is a pattern, the result is true if and only if the two
     * ObjectNames are equal as described for the {@link #equals(Object)}
     * method.
     * </p>
     *
     * @param name
    ...
    private final boolean matchKeys(ObjectName name) {
        ...
        //
        if (_property_value_pattern && !_property_list_pattern && (name._ca_array.length != _ca_array.length))
            return false;
    ...
    /*
     * Method inherited from QueryExp, no implementation needed here because
     * ObjectName is not relative to an MBeanServer and does not contain a
     * subquery.
     */
     ...
     * ObjectNames is not completely specified but is intended to be such that a
     * sorted list of ObjectNames will appear in an order that is convenient for
     * a person to read.
     * </p>
     * <p>
     * In particular, if the two ObjectName instances have different domains
     * then their order is the lexicographical order of the domains. The
     * ordering of the key property list remains unspecified.
     * </p>
     * <p>
     ...
     *         ObjectName is less than, equal to, or greater than the specified
     *         ObjectName.
     * @since 1.6
     */
