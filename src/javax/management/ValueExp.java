 ...
 * implementations of <CODE>ValueExp</CODE>.
 *
 * @since 1.5
 */
/*
 * We considered generifying this interface as ValueExp<T>, where T is the Java
 * type that this expression generates. This allows some additional checking in
 * the various methods of the Query class, but in practice not much. Typically
 * you have something like Query.lt(Query.attr("A"), Query.value(5)). We can
 * arrange for Query.value to have type ValueExp<Integer> (or maybe
 * ValueExp<Long> or ValueExp<Number>) but for Query.attr we can't do better
 * than ValueExp<?> or plain ValueExp. So even though we could define Query.lt
 * as: QueryExp <T> lt(ValueExp<T> v1, ValueExp<T> v2) and thus prevent
 * comparing a number against a string, in practice the first ValueExp will
 * almost always be a Query.attr so this check serves no purpose. You would have
 * to write Query.<Number>attr("A"), for example, which would be awful. And, if
 * you wrote Query.<Integer>attr("A") you would then discover that you couldn't
 ...
 * booleanAttr(String name); QueryExp <T> in(ValueExp<T> val, Set<ValueExp<T>>
 * valueSet). But it's not really clear what numberAttr should do if it finds
 * that the attribute is not in fact a Number.
 */
