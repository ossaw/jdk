 ...
 * subsequently removed with {@link #removeNotificationListener
 * removeNotificationListener}. If a filter was provided with that listener, and
 * if the filter's {@link NotificationFilter#isNotificationEnabled
 * isNotificationEnabled} method returns false, the listener is ignored.
 * Otherwise, the listener's {@link NotificationListener#handleNotification
 ...
 * However, if the method call throws an {@link Error}, then it is recommended
 * that processing of the notification stop at that point, and if it is possible
 ...
 * for an implementation to hold any locks while it is calling a listener. To
 * deal with the possibility that the list of listeners might change while a
 * notification is being dispatched, a good strategy is to use a
 * {@link CopyOnWriteArrayList} for this list.
 *
 * @since 1.5
 */
