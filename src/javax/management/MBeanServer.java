...
/**
 * <p>
 * This is the interface for MBean manipulation on the agent side. It contains
 ...
 * {@link javax.management.MBeanServerFactory} class.
 * </p>
 *
 * <p>
 ...
 * {@link MBeanServerFactory#createMBeanServer(String) createMBeanServer} or
 * {@link MBeanServerFactory#newMBeanServer(String) newMBeanServer} methods of
 * the {@link MBeanServerFactory} class applies security checks to its methods,
 * as follows.
 * </p>
 *
 * <p>
 * First, if there is no security manager ({@link System#getSecurityManager()}
 * is null), then an implementation of this interface is free not to make any
 * checks.
 * </p>
 *
 * <p>
 * Assuming that there is a security manager, or that the implementation chooses
 * to make checks anyway, the checks are made as detailed below. In what
 * follows, and unless otherwise specified, {@code className} is the string
 * returned by {@link MBeanInfo#getClassName()} for the target MBean.
 * </p>
 *
 * <p>
 ...
 *
 * <p>
 * For methods that can throw {@link InstanceNotFoundException}, this exception
 * is thrown for a non-existent MBean, regardless of permissions. This is
 * because a non-existent MBean has no <code>className</code>.
 * </p>
 *
 * <ul>
 *
 * <li>
 * <p>
 * For the {@link #invoke invoke} method, the caller's permissions must imply
 * {@link MBeanPermission#MBeanPermission(String,String,ObjectName,String)
 * MBeanPermission(className, operationName, name, "invoke")}.
 * </p>
 *
 * <li>
 * <p>
 * For the {@link #getAttribute getAttribute} method, the caller's permissions
 * must imply
 * {@link MBeanPermission#MBeanPermission(String,String,ObjectName,String)
 ...
 * <li>
 * <p>
 * For the {@link #getAttributes getAttributes} method, the caller's permissions
 * must imply
 * {@link MBeanPermission#MBeanPermission(String,String,ObjectName,String)
 * MBeanPermission(className, null, name, "getAttribute")}. Additionally, for
 * each attribute <em>a</em> in the {@link AttributeList}, if the caller's
 * permissions do not imply
 * {@link MBeanPermission#MBeanPermission(String,String,ObjectName,String)
 * MBeanPermission(className, <em>a</em>, name, "getAttribute")}, the MBean
 * server will behave as if that attribute had not been in the supplied list.
 * </p>
 *
 * <li>
 * <p>
 * For the {@link #setAttribute setAttribute} method, the caller's permissions
 * must imply
 * {@link MBeanPermission#MBeanPermission(String,String,ObjectName,String)
 * MBeanPermission(className, attrName, name, "setAttribute")}, where
 * <code>attrName</code> is {@link Attribute#getName() attribute.getName()}.
 * </p>
 *
 * <li>
 * <p>
 * For the {@link #setAttributes setAttributes} method, the caller's permissions
 * must imply
 * {@link MBeanPermission#MBeanPermission(String,String,ObjectName,String)
 * MBeanPermission(className, null, name, "setAttribute")}. Additionally, for
 * each attribute <em>a</em> in the {@link AttributeList}, if the caller's
 * permissions do not imply
 * {@link MBeanPermission#MBeanPermission(String,String,ObjectName,String)
 * MBeanPermission(className, <em>a</em>, name, "setAttribute")}, the MBean
 * server will behave as if that attribute had not been in the supplied list.
 * </p>
 *
 * <li>
 * <p>
 * For the <code>addNotificationListener</code> methods, the caller's
 ...
 * <li>
 * <p>
 * For the <code>removeNotificationListener</code> methods, the caller's
 * permissions must imply
 * {@link MBeanPermission#MBeanPermission(String,String,ObjectName,String)
 ...
 * <li>
 * <p>
 * For the {@link #getMBeanInfo getMBeanInfo} method, the caller's permissions
 * must imply
 * {@link MBeanPermission#MBeanPermission(String,String,ObjectName,String)
 ...
 * <li>
 * <p>
 * For the {@link #getObjectInstance getObjectInstance} method, the caller's
 * permissions must imply
 * {@link MBeanPermission#MBeanPermission(String,String,ObjectName,String)
 ...
 * <li>
 * <p>
 * For the {@link #isInstanceOf isInstanceOf} method, the caller's permissions
 * must imply
 * {@link MBeanPermission#MBeanPermission(String,String,ObjectName,String)
 ...
 * <li>
 * <p>
 * For the {@link #queryMBeans queryMBeans} method, the caller's permissions
 * must imply
 * {@link MBeanPermission#MBeanPermission(String,String,ObjectName,String)
 * MBeanPermission(null, null, null, "queryMBeans")}. Additionally, for each
 * MBean <em>n</em> that matches <code>name</code>, if the caller's permissions
 * do not imply
 * {@link MBeanPermission#MBeanPermission(String,String,ObjectName,String)
 * MBeanPermission(className, null, <em>n</em>, "queryMBeans")}, the MBean
 ...
 * affected are {@link Query#attr(String)}, {@link Query#attr(String,String)},
 * and {@link Query#classattr()}.
 * </p>
 *
 * <li>
 * <p>
 * For the {@link #queryNames queryNames} method, the checks are the same as for
 * <code>queryMBeans</code> except that <code>"queryNames"</code> is used
 * instead of <code>"queryMBeans"</code> in the <code>MBeanPermission</code>
 * objects. Note that a <code>"queryMBeans"</code> permission implies the
 * corresponding <code>"queryNames"</code> permission.
 * </p>
 *
 * <li>
 * <p>
 * For the {@link #getDomains getDomains} method, the caller's permissions must
 * imply {@link MBeanPermission#MBeanPermission(String,String,ObjectName,String)
 * MBeanPermission(null, null, null, "getDomains")}. Additionally, for each
 * domain <var>d</var> in the returned array, if the caller's permissions do not
 * imply {@link MBeanPermission#MBeanPermission(String,String,ObjectName,String)
 * MBeanPermission(null, null, new ObjectName("<var>d</var>:x=x"),
 * "getDomains")}, the domain is eliminated from the array. Here,
 * <code>x=x</code> is any <var>key=value</var> pair, needed to satisfy
 * ObjectName's constructor but not otherwise relevant.
 * </p>
 *
 * <li>
 * <p>
 ...
 * permissions must imply
 * {@link MBeanPermission#MBeanPermission(String,String,ObjectName,String)
 * MBeanPermission(className, null, loaderName, "getClassLoader")}.
 * </p>
 *
 * <li>
 * <p>
 * For the {@link #getClassLoaderFor getClassLoaderFor} method, the caller's
 * permissions must imply
 * {@link MBeanPermission#MBeanPermission(String,String,ObjectName,String)
 * MBeanPermission(className, null, mbeanName, "getClassLoaderFor")}.
 * </p>
 *
 * <li>
 * <p>
 * For the {@link #getClassLoaderRepository getClassLoaderRepository} method,
 ...
 * <li>
 * <p>
 * For the deprecated <code>deserialize</code> methods, the required permissions
 * are the same as for the methods that replace them.
 * </p>
 *
 * <li>
 * <p>
 * For the <code>instantiate</code> methods, the caller's permissions must imply
 * {@link MBeanPermission#MBeanPermission(String,String,ObjectName,String)
 * MBeanPermission(className, null, null, "instantiate")}, where
 * {@code className} is the name of the class which is to be instantiated.
 * </p>
 *
 * <li>
 * <p>
 * For the {@link #registerMBean registerMBean} method, the caller's permissions
 * must imply
 * {@link MBeanPermission#MBeanPermission(String,String,ObjectName,String)
 * MBeanPermission(className, null, name, "registerMBean")}.
 *
 * <p>
 * If the <code>MBeanPermission</code> check succeeds, the MBean's class is
 * validated by checking that its {@link java.security.ProtectionDomain
 * ProtectionDomain} implies
 * {@link MBeanTrustPermission#MBeanTrustPermission(String)
 * MBeanTrustPermission("register")}.
 * </p>
 *
 * <p>
 * Finally, if the <code>name</code> argument is null, another
 * <code>MBeanPermission</code> check is made using the <code>ObjectName</code>
 * returned by {@link MBeanRegistration#preRegister
 * MBeanRegistration.preRegister}.
 * </p>
 *
 * <li>
 * <p>
 * For the <code>createMBean</code> methods, the caller's permissions must imply
 * the permissions needed by the equivalent <code>instantiate</code> followed by
...
public interface MBeanServer extends MBeanServerConnection {
	 ...
	 *         registered MBean. If the contained <code>ObjectName</code> is
	 ...
	 *
	 * <p>
	 * If this method successfully unregisters an MBean, a notification is sent
	 * as described <a href="#notif">above</a>.
	 * </p>
	 *
	 * @throws RuntimeOperationsException
	 *             {@inheritDoc}
	...
	/**
	 * {@inheritDoc} If the source of the notification is a reference to an
	 ...
	 * the MBean server's {@link javax.management.loading.ClassLoaderRepository
	 * Class Loader Repository}. The object's class should have a public
	 * constructor. This method returns a reference to the newly created object.
	 * The newly created object is not registered in the MBean server.
	 * </p>
	 *
	 * <p>
	 * This method is equivalent to
	 * {@link #instantiate(String,Object[],String[]) instantiate(className,
	 ...
	 * public constructor. This method returns a reference to the newly created
	 * object. The newly created object is not registered in the MBean server.
	 * </p>
	 *
	 * <p>
	 * This method is equivalent to
	 * {@link #instantiate(String,ObjectName,Object[],String[])
	 ...
	 * <p>
	 * Instantiates an object using the list of all class loaders registered in
	 * the MBean server {@link javax.management.loading.ClassLoaderRepository
	 * Class Loader Repository}. The object's class should have a public
	 ...
	 * The class loader is found by loading the class <code>className</code>
	 * through the {@link javax.management.loading.ClassLoaderRepository Class
	 ...
	 *         <li>the result of <var>r</var>
	 *         {@link ClassLoader#loadClass(String) .loadClass(<var>s</var>)} is
	 *         the same as <var>l</var>{@link ClassLoader#loadClass(String)
	 *         .loadClass(<var>s</var>)} for any string <var>s</var>.
	 *         </ul>
	 *
	 *         What this means is that the ClassLoader may be wrapped in another
