 ...
 * interface, and that emits notifications.
 * </p>
 *
 * <p>
 * The following example shows how to use the public constructor
 * {@link #StandardEmitterMBean(Object, Class, NotificationEmitter)
 * StandardEmitterMBean(implementation, mbeanInterface, emitter)} to create an
 * MBean emitting notifications with any implementation class name <i>Impl</i>,
 * with a management interface defined (as for current Standard MBeans) by any
 * interface <i>Intf</i>, and with any implementation of the interface
 * {@link NotificationEmitter}. The example uses the class
 * {@link NotificationBroadcasterSupport} as an implementation of the interface
...
public class StandardEmitterMBean extends StandardMBean implements NotificationEmitter {
	 ...
	 * notifications are handled by the given {@code NotificationEmitter}. The
	 * resultant MBean implements the {@code NotificationEmitter} interface by
	 * forwarding its methods to {@code emitter}. It is legal and useful for
	 * {@code implementation} and {@code emitter} to be the same object.
	 * </p>
	 *
	 * <p>
	 * If {@code emitter} is an instance of {@code
	 * NotificationBroadcasterSupport} then the MBean's {@link #sendNotification
	 * sendNotification} method will call {@code emitter.}
	 * {@link NotificationBroadcasterSupport#sendNotification sendNotification}.
	 * </p>
	 *
	 * <p>
	 * The array returned by {@link #getNotificationInfo()} on the new MBean is
	 * a copy of the array returned by {@code emitter.}
	 * {@link NotificationBroadcaster#getNotificationInfo getNotificationInfo()}
	 * at the time of construction. If the array returned by
	 * {@code emitter.getNotificationInfo()} later changes, that will have no
	 * effect on this object's {@code getNotificationInfo()}.
	 * </p>
	 *
	 ...
	 * NotificationEmitter}. This constructor can be used to make either
	 * Standard MBeans or MXBeans. The resultant MBean implements the
	 * {@code NotificationEmitter} interface by forwarding its methods to
	 * {@code emitter}. It is legal and useful for {@code implementation} and
	 * {@code emitter} to be the same object.
	 * </p>
	 *
	 * <p>
	 * If {@code emitter} is an instance of {@code
	 ...
	 * <p>
	 * Make an MBean whose management interface is specified by
	 * {@code mbeanInterface}, and where notifications are handled by the given
	 * {@code NotificationEmitter}. The resultant MBean implements the
	 * {@code NotificationEmitter} interface by forwarding its methods to
	 * {@code emitter}.
	 * </p>
	 *
	 ...
	 * Make an MBean whose management interface is specified by
	 * {@code mbeanInterface}, and where notifications are handled by the given
	 * {@code NotificationEmitter}. This constructor can be used to make either
	 * Standard MBeans or MXBeans. The resultant MBean implements the
	 * {@code NotificationEmitter} interface by forwarding its methods to
	 * {@code emitter}.
	 * </p>
	 ...
	 *
	 * <p>
	 * If the {@code emitter} parameter to the constructor was an instance of
	 * {@code NotificationBroadcasterSupport} then this method will call
	 * {@code emitter.}{@link NotificationBroadcasterSupport#sendNotification
	 * sendNotification}.
	 * </p>
	 *
	 * @param n
