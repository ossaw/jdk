 ...
 * <P>
 * It implements the MBeanRegistration interface to be able to retrieve its
 * ObjectName and MBean Server.
 *
 * @since 1.5
 */
public class RelationService extends NotificationBroadcasterSupport implements RelationServiceMBean,
     ...
     * Constructor.
     *
     * @param immediatePurgeFlag
     *        flag to indicate when a notification is
     *        received for the
     *        unregistration of an MBean referenced in a
     *        relation, if an
     *        immediate "purge" of the relations (look for
     *        the relations no
     *        longer valid) has to be performed , or if that
     *        will be
     *        performed only when the purgeRelations method
     *        will be
     *        explicitly called.
     *        <P>
     *        true is immediate purge.
    ...
    /**
     * Creates a relation type (a RelationTypeSupport object) with given role
     * infos (provided by the RoleInfo objects), and adds it in the Relation
     * Service.
     *
    ...
    /**
     * Adds given object as a relation type. The object is expected to implement
     * the RelationType interface.
     *
     * @param relationTypeObj
     ...
     *            type with that name
     */
    public void addRelationType(RelationType relationTypeObj) throws IllegalArgumentException,
     ...
     *            with that name.
     */
    public List<RoleInfo> getRoleInfos(String relationTypeName) throws IllegalArgumentException,
    ...
    public RoleInfo getRoleInfo(String relationTypeName, String roleInfoName) throws IllegalArgumentException,
        ...
        RELATION_LOGGER.entering(RelationService.class.getName(), "getRoleInfo", new Object[] {
     ...
     * <P>
     * The relation objects of that type will be removed from the Relation
     * Service.
     *
     * @param relationTypeName
     ...
     *            type with that name
     */
    public void removeRelationType(String relationTypeName) throws RelationServiceNotRegisteredException,
     ...
     * The ones not initialized in this way are set to an empty ArrayList of
     * ObjectNames.
     * <P>
     * A RelationNotification, with type RELATION_BASIC_CREATION, is sent.
     *
     * @param relationId
     *        relation identifier, to identify uniquely the
     *        relation inside
     *        the Relation Service
     * @param relationTypeName
     ...
     *        Service)
     * @param roleList
     *        role list to initialize roles of the relation
     *        (can be null).
     * @exception RelationServiceNotRegisteredException
     *            if the Relation Service
     *            is not registered in the
     ...
     *            for that role
     *            <P>
     *            - an MBean provided for
     *            that role does not exist
     */
    public void createRelation(String relationId, String relationTypeName, RoleList roleList)
            throws RelationServiceNotRegisteredException, IllegalArgumentException, RoleNotFoundException,
        ...
        RELATION_LOGGER.entering(RelationService.class.getName(), "createRelation", new Object[] { relationId,
     ...
     * <P>
     * - have for RelationService ObjectName the ObjectName of current Relation
     * Service
     * <P>
     ...
     *            If the MBean does not
     *            implement the Relation
     *            interface
     * @exception InvalidRelationIdException
     *            if:
     *            <P>
     *            - no relation identifier
     *            in MBean
     *            <P>
     *            - the relation
     ...
     *            if the MBean for given
     *            ObjectName has not been
     *            registered
     ...
     *            in MBean does not
     *            correspond to a
     *            relation type created in
     *            the Relation Service
     * @exception InvalidRoleValueException
     *            if:
     *            <P>
     *            - the number of
     *            referenced MBeans in a
     *            role is less than
     ...
     *            the relation type
     */
    public void addRelation(ObjectName relationObjectName) throws IllegalArgumentException,
            RelationServiceNotRegisteredException, NoSuchMethodException, InvalidRelationIdException,
        ...
        try {
            relServObjName = (ObjectName) (myMBeanServer.getAttribute(relationObjectName,
                    "RelationServiceName"));
        ...
        try {
            myMBeanServer.setAttribute(relationObjectName, new Attribute("RelationServiceManagementFlag",
                    Boolean.TRUE));
        } catch (Exception exc) {
    ...
    /**
     * If the relation is represented by an MBean (created by the user and added
     * as a relation in the Relation Service), returns the ObjectName of the
     * MBean.
     *
     * @param relationId
     *        relation id identifying the relation
     * @return ObjectName of the corresponding relation MBean, or null if the
     *         relation is not an MBean.
     * @exception IllegalArgumentException
     *            if null parameter
     * @exception RelationNotFoundException
     *            there is no relation associated to
     *            that id
     */
    public ObjectName isRelationMBean(String relationId) throws IllegalArgumentException,
    ...
    /**
     * Returns the relation id associated to the given ObjectName if the MBean
     * has been added as a relation in the Relation Service.
     *
     * @param objectName
     *        ObjectName of supposed relation
     * @return relation id (String) or null (if the ObjectName is not a relation
     *         handled by the Relation Service)
     * @exception IllegalArgumentException
     *            if null parameter
     */
     ...
     * @param relationTypeName
     *        name of the relation type
     * @return an Integer wrapping an integer corresponding to possible problems
     *         represented as constants in RoleUnresolved:
     *         <P>
    ...
    public Integer checkRoleReading(String roleName, String relationTypeName) throws IllegalArgumentException,
        ...
        RELATION_LOGGER.entering(RelationService.class.getName(), "checkRoleReading", new Object[] { roleName,
                relationTypeName });
     ...
     *        initialization of a role, write access shall not
     *        be verified.
     * @return an Integer wrapping an integer corresponding to possible problems
     *         represented as constants in RoleUnresolved:
     *         <P>
    ...
    public Integer checkRoleWriting(Role role, String relationTypeName, Boolean initFlag)
        ...
        RELATION_LOGGER.entering(RelationService.class.getName(), "checkRoleWriting", new Object[] { role,
                relationTypeName, initFlag });
     ...
     * <P>
     * - RelationNotification.RELATION_MBEAN_CREATION if the relation is a MBean
     * added as a relation.
     * <P>
     * The source object is the Relation Service itself.
     * <P>
     * It is called in Relation Service createRelation() and addRelation()
     * methods.
     *
     * @param relationId
    ...
    public void sendRelationCreationNotification(String relationId) throws IllegalArgumentException,
        ...
        RELATION_LOGGER.entering(RelationService.class.getName(), "sendRelationCreationNotification",
                relationId);
     ...
     * internal to the Relation Service
     * <P>
     * - RelationNotification.RELATION_MBEAN_UPDATE if the relation is a MBean
     * added as a relation.
     * <P>
     * The source object is the Relation Service itself.
     * <P>
     * It is called in relation MBean setRole() (for given role) and setRoles()
     * (for each role) methods (implementation provided in RelationSupport
     * class).
     * <P>
     ...
     *            relation id
     */
    public void sendRoleUpdateNotification(String relationId, Role newRole, List<ObjectName> oldValue)
        ...
        RELATION_LOGGER.entering(RelationService.class.getName(), "sendRoleUpdateNotification", new Object[] {
     ...
     * object internal to the Relation Service
     * <P>
     * - RelationNotification.RELATION_MBEAN_REMOVAL if the relation is a MBean
     * added as a relation.
     * <P>
     * The source object is the Relation Service itself.
     * <P>
     ...
     *        List of ObjectNames of MBeans expected to be
     *        unregistered due
     *        to relation removal (can be null)
     * @exception IllegalArgumentException
     *            if null parameter
     * @exception RelationNotFoundException
     *            if there is no relation for given
     *            relation id
     */
    public void sendRelationRemovalNotification(String relationId, List<ObjectName> unregMBeanList)
            throws IllegalArgumentException, RelationNotFoundException {
        ...
        RELATION_LOGGER.entering(RelationService.class.getName(), "sendRelationRemovalNotification",
                new Object[] { relationId, unregMBeanList });

        // Can throw RelationNotFoundException
        sendNotificationInt(3, "Removal of relation " + relationId, relationId, unregMBeanList, null, null,
                null);
     ...
     * MBean unregistration) and to be able to perform queries, this method must
     * be called when a role is updated.
     *
     * @param relationId
     *        relation identifier of the updated relation
     * @param newRole
     ...
     *            id.
     */
    public void updateRoleMap(String relationId, Role newRole, List<ObjectName> oldValue)
        ...
        RELATION_LOGGER.entering(RelationService.class.getName(), "updateRoleMap", new Object[] { relationId,
                newRole, oldValue });
     ...
     * <P>
     * - RelationNotification.RELATION_BASIC_REMOVAL if the relation was only
     * internal to the Relation Service
     * <P>
     ...
     *            relation id
     */
    public void removeRelation(String relationId) throws RelationServiceNotRegisteredException,
     ...
     * <P>
     * In that case it is up to the user to call it to maintain the consistency
     * of the relations. To be kept in mind that if an MBean is unregistered and
     * the purge not done immediately, if the ObjectName is reused and assigned
     * to another MBean referenced in a relation, calling manually this
     * purgeRelations() method will cause trouble, as will consider the
     * ObjectName as corresponding to the unregistered MBean, not seeing the new
     * one.
     * <P>
     ...
     * reference is still in the cardinality range, keep the relation and update
     ...
     *        that type
     *        will be considered in the search. Else all
     *        relation types are
     *        considered.
     * @param roleName
     *        can be null; if specified, only the relations
     *        where the MBean
     *        is referenced in that role will be returned. Else
     *        all roles
     *        are considered.
     * @return an HashMap, where the keys are the relation ids of the relations
     *         where the MBean is referenced, and the value is, for each key, an
     *         ArrayList of role names (as an MBean can be referenced in several
     *         roles in the same relation).
     * @exception IllegalArgumentException
     *            if null parameter
     */
    public Map<String, List<String>> findReferencingRelations(ObjectName mbeanName, String relationTypeName,
        ...
        RELATION_LOGGER.entering(RelationService.class.getName(), "findReferencingRelations", new Object[] {
     ...
     *        can be null; if specified, only the relations
     *        where the MBean
     *        is referenced in that role will be considered.
     *        Else all roles
     *        are considered.
     * @return an HashMap, where the keys are the ObjectNames of the MBeans
     *         associated to given MBean, and the value is, for each key, an
     *         ArrayList of the relation ids of the relations where the key
     *         MBean is associated to given one (as they can be associated in
     *         several different relations).
     * @exception IllegalArgumentException
     *            if null parameter
     */
    public Map<ObjectName, List<String>> findAssociatedMBeans(ObjectName mbeanName, String relationTypeName,
            String roleName) throws IllegalArgumentException {
        ...
        RELATION_LOGGER.entering(RelationService.class.getName(), "findAssociatedMBeans", new Object[] {
                mbeanName, relationTypeName, roleName });

        // Retrieves the map <relation id> -> <role names> for those
        // criterias
        Map<String, List<String>> relId2RoleNamesMap = findReferencingRelations(mbeanName, relationTypeName,
                roleName);
     ...
     *            with that name.
     */
    public List<String> findRelationsOfType(String relationTypeName) throws IllegalArgumentException,
            RelationTypeNotFoundException {
     ...
     * @see #setRole
     */
    public List<ObjectName> getRole(String relationId, String roleName)
        ...
        RELATION_LOGGER.entering(RelationService.class.getName(), "getRole", new Object[] { relationId,
                roleName });
        ...
        if (relObj instanceof RelationSupport) {
            ...
            try {
                List<ObjectName> invokeResult = cast(myMBeanServer.invoke(((ObjectName) relObj), "getRole",
                        params, signature));
    ...
    public RoleResult getRoles(String relationId, String[] roleNameArray)
        ...
        if (relObj instanceof RelationSupport) {
            ...
            // or MBeanException
            try {
                result = (RoleResult) (myMBeanServer.invoke(((ObjectName) relObj), "getRoles", params,
                        signature));
            } catch (InstanceNotFoundException exc1) {
                throw new RuntimeException(exc1.getMessage());
            } catch (ReflectionException exc2) {
     ...
     *            Server
     */
    public RoleResult getAllRoles(String relationId) throws IllegalArgumentException,
            RelationNotFoundException, RelationServiceNotRegisteredException {
    ...
    public Integer getRoleCardinality(String relationId, String roleName) throws IllegalArgumentException,
        ...
        RELATION_LOGGER.entering(RelationService.class.getName(), "getRoleCardinality", new Object[] {
                relationId, roleName });
        ...
        if (relObj instanceof RelationSupport) {
            ...
            // Shall not throw InstanceNotFoundException or ReflectionException
            try {
                result = (Integer) (myMBeanServer.invoke(((ObjectName) relObj), "getRoleCardinality", params,
                        signature));
            } catch (InstanceNotFoundException exc1) {
                throw new RuntimeException(exc1.getMessage());
            } catch (ReflectionException exc2) {
     ...
     * @see #getRole
     */
    public void setRole(String relationId, Role role) throws RelationServiceNotRegisteredException,
        ...
        RELATION_LOGGER.entering(RelationService.class.getName(), "setRole", new Object[] { relationId,
                role });
     ...
     * provided in relation's relation type
     * <P>
     * The Relation Service keeps track of the changes to keep the consistency
     * of relations by handling referenced MBean deregistrations.
     *
     * @param relationId
     *        relation id
     * @param roleList
    ...
    public RoleResult setRoles(String relationId, RoleList roleList)
        ...
        RELATION_LOGGER.entering(RelationService.class.getName(), "setRoles", new Object[] { relationId,
                roleList });
        ...
        if (relObj instanceof RelationSupport) {
            ...
            try {
                result = (RoleResult) (myMBeanServer.invoke(((ObjectName) relObj), "setRoles", params,
                        signature));
            } catch (InstanceNotFoundException exc1) {
                throw new RuntimeException(exc1.getMessage());
            } catch (ReflectionException exc3) {
     ...
     *            if no relation for given relation id
     */
    public String getRelationTypeName(String relationId) throws IllegalArgumentException,
            RelationNotFoundException {
    ...
    /**
     * Invoked when a JMX notification occurs. Currently handles notifications
     * for unregistration of MBeans, either referenced in a relation role or
     * being a relation itself.
     *
     * @param notif
     *        The notification.
     * @param handback
     *        An opaque object which helps the listener to associate
     *        information regarding the MBean emitter (can be null).
     */
    ...
    public MBeanNotificationInfo[] getNotificationInfo() {
        ...
        String[] ntfTypes = new String[] { RelationNotification.RELATION_BASIC_CREATION,
                RelationNotification.RELATION_MBEAN_CREATION, RelationNotification.RELATION_BASIC_UPDATE,
                RelationNotification.RELATION_MBEAN_UPDATE, RelationNotification.RELATION_BASIC_REMOVAL,
    ...
    // type with that name
    private void addRelationTypeInt(RelationType relationTypeObj) throws IllegalArgumentException,
            InvalidRelationTypeException {
    ...
    //
    RelationType getRelationType(String relationTypeName) throws IllegalArgumentException,
            RelationTypeNotFoundException {
    ...
    private boolean addNewMBeanReference(ObjectName objectName, String relationId, String roleName)
        ...
        RELATION_LOGGER.entering(RelationService.class.getName(), "addNewMBeanReference", new Object[] {
    ...
    private boolean removeMBeanReference(ObjectName objectName, String relationId, String roleName,
            boolean allRolesFlag) throws IllegalArgumentException {
        ...
        RELATION_LOGGER.entering(RelationService.class.getName(), "removeMBeanReference", new Object[] {
    ...
    private void updateUnregistrationListener(List<ObjectName> newRefList, List<ObjectName> obsoleteRefList)
        ...
        RELATION_LOGGER.entering(RelationService.class.getName(), "updateUnregistrationListener",
        ...
        if (newRefList != null || obsoleteRefList != null) {
            ...
            synchronized (myUnregNtfFilter) {
                ...
                if (newListenerFlag) {
                    try {
                        myMBeanServer.addNotificationListener(MBeanServerDelegate.DELEGATE_NAME, this,
                                myUnregNtfFilter, null);
    ...
    private void addRelationInt(boolean relationBaseFlag, RelationSupport relationObj,
            ObjectName relationObjName, String relationId, String relationTypeName, RoleList roleList)
            throws IllegalArgumentException, RelationServiceNotRegisteredException, RoleNotFoundException,
            InvalidRelationIdException, RelationTypeNotFoundException, InvalidRoleValueException {

        if (relationId == null || relationTypeName == null || (relationBaseFlag && (relationObj == null
                || relationObjName != null)) || (!relationBaseFlag && (relationObjName == null
                        || relationObj != null))) {
            String excMsg = "Invalid parameter.";
            throw new IllegalArgumentException(excMsg);
        }

        RELATION_LOGGER.entering(RelationService.class.getName(), "addRelationInt", new Object[] {
                relationBaseFlag, relationObj, relationObjName, relationId, relationTypeName, roleList });
        ...
        // Can throw InvalidRoleValueException
        initializeMissingRoles(relationBaseFlag, relationObj, relationObjName, relationId, relationTypeName,
                roleInfoList);
    ...
    //
    // -exception IllegalArgumentException if null parameter
    private Integer checkRoleInt(int chkType, String roleName, List<ObjectName> roleValue, RoleInfo roleInfo,
            boolean writeChkFlag) throws IllegalArgumentException {
        ...
        RELATION_LOGGER.entering(RelationService.class.getName(), "checkRoleInt", new Object[] { chkType,
                roleName, roleValue, roleInfo, writeChkFlag });
    ...
    private void initializeMissingRoles(boolean relationBaseFlag, RelationSupport relationObj,
            ObjectName relationObjName, String relationId, String relationTypeName,
            List<RoleInfo> roleInfoList) throws IllegalArgumentException,
            RelationServiceNotRegisteredException, InvalidRoleValueException {

        if ((relationBaseFlag && (relationObj == null || relationObjName != null)) || (!relationBaseFlag
                && (relationObjName == null || relationObj != null)) || relationId == null
                || relationTypeName == null || roleInfoList == null) {
            String excMsg = "Invalid parameter.";
            throw new IllegalArgumentException(excMsg);
        }

        RELATION_LOGGER.entering(RelationService.class.getName(), "initializeMissingRoles", new Object[] {
                relationBaseFlag, relationObj, relationObjName, relationId, relationTypeName, roleInfoList });
    ...
    static void throwRoleProblemException(int pbType, String roleName) throws IllegalArgumentException,
            RoleNotFoundException, InvalidRoleValueException {
    ...
    private void sendNotificationInt(int intNtfType, String message, String relationId,
            List<ObjectName> unregMBeanList, String roleName, List<ObjectName> roleNewValue,
            List<ObjectName> oldValue) throws IllegalArgumentException, RelationNotFoundException {

        if (message == null || relationId == null || (intNtfType != 3 && unregMBeanList != null)
                || (intNtfType == 2 && (roleName == null || roleNewValue == null || oldValue == null))) {
            String excMsg = "Invalid parameter.";
            throw new IllegalArgumentException(excMsg);
        }

        RELATION_LOGGER.entering(RelationService.class.getName(), "sendNotificationInt", new Object[] {
                intNtfType, message, relationId, unregMBeanList, roleName, roleNewValue, oldValue });
        ...
        if (ntfType.equals(RelationNotification.RELATION_BASIC_CREATION) || ntfType.equals(
                RelationNotification.RELATION_MBEAN_CREATION) || ntfType.equals(
                        RelationNotification.RELATION_BASIC_REMOVAL) || ntfType.equals(
                                RelationNotification.RELATION_MBEAN_REMOVAL))

            // Creation or removal
            ntf = new RelationNotification(ntfType, this, seqNo.longValue(), timeStamp, message, relationId,
                    relTypeName, relObjName, unregMBeanList);

        else if (ntfType.equals(RelationNotification.RELATION_BASIC_UPDATE) || ntfType.equals(
                RelationNotification.RELATION_MBEAN_UPDATE)) {
            // Update
            ntf = new RelationNotification(ntfType, this, seqNo.longValue(), timeStamp, message, relationId,
                    relTypeName, relObjName, roleName, roleNewValue, oldValue);
        }
    ...
    private void handleReferenceUnregistration(String relationId, ObjectName objectName,
            List<String> roleNameList) throws IllegalArgumentException, RelationServiceNotRegisteredException,
            RelationNotFoundException, RoleNotFoundException {
        ...
        RELATION_LOGGER.entering(RelationService.class.getName(), "handleReferenceUnregistration",
        ...
        if (deleteRelFlag) {
            ...
            for (String currRoleName : roleNameList) {
                ...
                if (relObj instanceof RelationSupport) {
                    ...
                    // above)
                    try {
                        ((RelationSupport) relObj).handleMBeanUnregistrationInt(objectName, currRoleName,
                                true, this);
                    } catch (RelationTypeNotFoundException exc3) {
                        throw new RuntimeException(exc3.getMessage());
                    ...
                    // ReflectionException
                    try {
                        myMBeanServer.invoke(((ObjectName) relObj), "handleMBeanUnregistration", params,
                                signature);
                    } catch (InstanceNotFoundException exc1) {
                        throw new RuntimeException(exc1.getMessage());
                    } catch (ReflectionException exc3) {
