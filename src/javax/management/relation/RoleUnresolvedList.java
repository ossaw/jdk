 ...
 * representing roles not retrieved from a relation due to a problem encountered
 * when trying to access (read or write) the roles.
 *
 * @since 1.5
 */
/*
 * We cannot extend ArrayList<RoleUnresolved> because our legacy
 * add(RoleUnresolved) method would then override add(E) in ArrayList<E>, and
 * our return value is void whereas ArrayList.add(E)'s is boolean. Likewise for
 * set(int,RoleUnresolved). Grrr. We cannot use covariance to override the most
 * important methods and have them return RoleUnresolved, either, because that
 * would break subclasses that override those methods in turn (using the
 * original return type of Object). Finally, we cannot implement
 * Iterable<RoleUnresolved> so you could write for (RoleUnresolved r :
 * roleUnresolvedList) because ArrayList<> implements Iterable<> and the same
 * class cannot implement two versions of a generic interface. Instead we
 * provide the asList() method so you can write for (RoleUnresolved r :
...
public class RoleUnresolvedList extends ArrayList<Object> {
	 ...
	 * {@code List}'s iterator. The {@code RoleUnresolvedList} instance has an
	 ...
	 *                {@code list} parameter contains any non-RoleUnresolved
	 *                objects.
	 *
	...
	/**
	 * Return a view of this list as a {@code List<RoleUnresolved>}. Changes to
	 * the returned value are reflected by changes to the original
	 * {@code RoleUnresolvedList} and vice versa.
	 *
	 ...
	 *         will fail with an {@code IllegalArgumentException}. For
	 *         compatibility reasons, a {@code RoleUnresolvedList} on which this
	 *         method has never been called does allow objects other than
	 *         {@code RoleUnresolved}s to be added.
	 *         </p>
	 *
	 * @throws IllegalArgumentException
	 *             if this {@code RoleUnresolvedList} contains an element that
	 ...
	 * specified. Elements with an index greater than or equal to the current
	 * position are shifted up.
	 *
	 * @param index
	 *            - The position in the list where the new RoleUnresolved object
	 *            is to be inserted.
	 * @param role
	 ...
	 * non-RoleUnresolved to the List, and disallow that if asList has ever been
