 ...
 */
/*
 * We cannot extend ArrayList<Role> because our legacy add(Role) method would
 * then override add(E) in ArrayList<E>, and our return value is void whereas
 * ArrayList.add(E)'s is boolean. Likewise for set(int,Role). Grrr. We cannot
 * use covariance to override the most important methods and have them return
 * Role, either, because that would break subclasses that override those methods
 * in turn (using the original return type of Object). Finally, we cannot
 * implement Iterable<Role> so you could write for (Role r : roleList) because
 * ArrayList<> implements Iterable<> and the same class cannot implement two
 * versions of a generic interface. Instead we provide the asList() method so
 * you can write for (Role r : roleList.asList())
 */
public class RoleList extends ArrayList<Object> {
	...
	/**
	 * Constructs a {@code RoleList} containing the elements of the {@code List}
	 * specified, in the order in which they are returned by the {@code List}'s
	 * iterator. The {@code RoleList} instance has an initial capacity of 110%
	 ...
	 *                {@code list} parameter contains any non-Role objects.
	 *
	...
	/**
	 * Return a view of this list as a {@code List<Role>}. Changes to the
	 * returned value are reflected by changes to the original {@code RoleList}
	 * and vice versa.
	 *
	 ...
	 *         which is not a {@code Role} will fail with an
	 *         {@code IllegalArgumentException}. For compatibility reasons, a
	 *         {@code RoleList} on which this method has never been called does
	 *         allow objects other than {@code Role}s to be added.
	 *         </p>
	 *
	 * @throws IllegalArgumentException
	 *             if this {@code RoleList} contains an element that is not a
	 *             {@code Role}.
	 *
	 * @since 1.6
	 */
	 ...
	 * non-Role to the List, and disallow that if asList has ever been called on
	 * this instance.
	 */
