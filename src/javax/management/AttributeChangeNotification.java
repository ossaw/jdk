 ...
 * <CODE>NotificationBroadcaster</CODE> interface has to be implemented by any
 * MBean for which an attribute change is of interest.
 * <P>
 * Example: If an MBean called <CODE>myMbean</CODE> needs to notify registered
 * listeners when its attribute: <BLOCKQUOTE><CODE>
 *      String myString
 * </CODE></BLOCKQUOTE> is modified, <CODE>myMbean</CODE> creates and emits the
 * following notification: <BLOCKQUOTE><CODE>
...
public class AttributeChangeNotification extends javax.management.Notification {
	...
	/**
	 * Notification type which indicates that the observed MBean attribute value
	 * has changed. <BR>
	 ...
	 * information common to all notification, the caller must supply the name
	 * and type of the attribute, as well as its old and new values.
	 *
	 * @param source
	 *            The notification producer, that is, the MBean the attribute
	 *            belongs to.
	 ...
	 * @param oldValue
	 *            An object representing value of the attribute before the
	 *            change.
	 * @param newValue
	 *            An object representing value of the attribute after the
	 *            change.
	 */
	public AttributeChangeNotification(Object source, long sequenceNumber, long timeStamp,
			String msg, String attributeName, String attributeType, Object oldValue,
			Object newValue) {
