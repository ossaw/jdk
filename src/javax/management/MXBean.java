...
/**
 * <p>
 * Annotation to mark an interface explicitly as being an MXBean interface, or
 * as not being an MXBean interface. By default, an interface is an MXBean
 * interface if it is public and its name ends with {@code MXBean}, as in
 * {@code SomethingMXBean}. The following interfaces are MXBean interfaces:
 * </p>
 * 
 * <pre>
 ...
 * {@link javax.management.openmbean}. In this way, you can be sure that your
 * MBean will be usable by any client, including remote clients, without any
 ...
 * gives a coherent snapshot of a set of data items. For example, it might
 ...
 * separate {@link MBeanServer#getAttribute getAttribute} calls, then we could
 ...
 * <code>Serializable</code> (though it can be). On the other hand, we have
 ...
 * attribute if it does not know the class <code>MemoryUsage</code>. Suppose the
 * client is a generic console based on JMX technology. Then the console would
 * have to be configured with the model-specific classes of every application it
 * might connect to. The problem is even worse for clients that are not written
 ...
 * <em>converts</em> model-specific classes into standard classes from the Java
 * platform. Using arrays and the
 * {@link javax.management.openmbean.CompositeData CompositeData} and
 * {@link javax.management.openmbean.TabularData TabularData} classes from the
 ...
 * <code>MemoryPoolMXBean</code> and the class <code>MemoryUsage</code>, then it
 * can construct a <em>proxy</em>. This is the recommended way to interact with
 ...
 * <code><em>N</em></code> is a non-empty string, specifies that there is a
 * readable attribute called <code><em>N</em></code>. The Java type and Open
 ...
 * the type <code><em>T</em></code> in both must be the same. In this case the
 ...
 * describable using <em>Open Types</em>, that is the four standard subclasses
 ...
 * <li>The <em>mapped</em> Java type, <em>opendata(J)</em>, which is always the
 * same for any given <em>opentype(J)</em>. This is a Java class.</li>
 ...
 * {@link SimpleType#STRING}<code>)</code>, representing a 1-dimensional array
 * of <code>String</code>s.</li>
 * <li>The mapped Java type, <em>opendata(</em>{@code
   List<String>}<em>)</em>, is {@code String[]}.</li>
 * <li>A {@code List<String>} can be converted to a {@code String[]} using
 * {@link List#toArray(Object[]) List.toArray(new String[0])}.</li>
 ...
 * MXBean interface must be reconstructible, because when the MXBean framework
 * is invoking a method it will need to convert those parameters from
 * <em>opendata(J)</em> to <em>J</em>. In a proxy generated by
 * {@link JMX#newMXBeanProxy(MBeanServerConnection, ObjectName, Class)
 ...
 * <tr>
 * <td><em>E</em>{@code []}<br>
 * (an array with non-primitive element type <em>E</em>; this includes
 * {@code int[][]}, where <em>E</em> is {@code int[]})</td>
 ...
 * (declared in Java as {@code enum }<em>E</em> {@code {...}})</td>
 ...
 * {@code SimpleType}. Thus, <em>opentype(</em>{@code
 long}<em>)</em> is {@code SimpleType.LONG}, and <em>opendata(</em>{@code long}
 * <em>)</em> is {@code
 java.lang.Long}.
 * </p>
 * 
 ...
 * as an Open Type. Thus, <em>openType(</em>{@code
 long[]}<em>)</em> is {@code
 ArrayType.getPrimitiveArrayType(long[].class)}, and <em>opendata(</em>
 ...
 * interface that implements {@link java.lang.Comparable}. Thus, a
 * {@code SortedSet<String>} or {@code SortedSet<Integer>} is convertible, but a
 * {@code
   SortedSet<int[]>} or {@code SortedSet<List<String>>} is not. The conversion
 * of a {@code SortedSet} instance will fail with an
 ...
 * {@code >} as a {@code java.util.HashSet<}<em>E</em>{@code >}; a
 ...
 * implements {@link java.lang.Comparable}. Thus, a {@code SortedMap
 * <String,int[]>} is convertible, but a {@code SortedMap<int[],String>} is not.
 * The conversion of a {@code SortedMap} instance will fail with an {@code
 ...
 * interface. The {@code ModuleMXBean} object and the returned {@code
 ...
 * {@code SimpleType.OBJECTNAME}, and the corresponding {@code ObjectName} value
 ...
 * into a {@code ProductMXBean} by making another MXBean proxy. More formally,
 * when a proxy made with
 * {@link JMX#newMXBeanProxy(MBeanServerConnection, ObjectName, Class)
 * JMX.newMXBeanProxy(mbeanServerConnection, objectNameX, interfaceX)} needs to
 * map {@code objectNameY} back into {@code
 interfaceY}, another MXBean interface, it does so with {@code
 JMX.newMXBeanProxy(mbeanServerConnection, objectNameY,
 interfaceY)}. The implementation may return a proxy that was previously created
 ...
 * attribute is an {@code ObjectName}. When the attribute is set, the
 * {@code ObjectName} must be converted into the {@code ProductMXBean} object
 ...
 * argument back into an {@code ObjectName}. This will only work if the argument
 * is in fact another proxy, for a {@code ProductMXBean} in the same {@code
 MBeanServerConnection}. The proxy can have been returned from another proxy
 * (like {@code ModuleMXBean.getProduct()} which returns a proxy for a
 * {@code ProductMXBean}); or it can have been created by
 * {@link JMX#newMXBeanProxy(MBeanServerConnection, ObjectName, Class)
 * JMX.newMXBeanProxy}; or it can have been created using
 ...
 * if an MXBean object is already registered in an MBean Server and an attempt
 * is made to register it in the same MBean Server under another name, the
 * result is an {@link InstanceAlreadyExistsException}. Registering the same
 ...
 * interface, a method {@code getOwner} defines an attribute called
 * {@code Owner}, while in a Java Bean or mapped {@code CompositeType}, a method
 * {@code
 ...
 * instance of <em>J</em> to a {@code CompositeData} corresponding to the
 * {@code CompositeType} just described is done as follows. First, if <em>J</em>
 * implements the interface {@link CompositeDataView}, then that interface's
 * {@link CompositeDataView#toCompositeData toCompositeData} method is called to
 * do the conversion. Otherwise, the {@code CompositeData} is constructed by
 ...
 * {@code CompositeData}, or <em>J</em> is not reconstructible. If any item in
 ...
 * {@link java.beans.ConstructorProperties ConstructorProperties} annotation,
 * then one of those constructors (not necessarily always the same one) will be
 * called to reconstruct an instance of <em>J</em>. Every such annotation must
 * list as many strings as the constructor has parameters; each string must name
 * a property corresponding to a getter of <em>J</em>; and the type of this
 * getter must be the same as the corresponding constructor parameter. It is not
 * an error for there to be getters that are not mentioned in the
 ...
 * constructor to be called will be determined at runtime based on the items
 * actually present in the {@code CompositeData}, given that this {@code
 CompositeData} might come from an earlier version of <em>J</em> where not all
 * the items were present. A constructor is <em>applicable</em> if all the
 * properties named in its {@code ConstructorProperties} annotation are present
 ...
 * applicable constructor, or (c) one of the applicable constructors names a
 * proper superset of the properties named by each other applicable constructor.
 * (In other words, there should never be ambiguity over which constructor to
 * choose.) If this condition is not true, then <em>J</em> is not
 * reconstructible.
 * </p>
 * </li>
 * 
 ...
 * getter in <em>J</em> with type <em>T</em> and name <em>N</em> there is a
 * corresponding setter with the same name and type, then an instance of
 * <em>J</em> is constructed with the no-arg constructor and the setters are
 * called with the reconstructed items from the {@code CompositeData} to restore
 * the values. For example, if there is a method<br>
 ...
 * getters, an instance of <em>J</em> is constructed using a
 * {@link java.lang.reflect.Proxy} with a {@link CompositeDataInvocationHandler}
 * backed by the {@code
 ...
 * include the {@code java.beans} package, and where there is a mismatch between
 * the compile-time and runtime environment whereby <em>J</em> is compiled with
 ...
 * after it has been constructed. Notice that {@code CompositeData} itself is
 * immutable. Immutability has many advantages, notably with regard to
 ...
 * the following type could not be the type of an attribute, because it refers
 * to itself:
 * </p>
 * 
 * <pre>
 ...
 * type of an attribute, parameter, or operation return value is a primitive
 * type such as {@code int}, or is {@code void} (for a return type), then the
 * attribute, parameter, or operation will be represented respectively by an
 * {@link MBeanAttributeInfo}, {@link MBeanParameterInfo}, or
 * {@link MBeanOperationInfo} whose {@code getType()} or {@code
 getReturnType()} returns the primitive name ("{@code int}" etc). This is so
 ...
 * in the MBean Server will contain all of the public constructors of that
 * MXBean. If the class of the MXBean is not public then its constructors are
 * not considered public either. The list returned for an MXBean that is
 * constructed using the {@link StandardMBean} class is derived in the same way
 * as for Standard MBeans. Regardless of how the MXBean was constructed, its
 ...
 * registered in the MBean Server will be empty if the MXBean does not implement
 * the {@link NotificationBroadcaster} interface. Otherwise, it will be the
 * result of calling {@link NotificationBroadcaster#getNotificationInfo()} at
 * the time the MXBean was registered. Even if the result of this method changes
 * subsequently, the result of {@code MBeanInfo.getNotifications()} will not.
 ...
 * in the {@code MBeanInfo} will have a field {@code openType} whose value is
 * the {@link OpenType} specified by the mapping rules above. So even when
 * {@code getType()} is "{@code int}",
 ...
 * {@code Class.getName()}; a left angle bracket ({@code
 "<"}); <em>genericstring(A)</em> where <em>A</em> is the first type parameter;
 ...
 * interface is being analyzed, for example if it references a type like
 * {@link java.util.Random java.util.Random} that has no getters. Or it can
 * happen when an instance is being converted (a return value from a method in
 * an MXBean or a parameter to a method in an MXBean proxy), for example when
 * converting from {@code SortedSet<String>} to {@code
 ...
 * MXBean interface is being analyzed, for example if it references a type that
 * is not <em>reconstructible</em> according to the rules above, in a context
 * where a reconstructible type is required. Or it can happen when an instance
 * is being converted (a parameter to a method in an MXBean or a return value
 ...
 * {@link RuntimeMBeanException} or {@link UndeclaredThrowableException}. For
 * every thrown exception, the condition <em>C</em> will be true: "<em>e</em> is
 * {@code
 OpenDataException} or {@code InvalidObjectException} (as appropriate), or
 * <em>C</em> is true of <em>e</em>.{@link Throwable#getCause() getCause()}".
 * </p>
 * 
 * @since 1.6
 */
