...
/**
 * <p>
 * Provides an implementation of {@link javax.management.NotificationEmitter
 ...
 * is called within that thread. You can override this default by overriding
...
public class NotificationBroadcasterSupport implements NotificationEmitter {
	/**
	 * Constructs a NotificationBroadcasterSupport where each listener is
	 * invoked by the thread sending the notification. This constructor is
	 * equivalent to
	...
	/**
	 * Constructs a NotificationBroadcasterSupport where each listener is
	 * invoked using the given {@link java.util.concurrent.Executor}. When
	 * {@link #sendNotification sendNotification} is called, a listener is
	 * selected if it was added with a null {@link NotificationFilter}, or if
	 * {@link NotificationFilter#isNotificationEnabled isNotificationEnabled}
	 * returns true for the notification being sent. The call to
	 * <code>NotificationFilter.isNotificationEnabled</code> takes place in the
	 * thread that called <code>sendNotification</code>. Then, for each selected
	 * listener, {@link Executor#execute executor.execute} is called with a
	 * command that calls the <code>handleNotification</code> method. This
	 * constructor is equivalent to
	 * {@link NotificationBroadcasterSupport#NotificationBroadcasterSupport(Executor,
	 * MBeanNotificationInfo[] info) NotificationBroadcasterSupport(executor,
	 * null)}.
	 * 
	 * @param executor
	 *            an executor used by the method <code>sendNotification</code>
	 *            to send each notification. If it is null, the thread calling
	 ...
	 * notifications that may be sent. Each listener is invoked by the thread
	 * sending the notification. This constructor is equivalent to
	 * {@link NotificationBroadcasterSupport#NotificationBroadcasterSupport(Executor,
	 ...
	 * {@link #getNotificationInfo()} returns a new clone.
	 * </p>
	 *
	 * @param info
	...
	public void removeNotificationListener(NotificationListener listener, NotificationFilter filter,
		...
		if (!removed) {
			throw new ListenerNotFoundException(
					"Listener not registered " + "(with this filter and " + "handback)");
