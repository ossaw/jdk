...
public class MBeanAttributeInfo extends MBeanFeatureInfo implements Cloneable {
    ...
    static {
        /*
         * For complicated reasons, the serialVersionUID changed between JMX 1.0
         * and JMX 1.1, even though JMX 1.1 did not have compatibility code for
         * this class. So the serialization produced by this class with JMX 1.2
         * and jmx.serial.form=1.0 is not the same as that produced by this
         * class with JMX 1.1 and jmx.serial.form=1.0. However, the
         * serialization without that property is the same, and that is the only
         * form required by JMX 1.2.
         */
     ...
     * for reading and writing the attribute. The {@link Descriptor} of the
     * constructed object will include fields contributed by any annotations on
     * the {@code Method} objects that contain the {@link DescriptorKey}
     * meta-annotation.
     *
     * @param name
    ...
    public MBeanAttributeInfo(String name, String description, Method getter, Method setter)
        ...
        this(name, attributeType(getter, setter), description, (getter != null), (setter != null), isIs(
                getter), ImmutableDescriptor.union(Introspector.descriptorForElement(getter), Introspector
    ...
    public String toString() {
        ...
        return getClass().getName() + "[" + "description=" + getDescription() + ", " + "name=" + getName()
                + ", " + "type=" + getType() + ", " + access + ", " + (isIs() ? "isIs, " : "") + "descriptor="
                + getDescriptor() + "]";
    }
    ...
    public boolean equals(Object o) {
        ...
        return (Objects.equals(p.getName(), getName()) && Objects.equals(p.getType(), getType()) && Objects
                .equals(p.getDescription(), getDescription()) && Objects.equals(p.getDescriptor(),
                        getDescriptor()) && p.isReadable() == isReadable() && p.isWritable() == isWritable()
                && p.isIs() == isIs());
    }
    ...
    private static boolean isIs(Method getter) {
        return (getter != null && getter.getName().startsWith("is") && (getter.getReturnType().equals(
                Boolean.TYPE) || getter.getReturnType().equals(Boolean.class)));
    }
