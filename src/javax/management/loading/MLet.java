 ...
 * server coming from a remote URL. M-let is a shortcut for management applet.
 * The m-let service does this by loading an m-let text file, which specifies
 ...
 * <p>
 * &lt;<CODE>MLET</CODE><BR>
 * <CODE>CODE = </CODE><VAR>class</VAR><CODE> | OBJECT = </CODE>
 * <VAR>serfile</VAR><BR>
 ...
 * <DD>This attribute specifies the full Java class name, including package
 * name, of the MBean to be obtained. The compiled <CODE>.class</CODE> file of
 * the MBean must be contained in one of the <CODE>.jar</CODE> files specified
 * by the <CODE>ARCHIVE</CODE> attribute. Either <CODE>CODE</CODE> or
 * <CODE>OBJECT</CODE> must be present.</DD>
 * <DT><CODE>OBJECT = </CODE><VAR>serfile</VAR></DT>
 * <DD>This attribute specifies the <CODE>.ser</CODE> file that contains a
 ...
 * <CODE>OBJECT</CODE> must be present.</DD>
 * <DT><CODE>ARCHIVE = &quot;</CODE><VAR>archiveList</VAR><CODE>&quot;</CODE>
 * </DT>
 * <DD>This mandatory attribute specifies one or more <CODE>.jar</CODE> files
 * containing MBeans or other resources used by the MBean to be obtained. One of
 * the <CODE>.jar</CODE> files must contain the file specified by the
 * <CODE>CODE</CODE> or <CODE>OBJECT</CODE> attribute. If archivelist contains
 * more than one file:
 * <UL>
 ...
 * </UL>
 * All <CODE>.jar</CODE> files in <VAR>archivelist</VAR> must be stored in the
 * directory specified by the code base URL.</DD>
 * <DT><CODE>CODEBASE = </CODE><VAR>codebaseURL</VAR></DT>
 * <DD>This optional attribute specifies the code base URL of the MBean to be
 * obtained. It identifies the directory that contains the <CODE>.jar</CODE>
 * files specified by the <CODE>ARCHIVE</CODE> attribute. Specify this attribute
 * only if the <CODE>.jar</CODE> files are not in the same directory as the
 * m-let text file. If this attribute is not specified, the base URL of the
 * m-let text file is used.</DD>
 * <DT><CODE>NAME = </CODE><VAR>mbeanname</VAR></DT>
 * <DD>This optional attribute specifies the object name to be assigned to the
 ...
 * the default domain of the MBean server, as returned by
 * {@link javax.management.MBeanServer#getDefaultDomain()}.</DD>
 * <DT><CODE>VERSION = </CODE><VAR>version</VAR></DT>
 * <DD>This optional attribute specifies the version number of the MBean and
 * associated <CODE>.jar</CODE> files to be obtained. This version number can be
 * used to specify that the <CODE>.jar</CODE> files are loaded from the server
 * to update those stored locally in the cache the next time the m-let text file
 * is loaded. <VAR>version</VAR> must be a series of non-negative decimal
 * integers each separated by a period from the one that precedes it.</DD>
 * <DT><VAR>arglist</VAR></DT>
 * <DD>This optional attribute specifies a list of one or more parameters for
 * the MBean to be instantiated. This list describes the parameters to be passed
 * the MBean's constructor. Use the following syntax to specify each item in
 * <VAR>arglist</VAR>:
 * <DL>
 * <DT>&lt;<CODE>ARG TYPE=</CODE><VAR>argumentType</VAR> <CODE>VALUE=</CODE>
 * <VAR>value</VAR>&gt;</DT>
 * <DD>where:
 * <UL>
 * <LI><VAR>argumentType</VAR> is the type of the argument that will be passed
 * as parameter to the MBean's constructor.
 * </UL>
 * </DD>
 * </DL>
 * <P>
 ...
 * <P>
 * The m-let service extends the <CODE>java.net.URLClassLoader</CODE> and can be
 * used to load remote classes and jar files in the VM of the agent.
 * <p>
 * <STRONG>Note - </STRONG> The <CODE>MLet</CODE> class loader uses the
 * {@link javax.management.MBeanServerFactory#getClassLoaderRepository(javax.management.MBeanServer)}
 * to load classes that could not be found in the loaded jar files.
 *
 * @since 1.5
 */
public class MLet extends java.net.URLClassLoader implements MLetMBean, MBeanRegistration, Externalizable {
    ...
    /**
     * The list of instances of the <CODE>MLetContent</CODE> class found at the
     * specified URL.
     * 
     * @serial
     */
    ...
    /**
     * What ClassLoaderRepository, if any, to use if this MLet doesn't find an
     * asked-for class.
     */
    private transient ClassLoaderRepository currentClr;

    /**
     * True if we should consult the {@link ClassLoaderRepository} when we do
     * not find a class ourselves.
     */
     ...
     */
    private Map<String, Class<?>> primitiveClasses = new HashMap<String, Class<?>>(8);
    {
    ...
    /*
     * The constructor stuff would be considerably simplified if our parent,
     * URLClassLoader, specified that its one- and two-argument constructors
     * were equivalent to its three-argument constructor with trailing null
     * arguments. But it doesn't, which prevents us from having all the
     * constructors but one call this(...args...).
     */
     ...
     * the specified parent class loader. The parent argument will be used as
    ...
    public void addURL(String url) throws ServiceNotFoundException {
        try {
            ...
            if (MLET_LOGGER.isLoggable(Level.FINEST)) {
                MLET_LOGGER.logp(Level.FINEST, MLet.class.getName(), "addUrl", "Malformed URL: " + url, e);
            }
     ...
     * to the MBean server. The location of the text file is specified by a URL.
     * The MBeans specified in the MLET file will be instantiated and registered
     * in the MBean server.
     *
     * @param url
     *        The URL of the text file to be loaded as URL object.
     * @return A set containing one entry per MLET tag in the m-let text file
     *         loaded. Each entry specifies either the ObjectInstance for the
     *         created MBean, or a throwable object (that is, an error or an
     ...
     *            MLet MBean is not registered with an
     *            MBeanServer.
     */
     ...
     *            not found, a mandatory attribute of
     *            the MLET tag is not
     *            specified, the url is malformed.
     * @exception IllegalStateException
     *            MLet MBean is not registered with an
     *            MBeanServer.
     */
    public Set<Object> getMBeansFromURL(String url) throws ServiceNotFoundException {
        ...
        for (MLetContent elmt : mletList) {
            ...
            while (st.hasMoreTokens()) {
                ...
                if (MLET_LOGGER.isLoggable(Level.FINER)) {
                    MLET_LOGGER.logp(Level.FINER, MLet.class.getName(), mth, "Load archive for codebase <"
                            + codebase + ">, file <" + tok + ">");
                }
                // Check which is the codebase to be used for loading the jar
                // file.
                // If we are using the base MLet implementation then it will be
                // always the remote server but if the service has been extended
                // in
            ...
            if (code != null && serName != null) {
                final String msg = "CODE and OBJECT parameters cannot be specified at the "
            ...
            try {
                if (code != null) {
                    ...
                    if (signat.isEmpty()) {
                        ...
                        if (name == null) {
                            ...
                            objInst = server.createMBean(code, new ObjectName(name), mletObjectName, parms,
                                    signature);
                        }
                    }
                } else {
                ...
                MLET_LOGGER.logp(Level.FINER, MLet.class.getName(), mth, "InstanceAlreadyExistsException",
                        ex);
                mbeans.add(ex);
                continue;
     ...
     * registered in the MBean server. If the ObjectName is null, the m-let
     * provides a default name for its registration
     ...
     *            This exception should be caught by the MBean server and
     ...
     *        means that
     *        either the registration phase has failed.
     */
    ...
    /**
     * <p>
     * Save this MLet's contents to the given {@link ObjectOutput}. Not all
     * implementations support this method. Those that do not throw
     * {@link UnsupportedOperationException}. A subclass may override this
     ...
     * supports {@link #writeExternal} it must also support
     * {@link #readExternal} in such a way that what is written by the former
     * can be read by the latter.
     * </p>
     *
     * @param out
    ...
    /**
     * <p>
     * Restore this MLet's contents from the given {@link ObjectInput}. Not all
     * implementations support this method. Those that do not throw
     * {@link UnsupportedOperationException}. A subclass may override this
     * method to support it or to change the format of the read data.
     * </p>
     * <p>
     * The format of the read data is not specified, but if an implementation
     * supports {@link #readExternal} it must also support
     * {@link #writeExternal} in such a way that what is written by the latter
     * can be read by the former.
     * </p>
     *
     * @param in
    ...
    /**
     * <p>
     * Load a class, using the given {@link ClassLoaderRepository} if the class
     * is not found in this MLet's URLs. The given ClassLoaderRepository can be
     * null, in which case a {@link ClassNotFoundException} occurs immediately
     ...
     * @param clr
     *        The ClassLoaderRepository that will be used to search for the
     *        given class, if it is not found in this ClassLoader. May be
     *        null.
     * @return The resulting Class object.
     * @exception ClassNotFoundException
     *            The specified class could not be found
     *            in this ClassLoader
     *            nor in the given ClassLoaderRepository.
     */
    public synchronized Class<?> loadClass(String name, ClassLoaderRepository clr)
    ...
    protected Class<?> findClass(String name) throws ClassNotFoundException {
        /*
         * currentClr is context sensitive - used to avoid recursion in the
         * class loader repository. (This is no longer necessary with the new
         * CLR semantics but is kept for compatibility with code that might have
         * called the two-parameter loadClass explicitly.)
         */
     ...
     *
     * @param name
     *        The name of the class that we want to load/find.
     * @param clr
     *        The ClassLoaderRepository that can be used to search for the
     *        given class. This parameter is <code>null</code> when called
     *        from within the
     *        {@link javax.management.MBeanServerFactory#getClassLoaderRepository(javax.management.MBeanServer)
    ...
    Class<?> findClass(String name, ClassLoaderRepository clr) throws ClassNotFoundException {
        ...
        try {
            ...
            if (MLET_LOGGER.isLoggable(Level.FINER)) {
                MLET_LOGGER.logp(Level.FINER, MLet.class.getName(), "findClass", "Class " + name
                        + " loaded through MLet classloader");
            }
        } catch (ClassNotFoundException e) {
            // Drop through
            if (MLET_LOGGER.isLoggable(Level.FINEST)) {
                MLET_LOGGER.logp(Level.FINEST, MLet.class.getName(), "findClass", "Class " + name
                        + " not found locally");
            }
        }
        ...
        if (c == null) {
            MLET_LOGGER.logp(Level.FINEST, MLet.class.getName(), "findClass", "Failed to load class " + name);
     ...
     * this class loader. Libraries are searched in the JAR files using first
     * just the native library name and if not found the native library name
     * together with the architecture-specific path name (
     * <code>OSName/OSArch/OSVersion/lib/nativelibname</code>), i.e.
     * <p>
     * the library stat on Solaris SPARC 5.7 will be searched in the JAR file
     * as:
     * <OL>
     * <LI>libstat.so
     ...
     * </OL>
     * <p>
     * More specifically, let <em>{@code nativelibname}</em> be the result of
     * {@link System#mapLibraryName(java.lang.String) System.mapLibraryName}
     * {@code (libname)}. Then the following names are searched in the JAR
     * files, in order:<br>
     * <em>{@code nativelibname}</em><br>
     * {@code <os.name>/<os.arch>/<os.version>/lib/}
     * <em>{@code nativelibname}</em><br>
     * where {@code <X>} means {@code System.getProperty(X)} with any spaces in
     * the result removed, and {@code /} stands for the file separator character
     * ({@link File#separator}).
     * <p>
     * If this method returns <code>null</code>, i.e. the libraries were not
     * found in any of the JAR files loaded with this class loader, the VM
    ...
    protected String findLibrary(String libname) {
        ...
        //
        // See if the native library is accessible as a resource through the JAR
        // file.
        //
        if (MLET_LOGGER.isLoggable(Level.FINER)) {
            MLET_LOGGER.logp(Level.FINER, MLet.class.getName(), mth, "Search " + libname
                    + " in all JAR files");
        }
        ...
        //
        if (MLET_LOGGER.isLoggable(Level.FINER)) {
            MLET_LOGGER.logp(Level.FINER, MLet.class.getName(), mth, "loadLibraryAsResource(" + nativelibname
                    + ")");
        }
        abs_path = loadLibraryAsResource(nativelibname);
        if (abs_path != null) {
            if (MLET_LOGGER.isLoggable(Level.FINER)) {
                MLET_LOGGER.logp(Level.FINER, MLet.class.getName(), mth, nativelibname
                        + " loaded, absolute path = " + abs_path);
            }
            return abs_path;
        }
        ...
        //
        nativelibname = removeSpace(System.getProperty("os.name")) + File.separator + removeSpace(System
                .getProperty("os.arch")) + File.separator + removeSpace(System.getProperty("os.version"))
                + File.separator + "lib" + File.separator + nativelibname;
        if (MLET_LOGGER.isLoggable(Level.FINER)) {
            MLET_LOGGER.logp(Level.FINER, MLet.class.getName(), mth, "loadLibraryAsResource(" + nativelibname
                    + ")");
        }
        ...
        if (MLET_LOGGER.isLoggable(Level.FINER)) {
            MLET_LOGGER.logp(Level.FINER, MLet.class.getName(), mth, libname + " not found in any JAR file");
            MLET_LOGGER.logp(Level.FINER, MLet.class.getName(), mth, "Search " + libname + " along the path "
    ...
    private String getTmpDir() {
        ...
        try {
            ...
            MLET_LOGGER.logp(Level.FINEST, MLet.class.getName(), "getTmpDir",
    ...
    /**
     * Search the specified native library in any of the JAR files loaded by
     * this classloader. If the library is found copy it into the library
     * directory and return the absolute path. If the library is not found then
     * return null.
     */
    private synchronized String loadLibraryAsResource(String libname) {
        try {
            ...
            MLET_LOGGER.logp(Level.FINEST, MLet.class.getName(), "loadLibraryAsResource",
    ...
    /**
     * Removes any white space from a string. This is used to convert strings
     * such as "Windows NT" to "WindowsNT".
     */
     ...
     * caching and versioning. It is called from {@link #getMBeansFromURL
     * getMBeansFromURL} when the version, codebase, and jarfile have been
     * extracted from the MLet file, and can be used to verify that it is all
     * right to load the given MBean, or to replace the given URL with a
     * different one.
     * </p>
     * <p>
     * The default implementation of this method returns <code>codebase</code>
     * unchanged.
     * </p>
     *
     * @param version
     *        The version number of the <CODE>.jar</CODE> file stored
     *        locally.
     * @param codebase
     *        The base URL of the remote <CODE>.jar</CODE> file.
     * @param jarfile
     *        The name of the <CODE>.jar</CODE> file to be loaded.
     * @param mlet
     *        The <CODE>MLetContent</CODE> instance that represents the
     *        <CODE>MLET</CODE> tag.
     * @return the codebase to use for the loaded MBean. The returned value
     *         should not be null.
     * @exception Exception
     *            if the MBean is not to be loaded for some reason.
     *            The
     *            exception will be added to the set returned by
     ...
     * @exception ClassNotFoundException
     *            The specified serialized object could
     *            not be found.
     * @exception IOException
     *            An I/O error occurred while loading
     *            serialized object.
     */
    private Object loadSerializedObject(URL codebase, String filename) throws IOException,
            ClassNotFoundException {
        if (filename != null) {
            filename = filename.replace(File.separatorChar, '/');
        }
        if (MLET_LOGGER.isLoggable(Level.FINER)) {
            MLET_LOGGER.logp(Level.FINER, MLet.class.getName(), "loadSerializedObject", codebase.toString()
                    + filename);
        }
        ...
        if (is != null) {
            try {
                ...
                if (MLET_LOGGER.isLoggable(Level.FINEST)) {
                    MLET_LOGGER.logp(Level.FINEST, MLet.class.getName(), "loadSerializedObject",
            ...
            if (MLET_LOGGER.isLoggable(Level.FINEST)) {
                MLET_LOGGER.logp(Level.FINEST, MLet.class.getName(), "loadSerializedObject", "Error: File "
                        + filename + " containing serialized object not found");
            }
    ...
    /**
     * Converts the String value of the constructor's parameter to a basic Java
     * object with the type of the parameter.
     */
    private Object constructParameter(String param, String type) {
        ...
        if (c != null) {
            try {
                ...
                MLET_LOGGER.logp(Level.FINEST, MLet.class.getName(), "constructParameter",
