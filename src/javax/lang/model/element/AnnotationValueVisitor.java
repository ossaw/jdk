 ...
 * based on the type of data stored; there are no distinct subclasses for
 * storing, for example, {@code boolean} values versus {@code int} values.
 * Classes implementing this interface are used to operate on a value when the
 * type of that value is unknown at compile time. When a visitor is passed to a
 * value's {@link AnnotationValue#accept accept} method, the
 * <tt>visit<i>XYZ</i></tt> method applicable to that value is invoked.
 *
 * <p>
 ...
 * versions of the Java&trade; programming language. Therefore, visitor classes
 * directly implementing this interface may be source incompatible with future
 * versions of the platform. To avoid this source incompatibility, visitor
 * implementations are encouraged to instead extend the appropriate abstract
 ...
 * . However, default methods are only available on Java SE 8 and higher
 * releases and the {@code
 * javax.lang.model.*} packages bundled in Java SE 8 are required to also be
 * runnable on Java SE 7. Therefore, default methods <em>cannot</em> be used
 * when extending {@code javax.lang.model.*} to cover Java SE 8 language
 * features. However, default methods may be used in subsequent revisions of the
 * {@code javax.lang.model.*} packages that are only required to run on Java SE
