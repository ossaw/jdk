 ...
 * do not model the structure of a program inside a method body; for example
 * there is no representation of a {@code for} loop or {@code try}-
 ...
 * source code provides the fiducial (reference) representation of the construct
 * in question rather than a representation in an executable output like a class
 * file. Executable output may serve as the basis for creating a modeling
 * element. However, the process of translating source code to executable output
 * may not permit recovering some aspects of the source code representation. For
 * example, annotations with
 ...
 * resulting model. If the source code is not syntactically well-formed or has
 * some other irrecoverable error that could not be removed by the generation of
 * new types, a model may or may not be provided as a quality of implementation
 * issue. If a program is syntactically valid but erroneous in some other
 * fashion, any returned model must have no less information than if all the
 * method bodies in the program were replaced by {@code "throw
 * new RuntimeException();"}. If a program refers to a missing type XYZ, the
 * returned model must contain no less information than if the declaration of
 * type XYZ were assumed to be {@code "class XYZ {}"}, {@code "interface XYZ {}"
 * }, {@code "enum XYZ {}"}, or {@code "@interface XYZ {}"}. If a program refers
 * to a missing type {@code
