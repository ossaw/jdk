 ...
 * Invoking methods on the {@code SaslClient} instance process challenges and
 * create responses according to the SASL mechanism implemented by the
 * {@code SaslClient}. As the authentication proceeds, the instance encapsulates
 * the state of a SASL client's authentication exchange.
 * <p>
 * Here's an example of how an LDAP library might use a {@code SaslClient}. It
 * first gets an instance of a {@code SaslClient}: <blockquote>
 * 
 * <pre>
 * {
 * 	&#64;code
 * 	SaslClient sc = Sasl.createSaslClient(mechanisms, authorizationId, protocol, serverName,
 * 			props, callbackHandler);
 * }
 * </pre>
 * 
 * </blockquote> It can then proceed to use the client for authentication. For
 * example, an LDAP library might use the client as follows: <blockquote>
 * 
 * <pre>
 * {
 * 	&#64;code
 ...
 * with their first authentication command to the server, initiates the
 * authentication without first calling {@code hasInitialResponse()} or
 * {@code evaluateChallenge()}. When the server responds to the command, it
 * sends an initial challenge. For a SASL mechanism in which the client sends
 * data first, the server should have issued a challenge with no data. This will
 * then result in a call (on the client) to {@code evaluateChallenge()} with an
 * empty challenge.
 *
 * @since 1.5
 *
...
public abstract interface SaslClient {
	 ...
	 * true, caller should call {@code evaluateChallenge()} with an empty array
	 ...
	 * may be called at any time, but typically, it will not be called until the
