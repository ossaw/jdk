...
public class Sasl {
	 ...
	 * The property contains a comma-separated, ordered list of
	 * quality-of-protection values that the client or server is willing to
	 * support. A qop value is one of
	 * <ul>
	 ...
	 * these values. An application may use the Java Cryptography Extension
	 * (JCE) with JCE-aware mechanisms to control the selection of cipher suites
	 * that match the strength values. <BR>
	 * If this property is absent, the default strength is
	 * {@code "high,medium,low"}. The value of this constant is
	 * {@code "javax.security.sasl.strength"}.
	 */
	public static final String STRENGTH = "javax.security.sasl.strength";

	/**
	 * The name of a property that specifies whether the server must
	 * authenticate to the client. The property contains {@code "true"} if the
	 * server must authenticate the to client; {@code "false"} otherwise. The
	...
	/**
	 * The name of a property that specifies the bound server name for an
	 * unbound server. A server is created as an unbound server by setting the
	 * {@code serverName} argument in {@link #createSaslServer} as null. The
	 * property contains the bound host name after the authentication exchange
	 * has completed. It is only available on the server side. <br>
	 * The value of this constant is
	 ...
	 * buffer in bytes of {@code SaslClient}/{@code SaslServer}. The property
	 * contains the string representation of an integer. <br>
	 ...
	 * session information; it contains "false" if the implementation must not
	 * reuse previously authenticated session information. A setting of "true"
	 * serves only as a hint: it does not necessarily entail actual reuse
	 * because reuse might not be possible due to a number of reasons,
	 ...
	 * simple plain passive attacks (e.g., "PLAIN") are not permitted. The
	 * property contains {@code "true"} if such mechanisms are not permitted;
	 * {@code "false"} if such mechanisms are permitted. The default is
	 * {@code "false"}. <br>
	 * The value of this constant is
	 ...
	 * {@code "true"} if mechanisms susceptible to active attacks are not
	 * permitted; {@code "false"} if such mechanisms are permitted. The default
	 * is {@code "false"}. <br>
	 * The value of this constant is
	 ...
	 * {@code "true"} if mechanisms susceptible to dictionary attacks are not
	 * permitted; {@code "false"} if such mechanisms are permitted. The default
	 * is {@code "false"}. <br>
	 * The value of this constant is
	 ...
	 * breaking into future sessions. The property contains {@code "true"} if
	 * mechanisms that implement forward secrecy between sessions are required;
	 * {@code "false"} if such mechanisms are not required. The default is
	 * {@code "false"}. <br>
	 * The value of this constant is
	 ...
	 * credentials are required. The property contains {@code "true"} if
	 * mechanisms that pass client credentials are required; {@code "false"} if
	 * such mechanisms are not required. The default is {@code "false"}. <br>
	 * The value of this constant is
	...
	/**
	 * The name of a property that specifies the credentials to use. The
	 * property contains a mechanism-specific Java credential object. Mechanism
	 * implementations may examine the value of this property to determine
	 * whether it is a class that they support. The property may be used to
	 * supply credentials to a mechanism that supports delegated authentication.
	 * <br>
	 * The value of this constant is {@code "javax.security.sasl.credentials"}.
	 */
	 ...
	 * {@code createSaslClient()} on each factory instance on the list until one
	 * produces a non-null {@code SaslClient} instance. It returns the non-null
	 * {@code SaslClient} instance, or null if the search fails to produce a
	 * non-null {@code SaslClient} instance.
	 * <p>
	 * A security provider for SaslClientFactory registers with the JCA Security
	 * Provider Framework keys of the form <br>
	 ...
	 * See the
	 * "Java Cryptography Architecture API Specification &amp; Reference" for
	 * information about how to install and configure security service
	 * providers.
	 *
	 * @param mechanisms
	 *            The non-null list of mechanism names to try. Each is the
	 *            IANA-registered name of a SASL mechanism. (e.g. "GSSAPI",
	 *            "CRAM-MD5").
	 * @param authorizationId
	 *            The possibly null protocol-dependent identification to be used
	 *            for authorization. If null or empty, the server derives an
	 *            authorization ID from the client's authentication credentials.
	 *            When the SASL authentication completes successfully, the
	 ...
	 *            selected mechanism. For example, if {@code props} contains the
	 *            {@code Sasl.POLICY_NOPLAINTEXT} property with the value
	 *            {@code "true"}, then the selected SASL mechanism must not be
	 *            susceptible to simple plain passive attacks. In addition to
	 *            the standard properties declared in this class, other,
	 *            possibly mechanism-specific, properties can be included.
	 ...
	 *            requested by using a {@code NameCallback}. The password is
	 *            requested by using a {@code PasswordCallback}. The realm is
	 *            requested by using a {@code RealmChoiceCallback} if there is a
	 ...
	 *                If cannot create a {@code SaslClient} because of an error.
	 */
	public static SaslClient createSaslClient(String[] mechanisms, String authorizationId,
		...
		for (int i = 0; i < mechanisms.length; i++) {
			...
			for (int j = 0; provs != null && j < provs.length; j++) {
				...
				if (fac != null) {
					mech = fac.createSaslClient(new String[] { mechanisms[i] }, authorizationId,
	...
	private static Object loadFactory(Provider p, String className) throws SaslException {
		try {
			 ...
			 * used to load the provider. In order to get the class loader of a
			 * class, the caller's class loader must be the same as or an
			 * ancestor of the class loader being returned. Otherwise, the
			 * caller must have "getClassLoader" permission, or a
	 ...
	 * {@code createSaslServer()} on each factory instance on the list until one
	 * produces a non-null {@code SaslServer} instance. It returns the non-null
	 * {@code SaslServer} instance, or null if the search fails to produce a
	 * non-null {@code SaslServer} instance.
	 * <p>
	 ...
	 *
	 * @param mechanism
	 *            The non-null mechanism name. It must be an IANA-registered
	 *            name of a SASL mechanism. (e.g. "GSSAPI", "CRAM-MD5").
	 * @param protocol
	 *            The non-null string name of the protocol for which the
	 *            authentication is being performed (e.g., "ldap").
	 * @param serverName
	 *            The fully qualified host name of the server, or null if the
	 ...
	 *         will produce one.
	 * @exception SaslException
	 *                If cannot create a {@code SaslServer} because of an error.
	 **/
	public static SaslServer createSaslServer(String mechanism, String protocol, String serverName,
			Map<String, ?> props, javax.security.auth.callback.CallbackHandler cbh)
			throws SaslException {
