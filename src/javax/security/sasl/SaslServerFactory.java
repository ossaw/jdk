...
/**
 * An interface for creating instances of {@code SaslServer}. A class that
 * implements this interface must be thread-safe and handle multiple
 * simultaneous requests. It must also have a public constructor that accepts no
 * argument.
 * <p>
 * This interface is not normally accessed directly by a server, which will use
 * the {@code Sasl} static methods instead. However, a particular environment
 * may provide and install a new or different {@code SaslServerFactory}.
 *
 * @since 1.5
 *
 * @see SaslServer
...
public abstract interface SaslServerFactory {
	/**
	 * Creates a {@code SaslServer} using the parameters supplied. It returns
	 * null if no {@code SaslServer} can be created using the parameters
	 * supplied. Throws {@code SaslException} if it cannot create a
	 * {@code SaslServer} because of an error.
	 *
	 * @param mechanism
	 ...
	 * @param serverName
	 *            The fully qualified host name of the server to authenticate
	 *            to, or null if the server is not bound to any specific host
	 ...
	 *            selected mechanism. See the {@code Sasl} class for a list of
	 *            standard properties. Other, possibly mechanism-specific,
	 *            properties can be included. Properties not relevant to the
	 *            selected mechanism are ignored, including any map entries with
	 *            non-String keys.
	 *
	 * @param cbh
	 ...
	 *         {@code SaslServer} using the parameters supplied.
	 * @exception SaslException
	 *                If cannot create a {@code SaslServer} because of an error.
	 */
	public abstract SaslServer createSaslServer(String mechanism, String protocol,
	 ...
	 *            {@code props} contains the {@code Sasl.POLICY_NOPLAINTEXT}
	 *            property with the value {@code "true"}, then the factory must
	 *            not return any SASL mechanisms that are susceptible to simple
	 *            plain passive attacks. See the {@code Sasl} class for a
	 *            complete list of policy properties. Non-policy related
	 *            properties, if present in {@code props}, are ignored,
	 *            including any map entries with non-String keys.
	 * @return A non-null array containing a IANA-registered SASL mechanism
	 *         names.
	 */
