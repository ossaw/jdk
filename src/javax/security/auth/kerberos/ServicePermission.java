 ...
 * to access those services. There is a one to one mapping of a service
 * principal and the credentials necessary to access the service. Therefore
 * granting access to a service principal implicitly grants access to the
 * credential necessary to establish a security context with the service
 * principal. This applies regardless of whether the credentials are in a cache
 * or acquired via an exchange with the KDC. The credential can be either a
 * ticket granting ticket, a service ticket or a secret key from a key table.
 * <p>
 ...
 * {@code KerberosPrincipal} supplying the service, that is the
 * KerberosPrincipal represents a Kerberos service principal. This name is
 * treated in a case sensitive manner. An asterisk may appear by itself, to
...
public final class ServicePermission extends Permission implements java.io.Serializable {
    ...
    /**
     * Create a new {@code ServicePermission} with the specified
    ...
    boolean impliesIgnoreMask(ServicePermission p) {
        return ((this.getName().equals("*")) || this.getName().equals(p.getName()) || (p.getName().startsWith(
     ...
     * @return true if <i>obj</i> is a ServicePermission, and has the same
     *         service principal, and actions as this ServicePermission object.
     */
     ...
     * returns present actions in the following order: initiate, accept.
     */
     ...
     * objects. <br>
     * ServicePermission objects must be stored in a manner that allows them to
     ...
     * Note: if action is "-", action will be NONE, which means any action
     * implies it.
     *
     * @param action
     *        the action string.
    ...
    private static int getMask(String action) {
        ...
        while (i != -1) {
            ...
            if (i >= 7 && (a[i - 7] == 'i' || a[i - 7] == 'I') && (a[i - 6] == 'n' || a[i - 6] == 'N') && (a[i
                    - 5] == 'i' || a[i - 5] == 'I') && (a[i - 4] == 't' || a[i - 4] == 'T') && (a[i
                            - 3] == 'i' || a[i - 3] == 'I') && (a[i - 2] == 'a' || a[i - 2] == 'A') && (a[i
     ...
     * ServicePermission this_ = new ServicePermission(args[0], "accept");
     * ServicePermission that_ = new ServicePermission(args[1],
     * "accept,initiate"); System.out.println("-----\n"); System.out.println(
     * "this.implies(that) = " + this_.implies(that_));
...
final class KrbServicePermissionCollection extends PermissionCollection implements java.io.Serializable {
    ...
    public boolean implies(Permission permission) {
        ...
        synchronized (this) {
            ...
            // more than one action and the collection has individual
            // permissions
    ...
    /**
     * Adds a permission to the ServicePermissions. The key for the hash is the
     * name.
     *
     * @param permission
    ...
    /**
     * Returns an enumeration of all the ServicePermission objects in the
     * container.
     *
    ...
    /*
     * Reads in a Vector of ServicePermissions and saves them in the perms
     * field.
     */
