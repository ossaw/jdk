 ...
 * class in the private credential set of a {@link javax.security.auth.Subject
 * Subject} during the commit phase of the authentication process.
 * <p>
 * If a {@code KeyTab} object is obtained from {@link #getUnboundInstance()} or
 * {@link #getUnboundInstance(java.io.File)}, it is unbound and thus can be used
 * by any service principal. Otherwise, if it's obtained from
 ...
 * {@link #getInstance(java.io.File)} were created when there was no support for
 * unbound keytabs. These methods should not be used anymore. An object created
 * with either of these methods are considered to be bound to an unknown
 * principal, which means, its {@link #isBound()} returns true and
 * {@link #getPrincipal()} returns null.
 * <p>
 * It might be necessary for the application to be granted a
 * {@link javax.security.auth.PrivateCredentialPermission
 * PrivateCredentialPermission} if it needs to access the KeyTab instance from a
 * Subject. This permission is not needed when the application depends on the
 * default JGSS Kerberos mechanism to access the KeyTab. In that case, however,
 * the application will need an appropriate
 * {@link javax.security.auth.kerberos.ServicePermission ServicePermission}.
 * <p>
 * The keytab file format is described at
 * <a href="http://www.ioplex.com/utilities/keytab.txt"> http://www.ioplex.com/
 * utilities/keytab.txt</a>.
 * <p>
 * 
 * @since 1.7
 */
public final class KeyTab {
     ...
     * the create/add/save methods only used by the ktab command).
     */
     ...
     * bound to an unknown service principal.
     * <p>
     ...
     * <p>
     * Developers should call {@link #getInstance(KerberosPrincipal,File)} when
    ...
    public KerberosKey[] getKeys(KerberosPrincipal principal) {
        try {
            ...
            for (int i = 0; i < kks.length; i++) {
                ...
                kks[i] = new KerberosKey(principal, keys[i].getBytes(), keys[i].getEType(), tmp == null ? 0
                        : tmp.intValue());
                keys[i].destroy();
            }
            return kks;
    ...
    /**
     * Compares the specified Object with this KeyTab for equality. Returns true
     * if the given object is also a {@code KeyTab} and the two {@code KeyTab}
