 ...
 * {@code Configuration} specifies the LoginModule(s) to be used with a
 ...
 * users. The {@code CallbackHandler} may be used to prompt for usernames and
 * passwords, for example. Note that the {@code CallbackHandler} may be null.
 * LoginModules which absolutely require a {@code CallbackHandler} to
 ...
 * {@code Configuration}. The options are defined by the {@code LoginModule}
 * itself and control the behavior within it. For example, a {@code LoginModule}
 * may define options to support debugging/testing capabilities. Options are
 ...
 * proceeds in two distinct phases. In the first phase, the LoginModule's
 * {@code login} method gets invoked by the LoginContext's {@code login} method.
 * The {@code login} method for the {@code LoginModule} then performs the actual
 ...
 * LoginModule's {@code login} method either returns {@code true} (if it
 * succeeded) or {@code false} (if it should be ignored), or throws a
 * {@code LoginException} to specify a failure. In the failure case, the
 * {@code LoginModule} must not retry the authentication or introduce delays.
 ...
 * succeeded), then the {@code commit} method for the {@code LoginModule} gets
 * invoked. The {@code commit} method for a {@code LoginModule} checks its
 * privately saved state to see if its own authentication succeeded. If the
 * overall {@code LoginContext} authentication succeeded and the LoginModule's
 * own authentication succeeded, then the {@code commit} method associates the
