...
/**
 * <p>
 * The {@code LoginContext} class describes the basic methods used to
 * authenticate Subjects and provides a way to develop an application
 * independent of the underlying authentication technology. A
 * {@code Configuration} specifies the authentication technology, or
 * {@code LoginModule}, to be used with a particular application. Different
 * LoginModules can be plugged in under an application without requiring any
 * modifications to the application itself.
 *
 * <p>
 * In addition to supporting <i>pluggable</i> authentication, this class also
 * supports the notion of <i>stacked</i> authentication. Applications may be
 * configured to use more than one LoginModule. For example, one could configure
 * both a Kerberos LoginModule and a smart card LoginModule under an
 * application.
 *
 * <p>
 * A typical caller instantiates a LoginContext with a <i>name</i> and a
 * {@code CallbackHandler}. LoginContext uses the <i>name</i> as the index into
 * a Configuration to determine which LoginModules should be used, and which
 * ones must succeed in order for the overall authentication to succeed. The
 * {@code CallbackHandler} is passed to the underlying LoginModules so they may
 * communicate and interact with users (prompting for a username and password
 * via a graphical user interface, for example).
 *
 * <p>
 * Once the caller has instantiated a LoginContext, it invokes the {@code login}
 * method to authenticate a {@code Subject}. The {@code login} method invokes
 * the configured modules to perform their respective types of authentication
 * (username/password, smart card pin verification, etc.). Note that the
 * LoginModules will not attempt authentication retries nor introduce delays if
 * the authentication fails. Such tasks belong to the LoginContext caller.
 *
 * <p>
 * If the {@code login} method returns without throwing an exception, then the
 * overall authentication succeeded. The caller can then retrieve the newly
 * authenticated Subject by invoking the {@code getSubject} method. Principals
 * and Credentials associated with the Subject may be retrieved by invoking the
 * Subject's respective {@code getPrincipals}, {@code getPublicCredentials}, and
 * {@code getPrivateCredentials} methods.
 *
 * <p>
 * To logout the Subject, the caller calls the {@code logout} method. As with
 * the {@code login} method, this {@code logout} method invokes the
 * {@code logout} method for the configured modules.
 *
 * <p>
 ...
 * <li>{@code Subject}
 * <ul>
 * <li>If the constructor has a Subject input parameter, the LoginContext uses
 * the caller-specified Subject object.
 *
 * <li>If the caller specifies a {@code null} Subject and a {@code null} value
 * is permitted, the LoginContext instantiates a new Subject.
 *
 * <li>If the constructor does <b>not</b> have a Subject input parameter, the
 ...
 * <ul>
 * <li>If the constructor has a Configuration input parameter and the caller
 * specifies a non-null Configuration, the LoginContext uses the
 * caller-specified Configuration.
 * <p>
 * If the constructor does <b>not</b> have a Configuration input parameter, or
 * if the caller specifies a {@code null} Configuration object, the constructor
 * uses the following call to get the installed Configuration:
 * 
 * <pre>
 * config = Configuration.getConfiguration();
 * </pre>
 * 
 * For both cases, the <i>name</i> argument given to the constructor is passed
 * to the {@code Configuration.getAppConfigurationEntry} method. If the
 * Configuration has no entries for the specified <i>name</i>, then the
 * {@code LoginContext} calls {@code getAppConfigurationEntry} with the name,
 * "<i>other</i>" (the default entry name). If there is no entry for
 * "<i>other</i>", then a {@code LoginException} is thrown.
 *
 * <li>When LoginContext uses the installed Configuration, the caller requires
 * the createLoginContext.<em>name</em> and possibly createLoginContext.other
 * AuthPermissions. Furthermore, the LoginContext will invoke configured modules
 * from within an {@code AccessController.doPrivileged} call so that modules
 * that perform security-sensitive tasks (such as connecting to remote hosts,
 * and updating the Subject) will require the respective permissions, but the
 * callers of the LoginContext will not require those permissions.
 *
 * <li>When LoginContext uses a caller-specified Configuration, the caller does
 * not require any createLoginContext AuthPermission. The LoginContext saves the
 * {@code AccessControlContext} for the caller, and invokes the configured
 * modules from within an {@code AccessController.doPrivileged} call constrained
 * by that context. This means the caller context (stored when the LoginContext
 * was created) must have sufficient permissions to perform any
 * security-sensitive tasks that the modules may perform.
 * <p>
 * </ul>
 *
 * <li>{@code CallbackHandler}
 * <ul>
 * <li>If the constructor has a CallbackHandler input parameter, the
 * LoginContext uses the caller-specified CallbackHandler object.
 *
 * <li>If the constructor does <b>not</b> have a CallbackHandler input
 * parameter, or if the caller specifies a {@code null} CallbackHandler object
 * (and a {@code null} value is permitted), the LoginContext queries the
 * {@code auth.login.defaultCallbackHandler} security property for the fully
 * qualified class name of a default handler implementation. If the security
 * property is not set, then the underlying modules will not have a
 * CallbackHandler for use in communicating with users. The caller thus assumes
 * that the configured modules have alternative means for authenticating the
 * user.
 *
 *
 * <li>When the LoginContext uses the installed Configuration (instead of a
 ...
 * CallbackHandler implementation whose {@code handle} method implementation
 * invokes the specified CallbackHandler's {@code handle} method in a
 * {@code java.security.AccessController.doPrivileged} call constrained by the
 * caller's current {@code AccessControlContext}.
 * </ul>
 * </ol>
 *
...
public class LoginContext {
	...
	private static final sun.security.util.Debug debug = sun.security.util.Debug
			.getInstance("logincontext", "\t[LoginContext]");
	...
	private void init(String name) throws LoginException {
		...
		if (config == null) {
			config = java.security.AccessController
					.doPrivileged(new java.security.PrivilegedAction<Configuration>() {
		...
		if (entries == null) {
			...
			if (entries == null) {
				MessageFormat form = new MessageFormat(
						ResourcesMgr.getString("No.LoginModules.configured.for.name"));
				Object[] source = { name };
		...
		for (int i = 0; i < entries.length; i++) {
			...
			moduleStack[i] = new ModuleInfo(
					new AppConfigurationEntry(entries[i].getLoginModuleName(),
							entries[i].getControlFlag(), entries[i].getOptions()),
					null);
		}
	...
	private void loadDefaultCallbackHandler() throws LoginException {
		...
		try {
			...
			this.callbackHandler = java.security.AccessController
					.doPrivileged(new java.security.PrivilegedExceptionAction<CallbackHandler>() {
						public CallbackHandler run() throws Exception {
							String defaultHandler = java.security.Security
									.getProperty(DEFAULT_HANDLER);
							if (defaultHandler == null || defaultHandler.length() == 0)
								return null;
							Class<? extends CallbackHandler> c = Class
									.forName(defaultHandler, true, finalLoader)
	...
	/**
	 * Instantiate a new {@code LoginContext} object with a name and a
	 * {@code Subject} object.
	 *
	 * <p>
	 *
	 ...
	 *
	 * @param subject
	 *            the {@code Subject} to authenticate.
	 *
	 * @exception LoginException
	 *                if the caller-specified {@code name} does not appear in
	 *                the {@code Configuration} and there is no
	 *                {@code Configuration} entry for "<i>other</i>", if the
	 *                caller-specified {@code subject} is {@code null}, or if
	 *                the <i>auth.login.defaultCallbackHandler</i> security
	 *                property was set, but the implementation class could not
	 *                be loaded.
	 *                <p>
	 * @exception SecurityException
	 *                if a SecurityManager is set and the caller does not have
	 ...
	 *            the {@code CallbackHandler} object used by LoginModules to
	...
	public LoginContext(String name, CallbackHandler callbackHandler) throws LoginException {
		...
		if (callbackHandler == null)
			throw new LoginException(
					ResourcesMgr.getString("invalid.null.CallbackHandler.provided"));
		this.callbackHandler = new SecureCallbackHandler(
				java.security.AccessController.getContext(), callbackHandler);
	}

	/**
	 * Instantiate a new {@code LoginContext} object with a name, a
	 * {@code Subject} to be authenticated, and a {@code CallbackHandler}
	 * object.
	 *
	 * <p>
	 *
	 ...
	 *                {@code Configuration} entry for "<i>other</i>", or if the
	 *                caller-specified {@code subject} is {@code null}, or if
	 *                the caller-specified {@code callbackHandler} is
	 *                {@code null}.
	 *                <p>
	 * @exception SecurityException
	 *                if a SecurityManager is set and the caller does not have
	...
	/**
	 * Instantiate a new {@code LoginContext} object with a name, a
	 * {@code Subject} to be authenticated, a {@code CallbackHandler} object,
	 * and a login {@code Configuration}.
	 *
	 * <p>
	 *
	 * @param name
	 ...
	 *
	 * @param subject
	 *            the {@code Subject} to authenticate, or {@code null}.
	 *            <p>
	 *
	 * @param callbackHandler
	 *            the {@code CallbackHandler} object used by LoginModules to
	 *            communicate with the user, or {@code null}.
	 *            <p>
	 *
	 * @param config
	 *            the {@code Configuration} that lists the login modules to be
	 *            called to perform the authentication, or {@code null}.
	 *
	 * @exception LoginException
	 *                if the caller-specified {@code name} does not appear in
	 *                the {@code Configuration} and there is no
	 *                {@code Configuration} entry for "<i>other</i>".
	 *                <p>
	 * @exception SecurityException
	 *                if a SecurityManager is set, <i>config</i> is {@code null}
	 *                , and either the caller does not have AuthPermission(
	 *                "createLoginContext.<i>name</i>"), or if a configuration
	 *                entry for <i>name</i> does not exist and the caller does
	 *                not additionally have
	 *                AuthPermission("createLoginContext.other")
	 *
	 * @since 1.5
	 */
	 ...
	 * doPrivileged block restricted by creatorAcc (may be null).
	 *
	...
	private void invoke(String methodName) throws LoginException {
		...
		for (int i = moduleIndex; i < moduleStack.length; i++, moduleIndex++) {
			try {
				...
				if (moduleStack[i].module != null) {
					...
					Class<?> c = Class.forName(moduleStack[i].entry.getLoginModuleName(), true,
							contextClassLoader);
					...
					Object[] initArgs = { subject, callbackHandler, state,
				...
				// method defined. May improve to use LoginException in
				// the future.
				boolean status = ((Boolean) methods[mIndex].invoke(moduleStack[i].module, args))
						.booleanValue();
				...
				throwException(null,
						new LoginException(
								ResourcesMgr.getString("unable.to.find.LoginModule.class.")
										+ cnfe.getMessage()));
			} catch (IllegalAccessException iae) {
				throwException(null,
						new LoginException(ResourcesMgr.getString("unable.to.access.LoginModule.")
								+ iae.getMessage()));
				...
				if (moduleStack[i].entry
						.getControlFlag() == AppConfigurationEntry.LoginModuleControlFlag.REQUISITE) {
		...
		if (firstRequiredError != null) {
			...
			throwException(
					new LoginException(ResourcesMgr.getString("Login.Failure.all.modules.ignored")),
					null);
		} else {
			// success
	...
	/**
	 * Wrap the caller-specified CallbackHandler in our own and invoke it within
	 * a privileged block, constrained by the caller's AccessControlContext.
	 */
	...
	/**
	 * LoginModule information - incapsulates Configuration info and actual
	 * module instances
	 */
