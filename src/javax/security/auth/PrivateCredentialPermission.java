 ...
 * is, "read". The target name must abide by the following syntax:
 *
 * <pre>
 ...
 * com.sun.PrivateCredential owned by Subjects which have a com.sun.Principal
 * with the name, "duke". Note that although this example, as well as all the
 * examples below, do not contain Codebase, SignedBy, or Principal information
 * in the grant statement (for simplicity reasons), actual policy configurations
 ...
 * belonging to the specified {@code Subject}. If "PrincipalName" is "*", then
 * access is granted to the specified Credential owned by any {@code Subject}
 * that has the specified {@code Principal} (the actual PrincipalName doesn't
 * matter). For example, the following grants access to the a.b.Credential owned
 * by any {@code Subject} that has an a.b.Principal.
 *
 * <pre>
 *    grant {
 *      permission javax.security.auth.PrivateCredentialPermission
...
public final class PrivateCredentialPermission extends Permission {
    ...
    /**
     * @serial The Principals associated with this permission. The set contains
     *         elements of type, {@code PrivateCredentialPermission.CredOwner}.
     */
    private Set<Principal> principals; // ignored - kept around for
                                       // compatibility
    ...
    /**
     * Returns the {@code Principal} classes and names associated with this
     * {@code PrivateCredentialPermission}. The information is returned as a
     * two-dimensional array (array[x][y]). The 'x' value corresponds to the
     * number of {@code Principal} class and name pairs. When (y==0), it
     * corresponds to the {@code Principal} class value, and when (y==1), it
     * corresponds to the {@code Principal} name value. For example, array[0][0]
     ...
     * <li>the target name for <i>p</i> is implied by this object's target name.
     * For example:
     * 
     * <pre>
     ...
     * Checks that <i>obj</i> is a {@code PrivateCredentialPermission}, and has
    ...
    private boolean impliesCredentialClass(String thisC, String thatC) {
        ...
        /**
         * XXX let's not enable this for now -- if people want it, we'll enable
         * it later
         */
        /*
         * if (thisC.endsWith("*")) { String cClass = thisC.substring(0,
         * thisC.length() - 2); return thatC.startsWith(cClass); }
         */
