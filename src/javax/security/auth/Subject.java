...
/**
 * <p>
 * A {@code Subject} represents a grouping of related information for a single
 ...
 * simply bind names to a {@code Subject}. For example, a {@code Subject} that
 * happens to be a person, Alice, might have two Principals: one which binds
 * "Alice Bar", the name on her driver license, to the {@code Subject}, and
 ...
 * protection, such as private cryptographic keys, are stored within a private
 * credential {@code Set}. Credentials intended to be shared, such as public key
 ...
 * credentials belonging to a {@code Subject}, invoke the
 * {@code getPublicCredentials} method or {@code getPrivateCredentials} method,
 * respectively. To modify the returned {@code Set} of Principals and
 * credentials, use the methods defined in the {@code Set} class. For example:
 * 
 * <pre>
 * Subject subject;
 * Principal principal;
 ...
 * credentials associated with the {@code Subject} are not. Note that the
 * {@code java.security.Principal} class does not implement {@code Serializable}
 * . Therefore all concrete {@code Principal} implementations associated with
 * Subjects must implement {@code Serializable}.
 *
...
public final class Subject implements java.io.Serializable {
	 ...
	 * {@code AuthPermission("modifyPublicCredentials")}. To modify the private
	 * credential Set, the caller must have
	...
	public Subject() {
		...
		this.principals = Collections
				.synchronizedSet(new SecureSet<Principal>(this, PRINCIPAL_SET));
		this.pubCredentials = Collections
				.synchronizedSet(new SecureSet<Object>(this, PUB_CREDENTIAL_SET));
		this.privCredentials = Collections
				.synchronizedSet(new SecureSet<Object>(this, PRIV_CREDENTIAL_SET));
	}
	 ...
	 *
	 * @param readOnly
	 *            true if the {@code Subject} is to be read-only, and false
	 *            otherwise.
	 *            <p>
	 *
	 * @param principals
	 *            the {@code Set} of Principals to be associated with this
	 *            {@code Subject}.
	 *            <p>
	 *
	 * @param pubCredentials
	 *            the {@code Set} of public credentials to be associated with
	 *            this {@code Subject}.
	 *            <p>
	 *
	 * @param privCredentials
	 *            the {@code Set} of private credentials to be associated with
	 *            this {@code Subject}.
	 *
	 * @exception NullPointerException
	 *                if the specified {@code principals},
	 *                {@code pubCredentials}, or {@code privCredentials} are
	 *                {@code null}.
	 */
	public Subject(boolean readOnly, Set<? extends Principal> principals, Set<?> pubCredentials,
		...
		this.principals = Collections
				.synchronizedSet(new SecureSet<Principal>(this, PRINCIPAL_SET, principals));
		this.pubCredentials = Collections
				.synchronizedSet(new SecureSet<Object>(this, PUB_CREDENTIAL_SET, pubCredentials));
		this.privCredentials = Collections
				.synchronizedSet(new SecureSet<Object>(this, PRIV_CREDENTIAL_SET, privCredentials));
	 ...
	 *                if the caller does not have permission to set this
	 ...
	 *
	 * @exception NullPointerException
	 *                if the provided {@code AccessControlContext} is
	 *                {@code null}.
	 */
	 ...
	 * then instantiates a new {@code AccessControlContext} using the retrieved
	 * context along with a new {@code SubjectDomainCombiner} (constructed using
	 * the provided {@code Subject}). Finally, this method invokes
	 ...
	 *
	 * @param subject
	 *            the {@code Subject} that the specified {@code action} will run
	 ...
	 *
	 * @exception NullPointerException
	 *                if the {@code PrivilegedAction} is {@code null}.
	 *                <p>
	 *
	 * @exception SecurityException
	 ...
	 *                if the {@code PrivilegedExceptionAction.run} method throws
	 *                a checked exception.
	 *                <p>
	 *
	 * @exception NullPointerException
	 *                if the specified {@code PrivilegedExceptionAction} is
	 *                {@code null}.
	 *                <p>
	 *
	 * @exception SecurityException
	 ...
	 * the provided {@code AccessControlContext}. If the provided
	 ...
	 *
	 * @param acc
	 *            the {@code AccessControlContext} to be tied to the specified
	...
	public static <T> T doAsPrivileged(final Subject subject,
		...
		final AccessControlContext callerAcc = (acc == null
				? new AccessControlContext(NULL_PD_ARRAY) : acc);
	...
	private static AccessControlContext createContext(final Subject subject,
		...
		return java.security.AccessController
				.doPrivileged(new java.security.PrivilegedAction<AccessControlContext>() {
	 ...
	 *
	 * @exception NullPointerException
	 *                if the specified {@code Class} is {@code null}.
	 */
	 ...
	 * Returns true if the given object is also a Subject and the two
	 ...
	 *                private credentials for this {@code Subject}, or if the
	 *                caller does not have permission to access the private
	 *                credentials for the provided {@code Subject}.
	 */
	...
	/**
	 * package private convenience method to print out the Subject without
	 * firing off a security check when trying to access the Private Credentials
	 */
	 ...
	 *
	 * @exception SecurityException
	 *                if the caller does not have permission to access this
	...
	private void readObject(java.io.ObjectInputStream s)
		...
		try {
			principals = Collections
					.synchronizedSet(new SecureSet<Principal>(this, PRINCIPAL_SET, inputPrincs));
	...
	private static class SecureSet<E> extends AbstractSet<E> implements java.io.Serializable {
		...
		/**
		 * @serialField this$0
		 *                  Subject The outer Subject instance.
		...
		/**
		 * @serial An integer identifying the type of objects contained in this
		 *         set. If {@code which == 1}, this is a Principal set and all
		 *         the elements are of type {@code java.security.Principal}. If
		 *         {@code which == 2}, this is a public credential set and all
		 *         the elements are of type {@code Object}. If
		 *         {@code which == 3}, this is a private credential set and all
		...
		public boolean remove(Object o) {
			...
			while (e.hasNext()) {
				...
				if (which != Subject.PRIV_CREDENTIAL_SET) {
					...
					next = java.security.AccessController
							.doPrivileged(new java.security.PrivilegedAction<E>() {
								public E run() {
									return e.next();
								}
							});
		...
		public boolean contains(Object o) {
			...
			while (e.hasNext()) {
				...
				if (which != Subject.PRIV_CREDENTIAL_SET) {
					...
					SecurityManager sm = System.getSecurityManager();
					if (sm != null) {
						sm.checkPermission(new PrivateCredentialPermission(o.getClass().getName(),
								subject.getPrincipals()));
					}
					next = java.security.AccessController
							.doPrivileged(new java.security.PrivilegedAction<E>() {
		 ...
		 *             performed to ensure that the caller has permission to
	...
	/**
	 * This class implements a {@code Set} which returns only members that are
			...
			while (iterator.hasNext()) {
				...
				if (c.isAssignableFrom(next.getClass())) {
					if (which != Subject.PRIV_CREDENTIAL_SET) {
						...
						if (sm != null) {
							sm.checkPermission(new PrivateCredentialPermission(
									next.getClass().getName(), Subject.this.getPrincipals()));
						}
						set.add((T) next);
					}
				}
	...
	static class AuthPermissionHolder {
		...
		static final AuthPermission MODIFY_PRIVATE_CREDENTIALS_PERMISSION = new AuthPermission(
