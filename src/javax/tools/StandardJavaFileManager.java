...
/**
 * File manager based on {@linkplain File java.io.File}. A common way to obtain
 * an instance of this class is using
 * {@linkplain JavaCompiler#getStandardFileManager getStandardFileManager}, for
 * example:
 *
 * <pre>
 ...
 * </pre>
 *
 * This file manager creates file objects representing regular {@linkplain File
 ...
 * <ul>
 * <li>the method <code>{@linkplain FileObject#delete()}</code> is equivalent to
 * <code>{@linkplain File#delete()}</code>,</li>
 * <li>the method <code>{@linkplain FileObject#getLastModified()}</code> is
 * equivalent to <code>{@linkplain File#lastModified()}</code>,</li>
 * <li>the methods <code>{@linkplain FileObject#getCharContent(boolean)}</code>,
 * <code>{@linkplain FileObject#openInputStream()}</code>, and
 * <code>{@linkplain FileObject#openReader(boolean)}</code> must succeed if the
 * following would succeed (ignoring encoding issues): <blockquote>
 * 
 * <pre>
 ...
 * 
 * </blockquote></li>
 * <li>and the methods <code>{@linkplain FileObject#openOutputStream()}</code>,
 * and <code>{@linkplain FileObject#openWriter()}</code> must succeed if the
 * following would succeed (ignoring encoding issues): <blockquote>
 * 
 * <pre>
 ...
 * </ul>
 * </li>
 * <li>The {@linkplain java.net.URI URI} returned from
 * <code>{@linkplain FileObject#toUri()}</code>
 * <ul>
 * <li>must be {@linkplain java.net.URI#isAbsolute() absolute} (have a schema),
 * and</li>
 * <li>must have a {@linkplain java.net.URI#normalize() normalized}
 * {@linkplain java.net.URI#getPath() path component} which can be resolved
 * without any process-specific context such as the current directory (file
 * names must be absolute).</li>
 * </ul>
 * </li>
 * </ul>
 *
 ...
 * <ul>
 * <li><code>file:BobsApp/Test.java</code> (the file name is relative and depend
 * on the current directory)</li>
 * <li><code>jar:lib/vendorA.jar!com/vendora/LibraryClass.class</code> (the
 * first half of the path depends on the current directory, whereas the
 * component after ! is legal)</li>
 * <li><code>Test.java</code> (this URI depends on the current directory and
 * does not have a schema)</li>
 * <li>
 * <code>jar:///C:/Documents%20and%20Settings/UncleBob/BobsApp/../lib/vendorA.jar!com/vendora/LibraryClass.class</code>
 * (the path is not normalized)</li>
 * </ul>
 *
...
public interface StandardJavaFileManager extends JavaFileManager {
	 ...
	 * canonical file, zip file entry, or entry in any file system based
	 * container.
	 *
	 * @param a
	 *            a file object
	...
	/**
	 * Associates the given path with the given location. Any previous value
	 * will be discarded.
	 *
	 * @param location
	 *            a location
	 * @param path
	 *            a list of files, if {@code null} use the default path for this
	 *            location
	 * @see #getLocation
	 * @throws IllegalArgumentException
	 *             if location is an output location and path does not contain
	 *             exactly one element
	 * @throws IOException
	 *             if location is an output location and path does not represent
