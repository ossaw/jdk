...
public enum StandardLocation implements Location {
    ...
    /**
     * Gets a location object with the given name. The following property must
     * hold: {@code locationFor(x) ==
     * locationFor(y)} if and only if {@code x.equals(y)}. The returned location
     * will be an output location if and only if name ends with
     * {@code "_OUTPUT"}.
     *
     * @param name
     *             a name
    ...
    // where
    private static final ConcurrentMap<String, Location> locations = new ConcurrentHashMap<String, Location>();
