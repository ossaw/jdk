...
/**
 * {@code JLayer} is a universal decorator for Swing components which enables
 * you to implement various advanced painting effects as well as receive
 * notifications of all {@code AWTEvent}s generated within its borders.
 * <p>
 * {@code JLayer} delegates the handling of painting and input events to a
 * {@link javax.swing.plaf.LayerUI} object, which performs the actual
 * decoration.
 * <p>
 * The custom painting implemented in the {@code LayerUI} and events
 * notification work for the JLayer itself and all its subcomponents. This
 * combination enables you to enrich existing components by adding new advanced
 * functionality such as temporary locking of a hierarchy, data tips for
 * compound components, enhanced mouse scrolling etc and so on.
 * <p>
 * {@code JLayer} is a good solution if you only need to do custom painting over
 ...
 * </ul>
 * using any of of them will cause {@code UnsupportedOperationException} to be
 * thrown, to add a component to {@code JLayer} use {@link #setView(Component)}
...
public final class JLayer<V extends Component> extends JComponent implements Scrollable,
    ...
    /**
     * Creates a new {@code JLayer} object with the specified view component and
    ...
    /**
     * Returns the {@code JLayer}'s view component or {@code null}. <br>
     * This is a bound property.
     *
     * @return the {@code JLayer}'s view component or {@code null} if none
     *         exists
     * @see #setView(Component)
     */
    public V getView() {
    ...
    /**
     * Sets the {@code JLayer}'s view component, which can be {@code null}. <br>
     * This is a bound property.
     *
     * @param view
    ...
    /**
     * Sets the {@link javax.swing.plaf.LayerUI} which will perform painting and
    ...
    /**
     * Returns the {@code JLayer}'s glassPane component or {@code null}. <br>
     * This is a bound property.
     *
     * @return the {@code JLayer}'s glassPane component or {@code null} if none
     *         exists
     ...
     * prevent the layout manager from being set.
     * <p>
     * Note: If {@code mgr} is non-{@code null}, this method will throw an
     * exception as layout managers are not supported on a {@code JLayer}.
     *
     * @param mgr
    ...
    /**
     * A non-{@code null} border, or non-zero insets, isn't supported, to
     * prevent the geometry of this component from becoming complex enough to
     * inhibit subclassing of {@code LayerUI} class. To create a {@code JLayer}
     * with a border, add it to a {@code JPanel} that has a border.
     * <p>
     * Note: If {@code border} is non-{@code null}, this method will throw an
    ...
    /**
     * This method is not supported by {@code JLayer} and always throws
    ...
    /**
     * Always returns {@code true} to cause painting to originate from
     * {@code JLayer}, or one of its ancestors.
     *
     * @return true
     ...
     * {@link javax.swing.plaf.LayerUI#paintImmediately(int, int, int, int, JLayer)}
     ...
     * (in {@code JComponent}) to return {@code false}. This ensures that the
     * drawing machinery will call the {@code JLayer}'s {@code paint}
     * implementation rather than messaging the {@code JLayer}'s children
     * directly.
     *
     * @return false
     */
    ...
    /**
     * Enables the events from JLayer and <b>all its descendants</b> defined by
     * the specified event mask parameter to be delivered to the
     * {@link LayerUI#eventDispatched(AWTEvent, JLayer)} method.
     * <p>
     * Events are delivered provided that {@code LayerUI} is set for this
     * {@code JLayer} and the {@code JLayer} is displayable.
     * <p>
     * The following example shows how to correctly use this method in the
     * {@code LayerUI} implementations:
     * 
     * <pre>
     * public void installUI(JComponent c) {
     *     super.installUI(c);
     ...
     * </pre>
     *
     * By default {@code JLayer} receives no events and its event mask is
     * {@code 0}.
     *
    ...
    /**
     * Returns the bitmap of event mask to receive by this {@code JLayer} and
     * its {@code LayerUI}.
     * <p>
     * It means that
    ...
    /**
     * Delegates its functionality to the
     * {@link javax.swing.plaf.LayerUI#updateUI(JLayer)} method, if
     * {@code LayerUI} is set.
     */
     ...
     * <p>
     * If the view component of this layer implements {@link Scrollable}, this
     * method delegates its implementation to the view component.
     *
     ...
     * logical rows or columns in order to completely expose one block of rows
     * or columns, depending on the value of orientation.
     * <p>
     * If the view component of this layer implements {@link Scrollable}, this
     * method delegates its implementation to the view component.
     *
    ...
    public int getScrollableBlockIncrement(Rectangle visibleRect, int orientation, int direction) {
        if (getView() instanceof Scrollable) {
            return ((Scrollable) getView()).getScrollableBlockIncrement(visibleRect, orientation, direction);
        }
     ...
     * not determine the height of the layer, unless the preferred height of the
     * layer is smaller than the height of the viewport.
     * <p>
     * If the view component of this layer implements {@link Scrollable}, this
     * method delegates its implementation to the view component.
     *
     ...
     * determine the width of the layer, unless the preferred width of the layer
     * is smaller than the width of the viewport.
     * <p>
     * If the view component of this layer implements {@link Scrollable}, this
     * method delegates its implementation to the view component.
     *
    ...
    /**
     * Returns a scroll increment, which is required for components that display
     * logical rows or columns in order to completely expose one new row or
     * column, depending on the value of orientation. Ideally, components should
     * handle a partially exposed row or column by returning the distance
     * required to completely expose the item.
     * <p>
     * Scrolling containers, like {@code JScrollPane}, will use this method each
    ...
    public int getScrollableUnitIncrement(Rectangle visibleRect, int orientation, int direction) {
        if (getView() instanceof Scrollable) {
            return ((Scrollable) getView()).getScrollableUnitIncrement(visibleRect, orientation, direction);
        }
        return 1;
    }
    ...
    /**
     * Delegates its functionality to the
     * {@link javax.swing.plaf.LayerUI#doLayout(JLayer)} method, if
     * {@code LayerUI} is set.
     */
    ...
    private static class LayerEventController implements AWTEventListener {
        private ArrayList<Long> layerMaskList = new ArrayList<Long>();

        private long currentEventMask;

        private static final long ACCEPTED_EVENTS = AWTEvent.COMPONENT_EVENT_MASK
                | AWTEvent.CONTAINER_EVENT_MASK | AWTEvent.FOCUS_EVENT_MASK | AWTEvent.KEY_EVENT_MASK
                | AWTEvent.MOUSE_WHEEL_EVENT_MASK | AWTEvent.MOUSE_MOTION_EVENT_MASK
                | AWTEvent.MOUSE_EVENT_MASK | AWTEvent.INPUT_METHOD_EVENT_MASK | AWTEvent.HIERARCHY_EVENT_MASK
        ...
        private boolean isEventEnabled(long eventMask, int id) {
            return (((eventMask & AWTEvent.COMPONENT_EVENT_MASK) != 0 && id >= ComponentEvent.COMPONENT_FIRST
                    && id <= ComponentEvent.COMPONENT_LAST) || ((eventMask
                            & AWTEvent.CONTAINER_EVENT_MASK) != 0 && id >= ContainerEvent.CONTAINER_FIRST
                            && id <= ContainerEvent.CONTAINER_LAST) || ((eventMask
                                    & AWTEvent.FOCUS_EVENT_MASK) != 0 && id >= FocusEvent.FOCUS_FIRST
                                    && id <= FocusEvent.FOCUS_LAST) || ((eventMask
                                            & AWTEvent.KEY_EVENT_MASK) != 0 && id >= KeyEvent.KEY_FIRST
                                            && id <= KeyEvent.KEY_LAST) || ((eventMask
                                                    & AWTEvent.MOUSE_WHEEL_EVENT_MASK) != 0
                                                    && id == MouseEvent.MOUSE_WHEEL) || ((eventMask
                    ...
                    || ((eventMask & AWTEvent.MOUSE_EVENT_MASK) != 0 && id != MouseEvent.MOUSE_MOVED
                            ...
                            && id >= MouseEvent.MOUSE_FIRST && id <= MouseEvent.MOUSE_LAST) || ((eventMask
                                    & AWTEvent.INPUT_METHOD_EVENT_MASK) != 0
                                    && id >= InputMethodEvent.INPUT_METHOD_FIRST
                                    && id <= InputMethodEvent.INPUT_METHOD_LAST) || ((eventMask
                                            & AWTEvent.HIERARCHY_EVENT_MASK) != 0
                                            && id == HierarchyEvent.HIERARCHY_CHANGED) || ((eventMask
    ...
    /**
     * The default glassPane for the {@link javax.swing.JLayer}. It is a
     * subclass of {@code JPanel} which is non opaque by default.
     */
    private static class DefaultLayerGlassPane extends JPanel {
        ...
        /**
         * First, implementation of this method iterates through glassPane's
         * child components and returns {@code true} if any of them is visible
         * and contains passed x,y point. After that it checks if no
         * mouseListeners is attached to this component and no mouse cursor is
         * set, then it returns {@code false}, otherwise calls the super
