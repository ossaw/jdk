 ...
 * <code>ActionListener</code> interface, this interface allows the application
 * to define, in a single place:
 * <ul>
 ...
 * (typically, by subclassing <code>AbstractAction</code>). The
 * <code>Action</code> object can then be added to multiple <code>Action</code>
 * -aware containers and connected to <code>Action</code>-capable components.
 * The GUI controls can then be activated or deactivated all at once by invoking
 * the <code>Action</code> object's <code>setEnabled</code> method.
 * <p>
 * Note that <code>Action</code> implementations tend to be more expensive in
 * terms of storage than a typical <code>ActionListener</code>, which does not
 * offer the benefits of centralized control of functionality and broadcast of
 ...
 * <code>Action</code></h3>
 * <p>
 * Many of Swing's components have an <code>Action</code> property. When an
 * <code>Action</code> is set on a component, the following things happen:
 * <ul>
 * <li>The <code>Action</code> is added as an <code>ActionListener</code> to the
 * component.
 ...
 * <code>Action</code> so that the component can change its properties to
 * reflect changes in the <code>Action</code>'s properties.
 * </ul>
 * <p>
 * The following table describes the properties used by <code>Swing</code>
 * components that support <code>Actions</code>. In the table, <em>button</em>
 * refers to any <code>AbstractButton</code> subclass, which includes not only
 * <code>JButton</code> but also classes such as <code>JMenuItem</code>. Unless
 * otherwise stated, a <code>null</code> property value in an
 ...
 * <td>A <code>null</code> value or <code>Action</code> results in the button's
 * <code>mnemonic</code> property being set to <code>'\0'</code>.
 * <tr valign="top" align="left">
 * <td><b><code>text</code></b>
 * <td>All buttons
 * <td><code>NAME</code>
 * <td>If you do not want the text of the button to mirror that of the
 * <code>Action</code>, set the property <code>hideActionText</code> to
 * <code>true</code>. If <code>hideActionText</code> is <code>true</code>,
 * setting the <code>Action</code> changes the text of the button to
 * <code>null</code> and any changes to <code>NAME</code> are ignored.
 * <code>hideActionText</code> is useful for tool bar buttons that typically
 * only show an <code>Icon</code>. <code>JToolBar.add(Action)</code> sets the
 * property to <code>true</code> if the <code>Action</code> has a non-
 * <code>null</code> value for <code>LARGE_ICON_KEY</code> or
 * <code>SMALL_ICON</code>.
 * <tr valign="top" align="left">
 * <td><b><code>displayedMnemonicIndex</code></b>
 * <td>All buttons
 * <td><code>DISPLAYED_MNEMONIC_INDEX_KEY</code>
 * <td>If the value of <code>DISPLAYED_MNEMONIC_INDEX_KEY</code> is beyond the
 * bounds of the text, it is ignored. When <code>setAction</code> is called, if
 * the value from the <code>Action</code> is <code>null</code>, the displayed
 * mnemonic index is not updated. In any subsequent changes to
 * <code>DISPLAYED_MNEMONIC_INDEX_KEY</code>, <code>null</code> is treated as
 * -1.
 * <tr valign="top" align="left">
 * <td><b><code>icon</code></b>
 * <td>All buttons except of <code>JCheckBox</code>, <code>JToggleButton</code>
 * and <code>JRadioButton</code>.
 * <td>either <code>LARGE_ICON_KEY</code> or <code>SMALL_ICON</code>
 * <td>The <code>JMenuItem</code> subclasses only use <code>SMALL_ICON</code>.
 * All other buttons will use <code>LARGE_ICON_KEY</code>; if the value is
 * <code>null</code> they use <code>SMALL_ICON</code>.
 * <tr valign="top" align="left">
 ...
 * {@code null} value for {@code SELECTED_KEY} on a {@code JToggleButton}, the
 * {@code JToggleButton} will not update it's selected state in any way.
 * Similarly, any time the {@code JToggleButton}'s selected state changes it
 * will only set the value back on the {@code Action} if the {@code Action} has
 * a {@code non-null} value for {@code SELECTED_KEY}. <br>
 * Components that honor this property keep their selected state in sync with
 * this property. When the same {@code Action} is used with multiple components,
 * all the components keep their selected state in sync with this property.
 * Mutually exclusive buttons, such as {@code JToggleButton}s in a
 * {@code ButtonGroup}, force only one of the buttons to be selected. As such,
 * do not use the same {@code Action} that defines a value for the
 * {@code SELECTED_KEY} property with multiple mutually exclusive buttons.
 * </table>
 * <p>
 * <code>JPopupMenu</code>, <code>JToolBar</code> and <code>JMenu</code> all
 * provide convenience methods for creating a component and setting the
 * <code>Action</code> on the corresponding component. Refer to each of these
 * classes for more information.
 * <p>
 * <code>Action</code> uses <code>PropertyChangeListener</code> to inform
 * listeners the <code>Action</code> has changed. The beans specification
 * indicates that a <code>null</code> property name can be used to indicate
 * multiple values have changed. By default Swing components that take an
 * <code>Action</code> do not handle such a change. To indicate that Swing
 * should treat <code>null</code> according to the beans specification set the
 * system property <code>swing.actions.reconfigureOnNull</code> to the
