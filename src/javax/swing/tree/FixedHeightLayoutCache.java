...
public class FixedHeightLayoutCache extends AbstractLayoutCache {
	...
	/**
	 * Sets the height of each cell. If rowHeight is less than or equal to 0
	...
	/**
	 * Does nothing, FixedHeightLayoutCache doesn't cache width, and that is all
	 * that could change.
	 */
	...
	public Enumeration<TreePath> getVisiblePathsFrom(TreePath path) {
		...
		node = getNodeForPath(parentPath, true, false);
		if (node != null && node.isExpanded()) {
			return new VisibleFHTreeStateNodeEnumeration(node, treeModel.getIndexOfChild(
					parentPath.getLastPathComponent(), path.getLastPathComponent()));
		}
		return null;
	}
			...
			if (changedParent != null) {
				if (changedIndexs != null && (maxCounter = changedIndexs.length) > 0) {
					...
					for (int counter = 0; counter < maxCounter; counter++) {
						...
						if (child != null) {
							child.setUserObject(
									treeModel.getChild(parentValue, changedIndexs[counter]));
						}
					}
	...
	/**
	 * Returns the bounds for the given node. If <code>childIndex</code> is -1,
	...
	/**
	 * Adjust the large row count of the AbstractTreeUI the receiver was created
	 * with.
	 */
	...
	/**
	 * FHTreeStateNode is used to track what has been expanded. FHTreeStateNode
	 * differs from VariableHeightTreeState.TreeStateNode in that it is highly
	 * model intensive. That is almost all queries to a FHTreeStateNode result
	 * in the TreeModel being queried. And it obviously does not support
	 * variable sized row heights.
	 */
	private class FHTreeStateNode extends DefaultMutableTreeNode {
		 ...
		 * <code>index</code> index is the index of the child to start adjusting
		 * from, which is not necessarily the model index.
		 */
		...
		/**
		 * Resets the receivers children's rows. Starting with the child at
		...
		/**
		 * Collapses the receiver. If <code>adjustRows</code> is true, the rows
		 * of nodes after the receiver are adjusted.
		 */
		 ...
		 * about the found row is returned in <code>info</code>. This should be
		 * invoked on root with <code>nextRow</code> set to
		 ...
		 * <code>stopIndex</code>. This does not include the number of children
		 * that the child at <code>stopIndex</code> might have.
		 */
