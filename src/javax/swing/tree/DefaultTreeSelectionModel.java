...
public class DefaultTreeSelectionModel implements Cloneable, Serializable,
    ...
    /**
     * Handles maintaining the list selection model. The RowMapper is used to
     * map from a TreePath to a row, and the value is then placed here.
     */
    ...
    /**
     * Used to make sure the paths are unique, will contain all the paths in
     * <code>selection</code>.
     */
    ...
    /**
     * Sets the RowMapper instance. This instance is used to determine the row
     * for a particular TreePath.
     */
     ...
     * CONTIGUOUS_TREE_SELECTION or DISCONTIGUOUS_TREE_SELECTION. If mode is not
     * one of the defined value, <code>DISCONTIGUOUS_TREE_SELECTION</code> is
     * assumed.
     * <p>
     ...
     * <p>
     * Setting the mode to something other than the defined types will result in
     * the mode becoming <code>DISCONTIGUOUS_TREE_SELECTION</code>.
     */
    public void setSelectionMode(int mode) {
        ...
        if (oldMode != selectionMode && changeSupport != null)
            changeSupport.firePropertyChange(SELECTION_MODE_PROPERTY, Integer
    ...
    /**
     * Sets the selection. Whether the supplied paths are taken as the new
     * selection depends upon the selection mode. If the supplied array is
     * {@code null}, or empty, the selection is cleared. If the selection mode
     * is {@code SINGLE_TREE_SELECTION}, only the first path in {@code pPaths}
     * is used. If the selection mode is {@code CONTIGUOUS_TREE_SELECTION} and
     * the supplied paths are not contiguous, then only the first path in
     * {@code pPaths} is used. If the selection mode is
     ...
     * <p>
     * The paths returned from {@code getSelectionPaths} are in the same order
     * as those supplied to this method.
     *
     * @param pPaths
     *               the new selection
     */
    public void setSelectionPaths(TreePath[] pPaths) {
        ...
        if ((newCount + oldCount) != 0) {
            if (selectionMode == TreeSelectionModel.SINGLE_TREE_SELECTION) {
                ...
                /*
                 * If contiguous selection and paths aren't contiguous, only
                 * select the first path item.
                 */
            ...
            Vector<PathPlaceHolder> cPaths = new Vector<PathPlaceHolder>(
                    newCount + oldCount);
            List<TreePath> newSelectionAsList = new ArrayList<TreePath>(
                    newCount);
            ...
            for (oldCounter = 0; oldCounter < oldCount; oldCounter++)
                if (selection[oldCounter] != null && lastPaths.get(
                        selection[oldCounter]) == null)
                    cPaths.addElement(new PathPlaceHolder(selection[oldCounter],
                            false));
     ...
     * <p>
     * The lead path is set to the last element in <code>paths</code>.
     * <p>
     * If the selection mode is <code>CONTIGUOUS_TREE_SELECTION</code>, and
     * adding the new paths would make the selection discontiguous. Then two
     * things can result: if the TreePaths in <code>paths</code> are contiguous,
     * then the selection becomes these TreePaths, otherwise the TreePaths
     * aren't contiguous and the selection becomes the first TreePath in
     * <code>paths</code>.
     *
     * @param paths
     *              the new path to add to the current selection
        ...
        if (newPathLength > 0) {
            if (selectionMode == TreeSelectionModel.SINGLE_TREE_SELECTION) {
                ...
                if (selection == null)
                    ...
                    oldCount = selection.length;
                /*
                 * Determine the paths that aren't currently in the selection.
                 */
                lastPaths.clear();
                ...
                if (validCount > 0) {
                    ...
                    if (validCount != paths.length) {
                        /*
                         * Some of the paths in paths are already in the
                         * selection.
                         */
    ...
    public void removeSelectionPaths(TreePath[] paths) {
        if (paths != null && selection != null && paths.length > 0) {
            if (!canPathsBeRemoved(paths)) {
                ...
                for (int removeCounter = paths.length
                        - 1; removeCounter >= 0; removeCounter--) {
    ...
    /**
     * Returns the first path in the selection. This is useful if there if only
     ...
     * model.
     *
     * @return all of this model's <code>TreeSelectionListener</code>s or an
     ...
     * you can query a <code>DefaultTreeSelectionModel</code> <code>m</code> for
     * its tree selection listeners with the following code:
     *
     * <pre>
     * TreeSelectionListener[] tsls = (TreeSelectionListener[]) (m.getListeners(
    ...
    /**
     * Returns the selection in terms of rows. There is not necessarily a
     * one-to-one mapping between the {@code TreePath}s returned from
     * {@code getSelectionPaths} and this method. In particular, if a
     * {@code TreePath} is not viewable (the {@code
     * RowMapper} returns {@code -1} for the row corresponding to the
     * {@code TreePath}), then the corresponding row is not included in the
     * returned array. For example, if the selection consists of two paths,
     * {@code A} and {@code B}, with {@code A} at row {@code 10}, and {@code B}
     * not currently viewable, then this method returns an array with the single
     * entry {@code 10}.
     *
    ...
    /**
     * Returns the smallest value obtained from the RowMapper for the current
     * set of selected TreePaths. If nothing is selected, or there is no
     * RowMapper, this will return -1.
     */
    ...
    /**
     * Returns the largest value obtained from the RowMapper for the current set
     * of selected TreePaths. If nothing is selected, or there is no RowMapper,
     * this will return -1.
     */
    ...
    /**
     * Updates this object's mapping from TreePath to rows. This should be
     * invoked when the mapping from TreePaths to integers has changed (for
     * example, a node has been expanded).
     * <p>
     * You do not normally have to call this, JTree and its associated Listeners
     * will invoke this for you. If you are implementing your own View class,
     * then you will have to invoke this.
     * <p>
     * This will invoke <code>insureRowContinuity</code> to make sure the
     * currently selected TreePaths are still valid based on the selection mode.
     */
     ...
     *
     * @return all of this model's <code>PropertyChangeListener</code>s or an
    ...
    /**
     * Makes sure the currently selected <code>TreePath</code>s are valid for
     * the current selection mode. If the selection mode is
     * <code>CONTIGUOUS_TREE_SELECTION</code> and a <code>RowMapper</code>
     * exists, this will make sure all the rows are contiguous, that is, when
     * sorted all the rows are in order with no gaps. If the selection isn't
     * contiguous, the selection is reset to contain the first set, when sorted,
     * of contiguous rows.
     * <p>
     * If the selection mode is <code>SINGLE_TREE_SELECTION</code> and more than
     * one TreePath is selected, the selection is reset to contain the first
                ...
                for (int counter = min, maxCounter = lModel
                    ...
                    if (!lModel.isSelectedIndex(counter)) {
                        if (counter == min) {
                            ...
                            int selectionIndex[] = rowMapper.getRowsForPaths(
                                    selection);
                            // find the actual selection pathes corresponded to
                            // the
                            // rows of the new selection
                            for (int i = 0; i < selectionIndex.length; i++) {
                                if (selectionIndex[i] < counter) {
                                    newSel[selectionIndex[i]
                                            - min] = selection[i];
                                }
                            }
    ...
    /**
     * Returns true if the paths are contiguous, or this object has no
     * RowMapper.
     */
     ...
     * This will return true if <code>paths</code> is null (or empty), or this
     * object has no RowMapper, or nothing is currently selected, or the
     * selection mode is <code>DISCONTIGUOUS_TREE_SELECTION</code>, or adding
     * the paths to the current selection still results in a contiguous set of
    ...
    /**
     * Returns true if the paths can be removed without breaking the continuity
     * of the model. This is rather expensive.
     */
    ...
    protected void updateLeadIndex() {
        if (leadPath != null) {
            if (selection == null) {
                ...
                leadRow = leadIndex = -1;
                for (int counter = selection.length
                        - 1; counter >= 0; counter--) {
    ...
    /**
     * This method is obsolete and its implementation is now a noop. It's still
     * called by setSelectionPaths and addSelectionPaths, but only for backwards
     * compatibility.
     */
    ...
    public String toString() {
        ...
        for (int counter = 0; counter < selCount; counter++) {
            if (rows != null)
                retBuffer.append(selection[counter].toString() + "@" + Integer
                        .toString(rows[counter]) + " ");
            else
     ...
     *                                       never thrown by instances of this
     *                                       class
     */
    public Object clone() throws CloneNotSupportedException {
        ...
        clone.listSelectionModel = (DefaultListSelectionModel) listSelectionModel
                .clone();
