...
public class VariableHeightLayoutCache extends AbstractLayoutCache {
    /**
     * The array of nodes that are currently visible, in the order they are
     * displayed.
     */
    ...
    /**
     * The root node of the internal cache of nodes that have been shown. If the
     * treeModel is vending a network rather than a true tree, there may be one
     * cached node for each path to a modeled node.
     */
    private TreeStateNode root;

    /**
     * Used in getting sizes for nodes to avoid creating a new Rectangle every
     * time a size is needed.
     */
    ...
    /**
     * Sets the height of each cell. If the specified value is less than or
     ...
     * 
     * @param path
     *        the <code>TreePath</code> of interest
     * @param isExpanded
    ...
    /**
     * Returns the <code>Rectangle</code> enclosing the label portion into which
     * the item identified by <code>path</code> will be drawn.
     *
     * @param path
     *        the path to be drawn
     * @param placeIn
     *        the bounds of the enclosing rectangle
     * @return the bounds of the enclosing rectangle or <code>null</code> if the
     *         node could not be ascertained
     */
    ...
    /**
     * Returns the path for <code>row</code>. If <code>row</code> is not
     * visible, <code>null</code> is returned.
     *
     * @param row
     *        the location of interest
     * @return the path for <code>row</code>, or <code>null</code> if
    ...
    /**
     * Returns the row where the last item identified in path is visible. Will
     ...
     *
     * @param path
     *        the <code>TreePath</code> which is now invalid
     */
     ...
     * currently visible this will return <code>null</code>, otherwise it will
     * always return a valid path. If you need to test if the returned object is
     * exactly at x, y you should get the bounds for the returned path and test
     * x, y against that.
     *
     * @param x
     *        the x-coordinate
     ...
     * starting at the passed in location. The ordering of the enumeration is
     * based on how the paths are displayed.
     *
     * @param path
     *        the location in the <code>TreePath</code> to start
    ...
    /**
     * Informs the <code>TreeState</code> that it needs to recalculate all the
     * sizes it is referencing.
     */
    ...
    /**
     * Returns true if the value identified by <code>path</code> is currently
     * expanded.
     * 
     * @return true if the value identified by <code>path</code> is currently
     *         expanded
     */
     ...
     * in the file system.
     * <p>
     * <code>e.path</code> returns the path the parent of the changed node(s).
     * <p>
     * <code>e.childIndices</code> returns the index(es) of the changed node(s).
     *
     * @param e
     *        the <code>TreeModelEvent</code> of interest
     */
    public void treeNodesChanged(TreeModelEvent e) {
        if (e != null) {
            ...
            if (changedNode != null) {
                ...
                /*
                 * Update the size of the changed node, as well as all the child
                 * indexs that are passed in.
                 */
                ...
                if (changedNode.hasBeenExpanded() && changedIndexs != null) {
                    ...
                    for (counter = 0; counter < changedIndexs.length; counter++) {
                        changedChildNode = (TreeStateNode) changedNode.getChildAt(changedIndexs[counter]);
                        /* Reset the user object. */
                        changedChildNode.setUserObject(treeModel.getChild(changedValue,
     ...
     * <p>
     * <code>e.path</code> returns the parent of the new nodes.
     * <p>
     * <code>e.childIndices</code> returns the indices of the new nodes in
     * ascending order.
     *
     * @param e
     *        the <code>TreeModelEvent</code> of interest
        ...
        if (e != null) {
            ...
            if (changedParentNode != null && changedIndexs != null && changedIndexs.length > 0) {
                if (changedParentNode.hasBeenExpanded()) {
                    ...
                    makeVisible = ((changedParentNode == root && !rootVisible) || (changedParentNode
                            .getRow() != -1 && changedParentNode.isExpanded()));
                    for (counter = 0; counter < changedIndexs.length; counter++) {
                    ...
                    if (treeSelectionModel != null)
                        treeSelectionModel.resetRowSelection();
                    /*
                     * Update the y origins from the index of the parent to the
                     * end of the visible rows.
                     */
                    if (!isFixedRowHeight() && (makeVisible || (oldChildCount == 0 && changedParentNode
                            .isVisible()))) {
     ...
     * siblings removed.
     * <p>
     * <code>e.path</code> returns the former parent of the deleted nodes.
     * <p>
     * <code>e.childIndices</code> returns the indices the nodes had before they
     * were deleted in ascending order.
     *
     * @param e
     *        the <code>TreeModelEvent</code> of interest
     */
        ...
        if (e != null) {
            ...
            if (changedParentNode != null && changedIndexs != null && changedIndexs.length > 0) {
                if (changedParentNode.hasBeenExpanded()) {
                    ...
                    makeInvisible = ((changedParentNode == root && !rootVisible) || (changedParentNode
                            .getRow() != -1 && changedParentNode.isExpanded()));
                    for (counter = changedIndexs.length - 1; counter >= 0; counter--) {
                        removedNode = (TreeStateNode) changedParentNode.getChildAt(changedIndexs[counter]);
                     ...
                     * end of the visible rows.
                     */
                    if (!isFixedRowHeight() && (makeInvisible || (changedParentNode.getChildCount() == 0
                            && changedParentNode.isVisible()))) {
                        if (changedParentNode == root) {
                            /*
                             * It is possible for first row to have been removed
                             * if the root isn't visible, in which case
                             * ylocations will be off!
                             */
                            if (getRowCount() > 0)
                                getNode(0).setYOrigin(0);
                            updateYLocationsFrom(0);
     ...
     * <p>
     * <code>e.path</code> holds the path to the node.
     * <p>
     * <code>e.childIndices</code> returns <code>null</code>.
     *
     * @param e
     *        the <code>TreeModelEvent</code> of interest
     */
    ...
    /**
     * Returns the node previously added for <code>path</code>. This may return
     * null, if you to create a node use getNodeForPath.
     */
     ...
     * <code>placeIn</code>. If <code>placeIn</code> is null a new Rectangle
     ...
     * already exist and <I>parent</I> has been expanded at least once. The
     * newly created node will be made visible if <I>parent</I> is currently
     * expanded. This does not update the position of any cells, nor update the
     * selection if it needs to be. If succesful in creating the new
     * TreeStateNode, it is returned, otherwise null is returned.
     */
    private TreeStateNode createNodeAt(TreeStateNode parent, int childIndex) {
        ...
        if (newChildNode != null && parent.isExpanded() && (parent.getRow() != -1 || isParentRoot)) {
            ...
            if (childIndex == 0) {
                ...
                previousNode = (TreeStateNode) parent.getChildAt(childIndex - 1);
    ...
    /**
     * Returns the TreeStateNode identified by path. This mirrors the behavior
     * of getNodeForPath, but tries to take advantage of path if it is an
     * instance of AbstractTreePath.
     */
                ...
                while (path != null) {
                    ...
                    if (node != null) {
                        ...
                        while (node != null && paths.size() > 0) {
                            ...
                            int childIndex = treeModel.getIndexOfChild(node.getUserObject(), path
                                    .getLastPathComponent());

                            if (childIndex == -1 || childIndex >= node.getChildCount() || (onlyIfVisible
                                    && !node.isVisible())) {
                                node = null;
                            } else
    ...
    private void updateYLocationsFrom(int location) {
        if (location >= 0 && location < getRowCount()) {
            ...
            for (counter = location + 1, maxCounter = visibleNodes.size(); counter < maxCounter; counter++) {
                aNode = (TreeStateNode) visibleNodes.elementAt(counter);
    ...
    /**
     * Resets the y origin of all the visible nodes as well as messaging all the
     * visible nodes to updatePreferredSize(). You should not normally have to
     * call this. Expanding and contracting the nodes automaticly adjusts the
     * locations. updateAll determines if updatePreferredSize() is call on all
     * nodes or just those that don't have a valid size.
     */
    ...
    /**
     * Returns the index of the row containing location. If there are no rows,
     * -1 is returned. If location is beyond the last row index, the last row
     * index is returned.
     */
    ...
    /**
     * Responsible for creating a TreeStateNode that will be used to track
     * display information about value.
     */
    ...
    /**
     * TreeStateNode is used to keep track of each of the nodes that have been
     * expanded. This will also cache the preferred size of the value it
     * represents.
     */
    private class TreeStateNode extends DefaultMutableTreeNode {
        ...
        /**
         * Returns the children of the receiver. If the receiver is not
         * currently expanded, this will return an empty enumeration.
         */
        ...
        public Rectangle getNodeBounds(Rectangle placeIn) {
            if (placeIn == null)
                placeIn = new Rectangle(getXOrigin(), getYOrigin(), getPreferredWidth(),
                        getPreferredHeight());
            else {
        ...
        /**
         * Returns the number of children this will have. If the children have
         * not yet been loaded, this messages the model.
         */
        ...
        public int getVisibleChildCount() {
            ...
            if (isExpanded()) {
                ...
                for (int counter = 0; counter < maxCounter; counter++)
                    childCount += ((TreeStateNode) getChildAt(counter)).getVisibleChildCount();
            }
            return childCount;
        }
        ...
        /**
         * Returns the value the receiver is representing. This is a cover for
         * getUserObject.
         */
        ...
        /**
         * Sets y origin the user object will be drawn at to <I>newYOrigin</I>.
         */
        ...
        /**
         * Updates the preferred size by asking the current renderer for the
         * Dimension needed to draw the user object this instance represents.
         */
        protected void updatePreferredSize(int index) {
            Rectangle bounds = getNodeDimensions(this.getUserObject(), index, getLevel(), isExpanded(),
                    boundsBuffer);
        ...
        /**
         * Marks the receivers size as invalid. Next time the size, location is
         * asked for it will be obtained.
         */
        ...
        /**
         * Returns the children of the receiver. If the children haven't been
         * loaded from the model and <code>createIfNeeded</code> is true, the
         * children are first loaded.
         */
        protected Enumeration getLoadedChildren(boolean createIfNeeded) {
            ...
            if (childRow == -1) {
                for (int i = 0; i < count; i++) {
                    newNode = createNodeForValue(treeModel.getChild(realNode, i));
                    this.add(newNode);
        ...
        /**
         * Messaged from expand and collapse. This is meant for subclassers that
         * may wish to do something interesting with this.
         */
        ...
        /**
         * Expands this node in the tree. This will load the children from the
         * treeModel if this node has not previously been expanded. If
         * <I>adjustTree</I> is true the tree and selection are updated
         * accordingly.
         */
            ...
            if (!isExpanded() && !isLeaf()) {
                ...
                if (adjustTree && (originalRow != i || getPreferredHeight() != startHeight)) {
                    ...
                    if (!isFixed && ++i < getRowCount()) {
                        int counter;
                        int heightDiff = newYOrigin - (getYOrigin() + getPreferredHeight())
                                + (getPreferredHeight() - startHeight);

                        for (counter = visibleNodes.size() - 1; counter >= i; counter--)
                            ((TreeStateNode) visibleNodes.elementAt(counter)).shiftYOriginBy(heightDiff);
                    }
                    didAdjustTree();
                    visibleNodesChanged();
                }
        ...
        /**
         * Collapses this node in the tree. If <I>adjustTree</I> is true the
         * tree and selection are updated accordingly.
         */
        protected void collapse(boolean adjustTree) {
            if (isExpanded()) {
                ...
                if (!isFixed) {
                    while (cursor.hasMoreElements()) {
                        TreeStateNode node = (TreeStateNode) cursor.nextElement();
                        if (node.isVisible()) {
                            rowsDeleted++;
                ...
                if (myRow != -1 && adjustTree && (rowsDeleted > 0 || startHeight != getPreferredHeight())) {
                    ...
                    if (!isFixed && (myRow + 1) < getRowCount() && startYEnd != lastYEnd) {
                        int counter, maxCounter, shiftAmount;

                        shiftAmount = startYEnd - lastYEnd;
                        for (counter = myRow + 1, maxCounter = visibleNodes
                                .size(); counter < maxCounter; counter++)
                            ((TreeStateNode) visibleNodes.elementAt(counter)).shiftYOriginBy(shiftAmount);
                    }
                    didAdjustTree();
                    visibleNodesChanged();
                }
    ...
    private class VisibleTreeStateNodeEnumeration implements Enumeration<TreePath> {
        ...
        public TreePath nextElement() {
            ...
            if (nextIndex == -1) {
                ...
                TreeStateNode node = (TreeStateNode) parent.getChildAt(nextIndex);
        ...
        protected boolean updateNextIndex() {
            ...
            TreeStateNode child = (TreeStateNode) parent.getChildAt(nextIndex);
