...
public class DefaultMutableTreeNode implements Cloneable, MutableTreeNode, Serializable {
     ...
     * <code>ancestor</code>, inclusive. Each <code>nextElement()</code> message
     * is O(1).
     * <P>
     * Modifying the tree by inserting, removing, or moving a node invalidates
     * any enumerations created before the modification.
     ...
     * (if it has no parent or no siblings, this method returns <code>1</code>).
     *
    ...
    public DefaultMutableTreeNode getNextSibling() {
        ...
        if (myParent == null) {
            ...
            retval = (DefaultMutableTreeNode) myParent.getChildAfter(this); // linear
                                                                            // search
        }
    ...
    public DefaultMutableTreeNode getPreviousSibling() {
        ...
        if (myParent == null) {
            ...
            retval = (DefaultMutableTreeNode) myParent.getChildBefore(this); // linear
                                                                             // search
        }
     ...
     * should use <code>depthFirstEnumeration</code> to enumerate the nodes in
     * the tree and use <code>isLeaf</code> on each node to determine which are
     * leaves.
     *
     * @see #depthFirstEnumeration
     * @see #isLeaf
     ...
     * this node is a leaf, returns <code>1</code>. This method is O(n) where n
        ...
        public PathBetweenNodesEnumeration(TreeNode ancestor, TreeNode descendant) {
            ...
            while (current != ancestor) {
                ...
                if (current == null && descendant != ancestor) {
                    throw new IllegalArgumentException("node " + ancestor + " is not an ancestor of "
                            + descendant);
                }
                stack.push(current);
            }
