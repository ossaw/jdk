...
public abstract class AbstractLayoutCache implements RowMapper {
    ...
    /**
     * Height to use for each row. If this is &lt;= 0 the renderer will be used
     * to determine the height for each row.
     */
    protected int rowHeight;

    /**
     * Sets the renderer that is responsible for drawing nodes in the tree and
     * which is therefore responsible for calculating the dimensions of
     * individual nodes.
     *
     * @param nd
     *        a <code>NodeDimensions</code> object
     */
    ...
    /**
     * Sets the <code>TreeSelectionModel</code> used to manage the selection to
     * new LSM.
     *
     * @param newLSM
    ...
    /**
     * Returns the preferred width for the passed in region. The region is
     * defined by the path closest to <code>(bounds.x, bounds.y)</code> and ends
     * at <code>bounds.height + bounds.y</code>. If <code>bounds</code> is
     * <code>null</code>, the preferred width for all the nodes will be returned
     * (and this may be a VERY expensive computation).
     *
     * @param bounds
    ...
    public int getPreferredWidth(Rectangle bounds) {
        ...
        if (rowCount > 0) {
            ...
            if (paths != null && paths.hasMoreElements()) {
                Rectangle pBounds = getBounds((TreePath) paths.nextElement(), null);
                int width;
                ...
                while (pBounds != null && paths.hasMoreElements()) {
                    pBounds = getBounds((TreePath) paths.nextElement(), pBounds);
     ...
     *
     * @param path
     *        a <code>TreePath</code> specifying a node
     * @param placeIn
     *        a <code>Rectangle</code> object giving the available space
     ...
     *
     * @param path
     *        the <code>TreePath</code> being queried
     * @return the row where the last item in path is visible or -1 if any
     *         elements in path aren't currently visible
     */
    public abstract int getRowForPath(TreePath path);

    /**
     * Returns the path to the node that is closest to x,y. If there is nothing
     * currently visible this will return <code>null</code>, otherwise it'll
     ...
     * based on how the paths are displayed. The first element of the returned
     * enumeration will be path, unless it isn't visible, in which case
     ...
     * <code>TreePaths</code> in <code>path</code> is not valid its entry in the
     * array should be set to -1.
     *
     * @param paths
    ...
    /**
     * Returns, by reference in <code>placeIn</code>, the size needed to
     * represent <code>value</code>. If <code>inPlace</code> is
     * <code>null</code>, a newly created <code>Rectangle</code> should be
     * returned, otherwise the value should be placed in <code>inPlace</code>
     * and returned. This will return <code>null</code> if there is no renderer.
     *
     * @param value
     *        the <code>value</code> to be represented
     * @param row
     *        row being queried
     * @param depth
     ...
     * @param placeIn
     *        a <code>Rectangle</code> containing the size needed to
     *        represent <code>value</code>
     * @return a <code>Rectangle</code> containing the node dimensions, or
     *         <code>null</code> if node has no dimension
     */
    protected Rectangle getNodeDimensions(Object value, int row, int depth, boolean expanded,
            Rectangle placeIn) {
    ...
    /**
     * Used by <code>AbstractLayoutCache</code> to determine the size and x
     * origin of a particular node.
     */
    static public abstract class NodeDimensions {
        /**
         * Returns, by reference in bounds, the size and x origin to place value
         * at. The calling method is responsible for determining the Y location.
         * If bounds is <code>null</code>, a newly created
         * <code>Rectangle</code> should be returned, otherwise the value should
         * be placed in bounds and returned.
         *
         * @param value
         *        the <code>value</code> to be represented
         * @param row
         ...
         *        true if row is expanded, false otherwise
         * @param bounds
         *        a <code>Rectangle</code> containing the size needed
         *        to
         *        represent <code>value</code>
         * @return a <code>Rectangle</code> containing the node dimensions, or
         *         <code>null</code> if node has no dimension
         */
        public abstract Rectangle getNodeDimensions(Object value, int row, int depth, boolean expanded,
                Rectangle bounds);
    }
}
