 ...
 * {@link UIDefaults#getUI(JComponent)} for details on how the implementation of
 ...
 * corresponding property if the value has not been changed by the developer.
 * <p>
...
public abstract class LookAndFeel {
	...
	/**
	 * Convenience method for setting a component's border property with a value
	 * from the defaults. The border is only set if the border is {@code null}
	...
	/**
	 * Convenience method for uninstalling a border. If the border of the
	 * component is a {@code UIResource}, it is set to {@code
	 * null}.
	 *
	 * @param c
	...
	/**
	 * Convenience method for installing a property with the specified name and
	 * value on a component if that property has not already been set by the
	 * developer. This method is intended to be used by ui delegate instances
	 * that need to specify a default value for a property of primitive type
	 * (boolean, int, ..), but do not wish to override a value set by the
	 * client. Since primitive property values cannot be wrapped with the
	 * {@code UIResource} marker, this method uses private state to determine
	 * whether the property has been set by the client.
	 *
	 * @throws IllegalArgumentException
	 *             if the specified property is not one which can be set using
	 *             this method
	 * @throws ClassCastException
	 *             if the property value has not been set by the developer and
	 *             the type does not match the property's type
	 * @throws NullPointerException
	 *             if {@code c} is {@code null}, or the named property has not
	 *             been set by the developer and {@code propertyValue} is
	 *             {@code null}
	 * @param c
	 ...
	 * KeyBindings}. While this method is not deprecated, developers should
	 * instead use {@code ActionMap} and {@code InputMap} for supplying key
	 * bindings.
	 * <p>
	 * This method returns an array of {@code KeyBindings}, one for each
	 * alternating {@code key-action} pair in {@code keyBindingList}. A
	 * {@code key} can either be a {@code String} in the format specified by the
	 * <code>KeyStroke.getKeyStroke</code> method, or a {@code KeyStroke}. The
	 * {@code action} part of the pair is a {@code String} that corresponds to
	 ...
	 * {@code action} part of the {@code key-action} pair results in creating a
	 * {@code KeyBinding} with the corresponding value {@code null}. As other
	 * parts of Swing's expect {@code non-null} values in a {@code KeyBinding},
	 * you should avoid supplying {@code null} as either the {@code key} or
	 ...
	 *             {@code KeyStroke} or {@code String}, or the {@code action}
	...
	public static JTextComponent.KeyBinding[] makeKeyBindings(Object[] keyBindingList) {
		...
		for (int i = 0; i < rv.length; i++) {
			...
			KeyStroke keystroke = (o instanceof KeyStroke) ? (KeyStroke) o
	...
	/**
	 * Creates a {@code InputMapUIResource} from <code>keys</code>. This is a
	 * convenience method for creating a new {@code InputMapUIResource},
	 ...
	 * This is a convenience method for creating a new
	...
	/**
	 * Populates an {@code InputMap} with the specified bindings. The bindings
	 * are supplied as a list of alternating {@code keystroke-action key} pairs.
	 * The {@code keystroke} is either an instance of {@code KeyStroke}, or a
	 * {@code String} that identifies the {@code KeyStroke} for the binding.
	 * Refer to {@code KeyStroke.getKeyStroke(String)} for the specific format.
	 * The {@code action key} part of the pair is the key registered in the
	 * {@code InputMap} for the {@code KeyStroke}.
	 * <p>
	 ...
	 * {@code retMap} in any way.
	 * <p>
	 * Specifying a {@code null} {@code action key} results in removing the
	 * {@code keystroke's} entry from the {@code InputMap}. A {@code null}
	 ...
	 * @throws NullPointerException
	 *             if {@code keys} is {@code non-null}, not empty, and
	...
	/**
	 * Creates and returns a {@code UIDefault.LazyValue} that loads an image.
	 * The returned value is an implementation of {@code
	 * UIDefaults.LazyValue}. When {@code createValue} is invoked on the
	 * returned object, the image is loaded. If the image is {@code
	 ...
	 * <p>
	 * This method does not check the arguments in any way. It is strongly
	 * recommended that {@code non-null} values are supplied else exceptions may
	...
	/**
	 * Returns the <code>LayoutStyle</code> for this look and feel. This never
	 * returns {@code null}.
	 * <p>
	 * You generally don't use the <code>LayoutStyle</code> from the look and
	 * feel, instead use the <code>LayoutStyle</code> method
	 ...
	 * <code>Toolkit.getDefaultToolkit().getDesktopProperty()</code>. If the
	 * value of the specified property is {@code null}, {@code fallbackValue} is
	 * returned.
	 *
	 * @param systemPropertyName
	 *            the name of the system desktop property being queried
	 * @param fallbackValue
	 *            the object to be returned as the value if the system value is
	 *            null
	 ...
	 * called to generate the disabled <code>Icon</code>. If {@code null} is
	 * passed as <code>icon</code> this method returns {@code null}.
	 * <p>
	 * Some look and feels might not render the disabled {@code Icon}, in which
	 * case they will ignore this.
	 *
	 * @param component
	 *            {@code JComponent} that will display the {@code Icon}, may be
	 *            {@code null}
	 * @param icon
	...
	public Icon getDisabledIcon(JComponent component, Icon icon) {
		if (icon instanceof ImageIcon) {
			return new ImageIconUIResource(
	 ...
	 * and selected <code>Icon</code>. If {@code null} is passed as
	 * <code>icon</code> this methods returns {@code null}.
	 * <p>
	 ...
	 * "CDE/Motif". This string should be appropriate for a menu item. Distinct
	 * look and feels should have different names, e.g. a subclass of
	 * MotifLookAndFeel that changes the way a few components are rendered
	 * should be called "CDE/Motif My Way"; something that would be useful to a
	 * user trying to select a L&amp;F from a list of names.
	 *
	 ...
	 * used by applications/services that want to recognize well known look and
	 * feel implementations. Presently the well known names are "Motif",
	 * "Windows", "Mac", "Metal". Note that a LookAndFeel derived from a well
	 * known superclass that doesn't make any fundamental changes to the look or
	 ...
	 * "The CDE/Motif Look and Feel". This string is intended for the user, e.g.
	 * in the title of a window or in a ToolTip message.
	 *
	 ...
	 * <p>
	 * The default implementation returns {@code false}, subclasses that support
	 * {@code Window} decorations should override this and return {@code true}.
	 *
	 * @return {@code true} if the {@code RootPaneUI} instances created by this
	 *         look and feel support client side decorations
	 ...
	 * implementation of it, return {@code true}. For example, when the
	 * underlying platform is Solaris running CDE a CDE/Motif look and feel
	 * implementation would return {@code
	 * true}.
	 *
	...
	/**
	 * Initializes the look and feel. While this method is public, it should
	 * only be invoked by the {@code UIManager} when a look and feel is
	 * installed as the current look and feel. This method is invoked before the
	 * {@code UIManager} invokes {@code getDefaults}. This method is intended to
	 * perform any initialization for the look and feel. Subclasses should do
	 * any one-time setup they need here, rather than in a static initializer,
	 * because look and feel class objects may be loaded just to discover that
	 * {@code isSupportedLookAndFeel()} returns {@code false}.
	 *
	 * @see #uninitialize
	 * @see UIManager#setLookAndFeel
	 */
	...
	/**
	 * Uninitializes the look and feel. While this method is public, it should
	 * only be invoked by the {@code UIManager} when the look and feel is
	 * uninstalled. For example, {@code UIManager.setLookAndFeel} invokes this
	...
	/**
	 * Returns the look and feel defaults. While this method is public, it
	 * should only be invoked by the {@code UIManager} when the look and feel is
	 * set as the current look and feel and after {@code initialize} has been
	 * invoked.
	 *
