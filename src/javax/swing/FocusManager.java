...
public abstract class FocusManager extends DefaultKeyboardFocusManager {
	 ...
	 * specification is incompatible with the 1.4 focus APIs. The current
	 * FocusManager is no longer a property of the UI. Client code must query
	 * for the current FocusManager using
	 * <code>KeyboardFocusManager.getCurrentKeyboardFocusManager()</code>. See
	...
	/**
	 * Returns the current <code>KeyboardFocusManager</code> instance for the
	...
	/**
	 * Sets the current <code>KeyboardFocusManager</code> instance for the
	 * calling thread's context. If <code>null</code> is specified, then the
	 * current <code>KeyboardFocusManager</code> is replaced with a new instance
	 * of <code>DefaultKeyboardFocusManager</code>.
	 * <p>
	 * If a <code>SecurityManager</code> is installed, the calling thread must
	 * be granted the <code>AWTPermission</code> "replaceKeyboardFocusManager"
	 * in order to replace the the current <code>KeyboardFocusManager</code>. If
	 * this permission is not granted, this method will throw a
	 * <code>SecurityException</code>, and the current
