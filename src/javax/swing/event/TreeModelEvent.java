 ...
 * How to Write a Tree Model Listener</a>, a section in
...
public class TreeModelEvent extends EventObject {
	 ...
	 * specified by an array of <code>int</code>. The indexes in that array must
	 ...
	 * through the list from lowest to highest. Every time you do a delete, add
	 ...
	 * using <code>addElement()</code>. Then increment the delete-counter. The
	 * index positions stored in the Vector therefore reflect the effects of all
	 * previous deletes, so they represent each object's position in the initial
	 * tree. (You could also start at the highest index and working back towards
	 ...
	 * <code>Integer</code> objects to an array of <code>int</code> to create
	 * the event.
	 * <p>
	 * <b>Notes:</b>
	 * <ul style="list-style-type:none">
	 * <li>Like the <code>insertNodeInto</code> method in the
	 * <code>DefaultTreeModel</code> class, <code>insertElementAt</code> appends
	 * to the <code>Vector</code> when the index matches the size of the vector.
	 * So you can use <code>insertElementAt(Integer, 0)</code> even when the
	 * vector is empty.</li>
	 ...
	 * @param childIndices
	 *            an array of <code>int</code> that specifies the index values
	 *            of the removed items. The indices must be in sorted order,
	 ...
	 * TreePath object. For more information on how to specify the indexes and
	 ...
	 * different levels of the subtree. <blockquote> <b>Note:</b><br>
	 ...
	 *            modified subtree, where the first element of the array is the
	 *            object stored at the root node and the last element is the
	 ...
	 * the structure that has changed. This and <code>getChildIndices</code> are
	 * used to get a list of the effected nodes.
	 * <p>
	 * The one exception to this is a treeNodesChanged event that is to identify
	 * the root, in which case this will return the root and
	 * <code>getChildIndices</code> will return null.
	 *
	...
	/**
	 * Convenience method to get the array of objects from the TreePath instance
	 * that this event wraps.
	 *
	...
	/**
	 * Returns the values of the child indexes. If this is a removal event the
	 * indexes point to locations in the initial list where items were removed.
	 * If it is an insert, the indices point to locations in the final list
	 * where the items were added. For node changes, the indices point to the
	 * locations of the modified nodes.
	 *
