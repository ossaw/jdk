...
public class GroupLayout implements LayoutManager2 {
	 ...
	 *            the component
	 * @param honorsVisibility
	 *            whether visibility of this {@code component} should be
	 ...
	 *            whether a gap between the container and components that touch
	 ...
	 * @throws IllegalArgumentException
	 *             if {@code components} is {@code null}, or contains
	 *             {@code null}
	 ...
	 *            {@code newComponent}
	 * @param newComponent
	 *            the component to put in {@code existingComponent}'s place
	 * @throws IllegalArgumentException
	 *             if either of the components are {@code null} or
	 *             {@code existingComponent} is not being managed by this layout
	 *             manager
	 */
	 ...
	 * @throws IllegalArgumentException
	 *             if {@code parent} is not the same {@code Container} this was
	 *             created with
	 ...
	 * @throws IllegalArgumentException
	 *             if {@code parent} is not the same {@code Container} that this
	 *             was created with
	 * @throws IllegalStateException
	 *             if any of the components added to this layout are not in both
	 *             a horizontal and vertical group
		...
		for (ComponentInfo info : componentInfos.values()) {
			if (info.horizontalSpring == null) {
				throw new IllegalStateException(
						info.component + " is not attached to a horizontal group");
			}
			if (info.verticalSpring == null) {
				throw new IllegalStateException(
						info.component + " is not attached to a vertical group");
			}
		}
	}
	...
	private void createSpringDescription(StringBuffer buffer, Spring spring, String indent,
			int axis) {
		String origin = "";
		String padding = "";
		if (spring instanceof ComponentSpring) {
		...
		if (spring instanceof AutoPreferredGapSpring) {
			...
			padding = ", userCreated=" + paddingSpring.getUserCreated() + ", matches="
					+ paddingSpring.getMatchDescription();
		}
		buffer.append(indent + spring.getClass().getName() + " "
				+ Integer.toHexString(spring.hashCode()) + " " + origin + ", size="
				+ spring.getSize() + ", alignment=" + spring.getAlignment() + " prefs=["
				+ spring.getMinimumSize(axis) + " " + spring.getPreferredSize(axis) + " "
	...
	private abstract class Spring {
		 ...
		 *            if {@code true}, auto padding springs should be treated as
	...
	public abstract class Group extends Spring {
		 ...
		 * @return this {@code Group}
		 * @throws IllegalArgumentException
		 *             if {@code size} is less than {@code 0}
		 */
		 ...
		 * @throws IllegalArgumentException
		 *             if any of the values are less than {@code 0}
		 * @return this {@code Group}
		 */
	...
	public class SequentialGroup extends Group {
		 ...
		 *            the {@code Group} to add
		 * @param useAsBaseline
		 *            whether the specified {@code Group} should be used to
		 ...
		 *
		 * @param useAsBaseline
		 *            whether the specified {@code Component} should be used to
		 *            calculate the baseline for this {@code Group}
		 * @param component
		 *            the {@code Component} to add
		 * @return this {@code Group}
		 ...
		 * @throws IllegalArgumentException
		 *             if {@code type}, {@code comp1} or {@code comp2} is
		 *             {@code null}
		 * @see LayoutStyle
		 */
		...
		public SequentialGroup addPreferredGap(JComponent comp1, JComponent comp2,
			...
			return (SequentialGroup) addSpring(
					new PreferredGapSpring(comp1, comp2, type, pref, max));
		}
		 ...
		 *
		 * @param pref
		 *            the preferred size; one of {@code DEFAULT_SIZE} or a value
		 *            &gt;= 0
		 * @param max
		...
		void setValidSize(int axis, int origin, int size) {
			...
			if (size == pref) {
				...
				spring.setSize(axis, origin, Math.min(Math.max(size, spring.getMinimumSize(axis)),
		...
		void insertAutopadding(int axis, List<AutoPreferredGapSpring> leadingPadding,
			...
			List<AutoPreferredGapSpring> newLeadingPadding = new ArrayList<AutoPreferredGapSpring>(
					leadingPadding);
			List<AutoPreferredGapSpring> newTrailingPadding = new ArrayList<AutoPreferredGapSpring>(
					1);
				...
				if (spring instanceof AutoPreferredGapSpring) {
					...
					if (spring instanceof ComponentSpring) {
						...
						((Group) spring).insertAutopadding(axis, newLeadingPadding,
								newTrailingPadding, newLeading, newTrailing, insert);
						newLeading.clear();
						newLeadingPadding.clear();
	...
	public class ParallelGroup extends Group {
		...
		void setChildSize(Spring spring, int axis, int origin, int size) {
			...
			int springSize = Math.min(Math.max(spring.getMinimumSize(axis), size),
					spring.getMaximumSize(axis));
			if (alignment == null) {
		...
		void insertAutopadding(int axis, List<AutoPreferredGapSpring> leadingPadding,
			...
			for (Spring spring : springs) {
				if (spring instanceof ComponentSpring) {
					...
					((Group) spring).insertAutopadding(axis, leadingPadding, trailingPadding,
							leading, trailing, insert);
		...
		private void checkChildAlignment(Alignment alignment, boolean allowsBaseline) {
			...
			if (!allowsBaseline && alignment == Alignment.BASELINE) {
				throw new IllegalArgumentException(
						"Alignment must be one of:" + "LEADING, TRAILING or CENTER");
			}
		}
	}
			...
			for (Spring spring : springs) {
				if (spring.getAlignment() == null || spring.getAlignment() == Alignment.BASELINE) {
					...
					if (baseline >= 0) {
						...
						prefDescent = Math.max(prefDescent,
								spring.getPreferredSize(VERTICAL) - baseline);
						baselineSpringCount++;
					}
				}
			}
				...
				if (alignment == null || alignment == Alignment.BASELINE) {
					...
					if (baseline >= 0) {
						...
						case CONSTANT_ASCENT:
							...
							break;
						case CONSTANT_DESCENT:
							height = Math.min(ascent, springMax - springPref + baseline)
									+ (springPref - baseline);
	...
	private class PreferredGapSpring extends Spring {
		...
		PreferredGapSpring(JComponent source, JComponent target, ComponentPlacement type, int pref,
				int max) {
			this.source = source;
			this.target = target;
			this.type = type;
			...
			if (matches != null) {
				...
				for (int i = matches.size() - 1; i >= 0; i--) {
					...
					maxPadding = Math.max(maxPadding,
							calculatePadding(p, position, match.source, match.target));
				}
			}
			if (size == UNSET) {
				size = 0;
		...
		private int calculatePadding(LayoutStyle p, int position, ComponentSpring source,
			...
			if (delta >= 0) {
				...
				if ((source.getComponent() instanceof JComponent)
					...
					padding = p.getPreferredGap((JComponent) source.getComponent(),
							(JComponent) target.getComponent(), type, position, host);
				} else {
					padding = 10;
				}
				if (padding > delta) {
	...
	private class ContainerAutoPreferredGapSpring extends AutoPreferredGapSpring {
			...
			if (targets != null) {
				...
				for (int i = targets.size() - 1; i >= 0; i--) {
					...
					if (targetSpring.getComponent() instanceof JComponent) {
						padding = p.getContainerGap((JComponent) targetSpring.getComponent(),
								position, host);
		...
		private int updateSize(LayoutStyle p, ComponentSpring sourceSpring, int position) {
			...
			if (sourceSpring.getComponent() instanceof JComponent) {
				padding = p.getContainerGap((JComponent) sourceSpring.getComponent(), position,
						host);
			}
			int delta = Math.max(0,
					getParent().getSize() - sourceSpring.getSize() - sourceSpring.getOrigin());
			size = Math.max(size, padding - delta);
			return padding;
		}
