...
/**
 * {@code GroupLayout} is a {@code LayoutManager} that hierarchically groups
 * components in order to position them in a {@code Container}.
 * {@code GroupLayout} is intended for use by builders, but may be hand-coded as
 * well. Grouping is done by instances of the {@link Group Group} class. {@code
 * GroupLayout} supports two types of groups. A sequential group positions its
 * child elements sequentially, one after another. A parallel group aligns its
 * child elements in one of four ways.
 * <p>
 * Each group may contain any number of elements, where an element is a
 * {@code Group}, {@code Component}, or gap. A gap can be thought of as an
 * invisible component with a minimum, preferred and maximum size. In addition
 * {@code GroupLayout} supports a preferred gap, whose value comes from
 * {@code LayoutStyle}.
 * <p>
 ...
 * or a range determined by {@code
 * LayoutStyle}. The range for {@code Component}s is determined from the
 * {@code Component}'s {@code getMinimumSize}, {@code
 * getPreferredSize} and {@code getMaximumSize} methods. In addition, when
 * adding {@code Component}s you may specify a particular range to use instead
 * of that from the component. The range for a {@code
 * Group} is determined by the type of group. A {@code ParallelGroup}'s range is
 * the maximum of the ranges of its elements. A {@code
 * SequentialGroup}'s range is the sum of the ranges of its elements.
 * <p>
 * {@code GroupLayout} treats each axis independently. That is, there is a group
 * representing the horizontal axis, and a group representing the vertical axis.
 * The horizontal group is responsible for determining the minimum, preferred
 * and maximum size along the horizontal axis as well as setting the x and width
 * of the components contained in it. The vertical group is responsible for
 * determining the minimum, preferred and maximum size along the vertical axis
 * as well as setting the y and height of the components contained in it. Each
 * {@code Component} must exist in both a horizontal and vertical group,
 * otherwise an {@code IllegalStateException} is thrown during layout, or when
 * the minimum, preferred or maximum size is requested.
 * <p>
 * The following diagram shows a sequential group along the horizontal axis. The
 * sequential group contains three components. A parallel group was used along
 * the vertical axis.
 * <p style="text-align:center">
 * <img src="doc-files/groupLayout.1.gif" alt=
 ...
 * (horizontal) and red (vertical). For readability there is a gap between each
 * of the elements in the sequential group.
 * <p>
 ...
 * contains.
 * <p>
 * Along the vertical axis the parallel group is the maximum of the height of
 * each of the components. As all three components have the same height, the
 * parallel group has the same height.
 * <p>
 ...
 * axis.
 * <p style="text-align:center">
 * <img src="doc-files/groupLayout.2.gif" alt=
 * "Sequential group along the vertical axis in three components">
 * <p>
 * As {@code c1} is the largest of the three components, the parallel group is
 * sized to {@code c1}. As {@code c2} and {@code c3} are smaller than {@code c1}
 * they are aligned based on the alignment specified for the component (if
 * specified) or the default alignment of the parallel group. In the diagram
 * {@code c2} and {@code c3} were created with an alignment of {@code LEADING}.
 * If the component orientation were right-to-left then {@code c2} and
 * {@code c3} would be positioned on the opposite side.
 * <p>
 * The following diagram shows a sequential group along both the horizontal and
 * vertical axis.
 * <p style="text-align:center">
 * <img src="doc-files/groupLayout.3.gif" alt=
 * "Sequential group along both the horizontal and vertical axis in three
 * components"
 * >
 * <p>
 * {@code GroupLayout} provides the ability to insert gaps between
 * {@code Component}s. The size of the gap is determined by an instance of
 * {@code LayoutStyle}. This may be turned on using the
 * {@code setAutoCreateGaps} method. Similarly, you may use the
 * {@code setAutoCreateContainerGaps} method to insert gaps between components
 * that touch the edge of the parent container and the container.
 * <p>
 ...
 * <p style="text-align:center">
 * <img src="doc-files/groupLayout.example.png" alt=
 * "Produced horizontal/vertical form">
 * <p>
 * This layout consists of the following.
 * <ul>
 * <li>The horizontal axis consists of a sequential group containing two
 * parallel groups. The first parallel group contains the labels, and the second
 * parallel group contains the text fields.
 * <li>The vertical axis consists of a sequential group containing two parallel
 * groups. The parallel groups are configured to align their components along
 * the baseline. The first parallel group contains the first label and first
 * text field, and the second group consists of the second label and second text
 * field.
 * </ul>
 * There are a couple of things to notice in this code:
 * <ul>
 * <li>You need not explicitly add the components to the container; this is
 * indirectly done by using one of the {@code add} methods of {@code Group}.
 * <li>The various {@code add} methods return the caller. This allows for easy
 * chaining of invocations. For example,
 * {@code group.addComponent(label1).addComponent(label2);} is equivalent to
 * {@code group.addComponent(label1); group.addComponent(label2);}.
 * <li>There are no public constructors for {@code Group}s; instead use the
 * create methods of {@code GroupLayout}.
 * </ul>
 *
 * @author Tomas Pavek
 * @author Jan Stola
...
public class GroupLayout implements LayoutManager2 {
    ...
    /**
     * Indicates the preferred size from the component or gap should be used for
     * a particular range value.
     *
     * @see Group
     */
    ...
    /**
     * Enumeration of the possible ways {@code ParallelGroup} can align its
     * children.
     *
    ...
    public enum Alignment {
         ...
         * horizontal axis with a left to right orientation this means aligned
         * to the left edge. For the vertical axis leading means aligned to the
         * top edge.
         *
         * @see #createParallelGroup(Alignment)
         */
         ...
         * For the horizontal axis with a left to right orientation this means
         * aligned to the right edge. For the vertical axis trailing means
     ...
     *
     * @param host
     *        the {@code Container} the {@code GroupLayout} is the
     *        {@code LayoutManager} for
     * @throws IllegalArgumentException
     *         if host is {@code null}
     */
     ...
     * value of {@code false} indicates that components should be positioned and
     * sized regardless of visibility.
     * <p>
     * A value of {@code false} is useful when the visibility of components is
     * dynamically adjusted and you don't want surrounding components and the
     * sizing to change.
     * <p>
     ...
     * positioning. A value of {@code Boolean.TRUE} indicates that if
     * {@code component} is not visible it should not be treated as part of the
     * layout. A value of {@code false} indicates that {@code component} is
     * positioned and sized regardless of it's visibility. A value of
     * {@code null} indicates the value specified by the single argument method
     * {@code
     ...
     *        the component
     * @param honorsVisibility
     *        whether visibility of this {@code component}
     *        should be
     ...
     * For example, if this is {@code true} and you add two components to a
     * {@code SequentialGroup} a gap between the two components is automatically
     * be created. The default is {@code false}.
     *
    ...
    /**
     * Sets whether a gap between the container and components that touch the
     * border of the container should automatically be created. The default is
     * {@code false}.
     *
     * @param autoCreateContainerPadding
     *        whether a gap between the container and
     *        components that touch
     *        the border of the container should
    ...
    /**
     * Sets the {@code Group} that positions and sizes components along the
     * horizontal axis.
     *
     * @param group
     *        the {@code Group} that positions and sizes components along
     *        the horizontal axis
     * @throws IllegalArgumentException
     *         if group is {@code null}
     */
    ...
    /**
     * Returns the {@code Group} that positions and sizes components along the
     * horizontal axis.
     *
     * @return the {@code Group} responsible for positioning and sizing
     ...
     * should be generated the necessary springs are added.
     */
     ...
     * {@code Alignment.LEADING}. This is a cover method for the more general
     * {@code createParallelGroup(Alignment)} method.
     *
     ...
     * This is a cover method for the more general {@code
     * createParallelGroup(Alignment,boolean)} method with {@code true} supplied
     * for the second argument.
     *
     * @param alignment
     *        the alignment for the elements of the group
     * @throws IllegalArgumentException
     *         if {@code alignment} is {@code null}
     * @return a new {@code ParallelGroup}
    ...
    /**
     * Creates and returns a {@code ParallelGroup} with the specified alignment
     * and resize behavior. The {@code
     * alignment} argument specifies how children elements are positioned that
     * do not fill the group. For example, if a {@code
     * ParallelGroup} with an alignment of {@code TRAILING} is given 100 and a
     * child only needs 50, the child is positioned at the position 50 (with a
     * component orientation of left-to-right).
     * <p>
     * Baseline alignment is only useful when used along the vertical axis. A
     * {@code ParallelGroup} created with a baseline alignment along the
     * horizontal axis is treated as {@code LEADING}.
     * <p>
     ...
     *        the alignment for the elements of the group
     * @param resizable
     *        {@code true} if the group is resizable; if the group is
     *        not
     *        resizable the preferred size is used for the minimum and
    ...
    /**
     * Creates and returns a {@code ParallelGroup} that aligns it's elements
     * along the baseline.
     *
     * @param resizable
     *        whether the group is resizable
     * @param anchorBaselineToTop
     *        whether the baseline is anchored to the top or
     *        bottom of the
     *        group
     * @see #createBaselineGroup
     * @see ParallelGroup
     ...
     * preferred, minimum or maximum sizes. Components that are linked are given
     * the maximum of the preferred size of each of the linked components. For
     * example, if you link two components with a preferred width of 10 and 20,
     * both components are given a width of 20.
     * <p>
     * This can be used multiple times to force any number of components to
     * share the same size.
     * <p>
     * Linked Components are not be resizable.
     *
     * @param components
     *        the {@code Component}s that are to have the same size
     * @throws IllegalArgumentException
    ...
    /**
     * Forces the specified components to have the same size along the specified
     * axis regardless of their preferred, minimum or maximum sizes. Components
     * that are linked are given the maximum of the preferred size of each of
     * the linked components. For example, if you link two components along the
     * horizontal axis and the preferred width is 10 and 20, both components are
     * given a width of 20.
     * <p>
     * This can be used multiple times to force any number of components to
     * share the same size.
     ...
     *         {@code null} or
     *         {@code existingComponent} is not being
    ...
    /**
     * Sets the {@code LayoutStyle} used to calculate the preferred gaps between
     * components. A value of {@code null} indicates the shared instance of
     * {@code LayoutStyle} should be used.
     *
     * @param layoutStyle
     *        the {@code LayoutStyle} to use
     * @see LayoutStyle
     */
    ...
    /**
     * Returns the {@code LayoutStyle} used for calculating the preferred gap
     * between components. This returns the value specified to
     * {@code setLayoutStyle}, which may be {@code null}.
     *
     * @return the {@code LayoutStyle} used for calculating the preferred gap
     *         between components
     */
    ...
    //
    /**
     * Notification that a {@code Component} has been added to the parent
     * container. You should not invoke this method directly, instead you should
     * use one of the {@code Group} methods to add a {@code Component}.
     *
     * @param name
     *        the string to be associated with the component
     * @param component
     *        the {@code Component} to be added
     */
    public void addLayoutComponent(String name, Component component) {}

    /**
     * Notification that a {@code Component} has been removed from the parent
     * container. You should not invoke this method directly, instead invoke
     * {@code remove} on the parent {@code Container}.
     *
     * @param component
     ...
     * @throws IllegalArgumentException
     *         if {@code parent} is not the same
     *         {@code Container} this was
     *         created with
     * @throws IllegalStateException
     *         if any of the components added to this
     *         layout are not in both
    ...
    public Dimension preferredLayoutSize(Container parent) {
        checkParent(parent);
        prepare(PREF_SIZE);
        return adjustSize(horizontalGroup.getPreferredSize(HORIZONTAL), verticalGroup.getPreferredSize(
                VERTICAL));
    }
     ...
     *
     * @param parent
     *        the {@code Container} hosting this {@code LayoutManager}
     * @throws IllegalArgumentException
     *         if {@code parent} is not the same
     *         {@code Container} that this
     *         was created with
     ...
     *
     * @param parent
     *        the {@code Container} hosting this LayoutManager
     * @throws IllegalArgumentException
     *         if {@code parent} is not the same
     *         {@code Container} that this
     *         was created with
    ...
    private void prepare(int sizeType) {
        ...
        if (sizeType != SPECIFIC_SIZE && (getAutoCreateGaps() || getAutoCreateContainerGaps()
    ...
    /**
     * Returns the {@code ComponentInfo} for the specified Component, creating
     * one if necessary.
     */
     ...
     * If {@code insert} is {@code true} this will insert auto padding springs,
    ...
    /**
     * Returns a string representation of this {@code GroupLayout}. This method
     * is intended to be used for debugging purposes, and the content and format
    ...
    private void createSpringDescription(StringBuffer buffer, Spring spring, String indent, int axis) {
        String origin = "";
        String padding = "";
        if (spring instanceof ComponentSpring) {
        ...
        if (spring instanceof AutoPreferredGapSpring) {
            ...
            padding = ", userCreated=" + paddingSpring.getUserCreated() + ", matches=" + paddingSpring
                    .getMatchDescription();
        }
        buffer.append(indent + spring.getClass().getName() + " " + Integer.toHexString(spring.hashCode())
                + " " + origin + ", size=" + spring.getSize() + ", alignment=" + spring.getAlignment()
                + " prefs=[" + spring.getMinimumSize(axis) + " " + spring.getPreferredSize(axis) + " "
        ...
        if (spring instanceof Group) {
            ...
            for (int counter = 0; counter < springs.size(); counter++) {
                createSpringDescription(buffer, springs.get(counter), indent, axis);
            }
        }
    }
     ...
     * middle of that, and a location. Spring caches the min/max/pref. If the
     * min/pref/max has internally changes, or needs to be updated you must
     * invoke clear.
     */
    private abstract class Spring {
        ...
        /**
         * Returns {@code true} if this spring will ALWAYS have a zero size.
         * This should NOT check the current size, rather it's meant to quickly
         * test if this Spring will always have a zero size.
         *
         * @param treatAutopaddingAsZeroSized
         *        if {@code true}, auto padding
         *        springs should be treated as
    ...
    /**
     * {@code Group} provides the basis for the two types of operations
     * supported by {@code GroupLayout}: laying out components one after another
     * ({@link SequentialGroup SequentialGroup}) or aligned (
     * {@link ParallelGroup ParallelGroup}). {@code Group} and its subclasses
     * have no public constructor; to create one use one of
     * {@code createSequentialGroup} or {@code createParallelGroup}.
     * Additionally, taking a {@code Group} created from one {@code GroupLayout}
     * and using it with another will produce undefined results.
     * <p>
     * Various methods in {@code Group} and its subclasses allow you to
     * explicitly specify the range. The arguments to these methods can take two
     * forms, either a value greater than or equal to 0, or one of
     * {@code DEFAULT_SIZE} or {@code PREFERRED_SIZE}. A value greater than or
     * equal to {@code 0} indicates a specific size. {@code DEFAULT_SIZE}
     * indicates the corresponding size from the component should be used. For
     * example, if {@code
     * DEFAULT_SIZE} is passed as the minimum size argument, the minimum size is
     * obtained from invoking {@code getMinimumSize} on the component. Likewise,
     * {@code PREFERRED_SIZE} indicates the value from {@code getPreferredSize}
     * should be used. The following example adds {@code myComponent} to
     * {@code group} with specific values for the range. That is, the minimum is
     * explicitly specified as 100, preferred as 200, and maximum as 300.
     * 
     * <pre>
     * group.addComponent(myComponent, 100, 200, 300);
     * </pre>
     * 
     * The following example adds {@code myComponent} to {@code group} using a
     * combination of the forms. The minimum size is forced to be the same as
     * the preferred size, the preferred size is determined by using
     * {@code myComponent.getPreferredSize} and the maximum is determined by
     * invoking {@code getMaximumSize} on the component.
     * 
     * <pre>
     * group.addComponent(myComponent, GroupLayout.PREFERRED_SIZE, GroupLayout.PREFERRED_SIZE,
     *         GroupLayout.DEFAULT_SIZE);
     * </pre>
     * <p>
     * Unless otherwise specified all the methods of {@code Group} and its
     * subclasses that allow you to specify a range throw an
     * {@code IllegalArgumentException} if passed an invalid range. An invalid
     * range is one in which any of the values are &lt; 0 and not one of
     * {@code PREFERRED_SIZE} or {@code DEFAULT_SIZE}, or the following is not
     * met (for specific values): {@code min} &lt;= {@code pref} &lt;=
     * {@code max}.
     * <p>
     ...
     * that take a {@code Group} throw an {@code NullPointerException} if passed
     * {@code null}.
     *
    ...
    public abstract class Group extends Spring {
        ...
        /**
         * Adds a {@code Component} to this {@code Group} with the specified
         * size.
         *
         * @param component
         *        the {@code Component} to add
         ...
         * @throws IllegalArgumentException
         *         if any of the values are less than
         *         {@code 0}
         * @return this {@code Group}
         */
        ...
        /**
         * Adds the Spring to the list of {@code Spring}s and returns the
         * receiver.
         */
        Group addSpring(Spring spring) {
            springs.add(spring);
            spring.setParent(this);
            if (!(spring instanceof AutoPreferredGapSpring) || !((AutoPreferredGapSpring) spring)
                    .getUserCreated()) {
                springsChanged = true;
            }
            return this;
        ...
        /**
         * This is invoked from {@code setSize} if passed a value other than
         * UNSET.
         */
         ...
         * {@code getMinimumSize0}, {@code getPreferredSize0} or
         * {@code getMaximumSize0} methods. This will invoke to {@code operator}
        ...
        int calculateSize(int axis, int type) {
            ...
            int size = constrain(operator(getSpringSize(getSpring(0), axis, type), getSpringSize(getSpring(1),
                    axis, type)));
         ...
         * {@code insert} is true this will insert auto padding springs,
         * otherwise this will only adjust the springs that comprise auto
         * preferred padding springs.
         *
         * @param axis
         *        the axis of the springs; HORIZONTAL or
         *        VERTICAL
         * @param leadingPadding
         *        List of AutopaddingSprings that occur before
         *        this Group
         * @param trailingPadding
         *        any trailing autopadding springs are added to
         *        this on exit
         * @param leading
         *        List of ComponentSprings that occur before
         *        this Group
         * @param trailing
         *        any trailing ComponentSpring are added to this
         *        List
         * @param insert
         *        Whether or not to insert AutopaddingSprings or
         *        just adjust
    ...
    /**
     * A {@code Group} that positions and sizes its elements sequentially, one
     * after another. This class has no public constructor, use the
     * {@code createSequentialGroup} method to create one.
     * <p>
     * In order to align a {@code SequentialGroup} along the baseline of a
     * baseline aligned {@code ParallelGroup} you need to specify which of the
     * elements of the {@code SequentialGroup} is used to determine the
     * baseline. The element used to calculate the baseline is specified using
     * one of the {@code add} methods that take a {@code boolean}. The last
     * element added with a value of {@code true} for {@code useAsBaseline} is
    ...
    public class SequentialGroup extends Group {
         ...
         *        the {@code Group} to add
         * @param useAsBaseline
         *        whether the specified {@code Group} should be
         *        used to
         *        calculate the baseline for this {@code Group}
         * @return this {@code Group}
         */
         ...
         *        the {@code Component} to add
         * @param min
         *        the minimum size or one of {@code DEFAULT_SIZE}
         *        or
         *        {@code PREFERRED_SIZE}
         * @param pref
         *        the preferred size or one of
         *        {@code DEFAULT_SIZE} or
         *        {@code PREFERRED_SIZE}
         * @param max
         *        the maximum size or one of {@code DEFAULT_SIZE}
         *        or
         *        {@code PREFERRED_SIZE}
         * @return this {@code Group}
         */
        ...
        public SequentialGroup addPreferredGap(JComponent comp1, JComponent comp2, ComponentPlacement type,
            ...
            return (SequentialGroup) addSpring(new PreferredGapSpring(comp1, comp2, type, pref, max));
        }

        /**
         * Adds an element representing the preferred gap between the nearest
         * components. During layout, neighboring components are found, and the
         * size of the added gap is set based on the preferred gap between the
         * components. If no neighboring components are found the gap has a size
         * of {@code 0}.
         * <p>
         ...
         * @see LayoutStyle
         * @throws IllegalArgumentException
         *         if {@code type} is not one of
         *         {@code LayoutStyle.ComponentPlacement.RELATED}
         *         or
         *         {@code LayoutStyle.ComponentPlacement.UNRELATED}
         */
         ...
         * Adds an element representing the preferred gap between the nearest
         * components. During layout, neighboring components are found, and the
         * minimum of this gap is set based on the size of the preferred gap
         * between the neighboring components. If no neighboring components are
         * found the minimum size is set to 0.
         *
         * @param type
         *        the type of gap; one of
         *        {@code LayoutStyle.ComponentPlacement.RELATED} or
         ...
         *        {@code DEFAULT_SIZE} or a value &gt;= 0
         * @param max
         *        the maximum size of the gap; one of {@code DEFAULT_SIZE},
         *        {@code PREFERRED_SIZE} or a value &gt;= 0
         * @return this {@code SequentialGroup}
         * @throws IllegalArgumentException
         *         if {@code type} is not one of
        ...
        public SequentialGroup addPreferredGap(ComponentPlacement type, int pref, int max) {
            if (type != ComponentPlacement.RELATED && type != ComponentPlacement.UNRELATED) {
            ...
            return (SequentialGroup) addSpring(new AutoPreferredGapSpring(type, pref, max));
        }

        /**
         * Adds an element representing the preferred gap between an edge the
         * container and components that touch the border of the container. This
         * has no effect if the added gap does not touch an edge of the parent
         * container.
         * <p>
         * The element created to represent the gap is not resizable.
         *
         ...
         * the container and the next or previous {@code
         * Component} with the specified size. This has no effect if the next or
         * previous element is not a {@code
        ...
        public SequentialGroup addContainerGap(int pref, int max) {
            if ((pref < 0 && pref != DEFAULT_SIZE) || (max < 0 && max != DEFAULT_SIZE
                    && max != PREFERRED_SIZE) || (pref >= 0 && max >= 0 && pref > max)) {
                throw new IllegalArgumentException("Pref and max must be either DEFAULT_VALUE "
                        + "or >= 0 and pref <= max");
            }
            hasPreferredPaddingSprings = true;
            return (SequentialGroup) addSpring(new ContainerAutoPreferredGapSpring(pref, max));
        }
        ...
        void setValidSize(int axis, int origin, int size) {
            ...
            if (size == pref) {
                ...
                spring.setSize(axis, origin, Math.min(Math.max(size, spring.getMinimumSize(axis)), spring
                        .getMaximumSize(axis)));
            } else if (springs.size() > 1) {
        ...
        void insertAutopadding(int axis, List<AutoPreferredGapSpring> leadingPadding,
                List<AutoPreferredGapSpring> trailingPadding, List<ComponentSpring> leading,
                List<ComponentSpring> trailing, boolean insert) {
            List<AutoPreferredGapSpring> newLeadingPadding = new ArrayList<AutoPreferredGapSpring>(
                    leadingPadding);
            ...
            while (counter < springs.size()) {
                ...
                if (spring instanceof AutoPreferredGapSpring) {
                    ...
                    if (spring instanceof ComponentSpring) {
                        ...
                        ((Group) spring).insertAutopadding(axis, newLeadingPadding, newTrailingPadding,
                                newLeading, newTrailing, insert);
                        newLeading.clear();
                        newLeadingPadding.clear();
                        counter = indexOfNextNonZeroSpring(counter + 1, (newTrailing.size() == 0));
                        if (counter == springs.size()) {
        ...
        private void checkPreferredGapValues(int pref, int max) {
            if ((pref < 0 && pref != DEFAULT_SIZE && pref != PREFERRED_SIZE) || (max < 0
                    && max != DEFAULT_SIZE && max != PREFERRED_SIZE) || (pref >= 0 && max >= 0
                            && pref > max)) {
     ...
     * {@code ParallelGroup} aligns it's children in four possible ways: along
     * the baseline, centered, anchored to the leading edge, or anchored to the
     * trailing edge.
     * <h3>Baseline</h3> A {@code ParallelGroup} that aligns it's children along
     * the baseline must first decide where the baseline is anchored. The
     * baseline can either be anchored to the top, or anchored to the bottom of
     * the group. That is, the distance between the baseline and the beginning
     * of the group can be a constant distance, or the distance between the end
     * of the group and the baseline can be a constant distance. The possible
     * choices correspond to the {@code BaselineResizeBehavior} constants
     * {@link java.awt.Component.BaselineResizeBehavior#CONSTANT_ASCENT
     * CONSTANT_ASCENT} and
     * {@link java.awt.Component.BaselineResizeBehavior#CONSTANT_DESCENT
     * CONSTANT_DESCENT}.
     * <p>
     ...
     * If not explicitly specified, the baseline will be anchored to the bottom
     * if all the elements with a baseline, and that are aligned to the
     * baseline, have a baseline resize behavior of {@code CONSTANT_DESCENT};
     ...
     * baseline resize behavior of {@code CONSTANT_ASCENT} or
     * {@code CONSTANT_DESCENT}. Elements with a baseline resize behavior of
     * {@code OTHER} or {@code CENTER_OFFSET} are not resizable.
     * <p>
     * The baseline is calculated based on the preferred height of each of the
     * elements that have a baseline. The baseline is calculated using the
     * following algorithm:
     * {@code max(maxNonBaselineHeight, maxAscent + maxDescent)}, where the
     * {@code maxNonBaselineHeight} is the maximum height of all elements that
     * do not have a baseline, or are not aligned along the baseline.
     * {@code maxAscent} is the maximum ascent (baseline) of all elements that
     * have a baseline and are aligned along the baseline. {@code maxDescent} is
     * the maximum descent (preferred height - baseline) of all elements that
     * have a baseline and are aligned along the baseline.
     * <p>
     * A {@code ParallelGroup} that aligns it's elements along the baseline is
     * only useful along the vertical axis. If you create a baseline group and
     * use it along the horizontal axis an {@code IllegalStateException} is
     * thrown when you ask {@code GroupLayout} for the minimum, preferred or
     * maximum size or attempt to layout the components.
     * <p>
     * Elements that are not aligned to the baseline and smaller than the size
     * of the {@code ParallelGroup} are positioned in one of three ways:
     * centered, anchored to the leading edge, or anchored to the trailing edge.
     * <h3>Non-baseline {@code ParallelGroup}</h3> {@code ParallelGroup}s
     * created with an alignment other than {@code BASELINE} align elements that
     * are smaller than the size of the group in one of three ways: centered,
     * anchored to the leading edge, or anchored to the trailing edge.
     * <p>
     * The leading edge is based on the axis and {@code
     * ComponentOrientation}. For the vertical axis the top edge is always the
     * leading edge, and the bottom edge is always the trailing edge. When the
     * {@code ComponentOrientation} is {@code
     * LEFT_TO_RIGHT}, the leading edge is the left edge and the trailing edge
     * the right edge. A {@code ComponentOrientation} of {@code RIGHT_TO_LEFT}
     * flips the left and right edges. Child elements are aligned based on the
     * specified alignment the element was added with. If you do not specify an
     * alignment, the alignment specified for the {@code ParallelGroup} is used.
     * <p>
     * To align elements along the baseline you {@code createBaselineGroup}, or
     * {@code createParallelGroup} with an alignment of {@code BASELINE}. If the
     * group was not created with a baseline alignment, and you attempt to add
     * an element specifying a baseline alignment, an
    ...
    public class ParallelGroup extends Group {
        ...
        /**
         * Adds a {@code Group} to this {@code ParallelGroup} with the specified
         * alignment. If the child is smaller than the {@code Group} it is
         * aligned based on the specified alignment.
         *
         * @param alignment
         *        the alignment
         * @param group
         ...
         * @return this {@code Group}
         */
        public ParallelGroup addComponent(Component component, Alignment alignment, int min, int pref,
                int max) {
            checkChildAlignment(alignment);
        ...
        void setChildSize(Spring spring, int axis, int origin, int size) {
            ...
            int springSize = Math.min(Math.max(spring.getMinimumSize(axis), size), spring.getMaximumSize(
                    axis));
            if (alignment == null) {
        ...
        void insertAutopadding(int axis, List<AutoPreferredGapSpring> leadingPadding,
            ...
            for (Spring spring : springs) {
                if (spring instanceof ComponentSpring) {
                    ...
                    ((Group) spring).insertAutopadding(axis, leadingPadding, trailingPadding, leading,
                            trailing, insert);
    ...
    /**
     * An extension of {@code ParallelGroup} that aligns its constituent
     * {@code Spring}s along the baseline.
     */
        ...
        private void calculateBaselineAndResizeBehavior() {
            ...
            for (Spring spring : springs) {
                if (spring.getAlignment() == null || spring.getAlignment() == Alignment.BASELINE) {
                    ...
                    if (baseline >= 0) {
                        if (spring.isResizable(VERTICAL)) {
                            BaselineResizeBehavior brb = spring.getBaselineResizeBehavior();
                    ...
                    if (baseline >= 0) {
                        ...
                        switch (spring.getBaselineResizeBehavior()) {
                            case CONSTANT_ASCENT:
                                ...
                                break;
                            case CONSTANT_DESCENT:
                                height = Math.min(ascent, springMax - springPref + baseline) + (springPref
                                        - baseline);
    ...
    private class PreferredGapSpring extends Spring {
        ...
        PreferredGapSpring(JComponent source, JComponent target, ComponentPlacement type, int pref, int max) {
            this.source = source;
            this.target = target;
            this.type = type;
            this.pref = pref;
    ...
    private class AutoPreferredGapSpring extends Spring {
        ...
        private int calculatePadding(LayoutStyle p, int position, ComponentSpring source,
                ComponentSpring target) {
            int delta = target.getOrigin() - (source.getOrigin() + source.getSize());
            if (delta >= 0) {
                int padding;
                if ((source.getComponent() instanceof JComponent) && (target
                        .getComponent() instanceof JComponent)) {
                    padding = p.getPreferredGap((JComponent) source.getComponent(), (JComponent) target
                            .getComponent(), type, position, host);
                } else {
                    padding = 10;
                }
                if (padding > delta) {
        ...
        public void addTarget(ComponentSpring spring, int axis) {
            ...
            if (source != null) {
                if (areParallelSiblings(source.getComponent(), spring.getComponent(), oAxis)) {
                ...
                for (int counter = sources.size() - 1; counter >= 0; counter--) {
                    ...
                    if (areParallelSiblings(source.getComponent(), component, oAxis)) {
                        addValidTarget(source, spring);
                    }
                }
            }
    ...
    private class ContainerAutoPreferredGapSpring extends AutoPreferredGapSpring {
            ...
            if (targets != null) {
                ...
                for (int i = targets.size() - 1; i >= 0; i--) {
                    ...
                    if (targetSpring.getComponent() instanceof JComponent) {
                        padding = p.getContainerGap((JComponent) targetSpring.getComponent(), position, host);
        ...
        private int updateSize(LayoutStyle p, ComponentSpring sourceSpring, int position) {
            ...
            if (sourceSpring.getComponent() instanceof JComponent) {
                padding = p.getContainerGap((JComponent) sourceSpring.getComponent(), position, host);
            }
            int delta = Math.max(0, getParent().getSize() - sourceSpring.getSize() - sourceSpring
                    .getOrigin());
            size = Math.max(size, padding - delta);
            return padding;
        }
    ...
    private class ComponentInfo {
        ...
        boolean updateVisibility() {
            ...
            boolean newVisible = (honorsVisibility) ? component.isVisible() : true;
