 ...
 * <p>
 * Adding an item to, and using, this class instead of {@code String} for client
 * properties protects against conflicts with developer-set client properties.
 * Using this class also avoids a problem with {@code StringBuilder} and
 * {@code StringBuffer} keys, whereby the keys are not recognized upon
 * deserialization.
 * <p>
 * When a client property value associated with one of these keys does not
 * implement {@code Serializable}, the result during serialization depends on
 * how the key is defined here. Historically, client properties with values not
 * implementing {@code Serializable} have simply been dropped and left out of
 * the serialized representation. To define keys with such behavior in this
 * enum, provide a value of {@code false} for the
 * {@code reportValueNotSerializable} property. When migrating existing
 * properties to this enum, one may wish to consider using this by default, to
 ...
 * {@code true} for the {@code reportValueNotSerializable} property. This is
 * useful when the property represents something that the developer needs to
 * know about when it cannot be serialized.
 *
