 ...
 * <em>maximum</em> values. Each of these properties may be involved in defining
 * its fourth, <em>value</em>, property based on a series of rules.
 * <p>
 * An instance of the <code>Spring</code> class can be visualized as a
 * mechanical spring that provides a corrective force as the spring is
 * compressed or stretched away from its preferred value. This force is modelled
 * as linear function of the distance from the preferred value, but with two
 * different constants -- one for the compressional force and one for the
 ...
 * equal and opposite force to that which is created when it is at its maximum
 * value. The difference between the <em>preferred</em> and <em>minimum</em>
 * values, therefore, represents the ease with which the spring can be
 ...
 * ordinary (non-compound) <code>Spring</code>. We define the "+", "-",
 ...
 * arithmetic operators that can be applied to intervals (<code>[a, b]</code>
 * refers to the interval from <code>a</code> to <code>b</code>, where
 * <code>a &lt;= b</code>).
 *
 * <pre>
 ...
 * </pre>
 * <p>
 * With both intervals and <code>Spring</code>s we can define "-" and
 * <em>min</em> in terms of negation:
 *
 * <pre>
 *      X - Y = X + (-Y)
 *
 ...
 * </pre>
 * <p>
 * For the static methods in this class that embody the arithmetic operators, we
 * do not actually perform the operation in question as that would snapshot the
 * values of the properties of the method's arguments at the time the static
 * method is called. Instead, the static methods create a new
 * <code>Spring</code> instance containing references to the method's arguments
 * so that the characteristics of the new spring track the potentially changing
 * characteristics of the springs from which it was made. This is a little like
 * the idea of a <em>lazy value</em> in a functional language.
 * <p>
 * If you are implementing a <code>SpringLayout</code> you can find further
 * information and examples in
 * <a href="https://docs.oracle.com/javase/tutorial/uiswing/layout/spring.html">
 * How to Use SpringLayout</a>, a section in <em>The Java Tutorial.</em>
 * <p>
...
public abstract class Spring {
	...
	/**
	 * An integer value signifying that a property value has not yet been
	 * calculated.
	 */
	 ...
	 *
	 * @return the <code>minimumValue</code> property of this
	 *         <code>Spring</code>
	 */
	 ...
	 *
	 * @return the <code>maximumValue</code> property of this
	 *         <code>Spring</code>
	 */
	...
	/**
	 * Sets the current <em>value</em> of this <code>Spring</code> to
	 * <code>value</code>.
	 *
	 * @param value
	...
	/**
	 * Returns a strut -- a spring whose <em>minimum</em>, <em>preferred</em>,
	...
	/**
	 * Returns <code>-s</code>: a spring running in the opposite direction to
	 * <code>s</code>.
	 *
	...
	/**
	 * Returns <code>s1+s2</code>: a spring representing <code>s1</code> and
	 * <code>s2</code> in series. In a sum, <code>s3</code>, of two springs,
	 * <code>s1</code> and <code>s2</code>, the <em>strains</em> of
	 * <code>s1</code>, <code>s2</code>, and <code>s3</code> are maintained at
	 * the same level (to within the precision implied by their integer
	 * <em>value</em>s). The strain of a spring in compression is:
	 * 
	 * <pre>
	 *         value - pref
	 *         ------------
	 ...
	 * </pre>
	 * 
	 * When <code>setValue</code> is called on the sum spring, <code>s3</code>,
	 * the strain in <code>s3</code> is calculated using one of the formulas
	 * above. Once the strain of the sum is known, the <em>value</em>s of
	 * <code>s1</code> and <code>s2</code> are then set so that they are have a
	 * strain equal to that of the sum. The formulas are evaluated so as to take
	 * rounding errors into account and ensure that the sum of the
	 * <em>value</em>s of <code>s1</code> and <code>s2</code> is exactly equal
	 * to the <em>value</em> of <code>s3</code>.
	 *
	...
	/**
	 * Returns <code>max(s1, s2)</code>: a spring whose value is always greater
	 * than (or equal to) the values of both <code>s1</code> and <code>s2</code>
	 * .
	 *
	 * @return <code>max(s1, s2)</code>: a spring whose value is always greater
	 *         than (or equal to) the values of both <code>s1</code> and
	 *         <code>s2</code>
	 * @see Spring
	 */
	...
	/*
	 * public static Spring min(Spring s1, Spring s2) { return
	 * minus(max(minus(s1), minus(s2))); }
	 */

	/**
	 * Returns a spring whose <em>minimum</em>, <em>preferred</em>,
	 * <em>maximum</em> and <em>value</em> properties are each multiples of the
	 * properties of the argument spring, <code>s</code>. Minimum and maximum
	 * properties are swapped when <code>factor</code> is negative (in
	 * accordance with the rules of interval arithmetic).
	 * <p>
	 * When factor is, for example, 0.5f the result represents 'the mid-point'
	 * of its input - an operation that is useful for centering components in a
	 * container.
	 *
	 * @param s
	 *            the spring to scale
	...
	/**
	 * Returns a spring whose <em>minimum</em>, <em>preferred</em>,
	 * <em>maximum</em> and <em>value</em> properties are defined by the widths
	 * of the <em>minimumSize</em>, <em>preferredSize</em>, <em>maximumSize</em>
	 * and <em>size</em> properties of the supplied component. The returned
	 * spring is a 'wrapper' implementation whose methods call the appropriate
	 * size methods of the supplied component. The minimum, preferred, maximum
	 * and value properties of the returned spring therefore report the current
	 * state of the appropriate properties in the component and track them as
	 * they change.
	 *
	 * @param c
	...
	/**
	 * Returns a spring whose <em>minimum</em>, <em>preferred</em>,
	 * <em>maximum</em> and <em>value</em> properties are defined by the heights
	 * of the <em>minimumSize</em>, <em>preferredSize</em>, <em>maximumSize</em>
	 * and <em>size</em> properties of the supplied component. The returned
	 * spring is a 'wrapper' implementation whose methods call the appropriate
	 * size methods of the supplied component. The minimum, preferred, maximum
