 ...
 * similar to <code>ListModel</code> however there are some important
 * differences:
 * <ul>
 ...
 * </ul>
 * <p>
 * A <code>SpinnerModel</code> has three properties, only the first is
 * read/write.
 * <dl>
 * <dt><code>value</code>
...
public interface SpinnerModel {
	 ...
	 * the <code>editor</code> part of a <code>JSpinner</code>. If the
	 * <code>SpinnerModel</code> implementation doesn't support the specified
	 * value then an <code>IllegalArgumentException</code> is thrown. For
	 * example a <code>SpinnerModel</code> for numbers might only support values
	 * that are integer multiples of ten. In that case,
	 ...
	 * <code>getValue()</code>. If the end of the sequence has been reached then
	 ...
	 * <code>ChangeListeners</code> must be notified when models
