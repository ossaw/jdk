 ...
 * Swing Components and the Containment Hierarchy</a>, a section in
 ...
 * programmer or (optionally) selected by the user at runtime. The look and feel
 * for each component is provided by a <em>UI delegate</em> -- an object that
 * descends from {@link javax.swing.plaf.ComponentUI}. See <a href=
 * "https://docs.oracle.com/javase/tutorial/uiswing/lookandfeel/plaf.html">How
 ...
 * Use Key Bindings</a>, an article in <em>The Java Tutorial</em>, for more
 * information.
 * <li>Support for tool tips -- short descriptions that pop up when the cursor
 * lingers over a component. See <a href=
 * "https://docs.oracle.com/javase/tutorial/uiswing/components/tooltip.html">How
 ...
 * for borders. For more information see
 * <a href="http://www.oracle.com/technetwork/java/painting-140037.html#swing">
 * Painting</a> and <a href=
 * "https://docs.oracle.com/javase/tutorial/uiswing/components/border.htmll">How
 * to Use Borders</a>, both of which are sections in <em>The Java Tutorial</em>.
 * </ul>
...
public abstract class JComponent extends Container implements Serializable,
     ...
     * @see #readObject
     */
    private static final Hashtable<ObjectInputStream, ReadObjectCallback> readObjectCallbacks = new Hashtable<ObjectInputStream, ReadObjectCallback>(
            1);
     ...
     * AppContext.get(INPUT_VERIFIER_SOURCE_KEY) returns non-null, it indicates
     * the EDT is calling into the InputVerifier from the returned component.
     */
    ...
    /**
     * Set in <code>_paintImmediately</code>. Will indicate the child that
     * initiated the painting operation. If <code>paintingChild</code> is
     * opaque, no need to paint any child components after
     * <code>paintingChild</code>. Test used in <code>paintChildren</code>.
     */
    transient Component paintingChild;

    /**
     * Constant used for <code>registerKeyboardAction</code> that means that the
    ...
    /**
     * Constant used for <code>registerKeyboardAction</code> that means that the
     * command should be invoked when the receiving component is an ancestor of
     * the focused component or is itself the focused component.
     */
    public static final int WHEN_ANCESTOR_OF_FOCUSED_COMPONENT = 1;

    /**
     * Constant used for <code>registerKeyboardAction</code> that means that the
     * command should be invoked when the receiving component is in the window
     * that has the focus or is itself the focused component.
     */
     ...
     * <code>WHEN_IN_FOCUSED_WINDOW</code> are stashed in the client properties
     * under this string.
     */
    private static final String WHEN_IN_FOCUSED_WINDOW_BINDINGS = "_WhenInFocusedWindow";

    /**
     * The comment to display when the cursor is over the component, also known
     * as a "value tip", "flyover help", or "flyover label".
     */
    ...
    /**
     * <code>JPopupMenu</code> assigned to this component and all of its
     * children
     */
    ...
    private static Object componentObtainingGraphicsFromLock = new StringBuilder(
    ...
    /**
     * Returns the Set of <code>KeyStroke</code>s to use if the component is
     * managing focus for forward focus traversal.
     */
    static Set<KeyStroke> getManagingFocusForwardTraversalKeys() {
        synchronized (JComponent.class) {
            if (managingFocusForwardTraversalKeys == null) {
                ...
                managingFocusForwardTraversalKeys.add(KeyStroke.getKeyStroke(KeyEvent.VK_TAB,
    ...
    /**
     * Returns the Set of <code>KeyStroke</code>s to use if the component is
     * managing focus for backward focus traversal.
     */
    ...
    /**
     * Sets whether or not <code>getComponentPopupMenu</code> should delegate to
     * the parent if this component does not have a <code>JPopupMenu</code>
     * assigned to it.
     * <p>
    ...
    /**
     * Sets the <code>JPopupMenu</code> for this <code>JComponent</code>. The UI
     * is responsible for registering bindings and adding the necessary
     * listeners such that the <code>JPopupMenu</code> will be shown at the
     * appropriate time. When the <code>JPopupMenu</code> is shown depends upon
     * the look and feel: some may show it on a mouse event, some may enable a
     * key binding.
     * <p>
    ...
    /**
     * Returns <code>JPopupMenu</code> that assigned for this component. If this
     * component does not have a <code>JPopupMenu</code> assigned to it and
     * <code>getInheritsPopupMenu</code> is true, this will return
     * <code>getParent().getComponentPopupMenu()</code> (assuming the parent is
     * valid.)
     *
     * @return <code>JPopupMenu</code> assigned for this component or
     ...
     * . It does, however, set the component's locale property to the value
     ...
     * <code>JComponent</code> subclasses generally override this method to
     * narrow the argument type. For example, in <code>JSlider</code>:
     * 
     * <pre>
    ...
    protected void setUI(ComponentUI newUI) {
         ...
         * switch in order to enable the same UI instance *with different
    ...
    /**
     * Uninstalls the UI, if any, and any client properties designated as being
     * specific to the installed UI - instances of {@code UIClientPropertyKey}.
     */
    ...
    /**
     * Returns the graphics object used to paint this component. If
     * <code>DebugGraphics</code> is turned on we create a new
     * <code>DebugGraphics</code> object if necessary. Otherwise we just
     ...
     * to the passed in <code>Graphics</code>. For example, you should not alter
     * the clip <code>Rectangle</code> or modify the transform. If you need to
    ...
    /**
     * Paints this component's children. If <code>shouldUseBuffer</code> is
     * true, no component ancestor has a buffer and the component children can
     * use a buffer if they have one. Otherwise, one ancestor has a buffer
    ...
    protected void paintChildren(Graphics g) {
        ...
        synchronized (getTreeLock()) {
            ...
            for (; i >= 0; i--) {
                ...
                // See 6884960
                if ((!isWindowOpaque || isJComponent || isLightweightComponent(comp)) && comp.isVisible()) {
                    Rectangle cr;
                    ...
                    if (hitClip) {
                        if (checkSiblings && i > 0) {
                            ...
                            SwingUtilities.computeIntersection(clipBounds.x, clipBounds.y, clipBounds.width,
    ...
    public void paint(Graphics g) {
        ...
        try {
            ...
            int bw, bh;
            boolean printing = getFlag(IS_PRINTING);
            if (!printing && repaintManager.isDoubleBufferingEnabled() && !getFlag(ANCESTOR_USING_BUFFER)
                    && isDoubleBuffered() && (getFlag(IS_REPAINTING) || repaintManager.isPainting())) {
    ...
    /**
     * Returns true if this component, or any of its ancestors, are in the
     * processing of painting.
     */
    boolean isPainting() {
     ...
     * <code>Graphics</code>. This method will result in invocations of
     * <code>printComponent</code>, <code>printBorder</code> and
     * <code>printChildren</code>. It is recommended that you override one of
     * the previously mentioned methods rather than this one if your intention
     * is to customize the way printing looks. However, it can be useful to
     * override this method should you want to prepare state before invoking the
     * superclass behavior. As an example, if you wanted to change the
     * component's background color before printing, you could do the following:
     * 
     * <pre>
     ...
     * midst of a print operation. The <code>isPaintingForPrint</code> method
     * provides this ability and its return value will be changed by this
     * method: to <code>true</code> immediately before rendering and to
     * <code>false</code> immediately after. With each change a property change
     * event is fired on this component with the name
     * <code>"paintingForPrint"</code>.
     * <p>
     * This method sets the component's state such that the double buffer will
     * not be used: painting will be done directly on the passed in
     * <code>Graphics</code>.
     *
     * @param g
     *        the <code>Graphics</code> context in which to paint
     ...
     * invoke <code>paintComponent</code> on the component. Override this if you
     ...
     * <code>paintChildren</code> on the component. Override this if you wish to
     ...
     * <code>paintBorder</code> on the component. Override this if you wish to
     ...
     * useful when you want to customize what you print versus what you show on
     * the screen.
     * <p>
     ...
     *
     * @return true if the current painting operation on this component is part
    ...
    /**
     * In release 1.4, the focus subsystem was rearchitected. For more
     * information, see <a href=
     * "https://docs.oracle.com/javase/tutorial/uiswing/misc/focus.html"> How to
     ...
     * the specified <code>Component</code>'s previous <code>Component</code> in
     * the cycle.
     *
     * @param aComponent
    ...
    /**
     * Provides a hint as to whether or not this <code>JComponent</code> should
     * get focus. This is only a hint, and it is up to consumers that are
     ...
     * mouse press on a <code>JComponent</code> to steal focus, but did want the
     * <code>JComponent</code> to be traversable via the keyboard. If you do not
     * want this <code>JComponent</code> focusable at all, use the
     * <code>setFocusable</code> method instead.
     * <p>
     * Please see <a href=
     * "https://docs.oracle.com/javase/tutorial/uiswing/misc/focus.html"> How to
     * Use the Focus Subsystem</a>, a section in <em>The Java Tutorial</em>, for
     * more information.
     *
    ...
    /**
     * Requests that this <code>Component</code> gets the input focus. Refer to
     * {@link java.awt.Component#requestFocus() Component.requestFocus()} for a
     ...
     * {@link #requestFocusInWindow() requestFocusInWindow()}. If you would like
     * more information on focus, see <a href=
     * "https://docs.oracle.com/javase/tutorial/uiswing/misc/focus.html"> How to
     * Use the Focus Subsystem</a>, a section in <em>The Java Tutorial</em>.
     *
    ...
    /**
     * Requests that this <code>Component</code> gets the input focus. Refer to
     * {@link java.awt.Component#requestFocus(boolean)
     * Component.requestFocus(boolean)} for a complete description of this
     * method.
     * <p>
     * Note that the use of this method is discouraged because its behavior is
     * platform dependent. Instead we recommend the use of
     * {@link #requestFocusInWindow(boolean) requestFocusInWindow(boolean)}. If
     * you would like more information on focus, see <a href=
     * "https://docs.oracle.com/javase/tutorial/uiswing/misc/focus.html"> How to
     * Use the Focus Subsystem</a>, a section in <em>The Java Tutorial</em>.
     *
     * @param temporary
    ...
    /**
     * Requests that this <code>Component</code> gets the input focus. Refer to
     * {@link java.awt.Component#requestFocusInWindow()
     * Component.requestFocusInWindow()} for a complete description of this
     * method.
     * <p>
     * If you would like more information on focus, see <a href=
     * "https://docs.oracle.com/javase/tutorial/uiswing/misc/focus.html"> How to
     * Use the Focus Subsystem</a>, a section in <em>The Java Tutorial</em>.
     *
     * @return <code>false</code> if the focus change request is guaranteed to
    ...
    /**
     * Requests that this <code>Component</code> gets the input focus. Refer to
     * {@link java.awt.Component#requestFocusInWindow(boolean)
     ...
     * from <code>JComponent</code>, the look and feel implementation of many
     ...
     * other than <code>JPanel</code> or <code>JLabel</code>, we recommend that
     * you put the component in a <code>JPanel</code> and set the border on the
     * <code>JPanel</code>.
     * <p>
     * This is a bound property.
     *
    ...
    public void setBorder(Border border) {
        ...
        if (border != oldBorder) {
            if (border == null || oldBorder == null || !(border.getBorderInsets(this).equals(oldBorder
    ...
    /**
     * Returns the border of this component or <code>null</code> if no border is
     * currently set.
     *
    ...
    /**
     * Overrides <code>Container.getAlignmentY</code> to return the horizontal
     * alignment.
     *
    ...
    /**
     * Overrides <code>Container.getAlignmentX</code> to return the vertical
     * alignment.
     *
    ...
    /**
     * Returns this component's graphics context, which lets you draw on a
     * component. Use this method to get a <code>Graphics</code> object and then
     ...
     *           DebugGraphics.FLASH_OPTION BUFFERED_OPTION
     *           DebugGraphics.BUFFERED_OPTION description: Diagnostic options
     ...
     * behavior. For example, to bind the <code>KeyStroke</code>
     ...
     * <p>
     * Register a new keyboard action. <code>anAction</code> will be invoked if
     * a key event matching <code>aKeyStroke</code> occurs and
     * <code>aCondition</code> is verified. The <code>KeyStroke</code> object
     * defines a particular combination of a keyboard key and one or more
     ...
     * other words, whenever <em>any</em> component in the window has the focus,
     ...
     * with the same charCode and the same modifiers, <code>anAction</code> will
     * replace the action.
     *
     * @param anAction
    ...
    private void registerWithKeyboardManager(boolean onlyIfNew) {
        ...
        KeyStroke[] strokes;
        Hashtable<KeyStroke, KeyStroke> registered = (Hashtable<KeyStroke, KeyStroke>) getClientProperty(
    ...
    /**
     * Invoked from <code>ComponentInputMap</code> when its bindings change. If
     * <code>inputMap</code> is the current <code>windowInputMap</code> (or a
     * parent of the window <code>InputMap</code>) the
     ...
     * <code>InputMap</code>s to be active, whereas putting a dummy binding in
     * the <code>InputMap</code> effectively disables the binding from ever
     * happening.
     * <p>
     ...
     * <p>
     * For Java 2 platform v1.3, a <code>KeyStroke</code> can be associated with
     * more than one condition. For example, 'a' could be bound for the two
     ...
     * keystroke.
     *
     * @return the <code>ActionListener</code> object invoked when the keystroke
     *         occurs
     */
    ...
    /**
     * Unregisters all the bindings in the first tier <code>InputMaps</code> and
     * <code>ActionMap</code>. This has the effect of removing any local
     * bindings, and allowing the bindings defined in parent
     * <code>InputMap/ActionMaps</code> (the UI is usually defined in the second
     * tier) to persist.
     */
     ...
     * <code>condition</code> to <code>map</code>. A <code>null</code> value
     * implies you do not want any bindings to be used, even from the UI. This
     * will not reinstall the UI <code>InputMap</code> (if there was one).
    ...
    /**
     * Sets the <code>ActionMap</code> to <code>am</code>. This does not set the
     * parent of the <code>am</code> to be the <code>ActionMap</code> from the
     ...
     * binding. The returned <code>ActionMap</code>, unless otherwise set, will
     * have the <code>ActionMap</code> from the UI set as the parent.
     *
     ...
     * <code>condition</code>. If the <code>InputMap</code> hasn't been created,
     ...
     * baseline.
     * <p>
     * This method calls into the <code>ComponentUI</code> method of the same
     * name. If this component does not have a <code>ComponentUI</code> -1 will
     * be returned. If a value &gt;= 0 is returned, then the component has a
     * valid baseline for any size &gt;= the minimum size and
     ...
     * <p>
     * This method calls into the <code>ComponentUI</code> method of the same
     * name. If this component does not have a <code>ComponentUI</code>
     * <code>BaselineResizeBehavior.OTHER</code> will be returned. Subclasses
     * should never return <code>null</code>; if the baseline can not be
     * calculated return <code>BaselineResizeBehavior.OTHER</code>. Callers
     * should first ask for the baseline using <code>getBaseline</code> and if a
     * value &gt;= 0 is returned use this method. It is acceptable for this
     * method to return a value other than
     * <code>BaselineResizeBehavior.OTHER</code> even if
    ...
    /**
     * Returns the default locale used to initialize each JComponent's locale
     * property upon creation.
     * The default locale has "AppContext" scope so that applets (and
     * potentially multiple lightweight applications running in a single VM) can
     * have their own setting. An applet can safely alter its default locale
    ...
    /**
     * Invoked to process the key bindings for <code>ks</code> as the result of
     * the <code>KeyEvent</code> <code>e</code>. This obtains the appropriate
     * <code>InputMap</code>, gets the binding, gets the action from the
     * <code>ActionMap</code>, and then (if the action is found and the
     * component is enabled) invokes <code>notifyAction</code> to notify the
     * action.
     *
     * @param ks
     ...
     * consumed by the <code>FocusManager</code>, <code>KeyListeners</code>, or
     * the component. It will first try <code>WHEN_FOCUSED</code> bindings, then
     * <code>WHEN_ANCESTOR_OF_FOCUSED_COMPONENT</code> bindings, and finally
    ...
    boolean processKeyBindings(KeyEvent e, boolean pressed) {
        ...
        if (e.getID() == KeyEvent.KEY_TYPED) {
            ...
            if (e.getKeyCode() != e.getExtendedKeyCode()) {
                ksE = KeyStroke.getKeyStroke(e.getExtendedKeyCode(), e.getModifiers(), (pressed ? false
                        : true));
            }
        }
        ...
        while (parent != null && !(parent instanceof Window) && !(parent instanceof Applet)) {
            if (parent instanceof JComponent) {
                if (ksE != null && ((JComponent) parent).processKeyBinding(ksE, e,
                    ...
                    return true;
                if (((JComponent) parent).processKeyBinding(ks, e, WHEN_ANCESTOR_OF_FOCUSED_COMPONENT,
                        pressed))
                    return true;
            }
            ...
            if ((parent instanceof JInternalFrame) && JComponent.processKeyBindingsForAllComponents(e, parent,
                    pressed)) {
                return true;
            }
            parent = parent.getParent();
     ...
     * this returns any string set using <code>setToolTipText</code>. If a
     ...
     * <code>null</code> is returned, Swing will choose a location. The default
     ...
     * component's coordinate system. It is up to the look and feel to honor
     * this property, some may choose to ignore it. If {@code null}, the look
     ...
     * request, such as <code>JViewport</code>, override this method and perform
     * the scrolling.
     *
     * @param aRect
    ...
    /**
     * Sets the <code>autoscrolls</code> property. If <code>true</code> mouse
     * dragged events will be synthetically generated when the mouse is dragged
     * outside of the component's bounds and mouse motion has paused (while the
     * button continues to be held down). The synthetic events make it appear
     * that the drag gesture has resumed in the direction established when the
     * component's boundary was crossed. Components that support autoscrolling
     * must handle <code>mouseDragged</code> events by calling
     * <code>scrollRectToVisible</code> with a rectangle that contains the mouse
     * event's location. All of the Swing components that support item selection
     * and are typically displayed in a <code>JScrollPane</code> (
     * <code>JTable</code>, <code>JList</code>, <code>JTree</code>,
     * <code>JTextArea</code>, and <code>JEditorPane</code>) already handle
     * mouse dragged events in this way. To enable autoscrolling in any other
     * component, add a mouse motion listener that calls
     * <code>scrollRectToVisible</code>. For example, given a
     * <code>JPanel</code>, <code>myPanel</code>:
     * 
     * <pre>
    ...
    /**
     * Sets the {@code TransferHandler}, which provides support for transfer of
     * data into and out of this component via cut/copy/paste and drag and drop.
     * This may be {@code null} if the component does not support data transfer
     * operations.
     * <p>
     * If the new {@code TransferHandler} is not {@code null}, this method also
     * installs a <b>new</b> {@code DropTarget} on the component to activate
     * drop handling through the {@code TransferHandler} and activate any
     * built-in support (such as calculating and displaying potential drop
     ...
     * First, if the existing drop target on this component was explicitly set
     * by the developer to a {@code non-null} value. Second, if the system
     * property {@code suppressSwingDropSupport} is {@code true}. The default
     * value for the system property is {@code false}.
     * <p>
     * Please see
     ...
     *           data to and from the component
     */
    public void setTransferHandler(TransferHandler newHandler) {
        TransferHandler oldHandler = (TransferHandler) getClientProperty(JComponent_TRANSFER_HANDLER);
     ...
     * <code>null</code> is returned if this component doesn't calculate custom
     * drop locations. In this case, <code>TransferHandler</code> will provide a
     ...
     * cases, the component may need to use its internal selection temporarily
     ...
     * {@link java.awt.Component#processMouseEvent(MouseEvent)} for a complete
    ...
    static class KeyboardState implements Serializable {
        ...
        /**
         * Updates internal state of the KeyboardState and returns true if the
         * event should be processed further.
         */
     ...
     *             <code>java.awt.Component.setEnabled(boolean)</code>.
     */
    @Deprecated
    public void enable() {
        if (isEnabled() != true) {
     ...
     * developers, but is instead meant only to be subclassed by component
     * developers.
     * <p>
     * <strong>Warning:</strong> Serialized objects of this class will not be
     ...
     */
    public abstract class AccessibleJComponent extends AccessibleAWTContainer implements
            AccessibleExtendedComponent {
        /**
         * Though the class is abstract, this should be called by all
         * sub-classes.
         */
         ...
         * add/remove ContainerListener and FocusListener to track target
         * JComponent's state
         */
        private volatile transient int propertyListenersCount = 0;

        /**
         * This field duplicates the function of the accessibleAWTFocusHandler
         * field in java.awt.Component.AccessibleAWTComponent, so it has been
         * deprecated.
         */
        @Deprecated
        ...
        protected class AccessibleContainerHandler implements ContainerListener {
            public void componentAdded(ContainerEvent e) {
                ...
                if (c != null && c instanceof Accessible) {
                    AccessibleJComponent.this.firePropertyChange(AccessibleContext.ACCESSIBLE_CHILD_PROPERTY,
        ...
        /**
         * Fire PropertyChange listener, if one is registered, when focus events
         * happen
         * 
         * @since 1.3
         */
        ...
        /**
         * Removes a PropertyChangeListener from the listener list. This removes
         ...
         * return java.awt.Component.getName(), as that generally isn't a
         * localized name, and doesn't have meaning for the user. If the object
         * is fundamentally a text object (such as a menu item), the accessible
         * name should be the text of the object (for example, "save"). If the
         * object has a tooltip, the tooltip text may also be an appropriate
         * String to return.
         *
        ...
        /**
         * Gets the accessible description of this object. This should be a
         * concise, localized description of what this object is - what is its
         * meaning to the user. If the object has a tooltip, the tooltip text
         * may be an appropriate string to return, assuming it contains a
         * concise description of the object (instead of just the name of the
         * object - for example a "Save" icon on a toolbar that had "save" as
         * the tooltip text shouldn't return the tooltip text as the
         * description, but something like "Saves the current text document"
         * instead).
         *
     ...
     * for this component. If the <code>clientProperties</code> table doesn't
     ...
     * Component. Refer to {@link java.awt.Component#setFocusTraversalKeys} for
    ...
    /*
     * --- Transitional java.awt.Component Support --- The methods and fields in
     * this section will migrate to java.awt.Component in the next JDK release.
     */
    ...
    /**
     * Stores the bounds of this component into "return value" <code>rv</code>
     * and returns <code>rv</code>. If <code>rv</code> is <code>null</code> a
     ...
     * <code>rv</code> and returns <code>rv</code>. If <code>rv</code> is
     * <code>null</code> a new <code>Dimension</code> object is allocated. This
     * version of <code>getSize</code> is useful if the caller wants to avoid
     ...
     * <code>rv</code> and returns <code>rv</code>. If <code>rv</code> is
     * <code>null</code> a new <code>Point</code> is allocated. This version of
    ...
    /**
     * Returns the current x coordinate of the component's origin. This method
     * is preferable to writing <code>component.getBounds().x</code>, or
    ...
    /**
     * Returns the current y coordinate of the component's origin. This method
     * is preferable to writing <code>component.getBounds().y</code>, or
     ...
     * component's opaque children then returns true. Only direct children are
     * considered, more distant descendants are ignored. A
     * <code>JComponent</code> is opaque if <code>JComponent.isOpaque()</code>
     * returns true, other lightweight components are always considered
    ...
    boolean rectangleIsObscured(int x, int y, int width, int height) {
        ...
        for (int i = 0; i < numChildren; i++) {
            ...
            if (x >= cx && (x + width) <= (cx + cw) && y >= cy && (y + height) <= (cy + ch) && child
                    .isVisible()) {
                ...
                if (child instanceof JComponent) {
                    ...
                    /**
                     * Sometimes a heavy weight can have a bound larger than its
                     * peer size so we should always draw under heavy weights
                     */
                    return false;
                }
            }
     ...
     * <code>new Rectangle(0, 0, getWidth(), getHeight())</code>, and all of its
    ...
    /**
     * Supports reporting constrained property changes. This method can be
     * called when a constrained property has changed and it will send the
     * appropriate <code>PropertyChangeEvent</code> to any registered
     ...
     * component.
     *
     * @return all of the component's <code>VetoableChangeListener</code>s or an
     *         empty array if no vetoable change listeners are currently
     *         registered
     ...
     * <code>Window</code> or <code>Applet</code>), or <code>null</code> if this
     * component has not been added to any container.
     *
    ...
    public void addAncestorListener(AncestorListener listener) {
        ...
        if (ancestorNotifier == null) {
            ...
            putClientProperty(JComponent_ANCESTOR_NOTIFIER, ancestorNotifier);
        }
     ...
     * component.
     *
     * @return all of the component's <code>AncestorListener</code>s or an empty
     ...
     * <code><em>Foo</em>Listener</code>s upon this <code>JComponent</code>.
     ...
     * you can query a <code>JComponent</code> <code>c</code> for its mouse
     ...
     * <code>KeyboardAction</code> event listeners. This method is called by the
     ...
     * the chain of parent components are removed. This method is called by the
     * toolkit internally and should not be called directly by programs.
     *
     * @see #registerKeyboardAction
     */
    ...
    public void repaint(long tm, int x, int y, int width, int height) {
        RepaintManager.currentManager(SunToolkit.targetToAppContext(this)).addDirtyRegion(this, x, y, width,
                height);
    }
     ...
     * <code>JScrollPane</code>, and <code>JTextField</code> return true from
     * <code>isValidateRoot</code>.
     * <p>
     ...
     * of this component will cause the entire tree beginning with this root to
     * be validated. Returns false by default. <code>JScrollPane</code>
     ...
     * <code>JLayeredPane</code>, should override this method to return false.
     *
    ...
    /**
     * Returns {@code true} if a paint triggered on a child component should
     * cause painting to originate from this Component, or one of its ancestors.
     * <p>
     * Calling {@link #repaint} or {@link #paintImmediately(int, int, int, int)}
     * on a Swing component will result in calling the
     * {@link JComponent#paintImmediately(int, int, int, int)} method of the
     * first ancestor which {@code isPaintingOrigin()} returns {@code true}, if
     * there are any.
     * <p>
    ...
    /**
     * Returns whether this component should be guaranteed to be on top. For
     * example, it would make no sense for <code>Menu</code>s to pop up under
     * another component, so they would always return true. Most components will
     * want to return false, hence that is the default.
     *
     * @return always returns false
     */
    // package private
    ...
    void _paintImmediately(int x, int y, int w, int h) {
        ...
        // System.out.println("1) ************* in _paintImmediately for " +
        // this);
            ...
            if (!ontop && jc != null && !jc.isOptimizedDrawingEnabled()) {
                ...
                if (c != this) {
                    if (jc.isPaintingOrigin()) {
                        ...
                        switch (jc.getObscuredState(i, paintImmediatelyClip.x, paintImmediatelyClip.y,
        ...
        if (c == null || c.getPeer() == null || paintImmediatelyClip.width <= 0
            ...
            if ((g = safelyGetGraphics(paintingComponent, c)) != null) {
                try {
                    if (hasBuffer) {
                        ...
                        rm.beginPaint();
                        try {
                            rm.paint(paintingComponent, bufferedComponent, g, paintImmediatelyClip.x,
                        ...
                        g.setClip(paintImmediatelyClip.x, paintImmediatelyClip.y, paintImmediatelyClip.width,
                                paintImmediatelyClip.height);
    ...
    /**
     * Paints to the specified graphics. This does not set the clip and it does
     * not adjust the Graphics in anyway, callers must do that first. This
    ...
    /**
     * Returns whether or not the region of the specified component is obscured
     * by a sibling.
     *
    ...
    private int getObscuredState(int compIndex, int x, int y, int width, int height) {
        ...
        for (int i = compIndex - 1; i >= 0; i--) {
            ...
            } else if (retValue == NOT_OBSCURED && !((x + width <= siblingRect.x) || (y
                    + height <= siblingRect.y) || (x >= siblingRect.x + siblingRect.width)
    ...
    /**
     * Returns true, which implies that before checking if a child should be
     * painted it is first check that the child is not obscured by another
    ...
    static void setWriteObjCounter(JComponent comp, byte count) {
        comp.flags = (comp.flags & ~(0xFF << WRITE_OBJ_COUNTER_FIRST)) | (count << WRITE_OBJ_COUNTER_FIRST);
    }
    ...
    /**
     * Sets whether this component should use a buffer to paint. If set to true,
     * all the drawing from this component will be done in an offscreen painting
     * buffer. The offscreen painting buffer will the be copied onto the screen.
     * If a <code>Component</code> is buffered and one of its ancestor is also
    ...
    /**
     * This is called from Component by way of reflection. Do NOT change the
     * name unless you change the code in Component as well.
     */
    void compWriteObjectNotify() {
        ...
        /*
         * JTableHeader is in a separate package, which prevents it from being
         * able to override this package-private method the way the other
         * components can. We don't want to make this method protected because
    ...
    /**
     * This object is the <code>ObjectInputStream</code> callback that's called
     * after a complete graph of objects (including at least one
     * <code>JComponent</code>) has been read. It sets the UI property of each
     * Swing component that was read to the current default with
     * <code>updateUI</code>.
     * <p>
     * As each component is read in we keep track of the current set of root
     * components here, in the roots vector. Note that there's only one
     * <code>ReadObjectCallback</code> per <code>ObjectInputStream</code>,
     * they're stored in the static <code>readObjectCallbacks</code> hashtable.
     *
    ...
    private class ReadObjectCallback implements ObjectInputValidation {
        ...
        /**
         * This is the method that's called after the entire graph of objects
         * has been read in. It initializes the UI property of all of the
         * copmonents with <code>SwingUtilities.updateComponentTreeUI</code>.
         */
        ...
        private void registerComponent(JComponent c) {
            /*
             * If the Component c is a descendant of one of the existing roots
            ...
            /*
             * Otherwise: if Component c is an ancestor of any of the existing
             * roots then remove them and add c (the "new root") to the roots
             * vector.
             */
    ...
    /**
     * We use the <code>ObjectInputStream</code> "registerValidation" callback
     * to update the UI for the entire tree of components after they've all been
     * read in.
     *
     * @param s
    ...
    private void readObject(ObjectInputStream s) throws IOException, ClassNotFoundException {
         ...
         * this is the first call to JComponent.readObject() for this graph of
         * objects, then create a callback and stash it in the
         * readObjectCallbacks table. Note that the ReadObjectCallback
     ...
     * <code>ObjectOutputStream</code> we temporarily uninstall its UI. This is
     * tricky to do because we want to uninstall the UI before any of the
     * <code>JComponent</code>'s children (or its <code>LayoutManager</code>
     * etc.) are written, and we don't want to restore the UI until the most
    ...
    /**
     * Returns a string representation of this <code>JComponent</code>. This
    ...
    protected String paramString() {
        ...
        return super.paramString() + ",alignmentX=" + alignmentX + ",alignmentY=" + alignmentY + ",border="
                + borderString + ",flags=" + flags + // should beef
                                                                                                                                                         // this up a
                                                                                                                                                         // bit
                ",maximumSize=" + maximumSizeString + ",minimumSize=" + minimumSizeString + ",preferredSize="
                + preferredSizeString;
    }
