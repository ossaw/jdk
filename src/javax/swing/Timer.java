 ...
 * target = "_top">How to Use Timers</a>, a section in
 * <em>The Java Tutorial.</em> For more examples and help in choosing between
 * this <code>Timer</code> class and <code>java.util.Timer</code>, see
 * <a href="http://java.sun.com/products/jfc/tsc/articles/timer/" target="_top">
 * Using Timers in Swing Applications</a>, an article in
...
public class Timer implements Serializable {
     ...
     * between-event delay to {@code delay} milliseconds. If {@code delay} is
     * less than or equal to zero, the timer fires as soon as it is started. If
     * <code>listener</code> is not <code>null</code>, it's registered as an
     ...
     * <code><em>Foo</em>Listener</code>s upon this <code>Timer</code>.
     ...
     * you can query a <code>Timer</code> instance <code>t</code> for its action
    ...
    /**
     * Enables or disables the timer log. When enabled, a message is posted to
    ...
    /**
     * Sets the <code>Timer</code>'s between-event delay, the number of
     * milliseconds between successive action events. This does not affect the
     * initial delay property, which can be set by the {@code setInitialDelay}
     * method.
     *
     * @param delay
    ...
    /**
     * Sets the <code>Timer</code>'s initial delay, the time in milliseconds to
     * wait after the timer is started before firing the first event. Upon
     * construction, this is set to be the same as the between-event delay, but
     * then its value is independent and remains unaffected by changes to the
     * between-event delay.
     *
     * @param initialDelay
     ...
     * <code>ActionEvent</code>s fired by this timer. <code>null</code> is an
     * acceptable value.
     *
     * @param command
     *        the action command
     ...
     * <code>ActionEvent</code>s fired by this timer. May be <code>null</code>,
    ...
    /*
     * We have to use readResolve because we can not initialize final fields for
