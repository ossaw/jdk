 ...
 * <code>CellEditor</code> interface (See <code>DefaultCellEditor</code> for
 * example). The wrapper approach is particularly useful if the user want to use
 * a 3rd party ISV editor with <code>JTable</code>, but the ISV didn't implement
 * the <code>CellEditor</code> interface. The user can simply create an object
