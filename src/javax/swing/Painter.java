 ...
 * together. Implementations of this interface are also trivial to write, such
 * that if you can't find a <code>Painter</code> that does what you need, you
 ...
 * write a <code>Painter</code> that only works with subclasses of
 * {@link java.awt.Component}. In that case, when the <code>Painter</code> is
 * declared, you may declare that it requires a <code>Component</code>, allowing
 * the paint method to be type safe. Ex:
 * 
 * <pre>
 * {@code
...
public interface Painter<T> {
    /**
     * <p>
     * Renders to the given {@link java.awt.Graphics2D} object. Implementations
     * of this method <em>may</em> modify state on the <code>Graphics2D</code>,
     * and are not required to restore that state upon completion. In most
     ...
     * <code>Painter</code> that expected it could then read state from that
     ...
     * More specifically, the specified width and height instruct the painter
     ...
     * to fill the painted region. Thus, if I use this <code>Painter</code> to
     * paint a 500 x 500 region, the far left would be black, the far right
     * would be white, and a smooth gradient would be painted between. I could
     * then, without modification, reuse the <code>Painter</code> to paint a
     * region that is 20x20 in size. This region would also be black on the
     * left, white on the right, and a smooth gradient painted between.
     * </p>
     *
     * @param g
