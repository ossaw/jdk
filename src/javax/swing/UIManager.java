 ...
 * the Java platform. Typically the <code>swing.properties</code> file is
 * located in the <code>lib</code> subdirectory of the Java installation
 * directory. Refer to the release notes of the implementation being used for
 * further details.
 ...
 * different times in their life cycle. Some look and feels may aggressively
 * look up defaults, so that changing a default may not have an effect after
...
public class UIManager implements Serializable {
	 ...
	 * Swing applications the fields in this class could just as well be static
	 * members of <code>UIManager</code> however we give them "AppContext" scope
	 * instead so that applets (and potentially multiple lightweight
	...
	private static class LAFState {
		...
		/**
		 * Returns the SwingPropertyChangeSupport for the current AppContext. If
		 * <code>create</code> is a true, a non-null
	...
	/*
	 * Keys used in the <code>swing.properties</code> properties file. See
	 * loadUserProperties(), initialize().
	 */
	...
	/**
	 * Return a <code>swing.properties</code> file key for the attribute of
	 * specified look and feel. The attr is either "name" or "class", a typical
	 ...
	 * implementation-specific. It is typically located in the <code>lib</code>
	 * subdirectory of the Java installation directory. This method returns a
	 * bogus filename if <code>java.home</code> isn't defined.
	 */
	...
	/**
	 * Provides a little information about an installed <code>LookAndFeel</code>
	...
	public static class LookAndFeelInfo {
		...
		/**
		 * Constructs a <code>UIManager</code>s <code>LookAndFeelInfo</code>
		 * object.
		 *
		 * @param name
		 *            a <code>String</code> specifying the name of the look and
		 *            feel
		 * @param className
		 *            a <code>String</code> specifying the name of the class
		...
		/**
		 * Returns the name of the look and feel in a form suitable for a menu
	 ...
	 * <code>swing.properties</code> file is available or if the file doesn't
	 * contain a "swing.installedlafs" property.
	 *
	 ...
	 * <code>LookAndFeelInfo</code> objects can be used by an application to
	 * construct a menu of look and feel options for the user, or to determine
	 * which look and feel to set at startup time. To avoid the penalty of
	 * creating numerous {@code
	 * LookAndFeel} objects, {@code LookAndFeelInfo} maintains the class name of
	 * the {@code LookAndFeel} class, not the actual {@code LookAndFeel}
	 * instance.
	 * <p>
	 * The following example illustrates setting the current look and feel from
	 * an instance of {@code LookAndFeelInfo}:
	 * 
	 * <pre>
	...
	/**
	 * Sets the set of available look and feels. While this method does not
	 * check to ensure all of the {@code LookAndFeelInfos} are {@code non-null},
	 * it is strongly recommended that only {@code non-null} values are supplied
	 ...
	 * While this method allows a {@code null} {@code info}, it is strongly
	...
	/**
	 * Adds the specified look and feel to the set of available look and feels.
	 * While this method does not check the arguments in any way, it is strongly
	...
	/**
	 * Sets the current look and feel to {@code newLookAndFeel}. If the current
	 * look and feel is {@code non-null} {@code
	 * uninitialize} is invoked on it. If {@code newLookAndFeel} is
	 * {@code non-null}, {@code initialize} is invoked on it followed by
	 * {@code getDefaults}. The defaults returned from {@code
	 * newLookAndFeel.getDefaults()} replace those of the defaults from the
	 * previous look and feel. If the {@code newLookAndFeel} is {@code null},
	 * the look and feel defaults are set to {@code null}.
	 * <p>
	 * A value of {@code null} can be used to set the look and feel to
	 * {@code null}. As the {@code LookAndFeel} is required for most of Swing to
	 * function, setting the {@code LookAndFeel} to {@code null} is strongly
	 * discouraged.
	 * <p>
	 ...
	 *             if {@code newLookAndFeel} is {@code non-null} and
	...
	/**
	 * Loads the {@code LookAndFeel} specified by the given class name, using
	 * the current thread's context class loader, and passes it to
	 ...
	 *             if {@code className} does not identify a class that extends
	 *             {@code LookAndFeel}
	 */
	 ...
	 * the native system look and feel if there is one, otherwise the name of
	 * the default cross platform <code>LookAndFeel</code> class. This value can
	 ...
	 * This value can be overriden by setting the
	...
	/**
	 * Returns the defaults. The returned defaults resolve using the logic
	...
	/**
	 * Returns a font from the defaults that is appropriate for the given
	 * locale. If the value for {@code key} is not a {@code Font}, {@code null}
	 * is returned.
	 *
	 * @param key
	 *            an <code>Object</code> specifying the font
	...
	/**
	 * Returns a color from the defaults that is appropriate for the given
	 * locale. If the value for {@code key} is not a {@code Color}, {@code null}
	 * is returned.
	 *
	 * @param key
	 *            an <code>Object</code> specifying the color
	...
	/**
	 * Returns an <code>Icon</code> from the defaults that is appropriate for
	 * the given locale. If the value for {@code key} is not an {@code Icon},
	...
	/**
	 * Returns a border from the defaults that is appropriate for the given
	 * locale. If the value for {@code key} is not a {@code Border},
	 * {@code null} is returned.
	 *
	 * @param key
	 *            an <code>Object</code> specifying the border
	...
	/**
	 * Returns a string from the defaults that is appropriate for the given
	 * locale. If the value for {@code key} is not a {@code String},
	 * {@code null} is returned.
	 *
	 * @param key
	 *            an <code>Object</code> specifying the string
	...
	/**
	 * Returns an integer from the defaults that is appropriate for the given
	 * locale. If the value for {@code key} is not an {@code Integer}, or does
	...
	/**
	 * Returns a boolean from the defaults which is associated with the key
	 * value. If the key is not found or the key doesn't represent a boolean
	...
	/**
	 * Returns a boolean from the defaults which is associated with the key
	 * value and the given <code>Locale</code>. If the key is not found or the
	 * key doesn't represent a boolean value then {@code false} will be
	 * returned.
	 *
	 * @param key
	 *            an <code>Object</code> specifying the key for the desired
	...
	/**
	 * Returns an <code>Insets</code> object from the defaults. If the value for
	 ...
	 * appropriate for the given locale. If the value for {@code key} is not an
	...
	/**
	 * Returns a dimension from the defaults that is appropriate for the given
	 * locale. If the value for {@code key} is not a {@code Dimension},
	 * {@code null} is returned.
	 *
	 * @param key
	 *            an <code>Object</code> specifying the dimension object
	...
	/**
	 * Returns an object from the defaults that is appropriate for the given
	 * locale.
	 *
	 * @param key
	 *            an <code>Object</code> specifying the desired object
	 ...
	 * {@code getDefaults().put(key, value)}. This only effects the developer
	 * defaults, not the system or look and feel defaults.
	 *
	 * @param key
	 *            an <code>Object</code> specifying the retrieval key
	 * @param value
	 *            the <code>Object</code> to store; refer to {@code UIDefaults}
	 ...
	 * {@code getDefaults().getUI(target)}. However, if an auxiliary look and
	 * feel has been installed, this first invokes {@code getUI(target)} on the
	 * multiplexing look and feel's defaults, and returns that value if it is
	 * {@code non-null}.
	 *
	 * @param target
	 *            the <code>JComponent</code> to return the {@code ComponentUI}
	 *            for
	 ...
	 * obtained at the time the look and feel was installed.
	 * <p>
	 * In general, developers should use the {@code UIDefaults} returned from
	 * {@code getDefaults()}. As the current look and feel may expect certain
	 * values to exist, altering the {@code UIDefaults} returned from this
	...
	private static LookAndFeel getMultiLookAndFeel() {
		...
		if (multiLookAndFeel == null) {
			...
			String className = getLAFState().swingProps.getProperty(multiplexingLAFKey,
					defaultName);
			try {
				Class lnfClass = SwingUtilities.loadSystemClass(className);
	 ...
	 * other <code>LookAndFeel</code> classes for a component instance are to be
	 * used in addition to the default <code>LookAndFeel</code> class when
	 * creating a multiplexing UI. The change will only take effect when a new
	 * UI class is created or when the default look and feel is changed on a
	 * component instance.
	 * <p>
	...
	/**
	 * Removes a <code>LookAndFeel</code> from the list of auxiliary look and
	 * feels. The auxiliary look and feels tell the multiplexing look and feel
	 * what other <code>LookAndFeel</code> classes for a component instance are
	 * to be used in addition to the default <code>LookAndFeel</code> class when
	 * creating a multiplexing UI. The change will only take effect when a new
	 * UI class is created or when the default look and feel is changed on a
	 * component instance.
	 * <p>
	 ...
	 * The auxiliary look and feels tell the multiplexing look and feel what
	 * other <code>LookAndFeel</code> classes for a component instance are to be
	 * used in addition to the default LookAndFeel class when creating a
	 * multiplexing UI.
	 * <p>
	 * Note these are not the same as the installed look and feels.
	 *
	 ...
	 * this UIManager with addPropertyChangeListener().
	 *
	 ...
	 * <code>swing.installedlafs</code> property then initialize the
	 * <code>installedLAFs</code> field.
	 *
	 * @see #getInstalledLookAndFeels
	 */
	private static void initializeInstalledLAFs(Properties swingProps) {
		 ...
		 * property. For example given "swing.installedlafs=motif,windows" lafs
		 * = {"motif", "windows"}.
		 */
	...
	/**
	 * If the user has specified a default look and feel, use that. Otherwise
	 * use the look and feel that's native to this platform. If this code is
	 * called after the application has explicitly set it's look and feel, do
	 * nothing.
	 *
	 * @see #maybeInitialize
	 */
	...
	private static void initializeAuxiliaryLAFs(Properties swingProps) {
		...
		while (p.hasMoreTokens()) {
			...
			try {
				...
				System.err
						.println("UIManager: failed loading auxiliary look and feel " + className);
			}
		}

		/*
		 * If there were problems and no auxiliary look and feels were loaded,
		 * make sure we reset auxLookAndFeels to null. Otherwise, we are going
		 * to use the MultiLookAndFeel to get all component UI's, so we need to
		 * load it now.
		 */
	 ...
	 * <code>AppContext</code> specific LAFState object runs. When the
	 * AppContext corresponds to a set of applets it's possible for this method
