 ...
 * components. As a convenience, the {@code add}, {@code remove}, and
 * {@code setLayout} methods of this class are overridden, so that they delegate
 * calls to the corresponding methods of the {@code ContentPane}. For example,
 * you can add a child component to an internal frame as follows:
 * 
 * <pre>
...
public class JInternalFrame extends JComponent
	...
	/**
	 * The <code>JRootPane</code> instance that manages the content pane and
	 * optional menu bar for this internal frame, as well as the glass pane.
	 *
	 * @see JRootPane
	 ...
	 * is set to true when the <code>JInternalFrame</code> is constructed.
	 *
	...
	/**
	 * Constrained property name indicating that the internal frame is
	 * maximized.
	 */
	 ...
	 *
	 * @return the <code>InternalFrameUI</code> object that renders this
	 *         component
	 */
	...
	/**
	 * Notification from the <code>UIManager</code> that the look and feel has
	...
	/*
	 * This method is called if <code>updateUI</code> was called on the
	 * associated JDesktopIcon. It's necessary to avoid infinite recursion.
	 */
	 ...
	 * is not a child of the <code>JInternalFrame</code> this will forward the
	 ...
	 * <code>contentPane</code>. Refer to {@link javax.swing.RootPaneContainer}
	 ...
	 * <code>JInternalFrame</code>, or <code>null</code> if no menu bar has been
	 * set.
	 * 
	...
	/**
	 * Sets the <code>menuBar</code> property for this
	 * <code>JInternalFrame</code>.
	 *
	 * @param m
	 ...
	 * @beaninfo bound: true hidden: true description: The client area of the
	 *           internal frame where child components are normally inserted.
	 */
	 ...
	 *                <code>null</code>
	 * @see RootPaneContainer#setLayeredPane
	 * @beaninfo hidden: true bound: true description: The pane which holds the
	 *           various desktop layers.
	 */
	...
	/**
	 * Sets the <code>rootPane</code> property for this
	 ...
	 * 
	 * @return <code>true</code> if this internal frame is closed,
	 ...
	 * <code>closed</code> property to <code>true</code> unless a listener
	...
	public void setClosed(boolean b) throws PropertyVetoException {
		...
		if (isClosed) {
			...
			/*
			 * this bogus -- we haven't defined what setClosed(false) means.
			 */
	...
	/**
	 * Sets whether the <code>JInternalFrame</code> can be resized by some user
	 * action.
	 *
	 * @param b
	...
	/**
	 * Returns whether the <code>JInternalFrame</code> can be resized by some
	 * user action.
	 *
	 * @return <code>true</code> if this internal frame can be resized,
	 *         <code>false</code> otherwise
	 */
	...
	/**
	 * Sets the <code>iconable</code> property, which must be <code>true</code>
	 * for the user to be able to make the <code>JInternalFrame</code> an icon.
	 * Some look and feels might not implement iconification; they will ignore
	 * this property.
	 *
	 * @param b
	 *            a boolean, where <code>true</code> means this internal frame
	...
	/**
	 * Gets the <code>iconable</code> property, which by default is
	 * <code>false</code>.
	 *
	...
	/**
	 * Iconifies or de-iconifies this internal frame, if the look and feel
	 * supports iconification. If the internal frame's state changes to
	 * iconified, this method fires an <code>INTERNAL_FRAME_ICONIFIED</code>
	 * event. If the state changes to de-iconified, an
	...
	public void setIcon(boolean b) throws PropertyVetoException {
		 ...
		 * possible so that we can place the icon in its proper place on the
		 * desktop. I am not sure the call to validate() is necessary, since we
		 * are not going to display this frame yet
		 */
	...
	/**
	 * Sets the <code>maximizable</code> property, which determines whether the
	 * <code>JInternalFrame</code> can be maximized by some user action. Some
	 * look and feels might not support maximizing internal frames; they will
	 ...
	 *
	 * @return <code>true</code> if this internal frame is maximized,
	 *         <code>false</code> otherwise
	 */
	...
	/**
	 * Maximizes and restores this internal frame. A maximized frame is resized
	 * to fully fit the <code>JDesktopPane</code> area associated with the
	 * <code>JInternalFrame</code>. A restored frame's size is set to the
	...
	public void setMaximum(boolean b) throws PropertyVetoException {
		...
		/*
		 * setting isMaximum above the event firing means that property
		 * listeners that, for some reason, test it will get it wrong... See,
		 * for example, getNormalBounds()
		 */
		isMaximum = b;
	 ...
	 * <code>JInternalFrame</code> normally draws its title bar differently if
	 * it is the selected frame, which indicates to the user that this internal
	 * frame has the focus. When this method changes the state of the internal
	 * frame from deselected to selected, it fires an
	 * <code>InternalFrameEvent.INTERNAL_FRAME_ACTIVATED</code> event. If the
	 * change is from selected to deselected, an
	 * <code>InternalFrameEvent.INTERNAL_FRAME_DEACTIVATED</code> event is
	 * fired.
	 *
	 * @param selected
	 *            a boolean, where <code>true</code> means this internal frame
	...
	public void setSelected(boolean selected) throws PropertyVetoException {
		...
		/*
		 * We don't want to leave focus in the previously selected frame, so we
		 * have to set it to *something* in case it doesn't get set in some
		 * other way (as if a user clicked on a component that doesn't request
		 * focus). If this call is happening because the user clicked on a
		 * component that will want focus, then it will get transfered there
		 * later.
		 * 
		 * We test for parent.isShowing() above, because AWT throws a NPE if you
		 * try to request focus on a lightweight before its parent has been made
		 * visible
		 */
	...
	/**
	 * Returns whether the <code>JInternalFrame</code> is the currently
	 * "selected" or active frame.
	 *
	 * @return <code>true</code> if this internal frame is currently selected
	 *         (active)
	 * @see #setSelected
	 */
	 ...
	 * <code>setCursor</code> method that is not a resizable <code>Cursor</code>
	 * .
	 *
	 ...
	 *
	 * @param layer
	 *            an <code>Integer</code> object specifying this frame's desktop
	 *            layer
	 * @see JLayeredPane
	 ...
	 * method <code>setLayer(Integer)</code> should be used for layer values
	 * predefined in <code>JLayeredPane</code>. When using
	 * <code>setLayer(int)</code>, care must be taken not to accidentally clash
	 * with those values.
	 *
	 * @param layer
	 ...
	 *
	 * @return an <code>Integer</code> object specifying this frame's desktop
	 *         layer
	 * @see JLayeredPane
	 */
	public int getLayer() {
	 ...
	 * <code>JDesktop</code> instance. If <code>JInternalFrame</code> finds
	 * none, the <code>desktopIcon</code> tree is searched.
	 *
	 * @return the <code>JDesktopPane</code> this internal frame belongs to, or
	 ...
	 *
	 * @return a <code>Rectangle</code> containing the bounds of this frame when
	 *         in the normal state
	 * @since 1.3
	 */
	public Rectangle getNormalBounds() {
		...
		/*
		 * we used to test (!isMaximum) here, but since this method is used by
		 * the property listener for the IS_MAXIMUM_PROPERTY, it ended up
		 * getting the wrong answer... Since normalBounds get set to null when
		 * the frame is restored, this should work better
		 */
	...
	/**
	 * Sets the normal bounds for this internal frame, the bounds that this
	 * internal frame would be restored to from its maximized state. This method
	...
	/**
	 * If this <code>JInternalFrame</code> is active, returns the child that has
	 * focus. Otherwise, returns <code>null</code>.
	 *
	 * @return the component with focus, or <code>null</code> if no children
	 *         have focus
	 * @since 1.3
	 */
	...
	/**
	 * Returns the child component of this <code>JInternalFrame</code> that will
	 * receive the focus when this <code>JInternalFrame</code> is selected. If
	 * this <code>JInternalFrame</code> is currently selected, this method
	 * returns the same component as the <code>getFocusOwner</code> method. If
	 * this <code>JInternalFrame</code> is not selected, then the child
	 * component that most recently requested focus will be returned. If no
	 * child component has ever requested focus, then this
	 * <code>JInternalFrame</code>'s initial focusable component is returned. If
	 * no such child exists, then this <code>JInternalFrame</code>'s default
	 * component to focus is returned.
	 *
	...
	/**
	 * Requests the internal frame to restore focus to the last subcomponent
	 * that had focus. This is used by the UI when the user selected this
	 * internal frame -- for example, by clicking on the title bar.
	 *
	 * @since 1.3
	 */
	...
	/**
	 * Moves and resizes this component. Unlike other components, this
	 * implementation also forces re-layout, so that frame decorations such as
	...
	/**
	 * Adds the specified listener to receive internal frame events from this
	 * internal frame.
	 *
	 * @param l
	...
	/**
	 * Returns an array of all the <code>InternalFrameListener</code>s added to
	 * this <code>JInternalFrame</code> with
	...
	protected void fireInternalFrameEvent(int id) {
		...
		/*
		 * we could do it off the event, but at the moment, that's not how I'm
		 * implementing it
		 */
	 ...
	 * "close" on this internal frame. The possible choices are: <br>
	 * <br>
	 * <dl>
	 * <dt><code>DO_NOTHING_ON_CLOSE</code>
	 * <dd>Do nothing. This requires the program to handle the operation in the
	 * <code>internalFrameClosing</code> method of a registered
	...
	/**
	 * Causes subcomponents of this <code>JInternalFrame</code> to be laid out
	 * at their preferred size. Internal frames that are iconized or maximized
	 * are first restored and then packed. If the internal frame is unable to be
	 * restored its state is not changed and will not be packed.
	 *
	 ...
	 * front, makes it visible, and attempts to select it. The first time the
	 * internal frame is made visible, this method also fires an
	 * <code>INTERNAL_FRAME_OPENED</code> event. This method does nothing if the
	 * internal frame is already visible. Invoking this method has the same
	 * result as invoking <code>setVisible(true)</code>.
	 *
	 * @see #moveToFront
	 * @see #setSelected
	...
	public void show() {
		...
		/*
		 * icon default visibility is false; set it to true so that it shows up
		 * when user iconifies frame
		 */
	...
	/**
	 * Makes this internal frame invisible, unselected, and closed. If the frame
	 * is not already closed, this method fires an
	 * <code>INTERNAL_FRAME_CLOSED</code> event. The results of invoking this
	 * method are similar to <code>setClosed(true)</code>, but
	 * <code>dispose</code> always succeeds in closing the internal frame and
	 * does not fire an <code>INTERNAL_FRAME_CLOSING</code> event.
	 *
	...
	/**
	 * Brings this internal frame to the front. Places this internal frame at
	 * the top of the stacking order and makes the corresponding adjustment to
	 * other visible internal frames.
	 *
	...
	/**
	 * Sends this internal frame to the back. Places this internal frame at the
	 * bottom of the stacking order and makes the corresponding adjustment to
	 * other visible internal frames.
	 *
	...
	/**
	 * Does nothing because <code>JInternalFrame</code>s must always be roots of
	...
	/**
	 * Always returns <code>true</code> because all <code>JInternalFrame</code>s
	 * must be roots of a focus traversal cycle.
	 *
	 ...
	 * must always be roots of a focus traversal cycle.
	 *
	...
	/**
	 * Gets the warning string that is displayed with this internal frame. Since
	 * an internal frame is always secure (since it's fully contained within a
	 * window that might need a warning string) this method always returns
	 * <code>null</code>.
	 * 
	 * @return <code>null</code>
	...
	/**
	 * Returns a string representation of this <code>JInternalFrame</code>. This
	...
	protected String paramString() {
		...
		return super.paramString() + ",closable=" + closableString + ",defaultCloseOperation="
				...
				+ frameIconString + ",iconable=" + iconableString + ",isClosed=" + isClosedString
				+ ",isIcon=" + isIconString + ",isMaximum=" + isMaximumString + ",isSelected="
				+ isSelectedString + ",maximizable=" + maximizableString + ",opened=" + openedString
				+ ",resizable=" + resizableString + ",rootPane=" + rootPaneString
				+ ",rootPaneCheckingEnabled=" + rootPaneCheckingEnabledString + ",title="
				+ titleString;
	}
	 ...
	 * <code>JInternalFrame</code>. For internal frames, the
	 * <code>AccessibleContext</code> takes the form of an
	 * <code>AccessibleJInternalFrame</code> object. A new
	 * <code>AccessibleJInternalFrame</code> instance is created if necessary.
	 *
	...
	protected class AccessibleJInternalFrame extends AccessibleJComponent
		 ...
		 * implementation of the Java Accessibility API for this class, returns
		 ...
		 *
		 * @return value of the object -- can be <code>null</code> if this
		 ...
		 *
		 * @return Minimum value of the object; <code>null</code> if this object
		 ...
		 *
		 * @return Maximum value of the object; <code>null</code> if this object
	 ...
	 * <code>JInternalFrame</code>. This API should NOT BE USED by Swing
	 * applications, as it will go away in future versions of Swing as its
	 * functionality is moved into <code>JInternalFrame</code>. This class is
	 * public only so that UI objects can display a desktop icon. If an
	 * application wants to display a desktop icon, it should create a
	...
	static public class JDesktopIcon extends JComponent implements Accessible {
		 ...
		 * Returns the look-and-feel object that renders this component.
		 *
		 * @return the <code>DesktopIconUI</code> object that renders this
		 *         component
		 */
		 ...
		 *
		 * @return the <code>JInternalFrame</code> with which this icon is
		 *         associated
		 */
		 ...
		 *
		 * @return the <code>JDesktopPane</code> that contains this icon's
		 *         internal frame, or <code>null</code> if none found
		 */
		public JDesktopPane getDesktopPane() {
		...
		public void updateUI() {
			...
			if (internalFrame != null && internalFrame.getUI() != null) { // don't
																			// do
																			// this
																			// if
																			// UI
		...
		/**
		 * Returns the name of the look-and-feel class that renders this
		 * component.
		 *
		...
		/**
		 * Gets the AccessibleContext associated with this JDesktopIcon. For
		 * desktop icons, the AccessibleContext takes the form of an
		 * AccessibleJDesktopIcon. A new AccessibleJDesktopIcon instance is
		 ...
		 * implementation of the Java Accessibility API appropriate to desktop
		 * icon user-interface elements.
		 * <p>
		 * <strong>Warning:</strong> Serialized objects of this class will not
		 * be compatible with future Swing releases. The current serialization
		 * support is appropriate for short term storage or RMI between
		 * applications running the same version of Swing. As of 1.4, support
		 * for long term storage of all JavaBeans&trade; has been added to the
		...
		protected class AccessibleJDesktopIcon extends AccessibleJComponent
			 ...
			 *
			 * @return minimum value of the object; <code>null</code> if this
			 ...
			 *
			 * @return maximum value of the object; <code>null</code> if this
