 ...
 * <p>
 * Please see
 * <a href="https://docs.oracle.com/javase/tutorial/uiswing/dnd/index.html"> How
...
public class TransferHandler implements Serializable {
    ...
    /**
     * An <code>int</code> representing a &quot;copy&quot; transfer action. This
     * value is used when data is copied to a clipboard or copied elsewhere in a
     * drag and drop operation.
     */
    public static final int COPY = DnDConstants.ACTION_COPY;

    /**
     * An <code>int</code> representing a &quot;move&quot; transfer action. This
     * value is used when data is moved to a clipboard (i.e. a cut) or moved
    ...
    /**
     * An <code>int</code> representing a &quot;link&quot; transfer action. This
     * value is used to specify that data should be linked in a drag and drop
     * operation.
     *
    ...
    /**
     * Represents a location where dropped data should be inserted. This is a
     * base class that only encapsulates a point. Components supporting drop may
     * provide subclasses of this containing more information.
     * <p>
     * Developers typically shouldn't create instances of, or extend, this
     * class. Instead, these are something provided by the DnD implementation by
     * <code>TransferSupport</code> instances and by components with a
     * <code>getDropLocation()</code> method.
     *
    ...
    public static class DropLocation {
        ...
        /**
         * Returns the drop point, representing the mouse's current location
         * within the component.
         *
    ...
    /**
     * This class encapsulates all relevant details of a clipboard or drag and
     * drop transfer, and also allows for customizing aspects of the drag and
     * drop experience.
     * <p>
     * The main purpose of this class is to provide the information needed by a
     * developer to determine the suitability of a transfer or to import the
     * data contained within. But it also doubles as a controller for
     * customizing properties during drag and drop, such as whether or not to
     * show the drop location, and which drop action to use.
     * <p>
     * Developers typically need not create instances of this class. Instead,
     * they are something provided by the DnD implementation to certain methods
     * in <code>TransferHandler</code>.
     *
    ...
    public final static class TransferSupport {
         ...
         * {@code DropTargetDropEvent} for drops, and a {@code Transferable}
         * otherwise
         */
        ...
        private void setDNDVariables(Component component, DropTargetEvent event) {
            ...
            Point p = source instanceof DropTargetDragEvent ? ((DropTargetDragEvent) source).getLocation()
                    : ((DropTargetDropEvent) source).getLocation();

            if (SunToolkit.isInstanceOf(component, "javax.swing.text.JTextComponent")) {
                dropLocation = SwingAccessor.getJTextComponentAccessor().dropLocationForPoint(
                        (JTextComponent) component, p);
        ...
        /**
         * Returns whether or not this <code>TransferSupport</code> represents a
         * drop operation.
         *
        ...
        /**
         * Returns the current (non-{@code null}) drop location for the
         * component, when this {@code TransferSupport} represents a drop.
         * <p>
         * Note: For components with built-in drop support, this location will
         * be a subclass of {@code DropLocation} of the same type returned by
         * that component's {@code getDropLocation} method.
         * <p>
         * This method is only for use with drag and drop transfers. Calling it
         * when {@code isDrop()} is {@code false} results in an
        ...
        public DropLocation getDropLocation() {
            ...
            if (dropLocation == null) {
                /*
                 * component didn't give us a custom drop location, so lazily
                 * create a point-only location
                 */
                Point p = source instanceof DropTargetDragEvent ? ((DropTargetDragEvent) source).getLocation()
                        : ((DropTargetDropEvent) source).getLocation();
         ...
         * those components that automatically show the drop location when
         * appropriate during a drag and drop operation). By default, the drop
         * location is shown only when the {@code TransferHandler} has said it
         * can accept the import represented by this {@code TransferSupport}.
         * With this method you can force the drop location to always be shown,
         * or always not be shown.
         * <p>
         * This method is only for use with drag and drop transfers. Calling it
         * when {@code isDrop()} is {@code false} results in an
         * {@code IllegalStateException}.
         ...
         * to the given action, instead of the default user drop action. The
         * action must be supported by the source's drop actions, and must be
         * one of {@code COPY}, {@code MOVE} or {@code LINK}.
         * <p>
         * This method is only for use with drag and drop transfers. Calling it
         * when {@code isDrop()} is {@code false} results in an
         * {@code IllegalStateException}.
         ...
         * <p>
         * Unless explicitly chosen by way of {@code setDropAction}, this
         * returns the user drop action provided by {@code getUserDropAction}.
         * <p>
         * You may wish to query this in {@code TransferHandler}'s
         * {@code importData} method to customize processing based on the
         * action.
         * <p>
         * This method is only for use with drag and drop transfers. Calling it
         * when {@code isDrop()} is {@code false} results in an
         ...
         * {@link java.awt.dnd.DropTargetDropEvent}. A different action may be
         * chosen as the drop action by way of the {@code setDropAction} method.
         * <p>
         * You may wish to query this in {@code TransferHandler}'s
         * {@code canImport} method when determining the suitability of a drop
         * or when deciding on a drop action to explicitly choose.
         * <p>
         * This method is only for use with drag and drop transfers. Calling it
         * when {@code isDrop()} is {@code false} results in an
         * {@code IllegalStateException}.
        ...
        public int getUserDropAction() {
            ...
            return (source instanceof DropTargetDragEvent) ? ((DropTargetDragEvent) source).getDropAction()
         ...
         * combination of {@code COPY}, {@code MOVE} and {@code LINK}. You may
         * wish to query this in {@code TransferHandler}'s {@code canImport}
         * method when determining the suitability of a drop or when deciding on
         * a drop action to explicitly choose. To determine if a particular
         * action is supported by the source, bitwise-AND the action with the
         * source drop actions, and then compare the result against the original
         * action. For example:
         * 
         * <pre>
        ...
        public int getSourceDropActions() {
            ...
            return (source instanceof DropTargetDragEvent) ? ((DropTargetDragEvent) source).getSourceActions()
     ...
     * When performed, this action operates on the {@code JComponent} source of
     * the {@code ActionEvent} by invoking {@code exportToClipboard}, with a
     * {@code MOVE} action, on the component's {@code TransferHandler}.
     *
     ...
     * When performed, this action operates on the {@code JComponent} source of
     * the {@code ActionEvent} by invoking {@code exportToClipboard}, with a
     * {@code COPY} action, on the component's {@code TransferHandler}.
     *
     ...
     * source of the {@code ActionEvent} by invoking {@code importData}, with
     * the clipboard contents, on the component's {@code TransferHandler}.
     *
     ...
     *
     * @see java.awt.dnd.DragGestureEvent#startDrag(Cursor dragCursor, Image
     *      dragImage, Point imageOffset, Transferable transferable,
    ...
    /**
     * Sets the drag image parameter. The image has to be prepared for rendering
     * by the moment of the call. The image is stored by reference because of
    ...
    /**
     * Returns the drag image. If there is no image to drag, the returned value
     * is {@code null}.
     *
     ...
     *
     * @return a {@code Point} object that corresponds to coordinates of an
     *         anchor offset of the image relative to the upper left corner of
     *         the image. The point {@code (0,0)} returns by default.
     */
    ...
    /**
     * Causes the Swing drag support to be initiated. This is called by the
     * various UI implementations in the <code>javax.swing.plaf.basic</code>
     * package if the dragEnabled property is set on the component. This can be
     * called by custom UI implementations to use the Swing drag support. This
     * method can also be called by a Swing extension written as a subclass of
     * <code>JComponent</code> to take advantage of the Swing drag support.
     * <p>
     ...
     * <code>java.awt.dnd</code> mechanism, requiring no further effort from the
     * developer. The <code>exportDone</code> method will be called when the
     ...
     * transfer <em>will</em> be complete and the <code>exportDone</code> method
     * will be called with the action that occurred, before this method returns.
     * Should the clipboard be unavailable when attempting to place data on it,
     * the <code>IllegalStateException</code> thrown by
     * {@link Clipboard#setContents(Transferable, ClipboardOwner)} will be
     * propagated through this method. However, <code>exportDone</code> will
     ...
     * The <code>Transferable</code> to be imported and the component to
     * transfer to are contained within the <code>TransferSupport</code>.
     * <p>
     * While the drag and drop implementation calls {@code canImport} to
     * determine the suitability of a transfer before calling this method, the
     * implementation of paste does not. As such, it cannot be assumed that the
     * transfer is acceptable upon a call to this method for paste. It is
     * recommended that {@code canImport} be explicitly called to cover this
     * case.
     * <p>
     * Note: The <code>TransferSupport</code> object passed to this method is
     * only valid for the duration of the method call. It is undefined what
    ...
    public boolean importData(TransferSupport support) {
        return support.getComponent() instanceof JComponent ? importData((JComponent) support.getComponent(),
                support.getTransferable()) : false;
    }
     ...
     * <p>
     * Note: Swing now calls the newer version of <code>importData</code> that
     * takes a <code>TransferSupport</code>, which in turn calls this method (if
     * the component in the {@code TransferSupport} is a {@code JComponent}).
     * Developers are encouraged to call and override the newer version as it
     * provides more information (and is the only version that supports use with
     * a {@code TransferHandler} set directly on a {@code JFrame} or other non-
     * {@code JComponent}).
     *
     * @param comp
     ...
     * acceptability of transfers; with a return value of {@code true}
     ...
     * drag and drop, accepting the transfer, by default, tells them to show the
     * drop location. This can be changed by calling {@code setShowDropLocation}
     * on the {@code TransferSupport}.
     * <p>
     ...
     * this and choose a different action, from the supported source actions, by
     * calling {@code setDropAction} on the {@code TransferSupport}.
     * <p>
     * On every call to {@code canImport}, the {@code TransferSupport} contains
     * fresh state. As such, any properties set on it must be set on every call.
     * Upon a drop, {@code canImport} is called one final time before calling
     * into {@code importData}. Any state set on the {@code TransferSupport}
     * during that last call will be available in {@code importData}.
     * <p>
     * This method is not called internally in response to paste operations. As
     * such, it is recommended that implementations of {@code importData}
     * explicitly call this method for such cases and that this method be
     * prepared to return the suitability of paste operations as well.
     * <p>
     * Note: The <code>TransferSupport</code> object passed to this method is
     * only valid for the duration of the method call. It is undefined what
     * values it may contain after this method returns.
    ...
    public boolean canImport(TransferSupport support) {
        return support.getComponent() instanceof JComponent ? canImport((JComponent) support.getComponent(),
                support.getDataFlavors()) : false;
    }
     ...
     * <p>
     * Note: Swing now calls the newer version of <code>canImport</code> that
     * takes a <code>TransferSupport</code>, which in turn calls this method
     * (only if the component in the {@code TransferSupport} is a
     * {@code JComponent}). Developers are encouraged to call and override the
     * newer version as it provides more information (and is the only version
     * that supports use with a {@code TransferHandler} set directly on a
    ...
    /**
     * Returns the type of transfer actions supported by the source; any
     * bitwise-OR combination of {@code COPY}, {@code MOVE} and {@code LINK}.
     * <p>
     * Some models are not mutable, so a transfer operation of {@code MOVE}
     * should not be advertised in that case. Returning {@code NONE} disables
    ...
    /**
     * Returns an object that establishes the look of a transfer. This is useful
     * for both providing feedback while performing a drag operation and for
     ...
     * not alter the graphics clip or alpha level. The icon implementation need
     * not be rectangular or paint all of the bounding rectangle and logic that
     * calls the icons paint method should not assume the all bits are painted.
     * <code>null</code> is a valid return value for this method and indicates
     * there is no visual representation provided. In that case, the calling
     * logic is free to represent the transferable however it wants.
     * <p>
     ...
     * that was transferred if the action was <code>MOVE</code>.
     * <p>
     * This method is implemented to do nothing since <code>MOVE</code> is not a
     * supported action of this implementation (<code>getSourceActions</code>
     ...
     * transfer handler on the given component (transfer handler may be shared).
     * This returns <code>null</code> if the property descriptor can't be found
    ...
    /**
     * Fetches the data flavor from the array of possible flavors that has data
     * of the type represented by property type. Null is returned if there is no
     * match.
     */
    private DataFlavor getPropertyDataFlavor(Class<?> k, DataFlavor[] flavors) {
        for (int i = 0; i < flavors.length; i++) {
            DataFlavor flavor = flavors[i];
            if ("application".equals(flavor.getPrimaryType()) && "x-java-jvm-local-objectref".equals(flavor
                    .getSubType()) && k.isAssignableFrom(flavor.getRepresentationClass())) {
    ...
    static class PropertyTransferable implements Transferable {
        ...
        /**
         * Returns an array of <code>DataFlavor</code> objects indicating the
         * flavors the data can be provided in. The array should be ordered
         * according to preference for providing the data (from most richly
        ...
        public boolean isDataFlavorSupported(DataFlavor flavor) {
            Class<?> propertyType = property.getPropertyType();
            if ("application".equals(flavor.getPrimaryType()) && "x-java-jvm-local-objectref".equals(flavor
                    .getSubType()) && flavor.getRepresentationClass().isAssignableFrom(propertyType)) {
    ...
    /**
     * This is the default drop target for drag and drop operations if one isn't
     * provided by the developer. <code>DropTarget</code> only supports one
     * <code>DropTargetListener</code> and doesn't function properly if it isn't
     * set. This class sets the one listener as the linkage of drop handling to
     * the <code>TransferHandler</code>, and adds support for additional
     * listeners which some of the <code>ComponentUI</code> implementations
     * install to manipulate a drop insertion location.
     */
    static class SwingDropTarget extends DropTarget implements UIResource {
        ...
        public void dragExit(DropTargetEvent e) {
            ...
            if (!isActive()) {
                // If the Drop target is inactive the dragExit will not be
                // dispatched to the dtListener,
    ...
    private static class DropHandler implements DropTargetListener, Serializable, ActionListener {
         ...
         * maintained as a pair of rectangles. The region can cause a scroll if
         * the pointer sits inside it for the duration of the timer. The region
         * that causes the timer countdown is the area between the two
         * rectangles.
         * <p>
         * This is implemented to use the visible area of the component as the
         * outer rectangle, and the insets are fixed at 10. Should the component
         * be smaller than a total of 20 in any direction, autoscroll will not
         ...
         * cursor is in a corner of the autoscroll region, more than one axis
         * will scroll.
         */
        ...
        private void setComponentDropLocation(TransferSupport support, boolean forDrop) {
            ...
            if (SunToolkit.isInstanceOf(component, "javax.swing.text.JTextComponent")) {
                state = SwingAccessor.getJTextComponentAccessor().setDropLocation((JTextComponent) component,
        ...
        private void handleDrag(DropTargetDragEvent e) {
            ...
            boolean showLocation = support.showDropLocationIsSet ? support.showDropLocation : canImport;
    ...
    static class TransferAction extends UIAction implements UIResource {
        ...
        public boolean isEnabled(Object sender) {
            if (sender instanceof JComponent && ((JComponent) sender).getTransferHandler() == null) {
                return false;
            }
        ...
        public void actionPerformed(final ActionEvent e) {
            ...
            final AccessControlContext srcAcc = AWTAccessor.getComponentAccessor().getAccessControlContext(
                    (Component) src);
            final AccessControlContext eventAcc = AWTAccessor.getAWTEventAccessor().getAccessControlContext(
                    e);
