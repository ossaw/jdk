 ...
 * <p>
 * Each <code>SynthStyle</code> has a set of <code>ColorType</code>s that are
 * accessed by way of the {@link SynthStyle#getColor(SynthContext, ColorType)}
 * method. <code>SynthStyle</code>'s <code>installDefaults</code> will install
 * the <code>FOREGROUND</code> color as the foreground of the Component, and the
 * <code>BACKGROUND</code> color to the background of the component (assuming
 * that you have not explicitly specified a foreground and background color).
 * Some components support more color based properties, for example
 * <code>JList</code> has the property <code>selectionForeground</code> which
 * will be mapped to <code>FOREGROUND</code> with a component state of
 * <code>SynthConstants.SELECTED</code>.
 * <p>
 * The following example shows a custom <code>SynthStyle</code> that returns a
 * red Color for the <code>DISABLED</code> state, otherwise a black color.
 * 
 * <pre>
