...
class ImagePainter extends SynthPainter {
    private static final StringBuffer CACHE_KEY = new StringBuffer(
            "SynthCacheKey");
    ...
    ImagePainter(boolean tiles, boolean paintCenter, Insets sourceInsets,
            Insets destinationInsets, URL path, boolean center) {
    ...
    private void paint(SynthContext context, Graphics g, int x, int y, int w,
        ...
        if (Paint9Painter.validImage(image)) {
            ...
            imageCache.paint(context.getComponent(), g, x, y, w, h, image,
                    sInsets, dInsets, type, mask);
        }
    }
