...
public class SynthGraphicsUtils {
	 ...
	 * @param paintKey
	 *            Identifies the portion of the component being asked to paint,
	 *            may be null.
	 * @param g
	 ...
	 *            y destination
	 */
	public void drawLine(SynthContext context, Object paintKey, Graphics g, int x1, int y1, int x2,
			int y2) {
	 ...
	 * <p>
	 * Specifying <code>null</code> or any key different from
	...
	/**
	 * Lays out text and an icon returning, by reference, the location to place
	 * the icon and text.
	 *
	 * @param ss
	 *            SynthContext
	...
	public String layoutText(SynthContext ss, FontMetrics fm, String text, Icon icon, int hAlign,
		...
		if (icon instanceof SynthIcon) {
			...
			String formattedText = SwingUtilities.layoutCompoundLabel(ss.getComponent(), fm, text,
					wrapper, vAlign, hAlign, vTextPosition, hTextPosition, viewR, iconR, textR,
					iconTextGap);
	...
	public Dimension getMinimumSize(SynthContext ss, Font font, String text, Icon icon, int hAlign,
		...
		Dimension size = getPreferredSize(ss, font, text, icon, hAlign, vAlign, hTextPosition,
				vTextPosition, iconTextGap, mnemonicIndex);
	...
	public Dimension getPreferredSize(SynthContext ss, Font font, String text, Icon icon,
		...
		if (icon == null && (text == null || font == null)) {
			...
			layoutText(ss, fm, text, icon, hAlign, vAlign, hTextPosition, vTextPosition, viewR,
	...
	public void paintText(SynthContext ss, Graphics g, String text, Icon icon, int hAlign,
		...
		String clippedText = layoutText(ss, fm, text, icon, hAlign, vAlign, hTextPosition,
				vTextPosition, paintViewR, paintIconR, paintTextR, iconTextGap);
		...
		if (icon != null) {
			...
			SynthIcon.paintIcon(icon, ss, g, paintIconR.x, paintIconR.y, paintIconR.width,
					paintIconR.height);
			g.setColor(color);
		}
	 ...
	 * (from top to bottom) and ask each for its preferred size. Each menu item
	 * will add up the max width of each element (icons, text, accelerator
	 * spacing, accelerator text or arrow icon) encountered thus far, so by the
	 * time all menu items have been calculated, we will know the maximum
	 * (preferred) menu item size for that popup menu. Later when it comes time
	 * to paint each menu item, we can use those same accumulated max element
	 * sizes in order to layout the item.
	 */
	static Dimension getPreferredMenuItemSize(SynthContext context, SynthContext accContext,
			JComponent c, Icon checkIcon, Icon arrowIcon, int defaultTextIconGap,
			String acceleratorDelimiter, boolean useCheckAndArrow, String propertyPrefix) {

		JMenuItem mi = (JMenuItem) c;
		SynthMenuItemLayoutHelper lh = new SynthMenuItemLayoutHelper(context, accContext, mi,
				checkIcon, arrowIcon, MenuItemLayoutHelper.createMaxRect(), defaultTextIconGap,
				acceleratorDelimiter, SynthLookAndFeel.isLeftToRight(mi), useCheckAndArrow,
				propertyPrefix);
	...
	static void paint(SynthContext context, SynthContext accContext, Graphics g, Icon checkIcon,
			Icon arrowIcon, String acceleratorDelimiter, int defaultTextIconGap,
		...
		SynthMenuItemLayoutHelper lh = new SynthMenuItemLayoutHelper(context, accContext, mi,
				checkIcon, arrowIcon, viewRect, defaultTextIconGap, acceleratorDelimiter,
				leftToRight, MenuItemLayoutHelper.useCheckAndArrow(mi), propertyPrefix);
	...
	static void paintIcon(Graphics g, SynthMenuItemLayoutHelper lh,
		...
		if (lh.getIcon() != null) {
			...
			if (icon != null) {
				...
				SynthIcon.paintIcon(icon, lh.getContext(), g, iconRect.x, iconRect.y,
	...
	static void paintAccText(Graphics g, SynthMenuItemLayoutHelper lh,
		...
		if (accText != null && !accText.equals("")) {
			...
			lh.getAccGraphicsUtils().paintText(lh.getAccContext(), g, accText, lr.getAccRect().x,
					lr.getAccRect().y, -1);
		}
	}
	...
	/**
	 * Wraps a SynthIcon around the Icon interface, forwarding calls to the
	 * SynthIcon with a given SynthContext.
	 */
	private static class SynthIconWrapper implements Icon {
		private static final java.util.List<SynthIconWrapper> CACHE = new java.util.ArrayList<SynthIconWrapper>(
				1);
