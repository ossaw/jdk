...
public class MetalTreeUI extends BasicTreeUI {
    ...
    protected boolean isLocationInExpandControl(int row, int rowLevel, int mouseX, int mouseY) {
        if (tree != null && !isLeaf(row)) {
            ...
            boxLeftX += (((rowLevel + depthOffset - 1) * totalChildIndent) + getLeftChildIndent()) - boxWidth
                    / 2;
    ...
    protected void paintHorizontalSeparators(Graphics g, JComponent c) {
        ...
        int endRow = getRowForPath(tree, getClosestPathForLocation(tree, 0, clipBounds.y + clipBounds.height
                - 1));
