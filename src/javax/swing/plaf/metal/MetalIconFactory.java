...
public class MetalIconFactory implements Serializable {
	...
	public static class FolderIcon16 implements Icon, Serializable {
		...
		public void paintIcon(Component c, Graphics g, int x, int y) {
			...
			if (image == null) {
				if (gc != null) {
					image = gc.createCompatibleImage(getIconWidth(), getIconHeight(),
	...
	private static class VerticalSliderThumbIcon implements Icon, Serializable, UIResource {
		...
		public VerticalSliderThumbIcon() {
			controlBumps = new MetalBumps(6, 10, MetalLookAndFeel.getControlHighlight(),
					MetalLookAndFeel.getControlInfo(), MetalLookAndFeel.getControl());
			primaryBumps = new MetalBumps(6, 10, MetalLookAndFeel.getPrimaryControl(),
	...
	private static class HorizontalSliderThumbIcon implements Icon, Serializable, UIResource {
		...
		public HorizontalSliderThumbIcon() {
			controlBumps = new MetalBumps(10, 6, MetalLookAndFeel.getControlHighlight(),
					MetalLookAndFeel.getControlInfo(), MetalLookAndFeel.getControl());
			primaryBumps = new MetalBumps(10, 6, MetalLookAndFeel.getPrimaryControl(),
					MetalLookAndFeel.getPrimaryControlDarkShadow(),
					MetalLookAndFeel.getPrimaryControlShadow());
		}
	...
	private static class OceanVerticalSliderThumbIcon extends CachedPainter
		...
		public void paintIcon(Component c, Graphics g, int x, int y) {
			...
			paint(c, g, x, y, getIconWidth(), getIconHeight(), MetalUtils.isLeftToRight(c),
					c.hasFocus(), c.isEnabled(), MetalLookAndFeel.getCurrentTheme());
		}

		protected void paintToImage(Component c, Image image, Graphics g2, int w, int h,
				Object[] args) {
	...
	private static class OceanHorizontalSliderThumbIcon extends CachedPainter
		...
		static {
			THUMB_SHAPE = new Polygon(new int[] { 0, 14, 14, 7, 0 }, new int[] { 0, 0, 8, 15, 8 },
					5);
		}
