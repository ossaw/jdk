...
/**
 * Metal implementation of <code>MenuBarUI</code>. This class is responsible for
 * providing the metal look and feel for <code>JMenuBar</code>s.
 *
		...
		if (isOpaque && (c.getBackground() instanceof UIResource)
			...
			if (MetalToolBarUI.doesMenuBarBorderToolBar((JMenuBar) c)) {
				...
				if (tb.isOpaque() && tb.getBackground() instanceof UIResource) {
					MetalUtils.drawGradient(c, g, "MenuBar.gradient", 0, 0, c.getWidth(),
							c.getHeight() + tb.getHeight(), true);
					paint(g, c);
					return;
				}
			}
			MetalUtils.drawGradient(c, g, "MenuBar.gradient", 0, 0, c.getWidth(), c.getHeight(),
					true);
			paint(g, c);
		} else {
			super.update(g, c);
