...
/**
 * {@code MetalTheme} provides the color palette and fonts used by the Java Look
 * and Feel.
 * <p>
 * {@code MetalTheme} is abstract, see {@code DefaultMetalTheme} and
 * {@code OceanTheme} for concrete implementations.
 * <p>
 * {@code MetalLookAndFeel} maintains the current theme that the the
 * {@code ComponentUI} implementations for metal use. Refer to
 * {@link MetalLookAndFeel#setCurrentTheme
 * MetalLookAndFeel.setCurrentTheme(MetalTheme)} for details on changing the
 * current theme.
 * <p>
 * {@code MetalTheme} provides a number of public methods for getting colors.
 * These methods are implemented in terms of a handful of protected abstract
 * methods. A subclass need only override the protected abstract methods (
 * {@code getPrimary1}, {@code getPrimary2}, {@code getPrimary3},
 * {@code getSecondary1}, {@code getSecondary2}, and {@code getSecondary3});
 * although a subclass may override the other public methods for more control
 * over the set of colors that are used.
 * <p>
 * Concrete implementations of {@code MetalTheme} must return {@code non-null}
 * values from all methods. While the behavior of returning {@code null} is not
 * specified, returning {@code null} will result in incorrect behavior.
 * <p>
...
public abstract class MetalTheme {
     ...
     */
    protected abstract ColorUIResource getPrimary1(); // these are blue in Metal
                                                      // Default Theme
