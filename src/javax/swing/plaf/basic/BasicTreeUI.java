...
public class BasicTreeUI extends TreeUI {
			...
			while (!done && paintingEnumerator.hasMoreElements()) {
				...
				if (path != null) {
					...
					if (parentPath != null) {
						...
						paintHorizontalPartOfLeg(g, paintBounds, insets, bounds, path, row,
					...
					paintRow(g, paintBounds, insets, bounds, path, row, isExpanded, hasBeenExpanded,
							isLeaf);
	...
	protected Rectangle getDropLineRect(JTree.DropLocation loc) {
		...
		if (tree.getRowCount() == 0) {
			rect = new Rectangle(insets.left, insets.top,
					tree.getWidth() - insets.left - insets.right, 0);
		} else {
			...
			if (path.getLastPathComponent() == root && index >= model.getChildCount(root)) {
				...
				if (!tree.isRootVisible()) {
					...
					TreePath lastChildPath = path
							.pathByAddingChild(model.getChild(root, model.getChildCount(root) - 1));
				...
				rect = tree.getPathBounds(
						path.pathByAddingChild(model.getChild(path.getLastPathComponent(), index)));
			}
		}
	...
	protected void paintVerticalPartOfLeg(Graphics g, Rectangle clipBounds, Insets insets,
		...
		if (lineX >= clipLeft && lineX <= clipRight) {
			...
			if (parentBounds == null) {
				...
				top = Math.max(parentBounds.y + parentBounds.height + getVerticalLegBuffer(),
						clipTop);
	...
	protected void paintRow(Graphics g, Rectangle clipBounds, Insets insets, Rectangle bounds,
		...
		rendererPane.paintComponent(g, component, tree, bounds.x, bounds.y, bounds.width,
				bounds.height, true);
	}
	...
	protected void drawCentered(Component c, Graphics graphics, Icon icon, int x, int y) {
		icon.paintIcon(c, graphics, findCenteredX(x, icon.getIconWidth()),
	...
	protected boolean startEditing(TreePath path, MouseEvent event) {
		...
		if (cellEditor != null && tree.isPathEditable(path)) {
			...
			if (cellEditor.isCellEditable(event)) {
				...
				editingComponent.setBounds(nodeBounds.x, nodeBounds.y, nodeBounds.width,
						nodeBounds.height);
				editingPath = path;
				...
				if (event != null) {
					...
					Point componentPoint = SwingUtilities.convertPoint(tree,
							new Point(event.getX(), event.getY()), editingComponent);
					...
					Component activeComponent = SwingUtilities.getDeepestComponentAt(
							editingComponent, componentPoint.x, componentPoint.y);
					if (activeComponent != null) {
						MouseInputHandler handler = new MouseInputHandler(tree, activeComponent,
	...
	public class NodeDimensionsHandler extends AbstractLayoutCache.NodeDimensions {
		...
		public Rectangle getNodeDimensions(Object value, int row, int depth, boolean expanded,
			...
			if (currentCellRenderer != null) {
				...
				aComponent = currentCellRenderer.getTreeCellRendererComponent(tree, value,
	...
	public class TreePageAction extends AbstractAction {
		...
		private TreePageAction(int direction, String name, boolean addToSelection,
		...
		public void actionPerformed(ActionEvent e) {
			if (tree != null) {
				SHARED_ACTION.page(tree, BasicTreeUI.this, direction, addToSelection,
						changeSelection);
			}
		}
	...
	public class TreeIncrementAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			if (tree != null) {
				SHARED_ACTION.increment(tree, BasicTreeUI.this, direction, addToSelection,
						changeSelection);
			}
		}
	...
	public class TreeHomeAction extends AbstractAction {
		...
		private TreeHomeAction(int direction, String name, boolean addToSelection,
				boolean changeSelection) {
			this.direction = direction;
			this.changeSelection = changeSelection;
			this.addToSelection = addToSelection;
		...
		public void actionPerformed(ActionEvent e) {
			if (tree != null) {
				SHARED_ACTION.home(tree, BasicTreeUI.this, direction, addToSelection,
						changeSelection);
			}
		}
	...
	static class TreeTransferHandler extends TransferHandler
		...
		String getDisplayString(TreePath path, boolean selected, boolean leaf) {
			...
			Object node = path.getLastPathComponent();
			return tree.convertValueToText(node, selected, tree.isExpanded(row), leaf, row,
					hasFocus);
		}
		...
		public void treeNodesChanged(TreeModelEvent e) {
			if (treeState != null && e != null) {
				...
				if (indices == null || indices.length == 0) {
					...
					Object minChild = treeModel.getChild(parentPath.getLastPathComponent(),
							minIndex);
	...
	private static class Actions extends UIAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			BasicTreeUI ui = (BasicTreeUI) BasicLookAndFeel.getUIOfType(tree.getUI(),
					BasicTreeUI.class);
			if (ui == null) {
				return;
			}
		...
		private void scrollChangeSelection(JTree tree, BasicTreeUI ui, int direction,
			...
			if ((rowCount = ui.getRowCount(tree)) > 0 && ui.treeSelectionModel != null) {
				...
				if (direction == -1) {
					...
					visRect.x = Math.min(Math.max(0, tree.getWidth() - visRect.width),
