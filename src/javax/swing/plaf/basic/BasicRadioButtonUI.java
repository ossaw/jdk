...
public class BasicRadioButtonUI extends BasicToggleButtonUI {
	...
	public static ComponentUI createUI(JComponent b) {
		...
		BasicRadioButtonUI radioButtonUI = (BasicRadioButtonUI) appContext
				.get(BASIC_RADIO_BUTTON_UI_KEY);
	 ...
	 * RadioButtonUI.getPreferredSize() calls. Re-using rectangles rather than
	 * allocating them in each call substantially reduced the time it took
	 * getPreferredSize() to run. Obviously, this method can't be re-entered.
	 */
	...
	public Dimension getPreferredSize(JComponent c) {
		...
		SwingUtilities.layoutCompoundLabel(c, fm, text, buttonIcon, b.getVerticalAlignment(),
				b.getHorizontalAlignment(), b.getVerticalTextPosition(),
				b.getHorizontalTextPosition(), prefViewRect, prefIconRect, prefTextRect,
	...
	private boolean isValidRadioButtonObj(Object obj) {
		return ((obj instanceof JRadioButton) && ((JRadioButton) obj).isVisible()
	 ...
	 *            the event object.
	 * @param next,
	 *            indicate if it's next one
	 */
	private void selectRadioButton(ActionEvent event, boolean next) {
		// Get the source of the event.
		Object eventSrc = event.getSource();

		// Check whether the source is JRadioButton, it so, whether it is
		// visible
	...
	/**
	 * ButtonGroupInfo, used to get related info in button group for given radio
	 * button
	 */
	private class ButtonGroupInfo {
		...
		boolean getButtonGroupInfo() {
			...
			// If the button model is DefaultButtonModel, and use it, otherwise
			// return.
		...
		/**
		 * Find the new radio button that focus needs to be moved to in the
		 * group, select the button
		 *
		 * @param next,
		 *            indicate if it's arrow up/left or down/right
		 */
		void selectNewButton(boolean next) {
			...
			if (srcFound) {
				...
				if (next) {
					// Select Next button. Cycle to the first button if the
					// source
		...
		/**
		 * Find the button group the passed in JRadioButton belongs to, and move
		 * focus to next component of the last button in the group or previous
		 * component of first button
		 *
		 * @param next,
		 *            indicate if jump to next component or previous
		 */
	...
	private class KeyHandler implements KeyListener {
		...
		public void keyPressed(KeyEvent e) {
			if (e.getKeyCode() == KeyEvent.VK_TAB) {
				...
				// Check whether the source is a visible and enabled
				// JRadioButton
				if (isValidRadioButtonObj(eventSrc)) {
					e.consume();
					ButtonGroupInfo btnGroupInfo = new ButtonGroupInfo((JRadioButton) eventSrc);
