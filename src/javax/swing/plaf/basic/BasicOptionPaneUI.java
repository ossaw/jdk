 ...
 * <code>BasicMessagePaneUI</code> provides a means to place an icon, message
 * and buttons into a <code>Container</code>. Generally, the layout will look
 * like:
 * 
 * <pre>
 *        ------------------
 ...
 * <code>JLabel</code>. The message is an opaque object and is tested for the
 * following: if the message is a <code>Component</code> it is added to the
 * <code>Container</code>, if it is an <code>Icon</code> it is wrapped inside a
 * <code>JLabel</code> and added to the <code>Container</code> otherwise it is
 * wrapped inside a <code>JLabel</code>.
 * <p>
 * The above layout is used when the option pane's
 * <code>ComponentOrientation</code> property is horizontal, left-to-right. The
 * layout will be adjusted appropriately for other orientations.
 * <p>
 * The <code>Container</code>, message, icon, and buttons are all determined
...
public class BasicOptionPaneUI extends OptionPaneUI {
    ...
    /**
     * <code>JOptionPane</code> that the receiver is providing the look and feel
     * for.
     */
    ...
    /**
     * This is set to true in validateComponent if a Component is contained in
     * either the message or the buttons.
     */
    ...
    static {
        newline = java.security.AccessController.doPrivileged(new GetPropertyAction("line.separator"));
        if (newline == null) {
            newline = "\n";
        }
    }
    ...
    protected void installDefaults() {
        LookAndFeel.installColorsAndFont(optionPane, "OptionPane.background", "OptionPane.foreground",
                "OptionPane.font");
    ...
    /**
     * Returns the minimum size the option pane should be. Primarily provided
     * for subclassers wishing to offer a different minimum size.
     */
    ...
    /**
     * If <code>c</code> is the <code>JOptionPane</code> the receiver is
     * contained in, the preferred size that is returned is the maximum of the
     * preferred size of the <code>LayoutManager</code> for the
     * <code>JOptionPane</code>, and <code>getMinimumOptionPaneSize</code>.
     */
    public Dimension getPreferredSize(JComponent c) {
        if (c == optionPane) {
            ...
            if (lm != null) {
                ...
                if (ourMin != null)
                    return new Dimension(Math.max(lmSize.width, ourMin.width), Math.max(lmSize.height,
                            ourMin.height));
                return lmSize;
            }
            return ourMin;
    ...
    /**
     * Messaged from installComponents to create a Container containing the body
     * of the message. The icon is the created by calling <code>addIcon</code>.
     */
    protected Container createMessageArea() {
        ...
        cons.gridheight = 1;
        cons.anchor = DefaultLookup.getInt(optionPane, this, "OptionPane.messageAnchor",
     ...
     * it into <code>container</code>. If <code>msg</code> is an instance of
     * Component, it is added directly, if it is an Icon, a JLabel is created to
     * represent it, otherwise a JLabel is created for the string, if
     * <code>d</code> is an Object[], this method will be recursively invoked
     * for the children. <code>internallyCreated</code> is true if Objc is an
     * instance of Component and was created internally by this method (this is
    ...
    /**
     * Returns the icon from the JOptionPane the receiver is providing the look
     * and feel for, or the default icon as returned from
    ...
    /**
     * Creates and returns a Container containing the buttons. The buttons are
    ...
    protected Container createButtonArea() {
        ...
        bottom.setLayout(new ButtonAreaLayout(DefaultLookup.getBoolean(optionPane, this,
                "OptionPane.sameSizeButtons", true), DefaultLookup.getInt(optionPane, this,
                        "OptionPane.buttonPadding", 6), DefaultLookup.getInt(optionPane, this,
                                "OptionPane.buttonOrientation", SwingConstants.CENTER), DefaultLookup
                                        .getBoolean(optionPane, this, "OptionPane.isYesLast", false)));
     ...
     * <code>buttons</code> and adds it to <code>container</code>. This differs
     * from addMessageComponents in that it will recurse on <code>buttons</code>
     * and that if button is not a Component it will create an instance of
     * JButton.
     */
        ...
        if (buttons != null && buttons.length > 0) {
            ...
            for (int counter = 0; counter < numButtons; counter++) {
                ...
                if (button instanceof Component) {
                    ...
                    aButton.setMultiClickThreshhold(DefaultLookup.getInt(optionPane, this,
     ...
     * providing the look and feel for. If the JOptionPane has options set, they
     * will be provided, otherwise if the optionType is YES_NO_OPTION,
     * yesNoOptions is returned, if the type is YES_NO_CANCEL_OPTION
     * yesNoCancelOptions is returned, otherwise defaultButtons are returned.
     */
    protected Object[] getButtons() {
        if (optionPane != null) {
            ...
            if (suppliedOptions == null) {
                ...
                int minimumWidth = DefaultLookup.getInt(optionPane, this, "OptionPane.buttonMinimumWidth",
                        -1);
                if (type == JOptionPane.YES_NO_OPTION) {
                    defaultOptions = new ButtonFactory[2];
                    defaultOptions[0] = new ButtonFactory(UIManager.getString("OptionPane.yesButtonText", l),
                            getMnemonic("OptionPane.yesButtonMnemonic", l), (Icon) DefaultLookup.get(
                                    optionPane, this, "OptionPane.yesIcon"), minimumWidth);
                    defaultOptions[1] = new ButtonFactory(UIManager.getString("OptionPane.noButtonText", l),
                            getMnemonic("OptionPane.noButtonMnemonic", l), (Icon) DefaultLookup.get(
                                    optionPane, this, "OptionPane.noIcon"), minimumWidth);
                    ...
                    defaultOptions[1] = new ButtonFactory(UIManager.getString("OptionPane.noButtonText", l),
                            getMnemonic("OptionPane.noButtonMnemonic", l), (Icon) DefaultLookup.get(
                                    optionPane, this, "OptionPane.noIcon"), minimumWidth);
                    defaultOptions[2] = new ButtonFactory(UIManager.getString("OptionPane.cancelButtonText",
                            l), getMnemonic("OptionPane.cancelButtonMnemonic", l), (Icon) DefaultLookup.get(
                                    optionPane, this, "OptionPane.cancelIcon"), minimumWidth);
                } else if (type == JOptionPane.OK_CANCEL_OPTION) {
                    defaultOptions = new ButtonFactory[2];
                    defaultOptions[0] = new ButtonFactory(UIManager.getString("OptionPane.okButtonText", l),
                            getMnemonic("OptionPane.okButtonMnemonic", l), (Icon) DefaultLookup.get(
                                    optionPane, this, "OptionPane.okIcon"), minimumWidth);
                    defaultOptions[1] = new ButtonFactory(UIManager.getString("OptionPane.cancelButtonText",
                            l), getMnemonic("OptionPane.cancelButtonMnemonic", l), (Icon) DefaultLookup.get(
                                    optionPane, this, "OptionPane.cancelIcon"), minimumWidth);
                } else {
    ...
    /**
     * Returns the initial index into the buttons to select. The index is
     * calculated from the initial value from the JOptionPane and options of the
     * JOptionPane or 0.
     */
    ...
    /**
     * Sets the input value in the option pane the receiver is providing the
     * look and feel for based on the value in the inputComponent.
     */
    ...
    /**
     * Returns true if in the last call to validateComponent the message or
     * buttons contained a subclass of Component.
     */
     ...
     * If <code>syncAllWidths</code> is true, the widths of each component will
     * be set to the largest preferred size width.
    ...
    public static class ButtonAreaLayout implements LayoutManager {
         ...
         * orientation. This is done for backward compatibility for subclassers.
         */
                ...
                switch (getOrientation(container)) {
                    ...
                    case SwingConstants.CENTER:
                        if (getCentersChildren() || numChildren < 2) {
                            ...
                            x = insets.left;
                            if (getSyncAllWidths()) {
                                xOffset = (container.getWidth() - insets.left - insets.right
                                        - totalButtonWidth) / (numChildren - 1) + maxWidth;
                            } else {
                                xOffset = (container.getWidth() - insets.left - insets.right
                                        - totalButtonWidth) / (numChildren - 1);
                            }
                        }
                        break;
                }
            ...
            if (c != null) {
                ...
                if (children != null && children.length > 0) {
                    ...
                    if (syncAllWidths) {
                        ...
                        return new Dimension(extraWidth + (maxWidth * numChildren) + (numChildren - 1)
                                * padding, extraHeight + height);
                    } else {
                        int totalWidth = 0;
    ...
    public class PropertyChangeHandler implements PropertyChangeListener {
         ...
         * OPTIONS_PROPERTY or INITIAL_VALUE_PROPERTY, validateComponent is
         * invoked.
         */
    ...
    /**
     * Configures any necessary colors/fonts for the specified label used
    ...
    /**
     * Configures any necessary colors/fonts for the specified button used
    ...
    /**
     * A JTextField that allows you to specify an array of KeyStrokes that that
     * will have their bindings processed regardless of whether or not they are
     * registered on the JTextField. This is used as we really want the
     * ActionListener to be notified so that we can push the change to the
     * JOptionPane, but we also want additional bindings (those of the
     * JRootPane) to be processed as well.
     */
    private static class MultiplexingTextField extends JTextField {
        ...
        /**
         * Sets the KeyStrokes that will be additional processed for ancestor
         * bindings.
         */
    ...
    /**
     * Registered in the ActionMap. Sets the value of the option pane to
     ...
     * used so that addButtonComponents can tell which Buttons were created by
     * us vs subclassers or from the JOptionPane itself.
     */
