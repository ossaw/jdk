...
/**
 * Divider used by BasicSplitPaneUI. Subclassers may wish to override paint to
 * do something more interesting. The border effect is drawn in
 * BasicSplitPaneUI, so if you don't like that border, reset it there. To
 * conditionally drag from certain areas subclass mousePressed and call super
 * when you wish the dragging to begin.
 * <p>
...
public class BasicSplitPaneDivider extends Container implements PropertyChangeListener {
	...
	/**
	 * Handles mouse events from both this class, and the split pane. Mouse
	 * events are handled for the splitpane since you want to be able to drag
	 * when clicking on the border of the divider, which is not drawn by the
	 * divider.
	 */
	...
	/**
	 * Creates an instance of BasicSplitPaneDivider. Registers this instance for
	...
	/**
	 * Sets the size of the divider to <code>newSize</code>. That is the width
	 * if the splitpane is <code>HORIZONTAL_SPLIT</code>, or the height of
	...
	protected void oneTouchExpandableChanged() {
		if (!DefaultLookup.getBoolean(splitPane, splitPaneUI, "SplitPane.supportsOneTouchButtons",
				true)) {
	...
	protected class MouseHandler extends MouseAdapter implements MouseMotionListener {
		...
		public void mousePressed(MouseEvent e) {
			if ((e.getSource() == BasicSplitPaneDivider.this || e.getSource() == splitPane)
			...
			if (leftButton != null && rightButton != null && c == BasicSplitPaneDivider.this) {
				if (splitPane.isOneTouchExpandable()) {
					...
					if (orientation == JSplitPane.VERTICAL_SPLIT) {
						...
						rightButton.setBounds(extraX + oneTouchOffset + oneTouchSize * 2, y,
						...
						rightButton.setBounds(x, extraY + oneTouchOffset + oneTouchSize * 2,
