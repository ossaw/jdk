 ...
 * employed.
 * <ol>
 * <li>One strategy is to simply redefine the ViewFactory interface in the UI.
 * By default, this UI itself acts as the factory for View implementations. This
 * is useful for simple factories. To do this reimplement the {@link #create}
 * method.
 * <li>A common strategy for creating more complex types of documents is to have
 * the EditorKit implementation return a factory. Since the EditorKit ties all
 * of the pieces necessary to maintain a type of document, the factory is
 * typically an important part of that and should be produced by the EditorKit
 * implementation.
 * </ol>
 * <p>
...
public abstract class BasicTextUI extends TextUI implements ViewFactory {
	 ...
	 * BasicCaret is created. This method can be redefined to provide something
	 * else that implements the InputPosition interface or a subclass of JCaret.
	 *
	...
	/**
	 * Creates the object to use for adding highlights. By default an instance
	 * of BasicHighlighter is created. This method can be redefined to provide
	 * something else that implements the Highlighter interface or a subclass of
	 * DefaultHighlighter.
	 *
	 ...
	 * this UI. This is implemented to create a name based upon the classname.
	 * The name is the the name of the class with the package prefix removed.
	 *
	 * @return the name
	 */
	...
	/**
	 * Creates the keymap to use for the text component, and installs any
	 * necessary bindings into it. By default, the keymap is shared between all
	 * instances of this type of TextUI. The keymap has the name defined by the
	 * getKeymapName method. If the keymap is not found, then DEFAULT_KEYMAP
	 * from JTextComponent is used.
	 * <p>
	 * The set of bindings used to create the keymap is fetched from the
	 * UIManager using a key formed by combining the {@link #getPropertyPrefix}
	 * method and the string <code>.keyBindings</code>. The type is expected to
	 * be <code>JTextComponent.KeyBinding[]</code>.
	 *
	 * @return the keymap
	 * @see #getKeymapName
	...
	private void updateBackground(JTextComponent c) {
		...
		if (background instanceof UIResource) {
			...
			Color disabledBG = DefaultLookup.getColor(c, this, prefix + ".disabledBackground",
					null);
			Color inactiveBG = DefaultLookup.getColor(c, this, prefix + ".inactiveBackground",
					null);
			 ...
			 *
			 * JLabel label = new JLabel(); JTextArea area = new JTextArea();
	...
	/**
	 * Initializes component properties, such as font, foreground, background,
	 ...
	 * {@code null}. A property is considered overridden if its current value is
	 * not a {@code UIResource}.
	 *
	 * @see #installDefaults
	 * @see #uninstallUI
	 */
	...
	/**
	 * Invoked when the focus accelerator changes, this will update the key
			...
			if (km != null) {
				...
				if (accelerator != '\0') {
					km.put(KeyStroke.getKeyStroke(accelerator,
							BasicLookAndFeel.getFocusAcceleratorKeyMask()), "requestFocus");
				}
			}
		}
	}
	...
	void updateFocusTraversalKeys() {
		/*
		 * Fix for 4514331 Non-editable JTextArea and similar should allow Tab
		...
		if (editorKit != null && editorKit instanceof DefaultEditorKit) {
			...
			if (editor.isEditable()) {
				...
				backwardTraversalKeys
						.remove(KeyStroke.getKeyStroke(KeyEvent.VK_TAB, InputEvent.SHIFT_MASK));
			} else {
	...
	/**
	 * Returns the <code>TransferHandler</code> that will be installed if their
	 * isn't one installed on the <code>JTextComponent</code>.
	 */
	...
	ActionMap getActionMap() {
		...
		/*
		 * fix for bug 4515750 JTextField & non-editable JTextArea bind return
		 * key - default btn not accessible
		 *
		 * Wrap the return action so that it is only enabled when the component
		 * is editable. This allows the default button to be processed when the
		 * text component has focus and isn't editable.
		 *
		 */
	...
	/**
	 * Flags model changes. This is called whenever the model has changed. It is
	 * implemented to rebuild the view hierarchy to represent the default root
	 * element of the associated model.
	 */
	...
	/**
	 * Sets the current root of the view hierarchy and calls invalidate(). If
	 * there were any child components, they will be removed (i.e. there are
	 ...
	 * <li>The highlights (if any) are painted.
	 * <li>The view hierarchy is painted.
	 * <li>The caret is painted.
	 * </ol>
	 *
	 * @param g
	 *            the graphics context
	 ...
	 * <ol>
	 * <li>Sets the associated component to opaque if the opaque property has
	 * not already been set by the client program. This will cause the
	 * component's background color to be painted.
	 * <li>Installs the default caret and highlighter into the associated
	 * component. These properties are only set if their current value is either
	 * {@code null} or an instance of {@link UIResource}.
	 * <li>Attaches to the editor and model. If there is no model, a default one
	 * is created.
	 * <li>Creates the view factory and the view hierarchy used to represent the
	 * model.
	 * </ol>
	 *
	 * @param c
	...
	/**
	 * Superclass paints background in an uncontrollable way (i.e. one might
	 * want an image tiled into the background). To prevent this from happening
	 * twice, this method is reimplemented to simply paint.
	 * <p>
	 * <em>NOTE:</em> NOTE: Superclass is also not thread-safe in its rendering
	 * of the background, although that is not an issue with the default
	 * rendering.
	 */
	...
	/**
	 * Gets the preferred size for the editor component. If the component has
	...
	public Dimension getMaximumSize(JComponent c) {
		...
		try {
			d.width = (int) Math.min(
					(long) rootView.getMaximumSpan(View.X_AXIS) + (long) i.left + (long) i.right,
					Integer.MAX_VALUE);
			d.height = (int) Math.min(
					(long) rootView.getMaximumSpan(View.Y_AXIS) + (long) i.top + (long) i.bottom,
					Integer.MAX_VALUE);
		} finally {
			if (doc instanceof AbstractDocument) {
				((AbstractDocument) doc).readUnlock();
	 ...
	 * coordinate system. The component must have a non-zero positive size for
	 ...
	 * representative location in the model. The component must have a non-zero
	 ...
	 * the root of the hierarchy) that can be traversed to determine how the
	 * model is being represented spatially.
	 * <p>
	 * <font color=red><b>NOTE:</b>The View hierarchy can be traversed from the
	 * root view, and other things can be done as well. Things done in this way
	 * cannot be protected like simple method calls through the TextUI.
	 * Therefore, proper operation in the presence of concurrency must be
	...
	/**
	 * Creates a view for an element. If a subclass wishes to directly implement
	 * the factory producing the view(s), it should reimplement this method. By
	 * default it simply returns null indicating it is unable to represent the
	 * element.
	 *
	 * @param elem
	 *            the element
	...
	class RootView extends View {
		 ...
		 * out where various views are located, without assuming the views store
		 * their location. This returns the given allocation since this view
		 * simply acts as a gateway between the view hierarchy and the
		 ...
		 * might not allow access to some of the locations in the model. This
		 * method enables specifying a position to convert within the range of
		 * &gt;=0. If the value is -1, a position will be calculated
		 * automatically. If the value &lt; -1, the {@code BadLocationException}
		 * will be thrown.
		 *
		 * @param pos
		 *            the position to convert &gt;= 0
		 ...
		 * implemented to fetch the factory provided by the associated EditorKit
		 * unless that is null, in which case this simply returns the
		 * BasicTextUI itself which allows subclasses to implement a simple
		 * factory directly without creating extra objects.
		 *
		 * @return the factory
		 */
	...
	/**
	 * Handles updates from various places. If the model is changed, this class
	 * unregisters as a listener to the old model and registers with the new
	 * model. If the document model changes, the change is forwarded to the root
	...
	class UpdateHandler
		 ...
		 * editor. The factory is added as an argument to the update so that the
		...
		/**
		 * Lays out the container in the specified panel. This is implemented to
		 * position all components that were added with a View object as a
		 * constraint. The current allocation of the associated View is used as
		 * the location of the component.
		 * <p>
		 * A read-lock is acquired on the document to prevent the view tree from
		...
		/**
		 * The "layout constraints" for the LayoutManager2 implementation. These
		 * are View objects for those components that are represented by a View
		 * in the View tree.
		 */
	 ...
	 */
	static class DragListener extends MouseInputAdapter implements BeforeDrag {
			...
			TextTransferable(JTextComponent c, int start, int end) {
				...
				try {
					...
					if (c instanceof JEditorPane) {
						...
						ep.getEditorKit().write(sw, doc, p0.getOffset(),
