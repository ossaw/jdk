...
public class BasicTextFieldUI extends BasicTextUI {
        ...
        if (rootView.getViewCount() > 0) {
            ...
            if (height > 0) {
                ...
                if (fieldView instanceof I18nFieldView) {
                    int fieldBaseline = BasicHTML.getBaseline(fieldView, width - insets.left - insets.right,
                            height);
                    if (fieldBaseline < 0) {
                        return -1;
                    }
    ...
    /**
     * A field view that support bidirectional text via the support provided by
     * ParagraphView.
     */
    static class I18nFieldView extends ParagraphView {
         ...
         * index. There is no limit for a field since it scrolls, so this is
         * implemented to return <code>Integer.MAX_VALUE</code>.
         */
        ...
        /**
         * Adjusts the allocation given to the view to be a suitable allocation
         * for a text field. If the view has been allocated more than the
         * preferred span vertically, the allocation is changed to be centered
         * vertically. Horizontally the view is adjusted according to the
         * horizontal alignment property set on the associated JTextField (if
        ...
        /**
         * Update the visibility model with the associated JTextField (if there
         * is one) to reflect the current visibility as a result of changes to
         * the document model. The bounded range properties are updated. If the
         * view hasn't yet been shown the extent will be zero and we just set it
         * to be full until determined otherwise.
         */
