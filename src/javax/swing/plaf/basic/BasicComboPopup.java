 ...
 * <code>createxxxListener()</code> methods and internal classes. You can change
 * the behavior of this class by overriding the <code>createxxxListener()</code>
 * methods and supplying your own event listeners or subclassing from the ones
...
public class BasicComboPopup extends JPopupMenu implements ComboPopup {
    ...
    /**
     * Called when the UI is uninstalling. Since this popup isn't in the
     * component tree, it won't get it's uninstallUI() called. It removes the
     * listeners that were added in addComboBoxListeners().
     */
    ...
    protected void uninstallKeyboardActions() {
        ...
        // comboBox.unregisterKeyboardAction( KeyStroke.getKeyStroke(
        // KeyEvent.VK_ENTER, 0 ) );
    }
    ...
    protected void firePopupMenuWillBecomeVisible() {
        ...
        // comboBox.firePopupMenuWillBecomeVisible() is called from
        // BasicComboPopup.show() method
        // to let the user change the popup menu from the
    ...
    /**
     * Creates a listener that will watch for mouse-press and release events on
     * the combo box.
     * <strong>Warning:</strong> When overriding this method, make sure to
     * maintain the existing behavior.
     *
     * @return a <code>MouseListener</code> which will be added to the combo box
     *         or null
     */
     ...
     * maintain the existing behavior.
     *
     * @return a <code>MouseMotionListener</code> which will be added to the
     *         combo box or null
     */
    ...
    /**
     * Creates the key listener that will be added to the combo box. If this
     ...
     * the popup's list. If this method returns null then it will not be added
     * to the popup list.
     *
    ...
    /**
     * Creates a mouse listener that watches for mouse events in the popup's
     * list. If this method returns null then it will not be added to the combo
     * box.
     *
    ...
    /**
     * Creates a mouse motion listener that watches for mouse motion events in
     * the popup's list. If this method returns null then it will not be added
     * to the combo box.
     *
            ...
            public void processMouseEvent(MouseEvent e) {
                if (BasicGraphicsUtils.isMenuShortcutKeyDown(e)) {
                    ...
                    e = new MouseEvent((Component) e.getSource(), e.getID(), e.getWhen(), e.getModifiers()
                            ^ toolkit.getMenuShortcutKeyMask(), e.getX(), e.getY(), e.getXOnScreen(), e
                                    .getYOnScreen(), e.getClickCount(), e.isPopupTrigger(),
                            MouseEvent.NOBUTTON);
                }
    ...
    /**
     * Installs the listeners on the combo box model. Any listeners installed on
     * the combo box model should be removed in
    ...
    protected void installKeyboardActions() {
         ...
         * ActionListener action = new ActionListener() { public void
         * actionPerformed(ActionEvent e){ } };
         * comboBox.registerKeyboardAction( action, KeyStroke.getKeyStroke(
         * KeyEvent.VK_ENTER, 0 ), JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT
         * );
         */
    ...
    /**
     * A listener to be registered upon the combo box (<em>not</em> its popup
     * menu) to handle mouse events that affect the state of the popup menu. The
     * main purpose of this listener is to make the popup menu appear and
     * disappear. This listener also helps with click-and-drag scenarios by
     * setting the selection if the mouse was released over the list during a
     * drag.
     * <p>
     * <strong>Warning:</strong> We recommend that you <em>not</em> create
     * subclasses of this class. If you absolutely must create a subclass, be
     * sure to invoke the superclass version of each method.
     *
    ...
    protected class InvocationMouseHandler extends MouseAdapter {
        ...
        /**
         * Responds to the user terminating a click or drag that began on the
         * combo box.
         *
         * @param e
    ...
    /**
     * This listener watches for dragging and updates the current selection in
     * the list if it is dragging over the list.
     */
    ...
    /**
     * As of Java 2 platform v 1.4, this class is now obsolete and is only
     * included for backwards API compatibility. Do not instantiate or subclass.
     * <p>
     * All the functionality of this class has been included in BasicComboBoxUI
    ...
    /**
     * As of Java 2 platform v 1.4, this class is now obsolete, doesn't do
     * anything, and is only included for backwards API compatibility. Do not
     * call or override.
     */
    ...
    /**
     * As of 1.4, this class is now obsolete, doesn't do anything, and is only
    ...
    /**
     * This listener changes the selected item as you move the mouse over the
     * list. The selection change is not committed to the model, this is for
     * user feedback only.
     */
     ...
     * the superclass methods to ensure that the combo popup correctly handles
     * property changes.
     *
     * @see #createPropertyChangeListener
     */
    ...
    private class Handler implements ItemListener, MouseListener, MouseMotionListener, MouseWheelListener,
    ...
    protected void startAutoScrolling(int direction) {
        ...
        if (direction == SCROLL_UP) {
            ...
            autoscrollTimer = new Timer(100, new AutoScrollActionHandler(SCROLL_UP));
        } else if (direction == SCROLL_DOWN) {
            scrollDirection = SCROLL_DOWN;
            Dimension size = scroller.getSize();
            Point convertedPoint = SwingUtilities.convertPoint(scroller, new Point(1, (size.height - 1) - 2),
                    list);
            int bottom = list.locationToIndex(convertedPoint);
            list.setSelectedIndex(bottom);

            autoscrollTimer = new Timer(100, new AutoScrollActionHandler(SCROLL_DOWN));
        }
    ...
    protected MouseEvent convertMouseEvent(MouseEvent e) {
        ...
        MouseEvent newEvent = new MouseEvent((Component) e.getSource(), e.getID(), e.getWhen(), e
                .getModifiers(), convertedPoint.x, convertedPoint.y, e.getXOnScreen(), e.getYOnScreen(), e
                        .getClickCount(), e.isPopupTrigger(), MouseEvent.NOBUTTON);
        return newEvent;
    }

    /**
     * Retrieves the height of the popup based on the current ListCellRenderer
     * and the maximum row count.
     */
    ...
    private Point getPopupLocation() {
        ...
        popupSize.setSize(popupSize.width - (insets.right + insets.left), getPopupHeightForRowCount(comboBox
                .getMaximumRowCount()));
    ...
    /**
     * A utility method used by the event listeners. Given a mouse event, it
     * changes the list selection to the list item below the mouse.
     */
