...
final class ScrollBarThumbPainter extends AbstractRegionPainter {
	...
	private Color color9 = decodeColor("nimbusBase", 5.2034855E-5f, -0.41753247f, 0.09803921f,
			-222);
	private Color color10 = new Color(255, 200, 0, 255);
	private Color color11 = decodeColor("nimbusBase", -0.0017285943f, -0.362987f, 0.011764705f,
			-255);
	...
	private Path2D decodePath1() {
		...
		path.curveTo(decodeAnchorX(0.0f, 0.0f), decodeAnchorY(1.0666667222976685f, 6.0f),
				decodeAnchorX(1.0f, -10.0f), decodeAnchorY(2.0f, 0.0f), decodeX(1.0f),
				decodeY(2.0f));
		path.lineTo(decodeX(2.0f), decodeY(2.0f));
		path.curveTo(decodeAnchorX(2.0f, 10.0f), decodeAnchorY(2.0f, 0.0f),
				decodeAnchorX(3.0f, 0.0f), decodeAnchorY(1.0666667222976685f, 6.0f), decodeX(3.0f),
				decodeY(1.0666667f));
	...
	private Path2D decodePath2() {
		...
		path.curveTo(decodeAnchorX(0.06666667014360428f, -0.045454545454545414f),
				decodeAnchorY(1.0666667222976685f, 8.45454545454545f),
				decodeAnchorX(1.0f, -5.863636363636354f), decodeAnchorY(1.933333396911621f, 0.0f),
				decodeX(1.0f), decodeY(1.9333334f));
		path.lineTo(decodeX(2.0f), decodeY(1.9333334f));
		path.curveTo(decodeAnchorX(2.0f, 5.909090909090935f),
				decodeAnchorY(1.933333396911621f, -3.552713678800501E-15f),
				decodeAnchorX(2.933333396911621f, -0.045454545454546746f),
				decodeAnchorY(1.0666667222976685f, 8.36363636363636f), decodeX(2.9333334f),
				decodeY(1.0666667f));
	...
	private Path2D decodePath3() {
		...
		path.curveTo(decodeAnchorX(0.16060607135295868f, 0.0f),
				decodeAnchorY(1.5090909004211426f, 0.0f),
				decodeAnchorX(0.20000000298023224f, -0.9545454545454564f),
				decodeAnchorY(1.1363636255264282f, 1.5454545454545472f), decodeX(0.2f),
				decodeY(1.1363636f));
		path.curveTo(decodeAnchorX(0.20000000298023224f, 0.9545454545454564f),
				decodeAnchorY(1.1363636255264282f, -1.5454545454545472f),
				decodeAnchorX(0.4000000059604645f, 0.0f), decodeAnchorY(1.0f, 0.0f), decodeX(0.4f),
				decodeY(1.0f));
		path.closePath();
		return path;
	}
	...
	private Path2D decodePath5() {
		...
		path.curveTo(decodeAnchorX(2.6030304431915283f, 0.0f), decodeAnchorY(1.0f, 0.0f),
				decodeAnchorX(2.7787880897521973f, -0.6818181818181728f),
				decodeAnchorY(1.1333333253860474f, -1.227272727272727f), decodeX(2.778788f),
				decodeY(1.1333333f));
		path.curveTo(decodeAnchorX(2.7787880897521973f, 0.6818181818181728f),
				decodeAnchorY(1.1333333253860474f, 1.227272727272727f),
				decodeAnchorX(2.8393938541412354f, 0.0f), decodeAnchorY(1.5060606002807617f, 0.0f),
				decodeX(2.8393939f), decodeY(1.5060606f));
	...
	private Path2D decodePath6() {
		...
		path.curveTo(decodeAnchorX(2.556363582611084f, 0.0f), decodeAnchorY(1.0f, 0.0f),
				decodeAnchorX(2.7587878704071045f, -0.6818181818181728f),
				decodeAnchorY(1.1399999856948853f, -1.2272727272727266f), decodeX(2.7587879f),
				decodeY(1.14f));
	...
	private Paint decodeGradient2(Shape s) {
		...
		float h = (float) bounds.getHeight();
		return decodeGradient((0.5f * w) + x, (0.0f * h) + y, (0.5f * w) + x, (1.0f * h) + y,
				new float[] { 0.038922157f, 0.0508982f, 0.06287425f, 0.19610777f, 0.32934132f,
						0.48952097f, 0.6497006f, 0.8248503f, 1.0f },
	...
	private Paint decodeGradient3(Shape s) {
		...
		return decodeGradient((0.06818182f * w) + x, (-0.005952381f * h) + y, (0.3689091f * w) + x,
				(0.23929171f * h) + y, new float[] { 0.0f, 0.5f, 1.0f },
	...
	private Paint decodeGradient4(Shape s) {
		...
		float w = (float) bounds.getWidth();
		float h = (float) bounds.getHeight();
		return decodeGradient((0.9409091f * w) + x, (0.035928145f * h) + y, (0.5954546f * w) + x,
				(0.26347303f * h) + y, new float[] { 0.0f, 0.5f, 1.0f },
				new Color[] { color8, decodeColor(color8, color11, 0.5f), color11 });
	}
	...
	private Paint decodeGradient8(Shape s) {
		...
		return decodeGradient((0.06818182f * w) + x, (-0.005952381f * h) + y, (0.3689091f * w) + x,
				(0.23929171f * h) + y, new float[] { 0.0f, 0.5f, 1.0f },
				new Color[] { color24, decodeColor(color24, color9, 0.5f), color9 });
	}
	...
	private Paint decodeGradient9(Shape s) {
		...
		return decodeGradient((0.9409091f * w) + x, (0.035928145f * h) + y, (0.37615633f * w) + x,
				(0.34910178f * h) + y, new float[] { 0.0f, 0.5f, 1.0f },
				new Color[] { color25, decodeColor(color25, color11, 0.5f), color11 });
	}
