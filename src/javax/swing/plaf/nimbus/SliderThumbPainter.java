...
final class SliderThumbPainter extends AbstractRegionPainter {
	...
	private Color color6 = decodeColor("nimbusBlueGrey", -0.003968239f, 0.0014736876f, -0.25490198f,
			-156);
	...
	private Color color13 = decodeColor("nimbusBase", -0.0038217902f, -0.15532213f, -0.14901963f,
			0);
	...
	private Color color25 = decodeColor("nimbusBase", -0.0033828616f, -0.40608466f, -0.019607842f,
			0);
	...
	private Path2D decodePath1() {
		...
		path.curveTo(decodeAnchorX(0.8166667222976685f, 1.5643268796105616f),
				...
				decodeAnchorY(1.6116883754730225f, -0.46476349119779314f), decodeX(2.7925456f),
				decodeY(1.6116884f));
		path.curveTo(decodeAnchorX(2.7925455570220947f, -0.34086855855797005f),
				...
				decodeAnchorY(2.7693636417388916f, -0.006014915148298883f), decodeX(0.7006364f),
				decodeY(2.7693636f));
		path.curveTo(decodeAnchorX(0.7006363868713379f, -3.523395559100149f),
				...
				decodeAnchorY(0.5007575750350952f, 0.3689954354443423f), decodeX(0.8166667f),
				decodeY(0.5007576f));
		path.closePath();
		return path;
	}
	...
	private Path2D decodePath2() {
		...
		path.curveTo(decodeAnchorX(0.6155303120613098f, 0.9098089454358838f),
				...
				decodeAnchorY(1.611201286315918f, 0.9295520709665155f), decodeX(2.6151516f),
				decodeY(1.6112013f));
		path.curveTo(decodeAnchorX(2.6151516437530518f, -0.013655180248463239f),
				...
				decodeAnchorY(0.4071640372276306f, -1.424864396720248f), decodeX(0.60923916f),
				decodeY(0.40716404f));
		path.curveTo(decodeAnchorX(0.6092391610145569f, -0.7485208875763871f),
				...
				decodeAnchorY(2.595454692840576f, -1.0843510320300886f), decodeX(0.6155303f),
				decodeY(2.5954547f));
		path.closePath();
		return path;
	}
	...
	private Path2D decodePath3() {
		...
		path.curveTo(decodeAnchorX(0.8055605888366699f, 0.508208945236218f),
				...
				decodeAnchorY(1.613116979598999f, -0.6066882577419275f), decodeX(2.3692727f),
				decodeY(1.613117f));
		path.curveTo(decodeAnchorX(2.3692727088928223f, -0.0038901961210928704f),
				decodeAnchorY(1.613116979598999f, 0.7411076447438294f),
				decodeAnchorX(0.7945454716682434f, 0.38709738141524763f),
				decodeAnchorY(2.393272876739502f, 1.240782009971129f), decodeX(0.7945455f),
				decodeY(2.3932729f));
		path.curveTo(decodeAnchorX(0.7945454716682434f, -0.3863658307342148f),
				...
				decodeAnchorY(0.600969672203064f, 1.6626496533832493f), decodeX(0.8055606f),
				decodeY(0.6009697f));
		path.closePath();
		return path;
	}
	...
	private Path2D decodePath4() {
		...
		path.curveTo(decodeAnchorX(0.600595235824585f, 1.5643268796105612f),
				...
				decodeAnchorY(1.6116883754730225f, -1.1881161542467655f), decodeX(2.7925456f),
				decodeY(1.6116884f));
		path.curveTo(decodeAnchorX(2.7925455570220947f, -0.007364540661274788f),
				...
				decodeAnchorY(2.869363784790039f, -0.008974581987587271f), decodeX(0.7006364f),
				decodeY(2.8693638f));
		path.curveTo(decodeAnchorX(0.7006363868713379f, -3.75489914400509f),
				...
				decodeAnchorY(0.1172754317522049f, 0.3689954354443423f), decodeX(0.60059524f),
				decodeY(0.11727543f));
		path.closePath();
		return path;
	}
	...
	private Paint decodeGradient1(Shape s) {
		...
		return decodeGradient((0.5106101f * w) + x, (-4.553649E-18f * h) + y, (0.49933687f * w) + x,
				(1.0039787f * h) + y, new float[] { 0.0f, 0.5f, 1.0f },
	...
	private Paint decodeGradient2(Shape s) {
		...
		float w = (float) bounds.getWidth();
		float h = (float) bounds.getHeight();
		return decodeGradient((0.5023511f * w) + x, (0.0015673981f * h) + y, (0.5023511f * w) + x,
				(1.0f * h) + y, new float[] { 0.0f, 0.21256684f, 0.42513368f, 0.71256685f, 1.0f },
	...
	private Paint decodeGradient3(Shape s) {
		...
		float w = (float) bounds.getWidth();
		float h = (float) bounds.getHeight();
		return decodeGradient((0.51f * w) + x, (-4.553649E-18f * h) + y, (0.51f * w) + x,
				(1.0039787f * h) + y, new float[] { 0.0f, 0.5f, 1.0f },
	...
	private Paint decodeGradient4(Shape s) {
		...
		float w = (float) bounds.getWidth();
		float h = (float) bounds.getHeight();
		return decodeGradient((0.5f * w) + x, (0.0015673981f * h) + y, (0.5f * w) + x,
				(1.0f * h) + y,
				new float[] { 0.0f, 0.21256684f, 0.42513368f, 0.56149733f, 0.69786096f, 0.8489305f,
						1.0f },
				new Color[] { color9, decodeColor(color9, color10, 0.5f), color10,
						decodeColor(color10, color10, 0.5f), color10,
	...
	private Paint decodeGradient7(Shape s) {
		...
		return decodeGradient((0.5106101f * w) + x, (-4.553649E-18f * h) + y, (0.49933687f * w) + x,
				(1.0039787f * h) + y, new float[] { 0.0f, 0.5f, 1.0f },
				new Color[] { color14, decodeColor(color14, color19, 0.5f), color19 });
	}
	...
	private Paint decodeGradient8(Shape s) {
		...
		return decodeGradient((0.5023511f * w) + x, (0.0015673981f * h) + y, (0.5023511f * w) + x,
				(1.0f * h) + y,
				new float[] { 0.0f, 0.23796791f, 0.47593582f, 0.5360962f, 0.5962567f, 0.79812837f,
						1.0f },
				new Color[] { color20, decodeColor(color20, color21, 0.5f), color21,
						decodeColor(color21, color21, 0.5f), color21,
	...
	private Paint decodeGradient10(Shape s) {
		...
		float h = (float) bounds.getHeight();
		return decodeGradient((0.5f * w) + x, (0.0f * h) + y, (0.5f * w) + x, (1.0f * h) + y,
				new float[] { 0.061290324f, 0.1016129f, 0.14193548f, 0.3016129f, 0.46129033f,
						0.5983871f, 0.7354839f, 0.7935484f, 0.8516129f },
	...
	private Paint decodeGradient17(Shape s) {
		...
		float w = (float) bounds.getWidth();
		float h = (float) bounds.getHeight();
		return decodeGradient((0.4925773f * w) + x, (0.082019866f * h) + y, (0.4925773f * w) + x,
				(0.91798013f * h) + y,
				new float[] { 0.061290324f, 0.1016129f, 0.14193548f, 0.3016129f, 0.46129033f,
						0.5983871f, 0.7354839f, 0.7935484f, 0.8516129f },
				new Color[] { color28, decodeColor(color28, color29, 0.5f), color29,
