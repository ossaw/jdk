...
class ImageScalingHelper {
	...
	/**
	 * Specifies that all regions should be painted. If this is set any other
	 * regions specified will not be painted. For example PAINT_ALL |
	 * PAINT_CENTER will paint all but the center.
	 */
	 ...
	 *            Height to render to
	 * @param image
	 *            Image to render from, if <code>null</code> this method will do
	 *            nothing
	 * @param sInsets
	 ...
	 * @param dInsets
	 *            Destination insets specifying the portion of the image will be
	 *            stretched or tiled, if <code>null</code> empty
	 *            <code>Insets</code> will be used.
	 * @param paintType
	...
	public static void paint(Graphics g, int x, int y, int w, int h, Image image, Insets sInsets,
		...
		if (paintType == PaintType.CENTER) {
			...
			// Tile the image
			int lastIY = 0;
			for (int yCounter = y, maxY = y + h; yCounter < maxY; yCounter += (ih
					- lastIY), lastIY = 0) {
				int lastIX = 0;
				for (int xCounter = x, maxX = x + w; xCounter < maxX; xCounter += (iw
						- lastIX), lastIX = 0) {
					int dx2 = Math.min(maxX, xCounter + iw - lastIX);
					int dy2 = Math.min(maxY, yCounter + ih - lastIY);
					g.drawImage(image, xCounter, yCounter, dx2, dy2, lastIX, lastIY,
							lastIX + dx2 - xCounter, lastIY + dy2 - yCounter, null);
				}
			}
		} else {
			int st = sInsets.top;
			...
			if ((mask & PAINT_LEFT) != 0) {
				drawChunk(image, g, stretch, x, y + dt, x + dl, y + h - db, 0, st, sl, ih - sb,
						false);
			}
			...
			if ((mask & PAINT_TOP) != 0) {
				drawChunk(image, g, stretch, x + dl, y, x + w - dr, y + dt, sl, 0, iw - sr, st,
						true);
			}
			...
			if ((mask & PAINT_RIGHT) != 0) {
				drawChunk(image, g, stretch, x + w - dr, y + dt, x + w, y + h - db, iw - sr, st, iw,
						ih - sb, false);
			}
			...
			if ((mask & PAINT_BOTTOM) != 0) {
				drawChunk(image, g, stretch, x + dl, y + h - db, x + w - dr, y + h, sl, ih - sb,
						iw - sr, ih, true);
			}
			...
			if ((mask & PAINT_CENTER) != 0) {
				drawImage(image, g, x + dl, y + dt, x + w - dr, y + h - db, sl, st, iw - sr,
						ih - sb);
			}
		}
	}
	 ...
	 *            Graphics to render to
	 * @param stretch
	 *            Whether the image should be stretched or timed in the provided
	 *            space.
	 * @param dx1
	 *            X origin to draw to
	 * @param dy1
	 ...
	 */
	private static void drawChunk(Image image, Graphics g, boolean stretch, int dx1, int dy1,
			int dx2, int dy2, int sx1, int sy1, int sx2, int sy2, boolean xDirection) {
		...
		if (stretch) {
			...
			while (dx1 < dx2 && dy1 < dy2) {
				...
				g.drawImage(image, dx1, dy1, newDX2, newDY2, sx1, sy1, sx1 + newDX2 - dx1,
						sy1 + newDY2 - dy1, null);
				dx1 += deltaX;
				dy1 += deltaY;
			}
	...
	private static void drawImage(Image image, Graphics g, int dx1, int dy1, int dx2, int dy2,
