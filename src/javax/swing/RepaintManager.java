...
public class RepaintManager {
    ...
    // Whether or not a VolatileImage should be used for double-buffered
    // painting
    static boolean volatileImageBufferEnabled = true;
    /**
     * Type of VolatileImage which should be used for double-buffered painting.
     */
    ...
    //
    /**
     * True if we're in the process of painting the dirty regions. This is set
     * to true in <code>paintDirtyRegions</code>.
     */
    ...
    static {
        ...
        volatileImageBufferEnabled = "true".equals(AccessController.doPrivileged(new GetPropertyAction(
                "swing.volatileImageBufferEnabled", "true")));
        boolean headless = GraphicsEnvironment.isHeadless();
        if (volatileImageBufferEnabled && headless) {
            volatileImageBufferEnabled = false;
        }
        nativeDoubleBuffering = "true".equals(AccessController.doPrivileged(new GetPropertyAction(
        ...
        HANDLE_TOP_LEVEL_PAINT = "true".equals(AccessController.doPrivileged(new GetPropertyAction(
        ...
        if ((tk instanceof SunToolkit) && ((SunToolkit) tk).isSwingBackbufferTranslucencySupported()) {
     ...
     *        be used by an overridden version to return a different
     *        RepaintManager depending on the Component
    ...
    /**
     * Mark the component as in need of layout and queue a runnable for the
     * event dispatching thread that will validate the components first
     * isValidateRoot() ancestor.
     *
    ...
    public synchronized void addInvalidComponent(JComponent invalidComponent) {
         ...
         * validateRoot if it's not there already. If this validateRoot is
         * already in the vector, we're done.
         */
     ...
     * <i>c</i> already has a dirty region, the rectangle <i>(x,y,w,h)</i> will
     * be unioned with the region that should be redrawn.
     *
    ...
    private void addDirtyRegion0(Container c, int x, int y, int w, int h) {
         ...
         * note that it's still possible that c is completely obscured by an
         * opaque ancestor in the specified rectangle).
         */
        Component root = null;
        ...
        for (Container p = c; p != null; p = p.getParent()) {
            ...
            if ((p instanceof Window) || (p instanceof Applet)) {
                ...
                if (p instanceof Frame && (((Frame) p).getExtendedState()
    ...
    /**
     * Return the current dirty region for a component. Return an empty
     * rectangle if the component is not dirty.
     */
     ...
     * completely painted during the next paintDirtyRegions(). If computing
     * dirty regions is expensive for your component, use this method and avoid
    ...
    private void updateWindows(Map<Component, Rectangle> dirtyComponents) {
        ...
        for (Iterator<Component> it = dirtyComps.iterator(); it.hasNext();) {
            ...
            Window window = dirty instanceof Window ? (Window) dirty
    ...
    private void paintDirtyRegions(final Map<Component, Rectangle> tmpDirtyComponents) {
        ...
        try {
            for (int j = 0; j < count.get(); j++) {
                ...
                AccessControlContext stack = AccessController.getContext();
                AccessControlContext acc = AWTAccessor.getComponentAccessor().getAccessControlContext(
                        dirtyComponent);
                javaSecurityAccess.doIntersectionPrivilege(new PrivilegedAction<Void>() {
                    public Void run() {
                        Rectangle rect = tmpDirtyComponents.get(dirtyComponent);
                        // Sometimes when RepaintManager is changed during the
                        // painting
                        ...
                        if (dirtyComponent instanceof JComponent) {
                            ((JComponent) dirtyComponent).paintImmediately(rect.x, rect.y, rect.width,
                                    rect.height);
                        ...
                        // remove any components that are children of
                        // repaintRoot.
                        if (repaintRoot != null) {
                            adjustRoots(repaintRoot, roots, i + 1);
                            count.set(roots.size());
                            paintManager.isRepaintingRoot = true;
                            repaintRoot.paintImmediately(0, 0, repaintRoot.getWidth(), repaintRoot
                                    .getHeight());
    ...
    void collectDirtyComponents(Map<Component, Rectangle> dirtyComponents, Component dirtyComponent,
     ...
     * the component <code>c</code>. By default there is a double buffer per
     ...
     * with the specified component <code>c</code>. The image returned will be
     * an instance of VolatileImage, or null if a VolatileImage object could not
     * be instantiated. This buffer might be smaller than
     * <code>(proposedWidth,proposedHeight)</code>. This happens when the
     * maximum double buffer size has been set for this repaint manager.
     *
    ...
    public Image getVolatileOffscreenBuffer(Component c, int proposedWidth, int proposedHeight) {
        ...
        if (config == null) {
            config = GraphicsEnvironment.getLocalGraphicsEnvironment().getDefaultScreenDevice()
    ...
    private Image _getOffscreenBuffer(Component c, int proposedWidth, int proposedHeight) {
        ...
        if (doubleBuffer.needsReset || (doubleBuffer.image != null && (doubleBuffer.size.width < width
    ...
    private void clearImages(int width, int height) {
        if (standardDoubleBuffer != null && standardDoubleBuffer.image != null) {
            if (standardDoubleBuffer.image.getWidth(null) > width || standardDoubleBuffer.image.getHeight(
                    null) > height) {
     ...
     * default value for this property is set for optimal paint performance on
     * the given platform and it is not recommended that programs modify this
     * property directly.
     *
     * @param aFlag
     ...
     * be <code>false</code> to avoid unnecessary buffering in Swing. On
     * platforms where native double buffering is not supported, the default
     ...
     * invalid, the double buffer will then be recreated on the next invocation
     * of getOffscreenBuffer.
     */
    ...
    /**
     * Returns true if the current thread is the thread painting. This will
     * return false if no threads are painting.
     */
     ...
     *        Height
     */
    void paint(JComponent paintingComponent, JComponent bufferComponent, Graphics g, int x, int y, int w,
            int h) {
        PaintManager paintManager = getPaintManager();
        if (!isPaintingThread()) {
     ...
     *
     * @param clip
     *        Whether or not the copyArea needs to be clipped to the
     *        Component's bounds.
     */
    void copyArea(JComponent c, Graphics g, int x, int y, int w, int h, int deltaX, int deltaY,
            boolean clip) {
    ...
    /**
     * Notify the attached repaint listeners that an area of the {@code c}
     * component has been immediately repainted, that is without scheduling a
     * repaint runnable, due to performing a "blit" (via calling the
     * {@code copyArea} method).
     *
     * @param c
     *        the component
     * @param x
    ...
    /**
     * Invoked prior to any paint/copyArea method calls. This will be followed
     * by an invocation of <code>endPaint</code>. <b>WARNING</b>: Callers of
     * this method need to wrap the call in a <code>try/finally</code>,
     * otherwise if an exception is thrown during the course of painting the
     * RepaintManager may be left in a state in which the screen is not updated,
     * eg:
     * 
     * <pre>
     ...
     *
     * @param paintManager
     *        The PaintManager to use. Passing in null indicates
     *        the
     *        fallback PaintManager should be used.
     */
    ...
    private void scheduleProcessingRunnable(AppContext context) {
        if (processingRunnable.markPending()) {
            Toolkit tk = Toolkit.getDefaultToolkit();
            if (tk instanceof SunToolkit) {
                SunToolkit.getSystemEventQueueImplPP(context).postEvent(new InvocationEvent(Toolkit
                        .getDefaultToolkit(), processingRunnable));
            } else {
                Toolkit.getDefaultToolkit().getSystemEventQueue().postEvent(new InvocationEvent(Toolkit
                        .getDefaultToolkit(), processingRunnable));
            }
        }
    }
    ...
    static class PaintManager {
         ...
         */
        public boolean paint(JComponent paintingComponent, JComponent bufferComponent, Graphics g, int x,
                int y, int w, int h) {
            ...
            Image offscreen;
            if (repaintManager.useVolatileDoubleBuffer() && (offscreen = getValidImage(repaintManager
            ...
            // offscreen buffer
            if (!paintCompleted && (offscreen = getValidImage(repaintManager.getOffscreenBuffer(
        ...
        /**
         * Shows a region of a previously rendered component. This will return
         * true if successful, false otherwise. The default implementation
         * returns false.
         */
        ...
        /**
         * Invoked when the doubleBuffered or useTrueDoubleBuffering properties
         * of a JRootPane change. This may come in on any thread.
         */
         ...
         */
        protected void paintDoubleBuffered(JComponent c, Image image, Graphics g, int clipX, int clipY,
                int clipW, int clipH) {
            ...
            try {
                for (x = clipX, maxx = clipX + clipW; x < maxx; x += bw) {
                    for (y = clipY, maxy = clipY + clipH; y < maxy; y += bh) {
                        ...
                        if (volatileBufferType != Transparency.OPAQUE && osg instanceof Graphics2D) {
                        ...
                        g.setClip(x, y, bw, bh);
                        if (volatileBufferType != Transparency.OPAQUE && g instanceof Graphics2D) {
        ...
        /**
         * If <code>image</code> is non-null with a positive size it is
         ...
         * processing paint requests it'll process this request with the current
         * set of requests.
         */
     ...
     * Listener installed to detect display changes. When display changes,
     * schedules a callback to notify all RepaintManagers of the display
     * changes. Only one DisplayChangedHandler is ever installed. The singleton
     * instance will schedule notification for all AppContexts.
     */
            ...
            for (AppContext context : AppContext.getAppContexts()) {
                synchronized (context) {
                    if (!context.isDisposed()) {
                        ...
                        if (eventQueue != null) {
                            eventQueue.postEvent(new InvocationEvent(Toolkit.getDefaultToolkit(),
    ...
    private final class ProcessingRunnable implements Runnable {
        ...
        /**
         * Marks this processing runnable as pending. If this was not already
         * marked as pending, true is returned.
         */
