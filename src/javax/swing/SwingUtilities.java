...
public class SwingUtilities implements SwingConstants {
	...
	/**
	 * Indiciates if we've checked the system property for suppressing drop
	 * support.
	 */
	...
	private static boolean getSuppressDropTarget() {
		if (!checkedSuppressDropSupport) {
			suppressDropSupport = Boolean.valueOf(AccessController
					.doPrivileged(new GetPropertyAction("suppressSwingDropSupport")));
	 ...
	 * <code>destination</code> coordinate system. If <code>source</code> is
	 * {@code null}, <code>aPoint</code> is assumed to be in
	 * <code>destination</code>'s root component coordinate system. If
	 * <code>destination</code> is {@code null}, <code>aPoint</code> will be
	 * converted to <code>source</code>'s root component coordinate system. If
	...
	/**
	 * Convert the point <code>(x,y)</code> in <code>source</code> coordinate
	 * system to <code>destination</code> coordinate system. If
	 * <code>source</code> is {@code null}, <code>(x,y)</code> is assumed to be
	 * in <code>destination</code>'s root component coordinate system. If
	 * <code>destination</code> is {@code null}, <code>(x,y)</code> will be
	 * converted to <code>source</code>'s root component coordinate system. If
	 * both <code>source</code> and <code>destination</code> are {@code null},
	 ...
	 * <code>source</code> is {@code null}, <code>aRectangle</code> is assumed
	 * to be in <code>destination</code>'s root component coordinate system. If
	 * <code>destination</code> is {@code null}, <code>aRectangle</code> will be
	 * converted to <code>source</code>'s root component coordinate system. If
	 * both <code>source</code> and <code>destination</code> are {@code null},
	...
	/**
	 * Convenience method for searching above <code>comp</code> in the component
	 * hierarchy and returns the first object of <code>name</code> it finds. Can
	 * return {@code null}, if <code>name</code> cannot be found.
	 */
	 ...
	 * or none of <code>parent</code>'s visible descendents contain the
	...
	/**
	 * Returns a MouseEvent similar to <code>sourceEvent</code> except that its
	 * x and y members have been converted to <code>destination</code>'s
	 * coordinate system. If <code>source</code> is {@code null},
	 * <code>sourceEvent</code> x and y members are assumed to be into
	 * <code>destination</code>'s root component coordinate system. If
	 * <code>destination</code> is <code>null</code>, the returned MouseEvent
	 * will be in <code>source</code>'s coordinate system.
	 * <code>sourceEvent</code> will not be changed. A new event is returned.
	 * the <code>source</code> field of the returned event will be set to
	 * <code>destination</code> if destination is non-{@code null} use the
	...
	public static MouseEvent convertMouseEvent(Component source, MouseEvent sourceEvent,
		...
		if (sourceEvent instanceof MouseWheelEvent) {
			...
			newEvent = new MenuDragMouseEvent(newSource, sourceMenuDragEvent.getID(),
					sourceMenuDragEvent.getWhen(),
					sourceMenuDragEvent.getModifiers() | sourceMenuDragEvent.getModifiersEx(), p.x,
					p.y, sourceMenuDragEvent.getXOnScreen(), sourceMenuDragEvent.getYOnScreen(),
	...
	/**
	 * Return <code>true</code> if a component <code>a</code> descends from a
	...
	/**
	 * Convenience to calculate the intersection of two rectangles without
	 * allocating a new rectangle. If the two rectangles don't intersect, then
	 ...
	 *            the height of the first rectangle
	 * @param dest
	 *            the coordinates of the second rectangle; the union of the two
		...
		if (isRectangleContainingRectangle(rectA, rectB)) {
			...
			if (rectB.x <= rectA.x && rectB.y <= rectA.y) {
				...
				if ((rectB.x + rectB.width) >= (rectA.x + rectA.width)) {
					...
					t.setBounds((rectB.x + rectB.width), rectB.y,
							(rectA.x + rectA.width) - (rectB.x + rectB.width), rectB.height);
					if (t.width > 0 && t.height > 0) {
						b = new Rectangle(t);
						rectCount++;
					}
			...
			} else if (rectB.x <= (rectA.x + rectA.width)
	...
	public static String layoutCompoundLabel(JComponent c, FontMetrics fm, String text, Icon icon,
		...
		return layoutCompoundLabelImpl(c, fm, text, icon, verticalAlignment, hAlign,
	...
	/**
	 * Paints a component to the specified <code>Graphics</code>. This method is
	 * primarily useful to render <code>Component</code>s that don't exist as
	 * part of the visible containment hierarchy, but are used for rendering.
	 * For example, if you are doing your own rendering and want to render some
	 * text (or even HTML), you could make use of <code>JLabel</code>'s text
	 * rendering support and have it paint directly by way of this method,
	 * without adding the label to the visible containment hierarchy.
	 * <p>
	 * This method makes use of <code>CellRendererPane</code> to handle the
	 * actual painting, and is only recommended if you use one component for
	 * rendering. If you make use of multiple components to handle the
	 * rendering, as <code>JTable</code> does, use <code>CellRendererPane</code>
	 * directly. Otherwise, as described below, you could end up with a
	 * <code>CellRendererPane</code> per <code>Component</code>.
	 * <p>
	 * If <code>c</code>'s parent is not a <code>CellRendererPane</code>, a new
	 * <code>CellRendererPane</code> is created, <code>c</code> is added to it,
	 * and the <code>CellRendererPane</code> is added to <code>p</code>. If
	 * <code>c</code>'s parent is a <code>CellRendererPane</code> and the
	 * <code>CellRendererPane</code>s parent is not <code>p</code>, it is added
	 * to <code>p</code>.
	 * <p>
	 * The component should either descend from <code>JComponent</code> or be
	 * another kind of lightweight component. A lightweight component is one
	 * whose "lightweight" property (returned by the <code>Component</code>
	 * <code>isLightweight</code> method) is true. If the Component is not
	 * lightweight, bad things map happen: crashes, exceptions, painting
	 * problems...
	 *
	 * @param g
	...
	/**
	 * Paints a component to the specified <code>Graphics</code>. This is a
	 * cover method for
	 ...
	 * needs to update the GUI. In the following example the
	 * <code>invokeLater</code> call queues the <code>Runnable</code> object
	 * <code>doHelloWorld</code> on the event dispatching thread and then prints
	 * a message.
	 * 
	 * <pre>
	 ...
	 * example, from a JButton's ActionListener -- the <i>doRun.run()</i> will
	 * still be deferred until all pending events have been processed. Note that
	 * if the <i>doRun.run()</i> throws an uncaught exception the event
	 * dispatching thread will unwind (not the current thread).
	 * <p>
	...
	/**
	 * Causes <code>doRun.run()</code> to be executed synchronously on the AWT
	 * event dispatching thread. This call blocks until all pending AWT events
	 * have been processed and (then) <code>doRun.run()</code> returns. This
	 * method should be used when an application thread needs to update the GUI.
	 * It shouldn't be called from the event dispatching thread. Here's an
	 * example that creates a new application thread that uses
	 * <code>invokeAndWait</code> to print a string from the event dispatching
	 * thread and then, when that's finished, print a string from the
	 * application thread.
	 * 
	 * <pre>
	 ...
	 * </pre>
	 * 
	 * Note that if the <code>Runnable.run</code> method throws an uncaught
	 * exception (on the event dispatching thread) it's caught and rethrown, as
	 ...
	 *                if we're interrupted while waiting for the event
	 *                dispatching thread to finish executing
	 *                <code>doRun.run()</code>
	 * @exception InvocationTargetException
	 *                if an exception is thrown while running <code>doRun</code>
	 *
	 * @see #invokeLater
	 */
	 ...
	 * 
	 * @return the first ancestor of c that's a Window or the last Applet
	 *         ancestor
	 */
	 ...
	 * <code>JComponent</code> subclass. <code>JComponent</code> automatically
	 * processes bindings from within its <code>processKeyEvent</code> method,
	 ...
	 * <code>action</code> is enabled (and non-{@code null}). The command for
	 * the ActionEvent is determined by:
	 * <ol>
	 * <li>If the action was registered via <code>registerKeyboardAction</code>,
	 * then the command string passed in ({@code null} will be used if
	 * {@code null} was passed in).
	 * <li>Action value with name Action.ACTION_COMMAND_KEY, unless {@code null}
	 * .
	 * <li>String value of the KeyEvent, unless <code>getKeyChar</code> returns
	 ...
	 * <p>
	 * This will return {@code null} if the UI has not installed a InputMap of
	 * the specified type.
	 *
	 * @since 1.3
	 */
	...
	/**
	 * Returns true if the Icon <code>icon</code> is an instance of ImageIcon,
	...
	/**
	 * Returns index of the first occurrence of <code>mnemonic</code> within
	 ...
	 * component in <code>r</code> and returns <code>r</code>. The position and
	 * size specify the bounds of the component, adjusted so as not to include
	 * the border area (the insets). This method is useful for classes that
	...
	/**
	 * Returns the first ancestor of the {@code component} which is not an
	 * instance of {@link JLayer}.
	 *
	 * @param component
	 *            {@code Component} to get the first ancestor of, which is not a
	 ...
	 *
	 * @param viewport
	 *            {@code JViewport} to get the first descendant of, which in not
