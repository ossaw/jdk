...
/**
 * ZoneView is a View implementation that creates zones for which the child
 * views are not created or stored until they are needed for display or
 * model/view translations. This enables a substantial reduction in memory
 * consumption for situations where the model being represented is very large,
 * by building view objects only for the region being actively viewed/edited.
 * The size of the children can be estimated in some way, or calculated
 * asynchronously with only the result being saved.
 * <p>
 ...
 * this because:
 * <ul>
 * <li>Boxes are a heavily used view, and having a box that provides this
 * behavior gives substantial opportunity to plug the behavior into a view
 * hierarchy from the view factory.
 * <li>Boxes are tiled in one direction, so it is easy to divide them into zones
 * in a reliable way.
 * <li>Boxes typically have a simple relationship to the model (i.e. they create
 * child views that directly represent the child elements).
 * <li>Boxes are easier to estimate the size of than some other shapes.
 * </ul>
 * <p>
 * The default behavior is controlled by two properties, maxZoneSize and
 * maxZonesLoaded. Setting maxZoneSize to Integer.MAX_VALUE would have the
 * effect of causing only one zone to be created. This would effectively turn
 * the view into an implementation of the decorator pattern. Setting
 * maxZonesLoaded to a value of Integer.MAX_VALUE would cause zones to never be
 * unloaded. For simplicity, zones are created on boundaries represented by the
 * child elements of the element the view is responsible for. The zones can be
 * any View implementation, but the default implementation is based upon
 * AsyncBoxView which supports fairly large zones efficiently.
 *
...
public class ZoneView extends BoxView {
    ...
    /**
     * Set the desired maximum zone size. A zone may get larger than this size
     * if a single child view is larger than this size since zones are formed on
    ...
    /**
     * Get the current setting of the number of zones allowed to be loaded at
     * the same time.
     */
    ...
    /**
     * Sets the current setting of the number of zones allowed to be loaded at
     * the same time. This will throw an <code>IllegalArgumentException</code>
     * if <code>mzl</code> is less than 1.
     *
     * @param mzl
    ...
    public void setMaxZonesLoaded(int mzl) {
        if (mzl < 1) {
            throw new IllegalArgumentException(
                    "ZoneView.setMaxZonesLoaded must be greater than 0.");
        }
        maxZonesLoaded = mzl;
        unloadOldZones();
    }
    ...
    /**
     * Called by a zone when it gets loaded. This happens when an attempt is
     * made to display or perform a model/view translation on a zone that was in
     * an unloaded state. This is implemented to check if the maximum number of
    ...
    /**
     * Unload a zone (Convert the zone to its memory saving state). The zones
     * are expected to represent a subset of the child elements of the element
     * this view is responsible for. Therefore, the default implementation is to
    ...
    /**
     * Determine if a zone is in the loaded state. The zones are expected to
     * represent a subset of the child elements of the element this view is
     * responsible for. Therefore, the default implementation is to return true
     * if the view has children.
     */
    ...
    /**
     * Create a view to represent a zone for the given range within the model
     * (which should be within the range of this objects responsibility). This
     * is called by the zone management logic to create new zones. Subclasses
     * can provide a different implementation for a zone by changing this
     * method.
     *
     * @param p0
     ...
     * children directly (as they are created by the zones). This method creates
     * the initial set of zones. Zones don't actually get populated however
     * until an attempt is made to display them or to do model/view coordinate
     * translation.
     *
     * @param f
     *          the view factory
    ...
    /**
     * Returns the zone position to use for the end of a zone that starts at the
     * given position. By default this returns something close to half the max
     * zone size.
     */
    ...
    /**
     * The superclass behavior will try to update the child views which is not
     * desired in this case, since the children are zones and not directly
     ...
     * location that this view is responsible for. This is largely delegated to
     * the superclass, but is reimplemented to update the relevant zone (i.e.
     ...
     * location that this view is responsible for. This is largely delegated to
     * the superclass, but is reimplemented to update the relevant zones (i.e.
     ...
     * represent the children of the ZoneView that have been arranged in a zone.
     */
    class Zone extends AsyncBoxView {
        ...
        /**
         * Creates the child views and populates the zone with them. This is
         * done by translating the positions to child element index locations
         * and building views to those elements. If the zone is already loaded,
         * this does nothing.
         */
        public void load() {
            if (!isLoaded()) {
        ...
        /**
         * This method is reimplemented to not build the children since the
         * children are created when the zone is loaded rather then when it is
         * placed in the view hierarchy. The major span is estimated at this
         * point by building the first child (but not storing it), and calling
         * setEstimatedMajorSpan(true) followed by setSpan for the major axis
         * with the estimated span.
         */
        protected void loadChildren(ViewFactory f) {
         ...
         * <p>
         * This is reimplemented to stop the superclass behavior if the zone has
         * not yet been loaded. If the zone is unloaded for example, the last
         * seen major span is the best estimate and a calculated span for no
         * children is undesirable.
         */
        ...
        /**
         * Returns the child view index representing the given position in the
         * model. Since the zone contains a cluster of the overall set of child
         * elements, we can determine the index fairly quickly from the model by
         * subtracting the index of the start offset from the index of the
         * position given.
         *
         * @param pos
         *            the position >= 0
         ...
         * This is implemented to load the zone if its not already loaded, and
         ...
         * coordinate space of the model. This is implemented to first make sure
         ...
         * coordinate space of the view mapped to it. This is implemented to
         * provide the superclass behavior after first making sure the zone is
         * loaded (The zone must be loaded to make this calculation).
         *
         * @param pos
        ...
        /**
         * Gives notification that something was inserted into the document in a
         * location that this view is responsible for. If the zone has been
         * loaded, the superclass behavior is invoked, otherwise this does
         * nothing.
         *
         * @param e
         *          the change information from the associated document
         ...
         * a location that this view is responsible for. If the zone has been
         * loaded, the superclass behavior is invoked, otherwise this does
         * nothing.
         *
         * @param e
