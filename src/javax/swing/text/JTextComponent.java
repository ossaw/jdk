 ...
 * General Rules for Using Text Components</a>, a section in
 ...
 * <img src="doc-files/editor.gif" alt=
 * "Diagram showing interaction between Controller, Document, events, and
 * ViewFactory"
 * HEIGHT=358 WIDTH=587>
 * </p>
 *
 * <p>
...
public abstract class JTextComponent extends JComponent implements Scrollable,
	 ...
	 * editable. No layout manager is used, because layout is managed by the
	 * view subsystem of text. The document model is set to <code>null</code>.
	 */
	 ...
	 * component.
	 *
	 * @return all of this component's <code>CaretListener</code>s or an empty
	 ...
	 * parameters passed into the fire method. The listener list is processed in
	...
	public void setDocument(Document doc) {
		...
		try {
			...
			if (inputMethodRequestsHandler != null) {
				model.removeDocumentListener(
						(DocumentListener) inputMethodRequestsHandler);
			}
			model = doc;
	...
	public void setComponentOrientation(ComponentOrientation o) {
		...
		if (doc != null) {
			Boolean runDir = o.isLeftToRight() ? TextAttribute.RUN_DIRECTION_LTR
					: TextAttribute.RUN_DIRECTION_RTL;
			doc.putProperty(TextAttribute.RUN_DIRECTION, runDir);
		}
	...
	/**
	 * Sets the <code>NavigationFilter</code>. <code>NavigationFilter</code> is
	 * used by <code>DefaultCaret</code> and the default cursor movement actions
	 * as a way to restrict the cursor movement.
	 *
	 * @since 1.4
	 */
	 ...
	 *          the caret
	 * @see #getCaret
	 * @beaninfo description: the caret used to select/navigate bound: true
	 *           expert: true
	 */
	 ...
	 * desired. The highlighter can be set to <code>null</code> to disable it. A
	 * PropertyChange event ("highlighter") is fired when a new highlighter is
	 * installed.
	 *
	 * @param h
	 *          the highlighter
	 ...
	 *            the keymap
	 * @see #getKeymap
	 * @beaninfo description: set of key event to action bindings to use bound:
	 *           true
	 */
	 ...
	 * component's {@code TransferHandler} needs to be {@code non-null}. The
	 * default value of the {@code dragEnabled} property is {@code false}.
	 * <p>
	 * The job of honoring this property, and recognizing a user drag gesture,
	 * lies with the look and feel implementation, and in particular, the
	 * component's {@code TextUI}. When automatic drag handling is enabled, most
	 ...
	 * button over a selection and then moves the mouse a few pixels. Setting
	 * this property to {@code true} can therefore have a subtle effect on how
	 * selections behave.
	 * <p>
	 ...
	 * of <code>DropMode.INSERT</code> is recommended, however, for an improved
	 * user experience. It offers similar behavior of dropping between text
	 * locations, but does so without affecting the actual text selection and
	 * caret location.
	 * <p>
	...
	static {
		SwingAccessor.setJTextComponentAccessor(
				new SwingAccessor.JTextComponentAccessor() {
					public TransferHandler.DropLocation dropLocationForPoint(
							JTextComponent textComp, Point p) {
						return textComp.dropLocationForPoint(p);
					}

					public Object setDropLocation(JTextComponent textComp,
							TransferHandler.DropLocation location, Object state,
							boolean forDrop) {
						return textComp.setDropLocation(location, state,
								forDrop);
					}
				});
	}
	 ...
	 * <p>
	 * Note: This method is meant to override
	 * <code>JComponent.setDropLocation()</code>, which is package-private in
	 * javax.swing. <code>TransferHandler</code> will detect text components and
	 * call this method instead via reflection. It's name should therefore not
	 * be changed.
	 *
	 * @param location
	...
	Object setDropLocation(TransferHandler.DropLocation location, Object state,
		...
		if (dropMode == DropMode.USE_SELECTION) {
			if (textLocation == null) {
				if (state != null) {
					/*
					 * This object represents the state saved earlier. If the
					 * caret is a DefaultCaret it will be an Object array
					 * containing, in order: - the saved caret mark (Integer) -
					 * the saved caret dot (Integer) - the saved caret
					 * visibility (Boolean) - the saved mark bias
					 * (Position.Bias) - the saved dot bias (Position.Bias) If
					 * the caret is not a DefaultCaret it will be similar, but
					 * will not contain the dot or mark bias.
					 */
					Object[] vals = (Object[]) state;

					if (!forDrop) {
						if (caret instanceof DefaultCaret) {
							((DefaultCaret) caret).setDot(((Integer) vals[0])
									.intValue(), (Position.Bias) vals[3]);
							((DefaultCaret) caret).moveDot(((Integer) vals[1])
									.intValue(), (Position.Bias) vals[4]);
						} else {
				...
				if (dropLocation == null) {
					...
					if (caret instanceof DefaultCaret) {
						...
						retVal = new Object[] { Integer.valueOf(dc.getMark()),
								Integer.valueOf(dc.getDot()), Boolean.valueOf(
										visible), dc.getMarkBias(), dc
												.getDotBias() };
					} else {
						visible = caret.isVisible();
						retVal = new Object[] { Integer.valueOf(caret
								.getMark()), Integer.valueOf(caret.getDot()),
				...
				if (caret instanceof DefaultCaret) {
					((DefaultCaret) caret).setDot(textLocation.getIndex(),
							textLocation.getBias());
				} else {
	...
	/**
	 * Updates the <code>InputMap</code>s in response to a <code>Keymap</code>
	 * change.
	 * 
	 * @param oldKm
	 ...
	 *               collection
	 *               of named keymaps in the document); the name may be
	 *               <code>null</code> if the keymap is unnamed, but the caller
	 *               is
	 *               responsible for managing the reference returned as an
	 *               unnamed
	 *               keymap can't be fetched by name
	 * @param parent
	 *               the parent keymap; this may be <code>null</code> if
	 *               unspecified bindings need not be resolved in some other
	 *               keymap
	 * @return the keymap
	 */
	 ...
	 * <code>null</code> names may not be removed in this way.
	 *
	 * @param nm
	...
	/**
	 * Fetches a named keymap previously added to the document. This does not
	 * work with <code>null</code>-named keymaps.
	 *
	 * @param nm
	...
	private static HashMap<String, Keymap> getKeymapTable() {
		synchronized (KEYMAP_TABLE) {
			...
			HashMap<String, Keymap> keymapTable = (HashMap<String, Keymap>) appContext
					.get(KEYMAP_TABLE);
			if (keymapTable == null) {
	...
	/**
	 * <p>
	 * Loads a keymap with a bunch of bindings. This can be used to take a
	 * static table of definitions and load them into some keymap. The following
	 * example illustrates an example of binding some keys to the cut, copy, and
	 * paste actions associated with a JTextComponent. A code fragment to
	 * accomplish this might look as follows:
	 * 
	 * <pre>
	 * <code>
	 *
	 ...
	 *                 the set of actions
	 */
	public static void loadKeymap(Keymap map, KeyBinding[] bindings,
			Action[] actions) {
	 ...
	 *          the color
	 * @see #getCaretColor
	 * @beaninfo description: the color used to render the caret bound: true
	 *           preferred: true
	 */
	 ...
	 * @beaninfo description: color used to render selection background bound:
	 *           true preferred: true
	 */
	 ...
	 * @beaninfo description: color used to render selected text bound: true
	 *           preferred: true
	 */
	 ...
	 * @beaninfo description: color used to render disabled text bound: true
	 *           preferred: true
	 */
	 ...
	 * @return the offset &ge; 0 from the start of the document, or -1 if the
	...
	/**
	 * This is a convenience method that is only useful for <code>cut</code>,
	 ...
	 * this will attempt to install a <code>TransferHandler</code> and then use
	 * <code>altAction</code>.
	 */
	private void invokeAction(String name, Action altAction) {
		...
		action.actionPerformed(new ActionEvent(this,
				ActionEvent.ACTION_PERFORMED, (String) action.getValue(
						Action.NAME), EventQueue.getMostRecentEventTime(),
	 ...
	 *                                     <code>position</code> is less
	 *                                     than zero or greater than the
	 *                                     component's text length
	 ...
	 * platform-specific modifier key and the character given (converted to
	 * upper case). For example, the ALT key is used as a modifier on Windows
	 * and the CTRL+ALT combination is used on Mac. By default, there is no
	 * focus accelerator key. Any previous key accelerator setting will be
	 * superseded. A '\0' key setting will be registered, and has the effect of
	 * turning off the focus accelerator. When the new key is set, a
	 * PropertyChange event (FOCUS_ACCELERATOR_KEY) will be fired.
	 *
	 * @param aKey
	 *             the key
	 * @see #getFocusAccelerator
	 * @beaninfo description: accelerator character used to grab focus bound:
	 *           true
	 */
	 ...
	 * @param desc
	 *             an object describing the stream; this might be a string, a
	 ...
	 * document is <code>null</code>, does nothing. The position must be between
	 * 0 and the length of the component's text or else an exception is thrown.
	 *
	 * @param position
	 *                 the position
	 * @exception IllegalArgumentException
	 ...
	 * </code> is fired when it changes. To listen for changes to the text, use
	 ...
	 * @return the text
	 * @exception IllegalArgumentException
	 *                                     if the selection doesn't have a valid
	 *                                     mapping into the
	...
	/**
	 * Returns the boolean indicating whether this <code>TextComponent</code> is
	 * editable or not.
	 *
	...
	public void setEditable(boolean b) {
		if (b != editable) {
			...
			firePropertyChange("editable", Boolean.valueOf(oldVal), Boolean
					.valueOf(editable));
			repaint();
		}
	}
	 ...
	 * <code>moveCaretPosition</code>. The preferred way to manage selection is
	...
	/**
	 * Selects all the text in the <code>TextComponent</code>. Does nothing on a
	 * <code>null</code> or empty document.
	 */
	...
	/**
	 * Returns the string to be used as the tooltip for <code>event</code>. This
	 * will return one of:
	 * <ol>
	 * <li>If <code>setToolTipText</code> has been invoked with a non-
	 * <code>null</code> value, it will be returned, otherwise
	 * <li>The value from invoking <code>getToolTipText</code> on the UI will be
	 * returned.
	 * </ol>
	 * By default <code>JTextComponent</code> does not register itself with the
	 * <code>ToolTipManager</code>. This means that tooltips will NOT be shown
	 * from the <code>TextUI</code> unless <code>registerComponent</code> has
	...
	public String getToolTipText(MouseEvent event) {
		...
		if (retValue == null) {
			TextUI ui = getUI();
			if (ui != null) {
				retValue = ui.getToolTipText(this, new Point(event.getX(), event
						.getY()));
			}
		}
		return retValue;
	...
	/**
	 * A convenience print method that displays a print dialog, and then prints
	 * this {@code JTextComponent} in <i>interactive</i> mode with no header or
	 * footer text. Note: this method blocks until printing is done.
	 * <p>
	...
	/**
	 * A convenience print method that displays a print dialog, and then prints
	 * this {@code JTextComponent} in <i>interactive</i> mode with the specified
	 * header and footer text. Note: this method blocks until printing is done.
	 * <p>
	 * Note: In <i>headless</i> mode, no dialogs will be shown.
	 *
	 * <p>
	 ...
	 * 
	 * @param headerFormat
	 *                     the text, in {@code MessageFormat}, to be used as the
	 *                     header,
	 *                     or {@code null} for no header
	 * @param footerFormat
	 *                     the text, in {@code MessageFormat}, to be used as the
	 *                     footer,
	 *                     or {@code null} for no footer
	 * @return {@code true}, unless printing is canceled by the user
	 * @throws PrinterException
	 *                           if an error in the print system causes the job
	 *                           to be aborted
	 ...
	 *
	 * <p>
	 * {@code showPrintDialog boolean} parameter allows you to specify whether a
	 * print dialog is displayed to the user. When it is, the user may use the
	 * dialog to change printing attributes or even cancel the print.
	 *
	 * <p>
	 * {@code service} allows you to provide the initial {@code PrintService}
	 * for the print dialog, or to specify {@code PrintService} to print to when
	 * the dialog is not shown.
	 *
	 * <p>
	 * {@code attributes} can be used to provide the initial values for the
	 * print dialog, or to supply any needed attributes when the dialog is not
	 * shown. {@code attributes} can be used to control how the job will print,
	 * for example <i>duplex</i> or <i>single-sided</i>.
	 *
	 * <p>
	 * {@code interactive boolean} parameter allows you to specify whether to
	 * perform printing in <i>interactive</i> mode. If {@code true}, a progress
	 * dialog, with an abort option, is displayed for the duration of printing.
	 * This dialog is <i>modal</i> when {@code print} is invoked on the <i>Event
	 * Dispatch Thread</i> and <i>non-modal</i> otherwise. <b>Warning</b>:
	 ...
	 * being processed until printing is complete. It is only recommended when
	 * printing from an application with no visible GUI.
	 *
	 * <p>
	 * Note: In <i>headless</i> mode, {@code showPrintDialog} and
	 * {@code interactive} parameters are ignored and no dialogs are shown.
	 *
	 * <p>
	 ...
	 * dialog and then prints the {@code JTextComponent} in <i>interactive</i>
	 ...
	 * </pre>
	 * <p>
	 * Executing this code off the <i>Event Dispatch Thread</i> performs
	 * printing on the <i>background</i>. The following pattern might be used
	 * for <i>background</i> printing:
	 * 
	 * <pre>
	 ...
	 *
	 * @param headerFormat
	 *                        the text, in {@code MessageFormat}, to be used as
	 *                        the header,
	 *                        or {@code null} for no header
	 * @param footerFormat
	 *                        the text, in {@code MessageFormat}, to be used as
	 ...
	 *                        otherwise
	 * @param service
	 *                        initial {@code PrintService}, or {@code null} for
	 *                        the default
	 * @param attributes
	 *                        the job attributes to be applied to the print job,
	 *                        or
	 *                        {@code null} for none
	 * @param interactive
	 ...
	 */
	public boolean print(final MessageFormat headerFormat,
			final MessageFormat footerFormat, final boolean showPrintDialog,
		...
		final boolean isEventDispatchThread = SwingUtilities
				.isEventDispatchThread();
		final Printable textPrintable = getPrintable(headerFormat,
				footerFormat);
		if (interactive && !isHeadless) {
			printingStatus = PrintingStatus.createPrintingStatus(this, job);
			printable = printingStatus.createNotificationPrintable(
					textPrintable);
		} else {
			printingStatus = null;
		...
		/*
		 * there are three cases for printing: 1. print non interactively (!
		 * interactive || isHeadless) 2. print interactively off EDT 3. print
		 * interactively on EDT
		 * 1 and 2 prints on the current thread (3 prints on another thread) 2
		 * and 3 deal with PrintingStatusDialog
		 */
		...
		final FutureTask<Object> futurePrinting = new FutureTask<Object>(
				doPrint);
	 ...
	 * {@code JTextComponent}. The returned {@code Printable} prints the
	 * document as it looks on the screen except being reformatted to fit the
	 * paper. The returned {@code Printable} can be wrapped inside another
	 * {@code Printable} in order to create complex reports and documents.
	 *
	 *
	 * <p>
	 ...
	 * ensure that the {@code document} is not mutated while this
	 * {@code Printable} is used. Printing behavior is undefined when the
	 * {@code document} is mutated during printing.
	 *
	 * <p>
	 * Page header and footer text can be added to the output by providing
	 * {@code MessageFormat} arguments. The printing code requests
	 ...
	 *
	 * <p>
	 * The returned {@code Printable} when printed, formats the document content
	 * appropriately for the page size. For correct line wrapping the
	 * {@code imageable width} of all pages must be the same. See
	 * {@link java.awt.print.PageFormat#getImageableWidth}.
	 *
	 * <p>
	 * This method is thread-safe, although most Swing methods are not. Please
	 * see <A HREF=
	 ...
	 * the <i>Event Dispatch Thread</i>, regardless of what thread it is used
	 * on.
	 *
	 * @param headerFormat
	 *                     the text, in {@code MessageFormat}, to be used as the
	...
	public Printable getPrintable(final MessageFormat headerFormat,
		...
		return TextComponentPrintable.getPrintable(this, headerFormat,
				footerFormat);
	}
	 ...
	 * <code>JTextComponent</code>. For text components, the
	 ...
	 * <code>JTextComponent</code> class. It provides an implementation of the
	...
	public class AccessibleJTextComponent extends AccessibleJComponent
			...
			AccessibleAction, AccessibleEditableText, AccessibleExtendedText {
		...
		/**
		 * Handles caret updates (fire appropriate property change event, which
		 * are AccessibleContext.ACCESSIBLE_CARET_PROPERTY and
		 * AccessibleContext.ACCESSIBLE_SELECTION_PROPERTY). This keeps track of
		 * the dot position internally. When the caret moves, the internal
		 * position is updated after firing the event.
		 *
		 * @param e
		 *          the CaretEvent
		 */
		public void caretUpdate(CaretEvent e) {
			...
			if (caretPos != dot) {
				// the caret moved
				firePropertyChange(ACCESSIBLE_CARET_PROPERTY, new Integer(
						caretPos), new Integer(dot));
				caretPos = dot;
		...
		/**
		 * Handles document insert (fire appropriate property change event which
		 * is AccessibleContext.ACCESSIBLE_TEXT_PROPERTY). This tracks the
		 * changed offset via the event.
		 *
		 * @param e
		 *          the DocumentEvent
		 */
		 ...
		 * which is AccessibleContext.ACCESSIBLE_TEXT_PROPERTY). This tracks the
		 * changed offset via the event.
		 *
		 * @param e
		 *          the DocumentEvent
		...
		/**
		 * Gets the state set of the JTextComponent. The AccessibleStateSet of
		 * an object is composed of a set of unique AccessibleState's. A change
		 * in the AccessibleStateSet of an object will cause a
		 * PropertyChangeEvent to be fired for the
		 * AccessibleContext.ACCESSIBLE_STATE_PROPERTY property.
		 *
		 ...
		 * this object, which is responsible for implementing the AccessibleText
		...
		/**
		 * Many of these methods are just convenience methods; they just call
		 * the equivalent on the parent
		 */
		...
		/**
		 * Gets the editor's drawing rectangle. Stolen from the unfortunately
		 * named BasicTextUI.getVisibleEditorRect()
		 *
		 ...
		 * index is invalid a null rectangle is returned.
		 *
		 * The screen coordinates returned are "unscrolled coordinates" if the
		 * JTextComponent is contained in a JScrollPane in which case the
		 * resulting rectangle should be composed with the parent coordinates. A
		 * good algorithm to use is:
		 * 
		 * <pre>
		 * Accessible a:
		 ...
		 * </pre>
		 *
		 * Note: the JTextComponent must have a valid size (e.g. have been added
		 * to a parent container whose ancestor container is a valid top-level
		 * window) for this method to be able to return a meaningful (non-null)
		 * value.
		 *
		 * @param i
		 *          the index into the String &ge; 0
		 ...
		 *
		 * Note: The character to the right of the caret will have the same
		 * index value as the offset (the caret is between two characters).
		 *
		 ...
		 * the same. Return 0 if the text is empty, or the caret position if no
		 * selection.
		 *
		...
		/**
		 * IndexedSegment extends Segment adding the offset into the the model
		 * the <code>Segment</code> was asked for.
		 */
		...
		// TIGER - 4170173
		/**
		 * Returns the String at a given index. Whitespace between words is
		 * treated as a word.
		 *
		 * @param part
		...
		/**
		 * Returns the String after a given index. Whitespace between words is
		 * treated as a word.
		 *
		 * @param part
		 *              the CHARACTER, WORD, or SENTENCE to retrieve
		...
		/**
		 * Returns the String before a given index. Whitespace between words is
		 * treated a word.
		 *
		 * @param part
		 *              the CHARACTER, WORD, or SENTENCE to retrieve
		...
		/**
		 * Gets the word, sentence, or character at <code>index</code>. If
		 * <code>direction</code> is non-null this will find the next/previous
		...
		/**
		 * Returns the Segment at <code>index</code> representing either the
		 * paragraph or sentence as identified by <code>part</code>, or null if
		 * a valid paragraph/sentence can't be found. The offset will point to
		 * the start of the word/sentence in the array, and the modelOffset will
		 * point to the location of the word/sentence in the model.
		 */
		...
		/**
		 * Pastes the text from the system clipboard into the text starting at
		 * the specified index.
		 *
		 * @param startIndex
		 *                   the starting index in the text
		...
		/**
		 * Returns the AccessibleTextSequence at a given <code>index</code>. If
		 * <code>direction</code> is non-null this will find the next/previous
		 * word/sentence/character.
		 *
		 * @param part
		 ...
		 * @since 1.6
		 */
		private AccessibleTextSequence getSequenceAtIndex(int part, int index,
				int direction) {
			if (index < 0 || index >= model.getLength()) {
				return null;
			}
			...
			switch (part) {
				case AccessibleText.CHARACTER:
					...
					try {
						if (index + direction < model.getLength() && index
								+ direction >= 0) {
							charSequence = new AccessibleTextSequence(index
									+ direction, index + direction + 1, model
											.getText(index + direction, 1));
						}
				...
				case AccessibleText.SENTENCE:
					...
					try {
						...
						if (seg != null) {
							...
							if (seg != null && (seg.offset + seg.count) <= model
									.getLength()) {
								rangeSequence = new AccessibleTextSequence(
										seg.offset, seg.offset + seg.count,
										new String(seg.array, seg.offset,
												seg.count));
				...
				case AccessibleExtendedText.LINE:
					...
					try {
						int startIndex = Utilities.getRowStart(
								JTextComponent.this, index);
						int endIndex = Utilities.getRowEnd(JTextComponent.this,
								index);
						if (startIndex >= 0 && endIndex >= startIndex) {
							if (direction == 0) {
								lineSequence = new AccessibleTextSequence(
										startIndex, endIndex, model.getText(
												startIndex, endIndex
														- startIndex + 1));
							} else if (direction == -1 && startIndex > 0) {
								endIndex = Utilities.getRowEnd(
										JTextComponent.this, startIndex - 1);
								startIndex = Utilities.getRowStart(
										JTextComponent.this, startIndex - 1);
								if (startIndex >= 0 && endIndex >= startIndex) {
									lineSequence = new AccessibleTextSequence(
											startIndex, endIndex, model.getText(
							...
							} else if (direction == 1 && endIndex < model
									.getLength()) {
								startIndex = Utilities.getRowStart(
										JTextComponent.this, endIndex + 1);
								endIndex = Utilities.getRowEnd(
										JTextComponent.this, endIndex + 1);
								if (startIndex >= 0 && endIndex >= startIndex) {
									lineSequence = new AccessibleTextSequence(
					...
					try {
						...
						switch (direction) {
							case -1:
								...
								// (off-by-one counting)
								attributeRunEndIndex = getRunEdge(index,
										direction);
							...
							case 1:
								...
								// (off-by-one counting)
								attributeRunStartIndex = getRunEdge(index,
										direction);
						...
						attributeRunStartIndex = (attributeRunStartIndex != Integer.MIN_VALUE)
								? attributeRunStartIndex
								: getRunEdge(tempIndex, -1);
						attributeRunEndIndex = (attributeRunEndIndex != Integer.MIN_VALUE)
		 ...
		 * <code>direction</code>, return the edge of run that shares the same
		 * <code>AttributeSet</code> and parent element as those at
		 * <code>index</code>.
		 *
		 * Note: we assume the document is already locked...
		 */
		private int getRunEdge(int index, int direction)
				throws BadLocationException {
			if (index < 0 || index >= model.getLength()) {
			...
			for (indexElement = model.getDefaultRootElement(); !indexElement
					.isLeaf();) {
			...
			switch (direction) {
				...
				case 1:
					...
					while ((edgeElementIndex + direction) > 0
							...
							&& parent.getElement(edgeElementIndex + direction)
		...
		/**
		 * Returns the <code>Rectangle</code> enclosing the text between two
		 * indicies.
		 *
		 * @param startIndex
		...
		public boolean doAccessibleAction(int i) {
			...
			ActionEvent ae = new ActionEvent(JTextComponent.this,
					ActionEvent.ACTION_PERFORMED, null, EventQueue
	...
	/**
	 * The caret used to display the insert position and navigate throughout the
	 * document.
	 *
	 ...
	 */
	private static Cache<Class<?>, Boolean> METHOD_OVERRIDDEN = new Cache<Class<?>, Boolean>(
			Cache.Kind.WEAK, Cache.Kind.STRONG) {
		/**
		 * Returns {@code true} if the specified {@code type} extends
		 * {@link JTextComponent} and the
		 * {@link JTextComponent#processInputMethodEvent} method is overridden.
		 */
		@Override
	...
	/**
	 * Returns a string representation of this <code>JTextComponent</code>. This
	...
	protected String paramString() {
		...
		String caretColorString = (caretColor != null ? caretColor.toString()
				: "");
		String selectionColorString = (selectionColor != null ? selectionColor
				.toString() : "");
		...
		return super.paramString() + ",caretColor=" + caretColorString
				+ ",disabledTextColor=" + disabledTextColorString + ",editable="
				+ editableString + ",margin=" + marginString
	...
	static final JTextComponent getFocusedComponent() {
		return (JTextComponent) AppContext.getAppContext().get(
				FOCUSED_COMPONENT);
	}
	...
	private static final Object KEYMAP_TABLE = new StringBuilder(
	...
	static class DefaultKeymap implements Keymap {
		...
		public KeyStroke[] getBoundKeyStrokes() {
			...
			int i = 0;
			for (Enumeration<KeyStroke> e = bindings.keys(); e
					.hasMoreElements();) {
		...
		public Action[] getBoundActions() {
			...
			int i = 0;
			for (Enumeration<Action> e = bindings.elements(); e
					.hasMoreElements();) {
		...
		public KeyStroke[] getKeyStrokesForAction(Action a) {
			...
			for (Enumeration<KeyStroke> keys = bindings.keys(); keys
					.hasMoreElements();) {
			...
			if (parent != null) {
				...
				if (pStrokes != null) {
					...
					// are locally defined.
					int rCount = 0;
					for (int counter = pStrokes.length
							- 1; counter >= 0; counter--) {
					...
					if (rCount > 0 && rCount < pStrokes.length) {
						...
						if (keyStrokes == null) {
							...
							System.arraycopy(pStrokes, 0, retValue, keyStrokes
									.size(), pStrokes.length);
							keyStrokes = null;
						}
					}
				}
	...
	/**
	 * KeymapWrapper wraps a Keymap inside an InputMap. For KeymapWrapper to be
	 * useful it must be used with a KeymapActionMap. KeymapWrapper for the most
	 * part, is an InputMap with two parents. The first parent visited is ALWAYS
	 * the Keymap, with the second parent being the parent inherited from
	 * InputMap. If <code>keymap.getAction</code> returns null, implying the
	 * Keymap does not have a binding for the KeyStroke, the parent is then
	 * visited. If the Keymap has a binding, the Action is returned, if not and
	 * the KeyStroke represents a KeyTyped event and the Keymap has a
	 * defaultAction, <code>DefaultActionKey</code> is returned.
	 * <p>
	 * KeymapActionMap is then able to transate the object passed in to either
	 * message the Keymap, or message its default implementation.
	 */
	static class KeymapWrapper extends InputMap {
		...
		public Object get(KeyStroke keyStroke) {
			...
			if (retValue == null) {
				...
				if (retValue == null && keyStroke
						.getKeyChar() != KeyEvent.CHAR_UNDEFINED && keymap
	...
	/**
	 * Wraps a Keymap inside an ActionMap. This is used with a KeymapWrapper. If
	 * <code>get</code> is passed in <code>KeymapWrapper.DefaultActionKey</code>
	 * , the default action is returned, otherwise if the key is an Action, it
	 * is returned.
	 */
		...
		public Object[] keys() {
			...
			if (sCount == 0) {
				if (hasDefault) {
					...
					if (keymapCount > 1) {
						System.arraycopy(keymapKeys, 0, retValue, 0, keymapCount
								- 1);
					}
			...
			if (hasDefault) {
				...
				retValue[sCount + keymapCount
						- 1] = KeymapWrapper.DefaultActionKey;
			} else {
				System.arraycopy(keymapKeys, 0, retValue, sCount, keymapCount);
			}
			return retValue;
	...
	/**
	 * The default keymap that will be shared by all <code>JTextComponent</code>
	 * instances unless they have had a different keymap set.
	 */
	public static final String DEFAULT_KEYMAP = "default";

	/**
	 * Event to use when firing a notification of change to caret position. This
	 * is mutable so that the event can be reused since caret events can be
	 * fairly high in bandwidth.
	 */
	static class MutableCaretEvent extends CaretEvent implements ChangeListener,
		...
		/**
		 * Requests focus on the associated text component, and try to set the
		 * cursor position.
		 *
		 * @param e
		 *          the mouse event
				...
				if (beginIndex < composedStartIndex) {
					if (endIndex <= composedStartIndex) {
						...
						committed = getText(beginIndex, firstPartLength)
								+ getText(composedEndIndex, endIndex
										- beginIndex - firstPartLength);
					}
				} else {
					committed = getText(beginIndex + (composedEndIndex
							- composedStartIndex), endIndex - beginIndex);
				}
			} catch (BadLocationException ble) {
			...
			if (doc != null) {
				...
				if (composedTextContent != null) {
					if (composedTextEnd == null || composedTextStart == null) {
						/*
						 * fix for : 6355666 this is the case when this method
						 * is invoked from DocumentListener. At this point
						 * composedTextEnd and composedTextStart are not defined
						 * yet.
						 */
	...
	private void replaceInputMethodText(InputMethodEvent e) {
		...
		if (composedTextExists()) {
			try {
				doc.remove(composedTextStart.getOffset(), composedTextEnd
						.getOffset() - composedTextStart.getOffset());
			} catch (BadLocationException ble) {
			}
		...
		if (text != null) {
			...
			if (commitCount > 0) {
				...
				// Need to generate KeyTyped events for the committed text for
				// components
				// that are not aware they are active input method clients.
				if (shouldSynthensizeKeyEvents()) {
					for (char c = text.current(); commitCount > 0; c = text
							.next(), commitCount--) {
			...
			if (committedTextStartIndex != committedTextEndIndex) {
				try {
					latestCommittedTextStart = doc.createPosition(
							committedTextStartIndex);
					latestCommittedTextEnd = doc.createPosition(
	...
	private void createComposedTextAttribute(int composedIndex,
		...
		for (char c = text.setIndex(
				composedIndex); c != CharacterIterator.DONE; c = text.next()) {
			strBuf.append(c);
		}
	 ...
	 *
	 * The composed text (if any) around the specified position is saved in a
	 * backing store and removed from the document.
	 *
	 * @param pos
	...
	protected boolean saveComposedText(int pos) {
		if (composedTextExists()) {
			...
			int len = composedTextEnd.getOffset() - composedTextStart
					.getOffset();
		...
		if (binding != null) {
			...
			if (a != null) {
				ActionEvent ae = new ActionEvent(this,
						ActionEvent.ACTION_PERFORMED, committedText, EventQueue
								.getMostRecentEventTime(),
						getCurrentEventModifiers());
			...
			if (caretPos != null) {
				...
				if (index == 0) {
					...
					try {
						...
						Rectangle end = modelToView(composedTextEnd
								.getOffset());
	...
	private boolean shouldSynthensizeKeyEvents() {
		if (!checkedInputOverride) {
			...
			// If it is overridden, need not to generate KeyTyped events for
			// committed text.
	...
	class ComposedTextCaret extends DefaultCaret implements Serializable {
		...
		public void install(JTextComponent c) {
			...
			if (doc instanceof StyledDocument) {
				StyledDocument sDoc = (StyledDocument) doc;
				Element elem = sDoc.getCharacterElement(c.composedTextStart
						.getOffset());
		...
		protected void positionCaret(MouseEvent me) {
			...
			if ((offset < composedStartIndex) || (offset > composedTextEnd
					.getOffset())) {
				try {
					...
					// Post a caret positioning runnable to assure that the
					// positioning
					// occurs *after* committing the composed text.
					EventQueue.invokeLater(new DoSetCaretPosition(host,
							newPos));
				} catch (BadLocationException ble) {
