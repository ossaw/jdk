...
/**
 * A flow strategy that uses java.awt.font.LineBreakMeasureer to produce
 * java.awt.font.TextLayout for i18n capable rendering. If the child view being
 * placed into the flow is of type GlyphView and can be rendered by TextLayout,
 * a GlyphPainter that uses TextLayout is plugged into the GlyphView.
 *
...
class TextLayoutStrategy extends FlowView.FlowStrategy {
    ...
    /**
     * Constructs a layout strategy for paragraphs based upon
     ...
     * location that the given flow view is responsible for. The strategy should
     * update the appropriate changed region (which depends upon the strategy
     * used for repair).
     *
     * @param e
    ...
    /**
     * Does a a full layout on the given View. This causes all of the rows
     * (child views) to be rebuilt to match the given constraints for each row.
     * This is called by a FlowView.layout to update the child views in the
     * flow.
     *
     * @param fv
     *        the view to reflow
     ...
     * This is implemented to execute the superclass functionality (which fills
     * the row with child views or view fragments) and follow that with bidi
     * reordering of the unidirectional view fragments.
     *
     * @param row
     *        the row to fill in with views. This is assumed to be empty on
     *        entry.
     * @param pos
    ...
    /**
     * Adjusts the given row if possible to fit within the layout span. Since
     * all adjustments were already calculated by the LineBreakMeasurer, this is
     ...
     * chunk. This can be either an entire view from the logical view, or a
    ...
    protected View createView(FlowView fv, int startOffset, int spanLeft, int rowIndex) {
        ...
        if ((frag instanceof GlyphView) && (measurer != null)) {
            ...
            TextLayout tl = (isTab) ? null
                    : measurer.nextLayout(spanLeft, text.toIteratorIndex(endOffset), requireNextWord);
            if (tl != null) {
    ...
    /**
     * Calculate the limiting offset for the next view fragment. At most this
     * would be the entire view (i.e. the limiting offset would be the end
     * offset in that case). If the range contains a tab or a direction change,
     * that will limit the offset to something less. This value is then fed to
     * the LineBreakMeasurer as a limit to consider in addition to the remaining
     * span.
     *
     * @param v
    ...
    /**
     * Synchronize the strategy with its FlowView. Allows the strategy to update
     * its state to account for changes in that portion of the model represented
     * by the FlowView. Also allows the strategy to update the FlowView in
    ...
    static class AttributedSegment extends Segment implements AttributedCharacterIterator {
        ...
        /**
         * Get a boundary position for the font. This is implemented to assume
         * that two fonts are equal if their references are equal (i.e. that the
         * font came from a cache).
         *
         * @return the location in model coordinates. This is not the same as
         *         the Segment coordinates.
         */
        int getFontBoundary(int childIndex, int dir) {
            View child = v.getView(childIndex);
            Font f = getFont(childIndex);
            for (childIndex += dir; (childIndex >= 0) && (childIndex < v.getViewCount()); childIndex += dir) {
        ...
        /**
         * Returns the keys of all attributes defined on the iterator's text
         * range. The set is empty if no attributes are defined.
         */
