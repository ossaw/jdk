...
/**
 * An implementation of the AbstractDocument.Content interface implemented using
 * a gapped buffer similar to that used by emacs. The underlying storage is a
 * array of unicode characters with a gap somewhere. The gap is moved to the
 * location of changes to take advantage of common behavior where most changes
 * are in the same location. Changes that occur at a gap boundary are generally
 * cheap and moving the gap is generally cheaper than moving the array contents
 * directly to accommodate the change.
 * <p>
 * The positions tracking change are also generally cheap to maintain. The
 * Position implementations (marks) store the array index and can easily
 * calculate the sequential position from the current gap location. Changes only
 * require update to the the marks between the old and new gap boundaries when
 * the gap is moved, so generally updating the marks is pretty cheap. The marks
 * are stored sorted so they can be located quickly with a binary search. This
 * increases the cost of adding a mark, and decreases the cost of keeping the
 * mark updated.
 *
...
public class GapContent extends GapVector implements AbstractDocument.Content, Serializable {
    ...
    /**
     * Creates a new GapContent object, with the initial size specified. The
     * initial size will not be allowed to go below 2, to give room for the
    ...
    /**
     * Allocate an array to store items of the type appropriate (which is
    ...
    /**
     * Retrieves a portion of the content. If the desired content spans the gap,
     * we copy the content. If the desired content does not span the gap, the
    ...
    public Position createPosition(int offset) throws BadLocationException {
        ...
        if (sortIndex < marks.size() && (m = marks.elementAt(sortIndex)).index == index && (position = m
                .getPosition()) != null) {
    ...
    /**
     * Holds the data for a mark... separately from the real mark so that the
     * real mark (Position that the caller of createPosition holds) can be
     * collected if there are no more references to it. The update table holds
     * only a reference to this data.
     */
    final class MarkData extends WeakReference<StickyPosition> {
        ...
        /**
         * Fetch the location in the contiguous sequence being modeled. The
         * index in the gap array is held by the mark, so it is adjusted
         * according to it's relationship to the gap.
         */
    ...
    /**
     * Record used for searching for the place to start updating mark indexs
     * when the gap boundaries are moved.
     */
    ...
    /**
     * Make the gap bigger, moving any necessary data and updating the
     * appropriate marks
     */
    ...
    /**
     * Move the start of the gap to a new location, without changing the size of
     * the gap. This moves the data in the array and updates the marks
     * accordingly.
     */
    ...
    /**
     * Resets all the marks that have an offset of 0 to have an index of zero as
     * well.
     */
    ...
    /**
     * Adjust the gap end downward. This doesn't move any data, but it does
     * update any marks affected by the boundary change. All marks from the old
     * gap start down to the new gap start are squeezed to the end of the gap
     * (their location has been removed).
     */
    ...
    /**
     * Adjust the gap end upward. This doesn't move any data, but it does update
     * any marks affected by the boundary change. All marks from the old gap end
     * up to the new gap end are squeezed to the end of the gap (their location
     * has been removed).
     */
    ...
    static class MarkVector extends GapVector {
        ...
        /**
         * Replaces the elements in the specified range with the passed in
         * objects. This will NOT adjust the gap. The passed in indices do not
         * account for the gap, they are the same as would be used int
         * <code>elementAt</code>.
         */
    ...
    /**
     * Returns a Vector containing instances of UndoPosRef for the Positions in
     * the range <code>offset</code> to <code>offset</code> +
     * <code>length</code>. If <code>v</code> is not null the matching Positions
     * are placed in there. The vector with the resulting Positions are
     * returned.
     *
     * @param v
    ...
    /**
     * Used to hold a reference to a Mark that is being reset as the result of
     * removing from the content.
     */
    final class UndoPosRef {
        ...
        /**
         * Resets the location of the Position to the offset when the receiver
         * was instantiated.
         *
         * @param endOffset
    ...
    class InsertUndo extends AbstractUndoableEdit {
        ...
        /**
         * An array of instances of UndoPosRef for the Positions in the range
         * that was removed, valid after undo.
         */
    ...
    class RemoveUndo extends AbstractUndoableEdit {
        ...
        /**
         * An array of instances of UndoPosRef for the Positions in the range
         * that was removed, valid before undo.
         */
        protected Vector posRefs;
