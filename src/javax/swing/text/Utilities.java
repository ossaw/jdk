...
public class Utilities {
	 ...
	 */
	public static final int drawTabbedText(Segment s, int x, int y, Graphics g, TabExpander e,
			int startOffset) {
	 ...
	 * @return the X location at the end of the rendered text
	 */
	static final int drawTabbedText(View view, Segment s, int x, int y, Graphics g, TabExpander e,
			int startOffset) {
	...
	static final int drawTabbedText(View view, Segment s, int x, int y, Graphics g, TabExpander e,
		...
		for (int i = txtOffset; i < n; i++) {
			if (txt[i] == '\t'
				...
				if (flushLen > 0) {
					nextX = SwingUtilities2.drawChars(component, g, txt, flushIndex, flushLen, x,
							y);
					flushLen = 0;
				}
				flushIndex = i + 1;
	 ...
	 */
	public static final int getTabbedTextWidth(Segment s, FontMetrics metrics, int x, TabExpander e,
			int startOffset) {
	 ...
	 *            the font metrics to use for the calculation
	 * @param x0
	 *            the starting view location representing the start of the given
	 *            text &gt;= 0.
	 * @param x
	 *            the target view location to translate to an offset into the
	 *            text &gt;= 0.
	 * @param e
	 *            how to expand the tabs. If this value is null, tabs will be
	 *            expanded as a space character.
	...
	static final int getTabbedTextOffset(View view, Segment s, FontMetrics metrics, int x0, int x,
			TabExpander e, int startOffset, int[] justificationData) {
	...
	// if null not justification is needed
	static final int getTabbedTextOffset(View view, Segment s, FontMetrics metrics, int x0, int x,
			TabExpander e, int startOffset, boolean round, int[] justificationData) {
		if (x0 >= x) {
			// x before x0, return.
			return 0;
		}
		...
		for (int i = s.offset; i < n; i++) {
			...
			if (x < nextX) {
				...
				if (round) {
					...
					if (span < width) {
						while (offset > 0) {
							int nextWidth = offset > 1
									? metrics.charsWidth(txt, txtOffset, offset - 1) : 0;
	...
	public static final int getNextWord(JTextComponent c, int offs) throws BadLocationException {
		...
		for (nextWord = getNextWordInParagraph(c, line, offs,
				false); nextWord == BreakIterator.DONE; nextWord = getNextWordInParagraph(c, line,
						offs, true)) {
	...
	public static final int getPreviousWord(JTextComponent c, int offs)
		...
		for (prevWord = getPrevWordInParagraph(c, line,
				offs); prevWord == BreakIterator.DONE; prevWord = getPrevWordInParagraph(c, line,
						offs)) {
	...
	static int drawComposedText(View view, AttributeSet attr, Graphics g, int x, int y, int p0,
		...
		AttributedString as = (AttributedString) attr
				.getAttribute(StyleConstants.ComposedTextAttribute);
	...
	static void paintComposedText(Graphics g, Rectangle alloc, GlyphView v) {
		if (g instanceof Graphics2D) {
			...
			AttributedString as = (AttributedString) attrSet
					.getAttribute(StyleConstants.ComposedTextAttribute);
	...
	static int getNextVisualPositionFrom(View v, int pos, Position.Bias b, Shape alloc,
		...
		if (pos == -1) {
			...
			if (retValue == -1 && !top && v.getViewCount() > 1) {
				...
				retValue = child.getNextVisualPositionFrom(-1, biasRet[0], childBounds, direction,
						biasRet);
			}
		} else {
			...
			if (retValue == -1 && childIndex >= 0 && childIndex < v.getViewCount()) {
				...
				int nextPos = child.getNextVisualPositionFrom(-1, b, childBounds, direction,
						biasRet);
				if (biasRet[0] == b) {
					retValue = nextPos;
				} else {
