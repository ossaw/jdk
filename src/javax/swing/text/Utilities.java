...
public class Utilities {
    /**
     * If <code>view</code>'s container is a <code>JComponent</code> it is
     * returned, after casting.
     */
     ...
     * @param e
     *                    how to expand the tabs. If this value is null, tabs
     *                    will be
     ...
     *
     * @param view
     *                    View requesting rendering, may be null.
     * @param s
     *                    the source of the text
     * @param x
     *                    the X origin &gt;= 0
     ...
     * @return the X location at the end of the rendered text
     */
    static final int drawTabbedText(View view, Segment s, int x, int y,
    ...
    static final int drawTabbedText(View view, Segment s, int x, int y,
        ...
        if (justificationData != null) {
            ...
            spaceAddonLeftoverEnd = justificationData[Row.SPACE_ADDON_LEFTOVER_END]
                    + offset;
            startJustifiableContent = justificationData[Row.START_JUSTIFIABLE]
                    + offset;
            endJustifiableContent = justificationData[Row.END_JUSTIFIABLE]
                    + offset;
        }
        int n = s.offset + s.count;
        for (int i = txtOffset; i < n; i++) {
            if (txt[i] == '\t' || ((spaceAddon != 0
                    || i <= spaceAddonLeftoverEnd) && (txt[i] == ' ')
    ...
    public static final int getTabbedTextWidth(Segment s, FontMetrics metrics,
            int x, TabExpander e, int startOffset) {
    ...
    /**
     * Determines the relative offset into the given text that best represents
     * the given span in the view coordinate system. This is implemented in a
     * 1.1 style coordinate system where ints are used and 72dpi is assumed.
     *
     * @param s
     *                    the source of the text
     * @param metrics
     *                    the font metrics to use for the calculation
     * @param x0
     *                    the starting view location representing the start of
     *                    the given
     *                    text &gt;= 0.
     * @param x
     *                    the target view location to translate to an offset
     *                    into the
     *                    text &gt;= 0.
     * @param e
     *                    how to expand the tabs. If this value is null, tabs
    ...
    static final int getTabbedTextOffset(View view, Segment s,
            FontMetrics metrics, int x0, int x, TabExpander e, int startOffset,
            int[] justificationData) {
        return getTabbedTextOffset(view, s, metrics, x0, x, e, startOffset,
                true, justificationData);
    }

    public static final int getTabbedTextOffset(Segment s, FontMetrics metrics,
            int x0, int x, TabExpander e, int startOffset, boolean round) {
        ...
        for (int i = s.offset; i < n; i++) {
            ...
            if (x < nextX) {
                ...
                if (round) {
                    ...
                    if (span < width) {
                        while (offset > 0) {
                            int nextWidth = offset > 1 ? metrics.charsWidth(txt,
                                    txtOffset, offset - 1) : 0;
                    ...
                    while (offset > 0 && metrics.charsWidth(txt, txtOffset,
                            offset) > (x - x0)) {
                        offset--;
                    }
                }
     ...
     * This tries to find a word boundary.
     * 
     * @param s
     *                    the source of the text
     * @param metrics
    ...
    public static final int getBreakLocation(Segment s, FontMetrics metrics,
        ...
        int txtOffset = s.offset;
        int txtCount = s.count;
        int index = Utilities.getTabbedTextOffset(s, metrics, x0, x, e,
                startOffset, false);
    ...
    public static final int getNextWord(JTextComponent c, int offs)
        ...
        for (nextWord = getNextWordInParagraph(c, line, offs,
                false); nextWord == BreakIterator.DONE; nextWord = getNextWordInParagraph(
                        c, line, offs, true)) {
    ...
    /**
     * Finds the next word in the given elements text. The first parameter
     * allows searching multiple paragraphs where even the first offset is
     * desired. Returns the offset of the next word, or BreakIterator.DONE if
     * there are no more words in the element.
     */
    static int getNextWordInParagraph(JTextComponent c, Element line, int offs,
        ...
        if ((wordPosition == BreakIterator.DONE) || (wordPosition >= seg.offset
                + seg.count)) {
    ...
    public static final int getPreviousWord(JTextComponent c, int offs)
        ...
        int prevWord;
        Element line = getParagraphElement(c, offs);
        for (prevWord = getPrevWordInParagraph(c, line,
                offs); prevWord == BreakIterator.DONE; prevWord = getPrevWordInParagraph(
                        c, line, offs)) {
    ...
    /**
     * Finds the previous word in the given elements text. The first parameter
     * allows searching multiple paragraphs where even the first offset is
     * desired. Returns the offset of the next word, or BreakIterator.DONE if
     * there are no more words in the element.
     */
    static int getPrevWordInParagraph(JTextComponent c, Element line, int offs)
        ...
        BreakIterator words = BreakIterator.getWordInstance(c.getLocale());
        words.setText(seg);
        if (words.following(seg.offset + offs
                - lineStart) == BreakIterator.DONE) {
            words.last();
        }
    ...
    public static final Element getParagraphElement(JTextComponent c,
        ...
        if ((offs >= paragraph.getStartOffset()) && (offs < paragraph
                .getEndOffset())) {
            return paragraph;
        }
        return null;
     ...
     *             View hosting text
     * @param attr
     *             the attributes containing the composed text
     * @param g
     *             the graphics context
     * @param x
     *             the X origin
     ...
     *             the Y origin
     * @param p0
     *             starting offset in the composed text to be rendered
     * @param p1
     *             ending offset in the composed text to be rendered
     * @return the new insertion position
     */
    static int drawComposedText(View view, AttributeSet attr, Graphics g, int x,
        ...
        return x + (int) SwingUtilities2.drawString(getJComponent(view), g2d,
                aci, x, y);
    }
    ...
    static void paintComposedText(Graphics g, Rectangle alloc, GlyphView v) {
        if (g instanceof Graphics2D) {
            ...
            AttributedString as = (AttributedString) attrSet.getAttribute(
                    StyleConstants.ComposedTextAttribute);
            int start = v.getElement().getStartOffset();
            int y = alloc.y + alloc.height - (int) v.getGlyphPainter()
                    .getDescent(v);
            int x = alloc.x;
            ...
            // draw
            AttributedCharacterIterator aci = as.getIterator(null, p0 - start,
                    p1 - start);
     ...
     * <p>
     * This implementation assumes the views are layed out in a logical manner.
     * That is, that the view at index x + 1 is visually after the View at index
     * x, and that the View at index x - 1 is visually before the View at x.
     * There is support for reversing this behavior only if the passed in
     * <code>View</code> is an instance of <code>CompositeView</code>. The
     * <code>CompositeView</code> must then override the
    ...
    static int getNextVisualPositionFrom(View v, int pos, Position.Bias b,
        ...
        if (pos == -1) {
            ...
            if ((direction == SwingConstants.EAST
                    || direction == SwingConstants.WEST)
                    && (v instanceof CompositeView) && ((CompositeView) v)
                            .flipEastAndWestAtEnds(pos, b)) {
                increment *= -1;
            }
            childIndex += increment;
            if (retValue == -1 && childIndex >= 0 && childIndex < v
                    .getViewCount()) {
                child = v.getView(childIndex);
                childBounds = v.getChildAllocation(childIndex, alloc);
                retValue = child.getNextVisualPositionFrom(-1, b, childBounds,
                        direction, biasRet);
                ...
                if (retValue == pos && biasRet[0] != b) {
                    return getNextVisualPositionFrom(v, pos, biasRet[0], alloc,
                            direction, biasRet);
                }
            } else if (retValue != -1 && biasRet[0] != b && ((increment == 1
                    && child.getEndOffset() == retValue) || (increment == -1
                ...
                int nextPos = child.getNextVisualPositionFrom(-1, b,
                        childBounds, direction, biasRet);
                if (biasRet[0] == b) {
                    retValue = nextPos;
                } else {
