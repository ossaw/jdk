...
/**
 * This is the view associated with the html tag NOFRAMES. This view has been
 * written to ignore the contents of the NOFRAMES tag. The contents of the tag
 * will only be visible when the JTextComponent the view is contained in is
 * editable.
 *
 * @author Sunita Mani
 */
class NoFramesView extends BlockView {
	...
	/**
	 * If this view is not visible, then it returns. Otherwise it invokes the
	 * superclass.
	 *
	 * @param g
	...
	public void paint(Graphics g, Shape allocation) {
		...
		if (host != null && visible != ((JTextComponent) host).isEditable()) {
	...
	/**
	 * Determines if the JTextComponent that the view is contained in is
	 * editable. If so, then this view and all its child views are visible. Once
	 * this has been determined, the superclass is invoked to continue
	 * processing.
	 *
	 * @param p
	 *            the parent View.
	...
	/**
	 * Returns a true/false value that represents whether the view is visible or
	 * not.
	 */
	...
	/**
	 * Do nothing if the view is not visible, otherwise invoke the superclass to
	 * perform layout.
	 */
