...
public class HTMLWriter extends AbstractWriter {
	...
	public void write() throws IOException, BadLocationException {
		...
		while ((next = it.next()) != null) {
			if (!inRange(next)) {
				if (completeDoc && next.getAttributes()
	...
	protected void emptyTag(Element elem) throws BadLocationException, IOException {
		...
		if (matchNameAttribute(attr, HTML.Tag.CONTENT)) {
			...
			Object nameTag = (attr != null) ? attr.getAttribute(StyleConstants.NameAttribute)
					: null;
	 ...
	 * vector. If the tag is not found, then the tag is removed from the vector
