...
public class ImageView extends View {
    ...
    public void paint(Graphics g, Shape a) {
        ...
        paintBorder(g, rect);
        if (clip != null) {
            g.clipRect(rect.x + leftInset, rect.y + topInset, rect.width - leftInset - rightInset, rect.height
        ...
        if (img != null) {
            ...
            if (view != null && ((state & WIDTH_FLAG) == 0 || width > DEFAULT_WIDTH)) {
                ...
                Rectangle altRect = new Rectangle(rect.x + leftInset + DEFAULT_WIDTH, rect.y + topInset,
                        rect.width - leftInset - rightInset - DEFAULT_WIDTH, rect.height - topInset
                                - bottomInset);
    ...
    private void paintHighlights(Graphics g, Shape shape) {
        if (container instanceof JTextComponent) {
            ...
            if (h instanceof LayeredHighlighter) {
                ((LayeredHighlighter) h).paintLayeredHighlights(g, getStartOffset(), getEndOffset(), shape,
                        tc, this);
            }
        }
    }
    ...
    private void paintBorder(Graphics g, Rectangle rect) {
        ...
        if ((borderSize > 0 || image == null) && color != null) {
            ...
            for (int counter = 0; counter < n; counter++) {
                g.drawRect(rect.x + xOffset + counter, rect.y + yOffset + counter, rect.width - counter
                        - counter - xOffset - xOffset - 1, rect.height - counter - counter - yOffset - yOffset
                                - 1);
            }
        }
    }
    ...
    public void setSize(float width, float height) {
        ...
        if (getImage() == null) {
            ...
            if (view != null) {
                view.setSize(Math.max(0f, width - (float) (DEFAULT_WIDTH + leftInset + rightInset)), Math.max(
                        0f, height - (float) (topInset + bottomInset)));
            }
        }
    }
    ...
    private void refreshImage() {
        synchronized (this) {
            ...
            state = (state | LOADING_FLAG | RELOAD_IMAGE_FLAG | WIDTH_FLAG | HEIGHT_FLAG) ^ (WIDTH_FLAG
                    | HEIGHT_FLAG | RELOAD_IMAGE_FLAG);
            image = null;
            width = height = 0;
        }
    ...
    private class ImageHandler implements ImageObserver {
        ...
        public boolean imageUpdate(Image img, int flags, int x, int y, int newWidth, int newHeight) {
            ...
            if (image == img) {
                ...
                short changed = 0;
                if ((flags & ImageObserver.HEIGHT) != 0 && !getElement().getAttributes().isDefined(
                        HTML.Attribute.HEIGHT)) {
                    changed |= 1;
                }
                if ((flags & ImageObserver.WIDTH) != 0 && !getElement().getAttributes().isDefined(
