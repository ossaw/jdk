...
class LineView extends ParagraphView {
	...
	/**
	 * Preformatted lines are not suppressed if they have only whitespace, so
	...
	/**
	 * Determines the minimum span for this view along an axis. The preformatted
	...
	/**
	 * Lays out the children. If the layout span has changed, the rows are
	 * rebuilt. The superclass functionality is called after checking and
	 * possibly rebuilding the rows. If the height has changed, the
	 * <code>preferenceChanged</code> method is called on the parent since the
