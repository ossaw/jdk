 ...
 * Supports the frameborder, scrolling, marginwidth and marginheight attributes.
 *
 * @author Sunita Mani
 */
...
class FrameView extends ComponentView implements HyperlinkListener {
    ...
    /**
     * Sets the parent view for the FrameView. Also determines if the FrameView
     * should be editable or not based on whether the JTextComponent that
    ...
    /**
     * Also determines if the FrameView should be editable or not based on
     * whether the JTextComponent that contains it is editable. And then
    ...
    public void paint(Graphics g, Shape allocation) {
        ...
        if (host != null && htmlPane != null && htmlPane.isEditable() != ((JTextComponent) host)
                .isEditable()) {
    ...
    /**
     * If the marginwidth or marginheight attributes have been specified, then
     * the JEditorPane's margin's are set to the new values.
     */
    ...
    /**
     * If the frameborder attribute has been specified, either in the frame, or
     * by the frames enclosing frameset, the JScrollPane's setBorder() method is
     * invoked to achieve the desired look.
     */
    ...
    /**
     * This method creates the JScrollPane. The scrollbar policy is determined
     * by the scrolling attribute. If not defined, the default is "auto" which
    ...
    /**
     * Finds the outermost FrameSetView. It then returns that FrameSetView's
     * container.
     */
    ...
    /**
     * Notification of a change relative to a hyperlink. This method searches
     * for the outermost JEditorPane, and then fires an HTMLFrameHyperlinkEvent
     * to that frame. In addition, if the target is _parent, and there is not
     * nested framesets then the target is reset to _top. If the target is _top,
     * in addition to firing the event to the outermost JEditorPane, this method
     * also invokes the setPage() method and explicitly replaces the current
     * document with the destination url.
     *
    ...
    public void hyperlinkUpdate(HyperlinkEvent evt) {
        ...
        if (e.getEventType() == HyperlinkEvent.EventType.ACTIVATED) {
            ...
            if (!c.isEditable()) {
                c.fireHyperlinkUpdate(new HTMLFrameHyperlinkEvent(c, e.getEventType(), e.getURL(), e
                        .getDescription(), getElement(), e.getInputEvent(), target));
            }
        }
    }
     ...
     * location that this view is responsible for. Currently this view handles
    ...
    /**
     * Editor pane rendering frame of HTML document It uses the same editor kits
     * classes as outermost JEditorPane
     */
