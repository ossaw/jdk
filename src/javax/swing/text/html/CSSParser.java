 ...
 * <li>Selectors <code>handleSelector</code>. This is invoked for each string.
 * For example if the Reader contained p, bar , a {}, the delegate would be
 * notified 4 times, for 'p,' 'bar' ',' and 'a'.
 * <li>When a rule starts, <code>startRule</code>
 * <li>Properties in the rule via the <code>handleProperty</code>. This is
 * invoked one per property/value key, eg font size: foo;, would cause the
 * delegate to be notified once with a value of 'font size'.
 * <li>Values in the rule via the <code>handleValue</code>, this is notified for
 * the total value.
 * <li>When a rule ends, <code>endRule</code>
 * </ul>
 * This will parse much more than CSS 1, and loosely implements the
 * recommendation for <i>Forward-compatible parsing</i> in section 7.1 of the
 * CSS spec found at: <a
 * href=http://www.w3.org/TR/REC-CSS1>http://www.w3.org/TR/REC-CSS1</a>. If an
 * error results in parsing, a RuntimeException will be thrown.
 * <p>
...
class CSSParser {
     ...
     * than 0, stopping when <code>stopChar</code>, whitespace, or one of {}()[]
     * is hit.
     */
    ...
    /**
     * Reads till a <code>stopChar</code> is encountered, escaping characters as
     * necessary.
     */
    ...
    private int readChar() throws IOException {
         ...
         * (int)Character.toLowerCase((char)retValue); } return retValue;
         */
    }

    /**
     * Supports one character look ahead, this will throw if called twice in a
     * row.
     */
