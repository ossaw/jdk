...
/**
 * Constants used in the <code>HTMLDocument</code>. These are basically tag and
...
public class HTML {
	...
	/**
	 * Typesafe enumeration for an HTML tag. Although the set of HTML tags is a
	 * closed set, we have left the set open so that people can add their own
	 * tag types to their custom parser and still communicate to the reader.
	 */
	public static class Tag {
		 ...
		 * and with <code>causesBreak</code> and <code>isBlock</code> set to
		 * <code>false</code>.
		 *
		 * @param id
		...
		/**
		 * Creates a new <code>Tag</code> with the specified <code>id</code>;
		 * <code>causesBreak</code> and <code>isBlock</code> are defined by the
		 * user.
		 *
		 * @param id
		 *            the id of the new tag
		 * @param causesBreak
		 *            <code>true</code> if this tag causes a break to the flow
		 *            of data
		 * @param isBlock
		 *            <code>true</code> if the tag is used to add structure to a
		 *            document
		 */
		...
		/**
		 * Returns <code>true</code> if this tag is a block tag, which is a tag
		 * used to add structure to a document.
		 *
		 * @return <code>true</code> if this tag is a block tag, otherwise
		...
		/**
		 * Returns <code>true</code> if this tag causes a line break to the flow
		 * of data, otherwise returns <code>false</code>.
		 *
		 * @return <code>true</code> if this tag causes a line break to the flow
		 *         of data, otherwise returns <code>false</code>
		 */
		...
		/**
		 * Returns <code>true</code> if this tag is pre-formatted, which is true
		 * if the tag is either <code>PRE</code> or <code>TEXTAREA</code>.
		 *
		 * @return <code>true</code> if this tag is pre-formatted, otherwise
		 *         returns <code>false</code>
		 */
		...
		/**
		 * All text content must be in a paragraph element. If a paragraph
		 * didn't exist when content was encountered, a paragraph is
		 * manufactured.
		 * <p>
		 * This is a tag synthesized by the HTML reader. Since elements are
		 * identified by their tag type, we create a some fake tag types to mark
		 * the elements that were manufactured.
		 */
		...
		static final Tag allTags[] = { A, ADDRESS, APPLET, AREA, B, BASE, BASEFONT, BIG, BLOCKQUOTE,
				BODY, BR, CAPTION, CENTER, CITE, CODE, DD, DFN, DIR, DIV, DL, DT, EM, FONT, FORM,
				FRAME, FRAMESET, H1, H2, H3, H4, H5, H6, HEAD, HR, HTML, I, IMG, INPUT, ISINDEX,
				KBD, LI, LINK, MAP, MENU, META, NOBR, NOFRAMES, OBJECT, OL, OPTION, P, PARAM, PRE,
				SAMP, SCRIPT, SELECT, SMALL, SPAN, STRIKE, S, STRONG, STYLE, SUB, SUP, TABLE, TD,
	...
	public static class UnknownTag extends Tag implements Serializable {
		 ...
		 * <code>null</code> and is an <code>UnknownTag</code> object with the
		 * same name.
		 *
		 * @param obj
	...
	public static final class Attribute {
		...
		static final Attribute allAttributes[] = { FACE, COMMENT, SIZE, COLOR, CLEAR, BACKGROUND,
				BGCOLOR, TEXT, LINK, VLINK, ALINK, WIDTH, HEIGHT, ALIGN, NAME, HREF, REL, REV,
				TITLE, TARGET, SHAPE, COORDS, ISMAP, NOHREF, ALT, ID, SRC, HSPACE, VSPACE, USEMAP,
				LOWSRC, CODEBASE, CODE, ARCHIVE, VALUE, VALUETYPE, TYPE, CLASS, STYLE, LANG, DIR,
				DECLARE, CLASSID, DATA, CODETYPE, STANDBY, BORDER, SHAPES, NOSHADE, COMPACT, START,
				ACTION, METHOD, ENCTYPE, CHECKED, MAXLENGTH, MULTIPLE, SELECTED, ROWS, COLS, DUMMY,
				CELLSPACING, CELLPADDING, VALIGN, HALIGN, NOWRAP, ROWSPAN, COLSPAN, PROMPT,
				HTTPEQUIV, CONTENT, LANGUAGE, VERSION, N, FRAMEBORDER, MARGINWIDTH, MARGINHEIGHT,
				SCROLLING, NORESIZE, MEDIA, ENDTAG };
	}
	...
	/**
	 * Returns the set of actual HTML tags that are recognized by the default
	 * HTML reader. This set does not include tags that are manufactured by the
	 * reader.
	 */
	...
	/**
	 * Fetches a tag constant for a well-known tag name (i.e. one of the tags in
	 * the set {A, ADDRESS, APPLET, AREA, B, BASE, BASEFONT, BIG, BLOCKQUOTE,
	 * BODY, BR, CAPTION, CENTER, CITE, CODE, DD, DFN, DIR, DIV, DL, DT, EM,
	 * FONT, FORM, FRAME, FRAMESET, H1, H2, H3, H4, H5, H6, HEAD, HR, HTML, I,
	 * IMG, INPUT, ISINDEX, KBD, LI, LINK, MAP, MENU, META, NOBR, NOFRAMES,
	 * OBJECT, OL, OPTION, P, PARAM, PRE, SAMP, SCRIPT, SELECT, SMALL, SPAN,
	 * STRIKE, S, STRONG, STYLE, SUB, SUP, TABLE, TD, TEXTAREA, TH, TITLE, TR,
	 * TT, U, UL, VAR}. If the given name does not represent one of the
	 ...
	 * <code>StyleConstants</code> key <code>sc</code>. If no matching
	...
	/**
	 * Fetches an integer attribute value. Attribute values are stored as a
	...
	/**
	 * Fetches an attribute constant for a well-known attribute name (i.e. one
	 * of the attributes in the set {FACE, COMMENT, SIZE, COLOR, CLEAR,
	 * BACKGROUND, BGCOLOR, TEXT, LINK, VLINK, ALINK, WIDTH, HEIGHT, ALIGN,
	 * NAME, HREF, REL, REV, TITLE, TARGET, SHAPE, COORDS, ISMAP, NOHREF, ALT,
	 * ID, SRC, HSPACE, VSPACE, USEMAP, LOWSRC, CODEBASE, CODE, ARCHIVE, VALUE,
	 * VALUETYPE, TYPE, CLASS, STYLE, LANG, DIR, DECLARE, CLASSID, DATA,
	 * CODETYPE, STANDBY, BORDER, SHAPES, NOSHADE, COMPACT, START, ACTION,
	 * METHOD, ENCTYPE, CHECKED, MAXLENGTH, MULTIPLE, SELECTED, ROWS, COLS,
	 * DUMMY, CELLSPACING, CELLPADDING, VALIGN, HALIGN, NOWRAP, ROWSPAN,
	 * COLSPAN, PROMPT, HTTPEQUIV, CONTENT, LANGUAGE, VERSION, N, FRAMEBORDER,
	 * MARGINWIDTH, MARGINHEIGHT, SCROLLING, NORESIZE, MEDIA, ENDTAG}). If the
	 * given name does not represent one of the well-known attributes, then
