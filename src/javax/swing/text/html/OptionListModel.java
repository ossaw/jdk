...
class OptionListModel<E> extends DefaultListModel<E> implements ListSelectionModel, Serializable {
    ...
    /**
     * Returns an array of all the <code>ListSelectionListener</code>s added to
     * this OptionListModel with addListSelectionListener().
     *
    ...
    /**
     * Notify listeners that we are beginning or ending a series of value
     * changes
     */
    ...
    /**
     * Notify ListSelectionListeners that the value of the selection, in the
     * closed interval firstIndex,lastIndex, has changed.
     */
     ...
     * leadAnchorNotificationEnabled is true the model generates notification
     * events with bounds that cover all the changes to the selection plus the
     * changes to the lead and anchor indices. Setting the flag to false causes
     * a norrowing of the event's bounds to include only the elements that have
     * been selected or deselected since the last change. Either way, the model
     * continues to maintain the lead and anchor variables internally. The
     * default is true.
     * 
     ...
     * is itself selected, set all of the newly inserted items, otherwise leave
     * them unselected. This method is typically called to sync the selection
     * model with a corresponding change in the data model.
     */
    ...
    /**
     * Set the lead selection index, ensuring that values between the anchor and
     * the new lead are either all selected or all deselected. If the value at
     * the anchor index is selected, first clear all the values in the range
     ...
     * would be wider, including cells that had been first cleared and only to
     * later be set.
     * <p>
    ...
    /**
     * This method is responsible for storing the state of the initial
     * selection. If the selectionMode is the default, i.e allowing only for
     * SINGLE_SELECTION, then the very last OPTION that has the selected
     * attribute set wins.
     */
    ...
    /**
     * Fetches the BitSet that represents the initial set of selected items in
     * the list.
     */
