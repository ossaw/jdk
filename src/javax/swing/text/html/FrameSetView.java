...
class FrameSetView extends javax.swing.text.BoxView {
	...
	/**
	 * Initializes a number of internal state variables that store information
	 * about space allocation for the frames contained within the frameset.
	 */
	private void init() {
		...
		for (int i = 0; i < children.length; i++) {
			...
			if (children[i].endsWith("*")) {
				if (children[i].length() > 1) {
					relativeChildren[i] = Integer
							.parseInt(children[i].substring(0, children[i].length() - 1));
	...
	protected SizeRequirements[] getChildRequests(int targetSpan, int axis) {
		...
		for (int i = 0, sIndex = 0; i < n; i++) {
			...
			if ((v instanceof FrameView) || (v instanceof FrameSetView)) {
				reqs[i] = new SizeRequirements((int) v.getMinimumSpan(axis), span[sIndex],
						(int) v.getMaximumSpan(axis), 0.5f);
				sIndex++;
			} else {
	...
	/**
	 * This method is responsible for returning in span[] the span for each
	 * child view along the major axis. it computes this based on the
	 * information that extracted from the value of the ROW/COL attribute.
	 */
	private void spread(int targetSpan, int span[]) {
		...
		if (remainingSpace > 0 && relativeTotals > 0) {
			...
			for (int i = 0; i < span.length; i++) {
				// ok we know what our total space is, and we know how large
				// each
				// column should be relative to each other... therefore we can
				// use
				// that relative information to deduce their percentages of a
				// whole
			...
			// this is for just in case there is something left over.. if there
			// is we just
			// add it one pixel at a time to the frames in order.. We shouldn't
			// really ever get
			// here and if we do it shouldn't be with more than 1 pixel, maybe
			// two.
			int i = 0;
			while (remainingSpace != 0) {
				...
				// just in case there are more pixels than frames...should never
				// happen..
				if (i == span.length)
					i = 0;
			}
	...
	/*
	 * Users have been known to type things like "%25" and "25 %". Deal with it.
	 */
