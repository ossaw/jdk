...
/**
 * The Swing JEditorPane text component supports different kinds of content via
 * a plug-in mechanism called an EditorKit. Because HTML is a very popular
 * format of content, some support is provided by default. The default support
 * is provided by this class, which supports HTML version 3.2 (with some
 * extensions), and is migrating toward version 4.0. The &lt;applet&gt; tag is
 * not supported, but some support is provided for the &lt;object&gt; tag.
 * <p>
 * There are several goals of the HTML EditorKit provided, that have an effect
 * upon the way that HTML is modeled. These have influenced its design in a
 * substantial way.
 * <dl>
 * <dt>Support editing
 * <dd>It might seem fairly obvious that a plug-in for JEditorPane should
 * provide editing support, but that fact has several design considerations.
 * There are a substantial number of HTML documents that don't properly conform
 * to an HTML specification. These must be normalized somewhat into a correct
 * form if one is to edit them. Additionally, users don't like to be presented
 * with an excessive amount of structure editing, so using traditional text
 * editing gestures is preferred over using the HTML structure exactly as
 * defined in the HTML document.
 * <p>
 ...
 * <ol>
 * <li>The parser is replaceable. The default parser is the Hot Java parser
 * which is DTD based. A different DTD can be used, or an entirely different
 * parser can be used. To change the parser, reimplement the getParser method.
 * The default parser is dynamically loaded when first asked for, so the class
 * files will never be loaded if an alternative parser is used. The default
 * parser is in a separate package called parser below this package.
 * <li>The parser drives the ParserCallback, which is provided by HTMLDocument.
 * To change the callback, subclass HTMLDocument and reimplement the
 * createDefaultDocument method to return document that produces a different
 * reader. The reader controls how the document is structured. Although the
 * Document provides HTML support by default, there is nothing preventing
 * support of non-HTML tags that result in alternative element structures.
 * <li>The default view of the models are provided as a hierarchy of View
 * implementations, so one can easily customize how a particular element is
 * displayed or add capabilities for new kinds of elements by providing new View
 * implementations. The default set of views are provided by the
 * <code>HTMLFactory</code> class. This can be easily changed by subclassing or
 * replacing the HTMLFactory and reimplementing the getViewFactory method to
 * return the alternative factory.
 * <li>The View implementations work primarily off of CSS attributes, which are
 * kept in the views. This makes it possible to have multiple views mapped over
 * the same model that appear substantially different. This can be especially
 * useful for printing. For most HTML attributes, the HTML attributes are
 ...
 * loaded using <code>JEditorPane.setPage</code>. This is controlled by a
 * property on the document. The method {@link #createDefaultDocument
 * createDefaultDocument} can be overriden to change this. The batching of work
 ...
 * implementations. It is desirable for the look-and-feel to feed display
 * characteristics into the HTML views. An user with poor vision for example
 * would want high contrast and larger than typical fonts.
 * <p>
 ...
 * two operation. The policy of the HTMLEditorKit will be to store things not
 * recognized or not necessarily visible so they can be subsequently written
 * out. The model of the HTML document should therefore contain all information
 * discovered while reading the document. This is constrained in some ways by
 * the need to support editing (i.e. incorrect documents sometimes must be
 * normalized). The guiding principle is that information shouldn't be lost, but
 * some might be synthesized to produce a more correct model or it might be
 * rearranged.
 * </dl>
 *
...
public class HTMLEditorKit extends StyledEditorKit implements Accessible {
	...
	/**
	 * Try to get an HTML parser from the document. If no parser is set for the
	 * document, return the editor kit's default parser. It is an error if no
	 * parser could be obtained from the editor kit.
	 */
	...
	public static class LinkController extends MouseAdapter
					...
					if (curElem != elem || curElemImage) {
						...
						if (elem != null) {
							...
							if (anchor == null) {
								...
								if (curElemImage) {
									href = getMapHREF(editor, hdoc, elem, a, pos, e.getX(),
											e.getY());
								}
							} else {
		 ...
		 */
		private String getMapHREF(JEditorPane html, HTMLDocument hdoc, Element elem,
				AttributeSet attr, int offset, int x, int y) {
		 ...
		 */
		private boolean doesElementContainLocation(JEditorPane editor, Element e, int offset, int x,
				int y) {
		...
		HyperlinkEvent createHyperlinkEvent(JEditorPane html, HTMLDocument hdoc, String href,
			...
			if (!hdoc.isFrameDocument()) {
				...
				linkEvent = new HTMLFrameHyperlinkEvent(html, HyperlinkEvent.EventType.ACTIVATED, u,
						href, element, mouseEvent, target);
			}
			return linkEvent;
		}
		...
		void fireEvents(JEditorPane editor, HTMLDocument doc, String href, Element lastElem,
			...
			if (href != null) {
				...
				HyperlinkEvent entered = new HyperlinkEvent(editor,
						HyperlinkEvent.EventType.ENTERED, u, href, curElem, mouseEvent);
	...
	private static final Action[] defaultActions = {
			new InsertHTMLTextAction("InsertTable", INSERT_TABLE_HTML, HTML.Tag.BODY,
					HTML.Tag.TABLE),
			new InsertHTMLTextAction("InsertTableRow", INSERT_TABLE_HTML, HTML.Tag.TABLE,
					HTML.Tag.TR, HTML.Tag.BODY, HTML.Tag.TABLE),
			new InsertHTMLTextAction("InsertTableDataCell", INSERT_TABLE_HTML, HTML.Tag.TR,
					HTML.Tag.TD, HTML.Tag.BODY, HTML.Tag.TABLE),
			new InsertHTMLTextAction("InsertUnorderedList", INSERT_UL_HTML, HTML.Tag.BODY,
					HTML.Tag.UL),
			new InsertHTMLTextAction("InsertUnorderedListItem", INSERT_UL_HTML, HTML.Tag.UL,
					HTML.Tag.LI, HTML.Tag.BODY, HTML.Tag.UL),
			new InsertHTMLTextAction("InsertOrderedList", INSERT_OL_HTML, HTML.Tag.BODY,
					HTML.Tag.OL),
			new InsertHTMLTextAction("InsertOrderedListItem", INSERT_OL_HTML, HTML.Tag.OL,
					HTML.Tag.LI, HTML.Tag.BODY, HTML.Tag.OL),
			new InsertHRAction(),
	...
	public static abstract class HTMLTextAction extends StyledTextAction {
		...
		private Element[] getElementsAt(Element parent, int offset, int depth) {
			...
			Element[] retValue = getElementsAt(parent.getElement(parent.getElementIndex(offset)),
					offset, depth + 1);
			retValue[depth] = parent;
			return retValue;
		}
		...
		protected int elementCountToTag(HTMLDocument doc, int offset, HTML.Tag tag) {
			...
			while (e != null
					&& e.getAttributes().getAttribute(StyleConstants.NameAttribute) != tag) {
		...
		public void actionPerformed(ActionEvent ae) {
			...
			if (editor != null) {
				...
				if (!insertIntoTag(editor, doc, offset, parentTag, addTag)
					...
					// Then alternate.
					inserted = insertIntoTag(editor, doc, offset, alternateParentTag,
							alternateAddTag);
				} else {
					inserted = true;
				}
		...
		public void actionPerformed(ActionEvent ae) {
			...
			if (editor != null) {
				...
				if (paragraph.getParentElement() != null) {
					parentTag = (HTML.Tag) paragraph.getParentElement().getAttributes()
		...
		public void caretUpdate(CaretEvent e) {
			...
			if (src instanceof JTextComponent) {
				...
				if (kit != null && kit.foundLink) {
					kit.foundLink = false;
					// TODO: The AccessibleContext for the editor should
					// register
					...
					comp.getAccessibleContext().firePropertyChange(
							...
							Integer.valueOf(kit.prevHypertextOffset), Integer.valueOf(e.getDot()));
				}
			}
		}
		...
		private void activateLink(String href, HTMLDocument doc, JEditorPane editor, int offset) {
			try {
				...
				HyperlinkEvent linkEvent = new HyperlinkEvent(editor,
						HyperlinkEvent.EventType.ACTIVATED, url, url.toExternalForm(),
