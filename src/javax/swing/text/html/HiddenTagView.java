...
/**
 * HiddenTagView subclasses EditableView to contain a JTextField showing the
 * element name. When the textfield is edited the element name is reset. As this
 * inherits from EditableView if the JTextComponent is not editable, the
 * textfield will not be visible.
 *
 * @author Scott Violet
 */
class HiddenTagView extends EditableView implements DocumentListener {
	...
	/**
	 * This resets the text on the text component we created to match that of
	 * the AttributeSet for the Element we represent.
	 * <p>
	 * If this is invoked on the event dispatching thread, this directly invokes
	 * <code>_setTextFromModel</code>, otherwise
	 * <code>SwingUtilities.invokeLater</code> is used to schedule execution of
	 * <code>_setTextFromModel</code>.
	 */
	...
	/**
	 * This copies the text from the text component we've created to the
	 * Element's AttributeSet we represent.
	 * <p>
	 * If this is invoked on the event dispatching thread, this directly invokes
	 * <code>_updateModelFromText</code>, otherwise
	 * <code>SwingUtilities.invokeLater</code> is used to schedule execution of
	 * <code>_updateModelFromText</code>.
	 */
