...
/**
 * A stack of tags. Used while parsing an HTML document. It, together with the
 * ContentModelStates, defines the complete state of the parser while reading a
 * document. When a start tag is encountered an element is pushed onto the
 * stack, when an end tag is enountered an element is popped of the stack.
 *
 * @see Parser
 * @see DTD
...
final class TagStack implements DTDConstants {
	...
	/**
	 * Return the ContentModel that must be satisfied by what comes next in the
	 * input stream.
	 */
	...
	/**
	 * Return true if the element that is contained at the index specified by
	 * the parameter is part of the exclusions specified in the DTD for the
	 * element currently on the TagStack.
	 */
	...
	/**
	 * Advance the state by reducing the given element. Returns false if the
	 * element is not legal and the state is not advanced.
	 */
	...
	public String toString() {
		return (next == null) ? "<" + tag.getElement().getName() + ">"
