...
/**
 * A simple DTD-driven HTML parser. The parser reads an HTML file from an
 * InputStream and calls various methods (which should be overridden in a
 * subclass) when tags and data are encountered.
 * <p>
 * Unfortunately there are many badly implemented HTML parsers out there, and as
 * a result there are many badly formatted HTML files. This parser attempts to
 * parse most HTML files. This means that the implementation sometimes deviates
 * from the SGML specification in favor of HTML.
 * <p>
 * The parser treats \r and \r\n as \n. Newlines after starttags and before end
 * tags are ignored just as specified in the SGML/HTML specification.
 * <p>
 * The html spec does not specify how spaces are to be coalesced very well.
 * Specifically, the following scenarios are not discussed (note that a space
 * should be used here, but I am using &amp;nbsp to force the space to be
 * displayed):
 * <p>
 * '&lt;b&gt;blah&nbsp;&lt;i&gt;&nbsp;&lt;strike&gt;&nbsp;foo' which can be
 * treated as: '&lt;b&gt;blah&nbsp;&lt;i&gt;&lt;strike&gt;foo'
 * <p>
 * as well as: '&lt;p&gt;&lt;a
 * href="xx"&gt;&nbsp;&lt;em&gt;Using&lt;/em&gt;&lt;/a&gt;&lt;/p&gt;' which
 * appears to be treated as: '&lt;p&gt;&lt;a
 * href="xx"&gt;&lt;em&gt;Using&lt;/em&gt;&lt;/a&gt;&lt;/p&gt;'
 * <p>
 * If <code>strict</code> is false, when a tag that breaks flow, (
 * <code>TagElement.breaksFlows</code>) or trailing whitespace is encountered,
 * all whitespace will be ignored until a non whitespace character is
...
public class Parser implements DTDConstants {
    ...
    /**
     * The html spec does not specify how spaces are coalesced very well. If
     * strict == false, ignoreSpace is used to try and mimic the behavior of the
     * popular browsers.
     * <p>
     * The problematic scenarios are: '&lt;b>blah &lt;i> &lt;strike> foo' which
     * can be treated as: '&lt;b>blah &lt;i>&lt;strike>foo' as well as:
     * '&lt;p>&lt;a href="xx"> &lt;em>Using&lt;/em>&lt;/a>&lt;/p>' which appears
     * to be treated as: '&lt;p>&lt;a
     * href="xx">&lt;em>Using&lt;/em>&lt;/a>&lt;/p>'
     * <p>
     * When a tag that breaks flow, or trailing whitespace is encountered
     * ignoreSpace is set to true. From then on, all whitespace will be ignored.
     * ignoreSpace will be set back to false the first time a non whitespace
     * character is encountered. This appears to give behavior closer to the
     * popular browsers.
     */
    private boolean ignoreSpace;

    /**
     * This flag determines whether or not the Parser will be strict in
    ...
    //
    /**
     * The start position of the current block. Block is overloaded here, it
     * really means the current start position for the current comment, tag,
     * text. Use getBlockStartPosition to access this.
     */
    ...
    /**
     * array for mapping numeric references in range 130-159 to displayable
     * Unicode characters.
     */
    private static final char[] cp1252Map = { 8218, // &#130;
            402, // &#131;
            8222, // &#132;
            8230, // &#133;
            8224, // &#134;
    ...
    /**
     * Returns the start position of the current block. Block is overloaded
     * here, it really means the current start position for the current comment
     * tag, text, block.... This is provided for subclassers that wish to know
     * the start of the current block when called with one of the handleXXX
     * methods.
     */
    ...
    protected void handleError(int ln, String msg) {
        /*
         * Thread.dumpStack(); System.out.println("**** " + stack);
    ...
    /**
     * Handle a start tag. The new tag is pushed onto the tag stack. The
     * attribute list is checked for required attributes.
     */
    protected void startTag(TagElement tag) throws ChangedCharSetException {
        ...
        if (elem.isEmpty()) {
            handleEmptyTag(tag);
            /*
             * } else if (elem.getName().equals("form")) { handleStartTag(tag);
             */
        } else {
            recent = elem;
    ...
    boolean ignoreElement(Element elem) {
        ...
        /*
         * We ignore all elements that are not valid in the context of a table
         * except <td>, <th> (these we handle in legalElementContext()) and
         * #pcdata. We also ignore the <font> tag in the context of <ul> and
    ...
    /**
     * Error context. Something went wrong, make sure we are in the document's
     * body context
     */
    ...
    /**
     * Parse identifier. Uppercase characters are folded to lowercase when lower
     * is true. Returns falsed if no identifier is found. [55] 346:17
     */
     ...
     * are control chars in Unicode set) to displayable characters with other
     * codes.
     *
     * @param c
    ...
    void parseAttributeSpecificationList(Element elem) throws IOException {
        ...
        while (true) {
            ...
            } else if (!strict && ch == ',') { // allows for comma separated
    ...
    /**
     * Parses th Document Declaration Type markup declaration. Currently ignores
     * it.
     */
    ...
    /**
     * Parse markup declarations. Currently only handles the Document Type
    ...
    void parseTag() throws IOException {
        ...
        /**
         * In dealing with forms, we have decided to treat them as legal in any
         * context. Also, even though they do have a start and an end tag, we
         * will not put this tag on the stack. This is to deal several pages in
         * the web oasis that choose to start and end forms in any possible
         * location.
         **/

        /*
         * if (!strict && elem.getName().equals("form")) { if (lastFormSent ==
         * null) { lastFormSent = tag; } else { handleEndTag(lastFormSent);
         * lastFormSent = tag; } } else {
         */
    ...
    /**
     * Returns the end of line string. This will return the end of line string
     ...
     * method of BufferedReader for each byte. Measurements done 5/30/97 show
     * that there's no point in having a bigger buffer: Increasing the buffer to
     * 8192 had no measurable impact for a program discarding one character at a
     * time (reading from an http URL to a local machine). NOTE: If the current
     * encoding is bogus, and we read too much (past the content-type) we may
     * suffer a MalformedInputException. For this reason the initial size is 1
     * and when the body is encountered the size is adjusted to 256.
     */
    private char buf[] = new char[1];
    private int pos;
    private int len;
    /*
     * tracks position relative to the beginning of the document.
     */
