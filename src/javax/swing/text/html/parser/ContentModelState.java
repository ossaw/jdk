...
/**
 * A content model state. This is basically a list of pointers to the BNF
 * expression representing the model (the ContentModel). Each element in a DTD
 * has a content model which describes the elements that may occur inside, and
 * the order in which they can occur.
 * <p>
...
class ContentModelState {
	...
	/**
	 * Create a content model state for a content model given the remaining
	 * state that needs to be reduce.
	 */
	...
	/**
	 * Check if the state can be terminated. That is there are no more tokens
	 * required in the input stream.
	 * 
	...
	/**
	 * Advance this state to a new state. An exception is thrown if the token is
	 * illegal at this point in the content model.
	 * 
