 ...
 * really only use this class with the html dtd in swing). Reads an InputStream
 * of HTML and invokes the appropriate methods in the ParserCallback class. This
 * is the default parser used by HTMLEditorKit to parse HTML url's.
 * <p>
 * This will message the callback for all valid tags, as well as tags that are
 * implied but not explicitly specified. For example, the html string
 ...
 * explicitly specified, to be correct html they should have been present (head
 * isn't necessary, but it is still generated). For tags that are implied, the
 * AttributeSet argument will have a value of <code>Boolean.TRUE</code> for the
 * key <code>HTMLEditorKit.ParserCallback.IMPLIED</code>.
 * <p>
 * HTML.Attributes defines a type safe enumeration of html attributes. If an
 * attribute key of a tag is defined in HTML.Attribute, the HTML.Attribute will
 * be used as the key, otherwise a String will be used. For example &lt;p
 * foo=bar class=neat&gt; has two attributes. foo is not defined in
 * HTML.Attribute, where as class is, therefore the AttributeSet will have two
 * values in it, HTML.Attribute.CLASS with a String value of 'neat' and the
 * String key 'foo' with a String value of 'bar'.
 * <p>
 * The position argument will indicate the start of the tag, comment or text.
 * Similar to arrays, the first character in the stream has a position of 0. For
 * tags that are implied the position will indicate the location of the next
 * encountered tag. In the first example, the implied start body and html tags
 * will have the same position as the p tag, and the implied end p, html and
 * body tags will all have the same position.
 * <p>
 * As html skips whitespace the position for text will be the position of the
 * first valid character, eg in the string '\n\n\nblah' the text 'blah' will
 * have a position of 3, the newlines are skipped.
 * <p>
 * For attributes that do not have a value, eg in the html string
 * <code>&lt;foo blah&gt;</code> the attribute <code>blah</code> does not have a
 * value, there are two possible values that will be placed in the
 * AttributeSet's value:
 * <ul>
 * <li>If the DTD does not contain an definition for the element, or the
 * definition does not have an explicit value then the value in the AttributeSet
 * will be <code>HTML.NULL_ATTRIBUTE_VALUE</code>.
 * <li>If the DTD contains an explicit value, as in:
 * <code>&lt;!ATTLIST OPTION selected (selected) #IMPLIED&gt;</code> this value
 * from the dtd (in this case selected) will be used.
 * </ul>
 * <p>
...
public class DocumentParser extends javax.swing.text.html.parser.Parser {
	...
	protected void handleStartTag(TagElement tag) {
		...
		if (debugFlag) {
			if (tag.fictional()) {
				...
				debug("Start Tag: " + tag.getHTMLTag() + " attributes: " + getAttributes()
						+ " pos: " + getCurrentPos());
			}
		}
		if (tag.fictional()) {
	...
	protected void handleEmptyTag(TagElement tag) throws ChangedCharSetException {
		...
		if (inbody != 0 || elem == dtd.meta || elem == dtd.base || elem == dtd.isindex
				|| elem == dtd.style || elem == dtd.link) {
			if (debugFlag) {
				if (tag.fictional()) {
					debug("Empty Tag: " + tag.getHTMLTag() + " pos: " + getCurrentPos());
				} else {
					debug("Empty Tag: " + tag.getHTMLTag() + " attributes: " + getAttributes()
							+ " pos: " + getCurrentPos());
				}
			}
			if (tag.fictional()) {
