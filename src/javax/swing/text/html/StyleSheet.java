 ...
 * The StyleSheet is used to translate the HTML model into visual
 * characteristics. This enables views to be customized by a look-and-feel,
 * multiple views over the same model can be rendered differently, etc. This can
 * be thought of as a CSS rule repository. The key for CSS attributes is an
 * object of type CSS.Attribute. The type of the value is up to the StyleSheet
 * implementation, but the <code>toString</code> method is required to return a
 * string representation of CSS value.
 * <p>
 * The primary entry point for HTML View implementations to get their attributes
 * is the {@link #getViewAttributes getViewAttributes} method. This should be
 * implemented to establish the desired policy used to associate attributes with
 * the view. Each HTMLEditorKit (i.e. and therefore each associated JEditorPane)
 * can have its own StyleSheet, but by default one sheet will be shared by all
 * of the HTMLEditorKit instances. HTMLDocument instance can also have a
 * StyleSheet, which holds the document-specific CSS specifications.
 * <p>
 * In order for Views to store less state and therefore be more lightweight, the
 * StyleSheet can act as a factory for painters that handle some of the
 * rendering tasks. This allows implementations to determine what they want to
 * cache and have the sharing potentially at the level that a selector is common
 * to multiple views. Since the StyleSheet may be used by views over multiple
 * documents and typically the HTML attributes don't effect the selector being
 * used, the potential for sharing is significant.
 * <p>
 * The rules are stored as named styles, and other information is stored to
 * translate the context of an element to a rule quickly. The following code
 * fragment will display the named styles, and therefore the CSS rules
 * contained.
 * 
 * <pre>
 * <code>
 ...
 * </pre>
 * <p>
 * The semantics for when a CSS style should overide visual attributes defined
 * by an element are not well defined. For example, the html
 * <code>&lt;body bgcolor=red&gt;</code> makes the body have a red background.
 * But if the html file also contains the CSS rule
 * <code>body { background: blue }</code> it becomes less clear as to what color
 * the background of the body should be. The current implementation gives visual
 * attributes defined in the element the highest precedence, that is they are
 * always checked before any styles. Therefore, in the previous example the
 * background would have a red color as the body element defines the background
 * color to be red.
 * <p>
 * As already mentioned this supports CSS. We don't support the full CSS spec.
 * Refer to the javadoc of the CSS class to see what properties we support. The
 * two major CSS parsing related concepts we do not currently support are pseudo
 * selectors, such as <code>A:link { color: red }</code>, and the
 * <code>important</code> modifier.
 * <p>
 * <font color="red">Note: This implementation is currently incomplete. It can
 * be replaced with alternative implementations that are complete. Future
 * versions of this class will provide better CSS support.</font>
 *
...
public class StyleSheet extends StyleContext {
    ...
    /**
     * Fetches the style to use to render the given type of HTML tag. The
     * element given is representing the tag and can be used to determine the
     * nesting for situations where the attributes will differ if nesting inside
     * of elements.
     *
     * @param t
     *          the type to translate to visual attributes
     * @param e
     *          the element representing the tag; the element can be used to
     *          determine the nesting for situations where the attributes will
     ...
     * Where <code>selector</code> is a space separated String of the element
     * names. For example, <code>selector</code> might be 'html body tr td''
     * <p>
     * The attributes of the returned Style will change as rules are added and
     * removed. That is if you to ask for a rule with a selector "table p" and a
     * new rule was added with a selector of "p" the returned Style would
     * include the new attributes from the rule "p".
     */
    ...
    public void addRule(String rule) {
        if (rule != null) {
            ...
            if (rule == baseUnitsDisable) {
                ...
                rebaseSizeMap(Integer.parseInt(rule.substring(baseUnits
                        .length())));
                ...
                try {
                    parser.parse(getBase(), new StringReader(rule), false,
                            false);
    ...
    /**
     * Translates a CSS declaration to an AttributeSet that represents the CSS
     * declaration. Typically this would be called as a result of encountering
     ...
     * If there are collisions with existing rules, the newly specified rule
     * will win.
     *
     * @param in
     *            the stream to read the CSS grammar from
     * @param ref
     *            the reference URL. This value represents the location of the
     *            stream and may be null. All relative URLs specified in the
     *            stream will be based upon this parameter.
     */
    ...
    /**
     * Fetches a set of attributes to use in the view for displaying. This is
     * basically a set of attributes that can be used for View.getAttributes.
     */
            ...
            synchronized (this) {
                ...
                if (rule != null) {
                    ...
                    if (resolvedStyles.size() > 0) {
                        Enumeration<ResolvedStyle> values = resolvedStyles
                                .elements();
    ...
    /**
     * Adds the rules from the StyleSheet <code>ss</code> to those of the
     * receiver. <code>ss's</code> rules will override the rules of any
     * previously added style sheets. An added StyleSheet will never override
    ...
    /**
     * Returns an array of the linked StyleSheets. Will return null if there are
    ...
    /**
     * Imports a style sheet from <code>url</code>. The resulting rules are
     * directly added to the receiver. If you do not want the rules to become
     * part of the receiver, create a new StyleSheet and use addStyleSheet to
     * link it in.
     *
     * @since 1.3
     */
    ...
    /**
     * Sets the base. All import statements that are relative, will be relative
     * to <code>base</code>.
     *
     * @since 1.3
     */
    ...
    /**
     * Adds an attribute to the given set, and returns the new representative
     * set. This is reimplemented to convert StyleConstant attributes to CSS
     * prior to forwarding to the superclass behavior. The StyleConstants
     * attribute has no corresponding CSS entry, the StyleConstants attribute is
     * stored (but will likely be unused).
     *
     * @param old
     ...
     * StyleConstants attributes, they will be converted to CSS prior to
    ...
    /**
     * Removes a set of attributes for the element. If any of the attributes is
     * a StyleConstants attribute, the request will be converted to a CSS
    ...
    /**
     * Removes a set of attributes. If any of the attributes is a StyleConstants
     * attribute, the request will be converted to a CSS attribute prior to
     * forwarding to the superclass behavior.
     *
     * @param old
     ...
     * for subclasses that want to alter the behavior of SmallAttributeSet. This
     * can be reimplemented to return an AttributeSet that provides some sort of
     * attribute conversion.
     *
     * @param a
     *          The set of attributes to be represented in the the compact
     *          form.
     */
     ...
     * This set will not be shared. This is a hook for subclasses that want to
     * alter the behavior of the larger attribute storage format (which is
     * SimpleAttributeSet by default). This can be reimplemented to return a
     * MutableAttributeSet that provides some sort of attribute conversion.
     *
     * @param a
     *          The set of attributes to be represented in the the larger
     *          form.
     */
    ...
    /**
     * For any StyleConstants key in attr that has an associated HTML.Tag, it is
     * removed from old. The resulting AttributeSet is then returned.
     */
    ...
    /**
     * Converts a set of attributes (if necessary) so that any attributes that
     * were specified as StyleConstants attributes and have a CSS mapping, will
     * be converted to CSS attributes.
     */
    ...
    /**
     * Large set of attributes that does conversion of requests for attributes
     * of type StyleConstants.
     */
    ...
    /**
     * Small set of attributes that does conversion of requests for attributes
     * of type StyleConstants.
     */
    ...
    /**
     * Sets the base font size from the passed in String. The string can either
     * identify a specific font size, with legal values between 1 and 7, or
     * identify a relative font size such as +1 or -2.
     */
    ...
    /**
     * Converts a color string such as "RED" or "#NNNNNN" to a Color. Note: This
     * will only convert the HTML3.2 color strings or a string of length 7;
    ...
    /**
     * Returns the ImageIcon to draw in the background for <code>attr</code>.
     */
     ...
     *
     * @param selector
     *                    the selector to use for the rule. This will be a set
     *                    of simple
     *                    selectors, and must be a length of 1 or greater.
     * @param declaration
     *                    the set of CSS attributes that make up the rule.
     */
    void addRule(String[] selector, AttributeSet declaration,
        ...
        if (rule == null) {
            ...
            synchronized (this) {
                SelectorMapping mapping = getRootSelectorMapping();
                for (int i = n - 1; i >= 0; i--) {
                    mapping = mapping.getChildSelectorMapping(selector[i],
                            true);
                }
                ...
                if (rule == null) {
                    ...
                    refreshResolvedRules(selectorName, selector, rule, mapping
                            .getSpecificity());
                }
            }
        }
    ...
    /**
     * Updates the attributes of the rules to reference any related rules in
     * <code>ss</code>.
     */
    ...
    /**
     * Removes references to the rules in <code>ss</code>. <code>index</code>
     * gives the index the StyleSheet was at, that is how many StyleSheets had
     * been added before it.
     */
    ...
    /**
     * Returns a string that only has one space between simple
     * selectors, which may be the passed in String.
     */
    /* protected */ String cleanSelectorString(String selector) {
        boolean lastWasSpace = true;
        for (int counter = 0, maxCounter = selector
    ...
    /**
     * Returns a new String that contains only one space between non white space
     * characters.
     */
                ...
                switch (chars[counter]) {
                    case ' ':
                        if (!lastWasSpace) {
                            ...
                            if (lastIndex < counter) {
                                buff.append(chars, lastIndex, 1 + counter
                                        - lastIndex);
                            }
                        }
                        lastIndex = counter + 1;
                    ...
                    case '\t':
                        if (!lastWasSpace) {
                            lastWasSpace = true;
                            if (lastIndex < counter) {
                                buff.append(chars, lastIndex, counter
                                        - lastIndex);
                                buff.append(' ');
                            }
                        }
     ...
     * This is an inverted graph of the selectors.
     */
    ...
    /**
     * Returns the specificity of the passed in String. It assumes the passed in
     * string doesn't contain junk, that is each selector is separated by a
     * space and each selector at most contains one . or one #. A simple
     * selector has a weight of 1, an id selector has a weight of 100, and a
     * class selector has a weight of 10000.
     */
    ...
    /**
     * Returns the style that linked attributes should be added to. This will
    ...
    /**
     * Returns the resolved style for <code>selector</code>. This will create
     * the resolved style, if necessary.
     */
    private synchronized Style getResolvedStyle(String selector,
            Vector elements, HTML.Tag t) {
    ...
    /**
     * Adds <code>mapping</code> to <code>elements</code>. It is added such that
     * <code>elements</code> will remain ordered by specificity.
     */
    private void addSortedStyle(SelectorMapping mapping,
            Vector<SelectorMapping> elements) {
        ...
        if (size > 0) {
            ...
            for (int counter = 0; counter < size; counter++) {
                if (specificity >= elements.elementAt(counter)
                        .getSpecificity()) {
    ...
    /**
     * Adds <code>parentMapping</code> to <code>styles</code>, and recursively
     * calls this method if <code>parentMapping</code> has any child mappings
     * for any of the Elements in <code>elements</code>.
     */
    private synchronized void getStyles(SelectorMapping parentMapping,
            Vector<SelectorMapping> styles, String[] tags, String[] ids,
            String[] classes, int index, int numElements,
        ...
        for (int counter = index; counter < numElements; counter++) {
            ...
            if (tagString != null) {
                ...
                if (childMapping != null) {
                    getStyles(childMapping, styles, tags, ids, classes, counter
    ...
    private synchronized Style createResolvedStyle(String selector,
        ...
        Hashtable<SelectorMapping, SelectorMapping> tempHashtable = sb
                .getHashtable();
        ...
        try {
            ...
            if (classes[0] != null) {
                ...
                childMapping = mapping.getChildSelectorMapping(tagString + "."
                        + className, false);
                if (childMapping != null) {
                    getStyles(childMapping, tempVector, tags, ids, classes, 1,
                            numElements, tempHashtable);
                }
                childMapping = mapping.getChildSelectorMapping("." + className,
                        false);
                if (childMapping != null) {
                    getStyles(childMapping, tempVector, tags, ids, classes, 1,
                            numElements, tempHashtable);
            ...
            if (ids[0] != null) {
                ...
                childMapping = mapping.getChildSelectorMapping("#" + idName,
                        false);
                if (childMapping != null) {
                    getStyles(childMapping, tempVector, tags, ids, classes, 1,
                            numElements, tempHashtable);
            ...
            // Styles.
            int numLinkedSS = (linkedStyleSheets != null) ? linkedStyleSheets
                    .size() : 0;
            ...
            for (int counter = 0; counter < numLinkedSS; counter++) {
                AttributeSet attr = linkedStyleSheets.elementAt(counter)
                        .getRule(selector);
                if (attr == null) {
    ...
    /**
     * Creates and returns a Style containing all the rules that matches
     * <code>selector</code>.
     *
     * @param elements
     *                 a Vector of all the Elements the style is being asked
     *                 for. The
     *                 first Element is the deepest Element, with the last
     *                 Element
     *                 representing the root.
     * @param t
     *                 the Tag to use for the first Element in
     *                 <code>elements</code>
     */
    private Style createResolvedStyle(String selector, Vector elements,
        ...
        for (int counter = 0; counter < numElements; counter++) {
            ...
            if (attr != null) {
                HTML.Tag tag = (HTML.Tag) attr.getAttribute(
                ...
                if (attr.isDefined(HTML.Attribute.CLASS)) {
                    classes[counter] = attr.getAttribute(HTML.Attribute.CLASS)
                            .toString();
                } else {
    ...
    /**
     * Creates and returns a Style containing all the rules that match
     * <code>selector</code>. It is assumed that each simple selector in
     * <code>selector</code> is separated by a space.
     */
    private Style createResolvedStyle(String selector) {
        ...
        try {
            ...
            while (lastIndex < length) {
                ...
                if (dotIndex != -1 && poundIndex != -1 && dotIndex < spaceIndex
                        && poundIndex < spaceIndex) {
                    if (poundIndex < dotIndex) {
                        ...
                        if ((poundIndex + 1) == dotIndex) {
                            ...
                            elements.addElement(selector.substring(poundIndex
                                    + 1, dotIndex));
                        }
            ...
            for (int index = 0, eIndex = total
                    - 3; index < numTags; index++, eIndex -= 3) {
     ...
     * Goes through and refreshes the necessary resolved rules.
     */
    private synchronized void refreshResolvedRules(String selectorName,
            String[] selector, Style newStyle, int specificity) {
        if (resolvedStyles.size() > 0) {
            Enumeration<ResolvedStyle> values = resolvedStyles.elements();
            while (values.hasMoreElements()) {
                ResolvedStyle style = values.nextElement();
     ...
     * This is used to avoid allocing a lot of garbage when searching for rules.
     * Use the static method obtainSearchBuffer and releaseSearchBuffer to get a
     * SearchBuffer, and release it when done.
     */
    private static class SearchBuffer {
        ...
        /**
         * Returns an instance of SearchBuffer. Be sure and issue a
         * releaseSearchBuffer when done with it.
         */
        ...
        /**
         * Adds <code>sb</code> to the stack of SearchBuffers that can be used.
         */
    ...
    /**
     * Class to carry out some of the duties of CSS formatting. Implementations
     * of this class enable views to present the CSS formatting while not
     * knowing anything about how the CSS values are being cached.
     * <p>
     * As a delegate of Views, this object is responsible for the insets of a
     * View and making sure the background is maintained according to the CSS
     * attributes.
     */
    public static class BoxPainter implements Serializable {
        ...
        /**
         * Fetches a border to render for the given attributes. PENDING(prinz)
         * This is pretty badly hacked at the moment.
         */
        ...
        /**
         * Fetches the color to use for borders. This will either be the value
         * specified by the border-color attribute (which is not inherited), or
         * it will default to the color attribute (which is inherited).
         */
        ...
        /**
         * Fetches the inset needed on a given side to account for the margin,
         * border, and padding.
         *
         * @param side
         *             The size of the box to fetch the inset for. This can be
         *             View.TOP, View.LEFT, View.BOTTOM, or View.RIGHT.
         * @param v
         *             the view making the request. This is used to get the
         *             AttributeSet, and may be used to resolve percentage
         *             arguments.
        ...
        public void paint(Graphics g, float x, float y, float w, float h,
            ...
            float localLeftMargin = getOrientationMargin(HorizontalMargin.LEFT,
                    leftMargin, a, isLeftToRight);
            float localRightMargin = getOrientationMargin(
                    HorizontalMargin.RIGHT, rightMargin, a, isLeftToRight);
            ...
            if (bg != null) {
                g.setColor(bg);
                g.fillRect((int) (x + dx), (int) (y + dy), (int) (w + dw),
                        (int) (h + dh));
            }
            if (bgPainter != null) {
        ...
        /*
         * only certain tags are concerned about orientation <dir>, <menu>,
         * <ul>, <ol> for all others we return true. It is implemented this way
        ...
        static boolean isOrientationAware(View v) {
            ...
            AttributeSet attr;
            Object obj;
            if (v != null && (attr = v.getElement().getAttributes()) != null
                    && (obj = attr.getAttribute(
                            StyleConstants.NameAttribute)) instanceof HTML.Tag
                    && (obj == HTML.Tag.DIR || obj == HTML.Tag.MENU
                            || obj == HTML.Tag.UL || obj == HTML.Tag.OL)) {
                ret = true;
            }
         ...
         * for <dir>, <menu>,
         * <ul>
         * etc. margins are Left-To-Right/Right-To-Left depended. see 5088268
         * for more details margin-(left|right)-(ltr|rtl) were introduced to
         * describe it if margin-(left|right) is present we are to use it.
         *
         * @param side
         *                      The horizontal side to fetch margin for This can
         *                      be
         *                      HorizontalMargin.LEFT or HorizontalMargin.RIGHT
         * @param cssMargin
         *                      margin from css
         * @param a
         *                      AttributeSet for the View we getting margin for
         * @param isLeftToRight
        ...
        float getOrientationMargin(HorizontalMargin side, float cssMargin,
            ...
            switch (side) {
                case RIGHT: {
                    orientationMargin = (isLeftToRight) ? getLength(
                            CSS.Attribute.MARGIN_RIGHT_LTR, a)
                ...
                case LEFT: {
                    orientationMargin = (isLeftToRight) ? getLength(
                            CSS.Attribute.MARGIN_LEFT_LTR, a)
     ...
     * Implementations of this class enable views to present the CSS formatting
     * while not knowing anything about how the CSS values are being cached.
     */
    public static class ListPainter implements Serializable {
        ...
        ListPainter(AttributeSet attr, StyleSheet ss) {
            ...
            String imgstr = (String) attr.getAttribute(
            ...
            if (img == null) {
                type = (CSS.Value) attr.getAttribute(
        ...
        /**
         * Returns a string that represents the value of the HTML.Attribute.TYPE
         * attribute. If this attributes is not defined, then then the type
         * defaults to "disc" unless the tag is on Ordered list. In the case of
         * the latter, the default type is "decimal".
         */
         ...
         * <code>childIndex</code> + 1, unless <code>parentView</code> has some
         * Views that do not represent LI's, or one of the views has a
         * HTML.Attribute.START specified.
         */
        private int getRenderIndex(View parentView, int childIndex) {
            ...
            for (int counter = childIndex; counter >= 0; counter--) {
                AttributeSet as = parentView.getElement().getElement(counter)
                        .getAttributes();
                if (as.getAttribute(
                        StyleConstants.NameAttribute) != HTML.Tag.LI) {
                    retIndex--;
         ...
         * @param item
         *             which list item is being painted. This is a number
         *             greater
         *             than or equal to 0.
         */
        public void paint(Graphics g, float x, float y, float w, float h,
            ...
            isLeftToRight = host.getComponentOrientation().isLeftToRight();
            ...
            if (cv.getViewCount() > 0) {
                ...
                Object cName = pView.getElement().getAttributes().getAttribute(
                        StyleConstants.NameAttribute);
                if ((cName == HTML.Tag.P || cName == HTML.Tag.IMPLIED) && pView
                        .getViewCount() > 0) {
            ...
            Color c = (host.isEnabled() ? (ss != null ? ss.getForeground(cv
                    .getAttributes()) : host.getForeground())
            ...
            Font font = ((StyledDocument) cv.getDocument()).getFont(cv
                    .getAttributes());
            if (font != null) {
                g.setFont(font);
            }
            if (childtype == CSS.Value.SQUARE || childtype == CSS.Value.CIRCLE
                    || childtype == CSS.Value.DISC) {
                drawShape(g, childtype, (int) x, (int) y, (int) w, (int) h,
                        align);
         ...
         * @param ax
         *              x coordinate to place the bullet
         * @param ay
         *              y coordinate to place the bullet
         * @param aw
         *              width of the container the bullet is placed in
         * @param ah
         *              height of the container the bullet is placed in
         * @param align
         *              preferred alignment factor for the child view
         */
        void drawIcon(Graphics g, int ax, int ay, int aw, int ah, float align,
                Component c) {
         ...
         *              the graphics context
         * @param type
         *              type of bullet to draw (circle, square, disc)
         * @param ax
         *              x coordinate to place the bullet
         * @param ay
         *              y coordinate to place the bullet
         ...
         */
        void drawLetter(Graphics g, char letter, int ax, int ay, int aw, int ah,
        ...
        /**
         * Converts the item number into the ordered list number (i.e. 1 2 3, i
         * ii iii, a b c, etc.
         *
         * @param itemNum
         *                number to format
        ...
        static final char romanChars[][] = { { 'i', 'v' }, { 'x', 'l' }, { 'c',
                'd' }, { 'm', '?' }, };
    ...
    static class BackgroundImagePainter implements Serializable {
        ...
        BackgroundImagePainter(AttributeSet a, CSS css, StyleSheet ss) {
            ...
            CSS.BackgroundPosition pos = (CSS.BackgroundPosition) a
                    .getAttribute(CSS.Attribute.BACKGROUND_POSITION);
            if (pos != null) {
            ...
            CSS.Value repeats = (CSS.Value) a.getAttribute(
        ...
        void paint(Graphics g, float x, float y, float w, float h, View v) {
            ...
            if ((flags & 3) == 0) {
                ...
                if ((flags & 4) == 4) {
                    paintX = (int) (x + w * hPosition - (float) width
                            * hPosition);
                } else {
                    paintX = (int) x + (int) hPosition;
                }
                if ((flags & 8) == 8) {
                    paintY = (int) (y + h * vPosition - (float) height
                            * vPosition);
                } else {
                    paintY = (int) y + (int) vPosition;
                }
                if (clip == null || !((paintX + width <= clip.x) || (paintY
                        + height <= clip.y) || (paintX >= clip.x + clip.width)
        ...
        private boolean updatePaintCoordinates(Rectangle clip, int width,
                int height) {
            if ((flags & 3) == 1) {
                paintMaxY = paintY + 1;
            } else if ((flags & 3) == 2) {
                ...
                if ((flags & 1) == 1) {
                    if ((clip.x + clip.width) < paintMaxX) {
                        if ((clip.x + clip.width - paintX) % width == 0) {
                            ...
                            paintMaxX = ((clip.x + clip.width - paintX) / width
                                    + 1) * width + paintX;
                        }
                    }
                    if (clip.x > paintX) {
    ...
    /**
     * A subclass of MuxingAttributeSet that translates between CSS and HTML and
     * StyleConstants. The AttributeSets used are the CSS rules that match the
     * Views Elements.
     */
    class ViewAttributeSet extends MuxingAttributeSet {
                        ...
                        while (keys.hasMoreElements()) {
                            ...
                            if (key instanceof HTML.Tag) {
                                if (key == HTML.Tag.A) {
                                     ...
                                     * apply only for tags that have their href
                                     * attribute defined and not for anchors
                                     * that only have their name attributes
                                     * defined, i.e anchors that function as
                                     * destinations. Hence we do not add the
                                     * attributes for that latter kind of
                                     * anchors. When CSS2 support is added, it
                                     * will be possible to specificity this kind
                                     * of conditional behaviour in the
                                     * stylesheet.
                                     **/
                                    if (o != null
                                            && o instanceof AttributeSet) {
                                ...
                                AttributeSet cssRule = styles.getRule(
                                        (HTML.Tag) key, elem);
                                if (cssRule != null) {
        ...
        /**
         * Checks whether a given attribute is defined. This will convert the
         * key over to CSS if the key is a StyleConstants key that has a CSS
         * mapping.
         *
         * @param key
         *            the attribute key
        ...
        /**
         * Gets the value of an attribute. If the requested attribute is a
         * StyleConstants attribute that has a CSS mapping, the request will be
         * converted.
         *
         * @param key
         *            the attribute name
        ...
        /**
         * If not overriden, the resolving parent defaults to the parent
         * element.
         *
    ...
    /**
     * A subclass of MuxingAttributeSet that implements Style. Currently the
     * MutableAttributeSet methods are unimplemented, that is they do nothing.
     */
    ...
    static class ResolvedStyle extends MuxingAttributeSet implements
        ...
        /**
         * Inserts a Style into the receiver so that the styles the receiver
         * represents are still ordered by specificity. <code>style</code> will
         * be added before any extended styles, that is before extendedIndex.
         */
        ...
        /**
         * Adds <code>s</code> as one of the Attributesets to look up attributes
         * in.
         */
        ...
        /**
         * Adds <code>s</code> as one of the AttributeSets to look up attributes
         * in. It will be the AttributeSet last checked.
         */
        ...
        /**
         * Removes the style at <code>index</code> + <code>extendedIndex</code>.
         */
        ...
        /**
         * Returns true if the receiver matches <code>selector</code>, where a
         * match is defined by the CSS rule matching. Each simple selector must
         ...
         * thisCurrent, thisLast matches the substring of selector in the ranme
         * sCurrent to sLast based on CSS selector matching.
         */
        boolean matches(String selector, int sCurrent, int sLast,
            ...
            if (sDotIndex != -1) {
                ...
                if (sCurrent == sDotIndex) {
                    if ((thisLast - thisDotIndex) != (sLast - sDotIndex)
                            || !selector.regionMatches(sCurrent, name,
                                    thisDotIndex, (thisLast - thisDotIndex))) {
                        return false;
                    }
                } else {
                    // Has to fully match.
                    if ((sLast - sCurrent) != (thisLast - thisCurrent)
                            || !selector.regionMatches(sCurrent, name,
                                    thisCurrent, (thisLast - thisCurrent))) {
                        return false;
                    }
                }
                return true;
            ...
            return (((thisLast - thisCurrent) == (sLast - sCurrent)) && selector
                    .regionMatches(sCurrent, name, thisCurrent, thisLast
                            - thisCurrent));
        }

        /**
         * Similar to String.indexOf, but allows an upper bound (this is slower
         * in that it will still check string starting at start.
         */
     ...
     * Style. It can also reference children <code>SelectorMapping</code>s, so
     * that it behaves like a tree.
     * <p>
     * This is not thread safe, it is assumed the caller will take the necessary
     * precations if this is to be used in a threaded environment.
     */
    static class SelectorMapping implements Serializable {
         ...
         * <code>selector</code>, and <code>create</code> is true, a new one
         * will be created.
         */
        ...
        /**
         * Returns the specificity for the child selector <code>selector</code>.
         */
        ...
        private Style style;
        /**
         * Any sub selectors. Key will be String, and value will be another
         * SelectorMapping.
         */
    ...
    /**
     * Maps from selector (as a string) to Style that includes all relevant
     * styles.
     */
    ...
    /**
     * Default parser for CSS specifications that get loaded into the
     * StyleSheet.
     * <p>
     * This class is NOT thread safe, do not ask it to parse while it is in the
     * middle of parsing.
     */
    class CssParser implements CSSParser.CSSParserCallback {
         ...
         * <code>importStyleSheet</code> if a <code>MalformedURLException</code>
        ...
        /**
         * True if parsing a declaration, that is the Reader will not contain a
         * selector.
         */
    ...
    void rebaseSizeMap(int base) {
        ...
        for (int i = 0; i < sizeMapDefault.length; i++) {
            sizeMap[i] = Math.max(base * sizeMapDefault[i]
                    / sizeMapDefault[CSS.baseFontSizeIndex], minimalFontSize);
        }
