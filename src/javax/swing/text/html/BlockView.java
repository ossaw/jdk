...
public class BlockView extends BoxView {
    ...
    /**
     * Creates a new view that represents an html box. This can be used for a
     * number of elements.
     *
     * @param elem
     ...
     * {@link #setPropertiesFromAttributes()} method to set the paragraph
     * properties from the css attributes. The call is made at this time to
     * ensure the ability to resolve upward through the parents view attributes.
     *
     * @param parent
    ...
    /**
     * Calculate the requirements of the block along the major axis (i.e. the
     * axis along with it tiles). This is implemented to provide the superclass
     * behavior and then adjust it if the CSS width or height attribute is
    ...
    /**
     * Calculate the requirements of the block along the minor axis (i.e. the
     * axis orthogonal to the axis along with it tiles). This is implemented to
     * provide the superclass behavior and then adjust it if the CSS width or
     * height attribute is specified and applicable to the axis.
     */
    protected SizeRequirements calculateMinorAxisRequirements(int axis, SizeRequirements r) {
        ...
        if (!spanSetFromAttributes(axis, r, cssWidth, cssHeight)) {
             ...
             */
            /*
             * int min = 0; long pref = 0; int max = 0; int n = getViewCount();
             * for (int i = 0; i < n; i++) { View v = getView(i); min =
             * Math.max((int) v.getMinimumSpan(axis), min); pref =
             * Math.max((int) v.getPreferredSpan(axis), pref); if ( max =
             * Math.max((int) v.getMaximumSpan(axis), max);
        ...
        /*
         * Set the alignment based upon the CSS properties if it is specified.
         * For X_AXIS this would be text-align, for Y_AXIS this would be
         * vertical-align.
         */
        if (axis == X_AXIS) {
     ...
     */
    static boolean spanSetFromAttributes(int axis, SizeRequirements r, CSS.LengthValue cssWidth,
    ...
    /**
     * Constrains <code>want</code> to fit in the minimum size specified by
     * <code>min</code>.
     */
