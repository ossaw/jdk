 ...
 * &lt;input&gt;, &lt;textarea&gt;, and &lt;select&gt;. The model for the
 * component is stored as an attribute of the the element (using
 * StyleConstants.ModelAttribute), and is used to build the component of the
 * view. The type of the model is assumed to of the type that would be set by
 * <code>HTMLDocument.HTMLReader.FormAction</code>. If there are multiple views
 * mapped over the document, they will share the embedded component models.
 * <p>
...
public class FormView extends ComponentView implements ActionListener {
	...
	protected Component createComponent() {
		...
		if (t == HTML.Tag.INPUT) {
			...
			int rows = HTML.getIntegerAttributeValue(attr, HTML.Attribute.ROWS,
					1);
			area.setRows(rows);
			int cols = HTML.getIntegerAttributeValue(attr, HTML.Attribute.COLS,
					20);
			maxIsPreferred = 3;
		...
		if (model instanceof DefaultButtonModel) {
			...
			for (ListDataListener listener : listModel.getListDataListeners()) {
				if (listenerClass1.equals(listener.getClass().getName())
						|| listenerClass2.equals(listener.getClass()
								.getName())) {
	...
	/**
	 * Responsible for processing the ActionEvent. If the element associated
	...
	protected void submitData(String data) {
		...
		URL actionURL;
		try {
			actionURL = (action == null) ? new URL(base.getProtocol(), base
					.getHost(), base.getPort(), base.getFile())
					: new URL(base, action);
			if (!isPostMethod) {
		...
		if (!kit.isAutoFormSubmission() || doc.isFrameDocument()) {
			...
			formEvent = new FormSubmitEvent(FormView.this,
					HyperlinkEvent.EventType.ACTIVATED, actionURL, form, target,
					methodType, data);
	...
	private void storePostData(HTMLDocument doc, String target, String data) {
		...
		if (doc.isFrameDocument()) {
			...
			FrameView.FrameEditorPane p = (FrameView.FrameEditorPane) getContainer();
		...
		if (parent != null) {
			...
			while ((next = it.next()) != null) {
				if (next == elem) {
					...
					if (HTMLDocument.matchNameAttribute(elemAttr,
							HTML.Tag.INPUT)) {
	...
	void resetForm() {
		...
		if (parent != null) {
			...
			while ((next = it.next()) != null) {
				if (isControl(next)) {
					AttributeSet elemAttr = next.getAttributes();
					Object m = elemAttr.getAttribute(
							StyleConstants.ModelAttribute);
					if (m instanceof TextAreaDocument) {
						...
						try {
							...
							if (HTMLDocument.matchNameAttribute(elemAttr,
									HTML.Tag.INPUT)) {
								String value = (String) elemAttr.getAttribute(
	...
	private class BrowseFileAction implements ActionListener {
		...
		public void actionPerformed(ActionEvent ae) {
			...
			if (fc.showOpenDialog(
					getContainer()) == JFileChooser.APPROVE_OPTION) {
