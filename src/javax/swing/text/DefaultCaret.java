...
public class DefaultCaret extends Rectangle implements Caret, FocusListener, MouseListener,
     ...
     * changes are performed on the Event Dispatching Thread.
     * 
     * @see #setUpdatePolicy
     * @see #getUpdatePolicy
     * @since 1.5
    ...
    /**
     * Indicates that the caret should remain at the same absolute position in
     * the document regardless of any document updates, except when the document
     * length becomes less than the current caret position due to removal. In
     * that case the caret position is adjusted to the end of the document.
     *
     * @see #setUpdatePolicy
     * @see #getUpdatePolicy
     * @since 1.5
    ...
    /**
     * Indicates that the caret position is to be <b>always</b> updated
     * accordingly to the document changes regardless whether the document
     * updates are performed on the Event Dispatching Thread or not.
     *
     * @see #setUpdatePolicy
     * @see #getUpdatePolicy
     * @since 1.5
     ...
     * caret updates its absolute position within the document on insertions
     * occurred before or at the caret position and on removals before the caret
     * position. 'Absolute position' means here the position relative to the
     * start of the document. For example if a character is typed within
     * editable text component it is inserted at the caret position and the
     * caret moves to the next absolute position within the document due to
     * insertion and if <code>BACKSPACE</code> is typed then caret decreases its
     * absolute position due to removal of a character before it. Sometimes it
     * may be useful to turn off the caret position updates so that the caret
     * stays at the same absolute position within the document position
     * regardless of any document updates.
     * <p>
     * The following update policies are allowed:
     * <ul>
     * <li><code>NEVER_UPDATE</code>: the caret stays at the same absolute
     * position in the document regardless of any document updates, except when
     * document length becomes less than the current caret position due to
     * removal. In that case caret position is adjusted to the end of the
     * document. The caret doesn't try to keep itself visible by scrolling the
     * associated view when using this policy.</li>
     * <li><code>ALWAYS_UPDATE</code>: the caret always tracks document changes.
     * For regular changes it increases its position if an insertion occurs
     * before or at its current position, and decreases position if a removal
     * occurs before its current position. For undo/redo updates it is always
     * moved to the position where update occurred. The caret also tries to keep
     * itself visible by calling <code>adjustVisibility</code> method.</li>
     * <li><code>UPDATE_WHEN_ON_EDT</code>: acts like <code>ALWAYS_UPDATE</code>
     * if the document updates are performed on the Event Dispatching Thread and
     * like <code>NEVER_UPDATE</code> if updates are performed on other thread.
     * </li>
     * </ul>
     * <p>
    ...
    private void selectWord(MouseEvent e) {
        if (selectedWordEvent != null && selectedWordEvent.getX() == e.getX() && selectedWordEvent.getY() == e
                .getY()) {
        ...
        a.actionPerformed(new ActionEvent(getComponent(), ActionEvent.ACTION_PERFORMED, null, e.getWhen(), e
                .getModifiers()));
        selectedWordEvent = e;
    }
        ...
        if (!e.isConsumed()) {
            if (SwingUtilities.isLeftMouseButton(e)) {
                ...
                if (nclicks == 1) {
                    ...
                    a.actionPerformed(new ActionEvent(getComponent(), ActionEvent.ACTION_PERFORMED, null, e
                            .getWhen(), e.getModifiers()));
                }
            } else if (SwingUtilities.isMiddleMouseButton(e)) {
                // mouse 2 behavior
                if (nclicks == 1 && component.isEditable() && component.isEnabled() && SwingUtilities2
     ...
     * <code><em>Foo</em>Listener</code>s upon this caret. <code><em>Foo</em>
     ...
     * you can query a <code>DefaultCaret</code> <code>c</code> for its change
     ...
     * <p>
     * This method returns whether or not the <code>Caret</code> is currently in
     * a blinking state. It does not provide information as to whether it is
     * currently blinked on or off. To determine if the caret is currently
     * painted use the <code>isVisible</code> method.
     *
    ...
    /**
     * Indicates whether or not the caret is currently visible. As the caret
     * flashes on and off the return value of this will change between true,
     * when the caret is painted, and false, when the caret is not painted.
     * <code>isActive</code> indicates whether or not the caret is in a blinking
     * state, such that it <b>can</b> be visible, and <code>isVisible</code>
     * indicates whether or not the caret <b>is</b> actually visible.
     * <p>
     * Subclasses that wish to render a different flashing caret should override
     * paint and only paint the caret if this method returns true.
     *
     ...
     * understand the relationship between this method, <code>isVisible</code>
     * and <code>isActive</code>. Calling this method with a value of
     * <code>true</code> activates the caret blinking. Setting it to
     * <code>false</code> turns it completely off. To determine whether the
     * blinking is active, you should call <code>isActive</code>. In effect,
     * <code>isActive</code> is an appropriate corresponding "getter" method for
     * this one. <code>isVisible</code> can be used to fetch the current
     ...
     * <code>isActive</code> and <code>isVisible</code> after calling this
     * method:
     * <p>
     * <b><code>setVisible(true)</code></b>:
     * <ul>
     * <li>isActive(): true</li>
     * <li>isVisible(): true or false depending on whether or not the caret is
     * blinked on or off</li>
     * </ul>
     * <p>
    ...
    /**
     * Sets the caret position and mark to the specified position, with a
     * forward bias. This implicitly sets the selection range to zero.
     *
     * @param dot
     *        the position &gt;= 0
    ...
    /**
     * Moves the caret position to the specified position, with the specified
     * bias.
     *
     * @param dot
     *        the position &gt;= 0
    ...
    Position.Bias guessBiasForOffset(int offset, Position.Bias lastBias, boolean lastLTR) {
        ...
        } else if (lastBias != Position.Bias.Backward && lastLTR != isPositionLTR(offset,
     ...
     * happening on the event thread so that calling <code>modelToView</code> is
     * safe.
     */
    ...
    private void updateSystemSelection() {
        ...
        if (this.dot != this.mark && component != null && component.hasFocus()) {
            ...
            if (clip != null) {
                String selectedText;
                if (component instanceof JPasswordField && component.getClientProperty(
        ...
        public void insertUpdate(DocumentEvent e) {
            if (getUpdatePolicy() == NEVER_UPDATE || (getUpdatePolicy() == UPDATE_WHEN_ON_EDT
                ...
                if ((e.getOffset() <= dot || e.getOffset() <= mark) && selectionTag != null) {
                    try {
                        component.getHighlighter().changeHighlight(selectionTag, Math.min(dot, mark), Math
                                .max(dot, mark));
