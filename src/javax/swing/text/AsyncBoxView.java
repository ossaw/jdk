...
/**
 * A box that does layout asynchronously. This is useful to keep the GUI event
 * thread moving by not doing any layout on it. The layout is done on a
 * granularity of operations on the child views. After each child view is
 * accessed for some part of layout (a potentially time consuming operation) the
 * remaining tasks can be abandoned or a new higher priority task (i.e. to
 * service a synchronous request or a visible area) can be taken on.
 * <p>
 * While the child view is being accessed a read lock is acquired on the
 * associated document so that the model is stable while being accessed.
 *
...
public class AsyncBoxView extends View {
	...
	/**
	 * Fetch the major axis (the axis the children are tiled along). This will
	 * have a value of either X_AXIS or Y_AXIS.
	 */
	...
	/**
	 * Fetch the minor axis (the axis orthogonal to the tiled axis). This will
	 * have a value of either X_AXIS or Y_AXIS.
	 */
	 ...
	 *
	 * @param axis
	 *            the axis to determine the total insets along, either X_AXIS or
	 *            Y_AXIS.
	 * @since 1.4
	 */
	...
	/**
	 * Set the estimatedMajorSpan property that determines if the major span
	 * should be treated as being estimated. If this property is true, the value
	 * of setSize along the major axis will change the requirements along the
	 * major axis and incremental changes will be ignored until all of the
	 * children have been updated (which will cause the property to
	 * automatically be set to false). If the property is false the value of the
	 * majorSpan will be considered to be accurate and incremental changes will
	 * be added into the total as they are calculated.
	 *
	 * @since 1.4
	 */
	...
	/**
	 * Fetch the object representing the layout state of of the child at the
	 * given index.
	 *
	 * @param index
	...
	/**
	 * New ChildState records are created through this method to allow
	 * subclasses the extend the ChildState records to do/hold more
	 */
	...
	/**
	 * Requirements changed along the major axis. This is called by the thread
	 * doing layout for the given ChildState object when it has completed
	 * fetching the child views new preferences. Typically this would be the
	 * layout thread, but might be the event thread if it is trying to update
	 * something immediately (such as to perform a model/view translation).
	 * <p>
	 * This is implemented to mark the major axis as having changed so that a
	 * future check to see if the requirements need to be published to the
	 * parent view will consider the major axis. If the span along the major
	...
	/**
	 * Requirements changed along the minor axis. This is called by the thread
	 * doing layout for the given ChildState object when it has completed
	 * fetching the child views new preferences. Typically this would be the
	 * layout thread, but might be the GUI thread if it is trying to update
	 * something immediately (such as to perform a model/view translation).
	 */
	...
	/**
	 * Publish the changes in preferences upward to the parent view. This is
	 * normally called by the layout thread.
	 */
	...
	/**
	 * Calls the superclass to update the child views, and updates the status
	 * records for the children. This is expected to be called while a write
	 * lock is held on the model so that interaction with the layout thread will
	 * not happen (i.e. the layout thread acquires a read lock before doing
	 * anything).
	 *
	 * @param offset
	 ...
	 * {@link #setParent setParent} method. Subclasses can reimplement this to
	 * initialize their child views in a different manner. The default
	 * implementation creates a child view for each child element.
	 * <p>
	 * Normally a write-lock is held on the Document while the children are
	 * being changed, which keeps the rendering and layout threads safe. The
	 * exception to this is when the view is initialized to represent an
	 * existing element (via this method), so it is synchronized to exclude
	 ...
	 * model. This is implemented to fetch the view in the case where there is a
	...
	/**
	 * Update the layout in response to receiving notification of change from
	 * the model. This is implemented to note the change on the ChildLocator so
	...
	/**
	 * Sets the parent of the view. This is reimplemented to provide the
	 * superclass behavior as well as calling the <code>loadChildren</code>
	 * method if this view does not already have children. The children should
	 * not be loaded in the constructor because the act of setting the parent
	 * may cause them to try to search up the hierarchy (to get the hosting
	 * Container for example). If this view has children (the view is being
	 * moved from one place in the view hierarchy to another), the
	 ...
	 * has changed and should be reconsidered for layout. This is reimplemented
	 * to queue new work on the layout thread. This method gets messaged from
	...
	/**
	 * Sets the size of the view. This should cause layout of the view if the
	 * view caches any layout information.
	 * <p>
	 * Since the major axis is updated asynchronously and should be the sum of
	 * the tiled children the call is ignored for the major axis. Since the
	 * minor axis is flexible, work is queued to resize the children if the
	 * minor span changes.
	 *
	 * @param width
	 *            the width &gt;= 0
	...
	/**
	 * Sets the size of the view along an axis. Since the major axis is updated
	 * asynchronously and should be the sum of the tiled children the call is
	 * ignored for the major axis. Since the minor axis is flexible, work is
	 ...
	 * <p>
	 * This is implemented to determine whether or not the desired region to be
	 * rendered (i.e. the unclipped area) is up to date or not. If up-to-date
	 * the children are rendered. If not up-to-date, a task to build the desired
	 * area is placed on the layout queue as a high priority task. This keeps by
	 * event thread moving by rendering if ready, and postponing until a later
	...
	/**
	 * Gets the nth child view. Since there are no children by default, this
	 * returns null.
	 *
	 * @param n
	 ...
	 * location. This returns null since the default is to not have any child
	 * views.
	 *
	 * @param index
	 ...
	 * coordinate space of the model. The biasReturn argument will be filled in
	 * to indicate that the point given is closer to the next character in the
	 * model or the previous character in the model.
	 * <p>
	 * This is expected to be called by the GUI thread, holding a read-lock on
	 * the associated model. It is implemented to locate the child view and
	 * determine it's allocation with a lock on the ChildLocator object, and to
	 * call viewToModel on the child view with a lock on the ChildState object
	 ...
	 * access to some of the locations in the model. This method enables
	...
	/**
	 * Current span along the major axis. This is also the value returned by
	 * getMinimumSize, getPreferredSize, and getMaximumSize along the major
	 * axis.
	 */
	float majorSpan;
	...
	/**
	 * Current span along the minor axis. This is what layout was done against
	 * (i.e. things are flexible in this direction).
	 */
	float minorSpan;

	/**
	 * Object that manages the offsets of the children. All locking for
	 * management of child locations is on this object.
	 */
	...
	/**
	 * Child that is actively changing size. This often causes a
	 * preferenceChanged, so this is a cache to possibly speed up the marking
	 * the state. It also helps flag an opportunity to avoid adding to flush
	 * task to the layout queue.
	 */
	ChildState changing;

	/**
	 * A class to manage the effective position of the child views in a
	 * localized area while changes are being made around the localized area.
	 * The AsyncBoxView may be continuously changing, but the visible area needs
	 * to remain fairly stable until the layout thread decides to publish an
	...
	public class ChildLocator {
		...
		/**
		 * Notification that a child changed. This can effect whether or not new
		 * offset calculations are needed. This is called by a ChildState object
		 * that has changed it's major span. This can therefore be called by
		 * multiple threads.
		 */
		public synchronized void childChanged(ChildState cs) {
			if (lastValidOffset == null) {
				lastValidOffset = cs;
			} else if (cs.getChildView().getStartOffset() < lastValidOffset.getChildView()
					.getStartOffset()) {
				lastValidOffset = cs;
			}
		}
		...
		/**
		 * Fetch the allocation to use for a child view. This will update the
		 * offsets for all children not yet updated before the given index.
		 */
		public synchronized Shape getChildAllocation(int index, Shape a) {
			...
			if (cs.getChildView().getStartOffset() > lastValidOffset.getChildView()
					.getStartOffset()) {
		...
		/**
		 * Fetches the child view index at the given point. This is called by
		 * the various View methods that need to calculate which child to
		 * forward a message to. This should be called by a block synchronized
		 * on this object, and would typically be followed with one or more
		 * calls to getChildAllocation that should also be in the synchronized
		 * block.
		 *
		 * @param x
		 *            the X coordinate &gt;= 0
		...
		/**
		 * Copy the currently allocated shape into the Rectangle used to store
		 * the current allocation. This would be a floating point rectangle in a
		 * Java2D-specific implementation.
		 */
		...
		/**
		 * Locate the view responsible for an offset into the box along the
		 * major axis. Make sure that offsets are set on the ChildState objects
		 * up to the given target span past the desired offset.
		 *
		 * @return index of the view representing the given visual location
		 *         (targetOffset), or -1 if no view represents that location
		 */
		...
		/**
		 * Move the location of the last offset calculation forward to the
		 * desired offset.
		 */
		...
		/**
		 * The last seen allocation (for repainting when changes are flushed
		 * upward).
		 */
		protected Rectangle lastAlloc;

		/**
		 * A shape to use for the child allocation to avoid creating a lot of
		 * garbage.
		 */
	...
	/**
	 * A record representing the layout state of a child view. It is runnable as
	 * a task on another thread. All access to the child view that is based upon
	 * a read-lock on the model should synchronize on this object (i.e. The
	 * layout thread and the GUI thread can both have a read lock on the model
	 * at the same time and are not protected from each other). Access to a
	 * child view hierarchy is serialized via synchronization on the ChildState
	 * instance.
	 * 
	 * @since 1.3
	 */
	public class ChildState implements Runnable {
		...
		/**
		 * Construct a child status. This needs to start out as fairly large so
		 * we don't falsely begin with the idea that all of the children are
		 * visible.
		 * 
		 * @since 1.4
		 */
		...
		/**
		 * Update the child state. This should be called by the thread that
		 * desires to spend time updating the child state (intended to be the
		 * layout thread).
		 * <p>
		 * This acquires a read lock on the associated document for the duration
		 * of the update to ensure the model is not changed while it is
		 * operating. The first thing to do would be to see if any work actually
		 * needs to be done. The following could have conceivably happened while
		 * the state was waiting to be updated:
		 * <ol>
		 * <li>The child may have been removed from the view hierarchy.
		 * <li>The child may have been updated by a higher priority operation
		 * (i.e. the child may have become visible).
		 * </ol>
		 */
		public void run() {
			AbstractDocument doc = (AbstractDocument) getDocument();
