...
/**
 * An implementation of a gapped buffer similar to that used by emacs. The
 * underlying storage is a java array of some type, which is known only by the
 * subclass of this class. The array has a gap somewhere. The gap is moved to
 * the location of changes to take advantage of common behavior where most
 * changes occur in the same location. Changes that occur at a gap boundary are
 * generally cheap and moving the gap is generally cheaper than moving the array
 * contents directly to accommodate the change.
 *
...
abstract class GapVector implements Serializable {
	...
	/**
	 * Access to the array. The actual type of the array is known only by the
	 * subclass.
	 */
	...
	/**
	 * Replace the given logical position in the storage with the given new
	 * items. This will move the gap to the area being changed if the gap is not
	 * currently located at the change location.
	 *
	 * @param position
	 *            the location to make the replacement. This is not the location
	...
	/**
	 * Delete nItems at position. Squeezes any marks within the deleted area to
	 * position. This moves the gap to the best place by minimizing it's overall
	 * movement. The gap must intersect the target block.
	 */
