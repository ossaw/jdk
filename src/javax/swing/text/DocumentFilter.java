 ...
 * and <code>Element</code> related mutation methods such as <code>create</code>
 * , <code>insert</code> and <code>removeElement</code> in
...
public class DocumentFilter {
	 ...
	 * Document implementations that wish to support a DocumentFilter must
	 * provide an implementation that will not callback into the DocumentFilter
	 * when the following methods are invoked from the DocumentFilter.
	 * 
	 * @since 1.4
	 */
	public static abstract class FilterBypass {
		 ...
		 *            the number of characters to remove &gt;= 0
		 * @exception BadLocationException
		 *                some portion of the removal range was not a valid part
		 *                of the document. The location in the exception is the
		 ...
		 *            is legal.
		 * @exception BadLocationException
		 *                the given insert is not a valid position within the
		 *                document
		 */
