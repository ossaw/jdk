 ...
 * <code>DefaultEditorKit</code>, called <a href=#EndOfLineStringProperty>
 * <code>EndOfLineStringProperty</code></a>, which is defined automatically when
 * a document is loaded, to be the first occurrence of any of the newline
 * characters. When a document is loaded, <code>EndOfLineStringProperty</code>
 * is set appropriately, and when the document is written back out, the
 * <code>EndOfLineStringProperty</code> is used. But while the document is in
 * memory, the "\n" character is used to define a newline, regardless of how the
 * newline is defined when the document is on disk. Therefore, for searching
 * purposes, "\n" should always be used. When a new document is created, and the
 * <code>EndOfLineStringProperty</code> has not been defined, it will use the
 * System property when writing out the document.
 * <p>
...
public class DefaultEditorKit extends EditorKit {
	 ...
	 * default is <code>text/plain</code>.
	 *
	 * @return the type
	 */
	...
	/**
	 * Fetches a caret that can navigate through views produced by the
	 * associated ViewFactory.
	 *
	 * @return the caret
	 */
	...
	/**
	 * Creates an uninitialized text storage model (PlainDocument) that is
	...
	/**
	 * Gets the input attributes for the pane. This method exists for the
	 * benefit of StyledEditorKit so that the read method will pick up the
	 * correct attributes to apply to inserted text. This class's implementation
	 * simply returns null.
	 *
	 * @return null
	 */
	...
	/**
	 * When reading a document if a CRLF is encountered a property with this
	 * name is added and the value will be "\r\n".
	 */
	...
	/**
	 * Name of the action to place content into the associated document. If
	 * there is a selection, it is removed before the new content is added.
	 * 
	 * @see #getActions
	 */
	...
	/**
	 * Name of the action to place a line/paragraph break into the document. If
	 * there is a selection, it is removed before the break is added.
	 * 
	 * @see #getActions
	 */
	...
	/**
	 * Name of the action to place a tab character into the document. If there
	...
	/**
	 * Name of the action to delete the character of content that precedes the
	...
	/**
	 * Name of the action to delete the character of content that follows the
	 * current caret position.
	 * 
	 * @see #getActions
	 */
	...
	/**
	 * Name of the action to delete the word that follows the beginning of the
	 * selection.
	 * 
	 * @see #getActions
	...
	/**
	 * Name of the action to delete the word that precedes the beginning of the
	 * selection.
	 * 
	 * @see #getActions
	 * @see JTextComponent#getSelectionStart
	...
	/**
	 * Name of the action to cut the selected region and place the contents into
	...
	/**
	 * Name of the action to copy the selected region and place the contents
	...
	/**
	 * Name of the action to paste the contents of the system clipboard into the
	 * selected region, or before the caret if nothing is selected.
	 * 
	...
	/**
	 * Name of the Action for extending the selection by moving the caret
	...
	/**
	 * Name of the <code>Action</code> for moving the caret to the beginning of
	 * a word.
	 * 
	 * @see #getActions
	 */
	...
	/**
	 * Name of the <code>Action</code> for moving the caret to the beginning of
	 * a word, extending the selection.
	 * 
	 * @see #getActions
	 */
	...
	/**
	 * Name of the Action for moving the caret to the end of a word, extending
	 * the selection.
	 * 
	 * @see #getActions
	 */
	...
	/**
	 * Name of the <code>Action</code> for moving the selection to the beginning
	 * of the previous word, extending the selection.
	 * 
	 * @see #getActions
	 */
	...
	/**
	 * Name of the <code>Action</code> for moving the caret to the end of a
	 * line.
	 * 
	 * @see #getActions
	 */
	...
	/**
	 * Name of the <code>Action</code> for moving the caret to the beginning of
	 * a line, extending the selection.
	 * 
	 * @see #getActions
	 */
	...
	/**
	 * Name of the <code>Action</code> for moving the caret to the beginning of
	 * a paragraph, extending the selection.
	 * 
	 * @see #getActions
	 */
	...
	/**
	 * Name of the <code>Action</code> for moving the caret to the end of a
	 * paragraph, extending the selection.
	 * 
	 * @see #getActions
	 */
	...
	/**
	 * Name of the <code>Action</code> for moving the caret to the end of the
	 * document.
	 * 
	 * @see #getActions
	 */
	 ...
	 * @see #getActions
	 */
	/* public */ static final String toggleComponentOrientationAction = "toggle-componentOrientation";

	/**
	 * Name of the action that is executed by default if a
	 * <em>key typed event</em> is received and there is no keymap entry.
	 * 
	 * @see #getActions
	 */
	...
	private static final Action[] defaultActions = { new InsertContentAction(),
			new DeletePrevCharAction(), new DeleteNextCharAction(), new ReadOnlyAction(),
			new DeleteWordAction(deletePrevWordAction), new DeleteWordAction(deleteNextWordAction),
			new WritableAction(), new CutAction(), new CopyAction(), new PasteAction(),
			...
			new PageAction(selectionPageRightAction, false, true), new InsertBreakAction(),
			new BeepAction(),
			...
			new BeginAction(beginAction, false), new EndAction(endAction, false),
			new BeginAction(selectionBeginAction, true), new EndAction(selectionEndAction, true),
			new DefaultKeyTypedAction(), new InsertTabAction(), new SelectWordAction(),
			new SelectLineAction(), new SelectParagraphAction(), new SelectAllAction(),
			new UnselectAction(), new ToggleComponentOrientationAction(), new DumpModelAction() };
	 ...
	 * characters and those with the ALT modifier. It allows Control-Alt
	 * sequences through as these form legitimate unicode characters on some PC
	 * keyboards.
	 * <p>
	...
	/**
	 * Places content into the associated document. If there is a selection, it
	 * is removed before the new content is added.
	 * <p>
	 * <strong>Warning:</strong> Serialized objects of this class will not be
	 * compatible with future Swing releases. The current serialization support
	 * is appropriate for short term storage or RMI between applications running
	...
	/**
	 * Places a line/paragraph break into the document. If there is a selection,
	 * it is removed before the break is added.
	 * <p>
	 * <strong>Warning:</strong> Serialized objects of this class will not be
	 * compatible with future Swing releases. The current serialization support
	 * is appropriate for short term storage or RMI between applications running
	...
	/**
	 * Pastes the contents of the system clipboard into the selected region, or
	 * before the caret if nothing is selected.
	 * <p>
	 * <strong>Warning:</strong> Serialized objects of this class will not be
	 * compatible with future Swing releases. The current serialization support
	 * is appropriate for short term storage or RMI between applications running
					...
					try {
						...
						if (visible.contains(dotBounds.x, dotBounds.y)) {
							...
							newIndex = target.viewToModel(new Point(x,
									constrainY(target, dotBounds.y + scrollAmount, 0)));
						} else {
							...
							if (direction == -1) {
								...
								newIndex = target
										.viewToModel(new Point(x, newVis.y + visible.height));
							}
						}
			...
			if (target != null) {
				...
				if (selectedIndex != -1) {
					if (left) {
						...
						selectedIndex = target.viewToModel(new Point(visible.x + visible.width - 1,
	...
	/*
	 * Action to move the selection by way of the getNextVisualPositionFrom
	 * method. Constructor indicates direction to use.
	 */
	static class NextVisualPositionAction extends TextAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (target != null) {
				Caret caret = target.getCaret();
				DefaultCaret bidiCaret = (caret instanceof DefaultCaret) ? (DefaultCaret) caret
						: null;
				int dot = caret.getDot();
				...
				try {
					if (magicPosition == null && (direction == SwingConstants.NORTH
						...
						Rectangle r = (bidiCaret != null)
								? target.getUI().modelToView(target, dot, bidiCaret.getDotBias())
					...
					if (filter != null) {
						dot = filter.getNextVisualPositionFrom(target, dot, (bidiCaret != null)
								? bidiCaret.getDotBias() : Position.Bias.Forward, direction, bias);
					} else {
						dot = target.getUI()
								.getNextVisualPositionFrom(target, dot, (bidiCaret != null)
										? bidiCaret.getDotBias() : Position.Bias.Forward, direction,
										bias);
					}
					if (bias[0] == null) {
