 ...
 * adding special behavior for numbers. Among the specializations are (these are
 * only used if the <code>NumberFormatter</code> does not display invalid
 * numbers, for example, <code>setAllowsInvalid(false)</code>):
 * <ul>
 * <li>Pressing +/- (- is determined from the <code>DecimalFormatSymbols</code>
 * associated with the <code>DecimalFormat</code>) in any field but the exponent
 * field will attempt to change the sign of the number to positive/negative.
 * <li>Pressing +/- (- is determined from the <code>DecimalFormatSymbols</code>
 * associated with the <code>DecimalFormat</code>) in the exponent field will
 * attempt to change the sign of the exponent to positive/negative.
 * </ul>
 * <p>
 * If you are displaying scientific numbers, you may wish to turn on overwrite
 * mode, <code>setOverwriteMode(true)</code>. For example:
 * 
 * <pre>
 ...
 * </pre>
 * <p>
 * If you are going to allow the user to enter decimal values, you should either
 * force the DecimalFormat to contain at least one decimal (<code>#.0###</code>
 * ), or allow the value to be invalid <code>setAllowsInvalid(true)</code>.
 * Otherwise users may not be able to input decimal values.
 * <p>
 * <code>NumberFormatter</code> provides slightly different behavior to
 * <code>stringToValue</code> than that of its superclass. If you have specified
 * a Class for values, {@link #setValueClass}, that is one of of
 * <code>Integer</code>, <code>Long</code>, <code>Float</code>,
 * <code>Double</code>, <code>Byte</code> or <code>Short</code> and the Format's
 * <code>parseObject</code> returns an instance of <code>Number</code>, the
 * corresponding instance of the value class will be created using the
 * constructor appropriate for the primitive type the value class represents.
 * For example: <code>setValueClass(Integer.class)</code> will cause the
 * resulting value to be created via
 * <code>new Integer(((Number)formatter.parseObject(string)).intValue())</code>.
 * This is typically useful if you wish to set a min/max value as the various
 * <code>Number</code> implementations are generally not comparable to each
 * other. This is also useful if for some reason you need a specific
 * <code>Number</code> implementation for your values.
 * <p>
...
public class NumberFormatter extends InternationalFormatter {
     ...
     */
    void replace(DocumentFilter.FilterBypass fb, int offset, int length, String string, AttributeSet attr)
