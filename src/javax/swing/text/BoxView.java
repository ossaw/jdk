 ...
 * along an axis. The box is somewhat like that found in TeX where there is
 * alignment of the children, flexibility of the children is considered, etc.
 * This is a building block that might be useful to represent things like a
 * collection of lines, paragraphs, lists, columns, pages, etc. The axis along
 * which the children are tiled is considered the major axis. The orthogonal
 * axis is the minor axis.
 * <p>
 * Layout for each axis is handled separately by the methods
 * <code>layoutMajorAxis</code> and <code>layoutMinorAxis</code>. Subclasses can
 * change the layout algorithm by reimplementing these methods. These methods
 * will be called as necessary depending upon whether or not there is cached
 * layout information and the cache is considered valid. These methods are
 * typically called if the given size along the axis changes, or if
 * <code>layoutChanged</code> is called to force an updated layout. The
 * <code>layoutChanged</code> method invalidates cached layout information, if
 * there is any. The requirements published to the parent view are calculated by
 * the methods <code>calculateMajorAxisRequirements</code> and
 * <code>calculateMinorAxisRequirements</code>. If the layout algorithm is
 * changed, these methods will likely need to be reimplemented.
 *
...
public class BoxView extends CompositeView {
    ...
    /**
     * Paints a child. By default that is all it does, but a subclass can use
     * this to paint things relative to the child.
     *
     * @param g
    ...
    /**
     * Invalidates the layout and resizes the cache of requests/allocations. The
     * child allocations can still be accessed for the old layout, but the new
     * children will have an offset and span of 0.
     *
     * @param index
     ...
     * The current number of child views are used to produce the new array. The
     * contents of the old array are inserted into the new array at the
     * appropriate places so that the old layout information is transferred to
     * the new array.
     *
     * @param oldArray
     *                  the original layout array
     * @param offset
     *                  location where new views will be inserted
     * @param nInserted
     *                  the number of child views being inserted; therefore the
     *                  number
     *                  of blank spaces to leave in the new array at location
     *                  <code>offset</code>
    ...
    int[] updateLayoutArray(int[] oldArray, int offset, int nInserted) {
        ...
        System.arraycopy(oldArray, offset, newArray, offset + nInserted, n
                - nInserted - offset);
        return newArray;
    }
     ...
     * need to be notified of the change to the model. If a child changed its
     * requirements and the allocation was valid prior to forwarding the portion
     * of the box from the starting child to the end of the box will be
     * repainted.
     *
     * @param ec
     ...
     * This is implemented to throw away cached layout information so that new
     * calculations will be done the next time the children need an allocation.
     *
     * @param child
     *               the child view
     * @param width
    ...
    /**
     * Sets the size of the view along an axis. This should cause layout of the
     * view along the given axis.
     *
     * @param axis
     *             may be either <code>View.X_AXIS</code> or
     *             <code>View.Y_AXIS</code>
    ...
    /**
     * Returns the size of the view along an axis. This is implemented to return
     * zero.
     *
     * @param axis
     *             may be either <code>View.X_AXIS</code> or
     ...
     * view caches any layout information. This is implemented to call the
    ...
    /**
     * Renders the <code>BoxView</code> using the given rendering surface and
     * area on that surface. Only the children that intersect the clip bounds of
     * the given <code>Graphics</code> will be rendered.
     *
     * @param g
     ...
     * where various views are located. This is implemented to return
     * <code>null</code> if the layout is invalid, otherwise the superclass
    ...
    public Shape getChildAllocation(int index, Shape a) {
        if (a != null) {
            ...
            if ((ca != null) && (!isAllocationValid())) {
                ...
                Rectangle r = (ca instanceof Rectangle) ? (Rectangle) ca
                        : ca.getBounds();
     ...
     * coordinate space of the view mapped to it. This makes sure the allocation
     * is valid before calling the superclass.
     *
     * @param pos
     ...
     * implemented to give the total alignment needed to position the children
     * with the alignment points lined up along the axis orthogonal to the axis
     * that is being tiled. The axis being tiled will request to be centered
     * (i.e. 0.5f).
     *
     * @param axis
     *             may be either <code>View.X_AXIS</code> or
     ...
     *         from the origin; an alignment of 0.5 would be the center of the
     *         view
     * @exception IllegalArgumentException
     *                                     for an invalid axis
     */
     ...
     *                   the Y coordinate &gt;= 0
     * @param innerAlloc
     *                   the allocated region; this is the area inside of the
     *                   insets
     ...
     *              the Y coordinate &gt;= 0
     * @param alloc
     *              the parents inner allocation on entry, which should be
     *              changed
     ...
     *
     * @param index
     *              the index of the child view to allocate, &gt;= 0 &amp;&amp;
     *              &lt; getViewCount()
     * @param alloc
    ...
    /**
     * Returns the current height of the box. This is the height that it was
     * last allocated.
     * 
     ...
     * represents). The results of the layout (the offset and span for each
     * children) are placed in the given arrays which represent the allocations
    ...
    /**
     * Performs layout for the minor axis of the box (i.e. the axis orthogonal
     * to the axis that it represents). The results of the layout (the offset
     * and span for each children) are placed in the given arrays which
     ...
     * @param r
     *             the <code>SizeRequirements</code> object; if
     *             <code>null</code>
     *             one will be created
     ...
     *             the axis being studied
     * @exception IllegalArgumentException
     *                                     if <code>axis</code> is neither
     *                                     <code>View.X_AXIS</code>
     *                                     nor <code>View.Y_AXIS</code>
     */
     ...
     * <code>BoxView</code> given the <code>targetSpan</code>, which is the
     * width (or height) of the region we have to work with.
     *
     * @param targetSpan
     *                   the total span given to the view, which would be used
     *                   to
     *                   layout the children
     * @param axis
     *                   the axis being studied, either <code>View.X_AXIS</code>
     *                   or
     *                   <code>View.Y_AXIS</code>
     * @param offsets
     *                   an empty array filled by this method with values
     *                   specifying
     *                   the location of each child view
     * @param spans
     *                   an empty array filled by this method with values
     *                   specifying
     *                   the extent of each child view
     */
    protected void baselineLayout(int targetSpan, int axis, int[] offsets,
            int[] spans) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (v.getResizeWeight(axis) > 0) {
                ...
                if (align == 0.0f) {
                    // if the alignment is 0 then we need to fit into the
                    // descent
                    viewSpan = Math.max(Math.min(maxSpan, totalDescent),
                            minSpan);
                } else if (align == 1.0f) {
                    // if the alignment is 1 then we need to fit into the ascent
                    viewSpan = Math.max(Math.min(maxSpan, totalAscent),
                            minSpan);
                } else {
                    // figure out the span that we must fit into
                    float fitSpan = Math.min(totalAscent / align, totalDescent
                            / (1.0f - align));
    ...
    /**
     * Calculates the size requirements for this <code>BoxView</code> by
     * examining the size of each child view.
     *
     * @param axis
     *             the axis being studied
     * @param r
    ...
    protected SizeRequirements baselineRequirements(int axis,
        ...
        // loop through all children calculating the max of all their ascents
        // and
        ...
        // we now have an overall preferred, minimum, and maximum ascent and
        // descent

        // calculate the preferred span as the sum of the preferred ascent and
        // preferred descent
        r.preferred = (int) Math.min((long) totalAscent.preferred
                + (long) totalDescent.preferred, Integer.MAX_VALUE);

        // calculate the preferred alignment as the preferred ascent divided by
        // the preferred span
        if (r.preferred > 0) {
        ...
        if (r.alignment == 0.0f) {
            // if the preferred alignment is 0 then the minimum and maximum
            // spans are simply
            // the minimum and maximum descents since there's nothing above the
            // baseline
            r.minimum = totalDescent.minimum;
            r.maximum = totalDescent.maximum;
        } else if (r.alignment == 1.0f) {
            // if the preferred alignment is 1 then the minimum and maximum
            // spans are simply
            // the minimum and maximum ascents since there's nothing below the
            // baseline
            r.minimum = totalAscent.minimum;
            r.maximum = totalAscent.maximum;
        } else {
            // we want to honor the preferred alignment so we calculate two
            // possible minimum
            // span values using 1) the minimum ascent and the alignment, and 2)
            // the minimum
            // descent and the alignment. We'll choose the larger of these two
            // numbers.
            r.minimum = Math.round(Math.max(totalAscent.minimum / r.alignment,
                    totalDescent.minimum / (1.0f - r.alignment)));
            // a similar calculation is made for the maximum but we choose the
            // smaller number.
    ...
    /**
     * Determines in which direction the next view lays. Consider the View at
     * index n. Typically the <code>View</code>s are layed out from left to
     * right, so that the <code>View</code> to the EAST will be at index n + 1,
     * and the <code>View</code> to the WEST will be at index n - 1. In certain
     * situations, such as with bidirectional text, it is possible that the
     * <code>View</code> to EAST is not at index n + 1, but rather at index n -
     * 1, or that the <code>View</code> to the WEST is not at index n - 1, but
     * index n + 1. In this case this method would return true, indicating the
     * <code>View</code>s are layed out in descending order. Otherwise the
     * method would return false indicating the <code>View</code>s are layed out
     * in ascending order.
     * <p>
     * If the receiver is laying its <code>View</code>s along the
     * <code>Y_AXIS</code>, this will will return the value from invoking the
     * same method on the <code>View</code> responsible for rendering
     * <code>position</code> and <code>bias</code>. Otherwise this will return
     * false.
     *
     * @param position
     ...
     * @return true if the <code>View</code>s surrounding the <code>View</code>
     *         responding for rendering <code>position</code> and
     *         <code>bias</code> are layed out in descending order; otherwise
     *         false
     */
    protected boolean flipEastAndWestAtEnds(int position, Position.Bias bias) {
        if (majorAxis == Y_AXIS) {
            int testPos = (bias == Position.Bias.Backward) ? Math.max(0,
                    position - 1) : position;
