 ...
 * implementation provides a default implementation which treats text as styled
 * text and provides a minimal set of actions for editing styled text.
 *
...
public class StyledEditorKit extends DefaultEditorKit {
	...
	/**
	 * Gets the input attributes for the pane. When the caret moves and there is
	 * no selection, the input attributes are automatically mutated to reflect
	 * the character attributes of the current caret location. The styled
	 * editing actions use the input attributes to carry out their actions.
	 *
	...
	/**
	 * Fetches the element representing the current run of character attributes
	 * for the caret.
	 *
	 * @return the element
	 */
	 ...
	 * are produced by this kit. This is implemented to return View
	 * implementations for the following kinds of elements:
	 * <ul>
	...
	/**
	 * This listener will be attached to the caret of the text component that
	 * the EditorKit gets installed into. This should keep the input attributes
	 * updated for use by the styled actions.
	 */
	private AttributeTracker inputAttributeUpdater;

	/**
	 * Tracks caret movement and keeps the input attributes set to reflect the
	 * current set of attribute definitions at the caret position.
	 * <p>
	 * This implements PropertyChangeListener to update the input attributes
	...
	class AttributeTracker implements CaretListener, PropertyChangeListener,
		 ...
		 * give the positions of the selection in <code>c</code>.
		 */
		void updateInputAttributes(int dot, int mark, JTextComponent c) {
			...
			if (run != currentRun) {
				/*
				 * PENDING(prinz) All attributes that represent a single glyph
				 * position and can't be inserted into should be removed from
				 * the input attributes... this requires mixing in an interface
				 * to indicate that condition. When we can add things again this
	 ...
	 * StyledEditorKit (or subclass) installed. This has some convenience
	 * methods for causing character or paragraph level attribute changes. The
	 * convenience methods will throw an IllegalArgumentException if the
	 * assumption of a StyledDocument, a JEditorPane, or a StyledEditorKit fail
	 * to be true.
	 * <p>
	...
	public abstract static class StyledTextAction extends TextAction {
		...
		/**
		 * Applies the given attributes to paragraphs. If there is a selection,
		 * the attributes are applied to the paragraphs that intersect the
		 * selection. if there is no selection, the attributes are applied to
	...
	/**
	 * An action to set the font family in the associated JEditorPane. This will
	 * use the family specified as the command string on the ActionEvent if
	 * there is one, otherwise the family that was initialized with will be
	 * used.
	 * <p>
	 * <strong>Warning:</strong> Serialized objects of this class will not be
	 * compatible with future Swing releases. The current serialization support
	...
	/**
	 * An action to set the font size in the associated JEditorPane. This will
	 * use the size specified as the command string on the ActionEvent if there
	 * is one, otherwise the size that was initialized with will be used.
	 * <p>
	 * <strong>Warning:</strong> Serialized objects of this class will not be
	 * compatible with future Swing releases. The current serialization support
	 * is appropriate for short term storage or RMI between applications running
	 ...
	 * <code>StyleConstants.Foreground</code> attribute for the currently
	 * selected range of the target JEditorPane. This is done by calling
	 * <code>StyledDocument.setCharacterAttributes</code> on the styled document
	 * associated with the target JEditorPane.
	 * <p>
	 * If the target text component is specified as the source of the
	 * ActionEvent and there is a command string, the command string will be
	 * interpreted as the foreground color. It will be interpreted by called
	 * <code>Color.decode</code>, and should therefore be legal input for that
	 * method.
	 * <p>
	 * <strong>Warning:</strong> Serialized objects of this class will not be
	 * compatible with future Swing releases. The current serialization support
	 ...
	 * <code>StyleConstants.Alignment</code> attribute for the currently
	 * selected range of the target JEditorPane. This is done by calling
	 * <code>StyledDocument.setParagraphAttributes</code> on the styled document
	 * associated with the target JEditorPane.
	 * <p>
	 * If the target text component is specified as the source of the
	 * ActionEvent and there is a command string, the command string will be
	 * interpreted as an integer that should be one of the legal values for the
	 ...
	 * <code>DefaultEditorKit.InsertBreakAction</code>. That is when its
	 * <code>actionPerformed</code> method is invoked, a newline is inserted.
	 * Beyond that, this will reset the input attributes to what they were
