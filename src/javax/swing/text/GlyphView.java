 ...
 * text glyphs using character level attributes in some way. An implementation
 * of the GlyphPainter class is used to do the actual rendering and model/view
 ...
 * element (i.e. they are nested classes and carry only a small amount of state
 * of their own) so they can share its resources.
 * <p>
 * Since this view represents text that may have tabs embedded in it, it
 * implements the <code>TabableView</code> interface. Tabs will only be expanded
 * if this view is embedded in a container that does tab expansion.
 * ParagraphView is an example of a container that does tab expansion.
 * <p>
 *
 * @since 1.3
 *
...
public class GlyphView extends View implements TabableView, Cloneable {
    ...
    /**
     * Fetch the currently installed glyph painter. If a painter has not yet
     * been installed, and a default was not yet needed, null is returned.
     */
    ...
    /**
     * Fetch a reference to the text that occupies the given range. This is
     * normally used by the GlyphPainter to determine what characters it should
     * render glyphs for.
     *
     * @param p0
     ...
     * <code>StyledDocument.getBackground</code> if the associated document is a
     * styled document, otherwise it returns null.
     */
     ...
     * <code>StyledDocument.getBackground</code> if the associated document is a
     * StyledDocument. If the associated document is not a StyledDocument, the
     * associated components foreground color is used. If there is no associated
     * component, null is returned.
     */
    ...
    /**
     * Fetch the font that the glyphs should be based upon. This is implemented
     * to call <code>StyledDocument.getFont</code> if the associated document is
     * a StyledDocument. If the associated document is not a StyledDocument, the
     * associated components font is used. If there is no associated component,
     * null is returned.
     */
    public Font getFont() {
        Document doc = getDocument();
    ...
    /**
     * Determine if the glyphs should be underlined. If true, an underline
     * should be drawn through the baseline.
     */
    ...
    /**
     * Check to see that a glyph painter exists. If a painter doesn't exist, a
     * default glyph painter will be installed.
     */
            ...
            if (defaultPainter == null) {
                ...
                try {
                    ...
                    throw new StateInvariantError(
                            "GlyphView: Can't load glyph painter: "
                                    + classname);
                }
            }
    ...
    /**
     * Determines the desired span when using the given tab expansion
     * implementation.
     *
     * @param x
     *          the position the view would be located at for the purpose of
     *          tab expansion &gt;= 0.
     * @param e
    ...
    /**
     * Determines the span along the same axis as tab expansion for a portion of
     * the view. This is intended for use by the TabExpander for cases where the
     * tab expansion involves aligning the portion of text that doesn't have
     * whitespace relative to the tab stop. There is therefore an assumption
     * that the range given does not contain tabs.
     * <p>
    ...
    public int getEndOffset() {
        Element e = getElement();
        return (length > 0) ? e.getStartOffset() + offset + length
                : e.getEndOffset();
    }
    ...
    public void paint(Graphics g, Shape a) {
        ...
        if (c != null && !c.isEnabled()) {
            fg = (c instanceof JTextComponent ? ((JTextComponent) c)
            ...
            // selected text color is different from regular foreground
                ...
                if (h.length != 0) {
                    ...
                    for (int i = 0; i < h.length; i++) {
                        ...
                        if (!SwingUtilities2.useSelectedTextColor(highlight,
                                tc)) {
                            continue;
                        }
                    ...
                    if (!paintedText && viewSelectionCount > 0) {
                        ...
                        while (curPos++ < viewLen) {
                            ...
                            if (startPos != curPos) {
                                ...
                                paintTextUsingColor(g, a, fg, p0 + startPos, p0
                                        + curPos);
                            }
                            int checkSum = 0;
     ...
     */
    final void paintTextUsingColor(Graphics g, Shape a, Color c, int p0,
            int p1) {
        // render the glyphs
        g.setColor(c);
        ...
        if (underline || strike) {
            ...
            int x1 = x0 + (int) painter.getSpan(this, p0, p1, getTabExpander(),
                    x0);

            // calculate y coordinate
            int y = alloc.y + (int) (painter.getHeight(this) - painter
                    .getDescent(this));
            if (underline) {
                int yTmp = y + 1;
     ...
     *
     * <p>
     * This implementation returns the longest non-breakable area within the
     * view as a minimum span for {@code View.X_AXIS}.
     * </p>
     *
     * @param axis
     *             may be either {@code View.X_AXIS} or {@code View.Y_AXIS}
            ...
            case View.X_AXIS:
                if (minimumSpan < 0) {
                    ...
                    while (p1 > p0) {
                        ...
                        minimumSpan = Math.max(minimumSpan, getPartialSpan(
                                breakSpot, p1));
    ...
    /**
     * Determines the desired alignment for this view along an axis. For the
     * label, the alignment is along the font baseline for the y axis, and the
     * superclasses alignment along the x axis.
     *
     * @param axis
     *             may be either View.X_AXIS or View.Y_AXIS
     ...
     * be broken.
     * <p>
     * This is implemented to forward to the superclass for the Y_AXIS. Along
     * the X_AXIS the following values may be returned.
     * <dl>
     * <dt><b>View.ExcellentBreakWeight</b>
     * <dd>if there is whitespace proceeding the desired break location.
     * <dt><b>View.BadBreakWeight</b>
     * <dd>if the desired break location results in a break location of the
     * starting offset.
     * <dt><b>View.GoodBreakWeight</b>
     * <dd>if the other conditions don't occur.
     * </dl>
     * This will normally result in the behavior of breaking on a whitespace
     * location if one can be found, otherwise breaking between characters.
     *
     * @param axis
     *             may be either View.X_AXIS or View.Y_AXIS
     * @param pos
    ...
    /**
     * Breaks this view on the given axis at the given length. This is
     * implemented to attempt to break on a whitespace location, and returns a
     * fragment with the whitespace at the end. If a whitespace location can't
     * be found, the nearest character is used.
     *
     * @param axis
     *             may be either View.X_AXIS or View.Y_AXIS
     * @param p0
     *             the location in the model where the fragment should start
     *             it's
     *             representation &gt;= 0.
     * @param pos
     *             the position along the axis that the broken view would occupy
     *             &gt;= 0. This may be useful for things like tab calculations.
     * @param len
     *             specifies the distance along the axis where a potential break
     *             is desired &gt;= 0.
     ...
     * <p>
     * This view does support fragmenting. It is implemented to return a nested
     * class that shares state in this view representing only a portion of the
     * view.
     *
     * @param p0
    ...
    public int getNextVisualPositionFrom(int pos, Position.Bias b, Shape a,
        ...
        return painter.getNextVisualPositionFrom(this, pos, b, a, direction,
                biasRet);
    }

    /**
     * Gives notification that something was inserted into the document in a
     * location that this view is responsible for. This is implemented to call
    ...
    static class JustificationInfo {
        ...
        JustificationInfo(int start, int end, int leadingSpaces,
                int contentSpaces, int trailingSpaces, boolean hasTab,
                BitSet spaceMap) {
            this.start = start;
            this.end = end;
    ...
    JustificationInfo getJustificationInfo(int rowStartOffset) {
        ...
        justificationInfo = new JustificationInfo(startJustifiableContent,
                endJustifiableContent, leadingSpaces, contentSpaces,
     ...
     * rendering and model/view translation. At a minimum, the GlyphPainter
     * allows a View implementation to perform its duties independant of a
     * particular version of JVM and selection of capabilities (i.e. shaping for
     * i18n, etc).
     *
     * @since 1.3
     */
    public static abstract class GlyphPainter {
        ...
        /**
         * Determine the span the glyphs given a start location (for tab
         * expansion).
         */
        public abstract float getSpan(GlyphView v, int p0, int p1,
         ...
         */
        public abstract void paint(GlyphView v, Graphics g, Shape a, int p0,
                int p1);

        /**
         * Provides a mapping from the document model coordinate space to the
         * coordinate space of the view mapped to it. This is shared by the
         * broken views.
         *
         * @param v
         *             the <code>GlyphView</code> containing the destination
         *             coordinate space
         * @param pos
         *             the position to convert
         * @param bias
         ...
         * @see View#modelToView
         */
        public abstract Shape modelToView(GlyphView v, int pos,
                Position.Bias bias, Shape a) throws BadLocationException;
         ...
         *
         * @param v
         *                   the <code>GlyphView</code> to provide a mapping for
         * @param x
         *                   the X coordinate
         * @param y
         *                   the Y coordinate
         ...
         * @param biasReturn
         *                   either <code>Position.Bias.Forward</code> or
         *                   <code>Position.Bias.Backward</code> is returned as
         *                   the
         ...
         * that fits within the given span. This could be used to break the
         * given view. The result should be a location just shy of the given
         * advance. This differs from viewToModel which returns the closest
         ...
         * @param p0
         *            the location in the model where the fragment should start
         *            it's representation &gt;= 0.
         * @param x
         *            the graphic location along the axis that the broken view
         *            would occupy &gt;= 0. This may be useful for things like
         *            tab calculations.
         * @param len
         *            specifies the distance into the view where a potential
         *            break is desired &gt;= 0.
        ...
        /**
         * Create a painter to use for the given GlyphView. If the painter
         * carries state it can create another painter to represent a new
         * GlyphView that is being created. If the painter doesn't hold any
         * significant state, it can return itself. The default behavior is to
         * return itself.
         * 
         * @param v
         *           the <code>GlyphView</code> to provide a painter for
         * @param p0
         *           the starting document offset &gt;= 0
         * @param p1
         *           the ending document offset &gt;= p0
            ...
            switch (direction) {
                ...
                case View.SOUTH:
                    ...
                    if (container instanceof JTextComponent) {
                        ...
                        Point magicPoint;
                        magicPoint = (c != null) ? c.getMagicCaretPosition()
                                : null;
