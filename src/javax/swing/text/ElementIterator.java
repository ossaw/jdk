...
/**
 * <p>
 * ElementIterator, as the name suggests, iterates over the Element tree. The
...
public class ElementIterator implements Cloneable {
	...
	/**
	 * The StackItem class stores the element as well as a child index. If the
	 * index is -1, then the element represented on the stack is the element
	...
	private class StackItem implements Cloneable {
		...
		private StackItem(Element elem) {
			/**
			 * -1 index implies a self reference, as opposed to an index into
	...
	/**
	 * Creates a new ElementIterator. The root element is taken to get the
	 ...
	 *
	 * @return element on top of the stack or <code>null</code> if the root
	...
	/**
	 * Fetches the next Element. The strategy used to locate the next element is
	...
	public Element next() {
		...
		/*
		 * if current() has not been invoked and next is invoked, the very first
		...
		if (index + 1 < elem.getElementCount()) {
			...
			if (child.isLeaf()) {
				/*
				 * In this case we merely want to increment the child index of
				 * the item on top of the stack.
				 */
				item.incrementIndex();
			} else {
				/*
				 * In this case we need to push the child(branch) on the stack
				 * so that we can iterate over its children.
				 */
			...
			/*
			 * No more children for the item on top of the stack therefore pop
			 * the stack.
			 */
			elementStack.pop();
			if (!elementStack.isEmpty()) {
				/*
				 * Increment the child index for the item that is now on top of
				 * the stack.
				 */
				StackItem top = elementStack.peek();
				top.incrementIndex();
				/*
				 * We now want to return its next child, therefore call next()
				 * recursively.
				 */
				return next();
			}
	...
	/**
	 * Fetches the previous Element. If however the current element is the last
	...
	public Element previous() {
		...
		if (index > 0) {
			...
			/*
			 * this implies that current is the element's first child, therefore
			 * previous is the element itself.
			 */
			return elem;
		} else if (index == -1) {
			if (stackSize == 1) {
			...
			/*
			 * We need to return either the item below the top item or one of
			 * the former's children.
			 */
	...
	/**
	 * Returns the last child of <code>parent</code> that is a leaf. If the last
