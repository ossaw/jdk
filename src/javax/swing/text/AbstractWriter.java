...
/**
 * AbstractWriter is an abstract class that actually does the work of writing
 * out the element tree including the attributes. In terms of how much is
 * written out per line, the writer defaults to 100. But this value can be set
 * by subclasses.
 *
 * @author Sunita Mani
 */
...
public abstract class AbstractWriter {
	 ...
	 * EndOfLineStringProperty, it will be used for newlines. Otherwise the
	 * System property line.separator will be used. The line separator can also
	 * be set.
	 */
	private String lineSeparator;

	/**
	 * True indicates that when writing, the line can be split, false indicates
	 * that even if the line is > than max line length it should not be split.
	 */
	...
	/**
	 * This is used in <code>writeLineSeparator</code> instead of tempChars. If
	 * tempChars were used it would mean write couldn't invoke
	 * <code>writeLineSeparator</code> as it might have been passed tempChars.
	 */
	...
	/**
	 * Creates a new AbstractWriter. Initializes the ElementIterator with the
	...
	/**
	 * This method determines whether the current element is in the range
	 * specified. When no range is specified, the range is initialized to be the
	 * entire document. inRange() returns true if the range specified intersects
	...
	/**
	 * This abstract method needs to be implemented by subclasses. Its
	 * responsibility is to iterate over the elements and use the write()
	 * methods to generate output in the desired format.
	 */
	abstract protected void write() throws IOException, BadLocationException;

	/**
	 * Returns the text associated with the element. The assumption here is that
	 * the element is a leaf element. Throws a BadLocationException when
	 * encountered.
	 *
	 * @param elem
	...
	/**
	 * Enables subclasses to set the number of characters they want written per
	...
	/**
	 * Returns true if the current line should be considered empty. This is true
	 * when <code>getCurrentLineLength</code> == 0 || <code>indent</code> has
	 * been invoked on an empty line.
	 *
	 * @since 1.3
	 */
	...
	/**
	 * Sets whether or not lines can be wrapped. This can be toggled during the
	 * writing of lines. For example, outputting HTML might set this to false
	 * when outputting a quoted string.
	 *
	 * @since 1.3
	 */
	...
	/**
	 * Returns whether or not the lines can be wrapped. If this is false no
	 * lineSeparator's will be output.
	 *
	 * @since 1.3
	 */
	...
	/**
	 * Enables subclasses to specify how many spaces an indent maps to. When
	 * indentation takes place, the indent level is multiplied by this mapping.
	 * The default is 2.
	 *
	 * @param space
	...
	/**
	 * Sets the String used to represent newlines. This is initialized in the
	 * constructor from either the Document, or the System property
	 * line.separator.
	 *
	 * @since 1.3
	 */
	...
	/**
	 * Does indentation. The number of spaces written out is indent level times
	 * the space to map mapping. If the current line is empty, this will not
	...
	/**
	 * Writes out a character. This is implemented to invoke the
	...
	/**
	 * Writes out a string. This is implemented to invoke the <code>write</code>
	...
	/**
	 * Writes the line separator. This invokes <code>output</code> directly as
	 * well as setting the <code>lineLength</code> to 0.
	 *
	 * @since 1.3
	 */
	 ...
	 * returns false, this will call <code>output</code> with each sequence of
	 * <code>chars</code> that doesn't contain a NEWLINE, followed by a call to
	 * <code>writeLineSeparator</code>. On the other hand, if
	 * <code>getCanWrapLines()</code> returns true, this will split the string,
	 * as necessary, so <code>getLineLength</code> is honored. The only
	 * exception is if the current string contains no whitespace, and won't fit
	 * in which case the line length will exceed <code>getLineLength</code>.
	 *
	 * @since 1.3
	 */
			...
			while (lastIndex < endIndex) {
				...
				if (newlineIndex != -1 && (lineLength + (newlineIndex - lastIndex)) < maxLength) {
					...
					forceNewLine = true;
						...
						&& (lineLength + (endIndex - lastIndex)) < maxLength) {
	 ...
	 * make it to this method, which invokes <code>write</code> on the Writer.
	 * <p>
	 * This method also updates the line length based on <code>length</code>. If
	 * this is invoked to output a newline, the current line length will need to
	 * be reset as will no longer be valid. If it is up to the caller to do
	 * this. Use <code>writeLineSeparator</code> to write out a newline, which
	 * will property update the current line length.
	 *
	 * @since 1.3
	 */
