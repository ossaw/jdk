...
/**
 * View of plain text (text with only one font and color) that does
 * line-wrapping. This view expects that its associated element has child
 * elements that represent the lines it should be wrapping. It is implemented as
 * a vertical box that contains logical line views. The logical line views are
 * nested classes that render the logical line as multiple physical line if the
 * logical line is too wide to fit within the allocation. The line views draw
 * upon the outer class for its state to reduce their memory requirements.
 * <p>
 * The line views do all of their rendering through the <code>drawLine</code>
 * method which in turn does all of its rendering through the
 * <code>drawSelectedText</code> and <code>drawUnselectedText</code> methods.
 * This enables subclasses to easily specialize the rendering without concern
 * for the layout aspects.
 *
...
public class WrappedPlainView extends BoxView implements TabExpander {
	...
	/**
	 * Creates a new WrappedPlainView. Lines will be wrapped on character
	 * boundaries.
	 *
	 * @param elem
	...
	/**
	 * Creates a new WrappedPlainView. Lines can be wrapped on either character
	...
	private int drawText(Element elem, int p0, int p1, Graphics g, int x, int y)
		...
		if (Utilities.isComposedTextAttributeDefined(attr)) {
			...
			x = Utilities.drawComposedText(this, attr, g, x, y, p0 - elem.getStartOffset(),
	 ...
	 * <code>setParent</code> method. Subclasses can reimplement this to
	...
	/**
	 * Load the text buffer with the given range of text. This is used by the
	 * fragments broken off of this view as well as this view itself.
	 */
	 ...
	 * is implemented to stash the selection positions, selection colors, and
	...
	class WrappedLine extends View {
		...
		public Shape modelToView(int pos, Shape a, Position.Bias b) throws BadLocationException {
			...
			if (pos > p0) {
				...
				alloc.x += Utilities.getTabbedTextWidth(segment, metrics, alloc.x,
		...
		/**
		 * Returns line cache. If the cache was GC'ed, recreates it. If there's
		 ...
		 * (which is relative to the beginning of the view). This method assumes
		 * that cache exists.
		 */
