 ...
 * Java Platform. Since the JDK is backward compatible with JDK1.1 API, this
 * class will also function on Java 2. The JDK introduces improved API for
 * rendering text however, so the GlyphPainter2 is recommended for the DK.
 *
...
class GlyphPainter1 extends GlyphView.GlyphPainter {
	 ...
	 * @param biasReturn
	 *            always returns <code>Position.Bias.Forward</code> as the
	...
	public int getBoundedPosition(GlyphView v, int p0, float x, float len) {
		...
		int[] justificationData = getJustificationData(v);
		int index = Utilities.getTabbedTextOffset(v, s, metrics, (int) x, (int) (x + len), expander,
				p0, false, justificationData);
	...
	/**
	 * @return justificationData from the ParagraphRow this GlyphView is in or
	 *         {@code null} if no justification is needed
	 */
