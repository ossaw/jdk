 ...
 * by invoking the <code>toString</code> method. In order to convert the value
 * back to a String, your class must provide a constructor that takes a String
 * argument. If no single argument constructor that takes a String is found, the
 * returned value will be the String passed into <code>stringToValue</code>.
 * <p>
 * Instances of <code>DefaultFormatter</code> can not be used in multiple
 * instances of <code>JFormattedTextField</code>. To obtain a copy of an already
 * configured <code>DefaultFormatter</code>, use the <code>clone</code> method.
 * <p>
 ...
 * @since 1.4
 */
public class DefaultFormatter extends JFormattedTextField.AbstractFormatter implements Cloneable,
        Serializable {
    ...
    /**
     * Sets that class that is used to create new Objects. If the passed in
     * class does not have a single argument constructor that takes a String,
     ...
     * this is null, a String will be returned. If the constructor throws an
    ...
    /**
     * Returns the initial location to position the cursor at. This forwards the
     * call to <code>getNextNavigatableChar</code>.
     */
    ...
    /**
     * Subclasses should override this if they want cursor navigation to skip
     * certain characters. A return value of false indicates the character at
     * <code>offset</code> should be skipped when navigating throught the field.
     */
    ...
    /**
     * Returns true if the text in <code>text</code> can be inserted. This does
     * not mean the text will ultimately be inserted, it is used if text can
     * trivially reject certain characters.
     */
    ...
    /**
     * Returns the next editable character starting at offset incrementing the
     * offset by <code>direction</code>.
     */
     ...
     * <code>deleteLength</code> characters at <code>offset</code> and inserting
    ...
    /**
     * Pushes the value to the JFormattedTextField if the current value is valid
     * and invokes <code>setEditValid</code> based on the validity of the value.
     */
    void updateValue() {
        updateValue(null);
    }
    ...
    /**
     * Pushes the <code>value</code> to the editor if we are to commit on edits.
     * If <code>value</code> is null, the current value will be obtained from
     * the text component.
     */
     ...
     * <code>direction</code>. This uses <code>getNextNavigatableChar</code> as
     * well as constraining the location to the max position.
     */
    ...
    /**
     * Returns true if the edit described by <code>rh</code> will result in a
     * legal value.
     */
    ...
    /**
     * If the edit described by <code>rh</code> is legal, this will return true,
     * commit the edit (if necessary) and update the cursor position. This
     * forwards to <code>canReplace</code> and <code>isLegalInsertText</code> as
     * necessary to determine if the edit is in fact legal.
     * <p>
    ...
    boolean replace(ReplaceHolder rh) throws BadLocationException {
        ...
        if (getOverwriteMode() && rh.text != null && getFormattedTextField().getSelectedText() == null) {
            rh.length = Math.min(Math.max(rh.length, rh.text.length()), rh.fb.getDocument().getLength()
                    - rh.offset);
        }
        if ((rh.text != null && !isLegalInsertText(rh.text)) || !canReplace(rh) || (rh.length == 0
                && (rh.text == null || rh.text.length() == 0))) {
            valid = false;
        }
        if (valid) {
    ...
    /**
     * ReplaceHolder is used to track where insert/remove/replace is going to
     * happen.
     */
    static class ReplaceHolder {
        ...
        Object value;
        /**
         * Position the cursor should be adjusted from. If this is -1 the cursor
         * position will be adjusted based on the direction of the replace (-1:
         * offset, 1: offset + text.length()), otherwise the cursor position is
         * adusted from this position.
         */
        int cursorPosition;
