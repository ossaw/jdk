 ...
 * compresses them by sharing across uses and taking advantage of their
 * immutable nature. Since many styles are replicated, the potential for sharing
 * is significant, and copies can be extremely cheap. Larger sets reduce the
...
public class StyleContext implements Serializable,
	...
	/**
	 * Returns default AttributeContext shared by all documents that don't
	 * bother to define/supply their own context.
	 *
	 * @return the context
	 */
	...
	/**
	 * Adds a new style into the style hierarchy. Style attributes resolve from
	 ...
	 *               of
	 *               named styles in the document). The name may be null if the
	 *               style is unnamed, but the caller is responsible for
	 *               managing
	 *               the reference returned as an unnamed style can't be fetched
	 *               by
	 ...
	 * StyleContext with addChangeListener().
	 *
	...
	/**
	 * Gets the font from an attribute set. This is implemented to try and fetch
	 * a cached font for the given AttributeSet, and if that fails the font
	 * features are resolved and the font is fetched from the low-level font
	 * cache.
	 *
	 * @param attr
	 *             the attribute set
	...
	public Font getFont(AttributeSet attr) {
		...
		/**
		 * if either superscript or subscript is is set, we need to reduce the
		 * font size by 2.
		 */
		if (StyleConstants.isSuperscript(attr) || StyleConstants.isSubscript(
				attr)) {
			size -= 2;
		}
	 ...
	 * hue, etc. By default it simply returns the value specified by the
	 * StyleConstants.Foreground attribute.
	 *
	 * @param attr
	 ...
	 * specification. This might be used to specify things like brighter, more
	 * hue, etc. By default it simply returns the value specified by the
	 * StyleConstants.Background attribute.
	 *
	 * @param attr
	 *             the set of attributes
	 * @return the color
	...
	public Font getFont(String family, int style, int size) {
		...
		if (f == null) {
			...
			if (defaultStyle != null) {
				...
				Font defaultFont = (Font) defaultStyle.getAttribute(
						FONT_ATTRIBUTE_KEY);
				if (defaultFont != null && defaultFont.getFamily()
	 ...
	 * references. This would typically be called by the finalize method of the
	 * MutableAttributeSet implementation.
	 * <p>
	 * This method is thread safe, although most Swing methods are not. Please
	 * see <A HREF=
	 * "https://docs.oracle.com/javase/tutorial/uiswing/concurrency/index.html">
	 ...
	 * unique/immutable sets. Any sets above this limit will use hashtables and
	 * be a MutableAttributeSet.
	 *
	...
	/**
	 * Create a compact set of attributes that might be shared. This is a hook
	 ...
	 * If a matching set is found, return it. If a match is not found, we create
	 * a new set and add it to the pool.
	 */
	...
	/**
	 * Creates a mutable attribute set to hand out because the current needs are
	 * too big to try and use a shared version.
	 */
	 ...
	 * serialization. This will take special care to deal with static attribute
	 * keys that have been registered wit the
	 * <code>registerStaticAttributeKey</code> method. Any attribute key not
	 * registered as a static key will be serialized directly. All values are
			...
			if (key instanceof Serializable) {
				...
				if (ioFmt == null) {
					throw new NotSerializableException(key.getClass().getName()
							+ " is not serializable as a key in an AttributeSet");
				}
			...
			if (value instanceof Serializable) {
				...
				if (ioFmt == null) {
					throw new NotSerializableException(value.getClass()
							.getName()
							+ " is not serializable as a value in an AttributeSet");
				}
				out.writeObject(ioFmt);
			}
		}
	...
	/**
	 * Reads a set of attributes from the given object input stream that have
	 * been previously written out with <code>writeAttributeSet</code>. This
	 * will try to restore keys that were static objects to the static objects
	 * in the current virtual machine considering only those keys that have been
	 * registered with the <code>registerStaticAttributeKey</code> method. The
	 * attributes retrieved from the stream will be placed into the given
	 * mutable set.
	 *
	 * @param in
	 *           the object stream to read the attribute data from.
	 * @param a
	 *           the attribute set to place the attribute definitions in.
	...
	/**
	 * Registers an object as a static object that is being used as a key in
	 * attribute sets. This allows the key to be treated specially for
	 * serialization.
	 * <p>
	 * For operation under a 1.1 virtual machine, this uses the value returned
	 * by <code>toString</code> concatenated to the classname. The value
	 * returned by toString should not have the class reference in it (ie it
	 * should be reimplemented from the definition in Object) in order to be the
	 * same when recomputed later.
	 *
	 * @param key
	...
	private void readObject(ObjectInputStream s) throws ClassNotFoundException,
		...
		attributesPool = Collections.synchronizedMap(
				new WeakHashMap<SmallAttributeSet, WeakReference<SmallAttributeSet>>());
		s.defaultReadObject();
	}
	...
	/**
	 * Number of immutable sets that are not currently being used. This helps
	 * indicate when the sets need to be cleaned out of the hashtable they are
	 * stored in.
	 */
	private int unusedSets;

	/**
	 * The threshold for no longer sharing the set of attributes in an immutable
	 * table.
	 */
	static final int THRESHOLD = 9;

	/**
	 * This class holds a small number of attributes in an array. The storage
	 * format is key, value, key, value, etc. The size of the set is the length
	 * of the array divided by two. By default, this is the class that will be
	 * used to store attributes when held in the compact sharable form.
	 */
	public class SmallAttributeSet implements AttributeSet {
		...
		/**
		 * Clones a set of attributes. Since the set is immutable, a clone is
		...
		/**
		 * Checks whether the attribute set contains all of the given
		 * attributes.
		 *
		 * @param attrs
	...
	/**
	 * Sorts the key strings so that they can be very quickly compared in the
	...
	class KeyBuilder {
		...
		/**
		 * Initialize with a set of already sorted keys (data from an existing
		 * SmallAttributeSet).
		 */
		...
		/**
		 * Creates a table of sorted key/value entries suitable for creation of
		 * an instance of SmallAttributeSet.
		 */
		...
		/**
		 * The number of key/value pairs contained in the current key being
		 * forged.
		 */
		int getCount() {
			return keys.size();
	...
	/**
	 * A collection of attributes, typically used to represent character and
	 * paragraph styles. This is an implementation of MutableAttributeSet that
	 * can be observed if desired. These styles will take advantage of
	 * immutability while the sets are small enough, and may be substantially
	 * more efficient than something like SimpleAttributeSet.
	 * <p>
	 * <strong>Warning:</strong> Serialized objects of this class will not be
	...
	public class NamedStyle implements Style, Serializable {
		...
		/**
		 * Returns an array of all the <code>ChangeListener</code>s added to
		 * this NamedStyle with addChangeListener().
		 *
		...
		/**
		 * Gets attributes from the parent. If not overriden, the resolving
		 * parent defaults to the parent element.
		 *
		 * @return the attributes from the parent
		 * @see AttributeSet#getResolveParent
		 */
		...
		/**
		 * Only one ChangeEvent is needed per model instance since the event's
		...
		/**
		 * Inner AttributeSet implementation, which may be an immutable unique
		 * set being shared.
		 */
