...
public interface AttributeSet {
	 ...
	 * any attribute key that contributes to the determination of what font to
	 * use to render some text. This is not considered to be a closed set, the
	 * definition can change across version of the platform and can be amended
	...
	/**
	 * Checks whether the named attribute has a value specified in the set
	 ...
	 * locally, the search is continued upward through the resolving parent (if
	 * one exists) until the value is either found or there are no more parents.
	...
	/**
	 * Returns an enumeration over the names of the attributes that are defined
	 * locally in the set. Names of attributes defined in the resolving parent,
	 * if any, are not included. The values of the <code>Enumeration</code> may
	 * be anything and are not constrained to a particular <code>Object</code>
	 * type.
	 * <p>
	...
	/**
	 * Returns {@code true} if this set defines an attribute with the same name
	 * and an equal value. If such an attribute is not found locally, it is
	 ...
	 * given set with equal values. If an attribute is not found locally, it is
	 * searched through in the resolving parent hierarchy.
	 *
	 * @param attributes
