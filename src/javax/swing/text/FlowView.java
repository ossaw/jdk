...
/**
 * A View that tries to flow it's children into some partially constrained
 * space. This can be used to build things like paragraphs, pages, etc. The flow
 * is made up of the following pieces of functionality.
 * <ul>
 * <li>A logical set of child views, which as used as a layout pool from which a
 * physical view is formed.
 * <li>A strategy for translating the logical view to a physical (flowed) view.
 * <li>Constraints for the strategy to work against.
 * <li>A physical structure, that represents the flow. The children of this view
 * are where the pieces of of the logical views are placed to create the flow.
 * </ul>
 *
...
public abstract class FlowView extends BoxView {
	...
	/**
	 * Fetches the axis along which views should be flowed. By default, this
	 * will be the axis orthogonal to the axis along which the flow rows are
	 * tiled (the axis of the default flow rows themselves). This is typically
	 * used by the <code>FlowStrategy</code>.
	 */
	 ...
	 * This is called by the FlowStrategy while it is updating the flow. A flow
	 * can be shaped by providing different values for the row constraints. By
	 * default, the entire span inside of the insets along the flow axis is
	 * returned.
	 *
	 * @param index
	...
	/**
	 * Create a View that should be used to hold a a rows worth of children in a
	 * flow. This is called by the FlowStrategy when new children are added or
	 * removed (i.e. rows are added or removed) in the process of updating the
	 * flow.
	 */
	 ...
	 * children directly (as they are created in the process of formatting). If
	 * the layoutPool variable is null, an instance of LogicalView is created to
	...
	/**
	 * Lays out the children. If the span along the flow axis has changed,
	 * layout is marked as invalid which which will cause the superclass
	 * behavior to recalculate the layout along the box axis. The
	 * FlowStrategy.layout method will be called to rebuild the flow rows as
	 * appropriate. If the height of this view changes (determined by the
	 * preferred size along the box axis), a preferenceChanged is called.
	 * Following all of that, the normal box layout of the superclass is
	 * performed.
	 *
	 * @param width
	...
	protected void layout(int width, int height) {
		...
		if (!isLayoutValid(faxis)) {
			...
			if (oldFlowHeight != newFlowHeight) {
				...
				if (host != null) {
					// nb idk 12/12/2001 host should not be equal to null. We
	...
	/**
	 * Calculate requirements along the minor axis. This is implemented to
	...
	/**
	 * These are the views that represent the child elements of the element this
	 * view represents (The logical view to translate to a physical view). These
	 * are not directly children of this view. These are either placed into the
	 * rows directly or used for the purpose of breaking into smaller chunks, to
	 * form the physical view.
	 */
	protected View layoutPool;

	/**
	 * The behavior for keeping the flow updated. By default this is a singleton
	 * shared by all instances of FlowView (FlowStrategy is stateless).
	 * Subclasses can create an alternative strategy, which might keep state.
	 */
	protected FlowStrategy strategy;

	/**
	 * Strategy for maintaining the physical form of the flow. The default
	 * implementation is completely stateless, and recalculates the entire flow
	 * if the layout is invalid on the given FlowView. Alternative strategies
	 * can be implemented by subclassing, and might perform incremental repair
	 * to the layout or alternative breaking behavior.
	 * 
	 * @since 1.3
	 */
	public static class FlowStrategy {
		 ...
		 * exhausted, the adjustRow method will be called to perform adjustments
	...
	/**
	 * This class can be used to represent a logical view for a flow. It keeps
	 * the children updated to reflect the state of the model, gives the logical
	 * child views access to the view hierarchy, and calculates a preferred
	...
	static class LogicalView extends CompositeView {
		...
		/**
		 * Determines the minimum span for this view along an axis. The is
		 ...
		 * implemented to reparent the child to the logical view (the children
		 * may have been parented by a row in the flow if they fit without
		...
		protected void forwardUpdate(DocumentEvent.ElementChange ec, DocumentEvent e, Shape a,
			...
			// the changed place. Note: we update the views only when insertion
			// or
			// removal takes place.
		...
		/**
		 * Tests whether a point lies before the rectangle range. Implemented to
		...
		/**
		 * Tests whether a point lies after the rectangle range. Implemented to
		 * return false, as hit detection is not performed on the logical view.
		 *
		 * @param x
		 *            the X coordinate &gt;= 0
		...
		/**
		 * Fetches the child view at the given point. Implemented to return
		 ...
		 *            the Y coordinate &gt;= 0
		 * @param alloc
		 *            the parent's allocation on entry, which should be changed
		...
		/**
		 * Returns the allocation for a given child. Implemented to do nothing,
