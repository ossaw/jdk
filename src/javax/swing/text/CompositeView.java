...
public abstract class CompositeView extends View {
	 ...
	 * {@link #setParent} method. Subclasses can reimplement this to initialize
	 * their child views in a different manner. The default implementation
	 ...
	 * <code>Container</code> for example). If this view has children (the view
	 * is being moved from one place in the view hierarchy to another), the
	...
	/**
	 * Fetches the allocation for the given child view to render into. This
		...
		while (counter < viewCount) {
			...
			if ((v = getView(counter)) == v0 || v == v1) {
				...
				if (endView != null) {
					Shape endShape;
					if (endView == v1) {
						endShape = v1.modelToView(v1.getStartOffset(), Position.Bias.Forward, p1,
								b1, r1);
					} else {
						endShape = v0.modelToView(p0, b0, v0.getEndOffset(), Position.Bias.Backward,
								r0);
					}
	 ...
	 * access to some of the locations in the model. This is a convenience
	 * method for {@link #getNextNorthSouthVisualPositionFrom} and
	 * {@link #getNextEastWestVisualPositionFrom}. This method enables
	 ...
	 * model. This is implemented to call the
	 ...
	 * This is implemented to fetch the view in the case where there is a child
	...
	/**
	 * Translates the immutable allocation given to the view to a mutable
	 * allocation that represents the interior allocation (i.e. the bounds of
	 * the given allocation with the top, left, bottom, and right insets
	 * removed. It is expected that the returned value would be further mutated
	 * to represent an allocation to a child view. This is implemented to reuse
	 * an instance variable so it avoids creating excessive Rectangles.
	 * Typically the result of calling this method would be fed to the
	...
	/**
	 * Returns the next visual position for the cursor, in either the north or
	 * south direction.
	 *
	 * @param pos
	 *            the position to convert &gt;= 0
