 ...
 * <p>
 * This implementation is intended for operation under the JDK. It uses the
 * java.awt.font.TextLayout class to do i18n capable rendering.
 *
...
class GlyphPainter2 extends GlyphView.GlyphPainter {
	 ...
	 * This implementation assumes it has no tabs (i.e. TextLayout doesn't deal
	 * with tab expansion).
	 */
	...
	/**
	 * Fetch the ascent above the baseline for the glyphs corresponding to the
	...
	/**
	 * Fetch the descent below the baseline for the glyphs corresponding to the
	 * given range in the model.
	 */
	...
	/**
	 * Paint the glyphs for the given view. This is implemented to only render
	 * if the Graphics is of type Graphics2D which is required by TextLayout
	 * (and this should be the case if running on the JDK).
	 */
		...
		if (g instanceof Graphics2D) {
			...
			if (p0 > v.getStartOffset() || p1 < v.getEndOffset()) {
				try {
					...
					Shape s = v.modelToView(p0, Position.Bias.Forward, p1, Position.Bias.Backward,
							a);
	...
	/**
	 * Determines the model location that represents the maximum advance that
	 * fits within the given span. This could be used to break the given view.
	 * The result should be a location just shy of the given advance. This
	 * differs from viewToModel which returns the closest position which might
