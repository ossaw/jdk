...
/**
 * Takes a sequence of RTF tokens and text and appends the text described by the
 * RTF to a <code>StyledDocument</code> (the <em>target</em>). The RTF is lexed
 * from the character stream by the <code>RTFParser</code> which is this class's
 * superclass.
 * This class is an indirect subclass of OutputStream. It must be closed in
 * order to guarantee that all of the text has been sent to the text acceptor.
 *
 * @see RTFParser
...
class RTFReader extends RTFParser {
    ...
    /**
     * Miscellaneous information about the parser's state. This dictionary is
     * saved and restored when an RTF group begins or ends.
     */
    Dictionary<Object, Object> parserState; /* Current parser state */
    /**
     * This is the "dst" item from parserState. rtfDestination is the current
     * rtf destination. It is cached in an instance variable for speed.
     */
    ...
    /**
     * This is the RTF version number, extracted from the \rtf keyword. The
     * version information is currently not used.
     */
    int rtfversion;

    /**
     * <code>true</code> to indicate that if the next keyword is unknown, the
     * containing group should be ignored.
     */
    boolean ignoreGroupIfUnknownKeyword;

    /**
     * The parameter of the most recently parsed \\ucN keyword, used for
     * skipping alternative representations after a Unicode character.
     */
    ...
    /**
     * textKeywords maps RTF keywords to single-character strings, for those
     * keywords which simply insert some text.
     */
    ...
    static {
        ...
        /*
         * There is no Unicode equivalent to an optional hyphen, as far as I can
         * tell.
         */
    ...
    /**
     * Creates a new RTFReader instance. Text will be sent to the specified
     * TextAcceptor.
     *
     * @param destination
    ...
    /**
     * Called by the superclass when a new RTF group is begun. This
     * implementation saves the current <code>parserState</code>, and gives the
     * current destination a chance to save its own state.
     * 
    ...
    /**
     * Called by the superclass when the current RTF group is closed. This
     * restores the parserState saved by <code>begingroup()</code> as well as
     * invoking the endgroup method of the current destination.
     * 
    ...
    public void endgroup() {
        ...
        Dictionary<Object, Object> restoredState = (Dictionary<Object, Object>) parserState.get(
                "_savedState");
    ...
    protected void setRTFDestination(Destination newDestination) {
        /*
         * Check that setting the destination won't close the current
         * destination (should never happen)
         */
    ...
    /**
     * Called by the user when there is no more input (<i>i.e.</i>, at the end
     * of the RTF file.)
     *
    ...
    public boolean handleKeyword(String keyword) {
        ...
        if (keyword.equals("aftncn") || keyword.equals("aftnsep") || keyword.equals("aftnsepc") || keyword
                .equals("annotation") || keyword.equals("atnauthor") || keyword.equals("atnicn") || keyword
                        .equals("atnid") || keyword.equals("atnref") || keyword.equals("atntime") || keyword
                                .equals("atrfend") || keyword.equals("atrfstart") || keyword.equals("bkmkend")
                || keyword.equals("bkmkstart") || keyword.equals("datafield") || keyword.equals("do")
                || keyword.equals("dptxbxtext") || keyword.equals("falt") || keyword.equals("field")
                || keyword.equals("file") || keyword.equals("filetbl") || keyword.equals("fname") || keyword
                        .equals("fontemb") || keyword.equals("fontfile") || keyword.equals("footer")
                || keyword.equals("footerf") || keyword.equals("footerl") || keyword.equals("footerr")
                || keyword.equals("footnote") || keyword.equals("ftncn") || keyword.equals("ftnsep")
                || keyword.equals("ftnsepc") || keyword.equals("header") || keyword.equals("headerf")
                || keyword.equals("headerl") || keyword.equals("headerr") || keyword.equals("keycode")
                || keyword.equals("nextfile") || keyword.equals("object") || keyword.equals("pict") || keyword
                        .equals("pn") || keyword.equals("pnseclvl") || keyword.equals("pntxtb") || keyword
                                .equals("pntxta") || keyword.equals("revtbl") || keyword.equals("rxe")
                || keyword.equals("tc") || keyword.equals("template") || keyword.equals("txe") || keyword
                        .equals("xe")) {
    ...
    /**
     * Handles an RTF keyword and its integer parameter. This is called by the
     * superclass (RTFParser) when a keyword is found in the input stream.
     *
     * @returns <code>true</code> if the keyword is recognized and handled;
     *          <code>false</code> otherwise
     * @see RTFParser#handleKeyword
    ...
    private void setTargetAttribute(String name, Object value) {
        // target.changeAttributes(new
        // LFDictionary(LFArray.arrayWithObject(value),
        // LFArray.arrayWithObject(name)));
    }

    /**
     * setCharacterSet sets the current translation table to correspond with the
     * named character set. The character set is loaded if necessary.
     *
     * @see AbstractFilter
     */
    ...
    /**
     * Looks up a named character set. A character set is a 256-entry array of
     * characters, mapping unsigned byte values to their Unicode equivalents.
     * The character set is loaded if necessary.
     *
    ...
    public static Object getCharacterSet(final String name) throws IOException {
        ...
        if (set == null) {
            InputStream charsetStream = AccessController.doPrivileged(new PrivilegedAction<InputStream>() {
    ...
    /**
     * Parses a character set from an InputStream. The character set must
     * contain 256 decimal integers, separated by whitespace, with no
     * punctuation. B- and C- style comments are allowed.
     *
    ...
    /**
     * An interface (could be an entirely abstract class) describing a
     * destination. The RTF reader always has a current destination which is
     * where text is sent.
     *
     * @see RTFReader
     */
     ...
     * {\*\blegga blah blah blah} ) It accepts all keywords and text but does
     * nothing with them.
     */
    class DiscardingDestination implements Destination {
        ...
        public void begingroup() {
            /*
             * Ignore groups --- the RTFReader will keep track of the current
             * group level as necessary
             */
        }
    ...
    /**
     * Reads the fonttbl group, inserting fonts into the RTFReader's fontTable
     * dictionary.
     */
    ...
    /**
     * Reads the colortbl group. Upon end-of-group, the RTFReader's color table
     * is set to an array containing the read colors.
     */
    ...
    class StylesheetDestination extends DiscardingDestination implements Destination {
        ...
        public void close() {
            ...
            /*
             * (old debugging code) int i, m; if (characterStyles != null) { m =
             * characterStyles.length; for(i=0;i<m;i++)
             * warnings.println("chrStyle["+i+"]="+characterStyles[i]); } else
             * warnings.println("No character styles."); if (paragraphStyles !=
             * null) { m = paragraphStyles.length; for(i=0;i<m;i++)
             * warnings.println("pgfStyle["+i+"]="+paragraphStyles[i]); } else
             * warnings.println("No paragraph styles."); if (sectionStyles !=
             * null) { m = characterStyles.length; for(i=0;i<m;i++)
             * warnings.println("secStyle["+i+"]="+sectionStyles[i]); } else
        ...
        class StyleDefiningDestination extends AttributeTrackingDestination implements Destination {
            ...
            public Style realize() {
                ...
                /*
                 * NB: Swing StyleContext doesn't allow distinct styles with the
                 * same name; RTF apparently does. This may confuse the user.
                 */
     ...
     * simply tracks the attributes specified by the RTF control words in
     * internal form and can produce acceptable AttributeSets for the current
     * character, paragraph, and section attributes. It is up to the subclasses
     * to determine what is done with the actual text.
     */
    abstract class AttributeTrackingDestination implements Destination {
        /**
         * This is the "chr" element of parserState, cached for more efficient
         * use
         */
        MutableAttributeSet characterAttributes;
        /**
         * This is the "pgf" element of parserState, cached for more efficient
         * use
         */
        MutableAttributeSet paragraphAttributes;
        /**
         * This is the "sec" element of parserState, cached for more efficient
         * use
         */
        ...
        public void handleBinaryBlob(byte[] data) {
            /*
             * This should really be in TextHandlingDestination, but since
             * *nobody* does anything with binary blobs, this is more
             * convenient.
             */
        ...
        public void begingroup() {
            ...
            /*
             * It would probably be more efficient to use the resolver property
             * of the attributes set for implementing rtf groups, but that's
             * needed for styles.
             */
        ...
        /**
         * Returns a new MutableAttributeSet containing the default character
         * attributes
         */
        ...
        /**
         * Returns a new MutableAttributeSet containing the default paragraph
         * attributes
         */
        ...
        /**
         * Returns a new MutableAttributeSet containing the default section
         * attributes
         */
        ...
        MutableAttributeSet currentTextAttributes() {
            ...
            /*
             * TODO: catch exceptions for undefined attributes, bad font
             * indices, etc.? (as it stands, it is the caller's job to clean up
             * after corrupt RTF)
             */
        ...
        /**
         * Calculates the current paragraph attributes (with keys as given in
         * StyleConstants) from the current parser state.
         *
        ...
        /**
         * Calculates the current section attributes from the current parser
         * state.
         *
         * @returns a newly created MutableAttributeSet.
         */
         ...
         * attributes to their default values. Invoked to handle the \plain
         * keyword.
         */
         ...
         * attributes to their default values. Invoked to handle the \pard
         * keyword.
         */
         ...
         * attributes to their default values. Invoked to handle the \sectd
         * keyword.
         */
     ...
     * functionality. Subclasses must implement:
     * <dl>
     * <dt>deliverText()
    ...
    abstract class TextHandlingDestination extends AttributeTrackingDestination implements Destination {
        /**
         * <code>true</code> if the reader has not just finished a paragraph;
         * false upon startup
         */
        boolean inParagraph;
