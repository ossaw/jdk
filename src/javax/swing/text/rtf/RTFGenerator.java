...
/**
 * Generates an RTF output stream (java.io.OutputStream) from rich text (handed
 * off through a series of LTTextAcceptor calls). Can be used to generate RTF
 * from any object which knows how to write to a text acceptor (e.g.,
 * LTAttributedText and LTRTFFilter).
 * <p>
 * Note that this is a lossy conversion since RTF's model of text does not
 * exactly correspond with LightText's.
 *
...
class RTFGenerator extends Object {
    ...
    /*
     * An array of character-keyword pairs. This could be done as a dictionary
     * (and lookup would be quicker), but that would require allocating an
     * object for every character written (slow!).
     */
    ...
    static public void writeDocument(Document d, OutputStream to) throws IOException {
        ...
        /*
         * TODO this assumes a particular element structure; is there a way to
         * iterate more generically ?
         */
    ...
    public void writeRTFHeader() throws IOException {
        ...
        /*
         * TODO: Should the writer attempt to examine the text it's writing and
         * pick a character set which will most compactly represent the
         * document? (currently the writer always uses the ansi character set,
         * which is roughly ISO-8859 Latin-1, and uses Unicode escapes for all
         * other characters. However Unicode is a relatively recent addition to
         * RTF, and not all readers will understand it.)
         */
        writeBegingroup();
    ...
    protected void checkNumericControlWord(MutableAttributeSet currentAttributes, AttributeSet newAttributes,
            Object attrName, String controlWord, float dflt, float scale) throws IOException {
        Object parm;
    ...
    protected void checkControlWord(MutableAttributeSet currentAttributes, AttributeSet newAttributes,
    ...
    protected void checkControlWords(MutableAttributeSet currentAttributes, AttributeSet newAttributes,
            RTFAttribute words[], int domain) throws IOException {
        int wordIndex;
    ...
    public void writeTextElement(Element el) throws IOException {
        ...
        if (el.isLeaf()) {
            try {
                el.getDocument().getText(el.getStartOffset(), el.getEndOffset() - el.getStartOffset(),
                        this.workingSegment);
    ...
    public void writeCharacter(char ch) throws IOException {
        ...
        if (b == 0) {
            ...
            /*
             * In some cases it would be reasonable to check to see if the glyph
             * being written out is in the Symbol encoding, and if so, to switch
             * to the Symbol font for this character. TODO.
             */
            /*
             * Currently all unrepresentable characters are written as Unicode
             * escapes.
             */
    ...
    String approximationForUnicode(char ch) {
        /*
         * TODO: Find reasonable approximations for all Unicode characters in
         * all RTF code pages... heh, heh...
         */
        return "?";
    }

    /**
     * Takes a translation table (a 256-element array of characters) and creates
     * an output conversion table for use by convertCharacter().
     */
    /*
     * Not very efficient at all. Could be changed to sort the table for binary
     * search. TODO. (Even though this is inefficient however, writing RTF is
     * still much faster than reading it.)
     */
     ...
     * type) and returns the corresponding byte value (as an int, since bytes
     * are signed).
     */
