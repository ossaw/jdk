...
abstract class RTFParser extends AbstractFilter {
	...
	/**
	 * A stream to which to write warnings and debugging information while
	 * parsing. This is set to <code>System.out</code> to log any anomalous
	...
	/**
	 * Implemented by subclasses to interpret a parameter-less RTF keyword. The
	 * keyword is passed without the leading '/' or any delimiting whitespace.
	 */
	...
	public void write(char ch) throws IOException {
		...
		case S_parameter:
			if (Character.isDigit(ch)) {
				...
				if (pendingKeyword
						.equals("bin")) { /* magic layer-breaking kwd */
	...
	/**
	 * Flushes any buffered but not yet written characters. Subclasses which
	 * override this method should call this method <em>before</em> flushing any
	 * of their own buffers.
	 */
	...
	public void close() throws IOException {
		...
		if (state != S_text || level > 0) {
			...
			/*
			 * this will cause subclasses to behave more reasonably some of the
			 * time
			 */
			while (level > 0) {
				endgroup();
