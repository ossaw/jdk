...
public abstract class AbstractDocument implements Document, Serializable {
	...
	protected AbstractDocument(Content data, AttributeContext context) {
		...
		if (defaultI18NProperty == null) {
			...
			String o = java.security.AccessController
	 ...
	 *                the given remove position is not a valid position within
	 *                the document
	 * @see Document#remove
	 */
	...
	void updateBidi(DefaultDocumentEvent chng) {
		...
		if (firstSpanStart > 0) {
			...
			if (prevLevel == levels[0]) {
				...
				newElements.addElement(new BidiElement(bidiRoot, prevElem.getStartOffset(),
		...
		if (lastSpanEnd <= getLength()) {
			...
			if (nextLevel == levels[levels.length - 1]) {
				...
				newNextElem = new BidiElement(bidiRoot, lastPEnd, nextElem.getEndOffset(),
						nextLevel);
			} else {
				removeToIndex--;
			}
		...
		if ((firstSpanEnd == lastSpanStart) && (levels[0] == levels[levels.length - 1])) {
			newElements
					.addElement(new BidiElement(bidiRoot, firstSpanStart, lastSpanEnd, levels[0]));
		} else {
			...
			newElements.addElement(new BidiElement(bidiRoot, lastSpanStart + firstPStart,
					lastSpanEnd, levels[levels.length - 1]));
		}
	 ...
	 *                thrown on illegal lock attempt. If the document is
	 *                implemented properly, this can only happen if a document
	 *                listener attempts to mutate the document. This situation
	 *                violates the bean event model where order of delivery is
	 *                not guaranteed and all listeners should be notified before
