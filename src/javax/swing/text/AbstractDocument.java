 ...
 * <code>render</code> method. A write lock is acquired by the methods that
...
public abstract class AbstractDocument implements Document, Serializable {
	 ...
	 * <code>documentProperties</code> dictionary to annotate the document with
	...
	protected void fireUndoableEditUpdate(UndoableEditEvent e) {
		...
		for (int i = listeners.length - 2; i >= 0; i -= 2) {
			if (listeners[i] == UndoableEditListener.class) {
				...
				// if (e == null)
				// e = new ListSelectionEvent(this, firstIndex, lastIndex);
				((UndoableEditListener) listeners[i + 1]).undoableEditHappened(
						e);
			}
		}
	}
	 ...
	 * <code><em>Foo</em>Listener</code>s upon this document. <code><em>Foo</em>
	 ...
	 * you can query a document <code>d</code> for its document listeners with
	 * the following code:
	 *
	 * <pre>
	 * DocumentListener[] mls = (DocumentListener[]) (d.getListeners(
	...
	/**
	 * Gets the asynchronous loading priority. If less than zero, the document
	 * should not be loaded asynchronously.
	 *
	 * @return the asynchronous loading priority, or <code>-1</code> if the
	 *         document should not be loaded asynchronously
	 */
	public int getAsynchronousLoadPriority() {
		Integer loadPriority = (Integer) getProperty(
	 ...
	 * 
	 * @param p
	 *          the new asynchronous loading priority; a value less than zero
	 *          indicates that the document should not be loaded
	 *          asynchronously
	 */
	...
	/**
	 * Sets the <code>DocumentFilter</code>. The <code>DocumentFilter</code> is
	 * passed <code>insert</code> and <code>remove</code> to conditionally allow
	 * inserting/deleting of the text. A <code>null</code> value indicates that
	 * no filtering will occur.
	 *
	 * @param filter
	 *               the <code>DocumentFilter</code> used to constrain text
	...
	/**
	 * Returns the <code>DocumentFilter</code> that is responsible for filtering
	 * of insertion/removal. A <code>null</code> return value implies no
	 ...
	 * <p>
	 * This is implemented to acquire a read lock for the duration of the
	 * runnables execution. There may be multiple runnables executing at the
	 * same time, and all writers will be blocked while there are active
	 * rendering runnables. If the runnable throws an exception, its lock will
	 * be safely released. There is no protection against a runnable that never
	 * exits, which will effectively leave the document locked for it's
	 * lifetime.
	 * <p>
	 * If the given runnable attempts to make any mutations in this
	 * implementation, a deadlock will occur. There is no tracking of individual
	 * rendering threads to enable detecting this situation, but a subclass
	 * could incur the overhead of tracking them and throwing an error.
	 * <p>
	...
	/**
	 * Returns the length of the data. This is the number of characters of
	 * content that represents the users data.
	 *
	 * @return the length &gt;= 0
	 ...
	 *
	 * @param listener
	 *                 the <code>DocumentListener</code> to add
	 ...
	 *
	 * @param listener
	 *                 the <code>DocumentListener</code> to remove
	...
	/**
	 * Returns an array of all the document listeners registered on this
	 * document.
	 *
	 * @return all of this document's <code>DocumentListener</code>s or an empty
	 *         array if no document listeners are currently registered
	 *
	 ...
	 * appropriate DocumentEvent to be fired to keep the view(s) in sync with
	 * the model.
	 *
	 * @param listener
	 *                 the <code>UndoableEditListener</code> to add
	 ...
	 *
	 * @param listener
	 *                 the <code>UndoableEditListener</code> to remove
	 * @see Document#removeDocumentListener
	 */
	...
	/**
	 * Returns an array of all the undoable edit listeners registered on this
	 * document.
	 *
	 * @return all of this document's <code>UndoableEditListener</code>s or an
	 *         empty array if no undoable edit listeners are currently
	 *         registered
	 *
	...
	/**
	 * A convenience method for looking up a property value. It is equivalent
	 * to:
	 * 
	 * <pre>
	...
	public final void putProperty(Object key, Object value) {
		...
		if (key == TextAttribute.RUN_DIRECTION && Boolean.TRUE.equals(
				getProperty(I18NProperty))) {
			// REMIND - this needs to flip on the i18n property if run dir
			// is rtl and the i18n property is not already on.
			writeLock();
			try {
				DefaultDocumentEvent e = new DefaultDocumentEvent(0,
						getLength(), DocumentEvent.EventType.INSERT);
				updateBidi(e);
			} finally {
				writeUnlock();
			}
	...
	/**
	 * Removes some content from the document. Removing content causes a write
	 * lock to be held while the actual changes are taking place. Observers are
	 * notified of the change on the thread that called this method.
	 * <p>
	 * This method is thread safe, although most Swing methods are not. Please
	 * see <A HREF=
	 * "https://docs.oracle.com/javase/tutorial/uiswing/concurrency/index.html">
	...
	/**
	 * Performs the actual work of the remove. It is assumed the caller will
	 * have obtained a <code>writeLock</code> before invoking this.
	 */
	void handleRemove(int offs, int len) throws BadLocationException {
		if (len > 0) {
			if (offs < 0 || (offs + len) > getLength()) {
				throw new BadLocationException("Invalid remove", getLength()
						+ 1);
			}
			DefaultDocumentEvent chng = new DefaultDocumentEvent(offs, len,
	 ...
	 *               anything
	 * @param text
	 *               text to insert, <code>null</code> indicates no text to
	 *               insert
	 * @param attrs
	 *               AttributeSet indicating attributes of inserted text,
	 *               <code>null</code> is legal, and typically treated as an
	 *               empty
	 *               attributeset, but exact interpretation is left to the
	 *               subclass
	 * @exception BadLocationException
	 ...
	 * returned in the Segment will be the entire length requested and may or
	 * may not be a copy depending upon how the data was stored. If the
	 * partialReturn property is true, only the amount of text that can be
	 * returned without creating a copy is returned. Using partial returns will
	 * give better performance for situations where large parts of the document
	 * are being scanned. The following is an example of using the partial
	 * return to access the entire document:
	 *
	 * <pre>
	...
	/**
	 * Gets all root elements defined. Typically, there will only be one so the
	 * default implementation is to return the default root element.
	 *
	...
	/**
	 * Returns the <code>FilterBypass</code>. This will create one if one does
	 * not yet exist.
	 */
	 ...
	 * document. Its children represent character runs with a given Unicode bidi
	 * level.
	 */
	...
	/**
	 * Get the paragraph element containing the given position. Sub-classes must
	 * define for themselves what exactly constitutes a paragraph. They should
	 * keep in mind however that a paragraph should at least be the unit of text
	 * over which to run the Unicode bidirectional algorithm.
	 *
	 * @param pos
	 *            the starting offset &gt;= 0
	 * @return the element
	...
	/**
	 * Fetches the context for managing attributes. This method effectively
	 * establishes the strategy used for compressing AttributeSet information.
	 *
	 * @return the context
	 */
	 ...
	 * happen within a write lock. If a subclass of this class reimplements this
	 * method, it should delegate to the superclass as well.
	 *
	 * @param chng
	...
	protected void insertUpdate(DefaultDocumentEvent chng, AttributeSet attr) {
		...
		if (chng.type == DocumentEvent.EventType.INSERT && chng.getLength() > 0
	...
	/**
	 * Updates any document structure as a result of text removal. This method
	 * is called before the text is actually removed from the Content. This will
	 * happen within a write lock. If a subclass of this class reimplements this
	 * method, it should delegate to the superclass as well.
	 *
	 * @param chng
	...
	/**
	 * Updates any document structure as a result of text removal. This method
	 * is called after the text has been removed from the Content. This will
	 * happen within a write lock. If a subclass of this class reimplements this
	 * method, it should delegate to the superclass as well.
	 *
	 * @param chng
	...
	/**
	 * Update the bidi element structure as a result of the given change to the
	 * document. The given change will be updated to reflect the changes made to
	 * the bidi structure.
	 *
	 * This method assumes that every offset in the model is contained in
	 * exactly one paragraph. This method also assumes that it is called after
	 * the change is made to the default element structure.
	 */
	void updateBidi(DefaultDocumentEvent chng) {
		...
		// System.out.println("updateBidi: firstPStart = " + firstPStart + "
		// lastPEnd = " + lastPEnd );
		...
		if (firstSpanStart > 0) {
			...
			int prevLevel = StyleConstants.getBidiLevel(prevElem
					.getAttributes());
			// System.out.println("createbidiElements: prevElem= " + prevElem +
			// " prevLevel= " + prevLevel + "level[0] = " + levels[0]);
			if (prevLevel == levels[0]) {
				firstSpanStart = prevElem.getStartOffset();
			} else if (prevElem.getEndOffset() > firstPStart) {
				newElements.addElement(new BidiElement(bidiRoot, prevElem
						.getStartOffset(), firstPStart, prevLevel));
			} else {
				removeFromIndex++;
			}
		}
		...
		int firstSpanEnd = 0;
		while ((firstSpanEnd < levels.length)
				&& (levels[firstSpanEnd] == levels[0]))
			firstSpanEnd++;
		...
		if (lastSpanEnd <= getLength()) {
			...
			int nextLevel = StyleConstants.getBidiLevel(nextElem
					.getAttributes());
			if (nextLevel == levels[levels.length - 1]) {
				lastSpanEnd = nextElem.getEndOffset();
			} else if (nextElem.getStartOffset() < lastPEnd) {
				newNextElem = new BidiElement(bidiRoot, lastPEnd, nextElem
						.getEndOffset(), nextLevel);
			} else {
				removeToIndex--;
			}
		}
		...
		while ((lastSpanStart > firstSpanEnd) && (levels[lastSpanStart
				- 1] == levels[levels.length - 1]))
			lastSpanStart--;
		...
		if ((firstSpanEnd == lastSpanStart)
			...
			newElements.addElement(new BidiElement(bidiRoot, firstSpanStart,
					firstSpanEnd + firstPStart, levels[0]));
			...
			for (int i = firstSpanEnd; i < lastSpanStart;) {
				...
				int j;
				for (j = i; (j < levels.length)
						&& (levels[j] == levels[i]); j++)
					;
				newElements.addElement(new BidiElement(bidiRoot, firstPStart
						+ i, firstPStart + j, (int) levels[i]));
				i = j;
			}
			// Create an element for the last span.
			newElements.addElement(new BidiElement(bidiRoot, lastSpanStart
					+ firstPStart, lastSpanEnd, levels[levels.length - 1]));
		}
	...
	private byte[] calculateBidiLevels(int firstPStart, int lastPEnd) {
		...
		for (int o = firstPStart; o < lastPEnd;) {
			...
			// System.out.println("updateBidi: paragraph start = " + pStart + "
			// paragraph end = " + pEnd);
	...
	/**
	 * Creates a document leaf element. Hook through which elements are created
	 * to represent the document structure. Because this implementation keeps
	 * structure and content separate, elements grow automatically when content
	 * is extended so splits of existing elements follow. The document itself
	 * gets to decide how to generate elements to give flexibility in the type
	 * of elements used.
	 *
	 * @param parent
	 *               the parent element
	...
	/**
	 * Fetches the current writing thread if there is one. This can be used to
	 * distinguish whether a method is being called as part of an existing
	 * modification or if a lock needs to be acquired and a new transaction
	 * started.
	 *
	 * @return the thread actively modifying the document or <code>null</code>
	...
	/**
	 * Acquires a lock to begin mutating the document this lock protects. There
	 * can be no writing, notification of changes, or reading going on in order
	 * to gain the lock. Additionally a thread is allowed to gain more than one
	 * <code>writeLock</code>, as long as it doesn't attempt to gain additional
	 * <code>writeLock</code>s from within document notification. Attempting to
	 * gain a <code>writeLock</code> from within a DocumentListener notification
	 * will result in an <code>IllegalStateException</code>. The ability to
	 * obtain more than one <code>writeLock</code> per thread allows subclasses
	 * to gain a writeLock, perform a number of operations, then release the
	 * lock.
	 * <p>
	 * Calls to <code>writeLock</code> must be balanced with calls to
	 * <code>writeUnlock</code>, else the <code>Document</code> will be left in
	 * a locked state so that no reading or writing can be done.
	 *
	 * @exception IllegalStateException
	 *                                  thrown on illegal lock attempt. If the
	 *                                  document is
	 *                                  implemented properly, this can only
	 *                                  happen if a document
	 *                                  listener attempts to mutate the
	 *                                  document. This situation
	 *                                  violates the bean event model where
	 *                                  order of delivery is
	 *                                  not guaranteed and all listeners should
	 *                                  be notified before
	 *                                  further mutations are allowed.
	 */
	 ...
	 * After decrementing the lock count if there are no outstanding locks this
	 * will allow a new writer, or readers.
	 *
	 * @see #writeLock
	 */
	...
	/**
	 * Acquires a lock to begin reading some state from the document. There can
	 * be multiple readers at the same time. Writing blocks the readers until
	 * notification of the change to the listeners has been completed. This
	 * method should be used very carefully to avoid unintended compromise of
	 * the document. It should always be balanced with a <code>readUnlock</code>
	 * .
	 *
	 * @see #readUnlock
	 */
	 ...
	 * balanced with a readLock, and should occur in a finally statement so that
	 * the balance is guaranteed. The following is an example.
	 * 
	 * <pre>
	 * <code>
	 * &nbsp;   readLock();
	...
	/**
	 * Where the text is actually stored, and a set of marks that track change
	 * as the document is edited are managed.
	 */
	private Content data;

	/**
	 * Factory for the attributes. This is the strategy for attribute
	 * compression and control of the lifetime of a set of attributes as a
	 * collection. This may be shared with other documents.
	 */
	...
	/**
	 * Document property that indicates whether internationalization functions
	 * such as text reordering or reshaping should be performed. This property
	 * should not be publicly exposed, since it is used for implementation
	 * convenience only. As a side effect, copies of this property may be in its
	 * subclasses that live in different packages (e.g. HTMLDocument as of now),
	 * so those copies should also be taken care of when this property needs to
	 * be modified.
	 */
	static final String I18NProperty = "i18n";

	/**
	 * Document property that indicates if a character has been inserted into
	 * the document that is more than one byte long. GlyphView uses this to
	 * determine if it should use BreakIterator.
	 */
	static final Object MultiByteProperty = "multiByte";

	/**
	 * Document property that indicates asynchronous loading is desired, with
	 * the thread priority given as the value.
	 */
	 ...
	 * reflected by whether or not mutations return an UndoableEdit
	 * implementation.
	 * 
	...
	public interface Content {
		...
		/**
		 * Creates a position within the content that will track change as the
		 * content is mutated.
		 *
		 * @param offset
		 ...
		 * @param str
		 *              string to insert
		 * @return if the implementation supports a history mechanism, a
		 *         reference to an <code>Edit</code> implementation will be
		 *         returned, otherwise returns <code>null</code>
		 * @exception BadLocationException
		 *                                 thrown if the area covered by the
		 *                                 arguments is not
		 ...
		 * @return If the implementation supports a history mechanism, a
		 *         reference to an Edit implementation will be returned,
		 *         otherwise null.
		 * @exception BadLocationException
		 *                                 Thrown if the area covered by the
		 *                                 arguments is not
	 ...
	 * implementations to use pluggable attribute compression techniques. Each
	 * mutation of the attribute set can be used to exchange a previous
	 * AttributeSet instance with another, preserving the possibility of the
	 * AttributeSet remaining immutable. An implementation is provided by the
	 * StyleContext class.
	 *
	 * The Element implementations provided by this class use this interface to
	 * provide their MutableAttributeSet implementations, so that different
	 * AttributeSet compression techniques can be employed. The method
	 * <code>getAttributeContext</code> should be implemented to return the
	 * object responsible for implementing the desired compression technique.
	 *
	 * @see StyleContext
	 */
	public interface AttributeContext {
		...
		/**
		 * Adds an attribute to the given set, and returns the new
		 * representative set.
		 *
		 * @param old
		 ...
		 */
		public AttributeSet addAttribute(AttributeSet old, Object name,
				Object value);
		...
		/**
		 * Reclaims an attribute set. This is a way for a MutableAttributeSet to
		 * mark that it no longer need a particular immutable set. This is only
		 * necessary in 1.1 where there are no weak references. A 1.1
		 * implementation would call this in its finalize method.
		 *
		 * @param a
	...
	/**
	 * Implements the abstract part of an element. By default elements support
	 * attributes by having a field that represents the immutable part of the
	 * current attribute set for the element. The element itself implements
	 * MutableAttributeSet which can be used to modify the set by fetching a new
	 * immutable set. The immutable sets are provided by the AttributeContext
	...
	public abstract class AbstractElement implements Element,
			MutableAttributeSet, Serializable, TreeNode {
		...
		public Object getAttribute(Object attrName) {
			...
			if (value == null) {
				...
				// element.
				AttributeSet a = (parent != null) ? parent.getAttributes()
						: null;
				if (a != null) {
		...
		/**
		 * Gets the resolving parent. If not overridden, the resolving parent
		...
		public void setResolveParent(AttributeSet parent) {
			...
			if (parent != null) {
				attributes = context.addAttribute(attributes,
						StyleConstants.ResolveAttribute, parent);
			} else {
		...
		/**
		 * Returns the number of children <code>TreeNode</code>'s receiver
		 * contains.
		 * 
		 * @return the number of children <code>TreeNodews</code>'s receiver
		 *         contains
		 */
		 ...
		 * @return the index of <code>node</code> in the receiver's children, or
		 *         -1 if absent
		 */
	...
	public class BranchElement extends AbstractElement {
		...
		public void replace(int offset, int length, Element[] elems) {
			...
			if ((nchildren + delta) >= children.length) {
				...
				int newLength = Math.max(2 * children.length, nchildren
						+ delta);
		...
		public int getEndOffset() {
			Element child = (nchildren > 0) ? children[nchildren - 1]
					: children[0];
		...
		public int getElementIndex(int offset) {
			...
			// didn't find it, but we indicate the index of where it would
			// belong
			if (offset < p0) {
				index = mid;
			} else {
		...
		public Enumeration children() {
			...
			Vector<AbstractElement> tempVector = new Vector<AbstractElement>(
					nchildren);
	...
	public class LeafElement extends AbstractElement {
		 ...
		 * @since 1.4
		 */
		public LeafElement(Element parent, AttributeSet a, int offs0,
				int offs1) {
			super(parent, a);
			try {
				...
				p1 = null;
				throw new StateInvariantError(
						"Can't create Position references");
			}
		}
	...
	/**
	 * Represents the root element of the bidirectional element structure. The
	 * root element is the only element in the bidi element structure which
	 * contains children.
	 */
	...
	/**
	 * Stores document changes as the document is being modified. Can
	 * subsequently be used for change notification when done with the document
	 * modification transaction. This is used by the AbstractDocument class and
	 * its extensions for broadcasting change information to the document
	 * listeners.
	 */
	public class DefaultDocumentEvent extends CompoundEdit implements
			DocumentEvent {
		 ...
		 * @since 1.4
		 */
		public DefaultDocumentEvent(int offs, int len,
		...
		/**
		 * Adds a document edit. If the number of edits crosses a threshold,
		 * this switches on a hashtable lookup for ElementChange implementations
		 * since access of these needs to be relatively quick.
		 *
		 * @param anEdit
		...
		public boolean addEdit(UndoableEdit anEdit) {
			...
			if ((changeLookup != null)
					&& (anEdit instanceof DocumentEvent.ElementChange)) {
		...
		public void redo() throws CannotRedoException {
			...
			try {
				...
				UndoRedoDocumentEvent ev = new UndoRedoDocumentEvent(this,
						false);
		...
		/**
		 * Provides a localized, human readable description of the undoable form
		 * of this edit, e.g. for use as an Undo menu item. Typically derived
		 * from getDescription();
		 *
		 * @return the description
		 */
		...
		/**
		 * Provides a localized, human readable description of the redoable form
		 * of this edit, e.g. for use as a Redo menu item. Typically derived
		 * from getPresentationName();
		 *
		 * @return the description
		 */
	 ...
	 * It just wraps DefaultDocumentEvent and delegates all calls to it except
	 * getType() which depends on operation (Undo or Redo).
	 */
	...
	public static class ElementEdit extends AbstractUndoableEdit implements
		 ...
		 */
		public ElementEdit(Element e, int index, Element[] removed,
				Element[] added) {
			super();
			this.e = e;
			this.index = index;
		...
		public void redo() throws CannotRedoException {
			...
			// PENDING(prinz) need MutableElement interface, canRedo() should
			// check
		...
		public void undo() throws CannotUndoException {
			super.undo();
			// PENDING(prinz) need MutableElement interface, canUndo() should
			// check
			((AbstractDocument.BranchElement) e).replace(index, added.length,
					removed);
