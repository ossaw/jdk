 ...
 * of a <code>MaskFormatter</code> is controlled by way of a String mask that
 * specifies the valid characters that can be contained at a particular location
 * in the <code>Document</code> model. The following characters can be
 * specified:
 ...
 * <tr>
 * <td>U</td>
 * <td>Any character (<code>Character.isLetter</code>). All lowercase letters
 * are mapped to upper case.</td>
 * </tr>
 * <tr>
 * <td>L</td>
 * <td>Any character (<code>Character.isLetter</code>). All upper case letters
 * are mapped to lower case.</td>
 * </tr>
 * <tr>
 * <td>A</td>
 ...
 * methods. <code>setInvalidCharacters</code> allows you to specify which
 * characters are not legal. <code>setValidCharacters</code> allows you to
 * specify which characters are valid. For example, the following code block is
 * equivalent to a mask of '0xHHH' with no invalid/valid characters:
 * 
 * <pre>
 ...
 * </pre>
 * <p>
 * When initially formatting a value if the length of the string is less than
 * the length of the mask, two things can happen. Either the placeholder string
 * will be used, or the placeholder character will be used. Precedence is given
 * to the placeholder string. For example:
 * 
 * <pre>
 ...
 * <code>setPlaceholder("555-1212")</code> was invoked '123-1212' would result.
 * The placeholder String is only used on the initial format, on subsequent
 * formats only the placeholder character will be used.
 * <p>
 ...
 * necessary when editing. Consider a <code>MaskFormatter</code> with the mask
 * "###-####" and current value "555-1212". Using the right arrow key to
 * navigate through the field will result in (| indicates the position of the
 * caret):
 * 
 * <pre>
 *   |555-1212
 ...
 * '123-45' and '12345' into the <code>MaskFormatter</code> in the previous
 * example. Both inserts will result in the same String, '123-45__'. When
 * <code>MaskFormatter</code> is processing the insert at character position 3
 ...
 * change this with the <code>setAllowsInvalid</code> method, and will commit
 * edits on valid edits (use the <code>setCommitsOnValidEdit</code> to change
 * this).
 * <p>
 * By default, <code>MaskFormatter</code> is in overwrite mode. That is as
 * characters are typed a new character is not inserted, rather the character at
 * the current location is replaced with the newly typed character. You can
 * change this behavior by way of the method <code>setOverwriteMode</code>.
 * <p>
