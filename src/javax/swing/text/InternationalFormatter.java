 ...
 * using an instance of <code>java.text.Format</code> to handle the conversion
 * to a String, and the conversion from a String.
 * <p>
 ...
 * <code>setMinimum</code> and <code>setMaximum</code> methods. In order for
 * this to work the values returned from <code>stringToValue</code> must be
 ...
 * <code>InternationalFormatter</code>, as it is possible to create a situation
 * where certain values can not be input. Consider the date format 'M/d/yy', an
 * <code>InternationalFormatter</code> that is always valid (
 * <code>setAllowsInvalid(false)</code>), is in overwrite mode (
 * <code>setOverwriteMode(true)</code>) and the date 7/1/99. In this case the
 * user will not be able to enter a two digit month or day of month. To avoid
 * this, the format should be 'MM/dd/yy'.
 * <p>
 ...
 * from the <code>Format</code>. The cursor position will also be adjusted as
 * literal characters are added/removed from the resulting String.
 * <p>
 * <code>InternationalFormatter</code>'s behavior of <code>stringToValue</code>
 * is slightly different than that of <code>DefaultTextFormatter</code>, it does
 * the following:
 * <ol>
 * <li><code>parseObject</code> is invoked on the <code>Format</code> specified
 * by <code>setFormat</code>
 * <li>If a Class has been set for the values (<code>setValueClass</code>),
 * supers implementation is invoked to convert the value returned from
 * <code>parseObject</code> to the appropriate class.
 * <li>If a <code>ParseException</code> has not been thrown, and the value is
 * outside the min/max a <code>ParseException</code> is thrown.
 * <li>The value is returned.
 * </ol>
 * <code>InternationalFormatter</code> implements <code>stringToValue</code> in
 * this manner so that you can specify an alternate Class than
...
public class InternationalFormatter extends DefaultFormatter {
     ...
     * <code>AttributedCharacterIterator</code> that is obtained from the
     * <code>Format</code>. On every edit, assuming allows invalid is false, the
     * <code>Format</code> instance is invoked with
     * <code>formatToCharacterIterator</code>. A <code>BitSet</code> is also
     * kept upto date with the non-literal characters, that is for every index
     * in the <code>AttributedCharacterIterator</code> an entry in the bit set
     * is updated based on the return value from <code>isLiteral(Map)</code>.
     * <code>isLiteral(int)</code> then uses this cached information.
     * <p>
     ...
     * incrementing and decrementing. To enable this a subclass must override
     * <code>getSupportsIncrement</code> to return true, and override
     * <code>adjustValue</code> to handle the changing of the value. If you want
     * to support changing the value outside of the valid FieldPositions, you
     * will need to override <code>canIncrement</code>.
     */
    /**
     * A bit is set for every index identified in the
     * AttributedCharacterIterator that is not considered decoration. This
     * should only be used if validMask is true.
     */
    ...
    /**
     * If true, DocumentFilter methods are unconditionally allowed, and no
     * checking is done on their values. This is used when
     ...
     * <code>JFormattedTextField</code>. This will invoke
     * <code>valueToString</code> to convert the current value from the
     * <code>JFormattedTextField</code> to a String. This will then install the
     * <code>Action</code>s from <code>getActions</code>, the
     * <code>DocumentFilter</code> returned from <code>getDocumentFilter</code>
     * and the <code>NavigationFilter</code> returned from
     * <code>getNavigationFilter</code> onto the
     * <code>JFormattedTextField</code>.
     * <p>
     * Subclasses will typically only need to override this if they wish to
     * install additional listeners on the <code>JFormattedTextField</code>.
     * <p>
     * If there is a <code>ParseException</code> in converting the current value
     * to a String, this will set the text to an empty String, and mark the
     * <code>JFormattedTextField</code> as being in an invalid state.
     * <p>
     * While this is a public method, this is typically only useful for
     * subclassers of <code>JFormattedTextField</code>.
     * <code>JFormattedTextField</code> will invoke this method at the
    ...
    /**
     * Returns the <code>Format.Field</code> constants associated with the text
     * at <code>offset</code>. If <code>offset</code> is not a valid location
    ...
    /**
     * If <code>getSupportsIncrement</code> returns true, this returns two
     ...
     * <code>id</code>. This will return <code>-1</code> if the attribute can
     * not be found.
     */
    ...
    /**
     * Returns the <code>AttributedCharacterIterator</code> used to format the
     * last value.
     */
     ...
     * <code>formatToCharacterIterator</code> on the <code>Format</code>. If
     * this is successful, <code>updateMask(AttributedCharacterIterator)</code>
            ...
            if (doc != null) {
                ...
                if (string != null) {
                    try {
                        ...
                        updateMask(iterator);
                    } catch (ParseException pe) {} catch (IllegalArgumentException iae) {} catch (NullPointerException npe) {}
                }
            }
        }
    }
    ...
    /**
     * Returns true if the character at index is a literal, that is not
     * editable.
     */
    ...
    /**
     * Returns true if the character at offset is navigable too. This is
     * implemented in terms of <code>isLiteral</code>, subclasses may wish to
    ...
    /**
     * Returns the index of the next non-literal character starting at index. If
     ...
     * {@code rh.length} is corrected as to preserve trailing literals. If not
     * in overwrite mode, and there is text to insert it is inserted at the next
     * non literal index going forward. If there is only text to remove, it is
     * removed from the next non literal index going backward.
     */
    ...
    /**
     * When in !allowsInvalid mode the text is reset on every edit, thus supers
     * implementation will position the cursor at the wrong position. As such,
     * this invokes supers implementation and then invokes
    ...
    boolean replace(ReplaceHolder rh) throws BadLocationException {
        ...
        if (rh.length > 0 && (rh.text == null || rh.text.length() == 0) && (getFormattedTextField()
                .getSelectionStart() != rh.offset || rh.length > 1)) {
            direction = -1;
        }
        if (!getAllowsInvalid()) {
    ...
    /**
     * Repositions the cursor. <code>startLiteralCount</code> gives the number
     * of literals to the start of the deleted range, end gives the ending
    ...
    /**
     * Returns the character from the mask that has been buffered at
     * <code>index</code>.
     */
    ...
    /**
     * Updates the interal bitset from <code>iterator</code>. This will set
     * <code>validMask</code> to true if <code>iterator</code> is non-null.
     */
    ...
    /**
     * Returns true if <code>field</code> is non-null. Subclasses that wish to
     * allow incrementing to happen outside of the known fields will need to
     * override this.
     */
    ...
    /**
     * Returns the number of occurrences of <code>f</code> before the location
     * <code>start</code> in the current
    ...
    /**
     * Subclasses supporting incrementing must override this to handle the
     * actual incrementing. <code>value</code> is the current value,
     * <code>attributes</code> gives the field the cursor is in (may be null
     * depending upon <code>canIncrement</code>) and <code>direction</code> is
     * the amount to increment by.
     */
    Object adjustValue(Object value, Map attributes, Object field, int direction) throws BadLocationException,
            ParseException {
        return null;
    }
     ...
     * incrementing/decrementing the value should override this and return true.
     * Subclasses should also override <code>adjustValue</code>.
     */
     ...
     * ExtendedReplaceHolder keeps track of the offset and length passed into
     * canReplace.
     */
    static class ExtendedReplaceHolder extends ReplaceHolder {
        /**
         * Offset of the insert/remove. This may differ from offset in that if
         * !allowsInvalid the text is replaced on every edit.
         */
        int endOffset;
        /**
         * Length of the text. This may differ from text.length in that if
         * !allowsInvalid the text is replaced on every edit.
         */
        int endTextLength;
         ...
         * from invoking valueToString on the current value.
         */
    ...
    /**
     * IncrementAction is used to increment the value by a certain amount. It
     * calls into <code>adjustValue</code> to handle the actual incrementing of
     * the value.
     */
                ...
                if (isValidMask()) {
                    ...
                    if (start != -1) {
                        ...
                        if (canIncrement(field, start)) {
                            ...
                            } catch (ParseException pe) {} catch (BadLocationException ble) {}
                        }
                    }
                }
                if (!validEdit) {
