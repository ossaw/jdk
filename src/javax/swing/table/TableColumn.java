...
public class TableColumn extends Object implements Serializable {
	...
	/**
	 * Obsolete as of Java 2 platform v1.3. Please use string literals to
	 * identify properties.
	 */
	/*
	 * Warning: The value of this constant, "columWidth" is wrong as the name of
	 * the property is "columnWidth".
	 */
	 ...
	 * <code>JTable</code>; identifiers may be set in the
	 * <code>TableColumn</code> as as an optional way to tag and locate table
	 * columns. The table package does not modify or invoke any methods in these
	 * identifier objects other than the <code>equals</code> method which is
	 * used in the <code>getColumnIndex()</code> method in the
	 ...
	 */
	/*
	 * Counter used to disable posting of resizing notifications until the end
	 * of the resize.
	 */
	@Deprecated
	 ...
	 * <code>null</code> renderer and a <code>null</code> editor. This method is
	...
	/**
	 * Cover method, using a <code>null</code> renderer and a <code>null</code>
	 * editor.
	 * 
	 * @see #TableColumn(int, int, TableCellRenderer, TableCellEditor)
	 */
	...
	/**
	 * Creates and initializes an instance of <code>TableColumn</code> with the
	 * specified model index, width, cell renderer, and cell editor; all
	 * <code>TableColumn</code> constructors delegate to this one. The value of
	 * <code>width</code> is used for both the initial and preferred width; if
	 ...
	 * <code>getDefaultRenderer</code> or <code>getDefaultEditor</code> method,
	 * respectively, is used to provide defaults based on the type of the data
	 * in this column. This column-centric rendering strategy can be
	 * circumvented by overriding the <code>getCellRenderer</code> methods in
	 * <code>JTable</code>.
	 *
	 * @param modelIndex
	 *            the index of the column in the model that supplies the data
	 *            for this column in the table; the model index remains the same
	 ...
	 * <code>TableColumn</code>. As the <code>TableColumn</code> is moved around
	...
	/**
	 * Sets the <code>Object</code> whose string representation will be used as
	 * the value for the <code>headerRenderer</code>. When the
	 ...
	 * indicator. If you are using sorting and specify a renderer your renderer
	 ...
	 * <code>null</code>, the <code>JTableHeader</code> uses its
	 ...
	 * <code>cellRenderer</code> of the column not only controls the visual look
	 * for the column, but is also used to interpret the value object supplied
	 * by the <code>TableModel</code>. When the <code>cellRenderer</code> is
	 * <code>null</code>, the <code>JTable</code> uses a default renderer based
	 * on the class of the cells in that column. The default value for a
	...
	/**
	 * Returns the <code>TableCellEditor</code> used by the <code>JTable</code>
	 * to edit values for this column. When the <code>cellEditor</code> is
	 * <code>null</code>, the <code>JTable</code> uses a default editor based on
	 * the class of the cells in that column. The default value for a
	 ...
	 * <code>JTable</code>, use <code>setPreferredWidth</code> instead. Like a
	 * layout manager in the AWT, the <code>JTable</code> adjusts a column's
	 * width automatically whenever the table itself changes size, or a column's
	 * preferred width is changed. Setting widths programmatically therefore has
	 * no long term effect.
	 * <p>
	 ...
	 * <p>
	 * For details on how the widths of columns in the <code>JTable</code> (and
	 * <code>JTableHeader</code>) are calculated from the
	 * <code>preferredWidth</code>, see the <code>doLayout</code> method in
	 * <code>JTable</code>.
	 *
	 ...
	 * <code>minWidth</code>, adjusting the new minimum width if necessary to
	 ...
	 * <code>TableColumn</code>'s width can't be made less than this either by
	 ...
	 * <code>TableColumn</code>'s width can't be made larger than this either by
	 * the user or programmatically. The default maxWidth is Integer.MAX_VALUE.
	 *
	 ...
	 * This method does nothing if the header renderer is <code>null</code> (the
	 * default case). Otherwise, it sets the minimum, maximum and preferred
	 * widths of this column to the widths of the minimum, maximum and preferred
	 * sizes of the Component delivered by the header renderer. The transient
	 * "width" property of this TableColumn is also set to the preferred width.
	 * Note this method is not used internally by the table package.
	 *
	 * @see #setPreferredWidth
	 */
	public void sizeWidthToFit() {
		...
		Component c = headerRenderer.getTableCellRendererComponent(null, getHeaderValue(), false,
				false, 0, 0);
	 ...
	 * explicit call to <code>setFont</code>, <code>setBackground</code>, or
	 * <code>setForeground</code> on the current component. Note that if the
	 * current component is inheriting its foreground, background, or font from
	...
	/**
	 * Removes a <code>PropertyChangeListener</code> from the listener list. The
	 * <code>PropertyChangeListener</code> to be removed was registered for all
	 * properties.
	 *
	 * @param listener
	 ...
	 * this TableColumn with addPropertyChangeListener().
	 *
	 ...
	 * <code>TableColumn</code> constructor. Previously this method was used by
	 * the <code>TableColumn</code> to create a default header renderer. As of
	 * Java 2 platform v1.3, the default header renderer is <code>null</code>.
	 * <code>JTableHeader</code> now provides its own shared default renderer,
	 * just as the <code>JTable</code> does for its cell renderers.
	 *
