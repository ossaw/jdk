 ...
 * documentation about using <code>JApplet</code> in <em>The Java Tutorial</em>,
 * in the section <a href=
 * "https://docs.oracle.com/javase/tutorial/uiswing/components/applet.html">How
 * to Make Applets</a>.
 * <p>
 * The <code>JApplet</code> class is slightly incompatible with
 * <code>java.applet.Applet</code>. <code>JApplet</code> contains a
 * <code>JRootPane</code> as its only child. The <code>contentPane</code> should
 * be the parent of any children of the <code>JApplet</code>. As a convenience,
 * the {@code add}, {@code remove}, and {@code setLayout} methods of this class
 * are overridden, so that they delegate calls to the corresponding methods of
 * the {@code ContentPane}. For example, you can add a child component to an
 * applet as follows:
 * 
 * <pre>
 * applet.add(child);
 * </pre>
 *
 * And the child will be added to the <code>contentPane</code>. The
 * <code>contentPane</code> will always be non-<code>null</code>. Attempting to
 * set it to <code>null</code> will cause the <code>JApplet</code> to throw an
 * exception. The default <code>contentPane</code> will have a
 * <code>BorderLayout</code> manager set on it. Refer to
 * {@link javax.swing.RootPaneContainer} for details on adding, removing and
 * setting the <code>LayoutManager</code> of a <code>JApplet</code>.
 * <p>
 * Please see the <code>JRootPane</code> documentation for a complete
 * description of the <code>contentPane</code>, <code>glassPane</code>, and
 * <code>layeredPane</code> properties.
 * <p>
...
public class JApplet extends Applet implements Accessible, RootPaneContainer,
    ...
    public JApplet() throws HeadlessException {
         ...
         * on to a reference to this applet; unfortunately Image.getGraphics()
         * will continue to call JApplet.getForeground() and getBackground()
         * even after this applet has been destroyed. So we ensure that these
    ...
    /**
     * Gets the AccessibleContext associated with this JApplet. For JApplets,
     * the AccessibleContext takes the form of an AccessibleJApplet. A new
     * AccessibleJApplet instance is created if necessary.
     *
     * @return an AccessibleJApplet that serves as the AccessibleContext of this
     *         JApplet
     */
    ...
    /**
     * This class implements accessibility support for the <code>JApplet</code>
     * class.
     */
