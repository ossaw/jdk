...
/**
 * <code>RowFilter</code> is used to filter out entries from the model so that
 * they are not shown in the view. For example, a <code>RowFilter</code>
 ...
 * </pre>
 * 
 * <code>RowFilter</code> has two formal type parameters that allow you to
 * create a <code>RowFilter</code> for a specific model. For example, the
...
public abstract class RowFilter<M, I> {
	...
	/**
	 * Throws an IllegalArgumentException if any of the values in columns are
	 * {@literal <} 0.
	 */
	...
	/**
	 * Returns a <code>RowFilter</code> that uses a regular expression to
	 ...
	 * <code>RowFilter</code> that includes entries with at least one value
	 * starting with "a":
	 * 
	 * <pre>
	 ...
	 * @param indices
	 *            the indices of the values to check. If not supplied all values
	 *            are evaluated
	...
	/**
	 * Returns a <code>RowFilter</code> that includes entries that have at least
	 * one <code>Date</code> value meeting the specified criteria. For example,
	 * the following <code>RowFilter</code> includes only entries with at least
	...
	/**
	 * Returns a <code>RowFilter</code> that includes entries that have at least
	 * one <code>Number</code> value meeting the specified criteria. For
	 * example, the following filter will only include entries with at least one
	 * number value equal to 10:
	 * 
	 * <pre>
	 ...
	 * @throws IllegalArgumentException
	 *             if any of the <code>indices</code> are &lt; 0,
	 *             <code>type</code> is <code>null</code> or <code>number</code>
	 *             is <code>null</code>
	 */
	...
	/**
	 * Returns a <code>RowFilter</code> that includes entries if any of the
	 * supplied filters includes the entry.
	 * <p>
	 * The following example creates a <code>RowFilter</code> that will include
	...
	/**
	 * Returns a <code>RowFilter</code> that includes entries if all of the
	...
	/**
	 * Returns a <code>RowFilter</code> that includes entries if the supplied
	...
	public static abstract class Entry<M, I> {
		...
		/**
		 * Returns the number of values in the entry. For example, when used
		 * with a table this corresponds to the number of columns.
		 *
		...
		/**
		 * Returns the string value at the specified index. If filtering is
		 * being done based on <code>String</code> values this method is
		 * preferred to that of <code>getValue</code> as
		 * <code>getValue(index).toString()</code> may return a different result
		 * than <code>getStringValue(index)</code>.
		 * <p>
	...
	private static abstract class GeneralFilter extends RowFilter<Object, Object> {
		...
		protected abstract boolean include(Entry<? extends Object, ? extends Object> value,
				int index);
	}
