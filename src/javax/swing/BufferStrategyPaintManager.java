...
class BufferStrategyPaintManager extends RepaintManager.PaintManager {
	...
	private static final PlatformLogger LOGGER = PlatformLogger
			.getLogger("javax.swing.BufferStrategyPaintManager");

	/**
	 * List of BufferInfos. We don't use a Map primarily because there are
	 * typically only a handful of top level components making a Map overkill.
	 */
	private ArrayList<BufferInfo> bufferInfos;

	/**
	 * Indicates <code>beginPaint</code> has been invoked. This is set to true
	 * for the life of beginPaint/endPaint pair.
	 */
	private boolean painting;
	/**
	 * Indicates we're in the process of showing. All painting, on the EDT, is
	...
	/**
	 * Set to true if the bufferInfo needs to be disposed when current paint
	 * loop is done.
	 */
			...
			public Object run() {
				try {
					COMPONENT_CREATE_BUFFER_STRATEGY_METHOD = Component.class.getDeclaredMethod(
							...
							new Class[] { int.class, BufferCapabilities.class });
	...
	/**
	 * Shows the specified region of the back buffer. This will return true if
	 * successful, false otherwise. This is invoked on the toolkit thread in
	...
	public void copyArea(JComponent c, Graphics g, int x, int y, int w, int h, int deltaX,
		...
		if (prepare(c, root, false, 0, 0, 0, 0) && bufferInfo.isInSync()) {
			if (clip) {
				...
				int relX = xOffset + x;
				int relY = yOffset + y;
				bsg.clipRect(xOffset + cBounds.x, yOffset + cBounds.y, cBounds.width,
						cBounds.height);
	...
	public void endPaint() {
		if (LOGGER.isLoggable(PlatformLogger.Level.FINEST)) {
			LOGGER.finest("endPaint: region " + accumulatedX + " " + accumulatedY + " "
	...
	/**
	 * Invoked when the double buffering or useTrueDoubleBuffering changes for a
	 * JRootPane. If the rootpane is not double buffered, or true double
	 * buffering changes we throw out any cache we may have.
	 */
	public void doubleBufferingChanged(final JRootPane rootPane) {
		if ((!rootPane.isDoubleBuffered() || !rootPane.getUseTrueDoubleBuffering())
	 ...
	 */
	private boolean prepare(JComponent c, Container root, boolean isPaint, int x, int y, int w,
			int h) {
		if (bsg != null) {
			bsg.dispose();
			bsg = null;
		...
		if (root != null) {
			...
			if (!bufferInfo.hasBufferStrategyChanged()) {
				...
				if (isPaint && c == rootJ && x == 0 && y == 0 && c.getWidth() == w
						&& c.getHeight() == h) {
	...
	/**
	 * Returns the BufferInfo for the specified root or null if one hasn't been
	 * created yet.
	 */
	...
	/**
	 * BufferInfo is used to track the BufferStrategy being used for a
	 * particular Component. In addition to tracking the BufferStrategy it will
	 * install a WindowListener and ComponentListener. When the component is
	...
	private class BufferInfo extends ComponentAdapter implements WindowListener {
		...
		/**
		 * Whether or not the contents of the buffer strategy is in sync with
		 * the window. This is set to true when the root pane paints all, and
		 * false when contents are lost/restored.
		 */
		...
		/**
		 * Returns the BufferStartegy. This will return null if the
		 * BufferStartegy hasn't been created and <code>create</code> is false,
		...
		/**
		 * Returns true if the buffer strategy of the component differs from
		...
		/**
		 * Creates the BufferStrategy. If the appropriate system property has
		 * been set we'll try for flip first and then we'll try for blit.
		 */
		...
		private BufferStrategy createBufferStrategy(Container root, boolean isVsynced) {
			...
			if (isVsynced) {
				caps = new ExtendedBufferCapabilities(new ImageCapabilities(true),
