...
public interface ListSelectionModel {
	...
	/**
	 * A value for the selectionMode property: select one contiguous range of
	 * indices at a time.
	 *
	 * @see #setSelectionMode
	 */
	...
	/**
	 * A value for the selectionMode property: select one or more contiguous
	 ...
	 * therefore be used to grow the selection.
	 * <p>
	 * If this represents a change to the current selection, then each
	 * {@code ListSelectionListener} is notified of the change.
	 *
	 ...
	 * removeSelectionInterval(). The most recent index0 is considered the
	 * "anchor" and the most recent index1 is considered the "lead". Some
	 * interfaces display these indices specially, e.g. Windows95 displays the
	 * lead index with a dotted yellow outline.
	 *
	...
	/**
	 * Sets the {@code valueIsAdjusting} property, which indicates whether or
	 * not upcoming selection changes should be considered part of a single
	 ...
	 * listeners receive events with a {@code valueIsAdjusting} property set to
	 * {@code true}. At the end of the drag, when the change is finalized,
	 * listeners receive an event with the value set to {@code false}. Listeners
	 * can use this pattern if they wish to update only when a change has been
	 * finalized.
	 * <p>
	 * Setting this property to {@code true} begins a series of changes that is
	 * to be considered part of a single change. When the property is changed
	 ...
	 * mode, there's no restriction on what can be selected.
	 * </ul>
	 *
