...
class KeyboardManager {
    ...
    /**
     * Maps component/keystroke pairs to a topLevel container This is mainly
     * used for fast unregister operations
     */
     ...
     * Other types of keystrokes will be handled by walking the hierarchy That
     * simplifies some potentially hairy stuff.
     */
    public void registerKeyStroke(KeyStroke k, JComponent c) {
        ...
        if (tmp == null) {
            keyMap.put(k, c);
        } else if (tmp instanceof Vector) { // if there's a Vector there then
                                            // add to it.
            Vector v = (Vector) tmp;
            if (!v.contains(c)) { // only add if this keystroke isn't registered
                                  // for this component
                v.addElement(c);
            }
        } else if (tmp instanceof JComponent) {
            // if a JComponent is there then remove it and replace it with a
            // vector
            // Then add the old compoennt and the new compoent to the vector
            // then insert the vector in the table
            if (tmp != c) { // this means this is already registered for this
                            // component, no need to dup
    ...
    public void unregisterKeyStroke(KeyStroke ks, JComponent c) {
        ...
        } else if (tmp instanceof Vector) { // this means there is more than one
                                            // component reg for this key
            Vector v = (Vector) tmp;
            v.removeElement(c);
            if (v.isEmpty()) {
                keyMap.remove(ks); // remove the KeyStroke from the Map
    ...
    /**
     * This method is called when the focused component (and none of its
     * ancestors) want the key event. This will look up the keystroke to see if
    ...
    public boolean fireKeyboardAction(KeyEvent e, boolean pressed, Container topAncestor) {
        ...
        if (keyMap != null) { // this container isn't registered, so bail
            ...
            if (tmp == null) {
                ...
                if (c.isShowing() && c.isEnabled()) { // only give it out if
                                                      // enabled and visible
                    fireBinding(c, ks, e, pressed);
                }
            } else if (tmp instanceof Vector) { // more than one comp registered
                                                // for this
                Vector v = (Vector) tmp;
                ...
                for (int counter = v.size() - 1; counter >= 0; counter--) {
                    ...
                    // System.out.println("Trying collision: " + c + " vector =
                    // "+ v.size());
                    if (c.isShowing() && c.isEnabled()) { // don't want to give
                                                          // these out
                        fireBinding(c, ks, e, pressed);
                        if (e.isConsumed())
                            return true;
        ...
        if (keyMap != null) {
            ...
            if (v != null) {
                ...
                while (iter.hasMoreElements()) {
                    ...
                    if (mb.isShowing() && mb.isEnabled()) { // don't want to
                                                            // give these out
    ...
    /**
     * This class is used to create keys for a hashtable which looks up
     * topContainers based on component, keystroke pairs This is used to make
     * unregistering KeyStrokes fast
     */
