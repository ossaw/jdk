...
public abstract class LayoutStyle {
     ...
     * <code>null</code> results in using the <code>LayoutStyle</code> from the
     ...
     * has not been specified in <code>setInstance</code>, this will return the
     * <code>LayoutStyle</code> from the current <code>LookAndFeel</code>.
     *
    ...
    /**
     * <code>ComponentPlacement</code> is an enumeration of the possible ways
     * two components can be placed relative to each other.
     * <code>ComponentPlacement</code> is used by the <code>LayoutStyle</code>
     * method <code>getPreferredGap</code>. Refer to <code>LayoutStyle</code>
    ...
    public enum ComponentPlacement {
        /**
         * Enumeration value indicating the two components are visually related
         * and will be placed in the same parent. For example, a
         * <code>JLabel</code> providing a label for a <code>JTextField</code>
         * is typically visually associated with the <code>JTextField</code>;
         * the constant <code>RELATED</code> is used for this.
         */
        RELATED,

        /**
         * Enumeration value indicating the two components are visually
         * unrelated and will be placed in the same parent. For example,
         * groupings of components are usually visually separated; the constant
         * <code>UNRELATED</code> is used for this.
         */
        UNRELATED,

        /**
         * Enumeration value indicating the distance to indent a component is
         * being requested. For example, often times the children of a label
         * will be horizontally indented from the label. To determine the
         * preferred distance for such a gap use the <code>INDENT</code> type.
         * <p>
    ...
    /**
     * Creates a new <code>LayoutStyle</code>. You generally don't create a
     ...
     * 
     * <pre>
     * int gap = getPreferredGap(component1, component2, ComponentPlacement.RELATED, SwingConstants.NORTH,
     *         parent);
     * </pre>
     * 
     * The <code>type</code> parameter indicates the relation between the two
     * components. If the two components will be contained in the same parent
     * and are showing similar logically related items, use <code>RELATED</code>
     * . If the two components will be contained in the same parent but show
     * logically unrelated items use <code>UNRELATED</code>. Some look and feels
     * may not distinguish between the <code>RELATED</code> and
