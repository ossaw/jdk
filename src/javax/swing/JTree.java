...
/**
 * <a name="jtree_description"></a> A control that displays a set of
 * hierarchical data as an outline. You can find task-oriented documentation and
 * examples of using trees in <a href=
...
public class JTree extends JComponent implements Scrollable, Accessible {
	...
	/**
	 * Maps from <code>TreePath</code> to <code>Boolean</code> indicating
	 * whether or not the particular path is expanded. This ONLY indicates
	 * whether a given path is expanded, and NOT if it is visible or not. That
	 * information must be determined by visiting all the parent paths and
	 * seeing if they are visible.
	 */
	 ...
	 * <p>
	 * A handle is a small icon that displays adjacent to the node which allows
	 * the user to click once to expand or collapse the node. A common interface
	 * shows a plus sign (+) for a node which can be expanded and a minus sign
	 * (-) for a node which can be collapsed. Handles are always shown for nodes
	 * below the topmost level.
	 * <p>
	...
	/**
	 * Editor for the entries. Default is <code>null</code> (tree is not
	 * editable).
	 */
	...
	/**
	 * Is this tree a large model? This is a code-optimization setting. A large
	 * model can be used when the cell height is the same for all nodes. The UI
	 * will then cache very little information and instead continually message
	 * the model. Without a large model the UI caches most of the information,
	 * resulting in fewer method calls to the model.
	 * <p>
	 * This value is only a suggestion to the UI. Not all UIs will take
	 * advantage of it. Default value is false.
	 */
	protected boolean largeModel;

	/**
	 * Number of rows to make visible at one time. This value is used for the
	 * <code>Scrollable</code> interface. It determines the preferred size of
	 * the display area.
	 */
	 ...
	 * in tree changing or other means <code>stopCellEditing</code> is invoked,
	 * and changes are saved. If false, <code>cancelCellEditing</code> is
	 * invoked, and changes are discarded. Default is false.
	 */
	protected boolean invokesStopCellEditing;

	/**
	 * If true, when a node is expanded, as many of the descendants are scrolled
	 * to be visible.
	 */
	...
	/**
	 * Used when <code>setExpandedState</code> is invoked, will be a
	 * <code>Stack</code> of <code>Stack</code>s.
	 */
	 ...
	 * drop location for a <code>JTree</code>.
	 *
	 * @see #getDropLocation
	 * @since 1.6
	 */
	public static final class DropLocation extends
		...
		/**
		 * Returns the index where the dropped data should be inserted with
		 ...
		 * always be <code>-1</code>) as the only interesting data is the path
		 * over which the drop operation occurred.
		 * <p>
		 * For drop mode <code>DropMode.INSERT</code>, this index indicates the
		 * index at which the data should be inserted into the parent path
		 * represented by <code>getPath()</code>. <code>-1</code> indicates that
		 * the drop occurred over the parent itself, and in most cases should be
		 * treated as inserting into either the beginning or the end of the
		 * parent's list of children.
		 * <p>
		 * For <code>DropMode.ON_OR_INSERT</code>, this value will be an insert
		 * index, as described above, or <code>-1</code> if the drop occurred
		 ...
		 * component. If the drop mode is <code>DropMode.USE_SELECTION</code> or
		 * <code>DropMode.ON</code>, the return value is the path in the tree
		 * over which the data has been (or will be) dropped. <code>null</code>
		 * indicates that the drop is over empty space, not associated with a
		 * particular path.
		 * <p>
		 * If the drop mode is <code>DropMode.INSERT</code>, the return value
		 * refers to the path that should become the parent of the new data, in
		 * which case <code>getChildIndex()</code> indicates where the new item
		 * should be inserted into this parent path. A <code>null</code> path
		 * indicates that no parent path has been determined, which can happen
		 ...
		 * <code>getChildIndex</code> can be used to determine whether the drop
		 * is on top of the path itself (<code>-1</code>) or the index at which
		 * it should be inserted into the path (values other than
		 * <code>-1</code>).
		 *
	...
	/**
	 * When <code>addTreeExpansionListener</code> is invoked, and
	 * <code>settingUI</code> is true, this ivar gets set to the passed in
	 * <code>Listener</code>. This listener is then notified first in
	 * <code>fireTreeCollapsed</code> and <code>fireTreeExpanded</code>.
	 * <p>
	 * This is an ugly workaround for a way to have the UI listener get notified
	...
	/**
	 * Creates and returns a sample <code>TreeModel</code>. Used primarily for
	...
	/**
	 * Returns a <code>TreeModel</code> wrapping the specified object. If the
	 * object is:
	 * <ul>
	 ...
	 * children. Otherwise, a new root is created with a value of {@code "root"}
	 * .
	 *
	 * @param value
	 *              the <code>Object</code> used as the foundation for the
	 *              <code>TreeModel</code>
	...
	/**
	 * Returns a <code>JTree</code> with a sample model. The default model used
	 * by the tree defines a leaf node as any node without children.
	 *
	...
	/**
	 * Returns a <code>JTree</code> with each element of the specified array as
	 * the child of a new root node which is not displayed. By default, the tree
	...
	/**
	 * Returns a <code>JTree</code> created from a <code>Hashtable</code> which
	 * does not display with root. Each value-half of the key/value pairs in the
	 * <code>HashTable</code> becomes a child of the new root node. By default,
	...
	/**
	 * Returns a <code>JTree</code> with the specified <code>TreeNode</code> as
	 ...
	 *                           node; if true,
	 *                           only nodes that do not allow children are leaf
	 *                           nodes
	 * @see DefaultTreeModel#asksAllowsChildren
	 */
	...
	/**
	 * Returns the current <code>TreeCellRenderer</code> that is rendering each
	 * cell.
	 *
	...
	/**
	 * Sets the <code>TreeCellRenderer</code> that will be used to draw each
	 * cell.
	 * <p>
	 * This is a bound property.
	 *
	 ...
	 * @beaninfo bound: true description: The TreeCellRenderer that will be used
	 *           to draw each cell.
	 */
		...
		if (accessibleContext != null) {
			accessibleContext.firePropertyChange(
					AccessibleContext.ACCESSIBLE_STATE_PROPERTY, (oldValue
							...
							: null), (flag ? AccessibleState.EDITABLE : null));
		}
	}
	...
	/**
	 * Sets the cell editor. A <code>null</code> value implies that the tree
	 * cannot be edited. If this represents a change in the
	 * <code>cellEditor</code>, the <code>propertyChange</code> method is
	 * invoked on all listeners.
	 * <p>
	 * This is a bound property.
	 *
	 * @param cellEditor
	 *                   the <code>TreeCellEditor</code> to use
	 * @beaninfo bound: true description: The cell editor. A null value implies
	 *           the tree cannot be edited.
	 */
	 ...
	 *
	 * @return the <code>TreeCellEditor</code> in use, or <code>null</code> if
	 *         the tree cannot be edited
	 */
		...
		if (accessibleContext != null) {
			if (treeModel != null) {
				treeModel.removeTreeModelListener(
						(TreeModelListener) accessibleContext);
			}
			if (newModel != null) {
				newModel.addTreeModelListener(
						(TreeModelListener) accessibleContext);
			}
		}
	...
	/**
	 * Returns the height of each row. If the returned value is less than or
	 * equal to 0 the height for each row is determined by the renderer.
	 *
	 */
	 ...
	 *                 when
	 *                 editing is interrupted, and data is saved; false means
	 *                 that
	 *                 <code>cancelCellEditing</code> is invoked, and changes
	 *                 are
	 *                 lost
	 * @beaninfo bound: true description: Determines what happens when editing
	 *           is interrupted, selecting another node in the tree, a change in
	 *           the tree's data, or some other means.
	 */
	...
	/**
	 * Configures the <code>expandsSelectedPaths</code> property. If true, any
	 * time the selection is changed, either via the
	 * <code>TreeSelectionModel</code>, or the cover methods provided by
	 * <code>JTree</code>, the <code>TreePath</code>s parents will be expanded
	 * to make them visible (visible meaning the parent path is expanded, not
	 * necessarily in the visible rectangle of the <code>JTree</code>). If
	 * false, when the selection changes the nodes parent is not made visible
	 * (all its parents expanded). This is useful if you wish to have your
	 * selection model maintain paths that are not always visible (all parents
	 * expanded).
	 * <p>
	 * This is a bound property.
	 *
	 ...
	 *
	 * @since 1.3
	 * @beaninfo bound: true description: Indicates whether changes to the
	 *           selection should make the parent of the path visible.
	 */
	 ...
	 * tree's {@code TransferHandler} needs to be {@code non-null}. The default
	...
	DropLocation dropLocationForPoint(Point p) {
		...
		TreePath child;
		TreePath parent;
		boolean outside = row == -1 || p.y < bounds.y || p.y >= bounds.y
				+ bounds.height;
		...
		switch (dropMode) {
			...
			case ON_OR_INSERT:
				if (row == -1) {
					if (root != null && !model.isLeaf(root) && isExpanded(
							rootPath)) {
				...
				boolean checkOn = dropMode == DropMode.ON_OR_INSERT || !model
						.isLeaf(getPathForRow(row).getLastPathComponent());

				Section section = SwingUtilities2.liesInVertical(bounds, p,
						checkOn);
				if (section == LEADING) {
				...
				if (parent != null) {
					location = new DropLocation(p, parent, model
							.getIndexOfChild(parent.getLastPathComponent(),
	...
	/**
	 * Called to set or clear the drop location during a DnD operation. In some
	 * cases, the component may need to use it's internal selection temporarily
	 * to indicate the drop location. To help facilitate this, this method
	 * returns and accepts as a parameter a state object. This state object can
	 ...
	 * parameter. If it wants the DnD system to continue storing the same state,
	 * it must pass it back every time. Here's how this is used:
	 * <p>
	 * Let's say that on the first call to this method the component decides to
	 * save some state (because it is about to use the selection to show a drop
	 ...
	 * anything yet, so it simply passes back the same state object to have the
	 ...
	 * component for now, meaning it should restore state. At this point, it can
	...
	/**
	 * Called to indicate to this component that DnD is done. Allows for us to
	 * cancel the expand timer.
	 */
	void dndDone() {
		cancelDropTimer();
		dropTimer = null;
	...
	/**
	 * Returns the location that this component should visually indicate as the
	 * drop location during a DnD operation over the component, or {@code null}
	 ...
	 * editing begins to insure that the given path can be edited. This is
	 ...
	 * order to allow renderer's tips to be used if it has text set.
	 * <p>
	 ...
	 * {@code isRootVisible()} is {@code true}. This returns {@code 0} if the UI
	 * has not been set.
	 *
	 ...
	 * the path is hidden (under a collapsed node), and
	 * <code>getExpandsSelectedPaths</code> is true it is exposed (made
	 * viewable).
	 *
	 * @param path
	 ...
	 * component in any of the paths is hidden (under a collapsed node), and
	 * <code>getExpandsSelectedPaths</code> is true it is exposed (made
	 * viewable).
	 *
	 * @param paths
	 *              an array of <code>TreePath</code> objects that specifies the
	 *              nodes to select
	 */
	...
	/**
	 * Sets the path identifies as the lead. The lead may not be selected. The
	 * lead is not maintained by <code>JTree</code>, rather the UI will update
	 * it.
	 * <p>
	 * This is a bound property.
	 *
	 ...
	 * <code>JTree</code>, rather the UI will update it.
	 * <p>
	 * This is a bound property.
	 *
	 * @param newPath
	...
	/**
	 * Selects the nodes corresponding to each of the specified rows in the
	 * display. If a particular element of <code>rows</code> is &lt; 0 or &gt;=
	 * <code>getRowCount</code>, it will be ignored. If none of the elements in
	 * <code>rows</code> are valid rows, the selection will be cleared. That is
	 * it will be as if <code>clearSelection</code> was invoked.
	 *
	 * @param rows
	 *             an array of ints specifying the rows to select, where 0
	 *             indicates the first row in the display
	 */
	...
	/**
	 * Adds the node identified by the specified <code>TreePath</code> to the
	 * current selection. If any component of the path isn't viewable, and
	 * <code>getExpandsSelectedPaths</code> is true it is made viewable.
	 * <p>
	 * Note that <code>JTree</code> does not allow duplicate nodes to exist as
	 ...
	 * component of any of the paths isn't viewable and
	 * <code>getExpandsSelectedPaths</code> is true, it is made viewable.
	 * <p>
	 * Note that <code>JTree</code> does not allow duplicate nodes to exist as
	 * children under the same parent -- each sibling must be a unique object.
	 ...
	 *
	 * @param row
	 *            an integer specifying the row of the node to add, where 0 is
	 ...
	 *
	 * @param rows
	 *             an array of ints specifying the rows to add, where 0
	 *             indicates
	 *             the first row in the display
	 */
	...
	/**
	 * Returns the last path component of the selected path. This is a
	 ...
	 *
	 * @return the <code>TreePath</code> for the first selected node, or
	 ...
	 * forwarded to the <code>TreeSelectionModel</code>. If nothing is selected
	 * <code>null</code> or an empty array will be returned, based on the
	 * <code>TreeSelectionModel</code> implementation.
	 *
	...
	/**
	 * Returns the smallest selected row. If the selection is empty, or none of
	 * the selected paths are viewable, {@code -1} is returned.
	 *
	...
	/**
	 * Returns the largest selected row. If the selection is empty, or none of
	 * the selected paths are viewable, {@code -1} is returned.
	 *
	 ...
	 *
	 * @return an integer giving the row index of the lead path, where 0 is the
	 *         first row in the display; or -1 if <code>leadPath</code> is
	 *         <code>null</code>
	 */
	...
	public Enumeration<TreePath> getExpandedDescendants(TreePath parent) {
		...
		if (toggledPaths != null) {
			while (toggledPaths.hasMoreElements()) {
				...
				// expensive!
				if (path != parent && value != null && ((Boolean) value)
						.booleanValue() && parent.isDescendant(path)
						&& isVisible(path)) {
					if (elements == null) {
	 ...
	 * @return the <code>Rectangle</code> the node is drawn in, or
	 *         <code>null</code>
	 */
	 ...
	 * is displayed. Only works when this <code>JTree</code> is contained in a
	 * <code>JScrollPane</code>.
	 *
	 * @param path
	 *             the <code>TreePath</code> identifying the node to bring into
	 *             view
	 */
	...
	/**
	 * Scrolls the item identified by row until it is displayed. The minimum of
	 * amount of scrolling necessary to bring the row into view is performed.
	 * Only works when this <code>JTree</code> is contained in a
	 * <code>JScrollPane</code>.
	 *
	 * @param row
	 *            an integer specifying the row to scroll, where 0 is the first
	 *            row in the display
	 */
	 ...
	 * visible, or a {@code TreeUI} has not been set, <code>null</code> is
	 * returned.
	 *
	 * @param row
	 ...
	 * @return an integer specifying the display row, where 0 is the first row
	 *         in the display, or -1 if any of the elements in path are hidden
	 *         under a collapsed parent.
	 */
	...
	public void expandPath(TreePath path) {
		...
		if (path != null && model != null && !model.isLeaf(path
	...
	public TreePath getPathForLocation(int x, int y) {
		...
		if (closestPath != null) {
			...
			if (pathBounds != null && x >= pathBounds.x && x < (pathBounds.x
					+ pathBounds.width) && y >= pathBounds.y
	 ...
	 *          an integer giving the number of pixels vertically from the top
	 *          of the display area, minus any top margin
	 * @return the row corresponding to the location, or -1 if the location is
	 *         not within the bounds of a displayed cell
	 ...
	 * currently viewable, or there is no model, returns <code>null</code>,
	 * otherwise it always returns a valid path. To test if the node is exactly
	 * at x, y, get the node's bounds and test x, y against that.
	 *
	 * @param x
	 *          an integer giving the number of pixels horizontally from the
	 *          left edge of the display area, minus any left margin
	...
	/**
	 * Returns the row to the node that is closest to x,y. If no nodes are
	 * viewable or there is no model, returns -1. Otherwise, it always returns a
	 * valid row. To test if the returned object is exactly at x, y, get the
	 * bounds for the node at the returned row and test x, y against that.
	 *
	 * @param x
	 *          an integer giving the number of pixels horizontally from the
	 *          left edge of the display area, minus any left margin
	 ...
	 *          an integer giving the number of pixels vertically from the top
	 *          of the display area, minus any top margin
	 * @return the row closest to the location, -1 if nothing is viewable or
	 *         there is no model
	 *
	 ...
	 * can be obtained using <code>getSelectionPath</code>.
	 *
	...
	/**
	 * Ends the current editing session. (The <code>DefaultTreeCellEditor</code>
	 * object saves any edits that are currently in progress on a cell. Other
	 * implementations may operate differently.) Has no effect if the tree isn't
	 * being edited. <blockquote> <b>Note:</b><br>
	 ...
	 * The edit-attempt fails if the <code>CellEditor</code> does not allow
	 ...
	 * specified an empty <code>selectionModel</code> is used, which does not
	 * allow selections.
	 * <p>
	 * This is a bound property.
	 *
	 * @param selectionModel
	 *                       the <code>TreeSelectionModel</code> to use, or
	...
	public void setSelectionModel(TreeSelectionModel selectionModel) {
		...
		if (accessibleContext != null) {
			this.selectionModel.removeTreeSelectionListener(
					(TreeSelectionListener) accessibleContext);
	...
	/**
	 * Returns the paths (inclusive) between the specified rows. If the
	 * specified indices are within the viewable set of rows, or bound the
	 * viewable set of rows, then the indices are constrained by the viewable
	 * set of rows. If the specified indices are not within the viewable set of
	 * rows, or do not bound the viewable set of rows, then an empty array is
	 * returned. For example, if the row count is {@code 10}, and this method is
	 * invoked with {@code -1, 20}, then the specified indices are constrained
	 * to the viewable set of rows, and this is treated as if invoked with
	 * {@code 0, 9}. On the other hand, if this were invoked with
	 * {@code -10, -1}, then the specified indices do not bound the viewable set
	 * of rows, and an empty array is returned.
	 * <p>
	 * The parameters are not order dependent. That is, {@code
	 * getPathBetweenRows(x, y)} is equivalent to
	 * {@code getPathBetweenRows(y, x)}.
	 * <p>
	 * An empty array is returned if the row count is {@code 0}, or the
		...
		if (tree != null) {
			...
			if (rowCount > 0 && !((index0 < 0 && index1 < 0)
				...
				for (int counter = minIndex; counter <= maxIndex; counter++) {
					selection[counter - minIndex] = tree.getPathForRow(this,
							counter);
				}
				return selection;
			}
	...
	/**
	 * Selects the rows in the specified interval (inclusive). If the specified
	 * indices are within the viewable set of rows, or bound the viewable set of
	 * rows, then the specified rows are constrained by the viewable set of
	 * rows. If the specified indices are not within the viewable set of rows,
	 * or do not bound the viewable set of rows, then the selection is cleared.
	 * For example, if the row count is {@code
	 * 10}, and this method is invoked with {@code -1, 20}, then the specified
	 * indices bounds the viewable range, and this is treated as if invoked with
	 * {@code 0, 9}. On the other hand, if this were invoked with
	 * {@code -10, -1}, then the specified indices do not bound the viewable set
	 * of rows, and the selection is cleared.
	 * <p>
	 * The parameters are not order dependent. That is, {@code
	...
	/**
	 * Adds the specified rows (inclusive) to the selection. If the specified
	 * indices are within the viewable set of rows, or bound the viewable set of
	 * rows, then the specified indices are constrained by the viewable set of
	 * rows. If the indices are not within the viewable set of rows, or do not
	 * bound the viewable set of rows, then the selection is unchanged. For
	 * example, if the row count is {@code 10}, and this method is invoked with
	 * {@code -1, 20}, then the specified indices bounds the viewable range, and
	 * this is treated as if invoked with {@code 0, 9}. On the other hand, if
	 * this were invoked with {@code -10, -1}, then the specified indices do not
	...
	/**
	 * Removes the specified rows (inclusive) from the selection. If the
	 * specified indices are within the viewable set of rows, or bound the
	 * viewable set of rows, then the specified indices are constrained by the
	 * viewable set of rows. If the specified indices are not within the
	 * viewable set of rows, or do not bound the viewable set of rows, then the
	 * selection is unchanged. For example, if the row count is {@code
	 * 10}, and this method is invoked with {@code -1, 20}, then the specified
	 * range bounds the viewable range, and this is treated as if invoked with
	 * {@code 0, 9}. On the other hand, if this were invoked with
	 * {@code -10, -1}, then the specified range does not bound the viewable set
	 * of rows, and the selection is unchanged.
	 * <p>
	 * The parameters are not order dependent. That is, {@code
	 ...
	 *
	 * @param rows
	 *             an array of ints specifying display rows, where 0 is the
	 *             first
	 *             row in the display
	 */
	...
	/**
	 * Returns an array of all the <code>TreeExpansionListener</code>s added to
	 * this JTree with addTreeExpansionListener().
	 *
	...
	/**
	 * Returns an array of all the <code>TreeWillExpandListener</code>s added to
	 * this JTree with addTreeWillExpandListener().
	 *
	...
	public void fireTreeExpanded(TreePath path) {
		...
		for (int i = listeners.length - 2; i >= 0; i -= 2) {
			if (listeners[i] == TreeExpansionListener.class && listeners[i
					+ 1] != uiTreeExpansionListener) {
	 ...
	 *            when a node is selected or deselected (a "negative selection")
	 */
	...
	/**
	 * Returns an array of all the <code>TreeSelectionListener</code>s added to
	 * this JTree with addTreeSelectionListener().
	 *
	 ...
	 *
	 * @param e
	 *          the <code>TreeSelectionEvent</code> to be fired; generated by
	 *          the <code>TreeSelectionModel</code> when a node is selected or
	 *          deselected
	 * @see EventListenerList
	 */
	...
	/**
	 * Sets the number of rows that are to be displayed. This will only work if
	 * the tree is contained in a <code>JScrollPane</code>, and will adjust the
	 ...
	 * @beaninfo bound: true description: The number of rows that are to be
	 *           displayed.
	 */
	 ...
	 * To handle the conversion of a <code>TreePath</code> into a String,
	 ...
	 * @return the TreePath of the next tree element that starts with the
	 *         prefix; otherwise null
	 * @exception IllegalArgumentException
	 *                                     if prefix is null or startingRow is
	 *                                     out of bounds
	 * @since 1.4
	 */
	...
	private void readObject(ObjectInputStream s) throws IOException,
		...
		if (indexCounter < maxCounter && values.elementAt(indexCounter).equals(
				"selectionModel")) {
			selectionModel = (TreeSelectionModel) values.elementAt(
					++indexCounter);
			indexCounter++;
		}
		if (indexCounter < maxCounter && values.elementAt(indexCounter).equals(
	...
	/**
	 * Returns an object that can be archived indicating what nodes are expanded
	 * and what aren't. The objects from the model are NOT written out.
	 */
	...
	/**
	 * Updates the expanded state of nodes in the tree based on the previously
	 * archived state <code>state</code>.
	 */
	...
	/**
	 * Returns an array of integers specifying the indexs of the components in
	 * the <code>path</code>. If <code>path</code> is the root, this will return
	 * an empty array. If <code>path</code> is <code>null</code>,
	...
	private int[] getModelIndexsForPath(TreePath path) {
		if (path != null) {
			...
			for (int counter = 1; counter < count; counter++) {
				indexs[counter - 1] = model.getIndexOfChild(parent, path
						.getPathComponent(counter));
	...
	/**
	 * Returns a <code>TreePath</code> created by obtaining the children for
	 * each of the indices in <code>indexs</code>. If <code>indexs</code> or the
	 * <code>TreeModel</code> is <code>null</code>, it will return
	 * <code>null</code>.
	 */
	...
	protected static class EmptySelectionModel extends
		...
		/**
		 * This is overriden to do nothing; {@code EmptySelectionModel} does not
		 * allow a selection.
		 *
		 * @param paths
		 ...
		 *
		 * @param mapper
		 *               the {@code RowMapper} instance; this is ignored
		 * @since 1.7
		 */
	 ...
	 * <code>JTree</code> as the source and passing it off to all the listeners.
	 * <p>
	 * <strong>Warning:</strong> Serialized objects of this class will not be
	 * compatible with future Swing releases. The current serialization support
	 * is appropriate for short term storage or RMI between applications running
	...
	protected class TreeSelectionRedirector implements Serializable,
		...
		/**
		 * Invoked by the <code>TreeSelectionModel</code> when the selection
		 * changes.
		 *
		 * @param e
		 *          the <code>TreeSelectionEvent</code> generated by the
	 ...
	 * determined from <code>getVisibleRowCount</code> and the width is the
	...
	/**
	 * Returns the amount to increment when scrolling. The amount is the height
	 * of the first displayed row that isn't completely in view or, if it is
	 * totally displayed, the height of the next row in the scrolling direction.
	 *
	 * @param visibleRect
	 ...
	 * determine the width of the table, unless the preferred width of the tree
	 * is smaller than the viewports width. In other words: ensure that the tree
	 * is never smaller than its viewport.
	 *
	 ...
	 * determine the height of the table, unless the preferred height of the
	 * tree is smaller than the viewports height. In other words: ensure that
	 * the tree is never smaller than its viewport.
	 *
	...
	/**
	 * Sets the expanded state of this <code>JTree</code>. If <code>state</code>
	 * is true, all parents of <code>path</code> and path are marked as
	 * expanded. If <code>state</code> is false, all parents of
	 * <code>path</code> are marked EXPANDED, but <code>path</code> itself is
	 * marked collapsed.
	 * <p>
	...
	/**
	 * Returns an <code>Enumeration</code> of <code>TreePaths</code> that have
	 * been expanded that are descendants of <code>parent</code>.
	 */
	 ...
	 * <code>toRemove</code> that have been expanded.
	 *
	 * @param toRemove
	 ...
	 *                            if {@code toRemove} contains an element that
	 *                            is not a
	 *                            {@code TreePath}; {@code null} values are
	 *                            ignored
	 */
	protected void removeDescendantToggledPaths(
		...
		if (toRemove != null) {
			while (toRemove.hasMoreElements()) {
				Enumeration<?> descendants = getDescendantToggledPaths(toRemove
						.nextElement());
	 ...
	 * <code>TreeExpansionListener</code> events.
	 */
	...
	/**
	 * Creates and returns an instance of <code>TreeModelHandler</code>. The
	 * returned object is responsible for updating the expanded state when the
	...
	private TreePath[] getDescendantSelectedPaths(TreePath path,
		...
		if (selPaths != null) {
			...
			for (int counter = selPaths.length - 1; counter >= 0; counter--) {
				if (selPaths[counter] != null && path.isDescendant(
						selPaths[counter]) && (!path.equals(selPaths[counter])
								|| includePath))
					shouldRemove = true;
				else
	...
	/**
	 * Removes any paths from the selection model that are descendants of the
	 * nodes identified by in <code>e</code>.
	 */
	void removeDescendantSelectedPaths(TreeModelEvent e) {
		...
		if (sm != null && pPath != null && oldChildren != null
			...
			for (int counter = oldChildren.length
					- 1; counter >= 0; counter--) {
		...
		public void treeStructureChanged(TreeModelEvent e) {
			...
			if (parent.getPathCount() == 1) {
				...
				if (isExpanded) {
					...
					if (model == null || model.isLeaf(parent
							.getLastPathComponent()))
						collapsePath(parent);
					else
						expandedState.put(parent, Boolean.TRUE);
	 ...
	 * vectors/hashtables/arrays/strings and create the appropriate children
	 * tree nodes as necessary. It is dynamic in that it will only create the
	...
	public static class DynamicUtilTreeNode extends DefaultMutableTreeNode {
		 ...
		 * <code>children</code> is an array or vector all of its elements are
		 * added is children, otherwise if <code>children</code> is a hashtable
		 * all the key/value pairs are added in the order
		...
		public static void createChildren(DefaultMutableTreeNode parent,
			...
			if (children instanceof Vector) {
				...
				for (int counter = 0, maxCounter = childVector
					...
					parent.add(new DynamicUtilTreeNode(childVector.elementAt(
							counter), childVector.elementAt(counter)));
				...
				while (keys.hasMoreElements()) {
					...
					parent.add(new DynamicUtilTreeNode(aKey, childHT.get(
							aKey)));
				}
		 ...
		 * specified children. For the node to allow children, the
		 * children-object must be an array of objects, a <code>Vector</code>,
		 * or a <code>Hashtable</code> -- even if empty. Otherwise, the node is
		 * not allowed to have children.
		 *
		 * @param value
		 *                 the <code>Object</code> that is the value for the new
		 *                 node
		 * @param children
		 *                 an array of <code>Object</code>s, a
		 *                 <code>Vector</code>,
		 *                 or a <code>Hashtable</code> used to create the child
		 *                 nodes; if any other object is specified, or if the
		 *                 value
		 *                 is <code>null</code>, then the node is not allowed to
		 *                 have
		 *                 children
		 */
		 ...
		 * <code>childValue</code> is a <code>Vector</code> or array each
		 * element is added as a child, if <code>childValue</code> is a
		 * <code>Hashtable</code> each key/value pair is added in the order that
	...
	/**
	 * Returns a string representation of this <code>JTree</code>. This method
	...
	protected String paramString() {
		...
		return super.paramString() + ",editable=" + editableString
				...
				+ ",largeModel=" + largeModelString + ",rootVisible="
				+ rootVisibleString + ",rowHeight=" + rowHeight
				+ ",scrollsOnExpand=" + scrollsOnExpandString
				+ ",showsRootHandles=" + showsRootHandlesString
				+ ",toggleClickCount=" + toggleClickCount + ",visibleRowCount="
				+ visibleRowCount;
	}
	...
	/**
	 * Gets the AccessibleContext associated with this JTree. For JTrees, the
	 * AccessibleContext takes the form of an AccessibleJTree. A new
	 * AccessibleJTree instance is created if necessary.
	 *
	 * @return an AccessibleJTree that serves as the AccessibleContext of this
	 *         JTree
	 */
	...
	/**
	 * This class implements accessibility support for the <code>JTree</code>
	 * class. It provides an implementation of the Java Accessibility API
	 * appropriate to tree user-interface elements.
	 * <p>
	 * <strong>Warning:</strong> Serialized objects of this class will not be
	 * compatible with future Swing releases. The current serialization support
	 * is appropriate for short term storage or RMI between applications running
	...
	protected class AccessibleJTree extends AccessibleJComponent implements
		...
		public AccessibleJTree() {
			...
			leadSelectionAccessible = (leadSelectionPath != null)
					? new AccessibleJTreeNode(JTree.this, leadSelectionPath,
							JTree.this)
					: null;
		}
		...
		/**
		 * Fire a visible data property change notification. A 'visible' data
		 * property is one that represents something about the way the component
		 * appears on the display, where that appearance isn't bound to any
		 * other property. It notifies screen readers that the visual appearance
		 * of the component has changed, so they can notify the user.
		 */
		...
		/**
		 * Fire an active descendant property change notification. The active
		 * descendant is used for objects such as list, tree, and table, which
		 * may have transient children. It notifies screen readers the active
		 * child of the component has been changed so user can be notified from
		 * there.
		 *
		 * @param oldPath
		 *                - lead path of previous active child
		 * @param newPath
		 *                - lead path of current active child
		 *
		 */
		void fireActiveDescendantPropertyChange(TreePath oldPath,
				TreePath newPath) {
			if (oldPath != newPath) {
				Accessible oldLSA = (oldPath != null) ? new AccessibleJTreeNode(
						JTree.this, oldPath, null) : null;

				Accessible newLSA = (newPath != null) ? new AccessibleJTreeNode(
						JTree.this, newPath, null) : null;
				firePropertyChange(
		...
		private Component getCurrentComponent() {
			...
			if (JTree.this.isVisible(path)) {
				...
				if (ui != null) {
					...
					return r.getTreeCellRendererComponent(JTree.this, treeRoot,
							selected, expanded, model.isLeaf(treeRoot), row,
							hasFocus);
				}
			}
			return null;
		 ...
		 * at the local coordinate <code>Point</code>. Otherwise returns
		 * <code>null</code>.
		 *
		 * @param p
		 *          point in local coordinates of this <code>Accessible</code>
		...
		/**
		 * Returns the number of top-level children nodes of this JTree. Each of
		 * these nodes may in turn have children nodes.
		 *
		...
		/**
		 * Returns the number of items currently selected. If no items are
		 * selected, the return value will be 0.
		 *
		...
		/**
		 * Returns an Accessible representing the specified selected item in the
		 * object. If there isn't a selection, or there are fewer items selected
		...
		public Accessible getAccessibleSelection(int i) {
			...
			if (i == 0) {
				...
				TreePath childPath = new TreePath(rootPath);
				if (JTree.this.isPathSelected(childPath)) {
					return new AccessibleJTreeNode(JTree.this, childPath,
							JTree.this);
				}
			}
			return null;
		 ...
		 * item is added to any existing selection, otherwise it replaces any
		 * existing selection in the object. If the specified item is already
		 ...
		 * <code>JTree</code> child. It provides an implementation of the Java
		 * Accessibility API appropriate to tree nodes.
		 */
		protected class AccessibleJTreeNode extends AccessibleContext implements
			...
			private TreePath getChildTreePath(int i) {
				...
				if (i < 0 || i >= getAccessibleChildrenCount()) {
					...
					java.lang.System.arraycopy(objPath, 0, objChildPath, 0,
							objPath.length);
			...
			/**
			 * Get the AccessibleContext associated with this tree node. In the
			 * implementation of the Java Accessibility API for this class,
			 * return this object, which is its own AccessibleContext.
			 *
			 * @return this object
			 */
			public AccessibleContext getAccessibleContext() {
			 ...
			 *
			 * @return the localized name of the object; null if this object
			 ...
			 *
			 * @return the localized description of the object; null if this
			...
			public Accessible getAccessibleParent() {
				...
				if (accessibleParent == null) {
					...
					if (objPath.length > 1) {
						...
						java.lang.System.arraycopy(objPath, 0, objParentPath, 0,
								objPath.length - 1);
			...
			public AccessibleSelection getAccessibleSelection() {
				AccessibleContext ac = getCurrentAccessibleContext();
				if (ac != null && isLeaf) {
					return getCurrentAccessibleContext()
			...
			public Rectangle getBounds() {
				...
				if (parent != null) {
					if (parent instanceof AccessibleJTreeNode) {
						Point parentLoc = ((AccessibleJTreeNode) parent)
								.getLocationInJTree();
			 ...
			 * contained at the local coordinate <code>Point</code>. Otherwise
			 ...
			 *         specified location; else <code>null</code>
			 */
			public Accessible getAccessibleAt(Point p) {
				AccessibleContext ac = getCurrentAccessibleContext();
				if (ac instanceof AccessibleComponent) {
			...
			/**
			 * Returns an Accessible representing the specified selected item in
			 * the object. If there isn't a selection, or there are fewer items
			...
			public Accessible getAccessibleSelection(int i) {
				...
				for (int j = 0; j < childCount && i >= count; j++) {
					...
					if (tree.isPathSelected(childPath)) {
						if (count == i) {
							return new AccessibleJTreeNode(tree, childPath,
									this);
						} else {
							count++;
						}
			...
			/**
			 * Adds the specified selected item in the object to the object's
			 * selection. If the object supports multiple selections, the
			 * specified item is added to any existing selection, otherwise it
			 * replaces any existing selection in the object. If the specified
			 * item is already selected, this method has no effect.
			 *
			 * @param i
			 *          the zero-based index of selectable items
			 */
			 ...
			 * object's selection. If the specified item isn't currently
			 * selected, this method has no effect.
			 *
			 * @param i
			 *          the zero-based index of selectable items
			...
			/**
			 * Clears the selection in the object, so that nothing in the object
			 * is selected.
			 */
			public void clearAccessibleSelection() {
				int childCount = getAccessibleChildrenCount();
			...
			/**
			 * Causes every selected item in the object to be selected if the
			...
			/**
			 * Returns the number of accessible actions available in this tree
			 * node. If this node is not a leaf, there is at least one action
			 * (toggle expand), in addition to any available on the object
			...
			public int getAccessibleActionCount() {
				...
				if (ac != null) {
					...
					if (aa != null) {
						return (aa.getAccessibleActionCount() + (isLeaf ? 0
								: 1));
					}
				}
				return isLeaf ? 0 : 1;
			...
			/**
			 * Return a description of the specified action of the tree node. If
			 * this node is not a leaf, there is at least one action description
			 * (toggle expand), in addition to any available on the object
			 * behind the TreeCellRenderer.
			 *
			 * @param i
			...
			/**
			 * Perform the specified Action on the tree node. If this node is
			 * not a leaf, there is at least one action which can be done
			 * (toggle expand), in addition to any available on the object
			 * behind the TreeCellRenderer.
			 *
			 * @param i
