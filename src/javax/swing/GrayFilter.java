...
/**
 * An image filter that "disables" an image by turning it into a grayscale
 * image, and brightening the pixels in the image. Used by buttons to create an
 * image for a disabled button.
 *
 * @author Jeff Dinkins
 * @author Tom Ball
 * @author Jim Graham
...
public class GrayFilter extends RGBImageFilter {
	...
	/**
	 * Constructs a GrayFilter object that filters a color image to a grayscale
	...
	public int filterRGB(int x, int y, int rgb) {
		...
		int gray = (int) ((0.30 * ((rgb >> 16) & 0xff) + 0.59 * ((rgb >> 8) & 0xff)
				+ 0.11 * (rgb & 0xff)) / 3);
