 ...
 * How to Use Text Fields</a>, a section in <em>The Java Tutorial.</em>
 * <p>
...
public class JPasswordField extends JTextField {
	...
	/**
	 * Constructs a new <code>JPasswordField</code>, with a default document,
	 * <code>null</code> starting text string, and 0 column width.
	 */
	...
	/**
	 * Constructs a new <code>JPasswordField</code> initialized with the
	 * specified text. The document model is set to the default, and the number
	 * of columns to 0.
	 *
	 * @param text
	...
	/**
	 * Constructs a new <code>JPasswordField</code> that uses the given text
	 * storage model and the given number of columns. This is the constructor
	 * through which the other constructors feed. The echo character is set to
	 * '*', but may be changed by the current Look and Feel. If the document
	 ...
	 *            &gt;= 0; if columns is set to zero, the preferred width will
	...
	/**
	 * Returns the character to be used for echoing. The default is '*'. The
	 * default may be different depending on the currently running Look and
	 * Feel. For example, Metal/Ocean's default is a bullet character.
	 *
	...
	/**
	 * Sets the echo character for this <code>JPasswordField</code>. Note that
	 * this is largely a suggestion, since the view that gets installed can use
	 * whatever graphic techniques it desires to represent the field. Setting a
	 * value of 0 indicates that you wish to see the text as it is typed,
	 ...
	 *           characters attribute: visualUpdate true
	 */
	...
	/**
	 * Returns true if this <code>JPasswordField</code> has a character set for
	 * echoing. A character is considered to be set if the echo character is not
	 * 0.
	 *
	...
	/**
	 * Invokes <code>provideErrorFeedback</code> on the current look and feel,
	 * which typically initiates an error beep. The normal behavior of
	 * transferring the currently selected range in the associated text model to
	 * the system clipboard, and removing the contents from the model, is not
	 ...
	 * which typically initiates an error beep. The normal behavior of
	 * transferring the currently selected range in the associated text model to
	 * the system clipboard, and leaving the contents from the model, is not
	 * acceptable for a password field.
	 */
	public void copy() {
		if (getClientProperty("JPasswordField.cutCopyAllowed") != Boolean.TRUE) {
	 ...
	 * Returns the text contained in this <code>TextComponent</code>. If the
	 * underlying document is <code>null</code>, will give a
	 * <code>NullPointerException</code>. For stronger security, it is
	 * recommended that the returned character array be cleared after use by
	...
	/**
	 * Returns a string representation of this <code>JPasswordField</code>. This
	...
	/**
	 * This method is a hack to get around the fact that we cannot directly
	 * override setUIProperty because part of the inheritance hierarchy goes
	 * outside of the javax.swing package, and therefore calling a package
	 * private method isn't allowed. This method should return true if the
	 * property was handled, and false otherwise.
	 */
	 ...
	 * <code>AccessibleJPasswordField</code>. A new
	 * <code>AccessibleJPasswordField</code> instance is created if necessary.
	 *
	...
	protected class AccessibleJPasswordField extends AccessibleJTextField {
		...
		/**
		 * Gets the <code>AccessibleText</code> for the
		 * <code>JPasswordField</code>. The returned object also implements the
		...
		public String getAtIndex(int part, int index) {
			...
			if (part == AccessibleText.CHARACTER) {
				...
				if (password == null || index < 0 || index >= password.length) {
					return null;
				}
