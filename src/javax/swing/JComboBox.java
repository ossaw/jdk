 ...
 */
public class JComboBox<E> extends JComponent implements ItemSelectable,
	 ...
	 */
	protected boolean lightWeightPopupEnabled = JPopupMenu
	 ...
	 *
	 * @param ui
	 *           the <code>ComboBoxUI</code> L&amp;F object
	...
	/**
	 * Sets the data model that the <code>JComboBox</code> uses to obtain the
	 * list of items.
	 *
	 * @param aModel
	 *               the <code>ComboBoxModel</code> that provides the displayed
	...
	/**
	 * Sets the <code>lightWeightPopupEnabled</code> property, which provides a
	 * hint as to whether or not a lightweight <code>Component</code> should be
	 * used to contain the <code>JComboBox</code>, versus a heavyweight
	 * <code>Component</code> such as a <code>Panel</code> or a
	 * <code>Window</code>. The decision of lightweight versus heavyweight is
	 * ultimately up to the <code>JComboBox</code>. Lightweight windows are more
	 * efficient than heavyweight windows, but lightweight and heavyweight
	 * components do not mix well in a GUI. If your application mixes
	 * lightweight and heavyweight components, you should disable lightweight
	 * popups. The default value for the <code>lightWeightPopupEnabled</code>
	 * property is <code>true</code>, unless otherwise specified by the look and
	 * feel. Some look and feels always use heavyweight popups, no matter what
	 * the value of this property.
	 * <p>
	 * See the article <a href=
	 * "http://www.oracle.com/technetwork/articles/java/mixing-components-433992.html">
	 * Mixing Heavy and Light Components</a> This method fires a property
	 * changed event.
	 *
	 * @param aFlag
	 *              if <code>true</code>, lightweight popups are desired
	 *
	 ...
	 * editable <code>JComboBox</code> allows the user to type into the field or
	 * selected an item from the list to initialize the field, after which it
	 * can be edited. (The editing affects only the field, the list item remains
	 * intact.) A non editable <code>JComboBox</code> displays the selected item
	 * in the field, but the selection cannot be modified.
	 *
	 * @param aFlag
	...
	/**
	 * Returns true if the <code>JComboBox</code> is editable. By default, a
	...
	/**
	 * Sets the maximum number of rows the <code>JComboBox</code> displays. If
	 * the number of objects in the model is greater than count, the combo box
	 * uses a scrollbar.
	 *
	 * @param count
	 *              an integer specifying the maximum number of items to display
	 ...
	 * the selected item.
	 * <p>
	 * The default renderer displays a string or an icon. Other renderers can
	 * handle graphic images and composite items.
	 * <p>
	 ...
	 *
	 * @param aRenderer
	 *                  the <code>ListCellRenderer</code> that displays the
	 *                  selected
	 *                  item
	 * @see #setEditor
	 ...
	 *
	 * @param anEditor
	 *                 the <code>ComboBoxEditor</code> that displays the
	 *                 selected
	 *                 item
	 * @see #setRenderer
	 ...
	 * argument. If <code>anObject</code> is in the list, the display area shows
	 ...
	 * boxes, the selection will change to <code>anObject</code>.
	 * <p>
	 * If this constitutes a change in the selected item,
	 * <code>ItemListener</code>s added to the combo box will be notified with
	 * one or two <code>ItemEvent</code>s. If there is a current selected item,
	 * an <code>ItemEvent</code> will be fired and the state change will be
	 * <code>ItemEvent.DESELECTED</code>. If <code>anObject</code> is in the
	 * list and is not currently selected then an <code>ItemEvent</code> will be
	 * fired and the state change will be <code>ItemEvent.SELECTED</code>.
	 * <p>
	 * <code>ActionListener</code>s added to the combo box will be notified with
	 * an <code>ActionEvent</code> when this method is called.
	 *
	 * @param anObject
	 *                 the list object to select; use <code>null</code> to clear
	 *                 the
	 *                 selection
	 ...
	 * <p>
	 * If the combo box is editable, then this value may not have been added to
	 * the combo box with <code>addItem</code>, <code>insertItemAt</code> or the
	 * data constructors.
	 *
	 ...
	 *                                     if <code>anIndex</code> &lt; -1 or
	 *                                     <code>anIndex</code> is
	 ...
	 * result is not always defined if the <code>JComboBox</code> allows
	 * selected items that are not in the list. Returns -1 if there is no
	 * selected item or if the user specified an item which is not in the list.
	 * 
	 ...
	 * <p>
	 * If a prototype display value is specified, the preferred size of the
	 * combo box is calculated by configuring the renderer with the prototype
	 * display value and obtaining its preferred size. Specifying the preferred
	 * display value is often useful when the combo box will be displaying large
	 * amounts of data. If no prototype display value has been specified, the
	 * renderer must be configured for each value from the model and its
	 * preferred size obtained, which can be relatively expensive.
	 *
	 * @param prototypeDisplayValue
	 * @see #getPrototypeDisplayValue
	 * @since 1.4
	 * @beaninfo bound: true attribute: visualUpdate true description: The
	 *           display prototype value, used to compute display width and
	 *           height.
	 */
	 ...
	 * only if the <code>JComboBox</code> uses a mutable data model.
	 *
	 * @param item
	 *              the item to add to the list
	 * @param index
	 *              an integer specifying the position at which to add the item
	 * @see MutableComboBoxModel
	 */
	...
	/**
	 * Removes the item at <code>anIndex</code> This method works only if the
	 * <code>JComboBox</code> uses a mutable data model.
	 *
	 * @param anIndex
	 *                an int specifying the index of the item to remove, where 0
	 ...
	 *                             if <code>dataModel</code> is not an instance
	 *                             of
	 ...
	 * JComboBox with addActionListener().
	 *
	 ...
	 * <p>
	 * For all standard look and feels shipped with Java, the popup list portion
	 * of combo box is implemented as a <code>JPopupMenu</code>. A custom look
	 * and feel may not implement it this way and will therefore not receive the
	 * notification.
	 *
	 * @param l
	 ...
	 *
	 * @param aCommand
	 *                 a string containing the "command" that is sent to action
	 *                 listeners; the same listener can then do different things
	...
	/**
	 * Sets the <code>Action</code> for the <code>ActionEvent</code> source. The
	 * new <code>Action</code> replaces any previously set <code>Action</code>
	 * but does not affect <code>ActionListeners</code> independently added with
	 * <code>addActionListener</code>. If the <code>Action</code> is already a
	 * registered <code>ActionListener</code> for the <code>ActionEvent</code>
	 * source, it is not re-registered.
	 * <p>
	 * Setting the <code>Action</code> results in immediately changing all the
	 * properties described in <a href="Action.html#buttonActions"> Swing
	 * Components Supporting <code>Action</code></a>. Subsequently, the
	 * combobox's properties are automatically updated as the
	 * <code>Action</code>'s properties change.
	 * <p>
	 * This method uses three other methods to set and help track the
	 * <code>Action</code>'s property values. It uses the
	 * <code>configurePropertiesFromAction</code> method to immediately change
	 * the combobox's properties. To track changes in the <code>Action</code>'s
	 * property values, this method registers the
	 ...
	 *
	 * @param a
	 *          the <code>Action</code> for the <code>JComboBox</code>, or
	 *          <code>null</code>.
	 * @since 1.3
	 * @see Action
	 * @see #getAction
	 ...
	 *
	 * @param action
	 *                     the <code>Action</code> associated with this combobox
	 * @param propertyName
	 ...
	 * for compatibility with <code>ItemSelectable</code>.
	 *
	 ...
	 *
	 * @param b
	 *          a boolean value, where true enables the component and false
	 *          disables it
	 ...
	 *
	 * @param anEditor
	 *                 the <code>ComboBoxEditor</code> that displays the list
	 *                 item in
	 *                 the combo box field and allows it to be edited
	 * @param anItem
	...
	/**
	 * Handles <code>KeyEvent</code>s, looking for the Tab key. If the Tab key
	 * is found, the popup window is closed.
	 *
	 * @param e
	 *          the <code>KeyEvent</code> containing the keyboard key that was
	 *          pressed
	 */
	...
	protected boolean processKeyBinding(KeyStroke ks, KeyEvent e, int condition,
		...
		if (!isEditable() || condition != WHEN_FOCUSED || getEditor() == null
				|| !Boolean.TRUE.equals(getClientProperty(
	...
	/**
	 * Returns the list item at the specified index. If <code>index</code> is
	 * out of range (less than zero or greater than or equal to size) it will
	 ...
	 *              item
	 *              starts at zero
	 * @return the item at that list position; or <code>null</code> if out of
	 *         range
	 */
	...
	/**
	 * The interface that defines a <code>KeySelectionManager</code>. To qualify
	 * as a <code>KeySelectionManager</code>, the class needs to implement the
	 * method that identifies the list index given a character and the combo box
	 * data model.
	 */
	public interface KeySelectionManager {
		/**
		 * Given <code>aKey</code> and the model, returns the row that should
		 * become selected. Return -1 if no match was found.
		 *
		 * @param aKey
		 ...
		 *               pressed
		 * @param aModel
		 *               a ComboBoxModel -- the component's data model,
		 *               containing
	...
	/**
	 * Returns a string representation of this <code>JComboBox</code>. This
	...
	protected String paramString() {
		...
		String lightWeightPopupEnabledString = (lightWeightPopupEnabled ? "true"
				: "false");
	 ...
	 * boxes, the AccessibleContext takes the form of an AccessibleJComboBox. A
	 * new AccessibleJComboBox instance is created if necessary.
	 *
	 * @return an AccessibleJComboBox that serves as the AccessibleContext of
	 *         this JComboBox
	 */
	 ...
	 * <code>JComboBox</code> class. It provides an implementation of the Java
	 * Accessibility API appropriate to Combo Box user-interface elements.
	 * <p>
	 * <strong>Warning:</strong> Serialized objects of this class will not be
	...
	protected class AccessibleJComboBox extends AccessibleJComponent implements
		...
		public AccessibleJComboBox() {
			...
			JComboBox.this.addPropertyChangeListener(
					new AccessibleJComboBoxPropertyChangeListener());
		 ...
		 */
		private class AccessibleJComboBoxPropertyChangeListener implements
		...
		/*
		 * Listener for combo box popup menu TIGER - 4669379 4894434
		 */
		private class AccessibleJComboBoxPopupMenuListener implements
				PopupMenuListener {
			...
			public void popupMenuWillBecomeVisible(PopupMenuEvent e) {
				...
				previousSelectedAccessible = popupList.getAccessibleContext()
						.getAccessibleChild(selectedIndex);
			}
		...
		/*
		 * Handles changes to the popup list selection. TIGER - 4669379 4894434
		 * 4933143
		 */
		private class AccessibleJComboBoxListSelectionListener implements
			...
			public void valueChanged(ListSelectionEvent e) {
				...
				Accessible selectedAccessible = popupList.getAccessibleContext()
						.getAccessibleChild(selectedIndex);
		...
		/**
		 * Returns the nth Accessible child of the object. The child at index
		 * zero represents the popup. If the combo box is editable, the child at
		 * index one represents the editor.
		 *
		 * @param i
		...
		/**
		 * Returns the number of Actions available in this object. The default
		 * behavior of a combo box is to have one action.
		 *
		...
		/**
		 * Returns the number of Accessible children currently selected. If no
		 * children are selected, the return value will be 0.
		 *
		...
		/**
		 * Returns an Accessible representing the specified selected child in
		 * the popup. If there isn't a selection, or there are fewer children
		 * selected than the integer passed in, the return value will be null.
		 * <p>
		...
		public Accessible getAccessibleSelection(int i) {
			// Get the popup
			Accessible a = JComboBox.this.getUI().getAccessibleChild(
					JComboBox.this, 0);
		 ...
		 *
		 * @return true if the current child of this object is selected; else
		 *         false
		 * @param i
		...
		private class AccessibleEditor implements Accessible {
			public AccessibleContext getAccessibleContext() {
				if (editorAccessibleContext == null) {
					Component c = JComboBox.this.getEditor()
							.getEditorComponent();
					if (c instanceof Accessible) {
						editorAccessibleContext = new EditorAccessibleContext(
								(Accessible) c);
					}
				}
		...
		/*
		 * Wrapper class for the AccessibleContext implemented by the combo box
		 * editor. Delegates all method calls except getAccessibleIndexInParent
		 * to the editor. The getAccessibleIndexInParent method returns the
		...
		private class EditorAccessibleContext extends AccessibleContext {
			...
			/*
			 * @param a the AccessibleContext implemented by the combo box
			 * editor
			 */
			 ...
			 * accessibleName property of an object is a localized String that
			 * designates the purpose of the object. For example, the
			 * accessibleName property of a label or button might be the text of
			 * the label or button itself. In the case of an object that doesn't
			 * display its name, the accessibleName should still be set. For
			 * example, in the case of a text field used to enter the name of a
			 * city, the accessibleName for the en_US locale could be 'city.'
			 *
			 ...
			 * @see #addPropertyChangeListener
			 *
			 * @beaninfo preferred: true description: Sets the accessible name
			 *           for the component.
			 */
			 ...
			 * localized phrase describing the purpose of the object. For
			 * example, in the case of a 'Cancel' button, the
			 * accessibleDescription could be 'Ignore changes and close dialog
			 * box.'
			 *
			...
			/**
			 * Sets the accessible description of this object. Changing the name
			 * will cause a PropertyChangeEvent to be fired for the
			 ...
			 * @see #addPropertyChangeListener
			 *
			 * @beaninfo preferred: true description: Sets the accessible
			...
			/**
			 * Gets the role of this object. The role of the object is the
			 * generic purpose or use of the class of this object. For example,
			 * the role of a push button is AccessibleRole.PUSH_BUTTON. The
			 * roles in AccessibleRole are provided so component developers can
			 * pick from a set of predefined roles. This enables assistive
			 * technologies to provide a consistent interface to various tweaked
			 * subclasses of components (e.g., use AccessibleRole.PUSH_BUTTON
			 * for all components that act like a push button) as well as
			 * distinguish between subclasses that behave differently (e.g.,
			 * AccessibleRole.CHECK_BOX for check boxes and
			 ...
			 * used only in the situations where the actual component's parent
			 * should not be treated as the component's accessible parent and is
			 * a method that should only be called by the parent of the
			 * accessible child.
			 *
			 * @param a
			 ...
			 * Accessible children of an Accessible object are zero-based, so
			 * the first child of an Accessible child is at index 0, the second
			 ...
			 *
			 * @return AccessibleComponent if supported by object; else return
			 *         null
			 ...
			 *
			 * @return AccessibleSelection if supported by object; else return
			 *         null
			 ...
			 *
			 * @return AccessibleEditableText if supported by object; else
			 *         return null
			 ...
			 *
			 * @return an AccessibleTable if supported by object; otherwise
			 *         return null
			 * @see AccessibleTable
			 */
