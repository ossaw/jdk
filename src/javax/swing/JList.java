 ...
 * <p>
 * See <a href=
 * "https://docs.oracle.com/javase/tutorial/uiswing/components/list.html">How to
 * Use Lists</a> in <a href="https://docs.oracle.com/javase/tutorial/">
...
public class JList<E> extends JComponent implements Scrollable, Accessible {
    ...
    /**
     * Returns the {@code ListUI}, the look and feel object that renders this
     * component.
     *
    ...
    /**
     * Sets the {@code ListUI}, the look and feel object that renders this
     * component.
     *
     * @param ui
    ...
    /**
     * Resets the {@code ListUI} property by setting it to the value provided by
     * the current look and feel. If the current cell renderer was installed by
     * the developer (rather than the look and feel itself), this also causes
    ...
    /**
     * Returns {@code "ListUI"}, the <code>UIDefaults</code> key used to look up
     * the name of the {@code javax.swing.plaf.ListUI} class that defines the
        ...
        if (dropMode == DropMode.USE_SELECTION) {
            if (listLocation == null) {
                ...
                if (dropLocation == null) {
                    ...
                    retVal = new int[][] { inds, { getAnchorSelectionIndex(), getLeadSelectionIndex() } };
                } else {
                    retVal = state;
                }
     ...
     * <p>
     * By default, responsibility for listening for changes to this property and
     * indicating the drop location visually lies with the list's {@code ListUI}
     * , which may paint it directly and/or install a cell renderer to do so.
     * Developers wishing to implement custom drop location painting and/or
     * replace the default cell renderer, may need to honor this property.
     *
     ...
     * registered component with the <code>ToolTipManager</code>. This
     * registration is done automatically in the constructor. However, if at a
     * later point <code>JList</code> is unregistered, by way of a call to
     * {@code setToolTipText(null)}, tips from the renderers will no longer
     * display.
     *
     * @param event
    ...
    public String getToolTipText(MouseEvent event) {
        if (event != null) {
            ...
            if (index != -1 && r != null && (cellBounds = getCellBounds(index, index)) != null && cellBounds
                    .contains(p.x, p.y)) {
                ListSelectionModel lsm = getSelectionModel();
                Component rComponent = r.getListCellRendererComponent(this, getModel().getElementAt(index),
                        index, lsm.isSelectedIndex(index), (hasFocus() && (lsm
    ...
    public void setSelectedValue(Object anObject, boolean shouldScroll) {
        if (anObject == null)
            ...
            for (i = 0, c = dm.getSize(); i < c; i++)
                if (anObject.equals(dm.getElementAt(i))) {
                    ...
                    repaint(); /**
                                * FIX-ME setSelectedIndex does not redraw all
                                * the time with the basic l&f
                                **/
                    return;
                }
            setSelectedIndex(-1);
        ...
        repaint(); /**
                    * FIX-ME setSelectedIndex does not redraw all the time with
                    * the basic l&f
                    **/
    }
     ...
     * orientation:
     * <p>
     * <b>{@code VERTICAL}:</b> <br>
     * This is trivial if both {@code fixedCellWidth} and
     * {@code fixedCellHeight} have been set (either explicitly or by specifying
     * a prototype cell value). The width is simply the {@code fixedCellWidth}
     * plus the list's horizontal insets. The height is the
     * {@code fixedCellHeight} multiplied by the {@code visibleRowCount}, plus
     * the list's vertical insets.
     * <p>
     * If either {@code fixedCellWidth} or {@code fixedCellHeight} haven't been
     * specified, heuristics are used. If the model is empty, the width is the
     * {@code fixedCellWidth}, if greater than {@code 0}, or a hard-coded value
     * of {@code 256}. The height is the {@code fixedCellHeight} multiplied by
     * {@code visibleRowCount}, if {@code fixedCellHeight} is greater than
     ...
     * the list's vertical insets.
     * <p>
     * <b>{@code VERTICAL_WRAP} or {@code HORIZONTAL_WRAP}:</b> <br>
     * This method simply returns the value from {@code getPreferredSize}. The
     * list's {@code ListUI} is expected to override {@code getPreferredSize} to
     * return an appropriate value.
     *
     * @return a dimension containing the size of the viewport needed to display
     *         {@code visibleRowCount} rows
     * @see #getPreferredScrollableViewportSize
     * @see #setPrototypeCellValue
     */
     ...
     * <ul>
     * <li>if scrolling right, returns the distance to scroll so that the last
     * visible element becomes the first completely visible element
     ...
     * </ul>
     * <p>
     * For horizontal scrolling and {@code VERTICAL} orientation, returns
     ...
     * {@code JViewport} and the viewport is wider than the list's preferred
     * width, or if the layout orientation is {@code HORIZONTAL_WRAP} and
     * {@code visibleRowCount <= 0}; otherwise returns {@code false}.
     * <p>
    ...
    /**
     * Returns {@code true} if this {@code JList} is displayed in a
     * {@code JViewport} and the viewport is taller than the list's preferred
     * height, or if the layout orientation is {@code VERTICAL_WRAP} and
     * {@code visibleRowCount <= 0}; otherwise returns {@code false}.
     * <p>
    ...
    /*
     * See {@code readObject} and {@code writeObject} in {@code JComponent} for
    ...
    protected String paramString() {
        ...
        return super.paramString() + ",fixedCellHeight=" + fixedCellHeight + ",fixedCellWidth="
                ...
                + selectionForegroundString + ",visibleRowCount=" + visibleRowCount + ",layoutOrientation="
                + layoutOrientation;
    }
    ...
    /**
     * This class implements accessibility support for the {@code JList} class.
     * It provides an implementation of the Java Accessibility API appropriate
     * to list user-interface elements.
     * <p>
     * <strong>Warning:</strong> Serialized objects of this class will not be
    ...
    protected class AccessibleJList extends AccessibleJComponent implements AccessibleSelection,
        ...
        /**
         * Property Change Listener change method. Used to track changes to the
         * DataModel and ListSelectionModel, in order to re-set listeners to
         * those for reporting changes there via the Accessibility
        ...
        /**
         * List Selection Listener value change method. Used to fire the
         * property change
         *
         * @param e
         *        ListSelectionEvent
        ...
        /**
         * List Data Listener interval added method. Used to fire the visible
        ...
        /**
         * List Data Listener interval removed method. Used to fire the visible
         * data property change
         *
         * @param e
         *        ListDataEvent
        ...
        /**
         * List Data Listener contents changed method. Used to fire the visible
         * data property change
         *
         * @param e
         *        ListDataEvent
         ...
         * than the integer passed in, the return value will be
         * <code>null</code>.
         *
         * @param i
        ...
        /**
         * This class implements accessibility support appropriate for list
         * children.
         */
        protected class AccessibleJListChild extends AccessibleContext implements Accessible,
                AccessibleComponent {
            ...
            private Component getComponentAtIndex(int index) {
                ...
                if ((parent != null) && (listModel != null) && cellRenderer != null) {
                    ...
                    return cellRenderer.getListCellRendererComponent(parent, value, index, isSelected,
                            isFocussed);
                } else {
                    return null;
                }
            ...
            // Accessible Methods
            /**
             * Get the AccessibleContext for this object. In the implementation
             * of the Java Accessibility API for this class, returns this
            ...
            // TIGER - 4733624
            /**
             * Returns the icon for the element renderer, as the only item of an
             * array of <code>AccessibleIcon</code>s or a <code>null</code>
             * array if the renderer component contains no icons.
             *
             * @return an array containing the accessible icon or a
             *         <code>null</code> array if none
             * @since 1.3
             */
