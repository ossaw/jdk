 ...
 * <code>JEditorPane.setEditable(false);</code> has been called). If HTML frames
 * are embedded in the document, the typical response would be to change a
 * portion of the current document. The following code fragment is a possible
...
public class JEditorPane extends JTextComponent {
		...
		setFocusTraversalPolicy(new LayoutFocusTraversalPolicy() {
			public Component getComponentAfter(Container focusCycleRoot, Component aComponent) {
				if (focusCycleRoot != JEditorPane.this
					...
					return (rootAncestor != null) ? rootAncestor.getFocusTraversalPolicy()
							.getComponentAfter(rootAncestor, JEditorPane.this) : null;
				}
			}
			...
			public Component getComponentBefore(Container focusCycleRoot, Component aComponent) {
				if (focusCycleRoot != JEditorPane.this
					...
					Container rootAncestor = getFocusCycleRootAncestor();
					return (rootAncestor != null) ? rootAncestor.getFocusTraversalPolicy()
							.getComponentBefore(rootAncestor, JEditorPane.this) : null;
				}
			}
	...
	/**
	 * Creates a <code>JEditorPane</code> based on a string containing a URL
	 * specification.
	 *
	 * @param url
	 *            the URL
	...
	/**
	 * Returns an array of all the <code>HyperLinkListener</code>s added to this
	 * JEditorPane with addHyperlinkListener().
	 *
	...
	/**
	 * Sets the current URL being displayed. The content type of the pane is
	 * set, and if the editor kit for the pane is non-<code>null</code>, then a
	 * new default document is created and the URL is read into it. If the URL
	 * contains and reference location, the location will be scrolled to by
	 * calling the <code>scrollToReference</code> method. If the desired URL is
	 * the one currently being displayed, the document will not be reloaded. To
	 * force a document reload it is necessary to clear the stream description
	 ...
	 * </pre>
	 *
	 * If the desired URL is not the one currently being displayed, the
	 * <code>getStream</code> method is called to give subclasses control over
	 * the stream provided.
	 * <p>
	 * This may load either synchronously or asynchronously depending upon the
	 * document returned by the <code>EditorKit</code>. If the
	 * <code>Document</code> is of type <code>AbstractDocument</code> and has a
	 * value returned by
	 * <code>AbstractDocument.getAsynchronousLoadPriority</code> that is greater
	 * than or equal to zero, the page will be loaded on a separate thread using
	 * that priority.
	 * <p>
	 * If the document is loaded synchronously, it will be filled in with the
	 * stream prior to being installed into the editor with a call to
	 * <code>setDocument</code>, which is bound and will fire a property change
	 * event. If an <code>IOException</code> is thrown the partially loaded
	 * document will be discarded and neither the document or page property
	 * change events will be fired. If the document is successfully loaded and
	 * installed, a view will be built for it by the UI which will then be
	 * scrolled if necessary, and then the page property change event will be
	 * fired.
	 * <p>
	 * If the document is loaded asynchronously, the document will be installed
	 * into the editor immediately using a call to <code>setDocument</code>
	 * which will fire a document property change event, then a thread will be
	 * created which will begin doing the actual loading. In this case, the page
	 * property change event will not be fired by the call to this method
	 * directly, but rather will be fired when the thread doing the loading has
	 * finished. It will also be fired on the event-dispatch thread. Since the
	 * calling thread can not throw an <code>IOException</code> in the event of
	 * failure on the other thread, the page property change event will be fired
	 * when the other thread is done whether the load was successful or not.
	 *
	 * @param page
	 *            the URL of the page
	 * @exception IOException
	 *                for a <code>null</code> or invalid page specification, or
		...
		if ((loaded == null) || !loaded.sameFile(page) || (postData != null)) {
			...
			if (p < 0) {
				...
				if (kit != null) {
					...
					// behavior) or set the model and start to feed it on a
					// separate
	 ...
	 * <code>HTMLEditorKit</code>, and the <code>desc</code> parameter is an
	 * <code>HTMLDocument</code>, then it invokes the <code>HTMLEditorKit</code>
	 * to initiate the read. Otherwise it calls the superclass method which
	...
	/**
	 * This method invokes the <code>EditorKit</code> to initiate a read. In the
	 * case where a <code>ChangedCharSetException</code> is thrown this
	 * exception will contain the new CharSet. Therefore the <code>read</code>
	 * operation is then restarted after building a new Reader with the new
	 * charset.
	 *
	 * @param in
	...
	class PageLoader extends SwingWorker<URL, Object> {
		...
		/**
		 * Try to load the document, then scroll the view to the reference (if
		 * specified). When done, fire a page property change event.
		 */
		...
		/**
		 * The Document instance to load into. This is cached in case a new
		 * Document is created between the time the thread this is created and
		 * run.
		 */
		Document doc;
	}
	 ...
	 * <code>setPage</code> method. By default, this simply opens the URL and
	 * returns the stream. This can be reimplemented to do useful things like
	 * fetch the stream from a cache, monitor the progress of the stream, etc.
	 * <p>
	 * This method is expected to have the the side effect of establishing the
	 * content type, and therefore setting the appropriate
	 * <code>EditorKit</code> to use for loading the stream.
	 * <p>
	 * If this the stream was an http connection, redirects will be followed and
	 * the resulting URL will be set as the
	 * <code>Document.StreamDescriptionProperty</code> so that relative URL's
	 ...
	 * returned by the <code>UL.getRef</code> method for the URL being
	 * displayed). By default, this method only knows how to locate a reference
	 * in an HTMLDocument. The implementation calls the
	 * <code>scrollRectToVisible</code> method to accomplish the actual
	 * scrolling. If scrolling to a reference location is needed for document
	 * types other than HTML, this method should be reimplemented. This method
	 ...
	 * creation of the document, this will return <code>null</code>, and
	 * relative URL's will not be resolved.
	 *
	 ...
	 *            the URL for display
	 * @exception IOException
	 *                for a <code>null</code> or invalid URL specification
	 */
	...
	/**
	 * Creates the default editor kit (<code>PlainEditorKit</code>) for when the
	 ...
	 * <code>createDefaultEditorKit</code> is called to set up a default if
	 * necessary.
	 *
	 * @return the editor kit
	 */
	 ...
	 * This is defined to be the type associated with the currently installed
	 * <code>EditorKit</code>.
	 *
	 ...
	 * <code>getEditorKitForContentType</code>, and then
	 * <code>setEditorKit</code> if an editor kit can be successfully located.
	 * This is mostly convenience method that can be used as an alternative to
	 * calling <code>setEditorKit</code> directly.
	 * <p>
	 * If there is a charset definition specified as a parameter of the content
	 * type specification, it will be used when loading input streams using the
	 * associated <code>EditorKit</code>. For example if the type is specified
	 * as <code>text/html; charset=EUC-JP</code> the content will be loaded
	 * using the <code>EditorKit</code> registered for <code>text/html</code>
	 * and the Reader provided to the <code>EditorKit</code> to load unicode
	 * into the document will use the <code>EUC-JP</code> charset for
	 * translating to unicode. If the type is not recognized, the content will
	 * be loaded using the <code>EditorKit</code> registered for plain text,
	...
	/**
	 * This method gets the charset information specified as part of the content
	 * type in the http header information.
	 */
	private void setCharsetFromContentTypeParameters(String paramlist) {
		...
		try {
			...
			System.err.println(
					"JEditorPane.getCharsetFromContentTypeParameters failed on: " + paramlist);
			e.printStackTrace();
		}
	}
	 ...
	 * first deinstalled, then if kit is non-<code>null</code>, the new kit is
	 * installed, and a default document created for it. A
	 * <code>PropertyChange</code> event ("editorKit") is always fired when
	 * <code>setEditorKit</code> is called.
	 * <p>
	 * <em>NOTE: This has the side effect of changing the model,
	 * because the <code>EditorKit</code> is the source of how a particular type
	 * of content is modeled. This method will cause <code>setDocument</code> to
	 * be called on behalf of the caller to ensure integrity of the internal
	 * state.</em>
	 *
	 * @param kit
	...
	/**
	 * Fetches the editor kit to use for the given type of content. This is
	 * called when a type is requested that doesn't match the currently
	 * installed type. If the component doesn't have an <code>EditorKit</code>
	 * registered for the given type, it will try to create an
	 * <code>EditorKit</code> from the default <code>EditorKit</code> registry.
	 * If that fails, a <code>PlainEditorKit</code> is used on the assumption
	 * that all text documents can be represented as plain text.
	 * <p>
	 * This method can be reimplemented to use some other kind of type registry.
	 * This can be reimplemented to use the Java Activation Framework, for
	 * example.
	 *
	 * @param type
	 ...
	 * implementation might use this in conjunction with
	 * <code>createEditorKitForContentType</code> to install handlers for
	 ...
	 * the given text. If there is no replacement text (i.e. the content string
	 * is empty or <code>null</code>) this amounts to a removal of the current
	 * selection. The replacement text will have the attributes currently
	 * defined for input. If the component is not editable, beep and return.
	 *
	 * @param content
	 *            the content to replace the selection with. This value can be
	 *            <code>null</code>
	 */
	@Override
	...
	/**
	 * Creates a handler for the given type from the default registry of editor
	 * kits. The registry is created if necessary. If the registered class has
	 * not yet been loaded, an attempt is made to dynamically load the prototype
	 * of the kit for the given type. If the type was registered with a
	 * <code>ClassLoader</code>, that <code>ClassLoader</code> will be used to
	 * load the prototype. If there was no registered <code>ClassLoader</code>,
	 * <code>Class.forName</code> will be used to load the prototype.
	 * <p>
	 * Once a prototype <code>EditorKit</code> instance is successfully located,
	 ...
	 * <code>classname</code>. The class will be dynamically loaded later when
	 * actually needed, and can be safely changed before attempted uses to avoid
	 * loading unwanted classes. The prototype <code>EditorKit</code> will be
	 ...
	 * Establishes the default bindings of <code>type</code> to
	 * <code>classname</code>. The class will be dynamically loaded later when
	 * actually needed using the given <code>ClassLoader</code>, and can be
	...
	/**
	 * Returns the currently registered <code>EditorKit</code> class name for
	...
	/**
	 * This is invoked every time the registries are accessed. Loading is done
	 * this way instead of via a static as the static is only called once when
	 * running in plugin resulting in the entries only appearing in the first
	 * applet.
	 */
	...
	/**
	 * Returns the preferred size for the <code>JEditorPane</code>. The
	 * preferred size for <code>JEditorPane</code> is slightly altered from the
	 * preferred size of the superclass. If the size of the viewport has become
	 * smaller than the minimum size of the component, the scrollable definition
	 * for tracking width or height will turn to false. The default viewport
	 * layout will give the preferred size, and that is not desired in the case
	 * where the scrollable is tracking. In that case the <em>normal</em>
	 * preferred size is adjusted to the minimum size. This allows things like
	 * HTML tables to shrink down to their minimum size and then be laid out at
	 ...
	 * content, which is expected to be in the format of the content type of
	 * this editor. For example, if the type is set to <code>text/html</code>
	 * the string should be specified in terms of HTML.
	 * <p>
	 * This is implemented to remove the contents of the current document, and
	 * replace them by parsing the given string using the current
	 * <code>EditorKit</code>. This gives the semantics of the superclass by not
	 * changing out the model, while supporting the content type currently set
	 * on this component. The assumption is that the previous content is
	 * relatively small, and that the previous content doesn't have side
	 * effects. Both of those assumptions can be violated and cause undesirable
	 * results. To avoid this, create a new document,
	 ...
	 * <ol>
	 * <li>Leaving the existing model in place means that the old view will be
	 * torn down, and a new view created, where replacing the document would
	 * avoid the tear down of the old view.
	 * <li>Some formats (such as HTML) can install things into the document that
	...
	/**
	 * Returns the text contained in this <code>TextComponent</code> in terms of
	 * the content type of this editor. If an exception is thrown while
	 * attempting to retrieve the text, <code>null</code> will be returned. This
	 * is implemented to call <code>JTextComponent.write</code> with a
	 ...
	 *         height to match its own, false otherwise
	 */
	 ...
	 * <a href="http://www.w3.org/TR/CSS21/syndata.html#length-units"> w3c
	 * compliant</a> length units are used for html rendering.
	 * <p>
	 * By default this is not enabled; to enable it set the client
	 * {@link #putClientProperty property} with this name to
	 ...
	 * foreground color from the component are used if a font or foreground
	 * color is not specified in the styled text.
	 * <p>
	 * The default varies based on the look and feel; to enable it set the
	 * client {@link #putClientProperty property} with this name to
	 * <code>Boolean.TRUE</code>.
	 *
	 * @since 1.5
	 */
	...
	/**
	 * Gets the AccessibleContext associated with this JEditorPane. For editor
	 * panes, the AccessibleContext takes the form of an AccessibleJEditorPane.
	 * A new AccessibleJEditorPane instance is created if necessary.
	 *
	 * @return an AccessibleJEditorPane that serves as the AccessibleContext of
	 *         this JEditorPane
	 */
	public AccessibleContext getAccessibleContext() {
		...
		} else if (accessibleContext == null
				|| accessibleContext.getClass() != AccessibleJEditorPane.class) {
	 ...
	 * <code>JEditorPane</code> class. It provides an implementation of the Java
	 * Accessibility API appropriate to editor pane user-interface elements.
	 * <p>
	 * <strong>Warning:</strong> Serialized objects of this class will not be
	...
	protected class AccessibleJEditorPane extends AccessibleJTextComponent {
		 ...
		 *
		 * @return an instance of AccessibleStateSet describing the states of
		 *         the object
	...
	/**
	 * This class provides support for <code>AccessibleHypertext</code>, and is
	 * used in instances where the <code>EditorKit</code> installed in this
	 * <code>JEditorPane</code> is an instance of <code>HTMLEditorKit</code>.
	 * <p>
	 * <strong>Warning:</strong> Serialized objects of this class will not be
	 * compatible with future Swing releases. The current serialization support
	 * is appropriate for short term storage or RMI between applications running
	...
	protected class JEditorPaneAccessibleHypertextSupport extends AccessibleJEditorPane
		...
		public class HTMLLink extends AccessibleHyperlink {
			...
			/**
			 * Since the document a link is associated with may have changed,
			 * this method returns whether this Link is valid anymore (with
			...
			/**
			 * Return a String description of this particular link action. The
			 * string returned is the text within the document associated with
			...
			/**
			 * Return an object that represents the link anchor, as appropriate
			...
			/**
			 * Get the index with the hypertext document at which this link
			 * begins
			 *
			...
			if (d != null) {
				...
				while ((e = ei.next()) != null) {
					if (e.isLeaf()) {
						...
						href = (anchor != null) ? (String) anchor.getAttribute(HTML.Attribute.HREF)
								: null;
						if (href != null) {
		...
		/**
		 * Returns the index into an array of hyperlinks that index. If there is
		...
		/**
		 * Returns the contiguous text within the document that is associated
		 * with this hyperlink.
		 *
		 * @param linkIndex
		 *            into the set of hyperlinks for this hypertext doc.
	...
	static class PlainEditorKit extends DefaultEditorKit implements ViewFactory {
		 ...
		 * that are produced by this kit. The default is to have the UI produce
		 * the factory, so this method has no implementation.
		 *
		...
		static class PlainParagraph extends javax.swing.text.ParagraphView {
			...
			/**
			 * Fetch the constraining span to flow against for the given child
			 * index.
			 */
			...
			/**
			 * This class can be used to represent a logical view for a flow. It
			 * keeps the children updated to reflect the state of the model,
			 * gives the logical child views access to the view hierarchy, and
			 * calculates a preferred span. It doesn't do any rendering, layout,
			 * or model/view translation.
			 */
			static class LogicalView extends CompositeView {
				...
				/**
				 * Forward the DocumentEvent to the given child view. This is
				 * implemented to reparent the child to the logical view (the
				 * children may have been parented by a row in the flow if they
				 * fit without breaking) and then execute the superclass
				 * behavior.
				 *
				 * @param v
	...
	/*
	 * This is useful for the nightmare of parsing multi-part HTTP/RFC822
	 * headers sensibly: From a String like: 'timeout=15, max=5' create an array
	 * of Strings: { {"timeout", "15"}, {"max", "5"} } From one like: 'Basic
	 * Realm="FuzzFace" Foo="Biz Bar Baz"' create one like (no quotes in
	 * literal): { {"basic", null}, {"realm", "FuzzFace"} {"foo", "Biz Bar Baz"}
