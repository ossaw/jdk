...
/**
 * This is an implementation of the <code>DesktopManager</code>. It currently
 * implements the basic behaviors for managing <code>JInternalFrame</code>s in
 * an arbitrary parent. <code>JInternalFrame</code>s that are not children of a
 * <code>JDesktop</code> will use this component to handle their desktop-like
 * actions.
 * <p>
...
public class DefaultDesktopManager implements DesktopManager, java.io.Serializable {
    ...
    /**
     * Set to true when the user actually drags a frame vs clicks on it to start
     * the drag operation. This is only used when dragging with
     * FASTER_DRAG_MODE.
     */
     ...
     * specified. The means by which this occurs can vary depending on the
     * dragging algorithm being used. The actual logical location of the frame
    ...
    public void dragFrame(JComponent f, int newX, int newY) {
        ...
        if (dragMode == OUTLINE_DRAG_MODE) {
            ...
            if (desktopPane != null) {
                ...
                /*
                 * Work around for 6635462: XOR mode may cause a SurfaceLost on
                 * first use. Swing doesn't expect that its XOR drawRect did not
                 * complete, so believes that on re-entering at the next update
                 * location, that there is an XOR rect to draw out at
                 * "currentLoc". But in fact its now got a new clean surface
                 * without that rect, so drawing it "out" in fact draws it on,
        ...
        if (dragMode == DEFAULT_DRAG_MODE || dragMode == FASTER_DRAG_MODE) {
            ...
            if (desktopPane != null) {
                ...
                if (currentBounds != null) {
                    g.drawRect(currentBounds.x, currentBounds.y, currentBounds.width - 1, currentBounds.height
                            - 1);
                }
        ...
        while (!found) {
            ...
            for (int i = 0; i < components.length; i++) {
                ...
                if (components[i] instanceof JInternalFrame) {
                    ...
                    /*
                     * found a child that's neither an internal frame nor an
                     * icon. I don't believe this should happen, but at present
                     * it does and causes a null pointer exception. Even when
                     * that gets fixed, this code protects against the npe.
                     * hania
                     */
                    continue;

                //
                // If this icon intersects the current location, get next
                // location.
                //
            ...
            if (currentIcon == null)
                /*
                 * didn't find any useful children above. This probably
                 * shouldn't happen, but this check protects against an npe if
                 * it ever does (and it's happening now)
                 */
    ...
    /**
     * Gets the normal bounds of the component prior to the component being
     * maximized.
     * 
     * @param f
    ...
    /**
     * Returns <code>true</code> if the component has been iconized and the
     * bounds of the <code>desktopIcon</code> are valid, otherwise returns
     * <code>false</code>.
     *
     * @param f
     *        the <code>JInternalFrame</code> of interest
    ...
    private void dragFrameFaster(JComponent f, int newX, int newY) {
        ...
        Rectangle previousBounds = new Rectangle(currentBounds.x, currentBounds.y, currentBounds.width,
        ...
        try {
            if (!floaterCollision) {
                currentManager.copyArea(parent, desktopGraphics, visBounds.x, visBounds.y, visBounds.width,
                        visBounds.height, newX - previousBounds.x, newY - previousBounds.y, true);
            }
    ...
    /**
     * This method is here to clean up problems associated with a race condition
     * which can occur when the full contents of a copyArea's source argument is
     * not available onscreen. This uses brute force to clean up in case of
     * possible damage
     */
