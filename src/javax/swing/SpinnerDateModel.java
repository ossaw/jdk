...
/**
 * A <code>SpinnerModel</code> for sequences of <code>Date</code>s. The upper
 * and lower bounds of the sequence are defined by properties called
 * <code>start</code> and <code>end</code> and the size of the increase or
 * decrease computed by the <code>nextValue</code> and
 * <code>previousValue</code> methods is defined by a property called
 * <code>calendarField</code>. The <code>start</code> and <code>end</code>
 * properties can be <code>null</code> to indicate that the sequence has no
 ...
 * </ul>
 * However some UIs may set the calendarField before committing the edit to spin
...
public class SpinnerDateModel extends AbstractSpinnerModel implements
	 ...
	 * dates between <code>start</code> and <code>end</code>. The
	 ...
	 * <code>value</code> by the <code>calendarField</code> time unit. For a
	 * precise description of what it means to increment or decrement a
	 * <code>Calendar</code> <code>field</code>, see the <code>add</code> method
	 * in <code>java.util.Calendar</code>.
	 * <p>
	 ...
	 * specified and <code>minimum &gt; maximum</code> then an
	 ...
	 * equal to <code>Calendar.DAY_OF_MONTH</code>, and for which there are no
	...
	/**
	 * Changes the lower limit for Dates in this sequence. If <code>start</code>
	 * is <code>null</code>, then there is no lower limit. No bounds checking is
	 * done here: the new start value may invalidate the
	 * <code>(start &lt;= value &lt;= end)</code> invariant enforced by the
	 * constructors. This is to simplify updating the model. Naturally one
	 * should ensure that the invariant is true before calling the
	 * <code>nextValue</code>, <code>previousValue</code>, or
	...
	/**
	 * Changes the upper limit for <code>Date</code>s in this sequence. If
	 * <code>start</code> is <code>null</code>, then there is no upper limit. No
	 * bounds checking is done here: the new start value may invalidate the
	 * <code>(start &lt;= value &lt;= end)</code> invariant enforced by the
	 * constructors. This is to simplify updating the model. Naturally, one
	 * should ensure that the invariant is true before calling the
	 * <code>nextValue</code>, <code>previousValue</code>, or
	 ...
	 * <code>nextValue</code> and <code>previousValue</code> methods. The
	 * <code>calendarField</code> parameter must be one of the
	 * <code>Calendar</code> field constants like <code>Calendar.MONTH</code> or
	 * <code>Calendar.MINUTE</code>. The <code>nextValue</code> and
	 * <code>previousValue</code> methods simply move the specified
	 * <code>Calendar</code> field forward or backward by one unit with the
	 * <code>Calendar.add</code> method. You should use this method with care as
	 * some UIs may set the calendarField before committing the edit to spin the
	 * field under the cursor. If you only want one field to spin you can
	 * subclass and ignore the setCalendarField calls.
	 *
	 * @param calendarField
	 *                      one of
	 ...
	 * from by the <code>nextValue</code> and <code>previousValue</code>
	 * methods.
	 *
	 ...
	 * method is equivalent to <code>(Date)getValue</code>.
	 *
	...
	/**
	 * Sets the current <code>Date</code> for this sequence. If
	 * <code>value</code> is <code>null</code>, an
	 * <code>IllegalArgumentException</code> is thrown. No bounds checking is
	 * done here: the new value may invalidate the
	 * <code>(start &lt;= value &lt; end)</code> invariant enforced by the
	 * constructors. Naturally, one should ensure that the
	 * <code>(start &lt;= value &lt;= maximum)</code> invariant is true before
	 * calling the <code>nextValue</code>, <code>previousValue</code>, or
	 * <code>setValue</code> methods.
	 * <p>
