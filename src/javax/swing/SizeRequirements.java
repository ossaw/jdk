...
/**
 * For the convenience of layout managers, calculates information about the size
 * and position of components. All size and position calculation methods are
 * class methods that take arrays of SizeRequirements as arguments. The
 * SizeRequirements class supports two types of layout:
 * <blockquote>
 * <dl>
 * <dt>tiled
 * <dd>The components are placed end-to-end, starting either at coordinate 0
 * (the leftmost or topmost position) or at the coordinate representing the end
 * of the allocated span (the rightmost or bottommost position).
 * <dt>aligned
 * <dd>The components are aligned as specified by each component's X or Y
 * alignment value.
 * </dl>
 * </blockquote>
 * <p>
 * Each SizeRequirements object contains information about either the width (and
 * X alignment) or height (and Y alignment) of a single component or a group of
 * components:
 * <blockquote>
 * <dl>
 * <dt><code>minimum</code>
 * <dd>The smallest reasonable width/height of the component or component group,
 * in pixels.
 * <dt><code>preferred</code>
 * <dd>The natural width/height of the component or component group, in pixels.
 * <dt><code>maximum</code>
 * <dd>The largest reasonable width/height of the component or component group,
 * in pixels.
 * <dt><code>alignment</code>
 * <dd>The X/Y alignment of the component or component group.
 * </dl>
 * </blockquote>
 * <p>
...
public class SizeRequirements implements Serializable {
    ...
    /**
     * The minimum size required. For a component <code>comp</code>, this should
     * be equal to either <code>comp.getMinimumSize().width</code> or
     * <code>comp.getMinimumSize().height</code>.
     */
    public int minimum;

    /**
     * The preferred (natural) size. For a component <code>comp</code>, this
     * should be equal to either <code>comp.getPreferredSize().width</code> or
     * <code>comp.getPreferredSize().height</code>.
     */
    public int preferred;

    /**
     * The maximum size allowed. For a component <code>comp</code>, this should
     * be equal to either <code>comp.getMaximumSize().width</code> or
     * <code>comp.getMaximumSize().height</code>.
     */
    public int maximum;

    /**
     * The alignment, specified as a value between 0.0 and 1.0, inclusive. To
     * specify centering, the alignment should be 0.5.
     */
    public float alignment;

    /**
     * Creates a SizeRequirements object with the minimum, preferred, and
     * maximum sizes set to zero and an alignment value of 0.5 (centered).
     */
    ...
    /**
     * Returns a string describing the minimum, preferred, and maximum size
     * requirements, along with the alignment.
     *
     * @return the string
     */
    ...
    /**
     * Determines the total space necessary to place a set of components
     * end-to-end. The needs of each component in the set are represented by an
     * entry in the passed-in SizeRequirements array. The returned
     * SizeRequirements object has an alignment of 0.5 (centered). The space
    ...
    /**
     * Determines the total space necessary to align a set of components. The
     * needs of each component in the set are represented by an entry in the
     * passed-in SizeRequirements array. The total space required will never be
    ...
    /**
     * Creates a set of offset/span pairs representing how to lay out a set of
     * components end-to-end. This method requires that you specify the total
     * amount of space to be allocated, the size requirements for each component
     * to be placed (specified as an array of SizeRequirements), and the total
     * size requirement of the set of components. You can get the total size
     * requirement by invoking the getTiledSizeRequirements method. The
     * components will be tiled in the forward direction with offsets increasing
     * from 0.
     *
     * @param allocated
     ...
     * This method also requires a flag indicating whether components should be
     * tiled in the forward direction (offsets increasing from 0) or reverse
     * direction (offsets decreasing from the end of the allocated space). The
     * forward direction represents components tiled from left to right or top
     * to bottom. The reverse direction represents components tiled from right
    ...
    private static void expandedTile(int allocated, long min, long pref, long max, SizeRequirements[] request,
    ...
    /**
     * Creates a bunch of offset/span pairs specifying how to lay out a set of
     * components with the specified alignments. The resulting span allocations
     * will overlap, with each one fitting as well as possible into the given
     * total allocation. This method requires that you specify the total amount
     * of space to be allocated, the size requirements for each component to be
     * placed (specified as an array of SizeRequirements), and the total size
     * requirements of the set of components (only the alignment field of which
     * is actually used). You can get the total size requirement by invoking
    ...
    /**
     * Creates a set of offset/span pairs specifying how to lay out a set of
     * components with the specified alignments. The resulting span allocations
     * will overlap, with each one fitting as well as possible into the given
     * total allocation. This method requires that you specify the total amount
     * of space to be allocated, the size requirements for each component to be
     ...
     * is actually used) You can get the total size requirement by invoking
     * getAlignedSizeRequirements.
     * This method also requires a flag indicating whether normal or reverse
     * alignment should be performed. With normal alignment the value 0.0f
     * represents the left/top edge of the component to be aligned. With reverse
    ...
    // This method was used by the JTable - which now uses a different
    // technique.
    /**
