...
public class JViewport extends JComponent implements Accessible {
    ...
    /**
     * True when the viewport dimensions have been determined. The default is
     * false.
     */
     ...
     * <code>JList</code> and the selected cell is the last in the list, the
     * <code>scrollpane</code> autoscrolls. Here, the old selected cell needs
     * repainting and so we need a flag to make the viewport do the optimized
     * painting only when there is an explicit call to
     * <code>setViewPosition(Point)</code>. When <code>setBounds</code> is
     * called through other routes, the flag is off and the view repaints
     * normally. Another approach would be to remove this from the
     * <code>JViewport</code> class and have the <code>JList</code> manage this
     * case by using <code>setBackingStoreEnabled</code>. The default is
     * <code>false</code>.
     */
    ...
    /*
     * Only one <code>ChangeEvent</code> is needed per <code>JViewport</code>
    ...
    /**
     * Use <code>graphics.copyArea</code> to implement scrolling. This is the
     * fastest for most applications.
     *
     * @see #setScrollMode
     * @since 1.3
     */
    ...
    /**
     * Draws viewport contents into an offscreen image. This was previously the
     * default mode for <code>JTable</code>. This mode may offer advantages over
     * "blit mode" in some cases, but it requires a large chunk of extra RAM.
     *
     * @see #setScrollMode
     * @since 1.3
     */
    ...
    /**
     * This mode uses the very simple method of redrawing the entire contents of
     * the scrollpane each time it is scrolled. This was the default behavior in
     * Swing 1.0 and Swing 1.1. Either of the other two options will provide
    ...
    /**
     * This is set to true in <code>setViewPosition</code> if doing a window
     * blit and the viewport is obscured.
     */
    private transient boolean repaintAll;

    /**
     * This is set to true in paint, if <code>repaintAll</code> is true and the
     * clip rectangle does not match the bounds. If true, and scrolling happens
     * the repaint manager is not cleared which then allows for the repaint
     * previously invoked to succeed.
     */
    private transient boolean waitingForRepaint;

    /**
     * Instead of directly invoking repaint, a <code>Timer</code> is started and
     * when it fires, repaint is invoked.
     */
    ...
    /**
     * When view is changed we have to synchronize scrollbar values with
     * viewport (see the BasicScrollPaneUI#syncScrollPaneWithViewport method).
     * This flag allows to invoke that method while
     * ScrollPaneLayout#layoutContainer is running.
     */
     ...
     * <code>null</code>. (Since there is only one child which occupies the
     * entire viewport, the <code>constraints</code> and <code>index</code>
                ...
                if (view.isValid()) {
                    if (getParent().getComponentOrientation().isLeftToRight()) {
                        ...
                        if (extent.width > viewSize.width) {
                            ...
                            viewPosition.x = Math.max(0, Math.min(viewSize.width - extent.width,
                                    viewPosition.x));
                        }
                    }
    ...
    /**
     * Ascends the <code>Viewport</code>'s parents stopping when a component is
     * found that returns <code>true</code> to <code>isValidateRoot</code>. If
     * all the <code>Component</code>'s parents are visible,
     * <code>validate</code> will then be invoked on it. The
     * <code>RepaintManager</code> is then invoked with
     * <code>removeInvalidComponent</code>. This is the synchronous version of a
    ...
    /**
     * The viewport "scrolls" its child (called the "view") by the normal
     * parent/child clipping (typically the view is moved in the opposite
     * direction of the scroll). A non-<code>null</code> border, or non-zero
     * insets, isn't supported, to prevent the geometry of this component from
     * becoming complex enough to inhibit subclassing. To create a
     * <code>JViewport</code> with a border, add it to a <code>JPanel</code>
     * that has a border.
     * <p>
     * Note: If <code>border</code> is non-<code>null</code>, this method will
     * throw an exception as borders are not supported on a
     * <code>JViewPort</code>.
     *
     * @param border
    ...
    /**
     * Returns the insets (border) dimensions as (0,0,0,0), since borders are
     * not supported on a <code>JViewport</code>.
     *
     ...
     * <code>JViewport</code>s inset values. The passed-in <code>Insets</code>
     * object will be reinitialized, and all existing values within this object
     * are overwritten.
     *
     * @param insets
    ...
    /**
     * The <code>JViewport</code> overrides the default implementation of this
     * method (in <code>JComponent</code>) to return false. This ensures that
     * the drawing machinery will call the <code>Viewport</code>'s
     * <code>paint</code> implementation rather than messaging the
     * <code>JViewport</code>'s children directly.
     *
     * @return false
     */
    ...
    /**
     * Returns true if scroll mode is a {@code BACKINGSTORE_SCROLL_MODE} to
     * cause painting to originate from {@code JViewport}, or one of its
    ...
    /**
     * Depending on whether the <code>backingStore</code> is enabled, either
     * paint the image through the backing store or paint just the recently
     * exposed part, using the backing store to "blit" the remainder.
     * <blockquote> The term "blit" is the pronounced version of the PDP-10 BLT
     * (BLock Transfer) instruction, which copied a block of bits. (In case you
    ...
    public void paint(Graphics g) {
        ...
        if (backingStoreImage == null) {
            ...
            if (!scrollUnderway || lastPaintPosition.equals(getViewLocation())) {
                ...
                // The image was scrolled. Manipulate the backing store and
                // flush it to g.
    ...
    /**
     * Returns <code>true</code> if this viewport is maintaining an offscreen
    ...
    /**
     * If true if this viewport will maintain an offscreen image of its
     * contents. The image is used to reduce the cost of small one dimensional
     * changes to the <code>viewPosition</code>. Rather than repainting the
     * entire viewport we use <code>Graphics.copyArea</code> to effect some of
     * the scroll.
     *
     * @param enabled
    ...
    public void setView(Component view) {
        ...
        /*
         * Remove the viewport's existing children, if any. Note that
         * removeAll() isn't used here because it doesn't call remove() (which
         * JViewport overrides).
         */
     ...
     * otherwise return the view's current size. If there is no view, return
     * 0,0.
     *
     ...
     * the viewport, or 0,0 if there's no view.
     *
            ...
            if (!waitingForRepaint && isBlitting() && canUseWindowBlitter()) {
                ...
                if (dirty == null || !dirty.contains(jview.getVisibleRect())) {
                    ...
                    try {
                        ...
                        Rectangle r = new Rectangle(0, 0, getWidth(), Math.min(getHeight(), jview
                                .getHeight()));
                        g.setClip(r);
     ...
     * currently contains <code>oldLoc</code> in the upper left hand corner and
     * we're scrolling to <code>newLoc</code>. The parameters are modified to
     ...
     * <code>null</code>) in the constructor. Returns the
     * <code>LayoutManager</code> to install on the <code>JViewport</code>.
     *
    ...
    /**
     * Adds a <code>ChangeListener</code> to the list that is notified each time
     ...
     * JViewport with addChangeListener().
     *
    ...
    /**
     * Always repaint in the parents coordinate system to make sure only one
    ...
    /**
     * Returns a string representation of this <code>JViewport</code>. This
     ...
     * <code>windowBlit</code> property. (The <code>putClientProperty</code>
     * property is final).
     *
     * @param propertyName
    ...
    /**
     * Returns true if the component needs to be completely repainted after a
     * blit and a paint is received.
     */
    ...
    /**
     * If the repaint manager has a dirty region for the view, the view is asked
     * to paint.
     *
     * @param g
     *        the <code>Graphics</code> context within which to paint
    ...
    private boolean windowBlitPaint(Graphics g) {
        ...
        if (lastPaintPosition == null || lastPaintPosition.equals(getViewLocation())) {
            ...
            if (!canBlit) {
                ...
                blitDoubleBuffered(view, g, r.x, r.y, r.width, r.height, blitFrom.x, blitFrom.y, blitTo.x,
                        blitTo.y, blitSize.width, blitSize.height);
                retValue = true;
            }
        }
        lastPaintPosition = getViewLocation();
    ...
    private void blitDoubleBuffered(JComponent view, Graphics g, int clipX, int clipY, int clipW, int clipH,
            int blitFromX, int blitFromY, int blitToX, int blitToY, int blitW, int blitH) {
        // NOTE:
    ...
    /**
     * Called to paint the view, usually when <code>blitPaint</code> can not
     * blit.
     *
     * @param g
     *        the <code>Graphics</code> context within which to paint
    ...
    /**
     * Returns true if the viewport is not obscured by one of its ancestors, or
     * its ancestors children and if the viewport is showing. Blitting when the
     * view isn't showing will work, or rather <code>copyArea</code> will work,
    ...
    private boolean canUseWindowBlitter() {
        ...
        Rectangle dirtyRegion = RepaintManager.currentManager(this).getDirtyRegion((JComponent) getParent());
        ...
        for (parent = this; parent != null && isLightweightComponent(parent); parent = parent.getParent()) {
            ...
            if (lastParent != null && parent instanceof JComponent && !((JComponent) parent)
    ...
    /**
     * Gets the AccessibleContext associated with this JViewport. For viewports,
     * the AccessibleContext takes the form of an AccessibleJViewport. A new
     * AccessibleJViewport instance is created if necessary.
     *
     * @return an AccessibleJViewport that serves as the AccessibleContext of
     *         this JViewport
     */
     ...
     * <code>JViewport</code> class. It provides an implementation of the Java
     * Accessibility API appropriate to viewport user-interface elements.
     * <p>
     * <strong>Warning:</strong> Serialized objects of this class will not be
     * compatible with future Swing releases. The current serialization support
     * is appropriate for short term storage or RMI between applications running
