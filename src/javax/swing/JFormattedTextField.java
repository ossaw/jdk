 ...
 * been committed. Similarly if the focus lost policy is
 ...
 * not be queried when focus is lost or gained.
 * <p>
 * <code>JFormattedTextField.AbstractFormatter</code> is also responsible for
 * determining when values are committed to the <code>JFormattedTextField</code>
 * . Some <code>JFormattedTextField.AbstractFormatter</code>s will make new
 * values available on every edit, and others will never commit the value. You
 * can force the current value to be obtained from the current
...
public class JFormattedTextField extends JTextField {
	 ...
	 * should be invoked. If in committing the new value a
	 * <code>ParseException</code> is thrown, the invalid value will remain.
	 *
	 ...
	 * Constant identifying that when focus is lost, <code>commitEdit</code>
	 * should be invoked. If in committing the new value a
	 * <code>ParseException</code> is thrown, the value will be reverted.
	 *
	 * @see #setFocusLostBehavior
	 */
	...
	/**
	 * Constant identifying that when focus is lost, editing value should be
	 * reverted to current value set on the <code>JFormattedTextField</code>.
	 *
	 * @see #setFocusLostBehavior
	 */
	...
	/**
	 * Constant identifying that when focus is lost, the edited value should be
	 * left.
	 *
	 * @see #setFocusLostBehavior
	 */
	 ...
	 * <code>JFormattedTextField</code> to edit a particular type of value.
	 */
	...
	/**
	 * Creates a JFormattedTextField with the specified value. This will create
	 * an <code>AbstractFormatterFactory</code> based on the type of
	 * <code>value</code>.
	 *
	 * @param value
	 ...
	 * wrapped in an appropriate <code>AbstractFormatter</code> which is then
	 ...
	 * <code>AbstractFormatter</code>. The <code>AbstractFormatter</code> is
	 ...
	 * <code>JFormattedTextField.PERSIST</code> Note that some
	 * <code>AbstractFormatter</code>s may push changes as they occur, so that
	 ...
	 * <code>COMMIT_OR_REVERT</code>, <code>COMMIT</code>, <code>REVERT</code>
	 * or <code>PERSIST</code> Note that some <code>AbstractFormatter</code>s
	 * may push changes as they occur, so that the value of this will have no
	 * effect.
	 *
	 ...
	 * <code>AbstractFormatterFactory</code> is able to return an instance of
	 * <code>AbstractFormatter</code> that is used to format a value for
	 * display, as well an enforcing an editing policy.
	 * <p>
	 ...
	 * <code>Class</code> of the value. <code>NumberFormatter</code> will be
	 * used for <code>Number</code>s, <code>DateFormatter</code> will be used
	 * for <code>Dates</code>, otherwise <code>DefaultFormatter</code> will be
	 * used.
	 * <p>
	 ...
	 * <code>JFormattedTextField</code> will invoke this as the state of the
	 * <code>JFormattedTextField</code> changes and requires the value to be
	 * reset. <code>JFormattedTextField</code> passes in the
	...
	/**
	 * Returns the last valid value. Based on the editing policy of the
	 * <code>AbstractFormatter</code> this may not return the current value. The
	 * currently edited value can be obtained by invoking
	 ...
	 * <code>AbstractFormatter</code> and set as the current value. This has no
	 * effect if there is no current <code>AbstractFormatter</code> installed.
	 *
	 * @throws ParseException
	 *             if the <code>AbstractFormatter</code> is not able to format
	 *             the current value
	 */
	 ...
	 * invoke this. This will be invoked by the <code>AbstractFormatter</code>
	 * as the user edits the value.
	 * <p>
	 * Not all formatters will allow the component to get into an invalid state,
	 * and thus this may never be invoked.
	 * <p>
	 * Based on the look and feel this may visually change the state of the
	 * receiver.
	 *
	 * @param isValid
	...
	/**
	 * Invoked when the user inputs an invalid value. This gives the component a
	...
	/**
	 * Does the setting of the value. If <code>createFormat</code> is true, this
	 * will also obtain a new <code>AbstractFormatter</code> from the current
	 * factory. The property change event will be fired if <code>firePC</code>
	 * is true.
	 */
	 ...
	 * <code>AbstractFormatter</code>s when the <code>JFormattedTextField</code>
	 * has focus vs when it doesn't have focus.
	 * 
	 * @since 1.4
	 */
	 ...
	 * <code>JFormattedTextField</code> to handle the conversion both from an
	 * Object to a String, and back from a String to an Object.
	 * <code>AbstractFormatter</code>s can also enforce editing policies, or
	 * navigation policies, or manipulate the <code>JFormattedTextField</code>
	 * in any way it sees fit to enforce the desired policy.
	 * <p>
	 * An <code>AbstractFormatter</code> can only be active in one
	 * <code>JFormattedTextField</code> at a time.
	 * <code>JFormattedTextField</code> invokes <code>install</code> when it is
	 * ready to use it followed by <code>uninstall</code> when done. Subclasses
	 ...
	 * <code>getNavigationFilter</code> and <code>getDocumentFilter</code> to
	 * restrict the <code>JFormattedTextField</code> in a particular way.
	 * <p>
	 * Subclasses that allow the <code>JFormattedTextField</code> to be in a
	 * temporarily invalid state should invoke <code>setEditValid</code> at the
	 * appropriate times.
	 * 
	 * @since 1.4
	 */
	public static abstract class AbstractFormatter implements Serializable {
		 ...
		 * <code>JFormattedTextField</code> to a String. This will then install
		 * the <code>Action</code>s from <code>getActions</code>, the
		 * <code>DocumentFilter</code> returned from
		 * <code>getDocumentFilter</code> and the <code>NavigationFilter</code>
		 * returned from <code>getNavigationFilter</code> onto the
		 ...
		 * <p>
		 * If there is a <code>ParseException</code> in converting the current
		 * value to a String, this will set the text to an empty String, and
		 * mark the <code>JFormattedTextField</code> as being in an invalid
		 * state.
		 * <p>
		 ...
		 * appropriate times when the value changes, or its internal state
		 * changes. You will only need to invoke this yourself if you are
		 * subclassing <code>JFormattedTextField</code> and
		 * installing/uninstalling <code>AbstractFormatter</code> at a different
		...
		/**
		 * This should be invoked when the user types an invalid character. This
		 * forwards the call to the current JFormattedTextField.
		 */
		protected void invalidEdit() {
		 ...
		 * <code>JFormattedTextField</code>. If you an enforce a policy such
		 * that the <code>JFormattedTextField</code> is always in a valid state,
		 ...
		 * <code>Action</code>s. <code>install</code> will install these on the
		 * <code>JFormattedTextField</code>'s <code>ActionMap</code>.
		 *
		 ...
		 * <code>install</code> will install the returned value onto the
		 * <code>JFormattedTextField</code>.
		 *
		 ...
		 * where the user can navigate to. <code>install</code> will install the
		 * returned value onto the <code>JFormattedTextField</code>.
		 *
		...
		/**
		 * Clones the <code>AbstractFormatter</code>. The returned instance is
		 * not associated with a <code>JFormattedTextField</code>.
		 *
	...
	/**
	 * Used to commit the edit. This extends JTextField.NotifyAction so that
	 * <code>isEnabled</code> is true while a JFormattedTextField has focus, and
	 * extends <code>actionPerformed</code> to invoke commitEdit.
	 */
	 ...
	 * <code>actionPerformed</code> is invoked. It will only be enabled if the
	 * focused component is an instance of JFormattedTextField.
	 */
