...
public class DefaultListSelectionModel implements ListSelectionModel, Cloneable, Serializable {
	 ...
	 *
	 * <pre>
	 * ListSelectionListener[] lsls = (ListSelectionListener[]) (m
	 ...
	 * important, such as when the new lead or anchor needs to be updated in the
	 * view. Therefore, caution is urged when changing the default value.
	 *
	 ...
	 * without exception. Otherwise, if either index is less than {@code -1}, an
	 ...
	 *             if either index is less than {@code -1} (and neither index is
	 *             {@code -1})
	 * @see #addListSelectionListener
	 */
	 ...
	 * <p>
	 * In {@code SINGLE_SELECTION} selection mode, this is equivalent to calling
	 * {@code setSelectionInterval}, and only the second index is used. In
	 * {@code SINGLE_INTERVAL_SELECTION} selection mode, this method behaves
	 * like {@code setSelectionInterval}, unless the given interval is
	 * immediately adjacent to or overlaps the existing selection, and can
	 * therefore be used to grow it.
	 * <p>
	 * If this represents a change to the current selection, then each
	 ...
	 * {@code index0} doesn't have to be less than or equal to {@code index1}.
	 * <p>
	 * In {@code SINGLE_INTERVAL_SELECTION} selection mode, if the removal would
	 * produce two disjoint selections, the removal is extended through the
	 * greater end of the selection. For example, if the selection is
	...
	/**
	 * Insert length indices beginning before/after index. If the value at index
	 * is itself selected and the selection mode is not SINGLE_SELECTION, set
	 * all of the newly inserted items as selected. Otherwise leave them
	 ...
	 *                if the selection model does not both (a) implement the
	 ...
	 * leadAnchorNotificationEnabled is true, send a notification covering the
	...
	public void moveLeadSelectionIndex(int leadIndex) {
		...
		if (leadIndex == -1) {
			...
			/*
			 * PENDING(shannonh) - The following check is nice, to be consistent
			 * with setLeadSelectionIndex. However, it is not absolutely
			 * necessary: One could work around it by setting the anchor to
			 * something valid, modifying the lead, and then moving the anchor
			 * back to -1. For this reason, there's no sense in adding it at
			 * this time, as that would require updating the spec and officially
			 * committing to it.
			 * 
			 * // otherwise, don't do anything if the anchor is -1 } else if
			 * (this.anchorIndex == -1) { return;
			 */
	 ...
	 * and the new lead are either all selected or all deselected. If the value
	 * at the anchor index is selected, first clear all the values in the range
	 * [anchor, oldLeadIndex], then select all the values values in the range
	 * [anchor, newLeadIndex], where oldLeadIndex is the old leadIndex and
	 * newLeadIndex is the new one.
	 * <p>
	 ...
	 * purposes of generating minimal bounds in this event, do the operation in
	 * a single pass; that way the first and last index inside the
	 * ListSelectionEvent that is broadcast will refer to cells that actually
	 * changed value because of this method. If, instead, this operation were
	 * done in two steps the effect on the selection state would be the same but
	 * two events would be generated and the bounds around the changed values
	 * would be wider, including cells that had been first cleared only to later
	 * be set.
	 * <p>
