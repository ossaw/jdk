...
public class MenuSelectionManager {
    ...
    private static final StringBuilder MENU_SELECTION_MANAGER_KEY = new StringBuilder(
    ...
    /**
     * Changes the selection in the menu hierarchy. The elements in the array
     * are sorted in order from the root menu element to the currently selected
     * menu element.
     * <p>
     * Note that this method is public but is used by the look and feel engine
     ...
     * Call this method when a choice has been made
     */
    ...
    /**
     * When a MenuElement receives an event from a MouseListener, it should
     * never process the event directly. Instead all MenuElements should call
    ...
    public void processMouseEvent(MouseEvent event) {
        ...
        if ((type == MouseEvent.MOUSE_ENTERED || type == MouseEvent.MOUSE_EXITED) && ((modifiers
                & (InputEvent.BUTTON1_MASK | InputEvent.BUTTON2_MASK | InputEvent.BUTTON3_MASK)) != 0)) {
            return;
        }
        ...
        for (i = selectionSize - 1; i >= 0 && success == false; i--) {
            ...
            for (j = 0, d = subElements.length; j < d && success == false; j++) {
                ...
                /**
                 * Send the event to visible menu element if menu element
                 * currently in the selected path or contains the event location
                 */
                if ((p.x >= 0 && p.x < cWidth && p.y >= 0 && p.y < cHeight)) {
                    ...
                    if (currentSelection[currentSelection.length - 1] != path[i + 1]
                            && (currentSelection.length < 2 || currentSelection[currentSelection.length
                                    - 2] != path[i + 1])) {
                        Component oldMC = currentSelection[currentSelection.length - 1].getComponent();

                        MouseEvent exitEvent = new MouseEvent(oldMC, MouseEvent.MOUSE_EXITED, event.getWhen(),
                        ...
                        currentSelection[currentSelection.length - 1].processMouseEvent(exitEvent, path,
                                this);

                        MouseEvent enterEvent = new MouseEvent(mc, MouseEvent.MOUSE_ENTERED, event.getWhen(),
                                event.getModifiers(), p.x, p.y, event.getXOnScreen(), event.getYOnScreen(),
                                event.getClickCount(), event.isPopupTrigger(), MouseEvent.NOBUTTON);
                        subElements[j].processMouseEvent(enterEvent, path, this);
                    }
                    MouseEvent mouseEvent = new MouseEvent(mc, event.getID(), event.getWhen(), event
     ...
     *         selected path, null is returned.
     */
    public Component componentForPoint(Component source, Point sourcePoint) {
        ...
        for (i = selectionSize - 1; i >= 0; i--) {
            ...
            for (j = 0, d = subElements.length; j < d; j++) {
                ...
                /**
                 * Return the deepest component on the selection path in whose
                 * bounds the event's point occurs
                 */
                if (p.x >= 0 && p.x < cWidth && p.y >= 0 && p.y < cHeight) {
                    return mc;
                }
     ...
     * process the event directly. Instead all MenuElements should call this
    ...
    public void processKeyEvent(KeyEvent e) {
        ...
        for (int i = selSize - 1; i >= 0; i--) {
            ...
            for (int j = 0; j < subs.length; j++) {
                if (subs[j] == null || !subs[j].getComponent().isShowing() || !subs[j].getComponent()
                        .isEnabled()) {
                    continue;
                }
