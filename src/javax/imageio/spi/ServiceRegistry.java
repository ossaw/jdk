 ...
 * The classes in a provider typically implement the interface or subclass the
 * class defined by the service itself.
 *
 * <p>
 * Service providers are stored in one or more <i>categories</i>, each of which
 * is defined by a class of interface (described by a <code>Class</code> object)
 * that all of its members must implement. The set of categories may be changed
 * dynamically.
 *
 * <p>
 * Only a single instance of a given leaf class (that is, the actual class
 * returned by <code>getClass()</code>, as opposed to any inherited classes or
 * interfaces) may be registered. That is, suppose that the
 * <code>com.mycompany.mypkg.GreenServiceProvider</code> class implements the
 * <code>com.mycompany.mypkg.MyService</code> interface. If a
 * <code>GreenServiceProvider</code> instance is registered, it will be stored
 * in the category defined by the <code>MyService</code> class. If a new
 * instance of <code>GreenServiceProvider</code> is registered, it will replace
 * the previous instance. In practice, service provider objects are usually
 * singletons so this behavior is appropriate.
 *
 * <p>
 * To declare a service provider, a <code>services</code> subdirectory is placed
 * within the <code>META-INF</code> directory that is present in every JAR file.
 * This directory contains a file for each service provider interface that has
 * one or more implementation classes present in the JAR file. For example, if
 * the JAR file contained a class named
 * <code>com.mycompany.mypkg.MyServiceImpl</code> which implements the
 * <code>javax.someapi.SomeService</code> interface, the JAR file would contain
 * a file named:
 * 
 * <pre>
 ...
 *
 * <p>
 * The service provider classes should be to be lightweight and quick to load.
 * Implementations of these interfaces should avoid complex dependencies on
 * other classes and on native code. The usual pattern for more complex services
 * is to register a lightweight proxy for the heavyweight service.
 *
 * <p>
 * An application may customize the contents of a registry as it sees fit, so
 * long as it has the appropriate runtime permission.
 *
 * <p>
 * For more details on declaring service providers, and the JAR format in
 * general, see the <a href="../../../../technotes/guides/jar/jar.html"> JAR
...
public class ServiceRegistry {
    ...
    /**
     * Constructs a <code>ServiceRegistry</code> instance with a set of
    ...
    /**
     * Returns an <code>Iterator</code> of <code>Class</code> objects indicating
     * the current set of categories. The iterator will be empty if no
     * categories exist.
     *
    ...
    /**
     * Returns an Iterator containing the subregistries to which the provider
     * belongs.
     */
    ...
    /**
     * Adds a service provider object to the registry. The provider is
     * associated with the given category.
     *
     * <p>
     * If <code>provider</code> implements the <code>RegisterableService</code>
     * interface, its <code>onRegistration</code> method will be called. Its
     * <code>onDeregistration</code> method will be called each time it is
     * deregistered from a category, for example if a category is removed or the
     ...
     * <p>
     * If <code>provider</code> implements the <code>RegisterableService</code>
     * interface, its <code>onRegistration</code> method will be called once for
     * each category it is registered under. Its <code>onDeregistration</code>
     * method will be called each time it is deregistered from a category or
     ...
     * <code>Iterator</code> to the registry. Each provider is associated within
     * each category present in the registry whose <code>Class</code> it
     * implements.
     *
     * <p>
     * For each entry of <code>providers</code> that implements the
     * <code>RegisterableService</code> interface, its
     * <code>onRegistration</code> method will be called once for each category
     * it is registered under. Its <code>onDeregistration</code> method will be
     * called each time it is deregistered from a category or when the registry
     * is finalized.
     *
     * @param providers
    ...
    /**
     * Removes a service provider object from the given category. If the
     * provider was not previously registered, nothing happens and
     * <code>false</code> is returned. Otherwise, <code>true</code> is returned.
     * If an object of the same class as <code>provider</code> but not equal
     * (using <code>==</code>) to <code>provider</code> is registered, it will
     * not be deregistered.
     *
     * <p>
     * If <code>provider</code> implements the <code>RegisterableService</code>
    ...
    /**
     * Returns an <code>Iterator</code> containing all registered service
     * providers in the given category. If <code>useOrdering</code> is
     * <code>false</code>, the iterator will return all of the server provider
     * objects in an arbitrary order. Otherwise, the ordering will respect any
     * pairwise orderings that have been set. If the graph of pairwise orderings
     * contains cycles, any providers that belong to a cycle will not be
     * returned.
     *
     * @param category
     ...
     * <code>ServiceRegistry.getServiceProviders</code> to select providers
     * matching an arbitrary criterion. Classes that implement this interface
     * should be defined in order to make use of the
     * <code>getServiceProviders</code> method of <code>ServiceRegistry</code>
     * that takes a <code>Filter</code>.
     *
     * @see ServiceRegistry#getServiceProviders(Class, ServiceRegistry.Filter,
     *      boolean)
     */
    public interface Filter {

        /**
         * Returns <code>true</code> if the given <code>provider</code> object
    ...
    /**
     * Returns an <code>Iterator</code> containing service provider objects
     * within a given category that satisfy a criterion imposed by the supplied
     * <code>ServiceRegistry.Filter</code> object's <code>filter</code> method.
     *
     * <p>
     * The <code>useOrdering</code> argument controls the ordering of the
     * results using the same rules as
    ...
    /**
     * Returns the currently registered service provider object that is of the
     * given class type. At most one object of a given class is allowed to be
     * registered at any given time. If no registered object has the desired
    ...
    /**
     * Sets a pairwise ordering between two service provider objects within a
     * given category. If one or both objects are not currently registered
     * within the given category, or if the desired ordering is already set,
     * nothing happens and <code>false</code> is returned. If the providers
     * previously were ordered in the reverse direction, that ordering is
     * removed.
     *
     * <p>
     * The ordering will be used by the <code>getServiceProviders</code> methods
     ...
     * Sets a pairwise ordering between two service provider objects within a
     * given category. If one or both objects are not currently registered
     * within the given category, or if no ordering is currently set between
     * them, nothing happens and <code>false</code> is returned.
     *
     * <p>
     * The ordering will be used by the <code>getServiceProviders</code> methods
     * when their <code>useOrdering</code> argument is <code>true</code>.
    ...
    /**
     * Deregisters all service provider object currently registered under the
     * given category.
     *
     * @param category
...
/**
 * A class for wrapping <code>Iterators</code> with a filter function. This
 * provides an iterator for a subset without duplication.
 */
