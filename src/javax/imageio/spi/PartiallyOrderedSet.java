 ...
 * <code>setOrdering</code> and <code>unsetPreference</code> methods. This
 * difference is due to the fact that the relevant ordering between elements is
 * unlikely to be inherent in the elements themselves; rather, it is set
 * dynamically accoring to application policy. For example, in a service
...
class PartiallyOrderedSet extends AbstractSet {
    ...
    /**
     * Returns an iterator over the elements contained in this collection, with
     * an ordering that respects the orderings set by the
     * <code>setOrdering</code> method.
     */
    ...
    /**
     * Sets an ordering between two nodes. When an iterator is requested, the
     * first node will appear earlier in the sequence than the second node. If a
     * prior ordering existed between the nodes in the opposite order, it is
     * removed.
     *
