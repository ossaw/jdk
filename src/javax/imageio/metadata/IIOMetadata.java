 ...
 * (non-image data) associated with images and streams. Plug-ins represent
 * metadata using opaque, plug-in specific objects. These objects, however,
 * provide the ability to access their internal information as a tree of
 * <code>IIOMetadataNode</code> objects that support the XML DOM interfaces as
 * well as additional interfaces for storing non-textual data and retrieving
 * information about legal data values. The format of such trees is plug-in
 * dependent, but plug-ins may choose to support a plug-in neutral format
 * described below. A single plug-in may support multiple metadata formats,
 * whose names maybe determined by calling <code>getMetadataFormatNames</code>.
 * The plug-in may also support a single special format, referred to as the
 * "native" format, which is designed to encode its metadata losslessly. This
 * format will typically be designed specifically to work with a specific file
 * format, so that images may be loaded and saved in the same format with no
 * loss of metadata, but may be less useful for transferring metadata between an
 * <code>ImageReader</code> and an <code>ImageWriter</code> for different image
 * formats. To convert between two native formats as losslessly as the image
 * file formats will allow, an <code>ImageTranscoder</code> object must be used.
 *
...
public abstract class IIOMetadata {
    ...
    /**
     * A boolean indicating whether the concrete subclass supports the standard
     ...
     * representing the native metadata format, initialized to <code>null</code>
     * and set via the constructor.
     */
    protected String nativeMetadataFormatClassName = null;

    /**
     * An array of names of formats, other than the standard and native formats,
     * that are supported by this plug-in, initialized to <code>null</code> and
     ...
     * and representing the metadata formats, other than the standard and native
     * formats, that are supported by this plug-in, initialized to
     * <code>null</code> and set via the constructor.
     */
    protected String[] extraMetadataFormatClassNames = null;

    /**
     * An <code>IIOMetadataController</code> that is suggested for use as the
     * controller for this <code>IIOMetadata</code> object. It may be retrieved
     * via <code>getDefaultController</code>. To install the default controller,
     * call <code>setController(getDefaultController())</code>. This instance
     * variable should be set by subclasses that choose to provide their own
     * default controller, usually a GUI, for setting parameters.
     *
    ...
    /**
     * The <code>IIOMetadataController</code> that will be used to provide
     * settings for this <code>IIOMetadata</code> object when the
     * <code>activateController</code> method is called. This value overrides
     * any default controller, even when <code>null</code>.
     *
     * @see IIOMetadataController
    ...
    /**
     * Constructs an empty <code>IIOMetadata</code> object. The subclass is
     * responsible for supplying values for all protected instance variables
     * that will allow any non-overridden default implementations of methods to
    ...
    /**
     * Returns <code>true</code> if the standard metadata format is supported by
     * <code>getMetadataFormat</code>, <code>getAsTree</code>,
     * <code>setFromTree</code>, and <code>mergeTree</code>.
     * <p>
     ...
     * <code>mergeTree</code>, <code>setFromTree</code>, and <code>reset</code>
     * methods.
     *
     ...
     * typically allows for lossless encoding and transmission of the metadata
     * stored in the format handled by this plug-in. If no such format is
     * supported, <code>null</code>will be returned.
     * <p>
     * The structure and contents of the "native" metadata format are defined by
     * the plug-in that created this <code>IIOMetadata</code> object. Plug-ins
     * for simple formats will usually create a dummy node for the root, and
     * then a series of child nodes representing individual tags, chunks, or
     * keyword/value pairs. A plug-in may choose whether or not to document its
     * native format.
     * <p>
     * The default implementation returns the value of the
    ...
    /**
     * Returns an array of <code>String</code>s containing the names of
     * additional metadata formats, other than the native and standard formats,
     * recognized by this plug-in's <code>getAsTree</code>,
     * <code>setFromTree</code>, and <code>mergeTree</code> methods. If there
     * are no such additional formats, <code>null</code> is returned.
     * <p>
     * The default implementation returns a clone of the
     * <code>extraMetadataFormatNames</code> instance variable.
     *
     * @return an array of <code>String</code>s with length at least 1, or
     *         <code>null</code>.
     * @see #getAsTree
     * @see #setFromTree
     * @see #mergeTree
     ...
     * metadata formats, including the native and standard formats, recognized
     * by this plug-in's <code>getAsTree</code>, <code>setFromTree</code>, and
     * <code>mergeTree</code> methods. If there are no such formats,
     * <code>null</code> is returned.
     * <p>
     * The default implementation calls <code>getNativeMetadataFormatName</code>
     * , <code>isStandardMetadataFormatSupported</code>, and
     * <code>getExtraMetadataFormatNames</code> and returns the combined
     * results.
     *
    ...
    /**
     * Returns an <code>IIOMetadataFormat</code> object describing the given
     * metadata format, or <code>null</code> if no description is available. The
     * supplied name must be one of those returned by
     * <code>getMetadataFormatNames</code> (<i>i.e.</i>, either the native
     * format name, the standard format name, or one of those returned by
     * <code>getExtraMetadataFormatNames</code>).
     * <p>
     * The default implementation checks the name against the global standard
     * metadata format name, and returns that format if it is supported.
     ...
     * the <code>IIOMetadataFormat</code> class from
     * <code>nativeMetadataFormatClassName</code> or
     * <code>extraMetadataFormatClassNames</code> as appropriate, and constructs
    ...
    public IIOMetadataFormat getMetadataFormat(String formatName) {
        ...
        if (standardFormatSupported && formatName.equals(IIOMetadataFormatImpl.standardMetadataFormatName)) {
        ...
        try {
            ...
            try {
                cls = Class.forName(formatClassName, true, loader);
                ...
                loader = (ClassLoader) java.security.AccessController.doPrivileged(
                        new java.security.PrivilegedAction() {
                            public Object run() {
    ...
    /**
     * Returns an XML DOM <code>Node</code> object that represents the root of a
     * tree of metadata contained within this object according to the
    ...
    /**
     * Alters the internal state of this <code>IIOMetadata</code> object from a
     * tree of XML DOM <code>Node</code>s whose syntax is defined by the given
     * metadata format. The previous state is altered only as necessary to
     * accommodate the nodes that are present in the given tree. If the tree
     * structure or contents are invalid, an
     * <code>IIOInvalidTreeException</code> will be thrown.
     * <p>
     * As the semantics of how a tree or subtree may be merged with another tree
     * are completely format-specific, plug-in authors may implement this method
     * in whatever manner is most appropriate for the format, including simply
     ...
     * information of the standard <code>javax_imageio_1.0</code> metadata
     * format, or <code>null</code> if no such information is available. This
     * method is intended to be called by the utility routine
     * <code>getStandardTree</code>.
     * <p>
     * The default implementation returns <code>null</code>.
     * <p>
     * Subclasses should override this method to produce an appropriate subtree
     * if they wish to support the standard metadata format.
     *
    ...
    /**
     * Returns an <code>IIOMetadataNode</code> representing the compression
     * information of the standard <code>javax_imageio_1.0</code> metadata
     * format, or <code>null</code> if no such information is available. This
     * method is intended to be called by the utility routine
     * <code>getStandardTree</code>.
    ...
    /**
     * Returns an <code>IIOMetadataNode</code> representing the data format
     * information of the standard <code>javax_imageio_1.0</code> metadata
     * format, or <code>null</code> if no such information is available. This
     * method is intended to be called by the utility routine
     * <code>getStandardTree</code>.
    ...
    /**
     * Returns an <code>IIOMetadataNode</code> representing the dimension
     * information of the standard <code>javax_imageio_1.0</code> metadata
     * format, or <code>null</code> if no such information is available. This
     * method is intended to be called by the utility routine
     * <code>getStandardTree</code>.
    ...
    /**
     * Appends a new node to an existing node, if the new node is non-
     * <code>null</code>.
     */
    ...
    /**
     * A utility method to return a tree of <code>IIOMetadataNode</code>s
     * representing the metadata contained within this object according to the
     * conventions of the standard <code>javax_imageio_1.0</code> metadata
     * format.
     * <p>
     * This method calls the various <code>getStandard*Node</code> methods to
     * supply each of the subtrees rooted at the children of the root node. If
     * any of those methods returns <code>null</code>, the corresponding subtree
     * will be omitted. If all of them return <code>null</code>, a tree
     * consisting of a single root node will be returned.
     *
     * @return an <code>IIOMetadataNode</code> representing the root of a
     *         metadata tree in the <code>javax_imageio_1.0</code> format.
    ...
    /**
     * Sets the internal state of this <code>IIOMetadata</code> object from a
     * tree of XML DOM <code>Node</code>s whose syntax is defined by the given
     * metadata format. The previous state is discarded. If the tree's structure
     * or contents are invalid, an <code>IIOInvalidTreeException</code> will be
     * thrown.
     * <p>
     * The default implementation calls <code>reset</code> followed by
    ...
    /**
     * Sets the <code>IIOMetadataController</code> to be used to provide
     * settings for this <code>IIOMetadata</code> object when the
     * <code>activateController</code> method is called, overriding any default
     * controller. If the argument is <code>null</code>, no controller will be
     * used, including any default. To restore the default, use
     * <code>setController(getDefaultController())</code>.
     * <p>
     * The default implementation sets the <code>controller</code> instance
     ...
     * installed. This could be the default if there is one, <code>null</code>,
     * or the argument of the most recent call to <code>setController</code>.
     * <p>
     * The default implementation returns the value of the
     * <code>controller</code> instance variable.
     *
     * @return the currently installed <code>IIOMetadataController</code>, or
     *         <code>null</code>.
     * @see IIOMetadataController
     * @see #setController
     * @see #getDefaultController
    ...
    /**
     * Returns the default <code>IIOMetadataController</code>, if there is one,
     * regardless of the currently installed controller. If there is no default
     * controller, returns <code>null</code>.
     * <p>
     * The default implementation returns the value of the
    ...
    /**
     * Returns <code>true</code> if there is a controller installed for this
     * <code>IIOMetadata</code> object.
     * <p>
     * The default implementation returns <code>true</code> if the
     * <code>getController</code> method returns a non-<code>null</code> value.
     *
    ...
    /**
     * Activates the installed <code>IIOMetadataController</code> for this
     * <code>IIOMetadata</code> object and returns the resulting value. When
     * this method returns <code>true</code>, all values for this
     ...
     * subclass of <code>IIOMetadata</code> for a particular plug-in.
     * Controllers need not be GUIs, however.
     * <p>
     * The default implementation calls <code>getController</code> and the calls
     * <code>activate</code> on the returned object if
