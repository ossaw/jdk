 ...
 * <code>IIOMetadataFormat</code> interface. In addition, a static instance
 * representing the standard, plug-in neutral <code>javax_imageio_1.0</code>
 * format is provided by the <code>getStandardFormatInstance</code> method.
 * <p>
 * In order to supply localized descriptions of elements and attributes, a
 * <code>ResourceBundle</code> with a base name of
 * <code>this.getClass().getName() + "Resources"</code> should be supplied via
 * the usual mechanism used by <code>ResourceBundle.getBundle</code>. Briefly,
 * the subclasser supplies one or more additional classes according to a naming
 * convention (by default, the fully-qualified name of the subclass extending
 * <code>IIMetadataFormatImpl</code>, plus the string "Resources", plus the
 * country, language, and variant codes separated by underscores). At run time,
 * calls to <code>getElementDescription</code> or
 * <code>getAttributeDescription</code> will attempt to load such classes
 * dynamically according to the supplied locale, and will use either the element
 * name, or the element name followed by a '/' character followed by the
 * attribute name as a key. This key will be supplied to the
 * <code>ResourceBundle</code>'s <code>getString</code> method, and the
 * resulting localized description of the node or attribute is returned.
 * <p>
 * The subclass may supply a different base name for the resource bundles using
 * the <code>setResourceBaseName</code> method.
 * <p>
 * A subclass may choose its own localization mechanism, if so desired, by
 * overriding the supplied implementations of <code>getElementDescription</code>
 * and <code>getAttributeDescription</code>.
 *
...
public abstract class IIOMetadataFormatImpl implements IIOMetadataFormat {
    ...
    /**
     * A <code>String</code> constant containing the standard format name,
    ...
    /**
     * Constructs a blank <code>IIOMetadataFormatImpl</code> instance, with a
     * given root element name and child policy (other than
     * <code>CHILD_POLICY_REPEAT</code>). Additional elements, and their
     * attributes and <code>Object</code> reference information may be added
    ...
    /**
     * Constructs a blank <code>IIOMetadataFormatImpl</code> instance, with a
     * given root element name and a child policy of
     * <code>CHILD_POLICY_REPEAT</code>. Additional elements, and their
     * attributes and <code>Object</code> reference information may be added
     * using the various <code>add</code> methods.
     *
    ...
    /**
     * Sets a new base name for locating <code>ResourceBundle</code>s containing
     * descriptions of elements and attributes for this format.
     * <p>
     * Prior to the first time this method is called, the base name will be
     * equal to <code>this.getClass().getName() +
     * "Resources"</code>.
     *
    ...
    /**
     * Adds a new element type to this metadata document format with a child
    ...
    /**
     * Adds an existing element to the list of legal children for a given parent
     * node type.
     *
     * @param parentName
    ...
    /**
     * Removes an element from the format. If no element with the given name was
    ...
    /**
     * Adds a new attribute to a previously defined element that may be set to
     * an arbitrary value.
     *
     * @param elementName
    ...
    /**
     * Adds a new attribute to a previously defined element that will be defined
    ...
    /**
     * Adds a new attribute to a previously defined element that will be defined
     * by the enumerated values <code>TRUE</code> and <code>FALSE</code>, with a
    ...
    /**
     * Removes an attribute from a previously defined element. If no attribute
     * with the given name was present in the given element, nothing happens and
    ...
    /**
     * Allows an <code>Object</code> reference of a given class type to be
     * stored in nodes implementing the named element. The value of the
     * <code>Object</code> is unconstrained other than by its class type.
     * <p>
     * If an <code>Object</code> reference was previously allowed, the previous
     ...
     * Allows an <code>Object</code> reference of a given class type to be
     * stored in nodes implementing the named element. The value of the
     * <code>Object</code> must be one of the values given by
     ...
     * Allows an <code>Object</code> reference of a given class type to be
     * stored in nodes implementing the named element. The value of the
     * <code>Object</code> must be within the range given by
     * <code>minValue</code> and <code>maxValue</code>. Furthermore, the class
     * type must implement the <code>Comparable</code> interface.
     * <p>
     * If an <code>Object</code> reference was previously allowed, the previous
     * settings are overwritten.
     *
     ...
     * Allows an <code>Object</code> reference of a given class type to be
     * stored in nodes implementing the named element. The value of the
     * <code>Object</code> must an array of objects of class type given by
     * <code>classType</code>, with at least <code>arrayMinLength</code> and at
     * most <code>arrayMaxLength</code> elements.
     * <p>
     * If an <code>Object</code> reference was previously allowed, the previous
     * settings are overwritten.
     *
    ...
    /**
     * Disallows an <code>Object</code> reference from being stored in nodes
    ...
    private String getResource(String key, Locale locale) {
         ...
         * resource bundles, then the resource bundle will need to be accessed
         * via the applet class loader. So first try the context class loader to
         * locate the resource bundle. If that throws MissingResourceException,
         * then try the system class loader.
         */
     ...
     * <code>ResourceBundle</code> using the current resource base name set by
     * <code>setResourceBaseName</code> and the supplied <code>Locale</code>,
     * using the fallback mechanism described in the comments for
     * <code>ResourceBundle.getBundle</code>. If a <code>ResourceBundle</code>
     * is found, the element name will be used as a key to its
     * <code>getString</code> method, and the result returned. If no
     * <code>ResourceBundle</code> is found, or no such key is present,
     ...
     * using the fallback mechanism described in the comments for
     * <code>ResourceBundle.getBundle</code>. If a <code>ResourceBundle</code>
     * is found, the element name followed by a "/" character followed by the
     * attribute name (<code>elementName + "/" + attrName</code>) will be used
     * as a key to its <code>getString</code> method, and the result returned.
     * If no <code>ResourceBundle</code> is found, or no such key is present,
     * <code>null</code> will be returned.
     * <p>
     * If <code>locale</code> is <code>null</code>, the current default
     * <code>Locale</code> returned by <code>Locale.getLocale</code> will be
    ...
    /**
     * Returns an <code>IIOMetadataFormat</code> object describing the standard,
     * plug-in neutral <code>javax.imageio_1.0</code> metadata document format
     * described in the comment of the <code>javax.imageio.metadata</code>
     * package.
     *
