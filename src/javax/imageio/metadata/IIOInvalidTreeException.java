...
/**
 * An <code>IIOInvalidTreeException</code> is thrown when an attempt by an
 * <code>IIOMetadata</code> object to parse a tree of
 * <code>IIOMetadataNode</code>s fails. The node that led to the parsing error
 * may be stored. As with any parsing error, the actual error may occur at a
 * different point that that where it is detected. The node returned by
 * <code>getOffendingNode</code> should merely be considered as a clue to the
