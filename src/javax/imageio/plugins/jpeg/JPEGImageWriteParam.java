 ...
 * to the stream. If no tables are specified in the metadata, then an
 * abbreviated stream is written. If no tables are included in the metadata and
 * no tables are specified in a <code>JPEGImageWriteParam</code>, then an
 * abbreviated stream is encoded using the "standard" visually lossless tables.
 * This class is necessary for specifying tables when an abbreviated stream must
 * be written without writing any tables to a stream first. In order to use this
 * class, the metadata object passed into the writer must contain no tables, and
 ...
 * tables has been written, only tables in the metadata can override them for
 * subsequent writes, whether to the same stream or a different one. In order to
