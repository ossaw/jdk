 ...
 * The compression scheme specified will be honored if and only if it is
 * compatible with the type of image being written. If the specified compression
 * scheme is not compatible with the type of image being written then the
 * <code>IOException</code> will be thrown by the BMP image writer. If the
 * compression type is not set explicitly then <code>getCompressionType()</code>
 * will return <code>null</code>. In this case the BMP image writer will select
 * a compression type that supports encoding of the given image without loss of
