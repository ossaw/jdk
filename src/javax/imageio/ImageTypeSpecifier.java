 ...
 * <code>SampleModel</code> and <code>ColorModel</code>) to be specified in a
 * convenient manner.
 *
 */
public class ImageTypeSpecifier {
	...
	/**
	 * Cached specifiers for all of the standard <code>BufferedImage</code>
	 * types.
	 */
	...
	static {
		...
		BISpecifier = new ImageTypeSpecifier[BufferedImage.TYPE_BYTE_INDEXED + 1];
	}
	...
	/**
	 * Constructs an <code>ImageTypeSpecifier</code> directly from a
	 * <code>ColorModel</code> and a <code>SampleModel</code>. It is the
	 * caller's responsibility to supply compatible parameters.
	 *
	 * @param colorModel
	 *            a <code>ColorModel</code>.
	 * @param sampleModel
	 *            a <code>SampleModel</code>.
	 *
	 * @exception IllegalArgumentException
	 *                if either parameter is <code>null</code>.
	 * @exception IllegalArgumentException
	 *                if <code>sampleModel</code> is not compatible with
	 ...
	 * <code>RenderedImage</code>. If a <code>BufferedImage</code> is being
	 * used, one of the factory methods <code>createFromRenderedImage</code> or
	 * <code>createFromBufferedImageType</code> should be used instead in order
	...
	static class Packed extends ImageTypeSpecifier {
		...
		public Packed(ColorSpace colorSpace, int redMask, int greenMask, int blueMask,
			...
			int bits = 32;
			this.colorModel = new DirectColorModel(colorSpace, bits, redMask, greenMask, blueMask,
					alphaMask, isAlphaPremultiplied, transferType);
	 ...
	 * <code>DirectColorModel</code> and a packed <code>SampleModel</code> to
	 * store each pixel packed into in a single byte, short, or int.
	 *
	 * @param colorSpace
	 *            the desired <code>ColorSpace</code>.
	 * @param redMask
	 *            a contiguous mask indicated the position of the red channel.
	 * @param greenMask
	 *            a contiguous mask indicated the position of the green channel.
	 * @param blueMask
	 *            a contiguous mask indicated the position of the blue channel.
	 * @param alphaMask
	 *            a contiguous mask indicated the position of the alpha channel.
	 * @param transferType
	 *            the desired <code>SampleModel</code> transfer type.
	 ...
	 * @exception IllegalArgumentException
	 *                if <code>colorSpace</code> is not of type
	 *                <code>TYPE_RGB</code>.
	 * @exception IllegalArgumentException
	 *                if no mask has at least 1 bit set.
	 * @exception IllegalArgumentException
	 *                if <code>transferType</code> if not one of
	 *                <code>DataBuffer.TYPE_BYTE</code>,
	 *                <code>DataBuffer.TYPE_USHORT</code>, or
	 *                <code>DataBuffer.TYPE_INT</code>.
	 */
	public static ImageTypeSpecifier createPacked(ColorSpace colorSpace, int redMask, int greenMask,
			int blueMask, int alphaMask, // 0 if no alpha
			int transferType, boolean isAlphaPremultiplied) {
		return new ImageTypeSpecifier.Packed(colorSpace, redMask, greenMask, blueMask, alphaMask, // 0
																									// if
																									// no
																									// alpha
	...
	static ColorModel createComponentCM(ColorSpace colorSpace, int numBands, int dataType,
		...
		return new ComponentColorModel(colorSpace, numBits, hasAlpha, isAlphaPremultiplied,
	...
	static class Interleaved extends ImageTypeSpecifier {
		...
		public Interleaved(ColorSpace colorSpace, int[] bandOffsets, int dataType, boolean hasAlpha,
				boolean isAlphaPremultiplied) {
			if (colorSpace == null) {
				throw new IllegalArgumentException("colorSpace == null!");
			}
			...
			this.colorModel = ImageTypeSpecifier.createComponentCM(colorSpace, bandOffsets.length,
			...
			int w = 1;
			int h = 1;
			this.sampleModel = new PixelInterleavedSampleModel(dataType, w, h, pixelStride,
					w * pixelStride, bandOffsets);
		}

		public boolean equals(Object o) {
			if ((o == null) || !(o instanceof ImageTypeSpecifier.Interleaved)) {
				return false;
			}

			ImageTypeSpecifier.Interleaved that = (ImageTypeSpecifier.Interleaved) o;

			if ((!(this.colorSpace.equals(that.colorSpace))) || (this.dataType != that.dataType)
		...
		public int hashCode() {
			return (super.hashCode() + (4 * bandOffsets.length) + (25 * dataType)
					+ (hasAlpha ? 17 : 18));
		}
	}
	 ...
	 * <code>PixelInterleavedSampleModel</code> to store each pixel component in
	 * a separate byte, short, or int.
	 *
	 * @param colorSpace
	 *            the desired <code>ColorSpace</code>.
	 * @param bandOffsets
	 *            an array of <code>int</code>s indicating the offsets for each
	 *            band.
	 * @param dataType
	 *            the desired data type, as one of the enumerations from the
	 ...
	 * @exception IllegalArgumentException
	 *                if <code>dataType</code> is not one of the legal
	 *                <code>DataBuffer.TYPE_*</code> constants.
	 * @exception IllegalArgumentException
	 *                if <code>bandOffsets.length</code> does not equal the
	 *                number of color space components, plus 1 if
	 *                <code>hasAlpha</code> is <code>true</code>.
	 */
	...
	static class Banded extends ImageTypeSpecifier {
		...
		public Banded(ColorSpace colorSpace, int[] bankIndices, int[] bandOffsets, int dataType,
				boolean hasAlpha, boolean isAlphaPremultiplied) {
			if (colorSpace == null) {
				throw new IllegalArgumentException("colorSpace == null!");
			}
			...
			this.colorModel = ImageTypeSpecifier.createComponentCM(colorSpace, bankIndices.length,
					dataType, hasAlpha, isAlphaPremultiplied);
		...
		public boolean equals(Object o) {
			if ((o == null) || !(o instanceof ImageTypeSpecifier.Banded)) {
				return false;
			}

			ImageTypeSpecifier.Banded that = (ImageTypeSpecifier.Banded) o;
		...
		public int hashCode() {
			return (super.hashCode() + (3 * bandOffsets.length) + (7 * bankIndices.length)
					+ (21 * dataType) + (hasAlpha ? 19 : 29));
		}
	}

	/**
	 * Returns a specifier for a banded image format that will use a
	 * <code>ComponentColorModel</code> and a <code>BandedSampleModel</code> to
	 * store each channel in a separate array.
	 *
	 * @param colorSpace
	 *            the desired <code>ColorSpace</code>.
	 * @param bankIndices
	 *            an array of <code>int</code>s indicating the bank in which
	 *            each band will be stored.
	 * @param bandOffsets
	 *            an array of <code>int</code>s indicating the starting offset
	 *            of each band within its bank.
	 * @param dataType
	 *            the desired data type, as one of the enumerations from the
	 *            <code>DataBuffer</code> class.
	 * @param hasAlpha
	 ...
	 *                if <code>bandOffsets</code> is <code>null</code>.
	 * @exception IllegalArgumentException
	 *                if the lengths of <code>bankIndices</code> and
	...
	public static ImageTypeSpecifier createBanded(ColorSpace colorSpace, int[] bankIndices,
		...
		return new ImageTypeSpecifier.Banded(colorSpace, bankIndices, bandOffsets, dataType,
	...
	static class Grayscale extends ImageTypeSpecifier {
		...
		public Grayscale(int bits, int dataType, boolean isSigned, boolean hasAlpha,
				boolean isAlphaPremultiplied) {
			...
			if ((bits == 8 && dataType == DataBuffer.TYPE_BYTE) || (bits == 16
				...
				this.colorModel = new ComponentColorModel(colorSpace, nBits, hasAlpha,
				...
				int w = 1;
				int h = 1;
				this.sampleModel = new PixelInterleavedSampleModel(dataType, w, h, numBands,
						w * numBands, bandOffsets);
			} else {
	...
	/**
	 * Returns a specifier for a grayscale image format that will pack pixels of
	 * the given bit depth into array elements of the specified data type.
	 *
	 * @param bits
	 ...
	 *
	 * @exception IllegalArgumentException
	 *                if <code>bits</code> is not one of 1, 2, 4, 8, or 16.
	 * @exception IllegalArgumentException
	 ...
	 * @exception IllegalArgumentException
	 *                if <code>bits</code> is larger than the bit size of the
	 *                given <code>dataType</code>.
	 */
	...
	/**
	 * Returns a specifier for a grayscale plus alpha image format that will
	 * pack pixels of the given bit depth into array elements of the specified
	 * data type.
	 *
	 * @param bits
	 *            the number of bits per gray value (1, 2, 4, 8, or 16).
	 ...
	 *            <code>true</code> if negative values are to be represented.
	 * @param isAlphaPremultiplied
	 *            <code>true</code> if the luminance channel will be
	 *            premultipled by the alpha channel.
	 *
	 * @return an <code>ImageTypeSpecifier</code> with the desired
	 *         characteristics.
	 *
	...
	public static ImageTypeSpecifier createGrayscale(int bits, int dataType, boolean isSigned,
			boolean isAlphaPremultiplied) {
		return new ImageTypeSpecifier.Grayscale(bits, dataType, isSigned, true,
				isAlphaPremultiplied);
	}
	...
	static class Indexed extends ImageTypeSpecifier {
		...
		public Indexed(byte[] redLUT, byte[] greenLUT, byte[] blueLUT, byte[] alphaLUT, int bits,
				int dataType) {
			...
			int len = 1 << bits;
			if (redLUT.length != len || greenLUT.length != len || blueLUT.length != len
			...
			if (alphaLUT == null) {
				this.colorModel = new IndexColorModel(bits, redLUT.length, redLUT, greenLUT,
						blueLUT);
			} else {
				this.colorModel = new IndexColorModel(bits, redLUT.length, redLUT, greenLUT,
						blueLUT, alphaLUT);
			...
			if ((bits == 8 && dataType == DataBuffer.TYPE_BYTE) || (bits == 16
					&& (dataType == DataBuffer.TYPE_SHORT || dataType == DataBuffer.TYPE_USHORT))) {
				int[] bandOffsets = { 0 };
				this.sampleModel = new PixelInterleavedSampleModel(dataType, 1, 1, 1, 1,
						bandOffsets);
			} else {
				this.sampleModel = new MultiPixelPackedSampleModel(dataType, 1, 1, bits);
			}
	 ...
	 * index values of the given bit depth into array elements of the specified
	 * data type.
	 *
	 * @param redLUT
	 *            an array of <code>byte</code>s containing the red values for
	 *            each index.
	 * @param greenLUT
	 *            an array of <code>byte</code>s containing * the green values
	 *            for each index.
	 * @param blueLUT
	 *            an array of <code>byte</code>s containing the blue values for
	 *            each index.
	 * @param alphaLUT
	 *            an array of <code>byte</code>s containing the alpha values for
	 *            each index, or <code>null</code> to create a fully opaque LUT.
	 * @param bits
	 *            the number of bits in each index.
	 * @param dataType
	 *            the desired output type, as one of the enumerations from the
	 *            <code>DataBuffer</code> class.
	 *
	 * @return an <code>ImageTypeSpecifier</code> with the desired
	 *         characteristics.
	 *
	 * @exception IllegalArgumentException
	 *                if <code>redLUT</code> is <code>null</code>.
	 * @exception IllegalArgumentException
	 *                if <code>greenLUT</code> is <code>null</code>.
	 * @exception IllegalArgumentException
	 *                if <code>blueLUT</code> is <code>null</code>.
	 * @exception IllegalArgumentException
	 *                if <code>bits</code> is not one of 1, 2, 4, 8, or 16.
	 * @exception IllegalArgumentException
	 *                if the non-<code>null</code> LUT parameters do not have
	...
	/**
	 * Returns an <code>ImageTypeSpecifier</code> that encodes one of the
	 * standard <code>BufferedImage</code> types (other than
	 ...
	 *
	 * @exception IllegalArgumentException
	 *                if <code>bufferedImageType</code> is not one of the
	 *                standard types, or is equal to <code>TYPE_CUSTOM</code>.
	 *
	...
	/**
	 * Returns an <code>ImageTypeSpecifier</code> that encodes the layout of a
	 * <code>RenderedImage</code> (which may be a <code>BufferedImage</code>).
	 *
	 * @param image
	 *            a <code>RenderedImage</code>.
	 *
	 ...
	 *
	 * @return an <code>int</code> representing a <code>BufferedImage</code>
	 *         type.
	 *
	 * @see java.awt.image.BufferedImage
	...
	/**
	 * Return the number of color components specified by this object. This is
	 * the same value as returned by <code>ColorModel.getNumComponents</code>
	 *
	...
	/**
	 * Return the number of bands specified by this object. This is the same
	 * value as returned by <code>SampleModel.getNumBands</code>
	 *
	 ...
	 *
	 * @param band
	 *            the index of the band to be queried, as an int.
	 *
	 * @return an int specifying a number of bits.
	 *
	 * @exception IllegalArgumentException
	 *                if <code>band</code> is negative or greater than the
	 *                largest band index.
	 */
	...
	/**
	 * Returns a <code>SampleModel</code> based on the settings encapsulated
	 * within this object. The width and height of the <code>SampleModel</code>
	 ...
	 *
	 * @param width
	 *            the desired width of the returned <code>SampleModel</code>.
	 * @param height
	 *            the desired height of the returned <code>SampleModel</code>.
	 *
	 * @return a <code>SampleModel</code> with the given dimensions.
	 *
	 * @exception IllegalArgumentException
	 *                if either <code>width</code> or <code>height</code> are
	 *                negative or zero.
	 * @exception IllegalArgumentException
	...
	/**
	 * Creates a <code>BufferedImage</code> with a given width and height
	 * according to the specification embodied in this object.
	 *
	 * @param width
	 *            the desired width of the returned <code>BufferedImage</code>.
	 * @param height
	 *            the desired height of the returned <code>BufferedImage</code>.
	 *
	...
	/**
	 * Returns <code>true</code> if the given <code>Object</code> is an
	 * <code>ImageTypeSpecifier</code> and has a <code>SampleModel</code> and
	 * <code>ColorModel</code> that are equal to those of this object.
	 *
	 * @param o
	 *            the <code>Object</code> to be compared for equality.
	 *
	...
	private static ImageTypeSpecifier createSpecifier(int type) {
		...
		case BufferedImage.TYPE_INT_RGB:
			return createPacked(sRGB, 0x00ff0000, 0x0000ff00, 0x000000ff, 0x0, DataBuffer.TYPE_INT,
					false);

		case BufferedImage.TYPE_INT_ARGB:
			return createPacked(sRGB, 0x00ff0000, 0x0000ff00, 0x000000ff, 0xff000000,
		...
		case BufferedImage.TYPE_INT_BGR:
			return createPacked(sRGB, 0x000000ff, 0x0000ff00, 0x00ff0000, 0x0, DataBuffer.TYPE_INT,
					false);

		case BufferedImage.TYPE_3BYTE_BGR:
			return createInterleaved(sRGB, new int[] { 2, 1, 0 }, DataBuffer.TYPE_BYTE, false,
					false);

		case BufferedImage.TYPE_4BYTE_ABGR:
			return createInterleaved(sRGB, new int[] { 3, 2, 1, 0 }, DataBuffer.TYPE_BYTE, true,
					false);
		...
		case BufferedImage.TYPE_USHORT_565_RGB:
			return createPacked(sRGB, 0xF800, 0x07E0, 0x001F, 0x0, DataBuffer.TYPE_USHORT, false);

		case BufferedImage.TYPE_USHORT_555_RGB:
			return createPacked(sRGB, 0x7C00, 0x03E0, 0x001F, 0x0, DataBuffer.TYPE_USHORT, false);
