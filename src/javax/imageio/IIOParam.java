 ...
 * destination. When writing, these are reversed. In the case of writing,
 * destination regions may be used only with a writer that supports pixel
 * replacement.
 * <p>
 * Decimation subsampling may be specified for both readers and writers, using a
...
public abstract class IIOParam {
	...
	/**
	 * The decimation subsampling to be applied in the horizontal direction. By
	 * default, the value is <code>1</code>. The value must not be negative or
	 * 0.
	 */
	protected int sourceXSubsampling = 1;

	/**
	 * The decimation subsampling to be applied in the vertical direction. By
	 * default, the value is <code>1</code>. The value must not be negative or
	 * 0.
	 */
	 ...
	 * subsampling. The first pixel to be used will be offset this amount from
	 * the origin of the region, or of the image if no region is specified.
	 */
	...
	/**
	 * An array of <code>int</code>s indicating which source bands will be used,
	 * or <code>null</code>. If <code>null</code>, the set of source bands to be
	 * used is as described in the comment for the <code>setSourceBands</code>
	 * method. No value should be allowed to be negative.
	 */
	protected int[] sourceBands = null;

	/**
	 * An <code>ImageTypeSpecifier</code> to be used to generate a destination
	 * image when reading, or to set the output color type when writing. If non
	 * has been set the value will be <code>null</code>. By default, the value
	 * is <code>null</code>.
	 */
	protected ImageTypeSpecifier destinationType = null;

	/**
	 * The offset in the destination where the upper-left decoded pixel should
	 * be placed. By default, the value is (0, 0).
	 */
	protected Point destinationOffset = new Point(0, 0);

	/**
	 * The default <code>IIOParamController</code> that will be used to provide
	 * settings for this <code>IIOParam</code> object when the
	 * <code>activateController</code> method is called. This default should be
	 * set by subclasses that choose to provide their own default controller,
	...
	/**
	 * The <code>IIOParamController</code> that will be used to provide settings
	 * for this <code>IIOParam</code> object when the
	 * <code>activateController</code> method is called. This value overrides
	 * any default controller, even when null.
	 *
	 * @see IIOParamController
	 ...
	 * (0, 0) and increasing values down and to the right. The actual number of
	 * pixels used will depend on the subsampling factors set by
	...
	/**
	 * Returns the source region to be used. The returned value is that set by
	 * the most recent call to <code>setSourceRegion</code>, and will be
	 * <code>null</code> if there is no region set.
	 *
	 ...
	 * <code>sourceXSubsampling</code> and <code>sourceYSubsampling</code>
	 * parameters specify the subsampling period (<i>i.e.</i>, the number of
	 * rows and columns to advance after every source pixel). Specifically, a
	 * period of 1 will use every row or column; a period of 2 will use every
	 * other row or column. The <code>subsamplingXOffset</code> and
	 * <code>subsamplingYOffset</code> parameters specify an offset from the
	 * region (or image) origin for the first subsampled pixel. Adjusting the
	 * origin of the subsample grid is useful for avoiding seams when
	 ...
	 * subsampled tiles of a large image, where the tile width and height are
	 * not multiples of the subsampling periods. If the subsampling grid does
	 * not remain consistent from tile to tile, there will be artifacts at the
	 * tile boundaries. By adjusting the subsampling grid offset for each tile
	 * to compensate, these artifacts can be avoided. The tradeoff is that in
	 ...
	 * available source band index has been specified or if the number of source
	...
	/**
	 * Returns the set of of source bands to be used. The returned value is that
	 ...
	 * will return a new <code>BufferedImage</code> using the format specified
	 * by the supplied type specifier. As a side effect, any destination
	 ...
	 * and may be <code>null</code>. For example, a 4-banded image could
	 * represent either CMYK or RGBA data. If a destination type is set, its
	 * <code>ColorModel</code> will override any <code>ColorModel</code> on the
	 * image itself. This is crucial when <code>setSourceBands</code> is used
	 * since the image's <code>ColorModel</code> will refer to the entire image
	...
	/**
	 * Returns the type of image to be returned by the read, if one was set by a
	 * call to <code>setDestination(ImageTypeSpecifier)</code>, as an
	 * <code>ImageTypeSpecifier</code>. If none was set, <code>null</code> is
	 * returned.
	 *
	...
	/**
	 * Sets the <code>IIOParamController</code> to be used to provide settings
	 * for this <code>IIOParam</code> object when the
	 ...
	 * This could be the default if there is one, <code>null</code>, or the
	...
	/**
	 * Returns the default <code>IIOParamController</code>, if there is one,
	 ...
	 * <code>IIOParam</code> object. This will return <code>true</code> if
	...
	/**
	 * Activates the installed <code>IIOParamController</code> for this
	 * <code>IIOParam</code> object and returns the resulting value. When this
	 * method returns <code>true</code>, all values for this
