...
public interface IIOReadUpdateListener extends EventListener {
	...
	/**
	 * Reports that the current read operation is about to begin a progressive
	 ...
	 * <code>height</code> parameters. If the pass is interlaced, that is, it
	 * only updates selected rows or columns, the <code>periodX</code> and
	 * <code>periodY</code> parameters will indicate the degree of subsampling.
	 * The set of bands that may be affected is indicated by the value of
	 * <code>bands</code>.
	 *
	 * @param source
	 ...
	 * @param pass
	 *            the number of the pass that is about to begin, starting with
	 *            0.
	 * @param minPass
	 ...
	 * @param minX
	 *            the X coordinate of the leftmost updated column of pixels.
	 * @param minY
	 *            the Y coordinate of the uppermost updated row of pixels.
	 * @param periodX
	 *            the horizontal spacing between updated pixels; a value of 1
	 *            means no gaps.
	 * @param periodY
	 *            the vertical spacing between updated pixels; a value of 1
	 *            means no gaps.
	 * @param bands
	 *            an array of <code>int</code>s indicating the the set bands
	 *            that may be updated.
	 */
	void passStarted(ImageReader source, BufferedImage theImage, int pass, int minPass, int maxPass,
	 ...
	 * top-to-bottom, left-to-right-order, but others may use multiple passes of
	 * interlacing, tiling, etc. The sequence of updates may even differ from
	 * call to call depending on network speeds, for example. A call to this
	 ...
	 * the image has been updated since the last update. A region is described
	 * by its spatial bounding box (<code>minX</code>, <code>minY</code>,
	 * <code>width</code>, and <code>height</code>); X and Y subsampling factors
	 * (<code>periodX</code> and <code>periodY</code>); and a set of updated
	 * bands (<code>bands</code>). For example, the update:
	 *
	 * <pre>
	 * minX = 10
	 * minY = 20
	 ...
	 *            means no gaps.
	 * @param bands
	 *            an array of <code>int</code>s indicating which bands are being
	 *            updated.
	 */
	 ...
	 * Readers of formats that support progressive encoding should use this to
	 * notify clients when each pass is completed when reading a progressively
	 * encoded image.
	 *
	 * @param source
	 *            the <code>ImageReader</code> object calling this method.
	...
	/**
	 * Reports that the current thumbnail read operation is about to begin a
	 * progressive pass. Readers of formats that support progressive encoding
	 * should use this to notify clients when each pass is completed when
