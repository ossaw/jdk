 ...
 * <code>MemoryCacheImageOutputStream</code>. This class keeps an
 * <code>ArrayList</code> of 8K blocks, loaded sequentially. Blocks may only be
 * disposed of from the index 0 forward. As blocks are freed, the corresponding
 * entries in the array list are set to <code>null</code>, but no compacting is
 * performed. This allows the index for each block to never change, and the
...
class MemoryCache {
	...
	/**
	 * Writes out a portion of the cache to an <code>OutputStream</code>. This
	 * method preserves no state about the output stream, and does not dispose
	 * of any blocks containing bytes written. To dispose blocks, use
	 * {@link #disposeBefore <code>disposeBefore()</code>}.
	 *
	 * @exception IndexOutOfBoundsException
	 *                if any portion of the requested data is not in the cache
	 *                (including if <code>pos</code> is in a block already
	 *                disposed), or if either <code>pos</code> or
	 *                <code>len</code> is < 0.
	 */
	...
	/**
	 * Overwrites and/or appends the cache from a byte array. The length of the
	 ...
	 * @exception IndexOutOfBoundsException
	 *                if <code>off</code>, <code>len</code>, or <code>pos</code>
	 *                are negative, or if <code>off+len > b.length</code>.
	 */
	...
	/**
	 * Overwrites or appends a single byte to the cache. The length of the cache
	...
	/**
	 * Returns the total length of data that has been cached, regardless of
	 * whether any early blocks have been disposed. This value will only ever
	 * increase.
	 */
	...
	/**
	 * Returns the single byte at the given position, as an <code>int</code>.
	 * Returns -1 if this position has not been cached or has been disposed.
	 */
	...
	/**
	 * Copy <code>len</code> bytes from the cache, starting at cache position
	 * <code>pos</code>, into the array <code>b</code> at offset
	 * <code>off</code>.
	 *
	 * @exception NullPointerException
	 ...
	 * @exception IndexOutOfBoundsException
	 *                if <code>off</code>, <code>len</code> or <code>pos</code>
	 *                are negative or if <code>off + len > b.length</code> or if
	 *                any portion of the requested data is not in the cache
	 *                (including if <code>pos</code> is in a block that has
	...
	/**
	 * Free the blocks up to the position <code>pos</code>. The byte at
	 * <code>pos</code> remains available.
	 *
	 * @exception IndexOutOfBoundsException
	 *                if <code>pos</code> is in a block that has already been
	 *                disposed.
	 */
	...
	/**
	 * Erase the entire cache contents and reset the length to 0. The cache
	 * object may subsequently be reused as though it had just been allocated.
	 */
	public void reset() {
		cache.clear();
		cacheStart = 0;
