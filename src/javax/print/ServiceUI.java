 ...
 * accessible to the user through a vendor supplied tab panel Component. Such a
 * vendor extension is encouraged to use Swing! and to support its accessibility
 * APIs. The vendor extensions should return the settings as part of the
 * AttributeSet. Applications which want to preserve the user settings should
 * use those settings to specify the print job. Note that this class is not
 * referenced by any other part of the Java Print Service and may not be
 * included in profiles which cannot depend on the presence of the AWT packages.
 */
...
public class ServiceUI {
     ...
     * the PrintService's ServiceUIFactory when the PrintService is browsed. The
     * dialog will attempt to locate a MAIN_UIROLE first as a JComponent, then
     * as a Panel. If there is no ServiceUIFactory or no matching role the
     * custom tab will be empty or not visible.
     * <p>
     ...
     * array must be non-null and non-empty. Typically an application will pass
     * in only PrintServices capable of printing a particular document flavor.
     * <p>
     * An application may pass in a PrintService to be initially displayed. A
     * non-null parameter must be included in the array of browsable services.
     ...
     * is non-null choices presented to the user can be better validated against
     * those supported by the services. An application must pass in a
     * PrintRequestAttributeSet for returning user choices. On calling the
     * PrintRequestAttributeSet may be empty, or may contain
    ...
    public static PrintService printDialog(GraphicsConfiguration gc, int x, int y, PrintService[] services,
        ...
        if (owner instanceof Frame) {
            dialog = new ServiceDialog(gc, x + gcBounds.x, y + gcBounds.y, services, defaultIndex, flavor,
                    attributes, (Frame) owner);
        } else {
            dialog = new ServiceDialog(gc, x + gcBounds.x, y + gcBounds.y, services, defaultIndex, flavor,
                    attributes, (Dialog) owner);
        }
    ...
    /**
     * POSSIBLE FUTURE API: This method may be used down the road if we decide
     * to allow developers to explicitly display a "page setup" dialog.
     * Currently we use that functionality internally for the AWT print model.
     */
    /*
     * public static void pageDialog(GraphicsConfiguration gc, int x, int y,
     * PrintService service, DocFlavor flavor, PrintRequestAttributeSet
     * attributes) throws HeadlessException { if
     * (GraphicsEnvironment.isHeadless()) { throw new HeadlessException(); }
