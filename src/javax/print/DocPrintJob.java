...
public interface DocPrintJob {
	 ...
	 * {@link #getAttributes()} method call; that is, the returned attribute
	 ...
	 * values, call <code>getAttributes()</code> again and compare the new
	 * attribute set to the previous attribute set; alternatively, register a
	 * listener for print job events. The returned value may be an empty set but
	 * should not be null.
	 * 
	 ...
	 * listener is already registered, it will be registered again.
	 * 
	 ...
	 * was not previously added to this component. If listener is null, no
	 * exception is thrown and no action is performed. If a listener was
	 ...
	 * is null, no exception is thrown and no action is performed. To determine
	 * the attribute updates that may be reported by this job, a client can call
	 * <code>getAttributes()</code> and identify the subset that are interesting
	 * and likely to be reported to the listener. Clients expecting to be
	 * updated about changes in a specific job attribute should verify it is in
	 * that set, but updates about an attribute will be made only if it changes
	 * and this is detected by the job. Also updates may be subject to batching
	 ...
	 * reported to the listener. If the attribute set is null, then this means
	 * to listen on all dynamic attributes that the job supports. This may
	 ...
	 * the argument was not previously added to this component. If the listener
	 * is null, no exception is thrown and no action is performed. If a listener
	 * is registered more than once, even for a different set of attributes, no
	 * guarantee is made which listener is removed.
	 * 
	 ...
	 * <p>
	 * Print service implementors should close any print data streams (ie Reader
	 * or InputStream implementations) that they obtain from the client doc.
	 * Robust clients may still wish to verify this. An exception is always
