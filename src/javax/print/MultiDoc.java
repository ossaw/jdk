 ...
 * implements interface MultiDoc, and the Print Job calls methods on that object
 ...
 * and save the doc in a list. Get the next multidoc from the current multidoc,
 * and repeat until there are no more. Then iterate over the list of saved docs.
 * Get the print data representation object from the current doc. Get all the
 * print data from the print data representation object. Go to the next doc in
 * the list, and repeat until there are no more.
 * </OL>
 ...
 * proxy will call {@link #getDoc() getDoc()} one or more times until it
 * successfully obtains the current Doc object. The print service proxy will
 * then obtain the current doc's print data, not proceeding until all the print
 * data is obtained or an unrecoverable error occurs. If it is able to continue,
 * the print service proxy will then call {@link #next() next()} one or more
 * times until it successfully obtains either the next MultiDoc object or an
 * indication that there are no more. An implementation of interface MultiDoc
 * can assume the print service proxy will follow this interleaved pattern; for
 * any other pattern of usage, the MultiDoc implementation's behavior is
 * unspecified.
 * <P>
