...
public interface PrintService {
	 ...
	 * 
	 * @param listener
	 *            a PrintServiceAttributeListener, which monitors the status of
	 *            a print service
	 ...
	 * the listener is no longer interested in <code>PrintService</code> events.
	 * 
	 * @param listener
	 ...
	 * <CODE>getAttributes()</CODE> method call: that is, the returned attribute
	 * set's contents will <I>not</I> be updated if this print service's
	 * attribute set's contents change in the future. To detect changes in
	 * attribute values, call <CODE>getAttributes()</CODE> again and compare the
	 * new attribute set to the previous attribute set; alternatively, register
	 * a listener for print service events.
	 *
	 ...
	 * 
	 * @param category
	 *            the category of a PrintServiceAttribute supported by this
	 *            service - may not be null.
	 * @return the value of the supported attribute or null if the attribute is
	 ...
	 * job for this <code>PrintService</code>. A print data format is designated
	 * by a "doc flavor" (class {@link javax.print.DocFlavor DocFlavor})
	 ...
	 * attributes. Use <code>getUnsupportedAttributes(..)</code> to validate
	 ...
	 * <code>DocFlavor</code>. This is a convenience method to determine if the
	 * <code>DocFlavor</code> would be a member of the result of
	 ...
	 *
	 * @param flavor
	 *            the <code>DocFlavor</code>to query for support.
	 * @return <code>true</code> if this print service supports the specified
	 *         <code>DocFlavor</code>; <code>false</code> otherwise.
	 * @exception NullPointerException
	 *                (unchecked exception) Thrown if <CODE>flavor</CODE> is
	 *                null.
	 */
	 ...
	 * <code>DocFlavor</code> to validate the request before submitting it, such
	 * as <code>getSupportedAttributeValues(..)</code>.
	 *
	 ...
	 *         Each element in the array is a {@link java.lang.Class Class} that
	 *         implements interface {@link javax.print.attribute.Attribute
	 *         Attribute}. The array is empty if no categories are supported.
	 */
	 ...
	 * interface {@link javax.print.attribute.Attribute Attribute}. This method
	 ...
	 * particular <code>DocFlavor</code>). Use one of the methods which include
	 * a <code>DocFlavor</code> to validate the request before submitting it,
	 * such as <code>getSupportedAttributeValues(..)</code>.
	 * <P>
	 * This is a convenience method to determine if the category would be a
	 * member of the result of <code>getSupportedAttributeCategories()</code>.
	 *
	 * @param category
	 ...
	 *
	 * @return <code>true</code> if this print service supports specifying a
	 *         doc-level or job-level attribute in <CODE>category</CODE> in a
	 *         Print Request; <code>false</code> if it doesn't.
	 *
	 * @exception NullPointerException
	 *                (unchecked exception) Thrown if <CODE>category</CODE> is
	 *                null.
	 * @exception IllegalArgumentException
	 *                (unchecked exception) Thrown if <CODE>category</CODE> is
	 *                not a <code>Class</code> that implements interface
	 *                {@link javax.print.attribute.Attribute Attribute}.
	 */
	 ...
	 * implements interface {@link javax.print.attribute.Attribute Attribute}.
	 ...
	 * have a defaultvalue for <code>RequestingUser</code> i.e. a null return
	 * for a supported category means there is no service default value for that
	 * category. Use the <code>isAttributeCategorySupported(Class)</code> method
	 * to distinguish these cases.
	 *
	 * @param category
	 *            Printing attribute category for which the default attribute
	 *            value is requested. It must be a {@link java.lang.Class Class}
	 *            that implements interface
	 *            {@link javax.print.attribute.Attribute Attribute}.
	 *
	 * @return Default attribute value for <CODE>category</CODE>, or null if
	 *         this Print Service does not support specifying a doc-level or
	 *         job-level attribute in <CODE>category</CODE> in a Print Request,
	 *         or the service does not have a default value for this attribute.
	 *
	 * @exception NullPointerException
	 *                (unchecked exception) Thrown if <CODE>category</CODE> is
	 *                null.
	 * @exception IllegalArgumentException
	 *                (unchecked exception) Thrown if <CODE>category</CODE> is
	 *                not a {@link java.lang.Class Class} that implements
	 *                interface {@link javax.print.attribute.Attribute
	 *                Attribute}.
	 */
	 ...
	 * {@link javax.print.attribute.Attribute Attribute}.
	 * <P>
	 * If <CODE>flavor</CODE> is null and <CODE>attributes</CODE> is null or is
	 * an empty set, this method returns all the printing attribute values this
	 * Print Service supports for any possible job. If <CODE>flavor</CODE> is
	 * not null or <CODE>attributes</CODE> is not an empty set, this method
	 * returns just the printing attribute values that are compatible with the
	 * given doc flavor and/or set of attributes. That is, a null return value
	 * may indicate that specifying this attribute is incompatible with the
	 * specified DocFlavor. Also if DocFlavor is not null it must be a flavor
	 * supported by this PrintService, else IllegalArgumentException will be
	 * thrown.
	 * <P>
	 ...
	 * <code>Doc</code> must be included in this set to accurately represent the
	 * context.
	 * <p>
	 ...
	 * documentation for each printing attribute in package
	 ...
	 * an arbitrary text string. (The value of the returned attribute object is
	 * irrelevant.)
	 ...
	 * list of enumerated values. The type of the array is an array of the
	 ...
	 * category) that indicates bounds on the legal values -- used, for example,
	 * by an integer-valued attribute that must lie within a certain range.
	 * </UL>
	 * <P>
	 *
	 * @param category
	 *            Printing attribute category to test. It must be a
	 *            {@link java.lang.Class Class} that implements interface
	 *            {@link javax.print.attribute.Attribute Attribute}.
	 * @param flavor
	 *            Doc flavor for a supposed job, or null.
	 * @param attributes
	 *            Set of printing attributes for a supposed job (both job-level
	 *            attributes and document-level attributes), or null.
	 *
	 * @return Object indicating supported values for <CODE>category</CODE>, or
	 ...
	 *                not a {@link java.lang.Class Class} that implements
	 *                interface {@link javax.print.attribute.Attribute
	 *                Attribute}, or <code>DocFlavor</code> is not supported by
	 *                this service.
	 */
	 ...
	 * <P>
	 * If <CODE>flavor</CODE> is null and <CODE>attributes</CODE> is null or is
	 * an empty set, this method tells whether this Print Service supports the
	 * given printing attribute value for some possible combination of doc
	 ...
	 * of the result of <code>getSupportedAttributeValues(...)</code>.
	 *
	 * @param attrval
	 ...
	 * potential print job and identifying the specific attributes which cannot
	 * be supported. It is important to supply only a supported DocFlavor or an
	 * IllegalArgumentException will be thrown. If the return value from this
	 * method is null, all attributes are supported.
	 * <p>
	 * <code>DocAttribute</code>s which are to be specified on the
	 * <code>Doc</code> must be included in this set to accurately represent the
	 * context.
	 ...
	 * attribute categories that are unsupported from unsupported attribute
	 * values.
	 * <p>
	 ...
	 * <p>
	 * Use <code>isDocFlavorSupported()</code> to verify that a DocFlavor is
	 ...
	 *
	 * @exception IllegalArgumentException
	 *                if<CODE>flavor</CODE> is not supported by this
	 *                PrintService.
	 */
	 ...
	 * return null. Print Services which do provide UI but want to be supported
	 * in an environment with no UI support should ensure that the factory is
	 * not initialised unless the application calls this method to obtain the
	 * factory. See <code>ServiceUIFactory</code> for more information.
	 * 
	 ...
	 * <p>
	 * Services must implement this method and return true only if the service
	 * objects being compared may be used interchangeably by the client.
	 ...
	 * @return true if this service is the same as the obj argument, false
	 *         otherwise.
	 */
