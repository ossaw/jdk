 ...
 * of print data for a Print Job. "Doc" is a short, easy-to-pronounce term that
 * means "a piece of print data." The client passes to the Print Job an object
 * that implements interface Doc, and the Print Job calls methods on that object
 * to obtain the print data. The Doc interface lets a Print Job:
 * <UL>
 * <LI>Determine the format, or "doc flavor" (class {@link DocFlavor DocFlavor}
 * ), in which the print data is available. A doc flavor designates the print
 * data format (a MIME type) and the representation class of the object from
 * which the print data comes.
 * <P>
 * <LI>Obtain the print data representation object, which is an instance of the
 * doc flavor's representation class. The Print Job can then obtain the actual
 * print data from the representation object.
 * <P>
 * <LI>Obtain the printing attributes that specify additional characteristics of
 ...
 * javax.print.attribute}. The doc returns its printing attributes stored in an
 ...
 * return the same object each time the method is called. This has implications
 ...
 * <code>getReaderForText()</code> or <code>getStreamForBytes()</code> methods,
 * or because the print data source is already an InputStream or Reader, then
 * the print service should always close these streams for the client on all job
 * completion conditions. With the following caveat. If the print data is itself
 * a stream, the service will always close it. If the print data is otherwise
 * something that can be requested as a stream, the service will only close the
 * stream if it has obtained the stream before terminating. That is, just
 * because a print service might request data as a stream does not mean that it
 * will, with the implications that Doc implementors which rely on the service
 * to close them should create such streams only in response to a request from
 * the service.
 * <P>
 * <HR>
 */
