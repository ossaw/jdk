 ...
 * must be able to describe the located printers as instances of a PrintService.
 * Typically implementations of this service class are located automatically in
 * JAR files (see the SPI JAR file specification). These classes must be
 * instantiable using a default constructor. Alternatively applications may
 ...
 * Following this recommended policy means that untrusted code may not be able
 * to locate any print services. Downloaded applets are the most common example
 * of untrusted code.
 * <p>
 ...
 * <p>
 * Services which are registered by registerService(PrintService) will not be
 * included in lookup results if a security manager is installed and its
 * checkPrintJobAccess() method denies access.
 */
...
public abstract class PrintServiceLookup {
     ...
     *
     * @param flavor
     *                   the flavor to print. If null, this constraint is not
     *                   used.
     * @param attributes
     *                   attributes that the print service must support. If null
     *                   this
     *                   constraint is not used.
     *
     * @return array of matching <code>PrintService</code> objects representing
     *         print services that support the specified flavor attributes. If
     *         no services match, the array is zero-length.
     */
    ...
    /**
     * Locates MultiDoc print Services capable of printing MultiDocs containing
     * all the specified doc flavors.
     * <P>
     * This method is useful to help locate a service that can print a
     * <code>MultiDoc</code> in which the elements may be different flavors. An
     * application could perform this itself by multiple lookups on each
     * <code>DocFlavor</code> in turn and collating the results, but the lookup
     * service may be able to do this more efficiently.
     *
     * @param flavors
     *                   the flavors to print. If null or empty this constraint
     *                   is not
     *                   used. Otherwise return only multidoc print services
     *                   that can
    ...
    public static final MultiDocPrintService[] lookupMultiDocPrintServices(
        ...
        return (MultiDocPrintService[]) list.toArray(
                new MultiDocPrintService[list.size()]);
    }

    /**
     * Locates the default print service for this environment. This may return
     * null. If multiple lookup services each specify a default, the chosen
     * service is not precisely defined, but a platform native service, rather
     * than an installed service, is usually returned as the default. If there
     * is no clearly identifiable platform native default print service, the
     * default is the first to be located in an implementation-dependent manner.
     * <p>
     ...
     * a user setting the property javax.print.defaultPrinter. A service
     ...
     * registered, or cannot be registered, the method returns false.
     * <p>
     *
     * @param sp
     *           an implementation of a lookup service.
     * @return <code>true</code> if the new lookup service is newly registered;
     ...
     * be performed by the PrintServiceLookup class using the attribute values
     * and classes reported by the service. This may be less efficient than a
     * lookup service tuned for that service. Therefore registering a
     * <code>PrintServiceLookup</code> instance instead is recommended. The
     * method returns true if this service is not previously registered and is
     * now successfully registered. This method should not be called with
     * StreamPrintService instances. They will always fail to register and the
     * method will return false.
     * 
     * @param service
     *                an implementation of a print service.
     * @return <code>true</code> if the service is newly registered;
     *         <code>false</code> otherwise.
     */
     ...
     * combination of attributes and DocFlavors specified. This method is not
     * called directly by applications.
     * <p>
     * Implemented by a service provider, used by the static methods of this
     * class.
     * <p>
     * The results should be the same as obtaining all the PrintServices and
     * querying each one individually on its support for the specified
     ...
     * @param attributes
     *                   required to be supported. If null this constraint is
     *                   not used.
     * @return array of matching PrintServices. If no services match, the array
     *         is zero-length.
     */
     ...
     * 
     * @return array of all PrintServices known to this lookup service class. If
     *         none are found, the array is zero-length.
     */
     ...
     * class.
     * <p>
     * Locates MultiDoc print services which can be positively confirmed to
     * support the combination of attributes and DocFlavors specified.
     * <p>
     *
     * @param flavors
     ...
     * 
     * @return the default PrintService for this lookup service. If there is no
            ...
            try {
                java.security.AccessController.doPrivileged(
                        new java.security.PrivilegedExceptionAction() {
                            public Object run() {
                                Iterator<PrintServiceLookup> iterator = ServiceLoader
                                        .load(PrintServiceLookup.class)
                                        .iterator();
    ...
    private static ArrayList getServices(DocFlavor flavor,
        ...
        if (registeredServices != null) {
            PrintService[] services = (PrintService[]) registeredServices
                    .toArray(new PrintService[registeredServices.size()]);
            for (int i = 0; i < services.length; i++) {
                if (!listOfServices.contains(services[i])) {
                    if (flavor == null && attributes == null) {
                        listOfServices.add(services[i]);
                    } else if (((flavor != null && services[i]
                            .isDocFlavorSupported(flavor)) || flavor == null)
    ...
    private static ArrayList getMultiDocServices(DocFlavor[] flavors,
        ...
        while (psIterator.hasNext()) {
            try {
                PrintServiceLookup lus = (PrintServiceLookup) psIterator.next();
                MultiDocPrintService[] services = lus.getMultiDocPrintServices(
                        flavors, attributes);
                if (services == null) {
                    continue;
                }
