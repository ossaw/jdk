...
public abstract class PrintServiceLookup {
	 ...
	 * @param attributes
	 *            attributes that the print service must support. If null this
	 ...
	 *
	 * @param flavors
	 *            the flavors to print. If null or empty this constraint is not
	 *            used. Otherwise return only multidoc print services that can
						...
						.doPrivileged(new java.security.PrivilegedExceptionAction() {
							public Object run() {
								Iterator<PrintServiceLookup> iterator = ServiceLoader
										.load(PrintServiceLookup.class).iterator();
					...
					if (flavors == null || flavors.length == 0) {
						...
						for (int f = 0; f < flavors.length; f++) {
							if (services[i].isDocFlavorSupported(flavors[f])) {
								...
								if (services[i].getUnsupportedAttributes(flavors[f],
										attributes) != null) {
									supported = false;
									break;
								}
