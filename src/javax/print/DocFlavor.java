...
/**
 * Class <code>DocFlavor</code> encapsulates an object that specifies the format
 * in which print data is supplied to a {@link DocPrintJob}. "Doc" is a short,
 * easy-to-pronounce term that means "a piece of print data." The print data
 * format, or "doc flavor", consists of two things:
 * <UL>
 * <LI><B>MIME type.</B> This is a Multipurpose Internet Mail Extensions (MIME)
 * media type (as defined in <A HREF="http://www.ietf.org/rfc/rfc2045.txt">RFC
 * 2045</A> and <A HREF="http://www.ietf.org/rfc/rfc2046.txt">RFC 2046</A>) that
 * specifies how the print data is to be interpreted. The charset of text data
 * should be the IANA MIME-preferred name, or its canonical name if no preferred
 * name is specified. Additionally a few historical names supported by earlier
 * versions of the Java platform may be recognized. See
 * <a href="../../java/lang/package-summary.html#charenc"> character
 * encodings</a> for more information on the character encodings supported on
 * the Java platform.
 * <P>
 * <LI><B>Representation class name.</B> This specifies the fully-qualified name
 * of the class of the object from which the actual print data comes, as
 ...
 * <P>
 * <HR>
 * <H3>Client Formatted Print Data</H3> There are two broad categories of print
 * data, client formatted print data and service formatted print data.
 * <P>
 ...
 * URL for HTML code, or a disk file containing plain text in some encoding,
 * possibly obtained from an external source, and requires a way to describe the
 * data format to the print service.
 * <p>
 * The doc flavor's representation class is a conduit for the JPS
 * <code>DocPrintJob</code> to obtain a sequence of characters or bytes from the
 ...
 * <A HREF="http://www.iana.org/assignments/media-types/">Media Types
 ...
 * permitted):
 * <UL>
 * <LI>Character array (<CODE>char[]</CODE>) -- The print data consists of the
 * Unicode characters in the array.
 * <P>
 * <LI><code>String</code> -- The print data consists of the Unicode characters
 * in the string.
 * <P>
 ...
 * end-of-stream.
 * <P>
 * <LI>Byte array (<CODE>byte[]</CODE>) -- The print data consists of the bytes
 * in the array. The bytes are encoded in the character set specified by the doc
 ...
 * executing in a different environment. This means you should not use a URL
 * print data flavor to print a document at a restricted URL that the client can
 * see but the printer cannot see. This also means you should not use a URL
 * print data flavor to print a document stored in a local file that is not
 * available at a URL accessible independently of the client. For example, a
 * file that is not served up by an HTTP server or FTP server. To print such
 * documents, let the client open an input stream on the URL or file and use an
 * input stream data flavor.
 * </UL>
 * <p>
 * <HR>
 ...
 * corresponds to this and is suitable for use in a mime-type for a DocFlavor
 * can be obtained from {@link DocFlavor#hostEncoding DocFlavor.hostEncoding}
 * This may not always be the primary IANA name but is guaranteed to be
 * understood by this VM. For common flavors, the pre-defined *HOST DocFlavors
 * may be used.
 * <p>
 * <p>
 ...
 * Service instance might support for client formatted print data. Nested
 * classes inside class DocFlavor declare predefined static constant DocFlavor
 * objects for these example doc flavors; class DocFlavor's constructor can be
 * used to create an arbitrary doc flavor.
 * <UL>
 ...
 * value <CODE>"application/x-java-jvm-local-objectref"</CODE> indicating the
 * client will supply a reference to a Java object that implements the interface
 * named as the representation class. This MIME type is just a placeholder;
 * what's important is the print data representation class.
 * <P>
 ...
 * <UL>
 * <LI>Renderable image object -- The client supplies an object that implements
 * interface {@link java.awt.image.renderable.RenderableImage RenderableImage}.
 * The printer calls methods in that interface to obtain the image to be
 * printed.
 * <P>
 ...
 * methods in that interface to obtain the pages to be printed, one by one. For
 * each page, the printer supplies a graphics context, and whatever the client
 * draws in that graphics context gets printed.
 * <P>
 * <LI>Pageable object -- The client supplies an object that implements
 * interface {@link java.awt.print.Pageable Pageable}. The printer calls methods
 * in that interface to obtain the pages to be printed, one by one. For each
 * page, the printer supplies a graphics context, and whatever the client draws
 * in that graphics context gets printed.
 * </UL>
 * <P>
 * <HR>
 * <P>
 * <HR>
 * <H3>Pre-defined Doc Flavors</H3> A Java Print Service instance is not
 * <B><I>required</I></B> to support the following print data formats and print
 * data representation classes. In fact, a developer using this class should
 * <b>never</b> assume that a particular print service supports the document
 * types corresponding to these pre-defined doc flavors. Always query the print
 * service to determine what doc flavors it supports. However, developers who
 * have print services that support these doc flavors are encouraged to refer to
 * the predefined singleton instances created here.
 * <UL>
 * <LI>Plain text print data provided through a byte stream. Specifically, the
 * following doc flavors are recommended to be supported: <BR>
 * &#183;&nbsp;&nbsp; <CODE>("text/plain", "java.io.InputStream")</CODE> <BR>
 * &#183;&nbsp;&nbsp;
 * <CODE>("text/plain; charset=us-ascii", "java.io.InputStream")</CODE> <BR>
 * &#183;&nbsp;&nbsp;
 ...
 * recommended to be supported: <BR>
 * &#183;&nbsp;&nbsp;
 ...
 * <UL>
 * <LI>The character pair carriage return-line feed (CR-LF) means
 * "go to column 1 of the next line."
 * <LI>A carriage return (CR) character standing by itself means
 * "go to column 1 of the next line."
 * <LI>A line feed (LF) character standing by itself means
 * "go to column 1 of the next line."
 * <LI>
 * </UL>
 * <P>
 ...
 * <code>DataFlavor</code> is not used in the Java Print Service (JPS) API for
 * three reasons which are all rooted in allowing the JPS API to be shared by
 * other print services APIs which may need to run on Java profiles which do not
 * include all of the Java Platform, Standard Edition.
 * <OL TYPE=1>
 * <LI>The JPS API is designed to be used in Java profiles which do not support
 * AWT.
 * <P>
 * <LI>The implementation of class <code>java.awt.datatransfer.DataFlavor</code>
 * does not guarantee that equivalent data flavors will have the same serialized
 * representation. DocFlavor does, and can be used in services which need this.
 * <P>
 * <LI>The implementation of class <code>java.awt.datatransfer.DataFlavor</code>
 ...
 * class <I>name</I> of the representation class (a String object), rather than
 * the representation class itself (a Class object). This allows a client to
 * examine the doc flavors a Java Print Service instance supports without having
...
public class DocFlavor implements Serializable, Cloneable {
	 ...
	 * <a href="http://www.ietf.org/rfc/rfc2278.txt"> <i>RFC&nbsp;2278:&nbsp;
	 * IANA Charset Registration Procedures</i></a> except where historical
	 * names are returned for compatibility with previous versions of the Java
	 * platform. The value returned from method is valid only for the VM which
	 * returns it, for use in a DocFlavor. This is the charset for all the
	 * "HOST" pre-defined DocFlavors in the executing VM.
	 */
	...
	/**
	 * Returns a <code>String</code> representing a MIME parameter. Mime types
	 * may include parameters which are usually optional. The charset for text
	 * types is a commonly useful example. This convenience method will return
	 * the value of the specified parameter if one was specified in the mime
	 ...
	 * <code>DocFlavor</code>, has a MIME type equivalent to this doc flavor
	 * object's MIME type (that is, the MIME types have the same media type,
	 * media subtype, and parameters), and has the same representation class
	 * name as this doc flavor object. Thus, if two doc flavor objects' MIME
	 * types are the same except for comments, they are considered equal.
	 ...
	 *
	 * @serialData The serialised form of a DocFlavor is the String naming the
	 ...
	 * objects for example doc flavors using a byte array (<CODE>byte[]</CODE>)
	...
	public static class BYTE_ARRAY extends DocFlavor {
		...
		/**
		 * Doc flavor with MIME type = <CODE>"text/plain"</CODE>, encoded in the
		 * host platform encoding. See {@link DocFlavor#hostEncoding
		 * hostEncoding} Print data representation class name =
		 * <CODE>"[B"</CODE> (byte array).
		 */
		public static final BYTE_ARRAY TEXT_PLAIN_HOST = new BYTE_ARRAY(
		 ...
		 * <CODE>"text/plain; charset=utf-16be"</CODE> (big-endian byte
		 * ordering), print data representation class name = <CODE>"[B"</CODE>
		 * (byte array).
		 */
		...
		/**
		 * Doc flavor with MIME type =
		 * <CODE>"text/plain; charset=utf-16le"</CODE> (little-endian byte
		 * ordering), print data representation class name = <CODE>"[B"</CODE>
		 * (byte array).
		 */
		...
		/**
		 * Doc flavor with MIME type =
		 * <CODE>"text/plain; charset=us-ascii"</CODE>, print data
		 ...
		 * <CODE>"[B"</CODE> (byte array).
		 */
		public static final BYTE_ARRAY TEXT_HTML_HOST = new BYTE_ARRAY(
				"text/html; charset=" + hostEncoding);
		...
		/**
		 * Doc flavor with MIME type =
		 * <CODE>"text/html; charset=utf-16be"</CODE> (big-endian byte
		 * ordering), print data representation class name = <CODE>"[B"</CODE>
		 * (byte array).
		 */
		...
		/**
		 * Doc flavor with MIME type =
		 * <CODE>"text/html; charset=utf-16le"</CODE> (little-endian byte
		 * ordering), print data representation class name = <CODE>"[B"</CODE>
		 * (byte array).
		 */
		 ...
		 * class name = <CODE>"[B"</CODE> (byte array).
		 */
		...
		/**
		 * Doc flavor with MIME type = <CODE>"application/octet-stream"</CODE>,
		 * print data representation class name = <CODE>"[B"</CODE> (byte
	...
	public static class INPUT_STREAM extends DocFlavor {
		...
		/**
		 * Constructs a new doc flavor with the given MIME type and a print data
		 * representation class name of <CODE>"java.io.InputStream"</CODE> (byte
		 * stream).
		 *
		 * @param mimeType
		 *                 MIME media type string.
		 ...
		 * Doc flavor with MIME type =
		 * <CODE>"text/plain; charset=us-ascii"</CODE>, print data
		 * representation class name = <CODE>"java.io.InputStream"</CODE> (byte
		 * stream).
		 */
		 ...
		 * <CODE>"java.io.InputStream"</CODE> (byte stream).
		 */
		public static final INPUT_STREAM TEXT_HTML_HOST = new INPUT_STREAM(
				"text/html; charset=" + hostEncoding);
		 ...
		 * Doc flavor with MIME type =
		 * <CODE>"text/html; charset=us-ascii"</CODE>, print data representation
		 * class name = <CODE>"java.io.InputStream"</CODE> (byte stream).
		 */
		 ...
		 * Doc flavor with MIME type = <CODE>"application/octet-stream"</CODE>,
		 * print data representation class name =
		 * <CODE>"java.io.InputStream"</CODE> (byte stream). The client must
	 ...
	 * objects. For example doc flavors using a Uniform Resource Locator (
	 * {@link java.net.URL java.net.URL}) as the print data representation
	 * class.
	 * <P>
	 *
	 * @author Alan Kaminsky
	...
	public static class URL extends DocFlavor {
		 ...
		 * Doc flavor with MIME type = <CODE>"application/octet-stream"</CODE>,
		 * print data representation class name = <CODE>"java.net.URL"</CODE>.
		 * The client must determine that data described using this DocFlavor is
	 ...
	 * <CODE>char[]</CODE>) as the print data representation class. As such, the
	 ...
	 * java.lang.String}) as the print data representation class. As such, the
	 * character set is Unicode.
	 * <P>
	 *
	 * @author Alan Kaminsky
	 ...
	 * {@link java.io.Reader java.io.Reader}) as the print data representation
	 * class. As such, the character set is Unicode.
	 * <P>
	 *
	 * @author Alan Kaminsky
	 */
	public static class READER extends DocFlavor {
		...
		/**
		 * Constructs a new doc flavor with the given MIME type and a print data
		 * representation class name of\ <CODE>"java.io.Reader"</CODE>
		 * (character stream).
		 *
		 * @param mimeType
		 *                 MIME media type string. If it is a text media type,
	...
	public static class SERVICE_FORMATTED extends DocFlavor {
		 ...
		 * service formatted print data and the given print data representation
		 * class name.
		 *
		 * @param className
		 *                  Fully-qualified representation class name.
		 ...
		 * representation class name =
		 * <CODE>"java.awt.image.renderable.RenderableImage"</CODE> (renderable
		 * image object).
		 */
