 ...
 * using the <a href="../../../technotes/guides/jar/jar.html#Service Provider"
 ...
 * factory in place of a <code>PrintService</code> which represents a physical
 * printer device.
 */
...
public abstract class StreamPrintServiceFactory {
     ...
     * output a stream of data in the format specified by {@code outputMimeType}
     * .
     * <p>
     ...
     * printing services, it's typical to search for a particular desired
    ...
    public static StreamPrintServiceFactory[] lookupStreamPrintServiceFactories(DocFlavor flavor,
        ...
        return (StreamPrintServiceFactory[]) (list.toArray(new StreamPrintServiceFactory[list.size()]));
    }
    ...
    /**
     * Returns a <code>StreamPrintService</code> that can print to the specified
     ...
     * and to ensure that this Printer is not reused. The application should not
     * close this stream until any print job created from the printer is
     * complete. Doing so earlier may generate a <code>PrinterException</code>
     * and an event indicating that the job failed.
     * <p>
     * Whereas a <code>PrintService</code> connected to a physical printer can
     * be reused, a <code>StreamPrintService</code> connected to a stream
     * cannot. The underlying <code>StreamPrintService</code> may be disposed by
     * the print system with the {@link StreamPrintService#dispose() dispose}
     * method before returning from the
     * {@link DocPrintJob#print(Doc, javax.print.attribute.PrintRequestAttributeSet)
     * print} method of <code>DocPrintJob</code> so that the print system knows
     * this printer is no longer usable. This is equivalent to a physical
     * printer going offline - permanently. Applications may supply a null print
     * stream to create a queryable service. It is not valid to create a
     * PrintJob for such a stream. Implementations which allocate resources on
        ...
        synchronized (StreamPrintServiceFactory.class) {
            ...
            try {
                java.security.AccessController.doPrivileged(new java.security.PrivilegedExceptionAction() {
                    public Object run() {
                        Iterator<StreamPrintServiceFactory> iterator = ServiceLoader.load(
    ...
    private static ArrayList getFactories(DocFlavor flavor, String outType) {
        ...
        while (iterator.hasNext()) {
            ...
            if ((outType == null || outType.equalsIgnoreCase(factory.getOutputFormat())) && (flavor == null
