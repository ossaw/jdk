 ...
 * calling {@link #getValue() getValue()}. A date-time attribute's Date value is
 * established when it is constructed (see {@link #DateTimeSyntax(Date)
 * DateTimeSyntax(Date)}). Once constructed, a date-time attribute's value is
 * immutable.
 * <P>
 * To construct a date-time attribute from separate values of the year, month,
 * day, hour, minute, and so on, use a <code>java.util.Calendar</code> object to
 * construct a <code>java.util.Date</code> object, then use the
 * <code>java.util.Date</code> object to construct the date-time attribute. To
 * convert a date-time attribute to separate values of the year, month, day,
 * hour, minute, and so on, create a <code>java.util.Calendar</code> object and
 ...
 * </code> rather than a <code>java.util.Calendar</code> because it typically
 * takes less memory to store and less time to compare a
 * <code>java.util.Date</code> than a <code>java.util.Calendar</code>.
 * <P>
 *
...
public abstract class DateTimeSyntax implements Serializable, Cloneable {
	 ...
	 *                (unchecked exception) Thrown if <CODE>theValue</CODE> is
	 *                null.
	 */
	...
	public boolean equals(Object object) {
		return (object != null && object instanceof DateTimeSyntax
