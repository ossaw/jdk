...
/**
 * Interface PrintServiceAttributeSet specifies the interface for a set of print
 * job attributes, i.e. printing attributes that implement interface
 * {@link PrintServiceAttribute PrintServiceAttribute}. In the Print Service
 * API, the Print Service instance uses a PrintServiceAttributeSet to report the
 * status of the print service.
 * <P>
 * A PrintServiceAttributeSet is just an {@link AttributeSet AttributeSet} whose
 ...
 * The {@link #add(Attribute) add(Attribute)}, and {@link #addAll(AttributeSet)
 * addAll(AttributeSet)} operations are respecified below to guarantee this
...
public interface PrintServiceAttributeSet extends AttributeSet {
	 ...
	 * <tt>addAll()</tt> operation effectively modifies this attribute set so
	 * that its value is the <i>union</i> of the two sets.
	 * <P>
	 * The behavior of the <CODE>addAll()</CODE> operation is unspecified if the
	 * specified set is modified while the operation is in progress.
	 * <P>
	 * If the <CODE>addAll()</CODE> operation throws an exception, the effect on
	 * this attribute set's state is implementation dependent; elements from the
