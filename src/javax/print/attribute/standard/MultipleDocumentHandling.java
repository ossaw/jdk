 ...
 * <UL>
 * <LI>The multiple input documents are combined into a single output document.
 * Finishing operations ({@link Finishings Finishings}), are performed on this
 * single output document. The {@link Copies Copies} attribute tells how many
 * copies of this single output document to produce. The
 * MultipleDocumentHandling values SINGLE_DOCUMENT and SINGLE_DOCUMENT_NEW_SHEET
 * specify two variations of this possibility.
 * <P>
 * <LI>The multiple input documents remain separate output documents. Finishing
 * operations ({@link Finishings Finishings}), are performed on each output
 * document separately. The {@link Copies Copies} attribute tells how many
 * copies of each separate output document to produce. The
 * MultipleDocumentHandling values SEPARATE_DOCUMENTS_UNCOLLATED_COPIES and
 ...
 * instance of document data, then the result of processing the data in document
 * "<CODE>a</CODE>" is a sequence of media sheets represented by
 * "<CODE>a(*)</CODE>".
 * <P>
 * The standard MultipleDocumentHandling values are:
 * <UL>
 * <LI><A NAME="sdfi">{@link #SINGLE_DOCUMENT <B>SINGLE_DOCUMENT</B>}</A>. If a
 * print job has multiple documents -- say, the document data is called
 * <CODE>a</CODE> and <CODE>b</CODE> -- then the result of processing all the
 * document data (<CODE>a</CODE> and then <CODE>b</CODE>) must be treated as a
 * single sequence of media sheets for finishing operations; that is, finishing
 * would be performed on the concatenation of the sequences
 * <CODE>a(*),b(*)</CODE>. The printer must not force the data in each document
 * instance to be formatted onto a new print-stream page, nor to start a new
 * impression on a new media sheet. If more than one copy is made, the ordering
 * of the sets of media sheets resulting from processing the document data must
 * be <CODE>a(*),b(*),a(*),b(*),...</CODE>, and the printer object must force
 * each copy (<CODE>a(*),b(*)</CODE>) to start on a new media sheet.
 * <P>
 * <LI><A NAME="sducfi">{@link #SEPARATE_DOCUMENTS_UNCOLLATED_COPIES
 * <B>SEPARATE_DOCUMENTS_UNCOLLATED_COPIES</B>}</A>. If a print job has multiple
 ...
 * of processing the data in a single document to start on a new media sheet. If
 * more than one copy is made, the ordering of the sets of media sheets
 * resulting from processing the document data must be
 * <CODE>a(*),a(*),...,b(*),b(*)...</CODE>.
 * <P>
 * <LI><A NAME="sdccfi">{@link #SEPARATE_DOCUMENTS_COLLATED_COPIES
 * <B>SEPARATE_DOCUMENTS_COLLATED_COPIES</B>}</A>. If a print job has multiple
 * documents -- say, the document data is called <CODE>a</CODE> and
 * <CODE>b</CODE> -- then the result of processing the data in each document
 * instance must be treated as a single sequence of media sheets for finishing
 * operations; that is, the sets <CODE>a(*)</CODE> and <CODE>b(*)</CODE> would
 ...
 * <CODE>a(*),b(*),a(*),b(*),...</CODE>.
 * <P>
 * <LI><A NAME="sdnsfi">{@link #SINGLE_DOCUMENT_NEW_SHEET
 * <B>SINGLE_DOCUMENT_NEW_SHEET</B>}</A>. Same as SINGLE_DOCUMENT, except that
 * the printer must ensure that the first impression of each document instance
 * in the job is placed on a new media sheet. This value allows multiple
 ...
 * the job, while SEPARATE_DOCUMENTS_COLLATED_COPIES always forces the next
 * document or document copy on to a new sheet.
 * <P>
 * In addition, if a {@link Finishings Finishings} attribute of
 * {@link Finishings#STAPLE STAPLE} is specified, then:
 * <UL>
 * <LI>With SINGLE_DOCUMENT, documents <CODE>a</CODE> and <CODE>b</CODE> are
 * stapled together as a single document with no regard to new sheets.
 * <P>
 * <LI>With SINGLE_DOCUMENT_NEW_SHEET, documents <CODE>a</CODE> and
 * <CODE>b</CODE> are stapled together as a single document, but document
 * <CODE>b</CODE> starts on a new sheet.
 * <P>
 * <LI>With SEPARATE_DOCUMENTS_UNCOLLATED_COPIES and
 ...
 * <CODE>getName()</CODE> is the IPP attribute name. The enumeration's integer
 * value is the IPP enum value. The <code>toString()</code> method returns the
 * IPP string representation of the attribute value.
 * <P>
 *
 * @see Copies
 * @see Finishings
