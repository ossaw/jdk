 ...
 * printer can use to retrieve print data stored at a URI location. If a printer
 * supports doc flavors with a print data representation class of
 * <CODE>"java.net.URL"</CODE>, the printer uses instances of class
 * ReferenceUriSchemesSupported to advertise the URI schemes it can accept. The
 * acceptable URI schemes are included as service attributes in the lookup
 * service; this lets clients search the for printers that can get print data
 * using a certain URI scheme. The acceptable URI schemes can also be queried
 * using the capability methods in interface <code>PrintService</code>. However,
 * ReferenceUriSchemesSupported attributes are used solely for determining
 * acceptable URI schemes, they are never included in a doc's, print request's,
 * print job's, or print service's attribute set.
 * <P>
 ...
 * URI schemes</A>.
 * <p>
 * Class ReferenceUriSchemesSupported defines enumeration values for widely used
 * URI schemes. A printer that supports additional URI schemes can define them
 * in a subclass of class ReferenceUriSchemesSupported.
 * <P>
...
public class ReferenceUriSchemesSupported extends EnumSyntax implements Attribute {
    ...
    private static final String[] myStringTable = { "ftp", "http", "https", "gopher", "news", "nntp", "wais",
            "file", };

    private static final ReferenceUriSchemesSupported[] myEnumValueTable = { FTP, HTTP, HTTPS, GOPHER, NEWS,
            NNTP, WAIS, FILE, };
