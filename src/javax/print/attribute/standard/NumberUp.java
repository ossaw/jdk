 ...
 * the number of print-stream pages to impose upon a single side of an instance
 * of a selected medium. That is, if the NumberUp value is <I>n,</I> the printer
 * must place <I>n</I> print-stream pages on a single side of an instance of the
 ...
 * any value of {@link MultipleDocumentHandling MultipleDocumentHandling} makes
 * sense, and the printer's processing depends on the
 * {@link MultipleDocumentHandling MultipleDocumentHandling} value:
 * <UL>
 * <LI>SINGLE_DOCUMENT -- All the input docs will be combined together into one
 * output document. Each media impression will consist of <I>n</I>m print-stream
 * pages from the output document.
 * <P>
 * <LI>SINGLE_DOCUMENT_NEW_SHEET -- All the input docs will be combined together
 ...
 * it.
 * <P>
 * <LI>SEPARATE_DOCUMENTS_UNCOLLATED_COPIES -- The input docs will remain
 * separate. Each media impression will consist of <I>n</I> print-stream pages
 * from the input doc. Since the input docs are separate, the first impression
 * of each input doc will always start on a new media sheet; this means the last
 * impression of an input doc may have fewer than <I>n</I> print-stream pages on
 * it.
 * <P>
 * <LI>SEPARATE_DOCUMENTS_COLLATED_COPIES -- The input docs will remain
 * separate. Each media impression will consist of <I>n</I> print-stream pages
 * from the input doc. Since the input docs are separate, the first impression
 * of each input doc will always start on a new media sheet; this means the last
 * impression of an input doc may have fewer than <I>n</I> print-stream pages on
 ...
 * <P>
 * <LI>SEPARATE_DOCUMENTS_UNCOLLATED_COPIES -- The input docs will remain
 * separate. For input doc <I>i,</I> each media impression will consist of <I>n
 * <SUB>i</SUB></I> print-stream pages from the input doc. Since the input docs
 * are separate, the first impression of each input doc will always start on a
 * new media sheet; this means the last impression of an input doc may have
...
public final class NumberUp extends IntegerSyntax implements DocAttribute, PrintRequestAttribute,
     ...
     * <LI><CODE>object</CODE> is an instance of class NumberUp.
     * <LI>This number up attribute's value and <CODE>object</CODE>'s value are
     * equal.
     * </OL>
     *
     * @param object
