...
/**
 * Class PrinterStateReason is a printing attribute class, an enumeration, that
 * provides additional information about the printer's current state, i.e.,
 * information that augments the value of the printer's {@link PrinterState
 * PrinterState} attribute. Class PrinterStateReason defines standard printer
 ...
 * reason.
 * <P>
 * Instances of PrinterStateReason do not appear in a Print Service's attribute
 ...
 * or more than one PrinterStateReason objects which pertain to the Print
 * Service's status, and each PrinterStateReason object is associated with a
 * {@link Severity Severity} level of REPORT (least severe), WARNING, or ERROR
 * (most severe). The printer adds a PrinterStateReason object to the Print
 * Service's {@link PrinterStateReasons PrinterStateReasons} attribute when the
 * corresponding condition becomes true of the printer, and the printer removes
 * the PrinterStateReason object again when the corresponding condition becomes
 * false, regardless of whether the Print Service's overall {@link PrinterState
 ...
 * <CODE>toString()</CODE> methods, concatenated together with a hyphen (
 * <CODE>"-"</CODE>) in between, gives the IPP keyword value for a
 * {@link PrinterStateReasons}. The category name returned by
...
public class PrinterStateReason extends EnumSyntax implements Attribute {
     ...
     * {@link PrinterState PrinterState} becomes STOPPED, and the PAUSED value
     * replaces the MOVING_TO_PAUSED value in the {@link PrinterStateReasons
     ...
     * value must be supported if the printer can be paused.
     */
     ...
     * output device ceases accepting jobs, the printer's
     * {@link PrinterStateReasons PrinterStateReasons} will have this reason
    ...
    private static final String[] myStringTable = { "other", "media-needed", "media-jam", "moving-to-paused",
            "paused", "shutdown", "connecting-to-device", "timed-out", "stopping", "stopped-partly",
            "toner-low", "toner-empty", "spool-area-full", "cover-open", "interlock-open", "door-open",
            "input-tray-missing", "media-low", "media-empty", "output-tray-missing",
            "output-area-almost-full", "output-area-full", "marker-supply-low", "marker-supply-empty",
            "marker-waste-almost-full", "marker-waste-full", "fuser-over-temp", "fuser-under-temp",
            "opc-near-eol", "opc-life-over", "developer-low", "developer-empty",
            "interpreter-resource-unavailable" };

    private static final PrinterStateReason[] myEnumValueTable = { OTHER, MEDIA_NEEDED, MEDIA_JAM,
            MOVING_TO_PAUSED, PAUSED, SHUTDOWN, CONNECTING_TO_DEVICE, TIMED_OUT, STOPPING, STOPPED_PARTLY,
            TONER_LOW, TONER_EMPTY, SPOOL_AREA_FULL, COVER_OPEN, INTERLOCK_OPEN, DOOR_OPEN,
            INPUT_TRAY_MISSING, MEDIA_LOW, MEDIA_EMPTY, OUTPUT_TRAY_MISSING, OUTPUT_AREA_ALMOST_FULL,
            OUTPUT_AREA_FULL, MARKER_SUPPLY_LOW, MARKER_SUPPLY_EMPTY, MARKER_WASTE_ALMOST_FULL,
            MARKER_WASTE_FULL, FUSER_OVER_TEMP, FUSER_UNDER_TEMP, OPC_NEAR_EOL, OPC_LIFE_OVER, DEVELOPER_LOW,
            DEVELOPER_EMPTY, INTERPRETER_RESOURCE_UNAVAILABLE };
