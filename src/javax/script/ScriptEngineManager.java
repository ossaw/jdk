 ...
 * collection of key/value pairs storing state shared by all engines created by
 * the Manager. This class uses the
 * <a href="../../../technotes/guides/jar/jar.html#Service%20Provider">service
 * provider</a> mechanism to enumerate all the implementations of
 * <code>ScriptEngineFactory</code>. <br>
 * <br>
 * The <code>ScriptEngineManager</code> provides a method to return a list of
 * all these factories as well as utility methods which look up factories on the
 * basis of language name, file extension and mime type.
 * <p>
 * The <code>Bindings</code> of key/value pairs, referred to as the
 * "Global Scope" maintained by the manager is available to all instances of
 * <code>ScriptEngine</code> created by the <code>ScriptEngineManager</code>.
 * The values in the <code>Bindings</code> are generally exposed in all scripts.
 *
 * @author Mike Grogan
...
public class ScriptEngineManager {
     ...
     * <code>ClassLoader</code> using the
     * <a href="../../../technotes/guides/jar/jar.html#Service%20Provider">
     * service provider</a> mechanism.<br>
     * <br>
     * If loader is <code>null</code>, the script engine factories that are
     * bundled with the platform and that are in the usual extension directories
     * (installed extensions) are loaded. <br>
     * <br>
     *
     * @param loader
    ...
    /**
     * <code>setBindings</code> stores the specified <code>Bindings</code> in
     * the <code>globalScope</code> field. ScriptEngineManager sets this
    ...
    /**
     * <code>getBindings</code> returns the value of the
     * <code>globalScope</code> field. ScriptEngineManager sets this
     * <code>Bindings</code> as global bindings for <code>ScriptEngine</code>
     * objects created by it.
     *
    ...
    /**
     * Look up and create a <code>ScriptEngine</code> for a given extension. The
     * algorithm used by <code>getEngineByName</code> is used except that the
     * search starts by looking for a <code>ScriptEngineFactory</code>
    ...
    /**
     * Look up and create a <code>ScriptEngine</code> for a given mime type. The
     * algorithm used by <code>getEngineByName</code> is used except that the
     * search starts by looking for a <code>ScriptEngineFactory</code>
     ...
     * <code>ScriptEngineFactory</code> classes found by the discovery
     * mechanism.
     * 
