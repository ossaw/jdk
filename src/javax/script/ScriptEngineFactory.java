 ...
 * <code>ScriptEngines</code>. <br>
 * <br>
 * Each class implementing <code>ScriptEngine</code> has a corresponding factory
 * that exposes metadata describing the engine class. <br>
 * <br>
 * The <code>ScriptEngineManager</code> uses the service provider mechanism
 * described in the <i>Jar File Specification</i> to obtain instances of all
 * <code>ScriptEngineFactories</code> available in the current ClassLoader.
 *
 * @since 1.6
 */
public interface ScriptEngineFactory {
    /**
     * Returns the full name of the <code>ScriptEngine</code>. For instance an
     ...
     * <code>ScriptEngine</code>. The array is used by the
     ...
     * <code>ScriptEngine</code>, which may be used to identify the
     * <code>ScriptEngine</code> by the <code>ScriptEngineManager</code>. For
     * instance, an implementation based on the Mozilla Rhino Javascript engine
     * might return list containing {&quot;javascript&quot;, &quot;rhino&quot;}.
     * 
     ...
     * <code>getEngineName</code>, <code>getEngineVersion</code>,
     * <code>getLanguageName</code>, <code>getLanguageVersion</code> for the
     * first four keys respectively. For NAME, one of the Strings returned by
     * <code>getNames</code> is returned.<br>
     * <br>
     * A reserved key, <code><b>THREADING</b></code>, whose value describes the
     * behavior of the engine with respect to concurrent execution of scripts
     * and maintenance of state is also defined. These values for the
     * <code><b>THREADING</b></code> key are:<br>
     * <br>
     * <ul>
     * <li><code>null</code> - The engine implementation is not thread safe, and
     * cannot be used to execute scripts concurrently on multiple threads.
     * <li><code>&quot;MULTITHREADED&quot;</code> - The engine implementation is
     * internally thread-safe and scripts may execute concurrently although
     * effects of script execution on one thread may be visible to scripts on
     * other threads.
     * <li><code>&quot;THREAD-ISOLATED&quot;</code> - The implementation
     * satisfies the requirements of &quot;MULTITHREADED&quot;, and also, the
     * engine maintains independent values for symbols in scripts executing on
     * different threads.
     * <li><code>&quot;STATELESS&quot;</code> - The implementation satisfies the
     * requirements of
     * <li><code>&quot;THREAD-ISOLATED&quot;</code>. In addition, script
     * executions do not alter the mappings in the <code>Bindings</code> which
     * is the engine scope of the <code>ScriptEngine</code>. In particular, the
     * keys in the <code>Bindings</code> and their associated values are the
     ...
     * @return The value for the given parameter. Returns <code>null</code> if
     ...
     * using the syntax of the supported scripting language. For instance, an
     * implementation for a Javascript engine might be;
     *
     * <pre>
     * {@code
     ...
     * String using the syntax of the supported scripting language. For
     * instance, the implementation for a Perl engine might be;
     *
     * <pre>
     * <code>
     ...
     * statements. For instance an implementation for a PHP engine might be:
     *
     * <pre>
     * {@code
