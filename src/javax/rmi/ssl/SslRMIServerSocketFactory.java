...
/**
 * <p>
 * An <code>SslRMIServerSocketFactory</code> instance is used by the RMI runtime
 * in order to obtain server sockets for RMI calls via SSL.
 * </p>
 * <p>
 ...
 * <code>SSLSocketFactory</code> (see {@link SSLSocketFactory#getDefault}) or
 * the default <code>SSLServerSocketFactory</code> (see
 * {@link SSLServerSocketFactory#getDefault}) unless the constructor taking an
 * <code>SSLContext</code> is used in which case the SSL sockets are created
 * using the <code>SSLSocketFactory</code> returned by
 ...
 * authentication is required by the server). This behavior can be modified by
 * supplying an already initialized <code>SSLContext</code> instance.
 *
...
public class SslRMIServerSocketFactory implements RMIServerSocketFactory {
    ...
    /**
     * <p>
     * Creates a new <code>SslRMIServerSocketFactory</code> with the default SSL
     * socket configuration.
     * </p>
     * <p>
     * SSL connections accepted by server sockets created by this factory have
     * the default cipher suites and protocol versions enabled and do not
    ...
    /**
     * <p>
     * Creates a new <code>SslRMIServerSocketFactory</code> with the specified
     * SSL socket configuration.
     * </p>
     *
     * @param enabledCipherSuites
     *        names of all the cipher suites to enable on
     *        SSL connections
     *        accepted by server sockets created by this
     *        factory, or
     *        <code>null</code> to use the cipher suites
     *        that are enabled by
     *        default
     * @param enabledProtocols
     *        names of all the protocol versions to enable
     *        on SSL
     *        connections accepted by server sockets created
     *        by this
     *        factory, or <code>null</code> to use the
     *        protocol versions
     *        that are enabled by default
     * @param needClientAuth
     *        <code>true</code> to require client
     *        authentication on SSL
     *        connections accepted by server sockets created
     *        by this
     *        factory; <code>false</code> to not require
     *        client
     *        authentication
     * @exception IllegalArgumentException
     *            when one or more of the cipher suites
     *            named by the
     *            <code>enabledCipherSuites</code>
     *            parameter is not
     *            supported, when one or more of the
     *            protocols named by the
     *            <code>enabledProtocols</code>
     *            parameter is not supported
     *            or when a problem is encountered
     *            while trying to check if
     *            the supplied cipher suites and
     *            protocols to be enabled are
     *            supported.
     ...
     * <p>
     * Creates a new <code>SslRMIServerSocketFactory</code> with the specified
     * <code>SSLContext</code> and SSL socket configuration.
     * </p>
     *
     * @param context
     *        the SSL context to be used for creating SSL
     *        sockets. If
     *        <code>context</code> is null the default
     *        <code>SSLSocketFactory</code> or the default
     *        <code>SSLServerSocketFactory</code> will be
     *        used to create SSL
     *        sockets. Otherwise, the socket factory
     *        returned by
     *        <code>SSLContext.getSocketFactory()</code> or
     *        <code>SSLContext.getServerSocketFactory()</code>
     *        will be used
     *        instead.
     * @param enabledCipherSuites
     *        names of all the cipher suites to enable on
    ...
    public SslRMIServerSocketFactory(SSLContext context, String[] enabledCipherSuites,
        ...
        final SSLSocketFactory sslSocketFactory = context == null ? getDefaultSSLSocketFactory()
    ...
    /**
     * <p>
     * Returns the names of the cipher suites enabled on SSL connections
     * accepted by server sockets created by this factory, or <code>null</code>
    ...
    public final String[] getEnabledCipherSuites() {
        return enabledCipherSuites == null ? null : enabledCipherSuites.clone();
    }

    /**
     * <p>
     * Returns the names of the protocol versions enabled on SSL connections
     * accepted by server sockets created by this factory, or <code>null</code>
     * if this factory uses the protocol versions that are enabled by default.
     * </p>
     *
     * @return an array of protocol versions enabled, or <code>null</code>
     * @see SSLSocket#setEnabledProtocols
     */
    public final String[] getEnabledProtocols() {
        return enabledProtocols == null ? null : enabledProtocols.clone();
    }
    ...
    public ServerSocket createServerSocket(int port) throws IOException {
        ...
        return new ServerSocket(port) {
            public Socket accept() throws IOException {
                ...
                SSLSocket sslSocket = (SSLSocket) sslSocketFactory.createSocket(socket, socket
     ...
     * </p>
     * <p>
     * Two <code>SslRMIServerSocketFactory</code> objects are equal if they have
     * been constructed with the same SSL context and SSL socket configuration
     * parameters.
     * </p>
     * <p>
    ...
    private boolean checkParameters(SslRMIServerSocketFactory that) {
        ...
        // enabledCipherSuites
        //
        if ((enabledCipherSuites == null && that.enabledCipherSuites != null) || (enabledCipherSuites != null
                && that.enabledCipherSuites == null))
            return false;
        if (enabledCipherSuites != null && that.enabledCipherSuites != null) {
            List<String> thatEnabledCipherSuitesList = Arrays.asList(that.enabledCipherSuites);
        ...
        // enabledProtocols
        //
        if ((enabledProtocols == null && that.enabledProtocols != null) || (enabledProtocols != null
                && that.enabledProtocols == null))
            return false;
        if (enabledProtocols != null && that.enabledProtocols != null) {
            List<String> thatEnabledProtocolsList = Arrays.asList(that.enabledProtocols);
    ...
    /**
     * <p>
     * Returns a hash code value for this <code>SslRMIServerSocketFactory</code>
     * .
     * </p>
     *
     * @return a hash code value for this <code>SslRMIServerSocketFactory</code>
     *         .
     */
    public int hashCode() {
        return getClass().hashCode() + (context == null ? 0 : context.hashCode()) + (needClientAuth
                ? Boolean.TRUE.hashCode() : Boolean.FALSE.hashCode()) + (enabledCipherSuites == null ? 0
                        : enabledCipherSuitesList.hashCode()) + (enabledProtocols == null ? 0
                                : enabledProtocolsList.hashCode());
    }
