 ...
 * instances of this class are functionally equivalent. In particular, they all
 * share the same truststore, and the same keystore when client authentication
 * is required by the server. This behavior can be modified in subclasses by
 * overriding the {@link #createSocket(String,int)} method; in that case,
 ...
 * {@link SSLSocket#setEnabledCipherSuites(String[])} before returning the
 ...
 * {@link SSLSocket#setEnabledProtocols(String[])} before returning the socket.
 * The value of this system property is a string that is a comma-separated list
 * of SSL/TLS protocol versions to enable.
 * </p>
 *
...
public class SslRMIClientSocketFactory implements RMIClientSocketFactory,
     ...
     * method will call {@link SSLSocket#setEnabledCipherSuites(String[])}
