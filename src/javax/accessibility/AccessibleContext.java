...
public abstract class AccessibleContext {
     ...
     * The old value in the PropertyChangeEvent will be the old accessibleName
     ...
     * accessibleDescription and the new value will be the new
     ...
     * changed. The old value will be the old AccessibleState and the new value
     * will be the new AccessibleState in the accessibleStateSet. For example,
     * if a component that supports the vertical and horizontal states changes
     * its orientation from vertical to horizontal, the old value will be
     * AccessibleState.VERTICAL and the new value will be
     * AccessibleState.HORIZONTAL. Please note that either value can also be
     * null. For example, when a component changes from being enabled to
     * disabled, the old value will be AccessibleState.ENABLED and the new value
     * will be null.
     *
     ...
     * The old value in the PropertyChangeEvent will be a Number representing
     * the old value and the new value will be a Number representing the new
     * value
     *
    ...
    /**
     * Constant used to determine when the accessibleSelection has changed. The
     * old and new values in the PropertyChangeEvent are currently reserved for
     * future use.
     *
    ...
    /**
     * Constant used to determine when the accessibleText caret has changed. The
     * old value in the PropertyChangeEvent will be an integer representing the
     * old caret position, and the new value will be an integer representing the
     * new/current caret position.
     *
     * @see #addPropertyChangeListener
     */
     ...
     * changed. The old and new values in the PropertyChangeEvent are currently
     ...
     * from the object. If an Accessible child is being added, the old value
     * will be null and the new value will be the Accessible child. If an
     ...
     * Accessible representing the previous active child, and the new value will
     ...
     * value in the PropertyChangeEvent will be an Accessible representing the
     * previous table caption and the new value will be an Accessible
     ...
     * value in the PropertyChangeEvent will be an Accessible representing the
     * previous table summary and the new value will be an Accessible
     ...
     * the PropertyChangeEvent will be null and the new value will be an
     * AccessibleTableModelChange representing the table change.
     * 
     * @see AccessibleTable
     ...
     * value in the PropertyChangeEvent will be null and the new value will be
     * an AccessibleTableModelChange representing the header change.
     * 
     * @see AccessibleTable
     * @see AccessibleTableModelChange
     */
     ...
     * value in the PropertyChangeEvent will be null and the new value will be
     * an Integer representing the column index.
     * 
     * @see AccessibleTable
     */
     ...
     * The old value in the PropertyChangeEvent will be an Integer representing
     * the old number of actions supported and the new value will be an Integer
     * representing the new number of actions supported.
     * 
     ...
     * The old value in the PropertyChangeEvent will be an Integer representing
     * the start index in the document of the previous element that had focus
     * and the new value will be an Integer representing the start index in the
     * document of the current element that has focus. A value of -1 indicates
     * that an element does not or did not have focus.
     * 
    ...
    /**
     * PropertyChangeEvent which indicates that text has changed. <br>
     * For text insertion, the oldValue is null and the newValue is an
     * AccessibleTextSequence specifying the text that was inserted. <br>
     * For text deletion, the oldValue is an AccessibleTextSequence specifying
     * the text that was deleted and the newValue is null. <br>
     ...
     * subcomponents. The oldValue is null and the newValue is the component
     * whose children have become invalid.
     *
     * @see #getAccessibleText
     * @see #addPropertyChangeListener
     * @see AccessibleTextSequence
     ...
     * <br>
     * For attribute insertion, the oldValue is null and the newValue is an
     * AccessibleAttributeSequence specifying the attributes that were inserted.
     * <br>
    ...
    /**
     * PropertyChangeEvent which indicates that a change has occurred in a
     * component's bounds. The oldValue is the old component bounds and the
     ...
     * of the object. For example, the accessibleName property of a label or
     * button might be the text of the label or button itself. In the case of an
     * object that doesn't display its name, the accessibleName should still be
     * set. For example, in the case of a text field used to enter the name of a
     ...
     * cause a PropertyChangeEvent to be fired for the ACCESSIBLE_NAME_PROPERTY
     * property.
     *
     * @param s
     ...
     * accessibleDescription property of this object is a short localized phrase
     * describing the purpose of the object. For example, in the case of a
     * 'Cancel' button, the accessibleDescription could be 'Ignore changes and
     * close dialog box.'
     *
     ...
     * push button is AccessibleRole.PUSH_BUTTON. The roles in AccessibleRole
     * are provided so component developers can pick from a set of predefined
     * roles. This enables assistive technologies to provide a consistent
     * interface to various tweaked subclasses of components (e.g., use
     * AccessibleRole.PUSH_BUTTON for all components that act like a push
     * button) as well as distinguish between subclasses that behave differently
     * (e.g., AccessibleRole.CHECK_BOX for check boxes and
     * AccessibleRole.RADIO_BUTTON for radio buttons).
     * <p>
     * Note that the AccessibleRole class is also extensible, so custom
     * component developers can define their own AccessibleRole's if the set of
     * predefined roles is inadequate.
     *
    ...
    /**
     * Gets the state set of this object. The AccessibleStateSet of an object is
     * composed of a set of unique AccessibleStates. A change in the
     * AccessibleStateSet of an object will cause a PropertyChangeEvent to be
     * fired for the ACCESSIBLE_STATE_PROPERTY property.
     *
     ...
     *
     * @return the Accessible parent of this object; null if this object does
    ...
    /**
     * Sets the Accessible parent of this object. This is meant to be used only
     * in the situations where the actual component's parent should not be
     * treated as the component's accessible parent and is a method that should
     ...
     * children of an Accessible object are zero-based, so the first child of an
     * Accessible child is at index 0, the second child is at index 1, and so
     * on.
     *
     * @param i
     ...
     * registered for all Accessible properties and will be called when those
     * properties change.
     *
    ...
    /**
     * Gets the AccessibleAction associated with this object that supports one
     * or more actions.
     *
    ...
    /**
     * Gets the AccessibleText associated with this object presenting text on
     * the display.
     *
    ...
    /**
     * Gets the AccessibleEditableText associated with this object presenting
     * editable text on the display.
     *
    ...
    /**
     * Gets the AccessibleIcons associated with an object that has one or more
     * associated icons
     *
     * @return an array of AccessibleIcon if supported by object; otherwise
     *         return null
     * @see AccessibleIcon
     * @since 1.3
     */
     ...
     *
     * @return an AccessibleRelationSet if supported by object; otherwise return
     *         null
     ...
     * are not equal and the PropertyChangeEvent listener list is not empty,
     * then fire a PropertyChange event to each listener. In general, this is
     * for use by the Accessible objects themselves and should not be called by
