 ...
 * connection pooling component. A connection pooling component will usually be
 * provided by a JDBC driver vendor or another system software vendor. A JDBC
 * driver notifies a <code>ConnectionEventListener</code> object when an
 * application is finished using a pooled connection with which the listener has
 * registered. The notification occurs after the application calls the method
 * <code>close</code> on its representation of a <code>PooledConnection</code>
 * object. A <code>ConnectionEventListener</code> is also notified when a
 * connection error occurs due to the fact that the
 * <code>PooledConnection</code> is unfit for future use---the server has
 * crashed, for example. The listener is notified by the JDBC driver just before
 * the driver throws an <code>SQLException</code> to the application using the
 * <code>PooledConnection</code> object.
 *
 * @since 1.4
 */
