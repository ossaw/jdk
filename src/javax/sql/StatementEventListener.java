 ...
 * mechanism for enabling the pooling of <code>PreparedStatements</code> and to
 * specify the size of the statement pool. However, there was no way for a
 * driver to notify an external statement pool when a
 * <code>PreparedStatement</code> becomes invalid. For some databases, a
 * statement becomes invalid if a DDL operation is performed that affects the
 * table. For example an application may create a temporary table to do some
 * work on the table and then destroy it. It may later recreate the same table
 * when it is needed again. Some databases will invalidate any prepared
 * statements that reference the temporary table when the table is dropped.
 * <p>
 * Similar to the methods defined in the <code>ConnectionEventListener</code>
 * interface, the driver will call the
 * <code>StatementEventListener.statementErrorOccurred</code> method prior to
 * throwing any exceptions when it detects a statement is invalid. The driver
 * will also call the <code>StatementEventListener.statementClosed</code> method
 * when a <code>PreparedStatement</code> is closed.
 * <p>
 * Methods which allow a component to register a StatementEventListener with a
 * <code>PooledConnection</code> have been added to the
 * <code>PooledConnection</code> interface.
 * <p>
 * 
 * @since 1.6
 */
