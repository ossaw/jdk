 ...
 * populate itself with rows of data. A reader (an object implementing the
 * <code>RowSetReader</code> interface) may be registered with a
 * <code>RowSet</code> object that supports the reader/writer paradigm. When the
 * <code>RowSet</code> object's <code>execute</code> method is called, it in
 * turn calls the reader's <code>readData</code> method.
 *
 * @since 1.4
 */
...
public interface RowSetReader {
	 ...
	 * method <code>execute</code> will cause an <code>SQLException</code> to be
	 * thrown because <code>execute</code> may not be called recursively. Also,
	 * when a reader invokes <code>RowSet</code> methods, no listeners are
	 * notified; that is, no <code>RowSetEvent</code> objects are generated and
	 * no <code>RowSetListener</code> methods are invoked. This is true because
	 ...
	 *                if a database access error occurs or this method invokes
