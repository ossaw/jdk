 ...
 * makes available for composing an application. Because a
 * <code>JdbcRowSet</code> is a connected rowset, that is, it continually
 ...
 * <code>JdbcRowSet</code> can simply take calls invoked on it and in turn call
 * them on its <code>ResultSet</code> object. As a consequence, a result set
 * can, for example, be a component in a Swing application.
 * <P>
 * Another advantage of a <code>JdbcRowSet</code> object is that it can be used
 * to make a <code>ResultSet</code> object scrollable and updatable. All
 * <code>RowSet</code> objects are by default scrollable and updatable. If the
 * driver and database being used do not support scrolling and/or updating of
 * result sets, an application can populate a <code>JdbcRowSet</code> object
 ...
 * constructor. A new instance is initialized with default values, which can be
 * set with new values as needed. A new instance is not really functional until
 * its <code>execute</code> method is called. In general, this method does the
 * following:
 * <UL>
 ...
 * </UL>
 * If the <code>execute</code> method is successful, it will set the appropriate
 * private <code>JdbcRowSet</code> fields with the following:
 * <UL>
 * <LI>a <code>Connection</code> object -- the connection between the rowset and
 * the database
 * <LI>a <code>PreparedStatement</code> object -- the query that produces the
 * result set
 * <LI>a <code>ResultSet</code> object -- the result set that the rowset's
 * command produced and that is being made, in effect, a JavaBeans component
 * </UL>
 ...
 * <code>execute</code> and <code>close</code> may be called on the rowset. All
 * other public methods will throw an exception.
 * <P>
 * Before calling the <code>execute</code> method, however, the command and
 * properties needed for establishing a connection must be set. The following
 * code fragment creates a <code>JdbcRowSetImpl</code> object, sets the command
 * and connection properties, sets the placeholder parameter, and then invokes
 ...
 * <code>TITLES</code> where the type of book is biography. At this point,
 * operations called on <code>jrs</code> will affect the rows in the result set,
 * which is effectively a JavaBeans component.
 * <P>
 * The implementation of the <code>RowSet</code> method <code>execute</code> in
 * the <code>JdbcRowSet</code> reference implementation differs from that in the
 * <code>CachedRowSet</code>&trade; reference implementation to account for the
 * different requirements of connected and disconnected <code>RowSet</code>
 * objects.
 * <p>
 *
...
public interface JdbcRowSet extends RowSet, Joinable {
	 ...
	 *         none
	 * @throws SQLException
	 *             if this method is called on a closed <code>JdbcRowSet</code>
	 *             object
	 * @see RowSetWarning
	 */
	 ...
	 *             within this <code>JdbcRowSet</code> is in auto-commit mode
	 ...
	 * 
	 * @param autoCommit
	 *            {@code true} to enable auto-commit; {@code false} to disable
	 *            auto-commit
	 * @throws SQLException
	 *             if a database access error occurs
