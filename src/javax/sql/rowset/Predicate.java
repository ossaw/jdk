 ...
 * <code>FilteredRowSet</code> object consumes implementations of this interface
 * and enforces the constraints defined in the implementation of the method
 * <code>evaluate</code>. A <code>FilteredRowSet</code> object enforces the
 ...
 * filters definitions. By specifying a standard means and mechanism for a range
 * of filters to be defined and deployed with both the reference and vendor
 * implementations of the <code>FilteredRowSet</code> interface, this allows for
 * a flexible and application motivated implementations of
...
public interface Predicate {
	 ...
	 * internal methods (not public) that control the <code>RowSet</code>
	 * object's cursor moving from row to the next. In addition, if this
	 * internal method moves the cursor onto a row that has been deleted, the
	 * internal method will continue to ove the cursor until a valid row is
	 * found.
	 * 
	 * @param rs
	...
	/**
	 * This method is called by a <code>FilteredRowSet</code> object to check
	 * whether the value lies between the filtering criterion (or criteria if
	 * multiple constraints exist) set using the <code>setFilter()</code>
	 * method.
	 * <P>
	...
	/**
	 * This method is called by the <code>FilteredRowSet</code> object to check
	 * whether the value lies between the filtering criteria set using the
	 * setFilter method.
	 * <P>
	 * The <code>FilteredRowSet</code> object will use this method internally
	 * while inserting new rows to a <code>FilteredRowSet</code> instance.
