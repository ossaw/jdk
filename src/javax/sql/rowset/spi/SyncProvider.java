 ...
 * disconnected <code>RowSet</code> objects. A <code>SyncProvider</code>
 * implementation is a class that extends the <code>SyncProvider</code> abstract
 * class.
 * <P>
 * A <code>SyncProvider</code> implementation is identified by a unique ID,
 * which is its fully qualified class name. This name must be registered with
 * the <code>SyncFactory</code> SPI, thus making the implementation available to
 * all <code>RowSet</code> implementations. The factory mechanism in the
 * reference implementation uses this name to instantiate the implementation,
 * which can then provide a <code>RowSet</code> object with its reader (a
 * <code>javax.sql.RowSetReader</code> object) and its writer (a
 ...
 * <code>RIOptimisticProvider</code> and <code>RIXMLProvider</code>. The
 * <code>RIOptimisticProvider</code> can set any <code>RowSet</code>
 * implementation with a <code>RowSetReader</code> object and a
 * <code>RowSetWriter</code> object. However, only the
 * <code>RIXMLProvider</code> implementation can set an <code>XmlReader</code>
 * object and an <code>XmlWriter</code> object. A <code>WebRowSet</code> object
 ...
 * <UL>
 * <li>The name for a <code>SyncProvider</code> implementation is its fully
 * qualified class name.
 * <li>It is recommended that vendors supply a <code>SyncProvider</code>
 * implementation in a package named <code>providers</code>.
 * </UL>
 * <p>
 ...
 * implementation with the <code>SyncFactory</code> static instance.
 * 
 * <PRE>
 ...
 * with Oracle Corporation by sending email to jdbc@sun.com. Oracle will
 * maintain a database listing the available <code>SyncProvider</code>
 * implementations for use with compliant <code>RowSet</code> implementations.
 * This database will be similar to the one already maintained to list available
 * JDBC drivers.
 * <P>
 ...
 * synchronization providers are always available to the Java platform. If no
 * other pluggable synchronization providers have been correctly registered, the
 * <code>SyncFactory</code> will automatically generate an instance of the
 * default <code>SyncProvider</code> reference implementation. Thus, in the
 * preceding code fragment, if no implementation named
 ...
 * undefined behavior for all disconnected <code>RowSet</code> implementations
 * and their designated <code>SyncProvider</code> implementations. Not defining
 * the behavior when such violations occur offers greater flexibility for a
 * <code>SyncProvider</code> implementation to determine its own best course of
 * action.
 * <p>
 * A <code>SyncProvider</code> implementation may choose to implement a specific
 * handler to handle a subset of query violations. However if an original query
 * violation or a more general data source constraint violation is not handled
 ...
 * it is possible for an update to be performed to an underlying view, such an
 * update requires additional metadata, which may vary. The
 * <code>SyncProvider</code> class provides two constants to indicate whether an
 * implementation supports updating an SQL <code>VIEW</code>.
 * <ul>
 * <li><code><b>NONUPDATABLE_VIEW_SYNC</b></code> - Indicates that a
 * <code>SyncProvider</code> implementation does not support synchronization
 * with an SQL <code>VIEW</code> as the underlying source of data for the
 * <code>RowSet</code> object.
 * <li><code><b>UPDATABLE_VIEW_SYNC</b></code> - Indicates that a
 * <code>SyncProvider</code> implementation supports synchronization with an SQL
 * <code>VIEW</code> as the underlying source of data.
 * </ul>
 * <P>
 ...
 * <code>SyncProvider</code> objects may be implemented to perform
 * synchronization between a <code>RowSet</code> object and its underlying data
 * source with varying degrees of of care. The first group of constants indicate
 * how synchronization is handled. For example, <code>GRADE_NONE</code>
 * indicates that a <code>SyncProvider</code> object will not take any care to
 * see what data is valid and will simply write the <code>RowSet</code> data to
 * the data source. <code>GRADE_MODIFIED_AT_COMMIT</code> indicates that the
 * provider will check only modified data for validity. Other grades check all
 * data for validity or set locks when data is modified or loaded.
 * <OL>
...
public abstract class SyncProvider {
	 ...
	 *
	 * @return a <code>String</code> object with the fully qualified class name
	 *         of this <code>SyncProvider</code> object
	 */
	public abstract String getProviderID();

	/**
	 * Returns a <code>javax.sql.RowSetReader</code> object, which can be used
	 * to populate a <code>RowSet</code> object with data.
	 *
	...
	/**
	 * Returns a <code>javax.sql.RowSetWriter</code> object, which can be used
	 * to write a <code>RowSet</code> object's data back to the underlying data
	 * source.
	 *
	 ...
	 * <code>RowSet</code> object can expect from this <code>SyncProvider</code>
	 * object.
	 *
	 ...
	 * <i>datasource_lock</i>. This should cause the <code>SyncProvider</code>
	 * to adjust its behavior by increasing or decreasing the level of optimism
	 ...
	 *         active in this <code>SyncProvider</code> object; one of the
	 *         following:
	 * 
	 *         <pre>
	 *           SyncProvider.DATASOURCE_NO_LOCK,
	...
	/**
	 * Returns whether this <code>SyncProvider</code> implementation can perform
	 * synchronization between a <code>RowSet</code> object and the SQL
	 * <code>VIEW</code> in the data source from which the <code>RowSet</code>
	 * object got its data.
	 *
	 ...
	 *         following: SyncProvider.UPDATABLE_VIEW_SYNC,
	...
	/*
	 * Standard description of synchronization grades that a SyncProvider could
	 * provide.
	 */
	...
	/**
	 * Indicates the most pessimistic synchronization grade with respect to the
	 * originating data source. A <code>SyncProvider</code> implementation
	 * returning this grade will lock the entire view and/or table affected by
	 * the original statement used to populate a <code>RowSet</code> object.
	 */
	public static final int GRADE_LOCK_WHEN_LOADED = 5;

	/**
	 * Indicates that no locks remain on the originating data source. This is
	 * the default lock setting for all <code>SyncProvider</code>
	 * implementations unless otherwise directed by a <code>RowSet</code>
	 * object.
	 */
	public static final int DATASOURCE_NO_LOCK = 1;

	/**
	 * Indicates that a lock is placed on the rows that are touched by the
	 * original SQL statement used to populate the <code>RowSet</code> object
	 * that is using this <code>SyncProvider</code> object.
	 */
	public static final int DATASOURCE_ROW_LOCK = 2;

	/**
	 * Indicates that a lock is placed on all tables that are touched by the
	 * original SQL statement used to populate the <code>RowSet</code> object
	 * that is using this <code>SyncProvider</code> object.
	 */
	...
	/**
	 * Indicates that a lock is placed on the entire data source that is the
	 * source of data for the <code>RowSet</code> object that is using this
	 * <code>SyncProvider</code> object.
	 */
	public static final int DATASOURCE_DB_LOCK = 4;

	/**
	 * Indicates that a <code>SyncProvider</code> implementation supports
	 * synchronization between a <code>RowSet</code> object and the SQL
	 * <code>VIEW</code> used to populate it.
	 */
	public static final int UPDATABLE_VIEW_SYNC = 5;

	/**
	 * Indicates that a <code>SyncProvider</code> implementation does <B>not</B>
	 * support synchronization between a <code>RowSet</code> object and the SQL
	 * <code>VIEW</code> used to populate it.
	 */
