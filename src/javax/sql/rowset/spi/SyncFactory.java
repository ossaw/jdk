 ...
 * <code>RowSet</code> objects. The <code>SyncProvider</code> instances in turn
 * provide the <code>javax.sql.RowSetReader</code> object the
 * <code>RowSet</code> object needs to populate itself with data and the
 * <code>javax.sql.RowSetWriter</code> object it needs to propagate changes to
 * its data back to the underlying data source.
 * <P>
 * Because the methods in the <code>SyncFactory</code> class are all static,
 * there is only one <code>SyncFactory</code> object per Java VM at any one
 * time. This ensures that there is a single source from which a
 * <code>RowSet</code> implementation can obtain its <code>SyncProvider</code>
 * implementation.
 * <h3>1.0 Overview</h3> The <code>SyncFactory</code> class provides an internal
 * registry of available synchronization provider implementations (
 * <code>SyncProvider</code> objects). This registry may be queried to determine
 * which synchronization providers are available. The following line of code
 * gets an enumeration of the providers currently registered.
 * 
 * <PRE>
 * java.util.Enumeration e = SyncFactory.getRegisteredProviders();
 * </PRE>
 * 
 * All standard <code>RowSet</code> implementations must provide at least two
 * providers:
 * <UL>
 * <LI>an optimistic provider for use with a <code>CachedRowSet</code>
 * implementation or an implementation derived from it
 ...
 * <code>SyncProvider</code> implementations <code>RIOptimisticProvider</code>
 * and <code>RIXmlProvider</code>, which satisfy this requirement.
 * <P>
 ...
 * <p>
 * Other methods let <code>RowSet</code> persistence providers be registered or
 * de-registered with the factory mechanism. This allows additional
 * synchronization provider implementations to be made available to
 * <code>RowSet</code> objects at run time.
 * <p>
 * Applications can apply a degree of filtering to determine the level of
 * synchronization that a <code>SyncProvider</code> implementation offers. The
 * following criteria determine whether a provider is made available to a
 * <code>RowSet</code> object:
 * <ol>
 * <li>If a particular provider is specified by a <code>RowSet</code> object,
 * and the <code>SyncFactory</code> does not contain a reference to this
 * provider, a <code>SyncFactoryException</code> is thrown stating that the
 ...
 * The name of the provider is supplied on the command line, which will add the
 * provider to the system properties. For example:
 * 
 * <PRE>
 ...
 * The reference implementation is targeted to ship with J2SE 1.5, which will
 * include an additional resource file that may be edited by hand. Here is an
 * example of the properties file included in the reference implementation:
 * 
 * <PRE>
 ...
 * <code>SyncProvider</code> implementations that it contains. A developer or
 * vendor can add other implementations to this file. For example, here is a
 * possible addition:
 * 
 * <PRE>
 ...
 * Available providers can be registered on a JNDI context, and the
 * <code>SyncFactory</code> will attempt to load <code>SyncProvider</code>
 * implementations from that JNDI context. For example, the following code
 * fragment registers a provider implementation on a JNDI context. This is
 * something a deployer would normally do. In this example,
 * <code>MyProvider</code> is being registered on a CosNaming namespace, which
 * is the namespace used by J2EE resources.
 * 
 * <PRE>
 *    import javax.naming.*;
 *
 ...
 * <code>SyncFactory</code> instance verifies that the requested provider is a
 * valid extension of the <code>SyncProvider</code> abstract class and then
 * gives it to the <code>RowSet</code> object. In the following code fragment, a
 * new <code>CachedRowSet</code> object is created and initialized with
...
public class SyncFactory {
    ...
    private static synchronized void initMapIfNecessary() throws SyncFactoryException {
        ...
        if (implementations == null) {
            ...
            try {
                 ...
                 * Always available
                 */
                ROWSET_PROPERTIES = "javax" + strFileSep + "sql" + strFileSep + "rowset" + strFileSep
                        + "rowset.properties";
                ...
                try {
                    AccessController.doPrivileged((PrivilegedExceptionAction<Void>) () -> {
                        try (InputStream stream = (cl == null) ? ClassLoader.getSystemResourceAsStream(
                                ROWSET_PROPERTIES) : cl.getResourceAsStream(ROWSET_PROPERTIES)) {
                            if (stream == null) {
                                throw new SyncFactoryException("Resource " + ROWSET_PROPERTIES
                                        + " not found");
                            }
    ...
    private static String[] getPropertyNames(boolean append, String propertyIndex) {
        String dot = ".";
        String[] propertyNames = new String[] { SyncFactory.ROWSET_SYNC_PROVIDER,
                SyncFactory.ROWSET_SYNC_VENDOR, SyncFactory.ROWSET_SYNC_PROVIDER_VERSION };
        if (append) {
     ...
     * @see SecurityManager#checkPermission
     */
    public static synchronized void setJNDIContext(javax.naming.Context ctx) throws SyncFactoryException {
        SecurityManager sec = System.getSecurityManager();
        if (sec != null) {
            sec.checkPermission(SET_SYNCFACTORY_PERMISSION);
        }
     ...
     */
    private static void enumerateBindings(NamingEnumeration<?> bindings, Properties properties)
        ...
        try {
            ...
            while (bindings.hasMore()) {
                ...
                if (syncProviderObj) {
                    ...
                    properties.put(SyncFactory.ROWSET_SYNC_PROVIDER, sync.getProviderID());
