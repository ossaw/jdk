 ...
 * decide what should be done when a synchronization conflict occurs. Although
 * it is not mandatory for applications to resolve synchronization conflicts
 * manually, this framework provides the means to delegate to the application
 * when conflicts arise.
 * <p>
 * Note that a conflict is a situation where the <code>RowSet</code> object's
 * original values for a row do not match the values in the data source, which
 * indicates that the data source row has been modified since the last
 ...
 * of the <code>JdbcRowSet</code> interface) or a connected <code>RowSet</code>
 * object (an implementation of the <code>CachedRowSet</code> interface or one
 * of its subinterfaces). For information on the subinterfaces, see the
 * <a href="../package-summary.html"><code>javax.sql.rowset</code></a> package
 * description. The reference implementation for <code>SyncResolver</code>
 * implements the <code>CachedRowSet</code> interface, but other implementations
 * may choose to implement the <code>JdbcRowSet</code> interface to satisfy
 * particular needs.
 * <P>
 * After an application has attempted to synchronize a <code>RowSet</code>
 * object with the data source (by calling the <code>CachedRowSet</code> method
 * <code>acceptChanges</code>), and one or more conflicts have been found, a
 * rowset's <code>SyncProvider</code> object creates an instance of
 * <code>SyncResolver</code>. This new <code>SyncResolver</code> object has the
 * same number of rows and columns as the <code>RowSet</code> object that was
 * attempting the synchronization. The <code>SyncResolver</code> object contains
 ...
 * <code>acceptChanges</code> will throw this exception, which the application
 * can then catch and use to retrieve the <code>SyncResolver</code> object it
 * contains. The following code snippet uses the
 ...
 * </PRE>
 * <P>
 * With <i>resolver</i> in hand, an application can use it to get the
 * information it contains about the conflict or conflicts. A
 * <code>SyncResolver</code> object such as <i>resolver</i> keeps track of the
 * conflicts for each row in which there is a conflict. It also places a lock on
 * the table or tables affected by the rowset's command so that no more
 ...
 * delete, or insert) that a <code>RowSet</code> object was attempting to
 * perform when a conflict was discovered, and the fourth indicates that there
 * is no conflict. These constants are the possible return values when a
 * <code>SyncResolver</code> object calls the method <code>getStatus</code>.
 * 
 * <PRE>
 * {
 *     &#64;code
 ...
 * </PRE>
 *
 * <h3>The value in the data source that caused a conflict</h3> A conflict
 * exists when a value that a <code>RowSet</code> object has changed and is
 * attempting to write to the data source has also been changed in the data
 * source since the last synchronization. An application can call the
 * <code>SyncResolver</code> method <code>getConflictValue</code > to retrieve
 * the value in the data source that is the cause of the conflict because the
 * values in a <code>SyncResolver</code> object are the conflict values from the
 * data source.
 * 
 * <PRE>
 ...
 * </PRE>
 * 
 * Note that the column in <i>resolver</i> can be designated by the column
 * number, as is done in the preceding line of code, or by the column name.
 * <P>
 ...
 * which value should be persisted in the data source. The application then
 * calls the <code>SyncResolver</code> method <code>setResolvedValue</code>,
 * which sets the value to be persisted in the <code>RowSet</code> object and
 ...
 * <code>RowSet</code> object that is to be set with the given value. The column
 * number can also be used to designate the column.
 * <P>
 ...
 * application can use the <code>RowSet</code> method <code>next</code> to get
 * to each row and then call the <code>SyncResolver</code> method
 * <code>getStatus</code> to see if the row contains a conflict. In a row with
 * one or more conflicts, the application can iterate through the columns to
 * find any non-null values, which will be the values from the data source that
 * are in conflict.
 * <P>
 * To make it easier to navigate a <code>SyncResolver</code> object, especially
 * when there are large numbers of rows with no conflicts, the
 * <code>SyncResolver</code> interface defines the methods
 * <code>nextConflict</code> and <code>previousConflict</code>, which move only
 * to rows that contain at least one conflict value. Then an application can
 ...
 * <code>try</code> block, <i>crs</i> calls the method
 * <code>acceptChanges</code>, passing it the <code>Connection</code> object
 * <i>con</i>. If there are no conflicts, the changes in <i>crs</i> are simply
 * written to the data source. However, if there is a conflict, the method
 * <code>acceptChanges</code> throws a <code>SyncProviderException</code>
 * object, and the <code>catch</code> block takes effect. In this example, which
 * illustrates one of the many ways a <code>SyncResolver</code> object can be
 * used, the <code>SyncResolver</code> method <code>nextConflict</code> is used
 * in a <code>while</code> loop. The loop will end when
 * <code>nextConflict</code> returns <code>false</code>, which will occur when
 * there are no more conflict rows in the <code>SyncResolver</code> object
 * <i>resolver</i>. In This particular code fragment, <i>resolver</i> looks for
 * rows that have update conflicts (rows with the status
 * <code>SyncResolver.UPDATE_ROW_CONFLICT</code>), and the rest of this code
 * fragment executes only for rows where conflicts occurred because <i>crs</i>
 * was attempting an update.
 * <P>
 * After the cursor for <i>resolver</i> has moved to the next conflict row that
 * has an update conflict, the method <code>getRow</code> indicates the number
 * of the current row, and the cursor for the <code>CachedRowSet</code> object
 * <i>crs</i> is moved to the comparable row in <i>crs</i>. By iterating through
 * the columns of that row in both <i>resolver</i> and <i>crs</i>, the
 * conflicting values can be retrieved and compared to decide which one should
 * be persisted. In this code fragment, the value in <i>crs</i> is the one set
 * as the resolved value, which means that it will be used to overwrite the
 * conflict value in the data source.
 *
 * <PRE>
 * {@code
 *     try {
