...
/**
 * Indicates an error with the <code>SyncProvider</code> mechanism. This
 * exception is created by a <code>SyncProvider</code> abstract class extension
 * if it encounters violations in reading from or writing to the originating
 * data source.
 * <P>
 * If it is implemented to do so, the <code>SyncProvider</code> object may also
 * create a <code>SyncResolver</code> object and either initialize the
 * <code>SyncProviderException</code> object with it at construction time or set
 * it with the <code>SyncProvider</code> object at a later time.
 * <P>
 * The method <code>acceptChanges</code> will throw this exception after the
 * writer has finished checking for conflicts and has found one or more
 * conflicts. An application may catch a <code>SyncProviderException</code>
 * object and call its <code>getSyncResolver</code> method to get its
 * <code>SyncResolver</code> object. See the code fragment in the interface
 * comment for <a href="SyncResolver.html"><code>SyncResolver</code></a> for an
 * example. This <code>SyncResolver</code> object will mirror the
 * <code>RowSet</code> object that generated the exception, except that it will
 * contain only the values from the data source that are in conflict. All other
 * values in the <code>SyncResolver</code> object will be <code>null</code>.
 * <P>
 * The <code>SyncResolver</code> object may be used to examine and resolve each
 * conflict in a row and then go to the next row with a conflict to repeat the
 * procedure.
 * <P>
 * A <code>SyncProviderException</code> object may or may not contain a
 * description of the condition causing the exception. The inherited method
 * <code>getMessage</code> may be called to retrieve the description if there is
 * one.
 *
