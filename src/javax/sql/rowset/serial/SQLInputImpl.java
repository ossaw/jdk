 ...
 * values that are the attributes of a UDT.
 * <p>
 * This class is used by the driver behind the scenes when the method
 * <code>getObject</code> is called on an SQL structured or distinct type that
 * has a custom mapping; a programmer never invokes <code>SQLInputImpl</code>
 * methods directly. They are provided here as a convenience for those who write
 ...
 * possible to read the values in an <code>SQLInputImpl</code> object.
 * <P>
 * The method <code>wasNull</code> is used to determine whether the the last
 * value read was SQL <code>NULL</code>.
 * <P>
 * When the method <code>getObject</code> is called with an object of a class
 * implementing the interface <code>SQLData</code>, the JDBC driver calls the
 * method <code>SQLData.getSQLType</code> to determine the SQL type of the UDT
 * being custom mapped. The driver creates an instance of
 * <code>SQLInputImpl</code>, populating it with the attributes of the UDT. The
 * driver then passes the input stream to the method
 * <code>SQLData.readSQL</code>, which in turn calls the
 * <code>SQLInputImpl</code> reader methods to read the attributes from the
 * input stream.
 * 
 * @since 1.5
...
public class SQLInputImpl implements SQLInput {
	...
	/**
	 * The current index into the array of SQL structured type attributes that
	 * will be read from this <code>SQLInputImpl</code> object and mapped to the
	 * fields of a class in the Java programming language.
	 */
	private int idx;

	/**
	 * The array of attributes to be read from this stream. The order of the
	 * attributes is the same as the order in which they were listed in the SQL
	 * definition of the UDT.
	 */
	private Object attrib[];

	/**
	 * The type map to use when the method <code>readObject</code> is invoked.
	 * This is a <code>java.util.Map</code> object in which there may be zero or
	 * more entries. Each entry consists of the fully qualified name of a UDT
	 * (the value to be mapped) and the <code>Class</code> object for a class
	 * that implements <code>SQLData</code> (the Java class that defines how the
	 * UDT will be mapped).
	 */
	private Map<String, Class<?>> map;

	/**
	 * Creates an <code>SQLInputImpl</code> object initialized with the given
	 * array of attributes and the given type map. If any of the attributes is a
	 * UDT whose name is in an entry in the type map, the attribute will be
	 * mapped according to the corresponding <code>SQLData</code>
	 * implementation.
	 *
	 * @param attributes
	 *            an array of <code>Object</code> instances in which each
	 *            element is an attribute of a UDT. The order of the attributes
	 *            in the array is the same order in which the attributes were
	 ...
	 *            <code>String</code> giving the fully qualified name of the UDT
	 ...
	 *
	 * @return the next value in the input stream as an <code>Object</code> in
	 *         the Java programming language
	 * @throws SQLException
	 *             if the read position is located at an invalid position or if
	 *             there are no further values in the stream
	 */
	 ...
	 * returned type is the expected type; this responsibility is delegated to
	 * the UDT mapping as defined by a <code>SQLData</code> implementation.
	 * <p>
	 * 
	 * @return the next attribute in this <code>SQLInputImpl</code> object; if
	 *         the value is <code>SQL NULL</code>, return <code>null</code>
	 * @throws SQLException
	 *             if the read position is located at an invalid position or if
	 *             there are no further values in the stream.
	 */
	...
	/**
	 * Retrieves the next attribute in this <code>SQLInputImpl</code> as a
	 ...
	 * <P>
	 * This method does not perform type-safe checking to determine if the
	 * returned type is the expected type as this responsibility is delegated to
	 * the UDT mapping as implemented by a <code>SQLData</code> implementation.
	 *
	...
	/**
	 * Returns the next attribute in this <code>SQLInputImpl</code> object as a
	 ...
	 * as an <code>Object</code> in the Java programming language. The actual
	 * type of the object returned is determined by the default mapping of SQL
	 * types to types in the Java programming language unless there is a custom
	 * mapping, in which case the type of the object returned is determined by
	 * this stream's type map.
	 * <P>
	 ...
	 * this method returns <code>null</code>. If the datum is an SQL structured
	 * or distinct type with a custom mapping, this method determines the SQL
	 * type of the datum at the head of the stream, constructs an object of the
	 * appropriate class, and calls the method <code>SQLData.readSQL</code> on
	 * that object. The <code>readSQL</code> method then calls the appropriate
	 * <code>SQLInputImpl.readXXX</code> methods to retrieve the attribute
	 * values from the stream.
	 *
	 * @return the value at the head of the stream as an <code>Object</code> in
	 *         the Java programming language; <code>null</code> if the value is
	 *         SQL <code>NULL</code>
	 * @throws SQLException
	 *             if the read position is located at an invalid position; or if
	 *             there are no further values in the stream.
	 ...
	 *         value at the head of the stream; if the value is
	 *         <code>SQL NULL</code> return <code>null</code>
	 * @throws SQLException
	 *             if the read position is located at an invalid position; or if
	 *             there are no further values in the stream.
	 */
	 ...
	 * <code>SQLInputImpl</code> object as a <code>Blob</code> object in the
	 ...
	 *         value at the head of this stream; if the value is
	 *         <code>SQL NULL</code>, return <code>null</code>
	 * @throws SQLException
	 *             if the read position is located at an invalid position; or if
	 *             there are no further values in the stream.
	 ...
	 * <code>SQLInputImpl</code> object as a <code>Clob</code> object in the
	 * Java programming language.
	 * <P>
	 * This method does not perform type-safe checking to determine if the
	 * returned type is the expected type as this responsibility is delegated to
	...
	/**
	 * Reads an SQL <code>ARRAY</code> value from the stream and returns it as
	 * an <code>Array</code> object in the Java programming language.
	 * <P>
	 * This method does not perform type-safe checking to determine if the
	 * returned type is the expected type as this responsibility is delegated to
	 * the UDT mapping as implemented by a <code>SQLData</code> implementation.
	 ...
	 *         <code>ARRAY</code> value at the head of the stream; * if the
	 *         value is <code>SQL NULL</code>, return <code>null</code>
	 * @throws SQLException
	 *             if the read position is located at an invalid position; or if
	 *             there are no further values in the stream.
	 * 
	 ...
	 *         will return false
	 * @throws SQLException
	 *             if an error occurs determining the last value read was a
	 ...
	 * as an <code>URL</code> object in the Java programming language.
	 * <P>
	 * This method does not perform type-safe checking to determine if the
	 * returned type is the expected type as this responsibility is delegated to
	 * the UDT mapping as implemented by a <code>SQLData</code> implementation.
	 ...
	 *         <code>DATALINK</code> value at the head of the stream; * if the
	 *         value is <code>SQL NULL</code>, return <code>null</code>
	 * @throws SQLException
	 *             if the read position is located at an invalid position; or if
	 *             there are no further values in the stream.
	 ...
	 *
	 * @return a <code>SQLXML</code> object representing data of the SQL
	 ...
	 *
	 * @return a <code>RowId</code> object representing data of the SQL
