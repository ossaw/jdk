...
/**
 * A serialized version of an <code>Array</code> object, which is the mapping in
 * the Java programming language of an SQL <code>ARRAY</code> value.
 * <P>
 * The <code>SerialArray</code> class provides a constructor for creating a
 * <code>SerialArray</code> instance from an <code>Array</code> object, methods
 * for getting the base type and the SQL name for the base type, and methods for
 * copying all or part of a <code>SerialArray</code> object.
 * <P>
 * Note: In order for this class to function correctly, a connection to the data
 * source must be available in order for the SQL <code>Array</code> object to be
 * materialized (have all of its elements brought to the client server) if
 * necessary. At this time, logical pointers to the data in the data source,
...
public class SerialArray implements Array, Serializable, Cloneable {
    ...
    /**
     * A serialized array in which each element is an <code>Object</code> in the
     * Java programming language that represents an element in the SQL
     ...
     * <code>ARRAY</code> value that this <code>SerialArray</code> object
     * represents.
     * 
     * @serial
     */
    ...
    /**
     * The number of elements in this <code>SerialArray</code> object, which is
     * also the number of elements in the SQL <code>ARRAY</code> value that this
     ...
     * <P>
     * This method does custom mapping if the array elements are a UDT and the
     * given type map has an entry for that UDT. Custom mapping is recursive,
     * meaning that if, for instance, an element of an SQL structured type is an
     * SQL structured type that itself has an element that is an SQL structured
     * type, each structured type that has a custom mapping will be mapped
     * according to the given type map.
     * <P>
     * The new <code>SerialArray</code> object contains the same elements as the
     * <code>Array</code> object from which it is built, except when the base
     * type is the SQL type <code>STRUCT</code>, <code>ARRAY</code>,
     * <code>BLOB</code>, <code>CLOB</code>, <code>DATALINK</code> or
     * <code>JAVA_OBJECT</code>. In this case, each element in the new
     * <code>SerialArray</code> object is the appropriate serialized form, that
     * is, a <code>SerialStruct</code>, <code>SerialArray</code>,
     * <code>SerialBlob</code>, <code>SerialClob</code>,
     * <code>SerialDatalink</code>, or <code>SerialJavaObject</code> object.
     * <P>
     * Note: (1) The <code>Array</code> object from which a
     * <code>SerialArray</code> object is created must have materialized the SQL
     * <code>ARRAY</code> value's data on the client before it is passed to the
     * constructor. Otherwise, the new <code>SerialArray</code> object will
     * contain no data.
     * <p>
     * Note: (2) If the <code>Array</code> contains
     * <code>java.sql.Types.JAVA_OBJECT</code> types, the
     * <code>SerialJavaObject</code> constructor is called where checks are made
     * to ensure this object is serializable.
     * <p>
     * Note: (3) The <code>Array</code> object supplied to this constructor
     * cannot return <code>null</code> for any <code>Array.getArray()</code>
     * methods. <code>SerialArray</code> cannot serialize null array values.
     *
     * @param array
     ...
     * <code>Array</code> object.
     * <P>
     * This constructor does not do custom mapping. If the base type of the
     * array is an SQL structured type and custom mapping is desired, the
     * constructor <code>SerialArray(Array array, Map map)</code> should be
     * used.
     * <P>
     * The new <code>SerialArray</code> object contains the same elements as the
     * <code>Array</code> object from which it is built, except when the base
     * type is the SQL type <code>BLOB</code>, <code>CLOB</code>,
     * <code>DATALINK</code> or <code>JAVA_OBJECT</code>. In this case, each
     * element in the new <code>SerialArray</code> object is the appropriate
     * serialized form, that is, a <code>SerialBlob</code>,
     * <code>SerialClob</code>, <code>SerialDatalink</code>, or
     ...
     * contain no data.
     * <p>
     * Note: (2) The <code>Array</code> object supplied to this constructor
     * cannot return <code>null</code> for any <code>Array.getArray()</code>
     * methods. <code>SerialArray</code> cannot serialize <code>null</code>
     * array values.
     *
     * @param array
     *        the <code>Array</code> object to be serialized
    ...
    /**
     * Returns a new array that is a copy of this <code>SerialArray</code>
     * object, using the given type map for the custom mapping of each element
     * when the elements are SQL UDTs.
     * <P>
     * This method does custom mapping if the array elements are a UDT and the
     * given type map has an entry for that UDT. Custom mapping is recursive,
     * meaning that if, for instance, an element of an SQL structured type is an
     ...
     * <code>SerialArray</code> object, starting with the element at the given
     ...
     * object. The <code>int</code> returned is one of the constants in the
    ...
    /**
     * Retrieves a <code>ResultSet</code> object holding the elements of the
     * subarray that starts at index <i>index</i> and contains up to
     * <i>count</i> successive elements. This method uses the connection's type
     * map to map the elements of the array if the map contains an entry for the
    ...
    /**
     * Retrieves a <code>ResultSet</code> object that contains all of the
     * elements of the SQL <code>ARRAY</code> value represented by this
     * <code>SerialArray</code> object. This method uses the specified map for
     * type map customizations unless the base type of the array does not match
     * a user-defined type (UDT) in <i>map</i>, in which case it uses the
     ...
     *        for the <code>SQLData</code> implementation that defines how
     *        the UDT is to be mapped
     * @return a <code>ResultSet</code> object containing all of the elements in
    ...
    /**
     * Retrieves a <code>ResultSet</code> object that contains all of the
     * elements in the <code>ARRAY</code> value that this
     * <code>SerialArray</code> object represents. If appropriate, the elements
     * of the array are mapped using the connection's type map; otherwise, the
     ...
     * at Retrieves a <code>ResultSet</code> object that contains a subarray of
     * the elements in this <code>SerialArray</code> object, starting at index
     * <i>index</i> and containing up to <i>count</i> successive elements. This
     * method uses the specified map for type map customizations unless the base
     * type of the array does not match a user-defined type (UDT) in <i>map</i>,
     * in which case it uses the standard mapping. This version of the method
     * <code>getResultSet</code> uses either the given type map or the standard
     ...
     * SerialArray} object whose elements are identical to this object's
     * elements
     *
     * @param obj
     *        The object to compare this {@code SerialArray} against
    ...
    public boolean equals(Object obj) {
        ...
        if (obj instanceof SerialArray) {
            ...
            return baseType == sa.baseType && baseTypeName.equals(sa.baseTypeName) && Arrays.equals(elements,
                    sa.elements);
        }
        return false;
    }
     ...
     * {@code SerialArray} object is computed using the hash codes of the
     * elements of the {@code SerialArray} object
     *
     ...
     * reference to a clone of the underlying objects array, not a reference to
     * the original underlying object array of this {@code SerialArray} object.
     *
