 ...
 * URI:
 * <ul>
 * <li><a href="http://java.sun.com/xml/ns/jdbc/webrowset.xsd">http://java.sun.
 * com/xml/ns/jdbc/webrowset.xsd</a></li>
 * </ul>
 ...
 * the {@code WebRowSet} schema uses specific SQL/XML Schema annotations, thus
 * ensuring greater cross platform inter-operability. This is an effort
 * currently under way at the ISO organization. The SQL/XML definition is
 * available at the following URI:
 * <ul>
 * <li><a href="http://standards.iso.org/iso/9075/2002/12/sqlxml">http://
 * standards.iso.org/iso/9075/2002/12/sqlxml</a></li>
 * </ul>
 ...
 * provider properties in addition to the more general {@code RowSet}
 * properties.</li>
 * <li>metadata - This describes the metadata associated with the tabular
 * structure governed by a {@code WebRowSet} object. The metadata described is
 * closely aligned with the metadata accessible in the underlying
 * {@code java.sql.ResultSet} interface.</li>
 * <li>data - This describes the original data (the state of data since the last
 * population or last synchronization of the {@code WebRowSet} object) and the
 * current data. By keeping track of the delta between the original data and the
 ...
 * {@code WebRowSet} object makes it possible to describe them in XML. The
 * metadata describing the various standard JavaBeans properties as defined in
 * the RowSet interface plus the standard properties defined in the
 * {@code CachedRowSet}&trade; interface provide key details that describe
 * WebRowSet properties. Outputting the WebRowSet object to XML using the
 * standard {@code writeXml} methods describes the internal properties as
 * follows:
 * 
 * <PRE>
 * {@code
 ...
 * detailed below. Note both columns are described between the
 ...
 * details how the contents of a {@code WebRowSet} object is described in XML.
 * Note, that this describes a {@code WebRowSet} object that has not undergone
 * any modifications since its instantiation. A {@code currentRow} tag is mapped
 * to each row of the table structure that the {@code WebRowSet} object
 * provides. A {@code columnValue} tag may contain either the {@code stringData}
 * or {@code binaryData} tag, according to the SQL type that the XML value is
 * mapping back to. The {@code binaryData} tag contains data in the Base64
 * encoding and is typically used for {@code BLOB} and {@code CLOB} type data.
 * 
 * <PRE>
 * {@code
 * <data>
 ...
 * </PRE>
 * 
 * <h4>2.2 State 2 - Deleting a Row</h4> Deleting a row in a {@code WebRowSet}
 * object involves simply moving to the row to be deleted and then calling the
 * method {@code deleteRow}, as in any other {@code RowSet} object. The
 * following two lines of code, in which <i>wrs</i> is a {@code WebRowSet}
 ...
 * </PRE>
 * 
 * <h4>2.3 State 3 - Inserting a Row</h4> A {@code WebRowSet} object can insert
 * a new row by moving to the insert row, calling the appropriate updater
 * methods for each column in the row, and then calling the method
 * {@code insertRow}.
 * 
 * <PRE>
 * {@code
 ...
 * the current row, which is why the method {@code next} moves the cursor to the
 * correct row. Calling the method {@code acceptChanges} writes the change to
 * the data source.
 *
 * <PRE>
 ...
 * </PRE>
 * 
 * <h4>2.4 State 4 - Modifying a Row</h4> Modifying a row produces specific XML
 * that records both the new value and the value that was replaced. The value
 * that was replaced becomes the original value, and the new value becomes the
 * current value. The following code moves the cursor to a specific row,
 * performs some modifications, and updates the row when complete.
 * 
 * <PRE>
 *{@code
 * wrs.absolute(5);
 ...
 * </PRE>
 * 
 * In XML, this is described by the {@code modifyRow} tag. Both the original and
 * new values are contained within the tag for original row tracking purposes.
 * 
 * <PRE>
 * {@code
 * <data>
