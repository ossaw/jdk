 ...
 * compute the {@linkplain #getSupportedOptions options},
 ...
 * {@link javax.annotation.processing.Processor Processor} contract for that
 * method is obeyed.
 *
...
public abstract class AbstractProcessor implements Processor {
	...
	/**
	 * If the processor class is annotated with {@link SupportedOptions}, return
	 * an unmodifiable set with the same set of strings as the annotation. If
	 * the class is not so annotated, an empty set is returned.
	 *
	 ...
	 * , return an unmodifiable set with the same set of strings as the
	 * annotation. If the class is not so annotated, an empty set is returned.
	 *
	 * @return the names of the annotation types supported by this processor, or
			...
			if (isInitialized())
				processingEnv.getMessager().printMessage(Diagnostic.Kind.WARNING,
						"No SupportedAnnotationTypes annotation " + "found on "
								+ this.getClass().getName() + ", returning an empty set.");
			return Collections.emptySet();
		} else
	 ...
	 * return the source version in the annotation. If the class is not so
	 * annotated, {@link SourceVersion#RELEASE_6} is returned.
	 *
	 ...
	 * {@code processingEnv} field to the value of the {@code processingEnv}
	 * argument. An {@code
	 * IllegalStateException} will be thrown if this method is called more than
	 * once on the same object.
	 *
	 * @param processingEnv
	 *            environment to access facilities the tool framework provides
	 *            to the processor
